...
#define RTL83XX_INIT()		RTL8367R_init()
#elif defined(CONFIG_SW_83XX)
#define RTL83XX_INIT()		RTL83XX_init()
#endif
...
static unsigned int swcore_initialed = 0;
...
void Set_GPHYWB(unsigned int phyid, unsigned int page, unsigned int reg, unsigned int mask, unsigned int val)
{
	...
	for(; i<wphyid_end; i++)
	{
		wphyid = i;
		//change page 		
#if defined(CONFIG_RTL8198C) || defined(CONFIG_RTL8197F)
		if (i == 0)
...
#if defined(CONFIG_RTL8197F) && defined(CONFIG_8211F_SUPPORT)
int rp;
...
static void ckeck_link_speed(void){
	...
	for (rp = 0; rp < 2; rp++)
		rtl_mdio_read(P0_RG_PHYID, 1, &regData);

	if (regData & (1<<2) && !(prelink & (1<<2)) ){

		for (rp = 0; rp < 2; rp++)
			rtl_mdio_read(P0_RG_PHYID, 9, &regData1);
		
		for (rp = 0; rp < 2; rp++)
			rtl_mdio_read(P0_RG_PHYID, 10, &regData2);
		...
		if ( ((regData1>>8) & (regData2>>10)) & (1<<1)) { //force Giga
			...
			for (rp = 0; rp < 2; rp++)
				rtl_mdio_read(P0_RG_PHYID, 4, &regData1);
		
			for (rp = 0; rp < 2; rp++)
				rtl_mdio_read(P0_RG_PHYID, 5, &regData2);
...
static volatile unsigned int jiffies_t3=0;

static void timer3_interrupt(int num, void *ptr, struct pt_regs * reg)
{
//void mdc_timer_fn(unsigned long arg);
	REG32(BSP_TCIR) |= BSP_TC3IP; //TC3IP
//	mdc_timer_fn();
...
static struct irqaction irq_timer3 = {timer3_interrupt, 0, 10, "timer", NULL, NULL};                                   

//------------------------------------------------------------------------------
void timer3_init(unsigned long lexra_clock)
{
    /* Set timer mode and Enable timer */
    REG32(BSP_TCCNR) &= ~(BSP_TC3EN | BSP_TC3MODE_TIMER);	//using time3
	...
	REG32(BSP_TC3DATA) = (((SysClkRate / DIVISOR) / TICK_FREQ) + 1) <<4;
  ...
  // RTL8198
    ...
    REG32(BSP_IRR1) = 0x00000200; //timer3: IRR1 IRRIPtoCPU
      
    /* Enable timer interrupt */
    REG32(BSP_TCIR) |= BSP_TC3IE;
}
//------------------------------------------------------------------------------

void enable_t3_timer(void)
{
	extern long glexra_clock;     

	request_IRQ(10, &irq_timer3, NULL); //timer3
//	dprintf("timer init\n");
    timer3_init(glexra_clock);	
}
#endif

#ifdef CONFIG_RTL8197F
int Setting_RTL8197F_FEPHY(void)
{
	int i;
	unsigned int data;
...
#define GPIO_RESET_97FN			26		// GPIO_H2
#else
...
int init_p0(void)
{
	...
	if(gpio_simulate_mdc_mdio){
		...
		enable_t3_timer();
		REG32(PIN_MUX_SEL14) = (REG32(PIN_MUX_SEL14) & ~(0xf<<28)) | (0x2<<28);
		REG32(PEFGHCNR) &= ~(1<<GPIO_RESET_97FN);
		REG32(PEFGHDIR) |= (1<<GPIO_RESET_97FN);
		REG32(PEFGHDAT) &= ~(1<<GPIO_RESET_97FN);
		__mdelay(500);
		REG32(PEFGHDAT) |= (1<<GPIO_RESET_97FN);
		__mdelay(300);
...
//TODO:	REG32(0xb8000048) |= BIT(28);
	...
	if(gpio_simulate_mdc_mdio){
		rtl_mdio_read(P0_RG_PHYID, 4, &data );
		// Advertise support of asymmetric pause 
		// Advertise support of pause frames
		rtl_mdio_write(P0_RG_PHYID, 4, (data | 0xC00));
	} else {
...
#else
#define CONFIG_RTL_ESW_PHY_ID				5
#endif

#if defined(CONFIG_RTL8197F) && (defined(CONFIG_SW_8367R) || defined(CONFIG_SW_83XX))
#define GPIO_RESET				26		// GPIO_H2
#define GPIO_RESET_97FN			9		// GPIO_F1
...
int init_97f_8367r(void)
{
	...
	REG32(PEFGHDIR) |= (1<<reset_pin);

	/* ### important ### 
		when use dedicated mdc/mdio for 8367RB control path,
		need to set PITCR.Port0_TypeCfg and PCRP0.ExtPHYID before do RTL83XX_INIT(),
		check point: in first line of RTL83XX_INIT(), read reg 0x1202=0x88a8 (correct),
			if do not set PITCR and PCRP0 first, read reg 0x1202=0 (incorrect).
	 */
	// force mode
	REG32(PCRP0) = (REG32(PCRP0) & ~((0x1f << ExtPHYID_OFFSET)|AutoNegoSts_MASK)) 
			| ((CONFIG_RTL_ESW_PHY_ID << ExtPHYID_OFFSET) | MIIcfg_RXER |  EnablePHYIf | MacSwReset)
			| (EnForceMode| ForceLink|ForceSpeed1000M |ForceDuplex);
	REG32(PITCR) |= (1<<0);
	...
	for (i=0; i<3; i++) 
	{
		...
		REG32(PEFGHDAT) &= ~(1<<reset_pin);
		__mdelay(1000);
		REG32(PEFGHDAT) |= (1<<reset_pin);
		__mdelay(1000);
		
#ifdef CONFIG_RTL_ESW_CTRL_MDC_MDIO
		REG32(PIN_MUX_SEL2) &= ~(0x00000FF0); // set BIT_REG_IOCFG_P0MDC and BIT_REG_IOCFG_P0MDIO to MII mode
#else
		...
		if (bond_id == BSP_BOND_97FN) {
			if (IS_8197F()) {
				...
				// GPIO G7/H0
				REG32(PIN_MUX_SEL13) = (REG32(PIN_MUX_SEL13) & ~(0x0FF00000)) | 0x03300000;
				REG32(PEFGHCNR) &= ~(0x01800000);
				REG32(PEFGHDIR) |= (0x01800000);
				...
				REG32(PIN_MUX_SEL2) = 0x60; // TODO: need to find out
				REG32(PABCDCNR) &= ~(0x00080000);    //set GPIO pin C3
				REG32(PABCDDIR) |= (0x00080000);    //output pin

				// [31:28]    BIT_REG_IOCFG_P0MDC, 0011:GPIO        
				REG32(PIN_MUX_SEL13) = 0x30000000; // TODO: need to find out
				REG32(PEFGHCNR) &= ~(0x00400000);    //set GPIO pin G6
				REG32(PEFGHDIR) |= (0x00400000);    //output pin
	...
	#else
				// GPIO C3
				REG32(PIN_MUX_SEL2) = 0x60;
				REG32(PABCDCNR) &= ~(0x00080000);
				REG32(PABCDDIR) |= (0x00080000);

				// GPIO H0
				REG32(PIN_MUX_SEL13) = (REG32(PIN_MUX_SEL13) & ~(0x00F00000)) | 0x00300000;
				REG32(PEFGHCNR) &= ~(0x01000000);
				REG32(PEFGHDIR) |= (0x01000000);
			...
			// set to GPIO mode
			// GPIO_D1: 0xb800083c [31:28]	BIT_REG_IOCFG_MMC_D0, 0101:GPIO
			// GPIO_D7: 0xb800083c [7:4]	BIT_REG_IOCFG_MMC_D6, 0111:GPIO
			REG32(PIN_MUX_SEL15) = (REG32(PIN_MUX_SEL15) & ~0xF00000F0) | 0x50000070;
			REG32(PIN_MUX_SEL2) &= ~0x0FFFF000;
			REG32(PABCDCNR) &= ~(0x82000000);	//set GPIO pin, D1 and D7
			REG32(PABCDDIR) |= (0x82000000);	//output pin

            // MDC: D1, MDIO: D7
			smi_init(GPIO_PORT_D, 1, 7);					
	#elif defined(CONFIG_RTL_MDC_D5_MDIO_D7)
            // set to GPIO mode
            // GPIO_D5: 0xb800083c [15:12]  BIT_REG_IOCFG_MMC_D4, 0111:GPIO
            // GPIO_D7: 0xb800083c [7:4]    BIT_REG_IOCFG_MMC_D6, 0111:GPIO
            REG32(PIN_MUX_SEL15) = (REG32(PIN_MUX_SEL15) & ~0x0000F0F0) | 0x00007070;
            REG32(PIN_MUX_SEL2) &= ~0x0FFFF000;
            REG32(PABCDCNR) &= ~(0xA0000000);   //set GPIO pin, D5 and D7
            REG32(PABCDDIR) |= (0xA0000000);    //output pin

             // MDC: D5, MDIO: D7
             smi_init(GPIO_PORT_D, 5, 7);
	#else
			// set to GPIO mode
...
#if defined(CONFIG_SW_83XX)
	{ //enable PHY
	extern int rtk_port_phyEnableAll_set(rtk_enable_t enable);
...
#elif 0 //defined(CONFIG_RTL8197F) && defined (CONFIG_SW_83XX)
...
int init_97f_83xx(void)
{
	...
	/* ### important ### 
		when use dedicated mdc/mdio for 8367RB control path,
		need to set PITCR.Port0_TypeCfg and PCRP0.ExtPHYID before do RTL8367R_init(),
		check point: in first line of RTL8367R_init(), read reg 0x1202=0x88a8 (correct),
			if do not set PITCR and PCRP0 first, read reg 0x1202=0 (incorrect).
	 */
	// force mode
	REG32(PCRP0) = (REG32(PCRP0) & ~((0x1f << ExtPHYID_OFFSET)|AutoNegoSts_MASK)) 
...
int32 swCore_init()
{
	...
	if(gpio_simulate_mdc_mdio)
		REG32(BSP_PIN_MUX_SEL13) &= (0xf<<28);
	else
	...
	REG32(CPUICR1) |= (1 << CF_TXRX_DIV_LX_OFFSET);
	REG32(MACCTRL1) |= CF_CMAC_CLK_SEL;
#endif
