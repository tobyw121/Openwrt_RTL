...
#ifdef CONFIG_PARAM_PASSING_ADDR
#define PARAM_ADDR	CONFIG_PARAM_PASSING_ADDR
#else
...
static void boot_cmdline_linux(bootm_headers_t *images)
{
	...
	while (bootargs && *bootargs && linux_argc < LINUX_MAX_ARGS) {
		quote = (const char*)strchr(bootargs, '"');
		next = (const char*)strchr(bootargs, ' ');
		...
		while (next && quote && quote < next) {
			 ...
			 */
			next = (const char*)strchr(quote + 1, '"');
			if (next) {
				quote = (const char*)strchr(next + 1, '"');
