...
#define MDC_PIN		BSP_GPIO_PIN_H0
#define MDIO_PIN	BSP_GPIO_PIN_G7
#else
...
//	printk("PABCDCNR_REG=%X\n", REG32(PABCDCNR_REG));
	...
	//REG32(PABCDDIR_REG) |= 0x00080000;
	REG32(PEFGHDIR_REG) = REG32(PEFGHDIR_REG) | 0x00400000;
	//REG32(PEFGHDIR_REG) |= 0x00400000;
...
static void _smiGenWriteClk(void) 
{
	...
	REG32(mdiodat_reg) &= ~(1<<mdio_pin);// MDIO=0 
	REG32(mdcdat_reg) = REG32(mdcdat_reg) & ~(1<<mdc_pin);// MDC=0
	REG32(mdcdat_reg) &= ~(1<<mdc_pin);// MDC=0
...
#if defined(CONFIG_RTL_8197F_VG)
	//mdc : H0
	REG32(BSP_PIN_MUX_SEL13) = (REG32(BSP_PIN_MUX_SEL13) & ~(0xf<<20)) | (0x3<<20);
	REG32(PEFGHCNR_REG) &= ~(1<<mdc_pin);
	REG32(PEFGHDIR_REG) |= (1<<mdc_pin);
	REG32(PEFGHDAT_REG) &= ~(1<<mdc_pin);
	//mdio : G7
	REG32(BSP_PIN_MUX_SEL13) = (REG32(BSP_PIN_MUX_SEL13) & ~(0xf<<24)) | (0x3<<24);
	REG32(PEFGHCNR_REG) &= ~(1<<mdio_pin);
	REG32(PEFGHDIR_REG) |= (1<<mdio_pin);
	REG32(PEFGHDAT_REG) &= ~(1<<mdio_pin);
#else
	//mdc : G6
	REG32(BSP_PIN_MUX_SEL13) = (REG32(BSP_PIN_MUX_SEL13) & ~(0xf<<28)) | (0x3<<28);
	REG32(PEFGHCNR_REG) &= ~(1<<mdc_pin);
	REG32(PEFGHDIR_REG) |= (1<<mdc_pin);
	REG32(PEFGHDAT_REG) &= ~(1<<mdc_pin);
	//mdio : C3
	REG32(BSP_PIN_MUX_SEL2) = (REG32(BSP_PIN_MUX_SEL2) & ~(0xf<<4)) | (0x6<<4);
	REG32(PABCDCNR_REG) &= ~(1<<mdio_pin);
	REG32(PABCDDIR_REG) |= (1<<mdio_pin);
	REG32(PABCDDAT_REG) &= ~(1<<mdio_pin);
#endif
#endif


	dev_priv = (struct nfbi_dev_priv *)malloc(sizeof (struct nfbi_dev_priv), GFP_KERNEL);
	if(!dev_priv) {
	    dprintf(KERN_ERR DRIVER_NAME": unable to kmalloc for nfbi_dev_priv\n");
		return -ENOMEM;
	}
