...
#ifdef HAPD_DYNAMIC_DEBUG
static int debug_on = MSG_ERROR;
#define FILE_HAPD_DEBUG 	"/tmp/hapd_debug"
...
static void hapd_write_file(char *path_name, char *MSG)
{
	int fd;
...
static int hapd_get_file_value(char *filename, int *value)
{
	FILE *fp = NULL;
	char buff[1024] = {0};
...
{
	hapd_get_file_value(FILE_HAPD_DEBUG, &wpa_debug_level);
...
static void issue_signal_to_hostapd(int SIG_NUMBER ,char *interName)
{
	int pid;
	FILE *fp;
	char line[100]={0}, pid_filename[100]={0};

#ifdef RTK_HAPD_WPA_MSG_ONLY_GLOBAL
	snprintf(pid_filename, sizeof(pid_filename), "/var/run/hapd_pid.pid");
#else
	snprintf(pid_filename, sizeof(pid_filename), "/var/run/hapd_%s_pid.pid", interName);
#endif
...
static int parse_argument(int argc, char *argv[])
{
	...
	while (argNum < argc) {
		if ( !strcmp(argv[argNum], "-debug")) {
			...
			hapd_write_file(FILE_HAPD_DEBUG, tmpbuf);
		    issue_signal_to_hostapd(SIGUSR2 ,ifname);
			return -1;
		}
		argNum++;
	}
...
#ifdef HAPD_DYNAMIC_DEBUG
	eloop_register_signal(SIGUSR2, handle_debug, interfaces);
#endif
#endif /* CONFIG_NATIVE_WINDOWS */
