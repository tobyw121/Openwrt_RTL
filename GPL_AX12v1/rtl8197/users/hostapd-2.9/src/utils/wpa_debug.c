...
void wpa_msg(void *ctx, int level, const char *fmt, ...)
{
...
#ifdef RTK_HAPD_WPA_MSG_ONLY_GLOBAL
			res = os_snprintf(prefix2, sizeof(prefix2), "%s ",
					      ifname);
			if (os_snprintf_error(sizeof(prefix2), res))
				prefix2[0] = '\0';
#endif
		}
	}
	...
	if (wpa_msg_cb)
	{
		...
		if(strlen(prefix2))
		{
			buf2len = buflen+strlen(prefix2)+10;
			buf2 = os_malloc(buf2len);
			if (buf2 == NULL) {
				wpa_printf(MSG_ERROR, "wpa_msg: Failed to allocate message "
					   "buffer");
			...
			len2 = os_snprintf(buf2, buf2len, "%s%s",
					      prefix2, buf);
			if (os_snprintf_error(buf2len, len2))
			{
				bin_clear_free(buf2, buf2len);
				buf2 = NULL;
			}

			if(buf2)
				wpa_msg_cb(ctx, level, WPA_MSG_ONLY_GLOBAL, buf2, len2);
			else
