...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	if ((conf->wpa || conf->osen) && hostapd_setup_wpa(hapd))
		...
		if (hapd->conf->wpa == WPA_PROTO_WAPI && rtl_wapi_init(hapd)) {
			wpa_printf(MSG_ERROR, "RTL wapi initialization failed.");
			return -1;
		}
#endif
...
#ifdef RTK_DRV_ACL_OFFLOAD
		|| conf->bss[0]->macaddr_acl == ACL_DISABLED
#endif
	)
		return;
...
static int setup_interface(struct hostapd_iface *iface)
{
	...
	if (hapd->iconf->country[0] && hapd->iconf->country[1]) {
		...
		//remove by Realtek
		//reason:Here setting the country code to the wifi driver is not really implemented,
		//not need to delay setup wlan interface, country code implemented in later code
		/*
			   ...
			   int reassoc)
{
	...
	if (hapd->conf->wpa == WPA_PROTO_WAPI) {
		rtl_wapi_process_assoc_event(hapd->own_addr, sta->addr, sta->wapi_ie, sta->wapi_ie_len);
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
struct hostapd_data * rtl_get_hostapd_data(struct hapd_interfaces *interfaces,
					const u8 *mac)
{
	size_t i, j;
	...
	for (i = 0; i < interfaces->count; i++) {
		...
		for (j = 0; j < iface->num_bss; j++) {
			...
			if (os_memcmp(mac, hapd->own_addr, ETH_ALEN) == 0)
				return hapd;
		}
	}
