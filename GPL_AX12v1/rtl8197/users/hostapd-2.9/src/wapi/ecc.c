			  ...
			  unsigned char (*out_md)[EVP_MAX_MD_SIZE], size_t *out_md_len)
{
	...
	if (0 == EVP_DigestFinal_ex(&mdctx, (unsigned char *)out_md, (unsigned int *)&len)) {
		err = -5;
		goto DONE;
	}
...
static int
sha256_digest(const unsigned char *m, size_t m_len,
			  unsigned char *out_md, size_t *out_md_len)
{
	int err = -1;
	EVP_MD_CTX *mdctx;
	const EVP_MD *evpmd;
	...
	if (NULL == m || 0 == m_len || NULL == out_md || NULL == out_md_len) {
		err = -1;
		goto DONE;
	}
	...
	if (0 == EVP_DigestFinal_ex(mdctx, out_md, (unsigned int *)&len)) {
		err = -5;
		goto DONE;
	}
					...
					size_t *out_md_len)
{
	...
	if (NULL == BN_bin2bn(out_md, *out_md_len, bn)) {
		err = -4;
		goto DONE;
	}
...
eckey_calculate_digest(const EC_KEY *eckey, const unsigned char *m,
					   size_t m_len, unsigned char *out_md,
					   size_t* out_md_len)
{
	int err;
...
bin2ecdsa_sig(const unsigned char *in, size_t in_len)
{
	...
	if (   NULL == (sig->r = BN_bin2bn(in, in_len/2, sig->r))
		|| NULL == (sig->s = BN_bin2bn(in+in_len/2, in_len/2, sig->s)))
	{
		goto DONE;
	}
