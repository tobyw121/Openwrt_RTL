...
extern int delVIf(int vif_idx);
#define MYSIG 33   //SIGRTMIN+1
int get_wan_signo = 0, need_del_vif = 0;
...
{	 
	...
	//querierIp use network bytes order
		if((tempQuerierIp[0] == ((ntohl(querierIp)>>24)&0xFF))&&
			(tempQuerierIp[1] == ((ntohl(querierIp)>>16)&0xFF))&&
			(tempQuerierIp[2] == ((ntohl(querierIp)>>8)&0xFF))&&
			(tempQuerierIp[3] == ((ntohl(querierIp))&0xFF) ))
		{
...
int querierSelection(char *ifName, struct iphdr *ip, struct igmphdr *igmp)
{
	...
	if(ntohl(downStreamIp)<ntohl(otherQuerierIp))
	{
...
char igmp_down_if_name[IFNAMSIZ] = {0};
char igmp_down_if_idx;
#if defined(CONFIG_IGMPPROXY_MULTIWAN) || defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
char igmp_up_if_name[MAXWAN][IFNAMSIZ] = {0};
#else
char igmp_up_if_name[IFNAMSIZ];
#endif

#if defined(CONFIG_IGMPPROXY_MULTIWAN) || defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
char igmp_up_if_idx[MAXWAN] = {0};
#else
char igmp_up_if_idx;
#endif
#if defined(CONFIG_IGMPPROXY_MULTIWAN) || defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
int igmp_up_if_num = 0;
#endif
#if defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
int igmp_if_sock_old[MAX_IF] = {0}; // record old sock for remove fd
#endif
...
/*add igmp socket*/
#ifdef CONFIG_ISP_IGMPPROXY_MULTIWAN
int igmp_inf_create(struct IfDesc *dp)
{
	int i;
	int ret;
	...
	mreqn.imr_ifindex = dp->pif_idx;
	memcpy(&mreqn.imr_address, &dp->InAdr, sizeof(struct in_addr));
...
#if defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
	struct IfDesc *Dp;
	for(idx=0, Dp=IfDescVc; idx<MAX_IF; idx++, Dp++)
	{
		igmp_inf_create(Dp);
	}
#else
...
#if defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
	struct ip_mreqn mreqn;
	for(index=0, up_dp=IfDescVc; index<MAX_IF; index++, up_dp++)
	{
		if(strcmp(up_dp->Name,"")==0 || strcmp(up_dp->Name, igmp_down_if_name) == 0)
			continue;

		/* join multicast group */
		mreqn.imr_multiaddr.s_addr = group;
		mreqn.imr_address.s_addr = up_dp->InAdr.s_addr;
		mreqn.imr_ifindex=up_dp->pif_idx;
		#if 0
		printf("up_dp->sock = %d, ifindex:%d, src addr:%s, group:%s, [%s:%d]\n", 
			up_dp->sock, mreqn.imr_ifindex, inet_ntoa(mreqn.imr_address), inet_ntoa(mreqn.imr_multiaddr), __FUNCTION__, __LINE__);
		#endif
		ret = setsockopt(up_dp->sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void*)&mreqn, sizeof(mreqn));
		if(ret) {
			fprintf(stderr,"setsockopt IP_ADD_MEMBERSHIP %s on %s error!\n", 
				inet_ntoa(mreqn.imr_multiaddr), up_dp->Name);
		}
	}
#else
...
int add_mfc(__u32 group, __u32 src)
{
	...
	for(index=0, up_dp=IfDescVc; index<MAX_IF; index++, up_dp++)
	{
		...
		mrd.InVif = up_dp->vif_idx;
		mrd.OriginAdr.s_addr = htonl(mrd.InVif);
		//
...
int index;
	...
	for(index=0, up_dp=IfDescVc; index<MAX_IF; index++, up_dp++)
	{
		...
		#if 0
		printf("up_dp->sock = %d, ifindex:%d, src addr:%s, group:%s, [%s:%d]\n", 
			up_dp->sock, mreqn.imr_ifindex, inet_ntoa(mreqn.imr_multiaddr), inet_ntoa(mreqn.imr_address), __FUNCTION__, __LINE__);
		#endif
		ret = setsockopt(up_dp->sock, IPPROTO_IP, IP_DROP_MEMBERSHIP, (void*)&mreqn, sizeof(mreqn));
		if(ret)
...
int del_mfc(__u32 group)
{
	...
	for(index=0, up_dp=IfDescVc; index<MAX_IF; index++, up_dp++)
	{
		...
		mrd.OriginAdr.s_addr = htonl(up_dp->vif_idx);
		mrd.McAdr.s_addr = group;
		mrd.InVif = up_dp->vif_idx;
		memset(mrd.TtlVc, 0, sizeof(mrd.TtlVc));
		if(mrd.InVif >= 0)
	...
	for(index=0;index<igmp_up_if_num;index++) {
		/* delete multicast routing entry */
		mrd.OriginAdr.s_addr = htonl(igmp_up_if_idx[index]);
		mrd.McAdr.s_addr = group;
		mrd.InVif = igmp_up_if_idx[index];
		memset(mrd.TtlVc, 0, sizeof(mrd.TtlVc));
		delMRoute(&mrd);
...
int add_mfc(__u32 group, __u32 src)
{
	...
	/* add multicast routing entry */
	mrd.OriginAdr.s_addr = htonl(igmp_up_if_idx);
	//mrd.OriginAdr.s_addr = 0;
	// Kaohj --- special case, save the subscriber IP to kernel
	// in order to take the subscriber IP (source IP) to the upstream server
	mrd.SubsAdr.s_addr = src;
			...
			while( rec_id < ntohs(igmpv3->ngrec) )
			{
				...
				switch( igmpv3grec->grec_type )
				{
					...
					case IGMPV3_CHANGE_TO_INCLUDE: 
						//printf( "TO_IN\n" );
						if(ntohs(igmpv3grec->grec_nsrcs))
							add_group_and_src( group, src );
						else //empty
...
#if defined(CONFIG_IGMPPROXY_MULTIWAN) || defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
#if defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
	for(index=0, dp=IfDescVc; index<MAX_IF; index++, dp++)
	{
		if(strcmp(dp->Name, igmp_down_if_name) == 0)
			continue;
...
int igmp_query(__u32 dst, __u32 grp,__u8 mrt)
{
	...
	if(querierSelectionResult==QUERIER_IS_OTHER)
	{
		...
		(ntohl(otherQuerierIP)>>24)&0xFF,(ntohl(otherQuerierIP)>>16)&0xFF,(ntohl(otherQuerierIP)>>8)&0xFF,ntohl(otherQuerierIP)&0xFF);
		#endif
		return 0;
	}
...
int igmp_leave(__u32 grp, int if_idx)
{
	...
	ip->tot_len              = ntohs(MIN_IP_HEADER_LEN + IGMP_MINLEN);
	ip->ttl = 1;
...
#ifdef CONFIG_ISP_IGMPPROXY_MULTIWAN
void WanFlag2PifName(int WanFlag, char *Name)
{
	if(WanFlag & WAN_PIF_1)
		strcpy(Name, "eth1.1");
	else if(WanFlag & WAN_PIF_2)
		strcpy(Name, "eth1.2");
	else if(WanFlag & WAN_PIF_3)
		strcpy(Name, "eth1.3");
	else if(WanFlag & WAN_PIF_4)
		strcpy(Name, "eth1.4");
	else 
		strcpy(Name, "");

}
void WanFlag2VifName(int WanFlag, char *Name)
{	
	if(WanFlag & WAN_VIF_1)
		strcpy(Name, "ppp1");
	else if(WanFlag & WAN_VIF_2)
		strcpy(Name, "ppp2");
	else if(WanFlag & WAN_VIF_3)
		strcpy(Name, "ppp3");
	else if(WanFlag & WAN_VIF_4)
		strcpy(Name, "ppp4");
	else 
		strcpy(Name, "");
}

int addUpIfByName(char *Name)
{
	int i,j;
	for(i = 0; i < MAXWAN; i++)
	{	
		if(strcmp(Name, "")!=0 && strcmp(igmp_up_if_name[i], Name)==0 )
			break;
	}
	if(i == MAXWAN)
	{
		//not found
		for(j = 0; j < MAXWAN; j++)
			if(strcmp(igmp_up_if_name[j], "")==0)
				break;
				
		if(j == MAXWAN)
		{
			printf("too much wan interface! [%s:%d]\n", __FUNCTION__, __LINE__);
			return -1;
		}
		strcpy(igmp_up_if_name[j], Name);
		//printf("add if=%s, [%s:%d]\n", igmp_up_if_name[j], __FUNCTION__, __LINE__);
		igmp_up_if_num ++;
		return j;
	}
	else
...
int deleteUpIfByName(char *Name)
{
	int i,j;
	for(i=0; i<MAXWAN; i++)
	{
	...
	{
		//found
		strcpy(igmp_up_if_name[i], "");
		//printf("delete if=%s, [%s:%d]\n", Name, __FUNCTION__, __LINE__);
		igmp_up_if_num --;
		return i;
	}
	else
...
void sigWanUp(int signum, siginfo_t * info)
{	
	int WanIfFlag;
	char WanPIfName[IFNAMSIZ]={0}, WanVIfName[IFNAMSIZ]={0}, WandelIf[IFNAMSIZ] = {0};
	int i,j;
	int wan_add_index = -1, wan_del_index = -1;
	char UpIf[256] = {0}, UpIfCmd[256] = {0};

	//printf("signum = %d, parameter = %x, [%s:%d]\n", signum, info->si_int, __FUNCTION__, __LINE__);

	WanIfFlag = info->si_int;
	get_wan_signo = 1;
	if(WanIfFlag & NEED_DEL_VIF)
		need_del_vif = 1;
	if(WanIfFlag & ADD_WAN_IF)
	{
		//add pif
		WanFlag2PifName(WanIfFlag, WanPIfName);
		if(strcmp(WanPIfName, "")!=0)
		{
			WanFlag2VifName(WanIfFlag << 8, WandelIf);
			wan_del_index = deleteUpIfByName(WandelIf);
			wan_add_index = addUpIfByName(WanPIfName);

			//printf("add if:%s, delete if:%s, [%s:%d]\n", WanPIfName, WandelIf, __FUNCTION__, __LINE__);
			
		}
		else
		{
			WanFlag2VifName(WanIfFlag, WanVIfName);
			if(strcmp(WanVIfName, "")!=0)
			{
				WanFlag2PifName(WanIfFlag >> 8, WandelIf);
				wan_del_index = deleteUpIfByName(WandelIf);
				wan_add_index = addUpIfByName(WanVIfName);
				
				//printf("add if:%s, delete if:%s, [%s:%d]\n", WanVIfName, WandelIf, __FUNCTION__, __LINE__);
			}
		}

    }
	else if(WanIfFlag & DEL_WAN_IF)
	{	
		//printf("del interface! [%s:%d]\n",__FUNCTION__, __LINE__);
		//delete pif
		WanFlag2PifName(WanIfFlag, WanPIfName);
		if(strcmp(WanPIfName, ""))
		{
			wan_del_index = deleteUpIfByName(WanPIfName);
			//printf("del if: %s, [%s:%d]\n", WanPIfName, __FUNCTION__, __LINE__);
		}

		//delete vif
		WanFlag2VifName(WanIfFlag, WanVIfName);
		if(strcmp(WanVIfName, ""))
		{
			wan_del_index = deleteUpIfByName(WanVIfName);
			//printf("del if: %s, [%s:%d]\n", WanVIfName, __FUNCTION__, __LINE__);
		}
	...
	for(i=0, j=0;i<MAXWAN;i++)
	{
		if(strcmp(igmp_up_if_name[i], "")!=0)
		{
			if(j==0)
			{
				//first wan
				strcpy(UpIf, igmp_up_if_name[i]);
				j++;
			}
			else
			{
				strcat(UpIf, ",");
				strcat(UpIf, igmp_up_if_name[i]);
			}
		}
	}
	...
	if(strcmp(UpIf, "")!=0)
	{
		sprintf(UpIfCmd, "echo %s > /var/igmp_up", UpIf);
		//printf("UpIfCmd:%s\n", UpIfCmd);
		system(UpIfCmd);
	}
}
...
//#endif			
...
#if defined(CONFIG_IGMPPROXY_MULTIWAN)|| defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
#if defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)
	for(index=0, index_tmp=0;index<MAXWAN;index++)
	{
		if(strcmp(igmp_up_if_name[index], "") !=0)
		{
			if(index_tmp==0)
			{
				//first wan interface
 				strcpy(UpInf, igmp_up_if_name[index]);
				index_tmp++;
			}
			else
			{
				strcat(UpInf, ",");
				strcat(UpInf, igmp_up_if_name[index]);
			}
		}
	}
	if(strcmp(UpInf, ""))
	{
		sprintf(UpInfCmd, "echo %s > /var/igmp_up", UpInf);
		system(UpInfCmd);
	}
#else
	for(index=0;index<igmp_up_if_num;index++)
	...
	/*proc/igmp_proxy_wan_dev not used yet*/
#else
...
/*install signal begin*/	
    ...
    myAct.sa_sigaction=sigWanUp;
  
    if(sigaction(signo, &myAct, NULL)<0)  
    {  
        printf("signal is installed unsuccessfully\n");  
    } 
	else
	{	
		printf("signal is installed successfully\n");
	}
#endif
/*install signal end*/
	...
	for(index=0, IfDp=IfDescVc; index<MAX_IF; index++, IfDp++)
	{
		if(strcmp(IfDp->Name, "")!=0 && strcmp(IfDp->Name, igmp_down_if_name)!=0 && IfDp->sock > 0)
		{
			add_fd(IfDp->sock);
			flags = fcntl(IfDp->sock, F_GETFL);
			if (flags == -1 || fcntl(IfDp->sock, F_SETFL, flags | O_NONBLOCK) == -1)
...
/*2008-0919 add ,when l2pt disconnection or any reason ;when igmpProxy be restart
	...
	while(1)
	{
		...
		tv.tv_usec = 100000;
#ifdef CONFIG_ISP_IGMPPROXY_MULTIWAN
	//update wan interface info, block signal
		if(get_wan_signo)
		{
			buildIfVc();
			...
			for(index=0, IfDp=IfDescVc; index<MAX_IF; index ++, IfDp++)
			{
				if(strcmp(IfDp->Name, igmp_down_if_name) ==0)
					continue;
				
				addVIF(IfDp);
				if(strcmp(IfDp->Name, "")!=0 && IfDp->sock <= 0)
				{
					igmp_inf_create(IfDp);
				...
				else if(igmp_if_sock_old[index] >0 && strcmp(IfDp->Name, "")==0 && FD_ISSET(igmp_if_sock_old[index], &in_fds))
					remove_fd(igmp_if_sock_old[index]);
			}

			if(need_del_vif)
			{
				// only dhcp eth1.x disconnect need to del vif 2x-1,
				
				for(index=1; index<MAX_IF; index++)
				{
					for(index_tmp=0, IfDp=IfDescVc; index_tmp<MAX_IF; index_tmp ++, IfDp++)
					{
						if(strcmp(IfDp->Name, "")!=0 && (IfDp->vif_idx == 2*index-1))
							break;
					}
					if(index_tmp == MAX_IF)
					{
		...
		//2010-8-3
		IfDp =  getIfByName(igmp_down_if_name);
		if(IfDp && FD_ISSET(IfDp->sock, &in_fds))
		{
		...
		for(index=0, IfDp=IfDescVc; index<MAX_IF; index++, IfDp++)
		{		
			if(strcmp(IfDp->Name, "")!=0 && FD_ISSET(IfDp->sock, &in_fds))
			{
				recvlen = recvfrom(IfDp->sock,recv_buf,RECV_BUF_SIZE,
					   0, NULL, &recvlen);
				if (recvlen < 0) 
