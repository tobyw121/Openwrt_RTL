...
//keith_fwd
#include <unistd.h>
#if defined(KERNEL_3_10) || defined(KERNEL_4_4)
#include <linux/fs.h>
#endif
    ...
    if (fp)
		{
   		fgets(buf,150,fp);
			...
			FILE *file = fopen("/tmp/reinit_web", "r");
			char tmpbuf[10] = {0};
			int val = 0;
			if(file){
			...
			if(val == 1){
				shm_memory += sizeof(int);		//image_len
				//printf("fwd read shm_id=%d\n", shm_id);
				//printf("fwd read shm_memory = %s\n", shm_memory);
			}
#endif
