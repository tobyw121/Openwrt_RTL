...
#endif
typedef enum {DEFAULT_SW=0,DEFAULT_ALL=1,DEFAULT_HW=2,DEFAULT_BLUETOOTHHW=4,DEFAULT_CUSTOMERHW=8,DEFAULT_RF_DPK,DEFAULT_SAVEDEF} WRITE_DEFAULT_TYPE_T;
...
extern char *apmib_load_rf_dpk_conf(void);
...
#define DHCP6_OPTION16_FORMAT ("%d, %d, %d, %s")
#define DHCP6_OPTION17_FORMAT ("%d, %d, %d, %s, %s")
#endif
#ifdef CONFIG_APP_TR069
#ifdef CHECK_IPV4_DHCP_OPTION	
#define DHCP_OPTION60_125_FORMAT ("%d, %d, %d, %s, %d")
#endif
#endif
...
#ifdef IS_AX_SUPPORT
static int config_area_cust_hw_ext;
#endif
...
#endif
#ifdef CHECK_IPV6_DHCP_OPTION
static void showDhcp6Opt16Help(void);
static void showDhcp6Opt17Help(void);
#endif
...
int flash_mib_checksum_ok(CONFIG_DATA_T type);
...
static unsigned int getWLAN_ChipVersion()
{
	...
	sprintf(cmd,"wlan%d mib_rf", wlan_idx);
...
static void readFileSetParam(char *file)
{
...
#endif
#ifdef RF_DPK_SETTING_SUPPORT
	int rf_dpk_setting_found=0;
#endif /* RF_DPK_SETTING_SUPPORT */
	char *arrayval[2];
	mib_table_entry_T *pTbl;
#if defined(WIFI_SIMPLE_CONFIG) && defined(CONFIG_APP_TR069)	
	unsigned int entryNum, j;
	WSC_ER_T uuidEntry, uuidEntrytmp;
	int total_len = 0, devname_len = 0, found = 0;
#ifdef MULTI_WAN_SUPPORT	
	unsigned char cmdbuf[50] = {0};		
	unsigned int doCmd = 0, doCmd1 = 1;		
	#define WSC_TMP_FILE "./tmp/wsc_tr069_param_tmp"
#endif
#endif
...
#if defined(WIFI_SIMPLE_CONFIG) && defined(CONFIG_APP_TR069)
		if(strcmp(token, "WSC_ER_ADD")==0 || strcmp(token, "WSC_ER_DEL")==0 || strcmp(token, "WSC_ER_DELALL")==0){
			if ( !apmib_get(MIB_WLAN_WSC_ER_NUM, (void *)&entryNum)) {
				printf("Get wscER entry number error!");
				fclose(fp);
				return ;
			}
			if(strcmp(token, "WSC_ER_ADD")==0){				
				if ( (entryNum + 1) > MAX_WSC_ER_NUM) {
					printf("Cannot add new entry because wscER table is full!\n");
					fclose(fp);
					return ;
				}
			}
			if(strcmp(token, "WSC_ER_DEL")==0 || strcmp(token, "WSC_ER_DELALL")==0){
				if( entryNum==0 ){
					printf("Cannot del entry because wscER table is empty !\n");
					fclose(fp);
					return ;
				}
			}
			
			memset(&uuidEntry, 0, sizeof(uuidEntry));	
			if(strcmp(token, "WSC_ER_DELALL")==0){				
				apmib_set(MIB_WLAN_WSC_ER_DELALL, (void *)&uuidEntry);
			}else {
				// parse external registrar info							
				total_len = strlen(value);
				devname_len = total_len - (1+1+32+1); 					
				uuidEntry.wscERDisable = atoi(&value[0]);
				snprintf(uuidEntry.wscERUUID, UUID_STR_LEN, "%s", &value[2]);		
				snprintf(uuidEntry.wscERDeviceName, devname_len+1, "%s", &value[35]);

				if(strcmp(token, "WSC_ER_ADD")==0){				
					//printf("[ADD]%d\n", uuidEntry.wscERDisable);
					//printf("[ADD]%s, uuid_len:%d\n", uuidEntry.wscERUUID, strlen(uuidEntry.wscERUUID));
					//printf("[ADD]%s, dev_len:%d\n", uuidEntry.wscERDeviceName, strlen(uuidEntry.wscERDeviceName));

					//add same rule check
					for(j=1;j<=entryNum;j++)
					{
						memset(&uuidEntrytmp, 0x00, sizeof(uuidEntrytmp));
						*((char *)&uuidEntrytmp) = (char)j;
						if ( apmib_get(MIB_WLAN_WSC_ER_TBL, (void *)&uuidEntrytmp))
						{
							//printf("[%s %d]tmp uuid:%s\n", __FUNCTION__,__LINE__,uuidEntrytmp.wscERUUID);
							if (!strcmp(uuidEntrytmp.wscERUUID, uuidEntry.wscERUUID))
							{
								printf("[wsc_ER]rule already exist!\n")	;				
								fclose(fp);
								return ;
							}					
						}
					...
					apmib_set(MIB_WLAN_WSC_ER_DEL, (void *)&uuidEntry);
					if ( apmib_set(MIB_WLAN_WSC_ER_ADD, (void *)&uuidEntry) == 0) {
						printf("[wsc_ER]Add table entry error!\n");
						fclose(fp);
						return ;
					}
				}else if(strcmp(token, "WSC_ER_DEL")==0){
					//printf("[DEL]%d\n", uuidEntry.wscERDisable);
					//printf("[DEL]%s, uuid_len:%d\n", uuidEntry.wscERUUID, strlen(uuidEntry.wscERUUID));
					//printf("[DEL]%s, dev_len:%d\n", uuidEntry.wscERDeviceName, strlen(uuidEntry.wscERDeviceName));	

					//find the to be deleted entry
					for(j=1;j<=entryNum;j++)
					{
						memset(&uuidEntrytmp, 0x00, sizeof(uuidEntrytmp));
						*((char *)&uuidEntrytmp) = (char)j;
						if ( apmib_get(MIB_WLAN_WSC_ER_TBL, (void *)&uuidEntrytmp))
						{
							...
							if (!strcmp(uuidEntrytmp.wscERUUID, uuidEntry.wscERUUID))
							{
								printf("[wsc_ER]find the existed entry!\n");	
								found = 1;
								break;
							}					
						}
					...
					if( j>=entryNum && !found ){
						printf("[wsc_ER]the to be deleted entry not exist!\n");
						fclose(fp);
						return ;
					}

					// delete the specified entry
					if(apmib_set(MIB_WLAN_WSC_ER_DEL, (void *)&uuidEntry) == 0){
						printf("[wsc_ER]Del table entry error!\n");
						fclose(fp);
						return ;
					}
				}
...
#ifdef MULTI_WAN_SUPPORT
	if(strstr(token, "WSC_LAST_CONFIG_ERR"))
		doCmd1 = 0;
#endif
#endif		
		idx = searchMIB(token);
...
#endif
#ifdef RF_DPK_SETTING_SUPPORT
		else  if (config_area == RF_DPK_AREA) {
			rf_dpk_setting_found = 1;			
...
#endif
#ifdef RF_DPK_SETTING_SUPPORT
	if (rf_dpk_setting_found)
		apmib_update(RF_DPK_SETTING);
#endif /* RF_DPK_SETTING_SUPPORT */
	if (ds_setting_found)
    ...
    dir = opendir("/var/run");
    if (!dir) {
        printf("[%s]Cannot open %s", __FUNCTION__,"/var/run");
        return 0;
    }
	...
	if(dual_band_wsc){		
		if (stat(WSC_TMP_FILE, &set_status) == 0){					
			doCmd = 1;	
		}	
		/* first time to be called */
		if(doCmd1 && !doCmd){
			sprintf(cmdbuf, "echo 1 > %s" ,WSC_TMP_FILE);
			system(cmdbuf);			
		}			
	}else
		doCmd = 1;
...
extern int resetBackupCurSetFromCurSet(void);
extern int resetCurSetFromBackupCurSet(void);
#endif

#ifdef BACKUP_DEFAULT_SETTING_SUPPORT
extern int resetDefSetFromBackupDefSet(void);
extern int resetBackupDefSetFromDefSet(void);
#endif

static int resetDefault(void)//copy default setting  to current setting (and backup current setting)
{
	char *defMib;
	int fh;
...
#ifdef COMPRESS_MIB_SETTING	
	if(flash_mib_checksum_ok(type) == 1)
	{
...
#ifdef BACKUP_CURRENT_SETTING_SUPPORT
	printf("reset backup current setting!\n");
	resetBackupCurSetFromCurSet();
#endif
	return 0;
}
...
#define BT_CONFIG_FILE				 "/lib/firmware/rtlbt/rtl8761at_config"
#elif defined(CONFIG_RTK_BLUETOOTH_HW_RTL8761B_TV)
#define BT_CONFIG_FILE				 "/lib/firmware/rtlbt/rtl8761b_config"
#elif defined(CONFIG_RTK_BLUETOOTH_HW_RTL8761B_T)
#define BT_CONFIG_FILE				 "/lib/firmware/rtlbt/rtl8761bt_config"
#endif
...
unsigned int Get_HWsetting_Reallen(void){
#ifdef HEADER_LEN_INT
	HW_PARAM_HEADER_T hwHeader;
#else
	PARAM_HEADER_T hwHeader;
#endif
	...
	if(compress_hw_setting)
	{
		...
		if(memcmp(compHeader.signature, COMP_HS_SIGNATURE, COMP_SIGNATURE_LEN) == 0 ) //check whether compress mib data
		{
			...
			if ( (compLen > 0) && (compLen <= HW_SETTING_SECTOR_LEN) ) {
				...
				memcpy((char *)&hwHeader, expFile, sizeof(hwHeader));
			...
			COMP_TRACE(stderr,"\r\n ERR:no compress mib data __[%s-%u]", __FILE__,__LINE__);
			return 0;
		}
	}
...
#endif //#ifdef COMPRESS_MIB_SETTING
	{
		if ( flash_read((char *)&hwHeader, HW_SETTING_OFFSET, sizeof(hwHeader))==0 ) {
			printf("Read hw setting header failed!\n");
			return 0;
		}
		hwHeader.len=WORD_SWAP(hwHeader.len);
...
if(0)
{
	...
	//				fprintf(stderr,"%s:%d\n",__FUNCTION__,__LINE__);
		...
		else if ( !strcmp(argv[argNum], "default-dpk") ) {
#ifdef RF_DPK_SETTING_SUPPORT
			return writeDefault(DEFAULT_RF_DPK);
#endif /* RF_DPK_SETTING_SUPPORT */
			return 0;
		}
        ...
        else if ( !strcmp(argv[argNum], "test-backuphwconf") ) {
#ifdef BACKUP_HARDWARE_SETTING_SUPPORT
#if CONFIG_APMIB_SHARED_MEMORY == 1	
            apmib_sem_lock();
#endif
			if ((ptr=apmib_backuphwconf()) == NULL) {
#if CONFIG_APMIB_SHARED_MEMORY == 1	
                apmib_sem_unlock();
#endif
				return 2;
...
else if( !strcmp(argv[argNum], "test-backupbluetoothhwconf") ){
#ifdef BACKUP_HARDWARE_SETTING_SUPPORT
#ifdef BLUETOOTH_HW_SETTING_SUPPORT
#if CONFIG_APMIB_SHARED_MEMORY == 1
            apmib_sem_lock();
#endif
			if ((ptr=apmib_backupbluetoothHwconf()) == NULL) {
#if CONFIG_APMIB_SHARED_MEMORY == 1
                apmib_sem_unlock();
#endif
				return 2;
		...
		else if( !strcmp(argv[argNum], "test-dpkconf") ){
#ifdef RF_DPK_SETTING_SUPPORT
#if CONFIG_APMIB_SHARED_MEMORY == 1	
			apmib_sem_lock();
#endif
			if ((ptr=apmib_rfDPKconf()) == NULL) {
#if CONFIG_APMIB_SHARED_MEMORY == 1	
				apmib_sem_unlock();
#endif
				return 2;
...
#if CONFIG_APMIB_SHARED_MEMORY == 1
			apmib_shm_free(ptr, RF_DPK_SHM_KEY);
			apmib_sem_unlock();
#else
			free(ptr);
#endif
	...
	else if( !strcmp(argv[argNum], "test-backupcustomerhwconf") ){
#ifdef BACKUP_HARDWARE_SETTING_SUPPORT
#ifdef CUSTOMER_HW_SETTING_SUPPORT
#if CONFIG_APMIB_SHARED_MEMORY == 1	
		            apmib_sem_lock();
#endif
					if ((ptr=apmib_backupcustomerHwconf()) == NULL) {
#if CONFIG_APMIB_SHARED_MEMORY == 1	
		                apmib_sem_unlock();
#endif
						return 2;
...
else if(strcmp(argv[argNum], "test-backupcsconf") == 0)
		{
#ifdef BACKUP_CURRENT_SETTING_SUPPORT
#if CONFIG_APMIB_SHARED_MEMORY == 1
			apmib_sem_lock();
#endif
			if((ptr=apmib_backupcsconf()) == NULL)
			{
			printf("backup current setting load failed apmib.c!\n");
#if CONFIG_APMIB_SHARED_MEMORY == 1
				apmib_sem_unlock();
#endif
				return 2;
...
#if CONFIG_APMIB_SHARED_MEMORY == 1
			apmib_shm_free(ptr, BACKUP_CSCONF_SHM_KEY);
			apmib_sem_unlock();
#else
			free(ptr);
#endif
		...
		/* Backup HS -> HS */
		else if(strcmp(argv[argNum], "reset_hs_from_backuphs") == 0)
		{
#ifdef BACKUP_HARDWARE_SETTING_SUPPORT
			return resetHWSetFrombackupHWSet();
#else
			return 0;
#endif
		}
		/* HS -> Backup HS */
		else if(strcmp(argv[argNum], "reset_backuphs_from_hs") == 0)
		{
#ifdef BACKUP_HARDWARE_SETTING_SUPPORT
			return resetbackupHWSetFromHWSet();
#else
			return 0;
#endif
		}
		/* Backup BT HS -> BT HS */
		else if(strcmp(argv[argNum], "reset_bluetoothhs_from_backupbluetoothhs") == 0)
		{
#if defined(BACKUP_HARDWARE_SETTING_SUPPORT) && defined(BLUETOOTH_HW_SETTING_SUPPORT)
			return resetBTHWSetFrombackupBTHWSet();
#else
			return 0;
#endif
		}
		/* BT HS -> Backup BT HS */
		else if(strcmp(argv[argNum], "reset_backupbluetoothhs_from_bluetoothhs") == 0)
		{
#if defined(BACKUP_HARDWARE_SETTING_SUPPORT) && defined(BLUETOOTH_HW_SETTING_SUPPORT)
			return resetbackupBTHWSetFromBTHWSet();
#else
			return 0;
#endif
		}
		/* Backup CUS HS -> CUS HS */
		else if(strcmp(argv[argNum], "reset_customerhs_from_backupcustomerhs") == 0)
		{
#if defined(BACKUP_HARDWARE_SETTING_SUPPORT) && defined(CUSTOMER_HW_SETTING_SUPPORT)
			return resetCUHWSetFrombackupCUHWSet();
#else
			return 0;
#endif
		}
		/* CUS HS -> Backup CUS HS */
		else if(strcmp(argv[argNum], "reset_backupcustomerhs_from_customerhs") == 0)
		{
#if defined(BACKUP_HARDWARE_SETTING_SUPPORT) && defined(CUSTOMER_HW_SETTING_SUPPORT)
			return resetbackupCUHWSetFromCUHWSet();
#else
			return 0;
#endif
		}
		/* Backup DS -> DS */
		else if(strcmp(argv[argNum], "reset_ds_from_backupds") == 0)
		{
#ifdef BACKUP_DEFAULT_SETTING_SUPPORT
			printf("reset default setting from backup default setting!\n");
			return resetDefSetFromBackupDefSet();
#else
			return 0;
#endif
		}
		/* DS -> Backup DS */
		else if(strcmp(argv[argNum], "reset_backupds_from_ds") == 0)
		{
#ifdef BACKUP_DEFAULT_SETTING_SUPPORT
			printf("reset backup default setting from default setting!\n");	
			return resetBackupDefSetFromDefSet();
#else
			return 0;
#endif
		}
		/* Backup CS -> CS */
		else if(strcmp(argv[argNum], "reset_cs_from_backupcs") == 0)
		{
#ifdef BACKUP_CURRENT_SETTING_SUPPORT
#if defined(CONFIG_APP_APPLE_HOMEKIT)
			printf("hapserver hap_reset\n");
			system("hapserver hap_reset");
#endif
			printf("reset current setting from backup currenting!\n");
			return resetCurSetFromBackupCurSet();	
#else
			return 0;
#endif
		}
		/* CS -> Backup CS */
		else if(strcmp(argv[argNum], "reset_backupcs_from_cs") == 0)
		{
#ifdef BACKUP_CURRENT_SETTING_SUPPORT
			printf("reset backup current setting from current setting!\n");
			return resetBackupCurSetFromCurSet();
#else
			return 0;
#endif
		}

		else if(strcmp(argv[argNum], "test-alignment") == 0)
		{
		#ifndef CONFIG_MTD_NAND
			...
			int mtdIdx=0;
			uint64_t mtdSize=0;
			int erasesize = 0, tmp_erasesize = 0;
			unsigned int hw_next_page_addr, bluetooth_next_page_addr;
			unsigned int customer_page_addr, bluetooth_page_addr;
			int bit_num = 0;
			unsigned int hw_reallen = 0;
			char mtdName[32]={0};
			int retVal=0;
			if(!fp) return 0;
			while(fgets(mtdInfo,sizeof(mtdInfo),fp)){
			//	printf(mtdInfo);
				if(strstr(mtdInfo,"boot+cfg+linux")){
					sscanf(mtdInfo,"mtd%d: %x %x \"%s\"\n",&mtdIdx,&mtdSize,&erasesize,mtdName);
					//printf("mtd%d: %8.8llx %8.8x \"%s\"\n",mtdIdx,mtdSize,erasesize,mtdName);
					if(CONFIG_RTL_HW_SETTING_OFFSET%erasesize){
						printf("hw setting offset is 0x%x flash erasesize=0x%x!!!\n",
							CONFIG_RTL_HW_SETTING_OFFSET,erasesize);
						retVal=1;
					}
					hw_reallen=  Get_HWsetting_Reallen();
					if(hw_reallen == 0){
						printf("[%s:%d]get hw setting real lenght err!!!\n",__FUNCTION__,__LINE__);
						retVal=1;
					}else{

						tmp_erasesize = erasesize;
						while(tmp_erasesize >>= 1)
							bit_num++;
						#define GET_PAGE_START(x, page_shift)   (((x)>>(page_shift))<<(page_shift))
						hw_next_page_addr = GET_PAGE_START(HW_SETTING_OFFSET + hw_reallen, bit_num) + erasesize;
#ifdef CUSTOMER_HW_SETTING_OFFSET
						customer_page_addr = GET_PAGE_START(CUSTOMER_HW_SETTING_OFFSET, bit_num);
#else
						customer_page_addr = 0;
#endif
#ifdef BLUETOOTH_HW_SETTING_SUPPORT
						bluetooth_page_addr = GET_PAGE_START(BLUETOOTH_HW_SETTING_OFFSET, bit_num);
						bluetooth_next_page_addr = bluetooth_page_addr + erasesize;
#else
						bluetooth_page_addr = 0;
						bluetooth_next_page_addr = 0;
#endif
#ifdef CUSTOMER_HW_SETTING_OFFSET
						if((customer_page_addr != 0) && (hw_next_page_addr > customer_page_addr)){
							printf("customer hw setting offset is 0x%x flash erasesize=0x%x!!!\n", CUSTOMER_HW_SETTING_OFFSET,erasesize);
							retVal=1;
						}
#endif
#ifdef BLUETOOTH_HW_SETTING_SUPPORT
						if((bluetooth_page_addr != 0) && (hw_next_page_addr > bluetooth_page_addr)){
							printf("bluetooth hw setting offset is 0x%x flash erasesize=0x%x!!!\n", BLUETOOTH_HW_SETTING_OFFSET,erasesize);
							retVal=1;
						}
#endif
#if defined(CUSTOMER_HW_SETTING_OFFSET) && defined(BLUETOOTH_HW_SETTING_SUPPORT)
						if((customer_page_addr != 0) && (bluetooth_next_page_addr != 0) && (bluetooth_next_page_addr > customer_page_addr)){
							printf("customer hw setting offset is 0x%x, bluetooth hw setting offset is 0x%x flash erasesize=0x%x!!!\n", CUSTOMER_HW_SETTING_OFFSET, BLUETOOTH_HW_SETTING_OFFSET,erasesize);
							retVal=1;
						}
#endif
					}
					if(CONFIG_RTL_DEFAULT_SETTING_OFFSET%erasesize){
						printf("default setting offset is 0x%x flash erasesize=0x%x!!!\n",
							CONFIG_RTL_DEFAULT_SETTING_OFFSET,erasesize);
						retVal=1;
					}
					if(CONFIG_RTL_CURRENT_SETTING_OFFSET%erasesize){
						printf("current setting offset is 0x%x flash erasesize=0x%x!!!\n",
							CONFIG_RTL_CURRENT_SETTING_OFFSET,erasesize);
						retVal=1;
					}
					if(CONFIG_RTL_WEB_PAGES_OFFSET%erasesize){
						printf("webpage offset is 0x%x flash erasesize=0x%x!!!\n",
							CONFIG_RTL_WEB_PAGES_OFFSET,erasesize);
						retVal=1;
					}
					if(CONFIG_RTL_CODE_IMAGE_OFFSET%erasesize){
						printf("linux offset is 0x%x flash erasesize=0x%x!!!\n",
							CONFIG_RTL_CODE_IMAGE_OFFSET,erasesize);
						retVal=1;
					}
				}				
				bzero(mtdInfo,sizeof(mtdInfo));
			}
			fclose(fp);
			return retVal;
		#endif
...
#if defined(WIFI_SIMPLE_CONFIG) && defined(CONFIG_APP_TR069)
		else if ( !strcmp(argv[argNum], "-param_file_tr069") ) {
			vwlan_idx = 0;
			if ((argNum+2) < argc) {
				if (memcmp(argv[++argNum], "wlan", 4)) {
					printf("Miss wlan_interface argument!\n");
...
//#endif			
...
#ifdef CHECK_IPV6_DHCP_OPTION
	if ( action==2 && (!strcmp(mib, "DHCP6_OPTION16_TBL") || !strcmp(mib, "DEF_DHCP6_OPTION16_TBL")) ) {
		if (!valNum || (strcmp(value[0], "add") && strcmp(value[0], "del") && strcmp(value[0], "delall"))) {
			showDhcp6Opt16Help();
			goto error_return;
		}
	}
	if ( action==2 && (!strcmp(mib, "DHCP6_OPTION17_TBL") || !strcmp(mib, "DEF_DHCP6_OPTION17_TBL")) ) {
		if (!valNum || (strcmp(value[0], "add") && strcmp(value[0], "del") && strcmp(value[0], "delall"))) {
			showDhcp6Opt17Help();
			goto error_return;
		}
	...
	case 1: // get
...
#if defined(WIFI_SIMPLE_CONFIG) && defined(CONFIG_APP_TR069)
			else if(mib_wlan_table[idx].id == MIB_WLAN_WSC_ER_TBL){				
				APMIB_GET(MIB_WLAN_WSC_ER_NUM, (void *)&num);
			}
#endif
		}
#ifdef MULTI_WAN_SUPPORT
...
#ifdef CHECK_IPV6_DHCP_OPTION
			if(mib_waniface_tbl[idx].id == MIB_DHCP6_OPTION16_TBL)
				APMIB_GET(MIB_DHCP6_OPTION16_TBL_NUM, (void *)&num);
			else if(mib_waniface_tbl[idx].id == MIB_DHCP6_OPTION17_TBL)
				APMIB_GET(MIB_DHCP6_OPTION17_TBL_NUM, (void *)&num);
			else
#endif
#ifdef CONFIG_APP_TR069
#ifdef CHECK_IPV4_DHCP_OPTION
			if(mib_waniface_tbl[idx].id == MIB_DHCP_OPTION60_TBL)
				APMIB_GET(MIB_DHCP_OPTION60_TBL_NUM, (void *)&num);
			else if(mib_waniface_tbl[idx].id == MIB_DHCP_OPTION125_TBL)
				APMIB_GET(MIB_DHCP_OPTION125_TBL_NUM, (void *)&num);	
#endif
#endif	
		}
#endif
		...
		else if ( mib_table[idx].id == MIB_CWMP_WLANCONF_TBL)
	...
	else if ( mib_table[idx].id == MIB_MINIDLNA_SHAREDFOLDER_TBL)
		APMIB_GET(MIB_MINIDLNA_SHAREDFOLDER_TBL_NUM, (void *)&num);
#endif
       ...
       APMIB_GET(MIB_QOS_QUEUESTATS_TBL_NUM, (void *)&num);	   	
...
#endif
#if defined(CONFIG_APP_TR069)				
		else if (!strcmp(mib, "CWMP_BRIDGE_TBL")){
			APMIB_GET(MIB_CWMP_BRIDGE_TBL_NUM, (void *)&num);
		}
#endif
#if defined(SAMBA_WEB_SUPPORT)
		...
		getMIB(pTbl[idx].name, pTbl[idx].id, pTbl[idx].type, num, 1 ,value);
		break;
...
#endif
#ifdef RF_DPK_SETTING_SUPPORT
		updateHardcodeMib(DEFAULT_RF_DPK);
#endif
	}
...
#ifdef BACKUP_CURRENT_SETTING_SUPPORT
	//copy current setting to backup current setting after change the compress header,
	//current param_header is the same as current setting header.
	printf("reset backup current setting!\n");
	unsigned char *compPtr;
	COMPRESS_MIB_HEADER_T compHeader;
	compPtr = calloc(1, sizeof(def_setting_data));
	if(compPtr != NULL)
	{
		memcpy(compPtr, def_setting_data, sizeof(def_setting_data));         //copy def_setting_data to compPtr
		memcpy(&compHeader, def_setting_data, sizeof(COMPRESS_MIB_HEADER_T));//copy compress header to compHeader
		sprintf((char *)compHeader.signature, "%s", COMP_BC_SIGNATURE);      //modify compress header
		memcpy(compPtr, &compHeader, sizeof(COMPRESS_MIB_HEADER_T));          //write modified header to compPtr
		flash_write_withtype (compPtr, BACKUP_CURRENT_SETTING_OFFSET, sizeof(def_setting_data),BACKUP_CURRENT_SETTING);
		free(compPtr);
	}
	else
	{
		printf("calloc compPtr failed! [File:%s Line:%d]\n",__FILE__,__LINE__);
		return 0;
	}
#endif
...
#endif
#ifdef RF_DPK_SETTING_SUPPORT
	HW_PARAM_HEADER_T dpkHeader;
	RF_DPK_SETTING_T rfHwMib;
	//char rf_dpk_setting[6]={0};
	//int comp_rf_dpk_setting = 0;
#endif /* RF_DPK_SETTING_SUPPORT */
	char *data=NULL;
...
#elif defined(CONFIG_RTL_8198C)||defined(CONFIG_RTL_8196C) || defined(CONFIG_RTL_8198) || defined(CONFIG_RTL_819XD) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8198B) || defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8197G)
...
//!CONFIG_RTL_8196B =>rtl8651c+rtl8190
...
#endif	
#ifdef IS_AX_SUPPORT
		hwmib.hw_5g_on_wlan = 0;
		snprintf(hwmib.custom_para_path, sizeof(hwmib.custom_para_path), "%s", "/etc/conf/");
		hwmib.xcap_wlan0_ax = 0x40;
#endif
		// set RF parameters
		for (idx=0; idx<NUM_WLAN_INTERFACE; idx++) {
			...
			hwmib.wlan[idx].regDomain = FCC;
			memcpy(hwmib.wlan[idx].coutrystr, "US", 2);
			hwmib.wlan[idx].rfType = 10;
			hwmib.wlan[idx].xCap = 0;
			hwmib.wlan[idx].Ther = 0;
			#if defined(CONFIG_APP_APPLE_MFI_WAC)
		...
		//strcpy(customerHwMib.wlan1_wpaPSK,"wlan1_wpaPsk");
			...
			//mib_display_tlv_content(HW_SETTING, data, header.len);				
				...
				if(comp_customer_hw_setting == 1)
				{
					...
					{
						COMP_TRACE(stderr,"\r\n flash_mib_compress_write CUSTOMER HW_SETTING DONE, header.len=%d sig=%s __[%s-%u]\n",header.len,header.signature, __FILE__,__LINE__);			
					}
				}
				else
				{
			...
			else
			{
				...
				offset += sizeof(header);
				if ( flash_read((char *)buff, offset, DWORD_SWAP(header.len)) == 0) {
					printf("read hs MIB failed!\n");
					return -1;
				}
				status = CHECKSUM_OK(buff, DWORD_SWAP(header.len));
				if ( !status) {
					printf("hs Checksum error!\n");
					return -1;
				}else{
					printf("hs checksum ok\n");
				}
...
#ifdef RF_DPK_SETTING_SUPPORT
	/* reset to RF DPK setting default, when reset DEFAULT */
	if(isAll==DEFAULT_ALL||isAll==DEFAULT_RF_DPK)
	{
		// write RF DPK setting
		sprintf((char *)dpkHeader.signature, "%s%02d", RF_DPK_SETTING_HEADER_TAG, RF_DPK_SETTING_VER);
		dpkHeader.len = sizeof(rfHwMib) + sizeof(checksum);
		bzero(&rfHwMib,sizeof(RF_DPK_SETTING_T));

	#ifdef _LITTLE_ENDIAN_
		swap_mib_value(&rfHwMib, RF_DPK_SETTING);
	#endif

		data = (char *)&rfHwMib;
 
	#if 0//def HEADER_LEN_INT
		checksum = CHECKSUM((unsigned char *)data, hwHeader.len-1);	
	#else
		checksum = CHECKSUM((unsigned char *)data, dpkHeader.len-1);
	#endif
		lseek(fh, RF_DPK_SETTING_OFFSET, SEEK_SET);
		
	#ifdef _LITTLE_ENDIAN_
	  #if 0//def HEADER_LEN_INT
		hwHeader.len=WORD_SWAP(hwHeader.len);
	  #else
		dpkHeader.len=WORD_SWAP(dpkHeader.len);
	  #endif
	#endif
		...
		if ( write(fh, (const void *)&hwHeader, sizeof(hwHeader))!=sizeof(hwHeader) )
	#else
		if ( write(fh, (const void *)&dpkHeader, sizeof(dpkHeader))!=sizeof(dpkHeader) ) 
	#endif
		{
			printf("write RF DPK setting header failed!\n");
			return -1;
		}
		if ( write(fh, (const void *)&rfHwMib, sizeof(rfHwMib))!=sizeof(rfHwMib) ) {
			printf("write RF DPK setting MIB failed!\n");
			return -1;
		}
		if ( write(fh, (const void *)&checksum, sizeof(checksum))!=sizeof(checksum) ) {
			printf("write RF DPK setting checksum failed!\n");
			return -1;
		}
		close(fh);
		sync();
		
 		if(isAll==DEFAULT_RF_DPK)
		{
		
 			offset = RF_DPK_SETTING_OFFSET;
		#if 0//def HEADER_LEN_INT
			if ( flash_read((char *)&hwHeader, offset, sizeof(hwHeader)) == 0) 
			{
			...
			hwHeader.len=WORD_SWAP(hwHeader.len);
			if ( flash_read((char *)buff, offset, hwHeader.len) == 0) {
				printf("read RF DPK setting MIB failed!\n");
				return -1;
			}
			status = CHECKSUM_OK(buff, hwHeader.len);
			if ( !status) {
				printf("RF DPK setting Checksum error!\n");
				return -1;
			}
		#else
			if ( flash_read((char *)&dpkHeader, offset, sizeof(dpkHeader)) == 0) 
			{
				printf("read RF DPK setting header failed!\n");
				return -1;
			}
			offset += sizeof(dpkHeader);
			dpkHeader.len=WORD_SWAP(dpkHeader.len);
			if ( flash_read((char *)buff, offset, dpkHeader.len) == 0) {
				printf("read RF DPK setting MIB failed!\n");
				return -1;
			}
			status = CHECKSUM_OK(buff, dpkHeader.len);
			if ( !status) {
				printf("RF DPK setting Checksum error!\n");
				return -1;
			}
...
// MBO requires 11W capable and SHA256
#ifdef RTK_MBO_SUPPORT
				pMib->wlan[idx][i].wpa11w = (unsigned char) MGMT_FRAME_PROTECTION_OPTIONAL;
				pMib->wlan[idx][i].wpa2EnableSHA256 = 1;
#endif
#endif				
			}
			if(i <= 4)
...
#endif
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL8192E) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_RTL8192F)
	...
	memset(pMib->ntpServerIp1,0,sizeof(pMib->ntpServerIp1));
	memset(pMib->ntpServerIp2,0,sizeof(pMib->ntpServerIp2));
	memset(pMib->ntpServerIp3,0,sizeof(pMib->ntpServerIp3));
	memset(pMib->ntpServerIp4,0,sizeof(pMib->ntpServerIp4));
	memset(pMib->ntpServerIp5,0,sizeof(pMib->ntpServerIp5));
	memset(pMib->ntpServerIp6,0,sizeof(pMib->ntpServerIp6));
	#if 0
...
//#ifdef TR181_V6_SUPPORT
	...
	pMib->mapLogLevel = 0;
	pMib->mapPrimaryVid = 0;
	pMib->mapDefaultSecondaryVid = 0;
#endif
...
pMib->WANIfaceNum=WANIFACE_NUM;
{
	...
	for(i=0;i<WANIFACE_NUM;i++)
	{
		...
		snprintf((char *)pMib->WANIfaceArray[i].WanName,sizeof(pMib->WANIfaceArray[i].WanName), "WAN%d", i+1);
...
#else
#ifdef CONFIG_APP_TR069
	snprintf(pMib->cwmp_WanName,sizeof(pMib->cwmp_WanName),"WAN");
#endif
#endif
...
#if defined(CONFIG_APP_TR069)
	pMib->cwmp_DL_StartTime = 0;
	pMib->cwmp_DL_CompleteTime = 0;
	pMib->cwmp_DL_FaultCode = 0;
	pMib->cwmp_DL_Queue_Idx = 65535;
    // cwmp default debug info
    pMib->cwmp_debug_level = 0;         // LEVEL_ERROR
    pMib->cwmp_debug_module = (1<<1);   // MODULE_CORE
    // cwmp client key file password, this is password for client.pem
    sprintf(pMib->cwmp_CERT_Password, "client");
#endif
...
pMib->mibVer = 2;
pMib->opMode = BRIDGE_MODE;
#if defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8871AM) || defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8197G) || defined(CONFIG_RTL_8881A_SELECTIVE)
...
pMib-> repeaterEnabled1 =0;
	...
	pMib->httpRedirectEnabled = 1;
	strcpy(pMib->httpRedirectUrl, "www.realtek.com");
...
//fprintf(stderr,"\r\n header.len=%u, __[%s-%u]",header.len,__FILE__,__LINE__);
	...
	{
		COMP_TRACE(stderr,"\r\n flash_mib_compress_write DEFAULT_SETTING DONE, __[%s-%u]\n", __FILE__,__LINE__);			
	}
	else
	{
...
//mib_display_tlv_content(CURRENT_SETTING, data, header.len);		
	...
	if(flash_mib_compress_write(CURRENT_SETTING, data, &header, &checksum) == 1)
	{
		COMP_TRACE(stderr,"\r\n flash_mib_compress_write CURRENT_SETTING DONE, __[%s-%u]\n", __FILE__,__LINE__);			
	}
	else
	{
...
//write backup current setting, 
//param_header is the same as current setting's param_header, but comp_header is not the same
/*#ifdef BACKUP_CURRENT_SETTING_SUPPORT
	printf("reset backup current setting!\n");
	resetBackupCurSetFromCurSet();
#endif*/

//write backup default setting, 
//param_header is the same as default setting's param_header, but comp_header is not the same
/*#ifdef BACKUP_DEFAULT_SETTING_SUPPORT
	printf("reset backup default setting!\n");
	resetBackupDefSetFromDefSet();
#endif*/

#ifdef MIB_TLV
	if(mib_tlv_data != NULL)//free mib_tlv_data
	{
		free(mib_tlv_data);
	}
#endif

//check if hw, ds, cs and backup current setting's checksum is ok
	offset = HW_SETTING_OFFSET;
	type= HW_SETTING;
#ifdef COMPRESS_MIB_SETTING
	if(comp_hw_setting == 1)
	...
	offset = CURRENT_SETTING_OFFSET;
	//printf("CURRENT_SETTING_OFFSET=%u ,type=%d, [%s-%u]\n", offset,CURRENT_SETTING,__FILE__,__LINE__);
#ifdef COMPRESS_MIB_SETTING
	if(flash_mib_checksum_ok(type) == 1)
	{
		COMP_TRACE(stderr,"\r\n CURRENT_SETTING checksum_ok!\n");
	}
	else
	{
#endif // #ifdef COMPRESS_MIB_SETTING	
	if ( flash_read_withtype((char *)&header, offset, sizeof(header),type) == 0) {
	...
	offset += sizeof(header);
	printf("CURRENT_SETTING_OFFSET=%u ,type=%d, [%s-%u]\n", offset,CURRENT_SETTING,__FILE__,__LINE__);
	if ( flash_read_withtype ((char *)buff, offset, header.len,type ) == 0) {
...
if(resetBackupCurSetFromCurSet() == -1)
		{
			printf("resetBackupCurSet From CurSet is error! [File:%s Line:%d]\n",__FILE__,__LINE__);
			return -1;
		}
#endif 
#ifdef BACKUP_DEFAULT_SETTING_SUPPORT	
		...
		{
			printf("resetBackupDefSet From DefSet is error! [File:%s Line:%d]\n",__FILE__,__LINE__);
			return -1;
		}
#endif
#ifdef BACKUP_HARDWARE_SETTING_SUPPORT
...
//read backup current setting from flash and check if it's checksum is ok
#ifdef BACKUP_CURRENT_SETTING_SUPPORT
	offset = BACKUP_CURRENT_SETTING_OFFSET;
	type= BACKUP_CURRENT_SETTING;
#ifdef COMPRESS_MIB_SETTING

	if(flash_mib_checksum_ok(type) == 1)
	{
		COMP_TRACE(stderr,"\r\n backup CURRENT_SETTING checksum_ok!\n");
	}
	else
	{
	...
	if ( flash_read_withtype((char *)&header, offset, sizeof(header),type ) == 0) 
	{
		printf("read backup cs header failed!\n");
		return -1;
	}
	...
	if ( flash_read_withtype ((char *)buff, offset, header.len,type) == 0) 
	{
		printf("read backup cs MIB failed!\n");
		return -1;
	}
	status = CHECKSUM_OK(buff, header.len);
	...
	if ( !status) 
	{
		printf("backup cs Checksum error!\n");
		return -1;
	}
#ifdef COMPRESS_MIB_SETTING
	}
...
//read backup default setting from flash and check if it's checksum is ok
#ifdef BACKUP_DEFAULT_SETTING_SUPPORT
	offset = BACKUP_DEFAULT_SETTING_OFFSET;
	type=BACKUP_DEFAULT_SETTING;
#ifdef COMPRESS_MIB_SETTING
	if(flash_mib_checksum_ok(type) == 1)
	{
		COMP_TRACE(stderr,"\r\n backup default setting checksum ok!\n");
	}
	else
	{
#endif //COMPRESS_MIB_SETTING
		...
		if(flash_read_withtype ((char *)buff, offset, header.len,type) == 0)
		{
			printf("read backup default setting MIB failed![File:%s Line:%d]\n",__FILE__,__LINE__);
			return -1;
		}
		status = CHECKSUM_OK(buff, header.len);
		if(!status)
		{
			printf("backup default setting checksum error![File:%s Line:%d]\n",__FILE__,__LINE__);
			return -1;
		}
#ifdef COMPRESS_MIB_SETTING
	}
...
#ifdef IS_AX_SUPPORT
	config_area_cust_hw_ext = 0;
#endif

#if defined(IS_AX_SUPPORT) && defined(CUSTOMER_HW_SETTING_SUPPORT)
	if (!memcmp(token, SLOPE_K_PREFIX_5G, strlen(SLOPE_K_PREFIX_5G))){
		config_area = CUSTOMER_HW_AREA;
		config_area_cust_hw_ext = 1;
		strncpy(tmpBuf, &token[strlen(SLOPE_K_PREFIX_5G)], sizeof(tmpBuf));
...
#endif
#ifdef RF_DPK_SETTING_SUPPORT
	else if (!memcmp(token, "RF_DPK_", strlen("RF_DPK_"))) {
		config_area = RF_DPK_AREA; 	
		strcpy(tmpBuf, &token[strlen("RF_DPK_")]);
	}
#endif
...
//			printf("%s:%d tmpBuf=%s name=%s\n",__FUNCTION__,__LINE__,tmpBuf,customer_hwmib_table[idx].name);
		...
		while (rf_dpk_mib_table[idx].id) {
//			printf("%s:%d tmpBuf=%s name=%s\n",__FUNCTION__,__LINE__,tmpBuf,rf_dpk_mib_table[idx].name);
			if ( !strcmp(rf_dpk_mib_table[idx].name, tmpBuf)) {
				if (desired_config && config_area != desired_config)
					return -1;
				return idx;
			}
...
static void getMIB(char *name, int id, TYPE_T type, int num, int array_separate, char **val)
{
	...
	void *value;
	unsigned char tmpBuf[4096]={0}, *format=NULL, *buf, tmp1[400]={0}, tmp2[400]={0};
	int int_val, i;
	...
	|| config_area == DEF_MIB_WAN_AREA
		...
		sprintf(tmp1, "%s", "DEF_");
#ifdef BLUETOOTH_HW_SETTING_SUPPORT
	else if(config_area == BLUETOOTH_HW_AREA)//printf("BLUETOOTH_HW_");
		sprintf(tmp1, "%s", "BLUETOOTH_HW_");
#endif
#ifdef CUSTOMER_HW_SETTING_SUPPORT
	else if(config_area == CUSTOMER_HW_AREA){//printf("CUSTOMER_HW_");
#ifdef IS_AX_SUPPORT
		if(is_slope_k(id)==SLOPE_K_5G)
			snprintf(tmp1, sizeof(tmp1), "%s", SLOPE_K_PREFIX_5G);
		else
#endif
			snprintf(tmp1, sizeof(tmp1), "%s", "CUSTOMER_HW_");
	}
#endif
#ifdef RF_DPK_SETTING_SUPPORT
	else if (config_area == RF_DPK_AREA)
...
#ifdef MBSSID
		if ((config_area == DEF_MIB_WLAN_AREA || config_area == MIB_WLAN_AREA) && vwlan_idx > 0)//printf("WLAN%d_VAP%d_", wlan_idx, vwlan_idx-1);
			sprintf(tmp2, "WLAN%d_VAP%d_", wlan_idx, vwlan_idx-1);
		else //printf("WLAN%d_", wlan_idx);
#endif	
			sprintf(tmp2, "WLAN%d_", wlan_idx);

	}	
#ifdef MULTI_WAN_SUPPORT
	else if(config_area == DEF_MIB_WAN_AREA || config_area == MIB_WAN_AREA)//printf("WAN%d_", wan_idx);	
		sprintf(tmp2, "WAN%d_",wan_idx);
#endif
...
case QOS_POLICER_ARRAY_T:		//mark_qos
   case QOS_QUEUESTATS_ARRAY_T:
...
#endif		
#if defined(CONFIG_APP_TR069)
	case CWMP_BRIDGE_ARRAY_T:
#ifdef CHECK_IPV4_DHCP_OPTION		
	case DHCP_OPTION60_ARRAY_T:
	case DHCP_OPTION125_ARRAY_T:
#endif
#endif	
...
#endif	
#ifdef CONFIG_APP_MINIDLNA
	case MINIDLNA_SHAREDFOLDER_ARRAY_T:
#endif
	...
	if (type == BYTE_T || type == WORD_T)
	...
	else if (type == DWORD_ARRAY_T){
#ifdef MULTI_WAN_SUPPORT
		if(id==MIB_WANIFACE_BINDING_LAN_PORTS){
			for(i=0;i<WAN_INTERFACE_LAN_PORT_NUM + WAN_INTERFACE_WLAN_PORT_NUM;i++)
			{
				uint_val=((unsigned int*)value)[i];
				if(uint_val>WANIFACE_NUM) uint_val=NO_WAN;
				if(i==0)
					sprintf((char *)buf,"%d",uint_val);
				else
					sprintf((char *)buf,"%s,%d",buf,uint_val);
			}
		}
#endif
#ifdef RF_DPK_SETTING_SUPPORT
		if ((id >= MIB_RF_DPK_LUT_2G_EVEN_A0) && (id <= MIB_RF_DPK_LUT_2G_ODD_B2)) {
			for(i=0;i< LUT_2G_LEN*4;i++){
				sprintf((char *)tmp1, "%02x", array_val[i]);
				strcat((char *)buf, (char *)tmp1);
			}
			convert_lower((char *)buf);
		}
 		else if ((id >= MIB_RF_DPK_LUT_5G_EVEN_A0) && (id <= MIB_RF_DPK_LUT_5G_ODD_B8)) {
			for(i=0;i< LUT_5G_LEN*4;i++){
				sprintf((char *)tmp1, "%02x", array_val[i]);
				strcat((char *)buf, (char *)tmp1);
			}
			convert_lower((char *)buf);
		}
		else if ((id >= MIB_RF_DPK_PWSF_5G_A) && (id <= MIB_RF_DPK_PWSF_5G_B)) {
			for(i=0;i< PWSF_5G_LEN*4;i++){
				sprintf((char *)tmp1, "%02x", array_val[i]);
				strcat((char *)buf, (char *)tmp1);
			}
			convert_lower((char *)buf);
		...
		if(id==MIB_DHCP_RESERVED_IPADDR){
			for(i=0;i<16;i++)
			{
				uint_val=((unsigned int*)value)[i];
				if(i==0)
...
#endif
#if defined(IS_AX_SUPPORT)
		if((id >= MIB_HW_2G_CCK_TSSI_A) && (id <= MIB_HW_2G_CCK_TSSI_D))
			max_chan_num = MAX_2G_TSSI_CCK_SIZE_AX;
		else if((id >= MIB_HW_2G_BW40_1S_TSSI_A) && (id <= MIB_HW_2G_BW40_1S_TSSI_D))
			max_chan_num = MAX_2G_TSSI_BW40_SIZE_AX;
		else if((id >= MIB_HW_5G_BW40_1S_TSSI_A) && (id <= MIB_HW_5G_BW40_1S_TSSI_D))
			max_chan_num = MAX_5G_TSSI_BW40_SIZE_AX;
		else if(is_slope_k(id)==SLOPE_K_5G)
			max_chan_num = MAX_5G_TSSI_BW40_SIZE_AX;
#endif
#ifdef RF_DPK_SETTING_SUPPORT
		if ((id >= MIB_RF_DPK_PWSF_2G_A) && (id <= MIB_RF_DPK_PWSF_2G_B)) {
			max_chan_num = PWSF_2G_LEN;
			for(i=0;i< max_chan_num;i++){
				sprintf((char *)tmp1, "%02x", array_val[i]);
				strcat((char *)buf, (char *)tmp1);
			}
...
#endif /* RF_DPK_SETTING_SUPPORT */
#ifdef IS_AX_SUPPORT
		if(((id >= MIB_HW_2G_CCK_TSSI_A) && (id <= MIB_HW_5G_BW40_1S_TSSI_D))
			|| (is_slope_k(id)==SLOPE_K_5G))
		{
			for(i=0;i< max_chan_num;i++){
				snprintf((char *)tmp1, sizeof(tmp1), "%02x,", array_val[i]);
				strncat((char *)buf, (char *)tmp1, strlen(tmp1));
			}
			if(strlen(buf) != (max_chan_num*3))
				printf("invalid value\n");
			else{
				buf[strlen(buf)-1]='\0';//del last comma
			}
		}
		else
#endif
...
//			pEntry->interface, dnsType);
...
#if defined(CONFIG_APP_TR069)	
	else if (type == CWMP_BRIDGE_ARRAY_T) 
	{		
		CWMP_BRIDGE_Tp pEntry=(CWMP_BRIDGE_Tp)array_val;

		sprintf((char *)buf, "\nInstnum : %d \nName/Enabled : \"%s\"/%d \nVlan : %d \nPort(iface/inst) : %d ", 		
			pEntry->bridgeInstNum,
			pEntry->bridgeName, pEntry->bridgeEnabled, 
			pEntry->bridgeVid, pEntry->portMaxInstNum);
		
		for(i=0; i<TOTAL_INTERFACE_PORT_NUM; i++)
		{
			if(pEntry->portInstNum[i])
			{
				sprintf((char *)tmp1, "%d/%d ",
					pEntry->portIdx[i], pEntry->portInstNum[i]);
				strcat((char *)buf, (char *)tmp1);
			}
		}
...
#ifdef CONFIG_APP_MINIDLNA
	else if (type == MINIDLNA_SHAREDFOLDER_ARRAY_T) 
	{
		MINIDLNA_SHAREFOLDER_Tp pEntry=(MINIDLNA_SHAREFOLDER_Tp)array_val;
		sprintf((char *)buf, "%s %s", 			
			pEntry->minidlna_sharedfolder_type,pEntry->minidlna_sharefolder_name
		);
	}		
#endif
	...
	else if (type == SCHEDULE_ARRAY_T) 
	{
		...
		sprintf((char *)buf, "%s, %d, %d, %d, %s, %d, %d, %d, %d", ip_buff, 
			 pEntry->fromPort, pEntry->toPort, pEntry->protoType, rmt_ip_buff, pEntry->externelFromPort, pEntry->externelToPort, pEntry->enabled,
			pEntry->InstanceNum);
		#else
...
#endif //ROUTE
#ifdef CHECK_IPV6_DHCP_OPTION
	else if (type == DHCP6_OPTION16_ARRAY_T) {
		DHCP6_OPTION16_Tp pEntry=(DHCP6_OPTION16_Tp)array_val;
		sprintf(buf,DHCP6_OPTION16_FORMAT,
		pEntry->enabled,
		pEntry->type,
		pEntry->valueMode,
		pEntry->value);
	}
	else if (type == DHCP6_OPTION17_ARRAY_T) {
		DHCP6_OPTION17_Tp pEntry=(DHCP6_OPTION17_Tp)array_val;
		sprintf(buf,DHCP6_OPTION17_FORMAT,
		pEntry->enabled,
		pEntry->type,
		pEntry->suboptCode,
		pEntry->subOptionData,
		pEntry->value);
	}	
#endif
#ifdef CONFIG_APP_TR069
#ifdef CHECK_IPV4_DHCP_OPTION
	else if ((type == DHCP_OPTION60_ARRAY_T) || (type == DHCP_OPTION125_ARRAY_T)){
		MULWAN_DHCP_OPTION_T pEntry=(MULWAN_DHCP_OPTION_T)array_val;
		sprintf(buf,DHCP_OPTION60_125_FORMAT,
		pEntry->Enable,
		pEntry->Type,
		pEntry->ValueMode,
		pEntry->Value,
...
#if defined(WIFI_SIMPLE_CONFIG) && defined(CONFIG_APP_TR069)
	else if (type == WSC_ER_ARRAY_T) {		
		WSC_ER_Tp pEntry=(WSC_ER_Tp)array_val;		
		//sprintf((char *)buf, WSC_ER_FORMAT, pEntry->wscERDisable, pEntry->wscERUUID, pEntry->wscERDeviceName);			
		snprintf((char *)buf, UUID_STR_LEN+MAX_ER_DEVICENAME_LEN+2, WSC_ER_FORMAT, pEntry->wscERDisable, pEntry->wscERUUID, pEntry->wscERDeviceName);
		//printf("###[%s %d] %d, %s, %s\n",__FUNCTION__,__LINE__, pEntry->wscERDisable, pEntry->wscERUUID, pEntry->wscERDeviceName);
	}
#endif	
#ifdef WLAN_PROFILE
...
//added by shirley 2014/5/21
...
#endif
#ifdef CHECK_IPV6_DHCP_OPTION
	DHCP6_OPTION16_T dhcp6Opt16;
	DHCP6_OPTION17_T dhcp6Opt17;
#endif
#endif
...
#endif
#if defined(CONFIG_APP_TR069)	
	CWMP_BRIDGE_T cwmpBridge_entry;
#endif
#if defined(SAMBA_WEB_SUPPORT)	
...
#endif
#ifdef RF_DPK_SETTING_SUPPORT
	unsigned int dpk[LUT_2G_LEN] = {0};
    unsigned int dpk_5g[LUT_5G_LEN] = {0};
    unsigned int pwsf_5g[PWSF_5G_LEN] = {0};
#endif /* RF_DPK_SETTING_SUPPORT */
	...
	case BYTE_T:
	case WORD_T:
		if (!strncmp(val[0], "0x\0", 2))
			sscanf(val[0], "%x", &int_val);
		else
		int_val = atoi(val[0]);
		value = (void *)&int_val;
		break;
...
#ifdef RF_DPK_SETTING_SUPPORT
		if ((id >= MIB_RF_DPK_LUT_2G_EVEN_A0) && (id <= MIB_RF_DPK_LUT_2G_ODD_B2)) {
			if (valNum != LUT_2G_LEN) {
				printf("invalid mib!\n");
				return;
			}
			for(i=0; i < valNum ; i++) {
				unsigned int tmp_val = 0;
				sscanf(val[i], "%u", &tmp_val);
				dpk[i] = DWORD_SWAP(tmp_val);
				//printf("==> %d, %u\n", i, tmp_val);
			}
			value = (void *)dpk;
			break;
...
#endif
#if defined(IS_AX_SUPPORT)
		&& !(id >= MIB_HW_2G_CCK_TSSI_A && id <= MIB_HW_5G_BW40_1S_TSSI_D)
#ifdef CUSTOMER_HW_SETTING_SUPPORT
		&& !(is_slope_k(id)==SLOPE_K_5G)
#endif
#endif
#ifdef RF_DPK_SETTING_SUPPORT
			&& !((id >= MIB_RF_DPK_PWSF_2G_A) && (id <= MIB_RF_DPK_PWSF_2G_B))
#endif
			&& (id != MIB_L2TP_PAYLOAD)){
				printf("invalid mib!\n");
				return;
	...
	if(id == MIB_L2TP_PAYLOAD)
	{
		...
		if ((id >= MIB_RF_DPK_PWSF_2G_A) && (id <= MIB_RF_DPK_PWSF_2G_B)) {
			...
			for (i=0; i<max_chan_num ; i++) {
				...
				//printf("==> key[%d]=[%02x][%02x]\n",i, key[i+1], (unsigned char)int_val);
			}
			value = (void *)key;
			break;
		}
#endif /* RF_DPK_SETTING_SUPPORT */
#ifdef IS_AX_SUPPORT
		//format flash set xx aabbccddee
		if((id>=MIB_HW_2G_CCK_TSSI_A && id<=MIB_HW_5G_BW40_1S_TSSI_D) ||
			(is_slope_k(id)==SLOPE_K_5G))
		{
			if(val[0] == NULL)
				break;
			if ( strlen(val[0])!=(max_chan_num<<1) || !string_to_hex(val[0], key+1, (max_chan_num<<1))){
				printf("\ninvalid value:%s, len:%d\n", val[0], strlen(val[0]));
				return;
			}
			key[0] = max_chan_num;
		}
...
#endif
		{
			...
			if(tx_power_cnt == 1){
				...
				if(key[1] < 1 || key[1] > max_chan_num){
					if((key[1]<1) || ((id >= MIB_HW_TX_POWER_CCK_A &&  id <=MIB_HW_TX_POWER_DIFF_OFDM) || (id >= MIB_HW_TX_POWER_TSSI_CCK_A &&  id <=MIB_HW_TX_POWER_TSSI_HT40_1S_D)) ||
						 ((id >= MIB_HW_TX_POWER_5G_HT40_1S_A &&  id <=MIB_HW_TX_POWER_TSSI_5G_HT40_1S_D) && (key[1]>216))){
						printf("invalid channel number\n");
						return;
					}
				}
...
//added by shirley
	...
	case DHCPS_SERVING_POOL_ARRAY_T:
		...
		memset(&dhcp6Opt16, 0, sizeof(dhcp6Opt16));
		if ( !strcmp(val[0], "add")) {
			id = MIB_DHCP6_OPTION16_ADD;
			if ( valNum < 18 ) {
				printf("input argument is not enough!\n");
				return;
			}
			dhcp6Opt16.enabled=atoi(val[1]);
			dhcp6Opt16.type=atoi(val[2]);
			dhcp6Opt16.valueMode=atoi(val[3]);
			strcpy(dhcp6Opt16.value, val[4]);
			value=(void *)&dhcp6Opt16;
		}
		else if ( !strcmp(val[0], "del")) {
			id = MIB_DHCP6_OPTION16_DEL;
			if ( valNum < 2 ) {
				printf("input argument is not enough!\n");
				return;							
			}
			int_val = atoi(val[1]);
			if ( !APMIB_GET(MIB_DHCP6_OPTION16_TBL_NUM, (void *)&entryNum)) {
				printf("Get trigger-port entry number error!");
				return;
			}
			if ( int_val > entryNum ) {
			...
			*((char *)&dhcp6Opt16) = (char)int_val;
			if ( !APMIB_GET(MIB_DHCP6_OPTION16_TBL, (void *)&dhcp6Opt16)) {
				printf("Get trigger-port table entry error!");
				return;
			}
		}
		else if ( !strcmp(val[0], "delall")){
			id = MIB_DHCP6_OPTION16_DELALL;
		}
		value = (void *)&dhcp6Opt16;
		break;

		case DHCP6_OPTION17_ARRAY_T:
		memset(&dhcp6Opt17, 0, sizeof(dhcp6Opt17));
		if ( !strcmp(val[0], "add")) {
			id = MIB_DHCP6_OPTION17_ADD;
			if ( valNum < 18 ) {
				printf("input argument is not enough!\n");
				return;
			}
			dhcp6Opt17.enabled=atoi(val[1]);
			dhcp6Opt17.type=atoi(val[2]);
			dhcp6Opt17.suboptCode=atoi(val[3]);
			strcpy(dhcp6Opt17.subOptionData, val[4]);
			strcpy(dhcp6Opt17.value, val[5]);
			value=(void *)&dhcp6Opt17;
		}
		else if ( !strcmp(val[0], "del")) {
			id = MIB_DHCP6_OPTION17_DEL;
			if ( valNum < 2 ) {
				printf("input argument is not enough!\n");
				return;								
			}
			int_val = atoi(val[1]);
			if ( !APMIB_GET(MIB_DHCP6_OPTION17_TBL_NUM, (void *)&entryNum)) {
				printf("Get trigger-port entry number error!");
				return;
			}
			if ( int_val > entryNum ) {
			...
			*((char *)&dhcp6Opt17) = (char)int_val;
			if ( !APMIB_GET(MIB_DHCP6_OPTION17_TBL, (void *)&dhcp6Opt17)) {
				printf("Get trigger-port table entry error!");
				return;
			}
		}
		else if ( !strcmp(val[0], "delall")){
			id = MIB_DHCP6_OPTION17_DELALL;
		}
		value = (void *)&dhcp6Opt17;
		break;
#endif
	...
	case DHCPRSVDIP_ARRY_T:
		...
		if (!strcmp(val[0], "delall"))
			id = MIB_CWMP_BRIDGE_DELALL;
		value = (void *)&cwmpBridge_entry;
		break;
#endif
#if defined(SAMBA_WEB_SUPPORT)	
		case STORAGE_USER_ARRAY_T:
...
//#endif	// CONFIG_RTK_MESH && _MESH_ACL_ENABLE_
	...
	case WDS_ARRAY_T:
		...
		break;
#if defined(WIFI_SIMPLE_CONFIG) && defined(CONFIG_APP_TR069)		
	case WSC_ER_ARRAY_T: // wsc_ER		
		memset(&wsc_ER, 0, sizeof(wsc_ER));
		if ( !strcmp(val[0], "add")) {
			id = MIB_WLAN_WSC_ER_ADD;
			if( valNum < 3 )
			{
#ifdef RTL_FLASH_TOOL_SET_MULTI_MIB	
				RTL_LOG_RC_SM(RTL_RC__MIB_SET_ERROR, "MIB[%s] input argument is not enough! ", name)
#else
				printf("input argument is not enough!\n");
				return;
#endif				
			}
			wsc_ER.wscERDisable = atoi(val[1]);	
			//strncpy((char *)wsc_ER.wscERUUID, val[2], UUID_STR_LEN);
			snprintf((char *)wsc_ER.wscERUUID, UUID_STR_LEN, "%s", val[2]);
			//strcpy((char *)wsc_ER.wscERDeviceName, val[3]);		
			snprintf((char *)wsc_ER.wscERDeviceName, MAX_ER_DEVICENAME_LEN, "%s", val[3]);
			//printf("[%s %d][add]%d, %s, %s\n", __FUNCTION__,__LINE__,wsc_ER.wscERDisable, wsc_ER.wscERUUID, wsc_ER.wscERDeviceName);
		}
		else if ( !strcmp(val[0], "del")) {			
			id = MIB_WLAN_WSC_ER_DEL;
			if ( valNum < 2 ) {
#ifdef RTL_FLASH_TOOL_SET_MULTI_MIB	
				RTL_LOG_RC_SM(RTL_RC__MIB_SET_ERROR, "MIB[%s] input argument is not enough! ", name)
#else
			...
			int_val = atoi(val[1]); // the index to be deleted
			if ( !APMIB_GET(MIB_WLAN_WSC_ER_NUM, (void *)&entryNum)) {
#ifdef RTL_FLASH_TOOL_SET_MULTI_MIB
				RTL_LOG_RC_SM(RTL_RC__MIB_SET_ERROR, "MIB[%s] Get wsc_ER number error! ", name)
#else
				printf("Get wsc_ER number error!");
				return;
#endif
			...
			if ( int_val > entryNum ) {
#ifdef RTL_FLASH_TOOL_SET_MULTI_MIB
				RTL_LOG_RC_SM(RTL_RC__MIB_SET_ERROR, "MIB[%s] Element number is too large! ", name)
#else
				printf("Element number is too large!\n");
				return;
#endif				
			...
			*((char *)&wsc_ER) = (char)int_val;
			if ( !APMIB_GET(MIB_WLAN_WSC_ER_TBL, (void *)&wsc_ER)) {
#ifdef RTL_FLASH_TOOL_SET_MULTI_MIB
				RTL_LOG_RC_SM(RTL_RC__MIB_SET_ERROR, "MIB[%s] Get table entry error! ", name)
#else
				printf("Get table entry error!");
				return;
#endif				
			...
			id = MIB_WLAN_WSC_ER_DELALL;
		}
#ifdef RTL_FLASH_TOOL_SET_MULTI_MIB		
		else{
			RTL_LOG_RC_SM(RTL_RC__MIB_SET_ERROR, "MIB[%s] unknown table operation! ", name)
		}
#endif			
		value = (void *)&wsc_ER;
		break;
	...
	case TUNNEL6_T:
		 memset(&tunnelCfgParam, 0, sizeof(tunnelCfgParam));
...
#endif
#if defined(CONFIG_APP_TR069)				
		else if ( pTbl[idx].id == MIB_CWMP_BRIDGE_TBL){
			APMIB_GET(MIB_CWMP_BRIDGE_TBL_NUM, (void *)&num);
		}
#ifdef CHECK_IPV4_DHCP_OPTION
		else if ( pTbl[idx].id == MIB_DHCP_OPTION60_TBL){
			APMIB_GET(MIB_DHCP_OPTION60_TBL_NUM, (void *)&num);
		}
		else if ( pTbl[idx].id == MIB_DHCP_OPTION125_TBL){
			APMIB_GET(MIB_DHCP_OPTION125_TBL_NUM, (void *)&num);
		}
#endif
#endif
...
#endif //ROUTE
#ifdef CHECK_IPV6_DHCP_OPTION
		else if ( pTbl[idx].id == MIB_DHCP6_OPTION16_TBL)
			APMIB_GET(MIB_DHCP6_OPTION16_TBL_NUM, (void *)&num);
		else if ( pTbl[idx].id == MIB_DHCP6_OPTION17_TBL)
			APMIB_GET(MIB_DHCP6_OPTION17_TBL_NUM, (void *)&num);		
#endif
...
static void showAllMibName(void)
{
	...
	while (config_area++ < MIB_WLAN_AREA+1) {
		...
		else if(config_area == CUSTOMER_HW_AREA)
		{
			...
			while (pTbl[idx].id) {
#ifdef IS_AX_SUPPORT
				if(is_slope_k(pTbl[idx].id)==SLOPE_K_5G)
					printf(SLOPE_K_PREFIX_5G"%s\n", pTbl[idx].name);
				else
#endif
...
static void showDhcp6Opt16Help(void)
{
	printf("flash set DHCP6_OPTION16_TBL cmd\n");
	printf("cmd:\n");
	printf("   add Enable/Type/ValueMode/Value  -- add a dhcp6 option16 entry.\n");
	printf("   del entry-number -- delete a dhcp6 option16 entry.\n");
	printf("   delall -- delete all dhcp6 option16 entry.\n");
}
static void showDhcp6Opt17Help(void)
{
	printf("flash set DHCP6_OPTION17_TBL cmd\n");
	printf("cmd:\n");
	printf("   add Enable/Type/ValueMode/Value  -- add a dhcp6 option17 entry.\n");
	printf("   del entry-number -- delete a dhcp6 option17 entry.\n");
	printf("   delall -- delete all dhcp6 option17 entry.\n");
}
#endif
#endif
...
#endif /* #if defined(_PRMT_X_TELEFONICA_ES_DHCPOPTION_) */
#ifdef CHECK_IPV6_DHCP_OPTION
	DHCP6_OPTION16_Tp pDhcp6Opt16;
	DHCP6_OPTION17_Tp pDhcp6Opt17;
#endif
...
//#endif
	...
	case WSC_ER_ARRAY_T:
		...
		if (p1 == NULL || p2==NULL || p3==NULL) {
			printf("Invalid WSC_ER in argument!\n");
			break;
		}
		pWscER = (WSC_ER_Tp)(((long)pMib)+mib_table[i].offset+wscERNum*sizeof(WSC_ER_T));
		pWscER->wscERDisable = atoi(p1);
		//strcpy(pWscER->wscERUUID, p2);
		//strcpy(pWscER->wscERDeviceName, p3);
		snprintf(pWscER->wscERUUID, UUID_STR_LEN, "%s", p2);
		snprintf(pWscER->wscERDeviceName, MAX_ER_DEVICENAME_LEN, "%s", p3);					
		wscERNum++;
		//printf("[%s %d]%d %s %s %d\n",pWscER->wscERDisable, pWscER->wscERUUID, pWscER->wscERDeviceName, wscERNum);
		break;
#endif		
#ifdef HOME_GATEWAY
	case MACFILTER_ARRAY_T:
	...
	case DHCP6_OPTION16_ARRAY_T:		
		...
		getVal5((char *)value, &p1, &p2, &p3, &p4, &p5);
		if (p1 == NULL || p2 == NULL || p3 == NULL || p4==NULL) {
			printf("Invalid DHCP6 OPTION16 arguments!\n");
			break;
		}
		pDhcp6Opt16 = (DHCP6_OPTION16_Tp)(((long)pMib)+mib_table[i].offset+dhcp6Option16Num*sizeof(DHCP6_OPTION16_T));
		pDhcp6Opt16->enabled=(unsigned char)atoi(p1);
		pDhcp6Opt16->type=(unsigned int)atoi(p2);
		pDhcp6Opt16->valueMode=(unsigned int)atoi(p3);
		strcpy( pDhcp6Opt16->value, p4 );
		dhcp6Option16Num++;
		break;
	case DHCP6_OPTION17_ARRAY_T:		
		printf("%s %d\n",__FUNCTION__,__LINE__);
		getVal5((char *)value, &p1, &p2, &p3, &p4, &p5);
		if (p1 == NULL || p2 == NULL || p3 == NULL || p4==NULL) {
			printf("Invalid DHCP6 OPTION17 arguments!\n");
			break;
		}
		pDhcp6Opt17 = (DHCP6_OPTION16_Tp)(((long)pMib)+mib_table[i].offset+dhcp6Option17Num*sizeof(DHCP6_OPTION17_T));
		pDhcp6Opt17->enabled=(unsigned char)atoi(p1);
		pDhcp6Opt17->type=(unsigned int)atoi(p2);
		pDhcp6Opt17->suboptCode=(unsigned int)atoi(p3);
		strcpy( pDhcp6Opt17->subOptionData, p4 );
		strcpy( pDhcp6Opt17->value, p5 );
		dhcp6Option17Num++;
		break;		
#endif
...
typedef struct _BssDscr {
	...
	unsigned char meshid[MESHID_LEN];
	unsigned char	*meshidptr;
	unsigned short	meshidlen;
#endif 
    BssType bsstype;
    unsigned short beacon_prd;			// beacon period in Time Units
    unsigned char dtim_prd;			// DTIM period in beacon periods
    unsigned long t_stamp[2];			// 8 Octets from ProbeRsp/Beacon
    IbssParms ibss_par;			// empty if infrastructure BSS
	...
	size_t WholeFrameLength;
	unsigned char  WholeFrame[PROBE_FRAME_SIZE];	/* (timestamp, beacon interval, and capability information) */

	unsigned char mdid[2];
	unsigned char ft_cap;
	unsigned int	bss_akm_suite;
} BssDscr, *pBssDscr;
...
#if 1
		pmib->wscEntry.dot11DesiredSSIDLenBak = intVal2;
		memset(pmib->wscEntry.dot11DesiredSSIDbak, 0, 32);
		memcpy(pmib->wscEntry.dot11DesiredSSIDbak, buf1, intVal2);		
#endif
...
#ifdef RTK_MBO_SUPPORT
			apmib_get(MIB_WLAN_IEEE80211W,(void *)&intVal);
			if (0 == intVal && vwlan_idx != NUM_VWLAN_INTERFACE) {
...
/*97G 8812F 8814B is 0.25dBm per number, rather than is 0.5dBm*/
		...
		if(chipVersion == CHIP_RTL8197G || chipVersion == CHIP_RTL8812F || chipVersion == CHIP_RTL8814B)
			intVal<<=1;
#endif
...
//!CONFIG_RTL_8196B => rtl8651c+rtl8190
		...
		if(intVal==0)
		{
			pmib->meshCSA.mesh_csa_enable = 1;
		}
		else
		{
			pmib->meshCSA.mesh_csa_enable = 0;
		}
#endif
		...
		if(intVal==0)
		{
			pmib->sr_profile.csa_ready_timeout = 15;
		}
		else
		{
			pmib->sr_profile.csa_ready_timeout = 0;
		}
	...
	// set band
		...
		pmib->dot11OperationEntry.guest_access = intVal;
			...
			if ((strlen(map_ip) != 0) && (strlen(map_port) != 0)){
...
*/
	...
	if (vwlan_idx > 0 && pmib->dot11OperationEntry.guest_access)
		pmib->dot11OperationEntry.block_relay = 1;	
			...
			{
				pmib->dot1180211AuthEntry.dot11IEEE80211W = 2;
				pmib->dot1180211AuthEntry.dot11EnableSHA256 = 1;
			}

			if(encrypt == ENCRYPT_WPA2_WPA3_MIXED)
			{
				pmib->dot1180211AuthEntry.dot11IEEE80211W = 1;
				pmib->dot1180211AuthEntry.dot11EnableSHA256 = 1;
			}
#endif
#endif
...
&& encrypt < 7
	...
	{
		if (encrypt == ENCRYPT_WPA)
			intVal = PSK_ENABLE_WPA;
		else if (encrypt == ENCRYPT_WPA2)
			intVal = PSK_ENABLE_WPA2;
		else if (encrypt == ENCRYPT_WPA2_MIXED)
			intVal = (PSK_ENABLE_WPA|PSK_ENABLE_WPA2);
		else if (encrypt == ENCRYPT_WPA3)
			intVal = (PSK_ENABLE_WPA3);
		else if (encrypt == ENCRYPT_WPA2_WPA3_MIXED)
			intVal = (PSK_ENABLE_WPA2|PSK_ENABLE_WPA3);
		else {
			printf("invalid ENCRYPT value!\n");
			return -1;
		}
#ifdef WLAN_WPA3
		if(mode == CLIENT_MODE && (encrypt &ENCRYPT_WPA2) ){
			intVal |= PSK_ENABLE_WPA3;
		}
#endif
...
#if defined(CONFIG_WLAN_HAL_8812FE) || defined(CONFIG_WLAN_HAL_8197G)
	apmib_get(MIB_TELCO_SELECTED, (void *)&intVal);
	pmib->miscEntry.telco_selected = intVal;
#endif
#ifdef CONFIG_RTL_COMAPI_CFGFILE 
...
#if defined(WIFI_SIMPLE_CONFIG) && defined(CONFIG_APP_TR069)
#define WRITE_WSC_PARAM_EXT(dst, tmp, str, val1, val2) {	\
...
static int updateWscConf(char *in, char *out, int genpin, char *wlanif_name)
{
    ...
    int wlan0_wsc_disabled=0,wlan0_mesh_enabled=0, wlan0_mesh_enc=0, wlan0_mesh_cipher=0, wlan0_mesh_mode=0, wlan0_mesh_channel=0;
    unsigned char wlan0_meshid[100], wlan0_mesh_psk[100], wlan0_mesh_reserved[201];
#ifdef FOR_DUAL_BAND	
    int wlan1_wsc_disabled=0, wlan1_mesh_enabled=0, wlan1_mesh_enc=0, wlan1_mesh_cipher=0, wlan1_mesh_mode=0, wlan1_mesh_channel=0;
...
#define WSC_WPA2_AES		8
...
#ifdef CONFIG_RTK_MESH 
    apmib_get(MIB_WLAN_CHANNEL, (void *)&wlan0_mesh_channel);
    apmib_get(MIB_WLAN_MESH_ENABLE, (void *)&wlan0_mesh_enabled);
    apmib_get(MIB_WLAN_MESH_ID, (void *)&wlan0_meshid);
    apmib_get(MIB_WLAN_MESH_ENCRYPT, (void *)&wlan0_mesh_enc);
    apmib_get(MIB_WLAN_MESH_WPA2_CIPHER_SUITE, (void *)&wlan0_mesh_cipher);
    apmib_get(MIB_WLAN_MESH_WPA_PSK, (void *)&wlan0_mesh_psk);
    apmib_get(MIB_WLAN_MESH_WPS_MODE, (void *)&wlan0_mesh_mode);
    apmib_get(MIB_WLAN_MESH_RESERVED, (void *)&wlan0_mesh_reserved);
#endif    
    ...
    if(wlan0_wlan_disabled==0 && wlan0_mode==AP_MESH_MODE) { 
    ...
    WRITE_WSC_PARAM(ptr, tmpbuf, "wlan0_mesh_enabled = %d\n", wlan0_mesh_enabled); 
    // 1104
	...
	if(wlan0_mesh_enabled) {
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh_channel = %d\n", wlan0_mesh_channel);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh_id = \"%s\"\n", wlan0_meshid);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh_enc = %d\n", wlan0_mesh_enc);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh_cipher = %d\n", wlan0_mesh_cipher);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh_network_key = \"%s\"\n", wlan0_mesh_psk);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh_wps_mode = %d\n", wlan0_mesh_mode);	
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh_reserved = \"%s\"\n", wlan0_mesh_reserved);
	}
...
#endif
	apmib_get(MIB_WLAN_CHANNEL, (void *)&wlan1_mesh_channel);
#ifdef CONFIG_RTK_MESH 
	apmib_get(MIB_WLAN_MESH_ENABLE, (void *)&wlan1_mesh_enabled);
	apmib_get(MIB_WLAN_MESH_ID, (void *)&wlan1_meshid);
	apmib_get(MIB_WLAN_MESH_ENCRYPT, (void *)&wlan1_mesh_enc);
	apmib_get(MIB_WLAN_MESH_WPA2_CIPHER_SUITE, (void *)&wlan1_mesh_cipher);
	apmib_get(MIB_WLAN_MESH_WPA_PSK, (void *)&wlan1_mesh_psk);
	apmib_get(MIB_WLAN_MESH_WPS_MODE, (void *)&wlan1_mesh_mode);
	apmib_get(MIB_WLAN_MESH_RESERVED, (void *)&wlan1_mesh_reserved);
#endif
    ...
    if(wlan1_wlan_disabled==0 && wlan1_mode==AP_MESH_MODE) {
    ...
    WRITE_WSC_PARAM(ptr, tmpbuf, "wlan1_mesh_enabled = %d\n", wlan1_mesh_enabled);
	...
	if(wlan1_mesh_enabled) {
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh2_channel = %d\n", wlan1_mesh_channel);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh2_id = \"%s\"\n", wlan1_meshid);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh2_enc = %d\n", wlan1_mesh_enc);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh2_cipher = %d\n", wlan1_mesh_cipher);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh2_network_key = \"%s\"\n", wlan1_mesh_psk);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh2_wps_mode = %d\n", wlan1_mesh_mode);
		WRITE_WSC_PARAM(ptr, tmpbuf, "mesh2_reserved = \"%s\"\n", wlan1_mesh_reserved);
	}
	...
	if( auto_lockdown1 || auto_lockdown2)
		...
		auto_lockdown = 0;
	WRITE_WSC_PARAM(ptr, tmpbuf, "auto_lockdown = %d\n", auto_lockdown);
	...
	WRITE_WSC_PARAM(ptr, tmpbuf, "external_reg_num = %d\n", entryNum);
	if(entryNum){
		for(ii=1;ii<=entryNum;ii++)
		{
			...
			*((char *)&extReg) = (char)ii;
			...
			{				
				WRITE_WSC_PARAM_EXT(ptr, tmpbuf, "ER_disable%d = %d\n", ii, extReg.disable);
				WRITE_WSC_PARAM_EXT(ptr, tmpbuf, "ER_uuid%d= %s\n", ii, extReg.uuid);
				WRITE_WSC_PARAM_EXT(ptr, tmpbuf, "ER_devname%d = %s\n", ii, extReg.devicename);								
			}
		}		
	}
#endif
...
#endif
#ifdef BACKUP_CURRENT_SETTING_SUPPORT
		case BACKUP_CURRENT_SETTING_OFFSET:
			pcomp_sig = COMP_BC_SIGNATURE;
			type = BACKUP_CURRENT_SETTING;
			break;
#endif
#ifdef BACKUP_DEFAULT_SETTING_SUPPORT
		case BACKUP_DEFAULT_SETTING_OFFSET:
			pcomp_sig = COMP_BD_SIGNATURE;
			type = BACKUP_DEFAULT_SETTING;
			break;
#endif
#ifdef RF_DPK_SETTING_SUPPORT
		case RF_DPK_SETTING_OFFSET:
			pcomp_sig = COMP_RS_SIGNATURE;
			type = RF_DPK_SETTING;
			break;
#endif /* RF_DPK_SETTING_SUPPORT */
...
switch(type)
{
	...
	flash_read_withtype ((char *)&compHeader, offset, sizeof(COMPRESS_MIB_HEADER_T),type );
	...
	) //check whether compress mib data
	{
		...
		flash_read_withtype ((char *)compPtr, offset, compLen+sizeof(COMPRESS_MIB_HEADER_T),type );
...
int flash_mib_compress_write(CONFIG_DATA_T type, char *data, PARAM_HEADER_T *pheader, unsigned char *pchecksum)
{
...
#endif
	{
		...
		if ( flash_write_withtype ((char *)compPtr, dst, compLen+sizeof(COMPRESS_MIB_HEADER_T),type )==0 )
