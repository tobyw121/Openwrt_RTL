...
void formPortFw(request *wp, char *path, char *query)
{
	...
	if (strAddPort[0]) {
		...
		if(!pfWanMask){
			strcpy(tmpBuf, "<script>dw(portfw_wan_not_select)</script>");
			goto setErr_portfw;
		}	
	//	printf("[%s:%d] pfWanMask = %d\n",__FUNCTION__, __LINE__,pfWanMask);
		...
		if ( (v1 & v3) != (v2 & v3) ) {
			strcpy(tmpBuf, "<script>dw(portfw_ip_error)</script>");
			goto setErr_portfw;
		}
		...
		if (strRmtIp[0]) {			
			...
			if ( (v1 & v3) == (v2 & v3) ) {
				strcpy(tmpBuf, "<script>dw(portfw_ip_error2)</script>");
				goto setErr_portfw;
			}
		}
		#endif
		...
		if ( strComment[0] ) {
			if (strlen(strComment) > COMMENT_LEN-1) {
				strcpy(tmpBuf, ("<script>dw(ipfilter_comm_oversize)</script>"));
				goto setErr_portfw;
			}
		...
		//	printf("[%s:%d] i = %d\n",__FUNCTION__, __LINE__,i);
			...
			if ( (entryNum + 1) > MAX_FILTER_NUM) {
				strcpy(tmpBuf, "<script>dw(portfw_table_full)</script>");
				goto setErr_portfw;
			}			
			...
			for (j=1; j<=entryNum; j++) {
				...
				*((char *)&checkEntry) = (char)j;
				...
				if ( ( (entry.fromPort <= checkEntry.fromPort && entry.toPort >= checkEntry.fromPort) ||
					...
					strcpy(tmpBuf, "<script>dw(portfw_range_overlapped)</script>");
					goto setErr_portfw;
				}
			}
...
void formFilter(request *wp, char *path, char *query)
{
	...
	if (strAddIp[0]) {
		...
		{
			strcpy(tmpBuf, ("<script>dw(fmfwall_not_set_ip_addr)</script>"));
			goto setErr_filter;
		}
		...
		if ( (v1 & v3) != (v2 & v3) ) { 
			strcpy(tmpBuf, ("<script>dw(fmfwall_invalid_ip_addr)</script>"));
			goto setErr_filter;
		}
		if ( v1 == v2 ) {
			strcpy(tmpBuf, "<script>dw(dmz_ip_error2)</script>");
		...
		for(j=1;j<=entryNum;j++)
		{
			...
			*((char *)&macEntrytmp) = (char)j;
			if ( apmib_get(MIB_MACFILTER_TBL, (void *)&macEntrytmp))
			{
				...
				{
					strcpy(tmpBuf, ("<script>dw(ipfilter_rule_exist)</script>"));
					goto setErr_filter;
				}
			...
			*((char *)&entrytmp) = (char)j;
			if ( apmib_get(MIB_PORTFILTER_TBL, (void *)&entrytmp))
			{
				if ((entrytmp.fromPort == portEntry.fromPort) &&
					...
					((entrytmp.protoType==PROTO_UDP)&&portEntry.protoType==PROTO_BOTH)))
					{
						strcpy(tmpBuf, ("<script>dw(portfiletr_overlap)</script>"));
						goto setErr_filter;
					}
					if ((((entrytmp.fromPort >= portEntry.fromPort) &&
					(entrytmp.fromPort <= portEntry.toPort))||
			...
			if ( apmib_get(MIB_IPFILTER_TBL, (void *)&ipentrytmp))
			{
				...
				if (strEndIpAddr[0])
				{
					if (((*((unsigned int*)ipentrytmp.ipAddr)) == (*((unsigned int*)ipEntry.ipAddr)))&&
						...
						#ifdef CONFIG_IPV6
						&&(atoi(ip4Enable)==1)&&(ipentrytmp.ipVer == IPv4)
						#endif
						)
					{
						strcpy(tmpBuf, ("<script>dw(ipfilter_rule_exist)</script>"));
					...
					{
						strcpy(tmpBuf, ("<script>dw(ipfilter_ip_overlap)</script>"));
						goto setErr_filter;
					}
	...
	if (strAddIp[0] || strAddPort[0] || strAddMac[0] || strAddUrl[0]) {
		...
		if (strAddUrl[0])
		{
			if ( (entryNum + 1) > MAX_URLFILTER_NUM) {
				strcpy(tmpBuf, ("<script>dw(urlfilter_entry_full)</script>"));
				goto setErr_filter;
			}
		}
		else
...
void formDMZ(request *wp, char *path, char *query)
{
	...
	if (strSave[0]) {
		...
		if ( apmib_set(MIB_DMZ_HOST, (void *)&ipAddr) == 0) {
			strcpy(tmpBuf, "<script>dw(dmz_set_mib_error)</script>");
			goto setErr_dmz;
		}
	}
	...
	for (i=1; i<=entryNum; i++) {	
		*((char *)&entry) = (char)i;
		...
		if ( !strcmp(rmtip, "0.0.0.0"))
			rmtip = "<script>dw(portfw_any)</script>";
		
		sprintf(rmt_ip_buff, "%s", rmtip);
      
        if(entry.enabled)
			strcpy(enable, ("<script>dw(portfw_enable)</script>"));
		else
			strcpy(enable, ("<script>dw(portfw_disable)</script>"));

#ifdef MULTI_WAN_SUPPORT
		snprintf(wanBuf, sizeof(wanBuf), "WAN%d", entry.WANIfIndex);

		if(entry.enabled)
			strcpy(enable, ("<script>dw(fmfwall_enable)</script>"));
		else
			strcpy(enable, ("<script>dw(fmfwall_disable)</script>"));	
		...
		nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
			"<td align=center width=\"15%%\" ><font size=\"2\">%s</td>\n"
				...
				ip_buff, portRange, type, rmt_ip_buff, extPortRange, wanBuf, enable, entry.comment, i);
#else
		nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
			"<td align=center width=\"15%%\" ><font size=\"2\">%s</td>\n"
      			"<td align=center width=\"10%%\" ><font size=\"2\">%s</td>\n"
				...
				"<td align=center width=\"10%%\" ><font size=\"2\">%s</td>\n"
      			"<td align=center width=\"10%%\" ><input type=\"checkbox\" name=\"select%d\" value=\"ON\"></td></tr>\n"),
				ip_buff, portRange, type, rmt_ip_buff, extPortRange, enable, entry.comment, i);
...
#ifdef CONFIG_IPV6
      	"<td align=center ><font size=\"2\"><b><script>dw(portfiletr_tbl_ipversion)</script></b></font></td>\n"
#endif
...
int macFilterList(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"50%%\" ><font size=\"2\"><b><script>dw(macfilter_macaddr)</script></b></font></td>\n"
      	"<td align=center width=\"30%%\" ><font size=\"2\"><b><script>dw(macfilter_comment)</script></b></font></td>\n"
      	"<td align=center width=\"20%%\" ><font size=\"2\"><b><script>dw(macfilter_select)</script></b></font></td></tr>\n"));
...
int ipQosList(request *wp, int argc, char **argv)
{
	...
	strcpy(_mode1, ("<script>dw(ip_qos_guarantee_min_bandwidth)</script>"));
	strcpy(_mode2, ("<script>dw(ip_qos_restricted_max_bandwidth)</script>"));
...
#if defined(ADVANCED_IPQOS)
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_name)</script></b></font></td>\n"
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_ipversion)</script></b></font></td>\n"
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_proto)</script></b></font></td>\n"
#endif
      	"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_localaddr)</script></b></font></td>\n"
#if defined(ADVANCED_IPQOS)
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_localport)</script></b></font></td>\n"
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_remote_addr)</script></b></font></td>\n"
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_remote_port)</script></b></font></td>\n"
#endif
#if defined(CONFIG_IPV6)
			"<td align=center width=\"20%%\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_local_ipv6addr)</script></b></font></td>\n"
#endif
      	"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_macaddr)</script></b></font></td>\n"));
#if defined(ADVANCED_IPQOS)
		if(staticIpSupport)
			nBytesSent += req_format_write(wp, ("<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_remote_macaddr)</script></b></font></td>\n"));

		nBytesSent += req_format_write(wp, ("<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_Phyport)</script></b></font></td>\n"
...
#endif
      	"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_mode)</script></b></font></td>\n"
      	"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_upband)</script></b></font></td>\n"
      	"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_downband)</script></b></font></td>\n"));
      	
#if defined(ADVANCED_IPQOS)
		nBytesSent += req_format_write(wp, (
#if defined(CONFIG_RTL_HW_QOS_SUPPORT) 
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_priority)</script></b></font></td>\n"
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_weight)</script></b></font></td>\n"
#endif
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_dscp_remark)</script></b></font></td>\n"));
...
#else
		nBytesSent += req_format_write(wp, (
		"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_comm)</script></b></font></td>\n"));
#endif
		nBytesSent += req_format_write(wp, (
      	"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(ip_qos_tbl_select)</script></b></font></td></tr>\n"));
...
//displayPostDate(wp->post_data);
	...
	if (strAdd[0]) {
		...
		if(totalUplinkBw > MAX_BAND_WIDTH){
			strcpy(tmpBuf, ("<script>dw(traffic_uplink_exceed)</script>"));
			goto setErr;
		}

		if(totalDownlinkBw > MAX_BAND_WIDTH){
			strcpy(tmpBuf, ("<script>dw(traffic_downlink_exceed)</script>"));
			goto setErr;
		}
				...
				if ( apmib_get(MIB_QOS_RULE_TBL, (void *)&entry_tmp)){
					...
					{						
							...
							strIpStart[0] && strLocalPortStart[0])
						{
							...
							{
								strcpy(tmpBuf, ("<script>dw(ip_qos_conflict)</script>"));
								goto setErr;
							}
						}
						else if(entry_tmp.local_ip_start[0] && strIpStart[0])
						{
							if(((ips.s_addr < ip1) && (ipe.s_addr < ip1)) || 
								...
								strcpy(tmpBuf, ("<script>dw(ip_qos_ip_conflict)</script>"));
								goto setErr;
							}
						}
						else if(entry_tmp.local_port_start && strLocalPortStart[0])
						{
							if(((lPortS < entry_tmp.local_port_start) && (lPortE < entry_tmp.local_port_start)) || 
								...
								strcpy(tmpBuf, ("<script>dw(ip_qos_port_conflict)</script>"));
								goto setErr;
							}
						}
							...
							strRmtIpStart[0] && strRmtPortStart[0])
						{
							...
							{
								strcpy(tmpBuf, ("<script>dw(ip_qos_remote_conflict)</script>"));
								goto setErr;
							}
						}
						else if(entry_tmp.remote_ip_start[0] && strRmtIpStart[0])
						{
							if(((rIps.s_addr < ip3) && (rIpe.s_addr < ip3)) || 
								...
								strcpy(tmpBuf, ("<script>dw(ip_qos_remote_ip_conflict)</script>"));
								goto setErr;
							}
						}
						else if(entry_tmp.remote_port_start && strRmtPortStart[0])
						{
							if(((rPortS < entry_tmp.remote_port_start) && (rPortE < entry_tmp.remote_port_start)) || 
								...
								strcpy(tmpBuf, ("<script>dw(ip_qos_remote_port_conflict)</script>"));
								goto setErr;
							}
						}
				...
				if ( apmib_get(MIB_QOS_RULE_TBL, (void *)&entry_tmp))
				{
					...
					{						
							...
							(rmac[0] || rmac[1] || rmac[2] || rmac[3] || rmac[4] || rmac[5]))
						{
							...
							{
								strcpy(tmpBuf, ("<script>dw(ip_qos_remote_mac_conflict)</script>"));
								goto setErr;
							}
						}
						...
						{					
							...
							{
								strcpy(tmpBuf, ("<script>dw(ip_qos_mac_conflict)</script>"));
								goto setErr;
							}
						}				
					}
				...
				if ( apmib_get(MIB_QOS_RULE_TBL, (void *)&entry_tmp))
				{
					if(entry_tmp.mode & QOS_RESTRICT_PHYPORT)
					{
						if(entry_tmp.phyport==atoi(strPhyport)){
							strcpy(tmpBuf, ("<script>dw(ip_qos_phy_conflict)</script>"));
							goto setErr;
						}
					}
				}
				...
				if ( apmib_get(MIB_QOS_RULE_TBL, (void *)&entry_tmp))
				{
					if(entry_tmp.mode & QOS_RESTRICT_DSCP)
					{
						if(entry_tmp.dscp==atoi(strDscp)){
							strcpy(tmpBuf, ("<script>dw(ip_qos_dscp_conflict)</script>"));
							goto setErr;
						}
					}
				}
				...
				if ( apmib_get(MIB_QOS_RULE_TBL, (void *)&entry_tmp))
				{
					if(entry_tmp.mode & QOS_RESTRICT_1P)
					{
						if(entry_tmp.vlan_pri==atoi(strVlan_pri)){
							strcpy(tmpBuf, ("<script>dw(ip_qos_1p_conflict)</script>"));
							goto setErr;
						}
					}
				}
		...
		{
			strcpy(tmpBuf, ("<script>dw(ip_qos_without_bandwith)</script>"));
				goto setErr;
		}

		if((entry.bandwidth!=0 || entry.bandwidth_downlink!=0) && no_rule_flag==1)
		{
			strcpy(tmpBuf, ("<script>dw(ip_qos_with_nothing)</script>"));
				goto setErr;
		}
		...
		if(entry.mode &  QOS_RESTRICT_MIN){
			...
			if(ret==QOS_BW_CHECK_FAIL){
				...
				strcpy(tmpBuf, ("<script>dw(ip_qos_BOTHLINK_BW_OVERSIZE)</script>"));
				goto setErr;
			}
			else if(ret==QOS_DOWNLINK_BW_OVERSIZE){
				strcpy(tmpBuf, ("<script>dw(ip_qos_DOWNLINK_BW_OVERSIZE)</script>"));
				goto setErr;
			}
			else if(ret==QOS_UPLINK_BW_OVERSIZE){
				strcpy(tmpBuf, ("<script>dw(ip_qos_UPLINK_BW_OVERSIZE)</script>"));
				goto setErr;
			}
		}
			...
			for(j=1;j<=entryNum;j++)
			{
				*((char *)&entry_tmp) = (char)j;
				if ( apmib_get(MIB_QOS_RULE_TBL, (void *)&entry_tmp))
				{
					...
					{
						strcpy(tmpBuf, ("<script>dw(ip_qos_entry_conflict)</script>"));
						goto setErr;
					}
				}
			}	
