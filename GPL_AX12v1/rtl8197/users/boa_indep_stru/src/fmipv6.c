...
#endif
#ifdef CONFIG_IP6IP_TUNNEL_SUPPORT
int getIP6IPRemoteAddr6Info(addr6CfgParam_t *entry)
{
	if ( !apmib_get(MIB_IPV6_IP6IP_REMOTE_ADDR6_PARAM,(void *)entry)){
		return -1 ;        
	}
	return 0;
}
...
int create_RadvdCfgFile()
{
	...
	if(NULL == fp)
		return -1;
	if (radvdcfg.enabled==1)
	create_radvdIntCfg(fp,&(radvdcfg.interface));
#ifdef CONFIG_APP_RADVD_WAN
	if (radvdcfg_wan.enabled==1)
...
void formRadvd(request *wp, char *path, char *query)
{
	...
	/*Set parameters*/
	//if((0 == strcmp(req_get_cstream_var(wp,"submit",""), "Save"))||(0 == strcmp(req_get_cstream_var(wp, ("save_apply"), ""), "Save & Apply")))
	strval0=req_get_cstream_var(wp,"submit","");
	strval1=req_get_cstream_var(wp, ("save_apply"), "");
	if(strval0[0]||strval1[0])
	{
...
#ifdef CONFIG_APP_RADVD_WAN
void formRadvd_wan(request *wp, char *path, char *query)
{
	int pid;
	char tmpBuf[256];
	char *submitUrl;
	...
	if((0 == strcmp(req_get_cstream_var(wp,"submit",""), "Save"))||(0 == strcmp(req_get_cstream_var(wp, ("save_apply"), ""), "Save & Apply")))
	{
		set_RadvdParam(wp, path, query,&radvdCfgParam_wan);
	}
...
int  set_DhcpSParam(request *wp, char *path, char *query, dhcp6sCfgParam_t *dhcp6sCfgParam)
{
	...
	apmib_get(MIB_IPV6_LAN_ORIGIN,&val);
	if(enable && val) // only update dhcp6s mib when lan manual mode & enable dhcp6s.
	{
...
void formDhcpv6s(request *wp, char *path, char *query)
{
	...
	if(strval0[0]||strval1[0])
	{
		...
		if(val){ // only update IPV6_ADDR_LAN_PARAM mib when lan manual mode.
...
void formIpv6Setup(request *wp, char *path, char *query)
{
		...
		char* strval;
		uint32 val;
		int repid_commit=0, i=0;
		struct in_addr Ip4addr;
#ifdef CONFIG_IP6IP_TUNNEL_SUPPORT
		addr6CfgParam_t remoteaddr6_ip6ip;
		addr6CfgParam_t localaddr6_ip6ip;
#endif
		/*Set to Parameters*/
...
#endif			
		/*if(0 == strcmp(strval, "Apply Changes")||0 == strcmp(strval, "Save")
#if defined(APPLY_CHANGE_DIRECT_SUPPORT)
		   || 0 == strcmp(strval, "Save & Apply")
#endif
		   )*/
        if(strval[0])
		{			
			...
			if(val){
...
#ifdef CONFIG_SIXRD_SUPPORT
				strval=req_get_cstream_var(wp,"6rdMode","");
				if(strval[0]){
					if(strcmp(strval,"6rdAuto")==0)
						val=SIXRD_AUTO;
					else
					{
						val=SIXRD_MANUAL;
					}
					apmib_set(MIB_SIXRD_MODE,&val);
				}
#endif				
				...
				int dslite_enable=0;
				strval = req_get_cstream_var(wp, ("dsliteEnabled"), "");
				
				if(strval!=NULL){
					//printf("[%s:%d] strVal=%s\n",__FUNCTION__,__LINE__, strval);
					dslite_enable= atoi(strval);
					//printf("[%s:%d] dslite_enable=%d\n",__FUNCTION__,__LINE__, dslite_enable);
				}
				
				if ( !apmib_set(MIB_DSLITE_ENABLE, (void *)&dslite_enable)) {
					printf ("Set MIB_DSLITE_ENABLE error!\n");
					return;
				}
				if (dslite_enable)
				{
					...
					if ( strval && strval[0] ) 
					{
						if (!strcmp(strval, ("dsliteAuto")))
							dslite = 0;
						else if (!strcmp(strval, ("dsliteManual")))
							dslite = 1;

			
						if ( !apmib_set(MIB_DSLITE_MODE, (void *)&dslite)) {
							printf("Set DSLITE MODE MIB error!\n");
							return;
						}
						...
						if(dslite == 1)
						{
							...
							if(strAFTR[0])
							{
								...
								{
									printf("Invalid AFTR address value!\n");
									return;
								}
								
								if ( !apmib_set(MIB_IPV6_ADDR_AFTR_PARAM, (void *)&ipaddr6))
								{
									printf("Set AFTR MIB error!\n");
									return;
								}
							}
							else
				...
				for(i=0;i<8;i++)
				{
					if(addr6_6rd_prefix.addrIPv6[i]!=0x0000)
					{
...
#endif
#ifdef CONFIG_IP6IP_TUNNEL_SUPPORT
				/* set ipv6 in ipv4 tunnel parameters */
				strval=req_get_cstream_var(wp,"ip6iplocal_ip_0","");
				if(strval!=NULL)
					localaddr6_ip6ip.addrIPv6[0]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6iplocal_ip_1","");
				if(strval!=NULL)
					localaddr6_ip6ip.addrIPv6[1]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6iplocal_ip_2","");
				if(strval!=NULL)
					localaddr6_ip6ip.addrIPv6[2]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6iplocal_ip_3","");
				if(strval!=NULL)
					localaddr6_ip6ip.addrIPv6[3]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6iplocal_ip_4","");
				if(strval!=NULL)
					localaddr6_ip6ip.addrIPv6[4]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6iplocal_ip_5","");
				if(strval!=NULL)
					localaddr6_ip6ip.addrIPv6[5]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6iplocal_ip_6","");
				if(strval!=NULL)
					localaddr6_ip6ip.addrIPv6[6]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6iplocal_ip_7","");
				if(strval!=NULL)
					localaddr6_ip6ip.addrIPv6[7]=strtol(strval,NULL,16);
				if(strval!=NULL)
				strval=req_get_cstream_var(wp,"prefix_len_ip6iplocal","");
				if(strval!=NULL)
					localaddr6_ip6ip.prefix_len=atoi(strval);
				for(i=0;i<8;i++)
				{
					if(localaddr6_ip6ip.addrIPv6[i]!=0x0000)
					{
						apmib_set(MIB_IPV6_ADDR_WAN_PARAM,&localaddr6_ip6ip);
						break;
					}
				}
				strval=req_get_cstream_var(wp,"ip6ipremote_ip_0","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.addrIPv6[0]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6ipremote_ip_1","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.addrIPv6[1]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6ipremote_ip_2","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.addrIPv6[2]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6ipremote_ip_3","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.addrIPv6[3]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6ipremote_ip_4","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.addrIPv6[4]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6ipremote_ip_5","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.addrIPv6[5]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6ipremote_ip_6","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.addrIPv6[6]=strtol(strval,NULL,16);
				strval=req_get_cstream_var(wp,"ip6ipremote_ip_7","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.addrIPv6[7]=strtol(strval,NULL,16);				
				strval=req_get_cstream_var(wp,"prefix_len_ip6ipremote","");
				if(strval!=NULL)
					remoteaddr6_ip6ip.prefix_len=atoi(strval);
				for(i=0;i<8;i++)
				{
					if(remoteaddr6_ip6ip.addrIPv6[i]!=0x0000)
					{
						apmib_set(MIB_IPV6_IP6IP_REMOTE_ADDR6_PARAM,&remoteaddr6_ip6ip);
						break;
					}
				}
				strval=req_get_cstream_var(wp,"remoteIP","");
				if ( strval[0] )
				{
					if ( !inet_aton(strval, &Ip4addr) )
					{
						printf ("Set ip6ip tunnel remote IP error!\n");
						return;
					}
					if ( !apmib_set(MIB_IPV6_IP6IP_REMOTE_ADDR, (void *)&Ip4addr))
					{
						printf("Set MIB_IPV6_IP6IP_REMOTE_ADDR error!\n");
					}
				}
#endif
...
void formMapE(request *wp, char *path, char *query)
{
	char *value;
	char *msg;
	char *submitUrl;
	...
	int val, len;
	int psid_len, mask_len, prefix_len, psid_offset;

	/* enable MAP-E */
	val = atoi(req_get_cstream_var(wp,"enable_mape",""));
	apmib_set(MIB_MAPE_ENABLE, &val);
	if(val == 0)
		goto OUT;

	/* MAP-E mode */
	value = req_get_cstream_var(wp,"mape_mode","");
	if(0 == strcmp(value, "mapeAuto"))
		val = 0;
	else
		val = 1;
	apmib_set(MIB_MAPE_MODE, &val);
	if(val == 0)
		goto OUT;

	/* MAP-E IPv6 prefix */
	value = req_get_cstream_var(wp,"ipv6_prefix","");
	if(value == NULL)
	{
		msg = "IPv6 prefix is null!";
		goto FAIL;
	}
	len = strlen(value);
	if(len >= 64)
	{
		msg = "IPv6 prefix is too long!";
		goto FAIL;
	}
	...
	if(p == NULL)
	{
		msg = "Invalid IPv6 prefix address!";
		goto FAIL;
	}
	q = p + 1;
	*p = '\0';
	if(inet_pton6(tmpstr, addr6.addrIPv6) == 0)
	{
		msg = "Invalid IPv6 prefix address!";
		goto FAIL;
	}
	...
	addr6.prefix_len = atoi(q);
	prefix_len = addr6.prefix_len;
	if(addr6.prefix_len < 0 || addr6.prefix_len > 64)
	{
		msg = "IPv6 prefix length must be between 0-64!";
		goto FAIL;
	}
	apmib_set(MIB_IPV6_MAPE_PREFIX, &addr6);

	/* MAP-E IPv4 prefix */
	value = req_get_cstream_var(wp,"ipv4_prefix","");
	if(value == NULL)
	{
		msg = "IPv4 prefix is null!";
		goto FAIL;
	}
	len = strlen(value);
	if(len >= 64)
	{
		msg = "IPv4 prefix is too long!";
		goto FAIL;
	}
	memcpy(tmpstr, value, len);
	tmpstr[len] = '\0';
	...
	if(p == NULL)
	{
		msg = "Invalid IPv4 prefix address!";
		goto FAIL;
	}
	q = p + 1;
	*p = '\0';
	if(inet_pton4(tmpstr, (unsigned char*)&addr4) == 0)
	{
		msg = "Invalid IPv4 prefix address!";
		goto FAIL;
	}
	...
	if(val < 0 || val > 32)
	{
		msg = "IPv4 mask length must be between 0-32!";
		goto FAIL;
	}
	apmib_set(MIB_IPV4_MAPE, &addr4);
	apmib_set(MIB_IPV4_MAPE_PREFIX_LEN, &val);

	/* MAP-E BR address */
	value = req_get_cstream_var(wp,"br_address","");
	if(value == NULL)
	{
		msg = "BR address is null!";
		goto FAIL;
	}
	len = strlen(value);
	if(len >= 64)
	{
		msg = "BR address is too long!";
		goto FAIL;
	}
	memcpy(tmpstr, value, len);
	tmpstr[len] = '\0';
	...
	else
	{
		...
		*p = '\0';
		...
		if(*q == '\0')
		{
			msg = "Invalid BR address!";
			goto FAIL;
		}
		addr6.prefix_len = atoi(q);
		if(addr6.prefix_len < 0 || addr6.prefix_len > 128)
		{
			msg = "BR address prefix length must be between 0-128!";
			goto FAIL;
		}
	}
	if(inet_pton6(tmpstr, addr6.addrIPv6) == 0)
	...
	apmib_set(MIB_IPV6_MAPE_BR_ADDR, &addr6);

	/* psid offset */
	val = atoi(req_get_cstream_var(wp,"psid_offset",""));
	psid_offset = val;
	if(val < 0 || val > 16)
	{
		msg = "PSID offset must be between 0-16!";
		goto FAIL;
	}
	apmib_set(MIB_MAPE_PSID_OFFSET, &val);

	/* psid length */
	val = atoi(req_get_cstream_var(wp,"psid_length",""));
	psid_len = val;
	if(val < 0 || val > 16)
	{
		msg = "PSID length must be between 0-16!";
		goto FAIL;
	}
	if((psid_offset+psid_len) > 16)
	{
		msg = "PSID offset + PSID length must be less than 16!";
		goto FAIL;
	}
	apmib_set(MIB_MAPE_PSID_LEN, &val);

	/* psid */
	value = req_get_cstream_var(wp,"psid","");
	if(value[0] != '0' && value[1] != 'x')
	{
		msg = "PSID must be hex!";
		goto FAIL;
	}
	...
	apmib_set(MIB_MAPE_PSID, &val);

	if((prefix_len + (32 - mask_len) + psid_len) > 64)
	{
		msg = "End-user IPv6 prefix length too long!";
		goto FAIL;
	}
...
#ifdef CONFIG_MAP_E_SUPPORT
		req_format_write(wp,"ipv6.addItem(\"MAP-E\", \"map_e.htm\", \"\", \"MAP-E\");\n");
#endif
...
//////////Lan Mode////////////////////////////////////////////////////////////
	if(!strcmp(name,"ipv6lanOrigin")){
		if(!apmib_get(MIB_IPV6_LAN_ORIGIN,&val)){
			fprintf(stderr, "Read MIB_IPV6_LAN_ORIGIN Error\n");	
			return -1;			
		}
		else{
...
//////////wan radvd///////////////////////////////////////////////////////////////
	...
	else if(!strcmp(name,("is_radvd_wan")))
	{
		req_format_write(wp,"1");
	}
#else
...
///////////////DHCPv6 server//////////////////////////////////////
...
#endif
#ifdef CONFIG_MAP_E_SUPPORT
	else if(!strcmp(name,("enable_mape")))
	{
		if ( !apmib_get(MIB_MAPE_ENABLE,(void *)&val)){
			return -1 ;
		}
		req_format_write(wp,"%d", val);
	}
	else if(!strcmp(name,("mape_mode")))
	{
		if ( !apmib_get(MIB_MAPE_MODE,(void *)&val)){
			return -1 ;
		}
		req_format_write(wp,"%d", val);
	}
	else if(!strcmp(name,("mape_ipv6_prefix")))
	{
		if ( !apmib_get(MIB_IPV6_MAPE_PREFIX,(void *)&addr6)){
			return -1 ;
		}
		req_format_write(wp,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
			addr6.addrIPv6[0], addr6.addrIPv6[1], addr6.addrIPv6[2],
			addr6.addrIPv6[3], addr6.addrIPv6[4], addr6.addrIPv6[5],
			addr6.addrIPv6[6], addr6.addrIPv6[7], addr6.prefix_len);
	}
	else if(!strcmp(name,("mape_ipv4_prefix")))
	{
		if ( !apmib_get(MIB_IPV4_MAPE,(void *)&addr4)){
			return -1 ;
		}
		if ( !apmib_get(MIB_IPV4_MAPE_PREFIX_LEN,(void *)&val)){
			return -1 ;
		}
		req_format_write(wp,"%s/%d", inet_ntoa(addr4), val);
	}
	else if(!strcmp(name,("mape_br_address")))
	{
		if ( !apmib_get(MIB_IPV6_MAPE_BR_ADDR,(void *)&addr6)){
			return -1 ;
		}
		req_format_write(wp,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
			addr6.addrIPv6[0], addr6.addrIPv6[1], addr6.addrIPv6[2],
	...
	else if(!strcmp(name,("mape_psid_offset")))
	{
		if ( !apmib_get(MIB_MAPE_PSID_OFFSET,(void *)&val)){
			return -1 ;
		}
		req_format_write(wp,"%d", val);
	}
	else if(!strcmp(name,("mape_psid_length")))
	{
		if ( !apmib_get(MIB_MAPE_PSID_LEN,(void *)&val)){
			return -1 ;
		}
		req_format_write(wp,"%d", val);
	}
	else if(!strcmp(name,("mape_psid")))
	{
		if ( !apmib_get(MIB_MAPE_PSID,(void *)&val)){
			return -1 ;
		}
		req_format_write(wp,"0x%x", val);
	}
	else if(!strcmp(name,("is_mape")))
	{
		req_format_write(wp,"1");
	}
#else
...
uint32 getIPv6WanInfo(request *wp, int argc, char **argv)
{
	...
	if(!strcmp(name,"ipv6WanEnabled")){
		...
		else if(!strcmp(name,"wan_sixrdMode")){
			val = 0;
#ifdef CONFIG_SIXRD_SUPPORT
			if(!apmib_get(MIB_SIXRD_MODE,&val)){
				fprintf(stderr, "Read MIB_SIXRD_MODE Error\n");
				return -1;			
			}
			else
#endif
	...
	else if(!strcmp(name,"lan_ipv6Addr_0")){
...
//			req_format_write(wp,"false");
...
#endif	
#ifdef CONFIG_DSLITE_SUPPORT
	else if(!strcmp(name,"dsliteEnabled")){
		if(!apmib_get(MIB_DSLITE_ENABLE,&val)){
			fprintf(stderr, "Read MIB_DSLITE_ENABLE Error\n");
			return -1;			
		}
		else{
			req_format_write(wp,"%d",val);
	...
	/* ULA address */
		...
		/*IPv6 in IPv4 TUNNEL*/
#ifdef CONFIG_IP6IP_TUNNEL_SUPPORT
		else if(!strcmp(name,"ip6ip_local_ip")){
			if(!apmib_get(MIB_WAN_DHCP,&val)){
				fprintf(stderr, "Read MIB_WAN_DHCP Error\n");
				return -1;			
			}
		...
		else if(!strcmp(name,"ip6ip_remote_ip")){
			memset(buffer,0x00,sizeof(buffer));
			if(!apmib_get( MIB_IPV6_IP6IP_REMOTE_ADDR,  (void *)buffer)){
				fprintf(stderr, "Read MIB_IPV6_IP6IP_REMOTE_ADDR Error\n");
				return -1;			
			}
			else{
				req_format_write(wp, "%s", inet_ntoa(*((struct in_addr *)buffer)));
		...
		else if(getIP6IPRemoteAddr6Info(&remoteaddr6_ip6ip)<0){
			fprintf(stderr, "Read IP6IP Remote Addr6 mib Error\n");
			return -1;
		}
		else if(!strcmp(name,"ip6ipremote_ipv6Addr_0")){
			req_format_write(wp,"%04x",remoteaddr6_ip6ip.addrIPv6[0]);
		}
		else if(!strcmp(name,"ip6ipremote_ipv6Addr_1")){
			req_format_write(wp,"%04x",remoteaddr6_ip6ip.addrIPv6[1]);
		}
		else if(!strcmp(name,"ip6ipremote_ipv6Addr_2")){
			req_format_write(wp,"%04x",remoteaddr6_ip6ip.addrIPv6[2]);
		}
		else if(!strcmp(name,"ip6ipremote_ipv6Addr_3")){
			req_format_write(wp,"%04x",remoteaddr6_ip6ip.addrIPv6[3]);
		}
		else if(!strcmp(name,"ip6ipremote_ipv6Addr_4")){
			req_format_write(wp,"%04x",remoteaddr6_ip6ip.addrIPv6[4]);
		}
		else if(!strcmp(name,"ip6ipremote_ipv6Addr_5")){
			req_format_write(wp,"%04x",remoteaddr6_ip6ip.addrIPv6[5]);
		}
		else if(!strcmp(name,"ip6ipremote_ipv6Addr_6")){
			req_format_write(wp,"%04x",remoteaddr6_ip6ip.addrIPv6[6]);
		}
		else if(!strcmp(name,"ip6ipremote_ipv6Addr_7")){
			req_format_write(wp,"%04x",remoteaddr6_ip6ip.addrIPv6[7]);
		}
		else if(!strcmp(name,"ip6ipremote_prefixLen")){
			req_format_write(wp,"%d",remoteaddr6_ip6ip.prefix_len);
		}
		else if(getWanAdd6Info(&localaddr6_ip6ip)<0){
			fprintf(stderr, "Read IP6IP Local Addr6 mib Error\n");
			return -1;
		}
		else if(!strcmp(name,"ip6iplocal_ipv6Addr_0")){
			req_format_write(wp,"%04x",localaddr6_ip6ip.addrIPv6[0]);
		}
		else if(!strcmp(name,"ip6iplocal_ipv6Addr_1")){
			req_format_write(wp,"%04x",localaddr6_ip6ip.addrIPv6[1]);
		}
		else if(!strcmp(name,"ip6iplocal_ipv6Addr_2")){
			req_format_write(wp,"%04x",localaddr6_ip6ip.addrIPv6[2]);
		}
		else if(!strcmp(name,"ip6iplocal_ipv6Addr_3")){
			req_format_write(wp,"%04x",localaddr6_ip6ip.addrIPv6[3]);
		}
		else if(!strcmp(name,"ip6iplocal_ipv6Addr_4")){
			req_format_write(wp,"%04x",localaddr6_ip6ip.addrIPv6[4]);
		}
		else if(!strcmp(name,"ip6iplocal_ipv6Addr_5")){
			req_format_write(wp,"%04x",localaddr6_ip6ip.addrIPv6[5]);
		}
		else if(!strcmp(name,"ip6iplocal_ipv6Addr_6")){
			req_format_write(wp,"%04x",localaddr6_ip6ip.addrIPv6[6]);
		}
		else if(!strcmp(name,"ip6iplocal_ipv6Addr_7")){
			req_format_write(wp,"%04x",localaddr6_ip6ip.addrIPv6[7]);
		}
		else if(!strcmp(name,"ip6iplocal_prefixLen")){
			req_format_write(wp,"%d",localaddr6_ip6ip.prefix_len);
		}
#endif
		else if(!strcmp(name,"ip6ip_comment_start")){
#ifdef CONFIG_IP6IP_TUNNEL_SUPPORT	
			req_format_write(wp, "");
#else
			req_format_write(wp, "<!--");
		...
		else if(!strcmp(name,"ip6ip_comment_end")){
#ifdef CONFIG_IP6IP_TUNNEL_SUPPORT	
			req_format_write(wp, "");
#else
			req_format_write(wp, "-->");
		...
		/* ip6to4 */
		else if(!strcmp(name,"ip6to4_comment_start")){
#ifdef CONFIG_IP6TO4_TUNNEL_SUPPORT	
			req_format_write(wp, "");
#else
			req_format_write(wp, "<!--");
#endif
		}
		else if(!strcmp(name,"ip6to4_comment_end")){
#ifdef CONFIG_IP6TO4_TUNNEL_SUPPORT	
			req_format_write(wp, "");
#else
			req_format_write(wp, "-->");
...
int getIPv6Status(request *wp, int argc, char **argv)
{
			...
			while((ret=fscanf(fp,"%s %x %s %x %s %x %x %x %x %s",
				...
				if(strcmp(ifname,devname)==0){							
					...
					if(memcmp(&dst_addr,&any_addr,sizeof(struct in6_addr))==0){
						...
						if(gw_addr.s6_addr16[0]==ntohs(0xFE80)){										
	...
	else if(!strcmp(name,"wan-dns-ipv6"))
	{
		if(!apmib_get(MIB_IPV6_DNS_AUTO,  (void *)&val))
		{
			printf("get MIB_IPV6_DNS_AUTO failed\n");
		...
		if(val==0)  //Set DNS Manually 
		{
			...
			sprintf(dns, "%04x%04x%04x%04x%04x%04x%04x%04x",
				addr6_dns.addrIPv6[0], addr6_dns.addrIPv6[1],
				addr6_dns.addrIPv6[2], addr6_dns.addrIPv6[3], 
				addr6_dns.addrIPv6[4], addr6_dns.addrIPv6[5],
				addr6_dns.addrIPv6[6], addr6_dns.addrIPv6[7]);
		}
		else if(val==1) ////Attain DNS Automatically 
		{

				if(!apmib_get(MIB_IPV6_DHCP_MODE,&val)){
					printf("Read MIB_IPV6_DHCP_MODE Error\n");
					return -1;			
				}
				#ifdef CONFIG_IPV6_RA_RDNSS_SUPPORT
				if (val==IPV6_DHCP_STATELESS) {      // stateless mode, get name server from rdnssd.conf
					fp = fopen(DNSV6_RDNSSD_FILE,"r");
				} else
				#endif
				{
					fp = fopen(DNSV6_ADDR_FILE,"r"); // stateful mode, get name server from dns6.config	
				}
				
				
				if(fp == NULL)
				{
					printf("dhcp mode =%d,Open dns config error!\n",val);
					return -1;
				}

				fgets(tmpstr, sizeof(tmpstr), fp);
				//printf("get tmp str = %s\n",tmpstr);

				fclose(fp);

				tmpval = strlen("nameserver");

				if(strncmp(tmpstr, "nameserver", tmpval))
				{
					printf("dns addr data error!\n");
					return -1;
				}
			...
			if (val==IPV6_DHCP_STATEFUL) {
				for(i=0; i<8; i++)
				{
					strncpy(&dns[i*4], &tmpstr[tmpval+1+i*5], 4);
				}
			} else {
						i=0;
						int j=0;
						int k=0;
						int m=0;
						char dns_temp[4]= {'0','0','0','0'};
						char dns_1[64]={0};
						char dns_2[64]={0};
						int is_next=0;
						int loop=0;
						int l;
						while ((tmpstr[tmpval+1+i]!='\0') && (tmpstr[tmpval+1+i]!='\n')) {
                            if (strncmp(&tmpstr[tmpval+1+i], "::", 2)==0){
								i=i+2;
								if (is_next) return -1;
								is_next=1;

								for (l=0;l<4-m;l++) {
								  for (loop=0;loop<3;loop++){	  //dns_temp shift.
										   dns_temp[3-loop]=dns_temp[2-loop];
								  }
								  dns_temp[0]='0';
								}

								
					            strncpy(&dns_1[j*4], dns_temp, 4);
								j++;
								for (loop=0;loop<4;loop++)   //dns_temp clear.
                                      dns_temp[loop]='0';
								m=0;
							}
						
						    else if (tmpstr[tmpval+1+i] == ':') {
                               ...
                               for (l=0;l<4-m;l++) {
							    ...
							    dns_temp[m++]= tmpstr[tmpval+1+i];   // record in dns_temp
							    i++;
							}
							
						}
						
						//printf("parsing: j=%d, dns_1= [%s], k=%d, dns_2=[%s] \n",j,dns_1,k,dns_2);
						//printf("parsing: dns_temp = %c %c %c %c \n",dns_temp[3],dns_temp[2],dns_temp[1],dns_temp[0]);

					// merge to full dns addr.
					if (j)
					   strncpy(&dns[0], dns_1, j*4);
					if (k)
					   strncpy(&dns[j*4], dns_2, k*4);

					for (loop=0;loop< (8-j-k)*4;loop++)  // fill 0
						dns[j*4+k*4+loop] = '0';
					...
					strncpy(&dns[28], dns_temp, 4);					

		   }
		   //printf("dns= [%s] \n",dns);
		}

		req_format_write(wp, "%s", dns);
		return 0;
	}
}
