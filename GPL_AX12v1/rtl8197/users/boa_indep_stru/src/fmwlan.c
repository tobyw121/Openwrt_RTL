...
#ifdef IS_AX_SUPPORT
#define VHT_RATE_ID	(0x100)
#else
#define VHT_RATE_ID	(0xA0)
#endif
#define HE_RATE_ID	(0x180)
...
const unsigned short HE_MCS_DATA_RATE[3][1][48] =
{
	{	{17, 34, 52, 69, 103, 138, 155, 172, 207, 229, 258, 287,
		 ...
		 69, 138, 207, 275, 413, 551, 619, 688, 826, 918, 1032, 1147}			// GI, 20MHz
	},

	{	{34, 69, 103, 138, 207, 275, 310, 344, 413, 459, 516, 574,
		 69, 138, 207, 275, 413, 551, 619, 688, 826, 918, 1032, 1147,
		 103, 206, 310, 413, 620, 826, 929, 1032, 1239, 1376, 1549, 1721,
		 138, 275, 413, 551, 826, 1101, 1239, 1377, 1652, 1835, 2065, 2294}		// GI, 40MHz
	},
	...
	{	{72, 144, 216, 288, 432, 577, 649, 721, 865, 961, 1081, 1201,
		 ...
		 288, 577, 865, 1153, 1729, 2306, 2594, 2882, 3459, 3843, 4324, 4804}	// GI, 80MHz
	}
...
#if 0//def IS_AX_SUPPORT
	if ((pid=find_pid_by_name("systemd")) > 0){
		snprintf(tmpBuf, MAX_MSG_BUFFER_SIZE, "kill -9 %d 2> /dev/null", pid);
		system(tmpBuf);
		if(isFileExist(HOSTAPD_ACTION_PATH))
			unlink(HOSTAPD_ACTION_PATH);
	}
#endif
...
void formSchedule(request *wp, char *path, char *query)
{
	...
	if ( !apmib_set(MIB_WLAN_SCHEDULE_DELALL, (void *)&entry)) {
			strcpy(tmpBuf, ("<script>dw(wl_Del_tab_ent)</script>"));
	...
	{
			strcpy(tmpBuf, ("<script>dw(wl_Get_ent_num)</script>"));
			goto setErr_schedule;
	}
	if ( !apmib_set(MIB_WLAN_SCHEDULE_ADD,(void *)&entry)) {
			strcpy(tmpBuf, ("<script>dw(wl_Add_tab_ent)</script>"));
			goto setErr_schedule;
	}
...
//displayPostDate(wp->post_data);	
	...
	if(strTmp[0])
	{
		...
		{
			strcpy(tmpBuf, ("<script>dw(wl_set_MIB_WLAN_SCHEDULE_ENABLED_err)</script>"));
			goto setErr_schedule;
		}
	}
	
	if ( !apmib_set(MIB_WLAN_SCHEDULE_DELALL, (void *)&entry)) {
			strcpy(tmpBuf, ("<script>dw(wl_set_MIB_WLAN_SCHEDULE_DELALL_err)</script>"));
			goto setErr_schedule;
	}
	...
	for(i=1; i<=MAX_SCHEDULE_NUM ; i++)
	{
		...
		*((char *)&entry) = (char)i;
		...
		{				
			strcpy(tmpBuf, ("<script>dw(wl_set_MIB_WLAN_SCHEDULE_ADD_error)</script>"));				
			goto setErr_schedule;
		}
	}
...
int getScheduleInfo(request *wp, int argc, char **argv)
{
	...
	if ( !strcmp(name_arg, ("wlan_state")) ) {
		bss_info bss;
		unsigned int is_ax_support=0, wlanMode=0,rpt_enable=0;
		if (getWlBssInfo(WLAN_IF, &bss) < 0){
			...
			get_ax_flag(WLAN_IF, &is_ax_support);
			apmib_get(MIB_WLAN_MODE, (void *)&wlanMode);
			if(wlan_idx==0)
				apmib_get(MIB_REPEATER_ENABLED1, (void *)&rpt_enable);
			else if(wlan_idx==1)
				apmib_get(MIB_REPEATER_ENABLED2, (void *)&rpt_enable);
			if(is_ax_support){
				if(wlanMode == CLIENT_MODE)
					strcpy((char *)buffer, "Enabled");
				if((wlanMode == AP_MODE) && rpt_enable)
					strcpy((char *)buffer, "Enabled");
			}
		}
	...
	if ( !apmib_get(MIB_WLAN_SCHEDULE_TBL_NUM, (void *)&entryNum)) {
  		strcpy(tmpBuf, "<script>dw(wl_Get_tab_ent)</script>:");
		return -1;
	}
...
int addWlProfileHandler(request *wp, char *tmpBuf, int wlan_id)
{
	...
	if(add_to_profile == 1)
	{
		...
		if ( (entryNum + 1) > MAX_WLAN_PROFILE_NUM) {
			strcpy(tmpBuf, ("<script>dw(wl_Can_add_new)</script>"));
			goto setErr_end;
		}
				...
				if(pStatus != NULL)         //pStatus is valid only after site survey
				{
				    ...
				    for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) {
					    ...
					    if(!strcmp(entry.ssid,ssidbuf)){
						    find = 1;
						    if (pBss->t_stamp[0] & 0x0000ffff) {//WPA
							    entry.encryption = 3;
	...
	if ( strVal[0] ) {
		...
		if (encrypt &  ENCRYPT_WPA2_MIXED) {
...
#else
				strcpy(tmpBuf, ("<script>dw(wlbasic_client_with_radius)</script>"));
				goto setErr_wlan;
#endif
			}
		}
			...
			if (use1x & 1) {
				...
				//Support wlan client mode with Enterprise (RADIUS)
#else
				strcpy(tmpBuf, ("<script>dw(wl_You_can_set)</script>"));
				goto setErr_wlan;
#endif
			}
		}
		...
		if ( apmib_set( MIB_WLAN_MACCLONE_ENABLED, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_wla_Mac)</script>"));
			goto setErr_wlan;
		}
	}

	if ( apmib_set( MIB_WLAN_MODE, (void *)&mode) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_MODE_err)</script>"));
		goto setErr_wlan;
	}
...
//printf("\r\n oriEntry[roop].ssid=[%s],__[%s-%u]\r\n",oriEntry[roop].ssid,__FILE__,__LINE__);
		...
		if(del_ret == 1)
		{
			...
			strcpy(tmpBuf, ("<script>dw(wl_Add_Pro_dup)</script>:"));
			goto setErr_wlan;
		}
		...
		if ( strSSID[0] ) {
			if ( apmib_set(MIB_WLAN_SSID, (void *)strSSID) == 0) {
	   	 			strcpy(tmpBuf, ("<script>dw(wl_Set_SSI_err)</script>:"));
					goto setErr_wlan;
			}
		}
...
//printf("\r\n oriEntry[roop].ssid=[%s],__[%s-%u]\r\n",oriEntry[roop].ssid,__FILE__,__LINE__);
	...
	if ( strDelAllProfile[0]) {
		...
		if ( !apmib_set(profile_delall_id, (void *)&entry)) {
			strcpy(tmpBuf, ("<script>dw(wl_Del_all_tab)</script>"));
			goto setErr_wlan;
		}
	}
...
//copy prefer band's ssid and encryption to non-prefer band
void wlan_band_steering_update()
{
	unsigned char prefer_band = 0;
	char prefer_band_ssid[MAX_SSID_LEN];
	int encrypt, wep, auth_type, wep_key_type, wpa_auth,
		wpa_cipher_suite, wpa_group_rekey_time, psk_format,
		wpa2_pre_auth, wpa2_cipher_suite;
	unsigned char wpa_psk[MAX_PSK_LEN + 1];
	unsigned char wep64Key1[WEP64_KEY_LEN];
	unsigned char wep64Key2[WEP64_KEY_LEN];
	unsigned char wep64Key3[WEP64_KEY_LEN];
	unsigned char wep64Key4[WEP64_KEY_LEN];
	...
	int ieee80211w, sha256_enable;
#endif

	apmib_get(MIB_PREFER_BAND, (void *)&prefer_band);
#ifdef CONFIG_BAND_5G_ON_WLAN0
	if ((prefer_band == PREFER_BAND_24G && wlan_idx == 0)
		|| (prefer_band == PREFER_BAND_5G && wlan_idx == 1))
#else
	if ((prefer_band == PREFER_BAND_24G && wlan_idx == 1)
		|| (prefer_band == PREFER_BAND_5G && wlan_idx == 0))
#endif
		return;

	/* read MIB from prefer band */
	apmib_save_wlanIdx();
	wlan_idx = prefer_band;

	apmib_get(MIB_WLAN_SSID, (void *)prefer_band_ssid);
	apmib_get(MIB_WLAN_ENCRYPT, (void *)&encrypt);
	...
	if (encrypt == ENCRYPT_WEP) {
		...
		if (wep == WEP64) {
			apmib_get(MIB_WLAN_WEP64_KEY1, (void *)wep64Key1);
			apmib_get(MIB_WLAN_WEP64_KEY2, (void *)wep64Key2);
			apmib_get(MIB_WLAN_WEP64_KEY3, (void *)wep64Key3);
			apmib_get(MIB_WLAN_WEP64_KEY4, (void *)wep64Key4);
		}
		else {
			apmib_get(MIB_WLAN_WEP128_KEY1, (void *)wep128Key1);
			apmib_get(MIB_WLAN_WEP128_KEY2, (void *)wep128Key2);
			apmib_get(MIB_WLAN_WEP128_KEY3, (void *)wep128Key3);
			apmib_get(MIB_WLAN_WEP128_KEY4, (void *)wep128Key4);
		}
	}
	else if ((encrypt == ENCRYPT_WPA) || (encrypt == ENCRYPT_WPA2)
			|| (encrypt == ENCRYPT_WPA2_MIXED) || (encrypt == ENCRYPT_WPA3)
			|| (encrypt == ENCRYPT_WPA2_WPA3_MIXED)) {
		apmib_get(MIB_WLAN_WPA_AUTH, (void *)&wpa_auth);
		apmib_get(MIB_WLAN_WPA_CIPHER_SUITE, (void *)&wpa_cipher_suite);
		apmib_get(MIB_WLAN_WPA_PSK, (void *)&wpa_psk);
		apmib_get(MIB_WLAN_WPA_GROUP_REKEY_TIME, (void *)&wpa_group_rekey_time);
		apmib_get(MIB_WLAN_PSK_FORMAT, (void *)&psk_format);
		apmib_get(MIB_WLAN_WPA2_PRE_AUTH, (void *)&wpa2_pre_auth);
		apmib_get(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&wpa2_cipher_suite);
	}

#ifdef CONFIG_IEEE80211W
	apmib_get(MIB_WLAN_IEEE80211W, (void *)&ieee80211w);
	apmib_get(MIB_WLAN_SHA256_ENABLE, (void *)&sha256_enable);
#endif

	/* write MIB to non-prefer band */
	wlan_idx = (wlan_idx == 0) ? 1 : 0;

	apmib_set(MIB_WLAN_SSID, (void *)prefer_band_ssid);
	apmib_set(MIB_WLAN_ENCRYPT, (void *)&encrypt);

	if (encrypt == ENCRYPT_WEP) {
		apmib_set(MIB_WLAN_AUTH_TYPE, (void *)&auth_type);
		apmib_set(MIB_WLAN_WEP, (void *)&wep);
		apmib_set(MIB_WLAN_WEP_KEY_TYPE, (void *)&wep_key_type);
		if (wep == WEP64) {
			apmib_set(MIB_WLAN_WEP64_KEY1, (void *)wep64Key1);
			apmib_set(MIB_WLAN_WEP64_KEY2, (void *)wep64Key2);
			apmib_set(MIB_WLAN_WEP64_KEY3, (void *)wep64Key3);
			apmib_set(MIB_WLAN_WEP64_KEY4, (void *)wep64Key4);
		}
		else {
			apmib_set(MIB_WLAN_WEP128_KEY1, (void *)wep128Key1);
			apmib_set(MIB_WLAN_WEP128_KEY2, (void *)wep128Key2);
			apmib_set(MIB_WLAN_WEP128_KEY3, (void *)wep128Key3);
			apmib_set(MIB_WLAN_WEP128_KEY4, (void *)wep128Key4);
		...
		apmib_set(MIB_WLAN_WPA_CIPHER_SUITE, (void *)&wpa_cipher_suite);
		apmib_set(MIB_WLAN_WPA_PSK, (void *)&wpa_psk);
		apmib_set(MIB_WLAN_WPA_GROUP_REKEY_TIME, (void *)&wpa_group_rekey_time);
		apmib_set(MIB_WLAN_PSK_FORMAT, (void *)&psk_format);
		apmib_set(MIB_WLAN_WPA2_PRE_AUTH, (void *)&wpa2_pre_auth);
		apmib_set(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&wpa2_cipher_suite);
	}

#ifdef CONFIG_IEEE80211W
	apmib_set(MIB_WLAN_IEEE80211W, (void *)&ieee80211w);
	apmib_set(MIB_WLAN_SHA256_ENABLE, (void *)&sha256_enable);
#endif
...
void formAirtime(request *wp, char *path, char *query)
{
	...
	if (submit == sbNormal)
	{
		...
		if (apmib_set(enable_id, (void *)&enabled) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_ena_fla)</script>"));
			goto setErr_airtime;
		}
	}
	...
	if (enabled || submit != sbNormal) {
		...
		if (strVal[0]) {
			...
			if (mode < ATM_SSID_AUTO || mode > ATM_DEVICE_MANUAL) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_mod_err_unknown)</script>"));
				goto setErr_airtime;
			}

			if (apmib_set(mode_id, (void *)&mode) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_mod_err)</script>"));
				goto setErr_airtime;
			}
		}
	}
	...
	if (enabled && mode == ATM_SSID_MANUAL) {
		...
		if (NUM_VWLAN_INTERFACE+1 > 6) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_SSI_Bas)</script>"));
			goto setErr_airtime;
		}
		...
		for (i=0; i<=NUM_VWLAN_INTERFACE; i++) {
			...
			if (strVal[0]) {
				...
				if (val == 1)
					...
					strcpy(tmpBuf, ("<script>dw(wl_Set_SSI_Bas_empty)</script>"
						));
					goto setErr_airtime;	
				}
			}
			...
			if (time[i] <0 || time[i]>100) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_SSI_Bas_alloc)</script>"
					));
				goto setErr_airtime;	
			}
		...
		if (count > 100) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_SSI_Bas_total)</script>"));
			goto setErr_airtime;
		}
	...
	if (submit == sbAddStaion) {	
		...
		if (!strAlocation[0]) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Ple_fil_applocation)</script>"));
			goto setErr_airtime;
		}

		if (!strIpAddr[0] && !strMacAddr[0]) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Ple_fil)</script>"));
			goto setErr_airtime;
		}
		...
		if (strIpAddr[0]) {
			...
			/* check ip subnet */
			if ((v1 & v3) != (v2 & v3)) {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_IP_add)</script>"
					));
				goto setErr_airtime;
			}
				...
				if (apmib_get(get_id, (void *)&atmEntryTmp)) {
					if (*((unsigned long *)atmEntry.ipAddr) == 
						*((unsigned long *)atmEntryTmp.ipAddr)) {
						strcpy(tmpBuf, ("<script>dw(wl_ip_add_exi)</script>"));
						goto setErr_airtime;	
					}
				}
			}
		...
		if (strMacAddr[0]) {
			if (strlen(strMacAddr)!=12 || !string_to_hex(strMacAddr, atmEntry.macAddr, 12)) {
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_MAC)</script>"));
				goto setErr_airtime;
			}
				...
				*((char *)&atmEntryTmp) = (char)i;
				if (apmib_get(get_id, (void *)&atmEntryTmp)) {
					if (!memcmp(atmEntryTmp.macAddr, atmEntry.macAddr, 6)) {
						strcpy(tmpBuf, ("<script>dw(wl_mac_add_exi)</script>"));
						goto setErr_airtime;
					}
				}
			}
		...
		if (strAlocation[0]) {
			...
			if (total > 100) {
				strcpy(tmpBuf, ("<script>dw(wl_Tot_All_mus)</script>"));
				goto setErr_airtime;				
			}
		}
		...
		if (strVal[0]) {
			if (strlen(strVal) > COMMENT_LEN-1) {
				strcpy(tmpBuf, ("<script>dw(wl_Err_Com_len)</script>"));
				goto setErr_airtime;
			}
		...
		if ((entryNum + 1) > MAX_AIRTIME_NUM) {
			strcpy(tmpBuf, ("<script>dw(wl_Can_add_new_station)</script>"));
			goto setErr_airtime;
		}
  ...
  char *strSSID, *strChan, *strDisabled, *strVal, strtmp[80];
	...
	if(strVal[0]){
		...
		if(val != atoi(strVal)){
			...
			if ( apmib_set(MIB_HW_REG_DOMAIN, (void *)&val) == 0) {
					strcpy(tmpBuf, ("<script>dw(wlbasic_set_wlan_reg)</script>"));
					goto setErr_wlan;
			}
	...
	if(strVal[0]){
			if (apmib_set(MIB_WLAN_COUNTRY_STRING, (void *)strVal) == 0) {
					strcpy(tmpBuf, ("<script>dw(wlbasic_set_wlan_country)</script>"));
					goto setErr_wlan;
			}
	}
	...
	if(strVal[0])
	{
		...
		if (apmib_set(MIB_WLAN_TX_RESTRICT, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_mib_wlan_tx)</script>"));
			goto setErr_wlan;
		}
	}
	...
	if(strVal[0])
	{
		...
		if (apmib_set(MIB_WLAN_RX_RESTRICT, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_mib_wlan_rx)</script>"));
			goto setErr_wlan;
		}
	}
	...
	if ( strVal[0] ) {
		...
		*mode = strVal[0] - '0';
		...
		if(*mode != AP_MESH_MODE){
			int meshmode = 0;
			if ( apmib_set(MIB_WLAN_MESH_WPS_MODE, (void *)&meshmode) == 0) {
   	 			strcpy(tmpBuf, ("Set MESH_WPS_MODE error!"));
				goto setErr_wlan;			
			}
		}
#endif
		...
		if (*mode == CLIENT_MODE) {
			...
			if (encrypt &  ENCRYPT_WPA2_MIXED) {
				...
				if (use1x & 1) {
					...
					//Support wlan client mode with Enterprise (RADIUS)
#else
					strcpy(tmpBuf, ("<script>dw(wlbasic_cannot_set_client)</script>"));
					goto setErr_wlan;
#endif
				}
			}
			...
			if ( !strcmp(strVal, "ON"))
				...
				val = 0 ;
			if ( apmib_set( MIB_WLAN_MACCLONE_ENABLED, (void *)&val) == 0) {
				strcpy(tmpBuf, ("<script>dw(wlbasic_set_macclone)</script>"));
				goto setErr_wlan;
			}
		}
	...
	sprintf(varName, "ssid%d", wlan_id);
   	strSSID = req_get_cstream_var(wp, varName, "");	
	if(strlen(strSSID)>=MAX_SSID_LEN)
	{
		sprintf(tmpBuf, "<script>dw(wlbasic_set_ssid_toolong)</script>");
		goto setErr_wlan;
	}
	if ( strSSID[0] ) {
		if ( apmib_set(MIB_WLAN_SSID, (void *)strSSID) == 0) {
	...
	if ( strChan[0] ) {
		...
		if (errno) {
   			strcpy(tmpBuf, ("<script>dw(wlbasic_invalid_channel)</script>"));
			goto setErr_wlan;
		}
		if ( apmib_set( MIB_WLAN_CHANNEL, (void *)&chan) == 0) {
   			strcpy(tmpBuf, ("<script>dw(wlbasic_set_channel_number)</script>"));
			goto setErr_wlan;
		}
	}
	...
	if (strVal[0]) {
		if (strVal[0]!= '0' && strVal[0]!= '1') {
			strcpy(tmpBuf, ("<script>dw(wlbasic_invalid_network)</script>"));
			goto setErr_wlan;
		}
		if (strVal[0] == '0')
			net = INFRASTRUCTURE;
		...
		if ( apmib_set(MIB_WLAN_NETWORK_TYPE, (void *)&net) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_mib_wlan_network)</script>"));
			goto setErr_wlan;
		}
	}
	...
	if ( strVal[0] ) 
	{
		...
		if (band_no < 0 || band_no > 203) { //8812 //ac2g
			strcpy(tmpBuf, ("<script>dw(wlbasic_invalid_band)</script>"));
			goto setErr_wlan;
		}
		//val = (strVal[0] - '0' + 1);
		if(wlan_onoff_tkip == 0) //Wifi request
		{
			...
			if(*mode != CLIENT_MODE && (band_no == 7 || band_no == 9 || band_no == 10 || band_no == 11 || band_no == 63 || band_no == 71 || band_no == 75 || band_no == 203)) //7:n; 9:gn; 10:bgn 11:5g_an
			{
		...
		if ( apmib_set( MIB_WLAN_BAND, (void *)&val) == 0) {
   			strcpy(tmpBuf, ("<script>dw(wlbasic_set_band_error)</script>"));
			goto setErr_wlan;
		}
...
#if defined(CONFIG_RTL_8812_SUPPORT)
		{
			...
			if(band_no==3 || band_no==11 || band_no==63 || band_no==71 || band_no==75 || band_no==203)
				val = 2;
			else if(band_no==7)
			{
	...
	if ( strRate[0] ) {
		if ( strRate[0] == '0' ) { // auto
			...
			if ( apmib_set(MIB_WLAN_RATE_ADAPTIVE_ENABLED, (void *)&val) == 0) {
				strcpy(tmpBuf, ("<script>dw(wlbasic_set_rate_adaptive)</script>"));
				goto setErr_wlan;
			}
		}
		else  {
			...
			if(val<29)
				...
				val = ((1<<28) + val-29);
			else if (val >=45 && val <85)
				val = ((1<<31) + val-45);
#ifdef IS_AX_SUPPORT
			else
				//TBD ,40 and 85 is html select option's value which hard code and hooked with Wlbasic.htm  updateDataRate(), need fix this awkward behavior in future
				val =  ((1<<31) + 40+val-85);
#endif

			if ( apmib_set(MIB_WLAN_FIX_RATE, (void *)&val) == 0) {
				strcpy(tmpBuf, ("<script>dw(wlbasic_set_fixed_rate)</script>"));
				goto setErr_wlan;
			}
		}
	}
	...
	if ( strRate[0] ) {
		...
		if ( val && apmib_set(MIB_WLAN_BASIC_RATES, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_tx_basic)</script>"));
			goto setErr_wlan;
		}
	}
	...
	if ( strRate[0] ) {
		...
		if (val && apmib_set(MIB_WLAN_SUPPORTED_RATES, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_tx_operation)</script>"));
			goto setErr_wlan;
		}
	}
	...
	if (strVal[0]) {
		if ( strVal[0] == '0')
			...
			strcpy(tmpBuf, ("<script>dw(wlbasic_invalid_channel_bond)</script>"));
			goto setErr_wlan;
		}
		if ( apmib_set(MIB_WLAN_HIDDEN_SSID, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_hidden_ssid)</script>"));
			goto setErr_wlan;
		}
	}
	...
	if (strVal[0]) {
		if ( strVal[0] == '0')
			...
			strcpy(tmpBuf, ("<script>dw(wlbasic_invalid_wmm)</script>"));
			goto setErr_wlan;
		}
		if ( apmib_set(MIB_WLAN_WMM_ENABLED, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_mib_wlan_wmm)</script>"));
			goto setErr_wlan;
		}
	}else{
...
// for 11N
	...
	if (strVal[0]) {
		...
		if ( apmib_set(MIB_WLAN_CHANNEL_BONDING, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_mib_wlan_channel)</script>"));
			goto setErr_wlan;
		}
	}
	...
	if (strVal[0]) {
		if ( strVal[0] == '0')
			...
			strcpy(tmpBuf, ("<script>dw(wlbasic_invalid_control_sideband)</script>"));
			goto setErr_wlan;
		}
		if ( apmib_set(MIB_WLAN_CONTROL_SIDEBAND, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_mib_wlan_control_sideband)</script>"));
			goto setErr_wlan;
		}
	}
...
//
	...
	if ( strRate[0] ) {
		...
		if ( val && apmib_set(MIB_WLAN_BASIC_RATES, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlbasic_set_tx_basic_rate)</script>"));
			goto setErr_wlan;
		}
	}
    ...
    {	
    	...
    	if ((strVal==NULL || strVal[0]==0)   // not called from wizard	
    		...
    		if (val == 1) {
    			sprintf(varName, "repeaterSSID%d", wlan_id);
    			strVal = req_get_cstream_var(wp, varName, NULL);
				if(strlen(strVal)>=MAX_SSID_LEN){
					sprintf(tmpBuf,"<script>dw(wlbasic_ssid_of_extend_length)</script> %d,<script>dw(wlbasic_oversize)</script> %d!\n",strlen(strVal),MAX_SSID_LEN-1);
					goto setErr_wlan;
				}
    			if (strVal){
    				if (wlan_id == 0)
...
int meshWpaHandler(request *wp, char *tmpBuf, int wlan_id)
{
	...
	if (!strEncrypt[0]) {
 		strcpy(tmpBuf, ("<script>dw(wl_Err_no_enc)</script>"));
		goto setErr_mEncrypt;
	}
	encrypt = (ENCRYPT_T) strEncrypt[0] - '0';
	if (encrypt!=ENCRYPT_DISABLED &&  encrypt != ENCRYPT_WPA2 ) {
		strcpy(tmpBuf, ("<script>dw(wl_Inv_enc_met)</script>"));
		goto setErr_mEncrypt;
	}

	if (apmib_set( MIB_WLAN_MESH_ENCRYPT, (void *)&encrypt) == 0) {
  		strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_MESH_ENCRYPT_mib)</script>"));
		goto setErr_mEncrypt;
	}
	...
	if(encrypt == ENCRYPT_WPA2)
	{
		...
		if ( apmib_set(MIB_WLAN_MESH_WPA_AUTH, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_MESH_AUTH_TYPE_fai)</script>"));
				goto setErr_mEncrypt;
		}
		...
		if ( apmib_set(MIB_WLAN_MESH_WPA2_CIPHER_SUITE, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_MESH_WPA2_UNICIPHER_fai)</script>"));
				goto setErr_mEncrypt;
		}
		...
		if ( getPSK ) {
			...
			sprintf(varName, "mesh_pskFormat%d", wlan_id);
   			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Err_no_psk)</script>"));
				goto setErr_mEncrypt;
			}
			intVal = strVal[0] - '0';
			if (intVal != 0 && intVal != 1) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Err_inv_psk_format)</script>"));
				goto setErr_mEncrypt;
			}
			...
			if ( apmib_set(MIB_WLAN_MESH_PSK_FORMAT, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_MESH_PSK_FORMAT_fai)</script>"));
				goto setErr_mEncrypt;
			}
			...
			if ( !apmib_set(MIB_WLAN_MESH_WPA_PSK, (void *)strVal)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_MESH_WPA_PSK_err)</script>"));
				goto setErr_mEncrypt;
			}
		}	
	}
...
int wlMeshAcList(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"45%%\" ><font size=\"2\"><b><script>dw(wlmeshACL_mac_address)</script></b></font></td>\n"
      	"<td align=center width=\"35%%\" ><font size=\"2\"><b><script>dw(wlmeshACL_comment)</script></b></font></td>\n"
      	"<td align=center width=\"20%%\" ><font size=\"2\"><b><script>dw(wlmeshACL_select)</script></b></font></td></tr>\n"));
...
{
        char *submitUrl,*meshRootEnabled,*refresh, *strMeshID, *strEnabled, *strCrossbandEnabled, *strAutoConfigEnabled;
        int enabled,meshenable=0,mesh_crossband=0,capwapMode,wtp_id=1;
        char tmpBuf[MAX_MSG_BUFFER_SIZE]={0};
        int warn=0;
#ifdef WIFI_SIMPLE_CONFIG
        int wps_mesh_mode = 2;
		...
		strCrossbandEnabled = req_get_cstream_var(wp, ("mesh_crossband"), "");
#endif
#ifdef RTK_MESH_CONFIG
		strAutoConfigEnabled = req_get_cstream_var(wp, ("mesh_auto_config"), "");
#endif
        ...
        {
                strcpy( tmpBuf, ("<script>dw(wl_set_mesh_enable)</script>"));
                goto setErr_mesh;
        }
...
#ifdef RTK_MESH_METRIC_REFINE
		if (strCrossbandEnabled[0]) {
			if (!strcmp(strCrossbandEnabled, ("1")))
				mesh_crossband= 1;
			else
				mesh_crossband= 0;	  
		}
				
		if(apmib_set(MIB_WLAN_MESH_CROSSBAND_ENABLED, (void *)&mesh_crossband) == 0)
		{
		        strcpy( tmpBuf, ("<script>dw(wl_set_mesh_cross)</script>"));
                goto setErr_mesh;
		}
#endif
...
#ifdef RTK_MESH_CONFIG
		apmib_get(MIB_CAPWAP_MODE, (void*)&capwapMode);
		if(strAutoConfigEnabled[0]) {
			if (!strcmp(strAutoConfigEnabled, ("1")))
				capwapMode |= CAPWAP_AUTO_CONFIG_ENABLE;
			else
				capwapMode &= ~CAPWAP_AUTO_CONFIG_ENABLE;
			if(!apmib_set(MIB_CAPWAP_MODE, (void *)&capwapMode) || !apmib_set(MIB_CAPWAP_WTP_ID, (void *)&wtp_id))
			{
		        strcpy( tmpBuf, ("Set mesh auto config enable error!"));
                goto setErr_mesh;
	...
	if(!strcmp(meshRootEnabled, "ON"))
        ...
        {
                strcpy( tmpBuf, ("<script>dw(wl_set_mesh_root)</script>"));
                goto setErr_mesh;
        }
...
//              if (strlen(strMeshID)!=12 || !string_to_hex(strMeshID, tmpBuf, 12)) {
                if (strlen(strMeshID)>32) {
                        strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_Mes)</script>"));
                        goto setErr_mesh;
                }
                if ( apmib_set(MIB_WLAN_MESH_ID, (void *)strMeshID) == 0)
                {
                        strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_MESH_ID_err)</script>"));
                        goto setErr_mesh;
                }
#ifdef WIFI_SIMPLE_CONFIG 		
                /* for wps mesh, if mesh id is set, then fixed as wps mesh reg */
                if ( apmib_set(MIB_WLAN_MESH_WPS_MODE, (void *)&wps_mesh_mode) == 0)
                {
                    strcpy(tmpBuf, ("Set MIB_WLAN_MESH_WPS_MODE error!"));
                    goto setErr_mesh;
                }
#ifdef FOR_DUAL_BAND
...
//displayPostDate(wp->post_data);
  ...
  sprintf(tmpBuf,"%s","<h4><script>dw(wlsurvey_change_setting_success)</script><BR><BR><script>dw(wlsurvey_donot_turn_off)</script></h4>");
...
#ifdef BAND_STEERING_SUPPORT
	unsigned char band_steering_enable = 0;
#endif
...
//printf("\r\n __[%s-%u]\r\n",__FILE__,__LINE__);	
	...
	if (mode == 1) { // not AP mode
		...
		if(encrypt == ENCRYPT_WPA2_MIXED){
			...
			if ( apmib_set(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_WPA2_UNICIPHER_fai)</script>"));
				goto setErr_wlan;
			}
			intVal =   WPA_CIPHER_MIXED;
	...
	if (mode != 1) { // AP mode
		...
		if(encrypt == ENCRYPT_WPA){
			...
			strcpy(tmpBuf, ("<script>dw(wlsecurity_wpa_warning)</script>"));
			warn = 1;
		}
	}
	...
	int i, j;
	apmib_get(MIB_MAP_ENABLE_VLAN, (void *)&map_vlan_enable);
	if (map_vlan_enable) {
		strVal = req_get_cstream_var(wp, ("map_vid_info"), "");
		char* token;
		for (i = 0; i < 2; i++) {
			wlan_idx = i;
			for (j = 0; j < 5; j++) {
	...
	if (band_steering_enable)
		wlan_band_steering_update();
#endif
...
//printf("\r\n auahType=[%d],__[%s-%u]\r\n",auahType,__FILE__,__LINE__);				
	...
	if ( enabled ) {
		...
		if (!strKeyLen[0]) {
 			strcpy(tmpBuf, ("<script>dw(wl_Key_len_mus)</script>"));
			goto setErr_wep;
		}
		if (strKeyLen[0]!='1' && strKeyLen[0]!='2') {
 			strcpy(tmpBuf, ("<script>dw(wl_Inv_key_len)</script>"));
			goto setErr_wep;
		}
		if (strKeyLen[0] == '1')
			wep = WEP64;
	...
	if ( apmib_set( MIB_WLAN_WEP, (void *)&wep) == 0) {
  		strcpy(tmpBuf, ("<script>dw(wl_Set_WEP_MIB)</script>"));
		goto setErr_wep;
	}
	...
	if (!strFormat[0]) {
 		strcpy(tmpBuf, ("<script>dw(wl_Key_typ_mus)</script>"));
		goto setErr_wep;
	}

	if (strFormat[0]!='1' && strFormat[0]!='2') {
		strcpy(tmpBuf, ("<script>dw(wl_Inv_key_typ)</script>"));
		goto setErr_wep;
	}

	i = strFormat[0] - '0' - 1;
	if ( apmib_set( MIB_WLAN_WEP_KEY_TYPE, (void *)&i) == 0) {
  		strcpy(tmpBuf, ("<script>dw(wl_Set_WEP_key)</script>"));
		goto setErr_wep;
	}
		...
		if ( !isAllStar(wepKey) ) {
			if (strFormat[0] == '1') // ascii
				...
				if ( !string_to_hex(wepKey, key, keyLen)) {
	   				strcpy(tmpBuf, ("<script>dw(wl_Inv_wep_val)</script>"));
					goto setErr_wep;
				}
			}
			if (wep == WEP64){
			...
			if (!ret) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Set_wep_err)</script>"));
				goto setErr_wep;
			}
		}
	}
	...
	if ( strKeyId[0] ) {
		if ( strKeyId[0]!='1' && strKeyId[0]!='2' && strKeyId[0]!='3' && strKeyId[0]!='4' ) {
	 		strcpy(tmpBuf, ("<script>dw(wl_Inv_def_tx)</script>"));
   			goto setErr_wep;
		}
		i = strKeyId[0] - '0' - 1;
		if ( !apmib_set( MIB_WLAN_WEP_DEFAULT_KEY, (void *)&i ) ) {
	 		strcpy(tmpBuf, ("<script>dw(wl_Set_def_tx)</script>"));
   			goto setErr_wep;
		}
	}
	...
	if  (wepKey[0]) {
		if (strlen(wepKey) != keyLen) {
			strcpy(tmpBuf, ("<script>dw(wl_Inv_key_1)</script>"));
			goto setErr_wep;
		}
		if ( !isAllStar(wepKey) ) {
			if (strFormat[0] == '1') // ascii
				...
				if ( !string_to_hex(wepKey, key, keyLen)) {
	   				strcpy(tmpBuf, ("<script>dw(wl_Inv_wep1_val)</script>"));
					goto setErr_wep;
				}
			}
			if (wep == WEP64)
				...
				ret=apmib_set(MIB_WLAN_WEP128_KEY1, (void *)key);
			if (!ret) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Set_wep1_err)</script>"));
				goto setErr_wep;
			}
		}
	}
	...
	if  (wepKey[0]) {
		if (strlen(wepKey) != keyLen) {
			strcpy(tmpBuf, ("<script>dw(wl_Inv_key_2)</script>"));
			goto setErr_wep;
		}
		if ( !isAllStar(wepKey) ) {
			if (strFormat[0] == '1') // ascii
				...
				if ( !string_to_hex(wepKey, key, keyLen)) {
	   				strcpy(tmpBuf, ("<script>dw(wl_Inv_wep2_val)</script>"));
   					goto setErr_wep;
				}
			}
			if (wep == WEP64)
				...
				ret=apmib_set(MIB_WLAN_WEP128_KEY2, (void *)key);
			if (!ret) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Set_wep2_err)</script>"));
				goto setErr_wep;
			}
		}
	}
	...
	if  (wepKey[0]) {
		if (strlen(wepKey) != keyLen) {
			strcpy(tmpBuf, ("<script>dw(wl_Inv_key_3)</script>"));
			goto setErr_wep;
		}
		if ( !isAllStar(wepKey) ) {
			if (strFormat[0] == '1') // ascii
				...
				if ( !string_to_hex(wepKey, key, keyLen)) {
	   				strcpy(tmpBuf, ("<script>dw(wl_Inv_wep3_val)</script>"));
   					goto setErr_wep;
				}
			}
			if (wep == WEP64)
				...
				ret=apmib_set(MIB_WLAN_WEP128_KEY3, (void *)key);
			if (!ret) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Set_wep3_err)</script>"));
				goto setErr_wep;
			}
		}
	}
	...
	if  (wepKey[0]) {
		...
		if ( !isAllStar(wepKey) ) {
			if (strFormat[0] == '1') // ascii
				...
				if ( !string_to_hex(wepKey, key, keyLen)) {
	   				strcpy(tmpBuf, ("<script>dw(wl_Inv_wep4_val)</script>"));
   					goto setErr_wep;
				}
			}
			if (wep == WEP64)
				...
				ret=apmib_set(MIB_WLAN_WEP128_KEY4, (void *)key);
			if (!ret) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Set_wep4_err)</script>"));
				goto setErr_wep;
			}
		}
	}	
...
#endif
#ifdef WLAN_WPA3
		&& encrypt != ENCRYPT_WPA3 && encrypt != ENCRYPT_WPA2_WPA3_MIXED
#endif
) {
		strcpy(tmpBuf, ("<script>dw(wl_Inv_enc_met)</script>"));
		goto setErr_encrypt;
...
//					printf("%s(%d): WPA-RADIUS can be used when device is set to client mode\n",__FUNCTION__,__LINE__);//Added for test 
					...
					strcpy(tmpBuf, ("<script>dw(wl_Err_802_aut_wds)</script>"));
					goto setErr_encrypt;
					intVal = 0;
				}
#else
				strcpy(tmpBuf, ("<script>dw(wl_Err_802_aut)</script>"));
				goto setErr_encrypt;
				intVal = 0;				
#endif
			}
		...
		if(!enableRS) {
			if ( apmib_set( MIB_WLAN_HS2_ENABLE, (void *)&intVal) == 0) {
	  			strcpy(tmpBuf, ("<script>dw(wl_Set_hs2_ena)</script>"));
				goto setErr_encrypt;
			}
		}
#endif		
		if ( apmib_set( MIB_WLAN_ENABLE_1X, (void *)&intVal) == 0) {
  			strcpy(tmpBuf, ("<script>dw(wl_Set_1x_ena)</script>"));
			goto setErr_encrypt;
		}
		...
		if (encrypt == ENCRYPT_WEP) {
			...
			if ( !apmib_get( MIB_WLAN_WEP,  (void *)&wep) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Get_MIB_WLAN_WEP_MIB)</script>"));
				goto setErr_encrypt;
			}
			if (wep == WEP_DISABLED) {
				wep = WEP64;
			...
			if ( apmib_set( MIB_WLAN_MAC_AUTH_ENABLED, (void *)&intVal) == 0) {
  				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_MAC_AUTH_ENABLED_MIB)</script>"));
				goto setErr_encrypt;
			}
		}
#ifdef CONFIG_IEEE80211W
		...
		if ( apmib_set(MIB_WLAN_IEEE80211W, (void *)&intVal) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_IEEE80211W_fai)</script>"));
			goto setErr_encrypt;							
		}
		intVal = 0;
		if ( apmib_set(MIB_WLAN_SHA256_ENABLE, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_SHA256_ENABLE_fai)</script>"));
				goto setErr_encrypt;							
		}
#endif
	...
	else if(ENCRYPT_WAPI==encrypt)
	{
		...
		if (strVal[0]) 
		{
			if ( !strcmp(strVal, ("eap")))
			{
				apmib_get( MIB_WLAN_MODE, (void *)&intVal);
				if (intVal!=AP_MODE && intVal!=AP_WDS_MODE) { // not AP mode
					strcpy(tmpBuf, ("<script>dw(wl_Err_WAP_AS)</script>"));
					goto setErr_encrypt;
				}
			...
			{
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_wap)</script>"));
				goto setErr_encrypt;
			}

			if ( apmib_set(MIB_WLAN_WAPI_AUTH, (void *)&intVal) == 0) 
			{
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_WAPI_AUTH_fai)</script>"));
				goto setErr_encrypt;
			}
		}
		// pre-shared key
		if ( getPSK ) {
			...
			if (intVal==1) { // hex
				...
				else
				{
					...
					strcpy(tmpBuf, ("<script>dw(wl_Err_inv_psk_len)</script>"));
					goto setErr_encrypt;
				}					
				if(!apmib_set(MIB_WLAN_WAPI_PSKLEN,(void*)&len))
				{
					strcpy(tmpBuf,("<script>dw(wl_Err_Set_wap)</script>"));
				}
			}
			else { // passphras
				if (len==0 || len > (MAX_PSK_LEN-1)|| len < MIN_PSK_LEN ) {
	 				strcpy(tmpBuf, ("<script>dw(wl_Err_inv_psk)</script>"));
					goto setErr_encrypt;
				}
				if(!apmib_set(MIB_WLAN_WAPI_PSKLEN,(void*)&len))
				{
	...
	wapi_end:
		...
		{ 
			...
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("<script>dw(wl_No_WAP_AS)</script>"));
				goto setErr_encrypt;
			}
			if ( !inet_aton(strVal, &inIp) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_AS_IP-)</script>"));
				goto setErr_encrypt;
			}
			...
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
				strcpy(tmpBuf, ("<script>dw(wl_No_WAP_cer)</script>"));
				goto setErr_encrypt;
			}
			intVal=atoi(strVal);
			...
			for(i=0;i<NUM_VWLAN_INTERFACE+1;i++)
			{
				...
				if ( !apmib_set(MIB_WLAN_WAPI_ASIPADDR, (void *)&inIp)) {
					strcpy(tmpBuf, ("<script>dw(wl_Set_RS_IP-)</script>"));
					goto setErr_encrypt;
				}	
				if ( !apmib_set(MIB_WLAN_WAPI_CERT_SEL, (void *)&intVal)) {
					strcpy(tmpBuf, ("<script>dw(wl_Set_WAP_cer)</script>"));
					goto setErr_encrypt;
				}	
			}
		...
		if ( apmib_set( MIB_WLAN_ENABLE_SUPP_NONWPA, (void *)&intVal) == 0) {
  			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_ENABLE_SUPP_NONWPA_mib)</script>"));
			goto setErr_encrypt;
		}
		if ( intVal ) {
			...
			if ( apmib_set( MIB_WLAN_SUPP_NONWPA, (void *)&suppNonWPA) == 0) {
  				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_SUPP_NONWPA_mib)</script>"));
				goto setErr_encrypt;
			}
		}
...
//						printf("%s(%d): WPA-RADIUS can be used when device is set to client mode\n",__FUNCTION__,__LINE__);//Added for test 
						...
						strcpy(tmpBuf, ("<script>dw(wl_Err_WPA_can_wds)</script>"));
						goto setErr_encrypt;
					}
						
#else
					strcpy(tmpBuf, ("<script>dw(wl_Err_WPA_can)</script>"));
					goto setErr_encrypt;
#endif
				}
#endif
				...
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_wpa)</script>"));
				goto setErr_encrypt;
			}
...
// get cipher suite from user setting, for wpa-aes -------------------		
		...
		//if ((encrypt == ENCRYPT_WPA) || (encrypt == ENCRYPT_WPA2_MIXED)) 
		{
			...
			if (strVal[0]) {
				...
				if ( strstr(strVal, ("aes"))) 
					intVal |= WPA_CIPHER_AES;
				if (intVal == 0) {
					strcpy(tmpBuf, ("<script>dw(wl_Inv_val_of_cipher)</script>"));
					goto setErr_encrypt;
				}
			}
			else{
...
#ifdef WLAN_WPA3
			if (encrypt == ENCRYPT_WPA3 || encrypt == ENCRYPT_WPA2_WPA3_MIXED || encrypt == ENCRYPT_WPA2)
				intVal = 0;
#endif
...
#ifdef CONFIG_IEEE80211W
		{
			...
			if (strVal[0]) {
				if ( strstr(strVal, ("none"))) 
					intVal = NO_MGMT_FRAME_PROTECTION;
				else if ( strstr(strVal, ("capable"))) 	// only v3.4 supports pmf client mode
					intVal = MGMT_FRAME_PROTECTION_OPTIONAL;
				else if ( strstr(strVal, ("required"))) 
					intVal = MGMT_FRAME_PROTECTION_REQUIRED;
				else {
					intVal = NO_MGMT_FRAME_PROTECTION;
					strcpy(tmpBuf, ("<script>dw(wl_Inv_val_of_80211w)</script>"));
					goto setErr_encrypt;
				}
			}
			else
			...
			if(encrypt == ENCRYPT_WPA2)
			{
				sprintf(varName, "pmf_status%d", wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				printf("pmf_status, strVar=%s\n",strVal);
				if(strVal[0]) {
					if ( strstr(strVal, "0")) {
						intVal = NO_MGMT_FRAME_PROTECTION;
					}
					else if( strstr(strVal, "1")) {
						intVal = MGMT_FRAME_PROTECTION_OPTIONAL;
					}
					else if( strstr(strVal, "2")) {
						intVal = MGMT_FRAME_PROTECTION_REQUIRED;
					}
				}
			}
			...
			if (encrypt == ENCRYPT_WPA3)
				intVal = MGMT_FRAME_PROTECTION_REQUIRED;
			else if(encrypt == ENCRYPT_WPA2_WPA3_MIXED)
				intVal = MGMT_FRAME_PROTECTION_OPTIONAL;
			else if(encrypt == ENCRYPT_WPA2_MIXED && val == CLIENT_MODE)
				intVal = MGMT_FRAME_PROTECTION_OPTIONAL;
#endif
			if ( apmib_set(MIB_WLAN_IEEE80211W, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_IEEE80211W_fai)</script>"));
		...
		}
		{
			...
			//else{
				if (strVal[0]) {			
					if ( strstr(strVal, ("disable"))) 
						...
						intVal = 0;
						strcpy(tmpBuf, ("<script>dw(wl_Inv_val_ofsha)</script>"));
						goto setErr_encrypt;
					}
				}
				else
			...
			//}
#ifdef WLAN_WPA3
				if (encrypt == ENCRYPT_WPA3 || encrypt == ENCRYPT_WPA2_WPA3_MIXED)
					intVal = 1;
#ifdef IS_AX_SUPPORT
				if(encrypt == ENCRYPT_WPA2_WPA3_MIXED)
					intVal = 0;
				if ( val == CLIENT_MODE)
				{
					sprintf(varName, "pmf_status%d", wlan_id);
					...
					if(strVal[0] && encrypt == ENCRYPT_WPA2){
						if(intVal1 != 0)
							intVal = 1;
						else
							intVal = 0;
		...
		//if ((encrypt == ENCRYPT_WPA2) || (encrypt == ENCRYPT_WPA2_MIXED))
		{
			...
			strVal = req_get_cstream_var(wp, varName, "");
			printf("wpa2ciphersuite, strVar=%s\n",strVal);
			if (strVal[0]) {
				intVal = 0;
				if ( strstr(strVal, ("tkip")))
					intVal |= WPA_CIPHER_TKIP;
				if ( strstr(strVal, ("aes")))
					intVal |= WPA_CIPHER_AES;
				if (intVal == 0) {
					strcpy(tmpBuf, ("<script>dw(wl_Inv_val_of_wpa2)</script>"));
					goto setErr_encrypt;
				}
			}
			else{
			...
			if(encrypt == ENCRYPT_WPA3 || encrypt == ENCRYPT_WPA2_WPA3_MIXED)
				intVal = WPA_CIPHER_AES;
			//if(encrypt == ENCRYPT_WPA2_MIXED && val == CLIENT_MODE)
			//	intVal = WPA_CIPHER_AES;
#endif
...
rekey_time:
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_day)</script>"));
				goto setErr_encrypt;
			}
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_hr)</script>"));
				goto setErr_encrypt;
			}
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_min)</script>"));
				goto setErr_encrypt;
			}
			reKeyTime += intVal*60;
		}
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_sec)</script>"));
				goto setErr_encrypt;
			}
			reKeyTime += intVal;
		}
		if (reKeyTime) {
			if ( !apmib_set(MIB_WLAN_WPA_GROUP_REKEY_TIME, (void *)&reKeyTime)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_WPA_GROUP_REKEY_TIME_err)</script>"));
				goto setErr_encrypt;
			}
		}
	}
		...
		if (wlan_mode == CLIENT_MODE) { // wlan client mode
			...
			if(wlan_idx==wlanIdx_5G){
				...
				if ( !apmib_set(MIB_WLAN_RS_BAND_SEL, (void *)&rsBandSel)) {
					strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_RS_BAND_SEL_err)</script>"));
					goto setErr_encrypt;
				}
			}
			...
			if (strVal[0]) {
				if ( !string_to_dec(strVal, &intVal) ) {
					strcpy(tmpBuf, ("<script>dw(wl_Inv_802_EAP)</script>"));
					goto setErr_encrypt;
				}
				if ( !apmib_set(MIB_WLAN_EAP_TYPE, (void *)&intVal)) {
					strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_EAP_TYPE_err)</script>"));
					goto setErr_encrypt;
				}
			}
			else{
				strcpy(tmpBuf, ("<script>dw(wl_No_802_EAP_type)</script>"));
				goto setErr_encrypt;
			}
			...
			if(intVal == EAP_MD5){
				...
				if (strVal[0]) {
					if(strlen(strVal)>MAX_EAP_USER_ID_LEN){
						strcpy(tmpBuf, ("<script>dw(wl_EAP_use_ID)</script>"));
						goto setErr_encrypt;
					}
					if ( !apmib_set(MIB_WLAN_EAP_USER_ID, (void *)strVal)) {
						strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_EAP_USER_ID_err)</script>"));
						goto setErr_encrypt;
					}
				}
				else{
					strcpy(tmpBuf, ("<script>dw(wl_No_802_EAP)</script>"));
					goto setErr_encrypt;
				}
				...
				if (strVal[0]) {
					if(strlen(strVal)>MAX_RS_USER_NAME_LEN){
						strcpy(tmpBuf, ("<script>dw(wl_RAD_use_nam)</script>"));
						goto setErr_encrypt;
					}
					if ( !apmib_set(MIB_WLAN_RS_USER_NAME, (void *)strVal)) {
						strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_RS_USER_NAME_err)</script>"));
						goto setErr_encrypt;
					}
				}
				else{
					strcpy(tmpBuf, ("<script>dw(wl_No_802_RAD_name)</script>"));
					goto setErr_encrypt;
				}
				...
				if (strVal[0]) {
					if(strlen(strVal)>MAX_RS_USER_PASS_LEN){
						strcpy(tmpBuf, ("<script>dw(wl_RAD_use_pas)</script>"));
						goto setErr_encrypt;
					}
					if ( !apmib_set(MIB_WLAN_RS_USER_PASSWD, (void *)strVal)) {
						strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_RS_USER_PASSWD_err)</script>"));
						goto setErr_encrypt;
					}
				}
				else{
					strcpy(tmpBuf, ("<script>dw(wl_No_802_RAD_password)</script>"));
					goto setErr_encrypt;
				}
			}
				...
				if (strVal[0]) {
					if(strlen(strVal)>MAX_RS_USER_CERT_PASS_LEN){
						strcpy(tmpBuf, ("<script>dw(wl_RAD_use_cer)</script>"));
						goto setErr_encrypt;
					}
					if ( !apmib_set(MIB_WLAN_RS_USER_CERT_PASSWD, (void *)strVal)) {
						strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_RS_USER_CERT_PASSWD_err)</script>"));
						goto setErr_encrypt;
					}
				}
				else{
					if ( !apmib_set(MIB_WLAN_RS_USER_CERT_PASSWD, (void *)strVal)) {
						strcpy(tmpBuf, ("<script>dw(wl_Cle_MIB_WLAN_RS_USER_CERT_PASSWD_err)</script>"));
						goto setErr_encrypt;
					}
				...
				if(rsBandSel == PHYBAND_5G){
					if(isFileExist(RS_USER_CERT_5G) != 1){
						strcpy(tmpBuf, ("<script>dw(wl_No_802_RAD5u)</script>"));
						goto setErr_encrypt;
					}
					
					if(isFileExist(RS_ROOT_CERT_5G) != 1){
						strcpy(tmpBuf, ("<script>dw(wl_No_802_RAD5r)</script>"));
						goto setErr_encrypt;
					}
				}
				else if(rsBandSel == PHYBAND_2G){
					if(isFileExist(RS_USER_CERT_2G) != 1){
						strcpy(tmpBuf, ("<script>dw(wl_No_802_RAD2u)</script>"));
						goto setErr_encrypt;
					}
					
					if(isFileExist(RS_ROOT_CERT_2G) != 1){
						strcpy(tmpBuf, ("<script>dw(wl_No_802_RAD2r)</script>"));
						goto setErr_encrypt;
					}
				}
			}
				...
				if (strVal[0]) {
					if ( !string_to_dec(strVal, &intVal2) ) {
						strcpy(tmpBuf, ("<script>dw(wl_Inv_802_ins)</script>"));
						goto setErr_encrypt;
					}
					if ( !apmib_set(MIB_WLAN_EAP_INSIDE_TYPE, (void *)&intVal2)) {
						strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_EAP_INSIDE_TYPE_err)</script>"));
						goto setErr_encrypt;
					}
				}
				else{
					strcpy(tmpBuf, ("<script>dw(wl_No_802_ins)</script>"));
					goto setErr_encrypt;
				}
...
//					if(isFileExist(RS_USER_CERT) == 1){
						...
						if (strVal[0]) {
							...
							if ( !apmib_set(MIB_WLAN_RS_USER_CERT_PASSWD, (void *)strVal)) {
								strcpy(tmpBuf, ("<script>dw(wl_clear_MIB_WLAN_RS_USER_CERT_PASSWD_err)</script>"));
								goto setErr_encrypt;
							}
							//strcpy(tmpBuf, ("No 802.1x RADIUS user cert password!"));
							//goto setErr_encrypt;
...
//					}
					...
					strcpy(tmpBuf, ("<script>dw(wl_802_ins_tun)</script>"));
					goto setErr_encrypt;
				}
			}
			else{
				strcpy(tmpBuf, ("<script>dw(wl_802_EAP_typ)</script>"));
				goto setErr_encrypt;
			}
		}
		else
		...
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strVal[0]) {
			strcpy(tmpBuf, ("<script>dw(wl_No_RS_por)</script>"));
			goto setErr_encrypt;
		}
		if (!string_to_dec(strVal, &intVal) || intVal<=0 || intVal>65535) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_port)</script>"));
			goto setErr_encrypt;
		}
		if ( !apmib_set(MIB_WLAN_RS_PORT, (void *)&intVal)) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_RS_por)</script>"));
			goto setErr_encrypt;
		}
		sprintf(varName, "radiusIP%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strVal[0]) {
			strcpy(tmpBuf, ("<script>dw(wl_No_RS_IP)</script>"));
			goto setErr_encrypt;
		}
		if ( !inet_aton(strVal, &inIp) ) {
			strcpy(tmpBuf, ("<script>dw(wl_Inv_RS_IP-)</script>"));
			goto setErr_encrypt;
		}
		...
		if (strlen(strVal) > (MAX_RS_PASS_LEN -1) ) {
			strcpy(tmpBuf, ("<script>dw(wl_RS_pas_len)</script>"));
			goto setErr_encrypt;
		}
		if ( !apmib_set(MIB_WLAN_RS_PASSWORD, (void *)strVal)) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_RS_pas)</script>"));
			goto setErr_encrypt;
		}
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_RS_ret)</script>"));
				goto setErr_encrypt;
			}
			if ( !apmib_set(MIB_WLAN_RS_MAXRETRY, (void *)&intVal)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_RS_MAXRETRY_err)</script>"));
				goto setErr_encrypt;
			}
		}
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_RS_tim)</script>"));
				goto setErr_encrypt;
			}
			if ( !apmib_set(MIB_WLAN_RS_INTERVAL_TIME, (void *)&intVal)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_RS_INTERVAL_TIME_err)</script>"));
				goto setErr_encrypt;
			}
		}
		...
		if ( apmib_set( MIB_WLAN_ACCOUNT_RS_ENABLED, (void *)&intVal) == 0) {
  			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_ACCOUNT_RS_ENABLED_mib)</script>"));
			goto setErr_encrypt;
		}
		if (intVal == 0)
			goto get_wepkey;
		...
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strVal[0]) {
			strcpy(tmpBuf, ("<script>dw(wl_No_acc_RS)</script>"));
			goto setErr_encrypt;
		}
		if (!string_to_dec(strVal, &intVal) || intVal<=0 || intVal>65535) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val)</script>"));
			goto setErr_encrypt;
		}
		if ( !apmib_set(MIB_WLAN_ACCOUNT_RS_PORT, (void *)&intVal)) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_acc_RS)</script>"));
			goto setErr_encrypt;
		}
		sprintf(varName, "accountIP%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strVal[0]) {
			strcpy(tmpBuf, ("<script>dw(wl_No_acc_RS_ip)</script>"));
			goto setErr_encrypt;
		}
		if ( !inet_aton(strVal, &inIp) ) {
			strcpy(tmpBuf, ("<script>dw(wl_Inv_acc_RS)</script>"));
			goto setErr_encrypt;
		}
		if ( !apmib_set(MIB_WLAN_ACCOUNT_RS_IP, (void *)&inIp)) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_acc_RS_ip)</script>"));
			goto setErr_encrypt;
		}
		sprintf(varName, "accountPass%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strlen(strVal) > (MAX_RS_PASS_LEN -1) ) {
			strcpy(tmpBuf, ("<script>dw(wl_Acc_RS_pas)</script>"));
			goto setErr_encrypt;
		}
		if ( !apmib_set(MIB_WLAN_ACCOUNT_RS_PASSWORD, (void *)strVal)) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_acc_RS_pass)</script>"));
			goto setErr_encrypt;
		}
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_acc_RS_retry)</script>"));
				goto setErr_encrypt;
			}
			if ( !apmib_set(MIB_WLAN_ACCOUNT_RS_MAXRETRY, (void *)&intVal)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_ACCOUNT_RS_MAXRETRY_err)</script>"));
				goto setErr_encrypt;
			}
		}
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_acc_RS_time)</script>"));
				goto setErr_encrypt;
			}
			if ( !apmib_set(MIB_WLAN_ACCOUNT_RS_INTERVAL_TIME, (void *)&intVal)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_ACCOUNT_RS_INTERVAL_TIME_err)</script>"));
				goto setErr_encrypt;
			}
		}
		...
		if ( apmib_set( MIB_WLAN_ACCOUNT_RS_UPDATE_ENABLED, (void *)&intVal) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_ACCOUNT_RS_UPDATE_ENABLED_mib)</script>"));
			goto setErr_encrypt;
		}
		...
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_update)</script>"));
				goto setErr_encrypt;
			}
			if ( !apmib_set(MIB_WLAN_ACCOUNT_RS_UPDATE_DELAY, (void *)&intVal)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_ACCOUNT_RS_UPDATE_DELAY_mib)</script>"));
				goto setErr_encrypt;
			}
		}
...
get_wepkey:
		...
		if (strVal[0]) {
			if ( !strcmp(strVal, ("wep64")))
				...
				strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_wep)</script>"));
				goto setErr_encrypt;
			}
			if ( apmib_set(MIB_WLAN_WEP, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_WEP_fai)</script>"));
				goto setErr_encrypt;
			}
		}
	}
...
#endif		
	{
		strcpy(tmpBuf, ("<script>dw(wl_ssi_is_inv)</script>"));
		goto setErr_wlan;
	}
...
//printf("\r\n encrypt[%d],__[%s-%u]\r\n",encrypt,__FILE__,__LINE__);
	...
	else if (encrypt > ENCRYPT_WEP) 
	{
		...
		if (strWpaAuth[0]) 
		{
			if ( !strcmp(strWpaAuth, ("eap"))) {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_wpa_val)</script>"));
				//goto setErr_wlan;
			}
		...
		if(encrypt == ENCRYPT_WPA)
		{
			if (strCipherSuite[0]) {
				...
				{
					strcpy(tmpBuf, ("<script>dw(wl_Inv_val_of)</script>"));
					goto setErr_wlan;
				}
	...
	if ( !apmib_set(profile_mod_id, (void *)&target)) {
		strcpy(tmpBuf, ("<script>dw(wl_Mod_tab_ent)</script>"));
		goto setErr_wlan;
	}
...
//printf("\r\n ssid_idx=[%d],__[%s-%u]\r\n",ssid_idx,__FILE__,__LINE__);
...
#endif //#if defined(WLAN_PROFILE)		
	{
		...
		ENCRYPT_T encrypt = atoi(strEncrypt);
...
#ifdef BAND_STEERING_SUPPORT
					apmib_get(MIB_WIFI_STA_CONTROL_ENABLE, (void *)&band_steering_enable);
					if (band_steering_enable && (ssid_idx == 0))
						wlan_band_steering_update();
#endif
					return;	
				}
...
int wlMeshNeighborTable(request *wp, int argc, char **argv)
{
    ...
    nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
            "<td align=center width=\"17%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_mac_address)</script></b></font></td>\n"
            "<td align=center width=\"17%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_mode)</script></b></font></td>\n"
            "<td align=center width=\"17%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_tx_packets)</script></b></font></td>\n"
            "<td align=center width=\"17%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_rx_packets)</script></b></font></td>\n"
            "<td align=center width=\"17%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_tx_rate)</script></b></font></td>\n"
            "<td align=center width=\"17%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_rssi)</script></b></font></td>\n"
            "<td align=center width=\"17%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_expired_time)</script></b></font></td>\n"));
...
int wlMeshRoutingTable(request *wp, int argc, char **argv)
{
    ...
    nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
        "<td align=center width=\"15%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_destination_mesh_point)</script></b></font></td>\n"
        "<td align=center width=\"15%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_next_hop_mesh_point)</script></b></font></td>\n"
        "<td align=center width=\"10%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_portal_enable)</script></b></font></td>\n"
        "<td align=center width=\"10%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_metric)</script></b></font></td>\n"
        "<td align=center width=\"10%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_hop_count)</script></b></font></td>\n"));
...
int wlMeshPortalTable(request *wp, int argc, char **argv)
{
        ...
        nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
        "<td align=center width=\"16%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_portal_mac)</script></b></font></td>\n"));
...
int wlMeshProxyTable(request *wp, int argc, char **argv)
{
        ...
        nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
        "<td align=center width=\"50%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_owner)</script></b></font></td>\n"
        "<td align=center width=\"50%%\"><font size=\"2\"><b><script>dw(wlmeshinfo_client)</script></b></font></td></tr>\n"));
...
int wlAcList(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"45%%\" ><font size=\"2\"><b><script>dw(wlactrl_macaddr)</script></b></font></td>\n"
      	"<td align=center width=\"35%%\" ><font size=\"2\"><b><script>dw(wlactrl_comment)</script></b></font></td>\n"
      	"<td align=center width=\"20%%\" ><font size=\"2\"><b><script>dw(wlactrl_select)</script></b></font></td></tr>\n"));
...
void formWlAc(request *wp, char *path, char *query)
{
	...
	if (strAddMac[0] || strAddApply[0]) {
		...
		if ( apmib_set( MIB_WLAN_MACAC_ENABLED, (void *)&enabled) == 0) {
  			strcpy(tmpBuf, ("<script>dw(wlactrl_set_enable_flag)</script>"));
			goto setErr_ac;
		}
...
//			strcpy(tmpBuf, ("Error! No mac address to set."));
		...
		if (strlen(strVal)!=12 || !string_to_hex(strVal, macEntry.macAddr, 12)) {
			strcpy(tmpBuf, ("<script>dw(wlactrl_invalid_mac_address)</script>"));
			goto setErr_ac;
		}
		...
		if ( strVal[0] ) {
			if (strlen(strVal) > COMMENT_LEN-1) {
				strcpy(tmpBuf, ("<script>dw(wlactrl_comment_length_too_long)</script>"));
				goto setErr_ac;
			}
		...
		if ( !apmib_get(MIB_WLAN_MACAC_NUM, (void *)&entryNum)) {
			strcpy(tmpBuf, ("<script>dw(wlactrl_get_entry_number)</script>"));
			goto setErr_ac;
		}
		if ( (entryNum + 1) > MAX_WLAN_AC_NUM) {
			strcpy(tmpBuf, ("<script>dw(wlactrl_cannot_add_new_entry)</script>"));
			goto setErr_ac;
		}
		...
		for(j=1;j<=entryNum;j++)
		{
			...
			*((char *)&macEntrytmp) = (char)j;
			if ( apmib_get(MIB_WLAN_MACAC_ADDR, (void *)&macEntrytmp))
			{
				...
				{
					strcpy(tmpBuf, ("<script>dw(wlactrl_rule_exist)</script>"));
					goto setErr_ac;
				}
		...
		if ( apmib_set(MIB_WLAN_AC_ADDR_ADD, (void *)&macEntry) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlactrl_add_table_entry_error)</script>"));
			goto setErr_ac;
		}
	}
	...
	if (strDelMac[0]) {
		if ( !apmib_get(MIB_WLAN_MACAC_NUM, (void *)&entryNum)) {
			strcpy(tmpBuf, ("<script>dw(wlactrl_get_entry_number_error)</script>"));
			goto setErr_ac;
		}
		for (i=entryNum; i>0; i--) {
			snprintf(tmpBuf, 20, "select%d", i);
			...
			if ( !strcmp(strVal, "ON") ) {
				...
				*((char *)&macEntry) = (char)i;
				if ( !apmib_get(MIB_WLAN_MACAC_ADDR, (void *)&macEntry)) {
					strcpy(tmpBuf, ("<script>dw(wlactrl_get_table_entry_error)</script>"));
					goto setErr_ac;
				}
				if ( !apmib_set(MIB_WLAN_AC_ADDR_DEL, (void *)&macEntry)) {
					strcpy(tmpBuf, ("<script>dw(wlactrl_delete_table_entry_error)</script>"));
					goto setErr_ac;
				}
			}
		}
	...
	if ( strDelAllMac[0]) {
		if ( !apmib_set(MIB_WLAN_AC_ADDR_DELALL, (void *)&macEntry)) {
			strcpy(tmpBuf, ("<script>dw(wlactrl_delete_all_table_error)</script>"));
			goto setErr_ac;
		}
	}
...
int advanceHander(request *wp ,char *tmpBuf)
{
...
#ifdef BAND_STEERING_SUPPORT
	unsigned char band_steering_enable, prefer_band;
#endif
	...
	if (strAuth[0]) {
		if ( !strcmp(strAuth, ("open")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_aut)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_AUTH_TYPE, (void *)&authType) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_aut_fai)</script>"));
			goto setErr_advance;
		}
	}
	...
	if (strFragTh[0]) {
		if ( !string_to_dec(strFragTh, &val) || val<256 || val>2346) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_frag)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_FRAG_THRESHOLD, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_fra_thr)</script>"));
			goto setErr_advance;
		}
	}
	...
	if (strRtsTh[0]) {
		if ( !string_to_dec(strRtsTh, &val) || val<0 || val>2347) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_rts)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_RTS_THRESHOLD, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_RTS_thr)</script>"));
			goto setErr_advance;
		}
	}
	...
	if (strBeacon[0]) {
		if ( !string_to_dec(strBeacon, &val) || val<20 || val>1024) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_beacon)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_BEACON_INTERVAL, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_Bea_int)</script>"));
			goto setErr_advance;
		}
	}
	...
	if (strAckTimeout[0]) {
		if ( !string_to_dec(strAckTimeout, &val) || val<0 || val>255) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_ack)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_ACK_TIMEOUT, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_Ack_tim)</script>"));
			goto setErr_advance;
		}
	}
#if 0
	...
	if ( strRate[0] ) {
		if ( strRate[0] == '0' ) { // auto
			val = 1;
			if ( apmib_set(MIB_WLAN_RATE_ADAPTIVE_ENABLED, (void *)&val) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_rat_ada)</script>:"));
				goto setErr_advance;
			}
		}
		else  {
			...
			val = 1 << (val-1);
			if ( apmib_set(MIB_WLAN_FIX_RATE, (void *)&val) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_fix_rat)</script>:"));
				goto setErr_advance;
			}
			...
			if ( strRate[0] ) {
				...
				if ( apmib_set(MIB_WLAN_BASIC_RATES, (void *)&val) == 0) {
					strcpy(tmpBuf, ("<script>dw(wl_Set_Tx_bas)</script>:"));
					goto setErr_advance;
				}
			}
			...
			if ( strRate[0] ) {
				...
				if ( apmib_set(MIB_WLAN_SUPPORTED_RATES, (void *)&val) == 0) {
					strcpy(tmpBuf, ("<script>dw(wl_Set_Tx_ope)</script>:"));
					goto setErr_advance;
				}
			}	
		}
...
skip_rate_setting:
	...
	if (strPreamble[0]) {
		if (!strcmp(strPreamble, ("long")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_Pre)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_PREAMBLE_TYPE, (void *)&preamble) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_Pre_fai)</script>"));
			goto setErr_advance;
		}
	}
//move to basic setting page
	...
	if (strHiddenSSID[0]) {
		if (!strcmp(strHiddenSSID, ("no")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_hid)</script>:"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_HIDDEN_SSID, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_hid_ssi)</script>:"));
			goto setErr_advance;
		}
	}
#endif
	...
	if (strDtim[0]) {
		if ( !string_to_dec(strDtim, &val) || val<1 || val>255) {
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_val_dtim)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_DTIM_PERIOD, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_DTI_per)</script>"));
			goto setErr_advance;
		}
	}
	...
	if (strIapp[0]) {
		if (!strcmp(strIapp, ("no")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_IAP)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_IAPP_DISABLED, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_IAPP_DISABLED_fai)</script>"));
			goto setErr_advance;
		}
	}
#ifdef WLAN_HS2_CONFIG	
	...
	if (strHs2[0]) {
		if (!strcmp(strHs2, ("no")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_HS2)</script>"));
			goto setErr_advance;
		}
		
		if ( apmib_set(MIB_WLAN_HS2_ENABLE, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_HS2_ENABLED_fai)</script>"));
			goto setErr_advance;
		}
	}
#endif			
	...
	if (strProtection[0]) {
		if (!strcmp(strProtection, ("no")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_11g)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_PROTECTION_DISABLED, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_PROTECTION_DISABLED_fai)</script>"));
			goto setErr_advance;
		}
	}
#if 0	
// for WMM move to basic setting
	...
	if (strProtection[0]) {
		if (!strcmp(strProtection, ("on")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_WMM)</script>:"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_WMM_ENABLED, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_WMM_ENABLED_fai)</script>:"));
			goto setErr_advance;
		}
	}
#endif	
	...
	if (strTurbo[0]) {
		if (!strcmp(strTurbo, ("off")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_tur)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_TURBO_MODE, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_TURBO_MODE_fai)</script>"));
			goto setErr_advance;
		}
	}
	...
	if (strPower[0]) {		
		if (!strcmp(strPower, ("0")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_RF)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_RFPOWER_SCALE, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_RFPOWER_SCALE_fai)</script>"));
			goto setErr_advance;
		}
	}
#if 0
// for 11N
	...
	if (strProtection[0]) {
		if ( strProtection[0] == '0')
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_Cha)</script>:"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_CHANNEL_BONDING, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_CHANNEL_BONDING_fai)</script>:"));
			goto setErr_advance;
		}
	}
	...
	if (strProtection[0]) {
		if ( strProtection[0] == '0')
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_Con)</script>:"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_CONTROL_SIDEBAND, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_CONTROL_SIDEBAND_fai)</script>:"));
			goto setErr_advance;
		}
	}
#endif	
	...
	if (strProtection[0]) {
		...
		if ( apmib_set(MIB_WLAN_AGGREGATION, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_AGGREGATION_fai)</script>"));
			goto setErr_advance;
		}
	}
	...
	if (strProtection[0]) {
		if (!strcmp(strProtection, ("on")))
			...
			strcpy(tmpBuf, ("<script>dw(wl_Err_Inv_sho)</script>"));
			goto setErr_advance;
		}
		if ( apmib_set(MIB_WLAN_SHORT_GI, (void *)&val) == 0) {
			strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_SHORT_GI_fai)</script>"));
			goto setErr_advance;
		}
	}
	...
	wlan_idx = (wlan_idx == 0?1:0);
	vwlan_idx = 0;
	apmib_get(MIB_WLAN_DOT11V_ENABLE, (void *)&k);
	apmib_recov_wlanIdx();
		...
		if (!strcmp(strValue, ("enable"))){
			...
			val = 0;	
			if(!k) //if other interface 11v feature is disabled
				i &= ~CAPWAP_11V_ENABLE;	
		}
	...
	k = 1;
        ...
        strValue = req_get_cstream_var(wp, ("smart_roaming_"), "");
		apmib_get(MIB_CAPWAP_MODE, (void *)&val);
		int wtp_id = 1;
		if (!strcmp(strValue, ("enable")))
			val |= CAPWAP_ROAMING_ENABLE;
		else 
			val &= ~CAPWAP_ROAMING_ENABLE;	
       apmib_set(MIB_CAPWAP_MODE, (void *)&val);
       apmib_set(MIB_CAPWAP_WTP_ID, (void *)&wtp_id);
#endif

#ifdef BAND_STEERING_SUPPORT
		strValue = req_get_cstream_var(wp, ("band_steering_"), "");
		if (strValue[0]) {
			if (!strcmp(strValue, ("enable")))
				band_steering_enable = 1;
			else
				band_steering_enable = 0;
			apmib_set(MIB_WIFI_STA_CONTROL_ENABLE, (void *)&band_steering_enable);
		}

		strValue = req_get_cstream_var(wp, ("band_steering_prefer_band"), "");
		if (strValue[0]) {
			if (!strcmp(strValue, ("0")))
				prefer_band = 0;
			else
				prefer_band = 1;
			apmib_set(MIB_PREFER_BAND, (void *)&prefer_band);
		}
		...
		strValue = req_get_cstream_var(wp, ("crossband_"), "");
		if (!strcmp(strValue, ("enable")))
			val = 1;
		else
			val = 0;
		if(wlan_idx == 0){
			apmib_set(MIB_WLAN_CROSSBAND_ENABLE, (void *)&val);
			wlan_idx = 1; /*set 2G setting*/
			apmib_set(MIB_WLAN_CROSSBAND_ENABLE, (void *)&val);
			wlan_idx = 0;
		}
		...
		//### end
		strValue = req_get_cstream_var(wp, ("twt_"), "");
		if (strValue[0]) {
			if (!strcmp(strValue, ("enable")))
				val = 1;
			else
				val = 0;
			apmib_set(MIB_WLAN_TWT_ENABLED, (void *)&val);	
		}
...
void set_11ax_txrate(WLAN_STA_INFO_Tp pInfo, char *txrate)
{
	char channelWidth = 0;//20M 0,40M 1,80M 2
	unsigned short rate_idx = (pInfo->acTxOperaRate - HE_RATE_ID)/16 * 12
		+ (pInfo->acTxOperaRate - HE_RATE_ID)%16;
	...
	sprintf(txrate, "%d", HE_MCS_DATA_RATE[channelWidth][0][rate_idx]>>1);
}
...
void set_11ac_txrate(WLAN_STA_INFO_Tp pInfo,char* txrate)
{
	...
	if(pInfo->acTxOperaRate)
		rate_idx=pInfo->acTxOperaRate-VHT_RATE_ID;
	else
		rate_idx=pInfo->TxOperaRate-VHT_RATE_ID;
/*
...
int wirelessClientList(request *wp, int argc, char **argv)
{
	...
	if (argc == 2) {
		...
		sprintf(virtual_name, "-"VAP_IF_ONLY, virtual_index);
	...
	for (i=1; i<=MAX_STA_NUM; i++) {
		...
		if(pInfo->acTxOperaRate >= VHT_RATE_ID)
		{
			if(pInfo->acTxOperaRate >= HE_RATE_ID)
				set_11ax_txrate(pInfo, txrate);
			else
				set_11ac_txrate(pInfo, txrate);
		}
		else if(pInfo->TxOperaRate >= VHT_RATE_ID)
		{
			...
			txrate,
			( (pInfo->flags & STA_INFO_FLAG_ASLEEP) ? "<script>dw(wlstatbl_yes)</script>" : "<script>dw(wlstatbl_no)</script>"),
	...
	if (found == 0) {
		nBytesSent += req_format_write(wp,
	   		("<tr class=\"tbl_body\"><td><font size=2><script>dw(wlstatbl_none)</script></td>"
			"<td><font size=2>---</td>"
	     		"<td><font size=2>---</td>"
			"<td><font size=2>---</td>"
			"<td><font size=2>---</td>"
	...
	for (idx=1; idx<=4; idx++) {
		...
		if (strVal[0]) {
			...
			if(wlan_onoff_tkip == 0) //Wifi request
			{
				...
				if( band_no == 7 || band_no == 9 || band_no == 10 || band_no == 11|| band_no == 63 || band_no == 71 || band_no == 75 || band_no == 203) //7:n; 9:gn; 10:bgn 11:5g_an
				{
					if(wlan_encrypt ==ENCRYPT_WPA || wlan_encrypt ==ENCRYPT_WPA2){
						wpaCipher &= ~WPA_CIPHER_TKIP;
#if !(defined(CONFIG_APP_HOSTAPD) || defined(CONFIG_APP_WPA_SUPPLC))
							if(wpaCipher== 0)
								wpaCipher =  WPA_CIPHER_AES;
#endif
						apmib_set(MIB_WLAN_WPA_CIPHER_SUITE, (void *)&wpaCipher);
		...
		if(strlen(strVal)>=MAX_SSID_LEN)
		{
			sprintf(tmpBuf, "wlbasic_set_ssid_toolong");
		}
		if (strVal[0]) 
			apmib_set( MIB_WLAN_SSID, (void *)strVal);
...
*/
    ...
    if ( connect[0] ) 
    {
        char *wlanifp, *strSSID, tmp_wlanif[20]={0};
        int rptEnabled=0;
        int wlan_mode=0;
        int wlanvxd_mode=0;
        char *strChannel;
        int channelIdx;
        int tmp_wlan_index=0, tmp_vwlan_index=0;
#if defined(WLAN_PROFILE)
		...
		if(rtk_wlan_get_ifname_idx(wlanifp, &tmp_wlan_index, &tmp_vwlan_index) == -1)
		{
			strncpy(tmpBuf, ("<script>dw(wlsurvey_wlanif_error)</script>"), sizeof(tmpBuf));
			goto ss_err;
		}
		else
		{
			rtk_wlan_get_ifname(tmp_wlan_index, tmp_vwlan_index, tmp_wlanif);
			wlanifp = tmp_wlanif;
		}
//printf("\r\n ++++++++++++wlanifp=[%s],__[%s-%u]\r\n",wlanifp,__FILE__,__LINE__);
		...
		if (strSel[0]) {
			...
			int chan;
			if (pStatus == NULL) {
				strcpy(tmpBuf, ("<script>dw(wlsurvey_please_refresh_again)</script>"));
				goto ss_err;

			}
			sscanf(strSel, "sel%d", &idx);
			if ( idx < 0 || idx >= pStatus->number ) { // invalid index
				strcpy(tmpBuf, ("<script>dw(wlsurvey_connect_failed1)</script>"));
				goto ss_err;
			}
#ifdef CONFIG_RTK_MESH 
            if(mesh_connect) 
            { 
                ...
                {
                    strcpy(tmpBuf, ("<script>dw(wlsurvey_get_meshid_error)</script>"));
                    goto ss_err;
                }

                pMsg = "<script>dw(wlsurvey_connect_failed2)</script>";



                if ((pStatus->bssdb[idx].t_stamp[1] & 0x06) == 0x06)
                    offset = 1;/*HT_2NDCH_OFFSET_BELOW;*/
                else if ((pStatus->bssdb[idx].t_stamp[1]  & 0x06) == 0x02)
                ...
                if(!setWlJoinMesh(WLAN_IF, pStatus->bssdb[idx].meshid - 2, pStatus->bssdb[idx].meshidlen, pStatus->bssdb[idx].channel, offset)) // the problem of padding still exists
                { 
                    ...
                    {
                        if(!getWlMeshLink(WLAN_IF, pStatus->bssdb[idx].bssid, 6))
                        {
                            char tmp[MESHID_LEN]; 
                            int channel; 
                            memcpy(tmp, pStatus->bssdb[idx].meshid - 2, pStatus->bssdb[idx].meshidlen); // the problem of padding still exists
                            tmp[pStatus->bssdb[idx].meshidlen] = '\0';
                            if ( apmib_set(MIB_WLAN_MESH_ID, (void *)tmp) == 0)
                            { 
                                strcpy(tmpBuf, ("<script>dw(wlsurvey_set_meshid_error)</script>")); 
                                goto ss_err; 
                            } 
                            // channel = pStatus->bssdb[idx].channel;
                            channel = 0; // requirement of Jason, not me 
                            if ( apmib_set(MIB_WLAN_CHANNEL, (void*)&channel) == 0)
                            { 
                                strcpy(tmpBuf, ("<script>dw(wlsurvey_set_channel_error)</script>")); 
                                goto ss_err; 
                            } 

                            if(meshWpaHandler(wp, tmpBuf, wlan_idx) < 0) {
                                strcpy(tmpBuf, ("<script>dw(wlsurvey_set_wpa_error)</script>"));
                                goto ss_err;                             
                            }    
                ...
                if(strcmp(pMsg, "Connect successfully!!"))
                {
                    ...
                    strcpy(tmpBuf, ("<script>dw(wlsurvey_connect_failed3)</script>"));
                    goto ss_err; 
                }
            } 
            else 
			...
			{ 
                                ...
                                if(rtk_wlan_get_ifname_idx(wlanif, &tmp_wlan_index, &tmp_vwlan_index) == -1)
                                {
                                        strncpy(tmpBuf, ("<script>dw(wlsurvey_wlanif_error)</script>"), sizeof(tmpBuf));
                                        goto ss_err;
                                }
...
#ifdef IS_AX_SUPPORT
#ifdef WLAN_WPA3
										else if(val == ENCRYPT_WPA3 || val == ENCRYPT_WPA2_WPA3_MIXED)
										{
											if(wpaHandler(wp, tmpBuf, wlan_idx) < 0)
											{
												goto ss_err;
			...
			if ( !apmib_get( MIB_WLAN_ENCRYPT, (void *)&encrypt) ) {
				strcpy(tmpBuf, ("<script>dw(wl_Che_enc_err)</script>:"));
				goto ss_err;
			}
			else {
				// no encryption
				if (encrypt == ENCRYPT_DISABLED)
				{
					if (pStatus->bssdb[idx].capability & 0x00000010) {
						strcpy(tmpBuf, ("<script>dw(wl_Enc_typ_mis)</script>:"));
						goto ss_err;
					}
					else
						; // success
				...
				else if (encrypt == ENCRYPT_WAPI)
				{
					...
					else if (pStatus->bssdb[idx].t_stamp[0] != SECURITY_INFO_WAPI) {
						strcpy(tmpBuf, ("<script>dw(wl_Enc_typ_mis)</script>:"));
						goto ss_err;
					}
					else
...
//printf("\r\n tmpBuf=[%s],__[%s-%u]\r\n",tmpBuf,__FILE__,__LINE__);
			...
			if ( apmib_set(MIB_WLAN_NETWORK_TYPE, (void *)&net) == 0) {
				strcpy(tmpBuf, ("<script>dw(wlsurvey_set_networktype_failed)</script>"));
				goto ss_err;
			}
			...
			if (net == ADHOC) {
				...
				if ( apmib_set( MIB_WLAN_CHANNEL, (void *)&chan) == 0) {
   					strcpy(tmpBuf, ("<script>dw(wlsurvey_set_channelnum_error)</script>"));
					goto ss_err;
				}
				int is_40m_bw ;// = (pStatus->bssdb[idx].t_stamp[1] & 2) ? 1 : 0;
				if(pStatus->bssdb[idx].t_stamp[1] & 64)
					is_40m_bw = 2;
				else if(pStatus->bssdb[idx].t_stamp[1] & 2)
					is_40m_bw = 1;
				else
					is_40m_bw = 0;				
...
//
			...
			while (1) {
				if ( getWlJoinRequest(WLAN_IF, &pStatus->bssdb[idx], &res) < 0 ) {
					strcpy(tmpBuf, ("<script>dw(wl_Joi_req_fai)</script>:"));
					goto ss_err;
				}
				if ( res == 1 ) { // wait
				...
				#endif
					{
						strcpy(tmpBuf, ("<script>dw(wl_conreq_tim_)</script>:"));
						goto ss_err;
					}
					sleep(1);
					continue;
			...
			else 
			{
				...
				while (1) {
					if ( getWlJoinResult(WLAN_IF, &res) < 0 ) {
						strcpy(tmpBuf, ("<script>dw(wl_Get_Joi_res)</script>:"));
						goto ss_err;
					}
					...
					{
						if (wait_time++ > 10) {
							strcpy(tmpBuf, ("<script>dw(wl_con_tim_)</script>:"));
							goto ss_err;
						}
					}
					sleep(1);
...
#ifndef IS_AX_SUPPORT
			{
				...
				while (1) 
					{
						...
						if ( getWlJoinResult(WLAN_IF, &res) < 0 ) {
							strcpy(tmpBuf, ("<script>dw(wlsurvey_get_join_result_failed)</script>"));
							goto ss_err;
						}
					...
					{
						if (wait_time++ > max_wait_time) {
							strcpy(tmpBuf, ("<script>dw(wlsurvey_connect_timeout)</script>"));
							goto ss_err;
						}
					}
					sleep(1);
				...
				if ( res!=STATE_Bss && res!=STATE_Ibss_Idle && res!=STATE_Ibss_Active )
					pMsg = (unsigned char *)"<script>dw(wlsurvey_connect_failed4)</script>";
				else {					
					status = 0;
					...
					if (status)
						pMsg =  (unsigned char *)"<script>dw(wlsurvey_connect_timeout)</script>";//"Connect failed 5!";//Input negative AES password can trigger here
					else
						pMsg =  (unsigned char *)"<script>dw(wlsurvey_connect_successfully)</script>";
...
//printf("\r\n oriProfileEnabledVal=[%d],__[%s-%u]\r\n",oriProfileEnabledVal,__FILE__,__LINE__);
		...
		apmib_update(CURRENT_SETTING);
			...
			if(!status)
			{
				pMsg = "<script>dw(wlsurvey_connect_success_reboot)</script>";
			...
			{
#ifdef IS_AX_SUPPORT
				RET_SURVEY_PAGE_COMMON(pMsg, submitUrl, (status?0:1), wlan_idx, isWizard);
#else
...
#endif 
	BssDscr *pBss;
	char tmpBuf[MAX_MSG_BUFFER_SIZE], ssidbuf[40], tmp1Buf[20], tmp2Buf[20], wpa_tkip_aes[20],
		wpa2_tkip_aes[20], tmp3Buf[128], pmf_status[20];
	WLAN_MODE_T mode;
	bss_info bss;
	int ssidLen=0;
...
//#ifdef CONFIG_RTK_MESH
	...
	{ 
		...
		{ 
			...
			memcpy(meshidbuf, pBss->meshid - 2, pBss->meshidlen); // the problem of padding still exists
			meshidbuf[pBss->meshidlen] = '\0';
	...
	{
	nBytesSent += req_format_write(wp, ("<td align=center width=\"15%%\"><font size=\"2\"><b><script>dw(wlsurvey_tbl_select)</script></b></font></td></tr>\n"));
	}
	else
		nBytesSent += req_format_write(wp, ("</tr>\n"));
	...
	for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) {
		...
		ssidLen = ((pBss->ssidlen>=SSID_LEN)?SSID_LEN:pBss->ssidlen);
		memcpy(ssidbuf, pBss->ssid, ssidLen);
		ssidbuf[ssidLen] = '\0';
...
#ifdef IS_AX_SUPPORT
		else if(pBss->network== (BAND_11A |BAND_11N | BAND_5G_11AC|BAND_5G_11ANACAX))
			snprintf(tmp1Buf, sizeof(tmp1Buf), " (A+N+AC+AX)");
#endif
		...
		if ((pBss->capability & cPrivacy) == 0)
			...
			if (pBss->t_stamp[0] == 0)
				...
				if((pBss->t_stamp[1] & 0x00000800) || (pBss->bss_akm_suite & 0x00000100))
				{
					idx += sprintf(tmp2Buf+idx, "WPA3");
					wpa3_exist = 1;
				}
				//if (pBss->t_stamp[0] & 0xffff0000) {
				if ((pBss->t_stamp[0] & 0xf0000000) || (pBss->bss_akm_suite & 0x0004) || (!((pBss->t_stamp[1] & 0x00000800) || (pBss->bss_akm_suite & 0x00000100)) && (pBss->t_stamp[0] & 0xffff0000))) {
					if (wpa_exist || wpa3_exist)
						idx += sprintf(tmp2Buf+idx, "/");
					idx += sprintf(tmp2Buf+idx, "WPA2");
					if (((pBss->t_stamp[0] & 0xf0000000) >> 28) == 0x4 || (pBss->bss_akm_suite & 0x0004))
						idx += sprintf(tmp2Buf+idx, "-PSK");
					else if (((pBss->t_stamp[0] & 0xf0000000) >> 28) == 0x2)
						idx += sprintf(tmp2Buf+idx, "-1X");

					if (((pBss->t_stamp[1]&0x600)==0x600) || ((pBss->t_stamp[1]&0x600)==0x400)){
						idx += sprintf(tmp2Buf+idx, "");
					}
...
#if 0
		if( mesh_enable && (pBss->meshidlen > 0) )
		{
			nBytesSent += req_format_write(wp, ("<tr>"
			...
			"<td align=center width=\"10%%\" bgcolor=\"#C0C0C0\"><font size=\"2\">%d</td>\n"),
			ssidbuf, tmpBuf, pBss->channel, tmp1Buf, "Mesh Node", tmp2Buf, pBss->rssi);
		}
		else
#endif
		{
			...
			tmp3Buf, tmpBuf, pBss->channel, tmp1Buf,
			((pBss->capability & cIBSS) ? "Ad hoc" : "AP"), tmp2Buf, pBss->rssi);
		}
		...
		)
		{
			...
			"\"select\" value=\"sel%d\" onClick=\"enableConnect(%d, 0, %d)\"><input type=\"hidden\" id=\"pmf_status_%d\" value=\"%s\" ></td></tr>\n"),
			i,tmp3Buf, i,tmpBuf,i,pBss->channel,i,tmp2Buf,i,wpa_tkip_aes,i,wpa2_tkip_aes ,i,i, pBss->t_stamp[2], i, pmf_status);
		}
		else
			nBytesSent += req_format_write(wp, ("</tr>\n"));
	}
	...
	if( pStatus->number == 0 )
	{
		...
		{
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
	                "<td align=center width=\"20%%\" ><font size=\"2\"><script>dw(wlsurvey_tbl_none)</script></td>\n"
	...
	{ 
		...
		"<tr class=\"tbl_head\"><h4><font><br><br><script>dw(wlsurvey_list_mesh)</script></font></tr><tr>"
		"<td align=center width=\"30%%\" ><font size=\"2\"><b><script>dw(wlsurvey_tbl_meshid)</script></b></font></td>\n" 
		"<td align=center width=\"20%%\" ><font size=\"2\"><b><script>dw(wlsurvey_tbl_macaddr)</script></b></font></td>\n" 
		"<td align=center width=\"10%%\" ><font size=\"2\"><b><script>dw(wlsurvey_tbl_chan)</script></b></font></td>\n")); 
		nBytesSent += req_format_write(wp, ("<td align=center width=\"10%%\"><font size=\"2\"><b><script>dw(wlsurvey_tbl_select)</script></b></font></td></tr>\n"));
		...
		for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) 
		{
			...
			("<td align=center width=\"10%%\" >"
             ...
             "</td></tr>\n"),i, ssidbuf,  i, "no", i, pBss->channel, i, i);
		}
		if( mesh_count == 0 )
		{
			nBytesSent += req_format_write(wp, ("<tr>"
			"<td align=center width=\"30%%\"><font size=\"2\"><script>dw(wlsurvey_tbl_none)</script></td>\n"
			"<td align=center width=\"20%%\" ><font size=\"2\"></td>\n"
			"<td align=center width=\"10%%\" ><font size=\"2\"></td>\n"
			"<td align=center width=\"10%%\" ><font size=\"2\"></td>\n"));
		}
...
void formWlWds(request *wp, char *path, char *query)
{
	...
	if (strAddMac[0] || strSaveApply[0]) {
		if ( !strcmp(strEnabled, "ON"))
			...
			enabled = 0;
		if ( apmib_set( MIB_WLAN_WDS_ENABLED, (void *)&enabled) == 0) {
  			strcpy(tmpBuf, ("<script>dw(wlwds_set_enable_flag)</script>"));
			goto setErr_wds;
		}
		...
		if (strlen(strVal)!=12 || !string_to_hex(strVal, macEntry.macAddr, 12)) {
			strcpy(tmpBuf, ("<script>dw(wlwds_invalid_mac_address)</script>"));
			goto setErr_wds;
		}
		...
		if ( strVal[0] ) {
			if (strlen(strVal) > COMMENT_LEN-1) {
				strcpy(tmpBuf, ("<script>dw(wlwds_comment_length_too_long)</script>"));
				goto setErr_wds;
			}
			strcpy( (char *)macEntry.comment, strVal);
		}
		...
		if ( !apmib_get(MIB_WLAN_WDS_NUM, (void *)&entryNum)) {
			strcpy(tmpBuf, ("<script>dw(wlwds_get_entry_number)</script>"));
			goto setErr_wds;
		}
		if ( (entryNum + 1) > maxWDSNum) {
			strcpy(tmpBuf, ("<script>dw(wlwds_cannot_add_new_entry)</script>"));
			goto setErr_wds;
		}
		...
		for(j=1;j<=entryNum;j++)
		{
			...
			*((char *)&macEntrytmp) = (char)j;
			if ( apmib_get(MIB_WLAN_WDS, (void *)&macEntrytmp))
			{
				if (!memcmp(macEntrytmp.macAddr, macEntry.macAddr, 6))
				{
					strcpy(tmpBuf, ("<script>dw(wlwds_rule_exist)</script>"));
					goto setErr_wds;
				}
		...
		apmib_set(MIB_WLAN_WDS_DEL, (void *)&macEntry);
		if ( apmib_set(MIB_WLAN_WDS_ADD, (void *)&macEntry) == 0) {
			strcpy(tmpBuf, ("<script>dw(wlwds_add_table_entry_error)</script>"));
			goto setErr_wds;
		}
	}
	...
	if (strDelMac[0]) {
		if ( !apmib_get(MIB_WLAN_WDS_NUM, (void *)&entryNum)) {
			strcpy(tmpBuf, ("<script>dw(wlwds_get_entry_number_error)</script>"));
			goto setErr_wds;
		}
		for (i=entryNum; i>0; i--) {
			snprintf(tmpBuf, 20, "select%d", i);
			...
			if ( !strcmp(strVal, "ON") ) {
				...
				*((char *)&macEntry) = (char)i;
				if ( !apmib_get(MIB_WLAN_WDS, (void *)&macEntry)) {
					strcpy(tmpBuf, ("<script>dw(wlwds_get_table_entry_error)</script>"));
					goto setErr_wds;
				}
				if ( !apmib_set(MIB_WLAN_WDS_DEL, (void *)&macEntry)) {
					strcpy(tmpBuf, ("<script>dw(wlwds_delete_table_entry_error)</script>"));
					goto setErr_wds;
				}
			}
		}
	...
	if ( strDelAllMac[0]) {
		if ( !apmib_set(MIB_WLAN_WDS_DELALL, (void *)&macEntry)) {
			strcpy(tmpBuf, ("<script>dw(wlwds_delete_all_table_error)</script>"));
			goto setErr_wds;
		}
	}
...
int wlProfileTblList(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
			"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(wlbasic_wifi_config_ssid)</script></b></font></td>\n"
			"<td align=center width=\"\" ><font size=\"2\"><b><script>dw(wlbasic_wifi_config_encrypt)</script></b></font></td>\n"
			"<td align=center width=\"20%%\"><font size=\"2\"><b><script>dw(wlbasic_wifi_config_select)</script></b></font></td></tr>\n"));
...
int wlProfileList(request *wp, int argc, char **argv)
{
	...
	for (i=1; i<=entryNum; i++) {		
		...
		*((char *)&entry) = (char)i;
		...
		// fix for :translate_control_code_sprintf will change length of PSK
		// which will lead to out-of-bands access.
		char  tmpTranBuffwpaPSK[500];
		strcpy(tmpTranBuffwpaPSK, entry.wpaPSK);
		translate_control_code(tmpTranBuffwpaPSK);
		//Ssid|Encrypt|Authtype|wep|Wepkeytype|wpaCipher|wpa2Cipher|Pskformat|Wpapsk
		nBytesSent += req_format_write(wp, ("token[%d] =\'%s|%s|%d|%s|%d|%d|%d|%d|%s\';\n"),(i-1), ssidBuf,encryptBuf, entry.auth, wepBuf, entry.wepKeyType, wpaCipherVal, wpaCipherVal, entry.wpaPSKFormat, tmpTranBuffwpaPSK );
	}
	return nBytesSent;
...
int wlWdsList(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"35%%\" ><font size=\"2\"><b><script>dw(wlwds_mac_addr)</script></b></font></td>\n"
      	"<td align=center width=\"25%%\" ><font size=\"2\"><b><script>dw(wlwds_fmwlan_txrate)</script></b></font></td>\n"
      	"<td align=center width=\"30%%\" ><font size=\"2\"><b><script>dw(wlwds_fmwlan_comment)</script></b></font></td>\n"
      	"<td align=center width=\"10%%\" ><font size=\"2\"><b><script>dw(wlwds_fmwlan_select)</script></b></font></td></tr>\n"));
	...
	for (i=1; i<=entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		if(entry.fixedTxRate == 0){	
				sprintf(txrate, "%s","<script>dw(wlwds_rate_auto)</script>"); 
		}else{
			for(rateid=0; rateid<56;rateid++){
...
void formWdsEncrypt(request *wp, char *path, char *query)
{
	...
	if (strVal[0]) {
		...
		if (encrypt != WDS_ENCRYPT_DISABLED && encrypt != WDS_ENCRYPT_WEP64 &&
			encrypt != WDS_ENCRYPT_WEP128 && encrypt != WDS_ENCRYPT_TKIP &&
 			...
 			strcpy(tmpBuf, ("<script>dw(wl_enc_val_not)</script>"));
	...
	if (encrypt == WDS_ENCRYPT_WEP64 || encrypt == WDS_ENCRYPT_WEP128) {
		...
		if (strVal[0]) {
			if (strVal[0]!='0' && strVal[0]!='1') {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_wep_key)</script>"));
				goto setErr_wdsEncrypt;
		}
			intVal = strVal[0] - '0';
		...
		if  (strVal[0]) {
			if (strlen(strVal) != keyLen) {
				strcpy(tmpBuf, ("<script>dw(wl_Inv_wep_key_len)</script>"));
				goto setErr_wdsEncrypt;
		}
			if ( !isAllStar(strVal) ) {
				if (intVal == 0) { // ascii
...
void formAutoCfg(request *wp, char *path, char *query)
{
			...
			if (strVal[0])
				...
				if (strVal[0])
					...
					if (strVal[0])
						...
						strcpy(tmpBuf, ("<script>dw(wl_Err_no_act)</script>"));
						goto setErr_autocfg;
					}
				}
			}
	...
	if (isAP && isDoConfigQuestion) {
		strcpy(tmpBuf, ("<script>dw(wl_Err_inv_act)</script>"));
		goto setErr_autocfg;
	}
	...
	if ( strVal[0] ) {
		...
		&&  strVal[0]!= '8'
  			...
  			strcpy(tmpBuf, ("<script>dw(wl_Inv_mod_val)</script>"));
			goto setErr_autocfg;
		}
		mode = strVal[0] - '0';
	}
			...
			*r = '\0';
			...
			if (!isAP && isAdhoc) {
				...
				if (tmpBuf1[0] && tmpBuf[0] && strcmp(hashBuf, tmpBuf)) {
		  			strcpy(tmpBuf, ("<script>dw(wl_The_que_sel)</script>:"));
					goto setErr_autocfg;					
				}				
			}
#endif			
...
#if (defined WIFI_SIMPLE_CONFIG || defined RTK_MESH_CONFIG) && !defined WLAN_EASY_CONFIG
void sigHandler_autoconf(int signo)
{
	#define REINIT_WEB_FILE		("/tmp/reinit_web")
	struct stat status;
...
#ifdef RTK_MESH_CONFIG
	FILE *file = fopen(REINIT_WEB_FILE, "r");
	char tmpbuf[10] = {0};
	int val = 0;
	if(file){
		fgets(tmpbuf,10,file);
	...
	if(val == 1){
		...
		extern int set_shm_id(int id);
		shm_id = get_shm_id();
#endif	
		if(shm_id == 0){
			key_t shm_key = 6166529;
			shm_id = shmget(shm_key, 5000000, S_IRUSR |S_IWUSR);
			char *shm_addr = (char *)shmat(shm_id, NULL, 0);
			//printf("boa shared memory attached at address %p (id=%d)\n", shm_addr, shm_id);
			printf("boa read from capwap image_len = %d\n", *(int *)shm_addr);
#if defined(CONFIG_APP_FWD)
			set_shm_id(shm_id);
#endif
			char buffer[200];
			FirmwareUpgrade(shm_addr+sizeof(int), *(int *)shm_addr, 0, buffer);
			shmdt(shm_addr);
		}
		return;
	}
...
static void update_wps_configured(int reset_flag)
{
...
#ifdef WLAN_WPA3
	else if (encrypt1 == ENCRYPT_WPA3) {
		auth = WSC_AUTH_WPA3SAE;
		apmib_get(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&encrypt1);
		if (encrypt1 == WPA_CIPHER_TKIP)
			encrypt2 = WSC_ENCRYPT_TKIP;
		else if (encrypt1 == WPA_CIPHER_AES)
	...
	else if (encrypt1 == ENCRYPT_WPA2_WPA3_MIXED) {
		auth = WSC_AUTH_WPA3WPA2MIXED;
		apmib_get(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&encrypt1);
		if (encrypt1 == WPA_CIPHER_TKIP)
			encrypt2 = WSC_ENCRYPT_TKIP;
		else if (encrypt1 == WPA_CIPHER_AES)
...
#ifdef WLAN_WPA3
		|| encrypt1 == ENCRYPT_WPA3 || encrypt1 == ENCRYPT_WPA2_WPA3_MIXED
#endif
	) {
		apmib_get(MIB_WLAN_WPA_AUTH, (void *)&format);
		if (format & 2) { // PSK
...
void formWsc(request *wp, char *path, char *query)
{
	...
	int wlan_orig;
#ifdef WLAN_WPS_HAPD
	char cmd[100]={0}, wsc_pin[100]={0};
#endif
	unsigned int encrypt=0;
	unsigned char wps_start_led_num=2;
...
//			printf("Reset wlan to default fail!! No wlan name. %s,%d\n",__FUNCTION__ , __LINE__);
	...
	if(unlockclicked[0])
	{
		...
		printf("%s\n",tmpbuf);
		system(tmpbuf);	
		OK_MSG2(UNLOCK_MSG, ((mode==AP_MODE) ? "<script>dw(wlwps_client)</script>" : "AP"), submitUrl);	
		return;
	}
	/*WPS2DOTX ;check if unlock button be clicked ; brute force attack */
	...
	if (strVal[0]) {
		...
		if(mode==AP_MODE)
		{
			if (!intVal && encrypt != ENCRYPT_WEP) {
				snprintf(cmd, sizeof(cmd), "%s wps_pbc -i %s", HOSTAPD_CLI, WLAN_IF);
				system(cmd);
			}
		}
		else if(mode==CLIENT_MODE)
		{
			if(!intVal)
			{
				snprintf(cmd, sizeof(cmd), "%s wps_pbc -i %s", WPAS_CLI, WLAN_IF);
				system(cmd);
			}
		}
#else
	...
	if (strVal[0]) {
...
#ifndef NO_ACTION		
#ifdef WLAN_WPS_HAPD
			snprintf(ifname, sizeof(ifname), "%s", VXD_IF[wlan_idx]);
			snprintf(tmpbuf, sizeof(cmd), "%s wps_pbc -i %s", WPAS_CLI, ifname);
			system(tmpbuf);
#else
		...
		OK_MSG2(START_PBC_MSG, ((mode==AP_MODE) ? "<script>dw(wlwps_client)</script>" : "AP"), submitUrl);
		return;
	}
#endif //#if defined(UNIVERSAL_REPEATER) && defined(CONFIG_REPEATER_WPS_SUPPORT)
			...
			if (local_pin_changed) {
				...
				if(targetAPSsid[0]){					
					if(strlen(targetAPSsid)<= 32){
						char tmpTranBuff[128] = {0};
						strcpy(tmpTranBuff, targetAPSsid);
						translate_control_code_sprintf_shrink(tmpTranBuff);
						sprintf(tmpbuf, "iwpriv wlan%d set_mib wsc_specssid=\"%s\" ",wlan_idx, tmpTranBuff);
						system(tmpbuf);						
					}else{					
				...
				if(mode==CLIENT_MODE)
				{
					...
					snprintf(cmd, sizeof(cmd), "%s wps_pin any %s -i %s", WPAS_CLI, wsc_pin, WLAN_IF);
					system(cmd);
					snprintf(cmd,sizeof(cmd),"echo %x > /proc/gpio", wps_start_led_num);
					system(cmd);
				}
#else
				/* support  special SSID , 2011-0505 WPS2DOTX */
				...
				system(tmpbuf);
#endif
#endif // #ifdef WLAN_WPS_HAPD
			}			
#endif
		}
		OK_MSG2(START_PIN_MSG, ((mode==AP_MODE) ? "<script>dw(wlwps_client)</script>" : "AP"), submitUrl);
		return;
	}
	...
	if (strVal[0]) {
		...
		if (intVal) {
			...
			if (local_pin_changed) {
				...
				apmib_get(MIB_HW_WSC_PIN, (void *)wsc_pin);
				snprintf(ifname, sizeof(ifname), "%s", VXD_IF[wlan_idx]);
				snprintf(tmpbuf, sizeof(cmd), "%s wps_pin any %s -i %s", WPAS_CLI, wsc_pin, ifname);
				system(tmpbuf);
				snprintf(tmpbuf,sizeof(tmpbuf),"echo %x > /proc/gpio", wps_start_led_num);
				system(tmpbuf);
#else
		...
		if (strVal[0]) {
			...
			if (intVal) {
				...
				if (encrypt != ENCRYPT_WEP) {
					...
					snprintf(cmd, sizeof(cmd), "%s wps_pin any %s -i %s", HOSTAPD_CLI, tmpbuf, WLAN_IF);
					system(cmd);
					snprintf(tmpbuf,sizeof(tmpbuf),"echo %x > /proc/gpio", wps_start_led_num);
					system(tmpbuf);
				}else{
					printf("WPS not support under WEP encrypt.\n");
				}
#else
...
//	printf("MAC %s \n",strVal);
	...
	if(strVal)
	{
		...
		{
			strcpy(tmpBuf,"<script>dw(wl_Can_not_set_key)</script>");
			goto setErr_rekey;
...
//		printf("REKEY_M_POLICY %s \n",strVal);
	...
	if(strVal)
	{
		...
		{
			strcpy(tmpBuf,"<script>dw(wl_Can_not_set_tim)</script>");
			goto setErr_rekey;
		}
//		printf("REKEY_M_TIME %s \n",strVal);
	...
	if(strVal)
	{
		...
		{
			strcpy(tmpBuf,"<script>dw(wl_Can_not_set_pac)</script>");
			goto setErr_rekey;
		}
//		printf("REKEY_M_PACKET %s \n",strVal);
	...
	if(strVal)
	{
		...
		{
			strcpy(tmpBuf,"<script>dw(wl_Can_not_set_pol)</script>");
			goto setErr_rekey;
		}
//		printf("REKEY_POLICY %s \n",strVal);
	...
	if(strVal)
	{
		...
		{
			strcpy(tmpBuf,"<script>dw(wl_Can_not_set_utime)</script>");
			goto setErr_rekey;
		}
//		printf("REKEY_TIME %s \n",strVal);
	}
	...
	if(strVal)
	{
		...
		{
			strcpy(tmpBuf,"<script>dw(wl_Can_not_set_upacket)</script>");
			goto setErr_rekey;
		}
...
void formUploadWapiCert(request *wp, char * path, char * query)	// Only for Local AS
{
	...
	{
			strcpy(tmpBuf, "<script>dw(wl_Set_Sea_MIB)</script>");
			goto upload_ERR;
	}
	if(NULL == strstr(wp->upload_data,CERT_START) || NULL ==strstr(wp->upload_data,CERT_END))
	{
		strcpy(tmpBuf,"<script>dw(wl_Not_a_Cer)</script>");
		goto upload_ERR;
	}
	fp=fopen(TMP_CERT,"w");
	if(NULL == fp)
	{
		strcpy(tmpBuf,"<script>dw(wl_Can_not_ope)</script>");
		goto upload_ERR;
	}
	else
	{
		...
		{
			strcpy(tmpBuf,"<script>dw(wl_Unk_Cer_Fil)</script>");
			goto upload_ERR;
		}
...
//			system("storeWapiFiles -oneUser");
	...
	strcpy(tmpBuf,"<script>dw(wl_Cer_Ins_Suc)</script>");
	OK_MSG1(tmpBuf, submitUrl);
	return;
upload_ERR:
	ERR_MSG(tmpBuf);
...
//			printf("tmpBuf=%s\n",tmpBuf);//Added for test
	...
	if(5 == operation)
	{
		...
		if(strVal)
		{
			...
			{
				strcpy(tmpBuf, ("<script>dw(wl_Set_Sea_Ind)</script>"));
				goto setErr_cert;
			}
		}
//		printf("SELECT1 %s\n",strVal);
		...
		{
			strcpy(tmpBuf, ("<script>dw(wl_Set_Sea_Inf)</script>"));
			goto setErr_cert;
		}
//		printf("CERT_INFO %s\n",strVal);
		/*set search key*/
...
//	printf("tmpbuf :%s\n",tmpbuf);
	...
	system("cp /var/myca/user.cert /var/user.cer");
	sleep(1);
...
void formUpload8021xUserCert(request *wp, char * path, char * query)
{
	...
	if((rsBandSel != PHYBAND_5G) && (rsBandSel != PHYBAND_2G)){
		strcpy(tmpBuf, ("<script>dw(wl_Wro_rsB_!)</script>"));
		goto upload_ERR;
	}
	...
	if(deleteAllCerts[0]=='1')
	{
		...
		if(rsBandSel == PHYBAND_5G){
			...
			strcpy(tmpBuf,"<script>dw(wl_Del_all_802_5)</script>");
		}
		else{
			system("rsCert -rst_2g");
			strcpy(tmpBuf,"<script>dw(wl_Del_all_802_2)</script>");
		}
	}
	else
	{
		...
		if(NULL == strstr(wp->upload_data,RS_CERT_START)|| NULL ==strstr(wp->upload_data,RS_CERT_END))
		{
			...
			strcpy(tmpBuf,"<script>dw(wl_No_802_cer)</script>");
			tryFormChange=1;
		}
		...
		if((tryFormChange==0)&&(!strcmp(strVal,"user")))
		{
			...
			{			
				...
				strcpy(tmpBuf,"<script>dw(wl_No_802_pri)</script>");
				tryFormChange=1;
			}
		}	
		if(!strcmp(strVal,"user"))
		{		
			...
			strcpy(tmpBuf,"<script>dw(wl_802_use_cer)</script>");
		}
		...
		{		
			...
			else
			{
				...
				if(certOk != 1)
				{
					...
					strcpy(tmpBuf,"<script>dw(wl_No_802_cert_inclued)</script>");
					goto upload_ERR;
				}
			...
			strcpy(tmpBuf,"<script>dw(wl_802_roo_cer)</script>");
		}
		else
		{
...
void formCertUpload(request *wp, char * path, char * query)
{
	...
	strDelUser =   req_get_cstream_var(wp, ("deleteSelUser"), "");
		...
		strcpy(tmpBuf, "<script>dw(wl_error_handle)</script>");
		goto  ret_upload;
	}
	...
	if(strSelectCa[0]){ //set ca index
		...
		if ( strVal[0] ) {
			...
			if ( !apmib_set(MIB_ROOT_IDX, (void *)&intVal)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_CA_sel)</script>"));
				goto ret_upload;
			}
			if( intVal <= entryNum){
				pEntry = (void *) &rootEntry ;
		...
		if ( strVal[0] ) {
			...
			if ( !apmib_set(MIB_USER_IDX, (void *)&intVal)) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_Use_sel)</script>"));
				goto ret_upload;
			}
			if( intVal <= entryNum){
				pEntry = (void *) &userEntry ;
		...
		if(wp->post_data_len == 0){
			strcpy(tmpBuf, ("<script>dw(wl_Err_!_Upl)</script>"));
			goto  ret_upload;
		 }
		 ...
		 if ((fp = fopen(fileName, "w+b")) != NULL) {
			...
			sprintf(tmpBuf, ("<script>dw(wl_open_file_error)</script>"));
			goto ret_upload;
		 }
	...
	if ( refresh[0] ) {
		...
		while (1) 
		{
			...
			{ 
				case -2: 
					printf("-2\n"); 
					strcpy(tmpBuf, ("<script>dw(wl_Aut_sca_run)</script>")); 
					goto ss_err; 
					break; 
				case -1: 
					printf("-2\n"); 
					strcpy(tmpBuf, ("<script>dw(wl_Sit_req_fai)</script>")); 
					goto ss_err; 
					break; 
				default: 
					break; 
...
//printf("\r\n status=[%d],__[%s-%u]\r\n",status,__FILE__,__LINE__);
			...
			{	
				if (wait_time++ > 5) 
				{
					strcpy(tmpBuf, ("<script>dw(wl_sca_req_tim)</script>"));
		...
		while (1) {
			...
			if (res == 0xff)    // in progress
			{
				...
				if (wait_time++ > 20) 			
				{
					strcpy(tmpBuf, ("<script>dw(wl_sca_tim_)</script>"));
					if(pStatus)
					{
						free(pStatus);
						pStatus = NULL;
...
int wlP2PScanTbl(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
	"<td align=center width=\"30%%\" ><font size=\"2\"><b><script>dw(wlp2pwscconnect_device_name)</script></b></font></td>\n"
	"<td align=center width=\"10%%\" ><font size=\"2\"><b><script>dw(wlp2pwscconnect_channel)</script></b></font></td>\n"
	"<td align=center width=\"30%%\" ><font size=\"2\"><b><script>dw(wlp2pwscconnect_device_address)</script></b></font></td>\n"
  "<td align=center width=\"15%%\" ><font size=\"2\"><b><script>dw(wlp2pwscconnect_role)</script></b></font></td>\n"));
...
//printf("\r\n pBss->p2prole=[%d],__[%s-%u]\r\n",pBss->p2prole,__FILE__,__LINE__);
	   ...
	   else if(pBss->p2prole == R_P2P_GO)// 1:GO
	   {
		...
		"<td align=center ><font size=\"2\">%s</td>\n"     
    "<td align=center ><font size=\"2\">%s</td>\n"),
		pBss->p2pdevname, pBss->channel, tmpBuf, p2pTypeStr);
		...
		"),
		 i,i,i,pBss->p2pdevname,i,pBss->channel,i,tmpBuf2,i,p2pTypeStr,i,pBss->p2pwscconfig);
...
void formFt(request *wp, char *path, char *query)
{
	...
	if (strVal[0]) {
		...
		if (idx<0 || idx > NUM_VWLAN_INTERFACE) {
			strcpy(tmpbuf, "<script>dw(wl_not_ssid_type)</script>");
			goto setErr_ft;
		}
		vwlan_idx=idx;
	}
	...
	if (strVal[0]) {
		...
		if (!strVal[0]) {
			strcpy(tmpbuf, "<script>dw(wl_Err_No_mac)</script>");
			goto setErr_ft;
		}
		...
		if (!strVal[0]) {
			strcpy(tmpbuf, "<script>dw(wl_no_nas)</script>");
			goto setErr_ft;
		}
		...
		if (!strVal[0]) {
			strcpy(tmpbuf, "<script>dw(wl_err_no_r0r1_key)</script>");
			goto setErr_ft;
		}
        ...
        if (!strVal[0]) {
            strcpy(tmpbuf, "<script>dw(wl_err_no_op_class)</script>");
            goto setErr_ft;
        }
        ...
        if (!strVal[0]) {
            strcpy(tmpbuf, "<script>dw(wl_no_channel)</script>");
            goto setErr_ft;
        }
		...
		for (i=1; i<=entryNum; i++) {
			...
			*((char*)(&khEntryTmp))=(char)i;
			if (!apmib_get(MIB_WLAN_FTKH,(void*)&khEntryTmp)) {
				strcpy(tmpbuf, "<script>dw(wl_get_mbssid_tbl_error)</script>");
				goto setErr_ft;
			}
		
			if (!memcmp(khEntry.macAddr, khEntryTmp.macAddr, 6))
			{
				strcpy(tmpbuf, "<script>dw(wl_entry_exists)</script>");
				goto setErr_ft;
			}
		}
		...
		{
			strcpy(tmpbuf, "<script>dw(wl_err_table_full)</script>");
			goto setErr_ft;
		}
		// add new KH entry
		intVal = apmib_set(MIB_WLAN_FTKH_ADD,(void*)&khEntry);
		if (intVal == 0) {
			strcpy(tmpbuf, "<script>dw(wl_err_add_chain)</script>");
			goto setErr_ft;
		}
		...
		for (i=entryNum; i>0; i--) {
			*((char*)(&khEntryTmp))=(char)i;
			...
			if (!strcmp(strVal, "ON")) {
				...
				{
					strcpy(tmpbuf,"<script>dw(wl_del_chain_record)</script>");
					goto setErr_ft;
				}
			}
		}
		if (deleted <= 0) {
			strcpy(tmpbuf, "<script>dw(wl_no_item_selected)</script>");
			goto setErr_ft;
		}
