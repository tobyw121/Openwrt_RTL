...
int get_ax_flag(char *wlan_if, unsigned int *is_ax_support)
{
	unsigned int intVal=0;
	char filename_ax[64] = {0};
	DIR *dir=NULL;
	...
	if(pMib){
		...
		snprintf(filename_ax, sizeof(filename_ax), DIR_WIFI6_DRV, wlan_if);
		dir = opendir(filename_ax);
		if(dir){
			intVal = 1;
			closedir(dir);
		}
	}
	*is_ax_support = intVal;
	return 0;
}
...
{
	FILE *fp;
	char *p=NULL, *q=buffer;
	int headlen, tokenlen, datalen;
	...
	headlen = p - q+4;
	p = p+4;
	datalen = len - (headlen + 2*tokenlen + 80); 
...
int getWlStaNum( char *interface, int *num )
{
	...
	get_ax_flag(interface, &is_ax_support);
	if(is_ax_support){
...
void set_wlan_sta_info(WLAN_STA_INFO_Tp pInfo, sta_info_2_web *pstaInfo)
{
	sta_info_2_web *tmp_pstaInfo;
	WLAN_STA_INFO_Tp tmp_pInfo;
	int i, j=1;

	for (i=1; i<=MAX_STA_NUM; i++) {
		tmp_pstaInfo = (sta_info_2_web *)&pstaInfo[i];
		if(tmp_pstaInfo->aid && (tmp_pstaInfo->flags & STA_INFO_FLAG_ASOC))
		{
			tmp_pInfo = (WLAN_STA_INFO_Tp)&pInfo[j];
			tmp_pInfo->aid = tmp_pstaInfo->aid;
			memcpy(tmp_pInfo->addr, tmp_pstaInfo->addr, 6);
			tmp_pInfo->tx_packets = tmp_pstaInfo->tx_packets;
			tmp_pInfo->rx_packets = tmp_pstaInfo->rx_packets;
			tmp_pInfo->expired_time = tmp_pstaInfo->expired_time;
			tmp_pInfo->flags = tmp_pstaInfo->flags;
			tmp_pInfo->TxOperaRate = tmp_pstaInfo->TxOperaRate;
			tmp_pInfo->rssi = tmp_pstaInfo->rssi;
			tmp_pInfo->link_time = tmp_pstaInfo->link_time;
			tmp_pInfo->tx_fail = tmp_pstaInfo->tx_packets;
			tmp_pInfo->tx_bytes = tmp_pstaInfo->tx_bytes;
			tmp_pInfo->rx_bytes = tmp_pstaInfo->rx_bytes;
			tmp_pInfo->network = tmp_pstaInfo->network;
			tmp_pInfo->ht_info = tmp_pstaInfo->ht_info;
			tmp_pInfo->RxOperaRate = tmp_pstaInfo->RxOperaRate;
			tmp_pInfo->acTxOperaRate = tmp_pstaInfo->acTxOperaRate;
#ifdef RTK_MULTI_AP
			tmp_pInfo->multiap_profile = tmp_pstaInfo->multiap_profile;
#endif
			j++;
		}
	}
...
int getWlStaInfo( char *interface,  WLAN_STA_INFO_Tp pInfo )
{
	...
	{
#ifdef WLAN_DRVCOMMON_HEADER
		char tmp_buffer[sizeof(sta_info_2_web) * (MAX_STA_NUM+1)];
		memset(tmp_buffer, 0, sizeof(tmp_buffer));
		wrq.u.data.pointer = tmp_buffer;
		wrq.u.data.length = sizeof(tmp_buffer);
#else
		...
		close( skfd );
#ifdef WLAN_DRVCOMMON_HEADER
		set_wlan_sta_info(pInfo, (sta_info_2_web *)tmp_buffer);
#endif
	}
	else
	{
...
void set_bssdb_info(SS_STATUS_Tp pStatus, bss_desc_2_web *ss_result, int ss_number)
{
	...
	for(i=pStatus->number, j=0; j<ss_number; i++, j++)
	{
		...
		pStatus->bssdb[i].bss_akm_suite = 0;
		...
		else if(ss_result[j].encrypt == ENCRYP_PROTOCOL_RSN)
		{
			...
			if(ss_result[j].akm & 2)
                        	pStatus->bssdb[i].bss_akm_suite |= 0x0004;

			if(ss_result[j].akm & 128)//WPA3
			{
				pStatus->bssdb[i].t_stamp[1] |= 0x00000800;
				pStatus->bssdb[i].bss_akm_suite |= 0x00000100;
			}
			...
			if(ss_result[j].mfp_opt == 3)
				pStatus->bssdb[i].t_stamp[1] |= 0x00000400;
			else if(ss_result[j].mfp_opt == 2)
				pStatus->bssdb[i].t_stamp[1] |= 0x00000200;
...
{
	if (((BssDscr *)entry1)->rssi > ((BssDscr *)entry2)->rssi)
		return -1;
...
int getWlSiteSurveyResult(char *interface, SS_STATUS_Tp pStatus )
{
	...
	get_ax_flag(interface, &is_ax_support);
	if(is_ax_support){
		char buffer[sizeof(sheet_hdr_2_web)+ (MAX_BSS_DESC_AX*sizeof(BssDscr_ax))];
		...
		if(pStatus->number == 1){
			...
			sheet_sequence++;
			//printf("(%s-%d) %d %d %d %d %d\n",__func__, __LINE__,ss_hdr->sheet_total, ss_hdr->sheet_sequence, wrq.u.data.length, ss_number,sizeof(BssDscr_ax));
			while(sheet_sequence < sheet_total){
				...
				ss_number = (wrq.u.data.length-sizeof(sheet_hdr_2_web))/sizeof(BssDscr_ax);
				//printf("(%s-%d) %d %d %d %d %d\n",__func__,__LINE__, ss_hdr->sheet_total, ss_hdr->sheet_sequence, wrq.u.data.length, ss_number ,sizeof(BssDscr_ax));
				if((pStatus->number+ss_number) > MAX_BSS_DESC_AX){
			...
			// sort by rssi
			if(pStatus->number > 1)
				qsort(pStatus->bssdb, pStatus->number, sizeof(BssDscr), compareBSS);
		}
	}else
#endif
	{
...
int getMultiWanInfo(char* strWanIfName,char* strWanIP,char* strWanMask,
char*strWanDefGW,char*strWanHWAddr)
{
	struct in_addr	intaddr;
	struct sockaddr hwaddr;
	unsigned char *pMacAddr;

	if(!strWanIfName) return -1;
	if (getInAddr(strWanIfName, IP_ADDR, (void *)&intaddr ) )
			sprintf(strWanIP,"%s",inet_ntoa(intaddr));
	else
		sprintf(strWanIP,"%s","0.0.0.0");

	if ( getInAddr(strWanIfName, SUBNET_MASK, (void *)&intaddr ) )
		sprintf(strWanMask,"%s",inet_ntoa(intaddr));
	else
		sprintf(strWanMask,"%s","0.0.0.0");
	
	if ( getDefaultRoute(strWanIfName, &intaddr) )
		sprintf(strWanDefGW,"%s",inet_ntoa(intaddr));
	else
		sprintf(strWanDefGW,"%s","0.0.0.0");	

	if ( getInAddr(strWanIfName, HW_ADDR, (void *)&hwaddr ) ) 
	{
		pMacAddr = (unsigned char *)hwaddr.sa_data;
		sprintf(strWanHWAddr,"%02x:%02x:%02x:%02x:%02x:%02x",pMacAddr[0], pMacAddr[1],pMacAddr[2], pMacAddr[3], pMacAddr[4], pMacAddr[5]);
	}
	else
		sprintf(strWanHWAddr,"%s","00:00:00:00:00:00");
	return 0;
}
#endif
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8196C) || defined(CONFIG_RTL_8198) || defined(CONFIG_RTL_819XD) || defined(CONFIG_RTL_8196E)
...
void kill_processes(void)
{
	...
	system("killall WTP AC AACWTP"); // free capwap for more cpu and memory to speed up reboot
...
#elif defined(WIFI_SIMPLE_CONFIG)
...
int rtk_wlan_get_ifname_idx(char *ifname, int *wlan_index, int *vwlan_index)
{
	int idx=-1, vidx=-1;

	if(ifname == NULL || wlan_index == NULL || vwlan_index == NULL)
	{
	...
	{
		if(sscanf(ifname, WLAN_FORMAT_S, &idx) != 1)
		{
...
#ifdef MBSSID
		if(strstr(ifname, VAP_IF_SUFFIX))
		{
			if(sscanf(ifname, VAP_IF, &idx, &vidx) != 2)
			{
				printf("unkown ifname %s\n", ifname);
				return -1;
			}

			if(vidx < 0 || vidx >= NUM_VWLAN)
			{
...
#endif
#ifdef UNIVERSAL_REPEATER
		if(strstr(ifname, VXD_IF_SUFFIX))
		{
			if(sscanf(ifname, WLAN_VXD_FORMAT_S, &idx) != 1)
			{
				printf("unkown ifname %s\n", ifname);
				return -1;
			}
	...
	printf("ifname %s wlan_index=%d vwlan_index=%d\n", ifname, *wlan_index, *vwlan_index);
	return 0;
}
...
unsigned int findWANIFACE_MaxConDevInstNum(void)
{
	unsigned int ret=0, i,num;
	WANIFACE_T *p,wan_entity;
	apmib_get(MIB_WANIFACE_TBL_NUM, (void *)&num);
	for( i=1; i<=num;i++ )
	{
		p = &wan_entity;
		*((char *)&wan_entity) = (char)i;
		if(!apmib_get(MIB_WANIFACE_TBL, (void *)&wan_entity))
			continue;
...
unsigned int findWANMaxPPPConInstNum(unsigned int ConnDevId)
{
	unsigned int ret=0, i,num;
	WANIFACE_T *p,wan_entity;
...
unsigned int findWANMaxIPConInstNum(unsigned int ConnDevId)
{
	unsigned int ret=0, i,num;
	WANIFACE_T *p,wan_entity;
	...
	for( i=1; i<=num;i++ )
	{
		...
		*((char *)&wan_entity) = (char)i;
		...
		if( p->ConDevInstNum==ConnDevId )
		{
			if( (p->cmode == IP_ROUTE) ||(p->cmode == IP_BRIDGE))
			{
...
int getMultiWanApplicationType(unsigned int ServiceList,char* bufferServerType,char lenth)
{
	if(lenth<sizeof("INTERNET"))
		return -1;
	switch(ServiceList)
	{
		case 0:
			strcpy(bufferServerType,"NONE");
			break;
		case X_CT_SRV_TR069:
			strcpy(bufferServerType,"TR069");
			break;
		case X_CT_SRV_INTERNET:
