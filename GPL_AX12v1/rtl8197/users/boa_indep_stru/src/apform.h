...
#define ERR_FOR_VAP(msg) { \
	update_form_hander_name(wp); \
   	req_format_write(wp, "<html><head>"); \
   	getIncludeCss(wp);\
   	req_format_write(wp,"<script>alert(%s)</script></html>",msg);\
}
...
#define ERR_MSG(msg) { \
    ...
    req_format_write(wp, "<form name=\"setok\"><input type=\"button\" name=\"ok_msg\"onclick=\"history.go (-1)\" value=\"&nbsp;&nbsp;OK&nbsp;&nbsp\" name=\"OK\"><script>document.setok.ok_msg.value = okmsg_btn;</script></form></blockquote></body></html>"); \
}
...
#define RET_SURVEY_PAGE(pMsg, url, connectOK, wlan_id, isWizard) { \
	...
	req_format_write(wp, "<input type='hidden' value='%s' name='submit-url'>",url); \
	if(connectOK) req_format_write(wp, "<td><font size=2><b><input type=\"checkbox\" name=wizardAddProfile%d value=\"ON\">&nbsp;&nbsp;<script>dw(wlsurvey_connect_addto_profile)</script></b></td><br><br>", wlan_id); \
	if(!connectOK) req_format_write(wp, "<input id='restartLater' name='restartLater' type='submit' value='  OK  ' onclick=\"return true\" />&nbsp;&nbsp;"); \	
		if(!connectOK) req_format_write(wp,"<script>document.rebootSiteSurveyProfileForm.restartLater.value = wlsurvey_connect_ok;</script>"); \
	if(connectOK) req_format_write(wp, "<input id='restartLater' name='restartLater' type='submit' value='Save' OnClick=window.location.replace(\"%s\");return true>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", url); \
		if(connectOK) req_format_write(wp,"<script>document.rebootSiteSurveyProfileForm.restartLater.value = wlsurvey_connect_save;</script>"); \
	if(connectOK) req_format_write(wp, "<input id='restartNow' name='restartNow' type='submit' value='Save & Apply' onclick=\"return true\" />&nbsp;&nbsp;"); \
		if(connectOK) req_format_write(wp,"<script>document.rebootSiteSurveyProfileForm.restartNow.value = wlsurvey_connect_save_apply;</script>"); \
	req_format_write(wp, "</form></blockquote></body></html>");\
	log_boaform("formSiteSurveyProfile",wp);\
}

#ifdef IS_AX_SUPPORT
#define RET_SURVEY_PAGE_COMMON(pMsg, url, connectOK, wlan_id, isWizard) { \
	extern void log_boaform(char *form,request *);\
	needReboot = 1; \
	if(strlen(url) == 0) \
		strcpy(url,"/wizard.htm"); \
	...
	req_format_write(wp, "</head><body><blockquote><h4>%s</h4>", pMsg); \
	if(isWizard) req_format_write(wp, "Your changes have been saved. The router must be rebooted for the changes to take effect.<br> You can reboot now, or you can continue to make other changes and reboot later.\n"); \
	req_format_write(wp, "<input type='submit' value='OK' OnClick=window.location.replace(\"%s\");return true>", url); \
	req_format_write(wp, "</form></blockquote></body></html>");\
}
#endif
...
#define RET_SURVEY_PAGE(pMsg, url, connectOK, wlan_id, isWizard) { \
	...
	if(connectOK) req_format_write(wp, "<input id='restartNow' name='restartNow' type='submit' value='Reboot Now' onclick=\"return true\" />&nbsp;&nbsp;"); \
		if(connectOK) req_format_write(wp,"<script>document.rebootSiteSurveyProfileForm.restartNow.value = wlsurvey_reboot_now;</script>"); \
	if(connectOK) req_format_write(wp, "<input id='restartLater' name='restartLater' type='submit' value='Reboot Later' OnClick=window.location.replace(\"%s\");return true>", url); \
		if(connectOK) req_format_write(wp,"<script>document.rebootSiteSurveyProfileForm.restartLater.value = wlsurvey_reboot_later;</script>"); \
	req_format_write(wp, "</form></blockquote></body></html>");\
}
#define SUCCESS_INFO(info,url){\
	req_format_write(wp, "<html><head>"); \
...
#define OK_MSG1(msg, url) { \
	...
	getIncludeCss(wp);\
   	req_format_write(wp, "</head><body><blockquote><h4>%s</h4>\n", msg); \
	if (url) req_format_write(wp, "<form name=\"okmsg\"><input name=\"okname\" type=button value=\"  OK  \" OnClick=window.location.replace(\"%s\")><script>document.okmsg.okname.value = okmsg_btn;</script></form></blockquote></body></html>", url);\
	else req_format_write(wp, "<form name=\"okmsg\"><input name=\"okname\" type=button value=\"  OK  \" OnClick=window.close()><script>document.okmsg.okname.value = okmsg_btn;</script></form></blockquote></body></html>");\
}
...
#define OK_MSG_FW(msg, url, c, ip) { \
	...
	req_format_write(wp, "<P align=left><h4><script>dw(wait)</script> <B><SPAN id=show_sec></SPAN></B>&nbsp;<script>dw(seconds)</script> ...</h4></P>");\
...
#define START_PBC_MSG \
	"<script>dw(wlwps_start_pbc_successfully)</script>" \
	"<script>dw(wlwps_in)</script> %s <script>dw(wlwps_run_wps)</script>"
#define START_PIN_MSG \
	"<script>dw(wlwps_start_pin)</script>" \
	"<script>dw(wlwps_in)</script> %s <script>dw(wlwps_run_wps)</script>"
#define SET_PIN_MSG \
	"<script>dw(wlwps_apply_wps_pin_successfully)</script>" \
	"<script>dw(wlwps_runwps_within_2minutes)</script>"
#define STOP_MSG \
	"<script>dw(wlwps_apply_wps_stop)</script>"	
/*for WPS2DOTX brute force attack , unlock*/
#define UNLOCK_MSG \
	"<script>dw(wlwps_apply_wps_unlock)</script>"	
#endif
...
#if defined WLAN_EASY_CONFIG || defined WIFI_SIMPLE_CONFIG || defined RTK_MESH_CONFIG
...
extern void formHttpRedirect(request *wp, char * path, char * query);
#endif
...
extern void formquaggaroute(request * wp, char *path, char *query);
extern void formquaggadebug(request *wp, char *path, char *query);
extern int ospfareaList(request *wp, int argc, char **argv);
extern int ospflogList(request *wp, int argc, char **argv);

extern int ospfinterfaceList(request *wp, int argc, char **argv);
extern int ospfnetworkList(request *wp, int argc, char **argv);


extern void formbgpdebug(request * wp, char *path, char *query);
extern void formbgproute(request * wp, char *path, char *query);
extern int bgpnetList(request *wp, int argc, char **argv);
extern int bgppeerList(request *wp, int argc, char **argv);
extern int bgplogList(request *wp, int argc, char **argv);
extern int isislogList(request *wp, int argc, char **argv);
extern void formisisdebug(request * wp, char *path, char *query);
extern void formisisroute(request * wp, char *path, char *query);
extern int netAuthenticationList(request *wp, int argc, char **argv);
extern int isisinterfaceList(request *wp, int argc, char **argv);
#endif 
#ifdef CONFIG_APP_MINIDLNA
extern void formMiniDlna(request *wp, char *path, char *query);
#endif

#ifdef CONFIG_APP_SAMBA
extern void formSamba(request *wp, char *path, char *query);
#endif
...
extern int showChannelScanResults(request *wp, int argc, char **argv);
#endif
