...
static int getOneDhcpClient(char **ppStart, unsigned long *size, char *ip, char *mac, char *liveTime)
{
	...
	*size = *size - sizeof(entry);
	...
	if(entry.expires == 0xffffffff)
        	sprintf(liveTime,"%s", "<script>dw(dhcp_always)</script>");
        else
...
#ifdef CONFIG_IPV6
		// v6 setting should based on v4 setting
		int v6_origin=0;
		int v6_linkType=0;

		if (DHCP_DISABLED==dhcp){   //v4 set static, v6 set to static + IPoe
				v6_origin=IPV6_ORIGIN_STATIC;		
				v6_linkType=IPV6_LINKTYPE_IP;
		}
		else if (DHCP_CLIENT==dhcp){   // v4 set dhcp, v6 set auto+dhcp
				v6_origin=IPV6_ORIGIN_DHCP;		
				v6_linkType=IPV6_LINKTYPE_IP;
		}
		else if (PPPOE==dhcp) {  // v4 set pppoe, v6 set auto+PPPoe
				v6_origin=IPV6_ORIGIN_DHCP;		
				v6_linkType=IPV6_LINKTYPE_PPP;
		}

		if ( dhcp == PPPOE || dhcp == DHCP_DISABLED || dhcp == DHCP_CLIENT ) {
			apmib_set(MIB_IPV6_ORIGIN_TYPE,&v6_origin);		
			apmib_set(MIB_IPV6_LINK_TYPE,&v6_linkType);
		}
#endif
...
int dhcpRsvdIp_List(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"25%%\" ><font size=\"2\"><b><script>dw(tcpip_dhcp_ip)</script></b></font></td>\n"
#ifdef SUPPORT_DHCP_PORT_IP_BIND
      	"<td align=center width=\"45%%\" ><font size=\"2\"><b><script>dw(tcpip_dhcp_macport)</script></b></font></td>\n"
#else	
...
err:
	if (element == 0) {
		nBytesSent += req_format_write(wp,
			("<tr class=\"tbl_body\"><td><font size=2><script>dw(dhcp_none)</script></td><td><font size=2>----</td><td><font size=2>----</td></tr>"));
	}
	if (buf)
		free(buf);
...
void formMultiWanListTcpip(request *wp, char *path, char *query)
{
	...
	if (intVal){
		strVal = req_get_cstream_var(wp, ("webAccessPort"), "");
		if (!string_to_dec(strVal, &WebPort) || WebPort<1 || WebPort>65535) {
			strcpy(tmpBuf, ("<script>dw(tcpip_wanlist_err1)</script>"));
			goto setErr_tcpip;
		}
		if ( !apmib_set(MIB_WEB_WAN_ACCESS_PORT, (void *)&WebPort)) {
			strcpy(tmpBuf, ("Set WEB_WAN_ACCESS_ENABLED error!"));
	...
	for(i=1;i<=WANIFACE_NUM;i++)
	{
		...
		{//current vid conflict with wan[i]
			...
			{//current vlan enable
				...
				{//wan[i] enable
					sprintf(erroMsgBuf, "<script>dw(tcpip_vid_conflict)</script> %d !",i);
					return -1;
				}else
