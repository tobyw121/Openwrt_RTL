...
#define DEFAULT_ETH_WAN_PORT_MASK 0x0008
#else
...
static int rtl_is_8367d_type(void);
...
int getVlanInfo(request *wp, int argc, char **argv)
{
	...
	else if(!strcmp(argv[0], "is8367d"))
	{
		intVal = rtl_is_8367d_type();
...
int getVlanTable(request *wp, int argc, char **argv)
{
	...
	VLAN_CONFIG_T entry;
	char strBuf[200], strBuf1[50], errBuf[50];
		...
		if(opmode == GATEWAY_MODE)
		{
			if(entry.VlanType == VLAN_TYPE_NAT)
			{
				if(entry.VlanId==hw_nat_lan_vid)
					strcpy(strBuf1, "vlan_8021q_nathw");
				else
					strcpy(strBuf1, "vlan_8021q_nat");
			}
			else if(entry.VlanType == VLAN_TYPE_BRIDGE)
				strcpy(strBuf1, "vlan_8021q_br");
			else
				strcpy(strBuf1, "vlan_8021q_unknown");
		}
	...
	if(strTmp[0])
	{
		...
		switch(strTmp[0])
		{
			...
			case '1':
				...
				break;
			default:
				strcpy(errBuf, "<script>dw(vlan_8021q_error1)</script>");
				goto setErr;
		}
		...
		{
			strcpy(errBuf, "<script>dw(vlan_8021q_error2)</script>");
			goto setErr;
		}	
		if(vlan_id== PASSTHRU_VLAN_ID || vlan_id==DEFAULT_NAT_LAN_VID || vlan_id==DEFAULT_NAT_WAN_VID)
		{
			sprintf(errBuf, "<script>dw(vlan_8021q_vid2)</script> %d , %d <script>dw(vlan_8021q_and)</script> %d <script>dw(vlan_8021q_vid_reserve)</script>",
				PASSTHRU_VLAN_ID, DEFAULT_NAT_LAN_VID, DEFAULT_NAT_WAN_VID);
			goto setErr;
		}		
		...
		{
			strcpy(errBuf, "<script>dw(vlan_8021q_error3)</script>");
			goto setErr;
		}
		...
		{
			sprintf(errBuf, "<script>dw(vlan_8021q_max_table)</script> %d!", MAX_VLAN_CONFIG_NUM); 
			goto setErr;	
		}
		...
		if(! port_member_mask)
		{
			strcpy(errBuf, "<script>dw(vlan_8021q_error4)</script>"); 
			goto setErr;	
		}
		if(opmode == GATEWAY_MODE)
		{
				...
				new_entry.VlanType == VLAN_TYPE_BRIDGE)
			{
				...
				{
					strcpy(errBuf, "<script>dw(vlan_8021q_error5)</script>"); 
					goto setErr;	
				}
			}
			#else
			if(new_entry.VlanType == VLAN_TYPE_NAT)
			{
				if((port_member_mask & wan_mask)
					&& (port_member_mask & ~wan_mask))
				{
					strcpy(errBuf, "<script>dw(vlan_8021q_error6)</script>"); 
					goto setErr;	
				}
			}
			...
			if(new_entry.VlanType == VLAN_TYPE_BRIDGE)
			{
				if(!(port_member_mask & wan_mask) ||
					!(port_member_mask & ~wan_mask))
				{
					strcpy(errBuf, "<script>dw(vlan_8021q_error12)</script>"); 
					goto setErr;	
				}
			}
			#endif
				...
				new_entry.VlanType == VLAN_TYPE_NAT)
			{
				...
				{
					strcpy(errBuf, "<script>dw(vlan_8021q_error13)</script>"); 
					goto setErr;	
				}	
			}
			#endif
		...
		//Repeated VID check/repeated NAT wan check/repeated untagged wan group check.检查重复的vid/nat wan/untagged wan组
		...
		for(idx=1; idx<=entry_num; idx++)
		{
			*(char *)&entry = (char)idx;
			...
			{
				strcpy(errBuf, "<script>dw(vlan_8021q_error14)</script>"); 
				goto setErr;	
			}
			...
			if(opmode==GATEWAY_MODE && 
				...
				new_entry.VlanType == VLAN_TYPE_NAT)
			{
				strcpy(errBuf, "<script>dw(vlan_8021q_error15)</script>"); 
				goto setErr;
			}
			#else
			if(opmode==GATEWAY_MODE && 
			...
			{
				strcpy(errBuf, "<script>dw(vlan_8021q_error16)</script>"); 
				goto setErr;
			}
			#endif
			...
			if(opmode == GATEWAY_MODE)
			{
				...
				{
					strcpy(errBuf, "<script>dw(vlan_8021q_error7)</script>"); 
					goto setErr;
				}
			}
		}

		//Duplicated LAN/WLAN port check.(A LAN/WLAN port can only be included in one group.)一个lan/wlan port只能被放在一个vlan group中
		if(opmode == GATEWAY_MODE &&
			(new_entry.MemberPortMask & ~wan_mask))
		{
			for(port_idx=0; port_idx<port_num; port_idx++)
			{
				...
				for(idx=1; idx<=entry_num; idx++)
				{
					*(char *)&entry = (char)idx;
					...
					strcpy(errBuf, "<script>dw(vlan_8021q_error8)</script>"); 
					goto setErr;
				}
			}
		}
		...
		if(opmode == GATEWAY_MODE)
		{
			...
			if(hw_nat_flag)
			{
				if(new_entry.VlanType==VLAN_TYPE_BRIDGE)
				{
					strcpy(errBuf, "<script>dw(vlan_8021q_error9)</script>");
					goto setErr;
				}

				if(hw_nat_lan_vid>0 && hw_nat_lan_vid!=DEFAULT_NAT_LAN_VID)
				{
					strcpy(errBuf, "<script>dw(vlan_8021q_error10)</script>");
					goto setErr;
				}
	...
	if(strTmp[0])
	{
		...
		for(idx=1; idx<=port_num; idx++)
		{
			...
			if(strTmp[0])
			{
				...
				if(vlan_id<0 || vlan_id>4095)
				{
					strcpy(errBuf, "<script>dw(vlan_8021q_error11)</script>");
					goto setErr;
				}
...
static int rtl_is_8367d_type(void)
{
	...
	fp=fopen("/proc/rtl_8367_chip_type","r");	
	if(!fp){
		printf("open /proc/rtl_8367_chip_type fail!");
		return 0;
	}
	while(fgets(buff,sizeof(buff),fp)){
		if(sscanf(buff,"switchChip = %s\n",&chip_name)==1){			
			if(strcmp(chip_name,"8367D")==0){	
				ret = 1;
				fclose(fp);
				return ret;
			}
	...
	{	
		...
		for(vlan_idx=1; vlan_idx<=entry_num; vlan_idx++)
		{
			*(char *)&entry = (char)vlan_idx;
			...
			//Untagged bridge-wan.           先根据untagged bridge组的vid设wan的pvid
		...
		if(default_vid_find == 0)
		{
			for(vlan_idx=1; vlan_idx<=entry_num; vlan_idx++)
			{
				*(char *)&entry = (char)vlan_idx;
				...
				//Untagged nat group.        如果没有untagged bridge组，则根据untagged nat的vid设wan的pvid
				if(entry.VlanType == VLAN_TYPE_NAT &&
					(entry.MemberPortMask & wan_mask) &&
					!(entry.TaggedPortMask & wan_mask))
				{
		...
		if(default_vid_find)
			...
			*((short *)pVidArray + wan_idx) = DEFAULT_NAT_WAN_VID;       //既没有untagged Bridge也没有untagged nat，则wan的pvid设为8
	}
