		...
		switch(wan_type)
		{
			case	PPPOE:
					if(isWanPhy_Link < 0 || (0==isOwnIp))					
						strcpy(buffer,"fmget_pppoe_disconnect");
					else
						strcpy(buffer,"fmget_pppoe_connet");
					break;
			case	DHCP_DISABLED:
					if(isWanPhy_Link < 0 || (0==isOwnIp))					
						strcpy(buffer,"fmget_ip_disconnect");
					else
						strcpy(buffer,"fmget_ip_connect");
					break;
			case	DHCP_CLIENT:
					if(isWanPhy_Link < 0 || (0==isOwnIp))					
						strcpy(buffer,"fmget_ip_dhcp");
					else
						strcpy(buffer,"fmget_dhcp");				
					break;
			default:
					break;
		}
	...
	else if(strcmp(name, "static_dhcp_entry_num")==0)
	{
		...
		if (dhcp==DHCP_CLIENT) {
			if (!isDhcpClientExist(BRIDGE_IF) &&
				...
				return req_format_write(wp, "<script>dw(fmget_ip_dhcp)</script>");
			if (isDhcpClientExist(BRIDGE_IF))
				return req_format_write(wp, "DHCP");
		}
		return req_format_write(wp, "<script>dw(fmget_ip)</script>");
	}
		...
		LANGUAGE_TYPE_T langType=0;
		apmib_get(MIB_WEB_LANGUAGE,(void*)&langType);
		if(langType==ENGLISH)
			return req_format_write(wp, "language_en.js");
		else if(langType==SIMPLE_CHINESE)
			return req_format_write(wp, "language_sc.js");
#endif
		return req_format_write(wp, "language_en.js");
	}
	else if ( !strcmp(name, "support-winnuke-icmp-redirection")) {
#ifdef SUPPORT_WINNUKE_ICMP_REDIRECTION
			return req_format_write(wp, "1");
#else
			return req_format_write(wp, "0");
#endif
	}
	else if ( !strcmp(name, "ipv6_enable")) {
#ifdef CONFIG_IPV6
			return req_format_write(wp, "1");
#else
			return req_format_write(wp, "0");
...
//	for ap package of new ui, start
	...
	else if (!strcmp(name, "show_mgt_bluetooth_repeater")) {
...
// for ap package of new ui , end
	...
	else if(!strcmp(name, "quaggaroute_menu_onoff"))
{
#ifdef CONFIG_APP_RTL_QUAGGA 

	 req_format_write(wp, "menu.addItem(\"Dynamic routing\");");
	 req_format_write(wp, "dynamic_routing = new MTMenu();");
 #ifdef CONFIG_APP_RTL_QUAGGA
	 req_format_write(wp, "dynamic_routing.addItem(\"Ospf Setup\", \"quaggaroute.htm\", \"\", \"Ospf Setup\");");
 	req_format_write(wp, "dynamic_routing.addItem(\"Bgp Setup\", \"bgproute.htm\", \"\", \"Bgp Setup\");");
	 req_format_write(wp, "dynamic_routing.addItem(\"ISIS Setup\", \"isisroute.htm\", \"\", \"ISIS Setup\");");
 #endif
return req_format_write(wp, "menu.makeLastSubmenu(dynamic_routing);");
	...
	else if(!strcmp(name,"ddns_menu"))
	{
		...
		if(opmode == BRIDGE_MODE){
			return req_format_write(wp, "status_disconn");
		}
		...
		if ( dhcp == DHCP_CLIENT) {
			...
			//printf("\n\n\n wanDhcp-current: iface[%s] \n\n\n", iface);
		 	...
		 	if (!isDhcpClientExist(iface))
				return req_format_write(wp, "fmget_ip_dhcp");
			else{
				if(isWanPhy_Link < 0)
					return req_format_write(wp, "fmget_ip_dhcp");
				else
					return req_format_write(wp, "fmget_dhcp");
			}
			...
			if (opmode == WISP_MODE)
			{
				...
				if (bss.state == STATE_CONNECTED){
					return req_format_write(wp, "fmget_ip_connect");
				}
				else
				{
					return req_format_write(wp, "fmget_ip_disconnect");
				}				
			}
			else
			{
			...
			if (pppoeWithDhcpEnabled) {
				...
				if ( isConnectPPP()){
#ifdef MULTI_PPPOE
					req_format_write(wp, "fmget_pppoe_connet");
#endif
					if(isWanPhy_Link < 0)
						req_format_write(wp, "fmget_pppoe_disconnect");
					else
						req_format_write(wp, "fmget_pppoe_connet");
				}else
					req_format_write(wp, "fmget_pppoe_disconnect");
			...
			if ( isConnectPPP()){
				if(isWanPhy_Link < 0)
					return req_format_write(wp, "fmget_pptp_disconnect");
				else
					return req_format_write(wp, "fmget_pptp_connect");
			}else
				return req_format_write(wp, "fmget_pptp_disconnect");
		}
			...
			if ( isConnectPPP()){
				if(isWanPhy_Link < 0)
					return req_format_write(wp, "fmget_l2tp_disconnect");
				else
					return req_format_write(wp, "fmget_l2tp_connect");
			}else
				return req_format_write(wp, "fmget_l2tp_disconnect");
		}
#ifdef RTK_USB3G
	...
	else if(!strcmp(name, "vlan_wan_wifi_vap3_pri"))
	{
	  ...
	  sprintf(buffer, "%d", intVal );
		...
		if(bss.state == STATE_CONNECTED) {
			...
			if ( !apmib_get( intVal, (void *)buffer))
				return -1;
		}
		translate_control_code(buffer);
		return req_format_write(wp, "%s", buffer);
...
/*+++++added by Jack for TR-069 configuration+++++*/
	...
	else if(!strcmp(name, "quagga")) {
#ifdef CONFIG_APP_RTL_QUAGGA
		return req_format_write(wp,
				"document.write('"\
				"<tr><td><b>Dynamic routing</b></td></tr>"\
				"<tr><td><a href="quaggaroute.htm" target="view">Ospf setup</a></td></tr>"\
				"')");
#else
		return req_format_write(wp,"");
#endif
...
#if defined(TR069_ANNEX_G)
	else if(!strcmp(name, "tr069-stunStatus-0")) {
		if ( !apmib_get( MIB_CWMP_STUN_EN, (void *)&intVal) )
			return -1;
		if(intVal==0){
			return req_format_write(wp,"checked");
		}else{
	...
	else if(!strcmp(name, "tr069-stunStatus-1")) {
		if ( !apmib_get( MIB_CWMP_STUN_EN, (void *)&intVal) )
			return -1;
		if(intVal==1){
			return req_format_write(wp,"checked");
	...
	else if(!strcmp(name, "stun_url")) {
		if ( !apmib_get( MIB_CWMP_STUN_SERVER_ADDR, (void *)buffer) )
			return -1;
		return req_format_write(wp, buffer);
	}
	else if(!strcmp(name, "stun_port")) {
		if ( !apmib_get( MIB_CWMP_STUN_SERVER_PORT, (void *)&intVal) )
			return -1;
		sprintf(buffer, "%d", intVal );
		return req_format_write(wp, buffer);
	} 
...
/*-----end-----*/
	...
	else if(!strcmp(name, "bluetooth_repeater_config_menu")) {
#ifdef CONFIG_APP_BT_REPEATER_CONFIG
		return req_format_write(wp, "manage.addItem('Bluetooth Repeater', 'bluetoothRepeaterConfig.htm', '', 'Setup Bluetooth Repeater configuration      ');" );
#else
		return 0;
#endif
	}
...
//#if defined(CONFIG_RTL_8198) || defined(CONFIG_POCKET_ROUTER_SUPPORT)
#if 1 //defined(CONFIG_RTL_8198C) ||defined(CONFIG_RTL_8198) || defined(CONFIG_POCKET_ROUTER_SUPPORT) || defined(CONFIG_RTL_8196C) || defined(CONFIG_RTL_819XD) || defined(CONFIG_RTL_8196E) 
	...
	else if(!strcmp(argv[0],"isWPA3Support"))
	{
	...
	else if(!strcmp(argv[0],"isWapiLocalAsSupport"))
	{
#if defined(CONFIG_RTL_WAPI_LOCAL_AS_SUPPORT)
		sprintf(buffer, "%s", "1" );
#else
		sprintf(buffer, "%s", "0");
#endif
...
//		req_format_write(wp,"<option value=\"2\"> Use Cert from Local AS </option>");		
	...
	else if(!strcmp(argv[0],"wapiLocalAsCertsUploadForm"))
	{
		...
		req_format_write(wp,"<tr><td width=\"0.55\"><font size=2><b><script>dw(wlwapiinstalcert_certificate_local)</script></b></font></td>");
		req_format_write(wp,"<td width=\"0.45\"><font size=2> <input name=\"cert_type\" type=radio value=0 checked>X.509</font></td></tr>");
		req_format_write(wp,"<tr><td width=\"0.55\"><font size=2><b><script>dw(wlwapiinstalcert_ca_certificate_local)</script></b></font></td>");
		...
		req_format_write(wp,"<input onclick=sendClicked(this.form) type=button value=\"Upload\" name=\"send\">&nbsp;&nbsp;");
			req_format_write(wp,"<script>document.uploadCACert.send.value = wlwapiinstalcert_upload;</script>");
		req_format_write(wp,"<input type=\"reset\" value=\"Reset\" name=\"reset\">");
			req_format_write(wp,"<script>document.uploadCACert.reset.value = wlwapiinstalcert_reset;</script>");
		...
		req_format_write(wp,"<table border=\"0\" cellspacing=\"0\" width=\"500\">");
		req_format_write(wp,"<tr><font size=2></font></tr>");		
		req_format_write(wp,"<tr><td width=\"0.55\"><font size=2><b><script>dw(wlwapiinstalcert_asu_certificate_local)</script></b></font></td>");
		req_format_write(wp,"<td width=\"0.45\"><font size=2><input type=\"file\" name=\"asu_binary\" size=20></font></td></tr></table>");
		req_format_write(wp,"<input onclick=sendClicked(this.form) type=button value=\"Upload\" name=\"send\">&nbsp;&nbsp;");
			req_format_write(wp,"<script>document.uploadASUCert.send.value = wlwapiinstalcert_upload;</script>");
		req_format_write(wp,"<input type=\"reset\" value=\"Reset\" name=\"reset\">");
			req_format_write(wp,"<script>document.uploadASUCert.reset.value = wlwapiinstalcert_reset;</script>");
		req_format_write(wp,"<input type=\"hidden\" value=\"/wlwapiinstallcert.htm\" name=\"submit-url\">");
		...
		req_format_write(wp,"<tr><font size=2></font></tr>");
//		req_format_write(wp,"<tr><hr size=1 noshade align=top></tr>");
		req_format_write(wp,"<tr><td width=\"0.55\"><font size=2><b><script>dw(wlwapiinstalcert_user_certificate_local)</script></b></font></td>");
		req_format_write(wp,"<td width=\"0.45\"><font size=2><input type=\"file\" name=\"user_binary\" size=20></font></td></tr></table>");
		req_format_write(wp,"<input onclick=sendClicked(this.form) type=button value=\"Upload\" name=\"send\">&nbsp;&nbsp;");
			req_format_write(wp,"<script>document.uploadUserCert.send.value = wlwapiinstalcert_upload;</script>");
		req_format_write(wp,"<input type=\"reset\" value=\"Reset\" name=\"reset\">");
			req_format_write(wp,"<script>document.uploadUserCert.reset.value = wlwapiinstalcert_reset;</script>");
		req_format_write(wp,"<input type=\"hidden\" value=\"/wlwapiinstallcert.htm\" name=\"submit-url\">");
	...
	else if(!strcmp(argv[0],"status_warning"))
	{
		...
		if(needReboot == 1)
		{
			req_format_write(wp, "%s", "<tr><td></td></tr><tr><td><font size=2><font color='#FF0000'> \
 															<script>dw(status_current_setting_noeffect)</script> \
															</font></td></tr>");
		}
		else
#endif
...
#endif
		return 0;
	}else if(!strcmp(name, "show_url_redirect"))
	{
		...
		*((char *)&entry) = (char)i;
		...
		switch(entry.AddressType)
		{
			case DHCP_DISABLED:
				...
				if(isWanPhy_Link<0)
					strcpy(strWanCurrentInfo,"fmget_ip_disconnect");
				else
					strcpy(strWanCurrentInfo,"fmget_ip_connect");				
				break;
			case DHCP_CLIENT:
				if(getMultiWanInfo(strWanIfName,strWanIP,strWanMask,strWanDefGW,strWanHWAddr)<0)
				{
...
if ( !strcmp(name, "ospf_on")) {
 		if ( !apmib_get( MIB_OSPF_ENABLE, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "ospf_debug")) {
 		if ( !apmib_get( MIB_OSPF_DEBUG, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "route_id")) {
 		if ( !apmib_get( MIB_OSPF_ROUTERID, (void *)&val) )
			return -1;
		sprintf(buffer, "%u", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}

if ( !strcmp(name, "ospfnetworkNum")) {
 		if ( !apmib_get( MIB_OSPF_NET_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "ospfinterfaceNum")) {
 		if ( !apmib_get( MIB_OSPF_INTERFACE_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "ospfareaNum")) {
 		if ( !apmib_get( MIB_OSPF_AREA_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
...
if ( !strcmp(name, "bgp_on")) {
 		if ( !apmib_get( MIB_BGP_ENABLE, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "bgp_debug")) {
 		if ( !apmib_get( MIB_BGP_DEBUG, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "bgp_route_id")) {
 		if ( !apmib_get( MIB_BGP_ROUTERID, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "bgp_as")) {
 		if ( !apmib_get( MIB_BGP_AS_NUMBER, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "route_input")) {
 		if ( !apmib_get( MIB_BGP_ROUTE_INPUT, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "ospf_route_input")) {
 		if ( !apmib_get( MIB_BGP_OSPF_ROUTE_INPUT, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "bgpnetworkNum")) {
 		if ( !apmib_get( MIB_BGP_NETWORK_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "bgppeerNum")) {
 		if ( !apmib_get( MIB_BGP_PEER_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "isisnetworkNum")) {
 		if ( !apmib_get( MIB_ISIS_NET_AU_TYPE_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "isisinterfaceNum")) {
 		if ( !apmib_get( MIB_ISIS_IF_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "isis_on")) {
 		if ( !apmib_get( MIB_ISIS_ENABLE, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "isis_debug")) {
 		if ( !apmib_get( MIB_ISIS_DEBUG, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "process_id")) {
 		if ( !apmib_get( MIB_ISIS_PROCESS_ID, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "isis_net")) {
 		if ( !apmib_get( MIB_ISIS_NET, (void *)isis_net) )
			return -1;
		//sprintf(buffer, "%s",isis_net);
		req_format_write(wp, isis_net);
		return 0;
	}

if ( !strcmp(name, "isis_level")) {
 		if ( !apmib_get( MIB_ISIS_NET_LEVEL, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val);
		req_format_write(wp, buffer);
		return 0;
			...
			for (j = 0; j < 5; j++) {
				...
				if (!val) {
					if ( !apmib_get( MIB_WLAN_ENCRYPT, (void *)&val) )
						return -1;
#ifdef WLAN_WPA3
					if (val != ENCRYPT_WPA2 && val != ENCRYPT_WPA3 && val != ENCRYPT_WPA2_WPA3_MIXED) {
#else
					if (val != ENCRYPT_WPA2) {
#endif
	...
	if (!strcmp(name, "backhaul_link")) {
		...
		int widx_tmp = wlan_idx;
		int vwidx_tmp = vwlan_idx;
		if (!apmib_get(MIB_MAP_CONTROLLER, (void *)&map_state) )
			return -1;
		for (i = 0; i < wlan_num; i++) {
			wlan_idx = i;
			if (1 == map_state) {
	...
	else if ( !strcmp(name, "enable_vlan")) {
        if ( !apmib_get( MIB_MAP_ENABLE_VLAN, (void *)&val) ) {
			val = 0;
			sprintf(buffer, "%d", val);
			req_format_write(wp, buffer);
            return -1;
	...
	else if ( !strcmp(name, "primary_vid")) {
        if ( !apmib_get( MIB_MAP_PRIMARY_VID, (void *)&val) )
            return -1;
        sprintf(buffer, "%d", val);
        req_format_write(wp, buffer);
        return 0;
    }
	else if ( !strcmp(name, "default_sec_vid")) {
        if ( !apmib_get( MIB_MAP_DEFAULT_SECONDARY_VID, (void *)&val) )
            return -1;
        sprintf(buffer, "%d", val);
        req_format_write(wp, buffer);
        return 0;
    }
	else if ( NULL != strstr(name, "map_fronthaul_bss_")) {
		//findout which interface is being queried
		int length = strlen(name);
		int wlan_idx_tmp = wlan_idx;
		int vwlan_idx_tmp = vwlan_idx;
	...
	else if ( NULL != strstr(name, "map_vid_")) {
		//findout which interface is being queried
		int length = strlen(name);
		int wlan_idx_tmp = wlan_idx;
		int vwlan_idx_tmp = vwlan_idx;
		wlan_idx  = name[length - 2] - '0';
		vwlan_idx = name[length - 1] - '0';
		if ( !apmib_get( MIB_WLAN_MAP_VID, (void *)&val) ) {
			val = 0;
			sprintf(buffer, "%d", val);
			req_format_write(wp, buffer);
			wlan_idx = wlan_idx_tmp;
...
#if defined(CONFIG_IEEE80211V) || defined(HAPD_11V)
        sprintf(buffer,"%d", 1);
#else
        sprintf(buffer,"%d", 0);
#endif
...
else if ( !strcmp(name, "is_txbf_support"))
	{
#ifdef CONFIG_WLAN_8832B
		sprintf(buffer, "%d", 0);
#else
		sprintf(buffer, "%d", 1);
#endif
...
else if ( !strcmp(name, "mboEnabled"))
	{
	...
	else if(!strcmp(name,"multiWanNums"))
	{
		#ifdef MULTI_WAN_SUPPORT
			sprintf(buffer, "%d",WANIFACE_NUM);
			req_format_write(wp, buffer);
		#else
			sprintf(buffer, "%d",0);	
			req_format_write(wp, buffer);
...
//		char tmpStr[10];
	...
	else if(!strcmp(name,"is_hapd_wpas_wps"))
	{
...
//		char tmpStr[10];
	...
	else if ( !strcmp(name, "bandSteeringEnabled")) {
		if (!apmib_get(MIB_WIFI_STA_CONTROL_ENABLE, (void *)&val))
			return -1;
	...
	else if ( !strcmp(name, "bandSteeringPreferBand")) {
		if (!apmib_get(MIB_PREFER_BAND, (void *)&val))
			return -1;

#ifdef CONFIG_BAND_5G_ON_WLAN0
		if (val == PREFER_BAND_5G)
#else
		if (val == PREFER_BAND_24G)
#endif
		{
			sprintf(buffer, "%d", 0);
			req_format_write(wp, buffer);
...
#ifdef CONFIG_BAND_5G_ON_WLAN0
		else if (val == PREFER_BAND_24G)
#else
		else if (val == PREFER_BAND_5G)
#endif
		{
			sprintf(buffer, "%d", 1);
			req_format_write(wp, buffer);
...
#endif
	else if ( !strcmp(name, "isBandSteeringDefined")) {
#ifdef BAND_STEERING_SUPPORT
		sprintf(buffer,"%d", 1);
#else
		sprintf(buffer,"%d", 0);
	...
	else if(!strcmp(name, "meshAutoConfigEnabled")) {
		if(!apmib_get(MIB_CAPWAP_MODE, (void *)&val))
			return -1;
		if(val & CAPWAP_AUTO_CONFIG_ENABLE)
			val = 1;
		else
			val = 0;
		sprintf(buffer, "%d", val);
...
#endif
	else if (!strcmp(name, "isMeshAutoConfigDefined")) {
#ifdef RTK_MESH_CONFIG
		sprintf(buffer,"%d", 1);
#else
		sprintf(buffer,"%d", 0);
	...
	else if ( !strcmp(name, "isSambaSupport")) {
...
#ifdef CONFIG_APP_SAMBA
		if ( !apmib_get( MIB_SAMBA_ENABLED, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val) ;
#else
		sprintf(buffer,"%d", 0);
	...
	else if ( !strcmp(name, "isMiniDLNASupport")) {
#if defined(CONFIG_APP_MINIDLNA)
		sprintf(buffer, "%d", 1);
#else
		sprintf(buffer, "%d", 0);
#endif
	...
	else if ( !strcmp(name, "miniDLNAEnabled")) {
#ifdef CONFIG_APP_MINIDLNA
		if ( !apmib_get( MIB_MINIDLNA_ENABLED, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (int)val) ;
#else
		sprintf(buffer,"%d", 0);
...
#endif
	else if( !strcmp(name, "intf_support_ax")) {
			int flag = 0;
			memset(ifname, 0, sizeof(ifname));
			if(vwlan_idx == 0)
				snprintf(ifname, sizeof(ifname), "wlan%d", wlan_idx);
			else
				snprintf(ifname, sizeof(ifname), "wlan%d-vap%d", wlan_idx, vwlan_idx-1);

			get_ax_flag(ifname, &flag);
			sprintf(buffer, "%d", flag);
			req_format_write(wp, buffer);
			return 0;
		}
#ifdef IS_AX_SUPPORT
		else if( !strcmp(name, "wlan_ax_hidden_func")) {
			sprintf(buffer, "%s", "style=\"display: none\"");
			req_format_write(wp, buffer);
		}
#endif
		else if( !strcmp(name, "wlan_support_ax")) {
#ifdef IS_AX_SUPPORT
			sprintf(buffer, "%d", 1);
#else
			sprintf(buffer, "%d", 0);
...
//for 11N
	...
	else if ( !strcmp(name, "twt_enable")) {
		if ( !apmib_get( MIB_WLAN_TWT_ENABLED, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
...
//printf("\r\n wlan_idx=[%u],vwlan_idx=[%u],__[%s-%u]\r\n",wlan_idx,vwlan_idx,__FILE__,__LINE__);
	...
	else if(!strcmp(name,"wlan_support_8812fe")) //8812
	{
	...
	#endif
	#endif
#if defined(CONFIG_RTL8192E) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_RTL8192F) 
	isWlanMode2x2=1;
#endif
...
//### end
	...
	else if(!strcmp(name,"firmware_acs_managed")){
#if defined(CONFIG_APP_TR069)
        char vChar = 0;
        apmib_get(MIB_CWMP_ACS_UPGRADESMANAGED, (void *)&vChar);
        if(vChar==0)
            req_format_write(wp, "0");
        else
            req_format_write(wp, "1");
	...
	else if(!strcmp(name,"tr069_stun_on")){
	#if defined(TR069_ANNEX_G)
		req_format_write(wp, "1");
	#else
		req_format_write(wp, "0");
...
#if defined(TR069_ANNEX_G)
	else if(!strcmp(name,"tr069_stun_start")){
		req_format_write(wp, "");
	}
	else if(!strcmp(name,"tr069_stun_end")){
		req_format_write(wp, "");
	}
#else
	else if(!strcmp(name,"tr069_stun_start")){
...
/*	else if ( !strcmp(name, "StorageAnonAccessFtpEnable")) {
	...
	else if( !strcmp(name,"supportSuperUser"))
	{
		...
		else if( !strcmp(name,"bluetoothRepeaterConfig.htm")){
			sprintf(buffer,"%d",getHtmlAuth(name,wp->auth_flag));
			req_format_write(wp, buffer);
			return 0;
		}
		...
		else if(!strcmp(name, "sub_menu_multiap_r2_support")){
#ifdef RTK_MULTI_AP_R2
				sprintf(buffer,"%d", 1);
#else
				sprintf(buffer,"%d", 0);
		...
		else if( !strcmp(name,"band_5g_on_wlan0")){
#ifdef CONFIG_BAND_5G_ON_WLAN0
				snprintf(buffer, sizeof(buffer), "%d", 1);
#else
				snprintf(buffer, sizeof(buffer), "%d", 0);
...
#endif
#ifdef CONFIG_APP_BT_REPEATER_CONFIG
		else if(!strcmp(name,"bt_repeater_config_Enabled")) {		
			if ( !apmib_get( MIB_BT_CONFIG_ENABLED, (void *)&val) )
				return -1;
			sprintf(buffer, "%d", val);
			req_format_write(wp, buffer);
		}
#endif
#ifdef CONFIG_RTL_HTTP_REDIRECT
		else if(!strcmp(name, "RedirectEnabled")){
			if(!apmib_get(MIB_HTTP_REDIRECT_ENABLED, (void *)&val))
				return -1;
			
			sprintf(buffer, "%d", val);
			req_format_write(wp, buffer);
			return 0;
		}else if(!strcmp(name, "redirectURL")){
			char http_redirect_url[MAX_HTTP_URL_LEN] = {0};
			if(!apmib_get(MIB_HTTP_REDIRECT_URL, (void *)http_redirect_url))
				return -1;
	
			sprintf(buffer, "%s", http_redirect_url);
			req_format_write(wp, buffer);
			return 0;
		}
#endif
...
//#endif
		strcpy(WLAN_IF_old, WLAN_IF);
		sprintf(WLAN_IF, "%s-"VAP_IF_ONLY, WLAN_IF_old, vwlan_idx-1);
	}
		...
		if((operation_mode==1 && (val==0 ||val==1)) || (operation_mode==0) || (operation_mode==2)){
		   if(lan_dhcp_mode == 0){
			return req_format_write(wp,"<option selected value=\"0\"><script>dw(tcpip_lan_dhcp_disabled)</script></option>"
								"<option value=\"1\"><script>dw(tcpip_lan_dhcp_client)</script></option>"
								 "<option value=\"2\"><script>dw(tcpip_lan_dhcp_server)</script></option>"
								  "<option value=\"19\"><script>dw(tcpip_lan_dhcp_auto)</script></option>");
			  }
		if(lan_dhcp_mode == 1){
			return req_format_write(wp,"<option  value=\"0\"><script>dw(tcpip_lan_dhcp_disabled)</script></option>"
								"<option selected value=\"1\"><script>dw(tcpip_lan_dhcp_client)</script></option>"
		...
		if(lan_dhcp_mode == 2){
		   ...
		   if(lan_dhcp_mode == 19){
			return req_format_write(wp,"<option  value=\"0\"><script>dw(tcpip_lan_dhcp_disabled)</script></option>"
								"<option  value=\"1\"><script>dw(tcpip_lan_dhcp_client)</script></option>"
								 "<option value=\"2\"><script>dw(tcpip_lan_dhcp_server)</script></option>"
								 "<option selected value=\"19\"><script>dw(tcpip_lan_dhcp_auto)</script></option>");
			  }
		}
#else
 	if(lan_dhcp_mode == 0){
...
int getModeCombobox(request *wp, int argc, char **argv)
{
	...
	if ( !apmib_get( MIB_WLAN_MODE, (void *)&val) )
...
#ifdef CONFIG_NEW_MESH_UI
	  if ( val == 0 ) {
      	  	return req_format_write(wp, "<option selected value=\"0\"><script>dw(wlbasic_ap)</script></option>"
   	  	 "<option value=\"1\"><script>dw(wlbasic_client)</script></option>"
   	  	 "<option value=\"2\"><script>dw(wlbasic_wds)</script></option>"
   	  	 "<option value=\"3\"><script>dw(wlbasic_ap_wds)</script></option>"
   	  	 "<option value=\"4\"><script>dw(wlbasic_ap_mesh)</script></option>"
   	  	 "<option value=\"5\"><script>dw(wlbasic_mesh)</script></option>"  );
      	  }
	  if ( val == 1 ) {
     	  	 return req_format_write(wp,"<option value=\"0\"><script>dw(wlbasic_ap)</script></option>"
	  ...
	  if ( val == 2 ) {
     	  	 return req_format_write(wp,"<option value=\"0\"><script>dw(wlbasic_ap)</script></option>"
   	  	 "<option value=\"1\"><script>dw(wlbasic_client)</script> </option>"
 	  	 "<option selected value=\"2\"><script>dw(wlbasic_wds)</script></option>"
   	  	 "<option value=\"3\"><script>dw(wlbasic_ap_wds)</script></option>"
   	  	 "<option value=\"4\"><script>dw(wlbasic_ap_mesh)</script></option>"
   	  	 "<option value=\"5\"><script>dw(wlbasic_mesh)</script></option>"  );
   	  }
	  if ( val == 3 ) {
   	  	 ...
   	  	 "<option value=\"1\"><script>dw(wlbasic_client)</script> </option>"
 	  	 "<option  value=\"2\"><script>dw(wlbasic_wds)</script></option>"
   	  	 "<option selected value=\"3\"><script>dw(wlbasic_ap_wds)</script></option>"
   	  	 "<option value=\"4\"><script>dw(wlbasic_ap_mesh)</script></option>"
   	  	 "<option value=\"5\"><script>dw(wlbasic_mesh)</script></option>"  );
   	  }
   	  if ( val == 4 ) {
		 return req_format_write(wp,"<option value=\"0\"><script>dw(wlbasic_ap)</script></option>"
   	  	 ...
   	  	 "<option value=\"2\"><script>dw(wlbasic_wds)</script></option>"
   	  	 "<option value=\"3\"><script>dw(wlbasic_ap_wds)</script></option>"
   	  	 "<option selected value=\"4\"><script>dw(wlbasic_ap_mesh)</script></option>"
   	  	 "<option value=\"5\"><script>dw(wlbasic_mesh)</script></option>"  );
   	  }
   	  if ( val == 5 ) {
		 return req_format_write(wp,"<option value=\"0\"><script>dw(wlbasic_ap)</script></option>"
   	  	 ...
   	  	 "<option value=\"3\"><script>dw(wlbasic_ap_wds)</script></option>"
   	  	 "<option value=\"4\"><script>dw(wlbasic_ap_mesh)</script></option>"
   	  	 "<option selected value=\"5\"><script>dw(wlbasic_mesh)</script></option>"  );
   	  }
	  else
	  return 0;
  	...
  	if ( val == 0 ) {
  		...
  		sprintf(tmp,"%s","<option selected value=\"0\"><script>dw(wlbasic_ap)</script></option>");
#if defined(CONFIG_RTL_819X) && !defined(CONFIG_WLAN_CLIENT_MODE)// keith. disabled if no this mode in 96c
	...
	else
	{
...
#else
   	  strcat(tmp,"<option value=\"2\"><script>dw(wlbasic_wds)</script></option>"
   	  	 "<option value=\"3\"><script>dw(wlbasic_ap_wds)</script></option>"    );
#endif
	  ...
	  if ( val == 1 ) {
	  	char tmp[300];
  		memset(tmp,0x00,sizeof(tmp));
  		sprintf(tmp,"%s","<option value=\"0\"><script>dw(wlbasic_ap)</script></option>");
#if defined(CONFIG_RTL_819X) && !defined(CONFIG_WLAN_CLIENT_MODE)// keith. disabled if no this mode in 96c
#else
...
#if defined(CONFIG_RTL_819X) && !defined(CONFIG_WLAN_WDS_SUPPORT)// keith. disabled if no this mode in 96c
#else
   	  strcat(tmp,"<option selected value=\"2\"><script>dw(wlbasic_wds)</script></option>"
   	  	 "<option value=\"3\"><script>dw(wlbasic_ap_wds)</script></option>"    );
#endif
#ifdef CONFIG_RTL_P2P_SUPPORT
      SDEBUG("\n");
