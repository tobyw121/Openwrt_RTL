...
#ifdef CONFIG_IPV6
#define WAN_LINK_STATUS_FILE "/proc/eth1/link_status"
int checkWanLinkStatus()
{ 
	FILE *pfile = NULL;
	int status = -1;
	int ipv6_link_type = -1;
	int wanv6_enable=0;
	char tmpStr[256];
	char pidname[64];
	char wanv6_interface[16]={0};
	int opmode;
	int pid=-1;
	
	if(!isFileExist(WAN_LINK_STATUS_FILE))
	{
		printf("%s: %s is not exist!!\n",__FUNCTION__, WAN_LINK_STATUS_FILE);
		return -1;
	}
	apmib_get(MIB_IPV6_WAN_ENABLE,(void *)&wanv6_enable);
	if(0 == wanv6_enable)
	{	
		return -1;
	}

	apmib_get(MIB_IPV6_LINK_TYPE,(void *)&ipv6_link_type);
	if(IPV6_LINKTYPE_IP != ipv6_link_type) // only need to check ipoe+dhcp6c
	{	
		return -1;
	}

	apmib_get(MIB_OP_MODE,(void *)&opmode);
	if(opmode==GATEWAY_MODE){   // dhcp6c sense link down feature only support Gateway mode
		sprintf(wanv6_interface, "%s", "eth1");
	} else
		return -1;
	
	if((pfile = fopen(WAN_LINK_STATUS_FILE,"r+"))!= NULL)
	{
		fscanf(pfile,"%d",&status);
		if(status == 0) // link_status =0, link down , need restart dhcp6c if dhcp6s's pid exists
		{	

		    if (link_down_restart_dhcp6c==0) {/*kill dhcp6c daemon if exists*/
				sprintf(pidname,DHCP6C_PID_FILE);
				if(isFileExist(pidname)){
					pid=getPid_fromFile(pidname);
					if(pid>0){
						sprintf(tmpStr, "%d", pid);
						RunSystemCmd(NULL_FILE, "kill", "-15", tmpStr, NULL_STR);
					}
					unlink(pidname);

					//flush v6 global address
					sprintf(tmpStr, "ip -6 address flush dev %s scope global",wanv6_interface);
					system(tmpStr);

					// ipv6 disable->enable to send RS
					sprintf(tmpStr,"echo 1 >/proc/sys/net/ipv6/conf/%s/disable_ipv6",wanv6_interface);
			        system(tmpStr);
					sprintf(tmpStr,"echo 0 >/proc/sys/net/ipv6/conf/%s/disable_ipv6",wanv6_interface);
					system(tmpStr);
			
					/*restart dhcp6c daemon*/
					sprintf(tmpStr, "dhcp6c -c %s -p %s %s ", DHCP6C_CONF_FILE,DHCP6C_PID_FILE,wanv6_interface);
					system(tmpStr);	
					link_down_restart_dhcp6c=1;
				}
			}
		}				
		else // link_status=1, link up 
			link_down_restart_dhcp6c=0;
		fclose(pfile);
	}
	else
	{
...
void ppp_connect_ipv6(char *ifname, char *option)
{
	...
	switch(val){			
		case IPV6_ORIGIN_DHCP:
	        ...
	        sprintf(tmpStr,"echo 2 > /proc/sys/net/ipv6/conf/%s/accept_ra",ifname);  
			system(tmpStr);
			/*disable eth1 accept ra to avoid parsing RDNSS field from eth1*/
	        sprintf(tmpStr,"echo 0 > /proc/sys/net/ipv6/conf/eth1/accept_ra"); 		
			system(tmpStr);
			set_dhcp6c();
	...
	/* recover default proc setting for set 
 	 * ipv6 over pppoe on demand mode
 	 */
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "echo 1 > /proc/sys/net/ipv6/conf/%s/forwarding", ifname);
	system(tmpStr);
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "echo 3 > /proc/sys/net/ipv6/conf/%s/router_solicitations",ifname);
	system(tmpStr);
...
void clean_6rd()
{
	...
	printf("%s.%d. clean ipv6 wan 6rd tunnel\n",__FUNCTION__,__LINE__);
	snprintf(tmpStr, sizeof(tmpStr), "ip link set tun6rd down 2> /dev/null\n");
	system(tmpStr);
	
	snprintf(tmpStr, sizeof(tmpStr), "ip -6 tunnel del tun6rd 2> /dev/null\n");
	system(tmpStr);
	
	if(isFileExist(RADVD_CONF_FILE))/* if not delete radvd conf file it will not write radvd conf for 6rd*/
		unlink(RADVD_CONF_FILE);
}
#define IP_ADDR 0
void set_6rd_auto(char *wanip)
{
	/*0 32 2001:0db8:0000:0000:0000:0000:0000:0000 10.2.2.1 */
	addr6CfgParam_t	addr6={0}, addr6_6rd_pd={0};
	int mask=0; struct in_addr wanaddr={0};
	char addr4_6rd_br[32]={0}, addr6rdprefix[64]={0}; char tempbuf[256]={0};
	int val=0, i=0; FILE *fp=NULL;
	...
	if(val==0) /*0 : 6RD manual, 1 : 6RD auto*/
		return;

	/* get 6rd param */
	fp=fopen("/var/dhcp6rd.conf","r");
	if(fp==NULL)
	{
		fprintf(stderr, "open /var/dhcp6rd.conf fail!\n");
		return ;		
	}
	else
	{
		...
		{
			sscanf(tempbuf, "%d %d %s %s", &mask, &addr6.prefix_len, addr6rdprefix, addr4_6rd_br);
		}
		fclose(fp);
	}
	
	inet_aton(wanip, &wanaddr);
	
	inet_pton(AF_INET6, addr6rdprefix, addr6.addrIPv6);
	
	for(i=0;i<8;i++)
		addr6.addrIPv6[i]=htons(addr6.addrIPv6[i]);

	//printf("%s %d addr6.addrIPv6[0]=%04x, wanip=%s, addr4_6rd_br=%s\n",__FUNCTION__,__LINE__, addr6.addrIPv6[0], inet_ntoa(wanaddr), addr4_6rd_br);
...
#define IFTUN6IN4 "tun6in4"
void clean_ip6iptunnel()
{
	char tmpStr[128]={0};
	int val=0;
	...
	if(val != IPV6_ORIGIN_IP6INIP) //if not 6rd, return
		return;
	...
	printf("%s.%d. clean ipv6 wan 6rd tunnel\n",__FUNCTION__,__LINE__);
	snprintf(tmpStr, sizeof(tmpStr), "ip link set %s down 2>/dev/null",IFTUN6IN4);
	system(tmpStr);
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "ip -6 tunnel del %s 2>/dev/null",IFTUN6IN4);
	system(tmpStr);
}
void create_ip6ip_tunnel(char *remoteaddr6, char *localaddr6, int *plen, char *remoteaddr, char *wanaddr)
{
	char tmpStr[128];
	if(!localaddr6 || !plen || !remoteaddr6 || !remoteaddr || !wanaddr)
		return ;
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "ip tunnel add %s mode sit remote %s local %s ttl 255 2>/dev/null\n",IFTUN6IN4,remoteaddr, wanaddr);
	printf("%s_%d %s\n",__FUNCTION__, __LINE__,tmpStr);
	system(tmpStr);
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "ip link set %s up 2>/dev/null\n",IFTUN6IN4);
	printf("%s_%d %s\n",__FUNCTION__, __LINE__,tmpStr);
	system(tmpStr);
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "ip -6 addr add %s/%d dev %s 2>/dev/null\n",localaddr6, *plen, IFTUN6IN4);
	printf("%s_%d %s\n",__FUNCTION__, __LINE__,tmpStr);
	system(tmpStr);
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "ip -6 route add default via %s dev %s 2>/dev/null\n",remoteaddr6, IFTUN6IN4);
	printf("%s_%d %s\n",__FUNCTION__, __LINE__,tmpStr);
	system(tmpStr);
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "echo 1 > /proc/sys/net/ipv6/conf/all/forwarding 2>/dev/null\n");
	printf("%s_%d %s\n",__FUNCTION__, __LINE__,tmpStr);
	system(tmpStr);
}

void set_ip6iptunnel(char *wanip)
{
	addr6CfgParam_t	remoteaddr6, localaddr6;
	char wanaddr[20], remoteaddr[20], strlocaladdr6[64], strremoteaddr6[64];
	char tmpBuf[20]; int val, i, plen;
	...
	if(val != IPV6_ORIGIN_IP6INIP) /*if not ip6ip mode, return*/
		return;
	...
	/*get ip6ip remote addr6*/
	if ( !apmib_get(MIB_IPV6_IP6IP_REMOTE_ADDR6_PARAM,(void *)&remoteaddr6)){
		printf("Get MIB_IPV6_IP6IP_REMOTE_ADDR6_PARAM error!\n");
		return;        
	}
	for(i=0;i<8;i++)
		remoteaddr6.addrIPv6[i]=htons(remoteaddr6.addrIPv6[i]);
	inet_ntop(AF_INET6, &remoteaddr6.addrIPv6, strremoteaddr6, sizeof(strremoteaddr6));
	
	/*get ip6ip local addr6*/
	if ( !apmib_get(MIB_IPV6_ADDR_WAN_PARAM,(void *)&localaddr6)){
		printf("Get MIB_IPV6_ADDR_WAN_PARAM error!\n");
		return;        
	}
	for(i=0;i<8;i++)
		localaddr6.addrIPv6[i]=htons(localaddr6.addrIPv6[i]);
	inet_ntop(AF_INET6, &localaddr6.addrIPv6, strlocaladdr6, sizeof(strlocaladdr6));
	plen = localaddr6.prefix_len;

	/*get ip6ip remote addr*/
	if(!apmib_get( MIB_IPV6_IP6IP_REMOTE_ADDR,  (void *)&tmpBuf)){
		printf("Read MIB_IPV6_IP6IP_REMOTE_ADDR Error\n");
		return;			
	}
	sprintf(remoteaddr, "%s",inet_ntoa(*((struct in_addr *)tmpBuf)));

	
	/*get ip6ip local addr*/
	snprintf(wanaddr, sizeof(wanaddr), "%s", wanip);
	
	printf("%s_%d wanip=%s remoteaddr=%s\n",__FUNCTION__, __LINE__,wanip, remoteaddr);	

	/* create ipv6 in ipv4 tunnel*/	
	printf("%s_%d %s %s %d %s %s\n",__FUNCTION__, __LINE__,strremoteaddr6, strlocaladdr6, plen, remoteaddr, wanaddr);
	create_ip6ip_tunnel(strremoteaddr6, strlocaladdr6, &plen, remoteaddr, wanaddr);	
}
#endif
#ifdef CONFIG_IP6TO4_TUNNEL_SUPPORT
#define IFTUN6TO4 "tun6to4"
void clean_6to4tunnel()
{
	char tmpStr[128]={0};
	int val=0;
	...
	if(val==0)	/* ipv6 disabled */
		return;
	
	printf("%s.%d. clean ipv6 wan 6to4 tunnel\n",__FUNCTION__,__LINE__);
	snprintf(tmpStr, sizeof(tmpStr), "ip link set %s down 2>/dev/null",IFTUN6TO4);
	system(tmpStr);
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr, sizeof(tmpStr), "ip tunnel del %s 2>/dev/null",IFTUN6TO4);
	system(tmpStr);
	
}

void set_6to4tunnel(char *wan_iface)
{
	...
	if(val != IPV6_ORIGIN_IP6TO4) /*if not 6to4, return*/
		return;

	if(val==0) /* if unable ipv6, return*/
		return;
	
	printf("create 6to4 tunnel!\n");
	
	tmp_addr.s_addr = 0;

	tmp_addr.s_addr = do_ioctl_get_ipaddress(wan_iface);/*consider eth1 ppp0*/
	...
	/*add iptables*/
	bzero(tmpBuf,sizeof(tmpBuf));
	sprintf(tmpBuf,"iptables -A INPUT -p 41 -i %s -d %s -j ACCEPT\n",wan_iface, wanIP);/*need exist or not ? */
	system(tmpBuf);
	#endif
	
	/* create 6to4 tunnel*/
	bzero(tmpBuf,sizeof(tmpBuf));
	sprintf(tmpBuf,"ip tunnel add %s mode sit remote any local %s\n",IFTUN6TO4,wanIP);
	system(tmpBuf);	

	bzero(tmpBuf,sizeof(tmpBuf));
	sprintf(tmpBuf,"ifconfig %s up\n", IFTUN6TO4);
	system(tmpBuf);
	...
	bzero(tmpBuf,sizeof(tmpBuf));
	sprintf(tmpBuf,"ifconfig %s 2002:%02x%02x:%02x%02x::1\/16\n",IFTUN6TO4,atoi(p1),atoi(p2),atoi(p3),atoi(p4));
	system(tmpBuf);

	#if 0
	//br0
	bzero(tmpBuf,sizeof(tmpBuf));
	sprintf(tmpBuf,"ifconfig br0 2002:%02x%02x:%02x%02x:1::1\/64\n",atoi(p1),atoi(p2),atoi(p3),atoi(p4));
	system(tmpBuf);
	#endif

	#if 1
	bzero(tmpBuf,sizeof(tmpBuf));
	sprintf(tmpBuf,"echo 2 > /proc/sys/net/ipv6/conf/br0/accept_ra\n",atoi(p1),atoi(p2),atoi(p3),atoi(p4));
	system(tmpBuf);
	#endif

	/* add route */
	bzero(tmpBuf,sizeof(tmpBuf));
	sprintf(tmpBuf,"route -A inet6 add 2002::/16 gw ::%s dev %s\n",GW, IFTUN6TO4);
	system(tmpBuf);
	 ...
	 if(!strcmp(interface, "br0")){//do not care wan type, it is dhcp client only
		...
		for(x=0;x<2;x++){
			if(dns_server[x][0] != '\0'){ 
				...
				if(x==0)
					write_line_to_file(RESOLV_V4_CONF, 1, line);
				else
					write_line_to_file(RESOLV_V4_CONF, 2, line);
			}
		}
		update_dns_resolv_file();
	}
...
//#ifdef TR181_V6_SUPPORT
			...
			RunSystemCmd(NULL_FILE, "cp", PPP_RESOLV_FILE, "/var/resolv_v4.conf", NULL_STR);
            update_dns_resolv_file();
#else			
			RunSystemCmd(NULL_FILE, "cp", RESOLV_CONF, "/var/resolv_v4.conf", NULL_STR);
            		update_dns_resolv_file();
//			system("cat /etc/ppp/resolv.conf >> /var/resolv.conf");
...
#endif
#ifdef CONFIG_IPV6
#if defined(CONFIG_SIXRD_SUPPORT)&& !defined(CONFIG_APP_APPLE_MFI_WAC)
	clean_6rd();
	set_6rd_auto(wanip);
#endif
#if defined(CONFIG_IP6IP_TUNNEL_SUPPORT) && !defined(CONFIG_APP_APPLE_MFI_WAC)
	clean_ip6iptunnel();
	set_ip6iptunnel(wanip);
#endif
#if defined(CONFIG_IP6TO4_TUNNEL_SUPPORT) && !defined(CONFIG_APP_APPLE_MFI_WAC)
	clean_6to4tunnel();
	set_6to4tunnel(interface);
#endif
#endif
}
...
#ifdef CONFIG_IPV6
#if defined(CONFIG_SIXRD_SUPPORT)
		clean_6rd();
#endif
#if defined(CONFIG_IP6IP_TUNNEL_SUPPORT)
		clean_ip6iptunnel();
#endif
#if defined(CONFIG_IP6TO4_TUNNEL_SUPPORT)
		clean_6to4tunnel();
#endif
#endif
...
void set_v6_dns()
{
	...
	// update resolv_v6.conf & resolv.conf
	fh = open(RESOLV_V6_CONF, O_RDWR|O_CREAT|O_TRUNC, S_IRWXO|S_IRWXG); // 
	if ((fh < 0) || (fh==NULL)) {
		fprintf(stderr, "Create %s file error!\n", RESOLV_V6_CONF);
		fclose(fp);
		return;
...
void checkDnsv6()
{
		dhcp6sCfgParam_t dhcp6sCfgParam;
		char dns_server[64];
		char dns_server_dut[64];
		char dns_server_manual[64];
		int dnsforward=0;
		char tmpBuf[64];
		int pid=-1;
		FILE *fp;
		...
		int lanOrigin=0;
		if ( !apmib_get(MIB_IPV6_LAN_ORIGIN,(void *)&lanOrigin)){
			fprintf(stderr,"get MIB_IPV6_LAN_ORIGIN failed\n");
			return ;  
		}
		
		if (lanOrigin) {// lan manual mode
		...
		/************ dhcp6s daemon config ******************************************/

		apmib_get(MIB_IPV6_DNS_AUTO, (void *)&dnsMode);
		if(dnsMode==0)	{//Set DNS Manually 
	        // wan dns maual server
			addr6CfgParam_t addr6_dns;
			apmib_get(MIB_IPV6_ADDR_DNS_PARAM,	(void *)&addr6_dns);	
			snprintf(tmpBuf, sizeof(tmpBuf), "%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x", 
			addr6_dns.addrIPv6[0], addr6_dns.addrIPv6[1], addr6_dns.addrIPv6[2], addr6_dns.addrIPv6[3], 
			...
			if(strstr(tmpBuf, "0000:0000:0000:0000:0000:0000:0000:0000")==NULL) 
				//add Wan UI DNS 
				memcpy(dns_server_manual, tmpBuf, sizeof(tmpBuf));
		}else{
			/************ update resolv config file ******************************************/
			set_v6_dns();
		}

		// auto dns domain server
		fp=fopen(DNSV6_ADDR_FILE,"r");
		if(fp==NULL)
			return;
		...
		if(dnsforward==0)	// dns relay mode
		{
			...
			if(get_linklocal_addr(&addr6_dns,dhcp6sCfgParam.interfaceNameds)>0)
			{
				...
				if(strstr(tmpBuf, "0000:0000:0000:0000:0000:0000:0000:0000")==NULL) 
				{	
					memcpy(dns_server_dut, tmpBuf, sizeof(tmpBuf));
				}
			}
		} else
		#endif
		{
			memcpy(dns_server_dut, dns_server, sizeof(dns_server));
		}
		...
		if(flag){
			if(isFileExist(DHCP6S_PID_FILE)) {
				if ((fp = fopen(DHCP6S_PID_FILE, "r")) != NULL) {
					fscanf(fp, "%d\n", &pid);
					fclose(fp);
					RunSystemCmd(NULL_FILE, "killall", "-9", "dhcp6s", NULL_STR);
					unlink(DHCP6S_PID_FILE);
		...
		printf("%s:%d dns update to restart dhcp6s\n",__FUNCTION__,__LINE__);
		system("rm /var/dhcp6_dns_need_update;");
		return;
}
...
void simple_security_reconfig()
{
	char prefix_ipv6_str[64] = {0};
	char prefix_str_plus_pdlen[64] = {0};
	char *bInterface = "br0";
	char *wanInterface = "eth1";
	char *simple_security_chain ="simple_security";

	RunSystemCmd(NULL_FILE, "ip6tables", "-F", simple_security_chain,NULL_STR);
	RunSystemCmd(NULL_FILE, "ip6tables", "-D", "INPUT", "-j", simple_security_chain,NULL_STR);
	RunSystemCmd(NULL_FILE, "ip6tables", "-X", simple_security_chain,NULL_STR);
	RunSystemCmd(NULL_FILE, "ip6tables", "-N", simple_security_chain,NULL_STR);
	
	sprintf(prefix_str_plus_pdlen,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
				dhcp6_pd.addr6[0],dhcp6_pd.addr6[1],
				dhcp6_pd.addr6[2],dhcp6_pd.addr6[3],
				dhcp6_pd.addr6[4],dhcp6_pd.addr6[5],
	 ...
	 */
	RunSystemCmd(NULL_FILE, "ip6tables", "-I", simple_security_chain, "-i", bInterface, "!", "-s", prefix_str_plus_pdlen, "-j", "DROP", NULL_STR);
	/* RFC 6092 REC-6
	 ...
	 */
	RunSystemCmd(NULL_FILE, "ip6tables", "-I", simple_security_chain, "-i",  wanInterface,  "-s", prefix_str_plus_pdlen, "-j", "DROP", NULL_STR);
	
	RunSystemCmd(NULL_FILE, "ip6tables", "-I", "INPUT", "-j", simple_security_chain,NULL_STR);
...
void radvd_reconfig()
{
	...
	if(radvdCfgParam.enabled){
		...
		if(lanOrigin){ // lan manual mode , set prefix from Mib setting
			if(radvdCfgParam.interface.prefix[0].enabled > 0){
	...
	/*add prefix information from PD in lan_auto mode*/
	if(lanOrigin==0){       // lan auto mode, set prefix from PD.
		if (dhcp6_pd.flag){
...
//		printf("%s %d: prefix is %s.\n", __FUNCTION__, __LINE__, tmpBuf);
	...
	if(radvdCfgParam.interface.prefix[1].enabled > 0){
		...
		else
		{
			...
			} else if(isFileExist(DNSV6_ADDR_FILE))
			{
				if((fh=fopen("/var/dns6.conf","r"))!=NULL)
				{
				    int copylen=0;
					memset(tmpStr, 0, sizeof(tmpStr));
					while(fgets(tmpBuf, sizeof(tmpBuf), fh))
					{
						tmpBuf[strlen(tmpBuf)-1]=0;
						if(strstr(tmpBuf,"nameserver")!=NULL)
						{
							...
							}else {    //handle format:   nameserver fe80:0000:0000:0000:16fe:b5ff:fee9:5eab%eth1.400
								copylen = strstr(tmpBuf,"%") - strstr(tmpBuf,"nameserver") - strlen("nameserver") -1;
								strncat(tmpStr,strstr(tmpBuf,"nameserver")+strlen("nameserver")+1,copylen);
							}
							strcat(tmpStr, " ");
						}
					}
...
//		strcpy(entry[1].interface, interface);
		...
		write_line_to_file(RESOLV_V4_CONF,1, line_buffer);
...
//		strcpy(entry[1].interface, interface);
		...
		write_line_to_file(RESOLV_V4_CONF,2, line_buffer);
...
void start_ntp(void)
{
	...
	if(ntp_onoff == 1)
	{
		...
		RunSystemCmd(NULL_FILE, "ntp_inet", "-x", buffer, NULL_STR);
		}
}
...
#if (defined(SUPPORT_ZIONCOM_RUSSIA) || defined(_ALPHA_DUAL_WAN_SUPPORT_)) && defined(CONFIG_APP_TR069)
void addRouteForACS()
{
	FILE *fp=NULL;
