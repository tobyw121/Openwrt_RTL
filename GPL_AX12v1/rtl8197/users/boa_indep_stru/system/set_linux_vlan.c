...
#define DEFAULT_NAT_LAN_PRIORITY 0
...
#define WLAN_DEV_INDEX_MIN 5
#define WLAN_DEV_INDEX_MAX 16
...
static const char *portRealName[] =
{
	...
	#endif
#ifdef IS_AX_SUPPORT
	"wlan0", "wlan0-vap0", "wlan0-vap1", "wlan0-vap2", "wlan0-vap3", "wlan0-vxd",
	"wlan1", "wlan1-vap0", "wlan1-vap1", "wlan1-vap2", "wlan1-vap3", "wlan1-vxd"
#else
...
static const unsigned char portMaskBit[] =
{
	ETH_P0_MASK_BIT,
	ETH_P1_MASK_BIT,
	ETH_P2_MASK_BIT,
...
static int setDefaultPVid(void)
{
	...
	//LAN/WLAN port.       lan/wlan的PVID设为9，wan的设为8
...
static int opModeChangeDelAll(void)
{
	...
	{
		APMIB_SET(MIB_VLAN_DELALL, (void *)&entry);    //和当前模式不同，清空vlan table,重设pvid
...
static void setWlanDevMaskBit(char **devNames, char *devMaskBits)
{
	int i = 0;

	if(!devNames || !devMaskBits)
		printf("input null pointer! devNames: %p devMaskBits: %p\n", devNames, devMaskBits);

	for(i = WLAN_DEV_INDEX_MIN; i <= WLAN_DEV_INDEX_MAX; i++)
	{
		if(wlanDevUpFlag & (1 << i))
			RUN_CMD("echo %s %u > /proc/net/vlan/vlanMemberMap", devNames[i], devMaskBits[i]);
...
int setLinuxVlan(void)
{
	...
	setWlanDevMaskBit(portRealName, portMaskBit);
...
static int setVlanGatewayMode(void)
{
	...
	for(idx=1; idx<=entry_num; idx++)
	{
		*(char *)&entry = (char)idx;
		...
		#endif
		{
			...
			RUN_CMD("echo \"1,%x,%x,%d,%d,%u\" > /proc/net/vlan/vlanGroup",  \
				port_member_mask, port_tagged_mask, entry.VlanId, VLAN_TYPE_NAT, entry.VlanPriority);
			...
			RUN_CMD("vconfig set_egress_map eth0.%d 0 %u", entry.VlanId, entry.VlanPriority);
			
			sprintf(tmpBuf, "eth0.%d:", entry.VlanId);
			strcat(reinitBuf, tmpBuf);
			#else
			for(port_idx=0; port_idx<4; port_idx++)   //创建ethernet相关的虚拟设备并挂到br0下
			{
				...
				RUN_CMD("vconfig set_egress_map %s.%d 0 %u", portRealName[port_idx], entry.VlanId, entry.VlanPriority);
			...
			for(port_idx=5; port_idx<port_num; port_idx++)   //创建wirelesss相关的虚拟设备并挂到br0下
			{
	...
	for(idx=1; idx<=entry_num; idx++)
	{
		*(char *)&entry = (char)idx;
		...
		if(entry.VlanType == VLAN_TYPE_BRIDGE)
		{
			...
			RUN_CMD("echo \"1,%x,%x,%d,%d,%u\" > /proc/net/vlan/vlanGroup",  \
				port_member_mask, port_tagged_mask, entry.VlanId, VLAN_TYPE_BRIDGE, entry.VlanPriority);
			...
			RUN_CMD("vconfig set_egress_map eth1.%d 0 %u", entry.VlanId, entry.VlanPriority);
	...
	RUN_CMD("echo \"1,%x,%x,%d,%d,%u\" > /proc/net/vlan/vlanGroup",  \
		port_member_mask, port_tagged_mask, DEFAULT_NAT_LAN_VID, VLAN_TYPE_NAT, DEFAULT_NAT_LAN_PRIORITY);
	...
	if(lan_port_remained & 0x1f & (~wan_mask))
	{
		...
		RUN_CMD("vconfig set_egress_map eth0.%d 0 %u",	DEFAULT_NAT_LAN_VID, DEFAULT_NAT_LAN_PRIORITY);
		...
		for(port_idx=0; port_idx<4; port_idx++)
		{
			...
			RUN_CMD("vconfig set_egress_map %s.%d 0 %u", portRealName[port_idx], DEFAULT_NAT_LAN_VID, DEFAULT_NAT_LAN_PRIORITY);
...
static int setVlanBridgeMode(void)
{
	...
	RUN_CMD("echo \"1,%x,%x,%d,%d,%u\" > /proc/net/vlan/vlanGroup",  \
		port_member_mask, port_tagged_mask, DEFAULT_NAT_LAN_VID, VLAN_TYPE_BRIDGE, DEFAULT_NAT_LAN_PRIORITY);
