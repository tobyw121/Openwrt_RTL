...
#ifdef IS_AX_SUPPORT
int check_wlan_ax_support()
{
	int i=0, intVal=0, isMibChange=0,setVal=0;
	char ifname[64] = {0};
	DIR *dir=NULL, *dir1=NULL;
	char filename_ax[64] = {0}, filename_ac[64] = {0}, filename_new[64]={0};
	char cmd_file[128]={0};
	...
	for(i=0; i<NUM_WLAN_INTERFACE; ++i){
		...
		memset(filename_ax, 0, sizeof(filename_ax));
		memset(filename_ac, 0, sizeof(filename_ac));
		...
		if(NULL != opendir(filename_new))
		{
			...
			if(dir){
				...
				if(!intVal){
					setVal = 1;
					isMibChange = 1;
					apmib_set(MIB_WLAN_AX_SUPPORT, (void *)&setVal);
				}
				closedir(dir);
			}
		}
...
//add for start tr069 when system is boot
	...
	else if(argv[1] && (strcmp(argv[1],"dslitetunnel")==0))
	{ //sysconf dslitetunnel %s aftr_name=%s
		char intf_name[16]={0};
		char aftr_name[64]={0};
		if(argc < 4){
			printf("sysconf dslitetunnel Invalid agrments:%d!\n",argc);
			return 0;
		}
		sprintf(intf_name, "%s",argv[2]);
		if(strncmp("aftr_name=",argv[3],strlen("aftr_name="))==0)
		{
			strcpy(aftr_name,(argv[3]+strlen("aftr_name=")));
			printf("set dslite auto, intf_name=%s,aftr_name=%s\n",intf_name,aftr_name);
			set_dslite_auto(intf_name,aftr_name);
		}
...
#endif
#ifdef IS_AX_SUPPORT
	else if(argv[1] && ((strcmp(argv[1],"wps_wlan_restart")==0) || (strcmp(argv[1],"wlan_restart")==0)))
	{
		int wait=0;
		if(strcmp(argv[1],"wps_wlan_restart")==0){
			while(wait++< WPS_RESTART_TIMEOUT){
				sleep(1);
			}
