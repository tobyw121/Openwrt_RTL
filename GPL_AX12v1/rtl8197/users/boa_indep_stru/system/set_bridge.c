...
{	
	...
	for (i=1; i<=entrynum; i++) 
	{
		*((char *)&entry) = (char)i;
		...
		//unsigned int reserved_ip[16]={0};
		char tmp_reserved_ip[32]={0}, str_reserved_ip[288]={0};
		for(i=0;i<16;i++)
		{
			if(entry.reservedDhcpIp[i]>0)
			{
				memset(tmp_reserved_ip, 0, sizeof(tmp_reserved_ip));
				snprintf(tmp_reserved_ip, sizeof(tmp_reserved_ip), "%s ", inet_ntoa(*((struct in_addr*)&entry.reservedDhcpIp[i])));
				strcat(str_reserved_ip, tmp_reserved_ip);
			}
		}
		
		if(str_reserved_ip[0])	
		{
			snprintf(line_buffer, sizeof(line_buffer), "reserved_ip %s\n", str_reserved_ip);
			write_line_to_file(DHCPD_CONF_FILE, 2, line_buffer);
		}
#endif
...
{	
	...
	/*end static dhcp entry*/
		...
		unsigned int reserved_ip[16]={0};
		char tmp_reserved_ip[32]={0}, str_reserved_ip[288]={0};
		apmib_get(MIB_DHCP_RESERVED_IPADDR, (void *)reserved_ip);
		for(i=0;i<16;i++)
		{
			if(reserved_ip[i]>0)
			{
				memset(tmp_reserved_ip, 0, sizeof(tmp_reserved_ip));
				snprintf(tmp_reserved_ip, sizeof(tmp_reserved_ip), "%s ", inet_ntoa(*((struct in_addr*)&reserved_ip[i])));
				strcat(str_reserved_ip, tmp_reserved_ip);
			}
		}
...
{//// must be gateway mode!
	...
	//if(ret_val<1)
	create_dhcpd_default_configfile(interface, mode, ret_val);
#else	
...
#ifdef _PRMT_X_TELEFONICA_ES_DHCPOPTION_
				if(!entry.dhcpRsvdIpEntryEnabled)
					continue;
#endif
				memcpy(&reserve_ip, entry.ipAddr, 4);
				if((reserve_ip & lan_netmask) != (lan_ip & lan_netmask))
...
extern int setLinuxVlan(void);
extern int initLinuxVlan(void);
#endif
...
//for pass thru
		...
		#if defined(CONFIG_RTK_BRIDGE_VLAN_SUPPORT) || defined(CONFIG_RTL_HW_VLAN_SUPPORT) //mark_wvlan
			#if 1 //!defined(CONFIG_RTL_HW_VLAN_SUPPORT) //mark_wvlan
...
//printf("\r\n bridge_iface=[%s],__[%s-%u]\r\n",bridge_iface,__FILE__,__LINE__);
  ...
  if(opmode == GATEWAY_MODE && vlan_wan_bridge_enable)
	...
	//if(br_stp_enabled==0){/* Comment here because Enable Spanning tree and Clone Mac Address, then Apply changes, cannot access DUT .*/
		...
		sprintf(cmdBuffer, "echo update_br0_mac 0x%02x-%02x-%02x-%02x-%02x-%02x > /proc/rtl865x/brsc", (unsigned char)tmpBuff[0], (unsigned char)tmpBuff[1], (unsigned char)tmpBuff[2], (unsigned char)tmpBuff[3], (unsigned char)tmpBuff[4], (unsigned char)tmpBuff[5]);
		system(cmdBuffer);
	//}
	if(isFileExist(BR_INIT_FILE)==0){//bridge init file is not exist
