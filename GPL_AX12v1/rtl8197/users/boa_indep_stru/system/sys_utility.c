...
{
	int i=0,j=0;
	char dnsTmpFile[64]={0};
	char cmdbuff[256]={0};
	
	printf("%s.%d enter update_dns_resolv_file \n",__FUNCTION__,__LINE__ );
	sprintf(cmdbuff,"rm -rf %s 2>/dev/null",RESOLV_CONF);
	system(cmdbuff);

#ifdef CONFIG_IPV6
	snprintf(dnsTmpFile,sizeof(dnsTmpFile),"%s",RESOLV_V6_CONF);
	if(isFileExist(dnsTmpFile)){//then add this file to resolv.conf
		snprintf(cmdbuff,sizeof(cmdbuff),"cat %s >>%s",dnsTmpFile,RESOLV_CONF);
		system(cmdbuff);
	}
#endif

	snprintf(dnsTmpFile,sizeof(dnsTmpFile),"%s",RESOLV_V4_CONF);
	if(isFileExist(dnsTmpFile)){//then add this file to resolv.conf
		snprintf(cmdbuff,sizeof(cmdbuff),"cat %s >>%s",dnsTmpFile,RESOLV_CONF);
		system(cmdbuff);
	}
...
int rtk_get_interface_flag(const char *ifname, int count, int flag)
{
	...
	{
		//printf("======>%s %d %s interface doesn't run %d, %d\n", __FUNCTION__, __LINE__, ifname, count, ++loop_cnt);
		sysinfo(&sys_info);
		if((sys_info.uptime - timestamp) > count)
		{
			ret = 0;
...
static struct process_nice_table_entry process_nice_table[] = {
	//processName, pidFile, nice
	{"dnrd", "", -5},
	{"udhcpd", "", -5},
	{"hostapd", "/var/run/hapd_wlan0_pid.pid", -5},
	{"hostapd", "/var/run/hapd_wlan1_pid.pid", -5}
};

void renice_processes(){
	int pid = 0, idx, totalProcess, nice;

	totalProcess = ((sizeof(process_nice_table))/(sizeof(struct process_nice_table_entry)));
	for (idx = 0; idx < totalProcess; idx++){
		if (process_nice_table[idx].pidFile[0]){
			if (isFileExist(process_nice_table[idx].pidFile) == 1){
				pid = getPid_fromFile(process_nice_table[idx].pidFile);
			}else
				pid = find_pid_by_name(process_nice_table[idx].processName);
		}else{
			pid = find_pid_by_name(process_nice_table[idx].processName);
		}
		...
		if ((pid > 0) && (kill(pid, 0) == 0)){
			...
			if (nice != process_nice_table[idx].nice){
				setpriority(PRIO_PROCESS, pid, process_nice_table[idx].nice);
				printf("[%s:%d] change %s(pid=%d) nice from %d to %d\n",  __FUNCTION__,__LINE__,
							process_nice_table[idx].processName, pid, nice, process_nice_table[idx].nice);
			}
		}
	}	
}
