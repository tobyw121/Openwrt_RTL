...
#define HS2_PID_FILE "/var/hs2_pidname"
#endif
...
#ifdef WLAN_HS2_CONFIG
	if(isFileExist(HS2_PID_FILE)){
		FILE *fp;
		char line[100];
		fp = fopen(HS2_PID_FILE, "r");
		if (fp != NULL) {
			fgets(line, 100, fp);
			fclose(fp);
...
int setWlan_Applications(char *action, char *argv)
{
	...
	int wlan0_mode=1, wlan1_mode=1, both_band_ap=0;
	int wlan_wsc_disabled_root=0, wlan_network_type_root=0, wlan0_wsc_disabled_vxd=1, wlan1_wsc_disabled_vxd=1, wlan_macac_enable_root=1;
...
//for mesh========================================================
...
#endif
	//printf("all_wlan_interface is %s\n",all_wlan_interface);
	/*release auth*/
#if 1
	char wlan_if_name[16];
	char file_name[30];
	int wlan_idx_num,vwlan_idx_num;
	for(wlan_idx_num=0;wlan_idx_num<NUM_WLAN_INTERFACE;wlan_idx_num++)
	{
		memset(wlan_if_name,0x00,sizeof(wlan_if_name));
		memset(file_name,0x00,sizeof(file_name));
		sprintf(wlan_if_name,"wlan%d",wlan_idx_num);
		for(vwlan_idx_num=-1;vwlan_idx_num<NUM_VWLAN_INTERFACE;vwlan_idx_num++)
		{
			if(vwlan_idx_num==-1) //means root ap
				sprintf(file_name,"/var/run/auth-%s.pid",wlan_if_name);
#if defined(UNIVERSAL_REPEATER)
			else if(vwlan_idx_num==NUM_VWLAN_INTERFACE-1)
				sprintf(file_name,"/var/run/auth-%s-vxd.pid",wlan_if_name);
#endif
			else
				sprintf(file_name,"/var/run/auth-%s-va%d.pid",wlan_if_name,vwlan_idx_num);
			//printf("==%s(%d) file_name is %s\n",__FUNCTION__,__LINE__,file_name);
			if(isFileExist(file_name))
		    {
		        pid=getPid_fromFile(file_name);
		        if(pid != -1){
		            sprintf(strPID, "%d", pid);
		            RunSystemCmd(NULL_FILE, "kill", "-9", strPID, NULL_STR);
		        }
    ...
    apmib_get(MIB_WLAN_WLAN_DISABLED, (void *)&wlan1_disabled_root);
    apmib_get(MIB_WLAN_WSC_DISABLE, (void *)&wlan_wsc1_disabled);
    apmib_get( MIB_WLAN_MACAC_ENABLED, (void *)&wlan1_macac_enable_root);
#endif
    ...
    if(SetWlan_idx("wlan0")){
        ...
        apmib_get( MIB_WLAN_MACAC_ENABLED, (void *)&wlan_macac_enable_root);
			...
			if(strncmp(iface_name, "wlan", 4)==0){//wlan iface   
				...
				if(SetWlan_idx(iface_name)){
					...
					if (!strcmp(iface_name, "wlan0")) {
						...
						else if ((wlan_disabled == 0) && (wlan_encrypt == 4) && (wlan_wpa_auth == 1 || wlan_wpa_auth == 2))
#else
...
#ifdef RTK_MULTI_AP_R2
                        if ((wlan_disabled == 0) && (wlan_encrypt == 4) && (wlan_wpa_auth == 1 || wlan_wpa_auth == 2))
#else
...
//for 802.11k========================================================
#if defined(DOT11K) && !defined(RTK_MULTI_AP) && !defined(CONFIG_APP_MULTI_AP)
...
//for HS2
	...
	if (isFileExist("/bin/hs2")) {		
...
#else
		if (isFileExist("/etc/hs2_wlan0_map.conf") && (wlan0_hs2_enable == 1)) {
			//strcat(tmpBuff, "-c /tmp/hs2_wlan0_map.conf ");
			cmd_opt[cmd_cnt++] = "-c";
			cmd_opt[cmd_cnt++] = "/etc/hs2_wlan0_map.conf";
			//cmd_opt[cmd_cnt++] = "/etc/hs2_wlan0_map.conf";
		}
#endif
...
#else
		if (isFileExist("/etc/hs2_wlan1_map.conf") && (wlan1_hs2_enable == 1)) {
			//strcat(tmpBuff, "-c /tmp/hs2_wlan1_map.conf ");
			cmd_opt[cmd_cnt++] = "-c";
			cmd_opt[cmd_cnt++] = "/etc/hs2_wlan1_map.conf";
		}
#endif
		...
		wait_fifo=3;
		do{
			if(isFileExist(HS2_PID_FILE)){//check pid file is exist or not
				break;
            } 
			else {
				wait_fifo--;
        ...
        do{
            if (token == NULL){
                ...
                )
                {
                    ...
                    if(!strcmp(token, "wlan0") && ((wlan_wsc_disabled_root != 0) || (wlan_disabled_root != 0) || (wlan_mode_root == WDS_MODE) || (wlan_mode_root == MESH_MODE) || (wlan_macac_enable_root == 1))){
                        WSC=0;
                    }

#if defined(FOR_DUAL_BAND)		
                    else if(!strcmp(token, "wlan1") && ((wlan_wsc1_disabled != 0) || (wlan1_disabled_root != 0) || (wlan_mode_root == WDS_MODE) || (wlan_mode_root == MESH_MODE) || (wlan1_macac_enable_root == 1))){
                        WSC=0;
                    }
#endif
