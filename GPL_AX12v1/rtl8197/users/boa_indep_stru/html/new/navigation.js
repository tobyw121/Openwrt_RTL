                           ...
                           isDisplayCPU,isEnableBT,isDisplayTR069, disk_onoff, multi_ap_onoff)
{
  ...
  if(userDegreeFlag != 0){
	...
	if (disk_onoff){
     ...
     if(<%getIndex("sub_menu_wlan_support");%> != 0)
	 if(wlan_num==1){
		if(<%getIndex("band_5g_on_wlan0");%> ==1)
			add_topMenuItem(get_form("sub_menu_wlan.htm",0),menu_wlan1);
		else
...
function draw_subnav_wlan(is8021xClient, isWapiSupport, isWapiLocalAsSupport, isMeshDefined, is80211rSupport, isAirtimeDefined, isWDSDefined, userDegreeFlag)
{
	if(userDegreeFlag != 0){
		...
		if(is80211rSupport && <% getIndex("80211r_enable")%>)
			add_subMenuItem("wlft.htm", "Fast Roaming");
...
function draw_subnav_multi_ap(userDegreeFlag)
{
	if(userDegreeFlag != 0){
		...
		if ( 1 == <% getIndex("multi_ap_controller"); %>) {
			...
			if(<%getIndex("sub_menu_multiap_r2_support");%> == 1){
				add_subMenuItem("multi_ap_setting_vlan.htm","Vlan");
				add_subMenuItem("multi_ap_channel_scan.htm","Channel Scan");
			}
