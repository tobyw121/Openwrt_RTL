...
int flash_read_withtype(char *buf, int offset, int len,CONFIG_DATA_T srcSetting);
int flash_write_withtype(char *buf, int offset, int len,CONFIG_DATA_T srcSetting);
...
char *apmib_backupbluetoothHwconf(void)
{
...
#endif
	char bluetooth_hw_setting_start[6];
	if ( flash_read_withtype (bluetooth_hw_setting_start,BLUETOOTH_HW_SETTING_OFFSET, 6,BACKUP_BLUETOOTH_HW_SETTING)==0 ) {
	...
	if(bluetooth_compress_hw_setting){
		...
		flash_read_withtype((char *)&compHeader, BLUETOOTH_HW_SETTING_OFFSET, sizeof(COMPRESS_MIB_HEADER_T),BACKUP_BLUETOOTH_HW_SETTING);
		COMP_TRACE(stderr,"\r\n sig=%s __[%s-%u]",compHeader.signature,__FILE__,__LINE__);
		if(memcmp(compHeader.signature, COMP_BB_SIGNATURE, COMP_SIGNATURE_LEN) == 0 ) //check whether compress mib data
		{
			...
			if ( (compLen > 0) && (compLen <= BLUETOOTH_HW_SETTING_SECTOR_LEN) ) 
			{
				...
				flash_read_withtype ((char *)compFile, BLUETOOTH_HW_SETTING_OFFSET, compLen+sizeof(COMPRESS_MIB_HEADER_T),BACKUP_BLUETOOTH_HW_SETTING);
...
#endif //#ifdef COMPRESS_MIB_SETTING
{
	...
	if ( memcmp(blueToothHwHeader.signature, BACKUP_BLUETOOTH_HW_SETTING_HEADER_TAG, TAG_LEN) || // invalid signatur
		(ver != BLUETOOTH_HW_SETTING_VER) || // version not equal 
		(blueToothHwHeader.len < (sizeof(BLUETOOTH_HW_SETTING_T)+1)) ) { // length is less 
		printf("Invalid backup bluetooth hw setting signature or version number [sig=%c%c, ver=%d, len=%d]!\n",	blueToothHwHeader.signature[0], blueToothHwHeader.signature[1], ver, blueToothHwHeader.len);
...
char *apmib_backupcustomerHwconf(void)
{
...
#endif //#ifdef COMPRESS_MIB_SETTING
	{
		...
		(ver != CUSTOMER_HW_SETTING_VER) || // version not equal 
		(customerHwHeader.len < (sizeof(CUSTOMER_HW_SETTING_T)+1)) ) { // length is less 
			printf("Invalid backup customer setting signature or version number [sig=%c%c, ver=%d, len=%x]!\n",customerHwHeader.signature[0], customerHwHeader.signature[1], ver, customerHwHeader.len);
...
//mib_display_data_content(DEFAULT_SETTING, expFile+sizeof(PARAM_HEADER_T), dsHeader.len-1);
		...
		{
			COMP_TRACE(stderr,"\r\n ERR!Invalid checksum[%u] or mib_tlv_init() fail! __[%s-%u]\n",tlv_checksum,__FILE__,__LINE__);
			if(compFile != NULL)
			{
				free(compFile);
			}
...
//			apmib_get(MIB_L2TP_PAYLOAD_LENGTH, (void *)&payload_len);
			...
			if((id >= MIB_HW_2G_CCK_TSSI_A) && (id <= MIB_HW_5G_BW40_1S_TSSI_D)){
				...
				memcpy( (unsigned char *)value, (unsigned char *)(((long)pMibTbl) + pTbl[i].offset), max_chan_num);
			}else
#endif
	...
	// do nothing
...
#endif		
		{
			...
			if((id >= MIB_HW_2G_CCK_TSSI_A) && (id <= MIB_HW_2G_CCK_TSSI_D))
				...
				max_chan_num = MAX_5G_TSSI_BW40_SIZE_AX;
#endif
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8812FE)
...
//mib_display_tlv_content(type, data, tlv_content_len+1);
	...
	if(type & CURRENT_SETTING)
	{
		printf("resetBackupCurSetFromCurSet ! [File:%s Line:%d]\n",__FILE__,__LINE__);
	...
	{
	printf("resetBackupDefSetFromDefSet ! [File:%s Line:%d]\n",__FILE__,__LINE__);
...
int is_slope_k(int id)
{
	int ret = -1;
	if((id >= MIB_HW_5G_TSSI_GAIN_DIFF_A) && (id <= MIB_HW_5G_TSSI_CW_DIFF_D))
		return SLOPE_K_5G;
	else
		return ret;
}
