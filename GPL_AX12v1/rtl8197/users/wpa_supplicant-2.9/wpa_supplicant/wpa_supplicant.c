...
static int wpas_get_file_value(char *filename, int *value)
{
	FILE *fp = NULL;
	char buff[1024] = {0};
...
static void handle_debug(int sig, void *signal_ctx)
{
	wpas_get_file_value(FILE_WPAS_DEBUG, &wpa_debug_level);

	printf("wpa_debug_level:%d\n", wpa_debug_level);
...
enum wpa_states __wpa_supplicant_get_state(void *ctx) //realtek add
{
...
static int wapi_matching_ciphers(struct wpa_ssid *ssid, struct wpa_ie_data *ie)
{
	if ((!ie->has_group) || (!ie->has_pairwise))
		return 0;
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (bss_rsn && (ssid->proto & WPA_PROTO_RSN) &&
	    ...
	    wapi_matching_ciphers(ssid, &ie) &&
	    (ie.key_mgmt & ssid->key_mgmt)) {
		wpa_dbg(wpa_s, MSG_DEBUG, "WAPI: using WAPI");
	...
	wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_RSN_ENABLED,
			 !!(ssid->proto & (WPA_PROTO_RSN | WPA_PROTO_OSEN | WPA_PROTO_WAPI)));
#else
	wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_RSN_ENABLED,
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
		 || wpa_sm_set_ap_wapi_ie(wpa_s->wpa, bss_wapi,
					 bss_wapi ? 2 + bss_wapi[1] : 0)
#endif
			)
			return -1;
	}
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
		if (proto == WPA_PROTO_WAPI && bss_wapi) {
			wapi_ie_len = (2 + bss_wapi[1] > 255 ? 255 : 2 + bss_wapi[1]);
			memcpy(wpa_s->ap_wapi_ie, bss_wapi, wapi_ie_len);
	...
	if (0) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "WAPI: using KEY_MGMT WAPI_PSK");
	} else if (sel & WPA_KEY_MGMT_WAPI_CERT) {
		wpa_s->key_mgmt = WPA_KEY_MGMT_WAPI_CERT;
		wpa_dbg(wpa_s, MSG_DEBUG, "WAPI: using KEY_MGMT WAPI_CERT");
#endif
	} else {
	...
	if (proto == WPA_PROTO_WAPI) {
		wapi_ie_len = (*wpa_ie_len > 255 ? 255 : *wpa_ie_len);
		memcpy(wpa_s->assoc_wapi_ie, wpa_ie, wapi_ie_len);
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	} else if (bss && wpa_bss_get_ie(bss, WLAN_EID_WAPI) &&
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
 struct wpa_supplicant * rtl_get_wpa_supplicant(struct wpa_global *global,
						  const char *mac)
 {
	 struct wpa_supplicant *wpa_s;
 
	 for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
		 if (os_memcmp(mac, wpa_s->own_addr, ETH_ALEN) == 0)
			 return wpa_s;
	 }
	 return NULL;
 }
...
#ifdef WPAS_DYNAMIC_DEBUG
	eloop_register_signal(SIGUSR2, handle_debug, global);
#endif
