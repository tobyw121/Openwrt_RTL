					...
					struct wpa_ssid *ssid)
{
	...
	if ((ssid->key_mgmt & WPA_KEY_MGMT_WAPI_CERT) || 
		...
		wpa_printf(MSG_DEBUG, "%s: matched WAPI key_mgmt", __FUNCTION__);
		return 1;
	}
#endif
					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	while ((ssid->proto & WPA_PROTO_WAPI) && wapi_ie) {
		...
		if (!(ie.proto & ssid->proto)) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
					"   skip WAPI IE - proto mismatch");
			break;
		}

		if ((!ie.has_pairwise) || (!ie.has_group)) {
			wpa_printf(MSG_DEBUG, "%s: Invalid WAPI IE", __FUNCTION__);
			break;
		}
		...
		if (!(ie.pairwise_cipher & ssid->pairwise_cipher)) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
					"   skip WAPI IE - PTK cipher mismatch");
			break;
		}
		...
		if (!(ie.group_cipher & ssid->group_cipher)) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
					"   skip WAPI IE - GTK cipher mismatch");
			break;
		}
		...
		if (!(ie.key_mgmt & ssid->key_mgmt)) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
					"   skip WAPI IE - key mgmt mismatch");
			break;
		}

		if (debug_print)
			wpa_dbg(wpa_s, MSG_DEBUG,
				"   selected based on WAPI IE");
		return 1;		
	}
#endif
				     ...
				     int only_first_ssid, int debug_print)
{
	...
	wpa_printf(MSG_DEBUG, "%s: wapi_ie_len = %d", __FUNCTION__, wapi_ie_len);
#endif
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	wpa = wpa_ie_len > 0 || rsn_ie_len > 0 || wapi_ie_len;
#else
...
no_pfs:
	...
	while (p && l >= 2) {
		...
		if (!wapi_found &&
		    p[0] == WLAN_EID_WAPI && p[1] >= 2) {
			wapi_found = 1;
			wpa_sm_set_ap_wapi_ie(wpa_s->wpa, p, len);
		}
#endif
	...
	if (!wapi_found && data->assoc_info.beacon_ies)
		wpa_sm_set_ap_wapi_ie(wpa_s->wpa, NULL, 0);
#endif
				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->wpa_proto == WPA_PROTO_WAPI) {
		...
		if (wpa_s->wapi_init == 0) {
			if (rtl_wapi_init(wpa_s) < 0)
				return;
		}
		...
		u8 *req_wapi_ie = NULL;
		size_t req_wapi_ie_len = 0;

		if (data && data->assoc_info.req_ies) {
			req_wapi_ie = get_ie(data->assoc_info.req_ies, data->assoc_info.req_ies_len, WLAN_EID_WAPI);
			if (req_wapi_ie)
				req_wapi_ie_len = req_wapi_ie[1] + 2;
		}

		wpa_hexdump(MSG_DEBUG, "WAPI bssid", bssid, ETH_ALEN);
		wpa_hexdump(MSG_DEBUG, "WAPI own_addr", wpa_s->own_addr, ETH_ALEN);		
		
		rtl_wapi_process_assoc_event(bssid, wpa_s->own_addr, req_wapi_ie, req_wapi_ie_len);
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	if (wpa_s->wpa_proto == WPA_PROTO_WAPI && wpa_s->wapi_init) {	
		if (rtl_wapi_process_disassoc_event(bssid, wpa_s->own_addr) == 0) {
			wpa_s->wapi_init = 0;
