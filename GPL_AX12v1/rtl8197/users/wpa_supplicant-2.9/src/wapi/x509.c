...
void rtl_wapi_cert_free(cert_info **ppcert_infos)
{
	...
	if (cert_infos->asue_cert) {
		rtl_free_buffer(cert_infos->asue_cert, sizeof(cert_id));
		cert_infos->asue_cert = NULL;
	}
	if (cert_infos->asue_pubkey) {
		rtl_free_buffer(cert_infos->asue_pubkey, sizeof(tkey));
		cert_infos->asue_pubkey = NULL;
	}
	if (cert_infos->asue_prikey) {
		rtl_free_buffer(cert_infos->asue_prikey, sizeof(tkey));
		cert_infos->asue_prikey = NULL;
	}
	if (cert_infos->asue_id) {
		rtl_free_buffer(cert_infos->asue_id, sizeof(wai_fixdata_id));
		cert_infos->asue_id = NULL;
	}
...
int rtl_wapi_psk_init(struct wapi_asue_st *wapi_asue, KEY_TYPE kt, u32 kl, u8 *kv)
{	
	if (!wapi_asue)
		return -1;
	...
	wapi_asue->psk_infos = (psk_info *)rtl_allocate_buffer(sizeof(psk_info));
	if (!wapi_asue->psk_infos) {
	...
	psk_info *psk_infos = wapi_asue->psk_infos;
	psk_infos->kt = kt;
	psk_infos->kl = kl;
...
int rtl_wapi_cert_init(struct wapi_asue_st *wapi_asue, char *ca_cert_file, char *asu_cert_file, char *asue_cert_file)
{
	int len;
	cert_id *pcert = NULL;
	...
	if (!ca_cert_file && !asu_cert_file && !asue_cert_file)
		return -1;

	rtl_wapi_trace(MSG_DEBUG, "%s: Enter >>>>>>>>>>>>>>>>>", __FUNCTION__);

	if (wapi_asue->cert_infos)
		rtl_wapi_cert_free(&wapi_asue->cert_infos);

	wapi_asue->cert_infos = (cert_info *)rtl_allocate_buffer(sizeof(cert_info));
	if (!wapi_asue->cert_infos) {
		rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_allocate_buffer() fail.", __FUNCTION__, __LINE__);
		return -1;
	}
	...
	if (ca_cert_file) {	
		snprintf(wapi_asue->cert_infos->ca_cert_file, CERT_FILE_NAME_LEN, "%s", ca_cert_file);
		wapi_asue->cert_infos->ca_cert = (cert_id *)rtl_allocate_buffer(sizeof(cert_id));
		pcert = wapi_asue->cert_infos->ca_cert;		
		if (!pcert) {
			rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_allocate_buffer() fail.", __FUNCTION__, __LINE__);
			return -1;
		}
		...
		wapi_asue->cert_infos->ca_pubkey = rtl_wapi_cert_get_pubkey(pcert);
		if (!wapi_asue->cert_infos->ca_pubkey) {
			rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_wapi_cert_get_pubkey() fail.", __FUNCTION__, __LINE__);
			return -1;
		}

		//rtl_dump_buffer(wapi_asue->cert_infos->ca_pubkey->data, wapi_asue->cert_infos->ca_pubkey->length, "CA public key");
	}

	if (asu_cert_file) {
		snprintf(wapi_asue->cert_infos->asu_cert_file, CERT_FILE_NAME_LEN, "%s", asu_cert_file);
		wapi_asue->cert_infos->asu_cert = (cert_id *)rtl_allocate_buffer(sizeof(cert_id));
		pcert = wapi_asue->cert_infos->asu_cert;	
		if (!pcert) {
			rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_allocate_buffer() fail.", __FUNCTION__, __LINE__);
			return -1;
		}
		...
		if (strcmp(asu_cert_file, ca_cert_file)) {
			wapi_asue->cert_infos->asu_prikey = rtl_parse_private_key(pcert->data, pcert->length);
			if (!wapi_asue->cert_infos->asu_prikey) {
		...
		wapi_asue->cert_infos->asu_pubkey = rtl_wapi_cert_get_pubkey(pcert);
		if (!wapi_asue->cert_infos->asu_pubkey) {
			rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_wapi_cert_get_pubkey() fail.", __FUNCTION__, __LINE__);
			return -1;
		}

		wapi_asue->cert_infos->asu_id = (wai_fixdata_id *)rtl_allocate_buffer(sizeof(wai_fixdata_id));
		if (!wapi_asue->cert_infos->asu_id) {
			rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_allocate_buffer() fail.", __FUNCTION__, __LINE__);
			return -1;
		}
		rtl_wapi_fixdata_id(pcert, wapi_asue->cert_infos->asu_id);

		if (strcmp(asu_cert_file, ca_cert_file)) {
			if (rtl_wapi_cert_verify(wapi_asue->cert_infos->ca_pubkey, pcert) < 0) {
	...
	if (asue_cert_file) {
		snprintf(wapi_asue->cert_infos->asue_cert_file, CERT_FILE_NAME_LEN, "%s", asue_cert_file);
		wapi_asue->cert_infos->asue_cert = (cert_id *)rtl_allocate_buffer(sizeof(cert_id));
		pcert = wapi_asue->cert_infos->asue_cert;
		if (!pcert) {
			rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_allocate_buffer() fail.", __FUNCTION__, __LINE__);
			return -1;
		}

		len = rtl_load_cert_from_file(asue_cert_file, pcert->data, MAX_CERT_LEN);
		if (len <= 0) {
			rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_load_cert_from_file() fail.", __FUNCTION__, __LINE__);
			return -1;
		}
		...
		wapi_asue->cert_infos->asue_prikey = rtl_parse_private_key(pcert->data, pcert->length);
		if (!wapi_asue->cert_infos->asue_prikey) {
		...
		wapi_asue->cert_infos->asue_id = (wai_fixdata_id *)rtl_allocate_buffer(sizeof(wai_fixdata_id));
		if (!wapi_asue->cert_infos->asue_id) {
			rtl_wapi_trace(MSG_ERROR, "%s:%d Call rtl_allocate_buffer() fail.", __FUNCTION__, __LINE__);
			return -1;
		}
		rtl_wapi_fixdata_id(pcert, wapi_asue->cert_infos->asue_id);
