...
struct wapi_config {
	...
	u8 ae_wapi_ie[MAX_WAPI_IE_LEN];  //AE WAPI IE from Beacon/ProbeResp
	u8 ae_wapi_ie_len;

	u8 asue_wapi_ie[MAX_WAPI_IE_LEN]; //ASUE WAPI IE from (Re)AssocReq
	u8 asue_wapi_ie_len;	
	...
	union {
		...
		struct {
			...
			char asue_cert_file[CERT_FILE_NAME_LEN];			
		};
	} para;
};
