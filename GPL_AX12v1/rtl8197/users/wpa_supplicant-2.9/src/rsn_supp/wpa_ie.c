		     ...
		     struct wpa_ie_data *data)
{
	...
	if (wpa_ie_len >= 1 && wpa_ie[0] == WLAN_EID_WAPI)
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
int wpa_gen_wpa_ie_wapi(u8 *wapi_ie, size_t wapi_ie_len,
			      int pairwise_cipher, int group_cipher,
			      int key_mgmt, int mgmt_group_cipher)
{
	size_t ie_len;

	u8 buf_ie[64] = {0x44,22,1,0, 1,0,0,0x14,0x72,1, 1,0,0,0x14,0x72,1, 0,0x14,0x72,1, 0, 0, 0, 0};/* little endian*/

	if (key_mgmt == WPA_KEY_MGMT_WAPI_PSK)
		buf_ie[9] = 2;

	if (pairwise_cipher == WPA_CIPHER_GCM_SM4)
		buf_ie[15] = 2;
	...
	if(wapi_ie_len < ie_len) {
		wpa_printf(MSG_ERROR, "Invalid wapi ie len (%d).", ie_len);
		return -1;
	}

	os_memcpy(wapi_ie, buf_ie, ie_len);
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	if (sm->proto == WPA_PROTO_WAPI)
		return wpa_gen_wpa_ie_wapi(wpa_ie, wpa_ie_len,
					  sm->pairwise_cipher,
					  sm->group_cipher,
					  sm->key_mgmt, sm->mgmt_group_cipher);
#endif
