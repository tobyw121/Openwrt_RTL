...
static unsigned long server_addr, client_addr, relay_addr=0;
...
//#define SEND_ARP_TO_CHECK_SERVER_ALIVE   //default not enable
#endif
//brad add 20090314
...
#define NVRAM_STATIC_ROUTE_NAME "dhcp"
	...
	if (!(temp = get_option(packet, RFC_CLASSLESS_STATIC_ROUTE))) {
		...
		continue;
				...
				#if 1
				char exebuf[256];				
				snprintf(exebuf, sizeof(exebuf), "route add -net %u.%u.%u.%u/%d gw %u.%u.%u.%u metric 1",
						((unsigned char *)&des_ip)[0],((unsigned char *)&des_ip)[1],
						((unsigned char *)&des_ip)[2],((unsigned char *)&des_ip)[3],
						mask_num,
						((unsigned char *)&gw)[0],((unsigned char *)&gw)[1],
...
int check6rdOption(struct dhcpMessage *packet)
{
	int ret_val=-1,val=0, i=0;
	unsigned char *pOption6rd = NULL;
	unsigned char ipv4masklen=0,ipv6rdprefixlen=0,ipv6rdprefix[16]={0};
	unsigned long bripv4addr=0;
	unsigned char flag_offset=0, temp_len=0, data_len=0;
	char tempbuf[256]={0},oldbuf[256]={0};
	FILE *fp=NULL;

	pOption6rd = get_option( packet, DHCP_6RD);

	if(pOption6rd == NULL)
		return ret_val;
	
	data_len=*(pOption6rd-1);

	if(data_len < 22) /* rfc5969-7.1.1*/
		return ret_val;

	flag_offset = 0; /*ipv4masklen offset*/
	ipv4masklen = pOption6rd[flag_offset];
	if(ipv4masklen > 32 )
		return ret_val;

	flag_offset += 1; /*6rdprefix len offset*/
	ipv6rdprefixlen = pOption6rd[flag_offset];
	temp_len = 32 - ipv4masklen + ipv6rdprefixlen;
	if(temp_len > 128 || temp_len == 0)
		return ret_val;
		
	flag_offset += 1; /*6rdprefix offset*/
	for(i=0;i<16;i++)
		ipv6rdprefix[i] = pOption6rd[flag_offset+i];	

	flag_offset += 16; /*6rdbripv4addr offset*/
	if(flag_offset < data_len) /*just get one 6rdbripv4addr*/
	{
		((unsigned char *)&bripv4addr)[0] = pOption6rd[flag_offset+0];
		((unsigned char *)&bripv4addr)[1] = pOption6rd[flag_offset+1];
		((unsigned char *)&bripv4addr)[2] = pOption6rd[flag_offset+2];
		((unsigned char *)&bripv4addr)[3] = pOption6rd[flag_offset+3];
	}
	snprintf(tempbuf,sizeof(tempbuf), 
		"%d %d %02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x %u.%u.%u.%u",
		ipv4masklen,ipv6rdprefixlen,
		ipv6rdprefix[0],ipv6rdprefix[1],ipv6rdprefix[2],ipv6rdprefix[3],ipv6rdprefix[4],ipv6rdprefix[5],ipv6rdprefix[6],ipv6rdprefix[7],
		ipv6rdprefix[8],ipv6rdprefix[9],ipv6rdprefix[10],ipv6rdprefix[11],ipv6rdprefix[12],ipv6rdprefix[13],ipv6rdprefix[14],ipv6rdprefix[15],
		((unsigned char *)&bripv4addr)[0],((unsigned char *)&bripv4addr)[1],((unsigned char *)&bripv4addr)[2],((unsigned char *)&bripv4addr)[3]);

	//printf("\n%s:%d tempbuf=%s oldbuf=%s\n",__FUNCTION__,__LINE__,tempbuf,oldbuf);
	if(strncmp(tempbuf,oldbuf,sizeof(tempbuf))) /* changed */
	{
		fp=fopen("/var/dhcp6rd.conf","w+");
		if(fp)
		{
			fprintf(fp, "%s\n", tempbuf);
			fclose(fp);
		}
		strncpy(oldbuf,tempbuf,sizeof(tempbuf));
	}	
	ret_val = 0;
	return ret_val;
}
...
static unsigned int get_dhcpc_intf_pktnr(char *if_name, unsigned long long *rx_pkt, unsigned long long *tx_pkt)
{
	FILE *fp;
	unsigned long long rxb, rxnr, txb, txnr;
	char buf[512];
	int matched = 0;	
	...
	while (NULL != fgets(buf, sizeof(buf), fp)) 
	{
		...
		if (strcmp(tmp_ifname, if_name)==0) 
		{
			...
			//matched = sscanf(buf, "%*s %Lu %Lu %Lu", &rxb, &rxnr, &unknown);
			matched = sscanf(buf, "%*s %Lu %Lu %*Lu %*Lu %*Lu %*Lu %*Lu %*Lu %Lu %Lu", &rxb, &rxnr, &txb, &txnr);
...
unsigned long long last_xmit=0;

#define MAX_ARP_RETRY_NUM 5
...
{	
	...
	char *pchr=NULL;
	char phyIfname[16]={0};
	unsigned long long cur_received = 0, rcv_diff = 0, cur_xmit=0, tx_diff=0, max_diff=0;
	int i;
...
#ifndef SEND_ARP_TO_CHECK_SERVER_ALIVE

	return 1;

#else

	//if DUT get ip through dhcp relay, don't send arp
	if(relay_addr)
	{
		//printf("\n%s:%d ####relay_addr=%08x\n",__FUNCTION__,__LINE__,relay_addr);
		return 1;
	}
	...
	if(!strncmp(client_config.interface, "eth", 3))   
		get_dhcpc_intf_pktnr(phyIfname, &cur_received, &cur_xmit);
	else
		get_dhcpc_intf_pktnr(client_config.interface, &cur_received, &cur_xmit); 
	
	rcv_diff = cur_received - last_received;
	tx_diff = cur_xmit - last_xmit;
	last_received=cur_received;
	last_xmit=cur_xmit;

	max_diff=(rcv_diff>tx_diff ? rcv_diff : tx_diff);
	if(max_diff>1000)
	{				
		//printf("\n%s:%d cpu is busy! rcv_diff=%Lu tx_diff=%Lu max_diff=%Lu\n",__FUNCTION__,__LINE__,rcv_diff,tx_diff,max_diff);
		return 1;
	}			
	
	for(i=0;i<MAX_ARP_RETRY_NUM;i++)
	{
		if (arpping_unicast(dst_ip, src_ip, mac, dst_mac, client_config.interface, ret_hwaddr) == 0)
		{
			memcpy(dst_mac, ret_hwaddr, 6);
			return 1;
		}					
	}
	printf("\n%s:%d can't receive arp reply from server!\n",__FUNCTION__,__LINE__);
	return 0;
#endif
}
...
//#endif
	...
	memcpy(dst_mac, MAC_BCAST_ADDR, 6);
...
#define MAX_MSG_SIZE 576
	...
	for (;;) {
		...
		if (retval == 0) {
			...
			switch (state) {
			case INIT_SELECTING:
#if 1//def CONFIG_APP_SIMPLE_CONFIG
				if (packet_num < 30) {
#else
				if (packet_num < 3) {
#endif
...
#ifdef CONFIG_SIXRD_SUPPORT
					/* check 6rd option info */
					//check6rdOption(&packet);
#endif
#ifdef TR069_ANNEX_F
					/* handle option 125 */
					Option_VendorSpecInfo(&packet);
#endif
#if 1 //defined(SUPPORT_ZIONCOM_RUSSIA) || defined(_PRMT_X_TELEFONICA_ES_DHCPOPTION_)
