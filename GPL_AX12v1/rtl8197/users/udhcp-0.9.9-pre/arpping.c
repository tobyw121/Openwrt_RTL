...
int arpping_unicast(u_int32_t yiaddr, u_int32_t ip, unsigned char *src_mac, unsigned char *dst_mac, char *interface, char *ret_hwaddr)
{
	...
	memset(&arp, 0, sizeof(arp));
	//memcpy(arp.ethhdr.h_dest, MAC_BCAST_ADDR, 6);	/* MAC DA */
	memcpy(arp.ethhdr.h_dest, dst_mac, 6);	/* MAC DA */
