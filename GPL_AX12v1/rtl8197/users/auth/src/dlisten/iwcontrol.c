...
static int  wscd_wlan_num = 0;
static char wlan_tbl[MAX_WLAN_INTF][20];
static char wscd_wlan_tbl[MAX_WLAN_INTF][20];
...
int ProcessRequestEvent(char *wlan_name)
{
	...
	int iSend = FALSE, aSend=FALSE;	// iSend = 1 for test
	int iRet = 0;
	int i;
	...
	if(dlisten_SendBuf[5] != 0)
	{
		...
		{
		case	DOT11_EVENT_STA_ASSOCIATION_IND:
			dlisten_SendBuf[5] = DOT11_EVENT_ASSOCIATION_IND;
			sprintf(szEvent, (char*)"Receive Event %s", "STA_ASSOCIATION_IND");
            aSend = TRUE;
			break;
...
//			iSend = TRUE;
		...
		case	DOT11_EVENT_WSC_STOP:	// for P2P_SUPPORT
			...
			isWscdEvt = TRUE;
			break;			
        case    DOT11_EVENT_WSC_SWITCH_WLAN_MODE:   // for P2P_SUPPORT
            sprintf(szEvent, (char*)"Receive Event %s", "WSC_SWITCH_WLAN_MODE");
            isWscdEvt = TRUE;
            break;          
	...
	/* support  Assigned MAC Addr,Assigned SSID,dymanic change STA's PIN code, 2011-0505 */				
		...
		case	DOT11_EVENT_WSC_RM_PBC_STA:
			...
			isWscdEvt = TRUE;
			break;
		case	DOT11_EVENT_WSC_CHANGE_MAC_IND:
			sprintf(szEvent, (char*)"Receive Event %s", "WSC_MACADDR_CHANGED_IND");
			isWscdEvt = TRUE;
			break;
...
static void parse_wscd_wlan_table(char *d_name)
{
    char *token=NULL, *savestr1=NULL;
    int i=0, k=0;
    char tmpbuf[16];
    token = strtok_r(d_name, "-", &savestr1);
    do{
        for (k=0; k<16; k++) {
    		if (token[k] == '.')
    			token[k] = 0;
    	}
        if(strstr(token, "wlan")){
            strcpy(wscd_wlan_tbl[i++], token);
            wscd_wlan_num++;
        }else{
            sprintf(tmpbuf, "-%s", token);
            strcat(wscd_wlan_tbl[--i], tmpbuf);
            i++;
        }
...
//			auth[link_auth++].wlanName[5] = '\0' ;
		...
		if(!strncmp(next->d_name, "wscd", strlen("wscd"))){
            ...
            {
                parse_wscd_wlan_table(next->d_name);
                for(i=0; i<wscd_wlan_num; i++){
                    if(wscd_wlan_tbl[i][0]==0)
                        continue;
				
                    found = 0;
                    for(j=0; j<wlan_num; j++){
                        if(!strcmp(wscd_wlan_tbl[i], wlan_tbl[j])){
                            strcpy(wscd[link_wscd++].wlanName, wscd_wlan_tbl[i]);
                    ...
                    if(!found)
                        printf("/var/wscd-%s.fifo donot init! may cause error!\n", wscd_wlan_tbl[i]);
				}		
			}
			int idx;
