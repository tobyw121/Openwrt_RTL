...
#ifdef RTL_WPA2
extern int lib1x_authWPA2_parseIE(Dot1x_Authenticator * auth,
		Global_Params * global, u_char * pucIE, u_long ulIELength);
#endif
extern int lib1x_akmsm_GroupReKey_Timer_proc(Dot1x_Authenticator * auth);
...
TxRx_Params * lib1x_init_txrx(Dot1x_Authenticator * auth, u_char * oursvr_addr, u_char * svr_addr,
				...
				u_char * acctip_addr2, u_short udp_acctport2,
#endif
				u_char * dev_svr, u_char * dev_supp)
{
	...
	memset(dev_txrx, 0, sizeof(TxRx_Params));
	...
	//----endif
	{
		...
		strcpy( (char *)dev_txrx->device_svr, (char *)dev_svr );
...
#else
		strcpy( (char *)dev_txrx->device_svr, (char *)dev_supp );
#endif
		...
		dev_txrx->radsvraddr.sin_addr.s_addr = inet_addr( (char *)svrip_addr );
		...
		if ( inet_aton( (char *)ourip_addr, & dev_txrx->ourip_inaddr ) == 0 )
			lib1x_message(MESS_ERROR_FATAL, "Invalid OUR IP addr ");
		if (inet_aton( (char *)svrip_addr, & dev_txrx->svrip_inaddr ) == 0 )
			lib1x_message(MESS_ERROR_FATAL, "Invalid OUR IP addr ");
		...
		if(auth->AccountingEnabled)
		{
			...
			dev_txrx->acctsvraddr.sin_addr.s_addr = inet_addr( (char *)acctip_addr );
			if(lib1x_nal_connect( dev_txrx->network_svr,  & dev_txrx->acctsvraddr, sizeof(struct sockaddr_in ), LIB1X_IT_UDPSOCK_ACCT) < 0)
			{
#ifdef RTL_RADIUS_2SET
				if(auth->use_2nd_rad && lib1x_nal_connect( dev_txrx->network_svr2,  & dev_txrx->acctsvraddr, sizeof(struct sockaddr_in ), LIB1X_IT_UDPSOCK_ACCT) < 0)
#endif
				{
			...
			if (inet_aton( (char *)acctip_addr, & dev_txrx->acctip_inaddr ) == 0 )
				lib1x_message(MESS_ERROR_FATAL, "Invalid OUR IP addr ");
		...
		if (auth->use_2nd_rad) {
			...
			dev_txrx->radsvraddr2.sin_addr.s_addr = inet_addr( (char *)svrip_addr2 );
			...
			if (inet_aton( (char *)svrip_addr2, & dev_txrx->svrip_inaddr2 ) == 0 )
				lib1x_message(MESS_ERROR_FATAL, "Invalid OUR IP addr ");
			dev_txrx->udp_svrport2 = udp_svrport2;

			if(auth->AccountingEnabled)
			{
				dev_txrx->acctsvraddr2.sin_family = PF_INET;   // we need to 'connect' on udp to the server
				dev_txrx->acctsvraddr2.sin_port = htons( udp_acctport2 );
				dev_txrx->acctsvraddr2.sin_addr.s_addr = inet_addr( acctip_addr2 );
				if(lib1x_nal_connect( dev_txrx->network_svr2,  & dev_txrx->acctsvraddr2, sizeof(struct sockaddr_in ), LIB1X_IT_UDPSOCK_ACCT) < 0)
				{
					auth->AccountingEnabled = FALSE ;
				}

				if (inet_aton( acctip_addr2, & dev_txrx->acctip_inaddr2 ) == 0 )
					lib1x_message(MESS_ERROR_FATAL, "Invalid OUR IP addr ");
	...
	dev_txrx->device_supp = (u_char*)malloc(IFNAMSIZ + 10);
	memset( dev_txrx->device_supp, 0, sizeof(dev_txrx->device_supp));
    strcpy( (char *)dev_txrx->device_supp, (char *)dev_supp );
    dev_txrx->network_supp =  lib1x_nal_initialize( dev_txrx->device_supp, oursupp_addr, LIB1X_IT_PKTSOCK);

    //---------------------------------
	// wireless device initialization
	//---------------------------------
    dev_txrx->device_wlan0 = (char *)malloc(IFNAMSIZ + 10);
    memset( dev_txrx->device_wlan0, 0, sizeof(dev_txrx->device_wlan0));
	//sc_yang
	//strncpy( dev_txrx->device_wlan0, "wlan0", sizeof("wlan0") );
	strcpy( dev_txrx->device_wlan0, dev_txrx->device_supp);
...
#if defined(CONFIG_PMKCACHE)
	else
		auth->RSNVariable.max_pmksa = auth->pmksa_list.quota = 32;
#endif
#endif
	return TRUE;
...
Global_Params *  lib1x_init_authenticator(Dot1x_Authenticator *auth, TxRx_Params * dev_txrx)
{
	...
	strcpy( (char *)auth_pae->rinfo->nas_identifier, lib1x_nas_id); //sc_yang
	strcpy( (char *)auth_pae->rinfo->connectinfo, "CONNECT 11Mbps 802.11b");
...
#endif
#ifdef HOLD_AUTH_FAIL_SUPP_A_WHILE
	//printf("%s###Init auth->AuthFailSupp!\n",__FUNCTION__);
	memset(auth->AuthFailSupp, 0, MAX_HOLD_SUPP_NUM*sizeof(Dot1x_AuthFailSupp));
#endif
	return global;
}
...
//			bWPA2 = strncmp(wpa2IETag, pAssoInd->RSNIE, sizeof (wpa2IETag)) == 0? TRUE:FALSE;
			bWPA2 = (pAssoInd->RSNIE[0] == WPA2_ELEMENT_ID) && !strncmp((char *)wpa2IETag, (pAssoInd->RSNIE + 2), sizeof (wpa2IETag))? TRUE:FALSE;
            ...
            #endif
#endif
			if(usLength && ( (!strncmp((char *)wpaIETag, (pAssoInd->RSNIE + 2), sizeof (wpaIETag))) || bWPA2 ))

#else
			if(usLength && (!strncmp((char *)wpaIETag, pAssoInd->RSNIE, sizeof wpaIETag)) )
#endif
			//---- RSN Client ----
			{
				...
				{	
					...
					strcpy(ifr.ifr_name, (char *)dev_supp);
...
#ifdef HOLD_AUTH_FAIL_SUPP_A_WHILE
void record_auth_fail_supp(Dot1x_Authenticator * auth, unsigned char *mac_addr)
{
	if(mac_addr==NULL || memcmp( mac_addr, EMPTY_ADDR, ETHER_ADDRLEN )==0)
		return ;
	int i;
	for(i = 0 ; i < MAX_HOLD_SUPP_NUM ; i++)
	{
		if(auth->AuthFailSupp[i].holdTime>0)
			continue;
		
		memcpy(auth->AuthFailSupp[i].addr, mac_addr, ETHER_ADDRLEN);
		auth->AuthFailSupp[i].holdTime=FAILED_SUPP_HOLD_TIME;
		return;
	}
...
void lib1x_authsm_capture_svr( Global_Params * global, struct lib1x_nal_intfdesc * nal, struct lib1x_packet * pkt )
{
	...
	switch( rhdr->code  )
	{
		...
		case	LIB1X_RAD_ACCREJ:
			...
			#ifdef HOLD_AUTH_FAIL_SUPP_A_WHILE
						record_auth_fail_supp(global->auth, global->auth->Supp[global->index]->addr);
						//printf("%s###AUTHENTICATION FAILED !\n\n",__FUNCTION__);
						//printf("AUTH-Failure;note:%02x-%02x-%02x-%02x-%02x-%02x\n\n",
		//global->auth->Supp[global->index]->addr[0],global->auth->Supp[global->index]->addr[1],global->auth->Supp[global->index]->addr[2],
		//global->auth->Supp[global->index]->addr[3],global->auth->Supp[global->index]->addr[4],global->auth->Supp[global->index]->addr[5]);
			#endif
...
void lib1x_process_radiuschallenge( Global_Params * global, struct lib1x_packet * spkt)
{
	...
	if ( done )
	{
		...
		lib1x_message(MESS_DBG_RAD,"-------------   Done parsing RADIUS packet that is will be forwarded to Supp[i]", global->index);
...
void lib1x_auth_radius2_exchange(Global_Params * global)
{
	...
	struct lib1x_nal_intfdesc	*tmp_acct_svr;
	struct in_addr	tmp_acctip_inaddr;
	u_short			tmp_acctudp_svrport;
	struct sockaddr_in			tmp_acctsvraddr;
	OCTET_STRING	tmp_acctShared;
	...
	tmp_svrip_inaddr = global->TxRx->acctip_inaddr;
	global->TxRx->svrip_inaddr = global->TxRx->acctip_inaddr2;
	global->TxRx->acctip_inaddr2 = tmp_svrip_inaddr;

	tmp_acctudp_svrport = global->TxRx->udp_acctport;
	global->TxRx->udp_acctport = global->TxRx->udp_acctport2;
	global->TxRx->udp_acctport2 = tmp_acctudp_svrport;

	tmp_acctsvraddr = global->TxRx->acctsvraddr;
	global->TxRx->radsvraddr = global->TxRx->acctsvraddr2;
	global->TxRx->radsvraddr2 = tmp_acctsvraddr;

	tmp_acctShared.Length = global->auth->AcctShared2.Length;
	tmp_acctShared.Octet = global->auth->AcctShared2.Octet;
	global->auth->AcctShared2.Length = global->auth->AcctShared2.Length;
	global->auth->AcctShared2.Octet = global->auth->AcctShared2.Octet;
	global->auth->AcctShared2.Length = tmp_RadShared.Length;
	global->auth->AcctShared2.Octet = tmp_RadShared.Octet;
...
#ifdef HOLD_AUTH_FAIL_SUPP_A_WHILE
int check_is_hold_supp(Dot1x_Authenticator * auth, unsigned char *mac_addr)
{
	if(mac_addr==NULL)
		return ;

	printf("Supp Mac:%02x-%02x-%02x-%02x-%02x-%02x\n\n",
	...
	for(i = 0 ; i < MAX_HOLD_SUPP_NUM ; i++)
	{
		if(!memcmp( auth->AuthFailSupp[i].addr, mac_addr, ETHER_ADDRLEN ))
		{
			//printf("%s###The Supp is hold!\n",__FUNCTION__);
			return i;		
		}
	}
	//printf("%s###The Supp is not hold!\n",__FUNCTION__);
	return -1;
}
#endif
////////////////////////////////////////////////////////////////////////////////////////////////////
...
int	lib1x_search_supp(Dot1x_Authenticator * auth , struct lib1x_packet * spkt, u_char inttype)
{
	...
	if( inttype == LIB1X_IT_PKTSOCK)
	{
		...
		case 	LIB1X_EAPOL_KEY:
			#ifdef HOLD_AUTH_FAIL_SUPP_A_WHILE
			if(check_is_hold_supp(auth, eth->ether_shost)>=0)
				return -1;
			#endif
