...
#define RECONNECT_MSG(url) { \
    	...
    	getIncludeCss(wp);\
		req_format_write(wp, ("</head><body><blockquote><h4><script>dw(wlsurvey_change_setting_success)</script></h4>" \
...
//displayPostDate(wp->post_data);
	...
	/* STUN Connection */
	// stun enabled
	strData = req_get_cstream_var(wp, ("stunStatus"), (""));
	if ( strData[0] ) {
		unsigned char vChar=0;

		if( apmib_get( MIB_CWMP_STUN_EN, (void *)&vChar ) )
		{
			if(strData[0]=='0')
			{
				if( vChar==1 )
				{
					...
					changeflag = 1;
					if ( !apmib_set( MIB_CWMP_STUN_EN, (void *)&newVal)) {
						strcpy(tmpBuf, ("Set STUN to disable error!"));
						goto setErr_tr069;
					}
				}
			}else{
				if( vChar==0 )
				{
					...
					changeflag = 1;
					if ( !apmib_set( MIB_CWMP_STUN_EN, (void *)&newVal)) {
						strcpy(tmpBuf, ("Set STUN to enable error!"));
						goto setErr_tr069;
					}
				}
			}
		}else{
			strcpy(tmpBuf, ("get CWMP_STUN_EN error!"));
			goto setErr_tr069;
		}
	}
	
	// stun server url
	strData = req_get_cstream_var(wp, ("stunUrl"), (""));
	if ( strData[0] )
	{
		if ( strlen(strData)==0 )
		{
			strcpy(tmpBuf, "Set STUN server URL error!");
			goto setErr_tr069;
		}else{
			unsigned char oldURL[CWMP_STUN_SERVER_ADDR_LEN] = {0};
			if( apmib_get(MIB_CWMP_STUN_SERVER_ADDR, (void *)oldURL) )
			{
				if( strcmp(strData, oldURL)!=0 )
				{
					changeflag = 1;
					if( !apmib_set(MIB_CWMP_STUN_SERVER_ADDR, (void *)strData) )
					{
						strcpy(tmpBuf, "Set CWMP_STUN_SERVER_ADDR error!");
						goto setErr_tr069;
					}
				}
			}else{
				strcpy(tmpBuf, "Get CWMP_STUN_SERVER_ADDR error!");
				goto setErr_tr069;
			}
		}
	}
	...
	// stun server port 
	strData = req_get_cstream_var(wp, ("stunPort"), (""));
	if ( strData[0] ) {
		unsigned int newPort = atoi(strData);
		apmib_get( MIB_CWMP_STUN_SERVER_PORT, (void *)&vInt);
		if ( vInt != newPort ) {
			changeflag = 1;
			if ( !apmib_set( MIB_CWMP_STUN_SERVER_PORT, (void *)&newPort)) {
	...
	if ( strData[0] ) {
		...
		if( apmib_get( MIB_CWMP_FLAG, (void *)&cwmp_flag ) )
		{
			...
			if(strData[0]=='0') {
				...
				cwmp_flag = cwmp_flag | CWMP_FLAG_AUTORUN | CWMP_FLAG_SELFREBOOT;
				cwmp_flag_value = 1;
			}
...
// Magician: Commit immediately
	...
	if(req_get_cstream_var(wp, "save_apply", "")[0]==0){ 
		req_format_write(wp, "<html><head><meta http-equiv=\"refresh\" content=\"0;url=%s\"></head></html>", strData);
		return;
	}
...
#endif  // endif defined RTK_REINIT_SUPPORT
...
int save_cacert_to_flash(struct partition_file * cacert)
{
	int fh;
	int ret = 0;
	char mtdname[32] = {0};
	int len = cacert->len + sizeof(struct partition_file);
	
	rtl_name_to_mtdblock(CWMP_CACERT_MTD_NAME, mtdname);

	fh = open(mtdname, O_RDWR);
	if ( fh == -1 ){
		printf("open %s error,%s\n", mtdname, strerror(errno));
		return -1;
	}
	
	if (write(fh, cacert, len) != len){
...
void formTR069CACert(request *wp, char *path, char *query)
{
	...
	//printf("filesize is %d\n", nLen);
	cacert = calloc(1,nLen+1+sizeof(struct partition_file));
	if (!cacert)
	{
		strcpy(tmpBuf, (strMallocFail));
 		goto setErr_tr069ca;
 	}
	snprintf(cacert->payload, nLen+1, "%s", wp->upload_data+head_offset);
	
	fp_input= fopen(CA_FNAME,"w");
	if (!fp_input){
		snprintf(tmpBuf,sizeof(tmpBuf),"Create %s failed!",CA_FNAME);
		goto setErr_tr069ca;
	}
	fprintf(fp_input,"%s", cacert->payload);
	//printf("create file %s\n",CA_FNAME);
	
#if defined(MTD_NAME_MAPPING) && defined(CONFIG_RTL_CWMP_CACERT_PARTITION)
	cacert->tag = 0xabcddcba;
	cacert->len = nLen;
	save_cacert_to_flash(cacert);
#endif	
	free(cacert);
	fclose(fp_input);
...
//usually do not need to upload CPE cert file
	...
	nBytesSent += req_format_write(wp, ("\n"
		...
		"           <input type=\"submit\" value=\"Upload\" name=\"load\" onclick=\"return fileCheck()\">\n"
		"           </form>\n"
		"      </td>\n"
		"  </tr>\n"
		"\n"));
