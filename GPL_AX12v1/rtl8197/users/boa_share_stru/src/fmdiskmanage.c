...
void formDiskCreateShare(request *wp, char * path, char * query)
{
	...
	int	number,i,j, anon_access, samba_enabled;
...
#else
	sprintf(tmpBuff2,"reinitCli -e %d",REINIT_EVENT_APPLYCAHNGES);
	system(tmpBuff2);
#endif
...
void formDiskCfg(request *wp, char * path, char * query)
{
	...
	//	storage_UpdateSambaConf();
		...
		system("killall usbStorageAppController 2>/dev/null");
		if(apmib_get(MIB_SAMBA_ENABLED, (void *)&samba_enabled) && samba_enabled){
			sprintf(cmdBuffer,"usbStorageAppController -a %d &",mib_val);
			system(cmdBuffer);
		}	
#else
		sprintf(cmdBuffer,"reinitCli -e %d",REINIT_EVENT_APPLYCAHNGES);
		system(cmdBuffer);
#endif
	}else{
		...
		if(strDeleAll[0]){
			for(i = 0;i < user_num;i++)
			{
				...
				*((char*)s_users) = (char)(i+1);
				...
				snprintf(cmdBuffer,sizeof(cmdBuffer),"deluser \"%s\"",s_user.storage_user_name);
				system(cmdBuffer);
				bzero(cmdBuffer, sizeof(cmdBuffer));
				snprintf(cmdBuffer,sizeof(cmdBuffer),"rm -rf  /home/\"%s\"",s_user.storage_user_name);
				system(cmdBuffer);
				bzero(cmdBuffer, sizeof(cmdBuffer));
				snprintf(cmdBuffer,sizeof(cmdBuffer),"adduser \"%s\"",s_user.storage_user_name);
				system(cmdBuffer);
				//may be need modify
			...
			for(i = 0;i < number;i++)
			{
				...
				*((char*)&s_group) = (char)(i+1);
				...
				bzero(cmdBuffer, sizeof(cmdBuffer));
				snprintf(cmdBuffer,sizeof(cmdBuffer),"delgroup %s",s_group.storage_group_name);
				system(cmdBuffer);
...
//delete user
#ifndef CONFIG_APP_SAMBA_3_6_24
	snprintf(cmdBuffer,sizeof(cmdBuffer),"smbpasswd -del \"%s\"",s_user.storage_user_name);
#else
	snprintf(cmdBuffer,sizeof(cmdBuffer),"smbpasswd -x \"%s\"",s_user.storage_user_name);
#endif
	system(cmdBuffer);
	memset(cmdBuffer,'\0',sizeof(cmdBuffer));
//add user
#ifndef CONFIG_APP_SAMBA_3_6_24
	snprintf(cmdBuffer,sizeof(cmdBuffer),"smbpasswd \"%s\" \"%s\"",s_user.storage_user_name,s_user.storage_user_password);
#else
	snprintf(cmdBuffer,sizeof(cmdBuffer),"echo -e \"\"%s\"\\n\"%s\"\\n\" | smbpasswd -a \"%s\"",s_user.storage_user_password,s_user.storage_user_password,s_user.storage_user_name);
	fprintf(stderr,"samba-3.6.24");
#endif
	system(cmdBuffer);
