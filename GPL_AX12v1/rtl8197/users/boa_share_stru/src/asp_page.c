...
asp_name_t root_asp[] = {
	...
	{"ospfnetworkList", ospfnetworkList},
	{"ospfinterfaceList", ospfinterfaceList},
	{"ospfareaList", ospfareaList},
	{"ospflogList", ospflogList},
	
	{"bgpnetList", bgpnetList},
	{"bgppeerList", bgppeerList},
	{"bgplogList", bgplogList},
	{"isislogList", isislogList},
	{"netAuthenticationList", netAuthenticationList},
	{"isisinterfaceList", isisinterfaceList},
#endif
	...
	{"showChannelScanResults",showChannelScanResults},
#endif
	{NULL, NULL}
};
...
form_name_t root_form[] = {
...
#endif
#ifdef CONFIG_APP_RTL_QUAGGA
	{"formquaggaroute", formquaggaroute},
	{"formquaggadebug", formquaggadebug},
	{"formbgproute", formbgproute},
	{"formbgpdebug", formbgpdebug},
	{"formisisroute", formisisroute},
	{"formisisdebug", formisisdebug},
#endif
...
#endif //#if defined(WLAN_PROFILE)		
#endif	
#ifdef CONFIG_RTK_SYSCMD_SUPPORT
	...
	{"formBTRepeaterSetup",formBTRepeaterSetup},
#endif
	...
	{"formMultiAPVLAN",formMultiAPVLAN},
	{"formChannelScan", formChannelScan},
#endif
	{NULL, NULL}
};
...
unsigned int hash_IP_MAC(request *req)
{
	...
	//if(get_mac_from_IP(mac, req->remote_ip_addr))
	{
		...
		//sprintf(buf, "%02x%02x%02x%02x%02x%02x%08x", m1, m2, m3, m4, m5, m6, ip);
		sprintf(buf, "%s%08x", mac, ip);
	}
	else
	{
		/*no arp. from ppp interface eg. assign a special mac*/
		sprintf(buf,"%02x%02x%02x%02x%02x%02x%08x",0xfe,0xff,0xff,0xff,0xff,0xff,ip);
	}
...
int allocNewBuffer(request *req)
{
	...
	newBuffer = (char *)calloc(req->max_buffer_size*2+1,1);//init memory,make debug happy
	if (newBuffer == NULL)
		return FAILED;
...
{
	req_format_write(wp,"<meta http-equiv=\"Content-Type\" ");
	req_format_write(wp,"content=\"text/html; charset=utf-8\">");
	req_format_write(wp,"<script type=\"text/javascript\" ");
	req_format_write(wp,"src=\"/util_gw.js\"> </script>");
	char * argv[1]={0};
	argv[0]="lang";
	req_format_write(wp,"<SCRIPT language=Javascript src=/");
	getInfo(wp,1,argv);
	req_format_write(wp,"></SCRIPT>\n");
