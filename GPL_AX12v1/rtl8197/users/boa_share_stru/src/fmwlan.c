    ...
    {	
    	...
    	if ((strVal==NULL || strVal[0]==0)   // not called from wizard	
    		...
    		if (wlan_id == 0)
    			...
    			id = MIB_REPEATER_ENABLED2;
    		if (*mode == AP_MODE || *mode == AP_WDS_MODE || *mode == AP_MESH_MODE)
    		    apmib_set(id, (void *)&val);
...
//printf("\r\n ++++++++++++wlanifp=[%s],__[%s-%u]\r\n",wlanifp,__FILE__,__LINE__);
		...
		if (strSel[0]) {
            ...
            { 
		        ...
		        if(!setWlJoinMesh(WLAN_IF, pStatus->bssdb[idx].meshid, pStatus->bssdb[idx].meshidlen, pStatus->bssdb[idx].channel, offset))  
				{ 
                    ...
                    for(i = 0; i < 10; i++)	// This block might be removed when the mesh peerlink precedure has been completed
                    {
                        ...
                        { 							
							...
							memcpy(tmp, pStatus->bssdb[idx].meshid, pStatus->bssdb[idx].meshidlen); 
...
//#ifdef CONFIG_RTK_MESH
	...
	{ 
		...
		{ 
			...
			memcpy(meshidbuf, pBss->meshid, pBss->meshidlen);
...
//			"\"select\" value=\"sel%d\" onClick=\"enableConnect(%d, 0)\"></td></tr>\n"), i,ssidbuf,i,pBss->channel,i,tmp2Buf,i,wpa_tkip_aes,i,wpa2_tkip_aes ,i,i);			
			"\"select\" value=\"sel%d\" onClick=\"enableConnect(%d, 0, %d)\"></td></tr>\n"), i,tmp3Buf, i,tmpBuf,i,pBss->channel,i,tmp2Buf,i,wpa_tkip_aes,i,wpa2_tkip_aes ,i,i, pBss->t_stamp[2]);
		}
		else
			nBytesSent += req_format_write(wp, ("</tr>\n"));
	}
...
/* support  special SSID , 2011-0505 WPS2DOTX */
	...
	if (strVal[0]) {
		...
		system(tmpbuf);	
		OK_MSG2(STOP_MSG, ((mode==AP_MODE) ? "<script>dw(wlwps_client)</script>" : "AP"), submitUrl);
		return;
	}
/* support  special SSID , 2011-0505 WPS2DOTX */
