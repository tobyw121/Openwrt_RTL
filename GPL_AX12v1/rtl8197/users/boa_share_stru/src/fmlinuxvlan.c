...
void formVlan(request *wp, char *path, char *query)
{
	...
	if(strTmp[0])
	{
		...
		if(vlan_id==2 || vlan_id==DEFAULT_NAT_LAN_VID || vlan_id==DEFAULT_NAT_WAN_VID)
		{
			sprintf(errBuf, "<script>dw(vlan_8021q_vid2)</script> %d , %d <script>dw(vlan_8021q_vid_reserve)</script>",
