...
void set_dhcp6s()
{
	...
	if(isFileExist(DHCP6S_CONF_FILE) == 0) {
		...
		if (lanOrigin==0){ // lan auto mode, set dns auto
			...
			if(dnsforward==0)  // dns relay mode
			{
				if(get_linklocal_addr(&addr6_dns,dhcp6sCfgParam.interfaceNameds)>0)
				{
					...
					{
						snprintf(tmpStr,sizeof(tmpStr), "option domain-name-servers %s;\n", tmpBuf);
		...
		else { // lan manual mode, set dns manually
				snprintf(tmpStr,sizeof(tmpStr), "option domain-name-servers %s;\n", dhcp6sCfgParam.DNSaddr6);
				write(fh, tmpStr, strlen(tmpStr));
...
#ifdef	CONFIG_IPV6_DNSV6_MODE_SUPPORT
			//dns manual mode, add dns mib in resolv_v6.conf
			if(isFileExist(RESOLV_V6_CONF))
			{
				if(!checkDnsAddrIsExist(dns, RESOLV_V6_CONF))
					write_line_to_file(RESOLV_V6_CONF, 2, dns);
			}
			else
				write_line_to_file(RESOLV_V6_CONF, 1, dns);
...
//	system("echo fe80::287:42ff:fe16:9541 rlx.realsil >>/var/dnsmasq_hostfile");
	// when use dnsmasq, should kill dnrd.
...
void set_radvd()
{
	...
	if(val == 2) //if 6rd, radvd start with 6rd prefix delegation in manual
	{
		if(!apmib_get(MIB_SIXRD_MODE,&sixrdmode)){		
			fprintf(stderr, "Read mib %d error!\n", MIB_SIXRD_MODE);
			return ;			
		}
		if(sixrdmode==0) /*0 : 6RD manual, 1 : 6RD auto*/
			goto start_radvd;
		else 
			return;  /*6rd auto: wait for ipv4 wan connect, then start radvd*/
	}
...
#ifdef CONFIG_DSLITE_SUPPORT
		if(!apmib_get(MIB_DSLITE_ENABLE,&enable_dslite)){	
			fprintf(stderr, "get mib %d error!\n", MIB_WAN_DHCP);
			close(fh);
			return;
		}
...
void set_wanv6()
{
		...
		if(val == IPV6_LINKTYPE_PPP){
			system("echo 0 > /proc/sys/net/ipv6/conf/eth1/accept_ra");
			return;
		}
...
void set_dslite_static()
{
	...
	if(val != IPV6_ORIGIN_STATIC)  // set dslite static here.
		return;
...
char genAftrConf(char *strAftr)
{
	if(strAftr==NULL)
		return -1;
	char aftrAddr[INET6_ADDRSTRLEN]={0}, tmpStr[128]={0};
	memcpy(aftrAddr,strAftr,INET6_ADDRSTRLEN);
	FILE *fp=NULL;
	memset(tmpStr,0,sizeof(tmpStr));
	snprintf(tmpStr,sizeof(tmpStr),"/var/aftr_wan.conf");
	fp=fopen(tmpStr,"w+");
	if(fp)
	{
		fprintf(fp, "%s", aftrAddr);
		fclose(fp);
		return 1;
	}else
		return -1;
}
char get_aftr_addrinfo(char * aftr_name, char *aftr_addr)
{
	...
	ret = getaddrinfo(aftr_name, NULL, &hints, &res);
	if (ret != 0) {  
		printf("%s.%d.get aftr(%s) addrinfo fail: ret=%d error=%s\n",__FUNCTION__,__LINE__,aftr_name,ret,gai_strerror(res));
		return -1;
	}  
	for (cur = res; cur != NULL; cur = cur->ai_next) 
	{	
        sockaddr_ipv6 = (struct sockaddr_in6 *)( cur->ai_addr);   
        inet_ntop(AF_INET6, &sockaddr_ipv6->sin6_addr,aftr_addr,INET6_ADDRSTRLEN);   		
		printf("%s.%d. get aftr addr %s\n",__FUNCTION__,__LINE__,aftr_addr);
		genAftrConf(aftr_addr);
		freeaddrinfo(res);
		return 1;
	}  
	freeaddrinfo(res);
...
char getIpv6AddrByIfname(char *ifname, int addrtype, char *addr, unsigned char *plen)
{
	...
	if(fp!=NULL)
	{
		...
		{ 			
			...
			{									
				ret=sscanf(src,"%04hx%04hx%04hx%04hx%04hx%04hx%04hx%04hx",
				&addr6.s6_addr16[0], &addr6.s6_addr16[1], &addr6.s6_addr16[2], &addr6.s6_addr16[3],
				&addr6.s6_addr16[4], &addr6.s6_addr16[5], &addr6.s6_addr16[6], &addr6.s6_addr16[7]);
				for(i=0;i<8;i++)
					addr6.s6_addr16[i]=htons(addr6.s6_addr16[i]);
				switch(addrtype)
				{
					case IPV6_GLOBAL:						
						if(addr6.s6_addr16[0]!=htons(0xFE80)&& addr6.s6_addr16[0]!=0000)
						{
...
char getIPv6DefaultGw(char *ifname, char *addr)
{
	struct in6_addr	dst_addr={0},gw_addr={0},any_addr={0};
	uint32 flag=0, metrix=0, ref_cnt=0, usr_cnt=0;
	int dst_prefix=0, src_prefix=0,ret=0,i=0;
	char dst[64]={0},src[64]={0},gw[64]={0};
	FILE *fp=NULL;
	char devname[32]={0};
	...
	if(fp!=NULL){			
		...
		dst,&dst_prefix,src,&src_prefix,gw,&metrix,&ref_cnt,&usr_cnt,&flag,devname))!=EOF){	
			/*return value must be parameter's number*/ 						
			if(ret!=10)								
				continue;
			/*interface match?*/	
			if(strcmp(ifname,devname)==0){				
				sscanf(dst,"%04hx%04hx%04hx%04hx%04hx%04hx%04hx%04hx",
					&dst_addr.s6_addr16[0], &dst_addr.s6_addr16[1], &dst_addr.s6_addr16[2], &dst_addr.s6_addr16[3],
					&dst_addr.s6_addr16[4], &dst_addr.s6_addr16[5], &dst_addr.s6_addr16[6], &dst_addr.s6_addr16[7]);
				/*default route?*/
				if(memcmp(&dst_addr,&any_addr,sizeof(struct in6_addr))==0){
					sscanf(gw,"%04hx%04hx%04hx%04hx%04hx%04hx%04hx%04hx",
						&gw_addr.s6_addr16[0], &gw_addr.s6_addr16[1], &gw_addr.s6_addr16[2], &gw_addr.s6_addr16[3],
						&gw_addr.s6_addr16[4], &gw_addr.s6_addr16[5], &gw_addr.s6_addr16[6], &gw_addr.s6_addr16[7]);
					for(i=0;i<8;i++)
						gw_addr.s6_addr16[i]=htons(gw_addr.s6_addr16[i]);
						inet_ntop(AF_INET6, &gw_addr, addr,INET6_ADDRSTRLEN);
					break;
				}								
			}												
		}
...
void set_dslite_auto(char * wan_ifname, char *aftr_name)
{

	char local_addr6[INET6_ADDRSTRLEN]={0}, remote_addr6[INET6_ADDRSTRLEN]={0},gw_addr6[INET6_ADDRSTRLEN]={0};
	char cmdBuf[512]={0}, tmpStr[256]={0};
	int wan_plen=0;
	int val;
	...
	if(val == IPV6_ORIGIN_STATIC)  // set dslite dhcp(auto/manual) here.
		return;	
	...
	if(getIpv6AddrByIfname(wan_ifname,IPV6_GLOBAL,local_addr6,&wan_plen)<0)
	{
		printf("%s %d.get wan_addr fail\n",__FUNCTION__,__LINE__);
		return;
	}

	if(getIPv6DefaultGw(wan_ifname,gw_addr6)<0)
	{
		printf("%s %d.get wan_addr fail\n",__FUNCTION__,__LINE__);
		return;
	}

	printf("%s.%d. local_addr6=%s gw_addr6=%s aftr_name=%s\n",__FUNCTION__,__LINE__,local_addr6,gw_addr6,aftr_name);

	if(get_aftr_addrinfo(aftr_name,remote_addr6)<=0)
	{
		printf("%s %d.get aftr_addr fail\n",__FUNCTION__,__LINE__);
		return;		
	}
	...
	snprintf(tmpStr, sizeof(tmpStr),"#!/bin/sh\n\n");
	write(fh, tmpStr, strlen(tmpStr));
	
	snprintf(tmpStr, sizeof(tmpStr), "ip -6 tunnel del ds-lite 2> /dev/null\n");
	write(fh, tmpStr, strlen(tmpStr));
	
	snprintf(tmpStr, sizeof(tmpStr), "ip -6 tunnel add ds-lite mode ipip6 remote %s local %s dev %s\n",
		remote_addr6, local_addr6, wan_ifname);
	write(fh, tmpStr, strlen(tmpStr));
	
	snprintf(tmpStr, sizeof(tmpStr), "ip link set ds-lite up\n");
	write(fh, tmpStr, strlen(tmpStr));
	
	snprintf(tmpStr, sizeof(tmpStr), "ip addr add 192.0.0.2 peer 192.0.0.1 dev ds-lite\n");
	write(fh, tmpStr, strlen(tmpStr));

	snprintf(tmpStr, sizeof(tmpStr), "ip route add default via 192.0.0.1 dev ds-lite\n");
	write(fh, tmpStr, strlen(tmpStr));
		
 	snprintf(tmpStr, sizeof(tmpStr), "ip -6 route add %s/%d via %s dev %s",remote_addr6,128,gw_addr6,wan_ifname);
	write(fh, tmpStr, strlen(tmpStr));
...
#define CHAIN_NAME1(name) name##_##WAN
#define CHAIN_NAME_STR1(s)         #s
#define CHAIN_NAME_STR(s)         CHAIN_NAME_STR1(s)
#define DSLITE_INPUT			"DSLITE_INPUT"
#define DSLITE_FORWARD			"DSLITE_FORWARD"
#define CHAIN_NAME_LEN			32

int rtl_get_chain_name(char *prefix, char *name, int size)
{
	if (!prefix || !name)
		return -1;

	memset(name, '\0', size);
	snprintf(name, size, "%s", prefix );

	return 0;
}
void set_dslite_firewall_rules()
{

	char dev_name[12]={0},fileName[64] = {0};	
	int ret = -1;
	int dsliteEnable=0;
	int ipv6Enable=0;
	char dslite_input_chain[CHAIN_NAME_LEN], dslite_fwd_chain[CHAIN_NAME_LEN];

	if(!apmib_get(MIB_IPV6_WAN_ENABLE,&ipv6Enable)){		
		fprintf(stderr, "get mib MIB_IPV6_WAN_ENABLE error!\n");
		return ;			
	}
	...
	if(!apmib_get(MIB_DSLITE_ENABLE,&dsliteEnable)){	
		fprintf(stderr, "get mib MIB_DSLITE_ENABLE error!\n");
		return;
	}

	if(dsliteEnable ==0)
	{	printf("%s.%d. dslite not enable!\n",__FUNCTION__,__LINE__);
		return ;
	}

	/*init firewall chain ************************/
	//initFirewallChain(wan_p);
	snprintf(dev_name, sizeof(dev_name), "ds-lite");
	sprintf(fileName, "/var/system/ds-lite");
	ret = rtl_get_chain_name(CHAIN_NAME_STR(CHAIN_NAME1(DSLITE_INPUT)),  dslite_input_chain, sizeof(dslite_input_chain));
	if (ret != 0)
		return;
	ret = rtl_get_chain_name(CHAIN_NAME_STR(CHAIN_NAME1(DSLITE_FORWARD)),  dslite_fwd_chain, sizeof(dslite_fwd_chain));
	if (ret != 0)
		return;

	if(!isFileExist(fileName))
	{
		printf("%s.%d.init dslite firewall chain\n",__FUNCTION__,__LINE__);
		RunSystemCmd(fileName, "echo", "1", NULL_STR);
		/*create DSLITE user-defined chain	************************/
		RunSystemCmd(NULL_FILE, "iptables", "-N", dslite_input_chain,NULL_STR);
		RunSystemCmd(NULL_FILE, "iptables", "-N", dslite_fwd_chain,NULL_STR);
	}else
	{
		RunSystemCmd(NULL_FILE, "iptables", "-F", dslite_input_chain,NULL_STR);
		RunSystemCmd(NULL_FILE, "iptables", "-F", dslite_fwd_chain,NULL_STR);
		RunSystemCmd(NULL_FILE, "iptables", "-D", "INPUT","-j", dslite_input_chain, NULL_STR);		
		RunSystemCmd(NULL_FILE, "iptables", "-D", "FORWARD","-j", dslite_fwd_chain, NULL_STR);
	}
	/*jump rule for DSLITE user-defined chain ********************/
	RunSystemCmd(NULL_FILE, "iptables", "-I", "INPUT","-j", dslite_input_chain, NULL_STR);		
	RunSystemCmd(NULL_FILE, "iptables", "-I", "FORWARD","-j", dslite_fwd_chain, NULL_STR);	
	RunSystemCmd(NULL_FILE, "iptables", "-A", dslite_input_chain, "-i", dev_name, "-m", "state", "--state", "RELATED,ESTABLISHED", "-j", "ACCEPT", NULL_STR);
	RunSystemCmd(NULL_FILE, "iptables", "-A", dslite_fwd_chain, "-i", dev_name, "-m", "state", "--state", "RELATED,ESTABLISHED", "-j", "ACCEPT", NULL_STR);
	printf("%s.%d.set dslite rules done\n",__FUNCTION__,__LINE__);
...
//	printf("%s\n", tmpStr);
	...
	system("echo 2 > /proc/rtl_op_layer");
...
void set_wanipv6()
{
    ...
    {
        RunSystemCmd("/proc/sys/net/ipv6/conf/all/disable_ipv6", "echo", "1", NULL_STR);
        return;
    }
    else
    {
        RunSystemCmd("/proc/sys/net/ipv6/conf/all/disable_ipv6", "echo", "0", NULL_STR);
    }
