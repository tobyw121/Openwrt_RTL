...
void get_dhcp6_pd(struct dhcp6_pd_t* dhcp6_pd)
{
	FILE *fp=NULL;
	uint32 pltime=0;
	uint32 vltime=0;
	...
	sscanf(addr6,"%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx",
		...
		bzero(dhcp6_pd,sizeof(struct dhcp6_pd_t));
...
void restart_radvd(RA_INFO_ITEM_Tp rera_data)
{
	...
	if (flock(fh, LOCK_EX) != 0)
		printf("[%s %d]file lock by others\n",__FUNCTION__,__LINE__);
	
	printf("[%s %d]------------recreate radvd.conf----------------\n",__FUNCTION__,__LINE__);
	...
	if (lanOrigin==0) {  //lan auto mode, set prefix from PD
		...
		if(dhcp6_pd.flag){
			sprintf(tmpBuf,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
					...
					64);
			sprintf(tmpStr, "prefix %s\n", tmpBuf); write(fh, tmpStr, strlen(tmpStr));			
			sprintf(tmpStr, "{\n"); write(fh, tmpStr, strlen(tmpStr));
			sprintf(tmpStr, "	AdvOnLink on;\n");	write(fh, tmpStr, strlen(tmpStr));
			sprintf(tmpStr, "	AdvAutonomous on;\n");	write(fh, tmpStr, strlen(tmpStr));
			sprintf(tmpStr, "	AdvValidLifetime %u;\n", dhcp6_pd.vltime);  write(fh, tmpStr, strlen(tmpStr));
			sprintf(tmpStr, "	AdvPreferredLifetime %u;\n", dhcp6_pd.pltime);  write(fh, tmpStr, strlen(tmpStr));
			sprintf(tmpStr, "	AdvRouterAddr on;\n"); write(fh, tmpStr, strlen(tmpStr));
			sprintf(tmpStr, "};\n"); write(fh, tmpStr, strlen(tmpStr));
	...
	else  //Set DNS Auto
	{
		...
		if(dnsforward==1)   // dns forward mode
		{
			...
			{
				if((fp=fopen(DNSV6_RDNS_FILE,"r"))!=NULL)
				{	
				...
				printf("[%s %d]------------rdnss.conf not exist----------------\n",__FUNCTION__,__LINE__);
			}
...
void handler_radns(RA_INFO_ITEM_Tp recv_ra_data)
{
	...
	//if(recv_ra_data->dnsinfo.lifetime!=0)
	{
		...
		fprintf(fd,"%s","RDNSS");
		// DNS info get from  RA pkt.		
		for(i=0;i<(recv_ra_data->dnsinfo.length-1)/2;i++)
	    {
		   fprintf(fd, " %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
		   htons(recv_ra_data->dnsinfo.in6addr_dns[i].s6_addr16[0]),htons(recv_ra_data->dnsinfo.in6addr_dns[i].s6_addr16[1]),
		   htons(recv_ra_data->dnsinfo.in6addr_dns[i].s6_addr16[2]),htons(recv_ra_data->dnsinfo.in6addr_dns[i].s6_addr16[3]),
		   htons(recv_ra_data->dnsinfo.in6addr_dns[i].s6_addr16[4]),htons(recv_ra_data->dnsinfo.in6addr_dns[i].s6_addr16[5]),
		   htons(recv_ra_data->dnsinfo.in6addr_dns[i].s6_addr16[6]),htons(recv_ra_data->dnsinfo.in6addr_dns[i].s6_addr16[7]));
		}

		//DNS info get from dhcp6 server if M/O !=0/0
		if ((recv_ra_data->icmp6_managed !=0) ||(recv_ra_data->icmp6_other !=0))
		{
			if(isFileExist(DNSV6_ADDR_FILE))
			{
				if((fh=fopen(DNSV6_ADDR_FILE,"r"))!=NULL)
				{
					int copylen=0;
					memset(tmpStr, 0, sizeof(tmpStr));
					while(fgets(tmpBuf, sizeof(tmpBuf), fh))
					{
						...
						if(strstr(tmpBuf,"nameserver")!=NULL)
						{
							if(strstr(tmpBuf,"%")==NULL) {
	                            copylen = strlen(tmpBuf) - strlen("nameserver") -1;
...
void restart_dhcp6s(RA_INFO_ITEM_Tp recv_ra_data)
{
	...
	int lanOrigin=0;	
	int dhcpPdEnable=0;
	int dhcpMode=IPV6_DHCP_STATEFUL;
	radvdCfgParam_t radvdCfgParam;
	FILE *fp=NULL;

	//printf("%s.%d. restart_dhcp6s #######\n",__FUNCTION__,__LINE__);		
	...
	if ( !apmib_get(MIB_IPV6_DHCP_PD_ENABLE,(void *)&dhcpPdEnable)){
		printf("get MIB_IPV6_DHCP_PD_ENABLE failed\n");
		return;
	}
	...
	if (lanOrigin) // lan manual mode, no need to restart dhcp6s
		return;
        ...
        if (lanOrigin==0){ // lan auto mode
			...
			#ifdef CONFIG_IPV6_FORD_RA_PREFIX_SUPPORT
			else if (dhcpMode==IPV6_DHCP_STATELESS){
				    struct prefix_info *raPrefix=NULL;
				    raPrefix=&(recv_ra_data->prefixinfo);
					//use prefix 64 to set dhcp6s addr pool
					snprintf(dhcp6sCfgParam.addr6PoolS,sizeof(dhcp6sCfgParam.addr6PoolS),"%04x:%04x:%04x:%04x::16",
					htons(raPrefix->prefix.s6_addr16[0]),htons(raPrefix->prefix.s6_addr16[1]),
					htons(raPrefix->prefix.s6_addr16[2]),htons(raPrefix->prefix.s6_addr16[3]));
					snprintf(dhcp6sCfgParam.addr6PoolE,sizeof(dhcp6sCfgParam.addr6PoolE),"%04x:%04x:%04x:%04x::254",
					htons(raPrefix->prefix.s6_addr16[0]),htons(raPrefix->prefix.s6_addr16[1]),
					htons(raPrefix->prefix.s6_addr16[2]),htons(raPrefix->prefix.s6_addr16[3]));


					//  /bin/ifconfig br0 $ADDR1/$PREFIX1
					sprintf(tmpStr,"ip -6 address flush dev br0 scope global");
					system(tmpStr);
					
					snprintf(tmpStr,sizeof(tmpStr),"/bin/ifconfig br0 %04x:%04x:%04x:%04x::1/128",
					htons(raPrefix->prefix.s6_addr16[0]),htons(raPrefix->prefix.s6_addr16[1]),
					htons(raPrefix->prefix.s6_addr16[2]),htons(raPrefix->prefix.s6_addr16[3]));
					system(tmpStr);

					//add lan expand_prefix
					sprintf(tmpStr,"route -A inet6 add %04x:%04x:%04x:%04x::1/120 dev br0",
					htons(raPrefix->prefix.s6_addr16[0]),htons(raPrefix->prefix.s6_addr16[1]),
					htons(raPrefix->prefix.s6_addr16[2]),htons(raPrefix->prefix.s6_addr16[3]));
					system(tmpStr);
...
void update_dnsmode(RA_INFO_ITEM_Tp reso_data)
{
	...
	int dnsforward=0,dnsMode=1;
	managed=reso_data->icmp6_managed;
	others=reso_data->icmp6_other;
	apmib_get(MIB_IPV6_DNS_FORWARD, (void *)&dnsforward);
	printf("[%s.%d.] RA M/O=%d %d ,dnsforward=%d #######\n",__FUNCTION__,__LINE__,managed,others,dnsforward);
	if(0==dnsforward)   // dns relay mode
	{
		if(0==access("/var/dhcp6c_dnss_need_update",0)){
			printf("[%s.%d.] dhcp6c_dnss_need_update \n",__FUNCTION__,__LINE__);
			system("rm /var/dhcp6c_dnss_need_update");
		}
		if(0==access("/var/rdnssd_dnss_need_update",0)){
			printf("[%s.%d.] rdnssd_dnss_need_update \n",__FUNCTION__,__LINE__);
			system("rm /var/rdnssd_dnss_need_update");
		}
		if(0==access("/var/dhcp6c_dnsl_need_update",0)
#ifdef CONFIG_IPV6_FORD_RA_PREFIX_SUPPORT
		|| 0==access("/var/ra_prefix_need_update",0)
#endif
		)
		{
			restart_dhcp6s(reso_data);  //just forward dnsl by dhcpv6
			if(0==access("/var/dhcp6c_dnsl_need_update",0))	{
				printf("[%s.%d.] dhcp6c_dnsl_need_update \n",__FUNCTION__,__LINE__);
				system("rm /var/dhcp6c_dnsl_need_update");
			}
			#ifdef CONFIG_IPV6_FORD_RA_PREFIX_SUPPORT
			if(0==access("/var/ra_prefix_need_update",0))
				system("rm /var/ra_prefix_need_update");
			#endif
		}
		if(0==access("/var/rdnssd_dnsl_need_update",0))
		{
		    printf("[%s.%d.] rdnssd_dnsl_need_update \n",__FUNCTION__,__LINE__);
	...
	else //dns forward mode
	{
		...
		{
		    //printf("[%s.%d.] forward dhcp6c_ _need_update \n",__FUNCTION__,__LINE__);
			restart_dhcp6s(reso_data);
			if(0==access("/var/dhcp6c_dnss_need_update",0))
				system("rm /var/dhcp6c_dnss_need_update");
			if(0==access("/var/dhcp6c_dnsl_need_update",0))
				system("rm /var/dhcp6c_dnsl_need_update");
		...
		{
		    //printf("[%s.%d.] forward rdnssd _need_update \n",__FUNCTION__,__LINE__);
	...
	apmib_get(MIB_IPV6_DNS_AUTO, (void *)&dnsMode);
	if(dnsMode)	{//Set DNS Auto 
	// update resolv_v6.conf & resolv.conf
	fh = open(RESOLV_V6_CONF, O_RDWR|O_CREAT|O_TRUNC, S_IRWXO|S_IRWXG); // 
	if ((fh < 0) || (fh==NULL)) {
		fprintf(stderr, "[%s %d]Create %s file error,fh=%d!\n", __FUNCTION__,__LINE__,RESOLV_V6_CONF,fh);
		return;
	}
...
#ifdef CONFIG_IPV6_FORD_RA_PREFIX_SUPPORT
int ra_prefix_changed(RA_INFO_ITEM_Tp orig,RA_INFO_ITEM_Tp newOne)
{
	if(!orig || !newOne) return -1;
	if(memcmp(&orig->prefixinfo.prefix,&newOne->prefixinfo.prefix,sizeof(struct in6_addr))!=0
		|| orig->prefixinfo.prefix_len!= newOne->prefixinfo.prefix_len)
		return 1;
	return 0;
}
#endif
...
#ifdef CONFIG_IPV6_FORD_RA_PREFIX_SUPPORT
	if (ra_prefix_changed(&last_recv_data,&recv_data)==1)
	{
		//memcpy(&last_recv_data,&recv_data,sizeof(recv_data));
		// SLAAC+NOPD, LAN automode, expand lan prefix from RA prefix.
		int dhcpPdEnable=0;
		int dhcpMode=IPV6_DHCP_STATEFUL;
		if ( !apmib_get(MIB_IPV6_DHCP_MODE,(void *)&dhcpMode)){
			fprintf(stderr, "get mib %d error!\n", MIB_IPV6_DHCP_MODE);
		...
		if ((dhcpMode==IPV6_DHCP_STATELESS)&&(dhcpPdEnable==0))
		{
			//regen dhcp6 addr pool based on RA prefix, not from PD
			system("echo \"not update\">/var/ra_prefix_need_update");			
		}
		...
		//	printf("%s.%d.  RA last M/O=%d %d, now M/O=%d %d#######\n",__FUNCTION__,__LINE__,last_recv_data.icmp6_managed,
		//	last_recv_data.icmp6_other,recv_data.icmp6_managed,recv_data.icmp6_other);
			memcpy(&last_recv_data,&recv_data,sizeof(recv_data));
