...
OPTIMIZATION += $(CFLAG_-fstrict-overflow)
...
$(eval $(call cache-output-var,LDPIEFLAG,$(CC) -Wl$(comma)--help 2>/dev/null | grep -- -pie >& /dev/null && echo "-pie"))
...
ifndef ASNEEDED
export ASNEEDED:=$(shell $(CC) -Wl,--help 2>/dev/null | grep -- --as-needed >& /dev/null && echo "AS_NEEDED ( $(UCLIBC_LDSO) )" || echo "$(UCLIBC_LDSO)")
...
export UBACKTRACE_ASNEEDED:=$(shell $(CC) -Wl,--help 2>/dev/null | grep -- --as-needed >& /dev/null && \
...
export UARGP_ASNEEDED:=$(shell $(LD) --help 2>/dev/null | grep -- --as-needed >& /dev/null && echo "AS_NEEDED ( $(UARGP_FULL_NAME) )" || echo "$(UARGP_FULL_NAME)")
else
...
LDFLAGS += $(call qstrip,$(UCLIBC_EXTRA_CFLAGS))
LDFLAGS_NOSTRIP += $(call qstrip,$(UCLIBC_EXTRA_CFLAGS))
endif
ifneq ($(strip $(UCLIBC_EXTRA_LDFLAGS)),"")
LDFLAGS += $(call qstrip,$(UCLIBC_EXTRA_LDFLAGS))
LDFLAGS_NOSTRIP += $(call qstrip,$(UCLIBC_EXTRA_LDFLAGS))
endif
...
$(libc.depend): LIBGCC += $(if $(DODEBUG),$(LIBGCC_EH))
