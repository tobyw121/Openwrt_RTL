...
int rtl_isHwNatWan(struct sk_buff *skb)
{
	...
	if(!strncmp(skb->dev->name, RTL_DRV_PPP_NETIF_NAME, 4) && 
		strlen(skb->dev->name) == strlen(RTL_DRV_PPP_NETIF_NAME))
		return 1;
...
int32 rtl8198c_ipv6_router_add(struct rt6_info *rt)
{
	...
	if (tb6_id == RT6_TABLE_MAIN) {
		...
		if(fc_dst_len==128
			...
			&&rt&&rt->rt6i_idev && rt->rt6i_idev->dev && strncmp(rt->rt6i_idev->dev->name,"ppp",3)==0 
		...
		if ((((ipDst.v6_addr32[0]==0)&&(ipDst.v6_addr32[1]==0)&&(ipDst.v6_addr32[2]==0)&&(ipDst.v6_addr32[3]==0)) ||
		     ...
		     || isPppGloLo) {
			if( strncmp(rt->rt6i_idev->dev->name,"ppp",3)==0)
			{	
