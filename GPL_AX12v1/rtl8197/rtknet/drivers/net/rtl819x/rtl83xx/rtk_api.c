...
#if defined(CONFIG_RTL_8366SC_SUPPORT) || defined(CONFIG_RTL_8364NB_SUPPORT)
    rtk_port_rgmiiDelayExt_set(EXT_PORT_FOR_HOST, EXT1_RGMII_TX_DELAY, EXT1_RGMII_RX_DELAY);
#else
    rtk_port_rgmiiDelayExt_set(EXT_PORT_FOR_HOST, EXT0_RGMII_TX_DELAY, EXT0_RGMII_RX_DELAY);
#endif
...
#ifdef ENABLE_8367RB_RGMII2
    /* Set TX delay to 1 and RX delay to 3 */
...
int RTL83XX_vlan_set(void)
{
    ...
    for (i=0; i<5; i++)
    {
        rtk_l2_limitLearningCnt_set(i, 2112);
...
/*port, acl, dscp, 1q, 1ad, cvlan, da, sa*/
    ...
    if (buffer && !copy_from_user(tmp, buffer, count))
    {
        ...
        else if(!memcmp(tokptr, "max",3))
        {
            ...
            for(port=0; port<RTK_PORT_ID_MAX; port++)
            {
                /*Egress Port page number*/
                rtl8367c_setAsicReg(RTL8367C_REG_FLOWCTRL_DEBUG_CTRL0,port);
                rtl8367c_getAsicReg(RTL8367C_REG_FLOWCTRL_PORT_MAX_PAGE_COUNT,&page);

                for(qid=0; qid<=7; qid++)
                {
                    rtl8367c_getAsicReg(RTL8367C_REG_FLOWCTRL_QUEUE0_MAX_PAGE_COUNT+qid,&queue_page[qid]);
                }
        ...
        {
            printk("1p remap:\n");
            for(dot1q=0; dot1q<=RTL8367C_PRIMAX; dot1q++)
            {
        ...
        {
            printk("dscp remap:\n");
            for(dscp=0; dscp<=RTL8367C_DSCPMAX; dscp++)
            {
...
void rtl83xx_1p_remark_set(unsigned int remarkPort, unsigned int sysPri, unsigned int dot1pValue)
{
    ...
    rtk_qos_1pRemarkEnable_set(port, ENABLED);
...
void rtl83xx_flush_dscp_remark_setting(void)
{
    ...
    for(int_pri = 0; int_pri <= RTL8367C_PRIMAX; int_pri++)
    {
...
void rtl83xx_disable_dscp_remark_setting(void)
{
...
int rtl83xx_qosFlushPortBasePri(void)
{
    ...
    for(port=0; port<=RTK_PORT_ID_MAX; port++)
    {
        if(rtk_qos_portPri_set(port, 0) != RT_ERR_OK)
            ret = 0;
    }
    return ret;
}
...
int rtl83xx_qosFlush1pBasePri(void)
{
    rtk_pri_t dot1p_pri;
    int ret=1;
    for(dot1p_pri=0; dot1p_pri<=RTL8367C_PRIMAX; dot1p_pri++)
    {
...
int rtl83xx_qosRestorePriDecision(unsigned int flag)
{
    if(flag & (1<<PORT_BASE))
    {
        rtl8367c_setAsicPriorityDecision(PRIDEC_IDX0, PRIDEC_PORT, g_priorityDecision[PRIDEC_PORT]);
        rtl8367c_setAsicPriorityDecision(PRIDEC_IDX1, PRIDEC_PORT, g_priorityDecision[PRIDEC_PORT]);
    }

    if(flag & (1<<ACL_BASE))
    {
        rtl8367c_setAsicPriorityDecision(PRIDEC_IDX0, PRIDEC_ACL, g_priorityDecision[PRIDEC_ACL]);
        rtl8367c_setAsicPriorityDecision(PRIDEC_IDX1, PRIDEC_ACL, g_priorityDecision[PRIDEC_ACL]);
    }

    if(flag & (1<<DSCP_BASE))
    {
        rtl8367c_setAsicPriorityDecision(PRIDEC_IDX0, PRIDEC_DSCP, g_priorityDecision[PRIDEC_DSCP]);
        rtl8367c_setAsicPriorityDecision(PRIDEC_IDX1, PRIDEC_DSCP, g_priorityDecision[PRIDEC_DSCP]);
    }

    if(flag & (1<<D1P_BASE))
    {
        rtl8367c_setAsicPriorityDecision(PRIDEC_IDX0, PRIDEC_1Q, g_priorityDecision[PRIDEC_1Q]);
        rtl8367c_setAsicPriorityDecision(PRIDEC_IDX1, PRIDEC_1Q, g_priorityDecision[PRIDEC_1Q]);
    }
...
int rtl83xx_qosSetPriDecision(unsigned int priSrc, unsigned int decisionPri)
{
    ...
    if((retVal = rtl8367c_setAsicPriorityDecision(PRIDEC_IDX0, priSrc, decisionPri)) != RT_ERR_OK)
    {
        return retVal;
    }

    return rtl8367c_setAsicPriorityDecision(PRIDEC_IDX1, priSrc, decisionPri);
}

int rtl83xx_qosGetPriDecisionAll(unsigned int priDec[], unsigned int len)
{
    int retVal = RT_ERR_FAILED;
    unsigned int pri, intVal;

    if(priDec == NULL || len != PRIDEC_END)
        return retVal;

    for(pri = 0; pri < PRIDEC_END; pri++)
    {
        if ((retVal = rtl8367c_getAsicPriorityDecision(PRIDEC_IDX0, pri, &intVal)) != RT_ERR_OK)
            return retVal;
...
int rtl83xx_qosSetQueueType(unsigned int port, unsigned int queue_id, unsigned int weight)
{
    ...
    if(weight == 0)
    {
        retVal = rtl8367c_setAsicQueueType(port, queue_id, QTYPE_STRICT);
    }
    else
    {
        if ((retVal = rtl8367c_setAsicQueueType(port, queue_id, QTYPE_WFQ)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicWFQWeight(port, queue_id, weight)) != RT_ERR_OK)
            return retVal;
    }
...
int rtl83xx_qosGetQueueType(unsigned int port, unsigned int queue_id, unsigned int *weight)
{
    ...
    if((retVal = rtl8367c_getAsicQueueType(port, queue_id, &qtype)) != RT_ERR_OK)
        return retVal;
    ...
    {
        if ((retVal = rtl8367c_getAsicWFQWeight(port, queue_id, weight)) != RT_ERR_OK)
            return retVal;
    }
    else
    {
...
{
    return rtl8367c_setAsicPriorityToQIDMappingTable(queue_num-1, pri, qid);
}

int rtl83xx_qosGetPriToQid(unsigned int queue_num, unsigned int pri, unsigned int *qid)
{
    return rtl8367c_getAsicPriorityToQIDMappingTable(queue_num-1, pri, qid);
}
...
int rtl83xx_qosGetQueueRate(unsigned int port, unsigned int queue_id, unsigned int *bandwidth)
{
    int retVal = RT_ERR_FAILED, meterId;
    ...
    if((retVal = rtk_rate_egrQueueBwCtrlRate_get(logic_port, queue_id, &meterId)) != RT_ERR_OK)
        return retVal;

    if((retVal = rtk_rate_shareMeter_get(meterId, &type, bandwidth, &ifg_include)) != RT_ERR_OK)
        return retVal;
...
long rtl_83xx_restartPHYNway(unsigned long port)
{
    rtk_uint32 reg0 = 0;
#define RESTART_AUTONEGO   (1 << 9)
    ...
    rtl8367c_getAsicPHYReg(port, 0, &reg0 );

    /* enable 'restart Nway' bit */
    reg0 |= RESTART_AUTONEGO;

    /* write PHY reg 0 */
    rtl8367c_setAsicPHYReg(port, 0, reg0);
...
{
    rtk_uint32 iso_loation_leak_set;

    if(enable)
    {
        rtl8367c_getAsicReg(RTL8367C_REG_MIRROR_CTRL2,&iso_loation_leak_set);
        rtl8367c_setAsicReg(RTL8367C_REG_MIRROR_CTRL2,iso_loation_leak_set|0x1<<2|0x1<<3);
    }
    else
    {
        rtl8367c_getAsicReg(RTL8367C_REG_MIRROR_CTRL2,&iso_loation_leak_set);
