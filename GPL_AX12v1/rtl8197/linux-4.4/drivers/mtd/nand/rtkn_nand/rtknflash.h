...
typedef struct nand_chip_param
{
	...
	/* clk /pipe_lat*/
	unsigned int	maxclk;
	unsigned char 	pipe_lat;		//no used now
	/*io */
	bool			support_dio;
	bool			support_qio;
...
#define SPI_NAND_IO_TEST
#if 1//def SPI_NAND_IO_TEST
/* test type */
enum IO_READ_TYPE
{	
	READ_NORMAL = 0,
...
/* spi nand function */
#ifdef CONFIG_SPI_NAND_FLASH
unsigned int nand_spi_read_id(void);
void nand_spi_reset(void);
void nand_spi_chunk_read(void *dma_addr, unsigned int blk_pge_addr,unsigned int chunk_size,unsigned int oobsize);
void nand_spi_chunk_write(void *dma_addr, unsigned int blk_pge_addr,unsigned int chunk_size,unsigned int oobsize);
...
unsigned int nand_spi_get_spi_nand_ecc_status(void);
void nand_spi_enable_on_die_ecc(void);
void nand_spi_disable_on_die_ecc(void);
void nand_spi_set_buffer_mode(void);
void nand_spi_block_unprotect(void);
void nand_spi_default_quad_enable(void);
void nand_spi_winbond_quad_enable(void);
unsigned int nand_spi_get_feature_register(unsigned int feature_addr);
int nand_spi_get_on_die_ecc_status(void);
void nand_spi_set_read_latency(unsigned char val);
...
static inline int board_dieon_ecc_enable(void)
{
	if((REG32(0xb80007f0) & 0x7)==0x2)
		return 1;
	else if((REG32(0xb80007f0) & 0x7)==0x3)
		return 0;
	else{
		printk("other platform not support\n");
		BUG();	
	}
}
