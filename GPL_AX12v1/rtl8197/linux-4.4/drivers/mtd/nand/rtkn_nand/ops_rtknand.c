...
#else
	rtkn_ecc_write_page(mtd,chip,rtkn->tmpBuf,1,page);
#endif
	return 0;
}
...
#ifdef SPI_NAND_IO_TEST
static int rtk_read_proc_nandiotestinfo(struct seq_file *s, void *v)
{
	seq_printf(s,"%d\n", rtkn_io);
	return 0;	
}
...
static int rtk_nand_iotest_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char flag[20];
	...
	if (buffer && !copy_from_user(flag, buffer, 16)) {
		sscanf(flag,"%d %d",&read_type,&write_type);
		rtkn_io = rtkn_nand_iotest(read_type,write_type);
		return count;
	}
	else
		return -EFAULT;
...
#ifdef SPI_NAND_IO_TEST
int rtk_nand_iotest_read_open(struct inode *inode, struct file *file)
{
	return(single_open(file, rtk_read_proc_nandiotestinfo, NULL));
}

static ssize_t rtk_nand_iotest_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return rtk_nand_iotest_write_proc(file, userbuf,count, off);
}
#endif
...
#ifdef SPI_NAND_IO_TEST
static const struct file_operations rtk_nand_flash_iotest_ops = {
	.owner = THIS_MODULE,
	.open			= rtk_nand_iotest_read_open,
	.write			= rtk_nand_iotest_write,
	.read           = seq_read,
    .llseek         = seq_lseek,
    .release        = single_release,
};
...
#ifdef SPI_NAND_IO_TEST
	proc_create("flashtest", 0, NULL, &rtk_nand_flash_iotest_ops);
#endif
#endif
#endif
