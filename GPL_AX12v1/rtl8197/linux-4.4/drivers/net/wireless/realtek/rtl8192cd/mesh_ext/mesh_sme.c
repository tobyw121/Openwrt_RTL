...
void peer_expire(DRV_PRIV* priv)
{
    ...
    while(plist != phead)  	// 1.Check index  2.Check is it least element? (Because  next pointer to phead itself)
    {
        ...
        {
            MESH_DEBUG_MSG("MESH neighbor expire!!! %02X:%02X:%02X:%02X:%02X:%02X\n",
        ...
        else if(time_after(jiffies, pstat->mesh_neighbor_TBL.expire))
        {
            ...
            MESH_DEBUG_MSG("MESH send probe req to neighbor: %02X:%02X:%02X:%02X:%02X:%02X\n",
                           pstat->cmn_info.mac_addr[0],pstat->cmn_info.mac_addr[1],pstat->cmn_info.mac_addr[2],
                           pstat->cmn_info.mac_addr[3],pstat->cmn_info.mac_addr[4],pstat->cmn_info.mac_addr[5]);
        ...
        switch( pstat->mesh_neighbor_TBL.State)  	// ( D1.0, Page.116~120)
        {
	    	...
	    	case MP_CONFIRM_SENT:	// Timer was activity
				if (TRUE == deniedPeerLink) {
					/*update PEER ID avoid repeatly peer link status*/
					pstat->mesh_neighbor_TBL.PeerLinkID = recvLocalLinkID;
					return SUCCESS;	// no response
				}
				break;
            ...
            case MP_SUPERORDINATE_LINK_UP:
            case MP_SUBORDINATE_LINK_DOWN_E:
            case MP_SUPERORDINATE_LINK_DOWN: // Timer was activity
...
#endif
#if defined(CONFIG_RTL8672) && (!defined(CONFIG_TRIBAND_MESH))
