...
void CheckV6UDPandU2M(struct sk_buff *pskb)
{
	...
	iph = (struct ipv6hdr *)rtl_get_skb_ip_header(pskb, 0);
...
static char ___igmp_type_check(struct sk_buff *skb, unsigned char *gmac)
{
	...
	iph = rtl_get_skb_ip_header(skb, 0);
	hdrlen = iph->ihl << 2;
...
#ifdef A4_STA && !(defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
			if(priv->pmib->miscEntry.a4_enable) {
