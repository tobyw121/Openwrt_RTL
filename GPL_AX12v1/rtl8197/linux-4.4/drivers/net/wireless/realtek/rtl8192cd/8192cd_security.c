...
int DOT11_Process_Disconnect_Req(struct net_device *dev, struct iw_point *data)
{
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		ret = __DOT11_Process_Disconnect_Req(dev, data);
	}
#endif
		...
		DOT11_SET_KEY *pSetKey, unsigned char *pKey)
{
	...
	{
#if defined(HW_ENC_FOR_GROUP_CIPHER) && !defined(MBSSID) && !defined(UNIVERSAL_REPEATER)
...
#endif
#if defined(INCLUDE_WPA_PSK) || defined(WIFI_HAPD) || defined(RTK_NL80211)
					psk_indicate_evt(priv, DOT11_EVENT_DISASSOCIATION_IND, pstat_del->cmn_info.mac_addr, NULL, 0);
#endif

#ifdef RTK_NL80211
					event_indicate_cfg80211(priv, pstat_del->cmn_info.mac_addr, CFG80211_DEL_STA, NULL);
#endif
#ifdef WIFI_HAPD
					event_indicate_hapd(priv, pstat_del->cmn_info.mac_addr, HAPD_EXIRED, NULL);
#ifdef HAPD_DRV_PSK_WPS
					event_indicate(priv, pstat_del->cmn_info.mac_addr, 2);
#endif
#else
					event_indicate(priv, pstat_del->cmn_info.mac_addr, 2);
#endif
#ifdef STA_ASSOC_STATISTIC
					add_sta_assoc_status(priv, pstat_del->cmn_info.mac_addr, pstat_del->rssi, _RSON_MIC_FAILURE_ | 0x300);
#endif
...
int rtl8192cd_ioctl_priv_req(struct net_device *dev, struct iw_point *data)
{
	...
	switch(req.EventId)
	{
		...
		case DOT11_EVENT_DISCONNECT_REQ:
			if(!__DOT11_Process_Disconnect_Req(dev, (DOT11_DISCONNECT_REQ *)(data->pointer)))
			{}
			break;
