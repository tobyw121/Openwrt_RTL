...
struct halmac_api_arg api_arg_table[HALMAC_API_MAX] = {
		...
		{HALMAC_API_WRITE_LOGICAL_EFUSE_WORD, 2, {TLP_TYPE_PU32, TLP_TYPE_PU16} },
...
#endif
#if (HALMAC_VERSION(1, 6, 0) <= HALMAC_CURRENT_VERSION)
	case HALMAC_API_WRITE_LOGICAL_EFUSE_WORD:
		if (halmac_obj->version >= HALMAC_VERSION(1, 6, 0))
			state = halmac_obj->halmac_api_entry->halmac_write_logical_efuse_word(halmac_obj->halmac_adapter, *((u32 *)arg_list[0].ptr), *((u16 *)arg_list[1].ptr));
		break;
