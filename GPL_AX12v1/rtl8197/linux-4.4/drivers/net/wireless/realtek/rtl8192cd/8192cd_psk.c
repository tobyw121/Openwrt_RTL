...
#ifdef CONFIG_OWE
		/* TODO : Force OWE group suite set 4 */
		if (priv->owe_enble)
			dot11RSNGroupSuite.Type = _CCMP_PRIVACY_;
		else
			dot11RSNGroupSuite.Type = priv->wpa_global_info->MulticastCipher;
#else
		dot11RSNGroupSuite.Type = priv->wpa_global_info->MulticastCipher;
...
#ifdef CONFIG_OWE			
				if (priv->owe_enble) {
					pDot11RSNAuthSuite->dot11RSNIESuite[usSuitCount].Type = DOT11_AuthKeyType_OWE;
					log("18 only");
				} else {
					pDot11RSNAuthSuite->dot11RSNIESuite[usSuitCount].Type = DOT11_AuthKeyType_PSK_SHA256;
					log("6 only");
...
#endif
#ifdef CONFIG_OWE
		if (pDot11RSNIESuite->Type == DOT11_AuthKeyType_OWE) {
			log("DOT11_AuthKeyType_OWE");
		} else
#endif
		if (pDot11RSNIESuite->Type < DOT11_AuthKeyType_RSN ||
#ifdef CONFIG_IEEE80211W
...
#ifdef CONFIG_OWE
				&& pDot11RSNIESuite->Type != DOT11_AuthKeyType_OWE
#endif
		){
...
//	SMP_LOCK(flags);
	...
	// reset TXPN here for GTK rekey 2-1 RSC
	priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TXPN48.val48 = 1;
...
#ifdef CONFIG_OWE
	if (pStaInfo->AuthKeyMethod == DOT11_AuthKeyType_OWE){
...
#ifdef CONFIG_OWE
		if (priv->owe_enble)
			owe_calc_mic(EAPOLMsgSend, KeyDescriptorVer, pStaInfo->PTK, PTK_LEN_EAPOLMIC);
		else
...
#ifdef CONFIG_OWE
		if ((priv->owe_enble == TRUE) && (priv->owe_pmk_rdy == TRUE))
			memcpy(pstat->wpa_sta_info->PMK, priv->owe_pmk, PMK_LEN);
		else
...
#ifdef CONFIG_OWE
				if (priv->owe_enble == 1)
					if (owe_check_mic(priv, pStaInfo->EAPOLMsgRecvd, pStaInfo->PTK, PTK_LEN_EAPOLMIC))
						goto cont_msg;
#endif
...
//#ifndef RTL_WPA2
			...
			if (!CheckMIC(pStaInfo->EAPOLMsgRecvd, pStaInfo->PTK, PTK_LEN_EAPOLMIC)
				...
				&& (!owe_check_mic(priv, pStaInfo->EAPOLMsgRecvd, pStaInfo->PTK, PTK_LEN_EAPOLMIC))
#endif
			) { // errror
...
#ifdef CONFIG_OWE
		if (priv->owe_enble) {
			/* do not disable PSK_WPA2 for OWE */
			log("psk_enable has wpa2, owe\n");
		} else {
			/*AUTH_SAE_STA*/
