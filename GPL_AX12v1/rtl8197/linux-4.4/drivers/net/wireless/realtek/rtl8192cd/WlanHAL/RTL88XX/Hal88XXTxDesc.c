...
//}
    ...
    {
        elm_idx = hdrLen ? 2 : 1;
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[elm_idx].Dword0,
                    payloadLen, TXBD_DW0_TXBUFSIZE_MSK, TXBD_DW0_TXBUFSIZE_SH);

        if (payloadLen) {
            dma_addr = HAL_VIRT_TO_BUS1(Adapter, (PVOID)pdesc_data->pBuf, payloadLen, HAL_PCI_DMA_TODEVICE);
            SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[elm_idx].Dword1,
                   dma_addr + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                    TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
        }
#ifdef AP_SWPS_OFFLOAD
    ...
    if (pdesc_data->pIcv != NULL) {
        ...
        cur_txbd->TXBD_ELE[elm_idx].Dword0 = SET_DESC(0);
#ifdef AP_SWPS_OFFLOAD        
...
#endif // CFG_HAL_TX_AMSDU
    {
        ...
        // With CCI400 ,Do nothing for cache coherent code
			...
			if (payloadLen) {
                elm_idx = hdrLen ? 2 : 1;
                rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd->TXBD_ELE[elm_idx].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
		   		    (u4Byte)payloadLen,PCI_DMA_TODEVICE);
			...
			if (payloadLen) {
                elm_idx = hdrLen ? 2 : 1;
			    _dma_cache_wback(((GET_DESC_FIELD(cur_txbd->TXBD_ELE[elm_idx].Dword1, TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)|0x80000000) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
		   		    (u4Byte)payloadLen);
			} 
...
// TODO: consider both enable TX_AMSDU and HW_TX_SHORTCUT...
	...
	if (pdesc_data->aggreEn == FG_AGGRE_MSDU_LAST) {
        ...
        if (payloadLen) {
            elm_idx = hdrLen ? 2 : 1;
            HAL_CACHE_SYNC_WBACK(Adapter, 
                GET_DESC_FIELD(cur_txbd->TXBD_ELE[elm_idx].Dword1, TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL, 
                payloadLen, 
                HAL_PCI_DMA_TODEVICE);
        }
...
)
{
     ...
     PTX_DESC_DATA_88XX  pdesc_data = (PTX_DESC_DATA_88XX)pDescData;
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
     PH2D_TXDMA_INFO_DATA ph2d_txdma_info = pdesc_data->ph2d_txdma_info;
#endif
    if (pdesc_data->aggreEn <= FG_AGGRE_MSDU_FIRST)
#endif
    {
...
#if 0
			if (ph2d_txdma_info->dw1_macid == 1){
			panic_printk("%s %d pHdr 0x%x\n", __FUNCTION__, __LINE__,pdesc_data->pHdr);
			panic_printk("%s %d hdrLen 0x%x\n", __FUNCTION__, __LINE__,pdesc_data->hdrLen);
			panic_printk("%s %d pBuf 0x%x\n", __FUNCTION__, __LINE__,pdesc_data->pBuf);
			panic_printk("%s %d frLen 0x%x\n", __FUNCTION__, __LINE__,pdesc_data->frLen);
			panic_printk("%s %d ph2d_txdma_info: \n", __FUNCTION__, __LINE__);
			panic_printk("dw0_txpktsize 0x%x\n",ph2d_txdma_info->dw0_txpktsize);
			panic_printk("dw0_offset 0x%x\n",ph2d_txdma_info->dw0_offset);
			panic_printk("dw0_pkt_offset 0x%x\n",ph2d_txdma_info->dw0_pkt_offset);
			panic_printk("dw0_bk 0x%x\n",ph2d_txdma_info->dw0_bk);
			panic_printk("dw0_agg_en 0x%x\n",ph2d_txdma_info->dw0_agg_en);
			panic_printk("dw0_ie_end0x%x\n",ph2d_txdma_info->dw0_ie_end);
			panic_printk("\n");

			panic_printk("dw1_macid 0x%x\n",ph2d_txdma_info->dw1_macid);
			panic_printk("dw1_mc2u 0x%x\n",ph2d_txdma_info->dw1_mc2u);
			panic_printk("dw1_qsel 0x%x\n",ph2d_txdma_info->dw1_qsel);
			panic_printk("dw1_ext_edca 0x%x\n",ph2d_txdma_info->dw1_ext_edca);
			panic_printk("dw1_txbw 0x%x\n",ph2d_txdma_info->dw1_txbw);
			panic_printk("dw1_shm_cam 0x%x\n",ph2d_txdma_info->dw1_shm_cam);
			panic_printk("dw1_shm_en 0x%x\n",ph2d_txdma_info->dw1_shm_en);
			panic_printk("dw1_mhr_cp 0x%x\n",ph2d_txdma_info->dw1_mhr_cp);
			panic_printk("dw1_enc 0x%x\n",ph2d_txdma_info->dw1_enc);
			panic_printk("dw1_hw_aes_iv 0x%x\n",ph2d_txdma_info->dw1_hw_aes_iv);
			panic_printk("dw1_amsdu 0x%x\n",ph2d_txdma_info->dw1_amsdu);
			panic_printk("dw1_full_ofld 0x%x\n",ph2d_txdma_info->dw1_full_ofld);
			panic_printk("\n");


			panic_printk("dw2_h2d_type 0x%x\n",ph2d_txdma_info->dw2_h2d_type);
			panic_printk("dw2_if_id 0x%x\n",ph2d_txdma_info->dw2_if_id);
			panic_printk("dw2_h2d_tid 0x%x\n",ph2d_txdma_info->dw2_h2d_tid);
			panic_printk("dw2_last_mpdu 0x%x\n",ph2d_txdma_info->dw2_last_mpdu);
			panic_printk("dw2_mimops 0x%x\n",ph2d_txdma_info->dw2_mimops);
			panic_printk("dw2_dma_txagg_num 0x%x\n",ph2d_txdma_info->dw2_dma_txagg_num);
			panic_printk("dw2_max_amsdu_mode 0x%x\n",ph2d_txdma_info->dw2_max_amsdu_mode);
			panic_printk("dw2_dma_pri 0x%x\n",ph2d_txdma_info->dw2_dma_pri);
			panic_printk("dw2_chk_en 0x%x\n",ph2d_txdma_info->dw2_chk_en);
			panic_printk("\n");


			panic_printk("dw3_wheader_len 0x%x\n",ph2d_txdma_info->dw3_wheader_len);
			panic_printk("dw3_ie_cnt_en 0x%x\n",ph2d_txdma_info->dw3_ie_cnt_en);
			panic_printk("dw3_ie_cnt 0x%x\n",ph2d_txdma_info->dw3_ie_cnt);
			panic_printk("dw3_dma_channel 0x%x\n",ph2d_txdma_info->dw3_dma_channel);
			panic_printk("dw3_offload_size 0x%x\n",ph2d_txdma_info->dw3_offload_size);
			panic_printk(">>>>>>>>>>>>>>>>end<<<<<<<<<<<<<<<<<<<\n");
				}
#endif
