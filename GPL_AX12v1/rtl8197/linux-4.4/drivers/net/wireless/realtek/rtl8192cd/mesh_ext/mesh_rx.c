...
int relay_11s_dataframe_ucast(DRV_PRIV *priv, struct sk_buff *skb, int privacy, struct rx_frinfo *pfrinfo, DRV_PRIV *orig_priv)
{
    ...
    if(pEntry == NULL) // not have valid route path
    {
		...
		dst_pstat = get_stainfo(xmit_priv, nexthop);
	        ...
	        SAVE_INT_AND_CLI(flags);
	        SMP_LOCK_XMIT(flags);
	        __rtl8192cd_start_xmit_out(skb, dst_pstat, &tx_insn);
	        RESTORE_INT(flags);
...
int process_11s_datafrme(DRV_PRIV *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat)
{
	...
	unsigned char do_rc = 0, need_process = 1, is_amsdu = 0;
	...
	if ((priv->pmib->dot11BssType.net_work_type & WIRELESS_11N) && 
		...
		do_rc = 1;
		if (is_amsdu && IS_MCAST(GetAddr1Ptr(pframe))) {
			do_rc = 0;
		}
	}
	...
	if (do_rc) {
		*(unsigned int *)&(pfrinfo->pskb->cb[4]) = 0;
		...
		need_process = reorder_ctrl_check(priv, pstat, pfrinfo);
	}
