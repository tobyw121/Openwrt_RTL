	...
	for (i = 0; i < MAX_TRANS_LIST_NUM; i++) {
		...
		if(pstat) {
			...
			if (entry && entry->status == BTM_FAILED) {
				if ((priv->transition_list[i].mode & BIT(6)) == 0) {
...
#ifdef CONFIG_MBO
	if(pstat->mbo_cap) {
		memcpy(&pstat->mbo_reason_code, tmpbuf+17, 1);
	}
#endif
...
{	
	...
	unsigned char ports, report_count=0;
#endif
	...
	if(req_mode & (_WNM_PREFERRED_CANDIDATE_LIST_|_WNM_ABRIDGED_)) {
		...
		for(k = 0; k < MAX_NEIGHBOR_REPORT; k++) {
			...
			for(i = 0; i < pstat->non_preferred_channels_num; i++){
				if(priv->rm_neighbor_report[k].channel == pstat->non_preferred_channels[i]){
					non_preferred_ch = 1;
					break;
				}
			}
...
#ifdef RTK_MULTI_AP
			if (memcmp(BSSID, priv->rm_neighbor_report[k].bssid, MACADDRLEN) == 0) {
				self = 1;
			}
...
#ifdef CONFIG_MBO
		if(pstat->mbo_cap) {
			pbuf = construct_btm_req_mbo_ie(priv, pstat, pbuf, &frlen);
		}
#endif
#ifndef RTK_MULTI_AP
