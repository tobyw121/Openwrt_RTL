	...
	if ((e_llc->dsap == 0xaa) && (e_llc->ssap == 0xaa) && (e_llc->ctl == 0x03)) {
		if (!memcmp(e_snap->oui, oui_rfc1042, WLAN_IEEE_OUI_LEN)) {
			if(e_snap->type == __constant_htons( ETH_P_8021Q)) {
				unsigned short *vlan = (unsigned short *)((unsigned long)e_snap + sizeof(e_snap));
...
void txretry_handler(struct rtl8192cd_priv *priv, struct tx_retry *retry)
{
	...
	struct rtl8192cd_priv *priv_final = NULL;
	...
	for(macid_idx=0; macid_idx<2; macid_idx++)
	{
...
#ifdef MBSSID
		if(!pstat && GET_ROOT(priv)->pmib->miscEntry.vap_enable) {
			for (i=0; i<RTL8192CD_NUM_VWLAN; i++) {
				if (IS_DRV_OPEN(priv->pvap_priv[i])) {
					pstat = get_macidinfo(priv->pvap_priv[i], priv->pshare->sta_query_retry_macid[macid_idx]);
					if(pstat) {
						priv_final = priv->pvap_priv[i];
						break;
					}
				}
...
#ifdef UNIVERSAL_REPEATER
		if (!pstat && IS_DRV_OPEN(GET_VXD_PRIV(priv)))	{
			pstat = get_macidinfo(GET_VXD_PRIV(priv), priv->pshare->sta_query_retry_macid[macid_idx]);
			if(pstat) {
				priv_final = GET_VXD_PRIV(priv);
			}
		}
#endif
		...
		priv_final->ext_stats.tx_retrys += pstat->cur_tx_retry_cnt;
			...
			case CHANNEL_WIDTH_40:
				...
				if(pstat->bw_cnt[CHANNEL_WIDTH_20] == dybw_thrd) {
					*(unsigned int *)pstat->bw_cnt=0;
					...
					switch_20_stamap = orSTABitMap(&priv->switch_20_sta);
					setSTABitMap(&priv->switch_20_sta, REMAP_AID(pstat));
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
					if (switch_20_stamap == 0)
						dc_if_update(priv, 1);
#endif
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
									if (switch_20_stamap && orSTABitMap(&priv->switch_20_sta) == 0)
										dc_if_update(priv, 1);
#endif
...
#ifdef DTTP
/*3 degree thermal approximately ten degrees Celsius*/
void dttp(struct rtl8192cd_priv *priv){
	unsigned int curr_temp_avg = 0;
	char tmpbuf[16];
	curr_temp_avg = (priv->pmib->dot11RFEntry.currthermal[0]+priv->pmib->dot11RFEntry.currthermal[1])/2;
	#if 0
	if(priv->up_time%10 == 0)
		dttplog("curr ther[%d] ]",curr_temp_avg);
	#endif
	if(priv->pmib->dot11RFEntry.dttp_toggle==0){
		if(curr_temp_avg >= priv->pmib->dot11RFEntry.dttp_monitor_ther_up){
			dttplog("curr ther[%d] >= monitored Ther(up)[%d]",curr_temp_avg, priv->pmib->dot11RFEntry.dttp_monitor_ther_up);
			dttplog("decrease tssi target pwr [%d] dbm\n",priv->pmib->dot11RFEntry.dttp_dbm);
			priv->pmib->dot11RFEntry.dttp_toggle = 1;
			memset(tmpbuf,0,16);	
    	    sprintf((char *)tmpbuf, "all=-%d",priv->pmib->dot11RFEntry.dttp_dbm << 2);
			set_reg_pg(priv,tmpbuf);
		}
	}else if(priv->pmib->dot11RFEntry.dttp_toggle==1){
		if( curr_temp_avg <= priv->pmib->dot11RFEntry.dttp_monitor_ther_low){
			dttplog("curr ther[%d] , Monitored Ther(low)[%d]",curr_temp_avg, priv->pmib->dot11RFEntry.dttp_monitor_ther_low);
			dttplog("resume tssi target pwr [%d] dbm\n",priv->pmib->dot11RFEntry.dttp_dbm);
			priv->pmib->dot11RFEntry.dttp_toggle = 0;
			memset(tmpbuf,0,16);	
    	    sprintf((char *)tmpbuf, "reset");
			set_reg_pg(priv,tmpbuf);
		}
	}
}
