...
static enum halmac_ret_status
malloc_cfg_param_buf_88xx_v1(struct halmac_adapter *adapter, u8 full_fifo);

static enum halmac_ret_status
add_param_buf_88xx_v1(struct halmac_adapter *adapter,
		      struct halmac_phy_parameter_info *param, u8 *buf,
		      u8 *end_cmd);

static enum halmac_ret_status
cnv_cfg_param_state_88xx_v1(struct halmac_adapter *adapter,
			    enum halmac_cmd_construct_state dest_state);

static enum halmac_ret_status
gen_cfg_param_h2c_88xx_v1(struct halmac_adapter *adapter, u8 *buff);

static enum halmac_ret_status
send_cfg_param_h2c_88xx_v1(struct halmac_adapter *adapter);
...
get_real_pkt_id_88xx_v1(struct halmac_adapter *adapter,
			enum halmac_packet_id pkt_id);

static u8
packet_in_nlo_88xx_v1(struct halmac_adapter *adapter,
		      enum halmac_packet_id pkt_id);

static enum halmac_ret_status
send_h2c_update_packet_88xx_v1(struct halmac_adapter *adapter,
...
static enum halmac_ret_status
proc_ctrl_ch_switch_88xx_v1(struct halmac_adapter *adapter,
...
static enum halmac_ret_status
proc_p2pps_88xx_v1(struct halmac_adapter *adapter, struct halmac_p2pps *info);

static enum halmac_ret_status
parse_txdesc_ie1_88xx_v1(struct halmac_adapter *adapter, u8 *buf);
			...
			struct halmac_h2c_header_info *info, u16 *seq_num)
{
	...
	FW_OFFLOAD_H2C_SET_TOTAL_LEN(hdr, H2C_PKT_HDR_SIZE_88XX_V1 +
				     info->content_size);
			      ...
			      u32 size)
{
	...
	if (offset_accum != adapter->cfg_param_info.offset_accum ||
		...
		PLTFM_MSG_ERR("[ERR][C2H]offset_accu : %x, value_accu : %x\n",
 ...
 */
u8*
smart_malloc_88xx_v1(struct halmac_adapter *adapter, u32 size, u32 *new_size)
{
	u8 retry_num;
	u8 *malloc_buf = NULL;
...
malloc_cfg_param_buf_88xx_v1(struct halmac_adapter *adapter, u8 full_fifo)
{
	...
	info->buf = smart_malloc_88xx_v1(adapter, info->buf_size,
					 &info->buf_size);
	if (info->buf) {
...
send_cfg_param_h2c_88xx_v1(struct halmac_adapter *adapter)
{
	...
	if (cnv_cfg_param_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_H2C_SENT) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
	...
	*proc_status = HALMAC_CMD_PROCESS_SENDING;
	...
	status = dl_rsvd_page_88xx_v1(adapter, pg_addr, info->buf, info_size);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]dl rsvd pg!!\n");
		goto CFG_PARAM_H2C_FAIL;
	}

	gen_cfg_param_h2c_88xx_v1(adapter, h2c_buf);
	...
	if (status != HALMAC_RET_SUCCESS) {
		...
		reset_ofld_feature_88xx_v1(adapter, HALMAC_FEATURE_CFG_PARA);
	}
...
CFG_PARAM_H2C_FAIL:
	...
	if (cnv_cfg_param_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_IDLE) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
		       ...
		       struct halmac_phy_parameter_info *param, u8 full_fifo)
{
	...
	status = malloc_cfg_param_buf_88xx_v1(adapter, full_fifo);
	if (status != HALMAC_RET_SUCCESS)
		return status;

	if (cnv_cfg_param_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_CNSTR) !=
	    HALMAC_RET_SUCCESS) {
		PLTFM_FREE(info->buf, info->buf_size);
		info->buf = NULL;
		info->buf_wptr = NULL;
	...
	add_param_buf_88xx_v1(adapter, param, info->buf_wptr, &end_cmd);
	...
	status = send_cfg_param_h2c_88xx_v1(adapter);
	if (status != HALMAC_RET_SUCCESS) {
		if (info->buf) {
			PLTFM_FREE(info->buf, info->buf_size);
			info->buf = NULL;
			       ...
			       enum halmac_packet_id pkt_id, u8 *pkt, u32 size)
{
	...
	status = dl_rsvd_page_88xx_v1(adapter, pg_addr, pkt, size);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]dl rsvd pg!!\n");
		return status;
	}

	real_pkt_id = get_real_pkt_id_88xx_v1(adapter, pkt_id);
	...
	hdr_info.content_size = 4;
	if (packet_in_nlo_88xx_v1(adapter, pkt_id))
		hdr_info.ack = 0;
	else
		hdr_info.ack = 1;
	set_h2c_pkt_hdr_88xx_v1(adapter, h2c_buf, &hdr_info, &seq_num);
...
get_update_packet_page_size(struct halmac_adapter *adapter, u32 size)
{
	...
	return (total & 0x7f) > 0 ?
		(total >> TX_PAGE_SIZE_SHIFT_88XX_V1) + 1 :
		total >> TX_PAGE_SIZE_SHIFT_88XX_V1;
}
		      ...
		      enum halmac_packet_id pkt_id, u8 *pkt, u32 size)
{
	...
	*proc_status = HALMAC_CMD_PROCESS_SENDING;
	...
	status = send_h2c_update_packet_88xx_v1(adapter, pkt_id, pkt, size);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]send h2c!!\n");
...
static enum halmac_cmd_construct_state
scan_cmd_cnstr_state_88xx_v1(struct halmac_adapter *adapter)
{
	return adapter->halmac_state.scan_state.cmd_cnstr_state;
}

static enum halmac_ret_status
cnv_scan_state_88xx_v1(struct halmac_adapter *adapter,
		       enum halmac_cmd_construct_state dest_state)
{
	enum halmac_cmd_construct_state *state;
		    ...
		    struct halmac_ch_info *info)
{
	...
	state = scan_cmd_cnstr_state_88xx_v1(adapter);
	if (state != HALMAC_CMD_CNSTR_BUF_CLR &&
	    state != HALMAC_CMD_CNSTR_CNSTR) {
	...
	if (cnv_scan_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_CNSTR) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
			  ...
			  struct halmac_ch_extra_info *info)
{
	...
	if (scan_cmd_cnstr_state_88xx_v1(adapter) != HALMAC_CMD_CNSTR_CNSTR) {
			    ...
			    struct halmac_ch_switch_option *opt)
{
	...
	if (cnv_scan_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_H2C_SENT) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
	...
	*proc_status = HALMAC_CMD_PROCESS_SENDING;
	...
	if (opt->switch_en != 0) {
		...
		status = dl_rsvd_page_88xx_v1(adapter, pg_addr,
	...
	if (cnv_scan_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_IDLE) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
		       ...
		       struct halmac_ch_switch_option *info)
{
	...
	if (adapter->ch_sw_info.total_size +
	    ...
	    TX_PAGE_SIZE_SHIFT_88XX_V1) >
	    (u32)adapter->txff_alloc.rsvd_pg_num << TX_PAGE_SIZE_SHIFT_88XX_V1)
		return HALMAC_RET_RSVD_PG_OVERFLOW_FAIL;
	...
	status = proc_ctrl_ch_switch_88xx_v1(adapter, info);
	if (status != HALMAC_RET_SUCCESS) {
...
clear_ch_info_88xx_v1(struct halmac_adapter *adapter)
{
	...
	if (scan_cmd_cnstr_state_88xx_v1(adapter) ==
	    HALMAC_CMD_CNSTR_H2C_SENT) {
		PLTFM_MSG_WARN("[WARN]state(clear)\n");
		return HALMAC_RET_ERROR_STATE;
	}

	if (cnv_scan_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_BUF_CLR) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
...
chk_txdesc_88xx_v1(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	...
	if (!param.ie_offset || !param.ie_exist || !param.start_offset ||
	    !param.end_offset) {
		PLTFM_MSG_ERR("[ERR]memory allocation fail\n");
		status = HALMAC_RET_MALLOC_FAIL;
		goto OUT_FREE;
	}
	...
	*param.end_offset = adapter->hw_cfg_info.txdesc_body_size;
	...
	if (parse_txdesc_ie_88xx_v1(adapter, buf, &param) !=
	    HALMAC_RET_SUCCESS) {
	...
	if (*(param.ie_exist + 1) == 1) {
		...
		parse_txdesc_ie1_88xx_v1(adapter, buf + *(param.ie_offset + 1));
	}
...
OUT_FREE:
	...
	PLTFM_FREE(param.start_offset, 1);
	PLTFM_FREE(param.end_offset, 1);
...
p2pps_88xx_v1(struct halmac_adapter *adapter, struct halmac_p2pps *info)
{
	...
	status = proc_p2pps_88xx_v1(adapter, info);
	if (status != HALMAC_RET_SUCCESS) {
...
parse_txdesc_ie1_88xx_v1(struct halmac_adapter *adapter, u8 *buf)
{
	...
	/* check AMPDU_MAX_TIME is valid*/
	value = (u8)IE1_GET_TX_DESC_USE_MAX_TIME_EN(buf);
	if (value == 1) {
		value = (u8)IE1_GET_TX_DESC_AMPDU_MAX_TIME(buf);
		if (value > HALMAC_REG_R8(REG_AMPDU_MAX_TIME_V1))
			PLTFM_MSG_ERR("[ERR] wrong AMPDU_MAX_TIME in ie1\n");
	}
...
txfifo_is_empty_88xx_v1(struct halmac_adapter *adapter, u32 chk_num)
{
	...
	do {
		if ((HALMAC_REG_R32(REG_QUEUELIST_INFO_EMPTY) & 0x80FFFFFF) !=
		    0x80FFFFFF)
