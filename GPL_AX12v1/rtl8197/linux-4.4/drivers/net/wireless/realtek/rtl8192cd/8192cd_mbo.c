...
unsigned char *set_association_retry_delay_attr(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int *frlen)
{
	...
	pbuf = set_ie(pbuf, _ASSOCIATION_RETRY_DELAY_ATTR_ID_, 2, &temp, frlen);
	return pbuf;
}
...
unsigned char *construct_btm_req_mbo_ie(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pbuf, unsigned int *frlen)
{
	unsigned int length;
	unsigned char temp[4];
	...
	*frlen = *frlen + 6;
	...
	pbuf = set_ie(pbuf, _TRANSITION_REASON_CODE_ATTR_ID_, 1, &pstat->mbo_reason_code, frlen);
