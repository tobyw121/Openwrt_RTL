...
#if defined(CONFIG_RTL_ISP_EASY_MESH_SPECIAL_SUPPORT)
extern int lan_vlan_enable;
#endif
...
extern struct net_device *rtl_get_current_lan_dev(struct net *net);
#endif
#endif
...
/* Hotspot 2.0 Release 1 */
...
#ifdef RTK_MULTI_AP && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	...
	if(linux_vlan_enable){
		...
		//update br_port for br shorcut (only vlan dev in br,so need to use vlan dev to find br_port,otherwise wlan->lan br shortcut fail)
		if (*((uint16*)(pskb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021Q)) 
		{
			vid = ntohs(*(uint16*)(pskb->data+(ETH_ALEN<<1)+2)) & 0x0fff;
			if(vid)
			{				
	...
	#if defined(CONFIG_RTL_ISP_EASY_MESH_SPECIAL_SUPPORT)
	extern int multi_wan_process_rx(struct sk_buff **pskb);
	pskb->dev->priv_flags |=  IFF_DOMAIN_WLAN;
	if((multi_wan_process_rx((struct sk_buff **)(&pskb))) == FAILED)
	{
		//rtl_kfree_skb(priv, pskb, _SKB_RX_);
		return;
	}
	...
	if (extPortEnabled&&(rtl_wlan_frame_use_hw_nat(pskb->data)==SUCCESS)) {
		...
		#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
		if ((eth_dev = rtl_get_current_lan_dev(&init_net)) != NULL)
		#else
    ...
    if(
		#if defined(CONFIG_RTL_ISP_EASY_MESH_SPECIAL_SUPPORT)
		((OPMODE & WIFI_AP_STATE) || (OPMODE & WIFI_STATION_STATE))
		#else
	...
	#if defined(CONFIG_RTL_ISP_EASY_MESH_SPECIAL_SUPPORT)
		{
			extern int is_allow_pkt_to_br0(struct sk_buff *skb , int is_arp, int is_dhcp);
			extern int is_dhcp_packet(struct sk_buff *skb);
			extern unsigned int _br0_ip;
			int isArpReq = 0, offset = (ETH_ALEN << 1), dst_ip = 0;		
			int is_dhcp = 0,is_print = 0;
			{
			...
			is_dhcp = is_dhcp_packet(pskb);		

			if(is_allow_pkt_to_br0(pskb, isArpReq, is_dhcp) == 0 )
			{
				pskb->dev = priv->vdev;
    ...
    while (rest > WLAN_ETHHDR_LEN) {
        ...
        if (pnewskb) {
            ...
            if (OPMODE & WIFI_AP_STATE)
            {
                ...
                {
					if (IP_MCAST_MAC(pnewskb->data) && !IS_IGMP_PROTO(pnewskb->data))
					{
...
static void reorder_ctrl_pktout(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
	...
	unsigned char mesh_type = pskb->cb[3];
	unsigned char 	*pframe;
   ...
   *(unsigned int *)&(pskb->cb[4]) = 0;
	pfrinfo->pskb->cb[3] = 0;
#ifdef SUPPORT_TX_AMSDU_SHORTCUT
	pskb->cb[47] = 0; /* clear amsdu short-cut indicate */
#endif
    ...
    if (dst_pstat == 0) {
		...
		if (mesh_type == 2) {
			...
			if (__process_11s_datafrme(priv, pfrinfo, pstat, meshHdrPtr) == FAIL)
				rtl_kfree_skb(priv, pskb, _SKB_RX_);
		} else
#endif
        {
    ...
    else
    {
    	...
    	while (pskb)
		{
			...
			memcpy(&pskb->cb[10], dst_pstat->cmn_info.mac_addr, MACADDRLEN);
#endif
	            SMP_LOCK_XMIT(x); 
	            __rtl8192cd_start_xmit_out(pskb, dst_pstat, &txcfg);
	            SMP_UNLOCK_XMIT(x); 
	        }
	        else if(mesh_type == XMIT_11S) {
...
#ifdef A4_STA
#if defined(CONFIG_RTL_ISP_EASY_MESH_SPECIAL_SUPPORT) || (defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
					int vid = 0;
					unsigned char wlan_dev_name[IFNAMSIZ] = {0};
					struct net_device *wlan_dev = NULL;
...
static struct rx_frinfo *defrag_frame_main_offload(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat)
{
...
static struct rx_frinfo *defrag_frame_offload(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat)
{
...
#endif
#endif
	prx_frinfo = defrag_frame_main_offload(priv, pfrinfo, pstat);
...
int rtl8192cd_rx_dispatch_offload(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
...
#ifdef WDS
		if (pstat && pfrinfo->to_fr_ds == 3 && priv->pmib->dot11WdsInfo.wdsEnabled)
			pskb->dev = getWdsDevByAddr(priv, pstat->cmn_info.mac_addr);
#endif
#endif
	...
	/* reuse */
#ifdef CLIENT_MODE	
	if ((pfrinfo->to_fr_ds == WIFI_FRMCTL_FROM_DS) || (pfrinfo->to_fr_ds == WIFI_FRMCTL_MESH)) {
#ifdef A4_STA
		if (priv->pshare->rf_ft_var.a4_enable) {
							...
							(WIFI_STATION_STATE | WIFI_ASOC_STATE))
			{
				if (IS_BSSID(priv, pstat->cmn_info.mac_addr))
					if (a4_rx_check_reuse_offload(priv, pfrinfo, pstat, GET_MY_HWADDR))
						return FAIL;
			}		
		}	
#endif		
...
//TODO, power saving
	...
	/* defrag */
	if ((pfrinfo = defrag_frame_offload(priv, pfrinfo, pstat)) == NULL) 
		return SUCCESS;
    pskb = pfrinfo->pskb;
...
#ifdef A4_STA
               if((GET_BR_PORT(priv->dev) && priv->pmib->miscEntry.a4_enable == 2 && fdb_find_for_driver(GET_BR_PORT(priv->dev)->br, pfrinfo->da) == NULL) || (priv->pmib->miscEntry.a4_enable == 1 && IS_MCAST(pfrinfo->da))) {
					pnewskb = skb_copy(pskb, GFP_ATOMIC);
                    if (pnewskb) {
#ifdef GBWC
                        if (GBWC_forward_check(priv, pnewskb, pstat)) {
...
//		pstat = get_stainfo(priv,(unsigned char *)get_sa((unsigned char *)get_pframe(pfrinfo)));
		...
		//pstat = get_stainfo(priv, GetAddr2Ptr(pframe));
		#if 0
		if(pfrinfo->rxpkt_info){
	panic_printk(">>>>>>>%s %d pfrinfo->rxpkt_info->macid %d\n", __FUNCTION__, __LINE__,pfrinfo->rxpkt_info->macid);
	pstat = get_macidinfo(priv, pfrinfo->rxpkt_info->macid);
			}
		...
		if(pstat){
			...
			if((arp->ar_pro == __constant_htons(ETH_P_IP)) && (arp->ar_op == htons(ARPOP_REQUEST)||arp->ar_op == htons(ARPOP_REPLY)))
				panic_printk("%s %d aid %d arp->ar_op=%d\n", __FUNCTION__, __LINE__,pstat->cmn_info.aid,arp->ar_op);

		}else{
			panic_printk("%s %d aid NULL\n", __FUNCTION__, __LINE__);
		}
#endif
		memset(&rx_desc_status,0,sizeof(struct _RX_DESC_STATUS_88XX_));
