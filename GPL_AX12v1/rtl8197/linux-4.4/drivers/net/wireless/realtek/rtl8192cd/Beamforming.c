...
#ifdef CONFIG_WLAN_HAL_8814BE
	if(GET_CHIP_VER(priv) == VERSION_8814B) {
		Beamforming_NDPARate_8814B(priv, 0, 0, Rate);
	}		
#endif
...
#ifdef CONFIG_WLAN_HAL_8814BE
        if (GET_CHIP_VER(priv) == VERSION_8814B) {
            pSTA->cmn_info.bf_info.g_id = 63;
            pSTA->cmn_info.bf_info.p_aid = pEntry->P_AID;
        }
#endif		
	...
	)
{
    ...
    if (pBFeeEntry != NULL && (CurDelBFerBFeeEntrySel & BFeeEntry)) // ((CurDelBFerBFeeEntrySel == BFeeEntry) || (CurDelBFerBFeeEntrySel == BFerBFeeEntry))
    {
		...
		pBFeeEntry->pSTA->isMUCandidate= 0;
...
//	pBeamTimerInfo->Idx = Idx;
	...
	{
		odm_set_timer(ODMPTR, &pBeamInfo->BeamformingTimer, pBeamTimerInfo->BeamPeriod);
	} 
	else
	{
	...
	)
{
	...
	if(pBeamTimerInfo->Mode == SOUNDING_SW_VHT_TIMER || pBeamTimerInfo->Mode == SOUNDING_SW_HT_TIMER) 
	{
		odm_cancel_timer(ODMPTR, &pBeamInfo->BeamformingTimer);

		if((GET_CHIP_VER(priv) == VERSION_8822B) || GET_CHIP_VER(priv) == VERSION_8812F){
		odm_cancel_timer(ODMPTR, &pBeamInfo->BFSoundingTimeoutTimer);
			...
			if (pBeamformingTimerInfo->Mode == SOUNDING_SW_VHT_TIMER || pBeamformingTimerInfo->Mode == SOUNDING_SW_HT_TIMER) {
				if (shortenSoundingPeriod(priv)) {
					if (pBeamformingInfo->BeamformingPeriodState == 1 || pBeamformingInfo->BeamformingPeriodState == 2)
						odm_set_timer(ODMPTR, &pBeamformingInfo->BeamformingTimer, pBeamformingTimerInfo->BeamPeriod/100);
					else   // pBeamformingInfo->BeamformingPeriodState == 3
						odm_set_timer(ODMPTR, &pBeamformingInfo->BeamformingTimer, pBeamformingTimerInfo->BeamPeriod/200);
				}
				else
					odm_set_timer(ODMPTR, &pBeamformingInfo->BeamformingTimer, pBeamformingTimerInfo->BeamPeriod);
	...
	)
{
	...
	{	
		...
		pBeamInfo->BeamformingTimer.data = (unsigned long) &pBeamInfo->BeamformingTimer_event;
		pBeamInfo->BeamformingTimer.function = timer_event_timer_fn;
		INIT_TIMER_EVENT_ENTRY(&pBeamInfo->BeamformingTimer_event, Beamforming_SWTimerCallback, (unsigned long)priv);
#endif
	}
	...
	)
{
	...
	odm_cancel_timer(ODMPTR, &pBeamformingInfo->BeamformingTimer);
	if((GET_CHIP_VER(priv)== VERSION_8822B) || (GET_CHIP_VER(priv)== VERSION_8812F) || (GET_CHIP_VER(priv)== VERSION_8814B))
		odm_cancel_timer(ODMPTR, &pBeamformingInfo->BFSoundingTimeoutTimer);
		...
		if((GET_CHIP_VER(priv)== VERSION_8822B) || (GET_CHIP_VER(priv)== VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)) {
			...
			//if(pBeamInfo->BeamformCap & (BEAMFORMER_CAP_HT_EXPLICIT|BEAMFORMER_CAP_VHT_SU|BEAMFORM_CAP_VHT_MU_BFER))
			{
				if(pBeamInfo->SoundingInfoV2.State == SOUNDING_STATE_NONE)
				{
					...
					odm_set_timer(ODMPTR, &pBeamInfo->BeamformingTimer, 2000); // Start sounding after 2 sec.
				}
			}
		}
	}	
	...
	*pLength = 17;
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {		
		...
		if (pEntry->bUsed && pEntry->is_mu_sta && pEntry->pSTA && pEntry->pSTA->isSendNDPA) {
			...
			STAInfo.NcIndex = pEntry->CompSteeringNumofBFer;
			tmp16 = 0;
    ...
    )
{
	...
	do
	{
		if(pSoundingInfo->State == SOUNDING_STATE_INIT)
		{
			...
			{			
				...
				if(WaitCnt < 500)
				{
					...
					odm_set_timer(ODMPTR, &pBeamInfo->BeamformingTimer, 2); 
				} 
				else
				{
					WaitCnt=0;
			...
			if(pSoundingInfo->CandidateMUBFeeCnt > 0)
			{
				...
				{ /* More than 1 MU STA*/											
					...
					if((cnt == 1) && (AC_SIGMA_MODE == AC_SIGMA_NONE)) { //start mu sounding when 2 sta connected  for 11ac logo r2
						...
						SendVHTNDPAPacket_MU(pEntry->priv, pEntry->MacAddr, pEntry->AID, pEntry->BW, pEntry->mu_reg_index, NDPTxRate, 1);	
					} else if (cnt > 1) {
						...
						for(idx = 0; idx < cnt; idx++) {
							...
							if(idx == 0) {
								...
								if (GET_CHIP_VER(priv) == VERSION_8814B)
									RTL_W32(0x42c, RTL_R32(0x42c) | BIT(29));
#endif
								SendVHTNDPAPacket_MU(pEntry->priv, pEntry->MacAddr, pEntry->AID, pEntry->BW, pEntry->mu_reg_index, _NSS2_MCS0_RATE_, 0);
							}
							else if(idx == cnt-1)
								SendReportPollPacket(pEntry->priv, pEntry->MacAddr, pEntry->BW, pEntry->mu_reg_index, ReportPollRate,TRUE);	
							else
								SendReportPollPacket(pEntry->priv, pEntry->MacAddr, pEntry->BW, pEntry->mu_reg_index, ReportPollRate,FALSE);									
						}
		...
		if(bSetTimeoutTimer)
		{
			odm_set_timer(ODMPTR, &pBeamInfo->BFSoundingTimeoutTimer, TimeoutPeriod); //eric-mu
			break;
    ...
    )
{
	...
	{				
		...
		{			
			...
			if(NextSoundPeriod)
				odm_set_timer(ODMPTR, &pBeamInfo->BeamformingTimer, NextSoundPeriod);
			else
				beamform_SoundingTimerCallback(priv);
		}
		else
