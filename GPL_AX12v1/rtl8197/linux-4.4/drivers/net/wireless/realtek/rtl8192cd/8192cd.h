...
#define MAX_REG_FILE_SIZE			(14*1024)
#endif
...
// 48 is SIZE_TXDESC_88XX
	u4Byte  hal_hw_desc[64/sizeof(u4Byte)]; //extend to MAX size, SIZE_TXDESC_88XX_MAX
...
#if defined(CONFIG_IEEE80211V) || defined(DOT11K)
#pragma pack(1)
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	delay_timer_event;
#endif
    unsigned int measure_time_hi;
    unsigned int measure_time_lo; 
...
struct stat_info {
	...
	unsigned char mbo_cap;
	unsigned char mbo_reason_code;
   ...
   struct	dot11v_stat_info wnm;
	...
	unsigned int		useShortPreamble;
	unsigned int		expire_to;
	unsigned int		rtsdrop_expire_to;
...
//	UINT8		IOTPeer;			// Enum value.	HT_IOT_PEER_E
	...
	unsigned char assoc_req_length;
	unsigned char assoc_req_body[256];
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	unsigned long		tx_cmd_queue_drops;
#endif	
...
//	unsigned char		mp_dig_enable;
	...
	unsigned char		veriwave_debug_edca_mode;
#endif
#ifdef THER_TRIM
...
#ifdef CONFIG_OWE
	struct owe_vendor_ie_t	*owe_vendor_ie[2]; /* 0: root OPEN; 1: vap3 as OWE*/
	unsigned char owe_vd_ie_len[2];
#endif
...
//	unsigned short			F90_cnt;
	...
	unsigned char		tc_msta_mode;
	unsigned char		tc_msta_dual;
#endif
...
typedef struct rtl8192cd_priv {
	...
	unsigned int			txdesc_num;
#endif
	unsigned int   axidma_txdesc_num;
	unsigned int   hcidma_drxdesc_num;
	...
	bool owe_enble;
	bool owe_pmk_rdy;
	bool owe_pmk_cache_apply;
	unsigned char owe_pubkey[PMK_LEN];
	unsigned char owe_pmk[PMK_LEN];
	unsigned char owe_pmkid[PMKID_LEN];
	unsigned char owe_assoc_dh_ie[PMK_LEN + 3]; /* 3: Length + Element ID Extension + group */
	unsigned char owe_assoc_rsn_ie[OWE_MAX_RSNE_SIZE];
#endif
	...
	struct timer_list       multiap_channel_scan_timer;
...
//	unsigned char			*pBeaconErp;			// ptr of ERP field in beacon buf
	...
	unsigned int			nhm_cnt[14][NHM_RPT_NUM];
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	unsigned char					coexist_connection_bw;    
#endif	
#endif
