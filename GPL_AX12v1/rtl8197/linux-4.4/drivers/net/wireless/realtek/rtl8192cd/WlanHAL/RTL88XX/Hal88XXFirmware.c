...
)
{
    ...
    case C2H_88XX_BEACON_FAIL_RPT:
        Adapter->ext_stats.beacon_err_rson = tmpBuf[0];
        break;
    case 0xEE:
		phead = &Adapter->asoc_list;
		plist = phead;
		while ((plist = asoc_list_get_next(Adapter, plist)) != phead)
		{
			pstat = list_entry(plist, struct stat_info, asoc_list);
		
			if (pstat->cmn_info.aid == *tmpBuf){
				pstat->rtsdrop_expire_to = Adapter->pshare->rf_ft_var.rtsdrop_timer;
				break;
			}
