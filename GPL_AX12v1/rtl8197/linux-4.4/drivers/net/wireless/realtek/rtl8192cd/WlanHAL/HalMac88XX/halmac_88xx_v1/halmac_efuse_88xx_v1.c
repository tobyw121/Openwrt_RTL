...
efuse_cmd_cnstr_state_88xx_v1(struct halmac_adapter *adapter);

static enum halmac_ret_status
read_hw_efuse_88xx_v1(struct halmac_adapter *adapter, u32 offset, u32 size,
		      u8 *map);

static enum halmac_ret_status
read_log_efuse_map_88xx_v1(struct halmac_adapter *adapter, u8 *map);

static enum halmac_ret_status
proc_pg_efuse_by_map_88xx_v1(struct halmac_adapter *adapter,
			     struct halmac_pg_efuse_info *info,
			     enum halmac_efuse_read_cfg cfg);

static enum halmac_ret_status
dump_efuse_fw_88xx_v1(struct halmac_adapter *adapter);

static enum halmac_ret_status
dump_efuse_drv_88xx_v1(struct halmac_adapter *adapter);

static enum halmac_ret_status
proc_write_log_efuse_88xx_v1(struct halmac_adapter *adapter, u32 offset,
			     u8 value);

static enum halmac_ret_status
update_eeprom_mask_88xx_v1(struct halmac_adapter *adapter,
			   struct halmac_pg_efuse_info *info, u8 *updated_mask);

static enum halmac_ret_status
check_efuse_enough_88xx_v1(struct halmac_adapter *adapter,
			   struct halmac_pg_efuse_info *info, u8 *updated_mask);

static enum halmac_ret_status
pg_extend_efuse_88xx_v1(struct halmac_adapter *adapter,
			struct halmac_pg_efuse_info *info, u8 word_en,
			u8 pre_word_en, u32 eeprom_offset);

static enum halmac_ret_status
proc_pg_efuse_88xx_v1(struct halmac_adapter *adapter,
		      struct halmac_pg_efuse_info *info, u8 word_en,
		      u8 pre_word_en, u32 eeprom_offset);

static enum halmac_ret_status
program_efuse_88xx_v1(struct halmac_adapter *adapter,
		      struct halmac_pg_efuse_info *info, u8 *updated_mask);

/**
 * dump_efuse_map_88xx_v1() - dump "physical" efuse map
...
dump_efuse_map_88xx_v1(struct halmac_adapter *adapter,
		       enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	u8 *efuse_map;
	...
	if (*proc_status == HALMAC_CMD_PROCESS_SENDING) {
		PLTFM_MSG_WARN("[WARN]Wait event(dump efuse)...\n");
		return HALMAC_RET_BUSY_STATE;
	}

	if (efuse_cmd_cnstr_state_88xx_v1(adapter) != HALMAC_CMD_CNSTR_IDLE) {
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	status = switch_efuse_bank_88xx_v1(adapter, HALMAC_EFUSE_BANK_WIFI);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]switch efuse bank!!\n");
		return status;
	}

	status = proc_dump_efuse_88xx_v1(adapter, cfg);
	if (status != HALMAC_RET_SUCCESS) {
	...
	if (cnv_efuse_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_IDLE) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
...
/**
 * dump_efuse_map_bt_88xx_v1() - dump "BT physical" efuse map
 * @adapter : the adapter of halmac
 * @bank : bt efuse bank
 ...
 */
enum halmac_ret_status
dump_efuse_map_bt_88xx_v1(struct halmac_adapter *adapter,
			  enum halmac_efuse_bank bank, u32 size, u8 *map)
{
	...
	status = switch_efuse_bank_88xx_v1(adapter, bank);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]switch efuse bank!!\n");
		return status;
	}
	...
	status = read_hw_efuse_88xx_v1(adapter, 0, size, map);
	if (status != HALMAC_RET_SUCCESS) {
		bt_switch = 0;
...
/**
 * write_efuse_bt_88xx_v1() - write "BT physical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : Write value
 * @map : bt efuse map
 ...
 */
enum halmac_ret_status
write_efuse_bt_88xx_v1(struct halmac_adapter *adapter, u32 offset, u8 value,
		       enum halmac_efuse_bank bank)
{
	...
	status = write_hw_efuse_88xx_v1(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS) {
		bt_switch = 0;
...
/**
 * read_efuse_bt_88xx_v1() - read "BT physical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 ...
 */
enum halmac_ret_status
read_efuse_bt_88xx_v1(struct halmac_adapter *adapter, u32 offset, u8 *value,
		      enum halmac_efuse_bank bank)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = read_efuse_88xx_v1(adapter, offset, 1, value);
	if (status != HALMAC_RET_SUCCESS) {
		bt_switch = 0;
...
/**
 * cfg_efuse_auto_check_88xx_v1() - check efuse after writing it
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
cfg_efuse_auto_check_88xx_v1(struct halmac_adapter *adapter, u8 enable)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
/**
 * get_efuse_available_size_88xx_v1() - get efuse available size
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_efuse_available_size_88xx_v1(struct halmac_adapter *adapter, u32 *size)
{
	enum halmac_ret_status status;

	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);

	status = dump_log_efuse_map_88xx_v1(adapter, HALMAC_EFUSE_R_DRV);
...
/**
 * get_efuse_size_88xx_v1() - get "physical" efuse size
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_efuse_size_88xx_v1(struct halmac_adapter *adapter, u32 *size)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
/**
 * get_log_efuse_size_88xx_v1() - get "logical" efuse size
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_log_efuse_size_88xx_v1(struct halmac_adapter *adapter, u32 *size)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
...
/**
 * dump_log_efuse_map_88xx_v1() - dump "logical" efuse map
 * @adapter : the adapter of halmac
 * @cfg : dump efuse method
 * Author : Soar
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
dump_log_efuse_map_88xx_v1(struct halmac_adapter *adapter,
			   enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	if (adapter->efuse_map_valid == 1) {
		*proc_status = HALMAC_CMD_PROCESS_DONE;
		...
		if (eeprom_parser_88xx_v1(adapter, adapter->efuse_map, map) !=
		    HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, size);
...
enum halmac_ret_status
dump_log_efuse_mask_88xx_v1(struct halmac_adapter *adapter,
			    enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	u32 size = adapter->hw_cfg_info.eeprom_size;
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	if (adapter->efuse_map_valid == 1) {
		*proc_status = HALMAC_CMD_PROCESS_DONE;
		...
		if (eeprom_mask_parser_88xx_v1(adapter, adapter->efuse_map, map)
		    != HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, size);
			return HALMAC_RET_EEPROM_PARSING_FAIL;
		}
...
/**
 * read_logical_efuse_88xx_v1() - read logical efuse map 1 byte
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : 1 byte efuse value
 * Author : Soar
 ...
 */
enum halmac_ret_status
read_logical_efuse_88xx_v1(struct halmac_adapter *adapter, u32 offset,
			   u8 *value)
{
	u8 *map = NULL;
	u32 size = adapter->hw_cfg_info.eeprom_size;
	...
	if (*proc_status == HALMAC_CMD_PROCESS_SENDING) {
		PLTFM_MSG_WARN("[WARN]Wait/Rcvd event(dump efuse)...\n");
		return HALMAC_RET_BUSY_STATE;
	}
	if (efuse_cmd_cnstr_state_88xx_v1(adapter) != HALMAC_CMD_CNSTR_IDLE) {
		PLTFM_MSG_WARN("[WARN]Not idle state(dump efuse)...\n");
		return HALMAC_RET_ERROR_STATE;
	}
	...
	status = read_log_efuse_map_88xx_v1(adapter, map);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * write_log_efuse_88xx_v1() - write "logical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : value
 * Author : Soar
 ...
 */
enum halmac_ret_status
write_log_efuse_88xx_v1(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = proc_write_log_efuse_88xx_v1(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * pg_efuse_by_map_88xx_v1() - pg logical efuse by map
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
pg_efuse_by_map_88xx_v1(struct halmac_adapter *adapter,
			struct halmac_pg_efuse_info *info,
			enum halmac_efuse_read_cfg cfg)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	status = proc_pg_efuse_by_map_88xx_v1(adapter, info, cfg);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * mask_log_efuse_88xx_v1() - mask logical efuse
 * @adapter : the adapter of halmac
 * @info : efuse map information
 * Author : Soar
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
mask_log_efuse_88xx_v1(struct halmac_adapter *adapter,
		       struct halmac_pg_efuse_info *info)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
	...
	mask_eeprom_88xx_v1(adapter, info);
...
enum halmac_ret_status
switch_efuse_bank_88xx_v1(struct halmac_adapter *adapter,
			  enum halmac_efuse_bank bank)
{
	u8 reg_value;
	...
	if (!bt_switch) {
		if (cnv_efuse_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_BUSY) !=
		    HALMAC_RET_SUCCESS)
			return HALMAC_RET_ERROR_STATE;
	}
...
enum halmac_ret_status
proc_dump_efuse_88xx_v1(struct halmac_adapter *adapter,
			enum halmac_efuse_read_cfg cfg)
{
	u32 h2c_init;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	...
	*proc_status = HALMAC_CMD_PROCESS_SENDING;
	...
	if (cnv_efuse_state_88xx_v1(adapter, HALMAC_CMD_CNSTR_H2C_SENT) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
	...
	if (cfg == HALMAC_EFUSE_R_AUTO) {
		...
		if (adapter->halmac_state.dlfw_state == HALMAC_DLFW_NONE ||
		    h2c_init == 0)
			status = dump_efuse_drv_88xx_v1(adapter);
		else
			status = dump_efuse_fw_88xx_v1(adapter);
...
enum halmac_ret_status
cnv_efuse_state_88xx_v1(struct halmac_adapter *adapter,
		      ...
		      u8 *map)
{
	...
	HALMAC_REG_W8_CLR(REG_ANAPARLDO_POW_MAC, BIT(0));
	...
	for (addr = offset; addr < offset + size; addr++) {
		...
		do {
			...
			cnt--;
			if (cnt == 0) {
				PLTFM_MSG_ERR("[ERR]HALMAC_RET_EFUSE_R_FAIL\n");
...
enum halmac_ret_status
write_hw_efuse_88xx_v1(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	...
	/* Enable 2.5V LDO */
	HALMAC_REG_W8_SET(REG_ANAPARLDO_POW_MAC, BIT(0));
	...
	if (adapter->efuse_auto_check_en == 1) {
		if (read_hw_efuse_88xx_v1(adapter, offset, 1, &value_read) !=
		    HALMAC_RET_SUCCESS)
			return HALMAC_RET_EFUSE_R_FAIL;
		if (value_read != value) {
...
enum halmac_ret_status
eeprom_parser_88xx_v1(struct halmac_adapter *adapter, u8 *phy_map, u8 *log_map)
{
	u8 i;
	u8 value8;
	u8 blk_idx;
...
enum halmac_ret_status
eeprom_mask_parser_88xx_v1(struct halmac_adapter *adapter, u8 *phy_map,
			   u8 *log_mask)
{
	u8 i;
	u8 value8;
...
read_log_efuse_map_88xx_v1(struct halmac_adapter *adapter, u8 *map)
{
	...
	if (adapter->efuse_map_valid == 0) {
		...
		status = read_efuse_88xx_v1(adapter, 0, efuse_size, local_map);
		if (status != HALMAC_RET_SUCCESS) {
			PLTFM_MSG_ERR("[ERR]read efuse\n");
			     ...
			     enum halmac_efuse_read_cfg cfg)
{
	...
	status = update_eeprom_mask_88xx_v1(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
	...
	status = check_efuse_enough_88xx_v1(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
	...
	status = program_efuse_88xx_v1(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]pg efuse\n");
		PLTFM_FREE(updated_mask, mask_size);
		return status;
...
dump_efuse_drv_88xx_v1(struct halmac_adapter *adapter)
{
	...
	if (!adapter->efuse_map) {
		...
		if (!adapter->efuse_map) {
			PLTFM_MSG_ERR("[ERR]malloc adapter map!!\n");
			reset_ofld_feature_88xx_v1(adapter,
	...
	if (adapter->efuse_map_valid == 0) {
		...
		if (read_hw_efuse_88xx_v1(adapter, 0, efuse_size, map) !=
		    HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, efuse_size);
			return HALMAC_RET_EFUSE_R_FAIL;
		}
...
dump_efuse_fw_88xx_v1(struct halmac_adapter *adapter)
{
	u8 h2c_buf[H2C_PKT_SIZE_88XX_V1] = { 0 };
	u16 seq_num = 0;
	u32 efuse_size = adapter->hw_cfg_info.efuse_size;
	...
	hdr_info.ack = 1;
	set_h2c_pkt_hdr_88xx_v1(adapter, h2c_buf, &hdr_info, &seq_num);
	...
	if (adapter->efuse_map_valid == 0) {
		status = send_h2c_pkt_88xx_v1(adapter, h2c_buf);
		if (status != HALMAC_RET_SUCCESS) {
	...
	if (*(map + offset) != value) {
		...
		if (offset > 0x7f) {
			if (adapter->hw_cfg_info.efuse_size <=
			    4 + prtct_efuse_size + adapter->efuse_end) {
				PLTFM_FREE(map, eeprom_size);
				return HALMAC_RET_EFUSE_NOT_ENOUGH;
			}

			status = write_hw_efuse_88xx_v1(adapter, end, hdr);
			if (status != HALMAC_RET_SUCCESS) {
				PLTFM_FREE(map, eeprom_size);
				return status;
			}

			status = write_hw_efuse_88xx_v1(adapter, end + 1, hdr2);
			if (status != HALMAC_RET_SUCCESS) {
				PLTFM_FREE(map, eeprom_size);
				return status;
			}

			status = write_hw_efuse_88xx_v1(adapter, end + 2,
							byte1);
			if (status != HALMAC_RET_SUCCESS) {
				PLTFM_FREE(map, eeprom_size);
				return status;
			...
			status = write_hw_efuse_88xx_v1(adapter, end + 3,
							byte2);
			if (status != HALMAC_RET_SUCCESS) {
				PLTFM_FREE(map, eeprom_size);
				return status;
			...
			if (adapter->hw_cfg_info.efuse_size <=
			    3 + prtct_efuse_size + adapter->efuse_end) {
				PLTFM_FREE(map, eeprom_size);
				return HALMAC_RET_EFUSE_NOT_ENOUGH;
			}
			...
			status = write_hw_efuse_88xx_v1(adapter, end + 1,
							byte1);
			if (status != HALMAC_RET_SUCCESS) {
				PLTFM_FREE(map, eeprom_size);
				return status;
...
enum halmac_ret_status
read_efuse_88xx_v1(struct halmac_adapter *adapter, u32 offset, u32 size,
		   u8 *map)
{
	if (!map) {
		PLTFM_MSG_ERR("[ERR]malloc map\n");
	...
	if (adapter->efuse_map_valid == 1) {
		...
		if (read_hw_efuse_88xx_v1(adapter, offset, size, map) !=
		    HALMAC_RET_SUCCESS)
			return HALMAC_RET_EFUSE_R_FAIL;
	}
			   ...
			   struct halmac_pg_efuse_info *info, u8 *updated_mask)
{
	...
	for (i = 0; i < info->efuse_map_size; i = i + 16) {
		for (j = 0; j < 16; j = j + 2) {
			map_offset = i + j;
			mask_offset = i >> 4;
			...
			u8 pre_word_en, u32 eeprom_offset)
{
	...
	status = write_hw_efuse_88xx_v1(adapter, efuse_end, hdr);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]write efuse\n");
		return status;
	}
	status = write_hw_efuse_88xx_v1(adapter, efuse_end + 1, hdr2);
	if (status != HALMAC_RET_SUCCESS) {
	...
	for (i = 0; i < 4; i++) {
		if (((pre_word_en >> i) & 0x1) > 0) {
			status = write_hw_efuse_88xx_v1(adapter, efuse_end,
							*(info->efuse_map +
							eeprom_offset +
							(i << 1)));
			if (status != HALMAC_RET_SUCCESS) {
			...
			status = write_hw_efuse_88xx_v1(adapter, efuse_end + 1,
							*(info->efuse_map +
							eeprom_offset + (i << 1)
							+ 1));
			if (status != HALMAC_RET_SUCCESS) {
		      ...
		      u8 pre_word_en, u32 eeprom_offset)
{
	...
	for (i = 0; i < 4; i++) {
		if (((pre_word_en >> i) & 0x1) > 0) {
			status = write_hw_efuse_88xx_v1(adapter,
							efuse_end,
							*(info->efuse_map +
							eeprom_offset +
							(i << 1)));
		      ...
		      struct halmac_pg_efuse_info *info, u8 *updated_mask)
{
	...
	for (i = 0; i < info->efuse_map_size; i = i + 8) {
		...
		if (pre_word_en > 0) {
			if (eeprom_offset > 0x7f) {
				status = pg_extend_efuse_88xx_v1(adapter,
								 info,
								 word_en,
								 pre_word_en,
								 eeprom_offset);
				...
				status = proc_pg_efuse_88xx_v1(adapter, info,
							       word_en,
							       pre_word_en,
							       eeprom_offset);
				if (status != HALMAC_RET_SUCCESS) {
...
void
mask_eeprom_88xx_v1(struct halmac_adapter *adapter,
		    struct halmac_pg_efuse_info *info)
{
	u8 pre_word_en;
	u8 *updated_mask;
...
enum halmac_ret_status
get_efuse_data_88xx_v1(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	u8 seg_id;
	u8 seg_size;
	u8 seq_num;
	...
	if ((enum halmac_h2c_return_code)fw_rc == HALMAC_H2C_RETURN_SUCCESS) {
		...
		if (adapter->evnt.log_efuse_map == 1) {
			if (eeprom_parser_88xx_v1(adapter, adapter->efuse_map,
		...
		if (adapter->evnt.log_efuse_mask == 1) {
			if (eeprom_mask_parser_88xx_v1(adapter,
						       adapter->efuse_map, map)
						       != HALMAC_RET_SUCCESS) {
				PLTFM_FREE(map, eeprom_size);
				return HALMAC_RET_EEPROM_PARSING_FAIL;
			}
...
enum halmac_ret_status
get_dump_phy_efuse_status_88xx_v1(struct halmac_adapter *adapter,
				  enum halmac_cmd_process_status *proc_status,
				  u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_dump_log_efuse_status_88xx_v1(struct halmac_adapter *adapter,
				  enum halmac_cmd_process_status *proc_status,
				  u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_log_efuse_mask_status_88xx_v1(struct halmac_adapter *adapter,
				  enum halmac_cmd_process_status *proc_status,
				  u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_h2c_ack_phy_efuse_88xx_v1(struct halmac_adapter *adapter, u8 *buf,
			      u32 size)
{
	u8 seq_num = 0;
	u8 fw_rc;
...
u32
get_rsvd_efuse_size_88xx_v1(struct halmac_adapter *adapter)
{
	return adapter->hw_cfg_info.prtct_efuse_size;
}

/**
 * write_wifi_phy_efuse_88xx_v1() - write wifi physical efuse
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
write_wifi_phy_efuse_88xx_v1(struct halmac_adapter *adapter,
			     u32 offset, u8 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
...
/**
 * read_wifi_phy_efuse_88xx_v1() - read wifi physical efuse
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
read_wifi_phy_efuse_88xx_v1(struct halmac_adapter *adapter, u32 offset,
			    u32 size, u8 *value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = read_hw_efuse_88xx_v1(adapter, offset, size, value);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]read hw efuse\n");
		return status;
	}
