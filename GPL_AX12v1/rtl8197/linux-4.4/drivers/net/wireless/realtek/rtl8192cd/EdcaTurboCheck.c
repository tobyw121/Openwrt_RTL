	...
	)
{
	...
	u32 be_edca, vi_edca;
#endif
	unsigned int legacy_sta_low_txop=0;
	...
	if(GET_CHIP_VER(priv)==VERSION_8197F && priv->ht_legacy_sta_num)
		legacy_sta_low_txop++;
		...
		if (priv->pshare->ht_sta_num
			...
			if (GET_CHIP_VER(priv) == VERSION_8814B) {
				...
				if(priv->pshare->th_mode == 1){					
	                if(priv->cochannel_to)
						RTL_W32(0x508, (0x0138 << 16) | (6 << 12) | (3 << 8) | 0x2b);
					else{
			...
			else 
			{
				...
				if(GET_CHIP_VER(priv) >= VERSION_8814A)
				{
					...
					if(GET_CHIP_VER(priv) == VERSION_8197F && (priv->pshare->total_assoc_num>=4) && (priv->pshare->rssi_min<40) && (get_total_tx_average(priv)<50))
					{
						if (priv->pshare->th_mode == 2) {
							RTL_W32(0x508, 0x642b);		
						} else {
							if (legacy_sta_low_txop)
								RTL_W32(0x508, 0x20851f);
							else
								RTL_W32(0x508, 0x5e851f);					
						}
					} else
#endif					
#if defined(CONFIG_PCI_HCI)
					if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE)
					{
						...
						} else if( pstat && (pstat->IOTPeer == HT_IOT_PEER_OPPO) && (pstat->mimo_num == 1)) {			
							if(priv->pshare->th_mode == 2) 
								txop = (priv->pshare->rf_ft_var.rts_intf_mode ? 0x5e : 0);
							else
								txop = 0x138;
							if (priv->pmib->dot11RFEntry.phyBandSelect ==  PHY_BAND_2G) {
								cw_max = 0xa;
								aifs_time = 0x2b;
							} else if (priv->pmib->dot11RFEntry.phyBandSelect ==  PHY_BAND_5G && pstat->ht_cap_len) {
								cw_max = 0x8;
								aifs_time = 0x1f;
							} else {
								cw_max = 0x6;
                        ...
                        cw_min = (priv->pshare->th_mode == 3) ? 3 : 4;
						if((priv->pshare->th_mode != 2) && legacy_sta_low_txop && (txop > 0x20))
							txop = 0x20;
...
////////////////////////////////////////////////////////
	...
	if ((priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_2G)  && priv->pshare->tc_msta_mode) {
		RTL_W32(0x50c, 0x5ea42b);
		return;
	}
#endif 
...
if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
	...
	if (is_veriwave_testing(priv)) {
#if 1
		if (priv->pshare->rf_ft_var.veriwave_debug_edca_mode == 1) {
		...
		else if (priv->pshare->rf_ft_var.veriwave_debug_edca_mode == 2) {
			if ((priv->ext_stats.rx_avarage >> 17) > 15 && (priv->pshare->th_mode != 2)) {
				priv->pshare->th_mode = 2; // RX mode
				switch_turbo++;
			} else if((priv->ext_stats.rx_avarage >> 17) < 10 && (priv->pshare->th_mode != 1)) {
				priv->pshare->th_mode = 1; // TX mode
				switch_turbo++;
			}
		}
		...
		if((priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) &&
			(priv->ext_stats.rx_avarage>>7)>40 && priv->pshare->th_mode == 1 && priv->pshare->veriwave_sta_num==1 ){
			priv->pshare->th_mode = 2;
			switch_turbo++;
		}
#endif
	} else {
		if(((get_total_tx_average(priv) > 15) || priv->cochannel_to) && (priv->pshare->th_mode!=1)) {               
            priv->pshare->th_mode = 1; // TX mode
            switch_turbo++;
        } else if(((get_total_tx_average(priv) < 10)  && (priv->cochannel_to==0) ) && (priv->pshare->th_mode!=2)) {                
            priv->pshare->th_mode = 2; // RX mode
            switch_turbo++;
        }
        if (GET_CHIP_VER(priv) == VERSION_8814B) {
			...
			if( (is_cochannel ^ priv->is_cochannel) && priv->pshare->th_mode == 1){
				priv->is_cochannel = is_cochannel;
				switch_turbo++;
			}
		}
