		...
		*bRtsThreshold = TRUE;
		...
		{	// when HT MIMO Dynamic power save is set, RTS is needed
			*bRtsEnable = TRUE;
			...
			{
#ifdef COCHANNEL_RTS
				if (priv->pmib->miscEntry.auto_rts_mode == 1 && (priv->cochannel_to || priv->pshare->rf_ft_var.rts_intf_mode )&& (txcfg->rts_thrshld == DEFAULT_RTS_THRESHOLD))
				{
					*bNProtect = 1;
					*bRtsEnable = TRUE;
					*bCts2SelfEnable = FALSE;
				...
				if (*bRtsEnable && txcfg->pstat && txcfg->pstat->rtsdrop_expire_to){
						*bRtsEnable = FALSE;
						*bCts2SelfEnable = TRUE;
						*bHwRts = FALSE;
				}
...
static void rtl8192cd_fill_fwinfo_8812(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, struct tx_desc  *pdesc, unsigned int frag_idx)
{
	...
	else if (priv->pmib->miscEntry.auto_rts_mode == 4)
	{
		bRtsEnable = FALSE;
		bCts2SelfEnable = TRUE;
		bHwRts = FALSE;
 ...
 int amsdu_check(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat, struct tx_insn *txcfg)
{
	...
	if (GET_CHIP_VER(priv) == VERSION_8814B)
	{
		if (txcfg->is_11s & 1)
		{
			if (skb_headroom(skb) < sizeof(struct lls_mesh_header))
				return RET_AGGRE_BYPASS;	
		}
	}
#endif
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	if (OFFLOAD_ENABLE(priv) && check_full_ofld(priv, pstat, skb, txcfg)) {
		rtl8192cd_xmit_offload(pstat, skb, txcfg);
		return 0;
	}
#endif
...
#if defined(A4_STA)
	if (!(priv->pmib->miscEntry.a4_enable) && pstat->amsdu_level && !(pstat->state & WIFI_SLEEP_STATE))
#else
...
int __rtl8192cd_start_xmit_pci(struct sk_buff *skb, struct net_device *dev, int tx_flag)
{
	...
	unsigned char		*da;
#if defined(__KERNEL__) || defined(__OSK__)
#if !defined(CONFIG_RTL_VLAN_8021Q) && (defined(HS2_SUPPORT) || !defined(CONFIG_PCI_HCI))
	struct sk_buff *newskb = NULL;
#endif
#endif
#ifdef TX_SCATTER
...
//IGMP snooping
	...
	extern int multi_wan_wlan_process_tx(struct sk_buff **skb);
	if(FAILED == multi_wan_wlan_process_tx((struct sk_buff **)(&skb))) {
		//skb already freed...
		goto stop_proc;
	}
	#endif
