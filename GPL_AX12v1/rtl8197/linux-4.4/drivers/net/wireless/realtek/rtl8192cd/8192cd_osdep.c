...
struct rtl8192cd_priv *ipc_8814b_priv;
...
struct _device_info_ wlan_device[] =
{
    	...
    	{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_BIOS<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
	        ...
	        (defined(CONFIG_RTL_5G_SLOT_1) && defined(CONFIG_BAND_2G_ON_WLAN0)) || \
    ...
    #if defined(CONFIG_WLAN_HAL_8197G) && defined(CONFIG_BAND_2G_ON_WLAN0)
        {
            (MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_EMBEDDED<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 
            BSP_WLAN_CONF_ADDR, 
            BSP_WLAN_BASE_ADDR, 
    ...
    #endif  //CONFIG_WLAN_HAL_8197F
    #if defined(CONFIG_WLAN_HAL_8197G) && defined(CONFIG_BAND_5G_ON_WLAN0)
    {
        (MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_EMBEDDED<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 
        BSP_WLAN_CONF_ADDR, 
        BSP_WLAN_BASE_ADDR, 
...
#else
#ifndef __ECOS
void rtl8192cd_shutdown(struct notifier_block *nb, unsigned long cmd, void *p)
{
...
int_retry_process:    
	{
        ...
        if (txStart) {
            //printk("START: HISR %08x\n", RTL_R32(0x10A4));
            RTL_W32(REG_TXDMA_START_HISR, RTL_R32(REG_TXDMA_START_HISR));
        }
        if (txStop) {
            //printk("STOP: HISR %08x\n", RTL_R32(0x109C));
            RTL_W32(REG_TXDMA_STOP_HISR, RTL_R32(REG_TXDMA_STOP_HISR));
        }
#endif
...
#if defined(CONFIG_VERIWAVE_CHECK) && defined(CONFIG_VERIWAVE_MACBBTX)
...
#ifdef CONFIG_OWE
	/* open => set OWE ; take wlan0-va3 for OWE */
	if ((owe_is_root_open(priv)) && (owe_is_owebss(priv))
	  && (priv->pmib->dot1180211AuthEntry.dot11EnablePSK == 0)) {
		owe_init(priv);
		owe_set_itf_owebss(priv);
		owe_pmk_cache_init();
		panic_printk("OWE enable\n");
	} else {
		panic_printk("OWE not enable\n");
	}
#endif
...
#if (defined(CONFIG_SLOT_0_ANT_SWITCH)|| defined(CONFIG_SLOT_1_ANT_SWITCH)||defined(CONFIG_RTL_8881A_ANT_SWITCH) || defined(CONFIG_RTL_8197F_ANT_SWITCH) || defined(CONFIG_RTL_8197G_ANT_SWITCH))
	if(!CHIP_VER_92X_SERIES(priv))
...
/*cfg p2p cfg p2p*/
	...
	*/
	...
	// construct operation and basic rates set
	{
		...
		//set ACS type for 5G, acs_type=1 not support 5G
		if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G){
			...
			#endif
				panic_printk("[ACS] 5G should not use acs_type=1, change to acs_type=%d\n", priv->pmib->dot11RFEntry.acs_type);
			}
		}
...
/* No idea how to process AMSDU shortcut with MESH or WDS, so disable AMSDU shortcut when MESH or WDS enabled */
	...
	bool owe_ret;
	if (priv->owe_enble) {
		owe_ret = owe_construct_vd_ie(priv);
		if (owe_ret == FALSE)
			GDEBUG("construct vendor specific ie fail !\n");
	}
#endif
	...
	if (priv->pmib->dot1180211AuthEntry.dot11EnablePSK) {
		...
		if(priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum == 0) {
			/* For Most station, 4 is better significantly */
	...
	priv->multiap_backhaul_steer_timer.function = rtk_multi_ap_update_backhaul_steer_results;

	init_timer(&priv->multiap_unassoc_sta_timer);
	priv->multiap_unassoc_sta_timer.data = (unsigned long) priv;
	priv->multiap_unassoc_sta_timer.function = rtk_multi_ap_trigger_unassoc_sta_metric;

	priv->pmib->dot11StationConfigEntry.probe_info_enable = 1;
	priv->pmib->multi_ap.multiap_max_device_reached = 0;
	priv->pmib->multi_ap.multiap_monitor_mode_disable = 0;
...
#if defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	init_timer(&priv->multiap_channel_scan_timer);
	priv->multiap_channel_scan_timer.data = (unsigned long) priv;
	priv->multiap_channel_scan_timer.function = rtk_multi_ap_update_channel_scan_results;
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
	        priv->rm.delay_timer.data = (unsigned long) &priv->rm.delay_timer_event;
	        priv->rm.delay_timer.function = timer_event_timer_fn;
	        INIT_TIMER_EVENT_ENTRY(&priv->rm.delay_timer_event, rm_do_next_measure, (unsigned long)priv);
		}
#endif
    }
#endif
...
#ifdef RTK_MULTI_AP
	if (timer_pending(&priv->multiap_backhaul_steer_timer))
		del_timer_sync(&priv->multiap_backhaul_steer_timer);

	if (timer_pending(&priv->multiap_unassoc_sta_timer))
		del_timer_sync(&priv->multiap_unassoc_sta_timer);

#if defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	if (timer_pending(&priv->multiap_channel_scan_timer))
		del_timer_sync(&priv->multiap_channel_scan_timer);
...
&& (GET_VXD_PRIV(priv)->simple_config_status == 0)
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		...
		for (j=0; j<=max_qnum; j++) {
			...
			if((j > DATAQ_IDX_DCOFLD_MIN) && (j < DATAQ_IDX_DCOFLD_MAX))
				continue;
#endif			
#ifdef CONFIG_WLAN_HAL
			if (IS_HAL_CHIP(priv)) {
...
void init_vap_hw(struct rtl8192cd_priv *priv)
{
	...
	{	
		// clear the rest area of CAM
		int i=0,j=0;
		unsigned int camData[2],write_cmd;
		camData[1] = 0;
...
#ifdef SMP_LOAD_BALANCE_SUPPORT
void smp_assign_expire_timer(void *_dev)
{
...
//---------------------------------------------------------- david+2008-03-17
	...
	if (OFFLOAD_ENABLE(priv)) {
		...
		if (priv->pmib->dot11WdsInfo.wdsEnabled) {
			priv->pmib->dot11WdsInfo.wdsEnabled = 0;
			priv->pmib->dot11WdsInfo.wdsPure = 0;
			panic_printk("[%s] Now doesn't support WDS + offload driver, disable WDS\n", priv->dev->name);
		}
#endif
// #ifdef A4_STA
// 		if (priv->pmib->miscEntry.a4_enable) {
// 			priv->pmib->miscEntry.a4_enable = 0;
// 			panic_printk("[%s] Now doesn't support A4_STA + offload driver, disable A4_STA\n", priv->dev->name);
// 		}
// #endif
	}
#endif
...
//------------------- david+2006-06-30
	...
	if (OFFLOAD_ENABLE(priv)) {
        ...
        if (priv->pmib->miscEntry.dc_debug == 1) {
            rtl88XX_signin_h2d_cmd(priv, "0x80000000,pm", h2d_sys_config);
            priv->pshare->dc_debug_status = priv->pmib->miscEntry.dc_debug;
        }
...
//			priv->ps_ctrl = 0x11;
        ...
        if (priv->pshare->rf_ft_var.expire_timer_cpuid >= 0 && priv->pshare->rf_ft_var.expire_timer_cpuid < nr_cpu_ids)
            smp_call_function_single(priv->pshare->rf_ft_var.expire_timer_cpuid, smp_assign_expire_timer, dev, 0);
        else
#endif
		    mod_timer(&priv->expire_timer, jiffies + EXPIRE_TO);
...
#ifdef RTK_NL80211 /*kill tasklets before stop hw*/
#ifdef DFS
		/* prevent killing tasklet issue in interrupt */
		if (!priv->pmib->dot11DFSEntry.DFS_detected)
#endif
...
//Only ONE SLOT, always enable HIGH Power 
...
#if defined(CONFIG_WLAN_HAL_8192FE)
	if (GET_CHIP_VER(priv)==VERSION_8192F) {
#if defined(CONFIG_SLOT_0_RFE_TYPE_8)
		priv->pmib->dot11RFEntry.rfe_type = 8;
#elif defined(CONFIG_SLOT_0_RFE_TYPE_9)
		priv->pmib->dot11RFEntry.rfe_type = 9;
#elif defined(CONFIG_SLOT_0_RFE_TYPE_12)
	...
	if ((RX_BUF_LEN > 3900) || (GET_CHIP_VER(priv) == VERSION_8814B)) {//size more than amsdu size, it could recevie bigger packets in one rx-buffer 
...
#if defined(SW_TX_QUEUE) && !defined(WIFI_BUFFER_LIMITED)
...
/* Low power consumption in idle mode */
    ...
    if (GET_CHIP_VER(priv) == VERSION_8814B) {
		...
		priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum = 3;
	}
#endif
...
int nonCacheRXBDAllocate(struct rtl8192cd_priv *priv,unsigned int rx_dma_size)
{
	...
	rx_dma_ptr = dma_alloc_coherent(NULL, rx_dma_size+PAGE_SIZE,&(_GET_HAL_DATA(priv)->alloc_desc_rxdma_dmahandle), GFP_ATOMIC);
    if (rx_dma_ptr == NULL) {
        panic_printk(KERN_ERR "can't allocate rx descriptior page, abort!\n");
        _GET_HAL_DATA(priv)->desc_dma_rxbuf_len   = 0;
        return FAIL;
...
*****************/
	...
	if (OFFLOAD_ENABLE(priv) && IS_HARDWARE_TYPE_8814B(priv)) {
		...
		printk("OFLD: cpu_addr %p, dma_handle %p,txdma size %d\n", 
			...
			if (GET_HAL_INTERFACE(priv)->InitHCIDMAMemHandler(priv) == RT_STATUS_FAILURE){
				GET_HAL_INTERFACE(priv)->StopSWHandler(priv);
				printk("can't allocate descriptior page, abort!\n");
...
#elif defined(CONFIG_LUNA_WLAN_VAP_NAME)
		if(strcmp(dev->name,"wlan0-vap0")==0)				
			dev->vlan_member_map=WLAN0_VA0_MASK_BIT;
		else if(strcmp(dev->name,"wlan0-vap1")==0)
			dev->vlan_member_map=WLAN0_VA1_MASK_BIT;
		else if(strcmp(dev->name,"wlan0-vap2")==0)
			dev->vlan_member_map=WLAN0_VA2_MASK_BIT; 
		else if(strcmp(dev->name,"wlan0-vap3")==0)
			dev->vlan_member_map=WLAN0_VA3_MASK_BIT; 
		else if(strcmp(dev->name,"wlan1-vap0")==0)
			dev->vlan_member_map=WLAN1_VA0_MASK_BIT;
		else if(strcmp(dev->name,"wlan1-vap1")==0)
			dev->vlan_member_map=WLAN1_VA1_MASK_BIT;
		else if(strcmp(dev->name,"wlan1-vap2")==0)
			dev->vlan_member_map=WLAN1_VA2_MASK_BIT; 
		else if(strcmp(dev->name,"wlan1-vap3")==0)
			dev->vlan_member_map=WLAN1_VA3_MASK_BIT; 
...
#if !defined(NOT_RTK_BSP)
{
...
#ifdef CONFIG_WLAN_HAL_8814BE
		if (((vendor_id == 0x10ec) && (device_id == 0xb814)) || ((vendor_id == 0x10ec) && (device_id == 0xc814)))
		{
...
#if defined(CONFIG_PE_ENABLE)
			{
				ipc_8814b_priv = priv;	//hardcode for temporary
				register_wfo_ipc();
			}
#endif
		} 
...

		{
			...
			{
				DEBUG_INFO("version_id=VERSION_8192F found\n");
        ...
        if (((priv->pshare->type>>TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_DIRECT)
        {
...

			{
				...
				if (
                    ...
                    #endif
					#if defined(CONFIG_WLAN_HAL_8814CR)
                    && (vendor_deivce_id != ((unsigned long)((0xc814<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
					#if defined(CONFIG_WLAN_HAL_8192FE)
...
#endif
#if defined(CONFIG_WLAN_HAL_8814CR)
                if (vendor_deivce_id == ((unsigned long)((0xc814<<16)|PCI_VENDOR_ID_REALTEK))) {
                    priv->pshare->version_id = VERSION_8814B;
                    DEBUG_INFO("\n found 8814C !!! \n");
                    printk("\n found 8814C !!! \n");
                }
...
//#ifdef  CONFIG_SOC_RTL8881A
		...
		//FW
		{
		  	...
		  	ret = of_address_to_resource(np, 6, &mem_resource);
				...
				printk("fw virt = %x\r\n", addr);
				set_pe_fw_phys_addr(mem_resource.start);
				set_pe_fw_virt_addr(addr);						
		}
...
#ifdef USE_DMA_ALLOCATE
			page_ptr = dma_alloc_coherent(priv->pshare->cma_dev, DESC_DMA_PAGE_SIZE_IF(priv), &addr, GFP_KERNEL);
#else
...
#if 0//#ifdef CONFIG_ENABLE_CCI400
                kfree(_GET_HAL_DATA(priv)->desc_rxdma_buf);
#else
#ifdef CONFIG_RXBD_UNCACHE_COHERENT
                dma_free_coherent(&priv->dev,_GET_HAL_DATA(priv)->alloc_desc_rxdma_buf_size,_GET_HAL_DATA(priv)->alloc_desc_rxdma_addr,_GET_HAL_DATA(priv)->alloc_desc_rxdma_dmahandle);
#endif
#endif                
...
#else
			dma_free_coherent(priv->pshare->cma_dev, DESC_DMA_PAGE_SIZE_IF(priv), (void*)priv->pshare->phw->alloc_dma_buf,
				(dma_addr_t)(priv->pshare->phw->ring_dma_addr-page_align_phy));
#endif
		} else
#endif
...
static void MDL_DEVEXIT rtk_remove_one(struct pci_dev *pdev)
{
	...
	int cur_index;
	unsigned long x;
    struct rtl8192cd_priv *priv = wlan_device[cur_index].priv;
...
//------------------------------------
...
#endif
	{
        ...
        if ((((wlan_device[wlan_index].type >> TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_BIOS)||(((wlan_device[wlan_index].type >> TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_BIOS_DT)) {
...
__MIPS16
__IRAM_IN_865X
struct net_device *rtl_get_shortcut_dev(unsigned char *da)
{
	struct rtl8192cd_priv *shortcut_dev_priv;
	struct net_device *shortcut_dev = get_shortcut_dev(da);

	if(shortcut_dev)
	{
		shortcut_dev_priv = GET_DEV_PRIV(shortcut_dev);
		if (shortcut_dev_priv && (!(shortcut_dev_priv->pmib->dot11OperationEntry.disable_brsc)))
			return shortcut_dev;
	}
