				   ...
				   struct fw_bin_info *info)
{
	...
	if (info->dynamic_hdr_en) {
		...
		val32 = *(u32 *)(((u8 *)hdr) + (FWHDR_HDR_LEN +
						info->section_num * FWHDR_SECTION_LEN));
...
u32 mac_get_dynamic_hdr_ax(struct mac_ax_adapter *adapter, u8 *fw, u32 fw_len)
{
	...
	dynamic_hdr_len = *(u32 *)dynamic_hdr_content;
	dynamic_hdr_count = *(u32 *)(dynamic_hdr_content + 4);
	...
	for (val32 = 0; val32 < dynamic_hdr_count; val32++) {
		dynamic_section_len = *(u16 *)(dynamic_hdr_content);
	...
	for (val32 = 0; val32 < dynamic_hdr_count; val32++) {
		...
		case FWDL_DYNAMIC_HDR_FWCAP:
			adapter->fw_info.cap_buff = (u8 *)PLTFM_MALLOC(dynamic_section_len - 4);
			if (!adapter->fw_info.cap_buff) {
				PLTFM_MSG_ERR("[ERR]%s: Malloc Fail\n", __func__);
				PLTFM_FREE(adapter->fw_info.cap_buff, dynamic_section_len - 4);
				return MACBUFALLOC;
			}
...
#elif (MAC_AX_8852C_SUPPORT) || (MAC_AX_8192XB_SUPPORT)
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852C) ||
	    is_chip_id(adapter, MAC_AX_CHIP_ID_8192XB)) {
...
u32 mac_enable_cpu(struct mac_ax_adapter *adapter, u8 boot_reason, u8 dlfw)
{
	...
	MAC_REG_W32(R_AX_HISR0, val32 & ~B_AX_HALT_C2H_INT);
