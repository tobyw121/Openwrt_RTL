		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	ret = nla_put_u32(skb, ANDR_WIFI_ATTRIBUTE_NUM_FEATURE_SET, num);
	for (i = 0; i < num; i++)
		ret = nla_put_u32(skb, ANDR_WIFI_ATTRIBUTE_FEATURE_SET, reply[i]);
			...
			nla_for_each_nested(iter1, iter, rem1) {
				nla_for_each_nested(iter2, iter1, rem2) {
					...
					case RTT_ATTRIBUTE_TARGET_CHAN:
						_rtw_memcpy(&rtt_target->channel, nla_data(iter2),
		...
		const void *data, int len)
{
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		case RTT_ATTRIBUTE_TARGET_MAC:
			if (mac_addr)
				_rtw_memcpy(mac_addr++, nla_data(iter), ETHER_ADDR_LEN);
			else {
...
{
	_adapter *padapter = GET_PRIMARY_ADAPTER(wiphy_to_adapter(wiphy));
	HAL_DATA_TYPE *hal = GET_HAL_DATA(adapter_to_dvobj(padapter));
	int ret = 0, rem, type;
	int buf_len = 1024;
	char *buf_ptr;
	const struct nlattr *iter;
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		switch (type) {
			...
			case LOGGER_ATTRIBUTE_GET_FW:
				sprintf(buf_ptr, "v%d.%d", hal->fw_version, hal->fw_sub_version);
				break;
			default:
				RTW_ERR("Unknown type: %d\n", type);
				ret = -EINVAL;
