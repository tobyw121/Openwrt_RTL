...
__iram_func__
static void _iqk_read_fft_8832crvu(struct rf_info *rf, u8 path)
{
#if 0
	u8 i = 0x0;
	u32 fft[6] = {0x0};
...
__iram_func__
static void _iqk_read_xym_8832crvu(struct rf_info *rf, u8 path)
{
...
__iram_func__
static void _iqk_read_txcfir_8832crvu(struct rf_info *rf, u8 path, u8 group)
{
...
__iram_func__
static void _iqk_read_rxcfir_8832crvu(struct rf_info *rf, u8 path, u8 group)
{
...
__iram_func__
static bool _iqk_check_nbiqc_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,  u8 ktype, u8 path)
{
	u32 x = 0;
	u32 y = 0;
...
__iram_func__
static void _iqk_sram_8832crvu(struct rf_info *rf, u8 path)
{
...
__iram_func__
static void _iqk_rxk_setting_8832crvu(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	//u32 tmp = 0x0;
	...
	case CHANNEL_WIDTH_40:		
		...
		halrf_rxck_force_8832crvu(rf, path, true, ADC_480M);
	...
	case CHANNEL_WIDTH_80:
		...
		halrf_wreg(rf, 0x5670 + (path << 13), 0x00002000, 0x1);
		halrf_rxck_force_8832crvu(rf, path, true, ADC_960M);
	...
	case CHANNEL_WIDTH_160:
		...
		halrf_wreg(rf, 0x5670 + (path << 13), 0x00002000, 0x1);
		halrf_rxck_force_8832crvu(rf, path, true, ADC_1920M);
...
__iram_func__
static bool _iqk_check_cal_8832crvu(struct rf_info *rf, u8 path, u8 ktype)
{
...
__iram_func__
static bool _iqk_one_shot_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path, u8 ktype)
{
	...
	halrf_delay_us(rf, 1);
	fail = _iqk_check_cal_8832crvu(rf, path, ktype);

	switch(ktype ) {
	case ID_TXK:
		if (iqk_info->iqk_xym_en == true)
			_iqk_read_xym_8832crvu(rf, path);
		if (iqk_info->iqk_cfir_en == true) {
			_iqk_read_txcfir_8832crvu(rf, path, 0x1);
			_iqk_read_txcfir_8832crvu(rf, path, 0x2);
			_iqk_read_txcfir_8832crvu(rf, path, 0x3);
		}
		break;
	case ID_RXK:		
		if (iqk_info->iqk_sram_en == true)
			_iqk_sram_8832crvu(rf, path);
		if (iqk_info->iqk_xym_en == true)
			_iqk_read_xym_8832crvu(rf, path);
		if (iqk_info->iqk_cfir_en == true) {
			_iqk_read_rxcfir_8832crvu(rf, path, 0x0);
			_iqk_read_rxcfir_8832crvu(rf, path, 0x1);
			_iqk_read_rxcfir_8832crvu(rf, path, 0x2);
			_iqk_read_rxcfir_8832crvu(rf, path, 0x3);
		}
		break;
	default:
		break;
...
__iram_func__
static bool _rxk_group_sel_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{
	...
	for (gp = 0; gp < 0x4; gp++) {
		...
		fail = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_RXK);
...
__iram_func__
static bool _iqk_nbrxk_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
			      u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 gp = 0x0;
	...
	fail = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_NBRXK);
	halrf_wreg(rf, 0x9fe0, BIT(16 + gp + path * 4), fail);
	kfail = _iqk_check_nbiqc_8832crvu(rf, phy_idx, ID_NBRXK, path);
...
__iram_func__
static bool _txk_group_sel_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{
	...
	if (iqk_info->iqk_band[path] == BAND_ON_6G) {
		for (gp = 0x0; gp < 0x1; gp++) {
			...
			fail = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_TXK);
...
__iram_func__
static bool _iqk_nbtxk_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
			      u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool fail = false;
	...
	u32 a6_track_range[3] = {0x3, 0x3, 0x3};
	u32 a6_gain_bb[3] = {0x12,0x0a, 0x0f};
	...
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	fail = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_NBTXK);
	kfail = _iqk_check_nbiqc_8832crvu(rf, phy_idx, ID_NBTXK, path);
	if (!kfail) {
...
__iram_func__
static void _lok_res_table_8832crvu(struct rf_info *rf, u8 path, u8 ibias)
{
...
__iram_func__
static bool _lok_finetune_check_8832crvu(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
static bool _iqk_lok_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
			    u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	//bool fail = false;
...
//============START : _lok_coarse_fine_wi_swap ========
	...
	tmp = _iqk_one_shot_8832crvu(rf, phy_idx, path, tmp_id);
...
//Step 2
	...
	tmp = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_FLOK_vbuffer);
...
//Step 4 large rf gain
	...
	isfail = _lok_finetune_check_8832crvu(rf, path);
	return isfail;
}

__iram_func__
static void _iqk_txk_setting_8832crvu(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
static void _iqk_txclk_setting_8832crvu(struct rf_info *rf, u8 path)
{
#if 0
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
...
__iram_func__
static void _iqk_info_iqk_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path)
{
	...
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	iqk_info->thermal[path] = halrf_get_thermal_8832crvu(rf, path);	
...
__iram_func__
static void _iqk_disable_rxagc_8832crvu(struct rf_info *rf,u8 path, u8 en_rxgac)
{
...
__iram_func__
static void _iqk_by_path_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
				u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
#if 0	
	_iqk_txk_setting_8832crvu(rf, path);
	lok_is_fail = _iqk_lok_8832crvu(rf, phy_idx, path);
	iqk_info->iqk_tx_fail[0][path] = _iqk_nbtxk_8832crvu(rf, phy_idx, path);
	_iqk_rxk_setting_8832crvu(rf, path);
	iqk_info->iqk_rx_fail[0][path] =
			_iqk_nbrxk_8832crvu(rf, phy_idx, path);
#endif
#if 1
	//LOK
	if (iqk_info->iqk_band[path] == BAND_ON_24G)
	...
	for (i = 0; i < 3; i++) {
		_lok_res_table_8832crvu(rf, path, ibias + i*2);
		_iqk_txk_setting_8832crvu(rf, path);
		lok_is_fail = _iqk_lok_8832crvu(rf, phy_idx, path);
		if (!lok_is_fail)			
			break;
	...
	if (iqk_info->is_nbiqk) {
		iqk_info->iqk_tx_fail[0][path] =
			_iqk_nbtxk_8832crvu(rf, phy_idx, path);
	} else {
		iqk_info->iqk_tx_fail[0][path] =
			_txk_group_sel_8832crvu(rf, phy_idx, path);
	}
	//RX
	_iqk_rxk_setting_8832crvu(rf, path);
	if (iqk_info->is_nbiqk) {
		...
		iqk_info->iqk_rx_fail[0][path] =
			_rxk_group_sel_8832crvu(rf, phy_idx, path);
	}
#endif
	_iqk_info_iqk_8832crvu(rf, phy_idx, path);
	return;
}

__iram_func__
void iqk_backup_rf0_8832crvu(
	struct rf_info *rf, u8 path,
	u32 backup_rf0[rf_reg_num_8832crvu],
	u32 backup_rf_reg0[rf_reg_num_8832crvu])
{
	u8 i;
	...
	if(path != RF_PATH_A)
		return;
	RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	for (i = 0; i < rf_reg_num_8832crvu; i++) {
...
__iram_func__
void iqk_backup_rf1_8832crvu(
	struct rf_info *rf, u8 path,
	u32 backup_rf1[rf_reg_num_8832crvu],
	u32 backup_rf_reg1[rf_reg_num_8832crvu])
{
	u8 i;
...
__iram_func__
void iqk_set_info_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path)
{
...
__iram_func__
bool iqk_mcc_page_sel_8832crvu(struct rf_info *rf, enum phl_phy_idx phy, u8 path)
{
#if 0
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool flag = false;
	...
	if (rf->hal_com->band[phy].cur_chandef.center_ch ==  iqk_info->iqk_mcc_ch[0][path]) {
		...
		_iqk_mcc_load_lok_8832crvu(rf, phy, path, iqk_info->lok_idac[0][path]);
		flag = true;
		...
		_iqk_mcc_load_lok_8832crvu(rf, phy, path, iqk_info->lok_idac[1][path]);
		flag = true;
	} else
		flag = false;
#endif	
...
__iram_func__
void iqk_get_ch_info_8832crvu(struct rf_info *rf, enum phl_phy_idx phy, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	halrf_wreg(rf, 0x9fe0, 0xff000000, iqk_version_8832crvu);
	//2G5G6G = 0/1/2
	...
	ver = (u8) halrf_get_8832crvu_nctl_reg_ver();
	halrf_wreg(rf, 0x9fe8, 0x000000ff, ver);
	return;
}

__iram_func__
void halrf_iqk_reload_8832crvu(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u32 tmp;
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
...
__iram_func__
void iqk_start_iqk_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{

	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	_iqk_by_path_8832crvu(rf, phy_idx, path);
	return;
}

__iram_func__
void iqk_restore_8832crvu(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool fail;
	...
	fail = _iqk_check_cal_8832crvu(rf, path, 0x12);
	...
	//WA fro 52C
	if(iqk_info->iqk_band[path] == BAND_ON_6G) {
		halrf_iqk_lok_bypass_8832crvu(rf,  RF_PATH_A);
		halrf_iqk_lok_bypass_8832crvu(rf,  RF_PATH_B);
		halrf_iqk_tx_bypass_8832crvu(rf,  RF_PATH_A);
		halrf_iqk_tx_bypass_8832crvu(rf,  RF_PATH_B);
		halrf_iqk_rx_bypass_8832crvu(rf,  RF_PATH_A);
		halrf_iqk_rx_bypass_8832crvu(rf,  RF_PATH_B);
	}
#endif
...
__iram_func__
void iqk_afebb_restore_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{
	RF_DBG(rf, DBG_RF_IQK, "===> %s\n", __func__);
	switch(path) {
		case RF_PATH_A:	
	...
	//enable rxgac
	_iqk_disable_rxagc_8832crvu(rf, path, 0x1);
	return;
}

__iram_func__
void iqk_preset_8832crvu(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 idx = 0;
...
__iram_func__
void iqk_macbb_setting_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{
	RF_DBG(rf, DBG_RF_IQK, "[IQK]===> %s\n", __func__);
	...
	//disable rxgac
	_iqk_disable_rxagc_8832crvu(rf, path, 0x0);
	...
	halrf_txck_force_8832crvu(rf, path, true, DAC_960M);
	halrf_wreg(rf, 0x5670 | (path << 13), 0x00002000, 0x1);
	halrf_rxck_force_8832crvu(rf, path, true, ADC_1920M);
	halrf_wreg(rf, 0x5670 | (path << 13), 0x60000000, 0x2);
	halrf_wreg(rf, 0xc0d4 | (path << 8), 0x0c000000, 0x1);
...
__iram_func__
void halrf_iqk_toneleakage_8832crvu(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u32 a6_power_range[3] = {0x0, 0x0, 0x0};
	...
	iqk_get_ch_info_8832crvu(rf, HW_PHY_0, path);
	iqk_macbb_setting_8832crvu(rf, HW_PHY_0, path);	
	iqk_preset_8832crvu(rf, path);
...
__iram_func__
void halrf_iqk_dbcc_8832crvu(struct rf_info *rf, u8 path)
{
#if 0
	bool bkdbcc = false;	
	u8 phy_idx = 0x0;
	...
	//iqk_mcc_page_sel_8832crvu(rf,phy_idx, path);		
	iqk_get_ch_info_8832crvu(rf,phy_idx, path);
	iqk_macbb_setting_8832crvu(rf, phy_idx, path);
	iqk_preset_8832crvu(rf, path);
	iqk_start_iqk_8832crvu(rf, phy_idx, path);
	iqk_restore_8832crvu(rf, path);
	iqk_afebb_restore_8832crvu(rf, phy_idx, path);	
...
__iram_func__
void halrf_iqk_onoff_8832crvu(struct rf_info *rf, bool is_enable)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_tx_bypass_8832crvu(struct rf_info *rf, u8 path)
{
...
__iram_func__
void halrf_iqk_rx_bypass_8832crvu(struct rf_info *rf, u8 path)
{
	if (path == RF_PATH_A) { //path A
		/*ch0*/
		halrf_wreg(rf, 0x8124, MASKDWORD, 0x0);
...
__iram_func__
void halrf_iqk_lok_bypass_8832crvu(struct rf_info *rf, u8 path)
{
...
__iram_func__
void halrf_nbiqk_enable_8832crvu(struct rf_info *rf, bool nbiqk_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_xym_enable_8832crvu(struct rf_info *rf, bool iqk_xym_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_fft_enable_8832crvu(struct rf_info *rf, bool iqk_fft_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_sram_enable_8832crvu(struct rf_info *rf, bool iqk_sram_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_cfir_enable_8832crvu(struct rf_info *rf, bool iqk_cfir_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	struct rf_info *rf)
{
	...
	for (path = 0; path < 1; path++) {
		cur_ther = halrf_get_thermal_8832crvu(rf, path);
...
__iram_func__
bool halrf_iqk_get_ther_rek_8832crvu(struct rf_info *rf ) {
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
u8 halrf_iqk_get_mcc_ch0_8832crvu(struct rf_info *rf ) {
...
__iram_func__
u8 halrf_iqk_get_mcc_ch1_8832crvu(struct rf_info *rf ) {
...
__iram_func__
bool halrf_check_fwiqk_done_8832crvu(struct rf_info *rf)
{
	u32 counter = 0x0;
	bool flag = false;
	bool isfail = false;
...
__iram_func__
void halrf_enable_fw_iqk_8832crvu(struct rf_info *rf, bool is_fw_iqk)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__ 
u8 halrf_get_iqk_times_8832crvu(struct rf_info *rf) {
	u8 times  = 0x0;
...
__iram_func__ 
u32 halrf_get_iqk_ver_8832crvu(void)
{
	return iqk_version_8832crvu;
}

__iram_func__
void iqk_init_8832crvu(struct rf_info *rf)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 idx, path;
	...
	if (!iqk_info->is_iqk_init) {
		...
		for (idx = 0; idx < 2; idx++) { //channel		
			...
			for (path = 0; path < ss_8832crvu; path++) {//path
...
u8 halrf_iqk_get_rxevm_8832crvu(struct rf_info *rf)
{
	u8 rxevm =0x0;
#if 1
...
u32 halrf_iqk_get_rximr_8832crvu(struct rf_info *rf, u8 path, u32 idx)
{
	u32 rximr =0x0;
	u32 tone_idx = 0x0;
	u32 main_idx = 0x0;
...
#if 0
__iram_func__
void halrf_doiqk_8832crvu(struct rf_info *rf, bool force, enum phl_phy_idx phy_idx, u8 path)
{
	...
	iqk_info->version = iqk_version_8832crvu;
	iqk_info->iqk_step = 0;
	...
	if (iqk_mcc_page_sel_8832crvu(rf,phy_idx, path))
		return;
	iqk_get_ch_info_8832crvu(rf,phy_idx, path);
	iqk_backup_mac_bb_8832crvu(rf, path, backup_mac, backup_bb, backup_mac_reg, backup_bb_reg);
	iqk_backup_rf0_8832crvu(rf, path, backup_rf0, backup_rf_reg0);
	iqk_backup_rf1_8832crvu(rf, path, backup_rf1, backup_rf_reg1);
	iqk_macbb_setting_8832crvu(rf, phy_idx, path);
	iqk_preset_8832crvu(rf, path);
	iqk_start_iqk_8832crvu(rf, phy_idx, path);
	iqk_restore_8832crvu(rf, path);
	iqk_afebb_restore_8832crvu(rf, phy_idx, path);	
	iqk_restore_mac_bb_8832crvu(rf, path, backup_mac, backup_bb, backup_mac_reg, backup_bb_reg);
	iqk_restore_rf0_8832crvu(rf, path, backup_rf0, backup_rf_reg0);
	iqk_restore_rf1_8832crvu(rf, path, backup_rf1, backup_rf_reg1);
	iqk_info->iqk_times++;
...
__iram_func__
void halrf_iqk_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx, bool force)
{
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	iqk_init_8832crvu(rf);
	switch(halrf_kpath_8832crvu(rf, phy_idx)) {
	case RF_A:
		halrf_doiqk_8832crvu(rf, force, phy_idx, RF_PATH_A);
		break;
	case RF_B:		
		halrf_doiqk_8832crvu(rf, force, phy_idx, RF_PATH_B);
		break;
	case RF_AB:		
		halrf_doiqk_8832crvu(rf, force, phy_idx, RF_PATH_A);
		halrf_doiqk_8832crvu(rf, force, phy_idx, RF_PATH_B);
