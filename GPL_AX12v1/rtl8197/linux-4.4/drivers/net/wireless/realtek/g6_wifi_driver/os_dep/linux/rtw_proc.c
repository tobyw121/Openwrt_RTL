...
#define PHYDM_MSG_LEN	80*2400*4
				  ...
				  enum rtw_proc_cmd_type type)
{
	...
	if (buffer) {
		...
		if (type == RTW_PROC_CMD_CORE)
			_phl_proc_core_cmd(padapter, &cmd, phydm_msg, PHYDM_MSG_LEN);
		else
			rtw_phl_proc_cmd(GET_HAL_INFO(dvobj), type, &cmd, phydm_msg, PHYDM_MSG_LEN);
