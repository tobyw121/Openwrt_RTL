...
#endif
	return _dpk_sync_check_8852c(rf, path, kidx);
}
	...
	u8 kidx)
{
	...
	if (halrf_rreg(rf, 0x80ec, BIT(8)) == 0x1)
		_dpk_set_mdpd_para_8852c(rf, 0x2);	/*force 5,0,0*/
	else if (halrf_rreg(rf, 0x80ec, BIT(9)) == 0x1)
		_dpk_set_mdpd_para_8852c(rf, 0x1);	/*force 5,3,0*/
	else if (halrf_rreg(rf, 0x80ec, BIT(10)) == 0x1)
		_dpk_set_mdpd_para_8852c(rf, 0x0);	/*force 5,3,1*/
	else if (dpk->bp[path][kidx].bw == CHANNEL_WIDTH_5 ||
		 dpk->bp[path][kidx].bw == CHANNEL_WIDTH_10 ||
		 dpk->bp[path][kidx].bw == CHANNEL_WIDTH_20)
		_dpk_set_mdpd_para_8852c(rf, 0x2);	/*5,0,0*/
	else if(dpk->bp[path][kidx].bw == CHANNEL_WIDTH_40 ||
		dpk->bp[path][kidx].bw == CHANNEL_WIDTH_80)
		_dpk_set_mdpd_para_8852c(rf, 0x1);	/*5,3,0*/
	else
		_dpk_set_mdpd_para_8852c(rf, 0x0);	/*5,3,1*/
#if 0
	if (halrf_rreg(rf, 0x80ec, BIT(8)) == 0x1)
		_dpk_set_mdpd_para_8852c(rf, 0x2); /*5,0,0*/
	else if (halrf_rreg(rf, 0x80ec, BIT(9)) == 0x1)
	...
	u8 kidx)
{
	...
	halrf_wreg(rf, 0x81bc + (path << 8) + (kidx << 2),
			BIT(26) | BIT(25) | BIT(24), _dpk_order_convert_8852c(rf));
	...
	enum rf_path path)
{
	...
	halrf_set_rx_dck_8852c(rf, phy, path, false);
