...
{
	u32 d, s, i;
	for (i=0; i<4; i++) { /* AES_BLOCK_SIZE/sizeof(u32) */
		d = get_unaligned((u32 *)dst+i);
		s = get_unaligned((u32 *)src+i);
		d ^= s;
		put_unaligned(d, (u32 *)dst+i);
	}
}
			       ...
			       u8 *x)
{
	...
	rtk_aes_encrypt(aes, b, x); /* X_1 = E(K, B_0) */
	...
	rtk_aes_encrypt(aes, aad_buf, x); /* X_2 = E(K, X_1 XOR B_1) */

	if (aad_len > AES_BLOCK_SIZE - 2) {
		xor_aes_block(&aad_buf[AES_BLOCK_SIZE], x);
		/* X_3 = E(K, X_2 XOR B_2) */
		rtk_aes_encrypt(aes, &aad_buf[AES_BLOCK_SIZE], x);
	}
}
