...
enum rtw_phl_status rtw_phl_ser_fw_rsvd_dump(void *phl)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct hal_info_t *hal_info = (struct hal_info_t *)(phl_info->hal);

	PHL_TRACE(COMP_PHL_DBG, _PHL_ERR_, "rtw_phl_ser_fw_rsvd_dump !!!\n");

	rtw_hal_dump_fw_rsvd_ple(hal_info);
...
enum rtw_phl_status phl_fw_watchdog_timeout_notify(void *phl)
{
	...
	rtw_hal_fw_dbg_dump(phl_info->hal, 0);
