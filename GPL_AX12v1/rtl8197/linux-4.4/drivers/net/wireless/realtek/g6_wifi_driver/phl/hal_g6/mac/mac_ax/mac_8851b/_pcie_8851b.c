...
static struct mac_ax_intf_info intf_info_def_8851b = {
	MAC_AX_BD_TRUNC,
	MAC_AX_BD_TRUNC,
	MAC_AX_RXBD_PKT,
	MAC_AX_TAG_MULTI,
...
static struct mac_ax_pcie_ltr_param pcie_ltr_param_def_8851b = {
	0,
	0,
	MAC_AX_PCIE_ENABLE,
	MAC_AX_PCIE_ENABLE,
...
static struct mac_ax_pcie_cfgspc_param pcie_cfgspc_param_def_8851b = {
	0,
	0,
	MAC_AX_PCIE_DISABLE,
	MAC_AX_PCIE_ENABLE,
...
struct txbd_ram mac_bdram_tbl_8851b[] = {
		/* ACH0_QUEUE_IDX_8851BE */ {0, 5, 2},
		/* ACH1_QUEUE_IDX_8851BE */ {5, 5, 2},
		/* ACH2_QUEUE_IDX_8851BE */ {10, 5, 2},
		/* ACH3_QUEUE_IDX_8851BE */ {15, 5, 2},
		/* MGQ_B0_QUEUE_IDX_8851BE */ {20, 4, 1},
		/* HIQ_B0_QUEUE_IDX_8851BE */ {24, 4, 1},
		/* FWCMD_QUEUE_IDX_8851BE */ {28, 4, 1}
};
			    ...
			    struct mac_ax_pcie_cfgspc_param *param)
{
	...
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
#endif
	struct mac_ax_pcie_cfgspc_param *param_def = &pcie_cfgspc_param_def_8851b;
	...
	if (param->clkdly_ctrl != MAC_AX_PCIE_CLKDLY_IGNORE) {
		...
		if (ret != MACSUCCESS)
			return ret;
		if (tmp8 == PROC_LONG_DLY)
			clk_val = PCIE_CLKDLY_HW_500US;
#endif
			 ...
			 struct mac_ax_pcie_ltr_param *param)
{
	...
	struct mac_ax_pcie_ltr_param *param_def = &pcie_ltr_param_def_8851b;
	u32 val32;
...
u32 poll_dma_all_idle_pcie_8851b(struct mac_ax_adapter *adapter)
{
	...
	ret = poll_txdma_ch_idle_pcie_8851b(adapter, &txch_map);
	if (ret != MACSUCCESS) {
	...
	ret = poll_rxdma_ch_idle_pcie_8851b(adapter, &rxch_map);
	if (ret != MACSUCCESS) {
...
static u32 get_target_8851b(struct mac_ax_adapter *adapter, u16 *target,
				   ...
				   enum mac_ax_pcie_func_ctrl en)
{
	...
	ret = get_target_8851b(adapter, &tar, phy_rate);
	if (ret != MACSUCCESS) {
