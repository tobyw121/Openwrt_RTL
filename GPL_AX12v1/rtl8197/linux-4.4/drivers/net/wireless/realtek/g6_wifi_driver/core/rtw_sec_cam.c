...
s16 rtw_camid_alloc(_adapter *adapter, struct sta_info *sta, u8 kid, u8 gk, bool ext_sec, bool *used)
{
	...
	*used = _FALSE;
	...
	if ((MLME_IN_AP_STATE(mlmeinfo) || MLME_IS_STATE(mlmeinfo, WIFI_FW_ADHOC_STATE))
	    && !sta) {
		/*
...
void flush_all_cam_entry(struct _ADAPTER *a)
{
...
void rtw_iface_bcmc_sec_cam_map_restore(_adapter *adapter)
{
	...
	cam_id = rtw_phl_wrole_bcmc_id_get(GET_HAL_INFO(dvobj), adapter->phl_role);
...
int get_ax_address_cam(void* sel, struct _ADAPTER *a)
{
	...
	for (i = 0; i < loop_num; i++) {
		...
		#ifndef CONFIG_RTW_LINK_PHL_MASTER
		// 74b8420fe3902923e3a50f668c20b0b3c08beacd freddie.ho
		rtl_phl_dump_cam(phl, RTW_CAM_ADDR, i, addr_map);
		#endif /* CONFIG_RTW_LINK_PHL_MASTER */
...
int get_ax_sec_cam(void* sel, struct _ADAPTER *a)
{
	...
	for (i = 0; i < loop_num; i++) {
		...
		#ifndef CONFIG_RTW_LINK_PHL_MASTER
		// 74b8420fe3902923e3a50f668c20b0b3c08beacd freddie.ho
		rtl_phl_dump_cam(phl, RTW_CAM_SECURITY, i, sec_map);
		#endif /* CONFIG_RTW_LINK_PHL_MASTER */
		...
		, u8 sec_entry, u8 key_type, u8 keyid)
{
	...
	RTW_PRINT_SEL(sel, "%-5u %s "MAC_FMT"  " MAC_FMT" " MAC_FMT\
		...
		, enc_algo_to_string[GET_AX_SEC_CAM_TYPE(sec_map)>=ARRAY_SIZE(enc_algo_to_string)?0:GET_AX_SEC_CAM_TYPE(sec_map)] \
