...
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(4, 11, 12)) && !defined(CPTCFG_VERSION)
...
#if defined(CPTCFG_VERSION) || (KERNEL_VERSION(4, 7, 0) <= LINUX_VERSION_CODE)
...
#if (KERNEL_VERSION(4, 7, 0) <= LINUX_VERSION_CODE) || defined(CPTCFG_VERSION)
		struct cfg80211_scan_info info;
		...
		struct vif_params *params)
{
	...
	case NL80211_IFTYPE_MESH_POINT:
                ...
                if (padapter != NULL) {
                        ...
                        if (!padapter->netif_up) {
                                cfg80211_rtw_change_iface(wiphy, ndev, type, NULL);
                                wdev = ndev->ieee80211_ptr;
...
void rtw_cfg80211_rx_mframe(_adapter *adapter, union recv_frame *rframe, const char *msg)
{
	...
	RTW_INFO("RTW_Rx:ch=%d(%d), ta="MAC_FMT"\n", ch, sch, MAC_ARG(get_addr2_ptr(frame)));
	status = rtw_sae_preprocess(adapter, frame, frame_len, _FALSE);
	if (status== _FAIL) {
		if (msg)
			RTW_INFO("RTW_Rx:%s\n", msg);
		else
