...
/*8832BRVT DPK ver:0xa 20220420*/

void _dpk_bkup_kip_8832brvt(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][DPK_KIP_REG_NUM_8832BRVT],
	u8 path)
{
	u8 i;

	for (i = 0; i < DPK_KIP_REG_NUM_8832BRVT; i++) {
		reg_bkup[path][i] = halrf_rreg(rf, reg[i] + (path << 8), MASKDWORD);
		if (DPK_REG_DBG_8832BRVT)
			RF_DBG(rf, DBG_RF_DPK, "[DPK] Backup 0x%x = %x\n", reg[i]+ (path << 8), reg_bkup[path][i]);
	}
}

void _dpk_bkup_bb_8832brvt(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[DPK_BB_REG_NUM_8832BRVT])
{
	u8 i;

	for (i = 0; i < DPK_BB_REG_NUM_8832BRVT; i++) {
...
void _dpk_bkup_rf_8832brvt(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][DPK_RF_REG_NUM_8832BRVT],
	u8 path)
{
	u8 i;

	for (i = 0; i < DPK_RF_REG_NUM_8832BRVT; i++) {
...
void _dpk_information_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path)
{
...
void _dpk_rxagc_onoff_8832brvt(
	struct rf_info *rf,
	enum rf_path path,
	bool turn_on)
{
...
void _dpk_bb_afe_setting_8832brvt(
	struct rf_info *rf,
	enum rf_path path)
{
...
void _dpk_bb_afe_restore_8832brvt(
	struct rf_info *rf,
	enum rf_path path)
{
...
void _dpk_txpwr_bb_force_8832brvt(
	struct rf_info *rf,
	enum rf_path path,
	bool is_force)
{
...
void _dpk_kip_pwr_clk_onoff_8832brvt(
	struct rf_info *rf,
	bool turn_on)
{
	if (turn_on) {
...
void _dpk_kip_control_rfc_8832brvt(
	struct rf_info *rf,
	enum rf_path path,
	bool ctrl_by_kip)
{
	...
	u8 kidx)
{
	...
	_dpk_kip_control_rfc_8832brvt(rf, path, true);
	halrf_wreg(rf, 0x8104 + (path << 8), BIT(8), kidx); /*ch0/ch1 selection*/

	_dpk_one_shot_8832brvt(rf, phy, path, D_KIP_PRESET);
}

void _dpk_kip_restore_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path)
{
	_dpk_one_shot_8832brvt(rf, phy, path, D_KIP_RESTORE);
	...
	//_dpk_kip_control_rfc_8832brvt(rf, path, false);
	_dpk_txpwr_bb_force_8832brvt(rf, path, false);
#if 0
...
void _dpk_read_rxsram_8832brvt(
	struct rf_info *rf)
{
	u32 addr;
...
void _dpk_rf_reg_query_8832brvt(
	struct rf_info *rf,
	enum rf_path path,
	u32 reg)
{
	...
	_dpk_kip_control_rfc_8832brvt(rf, path, (bool)ori_ctrl);
#endif
}

void _dpk_kset_query_8832brvt(
	struct rf_info *rf,
	enum rf_path path)
{
	struct halrf_dpk_info *dpk = &rf->dpk;
	...
	u8 kidx)
{
	...
	//_dpk_rf_reg_query_8832brvt(rf, path, 0x00);
...
void _dpk_kip_set_txagc_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 dbm,
...
{	
	...
	_dpk_one_shot_8832brvt(rf, phy, path, D_TXAGC);
	_dpk_kset_query_8832brvt(rf, path);
	//_dpk_rf_reg_query_8832brvt(rf, path, 0x11);
}

bool _dpk_kip_set_rxagc_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx)
{
	...
	_dpk_one_shot_8832brvt(rf, phy, path, D_RXAGC);
#if 0
	halrf_wreg(rf, 0x80d4, 0x000F0000, 0x8);
	RF_DBG(rf, DBG_RF_DPK, "[DPK] set RXBB = 0x%x\n",
		halrf_rreg(rf, 0x80fc, 0x0000001F));
#endif
	return _dpk_sync_check_8832brvt(rf, path, kidx);
}

void _dpk_lbk_rxiqk_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	bool is_bypass)
{
	...
	_dpk_one_shot_8832brvt(rf, phy, path, LBK_RXIQK);
...
void _dpk_get_thermal_8832brvt(struct rf_info *rf, u8 kidx, enum rf_path path)
{
	struct halrf_dpk_info *dpk = &rf->dpk;
	...
	dpk->bp[path][kidx].ther_dpk = halrf_get_thermal_8832brvt(rf, path);
...
void _dpk_bypass_rxiqc_8832brvt(
	struct rf_info *rf,
	enum rf_path path)
{
	halrf_wreg(rf, 0x81a0 + (path << 8), BIT(7), 0x1);
...
s8 _dpk_dgain_mapping_8832brvt(
	struct rf_info *rf,
	u16 dgain)
{
...
u8 _dpk_gainloss_read_8832brvt(
	struct rf_info *rf)
{
	u8 result;
...
u8 _dpk_gainloss_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx)
{
	_dpk_one_shot_8832brvt(rf, phy, path, D_GAIN_LOSS);
	_dpk_kip_set_txagc_8832brvt(rf, phy, path, 0xff, false); /*set new tx power*/

	/*for k4 hw_bug*/
	halrf_wreg(rf, 0x81f0 + (path << 8), 0x0003FFFF, 0x0); /*[17:0]*/
	halrf_wreg(rf, 0x81f0 + (path << 8), 0xF0000000, 0x0); /*[31:28]*/

	return _dpk_gainloss_read_8832brvt(rf);
}

u8 _dpk_pas_read_8832brvt(
	struct rf_info *rf,
	enum rf_path path,
	u8 is_check)
{
	...
	u8 loss_only)
{
	...
	do {
		...
		case 0: /*SYNC and Dgain*/
			is_fail = _dpk_kip_set_rxagc_8832brvt(rf, phy, path, kidx);

			if (DPK_RXSRAM_DBG_8832BRVT)
				_dpk_read_rxsram_8832brvt(rf);
			...
			dgain = _dpk_dgain_read_8832brvt(rf);

			if (agc_cnt == 0) {
				if (rf->hal_com->band[phy].cur_chandef.bw < 2)
					_dpk_lbk_rxiqk_8832brvt(rf, phy, path, true);
				else
					_dpk_lbk_rxiqk_8832brvt(rf, phy, path, false);
			}
			i = 2;
			break;
		...
		case 1: /*Gain adjustment*/
			...
			offset = _dpk_dgain_mapping_8832brvt(rf, dgain);
		...
		case 2: /*GAIN_LOSS and idx*/
			tmp_gl_idx = _dpk_gainloss_8832brvt(rf, phy, path, kidx);
			if (DPK_PAS_DBG_8832BRVT)
				_dpk_pas_read_8832brvt(rf, path, false);

			if ((_dpk_pas_read_8832brvt(rf, path, true) == 2) && (tmp_gl_idx > 0))
				i = 4;
			else if ((tmp_gl_idx == 0 && _dpk_pas_read_8832brvt(rf, path, true) == 1) || tmp_gl_idx >= 7)
				i = 3; /*GL > criterion*/
		...
		case 3: /*GL > criterion*/
			if (tmp_dbm <= 7) {
				...
				tmp_dbm = tmp_dbm - 3;
				_dpk_kip_set_txagc_8832brvt(rf, phy, path, tmp_dbm, true);
			}
			i = 0;
			agc_cnt++;
			break;
...
void _dpk_set_mdpd_para_8832brvt(
	struct rf_info *rf,
	u8 order)
{
	switch (order) {
	...
	u8 kidx)
{
	...
	if (halrf_rreg(rf, 0x80ec, BIT(8)) == 0x1)
		_dpk_set_mdpd_para_8832brvt(rf, 0x0); /*5,3,1*/
	else if (halrf_rreg(rf, 0x80ec, BIT(9)) == 0x1)
		_dpk_set_mdpd_para_8832brvt(rf, 0x1); /*5,3,0*/
	else
		_dpk_set_mdpd_para_8832brvt(rf, 0x2); /*5,0,0*/

	halrf_wreg(rf, 0x809c, BIT(8), 0x0);

	_dpk_one_shot_8832brvt(rf, phy, path, D_MDPK_IDL);

	if (DPK_MDPK_DBG_8832BRVT)
		_dpk_rxsram_8832brvt(rf, path);

	if (halrf_get_8832brvt_nctl_reg_ver() >= 0xd) {
		_dpk_one_shot_8832brvt(rf, phy, path, 0x33);
		if (DPK_MDPK_DBG_8832BRVT)
			_dpk_rc_mtx_8832brvt(rf, path);

		_dpk_one_shot_8832brvt(rf, phy, path, D_MDPK_LDL);
	}

	is_rek = _dpk_rek_check_8832brvt(rf, phy, path, kidx, dpk->rek_cnt[path][0]);
#if 1
...
u8 _dpk_order_convert_8832brvt(
	struct rf_info *rf)
{
	u8 val;
...
void _dpk_gain_normalize_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx,
	bool is_execute)
{
	...
	if (is_execute) {
		...
		_dpk_one_shot_8832brvt(rf, phy, path, D_GAIN_NORM);
	} else
		halrf_wreg(rf, reg[kidx][dpk->cur_k_set] + (path << 8), 0x0000007F, 0x5b);
	...
	u8 kidx)
{
	...
	halrf_wreg(rf, 0x81bc + (path << 8) + (kidx << 2),
			BIT(26) | BIT(25) | BIT(24), _dpk_order_convert_8832brvt(rf));
	...
	_dpk_gain_normalize_8832brvt(rf, phy, path, kidx, false);
	//_dpk_para_query_8832brvt(rf, path, kidx);
}

bool _dpk_reload_check_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path)
{
	...
	enum rf_path path)
{
	...
	_dpk_kip_control_rfc_8832brvt(rf, path, false);
	halrf_rf_direct_cntrl_8832brvt(rf, path, false); /*switch control to direct write*/
	halrf_wrf(rf, path, 0x10005, MASKRF, 0x03ffd); /*only keep BB control TX_POWER*/

	_dpk_rf_setting_8832brvt(rf, path, kidx);
	halrf_set_rx_dck_8832brvt(rf, phy, path, false);

	_dpk_kip_pwr_clk_onoff_8832brvt(rf, true);
	_dpk_kip_preset_8832brvt(rf, phy, path, kidx);
	_dpk_txpwr_bb_force_8832brvt(rf, path, true);
	_dpk_kip_set_txagc_8832brvt(rf, phy, path, init_xdbm, true);
	_dpk_tpg_sel_8832brvt(rf, path, kidx);
#if 0
	is_fail = _dpk_kip_set_rxagc_8832brvt(rf, phy, path, kidx);
	...
	_dpk_gainloss_8832brvt(rf, phy, path, kidx);
	//_dpk_para_query_8832brvt(rf, path, kidx);
#else
	is_fail = _dpk_agc_8832brvt(rf, phy, path, kidx, init_xdbm, false);

	if (is_fail)
		goto _error;
#endif
	/*_dpk_pas_read_8832brvt(rf, path, false);*/
	//_dpk_get_thermal_8832brvt(rf, kidx, path);

	is_fail = _dpk_idl_mpa_8832brvt(rf, phy, path, kidx);
	_dpk_para_query_8832brvt(rf, path, kidx);
	...
	//_dpk_get_thermal_8832brvt(rf, kidx, path);
#if 0
	_dpk_coef_read_8832brvt(rf, path, kidx, gain);
#endif
	_dpk_on_8832brvt(rf, phy, path, kidx);
_error:
	_dpk_kip_control_rfc_8832brvt(rf, path, false);
	...
	u8 kpath)
{
	...
	u32 kip_bkup[DPK_RF_PATH_MAX_8832BRVT][DPK_KIP_REG_NUM_8832BRVT] = {{0}};
	//u32 bb_bkup[DPK_BB_REG_NUM_8832BRVT] = {0};
	u32 rf_bkup[DPK_RF_PATH_MAX_8832BRVT][DPK_RF_REG_NUM_8832BRVT] = {{0}};

	u32 kip_reg[] = {0x813c, 0x8124, 0x8120, 0xc0c4, 0xc0e8, 0xc0d4, 0xc0d8, 0xc0ec};
	//u32 bb_reg[] = {0x2344, 0x5800, 0x7800};
	u32 rf_reg[DPK_RF_REG_NUM_8832BRVT] = {0xde, 0xdf, 0x8b, 0x90, 0x96,
					     0x97, 0x85, 0x5, 0x10005};

	u8 path;
	bool is_fail = true;
	bool reloaded[DPK_RF_PATH_MAX_8832BRVT] = {false};

	if ((!phl_is_mp_mode(rf->phl_com)) && DPK_RELOAD_EN_8832BRVT) {
		for (path = 0; path < DPK_RF_PATH_MAX_8832BRVT; path++) {
			reloaded[path] = _dpk_reload_check_8832brvt(rf, phy, path);
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8832BRVT; path++) {
		if (kpath & BIT(path)) {
			_dpk_bkup_kip_8832brvt(rf, kip_reg, kip_bkup, path);
			_dpk_bkup_rf_8832brvt(rf, rf_reg, rf_bkup, path);
			_dpk_information_8832brvt(rf, phy, path);
			_dpk_init_8832brvt(rf, path);
			if (rf->is_tssi_mode[path])
				_dpk_tssi_pause_8832brvt(rf, path, true);
		}
	}
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8832BRVT; path++) {
		if (kpath & BIT(path)) {
			...
			_dpk_rxagc_onoff_8832brvt(rf, path, false);
			halrf_drf_direct_cntrl_8832brvt(rf, path, false);
			_dpk_bb_afe_setting_8832brvt(rf, path);
			is_fail = _dpk_main_8832brvt(rf, phy, path);
			halrf_dpk_onoff_8832brvt(rf, path, is_fail);
		}
	}

	for (path = 0; path < DPK_RF_PATH_MAX_8832BRVT; path++) {
		if (kpath & BIT(path)) {
			_dpk_kip_restore_8832brvt(rf, phy, path);
			_dpk_reload_kip_8832brvt(rf, kip_reg, kip_bkup, path);
			_dpk_reload_rf_8832brvt(rf, rf_reg, rf_bkup, path);
			_dpk_bb_afe_restore_8832brvt(rf, path);
			_dpk_rxagc_onoff_8832brvt(rf, path, true);
			if (rf->is_tssi_mode[path])
				_dpk_tssi_pause_8832brvt(rf, path, false);
		}
	}
	_dpk_kip_pwr_clk_onoff_8832brvt(rf, false);
}

u8 _dpk_bypass_check_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy)
{
...
void _dpk_force_bypass_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy)
{
	u8 path, kpath;

	kpath = halrf_kpath_8832brvt(rf, phy);

	for (path = 0; path < DPK_RF_PATH_MAX_8832BRVT; path++) {
		if (kpath & BIT(path))
			halrf_dpk_onoff_8832brvt(rf, path, true);
	}
}

void halrf_dpk_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	bool force)
{
	RF_DBG(rf, DBG_RF_DPK, "[DPK] ****** DPK Start (Ver: 0x%x, Cv: %d, RF_para: %d, NCTL: 0x%x) ******\n",
		DPK_VER_8832BRVT, rf->hal_com->cv, halrf_get_radio_ver_from_reg(rf),
		halrf_get_8832brvt_nctl_reg_ver());

	RF_DBG(rf, DBG_RF_DPK, "[DPK] Driver mode = %d\n", rf->phl_com->drv_mode);
#if 1
	if (_dpk_bypass_check_8832brvt(rf, phy))
		_dpk_force_bypass_8832brvt(rf, phy);
	else
		_dpk_cal_select_8832brvt(rf, force, phy, halrf_kpath_8832brvt(rf, phy));
#else
	//_dpk_information_8832brvt(rf, 0, RF_PATH_A);
	//halrf_drf_direct_cntrl_8832brvt(rf, RF_PATH_A, false);
	//_dpk_bb_afe_setting_8832brvt(rf, RF_PATH_A);
	
	//_dpk_main_8832brvt(rf, phy, RF_PATH_A);
	//halrf_delay_us(rf, 100);
	halrf_rf_direct_cntrl_8832brvt(rf, RF_PATH_A, false); /*switch control to direct write*/
	//halrf_delay_us(rf, 100);
	//halrf_wrf(rf, RF_PATH_A, 0x10005, MASKRF, 0x03ffd); /*only keep BB control TX_POWER*/
	_dpk_rf_setting_8832brvt(rf, RF_PATH_A, 0);
	//halrf_delay_us(rf, 70);
	halrf_set_rx_dck_8832brvt(rf, phy, RF_PATH_A, false);
	//_dpk_kip_pwr_clk_onoff_8832brvt(rf, true);
	//_dpk_kip_preset_8832brvt(rf, phy, RF_PATH_A, 0);
	//_dpk_txpwr_bb_force_8832brvt(rf, RF_PATH_A, true);
	//_dpk_kip_set_txagc_8832brvt(rf, phy, RF_PATH_A, 21, true);
	//_dpk_tpg_sel_8832brvt(rf, RF_PATH_A, 0);
	//_dpk_kip_set_rxagc_8832brvt(rf, phy, RF_PATH_A, 0);


#endif
}

void halrf_dpk_onoff_8832brvt(
	struct rf_info *rf,
	enum rf_path path,
	bool off)
{
