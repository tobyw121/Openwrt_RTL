	...
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, u32 gain_stage)
{
...
#else
	halrf_do_one_shot_8852c(rf, path, 0x8000, MASKDWORD, process_id1[path]);
	...
	//halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
#else
	halrf_do_one_shot_8852c(rf, path, 0x8000, MASKDWORD, process_id2[path]);
#endif
	...
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	...
	//if (channel >= 1 && channel <= 14)
		if (band == BAND_ON_24G)
			...
			_halrf_txgapk_track_table_nctl_5g_8852c(rf, phy, path, gain_stage_6g[group_of_6g]);		
	}
}
	...
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	...
	if (!is_dbcc) {
		...
		halrf_one_shot_nctl_done_check_8852c(rf, path);
#else
		halrf_do_one_shot_8852c(rf, path, 0x8000, MASKDWORD, process_id[path]);
#endif
