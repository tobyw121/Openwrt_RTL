...
void rtw_hal_bf_dbg_dump_entry_all(void *hal)
{
	...
	PHL_PRINT("-----DUMP BF BUSY LIST-----\n");
	if(list_empty(busy_list)) {
		PHL_PRINT("BF Entry BUSY LIST is Empty\n");
	} else {
		phl_list_for_loop(bf_entry, struct hal_bf_entry, busy_list,
					list) {
			PHL_PRINT("bf_entry->bf_idx 0x%x \n", bf_entry->bf_idx);
			PHL_PRINT("bf_entry->macid 0x%x \n", bf_entry->macid);
			PHL_PRINT("bf_entry->aid12 0x%x \n", bf_entry->aid12);
			PHL_PRINT("bf_entry->band 0x%x \n", bf_entry->band);
			PHL_PRINT("bf_entry->en_swap 0x%x \n", bf_entry->en_swap ? 1 : 0);
			PHL_PRINT("bf_entry->counter 0x%x \n", bf_entry->couter);
			PHL_PRINT("bf_entry->csi_buf 0x%x \n",
					bf_entry->csi_buf);
			PHL_PRINT("bf_entry->csi_buf_swap 0x%x \n",
					bf_entry->csi_buf_swap);
			if (NULL!=bf_entry->bfee) {
				PHL_PRINT("bfee->idx 0x%x \n",
					bf_entry->bfee->idx);
				PHL_PRINT("bfee->type 0x%x \n",
					bf_entry->bfee->type);
			}
		}
	}
	PHL_PRINT("-----DUMP BF IDLE LIST-----\n");
	if(list_empty(idle_list)) {
		PHL_PRINT("BF Entry IDLE LIST is Empty\n");
	} else {
				...
				void *entry)
{
	...
	if (bf_obj != NULL && bf_entry != NULL) {
		_os_spinlock(drv_priv, &bf_obj->bf_lock, _bh, NULL);
		if (bf_entry->bfee != NULL) {
			...
			_os_spinunlock(drv_priv, &bf_obj->bf_lock, _bh, NULL);
			_reset_sumu_entry(bfee);
			_os_spinlock(drv_priv, &bf_obj->bf_lock, _ps, NULL);
			list_del(&bfee->list);
			_os_spinunlock(drv_priv, &bf_obj->bf_lock, _ps, NULL);
			if (bfee->type == HAL_BFEE_MU) {
				_enqueue_idle_mu_entry(
					hal_info, bf_obj,bfee);
			} else {
...

{
	...
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
	struct hal_bf_obj *bf_obj = (struct hal_bf_obj *)hal_com->bf_obj;
	struct hal_bf_entry *bf_entry = (struct hal_bf_entry *)hal_sta->bf_entry;
	u8 bfee_idx = 0;
	u16 csi_buf = 0;
...
{
	struct hal_bf_entry *bf_entry = (struct hal_bf_entry *)sta->hal_sta->bf_entry;
  	if(bf_entry)
    {
		bf_entry->gid = gid;
		bf_entry->pid = pid;
