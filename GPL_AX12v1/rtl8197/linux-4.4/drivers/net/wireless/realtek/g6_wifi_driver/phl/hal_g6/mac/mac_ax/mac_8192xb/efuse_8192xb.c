...
u32 switch_efuse_bank_8192xb(struct mac_ax_adapter *adapter,
...
void enable_efuse_sw_pwr_cut_8192xb(struct mac_ax_adapter *adapter,
				    bool is_write)
{
	u16 val16;
	u8 val8;
	...
	if (dv_sel == DDV) {
		...
		MAC_REG_W16(R_AX_SYS_ISO_CTRL, val16 | B_AX_PWC_EV2EF_S);

		PLTFM_DELAY_US(1000);

		val16 = MAC_REG_R16(R_AX_SYS_ISO_CTRL);
		MAC_REG_W16(R_AX_SYS_ISO_CTRL, val16 | B_AX_PWC_EV2EF_B);
...
void disable_efuse_sw_pwr_cut_8192xb(struct mac_ax_adapter *adapter,
				     bool is_write)
{
	u16 value16;
	u8 value8;
	...
	if (dv_sel == DDV) {
		...
		value16 = MAC_REG_R16(R_AX_SYS_ISO_CTRL);
		MAC_REG_W16(R_AX_SYS_ISO_CTRL, value16 & ~(B_AX_PWC_EV2EF_B));
		PLTFM_DELAY_US(1000);

		value16 = MAC_REG_R16(R_AX_SYS_ISO_CTRL);
		MAC_REG_W16(R_AX_SYS_ISO_CTRL, value16 & ~(B_AX_PWC_EV2EF_S));
