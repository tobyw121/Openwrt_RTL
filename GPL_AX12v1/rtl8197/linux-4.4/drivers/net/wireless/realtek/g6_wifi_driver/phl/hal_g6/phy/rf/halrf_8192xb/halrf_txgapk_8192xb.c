...
void _txgapk_backup_bb_registers_8192xb(
	struct rf_info *rf,
	u32 *reg,
	u32 *reg_bkup,
	u8 reg_num)
...
void _txgapk_backup_kip_8192xb(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][TXGAPK_KIP_REG_NUM_8192XB],
	u8 path,
	u8 reg_num)
{
	u8 i;
...
void _txgapk_bkup_rf_8192xb(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][TXGAPK_RF_REG_NUM_8192XB],
	u8 path,
	u8 reg_num)
{
	u8 i;
...
void _txgapk_reload_bb_registers_8192xb(
	struct rf_info *rf,
	u32 *reg,
	u32 *reg_backup,
	u8 reg_num)
...
void _txgapk_reload_kip_8192xb(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][TXGAPK_KIP_REG_NUM_8192XB],
	u8 path,
...
void _halrf_txgapk_rxagc_onoff_8192xb(
	struct rf_info *rf,
	enum rf_path path,
	bool turn_on)
{
...
void _halrf_txgapk_bb_afe_by_mode_8192xb(struct rf_info *rf,
...
//			halrf_wreg(rf, 0xc0d8, 0x000001e0, 0x7);
			...
			halrf_wreg(rf, 0xc1ec, 0x00006000, 0x0);
...
void _halrf_txgapk_iqk_preset_by_mode_8192xb(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
	...
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	...
	halrf_wrf(rf, path, 0x83, 0x000f0, 0xe);
}


void _halrf_txgapk_track_table_nctl_2g_8192xb
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	...
	u32 gain_stage =  0x0001554;
	

	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);

	if (rfe_type > 50) //efem
		halrf_wreg(rf, 0x80e4, 0x0000003f, 0x3c);
	else
		halrf_wreg(rf, 0x80e4, 0x0000003f, 0x20);	
	...
	if (rfe_type > 50) //efem
		halrf_wreg(rf, 0x8038, 0x003f0000, 0x09);
	else
	...
	halrf_wreg(rf, itqt[path], 0x0000003f, 0x09);
	halrf_wreg(rf, 0x802c, 0x0fff0000, 0x009);

	#if 0
	halrf_wreg(rf, 0x8000, MASKDWORD, process_id1[path]);
	ODM_delay_us(100);
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	#else
	halrf_do_one_shot_8192xb(rf, path, 0x8000, MASKDWORD, process_id1[path]);
	...
	#if 0
	halrf_wreg(rf, 0x8000, MASKDWORD, process_id2[path]);
	ODM_delay_us(100);
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	#else
	halrf_do_one_shot_8192xb(rf, path, 0x8000, MASKDWORD, process_id2[path]);
	#endif
	...
	halrf_wreg(rf, itqt[path], 0x0000003f, 0x2d); //ItQt
...
void _halrf_txgapk_write_track_table_default_2g_8192xb
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	...
	for (i = 0; i < 17; i++) {
		...
		RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]	track	ta[%d][%d]=0x%x\n",
			path, i, txgapk_info->track_ta[path][i]);
	#else
		RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]	track	ta(6bit)[%d][%d]=0x%x\n",
			path, i, calcu_ta[i]);
	#endif
	}
...
void _halrf_txgapk_power_table_nctl_2g_8192xb
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	 u8 rfe_type = rf->phl_com->dev_cap.rfe_type; 
	...
	if (rfe_type > 50)
		halrf_wreg(rf, gapk_on_tbl_setting[path], 0x001fffff, 0x4000);
	else
		halrf_wreg(rf, gapk_on_tbl_setting[path], 0x001fffff, gain_stage);
...
void _halrf_txgapk_write_power_table_default_2g_8192xb
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	//New reset table method for 52c later series	
...
void _halrf_txgapk_iqk_bk_reg_by_mode_8192xb
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	...
	if (!is_dbcc) {
		...
		halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
		#else
		halrf_do_one_shot_8192xb(rf, path, 0x8000, MASKDWORD, process_id[path]);
...
void _halrf_txgapk_restore_iqk_kmod_bb_8192xb
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_txgapk_main_8192xb(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	//it doesn't need to write default
	

	//V01_BB_AFE_DPK_S0_20210913
	_halrf_txgapk_bb_afe_by_mode_8192xb(rf, phy, path, is_dbcc);
	
	//V02_IQK_Preset_path0_20210804	
	_halrf_txgapk_iqk_preset_by_mode_8192xb(rf, phy, path, is_dbcc);
	
	//V03_S0_RF_for_DPK_20210924.txt
	_halrf_txgapk_rf_dpk_2g_8192xb(rf, phy, path);

	
	//v05a_A_S0_GapK_Track_32BR.txt
	_halrf_txgapk_track_table_nctl_2g_8192xb(rf, phy, path);

	//v05b_A_S0_GapK_Power_32BR.txt
	_halrf_txgapk_power_table_nctl_2g_8192xb(rf, phy, path);

	//v98_S0_IQK_Reg_Restore_52C_20210804
	_halrf_txgapk_iqk_bk_reg_by_mode_8192xb(rf, phy, path, is_dbcc);

	//v99_BB_AFE_DPK_S0_restore_20210913.txt	
	_halrf_txgapk_restore_iqk_kmod_bb_8192xb(rf, phy, path, is_dbcc);
		
}



void _halrf_do_reset_tbl_txgapk_8192xb(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 path;
	
	u32 kip_bkup[TXGAPK_RF_PATH_MAX_8192XB][TXGAPK_KIP_REG_NUM_8192XB] = {{0}};
	//u32 bb_bkup[TXGAPK_BB_REG_NUM_8192XB] = {0};
	u32 rf_bkup[TXGAPK_RF_PATH_MAX_8192XB][TXGAPK_RF_REG_NUM_8192XB] = {{0}};
	...
	#if 0 //no need 
	_txgapk_backup_bb_registers_8192xb(rf, bb_reg, bb_bkup, TXGAPK_BB_REG_NUM_8192XB);
	#endif

	for (path = 0; path < TXGAPK_RF_PATH_MAX_8192XB; path++) {	
		_txgapk_backup_kip_8192xb(rf, kip_reg, kip_bkup, path, TXGAPK_KIP_REG_NUM_8192XB);
		_txgapk_bkup_rf_8192xb(rf, rf_reg, rf_bkup, path, TXGAPK_RF_REG_NUM_8192XB);
	}				
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8192XB; path++) {
		...
		//follow dpk flow
		_halrf_txgapk_rxagc_onoff_8192xb(rf, path, false);
	

		//_halrf_txgapk_main2(rf, phy, path, false);
		//main2 =========================
		//v01_BB_AFE_DPK_S0_20210913
		_halrf_txgapk_bb_afe_by_mode_8192xb(rf, phy, path, false);
		
		//v02_IQK_Preset_path0_20210804	
		_halrf_txgapk_iqk_preset_by_mode_8192xb(rf, phy, path, false);
		
		//v03_S0_RF_for_DPK_20210924.txt
		_halrf_txgapk_rf_dpk_2g_8192xb(rf, phy, path);		

		//v04_RF_RxDCK_S0_20210818.txt
		//_halrf_txgapk_rf_rxdck_2g_8192xb(rf, phy, path);
		
		//v05a_A_S0_GapK_Track_32BR.txt
		_halrf_txgapk_write_track_table_default_2g_8192xb(rf, phy, path);
		
		//v05b_A_S0_GapK_Power_32BR.txt
		_halrf_txgapk_write_power_table_default_2g_8192xb(rf, phy, path);
	
		//V 98_S0_IQK_Reg_Restore_52C_20210804
		_halrf_txgapk_iqk_bk_reg_by_mode_8192xb(rf, phy, path, false);

		//V99_BB_AFE_DPK_S0_restore_20210913.txt		
		_halrf_txgapk_restore_iqk_kmod_bb_8192xb(rf, phy, path, false);
		//======= end =======	
	}
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8192XB; path++) {
		_txgapk_reload_kip_8192xb(rf, kip_reg, kip_bkup, path, TXGAPK_KIP_REG_NUM_8192XB);
		_txgapk_reload_rf_8192xb(rf, rf_reg, rf_bkup, path, TXGAPK_RF_REG_NUM_8192XB);

		//follow dpk flow
		_halrf_txgapk_rxagc_onoff_8192xb(rf, path, true);
	}

	#if 0 //no need 
	_txgapk_reload_bb_registers_8192xb(rf, bb_reg, bb_bkup, TXGAPK_BB_REG_NUM_8192XB);
	#endif
}



void _halrf_do_non_dbcc_txgapk_8192xb(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 path;
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8192XB; path++) {
		...
		_halrf_txgapk_main_8192xb(rf, phy, path, false);
	}
...
void _halrf_do_dbcc_txgapk_8192xb(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	enum rf_path path = 0;
...
void _halrf_txgapk_get_ch_info_8192xb(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
...
void  _halrf_sel_hw_table_txgapk_8192xb(struct rf_info *rf, bool is_hw) 
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s \n", __func__);
...
void halrf_do_txgapk_8192xb(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
	
	_halrf_txgapk_get_ch_info_8192xb(rf, phy);
	...
	//_halrf_sel_hw_table_txgapk_8192xb(rf, phy, false); //use sw control
	
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s   phy=%d, dbcc_en = %d, rfe_type = %d\n", __func__, phy, rf->hal_com->dbcc_en, rf->phl_com->dev_cap.rfe_type);
	...
	if (rf->hal_com->dbcc_en)
		_halrf_do_dbcc_txgapk_8192xb(rf, phy);
	else
		_halrf_do_non_dbcc_txgapk_8192xb(rf, phy);
...
void halrf_txgapk_init_8192xb(struct rf_info *rf) 
{	
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	...
	if(!txgapk_info->is_gapk_init) {
		...
		_halrf_sel_hw_table_txgapk_8192xb(rf, false); //use sw control			
	}
}


#if 0
void halrf_txgapk_enable_8192xb
...
void halrf_txgapk_write_table_default_8192xb
	(struct rf_info *rf, enum phl_phy_idx phy)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s   phy=%d\n", __func__, phy);
	_halrf_do_reset_tbl_txgapk_8192xb(rf, phy);
}
