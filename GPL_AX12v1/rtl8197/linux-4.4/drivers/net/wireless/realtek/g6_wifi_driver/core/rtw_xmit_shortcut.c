...
void txsc_recycle_txreq_phyaddr(_adapter *padapter, struct rtw_xmit_req *txreq)
{
	...
	if (ptxreq_buf) {
		...
		pkt_list++;
		for (idx = 1; idx < ptxreq_buf->pkt_cnt; idx++) {
...
#ifdef CONFIG_VW_REFINE
	if (padapter->vw_enable || is_client_mode) {
...
get_txsc_entry:
	...
	if (txsc_pkt->step == TXSC_AMSDU_APPLY)
		...
		txsc_pkt->amsdu = false;
		if (txsc_pkt->is_amsdu_timeout) 	/* Exception case */
			res = _FAIL;
...
u8 txsc_apply_sc_cached_entry(_adapter *padapter, struct txsc_pkt_entry *txsc_pkt, u8 vw_pkt)
{
	...
	if (mdata->hw_seq_mode == 1)
		txreq_buf = (struct xmit_txreq_buf *)get_txreq_buffer(padapter, (u8 **)&txreq, (u8 **)&pkt_list, NULL, NULL, 1, psta);
	else
		txreq_buf = (struct xmit_txreq_buf *)get_txreq_buffer(padapter, (u8 **)&txreq, (u8 **)&pkt_list, (u8 **)&head, NULL, 1, psta);
	#else
		txreq_buf = (struct xmit_txreq_buf *)get_txreq_buffer(padapter, (u8 **)&txreq, (u8 **)&pkt_list, (u8 **)&head, NULL, 1, psta);
	#endif
	...
	/* WLAN header from cache */
		...
		if (is_hdr_need_update) {
			pkt_list0->vir_addr = ptxsc_wlhdr;
			pkt_list0->length = wlhdr_copy_len;
			#ifdef CONFIG_PCI_HCI
			txsc_fill_txreq_phyaddr(padapter, pkt_list0);
			#endif
			psta->txsc_wlhdr_copy++;
		}
		#endif
...
void txsc_amsdu_dump(_adapter *padapter, void *m)
{
	...
	for (i = 0; i < ARRAY_SIZE(pxmitpriv->cnt_txsc_amsdu_timeout_ok); i++) {
		if (pxmitpriv->cnt_txsc_amsdu_timeout_ok[i] != 0 ||
			...
			RTW_PRINT_SEL(m, "[amsdu] ac[%d] timeout ok/fail/bypass: %d / %d / %d\n", i,
