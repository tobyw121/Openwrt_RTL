...
static enum rtw_hal_status
_hal_bfer_init(struct hal_info_t *hal_info,
	       struct rtw_phl_stainfo_t *sta)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	struct rtw_wifi_role_t *wrole = NULL;
	bool enable_bfer = false;
	...
	do {
		...
		if ((wrole->proto_role_cap.he_su_bfmr ||
		    ...
		    (sta->asoc_cap.he_su_bfme || sta->asoc_cap.he_mu_bfme)) {
			enable_bfer = true;
		}

		if ((wrole->proto_role_cap.vht_su_bfmr ||
		     wrole->proto_role_cap.vht_mu_bfmr) &&
		    (sta->asoc_cap.vht_su_bfme || sta->asoc_cap.vht_mu_bfme)) {
			enable_bfer = true;
		}

		if (wrole->proto_role_cap.ht_su_bfmr &&
		    sta->asoc_cap.ht_su_bfme)
			enable_bfer = true;

		if (true == enable_bfer) {
			hal_rf_bf_config_rf(hal_info);
		}
		hstatus = RTW_HAL_STATUS_SUCCESS;
	} while (0);
				 ...
				 struct rtw_phl_stainfo_t *sta)
{
	...
	u16 cfg;
	enum rf_path path = (phl_com->phy_cap[0].tx_path_num == 1)?RF_PATH_B:RF_PATH_AB;
#endif
...
enum rtw_hal_status
rtw_hal_update_cctrl_tbl(struct hal_info_t *hal_info,
			    struct rtw_phl_stainfo_t *sta)
{
	struct rtw_wifi_role_t *wrole = NULL;
	enum rtw_hal_status sts = RTW_HAL_STATUS_FAILURE;
	...
	cctrl.mbssid = wrole->hw_mbssid;
	cctl_info_mask.mbssid = 0x0F;

	cctrl.multi_port_id = wrole->hw_port;
	cctl_info_mask.multi_port_id = 0x07;
	...
	*/
	...
	/* Apply STA's A-MPDU number if it has it in cap */
	if (sta->asoc_cap.num_ampdu) {
		PHL_INFO("M%u's A-MPDU number is %u\n",
		          sta->macid, sta->asoc_cap.num_ampdu);
	...
	if (cctrl.max_agg_num <= 0x3F)
		...
		cctrl.ba_bmap = 1; // 128
	else if (cctrl.max_agg_num > 0x7F)
		cctrl.ba_bmap = 2; // 256
	...
	if (IS_AP_ROLE_TYPE(sta->wrole->type)) {
...
#ifdef CONFIG_VW_REFINE
	if (adapter->rtsrate_cck)
	     cctrl.rtsrate = RTW_DATA_RATE_CCK1;
	else if ( 0 != adapter->vw_enable )
	     cctrl.rtsrate = RTW_DATA_RATE_OFDM54;
	else
	    cctrl.rtsrate = RTW_DATA_RATE_OFDM24;

	cctl_info_mask.rtsrate = 0x1FF;
	PHL_INFO("set rtsrate:0x%x\n", cctrl.rtsrate);
#endif
...
static enum rtw_hal_status
_hal_update_txpath_cctrl_tbl(struct hal_info_t *hal_info,
				struct rtw_phl_stainfo_t *sta, enum rf_path path)
{
	enum rtw_hal_status sts = RTW_HAL_STATUS_FAILURE;
	struct rtw_hal_mac_ax_cctl_info cctrl, cctl_info_mask;
	u16 cfg;
		      ...
		      struct rtw_phl_stainfo_t *sta)
{
	...
	/* Disable VLAN tag valid by default */
	dctrl.vlan_tag_valid = 0;
	dctl_info_mask.vlan_tag_valid = 1;
        ...
        struct rtw_phl_stainfo_t *sta)
{
	...
	csi_buf = (struct hal_csi_buf *)&sta->hal_sta->bf_csi_buf;
	csi_buf->sub_idx = CSI_BUF_SUB_IDX_NON;

	csi_buf = (struct hal_csi_buf *)&sta->hal_sta->bf_csi_buf_swap;
	csi_buf->sub_idx = CSI_BUF_SUB_IDX_NON;
...
rtw_hal_add_sta_entry(void *hal, struct rtw_phl_stainfo_t *sta)
{
	...
	rtw_hal_mac_start_h2c_agg(hal_info);
#endif
...
#ifdef CONFIG_ENABLE_MAC_H2C_AGG
	hal_status = rtw_hal_mac_stop_h2c_agg(hal_info, 1);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("[%s]rtw_hal_stop_h2c_agg failed\n", __func__);
		return hal_status;
	}
#endif
...
_exit:
#ifdef CONFIG_ENABLE_MAC_H2C_AGG
	rtw_hal_mac_stop_h2c_agg(hal_info, 0);
#endif
				...
				bool is_connect)
{
	...
	/*update cmac table*/
	if (RTW_HAL_STATUS_SUCCESS != rtw_hal_update_cctrl_tbl(hal_info, sta))
		PHL_WARN("rtw_hal_update_cctrl_tbl failed\n");
	...
	if (is_connect) {
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_bfer_init(hal_info, sta)) {
			PHL_ERR("_hal_bfer_init Fail!\n");
		}
						...
						enum phl_upd_mode mode)
{
	...
	hal_status = rtw_hal_mac_addr_cam_change_entry(hal_info, sta, mode, true);
	if (hal_status != RTW_HAL_STATUS_SUCCESS)
		PHL_ERR("rtw_hal_mac_addr_cam_change_entry failed\n");
...
rtw_hal_del_sta_entry(void *hal, struct rtw_phl_stainfo_t *sta)
{
	...
	hal_status = rtw_hal_snd_release_proc_sta_res(hal_info, sta);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "macid(%d) release bf failed\n", sta->macid);
	}
						...
						enum rf_path path)
{
	...
	/*update cmac table*/
	if (RTW_HAL_STATUS_SUCCESS != _hal_update_txpath_cctrl_tbl(hal_info,
		sta, path))
...
rtw_hal_get_edca(void *hal, struct rtw_wifi_role_t *wrole, u8 ac, u32 *edca)
{
	...
	hal_status = rtw_hal_mac_get_edca(hal_info->hal_com, wrole->hw_band,
					  wrole->hw_wmm, ac, edca);
...
rtw_hal_cfg_tx_ampdu(void *hal, struct rtw_phl_stainfo_t *sta)
{
	...
	if (64 == sta->asoc_cap.num_ampdu)
		hsts = rtw_hal_mac_set_hw_ampdu_cfg(hal_info, 0, 0x40, 0xA5);
	else if (128 == sta->asoc_cap.num_ampdu)
		hsts = rtw_hal_mac_set_hw_ampdu_cfg(hal_info, 0, 0x80, 0xA5);
	else if (256 == sta->asoc_cap.num_ampdu)
		hsts = rtw_hal_mac_set_hw_ampdu_cfg(hal_info, 0, 0x100, 0xA5);
	else
...
rtw_hal_query_txsts_rpt(void *hal, u16 macid0, u16 macid1)
{
	...
	if (RTW_HAL_STATUS_SUCCESS != rtw_hal_bb_query_txsts_rpt(hal_info, macid0, macid1))
		return RTW_HAL_STATUS_FAILURE;
	else
		return RTW_HAL_STATUS_SUCCESS;
}
