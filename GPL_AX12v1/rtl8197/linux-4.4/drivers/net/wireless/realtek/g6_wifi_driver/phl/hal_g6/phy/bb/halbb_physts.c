			   ...
			   struct physts_rxd *desc)
{
	...
	if (ch_physts->ch_info_state == CH_RPT_START_TO_WAIT) {
		...
		if (ch_physts->valid_ch_info_only_en) {
			...
			/* To avoid rxsc!= ch_info. -> partial ch_info. = 0*/
			if (!(psts_8->is_ch_info_len_valid && psts_8->rxsc == 0)) {
				return true;
			}

			/* To avoid HE40M error ch_len */
			if (physts->bb_physts_rslt_1_i.bw_idx == CHANNEL_WIDTH_40 &&
				...
				if (ch_physts->ch_info_len != err_len) {
					BB_DBG(bb, DBG_CH_INFO, "ch_physts->ch_info_len=%d != err_len=%d\n",
		...
		#if defined(BB_8852A_2_SUPPORT)
		/* tone-1 msb 8 bit is missing, fill with previous mem. content */
		if (cfg->ch_i_cmprs == 0) {
			   ...
			   struct physts_rxd *desc)
{
	...
	psts_10->sigb_raw_data_bits_addr = addr + 8;
	psts_10->sigb_len = ie_length - 8;


	if (bb->dbg_component & DBG_SNIFFER) {
		BB_DBG(bb, DBG_SNIFFER, "[Physts][IE-10] sigb_len=%d\n", psts_10->sigb_len);
		halbb_print_buff_64(bb, psts_10->sigb_raw_data_bits_addr, psts_10->sigb_len);
	}
...
PARSING_END:
	...
	if (desc->is_to_self) {
		...
		halbb_sniffer_phy_sts(bb, bb_rpt, physts_bitmap, desc);
		#else
		  ...
		  char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], "frc_mu") == 0) {
