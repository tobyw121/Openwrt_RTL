...
enum rtw_hal_status _hal_set_pcicfg_8192xbe(struct hal_info_t *hal_info)
{
...
enum rtw_hal_status _hal_ltr_sw_init_state_8192xbe(struct hal_info_t *hal_info)
{
...
enum rtw_hal_status _hal_ltr_set_pcie_8192xbe(struct hal_info_t *hal_info)
{
...
static void _hal_tx_ch_config_8192xbe(struct hal_info_t *hal)
{
...
static void _hal_pre_init_8192xbe(struct rtw_phl_com_t *phl_com,
				 struct hal_info_t *hal_info,
				 struct hal_init_info_t *init_92xbe)
{
	struct mac_ax_trx_info *trx_info = &init_92xbe->trx_info;
	struct mac_ax_intf_info *intf_info = &init_92xbe->intf_info;
	...
	_hal_tx_ch_config_8192xbe(hal_info);
	...
	/* others */
	if (hal_info->hal_com->chip_id == CHIP_WIFI6_8192XB)
		init_92xbe->ic_name = "rtl8192xbe";
	if (hal_info->hal_com->chip_id == CHIP_WIFI6_8832BR)
		init_92xbe->ic_name = "rtl8832bre";
}

void init_hal_spec_8192xbe(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal)
{
	...
	init_hal_spec_8192xb(phl_com, hal);
...
enum rtw_hal_status hal_get_efuse_8192xbe(struct rtw_phl_com_t *phl_com,
					 struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_92xbe;
	_os_mem_set(hal_to_drvpriv(hal_info), &init_92xbe, 0, sizeof(init_92xbe));
	_hal_pre_init_8192xbe(phl_com, hal_info, &init_92xbe);

	return hal_get_efuse_8192xb(phl_com, hal_info, &init_92xbe);
}


static u32 _hal_read32_8192xbe(struct rtw_hal_com_t *hal_com, u32 addr)
{
...
static u16 _hal_read16_8192xbe(struct rtw_hal_com_t *hal_com, u32 addr)
{
	u32 r_val = _hal_read32_8192xbe(hal_com, (addr & ~3));
	return (u16)(r_val >> (8 * (addr & 2)));
}

static u8 _hal_read8_8192xbe(struct rtw_hal_com_t *hal_com, u32 addr)
{
	u32 r_val = _hal_read32_8192xbe(hal_com, (addr & ~3));
	return (u8)(r_val >> (8 * (addr & 3)));
}

enum rtw_hal_status hal_init_8192xbe(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
	...
	#if 0
	/* Use chip level register read handling. No need for 32BR/92XB. */
	struct hal_io_priv *io_priv = &hal_com->iopriv;

	PHL_PRINT("Use chip layer register read functions!\n");
	io_priv->io_ops._read32 = _hal_read32_8192xbe;
	io_priv->io_ops._read16 = _hal_read16_8192xbe;
	io_priv->io_ops._read8 = _hal_read8_8192xbe;
	#endif
...
void hal_deinit_8192xbe(struct rtw_phl_com_t *phl_com,
		       struct hal_info_t *hal_info)
{
	u8 txch_num = 0;
...
enum rtw_hal_status hal_start_8192xbe(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_92xbe;
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	...
	hal_status = hal_start_8192xb(phl_com, hal_info, &init_92xbe);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_init_8192xb: status = %u\n",hal_status);
		return hal_status;
	}

	hal_status = _hal_set_pcicfg_8192xbe(hal_info);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("_hal_set_pcicfg_8192xbe: status = %u\n",hal_status);
		return hal_status;
	}

	hal_status = _hal_ltr_set_pcie_8192xbe(hal_info);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("_hal_ltr_set_pcie_8192xbe: status = %u\n",hal_status);
		return hal_status;
	}

	hal_status = _hal_ltr_sw_init_state_8192xbe(hal_info);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("_hal_ltr_sw_init_state_8192xbe: status = %u\n",hal_status);
		return hal_status;
	}
...
enum rtw_hal_status hal_stop_8192xbe(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	hal_status = hal_stop_8192xb(phl_com, hal);
	return hal_status;
}
...
enum rtw_hal_status
hal_wow_init_8192xbe(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
							struct rtw_phl_stainfo_t *sta)
{
	...
	*/
	...
	hal_status = hal_wow_init_8192xb(phl_com, hal_info, sta, &init_92xbe);
...
enum rtw_hal_status
hal_wow_deinit_8192xbe(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
								struct rtw_phl_stainfo_t *sta)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_92xbe;
	...
	*/
	...
	hal_status = hal_wow_deinit_8192xb(phl_com, hal_info, sta, &init_92xbe);

	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_wow_deinit_8192xbe: status = %u\n", hal_status);
		return hal_status;
	}

	if (RTW_HAL_STATUS_SUCCESS != _hal_set_pcicfg_8192xbe(hal_info))
		PHL_ERR("_hal_set_pcicfg_8192xbe: status = %u\n", hal_status);
...
u32 hal_hci_cfg_8192xbe(struct rtw_phl_com_t *phl_com,
		struct hal_info_t *hal, struct rtw_ic_info *ic_info)
{
	...
	/* not use work ring on eCos */
#if defined(RTW_WKARD_TXBD_UPD_LMT) && !defined(PHL_PLATFORM_ECOS)
...
static void _hal_dump_int_8192xbe(struct hal_info_t *hal, bool mask, bool imr, bool isr, const char *func)
{
...
#define HAL_DUMP_INT_8192XBE(hal, mask, imr, isr) \
		_hal_dump_int_8192xbe(hal, mask, imr, isr, __func__)

void hal_init_default_value_8192xbe(struct hal_info_t *hal)
{
	init_default_value_8192xb(hal);

	hal_init_int_default_value_8192xbe(hal, INT_SET_OPT_HAL_INIT);
}

void hal_init_int_default_value_8192xbe(struct hal_info_t *hal, enum rtw_hal_int_set_opt opt)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_enable_int_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	...
	HAL_DUMP_INT_8192XBE(hal, false, true, true);
}

void hal_disable_int_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
 ...
 */
void hal_disable_int_isr_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
 ...
 */
void hal_disable_int_rmn_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_clear_int_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
#if 0 /* 92xb do not clear this */
	if (hal_com->_intr_ind[0].en)
		hal_write32(hal_com, 0x30B4, hal_com->_intr_ind[0].val);
#endif
}

void hal_clear_mask_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_restore_int_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	if (hal_com->_intr_ind[0].en)
...
bool hal_recognize_int_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	 ...
	 * in wlan mac.
	 */
	hal_disable_int_isr_8192xbe(hal);
...
#ifndef CONFIG_SYNC_INTERRUPT
	/* clear isr */
	hal_clear_int_8192xbe(hal);
	/* restore imr */
	hal_restore_int_8192xbe(hal);
#endif /* CONFIG_SYNC_INTERRUPT */

	/*if (!recognized)
		HAL_DUMP_INT_8192XBE(hal, true, true, true);*/
...
static u32 hal_bcn_handler_8192xbe(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_rx_handler_8192xbe(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_tx_handler_8192xbe(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u32 handled0 = 0x0;
...
static u32 hal_cmd_handler_8192xbe(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_halt_c2h_handler_8192xbe(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_gt3_handler_8192xbe(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
...
static u32 hal_watchdog_timer_handler_8192xbe(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
u32 hal_int_hdler_8192xbe(struct hal_info_t *hal)
{
	...
	/* bit 1 : rx related */
	int_hdler_msk |= (hal_rx_handler_8192xbe(hal, handled)<<1);

	/* bit 2 : tx related */
	int_hdler_msk |= (hal_tx_handler_8192xbe(hal, handled)<<2);

	/* bit 4 : halt c2h */
	int_hdler_msk |= (hal_halt_c2h_handler_8192xbe(hal, handled) << 4);

	/* bit 5 : watchdog timeout */
	int_hdler_msk |= (hal_watchdog_timer_handler_8192xbe(hal, handled) << 5);

	/* <8> gt3 timer related; CONFIG_VW_REFINE || CONFIG_ONE_TXQ */
	int_hdler_msk |= (hal_gt3_handler_8192xbe(hal, handled) << 7);
...
void hal_rx_int_restore_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
enum rtw_hal_status
hal_mp_init_8192xbe(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_92xbe;
	...
	hal_status = hal_mp_init_8192xb(phl_com, hal_info, &init_92xbe);
...
enum rtw_hal_status
hal_mp_deinit_8192xbe(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_92xbe;
	...
	hal_status = hal_mp_deinit_8192xb(phl_com, hal_info, &init_92xbe);

	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_mp_deinit_8192xbe: status = %u\n", hal_status);
		return hal_status;
	}
