...
static void _set_halmac_tx_limit(struct btc_t *btc)
{
	...
	for (i = 0; i < MAX_WIFI_ROLE_NUMBER; i++) {
		...
		if (btc->hal->dbcc_en) {
			if (plink->phy != (u8)wl_rinfo->dbcc_2g_phy)
				continue;
		}
		...
		if (!enable) {
			/* restore the original tx-setup if no tx-limit */
			...
			/* set new tx-setup  if tx-limit on */
...
static void _set_dbcc_ant_ctrl(struct btc_t *btc)
{
	...
	if (is_dbcc_all_2g) {
		...
		_set_gnt_wl(btc, BIT(HW_PHY_0), BTC_GNT_SW_HI);
		_set_gnt_bt(btc, BIT(HW_PHY_0), BTC_GNT_SW_LO);
		plt_ctrl = BTC_PLT_NONE;
		_set_bt_plut(btc, BIT(HW_PHY_0), plt_ctrl, plt_ctrl);

		/* Set PHY-1(BTG) HW control */
		_set_gnt_wl(btc, BIT(HW_PHY_1), BTC_GNT_HW);
		_set_gnt_bt(btc, BIT(HW_PHY_1), BTC_GNT_HW);
	...
	for (i = 0; i < HW_PHY_MAX; i++) {
		...
		_set_gnt_bt(btc, BIT(i), BTC_GNT_HW);
