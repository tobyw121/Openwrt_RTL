...
u8 reg_read8_usb_1115e(struct mac_ax_adapter *adapter, u32 addr)
{
	return PLTFM_REG_R8(addr);
}

void reg_write8_usb_1115e(struct mac_ax_adapter *adapter, u32 addr, u8 val)
{
	PLTFM_REG_W8(addr, val);
}

u16 reg_read16_usb_1115e(struct mac_ax_adapter *adapter, u32 addr)
{
	return PLTFM_REG_R16(addr);
}

void reg_write16_usb_1115e(struct mac_ax_adapter *adapter, u32 addr, u16 val)
{
	PLTFM_REG_W16(addr, val);
}

u32 reg_read32_usb_1115e(struct mac_ax_adapter *adapter, u32 addr)
{
	return PLTFM_REG_R32(addr);
}

void reg_write32_usb_1115e(struct mac_ax_adapter *adapter, u32 addr, u32 val)
{
	PLTFM_REG_W32(addr, val);
}

u8 get_bulkout_id_1115e(struct mac_ax_adapter *adapter, u8 ch_dma, u8 mode)
{
	u8 bulkout_id = 0;
...
u32 usb_pre_init_1115e(struct mac_ax_adapter *adapter, void *param)
{
	u32 val32 = 0;

	val32 = PLTFM_REG_R32(R_AX_USB_HOST_REQUEST_2_V1) | B_AX_R_USBIO_MODE;
	PLTFM_REG_W32(R_AX_USB_HOST_REQUEST_2_V1, val32);
	// fix USB IO hang suggest by chihhanli@realtek.com
	val32 = PLTFM_REG_R32(R_AX_USB_WLAN0_1_V1) & ~B_AX_USBRX_RST & ~B_AX_USBTX_RST;
	...
	if (((PLTFM_REG_R32(R_AX_USB_ENDPOINT_3_V1) >> B_AX_AC_BULKOUT_SH) &
...
u32 usb_init_1115e(struct mac_ax_adapter *adapter, void *param)
{
	u32 val32;
	u8 val8;
	...
	adapter->usb_info.max_dma_txagg_msk = BE_TXD_DMA_TXAGG_NUM_MSK;
...
u32 usb_deinit_1115e(struct mac_ax_adapter *adapter, void *param)
{
	return MACSUCCESS;
}

u32 usb_flush_mode_1115e(struct mac_ax_adapter *adapter, u8 mode)
{
	u32 cnt, val32;

	if (mode == MAC_AX_FUNC_DIS) {
		val32 = PLTFM_REG_R32(R_AX_USB_WLAN0_1_V1) & ~B_AX_USBRX_RST
			& ~B_AX_USBTX_RST;
		PLTFM_REG_W32(R_AX_USB_WLAN0_1_V1, val32);
...
u32 read_usb2phy_para_1115e(struct mac_ax_adapter *adapter, u16 offset)
{
	u32 value32 = 0;
	u8 rdata = 0;
...
u32 write_usb2phy_para_1115e(struct mac_ax_adapter *adapter, u16 offset, u8 val)
{
	u32 value32 = 0;
...
u32 read_usb3phy_para_1115e(struct mac_ax_adapter *adapter, u16 offset, u8 b_sel)
{
	u32 value32 = 0, ret;
	u16 rdata = 0;
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
...
u32 write_usb3phy_para_1115e(struct mac_ax_adapter *adapter,
...
u32 u2u3_switch_1115e(struct mac_ax_adapter *adapter)
{
	u32 ret = 0;
...
u32 get_usb_support_ability_1115e(struct mac_ax_adapter *adapter)
{
	u32 u2force = 0;
	u32 u3force = 0;
...
u32 usb_tx_agg_cfg_1115e(struct mac_ax_adapter *adapter,
			 struct mac_ax_usb_tx_agg_cfg *agg)
{
	u32 dw1 = ((struct wd_body_usb_1115e *)agg->pkt)->dword1;

	((struct wd_body_usb_1115e *)agg->pkt)->dword1 =
		SET_CLR_WORD(dw1, agg->agg_num, AX_TXD_DMA_TXAGG_NUM_V1);
	return MACSUCCESS;
}

u32 usb_rx_agg_cfg_1115e(struct mac_ax_adapter *adapter,
...
#define COMPAT_RX_AGG_UNIT 4
	...
	val32 = MAC_REG_R32(R_BE_RXAGG_0_V1);
	MAC_REG_W32(R_BE_RXAGG_0_V1, (agg_en ? B_BE_RXAGG_EN : 0) |
		    (agg_mode ? B_BE_RXAGG_DMA_STORE : 0) |
		    (val32 & B_BE_RXAGG_SW_EN) |
		    SET_WORD(pkt_num == 0 ? B_BE_RXAGG_PKTNUM_TH_MSK : pkt_num,
			     B_BE_RXAGG_PKTNUM_TH) |
		    SET_WORD(timeout, B_BE_RXAGG_TIMEOUT_TH) |
		    SET_WORD(size, B_BE_RXAGG_LEN_TH));

	/* hw function not used, try to disable it */
	val32 = MAC_REG_R32(R_BE_RXAGG_1_V1);
	MAC_REG_W32(R_BE_RXAGG_1_V1, SET_WORD(0, B_BE_RXAGG_LEN_TH_HIGH_V1) |
		    SET_WORD(0, B_BE_RXAGG_SML_PKT_SIZE_V1) |
		    SET_WORD(B_BE_RXAGG_SML_PKTNUM_TH_MSK, B_BE_RXAGG_SML_PKTNUM_TH));
#undef COMPAT_RX_AGG_UNIT

	return MACSUCCESS;
}

u32 usb_pwr_switch_1115e(void *vadapter, u8 pre_switch, u8 on)
{
	return MACSUCCESS;
}

u32 set_usb_wowlan_1115e(struct mac_ax_adapter *adapter,
...
u32 usb_get_txagg_num_1115e(struct mac_ax_adapter *adapter, u8 band)
{
...
u32 usb_get_rx_state_1115e(struct mac_ax_adapter *adapter, u32 *val)
{
	u8 rxdma_cnt, ep_cnt, curr_rxdma_cnt, curr_ep_cnt;

	*val = 0;
	rxdma_cnt = GET_FIELD(PLTFM_REG_R32(R_AX_USB_DEBUG_1_V1), B_AX_RXDMA_DMA_COUNTER);
	ep_cnt = GET_FIELD(PLTFM_REG_R32(R_AX_USB_DEBUG_1_V1), B_AX_RXDMA_ENDPOINT_COUNTER);
	PLTFM_DELAY_MS(RX_POLLING_PERIOD);
	curr_rxdma_cnt = GET_FIELD(PLTFM_REG_R32(R_AX_USB_DEBUG_1_V1), B_AX_RXDMA_DMA_COUNTER);
	curr_ep_cnt = GET_FIELD(PLTFM_REG_R32(R_AX_USB_DEBUG_1_V1), B_AX_RXDMA_ENDPOINT_COUNTER);
...
u32 usb_ep_cfg_1115e(struct mac_ax_adapter *adapter, struct mac_ax_usb_ep *cfg)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
