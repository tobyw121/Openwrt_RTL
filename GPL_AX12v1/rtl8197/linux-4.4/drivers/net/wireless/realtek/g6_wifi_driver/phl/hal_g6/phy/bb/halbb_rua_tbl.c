			...
			struct halbb_dl_fix_sta_info_8ru *fix_sta_i)
{
	...
	fix_sta_i->fix_pwr_boost_factor_flag = sta_ent->fix_pwr_fac;
	halbb_ru_rate_cfg(bb, &(sta_ent->rate), &(fix_sta_i->rate_i));
	fix_sta_i->txbf = sta_ent->txbf;
	fix_sta_i->coding = sta_ent->coding;
...
u32 halbb_upd_dlru_fixtbl_ax8ru(struct bb_info *bb,
			struct rtw_dlru_fixtbl_ax8ru *info)
{
	...
	for (i = 0; i < HALBB_AX8RU_STA_NUM; i++)
		halbb_dlfix_sta_i_ax8ru_cfg(bb, &(info->sta[i]), &(fix_tbl_i->sta_i[i]));
	BB_DBG(bb, DBG_RUA_TBL, "content %x %x %x \n", *bb_h2c, *(bb_h2c), *(bb_h2c+2));
	ret_v = halbb_fill_h2c_cmd(bb, pkt_len, RUA_H2C_TABLE, HALBB_H2C_RUA, bb_h2c);
out:
	if (fix_tbl_i)
			...
			union rtw_dlru_fixtbl *union_info)
{
	...
	case BB_RTL8852C:
		ret = halbb_upd_dlru_fixtbl_ax8ru(bb, &(union_info->ax8ru));
		break;
	#endif
...
u32 halbb_upd_ulru_fixtbl_ax8ru(struct bb_info *bb,
			struct rtw_ulru_fixtbl_ax8ru *info)
{
	...
	for (i = 0; i < HALBB_AX8RU_STA_NUM; i++)
		halbb_ulfix_sta_i_ax8ru_cfg(bb, &(info->sta[i]), &(fix_tbl_i->sta_i[i]));
	BB_DBG(bb, DBG_RUA_TBL, "content %x %x %x \n", *bb_h2c, *(bb_h2c), *(bb_h2c+2));
	ret_v = halbb_fill_h2c_cmd(bb, pkt_len, RUA_H2C_TABLE, HALBB_H2C_RUA, bb_h2c);
out:
	if (fix_tbl_i)
			...
			union rtw_ulru_fixtbl *union_info)
{
	...
	#ifdef BB_8852C_SUPPORT
	case BB_RTL8852C:
		ret = halbb_upd_ulru_fixtbl_ax8ru(bb, &(union_info->ax8ru));
		break;
	#endif
...
u32 halbb_rua_rawread(struct bb_info *bb, u8 src_sel, u8 id, u8 ofst32){
	u32 ret = RTW_HAL_STATUS_FAILURE;
...
u32 halbb_rua_rawwrite(struct bb_info *bb, u8 src_sel, u8 id, u8 ofst32, u8 ofst8, u32 w_val){
	u32 ret = RTW_HAL_STATUS_FAILURE;

	u8 *bbinfo_i;
	u8 pkt_len = sizeof(u32)*2;
	u8 len = pkt_len;
	u32 *bb_h2c = NULL;
	//u8 i,j = 0;
	bool ret_v = false;
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], help) == 0) {
		...
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "{rua (9 src_sel id ofset [RUA C2H DBG raw read])}\n");			 
		goto out;
	}
	for (i = 0; i < 6; i++) {
		if (input[i + 1])
	...
	if (val[0] < 6)
		...
		halbb_rua_rawread(bb, (u8)val[1], (u8)val[2], (u8)val[3]);
	else if(val[0] == 255)
		halbb_rua_rawwrite(bb, (u8)val[1], (u8)val[2], (u8)val[3], (u8) val[4], (u8) val[5]);		
out:
	*_used = used;
	*_out_len = out_len;
}
