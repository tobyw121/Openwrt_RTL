...
void halbb_digital_cfo_comp_8852b(struct bb_info *bb, s32 curr_cfo)
{
	...
	BB_DBG(bb, DBG_CFO_TRK, "[8852B] cfo_avg_312 = %d\n", cfo_avg_312);
...
u8 halbb_ofdm_band_determine_8852b(struct bb_info *bb, u8 central_ch, enum band_type band_type, bool is_normal)
{
...
void halbb_set_gain_error_8852b(struct bb_info *bb, u8 central_ch, enum band_type band_type)
{
	...
	u32 lna_gain_mask[7] = {0x00ff0000, 0xff000000, 0x000000ff,
				    ...
				    0x000000ff};
	u32 lna_gain_a_bypass[BB_PATH_MAX_8852B][7] = {{0x4BB4, 0x4BB4, 0x4BB4,
	...
	u32 lna_gain_a_bypass_mask[7] = {0xff, 0xff00, 0xff0000, 0xff000000, 0xff,
				       0xff00, 0xff0000};
	u32 lna_gain_g_bypass[BB_PATH_MAX_8852B][7] = {{0x4BB8, 0x4BBC, 0x4BBC,
	...
	u32 lna_op1db_a[BB_PATH_MAX_8852B][7] = {{0x4668, 0x4668, 0x4668,
						 0x4668, 0x466c, 0x466c,
						 0x466c}, {0x474c, 0x474c,
						 0x474c, 0x474c, 0x4750,
						 0x4750, 0x4750}};
	u32 tia_lna_op1db_a[BB_PATH_MAX_8852B][7] = {{0x4670, 0x4670, 0x4670,
						     0x4670, 0x4674, 0x4674,
						     0x4674}, {0x4754, 0x4754,
						     0x4754, 0x4754, 0x4758,
						     0x4758, 0x4758}};
	u32 tia0_lna6_op1db_a[BB_PATH_MAX_8852B] = {0x4674, 0x4758};
	u32 op1db_a_mask[7] = {0xff, 0xff00, 0xff0000, 0xff000000, 0xff, 0xff00,
			       0xff0000};
	u32 lna_op1db_g[BB_PATH_MAX_8852B][7] = {{0x4684, 0x4684, 0x4684,
						 0x4688, 0x4688, 0x4688,
						 0x4688}, {0x4768, 0x4768,
						 0x4768, 0x476C, 0x476C,
						 0x476C, 0x476C}};
	u32 tia_lna_op1db_g[BB_PATH_MAX_8852B][7] = {{0x468C, 0x468C, 0x4690,
						     0x4690, 0x4690, 0x4690,
						     0x4694}, {0x4770, 0x4770,
						     0x4774, 0x4774, 0x4774,
						     0x4774, 0x4778}};
	u32 tia0_lna6_op1db_g[BB_PATH_MAX_8852B] = {0x4694, 0x4778};
	...
	band = halbb_ofdm_band_determine_8852b(bb, central_ch, band_type, false);

	if (bb->ic_sub_type == BB_IC_SUB_TYPE_8852B_8852B){
		for (path = RF_PATH_A; path < BB_PATH_MAX_8852B; path++) {
		...
		for (path = RF_PATH_A; path < BB_PATH_MAX_8852B; path++) {
			for (lna_idx = 0; lna_idx < 7; lna_idx++) {
					if (band_type == BAND_ON_24G) {
						...
						// Set op1dB value
						//if (bb->bb_gain_i.op1db_chk) {
						//	tmp = gain->lna_op1db[band][path][lna_idx];
						//	halbb_set_reg(bb, lna_op1db_g[path][lna_idx], op1db_g_mask[lna_idx], tmp);
						//	tmp = gain->tia_lna_op1db[band][path][lna_idx];
						//	halbb_set_reg(bb, tia_lna_op1db_g[path][lna_idx], tia_lna_op1db_g_mask[lna_idx], tmp);
						//}
					} else {
						// Set LNA value
						if (bb->bb_gain_i.gain_chk) {
...
#ifdef OP1DB_2G_DYNAMIC
					if (band_type == BAND_ON_24G)
						halbb_set_reg(bb, tia0_lna6_op1db_g[path], 0xff00, tmp);
					else
#else
					if (band_type != BAND_ON_24G)
#endif
...
void halbb_rx_op1db_dbg_8852b(struct bb_info *bb, char input[][16],
	...
	if (bb->ic_sub_type == BB_IC_SUB_TYPE_8852B_8852BP){
		if (_os_strcmp(input[2], "amode") == 0) {
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len -
			    *_used, "===[A mode][Path-%d]===\n", path);
			for (lna_idx = 0; lna_idx < 7; lna_idx++) {
				tmp = (s8)halbb_get_reg(bb, lna_op1db_a[path][lna_idx], op1db_a_mask[lna_idx]);
			...
			for (lna_idx = 0; lna_idx < 7; lna_idx++) {
				tmp = (s8)halbb_get_reg(bb, tia_lna_op1db_a[path][lna_idx], op1db_a_mask[lna_idx]);
				BB_DBG_CNSL(*_out_len, *_used, output + *_used,
				    *_out_len - *_used,
			...
			tmp = (s8)halbb_get_reg(bb, tia0_lna6_op1db_a[path], 0xff000000);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len -
			    *_used,
			...
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len -
			    *_used, "===[G mode][Path-%d]===\n", path);
			for (lna_idx = 0; lna_idx < 7; lna_idx++) {
				tmp = (s8)halbb_get_reg(bb, lna_op1db_g[path][lna_idx], op1db_g_mask[lna_idx]);
				BB_DBG_CNSL(*_out_len, *_used, output + *_used,
				    *_out_len - *_used,
				    "[LNA_op1db]Addr=0x%x, Bit mask=0x%x, Value=%d\n",
			...
			for (lna_idx = 0; lna_idx < 7; lna_idx++) {
				tmp = (s8)halbb_get_reg(bb, tia_lna_op1db_g[path][lna_idx], tia_lna_op1db_g_mask[lna_idx]);
				BB_DBG_CNSL(*_out_len, *_used, output + *_used,
				    *_out_len - *_used,
				    "[TIA_LNA_op1db]Addr=0x%x, Bit mask=0x%x, Value=%d\n",
