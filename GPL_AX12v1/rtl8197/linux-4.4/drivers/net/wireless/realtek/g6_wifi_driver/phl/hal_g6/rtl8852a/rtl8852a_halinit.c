					...
					struct hal_info_t *hal)
{
	...
	if (hal_com->cv < CCV)
		hal_com->dev_hw_cap.sta_ulru = RTW_HW_CAP_ULRU_DISABLE;
	else
	...
	hal_com->dev_hw_cap.ps_cap.lps_wow_cap =
		PS_CAP_PWRON | PS_CAP_RF_OFF | PS_CAP_CLK_GATED | PS_CAP_PWR_GATED;
	hal_com->dev_hw_cap.hw_stype_cap = EFUSE_HW_STYPE_NONE_8852A;
...
static u32 _hal_read_fw_8852a(enum rtw_fw_type fw_type,
...
static u32 _hal_cfg_extnal_fw_8852a(struct rtw_phl_com_t *phl_com,
	enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info, char *ic_name)
{
	...
	switch (fw_type) {
	case RTW_FW_NIC:
		_hal_read_fw_8852a(RTW_FW_NIC, fw_info->buf,
			&fw_info->buf_size, ic_name);
		/* preload wowlan fw */
		_hal_read_fw_8852a(RTW_FW_WOWLAN, fw_info->wow_buf,
	...
	case RTW_FW_WOWLAN:
		...
		break;
	case RTW_FW_SPIC:
		_hal_read_fw_8852a(RTW_FW_SPIC, fw_info->buf,
			&fw_info->buf_size, ic_name);
		tgt_buf = fw_info->buf;
		tgt_buf_size = fw_info->buf_size;
		break;
	case RTW_FW_AP:
		_hal_read_fw_8852a(RTW_FW_AP, fw_info->buf,
			&fw_info->buf_size, ic_name);
		tgt_buf = fw_info->buf;
		tgt_buf_size = fw_info->buf_size;
		break;
	...
	if (tgt_buf && (0 != tgt_buf_size)) {
		...
		fw_info->ram_size = 0;
		return _hal_read_fw_8852a(fw_type, fw_info->ram_buff,
				     ...
				     enum rtw_fw_type fw_type)
{
	...
	if (fw_cap->dlrom_en) {
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_rom_fw_8852a(fw_type, fw_info,
			ic_name))
			goto init_fw_fail;
	} else {
		if(fw_info->rom_buff) {
	...
	if (fw_cap->fw_src == RTW_FW_SRC_EXTNAL) {
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_extnal_fw_8852a(phl_com, fw_type,
			fw_info, ic_name))
			goto init_fw_fail;
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_intnal_fw_8852a(phl_com, fw_type,
			fw_info))
			goto init_fw_fail;
	} else {
		goto init_fw_fail;
...
enum rtw_hal_status hal_get_flash_8852a(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal,
					struct hal_init_info_t *init_info)
{
	/*enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
				   ...
				   struct hal_init_info_t *init_info)
{
	...
	if (hal_status != RTW_HAL_STATUS_SUCCESS)
		goto hal_init_fail;
	#ifdef RTW_WKARD_VHT_AMPDU_LEN
	/* For VHT mode, A-MPDU length must be limited to 0x3FFFF to
	 * workaround A-MSDU > 2 in A-MPDU failure.
	 * ToDo: DBCC support
	 */
	hal_write32(hal->hal_com, R_AX_AGG_LEN_VHT_0, 0x3FFFF);
	#endif /* RTW_WKARD_VHT_AMPDU_LEN */
...
enum rtw_hal_status hal_config_beacon_8852a(struct rtw_phl_com_t *phl_com,
                                            struct hal_info_t *hal,
...
enum rtw_hal_status hal_update_beacon_8852a(struct rtw_phl_com_t *phl_com,
                                            struct hal_info_t *hal,
                                            struct rtw_bcn_entry *bcn_entry)
{
