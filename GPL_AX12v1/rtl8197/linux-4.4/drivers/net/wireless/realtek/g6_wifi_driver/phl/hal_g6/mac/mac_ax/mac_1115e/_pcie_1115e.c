...
static struct mac_ax_intf_info intf_info_def_1115e = {
	MAC_AX_BD_NORM,
	MAC_AX_BD_NORM,
	MAC_AX_RXBD_SEP,
	MAC_AX_TAG_MULTI,
...
static struct mac_ax_pcie_ltr_param pcie_ltr_param_def_1115e = {
	0,
	0,
	MAC_AX_PCIE_ENABLE,
	MAC_AX_PCIE_ENABLE,
...
static u32 ltr_ltcy_regl_1115e[PCIE_LTR_IDX_MAX] = {
	R_BE_LTR_LATENCY_IDX0, R_BE_LTR_LATENCY_IDX1,
	R_BE_LTR_LATENCY_IDX2, R_BE_LTR_LATENCY_IDX3
};

static struct mac_ax_pcie_cfgspc_param pcie_cfgspc_param_def_1115e = {
	0,
	0,
	MAC_AX_PCIE_DISABLE,
	MAC_AX_PCIE_ENABLE,
...
struct txbd_ram mac_bdram_tbl_1115e[] = {
		/* ACH0_QUEUE_IDX_1115E */ {0, 5, 2},
		/* ACH1_QUEUE_IDX_1115E */ {5, 5, 2},
		/* ACH2_QUEUE_IDX_1115E */ {10, 5, 2},
		/* ACH3_QUEUE_IDX_1115E */ {15, 5, 2},
		/* ACH4_QUEUE_IDX_1115E */ {20, 5, 2},
		/* ACH5_QUEUE_IDX_1115E */ {25, 5, 2},
		/* ACH6_QUEUE_IDX_1115E */ {30, 5, 2},
		/* ACH7_QUEUE_IDX_1115E */ {35, 5, 2},
		/* MGQ_B0_QUEUE_IDX_1115E */ {40, 5, 1},
		/* HIQ_B0_QUEUE_IDX_1115E */ {45, 5, 1},
		/* MGQ_B1_QUEUE_IDX_1115E */ {50, 5, 1},
		/* HIQ_B1_QUEUE_IDX_1115E */ {55, 5, 1},
		/* FWCMD_QUEUE_IDX_1115E */ {60, 4, 1}
};
			 ...
			 enum mac_ax_pcie_clkdly def_ctrl)
{
	...
	case MAC_AX_PCIE_CLKDLY_V1_0:
		*val = SET_CLR_WOR2(*val, PCIE_CLKDLY_HW_V1_0,
				    B_BE_CLK_REQ_LAT_SH, B_BE_CLK_REQ_LAT_MSK);
		break;
...
get_pcie_info_def_1115e(struct mac_ax_adapter *adapter)
{
	return &intf_info_def_1115e;
}

struct txbd_ram *
get_bdram_tbl_pcie_1115e(struct mac_ax_adapter *adapter)
{
	return mac_bdram_tbl_1115e;
}

u32 mio_w32_pcie_1115e(struct mac_ax_adapter *adapter, u16 addr, u32 value)
{
	...
	page_sel = GET_FIEL2(addr, MIO_ADDR_PAGE_SH,
			     B_BE_PCIE_MIO_ADDR_PAGE_V1_MSK);
	addr_lsb = GET_FIEL2(addr, B_BE_PCIE_MIO_ADDR_SH,
			     B_BE_PCIE_MIO_ADDR_MSK);
	...
	MAC_REG_W32(R_BE_SYS_SDIO_CTRL, MAC_REG_R32(R_BE_SYS_SDIO_CTRL) |
		    B_BE_PCIE_AUXCLK_GATE);

	value32 = 0;
	value32 = SET_CLR_WOR2(value32, addr_lsb, B_BE_PCIE_MIO_ADDR_SH,
			       B_BE_PCIE_MIO_ADDR_MSK);
	value32 = SET_CLR_WOR2(value32, page_sel, B_BE_PCIE_MIO_ADDR_PAGE_V1_SH,
			       B_BE_PCIE_MIO_ADDR_PAGE_V1_MSK);
	value32 = SET_CLR_WOR2(value32, bytemap, B_BE_PCIE_MIO_WE_SH,
			       B_BE_PCIE_MIO_WE_MSK);

	MAC_REG_W32(R_BE_PCIE_MIO_INTF, value32);
	MAC_REG_W32(R_BE_PCIE_MIO_INTD, value);
	value32 |= B_BE_PCIE_MIO_BYIOREG;
	MAC_REG_W32(R_BE_PCIE_MIO_INTF, value32);

	count = MIO_SYNC_CNT;
	while (MAC_REG_R32(R_BE_PCIE_MIO_INTF) & B_BE_PCIE_MIO_BYIOREG) {
		PLTFM_DELAY_US(MIO_SYNC_DLY);
		if (--count == 0) {
			PLTFM_MSG_ERR("[ERR]MIO W32 timeout: %X\n",
				      MAC_REG_R32(R_BE_PCIE_MIO_INTF));
			value32 = SET_CLR_WOR2(value32, 0, B_BE_PCIE_MIO_WE_MSK,
					       B_BE_PCIE_MIO_WE_SH);
			value32 &= ~B_BE_PCIE_MIO_BYIOREG;
			MAC_REG_W32(R_BE_PCIE_MIO_INTF, value32);
			MAC_REG_W32(R_BE_SYS_SDIO_CTRL,
				    MAC_REG_R32(R_BE_SYS_SDIO_CTRL) &
				    ~B_BE_PCIE_AUXCLK_GATE);
			return MACPOLLTO;
		}
	}

	MAC_REG_W32(R_BE_SYS_SDIO_CTRL, MAC_REG_R32(R_BE_SYS_SDIO_CTRL) & ~B_BE_PCIE_AUXCLK_GATE);

	return MACSUCCESS;
}

u32 mio_r32_pcie_1115e(struct mac_ax_adapter *adapter, u16 addr, u32 *val)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u8 addr_lsb;
	u8 page_sel;
	...
	while (MAC_REG_R32(R_BE_PCIE_MIO_INTF) & B_BE_PCIE_MIO_BYIOREG) {
		...
		if (--count == 0) {
			PLTFM_MSG_ERR("[ERR]MIO R32 timeout: %X\n",
				      MAC_REG_R32(R_BE_PCIE_MIO_INTF));
			value32 &= ~(B_BE_PCIE_MIO_RE | B_BE_PCIE_MIO_BYIOREG);
			MAC_REG_W32(R_BE_PCIE_MIO_INTF, value32);
			MAC_REG_W32(R_BE_SYS_SDIO_CTRL,
				    MAC_REG_R32(R_BE_SYS_SDIO_CTRL) &
				    ~B_BE_PCIE_AUXCLK_GATE);
	...
	*val = MAC_REG_R32(R_BE_PCIE_MIO_INTD);
...
u32 get_txbd_reg_pcie_1115e(struct mac_ax_adapter *adapter, u8 dma_ch, u32 *reg,
	...
	*reg = MAC_AX_R32_FF;
	...
	case MAC_AX_DMA_ACH0:
		...
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH0_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH0_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH0_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				*reg = R_BE_DRV_FW_HSK_0;
			else
				*reg = R_BE_CH0_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH0_BDRAM_CTRL;
			break;
		default:
	...
	case MAC_AX_DMA_ACH1:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH1_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH1_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH1_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				*reg = R_BE_DRV_FW_HSK_1;
			else
				*reg = R_BE_CH1_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH1_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_ACH2:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH2_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH2_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH2_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				*reg = R_BE_DRV_FW_HSK_2;
			else
				*reg = R_BE_CH2_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH2_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_ACH3:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH3_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH3_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH3_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				*reg = R_BE_DRV_FW_HSK_3;
			else
				*reg = R_BE_CH3_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH3_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_ACH4:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH4_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH4_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH4_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_CH4_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH4_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_ACH5:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH5_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH5_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH5_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_CH5_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH5_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_ACH6:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH6_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH6_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH6_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_CH6_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH6_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_ACH7:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH7_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH7_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH7_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_CH7_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH7_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_B0MG:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH8_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH8_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH8_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				*reg = R_BE_DRV_FW_HSK_4;
			else
				*reg = R_BE_CH8_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH8_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_B0HI:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH9_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH9_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH9_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_CH9_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH9_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_B1MG:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH10_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH10_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH10_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_CH10_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH10_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_B1HI:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH11_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH11_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH11_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_CH11_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH11_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
	...
	case MAC_AX_DMA_H2C:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_CH12_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_CH12_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_CH12_TXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				*reg = R_BE_DRV_FW_HSK_5;
			else
				*reg = R_BE_CH12_TXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
			*reg = R_BE_CH12_BDRAM_CTRL;
			break;
		default:
			PLTFM_MSG_ERR("TXBD reg type%d invalid\n", type);
			return MACFUNCINPUT;
...
u32 set_txbd_reg_pcie_1115e(struct mac_ax_adapter *adapter, u8 dma_ch,
			    enum pcie_bd_ctrl_type type, u32 val0, u32 val1, u32 val2)
{
	...
	case PCIE_BD_CTRL_NUM:
		...
		val16 = SET_CLR_WORD(val16, val0, B_BE_TX_CH0_NUM);
		MAC_REG_W16(reg, val16);
		break;
	case PCIE_BD_CTRL_IDX:
		val16 = MAC_REG_R16(reg);
		val16 = SET_CLR_WORD(val16, val0, B_BE_TX_CH0_CPU_IDX);
		MAC_REG_W16(reg, val16);
		break;
	case PCIE_BD_CTRL_BDRAM:
		val32 = MAC_REG_R32(reg);
		val32 = SET_CLR_WORD(val32, val0, B_BE_CH0_BDRAM_SIDX);
		val32 = SET_CLR_WORD(val32, val1, B_BE_CH0_BDRAM_NUM);
		val32 = SET_CLR_WORD(val32, val2, B_BE_CH0_BDRAM_MIN);
...
u32 get_rxbd_reg_pcie_1115e(struct mac_ax_adapter *adapter, u8 dma_ch, u32 *reg,
			    enum pcie_bd_ctrl_type type)
{
	*reg = MAC_AX_R32_FF;

	switch (dma_ch) {
	case MAC_BE_RX_CH_RXQ:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_RXQ0_RXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_RXQ0_RXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_RXQ0_RXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				*reg = R_BE_DRV_FW_HSK_6;
			else
				*reg = R_BE_RXQ0_RXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
		default:
		...
		break;
	case MAC_BE_RX_CH_RPQ:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_RPQ0_RXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_RPQ0_RXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_RPQ0_RXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				*reg = R_BE_DRV_FW_HSK_7;
			else
				*reg = R_BE_RPQ0_RXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
		default:
			PLTFM_MSG_ERR("RXBD reg type%d invalid\n", type);
		...
		break;
	case MAC_BE_RX_CH_RXQ1:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_RXQ1_RXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_RXQ1_RXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_RXQ1_RXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_RXQ1_RXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
		default:
			PLTFM_MSG_ERR("RXBD reg type%d invalid\n", type);
		...
		break;
	case MAC_BE_RX_CH_RPQ1:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_BE_RPQ1_RXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_BE_RPQ1_RXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			*reg = R_BE_RPQ1_RXBD_NUM;
			break;
		case PCIE_BD_CTRL_IDX:
			if (adapter->mac_pwr_info.pwr_in_lps)
				return MACNOTSUP;
			else
				*reg = R_BE_RPQ1_RXBD_IDX;
			break;
		case PCIE_BD_CTRL_BDRAM:
		default:
			PLTFM_MSG_ERR("RXBD reg type%d invalid\n", type);
...
u32 set_rxbd_reg_pcie_1115e(struct mac_ax_adapter *adapter, u8 dma_ch,
			    enum pcie_bd_ctrl_type type, u32 val0, u32 val1, u32 val2)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	...
	case PCIE_BD_CTRL_NUM:
		val16 = MAC_REG_R16(reg);
		val16 = SET_CLR_WORD(val16, val0, B_BE_RPQ0_DESC_NUM);
		MAC_REG_W16(reg, val16);
		break;
	case PCIE_BD_CTRL_IDX:
		val16 = MAC_REG_R16(reg);
		val16 = SET_CLR_WORD(val16, val0, B_BE_RX_CH0_CPU_IDX);
		MAC_REG_W16(reg, val16);
		break;
	case PCIE_BD_CTRL_BDRAM:
	default:
...
u32 pcie_cfgspc_write_1115e(struct mac_ax_adapter *adapter,
			    struct mac_ax_pcie_cfgspc_param *param)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	struct mac_ax_pcie_cfgspc_param *param_def =
		&pcie_cfgspc_param_def_1115e;
	u32 ret, tmp32, l1_val, clk_val, crq_val, aspm_val;
	u8 val8;

	l1_val = MAC_REG_R32(R_BE_PCIE_MIX_CFG_V1);
	ret = p_ops->mio_r32_pcie(adapter, PCIE_ASPM_CTRL_V1, &aspm_val);
	if (ret != MACSUCCESS)
		return ret;
	clk_val = MAC_REG_R32(R_BE_PCIE_LAT_CTRL);
	crq_val = MAC_REG_R32(R_BE_L1_CLK_CTRL);
	...
	if (param->l0s_ctrl != MAC_AX_PCIE_IGNORE ||
	    param->l1_ctrl != MAC_AX_PCIE_IGNORE ||
	    param->l1ss_ctrl != MAC_AX_PCIE_IGNORE) {
		MAC_REG_W32(R_BE_PCIE_MIX_CFG_V1, l1_val);
	}
	...
	if (param->clkdly_ctrl != MAC_AX_PCIE_CLKDLY_IGNORE)
		MAC_REG_W32(R_BE_PCIE_LAT_CTRL, clk_val);

	if (param->crq_ctrl != MAC_AX_PCIE_IGNORE)
		MAC_REG_W32(R_BE_L1_CLK_CTRL, crq_val);

	return ret;
}

u32 pcie_cfgspc_read_1115e(struct mac_ax_adapter *adapter,
			   struct mac_ax_pcie_cfgspc_param *param)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	...
	if (ret != MACSUCCESS)
		return ret;
	clk_val = MAC_REG_R32(R_BE_PCIE_LAT_CTRL);
	clk_val = GET_FIEL2(clk_val, B_BE_CLK_REQ_LAT_SH, B_BE_CLK_REQ_LAT_MSK);
	crq_val = MAC_REG_R32(R_BE_L1_CLK_CTRL);

	param->l0s_ctrl = GET_PCIE_FUNC_STUS(l1_val, PCIE_BIT_L0S_V1);
	param->l1_ctrl = GET_PCIE_FUNC_STUS(l1_val, PCIE_BIT_L1_V1);
	param->l1ss_ctrl = GET_PCIE_FUNC_STUS(~l1_val, B_BE_L1SUB_DISABLE);
	param->wake_ctrl = MAC_AX_PCIE_ENABLE;
	param->crq_ctrl = GET_PCIE_FUNC_STUS(crq_val, B_BE_CLK_PM_EN);
...
u32 pcie_ltr_write_1115e(struct mac_ax_adapter *adapter,
			 struct mac_ax_pcie_ltr_param *param)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	struct mac_ax_pcie_ltr_param *param_def = &pcie_ltr_param_def_1115e;
	u32 ctrl0, ctrl1, dec_ctrl, idle_ltcy, act_ltcy;
	enum mac_ax_pcie_func_ctrl ctrl;
	u32 val32;

	ctrl0 = MAC_REG_R32(R_BE_LTR_CTRL_0);
	ctrl1 = MAC_REG_R32(R_BE_LTR_CTRL_1);
	dec_ctrl = MAC_REG_R32(R_BE_LTR_DECISION_CTRL);
	idle_ltcy = MAC_REG_R32(ltr_ltcy_regl_1115e[PCIE_LTR_IDX_IDLE]);
	act_ltcy = MAC_REG_R32(ltr_ltcy_regl_1115e[PCIE_LTR_IDX_ACT]);
	...
	if (!(param_def->ltr_hw_ctrl == MAC_AX_PCIE_IGNORE ||
		...
		if (ctrl == MAC_AX_PCIE_ENABLE)
			ctrl0 |= B_BE_LTR_WD_NOEMP_CHK | B_BE_LTR_HW_EN;
		dec_ctrl = ctrl == MAC_AX_PCIE_ENABLE ?
			   (dec_ctrl | B_BE_LTR_HW_DEC_EN) :
			   (dec_ctrl & ~B_BE_LTR_HW_DEC_EN);
	}
	...
	if (!(param_def->ltr_spc_ctrl == MAC_AX_PCIE_LTR_SPC_IGNORE ||
		...
		dec_ctrl = SET_CLR_WORD(dec_ctrl, val32, B_BE_LTR_SPACE_IDX);
	}
	...
	if (!(param_def->ltr_idle_timer_ctrl ==
		...
		val32 = param->ltr_idle_timer_ctrl ==
			...
			param_def->ltr_idle_timer_ctrl :
			param->ltr_idle_timer_ctrl;
		ctrl0 = SET_CLR_WORD(ctrl0, val32, B_BE_LTR_IDLE_TIMER_IDX);
	}
	...
	if (!(param_def->ltr_rx0_th_ctrl.ctrl == MAC_AX_PCIE_IGNORE ||
		...
		ctrl1 = SET_CLR_WORD(ctrl1, val32, B_BE_LTR_RX0_TH);
	}
	...
	if (!(param_def->ltr_rx1_th_ctrl.ctrl == MAC_AX_PCIE_IGNORE ||
		...
		ctrl1 = SET_CLR_WORD(ctrl1, val32, B_BE_LTR_RX1_TH);
	}
	...
	MAC_REG_W32(R_BE_LTR_CTRL_0, ctrl0);
	MAC_REG_W32(R_BE_LTR_CTRL_1, ctrl1);
	MAC_REG_W32(R_BE_LTR_DECISION_CTRL, dec_ctrl);
	MAC_REG_W32(ltr_ltcy_regl_1115e[PCIE_LTR_IDX_IDLE], idle_ltcy);
	MAC_REG_W32(ltr_ltcy_regl_1115e[PCIE_LTR_IDX_ACT], act_ltcy);

	return MACSUCCESS;
}

u32 pcie_ltr_read_1115e(struct mac_ax_adapter *adapter,
			struct mac_ax_pcie_ltr_param *param)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 ctrl0, ctrl1, dec_ctrl, idle_ltcy, act_ltcy;
	...
	param->ltr_hw_ctrl =
		((dec_ctrl & B_BE_LTR_HW_DEC_EN) && (ctrl0 & B_BE_LTR_HW_EN)) ?
		MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_DISABLE;

	param->ltr_spc_ctrl =
		(enum mac_ax_pcie_ltr_spc)
		GET_FIELD(dec_ctrl, B_BE_LTR_SPACE_IDX);
	param->ltr_idle_timer_ctrl =
		(enum mac_ax_pcie_ltr_idle_timer)
		GET_FIELD(ctrl0, B_BE_LTR_IDLE_TIMER_IDX);

	param->ltr_rx0_th_ctrl.val = (u16)GET_FIELD(ctrl1, B_BE_LTR_RX0_TH);
	param->ltr_rx1_th_ctrl.val = (u16)GET_FIELD(ctrl1, B_BE_LTR_RX1_TH);
...
u32 ltr_sw_trigger_1115e(struct mac_ax_adapter *adapter,
			 enum mac_ax_pcie_ltr_sw_ctrl ctrl)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;

	val32 = MAC_REG_R32(R_BE_LTR_DECISION_CTRL);
	if (!(val32 & B_BE_LTR_DRV_DEC_EN)) {
		val32 |= B_BE_LTR_DRV_DEC_EN;
		MAC_REG_W32(R_BE_LTR_DECISION_CTRL, val32);
	}
	switch (ctrl) {
	case MAC_AX_PCIE_LTR_SW_ACT:
		val32 = SET_CLR_WORD(val32, PCIE_LTR_IDX_ACT, B_BE_LTR_IDX_DRV);
		break;
	case MAC_AX_PCIE_LTR_SW_IDLE:
		val32 = SET_CLR_WORD(val32, PCIE_LTR_IDX_IDLE, B_BE_LTR_IDX_DRV);
		break;
	default:
		PLTFM_MSG_ERR("[ERR]Invalid LTR SW ctrl %d\n", ctrl);
		return MACFUNCINPUT;
	}
	val32 |= B_BE_LTR_REQ_DRV;
	MAC_REG_W32(R_BE_LTR_DECISION_CTRL, val32);

	return MACSUCCESS;
}

u32 get_avail_txbd_1115e(struct mac_ax_adapter *adapter, u8 ch_idx,
			 u16 *host_idx, u16 *hw_idx, u16 *avail_txbd)
{
	...
	*host_idx = (u16)GET_FIELD(tmp32, B_BE_TX_CH0_CPU_IDX);
	*hw_idx = (u16)GET_FIELD(tmp32, B_BE_TX_CH0_DMA_IDX);
...
u32 get_avail_rxbd_1115e(struct mac_ax_adapter *adapter, u8 ch_idx,
			 u16 *host_idx, u16 *hw_idx, u16 *avail_rxbd)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 ret = MACSUCCESS, reg = 0, tmp32;
	u16 bndy = adapter->pcie_info.rxbd_bndy;
	u8 rx_dma_ch = 0;
	...
	tmp32 = MAC_REG_R32(reg);
	*host_idx = (u16)GET_FIELD(tmp32, B_BE_RX_CH0_CPU_IDX);
	*hw_idx = (u16)GET_FIELD(tmp32, B_BE_RX_CH0_DMA_IDX);
	*avail_rxbd = calc_avail_rptr(*host_idx, *hw_idx, bndy);

	return MACSUCCESS;
}

u32 get_io_stat_pcie_1115e(struct mac_ax_adapter *adapter,
			   struct mac_ax_io_stat *out_st)
{
	return MACNOTSUP;
}

u32 ctrl_hci_dma_en_pcie_1115e(struct mac_ax_adapter *adapter,
			       enum mac_ax_pcie_func_ctrl txen,
			       enum mac_ax_pcie_func_ctrl rxen)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg = R_BE_HCI_FUNC_EN;
	u32 rval = MAC_REG_R32(reg);

	if (txen == MAC_AX_PCIE_ENABLE)
		rval |= B_BE_HCI_TXDMA_EN;
	else if (txen == MAC_AX_PCIE_DISABLE)
		rval &= ~B_BE_HCI_TXDMA_EN;

	if (rxen == MAC_AX_PCIE_ENABLE)
		rval |= B_BE_HCI_RXDMA_EN;
...
u32 ctrl_trxdma_pcie_1115e(struct mac_ax_adapter *adapter,
			   enum mac_ax_pcie_func_ctrl txen,
			   enum mac_ax_pcie_func_ctrl rxen,
			   enum mac_ax_pcie_func_ctrl ioen)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_init = R_BE_HAXI_INIT_CFG1;
	u32 val_init = MAC_REG_R32(reg_init);

	if (txen == MAC_AX_PCIE_ENABLE)
		val_init |= B_BE_TXDMA_EN;
	else if (txen == MAC_AX_PCIE_DISABLE)
		val_init &= ~B_BE_TXDMA_EN;

	if (rxen == MAC_AX_PCIE_ENABLE)
		val_init |= B_BE_RXDMA_EN;
	else if (rxen == MAC_AX_PCIE_DISABLE)
		val_init &= ~B_BE_RXDMA_EN;

	if (ioen == MAC_AX_PCIE_ENABLE)
		val_init &= ~B_BE_STOP_AXI_MST;
...
u32 ctrl_txdma_ch_pcie_1115e(struct mac_ax_adapter *adapter,
			     struct mac_ax_txdma_ch_map *ch_map)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_stop1 = R_BE_HAXI_DMA_STOP1;
	u32 val32;
...
u32 ctrl_wpdma_pcie_1115e(struct mac_ax_adapter *adapter,
...
u32 poll_io_idle_pcie_1115e(struct mac_ax_adapter *adapter)
{
#define B_IO_BUSY (B_BE_AXI_MST_BUSY)
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_busy1 = R_BE_HAXI_DMA_BUSY1;
	u32 val32;
	u32 cnt;
...
u32 poll_txdma_ch_idle_pcie_1115e(struct mac_ax_adapter *adapter,
				  struct mac_ax_txdma_ch_map *ch_map)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_busy1 = R_BE_HAXI_DMA_BUSY1;
	...
	if (ch_map->ch10 == MAC_AX_PCIE_ENABLE)
		val32 |= B_BE_CH10_BUSY;

	if (ch_map->ch11 == MAC_AX_PCIE_ENABLE)
		val32 |= B_BE_CH11_BUSY;

	if (ch_map->ch12 == MAC_AX_PCIE_ENABLE)
		val32 |= B_BE_CH12_BUSY;
...
u32 poll_rxdma_ch_idle_pcie_1115e(struct mac_ax_adapter *adapter,
				  struct mac_ax_rxdma_ch_map *ch_map)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_busy2 = R_BE_HAXI_DMA_BUSY2;
	u32 val32 = 0, rval32, cnt;

	if (ch_map->rxq == MAC_AX_PCIE_ENABLE)
		val32 |= B_BE_RXQ0_BUSY;
	if (ch_map->rpq == MAC_AX_PCIE_ENABLE)
		val32 |= B_BE_RPQ0_BUSY;
...
u32 poll_dma_all_idle_pcie_1115e(struct mac_ax_adapter *adapter)
{
	...
	ret = poll_txdma_ch_idle_pcie_1115e(adapter, &txch_map);
	if (ret != MACSUCCESS) {
	...
	ret = poll_rxdma_ch_idle_pcie_1115e(adapter, &rxch_map);
	if (ret != MACSUCCESS) {
...
u32 rst_bdram_pcie_1115e(struct mac_ax_adapter *adapter, u8 val)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_init = R_BE_HAXI_INIT_CFG1;
	u32 cnt, val32;

	MAC_REG_W32(reg_init, MAC_REG_R32(reg_init) | B_BE_SET_BDRAM_BOUND);

	cnt = PCIE_POLL_BDRAM_RST_CNT;
	while (cnt) {
		val32 = MAC_REG_R32(reg_init);
		if (!(val32 & B_BE_SET_BDRAM_BOUND))
			break;
		cnt--;
...
u32 trx_mit_pcie_1115e(struct mac_ax_adapter *adapter,
		       struct mac_ax_pcie_trx_mitigation *mit_info)
{
	...
	value32 = SET_CLR_WORD(value32, tmr_unit, B_BE_MIT_TXTIMER_UNIT);
	value32 = SET_CLR_WORD(value32, mit_info->tx_counter, B_BE_MIT_TXCOUNTER_MATCH);
	value32 = SET_CLR_WORD(value32, mit_info->tx_timer, B_BE_MIT_TXTIMER_MATCH);
	MAC_REG_W32(R_BE_INT_MIT_TX, value32);

	value32 = 0;
	if (mit_info->rxch_map->rxq == MAC_AX_PCIE_ENABLE)
		value32 |= B_BE_RXMIT_RXQ0P2_SEL;
	else if (mit_info->rxch_map->rxq == MAC_AX_PCIE_DISABLE)
		value32 &= ~B_BE_RXMIT_RXQ0P2_SEL;

	if (mit_info->rxch_map->rpq == MAC_AX_PCIE_ENABLE)
		value32 |= B_BE_RXMIT_RXQ0P1_SEL;
	...
	value32 = SET_CLR_WORD(value32, tmr_unit, B_BE_MIT_RXTIMER_UNIT);
	value32 = SET_CLR_WORD(value32, mit_info->rx_counter, B_BE_MIT_RXCOUNTER_MATCH);
	value32 = SET_CLR_WORD(value32, mit_info->rx_timer, B_BE_MIT_RXTIMER_MATCH);
	MAC_REG_W32(R_BE_INT_MIT_RX, value32);

	return MACSUCCESS;
}

u32 mode_op_pcie_1115e(struct mac_ax_adapter *adapter,
		       struct mac_ax_intf_info *intf_info)
{
	...
	u32 reg_init1 = R_BE_HAXI_INIT_CFG1;
	u32 reg_rxapp = R_BE_HAXI_DMA_RXQ_APPLEN;
	u32 reg_exp = R_BE_HAXI_EXP_CTRL;
	...
	if ((*rxbd_mode) == MAC_AX_RXBD_PKT) {
		val32_init1 = SET_CLR_WORD(val32_init1, PCIE_RXBD_NORM,
					   B_BE_RXQ_RXBD_MODE);
	} else if ((*rxbd_mode) == MAC_AX_RXBD_SEP) {
		if (intf_info->rx_sep_append_len > B_BE_RXDMA_EXT_LEN_MSK) {
			PLTFM_MSG_ERR("rx sep app len %d\n",
				      intf_info->rx_sep_append_len);
			return MACFUNCINPUT;
		}

		val32_init1 = SET_CLR_WORD(val32_init1, PCIE_RXBD_SEP,
					   B_BE_RXQ_RXBD_MODE);
		val32_rxapp = SET_CLR_WORD(val32_rxapp, intf_info->rx_sep_append_len,
					   B_BE_RXDMA_EXT_LEN);
	}

	val32_init1 = SET_CLR_WORD(val32_init1, *tx_burst, B_BE_MAX_TXDMA);
	val32_init1 = SET_CLR_WORD(val32_init1, *rx_burst, B_BE_MAX_RXDMA);

	val32_exp = SET_CLR_WORD(val32_exp, *multi_tag_num, B_BE_MAX_OUSTD_NUM);

	val32_init1 = SET_CLR_WORD(val32_init1, *wd_dma_idle_intvl,
				   B_BE_CFG_WD_PERIOD_IDLE);
	val32_init1 = SET_CLR_WORD(val32_init1, *wd_dma_act_intvl,
				   B_BE_CFG_WD_PERIOD_ACTIVE);
...
u32 get_err_flag_pcie_1115e(struct mac_ax_adapter *adapter,
			    struct mac_ax_pcie_err_info *out_info)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32 = MAC_REG_R32(R_BE_DBG_ERR_FLAG);

	out_info->txbd_len_zero = (val32 & B_BE_HAXI_TXBD_LEN0) ? 1 : 0;
	out_info->tx_stuck = (val32 & B_BE_TX_STUCK_V1) ? 1 : 0;
	out_info->rx_stuck = (val32 & B_BE_RX_STUCK_V1) ? 1 : 0;

	return MACSUCCESS;
}

static u32 get_target_1115e(struct mac_ax_adapter *adapter, u16 *target,
			    u32 phy_offset)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u16 tmp_u16, count;
...
u32 mac_auto_refclk_cal_pcie_1115e(struct mac_ax_adapter *adapter,
				   enum mac_ax_pcie_func_ctrl en)
{
	...
	ret = get_target_1115e(adapter, &tar, phy_offset);
	if (ret != MACSUCCESS) {
