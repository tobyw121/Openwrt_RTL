...
#if 0
	if(dma_ch==0 && ((hal_com->rx_int_array & (B_AX_RXDMA_INT_EN|B_AX_RDU_INT_EN))==0))
		return 0;
#endif

	if(dma_ch==1 && ((hal_com->rx_int_array & B_AX_RPQDMA_INT_EN)==0))
		return 0;
...
void _hal_mac_filter_crc_cnt(struct hal_info_t *hal_info, struct rtw_phl_rx_pkt *phl_rx){

	struct rtw_trx_stat *trx_stat = &hal_info->hal_com->trx_stat;
	struct rtw_r_meta_data *meta = &(phl_rx->r.mdata);

	if (0 == _os_mem_cmp(hal_info->hal_com->drv_priv, meta->ta, trx_stat->mp_fltr_ta, 6) &&
		0 == _os_mem_cmp(hal_info->hal_com->drv_priv, meta->mac_addr, trx_stat->mp_fltr_sa, 6)){

		if (meta->crc32 || meta->icverr)
			trx_stat->mp_fltr_rx_err_cnt++;
		else
			trx_stat->mp_fltr_rx_ok_cnt++;
	}
}
                         ...
                         struct rtw_phl_rx_pkt *rxpkt)
{
	...
	hstatus = trx_ops->handle_rx_buffer(phl_com, hal_info,
	                                    rxbuf, buf_size, rxpkt);

	if (phl_com->drv_mode != RTW_DRV_MODE_NORMAL){
		_hal_mac_filter_crc_cnt(hal_info, rxpkt);
	}
                        ...
                        struct rtw_phl_rx_pkt *rxpkt)
{
	...
	if (trx_ops->check_next_msdu == NULL)
		return RTW_HAL_STATUS_FAILURE;

	hstatus = trx_ops->check_next_msdu(phl_com, hal_info,
	                                   rxbuf, buf_size, rxpkt);
...
rtw_hal_rewind_rx_tag(struct rtw_phl_com_t *phl_com, void* hal, u8 ch_idx)
{
	...
	if (trx_ops->rewind_rx_tag == NULL)
		return;

	trx_ops->rewind_rx_tag(phl_com, ch_idx);
}
...
rtw_hal_get_hw_cnt(void *hal_info, enum rtw_hal_cnt_type cnt_type, u32 *oval)
{
	...
	struct hal_info_t *hal = (struct hal_info_t *)hal_info;
	u32 sum_counter;
	struct rtw_wp_rpt_stats *rpt_sts = (struct rtw_wp_rpt_stats *)hal->hal_com->trx_stat.wp_rpt_stats;
	struct hal_trx_ops *trx_ops = hal->trx_ops;
	u8 i;
	u8 txch_num = rtw_hal_query_txch_num(hal_info);
	u8 mgmt_ch = 0;
	...
	*oval = 0;
	...
	switch (cnt_type)
	{
	case HAL_CNT_TX_REACH_RTYLMT:
		if (rpt_sts) {
			for(i = 0; i < txch_num; i++) {
				*oval += rpt_sts[i].rty_fail_cnt;
				*oval += rpt_sts[i].delay_rty_fail_cnt;
			}
		}
		hal_status = RTW_HAL_STATUS_SUCCESS;
		break;
	case HAL_CNT_TX_LIFETIME_DROP:
		if (rpt_sts) {
			for(i = 0; i < txch_num; i++) {
				*oval += rpt_sts[i].lifetime_drop_cnt;
				*oval += rpt_sts[i].delay_lifetime_drop_cnt;
			}
		}
		hal_status = RTW_HAL_STATUS_SUCCESS;
		break;
	case HAL_CNT_TX_MACID_DROP:
		if (rpt_sts) {
			for(i = 0; i < txch_num; i++) {
				*oval += rpt_sts[i].macid_drop_cnt;
				*oval += rpt_sts[i].delay_macid_drop_cnt;
			}
		}
		hal_status = RTW_HAL_STATUS_SUCCESS;
		break;
	case HAL_CNT_WP_POLLUTED:
		*oval = hal->hal_com->trx_stat.wp_polluted;
		hal_status = RTW_HAL_STATUS_SUCCESS;
		break;
	default:
		break;
