...
#define AIRTIME_QUOTA	(4*1000)	/* unit of us (10^-6) */
#define MAX_STA_TS_LMT	(8*1000)	/* unit of us (10^-6) */
#define MAX_TXQ_AGG		128
#define MAX_TS_CONSUME	(50*1000)	/* unit of us */
#define TXQ_TIMEOUT		0		/* unit of ms */
#endif


#ifdef RTW_PHL_TX
#define RTW_MAX_FRAG_NUM 1 //max scatter number of a packet to xmit /* CONFIG_CORE_TXSC */
...
/*rate index based on enum rtw_data_rate from .\phl\rtw_general_def.h*/
#define IS_DATA_RATE_CCK(_rate)	((_rate) <= 0x3)
#define IS_DATA_RATE_OFDM(_rate)	((_rate) >= 0x4 && (_rate) <= 0xB)
...
struct xmit_txreq_buf {
	...
	u8 *pkt_list;
#ifdef CONFIG_CORE_TXSC
	u8 *pkt[MAX_TXSC_SKB_NUM];/* CONFIG_TXSC_AMSDU */
	u8 pkt_cnt;
	_adapter *adapter;
	struct sta_info *psta;
#endif
...
#endif
#ifdef CONFIG_XMIT_MGMT_ACK
	struct xmit_frame_ack_rpt ack_rpt;
#endif

	u8 *alloc_addr; /* the actual address this xmitframe allocated */
	u8 ext_tag; /* 0:data, 1:mgmt, 2:cmd */
...
#ifdef RTW_PHL_TX
#define XF_LIST 			(pxframe->list)
#define XF_ATTRIB			(pxframe->attrib)

//STA
#define XF_STA				(pxframe->attrib.psta)
#define XF_MACID            (pxframe->attrib.mac_id)

//PKT TYPE
#define XF_TYPE				(pxframe->xftype)

#define XF_SKB 				(pxframe->pkt)
#define XF_MGMTBUF			(pxframe->buf_addr)

#define XF_TXREQ			(pxframe->phl_txreq)
#define XF_TXREQ_CNT 		(pxframe->txreq_cnt)
#define XF_TXFREE_CNT 		(pxframe->txfree_cnt)

#define XF_ALLOC_HDR		(pxframe->alloc_hdr)
#define XF_ALLOC_TAIL		(pxframe->alloc_tail)

#define XF_WLHDR			pxframe->wlhdr
#define XF_WLTAIL			pxframe->wltail

//Get from HDR
#define XF_ETHTYPE			(pxframe->attrib.ether_type)
#define XF_VLAN_PROTO		(pxframe->attrib.vlan_proto)
#define XF_DA				(pxframe->attrib.dst)
#define XF_SA				(pxframe->attrib.src)
#define XF_RA				(pxframe->attrib.ra)
#define XF_TA				(pxframe->attrib.ta)

#define XF_SZ_PAYLOAD		(pxframe->attrib.pktlen) //(pxframe->attrib.sz_payload)

#define XF_DHCP				(pxframe->attrib.dhcp_pkt)
#define XF_ICMP				(pxframe->attrib.icmp_pkt)
#define XF_HI_PRI			(pxframe->attrib.hipriority_pkt)

#define XF_HDRLEN			(pxframe->attrib.pkt_hdrlen)

//PKTLEN
#define XF_LEN_ORIG			(pxframe->attrib.pktlen)


//Info of WLAN HDR
#define XF_WL_HDRLEN		(pxframe->attrib.hdrlen)
//#define XF_WL_HDRLEN		(pxframe->phl_txreq->mdata.hdr_len)
#define XF_WL_TYPE			(pxframe->attrib.type)
#define XF_WL_SUBTYPE		(pxframe->attrib.subtype)
#define XF_WL_QOS			(pxframe->attrib.qos_en)
#define XF_WL_TID 			(pxframe->attrib.priority)
//#define XF_WL_TID			(pxframe->phl_txreq->mdata.tid)
#define XF_WL_AMPDU			(pxframe->attrib.ampdu_en)
#define XF_WL_ACK_POLICY	(pxframe->attrib.ack_policy)
#define XF_WL_AMSDU			(pxframe->attrib.amsdu)
#define XF_WL_MDATA			(pxframe->attrib.mdata)
#define XF_WL_EOSP			(pxframe->attrib.eosp)
#define XF_WL_SEQ			(pxframe->attrib.seqnum)

//PKT Construct
#define XF_NR_FRAGS			(pxframe->attrib.nr_frags)
#define XF_FRAG_LEN			(pxframe->attrib.frag_len)
#define XF_FRAG_DATA_LEN	(pxframe->attrib.frag_datalen)
#ifdef CONFIG_CORE_TXSC
#define XF_FRAG_LEN_TXSC	(pxframe->attrib.frag_len_txsc)
#endif

//Security
#define XF_SWENC 			(pxframe->attrib.bswenc)
#define XF_SEC_TYPE 		(pxframe->attrib.encrypt)
//#define XF_SEC_TYPE 		(pxframe->phl_txreq->mdata.sec_type)
#define XF_KEYIDX 			(pxframe->attrib.key_idx)
#define XF_IV 				(pxframe->attrib.iv)
#define XF_IVLEN 			(pxframe->attrib.iv_len)
#define XF_ICV 				(pxframe->attrib.icv)
#define XF_ICVLEN 			(pxframe->attrib.icv_len)

//HW
#define XF_AMPDU_DENSITY	(pxframe->phl_txreq->mdata.ampdu_density)
#define XF_MAX_AGG			(pxframe->phl_txreq->mdata.max_agg_num)
#define XF_NO_RATE_FB		(pxframe->phl_txreq->mdata.dis_data_rate_fb)
#define XF_EN_TX_CNT_LMT	(pxframe->phl_txreq->mdata.data_tx_cnt_lmt_en)
#define XF_TX_CNT_LMT		(pxframe->phl_txreq->mdata.data_tx_cnt_lmt)
#define XF_RTY_LOWEST_RATE	(pxframe->phl_txreq->mdata.data_rty_lowest_rate)
#define XF_EN_LSIG_TXOP		(pxframe->phl_txreq->mdata.lsig_txop_en)
#define XF_F_TXOP			(pxframe->phl_txreq->mdata.force_txop)
#define XF_RTS				(pxframe->phl_txreq->mdata.rts_en)
#define XF_CTS2SELT			(pxframe->phl_txreq->mdata.cts2self)
#define XF_HWRTS			(pxframe->phl_txreq->mdata.hw_rts_en)
#define XF_F_BW				(pxframe->phl_txreq->mdata.f_bw)
#define XF_F_GI_LTF			(pxframe->phl_txreq->mdata.f_gi_ltf)
#define XF_F_RATE			(pxframe->phl_txreq->mdata.f_rate)
#define XF_F_ER				(pxframe->phl_txreq->mdata.f_er)
#define XF_F_DCM			(pxframe->phl_txreq->mdata.f_dcm)
#define XF_F_STBC			(pxframe->phl_txreq->mdata.f_stbc)
#define XF_F_LDPC			(pxframe->phl_txreq->mdata.f_ldpc)
#define XF_BMC				(pxframe->phl_txreq->mdata.bmc)
#define XF_RAW				(pxframe->phl_txreq->mdata.raw)
#endif
...
struct sta_xmit_priv {
	...
	_queue mgt_q;
	struct sta_tx_queue txq[MAX_TXQ];
	unsigned long tx_pending_bitmap;
	ATOMIC_T txq_total_len;
	_list tx_pending;
	_list ps_trigger;
	u8 ps_trigger_type;		/* BIT0: PS-Poll, BIT1: UAPSD trigger frame */
#ifdef CONFIG_ONE_TXQ
	u32 ts_used;	/* unit of us (10^-6) */
	u32 ts_limit;	/* unit of us (10^-6) */
	u32 ts_ratio;
	u32 txq_pkt_trigger;
	...
	u32 txq_limit;
	u32 txq_service;
	u32 txq_service_update;
	u32 txq_service_previous;
	u32 txq_service_group;
	u32 txq_avg_dequeue;
	u32 txq_cur_dequeue;
	u8 txq_timeout;
	ATOMIC_T txreq_used;
	ATOMIC_T txreq_ts_used;
	u32 txreq_ts_limit_exceed_cnt;
	u64 txreq_ts_used_total;

	u32 txq_extra_deq;
	u32 tx_rate_mbps;
	u32 tx_rate_mbps_retry;
#endif /* CONFIG_ONE_TXQ */

	/* _VW_ */
	//struct tx_servq	swq[TXQ_MAX];
	//u16 tx_pending;
	//u16 swq_total_len;
...
struct	xmit_priv	{
	...
	/* _VW_ */
	//_queue tx_pending_sta_queue[TXQ_MAX];
	//_queue sleep_q;
	_list bmc_list;
	//_list trigger_list;
	...
	uint full_xmitframe_cnt; /* CONFIG_CORE_TXSC */
#ifdef CONFIG_DYN_ALLOC_XMITFRAME
	uint alloc_fail_xmitframe_cnt;
#endif
	...
	uint full_xframe_ext_cnt;
#ifdef CONFIG_DYN_ALLOC_XMITFRAME
	uint alloc_fail_xmitframe_ext_cnt;
	uint alloc_fail_txreq_ext_cnt;
#endif
...
#if 0
	/* cmd xmitframe */
	u8 *xframe_cmd_alloc_addr;
	u8 *xframe_cmd;
	uint free_xframe_cmd_cnt;
	_queue	free_xframe_cmd_queue;
	uint full_xframe_cmd_cnt;
#endif
...
#define MAX_AGG_PKT_NUM 256 /* Max tx ampdu coounts		 */
...
#endif
#if defined(CONFIG_TX_DEFER) || defined(CPTCFG_WFO_VIRT_MODULE)
	_timer tx_defer_timer;
#endif
#ifdef DBG_TXBD_DESC_DUMP
	...
	/* AMSDU_BY_SIZE */
	u8 txsc_amsdu_mode;/* 0:default, 1:amsdu by size */
	u8 txsc_amsdu_force_num;
	u32 cnt_txsc_amsdu_enq[4];
	u32 cnt_txsc_amsdu_enq_abort[4];
	u32 cnt_txsc_amsdu_deq[4];
	u32 cnt_txsc_amsdu_dump[MAX_TXSC_SKB_NUM + 1];
	u32 cnt_txsc_amsdu_deq_empty;
	u32 cnt_txsc_amsdu_deq_ampdu;

	u32 cnt_txsc_amsdu_enq_ps;
	u32 cnt_txsc_amsdu_deq_ps;

	u32 cnt_txsc_amsdu_timeout_dump[MAX_TXSC_SKB_NUM + 1];
	u32 cnt_txsc_amsdu_timeout_deq_empty;
	u32 cnt_txsc_amsdu_timeout_ok[4];
	u32 cnt_txsc_amsdu_timeout_fail[4];
	u32 cnt_txsc_amsdu_timeout_bypass[4];

	u32 cnt_txsc_amsdu_dfree;
#endif /* CONFIG_TXSC_AMSDU */
#endif /* CONFIG_CORE_TXSC */
#if defined(CONFIG_TX_DEFER) || defined(CPTCFG_WFO_VIRT_MODULE)
	ATOMIC_T defer_tx_cnt;
	u8 defer_tx_flag;
#endif
	/* for rtw_core_tx debug */
	u32 core_tx_pkts;
	u32 core_tx_drop;
	u32 core_tx_abort[10];
};


static inline int tx_servq_len(struct tx_servq *ptxservq)
{
	return ptxservq->queue.qlen;
}
...
void rtw_core_upd_lmt_txreq_stats(_adapter *padapter, struct rtw_xmit_req *tx_req, struct xmit_frame *pxframe);
#endif
extern s32 core_tx_free_xmitframe(_adapter *padapter, struct xmit_frame *pxframe);
extern s32 rtw_core_free_xmitframe_queue(_adapter *padapter, _queue *queue);
extern void rtw_core_free_sta_xmit_queue(_adapter *padapter, struct sta_xmit_priv *pstaxmitpriv);
#endif
...
s32 rtw_core_tx(_adapter *padapter, struct sk_buff **ppkt, struct sta_info *psta);
#ifdef CONFIG_DBG_HNDSK_MGMT
void upd_hndsk_pkt_sts(_adapter *padapter, u8 category, u8 no_ack);
#endif
...
s32 core_tx_free_xmitframe(_adapter *padapter, struct xmit_frame *pxframe);
u8 *get_txreq_buffer(_adapter *padapter, u8 **txreq, u8 **pkt_list, u8 **head, u8 **tail, u8 txsc, struct sta_info *psta);
#endif /* CONFIG_CORE_TXSC */
#endif
