...
void halrf_tssi_backup_txagc_8852c(struct rf_info *rf, enum phl_phy_idx phy, bool enable)
{
	...
	_halrf_tssi_hw_tx_8852c(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, T_HT_MF, 0, true);

#if 0
	/*Path B*/
	tx_couter[RF_PATH_B] = halrf_rreg(rf, 0x1a40, 0xffff);
	
	_halrf_tssi_hw_tx_8852c(rf, phy, RF_PATH_B, 1000, 100, xdbm, T_HT_MF, 0, true);
	halrf_delay_ms(rf, 15);
	_halrf_tssi_hw_tx_8852c(rf, phy, RF_PATH_B, 1000, 100, xdbm, T_HT_MF, 0, false);
#endif
	...
	_halrf_tssi_hw_tx_8852c(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, T_HT_MF, 0, false);
...
void halrf_tssi_hw_tx_8852c(struct rf_info *rf,
			...
			bool enable)
{
	_halrf_tssi_hw_tx_8852c(rf, phy, path, cnt, 5000, dbm, T_HT_MF, 0, enable);
}
