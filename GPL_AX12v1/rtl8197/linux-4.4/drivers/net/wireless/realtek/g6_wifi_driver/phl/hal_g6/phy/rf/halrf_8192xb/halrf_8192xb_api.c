...
u8 halrf_get_thermal_8192xb(struct rf_info *rf, enum rf_path rf_path)
{
...
u32 halrf_mac_get_pwr_reg_8192xb(struct rf_info *rf, enum phl_phy_idx phy,
	u32 addr, u32 mask)
{
...
u32 halrf_mac_set_pwr_reg_8192xb(struct rf_info *rf, enum phl_phy_idx phy,
...
bool halrf_wl_tx_power_control_8192xb(struct rf_info *rf, u32 tx_power_val)
{
	...
	if (pwr->coex_pwr_ctl_enable == false && pwr->dpk_pwr_ctl_enable == false) {
		...
		result = halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd200, 0xfffffc00, 0x0);
		...
		/*all-time control*/
		result = halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd200, 0xfffffc00, ((tmp_pwr & 0x1ff) | BIT(9)));
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd200) fail, error code=%d\n",
				__func__, result);
			return false;
	...
	if (gnt_bt_control == 0xffff) {
		...
		result = halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd220, BIT(1), 0x0);
		result = halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd220, 0xfffff007, 0x0);
		if (result) {
		...
		result = halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd220, BIT(1), 0x1);
		result = halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd220, 0xfffff007, ((gnt_bt_control & 0x1ff) << 3));
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd220) fail, error code=%d\n",
				__func__, result);
			return false;
...
s8 halrf_get_ther_protected_threshold_8192xb(struct rf_info *rf)
{
	u8 tmp_a, tmp_b, tmp;
...
s8 halrf_xtal_tracking_offset_8192xb(struct rf_info *rf,
...
void halrf_txck_force_8192xb(struct rf_info *rf, enum rf_path path, bool force, enum dac_ck ck)
{
...
u32 ck480_cav_8192xb[][6] = {{0x8, 0x2, 0x3, 0xc, 0xc, 0x9}, {0x5, 0x0, 0x3, 0xe, 0xf, 0x9}};
u32 ck960_cav_8192xb[][6] = {{0x8, 0x2, 0x2, 0x5, 0x9, 0xa}, {0x8, 0x2, 0x2, 0x4, 0xf, 0xa}};
u32 ck1920_cav_8192xb[] = {0x8, 0x2, 0x0, 0x7, 0xc, 0xa};
u32 ck960_cbv_8192xb[] = {0x8, 0x2, 0x2, 0x4, 0xf, 0xa};
u32 ck1920_new_8192xb[] = {0x9, 0x1, 0x1, 0x3, 0xf, 0xa};
void halrf_rxck_force_8192xb(struct rf_info *rf, enum rf_path path, bool force, enum adc_ck ck)
{
	...
	if (hal_i->cv == CAV) {
		...
		case ADC_480M:
			if (efuse_idx == 3)
				data = ck480_cav_8192xb[0];
			else
				data = ck480_cav_8192xb[1];
			break;
		case ADC_960M:
			if (efuse_idx == 3)
				data = ck960_cav_8192xb[0];
			else
				data = ck960_cav_8192xb[1];
			break;
		case ADC_1920M:
			if (efuse_idx == 3)
				data = ck1920_cav_8192xb;
			else
				data = ck1920_new_8192xb;
			break;
		default:
			data = ck1920_new_8192xb;
			break;
...
void halrf_set_gpio_8192xb(struct rf_info *rf, enum phl_phy_idx phy)
{
	...
	if (rfe_type == 51 || rfe_type == 52) {
		...
		/*Set BB GPIO Setting*/
		/*PathA=0x1*/
		halrf_gpio_setting(rf, 11, 1, false, HALRF_PAPE_RFM);
		halrf_gpio_setting(rf, 7, 1, true, HALRF_TRSW_RFM);
		halrf_gpio_setting(rf, 6, 1, true, HALRF_LNAON_RFM);
		/*PathB=0x2*/
		halrf_gpio_setting(rf, 1, 2, false, HALRF_PAPE_RFM);
		halrf_gpio_setting(rf, 2, 2, true, HALRF_TRSW_RFM);
		halrf_gpio_setting(rf, 3, 2, true, HALRF_LNAON_RFM);
...
u32 bkup_kip_reg_8192xb[] = {0x813c, 0x8124, 0x8120, 0xc0d4, 0xc0d8, 0xc0ec};
u32 bkup_bb_reg_8192xb[] = {0x2344};
u32 bkup_rf_reg_8192xb[] = {0x5, 0x10005, 0xdf};

u32 bkup_kip_val_8192xb[RF_PATH_MAX_NUM][RF_BACKUP_KIP_REG_MAX_NUM] = {{0x0}};
u32 bkup_bb_val_8192xb[RF_BACKUP_BB_REG_MAX_NUM] = {0x0};
u32 bkup_rf_val_8192xb[RF_PATH_MAX_NUM][RF_BACKUP_RF_REG_MAX_NUM] = {{0x0}};

void do_bkup_kip_8192xb(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_kip_reg_8192xb);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_kip_val_8192xb[path][i] = halrf_rreg(rf, bkup_kip_reg_8192xb[i] + (path << 8), MASKDWORD);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Backup KIP(S%d) 0x%x = %x\n", path, bkup_kip_reg_8192xb[i] + (path << 8), bkup_kip_val_8192xb[path][i]);
	}
}

void do_bkup_bb_8192xb(struct rf_info *rf)
{
	u32 i;
	u32 num = ARRAY_SIZE(bkup_bb_reg_8192xb);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_bb_val_8192xb[i] =
			halrf_rreg(rf, bkup_bb_reg_8192xb[i], MASKDWORD);

		RF_DBG(rf, DBG_RF_RFK, "[RFK]backup bb reg : %x, value =%x\n",
		      bkup_bb_reg_8192xb[i], bkup_bb_val_8192xb[i]);
	}
}


void do_bkup_rf_8192xb(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_rf_reg_8192xb);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_rf_val_8192xb[path][i] = halrf_rrf(rf, path, bkup_rf_reg_8192xb[i], MASKRF);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Backup RF S%d 0x%x = %x\n",
			path, bkup_rf_reg_8192xb[i], bkup_rf_val_8192xb[path][i]);
	}
}

void do_reload_kip_8192xb(struct rf_info *rf, u8 path) 
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_kip_reg_8192xb);

	for (i = 0; i < num; i++) {
		halrf_wreg(rf, bkup_kip_reg_8192xb[i] + (path << 8), MASKDWORD, bkup_kip_val_8192xb[path][i]);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Reload KIP(S%d) 0x%x = %x\n",path, bkup_kip_reg_8192xb[i] + (path << 8),
			bkup_kip_val_8192xb[path][i]);
	}
}

void do_reload_bb_8192xb(struct rf_info *rf)
{
	u32 i;
	u32 num = ARRAY_SIZE(bkup_bb_reg_8192xb);

	for (i = 0; i < num; i++) {
		halrf_wreg(rf,  bkup_bb_reg_8192xb[i], MASKDWORD, bkup_bb_val_8192xb[i]);
...
void do_reload_rf_8192xb(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_rf_reg_8192xb);

	for (i = 0; i < num; i++) {
		halrf_wrf(rf, path, bkup_rf_reg_8192xb[i], MASKRF, bkup_rf_val_8192xb[path][i]);
...
void halrf_set_regulation_from_driver_8192xb(struct rf_info *rf,
	u8 regulation_idx)
{
	struct halrf_pwr_info *pwr = &rf->pwr_info;
...
void halrf_op5k_init_8192xb(struct rf_info *rf)
{
	...
	op5k->thermal_base_op5k[RF_PATH_A] = 26;
	op5k->thermal_base_op5k[RF_PATH_B] = 26;

	op5k->thermal_op5k[RF_PATH_A] = op5k->thermal_base_op5k[RF_PATH_A];
	op5k->thermal_op5k[RF_PATH_B] = op5k->thermal_base_op5k[RF_PATH_B];
}

#define OP5K_MAX_RF_PATH_8192XB 2
void halrf_op5k_trigger_8192xb(struct rf_info *rf)
{
	struct halrf_op5k_info *op5k = &rf->op5k_info;
	u32 i, j, k;
	u32 op5_diff = 5;
	s32 upper_op5[OP5K_MAX_RF_PATH_8192XB] = {0};
	s32 lower_op5[OP5K_MAX_RF_PATH_8192XB] = {0};
	...
	while ((retry[RF_PATH_A] < 2 || retry[RF_PATH_B] < 2)) {
		...
		for (i = 0; i < OP5K_RESET_CNT_DATA; i++) {
			for (j = 0; j < OP5K_MAX_RF_PATH_8192XB; j++) {
...
void halrf_op5k_tracking_8192xb(struct rf_info *rf)
{
	struct halrf_op5k_info *op5k = &rf->op5k_info;
	u8 i = 0, path;
	u8 thermal_op5k_avg_count = 0, thermal_value[OP5K_MAX_RF_PATH_8192XB] = {0};
	u32 thermal_op5k_avg[OP5K_MAX_RF_PATH_8192XB] = {0};
	s8 delta_op5k[OP5K_MAX_RF_PATH_8192XB] = {0}, delta_base_op5k[OP5K_MAX_RF_PATH_8192XB] = {0};
	...
	for (path = 0; path < OP5K_MAX_RF_PATH_8192XB; path++) {
...
void halrf_bw_setting_op5k_8192xb(struct rf_info *rf)
{
	struct halrf_op5k_info *op5k = &rf->op5k_info;
...
void halrf_set_share_xtal_8192xb(struct rf_info *rf)
{
	u8 value;

#if defined(CONFIG_AX_SHARE_XTAL) && defined(CONFIG_WLAN_SHARE_XTAL)

	RF_DBG(rf, DBG_RF_XTAL_TRACK, "======>%s share_xtal\n",
		__func__);
