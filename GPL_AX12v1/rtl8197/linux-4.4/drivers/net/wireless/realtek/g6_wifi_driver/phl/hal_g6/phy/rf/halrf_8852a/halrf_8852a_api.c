...
void halrf_set_syn01_8852a(struct rf_info *rf, bool dbcc_en)
{
	if (dbcc_en == 1) {
		/*SYN0 on + SYN1 on*/
...
u32 halrf_mac_set_pwr_reg_8852a(struct rf_info *rf, enum phl_phy_idx phy,
...
void halrf_wlan_tx_power_control_8852a(struct rf_info *rf,
	...
	u32 tx_power_val, bool enable)
{
	...
	if (enable == false) {
		if (pwr_ctrl_idx == ALL_TIME_CTRL) {
			...
			halrf_mac_set_pwr_reg_8852a(rf, phy_tmp, 0xd200, 0x3ff, 0x0);
		} else if (pwr_ctrl_idx == GNT_TIME_CTRL) {
			/*GNT_BT control Disable*/
			halrf_mac_set_pwr_reg_8852a(rf, phy_tmp, 0xd220, BIT(1), 0x0);
			halrf_mac_set_pwr_reg_8852a(rf, phy_tmp, 0xd220, 0xff8, 0x0);
		}
	} else {
		if (pwr_ctrl_idx == ALL_TIME_CTRL) {
			halrf_mac_set_pwr_reg_8852a(rf, phy_tmp, 0xd200, 0x3ff,
				((tx_power_val & 0x1ff) | BIT(9)));
		} else if (pwr_ctrl_idx == GNT_TIME_CTRL) {
			halrf_mac_set_pwr_reg_8852a(rf, phy_tmp, 0xd220, BIT(1), 0x1);
			halrf_mac_set_pwr_reg_8852a(rf, phy_tmp, 0xd220, 0xff8, tx_power_val & 0x1ff);
		} else {
...
bool halrf_wl_tx_power_control_8852a(struct rf_info *rf, u32 tx_power_val)
{
	...
	if (pwr->coex_pwr_ctl_enable == false && pwr->dpk_pwr_ctl_enable == false) {
		/*all-time control Disable*/
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd200, 0x3ff, 0x0);
		...
		/*all-time control*/
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd200, 0x3ff, ((tmp_pwr & 0x1ff) | BIT(9)));
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd200) fail, error code=%d\n",
				__func__, result);
			return false;
	...
	if (gnt_bt_control == 0xffff) {
		...
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd220, BIT(1), 0x0);
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd220, 0xff8, 0x0);
		if (result) {
		...
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd220, BIT(1), 0x1);
		result = halrf_mac_set_pwr_reg_8852a(rf, phy, 0xd220, 0xff8, gnt_bt_control & 0x1ff);
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd220) fail, error code=%d\n",
				__func__, result);
			return false;
...
void halrf_set_gpio_8852a(struct rf_info *rf, enum phl_phy_idx phy, u8 band)
{
	...
	u32 band_tmp = rf->hal_com->band[phy].cur_chandef.band;

	RF_DBG(rf, DBG_RF_INIT, "===>%s   rfe_type=%d   band=%d   phy=%d\n",
		__func__, rfe_type, band_tmp, phy);

	if (rfe_type == 37 || rfe_type == 38) {
		if (band_tmp == BAND_ON_5G) {
...
void halrf_set_regulation_from_driver_8852a(struct rf_info *rf,
	u8 regulation_idx)
{
	struct halrf_pwr_info *pwr = &rf->pwr_info;

	pwr->regulation_idx = regulation_idx;
	/*_os_dbgdump("HALRF set column [%d]\n", pwr->regulation_idx);*/
}

bool halrf_set_dbcc_8852a(struct rf_info *rf, bool dbcc_en)
{
	halrf_set_syn01_8852a(rf, dbcc_en);
	if (dbcc_en)
