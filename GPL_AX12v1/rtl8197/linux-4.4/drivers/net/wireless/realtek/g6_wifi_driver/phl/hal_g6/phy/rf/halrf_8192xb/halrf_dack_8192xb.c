...
void halrf_afe_init_8192xb(struct rf_info *rf)
{
#if 0
...
void halrf_dack_init_8192xb(struct rf_info *rf)
{

}

void halrf_dack_reset_8192xb(struct rf_info *rf, enum rf_path path)
{
	if (path == RF_PATH_A) {
...
void halrf_drck_8192xb(struct rf_info *rf)
{
	u32 c;
	u32 rck_d;
...
void halrf_addck_backup_8192xb(struct rf_info *rf)
{
...
void halrf_addck_reload_8192xb(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	/*S0*/
...
void halrf_dack_backup_s0_8192xb(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
...
void halrf_dack_backup_s1_8192xb(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
...
void halrf_dack_reload_by_path_8192xb(struct rf_info *rf, enum rf_path path, u8 index)
{
	struct halrf_dack_info *dack = &rf->dack;
...
void halrf_dack_reload_8192xb(struct rf_info *rf, enum rf_path path)
{
	u8 i;

	for (i = 0; i < 2; i++)
		halrf_dack_reload_by_path_8192xb(rf, path, i);
}

void halrf_check_addc_8192xb(struct rf_info *rf, enum rf_path path)
{
...
void halrf_addck_8192xb(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c;
	/*S0*/
...
void halrf_check_dadc_8192xb(struct rf_info *rf, enum rf_path path)
{
	...
	halrf_check_addc_8192xb(rf, path);
	if (path == RF_PATH_A) {
...
void halrf_dack_8192xb_s0(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c = 0;
...
//	halrf_wreg(rf, 0xc024, 0xfff00000, 0x30);
	...
	halrf_dack_reset_8192xb(rf, RF_PATH_A);
...
//	halrf_check_dadc_8192xb(rf, RF_PATH_A);

	/*backup here*/
	halrf_dack_backup_s0_8192xb(rf);
	halrf_dack_reload_8192xb(rf, RF_PATH_A);
	/*step 6: Set DAC & clk to normal mode */
	halrf_wreg(rf, 0x12b8, BIT(30), 0x0);
}

void halrf_dack_8192xb_s1(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c = 0;
...
//	halrf_wreg(rf, 0xc124, 0xfff00000, 0x30);
	...
	halrf_dack_reset_8192xb(rf, RF_PATH_B);
...
//	halrf_check_dadc_8192xb(rf, RF_PATH_A);
	...
	/*backup here*/
	halrf_dack_backup_s1_8192xb(rf);
	halrf_dack_reload_8192xb(rf, RF_PATH_B);
	/*step 6: Set DAC & clk to normal mode */
	halrf_wreg(rf, 0x32b8, BIT(30), 0x0);
}

void halrf_dack_8192xb(struct rf_info *rf)
{
	halrf_dack_8192xb_s0(rf);
	halrf_dack_8192xb_s1(rf);
}

void halrf_dack_dump_8192xb(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
	u8 t;
...
void halrf_dack_manual_off_8192xb(struct rf_info *rf)
{
...
void halrf_dac_cal_8192xb(struct rf_info *rf, bool force)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 rf0_0, rf1_0;
	u8 phy_map;
	...
	halrf_afe_init_8192xb(rf);
	halrf_drck_8192xb(rf);
	halrf_dack_manual_off_8192xb(rf);
#if 1
...
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_START);
	halrf_addck_8192xb(rf);
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_STOP);
	halrf_addck_backup_8192xb(rf);
	halrf_addck_reload_8192xb(rf);
...
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_START);
	...
	halrf_dack_8192xb(rf);
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_STOP);
	halrf_dack_dump_8192xb(rf);
	dack->dack_done = true;
