				   ...
				   void *buf, size_t len, bool fixed)
{
...
#if (DBG_SDIO >= 3)
	if (!error && !f0 && !cmd52
	    && (dvobj_to_sdio(d)->dbg_enable
		&& dvobj_to_sdio(d)->err_test && !dvobj_to_sdio(d)->err_test_triggered
		&& ((addr & 0x10000)
		    || (!(addr & 0xE000)
			&& !((addr >= 0x40) && (addr < 0x48)))))) {
		dvobj_to_sdio(d)->err_test_triggered = 1;
		error = -ETIMEDOUT;
	...
	if (error) {
		if (f0 || cmd52) {
			dvobj_to_sdio(d)->cmd52_err_cnt++;
		} else {
			dvobj_to_sdio(d)->cmd53_err_cnt++;
#if (DBG_SDIO >= 1)
