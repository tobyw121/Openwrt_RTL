...
void halbb_dfs_init(struct bb_info *bb)
{
	...
	bb_dfs->dynamic_h2l_by_tw = false;
	bb_dfs->dfs_nonTW_h2l_val = halbb_get_reg_cmn(bb, cr->dfs_h2l_th, cr->dfs_h2l_th_m, bb->bb_phy_idx);
	bb_dfs->dfs_TW_h2l_val = bb_dfs->dfs_nonTW_h2l_val - 3;
...
void halbb_dfs_rgn_dmn_dflt_cnfg(struct bb_info *bb)
{
	...
	if (bb_dfs->dfs_rgn_domain == DFS_REGD_ETSI) {
		...
		u16 pw_max_etsi_tab[DFS_RDR_TYP_NUM] = {25,75,150,150,15,15,5,1000};
				  ...
				  bool w56_band)
{
	...
	u8 pri_min_mic_w56_tab[DFS_RDR_TYP_NUM] = {55,57,80,6,8,8,40,13};
	u8 pri_max_mic_w56_tab[DFS_RDR_TYP_NUM] = {56,58,160,10,20,20,80,14};
	u8 ppb_mic_w56_tab[DFS_RDR_TYP_NUM] = {18,18,10,23,16,12,15,18};
				...
				u8 c_num, u8 p_num, bool is_sg1, u8 c_seg, u8 p_seg)
{
	...
	if (bb_dfs->dbg_hwdet_prnt_en) {
		if (bb_dfs->n_seq_flag || bb_dfs->n_seq_flag_sg1)
			BB_DBG(bb, DBG_DFS, "[cur_seq_num, pre_seq_num,is_sg1] = [%d, %d, %d]\n",
			       c_num, p_num, is_sg1);
	}
				 ...
				 struct hal_dfs_rpt *rpt, u16 dfs_rpt_idx)
{
	...
	halbb_radar_ptrn_cmprn(bb, dfs_rpt_idx, pri, pw, chrp_flag, is_sg1);
	...
	*/
	...
	bb_dfs->seg_rpt[dfs_rpt_idx] = is_sg1;
...
void halbb_dfs_dyn_setting(struct bb_info *bb)
{
	...
	if (bb_dfs->dynamic_h2l_by_tw) {
		if (bb_dfs->is_tw_en)
			halbb_set_reg(bb, cr->dfs_h2l_th, cr->dfs_h2l_th_m, bb_dfs->dfs_TW_h2l_val);//cr->dfs_h2l_th is s(8,0)
		else
			halbb_set_reg(bb, cr->dfs_h2l_th, cr->dfs_h2l_th_m, bb_dfs->dfs_nonTW_h2l_val);//cr->dfs_h2l_th is s(8,0)
	}
			 ...
			 char *output, u32 *_out_len)
{
	...
	if ((_os_strcmp(input[1], help) == 0)) {
		...
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "{6} Set dynamic_h2l_by_tw => {0}: Disable, {1}: Enable\n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "{7} Set Detection Parameter => \n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
		...
		*/
		...
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "---{24} Set the threshold of pw_diff_chrp_en_th => {Num}\n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "---{25} Set the threshold of invalid_lng_pulse_l_th => {Num}\n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "{8} Set SPEC pw/pri/ppb thd => \n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
		...
		BB_DBG_CNSL(out_len, used, output + used, out_len - used, "dynamic_h2l_by_tw = %d\n",
				bb_dfs->dynamic_h2l_by_tw);
		...
		*/
		...
		BB_DBG_CNSL(out_len, used, output + used, out_len - used, "dfs_pw_diff_chrp_en_th = %d\n",
				bb_dfs->dfs_pw_diff_chrp_en_th);
		...
		if (var[0] == 1) {
			...
			HALBB_SCAN(input[2], DCMD_DECIMAL, &var[1]);
			bb_dfs->dynamic_h2l_by_tw = (bool)var[1];
			BB_DBG_CNSL(out_len, used, output + used,
				out_len - used, "dynamic_h2l_by_tw = %d\n",
				bb_dfs->dynamic_h2l_by_tw);
		} else if (var[0] == 7) {
			HALBB_SCAN(input[2], DCMD_DECIMAL, &var[1]);
			if (var[1] == 1) {
			...
			*/
			...
			HALBB_SCAN(input[3], DCMD_DECIMAL, &var[2]);
			bb_dfs->dfs_pw_diff_chrp_en_th = (u16)var[2];
			BB_DBG_CNSL(out_len, used, output + used,
				out_len - used, "dfs_pw_diff_chrp_en_th = %d\n",
				bb_dfs->dfs_pw_diff_chrp_en_th);
			}else if (var[1] == 25) {
			HALBB_SCAN(input[3], DCMD_DECIMAL, &var[2]);
...
void halbb_cr_cfg_dfs_init(struct bb_info *bb)
{
	...
	case BB_AP2:
		...
		cr->dfs_h2l_th = DFS_H2L_TH;
		cr->dfs_h2l_th_m = DFS_H2L_TH_M;
#endif
		break;
