					 	...
					 	void *hal)
{
	...
	bus_cap->rxbuf_num = (bus_sw->rxbuf_num < bus_cap->rxbd_num) ?
		 (bus_cap->rxbd_num + 32) : bus_sw->rxbuf_num;
	bus_sw->rxbuf_num_effective = bus_cap->rxbuf_num;

	/* RPBUF NUM */
	bus_cap->rpbuf_num = (bus_sw->rpbuf_num < bus_cap->rpbd_num) ?
		 (bus_cap->rpbd_num + 32) : bus_sw->rpbuf_num;
	bus_sw->rpbuf_num_effective = bus_cap->rpbuf_num;
	...
	bus_sw->rxbuf_size_effective = bus_cap->rxbuf_size;
	...
	bus_sw->rpbuf_size_effective = bus_cap->rpbuf_size;
	...
	bus_cap->ltr_sw_ctrl = bus_hw->ltr_sw_ctrl ? true : false;
			...
			struct rtw_hal_com_t *hal_com)
{
	...
	if (phl_com->dev_cap.rx_amsdu_cut) {
		PHL_PRINT("RX A-MSDU cut adjusting: RXBD Num:%u RXBUF Num:%u RXBUF SZ:%u\n",
		          bus_cap->rxbd_num, bus_cap->rxbuf_num, bus_cap->rxbuf_size);

		#ifndef CONFIG_RTW_HW_RX_AMSDU_CUT_NO_HDR_COV
		/* RXBD NUM */
		if (bus_sw->msdu_rxbd_num)
			bus_cap->rxbd_num = MIN(bus_sw->msdu_rxbd_num, bus_hw->max_rxbd_num);

		/* RXBUF NUM */
		if (bus_sw->msdu_rxbuf_num)
			bus_cap->rxbuf_num = MAX(bus_sw->msdu_rxbuf_num, bus_cap->rxbuf_num);
		bus_sw->rxbuf_num_effective = bus_cap->rxbuf_num;
		#endif
		/* RXBUF SIZE */
		if (bus_sw->msdu_rxbuf_size)
			bus_cap->rxbuf_size = MIN(bus_sw->msdu_rxbuf_size,
			                          bus_hw->max_rxbuf_size);
		bus_sw->rxbuf_size_effective = bus_cap->rxbuf_size;

		PHL_PRINT("RX A-MSDU cut adjusted: RXBD Num:%u RXBUF Num:%u RXBUF SZ:%u\n",
		          bus_cap->rxbd_num, bus_cap->rxbuf_num, bus_cap->rxbuf_size);
	}
	...
	bus_cap->ltr_init_state = bus_sw->ltr_init_state;
	bus_cap->ltr_sw_ctrl = bus_hw->ltr_sw_ctrl ? true : false;
	bus_cap->ltr_cur_state = bus_cap->ltr_init_state;
	bus_cap->ltr_last_trigger_time = 0;
	bus_cap->ltr_sw_act_tri_cnt = 0;
	bus_cap->ltr_sw_idle_tri_cnt = 0;
}
#endif
...
void rtw_hal_final_cap_decision(struct rtw_phl_com_t *phl_com, void *hal)
{
	...
	if (dev_sw_cap->dbcc_sup && dev_hw_cap->dbcc_sup) {
		...
		if (dev_sw_cap->dbcc_force_rid) {
			dev_cap->dbcc_force_rid = dev_sw_cap->dbcc_force_rid;
	...
	dev_cap->btc_mode = dev_sw_cap->btc_mode;
        dev_cap->pwrlmt_type = PWLMT_DISABLE;

#ifdef RTW_WKARD_CUSTOM_PWRLMT_EN
	dev_sw_cap->pwrbyrate_off = PW_BY_RATE_ON;
	dev_sw_cap->pwrlmt_type = PWBYRATE_AND_PWLMT;
#endif

#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
	if (dev_sw_cap->rx_amsdu_cut && dev_hw_cap->rx_amsdu_cut)
		dev_cap->rx_amsdu_cut = true;
	else
		dev_cap->rx_amsdu_cut = false;
	if (dev_sw_cap->rx_hdr_conv && dev_hw_cap->rx_hdr_conv)
		dev_cap->rx_hdr_conv = true;
	else
		dev_cap->rx_hdr_conv = false;
#endif /* CONFIG_RTW_HW_RX_AMSDU_CUT */
	...
	dev_cap->mac_dump_cfg = dev_sw_cap->mac_dump_cfg;
}
...
void rtw_hal_reset_rf_path(void *hal)
{
	...
	hal_com->rfpath_rx_num = hal_com->def_rfpath_rx;
	hal_com->rfpath_tx_num = hal_com->def_rfpath_tx;
}
