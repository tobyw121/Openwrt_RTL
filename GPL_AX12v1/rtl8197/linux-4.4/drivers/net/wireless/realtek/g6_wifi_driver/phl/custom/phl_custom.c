...
{
	struct phl_custom_ctx *ctx = (struct phl_custom_ctx *)priv;
	struct phl_custom_evt_rpt *evt_rpt = (struct phl_custom_evt_rpt *)ctx->rpt_buf;
	...
	msg->inlen = evt_rpt->len +
	             sizeof(struct phl_custom_evt_rpt) - sizeof(u8);
...
#ifdef CONFIG_PHL_CUSTOM_FEATURE_FB
			if(cmd->customer_id == PHL_CUS_ID_FB) {
                         ...
                         struct phl_module_op_info* info)
{
	...
	struct phl_msg msg = {0};
	struct phl_msg_attribute attr = {0};
	struct phl_custom_cmd *cmd = NULL;
	u8 idx = 0xff;
	...
	switch (info->op_code) {
		case BK_MODL_OP_INPUT_CMD:
			...
			cmd = (struct phl_custom_cmd *)ctx->cmd_buf;
			SET_MSG_MDL_ID_FIELD(msg.msg_id, PHL_MDL_CUSTOM);
                           ...
                           u32 rpt_len)
{
	...
	evt_rpt->len = rpt_len;
	_os_mem_cpy(d, evt_rpt->val, rpt, rpt_len);
	return RTW_PHL_STATUS_SUCCESS;
}
struct phl_info_t*
