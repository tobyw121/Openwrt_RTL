...
void _tssi_backup_bb_registers_8852bp(
	struct rf_info *rf,
...
void _tssi_reload_bb_registers_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	u32 *reg,
	u32 *reg_backup,
...
u8 _halrf_ch_to_idx_8852bp(struct rf_info *rf, u8 channel)
{
	u8	channelIndex;
...
u8 _halrf_idx_to_ch_8852bp(struct rf_info *rf, u8 idx)
{
	u8 channelIndex;
...
void _halrf_tssi_hw_tx_8852bp(struct rf_info *rf,
...
void _halrf_tssi_rf_setting_8852bp(struct rf_info *rf,
...
void _halrf_tssi_set_sys_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
...
void _halrf_tssi_ini_txpwr_ctrl_bb_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
...
void _halrf_tssi_ini_txpwr_ctrl_bb_he_tb_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n", __func__, phy, path);
...
void _halrf_tssi_set_dck_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{	
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	if (path == RF_PATH_A) {
		if (band == BAND_ON_24G) {
			...
			halrf_wreg(rf, 0x580c, 0x0fff0000, 0x000);
			halrf_wreg(rf, 0x5814, 0x003ff000, 0x26f);
			halrf_wreg(rf, 0x5814, 0x18000000, 0x0);
		}	
	} else {
		if (band == BAND_ON_24G) {
			...
			halrf_wreg(rf, 0x780c, 0x0fff0000, 0x000);
			halrf_wreg(rf, 0x7814, 0x003ff000, 0x26f);
			halrf_wreg(rf, 0x7814, 0x18000000, 0x0);
		}
	}
}

void _halrf_tssi_set_bbgain_split_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n", __func__, phy, path);
...
void _halrf_tssi_set_tmeter_tbl_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	...
	if (band == BAND_ON_24G) {
		...
		} else if (channel >= 193 && channel <= 153) {
...
void _halrf_tssi_set_tmeter_tbl_zere_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 i;
	u32 thermal_offset_tmp = 0;
	u32 ftable_reg[TSSI_PATH_MAX_8852BP] = {0x5c00, 0x7c00};
	u32 ftable_base_reg[TSSI_PATH_MAX_8852BP] = {0x5810, 0x7810};
	u32 ftable_trigger_reg[TSSI_PATH_MAX_8852BP] = {0x5864, 0x7864};
	s8 thermal_offset[64] = {0};
...
#if 0
void _halrf_tssi_set_dac_gain_tbl_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_slope_cal_org_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n", __func__, phy, path);
...
#if 0
void _halrf_tssi_slope_cal_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	u32 tssi_trigger[TSSI_PATH_MAX_8852BP] = {0x5820, 0x7820};
	u32 tssi_cw_rpt_addr[TSSI_PATH_MAX_8852BP] = {0x1c18, 0x3c18};
	...
	for (j = 0; j < 4; j++) {
		...
		_halrf_tssi_hw_tx_8852bp(rf, phy, path, 100, 5000, power_high[j], rate, bw, true);
		...
		for (k = 0; halrf_rreg(rf, tssi_cw_rpt_addr[path], BIT(16)) == 0; k++) {
			...
			if (k > 100) {
				...
				RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI PA K] TSSI finish bit k > 100us path=%d\n",
					i);
				_halrf_tssi_hw_tx_8852bp(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
				return;
			}
		}
		_halrf_tssi_hw_tx_8852bp(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
		...
		_halrf_tssi_hw_tx_8852bp(rf, phy, path, 100, 5000, power_low[j], rate, bw, true);
...
void _halrf_tssi_set_alignment_default_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	if (path == RF_PATH_A) {
		if (band == BAND_ON_24G) {
			...
			halrf_wreg(rf, 0x5604, 0x00003f00, 0x26);
			halrf_wreg(rf, 0x5604, 0x003f0000, 0x31);
			...
			halrf_wreg(rf, 0x5634, 0x3ff00000, 0x3fb);
			halrf_wreg(rf, 0x5638, 0x000003ff, 0x02d);
			...
			if (band == BAND_ON_5G) {
				if (channel >= 36 && channel <= 64) {
					halrf_wreg(rf, 0x5634, 0x3ff00000, 0x0a);
					halrf_wreg(rf, 0x5638, 0x000003ff, 0x3d);
					halrf_wreg(rf, 0x5638, 0x000ffc00, 0x99);
				} else if (channel >= 100 && channel <= 144) {
					halrf_wreg(rf, 0x5634, 0x3ff00000, 0x06);
					halrf_wreg(rf, 0x5638, 0x000003ff, 0x3b);
					halrf_wreg(rf, 0x5638, 0x000ffc00, 0x90);
				} else if (channel >= 149 && channel <= 177) {
					halrf_wreg(rf, 0x5634, 0x3ff00000, 0x07);
					halrf_wreg(rf, 0x5638, 0x000003ff, 0x39);
					halrf_wreg(rf, 0x5638, 0x000ffc00, 0x8f);
				}
			}
		}
	} else {
		if (band == BAND_ON_24G) {
			...
			halrf_wreg(rf, 0x7604, 0x00003f00, 0x26);
			halrf_wreg(rf, 0x7604, 0x003f0000, 0x31);
			...
			halrf_wreg(rf, 0x7634, 0x3ff00000, 0x3fb);
			halrf_wreg(rf, 0x7638, 0x000003ff, 0x02d);
			halrf_wreg(rf, 0x7638, 0x000ffc00, 0x07f);
			...
			if (band == BAND_ON_5G) {
				if (channel >= 36 && channel <= 64) {
					halrf_wreg(rf, 0x7634, 0x3ff00000, 0x0a);
					halrf_wreg(rf, 0x7638, 0x000003ff, 0x3d);
					halrf_wreg(rf, 0x7638, 0x000ffc00, 0x99);
				} else if (channel >= 100 && channel <= 144) {
					halrf_wreg(rf, 0x7634, 0x3ff00000, 0x06);
					halrf_wreg(rf, 0x7638, 0x000003ff, 0x3b);
					halrf_wreg(rf, 0x7638, 0x000ffc00, 0x90);
				} else if (channel >= 149 && channel <= 177) {
					halrf_wreg(rf, 0x7634, 0x3ff00000, 0x07);
					halrf_wreg(rf, 0x7638, 0x000003ff, 0x39);
					halrf_wreg(rf, 0x7638, 0x000ffc00, 0x8f);
				}
			}
		}
	}
...
#if 0
void _halrf_tssi_alignment_default_8852bp(struct rf_info *rf,
...
void _halrf_tssi_run_slope_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n",
		__func__, phy, path);
...
void _halrf_tssi_sw_dck_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	u32 avg_adc[TSSI_PATH_MAX_8852BP][3] = {0};
	u32 delay_us = 500;
...
void _halrf_tssi_set_slope_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_set_tssi_track_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_set_txagc_offset_mv_avg_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
u32 _halrf_tssi_get_cck_efuse_group_8852bp(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
u32 _halrf_tssi_get_ofdm_efuse_group_8852bp(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
u32 _halrf_tssi_get_6g_efuse_group_8852bp(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
s8 _halrf_tssi_get_ofdm_efuse_tssi_de_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	if (band == BAND_ON_24G || band == BAND_ON_5G) {
		group_idx = _halrf_tssi_get_ofdm_efuse_group_8852bp(rf, phy);
		...
		group_idx = _halrf_tssi_get_6g_efuse_group_8852bp(rf, phy);
...
u32 _halrf_tssi_get_tssi_trim_efuse_group_8852bp(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 group_index = 0;
...
u32 _halrf_tssi_get_tssi_trim_6g_efuse_group_8852bp(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 group_index = 0;
...
s8 _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	if (band == BAND_ON_24G || band == BAND_ON_5G) {
		group_idx = _halrf_tssi_get_tssi_trim_efuse_group_8852bp(rf, phy);
		...
		group_idx = _halrf_tssi_get_tssi_trim_6g_efuse_group_8852bp(rf, phy);
...
void _halrf_tssi_alimentk_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	u32 tssi_cw_default_addr[TSSI_PATH_MAX_8852BP][4] =
	...
	_tssi_backup_bb_registers_8852bp(rf, phy, bb_reg, bb_reg_backup,
					backup_num);
	...
	for (j = 0; j < 2; j++) {
		...
		_halrf_tssi_hw_tx_8852bp(rf, phy, path, 100, 5000, power[j], rate, bw, true);
		...
		for (k = 0; halrf_rreg(rf, tssi_cw_rpt_addr[path], BIT(16)) == 0; k++) {
			...
			if (k > retry) {
				RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI PA K] TSSI finish bit k > %d mp:100ms normal:30us path=%d\n",
					k, path);
				_tssi_reload_bb_registers_8852bp(rf, phy, bb_reg, bb_reg_backup,
					backup_num);
				_halrf_tssi_hw_tx_8852bp(rf, phy, path, 100, 5000, power[j], rate, bw, false);
...
void _halrf_tssi_alimentk_done_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
#if 0
	struct halrf_tssi_info *tssi_info = &rf->tssi;
...
void halrf_tssi_backup_txagc_8852bp(struct rf_info *rf, enum phl_phy_idx phy, bool enable)
{
#if 0
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	s16 xdbm;
	u32 i, j, tx_couter = 0;
	u8 ch_idx = _halrf_ch_to_idx_8852bp(rf, channel);
	u8 channel_tmp;
	...
	_halrf_tssi_hw_tx_8852bp(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, T_HT_MF, 0, true);

#if 0
	/*Path B*/
	tx_couter[RF_PATH_B] = halrf_rreg(rf, 0x1a40, 0xffff);
	
	_halrf_tssi_hw_tx_8852bp(rf, phy, RF_PATH_B, 1000, 100, xdbm, T_HT_MF, 0, true);
	halrf_delay_ms(rf, 15);
	_halrf_tssi_hw_tx_8852bp(rf, phy, RF_PATH_B, 1000, 100, xdbm, T_HT_MF, 0, false);
#endif
	...
	tssi_info->backup_txagc_oft_ther[RF_PATH_A][ch_idx] =
		halrf_get_thermal_8852bp(rf, RF_PATH_A);

	tssi_info->backup_txagc_oft_ther[RF_PATH_B][ch_idx] =
		halrf_get_thermal_8852bp(rf, RF_PATH_B);

	_halrf_tssi_hw_tx_8852bp(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, T_HT_MF, 0, false);
	...
	//_tssi_reload_bb_registers_8852bp(rf, phy, bb_reg, bb_reg_backup, backup_num);
#endif
}

void halrf_do_tssi_8852bp(struct rf_info *rf,
...
//	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8852BP;
	...
	if (rf->hal_com->dbcc_en) {
		if (phy == HW_PHY_0) {
			...
			path_tmp = RF_PATH_B;
			path_max = TSSI_PATH_MAX_8852BP;
		}
	}

	tssi_info->check_backup_txagc[ch_idx] = false;

	halrf_tssi_disable_8852bp(rf, phy);

	for (i = path_tmp; i < path_max; i++) {
		halrf_write_fwofld_start(rf);		/*FW Offload Start*/

		_halrf_tssi_set_sys_8852bp(rf, phy, i);
		_halrf_tssi_ini_txpwr_ctrl_bb_8852bp(rf, phy, i);
		_halrf_tssi_ini_txpwr_ctrl_bb_he_tb_8852bp(rf, phy, i);
		_halrf_tssi_set_dck_8852bp(rf, phy, i);
		_halrf_tssi_set_bbgain_split_8852bp(rf, phy, i);
		_halrf_tssi_set_tmeter_tbl_8852bp(rf, phy, i);
		/*_halrf_tssi_set_dac_gain_tbl_8852bp(rf, phy, i);*/
		_halrf_tssi_slope_cal_org_8852bp(rf, phy, i);
		_halrf_tssi_set_alignment_default_8852bp(rf, phy, i);
		_halrf_tssi_set_slope_8852bp(rf, phy, i);
		_halrf_tssi_run_slope_8852bp(rf, phy, i);

		halrf_write_fwofld_end(rf);		/*FW Offload End*/

//		if (rfe_type == 50)
		_halrf_tssi_sw_dck_8852bp(rf, phy, i);

		//halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_START);
		//halrf_tmac_tx_pause(rf, phy, true);
		//if (hwtx_en == true)
		//	_halrf_tssi_alimentk_8852bp(rf, phy, i);
		//halrf_tmac_tx_pause(rf, phy, false);
		//halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_STOP);
	}
	...
	if (phl_is_mp_mode(rf->phl_com)) {
		if (tssi_info->tssi_type[HW_PHY_0] == TSSI_ON || tssi_info->tssi_type[HW_PHY_0] == TSSI_CAL) {
			halrf_tssi_enable_8852bp(rf, phy);
			halrf_tssi_set_efuse_to_de_8852bp(rf, phy);
		} else
			halrf_tssi_disable_8852bp(rf, phy);
	} else {
		if (rf->support_ability & HAL_RF_TX_PWR_TRACK) {
			...
			halrf_tssi_set_efuse_to_de_8852bp(rf, phy);
			//if (hwtx_en == true)
			//	halrf_tssi_backup_txagc_8852bp(rf, phy, true);
		}
	}
...
void halrf_do_tssi_scan_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy)
{
//	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8852BP;
...
//		if (rfe_type == 50)
		...
		if (tssi_info->alignment_done[i][band] == true)
			_halrf_tssi_alimentk_done_8852bp(rf, phy, i);
		else {
			if (rf->hal_com->cv == CAV)
				_halrf_tssi_alignment_default_8852bpa(rf, phy, i);
			else
				_halrf_tssi_alignment_default_8852bpb(rf, phy, i, false);
		}
#endif
	}
...
void halrf_tssi_get_efuse_8852bp(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	s8 de_offset = 0;
...
bool halrf_tssi_check_efuse_data_8852bp(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 i, j, countr = 0;
	...
	halrf_tssi_get_efuse_8852bp(rf, phy);
...
void halrf_set_tssi_de_for_tx_verify_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy, u32 tssi_de, u8 path)
{
	...
	tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy, path);
...
void halrf_set_tssi_de_offset_8852bp(struct rf_info *rf,
...
void halrf_set_tssi_de_offset_zero_8852bp(struct rf_info *rf,
							enum phl_phy_idx phy)
{
...
void halrf_tssi_enable_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8852BP;
	...
	for (i = path_tmp; i < path_max; i++) {
		/*_halrf_tssi_set_tssi_slope_8852bp(rf, phy, i);*/
		_halrf_tssi_set_tssi_track_8852bp(rf, phy, i);
		_halrf_tssi_set_txagc_offset_mv_avg_8852bp(rf, phy, i);
...
void halrf_tssi_disable_8852bp(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8852BP;
...
s32 halrf_get_online_tssi_de_8852bp(struct rf_info *rf, enum phl_phy_idx phy_idx,
				u8 path, s32 dbm, s32 puot)
{
	...
	tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy_idx, path);
...
void halrf_tssi_set_efuse_to_de_8852bp(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	u32 path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8852BP;

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======> %s\n", __func__);

	if (tssi_info->tssi_type[HW_PHY_0] == TSSI_CAL) {
		/*halrf_set_tssi_de_offset_zero_8852bp(rf, phy);*/
		RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======> %s TSSI_CAL mode set TSSI Trim !!!\n", __func__);

		for (i = 0; i < TSSI_PATH_MAX_8852BP; i++) {
			tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy, i);
	...
	for (i = path_tmp; i < path_max; i++) {
		/*CCK*/
		group_idx = _halrf_tssi_get_cck_efuse_group_8852bp(rf, phy);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy, i);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8852bp(rf, phy, i);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy, i);
...
void halrf_tssi_default_txagc_8852bp(struct rf_info *rf,
...
//			halrf_do_tssi_8852bp(rf, phy, false);
			return;
		}
	} else {
		/*SCAN_END*/
		...
		_halrf_tssi_alimentk_done_8852bp(rf, phy, RF_PATH_A);
		_halrf_tssi_alimentk_done_8852bp(rf, phy, RF_PATH_B);
...
void halrf_tssi_scan_ch_8852bp(struct rf_info *rf, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	if (!phl_is_mp_mode(rf->phl_com)) {
		...
		if (tssi_info->alignment_done[path][band] == true)
			_halrf_tssi_alimentk_done_8852bp(rf, 0, path);
		else {
#if 0
			if (rf->hal_com->cv == CAV)
				_halrf_tssi_alignment_default_8852bpa(rf, 0, path);
			else
				_halrf_tssi_alignment_default_8852bpb(rf, 0, path, false);
#endif
		}
...
u32 halrf_tssi_get_final_8852bp(struct rf_info *rf, enum rf_path path)
{
	u32 i;
...
void halrf_tssi_hw_tx_8852bp(struct rf_info *rf,
			enum phl_phy_idx phy, u8 path, u16 cnt, s16 dbm, u32 rate, u8 bw,
			bool enable)
{
	_halrf_tssi_hw_tx_8852bp(rf, phy, path, cnt, 5000, dbm, T_HT_MF, 0, enable);
}

void halrf_tssi_ant_open_8852bp(struct rf_info *rf)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	if ((halrf_rreg(rf, 0x1c60, 0xff000000) == 0x7f) ||
		...
		halrf_tssi_disable_8852bp(rf, HW_PHY_0);
		halrf_tssi_enable_8852bp(rf, HW_PHY_0);
	}
}

void halrf_get_tssi_info_8852bp(struct rf_info *rf,
		char input[][16], u32 *_used, char *output, u32 *_out_len)
{
	...
	s32 delta_tssi;
	s32 diff_ther;
	s32 int_tmp[TSSI_PATH_MAX_8852BP], float_tmp[TSSI_PATH_MAX_8852BP];
	s8 txagc_offset[TSSI_PATH_MAX_8852BP] = {0};
