...
void rtw_halbb_plcp_gen_init(struct bb_info *bb, struct halbb_plcp_info *in,
	...
	if (in->ppdu_type == HE_TB_FMT) {
		...
		case 0:
			if (((in->usr[0].ru_alloc >> 1) <= 8) || ((in->usr[0].ru_alloc >> 1) >= 37 && (in->usr[0].ru_alloc >> 1) <= 40)
				...
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0x80 : 0x1;
			break;
		...
		case 1:
			if (((in->usr[0].ru_alloc >> 1) <= 8) || ((in->usr[0].ru_alloc >> 1) >= 37 && (in->usr[0].ru_alloc >> 1) <= 40)
				...
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0x40 : 0x2;

			else if ((in->usr[0].ru_alloc >> 1) == 65)
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0xc0 : 0x3;
		...
		case 2:
			if (((in->usr[0].ru_alloc >> 1) <= 8) || ((in->usr[0].ru_alloc >> 1) >= 37 && (in->usr[0].ru_alloc >> 1) <= 40)
				...
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0x20 : 0x4;
				...
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0x10 : 0x8;
				...
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0x30 : 0xc;

			else if ((in->usr[0].ru_alloc >> 1) == 18)
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0x60 : 0x6;

			else if ((in->usr[0].ru_alloc >> 1) == 67)
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0xf0 : 0xf;
...
struct bb_h2c_he_sigb *he_sigb = &bb->bb_h2c_he_sigb_i;
	...
	if (phy_idx == HW_PHY_0) {
		for (i = 0; i < 16; i++) {
			he_sigb->n_sym_sigb_ch1[i].address_0 = (u8)(n_sym_sigb_ch1_phy0[i] & 0xff);
			he_sigb->n_sym_sigb_ch1[i].address_1 = (u8)((n_sym_sigb_ch1_phy0[i] & 0xff00) >> 8);
			he_sigb->n_sym_sigb_ch1[i].address_2 = (u8)((n_sym_sigb_ch1_phy0[i] & 0xff0000) >> 16);
			he_sigb->n_sym_sigb_ch1[i].address_3 = (u8)((n_sym_sigb_ch1_phy0[i] & 0xff000000) >> 24);

			he_sigb->n_sym_sigb_ch2[i].address_0 = (u8)(n_sym_sigb_ch2_phy0[i] & 0xff);
			he_sigb->n_sym_sigb_ch2[i].address_1 = (u8)((n_sym_sigb_ch2_phy0[i] & 0xff00) >> 8);
			he_sigb->n_sym_sigb_ch2[i].address_2 = (u8)((n_sym_sigb_ch2_phy0[i] & 0xff0000) >> 16);
			he_sigb->n_sym_sigb_ch2[i].address_3 = (u8)((n_sym_sigb_ch2_phy0[i] & 0xff000000) >> 24);
		}
	} /*else {
		for (i = 0; i < 16; i++) {
	...
	he_sigb->sigb_i.ppdu_bw = (u8)in->dbw;
	he_sigb->sigb_i.sta_list_num = (u8)in->n_user;

	for (i = 0; i < in->n_user; i++) {
		he_sigb->sigb_i.usr_info[i].coding = (u8)in->usr[i].fec;
		he_sigb->sigb_i.usr_info[i].nsts = (u8)(in->usr[i].nss << in->stbc) - 1;
		he_sigb->sigb_i.usr_info[i].ru_position = (u8)in->usr[i].ru_alloc;
		he_sigb->sigb_i.usr_info[i].dcm = (u8)in->usr[i].dcm;
		he_sigb->sigb_i.usr_info[i].mcs = (u8)in->usr[i].mcs;
	}
	...
	BB_DBG(bb, DBG_PHY_CONFIG, "[SIGB] {ppdu_bw, sta_list_num}={%d,%d}\n", he_sigb->sigb_i.ppdu_bw, he_sigb->sigb_i.sta_list_num);
	BB_DBG(bb, DBG_PHY_CONFIG, "[SIGB] {coding,nsts,ru_position,dcm,mcs}={%d,%d,%d,%d,%d}\n",
		he_sigb->sigb_i.usr_info[0].coding,
		he_sigb->sigb_i.usr_info[0].nsts,
		he_sigb->sigb_i.usr_info[0].ru_position,
		he_sigb->sigb_i.usr_info[0].dcm,
		he_sigb->sigb_i.usr_info[0].mcs);
	/*
		    ...
		    struct usr_plcp_gen_in *user, enum phl_phy_idx phy_idx)
{
	...
	rtw_halbb_plcp_gen_init(bb, in, &in_plcp);
