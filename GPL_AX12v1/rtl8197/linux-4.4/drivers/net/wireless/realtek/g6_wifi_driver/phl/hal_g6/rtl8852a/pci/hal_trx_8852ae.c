 ...
 */
static u8 hal_query_txch_num_8852ae(void)
{
	u8 ch_num = 0;

	ch_num = TX_DMA_CHANNEL_ENTRY_8852AE;
 ...
 */
static u8 hal_query_rxch_num_8852ae(void)
{
	u8 ch_num = 0;

	ch_num = RX_DMA_CHANNEL_ENTRY_8852AE;

	return ch_num;
}
static u8 hal_qsel_to_tid_8852ae(struct hal_info_t *hal, u8 qsel_id, u8 tid_indic)
{
	u8 tid = 0;
...
/**
 * Get target DMA channel's BD ram hw register of rtl8852a
 * @dma_ch: input target dma channel with the hw definition of rtl8852ae
 * return the BD ram hw register
 */
static u32 _hal_get_bd_ram_reg_8852ae(u8 dma_ch)
{
	u32 reg = 0;

	switch (dma_ch) {
	case ACH0_QUEUE_IDX_8852AE:
		reg = R_AX_ACH0_BDRAM_CTRL;
		break;
	case ACH1_QUEUE_IDX_8852AE:
		reg = R_AX_ACH1_BDRAM_CTRL;
		break;
	case ACH2_QUEUE_IDX_8852AE:
		reg = R_AX_ACH2_BDRAM_CTRL;
		break;
	case ACH3_QUEUE_IDX_8852AE:
		reg = R_AX_ACH3_BDRAM_CTRL;
		break;
	case ACH4_QUEUE_IDX_8852AE:
		reg = R_AX_ACH4_BDRAM_CTRL;
		break;
	case ACH5_QUEUE_IDX_8852AE:
		reg = R_AX_ACH5_BDRAM_CTRL;
		break;
	case ACH6_QUEUE_IDX_8852AE:
		reg = R_AX_ACH6_BDRAM_CTRL;
		break;
	case ACH7_QUEUE_IDX_8852AE:
		reg = R_AX_ACH7_BDRAM_CTRL;
		break;
	case MGQ_B0_QUEUE_IDX_8852AE:
		reg = R_AX_CH8_BDRAM_CTRL;
		break;
	case HIQ_B0_QUEUE_IDX_8852AE:
		reg = R_AX_CH9_BDRAM_CTRL;
		break;
	case MGQ_B1_QUEUE_IDX_8852AE:
		reg = R_AX_CH10_BDRAM_CTRL;
		break;
	case HIQ_B1_QUEUE_IDX_8852AE:
		reg = R_AX_CH11_BDRAM_CTRL;
		break;
	case FWCMD_QUEUE_IDX_8852AE:
...
/**
 * Get target DMA channel's BD num hw register of rtl8852a
 * @dma_ch: input target dma channel with the hw definition of rtl8852ae
 * return the BD num hw register
 */
static u32 _hal_get_bd_num_reg_8852ae(u8 dma_ch)
{
	u32 reg = 0;
	...
	case FWCMD_QUEUE_IDX_8852AE:
		...
		break;
	case RX_QUEUE_IDX_8852AE:
		reg = R_AX_RXQ_RXBD_NUM;
		break;
	case RP_QUEUE_IDX_8852AE:
...
/**
 * Get target DMA channel's BD Desc hw register of rtl8852a
 * @dma_ch: input target dma channel with the hw definition of rtl8852ae
 * return the BD Desc hw register
 */
static void _hal_get_bd_desc_reg_8852ae(u8 dma_ch, u32 *addr_l, u32 *addr_h)
{
	u32 reg = 0;
...
/**
 * Get target DMA channel's BD Index hw register of rtl8852a
 * @dma_ch: input target dma channel with the hw definition of rtl8852ae
 * return the BD Index hw register
 */
static u32 _hal_get_bd_idx_reg_8852ae(u8 dma_ch)
{
	u32 reg = 0;
 ...
 * to rtl8852ae hw tx dma channel
 ...
 *
 * returns the mapping hw dma channel defined by XXX_QUEUE_IDX_8852AE
 * if the input parameter is unknown value, returns ACH0_QUEUE_IDX_8852AE
 */
static u8 hal_mapping_hw_tx_chnl_8852ae(u16 macid, enum rtw_phl_ring_cat cat,
					u8 band)
{
	u8 dma_ch = 0;
	...
	if (0 == band) {
		...
		case RTW_PHL_RING_CAT_TID0:
		case RTW_PHL_RING_CAT_TID3:
			dma_ch = ACH4_QUEUE_IDX_8852AE;
			break;
		case RTW_PHL_RING_CAT_TID1:
		case RTW_PHL_RING_CAT_TID2:
			dma_ch = ACH5_QUEUE_IDX_8852AE;
			break;
		case RTW_PHL_RING_CAT_TID4:
		case RTW_PHL_RING_CAT_TID5:
			dma_ch = ACH6_QUEUE_IDX_8852AE;
			break;
		case RTW_PHL_RING_CAT_TID6:
		case RTW_PHL_RING_CAT_TID7:
			dma_ch = ACH7_QUEUE_IDX_8852AE;
			break;
		case RTW_PHL_RING_CAT_MGNT:
			dma_ch = MGQ_B1_QUEUE_IDX_8852AE;
			break;
		case RTW_PHL_RING_CAT_HIQ:
			dma_ch = HIQ_B1_QUEUE_IDX_8852AE;
			break;
		default:
			dma_ch = ACH0_QUEUE_IDX_8852AE;
			PHL_TRACE(COMP_PHL_DBG, _PHL_WARNING_, "[WARNING]unknown category (%d)\n",
 ...
 */
static u16 hal_get_avail_txbd_8852ae(struct rtw_hal_com_t *hal_com, u8 ch_idx,
				     u16 *host_idx, u16 *hw_idx)
{
	...
	tx_dma_ch = ACH0_QUEUE_IDX_8852AE + ch_idx;

	reg = _hal_get_bd_idx_reg_8852ae(tx_dma_ch);
	if (0xFFFF == reg) {
		...
		*hw_idx = (u16)((tmp32 >> 16) & 0x0FFF);
		...
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_get_avail_txbd_8852ae => dma_ch %d, host_idx %d, "
...
static u16 hal_get_avail_txbd_sw_8852ae(struct rtw_hal_com_t *hal_com,
...
static u16 hal_get_rxbd_num_8852ae(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
...
static u16 hal_get_rxbuf_num_8852ae(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
...
static u16 hal_get_rxbuf_size_8852ae(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
 ...
 * NOTE, input host_idx and hw_idx ptr shall NOT be NULL
 */
static u16 hal_get_avail_rxbd_8852ae(struct rtw_hal_com_t *hal_com, u8 ch_idx,
				     u16 *host_idx, u16 *hw_idx)
{
	u16 avail_rxbd = 0;
	u32 tmp32 = 0, reg = 0;
	u8 rx_dma_ch = 0;
	u16 rxbd_num = hal_get_rxbd_num_8852ae(hal_com, ch_idx);

	rx_dma_ch = RX_QUEUE_IDX_8852AE + ch_idx;

	reg = _hal_get_bd_idx_reg_8852ae(rx_dma_ch);
	if (0xFFFF == reg) {
...
void _hal_fill_wp_seq_field_8852ae(u8 *seq_info, u16 wp_seq)
{
	/* update sw wp seq */
...
void _hal_fill_wp_addr_info_8852ae(struct rtw_hal_com_t *hal_com,
				   ...
				   u8 num, u8 mpdu_ls, u8 msdu_ls)
{
	...
	if (hal_com->cv != CAV) /* 8852A doesn't support normal mode */
		SET_ADDR_INFO_ADDR_HIGH(addr_info, pkt->phy_addr_h);
}

u8 _hal_get_tid_indic_8852ae(u8 tid)
{
	u8 tid_indic = 0;
	switch (tid) {
		case RTW_PHL_RING_CAT_TID0:
...
_hal_txsc_update_wd_8852ae(struct hal_info_t *hal,
						struct rtw_phl_pkt_req *req, u32 *wd_len)
{
	...
	dma_ch = hal_mapping_hw_tx_chnl_8852ae(mdata->macid,
											mdata->cat,
											mdata->band);
 ...
 */
static enum rtw_hal_status
hal_update_wd_8852ae(struct hal_info_t *hal,
                     struct rtw_phl_pkt_req *req)
{
	...
	do {
...
#ifdef CONFIG_PHL_TXSC
		hstatus = _hal_txsc_update_wd_8852ae(hal, req, &wd_len);
#else
		/* connect with halmac */
		hstatus = rtw_hal_mac_ax_fill_txdesc(hal->mac, tx_req, req->wd_page,
						&wd_len);
		...
		tid_indic = _hal_get_tid_indic_8852ae(tx_req->mdata.tid);

		seq_ofst = wd_len;
		wp_seq = (1 == tid_indic) ?
				(req->wp_seq | WP_TID_INDIC_RESERVED_BIT) : req->wp_seq;
		_hal_fill_wp_seq_field_8852ae(req->wd_page + seq_ofst, wp_seq);
		...
		for (i = 0; i < tx_req->mdata.addr_info_num; i++) {
			...
			_hal_fill_wp_addr_info_8852ae(hal_com,
		...
		/* 8852AE length in txbd(wd_len) should be fix to 128 */
 ...
 */
static enum rtw_hal_status
hal_update_txbd_8852ae(struct hal_info_t *hal,
		       ...
		       u8 ch_idx, u16 wd_num)
{
	...
	do {
		...
		avail_txbd = hal_get_avail_txbd_sw_8852ae(hal->hal_com, txbd_ring, ch_idx,
						       &txbd_host_idx, &hw_idx);
		...
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_update_txbd_8852ae => ch_idx %d, host_idx %d, "
		...
		while (wcnt > 0) {
			...
			SET_TXBUFFER_DESC_ADD_LOW(target_txbd,
						  wd_page->phy_addr_l);
			if (hal_com->cv != CAV) {
				/* 8852A doesn't support normal mode */
 ...
 */
static enum rtw_hal_status
hal_trigger_txdma_8852ae(struct hal_info_t *hal,
			struct tx_base_desc *txbd_ring, u8 ch_idx)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	u8 tx_dma_ch;
	...
	do {
		/* connect with halmac */
		tx_dma_ch = ACH0_QUEUE_IDX_8852AE + ch_idx;
		txbd_reg = _hal_get_bd_idx_reg_8852ae(tx_dma_ch);
		if (0xFFFF == txbd_reg)
			break;
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_trigger_txdma_8852ae => dma_ch %d, host_idx %d.\n",
...
static enum rtw_hal_status hal_pltfm_tx_8852ae(void *hal, struct rtw_h2c_pkt *h2c_pkt)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
#if 0
	...
	_os_spinlock(hal_to_drvpriv(hal_info), &txbd_ring[FWCMD_QUEUE_IDX_8852AE].txbd_lock, _ps, NULL);
	hstatus = hal_update_txbd_8852ae(hal_info, txbd_ring, &wd_page,
							FWCMD_QUEUE_IDX_8852AE, 1);
	_os_spinunlock(hal_to_drvpriv(hal_info), &txbd_ring[FWCMD_QUEUE_IDX_8852AE].txbd_lock, _ps, NULL);

	/* enqueue busy queue */

	hstatus = hal_trigger_txdma_8852ae(hal_info, txbd_ring, FWCMD_QUEUE_IDX_8852AE);
#endif
	return hstatus;
}

u8 hal_get_fwcmd_queue_idx_8852ae(void)
{
	return FWCMD_QUEUE_IDX_8852AE;
}

static u8 hal_check_rxrdy_8852ae(struct rtw_phl_com_t *phl_com,
...
static void _hal_show_tx_failure_rsn_8852ae(u8 txsts)
{
...
u16 hal_handle_rx_report_8852ae(struct hal_info_t *hal, u8 *rp,
				...
				u8* macid, u8* ac_queue, u8 *txsts)
{
	...
	do {
		...
		*wp_seq &= (WP_RESERVED_SEQ);
		tid = hal_qsel_to_tid_8852ae(hal, qsel_value, tid_indic);
		*dma_ch = hal_mapping_hw_tx_chnl_8852ae(*macid, tid, band);
		...
		if (TX_STATUS_TX_DONE != *txsts) {
			_hal_show_tx_failure_rsn_8852ae(*txsts);
			*sw_retry = true;
 ...
 */
static enum rtw_hal_status
hal_handle_ppdusts_8852ae(void *hal, u8 *psbuf, u16 sz,
 ...
 **/
static enum rtw_hal_status
_hal_parsing_rx_wd_8852ae(struct hal_info_t *hal, u8 *desc,
...
static u8 hal_handle_rxbd_info_8852ae(struct hal_info_t *hal,
						u8 *rxbd_info, u16 *size)
{
	...
	if (fs == 1) {
		if (ls == 1)
			...
			if (hal->hal_com->max_fs_sz < pld_size)
				hal->hal_com->max_fs_sz = pld_size;
		...
		#ifdef CONFIG_RTW_DEBUG_RX_SZ
		if (hal->hal_com->max_non_fs_sz < pld_size)
			hal->hal_com->max_non_fs_sz = pld_size;
		#endif /* CONFIG_RTW_DEBUG_RX_SZ */
		if (ls == 1)
			pkt_rdy = false;
		else
 ...
 */
static enum rtw_hal_status
hal_update_rxbd_8852ae(struct hal_info_t *hal, struct rx_base_desc *rxbd,
 ...
 */
static enum rtw_hal_status
hal_notify_rxdone_8852ae(struct hal_info_t *hal,
		struct rx_base_desc *rxbd, u8 ch, u16 rxcnt)
{
	...
	do {
		rx_dma_ch = RX_QUEUE_IDX_8852AE + ch;
		reg = _hal_get_bd_idx_reg_8852ae(rx_dma_ch);
		/* connect with halmac */
		if (0xFFFF == reg)
			break;
...
struct bd_ram bdram_table[] = {
		/* ACH0_QUEUE_IDX_8852AE */ {0, 25, 4},
		/* ACH1_QUEUE_IDX_8852AE */ {25, 25, 4},
		/* ACH2_QUEUE_IDX_8852AE */ {50, 25, 4},
		/* ACH3_QUEUE_IDX_8852AE */ {75, 25, 4},
		/* ACH4_QUEUE_IDX_8852AE */ {100, 25, 4},
		/* ACH5_QUEUE_IDX_8852AE */ {125, 25, 4},
		/* ACH6_QUEUE_IDX_8852AE */ {150, 25, 4},
		/* ACH7_QUEUE_IDX_8852AE */ {175, 25, 4},
		/* MGQ_B0_QUEUE_IDX_8852AE */ {185, 10, 4},
		/* HIQ_B0_QUEUE_IDX_8852AE */ {195, 10, 4},
		/* MGQ_B1_QUEUE_IDX_8852AE */ {205, 10, 4},
		/* HIQ_B1_QUEUE_IDX_8852AE */ {215, 10, 4},
		/* FWCMD_QUEUE_IDX_8852AE */ {225, 10, 4}
};

static void _hal_tx_init_bd_ram_8852ae(struct hal_info_t *hal)
{
	u32 reg = 0;
	u32 value = 0;
	u8 i = 0;

	for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8852AE; i++) {
		/*if (FWCMD_QUEUE_IDX_8852AE == i)
			continue;*/
		value = 0;
		...
		reg = _hal_get_bd_ram_reg_8852ae(i);
		if (0 != reg)
...
static void _hal_trx_init_bd_num_8852ae(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u16 txbd_num = (u16)hal_com->bus_cap.txbd_num;
	u16 rxbd_num = 0;
	...
	for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8852AE; i++) {
		...
		reg = _hal_get_bd_num_reg_8852ae(i);
		if (0 != reg)
	...
	for (i = RX_QUEUE_IDX_8852AE;
		 i < RX_QUEUE_IDX_8852AE + RX_DMA_CHANNEL_ENTRY_8852AE; i++) {
		 /*if (FWCMD_QUEUE_IDX_8852AE == i)
			 continue;*/

		 ch_idx = (u8)(i - RX_QUEUE_IDX_8852AE);
		 rxbd_num = hal_get_rxbd_num_8852ae(hal_com, ch_idx);
...
static void _hal_trx_init_bd_8852ae(struct hal_info_t *hal, u8 *txbd_buf, u8 *rxbd_buf)
{
	...
	if (NULL != txbd_buf && NULL != rxbd_buf) {
		...
		for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8852AE; i++) {
			...
			_hal_get_bd_desc_reg_8852ae(i, &reg_addr_l, &reg_addr_h);
			if (0 != reg_addr_l)
		...
		for (i = 0; i < RX_DMA_CHANNEL_ENTRY_8852AE; i++) {
			 rxch_idx = i + RX_QUEUE_IDX_8852AE;
			_hal_get_bd_desc_reg_8852ae(rxch_idx, &reg_addr_l, &reg_addr_h);
			if (0 != reg_addr_l)
...
static void hal_cfg_wow_txdma_8852ae(struct hal_info_t *hal, u8 en)
{
...
static u8 hal_poll_txdma_idle_8852ae(struct hal_info_t *hal)
{
	struct mac_ax_txdma_ch_map ch_map;
...
/**
 * the function will deinitializing 8852ae specific data and hw configuration
 */
static void hal_trx_deinit_8852ae(struct hal_info_t *hal)
{
	/*struct rtw_hal_com_t *hal_com = hal->hal_com;*/
}

/**
 * the function will initializing 8852ae specific data and hw configuration
 */
static enum rtw_hal_status hal_trx_init_8852ae(struct hal_info_t *hal, u8 *txbd_buf, u8 *rxbd_buf)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
 ...
 */
void hal_query_freerun_cnt_8852a(struct hal_info_t *hal, u32 *freerun_l, u32 *freerun_h)
{
...
enum rtw_hal_status
hal_set_dlru_fix_tbl_8852ae(struct hal_info_t *hal, struct rtw_phl_dlru_fix_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;

	hstatus = rtw_hal_bb_set_dlru_fix_tbl_ax4ru(hal, tbl);

	return hstatus;
}

enum rtw_hal_status
hal_set_ulru_fix_tbl_8852ae(struct hal_info_t *hal, struct rtw_phl_ulru_fix_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;

	hstatus = rtw_hal_bb_set_ulru_fix_tbl_ax4ru(hal, tbl);
...
#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
static enum rtw_hal_status
hal_trx_watchdog_8852ae(struct rtw_phl_com_t *phl_com,
                        struct hal_info_t *hal)
{
	...
	do {
		/* RX read pointer */
		reg32 = hal_read32(hal_com, R_AX_RXQ_RXBD_IDX);
		host_idx = (u16)reg32;
		hw_idx = (u16)(reg32 >> 16);
		if (   (host_idx != hw_idx)
...
void hal_trx_ops_init_8852ae(void)
{
	ops.init = hal_trx_init_8852ae;
	ops.deinit = hal_trx_deinit_8852ae;
	ops.query_tx_res = hal_get_avail_txbd_8852ae;
	ops.query_tx_res_sw = hal_get_avail_txbd_sw_8852ae;
	ops.query_rx_res = hal_get_avail_rxbd_8852ae;
	ops.get_rxbd_num = hal_get_rxbd_num_8852ae;
	ops.get_rxbuf_num = hal_get_rxbuf_num_8852ae;
	ops.get_rxbuf_size = hal_get_rxbuf_size_8852ae;
	ops.cfg_wow_txdma = hal_cfg_wow_txdma_8852ae;
	ops.poll_txdma_idle = hal_poll_txdma_idle_8852ae;
	ops.map_hw_tx_chnl = hal_mapping_hw_tx_chnl_8852ae;
	ops.qsel_to_tid = hal_qsel_to_tid_8852ae;
	ops.query_txch_num = hal_query_txch_num_8852ae;
	ops.query_rxch_num = hal_query_rxch_num_8852ae;
	ops.update_wd = hal_update_wd_8852ae;
	ops.update_txbd = hal_update_txbd_8852ae;
	ops.tx_start = hal_trigger_txdma_8852ae;
	ops.check_rxrdy = hal_check_rxrdy_8852ae;
	ops.handle_rxbd_info = hal_handle_rxbd_info_8852ae;
	ops.handle_rx_buffer = hal_handle_rx_buffer_8852a;
	ops.update_rxbd = hal_update_rxbd_8852ae;
	ops.notify_rxdone = hal_notify_rxdone_8852ae;
	ops.handle_wp_rpt = hal_handle_rx_report_8852ae;
	ops.query_freerun = hal_query_freerun_cnt_8852a;
	ops.get_fwcmd_queue_idx = hal_get_fwcmd_queue_idx_8852ae;
#ifdef CONFIG_WFA_OFDMA_Logo_Test
	ops.set_dlru_fix_tbl = hal_set_dlru_fix_tbl_8852ae;
	ops.set_ulru_fix_tbl = hal_set_ulru_fix_tbl_8852ae;
#endif
#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
	ops.trx_watchdog = hal_trx_watchdog_8852ae;
#endif /* CONFIG_RTW_HW_TRX_WATCHDOG */
}
