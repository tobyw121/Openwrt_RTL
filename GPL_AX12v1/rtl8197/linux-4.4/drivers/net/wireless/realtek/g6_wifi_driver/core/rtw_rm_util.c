...
u8 rm_get_frame_rsni(struct rm_obj *prm, union recv_frame *pframe)
{
	...
	/* TODO: John_lei, current rx data rate is not pass inside recv_frame, 
	   so temparily remove this condition
	*/
	/*if (IS_CCK_RATE((HwRateToMPTRate(pframe->u.hdr.attrib.data_rate))))
		val8 = 255;
	else
	*/
	{
