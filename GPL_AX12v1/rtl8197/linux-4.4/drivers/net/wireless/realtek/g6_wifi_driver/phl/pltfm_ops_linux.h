...
#include "../../wnic_skb_pool/recycle_conf.h"
#include "../../wnic_skb_pool/recycle_skb.h"

struct rtw_rx_buf_alloc_data {
	void 		*drv_priv;
	int		pool_id;
};
...
static inline void _os_vm_cache_inv(void *d, void *v, u32 buf_sz)
{
	dma_cache_inv((size_t)v, buf_sz);
}
...
struct rx_buf_info {
	u8	type; /* 0: non-shared, 1: shared first half, 2: shared second half */
	u16	rx_buf_sz;
};
...
static inline size_t _os_pkt_get_rx_buf_sz(void *os_priv)
{
	struct sk_buff *skb = os_priv;
#ifdef CONFIG_RTW_RX_BUF_SHARING
	struct rx_buf_info *rxb_info = (struct rx_buf_info *)skb->cb;
	return rxb_info->rx_buf_sz;
#else
	return (skb_end_offset(skb) - PHL_RX_HEADROOM);
#endif /* CONFIG_RTW_RX_BUF_SHARING */
}
...
#ifdef CONFIG_RTW_RX_BUF_SHARING
	struct rx_buf_info *rxb_info = (struct rx_buf_info *)skb->cb;
	skb_buf_sz = rxb_info->rx_buf_sz;
	if (rxb_info->type == 2) {
		skb->data =   skb->head + PHL_RX_HEADROOM + skb_buf_sz
			    + PHL_RX_HEADROOM;
	} else {
		skb->data = skb->head + PHL_RX_HEADROOM;
	}
#else
	skb->data = skb->head + PHL_RX_HEADROOM;
	skb_buf_sz =  skb_end_offset(skb) - PHL_RX_HEADROOM;
#endif
	skb->len = 0;
...
#ifdef CONFIG_PCI_HCI
static inline void
_os_pkt_buf_unmap_rx_pci(struct dvobj_priv *pobj, u32 bus_addr_l,
			 u32 bus_addr_h, void *os_priv)
{
	PPCI_DATA pci_data = dvobj_to_pci(pobj);
	struct pci_dev *pdev = pci_data->ppcidev;
	struct sk_buff *skb = os_priv;
	u32 skb_buf_sz = _os_pkt_get_rx_buf_sz(os_priv);
...
#endif
} /* _os_pkt_buf_unmap_rx_pci */
#endif /*CONFIG_PCI_HCI*/

static inline void *
_os_pkt_buf_unmap_rx(void *d, u32 bus_addr_l, u32 bus_addr_h, u32 buf_sz,
                     void *os_priv)
{
#ifdef CONFIG_PCI_HCI
	struct dvobj_priv *pobj = (struct dvobj_priv *)d;
					 ...
					 void **os_priv)
{
	...
	struct rtw_rx_buf_alloc_data *alloc_data = d;
	struct dvobj_priv *pobj = alloc_data->drv_priv;
	#else
	struct dvobj_priv *pobj = d;
	#endif /* CONFIG_WNIC_RECYCLED_SKB */
	...
	u32 rxbuf_size = buf_sz;
	u32 skb_buf_sz;

	#ifdef CONFIG_WNIC_RECYCLED_SKB
	if (alloc_data->pool_id < 1) {
		printk_ratelimited(KERN_ERR"Alloc RX from invalid pool id %u\n", alloc_data->pool_id);
	}
	skb = dev_alloc_skb_recy_wnic(rxbuf_size, alloc_data->pool_id);
	#else
	...
	/* Remove system reserved header */
	skb->data = skb->head + PHL_RX_HEADROOM;
	skb_reset_tail_pointer(skb);
	skb_buf_sz =  skb_end_offset(skb) - PHL_RX_HEADROOM;
#ifdef CONFIG_RTW_RX_BUF_SHARING
	((struct rx_buf_info *)skb->cb)->rx_buf_sz = skb_buf_sz;
	((struct rx_buf_info *)skb->cb)->type = 0;
#endif /* CONFIG_RTW_RX_BUF_SHARING */
...
#ifdef CONFIG_RTW_RX_BUF_SHARING
/*
 * Please make sure you have obtain spinlock hci_info.rx_buf_info_lock,
 * if using hci_info.rx_buf_info as argument.
 */
static inline void *_os_pkt_buf_alloc_rx_2(void *d, u32 buf_sz,
					   u32 *bus_addr_l_1,
					   u32 *bus_addr_h_1,
					   void **os_priv_1,
					   u32 *bus_addr_l_2,
					   ...
					   void **data_2)
{
	...
	struct sk_buff *skb_1 = NULL, *skb_2 = NULL;
	struct rx_buf_info *rxb_info;
	u32 rxbuf_size = buf_sz;
	u32 skb_buf_sz;
	...
	skb_1 = dev_alloc_skb_recy_wnic(rxbuf_size, alloc_data->pool_id);
	#else
	skb_1 = rtw_skb_alloc(rxbuf_size);
	#endif

	if (skb_1 == NULL)
		return NULL;

	skb_2 = rtw_skb_clone(skb_1);
	if (skb_2 == NULL) {
		rtw_skb_free(skb_1);
		return NULL;
	}

	skb_buf_sz = skb_end_offset(skb_1);
	rxbuf_size = skb_buf_sz / 2;

	/* Remove system reserved header
	|<- head .......................................... end ->|sh info|
	|<--------rxbuf_size-------->|<--------rxbuf_size-------->|
	|h rm|<- skb1->data          |h rm|<- skb2->data          |
	|h rm|<-rxb_info->rx_buf_sz->|h rm|<-rxb_info->rx_buf_sz->|
	|<------- rxbuf_size ------->|h rm|
	 */
	skb_1->data = skb_1->head + PHL_RX_HEADROOM;
	skb_reset_tail_pointer(skb_1);
	rxb_info = (struct rx_buf_info *)skb_1->cb;
	...
	*os_priv_1 = skb_1;
	...
	skb_2->data = skb_1->data + rxbuf_size;
	skb_reset_tail_pointer(skb_2);
	rxb_info = (struct rx_buf_info *)skb_2->cb;
	rxb_info->rx_buf_sz = rxbuf_size - PHL_RX_HEADROOM;
	rxb_info->type = 2;
	#ifdef CONFIG_PCI_HCI
...
#ifdef CONFIG_PCI_HCI
#ifdef CONFIG_RTW_RX_BUF_SHARING
	skb_buf_sz = ((struct rx_buf_info *)skb->cb)->rx_buf_sz;
#else
	skb_buf_sz =  skb_end_offset(skb) - PHL_RX_HEADROOM;
#endif /* CONFIG_RTW_RX_BUF_SHARING */
	#ifdef PHL_DMA_ADDR_64
...
static inline void *_os_mem_alloc(void *d, u32 buf_sz)
{
	...
	/* if buf < 4K, use kmalloc or use vmalloc */
	if (buf_sz < 4096)
		return _os_kmem_alloc(d, buf_sz);
	else {
		if (in_atomic()) {
...
#endif/* CONFIG_PHL_USE_KMEM_ALLOC_BY_PAGE_SIZE */
...
static __inline u8 _os_sched_setscheduler_rt(void *drv_priv, _os_thread *thread)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	sched_set_fifo_low(thread->thread_handler);
	return RTW_PHL_STATUS_SUCCESS;
#else
	struct sched_param param = { .sched_priority = 1 };
	return sched_setscheduler_nocheck(thread->thread_handler, SCHED_RR, &param);
#endif
}
...
static inline u32 _os_nvm_get_info(void *d, u32 info_type, void *value, u8 size)
{
	return rtw_nvm_get_info(d, info_type, value, size);
}
#endif /* CONFIG_PHL_DRV_HAS_NVM */

static inline void _os_kick_watchdog(void)
{
...
static inline void _os_reschedule(void)
{
	cond_resched();
}
