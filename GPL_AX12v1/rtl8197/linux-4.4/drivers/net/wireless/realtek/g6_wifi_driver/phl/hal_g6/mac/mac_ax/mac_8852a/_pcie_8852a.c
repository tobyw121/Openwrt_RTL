...
static struct mac_ax_pcie_ltr_param pcie_ltr_param_def_8852a = {
	0,
	0,
	MAC_AX_PCIE_ENABLE,
	MAC_AX_PCIE_ENABLE,
...
static struct mac_ax_pcie_cfgspc_param pcie_cfgspc_param_def_8852a = {
	0,
	0,
	MAC_AX_PCIE_DISABLE,
	MAC_AX_PCIE_ENABLE,
			    ...
			    enum pcie_bd_ctrl_type type)
{
	*reg = MAC_AX_R32_FF;
	...
	case MAC_AX_DMA_ACH4:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_AX_ACH4_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_AX_ACH4_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
	...
	case MAC_AX_DMA_ACH5:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_AX_ACH5_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_AX_ACH5_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
	...
	case MAC_AX_DMA_ACH6:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_AX_ACH6_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_AX_ACH6_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
	...
	case MAC_AX_DMA_ACH7:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_AX_ACH7_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_AX_ACH7_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
	...
	case MAC_AX_DMA_B1MG:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_AX_CH10_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_AX_CH10_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
	...
	case MAC_AX_DMA_B1HI:
		switch (type) {
		case PCIE_BD_CTRL_DESC_L:
			*reg = R_AX_CH11_TXBD_DESA_L;
			break;
		case PCIE_BD_CTRL_DESC_H:
			*reg = R_AX_CH11_TXBD_DESA_H;
			break;
		case PCIE_BD_CTRL_NUM:
			 ...
			 struct mac_ax_pcie_ltr_param *param)
{
	...
	struct mac_ax_pcie_ltr_param *param_def = &pcie_ltr_param_def_8852a;
	u32 val32;
			     ...
			     struct mac_ax_txdma_ch_map *ch_map)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_stop1 = R_AX_PCIE_DMA_STOP1;
	u32 reg_stop2 = R_AX_PCIE_DMA_STOP2;
	u32 val32;
				  ...
				  struct mac_ax_txdma_ch_map *ch_map)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_busy1 = R_AX_PCIE_DMA_BUSY1;
	u32 reg_busy2 = R_AX_PCIE_DMA_BUSY2;
...
u32 poll_dma_all_idle_pcie_8852a(struct mac_ax_adapter *adapter)
{
	...
	ret = poll_txdma_ch_idle_pcie_8852a(adapter, &txch_map);
	if (ret != MACSUCCESS) {
	...
	ret = poll_rxdma_ch_idle_pcie_8852a(adapter, &rxch_map);
	if (ret != MACSUCCESS) {
			  ...
			  struct mac_ax_rxdma_ch_map *rxch_map)
{
	...
	u32 reg_clr_tx2 = R_AX_TXBD_RWPTR_CLR2;
...
static u32 get_target_8852a(struct mac_ax_adapter *adapter, u16 *target,
				   ...
				   enum mac_ax_pcie_func_ctrl en)
{
	...
	ret = get_target_8852a(adapter, &tar, phy_rate);
	if (ret != MACSUCCESS) {
