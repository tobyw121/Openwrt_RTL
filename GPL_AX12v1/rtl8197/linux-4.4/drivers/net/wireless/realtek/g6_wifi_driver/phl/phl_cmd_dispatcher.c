...
#if 1//def RTW_WKARD_AP_CMD_DISPATCH
#define MAX_PHL_MSG_NUM (128)
#else
...
u8* phl_get_cmd_obj(u8 *phlcmdbuf);
void dump_dispr_msg_ex(struct phl_dispr_msg_ex *msg)
{
	struct cmd_obj *cmd = NULL;
	if(MSG_EVT_ID_FIELD(msg->msg.msg_id) == MSG_EVT_LINUX_CMD_WRK) {			
		cmd = (struct cmd_obj *)phl_get_cmd_obj(msg->msg.inbuf);
		if(cmd->cmdcode == CMD_SET_DRV_EXTRA) {
			struct drvextra_cmd_parm *extra_parm =
				(struct drvextra_cmd_parm *)cmd->parmbuf;
			PHL_PRINT("idx(%d): msg_id 0x%x cmdcode %d ec_id %d\n", msg->idx, msg->msg.msg_id, cmd->cmdcode, extra_parm->ec_id);
		} else
			PHL_PRINT("idx(%d): msg_id 0x%x cmdcode %d\n", msg->idx, msg->msg.msg_id, cmd->cmdcode);
	} else
		PHL_PRINT("idx(%d): msg_id 0x%x\n", msg->idx, msg->msg.msg_id);
}

void dump_msg_queue(struct cmd_dispatcher *obj, struct phl_queue *q)
{
	u8 i = 0;
	void *d = phl_to_drvpriv(obj->phl_info);	
	_os_list *new_msg = NULL;
	struct phl_dispr_msg_ex *msg;
	if (pq_get_front(d, q, &new_msg, _bh) == false)
		return;
	do {
		msg = (struct phl_dispr_msg_ex *)new_msg;
		dump_dispr_msg_ex(msg);
	} while(pq_get_next(d,q, new_msg, &new_msg, _bh));
}

void dump_msg_wait_queue(struct cmd_dispatcher *obj)
{
	PHL_PRINT("dump wait queue:\n");
	dump_msg_queue(obj,&(obj->msg_wait_q));
}

void dump_msg_pending_queue(struct cmd_dispatcher *obj)
{
	PHL_PRINT("dump pending queue:\n");
	dump_msg_queue(obj,&(obj->msg_pend_q));
}

void dump_msg_idle_queue(struct cmd_dispatcher *obj)
{
	PHL_PRINT("dump idle queue:\n");
	dump_msg_queue(obj,&(obj->msg_idle_q));
}
...
void msg_pre_phase_hdl(struct cmd_dispatcher *obj, struct phl_dispr_msg_ex *ex)
{
	...
	for (i = PHL_MDL_PRI_MAX - 1 ; i >= PHL_MDL_PRI_ROLE ; i--) {
		...
		if (STOP_DISPATCH_MSG(ret))
			return;
#endif
		if (priority == i && _chk_bitmap_bit(ex->premap, owner_id < PHL_BK_MDL_END ? owner_id : 0)) {
...
void msg_post_phase_hdl(struct cmd_dispatcher *obj, struct phl_dispr_msg_ex *ex)
{
	...
	for (i = PHL_MDL_PRI_ROLE ; i < PHL_MDL_PRI_MAX ; i++) {
		...
		if (STOP_DISPATCH_MSG(ret))
			return;
#endif
		if (priority == i && _chk_bitmap_bit(ex->postmap, owner_id < PHL_BK_MDL_END ? owner_id : 0)) {
			ret = feed_mdl_msg(obj, ex->module, ex);
			if (STOP_DISPATCH_MSG(ret))
				return;
		}
...
#endif
	ret = feed_mdl_msg(obj, &(obj->controller), ex);
#ifdef CONFIG_CMD_DISP_SOLO_MODE
...
/*
#ifdef	RTW_WKARD_AP_CMD_DISPATCH
	if((RTW_PHL_STATUS_RESOURCE == sts) && (rtw_dump_once == 0)) {
		rtw_dump_once=1;
		dump_msg_wait_queue(obj);
		dump_msg_pending_queue(obj);
	}
#endif
*/
	return sts;
...
u8 dispr_is_msg_active(void *dispr, u32 *msg_hdl)
{
	struct cmd_dispatcher *obj = (struct cmd_dispatcher *)dispr;
	struct phl_dispr_msg_ex *msg_ex = NULL;
	u8 ret = false;
