...
s8 rtw_hal_get_tx_pwr_by_txrate (void *hal, enum channel_width bw, u8 mcs_idx)
{
	return rtw_hal_bb_get_tx_pwr_by_txrate(hal,bw,mcs_idx);
}
...
void rtw_hal_debug_tx_report(void *hal, u32 rpt_cnt)
{
	struct hal_info_t *hal_info = hal;

	if (hal_info->hal_ops.dbg_tx_rpt == NULL)
		return;

	hal_info->hal_ops.dbg_tx_rpt(hal_info->hal_com, rpt_cnt);
}
...
rtw_hal_debug_bcn_tx(void *hal, u32 fail_cnt)
{
	...
	PHL_PRINT("Set beacon report print threshold to %u\n", fail_cnt);
	hal_info->hal_com->dbg_bcn_tx = fail_cnt;
}
#endif /* CONFIG_RTW_DEBUG_BCN_TXCONFIG_RTW_DEBUG_BCN_TX */
enum rtw_hal_status
...
bool rtw_hal_check_ch_rfk(void *hal, struct rtw_chan_def *chandef)
{
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_, "rtw_hal_check_ch_rfk(): check status(%d), chan(%d)\n",
		check, chandef->chan);
	return check;
}
...
{
	struct hal_info_t *hal_info = hal;
	return rtw_hal_mac_stop_h2c_agg(hal, tx);
}
#endif
...
{
	return rtw_hal_rf_get_thermal(hal, rfpath, thermal);
}
