		...
		struct rtw_para_pwrlmt_info_t *para_info)
{
	...
	u8 ntx_idx = 0 , bf = 0 , regulation = 0, pshape_idx = 0;
	s8 powerlimit = 0;
	...
	if (_os_strncmp((char *)sregulation, (const char *)"FCC", 3) == 0)
		regulation = _PW_LMT_REGU_FCC;
	else if (_os_strncmp((char *)sregulation, (const char *)"ETSI", 4) == 0)
		regulation = _PW_LMT_REGU_ETSI;
	else if (_os_strncmp((char *)sregulation, (const char *)"MKK", 3) == 0)
		regulation = _PW_LMT_REGU_MKK;
	else if (_os_strncmp((char *)sregulation, (const char *)"IC", 2) == 0)
		regulation = _PW_LMT_REGU_IC;
	else if (_os_strncmp((char *)sregulation, (const char *)"KCC", 3) == 0)
		regulation = _PW_LMT_REGU_KCC;
	else if (_os_strncmp((char *)sregulation, (const char *)"ACMA", 4) == 0)
		regulation = _PW_LMT_REGU_ACMA;
	else if (_os_strncmp((char *)sregulation, (const char *)"CHILE", 5) == 0)
		regulation = _PW_LMT_REGU_CHILE;
	else if (_os_strncmp((char *)sregulation, (const char *)"UKRAINE", 7) == 0)
		regulation = _PW_LMT_REGU_UKRAINE;
	else {
	...
	array_tc_8852a_txpwr_lmt->reg = regulation;
...
{
	u8 band = 0, bandwidth = 0, ratesec = 0, channel = 0;
	u8 ntx_idx = 0 , regulation = 0, pshape_idx = 0;
	s8 powerlimit = 0;
	...
	array_tc_8852a_txpwr_lmt_ru->reg = regulation;
...
_hal_parse_txpwrtrack(void *drv_priv, void *para_info_t, u8 *psrc_buf, u32 buflen)
{
	...
	return (sizeof(struct hal_txpwr_track_t) + sizeof(u32) - 1) / sizeof(u32);
}
	...
	const char *file_name)
{
	...
	if (para_buf == NULL) {
		PHL_TRACE(COMP_PHL_DBG, _PHL_DEBUG_, "%s.para_buf=NULL \n", file_name);
		para_info->para_src = RTW_PARA_SRC_INTNAL;
		goto free_exit;
	}
	...
	const u8 *hal_phy_folder;
	u8 rfe_type[7]; //example RFE50a, 'a' is subtype
	u8 get_rfe;

#ifdef CONFIG_PHL_DRV_HAS_NVM
	hal_phy_folder = rtw_nvm_get_para_path(phl_com->drv_priv);
#else
	hal_phy_folder = rtw_phl_flash_get_para_path(phl_com->phl_priv);
#endif

	get_rfe = rtw_phl_get_rfe_type(phl_com->phl_priv);

	rtw_gen_sub_dir_name(phl_com->phl_priv, get_rfe, rfe_type);

	_os_snprintf(para_info->para_path, MAX_PATH_LEN, "%s%s%s%s%s%s",
		     hal_phy_folder ,ic_name, _os_path_sep, rfe_type, _os_path_sep, file_name);
	...
	PHL_TRACE(COMP_PHL_DBG, _PHL_DEBUG_, "%s:: %s\n",__FUNCTION__,
		  para_info->para_path);
	...
	if (para_size != 0) {
		...
		para_info->para_data_len = parser_fun(drv, para_info, para_buf, para_size);

		if (para_info->para_data_len) {
			u32 data_len_b = para_info->para_data_len * sizeof(u32);

			_os_mem_free(drv, para_buf, MAX_HWCONFIG_FILE_CONTENT);
			para_buf = NULL;

			/*if (para_data != NULL) {
				_os_mem_cpy(drv, para_data, para_info->para_data,
				            data_len_b);

				_os_mem_free(drv, para_info->para_data, MAX_HWCONFIG_FILE_CONTENT * sizeof(u32));
				para_info->para_data = NULL;
				para_info->para_data = para_data;
		...
		if (para_info->para_data_len) {
			PHL_TRACE(COMP_PHL_DBG, _PHL_ALWAYS_,
				"%s:: Download file \"%s\" (%u/%u) OK.\n",
				__FUNCTION__, para_info->para_path,
				para_info->para_data_len, para_size);
		} else {
...
free_exit:
...
#else /* CONFIG_DYNAMIC_PHY_PARA_MEM */
	u8 *hal_phy_folder;
	u8 rfe_type[7];
	u8 get_rfe;
	...
	if (para_info->para_src == RTW_PARA_SRC_EXTNAL) {
		...
		if (para_size != 0) {
			/* Parsing file content */
			para_info->para_data_len = parser_fun(drv,
							para_info, para_buf, para_size);
...
#endif /* CONFIG_DYNAMIC_PHY_PARA_MEM */
#endif /* CONFIG_LOAD_PHY_PARA_FROM_FILE */
}

#ifdef CONFIG_RTW_ENABLE_SUB_TYPE

void _hal_dl_ext_para_file(struct rtw_phl_com_t *phl_com,
	void *para_info_t, char *ic_name,
	int (*parser_fun)(void *drv_priv, void *para_info_t, u8 *psrc_buf, u32 buflen),
	const char *file_name)
{
...
#ifdef CONFIG_PHL_DRV_HAS_NVM
		hal_phy_folder = rtw_nvm_get_para_path(phl_com->drv_priv);
		rtw_get_fem_name(drv, sub_type, sizeof(sub_type) - 1);
#else
		hal_phy_folder = rtw_phl_flash_get_para_path(phl_com->phl_priv);
		rtw_gen_fem_name(phl_com->phl_priv, sub_type);
#endif

		get_rfe = rtw_phl_get_rfe_type(phl_com->phl_priv);
		rtw_gen_sub_dir_name(phl_com->phl_priv, get_rfe, rfe_type);

		_os_snprintf(para_info->para_path, MAX_PATH_LEN, "%s%s%s%s%s%s%s%s",
			hal_phy_folder ,ic_name, _os_path_sep, rfe_type, _os_path_sep, sub_type, _os_path_sep, file_name);
#else
...
#endif /*CONFIG_RTW_ENABLE_SUB_TYPE*/
				...
				struct rtw_para_pwrlmt_info_t *para_info)
{
	...
	if (para_info->para_src == RTW_PARA_SRC_EXTNAL) {
		...
		for (i = 0; i <= para_regd_str_arridx ; i++) {
			_os_strncpy(para_info->ext_regd_name[i] , _para_regd_str[i], sizeof(para_info->ext_regd_name[i]));
...
void
rtw_hal_dl_all_ext_para_file(struct rtw_phl_com_t *phl_com,
							char *ic_name, void *hal)
{
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE

	struct phy_sw_cap_t *phy_sw_cap = NULL;
	u8 idx = 0, max_phy_num = 1;

#ifdef CONFIG_RTW_ENABLE_SUB_TYPE

	/*Download extension para file in sub dir*/
	for (idx=0; idx < max_phy_num; idx++) {

		phy_sw_cap = &phl_com->phy_sw_cap[idx];

		_hal_dl_ext_para_file(phl_com, &phy_sw_cap->bb_phy_reg_info,
						  ic_name, _hal_parse_phyreg, "PHY_REG.txt");

		_hal_dl_ext_para_file(phl_com, &phy_sw_cap->bb_phy_reg_gain_info,
						  ic_name, _hal_parse_phyreg, "PHY_REG_GAIN.txt");

		_hal_dl_ext_para_file(phl_com, &phy_sw_cap->rf_radio_a_info,
						  ic_name, _hal_parse_radio, "RadioA.txt");

		_hal_dl_ext_para_file(phl_com, &phy_sw_cap->rf_radio_b_info,
						  ic_name, _hal_parse_radio, "RadioB.txt");

		_hal_dl_ext_para_file(phl_com, &phy_sw_cap->rf_txpwr_byrate_info,
		                  ic_name, _hal_parse_txpwr_by_rate, "TXPWR_ByRate.txt");

		_hal_dl_ext_para_file(phl_com, &phy_sw_cap->rf_txpwrtrack_info,
		                  ic_name, _hal_parse_txpwrtrack, "TXPWR_TrackTSSI.txt");

		_hal_dl_ext_para_file(phl_com, &phy_sw_cap->rf_txpwrlmt_info,
		                  ic_name, _hal_parse_txpwrlmt, "TXPWR_LMT.txt");

		_hal_dl_ext_para_file(phl_com, &phy_sw_cap->rf_txpwrlmt_ru_info,
		                  ic_name, _hal_parse_txpwrlmt_ru, "TXPWR_LMT_RU.txt");
	}
#endif /*CONFIG_RTW_ENABLE_SUB_TYPE*/
