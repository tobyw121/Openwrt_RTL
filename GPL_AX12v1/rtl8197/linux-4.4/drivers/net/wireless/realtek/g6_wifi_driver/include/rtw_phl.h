...
#define GET_HAL_DATA(_dvobj)	(_dvobj->phl_com)
#define GET_HAL_SPEC(_dvobj)	(&(GET_HAL_DATA(_dvobj)->hal_spec))

enum rtw_phl_rx_mode {
	PHL_RX_MODE_NORMAL,
	PHL_RX_MODE_SNIFFER,
};

/* refer to (registrypriv-> tx_nss,rx_nss / hal_spec->tx_nss_num,rx_nss_num)*/
#define GET_HAL_TX_NSS(_dvobj) ((GET_HAL_DATA(_dvobj))->phy_cap[0].txss)
#define GET_HAL_RX_NSS(_dvobj) ((GET_HAL_DATA(_dvobj))->phy_cap[0].rxss)
#define GET_HAL_RFPATH_NUM(_dvobj) ((GET_HAL_DATA(_dvobj))->rf_path_num)
/* refer to (hal_data->version_id.RFType / registrypriv->rf_path / 8814a from efuse or registrypriv)*/
#define GET_HAL_RFPATH(_dvobj) ((GET_HAL_DATA(_dvobj))->rf_type)

#if 0 /*CONFIG_CORE_XMITBUF*/
#define GET_HAL_XMITBUF_NR(_dvobj) ((GET_HAL_SPEC(_dvobj))->max_xmitbuf_nr)
#define GET_HAL_XMITBUF_SZ(_dvobj) ((GET_HAL_SPEC(_dvobj))->max_xmitbuf_sz)

#define GET_HAL_XMITBUF_EXT_NR(_dvobj) ((GET_HAL_SPEC(_dvobj))->max_xmit_extbuf_nr)
#define GET_HAL_XMITBUF_EXT_SZ(_dvobj) ((GET_HAL_SPEC(_dvobj))->max_xmit_extbuf_sz)
#endif


#define GET_HAL_RECVBUF_NR(_dvobj) ((GET_HAL_SPEC(_dvobj))->max_recvbuf_nr)
#ifdef CONFIG_PHL_015_devp_tmp
#define GET_HAL_RECVBUF_SZ(_dvobj) ((GET_HAL_SPEC(_dvobj))->max_recvbuf_sz)
#endif
...
int rtw_hw_do_rx_dck(struct _ADAPTER *a);
...
#ifdef CONFIG_DFS
bool rtw_dfs_hal_radar_detect_disable(_adapter *adapter);
bool rtw_dfs_hal_radar_detect_enable(_adapter *adapter);
BOOLEAN rtw_dfs_hal_radar_detect(_adapter *adapter);
...
void rtw_led_register_event(_adapter *a, enum rtw_led_event event, enum rtw_led_state state, struct rtw_led_action_args_t *action_args, u32 interval);
