...
void _halrf_psd_backup_bb_registers_8832br(
	struct rf_info *rf,
...
void _halrf_psd_reload_bb_registers_8832br(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	u32 *reg,
	u32 *reg_backup,
...
void _halrf_psd_bkup_rf_8832br(
	struct rf_info *rf,
	u32 *rf_reg,
	u8 path)
{
	...
	for (i = 0; i < PSD_RF_REG_NUM_8832BR; i++) {
...
void _halrf_psd_reload_rf_8832br(
	struct rf_info *rf,
	u32 *rf_reg,
	u8 path)
{
...
void _halrf_psd_set_dac_off_adc_8832br(struct rf_info *rf, enum phl_phy_idx phy)
...
void halrf_psd_init_8832br(struct rf_info *rf, enum phl_phy_idx phy,
			u8 path, u8 iq_path, u32 avg, u32 fft)
{
	struct halrf_psd_data *psd_info = &rf->psd;
	u32 reg_tmp;
	u32 bb_reg[PSD_BACKUP_NUM_8832BR] = {
	...
	u32 rf_reg[PSD_RF_REG_NUM_8832BR] = {0x0, 0x5, 0x8f, 0x10000, 0x10005};

	RF_DBG(rf, DBG_RF_PSD, "======> %s   phy=%d   path=%d   iq_path=%d   avg=%d   fft=%d\n",
		__func__, phy,  path, iq_path, avg, fft);

	if (psd_info->psd_progress == 0) {
		_halrf_psd_backup_bb_registers_8832br(rf, phy, bb_reg,
				psd_info->psd_reg_backup, PSD_BACKUP_NUM_8832BR);

		_halrf_psd_bkup_rf_8832br(rf, rf_reg, RF_PATH_A);
		_halrf_psd_bkup_rf_8832br(rf, rf_reg, RF_PATH_B);
	}
	...
	_halrf_psd_set_dac_off_adc_8832br(rf, phy);
...
void halrf_psd_restore_8832br(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct halrf_psd_data *psd_info = &rf->psd;
	...
	if (psd_info->psd_progress == 1) {
		_halrf_psd_reload_bb_registers_8832br(rf, phy, bb_reg,
			psd_info->psd_reg_backup, PSD_BACKUP_NUM_8832BR);
		_halrf_psd_reload_rf_8832br(rf, rf_reg, RF_PATH_A);
		_halrf_psd_reload_rf_8832br(rf, rf_reg, RF_PATH_B);
	}

	psd_info->psd_progress = 0;
}

u32 halrf_psd_get_point_data_8832br(struct rf_info *rf,
...
void halrf_psd_query_8832br(struct rf_info *rf, enum phl_phy_idx phy,
			u32 point, u32 start_point, u32 stop_point, u32 *outbuf)
{
	...
	while (i < stop_point) {
		...
		psd_info->psd_data[j] = halrf_psd_get_point_data_8832br(rf, phy, point_temp);
