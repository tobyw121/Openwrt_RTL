...
u32 mac_cmd_ofld_v1(struct mac_ax_adapter *adapter)
{
	struct mac_ax_cmd_ofld_info *ofld_info = &adapter->cmd_ofld_info;
	struct mac_ax_state_mach *sm = &adapter->sm;
	u32 ret = MACSUCCESS;
			...
			u8 send_h2c, u8 clear_after_send)
{
	...
	scanofld_info = &adapter->scanofld_info;
	PLTFM_MSG_TRACE("[scnofld][add] ==> drv_chlist_busy=%d, fw_chlist_busy=%d\n",
			scanofld_info->drv_chlist_busy, scanofld_info->fw_chlist_busy);
	PLTFM_MUTEX_LOCK(&scanofld_info->drv_chlist_state_lock);
	if (scanofld_info->drv_chlist_busy) {
		PLTFM_MSG_TRACE("[scnofld][add] Halmac scan list busy, abort adding.\n");
		PLTFM_MUTEX_UNLOCK(&scanofld_info->drv_chlist_state_lock);
		return MACPROCBUSY;
	}
	...
	if (chinfo->dfs_ch && chinfo->period < MAC_AX_BCN_INTERVAL_DEFAULT) {
		PLTFM_MSG_WARN("[scnofld][add] Period (%d) might be too short for a DFS ch\n",
			       chinfo->period);
	}
	...
	if (scanofld_info->fw_chlist_busy) {
		PLTFM_MSG_TRACE("[scnofld][add] FW scan list busy, abort sending.\n");
		PLTFM_MUTEX_LOCK(&scanofld_info->drv_chlist_state_lock);
		scanofld_info->drv_chlist_busy = 0;
		PLTFM_MUTEX_UNLOCK(&scanofld_info->drv_chlist_state_lock);
		PLTFM_MUTEX_UNLOCK(&scanofld_info->fw_chlist_state_lock);
...
u32 mac_scanofld(struct mac_ax_adapter *adapter, struct mac_ax_scanofld_param *scanParam)
{
	...
	if ((scanParam->target_ch_ch_band >= CHSW_CHBAND_MAX ||
		...
		PLTFM_MSG_ERR("[scnofld][cfg] TGT cent (%d) pri (%d) band (%d) bad val\n",
			      scanParam->target_central_ch, scanParam->target_pri_ch,
			      scanParam->target_ch_ch_band);
		return MACFUNCINPUT;
	}

	PLTFM_MSG_TRACE("[scnofld][cfg] op=%d (%d), fw_scan_busy=%d, fw_chlist_busy=%d\n",
			scanParam->operation, !!(scanParam->operation),
			scanofld_info->fw_scan_busy, scanofld_info->fw_chlist_busy);

	PLTFM_MUTEX_LOCK(&scanofld_info->fw_chlist_state_lock);
	scanofld_info->last_fw_chlist_busy = scanofld_info->fw_chlist_busy;
	PLTFM_MUTEX_UNLOCK(&scanofld_info->fw_chlist_state_lock);
	scanofld_info->last_fw_scan_busy = scanofld_info->fw_scan_busy;

	if (scanParam->operation != MAC_AX_SCAN_OP_GETRPT) {
		if (!!(scanParam->operation) && scanofld_info->fw_scan_busy) {
			PLTFM_MSG_TRACE("[scnofld][cfg] Cant start scanning while scanning\n");
			return MACPROCBUSY;
		}
		PLTFM_MUTEX_LOCK(&scanofld_info->fw_chlist_state_lock);
		if (!!(scanParam->operation) && scanofld_info->fw_chlist_busy) {
			PLTFM_MSG_TRACE("[scnofld][cfg] Cant start scanning when fw chlist busy\n");
			PLTFM_MUTEX_UNLOCK(&scanofld_info->fw_chlist_state_lock);
			return MACPROCBUSY;
		}
		...
		PLTFM_MSG_TRACE("[scnofld][cfg] fw_chlist_busy = %d, fw_scan_busy=%d",
				scanofld_info->fw_chlist_busy, scanofld_info->fw_scan_busy);
		PLTFM_MUTEX_UNLOCK(&scanofld_info->fw_chlist_state_lock);
	} else if (!scanofld_info->fw_scan_busy) {
		PLTFM_MSG_TRACE("[scnofld][cfg] Cant get rpt when fw is not scanning\n");
		return MACPROCERR;
	}

	PLTFM_MSG_TRACE("[scnofld][cfg] macid (%d) port (%d) phy_band (%d)\n",
			scanParam->macid, scanParam->port_id, scanParam->band);
	PLTFM_MSG_TRACE("[scnofld][cfg] op (%d) target_mode (%d) start_mode (%d) scan_type (%d)\n",
			scanParam->operation, scanParam->target_ch_mode,
			scanParam->start_mode, scanParam->scan_type);
	if (scanParam->target_ch_mode) {
		PLTFM_MSG_TRACE("[scnofld][cfg][tgt] bw (%d) pri (%d) cent (%d) band (%d)\n",
				scanParam->target_ch_bw, scanParam->target_pri_ch,
				scanParam->target_central_ch, scanParam->target_ch_ch_band);
		PLTFM_MSG_TRACE("[scnofld][cfg][tgt] #pkts (%d) [%d, %d, %d, %d]\n",
	...
	PLTFM_MSG_TRACE("[scnofld][cfg] norm_pd (%d) norm_cy (%d) slow_pd (%d)\n",
