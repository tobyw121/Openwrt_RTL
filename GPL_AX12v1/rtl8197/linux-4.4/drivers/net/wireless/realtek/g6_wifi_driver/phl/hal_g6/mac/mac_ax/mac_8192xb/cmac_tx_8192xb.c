...
u32 macid_idle_ck_8192xb(struct mac_ax_adapter *adapter,
			 struct mac_role_tbl *role)
{
	...
	while (--cnt) {
		...
		if ((val8 & B_AX_PTCL_TX_ON_STAT) && (val8 & B_AX_PTCL_DROP)) {
			...
			PLTFM_DELAY_US(SW_CVR_DUR_US);
			val32 = MAC_REG_R32(R_AX_PTCL_TX_MACID_0_V1);
			val32_1 = MAC_REG_R32(R_AX_PTCL_TX_MACID_1);
...
u32 stop_sch_tx_8192xb(struct mac_ax_adapter *adapter, enum sch_tx_sel sel,
