...
void halrf_afe_init_8832crvu(struct rf_info *rf)
{
#if 0
...
void halrf_dack_init_8832crvu(struct rf_info *rf)
{

}

void halrf_dack_reset_8832crvu(struct rf_info *rf, enum rf_path path)
{
	if (path == RF_PATH_A) {
...
void halrf_drck_8832crvu(struct rf_info *rf)
{
	u32 c;
	u32 rck_d;
...
void halrf_addck_backup_8832crvu(struct rf_info *rf)
{
...
void halrf_addck_reload_8832crvu(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	/*S0*/
...
void halrf_dack_reload_by_path_8832crvu(struct rf_info *rf, u8 path, u8 index)
{
	struct halrf_dack_info *dack = &rf->dack;
...
void halrf_dack_reload_8832crvu(struct rf_info *rf, enum rf_path path)
{
	u8 i;

	for (i = 0; i < 2; i++)
		halrf_dack_reload_by_path_8832crvu(rf, path, i);
	/*DACK result  1 : from reg_table ; 0 : from circuit Calibration*/
}
void halrf_dack_backup_s0_8832crvu(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
...
void halrf_dack_backup_s1_8832crvu(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
...
void halrf_check_addc_8832crvu(struct rf_info *rf, enum rf_path path)
{
...
void halrf_addck_8832crvu(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c;
	/*S0*/
...
void halrf_check_dadc_8832crvu(struct rf_info *rf, enum rf_path path)
{
	...
	halrf_check_addc_8832crvu(rf, path);
	if (path == RF_PATH_A) {
...
void halrf_dack_8832crvu_s0(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c = 0;

	/*step 1: Set clk to 160MHz for calibration  */
	halrf_txck_force_8832crvu(rf, RF_PATH_A, true, DAC_160M);
	...
	halrf_dack_reset_8832crvu(rf, RF_PATH_A);
	...
	halrf_txck_force_8832crvu(rf, RF_PATH_A, false, DAC_960M);
	RF_DBG(rf, DBG_RF_DACK, "[DACK]after S0 DADCK\n");
	/*halrf_check_dadc_8832crvu(rf, RF_PATH_A);*/

	/*backup here*/
	halrf_dack_backup_s0_8832crvu(rf);
	halrf_dack_reload_8832crvu(rf, RF_PATH_A);
	/*step 6: Set DAC & clk to normal mode */
	halrf_wreg(rf, 0x12b8, BIT(30), 0x0);
}

void halrf_dack_8832crvu_s1(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c = 0;

	/*step 1: Set clk to 160MHz for calibration  */
	halrf_txck_force_8832crvu(rf, RF_PATH_B, true, DAC_160M);
	/*step 2: DAC & clk enable */
	halrf_wreg(rf, 0x32b8, BIT(30), 0x1);
	halrf_wreg(rf, 0x030c, BIT(28), 0x1);
	halrf_wreg(rf, 0x032c, 0x80000000, 0x0);
	...
	halrf_dack_reset_8832crvu(rf, RF_PATH_B);
	...
	/*step 5: Set clk to 960MHz for normal mode */
	halrf_txck_force_8832crvu(rf, RF_PATH_B, false, DAC_960M);

	RF_DBG(rf, DBG_RF_DACK, "[DACK]after S0 DADCK\n");
	/*halrf_check_dadc_8832crvu(rf, RF_PATH_A);*/

	/*backup here*/
	halrf_dack_backup_s1_8832crvu(rf);
	halrf_dack_reload_8832crvu(rf, RF_PATH_B);
	/*step 6: Set DAC & clk to normal mode */
	halrf_wreg(rf, 0x32b8, BIT(30), 0x0);
}

void halrf_dack_8832crvu(struct rf_info *rf)
{
	halrf_dack_8832crvu_s0(rf);
	halrf_dack_8832crvu_s1(rf);
}

void halrf_dack_dump_8832crvu(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
	u8 t;
...
void halrf_dac_cal_8832crvu(struct rf_info *rf, bool force)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 rf0_0, rf1_0;
	u8 phy_map;
	...
	halrf_afe_init_8832crvu(rf);
	halrf_drck_8832crvu(rf);
#if 1
	...
	halrf_addck_8832crvu(rf);
	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_STOP);
	halrf_addck_backup_8832crvu(rf);
	halrf_addck_reload_8832crvu(rf);
	halrf_wrf(rf, RF_PATH_A, 0x1, MASKRF, 0x0);
	halrf_wrf(rf, RF_PATH_B, 0x1, MASKRF, 0x0);
	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_START);
	halrf_dack_8832crvu(rf);
	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_STOP);
	halrf_dack_dump_8832crvu(rf);
	dack->dack_done = true;
...
bool halrf_dack_reg_check_fail_8832crvu(struct rf_info *rf)
{
#if 1
	struct halrf_dack_info *dack = &rf->dack;
	u16 addck_d[2][2]; 
