...
void LPS_Enter(_adapter *padapter, const char *msg)
{
	...
	if (GET_HAL_DATA(dvobj)->fw_ready == _FALSE)
		return;
#endif
...
static void _rtw_ssmps(_adapter *adapter, struct sta_info *sta)
{
	...
	rtw_phl_cmd_change_stainfo(adapter->dvobj->phl, sta->phl_sta,
			STA_CHG_NSS, NULL, 0, PHL_CMD_DIRECTLY, 0);
}
void rtw_ssmps_enter_static(_adapter *adapter, struct sta_info *sta)
{
	if (sta->phl_sta == NULL || sta->phl_sta->asoc_cap.sm_ps == SM_PS_STATIC)
		return;

	RTW_INFO("[rtw_ssmps_enter_static]"ADPT_FMT" STA [" MAC_FMT "]\n",
...
void rtw_ssmps_enter_dynamic(_adapter *adapter, struct sta_info *sta)
{
	if (sta->phl_sta == NULL || sta->phl_sta->asoc_cap.sm_ps == SM_PS_DYNAMIC)
		return;

	RTW_INFO("[rtw_ssmps_enter_dynamic]"ADPT_FMT" STA [" MAC_FMT "]\n", ADPT_ARG(adapter), MAC_ARG(sta->phl_sta->mac_addr));
	sta->phl_sta->asoc_cap.sm_ps = SM_PS_DYNAMIC;
	/* restore nss_rx to nss_rx_assoc parsed by assoc req */
	sta->phl_sta->asoc_cap.nss_rx = sta->nss_rx_assoc;
	_rtw_ssmps(adapter, sta);
}
void rtw_ssmps_leave(_adapter *adapter, struct sta_info *sta)
{
	if (sta->phl_sta == NULL || sta->phl_sta->asoc_cap.sm_ps == SM_PS_DISABLE)
		return;

	RTW_INFO("[rtw_ssmps_leave]"ADPT_FMT" STA [" MAC_FMT "] \n", ADPT_ARG(adapter), MAC_ARG(sta->phl_sta->mac_addr));
