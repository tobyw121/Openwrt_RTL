...
bool rtw_halbb_raregistered(struct bb_info *bb, struct rtw_phl_stainfo_t *phl_sta_i)
{
	...
	BB_DBG(bb, DBG_RA, "ra mask from phl = %llx\n", mod_mask);
	...
	ra_cfg->band = phl_sta_i->chandef.band;
	...
	BB_DBG(bb, DBG_RA, "RA Register=>In: Dis_ra=%x, MD=%x, BW=%x, macid=%x, band=%d\n",
		   hal_sta_i->ra_info.dis_ra, mode, phl_sta_i->chandef.bw, macid, phl_sta_i->chandef.band);
...
u32 halbb_get_fw_ra_rpt(struct bb_info *bb, u16 len, u8 *c2h)
{
	...
	if (ra_rpt_i->is_mu) {
		...
		if (hal_sta_i->ra_info.mu_active[ra_rpt_i->u1_muidx]) {
			...
			BB_DBG(bb, DBG_RA,
			       "MU RA RPT: su_macid = %d, muidx{u0,u1} = {%d,%d}, mu_mode = %d, mu_giltf = %x, mu_mcs_nss = %x\n",
		...
		BB_DBG(bb, DBG_RA,
		       "SU RA RPT: macid = %d, mode = %d, giltf = %x, mcs_nss = %x\n",
...
u32 halbb_get_txsts_rpt(struct bb_info *bb, u16 len, u8 *c2h)
{
	...
	for (j = 0; j <= 1; j++) {
		c2h += j << 5; /*txsts_info is 32 bytes*/
		...
		if (phl_sta_i == NULL) {
			BB_DBG(bb, DBG_RA,
			       "phl_sta == NULL, Wrong C2H RA macid = %d!!\n",
			       macid_rpt);
			null_cnt++;
			continue;
		}
		...
		if (hal_sta_i == NULL) {
			BB_DBG(bb, DBG_RA,
			       "hal_sta == NULL, Wrong C2H RA macid = %d!!\n",
			       macid_rpt);
			null_cnt++;
			continue;
		}
...
{
	struct bb_h2c_lps_info *lps_info;
	u32 *bb_h2c;
	u8 cmdlen = sizeof(struct bb_h2c_lps_info);
	u16 lps_info_len = 0;
	...
	BB_DBG(bb, DBG_RA, "LPS info=>h2c start\n");
	...
	BB_DBG(bb, DBG_RA, "LPS info=>h2c[0]: %x, h2c_done: %d\n", bb_h2c[0], h2c_done);

	// Return h2c_done flag to default
	halbb_set_reg(bb, 0x1e0, BIT(0), 0);

	BB_DBG(bb, DBG_RA, "LPS info=>h2c[0]: %x\n", bb_h2c[0]);
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], help) == 0) {
		...
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "[is_mu/ss_mcs]: (Bitmap format) [6]: is_mu [5:4]: Nsts [3:0]: MCS\n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "==============================>\n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
	...
	if (0) { /* Test RA mode */
		...
		*/
		...
		tmp1 = halbb_get_reg(bb, 0x168, MASKBYTE3);
		tmp2 = halbb_get_reg(bb, 0x174, MASKBYTE0);
		tmp3 = halbb_get_reg(bb, 0x174, MASKBYTE1);
		if ((bb->ic_type == BB_RTL8852A) || (bb->ic_type == BB_RTL8852B)) {
			BB_DBG_CNSL(out_len, used, output + used, out_len - used,
				    "{PER,RDR,R4}={%d,%d,%d}\n", tmp1, tmp2, tmp3);
		} else {
			tmp4 = halbb_get_reg(bb, 0x2444, MASKBYTE0);
			BB_DBG_CNSL(out_len, used, output + used, out_len - used,
				    "{PER,RDR,R4,cls}={%d,%d,%d,%d}\n", tmp1, tmp2, tmp3, tmp4);
		}
		tmp1 = halbb_get_reg(bb, 0x168, MASKBYTE0);
		tmp2 = halbb_get_reg(bb, 0x16c, MASKLWORD);
		tmp3 = halbb_get_reg(bb, 0x16c, MASKHWORD);
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			    "{rate_up_lmt_cnt,PER_ma,VAR}={%d,%d,%d}\n", tmp1, tmp2, tmp3);
		tmp1 = halbb_get_reg(bb, 0x170, MASKLWORD);
		tmp2 = halbb_get_reg(bb, 0x170, MASKHWORD);
		tmp3 = halbb_get_reg(bb, 0x168, MASKBYTE1);
		tmp4 = halbb_get_reg(bb, 0x168, MASKBYTE2);
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			    "decision_offset:{n,p}={%d,%d},TH:{RD,RU}={%d,%d}\n",
			    tmp1, tmp2, tmp3, tmp4);
		tmp1 = halbb_get_reg(bb, 0x178, MASKBYTE2);
		tmp2 = halbb_get_reg(bb, 0x184, MASKBYTE3);
		tmp3 = halbb_get_reg(bb, 0x188, MASKBYTE0);
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			    "try:{PER,RDR,R4}={%d,%d,%d}\n", tmp1, tmp2, tmp3);
		tmp1 = halbb_get_reg(bb, 0x178, MASKBYTE3);
		tmp2 = halbb_get_reg(bb, 0x178, MASKBYTE1);
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			    "TxRPT.total_tmp=%d,RA_timer=%d\n", tmp1, tmp2);
		tmp1 = halbb_get_reg(bb, 0x174, MASKBYTE2);
		tmp2 = halbb_get_reg(bb, 0x174, MASKBYTE3);
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			    "rate decision early return:{total=0/disra/trying, R4}={%d,%d}\n",
			    tmp1, tmp2);
		tmp1 = halbb_get_reg(bb, 0x180, MASKDWORD);
		tmp2 = halbb_get_reg(bb, 0x17c, MASKDWORD);
		tmp3 = halbb_get_reg(bb, 0x184, MASKBYTE0);
		tmp4 = halbb_get_reg(bb, 0x184, MASKBYTE1);
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
		...
		if ((dev->rfe_type >= 50) && (bb->ic_type != BB_RTL8852A)) {
			tmp1 = halbb_get_reg(bb, 0x1a0, MASKDWORD);
			tmp2 = halbb_get_reg(bb, 0x1a4, MASKDWORD);
			tmp3 = halbb_get_reg(bb, 0x1a8, MASKDWORD);
			tmp4 = halbb_get_reg(bb, 0x1ac, MASKDWORD);
			BB_DBG_CNSL(out_len, used, output + used,
				    out_len - used,
				    "==== MU MIMO RA ====\n");
			BB_DBG_CNSL(out_len, used, output + used,
				    ...
				    tmp1 & MASKBYTE0, (tmp1 & MASKBYTE1) >> 8,
				    tmp2 & MASKBYTE0, (tmp2 & MASKBYTE1) >> 8);
			BB_DBG_CNSL(out_len, used, output + used,
				    out_len - used,
				    "mu_rd_th=%d, mu_ru_th=%d, mu_per=%d, mu_try_per=%d\n",
				    (tmp3 & MASKBYTE1) >> 8, tmp3 & MASKBYTE0,
				    tmp4 & MASKBYTE0, (tmp4 & MASKBYTE1) >> 8);
			BB_DBG_CNSL(out_len, used, output + used,
				    out_len - used,
