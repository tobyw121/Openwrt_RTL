				  ...
				  u8 path)
{
	...
	for (gp = 0; gp < 0x4; gp++) {
		...
		halrf_wreg(rf, 0x8024, 0x000fffff, rf0 |BIT(rf->hal_com->dbcc_en));
			    ...
			    u8 path)
{
	...
	halrf_wreg(rf, 0x8034, 0x00000030, 0x3);
	rf0 = halrf_rrf(rf, path, 0x0, MASKRF);
	halrf_wreg(rf, 0x8020, 0x000fffff, rf0 |BIT(rf->hal_com->dbcc_en));
	halrf_wreg(rf, 0x8124 + (path << 8), 0x00000f00, 0x0);
...
static void _iqk_txk_setting_8852ab(struct rf_info *rf, u8 path)
{
	...
	case BAND_ON_24G:
		...
		halrf_wrf(rf, path, 0x0, MASKRF, 0x403e1 |BIT(rf->hal_com->dbcc_en));
		halrf_delay_us(rf, 1);
		break;
	case BAND_ON_5G:
		//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
...
void iqk_get_ch_info_8852ab(struct rf_info *rf, enum phl_phy_idx phy, u8 path)
{
	...
	RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, iqk_info->syn1to2= 0x%x, 0x35c = 0x%x\n", path, iqk_info->syn1to2, halrf_rreg(rf, 0x35c, 0xffffffff));

	halrf_wreg(rf, 0x9fe0, 0xff000000, iqk_version_8852a);
	//2G5G6G = 0/1/2
...
void iqk_restore_8852ab(struct rf_info *rf, u8 path)
{
	...
	if(rf->hal_com->dbcc_en) {
		halrf_wreg(rf, 0x8120, 0x10000000, 0x1);
		halrf_wreg(rf, 0x8220, 0x10000000, 0x1);
		halrf_wreg(rf, 0x8220, 0x00010000, 0x1);
	} else {
	...
	RF_DBG(rf, DBG_RF_IQK, "[IQK]0x%x = 0x%x\n", 0x8120, halrf_rreg(rf, 0x8120, 0xffffffff));
	RF_DBG(rf, DBG_RF_IQK, "[IQK]0x%x = 0x%x\n", 0x8220, halrf_rreg(rf, 0x8220, 0xffffffff));
...
__iram_func__
bool halrf_check_fwiqk_done_8852ab(struct rf_info *rf)
{
	u32 counter = 0x0;
	bool flag = false;
	bool isfail  = false;
	...
	if(flag) {
		...
		RF_DBG(rf, DBG_RF_IQK, "[IQK] Load FW Done, 0x8010[7:0] = %x!!\n", halrf_rreg(rf, 0x8010, 0x000000ff));

	} else {
		RF_DBG(rf, DBG_RF_IQK, "[IQK] Load FW Fail, counter = %d!!\n", counter);	
		RF_DBG(rf, DBG_RF_IQK, "[IQK] Load FW Fail, 0x8010[7:0] = %x!!\n", halrf_rreg(rf, 0x8010, 0x000000ff));
...
void halrf_doiqk_8852ab(struct rf_info *rf, bool force, enum phl_phy_idx phy_idx, u8 path)
{
	...
	iqk_info->version = iqk_version_8852a;
	iqk_info->iqk_step = 0;
