...
void _halrf_psd_backup_bb_registers_8852bp(
	struct rf_info *rf,
...
void _halrf_psd_reload_bb_registers_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	u32 *reg,
	u32 *reg_backup,
...
void _halrf_psd_bkup_rf_8852bp(
	struct rf_info *rf,
	u32 *rf_reg,
	u8 path)
{
	...
	for (i = 0; i < PSD_RF_REG_NUM_8852BP; i++) {
...
void _halrf_psd_reload_rf_8852bp(
	struct rf_info *rf,
	u32 *rf_reg,
	u8 path)
{
...
void _halrf_psd_set_dac_off_adc_8852bp(struct rf_info *rf, enum phl_phy_idx phy)

{
	...
	/*07_CLK_Setting_RxIQK_BW20M40M_Non_DBCC_PHY0_path01*/
	if (psd_info->path == RF_PATH_A) {
		if (bw == CHANNEL_WIDTH_20 || bw == CHANNEL_WIDTH_40) {
			halrf_wreg(rf, 0x8120, MASKDWORD, 0x44000a08);
			halrf_wreg(rf, 0x8220, MASKDWORD, 0x44000a08);
			...
			halrf_wreg(rf, 0x8120, MASKDWORD, 0x4d000a08);
			halrf_wreg(rf, 0x8220, MASKDWORD, 0x4d000a08);
			halrf_wreg(rf, 0x12b8, 0x40000000, 0x1);
			halrf_wreg(rf, 0x32b8, 0x40000000, 0x1);
			halrf_delay_us(rf, 100);
			halrf_wreg(rf, 0x030c, 0xff000000, 0x0f);
...
void halrf_psd_init_8852bp(struct rf_info *rf, enum phl_phy_idx phy,
			u8 path, u8 iq_path, u32 avg, u32 fft)
{
	struct halrf_psd_data *psd_info = &rf->psd;
	u32 reg_tmp;
	u32 bb_reg[PSD_BACKUP_NUM_8852BP] = {
			0x20fc, 0x5864, 0x7864, 0x12b8, 0x32b8,
			0x030c, 0x032c, 0x58c8, 0x78c8, 0x2008,
			0x0c1c, 0x0700, 0x0c70, 0x0c60, 0x0c6c,
			0x58ac, 0x78ac, 0x0c3c, 0x2344, 0x4490,
			0x12a0, 0x32a0, 0x8080, 0x8008, 0x8088,
			0x80d0, 0x8074, 0x81dc, 0x82dc, 0x8120,
			0x8220, 0x8154, 0x8254, 0x8140, 0x8240,
			0x8144, 0x8244, 0x801c, 0x8000, 0x8034,
			0x8018, 0x81cc, 0x82cc, 0x802c, 0x80d4,
			0x80fc};

	u32 rf_reg[PSD_RF_REG_NUM_8852BP] = {0x0, 0x5, 0x8f, 0x10000, 0x10005};

	RF_DBG(rf, DBG_RF_PSD, "======> %s   phy=%d   path=%d   iq_path=%d   avg=%d   fft=%d\n",
		__func__, phy,  path, iq_path, avg, fft);

	if (psd_info->psd_progress == 0) {
		_halrf_psd_backup_bb_registers_8852bp(rf, phy, bb_reg,
				psd_info->psd_reg_backup, PSD_BACKUP_NUM_8852BP);

		_halrf_psd_bkup_rf_8852bp(rf, rf_reg, RF_PATH_A);
		_halrf_psd_bkup_rf_8852bp(rf, rf_reg, RF_PATH_B);
	}
	...
	/*02_BB_AFE_Non_DBCC_PHY0_path01_20211215*/
	...
	_halrf_psd_set_dac_off_adc_8852bp(rf, phy);

	/*08a_S0_IQKPSD_Apply_RxCFIR*/
	if (path == RF_PATH_A) {
		...
		halrf_wreg(rf, 0x81dc, MASKDWORD, 0x00000000);
	} else {	/*08a_S1_IQKPSD_Apply_RxCFIR*/
	...
	if (path == RF_PATH_A) {
		...
		halrf_wreg(rf, 0x8000, MASKDWORD, 0x0000000a);
	}

	halrf_wreg(rf, 0x8034, 0x00000030, 0x3);
	halrf_wreg(rf, 0x8018, 0xfffffff0, 0x4001010);
	...
	if (path == RF_PATH_A) {
		...
		halrf_wreg(rf, 0x81dc, MASKDWORD, 0x00000001);
		halrf_wreg(rf, 0x81dc, MASKDWORD, 0x00000000);
	} else {	/*06a_S1_IQKPSD_Apply_RxCFIR_52BP*/
		halrf_wreg(rf, 0x8254, 0x00000100, 0x0);
		halrf_wreg(rf, 0x8240, 0x00000100, 0x1);
		reg_tmp = halrf_rrf(rf, path, 0x00, 0x03800);
		halrf_wreg(rf, 0x8244, 0x00070000, reg_tmp);
...
void halrf_psd_restore_8852bp(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct halrf_psd_data *psd_info = &rf->psd;
	...
	//halrf_wrf(rf, RF_PATH_A, 0xde, 0xfe000, 0x0);
	//halrf_wrf(rf, RF_PATH_B, 0xde, 0xfe000, 0x0);
	//halrf_wrf(rf, RF_PATH_A, 0xef, 0x00004, 0x0);
	//halrf_wrf(rf, RF_PATH_B, 0xef, 0x00004, 0x0);
	halrf_wrf(rf, RF_PATH_A, 0x0, 0xf0000, 0x3);
	halrf_wrf(rf, RF_PATH_B, 0x0, 0xf0000, 0x3);
	//halrf_wrf(rf, RF_PATH_A, 0x5c, 0x80000, 0x0);
	//halrf_wrf(rf, RF_PATH_B, 0x5c, 0x80000, 0x0);
	//halrf_wrf(rf, RF_PATH_A, 0x5e, 0x80000, 0x0);
	//halrf_wrf(rf, RF_PATH_B, 0x5e, 0x80000, 0x0);
	...
	if (psd_info->psd_progress == 1) {
		_halrf_psd_reload_bb_registers_8852bp(rf, phy, bb_reg,
			psd_info->psd_reg_backup, PSD_BACKUP_NUM_8852BP);
		_halrf_psd_reload_rf_8852bp(rf, rf_reg, RF_PATH_A);
		_halrf_psd_reload_rf_8852bp(rf, rf_reg, RF_PATH_B);
	}

	psd_info->psd_progress = 0;
}

u32 halrf_psd_get_point_data_8852bp(struct rf_info *rf,
...
void halrf_psd_query_8852bp(struct rf_info *rf, enum phl_phy_idx phy,
			u32 point, u32 start_point, u32 stop_point, u32 *outbuf)
{
	...
	while (i < stop_point) {
		...
		psd_info->psd_data[j] = halrf_psd_get_point_data_8852bp(rf, phy, point_temp);
