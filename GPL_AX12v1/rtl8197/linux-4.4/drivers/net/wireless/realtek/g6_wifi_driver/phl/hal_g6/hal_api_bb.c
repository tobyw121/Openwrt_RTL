...
#include "phy/bb/halbb_env_mntr.h"
#endif
#ifdef CONFIG_MIB_PROC_CTL
#include "phy/bb/halbb_env_mntr.h"
#endif
...
rtw_hal_bb_cfg_rx_path(struct rtw_hal_com_t *hal_com, u8 rx_path)
{
	...
	if(status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s halbb_ctrl_rx_path - failed\n", __func__);
		_os_warn_on(1);
	}
...
rtw_hal_bb_cfg_tx_path(struct rtw_hal_com_t *hal_com, u8 tx_path)
{
	...
	if(status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s halbb_ctrl_tx_path - failed\n", __func__);
		_os_warn_on(1);
	}
...
s8 rtw_hal_bb_get_tx_pwr_by_txrate (void *hal, enum channel_width bw, u8 mcs_idx)
{
    ...
    u8 i;
    s8 pwr_by_rate = 0, pwr_by_lmt = 0, pwr = 0;

    pwr_by_rate = by_rate->pwr_by_rate[HAL_MAX_PATH-1][mcs_idx];
    if(bw == CHANNEL_WIDTH_80)
    pwr_by_lmt = lmt->pwr_lmt_80m[HAL_MAX_PATH-1][0][0];
    else if(bw == CHANNEL_WIDTH_40)
    pwr_by_lmt =  lmt->pwr_lmt_40m[HAL_MAX_PATH-1][0][0];
    else if(bw == CHANNEL_WIDTH_20)
    pwr_by_lmt =  lmt->pwr_lmt_20m[HAL_MAX_PATH-1][0][0];

    pwr = (tpu->pwr_lmt_en) ? MIN(pwr_by_rate, pwr_by_lmt) : pwr_by_rate;
    return pwr;
}
			 ...
			 struct rtw_phl_rx_pkt *phl_rx, u8 is_su)
{
	...
	struct rtw_phl_stainfo_t *sta = NULL;
	u16 macid;
	void *drv_priv = hal_info->hal_com->drv_priv;
	struct dvobj_priv *pobj = (struct dvobj_priv *)drv_priv;
	struct phl_info_t *phl_info = (struct phl_info_t*)pobj->phl;
	...
	if ((bb_rpt.rssi_avg != 0) || (bb_rpt.physts_rpt_valid == 1)) {
		...
		if(hal_ppdu->usr_num){
			for (i = 0; i < hal_ppdu->usr_num; i++) {
				macid = hal_ppdu->usr[i].macid;
				sta = rtw_phl_get_stainfo_by_macid(phl_info, macid);
				if(sta && macid !=0 && (mdata->ppdu_type == RTW_RX_8852A_DESC_PPDU_T_HE_SU)){
					sta->stats.HE_rx_rssi += bb_rpt.rssi_avg;
					sta->stats.HE_rx_rssi_cnt++;
				}
			}
		}
	}
...
enum rtw_hal_status rtw_hal_bb_get_rssi_ex(struct hal_info_t *hal_info, s8 *rssi_ex, enum rf_path rx_path, u8 nss, u8 cur_phy_idx)
{
	...
	for(i = 0; i < nss; i++) {
		*(rssi_ex + i) = (s8)rssi_t.rssi_seg[cur_phy_idx].rssi[i];
		PHL_INFO("[MP HAL API]%s: rssi_ex[%d]=%d\n", __FUNCTION__, i, *(rssi_ex + i));
	}

	return ret;
}

enum rtw_hal_status rtw_hal_bb_get_rpl(struct hal_info_t *hal_info, enum rf_path rx_path, u16 *rpl_rssi, u8 cur_phy_idx)
{
	enum rtw_hal_status ret=RTW_HAL_STATUS_SUCCESS;

	PHL_INFO("[MP HAL API]%s\n", __FUNCTION__);

	*rpl_rssi = halbb_mp_get_rpl(hal_info->bb, rx_path, cur_phy_idx);

	PHL_INFO("[MP HAL API]%s: rpl_rssi=%d\n", __FUNCTION__, *rpl_rssi);
...
enum rtw_hal_status
rtw_hal_bb_set_carry_suppression_tx(struct rtw_hal_com_t *hal_com, u8 enable,
...
#ifdef CONFIG_RTW_ACS
void rtw_hal_bb_acs_mntr_trigger(struct hal_info_t *hal_info, void *parm)
{
...
enum rtw_hal_status rtw_hal_bb_acs_mntr_result(struct hal_info_t *hal_info, void *rpt)
{
	...
	struct acs_mntr_rpt *acs_rpt = (struct acs_mntr_rpt *)rpt;
	...
	if ((result & (CLM_SUCCESS | NHM_SUCCESS)) != (CLM_SUCCESS | NHM_SUCCESS)) {
		...
		hal_mem_cpy(hal_info->hal_com, acs_rpt->nhm_rpt, mntr_rpt.nhm_rpt, NHM_RPT_NUM);
		return RTW_HAL_STATUS_SUCCESS;
	}
}
...
void rtw_hal_bb_rx_ndp_mp(void *hal)
{
	...
	halbb_rx_setting(hal_info->bb, 0);
	// r_en_sound_wo_ndp
...
void _dump_ru_tbl_hdr(struct rtw_rua_tbl_hdr *tbl_hdr)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "%s\n", __func__);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "info.tbl_hdr.rw 0x%x\n",tbl_hdr->rw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "info.tbl_hdr.idx 0x%x\n", tbl_hdr->idx);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "info.tbl_hdr.offset 0x%x\n", tbl_hdr->offset);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "info.tbl_hdr.len 0x%x\n", tbl_hdr->len);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "info.tbl_hdr.type 0x%x\n", tbl_hdr->type);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "info.tbl_hdr.tbl_class 0x%x\n", tbl_hdr->tbl_class);
}

void _dump_dlru_tbl(struct rtw_dl_ru_gp_tbl *tbl)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s \n", __func__);
	_dump_ru_tbl_hdr(&tbl->tbl_hdr);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ppdu_bw 0x%x\n", tbl->ppdu_bw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tx_pwr 0x%x\n", tbl->tx_pwr);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "txpwr_ofld_en 0x%x\n", tbl->txpwr_ofld_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "pwrlim_dis 0x%x\n", tbl->pwrlim_dis);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "pwr_boost_fac 0x%x\n", tbl->pwr_boost_fac);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_mode_flag 0x%x\n", tbl->fix_mode_flag);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.fix_ba 0x%x\n", tbl->tf.fix_ba);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.ru_psd 0x%x\n", tbl->tf.ru_psd);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.tf_rate 0x%x\n", tbl->tf.tf_rate);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.rf_gain_fix 0x%x\n", tbl->tf.rf_gain_fix);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.rf_gain_idx 0x%x\n", tbl->tf.rf_gain_idx);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.tb_ppdu_bw 0x%x\n", tbl->tf.tb_ppdu_bw);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.rate.ss 0x%x\n", tbl->tf.rate.ss);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.rate.dcm 0x%x\n", tbl->tf.rate.dcm);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf.rate.mcs 0x%x\n", tbl->tf.rate.mcs);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "gi_ltf 0x%x\n", tbl->tf.gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "doppler 0x%x\n", tbl->tf.doppler);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "stbc 0x%x\n", tbl->tf.stbc);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta_coding 0x%x\n", tbl->tf.sta_coding);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tb_t_pe_nom 0x%x\n", tbl->tf.tb_t_pe_nom);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "pr20_bw_en 0x%x\n", tbl->tf.pr20_bw_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ma_type 0x%x\n", tbl->tf.ma_type);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n");
}

void _dump_dlru_fix_tbl_ax4ru(union rtw_dlru_fixtbl *tbl)
{
	u8 i = 0;
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s \n", __func__);
	_dump_ru_tbl_hdr(&tbl->ax4ru.tbl_hdr);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "min_sta_num 0x%x\n", tbl->ax4ru.min_sta_num);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "max_sta_num 0x%x\n", tbl->ax4ru.max_sta_num);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fixru_flag 0x%x\n", tbl->ax4ru.fixru_flag);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "rupos_csht_flag 0x%x\n", tbl->ax4ru.rupos_csht_flag);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ru_swp_flg 0x%x\n", tbl->ax4ru.ru_swp_flg);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "gi_ltf 0x%x\n", tbl->ax4ru.gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ma_type  0x%x\n", tbl->ax4ru.ma_type);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "stbc 0x%x\n", tbl->ax4ru.stbc);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "doppler 0x%x\n", tbl->ax4ru.doppler);
	for (i = 0; i < HALBB_AX4RU_STA_NUM ;i++) {
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] mac_id 0x%x\n", i, tbl->ax4ru.sta[i].mac_id);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] ru_pos[0] 0x%x\n", i, tbl->ax4ru.sta[i].ru_pos[0]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] ru_pos[1] 0x%x\n", i, tbl->ax4ru.sta[i].ru_pos[1]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] ru_pos[2] 0x%x\n", i, tbl->ax4ru.sta[i].ru_pos[2]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] fix_rate 0x%x\n", i, tbl->ax4ru.sta[i].fix_rate);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] fix_coding 0x%x\n", i, tbl->ax4ru.sta[i].fix_coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] fix_txbf 0x%x\n", i, tbl->ax4ru.sta[i].fix_txbf);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] fix_pwr_fac 0x%x\n", i, tbl->ax4ru.sta[i].fix_pwr_fac);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] rate dcm 0x%x\n", i, tbl->ax4ru.sta[i].rate.dcm);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] rate ss 0x%x\n", i, tbl->ax4ru.sta[i].rate.ss);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] rate mcs 0x%x\n", i, tbl->ax4ru.sta[i].rate.mcs);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] txbf 0x%x\n", i, tbl->ax4ru.sta[i].txbf);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] coding 0x%x\n", i, tbl->ax4ru.sta[i].coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] pwr_boost_fac 0x%x\n", i, tbl->ax4ru.sta[i].pwr_boost_fac);
	}
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n");
}

void _dump_dlru_fix_tbl_ax8ru(union rtw_dlru_fixtbl *tbl)
{
	u8 i = 0;
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s \n", __func__);
	_dump_ru_tbl_hdr(&tbl->ax8ru.tbl_hdr);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "min_sta_num 0x%x\n", tbl->ax8ru.min_sta_num);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "max_sta_num 0x%x\n", tbl->ax8ru.max_sta_num);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fixru_flag 0x%x\n", tbl->ax8ru.fixru_flag);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "rupos_csht_flag 0x%x\n", tbl->ax8ru.rupos_csht_flag);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ru_swp_flg 0x%x\n", tbl->ax8ru.ru_swp_flg);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "gi_ltf 0x%x\n", tbl->ax8ru.gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ma_type  0x%x\n", tbl->ax8ru.ma_type);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "stbc 0x%x\n", tbl->ax8ru.stbc);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "doppler 0x%x\n", tbl->ax8ru.doppler);
	for (i = 0; i < HALBB_AX4RU_STA_NUM ;i++) {
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] mac_id 0x%x\n", i, tbl->ax8ru.sta[i].mac_id);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] ru_pos[0] 0x%x\n", i, tbl->ax8ru.sta[i].ru_pos[0]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] ru_pos[1] 0x%x\n", i, tbl->ax8ru.sta[i].ru_pos[1]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] ru_pos[2] 0x%x\n", i, tbl->ax8ru.sta[i].ru_pos[2]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] fix_rate 0x%x\n", i, tbl->ax8ru.sta[i].fix_rate);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] fix_coding 0x%x\n", i, tbl->ax8ru.sta[i].fix_coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] fix_txbf 0x%x\n", i, tbl->ax8ru.sta[i].fix_txbf);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] fix_pwr_fac 0x%x\n", i, tbl->ax8ru.sta[i].fix_pwr_fac);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] rate dcm 0x%x\n", i, tbl->ax8ru.sta[i].rate.dcm);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] rate ss 0x%x\n", i, tbl->ax8ru.sta[i].rate.ss);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] rate mcs 0x%x\n", i, tbl->ax8ru.sta[i].rate.mcs);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] txbf 0x%x\n", i, tbl->ax8ru.sta[i].txbf);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] coding 0x%x\n", i, tbl->ax8ru.sta[i].coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d] pwr_boost_fac 0x%x\n", i, tbl->ax8ru.sta[i].pwr_boost_fac);
	}
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n");
}

void _dump_ulru_tbl(struct rtw_ul_ru_gp_tbl *tbl)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "%s\n", __func__);
	_dump_ru_tbl_hdr(&tbl->tbl_hdr);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "grp_psd_max 0x%x\n", tbl->grp_psd_max);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "grp_psd_min 0x%x\n", tbl->grp_psd_min);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf_rate 0x%x\n", tbl->tf_rate);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_tf_rate 0x%x\n", tbl->fix_tf_rate);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ppdu_bw 0x%x\n", tbl->ppdu_bw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "rf_gain_fix 0x%x\n", tbl->rf_gain_fix);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "rf_gain_idx 0x%x\n", tbl->rf_gain_idx);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_mode_flag 0x%x\n", tbl->fix_mode_flag);
}

void _dump_ulru_fix_tbl_ax4ru(union rtw_ulru_fixtbl *tbl)
{
	u8 i = 0;
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "%s\n", __func__);
	_dump_ru_tbl_hdr(&tbl->ax4ru.tbl_hdr);
	...
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "gi_ltf 0x%x\n", tbl->ax4ru.gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "stbc 0x%x\n", tbl->ax4ru.stbc);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_tb_t_pe_nom: 0x%x\n", tbl->ax4ru.fix_tb_t_pe_nom);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tb_t_pe_nom 0x%x\n", tbl->ax4ru.tb_t_pe_nom);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fixru_flag 0x%x\n", tbl->ax4ru.fixru_flag);
	for (i = 0; i < HALBB_AX4RU_STA_NUM; i++) {
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].mac_id 0x%x\n",i, tbl->ax4ru.sta[i].mac_id);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].ru_pos[0] 0x%x\n",i, tbl->ax4ru.sta[i].ru_pos[0]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].ru_pos[1] 0x%x\n",i, tbl->ax4ru.sta[i].ru_pos[1]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].ru_pos[2] 0x%x\n",i, tbl->ax4ru.sta[i].ru_pos[2]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].tgt_rssi[0] 0x%x\n",i, tbl->ax4ru.sta[i].tgt_rssi[0]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].tgt_rssi[1] 0x%x\n",i, tbl->ax4ru.sta[i].tgt_rssi[1]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].tgt_rssi[2] 0x%x\n",i, tbl->ax4ru.sta[i].tgt_rssi[2]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].fix_tgt_rssi 0x%x\n",i, tbl->ax4ru.sta[i].fix_tgt_rssi);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].fix_rate 0x%x\n",i, tbl->ax4ru.sta[i].fix_rate);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].fix_coding 0x%x\n",i, tbl->ax4ru.sta[i].fix_coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].coding 0x%x\n",i, tbl->ax4ru.sta[i].coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].rate.dcm 0x%x\n",i, tbl->ax4ru.sta[i].rate.dcm);
...
void _dump_ulru_fix_tbl_ax8ru(union rtw_ulru_fixtbl *tbl)
{
	u8 i = 0;
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "%s\n", __func__);
	_dump_ru_tbl_hdr(&tbl->ax8ru.tbl_hdr);
	...
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "gi_ltf 0x%x\n", tbl->ax8ru.gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "stbc 0x%x\n", tbl->ax8ru.stbc);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_tb_t_pe_nom: 0x%x\n", tbl->ax8ru.fix_tb_t_pe_nom);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tb_t_pe_nom 0x%x\n", tbl->ax8ru.tb_t_pe_nom);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fixru_flag 0x%x\n", tbl->ax8ru.fixru_flag);
	for (i = 0; i < HALBB_AX4RU_STA_NUM; i++) {
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].mac_id 0x%x\n",i, tbl->ax8ru.sta[i].mac_id);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].ru_pos[0] 0x%x\n",i, tbl->ax8ru.sta[i].ru_pos[0]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].ru_pos[1] 0x%x\n",i, tbl->ax8ru.sta[i].ru_pos[1]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].ru_pos[2] 0x%x\n",i, tbl->ax8ru.sta[i].ru_pos[2]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].tgt_rssi[0] 0x%x\n",i, tbl->ax8ru.sta[i].tgt_rssi[0]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].tgt_rssi[1] 0x%x\n",i, tbl->ax8ru.sta[i].tgt_rssi[1]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].tgt_rssi[2] 0x%x\n",i, tbl->ax8ru.sta[i].tgt_rssi[2]);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].fix_tgt_rssi 0x%x\n",i, tbl->ax8ru.sta[i].fix_tgt_rssi);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].fix_rate 0x%x\n",i, tbl->ax8ru.sta[i].fix_rate);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].fix_coding 0x%x\n",i, tbl->ax8ru.sta[i].fix_coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].coding 0x%x\n",i, tbl->ax8ru.sta[i].coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[%d].rate.dcm 0x%x\n",i, tbl->ax8ru.sta[i].rate.dcm);
...
void _dump_ru_sta_info(struct rtw_ru_sta_info *sta)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	_dump_ru_tbl_hdr(&sta->tbl_hdr);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "gi_ltf_48spt 0x%x\n", sta->gi_ltf_48spt);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "gi_ltf_18spt 0x%x\n", sta->gi_ltf_18spt);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_info_en 0x%x\n", sta->dlsu_info_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_bw 0x%x\n", sta->dlsu_bw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_gi_ltf 0x%x\n", sta->dlsu_gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_doppler_ctrl 0x%x\n", sta->dlsu_doppler_ctrl);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_coding 0x%x\n", sta->dlsu_coding);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_txbf 0x%x\n", sta->dlsu_txbf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_stbc 0x%x\n", sta->dlsu_stbc);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dl_fwcqi_flag 0x%x\n", sta->dl_fwcqi_flag);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlru_ratetbl_ridx 0x%x\n", sta->dlru_ratetbl_ridx);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "csi_info_bitmap 0x%x\n", sta->csi_info_bitmap);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dl_swgrp_bitmap 0x%x\n", sta->dl_swgrp_bitmap);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_dcm 0x%x\n", sta->dlsu_dcm);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_rate 0x%x\n", sta->dlsu_rate);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlsu_pwr 0x%x\n", sta->dlsu_pwr);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_info_en 0x%x\n", sta->ulsu_info_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_bw 0x%x\n", sta->ulsu_bw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_gi_ltf 0x%x\n", sta->ulsu_gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_doppler_ctrl 0x%x\n", sta->ulsu_doppler_ctrl);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_dcm 0x%x\n", sta->ulsu_dcm);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_ss 0x%x\n", sta->ulsu_ss);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_mcs 0x%x\n", sta->ulsu_mcs);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_fwcqi_flag 0x%x\n", sta->ul_fwcqi_flag);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulru_ratetbl_ridx 0x%x\n", sta->ulru_ratetbl_ridx);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_stbc 0x%x\n", sta->ulsu_stbc);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_coding 0x%x\n", sta->ulsu_coding);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulsu_rssi_m 0x%x\n", sta->ulsu_rssi_m);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_swgrp_bitmap 0x%x\n", sta->ul_swgrp_bitmap);
}

void _dump_dl_macid_set(struct rtw_dl_macid_cfg *dlmacid_cfg)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->macid 0x%x\n", dlmacid_cfg->macid);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_rate_cfg 0x%x\n", dlmacid_cfg->dl_su_rate_cfg);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_rate 0x%x\n", dlmacid_cfg->dl_su_rate);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_bw 0x%x\n", dlmacid_cfg->dl_su_bw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_pwr_cfg 0x%x\n", dlmacid_cfg->dl_su_pwr_cfg);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_pwr 0x%x\n", dlmacid_cfg->dl_su_pwr);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->gi_ltf_4x8_support 0x%x\n", dlmacid_cfg->gi_ltf_4x8_support);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->gi_ltf_1x8_support 0x%x\n", dlmacid_cfg->gi_ltf_1x8_support);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_info_en 0x%x\n", dlmacid_cfg->dl_su_info_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_gi_ltf 0x%x\n", dlmacid_cfg->dl_su_gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_doppler_ctrl 0x%x\n", dlmacid_cfg->dl_su_doppler_ctrl);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_coding 0x%x\n", dlmacid_cfg->dl_su_coding);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_txbf 0x%x\n", dlmacid_cfg->dl_su_txbf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_stbc 0x%x\n", dlmacid_cfg->dl_su_stbc);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "dlmacid_cfg->dl_su_dcm 0x%x\n", dlmacid_cfg->dl_su_dcm);
}

void _dump_ul_macid_set(struct rtw_ul_macid_set *ulmacid_cfg)
{
	int i;

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	for (i = 0; i < 4; i++) {
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].macid 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].macid);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_info_en %d\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_info_en);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_bw 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_bw);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_gi_ltf 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_gi_ltf);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_doppler_ctrl 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_doppler_ctrl);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_dcm 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_dcm);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_ss 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_ss);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_mcs 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_mcs);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_stbc 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_stbc);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_coding 0x%x\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_coding);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].ul_su_rssi_m %d\n", i, ulmacid_cfg->ul_macid_cfg[i].ul_su_rssi_m);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_macid_cfg[%d].endcmd %d\n", i, ulmacid_cfg->ul_macid_cfg[i].endcmd);
	}
}

void _dump_sw_grp_set(struct rtw_sw_grp_set *sw_grp)
{
	int i;

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	for (i = 0; i < 4; i++) {
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "swgrp_bitmap[%d].macid 0x%x\n", i, sw_grp->swgrp_bitmap[i].macid);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "swgrp_bitmap[%d].en_upd_dl_swgrp 0x%x\n", i, sw_grp->swgrp_bitmap[i].en_upd_dl_swgrp);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "swgrp_bitmap[%d].en_upd_ul_swgrp 0x%x\n", i, sw_grp->swgrp_bitmap[i].en_upd_ul_swgrp);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "swgrp_bitmap[%d].cmdend 0x%x\n", i, sw_grp->swgrp_bitmap[i].cmdend);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "swgrp_bitmap[%d].dl_sw_grp_bitmap 0x%x\n", i, sw_grp->swgrp_bitmap[i].dl_sw_grp_bitmap);
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "swgrp_bitmap[%d].ul_sw_grp_bitmap 0x%x\n", i, sw_grp->swgrp_bitmap[i].ul_sw_grp_bitmap);
	}
}

void _dump_dl_ch_bw_cfg(struct rtw_ch_bw_notif *ch_bw_cfg)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "band_idx 0x%x\n", ch_bw_cfg->band_idx);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "band_type 0x%x\n", ch_bw_cfg->band_type);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "pri_ch 0x%x\n", ch_bw_cfg->pri_ch);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "central_ch 0x%x\n", ch_bw_cfg->central_ch);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "cbw 0x%x\n", ch_bw_cfg->cbw);
}

enum rtw_hal_status
rtw_hal_bb_set_dlru_tbl(struct hal_info_t *hal_info,
			struct rtw_phl_dlru_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	struct rtw_dl_ru_gp_tbl info = {{0}};

	info.tbl_hdr.rw = RUA_TBL_RW_WRITE;
	info.tbl_hdr.idx = tbl->tbl_hdr.idx;
	info.tbl_hdr.offset = 0;
	info.tbl_hdr.len = sizeof(struct rtw_dl_ru_gp_tbl);
	//info.tbl_hdr.type = RUA_TBL_TYPE_SW;
	info.tbl_hdr.type = tbl->tbl_hdr.type;
	info.tbl_hdr.tbl_class = RUA_TBL_CL_DLRU_SW;

#if 0
	if (CHANNEL_WIDTH_80 <= tbl->ppdu_bw) {
		info.ppdu_bw = tbl->ppdu_bw;
	} else {
		info.ppdu_bw = 2;
		PHL_INFO("%s : ppdu bw not support type!\n", __func__);
	}
#else
	info.ppdu_bw = tbl->ppdu_bw;
#endif

	info.tx_pwr = tbl->tx_pwr;
	info.pwr_boost_fac = tbl->pwr_boost_fac;
	info.fix_mode_flag = tbl->fix_mode_flag ? 1 : 0;
	info.txpwr_ofld_en = tbl->txpwr_ofld_en;
	info.pwrlim_dis = tbl->pwrlim_dis;
	info.tf.fix_ba = tbl->tf.fix_ba;
	info.tf.ru_psd = tbl->tf.ru_psd;
	info.tf.tf_rate = tbl->tf.tf_rate;
	info.tf.rf_gain_fix = tbl->tf.rf_gain_fix;
	info.tf.rf_gain_idx = tbl->tf.rf_gain_idx;
#if 0
	if (CHANNEL_WIDTH_80 <= tbl->tf.tb_ppdu_bw) {
		info.tf.tb_ppdu_bw = tbl->tf.tb_ppdu_bw;
	} else {
		info.tf.tb_ppdu_bw = 2;
		PHL_INFO("%s : tf ppdu bw not support type!\n", __func__);
	}
#else
	info.tf.tb_ppdu_bw = tbl->tf.tb_ppdu_bw;
#endif
	info.tf.rate.dcm = tbl->tf.rate.dcm;
	info.tf.rate.ss = tbl->tf.rate.ss;
	info.tf.rate.mcs = tbl->tf.rate.mcs;
	info.tf.gi_ltf = (u8)tbl->tf.gi_ltf;
	info.tf.doppler = (u8)tbl->tf.doppler;
	info.tf.stbc = (u8)tbl->tf.stbc;
	info.tf.sta_coding = (u8)tbl->tf.sta_coding;
	info.tf.tb_t_pe_nom = (u8)tbl->tf.tb_t_pe_nom;
	info.tf.pr20_bw_en = (u8)tbl->tf.pr20_bw_en;
	info.tf.ma_type = (u8)tbl->tf.ma_type;

	/* dump */
	_dump_dlru_tbl(&info);

	hstatus = halbb_upd_dlru_grptbl(hal_info->bb, &info);
	if (hstatus == RTW_HAL_STATUS_SUCCESS) {
		tbl->tbl_hdr.is_upd_to_fw = true;
	} else {
...
enum rtw_hal_status
rtw_hal_bb_set_dlru_fix_tbl_ax4ru(struct hal_info_t *hal_info,
			    struct rtw_phl_dlru_fix_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	union rtw_dlru_fixtbl info;
	u8 i = 0;

	_os_mem_set(hal_to_drvpriv(hal_info), &info, 0, sizeof(info));

	info.ax4ru.tbl_hdr.rw = RUA_TBL_RW_WRITE;
	info.ax4ru.tbl_hdr.idx = tbl->tbl_hdr.idx;
	info.ax4ru.tbl_hdr.offset = 0;
	info.ax4ru.tbl_hdr.len = sizeof(struct rtw_dlru_fixtbl_ax4ru);
	//info.tbl_hdr.type = RUA_TBL_TYPE_SW;
	info.ax4ru.tbl_hdr.type = tbl->tbl_hdr.type;
	info.ax4ru.tbl_hdr.tbl_class = RUA_TBL_CL_DLRU_SW_FIX;

	info.ax4ru.max_sta_num = tbl->max_sta_num;
	info.ax4ru.min_sta_num = tbl->min_sta_num;
	info.ax4ru.doppler = tbl->doppler;
	info.ax4ru.stbc = tbl->stbc;
	info.ax4ru.gi_ltf = tbl->gi_ltf;
	info.ax4ru.ma_type = tbl->ma_type;
	info.ax4ru.fixru_flag = tbl->fixru_flag ? 1 : 0;
	info.ax4ru.rupos_csht_flag = tbl->rupos_csht_flag;
	info.ax4ru.ru_swp_flg = tbl->ru_swp_flg;

	for(i = 0; i < HALBB_AX4RU_STA_NUM;i++) {
		info.ax4ru.sta[i].mac_id = (u8)tbl->sta[i].mac_id;
		if (tbl->ppdu_bw <= CHANNEL_WIDTH_80) {
			info.ax4ru.sta[i].ru_pos[0] =
				(((u8)tbl->sta[i].ru_pos[0]) << 1) & (~BIT(0));
			info.ax4ru.sta[i].ru_pos[1] =
				(((u8)tbl->sta[i].ru_pos[1]) << 1) & (~BIT(0));
			info.ax4ru.sta[i].ru_pos[2] =
				(((u8)tbl->sta[i].ru_pos[2]) << 1) & (~BIT(0));
		} else {
			//info.ax4ru.sta[i].ru_pos[0] =
			//	(((u8)tbl->sta[i].ru_pos[0]) << 1) | BIT(0);
			//info.ax4ru.sta[i].ru_pos[1] =
			...
			//info.ax4ru.sta[i].ru_pos[2] =
			//	(((u8)tbl->sta[i].ru_pos[2]) << 1) | BIT(0);
			info.ax4ru.sta[i].ru_pos[0] = (u8)tbl->sta[i].ru_pos[0];
			info.ax4ru.sta[i].ru_pos[1] = (u8)tbl->sta[i].ru_pos[1];
			info.ax4ru.sta[i].ru_pos[2] = (u8)tbl->sta[i].ru_pos[2];
		}
		info.ax4ru.sta[i].fix_rate = tbl->sta[i].fix_rate ? 1 :0;
		info.ax4ru.sta[i].fix_coding = tbl->sta[i].fix_coding ? 1 :0;
		info.ax4ru.sta[i].fix_txbf = tbl->sta[i].fix_txbf ? 1 :0;
		info.ax4ru.sta[i].fix_pwr_fac = tbl->sta[i].fix_pwr_fac ? 1 :0;
		info.ax4ru.sta[i].rate.ss = tbl->sta[i].rate.ss;
		info.ax4ru.sta[i].rate.dcm = tbl->sta[i].rate.dcm;
		info.ax4ru.sta[i].rate.mcs = tbl->sta[i].rate.mcs;
		info.ax4ru.sta[i].txbf = tbl->sta[i].txbf;
		info.ax4ru.sta[i].coding = tbl->sta[i].coding;
		info.ax4ru.sta[i].pwr_boost_fac = tbl->sta[i].pwr_boost_fac;
	}

	_dump_dlru_fix_tbl_ax4ru(&info);

	hstatus = halbb_upd_dlru_fixtbl(hal_info->bb, &info);
	if (hstatus == RTW_HAL_STATUS_SUCCESS) {
		tbl->tbl_hdr.is_upd_to_fw = true;
	} else {
		tbl->tbl_hdr.is_upd_to_fw = false;
...
enum rtw_hal_status
rtw_hal_bb_set_dlru_fix_tbl_ax8ru(struct hal_info_t *hal_info,
			    struct rtw_phl_dlru_fix_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	union rtw_dlru_fixtbl info;
	...
	info.ax8ru.tbl_hdr.rw = RUA_TBL_RW_WRITE;
	info.ax8ru.tbl_hdr.idx = tbl->tbl_hdr.idx;
	info.ax8ru.tbl_hdr.offset = 0;
	info.ax8ru.tbl_hdr.len = sizeof(struct rtw_dlru_fixtbl_ax8ru);
	//info.tbl_hdr.type = RUA_TBL_TYPE_SW;
	info.ax8ru.tbl_hdr.type = tbl->tbl_hdr.type;
	info.ax8ru.tbl_hdr.tbl_class = RUA_TBL_CL_DLRU_SW_FIX;

	info.ax8ru.max_sta_num = tbl->max_sta_num;
	info.ax8ru.min_sta_num = tbl->min_sta_num;
	info.ax8ru.doppler = tbl->doppler;
	info.ax8ru.stbc = tbl->stbc;
	info.ax8ru.gi_ltf = tbl->gi_ltf;
	info.ax8ru.ma_type = tbl->ma_type;
	info.ax8ru.fixru_flag = tbl->fixru_flag ? 1 : 0;
	info.ax8ru.rupos_csht_flag = tbl->rupos_csht_flag;
	info.ax8ru.ru_swp_flg = tbl->ru_swp_flg;

	for(i = 0; i < HALBB_AX8RU_STA_NUM;i++) {
		info.ax8ru.sta[i].mac_id = (u8)tbl->sta[i].mac_id;
		if (tbl->ppdu_bw <= CHANNEL_WIDTH_80) {
			info.ax8ru.sta[i].ru_pos[0] =
				(((u8)tbl->sta[i].ru_pos[0]) << 1) & (~BIT(0));
			info.ax8ru.sta[i].ru_pos[1] =
				(((u8)tbl->sta[i].ru_pos[1]) << 1) & (~BIT(0));
			info.ax8ru.sta[i].ru_pos[2] =
				(((u8)tbl->sta[i].ru_pos[2]) << 1) & (~BIT(0));
			info.ax8ru.sta[i].ru_pos[3] =
				(((u8)tbl->sta[i].ru_pos[3]) << 1) & (~BIT(0));
			info.ax8ru.sta[i].ru_pos[4] =
				(((u8)tbl->sta[i].ru_pos[4]) << 1) & (~BIT(0));
			info.ax8ru.sta[i].ru_pos[5] =
				(((u8)tbl->sta[i].ru_pos[5]) << 1) & (~BIT(0));
			info.ax8ru.sta[i].ru_pos[6] =
				(((u8)tbl->sta[i].ru_pos[6]) << 1) & (~BIT(0));
		} else {
			//info.ax8ru.sta[i].ru_pos[0] =
			//	(((u8)tbl->sta[i].ru_pos[0]) << 1) | BIT(0);
			//info.ax8ru.sta[i].ru_pos[1] =
			...
			//info.ax8ru.sta[i].ru_pos[6] =
			//	(((u8)tbl->sta[i].ru_pos[6]) << 1) | BIT(0);
			info.ax8ru.sta[i].ru_pos[0] = (u8)tbl->sta[i].ru_pos[0];
			info.ax8ru.sta[i].ru_pos[1] = (u8)tbl->sta[i].ru_pos[1];
			info.ax8ru.sta[i].ru_pos[2] = (u8)tbl->sta[i].ru_pos[2];
			info.ax8ru.sta[i].ru_pos[3] = (u8)tbl->sta[i].ru_pos[3];
			info.ax8ru.sta[i].ru_pos[4] = (u8)tbl->sta[i].ru_pos[4];
			info.ax8ru.sta[i].ru_pos[5] = (u8)tbl->sta[i].ru_pos[5];
			info.ax8ru.sta[i].ru_pos[6] = (u8)tbl->sta[i].ru_pos[6];
		}
		info.ax8ru.sta[i].fix_rate = tbl->sta[i].fix_rate ? 1 :0;
		info.ax8ru.sta[i].fix_coding = tbl->sta[i].fix_coding ? 1 :0;
		info.ax8ru.sta[i].fix_txbf = tbl->sta[i].fix_txbf ? 1 :0;
		info.ax8ru.sta[i].fix_pwr_fac = tbl->sta[i].fix_pwr_fac ? 1 :0;
		info.ax8ru.sta[i].rate.ss = tbl->sta[i].rate.ss;
		info.ax8ru.sta[i].rate.dcm = tbl->sta[i].rate.dcm;
		info.ax8ru.sta[i].rate.mcs = tbl->sta[i].rate.mcs;
		info.ax8ru.sta[i].txbf = tbl->sta[i].txbf;
		info.ax8ru.sta[i].coding = tbl->sta[i].coding;
		info.ax8ru.sta[i].pwr_boost_fac = tbl->sta[i].pwr_boost_fac;
	}

	_dump_dlru_fix_tbl_ax8ru(&info);
...
enum rtw_hal_status
rtw_hal_bb_set_ulru_tbl(struct hal_info_t *hal_info, struct rtw_phl_ulru_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	struct rtw_ul_ru_gp_tbl info = {{0}};

	info.tbl_hdr.rw = RUA_TBL_RW_WRITE;
	info.tbl_hdr.idx = tbl->tbl_hdr.idx;
	info.tbl_hdr.offset = 0;
	info.tbl_hdr.len = sizeof(struct rtw_ul_ru_gp_tbl);
	info.tbl_hdr.type = RUA_TBL_TYPE_SW;
	info.tbl_hdr.tbl_class = RUA_TBL_CL_ULRU_SW;

	info.grp_psd_max = tbl->grp_psd_max;
	info.grp_psd_min = tbl->grp_psd_min;
	info.tf_rate = tbl->tf_rate;
	info.fix_tf_rate = tbl->fix_tf_rate ? 1 : 0;
	info.ppdu_bw = tbl->ppdu_bw;
	info.rf_gain_fix = tbl->rf_gain_fix ? 1 : 0;
	info.rf_gain_idx = tbl->rf_gain_idx;
	info.fix_mode_flag = tbl->fix_mode_flags ? 1 : 0;

	_dump_ulru_tbl(&info);

	hstatus = halbb_upd_ulru_grptbl(hal_info->bb, &info);
	if (hstatus == RTW_HAL_STATUS_SUCCESS) {
		tbl->tbl_hdr.is_upd_to_fw = true;
	} else {
		tbl->tbl_hdr.is_upd_to_fw = false;
...
enum rtw_hal_status
rtw_hal_bb_set_ulru_fix_tbl_ax4ru(struct hal_info_t *hal_info,
			    struct rtw_phl_ulru_fix_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	union rtw_ulru_fixtbl info;
	u8 i = 0;
	...
	info.ax4ru.tbl_hdr.idx = tbl->tbl_hdr.idx;
	info.ax4ru.tbl_hdr.offset = 0;
	info.ax4ru.tbl_hdr.len = sizeof(struct rtw_ulru_fixtbl_ax4ru);
	info.ax4ru.tbl_hdr.type = RUA_TBL_TYPE_SW;
	info.ax4ru.tbl_hdr.tbl_class = RUA_TBL_CL_ULRU_SW_FIX;
	...
	info.ax4ru.gi_ltf = tbl->gi_ltf;
	info.ax4ru.stbc = tbl->stbc;
	info.ax4ru.fix_tb_t_pe_nom = tbl->fix_tb_t_pe_nom ? 1 : 0;
	info.ax4ru.tb_t_pe_nom = tbl->tb_t_pe_nom;
	info.ax4ru.fixru_flag = tbl->fixru_flag ? 1 : 0;
	for (i = 0; i < HALBB_AX4RU_STA_NUM; i++) {
		info.ax4ru.sta[i].mac_id = (u8)tbl->sta[i].mac_id;
		if (tbl->ppdu_bw <= CHANNEL_WIDTH_80) {
		...
		info.ax4ru.sta[i].tgt_rssi[0] = tbl->sta[i].tgt_rssi[0];
		info.ax4ru.sta[i].tgt_rssi[1] = tbl->sta[i].tgt_rssi[1];
		info.ax4ru.sta[i].tgt_rssi[2] = tbl->sta[i].tgt_rssi[2];
		info.ax4ru.sta[i].fix_tgt_rssi = tbl->sta[i].fix_tgt_rssi ? 1 : 0;
		info.ax4ru.sta[i].fix_rate = tbl->sta[i].fix_rate ? 1 : 0;
		info.ax4ru.sta[i].fix_coding = tbl->sta[i].fix_coding ? 1 : 0;
		info.ax4ru.sta[i].coding = tbl->sta[i].coding;
		info.ax4ru.sta[i].rate.dcm = tbl->sta[i].rate.dcm;
	...
	_dump_ulru_fix_tbl_ax4ru(&info);

	hstatus = halbb_upd_ulru_fixtbl(hal_info->bb, &info);
	if (hstatus == RTW_HAL_STATUS_SUCCESS) {
		tbl->tbl_hdr.is_upd_to_fw = true;
	} else {
		tbl->tbl_hdr.is_upd_to_fw = false;
...
enum rtw_hal_status
rtw_hal_bb_set_ulru_fix_tbl_ax8ru(struct hal_info_t *hal_info,
			    struct rtw_phl_ulru_fix_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	union rtw_ulru_fixtbl info;
	...
	info.ax8ru.tbl_hdr.idx = tbl->tbl_hdr.idx;
	info.ax8ru.tbl_hdr.offset = 0;
	info.ax8ru.tbl_hdr.len = sizeof(struct rtw_ulru_fixtbl_ax8ru);
	info.ax8ru.tbl_hdr.type = RUA_TBL_TYPE_SW;
	info.ax8ru.tbl_hdr.tbl_class = RUA_TBL_CL_ULRU_SW_FIX;
	...
	info.ax8ru.gi_ltf = tbl->gi_ltf;
	info.ax8ru.stbc = tbl->stbc;
	info.ax8ru.fix_tb_t_pe_nom = tbl->fix_tb_t_pe_nom ? 1 : 0;
	info.ax8ru.tb_t_pe_nom = tbl->tb_t_pe_nom;
	info.ax8ru.fixru_flag = tbl->fixru_flag ? 1 : 0;
	for (i = 0; i < HALBB_AX8RU_STA_NUM; i++) {
		info.ax8ru.sta[i].mac_id = (u8)tbl->sta[i].mac_id;
		if (tbl->ppdu_bw <= CHANNEL_WIDTH_80) {
		...
		info.ax8ru.sta[i].tgt_rssi[0] = tbl->sta[i].tgt_rssi[0];
		info.ax8ru.sta[i].tgt_rssi[1] = tbl->sta[i].tgt_rssi[1];
		info.ax8ru.sta[i].tgt_rssi[2] = tbl->sta[i].tgt_rssi[2];
		info.ax8ru.sta[i].tgt_rssi[3] = tbl->sta[i].tgt_rssi[3];
		info.ax8ru.sta[i].tgt_rssi[4] = tbl->sta[i].tgt_rssi[4];
		info.ax8ru.sta[i].tgt_rssi[5] = tbl->sta[i].tgt_rssi[5];
		info.ax8ru.sta[i].tgt_rssi[6] = tbl->sta[i].tgt_rssi[6];

		info.ax8ru.sta[i].fix_tgt_rssi = tbl->sta[i].fix_tgt_rssi ? 1 : 0;
		info.ax8ru.sta[i].fix_rate = tbl->sta[i].fix_rate ? 1 : 0;
		info.ax8ru.sta[i].fix_coding = tbl->sta[i].fix_coding ? 1 : 0;
		info.ax8ru.sta[i].coding = tbl->sta[i].coding;
		info.ax8ru.sta[i].rate.dcm = tbl->sta[i].rate.dcm;
...
enum rtw_hal_status
rtw_hal_bb_set_ru_sta_info(struct hal_info_t *hal_info,
			   struct rtw_phl_ru_sta_info *sta)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	struct rtw_ru_sta_info info = {{0}};

	info.tbl_hdr.rw = RUA_TBL_RW_WRITE;
	info.tbl_hdr.idx = sta->tbl_hdr.idx;
	info.tbl_hdr.offset = 0;
	info.tbl_hdr.len = sizeof(struct rtw_ru_sta_info);
	info.tbl_hdr.type = RUA_TBL_TYPE_SW;
	info.tbl_hdr.tbl_class = RUA_TBL_CL_RU_STA;

	info.gi_ltf_48spt = sta->gi_ltf_48spt;
	info.gi_ltf_18spt = sta->gi_ltf_18spt;
	info.dlsu_info_en = sta->dlsu_info_en ? 1 : 0;
	if (CHANNEL_WIDTH_80 <= sta->dlsu_bw) {
		info.dlsu_bw = sta->dlsu_bw;
	} else {
		info.dlsu_bw = 2;
		PHL_INFO("%s : ru sta dl su bw not support type!\n", __func__);
	}
	info.dlsu_gi_ltf = sta->dlsu_gi_ltf;
	info.dlsu_doppler_ctrl = sta->dlsu_doppler_ctrl;
	info.dlsu_coding = sta->dlsu_coding;
	info.dlsu_txbf = sta->dlsu_txbf;
	info.dlsu_stbc = sta->dlsu_stbc;
	info.dl_fwcqi_flag = sta->dl_fwcqi_flag ? 1 :0;
	info.dlru_ratetbl_ridx = (u8)sta->dlru_ratetbl_ridx;
	info.csi_info_bitmap = sta->csi_info_bitmap;
	info.dl_swgrp_bitmap = sta->dl_swgrp_bitmap;
	info.dlsu_dcm = sta->dlsu_dcm;
	info.dlsu_rate = sta->dlsu_rate;
	info.dlsu_pwr = sta->dlsu_pwr;
	info.ulsu_info_en = sta->ulsu_info_en ? 1 : 0;
	if (CHANNEL_WIDTH_80 <= sta->ulsu_bw) {
		info.ulsu_bw = sta->ulsu_bw;
	} else {
		info.ulsu_bw = 2;
		PHL_INFO("%s : ru sta ul su bw not support type!\n", __func__);
	}
	info.ulsu_gi_ltf = sta->ulsu_gi_ltf;
	info.ulsu_doppler_ctrl = sta->ulsu_doppler_ctrl;
	info.ulsu_dcm = sta->ulsu_rate.dcm;
	info.ulsu_ss = sta->ulsu_rate.ss;
	info.ulsu_mcs = sta->ulsu_rate.mcs;
	info.ul_fwcqi_flag = sta->ul_fwcqi_flag ? 1 : 0;
	info.ulru_ratetbl_ridx = (u8)sta->ulru_ratetbl_ridx;
	info.ulsu_stbc = sta->ulsu_stbc;
	info.ulsu_coding = sta->ulsu_coding;
	info.ulsu_rssi_m = sta->ulsu_rssi;
	info.ul_swgrp_bitmap = (u8)sta->ul_swgrp_bitmap;/*TODO: BB struct issue*/

	_dump_ru_sta_info(&info);

	hstatus = halbb_upd_rusta_info(hal_info->bb, &info);
	if (hstatus == RTW_HAL_STATUS_SUCCESS) {
		sta->tbl_hdr.is_upd_to_fw = true;
	} else {
		sta->tbl_hdr.is_upd_to_fw = false;
	}
	return hstatus;
}

enum rtw_hal_status
rtw_hal_bb_set_dlmacid_cfg(struct hal_info_t *hal_info,
			   struct rtw_phl_dlmacid_cfg *cfg)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	struct rtw_dl_macid_cfg dlmacid_cfg;

	_rtw_memset(&dlmacid_cfg, 0, sizeof(struct rtw_dl_macid_cfg));

	dlmacid_cfg.macid = cfg->macid;
	dlmacid_cfg.dl_su_rate_cfg = cfg->dl_su_rate_cfg;
	dlmacid_cfg.dl_su_rate = cfg->dl_su_rate;
	dlmacid_cfg.dl_su_bw = cfg->dl_su_bw;
	dlmacid_cfg.dl_su_pwr_cfg = cfg->dl_su_pwr_cfg;
	dlmacid_cfg.dl_su_pwr = cfg->dl_su_pwr;
	dlmacid_cfg.gi_ltf_4x8_support = cfg->gi_ltf_4x8_support;
	dlmacid_cfg.gi_ltf_1x8_support = cfg->gi_ltf_1x8_support;
	dlmacid_cfg.dl_su_info_en = cfg->dl_su_info_en;
	dlmacid_cfg.dl_su_gi_ltf = cfg->dl_su_gi_ltf;
	dlmacid_cfg.dl_su_doppler_ctrl = cfg->dl_su_doppler_ctrl;
	dlmacid_cfg.dl_su_coding = cfg->dl_su_coding;
	dlmacid_cfg.dl_su_txbf = cfg->dl_su_txbf;
	dlmacid_cfg.dl_su_stbc = cfg->dl_su_stbc;
	dlmacid_cfg.dl_su_dcm = cfg->dl_su_dcm;

	_dump_dl_macid_set(&dlmacid_cfg);

	hstatus = halbb_dlmacid_cfg(hal_info->bb, &dlmacid_cfg);

	return hstatus;
}

enum rtw_hal_status
rtw_hal_bb_set_ulmacid_cfg(struct hal_info_t *hal_info,
			   struct rtw_phl_ulmacid_set *cfg)
{
	...
	_rtw_memset(&ulmacid_cfg, 0, sizeof(struct rtw_ul_macid_set));

	for (i = 0; i < 8; i++) {
		ulmacid_cfg.ul_macid_cfg[i].macid= cfg->phl_ul_macid_cfg[i].macid;
		ulmacid_cfg.ul_macid_cfg[i].endcmd= cfg->phl_ul_macid_cfg[i].endcmd;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_info_en = cfg->phl_ul_macid_cfg[i].ul_su_info_en;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_bw = cfg->phl_ul_macid_cfg[i].ul_su_bw;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_gi_ltf = cfg->phl_ul_macid_cfg[i].ul_su_gi_ltf;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_doppler_ctrl = cfg->phl_ul_macid_cfg[i].ul_su_doppler_ctrl;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_dcm = cfg->phl_ul_macid_cfg[i].ul_su_dcm;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_ss = cfg->phl_ul_macid_cfg[i].ul_su_ss;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_mcs = cfg->phl_ul_macid_cfg[i].ul_su_mcs;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_stbc = cfg->phl_ul_macid_cfg[i].ul_su_stbc;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_coding = cfg->phl_ul_macid_cfg[i].ul_su_coding;
		ulmacid_cfg.ul_macid_cfg[i].ul_su_rssi_m = cfg->phl_ul_macid_cfg[i].ul_su_rssi_m;
	}

	_dump_ul_macid_set(&ulmacid_cfg);

	hstatus = halbb_ulmacid_cfg(hal_info->bb, &ulmacid_cfg);

	return hstatus;
}

enum rtw_hal_status
rtw_hal_bb_set_swgrp_set(struct hal_info_t *hal_info,
			    struct rtw_phl_sw_grp_set *info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	struct rtw_sw_grp_set sw_grp;
	int i;

	_rtw_memset(&sw_grp, 0, sizeof(struct rtw_sw_grp_set));

	for (i = 0; i < 8; i++) {
		sw_grp.swgrp_bitmap[i].macid = info->phl_swgrp_bitmap[i].macid;
		sw_grp.swgrp_bitmap[i].en_upd_dl_swgrp = info->phl_swgrp_bitmap[i].en_upd_dl_swgrp;
		sw_grp.swgrp_bitmap[i].en_upd_ul_swgrp = info->phl_swgrp_bitmap[i].en_upd_ul_swgrp;
		sw_grp.swgrp_bitmap[i].cmdend = info->phl_swgrp_bitmap[i].cmdend;
		sw_grp.swgrp_bitmap[i].dl_sw_grp_bitmap = info->phl_swgrp_bitmap[i].dl_sw_grp_bitmap;
		sw_grp.swgrp_bitmap[i].ul_sw_grp_bitmap = info->phl_swgrp_bitmap[i].ul_sw_grp_bitmap;
	}

	_dump_sw_grp_set(&sw_grp);

	hstatus = halbb_swgrp_hdl(hal_info->bb, &sw_grp);

	return hstatus;
}

enum rtw_hal_status
rtw_hal_bb_ch_bw_notif(struct hal_info_t *hal_info,
			   struct rtw_phl_ch_bw_notif *cfg)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	struct rtw_ch_bw_notif ch_bw_cfg;

	_rtw_memset(&ch_bw_cfg, 0, sizeof(struct rtw_ch_bw_notif));

	ch_bw_cfg.band_idx = cfg->band_idx;
	ch_bw_cfg.band_type = cfg->band_type;
	ch_bw_cfg.pri_ch = cfg->pri_ch;
	ch_bw_cfg.central_ch = cfg->central_ch;
	ch_bw_cfg.cbw = cfg->cbw;

	_dump_dl_ch_bw_cfg(&ch_bw_cfg);

	hstatus = halbb_ch_bw_notif(hal_info->bb, &ch_bw_cfg);

	return hstatus;
}

enum rtw_hal_status
rtw_hal_bb_pwrtbl_notif(struct hal_info_t *hal_info,
			   struct rtw_phl_pwrtbl_notif *cfg)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	struct rtw_pwrtbl_notif pwrtbl;

	_rtw_memset(&pwrtbl, 0, sizeof(struct rtw_pwrtbl_notif));

	pwrtbl.txpwrtbl_ofld_en = cfg->txpwrtbl_ofld_en;
	pwrtbl.band_idx = cfg->band_idx;

	hstatus = halbb_pwrtbl_notif(hal_info->bb, &pwrtbl);
...
/*s8 rtw_hal_bb_get_max_pwr_dbm(void *hal, u8 rate, enum rf_path path, bool is_bf){
	...
	if (tpu->pwr_lmt_en){
		...
		PHL_ERR("report max power limit dbm\n");


	} else {

		PHL_ERR("report max power by rate dbm\n");
...
void rtw_hal_bb_adjust_ref_offset(void *hal, s16 offset, enum phl_phy_idx phy_idx){

	struct hal_info_t *hal_info = (struct hal_info_t *) hal;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
	struct rtw_tpu_info *tpu = &hal_com->band[phy_idx].rtw_tpu_i;
	struct bb_info *bb = hal_info->bb;

	tpu->ref_pow_ofdm = offset << 2; // s(9,2)
	tpu->ref_pow_cck = offset << 2; // s(9,2)

	PHL_WARN("set ref_pow_ofdm = %d\n", tpu->ref_pow_ofdm);
	PHL_WARN("set ref_pow_cck  = %d\n", tpu->ref_pow_cck);

	halbb_set_tx_pow_ref(hal_info->bb, phy_idx);
}

void rtw_hal_bb_ser_pause(void *hal)
{
	struct hal_info_t *hal_info = (struct hal_info_t *) hal;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
	struct rtw_tpu_info *tpu = &hal_com->rtw_tpu_i;
	...
	/* Pause DIG when SER, pause with no set, resume with no recovery */
	halbb_pause_func(bb, F_DIG, HALBB_PAUSE_NO_SET, HALBB_PAUSE_LV_2, 0, val);
}

void rtw_hal_bb_ser_resume(void *hal)
{
	struct hal_info_t *hal_info = (struct hal_info_t *) hal;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
	struct rtw_tpu_info *tpu = &hal_com->rtw_tpu_i;
	...
	halbb_pause_func(bb, F_DIG, HALBB_RESUME_NO_RECOVERY, HALBB_PAUSE_LV_2, 0, val);
}
...
void rtw_hal_bb_set_dig_opmode(void *hal, u8 opmode) {

	struct hal_info_t *hal_info = (struct hal_info_t *) hal;

	halbb_dig_mode_update(hal_info->bb, opmode);
}
...
enum rtw_hal_status rtw_hal_bb_get_rpl(struct hal_info_t *hal_info,
					enum rf_path rx_path, u16 *rpl_rssi, u8 cur_phy_idx)
{
	return RTW_HAL_STATUS_SUCCESS;
}
