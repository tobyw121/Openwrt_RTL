	...
	struct phl_info_t *phl_info, struct rtw_wifi_role_t *wrole)
{
	...
	if (IS_AP_ROLE_TYPE(wrole->type) || wrole->type == PHL_RTYPE_P2P_GO || wrole->type == PHL_RTYPE_MESH) {
		#ifdef RTW_PHL_BCN
...
void phl_dump_cc_band_map(struct phl_info_t *phl_info, u16 cc_band_map)
{
	...
	if (cc_band_map & BIT(CC_BAND_6G))
		PHL_DATA(COMP_PHL_DBG, _PHL_INFO_, " %s ", "B_6G");
		 ...
		 bool (*issue_null_data)(void *priv, u8 ridx, bool ps))
{
	...
	phl_list_for_loop(chanctx, struct rtw_chan_ctx, &band_ctrl->chan_ctx_queue.queue, list) {
		...
		for (ridx = 0; ridx < MAX_WIFI_ROLE_NUMBER; ridx++) {
			if (chanctx->role_map & BIT(ridx)) {
				...
				else if (IS_AP_ROLE_TYPE(wr->type) || wr->type == PHL_RTYPE_MESH){
	...
	if(!found){
		PHL_INFO("No chanctx is the same as current channel!\n");
		return psts;
	}
		 ...
		 bool (*issue_null_data)(void *priv, u8 ridx, bool ps))
{
	enum rtw_phl_status psts = RTW_PHL_STATUS_FAILURE;
#ifdef CONFIG_MR_SUPPORT
	psts = _phl_mr_offch_hdl(phl_info, wrole->hw_band, off_ch, obj_priv,
				  issue_null_data);
	...
	} else if (IS_AP_ROLE_TYPE(wrole->type)) {
...
static inline bool phl_mr_is_trigger_dbcc(struct phl_info_t *phl_info)
{
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct mr_ctl_t *mr_ctl = phlcom_to_mr_ctrl(phl_com);
...
phl_mr_trig_dbcc_enable(struct phl_info_t *phl_info, bool trig)
{
	...
	if (trig == true) {
		...
		if (mr_ctl->is_sb == true) /*single band*/
...
{
	return phl_mr_trig_dbcc_enable((struct phl_info_t *)phl, true);
}

void phl_mr_dbcc_msg_complete(void* priv, struct phl_msg* msg)
{
...
/*notify core to handle protocol handshake*/
enum rtw_phl_status
phl_mr_dbcc_send_msg(struct phl_info_t *phl_info,
				struct rtw_wifi_role_t *wrole,
				bool dbcc_en)
{
	...
	dbcc_proto = _os_mem_alloc(drv, sizeof(struct dbcc_proto_msg));

	if (dbcc_proto == NULL) {
		PHL_ERR("%s alloc dbcc protocol msg failed\n", __func__);
		goto _err;
	}
	dbcc_proto->wr = wrole;
	dbcc_proto->dbcc_en = dbcc_en;

	msg.inbuf = (u8 *)dbcc_proto;
	msg.inlen = sizeof(struct dbcc_proto_msg);
	msg.band_idx = wrole->hw_band;
	attr.completion.completion = phl_mr_dbcc_msg_complete;
	attr.completion.priv = phl_info;

	SET_MSG_MDL_ID_FIELD(msg.msg_id, PHL_MDL_MRC);
	SET_MSG_EVT_ID_FIELD(msg.msg_id, MSG_EVT_DBCC_PROTOCOL_HDL);

	psts = phl_msg_hub_send(phl_info, &attr, &msg);
	if (RTW_PHL_STATUS_SUCCESS != psts) {
		PHL_ERR("%s sending message failed\n", __func__);
		_os_mem_free(drv, dbcc_proto, sizeof(struct dbcc_proto_msg));
		goto _err;
	}
	psts = RTW_PHL_STATUS_SUCCESS;
_err:
			       ...
			       struct phl_msg *msg)
{
	...
	bool (*core_issue_null_data)(void *, u8, bool) = NULL;
	...
	hal_ch = rtw_hal_get_cur_ch(phl_info->hal, role->hw_band);

	/* 1.issue null(1) - pause Rx*/
	/*switch ch to op-chan*/
	chctx_num = phl_mr_get_chandef_by_band(phl_info, role->hw_band, &op_chdef);
	PHL_DUMP_CHAN_DEF_EX(&op_chdef);
	hal_ch = rtw_hal_get_cur_ch(phl_info->hal, role->hw_band);
	if (chctx_num == 1) {
		if (hal_ch != op_chdef.chan) {
			PHL_ERR("hw chan(%d) not in op-chan(%d)\n", hal_ch, op_chdef.chan);
			phl_set_ch_bw(phl_info, role->hw_band, &op_chdef, false);
		}
	}
	...
	rtw_hal_clean_tx_queue(phl_info->hal);
	rtw_hal_dbcc_cfg(phl_info->hal, phl_com, true);

	/* 4.issue null(0) - unpause Rx*/
	phl_mr_offch_hdl(phl_info,
			  role,
			  false,
			  phl_com->drv_priv,
	...
	/* 5.reallocate hw resouce - role*/
	band_idx = HW_BAND_1;
	phl_wifi_role_realloc_band(phl_info, role, band_idx);
...
phl_mr_trig_dbcc_disable(struct phl_info_t *phl_info, bool trig)
{
	...
	if (trig == true) {
		_os_spinlock(phl_to_drvpriv(phl_info), &mr_ctl->lock, _bh, NULL);
		if (mr_ctl->is_sb == false) /*dual band*/
			mr_ctl->trigger_dbcc_cfg = true;
		_os_spinunlock(phl_to_drvpriv(phl_info), &mr_ctl->lock, _bh, NULL);
	} else {
		_os_spinlock(phl_to_drvpriv(phl_info), &mr_ctl->lock, _bh, NULL);
...
{
	return phl_mr_trig_dbcc_disable((struct phl_info_t *)phl, true);
}
			       ...
			       struct phl_msg *msg)
{
	...
	if ((module_id != PHL_MDL_MRC) &&
		...
		PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_,
			"%s: msg not from DIS-CONT/AP-STOP FG or MRC\n", __FUNCTION__);
		ret = MDL_RET_IGNORE;
		goto _exit;
	}
	...
	if ((MSG_EVT_ID_FIELD(msg->msg_id) == MSG_EVT_DISCONNECT_END) ||
		...
		/* 4.reallocate hw resouce - or other role -case B*/
		//phl_wifi_role_realloc_band(phl_info, role, HW_BAND_0);
		...
		_phl_mr_offch_hdl(phl_info,
				  ...
				  phl_com->drv_priv,
				  core_issue_null_data);
		/* 7.reallocate hw resouce - role-case A */
		phl_wifi_role_realloc_band(phl_info, role, HW_BAND_0);
				   ...
				   struct rtw_wifi_role_t *wrole)
{
	...
	psts = phl_mr_tdmra_disable(phl_info, wrole, wrole->hw_band);
	if (psts != RTW_PHL_STATUS_SUCCESS) {
		PHL_ERR("%s: phl_mr_tdmra_disable fail\n", __func__);
		goto _exit;
	}
#endif
	PHL_DUMP_MR_EX(phl_info);
			       ...
			       struct phl_msg *msg)
{
	...
	_os_mem_set(phl_to_drvpriv(phl_info), &scan_ch, 0, sizeof(scan_ch));
			 ...
			 struct phl_msg *msg)
{
	...
	switch(MSG_EVT_ID_FIELD(msg->msg_id)) {
		...
		case MSG_EVT_DBCC_ENABLE:
			...
			phl_disp_eng_clr_pending_msg(phl_info, HW_BAND_0);
			break;
#endif
		case MSG_EVT_CONNECT_START:
			...
			if (phl_mr_is_trigger_dbcc(phl_info)) {
				if (phl_mr_dbcc_send_msg(phl_info, role, true) !=
							   RTW_PHL_STATUS_SUCCESS) {
		...
		case MSG_EVT_AP_STOP_END:
			...
			if (MSG_EVT_ID_FIELD(msg->msg_id) == MSG_EVT_DISCONNECT_END)
				PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_,
				  "%s: MSG_EVT_DISCONNECT_END\n", __FUNCTION__);
			else
				PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_,
				  "%s: MSG_EVT_AP_STOP_END\n", __FUNCTION__);
			...
			if (phl_mr_is_trigger_dbcc(phl_info)) {
				...
				if (phl_mr_dbcc_send_msg(phl_info, role, false) !=
					RTW_PHL_STATUS_SUCCESS) {
					PHL_ERR("%s mr dbcc send_msg failed\n", __func__);
					ret = MDL_RET_FAIL;
					break;
		...
		case MSG_EVT_SCAN_START:
			...
			PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_, "MSG_EVT_SCAN_START: call phl_mr_mcc_state_change\n");
		...
		case MSG_EVT_SCAN_END:
			...
			phl_set_ch_bw(phl_info, role->hw_band, &chandef, false);
			...
			PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_, "MSG_EVT_SCAN_END: call phl_mr_mcc_state_change\n");
		...
		case MSG_EVT_BTC_REQ_BT_SLOT:
			...
			phl_mr_tdmra_handle(phl_info, NULL, (u16)(*(u32 *)msg->inbuf),
				msg->band_idx, MR_TDMRA_TRIG_BY_BT);
#endif
			ret = MDL_RET_SUCCESS;
			break;
		default:
		...
		if (cur_info->ap_num == 1) {
			...
			if (band_ctrl->op_mode == MR_OP_SWR) {
				...
				for (ridx = 0; ridx < MAX_WIFI_ROLE_NUMBER; ridx++) {
					if (role_map & BIT(ridx)) {
						wr = &(phl_com->wifi_roles[ridx]);
						if (IS_AP_ROLE_TYPE(wr->type) && wr->mstate == MLME_LINKED) {
							ap_wr = wr;
...
enum rtw_phl_status phl_mr_tdmra_disable(struct phl_info_t *phl_info,
...
enum rtw_phl_status
phl_mr_tdmra_handle(struct phl_info_t *phl_info,
				struct rtw_wifi_role_t *cur_wrole, u16 slot,
				enum phl_band_idx band_idx,
				enum mr_tdmra_trigger trgger)
{
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	...
	if (tdmra_need) {
		...
		case MR_TDMRA_TRIG_BY_BT:
			if (tdmra_inprogress) {
				...
				info.dur = slot;
				if (info.role != NULL)
					rtw_phl_tdmra_duration_change(phl_info, &info);
				else
				...
				if (ap_role_id < INVALID_WIFI_ROLE_IDX)
					rtw_phl_tdmra_enable(phl_info, &phl_com->wifi_roles[ap_role_id]);
				else
					PHL_ERR("%s: Do not find solo AP\n", __func__);
			}
			break;
		case MR_TDMRA_TRIG_BY_LINKING:
			/* enable TDMRA (for link flow) */
			if (cur_wrole != NULL) {
				phl_mr_stop_all_beacon(phl_info, cur_wrole, false);
				rtw_phl_tdmra_enable(phl_info, cur_wrole);
			} else {
				PHL_ERR("%s: cur_wrole = NULL, check code flow\n", __func__);
			}
			break;
		case MR_TDMRA_TRIG_BY_DIS_LINKING:
			/* find any existed role to trigger TDMRA */
			existed_role = _mr_find_existed_role(phl_info, band_idx);
			rtw_phl_tdmra_enable(phl_info, existed_role);
			break;
		case MR_TDMRA_TRIG_BY_CHG_SLOT:
			if (tdmra_inprogress) {
				/* update TDMRA (if TDMRAing) */
				info.bt_role = false;
				info.hw_band = band_idx;
		...
		default:
			...
			/* disable TDMRA (if TDMRAing) */
			psts = phl_mr_tdmra_disable(phl_info, cur_wrole, band_idx);
			if (psts != RTW_PHL_STATUS_SUCCESS)
				PHL_ERR("%s: phl_mr_tdmra_disable fail\n", __func__);
	}
	...
	for (ridx = 0; ridx < MAX_WIFI_ROLE_NUMBER; ridx++) {
		if (role_map & BIT(ridx)) {
			...
			if (wr->type == PHL_RTYPE_STATION)
				band_ctrl->cur_info.sta_num++;
			else if (IS_AP_ROLE_TYPE(wr->type))
				band_ctrl->cur_info.ap_num++;
			...
			struct rtw_wifi_role_t *wrole, bool mcc_en)
{
	...
	if (mcc_en){
		...
		psts = rtw_phl_tdmra_enable(phl_info, wrole);
		if (psts == RTW_PHL_STATUS_FAILURE)
			PHL_ERR("%s - rtw_phl_tdmra_enable failed\n", __func__);
	}
	else{
		psts = phl_mr_tdmra_disable(phl_info, wrole, wrole->hw_band);
		if (psts != RTW_PHL_STATUS_SUCCESS)
				PHL_ERR("%s - phl_mr_tdmra_disable failed\n",
					__func__);
...
#ifdef CONFIG_MCC_SUPPORT
	psts = phl_mr_tdmra_handle(phl_info, wrole, 0, wrole->hw_band, MR_TDMRA_TRIG_BY_LINKING);
	if (psts != RTW_PHL_STATUS_SUCCESS) {
		PHL_ERR("%s: phl_mr_tdmra_handle fail(%d)\n", __func__, psts);
		goto exit;
	}
#endif
#ifdef CONFIG_PHL_P2PPS
...
enum rtw_phl_status phl_mr_watchdog(struct phl_info_t *phl_info)
{
	...
	for (role_idx = 0; role_idx < MAX_WIFI_ROLE_NUMBER; role_idx++) {
		struct rtw_wifi_role_t *wrole;
		u32 busy_hiq_num;
		u32 hiq_no_recycle_time;

		if ((mr_ctl->role_map & BIT(role_idx)) == 0)
			continue;

		wrole = &phl_com->wifi_roles[role_idx];
		busy_hiq_num = _os_atomic_read(phl_com->drv_priv, &wrole->hiq_busy_cnt);
		if (busy_hiq_num == 0)
			continue;
		if (!wrole->active) {
			PHL_ERR("Inactive wrole %u has non-zero HiQ busy count %u!\n",
				role_idx, busy_hiq_num);
			continue;
		}

		hiq_no_recycle_time = phl_get_passing_time_ms(wrole->last_hiq_wp_time);
		if (hiq_no_recycle_time < (wrole->bcn_cmn.bcn_interval * 20))
			continue;

		/* Drop MBSSID */
		PHL_PRINT("Drop MBSSID %u HiQ for %u no RP\n",
		          wrole->hw_mbssid, hiq_no_recycle_time);
		#if 1
		rtw_hal_set_hiq_drop(phl_info->hal, wrole->hw_port, wrole->hw_mbssid);
		#else
		rtw_hal_hiq_drop(phl_info->hal, wrole->hw_port, wrole->hw_mbssid);
		#endif
		wrole->dropping_hiq = 2;
	}
