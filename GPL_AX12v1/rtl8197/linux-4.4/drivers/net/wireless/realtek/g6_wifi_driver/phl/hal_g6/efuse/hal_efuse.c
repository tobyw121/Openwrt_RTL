...
void rtw_efuse_deinit(struct rtw_hal_com_t *hal_com, void *efuse)
{
	...
	if (efuse_info) {
		if(efuse_info->bt_mask) {
			_os_mem_free(hal_com->drv_priv, efuse_info->bt_mask, efuse_info->bt_mask_size);
			efuse_info->bt_mask = NULL;
		}

		if(efuse_info->bt_shadow_map) {
			_os_mem_free(hal_com->drv_priv, efuse_info->bt_shadow_map, efuse_info->bt_log_efuse_size);
			efuse_info->bt_shadow_map = NULL;
		}

		if(efuse_info->mask) {
			_os_mem_free(hal_com->drv_priv, efuse_info->mask, efuse_info->mask_size);
			efuse_info->mask = NULL;
		}

		if(efuse_info->map_version) {
			_os_mem_free(hal_com->drv_priv, efuse_info->map_version, efuse_info->version_len);
			efuse_info->map_version = NULL;
		}

		if(efuse_info->mask_version) {
			 _os_mem_free(hal_com->drv_priv, efuse_info->mask_version, efuse_info->version_len);
			 efuse_info->mask_version = NULL;
		}

		if(efuse_info->shadow_map) {
			_os_mem_free(hal_com->drv_priv, efuse_info->shadow_map, efuse_info->log_efuse_size);
