...
u8 alloc_txring(_adapter * padapter)
{
	...
	PHLTX_ENTER;
	padapter->max_tx_ring_cnt = padapter->dvobj->phl_com->dev_id ? MAX_TX_RING_NUM_2G : MAX_TX_RING_NUM_5G;
...
void _rtw_free_xmit_priv(struct xmit_priv *pxmitpriv)
{
	...
	if(pxmitpriv->free_xmitframe_cnt != NR_XMITFRAME) {
		_list *plist, *phead;
		int allocated_xframe = NR_XMITFRAME - pxmitpriv->free_xmitframe_cnt;
...
static s32 update_attrib_sec_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta, enum eap_type eapol_type)
{
	...
	if (pattrib->encrypt &&
		(padapter->securitypriv.sw_encrypt == _TRUE/* || hw_decrypted == _FALSE*/)) {
...
s32 core_tx_alloc_xmitframe(_adapter *padapter, struct xmit_frame **pxmitframe, struct sk_buff *pskb)
{
	...
	if ( check_net_lmt( padapter) )  {
		if ( pxmitpriv->free_xmitframe_cnt <  NR_XMITFRAME - 30 ) {
	...
	else
	{
		...
		if (psta && (psta->state & WIFI_ASOC_STATE)) {
			...
			if(ru_ctrl->tx_phase){
				if(psta->core_current_xframe_cnt > (NR_XMITFRAME/ru_ctrl->netif_drop_thd))
				{
			       ...
			       u32 frag_perfr, u32 wl_frags, struct sta_info *psta)
{
	...
	/* TXREQ_QMGT */
	txreq_buf = (struct xmit_txreq_buf *)get_txreq_buffer(padapter, &txreq, &list, &head, &tail, 0, psta);
	if(txreq_buf == NULL){
...
void fill_txreq_mdata(_adapter *padapter, struct xmit_frame *pxframe)
{
	...
	if (XF_TYPE == RTW_TX_DRV_MGMT) {
		...
		if (get_frame_sub_type(pxframe->buf_addr) == WIFI_PROBERSP) {
