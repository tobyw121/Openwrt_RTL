...
#ifdef RTW_WKARD_GET_PROCESSOR_ID
	ret = p_ops->chk_proc_long_ldy_pcie(adapter, &val8);
	if (ret != MACSUCCESS)
		return ret;
	if (val8 == PROC_LONG_DLY)
		val16 = SET_CLR_WOR2(val16, PCIE_DPHY_DLY_25US,
				     BAC_CMU_EN_DLY_SH, BAC_CMU_EN_DLY_MSK);
#endif
				 ...
				 struct mac_ax_intf_info *intf_info)
{
	...
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852B) ||
	    is_chip_id(adapter, MAC_AX_CHIP_ID_8851B)) {
#ifdef RTW_WKARD_GET_PROCESSOR_ID
		ret = p_ops->chk_proc_long_ldy_pcie(adapter, &proc_type);
		if (ret != MACSUCCESS)
			return ret;

		if (proc_type != PROC_LONG_DLY)
			return MACSUCCESS;
#endif
	} else {
		return MACSUCCESS;
...
u32 lv1rst_stop_dma_pcie(struct mac_ax_adapter *adapter, u8 val)
{
	...
	struct mac_ax_pcie_err_info err_info;
	u32 ret;
	...
	if (ret != MACSUCCESS) {
		ret = p_ops->get_err_flag_pcie(adapter, &err_info);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]get err flag pcie %d\n", ret);
			return ret;
		}

		if (err_info.tx_stuck || err_info.txbd_len_zero)
			txen = MAC_AX_PCIE_DISABLE;
		if (err_info.rx_stuck)
			rxen = MAC_AX_PCIE_DISABLE;
...
u32 chk_proc_long_ldy(struct mac_ax_adapter *adapter, u8 *val)
{
	...
	*val = PROC_SHORT_DLY;
	for (proc = 0; proc < PROC_ID_LIST_NUM; proc++)
		if (adapter->hw_info->adpt_info.cust_proc_id.proc_id.proc_id_h ==
		    proc_id_long_dly[proc].proc_id_h &&
		    adapter->hw_info->adpt_info.cust_proc_id.proc_id.proc_id_l ==
		    proc_id_long_dly[proc].proc_id_l) {
			*val = PROC_SHORT_DLY;
			break;
		}
