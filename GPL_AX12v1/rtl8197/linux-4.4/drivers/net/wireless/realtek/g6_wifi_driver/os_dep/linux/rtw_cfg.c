...
#ifdef RTW_LOG_LEVEL
__DMEM_SECTION__ uint rtw_drv_log_level = (uint)RTW_LOG_LEVEL; /* from Makefile */
#else
__DMEM_SECTION__ uint rtw_drv_log_level = _DRV_INFO_;
#endif /* RTW_LOG_LEVEL */
...
module_param(phl_log_components, uint, 0644);
MODULE_PARM_DESC(phl_log_components, "Set PHL log components.");

extern u8 phl_log_level;
module_param(phl_log_level, byte, 0644);
MODULE_PARM_DESC(phl_log_level, "Set PHL log level.");

#endif /* CONFIG_RTW_DEBUG */

__DMEM_SECTION__ uint rtw_wifi_mode = 0;
module_param(rtw_wifi_mode, uint, 0644);
MODULE_PARM_DESC(rtw_wifi_mode, "wifi performance mode, 0: throughput, 1: balance");
__DMEM_SECTION__ uint rtw_2gtxreq_limit = (MAX_TX_RING_NUM_2G >> 2);
module_param(rtw_2gtxreq_limit, uint, 0644);
MODULE_PARM_DESC(rtw_2gtxreq_limit, "txreq limit, only valid in balance mode");
__DMEM_SECTION__ uint rtw_5gtxreq_limit = (MAX_TX_RING_NUM_5G >> 2);
module_param(rtw_5gtxreq_limit, uint, 0644);
MODULE_PARM_DESC(rtw_5gtxreq_limit, "txreq limit, only valid in balance mode");


#if defined(CONFIG_VW_REFINE) || defined(CONFIG_ONE_TXQ)
u8 rtw_tx_mode = 0;
module_param(rtw_tx_mode, byte, 0644);
MODULE_PARM_DESC(rtw_wifi_mode, "wifi tx mode, 0: normal, 1: veriwave, 2: airtime");
#endif

uint rtw_wmm_dm = 0;
module_param(rtw_wmm_dm, uint, 0644);
MODULE_PARM_DESC(rtw_wmm_dm, "Dynamic WMM flow detection and resource readustment.: 0: Disable, 1: Enable");
...
int rtw_fw_log = 0;
module_param(rtw_fw_log, int, 0644);
MODULE_PARM_DESC(rtw_fw_log, "Whether or not to enable FW log");
...
uint rtw_tx_bw_mode = (BW_CAP_160M << 4 ) | BW_CAP_40M;
...
#else
int rtw_bw_mode = ((BW_CAP_160M | BW_CAP_80M | BW_CAP_40M | BW_CAP_20M) << 4 ) | (BW_CAP_40M | BW_CAP_20M);
#endif
...
#ifdef CONFIG_RTLWIFI6
int rtw_he_enable = 2; /* 0:disable, 1:enable, 2:force auto enable */
#else
...
static void rtw_ap_rfe_config(struct dvobj_priv *dvobj)
{
	struct rtw_phl_com_t *phl_com = GET_HAL_DATA(dvobj);
	u8 set_rfe_type = 50;


	switch (phl_com->dev_sw_cap.band_sup){
...
*/
#ifdef RTW_CUSTOM_PARA_DIR
	int rtw_load_phy_file = (BIT1 | BIT3 | BIT4 | BIT5 | BIT6 | BIT7 | BIT8);
#else
	int rtw_load_phy_file = (BIT1 | BIT3);
#endif /*RTW_CUSTOM_PARA_DIR*/
...
#define SET_PARA_INFO_EXT(para_info_name) \
	do { \
		phl_com->phy_sw_cap[0].para_info_name.para_src = RTW_PARA_SRC_EXTNAL; \
		phl_com->phy_sw_cap[1].para_info_name.para_src = RTW_PARA_SRC_EXTNAL; \
		strcpy(phl_com->phy_sw_cap[0].para_info_name.para_path, \
		       para_path); \
		strcpy(phl_com->phy_sw_cap[1].para_info_name.para_path, \
		       para_path); \
	} while (0);


	struct rtw_phl_com_t *phl_com = GET_HAL_DATA(dvobj);
	char para_path[256] = {0};

	rtw_nvm_fill_full_para_path(dvobj, para_path, sizeof(para_path) - 1);

	RTW_PRINT(FUNC_DEVID_FMT" Parameter path: \"%s\"\n",
	          FUNC_DEVID_ARG(dvobj), para_path);

	if (rtw_load_phy_file & LOAD_BB_PHY_REG_FILE) {
		SET_PARA_INFO_EXT(bb_phy_reg_info);
	}

	if (rtw_load_phy_file & LOAD_RF_RADIO_FILE) {
		SET_PARA_INFO_EXT(rf_radio_a_info);
		SET_PARA_INFO_EXT(rf_radio_b_info);
	}

	if (rtw_load_phy_file & LOAD_RF_TXPWR_BY_RATE) {
		SET_PARA_INFO_EXT(rf_txpwr_byrate_info);
	}

	if (rtw_load_phy_file & LOAD_RF_TXPWR_TRACK_FILE) {
		SET_PARA_INFO_EXT(rf_txpwrtrack_info);
	}

	if (rtw_load_phy_file & LOAD_RF_TXPWR_LMT_FILE) {
		SET_PARA_INFO_EXT(rf_txpwrlmt_info);
	}

	if (rtw_load_phy_file & LOAD_RF_TXPWR_LMT_RU_FILE) {
		SET_PARA_INFO_EXT(rf_txpwrlmt_ru_info);
	}

	if (rtw_load_phy_file & LOAD_BB_PHY_REG_GAIN_FILE) {
		SET_PARA_INFO_EXT(bb_phy_reg_gain_info);
	}

	if (rtw_load_phy_file & LOAD_RF_TXPWR_LMT_6G_FILE) {
		SET_PARA_INFO_EXT(rf_txpwrlmt_6g_info);
	}

	if (rtw_load_phy_file & LOAD_RF_TXPWR_LMT_RU_6G_FILE) {
		SET_PARA_INFO_EXT(rf_txpwrlmt_ru_6g_info);
	}
...
void rtw_core_update_default_setting (struct dvobj_priv *dvobj)
{
	...
	phl_com->phy_sw_cap[0].proto_sup = rtw_wireless_mode;
		...
		&& (  defined(CONFIG_RTW_DEV_IS_SINGLE_BAND) \
		    || defined(CPTCFG_RTW_DEV_IS_SINGLE_BAND))
	phl_com->phy_sw_cap[0].band_sup = dev_get_band_cap(dvobj->pci_data.ppcidev);
	phl_com->dev_sw_cap.band_sup = phl_com->phy_sw_cap[0].band_sup;
	/* Set RX A-MSDU length cap according to RX buffer size */
	do {
		u32 rx_buf_size;
		if (phl_com->phy_sw_cap[0].band_sup & BAND_CAP_2G) {
			rx_buf_size = RXQ_1_BASE_BUF_SIZE;
		} else
			rx_buf_size = RXQ_0_BASE_BUF_SIZE;
		phl_com->proto_sw_cap[0].max_amsdu_len =   (rx_buf_size > 120000)
							 ? 2
							 : (  (rx_buf_size > 8000)
							    ? 1 : 0);
	} while (0);
	...
	#endif
	phl_com->phy_sw_cap[0].bw_sup = BW_CAP_160M |BW_CAP_80M | BW_CAP_40M | BW_CAP_20M; //rtw_bw_mode;
	...
	phl_com->proto_sw_cap[0].he_su_bfmr = 1;
	phl_com->proto_sw_cap[0].he_su_bfme = 1;
	phl_com->proto_sw_cap[0].he_mu_bfmr = 1;
	phl_com->proto_sw_cap[0].he_mu_bfme = 1;

	phl_com->proto_sw_cap[0].vht_su_bfmr = 1;
	phl_com->proto_sw_cap[0].vht_su_bfme = 1;
	phl_com->proto_sw_cap[0].vht_mu_bfmr = 1;
	phl_com->proto_sw_cap[0].vht_mu_bfme = 1;

	phl_com->proto_sw_cap[1].he_su_bfmr = 0;
	phl_com->proto_sw_cap[1].he_su_bfme = 0;
	phl_com->proto_sw_cap[1].he_mu_bfmr = 0;
	phl_com->proto_sw_cap[1].he_mu_bfme = 0;

	phl_com->proto_sw_cap[1].vht_su_bfmr = 0;
	phl_com->proto_sw_cap[1].vht_su_bfme = 0;
	phl_com->proto_sw_cap[1].vht_mu_bfmr = 0;
	phl_com->proto_sw_cap[1].vht_mu_bfme = 0;

	phl_com->role_sw_cap.stbc_cap = (HW_CAP_STBC_HT_TX |
					 HW_CAP_STBC_VHT_TX |
					 HW_CAP_STBC_HE_TX |
					 HW_CAP_STBC_HT_RX |
					 HW_CAP_STBC_VHT_RX |
	...
	phl_com->proto_sw_cap[0].stbc_he_rx = 1;
	phl_com->proto_sw_cap[0].stbc_vht_rx = 1;
	phl_com->proto_sw_cap[0].stbc_ht_rx = 1;
	phl_com->proto_sw_cap[0].stbc_he_tx = 1;
	phl_com->proto_sw_cap[0].stbc_vht_tx = 1;
	phl_com->proto_sw_cap[0].stbc_ht_tx = 1;
	phl_com->proto_sw_cap[0].stbc_tx_greater_80mhz = 0;
	phl_com->proto_sw_cap[0].stbc_rx_greater_80mhz = 0;

	phl_com->proto_sw_cap[1].stbc_he_rx = 1;
	phl_com->proto_sw_cap[1].stbc_vht_rx = 1;
	phl_com->proto_sw_cap[1].stbc_ht_rx = 1;
	phl_com->proto_sw_cap[1].stbc_he_tx = 1;
	phl_com->proto_sw_cap[1].stbc_vht_tx = 1;
	phl_com->proto_sw_cap[1].stbc_ht_tx = 1;
	phl_com->proto_sw_cap[1].stbc_tx_greater_80mhz = 0;
	phl_com->proto_sw_cap[1].stbc_rx_greater_80mhz = 0;
	...
	#endif /* CONFIG_PCI_ASPM */

	#if defined(CONFIG_RTW_DEV_IS_SINGLE_BAND) \
	    || defined(CPTCFG_RTW_DEV_IS_SINGLE_BAND)
	phl_com->hal_spec.band_cap = dev_get_band_cap(dvobj->pci_data.ppcidev);
	phl_com->dev_sw_cap.band_sup = phl_com->hal_spec.band_cap;
	#endif /* CONFIG_RTW_DEV_IS_SINGLE_BAND */

	phl_com->bus_sw_cap.txbd_num = 256;
	/* set RXBD, RXBUF */
	if (phl_com->dev_sw_cap.band_sup == BAND_CAP_5G) {
		phl_com->dev_sw_cap.bw_sup = BW_CAP_160M |BW_CAP_80M | BW_CAP_40M | BW_CAP_20M;
		phl_com->bus_sw_cap.rxbd_num = MAX_RX_BD_NUM_RXQ_0_CORE;
		phl_com->bus_sw_cap.rxbuf_num = MAX_RX_BUF_NUM_RXQ_0_CORE;
		phl_com->bus_sw_cap.rxbuf_size = RX_BUF_SIZE_RXQ_0_CORE;
		phl_com->bus_sw_cap.rpbd_num = MAX_RX_BD_NUM_RPQ_0_CORE;
		phl_com->bus_sw_cap.rpbuf_num = MAX_RX_BUF_NUM_RPQ_0_CORE;
		phl_com->bus_sw_cap.rpbuf_size = RX_BUF_SIZE_RPQ_0_CORE;
		#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
		phl_com->bus_sw_cap.msdu_rxbuf_size = RX_BUF_ONE_MSDU_SIZE;
		phl_com->bus_sw_cap.msdu_rxbd_num = MAX_MSDU_RXBD_NUM;
		phl_com->bus_sw_cap.msdu_rxbuf_num = MAX_MSDU_RXBUF_NUM;
		#endif /* CONFIG_RTW_HW_RX_AMSDU_CUT */
	} else if (phl_com->dev_sw_cap.band_sup == BAND_CAP_2G) {
		phl_com->dev_sw_cap.bw_sup = BW_CAP_40M | BW_CAP_20M;
		phl_com->bus_sw_cap.rxbd_num = MAX_RX_BD_NUM_RXQ_1_CORE;
		phl_com->bus_sw_cap.rxbuf_num = MAX_RX_BUF_NUM_RXQ_1_CORE;
		phl_com->bus_sw_cap.rxbuf_size = RX_BUF_SIZE_RXQ_1_CORE;
		phl_com->bus_sw_cap.rpbd_num = MAX_RX_BD_NUM_RPQ_1_CORE;
		phl_com->bus_sw_cap.rpbuf_num = MAX_RX_BUF_NUM_RPQ_1_CORE;
		phl_com->bus_sw_cap.rpbuf_size = RX_BUF_SIZE_RPQ_1_CORE;
		#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
		phl_com->bus_sw_cap.msdu_rxbuf_size = RX_BUF_ONE_MSDU_SIZE;
		phl_com->bus_sw_cap.msdu_rxbd_num = MAX_MSDU_RXBD_NUM;
		phl_com->bus_sw_cap.msdu_rxbuf_num = MAX_MSDU_RXBUF_NUM;
...
#ifdef CONFIG_RTW_DEV_IS_SINGLE_BAND
	rtw_ap_rfe_config(dvobj);
#else
	phl_com->dev_sw_cap.rfe_type = rtw_rfe_type;
#endif /* CONFIG_RTW_DEV_IS_SINGLE_BAND */

	dvobj->phl_com->dev_sw_cap.fw_cap.dlram_en = true;
	dvobj->phl_com->dev_sw_cap.fw_cap.dlrom_en = false;
#ifdef CONFIG_FILE_FWIMG
#ifdef CONFIG_DYN_FW_LOAD
		/* FW default use internal fw header file */
		dvobj->phl_com->dev_sw_cap.fw_cap.fw_src = RTW_FW_SRC_INTNAL;
#else
		dvobj->phl_com->dev_sw_cap.fw_cap.fw_src = RTW_FW_SRC_EXTNAL;
#endif
#else /* !CONFIG_FILE_FWIMG */
	dvobj->phl_com->dev_sw_cap.fw_cap.fw_src = RTW_FW_SRC_INTNAL;
#endif /* !CONFIG_FILE_FWIMG */
	dvobj->phl_com->dev_sw_cap.rfk_cap = rtw_rfk_ability;

	#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
	rtw_load_phy_file_path(dvobj);
	/* set bfree_para_info = true to free phy para file buffer after load phy para */
	dvobj->phl_com->dev_sw_cap.bfree_para_info = true;
	#endif /* CONFIG_LOAD_PHY_PARA_FROM_FILE */
	dvobj->phl_com->dev_sw_cap.sec_cap.hw_form_hdr = true;
	dvobj->phl_com->dev_sw_cap.sec_cap.hw_sec_iv = true;
	dvobj->phl_com->dev_sw_cap.sec_cap.hw_tx_search_key = true;
#ifdef CONFIG_RTW_TWT
	dvobj->phl_com->dev_sw_cap.twt_sup = RTW_PHL_TWT_RSP_SUP;
#endif
#ifdef CONFIG_RTW_ENABLE_HW_RX_AMSDU_CUT
	phl_com->dev_sw_cap.rx_amsdu_cut = true;
	phl_com->dev_sw_cap.hw_hdr_conv = true;
#endif /* CONFIG_RTW_ENABLE_HW_RX_AMSDU_CUT */
...
uint rtw_load_registry(_adapter *padapter)
{
	...
	registry_par->wireless_mode = (u8)rtw_wireless_mode;
	    ...
	    && (   defined(CONFIG_RTW_DEV_IS_SINGLE_BAND) \
	        || defined(CPTCFG_RTW_DEV_IS_SINGLE_BAND))
	registry_par->band_type = dev_get_band_cap(padapter->dvobj->pci_data.ppcidev);
	#else
...
#if defined(CONFIG_VW_REFINE) || defined(CONFIG_ONE_TXQ)
	registry_par->tx_mode = rtw_tx_mode;
#endif
