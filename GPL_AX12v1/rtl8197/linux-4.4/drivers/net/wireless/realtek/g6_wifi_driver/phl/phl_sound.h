...
#define MIN_SND_TP 1 /* 1Mbps */
#define STABLE_AD_CNT 100
...
enum snd_test_flag {
	...
	PHL_SND_TEST_F_FORCE_TXBF_MU_1SS = BIT(1), /* forced MU only group 1SS STA */
	...
	PHL_SND_TEST_F_ENABLE_MU_SND = BIT(5), /* Enable MU sounding */
	PHL_SND_TEST_F_ENABLE_MU_NONTB_SND = BIT(6), /* Enable MU do non-TB sounding */
	PHL_SND_TEST_F_DISABLE_TP_LIMIT = BIT(7), /* Disable apply SU */
	PHL_SND_TEST_F_FORCE_REMOVE_GRP = BIT(8), /* Force remove all grp  and re-grp */ 
	PHL_SND_TEST_F_OFFLOAD = BIT(9) /* Offload SND behavior to FW */
...
struct phl_snd_sta {
	...
	u8 del_cnt; /* delete from snd group */
};
...
#define SND_OFFLOAD_DISABLE 0
#define SND_OFFLOAD_START 1
#define SND_OFFLOAD_COMPLETE 2
...
struct phl_snd_fix_param {
	...
	u8 f_ru_tbl_40[MAX_SND_HE_BFRP_USER_NUM][MAX_SND_HE_BFRP_USER_NUM];
	u8 f_ru_tbl_80[MAX_SND_HE_BFRP_USER_NUM][MAX_SND_HE_BFRP_USER_NUM];
	u8 f_ru_tbl_160[MAX_SND_HE_BFRP_USER_NUM][MAX_SND_HE_BFRP_USER_NUM];
};
