...
static const char *req_pwr_st_cmd[REQ_PWR_ST_CMD_MAX] = {
	"open_rf",
	"close_rf"
};
		       ...
		       char *output, u32 out_len, u32 *used)
{
	...
	for (req_pwr_st = 0; req_pwr_st < REQ_PWR_ST_CMD_MAX; req_pwr_st++) {
		if (PLTFM_STRCMP(req_pwr_st_cmd[req_pwr_st], input[1]) == 0) {
			MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
				    "\n%s: req_pwr_st:%d\n", __func__, req_pwr_st);
