...
#define PROCESS_CHK_OFFSET 0x5EA
#define PROCESS_CHK_TSMC 0xFF
#define PROCESS_CHK_SMIC 0xFE

u32 mac_pwr_on_sdio_8192xb(struct mac_ax_adapter *adapter)
{
    ...
    /* 0x244[25] = 1 */
	val32 = MAC_REG_R32(R_AX_AFE_ON_CTRL1);
	MAC_REG_W32(R_AX_AFE_ON_CTRL1, val32 | B_AX_REG_VCO_KVCO);
	...
	/* 0x016[2] = 1 */
	val8 = MAC_REG_R8(R_AX_ANAPAR_POW_MAC);
	MAC_REG_W8(R_AX_ANAPAR_POW_MAC, val8 | B_AX_POW_PC_LDO_PORT0);

	/* 0x016[3] = 1 */
	val8 = MAC_REG_R8(R_AX_ANAPAR_POW_MAC);
	MAC_REG_W8(R_AX_ANAPAR_POW_MAC, val8 | B_AX_POW_PC_LDO_PORT1);

	/* 0x0CC[3] = 1 */
	val32 = MAC_REG_R32(R_AX_PMC_DBG_CTRL2);
	MAC_REG_W32(R_AX_PMC_DBG_CTRL2, val32 | B_AX_SYSON_DIS_WLR_AX_WRMSK);

	/* 0x080[3] = 0 */
	val32 = MAC_REG_R32(R_AX_SYS_ISO_CTRL_EXTEND);
	MAC_REG_W32(R_AX_SYS_ISO_CTRL_EXTEND, val32 & ~B_AX_R_SYM_ISO_P02PP);

	/* 0x080[6] = 0 */
	val32 = MAC_REG_R32(R_AX_SYS_ISO_CTRL_EXTEND);
	MAC_REG_W32(R_AX_SYS_ISO_CTRL_EXTEND, val32 & ~B_AX_R_SYM_ISO_P12PP);
	...
	/*enable GPIO16/GPIO17/GPIO18 internal PD for Internal Floatiing Leakage, 0x2E4*/
	...
	/* check power calibration 0x5E9=0xAA is done or not */
	...
	ret1 = mac_read_efuse_plus(adapter, PROCESS_CHK_OFFSET, 0x1, &process_chk_value,
				   MAC_AX_EFUSE_BANK_WIFI);
	if (ret1)
		PLTFM_MSG_ERR("%s: read efuse fail\n", __func__);

	if (pwr_k_chk_value != PWR_K_CHK_VALUE) {
		/* IF 0x5EA=0xFF for TSMC*/
		if (process_chk_value == PROCESS_CHK_TSMC) {
			if ((MAC_REG_R32(R_AX_SYS_CFG1) & B_AX_OPT1_ADIE) != B_AX_OPT1_ADIE) {
				/* 0xF0[9]=0 set 0x238 [5:2]=0xA */
				val32 = MAC_REG_R32(R_AX_SPS_ANALDO_ON_CTRL);
				val32 = SET_CLR_WORD(val32, 0xA, B_AX_VOL_ANALDO);
				MAC_REG_W32(R_AX_SPS_ANALDO_ON_CTRL, val32);
				/* set 0x200 [3:0]=0x9 */
				val32 = MAC_REG_R32(R_AX_SPS_DIG_ON_CTRL0);
				val32 = SET_CLR_WORD(val32, 0x9, B_AX_VOL_L1);
				MAC_REG_W32(R_AX_SPS_DIG_ON_CTRL0, val32);
				/* set 0x220 [3:0]=0x8 */
				val32 = MAC_REG_R32(R_AX_SPS_ANA_ON_CTRL0);
				val32 = SET_CLR_WORD(val32, 0x8, B_AX_VOL_L1_ANA);
				MAC_REG_W32(R_AX_SPS_ANA_ON_CTRL0, val32);
			} else {
				/* 0xF0[9]=1 set 0x220 [3:0]=0xA */
				val32 = MAC_REG_R32(R_AX_SPS_ANA_ON_CTRL0);
		...
		/* IF 0x5EA=0xFE for SMIC*/
		if (process_chk_value == PROCESS_CHK_SMIC) {
			if ((MAC_REG_R32(R_AX_SYS_CFG1) & B_AX_OPT1_ADIE) != B_AX_OPT1_ADIE) {
				/* 0xF0[9]=0 set 0x238 [5:2]=0xA */
				val32 = MAC_REG_R32(R_AX_SPS_ANALDO_ON_CTRL);
				val32 = SET_CLR_WORD(val32, 0xA, B_AX_VOL_ANALDO);
				...
				/* set 0x200 [3:0]=0x8 */
				val32 = MAC_REG_R32(R_AX_SPS_DIG_ON_CTRL0);
				val32 = SET_CLR_WORD(val32, 0x8, B_AX_VOL_L1);
				MAC_REG_W32(R_AX_SPS_DIG_ON_CTRL0, val32);
				/* set 0x220 [3:0]=0x7 */
				val32 = MAC_REG_R32(R_AX_SPS_ANA_ON_CTRL0);
				val32 = SET_CLR_WORD(val32, 0x7, B_AX_VOL_L1_ANA);
				MAC_REG_W32(R_AX_SPS_ANA_ON_CTRL0, val32);
			} else {
				/* 0xF0[9]=1 set 0x220 [3:0]=0x9 */
				val32 = MAC_REG_R32(R_AX_SPS_ANA_ON_CTRL0);
				val32 = SET_CLR_WORD(val32, 0x9, B_AX_VOL_L1_ANA);
				MAC_REG_W32(R_AX_SPS_ANA_ON_CTRL0, val32);
				/* set 0x200 [3:0]=0x8 */
				val32 = MAC_REG_R32(R_AX_SPS_DIG_ON_CTRL0);
				val32 = SET_CLR_WORD(val32, 0x8, B_AX_VOL_L1);
...
u32 mac_pwr_on_usb_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
...
u32 mac_pwr_on_ap_pcie_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
...
u32 mac_pwr_on_nic_pcie_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
...
u32 mac_pwr_off_sdio_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
...
u32 mac_pwr_off_usb_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
...
u32 mac_pwr_off_ap_pcie_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
...
u32 mac_pwr_off_nic_pcie_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
...
u32 mac_enter_lps_sdio_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
	...
	MAC_REG_W32(R_AX_FWD1IMR_V1, val32 | B_AX_FS_RPWM_INT_EN);
	...
	/* 0x90[31:0] = 0x04_80_A1_B2 */
	MAC_REG_W32(R_AX_WLLPS_CTRL, 0x0480A1B2);
...
u32 mac_enter_lps_usb_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
	...
	MAC_REG_W32(R_AX_WLLPS_CTRL, 0x0481A1B2);
...
u32 mac_enter_lps_pcie_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 ret;
...
u32 mac_leave_lps_sdio_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u8 val8;
...
u32 mac_leave_lps_usb_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u8 val8;
...
u32 mac_leave_lps_pcie_8192xb(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u8 val8;
