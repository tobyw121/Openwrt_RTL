 ...
 */
static enum rtw_phl_status
_phl_set_macid_pkt_drop(struct phl_info_t *phl_info, u16 macid, u8 sel,
					 	u8 port, u8 mbssid)
{
	enum rtw_phl_status phl_sts = RTW_PHL_STATUS_FAILURE;

	if(RTW_HAL_STATUS_SUCCESS ==
		rtw_hal_set_macid_pkt_drop(phl_info->hal, macid, sel, port, mbssid))
		phl_sts = RTW_PHL_STATUS_SUCCESS;
...
phl_cmd_set_macid_pkt_drop_hdl(struct phl_info_t *phl_info, u8 *param)
{
	...
	return _phl_set_macid_pkt_drop(phl_info, p->phl_sta->macid, p->sel,
				p->phl_sta->wrole->hw_port, p->phl_sta->wrole->hw_mbssid);
}
                      ...
                      u32 cmd_timeout)
{
	...
	if (cmd_type == PHL_CMD_DIRECTLY) {
		psts = _phl_set_macid_pkt_drop(phl_info, phl_sta->macid, sel,
					wifi_role->hw_port, wifi_role->hw_mbssid);
		goto _exit;
	}
                  ...
                  struct rtw_phl_stainfo_t *phl_sta)
{
	...
	phl_sta->grp_info.tx_grp_idx = 0xff;
	phl_sta->grp_info.tx_grp_idx_old = 0xff;
	phl_sta->grp_info.rx_grp_idx = 0xff;
	phl_sta->grp_info.rx_grp_idx_old = 0xff;
	#endif
...
phl_free_stainfo_sw(struct phl_info_t *phl_info, struct rtw_phl_stainfo_t *sta)
{
	...
	rtw_phl_free_rx_reorder(phl_info, sta);

	#ifdef CONFIG_WFA_OFDMA_Logo_Test
	rtw_phl_free_ru_sta(phl_info, sta);
	#endif
					...
					struct rtw_phl_stainfo_t *sta)
{
	...
	/* macid drop packet when free phl sta by type */
	if (wrole) {
		rtw_phl_cmd_set_macid_pkt_drop(wrole, sta, 0, PHL_CMD_DIRECTLY, 0);
		rtw_phl_cmd_set_macid_pkt_drop(wrole, sta, 1, PHL_CMD_DIRECTLY, 0);
		rtw_phl_cmd_set_macid_pkt_drop(wrole, sta, 2, PHL_CMD_DIRECTLY, 0);
		rtw_phl_cmd_set_macid_pkt_drop(wrole, sta, 3, PHL_CMD_DIRECTLY, 0);
	}
...
extern enum rtw_hal_status
_hal_set_dctrl_tbl_seq(void *hal,
                 struct rtw_phl_stainfo_t *sta, u32 seq);  // cliff_test

enum rtw_phl_status
_phl_set_dctrl_tbl_seq(void *phl, struct rtw_phl_stainfo_t *sta, u32 seq)
{
       struct phl_info_t *phl_info = (struct phl_info_t *)phl;

       return _hal_set_dctrl_tbl_seq(phl_info->hal, sta, seq);
}
#endif
			...
			u8 *sta_addr, bool is_connect)
{
	...
	if (is_connect == true && is_sta_linked == true) {
		PHL_WARN("%s STA (MAC_ID:%d) had connected\n", __func__, sta->macid);
		goto _exit;
	}
	if (is_connect == false && is_sta_linked == false) {
		/* handle connect abort case */
		if (wrole->mstate == MLME_LINKING) {
			PHL_WARN("%s MAC_ID(%d) connect abort\n", __func__, sta->macid);
			pstatus = RTW_PHL_STATUS_SUCCESS;
		} else {
			PHL_WARN("%s MAC_ID(%d) had disconnected\n", __func__, sta->macid);
		}
	...
	if (hstatus != RTW_HAL_STATUS_SUCCESS) {
		PHL_WARN("rtw_hal_update_sta_entry failure!\n");
		goto _exit;
	}
	...
	if (wrole->type == PHL_RTYPE_STATION
		...
		if (hstatus != RTW_HAL_STATUS_SUCCESS) {
			PHL_WARN("rtw_hal_role_cfg failure!\n");
			goto _exit;
		}
	}
	...
	struct rtw_phl_stainfo_t *sta, enum sta_chg_id chg_id, u8 *chg_info, u8 chg_info_len)
{
	...
	case STA_CHG_RAMASK:
	{
		...
		break;
	case STA_CHG_VCS:
		PHL_INFO("%s MACID:%d %02x:%02x:%02x:%02x:%02x:%02x update VCS\n",
				__func__, sta->macid,
				sta->mac_addr[0], sta->mac_addr[1], sta->mac_addr[2],
				sta->mac_addr[3], sta->mac_addr[4], sta->mac_addr[5]);
		break;
	...
	case STA_CHG_MBSSID:
		...
		break;
	case STA_CHG_AGG_NUM: {
		u16 new_num_ampdu = *(u16 *)chg_info;

		if (new_num_ampdu == sta->asoc_cap.num_ampdu)
			return RTW_PHL_STATUS_SUCCESS;

		sta->asoc_cap.num_ampdu = new_num_ampdu;
		PHL_INFO("%s MACID:%02d AGG_NUM:%u\n",
			  __func__, sta->macid,
			  sta->asoc_cap.num_ampdu);
		}
#ifdef CONFIG_VW_REFINE
		if ( 0 != phl_info->rate ) {
			  u32 addr = 0x40000, i = 0;
			  u32 val0 =0, val1 = 0, val2 = 0;

			  i = sta->macid * 0x20;
			  addr += i;

			  rtw_phl_write32(phl_info, 0xc04, 0x18840000);
			  rtw_phl_write16(phl_info, addr, phl_info->rate);

			  val0 = rtw_phl_read32(phl_info, addr );
			  val1 = rtw_phl_read32(phl_info, (addr + 0x08) );
			  val2 = rtw_phl_read32(phl_info, (addr + 0x0C) );


			  PHL_PRINT("addr:%x macid:%d rate:%x mlen:%d mtime:%d val0:%x val8:%x val12:%x\n", \
				  addr, sta->macid, phl_info->rate, phl_info->max_len, phl_info->max_time, \
				  val0, val1, val2);
		}
#endif
		return rtw_hal_update_cctrl_tbl(phl_info->hal, sta);
	case STA_CHG_MAX:
...
phl_sta_trx_tfc_upd(struct phl_info_t *phl_info)
{
	...
	u16 max_macid_num = 0;
	u16 mid = 0;
	u16 macid0 = 0xffff, macid1 = 0xffff;
	...
	for(mid = 0; mid < max_macid_num; mid++) {
		...
		if (phl_sta && rtw_hal_is_sta_linked(phl_info->hal, phl_sta)) {
			if (macid0 == 0xffff)
				macid0 = phl_sta->macid;
			else
				macid1 = phl_sta->macid;
		}

		if (macid0 != 0xffff && macid1 != 0xffff) {
			rtw_phl_txsts_rpt_config(phl_info, macid0, macid1);
			macid0 = macid1 = 0xffff;
		}
		#endif
 ...
 * @macid: macid
 */
__IMEM_WLAN_SECTION__ struct rtw_phl_stainfo_t *
...
u8
rtw_phl_get_frame_rssi(void *phl, u8 *mac, u8 ppdu_cnt)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	...
	if (_os_mem_cmp(phl_to_drvpriv(phl_info),
			mac, ppdu_info->sts_ent[0][ppdu_cnt].ta, MAC_ALEN) == 0)
	{
		rssi = ppdu_info->sts_ent[0][ppdu_cnt].phy_info.rssi;
	}
...
rtw_phl_txsts_rpt_config(void *phl, u16 macid0, u16 macid1)
{
	...
	if (RTW_HAL_STATUS_SUCCESS == rtw_hal_query_txsts_rpt(phl_info->hal, macid0, macid1))
		phl_sts = RTW_PHL_STATUS_SUCCESS;
...
#ifdef CONFIG_CMD_DISP
	if (cmd_type == PHL_CMD_DIRECTLY) {
		sts = phl_cmd_set_seciv_hdl(phl_info, (u8 *)sta);
		goto _exit;
	}
