...
#define WLAN_MAX_VENDOR_IE_NUM 35
...
#define WIFI_PROBEREQ_RX_VENDOR_IE_BIT BIT(7)
#ifdef CONFIG_P2P
#define WIFI_P2P_PROBEREQ_VENDOR_IE_BIT BIT(5)
#define WIFI_P2P_PROBERESP_VENDOR_IE_BIT BIT(6)
#define WLAN_MAX_VENDOR_IE_MASK_MAX 8
#else
#define WLAN_MAX_VENDOR_IE_MASK_MAX 6
#endif

enum WIFI_PROCESS_VSIE_TYPE {
	WIFI_VSIE_DEL,
	WIFI_VSIE_ADD,
	WIFI_VSIE_DELALL,
};
...
#define WIFI_EVENT_WITH_INFO	20
#define WIFI_EZMESH_ARP_LOOP	25
#define WIFI_SA_EQUAL_BR0_MAC	30
#endif
...
#define IS_1905_MCAST(mac)  ((mac[0] == 0x01)&&(mac[1] == 0x80)&&(mac[2] == 0xc2)&&(mac[3] == 0x00)&&(mac[4] == 0x00)&&(mac[5] == 0x13))
...
#define _RSN_EX_IE_				244
#define _SSN_IE_1_					221
#define _ERPINFO_IE_			42
#define _EXT_SUPPORTEDRATES_IE_	50
#define _EXT_SUPPORTEDRATES_H2E_ 0xFB
...
#if defined(PLATFORM_LINUX) || defined(PLATFORM_FREEBSD) || defined(PLATFORM_ECOS)
...
struct rtw_ieee80211_vht_mcs_info {
...
enum rtw_ieee80211_vht_chanwidth {
...
struct rtw_ieee80211_vht_operation {
	unsigned char chan_width;
	unsigned char center_freq_seg0_idx;
	unsigned char center_freq_seg1_idx;
	unsigned short basic_mcs_set;
} __attribute__((packed));
...
/* 802.11n VHT capabilities masks */
#define IEEE80211_VHT_CAP_SGI_80		0x0020
#define IEEE80211_VHT_CAP_SGI_160		0x0040
#define IEEE80211_VHT_CAP_SUPPORTED_CHANNEL_WIDTH		0x000C
...
/* #endif */
/*      ===============HTC A Control Section=============== */
#define ACONTROL_MASK  0x03
#define HTC_MASK       0x01

#define ACONTROL_ID_MASK 0x3C
#define TRS_ID  0
#define OM_ID   1
#define HAL_ID  2
#define BSR_ID  3
...
#define OM_RX_SS_MASK     0x1C0
#define OM_CH_WIDTH_MASK  0x600
#define OM_UL_MU_DISABLE  0x800
#define OM_TX_SS_MASK     0x7000
/*	===============WPS Section=============== */
/*	For WPSv1.0 */
...
#define ICMPV6_MCAST_MAC(mac)	((mac[0] == 0x33) && (mac[1] == 0x33))
...
#endif
#define PKT_ETH_TYPE(pskb) (*(u16*)(pskb->data+(ETH_ALEN<<1)))

/*-----------------------------------------------------------------------------
				Below is for Multi-AP related definition
------------------------------------------------------------------------------*/
#ifdef CONFIG_RTW_MULTI_AP
#define MAP_IE_TYPE (0x1b)
#define MAP_PROFILE_ONE (1)
#define MAP_PROFILE_TWO (2)
#define MAP_PROFILE_THREE (3)
#define GET_MAP_PROFILE(padapter) (GET_PRIMARY_ADAPTER(padapter)->registrypriv.wifi_mib.multiap_profile)
#define GET_MAP_BSS_TYPE(padapter) (padapter->registrypriv.wifi_mib.multiap_bss_type)
#ifdef CONFIG_RTW_MULTI_AP_R2
#define ETH_P_1905 (0x893a) // IEEE 1905.1 protocol
#define GET_MAP_VLAN_ENABLE(padapter) (GET_PRIMARY_ADAPTER(padapter)->registrypriv.wifi_mib.multiap_vlan_enable)
#define GET_MAP_VLAN_ID(padapter) (padapter->registrypriv.wifi_mib.multiap_vlan_id)
#endif
#endif

#if defined(CONFIG_RTW_MULTI_AP_R2) || defined(CONFIG_RTL_VLAN_8021Q)
#define ETH_P_8021X (0x888e) // IEEE 802.1X protocol
#define EAP_TYPE_KEY (0x3)
__inline static u8 is_eapol_key_pkt(struct sk_buff *pskb, u8 vlan_tagged)
{
	if(!pskb || !pskb->data)
		return 0;
	...
	if(vlan_tagged)
	{
		// the offset of type in a EAP packet is 1 byte
		if((*(u16*)(pskb->data+((ETH_ALEN<<1)+VLAN_TAG_LEN)) == __constant_htons(ETH_P_8021X))
		&& ((*(u8*)(pskb->data+ETH_HLEN+VLAN_TAG_LEN+1)) == EAP_TYPE_KEY))
			return 1;
	}
	else
	{
		if((*(u16*)(pskb->data+((ETH_ALEN<<1))) == __constant_htons(ETH_P_8021X))
		&& ((*(u8*)(pskb->data+ETH_HLEN+1)) == EAP_TYPE_KEY))
			return 1;
	}
