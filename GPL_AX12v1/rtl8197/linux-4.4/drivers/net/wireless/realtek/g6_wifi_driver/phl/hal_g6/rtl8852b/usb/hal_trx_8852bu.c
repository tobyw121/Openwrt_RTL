 ...
 */
enum rtw_hal_status hal_trx_init_8852bu(struct hal_info_t *hal)
{
...
/**
 * the function will deinitializing 8852bu specific data and hw configuration
 */
static void hal_trx_deinit_8852bu(struct hal_info_t *hal)
{
}
...
static enum rtw_hal_status hal_query_info_8852bu(struct hal_info_t *hal, u8 info_id, void *value)
{
...
static enum rtw_hal_status hal_pltfm_tx_8852bu(void *hal,
...
u8 hal_get_bulkout_id_8852bu(struct hal_info_t *hal, u8 dma_ch, u8 mode)
{
	return hal_mac_get_bulkout_id(hal, dma_ch, mode);
}

u8 hal_get_max_bulkout_wd_num_8852bu(struct hal_info_t *hal)
{
...
u16 hal_handle_rx_report_8852bu(struct hal_info_t *hal, u8 *rp,
	...
	do {
		...
		*ac_queue = qsel_value % RTW_MAX_AC_QUEUE_NUM;
		...
		if (TX_STATUS_TX_DONE != *txsts) {
			...
			_hal_show_tx_failure_rsn_8852bu(*txsts);
...
u8 hal_get_fwcmd_queue_idx_8852bu(void)
{
	return FWCMD_QUEUE_IDX_8852B;
}

static struct hal_trx_ops ops= {
	.init = hal_trx_init_8852bu,
