...
_phl_realloc_hw_port(struct phl_info_t *phl,
			struct rtw_wifi_role_t *wifi_role, u8 new_port)
{
	struct rtw_phl_com_t *phl_com = phl->phl_com;
	struct mr_ctl_t *mr_ctl = phlcom_to_mr_ctrl(phl_com);
	struct hw_band_ctl_t *band_ctrl;
...
static enum rtw_phl_status
_phl_alloc_wrhw_resource(struct phl_info_t *phl,
...
#endif
	{/* for AP/STA use different HW WMM*/
		/*alloc hw_wmm*/
...
static enum rtw_phl_status
_phl_free_wrhw_resource(struct phl_info_t *phl,
			struct rtw_wifi_role_t *wrole)
{
	...
	/*release hw_port*/
	#ifdef CONFIG_RTW_SUPPORT_MBSSID_VAP
	if (wrole->hw_mbssid != 0) { /* vap */
		PHL_INFO("Free MBSSID %u.\n", wrole->hw_mbssid);
		band_ctrl->mbssid_cnt--;
		band_ctrl->mbssid_map &= ~BIT(wrole->hw_mbssid);
		wrole->hw_mbssid = 0;
	}
	else
	#endif /* CONFIG_RTW_SUPPORT_MBSSID_VAP */
...
static enum rtw_phl_status
_phl_realloc_wrhw_resource(struct phl_info_t *phl,
					struct rtw_wifi_role_t *wrole, u8 new_band)
{
	...
	psts = _phl_free_wrhw_resource(phl, wrole);
	if (psts != RTW_PHL_STATUS_SUCCESS)
		goto _exit;

	psts = _phl_alloc_wrhw_resource(phl, wrole, new_band);
	if (psts != RTW_PHL_STATUS_SUCCESS)
		goto _exit;
				...
				u8 *mac_addr, enum role_type type, u8 ridx)
{
	...
	_os_spinlock(drv_priv, &mr_ctl->lock, _bh, NULL);
	...
	_os_spinunlock(drv_priv, &mr_ctl->lock, _bh, NULL);
	...
	_os_atomic_set(drv_priv, &role->hiq_busy_cnt, 0);
	role->last_hiq_wp_time = 0;
	role->dropping_hiq = 0;
	#endif /* RTW_WKARD_HIQ_STUCK */

#ifdef CONFIG_DBCC_FORCE
	if ((phl_com->dev_cap.dbcc_sup == true) &&
		(phl_com->dev_cap.dbcc_force_rid > 0)) {
		if (role->id == phl_com->dev_cap.dbcc_force_rid) {
			hw_band = HW_BAND_1;
		}
	}
#endif
	...
	if (_phl_alloc_wrhw_resource(phl_info, role, hw_band) != RTW_PHL_STATUS_SUCCESS) {
	...
	_os_mem_cpy(drv_priv, role->mac_addr, mac_addr, MAC_ALEN);
...
_phl_wifi_role_free_sw(struct phl_info_t *phl_info, struct rtw_wifi_role_t *wrole)
{
	...
	_phl_free_wrhw_resource(phl_info, wrole);
...
phl_cmd_get_cur_tsf_hdl(struct phl_info_t *phl_info, struct rtw_phl_port_tsf *tsf)
{
	...
	hstatus = rtw_hal_get_tsf(phl_info->hal, tsf->port, &tsf->tsf_h, &tsf->tsf_l);
...
phl_wifi_role_realloc_band(struct phl_info_t *phl_info,
			struct rtw_wifi_role_t *wrole, enum phl_band_idx new_band)
{
	enum rtw_phl_status psts = RTW_PHL_STATUS_FAILURE;
	enum rtw_hal_status hsts = RTW_HAL_STATUS_FAILURE;
	...
	/*disable hw port*/
	hsts = rtw_hal_role_cfg_ex(phl_info->hal, wrole, PCFG_FUNC_SW, &func_en);
	if(hsts != RTW_HAL_STATUS_SUCCESS) {
	...
	psts = _phl_realloc_wrhw_resource(phl_info, wrole, new_band);
	if (psts != RTW_PHL_STATUS_SUCCESS)
		goto _exit;
	/*phl_deregister_tx_ring - phl_sta*/
	/*phl_register_tx_ring - phl_sta*/
...
enum rtw_phl_status
rtw_phl_wifi_role_realloc_band(void *phl, struct rtw_wifi_role_t *wrole)
{
	enum phl_band_idx new_band;

	if (wrole->hw_band == HW_BAND_0)
		new_band = HW_BAND_1;
	else
		new_band = HW_BAND_0;
	phl_wifi_role_realloc_band((struct phl_info_t *)phl, wrole, new_band);
			...
			struct rtw_wifi_role_t *wrole, u8 new_port)
{
	...
	/*release hw port*/
	psts = _phl_release_hw_port(phl_info, wrole);
	if (psts != RTW_PHL_STATUS_SUCCESS) {
	...
	psts = _phl_realloc_hw_port(phl_info, wrole, new_port);
	if (psts != RTW_PHL_STATUS_SUCCESS) {
...
rtw_phl_wifi_role_realloc_port(void *phl,
		struct rtw_wifi_role_t *wrole, u8 new_port)
{
	return phl_wifi_role_realloc_port((struct phl_info_t *)phl, wrole, new_port);
}


enum rtw_phl_status
phl_wifi_role_realloc_wmm(struct phl_info_t *phl_info,
			struct rtw_wifi_role_t *wrole, u8 new_wmm)
{
	enum rtw_phl_status psts = RTW_PHL_STATUS_FAILURE;
	if (new_wmm == wrole->hw_wmm) {
		goto _exit;
	}
	psts = RTW_PHL_STATUS_SUCCESS;
				...
				enum wr_chg_id chg_id, void *chg_info)
{
	...
	case WR_CHG_TYPE:
	{
		...
		#ifdef CONFIG_RTW_SUPPORT_MBSSID_VAP
		/* VAP does not own a port. Only disable M-BSSID for VAPs
		 * or the root AP could be affected.
		 */
		if (wrole->type == PHL_RTYPE_VAP) {
			wrole->type = type;
	...
	case WR_CHG_AP_PARAM:
	{
		...
		else if (param->cfg_id == CFG_BCN_TX_EN) {
			u32 bcn_tx_en = param->value;

			rtw_hal_role_cfg_ex(phl_info->hal, wrole, PCFG_BCN_EN, &bcn_tx_en);
		}
		else if (param->cfg_id == CFG_BCN_DRP_ALL) {
			u32 bcn_drp_en = param->value;

			rtw_hal_role_cfg_ex(phl_info->hal, wrole, PCFG_BCN_DRP_ALL, &bcn_drp_en);
		}
...
_phl_wifi_role_stop(struct phl_info_t *phl_info, struct rtw_wifi_role_t *wrole)
{
	...
	if (wrole->type == PHL_RTYPE_VAP) {
		...
		rtw_hal_role_cfg_ex(phl_info->hal, wrole, PCFG_MBSSID_EN, &param);
	}
	#endif /* CONFIG_RTW_SUPPORT_MBSSID_VAP */
	...
	enum rf_path tx, enum rf_path rx)
{
	...
	PHL_WARN("Config tx path=%d rx path=%d\n", tx, rx);
	/* reset to default tx rx rfpath */
	rtw_hal_reset_rf_path(phl_info->hal);

	/* config rx path */
	hstatus = rtw_hal_cfg_trx_path(phl_info->hal, tx, (tx < RF_PATH_AB)?1:2,
				rx, (rx < RF_PATH_AB)?1:2);

	/* update tx rx path value and protocol cap */
	phl_com->phy_sw_cap[0].txss = (tx < RF_PATH_AB)?1:2;
	phl_com->phy_sw_cap[0].rxss = (rx < RF_PATH_AC)?1:2;
	rtw_hal_final_cap_decision(phl_com, phl_info->hal);
	phl_init_protocol_cap(phl_info, role);
	/* for those already set in cmac table */
	_os_spinlock(phl_com->drv_priv, &role->assoc_sta_queue.lock, _irq, &sp_flags);
	phl_list_for_loop(sta, struct rtw_phl_stainfo_t, &role->assoc_sta_queue.queue, list) {
		if (sta) {
			if (sta->asoc_cap.nss_rx > phl_com->phy_cap[0].txss)
				sta->asoc_cap.nss_rx = phl_com->phy_cap[0].txss;
			hstatus = rtw_hal_upd_sta_tx(phl_info->hal, sta, tx);
			if (hstatus != RTW_HAL_STATUS_SUCCESS)
				break;
		}
	}
	_os_spinunlock(phl_com->drv_priv, &role->assoc_sta_queue.lock, _irq, &sp_flags);
...
bool rtw_phl_role_is_ap_category(struct rtw_wifi_role_t *wrole)
{
	bool ret = false;

	if (IS_AP_ROLE_TYPE(wrole->type) || wrole->type == PHL_RTYPE_P2P_GO)
		ret = true;
	return ret;
}
