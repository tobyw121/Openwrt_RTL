...
void halbb_set_collision_thre(struct bb_info *bb)
{
	...
	halbb_set_reg(bb, cr->collision_r2t_th, cr->collision_r2t_th_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs0,
		      cr->collision_t2r_th_mcs0_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs1,
		      cr->collision_t2r_th_mcs1_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs2,
		      cr->collision_t2r_th_mcs2_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs3,
		      cr->collision_t2r_th_mcs3_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs4,
		      cr->collision_t2r_th_mcs4_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs5,
		      cr->collision_t2r_th_mcs5_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs6,
		      cr->collision_t2r_th_mcs6_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs7,
		      cr->collision_t2r_th_mcs7_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs8,
		      cr->collision_t2r_th_mcs8_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs9,
		      cr->collision_t2r_th_mcs9_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs10,
		      cr->collision_t2r_th_mcs10_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_mcs11,
		      cr->collision_t2r_th_mcs11_m, th);
	halbb_set_reg(bb, cr->collision_t2r_th_cck,
...
void halbb_set_edcca_thre(struct bb_info *bb)
{
	...
	halbb_set_reg(bb, cr->r_ppdu_level, cr->r_ppdu_level_m, l2h);
	...
	if ((bb->ic_type == BB_RTL8852C) || (bb->ic_sub_type == BB_IC_SUB_TYPE_8852B_8852BP)) {
		if (bb_edcca->edcca_mode == EDCCA_CBP_MODE && band == BAND_ON_6G)
			halbb_set_reg(bb, cr->r_obss_level, cr->r_obss_level_m, l2h);
	}
#endif
...
void halbb_edcca_log(struct bb_info *bb)
{
	...
	BB_DBG(bb, DBG_EDCCA,
	       ...
	       bb->dbg_buf, ext_loss[0], ext_loss[1], bb_edcca->pwrofst - 16);
#else
			      ...
			      char *output, u32 *_out_len)
{
	...
	if ((_os_strcmp(input[1], help) == 0)) {
		...
		BB_DBG(bb, DBG_FW_INFO, "[LPS] i-only cmd\n");
		HALBB_SCAN(input[2], DCMD_DECIMAL, &var[1]);
