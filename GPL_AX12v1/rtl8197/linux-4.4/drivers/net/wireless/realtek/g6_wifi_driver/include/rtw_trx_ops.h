...
static inline s32 rtw_intf_xmitframe_enqueue(_adapter *adapter,
						struct xmit_frame *pxmitframe)
{
	return adapter_to_dvobj(adapter)->intf_ops->xmitframe_enqueue(adapter, pxmitframe);
}
...
/************************ recv *******************/
static inline s32 rtw_intf_init_recv_priv(_adapter *adapter)
{
	return adapter_to_dvobj(adapter)->intf_ops->init_recv_priv(adapter);
}
static inline void rtw_intf_free_recv_priv(_adapter *adapter)
{
	return adapter_to_dvobj(adapter)->intf_ops->free_recv_priv(adapter);
}
