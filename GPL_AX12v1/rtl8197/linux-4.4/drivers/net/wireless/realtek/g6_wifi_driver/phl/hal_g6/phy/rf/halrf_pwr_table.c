	...
	enum phl_pwr_table pwr_table)
{
	...
	if (hal_com->chip_id == CHIP_WIFI6_8832CRVU) {
		halrf_set_power_8832crvu(rf, phy, pwr_table);
	}
#endif

#ifdef RF_8832BRVT_SUPPORT
	if (hal_com->chip_id == CHIP_WIFI6_8832BRVT) {
		halrf_set_power_8832brvt(rf, phy, pwr_table);
	}
#endif
...
u8 halrf_get_power_limit_extra(struct rf_info *rf)
{
	...
	if (pwr->regulation_force_en == true || pwrlmt_info->para_src == 0) {
		RF_DBG(rf, DBG_RF_POWER, "======>%s   pwr->regulation_force_en=%d   is_form_folder=%d   Return!!!\n",
			__func__, pwr->regulation_force_en, pwrlmt_info->para_src);
