 ...
 *
 *****************************************************************************/
#define _HAL_TRX_8192XB_C_
#include "../hal_headers.h"
#include "rtl8192xb_hal.h"

static void _hal_trx_8192xb_dump_rxcnt(struct hal_ppdu_sts *ppdu_sts)
{
...
static void _hal_dump_rxdesc(u8 *buf, struct rtw_r_meta_data *mdata)
{
 ...
 hal_handle_chaninfo_8192xb(struct hal_info_t *hal)
 {
...
#define WL_SEQ_MASK	0xfff
 ...
 /**
 * SW Parsing Rx Desc - hal_parsing_rx_wd_8192xb
 * description:
 ...
 */
static enum rtw_hal_status
_hal_parsing_rx_wd_8192xb(struct hal_info_t *hal,
#ifdef PHL_RXSC_AMPDU
	...
	u8 *desc, struct rtw_r_meta_data *mdata)
{
	...
	mdata->phy_rpt_en = GET_RX_AX_DESC_PHY_RPT_EN_8192XB(desc);

	if ((mdata->rpkt_type == RX_8192XB_DESC_PKT_T_WIFI)
		|| (mdata->rpkt_type == RX_8192XB_DESC_PKT_T_PPDU_STATUS)) {
		...
		while (mdata->rpkt_type == RX_8192XB_DESC_PKT_T_WIFI) {
			...
			mdata->hdr_conv = GET_RX_AX_DESC_HDR_CNV_8192XB(desc);

			#ifdef PHL_RXSC_AMPDU
			if (RXSC_INVALID_MACID == phl_com->rxsc_entry.cached_rx_macid)
				break;

			mdata->tid = GET_RX_AX_DESC_TID_8192XB(desc);
			mdata->macid = GET_RX_AX_DESC_MACID_8192XB(desc);
			mdata->seq = GET_RX_AX_DESC_SEQ_8192XB(desc);

			mdata->mc = GET_RX_AX_DESC_MC_8192XB(desc);
			mdata->bc = GET_RX_AX_DESC_BC_8192XB(desc);
			mdata->htc = GET_RX_AX_DESC_HTC_8192XB(desc);
			mdata->frame_type = GET_RX_AX_DESC_TYPE_8192XB(desc);
			...
			if (phl_com->rxsc_entry.cached_rx_macid == mdata->macid &&
				...
				mdata->qos = GET_RX_AX_DESC_QOS_8192XB(desc);
				mdata->addr_cam_vld = GET_RX_AX_DESC_CAM_VLD_8192XB(desc);
		...
		mdata->get_ch_info = GET_RX_AX_DESC_CH_INFO_8192XB(desc);
		mdata->rx_statistics = GET_RX_AX_DESC_STATISTICS_8192XB(desc);

		mdata->pattern_idx = GET_RX_AX_DESC_PATTERN_IDX_8192XB(desc);
		mdata->target_idc = GET_RX_AX_DESC_TARGET_IDC_8192XB(desc);
		mdata->chksum_ofld_en = GET_RX_AX_DESC_CHKSUM_OFFLOAD_8192XB(desc);
		mdata->with_llc = GET_RX_AX_DESC_WITH_LLC_8192XB(desc);

		if (mdata->long_rxd == 1) {
			mdata->frame_type = GET_RX_AX_DESC_TYPE_8192XB(desc);
			mdata->mc = GET_RX_AX_DESC_MC_8192XB(desc);
			mdata->bc = GET_RX_AX_DESC_BC_8192XB(desc);
			mdata->more_data = GET_RX_AX_DESC_MD_8192XB(desc);
			mdata->more_frag = GET_RX_AX_DESC_MF_8192XB(desc);
			mdata->pwr_bit = GET_RX_AX_DESC_PWR_8192XB(desc);
			mdata->qos = GET_RX_AX_DESC_QOS_8192XB(desc);

			mdata->tid = GET_RX_AX_DESC_TID_8192XB(desc);
			mdata->eosp = GET_RX_AX_DESC_EOSP_8192XB(desc);
			mdata->htc = GET_RX_AX_DESC_HTC_8192XB(desc);
			mdata->q_null = GET_RX_AX_DESC_QNULL_8192XB(desc);

			mdata->seq = GET_RX_AX_DESC_SEQ_8192XB(desc);
			mdata->frag_num = GET_RX_AX_DESC_FRAG_8192XB(desc);

			mdata->sec_cam_idx = GET_RX_AX_DESC_CAM_IDX_8192XB(desc);
			mdata->addr_cam = GET_RX_AX_DESC_ADDR_CAM_8192XB(desc);
			...
			mdata->rx_pl_id = GET_RX_AX_DESC_PL_ID_8192XB(desc);

			mdata->addr_fwd_en = GET_RX_AX_DESC_FWD_EN_8192XB(desc);
			mdata->rx_pl_match = GET_RX_AX_DESC_PL_MATCH_8192XB(desc);

			_os_mem_cpy(hal_com->drv_priv,
				(void*)&mdata->mac_addr, (void*)(desc + 24), MAC_ALEN);
			mdata->sec_type = GET_RX_AX_DESC_SEC_TYPE_8192XB(desc);
		}
	}
...
enum rtw_hal_status
hal_parsing_rx_wd_8192xb(struct rtw_phl_com_t *phl_com,
				...
				struct rtw_r_meta_data *mdata)
{
	...
	do {
		...
		if (!halmac_rx) {
#ifdef PHL_RXSC_AMPDU
			hstatus = _hal_parsing_rx_wd_8192xb(hal, phl_com, desc, mdata);
#else
			hstatus = _hal_parsing_rx_wd_8192xb(hal, desc, mdata);
#endif
		} else {
		...
		desc_l = mdata->long_rxd ? RX_DESC_L_SIZE_8192XB :
						RX_DESC_S_SIZE_8192XB;
		...
		shift = (u8)((mdata->shift * 2)
			     + (mdata->drv_info_size * RX_DESC_DRV_INFO_UNIT_8192XB)
			     + desc_l);

		if ((1 == mdata->mac_info_vld) &&
			(RX_8192XB_DESC_PKT_T_PPDU_STATUS != mdata->rpkt_type))
			*pkt = desc + shift + RX_PPDU_MAC_INFO_SIZE_8192XB;
		else
			*pkt = desc + shift;
...
static void
_hal_rx_wlanhdr_check_8192xb(void *drvpriv, void *hdr, struct rtw_r_meta_data *mdata)
{
	/*Check Retry BIT*/
	u8 retry = 0;
	u16 seq_num = 0;
...
static void
_hal_rx_sts_8192xb(struct hal_info_t *hal, struct rtw_r_meta_data *meta)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static void
_hal_rx_ampdu_cnt_8192xb(struct hal_info_t *hal, struct rtw_r_meta_data *mdata)
{
...
void
_hal_rx_ppdu_sts_normal_data_8192xb(struct rtw_phl_com_t *phl_com,
				   struct hal_info_t *hal,
				   void *hdr,
...
void
_hal_rx_ppdu_sts_8192xb(struct rtw_phl_com_t *phl_com,
			    struct rtw_r_meta_data *meta,
...
static inline void
_hal_parsing_rx_drvinfo_8192xb(struct hal_info_t *hal,
                               ...
                               u8 *drv_info)
{
	...
	phl_rx->r.phy_info.signal_strength =
			rtw_hal_bb_drv_info_dbm2rssi(hal,
			                             GET_RX_AX_DRV_INFO_RX_PWR_DBM_8192XB(drv_info),
			                             r->mdata.bw);
	phl_rx->r.phy_info.ch_idx =  GET_RX_AX_DRV_INFO_CENTRAL_CH_8192XB(drv_info);
	...
	#endif
	phl_rx->r.phy_info.rxsc =  GET_RX_AX_DRV_INFO_RX_SC_8192XB(drv_info);
	phl_rx->r.phy_info.is_drvinfo_vld = true;

	PHL_TRACE(COMP_PHL_PSTS, _PHL_DEBUG_,
		  "[8192XB][DrvInfo] DBM %d(signal_strength %d), RxSC %d, Central %d, BW 0x%x\n",
		  GET_RX_AX_DRV_INFO_RX_PWR_DBM_8192XB(drv_info),
		  phl_rx->r.phy_info.signal_strength,
                            ...
                            struct rtw_phl_rx_pkt *phl_rx)
{
	...
	hstatus = hal_parsing_rx_wd_8192xb(phl_com, hal, buf,
	                                   &frame, &frame_len,
	                                   &drv_info, mdata);

#ifdef TP_DEBUG
	_hal_rx_ampdu_cnt_8192xb(hal, mdata);
#endif
	...
	case RX_8192XB_DESC_PKT_T_WIFI :
	{
		void *wlan_hdr;
		...
		if (   (mdata->hdr_conv && phl_com->rx_hdr_conv_dump)
		    || (mdata->amsdu_cut && phl_com->rx_amsdu_cut_dump)) {
			mdata->debug_dump = 1;
			if (phl_com->rx_amsdu_cut_dump)
				phl_com->rx_amsdu_cut_dump--;
			if (phl_com->rx_hdr_conv_dump)
				phl_com->rx_hdr_conv_dump--;
			PHL_PRINT("\nR %c%c (%u/%u)\n",
			          mdata->hdr_conv ? 'H' : ' ',
				  mdata->amsdu_cut ? 'S' : ' ',
				  phl_com->rx_amsdu_cut_dump,
		...
		if (!mdata->hdr_conv) {
			...
			if (mdata->debug_dump) {
				...
				if (rxbuf->frame_len > 96) {
					debug_dump_buf(rxbuf->frame, 64, "MSDU H:");
			...
			u32 hdr_offset = 0;
			u32 drv_info_sz = mdata->drv_info_size * RX_DESC_DRV_INFO_UNIT_8192XB;
			...
			if (mdata->debug_dump) {
				PHL_PRINT("R B%pX D%pX H%pX F%pX L%u\n",
				          buf, drv_info,
				          wlan_hdr, rxbuf->frame, rxbuf->frame_len);
				debug_dump_buf(wlan_hdr, 48, "WHDR:");
...
#endif
		{
			_hal_rx_wlanhdr_check_8192xb(drv, wlan_hdr, mdata);
			_hal_rx_sts_8192xb(hal, mdata);
		...
		if ((1 == mdata->phy_rpt_en) && (0 < mdata->drv_info_size)) {
			...
			_hal_parsing_rx_drvinfo_8192xb(hal, phl_rx, drv_info);
		}
	}
	break;
	case RX_8192XB_DESC_PKT_T_TX_PD_RELEASE_HOST:
	{
		phl_rx->type = RTW_RX_TYPE_TX_WP_RELEASE_HOST;
	}
	break;
	case RX_8192XB_DESC_PKT_T_PPDU_STATUS :
	{
		...
		PHL_TRACE(COMP_PHL_PSTS, _PHL_INFO_,
			  "==> RX_8192XB_DESC_PKT_T_PPDU_STATUS :band %d ; ppdu_cnt  %d ; freerun_cnt %d \n",
		...
		if (ppdu_sts.rx_cnt_size != 0) {
			_hal_trx_8192xb_dump_rxcnt(&ppdu_sts);
		}
		if (ppdu_sts.phy_st_size != 0) {
			if((mdata->ppdu_type == RX_8192XB_DESC_PPDU_T_VHT_MU)||
				(mdata->ppdu_type == RX_8192XB_DESC_PPDU_T_HE_MU)||
				(mdata->ppdu_type == RX_8192XB_DESC_PPDU_T_HE_TB)) {
				is_su = 0;
			}
	...
	break;
	case RX_8192XB_DESC_PKT_T_DFS_RPT :
	{
	...
	break;
	case RX_8192XB_DESC_PKT_T_CHANNEL_INFO :
	{
	...
	break;
	case RX_8192XB_DESC_PKT_T_F2P_TX_CMD_RPT :
	{
		/* DL MU Report ; UL OFDMA Trigger Report */
	}
	break;
	case RX_8192XB_DESC_PKT_T_C2H :
	{
	...
	break;
	case RX_8192XB_DESC_PKT_T_TX_RPT:
	{
                           ...
                           struct rtw_phl_rx_pkt *phl_rx)
{
	...
	rpkt_type = GET_RX_AX_DESC_RPKT_TYPE_8192XB(desc);
	if (   (rpkt_type != RX_8192XB_DESC_PKT_T_WIFI)
	    && (rpkt_type != RX_8192XB_DESC_PKT_T_PPDU_STATUS)) {
		return RTW_HAL_STATUS_INVALID;
	}

	ppdu_cnt = GET_RX_AX_DESC_PPDU_CNT_8192XB(desc);
	seq = GET_RX_AX_DESC_SEQ_8192XB(desc);
	amsdu = GET_RX_AX_DESC_AMSDU_8192XB(desc);
	amsdu_cut = GET_RX_AX_DESC_AMSDU_CUT_8192XB(desc);
	mdata->last_msdu = GET_RX_AX_DESC_LAST_MSDU_8192XB(desc);
	...
	if (   (ppdu_cnt != mdata->ppdu_cnt)
	    || (seq != mdata->seq)
	    || (!amsdu || !amsdu_cut)) {
		PHL_PRINT("R P:%u/%u S:%u/%u A:%u-%u\n",
		          ppdu_cnt, mdata->ppdu_cnt,
		          seq, mdata->seq, amsdu, amsdu_cut);
		return RTW_HAL_STATUS_FAILURE;
	}

	shift = GET_RX_AX_DESC_SHIFT_8192XB(desc);
	mac_info_vld = GET_RX_AX_DESC_MAC_INFO_VLD_8192XB(desc);
	drv_info_size = GET_RX_AX_DESC_DRV_INFO_SIZE_8192XB(desc);

	shift = (u8)(  (shift * 2)
		     + (drv_info_size * RX_DESC_DRV_INFO_UNIT_8192XB)
		     + RX_DESC_L_SIZE_8192XB);

	if (mac_info_vld)
		shift += RX_PPDU_MAC_INFO_SIZE_8192XB;

	rxbuf->frame = desc + shift;
	rxbuf->frame_len = GET_RX_AX_DESC_PKT_LEN_8192XB(desc);
	rxbuf->hdr = NULL;
	rxbuf->hdr_len = 0;

	#ifdef CONFIG_RTW_DEBUG_HW_RX_AMSDU_CUT
	if (mdata->debug_dump) {
		PHL_PRINT("R B@%pX F@%pX L@%u %u %c\n", buf, rxbuf->frame, rxbuf->frame_len,
		          phl_com->rx_stats.amsdu_cut_cur_num,
		          mdata->last_msdu ? 'L' : ' ');
		if (rxbuf->frame_len > 64) {
			debug_dump_buf(rxbuf->frame, 32, "MSDU H:");
			debug_dump_buf(rxbuf->frame + rxbuf->frame_len - 32, 32, "MSDU T:");
