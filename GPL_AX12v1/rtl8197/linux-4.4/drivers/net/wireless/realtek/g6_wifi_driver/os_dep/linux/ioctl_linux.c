			  ...
			  char *extra)
{
	...
	_list *pscanned = padapter->dvobj->pscanned;
#else
	_queue	*queue	= &(pmlmepriv->scanned_queue);
	_list *pscanned = pmlmepriv->pscanned;
#endif
	...
	*/
	...
	pscanned = get_next(phead);

	while (1) {

		if ((rtw_end_of_queue_search(phead, pscanned)) == _TRUE) {
#if 0
			ret = -EINVAL;
			goto cancel_ps_deny;
		...
		pnetwork = LIST_CONTAINOR(pscanned, struct wlan_network, list);

		pscanned = get_next(pscanned);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (adapter_to_pwrctl(padapter)->brfoffbyhw && dev_is_drv_stopped(adapter_to_dvobj(padapter))) {
		ret = -EINVAL;
		goto exit;
	}
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	case 2:
		if (addr & 0x1) {
			RTW_INFO("Unaligned reg_read16(0x%x)\n", addr);
			ret = -EINVAL;
			goto exit;
		} else {
	...
	case 4:
		if (addr & 0x3) {
			RTW_INFO("Unaligned reg_red32(0x%x)\n", addr);
			ret = -EINVAL;
			goto exit;
		} else {
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	path = *(u32 *)extra;
	addr = *((u32 *)extra + 1);
	data32 = rtw_phl_read_rfreg(GET_HAL_INFO(dvobj), path, addr, 0xFFFFF);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	data32 = *((u32 *)extra + 2);
	/*	RTW_INFO("%s: path=%d addr=0x%02x data=0x%05x\n", __func__, path, addr, data32); */
	rtw_phl_write_rfreg(GET_HAL_INFO(dvobj), path, addr, 0xFFFFF, data32);
				   ...
				   union iwreq_data *wrqu, char *extra)
{
	...
	if (_SUCCESS != rtw_set_channel_plan(padapter, channel_plan_req, RTW_CHPLAN_6G_UNSPECIFIED))
		return -EPERM;
...
static void rtw_dbg_mode_hdl(_adapter *padapter, u32 id, u8 *pdata, u32 len)
{
	...
	case GEN_MP_IOCTL_SUBCODE(READ_RF_REG):
		...
		value = rtw_phl_read_rfreg(GET_HAL_INFO(dvobj), path, offset, 0xffffffff);
	...
	case GEN_MP_IOCTL_SUBCODE(WRITE_RF_REG):
		...
		rtw_phl_write_rfreg(GET_HAL_INFO(dvobj), path, offset, 0xffffffff, value);
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	memset(new_ifname, 0, IFNAMSIZ);
	if (copy_from_user(new_ifname, wrqu->data.pointer, IFNAMSIZ - 1))
		return -EFAULT;
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	case 0x74: /* read_rf */
		...
		break;
	case 0x75: /* write_rf */
		rtw_phl_write_rfreg(GET_HAL_INFO(dvobj), minor_cmd, arg, 0xffffffff, extra_arg);
...
int rtw_del_sta(struct net_device *dev, struct ieee_param *param)
{
	int ret = 0;
	struct sta_info *psta = NULL;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
static int rtw_ioctl_acl_add_sta(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	ret = rtw_acl_add_sta(padapter, RTW_ACL_PERIOD_BSS, param->sta_addr, 0);
...
int rtw_vendor_ie_get_data(struct net_device *dev, int vendor_ie_num, char *extra)
{
	...
	if (vendor_ie_mask & WIFI_PROBEREQ_RX_VENDOR_IE_BIT)
		pstring += sprintf(pstring , "[Probe Req Rx]");
#ifdef CONFIG_P2P
...
int rtw_vendor_ie_get(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if ((ret != 1) || (vendor_ie_num > WLAN_MAX_VENDOR_IE_NUM - 1) || (vendor_ie_num<0)) {
		ret = -EFAULT;
		RTW_PRINT("ret:%d, vendor_ie_num:%d\n", ret, vendor_ie_num);
		goto exit;
	}
...
_clear_path:
	_rtw_memset(pmlmepriv->vendor_ie[vendor_ie_num] , 0 , sizeof(pmlmepriv->vendor_ie[vendor_ie_num]));
	pmlmepriv->vendor_ielen[vendor_ie_num] = 0;
	pmlmepriv->vendor_ie_mask[vendor_ie_num] = 0;
	_update_beacon(padapter, 0xFFF, NULL, _TRUE, 0, "clear vendor ie");
	return -EFAULT;
}
#endif

#if defined(CONFIG_RTW_DRV_HAS_NVM) || defined(RTW_FLASH_98D)

int rtw_flash_set_data(struct net_device *dev,
			 struct iw_request_info *info,
	...
	*(extra + wrqu->length) = '\0';
	...
	do {
		...
		u8 key[MAX_STR_LEN+1] = {0};
		u8 value_str[MAX_STR_LEN+1] = {0};

		u8 delim[] = "=";
		u8 *token;

		token = strsep((char**)&extra, delim);
		if (token)
			strncpy(key, token, (strlen(token) > MAX_STR_LEN) ? MAX_STR_LEN : strlen(token));
		token = strsep((char**)&extra, delim);
		if (token)
			strncpy(value_str, token, (strlen(token) > MAX_STR_LEN) ? MAX_STR_LEN : strlen(token));

		ret = rtw_nvm_set_info(dvobj, key, value_str);
	} while (0);
	#else /* CONFIG_RTW_DRV_HAS_NVM */
	rtw_phl_flash_set_info(dvobj->phl, extra);
	#endif /* CONFIG_RTW_DRV_HAS_NVM */
	return ret;
}

int rtw_flash_get_data(struct net_device *dev,
			 struct iw_request_info *info,
			 struct iw_point *wrqu, char *extra){
	//driver write data to flash HW
	...
	struct flash_t *flash_info = hal_info->flash;

	RTW_INFO("call %s\n", __FUNCTION__);

	_rtw_memcpy(extra, flash_info->shadow_map, flash_info->log_efuse_size);
...
int rtw_flash_set_by_name(struct net_device *dev,
			 struct iw_request_info *info,
			 struct iw_point *wrqu, char *extra){
	...
	u8 delim[] = "=";
	u8 *token;
	u8 *dupstr = _os_mem_alloc(dvobj, wrqu->length);
	u32 get_hex;
	...
	*(extra + wrqu->length) = '\0';
	...
	_rtw_memcpy(dupstr, extra, wrqu->length);

	token = _os_strsep((char**)&dupstr, delim);
	//RTW_WARN("orig token = %s\n", token);
	_os_strcpy(cmd, token);
	token = _os_strsep((char**)&dupstr, delim);
	//RTW_WARN("orig token = %s\n", token);
	_os_strcpy(query_value, token);


	if(sscanf(query_value, "%x", &get_hex) != 1){
		RTW_ERR("Invalid input foramt\n");
		return ret;
	}

	RTW_ERR("[debug] did not call rtw_hal_flash_set_info\n");
	//rtw_hal_flash_set_info(hal_info->hal_com, cmd, &get_hex);
	//_os_mem_free(dvobj, dupstr, wrqu->length);
#endif
	return ret;
}

int rtw_flash_set_by_offset(struct net_device *dev,
			 struct iw_request_info *info,
			 struct iw_point *wrqu, char *extra){
	...
	//RTW_WARN("orig extra: %s\n", extra);

	if (sscanf(extra, "%x=%d", &set_offset, &set_value) != 2) {
		RTW_ERR("Invalid input foramt\n");
		return ret;
	}

	rtw_nvm_set_by_offset(dvobj, set_offset, set_value);

	return ret;
}

int rtw_flash_get_by_offset(struct net_device *dev,
                            struct iw_request_info *info,
                            struct iw_point *wrqu, char *extra)
{
	int ret = 0;
	...
	if (sscanf(extra, "%x", &query_offset) != 1) {
		RTW_ERR("Invalid input foramt\n");
		return ret;
	}

	RTW_PRINT("shadowmap[%x] = %x\n", query_offset,
			rtw_nvm_get_by_offset(dvobj, query_offset));

	return ret;
}

int rtw_flash_dump_shadowmap(struct net_device *dev,
						  struct iw_request_info *info,
						  struct iw_point *wrqu, char *extra){
...
int rtw_dump_all_reg(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrqu, char *extra){
	...
	const u32 reg_pg_size = 0x100;
	u32 i, j, k, page, srt, end;
	u32 val32;

	input=kmalloc(wrqu->length,GFP_KERNEL);
	if(input == NULL) {
		printk("kmalloc error %s %d\n",__FUNCTION__,__LINE__);
		return -EFAULT;
	}
	...
	if (strncmp(input, "mac", 3) == 0){
		srt = 0x0;
		end = 0xFF;
		sprintf(extra, "mac\n");

	} else if (strncmp(input, "bb", 2) == 0){
		srt = 0x100;
		end = 0x13F;
		sprintf(extra, "bb\n");

	} else if (strncmp(input, "rfa", 2) == 0){
		srt = 0x1C0;
		end = 0x1FF;
		sprintf(extra, "rf pathA\n");

	} else if (strncmp(input, "rfb", 2) == 0){
		srt = 0x1D0;
		end = 0x1FF;
		sprintf(extra, "rf pathB\n");

	} else{
		sprintf(extra, "please input arg as mac, bb, or rf\n");
	...
	for (i = srt; i <= end; i++) {
		page = i << 8;
		for (j = page; j < page + reg_pg_size; j += 16) {
			printk("%08xh : ", 0x18600000 + j);

			for (k = 0; k < 4; k++) {
				val32 = rtw_phl_read32(GET_HAL_INFO(adapter_to_dvobj(padapter)), j + 4 * k);
				printk("%08x ", val32);
			}
			printk("\n");
		}
...
int rtw_set_rf_regd_idx(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrqu, char *extra)
{
	#ifndef CONFIG_RTW_LINK_PHL_MASTER
	// 260d23b4901ffa9934cf29d627c6c0d7f2c3a5d3 shengyu.chang
	_adapter *padapter = rtw_netdev_priv(dev);
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(adapter_to_dvobj(padapter));
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	rtw_phl_set_rf_regulation(dvobj->phl, mode);
			...
			union iwreq_data *wdata, char *extra)
{
	...
	if (subcmd != MIB_SET && padapter->netif_up == _FALSE) {
		RTW_INFO(" %s fail =>(padapter->netif_up == _FALSE )\n", __FUNCTION__);
		return -ENETDOWN;
	}

	if (subcmd != MIB_SET && RTW_CANNOT_RUN(adapter_to_dvobj(padapter))) {
	...
	case MIB_SET:
		RTW_INFO("set case MIB_SET\n");
		rtw_mib_set(dev , info , wdata , extra);
		break;
#ifdef CONFIG_RTW_MULTI_AP
	case SIOCMAP_SEND_DISASSOC_VXD:
		RTW_INFO("set case SIOCMAP_SEND_DISASSOC_VXD\n");
		rtw_ioctl_send_disassoc_vxd(dev , info , wdata , extra);
		break;
#endif

#ifdef BEACON_VS_IE
	case SIOCSETCIE:
		RTW_INFO("set case SIOCSETCIE\n");
		rtw_vsie_set(dev , info , wdata , extra);
		break;
...
#ifdef SBWC
	case SIOCSBWC:
		RTW_INFO("set case SIOCSBWC\n");
		rtw_sbwc_setting(dev , info , wdata , extra);
		break;
#endif
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
	case ACL_ADD_STA:
		RTW_INFO("set case ACL_ADD_STA\n");
		rtw_dynamic_acl_add_sta(dev, info, wdata, extra);
		break;
	case ACL_REMOVE_STA:
		RTW_INFO("set case ACL_REMOVE_STA\n");
		rtw_dynami_acl_remove_sta_(dev, info, wdata, extra);
		break;
	case ACL_CLEAR_TABLE:
		RTW_INFO("set case ACL_CLEAR_TABLE\n");
		rtw_dynami_acl_clear_table(dev, info, wdata, extra);
		break;
	case ACL_CHECK_TABLE:
		RTW_INFO("set case ACL_CHECK_TABLE\n");
		rtw_dynami_acl_check_table(dev, info, wdata, extra);
		break;
#endif /* CONFIG_RTW_AP_EXT_SUPPORT */
#ifdef CONFIG_WIFI_DIAGNOSIS
	case WIFI_DIAG:
		RTW_INFO("set case WIFI_DIAG\n");
		return rtw_wifi_diag_start(dev, info, wdata, extra);
		break;
#endif
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
	case SITE_SURVEY:
		RTW_INFO("set case SITE_SURVEY\n");
		return rtw_site_survey_start(dev, info, wdata, extra);
		break;
#endif /* CONFIG_RTW_AP_EXT_SUPPORT */
	default:
		return -EIO;
...
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
			case BSS_INFO_GET:
				RTW_INFO("get case BSS_INFO_GET\n");
				status = rtw_bss_info_get(dev , info , wdata , extra);
				break;
			case STA_INFO_GET:
				RTW_INFO("get case STA_INFO_GET\n");
				status = rtw_sta_info_get(dev , info , wdata , extra);
				break;
			case STA_NUM_GET:
				RTW_INFO("get case STA_NUM_GET\n");
				status = rtw_sta_num_get(dev , info , wdata , extra);
				break;
			case MIB_GET:
				RTW_INFO("get case MIB_GET\n");
				status = rtw_mib_get(dev , info , wdata , extra, 1);
				break;
			case WIFI_PRIV_MIB_DUMP:
				RTW_INFO("get case WIFI_PRIV_MIB_DUMP\n");
				status = rtw_wifi_dump_priv_mib(dev , info , wdata , extra);
				break;
#endif
#ifdef CONFIG_WIFI_DIAGNOSIS
			case WIFI_DIAG:
				RTW_INFO("get case WIFI_DIAG\n");
				status = rtw_wifi_diag_result(dev, info, wdata, extra);
				break;
#endif
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
			case SITE_SURVEY:
				RTW_INFO("get case SITE_SURVEY\n");
				status = rtw_site_survey_result(dev, info, wdata, extra);
				break;

			case STA_ENTRY:
				RTW_INFO("get case STA_ENTRY\n");
				status = rtw_sta_entry_get(dev , info , wdata , extra);
				break;
#endif /* CONFIG_RTW_AP_EXT_SUPPORT */

			case DUMP_ALL_REG:
				RTW_INFO("get case DUMP_ALL");
				status = rtw_dump_all_reg(dev, info, wrqu, extra);
				break;

			case SET_REGD:
				RTW_INFO("Select power limit table index\n");
				status = rtw_set_rf_regd_idx(dev, info, wrqu, extra);
				break;
#if defined(CONFIG_RTW_DRV_HAS_NVM) || defined(RTW_FLASH_98D)
			case FLASH_SET:
				RTW_INFO("FLASH_SET\n");
				status = rtw_flash_set_data(dev, info, wrqu, extra);
				break;

			case FLASH_GET:
				RTW_INFO("FLASH_GET\n");
				status = rtw_flash_get_data(dev, info, wrqu, extra);
				break;

			case FLASH_SET_BY_NAME:
				RTW_INFO("FLASH_SET_BY_NAME\n");
				status = rtw_flash_set_by_name(dev, info, wrqu, extra);
				break;

			case FLASH_SET_BY_OFFSET:
				RTW_INFO("FLASH_SET_BY_OFFSET\n");
				status = rtw_flash_set_by_offset(dev, info, wrqu, extra);
				break;

			case FLASH_GET_BY_OFFSET:
				RTW_INFO("FLASH_GET_BY_OFFSET\n");
				status = rtw_flash_get_by_offset(dev, info, wrqu, extra);
				break;

			case FLASH_DUMP:
				RTW_INFO("FLASH_GET_BY_OFFSET\n");
				status = rtw_flash_dump_shadowmap(dev, info, wrqu, extra);
				break;
#endif /*#ifdef RTW_FLASH_98D*/
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	issue_nulldata_to_TDLS_peer_STA(padapter, ptdls_sta->phl_sta->mac_addr, 1, 3, 500);
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	if (ptdls_sta)
		issue_nulldata_to_TDLS_peer_STA(padapter, ptdls_sta->phl_sta->mac_addr, 0, 3, 500);
...
static const struct iw_priv_args rtw_private_args[] = {
	...
	{ SIOCIWFIRSTPRIV + 0x0E, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , ""},  /* set  */
	{ SIOCIWFIRSTPRIV + 0x0F, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK , ""},/* get */
	{ SIOCIWFIRSTPRIV + 0x19, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_BYTE | IW_PRIV_SIZE_MASK , ""},/* get
 * --- sub-ioctls definitions --- */

#ifdef CONFIG_APPEND_VENDOR_IE_ENABLE
	{ VENDOR_IE_SET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK , 0 , "vendor_ie_set" },
	{ VENDOR_IE_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "vendor_ie_get" },
#endif
#if defined(RTW_PHL_TX) || defined(RTW_PHL_RX) || defined(CONFIG_PHL_TEST_SUITE)
	{ PHL_TEST_SET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK , 0 , "phl_test" },
	{ PHL_TEST_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "" },
#endif
#ifdef CONFIG_WOWLAN
	{ MP_WOW_ENABLE , IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0, "wow_mode" },
	{ MP_WOW_SET_PATTERN , IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0, "wow_set_pattern" },
#endif
#ifdef CONFIG_AP_WOWLAN
	{ MP_AP_WOW_ENABLE , IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0, "ap_wow_mode" }, /* set  */
#endif
#ifdef CONFIG_SDIO_INDIRECT_ACCESS
...
#endif
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
	{ BSS_INFO_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "get_bss_info" },
	{ STA_INFO_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 	IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "get_sta_info" },
	{ STA_NUM_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "get_sta_num" },
	{ MIB_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "get_mib" },
	{ MIB_SET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "set_mib" },
	{ WIFI_PRIV_MIB_DUMP, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "dump_mib" },
#ifdef CONFIG_RTW_MULTI_AP
	{ SEND_DISASSOC_VXD, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "vxd_disassoc" },
#endif
#ifdef BEACON_VS_IE
	{ SIOCSETCIE, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "setCIE" },
#endif
#endif
#ifdef SBWC
	{ SIOCSBWC, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "sta_bw_control" },
#endif
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
	{ ACL_ADD_STA, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "add_acl_table" },
	{ ACL_REMOVE_STA, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "rm_acl_table" },
	{ ACL_CLEAR_TABLE, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "clear_acl_table" },
	{ ACL_CHECK_TABLE, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "check_acl_table" },
#endif /* CONFIG_RTW_AP_EXT_SUPPORT */
#ifdef CONFIG_MP_INCLUDED
	...
	{ READ_REG_BIT, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				    IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "read_reg_bit" },
	{ WRITE_REG_BIT, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
			   		 IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "write_reg_bit" },
	{ READ_RF_BIT, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
			  	   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "read_rf_bit" },
	{ WRITE_RF_BIT, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
			   		IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "write_rf_bit" },
	{ READ_BB_BIT, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				   IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "read_bb_bit" },
	{ WRITE_BB_BIT, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
					IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "write_bb_bit" },
	{ READ_MAC_BIT, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
		  		    IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "read_mac_bit" },
	{ WRITE_MAC_BIT, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
					 IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "write_mac_bit" },
	...
	{ MP_SWITCH_PWRTRK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_swh_pwrtrk" },
	...
	{ MP_SET_PHL_CONIFG_DBCC, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_dbcc" },
	...
	{ MP_RX_GAIN, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_rx_gain" },
#ifdef CONFIG_RTW_CUSTOMER_STR
	{ MP_CUSTOMER_STR, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "customer_str" },
#endif
	{ SET_MP_MODE, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "set_mp_mode" },
	{ MP_TSSI_SLP_START, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_slope_start" },
	{ MP_TSSI_SLP_STOP, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_slope_stop" },
	{ MP_GET_TSSICW_A, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_get_tssicw_a" },
	{ MP_GET_TSSICW_B, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_get_tssicw_b" },
	{ MP_SET_TSSI_SLP_A, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_set_slope_a" },
	{ MP_SET_TSSI_SLP_B, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_set_slope_b" },
	{ MP_GET_TSSI_SLP_A, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_get_slope_a" },
	{ MP_GET_TSSI_SLP_B, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_get_slope_b" },

#endif /* CONFIG_MP_CONFIG_FOR_AP */
#endif /* CONFIG_MP_IONCLUDED*/

	{ FLASH_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "flash_get" },
	{ FLASH_SET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "flash_set" },
	{ FLASH_SET_BY_NAME, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "flash_set_name" },
	{ FLASH_SET_BY_OFFSET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "flash_set_oft" },
	{ FLASH_GET_BY_OFFSET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "flash_get_oft" },
	{ FLASH_DUMP, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "flash_dump" },
	{ DUMP_ALL_REG, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "dump_all" },
	{ SET_REGD, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "set_regd" },

#ifdef CONFIG_WIFI_DIAGNOSIS
	{ WIFI_DIAG, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK , 0 , "diag_start" },
	{ WIFI_DIAG, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "diag_result" },
#endif
	{ SITE_SURVEY, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK , 0 , "ss_start" },
	{ SITE_SURVEY, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "ss_result" },
	{ STA_ENTRY, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "sta_entry_get" },
};

/* --- sub-ioctls definitions --- */
static const struct iw_priv_args rtw_mp_private_args[] = {

#ifndef CONFIG_MP_CONFIG_FOR_AP
...
#if !defined(CPTCFG_WFO_VIRT_MODULE) && !defined(CONFIG_RTK_WFO_NO_VIRT)
static
#endif /* !CPTCFG_WFO_VIRT_MODULE & !CONFIG_RTK_WFO_NO_VIRT */
...
EXPORT_SYMBOL(rtw_private_handler);
#endif /* CPTCFG_WFO_VIRT_MODULE */

#if WIRELESS_EXT >= 17
struct iw_statistics *rtw_get_wireless_stats(struct net_device *dev)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
