...
const u32 b_preld_hiq_mbid_en_l_92xb[MAC_AX_P0_MBID_MAX] = {
...
const u32 b_preld_hiq_port_en_l_92xb[MAC_AX_PORT_NUM] = {
			   ...
			   struct mac_ax_preld_cfg *cfg)
{
	...
	for (i = MAC_AX_PORT_0; i < MAC_AX_PORT_NUM; i++)
		val32 &= ~b_preld_hiq_port_en_l_92xb[i];
	for (i = MAC_AX_P0_MBID1; i < MAC_AX_P0_MBID_LAST; i++)
		val32 &= ~b_preld_hiq_mbid_en_l_92xb[i];
	if (cfg->hiq_en) {
		val32 = cfg->hiq_port == MAC_AX_PORT_0 ?
			val32 | b_preld_hiq_mbid_en_l_92xb[cfg->hiq_mbid] :
			val32 | b_preld_hiq_port_en_l_92xb[cfg->hiq_port];
	}
