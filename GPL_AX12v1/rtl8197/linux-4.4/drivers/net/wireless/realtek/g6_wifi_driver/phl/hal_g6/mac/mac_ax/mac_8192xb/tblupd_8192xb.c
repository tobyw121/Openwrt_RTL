...
u32 mac_f2p_test_cmd_8192xb(struct mac_ax_adapter *adapter,
...
u32 mac_upd_dctl_info_8192xb(struct mac_ax_adapter *adapter,
...
void _set_role_cctrl_v1_8192xb(struct mac_ax_adapter *adapter,
...
void mac_upd_role_cctrl_v1_8192xb(struct mac_ax_adapter *adapter,
				  struct rtw_hal_mac_ax_cctl_info *info,
				  struct rtw_hal_mac_ax_cctl_info *mask, u8 macid)
{
	...
	_set_role_cctrl_v1_8192xb(adapter, info, mask, &role->info.c_info);
}

u32 mac_upd_cctl_info_8192xb(struct mac_ax_adapter *adapter,
			     struct rtw_hal_mac_ax_cctl_info *info,
			     struct rtw_hal_mac_ax_cctl_info *mask,
			     u8 macid, u8 operation)
{
	...
	struct h2c_buf *h2cb;
	#endif
	struct fwcmd_cctlinfo_ud_v1 *tbl;

	h2cb = h2cb_alloc(adapter, H2CB_CLASS_DATA);
	if (!h2cb)
		return MACNPTR;

	buf = h2cb_put(h2cb, sizeof(struct fwcmd_cctlinfo_ud_v1));
	if (!buf) {
		ret = MACNOBUF;
		goto fail;
	}

	tbl = (struct fwcmd_cctlinfo_ud_v1 *)buf;
	tbl->dword0 =
	cpu_to_le32(SET_WORD(macid, FWCMD_H2C_CCTLINFO_UD_V1_MACID) |
		    (operation ? FWCMD_H2C_CCTLINFO_UD_V1_OP : 0));
...
#if MAC_AX_FEATURE_DBGPKG
		if (operation)
			cctl_info_debug_write_8192xb
			(adapter, (struct fwcmd_cctlinfo_ud_v1 *)buf, macid);
#else
		return MACFWNONRDY;
#endif
	}
	...
	mac_upd_role_cctrl_v1_8192xb(adapter, info, mask, macid);
...
u32 mac_init_cctl_info_8192xb(struct mac_ax_adapter *adapter, u8 macid)
{
...
#if MAC_AX_FEATURE_DBGPKG
u32 cctl_info_debug_write_8192xb(struct mac_ax_adapter *adapter,
				 struct fwcmd_cctlinfo_ud_v1 *tbl,
				 u8 macid)
{
	u32 val;
...
u32 dctl_info_debug_write_8192xb(struct mac_ax_adapter *adapter,
				 struct fwcmd_dctlinfo_ud_v1 *tbl,
				 u8 macid)
{
...
u32 mac_ss_dl_grp_upd_8192xb(struct mac_ax_adapter *adapter,
