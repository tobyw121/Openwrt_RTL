...
bool halrf_bw_setting_8192xb(struct rf_info *rf, enum rf_path path, enum channel_width bw, bool is_dav)
{
	u32 rf_reg18 = 0;
...
bool halrf_ctrl_bw_8192xb(struct rf_info *rf, enum phl_phy_idx phy, enum channel_width bw)
{
	...
	for (path = 0; path < 2; path++) {
		if (kpath & BIT(path)) {
			is_dav = true;
			halrf_bw_setting_8192xb(rf, path, bw, is_dav);
			is_dav = false;
			halrf_bw_setting_8192xb(rf, path, bw, is_dav);
	...
	halrf_bw_setting_op5k_8192xb(rf);

	return true;
}

bool halrf_ch_setting_8192xb(struct rf_info *rf,   enum rf_path path, u8 central_ch, enum band_type band, bool is_dav)
{
	u32 rf_reg18 = 0;	
	u32 reg_reg18_addr = 0x0;
	...
	case BAND_ON_24G :
		...
		if(rf->hal_com->cv >= CAV){
			if(central_ch <= 0x4) //ch4
				halrf_wrf(rf, path, 0xbe, MASKRF, 0x000a0);
			else if (central_ch <= 0xa)	//ch10	
				halrf_wrf(rf, path, 0xbe, MASKRF, 0x800a2);
			else if (central_ch <= 0xd)	//ch13	
				halrf_wrf(rf, path, 0xbe, MASKRF, 0x800a4);
			else if (central_ch <= 0xe) //ch14		
				halrf_wrf(rf, path, 0xbe, MASKRF, 0x000a5);
			else
				RF_DBG(rf, DBG_RF_RFK, "[RFK] 2G RF 0xbe setting fail\n");
		}		
		halrf_delay_us(rf, 5);
...
bool halrf_ctrl_ch_8192xb(struct rf_info *rf,  enum phl_phy_idx phy, u8 central_ch, enum band_type band)
{
	...
	for (path = 0; path < 2; path++) {
		if (kpath & BIT(path)) {
			is_dav = true;
			halrf_ch_setting_8192xb(rf, path, central_ch, band, is_dav);
			is_dav = false;
			halrf_ch_setting_8192xb(rf, path, central_ch, band, is_dav);	
		}
	}
...
void halrf_set_lo_8192xb(struct rf_info *rf, bool is_on, enum rf_path path)
{
	if (is_on) {
		halrf_rf_direct_cntrl_8192xb(rf, path, false);
		...
		halrf_rf_direct_cntrl_8192xb(rf, path, true);
	}
}
...
void halrf_lo_test_8192xb(struct rf_info *rf, bool is_on, enum rf_path path)
{
	switch (path) {
		case RF_PATH_A:
			halrf_set_lo_8192xb(rf, is_on, RF_PATH_A);
			halrf_set_lo_8192xb(rf, false, RF_PATH_B);
			break;
		case RF_PATH_B:
			halrf_set_lo_8192xb(rf, false, RF_PATH_A);
			halrf_set_lo_8192xb(rf, is_on, RF_PATH_B);
			break;
		case RF_PATH_AB:
			halrf_set_lo_8192xb(rf, is_on, RF_PATH_A);
			halrf_set_lo_8192xb(rf, is_on, RF_PATH_B);
...
void _rx_dck_info_8192xb(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_afe)
{
...
{
	u8 phy_map;
	u32 reg1, reg2, reg3, reg4, reg5, i = 0;

	phy_map = (BIT(phy) << 4) | BIT(path);

	_rx_dck_info_8192xb(rf, phy, path, is_afe);

	if (is_afe) {
		halrf_wrf(rf, path, 0x93, 0x0000f, 0x8); /*BIT3 1: from AFE*/

		reg1 = halrf_rreg(rf, 0x12a0 + (path << 13), MASKDWORD);
		reg2 = halrf_rreg(rf, 0xc0d4 + (path << 8), MASKDWORD);
		reg3 = halrf_rreg(rf, 0xc0d8 + (path << 8), MASKDWORD);
		reg4 = halrf_rreg(rf, 0x8034, MASKDWORD);
		reg5 = halrf_rreg(rf, 0x80d4, MASKDWORD);

		halrf_wreg(rf, 0x12b8 + (path << 13), BIT(30), 0x1); /*debug en*/
		halrf_wreg(rf, 0x12a0 + (path << 13), 0x000F0000, 0xb);

		halrf_wreg(rf, 0xc0d4 + (path << 8), BIT(27) | BIT(26), 0x0); /*for ADC 320M*/
		halrf_wreg(rf, 0xc0d8 + (path << 8), 0x000001E0, 0x7); /*[8:5] for ADC 320M*/
	
		halrf_wreg(rf, 0xc0f8 + (path << 8), BIT(29) | BIT(28), 0x3); /*offset manual en*/
		halrf_wreg(rf, 0xc0f4 + (path << 8), BIT(7) | BIT(6), 0x3); /**avg 0:16; 1:32; 2:64; 3:128**/
		halrf_wreg(rf, 0x032c, BIT(18) | BIT(17) | BIT(16), 0x1); /*connect with RXBB*/
		halrf_wreg(rf, 0x030c, 0x0f000000, 0xf); /*adc en and rst*/
		...
		halrf_wreg(rf, 0x5670, BIT(1), 0x1); /*iqk_control_sw_si*/

		halrf_wreg(rf, 0x8008, BIT(7), 0x1);
		halrf_wreg(rf, 0x8034, BIT(5) | BIT(4), 0x2);
		halrf_wreg(rf, 0x80d4, BIT(10) | BIT(9) | BIT(8), 0x1);
		halrf_wreg(rf, 0x81e0, BIT(22) | BIT(21) | BIT(20), 0x0);
		halrf_wreg(rf, 0x81e0, BIT(25), 0x0);
		halrf_wreg(rf, 0x82e0, BIT(22) | BIT(21) | BIT(20), 0x1);
		halrf_wreg(rf, 0x82e0, BIT(25), 0x0);
		halrf_wreg(rf, 0x8000, BIT(2) | BIT(1), path); /*subpage_id*/

		//halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_RXDCK, RFK_ONESHOT_START);

		halrf_wreg(rf, 0x80ac, MASKDWORD, 0xfc030001); /*DCK for RX mode*/
		halrf_wreg(rf, 0x80ac, MASKDWORD, 0x7c030001);
		...
		RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] S%d DCK done (for Rx, cnt=%d)\n",
			path, i);

		i = 0;
		halrf_wreg(rf, 0x80ac, MASKDWORD, 0xfc050001); /*DCK for DPK mode*/
		halrf_wreg(rf, 0x80ac, MASKDWORD, 0x7c050001);
		...
		RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] S%d DCK done (for DPK, cnt=%d)\n",
			path, i);
		...
		halrf_wreg(rf, 0x12a0 + (path << 13), MASKDWORD, reg1);
		halrf_wreg(rf, 0xc0d4 + (path << 8), MASKDWORD, reg2);
		halrf_wreg(rf, 0xc0d8 + (path << 8), MASKDWORD, reg3);
		halrf_wreg(rf, 0x8034, MASKDWORD, reg4);
		halrf_wreg(rf, 0x80d4, MASKDWORD, reg5);
		halrf_wreg(rf, 0x5670, BIT(1), 0x0); /*iqk_control_sw_si*/
...
bool halrf_rx_dck_check_8192xb(struct rf_info *rf, enum rf_path path)
{
	u8 addr;
	bool is_fail = false;
...
void halrf_rx_dck_8192xb(struct rf_info *rf, enum phl_phy_idx phy, bool is_afe) 
{
	...
	RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] ****** RXDCK Start (Ver: 0x%x, Cv: %d) ******\n",
		RXDCK_VER_8192XB, rf->hal_com->cv);
	...
	for (path = 0; path < 2; path++) {
		...
		halrf_set_rx_dck_8192xb(rf, phy, path, is_afe);
...
void iqk_backup_8192xb(struct rf_info *rf, enum rf_path path) 
{
	return;
}
...
void halrf_set_dpd_backoff_8192xb(struct rf_info *rf, enum phl_phy_idx phy)
{
...
{
	halrf_set_dpd_backoff_8192xb(rf, HW_PHY_0);
}

void halrf_set_rxbb_bw_8192xb(struct rf_info *rf, enum channel_width bw, enum rf_path path)
{
...
void halrf_rxbb_bw_8192xb(struct rf_info *rf, enum phl_phy_idx phy, enum channel_width bw)
{
	...
	for (path = 0; path < 2; path++) {
		if ((kpath & BIT(path)) && (rf->pre_rxbb_bw[path] != bw)) {
			halrf_set_rxbb_bw_8192xb(rf, bw, path);
...
bool halrf_one_shot_nctl_done_check_8192xb(struct rf_info *rf, enum rf_path path) {
	...
	while (count < 2000) {			
		...
		if (r_80fc == NCTL_FINAL_LINE_8192XB) {
			is_ready = true;
			break;
		}	
		halrf_delay_us(rf, 1);
...
bool halrf_do_one_shot_8192xb(struct rf_info *rf, enum rf_path path, u32 nctl_addr, u32 mask, u32 process_id)
{
	...
	halrf_before_one_shot_enable_8192xb(rf);
	halrf_wreg(rf, nctl_addr, mask, process_id); //set cal_path, process id

	return halrf_one_shot_nctl_done_check_8192xb(rf, path);
}
...
void halrf_lck_tracking_8192xb(struct rf_info *rf)
{
	...
	if(HALRF_ABS(rf->lck_ther, (u8)ther_avg) >= LCK_TH_8192XB)		
		halrf_lck_trigger(rf);
#endif
}

static u32 backup_mac_reg_8192xb[] = {0x0};
static u32 backup_bb_reg_8192xb[] = {0x2344,0xc0d4,0xc0d8, 0xc0c4, 0xc0e8, 0xc0ec, 0xc1d4, 0xc1d8, 0xc1c4, 0xc1e8, 0xc1ec};
static u32 backup_rf_reg_8192xb[] = {0xdc, 0xef, 0xde, 0x0, 0x1e, 0x5, 0x10005};

#if 1
static struct halrf_iqk_ops iqk_ops= {
    .iqk_kpath = halrf_kpath_8192xb,
    .iqk_mcc_page_sel = iqk_mcc_page_sel_8192xb,
    .iqk_get_ch_info = iqk_get_ch_info_8192xb,
    .iqk_preset = iqk_preset_8192xb,
    .iqk_macbb_setting = iqk_macbb_setting_8192xb,
    .iqk_start_iqk = iqk_start_iqk_8192xb,
    .iqk_restore = iqk_restore_8192xb,
    .iqk_afebb_restore = iqk_afebb_restore_8192xb,  
};

struct rfk_iqk_info rf_iqk_hwspec_8192xb = {
  	.rf_iqk_ops = &iqk_ops,
	.rf_max_path_num = 2,
	.rf_iqk_version = iqk_version_8192xb,
	.rf_iqk_ch_num = 2,
	.rf_iqk_path_num = 2,

#if 0
	.backup_mac_reg = backup_mac_reg_8192xb,
	.backup_mac_reg_num = ARRAY_SIZE(backup_mac_reg_8192xb),
#else
	.backup_mac_reg = backup_mac_reg_8192xb,
	.backup_mac_reg_num = 0,
#endif

    	.backup_bb_reg = backup_bb_reg_8192xb,
    	.backup_bb_reg_num = ARRAY_SIZE(backup_bb_reg_8192xb),
    	.backup_rf_reg = backup_rf_reg_8192xb,
    	.backup_rf_reg_num = ARRAY_SIZE(backup_rf_reg_8192xb),
};
