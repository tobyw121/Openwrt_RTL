	...
	struct sta_info *psta, u16 frame_type)
{
	...
	if (update_mgntframe_attrib(padapter, pattr) != _SUCCESS) {
	...
	while (len && prm->q.opt.bcn.opt_id_num < BCN_REQ_OPT_MAX_NUM) {
		...
		case bcn_req_ssid:
			...
			_rtw_memcpy(&(prm->q.opt.bcn.ssid.Ssid),
				DBG_BCN_REQ_SSID_NAME, MIN(i, sizeof(prm->q.opt.bcn.ssid.Ssid)-1));

#else /* original */
			prm->q.opt.bcn.ssid.SsidLength = pbody[p+1];
			_rtw_memcpy(&(prm->q.opt.bcn.ssid.Ssid),
				&pbody[p+2], MIN(pbody[p+1], sizeof(prm->q.opt.bcn.ssid.Ssid)-1));
#endif
#endif
...
#ifdef PLATFORM_ECOS
	prm->q.rpt = le16_to_cpu(get_unaligned((u16 *)(&pdiag_body[3])));
#else
...
void _rm_parse_bcn_rep_subelement(_adapter *padapter,
		struct dot11k_beacon_measurement_report *beacon_rep,
		struct nb_rpt_hdr *nb_report)
{
	...
	bssinfo_ptr = (u8 *)&nb_report->bss_info;

	pbuf = beacon_rep->subelements;
	len = beacon_rep->subelements_len;

	SET_RM_NB_RPT_BSSINFO_AP_REACH(bssinfo_ptr, 3);

	/* capability */
	if(len >= 12){
		capab_ie_data = RTW_GET_LE16(pbuf + 2 + 10);
	}
	else
		return;

	if(capab_ie_data & BIT(8))
		SET_RM_NB_RPT_BSSINFO_SPEC_MGMT(bssinfo_ptr, 1);
	if(capab_ie_data & BIT(9))
		SET_RM_NB_RPT_BSSINFO_QOS(bssinfo_ptr, 1);
	if(capab_ie_data & BIT(11))
		SET_RM_NB_RPT_BSSINFO_APSD(bssinfo_ptr, 1);
	if(capab_ie_data & BIT(14))
		SET_RM_NB_RPT_BSSINFO_DELAY_BA(bssinfo_ptr, 1);
	if(capab_ie_data & BIT(15))
		SET_RM_NB_RPT_BSSINFO_IMMED_BA(bssinfo_ptr, 1);

	/* wpa2 */
	p = rtw_get_ie(pbuf + 2 + _BEACON_IE_OFFSET_, _RSN_IE_2_,
						&ie_len, (len - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0) {
		if (rtw_parse_wpa2_ie(p, ie_len + 2, NULL, NULL, NULL, NULL, NULL) == _SUCCESS) {
			auth_algo = dot11AuthAlgrthm_8021X;
			wpa_psk |= BIT(1);
		}
	}
	/* wpa */
	ie_len = 0;
	for (p = pbuf + 2 + _BEACON_IE_OFFSET_; ; p += (ie_len + 2)) {
		p = rtw_get_ie(p , _SSN_IE_1_,
						&ie_len, (len - _BEACON_IE_OFFSET_ - (ie_len + 2)));
		if ((p) && (_rtw_memcmp(p + 2, OUI1, 4))) {
			if (rtw_parse_wpa_ie(p, ie_len + 2, NULL, NULL, NULL) == _SUCCESS) {
				auth_algo = dot11AuthAlgrthm_8021X;
				wpa_psk |= BIT(0);
			}
			break;
	...
	if(auth_algo == psecuritypriv->dot11AuthAlgrthm)
		SET_RM_NB_RPT_BSSINFO_KEY_SCOPE(bssinfo_ptr, 1);
	if(wpa_psk == psecuritypriv->wpa_psk)
		SET_RM_NB_RPT_BSSINFO_SECURITY(bssinfo_ptr, 1);

	/* RM */
	p = rtw_get_ie(pbuf + 2 + _BEACON_IE_OFFSET_, EID_RMEnabledCapability,
						&ie_len, (len - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0)
		SET_RM_NB_RPT_BSSINFO_RM(bssinfo_ptr, 1);

#ifdef CONFIG_80211N_HT
	p = rtw_get_ie(pbuf + 2 + _BEACON_IE_OFFSET_, _HT_CAPABILITY_IE_,
						&ie_len, (len - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0) {
		SET_RM_NB_RPT_BSSINFO_HTC(bssinfo_ptr, 1);
		nb_report->phy_type = DOT11_PHY_TYPE_HT;
	}
#endif /* CONFIG_80211N_HT */

#ifdef CONFIG_80211AC_VHT
	p = rtw_get_ie(pbuf + 2 + _BEACON_IE_OFFSET_, EID_VHTCapability,
						&ie_len, (len - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0) {
		SET_RM_NB_RPT_BSSINFO_VHT(bssinfo_ptr, 1);
		nb_report->phy_type = DOT11_PHY_TYPE_VHT;
	}
#endif /* CONFIG_80211AC_VHT */

#ifdef CONFIG_80211AX_HE
	p = rtw_get_ie_ex(pbuf + 2 + _BEACON_IE_OFFSET_, (len - _BEACON_IE_OFFSET_),
						WLAN_EID_EXTENSION, &he_cap_eid_ext, 1, NULL, &ie_len);
	if (p && ie_len > 0) {
		SET_RM_NB_RPT_BSSINFO_HE(bssinfo_ptr, 1);
		nb_report->phy_type = DOT11_PHY_TYPE_HE;
	}
...
void _rm_fill_bcn_rpt_to_nbr_rpt(_adapter *padapter,
	  struct dot11k_beacon_measurement_report *beacon_rep)
{
	struct nb_rpt_hdr nb_report;
	u8 idx;

	_rtw_memset(&nb_report, 0, sizeof(struct nb_rpt_hdr));
	nb_report.reg_class = beacon_rep->info.op_class;
	nb_report.ch_num = beacon_rep->info.channel;
	nb_report.phy_type = beacon_rep->info.frame_info & 0x7f;
	_rtw_memcpy(nb_report.bssid, beacon_rep->info.bssid, ETH_ALEN);
	nb_report.preference = beacon_rep->info.RCPI;
	_rm_parse_bcn_rep_subelement(padapter, beacon_rep, &nb_report);
	idx = rtw_rm_insert_nb_report(padapter, nb_report);
	RTW_INFO("[%s]idx = %d, report.bss_info = %x, bssid = " MAC_FMT "\n", __func__, idx, nb_report.bss_info, MAC_ARG(nb_report.bssid));
}

int _rm_parse_single_beacon_mens_report(_adapter *padapter,
						struct sta_info *psta,
                        u8 *pframe, int len, int frame_len)
{
	...
	psta->rm_beacon_rpt_status = pframe[len + 3];
	/*succeed*/
	if (pframe[len + 3] == 0) {
		//pstat->rm.measure_result = MEASUREMENT_RECEIVED;
		psta->rm_beacon_rpt_status = MEASUREMENT_RECEIVED;
		if (element_len <= 3)
			return len += 2 + element_len;

		#ifdef CONFIG_RTW_AP_EXT_SUPPORT
		for (i = 0 ; i < psta->rm_beacon_rpt_num; i++) {
			if (_TRUE ==
				_rtw_memcmp(psta->rm_beacon_rpt_list[i].info.bssid,
							pframe + len + 20, ETH_ALEN)) {
					beacon_rep = &psta->rm_beacon_rpt_list[i];
		...
		if (psta->rm_beacon_rpt_num < RTW_MAX_RADIO_RPT_NUM
				&& bssid_exist == 0) {
			#ifdef CONFIG_RTW_AP_EXT_SUPPORT
			beacon_rep = &psta->rm_beacon_rpt_list[psta->rm_beacon_rpt_num];
			memset(beacon_rep, 0x00, sizeof(struct dot11k_beacon_measurement_report));
			#endif /* CONFIG_RTW_AP_EXT_SUPPORT */
			#ifdef CONFIG_RTW_MULTI_AP
			psta->rm_beacon_rpt_len[psta->rm_beacon_rpt_num]	= element_len;
			psta->rm_becaon_meas_token[psta->rm_beacon_rpt_num]	= pframe[len+2];
			psta->rm_beacon_rpt_mode[psta->rm_beacon_rpt_num]	= pframe[len+3];
			#endif
			psta->rm_beacon_rpt_num++;
		}

		if (beacon_rep) {
			beacon_rep->info.op_class = pframe[len + 5];
			beacon_rep->info.channel = pframe[len + 6];
			beacon_rep->info.measure_time_lo = le32_to_cpu(*(u32 *)&pframe[len + 7]);
			beacon_rep->info.measure_time_hi = le32_to_cpu(*(u32 *)&pframe[len + 11]);
			beacon_rep->info.measure_duration= le16_to_cpu(*(u16 *)&pframe[len + 15]);
			beacon_rep->info.frame_info = pframe[len + 17];
			beacon_rep->info.RCPI = pframe[len + 18];
			beacon_rep->info.RSNI = pframe[len + 19];
			memcpy(beacon_rep->info.bssid, pframe + len + 20, MAC_ADDR_LEN);
			beacon_rep->info.antenna_id = pframe[len + 26];
			beacon_rep->info.parent_tsf = le32_to_cpu(*(u32 *)&pframe[len + 27]);
		}
        ...
        while (len + 2 <= frame_len)
        {
            ...
            else if (subelement_id == _VENDOR_SPECIFIC_IE_) {
        ...
        if (beacon_rep) {
			if(subelements_copy_len < MAX_BEACON_SUBLEMENT_LEN)
			{
				memcpy(beacon_rep->subelements, pframe + subelements_begin, subelements_copy_len);
				beacon_rep->subelements_len = subelements_copy_len;
			}
			else
			{
				memcpy(beacon_rep->subelements, pframe + subelements_begin, MAX_BEACON_SUBLEMENT_LEN);
				beacon_rep->subelements_len = MAX_BEACON_SUBLEMENT_LEN;
				RTW_ERR("In %s(%d), overrunning beacon_rep->subelements\n", __FUNCTION__, __LINE__);
			}

			if (padapter->registrypriv.wifi_mib.roaming_enable)
				_rm_fill_bcn_rpt_to_nbr_rpt(padapter, beacon_rep);
        }
		psta->rm_beacon_rpt_status = MEASUREMENT_SUCCEED;
        return len;
    }
	else if (pframe[len + 3] & BIT1) /*incapable*/
	{
#ifdef CONFIG_RTW_MULTI_AP
		psta->rm_beacon_rpt_len[0]			= element_len;
		psta->rm_becaon_meas_token[0]		= pframe[len+2];
		psta->rm_beacon_rpt_mode[0]			= pframe[len+3];
		psta->rm_beacon_rpt_num				= 1;
#endif
		psta->rm_beacon_rpt_status = MEASUREMENT_INCAPABLE;
	}
	else if (pframe[len + 3] & BIT2) /*refused*/
	{
		...
		psta->rm_beacon_rpt_num 			= 1;
#endif
		psta->rm_beacon_rpt_status = MEASUREMENT_REFUSED;
	}
	else
	{
		psta->rm_beacon_rpt_status = MEASUREMENT_UNKNOWN;
	}
    return len + element_len + 2;
}

void rm_parse_radio_mens_rep(_adapter *padapter,
	union recv_frame *precv_frame, struct sta_info *psta)
{
	int len;
    unsigned char element_id;
    unsigned char element_len;
    int frame_len = precv_frame->u.hdr.len - sizeof(struct rtw_ieee80211_hdr_3addr);
	u8 *pdiag_body = (u8 *)(precv_frame->u.hdr.rx_data +
		sizeof(struct rtw_ieee80211_hdr_3addr));
	...
	while (len + 5 <= frame_len) {
        element_id = pdiag_body[len];
        element_len = pdiag_body[len + 1];
        /*parsing every radio measurment report element*/
        if (element_id == _MEAS_RSP_IE_) {
            if(pdiag_body[len + 4] == bcn_rep) {
                len = _rm_parse_single_beacon_mens_report(padapter, psta, pdiag_body, len, frame_len);
                continue;
            }
        }
        len += 2 + element_len;
	...
	if ((psta->rm_beacon_rpt_status != MEASUREMENT_INCAPABLE) && psta->rm_beacon_rpt_status != MEASUREMENT_REFUSED) {
		while (psta->beacon_report_start_index < psta->rm_beacon_rpt_num) {
			core_map_report_beacon_metrics(psta);
		}
	}
#endif
}
	...
	union recv_frame *precv_frame, struct sta_info *psta)
{
	...
	rm_parse_radio_mens_rep(padapter, precv_frame, psta);
...
/* receive radio measurement neighbor request */
int rm_radio_mens_nb_req(_adapter *padapter,
	union recv_frame *precv_frame, struct sta_info *psta)
{
	struct rm_obj *prm;
	u32 rmid;
...
unsigned int rm_on_action(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case RM_ACT_NB_REP_REQ:
		...
		ret = rm_radio_mens_nb_req(padapter, precv_frame, psta);
		break;
...
static int retrieve_scan_result(struct rm_obj *prm)
{
	...
	queue = &padapter->dvobj->scanned_queue;
#else
...
void rtw_rm_construct_self_nb_report(_adapter *padapter)
{
	...
	bssinfo_ptr = (u8 *)&report.bss_info;
	_rtw_memset(&report, 0, sizeof(struct nb_rpt_hdr));

	_rtw_memcpy(report.bssid, pbss_network->MacAddress, MAC_ADDR_LEN);
	report.ch_num = rtw_get_oper_ch(padapter);
	report.reg_class = rm_get_oper_class_via_ch(report.ch_num);

	SET_RM_NB_RPT_BSSINFO_AP_REACH(bssinfo_ptr, 3);
	SET_RM_NB_RPT_BSSINFO_SECURITY(bssinfo_ptr, 1);
	SET_RM_NB_RPT_BSSINFO_KEY_SCOPE(bssinfo_ptr, 1);

	if(pmlmepriv->capab_ie_data & BIT(8))
		SET_RM_NB_RPT_BSSINFO_SPEC_MGMT(bssinfo_ptr, 1);
	if(pmlmepriv->capab_ie_data & BIT(9))
		SET_RM_NB_RPT_BSSINFO_QOS(bssinfo_ptr, 1);
	if(pmlmepriv->capab_ie_data & BIT(11))
		SET_RM_NB_RPT_BSSINFO_APSD(bssinfo_ptr, 1);
	if(pmlmepriv->capab_ie_data & BIT(14))
		SET_RM_NB_RPT_BSSINFO_DELAY_BA(bssinfo_ptr, 1);
	if(pmlmepriv->capab_ie_data & BIT(15))
		SET_RM_NB_RPT_BSSINFO_IMMED_BA(bssinfo_ptr, 1);

	if(padapter->rmpriv.enable)
		SET_RM_NB_RPT_BSSINFO_RM(bssinfo_ptr, 1);

#ifdef CONFIG_80211N_HT
	if(pmlmepriv->htpriv.ht_option) {
		SET_RM_NB_RPT_BSSINFO_HTC(bssinfo_ptr, 1);
		report.phy_type = DOT11_PHY_TYPE_HT;
	}
	...
	if(pmlmepriv->vhtpriv.vht_option) {
		SET_RM_NB_RPT_BSSINFO_VHT(bssinfo_ptr, 1);
		report.phy_type = DOT11_PHY_TYPE_VHT;
	}
	...
	if (pmlmepriv->hepriv.he_option) {
		SET_RM_NB_RPT_BSSINFO_HE(bssinfo_ptr, 1);
		report.phy_type = DOT11_PHY_TYPE_HE;
	}
#endif /* CONFIG_80211AX_HE */

	idx = rtw_rm_insert_nb_report(padapter, report);
	padapter->mlmepriv.nb_info.self_nb_rpt = report;
	RTW_INFO("[%s]idx = %d, report.bss_info = %x\n", __func__, idx, report.bss_info);
}
...
void rtw_rm_check_mgmt_tx(_adapter *padapter, const u8 *buf, u32 len)
{
	...
	psta = rtw_get_stainfo(&padapter->stapriv, GetAddr1Ptr(buf));
	switch (action) {
		case RM_ACT_RADIO_MEAS_REQ:
			if(len >= 10 && frame_body[9] == bcn_req)
				if(psta) {
#ifdef CONFIG_RTW_MULTI_AP
					psta->beacon_report_start_index = 0;
#endif
...
int rtw_rm_get_sta_beacon_report(_adapter *padapter, u8 *macaddr, u8 *result_buf)
{
	...
	if (psta) {
		pbuf = result_buf;
		*pbuf = psta->rm_beacon_rpt_status;
		*(pbuf + 1) = psta->rm_beacon_rpt_num;
		len = 2;
		if(psta->rm_beacon_rpt_num > 0 && psta->rm_beacon_rpt_num <= RTW_MAX_RADIO_RPT_NUM) {
			_rtw_memcpy(pbuf + len, psta->rm_beacon_rpt_list, psta->rm_beacon_rpt_num * sizeof(struct dot11k_beacon_measurement_report));
			len += psta->rm_beacon_rpt_num * sizeof(struct dot11k_beacon_measurement_report);
			RTW_INFO("====Beacon Report====\n");
			for (i = 0; i < psta->rm_beacon_rpt_num; i++) {
				RTW_INFO("CH%d\tBSSID "MAC_FMT" \tRCPI %d\t/RSNI %d\n",
					psta->rm_beacon_rpt_list[i].info.channel,
					MAC_ARG(psta->rm_beacon_rpt_list[i].info.bssid),
					psta->rm_beacon_rpt_list[i].info.RCPI,psta->rm_beacon_rpt_list[i].info.RSNI);
			}
		}
		else{
			RTW_INFO("%s(%d) The rm_beacon_rpt_num is invalid.\n", __FUNCTION__, __LINE__);
			return -1;
		}
	}
	else{
		RTW_INFO("%s(%d) Can not get sta info.\n", __FUNCTION__, __LINE__);
		return -1;
	}
