...
void ip6_route_input(struct sk_buff *skb)
{
	...
	struct flowi6 fl6 = {
		...
		if((0x33==skb->data[0])&&(0x33==skb->data[1])&&(0x0==skb->data[2]))
...
#if defined(CONFIG_NF_CONNTRACK_RTCACHE) && defined(CONFIG_NF_CONNTRACK_IPV6)
/*	remove genid check for Ipv6 dst not update genid while net may do */
struct dst_entry *rtl_ip6_dst_check(struct dst_entry *dst, u32 cookie)
{
	struct rt6_info *rt;
