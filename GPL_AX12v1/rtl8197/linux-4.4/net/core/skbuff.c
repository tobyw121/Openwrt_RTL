			    ...
			    int flags, int node)
{
	...
	skb->mac_header = (typeof(skb->mac_header))~0U;
#if defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT) || defined(CONFIG_RTL_ENABLE_OVERSEA_L2_ACCELERATE)
	skb->from_dev = NULL;
#endif
...
struct sk_buff *__build_skb(void *data, unsigned int frag_size)
{
	...
	//skb->srcVlanId=0;
#endif	
#if defined(CONFIG_NETFILTER_XT_MATCH_PHYPORT) || defined(CONFIG_RTL_FAST_FILTER) || defined(CONFIG_RTL_QOS_PATCH)|| defined(CONFIG_RTK_VOIP_QOS) || defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) ||defined(CONFIG_RTL_MAC_FILTER_CARE_INPORT) ||defined(CONFIG_RTL_HW_QOS_SUPPORT_WLAN) || defined(CONFIG_RTL_EN_UNKNOWNUC_ONLY_TO_CPU) || defined(CONFIG_RTL_NIC_QUEUE)
	skb->srcPhyPort=0xFF;
	skb->dstPhyPort=0xFF;
#endif
...
extern int rtw_mem_pool_free_buf(void *buf);
extern bool is_rtw_mem(void *buf);
#endif
...
static void skb_free_head(struct sk_buff *skb)
{
	...
	if (skb->head_frag)
		skb_free_frag(head);
#ifdef CONFIG_RTW_MEMPOOL
	else if(is_rtw_mem(head))
		rtw_mem_pool_free_buf(head);
#endif
	else
		kfree(head);
}
...
#if defined(CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT)
	CHECK_SKB_FIELD(vlan_passthrough_saved_tag);
#endif
...
#if defined(CONFIG_NETFILTER_XT_MATCH_PHYPORT) || defined(CONFIG_RTL_FAST_FILTER) || defined(CONFIG_RTL_QOS_PATCH) || defined(CONFIG_RTK_VOIP_QOS)|| defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) ||defined(CONFIG_RTL_MAC_FILTER_CARE_INPORT) ||defined(CONFIG_RTL_HW_QOS_SUPPORT_WLAN) || defined(CONFIG_RTL_DNS_TRAP)|| defined(CONFIG_RTL_CUSTOM_PASSTHRU)
	 C(srcPhyPort);
	 C(dstPhyPort);
#endif
