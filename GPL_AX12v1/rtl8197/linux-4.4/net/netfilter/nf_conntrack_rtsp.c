...
static void rtsp_set_data_stream_timeout(struct sk_buff *skb, struct nf_conn *ct)
{
	...
	for (; bucket < net->ct.htable_size; bucket++) {
		hlist_nulls_for_each_entry(h, n, &net->ct.hash[bucket], hnnode) {
			if (h) {
				...
				/*update data stream time expires when receive rtsp PAUSE */
				if(ct_iter->rtsp_data_mark == 1)
				{					
					nf_ct_refresh(ct_iter, skb, 3600*HZ);
				}
			}
		}
	}
...
static inline int
help_out(struct sk_buff *skb, unsigned int protoff, unsigned char *rb_ptr, unsigned int datalen,
                struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while (dataoff < datalen) {
		...
		{
			pr_debug("found a pause message\n");
			rtsp_set_data_stream_timeout(skb, ct);
		}
...
*/
	if ((skb->len - dataoff) < MAX_RTSP_BUFFER)
		rb_ptr = skb_header_pointer(skb, dataoff,
					    skb->len - dataoff, rtsp_buffer);
	else
	{
		pr_err("rtsp: the skb datalen %d > MAX_RTSP_BUFFER(%d),Need adjust MAX_RTSP_BUFFER\n",datalen,MAX_RTSP_BUFFER);
		rb_ptr = skb_header_pointer(skb, dataoff,
					    MAX_RTSP_BUFFER, rtsp_buffer);
	}
/*
 * TBS_END_TAG
 */
	...
	case IP_CT_DIR_ORIGINAL:
		ret = help_out(skb, protoff, rb_ptr, datalen, ct, ctinfo);
		break;
...
*/
	rtsp_buffer = kmalloc(MAX_RTSP_BUFFER, GFP_KERNEL);
	if (!rtsp_buffer)
		return -ENOMEM;
	...
	for (i = 0; (i < MAX_PORTS) && ports[i]; i++) {
		...
		strncpy(hlpr->name, tmpname, NF_CT_HELPER_NAME_LEN);
