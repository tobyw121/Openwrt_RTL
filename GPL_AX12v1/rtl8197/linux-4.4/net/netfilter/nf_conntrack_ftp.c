		...
		enum ip_conntrack_info ctinfo)
{
	...
	#ifdef RTL_NF_ALG_CTL
	FTP_ALG_CHECK_ONOFF(alg_type_ftp);
	#endif
...
skip_nl_seq:
	...
	if(alg_debug)
	{
		if(NF_CT_FTP_PORT==search[dir][i].ftptype)
			printk("ftp is in the post mode\n");
		else if(NF_CT_FTP_PASV==search[dir][i].ftptype)
			printk("ftp is in the passive mode\n");
