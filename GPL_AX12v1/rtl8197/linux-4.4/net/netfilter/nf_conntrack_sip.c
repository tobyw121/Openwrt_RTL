		       ...
		       unsigned int cseq)
{
	...
	if (hooks && ct->status & IPS_NAT_MASK) {
		...
		if ((ct->status & IPS_DST_NAT) 
			&& (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all == htons(SIP_PORT))
			&& (ct->tuplehash[dir].tuple.src.u.all == htons(SIP_PORT))
			){
				/*  usually, ct created by wanside sip client */
				memcpy(&rtp_addr, &ct->tuplehash[!dir].tuple.dst.u3, sizeof(rtp_addr));	
				//printk("%s %d ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all=0x%x, ct->tuplehash[dir].tuple.src.u.all=0x%x, ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all=0x%x, SIP_PORT=0x%x ct->status=0x%x\n", __func__, __LINE__, ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all, ct->tuplehash[dir].tuple.src.u.all, ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all, SIP_PORT, ct->status);
				
		}
		else if ((ct->status & IPS_SRC_NAT)
				&& (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all == htons(SIP_PORT))
				&& (ct->tuplehash[dir].tuple.src.u.all == htons(SIP_PORT))
			    ){
					/* ct may flushed, and may created by lanside sip server */
				memcpy(&rtp_addr, &ct->tuplehash[!dir].tuple.dst.u3, sizeof(rtp_addr));	
				//printk("%s %d ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all=0x%x, ct->tuplehash[dir].tuple.src.u.all=0x%x, ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all=0x%x, SIP_PORT=0x%x ct->status=0x%x\n", __func__, __LINE__, ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all, ct->tuplehash[dir].tuple.src.u.all, ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all, SIP_PORT, ct->status);
		}
	#endif
