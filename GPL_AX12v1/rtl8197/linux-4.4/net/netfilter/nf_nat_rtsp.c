                 ...
                 struct sk_buff* skb, uint tranoff, uint tranlen)
{
    ...
    int        rbuf1len;       /* Replacement len (one port) */
    char        rbufa[16];      /* Replacement buffer (all ports) */
    int        rbufalen;       /* Replacement len (all ports) */
    u_int32_t   newip;
    u_int16_t   loport, hiport;
    uint        off = 0;
    int        diff;           /* Number of bytes we removed */
        ...
        while (off < nextparamoff)
        {
            ...
            if (dstact != DSTACT_NONE && strncmp(ptran+off, "destination=", 12) == 0)
            {
                ...
                if (dstact == DSTACT_STRIP || (dstact == DSTACT_AUTO && !is_stun))
                {
                	...
                	//skip "destination="
                	dest_addr_start_offset = (ptran - ptcp) + off + 12;
					dest_addr_size = nextfieldoff-off - 12 -1;
					
                    //diff = nextfieldoff-off;
                    diff = dest_addr_size-extaddrlen;

					//printk("%s %d dest_addr_size=%d extaddrlen=%d\n", __func__, __LINE__, dest_addr_size, extaddrlen);
					if (!rtl_nf_nat_mangle_tcp_packet(skb, ct, ctinfo, protoff, 
														dest_addr_start_offset, dest_addr_size, szextaddr, extaddrlen, true))
					#else
                    diff = nextfieldoff-off;
...
static void expected(struct nf_conn* ct, struct nf_conntrack_expect *exp)
{
  ...
  //  mr.range[0].min_ip = mr.range[0].max_ip = newip;
   ...
   range.min_addr = range.max_addr =master->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3;
