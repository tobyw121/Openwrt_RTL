...
enum rt_status	
hal_Associate_8814BE_pe_enable(
...
enum rt_status	
hal_Associate_8814BE_pe_disable(
	 HAL_PADAPTER        Adapter,
    BOOLEAN             IsDefaultAdapter
)
{
	...
	)
{
	...
	odm_set_bb_reg(p_dm_odm, 0x1e14 , BIT26, 1); /*Select primary user power*/	
	...
	)
{
	...
	if ((pBeamformingInfo->beamformee_mu_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		...
		if((pBeamformeeEntry->BeamformEntryCap & BEAMFORM_CAP_VHT_MU_BFER) && pBeamformeeEntry->mu_reg_index == 0xFF)
		{
			...
			h2c_content[4] = Beamforming_GetSTANcIndex(pBeamformeeEntry->pSTA);
			odm_fill_h2c_cmd(p_dm_odm, PHYDM_H2C_MU, 5, h2c_content);
			{
				u4Byte ctrl_info_offset, index;
				u1Byte STA_ANTNUM = 0;
				/*Set Ctrl Info*/
				odm_write_2byte(p_dm_odm, 0x140, 0x660);
				ctrl_info_offset = 0x8000 + 64 * pBeamformeeEntry->MacId;
				/*Reset Ctrl Info*/
				//for (index = 0; index < 8; index++)
				//	odm_write_4byte(p_dm_odm, ctrl_info_offset + index*4, 0);
			
				odm_write_1byte(p_dm_odm, ctrl_info_offset+2, (pBeamformeeEntry->mu_reg_index + 1));
				if (odm_read_1byte(p_dm_odm, ctrl_info_offset+2) == 0) {
					odm_write_1byte(p_dm_odm, ctrl_info_offset+2, (pBeamformeeEntry->mu_reg_index + 1));
					panic_printk("Zero gtID, rewrite it! (gtID: %x, mu_reg_index: %d)\n", odm_read_1byte(p_dm_odm, ctrl_info_offset+2), pBeamformeeEntry->mu_reg_index);
					if (odm_read_1byte(p_dm_odm, ctrl_info_offset+2) == 0) {
						panic_printk("<< Warning >> gtID is still zero !!!\n");
						odm_write_1byte(p_dm_odm, ctrl_info_offset+2, (pBeamformeeEntry->mu_reg_index + 1));
						panic_printk("-> Now gtID=%x, mu_reg_index=%d !!!\n", odm_read_1byte(p_dm_odm, ctrl_info_offset+2), pBeamformeeEntry->mu_reg_index);
					}
				}

				switch (Beamforming_GetSTANcIndex(pBeamformeeEntry->pSTA))
				{
					case 3:
						// 4-antenna is NOT supported
						break;
					case 2:
						STA_ANTNUM = (BIT(5)|BIT(4)) ;
						break;
					case 1:
						STA_ANTNUM = BIT(5);
						break;
				...
				odm_write_1byte(p_dm_odm, ctrl_info_offset+42, (odm_read_1byte(p_dm_odm, ctrl_info_offset+42) & ~(BIT(4)|BIT(5))) | STA_ANTNUM);
			...
			if(pBeamformeeEntry->mu_reg_index < MAX_NUM_BEAMFORMEE_MU ) {
				...
				if(pBeamformingInfo->beamformee_mu_cnt > 0) {
					...
					if (pBeamformingInfo->beamformee_mu_cnt < 2) {
						if (priv->mu_bkup_agg_num) {
							RTL_W16(0x4ca, priv->mu_bkup_agg_num);
							priv->mu_bkup_agg_num = 0;
						}
					}
					#endif
				}
	...
	)
{
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_SOUNDDOWN) {
		...
		value32 = odm_read_4byte(p_dm_odm, REG_BF0_TIME_SETTING_8814B);
    value32 |= (BIT(28) | BIT(29));
    odm_write_4byte(p_dm_odm, REG_BF0_TIME_SETTING_8814B, value32);

    value32 = odm_read_4byte(p_dm_odm, REG_BF1_TIME_SETTING_8814B);
    value32 |= (BIT(28) | BIT(29));
    odm_write_4byte(p_dm_odm, REG_BF1_TIME_SETTING_8814B, value32); 
	} 
#if (MU_BEAMFORMING_SUPPORT == 1)	
	else {
