...
#include <bspchip.h>
#elif defined(CONFIG_RTL_8198F) //Charlie
...
#include <bsp/bspchip.h>
	...
	void  check_pcie_link_status(void)
	{
			...
			if( (REG32(BSP_PCIE1_H_CFG + 0x0728) &0x1f) != 0x11)
			{
...
#if defined(OSK)
//CONFIG_RTL8685SB
        #if defined(CONFIG_RTL8685) || defined(CONFIG_RTL8685SB)
                	REG32(WDTCNR) = WDT_E;
        #else
                	REG32(WDTCNR) = 0;
        #endif
...
#if defined(CONFIG_RTL_8198) || defined(CONFIG_RTL8196C) || defined(CONFIG_RTL_8196C) || defined(CONFIG_RTL_819XD) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8198B) || defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8197G)
#define MAX_PAYLOAD_SIZE_128B    0x00
...
#define PCIE0_RC_EXT_BASE (0xb8b21000)
#define PCIE1_RC_EXT_BASE (0xb8b01000)
#else
...
#ifdef CONFIG_RTL_TRIBAND_001
#define PABCDCNR_REG		(0x000 + GPIO_BASE)     /* Port EFGH control */
#define PABCDDIR_REG		(0x008 + GPIO_BASE)     /* Port EFGH direction */
#define PABCDDAT_REG		(0x00C + GPIO_BASE)     /* Port EFGH data */
#endif

void HostPCIe_SetPhyMdioWrite(unsigned int portnum, unsigned int regaddr, unsigned short val)
{
	#if defined(CONFIG_RTL8685SB) || defined(CONFIG_RTL9607C)
	...
	#define REG_ADDR_MASK 0x1f
        ...
        REG32(mdioaddr)= ( (regaddr&REG_ADDR_MASK)<<PCIE_MDIO_REG_OFFSET) | ((val&0xffff)<<PCIE_MDIO_DATA_OFFSET)  | (1<<PCIE_MDIO_RDWR_OFFSET) ;
        //delay
        for(i=0;i<5555;i++)  ;
	delay_ms(1);
}
...
static void PCIE_Device_PERST(int portnum)
{
	...
	#if defined(CONFIG_RTL_TRIBAND_001)
        // Added By Joseph Chen - GPIO_C0
        REG32(0xb8000110)= (REG32(0xb8000110)&(~(3<<16)))|(3<<16);
        delay_ms(500);
        printk("wifi before reg %X\n", REG32(PABCDCNR_REG));
        REG32(PABCDCNR_REG) &= ~(1<<16); // config as gpio
        printk("wifi after reg %X\n", REG32(PABCDCNR_REG));

        printk("wifi before reg %X\n", REG32(PABCDDIR_REG));
        REG32(PABCDDIR_REG) |= (1<<16);  // direction as output
        printk("wifi after reg %X\n", REG32(PABCDDIR_REG));

        printk("wifi before reg 1 %X\n", REG32(PABCDDAT_REG));
        REG32(PABCDDAT_REG) &= ~(1<<16);
        printk("wifi after reg 1 %X\n", REG32(PABCDDAT_REG));
        delay_ms(500);
        delay_ms(500);
        printk("wifi before reg 2 %X\n", REG32(PABCDDAT_REG));
        REG32(PABCDDAT_REG) |=  (1<<16);   //PERST=1
        printk("wifi after reg 2 %X\n", REG32(PABCDDAT_REG));
	#endif /* defined(CONFIG_RTL_TRIBAND_001) */
	...
	*/
        ...
        delay_ms(500);
        printk("wifi before reg %X\n", REG32(PEFGHCNR_REG));
        REG32(PEFGHCNR_REG) &= ~(1<<0); // config as gpio
        printk("wifi after reg %X\n", REG32(PEFGHCNR_REG));

        printk("wifi before reg %X\n", REG32(PEFGHDIR_REG));
        REG32(PEFGHDIR_REG) |= (1<<0);  // direction as output
        printk("wifi after reg %X\n", REG32(PEFGHDIR_REG));

        printk("wifi before reg 1 %X\n", REG32(PEFGHDAT_REG));
        REG32(PEFGHDAT_REG) &= ~(1<<0);
        printk("wifi after reg 1 %X\n", REG32(PEFGHDAT_REG));
        delay_ms(500);
        delay_ms(500);
        printk("wifi before reg 2 %X\n", REG32(PEFGHDAT_REG));
        REG32(PEFGHDAT_REG) |=  (1<<0);   //PERST=1
        printk("wifi after reg 2 %X\n", REG32(PEFGHDAT_REG));
    #else//ori
...
#if (defined(__LINUX_2_6__) && defined(USE_RLX_BSP)) || defined(__ECOS)
		WRITE_MEM32(BSP_PCIE0_H_CFG + 0x04, 0x00100007);
#if defined(CONFIG_RTL_8198C) & (defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8812FE))
...
/*redefined in 8192cd_cfg.h, so undef the old one*/
#undef PCIE0_H_CFG
#define PCIE0_H_CFG		BSP_PCIE0_H_CFG
#endif //USE_RLX_BSP
...
enum clk_source{
	...
	CLK25_9602C,			//9602C 25M clk
	CLK40_9602C,			//9602C 40M clk
	CLK25_8685SB_P0,                //8685SB 25M clk
	CLK25_8685SB_P1,                //8685SB 25M clk
	CLK25_8685G_P0,
	CLK25_8685G_P1,
	CLK25_9607C_P0,
	CLK25_9607C_P1,
...
#define PHY_PARAM_SIZE 40
struct pcie_para ePHY[][PHY_PARAM_SIZE] = {
    ...
    {{0, 3, 0x3031}, {0, 6, 0xe0b8}, {0, 0xe, 0x98c5}, {0, 0xf, 0x400f}, {0, 0x19, 0xfc70},
     {0xff,0xff,0xffff}}, //9602C 25M clk
	{{0, 3, 0x7b31}, {0, 6, 0xe2b8}, {0, 0xe, 0x98c5}, {0, 0xf, 0x400f}, {0, 0x19, 0xfc70},
     {0xff,0xff,0xffff}}, //9602C 40M clk
	 
    {{0, 0, 0x0011},  {0, 1, 0x0001}, {0, 2, 0x26f6}, {0, 3, 0x4b47},
    {0, 4, 0x8008}, {0, 5, 0x3fe0}, {0, 6, 0x18c1}, {0, 7, 0x842f},
    {0, 8, 0x5ac1}, {0, 9, 0x6380}, {0, 0xa, 0x3248},       {0, 0xb, 0x4bd9},
    {0, 0xc, 0x0800},       {0, 0xd, 0x30d0},       {0, 0xe, 0x0112},       {0, 0x1f, 0x0000},
    {0xff,0xff,0xffff}}, //8685SB 25M clk
	
	{{1, 0, 0x0011},  {1, 1, 0x0001}, {1, 2, 0x26f6}, {1, 3, 0x4b47},
    {1, 4, 0x8008}, {1, 5, 0x3fe0}, {1, 6, 0x18c1}, {1, 7, 0x842f},
    {1, 8, 0x5ac1}, {1, 9, 0x6380}, {1, 0xa, 0x3248},       {1, 0xb, 0x4bd9},
    {1, 0xc, 0x0800},       {1, 0xd, 0x30d0},       {1, 0xe, 0x0112},       {1, 0x1f, 0x0000},
    {0xff,0xff,0xffff}}, //8685SB 25M clk
	
	{	
		{ 0, 0x00, 0x8a51 }, { 0, 0x02, 0x26f9 }, { 0, 0x03, 0x6bcd }, { 0, 0x06, 0x104a }, 		
		{ 0, 0x0b, 0x0009 }, { 0, 0x0c, 0x0800 }, { 0, 0x20, 0x0105 }, { 0, 0x21, 0x1000 },
		{0xff,0xff,0xffff}
	}, // 8685G P0
	{		
		{ 1, 0x00, 0x8a51 }, { 1, 0x02, 0x26f9 }, { 1, 0x03, 0x6bcd }, { 1, 0x06, 0x104a }, 		
	...
	{		
		{ 1, 0x00, 0x8a50 }, { 1, 0x02, 0x26f9 }, { 1, 0x03, 0x6bcd }, { 1, 0x04, 0x8049 },
		{ 1, 0x06, 0x1088 }, { 1, 0x07, 0x52b3 }, { 1, 0x08, 0x5281 }, { 1, 0x09, 0x6300 },
		{ 1, 0x0b, 0x0009 }, { 1, 0x0c, 0x0800 }, { 1, 0x20, 0x0105 }, { 1, 0x21, 0x1000 },
		{0xff,0xff,0xffff}
	} // 9607C P1
...
static void __pcie_param_fixup(void) {	
	uint32  chipId,rev,subType  = 0;
    rtk_switch_version_get(&chipId,&rev,&subType);
	if(rev > CHIP_REV_ID_A) {
		printk("IC-B\n");
		if (ePHY[CLK25_9607C_P0][0].port==0) {
			struct pcie_para g2phy[PHY_PARAM_SIZE] = {
			...
			BUG_ON(sizeof(ePHY[CLK25_9607C_P0])!=sizeof(g2phy));			
			memcpy(ePHY[CLK25_9607C_P0], g2phy, sizeof(g2phy));
		}
	} else {
		printk("IC-A\n");
	}
...
#if defined(CONFIG_RTL_TRIBAND_SUPPORT) && defined(CONFIG_RTL9607C)
void bsp_pci_phy_reset_9607c(int port) {
        u32 mrst;
        switch (port) {
        case 0:
...
#endif /* defined(CONFIG_RTL_TRIBAND_SUPPORT) && defined(CONFIG_RTL9607C) */
...
#if 0
/*temp define for fpga board.*/
        #ifdef CONFIG_USE_PCIE_SLOT_1
	if (portnum==1)
		PCIE_gpio_RST = GPIO_B_5;
	else
    #endif
...
RETRY:
...
#endif
	delay_ms(10);
#ifdef CONFIG_RTL9607C /* use bsp provided reset */
	do {
		extern void bsp_pci_phy_reset(int port);
		pcie_reset_done[portnum]=1; /* stop other rst logic below */
     #if defined(CONFIG_RTL_TRIBAND_SUPPORT)
		bsp_pci_phy_reset_9607c(portnum);
     #else
		bsp_pci_phy_reset(portnum);
     #endif
	} while (0);
#endif 
...
#if defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1) || defined(CONFIG_RTL9607C)
	...
	if (mdio_reset)
	{
		if ((IS_RLE0315 ||IS_6166) && (REG32(MISC_PINSR) & CLKSEL))
			idx = CLK35_328_6166;
		else if ((IS_RLE0315 ||IS_6166) && !(REG32(MISC_PINSR) & CLKSEL))
			idx = CLK25_6166;
...
#if defined(CONFIG_PHY_WLAN_EAT_40MHZ) || defined(CONFIG_PHY_EAT_40MHZ)
			idx = CLK40_9602C;
#else
			idx = CLK25_9602C;
#endif
			...
			if (portnum==0) {
				printk("8685g pcie port 0\n");
				idx = CLK25_8685G_P0;
			} else {
				printk("8685g pcie port 1\n");
				idx = CLK25_8685G_P1;
			}
		}
		else if(IS_RLE0797) {
			if (portnum==0) {
				printk("8685sb pcie port 0\n");
				idx = CLK25_8685SB_P0;
			} else {
				printk("8685sb pcie port 1\n");
				idx = CLK25_8685SB_P1;
			}
		}
#endif
			...
			if (portnum==0) {
				printk("9607c pcie port 0\n");
				idx = CLK25_9607C_P0;
			} else {
				printk("9607c pcie port 1\n");
				idx = CLK25_9607C_P1;
			}
		}
#endif
	...
	REG32(PCIE_H_CFG + 0x80C) |= (1<<17);	
	do {
		u16 status;
		const char *str;
		mdelay(1);
		status = REG16(PCIE_H_CFG + 0x82);
...
#ifdef __OSK__
	//io and mem limit, setting to no litmit
	//jim 20160617 don't forget the dual band
    REG32((PCIE_H_CFG+0x1C)/*0xb8b0001c*/) = (2<<4) | (0<<12);   //  [7:4]=base  [15:12]=limit
    REG32((PCIE_H_CFG+0x20)/*0xb8b00020*/) = (2<<4) | (0<<20);   //  [15:4]=base  [31:20]=limit 
    REG32((PCIE_H_CFG+0x24)/*0xb8b00024*/) = (2<<4) | (0<<20);   //  [15:4]=base  [31:20]=limit 
    //printk("reset PCIE 2: b8b0001c:0x%08x, b8b00020:0x%08x, b8b00024:0x%08x\n", REG32(0xb8b0001c), REG32(0xb8b00020), REG32(0xb8b00024));
#endif

SET_BAR:
#if defined(CONFIG_RTL8685SB)
	if(IS_RLE0797||IS_RLE0822) {
		REG32(PCIE_D_CFG0+0x78) &= ~((0x7<<5)|(0x7<<12));
	}
#endif
...
#if defined(CONFIG_RTL8685SB) && (defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE))
		if (IS_RLE0797||IS_RLE0822){
			WRITE_MEM8(PCIE_H_CFG + 0x78, (READ_MEM8(conf_addr + 0x78) & (~0xE0)) | 0x20);  // Set MAX_PAYLOAD_SIZE to 256B
		} else 
#endif
		{
...
#if (defined(__ECOS) || defined(__OSK__)) && defined(CONFIG_RTL_8197F)
