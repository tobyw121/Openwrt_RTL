...
void rtw_init_recvbuf(struct rtl8192cd_priv *priv, struct recv_buf *precvbuf)
{
    precvbuf->transfer_len = 0;
    precvbuf->len = 0;
    precvbuf->ref_cnt = 0;
...
int rtw_init_recv_priv(struct rtl8192cd_priv *priv)
{
    struct recv_priv *precvpriv = &priv->recvpriv;
    int i, res = SUCCESS;
    ...
    tasklet_init(&precvpriv->recv_tasklet,
         (void(*)(unsigned long))rtw_recv_tasklet,
         (unsigned long)priv);

#if defined(CONFIG_USB_INTERRUPT_IN_PIPE)
...
#if defined(CONFIG_PREALLOC_RECV_SKB)
    {
        int i;
        SIZE_PTR tmpaddr=0;
...
#endif /* defined(CONFIG_PREALLOC_RECV_SKB) */
...
void rtw_free_recv_priv (struct rtl8192cd_priv *priv)
{
    int i;
    struct recv_buf *precvbuf;
    struct recv_priv *precvpriv = &priv->recvpriv;
...
#if 1//defined(CONFIG_RTL_88E_SUPPORT)
                ...
                for (i = 0; i < priv->pshare->nr_vap_bcn; ++i) {
                    if (status_ext & (HIMRE_88E_BCNDMAINT1 << i)) {
                        ...
                        if (val8 & 0x01) {
                            if (RTL_R8(BCN_CTRL) & DIS_ATIM)
                                RTL_W8(BCN_CTRL, (RTL_R8(BCN_CTRL) & (~DIS_ATIM)));
                            process_mcast_dzqueue_usb(target_priv);
...
#if defined(DBG_CONFIG_ERROR_DETECT_INT)
...
#endif /* defined(DBG_CONFIG_ERROR_DETECT_INT) */
...
#endif /* defined(CONFIG_SUPPORT_USB_INT) */
#endif /* defined(CONFIG_RTL_88E_SUPPORT) */
...
void rtl8192c_query_rx_desc_status(union recv_frame *precvframe, struct recv_stat *pdesc)
{
    ...
    /* Offset 12 */
#if defined(CONFIG_TCP_CSUM_OFFLOAD_RX)
...
#endif /* defined(CONFIG_RTL_92C_SUPPORT) */
...
#endif /* defined(CONFIG_USB_INTERRUPT_IN_PIPE) */
    ...
    do {
        ...
        if (pattrib->pkt_rpt_type == NORMAL_RX) { /* Normal rx packet */
            ...
            if (unlikely(pattrib->pkt_rpt_type == TX_REPORT1)) {
                ...
                //printk("rx TX RPT\n");
#if defined(TXREPORT)
#if defined(RATEADAPTIVE_BY_ODM)
...
_func_enter_;
    ...
    rtw_init_recvbuf(priv, precvbuf);
...
#else   /* !defined(CONFIG_USE_USB_BUFFER_ALLOC_RX) */
    ...
    do {
        ...
        if (pattrib->pkt_rpt_type == NORMAL_RX) { /* Normal rx packet */
            ...
            if (pattrib->pkt_rpt_type == C2H_PKT) {
                GET_HAL_INTERFACE(priv)->C2HPacketHandler(priv, pkt_copy->data, pattrib->pkt_len);
            }
...
#endif
{
    ...
    if (purb->status==0) { /* SUCCESS */
	#if defined(DBG_TAILROOM)
		int queue_tail = 0;
	#endif
        if ((purb->actual_length > MAX_RECVBUF_SZ) || (purb->actual_length < RXDESC_SIZE)) {
	#if defined(DBG_TAILROOM)
            if (net_ratelimit()) printk("usb_read_port_complete: (purb->actual_length(%d) > MAX_RECVBUF_SZ) || (purb->actual_length(%d) < RXDESC_SIZE)\n", purb->actual_length, purb->actual_length);
	#else
		...
		#if defined(DBG_TAILROOM)		
			if (unlikely(skb_tailroom(precvbuf->pskb) < purb->actual_length)) {
				if (net_ratelimit()) printk("usb_read_port_complete: tailroom(%d) < purb->actual_length(%d)\n", skb_tailroom(precvbuf->pskb), purb->actual_length);
				queue_tail = 1;
			} else {
...
#if 1
#if defined(DBG_TAILROOM)
#define MAX_RX_QLEN NR_PREALLOC_RECV_SKB
#else
#define MAX_RX_QLEN 600
#endif
			if (skb_queue_len(&precvpriv->free_recv_skb_queue) == 0 && skb_queue_len(&precvpriv->rx_skb_queue)+skb_queue_len(&precvpriv->free_recv_skb_queue) >= MAX_RX_QLEN) {
		#if defined(DBG_TAILROOM)
				queue_tail = 1;
			}
			if (queue_tail == 1) {
		#endif
				//if (net_ratelimit()) printk("%s: drop\n", __func__);
...
_func_enter_;
    ...
    if (NULL == precvbuf->pskb) {
		...
		unsigned long flags;
		struct sk_buff *skb;
		struct sk_buff_head *list = (struct sk_buff_head *)(&precvpriv->free_recv_skb_queue);

		spin_lock_irqsave(&list->lock, flags);
		skb = skb_peek(list);
		if (skb && skb->next && (unsigned int)(skb->next)%4 == 0) {
			__skb_unlink(skb, list);
		}
		else {
		#if 0//get from tail
			if (net_ratelimit()) printk("%s:%d, dequeue_tail\n", __func__, __LINE__);
			skb = skb_peek_tail(list);
			if (skb)
				__skb_unlink(skb, list);
		#else
			printk("next(%s), next_align(%s)\n", skb->next?"ok":"null", ((int)(skb->next)%4)?"false":"true");
...
#if defined(DBG_TAILROOM) 
{
	struct sk_buff *skb = precvbuf->pskb;
	if (skb->tail != skb->data || skb_tailroom(skb) < MAX_RECVBUF_SZ) {
		printk("check_tail(%d), ((skb->tail != skb->data))(%s), skb_tailroom(%d)\n", check_tail, (skb->tail != skb->data)?"treu":"false", skb_tailroom(skb));
		if (check_tail == 0) {			
			check_tail = 1;
			precvbuf->pskb = NULL;
			goto ALLOC_AGAIN;
...
unsigned int rtw_inirp_init(struct rtl8192cd_priv *priv)
{
    u8 i;
    struct recv_buf *precvbuf;
    uint status;
...
unsigned int rtw_inirp_deinit(struct rtl8192cd_priv *priv)
{
    printk("\n ===> usb_rx_deinit \n");
