...
int rtk_multi_ap_get_sta_info(struct rtl8192cd_priv *priv, unsigned char *tlv_buffer, const unsigned short limit, int *processed_bytes)
{
	int ret = -1;
	struct list_head *phead, *plist;
	struct stat_info *pstat;
	unsigned char *p = NULL;
	unsigned char sta_size = 0, sta_num = 0;
	...
	if (!(priv->drv_state & DRV_STATE_OPEN)) {
		printk("[%s]%s device error(%u)\n", __FUNCTION__, priv->dev->name, priv->drv_state);
		goto fail;
	}
	...
	*processed_bytes = 1;
	...
	rtw_list_for_each(plist, phead) {
		...
		if ((pstat->state & WIFI_ASOC_STATE) && (pstat->expire_to > 0)
			...
			if((*processed_bytes + sta_size) > limit) {//to avoid overflow
				printk("[%s]Buffer is not enough\n", __FUNCTION__);
#ifdef SMP_SYNC
				SMP_UNLOCK_ASOC_LIST(flags);
#endif
				goto fail;
			}
			memcpy(p, pstat->cmn_info.mac_addr, 6);
			p += ETH_ALEN;

			link_time = pstat->link_time;
			memcpy(p, &link_time, 4);
			p += 4;

			*p = pstat->multiap_profile;
			p++;
