...
void imp_membership_db_cleanup(imp_membership_db *p_md)
{
    ...
    while (p_mfc)
    {
        ...
        {
            printErr("Unsupported group type, p_md->type=%d\n", p_md->type);
        }
        else
        {
            if (p_md->pig.ss.ss_family == AF_INET)
            {
                bcm_mcast_update_igmp_snoop((const struct in_addr *)&p_md->pig.v4.sin_addr,
                    (const struct in_addr *)&p_md->pig.v4.sin_addr,
                    (struct in_addr *)&p_mfc->pia.v4.sin_addr,
                    MCPD_SNOOP_EX_CLEAR, p_mfc->iif_index);

                bcm_mcast_update_igmp_snoop((const struct in_addr *)&p_md->pig.v4.sin_addr,
                    (const struct in_addr *)&p_md->pig.v4.sin_addr,
                    (struct in_addr *)&p_mfc->pia.v4.sin_addr,
                    MCPD_SNOOP_IN_CLEAR, p_mfc->iif_index);
            }

#ifdef ENABLE_IMP_MLD
            else if (p_md->pig.ss.ss_family == AF_INET6)
            {
                bcm_mcast_update_mld_snooping((const struct in6_addr *)&p_md->pig.v6.sin6_addr,
                    (struct in6_addr *)&p_mfc->pia.v6.sin6_addr,
                    MCPD_SNOOP_EX_CLEAR, p_mfc->iif_index);
...
void imp_membership_db_mfc_add(int iif_index, pi_addr *p_ga, pi_addr *p_src, if_set *p_ttls)
{
...
#ifdef BCM_MCAST_SUPPORT
    int mode = 0;
    BOOL source_is_wan = TRUE;
    imp_interface *p_if = imp_interface_first();
    while(p_if)
    {
        if (p_if->if_index == iif_index && p_if->type == INTERFACE_DOWNSTREAM)
        {
            source_is_wan = FALSE;
            break;
        }
	...
	if (imp_membership_db_mfc_find(p_md, p_src) != NULL)
	{
		IMP_LOG_INFO("group %s's memship db existed\n", imp_pi_ntoa(p_ga));
		return ;
	}
    ...
    mode = p_md->type == GROUP_INCLUDE ? MCPD_SNOOP_IN_ADD : MCPD_SNOOP_EX_ADD;

    if (p_md->pig.ss.ss_family == AF_INET &&
        bcm_mcast_update_igmp_snoop((const struct in_addr *)&p_ga->v4.sin_addr,
        (const struct in_addr *)&p_ga->v4.sin_addr,
        (struct in_addr *)&p_src->v4.sin_addr,
        mode, iif_index) < 0)
    {
        printErr("bcm_mcast_update_igmp_snoop error...add group:%s src:%s\n", imp_pi_ntoa(p_ga), imp_pi_ntoa(p_src));
    }

#ifdef ENABLE_IMP_MLD
    else if (p_md->pig.ss.ss_family == AF_INET6 &&
        bcm_mcast_update_mld_snooping((const struct in6_addr *)&p_ga->v6.sin6_addr,
        (struct in6_addr *)&p_src->v6.sin6_addr,
        mode, iif_index) < 0)
    {
        printErr("bcm_mcast_mld_update_snoop error...add group:%s src:%s\n", imp_pi_ntoa(p_ga), imp_pi_ntoa(p_src));
    }
#endif
#endif
}
...
void imp_membership_db_update(pi_addr *p_ga)
{
    ...
    for (p_if = imp_interface_first(); p_if; p_if = LIST_NEXT(p_if, link))
    {
        imp_group  *p_gp = NULL;
#ifdef BCM_MCAST_SUPPORT
        if(p_if->if_addr.ss.ss_family != p_ga->ss.ss_family)
        {
            continue;
        }
#endif
...
#ifdef BCM_MCAST_SUPPORT
                    if(!TIMEVAL_ISZERO(p_is->timer->tm) && !TIMEVAL_ISZERO(p_gp->timer->tm))
                        continue;
#endif
    ...
    if (p_md->type == GROUP_UNDEFINED) {
        ...
        if (p_md->src_list)
        {
            k_mcast_source_leave(&p_md->pig, NULL, &p_md->src_list->addr);
            k_mcast_msfilter(&p_md->pig, p_md->src_list, p_md->type, NULL);
        }
        else
