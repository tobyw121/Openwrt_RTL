...
static STATUS load_config(const char *file)
{
    FILE    *fd;
    char    buffer[1024] = {0};
    char    downinf[10][IFNAMSIZ], upinf[MAX_UP_INF_NUM][IFNAMSIZ];
    int     down_num, up_num, i;

    down_num = 0;
    up_num = 0;
    memset(downinf, 0, 10 * IFNAMSIZ);
    memset(upinf, 0, MAX_UP_INF_NUM * IFNAMSIZ);
    ...
    while(fgets(buffer, 1024, fd))
    {
        ...
        if (strcmp(p_token, "igmp") == 0) {
    ...
    #endif
        else if (strcmp(p_token, "snooping") == 0) {
#ifdef BCM_MCAST_SUPPORT
            if (strcmp(p_value, "off") == 0) {
                mproxy.snooping = IMP_DISABLE;
            }
            else if (strcmp(p_value, "on") == 0){
                mproxy.snooping = IMP_ENABLE;
            }
            else {
                IMP_LOG_ERROR("error value %s, should be on or off\n", p_value);
                return STATUS_NOK;
            }
#endif
        }
            ...
            if (up_num >= MAX_UP_INF_NUM) {

                IMP_LOG_ERROR("only allow %d upstream interfaces\n", MAX_UP_INF_NUM);
                return STATUS_NOK;
            }
            strncpy(upinf[up_num], p_value, IFNAMSIZ);
            up_num++;
    ...
    if (upinf[0][0] == 0) {
        ...
        printf("improxy must be specified  at least one upstream interface\n");
        return STATUS_NOK;
    }
    if(mproxy.igmp_version != IM_DISABLE) {

        for (i = 0; i < up_num; i++) {
            imp_interface_create(upinf[i], AF_INET, INTERFACE_UPSTREAM);
        }
    ...
    if(mproxy.mld_version != IM_DISABLE) {
        ...
        for (i = 0; i < up_num; i++) {
            imp_interface_create(upinf[i], AF_INET6, INTERFACE_UPSTREAM);
        }
        for (i = 0; i < down_num; i++) {
...
int get_up_if_index(int index)
{
    if (index < 0 || index > MAX_UP_INF_NUM)
    {
        return -1;
    }
    return mproxy.upif_index[index];
}

int is_up_if_index(int if_index)
{
    int i;
    for (i = 0; i < MAX_UP_INF_NUM; i++) {
        if (mproxy.upif_index[i] == 0) {
            break;
        }
        if (mproxy.upif_index[i] == if_index) {
            return 1;
        }
    }
    return 0;
...
#ifdef RTL_MCAST_SUPPORT
/* 
 * 1: when igmp proxy open, bridge(br-lan) querier close
 * 0: when igmp proxy close, bridge querier open
 */
#define BR_LAN_QUERIER_TOGGLE "/proc/br_igmpProxy"
void open_br_querier(void)
{
	int fd=open(BR_LAN_QUERIER_TOGGLE, O_RDWR|O_TRUNC);
	char open = '0';
	if ( fd < 0 )
		goto out;

	//system("echo gonna exit, open br_querier now >> /dev/console");
	write(fd, &open, sizeof(open));
	close(fd);
...
#else
#define BR_LAN_QUERIER_TOGGLE "/sys/class/net/br-lan/bridge/multicast_querier"
void open_br_querier(void)
{
	int fd=open(BR_LAN_QUERIER_TOGGLE, O_RDWR|O_TRUNC);
	char open = '1';
...
#ifdef BCM_MCAST_SUPPORT
    if(mcpd_netlink_init() != STATUS_OK)
    {
        printErr("mcpd_netlink_init failed\n");
    }
...
#ifdef BCM_MCAST_SUPPORT
        if(mproxy.mcpd_netlink_socket > 0)
            FD_SET(mproxy.mcpd_netlink_socket, &fdset);
        if(max < mproxy.mcpd_netlink_socket)
            max = mproxy.mcpd_netlink_socket;
#endif
...
#ifdef BCM_MCAST_SUPPORT
        if (mproxy.mcpd_netlink_socket > 0 &&
            FD_ISSET(mproxy.mcpd_netlink_socket, &fdset)) {
