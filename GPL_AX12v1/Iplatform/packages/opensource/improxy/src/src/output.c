    ...
    pi_addr *p_dst, pi_addr *p_gp_addr, imp_source* p_sources, int sflag)
{
	...
	/*
	*	Hanjiayan@20190228 add,
	*	For sometimes, the query package is send out by other ipv6 source address
	*/
	bzero(&saddr,sizeof(saddr));
	saddr.sin6_family=AF_INET6;
	saddr.sin6_addr = p_if->if_addr.v6.sin6_addr;
	saddr.sin6_scope_id = p_if->if_index;
	if(bind(socket,(struct sockaddr*)&saddr,sizeof(saddr)) < 0)
		IMP_LOG_ERROR("bind fail %s\n", strerror(errno));
