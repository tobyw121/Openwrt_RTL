...
STATUS init_interface(mcast_proxy *p_mproxy)
{
    ...
    for(p_if = LIST_FIRST(&ifp_head);p_if;p_if = ifp_next)
    {
        ...
        if (p_if->type == INTERFACE_UPSTREAM) {
            if_exist = 0;
            for (i = 0; i < up_num; i++) {
                if ( p_mproxy->upif_index[i] == p_if->if_index) {
                    if_exist = 1;
                    break;
                }
            }
            if ( !if_exist ) {
                p_mproxy->upif_index[up_num++] = p_if->if_index;
            }
        }
    ...
    pa_list* p_src_list, group_type type, im_version version)
{
    ...
    if (p_src_list && version == IM_IGMPv3_MLDv2)
    {
        k_mcast_source_join(&p_gp->group_addr, NULL, &p_src_list->addr);
        k_mcast_msfilter(&p_gp->group_addr, p_src_list, type, NULL);
    }
    else {
