    ...
    pi_addr *p_group, int iif_index)
{
    ...
    while(p_if) {
        if (
#ifndef BCM_MCAST_SUPPORT
            p_if->type != INTERFACE_UPSTREAM &&
#endif
    ...
    if (get_down_to_up_enable() == 1 && !is_up_if_index(iif_index)) {

            for (i = 0; i < MAX_UP_INF_NUM; i++) {
                up_index = get_up_if_index(i);
                if (up_index != 0) {
                    vmif = k_get_vmif(up_index, p_group->ss.ss_family);
            IF_SET(vmif, p_ttls);
                }
            }
...
/* Russia ISP planeta will use a local scope ip as a IPTV multicast source, so out dut must deal with it.  */
#if 0
...
#ifdef BCM_MCAST_SUPPORT
    mode = igh->type == IGMP_HOST_LEAVE_MESSAGE ? BCM_MCAST_IN_CLEAR : BCM_MCAST_EX_ADD;
    bcm_mcast_update_snooping(&pig, (pa_list*)NULL, mode);
#endif
...
#ifdef BCM_MCAST_SUPPORT
    mode = p_mldhr->mld_type == MLD_LISTENER_REPORT ? BCM_MCAST_EX_ADD : BCM_MCAST_IN_CLEAR;
    bcm_mcast_update_snooping(&pig, (pa_list*)NULL, mode);
#endif
    imp_membership_db_update(&pig);
    return;
...
#ifdef BCM_MCAST_SUPPORT
        bcm_mcast_update_snooping(&pig, p_src_list, p_grec->grec_type);
#endif
        imp_membership_db_update(&pig);
        pa_list_cleanup(&p_src_list);
...
#ifdef BCM_MCAST_SUPPORT
        bcm_mcast_update_snooping(&pig, p_src_list, p_rec->rec_type);
#endif
        imp_membership_db_update(&pig);
        pa_list_cleanup(&p_src_list);
...
void mcast_recv_mld(int sockfd, int version)
{
	...
	mif = k_get_vmif(get_up_if_index(0), AF_INET6);
        iif_index = k_get_rlif(mif, AF_INET6);
        IMP_LOG_DEBUG("mif = %d iif_index =%d im6_mif= %d\n", mif, iif_index, p_mrmsg->im6_mif);
...
void mcast_recv_igmp(int sockfd, int version)
{
    ...
    ip = (struct iphdr*)buf;
	if(p_if == NULL){
		if (if_index == 0 && ip->protocol == 0)
		{
                        p_igm = (struct igmpmsg*)buf;
                        rlif = k_get_rlif(p_igm->im_vif, AF_INET);
			for(p_if = imp_interface_first();p_if;p_if = LIST_NEXT(p_if, link))
			{
				if(
#ifndef BCM_MCAST_SUPPORT
                p_if->type == INTERFACE_UPSTREAM &&
#endif 
                p_if->if_addr.ss.ss_family == AF_INET && p_if->if_index == rlif)
				{
					if_index = p_if->if_index;
					break;
				}
			}
		}
