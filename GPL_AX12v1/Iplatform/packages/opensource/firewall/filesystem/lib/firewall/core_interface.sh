...
fw_configure_interface() {
	...
	[ "$action" == "add" ] && {
		...
		#if iface is wan,zone-wan firewall should exist no matter if it is up
		[ "$iface" == "lan" ] || [ "$iface" == "wan" ] || [ "$status" == 1 ] || [ -n "$aliasnet" ] || return 0
	}
	...
	[ -n "$ifname" ] || {
        ...
        #if iface is lan,the bridge type ifname should always be br-lan
	[ "$iface" == "lan" ] &&{
		[ "$ifname" == "br-lan" ] || {
		local type=$(uci get network.lan.type)
		[ "$type" == "bridge" ] && {
		ifname="br-lan"
		}
		}
	...
	fw__do_rules() {
		...
		local rtl_hqos_support=$(uci get profile.@qos[0].rtl_hqos_support -c "/etc/profile.d" -q)
		...
		[ "$ifname" == "dslt-wan" -o "$ifname" == "v6plus-wan" ] && mss_rule="--set-mss 1420"
		[ "$(uci_get_state firewall core "${zone}_tcpmss")" == 1 ] && \
			fw $action $mode m ${chain}_MSSFIX TCPMSS $ \
				{ -o "$ifname" -p tcp --tcp-flags SYN,RST SYN "$mss_rule" $onet } && \
			fw $action $mode m ${chain}_MSSFIX TCPMSS $ \
                		{ -i "$ifname" -p tcp --tcp-flags SYN,RST SYN "$mss_rule" $onet }

		fw $action $mode f input   ${chain}         $ { -i "$ifname" $inet }
		fw $action $mode f forward ${chain}_forward $ { -i "$ifname" $inet }
		# for rtl hw_qos
		if [[ "${rtl_hqos_support}" == "yes" ]]; then
			if [[ "${zone}" == "wan" ]]; then
				fw $action $mode m POSTROUTING ${chain}_qos { -o "br-lan" $inet}
			elif [[ "${zone}" == "lan" ]]; then
				fw $action $mode m PREROUTING ${chain}_qos { -i "br-lan" $inet}
			else
				echo "no qos rules for zone ${zone}" > /dev/console
			fi
		else
			fw $action $mode m FORWARD ${chain}_qos $ { -i "$ifname" $inet }
		fi
		[ "$ifname" == "dslt-wan" ] || fw $action $mode n PREROUTING ${chain}_prerouting $ { -i "$ifname" $inet }               
		fw $action $mode r PREROUTING ${chain}_notrack    $ { -i "$ifname" $inet }
		[ "$ifname" == "dslt-wan" ] || {
			[ "$ifname" == "wg_c" -a "$action" == "add" -a "$(uci get network.vpn.nat)" != "on" ] || {
		...
		[ "$zone" = "lan" ] && {
			fw $action $mode f forwarding_lan ACCEPT $ { -o "$ifname" }
		}
