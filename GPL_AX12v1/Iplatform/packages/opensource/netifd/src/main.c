...
#define DEFAULT_LOG_LEVEL L_EMERG
...
int main(int argc, char **argv)
{
	...
	if (netifd_ubus_init(socket) < 0) {
		netifd_log_message(L_ERROR, "Failed to connect to ubus\n");
		return 1;
	}
	...
	if (system_init()) {
		netifd_log_message(L_ERROR, "Failed to initialize system control\n");
		return 1;
	}
    ...
    if (connect_init() != 0) {
        netifd_log_message(L_ERROR, "Failed to initialize connect control\n");
        return 1;
    }
