...
static const struct blobmsg_policy route_attr[__ROUTE_MAX] = {
	...
	[ROUTE_PRE] = { .name = "route_pre", .type = BLOBMSG_TYPE_BOOL },
};
			    ...
			    struct vlist_node *node_old)
{
	...
	if (node_new) {
		...
		if ((a_new->flags & DEVADDR_FAMILY) == DEVADDR_INET4 &&
			...
			{
				uint32_t b = ntohl(*a);
				if (0 == (b & 0x80000000))      /*A type IP addr*/
					mask >>= 8;
				else if (0 == (b & 0x40000000)) /*B type IP addr*/
					mask >>= 16;
				else                      /*C type IP addr*/
					mask >>= 24;
			}
...
static void
system_default_route_fallback(struct device *dev, struct device_route *route_old)
{
	bool have_gw;
	struct interface *iface, *parent_if = NULL;
	struct device_route *route_new = NULL;
	...
	if(strcmp(dev->ifname, "pptp-internet") &&
		strcmp(dev->ifname, "l2tp-internet")) 
	{
		return;
	}

	if ((route_old->flags & DEVADDR_FAMILY) != DEVADDR_INET4)
	{
		return;
	}

	have_gw = !!route_old->nexthop.in.s_addr;
	if(!have_gw) 
	{
		return;
	}

	parent_if = vlist_find(&interfaces, "wan", iface, node);
	if(!parent_if) 
	{
		return;
	}

	parent_dev = parent_if->l3_dev.dev;
	if(!parent_dev) 
	{
		return;
	}

	vlist_for_each_element(&parent_if->proto_ip.route, route_new, node) 
	{
		if(route_new->enabled && 
		((route_new->flags & DEVADDR_FAMILY) == DEVADDR_INET4) &&
		!!route_new->nexthop.in.s_addr ) 
		{
			system_add_route(parent_dev, route_new);
		}
	}
}
			     ...
			     struct vlist_node *node_old)
{
	...
	if (node_old) {
		if (!(route_old->flags & DEVADDR_EXTERNAL) && route_old->enabled && !keep)
		{
			...
			system_default_route_fallback(dev, route_old);
#ifdef FEATURE_3G4G_MODULES
			unset_nw(get_network_type(iface->name));
#endif			
		}
		free(route_old);
	}
...
#ifdef FEATURE_3G4G_MODULES
			if (((route_new->flags & DEVADDR_FAMILY) == DEVADDR_INET4) &&
				route_new->nexthop.in.s_addr)
			{
				set_ubus_curr_nw(get_network_type(iface->name));
			}
#endif
		}
...
#if FEATURE_3G4G_MODULES == 2
static void specified_interface_write_resolv_conf(struct interface *iface, FILE *f)
{
	if(iface == NULL || f == NULL)
	{
		return;
	}
	if (iface->state != IFS_UP || (
		vlist_simple_empty(&iface->proto_ip.dns_search) &&
...
interface_write_resolv_conf(void)
{
	struct interface *iface;
#if FEATURE_3G4G_MODULES == 2
	struct interface *wan_iface = NULL, *internet_iface = NULL, *mobile_iface = NULL;
#endif
...
#if FEATURE_3G4G_MODULES == 2
	if(get_backup_enable())
	{
		if(get_curr_nw() == NETWORK_TYPE_WIRED)
		{
			internet_iface = vlist_find(&interfaces, "internet", iface, node);
			specified_interface_write_resolv_conf(internet_iface, f);
			wan_iface = vlist_find(&interfaces, "wan", iface, node);
			specified_interface_write_resolv_conf(wan_iface, f);	
		}
		else
		{
			mobile_iface = vlist_find(&interfaces, "mobile", iface, node);
			specified_interface_write_resolv_conf(mobile_iface, f);
		}
	}
#endif
	vlist_for_each_element(&interfaces, iface, node) {
		if (iface->state != IFS_UP)
			continue;
#if FEATURE_3G4G_MODULES == 2
		if (iface == wan_iface || iface == internet_iface || iface == mobile_iface)
		{
			continue;
		}
#endif	
		/*add for vpn client dns split. zhu haiming 2019.4.10*/
		if (0 == strcmp(iface->name, "vpn"))
		{
			continue;
		}
