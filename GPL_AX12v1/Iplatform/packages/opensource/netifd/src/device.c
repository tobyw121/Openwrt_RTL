...
static const struct blobmsg_policy dev_attrs[__DEV_ATTR_MAX] = {
	...
	[DEV_ATTR_KEEPUP] = { .name = "keepup", .type = BLOBMSG_TYPE_BOOL },
};
...
device_init_settings(struct device *dev, struct blob_attr **tb)
{
    ...
    dev->keepup = true;
	if ((cur = tb[DEV_ATTR_KEEPUP]))
        dev->keepup = blobmsg_get_bool(cur);
...
device_get(const char *name, int create)
{
	...
	/* add by wanghao  */
	/* fix bug 408684, br-wan should be treated as other br-interface, otherwise when reload network, ip in br-wan will disappear  */
	/*
	if (strncmp(name, "br-wan", 6) == 0)
		return NULL;
	*/
	/* add end  */
#endif
...
device_dump_status(struct blob_buf *b, struct device *dev)
{
	...
	//blobmsg_add_u8(b, "link", !!dev->link_active);
