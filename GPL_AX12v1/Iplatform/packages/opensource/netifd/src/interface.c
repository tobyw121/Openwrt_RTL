...
enum {
	...
	IFACE_ATTR_IP6HINT,
    IFACE_ATTR_CONNECTABLE,
    IFACE_ATTR_CONN_MODE,
    IFACE_ATTR_IDLE_TIME,
    IFACE_ATTR_START_TIME,
    IFACE_ATTR_END_TIME,
	IFACE_ATTR_MAX
};
...
static const struct blobmsg_policy iface_attrs[IFACE_ATTR_MAX] = {
	...
	[IFACE_ATTR_PARENT] = { .name = "parent", .type = BLOBMSG_TYPE_STRING },
	[IFACE_ATTR_IP6ASSIGN] = { .name = "ip6assign", .type = BLOBMSG_TYPE_INT32 },
	[IFACE_ATTR_IP6HINT] = { .name = "ip6hint", .type = BLOBMSG_TYPE_STRING },
    [IFACE_ATTR_CONNECTABLE] = { .name = "connectable", .type = BLOBMSG_TYPE_BOOL },
    [IFACE_ATTR_CONN_MODE] = { .name = "conn_mode", .type = BLOBMSG_TYPE_STRING },
    [IFACE_ATTR_IDLE_TIME] = { .name = "idle_time", .type = BLOBMSG_TYPE_INT32 },
    [IFACE_ATTR_START_TIME] = { .name = "start_time", .type = BLOBMSG_TYPE_INT32 },
    [IFACE_ATTR_END_TIME] = { .name = "end_time", .type = BLOBMSG_TYPE_INT32 },
};
...
#ifdef FEATURE_3G4G_MODULES		
		if(strcmp(iface->name, mobileIfaceName) == 0)
			{
				system("/usr/lib/modem/modemLedCtrl.sh 1");	
			}
#endif		
		break;
	case IFPEV_DOWN:
...
static void
interface_set_conn_mode(struct interface *iface, const char *mode)
{
    iface->conn_mode = IFCM_INVALID;
    if (strcmp(mode, "auto") == 0) {
        iface->conn_mode = IFCM_AUTO;
    } else if (strcmp(mode, "demand") == 0) {
        iface->conn_mode = IFCM_DEMAND;
    } else if (strcmp(mode, "manual") == 0 || strcmp(mode, "manually") == 0) {
        iface->conn_mode = IFCM_MANUAL;
    } else if (strcmp(mode, "timebased") == 0 || strcmp(mode, "time_based") == 0) {
        iface->conn_mode = IFCM_TIMEBASED;
    } else {
        /* Invalid conn mode */
    }
    D(INTERFACE, "interface(%p): %s, mode = %d\n", iface, iface->name, iface->conn_mode);
}
	       ...
	       struct blob_attr *config)
{
	...
	iface->connectable = blobmsg_get_bool_default(tb[IFACE_ATTR_CONNECTABLE], false);

    if ((cur = tb[IFACE_ATTR_CONN_MODE])) {
        conn_mode = blobmsg_get_string(cur);
        interface_set_conn_mode(iface, conn_mode);
    }

    if ((cur = tb[IFACE_ATTR_IDLE_TIME])) {
        iface->conn_time.idle = blobmsg_get_u32(cur);
    }

    if ((cur = tb[IFACE_ATTR_START_TIME])) {
        iface->conn_time.start = blobmsg_get_u32(cur);
    }

    if ((cur = tb[IFACE_ATTR_END_TIME])) {
        iface->conn_time.end = blobmsg_get_u32(cur);
    }
}
...
static bool __interface_add(struct interface *iface, struct blob_attr *config, bool alias)
{
	...
	if ((cur = tb[IFACE_ATTR_PARENT])){
		iface->parent = blobmsg_data(cur);
	}
...
void interface_set_up_child(struct interface *iface){
	struct interface *temp_if;
	if (NULL == iface){
		return;
	}
	vlist_for_each_element(&interfaces, temp_if, node){
		if((temp_if->parent != NULL) && (strcmp(iface->name, temp_if->parent) == 0) && (temp_if->state == IFS_DOWN)){
			//netifd_dbg("set up child : %s", temp_if->name);
			__interface_set_up(temp_if);
		}
	}
}
...
void interface_set_down_child(struct interface *iface){
	struct interface *temp_if;
	if (NULL == iface){
		return;
	}
	vlist_for_each_element(&interfaces, temp_if, node){
		if((temp_if->parent != NULL) && (strcmp(iface->name, temp_if->parent) == 0)){
			//netifd_dbg("set down child : %s", temp_if->name);
			__interface_set_down(temp_if, false);
		}
	}
}
...
#define UPDATE(field, __var) ({						\
	...
	if_old->parent = if_new->parent;
	if_old->proto_handler = if_new->proto_handler;
    if_old->connectable = if_new->connectable;
    if_old->conn_mode = if_new->conn_mode;
    memcpy(&if_old->conn_time, &if_new->conn_time, sizeof(struct interface_conn_time));
