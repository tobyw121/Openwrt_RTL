	...
	if (cmd == PROTO_CMD_SETUP) {
		switch (state->sm) {
			case S_IDLE:
				...
				/* add by ccx, 防止无用的teardown产生的超时对最新的setup造成影响 */
        ...
        /* modify by ccx, 适当增加teardown时脚本执行超时时间，防止有些脚本执行过慢导致teardown异常 */
		uloop_timeout_set(&state->teardown_timeout, 10000);
	}

	D(SYSTEM, "run %s for interface '%s'\n", action, proto->iface->name);
