...
static void
bridge_member_ext_cb(struct device_user *dev, enum device_event ev)
{
	struct bridge_member *bm = container_of(dev, struct bridge_member, dev);
	struct bridge_state *bst = bm->bst;
...
void
bridge_add_member_ext(struct device *dev, struct device *member)
{
	...
	bm = calloc(1, sizeof(*bm) + strlen(member->ifname) + 1);
	if (!bm)
		return;

	bm->bst = bst;
	bm->dev.hotplug = 1;
	bm->dev.cb = bridge_member_ext_cb;
	strcpy(bm->name, member->ifname);
	bm->dev.dev = member;
	vlist_add(&bst->members, &bm->node, bm->name);
	bm->node.version = -1;
}
...
void
bridge_del_member_ext(struct device *dev, struct device *member)
{
	struct bridge_state *bst = container_of(dev, struct bridge_state, dev);
	struct bridge_member *bm;
