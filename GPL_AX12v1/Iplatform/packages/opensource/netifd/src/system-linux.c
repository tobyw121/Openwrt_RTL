...
/* For some toolchain in mips arch case, openat() lack in declartions. */
#ifdef EXTERN_OPENAT
extern int openat(int, const char *,int);
#endif
#ifndef FRA_OIFNAME
#define FRA_OIFNAME FRA_IFNAME
#endif
...
extern char connect_iface_wan_ifname[16];
#endif

static int sock_ioctl = -1;
static struct nl_sock *sock_rtnl = NULL;

static void run_cmd(const char *dev_type, const char *device, bool state);
static int cb_rtnl_event(struct nl_msg *msg, void *arg);
static void handle_hotplug_event(struct uloop_fd *u, unsigned int events);
static int system_addbr(const char *ifname);
...
handler_nl_event(struct uloop_fd *u, unsigned int events)
{
	...
	case ENOBUFS:
		...
		ev->bufsize *= 2;
		D(SYSTEM, "enlarge ev->bufsize to %d\n",ev->bufsize);
		    ...
		    int (*cb)(struct nl_msg *msg, void *arg))
{
	...
	// Increase rx buffer size to 65K * 4 on event sockets
	ev->bufsize = 65535 * 4;
	if (nl_socket_set_buffer_size(ev->sock, ev->bufsize, 0))
		return false;
...
#if defined(QCA_IPQ50XX)
#define link_detect_timeout 1000  // 1 sec
static struct uloop_timeout link_detect;
static bool old_link_status = false;

static void detect_link_status_loop(struct uloop_timeout *t)
{
	bool ret = false;
	int status = -1;
	status = system("wan_connected");
	if (status != -1 && WIFEXITED(status) != 0) {
		ret = WEXITSTATUS(status) == 0 ? true : false;
	}

	if (ret && !old_link_status)
	{
		//printf("__%s off to on__\n", connect_iface_wan_ifname);
		run_cmd("Network device", connect_iface_wan_ifname, ret);
		old_link_status = ret;
	}
	else if (!ret && old_link_status)
	{
		//printf("__eth1.4094 on to off__\n");
		old_link_status = ret;
	}

	uloop_timeout_set(&link_detect, link_detect_timeout);
}
#endif
...
#if defined(QCA_IPQ50XX)
	link_detect.cb = detect_link_status_loop;
	uloop_timeout_set(&link_detect, link_detect_timeout);
#endif
...
static void run_cmd(const char *dev_type, const char *device, bool state)
{
	...
	setenv("ACTION", state ? "up" : "down", 1);
	setenv("DEVTYPE", dev_type, 1);
	if (device)
...
static bool system_check_state(struct device *dev, unsigned int ifi_flags)
{
	bool state;

	D(SYSTEM, "link: %s '%s' ifi_flags %x %s %s %s\n", 
		dev->type->name, dev->ifname,
		ifi_flags,
		ifi_flags & IFF_LOWER_UP ? "IFF_LOWER_UP" : "",
		ifi_flags & IFF_RUNNING ? "IFF_RUNNING" : "",
		ifi_flags & IFF_DORMANT ? "IFF_DORMANT" : "");

	state = ifi_flags & IFF_LOWER_UP ? true : false;
	if (dev->external)
	{
		state = ifi_flags & IFF_RUNNING ? state : false;
		state = ifi_flags & IFF_DORMANT ? false : state;
	}
...
static int cb_rtnl_event(struct nl_msg *msg, void *arg)
{
	...
	state = system_check_state(dev, ifi->ifi_flags);
	D(SYSTEM, "link: %s '%s' link is %s\n", dev->type->name, dev->ifname, state ? "up" : "down" );
	if (strncmp(dev->type->name, "Network device", 14) == 0)
	{
		run_cmd(dev->type->name, dev->ifname, state);
	}
	device_set_ifindex(dev, ifi->ifi_index);
	device_set_link(dev, state);
...
static int
system_cb_ack(struct nl_msg *msg, void *arg)
{
	int *pending = arg;
	*pending = 0;
	return NL_STOP;
...
static int
system_cb_valid(struct nl_msg *msg, void *arg)
{
	int *pending = arg;
	*pending = 0;
	return NL_STOP;
...
static int 
system_cb_msg_in(struct nl_msg *msg, void *arg)
{
	int *pending = arg;

	if(msg->nm_nlh->nlmsg_seq == sock_rtnl->s_seq_expect)
	{
		*pending = 0;
	}
...
static int system_rtnl_call(struct nl_msg *msg)
{
	...
	nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, system_cb_valid, &pending);
	nl_cb_set(cb, NL_CB_ACK, NL_CB_CUSTOM, system_cb_ack, &pending);
	nl_cb_set(cb, NL_CB_MSG_IN, NL_CB_CUSTOM, system_cb_msg_in, &pending);
...
int system_bridge_delbr(struct device *bridge)
{
	...
	/*
	sprintf(cmdStr, "echo '###delbr %s' > /dev/console", bridge->ifname);
	system(cmdStr);
	memset(cmdStr, 0, 128);
	*/
#ifndef NETIFD_WITH_BCM_IPTV	

	if(strncmp(bridge->ifname, "br-lan", 6) == 0)
	{
		/* br-lan should keep alive because wifi int are attach with it */
		ret = ioctl(sock_ioctl, SIOCBRDELBR, bridge->ifname);
		(void)system_addbr(bridge->ifname);
		return 0;
	}
	
	
	snprintf(cmdStr, sizeof(cmdStr), "ifconfig %s down", bridge->ifname);
	system(cmdStr);
	
	snprintf(cmdStr, sizeof(cmdStr), "brctl delbr %s", bridge->ifname);
	system(cmdStr);
...
/* do not del bridge netdevice if present */
...
int system_if_get_flags(char *ifname)
{
	struct ifreq ifr;
	
	memset(&ifr, 0, sizeof(ifr));
	strlcpy(ifr.ifr_name, ifname, IFNAMSIZ);
	if (ioctl(sock_ioctl, SIOCGIFFLAGS, &ifr))
		return 0;
...
static int
cb_ack_event(struct nl_msg *msg, void *arg)
{
	int *pending = arg;
	*pending = 0;
	return NL_STOP;
...
system_if_clear_entries(struct device *dev, int type, int af)
{
	...
	nl_cb_set(cb, NL_CB_ACK, NL_CB_CUSTOM, cb_ack_event, &pending);
...
void system_if_clear_state(struct device *dev)
{
	...
	/* do not del or down bridge netdevice */
...
static int 
system_addbr(const char *ifname)
{
	return ioctl(sock_ioctl, SIOCBRADDBR, ifname);
}
...
#ifndef KEEP_BRIDGE
	if (system_addbr(bridge->ifname) < 0)
		return -1;
#else
	(void)system_addbr(bridge->ifname);
#endif
...
#if defined(QCA_IPQ50XX)
	memset(cmd, 0, 128);
	snprintf(cmd, 128, "[ -e /usr/sbin/mcsctl ] && /usr/sbin/mcsctl -s %s state %s &", bridge->ifname, cfg->igmp_snoop ? "enable" : "disable");
	system(cmd);
#endif
...
{
#ifdef BCM_490X
	/* for 4908/c5400s, using bcm vlan instead of linux vlan, 
	 * bcm vlan is always maintained by IPTV, here just create a dummy vlan interface, 
	 * no tx/rx rules appended, IPTV will delete this dummy vlan interface and create the correct one
	 */
	if (id >= 0)
	{
		char cmd[64];
		char realIf[16] = {0};
		char *dot = strchr(dev->ifname, '.');
		if (dot)
		{
			DPRINTF("create vlan %s\n", dev->ifname);
			memcpy(realIf, dev->ifname, dot-dev->ifname);
			sprintf(cmd, "vlanctl --if-create-name %s %s\n", realIf, dev->ifname);
			system(cmd);
			return 0;
		}
	}
...
#else
	int ret = system_vlan(dev, id);
	int iptv_wan = 0;
	int ipphone_wan = 0;
	int mciptv_wan = 0;

	char command[128] = { 0 };
	char format[128] = "uci get iptv.$(uci get iptv.iptv.mode).%s_vid";

	FILE *fp = NULL;

	memset(command, 0, sizeof(command));
	sprintf(command, format, "iptv");
	fp = popen(command, "r");
	if (fp)
	{
		fscanf(fp, "%d", &iptv_wan);
		pclose(fp);
	}

	memset(command, 0, sizeof(command));
	sprintf(command, format, "ipphone");
	fp = popen(command, "r");
	if (fp)
	{
		fscanf(fp, "%d", &ipphone_wan);
		pclose(fp);
	}
									
	memset(command, 0, sizeof(command));
	sprintf(command, format, "mciptv");
	fp = popen(command, "r");
	if (fp)
	{
		fscanf(fp, "%d", &mciptv_wan);
		pclose(fp);
	}

	if ( (id == iptv_wan) || (id == ipphone_wan) || (id == mciptv_wan) )
	{
		memset(command, 0, sizeof(command));
		sprintf(command, "ifconfig %s.%d hw ether $(uci get network.wan.macaddr)", dev->ifname, id);
		system(command);
	}
	return ret;
#endif
...
system_if_apply_settings(struct device *dev, struct device_settings *s)
{
	...
	struct ifreq ifr;
#ifndef NETIFD_HAS_DOWNUP_BUG
	unsigned curr_flags = 0;
#endif

	/* 
	**	make sure the target iface in down state, otherwise SIFHWADDR will fail, e.g. brcm et driver.
	*/
	memset(&ifr, 0, sizeof(ifr));
	strncpy(ifr.ifr_name, dev->ifname, sizeof(ifr.ifr_name));
	ioctl(sock_ioctl, SIOCGIFFLAGS, &ifr);
#ifndef NETIFD_HAS_DOWNUP_BUG
#ifdef QCA_IPQ50XX
	if(strcmp(ifr.ifr_name,"br-lan")!=0)
	{
#endif
		curr_flags = (unsigned)ifr.ifr_flags;
		if ((ifr.ifr_flags & IFF_UP) && (s->flags & DEV_OPT_MACADDR))
		{
...
#ifndef NETIFD_HAS_DOWNUP_BUG
#ifdef QCA_IPQ50XX
	/* If dev is IFF_UP before, restore its state! */
	if(strcmp(ifr.ifr_name,"br-lan")!=0)
	{
#endif
		if ((curr_flags & IFF_UP) && (s->flags & DEV_OPT_MACADDR))
		{
			system_if_flags(dev->ifname, IFF_UP, 0);
		}
#ifdef QCA_IPQ50XX
	}
#endif
...
int system_if_down(struct device *dev)
{
    ...
    if (!dev->keepup) {
        ret = system_if_flags(dev->ifname, 0, IFF_UP);
    }
...
static int cb_if_check_valid(struct nl_msg *msg, void *arg)
{
	...
	if (ifi->ifi_index > 0)
		device_set_ifindex(chk->dev, ifi->ifi_index);
	device_set_link(chk->dev, system_check_state(chk->dev, ifi->ifi_flags));
...
#ifdef NETIFD_BCM_FFS
char* get_ffs_ifname(char *ffs_ifname)
{
	struct uci_context *uciCtx  = NULL;
	struct uci_package *pkg     = NULL;
	struct uci_section *section = NULL;
	const char *ifname_option = NULL;

	uciCtx = uci_alloc_context();
	uci_set_confdir(uciCtx, "/etc/config");
	if (UCI_OK != uci_load(uciCtx, "amazon_ffs", &pkg))
	{
		uci_perror(uciCtx, "amazon_ffs");
		goto not_exist;
	}

    if (NULL != ( section = uci_lookup_section(uciCtx, pkg, "ffs") ))  
    {   
		if (NULL != (ifname_option = uci_lookup_option_string(uciCtx, section, "wireless_iface_ifname")))  
		{   	
			memset(ffs_ifname, 0, 16);
			strncpy(ffs_ifname, ifname_option,  16);
		}  
...
{
#ifdef NETIFD_BCM_FFS
	char ffs_ifname[32] = {0};
	get_ffs_ifname(ffs_ifname);
	if (!strncmp(ifname, ffs_ifname, 5))
		return 0;
#endif
	char buf[64];
	struct stat s;
...
bool 
system_get_link_state_from_ifname(const char *ifname)
{
	uint64_t link = 0;
	bool ret = false;
	char buf[64];
	...
	link_state_fd = open(buf, O_DIRECTORY);
	if (link_state_fd < 0)
		return false;


	if (read_uint64_file(link_state_fd, "carrier", &link))
	{
		ret =  link ? true : false; 
	}
	else
...
/* Judge dev carrier on or off */
bool 
system_get_link_state(struct device *dev)
{
	bool ret = false;
#if defined(QCA_IPQ50XX)
	int status = -1;
	...
	close(link_state_fd);
	/* it is no necerrary to close link_state_fd */
#endif
	return ret;
}
...
bool 
system_get_34g_link_state(struct device *dev)
{
	uint64_t link = 0;
	bool ret = false;
	char buf[64];
...
int system_if_get_addr(const char *ifname, uint32_t *ip)
{
	...
	memset(&ifr, 0, sizeof(ifr));
	strncpy(ifr.ifr_name, ifname, sizeof(ifr.ifr_name));
    ret = ioctl(sock_ioctl, SIOCGIFADDR, &ifr);
    if (ret != 0) {
        return ret;
    }
...
int system_if_get_mask(const char *ifname, uint32_t *mask)
{
	...
	memset(&ifr, 0, sizeof(ifr));
	strncpy(ifr.ifr_name, ifname, sizeof(ifr.ifr_name));
    ret = ioctl(sock_ioctl, SIOCGIFNETMASK, &ifr);
    if (ret != 0) {
        return ret;
    }
...
int system_get_localtime(struct tm *cur)
{
    ...
    if (localtime_r(&ts.tv_sec, cur) != cur) {
        return -1;
    }
...
int system_add_ip_tunnel(const char *name, struct blob_attr *attr)
{
	...
	if (!strcmp(str, "sit")) {
...
#ifdef NETIFD_WITH_V6PLUS
	else if (!strcmp(str, "ip4ip6")) {
		struct nl_msg *		nlm = nlmsg_alloc_simple(RTM_NEWLINK, NLM_F_REQUEST | NLM_F_REPLACE | NLM_F_CREATE);
		struct ifinfomsg	ifi = { .ifi_family = AF_UNSPEC };
		int 				ret = 0;
		struct in6_addr 	localaddr6buf;
		struct in6_addr 	remoteaddr6buf;
		struct in_addr 		fakeIpaddr ;
		unsigned int 		realttl = (ttl) ? ttl : 64;
		if (!nlm)
			return -1;
        ...
        nla_put_u8(nlm, IFLA_IPTUN_TTL, realttl);
		nla_put_u8(nlm, IFLA_IPTUN_ENCAP_LIMIT, 4);

		if ((cur = tb[TUNNEL_ATTR_LOCAL]))
		{
			if(inet_pton(AF_INET6, blobmsg_data(cur), &localaddr6buf) < 1)
			{
				ret =  -EINVAL;
				goto failure;
			}
			 nla_put(nlm, IFLA_IPTUN_LOCAL, sizeof(localaddr6buf), &localaddr6buf);
		}

		if ((cur = tb[TUNNEL_ATTR_REMOTE]))
		{
			if(inet_pton(AF_INET6, blobmsg_data(cur), &remoteaddr6buf) < 1)
			{
				ret =  -EINVAL;
				goto failure;
			}
			nla_put(nlm, IFLA_IPTUN_REMOTE, sizeof(remoteaddr6buf), &remoteaddr6buf);
		}

		if ((cur = tb[TUNNEL_ATTR_FAKEIP]))
		{
			if(inet_pton(AF_INET, blobmsg_data(cur), &fakeIpaddr) < 1)
			{
				ret =  -EINVAL;
				goto failure;
			}
			nla_put(nlm, IFLA_IPTUN_FAKE_IP, sizeof(fakeIpaddr), &fakeIpaddr);
		}

		nla_put_u32(nlm, IFLA_IPTUN_FMRS_TRUNK, 0);
#if defined(QCA_IPQ50XX)
...
failure:
		D(SYSTEM, " ######## system_add_ip_tunnel: %s %d tunnel nlmsg failure ############\n", __FUNCTION__, __LINE__);
		nlmsg_free(nlm);
		return ret;
	}
#endif /* NETIFD_WITH_V6PLUS */
	else
		return -EINVAL;
