...
static const struct blobmsg_policy connctl_policy[__CONNCTL_MAX] = {
	[CONNCTL_ACTION] = { .name = "action", .type = BLOBMSG_TYPE_STRING },
};

static int
netifd_handle_connctl(struct ubus_context *ctx, struct ubus_object *obj,
		     struct ubus_request_data *req, const char *method,
		     struct blob_attr *msg)
{
	struct blob_attr *tb[__CONNCTL_MAX];
    char *action = NULL;

	blobmsg_parse(connctl_policy, __CONNCTL_MAX, tb, blob_data(msg), blob_len(msg));

    action = blobmsg_data(tb[CONNCTL_ACTION]);
	if (!action) {
...
#ifdef FEATURE_3G4G_MODULES
static int
netifd_get_current_network(struct ubus_context *ctx, struct ubus_object *obj,
			  struct ubus_request_data *req, const char *method,
			  struct blob_attr *msg)
{
	blob_buf_init(&b, 0);
	blobmsg_add_string(&b, "current_network", get_ubus_curr_nw());
	ubus_send_reply(ctx, req, b.head);
...
static const struct blobmsg_policy backup_network_type_policy[__NETWORK_TYPE_MAX] = {
	[NETWORK_TYPE] = { .name = "type", .type = BLOBMSG_TYPE_STRING },
};
static int
netifd_get_backup_network_status(struct ubus_context *ctx, struct ubus_object *obj,
			  struct ubus_request_data *req, const char *method,
			  struct blob_attr *msg)
{
	bool connect_status[NETWORK_TYPE_MAX];
	blob_buf_init(&b, 0);
	get_ubus_backup_network_status(connect_status);
	blobmsg_add_u8(&b, "wired_online_status", connect_status[NETWORK_TYPE_WIRED]);
	blobmsg_add_u8(&b, "mobile_online_status", connect_status[NETWORK_TYPE_MOBILE]);
	ubus_send_reply(ctx, req, b.head);

	return 0;
}
static int
netifd_get_backup_max_ip_num(struct ubus_context *ctx, struct ubus_object *obj,
			  struct ubus_request_data *req, const char *method,
			  struct blob_attr *msg)
{
	blob_buf_init(&b, 0);
	blobmsg_add_u32(&b, "max_ip_num", DETECT_IP_MAX);
	ubus_send_reply(ctx, req, b.head);
	return 0;
}
static int
netifd_set_backup_current_network(struct ubus_context *ctx, struct ubus_object *obj,
			  struct ubus_request_data *req, const char *method,
			  struct blob_attr *msg)
{
	struct blob_attr *tb[__NETWORK_TYPE_MAX];
	char *type = NULL;
	blobmsg_parse(backup_network_type_policy, __NETWORK_TYPE_MAX, tb, blob_data(msg), blob_len(msg));

	if (!tb[NETWORK_TYPE]) {
		netifd_log_message(L_ERROR, "Missing input attribute\n");
		return UBUS_STATUS_INVALID_ARGUMENT;
	}
	type = blobmsg_data(tb[NETWORK_TYPE]);
	if(strncmp(type, "wired", sizeof("wired")) == 0)
	{
		switch_ubus_curr_nw(NETWORK_TYPE_WIRED);
	}
	else if(strncmp(type, "mobile", sizeof("mobile")) == 0)
	{
		switch_ubus_curr_nw(NETWORK_TYPE_MOBILE);
	}
	else
	{
		netifd_log_message(L_ERROR, "Error input attribute\n");
		return UBUS_STATUS_INVALID_ARGUMENT;
	}
	return 0;
}
...
static struct ubus_method main_object_methods[] = {
    ...
    UBUS_METHOD("connctl", netifd_handle_connctl, connctl_policy),
	UBUS_METHOD("add_host_route", netifd_add_host_route, route_policy),
	{ .name = "get_proto_handlers", .handler = netifd_get_proto_handlers },
#ifdef FEATURE_3G4G_MODULES
	{ .name = "get_current_network", .handler = netifd_get_current_network },
#if FEATURE_3G4G_MODULES == 2
	{ .name = "get_backup_network_status", .handler = netifd_get_backup_network_status },
	{ .name = "get_backup_max_ip_num", .handler = netifd_get_backup_max_ip_num },
	UBUS_METHOD("set_current_network", netifd_set_backup_current_network, backup_network_type_policy),
#endif
#endif
};
		   ...
		   struct blob_attr *msg)
{
	...
	interface_set_down_child(iface);

	return 0;
}

static int
netifd_handle_connect(struct ubus_context *ctx, struct ubus_object *obj,
                      struct ubus_request_data *req, const char *method,
                      struct blob_attr *msg)
{
	struct interface *iface;
...
static int
netifd_handle_disconnect(struct ubus_context *ctx, struct ubus_object *obj,
                         struct ubus_request_data *req, const char *method,
                         struct blob_attr *msg)
{
	struct interface *iface;
		     ...
		     struct blob_attr *msg)
{
	...
	blobmsg_add_u8(&b, "linkstate", iface->link_state);
	blobmsg_add_u8(&b, "connectable", iface->connectable);

	switch (iface->conn_mode) {
	case IFCM_AUTO:
		blobmsg_add_string(&b, "connmode", "auto");
		break;

	case IFCM_MANUAL:
		blobmsg_add_string(&b, "connmode", "manual");
		break;

	case IFCM_DEMAND:
		blobmsg_add_string(&b, "connmode", "demand");
		break;

	case IFCM_TIMEBASED:
		blobmsg_add_string(&b, "connmode", "timebased");
		break;

	default:
		blobmsg_add_string(&b, "connmode", "invalid");
		break;
	}

	switch (iface->state) {
	case IFS_SETUP:
		blobmsg_add_string(&b, "state", "connecting");
		break;

	case IFS_UP:
		blobmsg_add_string(&b, "state", "connected");
		break;

	case IFS_TEARDOWN:
		blobmsg_add_string(&b, "state", "disconnecting");
		break;

	case IFS_DOWN:
		blobmsg_add_string(&b, "state", "disconnected");
		break;
...
static int
netifd_static_routes_recovery(struct ubus_context *ctx, struct ubus_object *obj,
						struct ubus_request_data *req, const char *method,
						struct blob_attr *msg)
{
	struct interface *iface;
	...
	interface_static_routes_recovery(iface);
	return 0;
}
...
static struct ubus_method iface_object_methods[] = {
    ...
    { .name = "connect", .handler = netifd_handle_connect },
    { .name = "disconnect", .handler = netifd_handle_disconnect },
	...
	{ .name = "static_routes_recovery", .handler = netifd_static_routes_recovery },
};
...
static void netifd_add_object(struct ubus_object *obj)
{
	...
	if (ret != 0)
		netifd_log_message(L_ERROR, "Failed to publish object '%s': %s\n", obj->name, ubus_strerror(ret));
}
...
static int console_log_enable(int enable)
{
	int std;
	static int console_fd = -1;
	static int std_backup[3] = {-1, -1, -1};

	if (!enable) {
		for (std = 0; std < 3; std++)
			if (std_backup[std] != -1) {
				dup2(std_backup[std], std);
				close(std_backup[std]);
				std_backup[std] = -1;
			}
	...
	else if (console_fd == -1) {
		console_fd = open("/dev/console", O_WRONLY);
		if (console_fd == -1)
			return -1;

		for (std = 0; std < 3; std++) {
			std_backup[std] = dup(std);
			close(std);
			dup2(console_fd, std);
		}
	}
...
static const struct blobmsg_policy netifd_debug_policy[] = {
    { .name = "level", .type = BLOBMSG_TYPE_INT32 },
	{ .name = "mask", .type = BLOBMSG_TYPE_INT32 },
	{ .name = "console_debug", .type = BLOBMSG_TYPE_INT32 },
};
// Ubus method handler
static int netifd_debug_handler(struct ubus_context *ctx, struct ubus_object *obj,
                             struct ubus_request_data *req, const char *method,
                             struct blob_attr *msg) {
	// Declare a blob_attr array to hold the parsed data
    struct blob_attr *tb[ARRAY_SIZE(netifd_debug_policy)];
    int level = 0;
    unsigned int mask = 0;
    int console_debug = 0;
    // Parse the input message using the policy
    blobmsg_parse(netifd_debug_policy, ARRAY_SIZE(netifd_debug_policy), tb, blob_data(msg), blob_len(msg));
    // Check if the input attribute is present
    if (!tb[0] || !tb[1] || !tb[2]) {
        netifd_log_message(L_ERROR, "Missing input attribute\n");
        return UBUS_STATUS_INVALID_ARGUMENT;
    }
    // Get the input string from the parsed data
    level = blobmsg_get_u32(tb[0]);
    mask = blobmsg_get_u32(tb[1]);
    console_debug = blobmsg_get_u32(tb[2]);
    if (L_EMERG <= level && level <= L_DEBUG && (console_debug == 0 || console_debug == 1)) {
		log_level = level;
		debug_mask = mask;
		console_log_enable(console_debug);
	} else {
		return UBUS_STATUS_INVALID_ARGUMENT;
	}
    return 0;
...
static const struct ubus_method netifd_debug_methods[] = {
    UBUS_METHOD("log", netifd_debug_handler, netifd_debug_policy),
};
static struct ubus_object_type netifd_debug_object_type =
    UBUS_OBJECT_TYPE("netifd_debug", netifd_debug_methods);
static struct ubus_object netifd_debug_object = {
    .name = "netifd.debug",
    .type = &netifd_debug_object_type,
    .methods = netifd_debug_methods,
    .n_methods = ARRAY_SIZE(netifd_debug_methods),
};
...
netifd_ubus_init(const char *path)
{
	...
	netifd_add_object(&netifd_debug_object);
