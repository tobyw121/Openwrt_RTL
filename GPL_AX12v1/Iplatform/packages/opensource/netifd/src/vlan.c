...
static int vlan_set_device_state(struct device *dev, bool up)
{
	...
	/* If it's already system present, do NOT create it */
	if (dev->sys_present)
	{
		return 0;
	}
	...
	/*
	** Try to delete old vlan dev, then create it to fix multi-ip preblem temporarily!
	*/
	vldev->set_state(dev, false);
	system_vlan_del(dev);
...
static void vlan_dev_cb(struct device_user *dep, enum device_event ev)
{
	...
	switch(ev) {
	case DEV_EVENT_ADD:
		/* create vlan network device here, by TengFei. */
		vldev->dev.set_state(&vldev->dev, true);
		new_state = true;
