...
static struct list_head interface_used;
...
const char *
config_option_str(const char *secname, const char *name)
{
    struct uci_section *section;

    section = uci_lookup_section(uci_ctx, uci_network, secname);
    if (!section) {
        return NULL;
    }

    return uci_lookup_option_string(uci_ctx, section, name);
}
...
/*
 * fn           call_parse_interface 
 * brief        judge if the interface has been called,if not,call the "config_parse_interface"
 * 
 * param[in]    s: the section of uci
 * param[out]   N/A 
 * 
 * return       N/A 
 */
static void
call_parse_interface(struct uci_section *s)
{
	char flag = 0;
	struct interface_name *name;
	struct interface_name *member, *tmp;	

	if (!s)
		return;
	
	if (strlen(s->e.name) > NAME_LEN_MAX)
	{
		netifd_log_message(L_ERROR, "Name length error\n");
		return;
	}
	
	list_for_each_entry_safe(member, tmp, &interface_used, list)
	{
		if (!strcmp(member->value, s->e.name))
		{
			flag = 1;
			break;
		}
	...
	if (!flag)
	{
		name = (struct interface_name *)malloc(sizeof(struct interface_name) + strlen(s->e.name) + 1);
		if (!name)
		{
			netifd_log_message(L_ERROR, "Malloc fail\n");
			return;
		}
		memset(name, 0, sizeof(*name));
		strncpy(name->value, s->e.name, strlen(s->e.name) + 1);
		config_parse_interface(s, false);
		list_add(&name->list, &interface_used);
	}
}

/*
 * fn           call_rely_interface 
 * brief        call the relied interface 
 * 
 * param[in]    s: the section of uci
 * param[in]    rely_interface: the relied interface's name
 * param[out]   N/A 
 * 
 * return       N/A 
 */
static void
call_rely_interface(struct uci_section *s, const char *rely_interface)
{
	struct uci_section *section;
	const char *rely = NULL;
	
	if (!s || !rely_interface || !strlen(rely_interface))
	{
		//do nothing
		return;
	}
	...
	{
		section = uci_lookup_section(uci_ctx, uci_network, rely_interface);
		rely = uci_lookup_option_string(uci_ctx, section, "rely_interface");
		call_rely_interface(section, rely);
		call_parse_interface(section);
	}
}

#ifdef SUPPORT_WIRELESS_DEVICE

static int config_get_vap(char *name, char *vap, int maxlen)
{
	struct uci_element *e;

	if (!uci_wireless || !name || !vap || maxlen <= 0) {
		DPRINTF("input args error\n");
		return -1;
	}
...
config_init_wireless()
{
	...
	uci_foreach_element(&uci_network->sections, element) {
		...
		type = uci_lookup_option_string(uci_ctx, section, "type");
		if (!type || strcmp(type, "bridge") != 0)
			continue;

		ifnames = uci_lookup_option_string(uci_ctx, section, "wireless_ifname");
		if (!ifnames)
			continue;

		ifname = strtok((char *)ifnames, " ");
		while(ifname != NULL)
		{
			memset(vap, 0, sizeof(vap));
			if(config_get_vap(ifname, vap, sizeof(vap)-1) == 0)
				wireless_device_create(vap, ifname, NULL, devtype, section->e.name);

			ifname = strtok(NULL, " ");
		}
	}

	/*parse getway wireless device*/
	uci_foreach_element(&uci_wireless->sections, e) {
		struct uci_section *s = uci_to_section(e);
		const char *ifname;
		...
		uci_foreach_element(&uci_network->sections, element) {
			...
			name = uci_lookup_option_string(uci_ctx, section, "name");
			if (!name)
				continue;
			...
			{
				blob_buf_init(&b, 0);
				uci_to_blob(&b, section, devtype->config_params);
				wireless_device_create(s->e.name, ifname, b.head, devtype, CONNECT_IFACE_WAN);
				break;
			}
		}
	}
...
{
	struct uci_element *e;
	struct interface_name *c;
	const char *rely_interface = NULL;

	INIT_LIST_HEAD(&interface_used);
	...
	uci_foreach_element(&uci_network->sections, e) {
		...
		{
			rely_interface = uci_lookup_option_string(uci_ctx, s, "rely_interface");
			if (rely_interface && strlen(rely_interface))
			{
				call_rely_interface(s, rely_interface);
			}
			call_parse_interface(s);
		}
	}

	//delete all entry of the list
	while (!list_empty(&interface_used)) {
		c = list_first_entry(&interface_used, struct interface_name, list);
		if (c)
		{
			list_del(&c->list);
			free(c);
...
config_init_all(void)
{
	...
	if (!uci_network) {
		netifd_log_message(L_ERROR, "Failed to load network config\n");
		return;
	}
