...
void proxy::handle_stateless_advert(const address& saddr, const address& taddr, const std::string& ifname, bool use_via)
{
    ...
    if (_autowire == true) {
        /* set ipv6 route for DAD packet */
        if (saddr.is_empty())
        {
            se->handle_auto_wire(taddr, ifname, use_via);
        }
        else
        {
...
void proxy::handle_solicit(const address& saddr, const address& taddr, const std::string& ifname)
{
    ...
    if (!saddr.is_empty())
    {
        std::string proto = "";
        proto = uci_get_proto();
        if (proto == "pppoev6" || proto == "pppoeshare")
        {
            logger::debug() << "Current protocol is PPPoE: send advert.";
                se->send_advert(saddr);
            return;
        }
    }
    
    // If our session is confirmed then we can respond with an advert otherwise
