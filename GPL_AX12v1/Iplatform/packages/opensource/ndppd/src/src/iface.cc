...
// Set up filter.
	...
	static struct sock_filter filter[] = {
		...
		BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, IPPROTO_ICMPV6, 0, 2),
		...
		BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, ND_NEIGHBOR_SOLICIT, 3, 4),

		/* filt the DNS request packet */
		// Bail if it's* not* UDP.
		BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, 0x11, 0, 3),
		// Load the UDP.
		BPF_STMT(BPF_LD | BPF_H | BPF_ABS,
			sizeof(struct ether_header) + sizeof(ip6_hdr) + 2),
		// Bail if it's* not* DNS request.
		BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, 0x35, 0, 1),
		// Keep packet.
...
ptr<iface> iface::open_ifd(const std::string& name)
{
	...
	std::string ifname = "";
	proto = uci_get_proto();
	if ((proto == "pppoev6" || proto == "pppoeshare") && name != "br-lan")
	{
		logger::debug() << "PPPoE : wanv6";
		ifname = uci_get_ifname();
	}
	else
	{
		ifname = name;
...
void iface::handle_reverse_advert(const address& saddr, const address& taddr, const std::string& ifname)
{
    if (!saddr.is_unicast() && !saddr.is_empty())
        return;
    
    logger::debug()
        << "iface::handle_reverse_advert()";
    
    if (saddr.is_empty())
    {
        logger::debug()
            << "iface:: saddr is empty";
    }
    ...
    for (std::list<weak_ptr<proxy> >::iterator pit = parents_begin(); pit != parents_end(); pit++) {
        ...
        for (std::list<ptr<rule> >::iterator it = parent->rules_begin(); it != parent->rules_end(); it++) {
            ...
            /* DAD */
            if ((ru->addr() == taddr && saddr.is_empty()) &&
                 ru->daughter()->name() == ifname)
            {
                logger::debug() << " - generating artifical solicitation: " << ifname;
                parent->handle_stateless_advert(saddr, taddr, ifname, ru->autovia());
            }
        }
    }
}
...
int iface::poll_all()
{
    ...
    for (std::vector<struct pollfd>::iterator f_it = _pollfds.begin();
        ...
        if (!(f_it->revents & (POLLIN |POLLERR)) ) {
            continue;
        }
        ...
        if (is_pfd) {
            ...
            ifa->handle_reverse_advert(saddr, taddr, ifa->name());
