...
static int read_leases(time_t now, FILE *leasestream)
{
  ...
  int items;
  char *domain = NULL;
  int client_type = DEVICE_TYPE_NONE;
...
# error Buffer size breakage in leasefile parsing.
	...
	*daemon->namebuff = *daemon->dhcp_buff = *daemon->packet = '\0';
	...
	if (fscanf(leasestream, " %64s %255s %764s %d",
		   daemon->namebuff, daemon->dhcp_buff, daemon->packet, &client_type) != 4)
	  {
...
#ifdef SUPPORT_FING
if (client_type >= DEVICE_TYPE_NONE && client_type < DEVICE_TYPE_MAX)
#else
if (client_type >= DEVICE_TYPE_NONE && client_type <= DEVICE_TYPE_IOT)
#endif
{
  lease->client_type = client_type;
}
else
{
  lease->client_type = DEVICE_TYPE_NONE;
}
...
void lease_set_hostname(struct dhcp_lease *lease, const char *name, int auth, char *domain, char *config_domain)
{
  ...
  if (name)
    ...
    if (!memcmp(lease_tmp->hwaddr, lease->hwaddr, lease->hwaddr_len))
    {
	    kill_name(lease_tmp);
    }
	  break;
...
void lease_set_client_type(struct dhcp_lease *lease, DEVICE_TYPE type)
{
  if (type != lease->client_type)
  {
    lease->client_type = type;
    lease->flags |= LEASE_CHANGED;
  }
}
