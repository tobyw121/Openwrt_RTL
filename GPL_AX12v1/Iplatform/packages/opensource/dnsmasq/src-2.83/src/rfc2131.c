...
#include "log.h"
#define TP_LOG_DHCPS(msgid, ...) log(PROJ_LOG_ID_DHCPS, msgid, ##__VA_ARGS__)
#define TP_LOG_CLID_STR_LEN 64  /* temp buffer length*/
#endif
		  ...
		  int *is_inform, int pxe, struct in_addr fallback, time_t recvtime)
{
...
#endif
  DEVICE_TYPE client_type = DEVICE_TYPE_NONE;
  char *client_vendor = NULL;
  size_t client_max_message = 0;

    //add by zengwei for tp log 20180920 below
#ifdef CONFIG_UGLY_LOG
  char clmacstr[TP_LOG_CLID_STR_LEN] = {'\0'};
#endif
  //add by zengwei for tp log 20180920 above
  ...
  */
  ...
  if ((opt = option_find(mess, sz, OPTION_VENDOR_ID, 1)))
    {
      client_vendor = option_ptr(opt, 0);
  ...
  //add by zengwei for tp log 20180920 below
#ifdef CONFIG_UGLY_LOG
  memset(clmacstr, 0, TP_LOG_CLID_STR_LEN);
  print_mac(clmacstr, emac, emac_len >= TP_LOG_CLID_STR_LEN - 1 ? TP_LOG_CLID_STR_LEN - 1 : emac_len);
#endif 
  //add by zengwei for tp log 20180920 above	
  ...
  switch (mess_type)
    ...
    case DHCPRELEASE:
      //add by zengwei for tp log 20180920 below
#ifdef CONFIG_UGLY_LOG
      TP_LOG_DHCPS(DHCPS_RECV_RELEASE, LP_IPV4, ntohl(mess->ciaddr.s_addr), LP_STRING, clmacstr);
#endif
      //add by zengwei for tp log 20180920 below
    ...
    case DHCPDISCOVER:
      //add by zengwei for tp log 20180920 below
#ifdef CONFIG_UGLY_LOG
      TP_LOG_DHCPS(DHCPS_RECV_DISCOVER, LP_STRING, clmacstr);
#endif
	  //add by zengwei for tp log 20180920 above
      ...
      if (option_bool(OPT_RAPID_COMMIT) && option_find(mess, sz, OPTION_RAPID_COMMIT, 0))
	{
	  ...
	  //add by zengwei for tp log 20180920 below
#ifdef CONFIG_UGLY_LOG
	  opt = option_find(mess, sz, OPTION_DNSSERVER, INADDRSZ);
      TP_LOG_DHCPS(DHCPS_SEND_OFFER, LP_IPV4, ntohl(mess->yiaddr.s_addr), 
	  			LP_IPV4, opt ? ntohl(option_addr(opt).s_addr) : 0, LP_STRING, clmacstr);
#endif
	  //add by zengwei for tp log 20180920 above
    ...
    case DHCPREQUEST:
      //add by zengwei for tp log 20180920 below
#ifdef CONFIG_UGLY_LOG
      opt = option_find(mess, sz, OPTION_REQUESTED_IP, INADDRSZ);
      /*only record broadcast request*/
      if ((mess->flags & htons(0x8000)) || mess->ciaddr.s_addr == 0)
	{
	  TP_LOG_DHCPS(DHCPS_RECV_REQ, LP_IPV4, opt ? ntohl(option_addr(opt).s_addr) : 0, LP_STRING, clmacstr);
	}
#endif
      //add by zengwei for tp log 20180920 above
...
#ifdef CONFIG_UGLY_LOG
      TP_LOG_DHCPS(DHCPS_SEND_NAK, LP_STRING, clmacstr, LP_STRING, message);
#endif
      ...
      else
	{
	  ...
	  client_type = device_type_check(hostname, client_vendor, req_options, client_max_message);
	  if (DEVICE_TYPE_NONE != client_type)
	    lease_set_client_type(lease, client_type);
	  ...
	  //add by zengwei for tp log 20180920 below
#ifdef CONFIG_UGLY_LOG
	  //only log init bradcast ack
	  if ((mess->flags & htons(0x8000)) || mess->ciaddr.s_addr == 0)
	    {
	      opt = option_find(mess, sz, OPTION_DNSSERVER, INADDRSZ);
	      TP_LOG_DHCPS(DHCPS_SEND_ACK, LP_IPV4, ntohl(mess->yiaddr.s_addr), 
				LP_IPV4, opt ? ntohl(option_addr(opt).s_addr) : 0, LP_STRING, clmacstr);
	    }
#endif
	  //add by zengwei for tp log 20180920 above
...
static u32 get_NTPServerIP()
{
	/*NTPServer IP Network Byte Order*/
	u32 NTPServerIP = 0;
	char default_NTPServerIP[] = "139.78.100.163";
	FILE *fNTP = NULL;

	fNTP = fopen("/tmp/NTPServer","r");
	if(fNTP == NULL)
	{
		my_syslog(MS_DHCP | LOG_WARNING, _("dmasq open NTPServer file failed!!!"));
		return inet_addr(default_NTPServerIP);
	}
	fscanf(fNTP,"%u",&NTPServerIP);
	fclose(fNTP);
		       ...
		       unsigned short fuzz)
{
  ...
  if (context)
	...
	option_put(mess, end, OPTION_NETMASK, INADDRSZ, ntohl(context->netmask.s_addr));
      ...
      if (in_list(req_options, OPTION_NTPSERVER_IP) && !option_find2(OPTION_NTPSERVER_IP))
	option_put(mess, end, OPTION_NTPSERVER_IP, INADDRSZ, ntohl(get_NTPServerIP()));
