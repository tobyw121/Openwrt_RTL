...
#ifdef FFS_SUPPORT
#define LOPT_FFS_DOMAIN 311
#define LOPT_FFS_PROTO  312
#define LOPT_FFS_PORT   313
#define LOPT_FFS_ENABLE 314
#endif
...
#ifdef FFS_SUPPORT
    { "ffs_domain", 1, 0, LOPT_FFS_DOMAIN },
    { "ffs_protocol", 1, 0, LOPT_FFS_PROTO },
    { "ffs_port", 1, 0, LOPT_FFS_PORT },
    { "ffs_enable", 1, 0, LOPT_FFS_ENABLE },
#endif
    { NULL, 0, 0, 0 }
  };
...
#ifdef FFS_SUPPORT
  { LOPT_FFS_DOMAIN, ARG_DUP, "<domain>", gettext_noop("White domain list for ffs."), NULL },
  { LOPT_FFS_PROTO, ARG_DUP, "<protocol>", gettext_noop("White protocol list for ffs.."), NULL },
  { LOPT_FFS_PORT, ARG_DUP, "<port>", gettext_noop("White port list for ffs.."), NULL },
  { LOPT_FFS_ENABLE, ARG_DUP, "<enable>", gettext_noop("enable for ffs.."), NULL },
#endif
  { 0, 0, NULL, NULL, NULL }
}; 
		...
		for (new = daemon->cnames; new; new = new->next)
		  if (hostname_isequal(new->alias, arg))
		    problem = _("duplicate CNAME");
				...
				//target and alias's value need to be different
...
#ifdef FFS_SUPPORT
    case LOPT_FFS_DOMAIN:
    {
        if (!arg)
            problem = _("Bad ffs-domain");
        else
            ffs_domain_list_add(arg);
        break;
    }
    case LOPT_FFS_PROTO:
    {
        if (!arg)
            problem = _("Bad ffs-protocol");
        else
            ffs_proto_list_add(arg);
        break;
    }
    case LOPT_FFS_PORT:
    {
        if (!arg)
            ...
            ffs_port_array_add(arg);
        break;
    }
    case LOPT_FFS_ENABLE:
    {
        if (!arg || (strcmp(arg, "on") != 0 && strcmp(arg, "off") != 0))
            problem = _("Bad ffs-enable");
        else
            ffs_enable_set(arg);
        break;
    }
