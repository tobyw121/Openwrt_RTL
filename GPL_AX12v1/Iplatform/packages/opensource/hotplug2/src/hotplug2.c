...
#ifdef ROBUST_HOTPLUG
		} else if (!strcmp(*argv, "--usb-only")) {
			argv++;
			argc--;
			usb_only = 1;
			if (argc <= 0)
				break;
		} else if (!strcmp(*argv, "--nusb-only")) {
			argv++;
			argc--;
			nusb_only = 1;
			if (argc <= 0)
...
int main(int argc, char *argv[]) {
	...
	if ( cfd <= 0 )
	{
		system("echo open console fail !!!!!!!!!!!!!!!!!!!!! > /dev/console");
		exit(1);
	}
#endif
#endif
...
#ifdef ROBUST_HOTPLUG
		if ( nusb_only && uevent->is_subsys_dwc3 )
		{
#ifdef DEBUG
			char msg[256];
			char *mark=getenv("test");
			memset(msg, 0, 256);
			sprintf(msg, "~~~~~~usb event in nusb task, drop");
			if (mark && !strncmp(mark, "true", 4))
				write(cfd, msg, strlen(msg));
#endif
			goto cont;
		}

		if ( usb_only && !uevent->is_subsys_dwc3 )
		{
#ifdef DEBUG
			char msg[256];
			char *mark=getenv("test");
			memset(msg, 0, 256);
			sprintf(msg, "!!!!!!nusb event in usb task, drop");
			if (mark && !strncmp(mark, "true", 4))
				write(cfd, msg, strlen(msg));
#endif
			goto cont;
