...
static void read_mroute_list(FILE *ofp)
{
	...
	while (fgets(buf, sizeof(buf), fp)) {
		...
		if (filter_dev[0] && (vifi<0 || strcmp(filter_dev, viftable[vifi])))
			continue;
		if (filterrt.mdst.family && inet_addr_match(&maddr, &filterrt.mdst, filterrt.mdst.bitlen))
			continue;
		if (filterrt.msrc.family && inet_addr_match(&msrc, &filterrt.msrc, filterrt.msrc.bitlen))
			continue;
...
static int mroute_list(int argc, char **argv)
{
	while (argc > 0) {
		if (strcmp(*argv, "iif") == 0) {
			...
			NEXT_ARG();
			get_prefix(&filterrt.msrc, *argv, AF_INET);
		} else {
			if (strcmp(*argv, "to") == 0) {
				NEXT_ARG();
			}
			if (matches(*argv, "help") == 0)
				usage();
			get_prefix(&filterrt.mdst, *argv, AF_INET);
		}
		argv++; argc--;
	}
