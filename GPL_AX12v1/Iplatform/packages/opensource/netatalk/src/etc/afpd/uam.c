...
#if 1 //add by zw
	/* if we have a alias name try with it */
	if (obj->options.useralias && !strcmp(name, obj->options.useralias))
	{
		if (obj->options.userorigi && (pwent = getpwnam(obj->options.userorigi)))
		{
