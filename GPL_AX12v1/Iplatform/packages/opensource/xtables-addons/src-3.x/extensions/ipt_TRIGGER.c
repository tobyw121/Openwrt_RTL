...
static spinlock_t local_nf_conntrack_lock;
...
#define ASSERT_READ_LOCK(x) MUST_BE_READ_LOCKED(&local_nf_conntrack_lock)
#define ASSERT_WRITE_LOCK(x) MUST_BE_WRITE_LOCKED(&local_nf_conntrack_lock)
...
static void trigger_refresh(struct ipt_trigger *trig, unsigned long extra_jiffies)
{
    DEBUGP("%s: \n", __FUNCTION__);
    NF_CT_ASSERT(trig);
    WRITE_LOCK(&local_nf_conntrack_lock);
    ...
    WRITE_UNLOCK(&local_nf_conntrack_lock);
}
...
static void __del_trigger(struct ipt_trigger *trig)
{
    DEBUGP("%s: \n", __FUNCTION__);
    NF_CT_ASSERT(trig);
    MUST_BE_WRITE_LOCKED(&local_nf_conntrack_lock);
...
static void trigger_timeout(struct timer_list * ul_trig)
{
    struct ipt_trigger *trig= from_timer(trig, ul_trig, timeout);
...
trigger_check(const struct xt_tgchk_param *param)
{
	...
	if (nf_ct_netns_get(param->net, param->family) < 0){
...
static int __init init(void)
{
	int ret;
	DEBUGP(" lqt === 7777\n");
	spin_lock_init(&local_nf_conntrack_lock);
