...
static DEFINE_SPINLOCK(nf_conntrack_trigger_lock);
...
#define ASSERT_READ_LOCK(x) MUST_BE_READ_LOCKED(&nf_conntrack_trigger_lock)
#define ASSERT_WRITE_LOCK(x) MUST_BE_WRITE_LOCKED(&nf_conntrack_trigger_lock)
...
static void trigger_refresh(struct ipt_trigger *trig, unsigned long extra_jiffies)
{
    DEBUGP("%s: \n", __FUNCTION__);
    NF_CT_ASSERT(trig);
    WRITE_LOCK(&nf_conntrack_trigger_lock);
    ...
    WRITE_UNLOCK(&nf_conntrack_trigger_lock);
}
...
static void __del_trigger(struct ipt_trigger *trig)
{
    DEBUGP("%s: \n", __FUNCTION__);
    NF_CT_ASSERT(trig);
    MUST_BE_WRITE_LOCKED(&nf_conntrack_trigger_lock);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
static void trigger_timeout(struct timer_list *tl)
{
    struct ipt_trigger *trig= from_timer(trig, tl, timeout);
#else
...
trigger_dnat(struct sk_buff *skb, const struct xt_action_param *param)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
	enum nf_nat_manip_type maniptypex = HOOK2MANIP(param->state->hook);
#else
	...
	DEBUGP("maniptypex : %d\n\n", maniptypex);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
	NF_CT_ASSERT(param->state->hook == NF_IP_PRE_ROUTING);
#else
	...
	DEBUGP("%s:  found->srcip  %x;  found->dstip  %x;param->hooknum %d; \n",
		__FUNCTION__, found->srcip, found->dstip, param->state->hook);
#else
	DEBUGP("%s:  found->srcip  %x;  found->dstip  %x;param->hooknum %d; \n",
	...
	#else
	xt_nat_convert_range(&range, &newrange);
	#endif

	ret = nf_nat_setup_info(ct, &range, maniptypex);
...
static int __init init(void)
{
	int ret;
	DEBUGP(" lqt === 7777\n");
	spin_lock_init(&nf_conntrack_trigger_lock);
