...
#endif
#if defined(SUPPORT_QCA_NSS) || defined(REMOVE_HW_ACCEL_ENTRIES_VIA_KILLING_CT)
#include <net/netfilter/nf_conntrack_acct.h>
...
typedef struct _pctl_stats_day{
	unsigned int	timestamp;    /* last visited */
	unsigned int	cTimestamp;   /* stamp for today stats init time */
	unsigned int	total;        /* minutes */
	unsigned int	totalBonus;   /* minutes bonus time*/
...
extern int (*block_http_redirect)(struct sk_buff *skb, int reason);
#ifdef SUPPORT_QCA_NSS
extern int (*url_class_remove_related_hw_accel_entries)(unsigned int saddr, unsigned int daddr);
extern unsigned int ecm_nss_ipv4_register_L3_hook_status;
#endif
#endif
...
static ssize_t vercode_proc_write(struct file *file, const char __user *buf, size_t count,  loff_t *data);
...
static ssize_t payment_proc_write(struct file *file, const char __user *buf, size_t count,  loff_t *data);
...
static ssize_t urlclass_proc_write(struct file *file, const char __user *buf, size_t count,  loff_t *data);
...
static int generateVercode(unsigned char *mac, bool override)
{
	...
	if (!override && vercode && time_before(get_seconds(), vercode_stamp + URL_VERCODE_PERIOD)) {
...
int vercode_http_redirect(struct sk_buff *skb, int reason)
{
	int ret = -1;
...
static int __attribute__((unused)) redirectSkip(unsigned char *mac)
{
	int index;
...
static int __attribute__((unused)) redirectUpdate(void)
{
	int index;
...
static int http_response(struct sk_buff *skb, struct net_device *in, int reason)
{
	...
	PCTL_ERROR("[TPIPF] http_payload_end - http_payload_start=%ld", http_payload_end - http_payload_start);
...
static int http_response_ipv6(struct sk_buff *skb, struct net_device *in, int reason)
{
	...
	http_payload_start = (unsigned char *)pTcpHdr + pTcpHdr->doff*4;
    http_payload_end = (unsigned char *)pIpHdr + sizeof(struct ipv6hdr) + ntohs(pIpHdr->payload_len) - 1;
...
static inline void init_stats_daytime(pctl_stats_day* pStats)
{
    ...
    pStats->totalBonus = 0;
    pStats->timestamp = 0;
	pStats->cTimestamp = GET_TIMESTAMP();
    pStats->time_limit = -1;
    for(i = 0; i < 24; i++)
    {
        pStats->hours[i] = 0;
...
static inline void init_stats_daytime_yesteday(pctl_stats_day* pStats)
{
    int i = 0;
    pStats->total = 0;
    pStats->totalBonus = 0;
    pStats->timestamp = 0;
	pStats->cTimestamp = GET_TIMESTAMP() - 86400;
    pStats->time_limit = -1;
    for(i = 0; i < 24; i++)
    {
        pStats->hours[i] = 0;
...
//add by wanghao
static inline int strtoint(const char *string)
{
    int res = 0;
...
static ssize_t log_proc_write(struct file *file, const char __user *buf, size_t count,  loff_t *data)
{
	...
	if (count > 1024)
	{
		PCTL_ERROR("input count too large.");
		return -1;
	}

	if (copy_from_user((void *)kbuf, buf, count) != 0)
	{
		return -EFAULT;
	}
...
static int log_clear(int id, bool initFlag)
{
	...
	*/
    ...
    init_stats_daytime(&pOwner->today_stats);
		init_stats_daytime_yesteday(&pOwner->yesterday_stats);
	}
...
static int log_update_history(int id, unsigned int now)
{
    ...
    day_after = now / 86400 - pOwner->today_stats.cTimestamp / 86400;
    if (0 == day_after)
    {
		write_unlock_bh(&pOwner->lock);
        /* do nothing */
...
static ssize_t device_proc_write(struct file *file, const char __user *buf, size_t count,  loff_t *data)
{
    ...
    if(count != 2) 
    {
         PCTL_ERROR("count = %lu.", (unsigned long)count);
         return -1;
    }
    copy_from_user(&cmd, buf, 1);
...
bool skb_keepalive_cb(struct sk_buff *skb)
{
    const unsigned char *source = NULL;
...
#ifdef SUPPORT_HOMECARE_PRO_URL_CLASS	
/* add by wanghao */
static void __attribute__((unused)) skb_debugFy(const struct sk_buff *skb)
{
...
static int _url_class_list_init(void)
{
	...
	for (index = 0; index < MAX_URL_CLASS_ENTRY_LEN; index++) {
		pNode = kmalloc(sizeof(struct url_class_entry), GFP_ATOMIC);
		if (!pNode)
		{
...
static int _url_addr_list_init(void)
{
	...
	for (index = 0; index < MAX_URL_ADDR_ENTRY_LEN; index++) {
		pNode = kmalloc(sizeof(struct url_addr_entry), GFP_ATOMIC);
		if (!pNode)
		{
...
#ifdef REMOVE_HW_ACCEL_ENTRIES_VIA_KILLING_CT
static struct nf_conn* napt_ct_list_iterate(uint32_t *hash, struct hlist_nulls_node **iterate) {
	...
	if (*iterate == NULL) {
		...
		for (*hash = 0; *hash < nf_conntrack_htable_size; (*hash) ++) {
			*iterate = rcu_dereference(hlist_nulls_first_rcu(&nf_conntrack_hash[*hash]));
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 7, 0)
		if (*hash >= nf_conntrack_htable_size) {
#else
...
#ifdef SUPPORT_QCA_NSS
void block_rule_remove_hw_accel_entries(unsigned int saddr, unsigned int daddr)
{
	/*In order to get rid of block-rule related hw accelerated entries,
M1: we delete ct from  kernel.
M2: we delete hw accel entries directly, for example, we delete ecm_db_connections in QCA ax73v2.
	*/
#ifdef REMOVE_HW_ACCEL_ENTRIES_VIA_KILLING_CT
	unsigned int hash = 0;
	struct hlist_nulls_node *iterate = NULL;
	struct nf_conn *ct = NULL;
	struct nf_conntrack_tuple_hash *original_tuple_hash = NULL;

	if ( !ecm_nss_ipv4_register_L3_hook_status )
		return ;

	printk("[%s][%d] %x:%x\n", __func__, __LINE__, saddr, daddr);
	rcu_read_lock();
	while ((ct = napt_ct_list_iterate(&hash, &iterate)))
	{
		original_tuple_hash = &(ct->tuplehash[IP_CT_DIR_ORIGINAL]);
		/*saddr and daddr is now in host order*/
		if (original_tuple_hash->tuple.src.u3.ip == htonl(saddr) && \
				original_tuple_hash->tuple.dst.u3.ip == htonl(daddr) )
		{
			/*match, then destroy this ct*/
			printk("*************[%s][%d]%x:%x\n", __func__, __LINE__, saddr, daddr);
	...
	rcu_read_unlock();
#else
	int (*tmp_url_class_remove_related_hw_accel_entries)(unsigned int saddr, unsigned int daddr);
	rcu_read_lock();
	tmp_url_class_remove_related_hw_accel_entries = rcu_dereference(url_class_remove_related_hw_accel_entries);
	if ( !ecm_nss_ipv4_register_L3_hook_status || !tmp_url_class_remove_related_hw_accel_entries)
	{
		rcu_read_unlock();
		return ;
	}

	tmp_url_class_remove_related_hw_accel_entries(saddr, daddr);
	rcu_read_unlock();
...
static int _url_class_parse_response(const struct sk_buff *skb)
{
	...
	unsigned int dns_len = 0;
	unsigned short nQDCount = 0;
	unsigned short nANCount = 0;
#ifdef SUPPORT_QCA_NSS
	int ret = 0;
#endif
	...
	pDnsHdr = (void *) udph + sizeof(struct udphdr);
	trans_id = pDnsHdr->transID;
	dns_len = (unsigned int) ntohs(udph->len) - sizeof(struct udphdr) - sizeof(dns_header);
	nQDCount = ntohs(pDnsHdr->nQDCount);
	nANCount = ntohs(pDnsHdr->nANCount);
	...
	if (nANCount) {
		...
		for (index = 0; index < nANCount; index++) {
			...
			//data label
				...
				if (*(answers + domain_len + 1) != '\0')
				{
					printErr("domain_len overlength,return!!!!\n");
					//skb_debugFy(skb);
					spin_unlock_bh(&url_class_nl_lock);
					return -1;
				}
...
#ifndef SUPPORT_QCA_NSS
					block_rule_add(&rule, pos->cat_id, 0, pos->url, pos->url_len);
#else
					ret = block_rule_add(&rule, pos->cat_id, 0, pos->url, pos->url_len);
					if ( 0 == ret )// && is_nss_l3_hook_registered && *is_nss_l3_hook_registered)
					{
						block_rule_remove_hw_accel_entries(rule.tuple.src_ip.addr4, rule.tuple.dst_ip.addr4);
					}
#endif /* SUPPORT_QCA_NSS */
#endif
...
#else
				ret = block_rule_add(&rule, pEntry->cat_id, 0, pEntry->url, pEntry->url_len);
				if ( 0 == ret )//&& is_nss_l3_hook_registered && *is_nss_l3_hook_registered)
				{
					block_rule_remove_hw_accel_entries(rule.tuple.src_ip.addr4, rule.tuple.dst_ip.addr4);
				}
...
#else
				ret = block_rule_add(&rule, url_class_array[tmp_id].cat_id, 0, url_class_array[tmp_id].url, url_class_array[tmp_id].url_len);
				if ( 0 == ret )//&& is_nss_l3_hook_registered && *is_nss_l3_hook_registered)
				{
					block_rule_remove_hw_accel_entries(rule.tuple.src_ip, rule.tuple.dst_ip);
				}
#endif
#endif
...
out:
	if (nlhdr->nlmsg_flags & NLM_F_ACK)
		...
		netlink_ack(skb, nlhdr, ret, NULL);
#else
		netlink_ack(skb, nlhdr, ret);
#endif
	
	return;
}

static int url_class_send_thread(void  *data)
{
	struct url_class_entry *pos = NULL;
	unsigned long end = 0;
...
static int traffic_update(struct nf_conn *ct, u_int64_t traffic, bool is_current)
{
    ...
    for (i = 0; i < PCTL_OWNER_NUM; i++) {
        read_lock_bh(&owners[i].lock);
        if (owners[i].status != PCTL_STATUS_OK) {
            read_unlock_bh(&owners[i].lock);
            continue;
        }		
...
/*Hanjiayan@20190610 add*/
   ...
   if (PCTL_STATUS_OK == status)
   {
        ...
        else if(info->workday_limit || info->workday_bedtime
                    || info->weekend_limit || info->weekend_bedtime)
        {
#if defined(CONFIG_TP_AVIRA_PATCH)
            if(!BROADSTREAM_AVIRA_ENABLE())
#endif
            {
                if(blog_ptr(skb))
                {
