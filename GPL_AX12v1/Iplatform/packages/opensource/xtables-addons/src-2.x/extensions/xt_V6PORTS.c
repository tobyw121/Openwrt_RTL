...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0)
extern bool l4proto_in_range(const struct nf_conntrack_tuple *tuple,
			     enum nf_nat_manip_type maniptype,
			     const union nf_conntrack_man_proto *min,
			     const union nf_conntrack_man_proto *max);
extern bool nf_nat_inet_in_range(const struct nf_conntrack_tuple *t,
...
/* synchronous kernel hash algorithm */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
int selectPortFromSets(struct nf_conntrack_tuple *tuple,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
		off = nf_nat_ipv4_secure_port(tuple, maniptype == NF_NAT_MANIP_SRC
							  ? tuple->dst.u.all
							  : tuple->src.u.all);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
void nf_nat_l4proto_unique_v6ports_tuple(struct nf_conntrack_tuple *tuple,
				 const struct nf_nat_range_v6ports *range,
				 enum nf_nat_manip_type maniptype,
				 const struct nf_conn *ct,
				 u16 *rover)
{
	...
	index = selectPortFromSets(tuple, range, maniptype, ct, rover);
	return;
}
#else
...
static void
icmp_unique_v6ports_tuple(struct nf_conntrack_tuple *tuple,
		  const struct nf_nat_range_v6ports *range,
		  enum nf_nat_manip_type maniptype,
		  const struct nf_conn *ct)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
static void
tcp_unique_v6ports_tuple(struct nf_conntrack_tuple *tuple,
		 const struct nf_nat_range_v6ports *range,
		 enum nf_nat_manip_type maniptype,
		 const struct nf_conn *ct)
{
	nf_nat_l4proto_unique_v6ports_tuple(tuple, range, maniptype, ct,
				    &tcp_port_rover);
}

static void
udp_unique_v6ports_tuple(struct nf_conntrack_tuple *tuple,
		 const struct nf_nat_range_v6ports *range,
		 enum nf_nat_manip_type maniptype,
		 const struct nf_conn *ct)
{
...
static void
udplite_unique_v6ports_tuple(struct nf_conntrack_tuple *tuple,
		 const struct nf_nat_range_v6ports *range,
		 enum nf_nat_manip_type maniptype,
		 const struct nf_conn *ct)
{
...
static void
dccp_unique_v6ports_tuple(struct nf_conntrack_tuple *tuple,
		 const struct nf_nat_range_v6ports *range,
		 enum nf_nat_manip_type maniptype,
		 const struct nf_conn *ct)
{
...
static void
sctp_unique_v6ports_tuple(struct nf_conntrack_tuple *tuple,
		 const struct nf_nat_range_v6ports *range,
		 enum nf_nat_manip_type maniptype,
		 const struct nf_conn *ct)
{
...
static void
gre_unique_v6ports_tuple(struct nf_conntrack_tuple *tuple,
		 const struct nf_nat_range_v6ports *range,
		 enum nf_nat_manip_type maniptype,
		 const struct nf_conn *ct)
{
 ...
 */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
static int v6ports_in_range(const struct nf_conntrack_tuple *tuple,
		    const struct nf_nat_range_v6ports *range)
{
	int i = 1;
	int num;
	...
	if (range->flags & NF_NAT_RANGE_MAP_IPS &&
		!nf_nat_inet_in_range(tuple, &rangeTmp))
	{
		return 0;
	}
	...
	{
		if (!(range->flags & NF_NAT_RANGE_PROTO_SPECIFIED) ||
			l4proto_in_range(tuple, NF_NAT_MANIP_SRC,
...
static int
find_v6ports_appropriate_src(struct net *net, const struct nf_conntrack_zone *zone,
		     const struct nf_conntrack_tuple *tuple,
		     ...
		     const struct nf_nat_range_v6ports *range)
{
	...
	hlist_for_each_entry_rcu(ct, &nf_nat_bysource[h], nat_bysource) {
		if (same_src(ct, tuple) &&
			...
			if (v6ports_in_range(result, range))
				return 1;
		}
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
static void
find_v6ports_best_ips_proto(const struct nf_conntrack_zone *zone, struct nf_conntrack_tuple *tuple,
		    const struct nf_nat_range_v6ports *range,
		    const struct nf_conn *ct,
		    enum nf_nat_manip_type maniptype)
#else
		  ...
		  __u8 protocol)
{
	...
	if (maniptype == NF_NAT_MANIP_SRC &&
		...
		if (v6ports_in_range(orig_tuple, range)) {
			if (!nf_nat_used_tuple(orig_tuple, ct)) {
				*tuple = *orig_tuple;
				goto out;
			}
		}else if (find_v6ports_appropriate_src(net, zone,
	...
	*tuple = *orig_tuple;
	...
	if (!(range->flags & NF_NAT_RANGE_PROTO_RANDOM)) {
		if (range->flags & NF_NAT_RANGE_PROTO_SPECIFIED) {
			...
			{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
				if (l4proto_in_range(tuple, maniptype,
							  &range->min_proto[i],
							  &range->max_proto[i]) &&
	...
	//l4proto->unique_v6ports_tuple(l3proto, tuple, range, maniptype, ct);
		...
		{
			case IPPROTO_TCP:
				tcp_unique_v6ports_tuple(tuple, range, maniptype, ct);
				break;
			case IPPROTO_UDP:
				udp_unique_v6ports_tuple(tuple, range, maniptype, ct);
				break;
			case IPPROTO_UDPLITE:
				udplite_unique_v6ports_tuple(tuple, range, maniptype, ct);
				break;
			case IPPROTO_DCCP:
				dccp_unique_v6ports_tuple(tuple, range, maniptype, ct);
				break;
			case IPPROTO_SCTP:
				sctp_unique_v6ports_tuple(tuple, range, maniptype, ct);
				break;
			case IPPROTO_ICMP:
				icmp_unique_v6ports_tuple(tuple, range, maniptype, ct);
				break;
			case IPPROTO_GRE:
				gre_unique_v6ports_tuple(tuple, range, maniptype, ct);
				break;
			default:
				break;
	}
...
#else
		spin_lock_bh(&nf_nat_v6ports_lock);
#endif
...
#else
		spin_unlock_bh(&nf_nat_v6ports_lock);
#endif
#endif
	}
...
xt_v6ports_target_v0(struct sk_buff *skb, const struct xt_action_param *par)
{
    ...
    {
		struct in_device *in_dev = __in_dev_get_rcu(par->out);
#endif
