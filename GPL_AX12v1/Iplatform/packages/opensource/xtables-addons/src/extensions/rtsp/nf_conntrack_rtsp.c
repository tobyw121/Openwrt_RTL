...
void expect_func(struct nf_conn* ct, struct nf_conntrack_expect *exp)
{
...
static inline int
add_expected_link(struct sk_buff *skb, struct ip_ct_rtsp_expect *expinfo, __be32  srcs_addr,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,7,0)
	 struct nf_conn *ct, enum ip_conntrack_info ctinfo,
	 unsigned int protoff)
#else
...
#endif
{
	...
	if (nf_ct_expect_related(rtp_exp) == 0) {
		if (rtcp_exp && nf_ct_expect_related(rtcp_exp) != 0) {
			nf_ct_unexpect_related(rtp_exp);
			pr_info("nf_conntrack_expect_related failed for rtcp\n");
			printk("nf_conntrack_expect_related failed for rtcp\n");
			ret = NF_DROP;
		}
	} else {
		pr_info("nf_conntrack_expect_related failed for rtp\n");
		printk("nf_conntrack_expect_related failed for rtp\n");
		ret = NF_DROP;
	}
	...
	struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while (dataoff < datalen)
	{
		...
		if (strncmp(pdata+cmdoff, "RTSP/1.0 ", 9) != 0)
			continue;   /* not a SETUP message */
		pr_debug("found a response message\n");

		if (translen)
		{
			pr_debug("have transport data\n");
			...
			if ((pTemp = strstr(pTranData, "client_port=")))
			{
				...
				expinfo.loport = loPort;
				loPort -= portex;

				rbuf1len = sprintf(rbuf1, "%hu", loPort);

				pr_debug("the numlen is %d, using port %hu\n", numlen, loPort);
				if (pTranData[off]== '-')
				{
					off++;
					origlen ++;
					...
					off += numlen;
					origlen += numlen;
					expinfo.pbtype = pb_range;
					expinfo.hiport = hiPort;
					hiPort -= portex;
					rbufalen = sprintf(rbufa, "%hu-%hu", loPort, hiPort);
					pr_debug("using port %hu-%hu\n", loPort, hiPort);
			...
			if(locatlen && nf_strncasecmp(pdata + locatoff, "Location:", 9) == 0)
			{
				pr_debug("Location find\n");
				...
				if (srcs_addr)
				{
					add_expected_link(skb, &expinfo, srcs_addr, ct, ctinfo);
				}
			}
		}
	}
