...
#define OPENVPNC_REMOTE "/tmp/openvpnc_remote"
...
/* 
 * When client's status is CONNECTED, save remote ip and poort to file 
 */
static void
write_remote(struct context *c)
{
    struct gc_arena gc = gc_new();
    struct link_socket_actual *actual;
    struct buffer out = alloc_buf_gc(256, &gc);
    FILE *fp = platform_fopen(OPENVPNC_REMOTE, "w");
    if (!fp)
    {
        msg(M_ERR, "Open error on remote file %s", OPENVPNC_REMOTE);
        goto out;
    }

    actual = &get_link_socket_info(c)->lsa->actual;
    buf_printf(&out, "%s %s", proto_is_udp(c->options.ce.proto) ? "udp" : "tcp",
                              print_link_socket_actual(actual, &gc));
    msg(M_INFO, "save remote to file : %s", BSTR(&out));
    fprintf(fp, "%s\n", BSTR(&out));
