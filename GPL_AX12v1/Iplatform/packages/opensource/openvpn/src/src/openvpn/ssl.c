      ...
      if (true)
	{
	  ...
	  if (ks->state == S_PRE_START && FULL_SYNC)
	    {
	      ...
	      if (session->opt->server)
		{
		  TP_LOG_OPENVPNS(OPENSPNS_RECV_INIT_REQ, LP_STRING, 
		  					print_sockaddr(&ks->remote_addr.dest, &gc));
		}
#endif
	  ...
	  if (((ks->state == S_GOT_KEY && !session->opt->server) ||
	      ...
	      if (FULL_SYNC)
		{
	      ...
	      if (session->opt->server)
		{
	      TP_LOG_OPENVPNS(OPENSPNS_KEY_NEG_SUCCESS, LP_STRING, 
							  print_sockaddr(&ks->remote_addr.dest, &gc));
		}
#endif
	      //add by zengwei for tp log 20180920 above
	  ...
	  if (buf->len
	      ...
	      else
		{
	      ...
	      if (session->opt->server)
		{
	      TP_LOG_OPENVPNS(OPENSPNS_SSL_NEG_SUCCESS, LP_STRING, 
								print_sockaddr(&ks->remote_addr.dest, &gc));
		}
#endif
	      //add by zengwei for tp log 20180920 above
