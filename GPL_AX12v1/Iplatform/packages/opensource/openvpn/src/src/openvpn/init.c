...
#include "common.h" //add by zengwei for tp log 20180920
#endif
...
#ifdef CONFIG_UGLY_LOG
	  struct gc_arena gc = gc_new ();
	  TP_LOG_OPENVPNS(OPENSPNS_CONNECTED, LP_STRING, 
	      c->c2.to_link_addr ? print_sockaddr(&c->c2.to_link_addr->dest, &gc) : "");
	  gc_free (&gc);
#endif
