...
/*      VERIFY_PERMISSION (OPT_P_MTU); */
  ...
  else if (streq (p[0], "persist-tun"))
    {
      ...
      /* Fix Bug 716453 [Technical support feedback] The DUT uses the OpenVPN client function to connect to the server. 
      When the server is disconnected, the web UI shows that the VPN client status is connected. 
      The device in the device list is unable to access the network. It is restored after manual reconnection.*/
      // options->persist_tun = true;
      options->persist_tun = false;
    }
