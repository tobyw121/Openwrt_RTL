...
#include "log.h"
#define TP_LOG_PPTPVPNS(msgid, ...) log(PROJ_LOG_ID_PPTPVPNS, msgid, ##__VA_ARGS__)
#endif
...
#ifdef CONFIG_UGLY_LOG
	int pptp_ctrl_type; //add by zengwei for tp log 20180920
#endif
	...
	for (;;) {
		...
		if (FD_ISSET(clientSocket, &fds)) {
...
#ifdef CONFIG_UGLY_LOG
				TP_LOG_PPTPVPNS(PPP_PPTP_RECV_OCRQ, LP_STRING, inet_ntoa(inetaddrs[1]));
#endif
			...
			case START_CTRL_CONN_RQST:
				TP_LOG_PPTPVPNS(PPP_PPTP_RECV_START_CCRQ, LP_STRING, inet_ntoa(inetaddrs[1]));
				break;
#endif
			//add by zengwei for tp log 20180920 above
		...
		if (send_packet == TRUE)
		{
			...
			{
				TP_LOG_PPTPVPNS(PPP_PPTP_SEND_START_CCRP, LP_STRING, inet_ntoa(inetaddrs[1]));
			}
			else if (pptp_ctrl_type == OUT_CALL_RQST)
			{
				TP_LOG_PPTPVPNS(PPP_PPTP_SEND_OCRP, LP_STRING, inet_ntoa(inetaddrs[1]));
			}
		}
#endif
		//add by zengwei for tp log 20180920 above
...
/* 
 * FIXBUG: C5400S remote client dial pptp vpn server failed
