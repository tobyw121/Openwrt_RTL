...
#include "log.h"
#define TP_LOG_ICMPV6(msgid, ...) log(PROJ_LOG_ID_ICMPV6, msgid, ##__VA_ARGS__)
...
#ifdef CONFIG_UGLY_LOG
static void
print_prefix(struct AdvPrefix *prefix, char *buff, int buflen)
{
	...
	while (p)
	{
		if(p->enabled)
		{
			...
			//res = inet_ntop(AF_INET6, (void *)(&addr->sin6_addr), addr_str, INET6_ADDRSTRLEN);
			res = inet_ntop(AF_INET6, (void *)(&p->Prefix), addr_str, INET6_ADDRSTRLEN);
			addr_str[INET6_ADDRSTRLEN - 1] = '\0';
			if (res)
			{
				tmplen = strlen(addr_str);
				if (len + tmplen + 5 >= buflen) /*1-byte number less than 255 */
				{
					break;
				}
				len += sprintf(buff + len, "%s/%d ", addr_str, prefix->PrefixLen);
			}
		}
	...
	struct sockaddr_in6 *addr)
{
	...
	memset(addr_str, 0, INET6_ADDRSTRLEN);
	if (addr)
		inet_ntop(AF_INET6, (void *)(&addr->sin6_addr), addr_str, INET6_ADDRSTRLEN);
	TP_LOG_ICMPV6(ICMPV6_RECV_SOLICIT, LP_STRING, addr_str);
#endif
	//add by zengwei for tp log 20180920 above
	...
	if (addr) {
		...
		//add by zengwei for tp log 20180920 below
#ifdef CONFIG_UGLY_LOG
		print_prefix(iface->AdvPrefixList, tmpbuf, TP_LOG_BUFF_LEN);
		TP_LOG_ICMPV6(ICMPV6_SEND_ADVERTISE, LP_STRING, tmpbuf[0] != '\0' ? tmpbuf : "<null>",
			iface->AdvLinkMTU != 0 ? LP_UINT32 : LP_STRING, iface->AdvLinkMTU != 0 ? iface->AdvLinkMTU : "<null>");
#endif
		//add by zengwei for tp log 20180920 above
	}
