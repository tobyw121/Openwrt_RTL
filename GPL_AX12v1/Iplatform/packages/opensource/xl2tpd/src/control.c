...
int control_finish (struct tunnel *t, struct call *c)
{
    ...
    case 0:
		...
		if (t->chal_them.challenge)
            ...
            TP_LOG_PPP(PPP_L2TP_SEND_START_CCRQ, LP_INT32, t->ourtid, LP_STRING, IPADDY (t->peer.sin_addr));
#endif
...
/*					c->ourrws = c->lac->call_rws;
                ...
                //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
                TP_LOG_PPP(PPP_L2TP_SEND_ICRQ, LP_INT32, c->ourcid);
#endif
...
/*                                      c->ourrws = c->lac->call_rws;
    ...
    case SCCRQ:
        //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
        TP_LOG_PPP(PPP_L2TP_RECV_START_CCRQ, LP_INT32, t->tid);
#endif
        /*
        ...
        //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
        TP_LOG_PPP(PPP_L2TP_SEND_START_CCRP, LP_INT32, t->ourtid);
#endif
        if (gconfig.debug_state)
        {
			#ifdef TP_WEB_LOG
			log_to_web("sent SCCRP");
			#else
    ...
    case SCCRP:
        ...
        //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
        TP_LOG_PPP(PPP_L2TP_RECV_START_CCRP, LP_INT32, t->tid);
#endif
        if (t->fc < 0)
        {
            if (DEBUG)
        ...
        //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
        TP_LOG_PPP(PPP_L2TP_SEND_START_CCCN);
#endif
        if (gconfig.debug_state)
        {
    ...
    case ICRQ:
        //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
        TP_LOG_PPP(PPP_L2TP_RECV_ICRQ, LP_INT32, p->cid);
#endif
        p = t->call_head;
        if (!p->lns)
        {
...
/*		if (p->ourrws >=0)
        ...
        //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
        TP_LOG_PPP(PPP_L2TP_SEND_ICRP, LP_INT32, p->ourcid);
#endif
        if (gconfig.debug_state)
        {
			#ifdef TP_WEB_LOG
			log_to_web("sent ICRP");
			#else
    ...
    case ICRP:
        //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
        TP_LOG_PPP(PPP_L2TP_RECV_ICRP, LP_INT32, c->cid);
#endif
        if (c->cid < 0)
        {
            if (DEBUG)
