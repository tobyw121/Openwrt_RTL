...
#include <net/route.h>
#include <sys/ioctl.h>
#include <linux/version.h> /* for two kernel support. by HouXB, 11Nov10 */

#define sin_addr(s) (((struct sockaddr_in *)(s))->sin_addr)

#define DEBUG_HOUXB 
#ifdef DEBUG_HOUXB
/* for tmp debug info out. by HouXB, 29Oct10 */		
#define PRF(_fmt, args...)			\
	do {						\
		  printf("HouXB(Xl2tpd.c): " _fmt, ##args);		\
	} while(0)
#else
#define PRF(_fmt, args...) 
#endif
...
static int no_handle_child_sig = 0;

static void open_controlfd(void);

#ifdef TP_WEB_LOG
#define MSG_BUF_LEN 	80
void log_to_web(const char *fmt, ...)
{	
	va_list va_alist;	
	char msgBuf[MSG_BUF_LEN]; 
	//sprintf(stderr, "YangXv:in log_to_web\n");

	memset(msgBuf, 0, MSG_BUF_LEN);	
	va_start(va_alist, fmt);	
	vsnprintf(msgBuf, MSG_BUF_LEN, fmt, va_alist);	
	msgBuf[MSG_BUF_LEN - 1] = 0;	
	va_end(va_alist);	
	
	msglogd(LOG_INFO, LOGTYPE_PPP, msgBuf);
}
#else
void log_to_web(const char *fmt, ...){};
#endif
...
static void kill_child()
{
    ...
    while (t)
    {
        c = t->call_head;
        t = t->next;
        while (c)/* kill all child process */
        {
			if (c->pppd > 0)
			{
          		kill(c->pppd, SIGTERM);
...
#ifdef QSDK/*  by huangwenzhong, 07Mar14 */
	close = 1;
	kill_child();
	sleep(1);
#endif
...
/* use different interface for kernel 2.6.31 and kernel 2.6.15. by HouXB, 11Nov10 */
#if ( LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31) ) 		

       int session_fd;
       fd2 = socket(AF_PPPOX, SOCK_DGRAM, PX_PROTO_OL2TP);

       log_to_web("pppoxl2tp fd2:%d", fd2);
       if (fd2 < 0) {
	...
	/* set udp socket peer addr info.(Pppol2tp conncet error bug fix!) by HouXB, 02Nov10 */
	...
	session_fd = socket(AF_PPPOX, SOCK_DGRAM, PX_PROTO_OL2TP);
       log_to_web("pppol2tp session fd:%d", session_fd);
       if (session_fd < 0) {
           l2tp_log (LOG_WARNING, "%s: Unable to allocate PPPoL2TP socket.\n",
                __FUNCTION__);
		   close(fd2);
...
/* not use plugin mode, beacase pppol2tp is compiled into pppd rather than .so, by lqt. */
#if 0
    ...
    else if (!c->pppd)
    {
...
#endif
       {
        ...
        if( c->dialing[0] )
        {
	    	...
	    	log_to_web("Fun:%s ,Line:%d, info:%d, %s", __FUNCTION__, __LINE__, 0, c->dialing);
        }
...
void daemonize() {
    ...
    if (i != 0) {
		...
		//added by yangxv for debug, 2008.050.09
		#ifndef XL2TP_DEBUG
        if (dup2(0, 2) == -1)
