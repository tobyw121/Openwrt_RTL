...
//Init the alert info list
LIST_HEAD(alert_info);
...
int blocking_ip(sfaddr_t src_ip, sfaddr_t dst_ip, uint16_t port, bool if_port, int event_type)
{
	char cmd[CMD_LEN];
	char ip_src_trans_buf[IP_TRANS_LEN];
	char ip_dst_trans_buf[IP_TRANS_LEN];
	memset(ip_src_trans_buf, 0, sizeof(ip_src_trans_buf));
	memset(ip_dst_trans_buf, 0, sizeof(ip_dst_trans_buf));
	int cat = 0, reason_id = 0;

	if (event_type == ATTACK_NETWORK_PENETRATION)
	{
		cat = URL_CAT_SECURITY_INTRUSION;
		reason_id = 2;
	}
	else if (event_type == ATTACK_DDOS)
	{
		cat = URL_CAT_SECURITY_IOT;
		reason_id = 3;
	}
	else
		return -1;

	if(src_ip.family == AF_INET && dst_ip.family == AF_INET)
	{
		inet_ntop(AF_INET, &src_ip.ia32[3], ip_src_trans_buf, sizeof(ip_src_trans_buf));
		inet_ntop(AF_INET, &dst_ip.ia32[3], ip_dst_trans_buf, sizeof(ip_dst_trans_buf));
	}
#ifdef SNORT_SUPPORT_IPV6
	else if (src_ip.family == AF_INET6 && dst_ip.family == AF_INET6)
	{
		inet_ntop(AF_INET6, &src_ip.ia32[0], ip_src_trans_buf, sizeof(ip_src_trans_buf));
		inet_ntop(AF_INET6, &dst_ip.ia32[0], ip_dst_trans_buf, sizeof(ip_dst_trans_buf));
	}
#endif
	else
	{
	...
	if(if_port)
	{
		/* Only block tcp and udp protocol dst port */
		if(reason_id)
			sprintf(cmd, "blockingc -A -s %s -d %s -D %d -t 0x0E10 -c %d -r %d", ip_src_trans_buf, ip_dst_trans_buf, port, cat, reason_id);
		else
			return -1;
	}
	else
	{
		if(reason_id)
			sprintf(cmd, "blockingc -A -s %s -d %s -t 0x0E10 -c %d -r %d", ip_src_trans_buf, ip_dst_trans_buf, cat, reason_id);
		else
			return -1;
	}
	printf("[snort]blocking cmd is %s\n", cmd);
	system(cmd);
	return 0;
}

/*
 * judge the flow direction through src & dst ip
 * return 0 if incoming, return 1 if outgoing
 * return -1 if error
 */
int judge_flow_direction(struct attack_info *info)
{
	unsigned int cmp_src, cmp_dst;
	if(info->src_ip.family == AF_INET && info->dst_ip.family == AF_INET)
	{
		cmp_src = ntohl(info->src_ip.ia32[3]) >> 16;
		cmp_dst = ntohl(info->dst_ip.ia32[3]) >> 16;
		if(cmp_dst == IPV4_LOCAL && cmp_src != IPV4_LOCAL) {
			return 0;
		} else {
			return 1;
		}
	}
	else if(info->src_ip.family == AF_INET6 && info->dst_ip.family == AF_INET6)
	{
		cmp_src = ntohl(info->src_ip.ia32[0]) >> 16;
		cmp_dst = ntohl(info->dst_ip.ia32[0]) >> 22;
		if(cmp_dst == IPV6_LOCAL && (cmp_src == IPV6_PUBLIC1 ||
			cmp_src == IPV6_PUBLIC2 || cmp_src == IPV6_PUBLIC3)) {
			return 0;
		} else {
			return 1;
		}
...
int set_report_info(char *report_buf, struct attack_info * att_info)
{
	int len = 0, j =0;
	sprintf(report_buf, "report_upload_alert");
	len += 19;
	report_buf[len] = ' ';
	len += 1;
	//attack info
	...
	for(j = 0; j < 6; j++)
	{
		sprintf(report_buf + len + j*2, "%02x", att_info->src_mac[j]);
	}
	len += 12;
	report_buf[len] = ' ';
	len += 1;
	//source ip
	if(att_info->src_ip.family == AF_INET)
	{
		sprintf(report_buf + len, "%08x", ntohl(att_info->src_ip.ia32[3]));
		len += 8;
	}
	else if (att_info->src_ip.family == AF_INET6)
	{
		for(j = 0; j < 4; j++)
		{
			sprintf(report_buf + len, "%08x", ntohl(att_info->src_ip.ia32[j]));
			len += 8;
		}
	}
	report_buf[len] = ' ';
	len += 1;
	//dst ip
	if(att_info->dst_ip.family == AF_INET)
	{
		sprintf(report_buf + len, "%08x", ntohl(att_info->dst_ip.ia32[3]));
		len += 8;
	}
	else if (att_info->dst_ip.family == AF_INET6)
	{
		for(j = 0; j < 4; j++)
		{
			sprintf(report_buf + len, "%08x", ntohl(att_info->dst_ip.ia32[j]));
			len += 8;
		}
	}
	report_buf[len] = ' ';
	len += 1;
	//attack type
	sprintf(report_buf + len, "%02x", att_info->attack_type);
	len += 2;
	report_buf[len] = ' ';
	len += 1;
	//event type
	sprintf(report_buf + len, "%02x", att_info->event_type);
	len += 2;
	return 0;
}

int update_alert_list(struct _Packet *pack, int event_type, int attack_type, uint32_t rule_id)
{
	struct _Packet *p = pack;
	struct attack_info info_tmp;
	struct total_alert_info *tmp;
	struct total_alert_info *tmp_del;
	unsigned long long *y;
	char x[32] = {0};
	char write_buf[REPORT_BUF_SIZE];
	char *att_info;
	int flow_dir, i = 0;
	bool if_match = false;
	memset(&info_tmp, 0, sizeof(struct attack_info));
	/* copy mac, src and dst ip, attack type and event type */
	if(event_type == ATTACK_PORTSCANING)
	{
		memcpy(info_tmp.src_mac, p->eh->ether_src, 6);

		info_tmp.src_ip = *(GET_SRC_IP(p));
		info_tmp.dst_ip = *(GET_DST_IP(p));

		info_tmp.event_type = event_type;
		info_tmp.attack_type = attack_type;

		/* For portscan, only deal with LAN->WAN flow, ignore WAN->LAN alert */
		flow_dir = judge_flow_direction(&info_tmp);
		if(0 == flow_dir || -1 == flow_dir)
			return 0;
		printf("event_type: portscaning | attack_type: portscaning detected! Rule id is %d\n", rule_id);
	}

	if((event_type == ATTACK_NETWORK_PENETRATION) || (event_type == ATTACK_DDOS))
	{
		memcpy(info_tmp.src_mac, p->eh->ether_dst, 6);

		info_tmp.src_ip = *(GET_DST_IP(p));
		info_tmp.dst_ip = *(GET_SRC_IP(p));

		info_tmp.event_type = event_type;
		info_tmp.attack_type = attack_type;

		/* network penetration need to judge the attack direction */
		if(event_type == ATTACK_NETWORK_PENETRATION)
		{
			flow_dir = judge_flow_direction(&info_tmp);
			...
			if(0 == flow_dir)
			{
				printf("event_type: penetration | attack_type: incoming detected! Rule id is %d\n", rule_id);
				info_tmp.attack_type = PENETRATION_INCOMING;
			}
			else if (1 == flow_dir)
			{
				printf("event_type: penetration | attack_type: outgoing detected! Rule id is %d\n", rule_id);
				info_tmp.attack_type = PENETRATION_OUTGOING;
			}
			else
				return 0;
		}

		if((event_type == ATTACK_DDOS) && (attack_type == DDOS))
			printf("event_type: attack | attack_type: ddos detected! Rule id is %d\n", rule_id);
	}
	att_info = (char *)&info_tmp;
	/* calculate hash value */
	y = (unsigned long long *)SHA256(att_info, sizeof(struct attack_info), x);
	/* search the list to check whether attack info exists */
	/* If exists, count++. Remove the entry if the inital time excceds 60min */
	list_for_each_entry_safe(tmp, tmp_del, &alert_info, list) {
		if(time(NULL) - tmp->last_time > DEGRADATION_INTERVAL)
		{
			list_del(&tmp->list);
			free(tmp);
			continue;
		}
		/* If attack info exists in list */
		if(0 == memcmp((char *)&tmp->info, att_info, sizeof(struct attack_info)))
		{
			tmp->last_time = time(NULL);
			tmp->count++;
			if_match = true;
		}
	}
	...
	/* block ddos src ip */
	if((event_type == ATTACK_DDOS) || (event_type == ATTACK_NETWORK_PENETRATION))
	{
		uint16_t src_port = 0;
		bool if_port = false;

		/* Only deal with tcp/udp protocol dst port */
		if(pack->tcph || pack->udph)
		{
			if_port = true;
			src_port = pack->sp;
		}

		blocking_ip(info_tmp.src_ip, info_tmp.dst_ip, src_port, if_port, event_type);
	}

	/* If attack info exists in list, return and do not report */
	if(true == if_match)
		goto out;

	/* Print some detail alert information(ddos+penetration) */
	if(pack->tcph)
		printf("Alert packet l4 protocol is tcp, packet flags is %x, src port is %d, dst port is %d\n", p->tcph->th_flags, p->sp, p->dp);
	else if(pack->udph)
		printf("Alert packet l4 protocol is udp, src port is %d, dst port is %d\n", p->sp, p->dp);
	else if(pack->icmph)
		printf("Alert packet l4 protocol is icmp\n");

	/* add to list and report*/
	memset(write_buf, 0, sizeof(write_buf));
	set_report_info(write_buf, &info_tmp);
	printf("[snort]cloud report cmd is %s\n", write_buf);
	system(write_buf);
	struct total_alert_info *tmp_add;
	tmp_add = (struct total_alert_info *)malloc(sizeof(struct total_alert_info));
	memset(tmp_add, 0, sizeof(struct total_alert_info));
	tmp_add->info = info_tmp;
	tmp_add->cv = *y;
	tmp_add->init_time = time(NULL);
	tmp_add->last_time = time(NULL);
	tmp_add->count = 1;
	/* add to list */
	list_add(&tmp_add->list, &alert_info);
out:
	return 0;
}
