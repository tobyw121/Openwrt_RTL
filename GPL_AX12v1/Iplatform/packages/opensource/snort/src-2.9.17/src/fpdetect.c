...
*/
int fpLogEvent(RuleTreeNode *rtn, OptTreeNode *otn, Packet *p, bool is_ps)
{
    int action = -1,
        rateAction = -1,
        filterEvent = 0;
    ...
    */
    ...
    switch (action)
    {
        ...
        case RULE_TYPE__ALERT:
            ...
            if(is_ps)
            {
                update_alert_list(p, ATTACK_PORTSCANING, PORTSCANING, otn->sigInfo.id);
            }
            else
            {
                /* Juge whether ddos or penetration by rule sid */
                if(otn->sigInfo.id < PENETRATION_RULE_START && otn->sigInfo.id > DDOS_RULE_START)
                    update_alert_list(p, ATTACK_DDOS, DDOS, otn->sigInfo.id);
                else if (otn->sigInfo.id > PENETRATION_RULE_START)
                    /* Attack type judgement of penetration will be dealt in update_alert_list */
                    update_alert_list(p, ATTACK_NETWORK_PENETRATION, 0, otn->sigInfo.id);
            }
            SetTags(p, otn, rtn, event_id);
            break;
