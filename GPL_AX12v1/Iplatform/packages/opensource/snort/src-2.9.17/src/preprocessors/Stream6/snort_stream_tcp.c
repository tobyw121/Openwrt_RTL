...
#define STREAM_DEFAULT_TCP_PACKET_MEMCAP  1048576  /* 1MB */
...
static void addStreamTcpPolicyToList( StreamTcpConfig *config, StreamTcpPolicy *policy )
{
    ...
    {
        config->policy_list = ( StreamTcpPolicy ** ) SnortAlloc( sizeof( StreamTcpPolicy * ) );
    }
    else
    {
        uint32_t policyListSize = sizeof( StreamTcpPolicy * ) * ( config->num_policies );
        StreamTcpPolicy **tmpPolicyList = ( StreamTcpPolicy ** ) SnortAlloc( policyListSize );
...
{
    StreamTcpPolicy *tcp_policy = (StreamTcpPolicy *) SnortAlloc(sizeof(StreamTcpPolicy));
    ...
    for( i = 0; i < MAX_PORTS; i++ )
    {
        tcp_policy->flush_config[i].client.flush_policy = STREAM_FLPOLICY_IGNORE;
        tcp_policy->flush_config[i].server.flush_policy = STREAM_FLPOLICY_IGNORE;
    }
#ifdef TARGET_BASED
    for( i = 0; i < MAX_PROTOCOL_ORDINAL; i++)
    {
        tcp_policy->flush_config_protocol[i].client.flush_policy = STREAM_FLPOLICY_IGNORE;
        tcp_policy->flush_config_protocol[i].server.flush_policy = STREAM_FLPOLICY_IGNORE;
    }
#endif
...
StreamTcpPolicy *StreamTcpPolicyClone( StreamTcpPolicy *master )
{
    StreamConfig *config;
...
{
    if (flush_policy_for_dir == STREAM_FLPOLICY_FOOTPRINT_NOACK ||
...
{
    NetworkReassemblyPorts *nrp = calloc( 1, sizeof( NetworkReassemblyPorts ) );
...
{
    int idx;
    StreamConfig *config = (StreamConfig *) sfPolicyUserDataGet(stream_online_config, getParserPolicy(sc));
