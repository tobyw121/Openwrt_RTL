...
# TP-LINK
if [ -f ./os.tplink ]; then
	OS_VERSION="$(cat ./os.tplink)"
    if [ "$OS_VERSION" = "TP-Link" ];then
		OS_NAME=TP-Link
	else
		OS_NAME=TP-LINK
	fi
...
# MERCUSYS
if [ -f ./os.mercusys ]; then
	OS_VERSION="$(cat ./os.mercusys)"
    if [ "$OS_VERSION" = "MERCUSYS" ];then
		OS_NAME=MERCUSYS
	else
		OS_NAME=MERCUSYS
	fi
