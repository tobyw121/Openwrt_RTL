...
lease_file_remove(unsigned short eport, int proto)
{
	...
	while( fgets(buf, sizeof(buf)-1, fd) != NULL) {
		...
		if (buf_size < str_size || strncmp(str, buf, str_size)!=0 || (strncmp(str, buf, 
		str_size)==0 && buf[str_size]!=':')) {
...
int reload_from_lease_file()
{
	...
	fd = fopen( lease_file, "r");
	if (fd==NULL) {
		syslog(LOG_INFO, "could not open lease file: %s", lease_file);
		return -1;
	}
              ...
              unsigned int leaseduration)
{
	...
	if(r == 0) {
		...
		if (max_rules) {
			/* Max rules limit. */
			unsigned int number = (unsigned int)upnp_get_portmapping_number_of_entries();
			if (number >= max_rules) {
				syslog(LOG_INFO, "exceeded max rules %u", max_rules);
				return -7;
			}
		}
