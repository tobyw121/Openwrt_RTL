...
#elif !defined(CONFIG_GCC_VERSION_10_3)
...
/*uuid:e4c797e0-82ce-4f84-b56d-085d9d77c8cd*/
static int getRandomPrefix(char * uuid, char * prefix)
{
	long long int val = 0;
	char *token = NULL;
	int prefix_idx = 0;
	...
	strncpy(room, uuid, 63);

	token = strtok(room, "-");
	while(token && prefix_idx < RANDOM_PREFIX_LEN - 1)
	{
		val = strtoll(token, NULL, 16);
		prefix[prefix_idx++] = (char)(val%26 + 97); /* ASCII 97->'a' */
		token = strtok(NULL, "-");
	}
...
static int initRandomPath()
{
	struct {
		...
		{dummyPath, "/dummy.xml"},
#endif
		{wancfgPath, "/WANCfg.xml"},
		{wancfgControlURL, "/ctl/CmnIfCfg"},
		{wancfgEventURL, "/evt/CmnIfCfg"},
		{wanIPCPath, "/WANIPCn.xml"},
		{wanIPCControlURL, "/ctl/IPConn"},
		{wanIPCEventURL, "/evt/IPConn"},
#ifdef ENABLE_L3F_SERVICE
		{L3FPath, "/L3F.xml"},
		{L3FControlURL, "/ctl/L3F"},
		{L3FEventURL, "/evt/L3F"},
#endif
#ifdef ENABLE_6FC_SERVICE
		{wanIP6FCPath, "/WANIP6FC.xml"},
		{wanIP6FCControlURL, "/ctl/IP6FCtl"},
		{wanIP6FCEventURL, "/evt/IP6FCtl"},
#endif
	...
	while(pathObj[path_idx].path)
	{
		snprintf(pathObj[path_idx].path, RANDOM_PATH_LEN - 1, "/%s%s", randomprefix, pathObj[path_idx].value);
		pathObj[path_idx].path[RANDOM_PATH_LEN - 1] = '\0';
		path_idx++;
	}
	return 0;
}
	...
	else
	{
		for(i=0; i<(int)num_options; i++)
		{
			...
			case UPNPUUID:
				...
				getRandomPrefix(uuidvalue+5, randomprefix);
				break;
			case UPNPSERIAL:
            ...
            case UPNPMANUFACTURER:
				strncpy(manufacturer, ary_options[i].value, MANUFACTURER_MAX_LEN);
				manufacturer[MANUFACTURER_MAX_LEN-1] = '\0';
				break;    
            ...
            case UPNPMODEL_DESCRIPTION:
				strncpy(modeldescription, ary_options[i].value, MODELDESCRIPTION_MAX_LEN);
				modeldescription[MODELDESCRIPTION_MAX_LEN-1] = '\0';
				break;    
            case UPNPMODEL_NAME:
			...
			case UPNPCLEANINTERVAL:
				...
				break;
			case UPNPMAXRULES:
				max_rules = strtoul(ary_options[i].value, 0, 0);
				break;
#ifdef USE_PF
			case UPNPANCHOR:
