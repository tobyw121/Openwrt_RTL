...
/* bits of char and double significant part */
#define CHAR_BIT        (8)
...
static unsigned long adjust_uint(unsigned long n)
{
	...
	if (DBL_MANT_DIG < wordbits) {
		unsigned long one = 1;
		unsigned long sentinel = one << (wordbits - DBL_MANT_DIG);
		int shift = count_trailing_zeros(n | sentinel);
		unsigned long mask = (one << DBL_MANT_DIG) - 1;
...
#define tspl (G.exec_builtin__tspl)
	...
	case B_an:
		temp = getvar_i_int(av[0]) & getvar_i_int(av[1]);
		setvar_i(res, adjust_uint(temp));
		break;

	case B_co:
		temp = ~ getvar_i_int(av[0]);
		setvar_i(res, adjust_uint(temp));
		break;
