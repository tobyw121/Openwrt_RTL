...
#if (defined SUPPORT_PLATFORM_QCA)||(defined SUPPORT_PLATFORM_MTK)
#include <unistd.h>
#else
    ...
    for (p1 = p2 = client_get_first_client(); NULL != p1; p1 = p2) {
        ...
        if (!(p1 = client_list_find(ip, mac))) {
            ...
            /*if (p1->counters.last_updated +
				...
				<= current_time) */
			if ((current_time - p1->counters.last_updated) >= (config->checkinterval * p1->counters.timeout)){
                /* Timing out user */
                debug(LOG_ERR, "%s - %s - %s Inactive for more than %ld seconds, removing client and denying in firewall",
                        p1->ip, p1->mac, p1->token, config->checkinterval * p1->counters.timeout);
