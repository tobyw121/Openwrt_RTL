...
static ssize_t procfile_read_str(struct file * file, char *data, size_t len, loff_t *off)
{
...
static ssize_t procfile_write_str(struct file *file, const char *data, size_t len, loff_t *off)
{
    int length = 0;
...
unsigned int 
br_prerouting_hook(void *hookOps, struct sk_buff *skb,
                       ...
                       int (*okfn)(struct sk_buff *))
{
	...
	char *list_all = NULL;
	char *dev_name = NULL;
	unsigned char list_temp[MAX_INTERFACE_LIST*IFNAMSIZ] = {0};
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0))
unsigned int br_forward_hook(void *hookOps, struct sk_buff *skb,
                   const struct nf_hook_state *hookState)
{
    struct net_device *in = hookState->in;
#elif (LINUX_VERSION_CODE > KERNEL_VERSION(4,1,0))
...
static int __init br_wifidog_init(void)
{
    ...
    ret = nf_register_net_hook(&init_net, &brpho);
#else       
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,13,0)
    ret = nf_register_net_hook(&init_net, &brfho);
#else  
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,13,0)
    nf_unregister_net_hook(&init_net, &brpho);
    nf_unregister_net_hook(&init_net, &brfho);
#else     
