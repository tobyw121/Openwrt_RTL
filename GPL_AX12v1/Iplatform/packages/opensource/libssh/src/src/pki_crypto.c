...
#else
        if (DSA_set0_pqg(new->dsa, BN_dup(DSA_get0_p(key->dsa)), BN_dup(DSA_get0_q(key->dsa)),
                  BN_dup(DSA_get0_g(key->dsa))) == 0) {
            goto fail;
        }
#endif
...
#else
        if (DSA_set0_key(new->dsa, BN_dup(DSA_get0_pub_key(key->dsa)) ,NULL) == 0) {
            goto fail;
        }
#endif
...
#else
            if (DSA_set0_key(new->dsa, NULL, BN_dup(DSA_get0_priv_key(key->dsa))) == 0) {
                goto fail;
            }
#endif
        }
...
#else
        if (RSA_set0_key(new->rsa, BN_dup(RSA_get0_n(key->rsa)), 
                BN_dup(RSA_get0_e(key->rsa)), NULL) == 0) {
            goto fail;
        }
#endif
...
#else
            if (RSA_set0_key(new->rsa, NULL, NULL, BN_dup(RSA_get0_d(key->rsa))) == 0) {
                goto fail;
            }
            if (RSA_get0_p(key->rsa) != NULL) {
                if (RSA_set0_factors(new->rsa, BN_dup(RSA_get0_p(key->rsa)), NULL) == 0) {
                    goto fail;
                }
            }

            if (RSA_get0_q(key->rsa) != NULL) {
                if (RSA_set0_factors(new->rsa, NULL, BN_dup(RSA_get0_q(key->rsa))) == 0) {
                    goto fail;
                }
            }

            if (RSA_get0_dmp1(key->rsa) != NULL) {
                if (RSA_set0_crt_params(new->rsa, BN_dup(RSA_get0_dmp1(key->rsa)), NULL, NULL) == 0) {
                    goto fail;
                }
            }

            if (RSA_get0_dmq1(key->rsa) != NULL) {
                if (RSA_set0_crt_params(new->rsa, NULL, BN_dup(RSA_get0_dmq1(key->rsa)), NULL) == 0) {
                    goto fail;
                }
            }

             if (RSA_get0_iqmp(key->rsa) != NULL) {
                if (RSA_set0_crt_params(new->rsa, NULL, NULL, BN_dup(RSA_get0_iqmp(key->rsa))) == 0) {
                    goto fail;
                }
            }
#endif
...
#else
            if (bignum_cmp(DSA_get0_p(k1->dsa), DSA_get0_p(k2->dsa)) != 0) {
                return 1;
            }
            if (bignum_cmp(DSA_get0_q(k1->dsa), DSA_get0_q(k2->dsa)) != 0) {
                return 1;
            }
            if (bignum_cmp(DSA_get0_g(k1->dsa), DSA_get0_g(k2->dsa)) != 0) {
                return 1;
            }
            if (bignum_cmp(DSA_get0_pub_key(k1->dsa), DSA_get0_pub_key(k2->dsa)) != 0) {
                return 1;
            }

            if (what == SSH_KEY_CMP_PRIVATE) {
                if (bignum_cmp(DSA_get0_priv_key(k1->dsa), DSA_get0_priv_key(k2->dsa)) != 0) {
                    return 1;
                }
            }
#endif
...
#else
            if (bignum_cmp(RSA_get0_e(k1->rsa), RSA_get0_e(k2->rsa)) != 0) {
                return 1;
            }
            if (bignum_cmp(RSA_get0_n(k1->rsa), RSA_get0_n(k2->rsa)) != 0) {
                return 1;
            }

            if (what == SSH_KEY_CMP_PRIVATE) {
                if (bignum_cmp(RSA_get0_p(k1->rsa), RSA_get0_p(k2->rsa)) != 0) {
                    return 1;
                }

                if (bignum_cmp(RSA_get0_q(k1->rsa), RSA_get0_q(k2->rsa)) != 0) {
                    return 1;
                }
            }
#endif
...
#else
    if (DSA_set0_pqg(key->dsa, make_string_bn(p), make_string_bn(q),
            make_string_bn(g)) == 0) {
        DSA_free(key->dsa);
        return SSH_ERROR;
    }

    if (DSA_set0_key(key->dsa, make_string_bn(pubkey), NULL) == 0) {
        DSA_free(key->dsa);
        return SSH_ERROR;
    }
#endif
...
#else
    if (RSA_set0_key(key->rsa, make_string_bn(n), make_string_bn(e), NULL) == 0) {
        RSA_free(key->rsa);
        return SSH_ERROR;
    }
#endif
...
#else
            p = make_bignum_string(DSA_get0_p(key->dsa));
            if (p == NULL) {
                goto fail;
            }

            q = make_bignum_string(DSA_get0_q(key->dsa));
            if (q == NULL) {
                goto fail;
            }

            g = make_bignum_string(DSA_get0_g(key->dsa));
            if (g == NULL) {
                goto fail;
            }

            n = make_bignum_string(DSA_get0_pub_key(key->dsa));
            if (n == NULL) {
                goto fail;
            }
#endif
...
#else
            e = make_bignum_string(RSA_get0_e(key->rsa));
            if (e == NULL) {
                goto fail;
            }

            n = make_bignum_string(RSA_get0_n(key->rsa));
            if (n == NULL) {
                goto fail;
            }
#endif
...
#else
    e = bignum_bn2dec(RSA_get0_e(key->rsa));
    if (e == NULL) {
        return SSH_ERROR;
    }

    n = bignum_bn2dec(RSA_get0_n(key->rsa));
    if (n == NULL) {
        OPENSSL_free(e);
        return SSH_ERROR;
    }
...
#else
    const BIGNUM *dsa_sig_r, *dsa_sig_s;
    DSA_SIG_get0(sig->dsa_sig, &dsa_sig_r, &dsa_sig_s);

    r = make_bignum_string(dsa_sig_r);
    if (r == NULL) {
        return NULL;
    }
    s = make_bignum_string(dsa_sig_s);
    if (s == NULL) {
        ssh_string_free(r);
        return NULL;
    }
...
#else
            r = make_bignum_string(ECDSA_SIG_get0_r(sig->ecdsa_sig));
#endif
            if (r == NULL) {
                ssh_buffer_free(b);
                return NULL;
...
#else
            s = make_bignum_string(ECDSA_SIG_get0_s(sig->ecdsa_sig));
#endif
            if (s == NULL) {
                ssh_buffer_free(b);
                return NULL;
                                      ...
                                      enum ssh_keytypes_e type)
{
    ...
    switch(type) {
        case SSH_KEYTYPE_DSS:
            ...
            // DSA_SIG_set0 多次调用时，后面的会覆盖前面的结果,源码openssl/src/crypto/dsa/dsa_ans1.c:line 49
            rc = DSA_SIG_set0(sig->dsa_sig, make_string_bn(r), make_string_bn(s));
            ssh_string_free(r);
            ssh_string_free(s);
            if (rc == 0) {
                ssh_signature_free(sig);
        ...
        case SSH_KEYTYPE_ECDSA:
            ...
            { /* build ecdsa siganature */
                ...
                ecdsa_sig_r = BN_new();
                ecdsa_sig_s = BN_new();
                if (ecdsa_sig_r == NULL || ecdsa_sig_s == NULL) {
                    if (ecdsa_sig_r != NULL)
                        BN_free(ecdsa_sig_r);
                    if (ecdsa_sig_s != NULL)
                        BN_free(ecdsa_sig_s);
                ...
                make_string_bn_inplace(r, ecdsa_sig_r);
                ssh_string_burn(r);
                ssh_string_free(r);
                s = buffer_get_ssh_string(b);
                rlen = buffer_get_rest_len(b);
                ...
                ssh_print_hexa("s", ssh_string_data(s), ssh_string_len(s));
#endif
                make_string_bn_inplace(s, ecdsa_sig_s);
                ssh_string_burn(s);
                ssh_string_free(s);
                if (ECDSA_SIG_set0(sig->ecdsa_sig, ecdsa_sig_r, ecdsa_sig_s) == 0) {
                    BN_free(ecdsa_sig_r);
                    BN_free(ecdsa_sig_s);
                    ssh_signature_free(sig);
                }
