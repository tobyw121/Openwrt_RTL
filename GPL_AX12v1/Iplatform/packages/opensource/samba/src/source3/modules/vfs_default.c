...
static int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, SMB_OFF_T len)
{
	...
	fstatfs(fsp->fh->fd, &fs_buf);

	if(fs_buf.f_type != 0x5346544e)	/*manpage fstatfs :   0x5346544e--->NTFS;  0X4d44--->FAT */
	{
		if(fs_buf.f_type == 0x4d44)
		{
			//we do not preallocate space for FAT system, instead give a special handle here. The issue is that we increase disk fragment by doing so---wubaijun
			//#ifdef LARGE_SMB_OFF_T //we check if the filesystem is FAT---wubaijun
			if (len > 0x00000000ffffffff ) /* larger than 4GB */
			{
				errno = EFBIG;
				goto done;
			}
		...
		double space_avail;
		space_avail = (double)(fs_buf.f_bfree*fs_buf.f_bsize);
		DEBUG(1,("avail space is %f\n", space_avail));
		if(space_avail == -1)
		{
			goto done;
		}
	
		if((double)len > space_avail)
		{ //instead we just check if the file space needed is bigger than the available space on usb---wubaijun
			errno = ENOSPC;
			goto done;
		}
	
		if((double)len < space_avail)
		{ //shrink the file size should work fine for FAT---wubaijun
//			if (SMB_VFS_FSTAT(fsp, pst) == -1)
//			{
//				goto done;
//			}
			
			if(pst->st_ex_size <= len)
			{ //do not ftruncate if we need to expand the size
				result = 0;
			goto done;
			}
