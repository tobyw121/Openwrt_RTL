			    ...
			    enum auth_password_state password_state)
{
    ...
    TALLOC_CTX *ctx = talloc_tos();
    char *denynames = lp_deny_login_names();
    char **userlist; 
    char **useriter;

    DEBUG(5, ("smb_name = %s, denynames = %s\n", smb_name, denynames));
    userlist = str_list_make_v3(ctx, denynames, NULL);
    DEBUG(5, ("userlist = %s\n", userlist == NULL ? "NULL": userlist));
    if (userlist != NULL) {
        useriter = userlist;
        while (*useriter != NULL) {
            DEBUG(5, ("*useriter = %s\n", *useriter));
            if (strcmp(*useriter, smb_name) == 0) {
                TALLOC_FREE(userlist);
