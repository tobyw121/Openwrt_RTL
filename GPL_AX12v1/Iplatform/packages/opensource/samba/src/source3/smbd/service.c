					...
					NTSTATUS *pstatus)
{
	...
	*pstatus = share_sanity_checks(&sconn->client_id, snum, dev);
...
#ifdef GUEST_NETWORK_LIMIT
    /* TODO: Guest Network Handler */
    ret = getpeername(sconn->sock, (struct sockaddr *)&addr, &addrlen);
    if (ret != 0)
    {
    ...
    (void)gnet_is_guest(saddr->sin_addr.s_addr, &is_gnet);
    DEBUG(10, ("snum = %d, guest network enable = %d, is_gnet = %d\n", snum, 
          lp_guest_network_enable(snum), is_gnet));
    if (is_gnet == 1 && !lp_guest_network_enable(snum)) 
    {
        *pstatus = NT_STATUS_ACCESS_DENIED;
        goto err_root_exit;
    }
