	...
	*fixed_portion = 0;
	...
	switch (info_level) {
		case SMB_INFO_ALLOCATION:
		{
			...
			DEBUG(0, ("current path = %s\n", cur_path));
			in = setmntent("/proc/mounts", "r");
			while ((mnt = getmntent(in)) != NULL)
			{
				if (strncmp(cur_path, mnt->mnt_dir, strlen(cur_path)) == 0)
				{
					snprintf(custom_path, sizeof(custom_path), "%s", mnt->mnt_dir);
					break;
				}
			}
			(void)endmntent(in);
			...
			{
				if (get_dfree_info(conn,custom_path,False,&bsize,&dfree,&dsize) == (uint64_t)-1) {
					return map_nt_error_from_unix(errno);
				}
			}
