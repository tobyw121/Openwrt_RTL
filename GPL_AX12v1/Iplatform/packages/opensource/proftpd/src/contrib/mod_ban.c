...
#include "log.h"
#define TP_LOG_USB(msgid, ...) log(PROJ_LOG_ID_USB, msgid, ##__VA_ARGS__)
#endif
...
#ifdef CONFIG_UGLY_LOG
        TP_LOG_USB(USB_MAXLOGINATTEMPS_EXPIRED, LP_STRING, ban_lists->bans.bl_entries[i].be_name);  
#endif
          ...
          (void) pr_log_writefile(ban_logfd, MOD_BAN_VERSION,
            "added %s-triggered autoban for %s '%s'", event,
              ban_type == BAN_TYPE_USER ? "user" :
                ban_type == BAN_TYPE_HOST ? "host" : "class", src);
#ifdef CONFIG_UGLY_LOG
        TP_LOG_USB(USB_MAXLOGINATTEMPS_AUTOBAN , LP_STRING, src);  
#endif 
        }
...
#ifdef CONFIG_UGLY_LOG
      TP_LOG_USB(USB_MAXLOGINATTEMPS_REACHED);  
#endif 
