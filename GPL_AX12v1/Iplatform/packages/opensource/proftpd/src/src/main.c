...
static void fork_server(int fd, conn_t *l, unsigned char nofork) {
  ...
  pr_log_pri(PR_LOG_DEBUG, "Connection from %s [%s].",
      session.c->remote_name,
      pr_netaddr_get_ipstr(session.c->remote_addr));

#ifdef PR_USE_GUEST_LIMIT
  /* Guest Network determining */
  ipaddr = pr_netaddr_get_addrno(session.c->remote_addr);
  if (ipaddr != 0)
  {
      (void)gnet_is_guest(ipaddr, &session.is_gnet);
      pr_log_pri(PR_LOG_DEBUG, "Connection from %s [%s] is from Guest Network? [%d]",
          session.c->remote_name, pr_netaddr_get_ipstr(session.c->remote_addr), session.is_gnet);
  }
#endif 
...
int main(int argc, char *argv[], char **envp) {
  ...
  if (gnet_ubus_init() != 0)
  {
      pr_log_pri(PR_LOG_ERR, "Ubus Connected failed...");
      exit(1);
  }
#endif 
