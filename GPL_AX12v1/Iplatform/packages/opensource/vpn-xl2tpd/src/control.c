...
/*                                      c->ourrws = c->lac->call_rws;
    ...
    case StopCCN:
       ...
       /* Work around bug in MSL2TP client */
		...
		sprintf(logfile, "/tmp/tunnelid%d", t->ourtid);
		if ((f = fopen(logfile, "r")) != NULL) {
		    memset(mbuf, 0, sizeof(mbuf));
			while (fgets(mbuf, sizeof(mbuf), f) != NULL) {
				if (mbuf[strlen(mbuf) - 1] == '\n')
					mbuf[strlen(mbuf) - 1] = '\0';
			}
			l2tp_log (LOG_NOTICE, "tunnel ip file : %s\n", mbuf);
			sscanf(mbuf, "%s %s", pidof_pppd, assign_ip);
			l2tp_log (LOG_NOTICE, "pidof pppd : %s, assign_ip : %s\n", pidof_pppd, assign_ip);
		    fclose(f);
		}
		if (strlen(assign_ip) > 0) {
			memset(cmd, 0, sizeof(cmd));
			sprintf(cmd, "uci get l2tpoveripsec.l2tpoveripsec.access", c->pppd);
			pf = popen(cmd, "r");
			if (pf) {
				memset(mbuf, 0, sizeof(mbuf));
				while (fgets(mbuf, sizeof(mbuf), pf) != NULL) {
					if (mbuf[strlen(mbuf) - 1] == '\n')
						mbuf[strlen(mbuf) - 1] = '\0';
				}
				l2tp_log (LOG_NOTICE, "access : %s\n", mbuf);
				if (!strcmp(mbuf, "home")) {
					memset(cmd, 0, sizeof(cmd));
					sprintf(cmd, "iptables -D forwarding_rule_vpn -s %s -o eth0.4094 -j DROP", assign_ip);
					system(cmd);
				}
				pclose(pf);
			}
    ...
    case ICCN:
	    ...
	    */
        ...
        l2tp_log (LOG_NOTICE,
             ...
             c->serno);
		/* add by suweilin */
		sprintf(cmd, "cat /proc/%d/cmdline | xargs -0 echo", c->pppd);
		while (strlen(assign_ip) <= 0 && cnt < 5) {
			pf = popen(cmd, "r");
			if (pf) {
				memset(mbuf, 0, sizeof(mbuf));
				while (fgets(mbuf, sizeof(mbuf), pf) != NULL) {
				...
				l2tp_log (LOG_NOTICE, "remote ip : %s\n", mbuf);
				sscanf(mbuf, "%*[^:]:%s", assign_ip);
				l2tp_log (LOG_NOTICE, "assign_ip : %s\n", assign_ip);			
				pclose(pf);
			}
			if (strlen(assign_ip) > 0){
				break;
		...
		/*add by gkw*/
		//make sure we served as server, client is now connecting to us
		//if ( !strncmp(c->lns->authname, "TP_Link", 7) )
		{
			memset(logfile, 0, sizeof(logfile));
			sprintf(logfile, "/tmp/tunnel-ppp-%d", c->pppd);
			if ((f = fopen(logfile, "w")) != NULL) {
				/*this file will be used and then deleted in l2tp-vpn-server-up hotplug shell*/
				fprintf(f, "peer-ip:%s\n", IPADDY(t->peer.sin_addr));
				fprintf(f, "pppd:%d\n", c->pppd);
				fprintf(f, "peer-ppp-ip:%s\n", assign_ip);
				fclose(f);
			}
		}
		/*add end*/
		if (strlen(assign_ip) > 0) {
			...
			memset(cmd, 0, sizeof(cmd));
			sprintf(cmd, "uci get l2tpoveripsec.l2tpoveripsec.access");
			pf = popen(cmd, "r");
			if (pf) {
				memset(mbuf, 0, sizeof(mbuf));
				while (fgets(mbuf, sizeof(mbuf), pf) != NULL) {
				...
				if (!strcmp(mbuf, "home")) {
					memset(cmd, 0, sizeof(cmd));
					sprintf(cmd, "iptables -I forwarding_rule_vpn -s %s -o eth0.4094 -j DROP", assign_ip);
					system(cmd);
				}
				pclose(pf);
			}
