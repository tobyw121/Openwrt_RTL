...
void show_status (void)
{
	...
	/* add by suweilin */
	memset(path, 0, 64);
	sprintf(path, "/var/run/l2tp_status-%u", getpid());
	unlink(path);
	/* add end */
    while (t)
    {
		...
		memset(path, 0, 64);
		sprintf(path, "/var/run/l2tp_status-%u.lock", getpid());
		if (-1 == (lock_fd = open(path, O_WRONLY | O_CREAT | O_EXCL)))
		{
			l2tp_log (LOG_WARNING, "Create %s failed\n", path);
		}
		unlink(path);
		/* add end */
...
void magic_lac_dial (void *data)
{
    ...
    lac->rsched = NULL;
    lac->rtries++;
	l2tp_log(LOG_INFO, "[%s][%d] retry %d\n", __func__, __LINE__, lac->rtries);
...
route_ctrl(int ctrl, struct rtentry *rt)
{
	...
	if ((s = socket(AF_INET, SOCK_DGRAM, 0)) < 0 ||	ioctl(s, ctrl, rt) < 0)
		l2tp_log (LOG_CRIT, "%s: route_ctrl: %s\n", __FUNCTION__, strerror(errno));
	else errno = 0;
...
route_add(const struct in_addr inetaddr, struct rtentry *rt)
{
	...
	if (f == NULL) {
		l2tp_log (LOG_CRIT, "%s: /proc/net/route: %s\n", __FUNCTION__, strerror(errno));
		return -1;
	}
	...
	{
		l2tp_log (LOG_CRIT, "%s: no route to host\n", __FUNCTION__);
		return -1;
	}
	...
	if (mask == INADDR_BROADCAST) {
		l2tp_log (LOG_CRIT, "%s: not adding existing route\n", __FUNCTION__);
		return -1;
	}
