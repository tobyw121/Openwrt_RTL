...
#define RESULT_FILENAME_FORMAT_EX "/tmp/l2tpvpn/xl2tpd-control-%i.out"
...
int main (int argc, char *argv[])
{
    ...
    {
        print_error (ERROR_LEVEL,
            "warn: unable to open %s for reading.\n", result_filename);

		/*try another path, maybe we run as l3tpvpn*/
		memset(result_filename, 0, 128);
		snprintf (result_filename, 128, RESULT_FILENAME_FORMAT_EX, getpid());
		unlink (result_filename);
		mkfifo (result_filename, 0600);
		result_fd = open (result_filename, O_RDONLY | O_NONBLOCK, 0600);
		if (result_fd < 0)
