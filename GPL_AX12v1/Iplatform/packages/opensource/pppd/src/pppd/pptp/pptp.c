...
#else
/* change header file dir to parent dir, by lqt */
#include "../pppd.h"
#include "../fsm.h"
#include "../lcp.h"
#include "../ipcp.h"
...
/* define myself, because no config tool, config.h, by lqt, 2010-3-29 */
#undef VERSION
#define VERSION "1.8.3"
#define KERNELVERSION "2.6.15"
#define PPPD_VERSION "2.4.3"

extern char** environ;

static const char pppd_version[] = PPPD_VERSION;
...
static int pptp_start_client(void)
{
  	...
  	if (!hostinfo)
	{
		...
		log_to_web(LOG_INFO, "pptp server ip resolve failed!");
		return -1;
	}
	...
	dst_addr.sa_addr.pptp.sin_addr=*(struct in_addr*)hostinfo->h_addr;	
	{
		...
		if (connect(sock,(struct sockaddr*)&addr,sizeof(addr)))
		{
			...
			log_to_web(LOG_INFO, "PPTP: connect failed.");
			return -1;
		}
	...
	if (pptp_fd<0)
	{
		...
		log_to_web(LOG_INFO, "creat pppox pptp protocol socket failed.");
		return -1;
	}
	...
	if (nic_ifname[0])
	{
		setsockopt(pptp_fd, SOL_SOCKET, SO_BINDTODEVICE, nic_ifname, strlen(nic_ifname) + 1);
	}

	if (bind(pptp_fd,(struct sockaddr*)&src_addr,sizeof(src_addr)))
	{
		fatal("PPTP: failed to bind PPTP socket (%s)\n",strerror(errno));
		log_to_web(LOG_INFO, "bind pppox pptp socket failed.");
		return -1;
	}
  	...
  	if (connect(pptp_fd,(struct sockaddr*)&dst_addr,sizeof(dst_addr)))
	{
		...
		log_to_web(LOG_INFO, "connect pppox pptp socket failed.");
		return -1;
	}
...
static int open_callmgr(int call_id,struct in_addr inetaddr, char *phonenr,int window)
{
	...
	/* log pppd pid, so call mgr process can use it, by lqt, 2010.5.11 */
	pppd_pid = getpid();
