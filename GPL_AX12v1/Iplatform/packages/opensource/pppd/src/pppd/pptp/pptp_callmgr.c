...
struct in_addr g_inputaddr; //add by zengwei for tp log 20181120
#endif
...
void call_callback(PPTP_CONN *conn, PPTP_CALL *call, enum call_state state)
{
    ...
    switch(state) {
        ...
        case CALL_CLOSE_DONE:
             ...
             * are sync'ed */
            notice("Closing connection (call state)");
            ...
            break;
        default:
            notice("Unhandled call callback state [%d].", (int) state);
            break;
    }
}
...
/* fix bug 106950, route_add function do not add the correct rt_gateway of host route
   when pptp server and dhcp server is not in the same subnet,
   now we add the pptp server host route in ppp.sh according netifd */
#if 0
...
#ifdef CONFIG_UGLY_LOG
	memcpy(&g_inputaddr, &inetaddr, sizeof(struct in_addr)); //add by zengwei for tp log 20181120
#endif
        ...
        for (i = 0; i < max_fd + 1; i++)
            if (FD_ISSET(i, &read_set)) {
                ...
                if (retval) {
                    ...
                    notice("Closing connection (unhandled)");
                    if(lci->pid[0] > 1) kill(lci->pid[0], SIGTERM);
                    if(lci->pid[1] > 1) kill(lci->pid[1], SIGTERM);
                    free(lci);
...
shutdown:
    {
        ...
        for (i = 0; i < vector_size(call_list); i++) {
            ...
            notice("Closing connection (shutdown)");
            //pptp_call_close(conn, call);
            if(lci->pid[0] > 1) kill(lci->pid[0], SIGTERM);
            if(lci->pid[1] > 1) kill(lci->pid[1], SIGTERM);
        }
...
find_route(u_int32_t addr, int bHost)
{
	...
	while (fgets(buf, sizeof(buf), f)) 
	{
		...
		if ((flags & RTF_UP) == RTF_UP)
		{
			...
			{
				if (((flags & RTF_GATEWAY) == 0) && (mask != 0)/* 默 */
