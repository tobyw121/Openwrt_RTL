...
#include "log.h"
#define TP_LOG_PPP(msgid, ...) log(PROJ_LOG_ID_PPP, msgid, ##__VA_ARGS__)
extern struct in_addr g_inputaddr;
#endif
...
{
    if( cause >= 0)
        notice("Result code is %d '%s'. Error code is %d, Cause code is %d",
                result, result_text[result <= max_result ?  result : 0], error,
                cause );
    else
        notice("Reply result code is %d '%s'. Error code is %d",
                result, result_text[result <= max_result ?  result : 0], error);
    if ((error > 0) && (error <= MAX_GENERAL_ERROR)){
        if( result != PPTP_RESULT_GENERAL_ERROR )
            notice("Result code is something else then \"general error\", "
                    "so the following error is probably bogus.");
        notice("Error is '%s', Error message: '%s'",
...
static void ctrlp_rep( void * buffer, int size, int isbuff)
{
    ...
    notice("%s control packet type is %d '%s'\n",isbuff ? "Buffered" : "Sent",
...
#ifdef CONFIG_UGLY_LOG
			TP_LOG_PPP(PPP_PPTP_SEND_START_CCRQ, LP_STRING, inet_ntoa(g_inputaddr));
#endif
		}
        else
        ...
        char *phonenr,int window)
{
    ...
    if (pptp_send_ctrl_packet(conn, &packet, sizeof(packet))) {
        //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
        TP_LOG_PPP(PPP_PPTP_SEND_OCRQ, LP_INT32, call->call_id, LP_INT32, call->sernum);
#endif
        pptp_reset_timer();
...
void pptp_call_close(PPTP_CONN * conn, PPTP_CALL * call)
{
    ...
    //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
    TP_LOG_PPP(PPP_PPTP_SEND_CCRQ, LP_INT32, call->call_id);
#endif
...
void pptp_conn_close(PPTP_CONN * conn, u_int8_t close_reason)
{
    ...
    //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
    TP_LOG_PPP(PPP_PPTP_SEND_STOP_CCRQ);
#else
    notice("Closing PPTP connection");
#endif
    pptp_send_ctrl_packet(conn, &rqst, sizeof(rqst));
    pptp_reset_timer(); /* wait 60 seconds for reply */
    conn->conn_state = CONN_WAIT_STOP_REPLY;

	/* tell pppd to exit, by lqt, 2010.5.10 */
...
int pptp_write_some(PPTP_CONN * conn) {
    ...
    if (retval < 0) { /* error. */
        if (errno == EAGAIN || errno == EINTR) {
            ...
            notice("write error: %s", strerror(errno));
	    return -1;
        }
    }
...
int pptp_read_some(PPTP_CONN * conn)
{
    ...
    if (retval == 0) {
        notice("read returned zero, peer has closed");
        return -1;
    }
    if (retval < 0) {
	    ...
	    return 0;
            ...
            notice("read error: %s", strerror(errno));
            return -1;
        }
    }
...
int pptp_make_packet(PPTP_CONN * conn, void **buf, size_t *size)
{
    ...
    while ((conn->read_size-bad_bytes) >= sizeof(struct pptp_header)) {
        ...
        if (ntoh16(header->reserved0) != 0)
            notice("reserved0 field is not zero! (0x%x) Cisco feature? \n",
        ...
        *buf = malloc(*size);
        if (*buf == NULL) { notice("Out of memory."); return 0; /* ack! */ }
        ...
        memmove(conn->read_buffer, conn->read_buffer + bad_bytes + *size,
                conn->read_size);
        if (bad_bytes > 0)
            notice("%lu bad bytes thrown away.", (unsigned long) bad_bytes);
        return 1;
throwitout:
        bad_bytes++;
    }
...
int pptp_dispatch_packet(PPTP_CONN * conn, void * buffer, size_t size)
{
    ...
    switch (ntoh16(header->pptp_type)) {
        ...
        case PPTP_MESSAGE_MANAGE:
            ...
            notice("PPTP management message received, but not understood.");
            break;
        default:
            notice("Unknown PPTP control message type received: %u",
...
static void logecho( int type)
{
    ...
    if( nlogecho > 0) {
        notice( "Echo Re%s received.", type == PPTP_ECHO_RQST ? "quest" :"ply");
        if( --nlogecho == 0)
            notice("no more Echo Reply/Request packets will be reported.");
    }
}
...
int ctrlp_disp(PPTP_CONN * conn, void * buffer, size_t size)
{
    ...
    if (size < PPTP_CTRL_SIZE(ntoh16(header->ctrl_type))) {
        notice("Invalid packet received [type: %d; length: %d].",
    ...
    switch (ntoh16(header->ctrl_type)) {
        ...
        case PPTP_START_CTRL_CONN_RQST:
        {
            ...
            notice("Received Start Control Connection Request");
            ...
            if (conn->conn_state == CONN_IDLE) {
                if (ntoh16(packet->version) < PPTP_VERSION) {
                    ...
                    if (pptp_send_ctrl_packet(conn, &reply, sizeof(reply))) {
                        ...
                        notice("server connection ESTABLISHED.");
                        pptp_reset_timer();
                    }
                }
            }
        ...
        case PPTP_START_CTRL_CONN_RPLY:
        {
            ...
            //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
            TP_LOG_PPP(PPP_PPTP_RECV_START_CCRP, LP_INT32, packet->result_code, 
						LP_INT32, packet->error_code,LP_STRING, packet->hostname);
#else
            notice("Received Start Control Connection Reply");
#endif		
            if (conn->conn_state == CONN_WAIT_CTL_REPLY) {
                ...
                if (ntoh8(packet->result_code) != 1 &&
                    ...
                    notice("Negative reply received to our Start Control "
                            "Connection Request");
                ...
                notice("Client connection established.");
                if (conn->callback != NULL)
        ...
        case PPTP_STOP_CTRL_CONN_RQST:
        {
            ...
            //add by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
            TP_LOG_PPP(PPP_PPTP_RECV_STOP_CCRQ);
#else
            notice("Received Stop Control Connection Request.");
#endif
        ...
        {
            notice("Received Stop Control Connection Reply.");
        ...
        case PPTP_OUT_CALL_RQST:
        {
            ...
            notice("Received Outgoing Call Request.");
        ...
        case PPTP_OUT_CALL_RPLY:
        {
...
#ifdef CONFIG_UGLY_LOG
            //add by zengwei for tp log 20180920
            TP_LOG_PPP(PPP_PPTP_RECV_OCRP, LP_INT32, callid, LP_INT32, ntoh16(packet->call_id), 
						LP_INT32, packet->result_code, LP_INT32, packet->error_code);
#else
            notice("Received Outgoing Call Reply.");
#endif			
            if (!vector_search(conn->call, (int) callid, &call)) {
                notice("PPTP_OUT_CALL_RPLY received for non-existant call: "
                        "peer call ID (us)  %d call ID (them) %d.",
                        callid, ntoh16(packet->call_id));
                break;
            }
            if (call->call_type != PPTP_CALL_PNS) {
                notice("Ack!  How did this call_type get here?"); /* XXX? */
                break;
            }
            ...
            if (packet->result_code != 1) {
                ...
                notice("Our outgoing call request [callid %d] has not been "
                        "accepted.", (int) callid);
                ctrlp_error(packet->result_code, packet->error_code,
                ...
                notice("Outgoing call established (call ID %u, peer's "
        ...
        case PPTP_CALL_CLEAR_RQST:
        {
...
#ifdef CONFIG_UGLY_LOG
            //add by zengwei for tp log 20180920
            TP_LOG_PPP(PPP_PPTP_RECV_CCRQ, LP_INT32, ntoh16(packet->call_id));
#else
            notice("Received Call Clear Request.");
#endif
            ...
            if (vector_contains(conn->call, ntoh16(packet->call_id))) {
                ...
                pptp_send_ctrl_packet(conn, &reply, sizeof(reply));
                pptp_call_destroy(conn, call);
                notice("Call closed (RQST) (call id %d)", (int) call->call_id);
            }
            break;
        }
        case PPTP_CALL_CLEAR_NTFY:
        {
            ...
            notice("Call disconnect notification received (call id %d)",
        ...
        case PPTP_SET_LINK_INFO:
        {
            ...
            notice("PPTP_SET_LINK_INFO received from peer_callid %u",
                    (unsigned int) ntoh16(packet->call_id_peer));

            notice("  send_accm is %08lX, recv_accm is %08lX",
        ...
        default:
            ...
            notice("Unrecognized Packet %d received.",
