...
unsigned int pppoe_last_session_id = 0;
unsigned char *pppoe_last_remote_mac = NULL;	/* Hex string terminated with '\0' */
unsigned char *pppoe_last_local_mac = NULL;
...
PPPOEConnectDevice(void)
{
	...
	sprintf(buf,"echo %d > /proc/sys/net/netfilter/nf_athrs17_hnat_ppp_id", conn->session);
	system(buf);
#endif
	...
	sprintf(buf,"echo %02x%02x%02x%02x%02x%02x > /proc/sys/net/netfilter/nf_athrs17_hnat_ppp_peer_mac",
	...
	sprintf(buf,"echo %02x%02x%02x%02x%02x%02x > /proc/sys/net/netfilter/nf_athrs17_hnat_wan_mac",
...
PPPOEDisconnectDevice(void)
{
	...
	if (conn->discoverySocket >= 0) 
	{
		...
		TP_LOG_PPP(PPP_PPPOE_SEND_PADT, LP_STRING, "phase disconnect device");
#endif
		sendPADT(conn, NULL);
		close(conn->discoverySocket);
	}
...
fatalSys(char const *str)
{
	...
	//mod by zengwei for tp log 20180920
#ifdef CONFIG_UGLY_LOG
	TP_LOG_PPP(PPP_PPPOE_SEND_PADT, LP_STRING, buf);
#endif
    sendPADT(conn, buf);
    exit(1);
}
