...
#if defined(CONFIG_LIBC_USE_MUSL)
#include <netinet/in.h>
#elif defined(__GLIBC__) && defined(__GLIBC_PREREQ) && !__GLIBC_PREREQ(2,30) /* fix redefinition under glibc 2.30 */
#include <netinet/in.h>
#endif
...
#define PADI_ATTEMPTS_BEFORE_PADT 3
#define PADI_INTERVAL_ARRAY       {5, 10, 5, 5}
...
int sendPADTForLastSession(PPPoEConnection *conn, unsigned int sessionId, unsigned char *pRemoteMac, unsigned char *pLocalMac);
void initPPP(void);
...
extern unsigned int pppoe_last_session_id;
extern unsigned char *pppoe_last_remote_mac;
extern unsigned char *pppoe_last_local_mac;