...
sendPADT(PPPoEConnection *conn, char const *msg)
{
    ...
    syslog(LOG_INFO,"Sent PADT"); //del by zengwei for tp log 20180920
#endif
}

int readMACAddressFromHexString(unsigned char *mac, unsigned char *hex)
{
	int i, j, d;
	if (strlen(hex) != 12)
	{
...
int sendPADTForLastSession(PPPoEConnection *conn, unsigned int sessionId, 
		unsigned char *pRemoteMac, unsigned char *pLocalMac)
{
	PPPoEPacket packet;

	if (NULL == pRemoteMac || readMACAddressFromHexString(packet.ethHdr.h_dest, pRemoteMac) < 0)
	{
		memset(packet.ethHdr.h_dest, 0, ETH_ALEN);
	}

	if (NULL == pLocalMac || readMACAddressFromHexString(packet.ethHdr.h_source, pLocalMac) < 0)
	{
		memcpy(packet.ethHdr.h_source, conn->myEth, ETH_ALEN);
	}
	...
	packet.type = 1;
	packet.code = CODE_PADT;
	packet.session = (unsigned short)sessionId;
	packet.length = htons(0);

	sendPacket(conn, conn->discoverySocket, &packet, HDR_SIZE);
