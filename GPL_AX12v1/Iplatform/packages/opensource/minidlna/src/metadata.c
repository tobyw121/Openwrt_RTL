...
{
	int ret;
	ret = sql_get_int_field(db,"SELECT ID from DETAILS where PATH = \"%q\"",path);
	if(ret > 0)
		return ret;
...
GetImageMetadata(const char *path, char *name)
{
    ...
    if (file.st_size > (10 * 1024 * 1024))
    {
        /* Too large size */
        DPRINTF(E_DEBUG, L_METADATA, "Image file size over 10M: %s\n", path);
        return 0;
    }
    ...
    else if (ends_with(path, ".tiff") || ends_with(path, ".tif"))
    {
        image_type = IMAGE_TYPE_TIF;
        m.mime = strdup("image/tif");
    }
    else 
    {
        DPRINTF(E_WARN, L_GENERAL, "Unhandled file extension on %s\n", path);
	...
	if( !ed )
    {
        DPRINTF(E_DEBUG, L_GENERAL, "goto no_exifdata on %s\n", path);
		goto no_exifdata;
    }
...
error:
        ...
        if (width > 0 && height > 0)
        {
            if (width <= 48 && height <= 48)
                m.dlna_pn = strdup("JPEG_SM_ICO");
            else if (width <= 120 && height <= 120)
                m.dlna_pn = strdup("JPEG_LRG_ICO");
    ...
    case IMAGE_TYPE_TIF:
    {
        ...
        if (!tif)
        {
            DPRINTF(E_WARN, L_METADATA, "TIFFOpen %s failed!\n", path);
            goto out;
        }
        ...
        {
            DPRINTF(E_WARN, L_METADATA, "TIFFGetField(WIDTH, HEIGHT) %s failed!\n", path);
            TIFFClose(tif);
            goto out;
        }
    ...
    case IMAGE_TYPE_GIF:
    {
        ...
        {
            if (video_stream == -1 && ctx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO)
            {
                video_stream = i;
                vc = ctx->streams[video_stream]->codec;
                /* DPRINTF(E_DEBUG, L_METADATA, "codec: %dx%d\n", vc->width, vc->height); */
                break;
            }
        }
        ...
        if (!vc)
        {
            DPRINTF(E_DEBUG, L_METADATA, "%s\n", path);
            lav_close(ctx);
            goto out;
        }

        if (strcmp(ctx->iformat->name, "image2") != 0)
        {
            DPRINTF(E_DEBUG, L_METADATA, "%s\n", path);
            lav_close(ctx);
            goto out;
        ...
        if (vc->codec_id == CODEC_ID_PNG)
        {
            if (width <= 48 && height <= 48)
            {
                m.dlna_pn = strdup("PNG_SM_ICO");
            }
            else if (width <= 120 && height <= 120)
            {
                m.dlna_pn = strdup("PNG_LRG_ICO");
            }
            else if (width <= 160 && height <= 160)
            {
        ...
        else if (vc->codec_id == CODEC_ID_GIF)
        {
...
out:
    ...
    if (m.dlna_pn == NULL)
    {
        m.dlna_pn = strdup(mime_to_ext(m.mime));
    }
	if( !m.date )
	{
		m.date = malloc(20);
