...
int
directory_exist_in_db(const char *path)
{
	char **results;
	int rows;
	char *sql;
	...
	//directory
	sql = sqlite3_mprintf("SELECT PATH from DETAILS where PATH = '%q' and MIME is NULL ", path);
	sql_get_table(db, sql, &results, &rows, NULL);
...
CreateDatabase(void)
{
	...
	if( ret != SQLITE_OK )
		goto sql_failed;
	ret = sql_exec(db, "INSERT into SETTINGS values ('FILENUMS', '0')");
	if( ret != SQLITE_OK )
		goto sql_failed;
...
int
file_exist_in_db(sqlite3 *db, const char *path)
{
	char** results;
	int rows = 0;
	char *sql;
	...
	//file
	sql = sqlite3_mprintf("SELECT PATH from DETAILS where PATH = '%q' and MIME is not NULL", path);
	sql_get_table(db, sql, &results, &rows, NULL);
	sqlite3_free_table(results);
	sqlite3_free(sql);
	...
	//playlist
	sql = sqlite3_mprintf("SELECT PATH from PLAYLISTS where PATH = '%q'", path);
	sql_get_table(db, sql, &results, &rows, NULL);
	sqlite3_free_table(results);
	sqlite3_free(sql);
...
ScanDirectory(const char *dir, const char *parent, media_types dir_types)
{
	...
	DIR *pDir = NULL;
	struct dirent *entryItem = NULL;
	int flag = FLAG_NOT_FILE_OR_DIR;
	int ret = -1;

	if (g_scan_fileNumber >= max_scanfile_num)
	{
		return;
	}
	...
	while ( (entryItem= readdir(pDir)) != NULL )
	{
#if !USE_FORK
		if( quitting )
			break;
		...
		switch( dir_types )
		{
			...
			case TYPE_IMAGES:
				...
				break;
			default:
				flag = FLAG_NOT_FILE_OR_DIR;
				break;
		}

		if (flag == FLAG_NOT_FILE_OR_DIR)
		{
			continue;
		}
		...
		snprintf(full_path, PATH_MAX, "%s/%s", dir, entryItem->d_name);
		...
		else if (entryItem->d_type == DT_REG)
		{
			type = TYPE_FILE;
		}
		else
		...
		{
			if( !directory_exist_in_db(full_path))
			{
		...
		{
			if( !file_exist_in_db(db, full_path))
			{
		...
		if (g_scan_fileNumber >= max_scanfile_num)
		{
				DPRINTF(E_WARN, L_SCANNER, _("Scanning %s finished (%d files)!\n"), dir, g_scan_fileNumber);
				break;
		}
	}
	...
	if( !parent )
	{
		DPRINTF(E_WARN, L_SCANNER, _("Scanning %s finished (%d files)!max_scanfile_num = %d\n"), dir, g_scan_fileNumber, max_scanfile_num);
	}
...
start_scanner()
{
	...
	now = time(NULL) - now;
	DPRINTF(E_DEBUG, L_SCANNER, "Minidlna scanner complete! files and subdirectory:%u, time: %d\n", g_scan_fileNumber, now);
