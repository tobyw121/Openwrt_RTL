...
inotify_create_watches(int fd)
{
	...
	if( max_watches )
	{
		if( fscanf(max_watches, "%10u", &watch_limit) < 1 )
			watch_limit = 8192;
		fclose(max_watches);
		if( watch_limit < max_scanfile_num )
		{
			max_watches = fopen("/proc/sys/fs/inotify/max_user_watches", "w");
			if( max_watches )
			{
				fprintf(max_watches, "%u", max_scanfile_num);
				fclose(max_watches);
			}
			else
			{
	...
	DPRINTF(E_DEBUG, L_INOTIFY, "num_watches = %d, max_watches = %d \n", num_watches, watch_limit);
...
inotify_insert_file(char * name, const char * path)
{
	...
	if( !depth )
	{
		...
		if (insert_file(name, path, id+2, get_next_available_id("OBJECTS", id)) == 0)
		{
			scan_increaseFileNumber();
			sql_exec(db, "UPDATE SETTINGS set VALUE = '%u' where KEY = 'FILENUMS'", g_scan_fileNumber);
		}
...
start_inotify()
{
	...
	DPRINTF(E_DEBUG, L_INOTIFY,  "SELECT VALUE from SETTINGS where KEY = 'FILENUMS'\n");

	while (i < 5)
	{
		g_scan_fileNumber = sql_get_int_field(db, "SELECT VALUE from SETTINGS where KEY = 'FILENUMS'");
		DPRINTF(E_DEBUG, L_INOTIFY,  "'FILENUMS'= %d\n", g_scan_fileNumber);
		if (g_scan_fileNumber != -1)
			break;
		i++;
	}
