...
CFLAGS:=
ARCH:=$(subst i486,i386,$(subst i586,i386,$(subst i686,i386,$(call qstrip,$(CONFIG_LINUX_KARCH)))))
ARCH_PACKAGES:=$(call qstrip,$(CONFIG_TARGET_ARCH_PACKAGES))
BOARD:=$(call qstrip,$(CONFIG_TARGET_BOARD))
PRODUCT:=$(call qstrip,$(CONFIG_PRODUCT_NAME))
...
BIN_DIR:=$(TOPDIR)/bin/$(BOARD)-$(PRODUCT)
...
ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
  ...
  BUILD_DIR:=$(BUILD_DIR_BASE)/target-$(ARCH)$(ARCH_SUFFIX)$(DIR_SUFFIX)$(if $(BUILD_SUFFIX),_$(BUILD_SUFFIX))-$(PRODUCT)
  STAGING_DIR:=$(TOPDIR)/staging_dir/target-$(ARCH)$(ARCH_SUFFIX)$(DIR_SUFFIX)$(if $(BUILD_SUFFIX),_$(BUILD_SUFFIX))-$(PRODUCT)
  ...
  BUILD_DIR:=$(BUILD_DIR_BASE)/target-$(GNU_TARGET_NAME)$(if $(BUILD_SUFFIX),_$(BUILD_SUFFIX))-$(PRODUCT)
  STAGING_DIR:=$(TOPDIR)/staging_dir/target-$(GNU_TARGET_NAME)$(if $(BUILD_SUFFIX),_$(BUILD_SUFFIX))-$(PRODUCT)
...
TARGET_CPPFLAGS:=-I$(STAGING_DIR)/usr/include -I$(STAGING_DIR)/include -I$(TOPDIR)/staging_dir/usr-$(PRODUCT)/include
TARGET_LDFLAGS:=-L$(STAGING_DIR)/usr/lib -L$(STAGING_DIR)/lib -Wl,-rpath-link,$(STAGING_DIR)/usr/lib
TARGET_LDFLAGS_FOR_LD:=-L$(STAGING_DIR)/usr/lib -L$(STAGING_DIR)/lib -rpath-link $(STAGING_DIR)/usr/lib
...
string=$(subst /,$(space),$(subst ",,$(CONFIG_TOOLCHAIN_ROOT)))

ifeq ($(firstword $(string)),..)
	TOOLCHAIN_ROOT_DIR:=$(TOPDIR)/$(call qstrip,$(CONFIG_TOOLCHAIN_ROOT))
else
	ifeq ($(firstword $(string)),.)
	TOOLCHAIN_ROOT_DIR:=$(TOPDIR)/$(call qstrip,$(CONFIG_TOOLCHAIN_ROOT))
	else
...
string=$(subst /,$(space),$(subst ",,$(CONFIG_TOOLCHAIN64_ROOT)))
ifeq ($(firstword $(string)),..)
	TOOLCHAIN64_ROOT_DIR:=$(TOPDIR)/$(call qstrip,$(CONFIG_TOOLCHAIN64_ROOT))
else
	ifeq ($(firstword $(string)),.)
	TOOLCHAIN64_ROOT_DIR:=$(TOPDIR)/$(call qstrip,$(CONFIG_TOOLCHAIN64_ROOT))
	else
	TOOLCHAIN64_ROOT_DIR:=$(call qstrip,$(CONFIG_TOOLCHAIN64_ROOT))
	endif
endif

rootfs_string=$(subst /,$(space),$(subst ",,$(CONFIG_ROOTFS_PATH)))
ifeq ($(firstword $(rootfs_string)),..)
	ROOTFS_DIR:=$(TOPDIR)/$(call qstrip,$(CONFIG_ROOTFS_PATH))
else
	ifeq ($(firstword $(rootfs_string)),.)
	ROOTFS_DIR:=$(TOPDIR)/$(call qstrip,$(CONFIG_ROOTFS_PATH))
	else
	ROOTFS_DIR:=$(call qstrip,$(CONFIG_ROOTFS_PATH))
	endif
endif
  ...
  ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
    ...
    ifeq ($(CONFIG_NATIVE_TOOLCHAIN),)
      ...
      ifneq ($(TOOLCHAIN_BIN_DIRS),)
        TARGET_PATH:=$(TARGET_PATH):$(subst $(space),:,$(TOOLCHAIN_BIN_DIRS))
      endif
      ...
      TARGET_PATH:=$(TOOLCHAIN_DIR)/bin:$(TARGET_PATH)
      TARGET_CROSS64:=$(call qstrip,$(CONFIG_TOOLCHAIN64_PREFIX))
	  TOOLCHAIN64_BIN_DIRS=$(patsubst ./%,$(TOOLCHAIN64_ROOT_DIR)/%,$(call qstrip,$(CONFIG_TOOLCHAIN_BIN_PATH)))
	  ifneq ($(TOOLCHAIN64_BIN_DIRS),)
        TARGET_PATH:=$(TARGET_PATH):$(subst $(space),:,$(TOOLCHAIN64_BIN_DIRS))
      endif
    endif
  endif
endif
...
ifdef CONFIG_NEED_TOOLCHAIN_LIB
  TARGET_LIBRARY_PATH:=$(subst $(space),:,$(TOOLCHAIN_LIB_DIRS)):$LD_LIBRARY_PATH
  export LD_LIBRARY_PATH=$(TARGET_LIBRARY_PATH)
endif
...
TARGET_CC64:=$(TARGET_CROSS64)gcc
