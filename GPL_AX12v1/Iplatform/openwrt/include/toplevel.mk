...
BOARD := $(shell if [ -f .config ]; \
		then sed -n '/CONFIG_TARGET_BOARD/p' .config | cut -d \" -f 2; fi)

patch_kern_mconf:
	@if [ $(findstring bcm490x,$(BOARD)) != "bcm490x" ] && [ "$(KERN_SRC_PATH)" != "" ] && [ -d $(KERN_SRC_PATH) ] && [ "$(KERN_DEP_TAB)" != "$(PATCH_DEP_TAB)"  ]; then \
