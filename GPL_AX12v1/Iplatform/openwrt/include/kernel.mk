...
ifeq ($(DUMP),1)
  ...
  ifneq (,$(findstring uml,$(BOARD)))
    ...
    ifneq (,$(findstring bcm490x,$(BOARD)))
     KERNEL_CC?=$(TARGET_CC64)
     KERNEL_CROSS?=$(TARGET_CROSS64)
	else ifneq (,$(findstring ipq50xx,$(BOARD)))
     KERNEL_CC?=$(TARGET_CC64)
     KERNEL_CROSS?=$(TARGET_CROSS64)
    else
    KERNEL_CC?=$(TARGET_CC)
...
#  KERNEL_BUILD_DIR ?=$(BUILD_DIR_BASE)/linux-$(LINUX_VERSION)
	KERNEL_BUILD_DIR :=$(BUILD_DIR_BASE)/linux-$(BOARD)
  
 
words=$(subst /,$(space),$(subst ",,$(CONFIG_LINUX_KERNEL_PATH)))

ifeq ($(firstword $(words)),..)
	LINUX_DIR:=$(TOPDIR)/$(strip $(subst ",,$(CONFIG_LINUX_KERNEL_PATH)))
  else
	ifeq ($(firstword $(words)),.)
	LINUX_DIR:=$(TOPDIR)/$(strip $(subst ",,$(CONFIG_LINUX_KERNEL_PATH)))
	else
	LINUX_DIR:=$(strip $(subst ",,$(CONFIG_LINUX_KERNEL_PATH)))
	endif
  endif
...
#  MODULES_SUBDIR:=lib/modules/$(LINUX_UNAME_VERSION)
  MODULES_SUBDIR:=lib/modules/iplatform
...
LINUX_KARCH=$(strip $(subst ",,$(CONFIG_LINUX_KARCH)))
	...
	add_module() { \
		...
		for mod in $$$$$$$$mods; do \
			if [ -e $(2)/$(MODULES_SUBDIR)/$$$$$$$$mod.ko ]; then \
				...
				echo "$$$$$$$$mod$(if $(MODPARAMS), $(MODPARAMS))" >> $(2)/etc/modules.d/$$$$$$$$priority-$(1); \
			fi; \
		done; \
    ...
    ifneq ($(strip $(FILES)),)
      define Package/kmod-$(1)/install
		  @for mod in $$(FILES); do \
			...
			elif [ -e "$(LINUX_DIR)/modules/`basename $$$$$$$$mod`" ]; then \
                                echo "using common rules, tmp added by tengfei" ; \
                                mkdir -p $$(1)/$(MODULES_SUBDIR) ; \
                                $(CP) -L "$(LINUX_DIR)/modules/`(basename $$$$$$$$mod)`" $$(1)/$(MODULES_SUBDIR)/ ; \
