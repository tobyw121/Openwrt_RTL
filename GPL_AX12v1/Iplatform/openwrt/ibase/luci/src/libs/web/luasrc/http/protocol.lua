...
HTTP_MAX_CONTENT      = 1024*32		-- 16 kB maximum content size
...
local function decrypt(data, msg)
	...
	-- renew_aeskey is not true only when not login, or record it 
	if true == msg.encrypt.renew_aeskey then
		msg.encrypt.hash = sigtbl.hash
	else
		ret = service.check_hash(sigtbl.hash, msg.encrypt.hash)
		if not ret then
			return nil
		end
	end
	...
	if true == msg.encrypt.renew_aeskey then
		...
		if checknum ~= msg.encrypt.seqnum then
			return nil
		end
	end
...
function parse_message_body( src, msg, filecb )
	...
	then
		...
		msg.encrypt.need_encrypt = false --this type set no encrypt consistent
	...
	-- store whole buffer in message.content
		...
		-- ... else append to .content
			...
			if msg.encrypt.need_encrypt == true then
				local src_in = common_message_body_decrypt(src, msg)
				if not src_in then
					msg.encrypt.isdecfail = true --set decode fail flag when need encrypt
					return nil
				end
				msg.content = src_in
				msg.content_length = #src_in
