...
function httpdispatch(request, prefix)
	...
	-- check if http data need encrypt by url path and form
	local service = require "luci.service"
	local path = table.concat(context.request, ".")
	local form = http.formvalue("form", true)
	local need_encrypt = service.data_need_encrypt(path, form)
	local renew_aeskey = service.data_renew_aeskey(path, form)
	http.set_encryptflag(need_encrypt) --true means not parse http data
	http.set_renewaeskey(renew_aeskey) --true means need to renew aeskey from sinature in http payload
	if need_encrypt then
		local sauth = require "luci.sauth"
		local sess = luci.http.getcookie("sysauth")
		sess = sess and sess:match("^[a-f0-9]*$")
		local sdat = sauth.read(sess, true)
		if sdat then
			http.set_hash(sdat.hash)
			http.set_aeskey({key=sdat.aeskey, iv=sdat.aesiv})
			http.set_seqnum(sdat.seqnum)
		end
	end
...
-- get mac_addr from arp_table or neighbour_table by ip_addr
-- @param ip_addr		ipv4/ipv6 address
-- @return mac_addr		mac address
local function get_mac_by_ip(ip_addr)
    local mac_addr = ""
    if luci.model.checktypes.check_ipv6(ip_addr) then
        for _, v in ipairs(luci.sys.net.neightable()) do
            if ip_addr and ip_addr == v["IP address"] and v["HW address"] ~= nil then
                mac_addr = v["HW address"]
                break
            end
        end
    else
        for _, v in ipairs(luci.sys.net.arptable()) do
            if ip_addr and ip_addr == v["IP address"] then
                mac_addr = v["HW address"]
                break
            end
        end
    end
    return mac_addr:gsub(":", "-"):upper()
end
...
function dispatch(request)
            ...
            elseif ctx.urltoken.stok == sdat.token then
                ...
                if remote == sdat.remote then
                    if remote then
                        ...
                        local macaddr = get_mac_by_ip(ipaddr)
	...
	if type(target) == "function" then
		...
		if type(c.target) == "table" then
			...
			-- avoid empty session abnormal.
			if err and err == "Session data invalid!" then
				dbg.print("Session data invalid.")
				os.execute("rm -rf /tmp/luci-sessions/*")
			end
		else
