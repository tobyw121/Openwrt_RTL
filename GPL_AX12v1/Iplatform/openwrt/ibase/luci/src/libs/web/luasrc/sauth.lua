...
-- @param id            Session identifier
-- @param readonly      if true then only to read content and not to kill session
-- @return    Session data table or nil if the given id is not found
function read(id, readonly)
    ...
    if readonly ~= true and sess.atime and sess.atime + sessiontime < luci.sys.uptime() then
        kill(id)
        return nil
    end
