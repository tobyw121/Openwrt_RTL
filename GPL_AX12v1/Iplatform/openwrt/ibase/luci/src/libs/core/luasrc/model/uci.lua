...
--- Add an anonymous section at the first.
-- @param config	UCI config
...
--- Create a new section at the first and initialize it with data.
-- @param config	UCI config
-- @param type		UCI section type
-- @param name		UCI section name (optional)
-- @param values	Table of key - value pairs to initialize the section with
...
function Cursor.commit(self, ...)
    ...
    config = require "luci.sys.config"
    config.saveconfig()
    return success
end

--- Commit saved changes, for factory reset
-- @param config	UCI config
-- @return			Boolean whether operation succeeded
-- @see Cursor.revert
-- @see Cursor.save
function Cursor.factory_commit(self, ...)
    local success = true
    for _, config in ipairs{...} do
        success = self:rawcommit(config) and success
    end
    ...
    config = require "luci.sys.config"
    config.factory_saveconfig()
    return success
end

--- Commit saved changes without write flash.
-- @param config	UCI config
-- @return			Boolean whether operation succeeded
-- @see Cursor.revert
-- @see Cursor.save
function Cursor.commit_without_write_flash(self, ...)
    local success = true
    for _, config in ipairs{...} do
        success = self:rawcommit(config) and success
    end
