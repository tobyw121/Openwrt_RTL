...
--- Returns the current neighbour table entries
--- @return	Table of table containing the current neighbour entries.
--			The following fields are defined for neighbour entry objects:
--			{ "IP address", "Device", "HW address", "State" }
function net.neightable()
	local line, r, v, x, neigh_table
	local neighbour = luci.util.execi("ip neigh")
	if not neighbour then
		return
	end
	for line in neighbour do
		r = { }, v
		...
		neigh_table = neigh_table or { }
		neigh_table[#neigh_table+1] = x
	end
	return neigh_table
end
