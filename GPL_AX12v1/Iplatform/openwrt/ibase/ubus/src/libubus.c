...
ubus_queue_msg(struct ubus_context *ctx, struct ubus_msghdr_buf *buf)
{
	...
	if (!pending)
		return;
	pending->hdr.data = calloc(1, blob_raw_len(buf->data));
	if (!pending->hdr.data)
		return;

	memcpy(&pending->hdr.hdr, &buf->hdr, sizeof(buf->hdr));
	memcpy(pending->hdr.data, buf->data, blob_raw_len(buf->data));

	list_add(&pending->list, &ctx->pending);
}

void __hidden
ubus_process_msg(struct ubus_context *ctx, struct ubus_msghdr_buf *buf)
{

	switch(buf->hdr.type) {
	case UBUS_MSG_STATUS:
	case UBUS_MSG_DATA:
		ubus_process_req_msg(ctx, buf);
		break;
