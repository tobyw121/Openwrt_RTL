...
static int child_priority = -20;

#ifdef CONFIG_MEM_PROTECT
#include <sys/sysinfo.h>

#define LOCAL_UPGRADE_QUERY "save_upgrade" /* local upgrade, upload and upgrade */
#define CLOUD_DOWNLOAD_QUERY "cloud_upgrade" /* cloud upgrade, start download */
#define CLOUD_UPGRADE_QUERY "detect_upgrade_status" /* Check the download progress and upgrade when it reaches 100% */

enum uprgade_query_type
{
	NON_UPGRADE_TYPE,
	NORMAL_UPGRADE_TYPE,
	CLOUD_UPGRADE_TYPE
...
#ifdef CONFIG_MEM_PROTECT_SAFE_FREE_MEM
#define SAFE_FREE_MEM CONFIG_MEM_PROTECT_SAFE_FREE_MEM
#else
#define SAFE_FREE_MEM 15000
#endif

#define SAFE_FREE_MEM_NOT_STRICT 5000

static int
get_free_memory_info(int safe_mem)
{
    struct sysinfo info;
    ...
    memfree = info.freeram>>10;
    D("MemFree = %lu KB", memfree);
    if (memfree < safe_mem)
    {
        return false;
    }
...
int check_upgrade_query(struct path_info *pi)
{
	char *query;

	if(pi->query)
	{
		query = strchr(pi->query, '=');
		if(query)
		{
			query += 1;
			if(strcmp(query, CLOUD_UPGRADE_QUERY) == 0)
			{	
			
				return CLOUD_UPGRADE_TYPE;
			}
			else if( strcmp(query, LOCAL_UPGRADE_QUERY) == 0
				||strcmp(query, CLOUD_DOWNLOAD_QUERY) == 0  )
			{
				return NORMAL_UPGRADE_TYPE;
			}
		}
	}

	return NON_UPGRADE_TYPE;
}

#endif /*#ifdef CONFIG_MEM_PROTECT*/
...
#ifdef CONFIG_MEM_PROTECT
						cl->long_term ? LONG_TERM_PROCESS_TIMEOUT : cl->server->conf->script_timeout
#else
...
bool reclaim_cache(void)
{
	static time_t mem_timestamp = 0;
	time_t cur_time = 0;

	time(&cur_time);
	if ( cur_time > mem_timestamp + 5)
	{
		mem_timestamp = cur_time;

		FILE* pFile = fopen("/proc/sys/vm/drop_caches", "w");
		if (pFile) 
		{
			fprintf(pFile, "3");
			fclose(pFile);
					...
					struct interpreter *ip)
{
...
#ifdef CONFIG_MEM_PROTECT
	int upgrade_type = check_upgrade_query(pi);
	int safe_mem = (upgrade_type == CLOUD_UPGRADE_TYPE) ? SAFE_FREE_MEM_NOT_STRICT : SAFE_FREE_MEM;

	if (false == get_free_memory_info(safe_mem))
	{
		D("memfree less than %d KB", SAFE_FREE_MEM);
		if (!reclaim_cache())
		{
			uh_http_sendhf(cl, 500, "Internal Server Error", "Out of memory");
			return false;
		}
		else if (false == get_free_memory_info(safe_mem))
		{
			uh_http_sendhf(cl, 500, "Internal Server Error", "Out of memory, reclaim cache failed");
			return false;
		}
	}
#endif
	...
	case 0:
		...
		setpriority(PRIO_PROCESS, getpid(), child_priority);
