...
static const char * uh_file_mktag(struct stat *s)
{
	...
	snprintf(tag, sizeof(tag), "\"%x-%" PRIx64 "-%x\"",
...
bool uh_file_request(struct client *cl, struct path_info *pi, bool no_cache)
{
	int rlen;
	int ok = 1;
	int fd = -1;
	...
	pi->stat.st_mtime = m_modified_time;
	...
	if ((pi->stat.st_mode & S_IFREG) && ((fd = open(pi->phys, O_RDONLY)) > 0))
	{
		...
		if (ok > 0)
		{
			...
			/*fix clickjacking, used in older browsers*/
			ensure_out(uh_http_sendf(cl, NULL, "X-Frame-Options: deny\r\n"));
			/*fix clickjacking, used in newer browsers*/
			ensure_out(uh_http_sendf(cl, NULL, "Content-Security-Policy: frame-ancestors 'none'\r\n"));

#ifdef HAVE_TLS
			/* do not cache login.html/index.html */
			if(no_cache)
			{
			...
			ensure_out(uh_http_sendf(cl, NULL, "Content-Length: %" PRIu64 "\r\n",
									 (unsigned long long)pi->stat.st_size));
...
*/
            ...
            /* sleep 200ms before closing tcp connection when transfer file "timezone.json" */
            if(0 == strcmp(pi->phys, "/www/webpages/data/timezone.json")
            	 || 0 == strcmp(pi->phys, "/www/webmobile/data/timezone.json"))
            {
                usleep(200000);
            }
		}
