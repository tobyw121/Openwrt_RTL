					     ...
					     char *buf, size_t buflen)
{
	...
	ret = os_snprintf(pos, end - pos, "PIN Status: %s\n",
			pin_status_str(hapd->wps_stats.pin_status));
	...
	{
		ret = os_snprintf(pos, end - pos, "LockDown: Lock\n");
		if (os_snprintf_error(end - pos, ret))
			return pos - buf;
	}
	else
	{
		ret = os_snprintf(pos, end - pos, "LockDown: Unlock\n");
		if (os_snprintf_error(end - pos, ret))
			return pos - buf;
	}
	pos += ret;
...
static int hostapd_disassoc_accept_mac(struct hostapd_data *hapd)
{
	struct sta_info *sta;
...
static int hostapd_disassoc_deny_mac(struct hostapd_data *hapd)
{
	struct sta_info *sta;
	struct vlan_description vlan_id;
...
static int hostapd_ctrl_iface_get_vendor_elements(struct hostapd_data *hapd,
					     char *buf, size_t buflen)
{
	int ret = 0;
	char *pos, *end;
	...
	if ((hapd->conf != NULL) && (hapd->conf->vendor_elements != NULL))
	{
		for (i = 0; i < hapd->conf->vendor_elements->used; i++)
		{
			ret = os_snprintf(pos, end - pos, "%02x ", hapd->conf->vendor_elements->buf[i]);
			if (os_snprintf_error(end - pos, ret))
				return pos - buf;
			pos += ret;
		}
				  ...
				  char *buf, size_t buflen)
{
	...
	{
		res = hostapd_ctrl_iface_get_vendor_elements(hapd, buf, buflen);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	}
					      ...
					      socklen_t fromlen)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (hostapd_ctrl_iface_disassociate(hapd, buf + 13))
			reply_len = -1;
	} else if (os_strcmp(buf, "DISCONNECTSTAOFBSS") == 0) {
		if (hostapd_ctrl_iface_disconnect_all_sta(hapd))
			reply_len = -1;
	...
	}else if (os_strncmp(buf, "APPLY_ACL", 9) == 0) {
