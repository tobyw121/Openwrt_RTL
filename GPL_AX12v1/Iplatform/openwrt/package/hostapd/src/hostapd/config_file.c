...
static int hostapd_parse_scanlist(struct hostapd_config *conf, char *val)
{
	char *pos;
	...
	if (freq_range_list_parse(&conf->acs_scan_list, val))
		return -1;
...
static int hostapd_config_parse_acs_chan_ht40_bias(struct hostapd_config *conf,
					      char *pos)
{
	struct acs_ht40_bias *bias = NULL, *tmp;
	unsigned int num = 0;
	char *end;
	...
	os_free(conf->acs_chan_ht40_bias);
	conf->acs_chan_ht40_bias = bias;
	conf->num_acs_chan_ht40_bias = num;
			       ...
			       const char *buf, char *pos, int line)
{
	...
	} else if (os_strcmp(buf, "scanlist") == 0) {
		if (hostapd_parse_scanlist(conf, pos)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid channel list",
				   line);
			return 1;
		}
	...
	} else if (os_strcmp(buf, "acs_chan_ht40_bias") == 0) {
		if (hostapd_config_parse_acs_chan_ht40_bias(conf, pos)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid acs_chan_ht40_bias",
				   line);
			return -1;
		}
#endif /* CONFIG_ACS */
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
		...
		bss->rnr = atoi(pos);
	} else if (os_strcmp(buf, "force_2g_40m") == 0) {
		conf->force_2g_40m = atoi(pos);//Realtek add
