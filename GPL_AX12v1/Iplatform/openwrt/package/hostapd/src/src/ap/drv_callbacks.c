...
#define RTK_CHECK_SA_QUERY_BEFORE_VALIDATE_IE
			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
...
#ifdef RTK_CHECK_SA_QUERY_BEFORE_VALIDATE_IE
...
#endif /* RTK_CHECK_SA_QUERY_BEFORE_VALIDATE_IE */
	} else {
	...
	if (hapd->conf->wpa) {
...
#ifndef RTK_CHECK_SA_QUERY_BEFORE_VALIDATE_IE
			if (check_sa_query_need(hapd, sta)) {
				status = WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY;
		...
		if (sta->auth_alg == WLAN_AUTH_FT) {
			...
			wpa_printf(MSG_DEBUG, "FT: set reassoc = 1");
			reassoc = 1;
		}
...
/*compatible with some phones, which have mbo ie but keep pmf disabled*/
#if 0//def CONFIG_MBO
	...
	if ((hapd->conf->wpa_key_mgmt & WPA_KEY_MGMT_OWE) &&
		...
		if ((hapd->conf->wpa_key_mgmt & WPA_KEY_MGMT_DPP) &&
			...
			os_memcpy(p, wpabuf_head(sta->dpp_pfs->ie),wpabuf_len(sta->dpp_pfs->ie));
	...
	if (elems.ext_capab && elems.ext_capab_len > 2) {
		/* bit 19 - BSS Transition Supported */
    ...
    snprintf(mac_str, sizeof(mac_str), MACSTR, MAC2STR(sta->addr));
    snprintf(cmd, sizeof(cmd), "ubus call map sta_event '{\"action\":\"assoc\", \"mac\":\"%s\", \"ifname\":\"%s\"}'", mac_str, hapd->conf->iface);
    system(cmd);
...
void hostapd_notif_disassoc(struct hostapd_data *hapd, const u8 *addr)
{
	...
	snprintf(mac_str, sizeof(mac_str), MACSTR, MAC2STR(sta->addr));
	snprintf(cmd, sizeof(cmd), "ubus call map sta_event '{\"action\":\"disassoc\", \"mac\":\"%s\", \"ifname\":\"%s\"}'", mac_str, hapd->conf->iface);
	system(cmd);
		       ...
		       union wpa_event_data *data)
{
	...
	case EVENT_INTERFACE_ENABLED:
		/* For non-main interface, data is provided for looking up
		   the co-responding hapd interface */
		if (data != NULL) {
			int i;
			void *bss = (void *)data;
			for (i = 0; i < hapd->iface->num_bss; i++) {
				if (hapd->iface->bss[i]->drv_priv == bss) {
					hapd = hapd->iface->bss[i];
					wpa_printf(MSG_DEBUG,
					           "%s: EVENT_INTERFACE_ENABLED!\n",
					           hapd->conf->iface);
					break;
				}
			}
