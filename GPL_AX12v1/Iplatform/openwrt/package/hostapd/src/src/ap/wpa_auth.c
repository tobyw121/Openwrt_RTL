		  ...
		  const u8 *p2p_dev_addr)
{
	...
	if (wpa_auth == NULL ){
		wpa_printf(MSG_ERROR, "%s: wpa_auth is NULL and return", __func__);
		return NULL;
	}

	if (wpa_auth->group == NULL){
		wpa_printf(MSG_ERROR, "%s: wpa_auth->group is NULL and return", __func__);
		return NULL;
	}

	if (wpa_auth->group->wpa_group_state == WPA_GROUP_FATAL_FAILURE)
	{
		wpa_auth->group->wpa_group_state = WPA_GROUP_SETKEYS;//Realtek add
		wpa_printf(MSG_ERROR,
		   "WPA: reset group key when group state machine entering state FATAL_FAILURE");
		return NULL;
	}
