	...
	if (status_code == WNM_BSS_TM_ACCEPT) {
		if (end - pos < ETH_ALEN) {
			wpa_msg(hapd->msg_ctx, MSG_INFO, BSS_TM_RESP MACSTR
				" status_code=%u bss_termination_delay=%u token=%u",
				MAC2STR(addr), status_code, bss_termination_delay, dialog_token);
		...
		wpa_printf(MSG_DEBUG, "WNM: Target BSSID: " MACSTR,
			   MAC2STR(pos));
		wpa_msg(hapd->msg_ctx, MSG_INFO, BSS_TM_RESP MACSTR
			" status_code=%u bss_termination_delay=%u token=%u target_bssid="
			MACSTR,
			MAC2STR(addr), status_code, bss_termination_delay, dialog_token,
			MAC2STR(pos));
		pos += ETH_ALEN;
