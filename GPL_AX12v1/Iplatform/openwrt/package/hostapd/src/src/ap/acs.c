...
static int is_in_scanlist(struct hostapd_iface *iface,
			  struct hostapd_channel_data *chan)
{
	if (!iface->conf->acs_scan_list.num)
		return 1;

	return freq_range_list_includes(&iface->conf->acs_scan_list, chan->chan);
}

static int acs_chan_check_ht40_bias(struct hostapd_iface *iface,
			struct hostapd_channel_data *chan, int *ht40_bias)
{
	int i;
	struct acs_ht40_bias *bias;

	if (!iface->conf->acs_chan_ht40_bias)
		return 0;

	if (!ht40_bias)
		return 0;

	for (i = 0; i < iface->conf->num_acs_chan_ht40_bias; i++) {
		bias = &iface->conf->acs_chan_ht40_bias[i];
			 ...
			 long double *ideal_factor)
{
	...
	for (i = 0; i < mode->num_channels; i++) {
		...
		if (is_24ghz_mode(mode->mode) && 40 == bw)
			acs_chan_check_ht40_bias(iface, chan, &ht40_bias);

		if (!chan_bw_allowed(chan, bw, ht40_bias == 1 ? 1 : 0, 1)) {
		...
		for (j = 1; j < n_chans; j++) {
			adj_chan = acs_find_chan(iface, chan->freq + (j * 20) * ht40_bias);
			if (!adj_chan)
				break;
...
static void acs_adjust_secondary_channel(struct hostapd_iface *iface,
			struct hostapd_channel_data *ideal_chan)
{
	int ht40_bias = 1;
	int ret;

	wpa_printf(MSG_DEBUG, "ACS: ideal_chan %d secondary_channel: %d",
		ideal_chan->chan, iface->conf->secondary_channel);

	ret = acs_chan_check_ht40_bias(iface, ideal_chan, &ht40_bias);
	if (ret && iface->conf->secondary_channel != ht40_bias) {
		wpa_printf(MSG_DEBUG, "ACS: change secondary_channel to bias: %d->%d",
			iface->conf->secondary_channel, ht40_bias);
		iface->conf->secondary_channel = ht40_bias;
	}
}
...
static void acs_study(struct hostapd_iface *iface)
{
	...
	if (iface->conf->ieee80211n && iface->conf->secondary_channel)
		acs_adjust_secondary_channel(iface, ideal_chan);
