					 ...
					 char *cmd, char *buf, size_t buflen)
{
	...
	} else if (os_strcasecmp(cmd, "node_id") == 0) {
		res = os_snprintf(buf, buflen, "node_id = %04X, is_conn_specific_nodeid = %d", wpa_s->node_id, wpa_s->is_conn_specific_nodeid);
...
static int  wpa_supplicant_ctrl_iface_wps_get_status(struct wpa_supplicant *wpa_s,
					     char *buf, size_t buflen)
{
	int ret;
	char *pos, *end;
	...
	ret = os_snprintf(pos, end - pos, "PBC Status: %s\n",
			  pbc_status_str(wpa_s->wps_stats.pbc_status));

	if (os_snprintf_error(end - pos, ret))
		return pos - buf;
	pos += ret;

	ret = os_snprintf(pos, end - pos, "PIN Status: %s\n",
			pin_status_str(wpa_s->wps_stats.pin_status));
	...
	ret = os_snprintf(pos, end - pos, "Last WPS result: %s\n",
			  (wpa_s->wps_stats.status == WPS_STATUS_SUCCESS ?
			   "Success":
			   (wpa_s->wps_stats.status == WPS_STATUS_FAILURE ?
			    "Failed" : "None")));
	...
	if(wpa_s->wps_stats.status == WPS_STATUS_FAILURE &&
	   wpa_s->wps_stats.failure_reason > 0) {
		ret = os_snprintf(pos, end - pos,
				  "Failure Reason: %s\n",
				  wps_ei_str(wpa_s->wps_stats.failure_reason));
	...
	if (wpa_s->wps_stats.status) {
		ret = os_snprintf(pos, end - pos, "Peer Address: " MACSTR "\n",
				  MAC2STR(wpa_s->wps_stats.peer_addr));
	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
	const u8 *ie, *ie2, *osen_ie, *p2p, *mesh, *owe, *rsnxe, *onemesh = NULL;
	int i = 0;
	int onemesh_data_len = 0;
	...
	/* add for onemesh tpie */
	do {
		if ((onemesh = wpa_bss_get_vendor_ie(bss, ONEMESH_IE_VENDOR_TYPE))) {
			break;
		}
		if ((onemesh = wpa_bss_get_vendor_ie(bss, ONEMESH_IE_VENDOR_TYPE_V2))) {
			break;
		}
	} while (0);
	if (onemesh) {
		onemesh_data_len = onemesh[1] + 2;
		if (os_snprintf_error(end - pos, strlen("[TPIE:]") + onemesh_data_len * 2))
			return -1;
		ret = os_snprintf(pos, end - pos, "[TPIE:");
		pos += ret;
		for (i = 0; i < onemesh_data_len; i++) {
			ret = os_snprintf(pos, end - pos, "%02x", onemesh[i]);
			pos += ret;
		}
		*pos++ = ']';
		*pos = '\0';
	}
	/* end for onemesh tpie */
...
#else   // Get the lastest scan results
	if (wpa_s->last_scan_res) {
		for (i = 0; i < wpa_s->last_scan_res_used; i++) {
			if (!wpa_s->last_scan_res[i]) {
				continue;
			}
			bss = wpa_s->last_scan_res[i];
			ret = wpa_supplicant_ctrl_iface_scan_result(wpa_s, bss, pos,
									end - pos);
			if (ret < 0 || ret >= end - pos)
				return pos - buf;
...
static int wpa_supplicant_ctrl_iface_get_tp_mesh_enable(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
...
static int wpa_supplicant_ctrl_iface_tp_mesh_enable(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	int tp_mesh_enable = atoi(cmd);
	wpa_s->tp_mesh_enable = tp_mesh_enable;

	wpa_printf(MSG_DEBUG, "wpa_s->tp_mesh_enable = %d", wpa_s->tp_mesh_enable);
	return 0;
}

static int wpa_supplicant_ctrl_iface_get_node_id(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
	...
	ret = os_snprintf(pos, end - pos, "0x%04X\n", wpa_s->node_id);
	if (os_snprintf_error(end - pos, ret))
		return pos - buf;
	pos += ret;
...
static int wpa_supplicant_ctrl_iface_node_id(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	int node_id;

	if (os_strstr(cmd, "0x") || os_strstr(cmd, "0X")) {
		node_id = strtol(cmd, NULL, 16);
	}
	else {
		node_id = strtol(cmd, NULL, 0);
	}
	wpa_s->node_id = (u16)node_id;
	if(wpa_s->node_id == 0)
		wpa_s->is_conn_specific_nodeid = 0;
	else
		wpa_s->is_conn_specific_nodeid = 1;

	wpa_printf(MSG_DEBUG, "wpa_s->node_id = %04X, wpa_s->is_conn_specific_nodeid = %d",
	           wpa_s->node_id, wpa_s->is_conn_specific_nodeid);
	return 0;
}

static int wpa_supplicant_ctrl_iface_get_rootap_tpie(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	char *pos, *end;
	int i, ret;
	...
	if (wpa_s->current_ssid)
	{
		for (i = 0; i < MAX_ONEMESH_TPIE_LEN; i++)
		{
			ret = os_snprintf(pos, end - pos, "%02X", wpa_s->current_ssid->onemesh_tpie[i]);
			if (os_snprintf_error(end - pos, ret))
				return pos - buf;
			pos += ret;
		}
...
static int wpa_supplicant_ctrl_iface_get_easymesh_support(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
	...
	if (wpa_s->current_ssid)
	{
		ret = os_snprintf(pos, end - pos, "%d\n", wpa_s->current_ssid->easymesh_support);
		if (os_snprintf_error(end - pos, ret))
			return pos - buf;
		pos += ret;
	}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		reply_len = wpa_supplicant_ctrl_iface_wps_get_status(
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		reply_len = wpa_supplicant_ctrl_iface_log_level(
			wpa_s, buf + 9, reply, reply_size);
	} else if (os_strncmp(buf, "LOG_CONSOLE ", 12) == 0) {
		if (console_log_enable(buf[12] == '0' ? 0 : 1))
			reply_len = -1;
		...
		if (wpa_supplicant_ctrl_iface_ap_scan(wpa_s, buf + 8))
			reply_len = -1;
	}else if (os_strcmp(buf, "TP_MESH_ENABLE") == 0) {
		reply_len = wpa_supplicant_ctrl_iface_get_tp_mesh_enable(
			wpa_s, reply, reply_size);
	} else if (os_strncmp(buf, "TP_MESH_ENABLE ", 15) == 0) {
		if (wpa_supplicant_ctrl_iface_tp_mesh_enable(wpa_s, buf + 18))
			reply_len = -1;
	} else if (os_strcmp(buf, "NODE_ID") == 0) {
		reply_len = wpa_supplicant_ctrl_iface_get_node_id(
			wpa_s, reply, reply_size);
	} else if (os_strncmp(buf, "NODE_ID ", 8) == 0) {
		if (wpa_supplicant_ctrl_iface_node_id(wpa_s, buf + 8))
			reply_len = -1;
	} else if (os_strcmp(buf, "ROOTAP_TPIE_GET") == 0) {
		reply_len = wpa_supplicant_ctrl_iface_get_rootap_tpie(
			wpa_s, reply, reply_size);
	} else if (os_strcmp(buf, "EASYMESH_SUPPORT") == 0) {
		reply_len = wpa_supplicant_ctrl_iface_get_easymesh_support(
			wpa_s, reply, reply_size);
