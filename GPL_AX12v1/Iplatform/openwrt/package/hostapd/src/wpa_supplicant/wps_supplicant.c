				   ...
				   const struct wps_credential *cred)
{
	...
	if (ssid && (ssid->key_mgmt & WPA_KEY_MGMT_WPS)) {
		...
		if (ssid == NULL)
			return -1;
#ifdef CONFIG_TP_EASYMESH
		/*
		 * Each credential should be marked as supporting Multi-AP.
		 * TODO: multi_ap_backhaul_sta should be 0 if users disable MESH.
		 */
		ssid->multi_ap_backhaul_sta = 1;
#endif /* CONFIG_TP_EASYMESH */
					  ...
					  struct wps_event_fail *fail)
{
    ...
    os_memcpy(wpa_s->wps_stats.peer_addr, fail->peer_macaddr, ETH_ALEN);
    wpa_s->wps_stats.failure_reason = fail->error_indication;
				      ...
				      struct wps_event_success *success)
{
	...
	wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_DISABLE;
	wpa_s->wps_stats.status = WPS_STATUS_SUCCESS;
	os_memcpy(wpa_s->wps_stats.peer_addr, success->peer_macaddr, ETH_ALEN);

	wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_SUCCESS);
	wpa_s->wps_success = 1;

	/* 双频桥接时，避免出现wps两个频段桥上不同设备，当其中一个频段wps成功之后，取消其他频段wps流程 */
	for (tmp_wpa_s = wpa_s->global->ifaces; tmp_wpa_s ; tmp_wpa_s = tmp_wpa_s->next)
	{
		if (os_strcmp(tmp_wpa_s->ifname, wpa_s->ifname))
		{
			wpas_wps_cancel(tmp_wpa_s);
			wpa_printf(MSG_ERROR, "ifname: %s WPS_EVENT_SUCCESS, and cannel WPS ifname: %s ",wpa_s->ifname, tmp_wpa_s->ifname);
		}
	}
...
static void wpa_supplicant_wps_event_pbc_overlap(struct wpa_supplicant *wpa_s)
{
	/* Update WPS Status - PBC Overlap */
	wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_OVERLAP;
}

static void wpa_supplicant_wps_event_pbc_timeout(struct wpa_supplicant *wpa_s)
{
	/* Update WPS PBC Status:PBC Timeout */
	wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_TIMEOUT;
}

static void wpa_supplicant_wps_event_pbc_active(struct wpa_supplicant *wpa_s)
{
	/* Update WPS PBC status - Active */
	wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_ACTIVE;
}

static void wpa_supplicant_wps_event_pbc_disable(struct wpa_supplicant *wpa_s)
{
	/* Update WPS PBC status - Active */
	wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_DISABLE;
}

static void wpa_supplicant_wps_event_clean_status(struct wpa_supplicant *wpa_s)
{
	/* clean WPS PBC status */
	wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_DISABLE;
	wpa_s->wps_stats.status = 0;
	os_memset(wpa_s->wps_stats.peer_addr, 0, ETH_ALEN);
}
				     ...
				     union wps_event_data *data)
{
	...
	case WPS_EV_FAIL:
		...
		break;
	case WPS_EV_SUCCESS:
		wpa_supplicant_wps_event_success(wpa_s, &data->success);
		break;
	...
	case WPS_EV_PBC_OVERLAP:
        wpa_supplicant_wps_event_pbc_overlap(wpa_s);
        wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_OVERLAP);
		break;
	case WPS_EV_PBC_TIMEOUT:
        wpa_supplicant_wps_event_pbc_timeout(wpa_s);
        wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_TIMEOUT);
		break;
	case WPS_EV_PBC_ACTIVE:
        wpa_supplicant_wps_event_pbc_active(wpa_s);
		wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_ACTIVE);
		break;
	case WPS_EV_PBC_DISABLE:
        wpa_supplicant_wps_event_pbc_disable(wpa_s);
