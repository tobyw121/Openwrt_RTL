...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	/* change tp_mesh_enable */
	if (conf->tp_mesh_enable != 0) {
		wpa_s->tp_mesh_enable = 1;
	}
	else {
		wpa_s->tp_mesh_enable = 0;
	}
				     ...
				     const struct wpa_interface *iface)
{
	...
	if (iface->confname) {
		...
		if (wpa_s->conf->tp_mesh_enable != 0) {
			wpa_s->tp_mesh_enable = 1;
		}
		else {
			wpa_s->tp_mesh_enable = 0;
		}
		wpa_printf(MSG_ERROR, "wpa_supplicant_init_iface wpa_s->tp_mesh_enable = %d", wpa_s->tp_mesh_enable);
