...
struct wpa_bss_ie_tpinfo {
	/** IEEE80211_ELEMID_VENDOR 221(0xdd) */
	u8 id;
	/** Length in bytes */
	u8 len;
	/** Fix value: 0x00, 0x1d, 0x0f */
	u8 oui[3];
	/** OUI type, Deco:0x01, RE:0x10 */
	u8 type;
	/** subnet type:AP:0x01, RE:0x02 */
	u8 net_type;
	/** one mesh support, support : 1, not support:0, C7 and RE300v1 are set 1 */
	/** one mesh enable, enable: 1, unenable: 0, depends on user */
	/** C7? RE300v1 is not support 4addr (0) */
	/** C7? RE300v1 is not support 4addr (0) */
	u8 bitmap;
	/* level in the subnet, Router: 0, +1 */
	u8 level;
	/* uplink mask: 2.4G:0x01 5G:0x10 2.4G+5G:0x11 */
	u8 uplink_mask;
	/* mac of the subnet: Router AP's Lan MAC */
	u8 mac[6];
	/* label of the subnet: group id */
	u8 label[8];
	/* uplink rate */
	u16 uplink_rate;
	/* node_id */
	u16 node_id;
	/* product type: TPIE中产品标识，值域（待确认）*/
	u16 product_type;
	/* reserved: 保留字段, 未使用，默认值为0 */
	u16 reserved;
};
