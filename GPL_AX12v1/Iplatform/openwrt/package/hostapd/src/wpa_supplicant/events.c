					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	wpa_printf(MSG_ERROR, "wpa_s->tp_mesh_enable = %d",wpa_s->tp_mesh_enable);
	if(wpa_s->tp_mesh_enable != 0)
	{
		tpie = (u8 *)wpa_bss_get_vendor_ie(bss, ONEMESH_IE_VENDOR_TYPE);
		if(!tpie)
			tpie = (u8 *)wpa_bss_get_vendor_ie(bss, ONEMESH_IE_VENDOR_TYPE_V2);
		if(tpie)
		{
			rootapodeid = GET_NODE_ID_FROM_TPIE(tpie);
			wpa_printf(MSG_DEBUG, "wpa_supplicant_ssid_bss_match scan AP node id: 0x%04x, root ap node id: 0x%04x",
					   rootapodeid, wpa_s->node_id);
			if(wpa_s->is_conn_specific_nodeid && rootapodeid != wpa_s->node_id)
			{
				wpa_printf(MSG_DEBUG, "wpa_supplicant_ssid_bss_match Node ID not match!!!");
				return 0;
			}
			tpie_level = GET_LEVEL_FROM_TPIE(tpie);
			wpa_printf(MSG_DEBUG, "wpa_supplicant_ssid_bss_match scan AP level %d", tpie_level);
			if (tpie_level >= 2)
			{
				wpa_printf(MSG_DEBUG, "wpa_supplicant_ssid_bss_match Can't connect to AP which level(%d) greater than 2!!!",
						   tpie_level);
				return 0;
			}
		}
			  ...
			  int only_first_ssid)
{
	...
	struct wpa_bss *bss, *select_bss = NULL;
	struct wpa_bss_ie_tpinfo *tpie;
	struct wpa_ssid *ssid;
	int onemesh_bss = 0;
	int rssi_max = -255;
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		if (wpa_s->tp_mesh_enable != 0) {
			tpie = (struct wpa_bss_ie_tpinfo *)
					wpa_bss_get_vendor_ie(bss, ONEMESH_IE_VENDOR_TYPE);
			if (!tpie) {
				tpie = (struct wpa_bss_ie_tpinfo *)
						wpa_bss_get_vendor_ie(bss, ONEMESH_IE_VENDOR_TYPE_V2);
			}
			if (tpie) {
				wpa_dbg(wpa_s, MSG_DEBUG, "bss rssi=%d, tpie_level=%d", bss->level, tpie->level);
				onemesh_bss = tpie->bitmap & 0x03;
				wpa_dbg(wpa_s, MSG_DEBUG, "bss %ssupport onemesh", onemesh_bss ? "" : "un");

				if (bss->level > -75 && tpie->level == 0) {
					*selected_ssid = ssid;
					wpa_dbg(wpa_s, MSG_DEBUG, "   selected %sBSS " MACSTR
						" ssid='%s', onemesh %ssupported",
						bss == wpa_s->current_bss ? "current ": "",
						MAC2STR(bss->bssid),
						wpa_ssid_txt(bss->ssid, bss->ssid_len), onemesh_bss ? "" : "un");
					/* Update tpie */
					ssid->onemesh_support = onemesh_bss;
					os_memcpy(ssid->onemesh_tpie, tpie, sizeof(ssid->onemesh_tpie));
					wpa_dbg(wpa_s, MSG_DEBUG, "update tpie");
					return bss;
				}
			}

			if (rssi_max < bss->level) {
				rssi_max = bss->level;
				*selected_ssid = ssid;
				select_bss = bss;
				wpa_dbg(wpa_s, MSG_DEBUG, "find BSS "MACSTR" ssid=%s",
					MAC2STR(bss->bssid), wpa_ssid_txt(bss->ssid, bss->ssid_len));

				/* Update tpie */
				if (tpie) {
					ssid->onemesh_support = onemesh_bss;
					os_memcpy(ssid->onemesh_tpie, tpie, sizeof(ssid->onemesh_tpie));
					wpa_dbg(wpa_s, MSG_DEBUG, "find tpie and update");
				}
				else {
					ssid->onemesh_support = 0;
					os_memset(ssid->onemesh_tpie, 0, sizeof(ssid->onemesh_tpie));
					wpa_dbg(wpa_s, MSG_DEBUG, "not find tpie");
				}
			}
		}
		else {
			*selected_ssid = ssid;
			wpa_dbg(wpa_s, MSG_DEBUG, "selected %sBSS "MACSTR" ssid='%s'",
					bss == wpa_s->current_bss ? "current ": "",
					MAC2STR(bss->bssid),
					wpa_ssid_txt(bss->ssid, bss->ssid_len));
			return bss;
					...
					const u8 *ies, size_t ies_len)
{
	...
	{
		wpa_printf(MSG_INFO,
			   "multi_ap_process_assoc_resp : no multi_ap ie");
		if (wpa_s->current_ssid)
		{
			wpa_s->current_ssid->easymesh_support = 0;
		}
		return;
	}
	...
	{
			wpa_printf(MSG_INFO,
				   "multi_ap_process_assoc_resp : get map_sub_elem ie fail");
			if (wpa_s->current_ssid)
			{
				wpa_s->current_ssid->easymesh_support = 0;
			}
	...
	wpa_printf(MSG_INFO, "multi_ap_process_assoc_resp : easymesh_support = 1");
	if (wpa_s->current_ssid)
	{
		wpa_s->current_ssid->easymesh_support = 1;
	}
}
				       ...
				       union wpa_event_data *data)
{
	...
	/*4addr mode is under the control of tplink.sh 
