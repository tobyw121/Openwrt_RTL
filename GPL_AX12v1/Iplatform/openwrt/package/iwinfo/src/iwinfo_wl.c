...
static int wl_ifname(const char* ifname, char *mapIfname);
 ...
 * 2020-11-30
 * Content:
 *	Get max_assoc of current interface through iovar
 */
static uint32_t wl_get_maxassoc(const char *ifname)
{
	char buf[WLC_IOCTL_MAXLEN];
	char map_ifname[IFNAMSIZ];
	uint32_t maxassoc = WL_MAX_STA_COUNT;

	wl_ifname(ifname, &map_ifname);

	memset(buf, 0, WLC_IOCTL_MAXLEN);

	if (!wl_iovar(ifname, "maxassoc", NULL, 0, buf, WLC_IOCTL_MAXLEN))
	{
		maxassoc = dtoh32(*(uint32_t *)buf);
	}
...
static struct wl_maclist * wl_read_assoclist(const char *ifname)
{
	...
	uint32_t cur_maxassoc = wl_get_maxassoc(ifname);
	uint32_t maclen = 4 + cur_maxassoc * 6;

	if ((macs = (struct wl_maclist *) malloc(maclen)) != NULL)
	{
		memset(macs, 0, maclen);
		macs->count = cur_maxassoc;
	...
	/* Try again! Use WL_MAX_STA_COUNT instead, but it uses more memory */
	if (cur_maxassoc < WL_MAX_STA_COUNT)
	{
		maclen = 4 + WL_MAX_STA_COUNT * 6;
		if ((macs = (struct wl_maclist *) malloc(maclen)) != NULL)
		{
			memset(macs, 0, maclen);
...
int wl_get_channel(const char *ifname, int *buf)
{
	...
	if (chanspec != 0)
	{
        ...
        /* wangxiaolong@tp-link.com.cn
        ** 2018-09-25
        ** Note: Add 160MHz home channel 
        */
        else if (nbw == WL_CHANSPEC_BW_160)
		{
			nctrlsb = chanspec & WL_CHANSPEC_CTL_SB_MASK;
			...
			for (i = 0; i < 8; i++)
			{
				if (nctrlsb == sidebands[i])
				{
					home_channel = target_channel - (CH_80MHZ_APART - CH_10MHZ_APART) + (i * CH_20MHZ_APART);
					break;
				}
			}
...
int wl_get_range(const char *ifname, int *buf)
{
	...
	if ((error = wl_ioctl(ifname, WLC_GET_BAND, &band, sizeof(band))))
	{
		return error;
	}
	...
	else if ((band == BAND_6G) && (ch == 2))
		sf = WF_CHAN_FACTOR_6_G_CH2;
	else if (band == BAND_6G)
		sf = WF_CHAN_FACTOR_6_G;
	...
	struct iw_range *range, int *entrylen)
{
	...
	if( (error = wl_ioctl(ifname, WLC_WDS_RESULTS, list, buflen)) )
	{
		free(list);
		return error;
	}
	...
	list->count = dtoh32(list->count);
        ...
        if(list->count & (0x1f0000))
	{
		/*user first two bytes decide continue get results or not*/
		retVal = 1;
		list->count &= (0xffff);
	}
	...
	if( !wl_get_freqlist(ifname, chans, &len) )
	{
		for( e = (struct iwinfo_freqlist_entry *)chans; e->channel; e++ )
		{
			if( e->channel <= 14 )
			{
				...
				*buf |= IWINFO_80211_G;
                *buf |= IWINFO_80211_N;
#ifdef SUPPORT_BRCM_2G_AX
                *buf |= IWINFO_80211_AX;
#endif
#ifdef SUPPORT_BRCM_2G_BE
                *buf |= IWINFO_80211_BE;
#endif
			}
			else
...
#ifdef SUPPORT_BRCM_5G_AX
                *buf |= IWINFO_80211_AX;
#endif
#ifdef SUPPORT_BRCM_5G_BE
                *buf |= IWINFO_80211_BE;
#endif
...
/* check mapping interface name:
 * 1. get from luci config wireless
 * 2. return itself
 */
static int wl_ifname(const char* ifname, char *mapIfname)
{
	...
	ASSERT(ifname != NULL && mapIfname != NULL);
	...
	memset(opt_name,0, sizeof(opt_name));
	
    snprintf(opt_name, sizeof(opt_name), "wireless.%s.ifname", ifname);

    if (uci_lookup_ptr(ctx, &ptr, opt_name, true) != UCI_OK)
    {
        //uci_perror(ctx, "lookup ptr");
        uci_free_context(ctx);
		strcpy(mapIfname, ifname);
        return 0;
    }

	/* check if lookup complete */
	...
	{
		strcpy(mapIfname, ptr.o->v.string);	
	}
