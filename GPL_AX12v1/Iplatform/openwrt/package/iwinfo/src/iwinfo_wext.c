...
int wext_get_assoclist(const char *ifname, char *buf, int *len)
{
	...
	struct iwreq wrq;
	unsigned int staNum;
	unsigned int maxStaNum = WLAN_MAX_STA_NUM;
	struct iwinfo_assoclist_entry entry;
	int index, num;
	char tmpbuf[sizeof(unsigned int) + maxStaNum * sizeof(WLAN_STA_INFO)];
	WLAN_STA_INFO *ptr;
	
	//sizeof(buf) = IWINFO_BUFSIZE = 24*1024

	wrq.u.data.pointer = (caddr_t) &tmpbuf;
	wrq.u.data.length  = sizeof(tmpbuf);
	wrq.u.data.flags   = 0;
	memcpy(tmpbuf, &maxStaNum, sizeof(maxStaNum));
	if(wext_ioctl(ifname, RTPRIV_IOCTL_GSTAINFO, &wrq) >= 0)
	{
		memcpy(&staNum, tmpbuf, sizeof(unsigned int));
		ptr = (WLAN_STA_INFO *)(tmpbuf + sizeof(unsigned int));
		//memcpy(staInfo, buf + sizeof(unsigned int), sizeof(WLAN_STA_INFO) * num);
		
		for (index=0; index < staNum; index++)
		{
			// to do copy into iwinfo_assoclist_entry
			memset(&entry, 0, sizeof(entry));
			entry->rx_packets = ptr->rx_packets;
			entry->tx_packets = ptr->tx_packets;
			memcpy(entry.mac, ptr->addr, 6);
			ptr++;
			
			memcpy(buf + index * sizeof(struct iwinfo_assoclist_entry), &entry, sizeof(struct iwinfo_assoclist_entry));
		}
		*len = index * sizeof(struct iwinfo_assoclist_entry);
		return 0;
	}
#endif 
	return -1;
