	...
	else if (c->enabled)
	{
		...
		else if (c->wpa_version)
		{
			switch (c->wpa_version) {
				case 3:
					snprintf(buf, sizeof(buf), "mixed WPA/RSN %s (%s)",
						format_enc_suites(c->auth_suites),
						format_enc_ciphers(c->pair_ciphers | c->group_ciphers));
					break;

				case 2:
					snprintf(buf, sizeof(buf), "RSN %s (%s)",
						format_enc_suites(c->auth_suites),
						format_enc_ciphers(c->pair_ciphers | c->group_ciphers));
					break;
...
static void print_scanlist(const struct iwinfo_ops *iw, const char *ifname)
{
	...
	ret = iw->scanlist(ifname, buf, &len);
	printf("get ret %d\n", ret);
	if (ret)
	{
...
static void print_channel_utilization(const struct iwinfo_ops *iw, const char *ifname)
{
	int channel_utilization;
	
	if (iw->channel_utilization && iw->channel_utilization(ifname, &channel_utilization))
	{
		printf("Get channel utilization error\n");
		return;
	}
...
int main(int argc, char **argv)
{
	...
	for (i = 2; i < argc; i++)
	{
		...
		case 'u':
			print_channel_utilization(iw, argv[1]);
			break;
