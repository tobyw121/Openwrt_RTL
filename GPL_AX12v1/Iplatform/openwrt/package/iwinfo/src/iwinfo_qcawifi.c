...
uint8_t qcawifi_restricted(struct ieee80211_channel *chan)
{
    int noht20 = !(chan->ic_flags & (IEEE80211_CHAN_HT20 | IEEE80211_CHAN_VHT20
        | IEEE80211_CHAN_A | IEEE80211_CHAN_G | IEEE80211_CHAN_B));
    int noht40 = !(chan->ic_flags & (IEEE80211_CHAN_HT40PLUS | IEEE80211_CHAN_HT40MINUS
                            | IEEE80211_CHAN_VHT40PLUS | IEEE80211_CHAN_VHT40MINUS));
    int noht80 = !(chan->ic_flags & (IEEE80211_CHAN_VHT80));

    int chandfs = !!(chan->ic_flagext & IEEE80211_CHAN_DFS);
    int chanradar = !!(chan->ic_flagext & IEEE80211_CHAN_RADAR);
    int chansta = !!(chan->ic_flagext & IEEE80211_CHAN_DISALLOW_HOSTAP);
    int chanadhoc = !!(chan->ic_flagext & IEEE80211_CHAN_DISALLOW_ADHOC);
...
int qcawifi_get_assoclist(const char *ifname, char *buf, int *len)
{
	...
	if( (tl = get80211priv(ifname, IEEE80211_IOCTL_STA_INFO, tmp, 24*1024)) > 0 )
	{
		...
		do {
			...
			memcpy(stats.is_u.macaddr, &si->isi_macaddr, 6);

			entry.inactive = si->isi_inact * 1000;

			if ( get80211priv(ifname, IEEE80211_IOCTL_STA_STATS, &stats, sizeof(stats)) > 0 )
			{		
				entry.rx_packets = stats.is_stats.ns_rx_data;
				entry.tx_packets = stats.is_stats.ns_tx_data_success;
			}
			else
			{
...
int qcawifi_get_freqlist(const char *ifname, char *buf, int *len)
{
	...
	if( rc >= 0 )
	{
		...
		for( i = 0; i < chans.ic_nchans; i++ )
		{
			...
			entry.restricted = qcawifi_restricted(&chans.ic_chans[i]);
