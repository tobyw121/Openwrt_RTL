		...
		if (c->enabled)
		{
			...
			else if (c->wpa_version)
			{
				switch (c->wpa_version) {
					case 3:
						sprintf(desc, "mixed WPA/RSN %s (%s)",
...
static void set_rateinfo(lua_State *L, struct iwinfo_rate_entry *r, int rx)
{
...
static int iwinfo_L_assoclist(lua_State *L, int (*func)(const char *, char *, int *))
{
	...
	if (!(*func)(ifname, rv, &len))
	{
		for (i = 0; i < len; i += sizeof(struct iwinfo_assoclist_entry))
		{
			...
			set_rateinfo(L, &e->rx_rate, 1);
			set_rateinfo(L, &e->tx_rate, 0);
...
static int iwinfo_L_scanlist(lua_State *L, int (*func)(const char *, char *, int *))
{
	...
	if (!(*func)(ifname, rv, &len))
	{
		for (i = 0, x = 1; i < len; i += sizeof(struct iwinfo_scanlist_entry), x++)
		{
			...
			lua_pushnumber(L, (int8_t)e->signal);
			lua_setfield(L, -2, "signal");
...
static int iwinfo_L_freqlist(lua_State *L, int (*func)(const char *, char *, int *))
{
	...
	if (!(*func)(ifname, rv, &len))
	{
		for (i = 0, x = 1; i < len; i += sizeof(struct iwinfo_freqlist_entry), x++)
		{
			...
			/* flags (HT80-HT40-HT20-NA-ADHOC-IBSS-RADAR-DFS) */
			lua_pushinteger(L, e->restricted);
...
#ifdef USE_MTK
/* Mtk */
LUA_WRAP_INT(mtk,channel)
LUA_WRAP_INT(mtk,frequency)
LUA_WRAP_INT(mtk,frequency_offset)
LUA_WRAP_INT(mtk,txpower)
LUA_WRAP_INT(mtk,txpower_offset)
LUA_WRAP_INT(mtk,bitrate)
LUA_WRAP_INT(mtk,signal)
LUA_WRAP_INT(mtk,noise)
LUA_WRAP_INT(mtk,quality)
LUA_WRAP_INT(mtk,quality_max)
LUA_WRAP_STRING(mtk,ssid)
LUA_WRAP_STRING(mtk,bssid)
LUA_WRAP_STRING(mtk,country)
LUA_WRAP_STRING(mtk,hardware_name)
LUA_WRAP_STRUCT(mtk,mode)
LUA_WRAP_STRUCT(mtk,assoclist)
LUA_WRAP_STRUCT(mtk,txpwrlist)
LUA_WRAP_STRUCT(mtk,scanlist)
LUA_WRAP_STRUCT(mtk,freqlist)
LUA_WRAP_STRUCT(mtk,countrylist)
LUA_WRAP_STRUCT(mtk,hwmodelist)
LUA_WRAP_STRUCT(mtk,encryption)
LUA_WRAP_STRUCT(mtk,mbssid_support)
LUA_WRAP_STRUCT(mtk,hardware_id)
#endif

#ifdef USE_RTK
/* rtk */
LUA_WRAP_INT(rtk,channel)
LUA_WRAP_INT(rtk,frequency)
LUA_WRAP_INT(rtk,frequency_offset)
LUA_WRAP_INT(rtk,txpower)
LUA_WRAP_INT(rtk,txpower_offset)
LUA_WRAP_INT(rtk,bitrate)
LUA_WRAP_INT(rtk,signal)
LUA_WRAP_INT(rtk,noise)
LUA_WRAP_INT(rtk,quality)
LUA_WRAP_INT(rtk,quality_max)
LUA_WRAP_STRING(rtk,ssid)
LUA_WRAP_STRING(rtk,bssid)
LUA_WRAP_STRING(rtk,country)
LUA_WRAP_STRING(rtk,hardware_name)
LUA_WRAP_STRUCT(rtk,mode)
LUA_WRAP_STRUCT(rtk,assoclist)
LUA_WRAP_STRUCT(rtk,txpwrlist)
LUA_WRAP_STRUCT(rtk,scanlist)
LUA_WRAP_STRUCT(rtk,freqlist)
LUA_WRAP_STRUCT(rtk,countrylist)
LUA_WRAP_STRUCT(rtk,hwmodelist)
LUA_WRAP_STRUCT(rtk,encryption)
LUA_WRAP_STRUCT(rtk,mbssid_support)
LUA_WRAP_STRUCT(rtk,hardware_id)
#endif
...
#ifdef USE_INTEL
/* Intel */
LUA_WRAP_INT(intel,channel)
LUA_WRAP_INT(intel,frequency)
LUA_WRAP_INT(intel,frequency_offset)
LUA_WRAP_INT(intel,txpower)
LUA_WRAP_INT(intel,txpower_offset)
LUA_WRAP_INT(intel,bitrate)
LUA_WRAP_INT(intel,signal)
LUA_WRAP_INT(intel,noise)
LUA_WRAP_INT(intel,quality)
LUA_WRAP_INT(intel,quality_max)
LUA_WRAP_STRING(intel,ssid)
LUA_WRAP_STRING(intel,bssid)
LUA_WRAP_STRING(intel,country)
LUA_WRAP_STRING(intel,hardware_name)
LUA_WRAP_STRUCT(intel,mode)
LUA_WRAP_STRUCT(intel,assoclist)
LUA_WRAP_STRUCT(intel,txpwrlist)
LUA_WRAP_STRUCT(intel,scanlist)
LUA_WRAP_STRUCT(intel,freqlist)
LUA_WRAP_STRUCT(intel,countrylist)
LUA_WRAP_STRUCT(intel,hwmodelist)
LUA_WRAP_STRUCT(intel,encryption)
LUA_WRAP_STRUCT(intel,mbssid_support)
LUA_WRAP_STRUCT(intel,hardware_id)
#endif
...
#ifdef USE_RTK
/* rtk table */
static const luaL_reg R_rtk[] = {
	LUA_REG(rtk,channel),
	LUA_REG(rtk,frequency),
	LUA_REG(rtk,frequency_offset),
	LUA_REG(rtk,txpower),
	LUA_REG(rtk,txpower_offset),
	LUA_REG(rtk,bitrate),
	LUA_REG(rtk,signal),
	LUA_REG(rtk,noise),
	LUA_REG(rtk,quality),
	LUA_REG(rtk,quality_max),
	LUA_REG(rtk,mode),
	LUA_REG(rtk,ssid),
	LUA_REG(rtk,bssid),
	LUA_REG(rtk,country),
	LUA_REG(rtk,assoclist),
	LUA_REG(rtk,txpwrlist),
	LUA_REG(rtk,scanlist),
	LUA_REG(rtk,freqlist),
	LUA_REG(rtk,countrylist),
	LUA_REG(rtk,hwmodelist),
	LUA_REG(rtk,encryption),
	LUA_REG(rtk,mbssid_support),
	LUA_REG(rtk,hardware_id),
	LUA_REG(rtk,hardware_name),
	{ NULL, NULL }
};
#endif
...
#ifdef USE_RTK
	luaL_newmetatable(L, IWINFO_RTK_META);
	luaL_register(L, NULL, R_rtk);
	lua_pushvalue(L, -1);
	lua_setfield(L, -2, "__index");
	lua_setfield(L, -2, "rtk");
#endif
