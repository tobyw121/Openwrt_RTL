...
//			panic_printk("%s, 1-> %x, %x\n", __FUNCTION__, RTL_R8(0xc50), priv->pshare->restore);
		...
		if (priv->pshare->restore == 0) {
			...
			priv->pshare->restore = phy_query_bb_reg(priv, 0xc50, 0x7f);
#if defined(USE_OUT_SRC)
			...
			if(CHIP_VER_92X_SERIES(priv)) {			
				if (priv->pshare->rf_ft_var.one_path_cca == 0) 	{				
					phy_set_bb_reg(priv, 0xc50, 0x7f, value_IGI);
					phy_set_bb_reg(priv, 0xc58, 0x7f, value_IGI);
				} else if (priv->pshare->rf_ft_var.one_path_cca == 1) {
					phy_set_bb_reg(priv, 0xc50, 0x7f, value_IGI);
					phy_set_bb_reg(priv, 0xc58, 0x7f, getIGIFor1RCCA(value_IGI));
				} else if (priv->pshare->rf_ft_var.one_path_cca == 2) {
					phy_set_bb_reg(priv, 0xc50, 0x7f, getIGIFor1RCCA(value_IGI));
					phy_set_bb_reg(priv, 0xc58, 0x7f, value_IGI);
				}
			}
#endif
 ...
 void set_CCK_swing_index(struct rtl8192cd_priv *priv, short CCK_index)
{
	...
	if ((OPMODE & WIFI_MP_STATE) || priv->pshare->rf_ft_var.mp_specific)
		...
		if (GET_CHIP_VER(priv) == VERSION_8188E ||  GET_CHIP_VER(priv) == VERSION_8723B) {	
			...
			if (channel !=14) {
				RTL_W8( 0xa22, cck_swing_table_ch1_ch13_new[CCK_index][0]);
				RTL_W8( 0xa23, cck_swing_table_ch1_ch13_new[CCK_index][1]);
				RTL_W8( 0xa24, cck_swing_table_ch1_ch13_new[CCK_index][2]);
				RTL_W8( 0xa25, cck_swing_table_ch1_ch13_new[CCK_index][3]);
				RTL_W8( 0xa26, cck_swing_table_ch1_ch13_new[CCK_index][4]);
				RTL_W8( 0xa27, cck_swing_table_ch1_ch13_new[CCK_index][5]);
				RTL_W8( 0xa28, cck_swing_table_ch1_ch13_new[CCK_index][6]);
				RTL_W8( 0xa29, cck_swing_table_ch1_ch13_new[CCK_index][7]);
			}
			else{
				RTL_W8( 0xa22, cck_swing_table_ch14_new[CCK_index][0]);
				RTL_W8( 0xa23, cck_swing_table_ch14_new[CCK_index][1]);
				RTL_W8( 0xa24, cck_swing_table_ch14_new[CCK_index][2]);
				RTL_W8( 0xa25, cck_swing_table_ch14_new[CCK_index][3]);
				RTL_W8( 0xa26, cck_swing_table_ch14_new[CCK_index][4]);
				RTL_W8( 0xa27, cck_swing_table_ch14_new[CCK_index][5]);
				RTL_W8( 0xa28, cck_swing_table_ch14_new[CCK_index][6]);
				RTL_W8( 0xa29, cck_swing_table_ch14_new[CCK_index][7]);
			}		
		} else
#endif		
		...
		if (GET_CHIP_VER(priv) == VERSION_8192E) {	
			...
			if (channel !=14) {
				RTL_W8( 0xa22, cck_swing_table_ch1_ch13_92e[CCK_index][0]);
				RTL_W8( 0xa23, cck_swing_table_ch1_ch13_92e[CCK_index][1]);
				RTL_W8( 0xa24, cck_swing_table_ch1_ch13_92e[CCK_index][2]);
				RTL_W8( 0xa25, cck_swing_table_ch1_ch13_92e[CCK_index][3]);
				RTL_W8( 0xa26, cck_swing_table_ch1_ch13_92e[CCK_index][4]);
				RTL_W8( 0xa27, cck_swing_table_ch1_ch13_92e[CCK_index][5]);
				RTL_W8( 0xa28, cck_swing_table_ch1_ch13_92e[CCK_index][6]);
				RTL_W8( 0xa29, cck_swing_table_ch1_ch13_92e[CCK_index][7]);
			}
			else{
				RTL_W8( 0xa22, cck_swing_table_ch14_92e[CCK_index][0]);
				RTL_W8( 0xa23, cck_swing_table_ch14_92e[CCK_index][1]);
				RTL_W8( 0xa24, cck_swing_table_ch14_92e[CCK_index][2]);
				RTL_W8( 0xa25, cck_swing_table_ch14_92e[CCK_index][3]);
				RTL_W8( 0xa26, cck_swing_table_ch14_92e[CCK_index][4]);
				RTL_W8( 0xa27, cck_swing_table_ch14_92e[CCK_index][5]);
				RTL_W8( 0xa28, cck_swing_table_ch14_92e[CCK_index][6]);
				RTL_W8( 0xa29, cck_swing_table_ch14_92e[CCK_index][7]);
			}
			priv->pshare->thermal_value = 0; //force to do power tracking after setting cck swing		
		}	 else
#endif
...
void reset_FA_reg(struct rtl8192cd_priv *priv)
{
	...
	if((GET_CHIP_VER(priv)== VERSION_8812E)||(GET_CHIP_VER(priv)== VERSION_8881A)||(GET_CHIP_VER(priv)== VERSION_8814A)||(GET_CHIP_VER(priv)== VERSION_8822B)) {
		...
		phy_set_bb_reg(priv, 0x9a4, BIT(17), 1);
		phy_set_bb_reg(priv, 0x9a4, BIT(17), 0);

		// reset CCK FA counter
		phy_set_bb_reg(priv, 0xa2c, BIT(17), 0);
		phy_set_bb_reg(priv, 0xa2c, BIT(17), 1);

		// reset CCA counter
		phy_set_bb_reg(priv, 0xb58, BIT(17), 1);
		phy_set_bb_reg(priv, 0xb58, BIT(17), 0);
...
#if defined(CONFIG_RTL_NEW_AUTOCH) && (!defined(USE_OUT_SRC) || defined(_OUTSRC_COEXIST))
					...
					else
					{
						...
						if ((tp_now < 20) || ((tp_now < (priv->ext_stats.tp_average_pre-5))|| (tp_now > (priv->ext_stats.tp_average_pre+5)))){
							...
							if (!mw5_legacy_timer_pending(&priv->dnc_timer)) {
								...
								mw5_legacy_mod_timer(&priv->dnc_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(100));	// 100 ms
							}
						}
					}
...
void check_RA_by_rssi(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if (level != pstat->cmn_info.ra_info.rssi_level && CHIP_VER_92X_SERIES(priv)) {
	...
	if (pstat->ht_cap_len) {
		...
		if (set_sgi) {
			...
			if (GET_CHIP_VER(priv)==VERSION_8188E)
				...
				ODMPTR->ra_info[pstat->cmn_info.aid].sgi_enable = 1;
#else
				priv->pshare->RaInfo[pstat->cmn_info.aid].sgi_enable = 1;
#endif
			else
#endif
			...
			if (GET_CHIP_VER(priv)==VERSION_8188E)
#ifdef RATEADAPTIVE_BY_ODM
				ODMPTR->ra_info[pstat->cmn_info.aid].sgi_enable = 0;
#else				
				priv->pshare->RaInfo[pstat->cmn_info.aid].sgi_enable = 0;
#endif
		}
#endif
	}
...
#ifdef USE_OUT_SRC
	if(IS_OUTSRC_CHIP(priv)) {
		//if ((pstat->cmn_info.ra_info.rssi_level < 1) || (pstat->cmn_info.ra_info.rssi_level > 4)) 
			//odm_ra_state_check(ODMPTR, (s4Byte)pstat->rssi, FALSE, &pstat->cmn_info.ra_info.rssi_level) ;
	} else 
#endif
	{
...
// Client mode IOT issue, Button 2009.07.17
	...
	if (GET_CHIP_VER(priv)==VERSION_8188E) {
		...
		if (pstat->tx_ra_bitmap & 0xff000) {
			if (priv->pshare->is_40m_bw)
				priv->pshare->RaInfo[pstat->cmn_info.aid].rate_id = ARFR_1T_40M;
			else
				priv->pshare->RaInfo[pstat->cmn_info.aid].rate_id = ARFR_1T_20M;
		} else if (pstat->tx_ra_bitmap & 0xff0) {
			priv->pshare->RaInfo[pstat->cmn_info.aid].rate_id = ARFR_BG_MIX;
		} else {
			priv->pshare->RaInfo[pstat->cmn_info.aid].rate_id = ARFR_B_ONLY;
		}
		...
		if (pstat->tx_ra_bitmap & 0xff000) {
			if (priv->pshare->is_40m_bw)
				pRAInfo->rate_id = ARFR_1T_40M;
			else
				pRAInfo->rate_id = ARFR_1T_20M;
		} else if (pstat->tx_ra_bitmap & 0xff0) {
			pRAInfo->rate_id = ARFR_BG_MIX;
		} else {
			pRAInfo->rate_id = ARFR_B_ONLY;
		}
		phydm_ra_update_8188e(ODMPTR, pstat->cmn_info.aid, pRAInfo->rate_id, pstat->tx_ra_bitmap, pRAInfo->sgi_enable);
#endif		
	} else
#endif
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
...
{
	RTL_W16(0x894+2, (SS_AUTO_CHNL_NHM_TO_VALUE * (1000/4)));	//0x894[31:16]=0xffff	Time duration for NHM unit: 4us, 0x2710=40ms
	...
	phy_set_bb_reg(priv, 0xe28, bMaskByte0, 0xff);		//0xe28[7:0]=0xff		th_8
	phy_set_bb_reg(priv, 0x890, BIT10|BIT9|BIT8, 0x1);	//0x890[9:8]=3			enable CCX
	phy_set_bb_reg(priv, 0xc0c, BIT7, 0x1);		//0xc0c[7]=1		
}
...
void rtl8192cd_GetNHMCounterStatistics(struct rtl8192cd_priv *priv)
{
	...
	value32 = phy_query_bb_reg(priv, 0x8d8, bMaskDWord);
...
{
	phy_set_bb_reg(priv, 0x890, BIT1, 0);
	phy_set_bb_reg(priv, 0x890, BIT1, 1);
}
...
void rtl8192cd_SetLNA(struct rtl8192cd_priv *priv, int type)
{
	...
	{		
		...
		{
			phy_set_rf_reg(priv, rf_path, 0x10, 0xf0000, 0x3);				/*select Rx mode*/
			phy_set_rf_reg(priv, rf_path, 0x10, 0x0000f, 0xf);

			if ( type == disable_LNA )
				phy_set_rf_reg(priv, rf_path, 0x11, 0xfffff, 0xef500);		/*disable LNA*/
			else if ( type == enable_LNA )
				phy_set_rf_reg(priv, rf_path, 0x11, 0xfffff, 0xff500);		/*back to normal*/

			// verify only -----------------------------------------------------
			//unsigned char verify_0x05;
			//phy_set_rf_reg(priv, rf_path, 0x00, 0xf0000, 0x3);
			//verify_0x05 = phy_query_rf_reg(priv, rf_path, 0x05, 0x40000, 1);
	...
	else if ( GET_CHIP_VER(priv) == VERSION_8188E )							/* for osk platform*/
	{
		for ( rf_path = 0 ; rf_path != max_rf_path ; rf_path++ )
		{
			phy_set_rf_reg(priv, rf_path, 0xef, 0x80000, 0x1);
			phy_set_rf_reg(priv, rf_path, 0x30, 0xfffff, 0x18000);			/*select Rx mode*/
			phy_set_rf_reg(priv, rf_path, 0x31, 0xfffff, 0x0000f);

			if ( type == disable_LNA )
				phy_set_rf_reg(priv, rf_path, 0x32, 0xfffff, 0x37f82);		/*disable LNA*/
			else if ( type == enable_LNA )
				phy_set_rf_reg(priv, rf_path, 0x32, 0xfffff, 0x77f82);		/*back to normal*/
			
			phy_set_rf_reg(priv, rf_path, 0xef, 0x80000, 0x0);
		}
	}
}
...
{
	phy_set_bb_reg(priv, 0x824, BIT3|BIT2|BIT1, txMode);		// set TXmode to standby mode to remove outside noise affect
	phy_set_bb_reg(priv, 0x824, BIT22|BIT21|BIT20, rxMode);	// set RXmode to standby mode to remove outside noise affect

	if(get_rf_mimo_mode(priv) != RF_1T1R)
	{
		phy_set_bb_reg(priv, 0x82c, BIT3|BIT2|BIT1, txMode);		// set TXmode to standby mode to remove outside noise affect
		phy_set_bb_reg(priv, 0x82c, BIT22|BIT21|BIT20, rxMode);	// set RXmode to standby mode to remove outside noise affect
	}
...
{
	phy_set_bb_reg(priv,REG_OFDM_0_ECCA_THRESHOLD, bMaskByte0, (u1Byte)L2H);
	phy_set_bb_reg(priv,REG_OFDM_0_ECCA_THRESHOLD, bMaskByte2, (u1Byte)H2L);
}
...
void rtl8192cd_MACEDCCAState(struct rtl8192cd_priv *priv, u1Byte State)
{
	...
	{
		phy_set_bb_reg(priv, REG_TX_PTCL_CTRL, BIT15, 1);	//ignore EDCCA	reg520[15]=1
		phy_set_bb_reg(priv, REG_RD_CTRL, BIT11, 0);		//reg524[11]=0
	}
	else		// don't set MAC ignore EDCCA signal
	{
		phy_set_bb_reg(priv, REG_TX_PTCL_CTRL, BIT15, 0);	//don't ignore EDCCA	 reg520[15]=0
		phy_set_bb_reg(priv, REG_RD_CTRL, BIT11, 1);	//reg524[11]=1	
	}
}
...
void rtl8192cd_AdaptivityInit(struct rtl8192cd_priv *priv)
{
	...
	priv->pshare->rf_ft_var.adaptivity_enable_flag = FALSE;	// use this flag to judge enable or disable	

	if (priv->pshare->rf_ft_var.adaptivity_enable == 2)
		priv->pshare->rf_ft_var.dynamic_link_adaptivity = TRUE;
	else
		priv->pshare->rf_ft_var.dynamic_link_adaptivity = FALSE;
	...
	else
	{
		...
		else
		{
			...
			if(isCleanEnvironment == TRUE)
			{
				...
				priv->pshare->rf_ft_var.adaptivity_enable_flag = TRUE;
				priv->pshare->rf_ft_var.adaptivity_flag = TRUE;
			}
			else
			{
				...
				priv->pshare->rf_ft_var.adaptivity_enable_flag = FALSE;
...
void rtl8192cd_CheckAdaptivity(struct rtl8192cd_priv *priv)
{
	...
	{
		if(priv->pshare->rf_ft_var.dynamic_link_adaptivity == TRUE)
		{
...
void rtl8192cd_Adaptivity(struct rtl8192cd_priv *priv, u1Byte IGI)
{
	...
	DEBUG_INFO("adaptivity_flag=%d, adaptivity_enable_flag=%d, bLinked=%d\n", 
		priv->pshare->rf_ft_var.adaptivity_flag, priv->pshare->rf_ft_var.adaptivity_enable_flag, priv->pshare->rf_ft_var.bLinked);

	if ( priv->pshare->rf_ft_var.dynamic_link_adaptivity
		&& !priv->pshare->rf_ft_var.bLinked
		&& !priv->pshare->rf_ft_var.adaptivity_enable_flag )
	{
		rtl8192cd_SetEDCCAThreshold(priv, 0x7f, 0x7f);
		return;
	}
...
void RRSR_power_control_11n(struct rtl8192cd_priv *priv, int lower)
{
	...
	if ( lower && priv->pshare->phw->lower_tx_power== 0 )
	{
	  ...
	  val32 = RTL_R32(0x6d8) & 0xFFE3FFFF;
	  val32 |= priv->pshare->phw->TXPowerOffset << 18;
	...
	if (lower && priv->pshare->phw->lower_tx_power == 0) {
		...
		if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G && priv->pshare->rf_ft_var.sub_pwr) {
			...
			if (priv->pmib->dot11RFEntry.tx3path){
				//disable tx3path
				phy_set_bb_reg(priv,rTX_PATH_SEL_1, bMaskH12Bits,0x932);	// Tx path B/C/D for 1SS HT/VHT
				phy_set_bb_reg(priv,rTX_PATH_SEL_2, bMask4to15Bits,0x936);// Tx path B/C/D for 2SS HT/VHT
				phy_set_bb_reg(priv,rTXPATH_AC, bMask4to7Bits,0x2);		// Tx path B/C/D for CCK
				phy_set_bb_reg(priv,rCCK_RX_AC, bMaskH4Bits,0x4);
			}
		...
		if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G && priv->pshare->rf_ft_var.sub_pwr) {
			...
			if (priv->pmib->dot11RFEntry.tx3path){
				phy_set_bb_reg(priv,rTX_PATH_SEL_1, bMaskH12Bits,0x90e);	// Tx path B/C/D for 1SS HT/VHT
				phy_set_bb_reg(priv,rTX_PATH_SEL_2, bMask4to15Bits,0x90e);// Tx path B/C/D for 2SS HT/VHT
				phy_set_bb_reg(priv,rTXPATH_AC, bMask4to7Bits,0xe);		// Tx path B/C/D for CCK
				phy_set_bb_reg(priv,rCCK_RX_AC, bMaskH4Bits,0x7); 
			}
		}
	}
}
...
#define TX_POWER_NEAR_FIELD_THRESH 60
...
)
{
	...
	int disable_pwr_ctrl;
	
	if (!priv->pshare->rf_ft_var.tx_pwr_ctrl)
		return;

#ifdef CONFIG_DYNAMIC_TX_TWR
#if ((RTL8812A_SUPPORT == 1) || (RTL8881A_SUPPORT == 1) || (RTL8814A_SUPPORT == 1) || (RTL8822B_SUPPORT == 1) || (RTL8198F_SUPPORT == 1) || (RTL8814B_SUPPORT == 1))
	if (ODMPTR->support_ic_type & (ODM_RTL8812 | ODM_RTL8881A | ODM_RTL8814A | ODM_RTL8822B | ODM_RTL8198F | ODM_RTL8814B))
		pwr_thd = TX_POWER_NEAR_FIELD_THRESH;
#endif
#endif
	...
	if ((priv->up_time % 3) == 0)  {
	    ...
	    if(IS_OUTSRC_CHIP(priv)){
			...
			if (ODMPTR->original_dig_restore == 0)
				IGI = ODMPTR->dm_dig_table.cur_ig_value_tdma;
			else
		#endif
				IGI = ODMPTR->dm_dig_table.cur_ig_value;
	    }
#else
		IGI = RTL_R8(0xc50) & 0x7f;
#endif
		
		disable_pwr_ctrl = ((ODMPTR->false_alm_cnt.cnt_all > 1000) || ((ODMPTR->false_alm_cnt.cnt_all > 300) && (IGI >= 0x32))) ? 1 : 0;
		...
		while(pEntry) {
			if(pEntry && pEntry->expire_to) {
				if (disable_pwr_ctrl)
					pEntry->hp_level = 0;
				else if ((pEntry->hp_level == 0) && (pEntry->rssi > pwr_thd))
					pEntry->hp_level = 1;
				else if ((pEntry->hp_level == 1) && (pEntry->rssi < (pwr_thd - 8)))
					pEntry->hp_level = 0;
			}
			pEntry = findNextSTA(priv, &idx);
		}

#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8814BE) 
		if ((GET_CHIP_VER(priv) == VERSION_8192E)||(GET_CHIP_VER(priv) == VERSION_8822B)||(GET_CHIP_VER(priv) == VERSION_8198F)||(GET_CHIP_VER(priv) == VERSION_8814B)) {
			if (!disable_pwr_ctrl && (ODMPTR->rssi_min != 0xff)) {
				if (ODMPTR->rssi_min > pwr_thd)
					RRSR_power_control_11n(priv,  1);
				else if (ODMPTR->rssi_min < (pwr_thd - 8))
...
void _phy_save_adda_registers(struct rtl8192cd_priv *priv, unsigned int* adda_reg,	unsigned int* adda_backup, unsigned int RegisterNum)
{
	unsigned int	i;
	for( i = 0 ; i < RegisterNum ; i++){
		adda_backup[i] = phy_query_bb_reg(priv, adda_reg[i], bMaskDWord);
	}
}

void _PHY_SetADDARegisters(struct rtl8192cd_priv *priv, unsigned int* adda_reg,	unsigned int* ADDASettings, unsigned int RegisterNum)
{
	unsigned int	i;

	for( i = 0 ; i < RegisterNum ; i++){
		phy_set_bb_reg(priv, adda_reg[i], bMaskDWord, ADDASettings[i]);
	}
}

void _phy_save_mac_registers(struct rtl8192cd_priv *priv, unsigned int* MACReg, unsigned int* MACBackup)
{
	unsigned int	i;
...
void _phy_reload_adda_registers(struct rtl8192cd_priv *priv, unsigned int* adda_reg, unsigned int*	adda_backup, unsigned int RegiesterNum)
{
	unsigned int	i;
...
void _phy_reload_mac_registers(struct rtl8192cd_priv *priv,unsigned int* MACReg, unsigned int*	 MACBackup)
{
	unsigned int	i;
	for(i = 0 ; i < (IQK_MAC_REG_NUM - 1); i++){
...
void _phy_mac_setting_calibration(struct rtl8192cd_priv *priv, unsigned int* MACReg, unsigned int* MACBackup)
{
	unsigned int	i = 0;
...
void _phy_path_adda_on(struct rtl8192cd_priv *priv, unsigned int* adda_reg, char isPathAOn, char is2T)
{
	unsigned int	pathOn;
	unsigned int	i;
	...
	if(FALSE == is2T){
		pathOn = 0x0bdb25a0;
		phy_set_bb_reg(priv, adda_reg[0], bMaskDWord, 0x0b1b25a0);
	}
	else{
		phy_set_bb_reg(priv, adda_reg[0], bMaskDWord, pathOn);
	}

	for( i = 1 ; i < IQK_ADDA_REG_NUM ; i++){
		phy_set_bb_reg(priv, adda_reg[i], bMaskDWord, pathOn);
	}
}
#endif


void phy_lc_calibrate(struct rtl8192cd_priv *priv)
{
	...
	if ((tmpReg & 0x70) != 0) {
		/* Path-A */
		phy_set_rf_reg(priv, RF_PATH_A, 0x00, bMask20Bits, 0x10000);

		/* Path-B */
		if (get_rf_mimo_mode(priv) != RF_1T1R)
			phy_set_rf_reg(priv, RF_PATH_B, 0x00, bMask20Bits, 0x10000);
	}

	/* 3. Read RF reg18 */
	LC_Cal = phy_query_rf_reg(priv, RF_PATH_A, 0x18, bMask12Bits, 1);

	/* 4. Set LC calibration begin */
	phy_set_rf_reg(priv, RF_PATH_A, 0x18, bMask12Bits, LC_Cal|0x08000);
