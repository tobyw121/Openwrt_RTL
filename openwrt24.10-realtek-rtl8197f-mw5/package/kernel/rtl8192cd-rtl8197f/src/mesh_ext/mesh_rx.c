...
{	
    ...
    {
		unsigned char *payload, *protocol = (skb->data+(ETH_ALEN<<1));
		struct  lls_mesh_header *mesh_header = (struct  lls_mesh_header *)(skb->data + WLAN_ETHHDR_LEN);
		payload = (skb->data + WLAN_ETHHDR_LEN) + ((mesh_header->mesh_flag &1) ? 16 : 4);
	
		if(*((UINT16*)(protocol)) == __constant_htons(ETH_P_8021Q)) {
			protocol = payload;
			payload +=4;
		} 

        // all multicast managment packet try do m2u
        if( isSpecialFloodMac(priv,skb) /*|| IS_MDNSV4_MAC(skb->data)||IS_MDNSV6_MAC(skb->data)*/||IS_IGMP_PROTO2(protocol,payload) || isICMPv6Mng2(protocol,payload) || IS_ICMPV6_PROTO2(protocol,payload)|| isMDNS2(skb->data, protocol,payload))
        {
            force_m2u = 1;
        }    
    }
