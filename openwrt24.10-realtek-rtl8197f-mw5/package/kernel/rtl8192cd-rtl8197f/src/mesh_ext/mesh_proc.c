...
#endif
{
	...
	if(pstat->current_tx_rate >= VHT_RATE_ID){
		...
		PRINT_ONE("    Rate: VHT NSS", "%s", 0);
		PRINT_ONE(((pstat->current_tx_rate - VHT_RATE_ID)/10)+1, "%u", 0);
		PRINT_ONE("-MCS", "%s", 0);
		PRINT_ONE((pstat->current_tx_rate - VHT_RATE_ID)%10, "%u", 0);
		PRINT_ONE(rate, " %d", 1);
	}
	else
#endif
	if (is_MCS_rate(pstat->current_tx_rate)) {
		PRINT_ONE("    Rate: MCS", "%s", 0);
		PRINT_ONE((pstat->current_tx_rate - HT_RATE_ID), "%u", 0);
		unsigned char* rate = (unsigned char *)MCS_DATA_RATEStr[(pstat->ht_current_tx_info&BIT(0))?1:0][(pstat->ht_current_tx_info&BIT(1))?1:0][(pstat->current_tx_rate - HT_RATE_ID)];
		PRINT_ONE(rate, " %s", 1);
	}
	else
	{
		PRINT_SINGL_ARG("    Rate: ", pstat->current_tx_rate/2, "%u");
	}
...
#endif
	{
		...
		PRINT_SINGL_ARG("    rssi_metric: ", 100 - pstat->mesh_neighbor_TBL.rssi_metric, "%lu");
	...
	PRINT_ONE("    Expire Establish (jiffies/sec): ", "%s", 0);
	PRINT_ONE(pstat->mesh_neighbor_TBL.expire - jiffies, "%lu", 0);
	PRINT_ONE(" / ", "%s", 0);
	PRINT_ONE(RTL_JIFFIES_TO_SECOND(pstat->mesh_neighbor_TBL.expire - jiffies), "%lu", 1);
	PRINT_ONE("    Expire BootSeq & LLSA (jiffies/msec): ", "%s", 0);
	PRINT_ONE(tmp, "%lu", 0);
	PRINT_ONE(" / ", "%s", 0);
	PRINT_ONE(RTL_JIFFIES_TO_MILISECONDS(tmp), "%lu", 1);
...
#endif
{
	...
	if(priv->mesh_log)
	{
		...
		while (plist != phead) 
		{
			...
			if(pstat->cmn_info.mac_addr == pstat1->cmn_info.mac_addr )
			{
				result = 1;
				break;
			}
...
#endif
{
	...
	PRINT_SINGL_ARG("  ", RTK_MESH_VERSION, "%s");
#ifdef CONFIG_RTK_WPS_MESH
	PRINT_ONE("      [V] Mesh WPS", "%s", 1);
#endif
#ifdef RTK_MESH_METRIC_REFINE
	PRINT_ONE("      [V] Mesh Metric Refine", "%s", 1);
#endif
	if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable) {
		...
		PRINT_SINGL_ARG("      Peer_CAP_Capacity:    ", MESH_PEER_LINK_CAP_NUM(priv), "%hd");
		PRINT_SINGL_ARG("      Peer_CAP_Flags:       ", (priv->mesh_PeerCAP_flags & MESH_PEER_LINK_CAP_FLAGS_MASK), "%hX");
		...
		PRINT_ONE("  Mesh Channel Switch Announcement content...", "%s", 1);
		PRINT_SINGL_ARG("      Blocking:     ", priv->mesh_CSA_blocking, "%u");
#endif
#ifdef CONFIG_RTK_WPS_MESH
		PRINT_ONE("  Mesh Target/Serial content...", "%s", 1);
		PRINT_SINGL_ARG("      Target Block:       ", priv->mesh_target_block, "%d");
		PRINT_SINGL_ARG("      SS Serial Timer:    ", priv->ss_serial_timer, "%d");
		PRINT_SINGL_ARG("      SS Serial Count:    ", priv->ss_serial_cnt, "%d");
		PRINT_ONE("      ======================", "%s", 1);
		for(i = 0; i < priv->ss_serial_cnt; i++) {
			PRINT_ARRAY_ARG("      Serial Mac: ", priv->ss_serial_mac[i], "%02x", MACADDRLEN);
			PRINT_SINGL_ARG("      Serial No:  ", priv->ss_serial_no[i], "%s");
		}
#endif
	} else
		PRINT_ONE("  Mesh mode DISABLE !!", "%s", 1);
...
#endif
{
	...
	if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable) {
		...
		pfrinfo.rf_info.rssi = priv->mesh_fake_mib.establish_rssi_threshold + 1;	// +1: Ensure connect
