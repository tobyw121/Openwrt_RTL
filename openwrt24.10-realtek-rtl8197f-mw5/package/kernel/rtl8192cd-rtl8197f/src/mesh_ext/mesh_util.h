...
#define SMP_TRY_LOCK_MESH_PROXY(__x__, __y__)
...
#define SMP_TRY_LOCK_MESH_PROXY(__x__, __y__)	do { } while (0)
...
#elif defined(CONFIG_PCI_HCI) || defined(CONFIG_RTL_TRIBAND_SUPPORT)
...
#define SMP_LOCK_MESH_QUEUE(__x__)      do { \
											if (priv->hci_type == RTL_HCI_PCIE) { \
	                                            ...
	                                            if (priv->mesh_queue_lock_owner!=_cpu_id) { \
	                                            ...
	                                            priv->mesh_queue_lock_owner=_cpu_id;\
	                                            put_cpu(); \
											} \
						   else if (priv->hci_type == RTL_HCI_USB) { \
												__u32 _cpu_id = get_cpu(); \
	                                            if (priv->mesh_queue_lock_owner!=_cpu_id) { \
													spin_lock_bh(&priv->mesh_queue_lock); \
	                                            } else {\
	                                                panic_printk("[%s %d] recursion detection, caller=%p\n",__FUNCTION__,__LINE__,__builtin_return_address(0)); \
	                                                panic_printk("Previous Lock Function is %s\n",priv->mesh_queue_lock_func); \
	                                                panic_printk("Priv is %s\n",priv->dev->name); \
...
#define SMP_UNLOCK_MESH_QUEUE(__x__)	do { \
											if (priv->hci_type == RTL_HCI_PCIE) { \
												priv->mesh_queue_lock_owner=-1; \
												spin_unlock_irqrestore(&priv->mesh_queue_lock, __x__); \
											} \
						else if (priv->hci_type == RTL_HCI_USB) { \
												priv->mesh_queue_lock_owner=-1; \
												spin_unlock_bh(&priv->mesh_queue_lock); \
											} \
					} while(0)
...
#define SMP_LOCK_MESH_PATH(__x__)      do { \
											if (priv->hci_type == RTL_HCI_PCIE) { \
												__u32 _cpu_id = get_cpu(); \
	                                            if (priv->mesh_path_lock_owner!=_cpu_id) { \
	                                            ...
	                                            priv->mesh_path_lock_owner=_cpu_id;\
	                                            put_cpu(); \
											} \
						if (priv->hci_type == RTL_HCI_USB) { \
												__u32 _cpu_id = get_cpu(); \
	                                            if (priv->mesh_path_lock_owner!=_cpu_id) { \
													spin_lock_bh(&priv->mesh_path_lock); \
	                                            } else {\
	                                                panic_printk("[%s %d] recursion detection, caller=%p\n", __FUNCTION__,__LINE__,__builtin_return_address(0)); \
	                                                panic_printk("Previous Lock Function is %s\n",priv->mesh_path_lock_func); \
	                                                panic_printk("Priv is %s\n",priv->dev->name); \
...
#define SMP_UNLOCK_MESH_PATH(__x__)		do { \
											if (priv->hci_type == RTL_HCI_PCIE) { \
												priv->mesh_path_lock_owner=-1; \
												spin_unlock_irqrestore(&priv->mesh_path_lock, __x__); \
											} \
							if (priv->hci_type == RTL_HCI_USB) { \
												priv->mesh_path_lock_owner=-1; \
												spin_unlock_bh(&priv->mesh_path_lock); \
											} \
					} while(0)
...
#define SMP_LOCK_MESH_PROXY(__x__)      do { \
											if (priv->hci_type == RTL_HCI_PCIE) {\
												__u32 _cpu_id = get_cpu(); \
	                                            if(priv->mesh_proxy_lock_owner!=_cpu_id) { \
	                                            ...
	                                            priv->mesh_proxy_lock_owner=_cpu_id;\
	                                            put_cpu(); \
											} \
						if (priv->hci_type == RTL_HCI_USB) {\
												__u32 _cpu_id = get_cpu(); \
	                                            if(priv->mesh_proxy_lock_owner!=_cpu_id) { \
	                                               	spin_lock_bh(&priv->mesh_proxy_lock); \
	                                            } else {\
	                                                panic_printk("[%s %d] recursion detection, caller=%p\n", __FUNCTION__,__LINE__,__builtin_return_address(0)); \
	                                                panic_printk("Previous Lock Function is %s\n",priv->mesh_proxy_lock_func); \
	                                                panic_printk("Priv is %s\n",priv->dev->name); \
...
#define SMP_TRY_LOCK_MESH_PROXY(__x__, __y__)   do { \
													if (priv->hci_type == RTL_HCI_PCIE) { \
				                                        if(priv->mesh_proxy_lock_owner!=smp_processor_id()) { \
				                                            SMP_LOCK_MESH_PROXY(__x__); \
				                                            __y__ = 1; \
				                                        } else { \
				                                            __y__ = 0; \
				                                        } \
...
#define SMP_UNLOCK_MESH_PROXY(__x__)	do { \
											if (priv->hci_type == RTL_HCI_PCIE) { \
												priv->mesh_proxy_lock_owner=-1; \
												spin_unlock_irqrestore(&priv->mesh_proxy_lock, __x__); \
											} \
									if (priv->hci_type == RTL_HCI_USB) { \
												priv->mesh_proxy_lock_owner=-1; \
												spin_unlock_bh(&priv->mesh_proxy_lock); \
											} \
										} while(0)
...
#define SMP_LOCK_MESH_PROXYUPDATE(__x__) do { \
											if (priv->hci_type == RTL_HCI_PCIE) { \
												__u32 _cpu_id = get_cpu(); \
	                                            if (priv->mesh_proxyupdate_lock_owner!=_cpu_id) { \
	                                            ...
	                                            priv->mesh_proxyupdate_lock_owner=_cpu_id;\
	                                            put_cpu();\
											} \
					if (priv->hci_type == RTL_HCI_USB) { \
												__u32 _cpu_id = get_cpu(); \
	                                            if (priv->mesh_proxyupdate_lock_owner!=_cpu_id) { \
	                                              	spin_lock_bh(&priv->mesh_proxyupdate_lock); \
	                                            } else {\
	                                                panic_printk("[%s %d] recursion detection, caller=%p\n",__FUNCTION__,__LINE__,__builtin_return_address(0)); \
	                                                panic_printk("Previous Lock Function is %s\n",priv->mesh_proxyupdate_lock_func); \
	                                                panic_printk("Priv is %s\n",priv->dev->name); \
...
#define SMP_UNLOCK_MESH_PROXYUPDATE(__x__)	do { \
												if (priv->hci_type == RTL_HCI_PCIE) { \
													priv->mesh_proxyupdate_lock_owner=-1; \
													spin_unlock_irqrestore(&priv->mesh_proxyupdate_lock, __x__); \
												} \
							if (priv->hci_type == RTL_HCI_USB) { \
													priv->mesh_proxyupdate_lock_owner=-1; \
													spin_unlock_bh(&priv->mesh_proxyupdate_lock); \
												} \
											} while(0)
...
#define SMP_LOCK_MESH_PREQ(__x__)      do { \
											if (priv->hci_type == RTL_HCI_PCIE) { \
												__u32 _cpu_id = get_cpu(); \
	                                            if (priv->mesh_preq_lock_owner!=_cpu_id) { \
	                                            ...
	                                            priv->mesh_preq_lock_owner=_cpu_id;\
	                                            put_cpu();\
											} \
							if (priv->hci_type == RTL_HCI_USB) { \
												__u32 _cpu_id = get_cpu(); \
	                                            if (priv->mesh_preq_lock_owner!=_cpu_id) { \
													spin_lock_bh(&priv->mesh_preq_lock); \
	                                            } else {\
	                                                panic_printk("[%s %d] recursion detection, caller=%p\n",__FUNCTION__,__LINE__,__builtin_return_address(0)); \
	                                                panic_printk("Previous Lock Function is %s\n",priv->mesh_preq_lock_func); \
	                                                panic_printk("Priv is %s\n",priv->dev->name); \
...
#define SMP_UNLOCK_MESH_PREQ(__x__)		do { \
											if (priv->hci_type == RTL_HCI_PCIE) { \
												priv->mesh_preq_lock_owner=-1; \
												spin_unlock_irqrestore(&priv->mesh_preq_lock, __x__); \
											} \
							if (priv->hci_type == RTL_HCI_USB) { \
												priv->mesh_preq_lock_owner=-1; \
												spin_unlock_bh(&priv->mesh_preq_lock); \
											} \
					} while(0)
#endif
