...
#if defined(NOT_RTK_BSP) && !defined(CONFIG_MW5_2G_ONLY)
	{(MESH_NUM_CFG << MESH_SHIFT) | (WDS_NUM_CFG << WDS_SHIFT) | (TYPE_PCI_BIOS << TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
	{(MESH_NUM_CFG << MESH_SHIFT) | (WDS_NUM_CFG << WDS_SHIFT) | (TYPE_PCI_BIOS << TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
	{(MESH_NUM_CFG << MESH_SHIFT) | (WDS_NUM_CFG << WDS_SHIFT) | (TYPE_PCI_BIOS << TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
#elif defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1)
...
// TODO: we should modify code below
	...
	if (bcnInt) {
		...
		if(priv->pshare->rf_ft_var.beacon_igi){
			struct DM_Out_Source_Dynamic_Mechanism_Structure	*pDM_Odm = ODMPTR;	
#if defined(CONFIG_WLAN_HAL_8822BE) 
...
#endif
	{
		...
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
...
#endif
#if defined(GPIO_WAKEPIN) && (IS_EXIST_RTL8822BE ||IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE)
            if((IS_HARDWARE_TYPE_8822B(priv)) || (IS_HARDWARE_TYPE_8822C(priv))||(IS_HARDWARE_TYPE_8812F(priv)))
				...
				if (IS_ROOT_INTERFACE(priv))
					rtl8197f_wlan_phase_set(rc ? 229 : 221,
						rc ? "irq-request-failed" : "irq-ready", rc);
			}
#endif
		}
#endif // CONFIG_PCI_HCI
...
//8814B ADDRCAM Init for each interface
	...
	if(IS_VXD_INTERFACE(priv) && IS_HAL_CHIP(priv)) {
		if(priv->pmib->ethBrExtInfo.macclone_enable ){
			...
			memcpy(priv->dev->dev_addr, hwaddr, MACADDRLEN);
...
int  rtl8192cd_set_hwaddr(struct net_device *dev, void *addr)
{
	...
	memcpy(priv->dev->dev_addr, p, MACADDRLEN);
	...
	for (i=0; i<NUM_WDS; i++)
		if (priv->wds_dev[i])
			memcpy(priv->wds_dev[i]->dev_addr, p, MACADDRLEN);
#endif
#ifdef CONFIG_RTK_MESH
	if(NUM_MESH > 0)
		if (priv->mesh_dev)
			memcpy(priv->mesh_dev->dev_addr, p, MACADDRLEN);
#endif
		...
		if (GET_VXD_PRIV(priv)) {
...
#endif
			{
...
#if defined(VXD_DIFF_MAC_DRV)
				{
					...
					macAddr[0] |= BIT1;
					memcpy(GET_VXD_PRIV(priv)->dev->dev_addr, macAddr, MACADDRLEN);
				}
#endif
			}
		}
...
int nonCacheRXBDAllocate(struct rtl8192cd_priv *priv,unsigned int rx_dma_size)
{
	...
	_GET_HAL_DATA(priv)->desc_rxdma_buf = (unsigned char *)(((unsigned long)rx_dma_ptr + PAGE_SIZE-1) & ~(PAGE_SIZE-1));
    _GET_HAL_DATA(priv)->desc_phy_rxdma_buf = (unsigned char *)(((unsigned long)(_GET_HAL_DATA(priv)->alloc_desc_rxdma_dmahandle) + PAGE_SIZE-1) & ~(PAGE_SIZE-1));
...
#if  !(defined(CONFIG_NET_PCI) && defined(CONFIG_PCI_HCI)) && defined(MBSSID)
	int i;
#endif
...
void rtl_wlan_vdev_init(char *ifname, void *priv, char type)
{
	...
	memcpy((char*)vdev->dev_addr,"\x00\xe0\x4c\x81\x96\x96", MACADDRLEN);
