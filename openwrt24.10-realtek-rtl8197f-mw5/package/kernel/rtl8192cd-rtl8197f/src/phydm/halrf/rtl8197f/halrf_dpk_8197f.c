...
/*8197F DPK ver:0x8 20180731*/

void _backup_mac_bb_registers_8197f(
	struct dm_struct *dm,
	u32 *reg,
	u32 *reg_backup,
	u32 reg_num)
...
void _backup_rf_registers_8197f(
	struct dm_struct *dm,
	u32 *rf_reg,
	u32 rf_reg_backup[][DPK_RF_PATH_NUM_8197F])
{
	u32 i;

	for (i = 0; i < DPK_RF_REG_NUM_8197F; i++) {
...
void _reload_mac_bb_registers_8197f(
	struct dm_struct *dm,
	u32 *reg,
	u32 *reg_backup,
	u32 reg_num)
...
void _reload_rf_registers_8197f(
	struct dm_struct *dm,
	u32 *rf_reg,
	u32 rf_reg_backup[][DPK_RF_PATH_NUM_8197F])
{
	...
	struct dm_struct *dm)
{
	...
	odm_set_bb_reg(dm, R_0xb78, MASKDWORD, 0x4c104c10); /*1dB*/
...
void phy_path_a_dpk_init_8197f(
	struct dm_struct *dm)
{
	odm_set_bb_reg(dm, R_0xb00, MASKDWORD, 0x0001e018);
	...
	odm_set_bb_reg(dm, R_0xbc0, MASKDWORD, 0x00021d5f);
...
void phy_path_b_dpk_init_8197f(
	struct dm_struct *dm)
{
	odm_set_bb_reg(dm, R_0xb70, MASKDWORD, 0x0001e018);
	...
	odm_set_bb_reg(dm, R_0xbc4, MASKDWORD, 0x00021d5f);
...
void _dpk_mac_bb_setting_8197f(
	struct dm_struct *dm)
{
	...
	odm_set_bb_reg(dm, R_0x92c, MASKDWORD, 0x0C000108);
	...
	struct dm_struct *dm)
{
	...
	odm_set_bb_reg(dm, R_0xb08, MASKDWORD, 0x41382e21);
	odm_write_1byte(dm, R_0xbad, 0x2c); /*1.5dB*/
	...
	for (k = 0; k < 5; k++) {
		...
		RF_DBG(dm, DBG_RF_DPK, "[DPK] result[%d] = 0x%x\n", k,
		       result[k]);
	...
	RF_DBG(dm, DBG_RF_DPK, "[DPK] TXAGC_search_a = 0x%x\n", tx_agc_search);
	return tx_agc_search;
}

u8 phy_path_b_gainloss_8197f(
	struct dm_struct *dm)
{
	int k;
	u8 tx_agc_search = 0x0, result[5] = {0x0};
	...
	RF_DBG(dm, DBG_RF_DPK, "[DPK] TXAGC_search_b = 0x%x\n", tx_agc_search);
	return tx_agc_search;
}

u32 phy_path_a_dodpk_8197f(
	struct dm_struct *dm,
	u8 tx_agc_search,
	u8 k)
{
	u32 reg_b68, result;
	u8 tx_agc = 0x0, pwsf_idx = 0;

	odm_write_1byte(dm, R_0xb2b, 0x40);

	pwsf_idx = tx_agc_search - 0x7;
	reg_b68 = 0x11160200 | ((pwsf_idx & 0x1f) << 10);
	tx_agc = 0x19 + pwsf_idx;
...
void phy_path_a_pas_read_8197f(
	struct dm_struct *dm,
	boolean is_gainloss)
{
	int k;
...
void phy_path_b_pas_read_8197f(
	struct dm_struct *dm,
	boolean is_gainloss)
{
	int k;
	...
	u8 k)
{
	...
	for (addr = 0; addr < 64; addr++) {
		...
		if (dm->rf_calibrate_info.lut_2g_even_a[k][addr] > 0x100000) {
			...
			RF_DBG(dm, DBG_RF_DPK,
			       "[DPK] 8197F LUT value > 0x100000, please check path A[%d]\n",
			       k);
			return 0;
		}
		if (phy_path_a_iq_check_8197f(dm, k, addr, true)) {
			/*printk("[DPK] 8197F LUT IQ value < -1.9dB, please check path A[%d]\n", k);*/
			RF_DBG(dm, DBG_RF_DPK,
			       "[DPK] 8197F LUT IQ value < -1.9dB, please check path A[%d]\n",
			       k);
			return 0;
		}
		...
		if (dm->rf_calibrate_info.lut_2g_even_b[k][addr] > 0x100000) {
			...
			RF_DBG(dm, DBG_RF_DPK,
			       "[DPK] 8197F LUT value > 0x100000, please check path B[%d]\n",
			       k);
			return 0;
		}

		if (phy_path_b_iq_check_8197f(dm, k, addr, true)) {
			/*printk("[DPK] 8197F LUT IQ value < -1.9dB, please check path B[%d]\n", k);*/
			RF_DBG(dm, DBG_RF_DPK,
			       "[DPK] 8197F LUT IQ value < -1.9dB, please check path B[%d]\n",
			       k);
			return 0;
		}
	...
	struct dm_struct *dm)
{
	...
	group = phy_dpk_channel_transfer_8197f(dm);
	...
	for (addr = 0; addr < 64; addr++) {
		...
		if (DPK_SRAM_write_DBG_8197F && (addr < 16)) {
			RF_DBG(dm, DBG_RF_DPK,
	...
	struct dm_struct *dm)
{
	...
	for (path = 0; path < DPK_RF_PATH_NUM_8197F; path++) {
		for (group = 0; group < DPK_GROUP_NUM_8197F; group++) {
	...
	u8 path)
{
	...
	case RF_PATH_A:
		...
		tx_agc_search = phy_path_a_gainloss_8197f(dm);
		if (DPK_PAS_DBG_8197F)
			phy_path_a_pas_read_8197f(dm, TRUE);
		break;

	case RF_PATH_B:

		tx_agc_search = phy_path_b_gainloss_8197f(dm);
		if (DPK_PAS_DBG_8197F)
			phy_path_b_pas_read_8197f(dm, TRUE);
		break;
	...
	u8 group)
{
	...
	case RF_PATH_A:
		...
		result = phy_path_a_dodpk_8197f(dm, tx_agc_search, group);
		if (DPK_PAS_DBG_8197F)
			phy_path_a_pas_read_8197f(dm, false);
		break;

	case RF_PATH_B:

		result = phy_path_b_dodpk_8197f(dm, tx_agc_search, group);
		if (DPK_PAS_DBG_8197F)
			phy_path_b_pas_read_8197f(dm, false);
		break;
	...
	u8 group)
{
	...
	for (addr = 0; addr < 64; addr++) { /*A even*/
		...
		if (DPK_SRAM_IQ_DBG_8197F && (addr < 16))
			RF_DBG(dm, DBG_RF_DPK,
		               "[DPK] S0 LUT even[%2d] = 0x%x\n", addr, regbc0);	

		if (phy_path_a_iq_check_8197f(dm, group, addr, true)) {
			odm_set_bb_reg(dm, R_0xb2c, MASKDWORD, 0x00000000);
			return 0;
		}
	...
	for (addr = 0; addr < 64; addr++) { /*A odd*/
		...
		if (phy_path_a_iq_check_8197f(dm, group, addr, false)) {
			odm_set_bb_reg(dm, R_0xb2c, MASKDWORD, 0x00000000);
			return 0;
		}
	...
	u8 group)
{
	...
	for (addr = 0; addr < 64; addr++) { /*B even*/
		...
		if (phy_path_b_iq_check_8197f(dm, group, addr, true)) {
			odm_set_bb_reg(dm, R_0xb2c, MASKDWORD, 0x00000000);
			return 0;
		}
	...
	for (addr = 0; addr < 64; addr++) { /*B odd*/
		...
		if (phy_path_b_iq_check_8197f(dm, group, addr, false)) {
			odm_set_bb_reg(dm, R_0xb2c, MASKDWORD, 0x00000000);
			return 0;
		}
	...
	u8 group)
{
	...
	case RF_PATH_A:
		...
		if (!is_fail) {	
			if (_dpk_a_lut_sram_read_8197f(dm, group)) {
	...
	case RF_PATH_B:
		...
		if (!is_fail) {	
			if (_dpk_b_lut_sram_read_8197f(dm, group)) {
	...
	u8 path)
{
	...
	for (group = 0; group < DPK_GROUP_NUM_8197F; group++) {
		...
		for (retry_cnt = 0; retry_cnt < 6; retry_cnt++) {
			...
			tx_agc_search = _dpk_gainloss_8197f(dm, path);

			ODM_delay_ms(1);

			dpk_fail = _dpk_by_path_8197f(dm, tx_agc_search,
						      path, group);

			if (_dpk_check_fail_8197f(dm, dpk_fail, path, group))
				break;
		}
	}
...
#endif
	return (dpk_info->dpk_path_ok & BIT(path)) ? 1 : 0;
}
	...
	u8 path)
{
	...
	case RF_PATH_A:
		phy_path_a_dpk_enable_8197f(dm);
		break;

	case RF_PATH_B:
		phy_path_b_dpk_enable_8197f(dm);
		break;
...
void _dpk_path_select_8197f(
	struct dm_struct *dm)
{
#if (DPK_DO_PATH_A)
	_dpk_calibrate_8197f(dm, RF_PATH_A);
	_dpk_on_8197f(dm, RF_PATH_A);
#endif

#if (DPK_DO_PATH_B)
	_dpk_calibrate_8197f(dm, RF_PATH_B);
	_dpk_on_8197f(dm, RF_PATH_B);
#endif
}

void _dpk_result_summary_8197f(
	struct dm_struct *dm)
{
	/*struct dm_rf_calibration_struct *rfcal_info = &dm->rf_calibrate_info;*/
	struct dm_dpk_info *dpk_info = &dm->dpk_info;
	...
	void *dm_void)
{
	...
	u32 mac_reg_backup[DPK_MAC_REG_NUM_8197F];
	u32 bb_reg_backup[DPK_BB_REG_NUM_8197F];
	u32 rf_reg_backup[DPK_RF_REG_NUM_8197F][DPK_RF_PATH_NUM_8197F];

	u32 mac_reg[DPK_MAC_REG_NUM_8197F] = {
		R_0xd94, REG_RX_WAIT_CCA, R_0x520,
		REG_BCN_CTRL, REG_GPIO_MUXCFG};

	u32 bb_reg[DPK_BB_REG_NUM_8197F] = {
		...
		R_0x848, R_0x930, R_0x934, R_0x93c, R_0x92c, R_0xc30, R_0xc3c,
		R_0xc84};

	u32 rf_reg[DPK_RF_REG_NUM_8197F] = {RF_0x0, RF_0x18, RF_0x8f};

	if (!((dm->rfe_type == 0) || (dm->rfe_type == 2) || (dm->rfe_type == 3))) {
		printk("[DPK] Skip DPK due to RFE type != 0/2/3\n");
		return;
	}
	RF_DBG(dm, DBG_RF_DPK, "[DPK] DPK Start (Ver: %s), Cv is %d\n",
	       DPK_VER_8197F, dm->cut_version);

	/*phy_path_a_dpk_init_8197f(dm);*/
	/*phy_path_b_dpk_init_8197f(dm);*/

	_backup_mac_bb_registers_8197f(dm, mac_reg, mac_reg_backup, DPK_MAC_REG_NUM_8197F);
	_backup_mac_bb_registers_8197f(dm, bb_reg, bb_reg_backup, DPK_BB_REG_NUM_8197F);
	_backup_rf_registers_8197f(dm, rf_reg, rf_reg_backup);

	_dpk_mac_bb_setting_8197f(dm);
	_dpk_rf_setting_8197f(dm);

	_dpk_result_reset_8197f(dm);
	_dpk_path_select_8197f(dm);
	_dpk_result_summary_8197f(dm);

	_reload_rf_registers_8197f(dm, rf_reg, rf_reg_backup);
	_reload_mac_bb_registers_8197f(dm, bb_reg, bb_reg_backup, DPK_BB_REG_NUM_8197F);
	_reload_mac_bb_registers_8197f(dm, mac_reg, mac_reg_backup, DPK_MAC_REG_NUM_8197F);
	...
	if (rfcal_info->is_dp_path_aok == 1 && rfcal_info->is_dp_path_bok == 1)
		...
		phy_dpk_enable_disable_8197f(dm, false);
	...
	struct dm_struct *dm)
{
	...
	odm_set_rf_reg(dm, RF_PATH_A, RF_T_METER_8197F, BIT(17) | BIT(16), 0x3); /*thermal meter trigger*/
	ODM_delay_ms(1);
	thermal_value = (u8)odm_get_rf_reg(dm, RF_PATH_A, RF_T_METER_8197F, 0xfc00); /*get thermal meter*/
	...
	k = phy_dpk_channel_transfer_8197f(dm);
