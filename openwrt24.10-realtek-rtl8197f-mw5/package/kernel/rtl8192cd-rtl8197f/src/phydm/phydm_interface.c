...
{
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	mw5_legacy_mod_timer(timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(ms_delay));
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE) && defined(DM_ODM_CE_MAC80211)
	mw5_legacy_mod_timer(timer, jiffies + msecs_to_jiffies(ms_delay));
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE) && defined(DM_ODM_CE_MAC80211_V2)
	mw5_legacy_mod_timer(&timer->timer, jiffies + msecs_to_jiffies(ms_delay));
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE)
			  ...
			  const char *sz_id)
{
	...
	_mw5_legacy_timer_init(timer, adapter->pnetdev, call_back_func, dm);
#elif (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	void *adapter = dm->adapter;
	...
	rtw_mw5_legacy_timer_init(timer, adapter->pnetdev, (TIMER_FUN)call_back_func, dm, NULL);
#endif
}
...
{
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	mw5_legacy_del_timer(timer);
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE) && defined(DM_ODM_CE_MAC80211)
	mw5_legacy_del_timer(timer);
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE) && defined(DM_ODM_CE_MAC80211_V2)
	mw5_legacy_del_timer(&timer->timer);
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE)
...
#elif (DM_ODM_SUPPORT_TYPE & ODM_IOT)
	rtw_mw5_legacy_del_timer(timer);
#endif
}
		      ...
		      u8 *cmd_buf)
{
	...
	u8 h2c_pkt[8];
#else
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN) || (DM_ODM_SUPPORT_TYPE & ODM_CE) || (DM_ODM_SUPPORT_TYPE & ODM_IOT)
	void *adapter = dm->adapter;
#endif
#endif
