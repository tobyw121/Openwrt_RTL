...
void ConstructIE(struct rtl8192cd_priv *priv, unsigned char *pucOut, int *usOutLen)
{
	...
	unsigned char peer_11w = (priv->pmib->dot11Bss.t_stamp[1]>>9)&3;
#endif
#ifdef AUTH_SAE_STA
		...
		*/
		...
		if (OPMODE & WIFI_STATION_STATE) {
			...
			if(pstat && (pmkid_cached(pstat))){
				...
				ulIELength += 2;	// add PMDID Count (2bytes)
			}
		} else {
			ulIELength += 2;	// add PMDID Count (2bytes)
		}
		...
		/*IGTK Suite ; Construct Group Management Cipher Suite*/
		{
			if ((priv->pmib->dot1180211AuthEntry.dot11IEEE80211W &&
				...
				ulIELength += sizeof(dot11RSNIGTKSuite);
			}
		}
#endif
		...
		if (OPMODE & WIFI_STATION_STATE) {
			if(pmkid_ie_len == sizeof(struct pmk_suite)){
				...
				pucBlob += sizeof(struct pmk_suite);
			}
			else {
				pucBlob += 2;  // add PMDID Count (2bytes)
			}
		} else {
			pucBlob += 2; // add PMDID Count (2bytes)
		}
...
static int rtl_min_lex(unsigned char *ucStr1, unsigned char *ucStr2, unsigned int ulLen)
{
	int i;
	for (i = 0 ; i < ulLen ; i++) {
				   ...
				   )
{
	...
	if (rtl_min_lex(addr1, addr2, ETHER_ADDRLEN) <= 0) {
	...
	if (rtl_min_lex(nonce1, nonce2, KEY_NONCE_LEN) <= 0) {
...
void general_wlan_IndicateEvent(struct rtl8192cd_priv *priv, unsigned char event, void *data)
{
	...
	switch (event)
	{
			...
			{
				DOT11_ROAMING_REPORT sta_info_report;
				struct stat_info *pstat = (struct stat_info *)data;
				memset(&sta_info_report, 0, sizeof(DOT11_ROAMING_REPORT));
				sta_info_report.IsMoreEvent =1;
				sta_info_report.EventId = event;
				sta_info_report.channel = priv->pmib->dot11RFEntry.dot11channel;
				sta_info_report.link_time = pstat->link_time;
				sta_info_report.rssi = pstat->rssi;
				memcpy(sta_info_report.hwaddr, pstat->cmn_info.mac_addr, MACADDRLEN);
				memcpy(mac, pstat->cmn_info.mac_addr, MACADDRLEN);
				...
				memcpy(data_ptr, &sta_info_report.EventId, 1);
				data_ptr += 1;
				len += 1;
				memcpy(data_ptr, &sta_info_report.IsMoreEvent, 1);
				data_ptr += 1;
				len += 1;
				memcpy(data_ptr, sta_info_report.hwaddr, MACADDRLEN);
				data_ptr += MACADDRLEN;
				len += MACADDRLEN;
				memcpy(data_ptr, &sta_info_report.link_time, sizeof(sta_info_report.link_time));
				data_ptr += sizeof(sta_info_report.link_time);
				len += sizeof(sta_info_report.link_time);
				memcpy(data_ptr, &sta_info_report.rssi, 1);
				data_ptr += 1;
				len += 1;
				memcpy(data_ptr, &sta_info_report.channel, 1);
				data_ptr += 1;
				len += 1;
				memcpy(data_ptr, &sta_info_report.BSS_transmit_support, 1);
				data_ptr += 1;
				len += 1;
				memcpy(data_ptr, &sta_info_report.beacon_report_support, 1);
				len += 1;
			...
			case DOT11_EVENT_ROAMING_BSS_TRANSMIT_RESP:
			{
				...
				bss_tramsnit_resp.IsMoreEvent =1;
				memcpy(bss_tramsnit_resp.hwaddr, tmp+1, MACADDRLEN);
				memcpy(mac, tmp+1, MACADDRLEN);
				...
				data_ptr = info;
				memcpy(data_ptr, &bss_tramsnit_resp.EventId, 1);
				data_ptr += 1;
				len += 1;
				memcpy(data_ptr, &bss_tramsnit_resp.IsMoreEvent, 1);
				data_ptr += 1;
				len += 1;
				memcpy(data_ptr, bss_tramsnit_resp.hwaddr, MACADDRLEN);
				data_ptr += MACADDRLEN;
				len += MACADDRLEN;
				memcpy(data_ptr, &bss_tramsnit_resp.resultCode, 1);
				len += 1;
			}
			break;
#endif
		...
		{   /*tmp = [mac + pvsie(eid + len + oui + value)]*/
			...
			memcpy(mac, tmp, MACADDRLEN);
			probe_vsie_record.data_len = tmp[MACADDRLEN+1];
			if(probe_vsie_record.data_len > sizeof(probe_vsie_record.ie_data)){
				DEBUG_ERR("ie_data out of size : %d\n", probe_vsie_record.data_len);
				return;
			}
			...
			data_ptr += 1;
			len += 1;
			memcpy(data_ptr, probe_vsie_record.hwaddr, MACADDRLEN);
			data_ptr += MACADDRLEN;
			len += MACADDRLEN;
	...
	rtk_wlan_with_info_event_indicate(priv->dev->name, eventid, mac, info, len);
}
#endif
...
static void reset_sta_info(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if (mw5_legacy_timer_pending(&pInfo->resendTimer)) {
		mw5_legacy_del_timer(&pInfo->resendTimer);
	}
	...
	if (pInfo->resendTimer.data == NULL) {		// don't init_timer twice, may need refined
		mw5_legacy_timer_init(&pInfo->resendTimer);
			...
			if (pstat->wpa_sta_info->state == PSK_STATE_PTKINITDONE &&
				...
				if (priv->wpa_global_info->GKeyDoneStations == 0 && !priv->wpa_global_info->GkeyReady) {
					...
					if (priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime)
						mw5_legacy_mod_timer(&priv->wpa_global_info->GKRekeyTimer, jiffies + RTL_SECONDS_TO_JIFFIES(priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime));
...
void GKRekeyTimeout(unsigned long task_priv)
{
...
#ifdef AUTH_SAE
		else if(pstat->is_sae_sta)
			EapolKeyMsgSend.Octet[2] = 0x80;
#endif
		else
...
//	pskb->cb[0] = 7;	// use highest priority to xmit
	...
	mw5_legacy_mod_timer(&pstat->wpa_sta_info->resendTimer, jiffies + RESEND_TIME);
}
...
#endif
			mw5_legacy_mod_timer(&pStaInfo->resendTimer, jiffies + RTL_SECONDS_TO_JIFFIES(5));
		} else
	#endif
			mw5_legacy_mod_timer(&pStaInfo->resendTimer, jiffies + RESEND_TIME);
	}
	//--------------------------------------------------------------send_packet
}
#endif // CLIENT_MODE
...
static void UpdateGK(struct rtl8192cd_priv *priv)
{
	...
	if (pGblInfo->GTKAuthenticator && (pGblInfo->GTKRekey || pGblInfo->GInitAKeys)) {
		...
		if (mw5_legacy_timer_pending(&pGblInfo->GKRekeyTimer))
			mw5_legacy_del_timer_sync(&pGblInfo->GKRekeyTimer);
...
check_msg2:
			...
			if (!CheckMIC(pStaInfo->EAPOLMsgRecvd, pStaInfo->PTK, PTK_LEN_EAPOLMIC)) {
				...
				if (GET_MAP_PROFILE(priv) >= MAP_PROFILE_TWO) {
					if (GET_ROOT(priv)->pmib->multi_ap.multiap_report_fail_assoc) {
						rtk_multi_ap_trigger_fail_connection_message(pstat->cmn_info.mac_addr, _STATS_FAILURE_, RSN_MIC_failure);
					}
				}
#endif
...
cont_msg:
			...
			// delete resend timer
			SMP_LOCK_PSK_RESEND(flags);
			if (mw5_legacy_timer_pending(&pStaInfo->resendTimer)){
				mw5_legacy_del_timer(&pStaInfo->resendTimer);
			}
			SMP_UNLOCK_PSK_RESEND(flags);
...
//				global->portSecure = TRUE;
				...
				if (pGblInfo->GKeyDoneStations == 0 && !pGblInfo->GkeyReady) {
					...
					// start groupkey rekey timer
					if (priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime)
						mw5_legacy_mod_timer(&pGblInfo->GKRekeyTimer, jiffies + RTL_SECONDS_TO_JIFFIES(priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime));
				}
#if 0
				if (global->RSNVariable.PMKCached) {
				...
				#ifdef AUTH_SAE
				log("WPA2: 4-way handshake done");

				unsigned char pmkid_cache_idx=0;
				struct candidate *peer = NULL;

				pmkid_cache_idx = search_by_mac_pmkid_cache(priv, pstat->cmn_info.mac_addr);
				if(pmkid_cache_idx<NUM_PMKID_CACHE && pstat->is_sae_sta==1){
					peer =
			    		create_candidate(priv, pstat->cmn_info.mac_addr, GET_MY_HWADDR, (priv->curr_open >= priv->open_threshold));
					if (peer == NULL) {
						log("create_candidate for peer[%pM] fail ", pstat->cmn_info.mac_addr);
						return -1;
					}
					peer->t0 = 0;
					peer->t1 = jiffies;
					log("add_peer[%pm]",pstat->cmn_info.mac_addr);
				}
				#endif
...
#if defined(CONFIG_PE_ENABLE) && defined(PE_INFO_SYNC)
...
#ifdef MULTI_MAC_CLONE								
								if(priv->pmib->ethBrExtInfo.macclone_enable && ACTIVE_ID > 0 && priv->pshare->mclone_sta[ACTIVE_ID-1].usedStaAddrId != 0xff)
									toSetKey = 0; //avoid clone mac 4-way handshake fail
								else
									return; 
#else
								return;
...
error_reconn:
	if (error_code) {
		...
		DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, "issue_deauth - %02X:%02X:%02X:%02X:%02X:%02X\n",
								pstat->cmn_info.mac_addr[0],pstat->cmn_info.mac_addr[1],pstat->cmn_info.mac_addr[2],pstat->cmn_info.mac_addr[3],pstat->cmn_info.mac_addr[4],pstat->cmn_info.mac_addr[5]);
		delay_ms(100);//delay send deauth
...
#endif
		delay_ms(50);//delay ss after issue deauth
		...
		if (OPMODE & WIFI_AP_STATE) {
			...
			if (len > 2 && msg != NULL) {
				...
				if (isWPA2){
					...
					wpa2_ie_check(priv,tmpbuf,len+2 , &pmkid_ptr , &pmkid_cnt);
			...
			mw5_legacy_mod_timer(&pstat->wpa_sta_info->resendTimer, jiffies + WAIT_EAP_TIME);
#endif
		break;
...
void choose_cipher(struct rtl8192cd_priv *priv, struct bss_desc *bss_target)
{
	...
	if ( bss_target->capability & BIT4 ) {
		...
		if (support_wpa3(priv) && peer_support_wpa3(priv,t_stamp1)) {
			log("both DUT & peer support WPA3");
...
int ft_check_ft_reassocrsp(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pbuf, unsigned int limit, unsigned int *status)
{
	...
	pGblInfo->GN = keyID;
	memcpy(tmp, p, MIN(gtk_ie_len,sizeof(tmp)));
