...
void stactrl_init(struct rtl8192cd_priv *priv)
{
    ...
    mw5_legacy_mod_timer(&priv_sc->stactrl.stactrl_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(STACTRL_NON_PREFER_TIMER));    
#endif
...
#if STA_CONTROL_ALGO == STA_CONTROL_ALGO2
        mw5_legacy_del_timer_sync(&priv_sc->stactrl.stactrl_timer);
#endif
    }
}
...
#if STA_CONTROL_ALGO == STA_CONTROL_ALGO2
    mw5_legacy_mod_timer(&priv->stactrl.stactrl_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(STACTRL_NON_PREFER_TIMER));
#endif
}
...
static void stactrl_band_transition_detection_non_prefer_band(struct rtl8192cd_priv *priv)
{
	...
	plist = phead;
	while (!list_empty(plist) && ((plist = asoc_list_get_next(priv, plist)) != phead) && (num > 0))
	{
		pstat = list_entry(plist, struct stat_info, asoc_list);
