...
void update_bbrf_val8812(struct rtl8192cd_priv *priv, unsigned char channel)
{
	...
	/*Set 0x454( 	REG_CCK_CHECK) start*/	
	if (channel > 14 && (switch_bw || (priv->pshare->pre_channel == 0))){ //2.4G to 5G
		RTL_W8(0x454, (RTL_R8(0x454) | BIT(7)));
	}

	//Set fc_area 0x860 , bit(17~27)
	if (channel <= 14){
		phy_set_bb_reg(priv, rFc_area_Jaguar, 0x1ffe0000, 0x96a);		
	}else if ( 36<= channel && channel <= 48){
		phy_set_bb_reg(priv, rFc_area_Jaguar, 0x1ffe0000, 0x494); 	
	}else if (52<= channel  && channel <= 64){
		phy_set_bb_reg(priv, rFc_area_Jaguar, 0x1ffe0000, 0x453);  	
	}else if (100 <= channel  && channel <= 116){
		phy_set_bb_reg(priv, rFc_area_Jaguar, 0x1ffe0000, 0x452);  		
	}else if (118 <= channel && channel <= 177){
		phy_set_bb_reg(priv, rFc_area_Jaguar, 0x1ffe0000, 0x412);  			
	}
	...
	/*
	CH36~64 						CH100~140						CH>140
	RF_A reg18[18,17,16,9,8]=4'b00101	RF_A reg18[18,17,16,9,8]=4'b01101	RF_A reg18[18,17,16,9,8]=4'b10101
	RF_B reg18[18,17,16,9,8]=4'b00101	RF_B reg18[18,17,16,9,8]=4'b01101	RF_B reg18[18,17,16,9,8]=4'b10101
	*/		
	for (eRFPath = RF_PATH_A; eRFPath < curMaxRFPath; eRFPath++) {

		if (channel <= 14){
			phy_set_rf_reg(priv, eRFPath, RF_CHNLBW_Jaguar, BIT18|BIT17|BIT16|BIT9|BIT8, 0x000); //5'b00000); 			
		}else if (channel >= 36 && channel <= 64){
			phy_set_rf_reg(priv, eRFPath, RF_CHNLBW_Jaguar, BIT18|BIT17|BIT16|BIT9|BIT8, 0x101); //5'b00101); 					
		}else if (channel >= 100 && channel <= 140){
			phy_set_rf_reg(priv, eRFPath, RF_CHNLBW_Jaguar, BIT18|BIT17|BIT16|BIT9|BIT8, 0x301); //5'b01101); 			
		}else if (channel >= 140){
			phy_set_rf_reg(priv, eRFPath, RF_CHNLBW_Jaguar, BIT18|BIT17|BIT16|BIT9|BIT8, 0x501); //5'b10101); 		
		}else{
			panic_printk("[%s]unknow CHK!!!\n",__func__);
		}
	}
	...
	if (switch_bw) {
		...
		if (!current_is_5g) {
			...
			/*current is 2.4 and going to 5G*/



			phy_set_bb_reg(priv, 0x82c, (BIT(0) | BIT(1)), 1);					

			if (GET_CHIP_VER(priv) == VERSION_8881A) {
				phy_set_bb_reg(priv, 0xcb0, 0xff, 0x55);
				phy_set_bb_reg(priv, 0xcb0, 0xf000, 0x1);
			} else {
				if (IS_TEST_CHIP(priv)) {
					...
					if (priv->pshare->rf_ft_var.use_ext_pa) 
					{
						...
						if (priv->pmib->dot11RFEntry.pa_type == PA_RFMD_4501 || priv->pmib->dot11RFEntry.pa_type == PA_SKYWORKS_85712_HP) {
							phy_set_bb_reg(priv, 0xcb4, 0x3ff00000, 0x00);	
							phy_set_bb_reg(priv, 0xeb4, 0x3ff00000, 0x00);										
						} else {
							phy_set_bb_reg(priv, 0xcb4, 0x3ff00000, 0x10);	
							phy_set_bb_reg(priv, 0xeb4, 0x3ff00000, 0x10);					
						}					
	#endif		
					}
					else
					...
					// internal PA+PAPE
						...
						#endif
						{
							/*RFE Select , when 2G to 5G refill here, orig value is config form phy parameter,PW*/
							RTL_W32(0xcb0, 0x77547777);  /*PATHA*/ 
							RTL_W32(0xeb0, 0x77547777);  /*PATHB*/ 
			...
			/*r_ENABLE_CCK , let CCK always on , BIT(28) 1 enbable;0 disabled*/
			//RTL_W32(0x808, RTL_R32(0x808) | BIT(28));

						
			/*0x834 ; 5G 0x834[1:0]=0x2*/
			phy_set_bb_reg(priv, 0x834, (BIT(0) | BIT(1)), 2);			
			

			/*Antenna Config ,CCK Sel [3:0]0xf cck off*/			
			RTL_W8(0xa07, RTL_R8(0xa07) | 0xf);
			
			/* dis_vht_auto_detect , PS: 1: disable; 0: enable*/			
			RTL_W32(0x8c0, RTL_R32(0x8c0) & (~BIT(17)));
	
			
			/*0x830; 5G 0x830[17:13]=5'b10101*/
			phy_set_bb_reg(priv, 0x830, BIT(17)|BIT(16)|BIT(15)|BIT(14)|BIT(13), 0x15); 

			/*END of current is 2.4 and going to 5G*/
			
		}
		else {    		

			/*current is 5G and going to 2.4G*/



			/*0x82c  ,2G AGC table select	0x82c[1:0]=2'b00*/
			phy_set_bb_reg(priv, 0x82c, (BIT(0) | BIT(1)), 0);

			/*0x834 ; 2G 0x834[1:0]=0x1*/
			phy_set_bb_reg(priv, 0x834, (BIT(0) | BIT(1)), 1);
			...
			/*Antenna Config ,CCK Sel [3:2]cck1 [1:0]cck2*/
			RTL_W8(0xa07, (RTL_R8(0xa07) & 0xf0)|0x01);
			
			/* dis_vht_auto_detect , PS: 1: disable; 0: enable*/			
			RTL_W32(0x8c0, RTL_R32(0x8c0) | BIT(17));
			
			/*0x830; 2G 0x830[17:13]=5'b10111*/
			phy_set_bb_reg(priv, 0x830, BIT(17)|BIT(16)|BIT(15)|BIT(14)|BIT(13), 0x17);

			/*END of current is 5G and going to 2.4G*/
		...
		if (channel > 14) {
			...
			/*0x82c  ,5G AGC table select	0x82c[1:0]=2'b01*/
			phy_set_bb_reg(priv, 0x82c, (BIT(0) | BIT(1)), 1);					
			...
			/*r_ENABLE_CCK , if has band switch 2->5 don't disable CCK , BIT(28) 1 enbable;0 disabled*/
			//RTL_W32(0x808, RTL_R32(0x808) | BIT(28));
			...
			if (GET_CHIP_VER(priv) == VERSION_8881A) {
				phy_set_bb_reg(priv, 0xcb0, 0xff, 0x00);
				phy_set_bb_reg(priv, 0xcb0, 0xf000, 0x0);
			} else {
...
void SpurCheck8812(struct rtl8192cd_priv *priv, unsigned char ch_bw, unsigned char channel, unsigned int bandwidth)
{
	...
	if(ch_bw == 1 && (GET_CHIP_VER(priv) != VERSION_8723B)){ /* if switch channel */
		if((priv->pshare->CurrentChannelBW == CHANNEL_WIDTH_40 && (channel == 11 || channel == 12)) ||
		   (priv->pshare->CurrentChannelBW == CHANNEL_WIDTH_20 && (channel == 13 || channel == 14))){
			...
			}else if((priv->pshare->CurrentChannelBW == CHANNEL_WIDTH_40 && (priv->pshare->pre_channel == 11 || priv->pshare->pre_channel == 12)) ||
				   (priv->pshare->CurrentChannelBW == CHANNEL_WIDTH_20 && (priv->pshare->pre_channel == 13 || priv->pshare->pre_channel == 14))){

				/* Restore 0x8ac, 0x8c4*/
				phy_set_bb_reg(priv, 0x8ac, BIT(10), 0);	
				RTL_W32(0X8c4,0);
				...
				if (IS_C_CUT_8812(priv)) {
					...
					{
					case CHANNEL_WIDTH_5:
						phy_set_bb_reg(priv, 0x8c4, BIT(30), 0);	
						break;
					case CHANNEL_WIDTH_10:
						phy_set_bb_reg(priv, 0x8c4, BIT(30), 0);				
						break;
		...
		if((bandwidth == CHANNEL_WIDTH_20 && (priv->pmib->dot11RFEntry.dot11channel == 13 || priv->pmib->dot11RFEntry.dot11channel == 14)) || 
			(bandwidth == CHANNEL_WIDTH_40 && (priv->pmib->dot11RFEntry.dot11channel == 11 || priv->pmib->dot11RFEntry.dot11channel == 12))
			){
			RTL_W32(0x8ac,0x0ff0ff04);
			RTL_W32(0X8c4,0x40000000);
			if(priv->pshare->CurrentChannelBW == CHANNEL_WIDTH_40){
...
void Write_2S_A(struct rtl8192cd_priv *priv, unsigned int writeVal)
{
	...
	writeVal = (writeVal & 0xffff0000) | (RTL_R32(REG_TX_AGC_A_NSS2_INDEX1_NSS1_INDEX8_JAGUAR) & 0xffff);
	RTL_W32(REG_TX_AGC_A_NSS2_INDEX1_NSS1_INDEX8_JAGUAR, writeVal);
}
...
void use_DefaultOFDMTxPower_8812(struct rtl8192cd_priv *priv)
{
	...
	RTL_W32(REG_TX_AGC_A_OFDM18_OFDM6_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_OFDM54_OFDM24_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_MCS3_MCS0_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_MCS7_MCS4_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_MCS11_MCS8_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_MCS15_MCS12_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_NSS1_INDEX3_NSS1_INDEX0_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_NSS1_INDEX7_NSS1_INDEX4_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_NSS2_INDEX1_NSS1_INDEX8_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_NSS2_INDEX5_NSS2_INDEX2_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_A_NSS2_INDEX9_NSS2_INDEX6_JAGUAR, def_power);

	RTL_W32(REG_TX_AGC_B_OFDM18_OFDM6_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_OFDM54_OFDM24_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_MCS3_MCS0_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_MCS7_MCS4_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_MCS11_MCS8_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_MCS15_MCS12_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_NSS1_INDEX3_NSS1_INDEX0_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_NSS1_INDEX7_NSS1_INDEX4_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_NSS2_INDEX1_NSS1_INDEX8_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_NSS2_INDEX5_NSS2_INDEX2_JAGUAR, def_power);
	RTL_W32(REG_TX_AGC_B_NSS2_INDEX9_NSS2_INDEX6_JAGUAR, def_power);
...
void TxPG_CCK_8812(struct rtl8192cd_priv *priv)
{
	...
	tmp_dw = RTL_R32(REG_TX_AGC_A_CCK_11_CCK_1_JAGUAR);
	...
	*tmp_b4 = (unsigned char)(POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->CCKTxAgc_A[0]));
	...
	RTL_W32(REG_TX_AGC_A_CCK_11_CCK_1_JAGUAR, writeVal);

	tmp_dw = RTL_R32(REG_TX_AGC_B_CCK_11_CCK_1_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = (unsigned char)(POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->CCKTxAgc_B[0]));
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_CCK_11_CCK_1_JAGUAR, writeVal);
}
...
void TxPG_OFDM_8812_A(struct rtl8192cd_priv *priv)
{
	...
	tmp_dw = RTL_R32(REG_TX_AGC_A_OFDM18_OFDM6_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->OFDMTxAgcOffset_A[0]);
	...
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_A_OFDM18_OFDM6_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_A_OFDM54_OFDM24_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->OFDMTxAgcOffset_A[4]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_A_OFDM54_OFDM24_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_A_MCS3_MCS0_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->MCSTxAgcOffset_A[0]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_A_MCS3_MCS0_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_A_MCS7_MCS4_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->MCSTxAgcOffset_A[4]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_A_MCS7_MCS4_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_A_MCS11_MCS8_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->MCSTxAgcOffset_A[8]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_A_MCS11_MCS8_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_A_MCS15_MCS12_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->MCSTxAgcOffset_A[12]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_A_MCS15_MCS12_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_A_NSS1_INDEX3_NSS1_INDEX0_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->VHTTxAgcOffset_A[0]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_A_NSS1_INDEX3_NSS1_INDEX0_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_A_NSS1_INDEX7_NSS1_INDEX4_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->VHTTxAgcOffset_A[4]);
	...
	tmp_dw = RTL_R32(REG_TX_AGC_A_NSS2_INDEX1_NSS1_INDEX8_JAGUAR);
	tmp_dw1= RTL_R32(REG_TX_AGC_A_NSS1_INDEX7_NSS1_INDEX4_JAGUAR);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->VHTTxAgcOffset_A[8]);
	...
	tmp_dw = RTL_R32(REG_TX_AGC_A_NSS2_INDEX5_NSS2_INDEX2_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->VHTTxAgcOffset_A[12]);
	...
	tmp_dw = RTL_R32(REG_TX_AGC_A_NSS2_INDEX9_NSS2_INDEX6_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
...
void TxPG_OFDM_8812_B(struct rtl8192cd_priv *priv)
{
	...
	tmp_dw = RTL_R32(REG_TX_AGC_B_OFDM18_OFDM6_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->OFDMTxAgcOffset_B[0]);
	...
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_OFDM18_OFDM6_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_OFDM54_OFDM24_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->OFDMTxAgcOffset_B[4]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_OFDM54_OFDM24_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_MCS3_MCS0_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->MCSTxAgcOffset_B[0]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_MCS3_MCS0_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_MCS7_MCS4_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->MCSTxAgcOffset_B[4]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_MCS7_MCS4_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_MCS11_MCS8_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->MCSTxAgcOffset_B[8]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_MCS11_MCS8_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_MCS15_MCS12_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->MCSTxAgcOffset_B[12]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_MCS15_MCS12_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_NSS1_INDEX3_NSS1_INDEX0_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->VHTTxAgcOffset_B[0]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_NSS1_INDEX3_NSS1_INDEX0_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_NSS1_INDEX7_NSS1_INDEX4_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->VHTTxAgcOffset_B[4]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_NSS1_INDEX7_NSS1_INDEX4_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_NSS2_INDEX1_NSS1_INDEX8_JAGUAR);
	tmp_dw1= RTL_R32(REG_TX_AGC_B_NSS1_INDEX7_NSS1_INDEX4_JAGUAR);
	get_tx_pwr(tmp_dw1, tmp_b1, tmp_b2, tmp_b3, tmp_b5);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->VHTTxAgcOffset_B[8]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_NSS2_INDEX1_NSS1_INDEX8_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_NSS2_INDEX5_NSS2_INDEX2_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
	...
	*tmp_b4 = POWER_RANGE_CHECK(*tmp_b4 + priv->pshare->phw->VHTTxAgcOffset_B[12]);
	writeVal = (*tmp_b4 << 24) | (*tmp_b3 << 16) | (*tmp_b2 << 8) | *tmp_b1;
	RTL_W32(REG_TX_AGC_B_NSS2_INDEX5_NSS2_INDEX2_JAGUAR, writeVal);
	tmp_dw = RTL_R32(REG_TX_AGC_B_NSS2_INDEX9_NSS2_INDEX6_JAGUAR);
	get_tx_pwr(tmp_dw, tmp_b1, tmp_b2, tmp_b3, tmp_b4);
...
unsigned int assign_lmt_reg_value(struct rtl8192cd_priv *priv, unsigned int reg_offset, unsigned char max_idx, unsigned char max_idx2)
{
	...
	if((reg_offset == REG_TX_AGC_A_NSS2_INDEX1_NSS1_INDEX8_JAGUAR) ||
		(reg_offset == REG_TX_AGC_B_NSS2_INDEX1_NSS1_INDEX8_JAGUAR))
	{
...
void TxLMT_CCK_8812_A(struct rtl8192cd_priv *priv)
{
	...
	assign_lmt_reg_value(priv, REG_TX_AGC_A_CCK_11_CCK_1_JAGUAR, max_lmt_idx_cck, 0);
}
...
void TxLMT_CCK_8812_B(struct rtl8192cd_priv *priv)
{
	...
	assign_lmt_reg_value(priv, REG_TX_AGC_B_CCK_11_CCK_1_JAGUAR, max_lmt_idx_cck, 0);
}
...
void TxLMT_OFDM_8812_A(struct rtl8192cd_priv *priv)
{
	...
	assign_lmt_reg_value(priv, REG_TX_AGC_A_OFDM18_OFDM6_JAGUAR, max_lmt_idx_ofdm, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_A_OFDM54_OFDM24_JAGUAR, max_lmt_idx_ofdm, 0);
	
	assign_lmt_reg_value(priv, REG_TX_AGC_A_MCS3_MCS0_JAGUAR, max_lmt_idx_ht1s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_A_MCS7_MCS4_JAGUAR, max_lmt_idx_ht1s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_A_MCS11_MCS8_JAGUAR, max_lmt_idx_ht2s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_A_MCS15_MCS12_JAGUAR, max_lmt_idx_ht2s, 0);

	assign_lmt_reg_value(priv, REG_TX_AGC_A_NSS1_INDEX3_NSS1_INDEX0_JAGUAR, max_lmt_idx_vht1s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_A_NSS1_INDEX7_NSS1_INDEX4_JAGUAR, max_lmt_idx_vht1s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_A_NSS2_INDEX1_NSS1_INDEX8_JAGUAR, max_lmt_idx_vht1s, max_lmt_idx_vht2s);
	assign_lmt_reg_value(priv, REG_TX_AGC_A_NSS2_INDEX5_NSS2_INDEX2_JAGUAR, max_lmt_idx_vht2s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_A_NSS2_INDEX9_NSS2_INDEX6_JAGUAR, max_lmt_idx_vht2s, 0);
...
void TxLMT_OFDM_8812_B(struct rtl8192cd_priv *priv)
{
	...
	assign_lmt_reg_value(priv, REG_TX_AGC_B_OFDM18_OFDM6_JAGUAR, max_lmt_idx_ofdm, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_B_OFDM54_OFDM24_JAGUAR, max_lmt_idx_ofdm, 0);
	
	assign_lmt_reg_value(priv, REG_TX_AGC_B_MCS3_MCS0_JAGUAR, max_lmt_idx_ht1s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_B_MCS7_MCS4_JAGUAR, max_lmt_idx_ht1s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_B_MCS11_MCS8_JAGUAR, max_lmt_idx_ht2s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_B_MCS15_MCS12_JAGUAR, max_lmt_idx_ht2s, 0);

	assign_lmt_reg_value(priv, REG_TX_AGC_B_NSS1_INDEX3_NSS1_INDEX0_JAGUAR, max_lmt_idx_vht1s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_B_NSS1_INDEX7_NSS1_INDEX4_JAGUAR, max_lmt_idx_vht1s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_B_NSS2_INDEX1_NSS1_INDEX8_JAGUAR, max_lmt_idx_vht1s, max_lmt_idx_vht2s);
	assign_lmt_reg_value(priv, REG_TX_AGC_B_NSS2_INDEX5_NSS2_INDEX2_JAGUAR, max_lmt_idx_vht2s, 0);
	assign_lmt_reg_value(priv, REG_TX_AGC_B_NSS2_INDEX9_NSS2_INDEX6_JAGUAR, max_lmt_idx_vht2s, 0);
...
#endif
	{
		int ofdm6a = RTL_R32(REG_TX_AGC_A_OFDM18_OFDM6_JAGUAR) & 0x3f;
		int ofdm6b = RTL_R32(REG_TX_AGC_B_OFDM18_OFDM6_JAGUAR) & 0x3f;
		if(ofdm6a>priv->pshare->rf_ft_var.bcn_pwr_max) {
			ofdm6a = priv->pshare->rf_ft_var.bcn_pwr_max;
			RTL_W8(REG_TX_AGC_A_OFDM18_OFDM6_JAGUAR, ofdm6a);
		}
		if(ofdm6b>priv->pshare->rf_ft_var.bcn_pwr_max) {
			ofdm6b = priv->pshare->rf_ft_var.bcn_pwr_max;
			RTL_W8(REG_TX_AGC_B_OFDM18_OFDM6_JAGUAR, ofdm6b);
		}				
...
void PHY_SetCCKTxPower_8812(struct rtl8192cd_priv *priv, unsigned char channel)
{
	...
	if ((priv->pmib->dot11RFEntry.pwrlevelCCK_A[ch_idx] == 0)
		...
		RTL_W32(REG_TX_AGC_A_CCK_11_CCK_1_JAGUAR, def_power);
		RTL_W32(REG_TX_AGC_B_CCK_11_CCK_1_JAGUAR, def_power);
		return;
	}
...
// PCI/SDIO------use 4-Byte/1-Byte  (platform_efio_write_4byte)
...
{
	enum rt_status		rtStatus = RT_STATUS_SUCCESS;
...
)
{
	...
	BOOLEAN	bShortGI20MHz = FALSE, bShortGI40MHz = FALSE, bShortGI80MHz = FALSE, bShortGI160MHz = FALSE;
	...
	if (	(WirelessMode & WIRELESS_MODE_N_24G) ||
		...
		if ((WirelessMode & WIRELESS_MODE_AC_5G) && (GET_CHIP_VER(priv) != VERSION_8814B)) {
			if ( (cpu_to_le32(pEntry->vht_cap_buf.vht_cap_info) & BIT(SHORT_GI160M_E))
				&& priv->pmib->dot11nConfigEntry.dot11nShortGIfor160M)
			bShortGI160MHz = TRUE;		
		}
	#endif
	}
...
//			bCurTxBW40MHz = TRUE;
	...
	pstat->cmn_info.ra_info.ramask = ratr_bitmap | (ratr_bitmap_msb << 32);
	...
	pstat->cmn_info.ra_info.rate_id = MRateIdxToARFRId8812(priv, ratr_index, rf_mimo_mode) ;
	pstat->cmn_info.bw_mode = BW;
	pstat->cmn_info.ra_info.ra_bw_mode = BW;
	...
	fill_h2c_cmd8812(priv, H2C_8812_RA_MASK, 7, H2CCommand);

	SetBcnCtrlReg_8812(priv, BIT3, 0);
	/*
		panic_printk("UpdateHalRAMask8812E(): bitmap = %x ratr_index = %1x, MacID:%x, ShortGI:%x, MimoPs=%d\n",
			ratr_bitmap, pstat->cmn_info.ra_info.rate_id,  (pstat->cmn_info.aid), bShortGI, MimoPs);
...
)
{
	...
	H2CCommand[2] = 0;
	fill_h2c_cmd8812(priv, H2C_8812_MSRRPT, 3, H2CCommand);
...
void odm_TXPowerTrackingCallback_ThermalMeter_8812E(struct rtl8192cd_priv * priv)
{
	...
	if (priv->pshare->Power_tracking_on_8812 == 0) {
		...
		ThermalValue = (unsigned char)phy_query_rf_reg(priv, RF_PATH_A, 0x42, 0xfc00, 1);	//0x42: RF Reg[15:10] 88E
	...
	{
		priv->pshare->thermal_value_avg_8812[priv->pshare->thermal_value_avg_index_8812] = ThermalValue;
		...
		for (i = 0; i < AVG_THERMAL_NUM_8812; i++) {
			if (priv->pshare->thermal_value_avg_8812[i]) {
				ThermalValue_AVG += priv->pshare->thermal_value_avg_8812[i];
	...
	if (ThermalValue != priv->pshare->thermal_value) {
		...
		#ifdef CONFIG_RTL_8812_SUPPORT
			if(GET_CHIP_VER(priv) != VERSION_8723B) {
				phy_set_bb_reg(priv, 0xc1c, 0xffe00000, ofdm_swing_table_8812[(unsigned int)OFDM_index[0]]);
				if (rf_mimo_mode == RF_2T2R)
					phy_set_bb_reg(priv, 0xe1c, 0xffe00000, ofdm_swing_table_8812[(unsigned int)OFDM_index[1]]);
			}
		#endif

		}
/*
		printk("PathA >>>>> 0xc1c[31:21] = 0x%x, OFDM_index:%d\n", phy_query_bb_reg(priv, 0xc1c, 0xffe00000), OFDM_index[0]);
		if (rf_mimo_mode == RF_2T2R)
			printk("PathB >>>>> 0xe1c[31:21] = 0x%x, OFDM_index:%d\n", phy_query_bb_reg(priv, 0xe1c, 0xffe00000), OFDM_index[1]);
		printk("\n******** END:%s() ********\n", __FUNCTION__);
*/
...
void requestTxRetry_8812(struct rtl8192cd_priv *priv)
{
	...
	h2cresult = fill_h2c_cmd8812(priv, H2C_8812_TX_REPORT, 3 , H2CCommand);
	//WDEBUG("h2cresult=%d\n",h2cresult);
	...
	)
{
	...
	enum _BEAMFORMING_CAP		BeamformCap = BEAMFORMING_CAP_NONE;
	...
	if(get_rf_mimo_mode(priv) == RF_1T1R)
		return;
	phy_set_rf_reg(priv, RF_PATH_A, rf_welut_jaguar, 0x80000,0x1); // RF Mode table write enable
	phy_set_rf_reg(priv, RF_PATH_B, rf_welut_jaguar, 0x80000,0x1); // RF Mode table write enable
	...
	else if (!priv->pmib->dot11RFEntry.tx2path)
	{
		phy_set_rf_reg(priv, RF_PATH_A, rf_mode_table_addr, 0x78000, 0x3); // Select RX mode
		phy_set_rf_reg(priv, RF_PATH_A, rf_mode_table_data0, 0xfffff,0x3F7FF); // Set Table data
		phy_set_rf_reg(priv, RF_PATH_A, rf_mode_table_data1, 0xfffff,0xC26BF); // Disable TXIQGEN in RX mode
		phy_set_rf_reg(priv, RF_PATH_B, rf_mode_table_addr, 0x78000, 0x3); // Select RX mode
		phy_set_rf_reg(priv, RF_PATH_B, rf_mode_table_data0, 0xfffff,0x3F7FF); // Set Table data
		phy_set_rf_reg(priv, RF_PATH_B, rf_mode_table_data1, 0xfffff,0xC26BF); // Disable TXIQGEN in RX mode
	}
	phy_set_rf_reg(priv, RF_PATH_A, rf_welut_jaguar, 0x80000,0x0); // RF Mode table write disable
	phy_set_rf_reg(priv, RF_PATH_B, rf_welut_jaguar, 0x80000,0x0); // RF Mode table write disable
	
	if(bSelfBeamformer) {
		phy_set_bb_reg(priv, REG_TX_PATH_JAGUAR, bMaskByte1, 0x33);
		...
		phy_set_bb_reg(priv, REG_TX_PATH_JAGUAR, bMaskByte1, 0x11);
#ifdef RF_MIMO_SWITCH		
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
			priv->pshare->rf_phy_bb_backup[1] &= ~ bMaskByte1;
	...
	)
{
	...
	struct _RT_BEAMFORMER_ENTRY	BeamformerEntry;
	u2Byte					STAid = 0;
	...
	)
{
    ...
    struct _RT_BEAMFORMER_ENTRY	    BeamformerEntry = pBeamformingInfo->BeamformerEntry[Idx];
	...
	*/
	...
	if(Idx == 0)
	{	
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, BeamformeeEntryCap == NONE, Idx=%d\n", __FUNCTION__, Idx);
    ...
    if (BeamformerEntry.BeamformEntryCap == BEAMFORMING_CAP_NONE) 
    {
        if(Idx == 0)
        {
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, BeamformerEntryCap == NONE, Idx=%d\n", __FUNCTION__, Idx);
    ...
    if(((pBeamformingInfo->BeamformerEntry[0]).BeamformEntryCap == BEAMFORMING_CAP_NONE)
		...
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, All BeamformerEntryCap == NONE, STOP feedback CSI\n", __FUNCTION__, Idx);
...
#if defined(DFS) || defined(RTK_AC_SUPPORT) || defined(CSA2G) /*for 11ac logo*/
...
//#endif
	...
	phy_set_bb_reg(priv, 0x924, BIT(15), 0); // disable HW DFS report
#ifdef DFS
 	if (!priv->pmib->dot11DFSEntry.disable_DFS &&
		(OPMODE & WIFI_AP_STATE)) {
		if (mw5_legacy_timer_pending(&priv->DFS_timer))
			mw5_legacy_del_timer(&priv->DFS_timer);

		if (mw5_legacy_timer_pending(&priv->ch_avail_chk_timer))
		{
			mw5_legacy_del_timer(&priv->ch_avail_chk_timer);
		...
		if (mw5_legacy_timer_pending(&priv->dfs_det_chk_timer))
			mw5_legacy_del_timer(&priv->dfs_det_chk_timer);
 	}
#endif
		...
		priv->pmib->dot11RFEntry.band5GSelected == PHY_BAND_5G_3) 
	{
		...
		if (!is80MChannel(priv->available_chnl, priv->available_chnl_num, channel)) {
			priv->pshare->CurrentChannelBW = priv->pshare->is_40m_bw = CHANNEL_WIDTH_20;
		}
	}
#ifdef DFS
