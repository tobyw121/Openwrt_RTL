...
extern struct mw5_legacy_timer	LED_TimerCFGING;
#endif
...
Success : 0
------------------------------------------------------------*/
static int DOT11_Process_Set_RSNIE(struct net_device *dev, DOT11_SET_RSNIE *Set_RSNIE)
{
	...
	if(Set_RSNIE->Flag == DOT11_Ioctl_Set)
	{
		...
		/*AX1500(NL80211) directly use hostapd's RSN IE*/
		#else
		pdot11RsnIE->rsnielen = Set_RSNIE->RSNIELen;
		if (pdot11RsnIE->rsnielen <= sizeof(pdot11RsnIE->rsnie))
...
//for openwrt 8021x donot trigger site survey when rsnie init in init sw
			...
			if ((OPMODE & (WIFI_STATION_STATE | WIFI_ADHOC_STATE)) && IS_DRV_OPEN(priv))
	...
	/*AUTH_SAE   just for debug*/
	//dump_hex("RSN IE",pdot11RsnIE->rsnie,pdot11RsnIE->rsnielen);
	#ifdef WPA3_STA_DEBUG
	if( pdot11RsnIE->rsnie[0]== _RSN_IE_2_ ){
		log("my ie => wpa2_ie_check");
		wpa2_ie_check(priv,pdot11RsnIE->rsnie,pdot11RsnIE->rsnielen);
	}
	#endif
	return 0;
}
...
Success : 0
---------------------------------------------------*/
static int DOT11_Process_Association_Rsp(struct net_device *dev, DOT11_ASSOCIATIIN_RSP *Assoc_Rsp, int frame_type)
{
#ifndef SMP_SYNC
	unsigned long		flags;
#endif
	...
	issue_asocrsp(priv, Assoc_Rsp->Status, pstat, frame_type, 0);
...
static inline
#endif
int __DOT11_Process_Disconnect_Req(struct net_device *dev, DOT11_DISCONNECT_REQ	*Disconnect_Req)
{
#ifndef SMP_SYNC
	unsigned long	flags;
#endif
	...
	if(OPMODE & WIFI_STATION_STATE){
		...
		{
			DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, "issue_deauth - %02X:%02X:%02X:%02X:%02X:%02X, Reason: _RSON_DEAUTH_STA_LEAVING_\n",
								hwaddr[0],hwaddr[1],hwaddr[2],hwaddr[3],hwaddr[4],hwaddr[5]);
...
#endif
	{
		DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, "A STA is rejected - %02X:%02X:%02X:%02X:%02X:%02X, Reason=%d\n",
								pstat->cmn_info.mac_addr[0],pstat->cmn_info.mac_addr[1],pstat->cmn_info.mac_addr[2],pstat->cmn_info.mac_addr[3],pstat->cmn_info.mac_addr[4],pstat->cmn_info.mac_addr[5], Disconnect_Req->Reason);
		issue_deauth(priv, pstat->cmn_info.mac_addr, Disconnect_Req->Reason);
	}
	...
	if (asoc_list_del(priv, pstat))
	{
		...
		if (pstat->expire_to > 0)
		{
			cnt_assoc_num(priv, pstat, DECREASE, (char *)__FUNCTION__);
			check_sta_characteristic(priv, pstat, DECREASE);
			DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, "A STA is rejected - %02X:%02X:%02X:%02X:%02X:%02X\n",
								pstat->cmn_info.mac_addr[0],pstat->cmn_info.mac_addr[1],pstat->cmn_info.mac_addr[2],pstat->cmn_info.mac_addr[3],pstat->cmn_info.mac_addr[4],pstat->cmn_info.mac_addr[5]);
			LOG_MSG("A STA is rejected by 802.1x daemon - %02X:%02X:%02X:%02X:%02X:%02X\n",
					pstat->cmn_info.mac_addr[0],pstat->cmn_info.mac_addr[1],pstat->cmn_info.mac_addr[2],pstat->cmn_info.mac_addr[3],pstat->cmn_info.mac_addr[4],pstat->cmn_info.mac_addr[5]);
		}
...
int DOT11_Process_Disconnect_Req(struct net_device *dev, struct iw_point *data)
{
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		ret = __DOT11_Process_Disconnect_Req(dev, (DOT11_DISCONNECT_REQ *)data);
	}
#endif
...
Success : 0
-----------------------------------------------------------------------*/
int DOT11_Process_Delete_Key(struct net_device *dev, DOT11_DELETE_KEY *Delete_Key)
{
...
{
	if (pEncryptKey->dot11TTKeyLen <= sizeof(pEncryptKey->dot11TTKey.skey))
		memcpy(pEncryptKey->dot11TTKey.skey, src, pEncryptKey->dot11TTKeyLen);

	if (pEncryptKey->dot11TMicKeyLen <= sizeof(pEncryptKey->dot11TMicKey1.skey))
		memcpy(pEncryptKey->dot11TMicKey1.skey, src + 16, pEncryptKey->dot11TMicKeyLen);

	if (pEncryptKey->dot11TMicKeyLen <= sizeof(pEncryptKey->dot11TMicKey2.skey))
		memcpy(pEncryptKey->dot11TMicKey2.skey, src + 24, pEncryptKey->dot11TMicKeyLen);

	pEncryptKey->dot11TXPN48.val48 = 1;
}
		...
		DOT11_SET_KEY *pSetKey, unsigned char *pKey)
{
	...
	if ((priv->pmib->dot11WdsInfo.wdsEnabled == WDS_LAZY_ENABLE ||
			...
			for (i=0; i<32; i++)
			snprintf((char *)&priv->pmib->dot11WdsInfo.wdsPskPassPhrase[i*2], sizeof(priv->pmib->dot11WdsInfo.wdsPskPassPhrase) - i * 2, "%02x", key_combo[i]);
	...
	{
#if defined(HW_ENC_FOR_GROUP_CIPHER) 
...
Success : 0
-----------------------------------------------------------------------*/
static int DOT11_Process_Set_Port(struct net_device *dev, DOT11_SET_PORT *Set_Port)
{
	struct stat_info	*pstat;
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	struct wifi_mib		*pmib = priv->pmib;
...
#if 0//defined(CONFIG_PE_ENABLE) && defined(PE_INFO_SYNC)
...
static int DOT11_Process_QueryRSC(struct net_device *dev, DOT11_GKEY_TSC *pGkey_TSC)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	struct wifi_mib		*pmib = priv->pmib;
...
static int DOT11_Porcess_EAPOL_MICReport(struct net_device *dev, DOT11_MIC_FAILURE *MIC_Failure)
{
...
int __DOT11_Indicate_MIC_Failure(struct net_device *dev, struct stat_info *pstat)
{
	...
	if (pstat != NULL) // if pstat==NULL, it is called by upper layer
	{
		...
		//pstat->cmn_info.mac_addr[2], pstat->cmn_info.mac_addr[3], pstat->cmn_info.mac_addr[4], pstat->cmn_info.mac_addr[5]);

#ifdef __DRAYTEK_OS__
		cb_tkip_micFailure(dev, pstat->cmn_info.mac_addr);
		return 0;
#endif
		...
		memcpy(&Mic_Failure.MACAddr, pstat->cmn_info.mac_addr, MACADDRLEN);
		DOT11_EnQueue((unsigned long)priv, priv->pevent_queue, (UINT8 *)&Mic_Failure, sizeof(DOT11_MIC_FAILURE));
#endif
#ifdef INCLUDE_WPA_PSK
		psk_indicate_evt(priv, DOT11_EVENT_MIC_FAILURE, pstat->cmn_info.mac_addr, NULL, 0);
#endif

#ifdef WIFI_HAPD
		event_indicate_hapd(priv, pstat->cmn_info.mac_addr, HAPD_MIC_FAILURE, NULL);
#ifdef HAPD_DRV_PSK_WPS
		event_indicate(priv, pstat->cmn_info.mac_addr, 5);
#endif
#else
		event_indicate(priv, pstat->cmn_info.mac_addr, 5);
#endif
#ifdef WIFI_WPAS //_Eric ??
		event_indicate_wpas(priv, pstat->cmn_info.mac_addr, WPAS_MIC_FAILURE, NULL);
#endif
	}
	...
	if (priv->MIC_timer_on)
	{
		...
		if (mw5_legacy_timer_pending(&priv->MIC_check_timer))
		...
		mw5_legacy_mod_timer(&priv->assoc_reject_timer, jiffies + REJECT_ASSOC_PERIOD);
...
#endif
				del_station(priv, pstat_del, DEL_STA_SEND_DISASSOC, _RSON_MIC_FAILURE_);
			}
		}
		priv->assoc_num = 0;
	...
	else
	{
		...
		mw5_legacy_mod_timer(&priv->MIC_check_timer, jiffies + MIC_TIMER_PERIOD);
...
#if defined(CONFIG_PCI_HCI)
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		ret = __DOT11_Indicate_MIC_Failure(dev, pstat);
	}
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
...
//	int	i;
	...
	if (priv->MIC_timer_on) {
		...
		if (pstat_del) {
			memcpy((void *)Disassociation_Ind.MACAddr, (void *)pstat_del->cmn_info.mac_addr, MACADDRLEN);
			DEBUG_INFO("%s() disassoc %02X:%02X:%02X:%02X:%02X:%02X\n",__func__,pstat_del->cmn_info.mac_addr[0],pstat_del->cmn_info.mac_addr[1],pstat_del->cmn_info.mac_addr[2],pstat_del->cmn_info.mac_addr[3],pstat_del->cmn_info.mac_addr[4],pstat_del->cmn_info.mac_addr[5]);
			 ...
			 */
			delay_ms(20);
			issue_disassoc(priv, pstat_del->cmn_info.mac_addr, _RSON_MIC_FAILURE_);
...
void DOT11_Process_Acc_SetExpiredTime(struct net_device *dev, DOT11_SET_EXPIREDTIME	*Set_ExpireTime)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
void DOT11_Process_Acc_QueryStats(struct net_device *dev, DOT11_QUERY_STATS	*pStats)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
void DOT11_Process_Acc_QueryStats_All(struct net_device *dev, struct iw_point *data)
{
	...
	if( pStats != NULL ){
		...
		while (plist != phead) {
			...
			memcpy( pStats[cnt].MACAddr, pstat->cmn_info.mac_addr, MACADDRLEN );
...
static int DOT11_Process_STA_Query_Bssid(struct net_device *dev, DOT11_STA_QUERY_BSSID *pQuery)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
static int DOT11_Process_STA_Query_Ssid(struct net_device *dev, DOT11_STA_QUERY_SSID *pQuery)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
#ifdef WIFI_SIMPLE_CONFIG
static int DOT11_WSC_set_ie(struct net_device *dev, DOT11_SET_WSCIE *Set_RSNIE)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);

	if (Set_RSNIE->Flag == SET_IE_FLAG_BEACON) {
		DEBUG_INFO("WSC: set beacon IE\n");
		if (Set_RSNIE->RSNIELen <= sizeof(priv->pmib->wscEntry.beacon_ie)) {
...
#define TAG_SELECTED_REGISTRAR		0x1041
		...
		if (Set_RSNIE->RSNIELen <= sizeof(priv->pmib->wscEntry.probe_rsp_ie)) {
		...
		if (Set_RSNIE->RSNIELen <= sizeof(priv->pmib->wscEntry.probe_req_ie)) {
		...
		if (Set_RSNIE->RSNIELen <= sizeof(priv->pmib->wscEntry.assoc_ie)) {
...
#ifdef USER_ADDIE
static int DOT11_USER_set_ie(struct net_device *dev, DOT11_SET_USERIE *Set_USERIE)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	unsigned int i;
...
#ifdef CONFIG_IEEE80211W
static int DOT11_PMF_set_ie(struct net_device *dev, DOT11_SET_11W_Flags flags)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	...
	if(!pstat){
    	...
    	PMFDEBUG("set sta[%02x%02x%02x:%02x%02x%02x]'s PMF=[%d]\n",pstat->cmn_info.mac_addr[0],pstat->cmn_info.mac_addr[1],pstat->cmn_info.mac_addr[2],pstat->cmn_info.mac_addr[3],pstat->cmn_info.mac_addr[4],pstat->cmn_info.mac_addr[5], flags.isPMF);
    }
...
#ifdef HS2_SUPPORT
#ifndef REPLY_HS2_BY_HAPD
/* Hotspot 2.0 Release 1*/
static int DOT11_HS2_set_ie(struct net_device *dev, DOT11_SET_RSNIE *Set_RSNIE)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
#ifndef RTK_NL80211
static int DOT11_FT_get_key(struct net_device *dev, struct iw_point *data)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
static int DOT11_FT_set_key(struct net_device *dev, struct iw_point *data)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	...
	switch (proto.Type)
	{
		...
		case FTKEY_TYPE_PULL:
		{
			...
			if (pstat && (pstat->ft_state == state_ft_auth) && (pstat->wpa_sta_info->r1kh == NULL)) {
				pstat->wpa_sta_info->r1kh = search_r1kh(priv, pkey->s1kh_id, pkey->r1kh_id);
				if (pstat->wpa_sta_info->r1kh) {
                    SMP_LOCK_FT_R1KH(flags);
					memcpy(pstat->wpa_sta_info->r1kh->r0kh_id, pstat->wpa_sta_info->cache_r0kh_id, pstat->wpa_sta_info->cache_r0kh_id_len);
					pstat->wpa_sta_info->r1kh->r0kh_id_len = pstat->wpa_sta_info->cache_r0kh_id_len;
                    memcpy(pstat->wpa_sta_info->r1kh->pmk_r0_name, pstat->wpa_sta_info->cache_pmk_r0_name, PMKID_LEN);
					...
					if (pstat->wpa_sta_info->over_ds) {
						...
						*((unsigned short *)pos) = cpu_to_le16(_STATS_SUCCESSFUL_);
						...
						FT_IndicateEvent(priv, pstat, DOT11_EVENT_FT_ACTION_IND, &param);
									...
									issue_auth_resp(priv, pstat, (unsigned short)(_STATS_SUCCESSFUL_));
					}
				}
			}
			break;
...
static int DOT11_FT_action_ind(struct net_device *dev, struct iw_point *data)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
static int DOT11_FT_assoc_ind(struct net_device *dev, struct iw_point *data)
{
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	...
	DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, "FT_ASSOC_IND del_sta - %02X:%02X:%02X:%02X:%02X:%02X\n",
		addr[0],addr[1],addr[2],addr[3],addr[4],addr[5]);
	del_sta(priv, tmpBuf);
    return 0;
}

static int DOT11_FT_key_expire_ind(struct net_device *dev, struct iw_point *data)
{
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	...
	DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, "FT_KEY_EXPIRE_IND del_sta - %02X:%02X:%02X:%02X:%02X:%02X\n",
				addr[0],addr[1],addr[2],addr[3],addr[4],addr[5]);
	del_sta(priv, tmpBuf);
...
static int DOT11_FT_query_info(struct net_device *dev, struct iw_point *data)
{
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
static int DOT11_FT_set_info(struct net_device *dev, struct iw_point *data)
{
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
static int DOT11_FT_insert_r0_key(struct net_device *dev, struct iw_point *data)
{
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
static int DOT11_FT_insert_r1_key(struct net_device *dev, struct iw_point *data)
{
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
    ...
    pstat->wpa_sta_info->r1kh = search_r1kh(priv, insert_r1.sta_addr, insert_r1.bssid);

    return 0;
}

static int DOT11_FT_trigger_event(struct net_device *dev, struct iw_point *data)
{
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
...
/*-----------------------------------------------------------------------------
  ...
  The function is called by kernel space
  ------------------------------------------------------------------------------*/
#if defined(CONFIG_RTL_KERNEL_MIPS16_WLAN) && defined(CONFIG_RTL8196C)
__NOMIPS16
#endif
int rtl8192cd_ioctl_priv_req(struct net_device *dev, struct iw_point *data)
{
	DOT11_REQUEST	req;
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	...
	switch(req.EventId)
	{
		...
		case DOT11_EVENT_ASSOCIATION_RSP:
			if(!DOT11_Process_Association_Rsp(dev, (DOT11_ASSOCIATIIN_RSP *)(data->pointer), WIFI_ASSOCRSP))
			{}
			break;
		...
		case DOT11_EVENT_DISCONNECT_REQ:
            ...
            if (priv->hci_type == RTL_HCI_PCIE) {
			    __DOT11_Process_Disconnect_Req(dev, (DOT11_DISCONNECT_REQ *)(data->pointer));
            }
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		...
		case DOT11_EVENT_SET_KEY:
			...
			memset(&Set_Key,0,sizeof(struct _DOT11_SET_KEY));
			memset(KeyMaterial,0,sizeof(KeyMaterial));
			/*add len chk to avoid out-of-bounds access(overrun)*/
			if(data->length >= sizeof(DOT11_SET_KEY)){
				memcpy((void *)&Set_Key, (void *)data->pointer, sizeof(DOT11_SET_KEY));
				memcpy((void *)KeyMaterial,	((DOT11_SET_KEY *)data->pointer)->KeyMaterial, 32);
			}

#if defined(CONFIG_PCI_HCI)
            if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
    			if(!DOT11_Process_Set_Key(dev, NULL, &Set_Key, KeyMaterial))
    			{}
            }
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		...
		case DOT11_EVENT_SET_PORT:
			if(!DOT11_Process_Set_Port(dev, (DOT11_SET_PORT	*)(data->pointer)))
			{}
			break;

		case DOT11_EVENT_DELETE_KEY:
			if(!DOT11_Process_Delete_Key(dev, (DOT11_DELETE_KEY	*)(data->pointer)))
			{}
			break;
		...
		case DOT11_EVENT_SET_RSNIE:
			if(priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm != _NO_PRIVACY_ &&
				...
				if(!DOT11_Process_Set_RSNIE(dev, (DOT11_SET_RSNIE *)data->pointer))
				{}
			break;

		case DOT11_EVENT_GKEY_TSC:
			if(!DOT11_Process_QueryRSC(dev, (DOT11_GKEY_TSC	*)(data->pointer)))
			{}
			break;

		case DOT11_EVENT_MIC_FAILURE:
			if(!DOT11_Porcess_EAPOL_MICReport(dev, (DOT11_MIC_FAILURE *)(data->pointer)))
			{}
			break;
...
#ifdef RADIUS_ACCOUNTING
			DOT11_Process_Acc_SetExpiredTime(dev, (DOT11_SET_EXPIREDTIME *)(data->pointer));
#endif
...
#ifdef RADIUS_ACCOUNTING
			DOT11_Process_Acc_QueryStats(dev,(DOT11_QUERY_STATS	*)(data->pointer));
#endif
			DEBUG_INFO("trying to Set ACC Expiredtime\n");
			break;

		case DOT11_EVENT_REASSOCIATION_RSP:
			if(!DOT11_Process_Association_Rsp(dev,(DOT11_ASSOCIATIIN_RSP *)(data->pointer), WIFI_REASSOCRSP))
			{}
			break;

		case DOT11_EVENT_STA_QUERY_BSSID:
			if(!DOT11_Process_STA_Query_Bssid(dev, (DOT11_STA_QUERY_BSSID *)(data->pointer)))
			{};
			break;

		case DOT11_EVENT_STA_QUERY_SSID:
			if(!DOT11_Process_STA_Query_Ssid(dev, (DOT11_STA_QUERY_SSID	*)(data->pointer)))
			{};
			break;
#ifdef CONFIG_IEEE80211W
		case DOT11_EVENT_SET_PMF:
			{
				...
				memcpy((void *)&flags, (void *)data->pointer, sizeof(DOT11_SET_11W_Flags));
				if(!DOT11_PMF_set_ie(dev, flags))
				{};
			}
			break;
#endif
		default:
			printk("[%s] unknown driver ioctl command[%d]\n", __FUNCTION__,  req.EventId);
			break;
	} // switch(req->EventId)
...
/*-----------------------------------------------------------------------------
  ...
  The function is called by user space
  ------------------------------------------------------------------------------*/
#if defined(CONFIG_RTL_KERNEL_MIPS16_WLAN) && defined(CONFIG_RTL8196C)
__NOMIPS16
#endif
int rtl8192cd_ioctl_daemonreq(struct net_device *dev, struct iw_point *data)
{
	int		ret;
#ifndef WITHOUT_ENQUEUE
	...
	switch(req.EventId)
	{
		...
		case DOT11_EVENT_ASSOCIATION_RSP:
			{
				DOT11_ASSOCIATIIN_RSP	Assoc_Rsp;
				if (copy_from_user((void *)&Assoc_Rsp, (void *)(data->pointer), sizeof(DOT11_ASSOCIATIIN_RSP))) {
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
				if(!DOT11_Process_Association_Rsp(dev, &Assoc_Rsp, WIFI_ASSOCRSP))
				{}
			}
			break;
		...
		case DOT11_EVENT_DISCONNECT_REQ:
			{
				...
				if (copy_from_user((void *)&Disconnect_Req, (void *)(data->pointer), sizeof(DOT11_DISCONNECT_REQ))) {
...
#if defined(CONFIG_PCI_HCI)
                if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
				    __DOT11_Process_Disconnect_Req(dev, &Disconnect_Req);
                }
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
                if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
				    notify_disconnect_sta(dev, &Disconnect_Req);
                }
#endif
			}
			break;
		...
		case DOT11_EVENT_SET_KEY:
			{
				...
				if (copy_from_user((void *)KeyMaterial, ((DOT11_SET_KEY *)data->pointer)->KeyMaterial, 32)) {
					//printk("copy_from_user error!\n");
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
		...
		case DOT11_EVENT_SET_PORT:
			{
				...
				if (copy_from_user((void *)&Set_Port, (void *)(data->pointer), sizeof(DOT11_SET_PORT))) {

					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
				if(!DOT11_Process_Set_Port(dev, &Set_Port))
				{}
			}
			break;
		...
		case DOT11_EVENT_DELETE_KEY:
			{
				...
				if (copy_from_user((void *)&Delete_Key, (void *)(data->pointer), sizeof(DOT11_DELETE_KEY))) {
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
				if(!DOT11_Process_Delete_Key(dev, &Delete_Key))
				{}
			}
			break;
		...
		case DOT11_EVENT_SET_RSNIE:
			{
				...
				if(priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm != _NO_PRIVACY_ &&
						...
						priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm != _WEP_104_PRIVACY_)
				{
					if (copy_from_user((void *)&Set_RSNIE, (void *)(data->pointer), sizeof(DOT11_SET_RSNIE))) {
						printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
						return -1;
					}
					if(!DOT11_Process_Set_RSNIE(dev, &Set_RSNIE))
					{}
				}
			}
			break;
		...
		case DOT11_EVENT_GKEY_TSC:
			{
				...
				if (copy_from_user((void *)&pGkey_TSC, (void *)(data->pointer), sizeof(DOT11_GKEY_TSC))) {
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
				if(!DOT11_Process_QueryRSC(dev, &pGkey_TSC))
				{}
			}
			break;
		...
		case DOT11_EVENT_MIC_FAILURE:
			{
				DOT11_MIC_FAILURE MIC_Failure;
				if (copy_from_user((void *)&MIC_Failure, (void *)(data->pointer), sizeof(DOT11_MIC_FAILURE))) {
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
				if(!DOT11_Porcess_EAPOL_MICReport(dev, &MIC_Failure))
				{}
			}
			break;
		...
		case DOT11_EVENT_ACC_SET_EXPIREDTIME:
			...
			if (copy_from_user((void *)&Set_ExpireTime, (void *)(data->pointer), sizeof(DOT11_SET_EXPIREDTIME))) {
				printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
				return -1;
			}
			DOT11_Process_Acc_SetExpiredTime(dev, &Set_ExpireTime);
#endif
			DEBUG_INFO("trying to Set ACC Expiredtime\n");
			break;
		...
		case DOT11_EVENT_ACC_QUERY_STATS:
			{
				DOT11_QUERY_STATS	Stats;
				if (copy_from_user((void *)&Stats, (void *)(data->pointer), sizeof(DOT11_QUERY_STATS))) {
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
#ifdef RADIUS_ACCOUNTING
				DOT11_Process_Acc_QueryStats(dev, &Stats);
#endif
				DEBUG_INFO("trying to Set ACC Expiredtime\n");
			}
			break;
		...
		case DOT11_EVENT_STA_QUERY_BSSID:
			{
				DOT11_STA_QUERY_BSSID	Query ;
				if (copy_from_user((void *)&Query, (void *)(data->pointer), sizeof(DOT11_STA_QUERY_BSSID))) {
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
				if(!DOT11_Process_STA_Query_Bssid(dev, &Query))
				{
					if (copy_to_user((void *)((unsigned long)(data->pointer)), &Query, sizeof(DOT11_STA_QUERY_BSSID))) {
						DEBUG_ERR("copy_to_user fail!\n");
						return -1;
					}
				}
		...
		case DOT11_EVENT_STA_QUERY_SSID:
			{
				DOT11_STA_QUERY_SSID	Query;
				if (copy_from_user((void *)&Query, (void *)(data->pointer), sizeof(DOT11_STA_QUERY_SSID))) {
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}
				if(!DOT11_Process_STA_Query_Ssid(dev, &Query))
				{
					if (copy_to_user((void *)((unsigned long)(data->pointer)), &Query, sizeof(DOT11_STA_QUERY_SSID))) {
						DEBUG_ERR("copy_to_user fail!\n");
						return -1;
					}
				};
		...
		case DOT11_EVENT_WSC_SET_IE:
			{
				DOT11_SET_WSCIE Set_RSNIE ;

				if (copy_from_user((void *)&Set_RSNIE, (void *)(data->pointer), sizeof(DOT11_SET_WSCIE))) {
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}

				if(!DOT11_WSC_set_ie(dev, &Set_RSNIE))
				{};
			}
			break;
		...
		case DOT11_EVENT_WSC_SET_MIB:
						if (copy_from_user((void *)&getset_mib_t, (void *)(data->pointer), sizeof(DOT11_GETSET_MIB))) {
							printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
							return -1;
						}
		...
		case DOT11_EVENT_WSC_GET_MIB:
						...
						if (copy_to_user((void *)(data->pointer), (void *)&getset_mib_t, sizeof(DOT11_GETSET_MIB))) {
...
/* Hotsport 2.0 Release 1 */
		case DOT11_EVENT_HS2_SET_IE:
			{
	            ...
	            if(!DOT11_HS2_set_ie(dev, &Set_RSNIE))
	            {};
			}
			break;
        ...
        case DOT11_EVENT_HS2_GAS_RSP:
        	{
				...
				if (copy_from_user((void *)&gas_rsp, (void *)(data->pointer), sizeof(DOT11_HS2_GAS_RSP))) {
					printk("copy_from_user error!\n");
					return -1;
				}
				issue_GASrsp(priv, &gas_rsp);
        	}
			break;
        case DOT11_EVENT_HS2_TSM_REQ:
			...
			if (copy_from_user((void *)(&sesionInfoURL),(void *)(data->pointer), sizeof(DOT11_BSS_SessInfo_URL)))
			{
				DEBUG_ERR("copy_from_user fail!\n");
				return -1;
			}
			...
			HS2_DEBUG_INFO("issue_BSS_TSM_req and set timer to disassoc[%d]ms\n",(tsmreq.Disassoc_timer * 60 * 1000));
           	...
           	if (mw5_legacy_timer_pending(&priv->disassoc_timer))
        		mw5_legacy_del_timer_sync(&priv->disassoc_timer);
			mw5_legacy_mod_timer(&priv->disassoc_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(tsmreq.Disassoc_timer * 60 * 1000));
			}
			break;
		...
		case DOT11_EVENT_SET_PMF:
			{
				...
				if (copy_from_user((void *)(&flags),(void *)(data->pointer), data->length))
				{
					PMFDEBUG("memcpy fail!, ptr=%p, length=%d\n", data->pointer, data->length);
					return -1;
				}
		...
		case DOT11_EVENT_FT_GET_KEY:
			return DOT11_FT_get_key(dev, data);

		case DOT11_EVENT_FT_SET_KEY:
			return DOT11_FT_set_key(dev, data);

		case DOT11_EVENT_FT_ACTION_IND:
			return DOT11_FT_action_ind(dev, data);

		case DOT11_EVENT_FT_ASSOC_IND:
			return DOT11_FT_assoc_ind(dev, data);

		case DOT11_EVENT_FT_KEY_EXPIRE_IND:
			return DOT11_FT_key_expire_ind(dev, data);

		case DOT11_EVENT_FT_QUERY_INFO:
			return DOT11_FT_query_info(dev, data);

		case DOT11_EVENT_FT_SET_INFO:
			return DOT11_FT_set_info(dev, data);

		case DOT11_EVENT_FT_AUTH_INSERT_R0:
			return DOT11_FT_insert_r0_key(dev, data);

		case DOT11_EVENT_FT_AUTH_INSERT_R1:
			return DOT11_FT_insert_r1_key(dev, data);

		case DOT11_EVENT_FT_TRIGGER_EVENT:
			return DOT11_FT_trigger_event(dev, data);
#endif
#endif

#if defined(GENERAL_EVENT)
		case DOT11_EVENT_GENERAL_GET_EVENT:
			{
				if((ret = DOT11_DeQueue((unsigned long)priv, priv->pgeneralevent_queue, QueueData, &QueueDataLen)) != 0)
				{
					val8 = DOT11_EVENT_NO_EVENT;
                    memcpy(data->pointer, &val8, 1);
					val8 = 0;
                    memcpy((void *)((unsigned long)(data->pointer) + 1), &val8, 1);
					data->length = sizeof(DOT11_NO_EVENT);
				}
				...
				{
					QueueData[1] = (priv->pgeneralevent_queue->NumItem != 0)? 1 : 0;
                    memcpy(data->pointer, QueueData, QueueDataLen);
					data->length = QueueDataLen;
				}
                ret = data->length;
			}
...
#endif
#ifdef DOT11K
		case DOT11_EVENT_11K_NEIGHBOR_REPORT:
		{
			//unsigned char i;
			unsigned char *ptr;
			val8 = 0;
			...
			// 		continue;
			// 	*ptr = _NEIGHBOR_REPORT_IE_;
			// 	ptr++;
			// 	*ptr = sizeof(struct dot11k_neighbor_report);
			// 	ptr++;
			// 	QueueDataLen += 2;
			// 	memcpy(ptr, &priv->rm_neighbor_report[i], sizeof(struct dot11k_neighbor_report));
			// 	QueueDataLen += sizeof(struct dot11k_neighbor_report);
			// 	ptr += sizeof(struct dot11k_neighbor_report);
			// 	val8++;
			// }
			...
			if (val8 == 0) {
				...
				memcpy(neighbor_report.bssid, priv->pmib->dot11OperationEntry.hwaddr, MACADDRLEN);
				neighbor_report.op_class = rm_get_op_class(priv, priv->pmib->dot11RFEntry.dot11channel);// 1
				neighbor_report.channel = priv->pmib->dot11RFEntry.dot11channel;   // 1
				neighbor_report.phytype = 0;  // 1

				neighbor_report.subelemnt.subelement_id = _WNM_BSS_TRANS_CANDIDATE_PREFRENCE_;
				neighbor_report.subelemnt.len = 1;
				neighbor_report.subelemnt.preference = 255;
				...
				*ptr = sizeof(struct dot11k_neighbor_report);
				ptr++;
				QueueDataLen += 2;
				memcpy(ptr, &neighbor_report, sizeof(struct dot11k_neighbor_report));
				QueueDataLen += sizeof(struct dot11k_neighbor_report);
			}
			...
			if (copy_to_user((void *)(data->pointer), (void *)QueueData, QueueDataLen))
			{
				HS2_DEBUG_ERR("Neighbor report copy_to_user fail!\n");
				return -1;
			}
			data->length = QueueDataLen;
			break;
		...
		case DOT11_EVENT_USER_SETIE:
			{
				...
				if (copy_from_user((void *)&Set_USERIE, (void *)(data->pointer), sizeof(DOT11_SET_USERIE))) {
					DEBUG_ERR("copy_from_user error!\n");
					printk("[%s] copy_from_user error[%d]\n", __FUNCTION__,  req.EventId);
					return -1;
				}

				if(DOT11_USER_set_ie(dev, &Set_USERIE) < 0) {
		...
		case DOT11_EVENT_UCFGING_LED:
			{
				...
				//panic_printk("LED_Configuring=%d\n", LED_Configuring);
					...
					if(LED_Configuring == 1){
						if (mw5_legacy_timer_pending(&priv->pshare->LED_Timer))
						...
						if (mw5_legacy_timer_pending(&LED_TimerCFGING))
							mw5_legacy_del_timer_sync(&LED_TimerCFGING);
