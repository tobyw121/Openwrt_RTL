...
static void rm_check_result(struct rtl8192cd_priv *priv)
{
    ...
    if(need_process)
    {
        if(priv->rm.measure_interval[i])   /*need random delay*/
        {
            ...
            mw5_legacy_mod_timer(&priv->rm_delay_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(rand_time));
        }
        else
        {
...
{
    int i;
    if (mw5_legacy_timer_pending(&priv->rm_delay_timer))
				 ...
				 unsigned char *pframe, int frame_len)
{
   ...
   if (pstat->rm.beacon_report == NULL) {
	   ...
	   memset((void *)pstat->rm.beacon_report, BCN_RPT_BUF_LEN, 0);
