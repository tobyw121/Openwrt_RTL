...
void input_value_32(UINT32 *p, unsigned char start, unsigned char end, unsigned int value)
{
...
const u2Byte VHT_MCS_DATA_RATE[4][2][40] = 
	...
	)
{
	...
	if( ((VHTMcsRate>20)&&get_rf_mimo_mode(priv)==RF_2T2R)|| (priv->pshare->is_40m_bw > 2))
		return 600;
	else
...
//VHT CAPABILITIES INFO field
	...
	if((GET_CHIP_VER(priv) == VERSION_8814A) && (priv->pshare->is_40m_bw == CHANNEL_WIDTH_80))
	...
	if(is_ac2g(priv) && ((priv->pshare->CurrentChannelBW == CHANNEL_WIDTH_20)||(GET_CHIP_VER(priv)==VERSION_8814A))) //if bw = 20M, not support MCS9
	{
		...
		else if(get_rf_mimo_mode(priv) == RF_3T3R || get_rf_mimo_mode(priv) == RF_4T4R)
		{
	...
	if (GET_CHIP_VER(priv)==VERSION_8814A && get_rf_mimo_mode(priv) == RF_2T2R) {		
...
//// ===== VHT CAPABILITIES ELEMENT ===== /////
	...
	if((priv->pshare->is_40m_bw == CHANNEL_WIDTH_80) || (GET_CHIP_VER(priv)==VERSION_8814A))
	{
...
void construct_vht_ie(struct rtl8192cd_priv *priv, unsigned char channel_center)
{
	...
	unsigned int supported_vht = priv->pmib->dot11acConfigEntry.dot11SupportedVHT;
	unsigned char channel_center2 = 0;
...
//VHT CAPABILITIES INFO field
	...
	input_value_32(&vht_cap->vht_cap_info, EXT_NSS_BW_S, EXT_NSS_BW_E, 1);
	
	if ((priv->pshare->is_40m_bw == CHANNEL_WIDTH_80) || (priv->pshare->is_40m_bw == CHANNEL_WIDTH_160) || (priv->pshare->is_40m_bw == CHANNEL_WIDTH_80_80))
		input_value_32(&vht_cap->vht_cap_info, SHORT_GI80M_S, SHORT_GI80M_E, (priv->pmib->dot11nConfigEntry.dot11nShortGIfor80M ? 1 : 0));

	if (priv->pshare->is_40m_bw == CHANNEL_WIDTH_160 || priv->pshare->is_40m_bw == CHANNEL_WIDTH_80_80)
		input_value_32(&vht_cap->vht_cap_info, SHORT_GI160M_S, SHORT_GI160M_E, (priv->pmib->dot11nConfigEntry.dot11nShortGIfor160M ? 1 : 0));
	...
	if (priv->pmib->dot11nConfigEntry.dot11nSTBC)
	{
		if (get_rf_mimo_mode(priv) >= RF_2T2R) //eric_8814 
			input_value_32(&vht_cap->vht_cap_info, TX_STBC_S, TX_STBC_E, 1);
...
#endif	
	{
		...
		if(GET_CHIP_VER(priv)==VERSION_8814B && priv->pmib->dot11RFEntry.txbf) {
			txbf_max_ant = 3;
			if(get_rf_mimo_mode(priv) == RF_4T4R) {       
			    txbf_sounding_dim = 3;
			} else if(get_rf_mimo_mode(priv) == RF_3T3R) {
	...
	if((GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8812F) && priv->pmib->dot11RFEntry.txbf) {
	...
	if ((GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8814B || GET_CHIP_VER(priv) == VERSION_8812F) && priv->pmib->dot11RFEntry.txbf) {
...
#if defined(AC2G_256QAM) || defined(WLAN_HAL_8814AE) //need to check 98F/8814B
	if(is_ac2g(priv) && ((priv->pmib->dot11nConfigEntry.dot11nUse40M == CHANNEL_WIDTH_20)||(GET_CHIP_VER(priv)==VERSION_8814A))) //if bw = 20M, not support MCS9
	{
		if(get_rf_mimo_mode(priv) == RF_1T1R) 
		{
			supported_vht = 0xfffd;
	...
	} else if (priv->pmib->dot11nConfigEntry.dot11nUse40M == CHANNEL_WIDTH_80_80) {
		// 8814B can only use 2SS in 80+80 mode
		supported_vht = priv->pmib->dot11acConfigEntry.dot11SupportedVHT | 0xfffa;
	}

	{
		if(GET_CHIP_VER(priv)==VERSION_8198F) //98F don't support Rx VHT
			input_value_32(&vht_cap->vht_support_mcs[0], MCS_RX_MAP_S, MCS_RX_MAP_E, 0xffff);
		else
			input_value_32(&vht_cap->vht_support_mcs[0], MCS_RX_MAP_S, MCS_RX_MAP_E, supported_vht);
		value = (VHTMcsToDataRate(priv)+1)>>1;
		input_value_32(&vht_cap->vht_support_mcs[0], MCS_RX_HIGHEST_RATE_S, MCS_RX_HIGHEST_RATE_E, value);
...
//// ===== VHT CAPABILITIES ELEMENT ===== /////
	...
	if((priv->pshare->is_40m_bw == CHANNEL_WIDTH_80) 
		|| (priv->pshare->is_40m_bw == CHANNEL_WIDTH_160)
		|| (priv->pshare->is_40m_bw == CHANNEL_WIDTH_80_80)
		|| (GET_CHIP_VER(priv)==VERSION_8814A))
	{
		if (priv->pshare->is_40m_bw == CHANNEL_WIDTH_80) {
			vht_oper->vht_oper_info[0] = 1;     /* 80M */
		} else if (priv->pshare->is_40m_bw == CHANNEL_WIDTH_160) {
			vht_oper->vht_oper_info[0] = 2;     /* 160M */
		} else if (priv->pshare->is_40m_bw == CHANNEL_WIDTH_80_80) {
			if (priv->pmib->dot11nConfigEntry.dot11nComply160M)
				vht_oper->vht_oper_info[0] = 2; /* 160M */
			else
				vht_oper->vht_oper_info[0] = 3; /* 80+80M */
		} else {
			vht_oper->vht_oper_info[0] = 0;     /* 20M or 40M */
		}

		if (priv->pshare->is_40m_bw == CHANNEL_WIDTH_80 || priv->pshare->is_40m_bw == CHANNEL_WIDTH_80_80) {
			int channel = priv->pmib->dot11RFEntry.dot11channel;
			if (channel <= 48)
				channel_center = 42;
			else if (channel <= 64)
		...
		if (priv->pshare->is_40m_bw == CHANNEL_WIDTH_80_80) {
			int channel = priv->pmib->dot11RFEntry.dot11channel2;
			if (channel <= 48)
				channel_center2 = 42;
			else if (channel <= 64)
				channel_center2 = 58;
		...
		if (priv->pshare->is_40m_bw == CHANNEL_WIDTH_80_80 &&
			priv->pmib->dot11nConfigEntry.dot11nComply160M) {
			vht_oper->vht_oper_info[1] = (channel_center + channel_center2) / 2;
			vht_oper->vht_oper_info[2] = 0;
		}
		else {
			vht_oper->vht_oper_info[1] = channel_center;
			vht_oper->vht_oper_info[2] = channel_center2;
		}
	}
