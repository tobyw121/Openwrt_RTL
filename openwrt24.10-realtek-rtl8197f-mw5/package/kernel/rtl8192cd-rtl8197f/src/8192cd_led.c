...
struct mw5_legacy_timer	LED_TimerCFGING;
...
#if defined(CONFIG_RTL_8197F) && !defined(CONFIG_MW5_NO_LED)
...
static void set_sw_LED0(struct rtl8192cd_priv *priv, int flag)
{
    ...
    #if defined(CONFIG_RTL_8197F) && !defined(CONFIG_MW5_NO_LED) && !defined(__ECOS)
    else if(GET_CHIP_VER(priv) == VERSION_8197F) {
        if(flag) /* led off*/
...
void LED_Interval_timeout(unsigned long task_priv)
{
	...
	{
		mw5_legacy_mod_timer(&priv->pshare->LED_Timer, jiffies + LED_NOBLINK_TIME);
		return;
	}
#else
    ...
    if ( priv->pshare->LED_Toggle == priv->pshare->LED_ToggleStart) {
        ...
        if 	(LED_TYPE == LEDTYPE_SW_CUSTOM1)
            ...
            mw5_legacy_mod_timer(&priv->pshare->LED_Timer, jiffies + led_on_time); 
    }
...
#endif		
	mw5_legacy_mod_timer(&LED_TimerCFGING, jiffies + LED_UCFGING_TIME); 
...
void StartCFGINGTimer(void )
{
	...
	mw5_legacy_timer_init(&LED_TimerCFGING);
	...
	mw5_legacy_mod_timer(&LED_TimerCFGING, jiffies + LED_CFGING_Interval);
}
#endif
