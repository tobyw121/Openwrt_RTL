...
#define TX_PAGE_SIZE		PBP_PSTX_SIZE_V1
...
void rtl8821cs_xmit_tasklet(unsigned long data);
...
int _rtw_init_xmit_priv(struct rtl8192cd_priv *priv)
{
	...
	mw5_legacy_timer_init(&pshare->xmit_check_timer);
	pshare->xmit_check_timer.data = (unsigned long)priv;
	pshare->xmit_check_timer.function = rtl8192cd_xmit_check_timer;
	
	// init xmit tasklet
	tasklet_init(&pshare->xmit_tasklet, rtl8821cs_xmit_tasklet, (unsigned long)priv);
...
tcpack_out:
	...
	mw5_legacy_mod_timer(&pshare->xmit_check_timer, jiffies+msecs_to_jiffies(10));
}
...
static s32 rtw_sdio_check_tx_freepage(struct rtl8192cd_priv *priv, u8 page_idx, u16 page_num)
{
	HAL_INTF_DATA_TYPE *pHalData = GET_HAL_INTF_DATA(priv);
	u16 *free_tx_page = pHalData->SdioTxFIFOFreePage;
...
static void rtw_sdio_update_tx_freepage(struct rtl8192cd_priv *priv, u8 page_idx, u16 page_num)
{
	HAL_INTF_DATA_TYPE *pHalData = GET_HAL_INTF_DATA(priv);
	u16 *free_tx_page = pHalData->SdioTxFIFOFreePage;
	u16 requiredPublicPage = 0;

#ifdef TXDMA_ERROR_PAGE_DEBUG
	memcpy(pHalData->SdioTxFIFOFreePage_prev, free_tx_page, sizeof(u16)*SDIO_TX_FREE_PG_QUEUE);
#endif
...
#ifdef TXDMA_ERROR_PAGE_DEBUG
		, u32 reg230, u32 reg234, u32 reg238, u32 reg23c, u32 reg240
#endif
		)
{
...
#ifdef TXDMA_ERROR_PAGE_DEBUG
		printk(KERN_ERR "[REG][prev] 0x230=%08X, 0x234=%08X, 0x238=%08X 0x23c=%08X, 0x240=%08X\n",
			reg230, reg234, reg238, reg23c, reg240);
                printk(KERN_ERR "[REG][now ] 0x230=%08X, 0x234=%08X, 0x238=%08X 0x23c=%08X, 0x240=%08X\n",
			RTL_R32(0x230), RTL_R32(0x234), RTL_R32(0x238), RTL_R32(0x23c), RTL_R32(0x240));
...
do_check:
	...
	if (pxmitbuf) {
		if (!priv->pshare->freepage_updated) {
            ...
            pHalData->SdioTxIntQPage = pg_num + 2;
			RTL_W16(reg_freepage_thres[pxmitbuf->tx_page_idx], (pg_num+2)|BIT15);
...
s32 rtl8192es_dequeue_writeport(struct rtl8192cd_priv *priv, int from)
{
	...
	u32 deviceId;
#ifdef TXDMA_ERROR_PAGE_DEBUG
	u32 reg230, reg234, reg238, reg23c, reg240;
#endif
...
#ifdef TXDMA_ERROR_PAGE_DEBUG
	reg230 = RTL_R32(0x230);
	reg234 = RTL_R32(0x234);
	reg238 = RTL_R32(0x238);
	reg23c = RTL_R32(0x23c);
	reg240 = RTL_R32(0x240);
#endif
...
s32 is_fifo_page_avail(struct rtl8192cd_priv *priv)
{
    HAL_INTF_DATA_TYPE *pHalData = GET_HAL_INTF_DATA(priv);
    
    return rtw_sdio_check_tx_freepage(priv, pHalData->SdioTxIntQIdx, pHalData->SdioTxIntQPage);
}
...
s32 rtl8192es_dequeue_writeport(struct rtl8192cd_priv *priv)
{
	...
	u16 *freePage = pHalData->SdioTxFIFOFreePage;
	struct xmit_buf *pxmitbuf;
	u32 deviceId;
	u32 requiredPage;
	u8 PageIdx;
...
u32 sdio_submit_xmitbuf(struct rtl8192cd_priv *priv, struct xmit_buf *pxmitbuf)
{
	...
	if (BEACON_QUEUE == q_num) {
			...
			)
		{
			...
			// Note. [Beacon valid check] use 0x205[7] for root & vap2, use 0x207[7] for vap1 & vap3
			RTL_W8(0x205, RTL_R8(0x205));
			do {
...
const int RATE_80M_2SS_LGI[] = {
	29, 58, 87, 117, 175, 234, 263, 292, 351, 390,	// 1SS
	58, 117, 175, 234, 351, 468, 526, 585, 702, 780	// 2SS
};
...
static inline void update_remaining_timeslot(struct rtl8192cd_priv *priv, struct tx_desc_info *pdescinfo, unsigned int q_num, unsigned char is_40m_bw)
{
	...
	if (is_VHT_rate(pdescinfo->rate)) {
		txRate = RATE_80M_2SS_LGI[pdescinfo->rate - VHT_RATE_ID];
	} else if (is_MCS_rate(pdescinfo->rate)) {
		if (is_40m_bw)
...
void rtl8192cd_hal_fill_fwinfo(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, struct tx_desc  *pdesc, unsigned int frag_idx)
{
	...
	if (is_MCS_rate(txcfg->tx_rate))	// HT rates
	{
		...
		{
			if(txcfg->pstat && (txcfg->pstat->cmn_info.bw_mode == CHANNEL_WIDTH_80
			...
			else if(txcfg->pstat && (txcfg->pstat->cmn_info.bw_mode >= CHANNEL_WIDTH_40
		...
		if (priv->pshare->is_40m_bw) {
			if (txcfg->pstat && (txcfg->pstat->cmn_info.bw_mode == CHANNEL_WIDTH_40)
...
//		priv->pshare->phw->bErpProtection = TRUE;
	...
	if (bRtsEnable || bCts2SelfEnable) {
		...
		pdesc->Dword4 |= set_desc((0xf & TX_DW4_RTS_RTY_LOWEST_RATE_MSK) << TX_DW4_RTS_RTY_LOWEST_RATE_SH);
	}
	...
	if (n_txshort == 1 && txcfg->pstat && (txcfg->pstat->cmn_info.ra_info.disable_ra == 1))
		pdesc->Dword5 |= set_desc(1 << TX_DW5_DATA_SHORT_SH);
...
//		pdesc->Dword0 |= set_desc(TX_OWN | TX_FirstSeg | TX_LastSeg);
		...
		pdesc->Dword4 |= set_desc((0x1f & TX_DW4_DATA_RTY_LOWEST_RATE_MSK) << TX_DW4_DATA_RTY_LOWEST_RATE_SH);
		if (txcfg->fixed_rate)
			pdesc->Dword3 |= set_desc((1 << TX_DW3_DISDATAFB_SH) | (1 << TX_DW3_DISRTSFB_SH)
					| (1 << TX_DW3_USERATE_SH));

		if (txcfg->pstat && (txcfg->pstat->cmn_info.ra_info.disable_ra != 1))
			pdesc->Dword3 |= set_desc(1 << TX_DW3_USERATE_SH);
...
//			pdesc->Dword1 &= set_desc( ~(TX_SecTypeMask<< TX_SecTypeSHIFT));
		...
		if (*da == 0xff) {
			pdesc->Dword2 |= set_desc(0x3f << 24);
			pdesc->Dword4 |= set_desc(0x1 << 17);
			pdesc->Dword4 &= set_desc(0xff03e0ff);
		}
...
#ifdef SDIO_AP_OFFLOAD
	if (priv->pshare->offload_function_ctrl >= RTW_PM_PREPROCESS) {
		pdesc->Dword0 |= set_desc(1 << 31);
...
out_err:
	priv->tx_beacon_len = 0;       // refilll Bcn desc
...
{
	u8 *rsv_buf;
	u32 deviceId, err, cnt;
	struct tx_desc *pdesc;
	struct xmit_buf xmitbuf;
	u32 buf_len = len + TXDESC_SIZE;

	rsv_buf = (u8 *)rtw_zmalloc(buf_len);
	if (!rsv_buf) {
		printk("[%s] failed to allocate rsvd buffer\n");
		return 0;
	}

	pdesc = (struct tx_desc *)rsv_buf;

	pdesc->Dword0 |= set_desc(TXDESC_SIZE << TX_DW0_OFFSET_SH);
	pdesc->Dword0 |= set_desc((len & TX_DW0_TXPKSIZE_MSK) << TX_DW0_TXPKSIZE_SH);
	pdesc->Dword1 |= set_desc(TXDESC_QSEL_BCN << TX_DW1_QSEL_SH);
	memcpy(rsv_buf + TXDESC_SIZE, pbuf, len);

	rtl8192cd_usb_cal_txdesc_chksum(pdesc);

	xmitbuf.agg_num = 1;
	xmitbuf.q_num = BEACON_QUEUE;
	xmitbuf.len = buf_len;
	xmitbuf.pkt_data = rsv_buf;

	deviceId = ffaddr2deviceId(priv, BEACON_QUEUE);
	sdio_write_port(priv, deviceId, buf_len, (u8 *)&xmitbuf);
	
	rtw_mfree(rsv_buf, len + TXDESC_SIZE);
	return 1;
}
