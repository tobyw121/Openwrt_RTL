...
#ifdef CONFIG_WLAN_HAL_8812FE
		if(GET_CHIP_VER(priv) == VERSION_8812F)
			SetBeamformLeave8812F(priv,Idx);
#endif

#ifdef CONFIG_WLAN_HAL_8814BE
		if(GET_CHIP_VER(priv) == VERSION_8814B)
			SetBeamformLeave8814B(priv,Idx);
#endif
...
#ifdef CONFIG_WLAN_HAL_8812FE
		if(GET_CHIP_VER(priv) == VERSION_8812F)
			SetBeamformStatus8812F(priv,Idx);
#endif

#ifdef CONFIG_WLAN_HAL_8198F
		if(GET_CHIP_VER(priv) == VERSION_8198F)
			SetBeamformStatus8198F(priv,Idx);
#endif

#ifdef CONFIG_WLAN_HAL_8814BE
		if(GET_CHIP_VER(priv) == VERSION_8814B)
			SetBeamformStatus8814B(priv,Idx);
#endif
...
{
	PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s] Enter\n", __FUNCTION__); 
#ifdef CONFIG_WLAN_HAL_8192EE
		if (GET_CHIP_VER(priv)== VERSION_8192E)	
...
#ifdef CONFIG_WLAN_HAL_8812FE
		if(GET_CHIP_VER(priv) == VERSION_8812F)
			SetBeamformEnter8812F(priv,Idx);
#endif

#ifdef CONFIG_WLAN_HAL_8198F
		if(GET_CHIP_VER(priv) == VERSION_8198F)
			SetBeamformEnter8198F(priv,Idx);
#endif

#ifdef CONFIG_WLAN_HAL_8814BE
		if(GET_CHIP_VER(priv) == VERSION_8814B)
			SetBeamformEnter8814B(priv,Idx);
#endif
...
#ifdef CONFIG_WLAN_HAL_8812FE
		if(GET_CHIP_VER(priv) == VERSION_8812F) {
			Beamforming_CSIRate_8812F(priv);
		}		
#endif
...
#ifdef CONFIG_WLAN_HAL_8812FE
	if(GET_CHIP_VER(priv) == VERSION_8812F) {
		Beamforming_NDPARate_8812F(priv, 0, 0, Rate);
	}		
#endif

#ifdef CONFIG_WLAN_HAL_8198F
		if(GET_CHIP_VER(priv) == VERSION_8198F)
			Beamforming_NDPARate_8198F(priv, Mode, BW, Rate);
#endif

#ifdef CONFIG_WLAN_HAL_8814BE
	if(GET_CHIP_VER(priv) == VERSION_8814B) {
		Beamforming_NDPARate_8814B(priv, 0, 0, priv->pshare->rf_ft_var.force_ndpa_rate);
	}		
#endif
	...
	)
{
	...
	if(get_rf_mimo_mode(priv) == RF_4T4R)
		Ntx = 3;
	else if(get_rf_mimo_mode(priv) == RF_3T4R)
		Ntx = 2;
	else if(get_rf_mimo_mode(priv) == RF_3T3R)
		Ntx = 2;
	else if(get_rf_mimo_mode(priv) == RF_2T4R)
		Ntx = 1;
	else if(get_rf_mimo_mode(priv) == RF_2T3R)
		Ntx = 1;
...
#ifdef CONFIG_WLAN_HAL_8814BE
	u1Byte set_ndp_rate = priv->pshare->rf_ft_var.set_ndp_rate;
	if ((GET_CHIP_VER(priv)== VERSION_8814B) && (set_ndp_rate != 0)) {
		if (set_ndp_rate < 12)
			NDPTxRate = dot11_rate_table[set_ndp_rate];
		else {
			if (set_ndp_rate >= 44)
				NDPTxRate = VHT_RATE_ID + (set_ndp_rate - 44);
			else
				NDPTxRate = HT_RATE_ID + (set_ndp_rate - 12);
		}
		return NDPTxRate;
	}
#endif
...
#ifdef CONFIG_WLAN_HAL_8814BE
	if(GET_CHIP_VER(priv)== VERSION_8814B)
		Nr_index = TxBF_Nr(halTxbf8814B_GetNtx(priv), CompSteeringNumofBFer);		/*find Nr*/
	else
#endif
		Nr_index = TxBF_Nr(1, CompSteeringNumofBFer);		/*find Nr*/
	...
	)
{
	...
	PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]%s Start!\n", __FUNCTION__);

	PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]%s, RA= 0x %x%x%x%x%x%x\n",
		__FUNCTION__,
		RA[0],RA[1],RA[2],RA[3],RA[4],RA[5]);

	PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]%s, DelBFeeEntry_Idx0= 0x %x%x%x%x%x%x\n",
	__FUNCTION__,
    ...
    if( (isEqualMACAddr(RA, pBeamformingInfo->DelEntryListByMACAddr.BFeeEntry_Idx0)) ){
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, (memcmp(RA, pBeamformingInfo->DelEntryListByMACAddr.BFeeEntry_Idx0, MACADDRLEN)) == 1, return NULL!!\n", __FUNCTION__);
        return NULL;
    }
    else if((isEqualMACAddr(RA, pBeamformingInfo->DelEntryListByMACAddr.BFeeEntry_Idx1))){
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, (memcmp(RA, pBeamformingInfo->DelEntryListByMACAddr.BFeeEntry_Idx1, MACADDRLEN)) == 1, return NULL!!\n", __FUNCTION__);
        return NULL;
    }

	for(i = 0; i < BEAMFORMEE_ENTRY_NUM; i++)
	{
		//dbg_print("Check Point 1: BeamformeeEntry[%d].bUsed = %d\n", i, pBeamformingInfo->BeamformeeEntry[i].bUsed);
	...
	)
{
    ...
    if((isEqualMACAddr(RA, pBeamformingInfo->DelEntryListByMACAddr.BFerEntry_Idx0))){
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, (memcmp(RA, pBeamformingInfo->DelEntryListByMACAddr.BFerEntry_Idx0, MACADDRLEN)) == 1, return NULL!!\n", __FUNCTION__);
        return NULL;
    }
    else if((isEqualMACAddr(RA, pBeamformingInfo->DelEntryListByMACAddr.BFerEntry_Idx1))){
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, (memcmp(RA, pBeamformingInfo->DelEntryListByMACAddr.BFerEntry_Idx1, MACADDRLEN)) == 1, return NULL!!\n", __FUNCTION__);
        return NULL;
    }
...
//eric-mu ??
#if 1
	if ((GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)) { //MU-MIMO: to be done
		PRT_BEAMFORMING_INFO pBeamformingInfo = &(priv->pshare->BeamformingInfo);
		PRT_BEAMFORMING_ENTRY	pEntry;
		u1Byte idx;
		
		pstat->cmn_info.bf_info.g_id = 63;
		for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {
			pEntry = &(pBeamformingInfo->BeamformeeEntry[idx]);
			
			if (pstat->cmn_info.aid == pEntry->AID) {
				pstat->cmn_info.bf_info.p_aid = pEntry->P_AID;
				break;
			}
		}
	}else
	...
	{
		u2Byte	AID = (u2Byte) ((pstat->cmn_info.aid) & 0x1ff); 		//AID[0:8]
		u2Byte	bssid = 0;	

		pstat->cmn_info.bf_info.g_id = 63;
		
		bssid = ((BSSID[5] & 0xf0) >> 4) ^ (BSSID[5] & 0xf);	// BSSID[44:47] xor BSSID[40:43]
		pstat->cmn_info.bf_info.p_aid = (AID + bssid * 32) & 0x1ff;		// (dec(A) + dec(B)*32) mod 512		

	}
	else if (OPMODE & WIFI_ADHOC_STATE)
	{
		pstat->cmn_info.bf_info.p_aid = REMAP_AID(pstat);
		pstat->cmn_info.bf_info.g_id = 63;

	}
	else if (OPMODE & WIFI_STATION_STATE)
	{
		pstat->cmn_info.bf_info.g_id = 0;		
		pstat->cmn_info.bf_info.p_aid = ((int)(pstat->cmn_info.mac_addr[5])<<1) | (pstat->cmn_info.mac_addr[4]>>7);
	}	
}
	...
	)
{
	...
	PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]Get BFee Idx = %d\n", __FUNCTION__, *Idx);

	if(pEntry != NULL)
	{	

		if((GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)){
		...
		pEntry->priv = getBeamEntryDev(priv, pEntry);
		if(pEntry->priv == NULL)
			return NULL;
...
#else
		pEntry->SoundPeriod = priv->pshare->rf_ft_var.soundingPeriod/100;
#endif
	...
	if(pEntry != NULL)
	{
		...
		if((GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)){
			...
			if ((pBeamInfo->BeamformCap & BEAMFORM_CAP_VHT_MU_BFEE) &&
				...
				pEntry->AID = AID;
				PHYDM_DBG(ODMPTR, DBG_TXBF, "Add BFer entry beamformer_mu_cnt=%d\n", pBeamInfo->beamformer_mu_cnt);
			} 
			else 
			#endif						
			if ((pBeamInfo->BeamformCap & (BEAMFORMEE_CAP_VHT_SU|BEAMFORMEE_CAP_HT_EXPLICIT)) &&
				...
				PHYDM_DBG(ODMPTR, DBG_TXBF, "Add BFer entry beamformer_su_reg_maping=%#X, su_reg_index=%d\n", pBeamInfo->beamformer_su_reg_maping, pEntry->su_reg_index);
			}
		}
			PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, BFer Entry= %d\n", __FUNCTION__, *Idx);
	...
	)
{
    ...
    if (pBFerEntry != NULL && (CurDelBFerBFeeEntrySel & BFerEntry)) // ((CurDelBFerBFeeEntrySel == BFerEntry) || (CurDelBFerBFeeEntrySel == BFerBFeeEntry))
    {
    ...
    if (pBFeeEntry != NULL && (CurDelBFerBFeeEntrySel & BFeeEntry)) // ((CurDelBFerBFeeEntrySel == BFeeEntry) || (CurDelBFerBFeeEntrySel == BFerBFeeEntry))
    {
	...
	)
{
	...
	if (pSTA != NULL) {
		 if(OPMODE & WIFI_AP_STATE)
			AID = pSTA->cmn_info.aid;
		 else
		 	AID = pSTA->assocID;
		RA  = pSTA->cmn_info.mac_addr;
		MacID = pSTA->cmn_info.aid;

		WirelessMode = pSTA->wireless_mode;
		BW = pSTA->tx_bw;
	} else	// Client mode
		return FALSE;

	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, WirelessMode = %x, txbf_cap = %08x\n", __FUNCTION__, WirelessMode, cpu_to_le32(pSTA->ht_cap_buf.txbf_cap));
...
// BIT 3 implies capable of receiving NDPA (BFEE),
		...
		if(WirelessMode & WIRELESS_MODE_AC_5G || WirelessMode & WIRELESS_MODE_AC_24G)
		{
			...
			#if (MU_BEAMFORMING_SUPPORT == 1)
			if((GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8812F || GET_CHIP_VER(priv) == VERSION_8814B) && priv->pmib->dot11RFEntry.txbf_mu){
				
				u1Byte bfer_permit = 0;
#ifdef RTK_AC_SUPPORT
				if ((GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8812F) && get_sta_vht_mimo_mode(pSTA) == RF_1T1R)
					bfer_permit = 1;
				else if (GET_CHIP_VER(priv) == VERSION_8814B && 
					(get_sta_vht_mimo_mode(pSTA) == RF_1T1R || get_sta_vht_mimo_mode(pSTA) == RF_2T2R))
					bfer_permit = 1;
#endif				
				if(pSTA->vht_cap_len && bfer_permit && (cpu_to_le32(pSTA->vht_cap_buf.vht_cap_info) & BIT(MU_BFEE_S)) && (priv->pmib->dot11RFEntry.txbfer == 1)) {
	...
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, CompSteeringNumofBFer = %d, NumofSoundingDim = %d, BeamformCap=%d\n", __FUNCTION__, CompSteeringNumofBFer, NumofSoundingDim, BeamformCap);
// bfme
	if((BeamformCap & BEAMFORMEE_CAP_HT_EXPLICIT) || (BeamformCap & BEAMFORMEE_CAP_VHT_SU))
	{
		...
		if(pBeamformerEntry == NULL)
		{
			...
			if(pBeamformerEntry == NULL) {
				PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, Not enough BFer entry!\n", __FUNCTION__);
...
//bfer
	if(((BeamformCap & BEAMFORMER_CAP_HT_EXPLICIT) || (BeamformCap & BEAMFORMER_CAP_VHT_SU)) && (pSTA->IOTPeer != HT_IOT_PEER_INTEL))
	{
		...
		else
		{
			...
			{
				PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, Error State of Beamforming\n", __FUNCTION__);
				return FALSE;
			}	
			else
				pBeamformEntry->BeamformEntryState = BEAMFORMING_ENTRY_STATE_INITIALIZEING;
	...
	*BFerBFeeIdx = (BFerIdx<<4) | BFeeIdx;
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, BFerIdx=0x%x, BFeeIdx=0x%x, BFerBFeeIdx=0x%x \n", __FUNCTION__, BFerIdx, BFeeIdx, *BFerBFeeIdx);
	...
	)
{
	...
	for(Idx = 0; Idx < BEAMFORMEE_ENTRY_NUM; Idx++)
	{
		...
		{
			PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, Reset entry idx=%d\n", __FUNCTION__, Idx);
...
{
	u1Byte							SoundingBW = CHANNEL_WIDTH_20;
	RT_BEAMFORMING_ENTRY			Entry = pBeamInfo->BeamformeeEntry[Idx];
	RT_BEAMFORMING_PERIOD_INFO		BeamPeriodInfo = pBeamInfo->BeamformingPeriodInfo;
	...
	)
{
	...
	if(pBeamTimerInfo->Mode == SOUNDING_SW_VHT_TIMER || pBeamTimerInfo->Mode == SOUNDING_SW_HT_TIMER) 
	{
		odm_cancel_timer(ODMPTR, &priv->pshare->BeamformingTimer);

		if((GET_CHIP_VER(priv) == VERSION_8822B) || GET_CHIP_VER(priv) == VERSION_8812F){
			odm_cancel_timer(ODMPTR, &priv->pshare->BFSoundingTimeoutTimer);
	...
	)
{
	...
	if(IsDelete)
	{
		if(DeleteIdx<BEAMFORMEE_ENTRY_NUM)
		{
			...
			{
				PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, SW DeleteIdx is wrong!\n", __FUNCTION__); 
				return;
			}
		}
		else	
	...
	else
	{
		for(Idx = 0; Idx < BEAMFORMEE_ENTRY_NUM; Idx++)
		{
			...
			{
				PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, SW Reset entry %d\n", __FUNCTION__, Idx); 
	...
	)
{
...
#ifdef RTK_AC_SUPPORT	
	unsigned int txpkts = pEntry->pSTA->tx_pkts2_pre - pEntry->pSTA->tx_pkts2_pre2- (pEntry->pSTA->tx_fail_pre - pEntry->pSTA->tx_fail_pre2);
#endif
	...
	if(GET_CHIP_VER(priv)== VERSION_8814B)
		{	
		if (priv->pshare->rf_ft_var.Nsnding &&
			(not_snding_rate_interval(pEntry, _NSS4_MCS9_RATE_, _NSS4_MCS6_RATE_) || 
			 not_snding_rate_interval(pEntry, _NSS4_MCS3_RATE_, _NSS4_MCS0_RATE_) || 
			 not_snding_rate_interval(pEntry, _NSS3_MCS3_RATE_, _NSS3_MCS0_RATE_) || 
			 not_snding_rate_interval(pEntry, _NSS2_MCS2_RATE_, _NSS2_MCS0_RATE_) || 
			not_snding_rate_interval(pEntry, _NSS1_MCS2_RATE_, _NSS1_MCS0_RATE_))) {
			is_sounding = FALSE;
		}
	}
	else
	...
	if(priv->pshare->rf_ft_var.autoRspCsi == 0){
		//not response CSI report only in TX Nss2 8~9
	if(not_snding_rate_interval(pEntry, VHT_RATE_ID + ntx_num*10 + 9, VHT_RATE_ID + ntx_num*10+8)){
			RTL_W8(0x718, RTL_R8(0x718) & 0xFC);	//not Rsp Csi
	}else{		
			RTL_W8(0x718, RTL_R8(0x718) |0x3);	//Rsp CSi
		}
		
	}else if(priv->pshare->rf_ft_var.autoRspCsi == 1){
		//not reply CSI report in "TX Nss2 8~9 && tx/4 > rx   "
		if(not_snding_rate_interval(pEntry, VHT_RATE_ID + ntx_num*10 + 9, VHT_RATE_ID + ntx_num*10+8) &&
			((pEntry ->pSTA->tx_byte_cnt >> 2) >=  pEntry ->pSTA->rx_byte_cnt)  && txpkts > 36000 ){
			RTL_W8(0x718, RTL_R8(0x718) & 0xFC);	//not Rsp Csi
		}else{
			RTL_W8(0x718, RTL_R8(0x718) |0x3);	//Rsp CSi
		}
		
	}else if(priv->pshare->rf_ft_var.autoRspCsi == 2){
		RTL_W8(0x718, RTL_R8(0x718) & 0xFC);	//not Rsp Csi
	}else if(priv->pshare->rf_ft_var.autoRspCsi == 3){
		RTL_W8(0x718, RTL_R8(0x718) |0x3);	//Rsp CSi
	}
#endif
	...
	if (is_sounding)
		...
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, Not sounding, pEntry->Sounding_En = %d\n", __FUNCTION__, pEntry->Sounding_En);
	...
	)
{
	...
	if (pEntry->bUsed == FALSE) {
		...
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, pEntry->bUsed = FALSE\n", __FUNCTION__);
		return FALSE;
	} else {
...

		{
			...
			if (pstat && (pstat->state & WIFI_SLEEP_STATE)) {
				PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, SLEEP mode, no NDPA\n", __FUNCTION__);
				return FALSE;
			}
		}
...
//	
	if(Mode == SOUNDING_SW_HT_TIMER || Mode == SOUNDING_HW_HT_TIMER || Mode == SOUNDING_AUTO_HT_TIMER)
	{
		...
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, HT NDP Rate = %d\n", __FUNCTION__, NDPTxRate);
	} else {
		NDPTxRate = Beamforming_GetVHTNDPTxRate(priv, pEntry->CompSteeringNumofBFer);
		
		if (beamforming_by_rate_souding_decision(priv, pEntry)) {
			ret = SendVHTNDPAPacket(priv,RA, pEntry->AID, BW, pEntry->su_reg_index, NDPTxRate);
			if (ret)
				pEntry->Sounding_En = 1;
			else
				pEntry->Sounding_En = 0;
		...
		if(pEntry->Sounding_En == 0)
			Beamforming_End(priv,0);
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, VHT NDP Rate = %d\n", __FUNCTION__, NDPTxRate);
	}

	pEntry->bBeamformingInProgress = FALSE;

	if(ret == FALSE) {
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, failure sending NDPA for addr =\n", __FUNCTION__);
		pEntry->bBeamformingInProgress = TRUE;
		return FALSE;
	}
	...
	)
{
		...
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s Start!\n", __func__);
	
#if 1 //eric-mu
		Idx = pBeamInfo->BeamformeeCurIdx;
		bSounding = beamfomring_bSounding_8822B(pBeamInfo, &Idx);
#endif
		...
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, Before notify, bSounding=%d, Idx=%d\n", __func__, bSounding, Idx);
#if (MU_BEAMFORMING_SUPPORT == 1)
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s: beamformee_su_cnt = %d, beamformee_mu_cnt=%d\n", __func__, pBeamInfo->beamformee_su_cnt, pBeamInfo->beamformee_mu_cnt);
#endif
		...
		case BEAMFORMEE_NOTIFY_ADD_MU:
			PHYDM_DBG(ODMPTR, DBG_TXBF, "%s: BEAMFORMEE_NOTIFY_ADD_SU\n", __func__);
			beamforming_StartPeriod(priv, Idx);
		break;
	
		case BEAMFORMEE_NOTIFY_DELETE_SU:
		case BEAMFORMEE_NOTIFY_DELETE_MU:
			PHYDM_DBG(ODMPTR, DBG_TXBF, "%s: BEAMFORMEE_NOTIFY_DELETE_SU\n", __func__);
			if (pBeamTimerInfo->Mode == SOUNDING_FW_HT_TIMER || pBeamTimerInfo->Mode == SOUNDING_FW_VHT_TIMER) {
...
#endif
				{
					...
					PHYDM_DBG(ODMPTR, DBG_TXBF, "%s: No BFee left\n", __func__);
				}
			} else {
...
#if 1 //eric-mu
		Idx = pBeamInfo->BeamformeeCurIdx;
		bSounding = beamfomring_bSounding_8812F(pBeamInfo, &Idx);
#endif
	...
	)
{
	...
	if(GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8814B){
		Beamforming_Notify_8822B(priv);
		return;
	}
		
	if(GET_CHIP_VER(priv) == VERSION_8812F){
		Beamforming_Notify_8812F(priv);
		return;
	}
	...
	else if(pBeamInfo->BeamformState == BEAMFORMING_STATE_START_2BFee)
	{
		if(bSounding == BEAMFORMING_NOTIFY_ADD)
		{
			PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, should be block\n", __FUNCTION__);
	...
	)
{
	...
	PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s] Status = %d, BeamformEntryState=%d, pBFeeEntry->bUsed=%d, pBFeeEntry->Sounding_En = %d\n",
	 __FUNCTION__, Status,
	...
	{
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, VHT3SS 7,8,9, do not apply V matrix.\n", __FUNCTION__);
	...
	else if (Status == 1)        // Receive CSI successful
	{
		...
		Beamforming_SetBeamFormStatus(priv, (pBeamformingInfo->BeamformeeCurIdx));
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s], Sounding Success\n", __FUNCTION__);
	}
	else        // Receive CSI failure
	{
		...
		Beamforming_SetBeamFormStatus(priv, (pBeamformingInfo->BeamformeeCurIdx));
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s], Sounding Fail\n", __FUNCTION__);
	}

	pBFeeEntry->bBeamformingInProgress = FALSE;
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, pEntry->LogStatusFailCnt : %d\n", __FUNCTION__, pBFeeEntry->LogStatusFailCnt);

	//Receive CSI failure
	if (pBFeeEntry->LogStatusFailCnt > 50) {
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, LogStatusFailCnt > 50\n", __FUNCTION__);
       	if ((GET_CHIP_VER(priv)== VERSION_8814A) || (GET_CHIP_VER(priv)== VERSION_8197F) || (GET_CHIP_VER(priv)== VERSION_8198F))
			PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, 97F and 8814A don't remove entry\n", __FUNCTION__);
		else {
	...
	)
{
	...
	if (ret == FALSE)
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, BeamformingStart_V2 fail\n", __FUNCTION__);
			...
			if (pBeamformingTimerInfo->Mode == SOUNDING_SW_VHT_TIMER || pBeamformingTimerInfo->Mode == SOUNDING_SW_HT_TIMER) {
				if (shortenSoundingPeriod(priv)) {
					if (pBeamformingInfo->BeamformingPeriodState == 1 || pBeamformingInfo->BeamformingPeriodState == 2)
						odm_set_timer(ODMPTR, &priv->pshare->BeamformingTimer, pBeamformingTimerInfo->BeamPeriod/100);
					else   // pBeamformingInfo->BeamformingPeriodState == 3
						odm_set_timer(ODMPTR, &priv->pshare->BeamformingTimer, pBeamformingTimerInfo->BeamPeriod/200);
				}
				else
					odm_set_timer(ODMPTR, &priv->pshare->BeamformingTimer, pBeamformingTimerInfo->BeamPeriod);
	...
	)
{
	...
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, Init Timer\n", __FUNCTION__);
	...
	if((GET_CHIP_VER(priv)== VERSION_8822B) || (GET_CHIP_VER(priv)== VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)) {
		...
		priv->pshare->BeamformingTimer.function = beamform_SoundingTimerCallback;
...
#endif
	{
		...
		priv->pshare->BeamformingTimer.function = Beamforming_SWTimerCallback;
#elif defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		priv->pshare->BeamformingTimer.function = timer_event_timer_fn;
		INIT_TIMER_EVENT_ENTRY(&priv->pshare->BeamformingTimer_event, Beamforming_SWTimerCallback, (unsigned long)priv);
#endif
	}

	is_iqgen_setting_ok = beamforming_rf_iqgen_setting(priv);
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, is_iqgen_setting_ok = %d\n", __FUNCTION__, is_iqgen_setting_ok);
	...
	)
{
	...
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, Release Timer\n", __FUNCTION__);
}		
		...
		if((GET_CHIP_VER(priv)== VERSION_8822B) || (GET_CHIP_VER(priv)== VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)) {
			...
			//if(pBeamInfo->BeamformCap & (BEAMFORMER_CAP_HT_EXPLICIT|BEAMFORMER_CAP_VHT_SU|BEAMFORM_CAP_VHT_MU_BFER))
			{
				if(pBeamInfo->SoundingInfoV2.State == SOUNDING_STATE_NONE)
				{
					...
					PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, Set BeamformingTimer = 2000\n", __FUNCTION__);
					odm_set_timer(ODMPTR, &priv->pshare->BeamformingTimer, 2000); // Start sounding after 2 sec.
				}
			}
		}
	}	
	...
	)
{
	...
	TA[0] = TA[0] & 0xFE; 
        ...
        {
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s ,ClockResetTimes=%d, NDPALogSuccess=%d, clock reset is no longer needed!!\n",
			__FUNCTION__, 
        ...
        {
			 PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s ,ClockResetTimes=%d, NDPALogSuccess=%d, DeInit BFer Entry !!\n",
						__FUNCTION__,
						pBeamformerEntry->ClockResetTimes,
						pBeamformerEntry->NDPALogSuccess);
        ...
        PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s Start, Sequence=%d, NDPALogSeq=%d, NDPAPreLogSeq=%d, NDPALogRetryCnt=%d, ClockResetTimes=%d, NDPALogSuccess=%d\n",
					__FUNCTION__,
					Sequence,
	...
	else
	{
                ...
                if (pBeamformerEntry->NDPALogRetryCnt == 5)
                {
                    ...
                    PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s ,Clock Reset!!! ClockResetTimes=%d\n",
	                    __FUNCTION__, 
	                    pBeamformerEntry->ClockResetTimes);
#ifdef CONFIG_RTL_8812_SUPPORT	
	...
	)
{
	...
	//Duration = 2*aSifsTime + 44;
	Duration = 2*aSifsTime + 36 + (4 * mu_cnt);
	...
	*pLength = 17;
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {		
		...
		if (pEntry->bUsed && pEntry->is_mu_sta && pEntry->pSTA && pEntry->pSTA->isSendNDPA) {
			...
			STAInfo.NcIndex = Beamforming_GetSTANcIndex(pEntry->pSTA); 
			tmp16 = 0;
			memset(&tmp16, 0, sizeof(tmp16));
			
			input_value_16(&tmp16, 0, 11, STAInfo.AID);
			input_value_16(&tmp16, 12, 12, STAInfo.FeedbackType);
			input_value_16(&tmp16, 13, 15, STAInfo.NcIndex);
	...
	)
{
	...
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s() ==> bBFeeLeave(%d)\n", __func__, bBFeeLeave);
	...
	if(bBFeeLeave)
	{
		...
		for (i= 0; i < BEAMFORMEE_ENTRY_NUM; i++)
		{
			if (pBeamInfo->BeamformeeEntry[i].bUsed)
			{			
				PHYDM_DBG(ODMPTR, DBG_TXBF, "Idx(%d) SoundPeriod=%d!\n", i, pBeamInfo->BeamformeeEntry[i].SoundPeriod);
...
{
	if(priv->pshare->rf_ft_var.force_report_rate != 0)
		return priv->pshare->rf_ft_var.force_report_rate;
...
beamform_MUMatch(struct rtl8192cd_priv *priv, PRT_BEAMFORMING_ENTRY	pEntry, u1Byte *pairResult)
{
	...
	if(pstat->muPartner[0]) {
				...
				pstat->muPartner[0]->muFlagForAMSDU = 1;
				...
				pstat->muPartner[0]->mu_rate = ODMPTR->dm_ra_table.mu1_rate[groupIndex];
				pstat->muPartner[0]->mu_tx_rate = query_mu_vht_rate(pstat->muPartner[0]);
				
				if(pstat->mu_tx_rate > 0 && pstat->muPartner[0]->mu_tx_rate > 0) {
					pstat->mu_deq_num = 2* dqnum * (pstat->mu_tx_rate) / (pstat->mu_tx_rate + pstat->muPartner[0]->mu_tx_rate);
					pstat->muPartner[0]->mu_deq_num = 2* dqnum - pstat->mu_deq_num;
				} else {
					pstat->mu_deq_num = dqnum;
					pstat->muPartner[0]->mu_deq_num = dqnum;
				}
...
)
{
...

	{
			...
			if(bestCaseIdx >= 0) {
				for(j=0;j<numPair;j++) {
					...
					isCandidate = (pEntry->mu_reg_index != CandidateEntry[idx2]->mu_reg_index) && 
								  ...
								  (bitmap & bitmap_mask[pEntry->mu_reg_index] & bitmap_mask[CandidateEntry[idx2]->mu_reg_index]);
					if(isCandidate) {				
						pstat->muPartner_num = 1;
						pstat->muPartner[0] = CandidateEntry[idx2]->pSTA;				
						pstat->muPartner[0]->muPartner_num = 1;
						pstat->muPartner[0]->muPartner[0] = pstat;
		...
		if (GET_CHIP_VER(priv) == VERSION_8812F) {
			if(STAGrpCnt ){
				priv->pshare->rf_ft_var.isSTAGrp =1;				
			}else{
				priv->pshare->rf_ft_var.isSTAGrp =0;
			}
		}
#endif
	}
...
//OOLEAN						bAddSUSoundingList = FALSE;
	...
	for(i=0; i<BEAMFORMEE_ENTRY_NUM; i++)
	{
		if(pBeamInfo->BeamformeeEntry[i].bUsed)
		{
			...
			{		
				PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Invalid BFee idx(%d) Hw state=%d\n", __func__, i, pBeamInfo->BeamformeeEntry[i].HwState);
...
#if 1 //eric-mu
{
	...
	//PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: There are %d SU and %d MU BFees in this sounding period\n", __func__, SUIdx, MUIdx);
    ...
    )
{
	...
	u1Byte				ReportPollRate;
#endif	
	
	if (priv->pshare->rf_ft_var.disable_snd == 1) {
		if (!priv->pshare->rf_ft_var.manual_snd) {
			mw5_legacy_mod_timer(&priv->pshare->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(1000));
			return;
		}
		else
			priv->pshare->rf_ft_var.manual_snd = 0;
	}
	
	PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s] ===> pSoundingInfo->State = %d\n", __func__, pSoundingInfo->State);	
	...
	{
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Invalid State(%d) and return!\n", __func__, pSoundingInfo->State);
	}
	...
	do
	{
		if(pSoundingInfo->State == SOUNDING_STATE_INIT)
		{
			...
			if(rtStatus == RT_STATUS_FAILURE)
			{
				WaitCnt=0;
				pSoundingInfo->State = SOUNDING_STATE_NONE;
				PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: No BFees found. Set to SOUNDING_STATE_NONE \n", __func__);
				break;
			}
			...
			{			
				...
				if(WaitCnt < 500)
				{
					...
					odm_set_timer(ODMPTR, &priv->pshare->BeamformingTimer, 2); 
				} 
				else
				{
					WaitCnt=0;
					pSoundingInfo->State = SOUNDING_STATE_NONE;
					PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Wait changing Hw state timeout!!. Set to SOUNDING_STATE_NONE \n", __func__);
				}
				break;
			}
			...
			{
				WaitCnt=0;
				PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Unkown state! \n", __func__);
				break;
			}
		}
		...
		if((SUSoundingIdx < BEAMFORMEE_ENTRY_NUM))
		{
			...
			{ // SU Case
				...
				//PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set to SOUNDING_STATE_SU_START \n", __func__);
				...
				if (pEntry->pSTA && !(pEntry->pSTA->state & WIFI_SLEEP_STATE)) {			
					if(pEntry->BeamformEntryCap & BEAMFORMER_CAP_HT_EXPLICIT) { //eric-txbf
						if (beamforming_by_rate_souding_decision(priv, pEntry)) {
							...
							if (GET_CHIP_VER(priv) == VERSION_8814B)
								RTL_W32(0x42c, RTL_R32(0x42c) & ~BIT(29));
#endif
							NDPTxRate = Beamforming_GetHTNDPTxRate(priv, pEntry->CompSteeringNumofBFer);
							ret = SendHTNDPAPacket(pEntry->priv, pEntry->MacAddr, pEntry->BW, NDPTxRate);
							if (ret)
								pEntry->Sounding_En = 1;
							else
								pEntry->Sounding_En = 0;
...
#endif
							NDPTxRate = Beamforming_GetVHTNDPTxRate(priv, pEntry->CompSteeringNumofBFer);
							ret = SendVHTNDPAPacket(pEntry->priv,pEntry->MacAddr, pEntry->AID, pEntry->BW, pEntry->su_reg_index, NDPTxRate);
							if (ret)
								pEntry->Sounding_En = 1;
							else
								pEntry->Sounding_En = 0;
		...
		else
		{
			...
			if(pSoundingInfo->CandidateMUBFeeCnt > 0)
			{
				...
				PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set to SOUNDING_STATE_MU_START \n", __func__);
				...
				{ /* More than 1 MU STA*/											
...
#endif
					PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: cnt=%d \n", __func__, cnt);
					if((cnt == 1) && (AC_SIGMA_MODE == AC_SIGMA_NONE)) { //start mu sounding when 2 sta connected  for 11ac logo r2
						...
						if (pEntry->pSTA && !(pEntry->pSTA->state & WIFI_SLEEP_STATE)) {
							...
							SendVHTNDPAPacket_MU(pEntry->priv, pEntry->MacAddr, pEntry->AID, pEntry->BW, pEntry->mu_reg_index, NDPTxRate, 1, cnt);	
						}
					} else if (cnt > 1) {
						...
						for (idx = 0; idx < cnt; idx++) {
							...
							if (pEntry->pSTA) {
								if (pEntry->pSTA->state & WIFI_SLEEP_STATE) {
									ignore = 1;
									break;
								}
							}
						...
						ReportPollRate = Beamforming_GetReportPollTxRate(priv);
							...
							for(idx = 0; idx < cnt; idx++) {
								...
								if(idx == 0) {
									...
									SendVHTNDPAPacket_MU(pEntry->priv, pEntry->MacAddr, pEntry->AID, pEntry->BW, pEntry->mu_reg_index, NDPTxRate, 0, cnt);
								}
								else if(idx == cnt-1)
									SendReportPollPacket(pEntry->priv, pEntry->MacAddr, pEntry->BW, pEntry->mu_reg_index, ReportPollRate,TRUE,cnt);	
								else
									SendReportPollPacket(pEntry->priv, pEntry->MacAddr, pEntry->BW, pEntry->mu_reg_index, ReportPollRate,FALSE,cnt);									
							}
						}	
					} else if (cnt == 0) {						
		...
		if(bSetTimeoutTimer)
		{
			odm_set_timer(ODMPTR, &priv->pshare->BFSoundingTimeoutTimer, TimeoutPeriod); //eric-mu
			break;
			//PHYDM_DBG(ODMPTR, DBG_TXBF, "%s(): Set Timeout Timer %d\n", __func__, TimeoutPeriod);
...
VOID 
beamform_SoundingTimeout(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	PRT_BEAMFORMING_INFO 		pBeamInfo = &(priv->pshare->BeamformingInfo);
	PRT_SOUNDING_INFOV2			pSoundingInfo = &(pBeamInfo->SoundingInfoV2);
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_START)
	{
		if(pSoundingInfo->SUBFeeCurIdx < BEAMFORMEE_ENTRY_NUM)
			...
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Error Value of SUBFeeCurIdx = %d!\n", __func__, pSoundingInfo->SUBFeeCurIdx);
			return;
		}
		...
		{	// SU BFee
			...
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: The BFee entry[%d] is set to bSoundingTimeout!\n", __func__, pSoundingInfo->SUBFeeCurIdx);
			
			pSoundingInfo->State = SOUNDING_STATE_SOUNDING_TIMEOUT;
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set to SU SOUNDING_STATE_SOUNDING_TIMEOUT \n", __func__);
		}
	}
	else if(pSoundingInfo->State == SOUNDING_STATE_MU_START)
	{
		pSoundingInfo->State = SOUNDING_STATE_SOUNDING_TIMEOUT;
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set to MU SOUNDING_STATE_SOUNDING_TIMEOUT \n", __func__);
	}

	if(!beamform_IsLastSoundingPeer(priv))
	{				
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set next sounding start time to %d ms \n", __func__, NextSoundPeriod);
...
#if (MU_BEAMFORMING_SUPPORT == 1)
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]:SUSoundNumPerPeriod= %d, MUSoundNumPerPeriod= %d,  MinSoundingPeriod=%d \n", __func__, pSoundingInfo->SUSoundNumPerPeriod, pSoundingInfo->MUSoundNumPerPeriod, pSoundingInfo->MinSoundingPeriod);
#endif
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set to SOUNDING_STATE_INIT \n", __func__);
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set next sounding start time to %d ms \n", __func__, NextSoundPeriod);
			
			if(NextSoundPeriod)
				odm_set_timer(ODMPTR, &priv->pshare->BeamformingTimer, NextSoundPeriod);
			else
				beamform_SoundingTimerCallback(priv);
		}
		else
		{
			pSoundingInfo->State = SOUNDING_STATE_NONE;
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set to SOUNDING_STATE_NONE \n", __func__);
		}
	}
}
...
{	
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_START)
	{
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s(): SU entry[%d] sound down\n", __func__, pSoundingInfo->SUBFeeCurIdx);
		...
		if (!pEntry->bUsed) {
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s] WARNING! The entry[%d] bUsed=FALSE\n", __func__, pSoundingInfo->SUBFeeCurIdx);
			return;
		}
		...
		if (pEntry->bSoundingTimeout) {
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s] WARNING! The entry[%d] is bSoundingTimeout!\n", __func__, pSoundingInfo->SUBFeeCurIdx);
			pEntry->bSoundingTimeout = FALSE;
			return;
		}
		...
		else
		{
			...
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s] LogStatusFailCnt %d\n", __func__, pEntry->LogStatusFailCnt);
		}

		//pEntry->bBeamformingInProgress = FALSE;		
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set to SOUNDING_STATE_SU_SOUNDDOWN \n", __func__);
	}
}	 

u1Byte
Beamforming_GetSTANcIndex(struct sta_info *pstat)
{
#ifdef RTK_AC_SUPPORT
	u1Byte mimo = get_sta_vht_mimo_mode(pstat);
#else
	u1Byte mimo = RF_2T2R;
#endif
	u1Byte nc = 0;
