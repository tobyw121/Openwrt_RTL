...
Abstract:
	Defined RTL8192FE HAL Function
...
typedef enum _RTL8192F_C2H_EVT
{
	C2H_8192F_DBG = 0,
	C2H_8192F_LB = 1,
	C2H_8192F_TXBF = 2,
	C2H_8192F_TX_REPORT = 3,
	C2H_8192F_TX_RATE =4,
	C2H_8192F_BT_INFO = 9,
	C2H_8192F_BT_MP = 11,
	C2H_8192F_RA_RPT = 12,
#ifdef TXRETRY_CNT
	C2H_8192F_TX_RETRY = 13, //0x0D
#endif
	C2H_8192F_RA_PARA_RPT=14,
	C2H_8192F_EXTEND_IND = 0xFF,
	MAX_8192F_C2HEVENT
}RTL8192F_C2H_EVT;

typedef enum _RTL8192F_EXTEND_C2H_EVT
{
	EXTEND_C2H_8192F_DBG_PRINT = 0

}RTL8192F_EXTEND_C2H_EVT;
...
)
{
	...
	if (!HalPwrSeqCmdParsing88XX(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
			PWR_INTF_PCI_MSK, rtl8192F_card_enable_flow))
    {
...
)
{
	...
	if (_FALSE == HalPwrSeqCmdParsing88XX(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
			PWR_INTF_SDIO_MSK, rtl8192F_enter_lps_flow)) {
		RT_TRACE(COMP_INIT, DBG_SERIOUS, ("rtl8192F_enter_lps_flow\n"));
	}
	...
	if (_FALSE == HalPwrSeqCmdParsing88XX(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
			PWR_INTF_SDIO_MSK, rtl8192F_card_disable_flow)) {
		RT_TRACE(COMP_INIT, DBG_SERIOUS, ("rtl8192F_card_disable_flow\n"));
	}
...
{
	rtl8192cd_init_mbssid_usb(Adapter);
}
...
{
	rtl8192cd_stop_mbssid_usb(Adapter);
}
#endif

enum rt_status
SetMACIDSleep_8192FU(
    IN  HAL_PADAPTER Adapter,
    IN  BOOLEAN      bSleep,
    IN  u4Byte       aid
)
...
)
{
	...
	pHalFunc->InitPONHandler		= InitPON8192FU;
	pHalFunc->InitMACHandler		= InitMAC88XX;
	pHalFunc->InitFirmwareHandler	= InitFirmware88XX;

#if CFG_HAL_SUPPORT_MBSSID
	pHalFunc->InitMBSSIDHandler 	= InitMBSSID_8192FU;
	...
	//
#if CFG_HAL_SUPPORT_MBSSID
	pHalFunc->StopMBSSIDHandler 	= StopMBSSID_8192FU;
#endif  //CFG_HAL_SUPPORT_MBSSID
	pHalFunc->StopHWHandler 		= StopHW_8192FU;
	...
	//ISR Related
	//
	pHalFunc->InitIMRHandler = InitIMR_8192FU;
	pHalFunc->EnableIMRHandler = EnableIMR_8192FU;
	...
	pHalFunc->SetMACIDSleepHandler			=	SetMACIDSleep_8192FU;
	...
	pHalFunc->CAMProgramEntryHandler		=	CAMProgramEntry88XX_92F;
	...
	pHalFunc->UpdateHalMSRRPTHandler		= UpdateHalMSRRPT_8192FU;

	pHalFunc->C2HHandler					= C2HHandler88XX;
	pHalFunc->C2HPacketHandler				= C2HPacketHandler_92FU;
...
)
{
	switch(c2hCmdId)
	{
		case C2H_8192F_TXBF:
			...
			C2HTxBeamformingHandler_92F(priv, tmpBuf, c2hCmdLen);
#endif
			break;
		case C2H_8192F_TX_RATE:
#ifdef TXREPORT
			C2HTxTxReportHandler_92F(priv, tmpBuf, c2hCmdLen);

	#if  defined(CONFIG_USB_HCI)
		#ifdef TXRETRY_CNT
			notify_request_tx_retry(priv);
		#else
...
#endif
#ifdef BT_COEXIST
		case C2H_8192F_BT_INFO:
			C2HBTInfoHandler_92F(priv, tmpBuf, c2hCmdLen);
			break;
#endif
		default:
...
*/
	{
		...
		_C2HContentParsing92F(priv, c2hCmdId, c2hCmdLen, tmpBuf);
	}
}
	...
	)
{
	...
	{
		// Paath_A
		phy_set_rf_reg(priv, RF_PATH_A, RF_RCK_OS, 0xfffff,0x18000); // Select RX mode
		phy_set_rf_reg(priv, RF_PATH_A, RF_TXPA_G1, 0xfffff,0x0000f); // Set Table data
		phy_set_rf_reg(priv, RF_PATH_A, RF_TXPA_G2, 0xfffff,0x77f82); // Disable TXIQGEN in RX mode
		// Path_B
		phy_set_rf_reg(priv, RF_PATH_B, RF_RCK_OS, 0xfffff,0x18000); // Select RX mode
		phy_set_rf_reg(priv, RF_PATH_B, RF_TXPA_G1, 0xfffff,0x0000f); // Set Table data
		phy_set_rf_reg(priv, RF_PATH_B, RF_TXPA_G2, 0xfffff,0x77f82); // Disable TXIQGEN in RX mode
	}
	...
	if(bSelfBeamformer){
		phy_set_bb_reg(priv, 0x90c, 0xffffffff, 0x83321333);
		phy_set_bb_reg(priv, 0xa04, BIT31|BIT30, 0x3);
		...
		phy_set_bb_reg(priv, 0x90c, 0xffffffff, 0x81121313);
#ifdef RF_MIMO_SWITCH
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
	...
	)
{
	...
	SetBeamformRfMode92F(priv, pBeamformingInfo);
	...
	)
{
	...
	*/
    ...
    if(((pBeamformingInfo->BeamformerEntry[0]).BeamformEntryCap == BEAMFORMING_CAP_NONE)
        ...
        //PHYDM_DBG(ODMPTR, PHYDM_COMP_TXBF, "[Beamforming]@%s, All BeamformerEntryCap == NONE, STOP feedback CSI\n", __FUNCTION__);
	...
	)
{
	u2Byte					BeamCtrlVal;
	u4Byte					BeamCtrlReg;
//	struct _RT_BEAMFORMING_INFO* 	pBeamformingInfo = GET_BEAMFORM_INFO(&(Adapter->MgntInfo));
	struct _RT_BEAMFORMING_INFO* pBeamformingInfo = &(priv->pshare->BeamformingInfo);
...
//	platform_efio_write_2byte(Adapter, BeamCtrlReg, BeamCtrlVal);
...
//		if(priv->pmib->dot11RFEntry.txbf == 1)
		...
		{
//				BeamformingInit(priv, pstat->cmn_info.mac_addr, pstat->cmn_info.aid);
			if(pstat->vht_cap_len) //_eric_txbf
				BeamformingControl(priv, pstat->cmn_info.mac_addr, pstat->cmn_info.aid, 2, pstat->cmn_info.bw_mode);
			else
				BeamformingControl(priv, pstat->cmn_info.mac_addr, pstat->cmn_info.aid, 3, pstat->cmn_info.bw_mode);
		}
	...
	mw5_legacy_mod_timer(&priv->txbf_timer, jiffies + priv->pshare->rf_ft_var.soundingPeriod);

}
#endif

VOID
C2HTxBeamformingHandler_92F(
	struct rtl8192cd_priv *priv,
		pu1Byte			CmdBuf,
		u1Byte			CmdLen
)
...
VOID HW_VAR_HW_REG_TIMER_START_92F(struct rtl8192cd_priv *priv)
{
...
VOID HW_VAR_HW_REG_TIMER_INIT_92F(struct rtl8192cd_priv *priv, u2Byte t)
{
	panic_printk("[%d][%s]\n",__LINE__,__FUNCTION__);
	RTL_W8(0x164, 1);
	RTL_W16(0x15C, t);
...
VOID HW_VAR_HW_REG_TIMER_STOP_92F(struct rtl8192cd_priv *priv)
{
	panic_printk("[%d][%s]\n",__LINE__,__FUNCTION__);
	RTL_W8(0x15F, 0);
}
