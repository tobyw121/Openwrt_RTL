...
)
{
    ...
    pHalFunc->InitPONHandler        = InitPON_8192EU;
    pHalFunc->InitMACHandler        = InitMAC88XX;
    pHalFunc->InitFirmwareHandler   = InitFirmware88XX;
#if CFG_HAL_SUPPORT_MBSSID
    pHalFunc->InitMBSSIDHandler     = InitMBSSID_8192EU;
    ...
    //
#if CFG_HAL_SUPPORT_MBSSID
    pHalFunc->StopMBSSIDHandler     = StopMBSSID_8192EU;
#endif  //CFG_HAL_SUPPORT_MBSSID
    pHalFunc->StopHWHandler         = StopHW_8192EU;
    ...
    //ISR Related
    //
    pHalFunc->InitIMRHandler = InitIMR_8192EU;
    pHalFunc->EnableIMRHandler = EnableIMR_8192EU;
    ...
    pHalFunc->SetMACIDSleepHandler          =   SetMACIDSleep_8192EU;
    ...
    pHalFunc->UpdateHalMSRRPTHandler        = UpdateHalMSRRPT_8192EU;
	...
	)
{
	...
	*/	
    ...
    if(((pBeamformingInfo->BeamformerEntry[0]).BeamformEntryCap == BEAMFORMING_CAP_NONE)
        ...
        RT_TRACE(DBG_TXBF, DBG_LOUD, ("[Beamforming]@%s, All BeamformerEntryCap == NONE, STOP feedback CSI\n", __FUNCTION__));
...
//				BeamformingInit(priv, pstat->cmn_info.mac_addr, pstat->cmn_info.aid);
			if(pstat->vht_cap_len) //_eric_txbf
				BeamformingControl(priv, pstat->cmn_info.mac_addr, pstat->cmn_info.aid, 2, pstat->tx_bw);
			else
				BeamformingControl(priv, pstat->cmn_info.mac_addr, pstat->cmn_info.aid, 3, pstat->tx_bw);
		}
