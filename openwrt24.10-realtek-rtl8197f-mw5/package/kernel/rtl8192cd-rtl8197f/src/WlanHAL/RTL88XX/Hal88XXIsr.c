...
#if 0 // Flow Control
        if (pHalData->IntMask[0] & BIT_TXDMA_STOP_INT_MSK)
        	HAL_RTL_W32(REG_TXDMA_STOP_HIMR, pHalData->IntMask[9]);
        else
        	HAL_RTL_W32(REG_TXDMA_STOP_HIMR, 0);
        if (pHalData->IntMask[0] & BIT_TXDMA_START_INT_MSK)
        	HAL_RTL_W32(REG_TXDMA_START_HIMR, pHalData->IntMask[10]);
        else
        	HAL_RTL_W32(REG_TXDMA_START_HIMR, 0);
#endif
    }
...
)
{
    PHAL_DATA_TYPE              pHalData = _GET_HAL_DATA(Adapter);
    u1Byte                      result;
#if (IS_RTL8822C_SERIES || IS_RTL8812F_SERIES || IS_RTL8814A_SERIES || IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8821C_SERIES || IS_RTL8198F_SERIES || IS_RTL8814B_SERIES || IS_RTL8197G_SERIES || IS_RTL88XX_MAC_V4)
    u4Byte bit_hisr2_ind_mask, bit_hisr3_ind_mask;
#endif
    u4Byte bit_hisr1_ind_int=0;
...
#if (IS_RTL8814A_SERIES || IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8821C_SERIES || IS_RTL8198F_SERIES || IS_RTL8197G_SERIES)
	if ((GET_CHIP_VER(Adapter) == VERSION_8814A) || (GET_CHIP_VER(Adapter) == VERSION_8197F) || (GET_CHIP_VER(Adapter) == VERSION_8822B) || (GET_CHIP_VER(Adapter) == VERSION_8821C) || (GET_CHIP_VER(Adapter) == VERSION_8198F) || (GET_CHIP_VER(Adapter) == VERSION_8197G)) {
    ...
    pHalData->IntArray[0] &= (pHalData->IntMask[0] | BIT_RDU);

	if (pHalData->IntArray[0] & bit_hisr1_ind_int) {
		pHalData->IntArray[1] = HAL_RTL_R32(REG_HISR1);
		HAL_RTL_W32(REG_HISR1, pHalData->IntArray[1]);
		pHalData->IntArray[1] &= (pHalData->IntMask[1] | BIT_FOVW);
	} else {
...
0xB0 BIT22 indicate 0x1098 REG_TXDMA_STOP_HIMR    => 8814B add
0xB0 BIT23 indicate 0x10A4 REG_TXDMA_START_HIMR   => 8814B add
...
//       if ( _GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v2_v3_v4) {
        ...
        if((GET_CHIP_VER(Adapter) == VERSION_8822C) || (GET_CHIP_VER(Adapter) == VERSION_8812F)){
				...
				result = (result || (pHalData->IntArray[2]) || (pHalData->IntArray[3]));
        }
    ...
    if ( _GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v4) {
        ...
        if(pHalData->IntArray[0] & BIT_TXDMA_STOP_INT_MSK) {
            pHalData->IntArray[9] = HAL_RTL_R32(REG_TXDMA_STOP_HISR);
            HAL_RTL_W32(REG_TXDMA_STOP_HISR, pHalData->IntArray[9]);
            pHalData->IntArray[9] &= pHalData->IntMask[9];
        }else
            pHalData->IntArray[9] = 0;

        if(pHalData->IntArray[0] & BIT_TXDMA_START_INT_MSK) {
            pHalData->IntArray[10] = HAL_RTL_R32(REG_TXDMA_START_HISR);
            HAL_RTL_W32(REG_TXDMA_START_HISR, pHalData->IntArray[10]);
            pHalData->IntArray[10] &= pHalData->IntMask[10];
        }else
            pHalData->IntArray[10] = 0;
    }
#endif //IS_RTL88XX_MAC_V4
	...
	)
{
	...
	bit_pstimeout=0;bit_pstimeout1=0;bit_pstimeout2=0;bit_VIdok=0;
	bit_VOdok=0;bit_BEdok=0;bit_BKdok=0;bit_MGTdok=0;bit_HIGHdok=0;
#if IS_RTL88XX_MAC_V1_V2_V3
    if(_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1_v2_v3){
	...
	case HAL_INT_TYPE_GTIMER3:
        bResult = (pHalData->IntArray[0] & BIT_GTINT3) ? TRUE : FALSE;
		break;	
...
#if (IS_RTL8197F_SERIES) || (IS_RTL8198F_SERIES) || (IS_RTL8192F_SERIES) || (IS_RTL8814A_SERIES) || (IS_RTL8814B_SERIES) || (IS_RTL8821C_SERIES) || (IS_RTL8822B_SERIES) || (IS_RTL8822C_SERIES) || (IS_RTL8812F_SERIES) || (IS_RTL8197G_SERIES)
		if (IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter) || IS_HARDWARE_TYPE_8197G(Adapter) ||
	...
	)
{
    ...
    u4Byte bit_pstimeout,bit_pstimeout1,bit_pstimeout2;
    u4Byte bit_VIdok,bit_VOdok,bit_BEdok,bit_BKdok,bit_MGTdok,bit_HIGHdok;
	/*let V1~V3 as default value to avoid coverity warming*/
	...
	case HAL_INT_TYPE_GTIMER3:
		...
		case HAL_INT_TXDMA_STOP:
			...
			pHalData->IntMask[9] |= 0xf;
			break;

		case HAL_INT_TXDMA_START:
			pHalData->IntMask[0] |= BIT_TXDMA_START_INT_MSK;
			pHalData->IntMask[10] |= 0xf;
			break;
#endif
	...
	)
{
	...
	case HAL_INT_TYPE_GTIMER3:
		...
		case HAL_INT_TXDMA_STOP:
			pHalData->IntMask[0] &= ~BIT_TXDMA_STOP_INT_MSK;
			pHalData->IntMask[9] = 0;
			break;

		case HAL_INT_TXDMA_START:
			pHalData->IntMask[0] &= ~BIT_TXDMA_START_INT_MSK;
			pHalData->IntMask[10] = 0;
			break;
#endif
        ...
        case HAL_INT_TYPE_FS_TIMEOUT0:
#if (IS_RTL8197F_SERIES) || (IS_RTL8198F_SERIES) || (IS_RTL8192F_SERIES) || (IS_RTL8814A_SERIES) || (IS_RTL8814B_SERIES) || (IS_RTL8821C_SERIES) || (IS_RTL8822B_SERIES) || (IS_RTL8822C_SERIES) || (IS_RTL8812F_SERIES) || (IS_RTL8197G_SERIES)
			if (IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter) ||
				IS_HARDWARE_TYPE_8814B(Adapter) || IS_HARDWARE_TYPE_8821C(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter))
            pHalData->FtIntArray &= ~BIT_FS_PS_TIMEOUT0_EN;
#endif
            break;
...
*/
    ...
    HAL_RTL_W32(REG_HIMR1, pHalData->IntMask[1]);
#else
    HAL_RTL_W8(REG_HIMR0, (pHalData->IntMask[0] & ~ (BIT_RXOK | BIT_RDU)) & 0xff);
...
*/
    ...
    HAL_RESTORE_INT(flags);
#endif
    HAL_RTL_W8(REG_HIMR0, pHalData->IntMask[0] & 0xff);
    HAL_RTL_W32(REG_HIMR1, pHalData->IntMask[1]);
