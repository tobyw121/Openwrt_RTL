...
static int rtl8197f_spirom_probe(struct platform_device *pdev)
{
	...
	/*
	 * Do not enter SHEIPA user-command mode during early platform probing.
	 * Some RD05 cold/recovery boots stop immediately after the auto-map magic
	 * reads when WRDI/WREN is issued here.  Array reads are already valid, so
	 * register MTD first and calibrate non-destructively on the first actual
	 * erase/page-program request, when the controller and scheduler are stable.
	 */
	if (rom->write_enabled)
		dev_info(dev,
			 "SPI-NOR WEL calibration deferred until first bounded write/erase\n");
	...
	dev_info(dev, "RTL8197F SPI auto-map MTD registered%s\n",
		 rom->write_enabled ? " with lazy-calibrated bounded rootfs_data writes" : "");
	return 0;
}
