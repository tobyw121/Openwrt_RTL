...
#define DRV_VERSION			"1.4.11-sdk-mw5-fcs-uncached-v43.11"
 ...
 * the external RTL8367 protocol-9 tag before the DMA buffer while preserving
...
#define RTL_RTK_MW5_CPU_TAG_LEN	4
			   ...
			   dma_addr_t ring_dma, void *ring, u32 idx)
{
	 ...
	 *
	 * The MW5 DTS already requests the SDK KSEG1 bus-address form.  Use the
	 * matching CPU alias as well, while retaining the dma_alloc_coherent()
	 * pointer for allocation/free bookkeeping.
	 */
	if (of_machine_is_compatible("tenda,nova-mw5") &&
	    priv->desc_addr_kseg1)
					       ...
					       const char *reason)
{
	...
	/* Preserve the switch CPU tag after the source MAC for Linux DSA.  The
	 * collected RTL8197F + RTL8367R/C SDKs configure the non-VG MW5 path for
	 * the four-byte Realtek protocol-9 format, while RD05/RTL8367D retains the
	 * upstream eight-byte protocol-4 format.
	 *
	 * The vendor MW5 stack enables CFG_TX_CPUC_TAG because its TX path submits
	 * an untagged frame plus rtl865x descriptor/VLAN metadata and lets P0 create
	 * the external-switch CPU tag.  Linux DSA does the opposite: tag_rtl4_9
	 * already inserts the complete 0x8899/protocol-9 tag into the skb.  Leaving
	 * CFG_TX_CPUC_TAG enabled in that mode makes P0 parse/synthesise the tag a
	 * second time and the RTL8367 never transmits the reply on the selected
	 * user port.  v43.8 disables both P0 CPU-tag parser/generator bits on MW5:
	 * software DSA owns TX tagging, while RX prefers a wire-preserved 0x8899
	 * tag and falls back to reconstructing it from opts4[15:13] when this
	 * RTL8197FS revision strips it before DMA. PORT0_ROUTER_MODE remains set.
	 */
	...
	dev_info(priv->dev,
		 "%s p0 rgmii sdk: %s pcrp0=0x%08x p0gmii=0x%08x pitcr=0x%08x extpcr0=0x%08x maccr=0x%08x maccr1=0x%08x pad=0x%08x txdelay=%u rxdelay=%u cputag-rx=%u cputag-tx=%u preserve-tag=1 tag=%s rx-policy=%s txdp=0x1/dpext=0\n",
		 mw5 ? "mw5" : "rd05",
		 reason,
		 readl(priv->swcore + RTL_RTK_SWCORE_PCRP(0)),
		 readl(priv->swcore + RTL_RTK_SWCORE_P0GMIICR),
					     ...
					     unsigned int idx)
{
	...
	priv->rx_skb[idx] = skb;
	priv->rx_dma[idx] = dma;
	rtl8197f_rtk_rx_desc(priv, idx)->addr = cpu_to_le32(rtl8197f_rtk_hw_dma_addr(priv, dma));
	rtl8197f_rtk_rx_desc(priv, idx)->opts2 = 0;
	rtl8197f_rtk_rx_desc(priv, idx)->opts3 = 0;
	rtl8197f_rtk_rx_desc(priv, idx)->opts4 = 0;
	rtl8197f_rtk_rx_desc(priv, idx)->opts5 = 0;
	rtl8197f_rtk_desc_clear_padding(priv, rtl8197f_rtk_rx_desc(priv, idx));
	rtl8197f_rtk_rx_desc(priv, idx)->opts1 = cpu_to_le32(RTL_RTK_DESC_OWN |
					       ...
					       unsigned int idx, u32 opts1)
{
	...
	dev_info(priv->dev,
		 "mw5 tx-complete v43.11: idx=%u desc=%pad opts1=0x%08x cputpdcr0=0x%08x head=%u tail=%u\n",
		 idx, &desc_dma, opts1,
		 rtl8197f_rtk_read(priv, RTL_RTK_CPUTPDCR0),
...
static unsigned int rtl8197f_rtk_tx_clean(struct rtl8197f_rtknet_priv *priv)
{
	...
	/*
	 * RTL8197F OEM New_swNic uses USE_SWITCH_TX_CDP for the 24-byte TX
	 * descriptors.  CPUTPDCR0 points at the next descriptor the switch core
	 * owns/consumes; every software descriptor before that pointer is complete.
	 * Do not gate reclamation on OWN on MW5: the SDK explicitly warns that OWN
	 * can be stale with 24-byte descriptors on a 32-byte cache-line machine.
	 */
	if (mw5) {
	...
	while (priv->tx_tail != priv->tx_head) {
		...
		u32 opts1 = le32_to_cpu(READ_ONCE(desc->opts1));
		struct sk_buff *skb;
		...
		if (cdp_valid) {
			...
			if (opts1 & RTL_RTK_DESC_OWN)
				break;
			if (mw5)
				priv->mw5_tx_own_fallback_clean++;
		}
					       ...
					       struct sk_buff *skb)
{
	...
	/* Real MW5 captures prove this new-descriptor field: opts4[15:13]
	 * reports source port 1 for the LAN jack and port 3 for the WAN jack.
	 * Do not guess for any other value; leave the frame untouched and count
	 * it so future captures can extend the mapping safely.
	 */
	opts4 = le32_to_cpu(desc->opts4);
	...
	if (skb_tailroom(skb) < RTL_RTK_MW5_CPU_TAG_LEN &&
	    pskb_expand_head(skb, 0, RTL_RTK_MW5_CPU_TAG_LEN, GFP_ATOMIC)) {
		priv->mw5_rx_tag_expand_fail++;
		return -ENOMEM;
	}

	/* Convert a hardware-stripped frame back into the switch-to-CPU wire
	 * layout observed on MW5 before master eth_type_trans():
	 * DA | SA | 0x8899 | 0x0400/source-port | original EtherType | ...
	 *
	 * v43.8 incorrectly synthesised the CPU-to-switch protocol-9 word here.
	 * RX and TX are asymmetric on the real RTL8367 link.
	 */
	skb_put(skb, RTL_RTK_MW5_CPU_TAG_LEN);
	memmove(skb->data + 2 * ETH_ALEN + RTL_RTK_MW5_CPU_TAG_LEN,
		skb->data + 2 * ETH_ALEN,
		skb->len - RTL_RTK_MW5_CPU_TAG_LEN - 2 * ETH_ALEN);
	put_unaligned_be16(ETH_P_REALTEK, skb->data + 2 * ETH_ALEN);
	tag = RTL_RTK_MW5_RX_TAG_MARKER |
	      (source_port & RTL_RTK_MW5_RX_TAG_PORT_MASK);
	put_unaligned_be16(tag, skb->data + 2 * ETH_ALEN + sizeof(__be16));
	priv->mw5_rx_tag_synthesized++;

	if (priv->mw5_rx_tag_synthesized <= 32)
		dev_info(priv->dev,
			 "mw5 rxdesc-dsa v43.11: synth 0x8899 source-port=%u tag=0x%04x encap=0x%04x len=%u\n",
			 source_port, tag, proto, skb->len);
				...
				const char *reason)
{
	...
	dma_sync_single_for_cpu(priv->dev, priv->rx_dma[idx],
				priv->rx_buf_size, DMA_FROM_DEVICE);
	print_hex_dump(KERN_INFO, "mw5 rx-bad v43.11 data: ", DUMP_PREFIX_OFFSET,
		       16, 1, skb->data, min_t(u32, priv->rx_buf_size, 64),
		       false);
	dma_sync_single_for_device(priv->dev, priv->rx_dma[idx],
				   priv->rx_buf_size, DMA_FROM_DEVICE);
}
...
static int rtl8197f_rtk_rx_poll(struct rtl8197f_rtknet_priv *priv, int budget)
{
	...
	while (work_done < budget) {
		...
		/*
		 * Match RTL8197F OEM USE_SWITCH_RX_CDP semantics exactly:
		 *   sw_idx != hw_idx           -> descriptor is complete
		 *   sw_idx == hw_idx && OWN=0  -> RX runout completion
		 *   sw_idx == hw_idx && OWN=1  -> no packet available
		 * Only when CPURPDCR0 is invalid do we fall back to OWN alone.
		 */
		if (cdp_valid) {
			if (idx == hw_idx) {
				if (opts1 & RTL_RTK_DESC_OWN) {
				...
				priv->mw5_rx_runout_complete++;
			} else {
				priv->mw5_rx_cdp_complete++;
				if (opts1 & RTL_RTK_DESC_OWN)
					priv->rx_cdp_owned_advanced++;
			}
		} else {
			priv->mw5_rx_cdp_fallback++;
			if (opts1 & RTL_RTK_DESC_OWN) {
				priv->mw5_rx_own_wait++;
				break;
			}
		...
		dma_rmb();
		/* Re-read after the CDP snapshot/barrier before consuming metadata. */
		opts1 = le32_to_cpu(READ_ONCE(desc->opts1));
		old_skb = priv->rx_skb[idx];
		if (!old_skb) {
			priv->rx_missing_skb++;
		...
		if (of_machine_is_compatible("tenda,nova-mw5") &&
		    len > ndev->mtu + ETH_HLEN + VLAN_HLEN +
			  RTL_RTK_MW5_CPU_TAG_LEN + ETH_FCS_LEN) {
			rtl8197f_mw5_rx_bad(priv, idx, desc,
					    "oversize");
					      ...
					      struct sk_buff *skb)
{
	...
	if (!of_machine_is_compatible("tenda,nova-mw5") || skb->len < ETH_HLEN)
		return;

	outer = get_unaligned_be16(d + 2 * ETH_ALEN);
	if (outer == ETH_P_REALTEK && skb->len >= ETH_HLEN + RTL_RTK_MW5_CPU_TAG_LEN) {
		tag = get_unaligned_be16(d + 2 * ETH_ALEN + sizeof(__be16));
		encap = get_unaligned_be16(d + 2 * ETH_ALEN + RTL_RTK_MW5_CPU_TAG_LEN);
	}
	...
	dev_info(priv->dev,
		 "mw5 tx-wire v43.11: pre-dma len=%u outer=0x%04x tag=0x%04x encap=0x%04x da=%pM sa=%pM\n",
		 skb->len, outer, tag, encap, d, d + ETH_ALEN);
	print_hex_dump(KERN_INFO, "mw5 tx-wire v43.11 raw: ", DUMP_PREFIX_OFFSET,
						...
						u32 cdp_before)
{
	...
	desc_dma = priv->tx_desc_dma + idx * priv->desc_stride;
	dev_info(priv->dev,
		 "mw5 tx-submit v43.11: idx=%u desc=%pad buf=%pad opts=%08x/%08x/%08x/%08x/%08x cdp=0x%08x->0x%08x own_after=%u head=%u tail=%u\n",
		 idx, &desc_dma, &dma, opts1, opts2, opts3, opts4, opts5,
		 cdp_before, cdp_after, !!(own_after & RTL_RTK_DESC_OWN),
		 priv->tx_head, priv->tx_tail);
}
						   ...
						   struct net_device *ndev)
{
	...
	/* Capture the exact DSA-tagged bytes before DMA mapping. */
	...
	/* On MW5, software head/tail plus the validated CPUTPDCR0 boundary own
	 * the free-space decision.  The OEM driver explicitly does not use the
	 * 24-byte TX descriptor OWN bit for reclamation because it can be stale.
	 */
	if (!of_machine_is_compatible("tenda,nova-mw5") &&
	    (le32_to_cpu(desc->opts1) & RTL_RTK_DESC_OWN)) {
		priv->tx_busy_desc++;
		netif_stop_queue(ndev);
		spin_unlock_irqrestore(&priv->tx_lock, flags);
		dma_unmap_single(priv->dev, dma, skb->len, DMA_TO_DEVICE);
	...
	/* OEM New_swNic does not set TX EOR when DMA_CR1/DMA_CR4 tail-aware
	 * mode is active. Keep the generic wrap bit for other boards only.
	 */
	if (!of_machine_is_compatible("tenda,nova-mw5") &&
	    idx == priv->tx_ring_size - 1)
		opts1 |= RTL_RTK_DESC_WRAP;

	rtl8197f_rtk_rd05_txdesc_trace(priv, skb, len, &txm, opts1, opts2, opts3, opts4, opts5);
	tx_cdp_before = rtl8197f_rtk_read(priv, RTL_RTK_CPUTPDCR0);
	...
	rtl8197f_rtk_assert_trxrdy(priv);
	rtl8197f_rtk_mw5_tx_submit_trace(priv, idx, dma, opts1, opts2,
					 opts3, opts4, opts5, tx_cdp_before);
...
static int rtl8197f_rtk_rd05_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "rx_internal bad_desc=%llu dma_errors=%llu crc_strips=%llu\n",
	...
	seq_printf(m, "mw5_rx_dsa passthrough=%llu synthesized=%llu expand_fail=%llu invalid_port=%llu oversize_drop=%llu own_wait=%llu cdp_complete=%llu runout_complete=%llu empty_wait=%llu cdp_fallback=%llu addr_mismatch=%llu desc_trace=%llu bad_snapshot=%llu\n",
	...
	seq_printf(m, "mw5_tx_dma submit=%llu complete=%llu own_after_kick=%llu cdp_clean=%llu cdp_owned_clean=%llu cdp_invalid=%llu own_fallback_clean=%llu\n",
	...
	seq_printf(m, "tx_internal kicks=%llu ring_full=%llu busy_desc=%llu timeouts=%llu\n",
...
static int rtl8197f_rtk_probe(struct platform_device *pdev)
{
	...
	if (rtl8197f_rtk_p0_external_switch_board()) {
		priv->rd05_proc = proc_create_data("rd05-rtknet", 0600, NULL,
