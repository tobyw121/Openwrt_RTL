...
#define READREG8(r)	*(volatile unsigned char *)(r)
#define WRITEREG8(r,v)	*(volatile unsigned char *)(r) = (unsigned char)(v)

#define KSEG1ADDR(_x)	(((_x) & 0x1fffffff) | 0xa0000000)

#if defined(CONFIG_BOARD_XIAOMI_R4_RD05) || \
    defined(CONFIG_BOARD_TENDA_NOVA_MW5) || \
    defined(CONFIG_BOARD_TENDA_AC23) || \
    defined(CONFIG_BOARD_REALTEK_RTL8197F_EVB) || \
    defined(CONFIG_BOARD_REALTEK_RTL8197F_UBOOT_TEST)
#define RTL8197F_LOADER 1
#define RTL8197F_SYS_CHIP_ID	0xb8000000U
#define RTL8197F_WDTCNR	0xb800311cU
#define RTL8197F_WDT_STOP	0xa5000000U
#define RTL8197F_WDT_CLEAR	0x00800000U
#define UART_BASE		0xb8147000U
#define UART_LSR_ADDR		(UART_BASE + 0x14)
#define UART_THR_F_ADDR	(UART_BASE + 0x24)
#define UART_THR_VG_ADDR	(UART_BASE + 0x00)
#define UART_LSR_THRE		0x20

static unsigned int rtl8197f_uart_thr(void)
{
	/* Vendor rtkload: plain RTL8197F/FS uses the +0x24 byte alias,
	 * RTL8197F-VG/FH-VG uses the standard +0x00 byte register.
	 */
	if ((READREG(RTL8197F_SYS_CHIP_ID) & 0xfffff000U) == 0x8197f000U)
		return UART_THR_F_ADDR;
	return UART_THR_VG_ADDR;
}

static void rtl8197f_watchdog_stop(void)
{
	unsigned int ctrl;

	/* Realtek SDK rtl819x_wdt_stop(): kick before the exact disable magic. */
	ctrl = READREG(RTL8197F_WDTCNR) | RTL8197F_WDT_CLEAR;
	WRITEREG(RTL8197F_WDTCNR, ctrl);
	__asm__ __volatile__("sync" : : : "memory");
	WRITEREG(RTL8197F_WDTCNR, RTL8197F_WDT_STOP);
	__asm__ __volatile__("sync" : : : "memory");
}
...
static int uart_wait_tx_ready(void)
{
	...
	while ((READREG8(UART_LSR_ADDR) & UART_LSR_THRE) == 0) {
		if (--timeout == 0)
			return 0;
	}
...
void board_putc(int ch)
{
	if (!uart_wait_tx_ready())
		return;
	WRITEREG8(rtl8197f_uart_thr(), ch);
	uart_wait_tx_ready();
}
#else
...
{
#ifdef RTL8197F_LOADER
	rtl8197f_watchdog_stop();
	board_putc('[');
	board_putc('L');
	board_putc('D');
	board_putc('R');
