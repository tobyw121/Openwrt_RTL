...
int no_check = 0;
	...
	if (bind(fd, atm ? (struct sockaddr *) &satm : (struct sockaddr *) &sinme, atm ? satm.sas_family == AF_ATMPVC ?
	...
	if (trans) {
	    ...
	    if (connect(fd, atm ? (struct sockaddr *) &satm : (struct sockaddr *) &sinhim, atm ? sizeof(satm) :
	      sizeof(sinhim)) < 0)
		err("connect");
...
#else
	cp = (char *) "%Uuser %Ssys %Ereal %P %Xi+%Dd %Mmaxrss %F+%Rpf %Ccsw";
#endif
#endif
	for (; *cp; cp++)  {
		if (*cp != '%')
...
int count;
{
	...
	if( udp )  {
#if 0
		cnt = recvfrom(_fd, _buf, count, 0, &from, &len );
#else
		cnt = recv(_fd, _buf, count, 0);
#endif
		numCalls++;
	} else {
		if( b_flag )
			cnt = mread( _fd, _buf, count );	/* fill buf */
		else {
...
int count;
{
    ...
    if (udp)  {
      again:
	if (atm) cnt = write(_fd, _buf, count);
	else cnt = sendto(_fd, _buf, count, 0, (struct sockaddr *) &sinhim, sizeof(sinhim));
	numCalls++;
