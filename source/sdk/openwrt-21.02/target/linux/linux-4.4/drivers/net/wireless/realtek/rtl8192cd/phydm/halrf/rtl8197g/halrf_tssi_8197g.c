...
void _backup_bb_registers_8197g(
	void *dm_void,
	u32 *reg,
	u32 *reg_backup,
	u32 reg_num)
...
void _reload_bb_registers_8197g(
	void *dm_void,
	u32 *reg,
	u32 *reg_backup,
	u32 reg_num)
...
u8 _halrf_driver_rate_to_tssi_rate_8197g(
...
u8 _halrf_tssi_rate_to_driver_rate_8197g(
	void *dm_void, u8 rate)
{
	u8 driver_rate = 0;
	struct dm_struct *dm = (struct dm_struct *)dm_void;
...
void _halrf_calculate_txagc_codeword_8197g(
...
u32 _halrf_get_efuse_tssi_offset_8197g(
...
s8 _halrf_get_kfree_tssi_offset_8197g(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _hal_rf_ *rf = &dm->rf_table;
...
void _halrf_calculate_tssi_codeword_8197g(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
	       "======>%s samll_b = %d cart_version=%d path=%d\n",
	       __func__, samll_b, dm->cut_version, RF_PATH_A);
	...
	for (i = 0; i < TSSI_CODE_NUM; i++) {
		...
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "phydm_get_tx_power_mdbm = %d, rate=0x%x(%d) bandwidth=%d channel=%d rf_path=%d\n",
			phydm_get_tx_power_mdbm(dm, RF_PATH_A, rate, bandwidth, channel),
...
void _halrf_calculate_set_thermal_codeword_8197g(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	u32 thermal_offset_tmp = 0, thermal_offset_index = 0x10, tmp;
...
void _halrf_set_txagc_codeword_8197g(
...
void _halrf_set_tssi_codeword_8197g(
...
void _halrf_set_efuse_kfree_offset_8197g(
	void *dm_void)
{
#if 0
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	/*path s0*/
	/*2G CCK*/
	offset = (_halrf_get_efuse_tssi_offset_8197g(dm, 3) +
	 	_halrf_get_kfree_tssi_offset_8197g(dm)) & 0xff;
	...
	/*2G & 5G OFDM*/
	offset = (_halrf_get_efuse_tssi_offset_8197g(dm, 19) +
	 	_halrf_get_kfree_tssi_offset_8197g(dm)) & 0xff;
...
void _halrf_tssi_set_table_8197g(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	_halrf_calculate_set_thermal_codeword_8197g(dm);
	_halrf_calculate_tssi_codeword_8197g(dm);
	_halrf_set_tssi_codeword_8197g(dm, tssi->tssi_codeword);
}

void _halrf_thermal_init_8197g(
	void *dm_void)
{
#if 0
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	_halrf_calculate_set_thermal_codeword_8197g(dm);
	/*_halrf_calculate_tssi_codeword_8197g(dm);*/
	/*_halrf_calculate_txagc_codeword_8197g(dm, tssi->tssi_codeword, tssi->txagc_codeword);*/
	/*_halrf_set_txagc_codeword_8197g(dm, tssi->txagc_codeword);*/
	/*_halrf_set_tssi_codeword_8197g(dm, tssi->tssi_codeword);*/
	/*_halrf_set_efuse_kfree_offset_8197g(dm);*/
#endif
}

void _halrf_driver_flash_to_tssi_de_8197g(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	void *dm_void, u8 path)
{
	...
	odm_set_bb_reg(dm, tssi_setting[path], MASKDWORD, 0x703f0044);
...
void _halrf_tssi_init_8197g(
	void *dm_void, u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;

	/*tssi_init*/
	odm_set_bb_reg(dm, R_0x1e1c, 0x0000f000, 0x8);
	odm_set_bb_reg(dm, R_0x1e1c, 0x04000000, 0x1);
	odm_set_bb_reg(dm, R_0x1e1c, 0x80000000, 0x1);
	odm_set_bb_reg(dm, R_0x1de8, 0x80000000, 0x1);
	odm_set_bb_reg(dm, R_0x1ca4, 0x40000000, 0x0);
	odm_set_bb_reg(dm, R_0x1e7c, 0x40000000, 0x0);

	if (path == 0) {
		odm_set_bb_reg(dm, R_0x18f8, 0x80000000, 0x0);
		...
		/*odm_set_bb_reg(dm, R_0x189c, 0x000fff00, 0x000);*/

		if (dm->cut_version == ODM_CUT_A)
			odm_set_bb_reg(dm, R_0x18a8, 0x00007ffc, 0x1266);
		if (dm->cut_version == ODM_CUT_B)
			odm_set_bb_reg(dm, R_0x18a8, 0x00007ffc, 0x1166);

		odm_set_bb_reg(dm, R_0x189c, 0x3ff00000, 0x00);
		...
		odm_set_bb_reg(dm, R_0x1868, 0x08000000, 0x0);
		odm_set_bb_reg(dm, R_0x1868, 0xf0000000, 0x5);
		odm_set_bb_reg(dm, R_0x1800, 0x40000000, 0x1);
		odm_set_bb_reg(dm, R_0x189c, 0x40000000, 0x0);
		odm_set_bb_reg(dm, R_0x189c, 0x80000000, 0x0);
		odm_set_bb_reg(dm, R_0x18ec, 0x80000000, 0x1);

		if (dm->cut_version == ODM_CUT_A)
			odm_set_bb_reg(dm, R_0x1880, 0x00001fff, 0x1266);
		if (dm->cut_version == ODM_CUT_B)
			odm_set_bb_reg(dm, R_0x1880, 0x00001fff, 0x1166);

		odm_set_bb_reg(dm, R_0x1880, 0xffc00000, 0x00);
		odm_set_bb_reg(dm, R_0x1880, 0x003fe000, 0x100);
		...
		odm_set_bb_reg(dm, R_0x1ca4, 0x40000000, 0x0);
		odm_set_bb_reg(dm, R_0x1e7c, 0x40000000, 0x0);
		odm_set_bb_reg(dm, R_0x41f8, 0x80000000, 0x0);
		...
		/*odm_set_bb_reg(dm, R_0x419c, 0x000fff00, 0x000);*/

		if (dm->cut_version == ODM_CUT_A)
			odm_set_bb_reg(dm, R_0x1eec, 0x00001fff, 0x1266);
		if (dm->cut_version == ODM_CUT_B)
			odm_set_bb_reg(dm, R_0x1eec, 0x00001fff, 0x1166);

		odm_set_bb_reg(dm, R_0x419c, 0x3ff00000, 0x00);
		...
		odm_set_bb_reg(dm, R_0x4168, 0x08000000, 0x0);
		odm_set_bb_reg(dm, R_0x4168, 0xf0000000, 0x5);
		odm_set_bb_reg(dm, R_0x4100, 0x40000000, 0x1);
		odm_set_bb_reg(dm, R_0x419c, 0x40000000, 0x0);
		odm_set_bb_reg(dm, R_0x419c, 0x80000000, 0x0);
		odm_set_bb_reg(dm, R_0x41ec, 0x80000000, 0x1);

		if (dm->cut_version == ODM_CUT_A)
			odm_set_bb_reg(dm, R_0x4180, 0x00001fff, 0x1266);
		if (dm->cut_version == ODM_CUT_B)
			odm_set_bb_reg(dm, R_0x4180, 0x00001fff, 0x1166);

		odm_set_bb_reg(dm, R_0x4180, 0xffc00000, 0x00);
		odm_set_bb_reg(dm, R_0x4180, 0x003fe000, 0x100);
		odm_set_bb_reg(dm, R_0x1eec, 0x3fc00000, 0x00);
	}
}

void _halrf_tssi_init_de_8197g(
	void *dm_void, u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
...
void _halrf_tssi_rf_setting_8197g(
	void *dm_void, u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;

	/*tssi_rf setting*/
	odm_set_rf_reg(dm, path, RF_0x6e, 0x00002, 0x1);
	odm_set_rf_reg(dm, path, RF_0x6e, 0x001e0, 0x0);

	if (dm->cut_version == ODM_CUT_A)
		odm_set_rf_reg(dm, path, RF_0x6e, 0x03000, 0x3);
	if (dm->cut_version == ODM_CUT_B)
		odm_set_rf_reg(dm, path, RF_0x6e, 0x03000, 0x1);

	odm_set_rf_reg(dm, path, RF_0x6e, 0x30000, 0x0);
	odm_set_rf_reg(dm, path, RF_0x6e, 0x00800, 0x0);
	odm_set_rf_reg(dm, path, RF_0x6f, 0x00001, 0x0);
	odm_set_rf_reg(dm, path, RF_0x53, 0x70000, 0x7);
	odm_set_rf_reg(dm, path, RF_0x53, 0x00f00, 0xc);
	odm_set_rf_reg(dm, path, RF_0x6e, 0x0c000, 0x1);
	odm_set_rf_reg(dm, path, RF_0x6e, 0xc0000, 0x1);
}

void _halrf_tssi_set_de_8197g(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	if (dm->rf_calibrate_info.txpowertrack_control == 4) {
...
#if 0
		for (i = 0; i < MAX_PATH_NUM_8197G; i++) {
		...
		for (i = 0; i < MAX_PATH_NUM_8197G; i++) {
			...
			tmp = phydm_get_tssi_trim_de(dm, i);
			odm_set_bb_reg(dm, addr_d[i], addr_d_bitmask[i],
				(tmp & 0xff));

			RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				"==>%s OFDM 0x%x[%x] tssi_offset(%d)\n",
				__func__,
	...
	for (i = 0; i < MAX_PATH_NUM_8197G; i++) {
		...
		tssi_offest_de = (s8)_halrf_get_efuse_tssi_offset_8197g(dm, i);
...
void halrf_tssi_set_de_for_tx_verify_8197g(
	void *dm_void, u32 tssi_de, u8 path)
{
	...
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"==>%s OFDM 0x%x[%x] tssi_offset(%d)=tssi_de_tmp(%d)+tssi_trim(%d)\n",
		__func__,
	...
	void *dm_void)
{
	...
	u32 reg, dck_check;
	s32 reg_tmp, dck_check_tmp;
	u32 bb_reg[13] = {R_0x1e24, R_0x1e28, R_0x824, R_0x1808, R_0x4108,
			R_0x1864, R_0x4164, R_0x180c, R_0x410c, R_0x1d08,
			R_0x1e70, R_0x1b00, R_0x1a00};
	u32 bb_reg_backup[13] = {0};
	u32 backup_num = 13;

	//u32 tssi_setting[2] = {R_0x1830, R_0x4130};
	u32 dc_offset[2] = {R_0x189c, R_0x419c};
	u32 tssi_counter[2] = {R_0x18a4, R_0x41a4};
	u32 txchain_clk_gat_en[2] = {R_0x1860, R_0x4160};
	u32 debug_port[2] = {0x937, 0xb37};
	...
	_backup_bb_registers_8197g(dm, bb_reg, bb_reg_backup, backup_num);
	...
	for (i = 0; i < 2 ; i++){
		...
		for (k = 0; k < 3; k++) {
			...
			/*97G-tssi_anapar*/
			_halrf_tssi_anapar_8197g(dm, i);
		
			/*tssi_init*/
			_halrf_tssi_init_8197g(dm, i);

			/*tssi_rf setting*/
			_halrf_tssi_rf_setting_8197g(dm, i);

			odm_set_bb_reg(dm, dc_offset[i], 0x000fff00, 0x0);
			odm_set_bb_reg(dm, txchain_clk_gat_en[i], 0x00000800, 0x0);
			odm_set_bb_reg(dm, tssi_counter[i], 0x10000000, 0x0);
			odm_set_bb_reg(dm, txchain_clk_gat_en[i], 0x00000800, 0x1);
			...
			odm_set_rf_reg(dm, i, RF_0xde, 0x0000c, 0x3);
			odm_set_rf_reg(dm, i, RF_0x56, RFREGOFFSETMASK, 0x9dc00);
			...
			/*odm_set_bb_reg(dm, R_0x1c3c, 0x000fff00, 0x937);*/
			...
			if (reg & 0x00000200) {
				reg_tmp = (s32)(reg | 0xfffffc00);
				RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI] reg_tmp=0x%x reg=0x%x\n",
					reg_tmp, reg);
			} else
				reg_tmp = reg;

			odm_set_bb_reg(dm, dc_offset[i], 0x000fff00, (reg_tmp & 0xfff));
				
			RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI] 0x%x[19:8]=0x%x reg_tmp=0x%x\n",
				dc_offset[i], odm_get_bb_reg(dm, dc_offset[i], 0x000fff00), reg_tmp);
			...
			for (j = 0; j < 3; j++) {
				...
				phydm_release_bb_dbg_port(dm);
				if (dck_check & 0x00000200)
					dck_check_tmp = (s32)(dck_check | 0xfffffc00);
				else
					dck_check_tmp = dck_check;

				if (dck_check_tmp < 0x0) {
					reg_tmp = reg_tmp - 1;
					odm_set_bb_reg(dm, dc_offset[i], 0x000fff00, (reg_tmp & 0xfff));
					RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI] 0x2dbc[9:0]=0x%x < 0x0 Set 0x%x retry=%d\n",
						dck_check, reg_tmp, j);
				} else if (dck_check > 0x2) {
					reg_tmp = reg_tmp + 1;
					odm_set_bb_reg(dm, dc_offset[i], 0x000fff00, (reg_tmp & 0xfff));
					RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI] 0x2dbc[9:0]=0x%x > 0x2 Set 0x%x retry=%d\n",
						dck_check, reg_tmp, j);
				} else {
					RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI] 0x2dbc[9:0]=0x%x OK!!! retry=%d\n",
						dck_check, j);
					k = 3;
					break;
				}
		...
		odm_set_rf_reg(dm, i, RF_0xde, 0x0000c, 0x0);
	...
	_reload_bb_registers_8197g(dm, bb_reg, bb_reg_backup, backup_num);
	...
	void *dm_void, u8 path)
{
	...
	/*97G-tssi_anapar*/
	_halrf_tssi_anapar_8197g(dm, path);
		
	/*tssi_init*/
	_halrf_tssi_init_de_8197g(dm, path);

	/*tssi_rf setting*/
	_halrf_tssi_rf_setting_8197g(dm, path);
	...
	void *dm_void)
{
	...
	tssi_rate = _halrf_driver_rate_to_tssi_rate_8197g(dm, ODM_MGN_MCS7);
...
//	struct _halrf_tssi_data *tssi = &rf->halrf_tssi_data;
	...
	_halrf_driver_flash_to_tssi_de_8197g(dm);
}
...
//	struct _halrf_tssi_data *tssi = &rf->halrf_tssi_data;
	...
	halrf_disable_tssi_8197g(dm);
	_halrf_tssi_set_table_8197g(dm);

	for (i = 0; i < 2; i++)
		_halrf_tssi_init_8197g(dm, i);

	_halrf_tssi_set_de_8197g(dm);
	...
	u32 tssi_value)
{
	...
	u8 tssi_rate = _halrf_driver_rate_to_tssi_rate_8197g(dm, tx_rate);
	u8 rate = phydm_get_tx_rate(dm);
	s8 efuse, kfree;
	...
	_halrf_calculate_txagc_codeword_8197g(dm, tssi_codeword_tmp, txagc_codeword_tmp);
	_halrf_set_txagc_codeword_8197g(dm, txagc_codeword_tmp);
	_halrf_set_tssi_codeword_8197g(dm, tssi_codeword_tmp);

	kfree = _halrf_get_kfree_tssi_offset_8197g(dm);
...
void halrf_get_efuse_thermal_pwrtype_8197g(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
//	struct dm_rf_calibration_struct *cali_info = &(dm->rf_calibrate_info);
	...
	void *dm_void)
{
	...
	tssi_rate = _halrf_driver_rate_to_tssi_rate_8197g(dm, phydm_get_tx_rate(dm));
	tssi_codeword = tssi->tssi_codeword[tssi_rate];

	if (rate == ODM_MGN_1M || rate == ODM_MGN_2M || rate == ODM_MGN_5_5M || rate == ODM_MGN_11M) {
		efuse = _halrf_get_efuse_tssi_offset_8197g(dm, 3);
		kfree = _halrf_get_kfree_tssi_offset_8197g(dm);
	} else {
		efuse = _halrf_get_efuse_tssi_offset_8197g(dm, 19);
		kfree = _halrf_get_kfree_tssi_offset_8197g(dm);
	}
