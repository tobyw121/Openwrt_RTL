...
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_curr_bw(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_curr_bw(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif
{
	struct net_device *dev = PROC_GET_DEV();
	...
	if (priv->pshare->is_40m_bw == CHANNEL_WIDTH_160) {
		...
		PRINT_ONE((!priv->pshare->is_40m_bw) ? 20 :(priv->pshare->is_40m_bw*40), "%d", 1);
	}
...
#endif
{
	...
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_curr_bw);
...
void MDL_DEVINIT rtl8192cd_proc_init(struct net_device *dev)
{
	...
	RTK_CREATE_PROC_READ_ENTRY(p, "curr_bw", rtl8192cd_proc_curr_bw);
...
void /*__devexit*/MDL_EXIT rtl8192cd_proc_remove (struct net_device *dev)
{
	...
	remove_proc_entry("curr_bw", rtl8192cd_proc_root);
#ifdef RTL_MANUAL_EDCA
...
static struct _proc_table_ proc_table[] =
{
	...
	{"curr_bw",			rtl8192cd_proc_curr_bw,NULL},
#ifdef RTL_MANUAL_EDCA
