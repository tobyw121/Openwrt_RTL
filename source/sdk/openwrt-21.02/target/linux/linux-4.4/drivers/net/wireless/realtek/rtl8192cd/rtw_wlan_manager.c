...
void rtw_wlan_manager_recv_msg(struct nl_message *msg)
{
	...
	case NL_B_STEER_BLOCK_DEL_TYPE:
		while (offset < msg->len) {
			...
			case ELM_STA_INFO_ID:
				...
				break;
			default:
				panic_printk(KERN_DEBUG "[NETLINK] unknown element id.\n");
				break;
			}
			offset += hdr->len;
		}
	...
	case NL_B_STEER_ROAM_TYPE:
		...
		if (intf && roam_info) {
			...
			printk(KERN_DEBUG"[NETLINK] NL_B_STEER_ROAM_TYPE (hostapd_cli)!\n");
			printk(KERN_DEBUG"[NETLINK] sta_mac=%02x:%02x:%02x:%02x:%02x:%02x\n",
				roam_info->sta_mac[0], roam_info->sta_mac[1], roam_info->sta_mac[2],
				roam_info->sta_mac[3], roam_info->sta_mac[4], roam_info->sta_mac[5]);
			printk(KERN_DEBUG"[NETLINK] bss_mac=%02x:%02x:%02x:%02x:%02x:%02x bss_ch=%u method=%s\n",
...
#endif
	default:
		panic_printk(KERN_DEBUG "[NETLINK] unknown message type.\n");
		break;
	}
...
static void rtw_netlink_recv_msg(struct sk_buff *skb)
{
	...
	if (skb == NULL) {
		panic_printk(KERN_DEBUG "[NETLINK] skb is null.\n");
		return;
	}
...
static void rtw_netlink_send_msg(void *msg, u32 msg_len)
{
	...
	if (!skb) {
		panic_printk(KERN_DEBUG "[NETLINK] allocate skb failed.\n");
		goto func_return;
	}
	...
	if (!nlh) {
		panic_printk(KERN_DEBUG "[NETLINK] put netlink header failed.\n");
	...
	if (err < 0) {
		...
		panic_printk(KERN_DEBUG "[NETLINK] send netlink unicast failed.\n");
		goto func_return;
	}
...
void rtw_netlink_init(void)
{
	...
	if (!nl_sock)
		panic_printk(KERN_DEBUG "[NETLINK] create netlink falied.\n");
	else
		panic_printk(KERN_DEBUG"[NETLINK] create netlink success\n");
