...
static int rtl8192cd_stop_sw(struct rtl8192cd_priv *priv)
{
...
#ifdef RTK_MULTI_AP
	del_timer_sync(&priv->multiap_backhaul_steer_timer);
	del_timer_sync(&priv->multiap_unassoc_sta_timer);
...
#endif
#ifdef DFS
	del_timer_sync(&priv->dfs_cntdwn_timer);
#endif
#ifdef TCP_ACK_ACC
...
#endif
	{
		...
		for (i = 0; i < MBIDCAM_NUM_ALLOC; i++) {
			del_timer_sync(&priv->pshare->mclone_sta[i].reauth_timer);
			del_timer_sync(&priv->pshare->mclone_sta[i].reassoc_timer);
		}
#endif
...
#endif
#ifdef ADAPTIVITY_STATS_DEBUG
		del_timer_sync(&priv->pshare->ada_stats_timer);
#endif
	}
	...
	if (priv->pmib->dot11StationConfigEntry.dot11RadioMeasurementActivated) {
		del_timer_sync(&priv->rm_delay_timer);
	}
#endif
#ifdef CLIENT_MODE
...
#if defined(CONFIG_WLAN_HAL_8812FE) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8197G)
	if (dm->support_ic_type & ODM_RTL8812F || dm->support_ic_type & ODM_RTL8814B || dm->support_ic_type & ODM_RTL8197G)		
...
//Only ONE SLOT, always enable HIGH Power 
	...
	if (GET_CHIP_VER(priv) == VERSION_8197F || GET_CHIP_VER(priv) == VERSION_8197G){
		...
		priv->pmib->dot11RFEntry.rfe_type = 6;
#elif defined(CONFIG_SOC_RFE_TYPE_7)
		priv->pmib->dot11RFEntry.rfe_type = 7;
#elif defined(CONFIG_SOC_RFE_TYPE_8)
		priv->pmib->dot11RFEntry.rfe_type = 8;
#endif
...
// temp mark by eric for NETPCI
	...
	char baseDevName[8];
#endif
#if defined(CONFIG_RTK_MESH) || defined(WDS) || defined(EN_EFUSE) || defined(MULTI_MAC_CLONE)
	int i;
#endif
...
#endif
#ifdef ADAPTIVITY_STATS_DEBUG
		init_timer(&priv->pshare->ada_stats_timer);
		priv->pshare->ada_stats_timer.data = (unsigned long) priv;
		priv->pshare->ada_stats_timer.function = adaptivity_stats_timer;
#endif
#if (BEAMFORMING_SUPPORT == 1)
		init_timer(&priv->pshare->BeamformingTimer);	// timer function will init in open, per chip
#if defined(CONFIG_PCI_HCI)
...
#endif
		init_timer(&priv->pshare->BFSoundingTimeoutTimer);
...
#endif
#ifdef RTK_NL80211
		init_timer(&priv->pshare->remain_on_ch_timer);
...
//#endif
	...
	init_timer(&priv->ss_timer);	// site_survey timer
#if defined(CONFIG_PCI_HCI)
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
...
#ifdef RTK_MULTI_AP
	init_timer(&priv->multiap_backhaul_steer_timer);
	...
	init_timer(&priv->multiap_unassoc_sta_timer);
...
#endif
#if defined(INCLUDE_WPA_PSK) && !(defined(WIFI_HAPD) && !defined(HAPD_DRV_PSK_WPS) || defined(RTK_NL80211))
	init_timer(&priv->wpa_global_info->GKRekeyTimer);
...
#endif
#ifdef DOT11K
	init_timer(&priv->rm_delay_timer);
#if defined(CONFIG_PCI_HCI)
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
