...
#define DRV_VERSION			"1.1-rtl8367-family-v35"
...
#define RTL_RTK_P0GMIICR_RGTXC(_v)	(((_v) & 0x3U) << 18)
#define RTL_RTK_P0GMIICR_RGTXC_3	RTL_RTK_P0GMIICR_RGTXC(3)
...
struct rtl8197f_rtknet_priv {
	...
	bool rd05_board;
	bool rtl8367_dsa_master;
	bool rtl8367_legacy_master;
	bool rtl8367_external_master;
	u32 p0_rgmii_tx_delay;
	u32 p0_rgmii_rx_delay;
	u32 p0_rgtxc;
	u32 rx_poll_ms;
					       ...
					       const char *reason)
{
	...
	/* DSA mode preserves the RTL8367 8-byte protocol-4 tag after the source
	 * MAC. Legacy swconfig mode uses the same physical P0/RGMII cascade but
	 * leaves Realtek CPU-tag recognition disabled. The RTL8197F SDK default for
	 * external RTL83xx switches is TX0/RX5/RGTXC3; RTL8197FH-VG/RD05 uses RX6.
	 * Board DT may override all three timing fields after signal-integrity tests.
	 */
	...
	gmii |= RTL_RTK_P0GMIICR_RGTXC(priv->p0_rgtxc) |
		(priv->p0_rgmii_rx_delay & 0x7U);
	if (priv->p0_rgmii_tx_delay)
	...
	dev_info(priv->dev,
		 "rtl8197f p0 rgmii v35: %s pcrp0=0x%08x p0gmii=0x%08x pitcr=0x%08x maccr=0x%08x maccr1=0x%08x pad=0x%08x txdly=%u rxdly=%u rgtxc=%u cputag-rx=%u preserve-tag=%u txdp=0x1/dpext=0\n",
		 reason,
		 readl(priv->swcore + RTL_RTK_SWCORE_PCRP(0)),
		 readl(priv->swcore + RTL_RTK_SWCORE_P0GMIICR),
		 ...
		 priv->p0_rgmii_tx_delay, priv->p0_rgmii_rx_delay,
		 priv->p0_rgtxc, priv->p0_cpu_tag_passthrough,
		 priv->p0_cpu_tag_passthrough);
}
						 ...
						 const char *reason)
{
	...
	if (!priv->rtl8367_external_master || !priv->swcore)
		return;
...
{
	static const u8 rd05_lan_mac[ETH_ALEN] =
		{ 0xa4, 0xba, 0x70, 0x1b, 0xd4, 0x10 };
	const u8 *lan_mac = priv->rd05_board ? rd05_lan_mac : priv->ndev->dev_addr;
	u32 netif[5];

	if (!priv->rtl8367_external_master)
		return;
...
static void rtl8197f_rtk_rd05_seed_l2cpu(struct rtl8197f_rtknet_priv *priv)
{
	...
	static const u8 rd05_cpu_mac[ETH_ALEN] =
		{ 0x00, 0x00, 0x0a, 0x00, 0x00, 0x0f };
	const u8 *cpu_mac = priv->rd05_board ? rd05_cpu_mac : priv->ndev->dev_addr;
					       ...
					       const char *reason)
{
	...
	if ((actual & (RTL_RTK_CPUICR1_TXDESC_MASK |
	    ...
	    (expected & (RTL_RTK_CPUICR1_TXDESC_MASK |
		         RTL_RTK_CPUICR1_RXDESC_MASK)))
		dev_warn(priv->dev,
			 "rd05 poststart v34: CPUICR1 stride write rejected: expected=0x%08x actual=0x%08x\n",
			 expected, actual);
	else
		dev_info(priv->dev,
			 "rd05 poststart v34: %s cpuicr1=0x%08x rxbase=0x%08x txbase=0x%08x rxdesc0=0x%08x\n",
			 reason, actual,
...
static void rtl8197f_rtk_hw_start(struct rtl8197f_rtknet_priv *priv)
{
	...
	dev_info(priv->dev,
		 "rd05 cpuidma v34: cpuicr=0x%08x cpuicr1=0x%08x dma_cr0=0x%08x dma_cr1=0x%08x txringcr=0x%08x cpurpdcr0=0x%08x cpurmdcr0=0x%08x sirr=0x%08x macctrl1=0x%08x sysclk=0x%08x\n",
					 ...
					 const char *origin, int cmd)
{
	...
	dev_info(priv->dev,
		 "rd05 private ioctl/proc v34: origin=%s cmd=0x%x runs=%llu cpuicr=0x%08x cpuicr1=0x%08x dma_cr0=0x%08x cpurpdcr0=0x%08x cpurmdcr0=0x%08x sirr=0x%08x newdesc_rx=%d legacy_rx=%d\n",
...
static int rtl8197f_rtk_probe(struct platform_device *pdev)
{
	...
	struct rtl8197f_rtknet_priv *priv;
	bool dsa_master_prop;
	bool legacy_master_prop;
	struct net_device *ndev;
	int ret;
	...
	priv->dev = &pdev->dev;
	priv->ndev = ndev;
	priv->rd05_board = of_machine_is_compatible("xiaomi,r4-rd05");
	dsa_master_prop = np &&
		of_property_read_bool(np, "realtek,rtl8367-dsa-master");
	legacy_master_prop = np &&
		of_property_read_bool(np, "realtek,rtl8367-legacy-master");
	if (dsa_master_prop && legacy_master_prop) {
		dev_err(&pdev->dev,
			"rtl8367 DSA and legacy master modes are mutually exclusive\n");
		ret = -EINVAL;
		goto err_free_netdev;
	}
	priv->rtl8367_dsa_master = priv->rd05_board || dsa_master_prop;
	priv->rtl8367_legacy_master = legacy_master_prop;
	priv->rtl8367_external_master = priv->rtl8367_dsa_master ||
		priv->rtl8367_legacy_master;
	priv->p0_rgmii_tx_delay = 0;
	priv->p0_rgmii_rx_delay = priv->rd05_board ? 6 : 5;
	priv->p0_rgtxc = 3;
	...
	if (np) {
		...
		rtl8197f_rtk_read_u32_prop(np, "realtek,p0-rgmii-tx-delay",
					       &priv->p0_rgmii_tx_delay, 0, 1);
		rtl8197f_rtk_read_u32_prop(np, "realtek,p0-rgmii-rx-delay",
					       &priv->p0_rgmii_rx_delay, 0, 7);
		rtl8197f_rtk_read_u32_prop(np, "realtek,p0-rgtxc",
					       &priv->p0_rgtxc, 0, 3);
	...
	if (priv->rtl8367_external_master) {
		...
		/* DSA frames carry the rtl8_4 tag in-band; legacy swconfig frames do
		 * not. Force the parser mode from the selected master architecture so
		 * a stale DT property cannot accidentally double-tag legacy traffic.
		 */
		priv->p0_cpu_tag_passthrough = priv->rtl8367_dsa_master;
		/* RTL8197F plus an external RTL8367 has two cascaded switching
		 * domains. Seed only the SoC-internal P0 host-link path. Generic boards
		 * use the netdev MAC, while RD05 retains measured OEM bootstrap values.
		 */
	...
	if (priv->rtl8367_external_master) {
		 ...
		 * SoC-facing cascade link. The external switch endpoint is normally
		 * CPU6/EXT0 on C/RB-VB or CPU7/EXT1 on D/RB-VC. Direct TX to P0.
		 */
		...
		priv->tx_extspa = 0;
		dev_info(&pdev->dev,
			 "RTL8367 %s master v35: RTL8197F P0/RGMII TX%u/RX%u/RGTXC%u, direct DP=0x1, rtl8_4-tag=%u%s\n",
			 priv->rtl8367_dsa_master ? "DSA" : "legacy swconfig",
			 priv->p0_rgmii_tx_delay, priv->p0_rgmii_rx_delay,
			 priv->p0_rgtxc, priv->p0_cpu_tag_passthrough,
			 priv->rd05_board ? " (RD05 profile)" : "");
	...
	dev_info(&pdev->dev,
		 "registered %s: rx=%u tx=%u buf=%u desc-stride=%zu tx-port-mask=0x%x tx-dp-ext=0x%x tx-extspa=0x%x tx-hwlookup=%d tx-bridge=%d desc-kseg1=%d desc-phys=%d rx-poll=%d/%ums sdk-cpuif=%d sdk-swcore=%d legacy-pipeline=%d trxrdy=%d sdk-crc=%d rtl8367-external=%d dsa=%d legacy=%d p0-tx=%u p0-rx=%u p0-rgtxc=%u rd05=%d\n",
		 ...
		 priv->rtl8367_external_master, priv->rtl8367_dsa_master,
		 priv->rtl8367_legacy_master, priv->p0_rgmii_tx_delay,
		 priv->p0_rgmii_rx_delay, priv->p0_rgtxc, priv->rd05_board);
	return 0;
