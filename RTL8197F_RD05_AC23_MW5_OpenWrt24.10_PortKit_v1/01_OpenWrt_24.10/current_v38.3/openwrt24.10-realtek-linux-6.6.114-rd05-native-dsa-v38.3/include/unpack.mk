...
GZIP_DECOMPRESS=$(if $(wildcard $(STAGING_DIR_HOST)/bin/libdeflate-gzip),$(STAGING_DIR_HOST)/bin/libdeflate-gzip -dc,gzip -dc)
ZSTD_DECOMPRESS=$(if $(wildcard $(STAGING_DIR_HOST)/bin/zstdcat),$(STAGING_DIR_HOST)/bin/zstdcat,zstdcat)
...
ifeq ($(strip $(UNPACK_CMD)),)
  ifeq ($(strip $(PKG_CAT)),)
    ...
    ifeq ($(filter gz tgz,$(EXT)),$(EXT))
      ...
      DECOMPRESS_CMD:=$(GZIP_DECOMPRESS) $(DL_DIR)/$(PKG_SOURCE) |
    endif
    ...
    ifeq (zst,$(EXT))
      EXT:=$(call ext,$(PKG_SOURCE:.$(EXT)=))
      DECOMPRESS_CMD:=$(ZSTD_DECOMPRESS) $(DL_DIR)/$(PKG_SOURCE) |
    endif
  ...
  ifeq ($(strip $(UNPACK_CMD)),)
    ...
    ifeq ($(PKG_CAT),zcat)
      UNPACK_CMD=$(GZIP_DECOMPRESS) $(DL_DIR)/$(PKG_SOURCE) | $(TAR_CMD)
    endif
  endif
endif
