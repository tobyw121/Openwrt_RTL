...
phy_fiberTxDis_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    return (pHalCtrl->pPhy_ctrl[port]->pPhydrv->fPhydrv_fiberTxDis_set(unit, port, enable));
...
phy_fiberTxDisPin_set(uint32 unit, rtk_port_t port, uint32 data)
{
    ...
    RT_LOG(LOG_TRACE, MOD_HAL, "unit=%d, port=%d, data=%d", unit, port, data);
    ...
    return (pHalCtrl->pPhy_ctrl[port]->pPhydrv->fPhydrv_fiberTxDisPin_set(unit, port, data));
