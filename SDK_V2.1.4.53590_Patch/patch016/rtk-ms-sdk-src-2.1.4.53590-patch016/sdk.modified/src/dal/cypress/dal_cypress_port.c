...
typedef struct dal_cypress_mac_info_s {
    ...
    uint8   linkChange[RTK_MAX_NUM_OF_PORTS];
    uint8   linkSts[RTK_MAX_NUM_OF_PORTS];
...
/* Function Name:
 *      dal_cypress_port_10gLinkChange_handler
 * Description:
 *      Handle 10G link change event
 * Input:
 *      unit           - unit id
 *      port           - port id
 ...
 *      None
 */
int32 dal_cypress_port_10gLinkChange_handler(int32 unit)
{
    rtk_port_t          port;
    rtk_port_10gMedia_t media;
    int32               ret;
    uint8               linkChange, linkSts;
    ...
    for (port = HAL_GET_MIN_SERDES_PORT(unit); port <= HAL_GET_MAX_SERDES_PORT(unit); ++port)
    {
        ...
        linkChange = pMac_info[unit]->linkChange[port];
        pMac_info[unit]->linkChange[port] = FALSE;
        linkSts = pMac_info[unit]->linkSts[port];
        ...
        if (( ret = dal_cypress_port_10gMedia_get(unit, port, &media)) != RT_ERR_OK)
        {
            continue;
        }
        ...
        switch (media)
        {
            ...
            case PORT_10GMEDIA_DAC_300CM:
                if (PORT_LINKDOWN == linkSts)
                    ret = dal_cypress_port_10g_init(unit, port);
                else
                    ret = dal_cypress_port_10gSds_restart(unit, port);
                break;
            default:
                break;
        }
    ...
    return RT_ERR_OK;
}   /* end of dal_cypress_port_10gLinkChange_handler */

/* Function Name:
 *      _dal_cypress_port_linkChange_handler
 * Description:
 *      Handle link change notfiy
 * Input:
 *      unit           - unit id
 *      port           - port id
 *      new_linkStatus - new link status
 ...
 *      None
 */
int32 _dal_cypress_port_linkChange_handler(int32 unit, rtk_port_t port,
    rtk_port_linkStatus_t new_linkStatus)
{
    PORT_SEM_LOCK(unit);

    pMac_info[unit]->linkChange[port] = TRUE;
    pMac_info[unit]->linkSts[port] = new_linkStatus;

    PORT_SEM_UNLOCK(unit);

    return RT_ERR_OK;
}   /* end of _dal_cypress_port_linkChange_handler */
...
dal_cypress_port_init(uint32 unit)
{
    ...
    if (RTL8396M_CHIP_ID == HAL_GET_CHIP_ID(unit))
    {
        ...
        if ((ret = dal_linkMon_register(_dal_cypress_port_linkChange_handler)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
            return ret;
        }
    ...
    osal_memset(fiberRstSts, 0, sizeof(uint8) * 52);
...
int32
_dal_cypress_port_10gTxFifo_check(uint32 unit, uint32 port)
{
    uint32  val;
    ...
    return 1;
}   /* end of _dal_cypress_port_10gTxFifo_check */
...
dal_cypress_port_10gMedia_get(uint32 unit, rtk_port_t port, rtk_port_10gMedia_t *media)
{
    ...
    /* function body */
    PORT_SEM_LOCK(unit);
    ret = phy_8390_10gMedia_get(unit, port, media);
    PORT_SEM_UNLOCK(unit);
