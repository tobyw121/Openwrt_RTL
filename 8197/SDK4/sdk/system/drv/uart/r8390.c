...
static int32 r8390_serial_intr(void *isr_param);
 ...
 */
static void
r8390_serial_out(int reg_index, uint8 value)
{
...
r8390_uart_init(uint32 unit,drv_uart_rxcallback_t rxcallback,drv_uart_txcallback_t txcallback)
{
    ...
    r8390_uart_baudrate_set(unit, UART_BAUDRATE_19200);

    serial_info[unit].putRxChar = rxcallback;
    serial_info[unit].getTxChar = txcallback;
    
    while ((r8390_serial_in(UART_LSR) & UART_LSR_DR) != 0)
   {
	uint8 Data;
	Data = r8390_serial_in(UART_RX);
   }
    osal_isr_register(RTK_DEV_UART1, r8390_serial_intr, (void *)unit);
    //r8390_serial_out(UART_IER, UART_IER_RDI);
    return RT_ERR_OK;
...
{
	if (r8390_serial_in(UART_LSR) & UART_LSR_DR)
		return RT_ERR_OK;
	else
...
{
  while ((r8390_serial_in(UART_LSR) & UART_LSR_THRE) == 0);

	r8390_serial_out(UART_TX,data);
    return RT_ERR_OK;
} /* end of r8390_uart_putc */
...
r8390_uart_baudrate_get(uint32 unit, drv_uart_baudrate_t *pBaudrate)
{
    ...
    return RT_ERR_OK;
} /* end of r8390_uart_baudrate_get */
...
r8390_uart_baudrate_set(uint32 unit, drv_uart_baudrate_t baudrate)
{
  ...
  r8390_serial_out(UART_IER,0);
  r8390_serial_out(UART_LCR,(UART_LCR_DLAB | UART_LCR_WLEN8));
  r8390_serial_out(UART_DLL, 0);
	r8390_serial_out(UART_DLM, 0);
	r8390_serial_out(UART_DLL, (divisor& 0xff));
	r8390_serial_out(UART_DLM, (divisor >> 8));
	r8390_serial_out(UART_LCR,UART_LCR_WLEN8);
	r8390_serial_out(UART_FCR,FCRVAL);
	r8390_serial_out(UART_MCR,(MCRVAL|UART_MCR_TCRTLR));
	serial_info[unit].baudrate = baudrate;
  osal_time_sleep(1);
    return RT_ERR_OK;
} /* end of r8390_uart_baudrate_set */
 ...
 * Note:
 */
void   r8390_serial_starttx(uint32 unit)
{
	#if 0
  uint8 ier;
	ier = r8390_serial_in(UART_IER);
	ier |= UART_IER_THRI;
	r8390_serial_out(UART_IER, ier);
	#else
	r8390_serial_out(UART_IER, (UART_IER_RDI|UART_IER_THRI) );
	#endif
	return;
}
 ...
 */
void r8390_serial_clearfifo(uint32 unit)
{
	uint8 data;
...
static int32 r8390_serial_intr(void *isr_param)
{
	...
	iir = r8390_serial_in(UART_IIR);
  //printk("\n&&&&***** UART ISR LSR %.2x IIR %.2x*******&&&&&&&&&&&&&\n",lsr,iir); 
  cnt=0;
  while (!(iir & UART_IIR_NO_INT) && cnt < 15) 
  {  
 		lsr = r8390_serial_in(UART_LSR);
	  if (lsr & UART_LSR_DR)
	  {
	  	data = r8390_serial_in(UART_RX);
