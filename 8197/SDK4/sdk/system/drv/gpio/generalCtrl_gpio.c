    ...
    drv_generalCtrlGpio_devConf_t *pData)
{
    ...
    RT_LOG(LOG_FUNC_ENTER, MOD_GENERAL, "[%s():%d] (gpiodev=%d) access_mode=%d, address=%d\n", \
            __FUNCTION__, __LINE__, dev, pData->ext_gpio.access_mode, pData->ext_gpio.address );
    ...
    drv_generalCtrlGpio_pinConf_t *pData)
{
	...
	if(IS_INT_GPIO_DEV(dev))
	{
		...
		if((ret = drv_gpio_init(gpioId, pData->int_gpio.function, pin_direction, pData->int_gpio.interruptEnable))	!= RT_ERR_OK)
			return ret;
	}else{
#ifdef CONFIG_SDK_RTL8231
    ...
    uint32 data)
{
	...
	if(IS_INT_GPIO_DEV(dev))
	{
		...
		int_pin = (pinId % 8);
		gpioId = GPIO_ID(int_port, int_pin);
		if((ret = drv_gpio_dataBit_set(gpioId, data)) != RT_ERR_OK)
			return ret;		
	}else{
#ifdef CONFIG_SDK_RTL8231	
		RT_PARAM_CHK(!EXTERNAL_GPIO_PIN_CHK(pinId), RT_ERR_OUT_OF_RANGE);
    ...
    uint32 *pData)
{
	...
	if(IS_INT_GPIO_DEV(dev))
	{
		...
		int_pin = (pinId % 8);
		gpioId = GPIO_ID(int_port, int_pin);
		if((ret = drv_gpio_dataBit_get(gpioId, pData)) != RT_ERR_OK)
			return ret;		
	}else{
#ifdef CONFIG_SDK_RTL8231		
		RT_PARAM_CHK(!EXTERNAL_GPIO_PIN_CHK(pinId), RT_ERR_OUT_OF_RANGE);
