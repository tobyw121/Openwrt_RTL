...
{
    uint32  regVal = 0, chipId, chipRevId, chipId_msb16bit;
    int32   ret = RT_ERR_FAILED;
    ...
    if ((ret = drv_swcore_cid_get(unit, &chipId, &chipRevId)) != RT_ERR_OK)
        return RT_ERR_FAILED;
    chipId_msb16bit = chipId & 0xFFFF0000;
    if ((chipId_msb16bit == (RTL8352M_CHIP_ID & 0xFFFF0000)) || (chipId_msb16bit == (RTL8353M_CHIP_ID & 0xFFFF0000)) ||
        (chipId_msb16bit == (RTL8392M_CHIP_ID & 0xFFFF0000)) || (chipId_msb16bit == (RTL8393M_CHIP_ID & 0xFFFF0000)) ||
        (chipId_msb16bit == (RTL8396M_CHIP_ID & 0xFFFF0000)))
...
{
    uint32  regVal = 0, value;
    uint32  chipId, chipRevId, chipId_msb16bit;
    int32   ret = RT_ERR_FAILED;
