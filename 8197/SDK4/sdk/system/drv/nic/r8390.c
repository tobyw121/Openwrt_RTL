...
#endif
nic_collectArrayList_t      nicListHead;
uint8 rxFlag;
...
_cypress_nic_isr_handler(void *isr_param)
{
    ...
    if (cpu_iisr & INT_NTFY_DONE_MASK)
    {
    ...
    if (cpu_iisr & INT_NTFY_BUF_RUN_OUT_MASK)
    {
    ...
    if (cpu_iisr & INT_LOCAL_NTFY_BUF_RUN_OUT_MASK)
    {
...
static void _nic_rx_thread_bh(void *pInput)
{
    uint32  i;
    ...
    while(1)
    {
        osal_wait_event_interruptible(wq, rxFlag);
        rxFlag = 0;
...
r8390_init(uint32 unit, drv_nic_initCfg_t *pInitCfg)
{
    ...
    if ((osal_thread_t)NULL == (osal_thread_create("RTK NIC Rx Thread", NIC_RX_THREAD_STACK_SIZE, NIC_RX_THREAD_PRI, (void *)_nic_rx_thread_bh, NULL)))
    {
        osal_printf("RTK NIC Rx Thread create failed\n");
...
void tmpCallback(uint32 unit, drv_nic_pkt_t *pPacket, void *pCookie)
{
    return ;
}
