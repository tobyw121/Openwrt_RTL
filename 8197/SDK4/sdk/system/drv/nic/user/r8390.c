 ...
 *
 * $Revision: 29968 $
 * $Date: 2012-06-15 17:06:31 +0800 (Fri, 15 Jun 2012) $
 *
...
#define DMA_ADDR_KRN2USR(krn_addr)    (((uint32)krn_addr - (uint32)KRNVIRT(RTL8390_DMA_PHYS_BASE)) + dma_base)
#define DMA_ADDR_USR2KRN(usr_addr)    (((uint32)usr_addr - dma_base) + (uint32)KRNVIRT(RTL8390_DMA_PHYS_BASE))

/* Descriptor(ring and pktHdr) memory convert function */
#define DESC_ADDR_KRN2USR(unit, krn_addr)    (((uint32)krn_addr - (uint32)KRNVIRT(RTL8390_DESC_PHYS_BASE)) + desc_base[unit])
#define DESC_ADDR_USR2KRN(unit, usr_addr)    (((uint32)usr_addr - desc_base[unit]) + (uint32)KRNVIRT(RTL8390_DESC_PHYS_BASE))
...
#define RTL8390_MAX_REASON_NM  31
	...
	else if (reason <= RTL8390_MAX_REASON_NM)
	{
...
_nic_isr_rxRoutine(uint32 ringId)
{
    ...
    pNic_rxCDPIdx[ringId] = (uint32 *)(DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, temp));    /* The limitation */

    if (nic_debug_flag & DEBUG_RX_RAW_LEN_BIT)
    {
        osal_printf("RX Current descriptor: [%02X]\n ", temp);
        ioal_mem32_read(NIC_DEFAULT_UNIT_ID, RTL8390_DMA_IF_RX_BASE_DESC_ADDR_CTRL_ADDR(ringId), &temp);
    ...
    do
    {
        ...
        pktHdr = (nic_pkthdr_t *)((DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, (*pNic_rxRDPIdx[ringId]))) & NIC_ADDR_MASK);
        ...
        if (NULL == pktHdr->packet)
        {
            ...
            if (RT_ERR_OK == _nic_init_conf.pkt_alloc(NIC_DEFAULT_UNIT_ID, _nic_init_conf.pkt_size, 0, &pPacket))
            {
                ...
                pktHdr->buf_addr = (uint8 *)(DMA_ADDR_USR2KRN(pPacket->data));
...
_nic_isr_txRoutine(uint32 ringId)
{
    ...
    do
    {
        ...
        /* Prepare to be reclaim */
        pktHdr = (nic_pkthdr_t *)((DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, *pNic_txRDPIdx[ringId]) & NIC_ADDR_MASK));
...
_nic_isr_mbRoutine(void)
{
    ...
    for (i = 0; i < NIC_RXRING_NUM; i++)
    {
        ...
        temp = DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, temp);
        ...
        do
        {
            ...
            /* Prepare to be reclaim */
            pPktHdr = (nic_pkthdr_t *)((DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, (*pNic_rxRDPIdx[i])) & NIC_ADDR_MASK));
...
{
    uint32  temp, base, temp2, notif_baseAddr;
    int32   i, j, k;
    int32   ret = RT_ERR_FAILED;
    ...
    if (sizeof(nic_info_t) > RTL8390_DESC_MEM_SIZE)
    {
        RT_LOG(LOG_DEBUG, MOD_NIC, "Error: The required size of nic_info_t is bigger than allocated memory!");
        return RT_ERR_FAILED;
    }
    if ((NIC_RXRING_NUM * NIC_RXRING_SIZE * _nic_init_conf.pkt_size) > RTL8390_DMA_MEM_SIZE)
    {
    ...
    RT_ERR_CHK(osal_cache_memory_flush((uint32)KRNVIRT(RTL8390_DESC_PHYS_BASE), sizeof(nic_info_t)), ret);
    ...
    for (i = 0; i < NIC_RXRING_NUM; i++)
    { 
        ioal_mem32_write(unit, RTL8390_DMA_IF_RX_BASE_DESC_ADDR_CTRL_ADDR(i), (uint32)DESC_ADDR_USR2KRN(unit, pNic_rxFDPBase[i]));
    }

    for (i = 0; i < NIC_TXRING_NUM; i++)
    { 
        ioal_mem32_write(unit, RTL8390_DMA_IF_TX_BASE_DESC_ADDR_CTRL_ADDR(i), (uint32)DESC_ADDR_USR2KRN(unit, pNic_txFDPBase[i]));  
    }
...
_nic_pkt_tx(uint32 unit, drv_nic_pkt_t *pPacket, drv_nic_tx_cb_f fTxCb, void *pCookie)
{
    ...
    /* Step: Find a pktHdr */
    if ((DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, *pNic_txCDPIdx[txRingId]) & NIC_RING_SWOWNBIT) != 0)
    {
    ...
    pPktHdr = (nic_pkthdr_t *)((DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, *pNic_txCDPIdx[txRingId])) & NIC_ADDR_MASK);
    while (NULL != pPktHdr->packet && cnt < 100) {
        osal_time_usleep(10);
        cnt++;
    }
...
r8390_init(uint32 unit, drv_nic_initCfg_t *pInitCfg)
{
    ...
    RT_LOG(LOG_DEBUG, MOD_NIC, "Init NIC R8390...OK");    
...
r8390_pkthdrMbuf_dump(uint32 unit, uint32 mode, uint32 start, uint32 end, uint32 flags)
{
    ...
    for (i = start; i <= end; i++)
    {
        for (j = 0; j < ring_size; j++)
        {
            ...
            if (NIC_PKTHDR_MBUF_MODE_RX == mode)
                pRing_pkthdr = (nic_pkthdr_t *)(DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, *((uint32 *)(pNic_rxFDPBase[i]+j))) & NIC_ADDR_MASK);
            else
                pRing_pkthdr = (nic_pkthdr_t *)(DESC_ADDR_KRN2USR(NIC_DEFAULT_UNIT_ID, *((uint32 *)(pNic_txFDPBase[i]+j))) & NIC_ADDR_MASK);
            osal_printf("###################################################\n");
