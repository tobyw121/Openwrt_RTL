 ...
 *
 * $Revision: 38163 $
 * $Date: 2013-03-26 14:45:31 +0800 (Tue, 26 Mar 2013) $
 *
...
#define MAX_NBR_OF_PIE_CPU_ENTRY    64
#define DB_UNUSED                   (-1)
static drv_nic_CpuEntry_t pieCpuEntry[MAX_NBR_OF_PIE_CPU_ENTRY];
static int32 pieCpuEntry_aclIndex[MAX_NBR_OF_PIE_CPU_ENTRY];
#endif
...
static uint32 		reasonTbl[][2] = 
{
	...
	{NIC_RX_REASON_NEW_SA,							0,},
	{NIC_RX_REASON_L2_PMV,							0,},
	{0,  											0,},
	{0,  											0,},
	{0,  											0,},
	{0,  											0,},	/* 20 */
	{NIC_RX_REASON_UNKWN_UCST,						NIC_RX_REASON_UNKWN_UCST_MCST,},
	{NIC_RX_REASON_UNKWN_L2_MCST,					NIC_RX_REASON_UNKWN_UCST_MCST,},
	{NIC_RX_REASON_UNKWN_BCST,						0,},
	{NIC_RX_REASON_UNKWN_IP_MCST,					NIC_RX_REASON_UNKWN_UCST_MCST,},
	{0,  											0,},
	{0,  											0,},
	{0,  											0,},
	{0,  											0,},
...
#if defined(CONFIG_SDK_SOFTWARE_RX_CPU_TAG)
static int32 _pieCpuEntry_init(uint32 unit);
#endif
#if defined(CONFIG_SDK_RX_THREAD)
static int32 _nic_rx_intr_attach(void);
static void _esw_nic_rx_isr(uint32 nic_rx_iisr);
static void *_nic_rx_intr_thread(void *pArg);
static void _esw_nic_rx_isr_handler(uint32 cpu_iisr);
#endif
...
{
#define RTL8328_MAX_REASON_NM  68
#define FLD_TO_CPU_BIT  8
#define ESFLOW_BIT      9
#define IS_ACL_HIT_BIT  11
	uint16 reasonDetail;
	uint16 reason = pPacket->rx_tag.reason;
	
	if ((reason & (1 << FLD_TO_CPU_BIT)) == 0 && (reason & (1 << ESFLOW_BIT)) == 0 && (reason & (1 << IS_ACL_HIT_BIT)) == 0)
	{
        reasonDetail = reason & 0x3f;
		if (reasonDetail <= RTL8328_MAX_REASON_NM)
		{
			NIC_REASON_MASK_SET(*pPacket, reasonTbl[reasonDetail][0]);
			NIC_REASON_MASK_SET(*pPacket, reasonTbl[reasonDetail][1]);
		}
	}
	else if (reason & (1 << IS_ACL_HIT_BIT))
	{
		NIC_REASON_MASK_SET(*pPacket, NIC_RX_REASON_ACL_HIT);
        pPacket->rx_tag.acl_hit = 1;
        pPacket->rx_tag.acl_index = reason & 0x7ff;
	}
...
#if defined(CONFIG_SDK_SOFTWARE_RX_CPU_TAG)
    uint32  sw_db_index, l2_user_defined_idx, l2_user_defined_offset;
    uint32  ip_header_len;
    uint32  acl_hit = 0, reason_detail = 0, egress_sflow = 0, mirror2cpu = 0;
    drv_nic_CpuEntry_t  entry;
    uint32  l2_user_define_r0, l2_user_define_r1, l2_user_define_r2;
#endif
...
//    }
    ...
    do
    {
        ...
        pktHdr = (esw_nic_pkthdr_t *)((osal_ntohl(*pNic_rxRDPIdx[ringId])) & NIC_ADDR_MASK);
        pMbuf = (esw_nic_mbuf_t*)(osal_ntohl(pktHdr->ph_mbuf));
        ...
        if (1 == pktHdr->ph_tag)
        {
...
#if defined(CONFIG_SDK_SOFTWARE_RX_CPU_TAG)
            pPacket->rx_tag.sw_valid = 0; /* Initialize to 0 first */
            mirror2cpu = (pPacket->rx_tag.reason >> 15) & 1;
            acl_hit = (pPacket->rx_tag.reason >> 11) & 1;
            osal_memset(&entry, 0, sizeof(drv_nic_CpuEntry_t));
            if (1 == mirror2cpu)
            {
                /* Check match the ACL:mirror2cpu entry or not? */
                /* Transfer source-port to DPM */
                entry.dpm = (1 << pPacket->rx_tag.source_port);
                /* Get DMAC from packet raw-data */
                entry.dmac[0] = *(pMbuf->packet->data);
                entry.dmac[1] = *(pMbuf->packet->data+1);
                entry.dmac[2] = *(pMbuf->packet->data+2);
                entry.dmac[3] = *(pMbuf->packet->data+3);
                entry.dmac[4] = *(pMbuf->packet->data+4);
                entry.dmac[5] = *(pMbuf->packet->data+5);
                /* Get Ethertype from packet raw-data */
                if (pPacket->rx_tag.l2_format == 3)
                {
                    entry.ethertype = osal_ntohs(*(uint16 *) (pPacket->data + 12));
                    entry.vid = pPacket->rx_tag.inner_vid;
                    if (entry.ethertype == 0x0800)
                    {
                        entry.protocol_id = *(pPacket->data + 23);
                        ip_header_len = ((*(pPacket->data + 14)) & 0xf) * 5;
                        if (entry.protocol_id == 6 /*TCP*/ || entry.protocol_id == 17 /*UDP*/)
                            entry.l4_dst_port = osal_ntohs(*(uint16 *) (pPacket->data + 14 + ip_header_len + 2));
                    }
                }
                else if (pPacket->rx_tag.l2_format == 6 || pPacket->rx_tag.l2_format == 9)
                {
                    entry.ethertype = osal_ntohs(*(uint16 *) (pPacket->data + 16));
                    entry.vid = pPacket->rx_tag.inner_vid;
                    if (entry.ethertype == 0x0800)
                    {
                        entry.protocol_id = *(pPacket->data + 27);
                        ip_header_len = ((*(pPacket->data + 18)) & 0xf) * 5;
                        if (entry.protocol_id == 6 /*TCP*/ || entry.protocol_id == 17 /*UDP*/)
                            entry.l4_dst_port = osal_ntohs(*(uint16 *) (pPacket->data + 18 + ip_header_len + 2));
                    }
                }
                else if (pPacket->rx_tag.l2_format == 12)
                {
                    entry.ethertype = osal_ntohs(*(uint16 *) (pPacket->data + 20));
                    entry.vid = pPacket->rx_tag.inner_vid;
                    if (entry.ethertype == 0x0800)
                    {
                        entry.protocol_id = *(pPacket->data + 31);
                        ip_header_len = ((*(pPacket->data + 22)) & 0xf) * 5;
                        if (entry.protocol_id == 6 /*TCP*/ || entry.protocol_id == 17 /*UDP*/)
                            entry.l4_dst_port = osal_ntohs(*(uint16 *) (pPacket->data + 22 + ip_header_len + 2));
                    }
                }
                
                for (sw_db_index = 0; sw_db_index < MAX_NBR_OF_PIE_CPU_ENTRY; sw_db_index++)
                {
                    if (DB_UNUSED == pieCpuEntry_aclIndex[sw_db_index])
                        continue;
                    if (pieCpuEntry[sw_db_index].care_dpm)
                    {
                        if (((pieCpuEntry[sw_db_index].dpm & pieCpuEntry[sw_db_index].care_dpm) & entry.dpm) != (entry.dpm & pieCpuEntry[sw_db_index].care_dpm))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_vid)
                    {
                        if ((pieCpuEntry[sw_db_index].vid & pieCpuEntry[sw_db_index].care_vid) != (entry.vid & pieCpuEntry[sw_db_index].care_vid))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_ethertype)
                    {
                        if ((pieCpuEntry[sw_db_index].ethertype & pieCpuEntry[sw_db_index].care_ethertype) != (entry.ethertype & pieCpuEntry[sw_db_index].care_ethertype))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_protocol_id)
                    {
                        if ((pieCpuEntry[sw_db_index].protocol_id & pieCpuEntry[sw_db_index].care_protocol_id) != (entry.protocol_id & pieCpuEntry[sw_db_index].care_protocol_id))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_l4_dst_port)
                    {
                        if ((pieCpuEntry[sw_db_index].l4_dst_port & pieCpuEntry[sw_db_index].care_l4_dst_port) != (entry.l4_dst_port & pieCpuEntry[sw_db_index].care_l4_dst_port))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_dmac[0])
                    {
                        if ((pieCpuEntry[sw_db_index].dmac[0] & pieCpuEntry[sw_db_index].care_dmac[0]) != (entry.dmac[0] & pieCpuEntry[sw_db_index].care_dmac[0]))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_dmac[1])
                    {
                        if ((pieCpuEntry[sw_db_index].dmac[1] & pieCpuEntry[sw_db_index].care_dmac[1]) != (entry.dmac[1] & pieCpuEntry[sw_db_index].care_dmac[1]))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_dmac[2])
                    {
                        if ((pieCpuEntry[sw_db_index].dmac[2] & pieCpuEntry[sw_db_index].care_dmac[2]) != (entry.dmac[2] & pieCpuEntry[sw_db_index].care_dmac[2]))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_dmac[3])
                    {
                        if ((pieCpuEntry[sw_db_index].dmac[3] & pieCpuEntry[sw_db_index].care_dmac[3]) != (entry.dmac[3] & pieCpuEntry[sw_db_index].care_dmac[3]))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_dmac[4])
                    {
                        if ((pieCpuEntry[sw_db_index].dmac[4] & pieCpuEntry[sw_db_index].care_dmac[4]) != (entry.dmac[4] & pieCpuEntry[sw_db_index].care_dmac[4]))
                            continue;
                    }
                    if (pieCpuEntry[sw_db_index].care_dmac[5])
                    {
                        if ((pieCpuEntry[sw_db_index].dmac[5] & pieCpuEntry[sw_db_index].care_dmac[5]) != (entry.dmac[5] & pieCpuEntry[sw_db_index].care_dmac[5]))
                            continue;
                    }

                    pPacket->rx_tag.sw_index = pieCpuEntry_aclIndex[sw_db_index];
                    pPacket->rx_tag.sw_index_type = SW_TYPE_ACL;
                    pPacket->rx_tag.sw_valid = 1;
                    break;
                }
            }
            else
            {
                if (0 == acl_hit)
                {
                    egress_sflow = (pPacket->rx_tag.reason >> 9) & 1;
                    if (0 == egress_sflow)
                    {
                        reason_detail = (pPacket->rx_tag.reason) & 0x3f;
                        if (47 == reason_detail) /* 47: RMA reason */
                        {
                            if (pPacket->rx_tag.l3_l4_format == 0) /* 0: mean pure L2 */
                            {
                                /* Parsing to know the RMA index, then fill the sw_index */
                                entry.dmac[0] = *(pMbuf->packet->data);
                                entry.dmac[1] = *(pMbuf->packet->data+1);
                                entry.dmac[2] = *(pMbuf->packet->data+2);
                                entry.dmac[3] = *(pMbuf->packet->data+3);
                                entry.dmac[4] = *(pMbuf->packet->data+4);
                                entry.dmac[5] = *(pMbuf->packet->data+5);
                                if ((entry.dmac[5] <= 0x2F) && (entry.dmac[4] == 0x0) && (entry.dmac[3] == 0x0) &&
                                    (entry.dmac[2] == 0xC2) && (entry.dmac[1] == 0x80) && (entry.dmac[0] == 0x1))
                                {
                                    pPacket->rx_tag.sw_index = *(pMbuf->packet->data+5);
                                    pPacket->rx_tag.sw_index_type = SW_TYPE_RMA;
                                    pPacket->rx_tag.sw_valid = 1;
                                }
                                else
                                {
                                    for (l2_user_defined_idx = 0; l2_user_defined_idx < 4; l2_user_defined_idx++)
                                    {
                                        l2_user_defined_offset = (0x10 * l2_user_defined_idx);
                                        /* L2 User-Define 0 - 0x530038/0x53003C/0x530040 */
                                        /* L2 User-Define 1 - 0x530048/0x53004C/0x530050 */
                                        /* L2 User-Define 2 - 0x530058/0x53005C/0x530060 */
                                        /* L2 User-Define 3 - 0x530068/0x53006C/0x530070 */
                                        l2_user_define_r0 = *((uint32*)(0xBB530038+l2_user_defined_offset));
                                        l2_user_define_r1 = *((uint32*)(0xBB53003C+l2_user_defined_offset));
                                        l2_user_define_r2 = *((uint32*)(0xBB530040+l2_user_defined_offset));
                                        if (l2_user_define_r2 || (l2_user_define_r1 & 0xFFFF))
                                        {
                                            if (l2_user_define_r2 & 0xFF)
                                            {
                                                if ((entry.dmac[5] & ((l2_user_define_r2) & 0xFF)) != ((((l2_user_define_r1) >> 16) & 0xFF) & ((l2_user_define_r2) & 0xFF)))
                                                    continue;
                                            }
                                            if ((l2_user_define_r2>>8) & 0xFF)
                                            {
                                                if ((entry.dmac[4] & ((l2_user_define_r2>>8) & 0xFF)) != (((l2_user_define_r1 >> 24) & 0xFF) & ((l2_user_define_r2>>8) & 0xFF)))
                                                    continue;
                                            }
                                            if ((l2_user_define_r2>>16) & 0xFF)
                                            {
                                                if ((entry.dmac[3] & ((l2_user_define_r2>>16) & 0xFF)) != (((l2_user_define_r0) & 0xFF) & ((l2_user_define_r2>>16) & 0xFF)))
                                                    continue;
                                            }
                                            if ((l2_user_define_r2>>24) & 0xFF)
                                            {
                                                if ((entry.dmac[2] & ((l2_user_define_r2>>24) & 0xFF)) != (((l2_user_define_r0 >> 8) & 0xFF) & ((l2_user_define_r2>>24) & 0xFF)))
                                                    continue;
                                            }
                                            if ((l2_user_define_r1) & 0xFF)
                                            {
                                                if ((entry.dmac[1] & ((l2_user_define_r1) & 0xFF)) != (((l2_user_define_r0 >> 16) & 0xFF) & ((l2_user_define_r1) & 0xFF)))
                                                    continue;
                                            }
                                            if ((l2_user_define_r1>>8) & 0xFF)
                                            {
                                                if ((entry.dmac[0] & ((l2_user_define_r1>>8) & 0xFF)) != (((l2_user_define_r0 >> 24) & 0xFF) & ((l2_user_define_r1>>8) & 0xFF)))
                                                    continue;
                                            }
                                        }
                                        else
                                        ...
                                        pPacket->rx_tag.sw_index = (0x30 + l2_user_defined_idx);
                                        pPacket->rx_tag.sw_index_type = SW_TYPE_RMA;
                                        pPacket->rx_tag.sw_valid = 1;
                                        break;
                                    }
                            ...
                            else if (pPacket->rx_tag.l3_l4_format == 1)
                            {
                                pPacket->rx_tag.sw_index = NIC_SW_RMA_L34_ARP; /* ARP */
                                pPacket->rx_tag.sw_index_type = SW_TYPE_RMA;
                                pPacket->rx_tag.sw_valid = 1;
                            }
                            else if (pPacket->rx_tag.l3_l4_format == 2)
                            {
                                pPacket->rx_tag.sw_index = NIC_SW_RMA_L34_ICMP; /* ICMP */
                                pPacket->rx_tag.sw_index_type = SW_TYPE_RMA;
                                pPacket->rx_tag.sw_valid = 1;
                            }
                            else if (pPacket->rx_tag.l3_l4_format == 3)
                            {
                                pPacket->rx_tag.sw_index = NIC_SW_RMA_L34_IGMP; /* IGMP */
                                pPacket->rx_tag.sw_index_type = SW_TYPE_RMA;
                                pPacket->rx_tag.sw_valid = 1;
                            }
                            else if (pPacket->rx_tag.l3_l4_format == 7)
                            {
                                pPacket->rx_tag.sw_index = NIC_SW_RMA_L34_ICMPV6; /* ICMPv6 */
                                pPacket->rx_tag.sw_index_type = SW_TYPE_RMA;
                                pPacket->rx_tag.sw_valid = 1;
                            }
                        }
                        else if (34 == reason_detail) /* 34: Forward To CPU reason */
                        {
                            /* Check match the ACL:redirect2cpu entry or not? */
                            /* Transfer source-port to DPM */
                            entry.dpm = (1 << pPacket->rx_tag.source_port);
                            /* Get DMAC from packet raw-data */
                            entry.dmac[0] = *(pMbuf->packet->data);
...
#if defined(CONFIG_SDK_SOFTWARE_RX_CPU_TAG)
                osal_printf(" SW_VALID : %d \n", pPacket->rx_tag.sw_valid);
                osal_printf(" SW_INDEX_TYPE : %d \n", pPacket->rx_tag.sw_index_type);
                osal_printf(" SW_INDEX : %d \n", pPacket->rx_tag.sw_index);
#endif
            }
        }
        ...
        {   /* We have to free this packet here */
            _nic_init_conf.pkt_free(NIC_DEFAULT_UNIT_ID, pMbuf->packet);
        }
        ...
        if (RT_ERR_OK == _nic_init_conf.pkt_alloc(NIC_DEFAULT_UNIT_ID, _nic_init_conf.pkt_size, 0, &pPacket))
        {
            ...
            pMbuf->m_extbuf = (uint8*)osal_ntohl((uint8 *)(UNCACHE(pPacket->head)));
            pMbuf->m_extsize = osal_ntohs(pPacket->end - pPacket->head);
            pMbuf->m_data = (uint8*)osal_ntohl((uint8 *)(UNCACHE(pPacket->data)));
            pMbuf->m_len = 0;
...
_nic_isr_txRoutine(uint32 ringId)
{
    ...
    do
    {
        ...
        /* Prepare to be reclaim */
        pktHdr = (esw_nic_pkthdr_t *)((osal_ntohl(*pNic_txRDPIdx[ringId])) & NIC_ADDR_MASK);
        ...
        {
            pktHdr->tx_callback(NIC_DEFAULT_UNIT_ID, ((esw_nic_mbuf_t*)osal_ntohl(pktHdr->ph_mbuf))->packet, pktHdr->cookie);
            pktHdr->tx_callback = NULL;
        }
        ((esw_nic_mbuf_t*)osal_ntohl(pktHdr->ph_mbuf))->packet = NULL;
...
//    }
    ...
    do
    {
        ...
        /* Prepare to be reclaim */
        pMBuf = (esw_nic_mbuf_t *)((osal_ntohl(*pNic_mBRDPIdx)) & NIC_ADDR_MASK);
...
{    
    ...
    ioal_mem32_write(0, RTL8328_CPUIISR_ADDR, nic_rx_iisr);
...
{   
    ...
    {            
        ...
        */
        ...
        _esw_nic_rx_isr_handler(nic_rx_isr_process);             
    }    
    return NULL;
} /* end of _nic_rx_intr_thread */

static void _esw_nic_rx_isr_handler(uint32 nic_rx_isr_mask)
{
    ...
    return;
} /* end of _esw_nic_rx_isr_handler */
#endif

/* Function Name:
 *      _esw_nic_isr_handler
 * Description:
 ...
 */
osal_isrret_t
_esw_nic_isr_handler(void *isr_param)
{
    uint32  cpu_iisr;

    ioal_mem32_read(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, &cpu_iisr);

    /* Rx have 8 rings (0..7); need to process when any rx ring isr done */
    if (cpu_iisr & INT_RX_DONE7_INT_RX_DONE0_MASK)
    {
#if defined(CONFIG_SDK_RX_THREAD)
        _esw_nic_rx_isr(cpu_iisr & INT_RX_DONE7_INT_RX_DONE0_MASK);
#else
        if (cpu_iisr & INT_RX_DONE7_MASK)   /* Process Rx ring 7 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_RX_DONE7_MASK);
            _nic_isr_rxRoutine(7);
        }

        if (cpu_iisr & INT_RX_DONE6_MASK)   /* Process Rx ring 6 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_RX_DONE6_MASK);
            _nic_isr_rxRoutine(6);
        }

        if (cpu_iisr & INT_RX_DONE5_MASK)   /* Process Rx ring 5 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_RX_DONE5_MASK);
            _nic_isr_rxRoutine(5);
        }

        if (cpu_iisr & INT_RX_DONE4_MASK)   /* Process Rx ring 4 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_RX_DONE4_MASK);
            _nic_isr_rxRoutine(4);
        }

        if (cpu_iisr & INT_RX_DONE3_MASK)   /* Process Rx ring 3 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_RX_DONE3_MASK);
            _nic_isr_rxRoutine(3);
        }

        if (cpu_iisr & INT_RX_DONE2_MASK)   /* Process Rx ring 2 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_RX_DONE2_MASK);
            _nic_isr_rxRoutine(2);
        }

        if (cpu_iisr & INT_RX_DONE1_MASK)   /* Process Rx ring 1 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_RX_DONE1_MASK);
            _nic_isr_rxRoutine(1);
        }

        if (cpu_iisr & INT_RX_DONE0_MASK)   /* Process Rx ring 0 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_RX_DONE0_MASK);
            _nic_isr_rxRoutine(0);
        }
#endif
    }
    ...
    if (cpu_iisr & INT_TX_DONE1_INT_TX_DONE0_MASK)
    {
        ...
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_TX_DONE1_MASK);
            _nic_isr_txRoutine(1);
        }

        if (cpu_iisr & INT_TX_DONE0_MASK)   /* Process Tx ring 0 ISR */
        {
            ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_TX_DONE0_MASK);
    ...
    {
        ioal_mem32_write(NIC_DEFAULT_UNIT_ID, RTL8328_CPUIISR_ADDR, INT_MBDS_MASK);
        _nic_isr_mbRoutine();
    }
...
_nic_init(uint32 unit, drv_nic_initCfg_t *pInitCfg)
{
    ...
    pNicInfo = (nic_info_t *)0xB2000000;    /* MAP to virtual memory (UNCACHE) */
    ...
    for (j = 0; j < NIC_RXRING_NUM; j++)
    {
        for (k = 0; k < NIC_RXRING_SIZE; k++)
        {
            ...
            *(pNic_rxFDPBase[j] + k) = ((k + 1) == NIC_RXRING_SIZE)? \
                osal_ntohl((uint32)pPktHdr | NIC_RING_WRAPBIT | NIC_RING_SWOWNBIT) : \
                osal_ntohl((uint32)pPktHdr | NIC_RING_SWOWNBIT);
    ...
    for (j = 0; j < NIC_TXRING_NUM; j++)
    {
        for (k = 0; k < NIC_TXRING_SIZE; k++)
        {
            ...
            *(pNic_txFDPBase[j] + k) = ((k + 1) == NIC_TXRING_SIZE)? \
                osal_ntohl((uint32)pPktHdr | NIC_RING_WRAPBIT) : \
                osal_ntohl((uint32)pPktHdr);
    ...
    for (i = 0; i < NIC_MBRING_SIZE; i++)
    {
        ...
        *(pNic_mBFDPBase + i) = ((i + 1) == NIC_MBRING_SIZE)? \
            osal_ntohl((uint32)pMBuf | NIC_RING_WRAPBIT) : \
            osal_ntohl((uint32)pMBuf);
        ...
        for (k = 0; k < NIC_TXRING_SIZE; k++)
        {
            for (l = 0; l < NIC_TX_MBUF_MAX; l++)
            {
                ...
                if (0 == l)
                {
                    ...
                    pPktHdr = (esw_nic_pkthdr_t *)((osal_ntohl(pNicInfo->txFDPBase[j][k])) & NIC_ADDR_MASK);
                    pPktHdr->ph_mbuf = (esw_nic_mbuf_t*)osal_ntohl((uint32)pMBuf);
    ...
    for (i = 0; i < NIC_RXRING_NUM; i++)
        ioal_mem32_write(unit, RTL8328_CPURPDCR0_ADDR + (0x4 * i), (uint32)pNic_rxFDPBase[i]);  

    for (i = 0; i < NIC_TXRING_NUM; i++)
        ioal_mem32_write(unit, RTL8328_CPUTPDCR0_ADDR + (0x4 * i), (uint32)pNic_txFDPBase[i]);  
    
    ioal_mem32_write(unit, RTL8328_CPURMDCR_ADDR, (uint32)pNic_mBFDPBase);  
...
_nic_pkt_tx(uint32 unit, drv_nic_pkt_t *pPacket, drv_nic_tx_cb_f fTxCb, void *pCookie)
{
    ...
    /* Step: Find a pktHdr */
    if ((*pNic_txCDPIdx[txRingId] & osal_ntohl(NIC_RING_SWOWNBIT)) != 0)
    {
    ...
    pPktHdr = (esw_nic_pkthdr_t *)((osal_ntohl(*pNic_txCDPIdx[txRingId])) & NIC_ADDR_MASK);

    RT_LOG(LOG_DEBUG, MOD_NIC, "pPktHdr : 0x%p", pPktHdr);
    /* Step: Double Confirm (Check the pktHdr status) */
    if (NULL == pPktHdr || NULL == pPktHdr->ph_mbuf || NULL != ((esw_nic_mbuf_t*)(osal_ntohl(pPktHdr->ph_mbuf)))->packet)
    {
    ...
    pMBuf = (esw_nic_mbuf_t*)(osal_ntohl(pPktHdr->ph_mbuf));
    ...
    pMBuf->m_pkthdr = (esw_nic_pkthdr_t *)osal_ntohl(pPktHdr);
    pPktHdr->ph_mbuf = (esw_nic_mbuf_t *)osal_ntohl(pMBuf);
    ...
    *(pPktHdr->ring_entry) |= osal_ntohl(NIC_RING_SWOWNBIT);
    ...
    if (nic_debug_flag & DEBUG_TX_RAW_LEN_BIT)
    {
        ...
        for (i = 0; i < dump_len; i++)
        {
            ...
            if (0 == (i % 16))
                osal_printf("[%04X] ", i);
            osal_printf("%02X ", *((uint8*)(osal_ntohl(pMBuf->m_data)) + i));
            if (15 == (i % 16))
                osal_printf("\n");
        }
        osal_printf("\n");
...
r8328_pkthdrMbuf_dump(uint32 unit, uint32 mode, uint32 start, uint32 end, uint32 flags)
{
    ...
    for (i = start; i <= end; i++)
    {
        for (j = 0; j < ring_size; j++)
        {
            ...
            esw_nic_mbuf_t      *pMbuf;   
            if (NIC_PKTHDR_MBUF_MODE_RX == mode)
                pRing_pkthdr = (esw_nic_pkthdr_t *)osal_ntohl((*((uint32 *)(pNic_rxFDPBase[i]+j)) & NIC_ADDR_MASK));
            else
                pRing_pkthdr = (esw_nic_pkthdr_t *)osal_ntohl((*((uint32 *)(pNic_txFDPBase[i]+j)) & NIC_ADDR_MASK));
            pMbuf = (esw_nic_mbuf_t*)osal_ntohl(pRing_pkthdr->ph_mbuf);
            osal_printf("###################################################\n");
            ...
            if (NULL != pRing_pkthdr->ph_mbuf)
            {
                ...
                if ((pMbuf->packet != NULL) && (TRUE == flags))
                {
                    ...
                    uint8   *pPkt_data = (uint8*)osal_ntohl(pMbuf->m_data);
 ...
 *      _pieCpuEntry_init
 * Description:
 *      Initialize the pie CPU entry of software shadow in the specified device.
 * Input:
 *      unit      - unit id
 * Output:
 *      None
 ...
 */
static int32
_pieCpuEntry_init(uint32 unit)
{
    uint32  i;

    for (i = 0; i < MAX_NBR_OF_PIE_CPU_ENTRY; i++)
    {
        pieCpuEntry_aclIndex[i] = DB_UNUSED;
        osal_memset(&pieCpuEntry[i], 0, sizeof(drv_nic_CpuEntry_t));
    }

    return RT_ERR_OK;
} /* end of _pieCpuEntry_init */
...
r8328_pieCpuEntry_add(uint32 unit, uint32 entry_idx, drv_nic_CpuEntry_t *pCpuEntry)
{
    ...
    RT_PARAM_CHK((NULL == pCpuEntry), RT_ERR_NULL_POINTER);

    NIC_SEM_LOCK(unit);

    for (i = 0; i < MAX_NBR_OF_PIE_CPU_ENTRY; i++)
    {
        if (DB_UNUSED == pieCpuEntry_aclIndex[i])
        {
            osal_memcpy(&pieCpuEntry[i], pCpuEntry, sizeof(drv_nic_CpuEntry_t));
            pieCpuEntry_aclIndex[i] = entry_idx;
            NIC_SEM_UNLOCK(unit);
            return RT_ERR_OK;
        }
    }
    
    if (MAX_NBR_OF_PIE_CPU_ENTRY == i)
    {
        NIC_SEM_UNLOCK(unit);
        return RT_ERR_FAILED; /* Full */
    }

    NIC_SEM_UNLOCK(unit);
    return RT_ERR_OK;
} /* end of r8328_pieCpuEntry_add */
...
r8328_pieCpuEntry_del(uint32 unit, uint32 entry_idx)
{
    ...
    for (i = 0; i < MAX_NBR_OF_PIE_CPU_ENTRY; i++)
    {
        if (entry_idx == pieCpuEntry_aclIndex[i])
        {
            osal_memset(&pieCpuEntry[i], 0, sizeof(drv_nic_CpuEntry_t));
            pieCpuEntry_aclIndex[i] = DB_UNUSED;
            NIC_SEM_UNLOCK(unit);
    ...
    NIC_SEM_UNLOCK(unit);
    return RT_ERR_FAILED; /* Not found */
} /* end of r8328_pieCpuEntry_del */
...
r8328_pieCpuEntry_get(uint32 unit, uint32 entry_idx, drv_nic_CpuEntry_t *pCpuEntry)
{
    ...
    for (i = 0; i < MAX_NBR_OF_PIE_CPU_ENTRY; i++)
    {
        if (entry_idx == pieCpuEntry_aclIndex[i])
        {
            osal_memcpy(pCpuEntry, &pieCpuEntry[i], sizeof(drv_nic_CpuEntry_t));
            NIC_SEM_UNLOCK(unit);
            return RT_ERR_OK;
        }
    }
