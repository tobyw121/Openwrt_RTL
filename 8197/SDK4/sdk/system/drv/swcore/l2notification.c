...
drv_swcore_l2_notification_init(uint32 unit, uint32 blockNum)
{
    ...
    eventRing = osal_alloc(blockNum * sizeof(uint32));
    osal_memset(eventRing, 0, blockNum * sizeof(uint32));
    if ((ret = osal_cache_memory_flush((uint32)eventRing, blockNum * sizeof(uint32))) != RT_ERR_OK)
    {
        return ret;
    }
