 ...
 *
 * $Revision: 42106 $
 * $Date: 2013-08-16 13:53:54 +0800 (Fri, 16 Aug 2013) $
 *
...
r8390_watchdog_init(uint32 unit)
{
    ...
    if ((ret = r8390_watchdog_threshold_set(unit, &threshold)) != RT_ERR_OK)
        return ret;

    return RT_ERR_OK;
} /* end of r8390_watchdog_init */
...
#endif
    return RT_ERR_OK;
} /* end of r8390_watchdog_mode_set */
...
#endif
    return RT_ERR_OK;
} /* end of r8390_watchdog_mode_get */
...
r8390_watchdog_scale_set(uint32 unit, drv_watchdog_scale_t scale)
{
    ...
    if (flag_es)
        reg_addr = RTL8390ES_WDTCTRL;
    else
        reg_addr = RTL8390MP_WDTCTRL;
    
    /* set WDT_CLK_SC field to be 2b 00 */
    newWDTCTRL = (REG32(reg_addr) & (~RTL8390_WDT_WDT_CLK_SC_MASK));
    switch (scale)
    {
        case WATCHDOG_SCALE_1:                     /* fire after 1.7 sec */
            newWDTCTRL = (newWDTCTRL | RTL8390_WDT_SCALE_1); /* 2b 00, 2^25 */
            break;

        case WATCHDOG_SCALE_2:                     /* fire after 3.4 sec */
            newWDTCTRL = (newWDTCTRL | RTL8390_WDT_SCALE_2); /* 2b 01, 2^26 */
            break;

        case WATCHDOG_SCALE_3:                     /* fire after 6.7 sec */
            newWDTCTRL = (newWDTCTRL | RTL8390_WDT_SCALE_3); /* 2b 10, 2^27 */
            break;

        case WATCHDOG_SCALE_4:                     /* fire after 13.4 sec */
            newWDTCTRL = (newWDTCTRL | RTL8390_WDT_SCALE_4); /* 2b 11, 2^28 */
            break;
    ...
    return RT_ERR_OK;
} /* end of r8390_watchdog_scale_set */
...
r8390_watchdog_scale_get(uint32 unit, drv_watchdog_scale_t *pScale)
{
    ...
    *pScale = (REG32(reg_addr) & RTL8390_WDT_WDT_CLK_SC_MASK) >> RTL8390_WDT_WDT_CLK_SC_OFFSET;

    return RT_ERR_OK;
} /* end of r8390_watchdog_scale_get */
...
r8390_watchdog_enable_set(uint32 unit, uint32 enable)
{
    ...
    v_wdtcnr = v_wdtcnr & (~RTL8390_WDT_WDT_E_MASK);
    ...
    switch (enable) 
    {
        ...
        case ENABLED:           /* Enable */
            REG32(GIMR) |= (WDT_IP1_IE | WDT_IP2_IE);
            REG32(reg_addr) = v_wdtcnr | (1 << RTL8390_WDT_WDT_E_OFFSET);
            break;
        default:
            return RT_ERR_FAILED;
    }
    ...
    return RT_ERR_OK;
} /* end of r8390_watchdog_enable_set */
...
r8390_watchdog_enable_get(uint32 unit, uint32 *pEnable)
{
    ...
    v_wdtcnr = REG32(reg_addr);
    (*pEnable) = (v_wdtcnr & RTL8390_WDT_WDT_E_MASK) >> RTL8390_WDT_WDT_E_OFFSET;

    return RT_ERR_OK;
} /* end of r8390_watchdog_enable_get */
...
r8390_watchdog_kick(uint32 unit)
{
    ...
    if (flag_es)
        reg_addr = RTL8390ES_WDTCNTR;
    else
        reg_addr = RTL8390MP_WDTCNTR;

    /* Clear watchdog pending flag */
    REG32(reg_addr) |= RTL8390_WDT_KICK_MASK;

    return RT_ERR_OK;
} /* end of r8390_watchdog_kick */
...
r8390_watchdog_threshold_set(uint32 unit, drv_watchdog_threshold_t *pThreshold)
{
    ...
    v_wdtcnr = v_wdtcnr & (~RTL8390_WDT_PH1_TO_MASK);
    v_wdtcnr = v_wdtcnr | (pThreshold->phase_1_threshold << RTL8390_WDT_PH1_TO_OFFSET);
    v_wdtcnr = v_wdtcnr & (~RTL8390_WDT_PH2_TO_MASK);
    v_wdtcnr = v_wdtcnr | (pThreshold->phase_2_threshold << RTL8390_WDT_PH2_TO_OFFSET);

    REG32(reg_addr) = v_wdtcnr;

    return RT_ERR_OK;
} /* end of r8390_watchdog_threshold_set */
...
r8390_watchdog_threshold_get(uint32 unit, drv_watchdog_threshold_t *pThreshold)
{
    ...
    /* Get old watchdog controller value */
    v_wdtcnr = REG32(reg_addr);
    pThreshold->phase_1_threshold = (v_wdtcnr & RTL8390_WDT_PH1_TO_MASK) >> RTL8390_WDT_PH1_TO_OFFSET;
    pThreshold->phase_2_threshold = (v_wdtcnr & RTL8390_WDT_PH2_TO_MASK) >> RTL8390_WDT_PH2_TO_OFFSET;

    return RT_ERR_OK;
} /* end of r8390_watchdog_threshold_get */
