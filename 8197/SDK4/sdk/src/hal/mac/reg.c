...
reg_write(uint32 unit, uint32 reg, uint32 *pValue)
{
    ...
    for (i = 0; i < num; i++)
    {
        ret = ioal_mem32_read(unit, addr+4*i, &r_value);
        if (*(pValue+i) != r_value)
        {
            REG_SEM_LOCK(unit);
...
reg_field_get(
    ...
    uint32  *pData)
{
#if 0 /* Old, supoprted 32-bit register only */
    uint32          ret = 0;
#endif
    ...
    uint32  *pData)
{
    ...
    *pData &= ~(mask << pField->lsp);
    *pData |= ((value & mask) << pField->lsp);
#endif
...
reg_array_read(uint32 unit, uint32 reg, int32 index_1, int32 index_2, uint32 *pValue)
{
    ...
    RT_DBG(LOG_DEBUG, (MOD_HAL), "unit=%d, reg=%d", unit, reg);
    ...
    uint32  *pValue)
{
    ...
    RT_DBG(LOG_DEBUG, (MOD_HAL), "unit=%d, reg=%d, field=%d", unit, reg, field);
    RT_PARAM_CHK((reg >= HAL_GET_MAX_REG_IDX(unit)), RT_ERR_OUT_OF_RANGE);
    RT_PARAM_CHK((NULL == pValue), RT_ERR_NULL_POINTER);
    ...
    uint32  *pValue)
{
    ...
    RT_DBG(LOG_DEBUG, (MOD_HAL), "unit=%d, reg=%d, field=%d, value=0x%x", unit, reg, field, *pValue);
    RT_PARAM_CHK((reg >= HAL_GET_MAX_REG_IDX(unit)), RT_ERR_OUT_OF_RANGE);
    RT_PARAM_CHK((NULL == pValue), RT_ERR_NULL_POINTER);
    ...
    uint32  *pData)
{
    ...
    RT_DBG(LOG_DEBUG, (MOD_HAL), "unit=%d, reg=%d, index_1=%d, index_2=%d, field=%d, data=0x%x", unit, reg, index_1, index_2, field, *pData);
    RT_PARAM_CHK(((reg >= HAL_GET_MAX_REG_IDX(unit)) || (field >= HAL_GET_MAX_REGFIELD_IDX(unit))), RT_ERR_OUT_OF_RANGE);
    RT_PARAM_CHK((NULL == pValue), RT_ERR_NULL_POINTER);
    RT_PARAM_CHK((NULL == pData), RT_ERR_NULL_POINTER);
    pField = _reg_field_find(unit, reg, field);
    ...
    else
    {
        if (index_2 != REG_ARRAY_INDEX_NONE)
        {
            ret = data >> (((index_2-hal_ctrl[unit].pChip_driver->pReg_list[reg]->larray)%(32/hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset)) * hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset);
        }
        else if (index_1 != REG_ARRAY_INDEX_NONE)
        {
            ret = data >> (((index_1-hal_ctrl[unit].pChip_driver->pReg_list[reg]->lport)%(32/hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset)) * hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset);
        }
    }
    ...
    uint32  *pData)
{
    ...
    RT_DBG(LOG_DEBUG, (MOD_HAL), "unit=%d, reg=%d, index_1=%d, index_2=%d, field=%d, value=0x%x", unit, reg, index_1, index_2, field, *pValue);
    RT_PARAM_CHK(((reg >= HAL_GET_MAX_REG_IDX(unit)) || (field >= HAL_GET_MAX_REGFIELD_IDX(unit))), RT_ERR_OUT_OF_RANGE);
    RT_PARAM_CHK((NULL == pValue), RT_ERR_NULL_POINTER);
    RT_PARAM_CHK((NULL == pData), RT_ERR_NULL_POINTER);
    pField = _reg_field_find(unit, reg, field);
    ...
    else
    {
        if (index_2 != REG_ARRAY_INDEX_NONE)
        {
            *pData &= ~(mask << (((index_2-hal_ctrl[unit].pChip_driver->pReg_list[reg]->larray)%(32/hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset)) * hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset));
            *pData |= ((value & mask) << (((index_2-hal_ctrl[unit].pChip_driver->pReg_list[reg]->larray)%(32/hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset)) * hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset));
        }
        else if (index_1 != REG_ARRAY_INDEX_NONE)
        {
            *pData &= ~(mask << (((index_1-hal_ctrl[unit].pChip_driver->pReg_list[reg]->lport)%(32/hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset)) * hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset));
            *pData |= ((value & mask) << (((index_1-hal_ctrl[unit].pChip_driver->pReg_list[reg]->lport)%(32/hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset)) * hal_ctrl[unit].pChip_driver->pReg_list[reg]->bit_offset));
        }
    }
#endif
