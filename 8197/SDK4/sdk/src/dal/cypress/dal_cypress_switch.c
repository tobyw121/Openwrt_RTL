...
dal_cypress_switch_init(uint32 unit)
{
    ...
    dal_cypress_switch_pppoePassthrough_set(unit, ENABLED);
...
dal_cypress_switch_mgmtMacAddr_set(uint32 unit, rtk_mac_t *pMac)
{
    ...
    for (port = 0; port < max_port; port = port + 1)
    {
...
dal_cypress_switch_pkt2CpuFormat_get(uint32 unit, rtk_pktFormat_t *pFormat)
{
    ...
    if ((ret = dal_cypress_switch_pkt2CpuTypeFormat_get(unit, PKT2CPU_TYPE_TRAP, pFormat)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SWITCH), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
}   /* end of dal_cypress_switch_pkt2CpuFormat_get */
...
dal_cypress_switch_pkt2CpuFormat_set(uint32 unit, rtk_pktFormat_t format)
{
    ...
    if ((ret = dal_cypress_switch_pkt2CpuTypeFormat_set(unit, PKT2CPU_TYPE_FORWARD, format)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SWITCH), "");
        return ret;
    }

    if ((ret = dal_cypress_switch_pkt2CpuTypeFormat_set(unit, PKT2CPU_TYPE_TRAP, format)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SWITCH), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
}   /* end of dal_cypress_switch_pkt2CpuFormat_set */
