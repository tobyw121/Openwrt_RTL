...
#define DIAG_UTIL_EXTRACT_PORTLIST(portlist, portlist_pos) \
    diag_util_extract_portlist(unit, (uint8 *)context->parser->tokens[(portlist_pos)].buf, DIAG_PORTTYPE_ALL, &(portlist))
...
#define DIAG_UTIL_EXTRACT_MASK(_mask, _mask_pos, _type) \
    diag_util_extract_mask(unit, (uint8 *)context->parser->tokens[(_mask_pos)].buf, _type, &(_mask))

#define DIAG_UTIL_EXTRACT_QUEUEMASK(_mask, _mask_pos) \
    diag_util_extract_mask(unit, (uint8 *)context->parser->tokens[(_mask_pos)].buf, DIAG_MASKTYPE_QUEUE, &(_mask))

#define DIAG_UTIL_MASK_SCAN(_mask, _member)\
    for ((_member) = _mask.min; (_member) <= _mask.max; (_member)++)\
        if (RTK_PORTMASK_IS_PORT_SET((_mask).mask, (_member)))
...
int32 diag_util_ip2str_format(uint8 *str, uint32 ip, uint32 blankWidth);    /* Length of ip_str must more than 15 characters */
...
int32 diag_util_extract_portlist(uint32 unit, uint8 *pStr, uint32 type, diag_portlist_t *pPortmask);
