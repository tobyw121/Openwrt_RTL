    ...
    uint32_t *vlan_id_ptr)
{
    ...
    DIAG_UTIL_ERR_CHK(diag_util_str2ipv6(&ip6_mcast_data.sip.ipv6_addr[0], (uint8 *)TOKEN_STR(3)), ret);
    DIAG_UTIL_ERR_CHK(diag_util_str2ipv6(&ip6_mcast_data.dip.ipv6_addr[0], (uint8 *)TOKEN_STR(4)), ret);
    ...
    char **port_ptr)
{
    ...
    DIAG_UTIL_ERR_CHK(DIAG_UTIL_EXTRACT_PORTLIST(portlist, 7), ret); 
    fidMacLimitEntry.portmask.bits[0] = portlist.portmask.bits[0];
...
#ifdef CMD_L2_TABLE_SET_VLAN_LEARNING_METHOD_IVL_SVL
/*
  * l2-table set vlan-learning-method ( ivl | svl )
  */    
cparser_result_t cparser_cmd_l2_table_set_vlan_learning_method_ivl_svl(cparser_context_t *context)
{
    uint32                          unit = 0;
    int32                            ret = RT_ERR_FAILED;
    ...
    char **port_ptr)
{
    ...
    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        ...
        if (DIAG_OM_GET_REALCHIPID(RTL8328M_CHIP_ID))
        {
...
cparser_result_t cparser_cmd_l2_table_get_except_smac(cparser_context_t *context)
{
    ...
    if(DIAG_OM_GET_REALCHIPID(RTL8380M_CHIP_ID))
    ...
    uint32_t *vid_ptr)
{
    ...
    {
        diag_util_mprintf("SIP             | DIP             | VID  | Port     | Cross Vlan\n");
        diag_util_mprintf("----------------+-----------------+------+----------+-----------\n");
    }
    ...
    else
    {
        diag_util_ip2str_format(strBuf1, ip_mcast_data.sip, 15);
        diag_util_ip2str_format(strBuf2, ip_mcast_data.dip, 15);
        diag_util_mprintf("%s | %s | %4d | %s   ", strBuf1, strBuf2, (ip_mcast_data.dip & 0xff), ip_mcast_data.rvid, port_list);
    }
...
#ifdef CMD_L2_TABLE_GET_EXCEPT_SMAC_BCAST_MCAST_SA
/*
  *  l2-table get except-smac bcast-mcast-sa 
  */
cparser_result_t cparser_cmd_l2_table_get_except_smac_bcast_mcast_sa(cparser_context_t *context)
{
    uint32          unit = 0;
    int32           ret = RT_ERR_FAILED;
    uint32          val = 0;
    ...
    {
        diag_util_mprintf("    Broadcast or Multicast Src Mac Action : ");
        if (val == ACTION_DROP)
        {
            diag_util_mprintf("Drop\n");
        }
...
#ifdef CMD_L2_TABLE_ADD_MAC_UCAST_VID_MAC_PORT_SA_BLOCK_DA_BLOCK_STATIC_NEXTHOP_SUSPEND
/*
 * l2-table add mac-ucast <UINT:vid> <MACADDR:mac> <UINT:port> { sa-block } { da-block } { static } { nexthop } { suspend }
 */
cparser_result_t cparser_cmd_l2_table_add_mac_ucast_vid_mac_port_sa_block_da_block_static_nexthop_suspend(cparser_context_t *context,
    uint32_t *vid_ptr,
    cparser_macaddr_t *mac_ptr,
    uint32_t *port_ptr)
{
...
cparser_result_t cparser_cmd_l2_table_dump(cparser_context_t *context)
{
    ...
    if (DIAG_OM_GET_REALCHIPID(RTL8328M_CHIP_ID))
    {
        ...
        {
            diag_util_mprintf("\tIpmc Wrong Dmac Address Action                 : ");
            if (val == L2_IPMC_MIS_DROP)
            {
                diag_util_mprintf("Drop\n");
            }
    ...
    if (DIAG_OM_GET_FAMILYID(RTL8380_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8330_FAMILY_ID))
    {
        ...
        {
            diag_util_mprintf("\tIp6 Multicast Lookup Mode                      : ");
            if (val == LOOKUP_ON_FVID_AND_MAC)
            {
                diag_util_mprintf("VID + MAC\n");
            }
...
#if 0
    if (DIAG_OM_GET_REALCHIPID(RTL8328M_CHIP_ID) || DIAG_OM_GET_FAMILYID(RTL8390_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8350_FAMILY_ID))
    {
        diag_util_mprintf("Address Table Lookup Miss Configuration : \n");
        ret = rtk_l2_lookupMissAction_get(unit, DLF_TYPE_IPMC, &val);
        if (ret)
    ...
    char **port_ptr)
{
    ...
    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        ...
        if(ENABLED == enable)
        {
            diag_util_mprintf("\t\tLimit Learning Max Count  : ");    
            DIAG_UTIL_ERR_CHK(rtk_l2_portLimitLearningCnt_get(unit, port, &mac_cnt), ret);
            diag_util_mprintf("%d\n", mac_cnt);    
...
#ifdef CMD_L2_TABLE_GET_VLAN_LEARNING_METHOD_UNICAST_MULTICAST_VID
/*
  * l2-table get vlan-learning-method ( unicast | multicast ) <UINT:vid>
  */
cparser_result_t cparser_cmd_l2_table_get_vlan_learning_method_unicast_multicast_vid(cparser_context_t *context,
    uint32_t *vid_ptr)
{
    uint32  unit = 0;
    int32   ret = RT_ERR_FAILED;
...
cparser_result_t cparser_cmd_l2_table_dump_ip6_mcast(cparser_context_t *context)
{
    ...
    while (1)
    {
        ...
        DIAG_UTIL_ERR_CHK(diag_util_ipv62str(ipv6SipStr, &ip6_mcast_data.sip.ipv6_addr[0]), ret);
        DIAG_UTIL_ERR_CHK(diag_util_ipv62str(ipv6DipStr, &ip6_mcast_data.dip.ipv6_addr[0]), ret);
    ...
    char *index_ptr)
{
    ...
    ret = rtk_l2_lookupMissFloodPortMask_get(unit, type, &portmask);
    if (ret == RT_ERR_L2_PMSK_NOT_INIT)
    {
        memset(&portmask, 0, sizeof(rtk_portmask_t));
    }

    DIAG_UTIL_ERR_CHK(rtk_l2_lookupMissFloodPortMask_set_with_idx(unit, type, *index_ptr, &portmask), ret);
    ...
    char **flood_port_ptr)
{
    ...
    DIAG_UTIL_ERR_CHK(rtk_l2_lookupMissFloodPortMask_set_with_idx(unit, type, 0, &portmask), ret);
...
#ifdef CMD_L2_TABLE_SET_VLAN_LEANING_METHOD_IVL_UNICAST_MULTICAST_VID
/*
  * l2-table set vlan-learning-method ivl ( unicast | multicast ) <UINT:vid>
  */
cparser_result_t cparser_cmd_l2_table_set_vlan_learning_method_ivl_unicast_multicast_vid(cparser_context_t *context,
    uint32_t *vid_ptr)
{
    uint32  unit = 0;
    int32   ret = RT_ERR_FAILED;
...
#ifdef CMD_L2_TABLE_SET_VLAN_LEARNING_METHOD_SVL_UNICAST_MULTICAST_VID_FID_MSTI
/*
  * l2-table set vlan-learning-method svl ( unicast | multicast ) <UINT:vid> <UINT:fid_msti>
  */
cparser_result_t cparser_cmd_l2_table_set_vlan_learning_method_svl_unicast_multicast_vid_fid_msti(cparser_context_t *context,
    uint32_t *vid_ptr,
