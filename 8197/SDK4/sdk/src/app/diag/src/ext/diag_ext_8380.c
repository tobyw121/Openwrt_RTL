    ...
    uint32_t *start_ptr, uint32_t *end_ptr)
{
    ...
    /* mask for compiler, 2011-12-01*///int32   ret = RT_ERR_FAILED; 
    ...
    diag_util_printf("Run cparser_cmd_tc_test_case_id_start_end!\n");
    /* mask for compiler, 2011-12-01*///DIAG_UTIL_ERR_CHK(tc_exec(start, end), ret);
...
{
    enum IC_TYPE type = 0;
    /* mask for compiler, 2011-12-01*///vmac_getTarget(&type);
...
cparser_result_t cparser_cmd_var_get_RegType(cparser_context_t *context)
{
    ...
    /* mask for compiler, 2011-12-01*///vmac_getRegAccessType(&type);
    ...
    uint32_t *value_ptr)
{
    ...
    /* mask for compiler, 2011-12-01*///vmac_setTarget(type);
    return CPARSER_OK;    
}
/*
    ...
    uint32_t *value_ptr)
{
    ...
    /* mask for compiler, 2011-12-01*///vmac_setRegAccessType(type);
    return CPARSER_OK;     
}
