 ...
 *
 * $Revision: 38277 $
 * $Date: 2013-03-28 15:41:36 +0800 (Thu, 28 Mar 2013) $
 *
...
cparser_cmd_trap_dump_rma_l2_user_define(cparser_context_t *context)
{
    ...
    for (rma_index = 0; rma_index < 2; rma_index++)
    {
        ...
        else if (RMA_ACTION_FLOOD_IN_ALL_PORT == rma_action)
        {
...
cparser_result_t cparser_cmd_trap_get_arp_request(cparser_context_t *context)
{
    ...
    else if (RMA_ACTION_COPY2CPU == action)
    {
        diag_util_printf("  copy-to-cpu  \n");
    }
    else
...
#ifdef CMD_TRAP_GET_IP6_UNKNOWN_HEADER_PRIORITY
/*
...
cparser_result_t cparser_cmd_trap_get_crc_error_action(cparser_context_t *context)
{
    ...
    if (RMA_ACTION_DROP == action)
    {
        diag_util_printf(" drop      \n");
    }
    else if (RMA_ACTION_FORWARD == action)
    {
        diag_util_printf(" forward      \n");
    }
    else if (RMA_ACTION_TRAP2CPU == action)
...
cparser_result_t cparser_cmd_trap_set_rma_layer2_oam_rma_02_rma_03_rma_0E_rma_10_rma_20_rma_21_rma_22_rma_0X_rma_1X_rma_2X_action_drop_forward_forward_and_flood_trap_to_cpu(cparser_context_t *context)
{
    ...
    /*RMA Action*/
    if (('d' == TOKEN_CHAR(6,0)) && ('r' == TOKEN_CHAR(6,1)))
    {
        rma_action = RMA_ACTION_DROP;
    }
    else if (('f' == TOKEN_CHAR(6,0)) && ('-' == TOKEN_CHAR(6,7)))
    {
        rma_action = RMA_ACTION_FLOOD_IN_ALL_PORT;
    }
    else if (('f' == TOKEN_CHAR(6,0)) && ('o' == TOKEN_CHAR(6,1)))
    {
        rma_action = RMA_ACTION_FORWARD;
    }
    else if (('t' == TOKEN_CHAR(6,0)) && ('r' == TOKEN_CHAR(6,1)))
    {
        rma_action = RMA_ACTION_TRAP2CPU;
    }
    else
 ...
 */
cparser_result_t
cparser_cmd_trap_set_rma_port_rma_bpdu_port_all_drop_forward_forward_and_flood_trap_to_cpu(cparser_context_t *context,
    char **port_ptr)
{
    uint32                  unit = 0;
    int32                   ret = RT_ERR_FAILED;
...
cparser_cmd_trap_get_rma_bpdu_flood_portmask(cparser_context_t *context)
{
    ...
    return CPARSER_OK;
} /* end of cparser_cmd_trap_get_bpdu_flood_portmask */
#endif
    ...
    char **port_ptr)
{
    ...
    memcpy(&flood_portmask, &portlist.portmask, sizeof(rtk_portmask_t));
    DIAG_UTIL_ERR_CHK(rtk_trap_bpduFloodPortmask_set(unit, &flood_portmask), ret);

    return CPARSER_OK;
} /* end of cparser_cmd_trap_set_bpdu_flood_portmask_port_all */
#endif

#ifdef CMD_TRAP_SET_RMA_PORT_RMA_LLDP_PORT_ALL_DROP_FORWARD_FORWARD_AND_FLOOD_TRAP_TO_CPU
/*
 * trap set rma port-rma lldp ( <PORT_LIST:port> | all ) ( drop | forward | forward-and-flood | trap-to-cpu )
 */
cparser_result_t
cparser_cmd_trap_set_rma_port_rma_lldp_port_all_drop_forward_forward_and_flood_trap_to_cpu(cparser_context_t *context,
    char **port_ptr)
{
    uint32                  unit = 0;
    int32                   ret = RT_ERR_FAILED;
 ...
 */
cparser_result_t
cparser_cmd_trap_set_rma_port_rma_ptp_port_all_copy_to_cpu_drop_forward_trap_to_cpu(cparser_context_t *context,
    char **port_ptr)
{
    uint32                  unit = 0;
    int32                   ret = RT_ERR_FAILED;
...
#ifdef CMD_TRAP_SET_IP6_UNKNOWN_HEADER_PRIORITY_PRIORITY
/*
