 ...
 *
 * $Revision: 38009 $
 * $Date: 2013-03-21 15:49:37 +0800 (Thu, 21 Mar 2013) $
 *
    ...
    char **ports_ptr)
{
    ...
    {        
         ...
         if (DIAG_OM_GET_FAMILYID(RTL8380_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8330_FAMILY_ID))
        {
         ...
         if (DIAG_OM_GET_FAMILYID(RTL8389_FAMILY_ID))
        {
 ...
 */
cparser_result_t cparser_cmd_mib_dump_counter_smon_priority(cparser_context_t *context,
    uint32_t *priority_ptr)
{
    ...
    DIAG_OM_GET_CHIP_ID(unit);   
    DIAG_UTIL_OUTPUT_INIT();
    DIAG_UTIL_ERR_CHK(rtk_stat_smon_getAll(unit, *priority_ptr, &smon_cntrs), ret);  
    ...
    char **counter_name_ptr)
{
    ...
    {        
        ...
        if (DIAG_OM_GET_FAMILYID(RTL8390_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8350_FAMILY_ID))
        {
            if (0 == strcmp(context->parser->tokens[5].buf, "rx_lengthFieldError"))
            {
                diag_util_printf("\tRX_lengthFieldError : %u\n", port_cntrs.rxLengthFieldError);  
            }
        ...
        if (DIAG_OM_GET_FAMILYID(RTL8380_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8330_FAMILY_ID))
        {
            if (0 == strcmp(context->parser->tokens[5].buf, "RX_MacDiscards"))
            {
                diag_util_printf("\tRX_MacDiscards : %d\n", port_cntrs.rxMacDiscards);  
            }
        }  
