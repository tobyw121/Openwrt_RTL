        ...
        if(pstat) {
            ...
            if (newskb) {     
                ...
                if(pfrinfo) {
                    ...
                    memcpy(ptxinsn->nhop_11s, pstat->cmn_info.mac_addr, MACADDRLEN);   
...
{    
    ...
    static struct rtl8192cd_priv *bak_priv = NULL, *mesh_priv[NUM_WLAN_IFACE];
    if (bak_priv != priv) {
        bak_priv = priv;
    ...
    for(i = 0; i < NUM_WLAN_IFACE; i++) {
        ...
        while(plist != phead)  	// 1.Check index  2.Check is it least element? (Because  next pointer to phead itself)
        {
            ...
            if(pfrinfo && 
                (memcmp(pstat->cmn_info.mac_addr, pfrinfo->sa, MACADDRLEN) == 0 ||
                 memcmp(pstat->cmn_info.mac_addr, pfrinfo->prehop_11s, MACADDRLEN) == 0)) {
                 continue;
            }  
...
#if defined(CONFIG_TRIBAND_MESH_SINGLE_IFACE)
next_priv:
    if (i < NUM_WLAN_IFACE-1) {
        priv = (struct rtl8192cd_priv *)mesh_priv[i+1];
...
int mesh_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (priv->nl80211_state != NL80211_OPEN) {		
		printk("TX DROP: nl80211_state not open yet\n");
		dev_kfree_skb_any(skb);
		goto end;
	}
#endif
    ...
    if(memcmp(skb->data, zero14, sizeof(zero14))==0) 
    {
        ...
        if(IS_MCAST(GetAddr1Ptr(skb->data+14))) {
            ...
            for (i=0;i<NUM_WLAN_IFACE;i++) {
                ...
                if(tmp_priv && GET_MIB(tmp_priv)->dot1180211sInfo.mesh_enable ) { /*send to other band's mesh if exists*/
                    if (!IS_DRV_OPEN(tmp_priv)){
                        dev_kfree_skb_any(skb);
                        goto end;
                    }
                    if (i) memcpy(GetAddr2Ptr(skb->data+14), GET_MIB(tmp_priv)->dot11OperationEntry.hwaddr, MACADDRLEN);
                    issue_11s_mesh_action(tmp_priv, skb, dev);           
                }
            }
#else /* !defined(CONFIG_TRIBAND_MESH_SINGLE_IFACE) */
            if (!IS_DRV_OPEN(priv)){
	            dev_kfree_skb_any(skb);
	            goto end;
            }
    ...
    for (i=0;i<NUM_WLAN_IFACE;i++) {
        ...
        if(tmp_priv && IS_DRV_OPEN(tmp_priv)) { 
            pstat = get_stainfo(tmp_priv, skb->data);
