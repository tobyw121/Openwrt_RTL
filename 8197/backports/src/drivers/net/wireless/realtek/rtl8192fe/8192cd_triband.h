...
#define SMP_LOCK_AMSDU_LIST(__x__) \
	do { \
		if (priv->hci_type == RTL_HCI_PCIE) { \
			spin_lock_irqsave(&priv->amsdu_list_lock, (__x__)); \
		} \
	} while(0)
...
#define SMP_UNLOCK_AMSDU_LIST(__x__) \
	do { \
		if (priv->hci_type == RTL_HCI_PCIE) { \
			spin_unlock_irqrestore(&priv->amsdu_list_lock, (__x__)); \
		} \
	} while(0)
...
#define SMP_LOCK_BS_BLOCK_LIST(__x__) \
	do { \
		spin_lock_irqsave(&priv->bsteerpriv.lock, (__x__)); \
	} while(0)
		
#define SMP_UNLOCK_BS_BLOCK_LIST(__x__) \
	do { \
		spin_unlock_irqrestore(&priv->bsteerpriv.lock, (__x__)); \
	} while(0)
#endif
...
#define SMP_LOCK_STA_BWC(__x__)	\
	do { \
		...
		else { spin_lock_bh(&priv->pshare->lock_sta_bwc); } \
	} while(0)
...
#define SMP_UNLOCK_STA_BWC(__x__) \
	do { \
		...
		else { spin_unlock_bh(&priv->pshare->lock_sta_bwc); } \
	} while(0)
