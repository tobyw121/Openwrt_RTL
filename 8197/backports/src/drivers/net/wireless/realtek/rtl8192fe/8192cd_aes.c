			 ...
			 const unsigned char *addr[], const int *len, unsigned char *mac)
{
	...
	//	unsigned char _mac[AES_BLOCK_SIZE];
	u32 _mac[(AES_BLOCK_SIZE+sizeof(u32)-1)/sizeof(u32)] = {0};
	u32 *_pad;
	u32 __padbuf[(AES_BLOCK_SIZE+sizeof(u32)-1)/sizeof(u32)];

	ctx = AES_SetKey((const long unsigned int *)key, 128);
	if (ctx == NULL)
		return -1;
	...
	while (left >= AES_BLOCK_SIZE) {
		...
		if (left > AES_BLOCK_SIZE)
			AES_Encrypt((const long unsigned int *)cbc, (long unsigned int *)cbc);
		left -= AES_BLOCK_SIZE;
	}

	memset(pad, 0, AES_BLOCK_SIZE);
	
	AES_Encrypt((const long unsigned int *)pad, (long unsigned int *)pad);
	gf_mulx(pad);
	...
	/* fix mac un-alignment issue */

	if(((u32)pad)&0x3){ /*not doubleword aligned*/
		memcpy(__padbuf, pad, AES_BLOCK_SIZE);
		_pad= __padbuf;
	}else{ /*double word aligned*/
		_pad = (u32*)pad;
	}
	AES_Encrypt((const long unsigned int *)_pad, (long unsigned int *)_mac);
                ...
                )
{
    ...
    if (qc_exists && !a4_exists)
    {
        ...
        mic_header2[9] = 0x00; /* mpdu[25] & 0x00; */
    }
    ...
    if (qc_exists && a4_exists)
    {
        ...
        mic_header2[15] = 0x00; /* mpdu[31] & 0x00; */
    }
...
int aesccmp_checkmic(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, unsigned char *pmic)
{
    ...
    if ((keylen == 0)) {
        DEBUG_ERR("no encrypt key for AES due to keylen=0\n");
        return 0;
    }

    if(pfrinfo->pktlen - pfrinfo->hdr_len - 8 - 8 <= 0){
        DEBUG_ERR("data length <=0, may lack of MIC failed\n");
        return 0;
    }
