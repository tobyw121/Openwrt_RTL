	...
	struct dm_struct *dm)
{
	...
	if ((dm->rfe_type == 15) || (dm->rfe_type == 19) || (dm->rfe_type == 20))
		odm_set_bb_reg(dm, R_0x92c, MASKDWORD, 0x00020001);
	else {
	...
	void *dm_void)
{
	...
	if (!((dm->rfe_type == 3) || (dm->rfe_type == 9) || (dm->rfe_type == 15) || (dm->rfe_type == 19) || (dm->rfe_type == 20))) {
		printk("[DPK] Skip DPK due to RFE type != 3 or 9 or 15 or 19 or 20\n");
		return;
	}
	RF_DBG(dm, DBG_RF_DPK, "[DPK] DPK Start (Ver: %s), Cv is %d, RFE is %d\n",
	       DPK_VER_8192F, dm->cut_version, dm->rfe_type);
