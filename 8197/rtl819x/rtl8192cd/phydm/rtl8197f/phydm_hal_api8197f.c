	...
	)
{
	...
	} else if ((pDM_Odm->BoardType == (ODM_BOARD_EXT_TRSW|ODM_BOARD_EXT_LNA|ODM_BOARD_EXT_PA)) && (pDM_Odm->PackageType == 2)) { // 97FN type1
	...
	} else if ((pDM_Odm->BoardType == (ODM_BOARD_EXT_PA_5G)) && (pDM_Odm->TypeAPA == 0x5) && (pDM_Odm->PackageType == 2)) { // 97FN type3
		if ((bandwidth == ODM_BW20M)) {
			if (pDM_Odm->RXAntStatus == (ODM_RF_A|ODM_RF_B)) {
				regc3c |= ((0x4) << 3);
				regc3c |= (regc3c_8_6 << 6);
