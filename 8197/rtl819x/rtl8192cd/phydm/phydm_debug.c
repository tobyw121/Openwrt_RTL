...
/*									ODM_COMP_DYNAMIC_ANT_WEIGHTING	|*/
/*									ODM_COMP_MP					|*/
...
)
{
	...
	s1Byte    rxevm_0 = 0, rxevm_1 = 0;
	s4Byte	short_cfo_a = 0, short_cfo_b = 0, long_cfo_a = 0, long_cfo_b = 0;
	s4Byte	scfo_a = 0, scfo_b = 0, avg_cfo_a = 0, avg_cfo_b = 0; 
	s4Byte	cfo_end_a = 0, cfo_end_b = 0, acq_cfo_a= 0, acq_cfo_b = 0;

	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s\n", "BB Report Info"));

	/*AGC result*/
	value32 = ODM_GetBBReg(pDM_Odm, 0xdd0, bMaskDWord);
	rf_gain_a = value32 & 0x3f;
	rf_gain_a = rf_gain_a << 1;

	rf_gain_b = (value32 >> 8) & 0x3f;
	rf_gain_b = rf_gain_b << 1;

	rf_gain_c = (value32 >> 16) & 0x3f;
	rf_gain_c = rf_gain_c << 1;

	rf_gain_d = (value32 >> 24) & 0x3f;
	...
	rx_snr_a = value32 & 0xff;
	rx_snr_a = rx_snr_a >> 1;

	rx_snr_b = (value32>>8) & 0xff;
	rx_snr_b = rx_snr_b >> 1;

	rx_snr_c = (value32>>16) & 0xff;
	rx_snr_c = rx_snr_c >> 1;

	rx_snr_d = (value32>>24) & 0xff;
	rx_snr_d = rx_snr_d >> 1;
	
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s = %d / %d / %d / %d", "RXSNR(A/B/C/D, dB)", rx_snr_a, rx_snr_b, rx_snr_c, rx_snr_d));

	/* PostFFT related info*/
	value32 = ODM_GetBBReg(pDM_Odm, 0xdd8 , bMaskDWord);
	...
	/*CFO Report Info*/
	ODM_SetBBReg(pDM_Odm, 0xd00, BIT26, 1);
	
	/*Short CFO*/
	value32 = ODM_GetBBReg(pDM_Odm, 0xdac , bMaskDWord);
	value32_1 = ODM_GetBBReg(pDM_Odm, 0xdb0 , bMaskDWord);

	short_cfo_b = (s4Byte)(value32 & 0xfff);			/*S(12,11)*/
	short_cfo_a = (s4Byte)((value32 & 0x0fff0000)>>16);

	long_cfo_b = (s4Byte)(value32_1 & 0x1fff);		/*S(13,12)*/
	long_cfo_a = (s4Byte)((value32_1 & 0x1fff0000)>>16);
	...
	/*SCFO*/
	value32 = ODM_GetBBReg(pDM_Odm, 0xdb8 , bMaskDWord);
	value32_1 = ODM_GetBBReg(pDM_Odm, 0xdb4 , bMaskDWord);

	scfo_b = (s4Byte)(value32 & 0x7ff);			/*S(11,10)*/
	scfo_a = (s4Byte)((value32 & 0x07ff0000)>>16);
	...
	avg_cfo_b = (s4Byte)(value32_1 & 0x1fff);	/*S(13,12)*/
	avg_cfo_a = (s4Byte)((value32_1 & 0x1fff0000)>>16);
	...
	value32 = ODM_GetBBReg(pDM_Odm, 0xdbc , bMaskDWord);
	value32_1 = ODM_GetBBReg(pDM_Odm, 0xde0 , bMaskDWord);

	cfo_end_b = (s4Byte)(value32 & 0x1fff);		/*S(13,12)*/
	cfo_end_a = (s4Byte)((value32 & 0x1fff0000)>>16);
	...
	acq_cfo_b = (s4Byte)(value32_1 & 0x1fff);	/*S(13,12)*/
	acq_cfo_a = (s4Byte)((value32_1 & 0x1fff0000)>>16);
...
)
{
	...
	if (pDM_Odm->SupportICType & ODM_IC_11N_SERIES) {
		phydm_bb_debug_info_n_series(pDM_Odm, &used, output, &out_len);
		return;
	}
...
)
{
	...
	if (dm_value[0] == 100) {
		...
		PHYDM_SNPRINTF((output + used, out_len - used, "13. (( %s ))DYNAMIC_ANT_WEIGHTING\n", ((pDM_Odm->DebugComponents & ODM_COMP_DYNAMIC_ANT_WEIGHTING) ? ("V") : ("."))));
		...
		PHYDM_SNPRINTF((output + used, out_len - used, "21. (( %s ))TDMA_DIG\n", ((pDM_Odm->DebugComponents & ODM_COMP_TDMA_DIG) ? ("V") : ("."))));
...
)
{
	...
	case PHYDM_LA_MODE:
			...
			phydm_lamode_trigger_setting(pDM_Odm, &input[0], &used, output, &out_len, input_num);
			pDM_Odm->SupportAbility |= ODM_BB_FA_CNT;
		#else
