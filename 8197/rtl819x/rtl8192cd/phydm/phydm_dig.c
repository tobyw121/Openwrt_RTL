	...
	)
{
	...
	u1Byte						step_size_1 = 0, step_size_2 = 0, step_size_3 = 0;
#endif

	if(priv->pshare->rf_ft_var.original_dig_restore) {		
		CurrentIGI = pDM_DigTable->CurIGValue;
	} else {	
		if(pDM_DigTable->CurIGValue_TDMA == 0) {
			pDM_DigTable->CurIGValue_TDMA = pDM_DigTable->CurIGValue;
		}
		CurrentIGI = pDM_DigTable->CurIGValue_TDMA;
	}
...
#if (DIG_HW == 1)
	if (pDM_Odm->bLinked) {
		if (pDM_DigTable->pre_rssi_min <= pDM_Odm->RSSI_Min) {
			step_size_1 = 2;
			step_size_2 = 1;
			step_size_3 = 2;
		} else {
		...
		pDM_DigTable->pre_rssi_min = pDM_Odm->RSSI_Min;
	} else {
		step_size_1 = 2;
		step_size_2 = 1;
		step_size_3 = 2;
	}
	ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("RSSI_Min = %d, pre_rssi_min = %d\n", pDM_Odm->RSSI_Min, pDM_DigTable->pre_rssi_min));	
	ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("step_size_1 = %d, step_size_2 = %d, step_size_3 = %d\n", step_size_1, step_size_2, step_size_3));	
#endif
	...
	if(pDM_Odm->bLinked && bPerformance)
	{
		...
		else
		{
			...
			if((pFalseAlmCnt->Cnt_all > dm_FA_thres[2]) && DIG_GoUpCheck)
				CurrentIGI = CurrentIGI + step_size_1;
			else if ((pFalseAlmCnt->Cnt_all > dm_FA_thres[1]) && DIG_GoUpCheck)
				CurrentIGI = CurrentIGI + step_size_2;
			else if(pFalseAlmCnt->Cnt_all < dm_FA_thres[0])
				CurrentIGI = CurrentIGI - step_size_3;
			} else {
				if((pFalseAlmCntAcc->Cnt_all_1sec > dm_FA_thres[2]) && DIG_GoUpCheck)
					CurrentIGI = CurrentIGI + step_size_1;
				else if ((pFalseAlmCntAcc->Cnt_all_1sec > dm_FA_thres[1]) && DIG_GoUpCheck)
					CurrentIGI = CurrentIGI + step_size_2;
				else if(pFalseAlmCntAcc->Cnt_all_1sec < dm_FA_thres[0])
					CurrentIGI = CurrentIGI - step_size_3;				
			}
#else
	...
	else
	{
		...
		else
		{
			...
			if((pFalseAlmCnt->Cnt_all > dm_FA_thres[2]) && DIG_GoUpCheck)
				...
				if((pFalseAlmCntAcc->Cnt_all > dm_FA_thres[2]) && DIG_GoUpCheck)
					CurrentIGI = CurrentIGI + step_size_1;
				else if ((pFalseAlmCntAcc->Cnt_all > dm_FA_thres[1]) && DIG_GoUpCheck)
					CurrentIGI = CurrentIGI + step_size_2;
				else if(pFalseAlmCntAcc->Cnt_all < dm_FA_thres[0])
					CurrentIGI = CurrentIGI - step_size_3;
			}
#else
			if(pFalseAlmCnt->Cnt_all > dm_FA_thres[2])
	...
	//1 Update status
	{
...
#endif
		{
			if(priv->pshare->rf_ft_var.original_dig_restore) {
				...
				pDM_DigTable->CurIGValue_TDMA = CurrentIGI;
				// It is possible fa_acc_1sec_tsf >= 1sec while tdma_dig_state == 0
				if(pDM_DigTable->tdma_dig_state != 0) {
					ODM_Write_DIG(pDM_Odm, pDM_DigTable->CurIGValue_TDMA);
				}
			}
	...
	)
{
	...
	prtl8192cd_priv				priv = pDM_Odm->priv;
	pDIG_T						pDM_DigTable = &pDM_Odm->DM_DigTable;	
	u4Byte						delta_time_us =  priv->pshare->rf_ft_var.tdma_dig_timer_ms * 1000;
	u4Byte						timeout;
	u4Byte						cur_freerun_cnt, diff_freerun_cnt, regb0;
	
		
	//GET_HAL_INTERFACE(priv)->GetHwRegHandler(priv, REG_FREERUN_CNT, (pu1Byte)&current_TSF);
	cur_freerun_cnt = ODM_GetBBReg(pDM_Odm, 0x0568, bMaskDWord);/*This need to fix late by YuChen 20170317!!! for Jeffery*/
	timeout = cur_freerun_cnt + delta_time_us;

	diff_freerun_cnt = cur_freerun_cnt - pDM_DigTable->cur_tsf;
	pDM_DigTable->pre_tsf = pDM_DigTable->cur_tsf;
	pDM_DigTable->cur_tsf = cur_freerun_cnt;
	
	regb0 = ODM_GetBBReg(pDM_Odm, 0xb0, bMaskDWord);

	//regb0 = RTL_R32(REG_HIMR0);

	//panic_printk("[%s] (TSF between 2 interrupt) = %u\n", __FUNCTION__, diff_tsf_dig);
	
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("Set next tdma_dig_timer\n"));
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("cur_freerun_cnt=%d, delta_time_us=%d, timeout=%d, diff_tsf_dif=%d, Reg0xb0 = 0x%x\n",
		cur_freerun_cnt,
		delta_time_us,
		timeout,
		diff_freerun_cnt,
	...
	ODM_SetBBReg(pDM_Odm, 0x0588, bMaskDWord, timeout);/*This need to fix late by YuChen 20170317!!! for Jeffery*/
}
...
//		return;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("tdma_dig_cnt=%d, pre_tdma_dig_cnt=%d\n",
		pDM_DigTable->tdma_dig_cnt,
		pDM_DigTable->pre_tdma_dig_cnt));

// need init tdma_dig_cnt @ ODM_dm_INIT	
	if((pDM_DigTable->tdma_dig_cnt == 0) || (pDM_DigTable->tdma_dig_cnt == pDM_DigTable->pre_tdma_dig_cnt)) {
		...
		if(ODMPTR->SupportAbility & ODM_BB_DIG) {
			...
			regb0 = ODM_GetBBReg(pDM_Odm, 0xb0, bMaskDWord);
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("Check fail, Reg0xb0=0x%x, IntMask[0]=0x%x, restart tdma_dig_timer !!!\n", 
				regb0, 
				pHalData->IntMask[0]));
			...
			phydm_set_tdma_dig_timer(pDM_Odm);
		}
	} else
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("Check pass, update pre_tdma_dig_cnt\n"));	
	...
	)
{
	...
	prtl8192cd_priv				priv = pDM_Odm->priv;
	pDIG_T						pDM_DigTable = &pDM_Odm->DM_DigTable;
//	u1Byte						ratio_1sec_over_dig_timer_ms;
	u1Byte						reg_c50;
//	u4Byte						TSF, fa_tsf_diff, fa_tsf_diff_ms, secLizeFA;
	PFALSE_ALARM_STATISTICS 	FalseAlmCnt = (PFALSE_ALARM_STATISTICS)PhyDM_Get_Structure( pDM_Odm, PHYDM_FALSEALMCNT);

	reg_c50 = ODM_GetBBReg(pDM_Odm, 0xc50, bMaskByte0);
	
	pDM_DigTable->tdma_dig_state = (pDM_DigTable->tdma_dig_cnt % priv->pshare->rf_ft_var.tdma_dig_state_number);

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_TDMA_DIG, ODM_DBG_LOUD, ("tdma_dig_state=%d, regc50=0x%x\n",
	...
	if (pDM_DigTable->tdma_dig_state == 1) {
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("tdma_dig_state=%d, reset FA counter !!!\n", pDM_DigTable->tdma_dig_state));

	} else if (pDM_DigTable->tdma_dig_state == 0) {

		// update pDM_DigTable->CurIGValue, it may different from pDM_DigTable->CurIGValue_TDMA
		//3 TDMA IGI upperbond @ L-state = rf_ft_var.tdma_dig_low_upper_bond = 0x26
		if (pDM_DigTable->CurIGValue >= priv->pshare->rf_ft_var.tdma_dig_low_upper_bond) {
			//ODM_Write_DIG(pDM_Odm, priv->pshare->rf_ft_var.tdma_dig_low_upper_bond);
			pDM_DigTable->LowIGValue = priv->pshare->rf_ft_var.tdma_dig_low_upper_bond;
		} else {
			//ODM_Write_DIG(pDM_Odm, pDM_DigTable->CurIGValue);
			pDM_DigTable->LowIGValue = pDM_DigTable->CurIGValue;
		}
		
		ODM_Write_DIG(pDM_Odm, pDM_DigTable->LowIGValue);
		
		phydm_TDMA_FalseAlarmCounterCheck(pDM_Odm);
/*
		fa_tsf_diff_ms = fa_tsf_diff >> 10;
		
		if (fa_tsf_diff_ms <= 1000)
			secLizeFA = (FalseAlmCnt->Cnt_all) * ((1000/fa_tsf_diff_ms) + (1000%fa_tsf_diff_ms)/fa_tsf_diff_ms);
		else
			secLizeFA = (FalseAlmCnt->Cnt_all) * (1000%fa_tsf_diff_ms)/fa_tsf_diff_ms;
		
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD, ("tdma_dig_state=%d, fa_tsf_diff=%d, secLizeFA=%d !!!\n",
			pDM_DigTable->tdma_dig_state,
			fa_tsf_diff,
			secLizeFA));
*/
	...
	)
{
	...
	u4Byte						TSF, tsf_start, tsf_end;
//	u1Byte						regc50;
	u1Byte						tdma_dig_state_number;
	prtl8192cd_priv				priv = pDM_Odm->priv;
	pDIG_T						pDM_DigTable = &pDM_Odm->DM_DigTable;
	...
	if(pDM_DigTable->tdma_dig_state == 1) {
		...
		pDM_DigTable->fa_acc_1sec_tsf += (pDM_DigTable->fa_end_tsf - pDM_DigTable->fa_start_tsf);

		// prevent dumb
		if(priv->pshare->rf_ft_var.tdma_dig_state_number == 1)
			priv->pshare->rf_ft_var.tdma_dig_state_number = 2;

		tdma_dig_state_number = priv->pshare->rf_ft_var.tdma_dig_state_number;
		
		pDM_DigTable->normalize_1sec_factor = tdma_dig_state_number / (tdma_dig_state_number - 1);

		if (pDM_DigTable->fa_acc_1sec_tsf >= (ONE_SEC_TSF / pDM_DigTable->normalize_1sec_factor)) {

			rssi_dump_en = 1;
			odm_FalseAlarmCounterAcc(pDM_Odm, rssi_dump_en);

			//panic_printk("[%s] fa_acc_1sec_tsf = %u, total FA = %u\n", __FUNCTION__, pDM_DigTable->fa_acc_1sec_tsf, FalseAlmCnt_Acc->Cnt_all);

			ODM_RT_TRACE(pDM_Odm, ODM_COMP_TDMA_DIG, ODM_DBG_LOUD, ("normalize_1sec_factor = %u, fa_acc_1sec_tsf = %u, total FA = %u, blinked=%u\n",
				pDM_DigTable->normalize_1sec_factor,
				pDM_DigTable->fa_acc_1sec_tsf,
				FalseAlmCnt_Acc->Cnt_all,
				pDM_Odm->bLinked));
			...
			odm_FalseAlarmCounterAccReset(pDM_Odm);
...
//	prtl8192cd_priv				priv = pDM_Odm->priv;
	...
	FalseAlmCnt_Acc->Cnt_Parity_Fail += FalseAlmCnt->Cnt_Parity_Fail;
	FalseAlmCnt_Acc->Cnt_Rate_Illegal += FalseAlmCnt->Cnt_Rate_Illegal;
	FalseAlmCnt_Acc->Cnt_Crc8_fail += FalseAlmCnt->Cnt_Crc8_fail;
	FalseAlmCnt_Acc->Cnt_Mcs_fail += FalseAlmCnt->Cnt_Mcs_fail;
	FalseAlmCnt_Acc->Cnt_Ofdm_fail += FalseAlmCnt->Cnt_Ofdm_fail;
//	FalseAlmCnt_Acc->Cnt_Ofdm_fail_pre += FalseAlmCnt->Cnt_Parity_Fail;
	FalseAlmCnt_Acc->Cnt_Cck_fail += FalseAlmCnt->Cnt_Cck_fail;
	FalseAlmCnt_Acc->Cnt_all += FalseAlmCnt->Cnt_all;
//	FalseAlmCnt_Acc->Cnt_all_pre += FalseAlmCnt->Cnt_all_pre;
	FalseAlmCnt_Acc->Cnt_Fast_Fsync += FalseAlmCnt->Cnt_Fast_Fsync;
	FalseAlmCnt_Acc->Cnt_SB_Search_fail += FalseAlmCnt->Cnt_SB_Search_fail;
	FalseAlmCnt_Acc->Cnt_OFDM_CCA += FalseAlmCnt->Cnt_OFDM_CCA;
	FalseAlmCnt_Acc->Cnt_CCK_CCA += FalseAlmCnt->Cnt_CCK_CCA;
	FalseAlmCnt_Acc->Cnt_CCA_all += FalseAlmCnt->Cnt_CCA_all;
//	FalseAlmCnt_Acc->Cnt_BW_USC += FalseAlmCnt->Cnt_BW_USC;
//	FalseAlmCnt_Acc->Cnt_BW_LSC += FalseAlmCnt->Cnt_BW_LSC;
	FalseAlmCnt_Acc->cnt_cck_crc32_error += FalseAlmCnt->cnt_cck_crc32_error;
	FalseAlmCnt_Acc->cnt_cck_crc32_ok += FalseAlmCnt->cnt_cck_crc32_ok;
	FalseAlmCnt_Acc->cnt_ofdm_crc32_error += FalseAlmCnt->cnt_ofdm_crc32_error;
	FalseAlmCnt_Acc->cnt_ofdm_crc32_ok += FalseAlmCnt->cnt_ofdm_crc32_ok;
	FalseAlmCnt_Acc->cnt_ht_crc32_error += FalseAlmCnt->cnt_ht_crc32_error;
	FalseAlmCnt_Acc->cnt_ht_crc32_ok += FalseAlmCnt->cnt_ht_crc32_ok;
	FalseAlmCnt_Acc->cnt_vht_crc32_error += FalseAlmCnt->cnt_vht_crc32_error;
	FalseAlmCnt_Acc->cnt_vht_crc32_ok += FalseAlmCnt->cnt_vht_crc32_ok;
	FalseAlmCnt_Acc->cnt_crc32_error_all += FalseAlmCnt->cnt_crc32_error_all;
	FalseAlmCnt_Acc->cnt_crc32_ok_all += FalseAlmCnt->cnt_crc32_ok_all;

	if(rssi_dump_en == 1) {
		FalseAlmCnt_Acc->Cnt_all_1sec = FalseAlmCnt_Acc->Cnt_all * pDM_Odm->DM_DigTable.normalize_1sec_factor;
		FalseAlmCnt_Acc->Cnt_CCA_all_1sec = FalseAlmCnt_Acc->Cnt_CCA_all * pDM_Odm->DM_DigTable.normalize_1sec_factor;
		FalseAlmCnt_Acc->Cnt_Cck_fail_1sec = FalseAlmCnt_Acc->Cnt_Cck_fail * pDM_Odm->DM_DigTable.normalize_1sec_factor;
	}
...
//	prtl8192cd_priv				priv = pDM_Odm->priv;
	...
	FalseAlmCnt_Acc->Cnt_Parity_Fail = 0;
	FalseAlmCnt_Acc->Cnt_Rate_Illegal = 0;
	FalseAlmCnt_Acc->Cnt_Crc8_fail = 0;
	FalseAlmCnt_Acc->Cnt_Mcs_fail = 0;
	FalseAlmCnt_Acc->Cnt_Ofdm_fail = 0;
//	FalseAlmCnt_Acc->Cnt_Ofdm_fail_pre = 0;
	FalseAlmCnt_Acc->Cnt_Cck_fail = 0;
	FalseAlmCnt_Acc->Cnt_all = 0;
//	FalseAlmCnt_Acc->Cnt_all_pre = 0;
	FalseAlmCnt_Acc->Cnt_Fast_Fsync = 0;
	FalseAlmCnt_Acc->Cnt_SB_Search_fail = 0;
	FalseAlmCnt_Acc->Cnt_OFDM_CCA = 0;
	FalseAlmCnt_Acc->Cnt_CCK_CCA = 0;
	FalseAlmCnt_Acc->Cnt_CCA_all = 0;
//	FalseAlmCnt_Acc->Cnt_BW_USC = 0;
//	FalseAlmCnt_Acc->Cnt_BW_LSC = 0;
	FalseAlmCnt_Acc->cnt_cck_crc32_error = 0;
	FalseAlmCnt_Acc->cnt_cck_crc32_ok = 0;
	FalseAlmCnt_Acc->cnt_ofdm_crc32_error = 0;
	FalseAlmCnt_Acc->cnt_ofdm_crc32_ok = 0;
	FalseAlmCnt_Acc->cnt_ht_crc32_error = 0;
	FalseAlmCnt_Acc->cnt_ht_crc32_ok = 0;
	FalseAlmCnt_Acc->cnt_vht_crc32_error = 0;
	FalseAlmCnt_Acc->cnt_vht_crc32_ok = 0;
	FalseAlmCnt_Acc->cnt_crc32_error_all = 0;
	FalseAlmCnt_Acc->cnt_crc32_ok_all = 0;
...
//	if (pDM_Odm->bDualMacSmartConcurrent == FALSE)
	...
	if (pDM_DigTable->cckFaMa == 0xffffffff)
		...
		if (pDM_DigTable->cckFaMa == 0xffffffff)
			pDM_DigTable->cckFaMa = FalseAlmCntAcc->Cnt_Cck_fail_1sec;
		else
			pDM_DigTable->cckFaMa = ((pDM_DigTable->cckFaMa<<1) + pDM_DigTable->cckFaMa + FalseAlmCntAcc->Cnt_Cck_fail_1sec) >> 2;
	}
