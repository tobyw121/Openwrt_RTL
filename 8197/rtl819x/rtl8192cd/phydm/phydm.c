...
)
{
	...
	ODM_SetBBReg(pDM_Odm, 0xa00, BIT(15), 0x0); /*Disable antenna diversity*/		
	ODM_SetBBReg(pDM_Odm, 0xa70, BIT(7), 0); /*Concurrent CCA at LSB & USB*/
	ODM_SetBBReg(pDM_Odm, 0xa74, BIT(8), 0); /*RX path diversity enable*/
	ODM_SetBBReg(pDM_Odm, 0xa14, BIT(7), 0); /*r_en_mrc_antsel*/
	ODM_SetBBReg(pDM_Odm, 0xa20, (BIT(5) | BIT(4)), 1); /*MBC weighting*/

	if (pDM_Odm->SupportICType & (ODM_RTL8192E | ODM_RTL8197F)) {
		ODM_SetBBReg(pDM_Odm, 0xa08, BIT(28), 1); /*r_cck_2nd_sel_eco*/
		/**/
	} else if(pDM_Odm->SupportICType & (ODM_RTL8814A)) {
		ODM_SetBBReg(pDM_Odm, 0xa84, BIT(28), 1); /*2R CCA only*/
		/**/
	}
}
...
)
{
...
#if 0
	if (p_dm_odm->support_ic_type & (ODM_RTL8192E | ODM_RTL8812 | ODM_RTL8197F | ODM_RTL8822B)) {
		...
		if (path == PHYDM_A) {
			...
			en_2R_path = 0;
			en_2R_mrc = 0;
		} else if (path == PHYDM_C) {
			path_div_select = 0;
			cck_1_path = 2;
			cck_2_path = 2;
			en_2R_path = 0;
			en_2R_mrc = 0;
		} else if (path == PHYDM_D) {
			path_div_select = 0;
			cck_1_path = 3;
			cck_2_path = 3;
			en_2R_path = 0;
			...
			en_2R_path = 1;
			en_2R_mrc = 1;
		} else if (path == PHYDM_AC) {
			path_div_select = 1;
			cck_1_path = 0;
			cck_2_path = 2;
			en_2R_path = 1;
			en_2R_mrc = 1;
		} else if (path == PHYDM_AD) {
			path_div_select = 1;
			cck_1_path = 0;
			cck_2_path = 3;
			en_2R_path = 1;
			en_2R_mrc = 1;
		}  else if (path == PHYDM_BC) {
			path_div_select = 1;
			cck_1_path = 1;
			cck_2_path = 2;
			en_2R_path = 1;
			en_2R_mrc = 1;
		} else if (path == PHYDM_BD) {
			path_div_select = 1;
			cck_1_path = 1;
			cck_2_path = 3;
			en_2R_path = 1;
			en_2R_mrc = 1;
		} else if (path == PHYDM_CD) {
			path_div_select = 1;
			cck_1_path = 2;
			cck_2_path = 3;
			en_2R_path = 1;
	...
	ODM_SetBBReg(pDM_Odm, 0xa04, (BIT(27) | BIT(26)), cck_path[0]);	/*CCK_1 input signal path*/
	ODM_SetBBReg(pDM_Odm, 0xa04, (BIT(25) | BIT(24)), cck_path[1]);	/*CCK_2 input signal path*/
	ODM_SetBBReg(pDM_Odm, 0xa74, BIT(8), path_div_select);	/*enable Rx path diversity*/
	ODM_SetBBReg(pDM_Odm, 0xa2c, BIT(18), en_2R_path);	/*enable 2R Rx path*/
	ODM_SetBBReg(pDM_Odm, 0xa2c, BIT(22), en_2R_mrc);	/*enable 2R MRC*/
}
...
#endif	
	phydm_config_cck_rx_path(pDM_Odm, PHYDM_A, CCA_PATHDIV_ENABLE);
...
)
{
	...
	if (pDM_Odm->SupportICType & ODM_RTL8197F)
		phydm_init_hw_info_by_rfe_type_8197f(pDM_Odm);
#endif
}
	...
	)
{
...
#endif	
	if(pDM_Odm->priv->pshare->rf_ft_var.original_dig_restore) {
	odm_FalseAlarmCounterStatistics(pDM_Odm);
	}
	...
	odm_DIG(pDM_Odm);
		...
		phydm_tdma_dig_timer_check(pDM_Odm);
	{
		pDIG_T	pDM_DigTable = &pDM_Odm->DM_DigTable;
		Phydm_Adaptivity(pDM_Odm, pDM_DigTable->CurIGValue);
	}
	...
	)
{
	...
	switch	(CmnInfo)
	{
		...
		case	ODM_CMNINFO_RSSI_MIN_BY_PATH:
			pDM_Odm->RSSI_Min_ByPath= (u1Byte)Value;
			break;
...
)
{
	...
	if (pDM_Odm->SupportICType & ODM_IC_11N_SERIES) {
		...
		if (pDM_Odm->SupportICType & (ODM_RTL8822B|ODM_RTL8821C)) {
			ODM_SetBBReg(pDM_Odm, 0xc20, BIT(28), reg_value);
			if (pDM_Odm->RFType > ODM_1T1R)
				ODM_SetBBReg(pDM_Odm, 0xe20, BIT(28), reg_value);
		} else
