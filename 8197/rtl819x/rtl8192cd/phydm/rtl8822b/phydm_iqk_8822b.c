	...
	)
{
	...
	ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)0x0, 0xb8, bRFRegOffsetMask, 0x00A00);
	ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)0x0, 0xb8, bRFRegOffsetMask, 0x80A00);
}
	...
	)
{
	...
	if (step == 0x0) {
		...
		for (path = 0; path < 2; path++) {
			pIQK_info->LOK_IDAC[0][path] = ODM_GetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x58, bRFRegOffsetMask);
...
{	
	...
	for (path = 0; path < 2; path++) {
		if (reload_idx == 2) {
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xdf, BIT4, 0x1);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x58, bRFRegOffsetMask, pIQK_info->LOK_IDAC[channel][path]);
		}
...
{	
	...
	if (reset) {
		...
		pIQK_info->RFReg18 = ODM_GetRFReg(pDM_Odm, ODM_RF_PATH_A, 0x18, bRFRegOffsetMask);
	...
	)
{
	...
	for (path = 0; path < 2; path++) {
		...
		tmp = ODM_GetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xdf, bRFRegOffsetMask);
		tmp = (tmp&(~BIT4))|BIT1|BIT11;
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xdf, bRFRegOffsetMask, tmp);	
			
		/*release 0x56 TXBB*/
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x65, bRFRegOffsetMask, 0x09000);	
		
		if (*pDM_Odm->pBandType == ODM_BAND_5G) {
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xef, BIT19, 0x1);
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x33, bRFRegOffsetMask, 0x00026);
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x3e, bRFRegOffsetMask, 0x00037);
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x3f, bRFRegOffsetMask, 0xdefce);
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xef, BIT19, 0x0);
		} else {
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xef, BIT19, 0x1);
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x33, bRFRegOffsetMask, 0x00026);
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x3e, bRFRegOffsetMask, 0x00037);
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0x3f, bRFRegOffsetMask, 0x5efce);
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)path, 0xef, BIT19, 0x0);
		}
	}
}
	...
	)
{
	...
	case ODM_BAND_2_4G:
		...
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x56, bRFRegOffsetMask, 0x50df2);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x8f, bRFRegOffsetMask, 0xadc00);
		/* WE_LUT_TX_LOK*/
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0xef, BIT4, 0x1);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x33, BIT1|BIT0, 0x0);		
		break;
	case ODM_BAND_5G:
		ODM_Write1Byte(pDM_Odm, 0x1b2b, 0x80);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x56, bRFRegOffsetMask, 0x5086c);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x8f, bRFRegOffsetMask, 0xa9c00);
		/* WE_LUT_TX_LOK*/
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0xef, BIT4, 0x1);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x33, BIT1|BIT0, 0x1);
		break;
}
	...
	)
{
	...
	case ODM_BAND_2_4G:
		...
		break;
	case ODM_BAND_5G:
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x56, bRFRegOffsetMask, 0x500ef);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x8f, bRFRegOffsetMask, 0xa9c00);
		ODM_Write1Byte(pDM_Odm, 0x1b2b, 0x80);
		break;
	}
	...
	)
{
	...
	case ODM_BAND_2_4G:
		...
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x56, bRFRegOffsetMask, 0x510e0);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x8f, bRFRegOffsetMask, 0xacc00);
			break;
	case ODM_BAND_5G:
	...
	)
{
	...
	case ODM_BAND_5G:
		...
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x56, bRFRegOffsetMask, 0x51060);
		ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x8f, bRFRegOffsetMask, 0xa9c00);
		break;
	}
/*	ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]Set RXK setting!!!!\n"));*/
...
{	
	...
	u1Byte	IQMUX[4] = {0x9, 0x12, 0x1b, 0x24};
	u1Byte	idx;

	for (idx = 0; idx < 4; idx++)	
		if (pIQK_info->tmp1bcc == IQMUX[idx])
			break;
	...
	if (step == RXIQK2) {
		RFReg0 = ODM_GetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x0, bRFRegOffsetMask);
		ODM_Write4Byte(pDM_Odm, 0x1b00, 0xf8000008 | Path << 1);
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, 
		("[IQK]S%d ==> RF0x0 = 0x%x, tmp1bcc = 0x%x, idx = %d, 0x1b3c = 0x%x\n", Path, RFReg0, pIQK_info->tmp1bcc, idx, ODM_Read4Byte(pDM_Odm, 0x1b3c)));
		tmp = (RFReg0 & 0x1fe0) >> 5;
		...
		if (bb_idx == 0x1) {
			if (pIQK_info->lna_idx != 0x0)
				...
				pIQK_info->isbnd = true;
			fail = true;				
		} else if (bb_idx == 0xa) {
			if (idx != 0)
				idx--;
			else if (pIQK_info->lna_idx != 0x7)
				pIQK_info->lna_idx++;
			else
				pIQK_info->isbnd = true;				
			fail = true;
		...
		if (pIQK_info->isbnd == true)
			fail = false;
		
		pIQK_info->tmp1bcc = IQMUX[idx];
#endif
...
{	
	...
	if (pDM_Odm->DebugComponents && ODM_COMP_CALIBRATION) {
		...
		if (idx != TXIQK) 
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, 
				("[IQK]S%d ==> RF0x0 = 0x%x, RF0x56 = 0x%x\n", Path, ODM_GetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x0, bRFRegOffsetMask),
				ODM_GetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x56, bRFRegOffsetMask)));
	}
	...
	if (idx == RXIQK2) {
		pIQK_info->RXIQK_AGC[0][Path] =
			(u2Byte)(((ODM_GetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)Path, 0x0, bRFRegOffsetMask) >> 5) & 0xff) |
			(pIQK_info->tmp1bcc << 8));
...
{	
	...
	u1Byte		i;
		...
		case 2:		/*gain search_RXK2*/
			_IQK_RXK2Setting_8822B(pDM_Odm, Path, true);
			pIQK_info->isbnd = false;
			while (1) {
				KFAIL = _IQK_RXIQK_GainSearchFail_8822B(pDM_Odm, Path, RXIQK2);
			if (KFAIL && (pIQK_info->gs_retry_count[0][Path][RXIQK2] < rxiqk_gs_limit))
		...
		case 3:		/*RXK1*/
			...
			break;	
		case 4:		/*RXK2*/
			_IQK_RXK2Setting_8822B(pDM_Odm, Path, false);
			gonext = FALSE;
			while (1) {
...
_IQK_IQKbyPath_8822B(
	IN	PVOID		pDM_VOID,
	IN BOOLEAN		segment_iqk
)
{	
	PDM_ODM_T	pDM_Odm = (PDM_ODM_T)pDM_VOID;
	PIQK_INFO	pIQK_info = &pDM_Odm->IQK_info;
	BOOLEAN		KFAIL = TRUE;
	u1Byte		i, kcount_limit;
...
/*	ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, ("[IQK]IQKstep = 0x%x\n", pDM_Odm->RFCalibrateInfo.IQKstep)); */
	...
	if ((segment_iqk == true) && (pIQK_info->kcount == kcount_limit))
		break;		
#endif
	}
}
...
{	
	...
	_IQK_IQKbyPath_8822B(pDM_Odm, segment_iqk);
	...
	)
{
	...
	ODM_Write4Byte(pDM_Odm, 0x1b10, 0x88011C00);
}
	...
	)
{
	...
	while (1) {
		...
		_IQK_StartIQK_8822B(pDM_Odm, segment_iqk);
		...
		pIQK_info->kcount = 0;
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]delay 50ms!!!\n"));			
		ODM_delay_ms(50);
	};
	...
	_IQK_rf0xb0_workaround(pDM_Odm);

	if (!is_mp)
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]Total IQK ProgressingTime = %lld ms\n",
...
/*IQK version:v3.5, NCTL v0.6*/
/*1. add the workaround of RF0xb8 control for synthesizer*/
