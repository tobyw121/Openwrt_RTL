...
)
{
	...
	for(i = 0; i < numCandidate; i++) {
		...
		pEntry = CandidateEntry[i];				
		pstat = pEntry->pSTA;
		
		if(pstat->muPartner) { // already has Partner
			continue;
		}
		
		for(candidateIdx=i+1; candidateIdx < numCandidate; candidateIdx++) {
			isCandidate = FALSE;
			if(pEntry->mu_reg_index != CandidateEntry[candidateIdx]->mu_reg_index)
				if(bitmap & bitmap_mask[pEntry->mu_reg_index] & bitmap_mask[CandidateEntry[candidateIdx]->mu_reg_index])
					isCandidate = TRUE;
			if(isCandidate) {
				pstat->muPartner = CandidateEntry[candidateIdx]->pSTA;
				if(pstat->muPartner->muPartner) {
					pstat->muPartner = NULL;
					continue;
				}				
				pstat->muPartner->muPartner = pstat;
				pairResult[pEntry->mu_reg_index] = CandidateEntry[candidateIdx]->mu_reg_index;
				pairResult[CandidateEntry[candidateIdx]->mu_reg_index] = pEntry->mu_reg_index;
					
				break;
			}
		}

		// Calculate the ratio of tx_rate
		if(pstat->muPartner) {
			...
			groupIndex = e1e2_to_gid[pairResult[pEntry->mu_reg_index]][pEntry->mu_reg_index];		
			...
			if(pstat->mu_tx_rate > 0 && pstat->muPartner->mu_tx_rate > 0) {
				pstat->mu_deq_num = 2* priv->pshare->rf_ft_var.dqnum * (pstat->mu_tx_rate) / (pstat->mu_tx_rate + pstat->muPartner->mu_tx_rate);
				pstat->muPartner->mu_deq_num = 2* priv->pshare->rf_ft_var.dqnum - pstat->mu_deq_num;
			} else {
				pstat->mu_deq_num = priv->pshare->rf_ft_var.dqnum;
				pstat->muPartner->mu_deq_num = priv->pshare->rf_ft_var.dqnum;
			}
			...
			{				
				...
				{ /* More than 1 MU STA*/											
					...
					if((cnt == 1) && (AC_SIGMA_MODE == AC_SIGMA_NONE)) { //start mu sounding when 2 sta connected  for 11ac logo r2
						...
						NDPTxRate = Beamforming_GetVHTNDPTxRate(priv, pEntry->CompSteeringNumofBFer);
						SendVHTNDPAPacket_MU(priv, pEntry->MacAddr, pEntry->AID, pEntry->BW, NDPTxRate, 1);
					} else if (cnt > 1) {
						for(idx = 0; idx < cnt; idx++) {
							pEntry = pEntry_poll[idx];
							if(idx == 0)
								SendVHTNDPAPacket_MU(priv, pEntry->MacAddr, pEntry->AID, pEntry->BW, _NSS2_MCS0_RATE_, 0);
							else if(idx == cnt-1)
								SendReportPollPacket(priv, pEntry->MacAddr, pEntry->BW, _6M_RATE_,TRUE);	
							else
...
{	
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_START)
	{
		...
		if(0) {//if (pEntry->LogStatusFailCnt > 50) {
