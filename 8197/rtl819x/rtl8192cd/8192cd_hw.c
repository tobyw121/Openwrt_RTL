...
//eric-8822 ?? odm ability
	...
	if((GET_CHIP_VER(priv) == VERSION_8814A)&&(priv->pmib->dot11RFEntry.rfe_type == 7))
	...
	*/
...
#endif
    {
        ...
        if (IS_HAL_CHIP(priv)) {
			...
			if(ODMPTR->ConfigBBRF) //eric-8822
            {
...
#ifdef RTL8814_TP_TUNING
	// TXOP
	RTL_W8(0x50A, 0x5E);

	// agg num, not use 63 temporarily
	//RTL_W16(0x4CA, 0x3F3F);

	// prefetch
	//RTL_W8(0x3DB, 0xB);
    
	//ew 0xbb805100 0x11
	//ew 0xb8010000 0xe4000000
#endif
...
void TxACurrentCalibration(struct rtl8192cd_priv *priv)
{
	...
	panic_printk("********************************\n");
#if 0
	ReadEFuseByte(priv, 0x3D7, &val);
	panic_printk("-----------old efuse content 0x3D7 = 0x%x\n",val);
	ReadEFuseByte(priv, 0x3D8, &val);
	panic_printk("-----------old efuse content 0x3D8 = 0x%x\n",val);
#endif
