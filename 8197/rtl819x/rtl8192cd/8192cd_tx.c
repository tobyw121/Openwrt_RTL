		...
		*bRtsEnable = TRUE;
			...
			priv->pmib->dot11ErpInfo.protection)
		{
			*bErpProtect = 1;
			...
			(txcfg->pstat) && (txcfg->pstat->MIMO_ps & _HT_MIMO_PS_DYNAMIC_)  &&  is_2T_rate(txcfg->tx_rate))
...
#if (MU_BEAMFORMING_SUPPORT == 1)
__inline__ static int rtl8192cd_swq_bdfull_MU(struct rtl8192cd_priv *priv, unsigned int num,  unsigned char qnum)
{
    int *tx_head=NULL, *tx_tail=NULL;
...
#endif
    {
        tx_head = get_txhead_addr(GET_HW(priv), qnum);
        tx_tail = get_txtail_addr(GET_HW(priv), qnum);
        if((num) > CIRC_SPACE_RTK(*tx_head, *tx_tail, CURRENT_NUM_TX_DESC))
            return 1;
    }
...
__inline__ static int rtl8192cd_swq_enqueue(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info	*pstat)
{
	...
	if(pstat->muPartner){
		if(pstat->swq.swq_en[q_num]) {
            //if(pstat->tx_avarage < (priv->pshare->rf_ft_var.mutp_th_lower * 125000) || pstat->muPartner->tx_avarage < (priv->pshare->rf_ft_var.mutp_th_lower * 125000) || pstat->mu_deq_num != pstat->muPartner->mu_deq_num) {
            if(pstat->tx_avarage < (priv->pshare->rf_ft_var.mutp_th_lower * 125000) || pstat->muPartner->tx_avarage < (priv->pshare->rf_ft_var.mutp_th_lower * 125000)) 
			{
            pstat->swq.swq_en[q_num] = 0; /*disable swq*/  
			pstat->swq.swq_en[q_num] = 0; /*disable swq*/
            if (pstat->swq.swq_timer_id[q_num])
                rtl8192cd_swq_deltimer(priv, pstat, q_num);
			if (pstat->muPartner->swq.swq_timer_id[q_num])
                rtl8192cd_swq_deltimer(priv, pstat->muPartner, q_num);
            if(pstat->swq.swq_empty[q_num]) {
                rtl8192cd_swq_dequeue(priv, pstat, q_num, 0, 1);                          
            }
			if(pstat->muPartner->swq.swq_empty[q_num]) {
                rtl8192cd_swq_dequeue(priv, pstat->muPartner, q_num, 0, 1);                          
            }
        }
        }
		else {
			if(pstat->tx_avarage > (priv->pshare->rf_ft_var.mutp_th_up * 125000) && pstat->muPartner->tx_avarage > (priv->pshare->rf_ft_var.mutp_th_up* 125000)) {
				...
				pstat->muPartner->swq.swq_en[q_num] = 1; /*enable swq*/ 
		        pstat->muPartner->swq.swq_prev_timeout[q_num] = 1;
		        pstat->muPartner->swq.swq_timeout_change[q_num] = 0;
		        pstat->muPartner->swq.swq_keeptime[q_num] = 0;
			}
		}
	}
	else
...
#if (MU_BEAMFORMING_SUPPORT == 1)		
	if (pBeamInfo->beamformee_mu_cnt >= 2 && pstat->muPartner) {
		...
		qlen1 = skb_queue_len(&pstat->muPartner->swq.swq_queue[q_num]);
		
		if(qlen <= pstat->mu_deq_num || qlen1 <= pstat->muPartner->mu_deq_num){
			need_fire = 0;
		}	
				...
				if(qlen >= psta[0]->mu_deq_num && qlen2 >= psta[1]->mu_deq_num){
				    ...
				    {	
						...
						if(count > 0 && count2 > 0) {
							...
							if(rtl8192cd_swq_bdfull_MU(priv, sta_deq_num[0]+sta_deq_num[1], q_num)) {           
								break;
				    		}
	...
	if(pstat->muPartner) {
		if(pstat->swq.swq_empty[q_num]) { /*queue not empty, add timer for it*/        
	        if (pstat->swq.swq_timer_id[q_num] == 0)
	        {
            	...
            	pstat->swq.swq_timer_status[q_num] = rtl8192cd_swq_settimer(priv, priv->pshare->swq_timer[pstat->swq.swq_timer_status[q_num] - 1].timeout); 
        	}			
		}
		if(pstat->muPartner->swq.swq_empty[q_num]) { /*queue not empty, add timer for it*/        
	        if (pstat->muPartner->swq.swq_timer_id[q_num] == 0)
	        {
				if(priv->pshare->swq_use_hw_timer) {
					if(priv->pshare->rf_ft_var.qtime)
						tri_time = priv->pshare->rf_ft_var.qtime;
					if(pstat->muPartner->swq.swq_prev_timeout[q_num] != tri_time) {
	                    pstat->muPartner->swq.swq_prev_timeout[q_num] = tri_time;
	                    pstat->muPartner->swq.swq_timeout_change[q_num] = 1;                                 
	                }  
	                tri_time = RTL_R32(TSFTR1) + tri_time*1000;
				} else {
					tri_time = jiffies + RTL_MILISECONDS_TO_JIFFIES(tri_time);
				}
				rtl8192cd_swq_addtimer(priv, pstat->muPartner, q_num, tri_time);           
	            pstat->muPartner->swq.swq_timer_status[q_num] = rtl8192cd_swq_settimer(priv, tri_time);   /*need to re-setup timer*/                        
	        } else if(pstat->muPartner->swq.swq_timer_status[q_num]) { /*re-set the timer*/
            	pstat->muPartner->swq.swq_timer_status[q_num] = rtl8192cd_swq_settimer(priv, priv->pshare->swq_timer[pstat->muPartner->swq.swq_timer_status[q_num] - 1].timeout); 
        	}
		}
                ...
                if(bdfull && skb_queue_len(&pstat->swq.swq_queue[qnum]) < pstat->swq.q_aggnum[qnum]*2 ) {
                    ...
                    add_timer_timeout += current_time; 
                        ...
                        if(priv->pshare->swq_use_hw_timer) {
                        	...
                        	rtl8192cd_swq_dequeue(priv_this, pstat, qnum, 0, 1);
                            pstat->swq.q_TOCount[qnum]++;                        
                        }
                        else {
                        	pstat->swq.q_TOCount[qnum] += rtl8192cd_swq_dequeue(priv_this, pstat, qnum, 0, 1); 
                        }
                }
	...
	for(qnum = VO_QUEUE; qnum >= BK_QUEUE ; qnum--){
		...
		while(sta_cnt < NUM_STAT){
			...
			if (priv->pshare->aidarray[(i%NUM_STAT)] && (priv->pshare->aidarray[(i%NUM_STAT)]->used == TRUE)){
				...
				bd_full = rtl8192cd_swq_bdfull(priv, pstat, qnum);
				if(!bd_full){					
					burst_num = (priv->pshare->rf_ft_var.atm_aggmax*pstat->atm_burst_size)/priv->pshare->atm_max_burstsize;
					if(burst_num < priv->pshare->rf_ft_var.atm_aggmin)
						burst_num = priv->pshare->rf_ft_var.atm_aggmin;
