		   ...
		   Int32 dest_ip, int dest_port, unsigned char *dest_arp)
{
	...
	packet.udp.chksum= ipheader_chksum((unsigned short *)(&packet), sizeof(struct udp_dhcp_packet));
	packet.ip.len = htons(sizeof(struct udp_dhcp_packet));
	packet.ip.verhdrlen= (4<<4) |(sizeof(packet.ip) >> 2);
	packet.ip.ttl = 64;
	packet.ip.chksum= ipheader_chksum((unsigned short *)(&packet.ip), sizeof(packet.ip));

//	prepare_txpkt(0, FRAME_IP, dest_arp, (Int8 *)(&packet),packet.ip.len);
	prepare_txpkt(0, FRAME_IP, dest_arp, (Int8 *)(&packet), ntohs(packet.ip.len));
	return 0;
}
