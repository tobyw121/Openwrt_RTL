...
#define CHECK_PERIOD_S 5
#define CHECK_T_MS 200
#define CHECK_CNT (1000/CHECK_T_MS*CHECK_PERIOD_S)
#endif
...
int check_system_image(unsigned long addr,IMG_HEADER_Tp pHeader)
{
	...
	if (!memcmp(pHeader->signature, FW_SIGNATURE, SIG_LEN))
		...
		dprintf("1 no sys signature at %X!\n",addr);
...
int check_system_image(unsigned long addr,IMG_HEADER_Tp pHeader)
{
	...
	if (!memcmp(pHeader->signature, FW_SIGNATURE, SIG_LEN))
		...
		dprintf("2 no sys signature at %X!\n",addr);
#if defined(NEED_CHKSUM)	
	if (ret) {
		for (i=0; i<pHeader->len; i+=2) {
			sum += *((unsigned short *)(addr + sizeof(IMG_HEADER_T) + i));
...
int check_system_image(unsigned long addr,IMG_HEADER_Tp pHeader,SETTING_HEADER_Tp setting_header)
{
	...
	prom_printf("addr:0x%x, sig:%02x %02x %02x %02x\n", addr, pHeader->signature[0], pHeader->signature[1], pHeader->signature[2], 
	pHeader->signature[3]);
	...
	if (!memcmp(pHeader->signature, image_sig, SIG_LEN))
		...
		prom_printf("3 no sys signature at %X!\n",addr-FLASH_BASE);
	}		
...
#ifndef CONFIG_NAND_FLASH_BOOTING
	//flash mapping
	return_addr = (unsigned long)FLASH_BASE+CONFIG_LINUX_IMAGE_OFFSET_START+bank_offset;
	ret = check_system_image((unsigned long)FLASH_BASE+CONFIG_LINUX_IMAGE_OFFSET_START+bank_offset,pHeader, psetting_header);
#endif
...
//		Set_GPIO_LED_OFF();
		...
		REG32(0xb8003000) =0;
                REG32(0xb8003004) =0;
		REG32(0xb8003114)=0;
		REG32(0xb8000010)=(REG32(0xb8000010)&~(0xF0))|0x60;
