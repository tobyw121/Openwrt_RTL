...
typedef struct bootinfo_header {
	unsigned char tag[RTK_BOOTINFO_TAG_LEN] ;  // Tag + version
	unsigned int len ;
...
{
	memcpy(&boot->header.tag,RTK_BOOTINFO_SIGN,RTK_BOOTINFO_TAG_LEN);
...
int  rtk_check_bootinfo(BOOTINFO_P boot)
{
     ...
     if(!memcmp(boot->header.tag, RTK_BOOTINFO_SIGN, RTK_BOOTINFO_TAG_LEN)) //ok
	 	return 1;
