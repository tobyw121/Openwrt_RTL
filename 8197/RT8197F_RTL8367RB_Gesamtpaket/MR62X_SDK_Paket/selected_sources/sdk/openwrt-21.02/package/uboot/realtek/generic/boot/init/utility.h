...
#if defined(CONFIG_RTK_NORMAL_BBT) && defined(CONFIG_FS_JFFS2)
	#define _SYSTEM_HEAP_SIZE	(1024*64*4*16 + JFFS2_PART_SIZE)
#else
	#define _SYSTEM_HEAP_SIZE	1024*64*4*16	//wei add
#endif
#else
#ifdef CONFIG_FS_JFFS2
	#define _SYSTEM_HEAP_SIZE	1024*64*2	//wei add
#else
	#define _SYSTEM_HEAP_SIZE	1024*1024*4	//wei add
#endif
#endif
#ifdef CONFIG_CRYPTO_DEV_REALTEK
	#define _SYSTEM_HEAP_SIZE	1024*64*4*16
#endif
...
//flash mapping
#define ROOT_FS_OFFSET		(CONFIG_ROOT_IMAGE_OFFSET_START)
...
#define Set_GREEN_LED_OFF()		(REG32(BSP_GPIO_DAT_REG(BSP_GPIO_PIN_C3)) =  REG32(BSP_GPIO_DAT_REG(BSP_GPIO_PIN_C3)) & (~(1<<BSP_GPIO_BIT(BSP_GPIO_PIN_C3))))
#define Set_GREEN_LED_ON()      (REG32(BSP_GPIO_DAT_REG(BSP_GPIO_PIN_C3)) =  REG32(BSP_GPIO_DAT_REG(BSP_GPIO_PIN_C3)) | (1<<BSP_GPIO_BIT(BSP_GPIO_PIN_C3)))

#define Set_ORANGE_LED_OFF()	(REG32(BSP_GPIO_DAT_REG(BSP_GPIO_PIN_G6)) =  REG32(BSP_GPIO_DAT_REG(BSP_GPIO_PIN_G6)) & (~(1<<BSP_GPIO_BIT(BSP_GPIO_PIN_G6))))
#define Set_ORANGE_LED_ON()     (REG32(BSP_GPIO_DAT_REG(BSP_GPIO_PIN_G6)) =  REG32(BSP_GPIO_DAT_REG(BSP_GPIO_PIN_G6)) | (1<<BSP_GPIO_BIT(BSP_GPIO_PIN_G6)))
