...
#endif
#if 0
static void _rtl8198c_dumpRouteList(rtl8198c_ipv6_route_t *rtHead)
{
	rtl8198c_ipv6_route_t *rt = rtHead;
	printk("=============================================\n");
	while(rt){
		printk("%s:%d  dstNetif=%s address=%08x:%08x:%08x:%08x fc_dst_len=%d nextHop=%08x:%08x:%08x:%08x\n",__FUNCTION__,__LINE__,
			(rt->dstNetif&&rt->dstNetif->name)?(char*)rt->dstNetif->name:"NULL",
			rt->ipAddr.v6_addr32[0],rt->ipAddr.v6_addr32[1],rt->ipAddr.v6_addr32[2],rt->ipAddr.v6_addr32[3],
			rt->fc_dst_len,
			rt->nextHop.v6_addr32[0],rt->nextHop.v6_addr32[1],rt->nextHop.v6_addr32[2],rt->nextHop.v6_addr32[3]
			);
...
#endif
rtl8198c_ipv6_route_t * rtl8198c_get_ipv6_route_inusedHead(void)
{
	return rtl8198c_ipv6_route_inusedHead;
}
...
static rtl8198c_ipv6_route_t* _rtl8198c_getIpv6RouteEntry(inv6_addr_t ipAddr, int fc_len)
{
	...
	while(rt)
	{
		if((rt->valid==1) && (rt->fc_dst_len==fc_len) && (_rtl8198c_ipv6_prefix_equal((&rt->ipAddr), (&ipAddr), rt->fc_dst_len)))
			return rt;
...
{
	inv6_addr_t tmp_nexthop,zero_cmp;
...
#endif
#endif
	bzero(&zero_cmp,sizeof(inv6_addr_t));
	...
	*/
	...
	if ((fc_dst_len==0) 
		|| ((memcmp(&nextHop,&zero_cmp,sizeof(inv6_addr_t))!=0) && (memcmp(&nextHop,&ipAddr,sizeof(inv6_addr_t))!=0))) 
#else
	if (fc_dst_len==0)
#endif
	{
	...
	if(ifName && (strncmp(ifName, ppp_ext, 5)==0)   //ppp0
		&&((ipAddr.v6_addr32[0]>>16)!=0x0 && (ipAddr.v6_addr32[0]>>16)!=0xFE80) //global ip
		&& fc_dst_len==128) 
		{		
			if(_rtl8198c_getIpv6GloRouteCountByIfname(ifName,fc_dst_len)>2){ //For pppoe,  at most three globle route can be added to hw route table
					return RTL_EENTRYALREADYEXIST;
			}
			retval=_rtl8198c_Ipv6ToCpu(rt,netif);
	...
	if (entry->asicIdx == (IPV6_RT_ASIC_ENTRY_NUM-1)) {
		...
		*/
		...
		if(_rtl8198c_usedNetifInIpv6Route(entry->dstNetif->name) == FAILED)
		{
			...
			//for ipv6 hw multicast, v6 routing is always needed.
		#else
...
rtl8198c_ipv6_route_t* _rtl8198c_getIpv6RouteEntryByIp(inv6_addr_t dst)
{
	...
	while(rt)
	{
		if (rt->valid == 1 && (_rtl8198c_ipv6_prefix_equal((&rt->ipAddr), (&dst), rt->fc_dst_len)) && fc_len <= rt->fc_dst_len) {
