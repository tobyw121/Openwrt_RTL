...
extern char hw_nat_netif_name[IFNAMSIZ];
...
__u8 rtl_get_skb_orig_dscp(struct sk_buff *skb)
{
	...
	#else
	// when disable CONFIG_RTL_DSCP_IPTABLE_CHECK, skb don't have original_dscp field	
	return 0;
	#endif
}
...
uint16 rtl_get_skb_remark_vlanpri(struct sk_buff *skb)
{
	...
	return skb->srcVlanPriority;
	#else
	// when disable CONFIG_RTL_VLANPRI_IPTABLE_CHECK, skb don't have srcVlanPriority field	
	return 0;
	#endif
}
...
void rtl_set_skb_ori_vlanpri(struct sk_buff *skb)
{
	...
	skb->srcVlanPriority= rtl_get_skb_orig_vlanpri(skb);
	#else
...
{
	#if defined(CONFIG_RTL_QOS_8021P_SUPPORT)
	return skb->srcVlanPriority = pri;
	#else
	// when disable CONFIG_RTL_VLANPRI_IPTABLE_CHECK, skb don't have srcVlanPriority field	
	return 0;
	#endif
...
int32 rtl_qosGetSkbMarkByNaptEntry(rtl865x_napt_entry *naptEntry, rtl865x_qos_mark *qosMark, struct sk_buff *pskb)
{
	...
	//replace original dscp to check uplink mark
	oriTos = iph->tos;
	iph->tos = (iph->tos &0x3)|((rtl_get_skb_orig_dscp(pskb)) << 2);

	//replace original vlanpri to check uplink mark
	...
	if(proto == ETH_P_IP){
		...
		(list_empty(&net->nf.hooks[PF_INET][NF_IP_PRE_ROUTING]))?: \
			ipt_do_table(pskb,&state,dev_net(lanDev)->ipv4.iptable_mangle);
#else
	...
	if(proto == ETH_P_IP){
			...
			(list_empty(&net->nf.hooks[PF_INET][NF_IP_POST_ROUTING]))?: \
				ipt_do_table(pskb,&state,dev_net(lanDev)->ipv4.iptable_mangle);
#else
...
#endif
	{
		...
		if(proto == ETH_P_IP){
			...
			(list_empty(&net->nf.hooks[PF_INET][NF_IP_PRE_ROUTING]))?: \
				ipt_do_table(pskb,&state,dev_net(wanDev)->ipv4.iptable_mangle);
#else
			(list_empty(&nf_hooks[PF_INET][NF_IP_PRE_ROUTING]))?: \
	...
	//back to original dscp
	iph->tos = oriTos;

	//back to remark vlanpri
...
int32 rtl865x_handle_nat(struct nf_conn *ct, int act, struct sk_buff *skb)
{
	...
	#endif
	if((act == 1) && linux_vlan_enable && linux_vlan_hw_support_enable && 
		!(rtl_isHwNatLanVlan(ct)==TRUE && rtl_isHwNatWan(skb)))
		return -1;
	#endif
	...
	else
	{
		...
		{
#ifdef CONFIG_RTL_LAYERED_DRIVER_L4
			rtl865x_checkL3L2Valid(sip);
			rtl865x_checkL3L2Valid(dip);
	...
	/*2007-12-19*/
	printk_ratelimited("%s:%d:(%s): errno=%d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
			__FUNCTION__,__LINE__,((act)?"add_nat": "del_nat"), rc, ((proto)? "tcp": "udp"),
			NIPQUAD(sip), ntohs(sp), NIPQUAD(dip), ntohs(dp), NIPQUAD(gip), gp);
	#endif
...
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
EXPORT_SYMBOL(_br0_ip);
EXPORT_SYMBOL(_br0_mask);
#endif
...
void rtl_delConnCache(struct nf_conn *ct)
{
	...
	if((!rtl_isRouterType(ct)&&(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip
		   == ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip))||(rtl_isRouterType(ct)&&rtl_isRouterTypeWantoLan(ct)))
	#else
	...
	else if(( !rtl_isRouterType(ct)&&(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip
			== ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u3.ip))||(rtl_isRouterType(ct)&&rtl_isRouterTypeLantoWan(ct))) 
	#else
	else if(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip== ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u3.ip) 
	#endif
...
int32 rtl_connCache_timer_update(struct nf_conn *ct)
{
	...
	if (time_after_eq(jiffies, ct->timeout.expires)) {
		#if defined(CONFIG_RTL_IPTABLES_FAST_PATH)
		if ((fast_nat_fw) && (SUCCESS==rtl_fpTimer_update((void*)ct))) {
		...
		if(arp_req_get_ha(intIp, lan_dev, Mac)==0){
			...
			#endif
			{
				if((dst->dst->dev->vlan_id == rtk_vlan_hw_nat_lan_vid)
					#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
					|| ((extPortEnabled) && !strncmp(dst->dst->dev->name, "wlan", 4))
					#endif
				   )
					return TRUE;
			}
...
int rtl_isHwNatWan(struct sk_buff *skb)
{
	...
	if(!strncmp(skb->dev->name, RTL_DRV_BR_WAN_NETIF_NAME, 6) && 
		strlen(skb->dev->name) == strlen(RTL_DRV_BR_WAN_NETIF_NAME))
		return 1;
#endif

	if(!strncmp(skb->dev->name, RTL_DRV_WAN0_NETIF_NAME, 4) && 
		strlen(skb->dev->name) == strlen(RTL_DRV_WAN0_NETIF_NAME))
		return 1;
	
	if(!strncmp(skb->dev->name, RTL_DRV_PPP_NETIF_NAME, 5))
		return 1;
...
static int rtl_checkLanIp_for_ext_port(struct nf_conn *ct)
{
	 __be32 lanIp=0;
     ...
     if(rtl865x_isArpExisted(htonl(lanIp))==TRUE)
     {
        return SUCCESS;
     }
...
void rtl_addConnCache(struct nf_conn *ct, struct sk_buff *skb)
{
...
#if defined(CONFIG_RTL_URL_PATCH)
	if((urlPatchEnabled)&&((protocol == NP_TCP)&&(ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all) == URL_PROTO_PORT)&&assured))
		assured = 0;
#endif
#ifdef CONFIG_FP_BYPASS_PACKET
	...
	/*1.add "!(ct->helper)" to fix ftp-cmd type packet
	  ...
	  */
	if ((fast_nat_fw) && (assured || create_conn)) {
	...
	#endif
	{
		...
		{ /* inbound flow */
			sip	= ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u3.ip;
			dip 	= ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip;
		}
		...
		//	return;
		#if defined (CONFIG_RTL_EXT_PORT_SUPPORT)
		/*when open extension port, should check internal ip is in hardware arp table or not
         	*to fix: pppoe wan tye, wan server can't access web UI except 80 port*/
		if(extPortEnabled && (rtl_checkLanIp_for_ext_port(ct)==SUCCESS)){
		      ...
		      unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(&gHwNatSetting, buffer, 8)) {
		...
		{
			if((gHwNatSetting[1] == '5') /* L2TP */ || (gHwNatSetting[1] == '4') /* PPTP */)
			{
				gHwNatPPTPL2TP = 1;
			}
			else
			...
			||(gHwNatSetting[1] == '4') /* PPTP */))
		{
			...
			extern int rtl_add_Acl_for_out_of_br_subnet(void);
			rtl_add_Acl_for_out_of_br_subnet();
			#endif
		}
		...
		else if(gHwNatSetting[0] == '6'){
			extern int rtl_add_Acl_for_br_remain_ip(void);
			rtl_add_Acl_for_br_remain_ip();
		}
		#endif
		#endif
		      ...
		      unsigned long count, void *data)
{
	...
	#if defined(RTL_TRAP_BR_SECOND_IP_TO_CPU)
		else if(gSwNatSetting[0] == '6'){
			extern int rtl_add_Acl_for_br_remain_ip(void);
			rtl_add_Acl_for_br_remain_ip();
		}
	#endif
...
extern int rtl865x_curOpMode;
#if (defined(CONFIG_RTL_LOCAL_PUBLIC) || defined(CONFIG_RTL_MULTIPLE_WAN) || defined(CONFIG_RTL_REDIRECT_ACL_SUPPORT_FOR_ISP_MULTI_WAN) ||(defined(CONFIG_NET_SCHED)&&defined(CONFIG_RTL_IPTABLES_FAST_PATH)) || defined(CONFIG_RTL_HW_QOS_SUPPORT)) && defined(CONFIG_RTL_819X_SWCORE)
	...
	if (in_dev != NULL) {
		for (ifap=in_dev->ifa_list; ifap != NULL; ifap=ifap->ifa_next) {
			if (strcmp(dev->name, ifap->ifa_label) == 0){
				if (strncmp(dev->name, ppp_ext, 5)==0)
		      ...
		      unsigned long count, void *data)
{
      ...
      if (buffer && !copy_from_user(gQosSetting, buffer, count)) {
          ...
          if (gQosSetting[0] == '0')
                gQosEnabled = 0;
#if defined CONFIG_RTL_HW_QOS_SUPPORT
		  else if(gQosSetting[0] == '2')
		  {
		  		rtl865x_qosFlushBandwidth(RTL_PS_BR0_DEV_NAME);
				rtl865x_qosFlushBandwidth(RTL_PS_WAN0_DEV_NAME);
				rtl865x_closeQos(RTL_PS_BR0_DEV_NAME);
				rtl865x_closeQos(RTL_PS_WAN0_DEV_NAME);
		  }
#endif
          else
                gQosEnabled = 1;
...
#if defined CONFIG_RTL_83XX_QOS_SUPPORT
	rtl83xx_setFlowControl(gQosEnabled);
#endif
...
extern bool rtl_tcp_in_window(const struct nf_conn *ct,
		...
		struct sk_buff *skb)
{
	...
	if (!rtl_tcp_in_window(ct, &ct->proto.tcp, dir, index,
...
int syn_asic_arp(struct neighbour *n, int add)
{
...
#if defined(CONFIG_RTL_HARDWARE_IPV6_SUPPORT)
	{
		...
		if (n->tbl && (n->tbl->family==AF_INET6) && (n->tbl->key_len==sizeof(inv6_addr_t)))
			rc_ipv6 = add? rtl8198c_addIpv6Arp(tmp_ip, (void *)n->ha,n->dev?n->dev->name:NULL): rtl8198c_delIpv6Arp(tmp_ip);
	}
...
#ifdef CONFIG_TP_IMAGE
	if(memcmp(name, "pppoe", 5) == 0 || memcmp(name, "share", 5) == 0)
	{
		if(rtl_masq_info[0].ipAddr == ipAddr)
		{
			return SUCCESS;
		}
		else
		{
			/* PPPoE accelerates the first connection */
			rtl_masq_info[0].valid = 0;
		}
	}
#endif
...
static int rtl_get_masquerade_netif(struct xt_table_info *private,struct ipt_entry *masq_entry)
{
	...
	else
	{
		...
		while(e)
		{
			...
			{
#ifdef CONFIG_TP_IMAGE
				/* MASQUERADE entry found. Write to ASIC IP table. */
				if(masq_name[0] !='\0' && memcmp(masq_name, "pptp", 4) && memcmp(masq_name, "l2tp", 4))
				{
					...
					if ((dev)&&(dev->ip_ptr))
					{
						...
						if (ina!=NULL)
						{
							...
							if(memcmp(masq_name, "pppoe", 5) == 0 || memcmp(masq_name, "share", 5) == 0)
							{
								/* PPPoE/share only accelerates the first connection */
								break;
							}
						}
					}
				...
				/* Jump to the next entry until error */
			...
			//if error target
#ifdef CONFIG_TP_IMAGE
			if( ( jump_cnt >= 20 ) || (strcmp(t->u.kernel.target->name, "ERROR") == 0))
#else
	...
	if(multi_phy_vir_wan_enable)
	{
		if((masq_name[0] !='\0')&&(memcmp(masq_name, hw_nat_netif_name,strlen(hw_nat_netif_name))==0))
		{
			struct in_ifaddr *ina;
			dev = __dev_get_by_name(&init_net,masq_name);
			if ((dev)&&(dev->ip_ptr))
		...
		const unsigned int *underflows)
{
	...
	{	
		...
		if(multi_phy_vir_wan_enable)
		{
			if(rtl_masq_info[i].valid == 1 && rtl_masq_info[i].ipAddr && (memcmp(rtl_masq_info[i].ifName, hw_nat_netif_name,strlen(hw_nat_netif_name))==0))
			{		
...
int32 rtl_fn_insert(struct fib_table *tb, struct fib_config *cfg, struct fib_info *fi)
{
	...
	if ((tb->tb_id == RT_TABLE_MAIN) && (gHwNatEnabled==1)) {
		...
		#endif
		{
			#ifdef CONFIG_RTL_LAYERED_DRIVER_L3
			//printk("-------------------%s(%d)\n",__FUNCTION__,__LINE__);
			rc = rtl865x_addRoute(htonl(ipDst),htonl(ipMask),htonl(ipGw),dev_t,htonl(srcIp));
...
int32 rtl_fib_flush(u32 tb_id, u32 fn_key, u32 ip_mask)
{
	...
	if (tb_id==RT_TABLE_MAIN) {
		ipDst =fn_key;
		ipMask = inet_make_mask(ip_mask);

		if (!ipDst || (!ipv4_is_multicast(ipDst) && !ipv4_is_loopback(ipDst) && (ipDst != 0xffffffff))) {
			rc = 0;
			#ifdef CONFIG_RTL_LAYERED_DRIVER_L3
			rc = rtl865x_delRoute(ipDst, htonl(ipMask));
			#endif
		}
	}
	...
	if (cp->hw_acc) {
		...
		if (elapsed >= 0 && (cp->protocol==IPPROTO_UDP))
		{
			...
			printk("%s:%d:(%s): expired time = %d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
					__FUNCTION__,__LINE__,"poll_nat", expires-elapsed, (cp->protocol==IPPROTO_TCP)? "tcp": "udp",
...
void rtl_tcp_state_transition_check(struct ip_vs_conn *cp, int direction, const struct sk_buff *skb, struct ip_vs_protocol *pp)
{
	...
	*/
	...
	#endif
	{
		...
		/*2007-12-19*/
		printk("%s:%d:(%s): errno=%d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
				__FUNCTION__,__LINE__,"add_nat", rc, "tcp", NIPQUAD(cp->daddr), ntohs(cp->dport),
				NIPQUAD(cp->caddr), ntohs(cp->cport), NIPQUAD(cp->vaddr), ntohs(cp->vport)
		);
		#endif
	}
}
...
void rtl_udp_state_transition_check(struct ip_vs_conn *cp, int direction, const struct sk_buff *skb, struct ip_vs_protocol *pp)
{
	...
	if (!cp->hw_acc && !cp->app)
	{
		...
		/*2007-12-19*/
		printk("%s:%d:(%s): errno=%d\n %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
				__FUNCTION__,__LINE__,"add_nat", rc, "udp", NIPQUAD(cp->daddr), ntohs(cp->dport),
				NIPQUAD(cp->caddr), ntohs(cp->cport), NIPQUAD(cp->vaddr), ntohs(cp->vport)
		);
		#endif
	}
...
#if defined(CONFIG_RTL_HARDWARE_IPV6_SUPPORT)
unsigned int HardwareV6Enable=1;
int32 rtl8198c_ipv6_router_add(struct rt6_info *rt)
{
	char *dev_t;
	int rc, fc_dst_len,tmp_fc_dst_len, addr_type,isPppGloLo=0;
	inv6_addr_t ipDst, ipGw, tmp_dst, tmp_gw,in6_tmp;
	int j=0;
	struct net_device *netif;
	uint32 tb6_id = 0,tmp_ip1,tmp_ip2;
	struct inet6_dev *idev;
	struct inet6_ifaddr *ifa;
	int match=0;
	if(HardwareV6Enable==0)
		return -1;

#if defined(CONFIG_MAP_E_SUPPORT)
	return FAILED;
#endif
	...
	if (tb6_id == RT6_TABLE_MAIN) {
		...
		if(fc_dst_len==128
			&& (ipDst.v6_addr32[0]>>16)!=0xFE80 
			&&rt&&rt->rt6i_idev && rt->rt6i_idev->dev && strncmp(rt->rt6i_idev->dev->name, ppp_ext, 5)==0 
			&& rt->dst.dev&& strncmp(rt->dst.dev->name,"lo",2)==0			
			&& (!(addr_type&IPV6_ADDR_MULTICAST))&&(!(addr_type&IPV6_ADDR_LOOPBACK))&&(!(rt->rt6i_flags&RTF_ANYCAST)))
			{//make ipv6 pppoe global lo address add to hw route to cpu
				dev_t=rt->rt6i_idev->dev->name;
				isPppGloLo=1;
			}
		...
		if ((((ipDst.v6_addr32[0]==0)&&(ipDst.v6_addr32[1]==0)&&(ipDst.v6_addr32[2]==0)&&(ipDst.v6_addr32[3]==0)) ||
		     ...
		     || isPppGloLo) {
			if( strncmp(rt->rt6i_idev->dev->name, ppp_ext, 5)==0)
			{	
			
				idev=rt->rt6i_idev;
				if(idev)
				{
					if((tmp_dst.v6_addr32[0]&0xffff0000)!=0xfe800000)
					{
					list_for_each_entry(ifa, &idev->addr_list, if_list) {
						if(ifa->prefix_len!=fc_dst_len)
							continue;
						in6_tmp=*((inv6_addr_t *)&(ifa->addr));
						htonl_array((u32 *)&in6_tmp.v6_addr32[0], (u32 *)&in6_tmp.v6_addr32[0], 4);
						tmp_fc_dst_len=fc_dst_len/32;
						match=1;
						for(j=0;j<tmp_fc_dst_len;j++)
							if(in6_tmp.v6_addr32[j]!=tmp_dst.v6_addr32[j])
							{
								match=0;
								break;
							}
						if((fc_dst_len-tmp_fc_dst_len*32)>0)
						{
							tmp_ip1=in6_tmp.v6_addr32[j]>>(32-(fc_dst_len-tmp_fc_dst_len*32));
							tmp_ip2=tmp_dst.v6_addr32[j]>>(32-(fc_dst_len-tmp_fc_dst_len*32));
							if(tmp_ip1!=tmp_ip2)
								match=0;
								
						}
						if(match)
							rtl8198c_addIpv6Route(in6_tmp, 128, tmp_gw, dev_t);
...
{
	int rc, match=0,j,fc_dst_len, tmp_fc_dst_len,addr_type;
	inv6_addr_t ipDst, tmp_ip,in6_tmp;
	uint32 tb6_id = 0,tmp_ip1,tmp_ip2;
	struct inet6_dev *idev;
	struct inet6_ifaddr *ifa;
	char *dev_t;
	...
	if (tb6_id == RT6_TABLE_MAIN) {
		...
		if (((ipDst.v6_addr32[0]==0)&&(ipDst.v6_addr32[1]==0)&&(ipDst.v6_addr32[2]==0)&&(ipDst.v6_addr32[3]==0)) ||
			...
			{	
				...
				if(idev)
				{
					if((tmp_ip.v6_addr32[0]&0xffff0000)!=0xfe800000)
					{
					list_for_each_entry(ifa, &idev->addr_list, if_list) {
						if(ifa->prefix_len!=fc_dst_len)
							continue;
						...
						match=1;
						for(j=0;j<tmp_fc_dst_len;j++)
							if(in6_tmp.v6_addr32[j]!=tmp_ip.v6_addr32[j])
							{
								match=0;
								break;
							}
						...
						{
							tmp_ip1=in6_tmp.v6_addr32[j]>>(32-(fc_dst_len-tmp_fc_dst_len*32));
							tmp_ip2=tmp_ip.v6_addr32[j]>>(32-(fc_dst_len-tmp_fc_dst_len*32));
							if(tmp_ip1!=tmp_ip2)
								match=0;
								
						}
						if(match)
							rtl8198c_delIpv6Route(in6_tmp, 128);
