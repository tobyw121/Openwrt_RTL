...
static struct tp_multicast_group_entry * tp_multicast_group_hashtable[DEFAULT_HASH_TABLE_SIZE];
static struct tp_multicast_group_entry * tp_multicast_group_pool = NULL;


#define TP_MULTICAST_HASH_MASK 63 /* DEFAULT_HASH_TABLE_SIZE - 1  */
...
extern int check_last_member;


struct rtl865x_ReservedMCastRecord reservedMCastRecord[MAX_RESERVED_MULTICAST_NUM];
struct rtl865x_blackListMCastRecord blackListMacRecord[MAX_BLACKLIST_MAC_NUM];
...
static struct tp_multicast_group_entry* tp_init_multicast_group_pool(uint32 poolSize);
static struct tp_multicast_group_entry* tp_alloc_multicast_group_entry(void);
static void tp_free_multicast_group_entry(struct tp_multicast_group_entry* group_entry) ;
#ifdef CONFIG_RECORD_MCAST_FLOW
...
static void  rtl_parseMacFrame(uint32 moduleIndex, uint8* macFrame, uint32 verifyCheckSum, struct rtl_macFrameInfo* macInfo, uint32 macFrameLen);
...
static	uint32 rtl_processIgmpv3Mldv2Reports(uint32 moduleIndex, uint32 ipVersion, uint32 portNum,uint32 *clientAddr,uint8 *clientmacAddr, uint8 *pktBuf, uint32 pktLen);
...
static  uint32 rtl_processIgmpv3Mldv2Reports(uint32 moduleIndex, uint32 ipVersion, uint32 portNum,uint32 *clientAddr, uint8 *pktBuf, uint32 pktLen);
#endif
...
int32 rtl_initMulticastSnooping(struct rtl_mCastSnoopingGlobalConfig mCastSnoopingGlobalConfig)
{
	...
	/*initialize group entry pool*/
      ...
      printk("init tp multicast hash table\n");
      for (i = 0 ; i < DEFAULT_HASH_TABLE_SIZE ; i++)
      {	
      	tp_multicast_group_hashtable[i]=NULL;
      }
	
      tp_multicast_group_pool=tp_init_multicast_group_pool(DEFAULT_MAX_TP_GROUP_COUNT); 
      if(tp_multicast_group_pool==NULL)
      {
	      return FAILED;
      }
...
//used by igmp_delete, need to be checked
...
static struct tp_multicast_group_entry* tp_init_multicast_group_pool(uint32 poolSize)
{
	...
	uint32 idx=0;
	struct tp_multicast_group_entry *poolHead=NULL;
	struct tp_multicast_group_entry *entryPtr=NULL;
	...
	/* Allocate memory */
	poolHead = (struct tp_multicast_group_entry *)rtl_glueMalloc(sizeof(struct tp_multicast_group_entry) * poolSize);
	tp_group_memory=(void *)poolHead;
	
	if (poolHead != NULL)
	{
		memset(poolHead, 0,  (poolSize  * sizeof(struct tp_multicast_group_entry)));
		entryPtr = poolHead;
...
// allocate a multicast group entry  from the multicast flow pool
static  struct tp_multicast_group_entry* tp_alloc_multicast_group_entry(void)
{
	struct tp_multicast_group_entry *ret = NULL;

	rtl_glueMutexLock();	
		if (tp_multicast_group_pool!=NULL)
		{
			ret = tp_multicast_group_pool;
			if(tp_multicast_group_pool->next!=NULL)
			{
				tp_multicast_group_pool->next->previous=NULL;
			}
			tp_multicast_group_pool = tp_multicast_group_pool->next;
			memset(ret, 0, sizeof(struct tp_multicast_group_entry));
		}
...
// free a multicast flow entry and link it back to the multicast group entry pool, default is link to the pool head
static  void tp_free_multicast_group_entry(struct tp_multicast_group_entry* group_entry) 
{
	if (NULL==group_entry)
	{
	...
	rtl_glueMutexLock();	
	group_entry->next = tp_multicast_group_pool;
	if(tp_multicast_group_pool!=NULL)
	{
		tp_multicast_group_pool->previous=group_entry;
	}
	tp_multicast_group_pool=group_entry;	
	rtl_glueMutexUnlock();	
}
...
#if defined(CONFIG_HW_MCAST_DEBUG)
	printk("HW_MCAST:%s:%d Unlink group entry. idx:%d groupAddress:%d.%d.%d.%d\n", __func__, __LINE__, 
	groupEntry->idx,
	groupEntry->groupAddr[0] >> 24,
	(groupEntry->groupAddr[0] & 0x00ff0000) >> 16,
	(groupEntry->groupAddr[0] & 0x0000ff00) >> 8,
...
#if defined(CONFIG_HW_MCAST_DEBUG)
	printk("HW_MCAST:%s:%d Link client entry. group_idx:%d client_idx:%d clientAddr:%d.%d.%d.%d\n",
	__func__, __LINE__, groupEntry->idx,
	clientEntry->idx, 
...
#if defined(CONFIG_HW_MCAST_DEBUG)
	printk("HW_MCAST:%s:%d UnLink client entry. group_idx:%d client_idx:%d clientAddr:%d.%d.%d.%d\n",
		__func__, __LINE__, groupEntry->idx,
		clientEntry->idx, 
		clientEntry->clientAddr[0] >> 24,
		(clientEntry->clientAddr[0] & 0x00ff0000) >> 16,
...
static inline uint32 tp_igmpHashAlgorithm(uint32 ipVersion,uint32 *groupAddr)
{
	uint32 hashIndex=0;
	...
	{
		/*to do:change hash algorithm*/
		hashIndex=TP_MULTICAST_HASH_MASK&groupAddr[0];
	}
#if 0//def CONFIG_RTL_MLD_SNOOPING	
	else
	{
		hashIndex=rtl_hashMask&groupAddr[3];
	}
...
static void tp_link_multicast_group_entry(struct tp_multicast_group_entry* multicast_group_entry ,  struct tp_multicast_group_entry ** hashTable)
{
	uint32 hashIndex=0;
	rtl_glueMutexLock();//Lock resource
	if(NULL==multicast_group_entry)
	{
		rtl_glueMutexUnlock();
		return;
	}
	...
	hashIndex=tp_igmpHashAlgorithm(multicast_group_entry->ipVersion, multicast_group_entry->groupAddr);

	if(hashTable[hashIndex]!=NULL)
	{
		hashTable[hashIndex]->previous=multicast_group_entry;
	}
	multicast_group_entry->next = hashTable[hashIndex];
	hashTable[hashIndex]=multicast_group_entry;
	hashTable[hashIndex]->previous=NULL;
...
/* unlink a multicast group entry*/
static void tp_unlink_multicast_group_entry(struct tp_multicast_group_entry* multicast_group_entry,  struct tp_multicast_group_entry ** hashTable)
{	 
	uint32 hashIndex=0;
	if(NULL==multicast_group_entry)
	{
	...
	rtl_glueMutexLock();  /* lock resource*/	
	/* unlink entry node*/
	if(multicast_group_entry==hashTable[hashIndex]) /*unlink flow list head*/
	{
		hashTable[hashIndex]=multicast_group_entry->next;
		if(hashTable[hashIndex]!=NULL)
		{
			hashTable[hashIndex]->previous=NULL;
		}
	...
	{
		if(multicast_group_entry->previous!=NULL)
		{
			multicast_group_entry->previous->next=multicast_group_entry->next;
		}
		 
		if(multicast_group_entry->next!=NULL)
		{
			multicast_group_entry->next->previous=multicast_group_entry->previous;
		}
	}
	
	multicast_group_entry->previous=NULL;
	multicast_group_entry->next=NULL;
...
/* clear the content of multicast group  entry */
static void tp_clear_multicast_group_entry(struct tp_multicast_group_entry* multicast_group_entry)
{
	rtl_glueMutexLock();
	if (NULL!=multicast_group_entry)
	{
		memset(multicast_group_entry, 0, sizeof(struct tp_multicast_group_entry));
	}
	rtl_glueMutexUnlock();
}


static void tp_delete_multicast_group_entry( struct tp_multicast_group_entry* multicast_group_entry, struct tp_multicast_group_entry ** hashTable)
{	
	if(multicast_group_entry!=NULL)
	{
		tp_unlink_multicast_group_entry(multicast_group_entry, hashTable);
		tp_clear_multicast_group_entry(multicast_group_entry);
		tp_free_multicast_group_entry(multicast_group_entry);
	}

	return;	
}

static void tp_do_multicast_group_recycle(uint32 ipVersion)
{
	uint32 i;
	uint32 freeCnt=0;
	struct tp_multicast_group_entry* multicast_group_entry = NULL;
	struct tp_multicast_group_entry* next_multicast_group_entry = NULL;
	struct tp_multicast_group_entry* oldest_multicast_group_entry = NULL;

	for (i = 0 ; i < DEFAULT_HASH_TABLE_SIZE ; i++)
	{
		multicast_group_entry=tp_multicast_group_hashtable[i];
		
		if(oldest_multicast_group_entry==NULL)
		{
			oldest_multicast_group_entry=multicast_group_entry;
		}
		
		while (multicast_group_entry!=NULL)
		{	
			next_multicast_group_entry=multicast_group_entry->next;
			/*keep the most recently used entry*/
			if((multicast_group_entry->refreshTime+DEFAULT_TP_MULTICAST_GROUP_EXPIRE_TIME) < rtl_sysUpSeconds)
			{
				tp_delete_multicast_group_entry(multicast_group_entry, tp_multicast_group_hashtable);
				freeCnt++;
			}
			multicast_group_entry=next_multicast_group_entry;
		}
	}

	//printk_ratelimited("free multicast group count %d\n", freeCnt);
	if(freeCnt>0)
	{
		return;
	}
	...
	for (i = 0 ; i < DEFAULT_HASH_TABLE_SIZE ; i++)
	{
		...
		{	
			next_multicast_group_entry=multicast_group_entry->next;
			if(multicast_group_entry->refreshTime < oldest_multicast_group_entry->refreshTime)
			{
				oldest_multicast_group_entry=multicast_group_entry;
			}
	...
	if(oldest_multicast_group_entry!=NULL)
	{
		tp_delete_multicast_group_entry(oldest_multicast_group_entry, tp_multicast_group_hashtable);
	}
					
	return ;
}

uint32 tp_need_block_multicast_group(uint32 ipVersion, uint32 *groupAddr, uint32 svid)
{
	struct tp_multicast_group_entry* multicast_group_ptr = NULL;
	uint32 hashIndex=0;
	...
	hashIndex = tp_igmpHashAlgorithm(ipVersion, groupAddr);

	multicast_group_ptr = tp_multicast_group_hashtable[hashIndex];

	while (multicast_group_ptr != NULL)
	{	
#if 0 //def CONFIG_RTL_MLD_SNOOPING
		if(multicast_group_ptr->ipVersion != ipVersion)
		{
			goto nextFlow;
		}
...
#endif
		{
			if(groupAddr[0] == multicast_group_ptr->groupAddr[0])
			{
				if(multicast_group_ptr->need_block == 0 && multicast_group_ptr->svid != svid)
				{
					multicast_group_ptr->need_block = 1;
				}	

				//printk_ratelimited("found 0x%x == 0x%x, psvid=%d, need_block=%d\n", groupAddr[0], multicast_group_ptr->groupAddr[0],
				//	svid, multicast_group_ptr->need_block);
				
				/* multicast_group_ptr->refreshTime=rtl_sysUpSeconds; */
				return multicast_group_ptr->need_block;
			}
		}
#if 0//def CONFIG_RTL_MLD_SNOOPING
		else
...
nextFlow:
#endif	
		multicast_group_ptr = multicast_group_ptr->next;

	}
	
	if(svid && multicast_group_ptr == NULL)
	{
		multicast_group_ptr=tp_alloc_multicast_group_entry();
		if(multicast_group_ptr==NULL)
		{
			tp_do_multicast_group_recycle( ipVersion);
			
			multicast_group_ptr=tp_alloc_multicast_group_entry();
			if(multicast_group_ptr==NULL)
			{
				printk_ratelimited(KERN_ERR"run out of multicast group entry!\n");
				return 0;
			}
		}
		...
		if(ipVersion==IP_VERSION4)
		{
			multicast_group_ptr->groupAddr[0]=groupAddr[0];
		...
		multicast_group_ptr->ipVersion=ipVersion;

		multicast_group_ptr->svid = svid;
		
		multicast_group_ptr->refreshTime=rtl_sysUpSeconds;

		tp_link_multicast_group_entry(multicast_group_ptr, tp_multicast_group_hashtable);

		//printk_ratelimited("enabled new group 0x%x, svid=%d, need_block=%d\n", multicast_group_ptr->groupAddr[0],
		//			svid, multicast_group_ptr->need_block);
	}
...
//                   +Type(IPv4:0x0800, IPV6:0x86DD, PPPOE:0x8864, 2 bytes )+Data(46~1500 bytes)+CRC(4 bytes)
	...
	#endif
	{
		...
		if (ntohs(((struct ipv4Pkt *)(ptr))->length) > (macFrameLen - (ptr-macFrame)))
		{
			printk_ratelimited("%s:%d ipBuf length error.\n", __func__, __LINE__);
			return;
		}
	...
	else
	{
	...
	#endif
		{
			...
			if ((ntohs(((struct ipv6Pkt *)(ptr))->payloadLenth) + IPV6_HEADER_LENGTH) > (macFrameLen - (ptr-macFrame)))
			{
				printk_ratelimited("%s:%d ipBuf length error.\n", __func__, __LINE__);
				return;
			}
	...
	else if (ipVersion==IP_VERSION6)
	{
		...
		*/
		if(
			...
			&&((groupAddress[1] & 0xFFFFFFFF)==0x00000000)
			&&((groupAddress[2] & 0xFFFFFFFF)==0x00000000)
			&&((groupAddress[3] & 0xFFFFFFFF)==0x00010002))
		)
		{
			//printk("%s %d reverve mulitcast:%x:%x:%x:%x ipVersion=%u\n",__func__, __LINE__, groupAddress[0],groupAddress[1],groupAddress[2],groupAddress[3], ipVersion);
			ret=SUCCESS;
		}
	}
#endif
...
#endif
{
	...
	for(i=0; i<numOfRecords; i++)
	{
		if(ipVersion==IP_VERSION4)
		{		
			if ((uint8 *)(groupRecords + 8) > (uint8 *)(pktBuf + pktLen) 
				|| (uint8 *)(groupRecords+8+ntohs(((struct groupRecord *)groupRecords)->numOfSrc)*4+(((struct groupRecord *)(groupRecords))->auxLen)*4) > (uint8 *)(pktBuf + pktLen))
			{
				printk_ratelimited("%s:%d groupRecords error.\n", __func__, __LINE__);
				break;
			}
		...
		{
			if ((uint8 *)(groupRecords + 20) > (uint8 *)(pktBuf + pktLen) 
				|| (uint8 *)(groupRecords+20+ntohs(((struct mCastAddrRecord *)groupRecords)->numOfSrc)*16+(((struct mCastAddrRecord *)(groupRecords))->auxLen)*4) > (uint8 *)(pktBuf + pktLen))
			{
				printk_ratelimited("%s:%d groupRecords error.\n", __func__, __LINE__);
				break;
			}
		...
		if(ipVersion==IP_VERSION4)
		{
			RTL_IGMP_PRINT("(2) IGMPv3 Action %d,Group 0x%x,Client 0x%x,Return %d", recordType, ((struct groupRecord *)groupRecords)->groupAddr, 
					*clientAddr, returnVal);
		...
		{
			RTL_IGMP_PRINT("(2) MLDv2 Action %d,Group 0x%x,Client 0x%x,Return %d", recordType, ((struct mCastAddrRecord *)groupRecords)->mCastAddr[3], 
					*clientAddr, returnVal);
...
{	
	...
	switch(pktBuf[0])
	{
		...
		case IGMPV3_REPORT:
		#if defined (CONFIG_RTL_IGMPSNOOPING_MAC_BASED) || defined (M2U_DELETE_CHECK)
			 fwdPortMask=rtl_processIgmpv3Mldv2Reports(moduleIndex, ipVersion, portNum,clientAddr,clientMacAddr,pktBuf, pktLen);
		#else
			 fwdPortMask=rtl_processIgmpv3Mldv2Reports(moduleIndex, ipVersion, portNum, clientAddr,pktBuf, pktLen);
		#endif
		break;
	...
	RTL_IGMP_PRINT("(3) IGMP Type 0x%hx,Client 0x%x,PortNum %d,fwdPortMask 0x%x", pktBuf[0], *clientAddr, portNum, fwdPortMask);
...
/**************************************************************/
	//last current time :(rtl_startTime+rtl_sysUpSeconds)
	//estimated current time:(rtl_startTime+rtl_sysUpSeconds+1) 
	//true current time :currentSystemTime

	//adjust rtl_startTime in case of NTP update system time
	if(currentSystemTime> ((rtl_startTime+rtl_sysUpSeconds+1)+MAX_TIME_GAP))
	{
		rtl_startTime = currentSystemTime-(rtl_sysUpSeconds+1);
	}
	
	if((rtl_startTime+rtl_sysUpSeconds+1)>(currentSystemTime+MAX_TIME_GAP))
	{
		if(currentSystemTime>=(rtl_sysUpSeconds+1))
		{
			rtl_startTime = currentSystemTime-(rtl_sysUpSeconds+1);
		}
		else
		{
			rtl_startTime=maxTime-(rtl_sysUpSeconds-currentSystemTime);
		}
	}
/**************************************************************/
...
int32 rtl_igmpMldProcess(uint32 moduleIndex, uint8 * macFrame,  uint32 portNum, uint32 *fwdPortMask, uint32 macFrameLen)
{
	...
	*fwdPortMask=(~(1<<portNum)) & 0xFFFFFFFF;
	...
	RTL_IGMP_PRINT("(1) IGMP PortNum %d", portNum);
	...
	rtl_parseMacFrame(moduleIndex, macFrame, FALSE, &macFrameInfo, macFrameLen);
	if(  rtl_mCastModuleArray[moduleIndex].enableSnooping==TRUE)
	{
		...
		switch(macFrameInfo.l3Protocol)
		{
			...
			case IGMP_PROTOCOL:
				if(rtl_check_blackList_mac(macFrameInfo.srcMacAddr))
					return FAILED;
...
int32 rtl_getMulticastDataFwdInfo(uint32 moduleIndex, struct rtl_multicastDataInfo *multicastDataInfo, struct rtl_multicastFwdInfo *multicastFwdInfo)
{
	...
	if ((IS_RESERVE_MULTICAST_MDNS_ADDR_V4(multicastDataInfo->groupAddr[0])&&multicastDataInfo->ipVersion==IP_VERSION4)
		||((IS_RESERVE_MULTICAST_MDNS_ADDR_V6(multicastDataInfo->groupAddr) || IS_RESERVE_MULTICAST_DHCPV6_ADDR_V6(multicastDataInfo->groupAddr))&&multicastDataInfo->ipVersion==IP_VERSION6))
	{
...
int rtl_check_blackList_mac(uint8* mac)
{
	int ret =0;
	int i=0;
	...
	for(i=0; i<MAX_RESERVED_MULTICAST_NUM; i++)
	{
		if((blackListMacRecord[i].valid==1)
		&&(!memcmp(blackListMacRecord[i].mac,mac,sizeof(blackListMacRecord[i].mac)))
		)
		{	
			ret =1;			
			goto OUT;
...
int rtl_set_blackList_mac(uint8* mac, int flag)
{
	int ret =FAILED;
	int i=0;
	...
	if((flag!=CLEAR_BLACKLIST_MAC_FLAG)&&(flag!=ADD_BLACKLIST_MAC_FLAG)&&(flag!=DEL_BLACKLIST_MAC_FLAG) )
		return ret;
	
	if(CLEAR_BLACKLIST_MAC_FLAG == flag)
	{
		memset(blackListMacRecord, 0, sizeof(blackListMacRecord));
		return SUCCESS;
	}
		
	
	for(i=0; i<MAX_BLACKLIST_MAC_NUM; i++)
	{
		if((blackListMacRecord[i].valid==1)&&(flag==ADD_BLACKLIST_MAC_FLAG)
		&&(!memcmp(blackListMacRecord[i].mac,mac,sizeof(blackListMacRecord[i].mac)))
		)
		{	
			ret =SUCCESS;
		...
		if((blackListMacRecord[i].valid==0)&&(flag==ADD_BLACKLIST_MAC_FLAG))
		{
			blackListMacRecord[i].valid =1;
			memcpy(blackListMacRecord[i].mac,mac,6);
			ret =SUCCESS;
			goto OUT;
		}
		else if((blackListMacRecord[i].valid==1)
			&&(flag==DEL_BLACKLIST_MAC_FLAG)
			&&(!memcmp(blackListMacRecord[i].mac,mac,6))
			)
		{		
			blackListMacRecord[i].valid =0;			
			ret =SUCCESS;			
			goto OUT;
		}
	}
