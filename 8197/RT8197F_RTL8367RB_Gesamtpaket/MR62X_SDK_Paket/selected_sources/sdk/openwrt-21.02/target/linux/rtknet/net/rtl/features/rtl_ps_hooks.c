...
EXPORT_SYMBOL_GPL(rtl_nf_nat_packet_hooks);
...
EXPORT_SYMBOL_GPL(rtl_nat_init_hooks);
...
EXPORT_SYMBOL_GPL(rtl_nat_cleanup_hooks);
...
int32 rtl_fastpath_vlan_put_tag(struct sk_buff *skb, char *saved_vlan_tag)
{
	struct ethhdr *ehr = rtl_eth_hdr(skb);
	struct iphdr *iph = (struct iphdr *)(ehr + 1);
	...
	if (hdr->source==htons(1701))
	{
		...
		if(data[28] & 0x02) // offset option is on
			rightShift += 2;
		if (((struct iphdr *)(&data[38 + rightShift]))->protocol == IPPROTO_ICMP)
		{
			iph = rtl_ip_hdr(skb);
			if (iph->protocol == IPPROTO_ICMP) {
				ihr = (struct icmphdr *)((u_int32_t *)iph + iph->ihl);
				printk("[%s-%u] %s %u src %02X-%02X-%02X-%02X-%02X-%02X dst %02X-%02X-%02X-%02X-%02X-%02X\n",
				__func__, __LINE__, ((struct sk_buff *)skb)->dev ? ((struct sk_buff *)skb)->dev->name : "null", ihr->type,
				ehr->h_source[0], ehr->h_source[1], ehr->h_source[2], ehr->h_source[3],
				ehr->h_source[4], ehr->h_source[5], ehr->h_dest[0], ehr->h_dest[1],
				ehr->h_dest[2], ehr->h_dest[3], ehr->h_dest[4], ehr->h_dest[5]);
				printk("\tpkt_type %u data %p ihr %p ehr %p\n", skb->pkt_type, data, ihr, ehr);
			}
		}
	}
...
int32 rtl_netif_receive_skb_hooks(struct sk_buff **pskb)
{
	...
	#if (LINUX_VERSION_CODE>=KERNEL_VERSION(3,10,0)) && defined(CONFIG_RTL_VLAN_8021Q)
	char saved_vlan_tag[VLAN_HLEN] = {0};
	struct net_device *orig_dev = NULL;
	int16 vlan_id = 0;
	
	if (((*pskb)->pptp_flag & 0xffu) || ((*pskb)->l2tp_flag & 0xffu))
		return RTL_PS_HOOKS_CONTINUE;
	...
	#if defined(CONFIG_RTL_IPTABLES_FAST_PATH)
	if ((*pskb)->dev != NULL && FastPath_Enter(pskb)== NET_RX_DROP) {
		ret = RTL_PS_HOOKS_RETURN;
	} else
	#endif
	{
...
int32  rtl_neigh_periodic_timer_hooks(struct neighbour *n,  unsigned int  refresh)
{
	...
	{
		if (rtl8198c_ipv6ArpSync(*((inv6_addr_t *)n->primary_key), refresh,n&&n->dev?n->dev->name:NULL) > 0) {
			n->used = jiffies;
			n->dead=0;
...
int32 rtl_neigh_timer_handler_pre_update_hooks(struct neighbour *neigh, unsigned state)
{
	...
	if (state & NUD_REACHABLE) {
		...
		#if defined(CONFIG_RTL_HARDWARE_IPV6_SUPPORT)&& !defined(CONFIG_RTL_IPV6READYLOGO)
		if (neigh->tbl && (neigh->tbl->family==AF_INET6) && (neigh->tbl->key_len==sizeof(inv6_addr_t))) 
		{
			tval = rtl8198c_ipv6ArpSync(*((inv6_addr_t *)neigh->primary_key), 0,neigh&&neigh->dev?neigh->dev->name:NULL);
			if (tval > 0)
...
EXPORT_SYMBOL_GPL(rtl_masq_device_event_hooks);
...
EXPORT_SYMBOL_GPL(rtl_masq_inet_event_hooks);
...
int rtl_ct_seq_show_hooks(struct seq_file *s, struct nf_conn *ct)
{
	...
	if(seq_printf(s,"[%s] ",nat->hw_acc?state[0]:state[1]) != 0)
		return RTL_PS_HOOKS_BREAK;
	#else
		seq_printf(s,"[%s] ",nat->hw_acc?state[0]:state[1]);
	#endif
	#endif
	return RTL_PS_HOOKS_CONTINUE;
}
...
void rtl_update_ps_neigh_confirm(struct neighbour *neigh)
{
#if defined(RTL_UPDATE_PS_NEIGH_CONFIRM)
	unsigned long now = jiffies;

	/* avoid dirtying neighbour */
	/*jwj: As rtl_update_ps_neigh_confirm will be called in ip_finish_output2,
			so we do not need to add more lock protect.*/
	if (neigh->confirmed != now)
		neigh->confirmed = now;
#endif
...
int32 rtl_fdb_create_hooks(struct net_bridge_fdb_entry *fdb,const unsigned char *addr)
{
	...
	if(!(fdb) || !(fdb->dst) || !(fdb->dst->dev) || !(addr))
	{
		return RTL_PS_HOOKS_CONTINUE;		
	}

	 if (extPortEnabled&&(!memcmp(fdb->dst->dev->name, WLAN_BASE_NAME, strlen(WLAN_BASE_NAME)))&&((addr[0]&0x1)==0)) 
 	{
