...
int mirrorPort_enable = 0;
uint32 mirrorPort_mask = 0;
uint32 mirrorPort_txmask = 0;
...
extern int32 rtl_8367r_vlan_get(unsigned int i, unsigned int *mbrmsk, unsigned int *untagmsk, unsigned int *fid);
extern int32 rtk_vlan_set(uint32 vid, uint32 mbrmsk, uint32 untagmsk, uint32 fid);
extern int rtl_vlan_RTL8367R_set(unsigned short vid, unsigned int tagmask, unsigned int mask, unsigned int fid);
extern int32 rtk_vlan_portPvid_set(uint32 port, uint32 pvid, uint32 priority);
...
#endif
extern int rtk_port_phyReg_set(uint32 phyNo, uint32 phyAddr, uint32 phyData );
...
extern int rtl8367d_setAsicReg(uint32 reg, uint32 value);
extern int rtl8367c_getAsicReg(uint32 reg, uint32 *value);
extern int rtl8367d_getAsicReg(uint32 reg, uint32 *value);
#define rtl83xx_setAsicReg(reg, value)	((chip_type==4) ? rtl8367d_setAsicReg(reg, value) : rtl8367c_setAsicReg(reg, value))
#define rtl83xx_getAsicReg(reg, pValue)	((chip_type==4) ? rtl8367d_getAsicReg(reg, pValue) : rtl8367c_getAsicReg(reg, pValue))
extern int rtk_port_phyOCPReg_get(uint32 phyNo, uint32 ocpAddr, uint32 *pRegData);
extern int rtk_port_phyOCPReg_set(uint32 phyNo, uint32 ocpAddr, uint32 ocpData);
#define rtl83xx_getAsicPHYOCPReg(phyId, regId, regData) rtk_port_phyOCPReg_get(phyId, regId, regData)
#define rtl83xx_setAsicPHYOCPReg(phyId, regId, regData) rtk_port_phyOCPReg_set(phyId, regId, regData)
#else
extern int rtl8367c_setAsicReg(uint32 reg, uint32 value);
extern int rtl8367c_getAsicReg(uint32 reg, uint32 *value);
...
extern int rtl_83XX_vlan_get(unsigned int i, unsigned int *mbrmsk, unsigned int *untagmsk, unsigned int *fid);
extern int rtl_vlan_RTL83XX_set(unsigned short vid, unsigned int tagmask, unsigned int mask, unsigned int fid);
extern int rtl_port_isolation_RTL83XX_set(unsigned short port,unsigned int mask);
extern int rtk_vlan_tagMode_get(unsigned int port, unsigned int *pTag_mode);
extern int rtk_vlan_tagMode_set(unsigned int port, unsigned int tag_mode);
#endif
...
#define RTL8367R_MAX_VLANINDEX	15
#endif
...
extern unsigned int tx_ringFull_count[];
extern unsigned int tx_drop_count[];
unsigned int statistic_total;
unsigned int statistic_ps;
unsigned int statistic_ipv6_fp;

unsigned int statistic_ipv6_pppoe_xmit;
unsigned int statistic_ipv6_hard_xmit;
unsigned int statistic_ipv6_neigh_xmit;

#if !defined(CONFIG_OPENWRT_SDK)
unsigned int statistic_fp;
unsigned int statistic_normal;
unsigned int statistic_fp_failed;
unsigned int statistic_match_failed;
#endif
...
extern void rtl_dump_eth_skb_buf(int action);
#endif
...
extern int32 rtl_mirror_portIso_set(uint32 isolation);
extern int32 rtl_mirror_portIso_get(uint32 *isolation);
extern int rtl_port_isolation_leak_set(bool enable);
#endif
...
static struct proc_dir_entry *l3v6_entry, *swl3v6_entry,	*arp6_entry, *nexthop6_entry,	
...
static struct proc_dir_enty *kernel_group_entry;
#endif
			...
			else if (	(!memcmp(cmd_addr, "mod", 3)) )
			{
				...
				update_vlanconfig(vid & 4095,portMask & 0x1FF, untagPortMask & 0x1FF);
						...
						else if ((!memcmp(cmd_addr, "83xx", 4)))
						{
							char *sub_cmd = NULL;
							int ret = FAILED, i = 0;
							uint32 vid =0, mbrmsk = 0, untagmsk = 0, tagmsk = 0, fid = 0, port = 0, tagmode = 0;
							...
							if ((!memcmp(sub_cmd, "dump", 4))){
								...
								if ((!memcmp(tokptr, "all", 3))){
									...
									//dump all vlan info  del by ZYL
									/*
									for (i = 0; i <= RTL8367R_MAX_VLANINDEX; i++) 
									{	
										ret = rtl_8367r_vlan_get(i, &mbrmsk, &untagmsk, &fid);				
										if (ret != 0)
											continue;	
										rtlglue_printf("vid %d Mbrmsk 0x%x Untagmsk 0x%x fid %u\n", vid, mbrmsk, untagmsk, fid);
									}*/
									//dump all vlan info   add by ZYL  only dump special vid
									for (i = 0; i <= 500; i++) 
									{	
										if(i==1 || i==8 || i==9 || i==10 || i%100==0)
										{
											...
											ret = rtl_8367r_vlan_get(i, &mbrmsk, &untagmsk, &fid);	
							#elif defined(CONFIG_RTL_83XX_SUPPORT)
											ret = rtl_83XX_vlan_get(i, &mbrmsk, &untagmsk, &fid);	
							#endif
											if (ret != 0)
												continue;	
									...
									vid = simple_strtol(tokptr, NULL, 0);
						#if defined(CONFIG_RTL_8367R_SUPPORT)
									ret = rtl_8367r_vlan_get(vid, &mbrmsk, &untagmsk, &fid);
						#elif defined(CONFIG_RTL_83XX_SUPPORT)
									ret = rtl_83XX_vlan_get(vid, &mbrmsk, &untagmsk, &fid);
						#endif
									if (ret == 0){
										rtlglue_printf("vid %d Mbrmsk 0x%x Untagmsk 0x%x fid %u\n", vid, mbrmsk, untagmsk, fid);
									}
					...
					#if defined(CONFIG_RTL_8367R_SUPPORT)
								ret = rtl_vlan_RTL8367R_set(vid, tagmsk, mbrmsk, fid);
					#elif defined(CONFIG_RTL_83XX_SUPPORT)
								ret = rtl_vlan_RTL83XX_set(vid, tagmsk, mbrmsk,fid);
					#endif
								if (ret == 0){
								...
								tagmsk = untagmsk = mbrmsk = fid = 0;
								
								if (!memcmp(tokptr, "all", 3)){ 
									//for (i = 0; i <= RTL8367R_MAX_VLANINDEX; i++)  //del by ZYL
									for (i = 0; i <= 4095; i++)   //add by ZYL	 del all vlan(from 0~4095)
									{
						#if defined(CONFIG_RTL_8367R_SUPPORT)
										ret = rtl_vlan_RTL8367R_set(i, 0, 0, 0);
						#elif defined(CONFIG_RTL_83XX_SUPPORT)
										ret = rtl_vlan_RTL83XX_set(i, 0, 0, 0);
						#endif
									}
								}
								else{
								...
								ret = rtl_port_isolation_RTL83XX_set(port, mbrmsk);
								if (ret == 0){
									rtlglue_printf("%s isolate %d success!\n", sub_cmd, port);
								}
								else{
									rtlglue_printf("%s isolate %d failed!\n", sub_cmd, port);
								}
							}
							else if ((!memcmp(sub_cmd, "mode", 4))){
								/* set/get port tagmode */
								...
								if (tokptr!=NULL)
								{
									tagmode = simple_strtol(tokptr, NULL, 16);
									ret = rtk_vlan_tagMode_set(port, tagmode);
								}
								else
								{
									ret = rtk_vlan_tagMode_get(port, &tagmode);
								}
								
								if (ret == 0){
									rtlglue_printf("port=%d tagmode=0x%x\n", port, tagmode);
								}
								else{
									rtlglue_printf("set/get port %d tagmode 0x%x failed!\n", port, tagmode);
								}
							}
...
static int32 netif_read(struct seq_file *s, void *v)
{
	...
	//printk("---------%s(%d), SWTCR0 = 0x%x \n",__FUNCTION__,__LINE__,(READ_MEM32(SWTCR0) & (0x3 << 16))>>16);
	if((READ_MEM32(SWTCR0) & (0x3 << 16))>>16 == 0x00)
		seq_printf(s, "%s\n", "ASIC Network Interface Table is vlan-base:");
	else if((READ_MEM32(SWTCR0) & (0x3 << 16))>>16 == 0x10)
        seq_printf(s, "%s\n", "ASIC Network Interface Table is mac-base:");
	else if((READ_MEM32(SWTCR0) & (0x3 << 16))>>16 == 0x01)
	{
        seq_printf(s, "%s\n", "ASIC Network Interface Table is port-base:");
        for( i = 0; i <  RTL8651_PORT_NUMBER + rtl8651_totalExtPortNum; i++ )
        {
            uint32 netifidx;
			netifidx = (READ_MEM32(PLITIMR) & (0x7 << (i * 3)))>>(i * 3);		
			seq_printf(s, "Port[%d] ----- Netif[%d] \n", i, netifidx);
		}
	}
...
{
	char tmpbuf[128];
	char *strptr, *cmd_addr, *ifName;
	unsigned int enableRouteV6, enableRoute;
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		if(cmd_addr == NULL)
		{
			printk("[%s-%u] cmd_addr null\n", __func__, __LINE__);
			goto error;
		}
		if (!strncmp(cmd_addr, "slave", strlen("slave")))
		{
			cmd_addr = strsep(&strptr, " ");
			if (cmd_addr == NULL)
			{
				printk("[%s-%u] slave ifName null\n", __func__, __LINE__);
				goto error;
			}
			ifName = cmd_addr;
			
			if (sscanf(strptr, "%d %u %u", &ifType, &enableRouteV6, &enableRoute) != 3)
			{
				printk("[%s-%u] sscanf cmd_addr null\n", __func__, __LINE__);
				goto error;
			}
			printk("ifName %s 0x%x,%u,%u\n", ifName, ifType, enableRouteV6, enableRoute);
			entry = _rtl865x_getNotEthWanNetif((1 << IF_PPPOE) | (1 << IF_PPTP) | (1 << IF_L2TP));
			if (!entry)
			{
				printk("[%s-%u] there is no ether wan netif\n", __func__, __LINE__);
				return len;
			}
			SMP_LOCK_ETH(flags);
			if (rtk_netifName_change(entry->name, ifName))
			{
				SMP_UNLOCK_ETH(flags);
				printk("[%s-%u] %s,%s change name failed\n", __func__, __LINE__, entry->name, ifName);
				return len;
			}
			if ((1 << entry->if_type) & ((1 << IF_PPPOE) | (1 << IF_L2TP) | (1 << IF_PPTP)))
			{
				memset(ppp_ext, '\0', sizeof(ppp_ext));
				strcpy(ppp_ext, ifName);
				ppp_ext_len = strlen(ppp_ext);
				printk("ppp_ext: %s %d\n", ppp_ext, ppp_ext_len);
			}
			entry->enableRouteV6 = enableRouteV6 ? enableRouteV6 : entry->enableRouteV6;
			entry->enableRoute = enableRoute ? enableRoute : entry->enableRoute;
			entry->if_type = ifType;
			memset(entry->name,0,sizeof(entry->name));
			memcpy(entry->name,ifName,strlen(ifName));
			SMP_UNLOCK_ETH(flags);
		}
		else
		{
...
error:
	printk("usage: \n");
	printk("\techo slave pppoev6 1 1 1 > /proc/rtl865x/sw_netif\n");
	return len;
}
#if 0
...
#if defined(CONFIG_RTL_HARDWARE_MULTICAST_API)
extern int rtl_kernelMCast_show(struct seq_file *s, void *v);
extern int rtl_kernelMCast_write(struct file *file, const char __user *buffer, size_t count, loff_t *data);
int rtl_kernel_MCast_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtl_kernelMCast_show, NULL));
}

struct file_operations kernel_group_single_seq_file_operations = {
        .open           = rtl_kernel_MCast_single_open,
        .read           = seq_read,
        .write			= rtl_kernelMCast_write,
        .llseek         = seq_lseek,
        .release        = single_release,
};
#endif
...
static int32 pvid_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < SWITCH_MAX_PORT_NUM; i++){
		...
		seq_printf(s,"p%d: %d,", i, (ret==0)? pvid : -1);
	}
	seq_printf(s,"\n");
	#endif
...
static int32 mirrorPort_read(struct seq_file *s, void *v)
{
	...
	#endif	
	seq_printf(s,"  mirrorPort_enable: %d\n", mirrorPort_enable);
	return 0;
}
#else
static int32 mirrorPort_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	...
	#endif
	len += sprintf(page+len,"  mirrorPort_enable: %d\n", mirrorPort_enable);
...
static int32 mirrorPort_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "mirror", 6))
		{
			...
			if(rx_mask|tx_mask)
				rtl_port_isolation_leak_set(1);
			else
				rtl_port_isolation_leak_set(0);
		...
		#endif
			mirrorPort_txmask = tx_mask;
			mirrorPort_mask = port_mask;
		}
		#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
...
static int32 l2_read(struct seq_file *s, void *v)
{
	rtl865x_tblAsicDrv_l2Param_t asic_l2;
 	uint32 row, col, port, m=0, count=0;
...
static int32 l2_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	ether_addr_t mac;
	uint32 mac_tmp[6];
	extern int32 rtl865x_addFilterDatabaseEntry( uint32 fid, ether_addr_t * mac, uint32 portmask, uint32 type);
	extern int32 rtl865x_delFilterDatabaseEntry(uint16 l2Type, uint16 fid, ether_addr_t * macAddr);
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		strptr = tmpbuf;
		cmd_addr = strsep(&strptr," ");
		if(cmd_addr && !memcmp(cmd_addr, "add", strlen("add")))
		{
		
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%d", &fid);
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "0x%x-%x-%x-%x-%x-%x", &mac_tmp[0], &mac_tmp[1],&mac_tmp[2], &mac_tmp[3],&mac_tmp[4],&mac_tmp[5]);
			for(i=0;i<6;i++)
			{
				mac.octet[i]=mac_tmp[i]&0xff;
			}
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%x", &portmask);
			if(strptr==NULL)
				return len;
			sscanf(strptr, "%d", &type);
			rtl865x_addFilterDatabaseEntry(fid,&mac,portmask,type);
		}
		else if(cmd_addr && !memcmp(cmd_addr, "del", strlen("del")))
		{
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%d", &type);
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%d", &fid);
			if(strptr==NULL)
				return len;
			sscanf(strptr, "0x%x-%x-%x-%x-%x-%x", &mac_tmp[0], &mac_tmp[1],&mac_tmp[2], &mac_tmp[3],&mac_tmp[4],&mac_tmp[5]);
			for(i=0;i<6;i++)
			{
				mac.octet[i]=mac_tmp[i]&0xff;
			}
			rtl865x_delFilterDatabaseEntry(type,fid,&mac);
		}
		else if(cmd_addr && !memcmp(cmd_addr, "clear", strlen("clear")))
		{
...
static int32 rtl865x_proc_hw_mcast_read(struct seq_file *s, void *v)
{
	...
	{			
			...
			seq_printf(s, "\t[%d]  (OK)dip(%d.%d.%d.%d) sip(%d.%d.%d.%d) mbr(%x)\n", entry,
				...
				asic.sip>>24, (asic.sip&0x00ff0000)>>16, (asic.sip&0x0000ff00)>>8, (asic.sip&0xff),
				asic.mbr);
			seq_printf(s,"\t       svid:%d, spa:%d, extIP:%d, age:%d, cpu:%d", asic.svid, asic.port, asic.extIdx,
				asic.age, asic.cpu);
			
#if defined(CONFIG_RTL_8197F)
			seq_printf(s,", destInterface:%d", asic.destInterface);
#endif
			seq_printf(s,"\n");
	...
	for(entry=0; entry<RTL8651_MULTICASTTBL_SIZE; entry++)
	{
			...
			{	
				...
				seq_printf(s,"\t          spa:%d, age:%d, cpu:%d, mbr(0x%x)", asic6.port, asic6.age, asic6.cpu, asic6.mbr);
#if defined(CONFIG_RTL_8197F)
				seq_printf(s,", destInterface:%d", asic6.destInterface);
#endif
				seq_printf(s,"\n");
...
static int32 rtl865x_proc_hw_mcast_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		entry.destInterface = simple_strtol(tokptr, NULL, 0);
#endif
...
static int32 arp_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	ether_addr_t mac;
	uint32 mac_tmp[6];
	extern int32 rtl865x_addArp(ipaddr_t ip, ether_addr_t * mac);
	extern int32 rtl865x_arp_reinit(void);
	extern int32 rtl865x_delArp(ipaddr_t ip);
	ipaddr_t ipAddr;
	int i=0;
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		if(cmd_addr && !memcmp(cmd_addr, "add", strlen("add")))
		{
			...
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2], &ip[3]);
		...
		else if(cmd_addr && !memcmp(cmd_addr, "del", strlen("del")))
		{
			...
			if(strptr==NULL)
				return len;
			sscanf(strptr, "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2], &ip[3]);
			ipAddr=(ip[0]<<24)|(ip[1]<<16)|(ip[2]<<8)|(ip[3]);
			rtl865x_delArp(ipAddr);
		}
		else if(cmd_addr && !memcmp(cmd_addr, "clear", strlen("clear")))
		{
...
static int32 l3_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	int8 * ifName;
	uint32  ip[4];	 
	ipaddr_t ipAddr,ipMask,nextHop,srcIp;
	extern int32 rtl865x_addRoute(ipaddr_t ipAddr, ipaddr_t ipMask, ipaddr_t nextHop,int8 * ifName,ipaddr_t srcIp);
	extern int32 rtl865x_delRoute(ipaddr_t ipAddr, ipaddr_t ipMask);
	extern int32 rtl865x_reinitRouteTable(void);
	extern int32 rtl865x_reinitNxtHopTable(void);
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		tmpbuf[len] = '\0';
		strptr = tmpbuf;
		...
		if(cmd_addr && !memcmp(cmd_addr, "add", strlen("add")))
		{
			...
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2], &ip[3]);
			ipMask=(ip[0]<<24)|(ip[1]<<16)|(ip[2]<<8)|(ip[3]);
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2], &ip[3]);
			nextHop=(ip[0]<<24)|(ip[1]<<16)|(ip[2]<<8)|(ip[3]);
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr==NULL)
				return len;
			ifName=(int8 *)cmd_addr;
			if(strptr==NULL)
				return len;
			sscanf(strptr, "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2], &ip[3]);
			srcIp=(ip[0]<<24)|(ip[1]<<16)|(ip[2]<<8)|(ip[3]);
			rtl865x_addRoute(ipAddr,ipMask,nextHop,ifName,srcIp);
		}
		else if(cmd_addr && !memcmp(cmd_addr, "del", strlen("del")))
		{
			cmd_addr = strsep(&strptr," ");
			...
			sscanf(strptr, "%d.%d.%d.%d", &ip[0], &ip[1], &ip[2], &ip[3]);
			ipMask=(ip[0]<<24)|(ip[1]<<16)|(ip[2]<<8)|(ip[3]);
			rtl865x_delRoute(ipAddr,ipMask);
		}
		else if(cmd_addr && !memcmp(cmd_addr, "clear", strlen("clear")))
		{
...
static int32 ip_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	uint32 ip_type;
	uint32  ipAddr[4];	  
	extern int32 rtl865x_reinitIpTable(void);
	extern int32 rtl865x_addIp(ipaddr_t intIp, ipaddr_t extIp, uint32 ip_type);
	extern int32 rtl865x_delIp(ipaddr_t extIp);
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		if(cmd_addr && !memcmp(cmd_addr, "add", strlen("add")))
		{
			...
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%d.%d.%d.%d", &ipAddr[0], &ipAddr[1], &ipAddr[2], &ipAddr[3]);
			intIp=(ipAddr[0]<<24)|(ipAddr[1]<<16)|(ipAddr[2]<<8)|(ipAddr[3]);
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "%d.%d.%d.%d", &ipAddr[0], &ipAddr[1], &ipAddr[2], &ipAddr[3]);
			extIp=(ipAddr[0]<<24)|(ipAddr[1]<<16)|(ipAddr[2]<<8)|(ipAddr[3]);
			if(strptr==NULL)
				return len;
			sscanf(strptr, "%d",&ip_type );
			rtl865x_addIp(intIp,extIp,ip_type);
		}
		else if(cmd_addr && !memcmp(cmd_addr, "del", strlen("del")))
		{
			if(strptr==NULL)
				return len;
			sscanf(strptr, "%d.%d.%d.%d", &ipAddr[0], &ipAddr[1], &ipAddr[2], &ipAddr[3]);
			extIp=(ipAddr[0]<<24)|(ipAddr[1]<<16)|(ipAddr[2]<<8)|(ipAddr[3]);
			rtl865x_delIp(extIp);
		}
		else if(cmd_addr && !memcmp(cmd_addr, "clear", strlen("clear")))
...
//			       asic_tcpudp.isCollision, asic_tcpudp.isCollision2, asic_tcpudp.isStatic, asic_tcpudp.isTcp );
			...
			#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
			seq_printf(s, "   age(%d) offset(%d) tcpflag(%d) SelEIdx(%d) SelIPIdx(%d) priValid:%d pri(%d)\n",
...
#if defined(CONFIG_RTL_8197F)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
			seq_printf(s, "   NHIDXValid(%d) NHIDX(%d)\n",
...
extern uint32 cnt_rx_desc_full_and_buf_pool_not_empty;
#endif
...
EXPORT_SYMBOL(statistic_wlan_rx_unicast_pkt);
EXPORT_SYMBOL(statistic_wlan_out_napt_exist);
EXPORT_SYMBOL(statistic_wlan_xmit_to_eth);
#endif
...
static int32 stats_debug_entry_read(struct seq_file *s, void *v)
{
	...
	#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
	seq_printf(s,"    tx_ringFull_cnt (ring 0 ~ 3):	%u, %u, %u, %u\n",
		tx_ringFull_count[0],tx_ringFull_count[1],tx_ringFull_count[2],tx_ringFull_count[3]);
	seq_printf(s,"    tx_drop_cnt (ring 0 ~ 3):	%u, %u, %u, %u\n",
		tx_drop_count[0],tx_drop_count[1],tx_drop_count[2],tx_drop_count[3]);	
	#else
	...
	seq_printf(s,"    statistic_ipv6_pppoe_xmit:	%u\n",statistic_ipv6_pppoe_xmit);
	//seq_printf(s,"    statistic_ipv6_hard_xmit:	%u\n",statistic_ipv6_hard_xmit);
	//seq_printf(s,"    statistic_ipv6_neigh_xmit:	%u\n",statistic_ipv6_neigh_xmit);
	
	#if defined(CONFIG_OPENWRT_SDK)
	seq_printf(s,"    statistic_fastpath:	%d\n",statistic_fastpath);
	#else
	seq_printf(s,"    statistic_fp:	%d\n",statistic_fp);
	seq_printf(s,"    statistic_fp_failed: %d\n",statistic_fp_failed);
	seq_printf(s,"    statistic_normal:	%d\n",statistic_normal);
	seq_printf(s,"    statistic_match_failed: %d\n",statistic_match_failed);
	#endif
	...
	seq_printf(s,"    cnt_swcore_link_chg:	%d\n",cnt_swcore_link);
	seq_printf(s,"    cnt_swcore_runout:	%d\n",cnt_swcore_err);
#endif
	
	seq_printf(s,"    rx_desc_num: %d\n",NUM_RX_PKTHDR_DESC);
	seq_printf(s,"    tx_desc_num: %d\n",NUM_TX_PKTHDR_DESC);
...
#if defined(CONFIG_RTL_8197F) &&  defined(CONFIG_FINETUNE_RUNOUT_IRQ)
	seq_printf(s,"    rx_desc_full_and_buf_pool_not_empty:	%u\n",cnt_rx_desc_full_and_buf_pool_not_empty);
	...
	#endif
	seq_printf(s,"    WanPortMask: %u\n", RTL_WANPORT_MASK);
	seq_printf(s,"    LanPortMask: %u\n", RTL_LANPORT_MASK);
...
#endif
	len += sprintf(page+len,"    WanPortMask: %u\n", RTL_WANPORT_MASK);
	len += sprintf(page+len,"    LanPortMask: %u\n", RTL_LANPORT_MASK);
...

{
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, len)) {
		...
		if(strncmp(cmd_addr, "clear",5) == 0)
		{
			...
			#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
			tx_ringFull_count[0]=0;
			tx_ringFull_count[1]=0;
			tx_ringFull_count[2]=0;
			tx_ringFull_count[3]=0;
			tx_drop_count[0]=0;
			tx_drop_count[1]=0;
			tx_drop_count[2]=0;
			tx_drop_count[3]=0;
			...
			statistic_ps = 0;
			statistic_ipv6_fp = 0;
			statistic_ipv6_pppoe_xmit = 0;
			statistic_ipv6_hard_xmit = 0;
			statistic_ipv6_neigh_xmit = 0;
			#if defined(CONFIG_OPENWRT_SDK)
			statistic_fastpath = 0;
			#else
			statistic_fp = 0;
			statistic_fp_failed = 0;
			statistic_normal = 0;
			statistic_match_failed = 0;
			#endif
			#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
...
#endif
#if defined(CONFIG_RTL_8197F) && defined(CONFIG_FINETUNE_RUNOUT_IRQ)
			cnt_rx_desc_full_and_buf_pool_not_empty = 0;
#endif
		}
		...
		#if defined(CONFIG_RTL_ETH_PRIV_SKB) //for debug
		else if (!strncmp(cmd_addr, "allPriSkb", 9))
		{			
			rtl_dump_eth_skb_buf(0);
		}
		else if (!strncmp(cmd_addr, "remainPriSkb", 12))
		{
			rtl_dump_eth_skb_buf(1);
		}
		#endif

		#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
		else if (!strncmp(cmd_addr, "ringmap", 7))
		{
			uint32 data = REG32(TXRINGCR);
			rtlglue_printf(">> packet type and destination port to Tx Ring mapping:\n");
			rtlglue_printf("=======================================================\n");
			rtlglue_printf("   packet type: BC, MC => Tx Ring 3\n");
			rtlglue_printf("   packet type: UC\n");
			rtlglue_printf("       to WAN (mask 0x%x) or to LAN1 (mask 0x%x) => Tx Ring 0\n", RTL_WANPORT_MASK, RTL_LANPORT_MASK_1);
			rtlglue_printf("       to LAN2 (mask 0x%x) => Tx Ring 1\n", RTL_LANPORT_MASK_2);
			rtlglue_printf("       to LAN3 (mask 0x%x) => Tx Ring 2\n", RTL_LANPORT_MASK_3);
			rtlglue_printf("       to LAN4 (mask 0x%x) => Tx Ring 3\n", RTL_LANPORT_MASK_4);
			
			rtlglue_printf(">> Tx Ring to FIFO mapping: (0xB8010078 bit 19~12)\n");
			rtlglue_printf("==================================================\n");
			rtlglue_printf("   Tx Ring 0 => FIFO %d\n", (data >> CF_TXRING0_TO_FIFO_OFT) & 3);
			rtlglue_printf("   Tx Ring 1 => FIFO %d\n", (data >> CF_TXRING1_TO_FIFO_OFT) & 3);
			rtlglue_printf("   Tx Ring 2 => FIFO %d\n", (data >> CF_TXRING2_TO_FIFO_OFT) & 3);
			rtlglue_printf("   Tx Ring 3 => FIFO %d\n", (data >> CF_TXRING3_TO_FIFO_OFT) & 3);
		}
		#ifdef RTL_DYM_ADJ_RING_FIFO_MAP
		else if(strncmp(cmd_addr, "mr_tp_th", 8) == 0)
		{
			extern uint8 rf_tp_threshold;
			tokptr = strsep(&strptr," ");
			if (tokptr)
				rf_tp_threshold = (uint8)simple_strtoul(tokptr, NULL, 10);
			
			rtlglue_printf("throughput threshold for multiple ring mechanism is: %d (Mbps)\n", rf_tp_threshold);
		}
		#endif		
		#endif
...
#define SPEED1000M 	2
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "port", 4))
		{
			...
			if(strncmp(tokptr,"10_half", 7) == 0)
				type = HALF_DUPLEX_10M;
			else if(strncmp(tokptr,"100_half", 8) == 0)
				type = HALF_DUPLEX_100M;
			else if(strncmp(tokptr,"1000_half", 9) == 0)
				type = HALF_DUPLEX_1000M;
			else if(strncmp(tokptr,"10_full", 7) == 0)
				type = DUPLEX_10M;
			else if(strncmp(tokptr,"100_full", 8) == 0)
				type = DUPLEX_100M;
			else if(strncmp(tokptr,"1000_full", 9) == 0)
				type = DUPLEX_1000M;
			else if(strncmp(tokptr,"down", 4) == 0)
				type = PORT_DOWN;
			else if(strncmp(tokptr,"up", 2) == 0)
				type = PORT_UP;
			else if(strncmp(tokptr,"an_10m", 6) == 0)
				type = AN_10M;
			else if(strncmp(tokptr,"an_100m", 7) == 0)
				type = AN_100M;
			else
				type = PORT_AUTO;
			...
			for(port = 0; port < EXT_SWITCH_MAX_PHY_PORT; port++)
			{
				...
				{
					int ret = 0;
					ret = set_83XX_speed_mode(port, type);
					if(ret)
					{
						printk("ERROR: set rtl8367 port(%d) speed(type%d) failed, ret=0x%x", port, type, ret);
					}
				}
			}
#else
			...
			if ((type == AN_10M) || (type == AN_100M) || (type == AN_AUTO)) {
				for(port = 0; port < CPU; port++)
				{
					if ((1<<port) & port_mask) {
						...
						if ((port == 0) && (type == AN_100M))
							advCapability = (1<<HALF_DUPLEX_10M) | (1<<DUPLEX_10M) | (1<<HALF_DUPLEX_100M)| (1<<DUPLEX_100M);
						#endif
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		else if(strncmp(cmdptr, "dump",4) == 0)
		{
			...
			if(strncmp(cmdptr, "8367",4) == 0 || strncmp(cmdptr, "83xx",4) == 0) {
				// CONFIG_RTL_83XX_API_V1_4
				if ((cmdptr[4] >= '0') && (cmdptr[4] <= '5')) {
					portNum = cmdptr[4] - 0x30;
					if (portNum == 5)
						portNum = r8367_cpu_port;
...
#ifdef CONFIG_RTL_PROC_NEW
static int32 proc_RTL8367_Reg_read(struct seq_file *s, void *v)
{
	return PROC_READ_RETURN_VALUE;
}
...
/* RTL8367 Reg Read/Write */
static int32 proc_RTL8367_Reg_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	char 		tmpbuf[64];
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "r", 1))
		{
			...
			regAddr=simple_strtol(tokptr, NULL, 16);

			ret=rtl83xx_getAsicReg(regAddr, &regData);
			if(ret==SUCCESS)
			{
				rtlglue_printf("read regAddr=0x%x,regData=0x%x\n", regAddr, regData);
			}
			else
			{
				rtlglue_printf("error input!\n");
		...
		else if (!memcmp(cmd_addr, "w", 1))
		{
			...
			ret=rtl83xx_setAsicReg(regAddr, regData);
			if(ret==SUCCESS)
			{
				rtlglue_printf("write regAddr=0x%x,regData=0x%x\n", regAddr, regData);
			}
			else
			{
				rtlglue_printf("error input!\n");
...
static int32 l3v6_read(struct seq_file *s, void *v)
{
	...
	int8 *str[] = { "PPPoE", "L2", "ARP", " ", "CPU", "NxtHop", "DROP", " " };
	seq_printf(s, "%s \n", "add l3v6 example:");
	seq_printf(s, "%s \n", "add l3v6    ipv6_address                      prefix_lenth             ipv6_nexthop                interface_name ");
	seq_printf(s, "%s \n", "add l3v6    0xfe8000-0x0-0x0-0x0           64                 0xfe8000-0x0-0x0-0x0           eth0");
	seq_printf(s, "%s \n",  "del l3v6    ipv6_address                      prefix_lenth          ");
	seq_printf(s, "%s \n", "del l3v6 example:");
	seq_printf(s, "%s \n", "del l3v6    0xfe8000-0x0-0x0-0x0           64               ");
	seq_printf(s, "%s \n", "------------------------------------------------------------------------------------");
...
static int32 l3v6_write(struct file *filp, const char *buff, unsigned long len, void *data )
{
	...
	int fc_dst_len=0;
	char *if_name;
	extern int32 rtl8198c_addIpv6Route(inv6_addr_t ipAddr, int fc_dst_len, inv6_addr_t nextHop, int8 * ifName);
	extern int32 rtl8198c_addIpv6Arp(inv6_addr_t ip, ether_addr_t *mac,int8 * ifName);
	extern int32 rtl8198c_delIpv6Route(inv6_addr_t ipAddr, int fc_dst_len);
	extern int32 rtl8198c_delIpv6Arp(inv6_addr_t ip);
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		tmpbuf[len] = '\0';
		strptr = tmpbuf;
		...
		{
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr && !memcmp(cmd_addr, "l3v6", strlen("l3v6")))
			{
				cmd_addr = strsep(&strptr," ");
				if(cmd_addr==NULL)
					return len;
				sscanf(cmd_addr, "0x%x-%x-%x-%x", &ipAddr.v6_addr32[0], &ipAddr.v6_addr32[1], &ipAddr.v6_addr32[2], &ipAddr.v6_addr32[3]);
				cmd_addr = strsep(&strptr," ");
				if(cmd_addr==NULL)
					return len;
				sscanf(cmd_addr, "%d",&fc_dst_len );
				cmd_addr = strsep(&strptr," ");
				if(cmd_addr==NULL)
					return len;
				sscanf(cmd_addr, "0x%x-%x-%x-%x", &nexthop.v6_addr32[0], &nexthop.v6_addr32[1], &nexthop.v6_addr32[2], &nexthop.v6_addr32[3]);
				if(strptr==NULL)
					return len;
				if_name=strptr;
				if_name[strlen(if_name)-1]='\0';
				rtl8198c_addIpv6Route(ipAddr,fc_dst_len, nexthop,if_name);
			}
			else
			{
				return len;
		...
		else if(cmd_addr && !memcmp(cmd_addr, "del", strlen("del")))
		{
			...
			if(cmd_addr && !memcmp(cmd_addr, "l3v6", strlen("l3v6")))
			{
				...
				if(strptr==NULL)
					return len;
				sscanf(strptr, "%d",&fc_dst_len );
				rtl8198c_delIpv6Route(ipAddr,fc_dst_len);
			}
			else
			{
				return len;
...
extern rtl8198c_ipv6_route_t * rtl8198c_get_ipv6_route_inusedHead(void);
#ifdef CONFIG_RTL_PROC_NEW
static int32 swl3v6_read(struct seq_file *s, void *v)
{
	int count;
	...
	int8 *str[] = { "PPPoE", "L2", "ARP", " ", "CPU", "NxtHop", "DROP", " " };
	count=0;
	rt = rtl8198c_get_ipv6_route_inusedHead();
	seq_printf(s, "%s\n", "SW L3V6 Routing Table:\n");
	while(rt)
	{
		seq_printf(s,"%02d:",count);
		if (rt->valid == 1) {
			seq_printf(s,"rt:%p ip:%x:%x:%x:%x fc_dst_len %d asicIdx %d Process %s arp.subnetIdx %d\n",
				rt,rt->ipAddr.v6_addr32[0],rt->ipAddr.v6_addr32[1],rt->ipAddr.v6_addr32[2],rt->ipAddr.v6_addr32[3],
				rt->fc_dst_len,rt->asicIdx,str[rt->process],rt->arp.subnetIdx);
		}
		rt = rt->next;
		count++;
	}
...
#else
static int32 swl3v6_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	int len, count;
	rtl8198c_ipv6_route_t *rt;
#ifdef CONFIG_RTL_LAYERED_DRIVER_L3	
	...
	while(rt)
	{
		len += sprintf(page,"%02d:",count);
		if (rt->valid == 1) {
			len += sprintf(page,"rt:%p ip:%x:%x:%x:%x fc_dst_len %d asicIdx %d Process %s arp.subnetIdx %d\n",
				rt,rt->ipAddr.v6_addr32[0],rt->ipAddr.v6_addr32[1],rt->ipAddr.v6_addr32[2],rt->ipAddr.v6_addr32[3],
				rt->fc_dst_len,rt->asicIdx,str[rt->process],rt->arp.subnetIdx);
		}
		rt = rt->next;
...
#endif
static int32 swl3v6_write(struct file *filp, const char *buff, unsigned long len, void *data )
{
	char  tmpbuf[128] = {'\0'};
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
...
static int32 arp6_read(struct seq_file *s, void *v)
{
	...
	memset(&asic_arpv6, 0, sizeof(rtl8198C_tblAsicDrv_arpV6Param_t));
	seq_printf(s, "%s \n", "add arp6 example:");
	seq_printf(s, "%s \n", "add    arp6   ipv6_address                                      mac                                 interface_name ");
	seq_printf(s, "%s \n", "add    arp6    0xfe8000-0x0-0x0-0x0     0x00-0x00-0x00-0x00-0x00-0x00             eth0");
	seq_printf(s, "%s \n", "del arp6 example:");
	seq_printf(s, "%s \n", "del    arp6   ipv6_address                     ");
	seq_printf(s, "%s \n", "del    arp6    0xfe8000-0x0-0x0-0x0    ");
	seq_printf(s, "%s \n", "---------------------------------------------------------------------------------");
	seq_printf(s, "%s\n", "ASIC ArpV6 Table:\n");
...
static int32 arp6_write(struct file *filp, const char *buff, unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		{
			cmd_addr = strsep(&strptr," ");
			if(cmd_addr && !memcmp(cmd_addr, "arp6", strlen("arp6")))
			{
				cmd_addr = strsep(&strptr," ");
				if(cmd_addr==NULL)
					return len;
				...
				if_name=strptr;
				if_name[strlen(if_name)-1]='\0';
				rtl8198c_addIpv6Arp(ipAddr,((ether_addr_t *)mac),if_name);
		...
		else if(cmd_addr && !memcmp(cmd_addr, "del", strlen("del")))
		{
			...
			if(cmd_addr && !memcmp(cmd_addr, "arp6", strlen("arp6")))
			{
				...
				if(strptr==NULL)
					return len;
				sscanf(strptr, "0x%x-%x-%x-%x", &ipAddr.v6_addr32[0], &ipAddr.v6_addr32[1], &ipAddr.v6_addr32[2], &ipAddr.v6_addr32[3]);
				rtl8198c_delIpv6Arp(ipAddr);
...
static int32 acl_write(struct file *file, const char *buffer, unsigned long len, void *data)
{
	char tmp_buf[512];
	char *strptr;
	...
	if (buffer && !copy_from_user(tmp_buf, buffer, len)) {
		tmp_buf[len] = '\0';
		strptr = tmp_buf;
		
		if(!strncmp(strptr, "reg", 3) || !strncmp(strptr, "unReg", 5)){
			if(config_acl_chain(strptr) < 0){
				goto errout;
			}
			else{
				goto okout;
		...
		else if(!strncmp(strptr, "flush", 5)){
			if(flush_netif_acl(strptr) < 0){
				goto errout;
			}
			else{
				goto okout;
		...
		else if(!strncmp(strptr, "conf", 4)){
			if(config_acl_capacity(strptr) < 0){
				goto errout;
			}
			else{
				goto okout;
		...
		else if(!strncmp(strptr, "add", 3) || !strncmp(strptr, "del", 3)){
			if(config_acl_rules(strptr) < 0){
				goto errout;
			}
			else{
				goto okout;
		...
		else if(!strncmp(strptr, "help", 4)){
			panic_printk("Enabel/Disable acl:\n");
			panic_printk("	echo \"config [1/0] [in]\" > /proc/rtl865x/acl\n");
			panic_printk("\n");

			panic_printk("	Examples:\n");
			panic_printk("		echo \"config 1 in\" > /proc/rtl865x/acl\n");
			panic_printk("		echo \"config 0 in\" > /proc/rtl865x/acl\n");
			panic_printk("\n");
		
			panic_printk("Regiter/Unregister chain:\n");
			panic_printk("	echo \"[reg/unReg] [$netifName] [$prio] [in]\" > /proc/rtl865x/acl\n");
			panic_printk("\n");
			
			panic_printk("	Examples:\n");
			panic_printk("		echo \"reg eth1 -16000 in\" > /proc/rtl865x/acl\n");
			panic_printk("		echo \"unReg br0 -16001 in\" > /proc/rtl865x/acl\n");
			panic_printk("\n");
			
			panic_printk("Add/Del rule:\n");
			panic_printk("	[] is required, {} is optional.\n");
			panic_printk("		echo \"[add/del] [netif $name] [chain $prio] [act permit/toCPU/drop/rateLimitBps/priority $prio]\n");
			panic_printk("		{layer all/l2/l3/l4/l2_l3/l3_l4} {rateIdx 0-15} {inv 0/1} {com 0/1}\n");
		#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
			panic_printk("		[type mac/ip/ipv6/tcp/udp/srcfilter/dstfilter] [rule ***]\" > /proc/rtl865x/acl\n");
		#else
			panic_printk("		[type mac/ip/tcp/udp/srcfilter/dstfilter] [rule ***]\" > /proc/rtl865x/acl\n");
		#endif
			panic_printk("\n");
			
			panic_printk("	[rule ***] in hex. Any tuple or combinations work.\n");
			panic_printk("		ip:		{sip $ip $mask} {dip $ip $mask} {tos $tos $mask} {ipProto $proto $protomask}.\n");
		#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
			panic_printk("		ipv6:		{nheader nheader $mask}.\n");
		#endif
			panic_printk("		mac:		{smac $mac $mask} {dmac $mac $mask} {ethType $type $mask}\n");
			panic_printk("		tcp/udp:	port must before ip info.\n");
			panic_printk("				{sport $portL $portU} {flags $flag $mask} {dport $portL $portU} {sip $ip $mask} {dip $ip $mask} {tos $tos $mask}\n");
			panic_printk("		srcfilter:	{smac $mac $mask} {svid $vid $mask} {sip $ip $mask} {sport $portL $portU} {proto $proto} {srcPhyport $port}\n");
			panic_printk("		dstfilter:	{dmac $mac $mask} {dvid $vid $mask} {dip $ip $mask} {dport $portL $portU} {proto $proto}\n");
			panic_printk("\n");

			panic_printk("	Examples:\n");
			panic_printk("		echo \"add netif eth1 chain -16000 act rateLimitBps rateIdx 0 type ip rule sip C0A80214 FFFFFFFF\" > /proc/rtl865x/acl\n");
			panic_printk("		echo \"add netif eth1 chain -16000 act permit type ip rule tos 20 FF ipProto 02 FF\" > /proc/rtl865x/acl\n");
			panic_printk("		echo \"del netif br0 chain -16001 act priority 5 type mac rule ethType 0806 FFFF smac 000600030002 FFFFFFFFFFFF\" > /proc/rtl865x/acl\n");
			panic_printk("		echo \"add netif eth1 chain -16000 act drop type tcp rule sport 4E20 4E30 flags 10 3F dip C0A8010A FFFFFFFF tos 20 FF\" > /proc/rtl865x/acl\n");
			panic_printk("		echo \"add netif br0 chain -16001 act toCPU type srcfilter rule sip C0A8010D FFFFFFFF sport 0030 0060\" > /proc/rtl865x/acl\n");
			panic_printk("		echo \"add netif eth1 chain -16001 act drop type srcfilter rule srcPhyport 100\" > /proc/rtl865x/acl\n");
			panic_printk("		echo \"add netif br0 chain -16001 act toCPU type dstfilter rule dip C0A8020D FFFFFFFF dport 0030 0060\" > /proc/rtl865x/acl\n");
		#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
			panic_printk("		echo \"add netif eth1 chain -16000 act drop type ipv6 rule nheader 3A FF\" > /proc/rtl865x/acl\n");
		#if defined(CONFIG_RTL_8197F)
			panic_printk("		echo \"add netif eth1 chain -16000 act priority 5 type pattern rule 2 C 86dd ffff 14 0600 ff00\" > /proc/rtl865x/acl\n");
		#endif
		#endif
			panic_printk("\n");

			panic_printk("Flush all acl rules on given netif:\n");
			panic_printk("	echo \"flush [$netifName] [in]\" > /proc/rtl865x/acl\n");
			panic_printk("\n");

			panic_printk("	Examples:\n");
			panic_printk("		echo \"flush eth1 in\" > /proc/rtl865x/acl\n");
			panic_printk("\n");
...
errout:
		panic_printk("Fail: error input\n");
		return len;
	}

okout:
	panic_printk("success!\n");
	return len;
}
...
/*RLT8367 Reg*/
int RTL8367_Reg_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, proc_RTL8367_Reg_read, NULL));
}
static ssize_t RTL8367_Reg_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return proc_RTL8367_Reg_write(file, userbuf,count, off);
}
struct file_operations RTL8367_Reg_proc_fops= {
        .open           = RTL8367_Reg_single_open,
        .write		    = RTL8367_Reg_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
int swl3v6_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, swl3v6_read, NULL));
}
static ssize_t swl3v6_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return swl3v6_write(file, userbuf,count, off);
}
struct file_operations swl3v6_proc_fops= {
        .open           = swl3v6_single_open,
        .write		    = swl3v6_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
/*brsc*/
extern void rtw_brsc_dump(struct seq_file *s, void *v);
extern void rtw_brsc_enable(void);
extern void rtw_brsc_disable(void);
extern unsigned char rtw_brsc_get_disable(void);
extern int rtw_brsc_add_black_dev(char *name);
extern int rtw_brsc_del_black_dev(char *name);
extern int rtw_brsc_flush_black_dev(void);
extern int rtw_brsc_set_blacklist_enable(int enable);
extern int rtw_brsc_update_br0_mac(unsigned char *mac);

static int32 rtw_brsc_read(struct seq_file *s, void *v)
{
	rtw_brsc_dump(s, v);
	return 0;
}
static int32 rtw_brsc_write(struct file *filp, const char *buff, unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		if(cmd_addr == NULL)
			goto error;
		if(!strncmp(cmd_addr,"enable",6))
		{
			rtw_brsc_enable();
			printk("rtw_brsc.disable %d\n",rtw_brsc_get_disable());
		}
		else if(!strncmp(cmd_addr,"disable",7))
		{
			rtw_brsc_disable();
			printk("rtw_brsc.disable %d\n",rtw_brsc_get_disable());
		}
		else if(!strncmp(cmd_addr,"blacklist_enable",16))
		{
			rtw_brsc_set_blacklist_enable(1);
		}
		else if(!strncmp(cmd_addr,"blacklist_disable",17))
		{
			rtw_brsc_set_blacklist_enable(0);
		}
		else if(!strncmp(cmd_addr,"add_black_ifname",16))
		{
			if(rtw_brsc_add_black_dev(strptr))
			{
				printk("add_block_ifname %s fail\n",strptr);
			}
		}
		else if(!strncmp(cmd_addr,"del_black_ifname",16))
		{
			if(rtw_brsc_del_black_dev(strptr))
			{
				printk("del_block_ifname %s fail\n",strptr);
			}
		}
		else if(!strncmp(cmd_addr,"flush_black_ifname",18))
		{
			rtw_brsc_flush_black_dev();
		}
		else if(!strncmp(cmd_addr,"update_br0_mac",14))
		{
			cmd_addr = strsep(&strptr,",");
			if(cmd_addr==NULL)
				return len;
			sscanf(cmd_addr, "0x%x-%x-%x-%x-%x-%x", (uint32 *)&mac[0], (uint32 *)&mac[1],(uint32 *)&mac[2], (uint32 *)&mac[3],(uint32 *)&mac[4],(uint32 *)&mac[5]);
			rtw_brsc_update_br0_mac(mac);
			printk("update br0 mac %pM\n",mac);
		}
		else
		{
			goto error;
...
error:
	printk("usage: \n");
	printk("\techo disable > /proc/rtl865x/brsc\n");
	printk("\techo enable > /proc/rtl865x/brsc\n");
	printk("\techo blacklist_disable > /proc/rtl865x/brsc\n");
	printk("\techo blacklist_enable > /proc/rtl865x/brsc\n");
	printk("\techo add_block_ifname [ifname] > /proc/rtl865x/brsc\n");
	printk("\techo del_block_ifname [ifname] > /proc/rtl865x/brsc\n");
	printk("\techo flush_block_ifname > /proc/rtl865x/brsc\n");
	printk("\techo update_br0_mac 0x00-11-22-33-44-55 > /proc/rtl865x/brsc\n");
	return len;
}


int rtw_brsc_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtw_brsc_read, NULL));
}
static ssize_t rtw_brsc_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return rtw_brsc_write(file, userbuf,count, off);
}
struct file_operations rtw_brsc_proc_fops= {
        .open           = rtw_brsc_single_open,
        .write		    = rtw_brsc_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...

{
	...
	rtlglue_printf("%s\n", "ASIC VLAN Table:");
	{
		...
		for ( i = 0; i < 4095; i++ ) 
		{
			...
			if ( rtk_vlan_get(i,&vlanCfg) == FAILED )
				continue;
			if(vlanCfg.mbr.bits[0] == 0)
				continue;
			
			vlan.memberPortMask = vlanCfg.mbr.bits[0];
			vlan.untagPortMask = vlanCfg.untag.bits[0];
			vlan.fid = vlanCfg.fid_msti;
#endif
...
int32 rtl865x_proc_debug_init(void)
{
	...
	if(rtl865x_proc_dir)
	{
        ...
        {
#ifdef CONFIG_RTL_PROC_NEW
            proc_create_data("kernelMCast",0,rtl865x_proc_dir,&kernel_group_single_seq_file_operations,NULL);
#else
            kernel_group_entry=create_proc_entry("kernelMCast", 0, rtl865x_proc_dir);
            if(kernel_group_entry != NULL)
            {
                kernel_group_entry->proc_fops = &kernel_group_single_seq_file_operations;
                retval = SUCCESS;
            }
#endif
        }
		...
		/* 8367Reg */
		proc_create_data("8367Reg",0,rtl865x_proc_dir,&RTL8367_Reg_proc_fops,NULL);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
					proc_create_data("sw_l3v6",0,rtl865x_proc_dir,&swl3v6_proc_fops,NULL);
#else
					swl3v6_entry = create_proc_entry("sw_l3v6", 0, rtl865x_proc_dir);
					if (swl3v6_entry != NULL)
					{
						swl3v6_entry->read_proc = l3v6_read;
						swl3v6_entry->write_proc = l3v6_write;
						retval = SUCCESS;
					}
					else
					{
...
/*sw-arp6*/
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("brsc",0,rtl865x_proc_dir,&rtw_brsc_proc_fops,NULL);
#else
#endif
		}
#endif
