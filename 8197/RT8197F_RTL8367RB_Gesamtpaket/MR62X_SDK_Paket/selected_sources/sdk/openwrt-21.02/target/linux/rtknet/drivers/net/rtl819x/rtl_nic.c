...
#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
void rtl_map_ring_by_port(rtl_nicTx_info *nicTx);
void rtl_map_ring_by_L2_port(struct sk_buff* skb, rtl_nicTx_info *nicTx);
void rtl_find_port_to_ring(void);
static unsigned char last_mac[12];
static unsigned int last_ring_idx=0;
uint8 rf_port_to_ring[RTL8651_PHY_NUMBER]={0,0,0,0,0};

/*
	compile flag: RTL_DYM_ADJ_RING_FIFO_MAP
	description: dynamically adjust the Tx Ring and Tx FIFO mapping
	why to do this: 4 Tx Ring and only 3 Tx FIFO
	which configuration need this: 1WAN+4LAN
	which configuration do not need this: 1WAN+3LAN, 1WAN+2LAN
		but need to define RTL_LANPORT_MASK_1 ~ RTL_LANPORT_MASK_4 carefully
		(see rtl_map_ring_by_L2_port() function)
 */
#define RTL_DYM_ADJ_RING_FIFO_MAP		1
#define RF_TP_THRESHOLD		20 	// unit Mbps 

#ifdef RTL_DYM_ADJ_RING_FIFO_MAP
uint8 rf_adjusted=0;
uint8 rf_saved_traffic_portmask=0;
uint8 rf_1sec_toggle=0;
uint8 rf_tp_threshold=RF_TP_THRESHOLD;
uint64 rf_port_tx_bytes_bak[RTL8651_PHY_NUMBER]={0,0,0,0,0};
#endif
#endif

extern int mirrorPort_enable;
extern uint32 mirrorPort_mask; 
extern uint32 mirrorPort_txmask; 
...
#if defined(CONFIG_RTL_83XX_API_V1_4)
#include "rtl83xx_v1dot4/rtk_types.h"
#include "rtl83xx_v1dot4/vlan.h"
#include "rtl83xx_v1dot4/led.h"
#else
...
int32 rtl_init83xxACLProc(void);
void __exit rtl_exit83xxACLCtrl(void);
#endif
...
#if (defined(CONFIG_RTL_8198C)||defined(CONFIG_RTL_8197F))&&defined(CONFIG_RTL_HARDWARE_MULTICAST_API)
#include <linux/ipv6.h>
#endif

#ifdef CONFIG_TP_IMAGE
int tp_mcast_vlan_enabled = FALSE;
int tp_iptv_vid = -1;
int tp_mcast_vid = -1;
EXPORT_SYMBOL(tp_mcast_vlan_enabled);
EXPORT_SYMBOL(tp_iptv_vid);
EXPORT_SYMBOL(tp_mcast_vid);
#endif
...
EXPORT_SYMBOL_GPL(suppress_mode);
extern atomic_t loop_pkt_cnt;
...
extern uint32 vportmask;
extern uint32 vutpportmask;
extern uint32 r8367_cpu_port;
extern int rtl_vlan_RTL83XX_set(unsigned short vid, unsigned int tagmask, unsigned int mask, unsigned int fid);
#endif

#if defined(CONFIG_RTL_8367R_SUPPORT)
extern int32 rtl_8367r_restartPHYNway(uint32 port);
#elif defined(CONFIG_RTL_83XX_SUPPORT)
extern int32 rtl_83xx_restartPHYNway(uint32 port);
#endif
...
extern int rtl_83xx_add_acl_for_dns(unsigned int acl_idx);
extern int rtl_83xx_remove_acl_for_dns(unsigned int acl_idx);
#endif
#endif
...
#if defined(CONFIG_RTW_MEMPOOL)
#define RTW_ETH_MEMPOOL_NAME "eth_buffer"
#endif

int insert_vlan_tag2(uint16 vid,struct sk_buff **pskb)
{
...
#define CHECK_TX_ABNORMAL_AND_RESTART_THRESHOLD		5
...
static int32 rtl_reinit_swCore_and_failed_counter = 0;
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
extern int rtl865x_disableRtl83xxToCpuAcl(void);
...
int freeSkbThreshold=DEF_FREE_PRIV_SKB_TRHRESHOLD;
uint32 cnt_rx_desc_full_and_buf_pool_not_empty=0;
extern uint32 rx_desc_full(uint32 ring_idx);
...
inline int rtl_runout_resume(void);
#endif
...
extern uint32 get_uptime_by_sec(void);
uint32 port_change_time[8] = {0, 0, 0, 0, 0, 0, 1, 0};
uint32 port_link_status = 0;
uint32 if_change_time[2] = {0, 0};
uint32 phy_enable_all = 0;
#endif
...
typedef struct _one_sec_timer_priv {
	unsigned long task_priv;
	struct work_struct one_sec_timer_work;
} one_sec_timer_priv;
static one_sec_timer_priv rtl_one_sec_timer_priv;

static int rtl_rxTxDoneCnt = 0;
static atomic_t rtl_devOpened;
static atomic_t rtl_timer_id;
...
unsigned int tx_ringFull_count[4] = {0,0,0,0};
unsigned int tx_drop_count[4] = {0,0,0,0};
#endif
...
extern int rtk_vlan_portPvid_set(uint32 port, uint32 pPvid, uint32 pPriority);

void rtl_port_link_status_set(int port, int enable)
{
	if (enable)
		port_link_status &= ~(1 << port);
	else
	 	port_link_status |= (1 << port);
}
...
int wan_dev_mtu = 1500;
EXPORT_SYMBOL(wan_dev_mtu);
/*pppoe-internet*/
char ppp_ext[64] = {'\0'};
int ppp_ext_len = 0;
/*
...
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
	{	RTL_DRV_BR_WAN_NETIF_NAME,	 1,   IF_ETHER, RTL_WANVLANID,		RTL_WAN_FID,	RTL_WANPORT_MASK,		RTL_WANPORT_MASK,		1500,	{ { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 0	},
#endif
#endif
#if defined(CONFIG_RTK_VLAN_SUPPORT)
...
#endif
#endif
	/*RTL_DRV_PPP_NETIF_NAME == ppp_ext*/
	{	"ppp0", 1,   IF_PPPOE,    	RTL_WANVLANID,		RTL_WAN_FID,    	RTL_WANPORT_MASK,         	RTL_WANPORT_MASK,     	1500, { { 0x00, 0x12, 0x34, 0x56, 0x78, 0x91 } }, 1 },
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
#if defined(CONFIG_RTL_HW_DSLITE_SUPPORT)	
...
#define EXT_RX_CONTINUE 3
#define WLAN_BASE_NAME "wlan"
#define WLAN_NAME_NUM(name,num)	name#num
extern int gHwNatEnabled;
...
int wanport_mask = 0x10;
int lanport_mask = 0xf;
int lanport_mask_1 = 0x8; //port 0 
int lanport_mask_2 = 0x4; //port 1
int lanport_mask_3 = 0x2; //port 2
int lanport_mask_4 = 0x1; //port 3
char hw_nat_netif_name[IFNAMSIZ] = {'\0'};
int multi_phy_vir_wan_enable = 1;
struct wanport_fid{
	int portmask;
	int in_used;
	int wan_fid;
};
#define WAN_FID_NUM_MAX     3
struct wanport_fid wanport_fid_map[WAN_FID_NUM_MAX];
#include <net/rtl/phy_multi_wan_portmask.h>
static int hw_nat_netif_read(struct seq_file *s, void *v);
static int hw_nat_netif_write(struct file *file, const char *buffer, unsigned long count, void *data);
int hw_nat_netif_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, hw_nat_netif_read,NULL));
}
static ssize_t  hw_nat_netif_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return hw_nat_netif_write(file, userbuf,count, off);
}
struct file_operations hw_nat_netif_proc_fops= {
        .open           = hw_nat_netif_single_open,
        .write		    = hw_nat_netif_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};

static int phy_wan_portmask_read(struct seq_file *s, void *v);
static int phy_wan_portmask_write(struct file *file, const char *buffer, unsigned long count, void *data);
int  phy_wan_portmask_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file,  phy_wan_portmask_read,NULL));
}
static ssize_t   phy_wan_portmask_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return  phy_wan_portmask_write(file, userbuf,count, off);
}
struct file_operations  phy_wan_portmask_proc_fops= {
        .open           = phy_wan_portmask_single_open,
        .write		    = phy_wan_portmask_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};

static int port_and_netif_read(struct seq_file *s, void *v);
static int port_and_netif_write(struct file *file, const char *buffer, unsigned long count, void *data);
int port_and_netif_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, port_and_netif_read,NULL));
}
static ssize_t  port_and_netif_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return port_and_netif_write(file, userbuf,count, off);
}
struct file_operations port_and_netif_proc_fops= {
        .open           = port_and_netif_single_open,
        .write		    = port_and_netif_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#endif
#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
static int phy_multi_wan_rx(int port, struct sk_buff *skb, rtl_nicRx_info *info);
#endif
#if defined (CONFIG_RTL_IGMP_SNOOPING)||defined(CONFIG_BRIDGE_IGMP_SNOOPING)
static inline int32 rtl_isWanPort(struct dev_priv *cp, uint16 vid, struct sk_buff *skb, int pid, int vid_type);
#endif
#if defined (CONFIG_RTL_HARDWARE_MULTICAST)&&defined(CONFIG_RTL_IGMP_SNOOPING)
...
uint8 rtk_get_vlan_priority(uint16 vid);
...
#if defined(CONFIG_RTL_8367R_SUPPORT)  || defined(CONFIG_RTL_83XX_SUPPORT)
//extern int rtl865x_disableRtl83xxToCpuAcl(void);
extern int rtl865x_enableRtl83xxToCpuAcl(void);
extern int rtl865x_enableRtl83xxBCMCToCpu(unsigned int acl_idx);
...
int rtk_netifName_change(char *oldname, char *newname)
{
	int32 i;
	int len;
	...
	{
		if (strcmp(netIfName[i], oldname))
			continue;
		len = strlen(newname);
		if (len >= IFNAMSIZ)
			return FAILED;
		memset(netIfName[i], 0, IFNAMSIZ);
		memcpy(netIfName[i], newname, len);
		return SUCCESS;
	}
	return FAILED;
}
...
int rtk_insert_vlan_tag_and_priority(uint16 vid, uint8 priority, struct sk_buff **pskb)
{
	struct sk_buff *newskb = NULL;
	struct sk_buff *skb = *pskb;
	if (*((unsigned short *)(skb->data+ETH_ALEN*2)) != __constant_htons(ETH_P_8021Q)) 
	{
		...
		skb_push(skb,VLAN_HLEN);			
		*((uint16*)(skb->data+(ETH_ALEN<<1))) = __constant_htons(ETH_P_8021Q);
		*((uint16*)(skb->data+(ETH_ALEN<<1)+2)) = __constant_htons((0xfff & vid) | ((0x7 & priority)<<13));
	}
	return SUCCESS;
}
...
#ifdef CONFIG_RTL_PROC_NEW
static int32 rtl_asic_operation_layer_read(struct seq_file *s, void *v);
static int32 rtl_asic_operation_layer_write( struct file *filp, const char *buff,unsigned long len, void *data );

int rtl_asic_operation_layer_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtl_asic_operation_layer_read, NULL));
}
static ssize_t rtl_asic_operation_layer_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
...
#else
static int32 rtl_asic_operation_layer_read( char *page, char **start, off_t off, int count, int *eof, void *data );
static int32 rtl_asic_operation_layer_write( struct file *filp, const char *buff,unsigned long len, void *data );
#endif
...
extern int rtl_83xx_share_meter_read_proc(struct seq_file *s, void *v);
int rtl_83xx_share_meter_single_open(struct inode *inode, struct file *file)
{
	return(single_open(file, rtl_83xx_share_meter_read_proc,NULL));
}

struct file_operations rtl_83xx_share_meter_proc_fops= {
    .open           = rtl_83xx_share_meter_single_open,
    .read           = seq_read,
    .llseek         = seq_lseek,
    .release        = single_release,
};
...
int32 rtl_init_83xx_share_meter_proc(void)
{
#ifdef CONFIG_RTL_PROC_NEW
    proc_create_data("rtl_83xx_share_meter",0,&proc_root,&rtl_83xx_share_meter_proc_fops,NULL);
#endif

	return 0;
}

void __exit rtl_exit_83xx_share_meter_proc(void)
{
#ifdef CONFIG_RTL_PROC_NEW
	remove_proc_entry("rtl_83xx_share_meter", &proc_root);
#endif
	return;
}
...
extern int rtl_83xx_storm_ctrl_read_proc(struct seq_file *s, void *v);
extern int rtl_83xx_storm_ctrl_write_proc(struct file *file, const char *buffer, unsigned long count, void *data);

int rtl_83xx_storm_ctrl_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtl_83xx_storm_ctrl_read_proc,NULL));
}
static ssize_t rtl_83xx_storm_ctrl_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return rtl_83xx_storm_ctrl_write_proc(file, userbuf,count, off);
}
struct file_operations rtl_83xx_storm_ctrl_proc_fops= {
        .open           = rtl_83xx_storm_ctrl_single_open,
        .write		    = rtl_83xx_storm_ctrl_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};

int32 rtl_init_83xx_storm_ctrl_proc(void)
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtl_83xx_storm_ctrl",0,&proc_root,&rtl_83xx_storm_ctrl_proc_fops,NULL);
#endif

	return 0;
}

void __exit rtl_exit_83xx_storm_ctrl_proc(void)
{
#ifdef CONFIG_RTL_PROC_NEW
	remove_proc_entry("rtl_83xx_storm_ctrl", &proc_root);
#endif
	return;
}
#endif

/* l2 learning number limitation*/
#if defined(CONFIG_RTL_8197F)
#ifdef CONFIG_RTL_PROC_NEW
static int32 l2_learn_limitation_read(struct seq_file *s, void *v);
#else
static int32 l2_learn_limitation_read( char *page, char **start, off_t off, int count, int *eof, void *data );
#endif
static int32 l2_learn_limitation_write( struct file *filp, const char *buff,unsigned long len, void *data );
#endif
...
EXPORT_SYMBOL(rtl_vlan_passthrough_enable);
EXPORT_SYMBOL(rtl_process_vlan_passthrough_tx);
...
{    
		...
		if (wan_management_vlan.vlan)
		{
			...
			if (info)
			{
				*vid = info->id;
				...
				return info->tag;     //�����Э��ջ����wan�Ķಥ��������man vlan info��tag��Ϣ
			}
		}
		#endif
	}
    ...
    if ( (mc_info) )
    {    	  
          return mc_info->tag;     //<C6><E4><CB><FB><C7><E9><BF>??D8>mc_info<B5><C4>tag<D0><C5>Ϣ
    }
...
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
	if(memcmp(dev->name,RTL_PS_BR_WAN_DEV_NAME,strlen(RTL_PS_BR_WAN_DEV_NAME)) == 0)
	{
		for(i = 0; i < NETIF_NUMBER;i++)
		{
			if(ps_drv_netif_mapping[i].valid == 1 &&
				memcmp(ps_drv_netif_mapping[i].drvName,RTL_DRV_BR_WAN_NETIF_NAME,strlen(RTL_DRV_BR_WAN_NETIF_NAME)) == 0)
				return &ps_drv_netif_mapping[i];
		}
	}
#endif
	...
	for(i=0; vlanConfig1[i].ifname[0] != '\0'; i++)
		...
		for(j=0; j<vlanCnt; j++)
		{
			...
			else
			{
				...
				{
					/*ethernet interface*/
#if defined(CONFIG_RTL_MAC_BASED_NETIF) || defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
...
static void rtl865x_restartDevPHYNway(struct net_device *dev, struct dev_priv *cp)
{
	...
	for(port=0;port<RTL8651_AGGREGATOR_NUMBER;port++)
	{
		if((1<<port) & cp->portmask)
		{
			...
			rtl_8367r_restartPHYNway(port);
			#elif defined(CONFIG_RTL_83XX_SUPPORT)
			rtl_83xx_restartPHYNway(port);
			#else
...
#if defined(CONFIG_RTW_MEMPOOL)
__IRAM_FWD
int re865x_mempool_free_cb(void *ptr)
{
	rtl_runout_resume();
	return 0;
}
...
static void refill_rx_skb(void)
{
...
#endif
	{
		...
		#else
	#if defined(CONFIG_RTW_MEMPOOL)
	skb = rtw_mem_pool_alloc_skb(CROSS_LAN_MBUF_LEN);
	#else
...
unsigned char *alloc_rx_buf(void **skb, int buflen)
{
	...
	if(buflen>CROSS_LAN_MBUF_LEN){
		...
		if (rx_skb_queue.qlen == 0) {
			...
			#else
	  #if defined(CONFIG_RTW_MEMPOOL)
	  new_skb = rtw_mem_pool_alloc_skb(CROSS_LAN_MBUF_LEN);
	  #else
...
#endif
#endif
#if defined (CONFIG_RTL_IGMP_SNOOPING)||defined(CONFIG_BRIDGE_IGMP_SNOOPING) || defined(CONFIG_RTL_HARDWARE_MULTICAST_API)
...
#if defined (CONFIG_RTL_MLD_SNOOPING)||defined(CONFIG_BRIDGE_IGMP_SNOOPING)||(defined(CONFIG_RTL_HARDWARE_MULTICAST_API)&&(defined(CONFIG_RTL_8198C)||defined(CONFIG_RTL_8197F)))
...
static inline void re865x_relayTrappedMCast(struct sk_buff *skb, unsigned int vid, unsigned int mcastFwdPortMask, unsigned int keepOrigSkb)
{
	...
	if(skb2!=NULL)
	{
...
#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
				rtl_map_ring_by_port(&nicTx_wan);
#endif
		...
		if (rtl_vlan_passthrough_enable){
			if (rtl_process_vlan_passthrough_tx(&skb2) == FAILED)
				return;
		}
#endif
		nicTx.txIdx=0;
...
static bool rtl_MulticastRxFilterOff(struct sk_buff *skb, int ipversion)
{
	...
	if(ipversion ==4)
	{
		...
		if(type == PPPOE_IGMP)
		{
			...
			skb->network_header = (unsigned int)((sk_buff_data_t)rtl_get_pppoe_ipv4_hdr(skb->data,1,0) - skb->head);
			#else
	...
	#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
		skb->network_header = (unsigned int)((sk_buff_data_t)re865x_getIpv4Header(skb->data) - skb->head);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
		skb->network_header = (unsigned int)((sk_buff_data_t)re865x_getIpv6Header(skb->data) - skb->head);
#else
	...
	#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
		skb->mac_header = (unsigned int)(skb->data - skb->head);
	#else
...
int  rtl_MulticastRxCheck(struct sk_buff *skb,rtl_nicRx_info *info,int mCastFlag)
{
	...
	unsigned int vlanFloodingPortMask = 0;
...
#if 0//def CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT
	int pvid = 0;
#endif
#if defined (CONFIG_RT_MULTIPLE_BR_SUPPORT) 
...
/*set flooding port mask first*/
	...
	//vlanFloodingPortMask = rtk_get_vlan_portmask(vid) & (~(1<<pid)) & ((1<<RTL8651_MAC_NUMBER)-1);
	/*
		bridge mode can't get ipv6 address from wan and
		ping ipv6 fail,so change getting portmask pattern
		from software to hardware which include wan port.
	*/
	vlanFloodingPortMask = rtl865x_getVlanPortMask(vid) & (~(1<<pid)) & ((1<<RTL8651_MAC_NUMBER)-1);
	if(linux_vlan_enable && rtk_get_vlan_type(vid)==VLAN_TYPE_NAT&& rtl865x_curOpMode == GATEWAY_MODE)
	{
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
		if(!((rtk_get_vlan_portmask(vid) & RTL_WANPORT_MASK) && (rtk_get_vlan_portmask(vid) & RTL_LANPORT_MASK)))
#endif
		{
			vlanFloodingPortMask &= (~RTL_WANPORT_MASK);
		}
	}
#else
	vlanFloodingPortMask=rtl865x_getVlanPortMask(vid) & (~(1<<pid)) & ((1<<RTL8651_MAC_NUMBER)-1);
	...
	if (ret_flag){
		...
		vlanFloodingPortMask &= ~(res_mask);
	}
#endif
	vlanRelayPortMask = vlanFloodingPortMask;
...
//#if defined (CONFIG_RTL_HARDWARE_MULTICAST)
        ...
        {
            skb->srcVlanId = *((unsigned short *)(skb->vlan_passthrough_saved_tag+2));
            skb->srcVlanId = __constant_ntohs(skb->srcVlanId) & 0xfff;
        }
        else 
        #endif
        {
		...
		if(iph!=NULL)
		{
			...
			if((l4Protocol==IPPROTO_UDP) || (l4Protocol==IPPROTO_TCP)|| (l4Protocol==Any_0_hop_protocl))
			{
				...
				if(vlan_bridge_tag && !strcmp(cp_this->dev->name,RTL_PS_ETH_NAME_ETH2))
				{
					if(igmpsnoopenabled && (nicIgmpModuleIndex_2!=0xFFFFFFFF))
					{
						...
						vlanRelayPortMask = vlanFloodingPortMask & nicMCastFwdInfo.fwdPortMask & cp_this->portmask;
				...
				//multicast data with mac not in L2 will be flooded by hw, do noting here
					...
					vlanRelayPortMask = vlanFloodingPortMask & nicMCastFwdInfo.fwdPortMask &cp_this->portmask;
                #else
                    vlanRelayPortMask = vlanFloodingPortMask & nicMCastFwdInfo.fwdPortMask;
                #endif
			...
			else if(l4Protocol==IPPROTO_IGMP)
			{
				...
				if(vlan_bridge_tag && !strcmp(cp_this->dev->name,RTL_PS_ETH_NAME_ETH2))
				{
					if(igmpsnoopenabled && (nicIgmpModuleIndex_2!=0xFFFFFFFF))
					{
						rtl_igmpMldProcess(nicIgmpModuleIndex_2, skb->data, pid, &vlanRelayPortMask, skb_headlen(skb));
						//just flooding
						vlanRelayPortMask = vlanFloodingPortMask;
					}
				}
				...
				#endif
					rtl_igmpMldProcess(nicIgmpModuleIndex, skb->data, pid, &vlanRelayPortMask, skb_headlen(skb));
					vlanRelayPortMask &= vlanFloodingPortMask;
				}
		#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
				}
				...
				if(tp_mcast_vlan_enabled)
				{
					struct sk_buff *skb2=NULL;
					int iptv_vid = tp_iptv_vid;
					int mcast_vid = tp_mcast_vid;
					if(pid == RTL83XX_WAN)
					{
						if(vid == mcast_vid && (iptv_vid > 1 && iptv_vid < 4095))
						{
							skb2= skb_clone(skb, GFP_ATOMIC);
							if(skb2)
							{
								re865x_relayTrappedMCast(skb2, iptv_vid, 
									rtk_get_vlan_portmask(iptv_vid) & RTL_LANPORT_MASK, FALSE);
							}
						}
					}		
					else
					{
						if(vid == iptv_vid && (mcast_vid > 1 && mcast_vid < 4095))
						{
							skb2= skb_clone(skb, GFP_ATOMIC);
							if(skb2)
							{
								re865x_relayTrappedMCast(skb2, mcast_vid, 
									rtk_get_vlan_portmask(mcast_vid) & RTL_WANPORT_MASK, FALSE);
							}
						}
					}
				}
		...
		if(ipv6h!=NULL)
		{
			...
			if((l4Protocol==IPPROTO_UDP) || (l4Protocol==IPPROTO_TCP)|| (l4Protocol==Any_0_hop_protocl))
			{
				...
				if(nicIgmpModuleIndex!=0xFFFFFFFF)
				{
					...
					ret=rtl_getMulticastDataFwdInfo(nicIgmpModuleIndex, &multicastDataInfo, &nicMCastFwdInfo);
              	#ifndef CONFIG_RTL_VLAN_8021Q
					//when processing igmp, vlanRelayPortMask don't compare with cp_this->portmask, why?
					vlanRelayPortMask = vlanFloodingPortMask & nicMCastFwdInfo.fwdPortMask &cp_this->portmask;
             	#else
                    vlanRelayPortMask = vlanFloodingPortMask & nicMCastFwdInfo.fwdPortMask;
               	#endif
...
static inline int32 rtl_isWanDev(struct dev_priv *cp)
{
	...
	return (!cp->vlan_setting.is_lan);
		...
		{
			if(cp->portmask & RTL_WANPORT_MASK)
            	return TRUE;
			else
				return FALSE;
		} else
...
#if !defined(CONFIG_RTL_MULTI_LAN_DEV)
//#define RTL_RELAY_PKT_WITH_IP_OPTION 1
#endif

#if defined(RTL_RELAY_PKT_WITH_IP_OPTION)
static inline struct iphdr *rtl_get_ipv4_hdr(uint8 *mac_hdr)
{
	uint8 *ptr;
 
	ptr = mac_hdr + 12;
	if(*(uint16 *)ptr == htons(ETH_P_8021Q))
		ptr += 4;

	if(*(uint16 *)ptr == htons(ETH_P_IP))
		return (struct iphdr *)(ptr + 2);

	return NULL;
}

static inline void rtl_relay_pkt_with_ip_option(struct sk_buff *skb, struct dev_priv *cp_this, unsigned int vid, unsigned int pid, unsigned int keepOrigSkb)
{
		rtl_nicTx_info	 nicTx;
		struct iphdr *iph = NULL;
		struct sk_buff *skb2 = NULL;	
	
		iph = rtl_get_ipv4_hdr(skb->data);
	
		if ((iph==NULL) ||(iph->ihl<=0x5) ||(rtl_isWanDev(cp_this)))
			return;
		...
		if (skb2 != NULL)
		{
			nicTx.txIdx = 0;
			nicTx.vid = vid;
			nicTx.portlist = (cp_this->portmask) & (~((1<<pid)|0x100)) & RTL_LANPORT_MASK;
			...
			if (linux_vlan_enable)	 
			{
				nicTx.portlist = rtk_get_vlan_portmask(vid) & (~((1<<pid)|0x100));
				if ((*((unsigned short *)(skb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021Q)))
				{
					if (skb_cloned(skb2))
					{		
						struct sk_buff *newskb = NULL;
						newskb = skb_copy(skb2, GFP_ATOMIC);
						...
						{
							printk("%s[%d], Warning: skb copy failed!\n", __FUNCTION__, __LINE__);
						}
					}
			...
			printk("%s[%d], vid is %d, portlist is 0x%x, pid is %d\n", __FUNCTION__, __LINE__, vid, nicTx.portlist, pid);
			RTL_CACHE_WBACK((unsigned long)skb2->data, skb2->len);
#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
			rtl_map_ring_by_port(&nicTx);
#endif
...
static inline int32 rtl_decideRxDevice(rtl_nicRx_info *info)
{
	...
	#endif
	{
		...
		{
		#if defined (CONFIG_RTL_EXT_PORT_SUPPORT)
			/*	when extPortEnabled and wifi->ether hw fail, packets may end up trapped to CPU again (wifi rx -> nic tx -> nic rx).
				this time of nic rx, source port of packet will be extension port.
				here assign this kind of packets with device eth0 for further recover in rtl_fwdFromExtPort().	*/
			if(extPortEnabled && info->ext_pid && info->vid==PKTHDR_EXTPORT_MAGIC4){
				cp = ((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[0]));
				info->priv = cp;
				return SUCCESS;
			}
		#endif
...
int rtl_add_Acl_for_dns_trap(struct rtl865x_vlanConfig* vlanConfig)
{
	...
	for(i=0; pVlanConfig[i].vid != 0; i++)
	{
		...
		if(pVlanConfig[i].isWan==0)/*lan config*/
		{
			...
			rule.udpSrcPortUB_ = 0xffff;
			rule.udpSrcPortLB_ = 0;
			rule.udpDstPortUB_ = 53;
			rule.udpDstPortLB_ = 53;
		#if defined(TRAP_ONLY_ETH)
			rule.ipv6Combine_             = 1;
		#endif
			...
			// 0.0.0.0 - mask 255.255.255.255 refers to IPv6 packets
			rule.srcIpAddr_					= 0x0;
...
#ifdef CONFIG_IPV6
struct udphdr *ipv6_find_udp_hdr_v2(struct sk_buff *skb, struct udphdr *udph)
{
	int ret=0;
	unsigned int uhoff=0;
	int offset_bak=0;
	//struct udphdr udph_tmp_find,*udph;
	//struct udphdr udph_tmp_find;
	int udp_target=17;
...
inline int is_skb_dns_packet(struct sk_buff* skb)
{
	...
	int ret=0, version_type=0;
#ifdef CONFIG_IPV6
	struct udphdr udph_temp;
	struct ipv6hdr *ipv6h=NULL;
	unsigned int ipv6_offest=0;
#endif	
	if(!skb)
		return 0;
	...
	else if(ptr && (*(uint16 *)(ptr) == htons(ETH_P_IP)))
	{
		version_type=4;
	}
#ifdef CONFIG_IPV6
	else if(ptr && (*(uint16 *)(ptr) == htons(ETH_P_IPV6)))
	{
		version_type=6;
	} 
#endif
	...
	if(iph && iph->version == 4 && version_type == 4){ // skb is ipv4
		udph = (void *)iph + iph->ihl*4;
		if(iph->protocol==IPPROTO_UDP){
			if (udph!=NULL && (udph->dest == htons(53))){
				//skb->is_dns_pkt = 1;
				//printk("%s %d is dns packets\n", __func__, __LINE__);
				return 1;
			}
...
#ifdef CONFIG_IPV6	
	else if(ipv6h && ipv6h->version == 6 && version_type ==6){	// skb is ipv6
		ipv6_offest = ptr+2 - (uint8 *)skb->data;
		skb_pull(skb, ipv6_offest);
		
		udph = (void *)ipv6_find_udp_hdr_v2(skb,&udph_temp);
		
		if (udph == NULL) // find udp header fail
		{			
			//printk("%s %d is after skb_push1 sdk->data=%p \n",  __func__, __LINE__, skb->data);
			ret=0;
		}			
		else if (udph->dest == htons(53)) {
			//printk("%s %d is ipv6 dns packets\n", __func__, __LINE__);
			//skb->is_dns_pkt = 1;
			ret=1;
		}
		
		skb_push(skb, ipv6_offest);
...
#if defined(CONFIG_RTW_BRSC) || defined(CONFIG_RTL_EXT_PORT_SUPPORT)
int check_br_port_ready(struct net_device *dev)
{
#ifdef __KERNEL__
...
EXPORT_SYMBOL(check_br_port_ready);
#endif

#if defined(BR_SHORTCUT) || defined(CONFIG_RTW_BRSC)
#ifdef CONFIG_RTW_BRSC
...
int rtl_IsMcastIP(struct sk_buff *pskb)
{
	...
	if( L3_protocol ==__constant_htons(ETH_P_8021Q)) {
		L3_protocol = *(unsigned short *)(pskb->data+ETH_ALEN*2 + 4);
		vlanTag = 4;
	}
	if( L3_protocol == __constant_htons(0x0800) )
	{
		unsigned int DaIpAddr;
		struct iphdr* iph = (struct iphdr *)(pskb->data + ETH_HLEN + vlanTag);
	...
	{
		struct ipv6hdr *iph;
		iph  = (struct ipv6hdr *)(pskb->data + ETH_HLEN + vlanTag);
...
int rtw_brsc_should_go(struct net_device *dev, struct sk_buff *pskb)
{
...
#if 0
	if(!check_br_port_ready(dev))
		return 0;

	/*update wifi side sc entry*/
	rtw_update_brsc_info(pskb->data + ETH_ALEN, dev);
#endif
	...
	if((pskb->data[0] & 0x1) || rtl_IsMcastIP(pskb))
		return 0;
...
EXPORT_SYMBOL(rtw_brsc_should_go);
#endif
...
static inline int32 rtl_processBridgeShortCut(struct sk_buff *skb, struct dev_priv *cp_this, rtl_nicRx_info *info)
{
	...
	if(rtw_brsc_is_black_dev(cp_this->dev->name))
		return FAILED;
#endif
...
#ifdef CONFIG_RTW_BRSC
	rtw_update_brsc_info(skb->data+ETH_ALEN, cp_this->dev);
#endif
...
#endif
	#if defined(CONFIG_WIRELESS_LAN_MODULE) && !defined(CONFIG_TP_IMAGE) /* Follow the demo,  CONFIG_RTW_BRSC */
		(wirelessnet_hook_shortcut !=NULL ) && ((dev = wirelessnet_hook_shortcut(skb->data)) != NULL)
	#elif defined(CONFIG_RTW_BRSC)
		(((dev = rtw_get_brsc_dev(skb->data)) != NULL) && (dev != cp_this->dev ))
	#else
	  #if defined(CONFIG_RTL_ULINKER_BRSC)
		(((dev=brsc_get_cached_dev(0, skb->data))!=NULL) || ((dev = get_shortcut_dev(skb->data)) != NULL)) 
	  #else
		((dev = rtl_get_shortcut_dev(skb->data)) != NULL)
	  #endif
	#endif
    #ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
...
#if defined (CONFIG_RTL_EXT_PORT_SUPPORT)
extern struct net_device * rtw_get_brsc_dev(unsigned char *dmac);
...
int rtl_wifi_fdb_add_hook(const unsigned char *addr)
{
	int ret=0;
#if  defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT)
...
int rtl_wifi_fdb_del_hook(const unsigned char *addr)
{
	int ret=0;
#if  defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT)
...
EXPORT_SYMBOL(rtl_wifi_fdb_add_hook);
EXPORT_SYMBOL(rtl_wifi_fdb_del_hook);


inline struct iphdr * rtl_get_ipv4_header(uint8 *macFrame)
{
	uint8 *ptr;
	struct iphdr *iph=NULL;
...
int rtl_wlanFwdToEth(struct sk_buff *skb)
{
	struct net_device *eth_dev = NULL;
	struct net_device *ori_wlan_dev = NULL;

	if(!extPortEnabled || !skb)
		return EXT_RX_CONTINUE;
		
	if (rtl_wlan_frame_use_hw_nat(skb->data)==SUCCESS){
		
		#if defined(CONFIG_RTL_REFINE_MULTI_LAN_DEV)
		if ((eth_dev = __dev_get_by_name(&init_net, RTL_DRV_LAN_P0_ALIAS_NETIF_NAME)) != NULL) 
		#else
		if ((eth_dev = __dev_get_by_name(&init_net, "eth0")) != NULL) 
		#endif
		{
			if (skb->dev){
				ori_wlan_dev = skb->dev;
			}
			else
				ori_wlan_dev = __dev_get_by_name(&init_net, WLAN_NAME_NUM(WLAN_BASE_NAME,0));

			skb->dev = eth_dev;
			#if defined(CONFIG_COMPAT_NET_DEV_OPS)
			eth_dev->hard_start_xmit(skb, ori_wlan_dev);
			#else
			eth_dev->netdev_ops->ndo_start_xmit(skb, ori_wlan_dev);
			#endif
...
EXPORT_SYMBOL(rtl_wlanFwdToEth);
...
static int32 rtl_fwdFromExtPort(struct sk_buff *skb, struct dev_priv *cp_this, rtl_nicRx_info *info)
{
	...
	if(ret==SUCCESS)
	{
		if(1)//(info->pid==PKTHDR_EXTPORT_P3)
		{
			...
			ori_wlan_dev = rtw_get_brsc_dev(skb->data+ETH_ALEN);
			if(ori_wlan_dev)
			{
...
static inline void rtl_processRxFrame(rtl_nicRx_info *info)
{
...
#if 0 /* FIXME: Add Macro Definition */
	/* record vid to skb */
	if(vid)
	{
		skb->vlan_proto = htons(ETH_P_8021Q);
		skb->vlan_tci = VLAN_TAG_PRESENT | vid;
	}
#endif

#if 0 /* Debug , TODO: Export proc interface for debugging */
	if(printk_ratelimit())
	{
		if (skb && skb->data)
		{
			...
			{
				buf = (char *)(skb_mac_header(skb));
				printk("RX. len = %#4x, info_vid=%d, tci=0x%x\n", len, info->vid, skb->vlan_tci);
			}
		 	else
		 	{
		 		printk("RX len = %#4x, info_vid=%d, tci=0x%x\n", len, info->vid, skb->vlan_tci);
	...
	{
		uint16 vid_tmp = 0;
		rtl819x_getSwEthPvid(pid, &vid_tmp);
		skb->srcVlanId = vid_tmp&0xfff;
	}
#endif
#if defined (CONFIG_RTL_EXT_PORT_SUPPORT)
    ...
    if(linux_vlan_enable && linux_vlan_hw_support_enable)
    {
		...
		#elif defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
		if (multi_phy_vir_wan_enable) {
			ret = phy_multi_wan_rx(pid,skb,info);
			if(ret == 1)
        	{
            	data = skb->data;
...
#if !defined(CONFIG_VLAN_8021Q_EASYMESH_R2_SUPPORT)
...
#if defined(RTL_RELAY_PKT_WITH_IP_OPTION)
#if defined(CONFIG_RTL_PROCESS_PPPOE_IGMP_FOR_BRIDGE_FORWARD)
	if (((0==(data[0]&0x01))&&(type==0)) ||(0xff==data[0]))
#else
	if ((0==(data[0]&0x01)) ||(0xff==data[0]))
#endif
		rtl_relay_pkt_with_ip_option(skb, cp_this, vid, pid, TRUE);	/*jwj: The skb may access dut, so keepOrigSkb=TRUE.*/
#endif
...
#ifdef CONFIG_TP_BRIDGE
		if(suppress_mode && info->vid == rtk_vlan_hw_nat_lan_vid)
		{
			hyfi_loopback_hook_t *loopback_hook = rcu_dereference(hyfi_loopback_hook);
			if (loopback_hook)
			{
				if (loopback_hook(skb, NULL, NULL) == NF_DROP)
				{
...
__IRAM_FWD static void interrupt_dsr_rx_done(rtlInterruptRxData *rxData)
{
	...
	/*rtl_rx_tasklet_running=0 must be placed before rtl_runout_resume*/
...
#if !defined(CONFIG_RTW_MEMPOOL) && !defined(CONFIG_RTL_ETH_PRIV_SKB)
		rtl_runout_resume();
#endif
	}
#else
...
#ifdef CONFIG_TP_IMAGE
static uint32 last_wan_down_time = 0;
#endif
int rtk_get_port_change_time(uint32 curLink, uint32 newLink)
{
	uint32 port;

	for (port=PHY0; port<=PHY4; port++)
	{
		if ((curLink & (1<<port)) != (newLink & (1<<port)))
		{
			...
			if( port == RTL83XX_WAN )
			{
				...
				if (wan_dev)
				{
					if(!(curLink & (1<<RTL83XX_WAN)))
					{
						netif_carrier_on(wan_dev);
						printk("eth1: link is up\n");
					}
					else
					{
						if((port_change_time[RTL83XX_WAN] - last_wan_down_time) >= 5)
						{
							if ((port_link_status & (1 << RTL83XX_WAN)) == 0)
							{
								netif_carrier_off(wan_dev);
								printk("eth1: link is down\n");
							}
							last_wan_down_time = port_change_time[RTL83XX_WAN];
						}
					}
					dev_put(wan_dev);
				}
...
inline void link_enhancement(void)
{
	...
	#ifdef CONFIG_RTL_83XX_API_V1_4
	/* 8367C "auto duplex change" mechanism is done by Switch 8051, it needs cooperated with reading counter by SoC
	 * 8367D "auto duplex change" mechanism is done by hardware, rtk_refinePortDuplexMode() is no need
	 */
	if (chip_type != 4) // 4: 8367D
	#endif
...
#define BIT100H 	(1<<7)
#define BIT100F 	(1<<8)
#define BITTXPAUSE 	(1<<6)
#define BITRXPAUSE 	(1<<7)
...
static void ckeck_link_speed(void){
	...
	if (regData & (1<<2) && !(prelink & (1<<2)) ){
		...
		// flow control
		rtl_mdio_write(PORT0_RGMII_PHYID, 31 ,0xa43);
		rtl_mdio_read(PORT0_RGMII_PHYID, 0x1a, &regData1);
		rtl_mdio_write(PORT0_RGMII_PHYID, 31 , 0);

		regData1 = (regData1 & (BITTXPAUSE | BITRXPAUSE)) >> 6;

		switch(regData1) {
		case 0:
			REG32(PCRP0) = (REG32(PCRP0) & ~PauseFlowControl_MASK);
			break;
		case 1:
			REG32(PCRP0) = (REG32(PCRP0) & ~PauseFlowControl_MASK) | PauseFlowControlEtxDrx;
			break;
		case 2:
			REG32(PCRP0) = (REG32(PCRP0) & ~PauseFlowControl_MASK) | PauseFlowControlDtxErx;
			break;
		case 3:
			REG32(PCRP0) = (REG32(PCRP0) | PauseFlowControlEtxErx);
			break;
		default:
			break;
		}
...
#ifdef RTL_DYM_ADJ_RING_FIFO_MAP
void rtl_adj_ring_fifo_map(void)
{
	uint8 i, linkup_port=0, linkup_portmask=0;
	uint8 traffic_port=0, traffic_portmask=0, fifo_idx, ring_idx;
	uint32 ring_to_fifo;
	uint64 tx_tp;

	for (i=0; i<RTL8651_PHY_NUMBER; i++) {
		if (RTL_WANPORT_MASK != (1<<i)) // exclude wan port
	...
	if (linkup_port >= 2) { // 2, 3, or 4; exclude 0 or 1
		for (i=0; i<RTL8651_PHY_NUMBER; i++) {
			if (linkup_portmask & (1<<i)) { // exclude link down port

				tx_tp = rtl865xC_returnAsicCounter64(OFFSET_IFOUTOCTETS_P0+(i*MIB_ADDROFFSETBYPORT));
			
				if (tx_tp > rf_port_tx_bytes_bak[i]) {
					if (((tx_tp-rf_port_tx_bytes_bak[i])>>18) > rf_tp_threshold) { 
						// ">>18" = ">>20" / 2 * 8
						// 		">>20" => change to M-byte; "/2" => divide by 2(seconds); "*8" => change to M-bit
						traffic_port++;
						traffic_portmask |= (1<<i);
					}
				}
				rf_port_tx_bytes_bak[i] = tx_tp;
			}
		}

		if ((traffic_port==2) || (traffic_port==3)) {
			if (rf_adjusted && (rf_saved_traffic_portmask != traffic_portmask))
				rf_adjusted = 0;

			if (rf_adjusted == 0) {
				/* this is an easy way, forget the original "ring to fifo" setting, just re-set it
					reg: TXRINGCR
						n.19-n.18	cf_txring3_to_fifo
						n.17-n.16	cf_txring2_to_fifo
						n.15-n.14	cf_txring1_to_fifo
						n.13-n.12	cf_txring0_to_fifo
					rule:
						traffic_port==2
							Ex: port_to_ring result is ring 1 and 2
								set cf_txring1_to_fifo to fifo_2
								set cf_txring2_to_fifo to fifo_1
								cf_txring3_to_fifo and cf_txring0_to_fifo = fifo_0
						traffic_port==3
							Ex: port_to_ring result is ring 2, 3 and 0
								set cf_txring2_to_fifo to fifo_2
								set cf_txring3_to_fifo to fifo_1
								set cf_txring0_to_fifo to fifo_0
								cf_txring1_to_fifo = fifo_0								
				 */
				fifo_idx = 2;
				ring_to_fifo = 0;
				for (i=0; i<RTL8651_PHY_NUMBER; i++) {
					if (traffic_portmask & (1<<i)) {
						ring_idx = rf_port_to_ring[i];
						ring_to_fifo |= (fifo_idx << (ring_idx * 2));
						fifo_idx--;
					}
				}		 
				REG32(TXRINGCR) = (REG32(TXRINGCR) & ~(0xFF<<CF_TXRING0_TO_FIFO_OFT)) | (ring_to_fifo<<CF_TXRING0_TO_FIFO_OFT);
				rf_adjusted = 1;
			}
		}
		rf_saved_traffic_portmask = traffic_portmask;
	}
	return;
}
#endif
...
#define panic_printk         printk
	...
	#if defined(CONFIG_TR181_ETH)
	rtk_get_port_change_time(curLinkPortMask, newLinkPortMask);
	#endif
...
#ifdef RTL_DYM_ADJ_RING_FIFO_MAP
	if (rf_1sec_toggle)
		rtl_adj_ring_fifo_map(); // do it every 2 seconds
	rf_1sec_toggle ^= 1;
#endif
	...
	if (++one_second_counter >= 10) {
		...
		{
			suppress_mode = false;
			printk("suppress_mode\n");
		}

		if (atomic_read(&loop_pkt_cnt))
		{
			atomic_set(&loop_pkt_cnt, 0);
		}
		one_second_counter = 0;
	}
#endif
...
static void do_one_sec_timer_work(struct work_struct *work)
{
	one_sec_timer_priv *priv = container_of(work, one_sec_timer_priv, one_sec_timer_work);
	if (unlikely(priv == NULL))
		printk_ratelimited(KERN_WARNING "warning: can't call one_sec_timer with a null pointer !\n");
	else
		one_sec_timer(priv->task_priv);
}
static void rtl_one_sec_timer(unsigned long task_priv)
{
	// get dev_priv, which is used for restart the timer
	rtl_one_sec_timer_priv.task_priv = task_priv;
	// let workqueue finish the BH, which is in process context
	schedule_work(&rtl_one_sec_timer_priv.one_sec_timer_work);
}
#endif
...
static int re865x_open (struct net_device *dev)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0)
		rc = request_irq(dev->irq, interrupt_isr, 0, dev->name, dev);
#else
	...
	if (dev->name[3] == '0')
		...
		if (atomic_read(&rtl_timer_id) == 0)
		{
			atomic_set(&rtl_timer_id, dev->name[3]);
			#ifdef CONFIG_RTL_ADAPTABLE_TSO
				int window;     
			...
			cp->expire_timer.function = rtl_one_sec_timer;
			mod_timer(&cp->expire_timer, jiffies + HZ);
#ifdef DYNAMIC_ADJUST_TASKLET
			rx_cnt = 0;
#endif
			INIT_WORK(&rtl_one_sec_timer_priv.one_sec_timer_work, do_one_sec_timer_work);
...
#else
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
		rtl865x_addRoute(0,0,0,RTL_DRV_BR_WAN_NETIF_NAME,0);
#else
...
//#if defined(CONFIG_RTL_HARDWARE_IPV6_SUPPORT)
	...
	if (rtl865x_curOpMode == GATEWAY_MODE) {
		...
		#else
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
		rtl8198c_addIpv6Route(ipAddr, 0, nexthop, RTL_DRV_BR_WAN_NETIF_NAME);
#else
...
static int re865x_close (struct net_device *dev)
{
...
#if !defined(CONFIG_RTL_8196C)
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
		if(strcmp(dev->name, RTL_DRV_BR_WAN_NETIF_NAME) != 0)    /*for lan dhcp client to renew ip address*/
#else
		if(strcmp(dev->name, RTL_DRV_WAN0_NETIF_NAME) != 0)    /*for lan dhcp client to renew ip address*/
#endif
	...
	if (atomic_read(&rtl_timer_id) == dev->name[3])
	{
		atomic_set(&rtl_timer_id, 0);
	}
#endif
...
int rtl_process_vlan_passthrough_tx(struct sk_buff **ppskb)
{
	...
	{	
		...
		if (skb_cloned(skb))
		{		
			#if 0//defined(CONFIG_RTL_ETH_PRIV_SKB)
			newskb = priv_skb_copy(skb);
			#else
			newskb = skb_copy(skb, GFP_ATOMIC);
			#endif
			...
			if (newskb == NULL)
			{
				panic_printk("rtl_process_vlan_passthrough_tx skb_copy failed\n");
				cp->net_stats.tx_dropped++;
				dev_kfree_skb_any(skb);
				*ppskb = NULL;
				return FAILED;
...
#if !defined(CONFIG_RTL_8367R_SUPPORT)
    if(mirrorPort_enable==1 && (txInfo->portlist & mirrorPort_txmask))
	{
		txInfo->portlist |= mirrorPort_mask;
    }
#endif	
...
static inline int rtl_ip_option_check(struct sk_buff *skb)
{
	...
	if (((*((unsigned short *)(skb->data+ETH_ALEN*2)) == __constant_htons(ETH_P_IP))
		...
		(*((unsigned short *)(skb->data+ETH_ALEN*2+4)) == __constant_htons(ETH_P_IP))&&
		((*((unsigned char*)(skb->data+ETH_ALEN*2+6)) != 0x45))))		
			flag = TRUE;
	return flag;
}
...
//#if defined (CONFIG_RTL_MULTI_LAN_DEV) ||defined (CONFIG_POCKET_ROUTER_SUPPORT)
	...
	*portlist = cp->portmask;
		...
		if (rtl_isWanDev(cp)!=TRUE) {	// LAN interface
			...
			if (memcmp(skb->data, hw_csum_cache_mac, 6) == 0) {
				*portlist = hw_csum_cache_port;				
				...
				portmask = rtl865x_getPortMaskByMac(skb->data, 0);
				if (portmask != -1) {
					*portlist = portmask;
	...
	if(multi_phy_vir_wan_enable)
	{
		if((cp->portmask&RTL_LANPORT_MASK)&&((skb->srcPhyPort) > 4 && (skb->srcPhyPort)!=0xFF)&& (rtl_ip_option_check(skb) != TRUE))  // wlan->lan UC: HW lookup 
			return TRUE;
	...
	#endif
	{
		...
		if(mirrorPort_enable == 1 && (rtl_isWanDev(cp)==TRUE) && (rtl_ip_option_check(skb) != TRUE)) //not 8367R+mirrorport, wan UC tx use HW
		{
			flag = TRUE;
		}
		#endif
...
static inline int rtl_fill_txInfo(rtl_nicTx_info *txInfo)
{
    ...
    #ifdef  CONFIG_RTL_VLAN_8021Q
    if(vlan_ctl_p->flag) /* FIXME: always open vlan */
    {
        uint16 skb_vid=0;
		...
		#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)
		//packet is sent from peth0, only need add it to passthru vlan group(vid:2)
		if((SUCCESS==rtl_isPassthruFrame(skb->data))&&(!skb_vid))
		{				
			#if 0
			if(skb_vid)
			{
				if(remove_vlan_tag((struct sk_buff **)(&skb))==FAILED)
					return FAILED;
			}
			#endif
			
			txInfo->out_skb = skb;			
			txInfo->vid = PASSTHRU_VLAN_ID;
		}
		#endif
    }
    else
...
#if defined(CONFIG_RTL_DNS_TRAP)
	{
		uint16 tx_dns_vid = cp->id;
		#if defined(CONFIG_RTL_VLAN_8021Q)
		tx_dns_vid = txInfo->vid;
		#endif
		//uint32 portlist_temp;
		check_dns_pkt_tx(tx_dns_vid, skb, cp, (uint32 *)&txInfo->portlist);
	}
#endif
...
#if defined(CONFIG_RTL_HARDWARE_MULTICAST_API)
int rtl_process_hw_multicat_tx(struct sk_buff *skb)
{
	struct iphdr *iph = NULL;
#if defined(CONFIG_RTL_8198C)||defined(CONFIG_RTL_8197F)
	struct ipv6hdr *ipv6h = NULL;
#endif
	int32 l4Protocol = 0;
	uint32 groupAddr[4] = {0}, srcAddr[4]={0};
		...
		if(iph)
		{
			...
			if((l4Protocol==IPPROTO_UDP) || (l4Protocol==IPPROTO_TCP) )
			{
				if((skb->srcVlanId!=0) && (skb->srcPort!=0xFFFF))
				{
					...
					groupAddr[0] = ntohl(iph->daddr);
					srcAddr[0] = ntohl(iph->saddr);
					rtl_kernelMCastHwAccelerateV4(skb->srcPort,skb->srcVlanId,srcAddr[0],groupAddr[0]);
				}
			}
		}
	}
	...
	else if ((skb->data[0]==0x33) && (skb->data[1]==0x33) && (skb->data[2]!=0xff))
	{
		...
		if(ipv6h!=NULL)
		{
			...
			if((l4Protocol==IPPROTO_UDP) || (l4Protocol==IPPROTO_TCP))
			{
				...
				{
					/*from eth*/
					groupAddr[0] = ntohl(ipv6h->daddr.s6_addr32[0]);
					groupAddr[1] = ntohl(ipv6h->daddr.s6_addr32[1]);
					groupAddr[2] = ntohl(ipv6h->daddr.s6_addr32[2]);
					groupAddr[3] = ntohl(ipv6h->daddr.s6_addr32[3]);
					srcAddr[0] = ntohl(ipv6h->saddr.s6_addr32[0]);
					srcAddr[1] = ntohl(ipv6h->saddr.s6_addr32[1]);
					srcAddr[2] = ntohl(ipv6h->saddr.s6_addr32[2]);
					srcAddr[3] = ntohl(ipv6h->saddr.s6_addr32[3]);
					rtl_kernelMCastHwAccelerateV6(skb->srcPort,skb->srcVlanId,srcAddr,groupAddr);
				}				
			}
		}
	}
...
static int re865x_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	cp = NETDRV_PRIV(dev);
		#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)
	if ((strncmp(skb->dev->name, "peth", 4)!=0)&&((cp->id==0) || 
	#if defined(CONFIG_RTL_REFINE_MULTI_LAN_DEV)
...
//call extract_magicNum() in rtl_fp_dev_queue_xmit_check()
...
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
	if (dev && (strncmp(dev->name, WLAN_BASE_NAME, strlen(WLAN_BASE_NAME))==0))
		nicTx.wFlags = 1;
	else
		nicTx.wFlags = 0;
#endif
	...
	#if 0//!defined(CONFIG_RTL_8367R_SUPPORT)
	if(mirrorPort_enable==1 && (cp->portmask & RTL_WANPORT_MASK)&&(nicTx.flags & PKTHDR_HWLOOKUP))
	{
		skb = nicTx.out_skb;
		if(rtk_get_skb_vid(skb)==0)
			tx_skb = nicTx.out_skb = insert_vlan_tag(skb, nicTx.vid);
	}
	#endif
	...
	#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)  //wlan->lan:hw lookup need insert vlan tag
if(multi_phy_vir_wan_enable)
{
    if(linux_vlan_enable && (nicTx.flags & PKTHDR_HWLOOKUP))
    {
       skb = nicTx.out_skb;
	   nicTx.vid =  rtk_get_vid_by_port(ffs(cp->portmask) - (1));   //insert port's pvid, not cp->id
	   	   
	   if(insert_vlan_tag2(nicTx.vid, &skb)==FAILED)
	   {
	   		#if defined(CONFIG_SMP)
			SMP_UNLOCK_ETH_XMIT(flags);
			#endif
	...
	if(skb->srcPhyPort>PHY4 &&skb->srcPhyPort!=0xff
		&&(!(skb->data[0]&0x1))&&rtl_isWanDev( NETDRV_PRIV(skb->dev))&&(SUCCESS == rtl_isPassthruFrame(skb->data)))
	{
...
#endif
	{
		if(rtl865x_curOpMode == GATEWAY_MODE)
		{
			#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
			if (multi_phy_vir_wan_enable) {
				if ((nicTx.portlist&0x1f) & RTL_WANPORT_MASK) &&(rtk_get_vlan_tagmask(nicTx.vid) & RTL_WANPORT_MASK))
				{
					uint8 priority = 0;
					//rtk_insert_vlan_tag(nicTx.vid, tx_skb);
					skb = nicTx.out_skb;
					priority = 0x7 &  rtk_get_vlan_priority(nicTx.vid);
					if(rtk_insert_vlan_tag_and_priority(nicTx.vid, priority, (struct sk_buff **)(&skb))==FAILED)
					{
						#if defined(CONFIG_SMP)
						SMP_UNLOCK_ETH_XMIT(flags);
						#endif
	...
	if (tx_skb->len < 60) 
	{
		...
		if (skb_cloned(tx_skb))
		{		
			struct sk_buff *newskb = NULL;
			pending_flag = 0;
			newskb = skb_copy(tx_skb, GFP_ATOMIC);
			
			if (newskb != NULL)
			{
				pending_flag = 1;
				dev_kfree_skb_any(tx_skb);
				tx_skb = nicTx.out_skb = newskb;
			}
		}

		if (pending_flag == 1) 
		{
			memset(tx_skb->data+tx_skb->len, 0, (64-tx_skb->len));
			tx_skb->len = 60;
		}
	}
	
#if defined(CONFIG_RTL_HARDWARE_MULTICAST_API)
	rtl_process_hw_multicat_tx(tx_skb);
#endif
...
#if defined(CONFIG_RTL_SWITCH_NEW_DESCRIPTOR)
	#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
		if(memcmp(last_mac, (tx_skb->data), 12))
		{
			rtl_map_ring_by_L2_port(tx_skb, &nicTx);
			memcpy(last_mac, (tx_skb->data), 12);
			last_ring_idx=nicTx.txIdx;
		} else {
			nicTx.txIdx=last_ring_idx;
		}
	#endif
...
int re865x_ioctl (struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	switch (args[0])
	{
#if defined (CONFIG_RTL_SUPPORT_STATISTICS_LAN)
		case RTL8651_IOCTL_GETALLPORTSTATUS:
		{
#if 0
			#define DEBUG(fmt, args...) \
			{\
				printk(KERN_WARNING"[%s][line:%d][%s] "fmt"\n","rtl_nic.c",__LINE__,__FUNCTION__,##args);\
			}
#endif
			...
			typedef struct lan_port_raw_trx_statistic_with_mac
			{
				uint8 mac[ETH_ALEN];
				uint64 tx_bytes; 
				uint64 rx_bytes;
			}lan_port_trx_statistic_with_mac_t;

			lan_port_trx_statistic_with_mac_t tmp[RTL8651_PHY_NUMBER];
			...
			for(i = 0; i < RTL8651_ARPTBL_SIZE; i++)
			{
				...
				for(j = 0; j < RTL8651_ROUTINGTBL_SIZE; j++)
				{
					...
					for(port = 0; port < RTL8651_PHY_NUMBER; port++)
					{
						...
						{
							memcpy(tmp[port].mac, asic_l2.macAddr.octet, ETH_ALEN);
						}
					}
				}	
			}
#if 0
			printk(KERN_DEBUG"============ lan ports info ============\n");
#endif
			for (i = 0; i < RTL8651_PHY_NUMBER; ++i)
			{
				rtk_stat_port_get(i, STAT_IfInOctets, &tmp[i].rx_bytes);
				rtk_stat_port_get(i, STAT_IfOutOctets, &tmp[i].tx_bytes);
#if 0
				printk(KERN_DEBUG"port:%u\nrx_bytes: %llu\ntx_bytes: %llu\n"
					"mac: %2x:%2x:%2x:%2x:%2x:%2x\n\n",
					i, tmp[i].rx_bytes, tmp[i].tx_bytes, 
...
#endif
			/* user_cmd_data.buf */
			if (copy_to_user(((ioctl_data_t *)rq->ifr_data)->buf, (void *)tmp, sizeof(tmp)))
			{	
				return -EFAULT;
			}
			pRet = (int32 *)args[3]; /* 用户用于接收返回值的变量ret的地址 */
			rc = SUCCESS;
			*pRet = RTL8651_PHY_NUMBER; /* unsigned int => int32 */
			break;
		}
#endif
...
int rtl865x_set_hwaddr(struct net_device *dev, void *addr)
{
	...
	#if defined(RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
	struct dev_priv *cp = NETDRV_PRIV(dev);
	#endif
	char drvName[MAX_IFNAMESIZE];
...
#if defined(RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
	if(cp) { 
		if (rtl_isWanDev(cp) == 0)
		{	
			strncpy(drvName, RTL_DRV_LAN_NETIF_NAME, MAX_IFNAMESIZE);
		}
		else
		{  
			strncpy(drvName, dev->name, MAX_IFNAMESIZE);
			if((strncmp(dev->name, "eth0", 4)==0)&&(strncmp(hw_nat_netif_name, dev->name, 4)==0))
				strncpy(drvName, RTL_DRV_LAN_NETIF_NAME, MAX_IFNAMESIZE);
		}
	}
#else
	strncpy(drvName, mapp_entry->drvName, MAX_IFNAMESIZE);
#endif

	vlancfg_entry = rtl_get_vlanconfig_by_netif_name(drvName);
	...
	if(vlancfg_entry->vid != 0)
	{
		...
		#if defined(RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)		
		if(strncmp(hw_nat_netif_name, dev->name, 4)==0) //when set hw_nat_netif's wan, update hw_nat_netif's MAC
		{	
			memcpy(netif.name,hw_nat_netif_name,MAX_IFNAMESIZE);
		}
		#endif
...
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
		if(!strncmp(dev->name, RTL_DRV_BR_WAN_NETIF_NAME, MAX_IFNAMESIZE))
#else
		if(!strncmp(dev->name, RTL_DRV_WAN0_NETIF_NAME, MAX_IFNAMESIZE))
#endif
			wan_dev_mtu = new_mtu;
...
static int rtl865x_addAclForIPV6ReadyLogo(struct rtl865x_vlanConfig* vlanConfig)
{
	...
	for(i=0; pVlanConfig[i].vid != 0; i++)
	{
		...
		else/*wan config*/
		{
			...
			rule.ruleType_ = RTL865X_ACL_IPV6;
			rule.actionType_		= RTL865X_ACL_TOCPU;
			/* lan pc dhcpv6 cannot get ipv6 address when lan wan bridge
			  * so change pktOpApp_ from RTL865X_ACL_ALL_LAYER to RTL865X_ACL_L3_AND_L4
			*/
...
/*per port l2 learning number limitation*/		
#if defined(CONFIG_RTL_8197F)
int l2_learn_limitation_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, l2_learn_limitation_read,NULL));
}
static ssize_t l2_learn_limitation_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return l2_learn_limitation_write(file, userbuf,count, off);
}

struct file_operations l2_learn_limitation_proc_fops= {
        .open           = l2_learn_limitation_single_open,
        .write		    = l2_learn_limitation_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
static unsigned char br_subnet_from_wan_to_cpu_flag=0;
int rtl_add_Acl_for_br_subnet_from_wan(void);
static int br_subnet_from_wan_to_cpu_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%d\n",br_subnet_from_wan_to_cpu_flag);
	return 0;
}

static int32 br_subnet_from_wan_to_cpu_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	char tmpbuf[4];
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		br_subnet_from_wan_to_cpu_flag=tmpbuf[0]-'0';
		if(br_subnet_from_wan_to_cpu_flag)
			rtl_add_Acl_for_br_subnet_from_wan();
	}
	return len;
}

/*trap wan packet which subnet is same with br0 to cpu */
int br_subnet_from_wan_to_cpu_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, br_subnet_from_wan_to_cpu_read, (void *)PDE_DATA(inode)));
}
static ssize_t br_subnet_from_wan_to_cpu_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return br_subnet_from_wan_to_cpu_write(file, userbuf,count, off);
}
struct file_operations br_subnet_from_wan_to_cpu_proc_fops= {
        .open           = br_subnet_from_wan_to_cpu_single_open,
        .write		    = br_subnet_from_wan_to_cpu_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
/*rtl_8367_chip_type*/
int rtl_8367_chip_type_single_open(struct inode *inode, struct file *file)
{
		return(single_open(file, rtl_8367_chip_type_read,NULL));
}
static ssize_t rtl_8367_chip_type_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return 0;
}
struct file_operations rtl_8367_chip_type_proc_fops= {
        .open           = rtl_8367_chip_type_single_open,
        .write		    = rtl_8367_chip_type_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#ifdef CONFIG_TP_IMAGE
/* port link status (port 0~4)*/
static int port_link_status_read(struct seq_file *seq, void *v)
{
	seq_printf(seq, "0x%x\n", curLinkPortMask);
	return 0;
}

static int port_link_status_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, port_link_status_read, NULL));
}
static ssize_t port_link_status_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	char tmp[32];
	
	if (userbuf && !copy_from_user(tmp, userbuf, count))
	{
		int num = sscanf(tmp, "%u", &phy_enable_all);
		if (num == 1) 
		{
			extern rtk_api_ret_t rtk_port_phyEnableAll_set(uint32 enable);
			if(phy_enable_all == 1)
			{
				if(rtk_port_phyEnableAll_set(1) == 0)
				{
					printk("phyEnableAll_set OK\n");
					return count;
				}
			}
			else if(phy_enable_all == 0)
			{
				if(rtk_port_phyEnableAll_set(0) == 0)
				{
					printk("phyEnableAll_set down OK\n");
					return count;
				}
			}
		}
	...
	printk(KERN_ERR"phyEnableAll_set Failed!\n");
	return count;
}
struct file_operations port_link_status_proc_fops= {
        .open           = port_link_status_single_open,
        .write		    = port_link_status_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};

static int wan_led_status_read(struct seq_file *seq, void *v)
{
	rtk_led_force_mode_t mode = LED_FORCE_ON;
	seq_printf(seq, "%d\n", rtk_led_modeForce_get(RTL83XX_WAN - 2, LED_GROUP_0, &mode));
	return 0;
}

static int wan_led_status_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, wan_led_status_read, NULL));
}

static int wan_led_status_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char tmpBuf[4];
	if (buffer && !copy_from_user(tmpBuf, buffer, count))
	{
		if (!strncmp(tmpBuf,"1",1))
		{
			rtk_led_modeForce_set(RTL83XX_WAN - 2, LED_GROUP_0, LED_FORCE_ON);
			return count;
		}else if (!strncmp(tmpBuf,"0",1))
		{
			rtk_led_modeForce_set(RTL83XX_WAN - 2, LED_GROUP_0, LED_FORCE_OFF);
			return count;
		}else
		{
			return -EFAULT;
...
static ssize_t wan_led_status_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return wan_led_status_write(file, userbuf,count, off);
}

struct file_operations wan_led_status_proc_fops= {
        .open           = wan_led_status_single_open,
        .write		    = wan_led_status_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};

static int lan1_led_status_read(struct seq_file *seq, void *v)
{
	rtk_led_force_mode_t mode = LED_FORCE_ON;
	seq_printf(seq, "%d\n", rtk_led_modeForce_get(RTL83XX_LAN1 - 2, LED_GROUP_0, &mode));
	return 0;
}

static int lan1_led_status_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, lan1_led_status_read, NULL));
}

static int lan1_led_status_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char tmpBuf[4];
	if (buffer && !copy_from_user(tmpBuf, buffer, count))
	{
		if (!strncmp(tmpBuf,"1",1))
		{
			rtk_led_modeForce_set(RTL83XX_LAN1 - 2, LED_GROUP_0, LED_FORCE_ON);
			return count;
		}else if (!strncmp(tmpBuf,"0",1))
		{
			rtk_led_modeForce_set(RTL83XX_LAN1 - 2, LED_GROUP_0, LED_FORCE_OFF);
			return count;
		}else
		{
			return -EFAULT;
...
static ssize_t lan1_led_status_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return lan1_led_status_write(file, userbuf,count, off);
}

struct file_operations lan1_led_status_proc_fops= {
        .open           = lan1_led_status_single_open,
        .write		    = lan1_led_status_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
static int lan2_led_status_read(struct seq_file *seq, void *v)
{
	rtk_led_force_mode_t mode = LED_FORCE_ON;
	seq_printf(seq, "%d\n", rtk_led_modeForce_get(RTL83XX_LAN2 - 2, LED_GROUP_0, &mode));
	return 0;
}

static int lan2_led_status_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, lan2_led_status_read, NULL));
}

static int lan2_led_status_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char tmpBuf[4];
	if (buffer && !copy_from_user(tmpBuf, buffer, count))
	{
		if (!strncmp(tmpBuf,"1",1))
		{
			rtk_led_modeForce_set(RTL83XX_LAN2 - 2, LED_GROUP_0, LED_FORCE_ON);
			return count;
		}else if (!strncmp(tmpBuf,"0",1))
		{
			rtk_led_modeForce_set(RTL83XX_LAN2 - 2, LED_GROUP_0, LED_FORCE_OFF);
			return count;
		}else
		{
			return -EFAULT;
...
static ssize_t lan2_led_status_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return lan2_led_status_write(file, userbuf,count, off);
}

struct file_operations lan2_led_status_proc_fops= {
        .open           = lan2_led_status_single_open,
        .write		    = lan2_led_status_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
void show_soc_name(void)
{
#ifdef CONFIG_RTL_8197F
	const static char soc_name[][6] = { "FB", "FH", "FS", "FB-VG", "FH-VG", "FS-VG" };
	uint32 i = rtl819x_bond_option()-1;
	if (IS_8197F_VG()) 
		i += 3;
	rtlglue_printf("  SoC: %X%s\n", (REG32(REVR) & 0xFFFF0000) >> 16, soc_name[i]);
#else
	rtlglue_printf("  SoC: %X\n", (REG32(REVR) & 0xFFFFF000) >> 12);
#endif
}
...
#if defined(CONFIG_RTL_8197F)
#ifndef CONFIG_RTL_PROC_NEW
	struct proc_dir_entry *l2_learn_limitation_entry;
#endif
#endif
...
#if !defined(CONFIG_RTL_PROC_NEW)
	struct proc_dir_entry *rtl_op_layer_entry = NULL;
#endif
...
struct proc_dir_entry *rtl_8367r_vlan;
	...
	spin_lock_init(&lock_eth_event);
	spin_lock_init(&lock_eth_smi);
	spin_lock_init(&lock_proc_cmd);
	spin_lock_init(&lock_eth_phy);
#endif
	...
	show_soc_name();
	memset(ppp_ext, '\0', sizeof(ppp_ext));
	strcpy(ppp_ext, "ppp0");
	ppp_ext_len = strlen(ppp_ext);
	REG32(CPUIIMR) = 0x00;
		...
		*/
		if (RTL865X_PORTMASK_UNASIGNED==rtl865x_wanPortMask)
		{
			...
			if (rtl819x_bond_option() == 3){
				rtl865x_wanPortMask = 0;
				if (IS_8197F_VG())
					rtl865x_lanPortMask = RTL8197FS_VG_LAN_PORT_MASK;
				else
					rtl865x_lanPortMask = RTL8197FS_LAN_PORT_MASK;
				for(i=0;i<totalVlans;i++)
				{
...
//	rtlglue_printf("	creating eth0~eth%d...\n",totalVlans-1 );
	...
	spin_lock_init(&lock_igmp_concurrent);
	#endif
...
#if defined(CONFIG_RTW_MEMPOOL)
  rtw_create_mem_pool_ex("eth_buffer",
#ifdef CONFIG_TP_MEMORY_OPTIMIZATION
	NUM_RX_PKTHDR_DESC*6,
#else
	NUM_RX_PKTHDR_DESC*8,
#endif
  	(NUM_RX_PKTHDR_DESC+NUM_RX_PKTHDR_DESC1+NUM_RX_PKTHDR_DESC2+NUM_RX_PKTHDR_DESC3+
	NUM_RX_PKTHDR_DESC4+ NUM_RX_PKTHDR_DESC5+MAX_PRE_ALLOC_RX_SKB),CROSS_LAN_MBUF_LEN);
	rtw_mem_pool_register_cb(RTW_ETH_MEMPOOL_NAME, RTW_MEM_POOL_FREE_CB, re865x_mempool_free_cb);
#endif
	for(i=0;i<totalVlans;i++)
	{
		struct net_device *dev;
...
#ifdef  RTL_TRAP_BR_SUBNET_PKT_FROM_WAN_TO_CPU
	proc_create_data("br_subnet_from_wan_to_cpu",0644, &proc_root, &br_subnet_from_wan_to_cpu_proc_fops,NULL);
#endif	
...
#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
#ifdef CONFIG_RTL_PROC_NEW
    	proc_create_data("hw_nat_netif",0,&proc_root,&hw_nat_netif_proc_fops,NULL);
#else
		static struct proc_dir_entry *hw_nat_netif_proc=NULL; 
    	hw_nat_netif_proc = create_proc_entry("hw_nat_netif", 0, NULL);
		if(hw_nat_netif_proc)
		{
			hw_nat_netif_proc->read_proc = hw_nat_netif_read;
			hw_nat_netif_proc->write_proc = hw_nat_netif_write;
		}
#endif

#ifdef CONFIG_RTL_PROC_NEW
    	proc_create_data("phy_wan_portmask",0,&proc_root,&phy_wan_portmask_proc_fops,NULL);
#else
		static struct proc_dir_entry *phy_wan_portmask_proc=NULL; 
    	phy_wan_portmask_proc = create_proc_entry("phy_wan_portmask", 0, NULL);
		if(phy_wan_portmask_proc)
		{
			phy_wan_portmask_proc->read_proc = phy_wan_portmask_read;
			phy_wan_portmask_proc->write_proc = phy_wan_portmask_write;
		}
#endif

#ifdef CONFIG_RTL_PROC_NEW
    	proc_create_data("port_and_netif",0,&proc_root,&port_and_netif_proc_fops,NULL);
#else
		static struct proc_dir_entry *port_and_netif_proc=NULL; 
    	port_and_netif_proc = create_proc_entry("port_and_netif", 0, NULL);
		if(port_and_netif_proc)
		{
			port_and_netif_proc->read_proc = port_and_netif_read;
			port_and_netif_proc->write_proc = port_and_netif_write;
		}
#endif
...
#if defined(CONFIG_RTL_8197F)   //l2 learning number limitation
#ifdef CONFIG_RTL_PROC_NEW
    proc_create_data("l2_learn_limitation",0,&proc_root,&l2_learn_limitation_proc_fops,NULL);
#else
	l2_learn_limitation_entry = create_proc_entry("l2_learn_limitation",0,NULL);
	if (l2_learn_limitation_entry)
	{
	    l2_learn_limitation_entry->read_proc = l2_learn_limitation_read;
	    l2_learn_limitation_entry->write_proc = l2_learn_limitation_write;
	}
#endif
#endif
	...
	{
	    rtl_op_layer_entry->read_proc = rtl_asic_operation_layer_read;
	    rtl_op_layer_entry->write_proc = rtl_asic_operation_layer_write;
	}
#endif
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
#ifdef CONFIG_RTL_PROC_NEW	
	proc_create_data("rtl_8367_chip_type",0,&proc_root,&rtl_8367_chip_type_proc_fops,NULL);
...
#ifdef CONFIG_TP_IMAGE
    proc_create_data("link",0,rtl865x_proc_dir,&port_link_status_proc_fops,NULL);
	proc_create_data("wan_led_status",0,rtl865x_proc_dir,&wan_led_status_proc_fops,NULL);
	proc_create_data("lan1_led_status",0,rtl865x_proc_dir,&lan1_led_status_proc_fops,NULL);
	proc_create_data("lan2_led_status",0,rtl865x_proc_dir,&lan2_led_status_proc_fops,NULL);
#endif
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
	rtl_init_83xx_share_meter_proc();
	rtl_init_83xx_storm_ctrl_proc();
#endif

#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
	rtl_find_port_to_ring();
#endif
...
#if defined(CONFIG_RTW_MEMPOOL)
	rtw_remove_mem_pool_ex(RTW_ETH_MEMPOOL_NAME);
#endif
...
#if defined(CONFIG_RTL_8197F)
	rtl8651_exitextendStormCtrl();
#endif

#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
	rtl_exit_83xx_share_meter_proc();
	rtl_exit_83xx_storm_ctrl_proc();
#endif
...
@comm
	Its important to call this API before using the driver. Note that you can not call this API twice !
*/
int32 rtl865x_init(void)
{
	int32 retval = 0;
...
#else
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
			rtl865x_attachMasterNetif(netif.name, RTL_DRV_BR_WAN_NETIF_NAME);
#else
...
static int re865x_reInitIgmpSetting(int mode)
{
	...
	for(i=0; i<totalVlans; i++)
	{
		...
		rtl_setIpv4UnknownMCastFloodMap(nicIgmpModuleIndex, 0);
#if defined(CONFIG_RTL_MLD_SNOOPING)
		rtl_setIpv6UnknownMCastFloodMap(nicIgmpModuleIndex, 0);
#endif
	}
	#endif
...
static int rtl_config_perport_perdev_vlanconfig(int mode)
{
	...
	#if defined(RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)	
	((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[1]))->portmask = 0x10; //eth1->port4
	#else
	...
	for(i=0; i<RTL8651_PORT_NUMBER + 3; i++)
	{
		...
		if (pvid!=0)
		{
			...
			if((1<<i) & RTL_WANPORT_MASK)
				rtl865x_setPortToNetif(RTL_DRV_BR_WAN_NETIF_NAME, i);
#endif
		}
	}
...
/*l2 learning limitation*/
#if defined(CONFIG_RTL_8197F)
#ifdef CONFIG_RTL_PROC_NEW
static int l2_learn_limitation_read(struct seq_file *s, void *v)
#else
static int32 l2_learn_limitation_read( char *page, char **start, off_t off, int count, int *eof, void *data )
#endif
{
    uint32 port_id ,enable, limitnum;
	
	seq_printf(s, "\n%s\n", "l2 learn limitation setting(port0~port5):");
	seq_printf(s, "\n%s\n", "port  enable  liminum:");
	
	for(port_id=0; port_id<=RTL8651_PHY_NUMBER; port_id++)
	{
	    rtl8651_getAsicL2LimitRule(port_id, &enable, &limitnum);
		seq_printf(s, "%d,    %d,    %d\n", 
		port_id,	
		enable,
		limitnum
...
static int32 l2_learn_limitation_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		num=sscanf(tmpbuf,"%d,%d,%d", &port_id, &enable, &limitnum);
		if(num!=3){
		   return len;
		}

		rtl8651_setAsicL2LimitRule(port_id, enable, limitnum);
		
        return len;
errout:
		rtlglue_printf("l2 learning number limitation format:	\"port_id,enable,num\"\n");
		rtlglue_printf("\"port_id(0~5),enable(0/1),num(0~2047)\"\n");
	}			
...
//READ and SET hw nat wan netif's name
#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)   
#ifdef CONFIG_RTL_PROC_NEW
static int hw_nat_netif_read(struct seq_file *s, void *v)
{
	seq_printf(s, "hw_nat_netif_name=%s \n", hw_nat_netif_name);
	return 0;
}
#else
static int hw_nat_netif_read(char *page, char **start, off_t off,
	                         int count, int *eof, void *data)
{
	int len = 0;
		
	len += sprintf(page+len, "hw_nat_netif_name=%s \n", hw_nat_netif_name);
...
#endif
static int hw_nat_netif_write(struct file *file, const char *buffer,
		      unsigned long count, void *data)
{
	int num = 0;
	char tmpbuf[16] = {'\0'};	
	struct rtl865x_netif_s netif;
	int i=0,j=1, idx = 0 ,retval = 0, hw_vid= 0;
	...
	tmpbuf[count-1] = '\0';
	//printk("%s %d tmpbuf=%s hw_nat_netif_name=%s \n", __func__, __LINE__, tmpbuf, hw_nat_netif_name);
	if(strncmp(hw_nat_netif_name, tmpbuf, strlen(tmpbuf)))   //if change netif name, del old 
		rtl865x_delNetif(hw_nat_netif_name);
		
	num = sscanf(tmpbuf, "%s", hw_nat_netif_name);
    for (i=0; vlanconfig[i].ifname[0] != '\0'; i++)
	{
		if (strncmp(hw_nat_netif_name, vlanconfig[i].ifname, 4)) {	
			idx = i;
			break;
		}
	}
	...
	for( i = count-2; i>4; i--)
	{
        hw_vid += ((hw_nat_netif_name[i]-'0')* j);
		j *= 10;
	}
	memset(&netif, 0, sizeof(rtl865x_netif_t));
	memcpy(netif.name, hw_nat_netif_name, MAX_IFNAMESIZE);
	memcpy(netif.macAddr.octet, vlanconfig[idx].mac.octet, ETHER_ADDR_LEN);   //ethx.xxx's MAC will be change when set ethx's MAC
	netif.mtu = vlanconfig[idx].mtu;
	netif.if_type = vlanconfig[idx].if_type;
	netif.vid = hw_vid;	//set hw netif.vid to ethx.xxx's vid	
	netif.is_wan = 1;
	...
	retval = rtl865x_addNetif(&netif);
	if(retval==FAILED)
		panic_printk("[%s]   [%d]     add netif failed!!!   \n",__FUNCTION__,__LINE__);
		
	return count;

errout:					
		printk("echo \"if_name\" > /proc/hw_nat_netif \n");

	return count;	
}
//READ and SET physical wan portmask
#ifdef CONFIG_RTL_PROC_NEW
static int phy_wan_portmask_read(struct seq_file *s, void *v)
{
	int i = 0;
	seq_printf(s, "multi_phy_vir_wan_enable=%d  phy_wan_portmask=0x%x \n", multi_phy_vir_wan_enable,wanport_mask);
	for(i = 0; i < WAN_FID_NUM_MAX; i++)
	{
		seq_printf(s, "wan_fid_map[i] = {0x%x,%d,%d} \n", 
			wanport_fid_map[i].portmask,
			wanport_fid_map[i].in_used,
			wanport_fid_map[i].wan_fid);
	}
	return 0;
}
#else
static int phy_wan_portmask_read(char *page, char **start, off_t off,
							int count, int *eof, void *data)
{
	int len = 0;
					  
	len += sprintf(page+len, "multi_phy_vir_wan_enable=%d  phy_wan_portmask=0x%x \n", multi_phy_vir_wan_enable,wanport_mask);
...
#endif
static int phy_wan_portmask_write(struct file *file, const char *buffer,
							unsigned long count, void *data)
{
	int num = 0 , i = 0;
	char tmpbuf[100] = {'\0'};  
	unsigned int wanmask = 0,phywan_enable = 0;
			  
	if (buffer && !copy_from_user(tmpbuf, buffer, count))	
	{		
		num=sscanf(tmpbuf,"%d %x",&phywan_enable,&wanmask);		
		if(num>2){			
			return count;		
		}		
		if(wanmask)
		{
			wanport_mask = wanmask;		
            lanport_mask = 0x1f & (~wanport_mask);
		}

		multi_phy_vir_wan_enable = phywan_enable;

		for(i = 0; i < WAN_FID_NUM_MAX; i++)
		{
			wanport_fid_map[i].portmask = 0;
			wanport_fid_map[i].in_used = 0;
			wanport_fid_map[i].wan_fid = i+1;
		}
	}	
...
static int port_and_netif_read(struct seq_file *s, void *v)
{
	...
	{
		cp = ((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[i]));
	    seq_printf(s, "netif_name=%s    portmask=%x    \n", cp->dev->name , cp->portmask);
	}
	return 0;
}
#else
static int port_and_netif_read(char *page, char **start, off_t off,
	                         int count, int *eof, void *data)
{
	int len = 0, i;
	struct dev_priv	*cp;
	...
	{
		cp = ((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[i]));
		len += sprintf(page+len,  "netif_name=%s    portmask=%x    \n", cp->dev->name ,cp->portmask);
	}
...
#endif
static int port_and_netif_write(struct file *file, const char *buffer,
		      unsigned long count, void *data)
{
	return 1;
}
...
extern unsigned int HardwareV6Enable;
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
uint32 arp_v6_aging = FALSE;
#else
uint32 arp_v6_aging = TRUE;			// for 97G and later series, aging should be enabled
#endif
...
static int rtk_hwL3v6_read(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "%s %d\n", "arp6_aging_enable:", arp_v6_aging);
	return 0;
}
#else
static int32 rtk_hwL3v6_read( char *page, char **start, off_t off, int count, int *eof, void *data )
...
static int32 rtk_hwL3v6_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		if (cmd_addr && !memcmp(cmd_addr, "by_ipv6_route", strlen("by_ipv6_route"))){
		...
		#endif
		else if (!memcmp(cmd_addr, "arp6_aging", strlen("arp6_aging"))){
			sscanf(strptr, "%x", &enable);
			if(enable){
				arp_v6_aging  = TRUE;
			}
			...
			rtl8198C_setAsicArp6Aging(arp_v6_aging);
		}
		else{
			if(tmpbuf[0] == '1')
			{
			...
			else
			{
				...
				printk("echo \"arp6_aging $enable\" >/proc/hwL3v6\n");
				printk("$enable means disable or enable arp6 aging, valid value is 0/1\n");
				return len;
			}
		}
	}
...
static int32 rtk_vlan_support_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		if(tmpbuf[0] == '0')
		{
			...
			#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
			rtl865x_disableRtl83xxToCpuAcl();
			#endif
   ...
   if(rtl_hw_vlan_enable && (rtl865x_curOpMode == GATEWAY_MODE))
	{
		...
		// p4 (or CPU port) setting is changed to used by Management vlan
    ...
    if(rtl_hw_vlan_enable && (rtl865x_curOpMode == GATEWAY_MODE))
    {
		...
		//rtl865x_enableRtl83xxToCpuAcl();
		rtl865x_enableRtl83xxBCMCToCpu(2);
#endif
 }	
...
#if defined(CONFIG_RTL_VLAN_8021Q)
static int rtl_vlan_ipv6_passthru_config(int passthruflag)
{			    
	int i;
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
	int cmdFlag=0;
#endif
	int vlan_id = 0;
	uint32 portmask_tmp=0, membermask_tmp=0, memberport_flag=0;
		
	//calc bridge lan portmask
	for(vlan_id=0; vlan_id<VLAN_GROUP_NUM; vlan_id++)
	{
		if(vlan_ctl_p->group[vlan_id].flag)
		{
		   if(vlan_ctl_p->group[vlan_id].vlanType == VLAN_TYPE_BRIDGE)
			{
			   portmask_tmp = vlan_ctl_p->group[vlan_id].memberMask;
			   membermask_tmp |= portmask_tmp;
			   //printk("++++++++%s:%d membermask_tmp:%d portmask_tmp:%d\n",__FUNCTION__, __LINE__, membermask_tmp, portmask_tmp);
			}
		}	
	}
	membermask_tmp &= RTL_LANPORT_MASK;

	if(passthruflag & IP6_PASSTHRU_MASK)
	{//add
		for(i=0; i<RTL865XC_PORT_NUMBER; i++)
		{
			memberport_flag = (1<<i) & membermask_tmp;
			if(memberport_flag)
			{
				//close bridge lan port ipv6 passthru
				printk("%s:%d port:%d close hw ipv6 passthru\n",__FUNCTION__, __LINE__, i);
			}
			else
			{
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)			
	rtl8367_setProtocolBasedVLAN_ipv6PassThru(ETH_P_IPV6,PASSTHRU_VLAN_ID,cmdFlag, membermask_tmp);
#endif
...
#if defined(CONFIG_RTL_VLAN_8021Q)
extern int rtl8367_setProtocolBasedVLAN_ipv6PassThru(uint32 proto_type,uint32 cvid, int cmdFlag, int portmask);
#endif
#endif
	...
	if (buffer && !copy_from_user(&passThru_flag, buffer, count))
	{
		...
		if(flag ^ oldStatus)
		{
			...
			if((flag & IP6_PASSTHRU_MASK) ^ (oldStatus & IP6_PASSTHRU_MASK))
			{
				...
				if(linux_vlan_enable)
				{
					rtl_vlan_ipv6_passthru_config(flag);	
				}
				else
				{
...
int rtl_addAclByDestMac(uint8 *mac)
{
       rtl865x_AclRule_t       rule;
       int ret;
       ...
       memcpy(rule.dstMac_.octet, mac, 6);
       memset(rule.dstMacMask_.octet, 0xff, 6);
       ret= rtl865x_add_acl(&rule, vlanconfig[0].ifname, RTL865X_ACL_IGMP_USED);
...
#ifdef CONFIG_RTK_MULTI_AP
       int rtl_addAclByDestMac(uint8 *mac);
       // Prevent LLDP Discovery Message from being forwarded
       uint8 mac[6] = { 0x01, 0x80, 0xC2, 0x00, 0x00, 0x0E };
       rtl_addAclByDestMac(mac);
#endif
...
#if defined(CONFIG_RTL_8197F)
#define METER_NUMBER				4

static char STR_TIME_UNIT[METER_NUMBER][32]={
	"25ms(link speed 10Mbps)",
	"2.5ms(link speed 100Mbps)",
	"0.25ms(link speed 1000Mbps)",
	"0.5ms(link speed 500Mbps)"
};
...
static int rtl865x_extendStormCtrlReadProc(struct seq_file *s, void *v)
{
	...
	uint32 i;
	char *str0 = NULL, *str1 = NULL, *str2 = NULL, *str3 = NULL;
	...
	// 2 bits
	timeUnit = ((regDataCounter&TUSSCM0)>>TUSSCM0_OFFSET) &0x3;
	str0 = STR_TIME_UNIT[timeUnit];

	timeUnit = ((regDataCounter&TUSSCM1)>>TUSSCM1_OFFSET) &0x3;
	str1 = STR_TIME_UNIT[timeUnit];

	timeUnit = ((regDataCounter&TUSSCM2)>>TUSSCM2_OFFSET) &0x3;
	str2 = STR_TIME_UNIT[timeUnit];

	timeUnit = ((regDataCounter&TUSSCM3)>>TUSSCM3_OFFSET) &0x3;
	str3 = STR_TIME_UNIT[timeUnit];
	
	seq_printf(s, "extend storm control setting:\n");
	seq_printf(s,"	meter 0, %s, timing unit %s, rate(%d)\n", regDataCounter&ENDROPSC0?"enable":"disable", str0,((regData<<16)>>16)*100*METER_COUNTER_UNNIT/30360);
	
	regData = READ_MEM32(EXTSTMCR4);
	seq_printf(s,"	meter 1, %s, timing unit %s, rate(%d)\n", regDataCounter&ENDROPSC1?"enable":"disable", str1, (regData>>16)*100*METER_COUNTER_UNNIT/30360);

	regData = READ_MEM32(EXTSTMCR5); 
	seq_printf(s,"	meter 2, %s, timing unit %s, rate(%d)\n", regDataCounter&ENDROPSC2?"enable":"disable", str2, ((regData<<16)>>16)*100*METER_COUNTER_UNNIT/30360);

	regData = READ_MEM32(EXTSTMCR5);
	seq_printf(s,"	meter 3, %s, timing unit %s, rate(%d)\n\n", regDataCounter&ENDROPSC3?"enable":"disable", str3, (regData>>16)*100*METER_COUNTER_UNNIT/30360);
	...
	for (i=0; i < RTL8651_PORT_NUMBER; i++){
		...
		if(i & 0x1){
			seq_printf(s,"	port%d, %s unknown unicast, meter %d\n", i,regData&OUUSCS?"enable":"disable", (regData>>OUUSCM_OFFSET)&0x3);
			seq_printf(s,"	port%d, %s unknown multicast, meter %d\n", i,regData&OUMSCS?"enable":"disable", (regData>>OUMSCM_OFFSET)&0x3);
			seq_printf(s,"	port%d, %s ARP, meter %d\n", i,regData&OASCS?"enable":"disable", (regData>>OASCM_OFFSET)&0x3);
			seq_printf(s,"	port%d, %s DHCP, meter %d\n", i,regData&ODSCS?"enable":"disable", (regData>>ODSCM_OFFSET)&0x3);
			seq_printf(s,"	port%d, %s IGMP/MLD, meter %d\n\n", i,regData&OIMSCS?"enable":"disable", (regData>>OIMSCM_OFFSET)&0x3);
		}
		else{
			seq_printf(s,"	port%d, %s unknown unicast, meter %d\n", i, regData&EUUSCS?"enable":"disable", (regData>>EUUSCM_OFFSET)&0x3);
			seq_printf(s,"	port%d, %s unknown multicast, meter %d\n", i, regData&EUMSCS?"enable":"disable", (regData>>EUMSCM_OFFSET)&0x3);
			seq_printf(s,"	port%d, %s ARP, meter %d\n", i, regData&EASCS?"enable":"disable", (regData>>EASCM_OFFSET)&0x3);
			seq_printf(s,"	port%d, %s DHCP, meter %d\n", i, regData&EDSCS?"enable":"disable", (regData>>EDSCM_OFFSET)&0x3);
			seq_printf(s,"	port%d, %s IGMP/MLD, meter %d\n\n", i, regData&EIMSCS?"enable":"disable", (regData>>EIMSCM_OFFSET)&0x3);
		}
	}
...
{	
	//uint32 tmpBuf[32];
	char tmpBuf[128], strType[16];	
	uint32 enableStormCtrl=FALSE;
	uint32 percentage=0;	
	int32 stormCtrlType = -1, num;
	...
	{
		tmpBuf[count-1]='\0';
		num = sscanf(tmpBuf, "%s %d", strType, &percentage);
		if (num != 2){
			goto ESC_ERROR;
		}
		...
		else if(strcmp(strType, "arp") == 0){
			stormCtrlType = ARP_STORM_CONTROL ;
		}
		else if(strcmp(strType, "dhcp") == 0){
			stormCtrlType = DHCP_STORM_CONTROL ;
		}
		else if(strcmp(strType, "igmp") == 0){
...
#ifdef CONFIG_RTL_TX_CACHE_ALIGN
		REG32(CPUICR1) = (REG32(CPUICR1) & ~CF_TXDESC_MASK) | (0x8<<CF_TXDESC_OFFSET);
#endif
#ifdef CONFIG_RTL_RX_CACHE_ALIGN
		REG32(CPUICR1) = (REG32(CPUICR1) & ~CF_RXDESC_MASK) | (0x8<<CF_RXDESC_OFFSET);
#endif
...
int rtl865x_reinitSwitchCore(void)
{
	...
	#if defined(CONFIG_RTL_8197F)
	if (IS_8197F_VG())
		REG32(DMA_CR0) = (REG32(DMA_CR0) & ~(LowFifoMark_MASK|HiFifoMark_MASK)) | ((LOW_FIFO_MARK_VALUE<<LowFifoMark_OFFSET)|HIGH_FIFO_MARK_VALUE_8197F_VG);
	else
		REG32(DMA_CR0) = (REG32(DMA_CR0) & ~(LowFifoMark_MASK|HiFifoMark_MASK)) | ((LOW_FIFO_MARK_VALUE<<LowFifoMark_OFFSET)|HIGH_FIFO_MARK_VALUE);
...
static int rtl_check_swCore_tx_hang_read_proc(struct seq_file *s, void *v)
{
	seq_printf(s, "check_tx_hang_interval=%d, reinit_swCore_threshold=%d, rtl_reinit_swCore_counter=%d, rtl_reinit_swCore_and_failed_counter=%d\n",
		rtl_check_swCore_tx_hang_interval, rtl_reinit_swCore_threshold, rtl_reinit_swCore_counter, rtl_reinit_swCore_and_failed_counter);
		      ...
		      unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, count))
	{
		...
		if(strncmp(buffer, "rc", 2) == 0) { /* reboot check */
			if (rtl_reinit_swCore_and_failed_counter >= CHECK_TX_ABNORMAL_AND_RESTART_THRESHOLD) {
				printk("Switch Tx hang and reinitialization failed count is more than %d times, reboot the system.\n", rtl_reinit_swCore_and_failed_counter);
				REG32(WDTCNR) = 0xA5240000;
				machine_restart();
			}		
			return count;
		}
#endif
		num = sscanf(tmpBuf, "%d %d", &rtl_check_swCore_tx_hang_interval, &rtl_reinit_swCore_threshold);
...
int rtl_check_swCore_tx_hang(void)
{
	...
	if (((rtl_check_swCore_timer++) % rtl_check_swCore_tx_hang_interval)==0) {
		...
		if (rtl_swCore_tx_hang_cnt == 0) {
			rtl_reinit_swCore_and_failed_counter = 0;
		}
...
#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
		else if (!memcmp(cmd_addr, "msend", 4))
		{
			uint32 enRing=REG32(TXRINGCR)&0xf;
			if(enRing==1)
				activeRingNum=1;
			else if(enRing==0x3)
				activeRingNum=2;
				...
				activeRingNum=4;
			//printk("activeRingNum = %d\n", activeRingNum);

			for(i=0;i<activeRingNum;i++){
				ringPort[i]=(uint32)simple_strtol(tokptr, NULL, 16);
				tokptr = strsep(&strptr," ");
				if (tokptr==NULL)
				{
					goto errout;
			...
			pktLen = simple_strtol(tokptr, NULL, 0);
			if (pktLen < 50)
				goto errout;
			...
			if(interval)
				...
				tx_cnt = tx_time * 1000;
			//tx_cnt *= activeRingNum;
			panic_printk("test packet transmitting...");
			ringIndex=0; //ring index
			for (j=0;j<tx_cnt;j++) // for sending multiple packets
			{					
				nicTx.txIdx=ringIndex;
				nicTx.portlist = ringPort[ringIndex];
				//printk("txIdx=%d, ringPort[%d]=%#x\n", nicTx.txIdx, ringIndex, nicTx.portlist);
				alloc_rx_buf((void **)&skb, MBUF_LEN);

				memcpy(skb->data, eth_da_sa, sizeof(eth_da_sa));
				*(unsigned short *)(skb->data+12) = pktLen;
				...
				if (mode == 0) {
					...
					memset(skb->data+14, 0, pktLen);
				else if (mode == 2)
					memset(skb->data+14, 0xff, pktLen);
				else if (mode == 3)
					memset(skb->data+14, ringIndex, pktLen);
				...
				retval=RTL_swNic_send((void *)skb, (void *) skb->data, skb->len, &nicTx); 
				if(interval){
					mdelay(interval);
					if (j % (1000/interval) == 0)
						panic_printk(".");
				...
				ringIndex = (ringIndex+1)%activeRingNum;
			}
			
			REG32(CPUIIMR) |= RX_DONE_IE_ALL;
			}
			panic_printk("\n  ==> %d packet(s) with type %d sent out to %d rings.\n", tx_cnt, mode, activeRingNum);

		}
#endif /* CONFIG_RTL_NIC_MULTI_TX_RING */
		else
		{
			goto errout;
		}
...
#ifdef CONFIG_RTL_NIC_MULTI_TX_RING
		panic_printk("msend [Ring0] [Ring1] [Ring2] [Ring3] [mode] [second] [interval] [pktLen]\n");
#endif
	}
...
// in case customer use second RGMII port as LAN/WAN port, then define it
//#define ENABLE_8367RB_RGMII2	1

#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
#define _83XX_STAT_TBL_SIZE		13
uint64 stat_cntr[_83XX_STAT_TBL_SIZE];
		
typedef enum tr181_stat_e
{
    TR_IfInOctets = 0,
    TR_IfInUcastPkts,
    TR_EtherStatsMulticastPkts,
    TR_EtherStatsBroadcastPkts,
    TR_Dot3StatsFCSErrors,
    TR_Dot3StatsSymbolErrors,
    TR_Dot1dTpPortInDiscards,
    TR_IfOutOctets,
    TR_IfOutUcastPkts,
    TR_IfOutMulticastPkts,
    TR_IfOutBroadcastPkts,
    TR_EtherStatsCollisions,
    TR_Dot1dBasePortDelayExceededDiscards,
    TR_END
} tr181_stat_t;

static uint32 _83xx_stat_table_for_tr181[_83XX_STAT_TBL_SIZE] = {
	STAT_IfInOctets,
	STAT_IfInUcastPkts,
...
#if 1
		for(j = 0; j < _83XX_STAT_TBL_SIZE; j++) {
			stat_cntr[j] = 0;
			rtk_stat_port_get(i, _83xx_stat_table_for_tr181[j], &stat_cntr[j]);
		}
		seq_printf(s, "port%d: %llu %u %u %u  0  0  0  %u  ", i,
			   stat_cntr[TR_IfInOctets], 
			   (uint32)(stat_cntr[TR_IfInUcastPkts]+stat_cntr[TR_EtherStatsMulticastPkts]+stat_cntr[TR_EtherStatsBroadcastPkts]),
			   (uint32)(stat_cntr[TR_Dot3StatsFCSErrors]+stat_cntr[TR_Dot3StatsSymbolErrors]), 
			   (uint32)(stat_cntr[TR_Dot1dTpPortInDiscards]),
			   (uint32)(stat_cntr[TR_EtherStatsMulticastPkts]));

		seq_printf(s, "%llu %u %u %u  0  0  0 0  ",
			   stat_cntr[TR_IfOutOctets], 
			   (uint32)(stat_cntr[TR_IfOutUcastPkts]+stat_cntr[TR_IfOutMulticastPkts]+stat_cntr[TR_IfOutBroadcastPkts]),
			   (uint32)(stat_cntr[TR_EtherStatsCollisions]), 
			   (uint32)(stat_cntr[TR_Dot1dBasePortDelayExceededDiscards]));

		seq_printf(s, "%u %u %u %u  %u\n",
			   (uint32)(stat_cntr[TR_IfInUcastPkts]), 
			   (uint32)(stat_cntr[TR_EtherStatsBroadcastPkts]), (uint32)(stat_cntr[TR_IfOutUcastPkts]),
			   (uint32)(stat_cntr[TR_IfOutMulticastPkts]), (uint32)(stat_cntr[TR_IfOutBroadcastPkts]));

		if (eth0_portmask & (1 << i))
			p_if_stats = &eth0_stats;
		else
			p_if_stats = &eth1_stats;
		
		p_if_stats->BytesSent += stat_cntr[TR_IfOutOctets];
		p_if_stats->BytesReceived += stat_cntr[TR_IfInOctets];
		p_if_stats->ErrorsSent += (uint32)(stat_cntr[TR_EtherStatsCollisions]);
		p_if_stats->ErrorsReceived += (uint32)(stat_cntr[TR_Dot3StatsFCSErrors] + stat_cntr[TR_Dot3StatsSymbolErrors]);
		p_if_stats->UnicastPacketsSent += stat_cntr[TR_IfOutUcastPkts];
		p_if_stats->UnicastPacketsReceived += stat_cntr[TR_IfInUcastPkts];
		p_if_stats->DiscardPacketsSent += (uint32)(stat_cntr[TR_Dot1dBasePortDelayExceededDiscards]);
		p_if_stats->DiscardPacketsReceived += (uint32)(stat_cntr[TR_Dot1dTpPortInDiscards]);
		p_if_stats->MulticastPacketsSent += stat_cntr[TR_IfOutMulticastPkts];
		p_if_stats->MulticastPacketsReceived += stat_cntr[TR_EtherStatsMulticastPkts];
		p_if_stats->BroadcastPacketsSent += stat_cntr[TR_IfOutBroadcastPkts];
		p_if_stats->BroadcastPacketsReceived += stat_cntr[TR_EtherStatsBroadcastPkts];
		p_if_stats->PacketsSent += stat_cntr[TR_IfOutUcastPkts] + stat_cntr[TR_IfOutMulticastPkts] + stat_cntr[TR_IfOutBroadcastPkts];
		p_if_stats->PacketsReceived += stat_cntr[TR_IfInUcastPkts] + stat_cntr[TR_EtherStatsMulticastPkts]+ stat_cntr[TR_EtherStatsBroadcastPkts];
#else
...
static netif_name_mapping_t  netif_name_mapping[NETIF_MAPPING_NUMBER];
/*  flag:1 ->brctl addif
 *  flag: 2 ->brctl delif
 */
int rtk_dev_name_netif_mapping(char *br_name, char *br_port_name, int flag)
{
	...
	{
		case 1:
			for (i = 0; i < NETIF_MAPPING_NUMBER; i++)
			{
					//null space
...
static inline int rtk_isHwlookup_vlan(uint16 vid, struct sk_buff *skb, uint32 *portlist)
{
	...
	if(rtl865x_curOpMode == GATEWAY_MODE)
	{
		...
		if(cp->id == rtk_vlan_wan_vid && rtk_get_skb_vid(skb) == 0)
		{
			...
			*portlist = RTK_WAN_PORT_MASK;
			flag = FALSE;
			#if 0//!defined(CONFIG_RTL_8367R_SUPPORT)
			if(mirrorPort_enable == 1 && rtl_ip_option_check(skb)!= TRUE)  //not 8367R+mirrorport, wan UC tx use HW
			{
				flag = TRUE;
			}
			#endif
	...
	#endif
	{
...
#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)
		if (vid == PASSTHRU_VLAN_ID){
			*portlist = rtl865x_getVlanPortMask(vid);
		}
		else 
#endif
		...
		//only forwad it with nic direct tx way and set the portmask to only wan port(donnot support multiable wan for now)
		if(vid == rtk_vlan_wan_vid)
		{
		...
		{
		//panic_printk("%s   %d -------portlist=%x ------ vid=%d-----------",__FUNCTION__,__LINE__,*portlist , vid);
			flag = FALSE;	
		}
	}
...
struct dev_priv *rtk_get_cp_by_vid_pid(uint16 vid, int pid, struct sk_buff *skb)
{
	...
	if(rtl865x_curOpMode == GATEWAY_MODE)
	{
		if((1 << pid) & RTL_WANPORT_MASK)
		{
			dp = rtk_get_default_wan_cp();   //eth1
		}
		else
		{
			dp = rtk_get_default_cp();   //eth0
		}
	}
...
#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
if(multi_phy_vir_wan_enable)
   return dp ;                  //only use pid to get cp 
#endif
...
{
#if 1 /* FIXME: fix /proc/net/vlan/vlanGroup, or  Add Macro Definition */
	uint32 mask;	
...
{
	uint8 vlan_priority;	
	vlan_priority = vlan_ctl_p->group[vid].priority;
...
static int rtk_process_11q_rx(int port, struct sk_buff *skb, struct sk_buff **pskb1, rtl_nicRx_info *info)
{
	...
	*pskb1 = NULL;	
	...
	if(rtl865x_curOpMode == GATEWAY_MODE)
	{
		#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_SRC_TAG) && (defined(CONFIG_RTL_8367R_SUPPORT)||defined(CONFIG_RTL_83XX_SUPPORT))
...
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
				|| ((rtk_get_vlan_type(rtk_get_vid_by_port(port)) == VLAN_TYPE_NAT) && (rtk_get_vlan_portmask(rtk_get_vid_by_port(port))& RTL_WANPORT_MASK))
#endif
			)
			{
...
/* Comment to avoid duplicate packets */
#if 0//defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
				|| ((rtk_get_vlan_type(pvid) == VLAN_TYPE_NAT) && (rtk_get_vlan_portmask(pvid) & RTL_LANPORT_MASK))
#endif
				)
				{
					skb_vlan_type = VLAN_TYPE_BRIDGE;
...
#if 0//defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
				if (rtk_get_vlan_type(vid) == VLAN_TYPE_NAT && (rtk_get_vlan_portmask(vid) & RTL_LANPORT_MASK)) 
				{
					/*pskb1 should with vlan tag now.*/									
					*pskb1 = skb_copy(skb, GFP_ATOMIC);
					 if(*((uint16 *)((*pskb1)->data+(ETH_ALEN<<1)))== __constant_htons(ETH_P_8021Q))   
						*((uint16*)((*pskb1)->data+(ETH_ALEN<<1)+2)) = __constant_htons(vid);
					*(uint16 *)((*pskb1)->linux_vlan_src_tag) = __constant_htons(ETH_P_8021Q);
					*(uint16 *)((*pskb1)->linux_vlan_src_tag+2) = __constant_htons(vid);
	...
	if(vid == 0) //if packet without tag, check pvid to add tag
    {
   	...
   	else //if packet with tag , mostly don't touch 
   	{
		...
		if(rtl865x_curOpMode == GATEWAY_MODE)
		{
			if(vid == rtk_vlan_wan_vid && skb_vlan_type == VLAN_TYPE_NAT)
				REMOVE_TAG(skb);
		}
#endif
    }
...
#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)
	if (SUCCESS == rtl_isPassthruFrame(skb->data) &&
		dp->id == rtk_vlan_wan_vid && 
		(((skb->data[0]&0x01)&&(skb_vlan_type == VLAN_TYPE_NAT))||(!(skb->data[0]&0x01) && skb_vlan_type==VLAN_TYPE_BRIDGE)))
	{
		#if defined(CONFIG_IPV6)
		if (rtl_isIpv6McFrame(skb->data) == SUCCESS)
			info->oriPriv = info->priv;
		#endif
		if (!(skb->data[0]&0x01)&&skb_vlan_type==VLAN_TYPE_BRIDGE){
			int pvid1 = rtk_get_vid_by_port(port);
			//pvid1 is wan port pvid,when vid == pvid1, is untag nat +untag bridge,thus need copy 2 skb
			if (vid == pvid1){
				/* untagged nat + untagged bridge case:  
				* wan port rx bridge untagged passthrough frame, copy
				* original eth1 -> peth0
				* copied to eth1.xxx
				*/
				*pskb1 = skb_copy(skb, GFP_ATOMIC);				
				*(uint16 *)((*pskb1)->linux_vlan_src_tag) = __constant_htons(ETH_P_8021Q);
				*(uint16 *)((*pskb1)->linux_vlan_src_tag+2) = __constant_htons(pvid1);
				...
				////wan port rx bridge tagged passthrough frame, use eth1.xxx as rx dev 
			}
			
		}
		else{
			/*   
			* nat always to peth0 as untag packets
			*/
			vid = dp->id;
			skb->dev = _rtl86xx_dev.pdev;
			dp = info->priv = NETDRV_PRIV(_rtl86xx_dev.pdev);
...
static int phy_multi_wan_rx(int port, struct sk_buff *skb, rtl_nicRx_info *info)
{
	...
	if(rtl865x_curOpMode == GATEWAY_MODE)
	{
		if( vid==0 && ((1<<port)&RTL_WANPORT_MASK))  //pkt from wan and untag, use pvid to add tag
		{
	   	 	vid = rtk_get_vid_by_port(port); 
			rtk_insert_vlan_tag(vid, skb);
		}
...
static inline uint32 rtk_hw_port_map(uint16 vid ,uint32 portmask)
{
	...
	if(multi_phy_vir_wan_enable)
{
   			 //do nothing     wan vlan need cpu port
	}
    else
    #endif
    {
...
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
static void rtk_update_wan_netif_vlan(unsigned char *if_name,uint16 vid , uint32 portmask)
{
	struct net_device *dev;
	struct dev_priv	  *dp;
	int j;
    ...
    // update hw netif table vid , depend on vlanconfig idx
	rtl865x_setNetifVid(if_name,vid);	
	//update wan slave netif(ppp0) vid
	rtl865x_setNetifVid(RTL_DRV_PPP_NETIF_NAME,vid);

	//update sw info
    vlanconfig[1].vid = vid;
	vlanconfig[1].memPort = portmask;
	vlanconfig[2].vid = vid;
	vlanconfig[2].memPort = portmask;
	
	((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[1]))->id = vid; 
	((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[1]))->portmask = portmask; 
...
#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
int get_wan_fid_by_portmask(int wanportmask)
{
	int i = 0;
	for (i = 0; i < WAN_FID_NUM_MAX; i++)
	{
		if (wanportmask == wanport_fid_map[i].portmask) {
			if(wanport_fid_map[i].in_used)
				return wanport_fid_map[i].wan_fid;
		}
	}

	for (i = 0; i < WAN_FID_NUM_MAX; i++)
	{
		if(wanport_fid_map[i].in_used == 0) {
			 wanport_fid_map[i].portmask = wanportmask;
			 wanport_fid_map[i].in_used = 1;
			 return wanport_fid_map[i].wan_fid;
		}
	}
	return 0;
...
/*
* action: 0 means delete; 1 means add; 2 means flush.
*/
int add_or_remove_vlan_priority(int priority, uint64_t portmask, int action)
{
	int i = 0;
	uint32 port_mask = portmask & 0xffffffff;	
	#if defined(CONFIG_RTL_8367R_SUPPORT)
	uint32  pvid = 0, pri = 0;
	#endif
	if (action == 2){
		for (i = 0; i < RTL8651_AGGREGATOR_NUMBER; i++){
			rtl8651_setAsicPortVlanPri(i, 0);
			#if defined(CONFIG_RTL_8367R_SUPPORT)
	...
	for (i = 0; i < RTL8651_AGGREGATOR_NUMBER; i++){
		if ((1<<i) & port_mask){
			if (action == 1){
				rtl8651_setAsicPortVlanPri(i, priority);			
				#if defined(CONFIG_RTL_8367R_SUPPORT)
				rtk_vlan_portPvid_get(i, &pvid, &pri);
				rtk_vlan_portPvid_set(i, pvid, priority);
				#endif
			}
			else{
				rtl8651_setAsicPortVlanPri(i, 0);
...
int dump_vlan_priority(void)
{
	...
	uint32 pvid = 0;
	#endif
	printk("dump port vlan priority:\n");
	for (i = 0; i < RTL8651_AGGREGATOR_NUMBER; i++){
		priority = 0;
		rtl8651_getAsicPortVlanPri(i, &priority); 
		printk("port%u: %u\n", i, priority);
	}
	#if defined(CONFIG_RTL_8367R_SUPPORT)
	printk("\ndump 8367r port vlan priority:\n");
	for (i = 0; i < RTL8651_AGGREGATOR_NUMBER; i++){
		pvid = priority = 0;
		rtk_vlan_portPvid_get(i, &pvid, &priority);
		printk("port%u: %u\n", i, priority);
	}
	#endif
	printk("\n\n");
...
void rtk_add_eth_vlan(uint16 vid , uint32 portmask,uint32 tagmask,uint32 vlantype)
{
    ...
    //if portmask has wan port then set fid to 1
    if(rtl865x_curOpMode == GATEWAY_MODE)
    {
		#ifdef CONFIG_RTL_8021Q_VLAN_SUPPORT_SRC_TAG
		...
		#else
		#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
		if( ((portmask & RTK_WAN_PORT_MASK) && multi_phy_vir_wan_enable)||(!multi_phy_vir_wan_enable &&( (portmask & RTK_WAN_PORT_MASK) == RTK_WAN_PORT_MASK )) )
		#else
		if( (portmask & RTK_WAN_PORT_MASK ) == RTK_WAN_PORT_MASK )
		#endif
        {
			#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
			wan_fid = get_wan_fid_by_portmask(portmask & RTK_WAN_PORT_MASK);
			if(wan_fid)
				rtl865x_setVlanFilterDatabase(vid, wan_fid);
			else
			#endif
			rtl865x_setVlanFilterDatabase(vid, RTL_WAN_FID);
		  	...
		  	if( vid == rtk_vlan_wan_vid ) // if it is nat wan vlan , need to update netif table ,mark_vc
		   	{
				...
				#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
				if(multi_phy_vir_wan_enable)
					rtl865x_setNetifVid(hw_nat_netif_name,vid);       //updata hw-nat-wan's netif vid
				else
				#endif
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
					//updata hw-nat-wan's netif vid			
					rtk_update_wan_netif_vlan(RTL_DRV_BR_WAN_NETIF_NAME, vid, rtk_hw_port_map(vid,portmask));
#else
        ...
        {     		
			...
			if(vid == rtk_vlan_hw_nat_lan_vid)
			{
				#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_MULTI_PHY_VIR_WAN)
				if(multi_phy_vir_wan_enable)
               		rtl865x_setNetifVid(RTL_DRV_LAN_NETIF_NAME,vid);       //updata hw-nat-lan's netif vid
				else
				#endif
	...
	#if defined(CONFIG_RTL_83XX_SUPPORT)
	if (portmask & RTK_CPU_PORT_MASK)
		portmask = (portmask & ~(RTK_CPU_PORT_MASK)) | (1<<r8367_cpu_port);
	
	if (tagmask & RTK_CPU_PORT_MASK)
		tagmask = (tagmask & ~(RTK_CPU_PORT_MASK)) | (1<<r8367_cpu_port);
	#endif
	...
	{
	#if defined(CONFIG_RTL_8367R_SUPPORT)
			rtl_vlan_RTL8367R_set(vid, tagmask, portmask, wan_fid);
	#elif defined(CONFIG_RTL_83XX_SUPPORT)	
			rtl_vlan_RTL83XX_set(vid, tagmask, portmask, wan_fid);
	#endif
	}
	else
#endif	
...
if(multi_phy_vir_wan_enable)
{
	for (i=0; i<RTL8651_PORT_NUMBER; i++) 
	{
		if(strncmp(hw_nat_netif_name, "eth", 3)==0)
		{
			if ((1<<i) & RTL_WANPORT_MASK)
				rtl865x_setPortToNetif(hw_nat_netif_name,i);
			else
				rtl865x_setPortToNetif(RTL_DRV_LAN_NETIF_NAME,i);
		}
		else
			rtl865x_setPortToNetif(RTL_DRV_LAN_NETIF_NAME,i);
	}
...
#endif
#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)	
    // when setting vlan after setting ipv6 passthru
	passthruflag=(int)atoi_dec(passThru_flag);

		if(passthruflag ^ oldStatus)
		{
			//IPv6 PassThru
			if((passthruflag & IP6_PASSTHRU_MASK) ^ (oldStatus & IP6_PASSTHRU_MASK))
			{		
				rtl_vlan_ipv6_passthru_config(passthruflag);	
			}
		}
#endif
}
...
#if defined(CONFIG_RTL_83XX_ACL_SUPPORT)
#if defined CONFIG_RTL_PROC_NEW
extern int rtl_83xxACLReadProc(struct seq_file *s, void *v);
extern int rtl_83xxSwACLReadProc(struct seq_file *s, void *v);
#else
struct proc_dir_entry *rtl_83xxACLProc=NULL;
struct proc_dir_entry *rtl_83xxSwACLProc=NULL;

extern int rtl_83xxACLReadProc(char *page, char **start, off_t off,
		     int count, int *eof, void *data);
extern int rtl_83xxSwACLReadProc(char *page, char **start, off_t off,
		     int count, int *eof, void *data);

#endif
extern int rtl_83xxACLWriteProc(struct file *file, const char *buffer, unsigned long count, void *data);
extern int rtl_83xxSwACLWriteProc(struct file *file, const char *buffer, unsigned long count, void *data);
#if defined CONFIG_RTL_PROC_NEW
int rtl_83xxACL_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtl_83xxACLReadProc,NULL));
}
static ssize_t rtl_83xxACL_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return rtl_83xxACLWriteProc(file, userbuf,count, off);
}
struct file_operations rtl_83xxACL_proc_fops= {
        .open           = rtl_83xxACL_single_open,
        .write		    = rtl_83xxACL_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
int rtl_83xxSwACL_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtl_83xxSwACLReadProc,NULL));
}
static ssize_t rtl_83xxSwACL_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return rtl_83xxSwACLWriteProc(file, userbuf,count, off);
}
struct file_operations rtl_83xxSwACL_proc_fops= {
        .open           = rtl_83xxSwACL_single_open,
        .write		    = rtl_83xxSwACL_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
{
#ifdef CONFIG_RTL_PROC_NEW
    proc_create_data("rtl_83xx_hw_acl",0,&proc_root,&rtl_83xxACL_proc_fops,NULL);
	proc_create_data("rtl_83xx_sw_acl",0,&proc_root,&rtl_83xxSwACL_proc_fops,NULL);
#else
	rtl_83xxACLProc = create_proc_entry("rtl_83xx_hw_acl", 0, NULL);
	if(rtl_83xxACLProc)
	{
		rtl_83xxACLProc->read_proc = rtl_83xxACLReadProc;
		rtl_83xxACLProc->write_proc = rtl_83xxACLWriteProc;
	}
	rtl_83xxSwACLProc = create_proc_entry("rtl_83xx_sw_acl", 0, NULL);
	if(rtl_83xxSwACLProc)
	{
		rtl_83xxSwACLProc->read_proc = rtl_83xxSwACLReadProc;
		rtl_83xxSwACLProc->write_proc = rtl_83xxSwACLWriteProc;
	}
#endif
...
{
#ifdef CONFIG_RTL_PROC_NEW
	remove_proc_entry("rtl_83xx_hw_Acl", &proc_root);
	remove_proc_entry("rtl_83xx_sw_Acl", &proc_root);
#else
	if (rtl_83xxACLProc) {
		remove_proc_entry("rtl_83xx_hw_Acl", rtl_83xxACLProc);
		rtl_83xxACLProc = NULL;
	}

	if (rtl_83xxSwACLProc) {
		remove_proc_entry("rtl_83xx_sw_Acl", rtl_83xxSwACLProc);
		rtl_83xxSwACLProc = NULL;
	}
#endif
}
#endif
...
extern int rtl83xx_qos_test(void);
#endif
#if defined (CONFIG_RTL_PROC_NEW)
extern int rtl_83xxQosReadProc(struct seq_file *s, void *v);
#else
struct proc_dir_entry *rtl_83xxQosProc=NULL;
extern int rtl_83xxQosReadProc(char *page, char **start, off_t off,
		     int count, int *eof, void *data);
#endif
extern int rtl_83xxQosWriteProc(struct file *file, const char *buffer, unsigned long count, void *data);

#if defined (CONFIG_RTL_PROC_NEW)
int rtl_83xxQos_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtl_83xxQosReadProc,NULL));
}
static ssize_t rtl_83xxQos_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return rtl_83xxQosWriteProc(file, userbuf,count, off);
}
struct file_operations rtl_83xxQos_proc_fops= {
        .open           = rtl_83xxQos_single_open,
        .write		    = rtl_83xxQos_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#if defined (CONFIG_RTL_PROC_NEW)
extern int rtl_83xxQosConfigReadProc(struct seq_file *s, void *v);
#else
struct proc_dir_entry *rtl_83xxQosConfigProc=NULL;
extern int rtl_83xxQosConfigReadProc(char *page, char **start, off_t off,
		     int count, int *eof, void *data);
#endif
extern int rtl_83xxQosConfigWriteProc(struct file *file, const char *buffer, unsigned long count, void *data);

#if defined CONFIG_RTL_PROC_NEW
int rtl_83xxQos_config_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, rtl_83xxQosConfigReadProc,NULL));
}
static ssize_t rtl_83xxQos_config_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return rtl_83xxQosConfigWriteProc(file, userbuf,count, off);
}
struct file_operations rtl_83xxQos_config_proc_fops= {
        .open           = rtl_83xxQos_config_single_open,
        .write		    = rtl_83xxQos_config_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
int32 rtl_init83xxQosProc(void)
{
#ifdef CONFIG_RTL_PROC_NEW
    proc_create_data("rtl_83xxQos",0,&proc_root,&rtl_83xxQos_proc_fops,NULL);
	proc_create_data("rtl_83xxQos_config",0,&proc_root,&rtl_83xxQos_config_proc_fops,NULL);
#else
	rtl_83xxQosConfigProc = create_proc_entry("rtl_83xxQos_config", 0, NULL);
	if(rtl_83xxQosConfigProc)
	{
		rtl_83xxQosConfigProc->read_proc = rtl_83xxQosConfigReadProc;
		rtl_83xxQosConfigProc->write_proc = rtl_83xxQosConfigWriteProc;
	}

	rtl_83xxQosProc = create_proc_entry("rtl_83xxQos", 0, NULL);
	if(rtl_83xxQosProc)
	{
		rtl_83xxQosProc->read_proc = rtl_83xxQosReadProc;
		rtl_83xxQosProc->write_proc = rtl_83xxQosWriteProc;
	}
#endif
...
void __exit rtl_exit83xxQosCtrl(void)
{
#ifdef CONFIG_RTL_PROC_NEW
		remove_proc_entry("rtl_83xxQos", &proc_root);
	remove_proc_entry("rtl_83xxQos_config", &proc_root);
#else
	if (rtl_83xxQosProc) {
		remove_proc_entry("rtl_83xxQos", rtl_83xxQosProc);
		rtl_83xxQosProc = NULL;
	}
	
	if (rtl_83xxQosConfigProc) {
		remove_proc_entry("rtl_83xxQos_config", rtl_83xxQosConfigProc);
		rtl_83xxQosConfigProc = NULL;
	}
#endif
}
#endif
...
inline int rtl_runout_resume(void)
{
	...
	if(disable_runout_irq)
	{
		...
		if (rx_desc_not_empty(0)) {
			cp = NETDRV_PRIV(irqDev);
			rtl_rx_interrupt_process(RX_DONE_IP_ALL, cp);
			cnt_rx_desc_full_and_buf_pool_not_empty++;
		}
		else {
...
int rtl_add_Acl_for_br_subnet_from_wan(void)
{
	...
	{		
		...
		//panic_printk("\n%s:%d _br0_ip_tmp=%08x _br0_mask_tmp=%08x!!!\n",__FUNCTION__,__LINE__,_br0_ip_tmp,_br0_mask_tmp);

		if ((pVlanConfig[i].isWan) && (_br0_ip_tmp!=0) && (_br0_mask_tmp!=0))
		{			
			rtl865x_regist_aclChain(pVlanConfig[i].ifname, RTL865X_ACL_BR_SUBNET_FROM_WAN_USED, RTL865X_ACL_INGRESS);
			...
			if ((_br0_ip_bak!=0) && (_br0_mask_bak!=0)) {
				...
				rule.srcIpAddr_ 	= _br0_ip_bak; 
				rule.srcIpAddrMask_	= _br0_mask_bak;
				rule.dstIpAddr_ 	= _br0_ip_bak; 
				rule.dstIpAddrMask_	= _br0_mask_bak;

				ret = rtl865x_del_acl(&rule, pVlanConfig[i].ifname, RTL865X_ACL_BR_SUBNET_FROM_WAN_USED);
			...
			if (rtl865x_curOpMode == GATEWAY_MODE) {
				...
				ret = rtl865x_add_acl(&rule, pVlanConfig[i].ifname, RTL865X_ACL_BR_SUBNET_FROM_WAN_USED);
				...
				rule.srcIpAddr_ 	= _br0_ip_tmp; 
				rule.srcIpAddrMask_ = _br0_mask_tmp;
				rule.dstIpAddr_ 	= _br0_ip_tmp; 
				rule.dstIpAddrMask_ = _br0_mask_tmp;

				//panic_printk("\n%s:%d call rtl865x_add_acl!!! pVlanConfig[i].ifname=%s\n",__FUNCTION__,__LINE__,pVlanConfig[i].ifname);
				ret = rtl865x_add_acl(&rule, pVlanConfig[i].ifname, RTL865X_ACL_BR_SUBNET_FROM_WAN_USED);
	...
	#if defined(CONFIG_RTL_8367R_SUPPORT) 
	//default 8367r packets already trap to 97f
	#endif
	return SUCCESS;
}
#endif
...
#if defined(CONFIG_RTL_ETH_PRIV_SKB)
// for debug
void rtl_dump_eth_skb_buf(int action)
{
	int i = 0, j = 0, reserve_len = 0, num = 0;
	unsigned char *tmp = NULL, *tmp_buff = NULL, *data_buff = NULL;
	reserve_len = (RX_OFFSET + RTL_PRIV_DATA_SIZE);

	num = MAX_ETH_SKB_NUM + 1;
	
	printk("MAX_ETH_SKB_NUM=%d sizeof(struct priv_skb_buf2)=%d sizeof(eth_skb_buf)=%d \n", MAX_ETH_SKB_NUM, sizeof(struct priv_skb_buf2), sizeof(eth_skb_buf));
	printk("=======================================DUMP START=============================\n");
	for(i = 0;i < MAX_ETH_SKB_NUM + 1;i++){
		tmp = (unsigned char*)(&(eth_skb_buf[i]));
		tmp_buff = (unsigned char*)(&(eth_skb_buf[i].buf[0]));
		data_buff = tmp_buff + reserve_len;

		#if defined(CONFIG_RTL_SWITCH_NEW_DESCRIPTOR)
		if (action && is_rxring_used((unsigned int)data_buff)){
			num -= 1;
			continue;
		}
		#endif
		
		printk("======================eth_skb_buf[%d]'s addr:0x%08x========================\n",i, (unsigned int)tmp);
		for(j = 0;j < 24;j++){
		...
		printk("======eth_skb_buf[%d].buf's addr: 0x%08x data_buff's addr:0x%08x======\n",i, (unsigned int)tmp_buff, (unsigned int)data_buff);
		for(j = 0;j < 128;j++){
			if ((j !=0) && (j%16 ==0))
				printk("\n");
			
			printk("0x%02x ",	*(data_buff+j));
		}
		printk("\n\n\n");
		...
		//printk("index:%d,magic:%s,point:%x\n",i,eth_skb_buf[i].magic,(unsigned int)(eth_skb_buf[i].buf_pointer));
	}
	
	printk("=====================================DUMP END NUM=%d=============================\n", num);
}
#endif
...
#endif
{
		...
		len = sprintf(page, "asic operation layer: %d \n",rtl8651_getAsicOperationLayer());
...
{
	char tmpbuf[32] = {0};
	int num = 0, op_layer = 0;

	if (buff && !copy_from_user(tmpbuf, buff, len))
	{		
		tmpbuf[len-1] = '\0';

		num = sscanf(tmpbuf, "%d", &op_layer);
		panic_printk("%s %d num=%d op_layer=%d tmpbuf=%s \n", __FUNCTION__, __LINE__, num,op_layer, tmpbuf);
		if ((num == 1) && (op_layer <= 4) && (op_layer >= 1)){
			rtl8651_setAsicOperationLayer(op_layer);
		}
		else{
			panic_printk("%s %d invalid command, op_layer:1~4 \n", __FUNCTION__, __LINE__);
		}
...
void rtl_map_ring_by_L2_port(struct sk_buff* skb, rtl_nicTx_info *nicTx)
{
	...
	// put BC, MC to ring 3 for highest priority in ring strict mode
	if((nicTx->flags & PKT_BCAST) || (nicTx->flags & PKT_MCAST)){
		nicTx->txIdx=3;
		return;
	}

	nicTx->txIdx=0;
	portmask = rtl865x_getPortMaskByMac(skb->data, 0xff);
	if (portmask < 0)
		return;
...
void rtl_map_ring_by_port(rtl_nicTx_info *nicTx)
{
	...
	if(nicTx->portlist&RTL_LANPORT_MASK_4){	// has lan4 bit: ring3
		nicTx->txIdx=3;
	}
	else if(nicTx->portlist&RTL_LANPORT_MASK_3){	// has lan3 bit (no lan4 bit): ring2
		nicTx->txIdx=2;
	}
	else if(nicTx->portlist&RTL_LANPORT_MASK_2){	// has lan2 bit (no lan4/lan3 bit): ring1
		nicTx->txIdx=1;
	}
	else
		nicTx->txIdx=0;
...
void rtl_find_port_to_ring(void)
{
	 ...
	 */
	for (i=0; i<RTL8651_PHY_NUMBER; i++) {
		if (RTL_LANPORT_MASK_1 & (1<<i))
			rf_port_to_ring[i] = 0;
		else if (RTL_LANPORT_MASK_2 & (1<<i))
			rf_port_to_ring[i] = 1;
		else if (RTL_LANPORT_MASK_3 & (1<<i))
			rf_port_to_ring[i] = 2;
		else if (RTL_LANPORT_MASK_4 & (1<<i))
			rf_port_to_ring[i] = 3;
		else
			rf_port_to_ring[i] = 0;
	}
