...
static int32 _rtl8198c_synIpv6NxtHopToAsic(rtl8198c_ipv6_nextHopEntry_t *entry_t)
{
	...
	if (entry_t->nextHopType == IF_ETHER) {
		...
		if ((entry_t->un.nexthop.v6_addr32[0]!=0)||(entry_t->un.nexthop.v6_addr32[1]!=0)||
			...
			if(((entry_t->un.nexthop.v6_addr32[0]>>16)&0xFFFF)==0xFE80){
				dev = dev_get_by_name(&init_net,entry_t->dstNetif->name);

				if(dev){
					inv6_addr_t net_ipv6_addr;
					int i;

					for(i=0;i<4;i++)
						net_ipv6_addr.v6_addr32[i] = htonl(entry_t->un.nexthop.v6_addr32[i]);
					neigh = neigh_lookup(&nd_tbl, &net_ipv6_addr, dev);
				}
				
				if(neigh){
					//printk("[%s:%d]find neigh\n",__FUNCTION__,__LINE__);
					read_lock_bh(&neigh->lock);
					memcpy(mac.octet, neigh->ha, ETHER_ADDR_LEN);
					read_unlock_bh(&neigh->lock);
					ret_arpFound = SUCCESS;
				}
			}	
			else{
				ret_arpFound = rtl8198c_getIpv6ArpMapping(entry_t->un.nexthop, &arp_t);
				if(ret_arpFound==SUCCESS)
					memcpy(mac.octet,arp_t.mac.octet,ETHER_ADDR_LEN);
			}
		 }

		//printk("[%s:%d]mac %x:%x:%x:%x:%x:%x\n",__FUNCTION__,__LINE__,mac.octet[0],mac.octet[1],
			//mac.octet[2],mac.octet[3],mac.octet[4],mac.octet[5]);

		rtl865x_getVlanFilterDatabaseId(entry_t->dstNetif->vid, &fid);
		retval = rtl865x_Lookup_fdb_entry(fid, (ret_arpFound == SUCCESS)? &mac : &reservedMac, FDB_DYNAMIC, &columIdx,&asic_l2);

		if (SUCCESS == retval){
			asic.nextHopRow = rtl8651_filterDbIndex( (ret_arpFound == SUCCESS)? &mac : &reservedMac, fid );
		}
		else{
			//always to cpu
			asic.nextHopRow = rtl8651_filterDbIndex(&reservedMac, RTL_LAN_FID);
			//printk("%s %d netifname=%s netifvid=%u fid=%u \n", __func__, __LINE__, entry_t->dstNetif->name, entry_t->dstNetif->vid, fid);
		}
