...
static int32 _rtl865x_addRoute(ipaddr_t ipAddr, ipaddr_t ipMask, ipaddr_t nextHop, int8 * ifName,ipaddr_t srcIp)
{
	...
	*/
	printk("[%s-%u] ifName %s type %u srcIp %u.%u.%u.%u ipAddr %u.%u.%u.%u mask %u.%u.%u.%u nextHop %u.%u.%u.%u\n",
		__func__, __LINE__, netif->name, netif->if_type, NIPQUAD(srcIp), NIPQUAD(ipAddr), NIPQUAD(ipMask), NIPQUAD(nextHop));
			...
			case IF_L2TP:
				{
					...
					rt->process = RT_L2;
					ppp = rtl865x_getPppByNetifName(netif->name);					
					printk("[%s-%u] %s->%p\n", __func__, __LINE__, netif->name, ppp);
					if(ppp == NULL)
					{
						/*printk("Warning!!CAn't get pptp/l2tp session information by interface name(%s)\n",netif->name);*/
						goto addFailed;
					}
					printk("[%s-%u] %02X-%02X-%02X-%02X-%02X-%02X\n", __func__, __LINE__,
						ppp->server_mac.octet[0], ppp->server_mac.octet[1], ppp->server_mac.octet[2], 
						ppp->server_mac.octet[3], ppp->server_mac.octet[4], ppp->server_mac.octet[5]);
			...
			case IF_L2TP:
				{
					...
					printk("[%s-%u] %s pppoe %p\n", __func__, __LINE__, netif->name, pppoe);
					if(pppoe != NULL)
					{
						/*got pppoe session*/
...
#endif	
#endif
	printk("[%s-%u] ipAddr %u.%u.%u.%u mask %u.%u.%u.%u\n",
		__func__, __LINE__, NIPQUAD(ipAddr), NIPQUAD(ipMask));
...
#if 1//#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
#include <net/arp.h>
unsigned int neigh_connected_check = 1;
int get_mac_addr_by_ip(unsigned int ip, unsigned char *mac_addr)
{
	int i = 0, ret = -1;
	...
	unsigned char *pkey = (unsigned char *)&ip;
	...
	for (i = 0; i < (1 << nht->hash_shift); i++){
		for (n = rcu_dereference_bh(nht->hash_buckets[i]);
			...
			if (!memcmp(n->primary_key, pkey, key_len)) {
				// find it, 
				if((neigh_connected_check && (n->nud_state & NUD_VALID)) || !neigh_connected_check){
					memcpy(mac_addr, n->ha, ETH_ALEN);
					ret = 0;
					#if 0
					printk("[%s:%d] neigh_connected_check: %d, neigh->nud_state: 0x%x, NUD_CONNECTED: 0x%x\n", 
					__FUNCTION__, __LINE__, neigh_connected_check, n->nud_state, NUD_CONNECTED);
					printk("[%s:%d] ip: 0x%x mac:0x%x:%x:%x:%x:%x:%x\n", 
					__FUNCTION__, __LINE__, ip, mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
					#endif
					break;
				}
...
int rtl_check_arp_for_default_gw(void)
{
	int ret = FALSE;
	rtl865x_route_t* rt = NULL;
	...
	rt = _rtl865x_getDefaultRoute();
	if (rt && rt->valid && rt->dstNetif && (rt->dstNetif->if_type==IF_ETHER) && (rt->nextHop)){
		gw_ip = rt->nextHop;
		//printk("%s %d gw_ip=0x%x \n", __func__, __LINE__, gw_ip);
		if(!rtl865x_isArpExisted(gw_ip))
		{
			//sync 
			unsigned char macAddr[ETHER_ADDR_LEN] = {0};
			if(!get_mac_addr_by_ip(ntohl(gw_ip), macAddr)){	// find a match
				ether_addr_t mac;
				memset(&mac, 0, sizeof(ether_addr_t));
				memcpy(mac.octet, macAddr, ETHER_ADDR_LEN);
		
				ret = rtl865x_addArp(gw_ip, &mac);
                          #if 0
				printk("[%s:%d] sync ip(0x%x) mac(%x:%x:%x:%x:%x:%x) to asic, ret(%d)\n", 
					__FUNCTION__, __LINE__, gw_ip, 
					mac.octet[0], mac.octet[1], mac.octet[2], 
					mac.octet[3], mac.octet[4], mac.octet[5], ret);
                          #endif
			}
		}
	}
...
int rtl865x_checkL3L2Valid(ipaddr_t ipAddr )
{
	...
	//check default gateway's arp
	rtl_check_arp_for_default_gw();

	
#if 1//defined(CONFIG_RTL_HW_NAPT_SUPPORT_WAN_CLASSAB_IP)
	if(!rtl865x_isArpExisted(ntohl(ipAddr)))
	{
		//sync all non-exist hw arp entry, inclue lan and wan
		unsigned char macAddr[ETHER_ADDR_LEN] = {0};
		if(!get_mac_addr_by_ip(ipAddr, macAddr)){		// find a match
			ether_addr_t mac;
			memset(&mac, 0, sizeof(ether_addr_t));
			memcpy(mac.octet, macAddr, ETHER_ADDR_LEN);
	
			ret = rtl865x_addArp(ntohl(ipAddr), &mac);
             
			printk("[%s:%d] sync ip(0x%x) mac(%x:%x:%x:%x:%x:%x) to asic, ret(%d)\n",
				__FUNCTION__, __LINE__, ntohl(ipAddr),
				mac.octet[0], mac.octet[1], mac.octet[2],
				mac.octet[3], mac.octet[4], mac.octet[5], ret);    
		}	
	}
	...
	{	
		...
		{		
			if((rt->ipAddr&rt->ipMask)==(htonl(ipAddr)&rt->ipMask))
			{
				switch (rt->process)
				{
				case RT_PPPOE:
				...
				case RT_ARP:
					if(rtl865x_isArpExisted(htonl(ipAddr)))
					{
						return 1;
					}
					else
					...
					switch(rt->dstNetif->if_type)
					{
						case IF_ETHER:
							...
							else
							{
								...
								if(gHwNatPPTPL2TP)
								{
									/* The second connection of PPTP/L2TP does not check the nexthop */								
									return 1;
								}
								else
#endif
