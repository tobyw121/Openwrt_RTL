...
#if defined(CONFIG_RTL_8197F) && !defined(CONFIG_OPENWRT_SDK)
...
int32 rtl8651_setAsicPortVlanPri(uint32 port, uint32 vlan_pri)
{
	uint32 regValue,offset;
	if(port>=RTL8651_AGGREGATOR_NUMBER || vlan_pri>=8)
		return FAILED;
	offset=(port*2)&(~0x3);
	regValue=READ_MEM32(PVCR0+offset);
	if((port&0x1))
	{
		regValue=  ((vlan_pri &0x7) <<28) | (regValue&~0x70000000);
	}
	else
	{	
		regValue =  ((vlan_pri &0x7) <<12) | (regValue &~0x7000);
	}
	WRITE_MEM32(PVCR0+offset,regValue);
	return SUCCESS;
}
int32 rtl8651_getAsicPortVlanPri(uint32 port, uint32 *vlan_pri) 
{
	uint16 offset;
	offset=(port*2)&(~0x3);
	if(port>=RTL8651_AGGREGATOR_NUMBER || vlan_pri == NULL)
		return FAILED;
	if((port&0x1))
	{
		*vlan_pri=(((READ_MEM32(PVCR0+offset)>>28)&0x7));		
	}
	else
	{
		*vlan_pri=(((READ_MEM32(PVCR0+offset)>>12)&0x7));
	}
	return SUCCESS;
}
...
/*2007-12-19*/
	...
	entry.enHWRouteV6 = (intfp->enableRouteV6==TRUE? 1: 0);
#endif

	//entry.enHWRoute   = (rtl8651_getAsicOperationLayer()>2)?	(intfp->enableRoute==TRUE? 1: 0):0;
	entry.enHWRoute   = (intfp->enableRoute==TRUE? 1: 0);
...
/*
@func int32 | rtl8651_getAsicL2Rule | get L2 port learning number limitation rule
@rvalue SUCCESS | 
@comm
 */
 #if defined(CONFIG_RTL_8197F)
int32 rtl8651_getAsicL2LimitRule(uint32 port_id, uint32 *enable, uint32 *limitnum)
{
    if(port_id<0 || port_id > (RTL8651_PORT_NUMBER-1)){
		return FAILED;
	}
    else{
	    *enable = ((READ_MEM32(L2LEARNLIMIT0+0x4*(port_id/2))>>16*(port_id%2)) & 0x1 );
		*limitnum = ((READ_MEM32(L2LEARNLIMIT0+0x4*(port_id/2))>>(3+16*(port_id%2))) & 0x7FF) ;
	}
	
	return SUCCESS;
}
/*
@func int32 | rtl8651_setAsicL2Rule | set L2 port learning number limitation rule
@rvalue SUCCESS | 
@comm
	When enable L2 port learning number limitation rule,num of MACaddr of port is limited.
 */
int32 rtl8651_setAsicL2LimitRule(uint32 port_id, uint32 enable, uint32 limitnum)
{
    if(port_id < 0 || port_id > (RTL8651_PORT_NUMBER-1)){
		return FAILED;
	}
	...
	else if(limitnum < 0 || limitnum > RTL8651_L2_LIMITNUM){
		return FAILED;
    }
	else{
		WRITE_MEM32(L2LEARNLIMIT0+0x4*(port_id/2),((READ_MEM32(L2LEARNLIMIT0+0x4*(port_id/2)) & (~(0x1 << 16*(port_id%2)))) | ((enable & 0x1)<< 16*(port_id%2))));
		WRITE_MEM32(L2LEARNLIMIT0+0x4*(port_id/2),((READ_MEM32(L2LEARNLIMIT0+0x4*(port_id/2)) & (~(0x7FF << (3+16*(port_id%2))) ))| ((limitnum & 0x7FF)<< (3+16*(port_id%2)))));	
	}
...
#ifndef RTL865X_ACL_INGRESS
#define RTL865X_ACL_INGRESS	0
#endif

int32 rtl8651_configAclStatus(uint8 op, uint8 dir)
{
	if(op){		// enable
		if(dir == RTL865X_ACL_INGRESS){
