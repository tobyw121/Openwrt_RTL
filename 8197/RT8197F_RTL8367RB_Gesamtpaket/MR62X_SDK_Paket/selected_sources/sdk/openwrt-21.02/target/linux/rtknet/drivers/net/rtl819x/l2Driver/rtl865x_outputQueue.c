...
extern void rtl83xx_disable_dscp_remark_setting(unsigned int port);
#else
...
#if defined(CONFIG_TPLINK_QOS)
/*tplink qos requirement, all wifi device should share shaping bandwidth
 *if packets not enter extension port, linux TC can do shaping
 *if packets enter extension port, uplink shaping can do in wan port egress direction
 *and downlink shpaing should do in 97H cpu port egress direction
 *for ethernet flow, if it transmit by cpu, it will be in default queue
*/
#define DOWNLINK_PORTMASK  0x40
#endif
...
#if defined(CONFIG_TPLINK_QOS)
	if(strncmp(netIfName,RTL_BR_NAME,strlen(RTL_BR_NAME)))
#endif
	{
		//printk("[%s:%d] netIfName %s RTL_BR_NAME %s memberport 0x%x \n",__FUNCTION__,__LINE__,netIfName,RTL_BR_NAME,memberPort);
		rtl83xx_qosProcessQueue(memberPort, queueNum, queueFlag, queueId, queueBw, queueWeight, RTL_WANPORT_MASK);
	}
#if defined(CONFIG_TPLINK_QOS)
	else{
		//set 8367 qos shaping for eth device in lan port
		rtl83xx_qosProcessQueue(memberPort, queueNum, queueFlag, queueId, queueBw, queueWeight, RTL_WANPORT_MASK);

		//set qos shaping for wifi device in 97H cpu port
		rtl865xC_lockSWCore();
		//printk("[%s:%d] netIfName %s RTL_BR_NAME %s memberport 0x%x\n",__FUNCTION__,__LINE__,netIfName,RTL_BR_NAME,memberPort);

		rtl8651_setAsicOutputQueueNumber(CPU, queueNum);

		//set priority to qid mapping for ext port
		for(pri=0;pri<TOTAL_VLAN_PRIORITY_NUM;pri++){
			rtl8651_setAsicCPUPriorityToQIDMappingTable(EXT1, pri, priorityMatrix[queueNum-1][pri]);
			rtl8651_setAsicCPUPriorityToQIDMappingTable(EXT2, pri, priorityMatrix[queueNum-1][pri]);
			rtl8651_setAsicCPUPriorityToQIDMappingTable(EXT3, pri, priorityMatrix[queueNum-1][pri]);
		}
		...
		for (queue=0;queue<RTL8651_OUTPUTQUEUE_SIZE;queue++)
		{
			...
			asicBandwidth = ((qosInfo[queue].ceil)>>(EGRESS_BANDWIDTH_GRANULARITY_BITLEN_97H)) - 1;
			//printk("[%s:%d] port %d queue %d asicBandwidth %d\n",__FUNCTION__,__LINE__,CPU,qosInfo[queue].queueId,asicBandwidth);
			if ((qosInfo[queue].ceil)&(1<<(EGRESS_BANDWIDTH_GRANULARITY_BITLEN_97H-1)))
				asicBandwidth += 1;

			if ((qosInfo[queue].flags&QOS_TYPE_MASK)==QOS_TYPE_STR)
			{
				rtl8651_setAsicQueueRate(CPU, qosInfo[queue].queueId, 
					PPR_MASK>>PPR_OFFSET, 
					L1_MASK>>L1_OFFSET, 
					asicBandwidth);
				rtl8651_setAsicQueueWeight(CPU, qosInfo[queue].queueId, STR_PRIO, 0);
			}
			else
			{
				rtl8651_setAsicQueueRate(CPU, qosInfo[queue].queueId, 
				...
				#if defined(CONFIG_OPENWRT_SDK)
				if(weight<0)
					rtl8651_setAsicQueueWeight(CPU, qosInfo[queue].queueId, WFQ_PRIO, 0);
				else
				#endif	
				rtl8651_setAsicQueueWeight(CPU, qosInfo[queue].queueId, WFQ_PRIO, qosInfo[queue].bandwidth-1);
			}

			//disable cpu port flow control for used queue, except queue0 
			//if((qosInfo[queue].queueId != 0) && (asicBandwidth != 0xEE6B2)){
            if(qosInfo[queue].queueId != 0){
				if(qosInfo[queue].queueId > 5)
					rtl8651_setAsicQueueFlowControlConfigureRegister(CPU,qosInfo[queue].queueId-2,0);
				else
					rtl8651_setAsicQueueFlowControlConfigureRegister(CPU,qosInfo[queue].queueId,0);
			}
		}
		...
		for(qosMarkNumIdx=0;qosMarkNumIdx<MAX_MARK_NUM_PER_DEV;qosMarkNumIdx++)
		{
			if(mark2Priority[netifIdx][qosMarkNumIdx].mark )
			{
				...
				if(bridge_qos_enable == 1){
					...
					#if defined(CONFIG_RTL_83XX_API_V1_4)
						rtl83xx_disable_dscp_remark_setting(port);
					#else
...
#if defined (CONFIG_RTL_83XX_QOS_SUPPORT)
#if defined(CONFIG_TPLINK_QOS)
	/*for tplink, downlink shaping set in 97H port0(for lan)/port6(for wifi)
	 *only uplink shaping set in 8367
	*/
	if(strncmp(netIfName,RTL_BR_NAME,strlen(netIfName)))
#endif
		...
		for(i=0;i<RTL8651_OUTPUTQUEUE_SIZE;i++)
		{
			rtl8651_setAsicQueueWeight(CPU, i, STR_PRIO, 0);
			rtl8651_setAsicQueueRate(CPU,i, PPR_MASK>>PPR_OFFSET, L1_MASK>>L1_OFFSET,APR_MASK>>APR_OFFSET);
			//enable cpu port flow control
			rtl8651_setAsicQueueFlowControlConfigureRegister(CPU,i,1);
		}
...
//8367d - chip_type==4
#define MAX_PRIO_DEC ((chip_type==4) ? 4 : 7)
#else
#define MAX_PRIO_DEC 0x80
#endif
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		else if(!memcmp(tokptr, "nat",3))
		{
			...
			if(nat_qos_enable)
			{
				...
				rtl83xx_qosSetPriDecision(PRIDEC_1Q, MAX_PRIO_DEC);
			}
			else
			{
