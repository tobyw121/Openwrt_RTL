...
rtk_api_ret_t rtk_port_phyEnableAll_set(rtk_enable_t enable)
{
    ...
    RTK_API_LOCK();
    rtl_port_link_status_set(RTL83XX_WAN, enable);
 ...
 */
rtk_api_ret_t rtk_port_duplexAutoChange_set(rtk_enable_t state)
{
    rtk_api_ret_t retVal;

    if (NULL == RT_MAPPER->port_duplexAutoChange_set)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->port_duplexAutoChange_set(state);
    RTK_API_UNLOCK();
 ...
 */
rtk_api_ret_t rtk_port_duplexAutoChange_get(rtk_enable_t *pState)
{
    rtk_api_ret_t retVal;

    if (NULL == RT_MAPPER->port_duplexAutoChange_get)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->port_duplexAutoChange_get(pState);
    RTK_API_UNLOCK();
...
rtk_api_ret_t rtk_port_macForceLinkExtSSC_set(rtk_port_t port, rtk_enable_t state)
{
    ...
    if (NULL == RT_MAPPER->port_macForceLinkExtSSC_set)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->port_macForceLinkExtSSC_set(port, state);
    RTK_API_UNLOCK();
...
rtk_api_ret_t rtk_port_macForceLinkExtSSC_get(rtk_port_t port, rtk_enable_t *pState)
{
    ...
    if (NULL == RT_MAPPER->port_macForceLinkExtSSC_get)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->port_macForceLinkExtSSC_get(port, pState);
    RTK_API_UNLOCK();
 ...
 */
rtk_api_ret_t rtk_port_autoDownSpeed10M_set(rtk_enable_t state)
{
    rtk_api_ret_t retVal;

    if (NULL == RT_MAPPER->port_autoDownSpeed10M_set)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->port_autoDownSpeed10M_set(state);
    RTK_API_UNLOCK();
 ...
 */
rtk_api_ret_t rtk_port_autoDownSpeed10M_get(rtk_enable_t *pState)
{
    rtk_api_ret_t retVal;

    if (NULL == RT_MAPPER->port_autoDownSpeed10M_get)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->port_autoDownSpeed10M_get(pState);
    RTK_API_UNLOCK();
