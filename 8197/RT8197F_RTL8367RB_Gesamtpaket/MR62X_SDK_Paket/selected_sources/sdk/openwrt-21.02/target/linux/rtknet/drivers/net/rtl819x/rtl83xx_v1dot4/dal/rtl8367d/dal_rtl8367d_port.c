...
/* try up to 5 times */
rtk_api_ret_t force_dal_rtl8367d_getAsicPHYReg(rtk_uint32 phyNo, rtk_uint32 phyAddr, rtk_uint32 *pRegData )
{
	int i = 0;
	rtk_api_ret_t ret = 0;
	...
	for( ; i < 5; i++)
	{
		ret = _dal_rtl8367d_getAsicPHYReg(phyNo, phyAddr, pRegData);
		if(ret != RT_ERR_OK)
		{
			msleep(100);
			continue;
	...
	if(i)
	{
		printk("WARNING: port %d, addr 0x%x, read failed(cnt=%d)\n", phyNo, phyAddr, i);
	}

	return ret;
}

rtk_api_ret_t force_dal_rtl8367d_setAsicPHYReg(rtk_uint32 phyNo, rtk_uint32 phyAddr, rtk_uint32 phyData )
{
	int i = 0;
	rtk_api_ret_t ret = 0;
	...
	for( ; i < 5; i++)
	{
		ret = _dal_rtl8367d_setAsicPHYReg(phyNo, phyAddr, phyData);
		if(ret != RT_ERR_OK)
		{
			msleep(100);
			continue;
	...
	if(i)
	{
		printk("WARNING: port %d, addr 0x%x, write failed(cnt=%d)\n", phyNo, phyAddr, i);
	}
...
rtk_api_ret_t dal_rtl8367d_port_phyAutoNegoAbility_set(rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    ...
    if ((retVal = force_dal_rtl8367d_getAsicPHYReg(rtk_switch_port_L2P_get(port), PHY_1000_BASET_CONTROL_REG, &phyData)) != RT_ERR_OK)
        return retVal;

    phyData = (phyData & (~0x0200)) | phyEnMsk9 ;

    if ((retVal = force_dal_rtl8367d_setAsicPHYReg(rtk_switch_port_L2P_get(port), PHY_1000_BASET_CONTROL_REG, phyData)) != RT_ERR_OK)
        return retVal;

    /*Auto-Negotiation control register setting*/
    if ((retVal = force_dal_rtl8367d_getAsicPHYReg(rtk_switch_port_L2P_get(port), PHY_AN_ADVERTISEMENT_REG, &phyData)) != RT_ERR_OK)
        return retVal;

    phyData = (phyData & (~0x0DE0)) | phyEnMsk4;
    if ((retVal = force_dal_rtl8367d_setAsicPHYReg(rtk_switch_port_L2P_get(port), PHY_AN_ADVERTISEMENT_REG, phyData)) != RT_ERR_OK)
        return retVal;

    /*Control register setting and restart auto*/
    if ((retVal = force_dal_rtl8367d_getAsicPHYReg(rtk_switch_port_L2P_get(port), PHY_CONTROL_REG, &phyData)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = force_dal_rtl8367d_setAsicPHYReg(rtk_switch_port_L2P_get(port), PHY_CONTROL_REG, phyData)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t dal_rtl8367d_port_phyEnableAll_set(rtk_enable_t enable)
{
	...
	rtk_uint32 i;
	bool up_success_flag;
	rtk_uint32 up_fail_cnt; 
#endif
    ...
    RTK_SCAN_ALL_LOG_PORT(port)
    {
        if(rtk_switch_isUtpPort(port) == RT_ERR_OK)
        {
			...
			if (ENABLED == enable)
			{
				...
				if(up_fail_cnt || !up_success_flag)
				{
					printk("phyEnable %d %s, fail_cnt=%d\n", port, up_success_flag ? "success":"fail", up_fail_cnt);
				}
			}
			else
			{
 ...
 */
rtk_api_ret_t dal_rtl8367d_port_duplexAutoChange_set(rtk_enable_t state)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_CHG_DUPLEX_CFG, RTL8367D_CFG_CHG_DUP_EN_OFFSET, (state == ENABLED) ? 1 : 0)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t dal_rtl8367d_port_duplexAutoChange_get(rtk_enable_t *pState)
{
    rtk_api_ret_t retVal;
    rtk_uint32 data;
    ...
    if ((retVal = rtl8367d_getAsicRegBit(RTL8367D_REG_CHG_DUPLEX_CFG, RTL8367D_CFG_CHG_DUP_EN_OFFSET, &data)) != RT_ERR_OK)
        return retVal;

    *pState = (rtk_enable_t)data;
    return RT_ERR_OK;
}
 ...
 */
rtk_api_ret_t dal_rtl8367d_port_macForceLinkExtSSC_set(rtk_port_t port, rtk_enable_t state)
{
    rtk_api_ret_t retVal;
    ...
    if (state == ENABLED)
    {
        if ((retVal = rtl8367d_setAsicReg(0x1d53, 0x0001)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367d_setAsicReg(0x1d55, 0x000f)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367d_setAsicReg(0x1d54, 0x05fa)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367d_setAsicReg(0x1d52, 0x2473)) != RT_ERR_OK)
            return retVal;
    }
    else
    {
        if ((retVal = rtl8367d_setAsicReg(0x1d53, 0x8000)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367d_setAsicReg(0x1d55, 0x000f)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367d_setAsicReg(0x1d54, 0x0008)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367d_setAsicReg(0x1d52, 0x2411)) != RT_ERR_OK)
            return retVal;
    }
 ...
 *      The API can get the state of SSC.
 */
rtk_api_ret_t dal_rtl8367d_port_macForceLinkExtSSC_get(rtk_port_t port, rtk_enable_t *pState)
{
    rtk_api_ret_t retVal;
    rtk_uint32 data;
    ...
    if ((retVal = rtl8367d_getAsicReg(0x1d52, &data)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t dal_rtl8367d_port_autoDownSpeed10M_set(rtk_enable_t state)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    rtk_uint32 port;
    ...
    RTK_SCAN_ALL_PHY_PORTMASK(port)
    {
        if (rtk_switch_isUtpPort(port) == RT_ERR_OK)
        {
            if ((retVal = dal_rtl8367d_port_phyOCPReg_get(port, 0xA442, &regData)) != RT_ERR_OK)
                return retVal;
            ...
            if ((retVal = dal_rtl8367d_port_phyOCPReg_set(port, 0xA442, regData)) != RT_ERR_OK)
                return retVal;
        }
    }
 ...
 */
rtk_api_ret_t dal_rtl8367d_port_autoDownSpeed10M_get(rtk_enable_t *pState)
{
    rtk_api_ret_t retVal;
    rtk_uint32 regData;
    rtk_uint32 port;
