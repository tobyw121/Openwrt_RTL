...
rtk_api_ret_t rtk_stat_port_getSerial(rtk_port_t port, rtk_stat_port_cntr_t *pPort_cntrs)
{
    rtk_api_ret_t retVal;

    if (NULL == RT_MAPPER->stat_port_getSerial)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->stat_port_getSerial(port, pPort_cntrs);
    RTK_API_UNLOCK();
...
rtk_api_ret_t rtk_stat_port_get_in_out_octets(rtk_port_t port, rtk_stat_port_cntr_t *pPort_cntrs)
{
    rtk_api_ret_t retVal;

    if (NULL == RT_MAPPER->stat_port_get_in_out)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->stat_port_get_in_out(port, pPort_cntrs);
    RTK_API_UNLOCK();
