...
rtk_api_ret_t rtk_switch_utpPortMask_get(rtk_portmask_t *pPmask)
{
    ...
    if (halCtrl) {
        pPmask->bits[0] = halCtrl->valid_utp_portmask;
        return RT_ERR_OK;
    }
    else 
...
rtk_api_ret_t rtk_switch_portMask_get(rtk_portmask_t *pPmask)
{
    if(init_state != INIT_COMPLETED)
        return RT_ERR_NOT_INIT;
    ...
    if (halCtrl) {
        pPmask->bits[0] = halCtrl->valid_portmask;
        return RT_ERR_OK;
    }
    else 
        return RT_ERR_FAILED;
