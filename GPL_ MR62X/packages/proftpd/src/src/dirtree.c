...
#ifdef PR_USE_GUEST_LIMIT
  gnet_param = get_param_ptr(set, "AllowGuestNetwork", FALSE);
  if (gnet_param != NULL && *gnet_param == FALSE) 
  {
      gnet_enable = 0;
  }

  if (session.is_gnet == 1 && !gnet_enable)
  {
      pr_log_pri(PR_LOG_WARNING, "Guest network disable.\n");
      return 0;
  }
#endif 
