...
static int fw_quiet = 1;

/* add by marujun, to communicate with br_wifidog */
...
static int br_wifidog_setsockopt(int level, int optname, const void *optval, socklen_t optlen)
{
	int sock = -1;
	int ret = -1;
	...
	if (sock < 0)
	{
		perror("br_wifidogSocket");
		return ret;
	}

	debug(LOG_DEBUG, "optval: %s", optval);

	if (setsockopt(sock, level, optname, optval, optlen) < 0)
	{
		perror("br_wifidogSetsockopt");
		debug(LOG_ERR, "%d opt error", optname);
...
iptables_insert_gateway_id(char **input)
{
	...
	//printf("the config gw_interface is %s\n",config->gw_interface);
	#if 1
	safe_asprintf(&buffer, *input, config->gw_interface);
	#else
	safe_asprintf(&buffer, *input, "br-lan");
	#endif
 ...
 * */
static int
conntrack_do_command(const char *format, ...)
{
	va_list vlist;
	char *fmt_cmd;
	char *cmd;
	...
	safe_asprintf(&cmd, "conntrack %s", fmt_cmd);
	free(fmt_cmd);
	...
	if (rc!=0)
		debug(LOG_DEBUG, "conntrack command failed(%d): %s", rc, cmd);
...
iptables_fw_init(void)
{
...
#if 0/*here to avoid detect ext_interface failed ,make iptables init return*/
	...
	if (config->external_interface) {
		...
		debug(LOG_DEBUG, "FATAL: no external interface");
		//return 0;
	}
#endif
	/*
	 ...
	 *
	 */
	debug(LOG_DEBUG, "config->gw_interface is %s",config->gw_interface);
	/* Create new chains */
	...
	iptables_do_command("-t mangle -A PREROUTING -m mark --mark 0x%u/0x%u -j " TABLE_WIFIDOG_TRUSTED,FW_MAKR_INTERFACE, FW_MARK_MASK);//this rule will be inserted before the prior one
	iptables_do_command("-t mangle -A PREROUTING -m mark --mark 0x%u/0x%u -j " TABLE_WIFIDOG_OUTGOING,FW_MAKR_INTERFACE, FW_MARK_MASK);
	iptables_do_command("-t mangle -A POSTROUTING -m mark --mark 0x%u/0x%u -j " TABLE_WIFIDOG_INCOMING,FW_MAKR_INTERFACE, FW_MARK_MASK);
	...
	/* Assign links and rules to these new chains */
    iptables_do_command("-t nat -A PREROUTING -m mark --mark 0x%u/0x%u -j " TABLE_WIFIDOG_OUTGOING, FW_MAKR_INTERFACE, FW_MARK_MASK);
	...
	iptables_do_command("-t nat -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0x%u -j ACCEPT", FW_MARK_KNOWN, FW_MARK_MASK);
	iptables_do_command("-t nat -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0x%u -j ACCEPT", FW_MARK_PROBATION, FW_MARK_MASK);
    ...
    iptables_do_command("-t filter -I FORWARD -m mark --mark 0x%u/0x%u -j " TABLE_WIFIDOG_WIFI_TO_INTERNET, FW_MAKR_INTERFACE, FW_MARK_MASK);
	...
	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0x%u -j " TABLE_WIFIDOG_LOCKED, FW_MARK_LOCKED, FW_MARK_MASK);
	...
	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0x%u -j " TABLE_WIFIDOG_VALIDATE, FW_MARK_PROBATION, FW_MARK_MASK);
	iptables_load_ruleset("filter", "validating-users", TABLE_WIFIDOG_VALIDATE);

	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0x%u -j " TABLE_WIFIDOG_KNOWN, FW_MARK_KNOWN, FW_MARK_MASK);
...
iptables_fw_access(fw_access_t type, const char *ip, const char *mac, int tag)
{
	...
	switch(type) {
		case FW_ACCESS_ALLOW:
			iptables_do_command("-t mangle -A " TABLE_WIFIDOG_OUTGOING " -s %s -m mac --mac-source %s -j MARK --set-mark 0x%u/0x%u", ip, mac, tag, FW_MARK_MASK);
			rc = iptables_do_command("-t mangle -A " TABLE_WIFIDOG_INCOMING " -d %s -j ACCEPT", ip);
			/* add by marujun, return status not care */
			br_wifidog_setsockopt(IPPROTO_IP, BR_WIFIDOG_ADD_MAC, mac, strlen(mac));

			/* add end */
			break;
		case FW_ACCESS_DENY:
			iptables_do_command("-t mangle -D " TABLE_WIFIDOG_OUTGOING " -s %s -m mac --mac-source %s -j MARK --set-mark 0x%u/0x%u", ip, mac, tag, FW_MARK_MASK);
			rc = iptables_do_command("-t mangle -D " TABLE_WIFIDOG_INCOMING " -d %s -j ACCEPT", ip);
			// memset(command, 0, MAX_BUF);
			// snprintf(command, sizeof(command),  "conntrack -D -s %s & fc flush --mac %s &", ip, mac);
			// system(command);
			// debug(LOG_ERR, "system: %s", command);
			/* add by marujun, return status not care */
			rc = conntrack_do_command("-D -s %s", ip);
			br_wifidog_setsockopt(IPPROTO_IP, BR_WIFIDOG_DEL_MAC, mac, strlen(mac));
			break;
		default:
			rc = -1;
			break;
