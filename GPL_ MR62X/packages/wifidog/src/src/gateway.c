			...
			while ((token1 = strsep(&running1, "|")) != NULL) {
				...
				if (key && value) {
					if (strcmp(command, "CLIENT") == 0) {
						...
						else if (strcmp(key, "counters_timeout") == 0){
							client->counters.timeout = atol(value);
						}
						else {
...
main_loop(void)
{
	...
	if (!config->gw_address) {
		debug(LOG_DEBUG, "Finding IP address of %s", config->gw_interface);
		if ((config->gw_address = safe_strdup("192.168.0.1")) == NULL) {
			debug(LOG_ERR, "Could not get IP address information of %s, exiting...", config->gw_interface);
			exit(1);
		}
		debug(LOG_DEBUG, "%s = %s", config->gw_interface, config->gw_address);
	...
	if ((webserver = httpdCreate(config->gw_address, config->gw_port)) == NULL) {
		debug(LOG_ERR, "config->gw_address is: %s  AND port: %s",config->gw_address,config->gw_port);
	...
	httpdAddFileContent(webserver, "/", "lan.js", 0, NULL, DEFAULT_PORTALLANJS);
	/*change the jpg htm using*/
	#if 0
    int fd;
    fd=open(DEFAULT_USERPORTALBGRJPG, O_RDONLY);
    if (fd==-1) {
        debug(LOG_ERR, "the user set background jpg not exist use default %s", DEFAULT_PORTALBGRJPG);
        httpdAddFileContent(webserver, "/", "portal.jpg", 0, NULL, DEFAULT_PORTALBGRJPG);
    }else
    	{
    		httpdAddFileContent(webserver, "/", "portal.jpg", 0, NULL, DEFAULT_USERPORTALBGRJPG);
			debug(LOG_ERR, "the user set background jpg  %s", DEFAULT_USERPORTALBGRJPG);
    	}
	close(fd);

	int fdlogo;
    fdlogo=open(DEFAULT_USERPORTALLOGOJPG, O_RDONLY);
    if (fdlogo==-1) {
        debug(LOG_ERR, "the user set logo jpg not exist use default %s", DEFAULT_PORTALLOGOJPG);
       httpdAddFileContent(webserver, "/", "portal-head.png", 0, NULL, DEFAULT_PORTALLOGOJPG);
    }else
    	{
    		httpdAddFileContent(webserver, "/", "portal-head.png", 0, NULL, DEFAULT_USERPORTALLOGOJPG);
			debug(LOG_ERR, "the user set logo jpg  %s", DEFAULT_USERPORTALLOGOJPG);
    	}
	close(fdlogo);
	#else
	httpdAddFileContent(webserver, "/", "portal_logo.png", 0, NULL, DEFAULT_USERPORTALLOGOJPG);
	httpdAddFileContent(webserver, "/", "portal_back.jpg", 0, NULL, DEFAULT_USERPORTALBGRJPG);
	#endif
	httpdAddFileContent(webserver, "/", "Close.png", 0, NULL, DEFAULT_PORTALCLOSEICONPNG);
	httpdAddFileContent(webserver, "/", "Enable1.png", 0, NULL, DEFAULT_PORTALENABLE1ICONPNG);
	httpdAddFileContent(webserver, "/", "Enable2.png", 0, NULL, DEFAULT_PORTALENABLE2ICONPNG);
	httpdAddFileContent(webserver, "/", "Invisible.png", 0, NULL, DEFAULT_PORTALINVISIBLEICONPNG);
	httpdAddFileContent(webserver, "/", "Visible.png", 0, NULL, DEFAULT_PORTALVISIBLEICONPNG);
	httpdAddFileContent(webserver, "/", "red-eclamation.png", 0, NULL, DEFAULT_PORTALREDECLAMATIONICONPNG);
	httpdAddFileContent(webserver, "/", "jquery.min.js", 0, NULL, DEFAULT_PORTALJQUER);
	
	/*to avoid local debug chrome request dut logo 2016.7.13*/
	httpdAddCContent(webserver, "/", "favicon.ico", 0, NULL, http_callback_favicon);
	...
	*/
	...
	pool = threadpool_init(10, 2*1024*1024,100);
	while(1) {
		...
		if (webserver->lastError == -1) {
			...
			*(params + 1) = r;
			...
			#else
			result = threadpool_add_job(pool, (void *)thread_httpd, (void *)params);
			if (result != 0) {
				debug(LOG_ERR, "FATAL: Failed to create a new thread (httpd) - exiting");
				termination_handler(0);
			}
...
int main(int argc, char **argv) {
	...
	if (restart_orig_pid) {
		...
		debug(LOG_DEBUG, "Parent PID %d seems to be dead. Continuing loading.");
	}

	if (config->daemon) {

		debug(LOG_DEBUG, "Forking into background");
