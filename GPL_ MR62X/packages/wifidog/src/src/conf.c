...
static const struct {
	...
	{ "authtype",			oAuthType },
	{ "authpassword",			oAuthPassword },
	{ "redienable",			oRediEnable },
	{ "rediurl",			oRediUrl },
	{ "authtitle",			oAuthTitle },
	{ "authfontcolor",			oAuthFontColor },
	{ "auththemecolor",			oAuthThemeColor },
	{ "authfontopacity",			oAuthFontOpacity },
	{ "auththemeopacity",			oAuthThemeOpacity },
	{ "authterm",			oAuthTerm },
	{ "authtimeout",			oAuthTimeout },
...
config_init(void)
{
	...
	FILE *fd;
	if (!(fd = fopen(config.configfile, "r"))) {
		debug(LOG_ERR, "Could not open configuration file '%s', "
				"and change configfile to default...", config.configfile);
		strncpy(config.configfile, DEFAULT_CONFIGPATH, sizeof(config.configfile));
	}
	...
	config.gw_interface = DEFAULT_GATEWAY;
	config.gw_address = NULL;
	config.gw_port = DEFAULT_GATEWAYPORT;
	config.auth_servers = NULL;
	config.httpdname = DEFAULT_HTTPDNAME;
	config.httpdrealm = DEFAULT_HTTPDNAME;
	config.httpdusername = NULL;
	config.httpdpassword = NULL;
	config.authtype = DEFAULT_AUTHTYPE;
	config.authpassword = DEFAULT_AUTHPASSWORD;
	config.redienable = DEFAULT_REDIENABLE;
	config.rediurl = DEFAULT_REDIURL;
	config.authterm = DEFAULT_AUTHTERM;
	config.authtitle = DEFAULT_AUTHTITLE;
	config.authfontcolor = DEFAULT_AUTHFONTCOLOR;
	config.auththemecolor = DEFAULT_AUTHTHEMECOLOR;
	config.authfontopacity = DEFAULT_AUTHFONTOPACITY;
	config.auththemeopacity = DEFAULT_AUTHTHEMEOPACITY;
	config.authtimeout = DEFAULT_AUTHTIMEOUT;
...
config_read(const char *filename)
{
	...
	while (!feof(fd) && fgets(line, MAX_BUF, fd)) {
		...
		/*to be fixed about read wifidog.config to avoid blank 2016.07.15*/
#if 0
		if (p1) {
			p1++;
		...
		if (p1 && p1[0] != '\0') {
			...
			if ((strncmp(s, "#", 1)) != 0) {
				...
				case oHTTPDPassword:
					...
					break;
				case oAuthType:
					config.authtype = safe_strdup(p1);
					break;
				case oAuthPassword:
					config.authpassword = safe_strdup(p1);
					break;
				case oRediEnable:
					config.redienable = safe_strdup(p1);
					break;
				case oRediUrl:
					config.rediurl = safe_strdup(p1);
					break;
				case oAuthTitle:
					config.authtitle = safe_strdup(p1);
					break;
				case oAuthFontColor:
					config.authfontcolor = safe_strdup(p1);
					break;
				case oAuthThemeColor:
					config.auththemecolor = safe_strdup(p1);
					break;
				case oAuthFontOpacity:
					config.authfontopacity = safe_strdup(p1);
					break;
				case oAuthThemeOpacity:
					config.auththemeopacity = safe_strdup(p1);
					break;
				case oAuthTerm:
					config.authterm = safe_strdup(p1);
					break;
				case oAuthTimeout:
					config.authtimeout = safe_strdup(p1);
					break;
				case oBadOption:
					debug(LOG_ERR, "Bad option on line %d "
							"in %s.", linenum,
...
void config_ui_set()
{
	if (strcasecmp(config.authtimeout, "1hours") == 0){
		config.clienttimeout = 1*60;
	}
	if (strcasecmp(config.authtimeout, "2hours") == 0){
		config.clienttimeout = 2*60;
	}
	if (strcasecmp(config.authtimeout, "4hours") == 0){
		config.clienttimeout = 4*60;
	}
	if (strcasecmp(config.authtimeout, "8hours") == 0){
		config.clienttimeout = 8*60;
	}
	if (strcasecmp(config.authtimeout, "always") == 0){
		config.clienttimeout = 20*12*30*24*60;
	}
	if (strcasecmp(config.authtimeout, "test") == 0){
