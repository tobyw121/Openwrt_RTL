...
static int old_get_bridge_info(const char *bridge, struct bridge_info *info)
{
	...
	unsigned long args[4] = { BRCTL_GET_BRIDGE_INFO,
				  (unsigned long) &i, 0, 0 };
#else
	unsigned long long args[4] = { BRCTL_GET_BRIDGE_INFO,
...
#else
		unsigned long long args[4] = { BRCTL_GET_PORT_INFO,
					   (unsigned long long) &i, index, 0 };
#endif
...
#else
		unsigned long long args[4] = { oldcode, value, 0, 0 };
#endif
		...
		unsigned long offset, int num)
{
	...
	if (f) {
		...
		unsigned long args[4] = { BRCTL_GET_FDB_ENTRIES,
					  ...
					  num, offset };
#else
		unsigned long long args[4] = { BRCTL_GET_FDB_ENTRIES,
