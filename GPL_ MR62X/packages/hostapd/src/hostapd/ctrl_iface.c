...
static const char * pin_status_str(enum pin_status status)
{
	switch (status) {
	case WPS_PIN_STATUS_DISABLE:
			return "Disabled";
	case WPS_PIN_STATUS_ACTIVE:
			return "Active";
	case WPS_PIN_STATUS_TIMEOUT:
			return "Timed-out";
	default:
			return "Unknown";
	}
...
static int hostapd_ctrl_iface_pin_lock_status(struct hostapd_data *hapd,
					     char *buf, size_t buflen)
{
	int ret;
	char *pos, *end;
	...
	if (!hapd->conf->ap_setup_locked &&
		hapd->ap_pin_failures_consecutive >= 3)
	{
					      ...
					      socklen_t fromlen)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		reply_len = hostapd_ctrl_iface_wps_get_status(hapd, reply,
							      reply_size);
	} else if (os_strncmp(buf, "PIN_LOCK_STATUS", 15) == 0) {
		reply_len = hostapd_ctrl_iface_pin_lock_status(hapd, reply,
								  reply_size);
#ifdef CONFIG_WPS_NFC
