				   ...
				   const char *capab)
{
	...
	if (os_strstr(capab, "[FORCE_BW]")) {
		/*[RTK]disable auto 20/40M*/
		conf->rtk_force_bandwidth = 1;
	}
	if (os_strstr(capab, "[HT40+]")) {
		conf->ht_capab |= HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET;
		conf->secondary_channel = 1;
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
	...
	} else if (os_strcmp(buf, "airtime_fairness") == 0) {
		bss->airtime_fairness = atoi(pos);
		...
		int len = os_strlen(pos);
		if (len < 8 || len > 64) {
			wpa_printf(MSG_ERROR, "Line %d: invalid WPA passphrase length %d (expected 8..64)",
				   line, len);
			return 1;
		}
		if(len == 64){
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	else if (os_strcmp(buf, "wapi_role") == 0) {
		bss->wapi_role = atoi(pos);
	} else if (os_strcmp(buf, "asu_ip") == 0) {
		if (!inet_aton(pos, &bss->asu_ip)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid asu ip", line);
			return 1;
		}
	} else if (os_strcmp(buf, "wapi_ca_cert") == 0) {
		os_free(bss->wapi_ca_cert);
		bss->wapi_ca_cert = os_strdup(pos);
	} else if (os_strcmp(buf, "wapi_asu_cert") == 0) {
		os_free(bss->wapi_asu_cert);
		bss->wapi_asu_cert = os_strdup(pos);
	} else if (os_strcmp(buf, "wapi_ae_cert") == 0) {
		os_free(bss->wapi_ae_cert);
		bss->wapi_ae_cert = os_strdup(pos);
	} else if (os_strcmp(buf, "key_update_time") == 0) {
		bss->key_update_time = atoi(pos);
	}
#endif
	...
	PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
		...
		if (atoi(pos))
			bss->radio_measurements[0] |=
				...
				WLAN_RRM_CAPS_BEACON_REPORT_MEASURE;
