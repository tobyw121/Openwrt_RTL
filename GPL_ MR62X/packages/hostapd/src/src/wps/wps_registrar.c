...
unsigned int get_backhaul_encrypt_mode_flag(void)
{
	FILE *fp=NULL;
	char buffer[10]={0};
	if((fp = fopen("/tmp/backhaul_encrypt_mode", "r")) == NULL)
	{
...
#ifdef CONFIG_USER_RTK_MULTI_AP_OPEN_ENCRYPT_SUPPORT
	unsigned char encryption_none_en = 0;
	encryption_none_en = get_backhaul_encrypt_mode_flag();
#endif 
...
#ifdef CONFIG_USER_RTK_MULTI_AP_OPEN_ENCRYPT_SUPPORT
		if(encryption_none_en){
			wps->cred.auth_type = WPS_AUTH_OPEN;
			wps->cred.encr_type = WPS_ENCR_NONE;
