...
{
	if (RSN_SELECTOR_GET(s) == RSN_CIPHER_SUITE_SMS4)
		return WPA_CIPHER_SMS4;
	if (RSN_SELECTOR_GET(s) == RSN_CIPHER_SUITE_GCM_SM4)
...
{
	if (RSN_SELECTOR_GET(s) == RSN_AUTH_KEY_MGMT_WAPI_PSK)
		return WPA_KEY_MGMT_WAPI_PSK;
	if (RSN_SELECTOR_GET(s) == RSN_AUTH_KEY_MGMT_WAPI_CERT)
		return WPA_KEY_MGMT_WAPI_CERT;
	
	return 0;
}

int wpa_parse_wpa_ie_wapi(const u8 *wapi_ie, size_t wapi_ie_len,
			 struct wpa_ie_data *data)
{
	const u8 *pos;
	int left;
	...
	if (wapi_ie_len < MIN_WAPI_IE_LEN) {
	...
	hdr = (const struct rsn_ie_hdr *) wapi_ie;

	if (hdr->elem_id != WLAN_EID_WAPI ||
	    hdr->len != wapi_ie_len - 2 ||
	    WPA_GET_LE16(hdr->version) != WAPI_VERSION) {
		wpa_printf(MSG_DEBUG, "%s: malformed ie or now just support WAPI 1.0",
			   __func__);
		return -2;
	}
	...
	if (left >= 2) {
		...
		wpa_printf(MSG_DEBUG, "%s: ie too short", __func__);
		return -7;
	}
	...
	if (left >= 4) {
		data->mgmt_group_cipher = wapi_selector_to_bitfield(pos);
		if (!wpa_cipher_valid_group(data->mgmt_group_cipher)) {
			wpa_printf(MSG_DEBUG,
	...
	if (left > 0) {
		wpa_hexdump(MSG_DEBUG,
			    "wpa_parse_wpa_ie_wapi: ignore trailing bytes",
			    pos, left);
	}
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "CCMP-256") == 0)
			...
			val |= WPA_CIPHER_GCMP;
	#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
		else if (os_strcmp(start, "WPI-SM4") == 0)
			val |= WPA_CIPHER_SMS4;
		else if (os_strcmp(start, "WPI-GCM-SM4") == 0)
			val |= WPA_CIPHER_GCM_SM4;
	#endif
...
int wpa_select_ap_group_cipher(int wpa, int wpa_pairwise, int rsn_pairwise)
{
	...
	if (wpa == WPA_PROTO_WAPI)
		pairwise |= rsn_pairwise;

	if (pairwise & WPA_CIPHER_SMS4)
		return WPA_CIPHER_SMS4;

	if (pairwise & WPA_CIPHER_GCM_SM4)
		return WPA_CIPHER_GCM_SM4;	
#endif
