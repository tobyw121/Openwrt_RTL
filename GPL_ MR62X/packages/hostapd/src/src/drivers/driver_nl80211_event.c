...
#define C2S(x) case x: return #x;
	...
	C2S(NL80211_CMD_WAPI_UPDATE_KEY)
#endif
	default:
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
void nl80211_wapi_update_key_event(struct wpa_driver_nl80211_data *drv, struct nlattr **tb)
{
	u8 *addr;
	enum nl80211_key_type key_type;

	if (tb[NL80211_ATTR_MAC] == NULL || tb[NL80211_ATTR_KEY_TYPE] == NULL)
		return;
	
	addr = nla_data(tb[NL80211_ATTR_MAC]);
	wpa_printf(MSG_DEBUG, "nl80211: mac " MACSTR, MAC2STR(addr));
	
	key_type = nla_get_u32(tb[NL80211_ATTR_KEY_TYPE]);
	wpa_printf(MSG_DEBUG, "nl80211: Key Type %d", key_type);
	
	if (key_type == NL80211_KEYTYPE_PAIRWISE)
		rtl_wapi_process_usk_update_event(addr);
	else if (key_type == NL80211_KEYTYPE_GROUP)
		rtl_wapi_process_msk_update_event(addr);
}
#endif
				 ...
				 struct nlattr **tb)
{
	...
	case NL80211_CMD_UPDATE_OWE_INFO:
		...
		break;
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	case NL80211_CMD_WAPI_UPDATE_KEY:
		nl80211_wapi_update_key_event(drv, tb);
		break;
#endif
	default:
