						 ...
						 u8 *buf, size_t len)
{
	...
	if (!drv->if_disabled && !(ifi->ifi_flags & IFF_UP)) {
		...
		if (os_strcmp(drv->first_bss->ifname, ifname) != 0) {
			...
			if(bss)
			{
				wpa_driver_nl80211_stop_ap(bss);

				wpa_supplicant_event(drv->ctx,
						     EVENT_INTERFACE_DISABLED,
						     (union wpa_event_data *)bss);
			}
			...
			wpa_driver_nl80211_stop_ap(drv->first_bss);
	...
	if (drv->if_disabled && (ifi->ifi_flags & IFF_UP)) {
		...
		wpa_printf(MSG_INFO, "nl80211: Interface up (%s)",
			   ifname);
		if (os_strcmp(drv->first_bss->ifname, ifname) != 0) {
			struct i802_bss *bss = NULL;
...
static int wpa_driver_nl80211_del_beacon(struct i802_bss *bss)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Remove \"%s\" beacon.",
		   bss->ifname);
	nl80211_put_wiphy_data_ap(bss);
...
static u32 wpa_alg_to_cipher_suite(enum wpa_alg alg, size_t key_len)
{
	...
	case WPA_ALG_GCM_SM4:
		return RSN_CIPHER_SUITE_GCM_SM4;
#endif
	case WPA_ALG_KRK:
				       ...
				       int max_suites)
{
	...
	if (num_suites < max_suites && ciphers & WPA_CIPHER_GCM_SM4)
		suites[num_suites++] = RSN_CIPHER_SUITE_GCM_SM4;
#endif
...
static int nl80211_set_bss(struct i802_bss *bss, int cts, int preamble,
			   int slot, int ht_opmode, int ap_isolate, int airtime_fairness,
			   const int *basic_rates)
{
	...
	if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_BSS)) ||
		...
		(airtime_fairness >= 0 &&
		nla_put_u8(msg, NL80211_ATTR_AIRTIME_FAIRNESS, airtime_fairness)) ||
				     ...
				     struct wpa_driver_ap_params *params)
{
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	if (params->wpa_version == WPA_PROTO_WAPI)
	...
	if (params->key_mgmt_suites & WPA_KEY_MGMT_WAPI_PSK)
		suites[num_suites++] = RSN_AUTH_KEY_MGMT_WAPI_PSK;
	if (params->key_mgmt_suites & WPA_KEY_MGMT_WAPI_CERT)
		suites[num_suites++] = RSN_AUTH_KEY_MGMT_WAPI_CERT;
#endif
	if (num_suites &&
	...
	if (ret) {
		...
		nl80211_set_bss(bss, params->cts_protect, params->preamble,
				...
				params->isolate, params->airtime_fairness, params->basic_rates);
...
int nl80211_sta_auth(void *priv, struct wpa_driver_sta_auth_params *params)
{
	...
	wpa_driver_nl80211_send_mlme(bss, buf, rlen, 0, 0, 0, 0, 0, NULL, 0);
