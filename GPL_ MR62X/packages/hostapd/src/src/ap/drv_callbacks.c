			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	if (elems.wps_ie) {
		...
		wpa_printf(MSG_DEBUG, "STA included WAPI IE in (Re)AssocReq");
#endif
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	if (elems.wapi_ie && sta) {
		memcpy(sta->wapi_ie, ie, ielen);
		sta->wapi_ie_len = ielen;
	}
#endif
...
void hostapd_notif_disassoc(struct hostapd_data *hapd, const u8 *addr)
{
	...
	if (hapd->conf->wpa == WPA_PROTO_WAPI)
		rtl_wapi_process_disassoc_event(hapd->own_addr, sta->addr);
#endif
	ap_free_sta(hapd, sta);
}
			     ...
			     int finished)
{
	...
	wpa_msg(hapd->msg_ctx, MSG_INFO,
		"%sfreq=%d ht_enabled=%d ch_offset=%d ch_width=%s cf1=%d cf2=%d dfs=%d channel=%d",
		finished ? WPA_EVENT_CHANNEL_SWITCH :
		WPA_EVENT_CHANNEL_SWITCH_STARTED,
		freq, ht, offset, channel_width_to_string(width),
		cf1, cf2, is_dfs,channel);
	if (!finished)
		return;
			       ...
			       struct auth_info *rx_auth)
{
	...
	if (rx_auth->auth_type == WLAN_AUTH_FT && hapd->wpa_auth) {
		...
		wpa_printf(MSG_DEBUG,
			"FT: set auth_alg(%d) to WLAN_AUTH_OPEN", sta->auth_alg);
