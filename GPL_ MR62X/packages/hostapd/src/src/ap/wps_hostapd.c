...
{
	struct hostapd_data *hapd = ctx;
	int is_beacon_same = 0, is_probe_resp_same = 0;//Realtek add

	//Realtek add
	if(hapd->wps_beacon_ie && beacon_ie)
	{
		if(hapd->wps_beacon_ie->buf && hapd->wps_beacon_ie->used && beacon_ie->buf && beacon_ie->used)
		{
			if(hapd->wps_beacon_ie->used == beacon_ie->used)
			{
				if(memcmp(hapd->wps_beacon_ie->buf, beacon_ie->buf, hapd->wps_beacon_ie->used) == 0)
				{
					is_beacon_same = 1;
					wpa_printf(MSG_DEBUG, "WPS: beacon ie same");
				}
			}
		}
	}
	...
	//Realtek add
	if(hapd->wps_probe_resp_ie && probe_resp_ie)
	{
		if(hapd->wps_probe_resp_ie->buf && hapd->wps_probe_resp_ie->used && probe_resp_ie->buf && probe_resp_ie->used)
		{
			if(hapd->wps_probe_resp_ie->used == probe_resp_ie->used)
			{
				if(memcmp(hapd->wps_probe_resp_ie->buf, probe_resp_ie->buf, hapd->wps_probe_resp_ie->used) == 0)
				{
					is_probe_resp_same = 1;
					wpa_printf(MSG_DEBUG, "WPS: proberesp ie same");
				}
			}
		}
	}
	...
	//Realtek add
	if(is_beacon_same && is_probe_resp_same)
	{
		wpa_printf(MSG_DEBUG, "WPS: all same and not change ie");
		return 0;
	}
...
{
	/* Update WPS PIN Status:PIN Timeout */
	hapd->wps_stats.pin_status = WPS_PIN_STATUS_TIMEOUT;
	hapd->wps_stats.status = WPS_STATUS_NONE;
}

static void hostapd_wps_event_pin_active(struct hostapd_data *hapd)
{
	/* Update WPS PIN status - Active */
	hapd->wps_stats.pin_status = WPS_PIN_STATUS_ACTIVE;
	hapd->wps_stats.status = WPS_STATUS_NONE;
}

static void hostapd_wps_event_pin_disable(struct hostapd_data *hapd)
{
	/* Update WPS PIN status - Active */
	hapd->wps_stats.pin_status = WPS_PIN_STATUS_DISABLE;
}
...
static void hostapd_wps_event_clean_status(struct hostapd_data *hapd)
{
	/* clean WPS PBC status */
	hapd->wps_stats.pbc_status = WPS_PBC_STATUS_DISABLE;
	hapd->wps_stats.status = 0;
	os_memset(hapd->wps_stats.peer_addr, 0, ETH_ALEN);
}
				 ...
				 union wps_event_data *data)
{
	...
	case WPS_EV_PIN_ACTIVE:
		hostapd_wps_event_pin_active(hapd);
		wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_ACTIVE);
		break;
	case WPS_EV_PIN_DISABLE:
		hostapd_wps_event_pin_disable(hapd);
		wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_DISABLE);
		break;
		     ...
		     struct hostapd_bss_config *conf)
{
	...
	if (conf->wpa & WPA_PROTO_RSN) {
		...
		if (conf->wpa_key_mgmt & WPA_KEY_MGMT_PSK_SHA256)//Realtek add
			wps->auth_types |= WPS_AUTH_WPA2PSK;
...
static int wps_add_pin(struct hostapd_data *hapd, void *ctx)
{
	...
	if (hapd->wps == NULL)
		return 0;
	hostapd_wps_event_clean_status(hapd);
