...
int wpa_write_wapi_ie(struct wpa_auth_config *conf, u8 *buf, size_t len)
{
	size_t wapi_ie_len;
	
	u8 buf_ie[64] = {0x44,20,1,0, 1,0,0,0x14,0x72,1, 1,0,0,0x14,0x72,1, 0,0x14,0x72,1, 0, 0};/* little endian*/

	if (conf->wpa_key_mgmt == WPA_KEY_MGMT_WAPI_PSK)
		buf_ie[9] = 2;

	if (conf->rsn_pairwise == WPA_CIPHER_GCM_SM4)
		buf_ie[15] = 2;

	if (buf_ie[19] != buf_ie[15])
		buf_ie[19] = buf_ie[15];

	wapi_ie_len = buf_ie[1]+2;

	if(wapi_ie_len > len) {
		wpa_printf(MSG_ERROR, "Invalid wapi ie len (%d).", wapi_ie_len);
		return -1;
	}

	os_memcpy(buf, buf_ie, wapi_ie_len);
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	if (wpa_auth->conf.wpa == WPA_PROTO_WAPI) {
		res = wpa_write_wapi_ie(&wpa_auth->conf,
			...
			const u8 *owe_dh, size_t owe_dh_len)
{
	...
	if (version == WPA_PROTO_RSN) {
		...
		res = wpa_parse_wpa_ie_wapi(wpa_ie, wpa_ie_len, &data);
#endif
	} else {
	...
	if (0) {
		...
		sm->wpa_key_mgmt = WPA_KEY_MGMT_WAPI_PSK;
	else if (key_mgmt & WPA_KEY_MGMT_WAPI_CERT)
		sm->wpa_key_mgmt = WPA_KEY_MGMT_WAPI_CERT;
#endif
#ifdef CONFIG_OWE
	...
	if (!ciphers) {
		...
		wpa_printf(MSG_DEBUG, "Invalid %s pairwise cipher (0x%x) "
			   "from " MACSTR,
			   version == WPA_PROTO_RSN ? "RSN" : version == WPA_PROTO_WAPI ? "WAPI" : "WPA",
