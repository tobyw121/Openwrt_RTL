...
#else /* TP_ROAMING_SUPPORT */
int beacon_report_parse(char *d, char *s)
{
	int len = 0;
	memset(d, 0, sizeof(d));
	if (strlen(s) < 42)
	...
	memcpy(d + len, s + 30, 12);	//copy bssid
	len += 12;
	memcpy(d + len, s + 26, 2);		//copy rcpi
	len += 2;
	memcpy(d + len, s + 2, 2);		//copy channel
	len += 2;
...
static void hostapd_handle_beacon_report_merge(struct hostapd_data *hapd,
					 const u8 *addr, char *buf, size_t num)
{
	if (num == 0)
		return;

	wpa_msg(hapd->msg_ctx, MSG_INFO, BEACON_RESP_RX MACSTR " %d %s", MAC2STR(addr), num, buf);
}
#endif /* TP_ROAMING_SUPPORT */
					     ...
					     const u8 *buf, size_t len)
{
	...
	while ((ie = get_ie(pos, end - pos, WLAN_EID_MEASURE_REPORT))) {
		...
		case MEASURE_TYPE_BEACON:
			...
			if ((ie[1] > 3) && (wpa_snprintf_hex(tmp_buf1, sizeof(report), ie + 5, ie[1] - 3) > 0)) {
				tmp_len = beacon_report_parse(tmp_buf2, tmp_buf1);
				if (tmp_len > 0) {
					memcpy(report + len2, tmp_buf2, tmp_len);
					len2 += tmp_len;
					num++;
				}
			}
...
#ifdef TP_ROAMING_SUPPORT
	hostapd_handle_beacon_report_merge(hapd, mgmt->sa, report, num);
#endif /* TP_ROAMING_SUPPORT */
}
