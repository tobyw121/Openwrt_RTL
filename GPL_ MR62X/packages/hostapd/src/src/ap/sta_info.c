...
#ifdef WFO_VIRT
	hapd->conf->ap_max_inactivity = AP_MAX_INACTIVITY;
#endif /* WFO_VIRT */
			   ...
			   int authorized)
{
	...
	if (authorized) {
		...
		wpa_msg(hapd->msg_ctx, MSG_INFO, AP_STA_CONNECTED "%s%s%s 11k=%d",
			buf, ip_addr, keyid_buf, 
			(sta->rrm_enabled_capa[0] & (WLAN_RRM_CAPS_NEIGHBOR_REPORT | WLAN_RRM_CAPS_BEACON_REPORT_PASSIVE |
				 WLAN_RRM_CAPS_BEACON_REPORT_ACTIVE | WLAN_RRM_CAPS_BEACON_REPORT_TABLE)) ? 1 : 0);
