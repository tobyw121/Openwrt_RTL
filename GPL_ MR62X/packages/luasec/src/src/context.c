...
{
  if (!strcmp(method, "sslv3"))  return SSLv3_client_method();
  if (!strcmp(method, "tlsv1"))  return TLSv1_client_method();
  if (!strcmp(method, "sslv23")) return SSLv23_method();
  if (!strcmp(method, "tlsv1.2")) return TLSv1_2_client_method();
  if (!strcmp(method, "tlsv1.1")) return TLSv1_1_client_method();
  return NULL;
}
