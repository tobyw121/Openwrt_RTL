...
#if defined(CONFIG_IGMP_DEBUG)
int igmpsnoop_debug=0;
#endif

#if defined (CONFIG_RTL_HW_MCAST_WIFI)
int hwwifiEnable=0;
#endif
int igmp_netfilter = 0;//default disable
int check_last_member = 0;
...
extern struct rtl865x_blackListMCastRecord blackListMacRecord[MAX_BLACKLIST_MAC_NUM];
...
#endif
#if defined(RTL_MODIFY_IGMP_EXPIRE_TIME)
extern unsigned long igmp_expire_time;
int rtl_modifyIgmpM2UExpireTime(int groupTimer);
#endif
...
extern void rtl_modifyGroupTimer(int groupAgeTime);
...
#endif
{
	...
	rtl_getMulticastParameters(&mCastTimerParameters);
	groupAgeTime = mCastTimerParameters.groupMemberAgingTime;
	
#ifdef CONFIG_RTL_PROC_NEW
	seq_printf(s, "group aging time:%d\n\n",groupAgeTime);
#else
	len += sprintf(page+len, "group aging time::%d\n\n", groupAgeTime);
#endif

#ifdef CONFIG_RTL_PROC_NEW
	seq_printf(s, "igmp netfilter:%c\n\n",igmp_netfilter + '0');
#else
    len += sprintf(page, "igmp netfilter:%c\n\n", igmp_netfilter + '0');
#endif
#if defined (IMPROVE_MCAST_PERFORMANCE_WITH_RTL8367)
#ifdef CONFIG_RTL_PROC_NEW
...
#ifdef CONFIG_RTL_PROC_NEW
	cnt = 0;
	seq_printf(s,  "blackList mac address:\n");	
	for(i=0; i<MAX_BLACKLIST_MAC_NUM; i++)
	{
		if(blackListMacRecord[i].valid==1)
		{		
			cnt++;
			seq_printf(s, "    [%d] Group address:0x%02X:%02X:%02X:%02X:%02X:%02X\n",cnt,
			blackListMacRecord[i].mac[0],blackListMacRecord[i].mac[1],blackListMacRecord[i].mac[2],
			blackListMacRecord[i].mac[3],blackListMacRecord[i].mac[4],blackListMacRecord[i].mac[5]);
		}
	}
	if(cnt==0)
		seq_printf(s, "NULL\n");
...
#endif
#ifdef CONFIG_RTL_PROC_NEW
	seq_printf(s,  "Check last member:%d\n\n",check_last_member); 
	seq_printf(s, "\n");
#else
	len += sprintf(page+len, "Check last member:%d\n\n",check_last_member); 
	len += sprintf(page+len, "\n");
#endif
...
#if defined(CONFIG_IGMP_DEBUG)
void dump_br_portnum_list(const char* br_dev_name)
{
	struct net_device *br_dev = NULL;
	struct net_bridge *br = NULL;
	struct net_bridge_port *p, *n;
	unsigned int all_port_bitmask=0;
	
	if(!br_dev_name)
	{
		printk("ERROR: br_dev_name is NULL\n");
		return;
	}
	
	br_dev = dev_get_by_name(&init_net, br_dev_name);
	if (br_dev)
	{
		if (br_dev->priv_flags & IFF_EBRIDGE) {
			br = netdev_priv(br_dev);

			printk("bridge[%s] port list:\n", br_dev_name);
			list_for_each_entry_safe(p, n, &br->port_list, list) 
			{
				all_port_bitmask |= (1 << p->port_no);
				printk("%s, port num %d\n", p->dev->name, p->port_no);
			}
			printk("all port mask: 0x%x\n", all_port_bitmask);
		}
	...
	{
		printk("ERROR: br_dev[%s] is not exist\n", br_dev_name);
	}
}
#endif
...
#endif
			else if(!memcmp(tokptr, "grouptime", 4))
			{
				tokptr = strsep(&strptr," ");
				if (tokptr==NULL)
				{
              	...
              	groupAgeTime = simple_strtol(tokptr, NULL, 0);
				if(groupAgeTime !=0)
				{
					memset(&mCastTimerParameters, 0, sizeof(struct rtl_mCastTimerParameters));
					mCastTimerParameters.groupMemberAgingTime = groupAgeTime;
					rtl_setMulticastParameters(mCastTimerParameters);
					rtl_modifyGroupTimer(groupAgeTime);

			#if defined(RTL_MODIFY_IGMP_EXPIRE_TIME)
				    rtl_modifyIgmpM2UExpireTime(groupAgeTime);
					igmp_expire_time = groupAgeTime;
			#endif
				}
			}
			else if(!memcmp(tokptr,"netfilter",9))
			{
				tokptr = strsep(&strptr," ");
				if (tokptr==NULL)
				{
				...
				igmp_netfilter = simple_strtol(tokptr, NULL, 0);
			}
			else if(!memcmp(tokptr, "lastMemberCheck", 15))
			{
				tokptr = strsep(&strptr," ");
				if (tokptr==NULL)
				{
				...
				check_last_member = simple_strtol(tokptr, NULL, 0);
			}
...
#endif           
#endif		
			else if(!memcmp(tokptr, "macFilter", 9))
			{
				flag = -1;
				...
				|| (!memcmp(tokptr, "Clear", 5))
				||(!memcmp(tokptr, "CLEAR", 5)))	
				{
					flag = 2;
					rtl_set_blackList_mac(mac, flag);
					return count;
				}
				else
				{
				...
				string_to_hex(tokptr, mac, 12);
				
				rtl_set_blackList_mac(mac, flag); 
			}
#if defined(CONFIG_IGMP_DEBUG)
			else if(!memcmp(tokptr,"debug",5))
			{
				tokptr = strsep(&strptr," ");
				if (tokptr==NULL)
				{
				...
				igmpsnoop_debug = simple_strtol(tokptr, NULL, 0);
			}
			else if(!memcmp(tokptr,"portList",8))
			{
				int bridge_id = 0;
				tokptr = strsep(&strptr," ");
				if (tokptr)
				{
					bridge_id = simple_strtol(tokptr, NULL, 0);
				}

				if(bridge_id == 2 /*br-iptv*/)
				{
					dump_br_portnum_list("br-iptv");
				}
				else /*br-lan */
				{
					dump_br_portnum_list("br-lan");
				}
			}
#endif
			else
...
void br_mCastQueryTimerExpired(unsigned long arg)
{
	...
	//printk("[%s:%d]\n", __FUNCTION__, __LINE__);
		...
		{
			//printk("br:%s, [%s:%d]\n", br->dev->name, __FUNCTION__, __LINE__);
...
extern int rtl_mc_entry_add(uint32 *srcAddr, uint32 *grpAddr, int32 portNum, int8 filterMode, int32 wanOps, int32 wanPPP, int32 ipVersion);
extern int rtl_mc_entry_del(uint32 *srcAddr, uint32 *grpAddr, int32 portNum, int32 ipVersion);
#endif
#if 0//defined(CONFIG_RTL_819X)&&defined (CONFIG_RTL_IGMP_SNOOPING)
extern struct net_bridge *find_br_by_name(char *name);
void br_signal_igmpProxy(void)
{
	struct task_struct *task;
...
#if defined(CONFIG_RTL_HARDWARE_MULTICAST_API)
#define 	HW_MC_API_INCLUDE		0x0
#define 	HW_MC_API_EXCLUDE		0x1
int rtl_getPortNumbyBridgePort(struct net_bridge_port *pst_prt)
{
	return -1;
}
int hi_mc_igmp_entry_add(int32 i_mode,struct in_addr *pst_src, struct in_addr *pst_grp,struct net_bridge_port *pst_prt,int32 i_wan_ops,int32 i_wan_ppp)
{
	uint32 groupAddr[1]={0}, srcAddr[1]={0};
	int32 portNum = -1;
	if(pst_grp==NULL || pst_prt==NULL || (i_mode==HW_MC_API_INCLUDE&&pst_src==NULL))
		return FAILED;
	if(pst_src==NULL)
		srcAddr[0] = 0;
	else
		srcAddr[0] = htonl(pst_src->s_addr);
	groupAddr[0] = htonl(pst_grp->s_addr);
	portNum = rtl_getPortNumbyBridgePort(pst_prt);
	rtl_mc_entry_add(srcAddr,groupAddr,portNum,i_mode, i_wan_ops,i_wan_ppp, 4);
	return SUCCESS;
}
int hi_mc_igmp_entry_del(int32 i_mode,struct in_addr *pst_src,struct in_addr *pst_grp, struct net_bridge_port *pst_prt)
{
	uint32 groupAddr[1]={0}, srcAddr[1]={0};
	int32 portNum = -1;
	if(pst_grp==NULL || pst_prt==NULL)
		return FAILED;
	if(pst_src==NULL)
		srcAddr[0] = 0;
	else
	...
	groupAddr[0] = htonl(pst_grp->s_addr);
	portNum = rtl_getPortNumbyBridgePort(pst_prt);
	rtl_mc_entry_del(srcAddr,groupAddr,portNum,4);
	return SUCCESS;
}
int hi_mc_mld_entry_add(int32 i_mode,struct in6_addr *pst_src,struct in6_addr *pst_grp,struct net_bridge_port *pst_prt, int32 i_wan_ops,int32 i_wan_ppp)
{
	...
	{
		srcAddr[0] = htonl(pst_src->s6_addr32[0]);
		srcAddr[1] = htonl(pst_src->s6_addr32[1]);
		srcAddr[2] = htonl(pst_src->s6_addr32[2]);
		srcAddr[3] = htonl(pst_src->s6_addr32[3]);
	}
	groupAddr[0] = htonl(pst_grp->s6_addr32[0]);
	groupAddr[1] = htonl(pst_grp->s6_addr32[1]);
	groupAddr[2] = htonl(pst_grp->s6_addr32[2]);
	groupAddr[3] = htonl(pst_grp->s6_addr32[3]);
	portNum = rtl_getPortNumbyBridgePort(pst_prt);
	rtl_mc_entry_add(srcAddr,groupAddr,portNum,i_mode, i_wan_ops,i_wan_ppp,6);
	return SUCCESS;
}
int hi_mc_mld_entry_del(int32 i_mode,struct in6_addr *pst_src,struct in6_addr *pst_grp,struct net_bridge_port *pst_prt)
{
	uint32 groupAddr[4]={0}, srcAddr[4]={0};
	int32 portNum = -1;
	if(pst_grp==NULL || pst_prt==NULL)
	...
	groupAddr[3] = htonl(pst_grp->s6_addr32[3]);
	portNum = rtl_getPortNumbyBridgePort(pst_prt);
	rtl_mc_entry_del(srcAddr,groupAddr,portNum,6);
	return SUCCESS;
}
#endif
...
#if defined(CONFIG_TP_BRIDGE) || defined(CONFIG_TP_IMAGE)
