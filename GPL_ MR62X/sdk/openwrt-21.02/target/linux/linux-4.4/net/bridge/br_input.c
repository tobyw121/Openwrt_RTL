...
int br_handle_frame_finish(struct net *net, struct sock *sk, struct sk_buff *skb)
{
	...
	if (skb2)
		...
		if((MULTICAST_MAC(dest)||IPV6_MULTICAST_MAC(dest)) && (strcmp(br->dev->name, RTL_PS_BR0_DEV_NAME)!=0 && strcmp(br->dev->name, RTL_DRV_BR_WAN_NETIF_NAME) != 0))
		{
		}
		else
		{
...
#endif
	{
...
#endif
					{
						...
						{	
							...
							rtl_igmpMldProcess(igmpModuleIndex, skb_mac_header(skb), p->port_no, &fwdPortMask, (skb->tail - skb_mac_header(skb)));
			#else
	        	#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
							if(!strcmp(br->dev->name,RTL_PS_BR1_DEV_NAME))
							{
								rtl_igmpMldProcess(brIgmpModuleIndex_2, skb_mac_header(skb), p->port_no, &fwdPortMask, (skb->tail - skb_mac_header(skb)));
								//flooding igmp packet
								fwdPortMask=(~(1<<(p->port_no))) & 0xFFFFFFFF;
							}
							else
			#endif
							rtl_igmpMldProcess(brIgmpModuleIndex, skb_mac_header(skb), p->port_no, &fwdPortMask, (skb->tail - skb_mac_header(skb)));
			#endif
...
#endif
					{
						...
						else if ((proto ==IPPROTO_UDP) ||(proto ==IPPROTO_TCP)||(proto ==Any_0_hop_protocl))
						{
							...
							br_multicast_forward(br, multicastFwdInfo.fwdPortMask, skb, 0);
#if 0
							printk_ratelimited("[%s:%d]ipv6 TCP&&UDP,ret=%d,mask=%X,cpuFlag=%d,srcVlanId=%d,srcPort=%d\n",
								__FUNCTION__,
								__LINE__,
								ret,
...
#if defined(CONFIG_RTL_DNS_TRAP) && !defined(CONFIG_TP_IMAGE)
          	...
          	for(i=0;i< ntohs(mldv2report->mld2r_ngrec);i++)
           	{
            	if(i==0)
               	...
               	if (((unsigned char*)mldv2grec + 4 + 16) > (unsigned char*)skb->tail 
					|| (unsigned char*)((unsigned char*)mldv2grec+4+16+ntohs(mldv2grec->grec_nsrcs)*16+(mldv2grec->grec_auxwords)*4) > (unsigned char*)skb->tail)
               	{
	...
	*moreFlag=0;
	    ...
	    (igmph->type==IGMPV2_HOST_MEMBERSHIP_REPORT)) 
	{
		...
		for(i=0;i<ntohs(igmpv3report->ngrec);i++)
		{
			...
			if (((unsigned char*)igmpv3grec + 8) > (unsigned char*)skb->tail 
				|| (unsigned char*)((unsigned char*)igmpv3grec+8+ntohs(igmpv3grec->grec_nsrcs)*4+(igmpv3grec->grec_auxwords)*4) > (unsigned char*)skb->tail)
			{
				printk_ratelimited("%s:%d groupRecords error.\n", __func__, __LINE__);
				*moreFlag=0;
				return -1;
