...
EXPORT_SYMBOL(dns_filter_enable);
...
#define LANIF	"br-lan"
#define LAN_ALIAS	"br-lan:1"    // alias for secondary IP
...
#ifdef CONFIG_IPV6
struct udphdr *ipv6_find_udp_hdr(struct sk_buff *skb, struct udphdr *udph)
{
	int uproto;
	unsigned int uhoff=0;
	//struct udphdr udph_tmp_find,*udph;;
	uproto = ipv6_find_hdr(skb, &uhoff, -1, NULL, NULL);	
...
#ifdef CONFIG_IPV6
int br_dns_packet_recap(struct sk_buff *skb,short type,short ip_ver)
#else
...
#endif
{
	...
	struct iphdr *iph;
	struct udphdr *udph,udph_temp;
	struct net_device *br0_dev=NULL; 
	struct in_device *br0_in_dev=NULL;
	dnsheader_t *dns_pkt;
	...
	br0_dev = dev_get_by_name(&init_net,LANIF); 
	if(br0_dev)
	{
	...
	if (ip_ver == 1) {
		ipv6h = ipv6_hdr(skb);
		udph = ipv6_find_udp_hdr(skb, &udph_temp);
		if (NULL == udph){
	...
	if ((p==NULL) || (p->br==NULL) || (p->br->dev==NULL))
	{
		if (br0_in_dev)
			in_dev_put(br0_in_dev);
		if (br0_dev)
	...
	memcpy(eth_hdr(skb)->h_dest, eth_hdr(skb)->h_source, ETH_ALEN);
	memcpy(eth_hdr(skb)->h_source, p->br->dev->dev_addr, ETH_ALEN);
	...
	DBGP_DNS_TRAP("[%s:%d]udph->len:%d\n",__FUNCTION__,__LINE__,ntohs(udph->len));
	DBGP_DNS_TRAP("[%s:%d]dns_pkt->u:%x\n",__FUNCTION__,__LINE__,ntohs(dns_pkt->u));
	DBGP_DNS_TRAP("[%s:%d]dns_pkt->qdcount:%x\n",__FUNCTION__,__LINE__,ntohs(dns_pkt->qdcount));
	DBGP_DNS_TRAP("[%s:%d]dns_pkt->ancount:%x\n",__FUNCTION__,__LINE__,ntohs(dns_pkt->ancount));
	DBGP_DNS_TRAP("[%s:%d]dns_pkt->nscount:%x\n",__FUNCTION__,__LINE__,ntohs(dns_pkt->nscount));
	DBGP_DNS_TRAP("[%s:%d]dns_pkt->arcount:%x\n",__FUNCTION__,__LINE__,ntohs(dns_pkt->arcount));
	/* pad Answers */
	if (type == 0x01)
	{
		int exist_br0_ip = 0;
		...
		DBGP_DNS_TRAP("[%s:%d] br0 addr4: %pI4 \n",__FUNCTION__,__LINE__, &ifa_address);
	...
	else if (type == 0x1c)
	{
		...
		if (ifa != NULL){
			...
			DBGP_DNS_TRAP("[%s:%d] br0 addr6: %pI6c \n",__FUNCTION__,__LINE__, &(ifa->addr));
		}
		else
			memcpy(ptr+12,0,16);
...
static int is_domain_name_equal(char *domain_name1, char * domain_name2)
{
	...
	if (!strncmp(domain_name1,"www.",4)){
		strlcpy(temp1,domain_name1+4,sizeof(temp1));
	}
	else{
		strlcpy(temp1,domain_name1,sizeof(temp1));
	}
	if (!strncmp(domain_name2,"www.",4)){
		strlcpy(temp2,domain_name2+4,sizeof(temp2));
	}
	else{
		strlcpy(temp2,domain_name2,sizeof(temp2));
	}
...
static int is_valid_dns_query_header(dnsheader_t *dns_header)
{
	...
	if (((dns_header->u & 0x7400)>>11) != 0)/*opcode: 0:standard,1:reverse,2:server status*/
	{
...
int br_dns_filter_enter(struct sk_buff *skb)
{
	...
	if (iph && iph->version == 4){	// skb is ipv4
	    ip_ver =0;
		udph = (void *)iph + iph->ihl*4;
	...
	#ifdef CONFIG_IPV6	
	else if (ipv6h && ipv6h->version == 6){	// skb is ipv6
		ip_ver =1;
		udph = ipv6_find_udp_hdr(skb, &udph_temp);		
    ...
    if (is_dns_pkt)
	{
		...
		if(trap_all){
			#ifdef CONFIG_IPV6
			br_dns_packet_recap(skb,type,ip_ver);
			#else
		      ...
		      unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(domain_name, buffer, sizeof(domain_name))) {
...
int __init br_dns_filter_init(void)
{
	...
	strlcpy(domain_name, DEFAULT_HOSTNAME, sizeof(domain_name));
	str_to_lower(domain_name);
	return 0;
}
