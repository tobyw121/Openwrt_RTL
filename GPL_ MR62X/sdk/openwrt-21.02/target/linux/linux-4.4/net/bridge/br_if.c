...
#if defined (CONFIG_RTK_MESH) || defined (CONFIG_RTL_IGMP_SNOOPING) //||defined(CONFIG_BRIDGE_IGMP_SNOOPING)
...
{
	unsigned int ret=RTL_IMGP_MAX_BRMODULE;
	int i=0;
	for (i=0;i<RTL_IMGP_MAX_BRMODULE;i++)
	{
...
{
	int ret=-1;
	int index=RTL_IMGP_MAX_BRMODULE;
...
int br_add_bridge(struct net *net, const char *name)
{
	...
	br_register_igmpsnoopingmodule_process(netdev_priv(dev),1,0);
#else
...
rtk_dev_name_netif_mapping(br->dev->name, dev->name, 1);
		...
		if(!strcmp(dev->name, "wlan0-msh0") || !strcmp(dev->name, "wlan1-msh0")) {
			mesh_port_no |= (1<<p->port_no);
		}
