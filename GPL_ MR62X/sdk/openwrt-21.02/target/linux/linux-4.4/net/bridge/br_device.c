	...
	get_dst_hook = rcu_dereference(br_get_dst_hook);
		...
		if (dest[0] & 1)
		{
			...
			{	
				...
				{
				/* TODO: Does the multicast table of hyfi-bridge need to be maintained? */
				#if 0 //#ifdef CONFIG_TP_BRIDGE
                    ...
                    //224.0.0.9: RIPv2 packet DIP
			#if defined(CONFIG_USB_UWIFI_HOST)
					if(iph->daddr == htonl(0xEFFFFFFA) || iph->daddr == htonl(0xE1010101) || iph->daddr == htonl(0xE0000009))
			#else
					if(iph->daddr == htonl(0xEFFFFFFA) || iph->daddr == htonl(0xE00000FB) || iph->daddr == htonl(0xE0000009))
			#endif
						{
						/*for microsoft upnp and RIPv2*/
							reserved=1;
						}
					...
					/* 
					 * TTL: 1 -> 128 -> 127
					 * If TTL is equal to 1, hardware acceleration is not allowed.
					 * Otherwise, multicast packets cannot be forwarded between different VLANs. 
					 */
					if(tp_need_block_multicast_group(multicastDataInfo.ipVersion, multicastDataInfo.groupAddr, srcVlanId) == 0
						&& iph->ttl != 127 && 
						((hwwifiEnable && (ret==SUCCESS)) ||
					...
					if((ret==SUCCESS) && (multicastFwdInfo.cpuFlag==0))
					#endif
						{
								...
								if((srcVlanId!=0) && (srcPort!=0xFFFF))
								{
									...
									#else
									//printk_ratelimited("HW(TX)! group=0x%x, src_vlan=%d, cpuFlag=%x, fwdPortMask =0x%x,mapPortMask=0x%x\n", multicastDataInfo.groupAddr[0], srcVlanId,multicastFwdInfo.cpuFlag, multicastFwdInfo.fwdPortMask, mapPortMask);
									rtl865x_ipMulticastHardwareAccelerate(br, multicastFwdInfo.fwdPortMask,srcPort,srcVlanId, multicastDataInfo.sourceIp[0], multicastDataInfo.groupAddr[0], mapPortMask);
									#endif
...
static int netif_br_wan_Initialized = 0;
#endif
...
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)
		rtl865x_set_hwaddr(dev, addr);
#endif
	}
#if defined(CONFIG_RTL_BR_WAN_VLAN_SUPPORT)	&& defined(CONFIG_TP_IMAGE)
	else if(!netif_br_wan_Initialized && strncmp(dev->name, "br-wan", 6) == 0)
	{
		rtl865x_set_hwaddr(dev, addr);
		netif_br_wan_Initialized = 1;
	}
#endif
...
struct net_device *br_get_first_slave(struct net_device *br_if)
{
	struct net_bridge *br;
	struct net_bridge_port *p;

	if ((br_if->priv_flags & IFF_EBRIDGE) == 0)
		return NULL;
	br = netdev_priv(br_if);
	p = list_first_entry(&br->port_list, struct net_bridge_port, list);
	return p->dev;
}
...
extern uint32 rtl_get_wlan_port_mask_by_dev_name(char *name);
...
{	
	unsigned int wlanPhyPortMask = 0;
	...
	list_for_each_entry_safe(p, n, &br->port_list, list) 
	{
		...
		if(port_bitmask&brFwdPortMask)
		{
			if(strncmp(p->dev->name, RTL_WLAN_NAME, 4) == 0)
			{
				bindmask = rtl_get_wlan_port_mask_by_dev_name(p->dev->name);
				if(bindmask)
					wlanPhyPortMask |= bindmask;
			}
		}
	}
	return wlanPhyPortMask;
}
#endif
...
#endif
{
	...
	int joinBrNum = 0;
#if defined(CONFIG_RTL_VLAN_8021Q)&&defined(CONFIG_RTL_MULTICAST_PORT_MAPPING)
	unsigned int wlanPhyPortMask = 0xFFFFFFFF;
	struct net_bridge *br_tmp = NULL;
#endif
#endif
	...
	if(hwwifiEnable == 0)
	{
		...
		for (i=0;i<RTL_IMGP_MAX_BRMODULE;i++)
		{
			...
			{
				br_tmp = rtl_get_brByIndex(i, brName);
				wlanPhyPortMask = rtl865x_getWlanPhyFwdPortMaskbyVlan(br_tmp, multicastFwdInfo.fwdPortMask);
				if(wlanPhyPortMask&mapPortMask==0) //forward to wlan in other vlan
