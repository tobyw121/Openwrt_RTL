...
#if defined(RTL_MODIFY_IGMP_EXPIRE_TIME)
unsigned long igmp_expire_time = IGMP_EXPIRE_TIME;
...
atomic_t loop_pkt_cnt;
EXPORT_SYMBOL_GPL(loop_pkt_cnt);
...
static void fdb_delete(struct net_bridge *br, struct net_bridge_fdb_entry *f)
{
	...
	//printk("[%s:%d]=========== f->addr.addr is 0x%x:%x:%x:%x:%x:%x==========\n",__FUNCTION__,__LINE__,
		...
		if(ent->igmp_fdb_arr[i2].valid == 1){
			...
			if(time_before_eq(ent->igmp_fdb_arr[i2].ageing_time, igmp_walktimeout))
			{
				...
				if (dev) {		
					...
					#ifdef CONFIG_RTL_MESH_SINGLE_IFACE
						dev = __dev_get_by_name(&init_net, RTL_PS_MESH_DEV_NAME);					
						if (dev) {		
							unsigned char StaMacAndGroup[20];
							memcpy(StaMacAndGroup, DA , 6);
							memcpy(StaMacAndGroup+6, SA, 6);
							...
							if (dev->netdev_ops->ndo_do_ioctl != NULL) {
								dev->netdev_ops->ndo_do_ioctl(dev, (struct ifreq*)StaMacAndGroup, 0x8B81);
							#endif
								DEBUG_PRINT("(fdb expire) %s ioctl to DEL! M2U entry da:%02x:%02x:%02x-%02x:%02x:%02x; sa:%02x:%02x:%02x-%02x:%02x:%02x\n",
								RTL_PS_MESH_DEV_NAME,StaMacAndGroup[0],StaMacAndGroup[1],StaMacAndGroup[2],StaMacAndGroup[3],StaMacAndGroup[4],StaMacAndGroup[5],
								StaMacAndGroup[6],StaMacAndGroup[7],StaMacAndGroup[8],StaMacAndGroup[9],StaMacAndGroup[10],StaMacAndGroup[11]);
							}
						}
					#else	//!CONFIG_RTL_MESH_SINGLE_IFACE
						dev = __dev_get_by_name(&init_net, RTL_PS_MESH1_DEV_NAME);					
						if (dev) {		
							unsigned char StaMacAndGroup[20];
							memcpy(StaMacAndGroup, DA , 6);
							memcpy(StaMacAndGroup+6, SA, 6);
							...
							#endif
								DEBUG_PRINT("(fdb expire) %s ioctl to DEL! M2U entry da:%02x:%02x:%02x-%02x:%02x:%02x; sa:%02x:%02x:%02x-%02x:%02x:%02x\n",
									RTL_PS_MESH1_DEV_NAME,StaMacAndGroup[0],StaMacAndGroup[1],StaMacAndGroup[2],StaMacAndGroup[3],StaMacAndGroup[4],StaMacAndGroup[5],
									StaMacAndGroup[6],StaMacAndGroup[7],StaMacAndGroup[8],StaMacAndGroup[9],StaMacAndGroup[10],StaMacAndGroup[11]);
							}
						}

						dev = __dev_get_by_name(&init_net, RTL_PS_MESH2_DEV_NAME);					
						if (dev) {		
							unsigned char StaMacAndGroup[20];
							memcpy(StaMacAndGroup, DA , 6);
							memcpy(StaMacAndGroup+6, SA, 6);
							...
							#endif
								DEBUG_PRINT("(fdb expire) %s ioctl to DEL! M2U entry da:%02x:%02x:%02x-%02x:%02x:%02x; sa:%02x:%02x:%02x-%02x:%02x:%02x\n",
									RTL_PS_MESH2_DEV_NAME,StaMacAndGroup[0],StaMacAndGroup[1],StaMacAndGroup[2],StaMacAndGroup[3],StaMacAndGroup[4],StaMacAndGroup[5],
									StaMacAndGroup[6],StaMacAndGroup[7],StaMacAndGroup[8],StaMacAndGroup[9],StaMacAndGroup[10],StaMacAndGroup[11]);
							}
						}
...
EXPORT_SYMBOL(fdb_find_for_driver);
...
int check_srcmac_in_fdb_for_ax_driver(struct net_device *dev, const unsigned char *addr)
{
	...
	{
		br = port->br;
		head = &br->hash[br_mac_hash(addr, 0)];
		hlist_for_each_entry_rcu(fdb, head, hlist) {
			if (ether_addr_equal(fdb->addr.addr, addr)) {
				if(fdb->dst && fdb->dst->dev)
				{
					if(strncmp(fdb->dst->dev->name, dev->name, IFNAMSIZ))
					{
						ret = 1;
					}
				}
				else if(fdb->is_local == 1)
				{
					ret = 1;
				}
				break;
...
EXPORT_SYMBOL(check_srcmac_in_fdb_for_ax_driver);
...
#if defined (CONFIG_RTL_IGMP_SNOOPING)
int br_fdb_create(struct net_bridge *br,
					   struct net_bridge_port *source,
					   const unsigned char *addr,
					   __u16 vid)
	...
	if (likely(fdb)) {
		...
		if (unlikely(fdb->is_local)) {
#ifdef CONFIG_TP_BRIDGE
			if(!suppress_mode && atomic_read(&loop_pkt_cnt) > 50)
			{
				suppress_mode = true;
			}
			atomic_inc(&loop_pkt_cnt);
			
			fdb_notify(br, fdb, RTM_NEIGHLOOP);
#endif
			if (net_ratelimit())
...
#endif
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
					rtl_del_ext_fdb_entry(fdb->addr.addr);
#endif
			}
			fdb->updated = jiffies;
			if (unlikely(added_by_user))
				fdb->added_by_user = 1;
#ifdef CONFIG_TP_BRIDGE
			if (unlikely(fdb_modified && !fdb_compat))
				fdb_notify(br, fdb, RTM_UPDATENEIGH);
#endif
...
EXPORT_SYMBOL_GPL(br_fdb_update);
		       ...
		       const struct net_bridge_fdb_entry *fdb, int type)
{
	...
	if (type == RTM_NEIGHLOOP)
	{
		__br_notify(RTNLGRP_NEIGH, type, br);
		return;
	}
#endif
...
{
	int i,j,k;
	unsigned long igmp_walktimeout = jiffies - igmp_expire_time;
	unsigned long igmp_walktimeout_new = jiffies - groupTimer;
	...
	for (i = 0; i < BR_HASH_SIZE; i++) 
	{
		...
		{
			if(MULTICAST_MAC(f->addr.addr)||IPV6_MULTICAST_MAC(f->addr.addr))
			{			
				for(k=0;k<FDB_IGMP_EXT_NUM;k++)
				{
					...
					{
						//not time out
						if(time_after(f->igmp_fdb_arr[k].ageing_time, igmp_walktimeout))
						{
							if(time_before_eq(f->igmp_fdb_arr[k].ageing_time, igmp_walktimeout_new))
							{
								f->igmp_fdb_arr[k].ageing_time = igmp_walktimeout_new;
							}
						}
					}
				}
