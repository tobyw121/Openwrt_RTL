...
static int htb_syncHwQueue(struct net_device *dev)
{
	...
	int				i;
	int egress_bandwidth_granularity_bytelen,egress_bandwidth_granularity;
		...
		{   
			...
			{ 
				...
				}
				{
					...
					//printk("[%s:%d]cl->prio : %d !\n",__FUNCTION__,__LINE__,cl->prio);					
					queueInfo[queueNum].prio= 7-cl->prio;
					#endif
					//printk("[%s:%d]cl->rate : %d cl->ceil : %d!\n",__FUNCTION__,__LINE__,cl->rate.rate_bytes_ps,cl->ceil.rate_bytes_ps);	
					queueInfo[queueNum].bandwidth = cl->rate.rate_bytes_ps<<3; //chage byte per second to  bit per second
					queueInfo[queueNum].ceil = cl->ceil.rate_bytes_ps<<3;
	...
	else
	{
		...
		portBandwidth = 0;
		//port_rate = sum of root class's ceil
		for(idx=0;idx<topClassNum;idx++)
		{
			{
				//printk("[%s:%d]classHandle[%d]->ceil: %d!\n",__FUNCTION__,__LINE__,classHandle[idx]->ceil.rate_bytes_ps<<3);
				portBandwidth += (classHandle[idx]->ceil.rate_bytes_ps<<3);//chage byte per second to  bit per second
			}
		}
		...
		//printk("[%s:%d]port_rate:%d!\n",__FUNCTION__,__LINE__,tmpPortBandwidth);
		...
		{
		    /*Guaranteed minimum bandwidth:rate != ceil && ceil == portBandwidth*/
			...
			/*Restricted max bandwidth:ceil == rate && ceil <= portBandwidth*/
			else if (queueInfo[i].ceil<portBandwidth)
			{
				...
				if(strncmp(dev->name,RTL_BR_NAME,strlen(RTL_BR_NAME))){
					egress_bandwidth_granularity_bytelen=EGRESS_BANDWIDTH_GRANULARITY_BYTELEN;
					egress_bandwidth_granularity=EGRESS_BANDWIDTH_GRANULARITY;
				}
				else{
					egress_bandwidth_granularity_bytelen=EGRESS_BANDWIDTH_GRANULARITY_BYTELEN_97H;
					egress_bandwidth_granularity=EGRESS_BANDWIDTH_GRANULARITY_97H;
				}
				#else
				egress_bandwidth_granularity_bytelen=EGRESS_BANDWIDTH_GRANULARITY_BYTELEN;
				egress_bandwidth_granularity=EGRESS_BANDWIDTH_GRANULARITY;
				...
				//printk("[%s:%d] queue[%d]->ceil:%d  queue[%d]->rate:%d!\n",__FUNCTION__,__LINE__,i,
					//queueInfo[i].ceil,i,queueInfo[i].bandwidth);
		...
		{
			/*	WFQ mode :Should reduce the R type bandwidth	*/
