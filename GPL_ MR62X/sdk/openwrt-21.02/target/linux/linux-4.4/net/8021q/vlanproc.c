...
static int32 vlan_priority_read(struct seq_file *s, void *v);
static int32 vlan_priority_write( struct file *filp, const char *buff,unsigned long len,void *data);
static int32 single_vlan_pvid_read(struct seq_file *s, void *v);
static int32 single_vlan_pvid_write( struct file *filp, const char *buff,unsigned long len,void *data);
#endif
...
static int32 vlan_fid_write( struct file *filp, const char *buff,unsigned long len,void *data);
static int32 vlan_fid_read(struct seq_file *s, void *v);
...
static int32 vlan_member_map_read(struct seq_file *s, void *v);
static int32 vlan_member_map_write(struct file *filp, const char *buff, unsigned long len, void *data);
#if defined(CONFIG_VLAN_PASSTHRU_SUPPORT_LINUX_VLAN)
static int32 vlan_passthrough_speical_vid_write( struct file *filp, const char *buff,unsigned long len,void *data);
static int32 vlan_passthrough_speical_vid_read(struct seq_file *s, void *v);
#endif
...
extern int dump_vlan_priority(void);
extern int add_or_remove_vlan_priority(int priority, uint64_t portmask, int action);
#ifdef CONFIG_TP_IMAGE
extern int rtk_qos_portPri_set(uint32 port, uint32 int_pri) ;
extern int rtl865x_enableRtl83xxDropVlan(unsigned int portmask);
extern int rtl865x_disableRtl83xxDropVlan(void);
#endif
...
#ifdef CONFIG_TP_IMAGE
int vlan_priority_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, vlan_priority_read, PDE_DATA(inode)));
}
static ssize_t vlan_priority_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return vlan_priority_write(file, userbuf,count, off);
}
struct file_operations vlan_priority_proc_fops= {
		.owner	 = THIS_MODULE,
        .open           = vlan_priority_single_open,
        .write		    = vlan_priority_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};

int a_vlan_pvid_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, single_vlan_pvid_read, PDE_DATA(inode)));
}
static ssize_t a_vlan_pvid_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return single_vlan_pvid_write(file, userbuf,count, off);
}
struct file_operations a_vlan_pvid_proc_fops= {
		.owner	 = THIS_MODULE,
        .open           = a_vlan_pvid_single_open,
        .write		    = a_vlan_pvid_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
int vlan_fid_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, vlan_fid_read, PDE_DATA(inode)));
}
static ssize_t vlan_fid_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return vlan_fid_write(file, userbuf,count, off);
}
struct file_operations vlan_fid_proc_fops= {
		.owner	 = THIS_MODULE,
        .open           = vlan_fid_single_open,
        .write		    = vlan_fid_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
int vlan_member_map_single_open(struct inode *inode, struct file *file)
{
	return(single_open(file, vlan_member_map_read, PDE_DATA(inode)));
}

static ssize_t vlan_member_map_single_write(struct file *file,
													 const char __user *userbuf,
													 size_t count, loff_t *off)
{
	return vlan_member_map_write(file, userbuf,count, off);
}

struct file_operations vlan_member_map_proc_fops = {
	.owner		= THIS_MODULE,
	.open		= vlan_member_map_single_open,
	.write		= vlan_member_map_single_write,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
#if defined(CONFIG_VLAN_PASSTHRU_SUPPORT_LINUX_VLAN)
int vlan_passthrough_speical_vid_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, vlan_passthrough_speical_vid_read, PDE_DATA(inode)));
}
static ssize_t vlan_passthrough_speical_vid_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	    return vlan_passthrough_speical_vid_write(file, userbuf,count, off);
}
struct file_operations vlan_passthrough_speical_vid_proc_fops= {
		.owner	 = THIS_MODULE,
        .open           = vlan_passthrough_speical_vid_single_open,
        .write		    = vlan_passthrough_speical_vid_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#define RTL_DRV_WLAN0_VXD_DEV_NAME		"wlan0-vxd"
#define RTL_DRV_WLAN1_VXD_DEV_NAME		"wlan1-vxd"
...
struct wlan_dev_mask wlan_dev_mask_mapping[WLAN_DEV_NUM_MAX] = 
{
	{RTL_DRV_WLAN0_DEV_NAME, 			(1<<WLAN0_MASK_BIT)},
	{RTL_DRV_WLAN0_VA0_DEV_NAME, 		(1<<WLAN0_VA0_MASK_BIT)},
	{RTL_DRV_WLAN0_VA1_DEV_NAME, 		(1<<WLAN0_VA1_MASK_BIT)},
	{RTL_DRV_WLAN0_VA2_DEV_NAME, 		(1<<WLAN0_VA2_MASK_BIT)},
	{RTL_DRV_WLAN0_VA3_DEV_NAME, 		(1<<WLAN0_VA3_MASK_BIT)},
	{RTL_DRV_WLAN0_VXD_DEV_NAME,		(1<<WLAN0_VXD_MASK_BIT)},
	
	{RTL_DRV_WLAN1_DEV_NAME, 			(1<<WLAN1_MASK_BIT)},
	{RTL_DRV_WLAN1_VA0_DEV_NAME, 		(1<<WLAN1_VA0_MASK_BIT)},
	{RTL_DRV_WLAN1_VA1_DEV_NAME, 		(1<<WLAN1_VA1_MASK_BIT)},
	{RTL_DRV_WLAN1_VA2_DEV_NAME, 		(1<<WLAN1_VA2_MASK_BIT)},
	{RTL_DRV_WLAN1_VA3_DEV_NAME, 		(1<<WLAN1_VA3_MASK_BIT)},
	{RTL_DRV_WLAN1_VXD_DEV_NAME,		(1<<WLAN1_VXD_MASK_BIT)},
};

unsigned int rtl_get_wlan_port_mask_by_dev_name(char *name)
{
	int i;
	char rIfName[IFNAMSIZ];
	int length = (sizeof(wlan_dev_mask_mapping)/sizeof(wlan_dev_mask_mapping[0]));
	
	strncpy(rIfName, name, IFNAMSIZ);	
	...
	for (i=0; i< length; i++)
	{
		if (strncmp(rIfName, wlan_dev_mask_mapping[i].ifname, IFNAMSIZ) == 0) {
			return wlan_dev_mask_mapping[i].wlan_mask;
		}
	}
...
#if defined(CONFIG_VLAN_PASSTHRU_SUPPORT_LINUX_VLAN)
EXPORT_SYMBOL(special_iptv_vid);
#endif
#endif
...
int __net_init vlan_proc_init(struct net *net)
{
...
#ifdef CONFIG_TP_IMAGE
	proc_create_data("priority",0,vn->proc_vlan_dir,&vlan_priority_proc_fops,NULL);
	proc_create_data("apvid",0,vn->proc_vlan_dir,&a_vlan_pvid_proc_fops,NULL);
#endif
    proc_create_data("pvid",0,vn->proc_vlan_dir,&vlan_pvid_proc_fops,NULL);
    proc_create_data("fid",0,vn->proc_vlan_dir,&vlan_fid_proc_fops,NULL);
	...
	proc_create_data("vlanMemberMap",0,vn->proc_vlan_dir,&vlan_member_map_proc_fops,NULL);
#if defined(CONFIG_VLAN_PASSTHRU_SUPPORT_LINUX_VLAN)
	proc_create_data("speicalVlanId",0,vn->proc_vlan_dir,&vlan_passthrough_speical_vid_proc_fops,NULL);
#endif
    #endif
...
#ifdef CONFIG_TP_IMAGE
/* port 3 , port 4*/
#define TP_LAN_MASK 0x18

/* port2, port 3 , port 4*/
#define TP_NORMAL_PORT_MASK 0x1c

/* WLAN->WAN HWNAT Priority */
#define RTL_8367D_PRIORITY_CPU_PORT_MASK (1 << ETH_P8_MASK_BIT )

/* port 0 ~ 4 */
static uint32 port_priority_list[5] = {0};
static uint32 port_drop_vlan_mask = 0;
...
static int32 vlan_priority_write( struct file *filp, const char *buff,unsigned long len,void *data)
{
	...
	{
		num=sscanf(tmpbuf,"%d,%d", &vid, &priority);

		if(num != 2){
			printk("echo \"VLAN,Priority\"\n");
			return len;
		}	
		...
		if(vid > 1 && vid < 4095)
		{
			...
			/* set priority: 0~7 */
			/* Write to nic driver. */
			vlan_group_p->priority = priority & 0x7;

			if(vid == rtk_vlan_wan_vid)
			{
				/* Internet VLAN: Write to register of RTL8197H switch */
				add_or_remove_vlan_priority(vlan_group_p->priority, vlan_group_p->memberMask & TP_NORMAL_PORT_MASK, 1);

				/* 
				 * Configure the same priority for the LAN interface as for the WAN interface.
				 * Fix bug 697206, In hardware acceleration, WAN packets have no priority.
				 */
				if(rtk_vlan_hw_nat_lan_vid > 1 && rtk_vlan_hw_nat_lan_vid < 4095)
				{
					add_or_remove_vlan_priority(vlan_group_p->priority, 
						vlan_ctl_p->group[rtk_vlan_hw_nat_lan_vid].memberMask
						| RTL_8367D_PRIORITY_CPU_PORT_MASK, 1);

					/* WLAN -> WAN priority */
					vlan_group_p = &vlan_ctl_p->group[rtk_vlan_hw_nat_lan_vid];
					vlan_group_p->priority = priority & 0x7;
					
					printk(KERN_DEBUG"enabled set lan port[0x%x] priority[%d] of vid[%d]\n", 
						vlan_ctl_p->group[rtk_vlan_hw_nat_lan_vid].memberMask, vlan_group_p->priority,
						rtk_vlan_hw_nat_lan_vid);
				}
			}
			else
			{
				/* IPTV VLAN: Write to register of RTL8197H switch */
				add_or_remove_vlan_priority(vlan_group_p->priority, vlan_group_p->memberMask & TP_LAN_MASK, 1);
			}

			/* 
			 * RTL8367S-VB (RTL8367D) does NOT support the priority of switch vlan. 
			 * We use port priority to implement this function.
			 * We only care about LAN port (3, 4).
			 *
			 * Support LAN -> WAN
			 */
			 for(port_id = 3; port_id <= 4; port_id++)
			 {
			 	if(vlan_group_p->memberMask & (1 << port_id))
			 	{
			 		printk(KERN_DEBUG"enabled set port[%d] priority[%d] of vid[%d]?\n", port_id, vlan_group_p->priority, vid);
			 		port_priority_list[port_id] = vlan_group_p->priority;
			 		rtk_qos_portPri_set(port_id, vlan_group_p->priority);
			 	}
			 }
		}
		else if(vid == 0 && priority == 0)
		{
			/* Clear the priority of the previously configured port. */
			 for(port_id = 3; port_id <= 4; port_id++)
			 {
			 	if(port_priority_list[port_id])
			 	{
			 		printk(KERN_DEBUG"enabled clean port[%d] priority[%d]\n", port_id, port_priority_list[port_id]);
			 		port_priority_list[port_id] = 0;
			 		rtk_qos_portPri_set(port_id, 0);
			 	}
			 }
		}
	}
...
static int32 single_vlan_pvid_write( struct file *filp, const char *buff,unsigned long len,void *data)
{
	char tmpbuf[100];
	int num ;
	uint32 port, pvid, pri;
	int i;
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		num=sscanf(tmpbuf,"%d,%d", &port, &pvid);

		if(num != 2){
			printk("echo \"Port,Pvid\"\n");
			return len;
		}	

		/* Only care about port 0~8 */
		if(port >= 9)
		{
			if(port == 9 && port_drop_vlan_mask)
			{
				if(pvid == 0)
				{
					/* clear the acl of drop vlan. */
					rtl865x_disableRtl83xxDropVlan();
					printk(KERN_DEBUG"disabled drop vlan mask[%x]\n", port_drop_vlan_mask);
					port_drop_vlan_mask = 0;
					return len;
				}
				else if(pvid == 1)
				{
					/* set the acl of drop vlan. */
					if(rtk_vlan_wan_vid == 1)
					{
						/* Non-VLAN */
						rtl865x_enableRtl83xxDropVlan(port_drop_vlan_mask);
					}
					else
					{
						/* IPTV/VLAN On */
						rtl865x_enableRtl83xxDropVlan(port_drop_vlan_mask & TP_LAN_MASK);
					}
					printk(KERN_DEBUG"enabled drop vlan mask[%x], wan vlan %d\n", port_drop_vlan_mask, rtk_vlan_wan_vid);
					return len;
				}
			}
		...
		vlan_ctl_p->pvid[port]=(uint16)pvid;
...
#endif			
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
		rtl8651_getAsicPortVlanPri(port, &pri); 
		rtk_vlan_portPvid_set(port, pvid, pri);
#endif	

		port_drop_vlan_mask |= (1 << port);
	}
	return len;
}
#endif
...
static int32 vlan_group_read(struct seq_file *s, void *v)
{
	...
	for(vlan_id=0; vlan_id<VLAN_GROUP_NUM; vlan_id++)
	{
		if(vlan_ctl_p->group[vlan_id].flag)
			...
			vlan_ctl_p->group[vlan_id].priority
			);
	}
...
static int32 vlan_pvid_write( struct file *filp, const char *buff,unsigned long len,void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		for(i=0;i<num;i++){
			...
			#if defined(CONFIG_RTL_8021Q_VLAN_SUPPORT_SRC_TAG) && defined(CONFIG_RTL_819X_SWCORE)
			...
			#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
			if(i<9 &&  pvid[i]){
				rtl8651_getAsicPortVlanPri(i, &pri); 
				rtk_vlan_portPvid_set(i, pvid[i], pri);
			}
			#endif
		}
...
static void rtk_init_vlan_tbl(void)
{
   ...
   linux_vlan_group_t *vlan_group_p=NULL;	
	 ...
	 add_or_remove_vlan_priority(0, 0, 2);
     //memset(&rtk_vlan_idx,0,sizeof(uint16)*RTK_MAX_VLAN_GROUP);

    //mark_vc , sync with rtk default vlan  ,  if not gateway mode ?
    //rtk_vlan_idx[0] = RTL_LANVLANID ;
    //rtk_vlan_idx[1] = RTL_WANVLANID ;
...
static int32 vlan_ctl_write( struct file *filp, const char *buff,unsigned long len,void *data)
{
...
#if defined(CONFIG_VLAN_PASSTHRU_SUPPORT_LINUX_VLAN)
	{
		...
		if (rtl_vlan_passthrough_enable == 1) {
			extern void rtl_set_switch_full_vlan_entry(void);
			rtl_set_switch_full_vlan_entry();
		}
	}
#endif
	return len;
...
static int32 vlan_group_write( struct file *filp, const char *buff,unsigned long len,void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		num=sscanf(tmpbuf,"%d,%x,%x,%d,%d,%d",&flag,&mask,&tagMask,&vid,&vlanType, &priority);
		if(num < 5){
			printk("echo \"Flag,MemberMask,tagMask,vid,vlanType,Priority\"\n");
			return len;
		}						

		printk("%d,%x,%x,%d,%d,%d\n",flag,mask,tagMask,vid,vlanType, priority);
		...
		if(vlan_group_p->vid && vlan_group_p->flag)
		{
			if((vlan_group_p->memberMask & RTL_WANPORT_MASK)&&(rtl865x_curOpMode == 0))//gate way mode
			{
			#if defined(CONFIG_RTL_8367R_SUPPORT)
				rtl_vlan_RTL8367R_set(vlan_group_p->vid, 0, 0, 1);
			#elif defined(CONFIG_RTL_83XX_SUPPORT)
				rtl_vlan_RTL83XX_set(vlan_group_p->vid, 0, 0, 1);
			#endif
			}
			else
			{
			...
			#elif defined(CONFIG_RTL_83XX_SUPPORT)
				rtl_vlan_RTL83XX_set(vlan_group_p->vid, 0, 0, 0);
			#endif
			}
		}
		#endif
		...
		/* FIXME: Useless? */
		if(vlan_group_p->vid!=9)   
#endif
		add_or_remove_vlan_priority(vlan_group_p->priority, vlan_group_p->memberMask, 1);
			...
			if(vlan_group_p->flag){
                if(!linux_vlan_hw_support_enable)
                {
                    ...
                    if((mask&RTL_WANPORT_MASK)&&(rtl865x_curOpMode == 0))//gate way mode
					...
					#elif defined(CONFIG_RTL_83XX_SUPPORT)
					if((mask&RTL_WANPORT_MASK)&&(rtl865x_curOpMode == 0))//gate way mode
                    	rtl_vlan_RTL83XX_set(vlan_group_p->vid, tagMask, mask, 1);
					else
                    	rtl_vlan_RTL83XX_set(vlan_group_p->vid, tagMask, mask, 0);	
                    #endif
                }
                else
                {
...
static int32 vlan_member_map_read(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "dev\t\tvlan member map\n", dev->name);

	rcu_read_lock();

	for_each_netdev_rcu(net, dev)
	{
		seq_printf(s, "%-16s%d\n", dev->name, dev->vlan_member_map);
	}
...
static int32 vlan_member_map_write(struct file *filp, const char *buff,
										  unsigned long len, void *data)
{
	int32 num = 0;
	uint32 dev_mask_bit = 0;
	struct net_device *dev = NULL;
	char tmpbuf[100], dev_name[IFNAMSIZ];

	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		num = sscanf(tmpbuf,"%s %u", dev_name, &dev_mask_bit);
		if(num != 2)
		{
			printk("invalid input!\n");
			printk("command format:\n");
			printk("echo [dev name] [dev mask bit] > /proc/net/vlan/vlanMemberMap\n");
			return len;
		}

		if(dev_mask_bit > WLAN1_VXD_MASK_BIT)
		{
			printk("dev mask bit should be %d-%d!\n", ETH_P0_MASK_BIT, WLAN1_VXD_MASK_BIT);
			return len;
		}

		dev =  __dev_get_by_name(&init_net, dev_name);
		if(dev)
			dev->vlan_member_map = dev_mask_bit;
		else
...
static int32 vlan_passthrough_speical_vid_read(struct seq_file *s, void *v)
{
	seq_printf(s, "speical iptv id = %d\n", special_iptv_vid);	
   return 0;
}
...
static int32 vlan_fid_write( struct file *filp, const char *buff,unsigned long len,void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		num = sscanf(tmpbuf,"%d,%d", &vid, &fid);

		printk("num is %d, vid is %d, fid is %d\n", num, vid, fid);
		if ((num > 2) ||((vid<0) ||(vid>4095)) ||((fid<0) ||(fid>3))) 
			return len;
