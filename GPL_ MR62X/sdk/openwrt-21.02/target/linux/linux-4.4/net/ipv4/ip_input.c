...
#define CB_WLAN_GUEST_ACCESS 30
...
static int ip_rcv_finish(struct net *net, struct sock *sk, struct sk_buff *skb)
{
	...
	if (dst&&dst->input == &ip_local_deliver) {
		if ((skb->__unused == 0xe5 || (skb->cb[CB_WLAN_GUEST_ACCESS] & 0xff) ==0xe5) && iph->protocol== IPPROTO_UDP) {
			...
			if (htons(hdr->dest) == 53 || htons(hdr->dest) == 67){ // DNS Domain or dhcp
				if(skb->__unused == 0xe5)
					skb->__unused = 0;
				if((skb->cb[CB_WLAN_GUEST_ACCESS] & 0xff)==0xe5){
					skb->cb[CB_WLAN_GUEST_ACCESS] = 0;
				}
			}
		}

		if ((skb->__unused == 0xe5 || (skb->cb[CB_WLAN_GUEST_ACCESS] & 0xff)==0xe5) && iph->protocol== IPPROTO_TCP) {
			struct udphdr *hdr = (struct udphdr *)((u_int32_t *)iph + iph->ihl);
		...
		if ((skb->cb[CB_WLAN_GUEST_ACCESS] & 0xff) == 0xe5 && iph->protocol== IPPROTO_ICMP) {
			skb->cb[CB_WLAN_GUEST_ACCESS] = 0;
		}

		if (skb->__unused == 0xe5 || (skb->cb[CB_WLAN_GUEST_ACCESS] & 0xff)==0xe5){
			goto drop;
		}
	}
#endif
