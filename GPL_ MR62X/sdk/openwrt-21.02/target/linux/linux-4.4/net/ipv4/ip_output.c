...
char ip_pptp_buf[512];
void ip_pptp_trace(struct sk_buff *skb, const char *func, unsigned int line, char *special_buf)
{
	...
	if (iph->protocol == IPPROTO_GRE) {
		greh = (struct pptp_gre_header *)((unsigned long)iph + (iph->ihl << 2));
		headersize  = sizeof(*greh);
		if (PPTP_GRE_IS_A(greh->ver) == 0)
			headersize -= sizeof(greh->ack);
		payload = (__u8 *)((unsigned long)greh + headersize);
		printk("[%s-%u] STA 0x%x UI 0x%x PROTOCOL 0x%x CODE 0x%x ID 0x%x L 0x%x\n",
			func, line, PPP_ADDRESS(payload), PPP_CONTROL(payload), PPP_PROTOCOL(payload),
			payload[4], payload[5], (payload[6] << 8) | payload[7]);
		printk("[%s-%u] data 0x%x pptp 0x%x %s src %u.%u.%u.%u dst %u.%u.%u.%u\n",
			func, line, skb->data + skb->len, greh, special_buf,
...
#endif
	return nf_hook(NFPROTO_IPV4, NF_INET_LOCAL_OUT, net, sk, skb, NULL,
...
int ip_output(struct net *net, struct sock *sk, struct sk_buff *skb)
{
	...
	return NF_HOOK_COND(NFPROTO_IPV4, NF_INET_POST_ROUTING, net, sk, skb, NULL, dev,
			    ip_finish_output,
			    !(IPCB(skb)->flags & IPSKB_REROUTED));
	}else{
...
#if (defined(CONFIG_RTL_QOS_PATCH) || defined(CONFIG_RTK_VOIP_QOS)|| defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) ||defined(CONFIG_RTL_HW_QOS_SUPPORT_WLAN) || defined(CONFIG_RTL_NIC_QUEUE))&& defined(CONFIG_RTL_819X) 
