...
static void arp_timer_start(void)
{
	...
	arp_timer.function = (void *)arp_timer_handle;
...
static int arp_req_get(struct arpreq *r, struct net_device *dev)
{
	...
	if (neigh) 
	{
		...
		if( (memcmp(neigh->ha, zero_ha , dev->addr_len)==0) &&
			((dev!=NULL) && (strncmp(dev->name, CONFIG_TP_LAN_BRIDGE_NAME, strlen(CONFIG_TP_LAN_BRIDGE_NAME))==0)))
		{			
	...
	{
		if((dev!=NULL) && (strncmp(dev->name, CONFIG_TP_LAN_BRIDGE_NAME, strlen(CONFIG_TP_LAN_BRIDGE_NAME))==0))
		{
