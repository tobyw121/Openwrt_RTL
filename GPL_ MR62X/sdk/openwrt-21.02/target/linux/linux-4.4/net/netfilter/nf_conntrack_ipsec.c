...
{
	seq_printf(s, "timeout=%lu, stream_timeout=%lu ",
...
static inline struct nf_generic_net *esp_pernet(struct net *net)
{
...
static void nf_conntrack_ipsec_fini(void)
{
    ...
    nf_ct_l4proto_unregister(&nf_conntrack_protocol_esp);
}
...
static int __init nf_conntrack_ipsec_init(void)
{
	...
	for (i = 0; (i < MAX_PORTS) && ports[i]; i++) {
	 ...
	 strncpy( ipsec_helpers[i].name, tmpname, NF_CT_HELPER_NAME_LEN);
	...
	nf_ct_l4proto_register(&nf_conntrack_protocol_esp);
