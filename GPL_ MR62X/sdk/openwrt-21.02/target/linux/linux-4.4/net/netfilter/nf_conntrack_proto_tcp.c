...
#if defined(FAST_PATH_SPI_ENABLED) || !(defined(CONFIG_RTL_IPTABLES_FAST_PATH) ||defined(CONFIG_RTL_HARDWARE_NAT))
bool rtl_tcp_in_window(const struct nf_conn *ct,
			  ...
			  u_int8_t pf)
{
	...
	pr_debug("rtl_tcp_in_window: START\n");
	pr_debug("rtl_tcp_in_window: ");
	nf_ct_dump_tuple(tuple);
	pr_debug("seq=%u ack=%u+(%d) sack=%u+(%d) win=%u end=%u\n",
		 seq, ack, receiver_offset, sack, receiver_offset, win, end);
	pr_debug("rtl_tcp_in_window: sender end=%u maxend=%u maxwin=%u scale=%i "
	...
	pr_debug("rtl_tcp_in_window: I=%i II=%i III=%i IV=%i\n",
	...
	pr_debug("rtl_tcp_in_window: res=%u sender end=%u maxend=%u maxwin=%u "
...
extern int fast_spi;
	...
	if (
		...
		(fast_spi == 1) &&
#endif
		!rtl_tcp_in_window(ct, &ct->proto.tcp, dir, index,
			   skb, dataoff, th, pf)) {
		spin_unlock_bh(&ct->lock);
		return -NF_ACCEPT;
	}
