...
#if defined(CONFIG_RTL_819X) && defined(CONFIG_NF_CONNTRACK_RTCACHE)
extern struct dst_entry *rtl_ip6_dst_check(struct dst_entry *dst, u32 cookie);
#endif
#endif
...
static u32 nf_rtcache_get_cookie(int pf, const struct dst_entry *dst)
{
	...
	if (pf == NFPROTO_IPV6) {
		...
		/* for ipv6 rt may be a per cpu rt, and dst_check checks real rt cookie, thus cache real rt cookie. */
...
extern unsigned int statistic_linux_rtcache_success;
extern unsigned int statistic_linux_rtcache_fail;
...
#if defined(CONFIG_RTL_819X) && defined(CONFIG_NF_CONNTRACK_RTCACHE) && defined(CONFIG_NF_CONNTRACK_IPV6)
  if(NFPROTO_IPV6 == pf) {
	  dst = rtl_ip6_dst_check(dst, cookie);
  }
  else 
#endif
  dst = dst_check(dst, cookie);
...
#if defined(CONFIG_RTL_819X)
	// dst may be fake entry like br->fake_rtable, should check dst input validation
	if (unlikely(dst->input == NULL))
		return NF_ACCEPT;
#endif
