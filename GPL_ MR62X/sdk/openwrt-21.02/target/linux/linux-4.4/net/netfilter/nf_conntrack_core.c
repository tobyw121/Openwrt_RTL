...
void nf_conntrack_all_lock(void)
{
	int i;
...
EXPORT_SYMBOL_GPL(nf_conntrack_all_lock);

void nf_conntrack_all_unlock(void)
{
	int i;
...
EXPORT_SYMBOL_GPL(nf_conntrack_all_unlock);
...
bool nf_ct_delete_forced(struct nf_conn *ct, u32 portid, int report)
{
...
EXPORT_SYMBOL_GPL(nf_ct_delete_forced);
#endif
			   ...
			   void *data, u32 portid, int report)
{
	...
	while ((ct = get_next_corpse(net, iter, data, &bucket)) != NULL) {
		...
		if (del_timer(&ct->timeout)){
#if defined(CONFIG_RTL_819X)	//(CONFIG_RTL_NF_CONNTRACK_GARBAGE_NEW)
		nf_ct_delete_forced(ct, portid, report);
#else
...
static
#endif
int kill_all(struct nf_conn *i, void *data)
{
	return 1;
}
#ifdef CONFIG_RTL_819X
EXPORT_SYMBOL_GPL(kill_all);
#endif
...
static int proc_alg_debug_write( struct file *filp, const char __user *buf,unsigned long len, void *data )
{
	...
	if (tokptr != NULL) 
	{
		...
		else
		{
			for (i = 0; i < alg_type_end; i++)
			{
	   			if (0 == strcmp(tokptr, alg_list[i].name))
    			{
    				ret = sscanf(strptr, "%d", (int*)&val);
					if(ret != 1 || val < 0 )
					{
