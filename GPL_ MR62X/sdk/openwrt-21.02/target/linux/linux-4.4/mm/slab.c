...
void slab_info_dump(void)
{
	...
	int node, i;
	unsigned long offset, size;
	char modname[MODULE_NAME_LEN], name_sym[KSYM_NAME_LEN];
	static unsigned long trace_rec[PAGE_SIZE / sizeof(unsigned long)];
#endif

	list_for_each(lh, &slab_caches) {
		s = list_entry(lh, struct kmem_cache, list);
...
#ifdef CONFIG_DEBUG_SLAB_LEAK
	trace_rec[0] = sizeof(trace_rec) / (2 * sizeof(unsigned long));
	list_for_each(lh, &slab_caches) {
		s = list_entry(lh, struct kmem_cache, list);
		...
		for_each_kmem_cache_node(s, node, n) {
			...
			list_for_each_entry(page, &n->slabs_full, lru)
				handle_slab(trace_rec, s, page);
			list_for_each_entry(page, &n->slabs_partial, lru)
				handle_slab(trace_rec, s, page);
			spin_unlock_irq(&n->list_lock);
		}
		name = s->name;
		for (i = 0; i < trace_rec[1]; i++) {
			printk("%s: %lu ", name, trace_rec[2*i+3]);
			if (lookup_symbol_attrs(trace_rec[2*i+2], &size, &offset, modname, name_sym) == 0) {
