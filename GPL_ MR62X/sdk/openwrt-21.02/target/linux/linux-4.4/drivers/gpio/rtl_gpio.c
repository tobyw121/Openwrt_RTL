...
static unsigned int wps_btn_gnum, wps_led_gnum, reset_btn_gnum, reset_led_gnum, rtl819x_gpio_actlow = 1;
...
typedef int (*RTLWIFINIC_GPIO_read_ptr_t)(unsigned int);
typedef void (*RTLWIFINIC_GPIO_write_ptr_t)(unsigned int,unsigned int);

RTLWIFINIC_GPIO_read_ptr_t RTLWIFINIC_GPIO_read_ptr=NULL;
RTLWIFINIC_GPIO_write_ptr_t RTLWIFINIC_GPIO_write_ptr=NULL;
EXPORT_SYMBOL(RTLWIFINIC_GPIO_read_ptr);
EXPORT_SYMBOL(RTLWIFINIC_GPIO_write_ptr);

#if defined(CONFIG_RTL_8211F_SUPPORT)
int rtw_gpio_read(unsigned int gpio_num)
{
	#if defined(CONFIG_RTL8192CD)
	return (RTLWIFINIC_GPIO_read(gpio_num));
	#else
	if (RTLWIFINIC_GPIO_read_ptr)
		return (RTLWIFINIC_GPIO_read_ptr(gpio_num));
	else
		return 0;
	#endif
}

void rtw_gpio_write(unsigned int gpio_num, unsigned int value)
{
	#if defined(CONFIG_RTL8192CD)
	RTLWIFINIC_GPIO_write(gpio_num, value);
	#else
	if (RTLWIFINIC_GPIO_write_ptr)
		RTLWIFINIC_GPIO_write_ptr(gpio_num, value);
	#endif
	return;
}
#endif
...
{	
	...
	if(rtl819x_bond_option() == BSP_BOND_97FN) {
		...
		DPRINTK("line: %d\n", __LINE__);
		return !rtw_gpio_read(reset_btn_gnum) ^ (rtl819x_gpio_actlow ? 1 : 0 );
#endif
	}
#endif
...
#if !defined(CONFIG_FREE_WBB_PIN)	
		DPRINTK("line: %d\n", __LINE__);
		rtw_gpio_write(reset_led_gnum, (rtl819x_gpio_actlow ? 0 : 1));
		return;
#endif
	}
#endif
...
#if !defined(CONFIG_FREE_WBB_PIN)	
		DPRINTK("line: %d\n", __LINE__);
		rtw_gpio_write(reset_led_gnum, (rtl819x_gpio_actlow ? 1 : 0));
		return;
#endif
	}
#endif
...
static void rtl_gpio_timer(unsigned long data)
{
	...
	else if (probe_state == PROBE_ACTIVE)
	{
		...
		{
			probe_counter++;
			if ((probe_counter >=5 ) && (probe_counter <=PROBE_TIME))
			{
				DPRINTK("5-10 turn on led\n");
				//turn on LED0
				reset_led_on();
			}
...
static int rtl819x_led_probe(struct platform_device *dev) {
	...
	int ret = 0;
		...
		goto OUT;
#endif
	if (0 == strcmp(pdata->name, "green led"))
	{
	...
	{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
		ret = devm_gpio_request_one(&dev->dev, pdata->gpio, GPIOF_OUT_INIT_LOW | GPIOF_EXPORT_DIR_FIXED, pdata->name);
#else
		if((ret = gpio_request_one(pdata->gpio, GPIOF_OUT_INIT_LOW, pdata->name)) == 0)
			ret = gpio_export(pdata->gpio, false);
#endif
	}
OUT:
