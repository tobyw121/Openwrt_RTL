...
/* decide timer to TIMER_BASE_SEC*2^n second base,
 * default 4 sec means timer is 4,8,16....etc
 * 0 is forbidden!!
 */
...
static unsigned char wdt_stopped;
#endif
...
static inline int rtk_get_match_timer_idx(unsigned int t)
{
	...
	for (i = 0; i < TIMER_MAX_N; i++) {
		...
		/* find a timer that > set_timerout */
		if (timer >= t)
			return i;
	}
...
static void rtk_set_timer(unsigned int t)
{
	...
	reg_val =
	    reg_val & ~(OVSEL_MASK << OVSEL_L_OFFSET) & (OVSEL_MASK <<
							 OVSEL_H_OFFSET);
...
#endif
	/* Occasionally, write 0xa5000000 to control register
	 * then make system reset immediately. So, OR current value as dummy bits.
	 * This is not side affect to function of disabling watchdog counting
...
static int rtl819x_wdt_clk_init(void)
{
	 ...
	 * then the default timer will be 4 sec ,
	 * the timer can be configured to 4x2^n (n=0~9)
	 */
...
static int rtl819x_wdt_probe(struct platform_device *pdev)
{
	...
	/* TIMER_BASE_SEC * 2^N */
	rtl819x_wdt_dev.max_timeout = TIMER_BASE_SEC * (1 << TIMER_MAX_N);
