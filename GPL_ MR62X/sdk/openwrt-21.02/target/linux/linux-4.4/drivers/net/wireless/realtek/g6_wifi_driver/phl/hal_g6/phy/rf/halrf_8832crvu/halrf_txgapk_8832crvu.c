...
void _txgapk_backup_bb_registers_8832crvu(
	struct rf_info *rf,
	u32 *reg,
	u32 *reg_bkup,
	u8 reg_num)
...
void _txgapk_backup_kip_8832crvu(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][TXGAPK_KIP_REG_NUM_8832CRVU],
	u8 path,
	u8 reg_num)
{
	u8 i;
...
void _txgapk_bkup_rf_8832crvu(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][TXGAPK_RF_REG_NUM_8832CRVU],
	u8 path,
	u8 reg_num)
{
	u8 i;
...
void _txgapk_reload_bb_registers_8832crvu(
	struct rf_info *rf,
	u32 *reg,
	u32 *reg_backup,
	u8 reg_num)
...
void _txgapk_reload_rf_8832crvu(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][TXGAPK_RF_REG_NUM_8832CRVU],
	u8 path,
...
void _txgapk_reload_kip_8832crvu(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][TXGAPK_KIP_REG_NUM_8832CRVU],
	u8 path,
...
void _halrf_txgapk_rxagc_onoff_8832crvu(
	struct rf_info *rf,
	enum rf_path path,
	bool turn_on)
{
...
void _halrf_txgapk_bb_afe_by_mode_8832crvu(struct rf_info *rf,
...
void _halrf_txgapk_iqk_preset_by_mode_8832crvu(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_txgapk_clk_setting_dac960mhz_by_mode_8832crvu
...
void _halrf_txgapk_rf_dpk_2g_8832crvu
...
void _halrf_txgapk_rf_dpk_5g_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
...
void _halrf_txgapk_rf_dpk_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	...
	if (band == BAND_ON_24G)
		_halrf_txgapk_rf_dpk_2g_8832crvu(rf, phy, path);
	else
		_halrf_txgapk_rf_dpk_5g_8832crvu(rf, phy, path);
}


void _halrf_txgapk_rf_rxdck_2g_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	//u8 i = 0;
...
void _halrf_txgapk_rf_rxdck_5g_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	//u8 i = 0;
...
void _halrf_txgapk_rf_rxdck_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	//u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	//if (channel >= 1 && channel <= 14)
	if (band == BAND_ON_24G)
		_halrf_txgapk_rf_rxdck_2g_8832crvu(rf, phy, path);
	else
		_halrf_txgapk_rf_rxdck_5g_8832crvu(rf, phy, path);
}


void _halrf_txgapk_track_table_nctl_2g_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, u32 gain_stage)
{
...
#if 0
	halrf_before_one_shot_enable_8832crvu(rf);
	halrf_wreg(rf, 0x8000, MASKDWORD, cal_path_step1[path]); //set cal_path, process id 
	halrf_one_shot_nctl_done_check_8832crvu(rf, path);	
	//halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
#else
	halrf_do_one_shot_8832crvu(rf, path, 0x8000, MASKDWORD, process_id1[path]);
	...
	//halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
#else
	halrf_do_one_shot_8832crvu(rf, path, 0x8000, MASKDWORD, process_id2[path]);
#endif
...
void _halrf_txgapk_track_table_nctl_5g_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, u32 gain_stage)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
...
void _halrf_txgapk_track_table_nctl_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	u32 gain_stage_6g[4] = {0xd4d0, 0xd550, 0xd4d0, 0x6a68}; //LB MB HB UHB
	...
	if (rfe_type > 50) { //efem
		if (band == BAND_ON_24G) //TBD
			_halrf_txgapk_track_table_nctl_2g_8832crvu(rf, phy, path, gain_stage_2g);
		else
			_halrf_txgapk_track_table_nctl_5g_8832crvu(rf, phy, path, gain_stage_5g_efem);
	} else {
	//if (channel >= 1 && channel <= 14)
		if (band == BAND_ON_24G)
			_halrf_txgapk_track_table_nctl_2g_8832crvu(rf, phy, path, gain_stage_2g);
			...
			if (channel >= 36 && channel <= 64)
				_halrf_txgapk_track_table_nctl_5g_8832crvu(rf, phy, path, gain_stage_5g[0]);
			else if (channel >= 100 && channel <= 144)
				_halrf_txgapk_track_table_nctl_5g_8832crvu(rf, phy, path, gain_stage_5g[1]);
			else if (channel >= 149 && channel <= 177)
				_halrf_txgapk_track_table_nctl_5g_8832crvu(rf, phy, path, gain_stage_5g[2]);
		}
		else
			_halrf_txgapk_track_table_nctl_5g_8832crvu(rf, phy, path, gain_stage_6g[group_of_6g]);		
	}
}



void _halrf_txgapk_power_table_nctl_2g_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, u32 gain_stage)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	/* u8 rfe_type = rf->phl_com->dev_cap.rfe_type; */
	...
	halrf_before_one_shot_enable_8832crvu(rf);
	halrf_wreg(rf, 0x8000, MASKDWORD, cal_path_step2[path]);
	halrf_one_shot_nctl_done_check_8832crvu(rf, path); //set cal_path, process id
	//halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	#else
	halrf_do_one_shot_8832crvu(rf, path, 0x8000, MASKDWORD, process_id2[path]);
	#endif
...
void _halrf_txgapk_power_table_nctl_5g_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, u32 gain_stage)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
...
void _halrf_txgapk_power_table_nctl_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	//u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	//if (channel >= 1 && channel <= 14)
	if (band == BAND_ON_24G)
		_halrf_txgapk_power_table_nctl_2g_8832crvu(rf, phy, path, gain_stage_2g);
	else if (band == BAND_ON_5G)
		_halrf_txgapk_power_table_nctl_5g_8832crvu(rf, phy, path, gain_stage_5g);
	else
		_halrf_txgapk_power_table_nctl_5g_8832crvu(rf, phy, path, gain_stage_6g);	

}



void _halrf_txgapk_iqk_bk_reg_by_mode_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	...
	if (!is_dbcc) {
		...
		halrf_one_shot_nctl_done_check_8832crvu(rf, path);
#else
		halrf_do_one_shot_8832crvu(rf, path, 0x8000, MASKDWORD, process_id[path]);
#endif
...
void _halrf_txgapk_restore_iqk_kmod_bb_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_txgapk_restore_iqk_kmod_drfc_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_txgapk_main_8832crvu(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	/*_halrf_txgapk_iqk_dpk_init_reg_8832crvu(rf, phy, RF_PATH_A);*/
	/*_halrf_txgapk_nctl_8832crvu(rf, phy, RF_PATH_A);*/

	#if 0 //it doesn't need to write default
	_halrf_txgapk_write_track_table_default_8832crvu(rf, phy, path);
	_halrf_txgapk_write_power_table_default_8832crvu(rf, phy, path);
	#endif

	_halrf_txgapk_bb_afe_by_mode_8832crvu(rf, phy, path, false);
	
	_halrf_txgapk_iqk_preset_by_mode_8832crvu(rf, phy, path, false);

	_halrf_txgapk_rf_dpk_8832crvu(rf, phy, path);
	
	_halrf_txgapk_rf_rxdck_8832crvu(rf, phy, path);

	_halrf_txgapk_track_table_nctl_8832crvu(rf, phy, path);
	//it doesn't need to write table
	
	_halrf_txgapk_power_table_nctl_8832crvu(rf, phy, path);
	//it doesn't need to write table

	_halrf_txgapk_iqk_bk_reg_by_mode_8832crvu(rf, phy, path, false);
	
	_halrf_txgapk_restore_iqk_kmod_bb_8832crvu(rf, phy, path, false);
	
	_halrf_txgapk_restore_iqk_kmod_drfc_8832crvu(rf, phy, path, false);
	
}

void _halrf_do_reset_tbl_txgapk_8832crvu(struct rf_info *rf,
...
//	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	...
	u32 kip_bkup[TXGAPK_RF_PATH_MAX_8832CRVU][TXGAPK_KIP_REG_NUM_8832CRVU] = {{0}};
	//u32 bb_bkup[TXGAPK_BB_REG_NUM_8832CRVU] = {0};
	u32 rf_bkup[TXGAPK_RF_PATH_MAX_8832CRVU][TXGAPK_RF_REG_NUM_8832CRVU] = {{0}};
	...
	//_txgapk_backup_bb_registers_8832crvu(rf, bb_reg, bb_bkup, TXGAPK_BB_REG_NUM_8832CRVU);

	for (path = 0; path < TXGAPK_RF_PATH_MAX_8832CRVU; path++) {	
		_txgapk_backup_kip_8832crvu(rf, kip_reg, kip_bkup, path, TXGAPK_KIP_REG_NUM_8832CRVU);
		_txgapk_bkup_rf_8832crvu(rf, rf_reg, rf_bkup, path, TXGAPK_RF_REG_NUM_8832CRVU);
	}				
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8832CRVU; path++) {
		...
		//follow dpk flow
		_halrf_txgapk_rxagc_onoff_8832crvu(rf, path, false);
		...
		if (band == BAND_ON_24G) {
			_halrf_txgapk_track_table_nctl_2g_8832crvu(rf, phy, path, 0x0);
			_halrf_txgapk_power_table_nctl_2g_8832crvu(rf, phy, path, 0x0);
		} else {
			_halrf_txgapk_track_table_nctl_5g_8832crvu(rf, phy, path, 0x0);
			_halrf_txgapk_power_table_nctl_5g_8832crvu(rf, phy, path, 0x0);
		}
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8832CRVU; path++) {
		_txgapk_reload_kip_8832crvu(rf, kip_reg, kip_bkup, path, TXGAPK_KIP_REG_NUM_8832CRVU);
		_txgapk_reload_rf_8832crvu(rf, rf_reg, rf_bkup, path, TXGAPK_RF_REG_NUM_8832CRVU);

		//follow dpk flow
		_halrf_txgapk_rxagc_onoff_8832crvu(rf, path, true);
	}

	//_txgapk_reload_bb_registers_8832crvu(rf, bb_reg, bb_bkup, TXGAPK_BB_REG_NUM_8832CRVU);
}



					
void _halrf_do_non_dbcc_txgapk_8832crvu(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 path;
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8832CRVU; path++) {
		...
		_halrf_txgapk_main_8832crvu(rf, phy, path, false);
	}
...
void _halrf_do_dbcc_txgapk_8832crvu(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	enum rf_path path = 0;
...
void _halrf_txgapk_get_ch_info_8832crvu(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
...
void  _halrf_sel_hw_table_txgapk_8832crvu(struct rf_info *rf, bool is_hw) 
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s \n", __func__);
...
void halrf_do_txgapk_8832crvu(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
	_halrf_txgapk_get_ch_info_8832crvu(rf, phy);

	//0x10055[19] SW gapk table selection
	//move to initial
	//_halrf_sel_hw_table_txgapk_8832crvu(rf, phy, false); //use sw control
	...
	if (rf->hal_com->dbcc_en)
		_halrf_do_dbcc_txgapk_8832crvu(rf, phy);
	else
		_halrf_do_non_dbcc_txgapk_8832crvu(rf, phy);
...
void halrf_txgapk_init_8832crvu(struct rf_info *rf) 
{	
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	...
	if(!txgapk_info->is_gapk_init) {
		...
		_halrf_sel_hw_table_txgapk_8832crvu(rf, false); //use sw control			
	}
}

#if 0
void halrf_txgapk_enable_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy)
{
	...
	for (i = RF_PATH_A; i < 2; i++) {
		_halrf_txgapk_write_track_table_8832crvu(rf, phy, i);
		_halrf_txgapk_write_power_table_8832crvu(rf, phy, i);
	}
}
#endif


void halrf_txgapk_write_table_default_8832crvu
	(struct rf_info *rf, enum phl_phy_idx phy)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s   phy=%d\n", __func__, phy);
	
	_halrf_do_reset_tbl_txgapk_8832crvu(rf, phy);
}
