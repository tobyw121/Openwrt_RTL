                      ...
                      u32 cmd_timeout)
{
	...
	if (!wifi_role)
	{
		PHL_ERR("%s: assign phl_info/drv failed!\n", __func__);
		goto _exit;
	}

	phl_info = wifi_role->phl_com->phl_priv;
	drv = wifi_role->phl_com->drv_priv;
...
void rtw_phl_reset_chdef(struct rtw_wifi_role_t *wifi_role)
{
	...
	rtw_hal_reset_chdef(phl_info->hal, wifi_role->hw_band);
}
		...
		struct rtw_mr_chctx_info *mr_cc_info)
{
	...
	} else { /*already in DBCC*/
		/*TODO - loop band0/band1*/
	}
	#endif
		...
		struct rtw_mr_chctx_info *mr_cc_info)
{
	...
	enum mr_op_mode sop_mode = MR_OP_MAX;
	u8 band_role_num = 0;
	int chanctx_num = 0;
