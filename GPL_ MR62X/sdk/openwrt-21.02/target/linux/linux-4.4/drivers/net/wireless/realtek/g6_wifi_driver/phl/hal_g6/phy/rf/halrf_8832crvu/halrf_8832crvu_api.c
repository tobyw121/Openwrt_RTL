...
u8 halrf_get_thermal_8832crvu(struct rf_info *rf, enum rf_path rf_path)
{
...
void halrf_set_syn01_8832crvu(struct rf_info *rf, bool dbcc_en)
{
	if (dbcc_en == 1) {
		/*SYN0 on + SYN1 on*/
		halrf_wrf(rf, RF_PATH_A, 0xA0, (BIT(3) | BIT(2)), 0x3);
		halrf_wrf(rf, RF_PATH_B, 0xA0, (BIT(3) | BIT(2)), 0x3);
		/*halrf_by_rate_dpd_8832crvu(rf, HW_PHY_1);*/
	} else if (dbcc_en == 0) {
		/*SYN0 on + SYN1 off*/
		halrf_wrf(rf, RF_PATH_A, 0xA0, (BIT(3) | BIT(2)), 0x3);
...
u32 halrf_mac_get_pwr_reg_8832crvu(struct rf_info *rf, enum phl_phy_idx phy,
	u32 addr, u32 mask)
{
...
u32 halrf_mac_set_pwr_reg_8832crvu(struct rf_info *rf, enum phl_phy_idx phy,
...
void halrf_wlan_tx_power_control_8832crvu(struct rf_info *rf,
	...
	u32 tx_power_val, bool enable)
{
	...
	if (enable == false) {
		if (pwr_ctrl_idx == ALL_TIME_CTRL) {
			...
			halrf_mac_set_pwr_reg_8832crvu(rf, phy_tmp, 0xd200, 0x3ff, 0x0);
		} else if (pwr_ctrl_idx == GNT_TIME_CTRL) {
			/*GNT_BT control Disable*/
			halrf_mac_set_pwr_reg_8832crvu(rf, phy_tmp, 0xd220, BIT(1), 0x0);
			halrf_mac_set_pwr_reg_8832crvu(rf, phy_tmp, 0xd220, 0xff8, 0x0);
		}
	} else {
		if (pwr_ctrl_idx == ALL_TIME_CTRL) {
			halrf_mac_set_pwr_reg_8832crvu(rf, phy_tmp, 0xd200, 0x3ff,
				((tx_power_val & 0x1ff) | BIT(9)));
		} else if (pwr_ctrl_idx == GNT_TIME_CTRL) {
			halrf_mac_set_pwr_reg_8832crvu(rf, phy_tmp, 0xd220, BIT(1), 0x1);
			halrf_mac_set_pwr_reg_8832crvu(rf, phy_tmp, 0xd220, 0xff8, tx_power_val & 0x1ff);
		} else {
...
bool halrf_wl_tx_power_control_8832crvu(struct rf_info *rf, u32 tx_power_val)
{
	...
	if (pwr->coex_pwr_ctl_enable == false && pwr->dpk_pwr_ctl_enable == false) {
		/*all-time control Disable*/
		result = halrf_mac_set_pwr_reg_8832crvu(rf, phy, 0xd200, 0x3ff, 0x0);
		...
		/*all-time control*/
		result = halrf_mac_set_pwr_reg_8832crvu(rf, phy, 0xd200, 0x3ff, ((tmp_pwr & 0x1ff) | BIT(9)));
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd200) fail, error code=%d\n",
				__func__, result);
			return false;
	...
	if (gnt_bt_control == 0xffff) {
		...
		result = halrf_mac_set_pwr_reg_8832crvu(rf, phy, 0xd220, BIT(1), 0x0);
		result = halrf_mac_set_pwr_reg_8832crvu(rf, phy, 0xd220, 0xff8, 0x0);
		if (result) {
		...
		result = halrf_mac_set_pwr_reg_8832crvu(rf, phy, 0xd220, BIT(1), 0x1);
		result = halrf_mac_set_pwr_reg_8832crvu(rf, phy, 0xd220, 0xff8, gnt_bt_control & 0x1ff);
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd220) fail, error code=%d\n",
				__func__, result);
			return false;
...
s8 halrf_get_ther_protected_threshold_8832crvu(struct rf_info *rf)
{
	u8 tmp_a, tmp_b, tmp;
	u8 therml_max = 0x32;
...
s8 halrf_xtal_tracking_offset_8832crvu(struct rf_info *rf,
...
void halrf_syn1_onoff_8832crvu(struct rf_info *rf, enum phl_phy_idx phy, u8 path, bool syn1_turn_on)
{
	if(path != RF_PATH_B)
		return;
...
void halrf_txck_force_8832crvu(struct rf_info *rf, enum rf_path path, bool force, enum dac_ck ck)
{
...
void halrf_rxck_force_8832crvu(struct rf_info *rf, enum rf_path path, bool force, enum adc_ck ck)
{
...
void halrf_set_gpio_8832crvu(struct rf_info *rf, enum phl_phy_idx phy)
{
...
u32 bkup_kip_reg_8832crvu[] = {0x813c, 0x8124, 0x8120, 0xc0d4, 0xc0d8};
u32 bkup_bb_reg_8832crvu[] = {0x2344};
u32 bkup_rf_reg_8832crvu[] = {0x5, 0x10005, 0xdf};

u32 bkup_kip_val_8832crvu[RF_PATH_MAX_NUM][RF_BACKUP_KIP_REG_MAX_NUM] = {{0x0}};
u32 bkup_bb_val_8832crvu[RF_BACKUP_BB_REG_MAX_NUM] = {0x0};
u32 bkup_rf_val_8832crvu[RF_PATH_MAX_NUM][RF_BACKUP_RF_REG_MAX_NUM] = {{0x0}};

void do_bkup_kip_8832crvu(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_kip_reg_8832crvu);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_kip_val_8832crvu[path][i] = halrf_rreg(rf, bkup_kip_reg_8832crvu[i] + (path << 8), MASKDWORD);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Backup KIP(S%d) 0x%x = %x\n", path, bkup_kip_reg_8832crvu[i] + (path << 8), bkup_kip_val_8832crvu[path][i]);
	}
}

void do_bkup_bb_8832crvu(struct rf_info *rf)
{
	u32 i;
	u32 num = ARRAY_SIZE(bkup_bb_reg_8832crvu);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_bb_val_8832crvu[i] =
			halrf_rreg(rf, bkup_bb_reg_8832crvu[i], MASKDWORD);

		RF_DBG(rf, DBG_RF_RFK, "[RFK]backup bb reg : %x, value =%x\n",
		      bkup_bb_reg_8832crvu[i], bkup_bb_val_8832crvu[i]);
	}
}


void do_bkup_rf_8832crvu(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_rf_reg_8832crvu);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_rf_val_8832crvu[path][i] = halrf_rrf(rf, path, bkup_rf_reg_8832crvu[i], MASKRF);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Backup RF S%d 0x%x = %x\n",
			path, bkup_rf_reg_8832crvu[i], bkup_rf_val_8832crvu[path][i]);
	}
}

void do_reload_kip_8832crvu(struct rf_info *rf, u8 path) 
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_kip_reg_8832crvu);

	for (i = 0; i < num; i++) {
		halrf_wreg(rf, bkup_kip_reg_8832crvu[i] + (path << 8), MASKDWORD, bkup_kip_val_8832crvu[path][i]);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Reload KIP(S%d) 0x%x = %x\n",path, bkup_kip_reg_8832crvu[i] + (path << 8),
			bkup_kip_val_8832crvu[path][i]);
	}
}

void do_reload_bb_8832crvu(struct rf_info *rf)
{
	u32 i;
	u32 num = ARRAY_SIZE(bkup_bb_reg_8832crvu);

	for (i = 0; i < num; i++) {
		halrf_wreg(rf,  bkup_bb_reg_8832crvu[i], MASKDWORD, bkup_bb_val_8832crvu[i]);
...
void do_reload_rf_8832crvu(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_rf_reg_8832crvu);

	for (i = 0; i < num; i++) {
		halrf_wrf(rf, path, bkup_rf_reg_8832crvu[i], MASKRF, bkup_rf_val_8832crvu[path][i]);
...
void halrf_set_regulation_from_driver_8832crvu(struct rf_info *rf,
	u8 regulation_idx)
{
	struct halrf_pwr_info *pwr = &rf->pwr_info;
...
bool halrf_set_dbcc_8832crvu(struct rf_info *rf, bool dbcc_en)
{
	halrf_set_syn01_8832crvu(rf, dbcc_en);
	if (dbcc_en) {
...
void halrf_set_share_xtal_8832crvu(struct rf_info *rf)
{
	u8 value;
