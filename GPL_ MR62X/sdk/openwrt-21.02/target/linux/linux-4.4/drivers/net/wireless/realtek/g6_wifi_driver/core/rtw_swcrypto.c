	...
	const u8 *data, size_t data_len, u8 *mic)
{
	...
	if (key_len == 16) {
		if (omac1_aes_128_g6(key, data, data_len, mic)) {
			res = _FAIL;
			RTW_ERR("%s : omac1_aes_128_g6 fail!", __func__);
		}
	} else if (key_len == 32) {
...
void _tdls_generate_tpk(void *sta, const u8 *own_addr, const u8 *bssid)
{
	...
	sha256_prf_g6(key_input, SHA256_MAC_LEN, "TDLS PMK", data, sizeof(data), (u8 *)&psta->tpk, sizeof(psta->tpk));
}
#endif /* CONFIG_TDLS */
