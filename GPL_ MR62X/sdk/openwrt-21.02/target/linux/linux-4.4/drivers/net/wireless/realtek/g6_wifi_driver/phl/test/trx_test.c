...
void rtw_phl_test_rx_callback(void *context)
{
	...
	while (!list_empty(&rx_req_pool->busy_list)) {
		...
		frame = recvpkt->rx_buf->frame;
		frame_len = recvpkt->rx_buf->frame_len;
		#else
		frame = recvpkt->pkt_list[0].vir_addr;
		frame_len = recvpkt->pkt_list[0].length;
		#endif /* CONFIG_RTW_HW_RX_AMSDU_CUT */
		debug_dump_data(frame, frame_len, "Rx Pkt: ");
...
void _phl_rx_test_pattern(struct phl_info_t *phl_info, void *ptr)
{
	...
	#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
	frame = rpkt->rx_buf->frame;
	frame_len = rpkt->rx_buf->frame_len;
	#else
	frame = rpkt->pkt_list[0].vir_addr;
	frame_len = rpkt->pkt_list[0].length;
	#endif /* CONFIG_RTW_HW_RX_AMSDU_CUT */
	PHL_INFO("rpkt->buf_len = %d\n", frame_len);
	debug_dump_data(frame, (u16)frame_len, "dump_rx");
