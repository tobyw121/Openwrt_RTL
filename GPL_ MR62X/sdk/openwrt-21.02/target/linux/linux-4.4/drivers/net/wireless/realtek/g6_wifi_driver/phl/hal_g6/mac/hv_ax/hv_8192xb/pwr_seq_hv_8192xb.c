...
#include "pwr_seq_hv_8192xb.h"
#include "../../mac_ax/mac_priv.h"
#include "../../mac_ax/mac_8192xb/pwr_seq_func_8192xb.h"
#if MAC_AX_8192XB_SUPPORT

u32 hv_run_pwr_seq_8192xb(struct mac_ax_adapter *adapter,
			  enum hv_ax_pwr_seq_sel sel)
{
	...
	case HV_AX_PWR_SEQ_SEL_ON_NIC:
		...
		if (adapter->hw_info->intf == MAC_AX_INTF_PCIE)
			p_ops->pwr_on = mac_pwr_on_nic_pcie_8192xb;
	...
	case HV_AX_PWR_SEQ_SEL_LEAVE_LPS:
		...
		if (adapter->hw_info->intf == MAC_AX_INTF_PCIE)
			mac_leave_lps_pcie_8192xb(adapter);
		else if (adapter->hw_info->intf == MAC_AX_INTF_USB)
			mac_leave_lps_usb_8192xb(adapter);
		else if (adapter->hw_info->intf == MAC_AX_INTF_SDIO)
			mac_leave_lps_sdio_8192xb(adapter);
		else
			ret = MACINTF;
		break;
	case HV_AX_PWR_SEQ_SEL_ON_AP:
		tmp = p_ops->pwr_on;
		if (adapter->hw_info->intf == MAC_AX_INTF_PCIE)
			p_ops->pwr_on = mac_pwr_on_ap_pcie_8192xb;
		ret = adapter->ops->pwr_switch(adapter, 1);
		p_ops->pwr_on = tmp;
		break;
	case HV_AX_PWR_SEQ_SEL_OFF_AP:
		tmp = p_ops->pwr_off;
		if (adapter->hw_info->intf == MAC_AX_INTF_PCIE)
			p_ops->pwr_off = mac_pwr_off_ap_pcie_8192xb;
		ret = adapter->ops->pwr_switch(adapter, 0);
		p_ops->pwr_off = tmp;
		break;
	default:
