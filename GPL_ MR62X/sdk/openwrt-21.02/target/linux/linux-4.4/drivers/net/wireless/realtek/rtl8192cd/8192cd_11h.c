/*
 *  802.11h type definition and declaration
 *
...
static const BAND_TABLE_ELEMENT country_ie_channel_5g[] = {
	...
	/* (2) IC */		  {22, {{36,1,30}, {40,1,30}, {44,1,30}, {48,1,30},
                            {52,1,30}, {56,1,30}, {60,1,30}, {64,1,30},
                            {100,1,20},{104,1,20},{108,1,20},{112,1,20},{116,1,20},{132,1,20},{136,1,20},{140,1,20},{144,1,20},
                            {149,1,30},{153,1,30},{157,1,30},{161,1,30},{165,1,30}  }
                      },                 
	...
	/* (6) MKK */     {20,  {{36,1,30}, {40,1,30}, {44,1,30}, {48,1,30}, 
	                         {52,1,30}, {56,1,30}, {60,1,30}, {64,1,30}, 
	                         {100,1,20},{104,1,20},{108,1,20},{112,1,20},{116,1,20},{120,1,20},{124,1,20},{128,1,20},{132,1,20},{136,1,20},{140,1,20},{144,1,20}  }
                      },
	...
	if (COUNTRY_CODE_ENABLED || priv->pmib->dot11hTPCEntry.tpc_enable) {
		...
		for (i = 0; i < country_num; i++) {
			if (!memcmp(priv->pmib->dot11dCountry.dot11CountryString, countryIEArray[i].countryA2, 2)) {
				...
				//printk("countryTableIdx(%d)\n", priv->countryTableIdx);
				break;
			}
		}
...
unsigned char *construct_country_ie(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int *frlen)
{
	...
	unsigned int len = 0;
	unsigned char last_octet_in_country_string = 0x04;	// indicates use of the Gloabal Operating Class Table
	...
	if (band_table) {
		memcpy(temp + len, country_ie->countryA2, 2);
		len += 2;
		memcpy(temp + len, &last_octet_in_country_string, 1);
		len += 1;
		...
		priv->country_ie_len = len;
		memcpy(priv->country_ie,temp,priv->country_ie_len);
