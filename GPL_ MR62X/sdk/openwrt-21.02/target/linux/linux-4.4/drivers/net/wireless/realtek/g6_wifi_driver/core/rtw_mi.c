...
u8 rtw_mi_stayin_union_ch_chk(_adapter *adapter)
{
	...
	if (rst == _FALSE) {
		...
		if (GET_HAL_DATA(adapter_to_dvobj(adapter))->bScanInProcess == _TRUE)
...
int rtw_mi_get_ch_setting_union_by_ifbmp(struct dvobj_priv *dvobj, u16 ifbmp, u8 *ch, u8 *bw, u8 *offset)
{
	_adapter *iface;
...
{
	return rtw_mi_get_ch_setting_union_by_ifbmp(adapter_to_dvobj(adapter), 0xFFFF, ch, bw, offset);
}

inline int rtw_mi_get_ch_setting_union_no_self(_adapter *adapter, u8 *ch, u8 *bw, u8 *offset)
{
	return rtw_mi_get_ch_setting_union_by_ifbmp(adapter_to_dvobj(adapter), 0xFFFF & ~BIT(adapter->iface_id), ch, bw, offset);
}

/* For now, not return union_ch/bw/offset */
u16 rtw_mi_status_by_ifbmp(struct dvobj_priv *dvobj, u16 ifbmp, struct mi_state *mstate)
{
	_adapter *iface;
	int i;
	u16 ret = ifbmp;
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if_bit = BIT(iface->iface_id);

		if (!(ifbmp & if_bit))
			continue;

		/* Don't care interface not up. Remove it from bitmap */
		if (!iface->netif_up) {
			ret &= ~if_bit;
			continue;
		}
...
u8 rtw_mi_hal_iface_init(_adapter *padapter)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (iface) {
			if (iface->run_cmd_en == 0) {
				if (rtw_set_run_cmd_en(iface, 1, RTW_CMDF_WAIT_ACK) != _SUCCESS)
					RTW_INFO("[%s %d] rtw_set_run_cmd_en fail, value = %u\n", __FUNCTION__, __LINE__, iface->run_cmd_en);
				else
...
static u8 _rtw_mi_traffic_statistics(_adapter *padapter , void *data)
{
	...
	for (i = 0; i < 4; i++)
		pdvobjpriv->tx_wmm_pkts[i] += padapter->xmitpriv.tx_wmm_pkts[i];
...
static u8 _rtw_mi_sw_feature_timer_handlder(_adapter *adapter, void *data)
{
	rtw_iface_sw_feature_timer_handlder(adapter);
	return _TRUE;
}
u8 rtw_mi_sw_feature_timer_handlder(_adapter *padapter)
{
	return _rtw_mi_process(padapter, _FALSE, NULL, _rtw_mi_sw_feature_timer_handlder);
}
...
_adapter *rtw_get_iface_by_macddr(_adapter *padapter, const u8 *mac_addr)
{
	...
	if (phl_is_mp_mode(dvobj->phl_com))
		return GET_PRIMARY_ADAPTER(padapter);
...
u16 rtw_mi_get_ld_sta_ifbmp(_adapter *adapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	int i;
	_adapter *iface = NULL;
...
u16 rtw_mi_get_ap_mesh_ifbmp(_adapter *adapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	int i;
	_adapter *iface = NULL;
