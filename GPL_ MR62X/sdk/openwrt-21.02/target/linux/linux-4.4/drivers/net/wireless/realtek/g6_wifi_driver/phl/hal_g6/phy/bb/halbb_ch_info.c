			 ...
			 char *output, u32 *_out_len)
{
	...
	BB_TRACE("Data=%d Byte, grp=1/%d, H:[%dx%d]\n",
		 ...
		 size->n_r, size->n_c);
	halbb_print_devider(bb, BB_DEVIDER_LEN_32, true);
	...
	for (i = 0; i < tone_num; i++) {
		...
		if (size->data_byte == 1 && i == shift_tone) {
		...
		halbb_print_devider(bb, BB_DEVIDER_LEN_16, false);
	}
}
			     ...
			     struct bb_ch_rpt_size_info *exp_rpt_size,enum phl_phy_idx phy_idx)
{
	...
	halbb_mem_cpy(bb, exp_rpt_size, &ch_rpt->bb_ch_rpt_size_i, sizeof(struct bb_ch_rpt_size_info));
}
...
void halbb_ch_info_status_en(struct bb_info *bb, bool en, enum phl_phy_idx phy_idx)
{
	...
	BB_DBG(bb, DBG_CH_INFO, "[%s] en=%dn", __func__, en);
					...
					struct bb_ch_info_drv_rpt *drv)
{
	...
	BB_DBG(bb, DBG_CH_INFO, "[IN] skip=%d\n", ch_rpt->skip_ch_info);

	BB_DBG(bb, DBG_CH_INFO, "[Hdr]lenth_curr: %d = %d - %d\n",
	...
	/*SEG = N-1 (LAST SEG)*/
		...
		BB_DBG(bb, DBG_CH_INFO,
		 ...
		 if (ch_rpt->skip_ch_info) {
			...
			BB_DBG(bb, DBG_CH_INFO, "[LAST][FAIL]drv_idx=%d != hdr_idx=%d\n",
			...
			BB_DBG(bb, DBG_CH_INFO, "[LAST][FAIL]raw_data_len=%d < total_len_remnant=%d\n",
			...
			BB_DBG(bb, DBG_CH_INFO, "[LAST] len_remnant=%d\n", ch_rpt->total_len_remnant);
	...
	if (ch_rpt->skip_ch_info) {
		BB_DBG(bb, DBG_CH_INFO, "[OUT]skip=%d\n", ch_rpt->skip_ch_info);
		return BB_CH_INFO_FAIL;
	}

	BB_DBG(bb, DBG_CH_INFO, "seg_idx_pre=%d\n", ch_rpt->seg_idx_pre);
	...
	/*SEG = 0*/
		...
		BB_DBG(bb, DBG_CH_INFO, "[PHY_info]lenth_curr: %d = %d - %d\n",
			drv->raw_data_len, len_tmp, ch_rpt->phy_info_len);

		BB_DBG(bb, DBG_CH_INFO, "[PHY_info]RSSI= {%d, %d, %d}\n",
		...
		if (drv->raw_data_len > ch_rpt->total_len_remnant) {
			BB_DBG(bb, DBG_CH_INFO, "[FAIL]raw_data_len > total_len_remnant\n");
			ch_rpt->skip_ch_info = true;
			return BB_CH_INFO_FAIL;
		} else if (phy_info->rsvd_0 != 0 || phy_info->rsvd_1 != 0 ||
		    phy_info->rsvd_2 != 0) {
		    	BB_DBG(bb, DBG_CH_INFO, "[FAIL]rsvd_0,1,2: %d, %d, %d\n",
		       		phy_info->rsvd_0, phy_info->rsvd_1, phy_info->rsvd_2);
			ch_rpt->skip_ch_info = true;
			return BB_CH_INFO_FAIL;
		} else if (drv->seg_idx_curr != hdr->seq_num) {
			BB_DBG(bb, DBG_CH_INFO, "[FAIL]drv_idx=%d != hdr_idx=%d\n",
			       drv->seg_idx_curr, hdr->seq_num);
	...
	/*SEG = 1~N-2*/
		...
		if (drv->seg_idx_curr >= (hdr->total_seg_num - 1)) { /*@ >= 3*/
			BB_DBG(bb, DBG_CH_INFO, "[FAIL]Los Last Seg\n");
			ch_rpt->skip_ch_info = true;
			return BB_CH_INFO_FAIL;
		}
		...
		BB_DBG(bb, DBG_CH_INFO, "lenth_curr: %d\n", drv->raw_data_len); /*@88 = 96 - 8*/
...
void halbb_ch_info_init(struct bb_info *bb)
{
	...
	u16 tone_num_lgcy[1][CH_DESI_OPT_NUM] = {{52, 26, 13, 4}}; /*lgcy 20M*/
	u16 tone_num[CH_INFO_BW_NUM][CH_DESI_OPT_NUM] = {{56, 28, 14, 4}, /*20M*/
							 {114, 58, 30, 8}, /*40M*/
							 {242, 122, 62, 16}, /*80M*/
							 {500, 300, 150, 32}}; /*160M TBD*/
	u16 tone_num_he[CH_INFO_BW_NUM][CH_DESI_OPT_NUM]= {{244, 122, 62, 16}, /*20M*/
							   {488, 244, 122, 32}, /*40M*/
							   {1000, 498, 250, 64}, /*80M*/
							   {2000, 1000, 500, 128}}; /*160M TBD*/
#if 0
	u8 i, j;

	for (i = 0; i < CH_INFO_BW_NUM; i++) {
		for (j = 0; j < CH_DESI_OPT_NUM; j++) {
			tone_num_he[i][j] = tone_num[i][j] << 2;
		}
	}
#endif
