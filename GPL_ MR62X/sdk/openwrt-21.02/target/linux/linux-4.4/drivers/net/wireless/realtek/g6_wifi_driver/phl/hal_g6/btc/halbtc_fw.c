			...
			u8 *prptbuf, u32 index)
{
	...
	if (rpt_type == BTC_RPT_TYPE_CTRL) {
		...
		hal_mem_cpy(hal, (void *)&dm->gnt_val[HW_PHY_0],
			    (void *)&prpt->gnt_val[HW_PHY_0][0],
			    sizeof(struct btc_gnt_ctrl));
		hal_mem_cpy(hal, (void *)&dm->gnt_val[HW_PHY_1],
			    (void *)&prpt->gnt_val[HW_PHY_1][0],
			    sizeof(struct btc_gnt_ctrl));
		...
		if (prpt->bt_cnt[BTC_BCNT_RFK_TIMEOUT] > 0)
