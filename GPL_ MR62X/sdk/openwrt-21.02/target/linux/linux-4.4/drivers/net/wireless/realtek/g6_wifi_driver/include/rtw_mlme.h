...
#define WIFI_DELETE_STATE			0x00001000
...
#define __CHK_MLME_STATE(adapter, state) __check_fwstate(adapter, (state))

#undef MLME_IS_STA
#define MLME_IS_STA(adapter) __CHK_MLME_STATE(adapter, WIFI_STATION_STATE)

#undef MLME_IS_AP
#define MLME_IS_AP(adapter) __CHK_MLME_STATE(adapter, WIFI_AP_STATE)

#undef MLME_IS_MESH
#define MLME_IS_MESH(adapter) __CHK_MLME_STATE(adapter, WIFI_MESH_STATE)

#undef MLME_IS_MP
#define MLME_IS_MP(adapter) __CHK_MLME_STATE(adapter, WIFI_MP_STATE)
...
#define rtw_ft_enable(a) rtw_ft_chk_flags(a, RTW_FT_EN)
...
#define rtw_wnm_get_ext_cap_btm(_pEleStart) \
...
struct mlme_priv {
	...
	/* Overlapping Legacy BSS Condition (Legacy b/g)*/
	ATOMIC_T olbc_ap;
	/* Overlapping Legacy STA Condition (Legacy b/g)*/
	ATOMIC_T olbc_sta;
	int olbc_count;
	...
	u8 vendor_mac[WLAN_MAX_VENDOR_IE_NUM][ETH_ALEN];
	u8 vendor_mac_is_set[WLAN_MAX_VENDOR_IE_NUM];
	u8 vendor_mac_is_same[WLAN_MAX_VENDOR_IE_NUM];
	u32 vendor_ie_duration[WLAN_MAX_VENDOR_IE_NUM];
	u32 vendor_ie_duration_count[WLAN_MAX_VENDOR_IE_NUM];
	u8 vendor_service_name_len[WLAN_MAX_VENDOR_IE_NUM];
	u8 vendor_service_name[WLAN_MAX_VENDOR_IE_NUM][64];
	u8 vendor_timestamp_is_set[WLAN_MAX_VENDOR_IE_NUM];
	u64 vendor_timestamp[WLAN_MAX_VENDOR_IE_NUM];
#endif
};
...
extern void rtw_sw_feature_timer_handlder(void *ctx);
extern void rtw_iface_sw_feature_timer_handlder(_adapter *adapter);
...
#define GET_IPV4_IPID(_iphdr)		BE_BITS_TO_2BYTE(((u8 *)(_iphdr)) + 4, 0, 16)
...
#define GET_IPV6_PROTOCOL(_ip6hdr)	BE_BITS_TO_1BYTE(((u8 *)(_ip6hdr)) + 6, 0, 8)
#define GET_IPV6_opt_nexttype(_opthdr)	BE_BITS_TO_1BYTE(((u8 *)(_opthdr)) + 0, 0, 8)
#define GET_IPV6_opt_hdrlen(_opthdr)	BE_BITS_TO_1BYTE(((u8 *)(_opthdr)) + 1, 0, 8)
