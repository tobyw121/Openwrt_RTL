 ...
 */
static enum rtw_hal_status
_hal_parsing_rx_wd_8852a(struct hal_info_t *hal,
#ifdef PHL_RXSC_AMPDU
	...
	u8 *desc, struct rtw_r_meta_data *mdata)
{
	...
	if (RXSC_INVALID_MACID != phl_com->rxsc_entry.cached_rx_macid &&
		RX_8852A_DESC_PKT_T_WIFI == mdata->rpkt_type) {
		...
		if (mdata->long_rxd == 1) {
			...
			mdata->sec_type = GET_RX_AX_DESC_SEC_TYPE_8852A(desc);
		}
	}
...
void _hal_revise_rx_rate(struct hal_info_t *hal, struct rtw_r_meta_data *mdata)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u16 nss;
	...
	if (mdata->rpkt_type != RX_8852A_DESC_PKT_T_WIFI ||
		mdata->ppdu_type != RX_8852A_DESC_PPDU_T_HE_SU ||
		mdata->rx_gi_ltf != RX_8852A_DESC_RX_GI_LTF_4X_0_8)
		return;

	nss = (mdata->rx_rate & 0x70) >> 4;
	if (nss == 0)
		mdata->rx_rate = (mdata->rx_rate & 0xff8f) | 0x10;
	else if (nss == 7)
		mdata->rx_rate = (mdata->rx_rate & 0xff8f);
}
				...
				struct rtw_r_meta_data *mdata)
{
	...
	do {
		...
		if (!halmac_rx) {
#ifdef PHL_RXSC_AMPDU
			hstatus = _hal_parsing_rx_wd_8852a(hal, phl_com, desc, mdata);
#else
			hstatus = _hal_parsing_rx_wd_8852a(hal, desc, mdata);
#endif
			_hal_revise_rx_rate(hal, mdata);
		} else {
...
void
_hal_rx_ppdu_sts_normal_data_8852a(struct rtw_phl_com_t *phl_com,
				   struct hal_info_t *hal,
				   void *hdr,
...
void
_hal_rx_ppdu_sts_8852a(struct rtw_phl_com_t *phl_com,
			    struct rtw_r_meta_data *meta,
...
#endif
		{
		_hal_rx_wlanhdr_check_8852a(drv, frame, mdata);
		_hal_rx_sts_8852a(hal, mdata);
		hal_rx_ppdu_sts_normal_data(phl_com, frame, mdata);
}
	}
	break;
