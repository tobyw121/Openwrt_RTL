...
void _dpk_fem_control_setting_8197g(
	struct dm_struct *dm)
{	
	if ((dm->rfe_type == 3) || (dm->rfe_type == 4)) { /*Force TRSW to Tx path*/
		/*WBB1(TRSWP0)=RFE_CTRL8*/
		odm_set_bb_reg(dm, R_0x1844, 0x0000000f, 0x7);
		odm_set_bb_reg(dm, R_0x183c, BIT(8), 0x1);
		/*WBB7(LNAON0)=RFE_CTRL4*/
		odm_set_bb_reg(dm, R_0x1840, 0x000f0000, 0x7);
		/*WBB4(TRSWP1)=RFE_CTRL1 */
		odm_set_bb_reg(dm, R_0x4140, 0x000000f0, 0x7);
		odm_set_bb_reg(dm, R_0x413c, BIT(1), 0x1);
		/*WBB2(LNAON1)=RFE_CTRL9*/
		odm_set_bb_reg(dm, R_0x4144, 0x000000f0, 0x7);
	} else if (dm->rfe_type == 6) { /*iPA + RX_FEM*/
		/*WBB8(PAPE0)=RFE_CTRL5*/
		odm_set_bb_reg(dm, R_0x1840, 0x00F00000, 0x7); /*[23:20]*/
		odm_set_bb_reg(dm, R_0x183c, BIT(5), 0x1);
		/*WBB7(LNAON0)=RFE_CTRL4*/
		odm_set_bb_reg(dm, R_0x1840, 0x000F0000, 0x7); /*[19:16]*/
		odm_set_bb_reg(dm, R_0x183c, BIT(4), 0x0);
		/*WBB1(TRSWN0)=RFE_CTRL8*/
		odm_set_bb_reg(dm, R_0x1844, 0x0000000F, 0x7); /*[3:0]*/
		odm_set_bb_reg(dm, R_0x183c, BIT(8), 0x0);
		/*WBB3(PAPE1)=RFE_CTRL0 */
		odm_set_bb_reg(dm, R_0x4140, 0x0000000F, 0x7); /*[3:0]*/
		odm_set_bb_reg(dm, R_0x413c, BIT(0), 0x1);
		/*WBB2(LNAON1)=RFE_CTRL9*/
		odm_set_bb_reg(dm, R_0x4144, 0x000000F0, 0x7); /*[7:4]*/
		odm_set_bb_reg(dm, R_0x413c, BIT(9), 0x0);
		/*WBB4(TRSWN1)=RFE_CTRL1*/
		odm_set_bb_reg(dm, R_0x4140, 0x000000F0, 0x7); /*[7:4]*/
		odm_set_bb_reg(dm, R_0x413c, BIT(1), 0x0);
	...
	RF_DBG(dm, DBG_RF_DPK, "[DPK] FEM control for RFE type %d\n", dm->rfe_type);
}
...
//	struct dm_dpk_info *dpk_info = &dm->dpk_info;
	...
	_dpk_fem_control_setting_8197g(dm);
	...
	struct dm_struct *dm)
{
	...
	for (path = 0; path < DPK_RF_PATH_NUM_8197G; path++) {
		...
		if (dm->rfe_type == 6)
			odm_set_bb_reg(dm, R_0x1bbc, MASKDWORD, 0x000055df);
	}
	...
	void *dm_void)
{
	...
	u32 bb_reg[DPK_BB_REG_NUM_8197G] = {
		...
		R_0x18a4, R_0x41a4, R_0x1e7c, R_0x183c, R_0x1840,
		R_0x1844, R_0x413c, R_0x4140, R_0x4144};
	...
	void *dm_void)
{
	...
	u8 i = 0, path, pwsf[2];//is_increase,  k = 0,
	...
	if ((dpk_info->thermal_dpk[0] == 0) && (dpk_info->thermal_dpk[1] == 0)) {
		...
		RF_DBG(dm, DBG_RF_DPK_TRACK, "[DPK_track] No DPK tracking for A-Cv!!!!\n");
			return;
	} else	
