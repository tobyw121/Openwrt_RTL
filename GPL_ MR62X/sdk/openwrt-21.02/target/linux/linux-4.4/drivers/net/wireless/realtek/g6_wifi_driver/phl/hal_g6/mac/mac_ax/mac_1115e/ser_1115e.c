...
static u32 scheduler_imr_enable_1115e(struct mac_ax_adapter *adapter, u8 band)
{
...
static u32 ptcl_imr_enable_1115e(struct mac_ax_adapter *adapter, u8 band)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32, reg;
...
static u32 cdma_imr_enable_1115e(struct mac_ax_adapter *adapter, u8 band)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32, reg, reg_2;
...
static u32 phy_intf_imr_enable_1115e(struct mac_ax_adapter *adapter, u8 band)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	u32 reg;
...
static u32 rmac_imr_enable_1115e(struct mac_ax_adapter *adapter, u8 band)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32, reg;
...
static u32 tmac_imr_enable_1115e(struct mac_ax_adapter *adapter, u8 band)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32, reg;
...
static u32 wdrls_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 wsec_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;

	val32 = MAC_REG_R32(R_AX_SEC_ERROR_FLAG);
	...
	MAC_REG_W32(R_AX_SEC_ERROR_FLAG, val32);
	return 0;
}

static u32 mpdu_trx_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 sta_sch_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 txpktctl_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 wde_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 ple_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 pktin_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 dispatcher_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 cpuio_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
static u32 bbrpt_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;

	val32 = MAC_REG_R32(R_AX_BBRPT_COM_ERR_IMR);
	val32 &= ~(B_AX_BBRPT_COM_HANG_EN |
		   B_AX_BBRPT_COM_NULL_PLPKTID_ERR_INT_EN_V1);
...
static u32 haxidma_imr_enable_1115e(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;

	val32 = MAC_REG_R32(R_BE_HAXI_IDCT_MSK);
	val32 &= ~(B_BE__TXBD_LEN0_ERR_IDCT_MSK |
		   B_BE__TXBD_4KBOUND_ERR_IDCT_MSK |
		   B_BE_RXMDA_STUCK_IDCT_MSK |
		   B_BE_TXMDA_STUCK_IDCT_MSK);

	val32 |= ((B_BE__TXBD_LEN0_ERR_IDCT_MSK &
		   B_BE__TXBD_LEN0_ERR_IDCT_SER_EN) |
		  (B_BE__TXBD_4KBOUND_ERR_IDCT_MSK &
		   B_BE__TXBD_4KBOUND_ERR_IDCT_SER_EN) |
		  (B_BE_RXMDA_STUCK_IDCT_MSK &
		   B_BE_RXMDA_STUCK_IDCT_SER_EN) |
		  (B_BE_TXMDA_STUCK_IDCT_MSK &
		   B_BE_TXMDA_STUCK_IDCT_SER_EN));
	MAC_REG_W32(R_BE_HAXI_IDCT_MSK, val32);

	if (adapter->hw_info->intf == MAC_AX_INTF_USB ||
	    adapter->hw_info->intf == MAC_AX_INTF_SDIO) {
		val32 = MAC_REG_R32(R_BE_HAXI_IDCT_MSK);
		val32 &= ~(B_BE_RXMDA_STUCK_IDCT_MSK);
		MAC_REG_W32(R_BE_HAXI_IDCT_MSK, val32);
	}

	/* 1115e H_AXIDMA SER IMR Patch code */
			 ...
			 enum mac_ax_hwmod_sel sel)
{
	...
	if (sel == MAC_AX_DMAC_SEL) {
		ret = wdrls_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]wdrls_imr_enable %d\n", ret);
			return ret;
		}

		ret = wsec_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]wsec_imr_enable %d\n", ret);
			return ret;
		}

		ret = mpdu_trx_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]mpdu_trx_imr_enable %d\n", ret);
			return ret;
		}

		ret = sta_sch_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]sta_sch_imr_enable %d\n", ret);
			return ret;
		}

		ret = txpktctl_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]txpktctl_imr_enable %d\n", ret);
			return ret;
		}

		ret = wde_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]wde_imr_enable %d\n", ret);
			return ret;
		}

		ret = ple_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]ple_imr_enable %d\n", ret);
			return ret;
		}

		ret = pktin_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]pktin_imr_enable %d\n", ret);
			return ret;
		}

		ret = dispatcher_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]dispatcher_imr_enable %d\n", ret);
			return ret;
		}

		ret = cpuio_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]cpuio_imr_enable %d\n", ret);
			return ret;
		}

		ret = bbrpt_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]bbrpt_imr_enable %d\n", ret);
			return ret;
		}

		ret = haxidma_imr_enable_1115e(adapter);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]haxidma_imr_enable %d\n", ret);
			return ret;
		}
	} else if (sel == MAC_AX_CMAC_SEL) {
		ret = scheduler_imr_enable_1115e(adapter, band);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]scheduler_imr_enable %d\n", ret);
			return ret;
		}

		ret = ptcl_imr_enable_1115e(adapter, band);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]ptcl_imr_enable %d\n", ret);
			return ret;
		}

		ret = cdma_imr_enable_1115e(adapter, band);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]cdma_imr_enable %d\n", ret);
			return ret;
		}

		ret = phy_intf_imr_enable_1115e(adapter, band);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]phy_intf_imr_enable %d\n", ret);
			return ret;
		}

		ret = rmac_imr_enable_1115e(adapter, band);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]rmac_imr_enable %d\n", ret);
			return ret;
		}

		ret = tmac_imr_enable_1115e(adapter, band);
		if (ret) {
