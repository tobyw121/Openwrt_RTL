...
bool halrf_bw_setting_8832brvt(struct rf_info *rf, enum rf_path path, enum channel_width bw, bool is_dav)
{
	u32 rf_reg18 = 0;
...
bool halrf_ctrl_bw_8832brvt(struct rf_info *rf, enum phl_phy_idx phy, enum channel_width bw)
{
	...
	for (path = 0; path < 2; path++) {
		if (kpath & BIT(path)) {
			is_dav = true;
			halrf_bw_setting_8832brvt(rf, path, bw, is_dav);
			is_dav = false;
			halrf_bw_setting_8832brvt(rf, path, bw, is_dav);
	...
	halrf_bw_setting_op5k_8832brvt(rf);

	return true;
}

bool halrf_ch_setting_8832brvt(struct rf_info *rf,   enum rf_path path, u8 central_ch, enum band_type band, bool is_dav)
{
	u32 rf_reg18 = 0;	
	u32 reg_reg18_addr = 0x0;
...
bool halrf_ctrl_ch_8832brvt(struct rf_info *rf,  enum phl_phy_idx phy, u8 central_ch, enum band_type band)
{
	...
	for (path = 0; path < 2; path++) {
		if (kpath & BIT(path)) {
			is_dav = true;
			halrf_ch_setting_8832brvt(rf, path, central_ch, band, is_dav);
			is_dav = false;
			halrf_ch_setting_8832brvt(rf, path, central_ch, band, is_dav);	
		}
	}
...
void halrf_set_lo_8832brvt(struct rf_info *rf, bool is_on, enum rf_path path)
{
	if (is_on) {
		halrf_rf_direct_cntrl_8832brvt(rf, path, false);
		...
		halrf_rf_direct_cntrl_8832brvt(rf, path, true);
...
void halrf_lo_test_8832brvt(struct rf_info *rf, bool is_on, enum rf_path path)
{
	switch (path) {
		case RF_PATH_A:
			halrf_set_lo_8832brvt(rf, is_on, RF_PATH_A);
			halrf_set_lo_8832brvt(rf, false, RF_PATH_B);
			break;
		case RF_PATH_B:
			halrf_set_lo_8832brvt(rf, false, RF_PATH_A);
			halrf_set_lo_8832brvt(rf, is_on, RF_PATH_B);
			break;
		case RF_PATH_AB:
			halrf_set_lo_8832brvt(rf, is_on, RF_PATH_A);
			halrf_set_lo_8832brvt(rf, is_on, RF_PATH_B);
...
void _rx_dck_info_8832brvt(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_afe)
{
...
void halrf_set_rx_dck_8832brvt(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_afe)
{
	...
	_rx_dck_info_8832brvt(rf, phy, path, is_afe);
...
bool halrf_rx_dck_check_8832brvt(struct rf_info *rf, enum rf_path path)
{
	u8 addr;
	bool is_fail = false;
...
void halrf_rx_dck_8832brvt(struct rf_info *rf, enum phl_phy_idx phy, bool is_afe) 
{
	...
	RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] ****** RXDCK Start (Ver: 0x%x, Cv: %d) ******\n",
		RXDCK_VER_8832BRVT, rf->hal_com->cv);
	...
	for (path = 0; path < 2; path++) {
		...
		halrf_set_rx_dck_8832brvt(rf, phy, path, is_afe);
#if 0
		if (halrf_rx_dck_check_8832brvt(rf, path)) {
...
void iqk_backup_8832brvt(struct rf_info *rf, enum rf_path path) 
{
	return;
}
...
void halrf_set_dpd_backoff_8832brvt(struct rf_info *rf, enum phl_phy_idx phy)
{
...
{
	halrf_set_dpd_backoff_8832brvt(rf, HW_PHY_0);
}

void halrf_set_rxbb_bw_8832brvt(struct rf_info *rf, enum channel_width bw, enum rf_path path)
{
...
void halrf_rxbb_bw_8832brvt(struct rf_info *rf, enum phl_phy_idx phy, enum channel_width bw)
{
	...
	for (path = 0; path < 2; path++) {
		if ((kpath & BIT(path)) && (rf->pre_rxbb_bw[path] != bw)) {
			halrf_set_rxbb_bw_8832brvt(rf, bw, path);
...
bool halrf_one_shot_nctl_done_check_8832brvt(struct rf_info *rf, enum rf_path path) {
	...
	while (count < 2000) {			
		...
		if (r_80fc == NCTL_FINAL_LINE_8832BRVT) {
			is_ready = true;
			break;
		}	
		halrf_delay_us(rf, 1);
...
bool halrf_do_one_shot_8832brvt(struct rf_info *rf, enum rf_path path, u32 nctl_addr, u32 mask, u32 process_id)
{
	...
	return halrf_one_shot_nctl_done_check_8832brvt(rf, path);
}
...
void halrf_lck_8832brvt(struct rf_info *rf)
{
	...
	halrf_set_s0_arfc18_8852br(rf, temp18);
...
void halrf_lck_tracking_8832brvt(struct rf_info *rf)
{
	...
	RF_DBG(rf, DBG_RF_LCK, "[LCK]MAX_delta = %d, LCK_TH=%x\n",
		temp, LCK_TH_8832BRVT);

	if(temp >= LCK_TH_8832BRVT)		
		halrf_lck_trigger_g6(rf);
#if 0
	/*thermal FIFO*/
	for (i = 0; i < 3; i++) {
	...
	if(HALRF_ABS(rf->lck_ther, (u8)ther_avg) >= LCK_TH_8832BRVT)		
		halrf_lck_trigger_g6(rf);
#endif
}

static u32 backup_mac_reg_8832brvt[] = {0x0};
static u32 backup_bb_reg_8832brvt[] = {0x2344,0xc0d4,0xc0d8, 0xc0c4, 0xc0e8, 0xc0ec, 0xc1d4, 0xc1d8, 0xc1c4, 0xc1e8, 0xc1ec};
static u32 backup_rf_reg_8832brvt[] = {0xdc, 0xef, 0xde, 0x0, 0x1e, 0x5, 0x10005};

#if 1
static struct halrf_iqk_ops iqk_ops= {
    .iqk_kpath = halrf_kpath_8832brvt,
    .iqk_mcc_page_sel = iqk_mcc_page_sel_8832brvt,
    .iqk_get_ch_info = iqk_get_ch_info_8832brvt,
    .iqk_preset = iqk_preset_8832brvt,
    .iqk_macbb_setting = iqk_macbb_setting_8832brvt,
    .iqk_start_iqk = iqk_start_iqk_8832brvt,
    .iqk_restore = iqk_restore_8832brvt,
    .iqk_afebb_restore = iqk_afebb_restore_8832brvt,  
};

struct rfk_iqk_info rf_iqk_hwspec_8832brvt = {
  	.rf_iqk_ops = &iqk_ops,
	.rf_max_path_num = 2,
	.rf_iqk_version = iqk_version_8832brvt,
	.rf_iqk_ch_num = 2,
	.rf_iqk_path_num = 2,

#if 0
	.backup_mac_reg = backup_mac_reg_8832brvt,
	.backup_mac_reg_num = ARRAY_SIZE(backup_mac_reg_8832brvt),
#else
	.backup_mac_reg = backup_mac_reg_8832brvt,
	.backup_mac_reg_num = 0,
#endif

    	.backup_bb_reg = backup_bb_reg_8832brvt,
    	.backup_bb_reg_num = ARRAY_SIZE(backup_bb_reg_8832brvt),
    	.backup_rf_reg = backup_rf_reg_8832brvt,
    	.backup_rf_reg_num = ARRAY_SIZE(backup_rf_reg_8832brvt),
};
