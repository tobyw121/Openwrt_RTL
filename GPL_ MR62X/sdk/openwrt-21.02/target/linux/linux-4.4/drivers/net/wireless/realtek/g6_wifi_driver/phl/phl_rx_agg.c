...
void phl_tid_ampdu_rx_free(struct phl_tid_ampdu_rx *r)
{
	...
	_os_kmem_free(drv_priv, r->reorder_sn,
			buf_size * sizeof(u16));
...
void rtw_phl_free_rx_reorder(void *phl, struct rtw_phl_stainfo_t *sta)
{
	...
	for (i = 0; i < ARRAY_SIZE(sta->tid_rx); i++) {
		...
		_os_spinlock(drv, &phl_info->phl_com->rxsc_entry.rxsc_lock, _bh, NULL);
		if (phl_info->phl_com->rxsc_entry.cached_r == sta->tid_rx[i])
		{
			/*printk("IMPORTANT set rxsc_entry(0x%x).cached_r(0x%x) to NULL before free\n", (unsigned int)&phl_info->phl_com->rxsc_entry,
				(unsigned int)phl_info->phl_com->rxsc_entry.cached_r);*/
				
			phl_info->phl_com->rxsc_entry.cached_r = NULL;
		}
		_os_spinunlock(drv, &phl_info->phl_com->rxsc_entry.rxsc_lock, _bh, NULL);
#endif
...
#ifdef PHL_RXSC_AMPDU
	_os_spinlock(drv_priv, &phl_info->phl_com->rxsc_entry.rxsc_lock, _bh, NULL);
	if (phl_info->phl_com->rxsc_entry.cached_r == r)
		phl_info->phl_com->rxsc_entry.cached_r = NULL;
	_os_spinunlock(drv_priv, &phl_info->phl_com->rxsc_entry.rxsc_lock, _bh, NULL);
#endif
                                                ...
                                                u16 buf_size)
{
	...
	r->reorder_sn = 
		_os_kmem_alloc(drv_priv, buf_size * sizeof(u16));
	if (!r->reorder_sn)
		goto fail;
	_os_mem_set(drv_priv, r->reorder_time, 0xff,
					buf_size * sizeof(u16));
	/* init other fields */
	r->sta = sta;
	r->ssn = ssn;
...
#ifdef PHL_RXSC_AMPDU
		_os_spinlock(drv_priv, &phl_info->phl_com->rxsc_entry.rxsc_lock, _bh, NULL);
		if (phl_info->phl_com->rxsc_entry.cached_r == sta->tid_rx[tid])
		{
			/*printk("IMPORTANT set rxsc_entry(0x%x).cached_r(0x%x) to NULL before free\n", (unsigned int)&phl_info->phl_com->rxsc_entry,
				(unsigned int)phl_info->phl_com->rxsc_entry.cached_r);*/
