...
_continue:
	...
	if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
		...
		issue_probersp_g6(padapter, get_sa(pframe), _FALSE);
...
_issue_probersp:
		if (((check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
			...
			/* RTW_INFO("+issue_probersp_g6 during ap mode\n"); */
			issue_probersp_g6(padapter, get_sa(pframe), is_valid_p2p_probereq);
		}
...
unsigned int OnBeacon(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network), ETH_ALEN)) {
		if ((pmlmeinfo->state & WIFI_FW_AUTH_NULL)
			...
			if (pbss) {
				if (collect_bss_info_g6(padapter, precv_frame, pbss) == _SUCCESS) {
		...
		if (   MLME_IS_STATE(pmlmeinfo, WIFI_FW_STATION_STATE)
			...
			if (psta != NULL) {
				...
				if (cmp_pkt_chnl_diff(padapter, pframe, len) != 0) {
					...
					issue_deauth_g6(padapter, (&(pmlmeinfo->network))->MacAddress, WLAN_REASON_DEAUTH_LEAVING);
...
void rtw_parse_vendor_vw_sta(_adapter *padapter, struct sta_info *pstat)
{
	...
	if ((pstat->vendor == HT_IOT_PEER_VERIWAVE || pstat->vendor == HT_IOT_PEER_SPIRENT) &&
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	issue_asocrsp_g6(padapter, status, pstat, WIFI_ASSOCRSP);
	return _SUCCESS;
#endif


	RTW_INFO("%s sa="MAC_FMT"\n", __FUNCTION__, MAC_ARG(get_addr2_ptr(pframe)));
	...
	if (pstat && (pstat->state & WIFI_FW_ASSOC_SUCCESS) && (_STATS_SUCCESSFUL_ == status)) {
		...
		if(pmlmeinfo->is_MBO_4_2_5_3) {
			if (frame_type == WIFI_ASSOCREQ)
				...
				issue_asocrsp_g6(padapter, status, pstat, WIFI_REASSOCRSP);
		}
#endif
#else
		if (frame_type == WIFI_ASSOCREQ)
		...
		rtw_indicate_l2uf( padapter->pnetdev, pstat->phl_sta->mac_addr);
	}
...
#ifdef CONFIG_NATIVEAP_MLME
	issue_deauth_g6(padapter, (void *)get_addr2_ptr(pframe), status);
#endif
...
static void sta_config_EDCA_para(_adapter *padapter)
{
	...
	if (GET_STA_AC_VO_PARA.aifsn) {
		...
		rtw_phl_write32(padapter->dvobj->phl, 0xc30C, vo_edca);
	}
	...
	if (GET_STA_AC_VI_PARA.aifsn) {
		...
		rtw_phl_write32(padapter->dvobj->phl, 0xc308, vi_edca);
	}
	...
	if (GET_STA_AC_BE_PARA.aifsn) {
		...
		rtw_phl_write32(padapter->dvobj->phl, 0xc300, be_edca);
	}
	...
	if (GET_STA_AC_BK_PARA.aifsn) {
		...
		rtw_phl_write32(padapter->dvobj->phl, 0xc304, bk_edca);
	}
}
#endif
							...
							u8 disassoc_imminent, u8 preference_sta)
{
	...
	*pbuf_ptr = sub_len;
	...
	RTW_PUT_LE32(pbuf_ptr, cpu_to_le32(report.bss_info));
	pbuf_ptr += 4;
...
#ifdef CONFIG_DFS_CSA_IE
		if (rfctl->csa_set_ie && rfctl->csa_ch) {
			pframe = rtw_set_ie_ch_switch(pframe, &(pattrib->pktlen), 1, rfctl->csa_ch, rfctl->csa_cntdown);
			pframe = rtw_set_ie_ch_switch_wrapper(pframe, &(pattrib->pktlen), cur_network->Configuration.Bw,
				rtw_get_center_ch(rfctl->csa_ch, cur_network->Configuration.Bw, cur_network->Configuration.Offset));
		}
#endif
...
void issue_probersp_g6(_adapter *padapter, unsigned char *da, u8 is_valid_p2p_probereq)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	unsigned char					*pframe;
...
static void _auth_data_txsts(struct rtw_txfb_t *txfb)
{
	...
	upd_hndsk_pkt_sts(padapter, HNDSK_AUTH_PKT, txfb->txsts);
#endif
...
#ifdef CONFIG_DBG_HNDSK_MGMT
	upd_hndsk_pkt_sts(padapter, HNDSK_ASSOC_RSP_PKT, txfb->txsts);
#endif
...
void issue_asocrsp_g6(_adapter *padapter, unsigned short status, struct sta_info *pstat, int pkt_type)
{
#ifdef CONFIG_AP_MODE
	struct xmit_frame	*pmgntframe;
	struct rtw_ieee80211_hdr	*pwlanhdr;
...
void _issue_assocreq(_adapter *padapter, u8 is_reassoc)
{
	...
	*(fctrl) = 0;
	...
	for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;) {
		...
		case EID_WAPI:
			//printk("\n%s:%d set WAPI IE!!!\n", __FUNCTION__,__LINE__);
			pframe = rtw_set_ie(pframe, EID_WAPI, pIE->Length, pIE->data, &(pattrib->pktlen));
			break;
#endif
		...
		case EID_HTCapability:
			if (padapter->mlmepriv.htpriv.ht_option == _TRUE) {
				if (!(is_ap_in_tkip(padapter))) {
					...
					/* pIE->data is LE, should not transfer to le16 again */
...
void issue_assocreq_g6(_adapter *padapter)
{
...
/*l2uf pkt for roaming*/
int rtw_indicate_l2uf(struct net_device * dev, unsigned char *src)
{
	int i = 0;
	struct sk_buff *skb;
	struct ethhdr *ehdr;
	...
	char brocast_mac[ETH_ALEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
...
#define L2UF_SIZE 60
	...
	{
#ifdef WKARD_98D
		skb =  __dev_alloc_skb(L2UF_SIZE, GFP_ATOMIC);
#else
		skb =  __dev_alloc_skb(L2UF_SIZE, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
#endif
		if(!skb)
		{
			printk("%s alloc llc pkt failed\n", __func__);
			return _FAIL;
		}

		_rtw_memset(skb->data, 0, L2UF_SIZE);
...
//		skb->len =  L2UF_SIZE;
		...
		_rtw_memcpy(ehdr->h_dest, brocast_mac, ETH_ALEN);
		_rtw_memcpy(ehdr->h_source, src, ETH_ALEN);
		ehdr->h_proto = htons(ETH_ALEN);

		_rtw_memcpy(skb->data+14, l2ufHex[i], 6);
...
int issue_deauth_g6(_adapter *padapter, unsigned char *da, unsigned short reason)
{
...
int issue_disassoc_g6(_adapter *padapter, unsigned char *da, unsigned short reason)
{
	RTW_INFO("%s reason(%u) to "MAC_FMT"\n", __func__, reason, MAC_ARG(da));
	...
	if (category == 3) {
		...
		case RTW_WLAN_ACTION_ADDBA_REQ:
			...
			start_seq = rtw_phl_get_hw_seq(padapter->dvobj->phl,
							psta->phl_sta, tid) + 1;/* CONFIG_RTW_TXSC_USE_HW_SEQ */
				#endif /* CONFIG_RTW_LINK_PHL_MASTER */
...
u8 collect_bss_info_g6(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	int	i;
	sint len;
	u8	*p;
...
void start_clnt_join_g6(_adapter *padapter)
{
	unsigned short	caps;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
...
void start_clnt_auth_g6(_adapter *padapter)
{
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
...
void start_clnt_assoc_g6(_adapter *padapter)
{
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
...
void report_survey_event(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (collect_bss_info_g6(padapter, precv_frame, (WLAN_BSSID_EX *)&psurvey_evt->bss) == _FAIL) {
		rtw_mfree((u8 *)pcmd_obj, sizeof(struct cmd_obj));
...
int rtw_sae_preprocess_g6(_adapter *adapter, const u8 *buf, u32 len, u8 tx)
{
#ifdef CONFIG_IOCTL_CFG80211
...
void rtw_assoc_preprocess(_adapter *padapter, const u8 *buf, u32 len, int type)
{
	...
	issue_asocrsp_g6(padapter, le16_to_cpu(mgmt->u.assoc_resp.status_code), pstat, type);
