...
EXPORT_SYMBOL(wlan_root_netdev);
EXPORT_SYMBOL(wlan_vap_netdev);
EXPORT_SYMBOL(wlan_wds_netdev);
EXPORT_SYMBOL(wlan_vxd_netdev);
EXPORT_SYMBOL(wlan1_root_netdev);
EXPORT_SYMBOL(wlan1_vap_netdev);
EXPORT_SYMBOL(wlan1_wds_netdev);
EXPORT_SYMBOL(wlan1_vxd_netdev);	
...
int rtk_rg_wlan_netDevice_set(rtk_rg_mbssidDev_t wlanDevIdx, struct net_device *pDev)
{
    ...
    int (*_hook_func_fwdEngine_wifi_rx)(struct sk_buff *skb) = NULL;
    EXPORT_SYMBOL(_hook_func_fwdEngine_wifi_rx);
#endif
...
void rtk_rg_netif_rx(struct sk_buff *pskb)
{
    ...
    //printk("[%s]\n",pskb->dev->name);
        ...
        #ifdef CONFIG_RTL8198F_WLAN_MODULE
            ret=fwdEngine_wifi_rx(pskb);
        #else
            if (_hook_func_fwdEngine_wifi_rx == NULL)
            	ret = RE8670_RX_CONTINUE;
            else 
                ret = _hook_func_fwdEngine_wifi_rx(pskb);
        #endif
    #else
    ret=fwdEngine_wifi_rx(pskb);
    #endif
