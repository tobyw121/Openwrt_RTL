...
struct recv_frame_hdr {
	...
	u8 *rx_tail;
	u8 *rx_end;
	/* Point to wlan header when RX header conversion enabled */
	u8 *wlan_hdr;
	u8 *mdata;
	...
	void *rx_req;
	/* flag to mark this is a cloned RX frame */
	u8 cloned;
#ifdef CONFIG_RTW_CORE_RXSC
...
extern s32 rtw_core_alloc_recvframe(_adapter *adapter, union recv_frame **prframe);
...
extern void rtw_core_free_recvframe(_adapter *adapter, union recv_frame *prframe);
...
extern void rtw_rx_recycle_enq(struct dvobj_priv *dvobj, u8* recvpkt);
#define rtw_return_phl_rx_pkt(dvobj, recvpkt) rtw_rx_recycle_enq(dvobj, recvpkt)
#else
#define rtw_return_phl_rx_pkt(dvobj, recvpkt) rtw_phl_return_rxbuf(GET_HAL_INFO(dvobj), recvpkt)
#endif
#endif /* RTW_PHL_RX */

#ifdef CONFIG_SMP_NETIF_RX
extern void rtw_netif_rx_enq(_adapter *padapter, struct sk_buff *pskb);
#endif
