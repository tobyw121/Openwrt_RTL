...
#if defined(CONFIG_CTC_FEATURE) || defined(CONFIG_RTW_80211K)
...
void _fill_sta_info(sta_info_2_web *sta_info, struct sta_info *psta)
{
	...
	get_current_rate(psta, current_rate, &rate, tmp_rate);
	sta_info->current_rx_rate = rate;
	
	current_rate = psta->cur_tx_data_rate;
	get_current_rate(psta, current_rate, &rate, tmp_rate);
	sta_info->current_tx_rate = rate;
...
#ifdef RTW_STA_BWC
void g6_decide_limit_tp(_adapter *padapter, int level)
{
	struct sta_priv *pstapriv = &padapter->stapriv;
...
void g6_cancel_limit_tp(_adapter *padapter)
{
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct sta_info *pstat = NULL;
	struct list_head *phead, *plist;
...
int __rtw_ioctl_private(struct net_device *dev, struct iwreq *wrq, int exec){
	...
	switch (subcmd){
		...
		case SIOCGICTCBSSTRANSREQ:
#endif /* CONFIG_IEEE80211V */
#if defined(CONFIG_WFO_VIRT_MODULE) && !defined(PLATFORM_ECOS)
			if (exec==0) {
				/* get dir for wfo_virt_ioctl */
				ret |= (1<<WFO_IOCTL_SET);
			} else
#endif /* CONFIG_WFO_VIRT_MODULE & !PLATFORM_ECOS */
