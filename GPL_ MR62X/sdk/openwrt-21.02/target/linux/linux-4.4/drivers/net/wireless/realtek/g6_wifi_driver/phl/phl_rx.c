...
#if 1 /* debug */
		if (r && ((unsigned int)r->sta & 0x80000000u) && is_match_seq == true) {
#else
...
#if 1 /* debug */
				PHL_ERR("[%s]RXSC: cached_r is NOT NULL! (is_match_seq:%d),cached_r=0x%x,sta=0x%x\n", __func__, is_match_seq, (unsigned int)r, (unsigned int)r->sta);
#else
