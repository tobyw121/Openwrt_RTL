	...
	struct mp_context *mp, struct mp_config_arg *arg)
{
	...
	hal_status = rtw_hal_set_ch_bw(mp->hal,mp->cur_phy, &chdef, false, false, false);
	if(hal_status != RTW_HAL_STATUS_SUCCESS) {
	...
	hal_status = rtw_hal_reset(hal_com, HW_PHY_0, HW_BAND_0, true);
	if(hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s reset fail! (rst = true)\n", __FUNCTION__);
	}

	hal_status = rtw_hal_reset(hal_com, HW_PHY_0, HW_BAND_0, false);
	if(hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s reset fail! (rst = false)\n", __FUNCTION__);
	}
...
enum rtw_hal_status rtw_hal_mp_ic_hw_setting_init(struct mp_context *mp)
{
	...
	if (phl_is_mp_mode(mp->phl_com)) {
		char ic_name[20] = "";

		sprintf(ic_name, "%s%c", mp->phl_com->hal_spec.ic_name, 'e');

		rtw_hal_dl_all_para_file(mp->phl_com, ic_name, mp->hal);
		rtw_hal_mac_enable_bb_rf(mp->hal, 0);
		rtw_hal_mac_enable_bb_rf(mp->hal, 1);
		rtw_hal_init_bb_reg(mp->hal);
		rtw_hal_init_rf_reg(mp->phl_com, mp->hal);

		rtw_hal_dl_all_ext_para_file(mp->phl_com, ic_name, mp->hal);
		rtw_hal_init_bb_reg(mp->hal);
		rtw_hal_init_rf_reg(mp->phl_com, mp->hal);

		mp->phl_com->phy_sw_cap[0].bfreed_para = false;
		mp->phl_com->phy_sw_cap[1].bfreed_para = false;
		rtw_phl_init_free_para_buf(mp->phl_com);
	}
#endif

	rtw_hal_bb_dm_init(mp->hal);
	rtw_hal_rf_dm_init(mp->hal);

	return RTW_HAL_STATUS_SUCCESS;
}

enum rtw_hal_status rtw_hal_mp_bb_rf_dm_init(struct mp_context *mp)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_info_t *hal_info = (struct hal_info_t *)mp->hal;
