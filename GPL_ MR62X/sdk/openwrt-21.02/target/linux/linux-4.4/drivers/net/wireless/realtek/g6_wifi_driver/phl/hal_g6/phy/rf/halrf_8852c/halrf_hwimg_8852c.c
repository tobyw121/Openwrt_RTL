...
halrf_wrf(rf, RF_PATH_B, 0x100ee, RFREGOFFSETMASK, 0x00000);
	...
	if (rf->hal_com->cv == CAV) {
		array_len = sizeof(array_mp_8852c_nctl_reg_a) / sizeof(u32);
		array = (u32 *) &array_mp_8852c_nctl_reg_a;
	} else {
	...
	enum phl_phy_idx phy)
{
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8852c_txpwr_lmt_ru_6g_low) / sizeof(struct halrf_tx_pw_lmt_ru);
