...
static enum rtw_phl_status phl_mp_rx_get_rpl(
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	hal_status = rtw_hal_mp_rx_get_rpl(mp, arg);
...
enum rtw_phl_status phl_mp_rx_filter_rx_start(
	struct mp_context *mp, struct mp_rx_arg *arg){

	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	hal_status = rtw_hal_mp_rx_filter_start(mp, arg);
...
enum rtw_phl_status phl_mp_rx_filter_rx_reset(
	struct mp_context *mp, struct mp_rx_arg *arg){

	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	hal_status = rtw_hal_mp_rx_filter_reset(mp, arg);
...
enum rtw_phl_status mp_rx(struct mp_context *mp, struct mp_rx_arg *arg)
{
	...
	case MP_RX_CMD_GET_RSSI_EX:
		...
		break;
	case MP_RX_CMD_GET_RPL:
		PHL_INFO("%s: CMD = MP_RX_CMD_GET_RPL\n", __FUNCTION__);
		phl_status = phl_mp_rx_get_rpl(mp, arg);
		break;

	case MP_RX_CMD_FILTER_RX_START:
		PHL_INFO("%s: CMD = MP_RX_CMD_FILTER_RX_START\n", __FUNCTION__);
		phl_status = phl_mp_rx_filter_rx_start(mp, arg);
		break;

	case MP_RX_CMD_FILTER_RX_RESET:
		PHL_INFO("%s: CMD = MP_RX_CMD_FILTER_RX_RESET\n", __FUNCTION__);
		phl_status = phl_mp_rx_filter_rx_reset(mp, arg);
		break;
