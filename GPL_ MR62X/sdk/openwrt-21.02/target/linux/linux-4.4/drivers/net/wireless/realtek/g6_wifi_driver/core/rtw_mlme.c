...
void rtw_init_mlme_timer(_adapter *padapter)
{
	...
	rtw_init_timer(&(pmlmepriv->mbopriv.bss_termination_timer), rtw_bss_termination_timeout_handler, padapter);
	rtw_init_timer(&(pmlmepriv->mbopriv.bss_termination_dur_timer), rtw_bss_termination_dur_timeout_handler, padapter);
#endif
}

static void set_scanned_network_val(struct mlme_priv *pmlmepriv, sint val)
{
	_adapter *adapter = mlme_to_adapter(pmlmepriv);
	_rtw_spinlock_bh(&pmlmepriv->lock);
#ifdef RTW_MI_SHARE_BSS_LIST
	adapter->dvobj->num_of_scanned = val;
#else
...
sint	_rtw_init_mlme_priv(_adapter *padapter)
{
	...
	{
		_rtw_init_queue(&(d->free_bss_pool));
		_rtw_init_queue(&(d->scanned_queue));
		d->pscanned = NULL;
		set_scanned_network_val(pmlmepriv, 0);

		if (padapter->registrypriv.max_bss_cnt != 0)
			d->max_bss_cnt = padapter->registrypriv.max_bss_cnt;
		else if (rfctl->max_chan_nums <= MAX_CHANNEL_NUM_2G)
			d->max_bss_cnt = MAX_BSS_CNT;
		else
			d->max_bss_cnt = MAX_BSS_CNT + MAX_BSS_CNT;

		pbuf = rtw_zvmalloc(d->max_bss_cnt * (sizeof(struct wlan_network)));
		if (pbuf == NULL) {
			res = _FAIL;
			goto exit;
		}
		...
		/*RTW_PRINT("[BSS] %s alloc bss_buf:%ld max_bss_cnt:%ld\n",
		padapter->pnetdev->name,
		d->max_bss_cnt * (sizeof(struct wlan_network)),
		d->max_bss_cnt);*/

		pnetwork = (struct wlan_network *)pbuf;
		for (i = 0; i < d->max_bss_cnt; i++) {
			_rtw_init_listhead(&(pnetwork->list));
			rtw_list_insert_tail(&(pnetwork->list), &(d->free_bss_pool.queue));
			pnetwork++;
		}
	}
#else
	...
	/*RTW_PRINT("[BSS] %s alloc bss_buf:%ld max_bss_cnt:%ld\n",
		padapter->pnetdev->name,
		pmlmepriv->max_bss_cnt * (sizeof(struct wlan_network)),
		pmlmepriv->max_bss_cnt);*/
...
#define RTW_ROAM_RSSI_THRESHOLD 70
	...
	/* 10 STAs connecting in 5 ms */
	pmlmepriv->defs_lmt_sta = 10;
...
#ifdef RTW_MI_SHARE_BSS_LIST
	_adapter *adapter = mlme_to_adapter(pmlmepriv);
	struct dvobj_priv *d = adapter->dvobj;
#endif

	_rtw_spinlock_free(&pmlmepriv->lock);
#ifdef RTW_MI_SHARE_BSS_LIST
	if (is_primary_adapter(adapter)) {
		_rtw_spinlock_free(&(d->free_bss_pool.lock));
		_rtw_spinlock_free(&(d->scanned_queue.lock));
	}
#else
...
void _rtw_free_mlme_priv(struct mlme_priv *pmlmepriv)
{
...
#ifdef RTW_MI_SHARE_BSS_LIST
	struct dvobj_priv *d = adapter->dvobj;
	u8 *pfree_bss_buf = d->free_bss_buf;
	u32 max_bss_cnt = d->max_bss_cnt;
#else
	u8 *pfree_bss_buf = pmlmepriv->free_bss_buf;
	u32 max_bss_cnt = pmlmepriv->max_bss_cnt;
#endif
	if (NULL == pmlmepriv) {
		rtw_warn_on(1);
		goto exit;
	...
	if (pmlmepriv) {
		...
		if (pfree_bss_buf) {
			rtw_vmfree(pfree_bss_buf, max_bss_cnt * sizeof(struct wlan_network));
#ifdef RTW_MI_SHARE_BSS_LIST
			d->free_bss_buf = NULL;
#endif
		}
...
#ifdef RTW_MI_SHARE_BSS_LIST
	_adapter *adapter = mlme_to_adapter(pmlmepriv);
	_queue *free_queue = &adapter->dvobj->free_bss_pool;
#else
...
void _rtw_free_network_queue(_adapter *padapter, u8 isfreeall)
{
	...
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
#ifdef RTW_MI_SHARE_BSS_LIST
	_queue *scanned_queue = &padapter->dvobj->scanned_queue;
#else
...
#if defined(CONFIG_IOCTL_CFG80211) && !defined(RTW_WRARD_FREE_BSS_IN_CFG80211_WQ)
	/*single core cpu do this will causes cfg80211 warning call trace (wqcap3440)*/
...
bool rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target)
{
...
#ifdef RTW_MI_SHARE_BSS_LIST
	_queue *pfree_bss_pool = &adapter->dvobj->free_bss_pool;
	_queue *queue = &adapter->dvobj->scanned_queue;
#else
	_queue *pfree_bss_pool = &(pmlmepriv->free_bss_pool);
	...
	/* if (rtw_end_of_queue_search(phead,plist)== _TRUE) { */
	if (!target_find) {
		if (_rtw_queue_empty(pfree_bss_pool) == _TRUE) {
...
#ifdef CONFIG_ANTENNA_DIVERSITY
			rtw_hal_get_phydm_var(adapter, HAL_PHYDM_ANTDIV_SELECT, &(target->PhyInfo.Optimum_antenna), NULL);
#endif
		...
		if(update_ie == _TRUE)
		{
			if((pnetwork->network.dtim_prd != target->dtim_prd) && target->dtim_prd == 0)
			{
				target->dtim_prd = pnetwork->network.dtim_prd;
			}
		}
		else
		{
			if((pnetwork->network.dtim_prd != target->dtim_prd) && target->dtim_prd != 0)
			{
				pnetwork->network.dtim_prd = target->dtim_prd;
			}
		}
...
void rtw_survey_event_callback(_adapter	*adapter, u8 *pbuf)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)) == _TRUE) {
		if (_rtw_memcmp(&(pmlmepriv->cur_network.network.MacAddress), pnetwork->MacAddress, ETH_ALEN)) {
			...
			_rtw_spinlock_bh(&(queue->lock));
			ibss_wlan = _rtw_find_network(queue,  pnetwork->MacAddress);
			if (ibss_wlan) {
...
int _rtw_compare_signal_strength(void *priv, const struct list_head *a, const struct list_head *b)
#else
int _rtw_compare_signal_strength(void *priv, struct list_head *a, struct list_head *b)
#endif
{
	struct wlan_network	*pnetwork1 = LIST_CONTAINOR(a, struct wlan_network, list);
	struct wlan_network	*pnetwork2 = LIST_CONTAINOR(b, struct wlan_network, list);

	if (pnetwork1->network.PhyInfo.SignalStrength > pnetwork2->network.PhyInfo.SignalStrength)
		return -1;
	if (pnetwork1->network.PhyInfo.SignalStrength < pnetwork2->network.PhyInfo.SignalStrength)
		return 1;
	return 0;
...
void rtw_surveydone_event_callback(_adapter	*adapter, u8 *pbuf)
{
...
#ifdef CONFIG_RSSI_PRIORITY
	if(_rtw_queue_empty(queue) == _FALSE){
		_rtw_spinlock_bh(&queue->lock);
		phead = get_list_head(queue);
		RTW_INFO("%s: Sort the site survey list according to signal strength", __FUNCTION__);
		list_sort(NULL, phead, _rtw_compare_signal_strength);
		_rtw_spinunlock_bh(&queue->lock);
	}
#endif
	...
	rtw_opclass_obtain_site_survey_result(adapter);/*sync result*/
#ifdef CONFIG_RTW_MULTI_AP_R2
	core_map_update_channel_scan_result(adapter);/*send result*/
#endif
#endif
	...
	if (rtw_mesh_offch_candidate_accepted(adapter)) {
		...
		if (ch && pmlmepriv->cur_network.network.Configuration.DSConfig != ch) {
			u16 ifbmp = rtw_mi_get_ap_mesh_ifbmp(adapter);
	...
	if (MLME_IS_AP(adapter))
		rtw_led_control(adapter, LED_CTL_LINK);

#ifdef CONFIG_RTW_ACS
#ifdef CONFIG_RTW_DACS
	if (adapter->registrypriv.wifi_mib.dacs) {
		rtw_dacs_mnt_result(adapter);
	} else
#endif
	if (parm->acs) {
		rtw_select_clean_channel(adapter);
	}
#endif
}
...
static void free_scanqueue(struct	mlme_priv *pmlmepriv)
{
	...
	_adapter *adapter = mlme_to_adapter(pmlmepriv);
	_queue *free_queue = &adapter->dvobj->free_bss_pool;
	_queue *scan_queue = &adapter->dvobj->scanned_queue;
#else
	_queue *free_queue = &pmlmepriv->free_bss_pool;
...
void rtw_free_assoc_resources(_adapter *adapter, u8 lock_scanned_queue)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS) || (pmlmepriv->wpa_phase == _TRUE)){
		...
		pwlan = _rtw_find_same_network(queue, tgt_network);
		if (pwlan) {
			pwlan->fixed = _FALSE;
...
void rtw_indicate_connect(_adapter *padapter)
{
	...
	if(MLME_IS_AP(padapter) && padapter->a4_enable)
		core_a4_gptr_flush(padapter);
	#endif
}
...
void rtw_indicate_disconnect(_adapter *padapter, u16 reason, u8 locally_generated)
{
	...
	if (MLME_IS_STA(adapter))
		rtw_wapi_return_one_sta_info(padapter, psta->phl_sta->mac_addr);
...
#ifdef CONFIG_LIFETIME_FEATURE
       if (GET_PRIMARY_ADAPTER(padapter)->registrypriv.wifi_mib.lifetime) {
               us256 = RTL_MS_TO_256US(GET_PRIMARY_ADAPTER(padapter)->registrypriv.wifi_mib.lifetime);
...
static u32 _rtw_wait_scan_done(_adapter *adapter, u32 timeout_ms)
{
	...
	if (_TRUE == abort_timeout && check_fwstate(pmlmepriv, WIFI_UNDER_SURVEY)) {
		...
		_clr_fwstate_(pmlmepriv, WIFI_UNDER_SURVEY);
		_rtw_spinunlock_bh(&pmlmepriv->lock);
		RTW_PRINT(FUNC_NDEV_FMT"under survey state cleared!\n", FUNC_NDEV_ARG(adapter->pnetdev));
		#endif
...
void rtw_scan_wait_completed(_adapter *adapter)
{
	...
	_rtw_wait_scan_done(adapter, ss->scan_timeout_ms);
}

u32 rtw_scan_abort_timeout(_adapter *adapter, u32 timeout_ms)
{
	return _rtw_wait_scan_done(adapter, timeout_ms);
}
...
u32 rtw_scan_abort(_adapter *adapter, u32 timeout_ms)
{
	...
	if (   check_fwstate(pmlmepriv, WIFI_UNDER_SURVEY)
	    #ifdef CONFIG_P2P
	    || rtw_cfg80211_get_is_roch(adapter)
	    #endif
	   ) {
...
static struct sta_info *rtw_joinbss_update_stainfo(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	if (psta == NULL) {
		...
		psta = rtw_alloc_stainfo(pstapriv, pnetwork->network.MacAddress, PHL_CMD_DIRECTLY);
	}
	...
	if (psta) { /* update ptarget_sta */
...
#endif
		{
		...
		else if (padapter->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_WAPI) {
			u8 *ie = NULL;
			int ie_len = 0;
			u8 mfp_opt = MFP_NO;
			...
			ie = rtw_get_ie(pnetwork->network.IEs + _BEACON_IE_OFFSET_, _WAPI_IE_,
					&ie_len, (pnetwork->network.IELength - _BEACON_IE_OFFSET_));
			if (ie && ie_len > 0) {
				if (rtw_parse_wapi_ie(ie, ie_len + 2, NULL, NULL, NULL, NULL, &mfp_opt) == _SUCCESS) {
					if (padapter->securitypriv.mfp_opt >= MFP_OPTIONAL && mfp_opt >= MFP_OPTIONAL)
						psta->flags |= WLAN_STA_MFP;
				}
			}
		...
		if (check_fwstate(pmlmepriv, WIFI_UNDER_LINKING)) {
			...
			if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE)) {
				if (the_same_macaddr == _TRUE)
					ptarget_wlan = _rtw_find_network(queue, cur_network->network.MacAddress);
				else {
					pcur_wlan = _rtw_find_network(queue, cur_network->network.MacAddress);
					if (pcur_wlan)
					...
					ptarget_wlan = _rtw_find_network(queue, pnetwork->network.MacAddress);
					if (MLME_IS_STA(adapter)) {
						if (ptarget_wlan)
				...
				ptarget_wlan = _rtw_find_same_network(queue, pnetwork);
				if (MLME_IS_STA(adapter)) {
					if (ptarget_wlan)
						ptarget_wlan->fixed = _TRUE;
				}
...
void rtw_stassoc_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	if (MLME_IS_AP(adapter) || MLME_IS_MESH(adapter)) {
		...
		if (psta) {
			...
			if (!psta->phl_sta->ss_ul_active) {
				phl_status = rtw_phl_upd_ss_ul_sta(adapter->dvobj->phl, psta->phl_sta, 1);
				if (phl_status == RTW_PHL_STATUS_SUCCESS)
					psta->phl_sta->ss_ul_active = 1;
				else
					RTW_ERR("upd_ss_ul_sta(ADD) was failed\n");
			}

#if defined(CONFIG_VW_REFINE)
		if ( 2 == adapter->vw_enable ) {
			extern void vw_latency_txforce_set(_adapter *adapter, struct sta_info *psta);
			vw_latency_txforce_set(adapter, psta);
		}
#endif

#ifdef CONFIG_MCC_MODE
			rtw_hal_mcc_update_macid_bitmap(adapter, psta->phl_sta->macid, _TRUE);
			...
			txsc_amsdu_sta_init(adapter, psta);
#endif
...
void rtw_ft_update_stainfo(_adapter *padapter, WLAN_BSSID_EX *pnetwork)
{
	...
	if (psta == NULL)
		psta = rtw_alloc_stainfo(pstapriv, pnetwork->MacAddress, PHL_CMD_DIRECTLY);
	...
	_adapter *padapter, struct sitesurvey_parm *pparm)
{
	...
	pparm->ch_num = MIN(RTW_MAX_NB_RPT_NUM, RTW_CHANNEL_SCAN_AMOUNT);
...
void rtw_sta_mstatus_report(_adapter *adapter)
{
	...
	if (MLME_IS_STA(adapter) && check_fwstate(pmlmepriv, WIFI_ASOC_STATE)) {
		psta = rtw_get_stainfo(&adapter->stapriv, tgt_network->network.MacAddress);
		if (psta)
			rtw_sta_mstatus_disc_rpt(adapter, psta->phl_sta->macid);
		else {
			RTW_WARN(FUNC_ADPT_FMT ": fail to find stainfo"
				 "(" MAC_FMT ")\n",
				 FUNC_ADPT_ARG(adapter),
				 MAC_ARG(tgt_network->network.MacAddress));
			/*rtw_warn_on(1);*/
...
void rtw_stadel_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	if (!MLME_IS_MESH(adapter))
		rtw_core_twt_teardown_sta_all(adapter, psta);
#endif

	if (psta) {
		rtw_wfd_st_switch(psta, 0);
		psta->hw_decrypted = _FALSE;
#ifdef RTW_WKARD_UPDATE_DMAC_KEYID
		psta->dmac_updated = _FALSE;
#endif
	}
	...
	if (MLME_IS_AP(adapter)) {
		...
		if (psta && psta->phl_sta && psta->phl_sta->ss_ul_active) {
			phl_status = rtw_phl_upd_ss_ul_sta(adapter->dvobj->phl, psta->phl_sta, 0);
			if (phl_status == RTW_PHL_STATUS_SUCCESS)
				psta->phl_sta->ss_ul_active = 0;
			else
				RTW_ERR("upd_ss_ul_sta(DELETE) was failed\n");
		}
	...
	if (MLME_IS_ADHOC_MASTER(adapter) ||
		...
		if (adapter->stapriv.asoc_sta_count == 1) { /* a sta + bc/mc_stainfo (not Ibss_stainfo) */
			...
			pwlan = _rtw_find_network(queue, tgt_network->network.MacAddress);
			if (pwlan) {
				pwlan->fixed = _FALSE;
				rtw_free_network_nolock(adapter, pwlan);
			}
...
void rtw_iface_dynamic_check_handlder(struct _ADAPTER *a)
{
	...
	if (rcu_dereference(a->pnetdev->rx_handler_data)
		...
		/* expire NAT2.5 entry */
		void nat25_db_expire_g6(_adapter *priv);
		nat25_db_expire_g6(a);
...
#ifdef CONFIG_RTW_A4_STA
	if(MLME_IS_AP(adapter) && adapter->a4_enable)
	core_a4_gptr_expire(adapter);
#endif
...
static void collect_traffic_statistics(_adapter *padapter)
{
	...
	for (i = 0; i < 4; i++) {
		pdvobjpriv->tx_wmm_pkts_prev[i] = pdvobjpriv->tx_wmm_pkts[i];
		pdvobjpriv->tx_wmm_pkts[i] = 0;
	}
...
static void collect_hw_statistics(_adapter *padapter)
{
	u32 fa_total_cnt = 0;
	enum rtw_phl_status phl_stats;
	struct dvobj_priv *pdvobjpriv = adapter_to_dvobj(padapter);
...
#if 1
	pdvobjpriv->hw_stat.cur_fa_cnt = hal_com->band[0].stat_info.cnt_fail_all >> 1;
	pdvobjpriv->hw_stat.cur_cca_cnt = hal_com->band[0].stat_info.cnt_cca_all >> 1;
#else
	pdvobjpriv->hw_stat.last_fa_cnt = pdvobjpriv->hw_stat.fa_cnt;
	phl_stats = rtw_phl_get_rx_fa(padapter->dvobj->phl, padapter->phl_role->hw_band, &pdvobjpriv->hw_stat.fa_cnt);
	if (RTW_PHL_STATUS_SUCCESS != phl_stats)
		pdvobjpriv->hw_stat.fa_cnt = pdvobjpriv->hw_stat.last_fa_cnt;

	if (pdvobjpriv->hw_stat.fa_cnt >= pdvobjpriv->hw_stat.last_fa_cnt)
		pdvobjpriv->hw_stat.cur_fa_cnt = pdvobjpriv->hw_stat.fa_cnt - pdvobjpriv->hw_stat.last_fa_cnt;
	else
		pdvobjpriv->hw_stat.cur_fa_cnt = pdvobjpriv->hw_stat.fa_cnt;
	pdvobjpriv->hw_stat.cur_fa_cnt = pdvobjpriv->hw_stat.cur_fa_cnt >> 1;
#endif
}
...
#if defined(CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME)
void rtw_beacon_vsie_timeout_expire(_adapter *padapter)
{
	int vendor_ie_num=0;

	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);

	if (MLME_IS_AP(padapter) == 0 && MLME_IS_MESH(padapter) == 0)
		return;
	for (vendor_ie_num = 0 ; vendor_ie_num < WLAN_MAX_VENDOR_IE_NUM ; vendor_ie_num++)
	{
		if (pmlmepriv->vendor_service_name_len[vendor_ie_num] && pmlmepriv->vendor_ielen[vendor_ie_num] > 0 && pmlmepriv->vendor_ie_mask[vendor_ie_num] & WIFI_BEACON_VENDOR_IE_BIT)
		{
			if(pmlmepriv->vendor_ie_duration[vendor_ie_num])
			{
				if(pmlmepriv->vendor_ie_duration_count[vendor_ie_num] >= pmlmepriv->vendor_ie_duration[vendor_ie_num])
				{
					...
					_rtw_memset(pmlmepriv->vendor_mac[vendor_ie_num] , 0 , sizeof(pmlmepriv->vendor_mac[vendor_ie_num]));
					pmlmepriv->vendor_mac_is_set[vendor_ie_num] = 0;
					pmlmepriv->vendor_ie_duration[vendor_ie_num] = 0;
					pmlmepriv->vendor_ie_duration_count[vendor_ie_num] = 0;
					_rtw_memset(pmlmepriv->vendor_service_name[vendor_ie_num] , 0 , sizeof(pmlmepriv->vendor_service_name[vendor_ie_num]));
					pmlmepriv->vendor_service_name_len[vendor_ie_num] = 0;
					_update_beacon(padapter, 0xFFF, NULL, _TRUE, 0, "update vendor ie");
				}
				else
				{
					pmlmepriv->vendor_ie_duration_count[vendor_ie_num]++;
				}
			}
		}
	}
...
/* only SW features can be placed in this function,
   HW related features should prevent direct access */
void rtw_iface_sw_feature_timer_handlder(_adapter *adapter)
{
	if (!adapter->netif_up)
		return;
...
#ifdef CONFIG_APPEND_VENDOR_IE_ENABLE
#if defined(CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME)
		rtw_beacon_vsie_timeout_expire(adapter);
#endif
#endif
#endif

#if CONFIG_RTW_MACADDR_ACL
	rtw_acl_expire_sta(adapter);
#endif /* CONFIG_RTW_MACADDR_ACL */
}

void rtw_sw_feature_timer_handlder(void *ctx)
{
	struct dvobj_priv *pdvobj = (struct dvobj_priv *)ctx;
	_adapter *adapter = dvobj_get_primary_adapter(pdvobj);
	...
	rtw_mi_sw_feature_timer_handlder(adapter);

exit:
	_set_timer(&pdvobj->core_feature_sw_timer, 1000);
}
...
void rtw_core_watchdog_sw_hdlr(void *drv_priv)
{
	...
	rtw_phl_check_ru_group(adapter);
#endif

#ifdef CONFIG_PHL_TX_STATS_CORE
	rtw_phl_reset_device_tx_statistics(adapter->dvobj->phl);
#endif

#ifdef RTW_BLOCK_STA_CONNECT
	block_sta_conn_expire(adapter);
#endif
	collect_traffic_statistics(adapter);
	collect_hw_statistics(adapter);
...
int rtw_select_roaming_candidate(struct mlme_priv *mlme)
{
	...
	_adapter *adapter;
#ifdef RTW_MI_SHARE_BSS_LIST
	_adapter *adp = mlme_to_adapter(mlme);
	_queue *queue = &adp->dvobj->scanned_queue;
	_list *pscanned = adp->dvobj->pscanned;
#else
	_queue	*queue	= &(mlme->scanned_queue);
	_list *pscanned = mlme->pscanned;
#endif
	struct	wlan_network	*pnetwork = NULL;
	...
	while (!rtw_end_of_queue_search(phead, pscanned)) {
...
int rtw_select_and_join_from_scanned_queue(struct mlme_priv *pmlmepriv)
{
	...
	_adapter *adapter;
#ifdef RTW_MI_SHARE_BSS_LIST
	_adapter *adp = mlme_to_adapter(pmlmepriv);
	_queue *queue = &adp->dvobj->scanned_queue;
	_list *pscanned = adp->dvobj->pscanned;
#else
	_queue	*queue	= &(pmlmepriv->scanned_queue);
...
candidate_exist:
	...
	if (_TRUE == bSupportAntDiv) {
		u8 CurrentAntenna;
		rtw_hal_get_phydm_var(adapter, HAL_PHYDM_ANTDIV_SELECT, &(CurrentAntenna), NULL);
...
int rtw_rsn_sync_pmkid(_adapter *adapter, u8 *ie, uint ie_len, int i_ent)
{
	...
	if (i_ent >= 0 && info.pmkid_cnt == 1 && (info.pmkid_list != NULL) && _rtw_memcmp(info.pmkid_list, sec->PMKIDList[i_ent].PMKID, 16)) {
	...
	if (info.pmkid_list && info.pmkid_cnt) {
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	if (ndisauthmode == Ndis802_11AuthModeWAPI)
		authmode = _WAPI_IE_;
#endif
				...
				struct role_cap_t *dft_cap)
{
	...
	if (pregistrypriv->wifi_spec || pregistrypriv->ht_20_40_coexist)
	...
	bHwSTBCSupport = dft_proto_cap->stbc_ht_tx ? _TRUE : _FALSE;
	if (bHwSTBCSupport) {
	...
	{
		bHwSupportBeamformer = (dft_proto_cap->su_bfmr) ? _TRUE : _FALSE;
		bHwSupportBeamformee = (dft_proto_cap->su_bfme) ? _TRUE : _FALSE
...
void	rtw_ht_apply_mib_setting(_adapter *padapter)
{
	struct mlme_priv		*pmlmepriv = &padapter->mlmepriv;
	struct ht_priv		*phtpriv = &pmlmepriv->htpriv;
	struct registry_priv	*pregistrypriv = &padapter->registrypriv;
	...
	/* LDPC */
	if (!pregistrypriv->wifi_mib.ldpc)
		CLEAR_FLAGS(phtpriv->ldpc_cap);

	/* STBC */
	if (!(pregistrypriv->wifi_mib.stbc & STBC_RX_EN))
		CLEAR_FLAG(phtpriv->stbc_cap, STBC_HT_ENABLE_RX);
	if (!(pregistrypriv->wifi_mib.stbc & STBC_TX_EN))
...
void	rtw_ht_use_default_setting(_adapter *padapter)
{
	...
	if(padapter->phl_role==NULL) {
		RTW_ERR("[%s %d] padapter->phl_role is null pointer\n", __FUNCTION__, __LINE__);
		return;
	}
...
{
	u32 ielen, out_len;
	u32 rx_packet_offset = 0, max_recvbuf_sz = 0;
...
#ifdef PLATFORM_ECOS
	out_len = get_unaligned(pout_len);
#else
	out_len = *pout_len;
#endif
	...
	if (cbw40_enable) {
		...
		if (oper_bw == CHANNEL_WIDTH_40
			...
			if (IS_DFS_SLAVE_WITH_RD(rfctl) && !rtw_rfctl_dfs_domain_unknown_g6(rfctl))
	...
	if (TEST_FLAG(phtpriv->stbc_cap, STBC_HT_ENABLE_RX)) {
		if ((pregistrypriv->rx_stbc == 0x3) ||							/* enable for 2.4/5 GHz */
			...
			switch(rx_stbc_nss){
				case 1:
					ht_capie.cap_info |= IEEE80211_HT_CAP_RX_STBC_1R;
					break;
				case 2:
					ht_capie.cap_info |= IEEE80211_HT_CAP_RX_STBC_2R;
					break;
				case 3:
					ht_capie.cap_info |= IEEE80211_HT_CAP_RX_STBC_3R;
					break;
				default:
					RTW_INFO("[HT] unknow RX STBC[%d]\n", rx_stbc_nss);
					break;
			}
	...
	*/
	...
	ht_capie.cap_info = cpu_to_le16(ht_capie.cap_info);
...
#ifdef PLATFORM_ECOS
		pframe = rtw_set_ie(out_ie + get_unaligned(pout_len), EID_EXTCapability, 8, cap_content , pout_len);
#else
