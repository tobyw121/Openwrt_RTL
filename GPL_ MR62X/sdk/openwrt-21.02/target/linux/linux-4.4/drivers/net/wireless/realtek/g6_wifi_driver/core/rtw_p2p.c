...
static u32 go_add_group_info_attr(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		if (psta->is_p2p_device) {
			...
			_rtw_memcpy(pcur, psta->phl_sta->mac_addr, ETH_ALEN);
			pcur += ETH_ALEN;
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO, NULL, (uint *)&attr_contentlen)) {
			...
			if (pattr_content) {
				...
				if (num_of_secdev_type == 0)
					...
					len2 = (sizeof(psta->secdev_types_list) < (num_of_secdev_type * 8)) ? (sizeof(psta->secdev_types_list)) : (num_of_secdev_type * 8);

					_rtw_memcpy(psta->secdev_types_list, pattr_content, len2);
				...
				if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_ID, dev_addr, &attr_contentlen)) {
					...
					while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
						...
						if (psta->is_p2p_device && (psta->dev_cap & P2P_DEVCAP_CLIENT_DISCOVERABILITY) &&
							...
							issue_group_disc_req(pwdinfo, psta->phl_sta->mac_addr);
							/* _rtw_spinlock_bh(&pstapriv->asoc_list_lock); */
	...
	if (rtw_mi_check_status(padapter, MI_LINKED)) {
		...
		if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ) || rtw_p2p_chk_state(pwdinfo, P2P_STATE_RX_PROVISION_DIS_RSP)) {
			set_channel_bwmode(padapter,
				union_ch, union_offset, union_bw, _FALSE);
...
void pre_tx_invitereq_handler(_adapter	*padapter)
{
	...
	set_channel_bwmode(padapter,
			pwdinfo->invitereq_info.peer_ch,
			CHAN_OFFSET_NO_EXT,
			CHANNEL_WIDTH_20,
			_FALSE);
			...
			if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_IDLE)) {
				...
				if (pwdinfo->ext_listen_period > 0) {
					...
					if (!rtw_mi_check_mlmeinfo_state(padapter, WIFI_FW_AP_STATE)
					    ...
					    && !rtw_mi_check_mlmeinfo_state(padapter, WIFI_FW_VAP_STATE)
#endif
					) {
						val8 = 1;
						rtw_hal_set_hwreg(padapter, HW_VAR_MLME_SITESURVEY, (u8 *)(&val8));
				...
				/*
				val8 = 1;
				set_channel_bwmode(padapter, , CHAN_OFFSET_NO_EXT, CHANNEL_WIDTH_20);
				rtw_hal_set_hwreg(padapter, HW_VAR_MLME_SITESURVEY, (u8 *)(&val8));
				issue_probereq_p2p(padapter, NULL);
				_set_timer( &pwdinfo->pre_tx_scan_timer, P2P_TX_PRESCAN_TIMEOUT );
				*/
...
void rtw_init_cfg80211_wifidirect_info(_adapter	*padapter)
{
	...
	rtw_init_timer(&pcfg80211_wdinfo->remain_on_ch_timer, ro_ch_timer_process, padapter);
}
...
#ifdef CONFIG_CONCURRENT_MODE
	rtw_init_timer(&pwdinfo->ap_p2p_switch_timer, ap_p2p_switch_timer_process, padapter);
#endif
}
