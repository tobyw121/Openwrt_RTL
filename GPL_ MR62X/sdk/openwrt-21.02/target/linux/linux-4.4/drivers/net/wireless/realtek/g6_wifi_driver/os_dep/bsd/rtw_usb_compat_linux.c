...
static int      rtw_usb_unlink_urb_sub(struct urb *, uint8_t);
...
DRIVER_MODULE(rtw_usb_linux, uhub, usb_linux_driver, usb_linux_devclass, NULL, 0);
...
usb_linux_probe(device_t dev)
{
	...
	if (uaa->usb_speed != USB_MODE_HOST) {
		return (ENXIO);
	}
	mtx_lock(&Giant);
...
usb_linux_attach(device_t dev)
{
...
#if 1 // kenny
        if ( usb_linux_init_endpoints(uaa->device) ) {
                return (ENXIO);
        }
#endif
	...
	sc->sc_fbsd_dev = dev;
	sc->sc_udrv = udrv;
        sc->sc_ui = rtw_usb_ifnum_to_if(uaa->device, uaa->info.bIfaceNum);
	if (sc->sc_ui == NULL) {
		return (EINVAL);
	}
	if (udrv->probe) {
 ...
 *------------------------------------------------------------------------*/
int
rtw_usb_submit_urb(struct urb *urb, uint16_t mem_flags)
{
 ...
 *------------------------------------------------------------------------*/
int
rtw_usb_unlink_urb(struct urb *urb)
{
        return (rtw_usb_unlink_urb_sub(urb, 0));
}
...
static int
rtw_usb_unlink_urb_sub(struct urb *urb, uint8_t drain)
{
	struct usb_host_endpoint *uhe;
	uint16_t x;
	uint8_t do_unlock;
 ...
 *------------------------------------------------------------------------*/
int
rtw_usb_clear_halt(struct usb_device *dev, struct usb_host_endpoint *uhe)
{
	...
	usbd_clear_data_toggle(dev, ep);
        ...
        return (rtw_usb_control_msg(dev, &dev->ep0,
 ...
 *------------------------------------------------------------------------*/
int
rtw_usb_control_msg(struct usb_device *dev, struct usb_host_endpoint *uhe,
    ...
    uint16_t size, usb_timeout_t timeout)
{
	...
	if (dev->flags.usb_speed != USB_MODE_HOST) {
		/* not supported */
		return (-EINVAL);
	}
        err = rtw_usb_setup_endpoint(dev, uhe, 1 /* dummy */ );
	 ...
	 */
        urb = rtw_usb_alloc_urb(0xFFFF, size);
	if (urb == NULL)
		return (-ENOMEM);
 ...
 *------------------------------------------------------------------------*/
int
rtw_usb_set_interface(struct usb_device *dev, uint8_t iface_no, uint8_t alt_index)
{
        struct usb_interface *p_ui = rtw_usb_ifnum_to_if(dev, iface_no);
	int err;
 ...
 *------------------------------------------------------------------------*/
int
rtw_usb_setup_endpoint(struct usb_device *dev,
...
printf("%s() - usbd_transfer_setup()...\n", __FUNCTION__);
		if (usbd_transfer_setup(dev, &uhe->bsd_iface_index,
		    uhe->bsd_xfer, cfg, 1, uhe, &Giant)) {
			return (-EINVAL);
		}
printf("%s() - usbd_transfer_setup() done.\n", __FUNCTION__);
	}
	return (0);
}
...
#if 1 // debug
					printf("%s() - p_uhe->desc.bDescriptorType=%d\n", __FUNCTION__, p_uhe->desc.bDescriptorType);
					printf("%s() - p_uhe->desc.bEndpointAddress=%x\n", __FUNCTION__, p_uhe->desc.bEndpointAddress);
					printf("%s() - p_uhe->bsd_iface_index=%x\n", __FUNCTION__, p_uhe->bsd_iface_index);
#endif // debug
					p_uhe++;
				}
				if (p_uhi) {
 ...
 *------------------------------------------------------------------------*/
struct urb *
rtw_usb_alloc_urb(uint16_t iso_packets, uint16_t mem_flags)
{
	struct urb *urb;
	usb_size_t size;
	...
	urb = malloc(size, M_USBDEV, M_NOWAIT | M_ZERO);
	if (urb) {
...
rtw_usb_find_host_endpoint(struct usb_device *dev, uint8_t type, uint8_t ep)
{
	struct usb_host_endpoint *uhe;
...
rtw_usb_altnum_to_altsetting(const struct usb_interface *intf, uint8_t alt_index)
{
 ...
 *------------------------------------------------------------------------*/
struct usb_interface *
rtw_usb_ifnum_to_if(struct usb_device *dev, uint8_t iface_no)
{
...
/*------------------------------------------------------------------------*
 *      rtw_usbd_get_intfdata
 *------------------------------------------------------------------------*/
void   *
rtw_usbd_get_intfdata(struct usb_interface *intf)
{
 ...
 *------------------------------------------------------------------------*/
void
rtw_usb_linux_register(void *arg)
{
 ...
 *------------------------------------------------------------------------*/
void
rtw_usb_linux_deregister(void *arg)
{
	struct usb_driver *drv = arg;
 ...
 *------------------------------------------------------------------------*/
void
rtw_usb_linux_free_device(struct usb_device *dev)
{
	struct usb_host_endpoint *uhe;
	struct usb_host_endpoint *uhe_end;
	int err;
	...
	while (uhe != uhe_end) {
                err = rtw_usb_setup_endpoint(dev, uhe, 0);
		uhe++;
	}
        err = rtw_usb_setup_endpoint(dev, &dev->ep0, 0);
 ...
 *------------------------------------------------------------------------*/
void
rtw_usb_free_urb(struct urb *urb)
{
	if (urb == NULL) {
		return;
	}
	/* make sure that the current URB is not active */
        rtw_usb_kill_urb(urb);
 ...
 * is not recommended to use this function. Use "rtw_usb_alloc_urb()"
 * instead.
 *------------------------------------------------------------------------*/
void
rtw_usb_init_urb(struct urb *urb)
{
	if (urb == NULL) {
		return;
	}
 ...
 *------------------------------------------------------------------------*/
void
rtw_usb_kill_urb(struct urb *urb)
{
        rtw_usb_unlink_urb_sub(urb, 1);
}

/*------------------------------------------------------------------------*
 *      rtw_usb_set_intfdata
 *
 * The following function sets the per Linux USB interface private
 * data pointer. It is used by most Linux USB device drivers.
 *------------------------------------------------------------------------*/
void
rtw_usb_set_intfdata(struct usb_interface *intf, void *data)
{
...
/*------------------------------------------------------------------------*
 *      rtw_usb_fill_bulk_urb
 *------------------------------------------------------------------------*/
void
rtw_usb_fill_bulk_urb(struct urb *urb, struct usb_device *udev,
 ...
 *------------------------------------------------------------------------*/
int
rtw_usb_bulk_msg(struct usb_device *udev, struct usb_host_endpoint *uhe,
    void *data, int len, uint16_t *pactlen, usb_timeout_t timeout)
{
	...
	if (len < 0)
        ...
        err = rtw_usb_setup_endpoint(udev, uhe, 4096 /* bytes */);
	if (err)
		return (err);

        urb = rtw_usb_alloc_urb(0, 0);
	if (urb == NULL)
		return (-ENOMEM);

        rtw_usb_fill_bulk_urb(urb, udev, uhe, data, len,
...
#if 1 // Add by Kenny for RTW BSD driver
/*------------------------------------------------------------------------*
 *	usb_get_intfdata
 *------------------------------------------------------------------------*/
void   *
...
/*-----------------------------------------------------------------------*
 * usb_linux_init_endpoints
 *
 * Before using any Linux endpoints, they need to be set up using the
 * rtw_usb_setup_endpoint (compatibility layer) function.
 */

int
usb_linux_init_endpoints(struct usb_device *udev)
{
        //struct usb_interface *usb_intf;
        //struct usb_host_interface *usb_host_intf;
        struct usb_host_endpoint *usb_host_ep;
        //unsigned int bulk_buffer_size;
        //unsigned int control_buffer_size;
        //int num_endpoints;
        unsigned int buf_size;
        int err;

        /* Iterate over each usb_interface. */
        for (usb_host_ep = udev->linux_endpoint_start;
             usb_host_ep != udev->linux_endpoint_end;
             usb_host_ep++) {
                
                                      
		switch ( usb_host_ep->desc.bmAttributes & 
			UE_XFERTYPE ) {
                                        
		case UE_BULK:
			if ( usb_host_ep->desc.bEndpointAddress & 0x80 ) {
				buf_size =  MAX_RECVBUF_SZ;
			} else {
				buf_size =  MAX_XMITBUF_SZ;
			}

                        err = rtw_usb_setup_endpoint(udev, usb_host_ep, buf_size);
		
                        printf("%s()-%d: bEndpointAddress = %02x, rtw_usb_setup_endpoint(udev, p_uhe, %d) = %d\n", 
				__FUNCTION__, __LINE__, usb_host_ep->desc.bEndpointAddress, buf_size, err);
		...
		default:
			/* Error */
			printf("%s() - line %d: ERROR! ue_type = %d\n", __FUNCTION__, __LINE__, 
				(usb_host_ep->desc.bmAttributes & UE_XFERTYPE));
			return (EINVAL);
		}
        }
...
#endif // Add by Kenny for RTW BSD driver
