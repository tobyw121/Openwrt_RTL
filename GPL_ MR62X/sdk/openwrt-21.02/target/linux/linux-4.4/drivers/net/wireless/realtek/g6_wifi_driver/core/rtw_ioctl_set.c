				...
				NDIS_802_11_NETWORK_INFRASTRUCTURE networktype, u8 flags)
{
	...
	if (*pold_state != networktype) {
		...
		RTW_PRINT("change mode, old_mode=%d, new_mode=%d, fw_state=0x%x\n",
		...
		*pold_state = networktype;
...
#ifdef CONFIG_NATIVEAP_MLME
			#ifdef CONFIG_RTW_DEBUG_MBSSID_VAP
			RTW_PRINT(FUNC_ADPT_FMT "Start AP mode! (%u)\n",
			          FUNC_ADPT_ARG(padapter), MLME_IS_AP(padapter));
	...
	else if ((networktype == Ndis802_11APMode) && !(check_fwstate(pmlmepriv, WIFI_AP_STATE))) {
		#ifdef CONFIG_RTW_DEBUG_MBSSID_VAP
			RTW_PRINT("(%s) wrong fw_state (0x%08x)! start to reset fw state. \n",
			          padapter->pnetdev->name, MLME_STATE(padapter));
		#endif /* CONFIG_RTW_DEBUG_MBSSID_VAP */
		_rtw_spinlock_bh(&pmlmepriv->lock);
		_clr_fwstate_(pmlmepriv, ~WIFI_NULL_STATE);
		set_fwstate(pmlmepriv, WIFI_AP_STATE);
		start_ap_mode(padapter);
		#ifdef CONFIG_RTW_DEBUG_MBSSID_VAP
			RTW_PRINT("(%s) setting fw_state (0x%08x) success. \n",
			          padapter->pnetdev->name, MLME_STATE(padapter));
		#endif /* CONFIG_RTW_DEBUG_MBSSID_VAP */
		_rtw_spinunlock_bh(&pmlmepriv->lock);
	}
...
u8 rtw_set_acs_sitesurvey(_adapter *adapter)
{
	...
	if(adapter->registrypriv.wifi_mib.acs == 2){
		parm.acs = 2;
	}
	else{
		parm.acs = 1;
...
int rtw_set_channel_plan(_adapter *adapter, u8 channel_plan, u8 chplan_6g)
{
	...
	return rtw_set_chplan_cmd(adapter, RTW_CMDF_WAIT_ACK, channel_plan, chplan_6g, 1);
}
...
{
#if 1
	return rtw_set_country_cmd(adapter, RTW_CMDF_DIRECTLY, country_code, 1);
#else
