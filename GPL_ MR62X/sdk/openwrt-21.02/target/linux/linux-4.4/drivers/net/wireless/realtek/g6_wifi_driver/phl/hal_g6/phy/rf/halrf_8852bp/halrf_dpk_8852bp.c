...
/*8852BP DPK ver:0x2 20210929*/

void _dpk_bkup_kip_8852bp(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][DPK_KIP_REG_NUM_8852BP],
	u8 path)
{
	u8 i;

	for (i = 0; i < DPK_KIP_REG_NUM_8852BP; i++) {
...
void _dpk_bkup_bb_8852bp(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[DPK_BB_REG_NUM_8852BP])
{
	u8 i;

	for (i = 0; i < DPK_BB_REG_NUM_8852BP; i++) {
...
void _dpk_bkup_rf_8852bp(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][DPK_RF_REG_NUM_8852BP],
	u8 path)
{
	u8 i;

	for (i = 0; i < DPK_RF_REG_NUM_8852BP; i++) {
...
void _dpk_bb_afe_setting_8852bp(
	struct rf_info *rf,
	enum rf_path path)
{
	...
	/*3.Set DAC clk*/
	halrf_txck_force_8852bp(rf, path, true, DAC_960M);
	/*4. Set ADC clk*/
	halrf_rxck_force_8852bp(rf, path, true, ADC_1920M);
...
void _dpk_bb_afe_restore_8852bp(
	struct rf_info *rf,
	enum rf_path path)
{
...
void _dpk_txpwr_bb_force_8852bp(
	struct rf_info *rf,
	enum rf_path path,
	bool is_force)
{
...
void _dpk_kip_pwr_clk_onoff_8852bp(
	struct rf_info *rf,
	bool turn_on)
{
	if (turn_on) {
...
void _dpk_kip_control_rfc_8852bp(
	struct rf_info *rf,
	enum rf_path path,
	bool ctrl_by_kip)
{
	...
	u8 kidx)
{
	...
	halrf_wreg(rf, 0x81bc + (path << 8) + (kidx << 2), 0x00003F00, 0x01); /*[13:8]*/

	_dpk_kip_control_rfc_8852bp(rf, path, true);
	...
	_dpk_one_shot_8852bp(rf, phy, path, D_KIP_PRESET);
}

void _dpk_kip_restore_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path)
{
	_dpk_one_shot_8852bp(rf, phy, path, D_KIP_RESTORE);
	...
	_dpk_kip_control_rfc_8852bp(rf, path, false);
	_dpk_txpwr_bb_force_8852bp(rf, path, false);
#if 0
	...
	u32 reg)
{
	...
	_dpk_kip_control_rfc_8852bp(rf, path, (bool)ori_ctrl);
#endif
}

void _dpk_kset_query_8852bp(
	struct rf_info *rf,
	enum rf_path path)
{
	struct halrf_dpk_info *dpk = &rf->dpk;
	...
	u8 kidx)
{
	...
	//_dpk_rf_reg_query_8852bp(rf, path, 0x00);
...
void _dpk_kip_set_txagc_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 dbm,
...
{	
	...
	_dpk_one_shot_8852bp(rf, phy, path, D_TXAGC);
	_dpk_kset_query_8852bp(rf, path);
	//_dpk_rf_reg_query_8852bp(rf, path, 0x11);
}

bool _dpk_kip_set_rxagc_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx)
{
	_dpk_one_shot_8852bp(rf, phy, path, D_RXAGC);
#if 0
	halrf_wreg(rf, 0x80d4, 0x000F0000, 0x8);
	RF_DBG(rf, DBG_RF_DPK, "[DPK] set RXBB = 0x%x\n",
		halrf_rreg(rf, 0x80fc, 0x0000001F));
#endif
	return _dpk_sync_check_8852bp(rf, path, kidx);
}

void _dpk_lbk_rxiqk_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path)
{
	...
	halrf_wrf(rf, path, 0x98, 0x0000007F, 0x3); /*[6:0] DPK_RXIQK Att*/
	...
	_dpk_one_shot_8852bp(rf, phy, path, LBK_RXIQK);
...
void _dpk_get_thermal_8852bp(struct rf_info *rf, u8 kidx, enum rf_path path)
{
	struct halrf_dpk_info *dpk = &rf->dpk;
	...
	dpk->bp[path][kidx].ther_dpk = halrf_get_thermal_8852bp(rf, path);
	...
	u8 kidx)
{
	...
	RF_DBG(rf, DBG_RF_DPK, "[DPK] ARF 0x0/0x83/0x9e/0x1a/0xdf = 0x%x/ 0x%x/ 0x%x/ 0x%x/ 0x%x\n",
...
u8 _dpk_gainloss_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx)
{
	_dpk_one_shot_8852bp(rf, phy, path, D_GAIN_LOSS);
	_dpk_kip_set_txagc_8852bp(rf, phy, path, 0xff, false); /*set new tx power*/

	/*for k3 hw_bug*/
	halrf_wreg(rf, 0x81f0 + (path << 8), 0x0003FFFF, 0x0); /*[17:0]*/
	halrf_wreg(rf, 0x81f0 + (path << 8), 0xF0000000, 0x0); /*[31:28]*/

	return _dpk_gainloss_read_8852bp(rf);
}

u8 _dpk_pas_read_8852bp(
	struct rf_info *rf,
	u8 is_check)
{
	u8 i;
	...
	u8 loss_only)
{
	...
	do {
		...
		case 0: /*SYNC and Dgain*/
			is_fail = _dpk_kip_set_rxagc_8852bp(rf, phy, path, kidx);

			if (DPK_RXSRAM_DBG)
				_dpk_read_rxsram_8852bp(rf);
			...
			dgain = _dpk_dgain_read_8852bp(rf);

			if (agc_cnt == 0)
				_dpk_bypass_rxiqc_8852bp(rf, path);
	
			i = 1;
			break;

		case 1: /*GAIN_LOSS and idx*/
			tmp_gl_idx = _dpk_gainloss_8852bp(rf, phy, path, kidx);
			/*_dpk_pas_read_8852bp(rf, false);*/

			if ((tmp_gl_idx == 0 && _dpk_pas_read_8852bp(rf, true) == 1) || tmp_gl_idx >= 7)
				i = 2; /*GL > criterion*/
		...
		case 2: /*GL > criterion*/
			if (tmp_dbm <= 7) {
				...
				tmp_dbm = tmp_dbm - 3;
				_dpk_kip_set_txagc_8852bp(rf, phy, path, tmp_dbm, true);
			}
			i = 0;
			agc_cnt++;
			break;
...
void _dpk_set_mdpd_para_8852bp(
	struct rf_info *rf,
	u8 order)
{
	switch (order) {
	...
	u8 kidx)
{
	...
	_dpk_set_mdpd_para_8852bp(rf, 0x0); /*5,3,1*/

	halrf_wreg(rf, 0x809c, BIT(8), 0x0);

	_dpk_one_shot_8852bp(rf, phy, path, D_MDPK_IDL);
}

u8 _dpk_order_convert_8852bp(
	struct rf_info *rf)
{
	u8 val;
...
void _dpk_gain_normalize_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx,
	bool is_execute)
{
	...
	if (is_execute) {
		...
		_dpk_one_shot_8852bp(rf, phy, path, D_GAIN_NORM);
	} else
		halrf_wreg(rf, reg[kidx][dpk->cur_k_set] + (path << 8), 0x0000007F, 0x5b);
	...
	u8 kidx)
{
	...
	halrf_wreg(rf, 0x81bc + (path << 8) + (kidx << 2),
			BIT(26) | BIT(25) | BIT(24), _dpk_order_convert_8852bp(rf));
	...
	_dpk_gain_normalize_8852bp(rf, phy, path, kidx, true);
	//_dpk_para_query_8852bp(rf, path, kidx);
}

void _dpk_coef_read_8852bp(
	struct rf_info *rf,
	enum rf_path path,
	u8 kidx)
{
	...
	enum rf_path path)
{
	...
	_dpk_kip_control_rfc_8852bp(rf, path, false);
	halrf_rf_direct_cntrl_8852bp(rf, path, false); /*switch control to direct write*/
	halrf_wrf(rf, path, 0x10005, MASKRF, 0x03ffd); /*only keep BB control TX_POWER*/

	_dpk_rf_setting_8852bp(rf, path, kidx);
	halrf_set_rx_dck_8852bp(rf, phy, path, false);

	_dpk_kip_pwr_clk_onoff_8852bp(rf, true);
	_dpk_kip_preset_8852bp(rf, phy, path, kidx);
	_dpk_txpwr_bb_force_8852bp(rf, path, true);
	_dpk_kip_set_txagc_8852bp(rf, phy, path, init_xdbm, true);
	_dpk_tpg_sel_8852bp(rf, path, kidx);
#if 0
	is_fail = _dpk_kip_set_rxagc_8852bp(rf, phy, path, kidx);
	...
	_dpk_dgain_read_8852bp(rf);
	_dpk_bypass_rxiqc_8852bp(rf, path);
	_dpk_gainloss_8852bp(rf, phy, path, kidx);
	_dpk_para_query_8852bp(rf, path, kidx);
#else
	is_fail = _dpk_agc_8852bp(rf, phy, path, kidx, init_xdbm, false);

	if (is_fail)
		goto _error;
#endif
	/*_dpk_pas_read_8852bp(rf, false);*/
	//_dpk_get_thermal_8852bp(rf, kidx, path);

	_dpk_idl_mpa_8852bp(rf, phy, path, kidx);
	_dpk_para_query_8852bp(rf, path, kidx);

	//_dpk_kip_control_rfc_8852bp(rf, path, false);
	//_dpk_get_thermal_8852bp(rf, kidx, path);
#if 0
	_dpk_coef_read_8852bp(rf, path, kidx, gain);
#endif
	_dpk_on_8852bp(rf, phy, path, kidx);
_error:
	_dpk_kip_control_rfc_8852bp(rf, path, false);
	...
	u8 kpath)
{
	...
	u32 kip_bkup[DPK_RF_PATH_MAX_8852BP][DPK_KIP_REG_NUM_8852BP] = {{0}};
	//u32 bb_bkup[DPK_BB_REG_NUM_8852BP] = {0};
	u32 rf_bkup[DPK_RF_PATH_MAX_8852BP][DPK_RF_REG_NUM_8852BP] = {{0}};

	u32 kip_reg[] = {0x813c, 0x8124, 0x8120, 0xc0d4, 0xc0d8};
	//u32 bb_reg[] = {0x2344, 0x5800, 0x7800};
	u32 rf_reg[DPK_RF_REG_NUM_8852BP] = {0xdf, 0x5, 0x10005};

	u8 path;
	bool is_fail = true;
	bool reloaded[DPK_RF_PATH_MAX_8852BP] = {false};

	if ((!phl_is_mp_mode(rf->phl_com)) && DPK_RELOAD_EN_8852BP) {
		for (path = 0; path < DPK_RF_PATH_MAX_8852BP; path++) {
			reloaded[path] = _dpk_reload_check_8852bp(rf, phy, path);
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8852BP; path++) {
		if (kpath & BIT(path)) {
			_dpk_bkup_kip_8852bp(rf, kip_reg, kip_bkup, path);
			_dpk_bkup_rf_8852bp(rf, rf_reg, rf_bkup, path);
			_dpk_information_8852bp(rf, phy, path);
			_dpk_init_8852bp(rf, path);
			if (rf->is_tssi_mode[path])
				_dpk_tssi_pause_8852bp(rf, path, true);
		}
	}
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8852BP; path++) {
		if (kpath & BIT(path)) {
			...
			_dpk_rxagc_onoff_8852bp(rf, path, false);
			halrf_drf_direct_cntrl_8852bp(rf, path, false);
			_dpk_bb_afe_setting_8852bp(rf, path);
			is_fail = _dpk_main_8852bp(rf, phy, path);
			halrf_dpk_onoff_8852bp(rf, path, is_fail);
		}
	}

	for (path = 0; path < DPK_RF_PATH_MAX_8852BP; path++) {
		if (kpath & BIT(path)) {
			_dpk_kip_restore_8852bp(rf, phy, path);
			_dpk_reload_kip_8852bp(rf, kip_reg, kip_bkup, path);
			_dpk_reload_rf_8852bp(rf, rf_reg, rf_bkup, path);
			_dpk_bb_afe_restore_8852bp(rf, path);
			_dpk_rxagc_onoff_8852bp(rf, path, true);
			if (rf->is_tssi_mode[path])
				_dpk_tssi_pause_8852bp(rf, path, false);
		}
	}
	_dpk_kip_pwr_clk_onoff_8852bp(rf, false);
}

u8 _dpk_bypass_check_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy)
{
	...
	if (fem->epa_2g && (rf->hal_com->band[phy].cur_chandef.band == BAND_ON_24G)) {
		...
		result = 1;
	} else if (rf->hal_com->band[phy].cur_chandef.band != BAND_ON_24G) {
		RF_DBG(rf, DBG_RF_DPK, "[DPK] Skip DPK due to not 2G!!\n");
		result = 1;
	} else
		result = 0;
	...
	enum phl_phy_idx phy)
{
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8852BP; path++) {
		if (kpath & BIT(path))
			halrf_dpk_onoff_8852bp(rf, path, true);
	}
}

void halrf_dpk_8852bp(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	bool force)
{
	RF_DBG(rf, DBG_RF_DPK, "[DPK] ****** DPK Start (Ver: 0x%x, Cv: %d, RF_para: %d) ******\n",
		DPK_VER_8852BP, rf->hal_com->cv, halrf_get_radio_ver_from_reg(rf));

	RF_DBG(rf, DBG_RF_DPK, "[DPK] Driver mode = %d\n", rf->phl_com->drv_mode);
#if 1
	if (_dpk_bypass_check_8852bp(rf, phy))
		_dpk_force_bypass_8852bp(rf, phy);
	else
		_dpk_cal_select_8852bp(rf, force, phy, halrf_kpath_8852bp(rf, phy));
#else
	//_dpk_information_8852bp(rf, 0, RF_PATH_A);
	//halrf_drf_direct_cntrl_8852bp(rf, RF_PATH_A, false);
	//_dpk_bb_afe_setting_8852bp(rf, RF_PATH_A);
	
	//_dpk_main_8852bp(rf, phy, RF_PATH_A);
	//halrf_delay_us(rf, 100);
	halrf_rf_direct_cntrl_8852bp(rf, RF_PATH_A, false); /*switch control to direct write*/
	//halrf_delay_us(rf, 100);
	//halrf_wrf(rf, RF_PATH_A, 0x10005, MASKRF, 0x03ffd); /*only keep BB control TX_POWER*/
	_dpk_rf_setting_8852bp(rf, RF_PATH_A, 0);
	//halrf_delay_us(rf, 70);
	halrf_set_rx_dck_8852bp(rf, phy, RF_PATH_A, false);
	//_dpk_kip_pwr_clk_onoff_8852bp(rf, true);
	//_dpk_kip_preset_8852bp(rf, phy, RF_PATH_A, 0);
	//_dpk_txpwr_bb_force_8852bp(rf, RF_PATH_A, true);
	//_dpk_kip_set_txagc_8852bp(rf, phy, RF_PATH_A, 21, true);
	//_dpk_tpg_sel_8852bp(rf, RF_PATH_A, 0);
	//_dpk_kip_set_rxagc_8852bp(rf, phy, RF_PATH_A, 0);


#endif
}

void halrf_dpk_onoff_8852bp(
	struct rf_info *rf,
	enum rf_path path,
	bool off)
{
