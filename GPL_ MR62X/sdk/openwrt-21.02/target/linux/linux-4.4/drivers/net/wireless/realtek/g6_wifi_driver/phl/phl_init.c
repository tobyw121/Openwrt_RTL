					...
					struct rtw_ic_info *ic_info)
{
	...
	*phl = phl_info;
	...
	/*default settings */
	phl_info->phl_com->check_flag.achq_stuck_check = 1;
...
enum rtw_phl_status rtw_phl_rf_off(void *phl)
{
	...
	rtw_hal_backup_key(phl_info->hal);
#endif
...
#ifdef CONFIG_ENABLE_MAC_KEY_BACKUP
	rtw_hal_restore_key(phl_info->hal);
#endif
...
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	return rtw_hal_get_trx_wdg_fail_cnt(phl_info->hal);
}
#endif
