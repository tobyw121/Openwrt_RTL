...
struct rtw_t_meta_data {
	...
	/* tx cnt & rty rate */
	u8 dis_rts_rate_fb:1;
	u8 dis_data_rate_fb:1;
	u16 data_rty_lowest_rate:12;
	u8 data_tx_cnt_lmt:6;
	u8 data_tx_cnt_lmt_en:1;
...
#if defined(MAC_AX_FEATURE_HV) && (MAC_AX_FEATURE_HV == 1)
	void *mac_priv;
#endif
...
struct rtw_r_meta_data {
	...
	u8 rxsc_parse_wd_matched;
#endif
#ifdef CONFIG_RTW_DEBUG_HW_RX_AMSDU_CUT
	u8 debug_dump;		/* Debug dump */
...
struct rtw_xmit_req {
	...
	u8 is_msdu; /* if msdu transfer in phl, 0:CORE 1:PHL */
#endif
#endif
...
struct rtw_recv_pkt {
	...
	#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
	struct list_head rx_buf_lst_head; /* List of RX buffers */
	struct rtw_rx_buf_base *rx_buf; /* First RX buffer */
	#else
