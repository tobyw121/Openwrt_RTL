...
#define WP_OFFSET_UNIT_8851E 0x8
...
u32 mac_txdesc_len_8851e(struct mac_ax_adapter *adapter,
...
static u32 txdes_proc_h2c_fwdl_8851e(struct mac_ax_adapter *adapter,
				     struct rtw_t_meta_data *info,
				     u8 *buf, u32 len)
{
...
static u32 txdes_proc_hv_8851e(struct mac_ax_adapter *adapter,
			       struct rtw_t_meta_data *info,
...
static u32 txdes_proc_data_8851e(struct mac_ax_adapter *adapter,
				 struct rtw_t_meta_data *info, u8 *buf, u32 len)
{
	...
	if (len != mac_txdesc_len_8851e(adapter, info))
		return MACBUFSZ;
...
#if MAC_AX_FEATURE_HV
	txdes_proc_hv_8851e(adapter, info, wdb, wdi);
#endif
	if (info->wdinfo_en != 0)
		PLTFM_MEMCPY(buf + WD_BODY_LEN_V1, (u8 *)wdi, WD_INFO_LEN);

	if (adapter->hw_info->wd_checksum_en)
		mac_wd_checksum_8851e(adapter, info, buf);

	return MACSUCCESS;
}

static u32 txdes_proc_mgnt_8851e(struct mac_ax_adapter *adapter,
				 struct rtw_t_meta_data *info, u8 *buf, u32 len)
{
	struct wd_body_t_v1 *wdb;
	struct wd_info_t *wdi;
...
static struct txd_proc_type txdes_proc_mac_8851e[] = {
	{RTW_PHL_PKT_TYPE_H2C, txdes_proc_h2c_fwdl_8851e},
	{RTW_PHL_PKT_TYPE_FWDL, txdes_proc_h2c_fwdl_8851e},
	{RTW_PHL_PKT_TYPE_DATA, txdes_proc_data_8851e},
	{RTW_PHL_PKT_TYPE_MGNT, txdes_proc_mgnt_8851e},
	{RTW_PHL_PKT_TYPE_MAX, NULL},
};

u32 mac_build_txdesc_8851e(struct mac_ax_adapter *adapter,
			   struct rtw_t_meta_data *info, u8 *buf, u32 len)
{
	struct txd_proc_type *proc = txdes_proc_mac_8851e;
...
u32 mac_refill_txdesc_8851e(struct mac_ax_adapter *adapter,
			    ...
			    struct mac_ax_refill_info *info)
{
	...
	if (adapter->hw_info->wd_checksum_en)
		mac_wd_checksum_8851e(adapter, txpkt_info, info->pkt);

	return MACSUCCESS;
}

static u32 rxdes_parse_comm_8851e(struct mac_ax_adapter *adapter,
...
static u32 rxdes_parse_wifi_8851e(struct mac_ax_adapter *adapter,
...
static u32 rxdes_parse_c2h_8851e(struct mac_ax_adapter *adapter,
				 struct mac_ax_rxpkt_info *info, u8 *buf, u32 len)
{
...
static u32 rxdes_parse_ch_info_8851e(struct mac_ax_adapter *adapter,
				     struct mac_ax_rxpkt_info *info, u8 *buf, u32 len)
{
...
static u32 rxdes_parse_dfs_8851e(struct mac_ax_adapter *adapter,
				 struct mac_ax_rxpkt_info *info, u8 *buf, u32 len)
{
...
static u32 rxdes_parse_ppdu_8851e(struct mac_ax_adapter *adapter,
				  struct mac_ax_rxpkt_info *info, u8 *buf, u32 len)
{
	u32 hdr_val = le32_to_cpu(((struct rxd_short_t *)buf)->dword0);
...
static struct rxd_parse_type rxdes_parse_mac_8851e[] = {
	{RXD_S_RPKT_TYPE_WIFI, rxdes_parse_wifi_8851e},
	{RXD_S_RPKT_TYPE_C2H, rxdes_parse_c2h_8851e},
	{RXD_S_RPKT_TYPE_PPDU, rxdes_parse_ppdu_8851e},
	{RXD_S_RPKT_TYPE_CH_INFO, rxdes_parse_ch_info_8851e},
	{RXD_S_RPKT_TYPE_DFS_RPT, rxdes_parse_dfs_8851e},
	{RXD_RPKT_TYPE_INVALID, NULL},
};

u32 mac_parse_rxdesc_8851e(struct mac_ax_adapter *adapter,
			   struct mac_ax_rxpkt_info *info, u8 *buf, u32 len)
{
	struct rxd_parse_type *parse = rxdes_parse_mac_8851e;
	u8 rpkt_type;
	u32 hdr_val;
	u32 (*handler)(struct mac_ax_adapter *adapter,
	...
	//rxdes_parse_comm_8851e(adapter, info, buf);
...
u32 mac_wd_checksum_8851e(struct mac_ax_adapter *adapter,
			  struct rtw_t_meta_data *info, u8 *wddesc)
{
	...
	data = (u16 *)(wddesc);
	/*unit : 4 bytes*/
	wddesc_size = mac_txdesc_len_8851e(adapter, info) >> 2;
...
u32 mac_patch_rx_rate_8851e(struct mac_ax_adapter *adapter,
			    struct rtw_r_meta_data *info)
{
	return MACSUCCESS;
}

u32 mac_get_wp_offset_8851e(struct mac_ax_adapter *adapter,
			    struct mac_txd_ofld_wp_offset *ofld_conf, u16 *val)
{
	...
	*val = (ret_val / WP_OFFSET_UNIT_8851E) + (ret_val % WP_OFFSET_UNIT_8851E ? 0x1 : 0x0);
