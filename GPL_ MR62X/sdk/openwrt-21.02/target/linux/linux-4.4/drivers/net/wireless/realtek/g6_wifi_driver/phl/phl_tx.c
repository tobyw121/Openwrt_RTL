...
void rtw_phl_reset_device_tx_statistics(void *phl)
{
	...
	phl_com->phl_stats.tx_byte_total = 0;
	phl_com->phl_stats.tx_byte_uni = 0;
	phl_com->phl_stats.tx_tp_kbits = 0;
...
void rtw_phl_acc_device_tx_statistics(void *phl, u64 tx_byte_total, u64 tx_byte_uni, u64 tx_tp_kbits)
{
	...
	phl_stats->tx_byte_total += tx_byte_total;
	phl_stats->tx_byte_uni += tx_byte_uni;
	phl_stats->tx_tp_kbits += tx_tp_kbits;
}

void rtw_phl_sync_tx_statistics(struct rtw_phl_stainfo_t *sta, u64 tx_byte_total, u64 tx_byte_uni, u64 tx_tp_kbits)
{
	sta->stats.tx_byte_total = tx_byte_total;
	sta->stats.tx_byte_uni = tx_byte_uni;
	sta->stats.tx_tp_kbits = tx_tp_kbits;
}
#endif
...
#ifndef CONFIG_PHL_TX_STATS_CORE
				...
				struct rtw_phl_tring_list *ring_list)
{
	...
	for (i = 0; i < MAX_PHL_RING_CAT_NUM; i++) {
		...
		_os_kmem_free(drv_priv, ring->entry, MAX_PHL_RING_ENTRY_NUM * sizeof(u8 *));
		ring->entry = NULL;
	}
#ifdef CORE_RX_PRIO_HIGH
	_os_kmem_free(drv_priv, ring_list, sizeof(*ring_list));
#else
	_os_mem_free(drv_priv, ring_list, sizeof(*ring_list));
#endif
}
			...
			u8 hw_band, u8 hw_wmm, u8 hw_port)
{
	...
	phl_tring_list = (struct rtw_phl_tring_list *)_os_kmem_alloc(drv_priv,
								buf_len);
#else
	phl_tring_list = (struct rtw_phl_tring_list *)_os_mem_alloc(drv_priv,
								buf_len);
#endif
	...
	if (NULL != phl_tring_list) {
		...
		for (i = 0; i < MAX_PHL_RING_CAT_NUM; i++) {
			...
			phl_tring_list->phl_ring[i].entry = (u8 **)_os_kmem_alloc(drv_priv, MAX_PHL_RING_ENTRY_NUM * sizeof(u8 *));
			if (phl_tring_list->phl_ring[i].entry == NULL) {
				PHL_DBG("cannot malloc %lu bytes, stop!", (unsigned long int)(MAX_PHL_RING_ENTRY_NUM * sizeof(u8 *)));
				goto exit;
			}
		}
...
phl_register_tx_ring(void *phl, u16 macid, u8 hw_band, u8 hw_wmm, u8 hw_port)
{
	...
	if (NULL != phl_tring_list) {
		...
		phl_info->phl_tring_list[macid] = phl_tring_list;/* _PHL_REFINE_VW_ */
...
enum rtw_phl_status phl_deregister_tx_ring(void *phl, u16 macid)
{
	...
	phl_list_for_loop_safe(phl_tring_list, t, struct rtw_phl_tring_list,
		...
		if (macid == phl_tring_list->macid) {
			...
			phl_info->phl_tring_list[macid] = NULL;/* _PHL_REFINE_VW_ */
			phl_status = RTW_PHL_STATUS_SUCCESS;
			break;
		}
	}
...
struct rtw_phl_tring_list *phl_del_tring_list(struct phl_info_t *phl_info)
{
	void *drv_priv = phl_to_drvpriv(phl_info);
	struct rtw_phl_tring_list *phl_tring_list = NULL, *t;
	_os_list *ring_list = NULL;
	...
	phl_list_for_loop_safe(phl_tring_list, t, struct rtw_phl_tring_list, ring_list, list) {
		list_del(&phl_tring_list->list);
		break;
	}
...
void phl_free_deferred_tx_ring(struct phl_info_t *phl_info)
{
	...
	while((phl_tring_list = phl_del_tring_list(phl_info))) {
		_phl_free_phl_tring_list(phl_info, phl_tring_list);
	}
}
...
u8 phl_check_xmit_ring_resource(struct phl_info_t *phl_info, _os_list *sta_list)
{
	...
	struct rtw_phl_tring_list *tring_list_start = NULL;
	int start=1;
	...
	phl_list_for_loop_safe(tring_list, t, struct rtw_phl_tring_list,
		...
		if(start){
			psta = rtw_phl_get_stainfo_by_macid_wo_lock(phl_info, tring_list->macid);
			if(psta){
				psta->phl_tx_ring_start_cnt++;
			}
		}
		...
		if(start){
			list_del(&tring_list->list);
			tring_list_start = tring_list;
			start = 0;
		}
	}
	if (!phl_info->use_onetxring && tring_list_start)
		list_add_tail(&tring_list_start->list, tring_list_head);
...
struct rtw_h2c_pkt *phl_query_idle_h2c_pkt(struct phl_info_t *phl_info, u8 type)
{
	...
	if (idle_cnt)
		PHL_TRACE(COMP_PHL_DBG, _PHL_DEBUG_, "phl_query_idle_h2c_pkt => remaining %d (type %d).\n", *idle_cnt, h2c_type);
...
static void _phl_ser_callback_pcie(void *context)
{
	...
	phl_status = phl_ser_send_msg(phl_info, RTW_PHL_SER_EVENT_CHK);
}

static void _phl_fw_wdt_callback_pcie(void *context)
{
	enum rtw_phl_status phl_status = RTW_PHL_STATUS_SUCCESS;
	struct rtw_phl_handler *phl_handler
		= (struct rtw_phl_handler *)phl_container_of(context,
	...
	PHL_PRINT("%s : send msg for ser l2 reset\n", __func__);
	phl_status = phl_ser_send_msg(phl_info, RTW_PHL_SER_L2_RESET);
}
...
void phl_datapath_deinit(struct phl_info_t *phl_info)
{
	...
	struct rtw_phl_handler *phl_ser_handler = &phl_info->phl_ser_handler;
	struct rtw_phl_handler *phl_fw_wdt_handler = &phl_info->phl_fw_wdt_handler;
	struct phl_hci_trx_ops *hci_trx_ops = phl_info->hci_trx_ops;
	void *drv_priv = NULL;
	FUNCIN();
	drv_priv = phl_to_drvpriv(phl_info);
	...
	phl_deregister_handler(phl_info->phl_com, phl_ser_handler);
	phl_deregister_handler(phl_info->phl_com, phl_fw_wdt_handler);
	...
	_os_spinlock_free(drv_priv, &phl_info->rx_lock);
#endif
	FUNCOUT();
}
...
enum rtw_phl_status phl_datapath_init(struct phl_info_t *phl_info)
{
	...
	struct rtw_phl_handler *fw_wdt_handler = &phl_info->phl_fw_wdt_handler;
	void *drv_priv = NULL;
	FUNCIN_WSTS(pstatus);
	drv_priv = phl_to_drvpriv(phl_info);
	...
	do {
...
#ifdef RTW_WKARD_AP_RESET_RX_LOCK
		_os_spinlock_init(drv_priv, &phl_info->rx_lock);
#endif
		...
		ser_handler->type = RTW_PHL_HANDLER_PRIO_HIGH;
		ser_handler->callback = _phl_ser_callback_pcie;
		ser_handler->context = phl_info;
		ser_handler->drv_priv = drv_priv;
		ser_handler->status = 0;

		#ifdef CONFIG_RTW_OS_HANDLER_EXT
		ser_handler->id = RTW_PHL_SER_HANDLER;
		#endif /* CONFIG_RTW_OS_HANDLER_EXT */
		...
		fw_wdt_handler->type = RTW_PHL_HANDLER_PRIO_HIGH;
		fw_wdt_handler->callback = _phl_fw_wdt_callback_pcie;
		fw_wdt_handler->context = phl_info;
		fw_wdt_handler->drv_priv = drv_priv;
		fw_wdt_handler->status = 0;

		#ifdef CONFIG_RTW_OS_HANDLER_EXT
		fw_wdt_handler->id = RTW_PHL_FW_WDT_HANDLER;
		#endif /* CONFIG_RTW_OS_HANDLER_EXT */

		pstatus = phl_register_handler(phl_info->phl_com, fw_wdt_handler);
		if (RTW_PHL_STATUS_SUCCESS != pstatus)
			break;
...
_phl_select_tx_ring(struct phl_info_t *phl_info, struct rtw_xmit_req *tx_req)
{
	...
	if (phl_info->use_onetxring) {
		...
		tring_list = phl_info->phl_tring_list[macid];
		if(tring_list != NULL)
			ring = &tring_list->phl_ring[tx_req->mdata.cat];
		#else
		phl_list_for_loop_safe(tring_list, t, struct rtw_phl_tring_list,
				...
				struct rtw_xmit_req *tx_req)
{
	...
	if (NULL != ring) {
		...
		ring_res = phl_calc_avail_wptr(rptr, ring->core_idx,
						MAX_PHL_RING_ENTRY_NUM);
#if 0
		if (ring->dma_ch == 9 && tx_req->total_len > 1500) {
			PHL_ERR("TX CH9 M%u L%u C%u T%u\n",
			        tx_req->mdata.macid,
			        tx_req->mdata.pktlen,
			        tx_req->mdata.cat,
			        tx_req->treq_type);
		...
		if (ring_res > 0)
		{
			...
			#endif
			req_cnt = atomic_inc_return(&ring->tx_req_cnt);
			if (ring->tx_req_cnt_max < req_cnt)
				ring->tx_req_cnt_max = req_cnt;
		} else {
			PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_, "no ring resource to add new tx request!\n");
			#ifdef CONFIG_WFA_OFDMA_Logo_Test_Statistic
			if(sta){
				if(sta->active)
				sta->add_tx_ring_fail++;
			}
			#endif
			pstatus = RTW_PHL_STATUS_RESOURCE;
		}
...
u16 rtw_phl_tring_rsc(void *phl, u16 macid, u8 tid)
{
	...
	tring_list = phl_info->phl_tring_list[macid];/* _PHL_REFINE_VW_ */
	ring = &tring_list->phl_ring[tid];
	if (NULL != ring) {
		rptr = (u16)_os_atomic_read(drv_priv, &ring->phl_idx);
		ring_res = phl_calc_avail_rptr(rptr, ring->core_idx, MAX_PHL_RING_ENTRY_NUM);
	}
...
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_phl_tring_list *tring = phl_info->phl_tring_list[macid];
	int i;
	...
	for (i=0; i<MAX_PHL_RING_CAT_NUM; i++) {
		_os_dbgdump("CAT %d: max %4d cur %4d\n", i,
				tring->phl_ring[i].tx_req_cnt_max,
				atomic_read(&tring->phl_ring[i].tx_req_cnt));
		if (reset)
			tring->phl_ring[i].tx_req_cnt_max = 0;
	}
}
...
void rtw_phl_dump_idle_wd_cnt(void *phl)
{
	...
	for (i=0; i<txch_num; i++)
		_os_dbgdump("CH %d: idle %d busy %d pending %d\n", i,
				wd_ring[i].idle_wd_page_cnt, wd_ring[i].busy_wd_page_cnt,
				wd_ring[i].pending_wd_page_cnt);
}
...
void rtw_phl_get_txbd_sw(void *phl, u8 ch, u16 *host_idx, u16 *hw_idx, u16 *hw_res)
{
	...
	*host_idx = txbd[ch].host_idx;
	*hw_idx = txbd[ch].hw_idx;
	*hw_res = _os_atomic_read(NULL, &txbd[ch].avail_num);
}
...
void rtw_phl_dump_tx_ring(void *phl, void *m)
{
	...
	struct rtw_phl_tring_list *tring_list, *t;
	struct rtw_phl_tx_ring *ring = NULL;
	u16 phl_idx = 0, core_idx = 0, phl_next_idx = 0;
	s16	diff = 0;
	u16 avail = 0;

	RTW_PRINT_SEL(m, "===Dump PHL TX Ring===\n");

	_os_spinlock(drv_priv, &phl_info->t_ring_list_lock, _bh, NULL);
	phl_list_for_loop_safe(tring_list, t, struct rtw_phl_tring_list,
				list_head, list) {
		ring = &tring_list->phl_ring[0];
		phl_idx = (u16)_os_atomic_read(drv_priv, &ring->phl_idx);
		//core_idx = (u16)_os_atomic_read(drv_priv, &ring->core_idx);
		phl_next_idx = (u16)_os_atomic_read(drv_priv, &ring->phl_next_idx);

		avail = phl_calc_avail_rptr(phl_idx, ring->core_idx, MAX_PHL_RING_ENTRY_NUM);

		RTW_PRINT_SEL(m, "	macid[%d] ring[0] avail entry: %d\n", tring_list->macid, avail);

#if 0
		ring = &tring_list->phl_ring[1];
		phl_idx = (u16)_os_atomic_read(drv_priv, &ring->phl_idx);
		//core_idx = (u16)_os_atomic_read(drv_priv, &ring->core_idx);
		phl_next_idx = (u16)_os_atomic_read(drv_priv, &ring->phl_next_idx);
		...
		_os_dbgdump("	macid[%d] ring[1] avail entry: %d\n", tring_list->macid, avail);

		ring = &tring_list->phl_ring[2];
		phl_idx = (u16)_os_atomic_read(drv_priv, &ring->phl_idx);
		//core_idx = (u16)_os_atomic_read(drv_priv, &ring->core_idx);
		phl_next_idx = (u16)_os_atomic_read(drv_priv, &ring->phl_next_idx);
		...
		_os_dbgdump("	macid[%d] ring[2] avail entry: %d\n", tring_list->macid, avail);
#endif
	}
	_os_spinunlock(drv_priv, &phl_info->t_ring_list_lock, _bh, NULL);
	...
	u32 timeout)
{
	...
	if (rtw_hal_set_gt3(hal, count_en, mode, gt3_en, sort_en, timeout) == RTW_HAL_STATUS_SUCCESS)
		return RTW_PHL_STATUS_SUCCESS;
	else
		return RTW_PHL_STATUS_FAILURE;
}
...
rtw_phl_cmd_cfg_hw_seq(void *phl,
	...
	u32 cmd_timeout)
{
	return _phl_set_dctrl_tbl_seq(phl, sta, seq);
}
	...
	u32 cmd_timeout)
{
	...
	if(rtw_hal_getpkt(phl_com, hal, macid, type) == RTW_HAL_STATUS_SUCCESS)
		return RTW_PHL_STATUS_SUCCESS;
	else
		return RTW_PHL_STATUS_FAILURE;
}
