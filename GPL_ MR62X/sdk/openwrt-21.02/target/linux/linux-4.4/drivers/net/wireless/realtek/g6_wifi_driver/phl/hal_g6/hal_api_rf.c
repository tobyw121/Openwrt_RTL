			...
			struct hal_info_t *hal_info)
{
	...
	hal_status = halrf_init_g6(phl_com, hal_com, &(hal_info->rf));
...
void rtw_hal_init_rf_reg(struct rtw_phl_com_t *phl_com, void *hal)
{
	...
	for (phy_idx = 0; phy_idx < 1 ; phy_idx++) {
		halrf_config_radio(hal_info->rf, phy_idx);
	}
...
enum rtw_hal_status rtw_hal_rf_get_online_tssi_de(struct hal_info_t *hal_info, u8 phy_idx, u8 rf_path,s32 dbm, s32 pout, s32 *tssi_de)
{
	...
	*tssi_de = halrf_get_online_tssi_de_g6(hal_info->rf, phy_idx, rf_path, dbm, pout);
	...
	bool dbcc_en)
{
	...
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
#if 0 /* Temporarily disable DBCC configuration */
	struct hal_info_t *hal_info = (struct hal_info_t *)hal_com->hal_priv;
...
#if 0
	hal_status = halrf_dack_trigger_g6(hal_info->rf, force);
#else
	hal_status = RTW_HAL_STATUS_SUCCESS;
#endif
						...
						u8 phy_idx, u8 force)
{
	...
	hal_status = halrf_iqk_trigger_g6(hal_info->rf, phy_idx, force);
...
#if 0
	hal_status = halrf_lck_trigger_g6(hal_info->rf);
#else
	hal_status = RTW_HAL_STATUS_SUCCESS;
#endif
						...
						u8 phy_idx, u8 force)
{
	...
	hal_status = halrf_dpk_trigger_g6(hal_info->rf, phy_idx, force);
...
#if 0
	hal_status = halrf_tssi_trigger_g6(hal_info->rf, phy_idx);
#else
	hal_status = RTW_HAL_STATUS_SUCCESS;
#endif
					...
					char *output, u32 out_len)
{
	...
	if(incmd->in_type == RTW_ARG_TYPE_BUF)
		...
		halrf_cmd_parser_g6(hal_info->rf, incmd->in.vector,
...
rtw_hal_rf_dpk_switch(void *hal, bool enable)
{
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_, "rtw_hal_rf_dpk_switch(): enable(%d)\n",
		enable);
	halrf_dpk_switch_g6(hal_info->rf, enable);
}
					...
					u8 path, u8 iq_path, u32 avg, u32 fft)
{
	...
	halrf_psd_init_g6(hal_info->rf, cur_phy_idx, path, iq_path, avg, fft);
					...
					u32 point, u32 start_point, u32 stop_point, u32 *outbuf)
{
	...
	halrf_psd_query_g6(hal_info->rf, cur_phy_idx, point, start_point, stop_point, outbuf);
...
void
hal_rf_bf_config_rf(struct hal_info_t *hal_info)
{
	halrf_bf_config_rf(hal_info->rf);
}
...
void rtw_hal_rf_set_regulation(void *hal, u8 regular_idx)
{
	...
	halrf_set_regulation_from_driver(hal_info->rf, regular_idx);
	halrf_set_power(hal_info->rf, 0, PWR_LIMIT);
	halrf_set_power(hal_info->rf, 0, PWR_LIMIT_RU);
}
...
u32 rtw_hal_get_radio_ver(void *hal)
{
	...
	return halrf_get_radio_ver_from_reg(hal_info->rf);
}
...
void rtw_hal_rf_set_power_to_struct(void *hal, enum phl_phy_idx phy, s8 dbm){
	...
	halrf_set_fix_power_to_struct(hal_info->rf, phy, dbm);

	PHL_WARN("set fixed power = %d.%d dbm\n", (dbm / 2), (dbm * 10 / 2 % 10));
}
...
void rtw_hal_rf_tssi_do_slope(void *hal)
{
	...
	PHL_INFO ("[HAL] %s\n", __func__);
	halrf_tssi_do_slope(hal_info->rf);
}
...
void rtw_hal_rf_tssi_finish_slope(void *hal)
{
	...
	PHL_INFO ("[HAL] %s\n", __func__);
	halrf_tssi_finish_slope(hal_info->rf);
}
...
u32 rtw_hal_rf_tssi_get_cw(void *hal, enum rf_path path)
{
	...
	PHL_INFO ("[HAL] %s: path=%d\n", __func__, path);
	return halrf_tssi_get_cw(hal_info->rf, path);
}
					 ...
					 u32 gain_diff, u32 cw_diff, enum rf_path path)
{
	...
	PHL_INFO ("[HAL] %s: gain_diff=0x%x, cw_diff=0x%x, path=%d\n", __func__,
				gain_diff, cw_diff, path);
	halrf_tssi_current_slope_apply(hal_info->rf, gain_diff, cw_diff, path);
}
				 ...
				 u32* gain_diff, u32* cw_diff)
{
	...
	PHL_INFO ("[HAL] %s: gain_1st=%d, gain_2nd=%d, cw_1st=%d, cw_2nd=%d, path=%d\n", __func__,
				gain_1st, gain_2nd, cw_1st, cw_2nd, path);
	halrf_tssi_slope_apply(hal_info->rf,
				gain_1st, gain_2nd, cw_1st, cw_2nd,
				path, gain_diff, cw_diff);
	PHL_INFO ("[HAL] %s: gain_diff=%d, cw_diff=%d\n", __func__, *gain_diff, *cw_diff);
}

void rtw_hal_rf_control_tx_rate_power(void *hal, enum phl_phy_idx phy,
					s32 oft_dbm)
{
	struct hal_info_t *hal_info = (struct hal_info_t *) hal;

	PHL_INFO ("[HAL] %s: phy_idx=%d, oft_dbm=%d\n", __func__, phy, oft_dbm);

	halrf_control_tx_rate_power(hal_info->rf, phy, oft_dbm*100);
}
