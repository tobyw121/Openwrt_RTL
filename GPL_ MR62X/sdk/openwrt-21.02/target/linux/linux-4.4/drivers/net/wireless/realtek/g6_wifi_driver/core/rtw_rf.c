...
{
	struct rtw_chan_def chan_def = {.band = rtw_phl_get_band_type(ch),
					.chan = ch,
					.bw = bw,
					.offset = offset};
	return rtw_phl_get_center_ch(&chan_def);
}
...
const char *const _ch_offset_str[CHAN_OFFSET_MAX] = {
	"NO_EXT",
	"UPPER",
	"NO_DEF",
	"LOWER",
...
int op_class_pref_init(_adapter *adapter)
{
	...
	bw_bmp[BAND_ON_24G] = (ch_width_to_bw_cap(REGSTY_BW_2G(regsty) + 1) - 1) & (GET_HAL_DATA(dvobj)->dev_sw_cap.bw_sup);
	bw_bmp[BAND_ON_5G] = (ch_width_to_bw_cap(REGSTY_BW_5G(regsty) + 1) - 1) & (GET_HAL_DATA(dvobj)->dev_sw_cap.bw_sup);
		...
		for (j = 0; opc_pref->chs[j].ch != 0; j++) {
			...
			for (k = 0, l = 0; k < op_ch_num; k++) {
				...
				if (bw >= CHANNEL_WIDTH_160 && (chset[chset_idx].flags & RTW_CHF_NO_160MHZ))
					break;
				if ((chset[chset_idx].flags & RTW_CHF_DFS) && rtw_rfctl_dfs_domain_unknown_g6(rfctl))
					continue;
...
#if 0
void rtw_hal_dump_txpwr_lmt(void *sel, _adapter *adapter)
{
#define TMP_STR_LEN 16
	...
	for (band = BAND_ON_24G; band <= BAND_ON_5G; band++) {
		if (!rtw_hw_is_band_support(dvobj, band))
			continue;
