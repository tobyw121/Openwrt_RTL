...
static int confirm_to_peer(struct rtl8192cd_priv *priv, u8 * sa)
{
	...
	20200514 after refine sae sequence by order of Figure 4-29—Example using SAE authentication
	IEEE802.11_2016 don't need printk it */
	log("peer[%pm] st[%s],seq[%s] ", 
		peer->peer_mac, state_to_string(peer->state),seq_to_string(ieee_order(frame->authenticate.auth_seq)));
...
//rtk_sae token
	...
	log("peer[%pm] st[%s] (%s token)],gp[%d]",
		peer->peer_mac,state_to_string(peer->state),
		(token_len ? "with" : "no"),peer->grp_def->group_num);

	//dump_hex("we will send out..", frame, 128);
	//log("issue_sae_frame");
	if (issue_sae_frame(priv, (u8 *) frame, len) != 0) {
		log("tx fail [%pm]", peer->peer_mac);
		if (peer->private_val) {
			mbedtls_mpi_free(peer->private_val);
			kfree(peer->private_val);
			peer->private_val = NULL;
...
static int request_token(struct rtl8192cd_priv *priv, struct ieee80211_mgmt_frame *req, unsigned char *me, struct candidate *peer)	//rtk_sae
{
	...
	if (1) {
		dump_hex("token", ptr, MBEDTLS_DIGEST_LENGTH);
	}
	mbedtls_md_free(&ctx);
	len += MBEDTLS_DIGEST_LENGTH;
...
sae_process_auth(struct rtl8192cd_priv *priv, struct ieee80211_mgmt_frame *frame, int framelen)
{
	...
	/*before handling sae frame*/
	log("#1 peer[%pm] st[%s],seq[%s] ", frame->sa, state_to_string(peer->state),seq_to_string(seq));
	...
	case SAE_NOTHING:
		...
		case SAE_AUTH_COMMIT:
			...
			/*need to put before process_commit(), or it will crash...eric*/
			prepare_commit_to_peer(priv, frame->sa, NULL, 0, &resp_buf, &resp_buf_len);
			if (process_commit(priv, frame, framelen) < 0) {
				log("chk commit fail, reject");
				reject_to_peer(priv, peer, frame);
				return ERR_FATAL;
			}

			//log("send_commit_to_peer[%pm],len[%d]", peer->peer_mac,resp_buf_len);
		...
		case SAE_AUTH_CONFIRM:
			return ERR_FATAL;
		default:
			log("unknown seq(%d) from[%pm]", seq, peer->peer_mac);
		...
		case SAE_AUTH_COMMIT:
			...
			*/
			if (status == WLAN_STATUS_ANTI_CLOGGING_TOKEN_NEEDED){
				...
				log("RX token request,add token,len:%d, and resend commit", (framelen - (IEEE802_11_HDR_LEN + 8)));

				commit_to_peer(priv, frame->sa, frame->authenticate.u.var8 + 2,(framelen - (IEEE802_11_HDR_LEN + 8)));
				peer->sync = 0;
				peer->t0 = jiffies;
				break;
			}
	...
	case SAE_CONFIRMED:
		...
		case SAE_AUTH_CONFIRM:
					...
					&& (delme->state == SAE_ACCEPTED)) 
			{
				...
				log("peer[%pm] st[%s] has just ACCEPTED, found another in [%s], deleting",
					frame->sa, state_to_string(peer->state),state_to_string(delme->state));
	...
	/*after handling sae frame*/
	log("#2;peer[%pm] st[%s]\n\n", frame->sa, state_to_string(peer->state));
	if(peer->state==SAE_ACCEPTED){
		log("[%s](SAE done with peer[%pm])\n", priv->dev->name,frame->sa);
	}
	return NO_ERR;
}
	...
	GD * grp, unsigned char *password, unsigned short num)
{
	...
	if (strlen(password) == 64) {	/*format is hex , need do str to hex*/
		log("len==64, and wsc_enable:%d", priv->pmib->wscEntry.wsc_enable);
		get_array_val(key, password, 64);
		memcpy(grp->password, key, 32);
	}else{
...
int sae_initialize(struct rtl8192cd_priv *priv)
{
	...
	priv->mesh_profile[0].AuthenticationProtocol = 1;
#endif
	/*initialize globals*/
	...
	//retrans = RETRANS;
	priv->pmk_expiry = priv->pshare->rf_ft_var.pmk_expire;	/* one day */

	/*for AP mode we need support group 19/20/21 concurrent*/
	//clear group
	if(priv->gd){
		if(priv->gd->next){
	...
	if(OPMODE & WIFI_AP_STATE){
		for(idx=0;idx<AP_MODE_SUPPORT_GROUP_NUM;idx++){
			...
			if(idx==1 && priv->gd){
				curr_group = priv->gd->next;
				log("idx[%d]",idx);
			}else 
			if(idx==2 && priv->gd->next){
				curr_group =  priv->gd->next->next;
				log("idx[%d]",idx);
			}

			if(curr_group==NULL){
				log("create group definition!\n");
				if ((new_group = kmalloc(sizeof(struct group_def_), GFP_ATOMIC)) == NULL) {
					log("fail");
					return -1;
				}
				new_group->next = NULL;
				...
				if(idx==1 && priv->gd){
					priv->gd->next = new_group;
					log("%p",new_group);
				}else
				if(idx==2&&priv->gd->next){
					priv->gd->next->next = new_group;
					log("%p",new_group);
				}
			}
			else{
			...
			memset(new_group, 0, sizeof(struct group_def_));
			if (compute_group_definition(priv,
					new_group, priv->pmib->dot1180211AuthEntry.dot11PassPhrase,idx+19)) {
				log("fail");
			}
			//dump_hex("grp->password",new_group->password, strlen(new_group->password));
			log("group [%d] is configured, prime is %d bytes\n", new_group->group_num, mbedtls_mpi_size(new_group->prime));

		}
	}
	else{
		/*for STA/mesh mode SAE group assigned by dot11SAEGroup*/
		if(priv->gd==NULL){
			log("create group definition!\n");
			if ((new_group = kmalloc(sizeof(struct group_def_), GFP_ATOMIC)) == NULL) {
				log("fail");return -1;
		...
		memset(new_group, 0, sizeof(struct group_def_));
		if (compute_group_definition(priv,
				new_group, priv->pmib->dot1180211AuthEntry.dot11PassPhrase,
				priv->pmib->dot1180211AuthEntry.dot11SAEGroup)) {
			log("fail\n");
		}
		//dump_hex("grp->password",new_group->password, strlen(new_group->password));
		log("group %d is configured, prime is %d bytes", new_group->group_num, mbedtls_mpi_size(new_group->prime));
	}
...
void sae_received_beacon(struct rtl8192cd_priv *priv, unsigned char *sa)
{
	...
	if (assign_group_to_peer(priv, priv->gd, sa) < 0) {
		...
		peer->state = SAE_COMMITTED;
		RESTORE_INT(flags);
		log("peer[%pm] st[%s]", peer->peer_mac , state_to_string(peer->state));
	}
...
enum result sae_received_auth(struct rtl8192cd_priv *priv, unsigned int framelen, unsigned char *sae_frame)
{
	...
	if (peer != NULL) {
		log("peer[%pm] st[%s],seq[%d]", peer->peer_mac, state_to_string(peer->state),ieee_order(frame->authenticate.auth_seq));
	}
	...
	case SAE_AUTH_COMMIT:
		if ((peer != NULL) && (peer->state != SAE_ACCEPTED)) {
			...
			/* check if this is the same scalar that was sent when we accepted */
			if ((peer != NULL) && (peer->state == SAE_ACCEPTED)) {
			...
			log("clogging[%d] Treshold[%d]", priv->curr_open, priv->open_threshold);
			...
			if (!(priv->curr_open < priv->open_threshold)) {
				...
				if (need_token < 0) {
					 ...
					 */
					log("token needed for COMMIT (curr_open=%d )", priv->curr_open);
			 ...
			 */
			peer = create_candidate(priv, frame->sa, GET_MY_HWADDR, (priv->curr_open >= priv->open_threshold));
			if (peer == NULL) {
...
int sae_peer_table_expire(struct rtl8192cd_priv *priv)
{
	...
	for (i = 0; i < tbl_sz; i++) {
		...
		if (GET_ROOT(priv)->sae_peer_table->entry_array[i].dirty) {
			...
			if (priv->gd != NULL &&
				(peer_entry->t1 != 0) &&
				RTL_JIFFIES_TO_SECOND(now - peer_entry->t1) > priv->pshare->rf_ft_var.pmk_expire) {
