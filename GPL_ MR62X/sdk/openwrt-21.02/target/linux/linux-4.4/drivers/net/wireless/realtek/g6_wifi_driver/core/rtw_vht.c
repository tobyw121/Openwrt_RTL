...
const u16 VHT_MCS_DATA_RATE[4][2][30] = {
	{	{
			...
			struct protocol_cap_t *dft_proto_cap, struct role_cap_t *dft_cap)
{
	...
	bHwSTBCSupport = (dft_proto_cap->stbc_vht_tx) ? _TRUE : _FALSE;
	if (bHwSTBCSupport) {
				  ...
				  && !MLME_IS_MESH(padapter))
	{
		...
		mu_bfer = (dft_proto_cap->mu_bfmr) ? _TRUE : _FALSE;
		mu_bfee = (dft_proto_cap->mu_bfme) ? _TRUE : _FALSE;
...
void	rtw_vht_apply_mib_setting(_adapter *padapter)
{
	struct mlme_priv		*pmlmepriv = &padapter->mlmepriv;
	struct vht_priv		*pvhtpriv = &pmlmepriv->vhtpriv;
	struct registry_priv	*pregistrypriv = &padapter->registrypriv;
...
void	rtw_vht_use_default_setting(_adapter *padapter)
{
	...
	if (padapter->phl_role->proto_role_cap.vht_ldpc)
		SET_FLAG(pvhtpriv->ldpc_cap, LDPC_VHT_ENABLE_RX);

	if (padapter->phl_role->cap.tx_vht_ldpc)
		SET_FLAG(pvhtpriv->ldpc_cap, LDPC_VHT_ENABLE_TX);
	...
	/* STBC */
	if (padapter->phl_role->proto_role_cap.stbc_vht_tx)
		SET_FLAG(pvhtpriv->stbc_cap, STBC_VHT_ENABLE_TX);

	if (padapter->phl_role->proto_role_cap.stbc_vht_rx) {
		SET_FLAG(pvhtpriv->stbc_cap, STBC_VHT_ENABLE_RX);
		pvhtpriv->rx_stbc_nss = padapter->phl_role->proto_role_cap.stbc_vht_rx;
	}
	...
	if (padapter->phl_role->proto_role_cap.vht_su_bfmr) {
		SET_FLAG(pvhtpriv->beamform_cap, BEAMFORMING_VHT_BEAMFORMER_ENABLE);
		RTW_INFO("[VHT] HAL Support Beamformer\n");
		if (padapter->phl_role->proto_role_cap.vht_mu_bfmr) {
			SET_FLAG(pvhtpriv->beamform_cap, BEAMFORMING_VHT_MU_MIMO_AP_ENABLE);
			RTW_INFO("[VHT] Support MU-MIMO AP\n");
		}
	}

	if (padapter->phl_role->proto_role_cap.vht_su_bfme) {
		u8 bfme_sts = 0;
		...
		if (padapter->phl_role->proto_role_cap.vht_mu_bfme) {
			SET_FLAG(pvhtpriv->beamform_cap, BEAMFORMING_VHT_MU_MIMO_STA_ENABLE);
			RTW_INFO("[VHT] Support MU-MIMO STA\n");
		}
	}
...
/* ToDo: check bfee flow will move change role type and modify wifi_role->proto_role_cap */
	...
	if (padapter->driver_rx_amsdu_size != 0xF)
		pvhtpriv->max_mpdu_len = padapter->driver_rx_amsdu_size;
	else
		pvhtpriv->max_mpdu_len = padapter->phl_role->proto_role_cap.max_amsdu_len;
...
void update_sta_vht_info_apmode_bf_cap(_adapter *padapter, struct sta_info *psta)
{
	...
	if (TEST_FLAG(pvhtpriv_ap->beamform_cap, BEAMFORMING_VHT_BEAMFORMEE_ENABLE) &&
		...
		psta->phl_sta->asoc_cap.vht_su_bfmr = 1;
	}
	...
	if (TEST_FLAG(pvhtpriv_ap->beamform_cap, BEAMFORMING_VHT_BEAMFORMER_ENABLE) &&
		...
		psta->phl_sta->asoc_cap.vht_su_bfme = 1;
	}


	/* B19 MU Beamformer Capable, the target supports Beamformer and we are Beamformee */
	if (TEST_FLAG(pvhtpriv_ap->beamform_cap, BEAMFORMING_VHT_MU_MIMO_STA_ENABLE) &&
	    GET_VHT_CAPABILITY_ELE_MU_BFER(pvhtpriv_sta->vht_cap)) {
		SET_FLAG(cur_beamform_cap, BEAMFORMING_VHT_MU_MIMO_AP_ENABLE);
		psta->phl_sta->asoc_cap.vht_mu_bfmr = 1;
	}

	/* B20 MU Beamformee Capable, the target supports Beamformee and we are Beamformer */
	if (TEST_FLAG(pvhtpriv_ap->beamform_cap, BEAMFORMING_VHT_MU_MIMO_AP_ENABLE) &&
	    GET_VHT_CAPABILITY_ELE_MU_BFEE(pvhtpriv_sta->vht_cap)) {
		SET_FLAG(cur_beamform_cap, BEAMFORMING_VHT_MU_MIMO_STA_ENABLE);

		psta->phl_sta->asoc_cap.vht_mu_bfme = 1;
	}


	if (cur_beamform_cap)
		RTW_WARN("Current STA(%d) VHT Beamforming Setting = %02X\n", psta->phl_sta->aid, cur_beamform_cap);
...
void	update_sta_vht_info_apmode(_adapter *padapter, void *sta)
{
	...
	pvhtpriv_sta->max_mpdu_len = GET_VHT_CAPABILITY_ELE_MAX_MPDU_LENGTH(pvhtpriv_sta->vht_cap);
	/* B2 B3 Bandwidth */
	...
	/* B8 B9 B10 Rx STBC */
	psta->phl_sta->asoc_cap.stbc_vht_rx = GET_VHT_CAPABILITY_ELE_RX_STBC(pvhtpriv_sta->vht_cap);
	...
	psta->phl_sta->asoc_cap.nss_rx = psta->phl_sta->asoc_cap.nss_tx = rtw_vht_mcsmap_to_nss(psta->vhtpriv.vht_mcs_map);
}
...
void rtw_process_vht_op_mode_notify(_adapter *padapter, u8 *pframe, void *sta)
{
	...
	if (target_bw != psta->phl_sta->chandef.bw) {
		if (rtw_hw_is_band_support(adapter_to_dvobj(padapter), target_bw)
...
u32	rtw_build_vht_operation_ie(_adapter *padapter, u8 *pbuf, u8 channel)
{
	...
	struct vht_priv		*pvhtpriv = &pmlmepriv->vhtpriv;
	/* struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv; */
	u8	ChnlWidth = 0, center_freq = 0, center_freq1 = 0, bw_mode = 0;
	u32	len = 0;
	u8	operation[5];
	...
	if (rtw_hw_chk_bw_cap(adapter_to_dvobj(padapter), BW_CAP_80M | BW_CAP_160M)
	   ...
	   if (bw_mode == BW_CAP_160M){
	   		center_freq = rtw_get_center_ch(channel, CHANNEL_WIDTH_80, CHAN_OFFSET_UPPER);

	   		center_freq1 = rtw_get_center_ch(channel, get_highest_bw_cap(bw_mode), CHAN_OFFSET_UPPER);
			ChnlWidth = 1;

	   }else {
		center_freq = rtw_get_center_ch(channel, get_highest_bw_cap(bw_mode), CHAN_OFFSET_UPPER);
		ChnlWidth = 1;
	   }
	} else {
		center_freq = 0;
	...
	SET_VHT_OPERATION_ELE_CHL_CENTER_FREQ1(operation, center_freq);/* Todo: need to set correct center channel */
	  ...
	  if (bw_mode == BW_CAP_160M)
		SET_VHT_OPERATION_ELE_CHL_CENTER_FREQ2(operation, center_freq1);
	else
...
u32	rtw_build_vht_cap_ie(_adapter *padapter, u8 *pbuf)
{
	...
	if (rtw_hw_chk_bw_cap(adapter_to_dvobj(padapter), BW_CAP_80_80M) && REGSTY_IS_BW_5G_SUPPORT(pregistrypriv, BW_CAP_80_80M))
		SET_VHT_CAPABILITY_ELE_CHL_WIDTH(pcap, 2);
	else if (rtw_hw_chk_bw_cap(adapter_to_dvobj(padapter), BW_CAP_160M) && REGSTY_IS_BW_5G_SUPPORT(pregistrypriv, BW_CAP_160M))
	...
	if (pvhtpriv->sgi_80m && GET_VHT_CAPABILITY_ELE_CHL_WIDTH(pcap))
	...
	if (TEST_FLAG(pvhtpriv->beamform_cap, BEAMFORMING_VHT_BEAMFORMEE_ENABLE)) {
		...
		SET_VHT_CAPABILITY_ELE_BFER_ANT_SUPP(pcap, bfme_sts);
	...
	HighestRate = VHT_MCS_DATA_RATE[bw < 4 ? bw : 0][pvhtpriv->sgi_80m][((pvhtpriv->vht_highest_rate - MGN_VHT1SS_MCS0) & 0x3f)];
	HighestRate = (HighestRate + 1) >> 1;

	SET_VHT_CAPABILITY_ELE_MCS_RX_HIGHEST_RATE(pcap, HighestRate); /* indicate we support highest rx rate is 600Mbps. */
	SET_VHT_CAPABILITY_ELE_MCS_TX_HIGHEST_RATE(pcap, HighestRate); /* indicate we support highest tx rate is 600Mbps. */
	if (bw & CHANNEL_WIDTH_160)
		SET_VHT_CAPABILITY_ELE_MCS_TX_EXTENDED_NSS_BW_CAPABLE(pcap, 1);
...
u32 rtw_restructure_vht_ie(_adapter *padapter, u8 *in_ie, u8 *out_ie, uint in_len, uint *pout_len)
{
...
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
	rtw_vht_apply_mib_setting(padapter);
#endif
	...
	*pout_len += rtw_build_vht_cap_ie(padapter, out_ie + *pout_len);
	...
	if (max_bw >= CHANNEL_WIDTH_40) {
		...
		if (oper_bw == CHANNEL_WIDTH_40) {
			...
			case 1: /* 80/160/80+80 MHz */
				c_freq1 = GET_VHT_OPERATION_ELE_CENTER_FREQ1(vht_op_ie + 2);
				c_freq2 = GET_VHT_OPERATION_ELE_CENTER_FREQ2(vht_op_ie + 2);
	...
	if (oper_bw < CHANNEL_WIDTH_80) {
		...
		SET_VHT_OPERATION_ELE_CHL_WIDTH(out_vht_op_ie + 2, 1);
		SET_VHT_OPERATION_ELE_CHL_CENTER_FREQ1(out_vht_op_ie + 2, cch-8);
		SET_VHT_OPERATION_ELE_CHL_CENTER_FREQ2(out_vht_op_ie + 2, cch);
	} else {
...
void rtw_vht_ies_attach(_adapter *padapter, WLAN_BSSID_EX *pnetwork)
{
	...
	if (pmlmepriv->ext_capab_ie_len < 8)
		pmlmepriv->ext_capab_ie_len = 8;
	...
	if (MLME_IS_AP(padapter)
		...
		if (p && ie_len == VHT_CAP_IE_LEN)
			memcpy(pmlmepriv->vhtpriv.vht_cap, p+2, VHT_CAP_IE_LEN);

		p = rtw_get_ie(pnetwork->IEs + _BEACON_IE_OFFSET_, EID_VHTOperation, &ie_len,
				(pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (p && ie_len == VHT_OP_IE_LEN)
			memcpy(pmlmepriv->vhtpriv.vht_op, p+2, VHT_OP_IE_LEN);
	}
#endif
