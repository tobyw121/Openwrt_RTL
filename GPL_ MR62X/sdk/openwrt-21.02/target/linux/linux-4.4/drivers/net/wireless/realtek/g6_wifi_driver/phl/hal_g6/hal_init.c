...
static enum rtw_hal_status hal_ops_check(struct hal_info_t *hal)
{
	...
	if (!trx_ops->query_tx_res_sw) {
		hal_error_msg("trx query_tx_res_sw");
		status = RTW_HAL_STATUS_FAILURE;
	}
	...
	if (!trx_ops->query_freerun) {
		hal_error_msg("trx query_freerun");
		status = RTW_HAL_STATUS_FAILURE;
	}
...
enum rtw_hal_status hal_bcn_init(struct hal_info_t *hal_info)
{
	...
	_os_spinlock_init(hal_to_drvpriv(hal_info), &bcn_pool->bcn_lock);
...
#if 0/* _TC_ */
extern u8 process_pending_h2c(struct mac_ax_adapter *adapter, u8 seq, u8 from);
enum rtw_hal_status rtw_hal_handle_h2c(struct rtw_phl_com_t *phl_com, void *hal)
{
        struct hal_info_t *hal_info = (struct hal_info_t *)hal;
        struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
        struct rtw_hal_com_t *hal_com = (struct rtw_hal_com_t *)hal_info->hal_com;

        struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;

        process_pending_h2c(mac, 0, 3);
...
enum rtw_hal_status rtw_hal_set_gt3(void *hal, bool count_en, u8 mode, bool gt3_en,
				bool sort_en, u32 timeout)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	struct mac_ax_gt3_cfg cfg;

	cfg.count_en = count_en;
	cfg.mode = mode;
	cfg.gt3_en = gt3_en;
	cfg.sort_en = sort_en;
	cfg.timeout = timeout;

	mac->ops->set_hw_value(mac, MAC_AX_HW_SET_GT3_TIMER, &cfg);
	return RTW_HAL_STATUS_SUCCESS;
}

enum rtw_hal_status rtw_hal_getpkt(struct rtw_phl_com_t *phl_com, void *hal, u8 macid, u8 pkttype)
{
        struct hal_info_t *hal_info = (struct hal_info_t *)hal;
        struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
        ...
        if( !mac->ops->mac_host_getpkt_h2c )
              return RTW_HAL_STATUS_FAILURE;

        return mac->ops->mac_host_getpkt_h2c(mac, macid, pkttype);
}
		...
		u8 bcn_id)
{
	...
	do {
		...
		/* Beacon timing is port-wide setting. Ignore VAP.
		   but configure port for M-BSSID */
		struct rtw_wifi_role_t *wrole = &phl_com->wifi_roles[bcn_entry->bcn_cmn->role_idx];
		if (wrole->type == PHL_RTYPE_VAP) {
			u32	param = 1;
			if (rtw_hal_mac_port_cfg(hal_info, wrole, PCFG_MBSSID_EN, &param))
				PHL_ERR("Failed to enable MBSSID for VAP\n");
			break;
		}
...
enum rtw_hal_status rtw_hal_get_beacon_cnt(struct rtw_phl_com_t *phl_com, void *hal,
		u8 bcn_id, u32 *bcn_ok, u32 *bcn_fail)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
	struct rtw_bcn_entry *bcn_entry = NULL;
	...
	*bcn_ok = bcn_entry->bcn_stats.bcn_ok;
	*bcn_fail = bcn_entry->bcn_stats.bcn_fail;
...
void rtw_hal_dump_beacon_stats(struct rtw_phl_com_t *phl_com, void *hal,
		int reset)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
	struct rtw_bcn_entry *bcn_entry, *bcn_array[CONFIG_LIMITED_AP_NUM] = {0};
	struct rtw_bcn_entry bcn_summary = {{0}};
	int i, bcn_fail[CONFIG_LIMITED_AP_NUM] = {0};
	u32 total_bcn_fail = 0, total_dbg_bcn_tx = 0;

	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		hal_get_bcn_entry(phl_com, hal_info, &bcn_array[i], i);
	}

	if (reset) {
		for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
			bcn_entry = bcn_array[i];
			if (bcn_entry)
				bcn_entry->bcn_stats.ok_cnt =
				bcn_entry->bcn_stats.cca_cnt =
				bcn_entry->bcn_stats.edcca_cnt =
				bcn_entry->bcn_stats.nav_cnt =
				bcn_entry->bcn_stats.txon_cnt =
				bcn_entry->bcn_stats.mac_cnt =
				bcn_entry->bcn_stats.others_cnt =
				bcn_entry->bcn_stats.lock_cnt =
				bcn_entry->bcn_stats.cmp_cnt =
				bcn_entry->bcn_stats.invalid_cnt =
				bcn_entry->bcn_stats.srchend_cnt =
				bcn_entry->bcn_stats.dbg_bcn_tx_cnt = 0;
		}
		return;
	}

	printk(KERN_CONT "ok_cnt       = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.ok_cnt);
			bcn_summary.bcn_stats.ok_cnt += bcn_entry->bcn_stats.ok_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.ok_cnt);

	printk(KERN_CONT "cca_cnt      = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.cca_cnt);
			bcn_summary.bcn_stats.cca_cnt += bcn_entry->bcn_stats.cca_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.cca_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.cca_cnt);

	printk(KERN_CONT "edcca_cnt    = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.edcca_cnt);
			bcn_summary.bcn_stats.edcca_cnt += bcn_entry->bcn_stats.edcca_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.edcca_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.edcca_cnt);

	printk(KERN_CONT "nav_cnt      = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.nav_cnt);
			bcn_summary.bcn_stats.nav_cnt += bcn_entry->bcn_stats.nav_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.nav_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.nav_cnt);

	printk(KERN_CONT "txon_cnt     = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.txon_cnt);
			bcn_summary.bcn_stats.txon_cnt += bcn_entry->bcn_stats.txon_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.txon_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.txon_cnt);

	printk(KERN_CONT "mac_cnt      = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.mac_cnt);
			bcn_summary.bcn_stats.mac_cnt += bcn_entry->bcn_stats.mac_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.mac_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.mac_cnt);

	printk(KERN_CONT "others_cnt   = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.others_cnt);
			bcn_summary.bcn_stats.others_cnt += bcn_entry->bcn_stats.others_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.others_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.others_cnt);

	printk(KERN_CONT "lock_cnt     = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.lock_cnt);
			bcn_summary.bcn_stats.lock_cnt += bcn_entry->bcn_stats.lock_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.lock_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.lock_cnt);

	printk(KERN_CONT "cmp_cnt      = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.cmp_cnt);
			bcn_summary.bcn_stats.cmp_cnt += bcn_entry->bcn_stats.cmp_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.cmp_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.cmp_cnt);

	printk(KERN_CONT "invalid_cnt  = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.invalid_cnt);
			bcn_summary.bcn_stats.invalid_cnt += bcn_entry->bcn_stats.invalid_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.invalid_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.invalid_cnt);

	printk(KERN_CONT "srchend_cnt  = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.srchend_cnt);
			bcn_summary.bcn_stats.srchend_cnt += bcn_entry->bcn_stats.srchend_cnt;
			bcn_fail[i] += bcn_entry->bcn_stats.srchend_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.srchend_cnt);

	printk(KERN_CONT "bcn fail     = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_fail[i]);
			total_bcn_fail += bcn_fail[i];
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	...
	printk(KERN_CONT "dbg_bcn_tx   = ");
	for (i=0; i<CONFIG_LIMITED_AP_NUM; i++) {
		bcn_entry = bcn_array[i];
		if (bcn_entry) {
			printk(KERN_CONT "%6d ", bcn_entry->bcn_stats.dbg_bcn_tx_cnt);
			bcn_summary.bcn_stats.dbg_bcn_tx_cnt += bcn_entry->bcn_stats.dbg_bcn_tx_cnt;
		} else {
			printk(KERN_CONT "%6s", "-");
		}
	}
	printk(KERN_CONT "| %6d \n", bcn_summary.bcn_stats.dbg_bcn_tx_cnt);
	...
	struct rtw_phl_com_t *phl_com, void **hal, enum rtl_ic_id ic_id)
{
	...
	*hal = hal_info;
	...
	hal_status = rtw_hal_flash_init(phl_com, hal_info);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("rtw_hal_flash_init failed\n");
	...
	_os_spinlock_init(hal_to_drvpriv(hal_info), &hal_info->hal_com->h2c_agg_lock);
#endif

#ifdef CONFIG_RTW_DEBUG_BCN_TX
	hal_info->hal_com->dbg_bcn_tx = CONFIG_RTW_DEBUG_BCN_TX;
#endif
...
#ifndef CONFIG_PHL_DRV_HAS_NVM
error_flash_init:
	rtw_hal_flash_deinit(phl_com, hal_info);
#endif
...
enum rtw_hal_status rtw_hal_start(struct rtw_phl_com_t *phl_com, void *hal)
{
...
#if 0 /* Temporarily skip rfe type checking condition */
	...
	/* Clear TX pause flags to ensure TX pause status is clean */
	for (band_idx = 0; band_idx < ARRAY_SIZE(hal_info->hal_com->band); band_idx++) {
		struct rtw_hw_band *band = &hal_info->hal_com->band[band_idx];

		_os_mem_set(hal_to_drvpriv(hal_info), band->tx_pause,
			    0, sizeof(band->tx_pause));

		band->txen_set_time = 0;
		band->txen_final = 0;
		band->txen_mismatch_cnt = 0;
	}

	hal_info->hal_com->is_hal_init = true;
	rtw_hal_set_default_var(hal_info);
	//Temp disable power limit
#ifdef RTW_WKARD_PWRLIMIT_DISABLE
	if(1)
	{
		u32 result, tmp;
		struct rtw_tpu_info *tpu = &hal_info->hal_com->rtw_tpu_i;
		//struct rf_info *rf = (struct rf_info *)hal_info->rf;
		result = rtw_hal_mac_get_pwr_reg(hal_info->hal_com, HW_PHY_0, 0xd200, &tmp);
		if (result) {
			PHL_PRINT("=======>%s Get MAC(0xd200) fail, error code=%d\n",
				__func__, result);
			return false;
		} else
			PHL_PRINT("%s: Get MAC(0xd200) ok!!! 0x%08x\n", __func__, tmp);

	#ifdef CONFIG_RTW_DISABLE_POWER_LMT
		PHL_WARN("power limit is disabled (use power by rate only)\n");
		tmp |= 0x10000000;
	#else
		PHL_WARN("power limit switching is available \n");
		tmp &= 0x1FFFFFFF;
		tpu->pwr_lmt_en = false;
		rtw_hal_mac_write_pwr_limit_en(hal_info->hal_com, HW_PHY_0);

	#endif //CONFIG_RTW_DISABLE_POWER_LMT

		/*Set regulation default to FCC*/
		rtw_hal_rf_set_regulation(hal_info, REGULATION_FCC);
		PHL_PRINT("Set regular_idx:%d (%s) \n", REGULATION_FCC, "REGULATION_FCC");

		result = rtw_hal_mac_set_pwr_reg(hal_info->hal_com, HW_PHY_0, 0xd200, tmp);
		if (result) {
			PHL_PRINT("=======>%s Set MAC(0xd200) fail, error code=%d\n",
				__func__, result);
			return false;
		} else
			PHL_PRINT("Set MAC(0xd200) ok!!! 0x%08x\n", tmp);
	}
#endif /*RTW_WKARD_PWRLIMIT_DISABLE*/
	...
	rtw_hal_enable_rx_amsdu_cut(phl_com, hal_info);
	#endif /* CONFIG_RTW_HW_RX_AMSDU_CUT */

	rtw_hal_mac_set_dbglog_cfg(hal_info, phl_com->dev_cap.mac_dump_cfg);
...
rtw_hal_role_cfg(void *hal, struct rtw_wifi_role_t *wrole)
{
	...
	#ifdef CONFIG_RTW_SUPPORT_MBSSID_VAP
	PHL_PRINT("Config role #%u of type %u. MBSSID: %u.\n",
	          wrole->id, wrole->type, wrole->hw_mbssid);
	if ((wrole->type == PHL_RTYPE_STATION) && wrole->hw_mbssid) {
		u32	param = 0;
		/* Configure port for M-BSSID */
		hal_status = rtw_hal_mac_port_cfg(hal_info, wrole,
...
enum rtw_hal_status
rtw_hal_tx_mode_sel(void *hal, u8 fw_tx, u8 txop_wmm_en_bm)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	return rtw_hal_mac_tx_mode_sel(hal_info, fw_tx, txop_wmm_en_bm);
}
...
void _hal_hw_trx_watchdog(struct rtw_phl_com_t *phl_com, void *hal)
{
	enum rtw_phl_status ret;
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
	int ridx;
	struct rtw_wifi_role_t *wroles = phl_com->wifi_roles;
	int not_to_check = 0;
	int ap_client_cnt = 0;
	u16 *tx_off;

	/* Leave if watchdog disabled, no T/RX watchdog handler available
	 * or HAL is not initialized */
	if (   phl_com->disable_hw_trx_watchdog
	    || (hal_info->trx_ops->trx_watchdog == NULL)
	    || !rtw_hal_is_inited(phl_com, hal))
		return;

	/* Not check if DFS TX pause is in progress. */
	tx_off = hal_com->band[0].tx_pause;
	if (tx_off[PAUSE_RSON_DFS] != 0)
		return;

	/* Not check if any client is connected, except achq stuck check */
	if(!phl_com->check_flag.achq_stuck_check){
		for (ridx = 0; ridx < MAX_WIFI_ROLE_NUMBER; ridx++) {
			struct rtw_wifi_role_t *role = wroles + ridx;
			if (   role->active
				&& IS_AP_ROLE_TYPE(role->type)
				&& (role->mstate == MLME_LINKED)
				&& (role->assoc_sta_queue.cnt > 1)) {
				ap_client_cnt += (role->assoc_sta_queue.cnt - 1);
			}
		}
	}
	...
	/* Trigger L2 to recover if watchdog check failed */
	ret = hal_info->trx_ops->trx_watchdog(phl_com, hal);
	if (ret != RTW_PHL_STATUS_SUCCESS) {
		PHL_ERR("MAC T/RX abnormal. Recovering...\n");
		hal_com->trx_stat.wdg_data.trx_wdg_recovery_cnt++;
		rtw_hal_trigger_bus_err(hal);
	}
}
#endif /* CONFIG_RTW_HW_TRX_WATCHDOG */

enum rtw_hal_status
rtw_hal_watchdog(struct rtw_phl_com_t *phl_com, void *hal)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	#ifdef CONFIG_RTW_TXEN_SYNC_PATCH
	/* Check TXEN status of band 0 to fix unexpected TXEN setting. */
	rtw_hal_check_tx_en_sync(hal, 0);
	#endif /* CONFIG_RTW_TXEN_SYNC_PATCH */

	#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
	_hal_hw_trx_watchdog(phl_com, hal);
	#endif /* CONFIG_RTW_HW_TRX_WATCHDOG */
...
enum rtw_hal_status rtw_hal_set_sw_gpio_mode(struct rtw_phl_com_t *phl_com, void *hal, enum rtw_gpio_mode mode, u8 gpio)
{
	...
	PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_, "%s : status(%u).\n", __func__, hal_status);
...
enum rtw_hal_status rtw_hal_get_sw_gpio_value(struct rtw_phl_com_t *phl_com, void *hal, u8* val, u8 gpio)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	hal_status = rtw_hal_mac_get_sw_gpio_value(hal_info, val, gpio);
	PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_, "%s : status(%u).\n", __func__, hal_status);
...
enum rtw_hal_status
rtw_hal_upd_ss_ul_sta(void *hal, struct rtw_phl_stainfo_t *phl_sta, u8 enable)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	return rtw_hal_mac_upd_ss_ul_sta(hal_info, phl_sta, enable);
}

#ifdef WKARD_DBCC
 enum rtw_hal_status
 rtw_hal_cfg_share_xstal(void *hal, u8 wlan_idx, bool is_share)
 {
	 struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	 enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	u8 band = rtw_phl_get_phyband_on_dev(wlan_idx);

 	if (is_share){
		/* share xstal case */
		if (band == BAND_ON_5G){
			/*WLAN0 0x288[5:4] all 0*/
			rtw_hal_mac_set_xtal_aac(hal_info->hal_com, 0x0);
		} else if (band == BAND_ON_24G){
			/*WLAN1 0x280[23:10] set all 1*/
			rtw_hal_mac_set_xcap(hal_info->hal_com, 0, 0xFF);
			rtw_hal_mac_set_xcap(hal_info->hal_com, 1, 0xFF);
		}
	}else {
		/* single xstal case */
		if (band == BAND_ON_5G){
			/*WLAN0 0x288[5:4] 0x10 for indivisual xstal case*/
			rtw_hal_mac_set_xtal_aac(hal_info->hal_com, 0x2);
		}
	}
...
enum rtw_hal_status
rtw_hal_prepare_fw_init(void *hal, u32 dw0, u32 dw1)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	hal_status = rtw_hal_mac_prepare_fw_init(hal_info, dw0, dw1);
...
#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
__IMEM_WLAN_SECTION__
u32 rtw_hal_get_trx_wdg_fail_cnt(void *hal)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;

	return hal_com->trx_stat.wdg_data.trx_wdg_fail_cnt;
}
#endif
