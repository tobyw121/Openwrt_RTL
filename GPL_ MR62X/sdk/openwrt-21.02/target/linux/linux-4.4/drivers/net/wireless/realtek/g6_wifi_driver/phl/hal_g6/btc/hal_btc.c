...
static u8 _chk_dbcc_2g_link(struct btc_t *btc, struct rtw_chan_def* ch, u8* phy, u8* role, u8* dbcc_2g_phy)
{
	struct btc_wl_info *wl = &btc->cx.wl;
	...
	*dbcc_2g_phy = *(phy+dbcc_2g_cid);
	...
	/* connect_cnt <= 2 */
	if (wl_rinfo->connect_cnt < BTC_TDMA_WLROLE_MAX + 1)
		 return (_get_role_link_mode(btc, (*role+dbcc_2g_cid)));

	/* find the other-port in the 2G-PHY */
	for (k = 0; k < wl_rinfo->connect_cnt; k++) {
		if (k == dbcc_2g_cid) {
			continue;
		} else if (*(phy+k) == *dbcc_2g_phy) {
...
static void _update_wl_info(struct btc_t *btc, u8 rid, enum role_state reason)
{
	...
	u8 dbcc_2g_phy = HW_PHY_MAX, phy_now = 0, link_mode_ori;
	u32 noa_duration = 0;
	...
	for (i = 0; i < MAX_WIFI_ROLE_NUMBER; i++) {
		...
		if (wl_linfo[i].pid >= MAX_WIFI_ROLE_NUMBER)
			continue;

		cnt_active++;

		/* Extract wl_link_info to wl_role_info */
		active_role = &wl_rinfo->active_role[wl_linfo[i].pid];
		...
		if (rid == i)
			phy_now = active_role->phy;

		/* check if role connect? */
		if (wl_linfo[i].connected == MLME_NO_LINK) {
			active_role->connected = 0;
			if (rid == i && active_role->role == PHL_RTYPE_STATION)
				btc->dm.leak_ap = 0;
			continue;
		 ...
		 */
		if (h->dbcc_en) {
			wl_dinfo->role[wl_linfo[i].phy] |= BIT(wl_linfo[i].role);
	...
	if (cnt_connect == 0) {
		...
		wl_rinfo->link_mode = _chk_dbcc_2g_link(btc, cid_ch, cid_phy,
							cid_role, &dbcc_2g_phy);
	...
	if (link_mode_ori != wl_rinfo->link_mode)
		...
		wl_rinfo->link_mode_chg = 0;

	if (h->dbcc_en) {
		if (dbcc_2g_phy == HW_PHY_1)
			_set_wl_req_mac(btc, HW_PHY_1);
		else
