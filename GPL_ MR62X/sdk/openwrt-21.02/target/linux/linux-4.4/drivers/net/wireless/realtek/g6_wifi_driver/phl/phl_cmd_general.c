...
{
	struct phl_cmd_obj *phl_cmd = (struct phl_cmd_obj *)phlcmdbuf;
	return phl_cmd->buf;
}
                ...
                u32 cmd_timeout)
{
	...
	if (evt_id == MSG_EVT_LINUX_CMD_WRK) {
		struct cmd_obj *cmd = (struct cmd_obj *)cmd_buf;
		msg_hdl = cmd->msg_hdl;
	}

	psts = phl_disp_eng_send_msg(phl_info, &msg, &attr, msg_hdl);
