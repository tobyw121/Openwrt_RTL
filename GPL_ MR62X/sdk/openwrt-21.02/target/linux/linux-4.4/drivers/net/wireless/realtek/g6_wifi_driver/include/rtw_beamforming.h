...
#define GET_BEAMFORM_INFO(adapter)	(&GET_HAL_DATA(adapter_to_dvobj(adapter))->beamforming_info)
...
struct beamformer_entry *rtw_bf_bfer_get_entry_by_addr(_adapter *, u8 *ra);
struct beamformee_entry *rtw_bf_bfee_get_entry_by_addr(_adapter *, u8 *ra);
void rtw_bf_get_ndpa_packet(_adapter *, union recv_frame *);
u32 rtw_bf_get_report_packet(_adapter *, union recv_frame *);
u8 rtw_bf_send_vht_gid_mgnt_packet(_adapter *, u8 *ra, u8 *gid, u8 *position);
void rtw_bf_get_vht_gid_mgnt_packet(_adapter *, union recv_frame *);
void rtw_bf_init(_adapter *);
void rtw_bf_cmd_hdl(_adapter *, u8 type, u8 *pbuf);
u8 rtw_bf_cmd(_adapter *, s32 type, u8 *pbuf, s32 size, u8 enqueue);
void rtw_bf_update_attrib(_adapter *, struct pkt_attrib *, struct sta_info *);
void rtw_bf_c2h_handler(_adapter *, u8 id, u8 *buf, u8 buf_len);
void rtw_bf_update_traffic(_adapter *);
