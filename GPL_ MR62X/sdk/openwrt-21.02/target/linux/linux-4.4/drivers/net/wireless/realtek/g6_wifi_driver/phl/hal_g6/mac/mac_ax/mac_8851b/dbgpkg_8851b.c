...
u32 tx_dbg_dump_8851b(struct mac_ax_adapter *adapter)
{
...
u32 crit_dbg_dump_8851b(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
...
u32 cmac_dbg_dump_8851b(struct mac_ax_adapter *adapter, enum mac_ax_band band)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 ret, reg;

	if (band == MAC_AX_BAND_1 && is_chip_id(adapter, MAC_AX_CHIP_ID_8851B))
		return MACSUCCESS;
...
u32 dmac_dbg_dump_8851b(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 ret;
...
u32 ss_stat_chk_8851b(struct mac_ax_adapter *adapter)
{
	u32 val32 = 0;
	u32 r_val32;
	u8 fw_vld;
...
u32 tx_flow_ptcl_dbg_port_8851b(struct mac_ax_adapter *adapter, u8 band)
{
	u8 i;
	u16 val16;
	u32 ret = MACSUCCESS;
...
u32 tx_flow_sch_dbg_port_8851b(struct mac_ax_adapter *adapter, u8 band)
{
	u8 i;
...
u32 dbg_port_sel_8851b(struct mac_ax_adapter *adapter,
...
u32 dbg_port_sel_rst_8851b(struct mac_ax_adapter *adapter, u32 sel)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
u8 is_dbg_port_not_valid_8851b(struct mac_ax_adapter *adapter, u32 dbg_sel)
{
	if (adapter->hw_info->intf != MAC_AX_INTF_PCIE &&
