...
void _tssi_backup_bb_registers_8832brvt(
	struct rf_info *rf,
...
void _tssi_reload_bb_registers_8832brvt(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	u32 *reg,
	u32 *reg_backup,
...
u8 _halrf_ch_to_idx_8832brvt(struct rf_info *rf, u8 channel)
{
	u8	channelIndex;
...
u8 _halrf_idx_to_ch_8832brvt(struct rf_info *rf, u8 idx)
{
	u8 channelIndex;
...
void _halrf_tssi_hw_tx_8832brvt(struct rf_info *rf,
...
void _halrf_tssi_rf_setting_8832brvt(struct rf_info *rf,
...
void _halrf_tssi_set_sys_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
...
void _halrf_tssi_ini_txpwr_ctrl_bb_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
...
void _halrf_tssi_ini_txpwr_ctrl_bb_he_tb_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n", __func__, phy, path);
...
void _halrf_tssi_set_dck_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{	
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
...
void _halrf_tssi_set_bbgain_split_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n", __func__, phy, path);
...
void _halrf_tssi_set_tmeter_tbl_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	...
	if (path == RF_PATH_A) {
		...
		RF_DBG(rf, DBG_RF_TX_PWR_TRACK,
		       "[TSSI] channel=%d thermal_pahtB=0x%x tssi_info->tssi_type[HW_PHY_0]=%d\n",
		       channel, thermal, tssi_info->tssi_type[HW_PHY_0]);
...
void _halrf_tssi_set_tmeter_tbl_zere_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 i;
	u32 thermal_offset_tmp = 0;
	u32 ftable_reg[TSSI_PATH_MAX_8832BRVT] = {0x5c00, 0x7c00};
	u32 ftable_base_reg[TSSI_PATH_MAX_8832BRVT] = {0x5810, 0x7810};
	u32 ftable_trigger_reg[TSSI_PATH_MAX_8832BRVT] = {0x5864, 0x7864};
	s8 thermal_offset[64] = {0};
...
#if 0
void _halrf_tssi_set_dac_gain_tbl_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_slope_cal_org_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	if (path == RF_PATH_A) {
		...
		halrf_wreg(rf, 0x5610, 0x000001ff, 0x8);
		halrf_wreg(rf, 0x5610, 0x07fc0000, 0x8);
		...
		halrf_wreg(rf, 0x5624, 0x000000ff, 0x08);
		halrf_wreg(rf, 0x5624, 0xffff0000, 0x0808);
		halrf_wreg(rf, 0x5628, 0xffffffff, 0x08080808);
		halrf_wreg(rf, 0x562c, 0x0000ffff, 0x0808);

		if (tssi_info->tssi_slope_no_pg && tssi_info->tssi_type[HW_PHY_0] != TSSI_CAL) {
		...
		halrf_wreg(rf, 0x7610, 0x000001ff, 0x8);
		halrf_wreg(rf, 0x7610, 0x07fc0000, 0x8);
		...
		halrf_wreg(rf, 0x7624, 0x000000ff, 0x08);
		halrf_wreg(rf, 0x7624, 0xffff0000, 0x0808);
...
#if 0
void _halrf_tssi_slope_cal_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	u32 tssi_trigger[TSSI_PATH_MAX_8832BRVT] = {0x5820, 0x7820};
	u32 tssi_cw_rpt_addr[TSSI_PATH_MAX_8832BRVT] = {0x1c18, 0x3c18};
	...
	for (j = 0; j < 4; j++) {
		...
		_halrf_tssi_hw_tx_8832brvt(rf, phy, path, 100, 5000, power_high[j], rate, bw, true);
		...
		for (k = 0; halrf_rreg(rf, tssi_cw_rpt_addr[path], BIT(16)) == 0; k++) {
			...
			if (k > 100) {
				...
				RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI PA K] TSSI finish bit k > 100us path=%d\n",
					i);
				_halrf_tssi_hw_tx_8832brvt(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
				return;
			}
		}
		_halrf_tssi_hw_tx_8832brvt(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
		...
		_halrf_tssi_hw_tx_8832brvt(rf, phy, path, 100, 5000, power_low[j], rate, bw, true);
...
void _halrf_tssi_set_alignment_default_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	if (band == BAND_ON_5G) {
		if (path == RF_PATH_A) {
			...
			halrf_wreg(rf, 0x5604, 0x0000003f, 0x21);
			halrf_wreg(rf, 0x5604, 0x00003f00, 0x27);
			halrf_wreg(rf, 0x5604, 0x003f0000, 0x2d);
			...
			if (channel >= 36 && channel <= 64) {
				halrf_wreg(rf, 0x5634, 0x000ffc00, 0x3d8);
				halrf_wreg(rf, 0x5634, 0x3ff00000, 0x3fe);
				halrf_wreg(rf, 0x5638, 0x000003ff, 0x008);
				halrf_wreg(rf, 0x5638, 0x000ffc00, 0x07f);
			} else if (channel >= 100 && channel <= 144) {
				halrf_wreg(rf, 0x5634, 0x000ffc00, 0x3dc);
				halrf_wreg(rf, 0x5634, 0x3ff00000, 0x002);
				halrf_wreg(rf, 0x5638, 0x000003ff, 0x00c);
				halrf_wreg(rf, 0x5638, 0x000ffc00, 0x079);
			} else if (channel >= 149 && channel <= 177) {
				halrf_wreg(rf, 0x5634, 0x000ffc00, 0x3dd);
				halrf_wreg(rf, 0x5634, 0x3ff00000, 0x003);
				halrf_wreg(rf, 0x5638, 0x000003ff, 0x00d);
				halrf_wreg(rf, 0x5638, 0x000ffc00, 0x078);
			}
			...
			halrf_wreg(rf, 0x7604, 0x0000003f, 0x21);
			halrf_wreg(rf, 0x7604, 0x00003f00, 0x27);
			halrf_wreg(rf, 0x7604, 0x003f0000, 0x2d);
			...
			if (channel >= 36 && channel <= 64) {
				halrf_wreg(rf, 0x7634, 0x000ffc00, 0x3d9);
				halrf_wreg(rf, 0x7634, 0x3ff00000, 0x3ff);
				halrf_wreg(rf, 0x7638, 0x000003ff, 0x008);
				halrf_wreg(rf, 0x7638, 0x000ffc00, 0x081);
			} else if (channel >= 100 && channel <= 144) {
				halrf_wreg(rf, 0x7634, 0x000ffc00, 0x3dd);
				halrf_wreg(rf, 0x7634, 0x3ff00000, 0x003);
				halrf_wreg(rf, 0x7638, 0x000003ff, 0x00c);
				halrf_wreg(rf, 0x7638, 0x000ffc00, 0x07d);
			} else if (channel >= 149 && channel <= 177) {
				halrf_wreg(rf, 0x7634, 0x000ffc00, 0x3dd);
				halrf_wreg(rf, 0x7634, 0x3ff00000, 0x003);
				halrf_wreg(rf, 0x7638, 0x000003ff, 0x00c);
				halrf_wreg(rf, 0x7638, 0x000ffc00, 0x07a);
			}
...
#if 0
void _halrf_tssi_alignment_default_8832brvt(struct rf_info *rf,
...
void _halrf_tssi_run_slope_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n",
		__func__, phy, path);
...
void _halrf_tssi_sw_dck_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct rtw_hal_com_t *hal = rf->hal_com;

	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	u32 avg_adc[TSSI_PATH_MAX_8832BRVT][3];
	u32 delay_us = 500;
...
void _halrf_tssi_set_slope_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_set_tssi_track_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_set_txagc_offset_mv_avg_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
u32 _halrf_tssi_get_cck_efuse_group_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
u32 _halrf_tssi_get_ofdm_efuse_group_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
s8 _halrf_tssi_get_ofdm_efuse_tssi_de_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 group_idx;
	...
	group_idx = _halrf_tssi_get_ofdm_efuse_group_8832brvt(rf, phy);
...
s8 _halrf_tssi_get_ofdm_efuse_slope_gain_diff_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 group_idx;
...
s8 _halrf_tssi_get_ofdm_efuse_slope_cw_diff_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 group_idx;
...
u32 _halrf_tssi_get_tssi_trim_efuse_group_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 group_index = 0;
...
s8 _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 group_idx;
	...
	group_idx = _halrf_tssi_get_tssi_trim_efuse_group_8832brvt(rf, phy);
...
bool _halrf_tssi_get_cw_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path,
					s32 power, u32 *power_cw)
{
	u32 i, delay_100us_times = 2000;

	halrf_wreg(rf, 0x56cc | (path << 13), 0x0ff80000, power & 0x1ff);	/*set power*/

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] 0x%x = 0x%x\n",
		0x56cc | (path << 13), halrf_rreg(rf, 0x56cc | (path << 13), 0x0ff80000));
		...
		if (halrf_rreg(rf, 0x1c18 | (path << 13), BIT(16))) {
			*power_cw = halrf_rreg(rf, 0x1c18 | (path << 13), 0x000003ff);
			RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] TSS CW=0x%x  ADC_CW=0x%x\n",
				*power_cw, halrf_rreg(rf, 0x1c0c | (path << 13), 0x00000fff));
			break;
		} else {
			RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] delay time = %dus\n",
				(i + 1) * 100);
					...
					enum phl_phy_idx phy, enum rf_path path)
{
	...
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	s32 power[4] = {76, 44, 0, 0};
	u32 power_cw[2] = {0}, power_cw_tmp[3] = {0}, power_cw_first = 0;
	s32 slopek_cw_diff, tssi_cw_diff;
	u32 bb_reg[2] = {0x5818, 0x7818};
	u32 bb_reg_backup[2] = {0};
	u32 backup_num = 2;

	u32 start_time, finish_time;

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] ======> %s   0x5820=0x%x   0x7820=0x%x\n",
		__func__, halrf_rreg(rf, 0x5820, 0xffffffff), halrf_rreg(rf, 0x7820, 0xffffffff));

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] ======> %s   0x5818=0x%x   0x7818=0x%x\n",
		__func__, halrf_rreg(rf, 0x5818, 0xffffffff), halrf_rreg(rf, 0x7818, 0xffffffff));

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] ======> %s   path=%d\n",
		__func__, path);

	start_time = _os_get_cur_time_us();

	_tssi_backup_bb_registers_8832brvt(rf, phy, bb_reg, bb_reg_backup,
					backup_num);

	halrf_wreg(rf, 0x5818 | (path << 13), 0x000000ff, 0x0);
	...
	halrf_wreg(rf, 0x5684 | (path << 13), 0x007f0000, 0x20);
	...
	halrf_wreg(rf, 0x5814 | (path << 13), 0x00000800, 0x1);			/*wire r_tssi_bypass_tssi_C*/

	/*Pre slope k*/
	if (!_halrf_tssi_get_cw_8832brvt(rf, phy, path, power[0], &power_cw_first))
		goto _fail;

	if (power_cw_first >= 316 && power_cw_first <= 332) {
		halrf_wreg(rf, 0x5818 | (path << 13), 0x000000ff, 0x0);

		RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] 316 <= power_cw_first(%d) <= 332  0x%x[7:0]=0x%x  path=%d\n",
			power_cw_first,
			0x5818 | (path << 13), halrf_rreg(rf, 0x5818 | (path << 13), 0x000000ff),
			path);
	} else if (power_cw_first > 332) {
		tssi_cw_diff = 332 - power_cw_first;
		halrf_wreg(rf, 0x5818 | (path << 13), 0x000000ff, (tssi_cw_diff & 0xff));

		RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] %d - %d = %d(0x%x)  0x%x[7:0]=0x%x  path=%d\n",
			332, power_cw_first, tssi_cw_diff, tssi_cw_diff,
			0x5818 | (path << 13), halrf_rreg(rf, 0x5818 | (path << 13), 0x000000ff),
			path);
	} else if (power_cw_first < 316) {
		tssi_cw_diff = 316 - power_cw_first;
		halrf_wreg(rf, 0x5818 | (path << 13), 0x000000ff, (tssi_cw_diff & 0xff));

		RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] %d - %d = %d(0x%x)  0x%x[7:0]=0x%x  path=%d\n",
			316, power_cw_first, tssi_cw_diff, tssi_cw_diff,
			0x5818 | (path << 13), halrf_rreg(rf, 0x5818 | (path << 13), 0x000000ff),
			path);
	}
	...
	/*1*/
	/*get tssi cw*/
	if (!_halrf_tssi_get_cw_8832brvt(rf, phy, path, power[0], &power_cw_tmp[0]))
		goto _fail;

	/*2*/
	/*get tssi cw*/
	if (!_halrf_tssi_get_cw_8832brvt(rf, phy, path, power[0], &power_cw_tmp[1]))
		goto _fail;

	/*3*/
	/*get tssi cw*/
	if (!_halrf_tssi_get_cw_8832brvt(rf, phy, path, power[0], &power_cw_tmp[2]))
		goto _fail;

	power_cw[0] = (power_cw_tmp[0] + power_cw_tmp[1] + power_cw_tmp[2]) / 3;

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] power_cw[0]=0x%x   path=%d\n",
		power_cw[0], path);

	/*Low power*/
	/*1*/
	/*get tssi cw*/
	if (!_halrf_tssi_get_cw_8832brvt(rf, phy, path, power[1], &power_cw_tmp[0]))
		goto _fail;

	/*2*/
	/*get tssi cw*/
	if (!_halrf_tssi_get_cw_8832brvt(rf, phy, path, power[1], &power_cw_tmp[1]))
		goto _fail;

	/*3*/
	/*get tssi cw*/
	if (!_halrf_tssi_get_cw_8832brvt(rf, phy, path, power[1], &power_cw_tmp[2]))
		goto _fail;

	power_cw[1] = (power_cw_tmp[0] + power_cw_tmp[1] + power_cw_tmp[2]) / 3;

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] power_cw[1]=0x%x   path=%d\n",
		power_cw[1], path);

	slopek_cw_diff = power_cw[0] - power_cw[1];
	tssi_info->slopek_cw_diff[path] = slopek_cw_diff;

	halrf_wreg(rf, 0x5624 | (path << 13), 0x0000ff00, 0x40);

#if 0
	if (slopek_cw_diff <= 53)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x34);
	else if (slopek_cw_diff >= 54 && slopek_cw_diff <= 56)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x37);
	else if (slopek_cw_diff >= 57 && slopek_cw_diff <= 59)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x3a);
	else if (slopek_cw_diff >= 60 && slopek_cw_diff <= 62)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x3d);
	else if (slopek_cw_diff >= 63 && slopek_cw_diff <= 65)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x40);
	else if (slopek_cw_diff >= 66 && slopek_cw_diff <= 68)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x43);
	else if (slopek_cw_diff >= 69 && slopek_cw_diff <= 71)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x46);
	else if (slopek_cw_diff >= 72 && slopek_cw_diff <= 74)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x49);
	else if (slopek_cw_diff >= 75 && slopek_cw_diff <= 77)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x4c);
	else if (slopek_cw_diff >= 78 && slopek_cw_diff <= 80)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x4f);
	else if (slopek_cw_diff >= 81 && slopek_cw_diff <= 83)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x52);
	else if (slopek_cw_diff >= 84 && slopek_cw_diff <= 86)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x55);
	else if (slopek_cw_diff >= 87 && slopek_cw_diff <= 89)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x58);
	else if (slopek_cw_diff >= 90 && slopek_cw_diff <= 92)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x5b);
	else if (slopek_cw_diff >= 93 && slopek_cw_diff <= 95)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x5e);
	else if (slopek_cw_diff >= 96)
		halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, 0x61);
#else
	halrf_wreg(rf, 0x5610 | (path << 13), 0x0003fe00, slopek_cw_diff);
#endif

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] slopek_cw_diff=0x%x   0x%x=0x%x   0x%x=0x%x\n",
		slopek_cw_diff,
		0x5624 | (path << 13), halrf_rreg(rf, 0x5624 | (path << 13), 0x0000ff00),
		0x5610 | (path << 13), halrf_rreg(rf, 0x5610 | (path << 13), 0x0003fe00));
	
	/*4_enable_slope_cal_reg*/
#if 0
	if (rf->tssi_slope_type[path] == TSSI_SLOPE_DEFAULT) {
		if (slopek_cw_diff >= 75 || slopek_cw_diff <= 56)
			halrf_wreg(rf, 0x581c | (path << 13), 0x00100000, 0x1);
		else
			halrf_wreg(rf, 0x581c | (path << 13), 0x00100000, 0x0);
	} else if (rf->tssi_slope_type[path] == TSSI_SLOPE_ON)
		halrf_wreg(rf, 0x581c | (path << 13), 0x00100000, 0x1);
...
_fail:
	...
	_tssi_reload_bb_registers_8832brvt(rf, phy, bb_reg, bb_reg_backup,
					backup_num);

	finish_time = _os_get_cur_time_us();
	tssi_info->tssi_slope_time = tssi_info->tssi_slope_time + (finish_time - start_time);

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Slope K] %s processing time = %d.%dms path=%d\n",
		__func__,
...
void _halrf_tssi_alimentk_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	u32 tssi_cw_default_addr[TSSI_PATH_MAX_8832BRVT][4] =
	...
	for (j = 0; j < 2; j++) {
		...
		_halrf_tssi_hw_tx_8832brvt(rf, phy, path, 100, 5000, power[j], rate, bw, true);
		...
		for (k = 0; halrf_rreg(rf, tssi_cw_rpt_addr[path], BIT(16)) == 0; k++) {
			...
			if (k > retry) {
				...
				_tssi_reload_bb_registers_8832brvt(rf, phy, bb_reg, bb_reg_backup,
					backup_num);
				_halrf_tssi_hw_tx_8832brvt(rf, phy, path, 100, 5000, power[j], rate, bw, false);
...
void _halrf_tssi_alimentk_done_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
#if 0
	struct halrf_tssi_info *tssi_info = &rf->tssi;
...
void halrf_tssi_backup_txagc_8832brvt(struct rf_info *rf, enum phl_phy_idx phy, bool enable)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	s16 xdbm;
	u32 i, j, tx_couter = 0;
	u8 ch_idx = _halrf_ch_to_idx_8832brvt(rf, channel);
	u8 channel_tmp;
	...
	_halrf_tssi_hw_tx_8832brvt(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, T_HT_MF, 0, true);

#if 0
	/*Path B*/
	tx_couter[RF_PATH_B] = halrf_rreg(rf, 0x1a40, 0xffff);
	
	_halrf_tssi_hw_tx_8832brvt(rf, phy, RF_PATH_B, 1000, 100, xdbm, T_HT_MF, 0, true);
	halrf_delay_ms(rf, 15);
	_halrf_tssi_hw_tx_8832brvt(rf, phy, RF_PATH_B, 1000, 100, xdbm, T_HT_MF, 0, false);
#endif
	...
	tssi_info->backup_txagc_oft_ther[RF_PATH_A][ch_idx] =
		halrf_get_thermal_8832brvt(rf, RF_PATH_A);

	tssi_info->backup_txagc_oft_ther[RF_PATH_B][ch_idx] =
		halrf_get_thermal_8832brvt(rf, RF_PATH_B);

	_halrf_tssi_hw_tx_8832brvt(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, T_HT_MF, 0, false);
...
#endif
	//_tssi_reload_bb_registers_8832brvt(rf, phy, bb_reg, bb_reg_backup, backup_num);
}

void halrf_do_tssi_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	tssi_info->tssi_slope_time = 0;
	tssi_info->high_pwr_rst_cnt = 0;
	tssi_info->check_backup_txagc[ch_idx] = false;

	halrf_tssi_disable_8832brvt(rf, phy);

	for (i = RF_PATH_A; i < TSSI_PATH_MAX_8832BRVT; i++) {
		/*_halrf_tssi_rf_setting_8832brvt(rf, phy, i);*/
		_halrf_tssi_set_sys_8832brvt(rf, phy, i);
		_halrf_tssi_ini_txpwr_ctrl_bb_8832brvt(rf, phy, i);
		_halrf_tssi_ini_txpwr_ctrl_bb_he_tb_8832brvt(rf, phy, i);
		_halrf_tssi_set_dck_8832brvt(rf, phy, i);
		_halrf_tssi_set_bbgain_split_8832brvt(rf, phy, i);
		_halrf_tssi_set_tmeter_tbl_8832brvt(rf, phy, i);
		/*_halrf_tssi_set_dac_gain_tbl_8832brvt(rf, phy, i);*/
		_halrf_tssi_slope_cal_org_8832brvt(rf, phy, i);
		_halrf_tssi_set_alignment_default_8832brvt(rf, phy, i);
		_halrf_tssi_set_slope_8832brvt(rf, phy, i);
		_halrf_tssi_run_slope_8832brvt(rf, phy, i);

		//_halrf_tssi_slope_cal_8832brvt(rf, phy, i);
		halrf_tssi_set_efuse_to_slope_8832brvt(rf, phy);

		//halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_START);
		//halrf_tmac_tx_pause(rf, phy, true);
		//_halrf_tssi_alimentk_8832brvt(rf, phy, i);
		//halrf_tmac_tx_pause(rf, phy, false);
		//halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_STOP);
	}
	...
	if (phl_is_mp_mode(rf->phl_com)) {
		if (tssi_info->tssi_type[HW_PHY_0] == TSSI_ON || tssi_info->tssi_type[HW_PHY_0] == TSSI_CAL) {
			halrf_tssi_enable_8832brvt(rf, phy);
			halrf_tssi_set_efuse_to_de_8832brvt(rf, phy);
		} else
			halrf_tssi_disable_8832brvt(rf, phy);
	} else {
		if (rf->support_ability & HAL_RF_TX_PWR_TRACK) {
			halrf_tssi_enable_8832brvt(rf, phy);
			halrf_tssi_set_efuse_to_de_8832brvt(rf, phy);
			/*halrf_tssi_backup_txagc_8832brvt(rf, phy, true);*/
		}
	}
}

void halrf_do_tssi_scan_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy)
{
//	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 i;
	...
	for (i = RF_PATH_A; i < TSSI_PATH_MAX_8832BRVT; i++) {
		...
		if (tssi_info->alignment_done[i][band] == true)
			_halrf_tssi_alimentk_done_8832brvt(rf, phy, i);
		else {
			if (rf->hal_com->cv == CAV)
				_halrf_tssi_alignment_default_8832brvta(rf, phy, i);
			else
				_halrf_tssi_alignment_default_8832brvtb(rf, phy, i, false);
		}
#endif
	}
...
void halrf_tssi_get_efuse_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	s8 flash_tmp = 0;
	...
	for (i = EFUSE_INFO_RF_2G_CCK_A_TSSI_DE_1, j = 0; i <= EFUSE_INFO_RF_2G_CCK_A_TSSI_DE_6; i++, j++) {
		...
		tssi_info->tssi_efuse[RF_PATH_A][EFUSE_TSSI_CCK][j] = flash_tmp;
	...
	for (i = EFUSE_INFO_RF_2G_BW40M_A_TSSI_DE_1, j = 0; i <= EFUSE_INFO_RF_5G_BW40M_A_TSSI_DE_14; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		tssi_info->tssi_efuse[RF_PATH_A][EFUSE_TSSI_MCS][j] = flash_tmp;
	...
	for (i = FLASH_INFO_RF_2G_CCK_TSSI_GAIN_DIFF_A_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_2G_CCK_TSSI_GAIN_DIFF_A_8832BRVT_6; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
	...
	for (i = FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_A_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_TSSI_GAIN_DIFF_A_8832BRVT_14; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		tssi_info->tssi_efuse_slope_gain_diff[RF_PATH_A][EFUSE_TSSI_MCS][j] = flash_tmp;
		if (i <= FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_A_8832BRVT_5 && flash_tmp == 0x0)
			no_pg_count_2g++;
		else if (i > FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_A_8832BRVT_5 && flash_tmp == 0x0)
			no_pg_count_5g++;
	...
	for (i = FLASH_INFO_RF_2G_CCK_TSSI_CW_DIFF_A_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_2G_CCK_TSSI_CW_DIFF_A_8832BRVT_6; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
	...
	for (i = FLASH_INFO_RF_2G_TSSI_CW_DIFF_A_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_TSSI_CW_DIFF_A_8832BRVT_14; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		tssi_info->tssi_efuse_slope_cw_diff[RF_PATH_A][EFUSE_TSSI_MCS][j] = flash_tmp;
		if (i <= FLASH_INFO_RF_2G_TSSI_CW_DIFF_A_8832BRVT_5 && flash_tmp == 0x0)
			no_pg_count_2g++;
		else if (i > FLASH_INFO_RF_2G_TSSI_CW_DIFF_A_8832BRVT_5 && flash_tmp == 0x0)
			no_pg_count_5g++;
	...
	for (i = EFUSE_INFO_RF_2G_CCK_B_TSSI_DE_1, j = 0; i <= EFUSE_INFO_RF_2G_CCK_B_TSSI_DE_6; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		tssi_info->tssi_efuse[RF_PATH_B][EFUSE_TSSI_CCK][j] = flash_tmp;
	...
	for (i = EFUSE_INFO_RF_2G_BW40M_B_TSSI_DE_1, j = 0; i <= EFUSE_INFO_RF_5G_BW40M_B_TSSI_DE_14; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		tssi_info->tssi_efuse[RF_PATH_B][EFUSE_TSSI_MCS][j] = flash_tmp;
	...
	for (i = FLASH_INFO_RF_2G_CCK_TSSI_GAIN_DIFF_B_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_2G_CCK_TSSI_GAIN_DIFF_B_8832BRVT_6; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
	...
	for (i = FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_B_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_TSSI_GAIN_DIFF_B_8832BRVT_14; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		tssi_info->tssi_efuse_slope_gain_diff[RF_PATH_B][EFUSE_TSSI_MCS][j] = flash_tmp;
		if (i <= FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_B_8832BRVT_5 && flash_tmp == 0x0)
			no_pg_count_2g++;
		else if (i > FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_B_8832BRVT_5 && flash_tmp == 0x0)
			no_pg_count_5g++;
	...
	for (i = FLASH_INFO_RF_2G_CCK_TSSI_CW_DIFF_B_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_2G_CCK_TSSI_CW_DIFF_B_8832BRVT_6; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
	...
	for (i = FLASH_INFO_RF_2G_TSSI_CW_DIFF_B_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_TSSI_CW_DIFF_B_8832BRVT_14; i++, j++) {
		halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		tssi_info->tssi_efuse_slope_cw_diff[RF_PATH_B][EFUSE_TSSI_MCS][j] = flash_tmp;
		if (i <= FLASH_INFO_RF_2G_TSSI_CW_DIFF_B_8832BRVT_5 && flash_tmp == 0x0)
			no_pg_count_2g++;
		else if (i > FLASH_INFO_RF_2G_TSSI_CW_DIFF_B_8832BRVT_5 && flash_tmp == 0x0)
			no_pg_count_5g++;
	...
	if (no_pg_count_5g == 56)
		...
		tssi_info->tssi_slope_no_pg = false;

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI][Efuse] no_pg_count_2g=%d no_pg_count_5g=%d tssi_info->tssi_slope_no_pg=%d\n",
		no_pg_count_2g, no_pg_count_5g, tssi_info->tssi_slope_no_pg);
}

bool halrf_tssi_check_efuse_data_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 i, j, countr = 0;
	...
	halrf_tssi_get_efuse_8832brvt(rf, phy);
...
void halrf_set_tssi_de_for_tx_verify_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, u32 tssi_de, u8 path)
{
	...
	tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy, path);
...
void halrf_set_tssi_de_offset_8832brvt(struct rf_info *rf,
...
void halrf_set_tssi_de_offset_zero_8832brvt(struct rf_info *rf,
							enum phl_phy_idx phy)
{
...
void halrf_tssi_enable_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	u8 i;
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s\n", __func__);

	for (i = 0; i < TSSI_PATH_MAX_8832BRVT; i++) {
		/*_halrf_tssi_set_tssi_slope_8832brvt(rf, phy, i);*/
		_halrf_tssi_set_tssi_track_8832brvt(rf, phy, i);
		_halrf_tssi_set_txagc_offset_mv_avg_8832brvt(rf, phy, i);
...
void halrf_tssi_disable_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy)
{
...
s32 halrf_get_online_tssi_de_8832brvt(struct rf_info *rf, enum phl_phy_idx phy_idx,
				u8 path, s32 dbm, s32 puot)
{
	...
	tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy_idx, path);
...
void halrf_tssi_set_efuse_to_de_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	if (tssi_info->tssi_type[HW_PHY_0] == TSSI_CAL) {
		/*halrf_set_tssi_de_offset_zero_8832brvt(rf, phy);*/
		RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======> %s TSSI_CAL mode set TSSI Trim !!!\n", __func__);

		for (i = 0; i < TSSI_PATH_MAX_8832BRVT; i++) {
			tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy, i);
	...
	for (i = 0; i < TSSI_PATH_MAX_8832BRVT; i++) {
		/*CCK*/
		group_idx = _halrf_tssi_get_cck_efuse_group_8832brvt(rf, phy);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy, i);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8832brvt(rf, phy, i);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy, i);
...
void halrf_tssi_set_efuse_to_slope_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	for (i = 0; i < TSSI_PATH_MAX_8832BRVT; i++) {
		...
		/*OFDM*/
		ofmd_gain_diff = _halrf_tssi_get_ofdm_efuse_slope_gain_diff_8832brvt(rf, phy, i);
		ofmd_cw_diff = _halrf_tssi_get_ofdm_efuse_slope_cw_diff_8832brvt(rf, phy, i);
...
void halrf_tssi_default_txagc_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, bool enable)
{
	...
	if (enable == true) {
		...
		_halrf_tssi_alimentk_done_8832brvt(rf, phy, RF_PATH_A);
		_halrf_tssi_alimentk_done_8832brvt(rf, phy, RF_PATH_B);
...
void halrf_tssi_scan_ch_8832brvt(struct rf_info *rf, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	if (!phl_is_mp_mode(rf->phl_com)) {
		...
		if (tssi_info->alignment_done[path][band] == true)
			_halrf_tssi_alimentk_done_8832brvt(rf, 0, path);
		else {
#if 0
			if (rf->hal_com->cv == CAV)
				_halrf_tssi_alignment_default_8832brvta(rf, 0, path);
			else
				_halrf_tssi_alignment_default_8832brvtb(rf, 0, path, false);
#endif
		}
...
u32 halrf_tssi_get_final_8832brvt(struct rf_info *rf, enum rf_path path)
{
	u32 i;
...
void halrf_tssi_hw_tx_8832brvt(struct rf_info *rf,
			enum phl_phy_idx phy, u8 path, u16 cnt, s16 dbm, u32 rate, u8 bw,
			bool enable)
{
	_halrf_tssi_hw_tx_8832brvt(rf, phy, path, cnt, 5000, dbm, T_HT_MF, 0, enable);
}

void halrf_tssi_ant_open_8832brvt(struct rf_info *rf)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	if ((halrf_rreg(rf, 0x1c60, 0xff000000) == 0x7f) ||
		...
		halrf_tssi_disable_8832brvt(rf, HW_PHY_0);
		halrf_tssi_enable_8832brvt(rf, HW_PHY_0);
	}
}

void halrf_tssi_slope_onoff_8832brvt(struct rf_info *rf, u8 tssi_slope_type)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 path;
	...
	for (path = RF_PATH_A; path < TSSI_PATH_MAX_8832BRVT; path++) {
	...
	for (path = RF_PATH_A; path < TSSI_PATH_MAX_8832BRVT; path++) {
		rf->tssi_slope_type[path] = tssi_slope_type;
		if (rf->tssi_slope_type[path] == TSSI_SLOPE_DEFAULT) {
			if (tssi_info->slopek_cw_diff[path] >= 75 || tssi_info->slopek_cw_diff[path] <= 56)
				halrf_wreg(rf, 0x581c | (path << 13), 0x00100000, 0x1);
			else
				halrf_wreg(rf, 0x581c | (path << 13), 0x00100000, 0x0);
		} else if (rf->tssi_slope_type[path] == TSSI_SLOPE_ON)
...
void halrf_get_tssi_info_8832brvt(struct rf_info *rf,
		char input[][16], u32 *_used, char *output, u32 *_out_len)
{
	...
	s32 delta_tssi;
	s32 diff_ther;
	s32 int_tmp[TSSI_PATH_MAX_8832BRVT], float_tmp[TSSI_PATH_MAX_8832BRVT];
	s8 txagc_offset[TSSI_PATH_MAX_8832BRVT] = {0};
...
#if 0
	RF_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used, " %-25s = %d.%03d / %d.%03d\n",
		"Org Slope A / B", 
		tssi_info->slopek_cw_diff[RF_PATH_A] / 8,
		((tssi_info->slopek_cw_diff[RF_PATH_A] * 1000) / 8) % 1000,
		tssi_info->slopek_cw_diff[RF_PATH_B] / 8,
		((tssi_info->slopek_cw_diff[RF_PATH_B] * 1000) / 8) % 1000
		);
...
/*MP sloep K*/
void halrf_tssi_do_slope_8832brvt(struct rf_info *rf)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
...
/*MP sloep K*/
u32 halrf_tssi_get_cw_8832brvt(struct rf_info *rf, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
...
/*MP sloep K*/
void halrf_tssi_slope_apply_8832brvt(struct rf_info *rf,
...
/*MP sloep K*/
void halrf_tssi_current_slope_apply_8832brvt(struct rf_info *rf,
...
/*MP sloep K*/
void halrf_tssi_finish_slope_8832brvt(struct rf_info *rf)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
