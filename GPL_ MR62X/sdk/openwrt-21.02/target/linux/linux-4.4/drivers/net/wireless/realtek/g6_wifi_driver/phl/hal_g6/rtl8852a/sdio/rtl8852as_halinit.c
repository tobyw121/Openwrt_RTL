 ...
 *
 *****************************************************************************/
#define _RTL8852AS_HALINIT_C_
#include "../../hal_headers.h"
#include "../rtl8852a_hal.h"
#include "rtl8852as_halinit.h"

static void _hal_pre_init_8852as(struct rtw_phl_com_t *phl_com,
				 struct hal_info_t *hal_info,
				 struct hal_init_info_t *init_52as)
{
	struct mac_ax_trx_info *trx_info = &init_52as->trx_info;
	struct mac_ax_host_rpr_cfg *rpr_cfg = (struct mac_ax_host_rpr_cfg *)hal_info->rpr_cfg;
	/*struct mac_ax_intf_info *intf_info = &init_52as->intf_info;*/
	...
	init_52as->ic_name = "rtl8852as";
}

void init_hal_spec_8852as(struct rtw_phl_com_t *phl_com,
...
enum rtw_hal_status hal_get_efuse_8852as(struct rtw_phl_com_t *phl_com,
					 struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_52as;

	_os_mem_set(hal_to_drvpriv(hal_info), &init_52as, 0, sizeof(init_52as));
	_hal_pre_init_8852as(phl_com, hal_info, &init_52as);

	return hal_get_efuse_8852a(phl_com, hal_info, &init_52as);
}

enum rtw_hal_status hal_init_8852as(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
...
void hal_deinit_8852as(struct rtw_phl_com_t *phl_com,
		       struct hal_info_t *hal_info)
{
	/* free memory for hal */
...
enum rtw_hal_status hal_start_8852as(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_52as;
	...
	return hal_start_8852a(phl_com, hal_info, &init_52as);
}

enum rtw_hal_status hal_stop_8852as(struct rtw_phl_com_t *phl_com,
				      struct hal_info_t *hal)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	...
	if (_SUCCESS == rtl8852as_fw_ips_deinit(adapter))
		return _SUCCESS;
	#endif

	return hal_deinit_rtl8852a(adapter);
#endif
	return hal_status;
}
...
#ifdef CONFIG_WOWLAN
enum rtw_hal_status
hal_wow_init_8852as(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
						struct rtw_phl_stainfo_t *sta)
{
	...
	*/
	...
	return hal_wow_init_8852a(phl_com, hal_info, sta, &init_52as);
}

enum rtw_hal_status
hal_wow_deinit_8852as(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
							struct rtw_phl_stainfo_t *sta)
{
	struct hal_init_info_t init_52as;
	struct mac_ax_trx_info *trx_info = &init_52as.trx_info;
	...
	*/
	...
	return hal_wow_deinit_8852a(phl_com, hal_info, sta, &init_52as);
}
#endif /* CONFIG_WOWLAN */


void init_default_value_8852as(struct hal_info_t *hal)
{
	init_default_value_8852a(hal);

	init_int_default_value_8852as(hal, INT_SET_OPT_HAL_INIT);
}

void init_int_default_value_8852as(struct hal_info_t *hal, enum rtw_hal_int_set_opt opt)
{
...
u32 hal_hci_cfg_8852as(struct rtw_phl_com_t *phl_com,
...
static void _hal_config_int_8852as(struct hal_info_t *hal, enum rtw_hal_config_int hal_int_mode)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_config_int_8852as(struct hal_info_t *hal, enum rtw_phl_config_int int_mode)
{
	...
	if (hal_int_mode != RTW_HAL_CONFIG_INT_MAX)
		_hal_config_int_8852as(hal, hal_int_mode);
}

void hal_enable_int_8852as(struct hal_info_t *hal)
{
	_hal_config_int_8852as(hal, RTW_HAL_EN_DEFAULT_INT);
}

void hal_disable_int_8852as(struct hal_info_t *hal)
{
	_hal_config_int_8852as(hal, RTW_HAL_DIS_DEFAULT_INT);
}

bool hal_recognize_int_8852as(struct hal_info_t *hal)
{
	return true;
}

bool hal_recognize_halt_c2h_int_8852as(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_clear_interrupt_8852as(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u32 hisr; /* SDIO hisr */
...
u32 hal_int_hdler_8852as(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u32 hisr; /* SDIO hisr */
	...
	if (hisr & B_AX_RX_REQUEST_INT_EN){
		...
		_hal_config_int_8852as(hal, RTW_HAL_STOP_RX_INT);
		phl_int |= BIT1;
	}
	...
	if (hisr & B_AX_SDIO_HS0ISR_IND_EN) {
		_hal_config_int_8852as(hal, RTW_HAL_DIS_HCI_INT);
		phl_int |= BIT6;
	}
...
enum rtw_hal_status
hal_mp_init_8852as(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_52as;
	...
	hal_status = hal_mp_init_8852a(phl_com, hal_info, &init_52as);
...
enum rtw_hal_status
hal_mp_deinit_8852as(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_52as;
	...
	hal_status = hal_mp_deinit_8852a(phl_com, hal_info, &init_52as);

	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_mp_deinit_8852as: status = %u\n", hal_status);
		return hal_status;
	}
