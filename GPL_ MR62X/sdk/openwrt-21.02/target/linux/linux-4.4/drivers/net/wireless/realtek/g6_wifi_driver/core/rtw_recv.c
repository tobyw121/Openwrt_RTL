	...
	if (MLME_IS_STA(adapter)
		...
		if (_rtw_memcmp(pattrib->bssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
			...
			if (!bmcast
				...
				RTW_INFO(ADPT_FMT" -issue_deauth_g6 to the nonassociated ap=" MAC_FMT " for the reason(7)\n", ADPT_ARG(adapter), MAC_ARG(pattrib->bssid));
				issue_deauth_g6(adapter, pattrib->bssid, WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
			}
#endif
			ret = _FAIL;
			goto exit;
			...
			*psta = rtw_get_stainfo(pstapriv, pattrib->ta);
			if (*psta == NULL) {
				...
				if (rtw_get_passing_time_ms(send_issue_deauth_time) > 10000 || send_issue_deauth_time == 0) {
					...
					RTW_INFO("issue_deauth_g6 to the ap=" MAC_FMT " for the reason(7)\n", MAC_ARG(pattrib->bssid));
...
#ifndef CONFIG_CUSTOMER_ALIBABA_GENERAL
				RTW_INFO("issue_deauth_g6 to sta=" MAC_FMT " for the reason(7)\n", MAC_ARG(pattrib->src));
				issue_deauth_g6(adapter, pattrib->src, WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
#endif
			}
...
*/
	if (3 == rattrib->to_fr_ds) {
		if (MLME_IS_AP(adapter))
			...
			if(RTW_RX_MSDU_ACT_NONE == act && a4_l2uf_pkt_check(prframe)){
