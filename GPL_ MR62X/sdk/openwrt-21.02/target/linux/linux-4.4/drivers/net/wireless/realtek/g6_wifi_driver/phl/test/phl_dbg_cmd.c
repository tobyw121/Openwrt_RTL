...
static void _phl_set_trx_check_flag(struct phl_info_t *phl_info, u32 bitmap)
{
	struct rtw_phl_com_t *phl_com;
	phl_com = phl_info->phl_com;
	if(bitmap & RXQ_STUCK_CHECK){
		PHL_PRINT("Set rxq stuck check.\n");
		phl_com->check_flag.rxq_stuck_check = 1;
	}
	if(bitmap & ACHQ_STUCK_CHECK){
		PHL_PRINT("Set achq stuck check.\n");
		phl_com->check_flag.achq_stuck_check = 1;
	}
	if(bitmap & MGNTQ_STUCK_CHECK){
		PHL_PRINT("Set mgntq stuck check.\n");
		phl_com->check_flag.mgntq_stuck_check = 1;
	}
	if(bitmap & HIQ_STUCK_CHECK){
		PHL_PRINT("Set hiq stuck check.\n");
		phl_com->check_flag.hiq_stuck_check = 1;
	}
	if(bitmap & H2CQ_STUCK_CHECK){
		PHL_PRINT("Set h2cq stuck check.\n");
		phl_com->check_flag.h2cq_stuck_check = 1;
	}
	if(bitmap & RX_CCK_OFDM_CNT_CHECK){
		PHL_PRINT("Set rx cck ofdm cnt check.\n");
		phl_com->check_flag.rx_cck_ofdm_cnt_check = 1;
	}
	if(bitmap & RX_DATA_CNT_CHECK){
		PHL_PRINT("Set rx data cnt check.\n");
		phl_com->check_flag.rx_data_cnt_check = 1;
	}
	if(bitmap & TX_CCK_OFDM_CNT_CHECK){
		PHL_PRINT("Set tx cck ofdm cnt check.\n");
		phl_com->check_flag.tx_cck_ofdm_cnt_check = 1;
	}
	return;
}
               		      ...
               		      u32 input_num, char *output, u32 out_len)
{
	...
	if (input_num == 1) {
		...
		PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
						 "\tbit0(( %s ))RXQ STUCK CHECK\n"
						 "\tbit1(( %s ))ACHQ STUCK CHECK\n"
						 "\tbit2(( %s ))MGNTQ STUCK CHECK\n"
						 "\tbit3(( %s ))HIQ STUCK CHECK\n"
						 "\tbit4(( %s ))H2CQ STUCK CHECK\n"
						 "\tbit5(( %s ))RX CCK OFDM CNT CHECK\n"
						 "\tbit6(( %s ))RX DATA CNT CHECK\n"
						 "\tbit7(( %s ))TX CCK OFDM CNT CHECK\n"
						 , (phl_com->check_flag.rxq_stuck_check ? "V" : ".")
						 , (phl_com->check_flag.achq_stuck_check ? "V" : ".")
						 , (phl_com->check_flag.mgntq_stuck_check ? "V" : ".")
						 , (phl_com->check_flag.hiq_stuck_check ? "V" : ".")
						 , (phl_com->check_flag.h2cq_stuck_check ? "V" : ".")
						 , (phl_com->check_flag.rx_cck_ofdm_cnt_check ? "V" : ".")
						 , (phl_com->check_flag.rx_data_cnt_check ? "V" : ".")
						 , (phl_com->check_flag.tx_cck_ofdm_cnt_check ? "V" : ".")
						);
	...
	} else if(!_os_strcmp(input[1], "check_flag")){
			_get_hex_from_string(input[2], &bitmap);
			_phl_set_trx_check_flag(phl_info, bitmap);
	}
