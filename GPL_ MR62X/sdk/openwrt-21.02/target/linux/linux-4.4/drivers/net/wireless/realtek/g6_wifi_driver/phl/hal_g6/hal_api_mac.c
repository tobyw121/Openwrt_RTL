...
static void hal_mac_mutex_init(void *h, mac_ax_mutex *mutex)
{
	...
	_os_spinlock_init(hal->drv_priv, (_os_lock *)mutex);
#else
...
static void hal_mac_mutex_deinit(void *h, mac_ax_mutex *mutex)
{
	struct rtw_hal_com_t *hal = (struct rtw_hal_com_t *)h;
#ifdef CONFIG_RTW_HALMAC_USE_SPINLOCK
	_os_spinlock_free(hal->drv_priv, (_os_lock *)mutex);
#else
...
static void hal_mac_mutex_lock(void *h, mac_ax_mutex *mutex)
{
	struct rtw_hal_com_t *hal = (struct rtw_hal_com_t *)h;
#ifdef CONFIG_RTW_HALMAC_USE_SPINLOCK
	_os_spinlock(hal->drv_priv, (_os_lock *)mutex, _bh, NULL);
#else
...
static void hal_mac_mutex_unlock(void *h, mac_ax_mutex *mutex)
{
	struct rtw_hal_com_t *hal = (struct rtw_hal_com_t *)h;
#ifdef CONFIG_RTW_HALMAC_USE_SPINLOCK
	_os_spinunlock(hal->drv_priv, (_os_lock *)mutex, _bh, NULL);
#else
...
#define MSG_LEN		100
	...
	struct rtw_hal_com_t *hal = (struct rtw_hal_com_t *)p;

	if (   (hal->mac_prt_cnt++ & CONFIG_RTW_WATCHDOG_KICK)
	    == CONFIG_RTW_WATCHDOG_KICK)
		_os_kick_watchdog();
	#endif /* CONFIG_RTW_WATCHDOG_KICK */

	va_start(args, fmt);

	if (dbg_level == _PHL_ALWAYS_) {
		_os_vdbgdump(fmt, args);
		va_end(args);
	} else {
		char str[MSG_LEN] = {0};
		int err;
...
void rtw_plt_cb_init(void)
{
	...
	rtw_plt_cb.rtl_recycle_h2c = hal_recycle_h2c_pkt;
#endif
#endif
...
bool rtw_hal_mac_reg_chk(struct rtw_hal_com_t *hal_com, u32 addr)
{
	...
	if (mac != NULL && mac->ops->io_chk_access) {
		...
		if (mac_rst != MACSUCCESS) {
			rst = false;
			PHL_INFO("%s failed - addr(0x%08x) is err code(%d)\n",
					 ...
					 u8 dbcc_en)
{
	...
	if (dbcc_en) {
		...
		rtw_hal_mac_ppdu_stat_cfg(
				hal_info, HW_BAND_1, true,
				0 | HAL_PPDU_MAC_INFO | HAL_PPDU_PLCP | HAL_PPDU_RX_CNT,
				0 | HAL_PPDU_HAS_CRC_OK);
					...
					struct rtw_wifi_role_t *wifi_role)
{
	...
	if (ppara.net_type == MAC_AX_NET_TYPE_INFRA) {
		...
		if (wifi_role->hw_port == 0) {
			PHL_PRINT("Initializing port 0 with %u M-BSSID.\n", ppara.mbid_num);
	...
	// Fast drop in repeater vxd or client mode of AP
	if (wifi_role->type == PHL_RTYPE_STATION) {
		cfg_para.band = wifi_role->hw_band;
		cfg_para.port = wifi_role->hw_port;
		cfg_para.mbssid_idx = 0;

		cfg_para.val = 1;	// fast drop
		if (mac->ops->port_cfg(mac, MAC_AX_PCFG_BCN_HOLD_TIME, &cfg_para) != MACSUCCESS)
			return RTW_HAL_STATUS_FAILURE;
	}

#ifdef RTW_WKARD_BCN_ERR
	if (ppara.net_type == MAC_AX_NET_TYPE_AP) {
		struct mac_ax_edca_param edca_param;
		u32 mac_ret;
		...
		cfg_para.mbssid_idx = 0;
		/* Beacon hold time */
		cfg_para.val = WKARD_BCN_ERR_HOLD_TIME;
		mac_ret = mac->ops->port_cfg(mac, MAC_AX_PCFG_BCN_HOLD_TIME,
		                             &cfg_para);
		if (mac_ret == MACSETVALERR) {
			PHL_WARN("Failed to set beacon hold time %u. Set to adjusted %u.\n",
			         WKARD_BCN_ERR_HOLD_TIME, cfg_para.val);
			mac_ret = mac->ops->port_cfg(mac, MAC_AX_PCFG_BCN_HOLD_TIME,
			                             &cfg_para);
		}

		if (mac_ret != MACSUCCESS) {
			PHL_ERR("Failed to set beacon hold time %u\n",
			        cfg_para.val);
		}
		/* Beacon EDCA to reduce collision */
		edca_param.band = wifi_role->hw_band;
		edca_param.path = MAC_AX_CMAC_PATH_SEL_BCN;
		if (mac->ops->get_hw_value(mac, MAC_AX_HW_GET_EDCA_PARAM,
		                           &edca_param)
		    != MACSUCCESS) {
			PHL_ERR("Failed get EDCA Param!\n");
			return RTW_HAL_STATUS_SUCCESS;
		}

		edca_param.ecw_max = WKARD_BCN_ERR_ECW_MAX;
		edca_param.ecw_min = WKARD_BCN_ERR_ECW_MIN;
		edca_param.aifs_us = WKARD_BCN_ERR_AIFS;
		mac->ops->set_hw_value(mac, MAC_AX_HW_SET_EDCA_PARAM,
		                       &edca_param);
		#if 0
		/* CCA during SIFS */
		mac->ops->sifs_chk_cca_en(mac, wifi_role->hw_band, 1);
		#endif
	}
...
enum rtw_hal_status
rtw_hal_mac_port_tx_mode_sel(struct hal_info_t *hal_info,
					u8 enable)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_mac_tx_mode_sel tx_mode;
	if (enable) {
		tx_mode.sw_mode_band0_en = 1;
		tx_mode.txop_rot_wmm0_en = 1;
		tx_mode.txop_rot_wmm1_en = 0;
		tx_mode.txop_rot_wmm2_en = 0;
		tx_mode.txop_rot_wmm3_en = 0;
	} else {
		tx_mode.sw_mode_band0_en = 0;
		tx_mode.txop_rot_wmm0_en = 0;
		tx_mode.txop_rot_wmm1_en = 0;
		tx_mode.txop_rot_wmm2_en = 0;
		tx_mode.txop_rot_wmm3_en = 0;
	}
	if (mac->ops->tx_mode_sel(mac, &tx_mode) != MACSUCCESS) {
		return RTW_HAL_STATUS_FAILURE;
	}
	RTW_INFO("[%s]Enable %d FW Tx!\n", __func__, enable);
	return RTW_HAL_STATUS_SUCCESS;
}
...
enum rtw_hal_status
rtw_hal_mac_trigger_bus_err(struct hal_info_t *hal_info)
{
	PHL_PRINT("Triggering bus error...\n");
	hal_write32(hal_info->hal_com, R_AX_HALT_C2H, 0x1FFFFFFF);
	hal_write32(hal_info->hal_com, R_AX_HALT_C2H_CTRL, B_AX_HALT_C2H_TRIGGER);
								...
								struct rtw_gtk_ofld_info *cfg)
{
	...
	if (en) {
		...
		if (info.gtk_en) {
			...
			if (cfg->gtk_ofld_content.rxmickey != NULL)
...
/**
* the function to update nav padding setting by halmac api
* @hal_info: see struct hal_info_t
* @band: target band
* @nav_pad_en: enaebl/disable this feature
* @over_txop_en:
* @nav_padding:
*
...
enum rtw_hal_status
rtw_hal_mac_set_nav_padding(struct hal_info_t *hal_info, u8 band,
					 u8 nav_pad_en, u8 over_txop_en, u16 nav_padding)
{
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	struct mac_ax_nav_padding info;
	u32 mac_err;

	_os_mem_set(hal_to_drvpriv(hal_info), &info, 0, sizeof(struct mac_ax_nav_padding));

	info.band = band;
	info.nav_pad_en = nav_pad_en;
	info.over_txop_en = over_txop_en;
	info.nav_padding = nav_padding;

	mac_err = mac->ops->set_hw_value(mac, MAC_AX_HW_SET_NAV_PADDING, &info);
...
enum rtw_hal_status
rtw_hal_mac_ax_set_mu_fix_mode(
	void *mac, u16 macid, u8 gid, enum rtw_hal_protection_type prot_type,
	...
	bool fix_mu, bool he, bool fix_resp, bool fix_prot)
{
	...
	fix_info.tbl_hdr.rw = 1;
	fix_info.tbl_hdr.idx = macid;
...
void rtw_hal_mac_upd_ul_fixinfo_dump(struct rtw_phl_ax_ul_fixinfo *tbl){

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "cfg.mode 0x%x\n", tbl->cfg.mode);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "cfg.storemode 0x%x\n", tbl->cfg.storemode);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "cfg.interval %d\n", tbl->cfg.interval);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "store_idx 0x%x\n", tbl->store_idx);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulfix_usage 0x%x\n", tbl->ulfix_usage);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "data_rate 0x%x\n", tbl->data_rate);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "data_bw 0x%x\n", tbl->data_bw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "gi_ltf 0x%x\n", tbl->gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tf_type 0x%x\n", tbl->tf_type);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_logo_test 0x%x\n", tbl->ul_logo_test);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta_num 0x%x\n", tbl->ulrua.sta_num);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.gi_ltf 0x%x\n", tbl->ulrua.gi_ltf);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.n_ltf_and_ma 0x%x\n", tbl->ulrua.n_ltf_and_ma);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.ppdu_bw 0x%x\n", tbl->ulrua.ppdu_bw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "apep_len 0x%x\n", tbl->apep_len);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[0].macid 0x%x\n", tbl->sta[0].macid);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[0].mac_id 0x%x\n", tbl->ulrua.sta[0].mac_id);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[0].coding 0x%x\n", tbl->ulrua.sta[0].coding);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[0].rate.mcs 0x%x\n", tbl->ulrua.sta[0].rate.mcs);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[0].rate.ss 0x%x\n", tbl->ulrua.sta[0].rate.ss);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[0].ru_pos 0x%x\n", tbl->ulrua.sta[0].ru_pos);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[1].macid 0x%x\n", tbl->sta[1].macid);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[1].mac_id 0x%x\n", tbl->ulrua.sta[1].mac_id);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[1].coding 0x%x\n", tbl->ulrua.sta[1].coding);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[1].rate.mcs 0x%x\n", tbl->ulrua.sta[1].rate.mcs);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[1].rate.ss 0x%x\n", tbl->ulrua.sta[1].rate.ss);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[1].ru_pos 0x%x\n", tbl->ulrua.sta[1].ru_pos);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[2].macid 0x%x\n", tbl->sta[2].macid);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[2].mac_id 0x%x\n", tbl->ulrua.sta[2].mac_id);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[2].coding 0x%x\n", tbl->ulrua.sta[2].coding);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[2].rate.mcs 0x%x\n", tbl->ulrua.sta[2].rate.mcs);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[2].rate.ss 0x%x\n", tbl->ulrua.sta[2].rate.ss);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[2].ru_pos 0x%x\n", tbl->ulrua.sta[2].ru_pos);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "sta[3].macid 0x%x\n", tbl->sta[3].macid);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[3].mac_id 0x%x\n", tbl->ulrua.sta[3].mac_id);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[3].coding 0x%x\n", tbl->ulrua.sta[3].coding);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[3].rate.mcs 0x%x\n", tbl->ulrua.sta[3].rate.mcs);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[3].rate.ss 0x%x\n", tbl->ulrua.sta[3].rate.ss);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulrua.sta[3].ru_pos 0x%x\n", tbl->ulrua.sta[3].ru_pos);

	return;
}

void
rtw_hal_mac_fixmode_para_dump(struct mac_ax_fixmode_para *fixmode_para)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tbl_hdr.rw 0x%x\n", fixmode_para->tbl_hdr.rw);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tbl_hdr.idx 0x%x\n", fixmode_para->tbl_hdr.idx);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tbl_hdr.offset 0x%x\n", fixmode_para->tbl_hdr.offset);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tbl_hdr.len 0x%x\n", fixmode_para->tbl_hdr.len);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "tbl_hdr.type 0x%x\n", fixmode_para->tbl_hdr.type);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "force_sumuru_en 0x%x\n", fixmode_para->force_sumuru_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "forcesu 0x%x\n", fixmode_para->forcesu);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "forcemu 0x%x\n", fixmode_para->forcemu);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "forceru 0x%x\n", fixmode_para->forceru);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_fe_su_en 0x%x\n", fixmode_para->fix_fe_su_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_fe_vhtmu_en 0x%x\n", fixmode_para->fix_fe_vhtmu_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_fe_hemu_en 0x%x\n", fixmode_para->fix_fe_hemu_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_fe_heru_en 0x%x\n", fixmode_para->fix_fe_heru_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_fe_ul_en 0x%x\n", fixmode_para->fix_fe_ul_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_frame_seq_su 0x%x\n", fixmode_para->fix_frame_seq_su);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_frame_seq_vhtmu 0x%x\n", fixmode_para->fix_frame_seq_vhtmu);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_frame_seq_hemu 0x%x\n", fixmode_para->fix_frame_seq_hemu);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_frame_seq_heru 0x%x\n", fixmode_para->fix_frame_seq_heru);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_frame_seq_ul 0x%x\n", fixmode_para->fix_frame_seq_ul);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "is_dlruhwgrp 0x%x\n", fixmode_para->is_dlruhwgrp);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "is_ulruhwgrp 0x%x\n", fixmode_para->is_ulruhwgrp);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "prot_type_su 0x%x\n", fixmode_para->prot_type_su);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "prot_type_vhtmu 0x%x\n", fixmode_para->prot_type_vhtmu);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "resp_type_vhtmu 0x%x\n", fixmode_para->resp_type_vhtmu);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "prot_type_hemu 0x%x\n", fixmode_para->prot_type_hemu);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "resp_type_hemu 0x%x\n", fixmode_para->resp_type_hemu);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "prot_type_heru 0x%x\n", fixmode_para->prot_type_heru);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "resp_type_heru 0x%x\n", fixmode_para->resp_type_heru);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ul_prot_type 0x%x\n", fixmode_para->ul_prot_type);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "rugrpid 0x%x\n", fixmode_para->rugrpid);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "mugrpid 0x%x\n", fixmode_para->mugrpid);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "ulgrpid 0x%x\n", fixmode_para->ulgrpid);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "rsvd1 0x%x\n", fixmode_para->rsvd1);

	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "fix_txcmdnum_en 0x%x\n", fixmode_para->fix_txcmdnum_en);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "force_to_one 0x%x\n", fixmode_para->force_to_one);

}

void
rtw_hal_mac_dl_grp_info_dump(struct mac_ax_ss_dl_grp_upd *info)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "grp_valid 0x%x\n", info->grp_valid);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "grp_id 0x%x\n", info->grp_id);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "is_hwgrp 0x%x\n", info->is_hwgrp);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "macid_u0 0x%x\n", info->macid_u0);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "macid_u1 0x%x\n", info->macid_u1);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "macid_u2 0x%x\n", info->macid_u2);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "macid_u3 0x%x\n", info->macid_u3);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "next_protecttype 0x%x\n", info->next_protecttype);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "next_rsptype 0x%x\n", info->next_rsptype);
}

enum rtw_hal_status
rtw_hal_mac_set_upd_ul_fixinfo(void *hal,
					struct rtw_phl_ax_ul_fixinfo *tbl)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);

	rtw_hal_mac_upd_ul_fixinfo_dump(tbl);
...
enum rtw_hal_status
rtw_hal_mac_set_fixmode_mib(void *hal,
					struct rtw_phl_mac_ax_fixmode_para *tbl)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);

	struct mac_ax_fixmode_para fixmode_para = {{0}};

	fixmode_para.tbl_hdr.rw = tbl->tbl_hdr.rw;
	fixmode_para.tbl_hdr.idx = tbl->tbl_hdr.idx;
	fixmode_para.force_sumuru_en = tbl->force_sumuru_en;
	fixmode_para.forceru = tbl->forceru;
	fixmode_para.forcesu = tbl->forcesu;
	fixmode_para.fix_fe_heru_en = tbl->fix_fe_heru_en;
	fixmode_para.fix_frame_seq_heru = tbl->fix_frame_seq_heru;
	fixmode_para.rugrpid = tbl->rugrpid;
	fixmode_para.prot_type_heru = tbl->prot_type_heru;
	fixmode_para.resp_type_heru = tbl->resp_type_heru;
	fixmode_para.is_dlruhwgrp = tbl->is_dlruhwgrp;

	fixmode_para.fix_txcmdnum_en = tbl->fix_txcmdnum_en;
	fixmode_para.force_to_one= tbl->force_to_one;

	rtw_hal_mac_fixmode_para_dump(&fixmode_para);

	if (mac->ops->set_fw_fixmode(mac, &fixmode_para) == MACSUCCESS)
		return RTW_HAL_STATUS_SUCCESS;
...
enum rtw_hal_status
rtw_hal_mac_mu_set_dl_grp_info(void *hal,
					struct rtw_phl_mac_ss_dl_grp_upd *tbl)
{

	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);

	struct mac_ax_ss_dl_grp_upd info = {0};


	info.grp_valid = tbl->grp_valid; //0: non valid 1: valid
	info.grp_id = tbl->grp_id; //grp 0~16
	info.is_hwgrp = tbl->is_hwgrp;
	info.mru = tbl->mru;
	info.macid_u0 = tbl->macid_u0;
	info.macid_u1 = tbl->macid_u1;

	info.macid_u2 = tbl->macid_u2;
	info.macid_u3 = tbl->macid_u3;
	info.macid_u4 = tbl->macid_u4;
	info.macid_u5 = tbl->macid_u5;
	info.macid_u6 = tbl->macid_u6;
	info.macid_u7 = tbl->macid_u6;

	info.ac_bitmap_u0 = tbl->ac_bitmap_u0;
	info.ac_bitmap_u1 = tbl->ac_bitmap_u1;



	//rtw_hal_mac_dl_grp_info_dump(&info);

	if (mac->ops->ss_dl_grp_upd(mac, &info) == MACSUCCESS)
		return RTW_HAL_STATUS_SUCCESS;
...
enum rtw_hal_status
rtw_hal_mac_set_dl_grp_info(void *hal,
					struct rtw_phl_mac_ss_dl_grp_upd *tbl)
{
	...
	info.grp_valid = 1; //0: non valid 1: valid
	info.grp_id = tbl->grp_id; //grp 0~16
	info.is_hwgrp = 1;
	info.mru = 0;
	...
	info.macid_u5 = tbl->macid_u5;
	info.macid_u6 = tbl->macid_u6;
	info.macid_u7 = tbl->macid_u7;

	info.ac_bitmap_u0 = 0xf;
	info.ac_bitmap_u1 = 0xf;
	info.ac_bitmap_u2 = 0xf;
	info.ac_bitmap_u3 = 0xf;
	info.ac_bitmap_u4 = 0xf;
	info.ac_bitmap_u5 = 0xf;
	info.ac_bitmap_u6 = 0xf;
	info.ac_bitmap_u7 = 0xf;
	info.next_protecttype = tbl->next_protecttype;
	info.next_rsptype = tbl->next_rsptype;
...
enum rtw_hal_status
rtw_hal_mac_set_ul_grp_info(void *hal)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);

	struct mac_ax_ss_ul_grp_upd info = {0};

	// add macid 1 & 2 into grp 0
	info.macid_u0 = 0x1;
	info.macid_u1 = 0x2;
	info.grp_bitmap = 0x0;
	if (mac->ops->ss_ul_grp_upd(mac, &info) != MACSUCCESS)
		return RTW_HAL_STATUS_FAILURE;

	// add macid 3 & 4 into grp 0
	info.macid_u0 = 0x3;
	info.macid_u1 = 0x4;
	info.grp_bitmap = 0x0;
	if (mac->ops->ss_ul_grp_upd(mac, &info) != MACSUCCESS)
...
enum rtw_hal_status
rtw_hal_mac_parse_fwc2h(void *hal, void *c, void *c2h_sts)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	...
	//if(mac->ops->fwc2h_sts_parse)
	//	hal_status = mac->ops->fwc2h_sts_parse(mac, fw_c2h_sts, (u32 *)c2h->content);
	if(mac->ops->fwc2h_ofdma_sts_parse)
		hal_status = mac->ops->fwc2h_ofdma_sts_parse(mac, fw_c2h_sts, (u32 *)c2h->content);

	return hal_status;
}

enum rtw_hal_status
rtw_hal_mac_fwc2h_en(void *hal, struct rtw_phl_mac_ax_ru_fwc2h_en *info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	...
	fwsts_para.en = info->en;
	fwsts_para.intvl_ms = info->intvl_ms;

	printk("%s: en:%d, intvl_ms:%d\n", __func__, fwsts_para.en, fwsts_para.intvl_ms);

	//if(mac->ops->fw_sts_en)
	//	hal_status = mac->ops->fw_sts_en(mac, &fwsts_para);
	if(mac->ops->fw_ofdma_sts_en)
		hal_status = mac->ops->fw_ofdma_sts_en(mac, &fwsts_para);
...
enum rtw_hal_status
hal_mac_ax_config_beacon(struct hal_info_t *hal,
...
enum rtw_hal_status rtw_hal_mac_set_append_fcs(struct hal_info_t *hal_info, u8 val)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_ops *ops = mac->ops;
	u8 _val = val;

	if (ops->set_hw_value(mac, MAC_AX_HW_SET_APP_FCS, &_val) != MACSUCCESS)
		return RTW_HAL_STATUS_FAILURE;
	else
		return RTW_HAL_STATUS_SUCCESS;
}
...
enum rtw_hal_status rtw_hal_mac_set_acpt_icv_err(struct hal_info_t *hal_info, u8 val)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_ops *ops = mac->ops;
	u8 _val = val;

	if (ops->set_hw_value(mac, MAC_AX_HW_SET_RX_ICVERR, &_val) != MACSUCCESS)
		return RTW_HAL_STATUS_FAILURE;
	else
		return RTW_HAL_STATUS_SUCCESS;
}
						...
						struct pkt_ofld_entry *entry)
{
	...
	PHL_INFO("macid %d, probersp %d, pspoll %d, nulldata %d, qosnull %d, cts2self %d.\n",
		mac_ids.macid,
		mac_ids.probersp,
		mac_ids.pspoll,
		mac_ids.nulldata,
...
rtw_hal_mac_read_hidden_rpt(struct rtw_hal_com_t *hal_com)
{
	...
	if (rpt.tx_spatial_stream != 7 && rpt.tx_spatial_stream > 0) {
		...
		hal_com->def_rfpath_tx = rpt.tx_spatial_stream;
	}
	if (rpt.rx_spatial_stream != 7 && rpt.rx_spatial_stream > 0) {
		...
		hal_com->def_rfpath_rx = rpt.rx_spatial_stream;
	}
	if (rpt.bandwidth > 0 && (rpt.bandwidth & BIT(2))) {
		hal_com->phy_hw_cap[0].bandwidth = 0;
		hal_com->phy_hw_cap[1].bandwidth = 0;
		for (i = 0; i <= (rpt.bandwidth & 0x03); i++) {
			hal_com->phy_hw_cap[0].bandwidth |= BIT(i);
			hal_com->phy_hw_cap[1].bandwidth |= BIT(i);
		}
	}
...
enum rtw_hal_status
rtw_hal_mac_set_rxd_zld_en(void *hal, u8 value){
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_ops *ops = mac->ops;
	enum rtw_hal_status ret = RTW_HAL_STATUS_SUCCESS;

	ret = ops->set_hw_value(mac, MAC_AX_HW_SET_RXD_ZLD_EN, &value);

	if (ret != MACSUCCESS)
		PHL_TRACE(COMP_PHL_DBG, _PHL_ERR_, "Set rxd_zld_en failure, status = %d\n", ret);
						...
						u16 tx_en, u16 tx_en_mask)
{
	...
	hal_com->band[band_idx].txen_final = tx_en;
	hal_com->band[band_idx].txen_set_time = _os_get_cur_time_ms();
	hal_com->band[band_idx].txen_mismatch_cnt = 0;
...
rtw_hal_mac_get_sch_tx_en(struct rtw_hal_com_t *hal_com, u8 band_idx, u16 *tx_en)
{
	...
	result = ops->get_hw_value(mac, MAC_AX_HW_GET_SCH_TXEN_STATUS, &cfg);

	if (result != MACSUCCESS) {
		PHL_TRACE(COMP_PHL_DBG, _PHL_ERR_, "Get tx pause failure, status = %d\n", result);
		return RTW_HAL_STATUS_FAILURE;
	}

	*tx_en = 0;
	*tx_en |= (cfg.tx_en.be0 ? RTW_TXEN_BE0 : 0);
	*tx_en |= (cfg.tx_en.bk0 ? RTW_TXEN_BK0 : 0);
	*tx_en |= (cfg.tx_en.vi0 ? RTW_TXEN_VI0 : 0);
	*tx_en |= (cfg.tx_en.vo0 ? RTW_TXEN_VO0 : 0);
	*tx_en |= (cfg.tx_en.be1 ? RTW_TXEN_BE1 : 0);
	*tx_en |= (cfg.tx_en.bk1 ? RTW_TXEN_BK1 : 0);
	*tx_en |= (cfg.tx_en.vi1 ? RTW_TXEN_VI1 : 0);
	*tx_en |= (cfg.tx_en.vo1 ? RTW_TXEN_VO1 : 0);
	*tx_en |= (cfg.tx_en.mg0 ? RTW_TXEN_MG0 : 0);
	*tx_en |= (cfg.tx_en.mg1 ? RTW_TXEN_MG1 : 0);
	*tx_en |= (cfg.tx_en.mg2 ? RTW_TXEN_MG2 : 0);
	*tx_en |= (cfg.tx_en.hi ? RTW_TXEN_HI : 0);
	*tx_en |= (cfg.tx_en.bcn ? RTW_TXEN_BCN : 0);
	*tx_en |= (cfg.tx_en.ul ? RTW_TXEN_UL : 0);
	*tx_en |= (cfg.tx_en.twt0 ? RTW_TXEN_TWT0 : 0);
	*tx_en |= (cfg.tx_en.twt1 ? RTW_TXEN_TWT1 : 0);

	return RTW_HAL_STATUS_SUCCESS;
}

#ifdef CONFIG_RTW_TXEN_SYNC_PATCH
enum rtw_hal_status
rtw_hal_check_tx_en_sync(void *hal, u8 band_idx)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	...
	u32 cur_time;
	u16 cur_tx_en;
	struct rtw_hw_band *band = &hal_com->band[band_idx];

	if (band->txen_set_time == 0)
		return RTW_HAL_STATUS_SUCCESS;

	cur_time = _os_get_cur_time_ms();
	if ((cur_time - band->txen_set_time) <= 2)
		return RTW_HAL_STATUS_SUCCESS;

	hstatus = rtw_hal_mac_get_sch_tx_en(hal_com, band_idx, &cur_tx_en);
	if (hstatus != RTW_HAL_STATUS_SUCCESS)
		return RTW_HAL_STATUS_FAILURE;

	if (cur_tx_en == band->txen_final)
		return RTW_HAL_STATUS_SUCCESS;

	band->txen_mismatch_cnt++;

	if (band->txen_mismatch_cnt >= TXEN_SYNC_CHECK_CNT) {
		PHL_ERR("TXEN mismatch (D_%04X:F_%04X) for %u times. "
			 "Reset it to driver's.\n",
			 band->txen_final, cur_tx_en,
			 band->txen_mismatch_cnt);
		...
		if (hstatus == RTW_HAL_STATUS_SUCCESS) {
			hal_com->band[band_idx].txen_mismatch_cnt = 0;
			hal_com->band[band_idx].txen_set_time = cur_time;
		}
	}

	return hstatus;
} /* rtw_hal_check_tx_en_sync */
#endif /* CONFIG_RTW_TXEN_SYNC_PATCH */
		 ...
		 u8 band_idx, bool tx_pause, enum tx_pause_rson rson)
{
	...
	if (tx_pause == true) {
		...
		case PAUSE_RSON_NOR_SCAN: {
			/* Pause for scan. Turning off TBTT TX prohibit to
			 * prevent TX probe request from being blocked.
			 */
			struct hal_info_t *hal_info = hal_com->hal_priv;
			struct mac_ax_adapter *mac = hal_to_mac(hal_info);
			struct mac_ax_port_cfg_para cfg_para = {0};

			mac->ops->port_cfg(mac, MAC_AX_PCFG_BCN_PRCT, &cfg_para);
		...
		*tx_off = 0;
		/* Turning off scan pause. Re-enable beacon TBTT TX prohibit */
		if (rson == PAUSE_RSON_NOR_SCAN) {
			struct hal_info_t *hal_info = hal_com->hal_priv;
			struct mac_ax_adapter *mac = hal_to_mac(hal_info);
			struct mac_ax_port_cfg_para cfg_para = {0};
                             ...
                             u32 *macid_arr, u8 macid_arr_sz, bool pause)
{
	...
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	struct mac_ax_ops *ops = mac->ops;
	struct mac_ax_macid_pause_grp cfg ;
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	u8 pause_grp_sz = 0, mac_grp_sz = 0, arr_idx = 0, bit_idx = 0;
	u32 mac_ret = MACSUCCESS;

	_rtw_memset(&cfg, 0, sizeof(struct mac_ax_macid_pause_grp));
	/* size check */
...
rtw_hal_mac_set_macid_pkt_drop(struct rtw_hal_com_t *hal_com,
                            u16 macid, u8 sel, u8 port, u8 mbssid)
{
	struct hal_info_t *hal = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	struct mac_ax_ops *ops = mac->ops;
	struct mac_ax_pkt_drop_info cfg = {0};
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
...
enum rtw_hal_status
rtw_hal_mac_hiq_drop(struct rtw_hal_com_t *hal_com, u8 port, u8 mbssid)
{
	struct hal_info_t *hal = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	struct mac_ax_ops *ops = mac->ops;
	...
	cfg.sel = MAC_AX_PKT_DROP_SEL_HIQ_ONCE;
	cfg.port = port;
	cfg.mbssid = mbssid;
...
enum rtw_hal_status
rtw_hal_mac_set_hiq_drop(struct rtw_hal_com_t *hal_com, u8 port, u8 mbssid)
{
	struct hal_info_t *hal = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	struct mac_ax_ops *ops = mac->ops;
	...
	cfg.sel = MAC_AX_PKT_DROP_SEL_HIQ_MBSSID;
	cfg.port = port;
	cfg.mbssid = mbssid;
...
enum rtw_hal_status
rtw_hal_mac_rel_hiq_drop(struct rtw_hal_com_t *hal_com, u8 port, u8 mbssid)
{
	struct hal_info_t *hal = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	struct mac_ax_ops *ops = mac->ops;
	...
	cfg.sel = MAC_AX_PKT_DROP_SEL_REL_HIQ_MBSSID;
	cfg.port = port;
	cfg.mbssid = mbssid;
			...
			struct rtw_hal_fw_log_cfg *fl_cfg)
{
	...
	{
		#if 1
		/* ToDo: HALMAC to support firmware console output setting */
		hal_write32(hal_com, 0x02d4, 0xf1177ff6);
		hal_write32(hal_com, 0x0040, 0x00000104);
		hal_write8(hal_com, 0x01e8, 0x51);
		hal_write32(hal_com, 0x0088, 0x000004cf);
		#else
			       ...
			       u8 band, enum rtw_rx_fltr_mode mode)
{
	struct hal_info_t *hal_info = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct rtw_phl_com_t *phl_com = (struct rtw_phl_com_t *)mac->phl_adapter;
	...
	/* RX packet length filter set to RX A-MSDU size */
	ctrl.pkt_len_fltr = 8 * (phl_com->proto_sw_cap[0].max_amsdu_len + 1); /* x 512B */;
	mask.pkt_len_fltr = 0x03F;

	do {
		struct mac_ax_rx_fltr_ctrl_t cur_ctrl = {0};
		u32 *pctrl = (u32 *)&ctrl;
		u32 *pmask = (u32 *)&mask;
		u32 *pcur_ctrl = (u32 *)&cur_ctrl;

		err = mac->ops->get_rx_fltr_opt(mac, &cur_ctrl, band);
		if (   (err == 0)
		    && (*pcur_ctrl & *pmask) != (*pctrl & *pmask))
			PHL_PRINT("Set RX packet size filter to %uB\n",
			          ctrl.pkt_len_fltr * 512);
	} while (0);
                             ...
                             u8 *buf, u32 len, u32 dbg_path, u8 type)
{
         ...
         if ( 0 == type) {
              ...
              //printk("----------------------------------- MAC_AX_MEM_AXIDMA  \n");
              //mac_mem_dump(mac, MAC_AX_MEM_AXIDMA, strt_addr, buf, len, dbg_path);
         }
}
...
static enum mac_ax_cmac_path_sel _ac_drv2mac(u8 ac, u8 wmm)
{
	...
	case 4:
		/* MG */
		return MAC_AX_CMAC_PATH_SEL_MG0_1;
	case 5:
		/* BCN */
		return MAC_AX_CMAC_PATH_SEL_BCN;
	}
 ...
 */
enum rtw_hal_status
rtw_hal_mac_get_edca(struct rtw_hal_com_t *hal_com, u8 band, u8 wmm, u8 ac,
		     u32 *param)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	...
	*param = (u32)edca.txop_32us << 16;
	*param |= (u32)(edca.ecw_max & 0x0F) << 12;
	*param |= (u32)(edca.ecw_min & 0x0F) << 8;
	*param |= (u32)edca.aifs_us & 0xFF;
			      ...
			      bool he, u8 sta_nr,bool mode)
{
	...
	if (he) {
		if(mode==1){
			for (i = 0; (i < 2)&&(i < HAL_MAX_HE_SND_STA_NUM); i++) {  //charlie sta_nr
				hal_he_sta = (struct hal_he_ndpa_sta_info *)
					      &hal_ndpa->ndpa_sta_info[0] ;/////////////////charlie
				...
				//mac_ndpa->he_para.sta_info[i].cb = hal_he_sta->cb;
				mac_ndpa->he_para.sta_info[i].cb = 1;
				
				//MU 1+1
			if(sta_nr == 2)
				mac_ndpa->he_para.sta_info[i].nc =0;//cb nc
			else
			...
			if( mac_ndpa->he_para.sta_info[0].aid == mac_ndpa->he_para.sta_info[1].aid)
				mac_ndpa->he_para.sta_info[1].aid = 100;
		}else{
			      ...
			      u8 num_1, u8 num_2)
{
	...
	if (num_2) {
		...
		mac_he_bfrp->common.ul_siga2_rsvd =
					hal_he_bfrp->common.ul_siga2_rsvd;
		for( i = 0; (i < num_2)&&(i < HAL_MAX_HE_BFRP_STA_NUM); i++) {
			mac_he_bfrp->user[i].aid12 =
					hal_he_bfrp->user[i].aid12;
			mac_he_bfrp->user[i].fbseg_rexmit_bmp =
					hal_he_bfrp->fbseg_rexmit_bmp[i];
		       ...
		       struct hal_ax_fwcmd_snd *hal_cmd)
{
	...
	snd_cmd.mode = hal_cmd->mode;
	snd_cmd.sfp.cr_idx = hal_cmd->cr_idx;

	for (i = 0; i < 8; i++) {    //HAL_MAX_HE_SND_STA_NUM: wait for halmac temporarily
		snd_cmd.macid[i] = (u8)hal_cmd->macid[i];
	}

#if 1
		if(snd_cmd.bfrp1_user_num )
		{
			snd_cmd.macid[1] = 0;
			snd_cmd.macid[8] = (u8)hal_cmd->macid[1];
		}
#endif
	...
	if (he) {
		...
		for (i = 0; i < (sta_nr - 1)&&(i < HAL_MAX_VHT_BFRP_NUM);
			...
			_hal_max_ax_snd_cmd_wd(&snd_cmd.wd[2 + i],
					       &hal_cmd->wd[(2 + i) < (HLA_MAX_BFRP_NUM + 2) ? (2 + i) : 0]);
		}
	}
...
void hal_mac_process_bcn_stats(void *hal, struct mac_ax_bcn_cnt bcn_cnt)
{
	...
	_os_spinlock(drv_priv, &bcn_pool->bcn_lock, BCN_LOCK_TYPE, NULL);
	...
	if (bcn_entry) {
		...
		if (!(phl_is_mp_mode(phl_com))
		    && (bcn_cnt.ok_cnt < hal_info->hal_com->dbg_bcn_tx)) {
#ifdef CONFIG_RTW_DEBUG_BCN_STATS
			bcn_entry->bcn_stats.dbg_bcn_tx_cnt++;
#endif /* CONFIG_RTW_DEBUG_BCN_STATS */
			PHL_WARN("(Band:%u, Port:%u, Mbssid:%u) OK%u C%u E%u N%u T%u M%u O%u L%u P%u I%u S%u\n",
				hal_com->band[bcn_cnt.band].cur_chandef.band,
				bcn_cnt.port, bcn_cnt.mbssid,
				bcn_cnt.ok_cnt, bcn_cnt.cca_cnt,
	...
	_os_spinunlock(drv_priv, &bcn_pool->bcn_lock, BCN_LOCK_TYPE, NULL);
...
u32 rtw_hal_mac_process_c2h(void *hal, struct rtw_c2h_info *c2h, struct c2h_evt_msg *c2h_msg)
{
	...
	if (retid == MSG_EVT_CCX_REPORT_TX_OK || retid == MSG_EVT_CCX_REPORT_TX_FAIL) {
		#if 0
...
#ifdef CONFIG_PHL_TWT
	if (retid == MSG_EVT_TWT_WAIT_ANNOUNCE) {
...
u32 rtw_hal_get_hw_seq(void *hal, u32 macid, u8 tid)
{
	static const u32 tid_2_qsel[8] = {0 ,1, 1, 0,
					  2, 2, 3, 3};
	static const u32 tid_remap[8] = {0, 1, 1, 0,
					 4, 4, 6, 6};
	...
	u32			dmac_table[8];
	u32			entry_sz;
	u32			ssn_sel = tid_2_qsel[tid_remap[tid & 0x7]];
	u32			hw_seq;
	...
	halmac_ret = mac->ops->mem_dump(mac, MAC_AX_MEM_DMAC_TBL,
					macid * entry_sz,
					(u8 *)&dmac_table[0], entry_sz, 1);

	if (halmac_ret != MACSUCCESS) {
		PHL_WARN("Failed to get dump DMAC table (%d).\n", halmac_ret);
		return (-1);
	}

	switch (ssn_sel) {
	case 0:
		hw_seq = le32_to_cpu(dmac_table[2]) & 0x0FFF;
		break;
	case 1:
		hw_seq = (le32_to_cpu(dmac_table[2]) >> 12) & 0x0FFF;
		break;
	case 2:
		hw_seq = le32_to_cpu(dmac_table[3]) & 0x0FFF;
		break;
	default:
		hw_seq = (le32_to_cpu(dmac_table[3]) >> 12) & 0x0FFF;
		break;
	}
...
struct cam_info_s {
	enum mac_ax_mem_sel	cam_mem_sel;
	u32			entry_size;
	const char		*name;
};

void rtl_hal_mac_dump_cam(struct hal_info_t *hal_info,
                          enum rtw_cam_table_e cam_id,
                          u32 idx, u8 *buf)
{
	struct mac_ax_adapter	*mac = (struct mac_ax_adapter *)hal_info->mac;
	u32			halmac_ret;
	u8			_buf[256];
	u32			entry_sz = sizeof(_buf);

	struct cam_info_s cam_info[] = {
		// RTW_CAM_DMAC_TBL
		{ MAC_AX_MEM_DMAC_TBL,
		  sizeof(struct mac_ax_dctl_info),
		  "DMAC table"},
		// RTW_CAM_SHCUT_MACHDR
		{ MAC_AX_MEM_SHCUT_MACHDR,
		  sizeof(struct mac_ax_shcut_mhdr),
		  "TX WHDR shortcut"},
		// RTW_CAM_SECURITY
		...
		// RTW_CAM_CMAC_TBL
		{ MAC_AX_MEM_CMAC_TBL,
		  sizeof(struct mac_ax_cctl_info),
		  "CMAC Table"},
		// RTW_CAM_ADDR
		{ MAC_AX_MEM_ADDR_CAM,
		  64,
	...
	if (cam_id >= (sizeof(cam_info)/sizeof(cam_info[0]))) {
		PHL_ERR("%s: invalid CAM ID %u\n", __FUNCTION__, cam_id);
		return;
	}

	if (cam_info[cam_id].entry_size < entry_sz) {
		entry_sz = cam_info[cam_id].entry_size;
	}

	halmac_ret = mac->ops->mem_dump(mac, cam_info[cam_id].cam_mem_sel,
	                                idx * entry_sz,
	                                _buf, entry_sz, 1);
	if (halmac_ret != MACSUCCESS) {
		PHL_WARN("Failed to dump %s of index %d with reason %d.\n",
		         cam_info[cam_id].name, idx, halmac_ret);
		return;
	}
	...
	if (buf != NULL) {
		...
		PHL_PRINT("Dumping %s of index %u...\n",
		          cam_info[cam_id].name, idx);
		debug_dump_buf(_buf, entry_sz, cam_info[cam_id].name);
	}
} /* rtl_hal_mac_dump_cam */
...
enum rtw_hal_status rtw_hal_mac_get_sw_gpio_value(struct hal_info_t *hal_info,
					 u8* val, u8 gpio)
{
	...
	if (mac->ops->get_gpio_val(mac, gpio, val) != MACSUCCESS)
			return RTW_HAL_STATUS_FAILURE;
...
enum rtw_hal_status
rtw_hal_mac_dumpwlanc(struct rtw_hal_com_t *hal_com, u32 cmdid)
{
	struct hal_info_t *hal_info = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_dumpwlanc dumpwlanc;

	dumpwlanc.cmdid = cmdid;

	if (mac->ops->mac_dumpwlanc(mac,&dumpwlanc) != MACSUCCESS){
		return RTW_HAL_STATUS_FAILURE;
	}
...
enum rtw_hal_status
rtw_hal_mac_dumpwlans(struct rtw_hal_com_t *hal_com, u32 cmdid, u32 macid)
{
	struct hal_info_t *hal_info = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_dumpwlans dumpwlans;
	dumpwlans.cmdid = cmdid;
	dumpwlans.macid_grp = macid;

	if (mac->ops->mac_dumpwlans(mac,&dumpwlans) != MACSUCCESS){
		return RTW_HAL_STATUS_FAILURE;
	}
...
enum rtw_hal_status
rtw_hal_mac_set_lifetime(void *hal, u8 en, u16 to)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_lifetime_cfg lifetime_cfg = {0};
	u8 mac_status = MACSUCCESS;

	lifetime_cfg.band = 0;
	lifetime_cfg.en.acq_en = en;
	lifetime_cfg.val.acq_val_1 = to;
	lifetime_cfg.val.acq_val_2 = to;
	lifetime_cfg.val.acq_val_3 = to;
	lifetime_cfg.val.acq_val_4 = to;
	mac_status = mac->ops->set_hw_value(mac,
			MAC_AX_HW_SET_LIFETIME_CFG, &lifetime_cfg);
	if (MACSUCCESS == mac_status) {
		hstatus = RTW_HAL_STATUS_SUCCESS;
	} else {
		PHL_ERR("%s: fail! mac_status:%d\n", __func__, mac_status);
	}
...
enum rtw_hal_status
rtw_hal_mac_get_lifetime(void *hal)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	...
	lifetime_cfg.band = 0;
	mac_status = mac->ops->get_hw_value(mac,
			MAC_AX_HW_GET_LIFETIME_CFG, &lifetime_cfg);
	if (MACSUCCESS == mac_status) {
		PHL_PRINT("LIFETIME: band %d\n", lifetime_cfg.band);
		PHL_PRINT("  MGQ: en %d val %d\n", lifetime_cfg.en.mgq_en, lifetime_cfg.val.mgq_val);
		PHL_PRINT("  ACQ: en %d val %d %d %d %d\n", lifetime_cfg.en.acq_en,
					lifetime_cfg.val.acq_val_1, lifetime_cfg.val.acq_val_2,
					lifetime_cfg.val.acq_val_3, lifetime_cfg.val.acq_val_4);
		hstatus = RTW_HAL_STATUS_SUCCESS;
	} else {
		PHL_ERR("%s: fail! mac_status:%d\n", __func__, mac_status);
	}
...
enum rtw_hal_status
rtw_hal_mac_enable_tx_partial_mode(struct hal_info_t *hal_info, u8 enable)
{
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;

	mac->ops->set_hw_value(mac, MAC_AX_HW_SET_PARTIAL_PLD_MODE, &enable);
	return RTW_HAL_STATUS_SUCCESS;
}

#ifdef CONFIG_ENABLE_MAC_KEY_BACKUP
enum rtw_hal_status
rtw_hal_mac_backup_key(struct hal_info_t *hal_info)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);

	rtl_hal_dump_sec_cam_tbl(hal_info->hal_com);
	if (mac->ops->sta_keycam_backup(mac, SEC_CAM_BACKUP) == MACSUCCESS) {
		return RTW_HAL_STATUS_SUCCESS;

	}
	PHL_ERR("rtw_hal_mac_backup_key fail!\n");
	return RTW_HAL_STATUS_FAILURE;
}

enum rtw_hal_status
rtw_hal_mac_restore_key(struct hal_info_t *hal_info)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);

	if (mac->ops->sta_keycam_backup(mac, SEC_CAM_RESTORE) == MACSUCCESS){
		rtl_hal_dump_sec_cam_tbl(hal_info->hal_com);
		return RTW_HAL_STATUS_SUCCESS;

	}

	PHL_ERR("rtw_hal_mac_restore_key fail!\n");
	return RTW_HAL_STATUS_FAILURE;
}
#endif /* CONFIG_ENABLE_MAC_KEY_BACKUP */

enum rtw_hal_status
hal_recycle_h2c_pkt(struct rtw_phl_com_t *phl_com, struct rtw_h2c_pkt * h2c_pkt)
{
	rtw_phl_recycle_h2c_pkt(phl_com, h2c_pkt);
	return RTW_HAL_STATUS_SUCCESS;
}

enum rtw_hal_status
rtw_hal_mac_upd_ss_ul_sta(struct hal_info_t *hal_info, struct rtw_phl_stainfo_t *sta, u8 enable)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	...
	info.mode = enable;
	info.macid[0] = sta->macid;
	info.macid[1] = info.macid[2] = info.macid[3] = 0xff;
	info.bsr_len[0] = info.bsr_len[1] = 0xff;

	if (mac->ops->ss_ul_sta_upd(mac, &info) == MACSUCCESS)
		return RTW_HAL_STATUS_SUCCESS;
...
#ifdef CONFIG_ENABLE_MAC_H2C_AGG
void
rtw_hal_mac_start_h2c_agg(struct hal_info_t *hal_info)
{
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	void *drv_priv = hal_to_drvpriv(hal_info);
	_os_spinlockfg sp_flags = 0;

	_os_spinlock(drv_priv, &hal_info->hal_com->h2c_agg_lock, _irq, &sp_flags);
	mac->ops->h2c_agg_en(mac, 1);
}

enum rtw_hal_status
rtw_hal_mac_stop_h2c_agg(struct hal_info_t *hal_info, u8 tx)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_SUCCESS;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	void *drv_priv = hal_to_drvpriv(hal_info);
	...
	mac->ops->h2c_agg_en(mac, 0);
	if (tx)
		hal_status = mac->ops->h2c_agg_tx(mac);
	else
		mac->ops->h2c_agg_flush(mac);
	_os_spinunlock(drv_priv, &hal_info->hal_com->h2c_agg_lock, _irq, &sp_flags);
	return hal_status;
}
#endif
...
enum rtw_hal_status
rtw_hal_mac_enable_rx_hdr_conv(struct hal_info_t *hal_info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	u32 mret = MACSUCCESS;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	void *drv_priv = hal_to_drvpriv(hal_info);
	struct mac_ax_rx_hdr_conv_cfg rx_hdr_conv_cfg
		= {.en = 1,
		   .chk_addr_cam_hit = 1,
		   .rx_hdr_conv_type = MAC_AX_RX_HDR_TYPE_ETH_II};
	struct mac_ax_rx_driv_info_hdr_cfg drv_info_cfg
		= {.en = 1,
		   .driv_info_hdr_type = MAC_AX_RX_DRIV_INFO_HDR_FULLY_HDR};

	if (mret == MACSUCCESS)
		mret = mac->ops->hdr_conv_rx_en_driv_info_hdr(mac, &drv_info_cfg);
	if (mret == MACSUCCESS)
		mret = mac->ops->hdr_conv_rx_en(mac, &rx_hdr_conv_cfg);

	if (mret != MACSUCCESS) {
		hstatus = RTW_HAL_STATUS_FAILURE;
		PHL_ERR("Failed to enabled RX header conversion. (%u)", mret);
	} else
		PHL_PRINT("Enabled RX header conversion.\n");

	return hstatus;
}

enum rtw_hal_status
rtw_hal_mac_disable_rx_hdr_conv(struct hal_info_t *hal_info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	u32 mret = MACSUCCESS;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	void *drv_priv = hal_to_drvpriv(hal_info);
	struct mac_ax_rx_hdr_conv_cfg rx_hdr_conv_cfg = {0};
	struct mac_ax_rx_driv_info_hdr_cfg drv_info_cfg = {0};
	...
	if (mret != MACSUCCESS) {
		hstatus = RTW_HAL_STATUS_FAILURE;
		PHL_ERR("Failed to Disabled RX header conversion. (%u)\n", mret);
	} else
		PHL_PRINT("Disabled RX header conversion.\n");

	return hstatus;
}

enum rtw_hal_status
rtw_hal_mac_enable_rx_amsdu_cut(struct hal_info_t *hal_info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	u32 mret = MACSUCCESS;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	...
	mret = mac->ops->enable_cut_hwamsdu(mac, 1, MAC_AX_BYTE_ALIGNED_4);
	if (mret == MACSUCCESS)
		mac->ops->cut_hwamsdu_chk_mpdu_len_en(mac, 0, 14, 1600);

	/* 9c40 bit30 set 0 , 9cf4 set 8 */
	/* Workaround RX A-MSDU cut length check issue before HALMAC fix it */
	do {
		u32 reg_val;

		reg_val = hal_read32(hal_info->hal_com, R_AX_MPDU_RX_ERR_IMR);
		reg_val &= ~B_AX_MHDRLEN_ERR_INT_EN;
		hal_write32(hal_info->hal_com, R_AX_MPDU_RX_ERR_IMR, reg_val);

		reg_val = hal_read32(hal_info->hal_com, R_AX_MPDU_RX_DBG);
		reg_val |= B_AX_MPDU_RX_CKEN_DIS;
		hal_write32(hal_info->hal_com, R_AX_MPDU_RX_DBG, reg_val);

	} while (0);

	if (mret != MACSUCCESS) {
		hstatus = RTW_HAL_STATUS_FAILURE;
		PHL_PRINT("Failed to enabled RX A-MSDU cut.\n");
	} else
		PHL_PRINT("Enabled RX A-MSDU cut.\n");

	return hstatus;
}

enum rtw_hal_status
rtw_hal_mac_disable_rx_amsdu_cut(struct hal_info_t *hal_info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	u32 mret = MACSUCCESS;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	...
	mret = mac->ops->enable_cut_hwamsdu(mac, 0, MAC_AX_BYTE_ALIGNED_4);

	if (mret != MACSUCCESS) {
		hstatus = RTW_HAL_STATUS_FAILURE;
		PHL_PRINT("Failed to disabled RX A-MSDU cut.\n");
	} else
		PHL_PRINT("Disabled RX A-MSDU cut.\n");
...
enum rtw_hal_status
rtw_hal_mac_prepare_fw_init(struct hal_info_t *hal_info, u32 dw0, u32 dw1)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	u32 mac_err;

	PHL_INFO("%s\n", __func__);
	mac_err = mac->ops->host_efuse_rec(mac, dw0, dw1);
...
void
rtw_hal_mac_set_dbglog_cfg(struct hal_info_t *hal, u8 cfg)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	u32 err = 0;

	err = mac->ops->set_hw_value(mac, MAC_AX_HW_SET_SER_DBG_LVL, &cfg);
