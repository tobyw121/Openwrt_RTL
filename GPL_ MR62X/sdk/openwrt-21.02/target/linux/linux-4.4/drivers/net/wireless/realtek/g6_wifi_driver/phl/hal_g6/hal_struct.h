...
struct hal_trx_ops {
	...
	u16 (*query_tx_res_sw)(struct rtw_hal_com_t *hal_com, struct tx_base_desc *txbd_ring,
				u8 dma_ch,u16 *host_idx, u16 *hw_idx);
	...
	enum rtw_hal_status
		(*check_next_msdu)(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal,
					struct rtw_rx_buf *rxbuf,
					u32 buf_size,
					struct rtw_phl_rx_pkt *rxpkt);
	void
		(*rewind_rx_tag)(struct rtw_phl_com_t *phl_com,
				 u8 ch_idx);
	...
	enum rtw_hal_status
		(*update_rxbd)(struct hal_info_t *hal,
				...
				struct rtw_rx_buf *rx_buf, u8 ch);
	...
	void (*query_freerun)(struct hal_info_t *hal,
						  u32 *freerun_l, u32 *freerun_h);
	#ifdef CONFIG_WFA_OFDMA_Logo_Test
	enum rtw_hal_status (*set_dlru_fix_tbl)(struct hal_info_t *hal, struct rtw_phl_dlru_fix_tbl *tbl);
	enum rtw_hal_status (*set_ulru_fix_tbl)(struct hal_info_t *hal, struct rtw_phl_ulru_fix_tbl *tbl);
	#endif

	#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
	enum rtw_hal_status (*trx_watchdog)(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal);
	...
	#ifdef CONFIG_WFA_OFDMA_Logo_Test
	void (*set_dlru_fix_tbl)(struct hal_info_t *hal, struct rtw_phl_dlru_fix_tbl *tbl);
	void (*set_ulru_fix_tbl)(struct hal_info_t *hal, struct rtw_phl_ulru_fix_tbl *tbl);
	#endif
...
struct hal_ops_t {
	...
	enum rtw_hal_status (*hal_get_efuse)(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal);
	enum rtw_hal_status (*hal_get_flash)(struct rtw_phl_com_t *phl_com,
						struct hal_info_t *hal);
	...
	void (*dbg_tx_rpt)(struct rtw_hal_com_t *phl_com, u32 rpt_cnt);
