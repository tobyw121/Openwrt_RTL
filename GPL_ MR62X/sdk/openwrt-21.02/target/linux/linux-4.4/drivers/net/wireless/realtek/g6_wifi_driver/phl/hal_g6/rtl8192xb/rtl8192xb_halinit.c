 ...
 *
 *****************************************************************************/
#define _RTL8192XB_HALINIT_C_
#include "../hal_headers.h"
#include "../hal_api.h"
#include "rtl8192xb_hal.h"


void init_hal_spec_8192xb(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal)
{
	...
	hal_com->def_rfpath_rx = 2;
	hal_com->def_rfpath_tx = 2;
	...
	if (hal_com->chip_id == CHIP_WIFI6_8192XB) {
		hal_spec->ic_name = "rtl8192xb";
		hal_spec->band_cap = BAND_CAP_2G;
		dev_hw_cap->band_sup = BAND_CAP_2G;
		dev_hw_cap->bw_sup = BW_CAP_20M | BW_CAP_40M;
	} else if (hal_com->chip_id == CHIP_WIFI6_8832BR) {
		hal_spec->ic_name = "rtl8832br";
		hal_spec->band_cap = BAND_CAP_5G;
		dev_hw_cap->band_sup = BAND_CAP_5G;
	...
	hal_spec->fw_dlru_grp_num = 32;
	hal_spec->fw_ulru_grp_num = 32;
	hal_spec->hw_dlru_grp_num = 16;
	hal_spec->hw_ulru_grp_num = 15;
	hal_spec->fw_ru_sta_num = 128;
	#endif
...
#else
	hal_com->dev_hw_cap.hw_hdr_conv = false;
#endif
	...
	hal_com->dev_hw_cap.hw_stype_cap = EFUSE_HW_STYPE_NONE_8192XB;
	...
	hal_com->dev_hw_cap.rx_amsdu_cut = true;
	#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT_NO_HDR_COV
	hal_com->dev_hw_cap.rx_hdr_conv = false;
	#else
	hal_com->dev_hw_cap.rx_hdr_conv = true;
	#endif
	#endif /* CONFIG_RTW_HW_RX_AMSDU_CUT */
}


void init_default_value_8192xb(struct hal_info_t *hal)
{
...
u32 _hal_cfg_rom_fw_8192xb(enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info,
			  char *ic_name)
{
	...
	_os_snprintf(fw_info->rom_path, MAX_PATH_LEN, "%s%s%s%s", hal_phy_folder,
		     ic_name, _os_path_sep, "rtl8192xbfw_rom.bin");
...
u32 _hal_cfg_intnal_fw_8192xb(struct rtw_phl_com_t *phl_com, enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info)
{
	/* any related to fw from header can be defined here */
	return RTW_HAL_STATUS_SUCCESS;
}

static u32 _hal_read_fw_8192xb(enum rtw_fw_type fw_type,
	u8 *tgt_buf, u32 *tgt_buf_size, char *ic_name)
{
	...
	_os_snprintf(path, MAX_PATH_LEN, "%s%s%s%s%s%s", hal_phy_folder,
		     ic_name, _os_path_sep, "RTL8192XBfw", filename_postfix, ".bin");
...
static u32 _hal_cfg_extnal_fw_8192xb(struct rtw_phl_com_t *phl_com,
	enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info, char *ic_name)
{
	...
	switch (fw_type) {
	case RTW_FW_NIC:
		_hal_read_fw_8192xb(RTW_FW_NIC, fw_info->buf,
			&fw_info->buf_size, ic_name);
		/* preload wowlan fw */
		_hal_read_fw_8192xb(RTW_FW_WOWLAN, fw_info->wow_buf,
	...
	case RTW_FW_WOWLAN:
		...
		break;
	case RTW_FW_SPIC:
		_hal_read_fw_8192xb(RTW_FW_SPIC, fw_info->buf,
			&fw_info->buf_size, ic_name);
		tgt_buf = fw_info->buf;
		tgt_buf_size = fw_info->buf_size;
		break;
	case RTW_FW_AP:
		_hal_read_fw_8192xb(RTW_FW_AP, fw_info->buf,
			&fw_info->buf_size, ic_name);
		tgt_buf = fw_info->buf;
		tgt_buf_size = fw_info->buf_size;
		break;
	...
	if (tgt_buf && (0 != tgt_buf_size)) {
		...
		fw_info->ram_size = 0;
		return _hal_read_fw_8192xb(fw_type, fw_info->ram_buff,
			&fw_info->ram_size, ic_name);
	}

}

enum rtw_hal_status hal_cfg_fw_8192xb(struct rtw_phl_com_t *phl_com,
				     struct hal_info_t *hal,
				     char *ic_name,
				     enum rtw_fw_type fw_type)
{
	...
	if (fw_cap->dlrom_en) {
		if (RTW_HAL_STATUS_SUCCESS !=_hal_cfg_rom_fw_8192xb(fw_type, fw_info,
			ic_name))
			goto init_fw_fail;
	} else {
		if (fw_info->rom_buff) {
			hal_mem_free(hal->hal_com, fw_info->rom_buff, RTW_MAX_FW_SIZE);
	...
	if (fw_cap->fw_src == RTW_FW_SRC_EXTNAL) {
		...
		if (fw_info->ram_buff == NULL) {
			fw_info->ram_buff = hal_mem_alloc(hal->hal_com, RTW_MAX_FW_SIZE);
			if (fw_info->ram_buff == NULL) {
				PHL_TRACE(COMP_PHL_DBG, _PHL_ERR_, "%s : Allocate ram_buff fail.\n", __func__);
				goto init_fw_fail;
			}
		}
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_extnal_fw_8192xb(phl_com, fw_type,
			fw_info, ic_name))
			goto init_fw_fail;
	} else if (fw_cap->fw_src == RTW_FW_SRC_INTNAL) {
		fw_info->fw_src = RTW_FW_SRC_INTNAL;
		if (fw_info->ram_buff) {
			hal_mem_free(hal->hal_com, fw_info->ram_buff, RTW_MAX_FW_SIZE);
			fw_info->ram_buff = NULL;
		}
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_intnal_fw_8192xb(phl_com, fw_type,
			fw_info))
			goto init_fw_fail;
	} else {
		goto init_fw_fail;
...
enum rtw_hal_status hal_get_efuse_8192xb(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal,
...
enum rtw_hal_status hal_get_flash_8192xb(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal,
					struct hal_init_info_t *init_info)
{
	/*enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
...
/*hal_power_off_fail:
hal_fast_init_fail:
	PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_, "==> %s : hal get flash fail\n", __func__);
	return hal_status;*/
}
#ifdef PHL_DIS_CTRL_FRAME_TO_HOST
static enum rtw_hal_status _hal_cfg_ctrl_rxfltr(struct rtw_hal_com_t *hal_com)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

#if (PHL_DIS_CTRL_FRAME_TO_HOST_POLICY == PHL_DIS_CTRL_FRAME_TO_HOST_ALL)
	hal_status = rtw_hal_mac_set_rxfltr_by_type(hal_com, HW_BAND_0, RTW_PHL_PKT_TYPE_CTRL, 0);
#elif (PHL_DIS_CTRL_FRAME_TO_HOST_POLICY == PHL_DIS_CTRL_FRAME_TO_HOST_POLICY_1)
	/* Disable B_AX_A_CTRL13 of R_AX_CTRL_FLTR for ACK */
	hal_status = rtw_hal_mac_set_rxfltr_by_subtype(hal_com, HW_BAND_0, RTW_PHL_PKT_TYPE_CTRL,
		13, 0);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s: Disable B_AX_A_CTRL13 fail(%d)!!\n", __func__, hal_status);
		return hal_status;
	}
	/* Disable B_AX_A_CTRL9 of R_AX_CTRL_FLTR for BA */
	hal_status = rtw_hal_mac_set_rxfltr_by_subtype(hal_com, HW_BAND_0, RTW_PHL_PKT_TYPE_CTRL,
		9, 0);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s: Disable B_AX_A_CTRL9 fail(%d)!!\n", __func__, hal_status);
		return hal_status;
	}
	/* Disable B_AX_A_CTRL11 of R_AX_CTRL_FLTR for RTS */
	hal_status = rtw_hal_mac_set_rxfltr_by_subtype(hal_com, HW_BAND_0, RTW_PHL_PKT_TYPE_CTRL,
		11, 0);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s: Disable B_AX_A_CTRL11 fail(%d)!!\n", __func__, hal_status);
		return hal_status;
	}
	/* Disable B_AX_A_CTRL12 of R_AX_CTRL_FLTR for CTS */
	hal_status = rtw_hal_mac_set_rxfltr_by_subtype(hal_com, HW_BAND_0, RTW_PHL_PKT_TYPE_CTRL,
		12, 0);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s: Disable B_AX_A_CTRL12 fail(%d)!!\n", __func__, hal_status);
		return hal_status;
	}
#endif
...
#endif
enum rtw_hal_status hal_start_8192xb(struct rtw_phl_com_t *phl_com,
				   struct hal_info_t *hal,
				   struct hal_init_info_t *init_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	...
	rtw_hal_set_rxfltr_by_mode(hal, HW_BAND_0,
...
#ifdef PHL_DIS_CTRL_FRAME_TO_HOST
	_hal_cfg_ctrl_rxfltr(hal->hal_com);
#endif
	...
	/* AP eFEM requirement to load extension parameter in subdir */
#if defined(CONFIG_RTW_ENABLE_SUB_TYPE) && defined(USE_TRUE_PHY)
	if (phl_com->dev_cap.rfe_type >= 50) {
		rtw_hal_dl_all_ext_para_file(phl_com, init_info->ic_name, hal);
		rtw_hal_init_bb_reg(hal);
		rtw_hal_init_rf_reg(phl_com, hal);
	}
#endif
	...
	rtw_hal_mac_set_append_fcs(hal, 0);
	...
	rtw_hal_mac_set_acpt_icv_err(hal, 0);
	...
	/* default set noise mode for open space */
	//0xCC24[12]=0 0xcc24[15:14]=1, follow RXSC/duplicate CTS
	hal_write32(hal->hal_com, R_AX_TRXPTCL_CTS_RRSR,
		(hal_read32(hal->hal_com, R_AX_TRXPTCL_CTS_RRSR)|BIT14)&(~BIT12));
	//0xCC08[11:10]=2, duplicate ACL/BA
	hal_write32(hal->hal_com, R_AX_TRXPTCL_RRSR_CTL_0,
		hal_read32(hal->hal_com, R_AX_TRXPTCL_RRSR_CTL_0)|BIT11);
...
enum rtw_hal_status hal_stop_8192xb(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
...
#ifdef CONFIG_WOWLAN
enum rtw_hal_status
hal_wow_init_8192xb(struct rtw_phl_com_t *phl_com,
...
enum rtw_hal_status
hal_wow_deinit_8192xb(struct rtw_phl_com_t *phl_com,
				struct hal_info_t *hal_info, struct rtw_phl_stainfo_t *sta,
					struct hal_init_info_t *init_info)
{
	struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
...
#ifdef RTW_PHL_BCN //fill 8192xb bcn ops

enum rtw_hal_status hal_config_beacon_8192xb(struct rtw_phl_com_t *phl_com,
                                            struct hal_info_t *hal,
                                            struct rtw_bcn_entry *bcn_entry)
{
	if (   hal_mac_ax_config_beacon(hal, phl_com->wifi_roles, bcn_entry)
...
enum rtw_hal_status hal_update_beacon_8192xb(struct rtw_phl_com_t *phl_com,
                                            struct hal_info_t *hal,
                                            struct rtw_bcn_entry *bcn_entry)
{
...
enum rtw_hal_status
hal_mp_init_8192xb(struct rtw_phl_com_t *phl_com,
					...
					struct hal_init_info_t *init_info)
{
	...
	fw_type = RTW_FW_NIC_MP;
#elif defined(PHL_FEATURE_AP)
	fw_type = RTW_FW_AP;
	//fw_type = RTW_FW_AP_MP;
#else
	fw_type  = RTW_FW_MAX;
#endif
...
enum rtw_hal_status
hal_mp_deinit_8192xb(struct rtw_phl_com_t *phl_com,
				struct hal_info_t *hal_info,
					struct hal_init_info_t *init_info)
{
	struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
