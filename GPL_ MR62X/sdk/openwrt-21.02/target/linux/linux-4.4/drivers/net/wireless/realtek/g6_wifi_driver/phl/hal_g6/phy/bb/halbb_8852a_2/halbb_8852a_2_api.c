...
void halbb_digital_cfo_comp_8852a_2(struct bb_info *bb, s32 curr_cfo)
{
	...
	BB_DBG(bb, DBG_CFO_TRK, "[8852A] cfo_avg_312 = %d\n", cfo_avg_312);
...
void halbb_gpio_setting_init_8852a_2(struct bb_info *bb)
{
	...
	halbb_gpio_trsw_table_8852a_2(bb, BB_PATH_A, 0, 1, 0, 1, 0);
	halbb_gpio_trsw_table_8852a_2(bb, BB_PATH_A, 0, 1, 1, 1, 0);
	...
	halbb_gpio_trsw_table_8852a_2(bb, BB_PATH_B, 0, 1, 1, 1, 0);
	...
	halbb_gpio_rfm_8852a_2(bb, BB_PATH_A, TRSW_RFM, 0, 0, 0, 0x4);
	halbb_gpio_rfm_8852a_2(bb, BB_PATH_A, LNAON_RFM, 0, 0, 0, 0x8);

	halbb_gpio_rfm_8852a_2(bb, BB_PATH_B, PAPE_RFM, 0, 0, 0, 0x0);
	halbb_gpio_rfm_8852a_2(bb, BB_PATH_B, TRSW_RFM, 0, 0, 0, 0x4);
	halbb_gpio_rfm_8852a_2(bb, BB_PATH_B, LNAON_RFM, 0, 0, 0, 0x8);
}
...
void halbb_restore_info_8852a_2(struct bb_info *bb, enum phl_phy_idx phy_idx)
{
	...
	BB_DBG(bb, DBG_PHY_CONFIG, "[Restore Info] [PHY%d] Rx path = %x\n", phy_idx, bb->bb_cmn_backup_i.cur_rx_path);
...
void halbb_ext_loss_avg_update_8852a_2(struct bb_info *bb)
{
...
void halbb_get_normal_efuse_init_8852a_2(struct bb_info *bb)
{
	...
	bb->bb_efuse_i.efuse_ofst[HW_PHY_0] = (s8)halbb_cnvrt_2_sign(tmp_ofst, 5);
	tmp_ofst = (u8)halbb_get_reg(bb, 0x4964, 0xfe00000);
	bb->bb_efuse_i.efuse_ofst_tb[HW_PHY_0] = (s8)halbb_cnvrt_2_sign(tmp_ofst, 7);
...
void halbb_set_efuse_8852a_2(struct bb_info *bb, u8 central_ch, enum rf_path path, enum phl_phy_idx phy_idx)
{
	...
	if (bb->bb_efuse_i.hidden_efuse_check) {
		for (path = RF_PATH_A; path < BB_PATH_MAX_8852A; path++) {
			...
			if (central_ch >= 0 && central_ch <= 14) {
				hidden_efuse = (s32)((gain << 2) + bb->bb_gain_i.efuse_ofst[band][path][0]);
	...
	if (bb->bb_efuse_i.normal_efuse_check) {
		...
		if (normal_efuse > 3) {
			...
			// r_1_rpl_bias_comp
			halbb_set_reg_cmn(bb, 0x494c, 0xf8000000, (bb->bb_efuse_i.efuse_ofst[HW_PHY_0] & 0x1f), phy_idx);
			// r_tb_rssi_bias_comp
			halbb_set_reg_cmn(bb, 0x4964, 0xfe00000, (bb->bb_efuse_i.efuse_ofst_tb[HW_PHY_0] & 0x7f), phy_idx);
			// r_1_rpl_bias_comp by BW
		...
		/*ext_loss*/
		bb->bb_ch_i.ext_loss[RF_PATH_A] = (s8)((((-1) * bb->bb_efuse_i.gain_offset[RF_PATH_A][band + 1]) << 2) +
						       (bb->bb_efuse_i.efuse_ofst[HW_PHY_0] >> 2));
		bb->bb_ch_i.ext_loss[RF_PATH_B] = (s8)((((-1) * bb->bb_efuse_i.gain_offset[RF_PATH_B][band + 1]) << 2) +
						       (bb->bb_efuse_i.efuse_ofst[HW_PHY_0] >> 2));
	...
	halbb_ext_loss_avg_update_8852a_2(bb);
}
				       ...
				       enum phl_phy_idx phy_idx)
{
	...
	if (rx_gain_offset > 3) {
		...
		tmp = (rx_gain_offset << 4) + (bb->bb_efuse_i.efuse_ofst[HW_PHY_0] << 2) - upper_bound;
		// r_1_rpl_bias_comp
		halbb_set_reg_cmn(bb, 0x494c, 0xf8000000, ((tmp >> 2) & 0x1f), phy_idx);
		// r_tb_rssi_bias_comp
		halbb_set_reg_cmn(bb, 0x4964, 0xfe00000, (tmp & 0x7f), phy_idx);
		...
		tmp = (rx_gain_offset << 4) + (bb->bb_efuse_i.efuse_ofst[HW_PHY_0] << 2) + lower_bound;
		// r_1_rpl_bias_comp
		halbb_set_reg_cmn(bb, 0x494c, 0xf8000000, ((tmp >> 2) & 0x1f), phy_idx);
		// r_tb_rssi_bias_comp
		halbb_set_reg_cmn(bb, 0x4964, 0xfe00000, (tmp & 0x7f), phy_idx);
