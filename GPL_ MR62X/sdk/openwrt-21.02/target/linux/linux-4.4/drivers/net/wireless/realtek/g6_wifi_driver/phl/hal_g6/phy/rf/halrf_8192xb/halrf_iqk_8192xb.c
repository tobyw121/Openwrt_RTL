...
__iram_func__
static void _iqk_read_fft_8192xb(struct rf_info *rf, u8 path)
{
#if 0
	u8 i = 0x0;
	u32 fft[6] = {0x0};
...
__iram_func__
static void _iqk_read_xym_8192xb(struct rf_info *rf, u8 path)
{
...
__iram_func__
static void _iqk_read_txcfir_8192xb(struct rf_info *rf, u8 path, u8 group)
{
...
__iram_func__
static void _iqk_read_rxcfir_8192xb(struct rf_info *rf, u8 path, u8 group)
{
...
__iram_func__
static bool _iqk_check_nbiqc_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,  u8 ktype, u8 path)
{
	u32 x = 0;
	u32 y = 0;
...
__iram_func__
static void _iqk_sram_8192xb(struct rf_info *rf, u8 path)
{
...
__iram_func__
static void _iqk_rxk_setting_8192xb(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	case CHANNEL_WIDTH_40:		
		...
		if (hal_i->cv == CAV) {
			halrf_rxck_force_8192xb(rf, path, true, ADC_480M);
	...
	//		halrf_wreg(rf, 0xc0d8 + (path << 8), 0x000001e0, 0xf);
			...
			halrf_rxck_force_8192xb(rf, path, true, ADC_960M);
			halrf_wreg(rf, 0x5670 + (path << 13), 0x60000000, 0x0);
	//		halrf_wreg(rf, 0xc0d4 + (path << 8), 0x0c000000, 0x2);
	//		halrf_wreg(rf, 0xc0d8 + (path << 8), 0x000001e0, 0xf);
...
__iram_func__
static bool _iqk_check_cal_8192xb(struct rf_info *rf, u8 path, u8 ktype)
{
...
__iram_func__
static bool _iqk_one_shot_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path, u8 ktype)
{
...
#if 1
	RF_DBG(rf, DBG_RF_IQK, "[IQK] CMD_ID = %x\n",  iqk_cmd + 1);
	halrf_wreg(rf, 0x8000, MASKDWORD, iqk_cmd + 1);
	//halrf_delay_us(rf, 1);

	halrf_delay_us(rf, 1);
	fail = _iqk_check_cal_8192xb(rf, path, ktype);
#else
		halrf_delay_us(rf, 10);
		fail = halrf_do_one_shot_8192xb(rf, path, 0x8000, MASKDWORD, iqk_cmd + 1);
	...
	case ID_NBTXK:
		if (iqk_info->iqk_xym_en == true)
			_iqk_read_xym_8192xb(rf, path);
		if (iqk_info->iqk_cfir_en == true) {
			_iqk_read_txcfir_8192xb(rf, path, 0x1);
			_iqk_read_txcfir_8192xb(rf, path, 0x2);
			_iqk_read_txcfir_8192xb(rf, path, 0x3);
		}
		break;
	case ID_RXK:
	case ID_NBRXK:		
		if (iqk_info->iqk_sram_en == true)
			_iqk_sram_8192xb(rf, path);
		if (iqk_info->iqk_xym_en == true)
			_iqk_read_xym_8192xb(rf, path);
		if (iqk_info->iqk_cfir_en == true) {
			_iqk_read_rxcfir_8192xb(rf, path, 0x0);
			_iqk_read_rxcfir_8192xb(rf, path, 0x1);
			_iqk_read_rxcfir_8192xb(rf, path, 0x2);
			_iqk_read_rxcfir_8192xb(rf, path, 0x3);
		}
		break;
	default:
		break;
...
__iram_func__
static bool _rxk_group_sel_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{

	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 gp = 0x0;
	u32 g_idxrxgain[4] = {0x150, 0x158, 0x2D0, 0x2D8};
	u32 g_idxattc2[4] = {0x0, 0x0, 0x0, 0x0};
	...
	case BAND_ON_24G:
		...
		halrf_wrf(rf, path, 0x84, 0xf0000, 0xa);
	break;
	case BAND_ON_5G:
	break;	
	case BAND_ON_6G:
	...
	for (gp = 0; gp < 0x4; gp++) {
		...
		fail = _iqk_one_shot_8192xb(rf, phy_idx, path, ID_RXAGC);
		halrf_delay_us(rf,10);		
		RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, after_agc 0x00 = %x\n", path, halrf_rrf(rf, path, 0x00, MASKDWORD));
#endif
		fail = _iqk_one_shot_8192xb(rf, phy_idx, path, ID_RXK);
		halrf_wreg(rf, 0x9fe0, BIT(16 + gp + path * 4), fail);
		kfail = kfail | fail;
	}
	if(path == RF_PATH_B)
		halrf_wrf(rf, RF_PATH_B, 0xdc, 0x00300, 0x0);
	if (kfail) {
...
__iram_func__
static bool _iqk_nbrxk_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
			      u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 gp = 0x0;
	...
	halrf_delay_us(rf, 100);
	halrf_wreg(rf, 0x802c, 0x0fff0000, 0x11);
	fail = _iqk_one_shot_8192xb(rf, phy_idx, path, ID_NBRXK);
	halrf_wreg(rf, 0x9fe0, BIT(16 + gp + path * 4), fail);
	kfail = _iqk_check_nbiqc_8192xb(rf, phy_idx, ID_NBRXK, path);
...
__iram_func__
static bool _txk_group_sel_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{
	...
	u32 g_gain_bb[4] = {0x14, 0x14, 0x08, 0x0e};
	u32 g_itqt[4] = {0x12, 0x12, 0x12, 0x12};
	
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);		
	for (gp = 0x1; gp < 0x4; gp++) {
		switch (iqk_info->iqk_band[path]) {
		case BAND_ON_24G:
			...
			halrf_wreg(rf, 0x81cc + (path << 8), 0x0000003f, g_itqt[gp]);
			break;
		case BAND_ON_5G:
		...
		fail = _iqk_one_shot_8192xb(rf, phy_idx, path, ID_TXK);
...
__iram_func__
static bool _iqk_nbtxk_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
			      u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool fail = false;
	...
	u32 g_power_range[4] = {0x0, 0x0, 0x0, 0x0};
	u32 g_track_range[4] = {0x4, 0x4, 0x6, 0x6};
	u32 g_gain_bb[4] = {0x0e, 0x0e, 0x08, 0x0e};
	u32 g_itqt[4] = {0x09, 0x09, 0x12, 0x12};
	...
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	fail = _iqk_one_shot_8192xb(rf, phy_idx, path, ID_NBTXK);
	kfail = _iqk_check_nbiqc_8192xb(rf, phy_idx, ID_NBTXK, path);
	if (!kfail) {
...
__iram_func__
static void _lok_res_table_8192xb(struct rf_info *rf, u8 path, u8 ibias)
{
#if 0
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
static bool _lok_finetune_check_8192xb(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
static bool _iqk_lok_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
			    u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	//bool fail = false;
...
//============START : _lok_coarse_fine_wi_swap ========
	...
	case BAND_ON_24G:
		...
		halrf_wreg(rf, 0x81cc + (path << 8), MASKDWORD, 0x09);
		break;
	case BAND_ON_5G:
		break;
	case BAND_ON_6G:
	...
	tmp = _iqk_one_shot_8192xb(rf, phy_idx, path, ID_G_FLoK_coarse);
...
//Step 2
	...
	tmp = _iqk_one_shot_8192xb(rf, phy_idx, path, ID_FLOK_vbuffer);
...
//Step 3
	...
	tmp = _iqk_one_shot_8192xb(rf, phy_idx, path, ID_G_FLoK_fine);
...
//Step 4 large rf gain
	...
	isfail = _lok_finetune_check_8192xb(rf, path);
	return isfail;
}

__iram_func__
static void _iqk_txk_setting_8192xb(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
static void _iqk_txclk_setting_8192xb(struct rf_info *rf, u8 path)
{
#if 0
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
...
__iram_func__
static void _iqk_info_iqk_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path)
{
	...
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	iqk_info->thermal[path] = halrf_get_thermal_8192xb(rf, path);	
...
__iram_func__
static void _iqk_disable_rxagc_8192xb(struct rf_info *rf,u8 path, u8 en_rxgac)
{
...
__iram_func__
static void _iqk_by_path_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
				u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	//LOK
	for (i = 0; i < 1; i++) {
		_iqk_txk_setting_8192xb(rf, path);
		lok_is_fail = _iqk_lok_8192xb(rf, phy_idx, path);
		if (!lok_is_fail)			
			break;
	}
	...
	if (iqk_info->is_nbiqk) {
		iqk_info->iqk_tx_fail[0][path] =
			_iqk_nbtxk_8192xb(rf, phy_idx, path);
	} else {
		iqk_info->iqk_tx_fail[0][path] =
			_txk_group_sel_8192xb(rf, phy_idx, path);
	}
	//RX
	_iqk_rxk_setting_8192xb(rf, path);
	if (iqk_info->is_nbiqk) {
		iqk_info->iqk_rx_fail[0][path] =
			_iqk_nbrxk_8192xb(rf, phy_idx, path);
	} else {
		iqk_info->iqk_rx_fail[0][path] =
			_rxk_group_sel_8192xb(rf, phy_idx, path);
	}
#endif
	_iqk_info_iqk_8192xb(rf, phy_idx, path);
	return;
}

__iram_func__
void iqk_backup_rf0_8192xb(
	struct rf_info *rf, u8 path,
	u32 backup_rf0[rf_reg_num_8192xb],
	u32 backup_rf_reg0[rf_reg_num_8192xb])
{
	u8 i;
...
__iram_func__
void iqk_backup_rf1_8192xb(
	struct rf_info *rf, u8 path,
	u32 backup_rf1[rf_reg_num_8192xb],
	u32 backup_rf_reg1[rf_reg_num_8192xb])
{
	u8 i;
...
__iram_func__
void iqk_set_info_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path)
{
...
bool iqk_mcc_page_sel_8192xb(struct rf_info *rf, enum phl_phy_idx phy, u8 path)
{
	...
	if (rf->hal_com->band[phy].cur_chandef.center_ch ==  iqk_info->iqk_mcc_ch[0][path]) {
		...
		_iqk_mcc_load_lok_8192xb(rf, phy, path, iqk_info->lok_idac[0][path]);
		flag = true;
		...
		_iqk_mcc_load_lok_8192xb(rf, phy, path, iqk_info->lok_idac[1][path]);
		flag = true;
	} else
		flag = false;
#endif	
...
void iqk_get_ch_info_8192xb(struct rf_info *rf, enum phl_phy_idx phy, u8 path)
{
	...
	halrf_wreg(rf, 0x9fe0, 0xff000000, iqk_version_8192xb);
	//2G5G6G = 0/1/2
	...
	ver = (u8) halrf_get_8192xb_nctl_reg_ver();
...
__iram_func__
void iqk_start_iqk_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{

	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	_iqk_by_path_8192xb(rf, phy_idx, path);
	return;
}
...
void iqk_restore_8192xb(struct rf_info *rf, u8 path)
{
	...
	fail = _iqk_check_cal_8192xb(rf, path, 0x12);
#else
	fail = _iqk_one_shot_8192xb(rf, 0x0, path, ID_IQK_Restore);
#endif
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
...
void iqk_afebb_restore_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{
	...
	//enable rxgac
	_iqk_disable_rxagc_8192xb(rf, path, 0x1);
	return;
}
...
void iqk_macbb_setting_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{
	...
	//disable rxgac
	_iqk_disable_rxagc_8192xb(rf, path, 0x0);
...
void halrf_iqk_toneleakage_8192xb(struct rf_info *rf, u8 path)
{
	...
	iqk_get_ch_info_8192xb(rf, HW_PHY_0, path);
	iqk_macbb_setting_8192xb(rf, HW_PHY_0, path);	
	iqk_preset_8192xb(rf, path);
...
void halrf_iqk_dbcc_8192xb(struct rf_info *rf, u8 path)
{
	...
	//iqk_mcc_page_sel_8192xb(rf,phy_idx, path);		
	iqk_get_ch_info_8192xb(rf,phy_idx, path);
	iqk_macbb_setting_8192xb(rf, phy_idx, path);
	iqk_preset_8192xb(rf, path);
	iqk_start_iqk_8192xb(rf, phy_idx, path);
	iqk_restore_8192xb(rf, path);
	iqk_afebb_restore_8192xb(rf, phy_idx, path);	
	...
	struct rf_info *rf)
{
	...
	for (path = 0; path < 1; path++) {
		cur_ther = halrf_get_thermal_8192xb(rf, path);
...
bool halrf_check_fwiqk_done_8192xb(struct rf_info *rf)
{
	...
	if(flag) {
		...
		halrf_iqk_tx_bypass_8192xb(rf, RF_PATH_A);
		halrf_iqk_tx_bypass_8192xb(rf, RF_PATH_B);
		halrf_iqk_rx_bypass_8192xb(rf, RF_PATH_A);
		halrf_iqk_rx_bypass_8192xb(rf, RF_PATH_B);
		halrf_iqk_lok_bypass_8192xb(rf, RF_PATH_A);		
		halrf_iqk_lok_bypass_8192xb(rf, RF_PATH_B);
	}
...
void iqk_init_8192xb(struct rf_info *rf)
{
	...
	if (!iqk_info->is_iqk_init) {		
		...
		for (idx = 0; idx < 2; idx++) { //channel		
			...
			for (path = 0; path < ss_8192xb; path++) {//path
...
#if 0
__iram_func__
void halrf_doiqk_8192xb(struct rf_info *rf, bool force, enum phl_phy_idx phy_idx, u8 path)
{
	...
	iqk_info->version = iqk_version_8192xb;
	iqk_info->iqk_step = 0;
	...
	if (iqk_mcc_page_sel_8192xb(rf,phy_idx, path))
		return;
	iqk_get_ch_info_8192xb(rf,phy_idx, path);
	iqk_backup_mac_bb_8192xb(rf, path, backup_mac, backup_bb, backup_mac_reg, backup_bb_reg);
	iqk_backup_rf0_8192xb(rf, path, backup_rf0, backup_rf_reg0);
	iqk_backup_rf1_8192xb(rf, path, backup_rf1, backup_rf_reg1);
	iqk_macbb_setting_8192xb(rf, phy_idx, path);
	iqk_preset_8192xb(rf, path);
	iqk_start_iqk_8192xb(rf, phy_idx, path);
	iqk_restore_8192xb(rf, path);
	iqk_afebb_restore_8192xb(rf, phy_idx, path);	
	iqk_restore_mac_bb_8192xb(rf, path, backup_mac, backup_bb, backup_mac_reg, backup_bb_reg);
	iqk_restore_rf0_8192xb(rf, path, backup_rf0, backup_rf_reg0);
	iqk_restore_rf1_8192xb(rf, path, backup_rf1, backup_rf_reg1);
	iqk_info->iqk_times++;
...
void halrf_iqk_8192xb(struct rf_info *rf, enum phl_phy_idx phy_idx, bool force)
{
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	iqk_init_8192xb(rf);
	switch(halrf_kpath_8192xb(rf, phy_idx)) {
	case RF_A:
		halrf_doiqk_8192xb(rf, force, phy_idx, RF_PATH_A);
		break;
	case RF_B:		
		halrf_doiqk_8192xb(rf, force, phy_idx, RF_PATH_B);
		break;
	case RF_AB:		
		halrf_doiqk_8192xb(rf, force, phy_idx, RF_PATH_A);
		halrf_doiqk_8192xb(rf, force, phy_idx, RF_PATH_B);
