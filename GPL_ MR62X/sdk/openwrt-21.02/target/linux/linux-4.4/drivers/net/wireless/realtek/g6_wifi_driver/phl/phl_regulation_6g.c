			...
			u16 domain, enum regulation_rsn reason)
{
	...
	if (domain == RSVD_DOMAIN) {
		...
		rg->domain_6g.code = rdmap6[did < MAX_RD_MAP_NUM_6GHZ ? did : 0].domain_code;
		regulation = rdmap6[did < MAX_RD_MAP_NUM_6GHZ ? did : 0].regulation;
		ch_idx = rdmap6[did < MAX_RD_MAP_NUM_6GHZ ? did : 0].ch_idx;
