...
#define CONFIG_PPP_IDLE_TIMEOUT_REFINE
#endif
...
static void rtl_ppp_shutdown_interface(struct ppp *ppp);
...
static int ppp_release(struct inode *unused, struct file *file)
{
	...
	if (pf) {
		...
		if (pf->kind == INTERFACE) {
			...
			if (file == ppp->owner)
			#ifdef CONFIG_RTL_819X
				rtl_ppp_shutdown_interface(ppp);
			#else
...
extern int set_pppoe_info(struct net_device *ppp0_dev, struct net_device *wan_dev, unsigned short sid,
							unsigned int our_ip,unsigned int	peer_ip,
							unsigned char * our_mac, unsigned char *peer_mac);

extern int clear_pppoe_info(struct net_device *ppp0_dev, struct net_device *wan_dev, unsigned short sid,
								unsigned int our_ip,unsigned int	peer_ip,
								unsigned char * our_mac, unsigned char *peer_mac);
extern int get_pppoe_last_rx_tx(struct net_device *ppp0_dev, struct net_device *wan_dev, unsigned short sid,
									unsigned int our_ip,unsigned int peer_ip,
									unsigned char * our_mac,unsigned char * peer_mac,
...
extern void clear_magicNum(char *);
#else
...
#if defined (CONFIG_RTL_FAST_PPPOE)
				set_pppoe_info(pch->ppp->dev, local_dev, htons(po->pppoe_pa.sid),
...
#if defined (CONFIG_RTL_FAST_PPPOE)
				clear_pppoe_info(pch->ppp->dev, NULL, 0, 0,0,NULL, NULL); 
#endif

#if defined (CONFIG_RTL_PPPOE_DIRECT_REPLY)
#ifdef CONFIG_RTL_ISP_MULTI_WAN_SUPPORT
				//panic_printk("%s:%d dev_name=%s!\n",__FUNCTION__,__LINE__,dev_name);
				clear_magicNum(dev_name); 
#else
				clear_magicNum(); 
#endif
#endif
	...
	case PPPIOCGIDLE:
...
#if defined (CONFIG_RTL_FAST_PPPOE)
		{
			...
			if(fast_pppoe_fw)
			{
				if(ppp->dev!=NULL)
				{
					get_pppoe_last_rx_tx(ppp->dev,NULL,0,0,0,NULL,NULL,&fast_pppoe_last_rx,&fast_pppoe_last_tx);
...
#endif
{
	...
	if (fast_pptp_fw) {
		...
		extern int fast_pptp_to_wan(struct sk_buff *skb);
		if (skb->protocol == htons(ETH_P_IP) && is_pptp_device(ppp->dev->name) && fast_pptp_to_wan((skb)))	// sync from voip customer for multiple ppp
			return 0;
	}
	#endif
#else
...
#ifdef CONFIG_RTL_819X
//only forward packet or dns request packet will trigger ppp dail
int limit_trigger_packet(struct sk_buff *skb, int offset)
{
	int ret_val=0;
	if (skb->sk)  //packet send from DUT
	{	
		int oft=offset-2;
		
		if ( (((u8*)skb->data)[0+oft]==0x00 && ((u8*)skb->data)[1+oft] == 0x21)    /* IPV4 */
		&& (((u8*)skb->data)[11+oft]==0x11) 					/* UDP */
		&& (((u8*)skb->data)[24+oft]==0x00 && ((u8*)skb->data)[25+oft] == 0x35) /* DNS query*/
		)
		{			
			//printk("\n%s:%d send IPV4 dns request!!!\n",__FUNCTION__,__LINE__);
			ret_val=0;	
		}
		else if ( (((u8*)skb->data)[0+oft]==0x00 && ((u8*)skb->data)[1+oft] == 0x57)    /* IPV6 */
		&& (((u8*)skb->data)[8+oft]==0x11) 					/* UDP */
		&& (((u8*)skb->data)[44+oft]==0x00 && ((u8*)skb->data)[45+oft] == 0x35) /* DNS query*/
		)
		{			
			//printk("\n%s:%d send IPV6 dns request!!!\n",__FUNCTION__,__LINE__);
			ret_val=0;
		}
		else
			ret_val=1;			
...
#if defined(CONFIG_PPP_IDLE_TIMEOUT_REFINE)
int timeoutCheck_skipp_pkt(struct sk_buff *skb, int offset)
{
	return limit_trigger_packet(skb, offset);
}
...
#endif
{
#ifdef CONFIG_RTL_819X
	int offset=2;
	int proto, addr_ctrl_flag=0;
	if(skb->data[0]==0xff && skb->data[1]==0x03)
	{
		proto = PPP_PROTOCOL(skb->data);
		addr_ctrl_flag=1;
		offset+=2;
	}
	else			
	...
	if ((ppp->xstate & SC_COMP_RUN) && ppp->xc_state &&
		...
		#ifdef CONFIG_RTL_819X
		//in function ppp->xcomp->compress(), it input skb->data don't include A/C bytes
		if(addr_ctrl_flag==1)
			skb_pull(skb, 2);		
		#endif
...
#ifdef CONFIG_RTL_819X
		if(limit_trigger_packet(skb, offset))
			goto drop;
#endif		
...
#ifdef FAST_L2TP
extern void set_ppp_magic_info(char *ppp_device, unsigned int magicNum);

void set_ppp_device(struct channel *pch, struct sk_buff *skb)
{
	int proto;
	...
	if(fast_l2tp_fw)
	{
		...
		{
			cp=skb->data+2;
			if(*cp==0x01) //lcp configure request
			{
				payloadLen = ntohs(*((unsigned short *)(cp+2)));
				...
				while(payloadLen>0)
				{
					...
					//printk("%s:%d type is %d\n",__FUNCTION__,__LINE__,type);
					if(type==0x05) /*magic number option*/
					{
						memcpy(&magicNum, cp+2 , 4);					
						//printk("%s:%d  ppp_name=%s magic number is 0x%08x\n",__FUNCTION__,__LINE__,pch->ppp->dev->name, ntohl(magicNum));		
						break;
					}
					if(len>payloadLen)
					{
			...
			set_ppp_magic_info(pch->ppp->dev->name, magicNum);
		}
	}
}
#endif
...
#endif
{
	...
	if (npi < 0) {
		...
		if ((ppp->dev->flags & IFF_UP) == 0 ||
			...
			/* Now skb->data is pointed to ipv4 header,
...
EXPORT_SYMBOL_GPL(ppp_channel_pppoe);
#endif
...
#if defined(CONFIG_RTL_LAYERED_DRIVER)&&defined(CONFIG_RTL_LAYERED_DRIVER_L3)
	rtl_del_ps_drv_netif_mapping(ppp->dev->name);
#endif

#if defined (CONFIG_RTL_FAST_PPPOE)
	clear_pppoe_info(ppp->dev, NULL, 0,0,0,NULL, NULL);

#if defined (CONFIG_RTL_PPPOE_DIRECT_REPLY)
#ifdef CONFIG_RTL_ISP_MULTI_WAN_SUPPORT
	//panic_printk("%s:%d ppp->dev->name=%s!\n",__FUNCTION__,__LINE__,ppp->dev->name);
	clear_magicNum(ppp->dev->name); 
#else
	clear_magicNum(); 
#endif
#endif
