		   ...
		   enum phl_phy_idx phy_idx)
{
	...
	#ifdef BB_8192XB_SUPPORT
	case BB_RTL8192XB:
		rpt = halbb_ctrl_bw_8192xb(bb, pri_ch, bw, phy_idx);
		break;
	#endif
		      ...
		      enum channel_width bw, enum phl_phy_idx phy_idx)
{
	...
	case BB_RTL8852C:
		...
		if (halbb_check_fw_ofld(bb)) {
			rpt = halbb_fwofld_ctrl_bw_ch_8852c(bb, pri_ch, central_ch_seg0, bw,
										band, phy_idx);
		} else {
			...
			char *output, u32 *_out_len)
{
	...
	#ifdef BB_8852C_SUPPORT
	case BB_RTL8852C:
		//halbb_rx_op1db_dbg_8852c(bb, is_amode, path);
		break;
	#endif

	#ifdef BB_8852B_SUPPORT
	case BB_RTL8852B:
		halbb_rx_op1db_dbg_8852b(bb, input, _used, output, _out_len);
		break;
	#endif
...
void halbb_fix_rx_htmcs0(struct bb_info *bb, bool en, enum phl_phy_idx phy_idx)
{
	switch (bb->ic_type) {

	#ifdef BB_8852C_SUPPORT
	case BB_RTL8852C:
		halbb_fix_rx_htmcs0_8852c(bb, en, phy_idx);
		break;
	#endif

	#ifdef BB_8192XB_SUPPORT
	case BB_RTL8192XB:
		halbb_fix_rx_htmcs0_8192xb(bb, en, phy_idx);
		break;
	#endif
			...
			char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], "-h") == 0) {
		...
		HALBB_SCAN(input[2], DCMD_DECIMAL, &val[0]);
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[1]);
		halbb_fix_rx_htmcs0(bb, (bool)val[0], (enum phl_phy_idx)val[1]);
	#ifdef BB_8852B_SUPPORT
