...
#define DMAC_TBL_SIZE_1115E	0x20
...
u32 sec_info_tbl_init_1115e(struct mac_ax_adapter *adapter, u8 op_mode)
{
	u8 i = 0;
...
u32 free_sec_info_tbl_1115e(struct mac_ax_adapter *adapter, u8 op_mode)
{
	u8 i;
...
u32 fill_sec_cam_info_1115e(struct mac_ax_adapter *adapter,
...
u32 mac_upd_sec_infotbl_1115e(struct mac_ax_adapter *adapter,
...
u8 check_key_index_1115e(u8 addr_cam_sec_mode, u8 key_type, u8 key_index)
{
...
u8 decide_key_index_1115e(u8 addr_cam_sec_mode, u8 key_type)
{
	u8 key_index = 0;
...
u8 decide_sec_cam_index_1115e(struct mac_ax_adapter *adapter, u8 *sec_cam_idx)
{
	u8 sec_idx = 0, i = 0;
	/* call by pointer */
	struct sec_cam_table_t **sec_cam_table =
...
u8 delete_key_from_dmac_tbl_1115e(struct mac_ax_adapter *adapter,
				  u8 mac_id, u8 key_type,
				  u8 key_id, u8 *sec_cam_idx)
{
	struct fwcmd_dctlinfo_ud_v1 dctlinfo_v1;
	u8 key_index = 0, key_valid_byte = 0, i = 0, key_id_sh = 0;
	u8 sec_ent_mode = 0, key_valid_byte_ori = 0;
	u8 key_valid[7] = {0}, hit_flag = 0, key_idx_sh = 0;
	u8 sec_ent_keyid[7] = {0}, sec_cam_index[7] = {0};
	u32 ret = 0, dmac_tbl_addr = 0, key_cam_idx_sh = 0;
	u32 DMAC_TBL[8] = {0};

//	key_valid_byte = role->info.a_info.sec_ent_valid;
	dmac_tbl_addr = mac_id * DMAC_TBL_SIZE_1115E;
	for (i = 0; i < 7; i++) {
		DMAC_TBL[i] = mac_sram_dbg_read(adapter, dmac_tbl_addr,
						DMAC_TBL_SEL);
		dmac_tbl_addr += 4;
	}

	//sec_ent_mode = (DMAC_TBL[4] >> SEC_MODE_SH) & SEC_MODE_MSK;
	sec_ent_mode = SEC_DEFAULT_MODE;

	key_valid_byte = DMAC_TBL[5] & KEY_VALID_MSK;
	key_valid_byte_ori = key_valid_byte;

	key_id_sh = SEC_ENT0_KEYID_SH;
	for (i = 0; i < 7; i++) {
		sec_ent_keyid[i] = (DMAC_TBL[4]  >> key_id_sh) & KEYID_MSK;
		key_id_sh += 2;
	}

	key_index = decide_key_index_1115e(sec_ent_mode, key_type);
	...
	key_cam_idx_sh = 0;
	for (i = 0; i < DTBL_DWORD5_KEY_NUM; i++) {
		key_cam_idx_sh = (i + 1) * KEY_CAM_IDX_SH;
		sec_cam_index[i] =
			(DMAC_TBL[5] >> key_cam_idx_sh) &
			KEY_CAM_IDX_MSK;
	}

	key_cam_idx_sh = 0;
	for (i = DTBL_DWORD5_KEY_NUM; i < DTBL_DWORD6_KEY_NUM; i++) {
		key_cam_idx_sh = (i - DTBL_DWORD5_KEY_NUM) * KEY_CAM_IDX_SH;
		sec_cam_index[i] =
			(DMAC_TBL[6] >> key_cam_idx_sh) &
			KEY_CAM_IDX_MSK;
	}
	...
	*sec_cam_idx = sec_cam_index[key_index];

	for (i = 0; i < 7; i++) {
		if (i == key_index) {
			key_id_sh = SEC_ENT0_KEYID_SH + key_index * 2;
			DMAC_TBL[4] &= ~(u32)(KEYID_MSK << key_id_sh);
		}
	}

	//role->info.a_info.sec_ent_keyid[key_index] = key_id;
	DMAC_TBL[5] &= ~(BIT(key_index));

	for (i = 0; i < 7; i++) {
		if (i == key_index) {
			if (i < 3) {
				key_idx_sh = KEY_CAM_IDX_SH + key_index * 8;
				DMAC_TBL[5] &= ~(u32)(KEY_CAM_IDX_MSK <<
						      key_idx_sh);
			} else {
				key_idx_sh = (key_index - 3) * 8;
				DMAC_TBL[6] &= ~(u32)(KEY_CAM_IDX_MSK <<
						      key_idx_sh);
			}
		}
	}

	dctlinfo_v1.dword1 = DMAC_TBL[0];
	dctlinfo_v1.dword2 = DMAC_TBL[1];
	dctlinfo_v1.dword3 = DMAC_TBL[2];
	dctlinfo_v1.dword4 = DMAC_TBL[3];
	dctlinfo_v1.dword5 = DMAC_TBL[4];
	dctlinfo_v1.dword6 = DMAC_TBL[5];
	dctlinfo_v1.dword7 = DMAC_TBL[6];

	ret = dctl_info_debug_write_1115e(adapter, &dctlinfo_v1, mac_id);

	return MACSUCCESS;
}

u8 insert_key_to_dmac_tbl_1115e(struct mac_ax_adapter *adapter,
				u8 mac_id, u8 key_type,
				u8 key_id, u8 sec_cam_idx)
{
	struct fwcmd_dctlinfo_ud_v1 dctlinfo_v1;
	u8 key_index = 0, key_valid_byte = 0, i = 0, keyid_sh = 0;
	u8 sec_ent_mode = 0, key_valid_byte_ori = 0;
	u8 key_valid[7] = {0}, hit_flag = 0, key_idx_sh = 0;
	u32 ret = 0, dmac_tbl_addr = 0;
	u32 DMAC_TBL[7] = {0};
...
//	key_valid_byte = role->info.a_info.sec_ent_valid;
	...
	//debug
	for (i = 0; i < 7; i++) {
		PLTFM_MSG_TRACE("DMAC_TBL[%d]: %d\n", i, DMAC_TBL[i]);
		PLTFM_MSG_TRACE("key_valid[%d]: %d\n", i, key_valid[i]);
	}
	PLTFM_MSG_TRACE("key index: %d\n", key_index);
	PLTFM_MSG_TRACE("key_valid_byte: %d\n", key_valid_byte);

	if (check_key_index_1115e(sec_ent_mode, key_type, key_index)) {
	...
	for (i = 0; i < 7; i++) {
		if (i == key_index) {
			keyid_sh = SEC_ENT0_KEYID_SH + key_index * 2;
			DMAC_TBL[4] &= (u32)(~(KEYID_MSK << keyid_sh));
			DMAC_TBL[4] |= (u32)((key_id & KEYID_MSK) <<
					     keyid_sh);
		}
	}

	DMAC_TBL[4] |= (u32)((SEC_DEFAULT_MODE & KEYID_MSK) << SEC_MODE_SH);
	...
	for (i = 0; i < 7; i++) {
		if (i == key_index) {
			if (i < 3) {
				key_idx_sh = KEY_CAM_IDX_SH + key_index * 8;
				DMAC_TBL[5] |= ((sec_cam_idx & KEY_CAM_IDX_MSK)
						<< key_idx_sh);
			} else {
				key_idx_sh = (key_index - 3) * 8;
				DMAC_TBL[6] |= ((sec_cam_idx & KEY_CAM_IDX_MSK)
						<< key_idx_sh);
			}
		}
	}
...
u32 m_security_cam_hal_1115e(struct mac_ax_adapter *adapter,
...
u32 disconnect_flush_key_1115e(struct mac_ax_adapter *adapter,
			       struct mac_role_tbl *role)
{
	struct mac_ax_sec_cam_info *sec_cam_info = NULL;
	struct fwcmd_dctlinfo_ud_v1 dctlinfo_v1;
	u8 i, mac_id, key_type, key_index, key_valid_byte;
	u8 key_id_sh = 0, key_cam_idx_sh = 0;
	u8 key_valid[7] = {0}, sec_cam_index[7] = {0}, sec_ent_keyid[7] = {0};
	u32 DMAC_TBL[8] = {0}, dmac_tbl_addr = 0, ret = 0;
	...
	key_cam_idx_sh = 0;
	for (i = 3; i < 7; i++) {
		key_cam_idx_sh = (i - 3) * KEY_CAM_IDX_SH;
		sec_cam_index[i] =
			(DMAC_TBL[6] >> key_cam_idx_sh) &
			KEY_CAM_IDX_MSK;
	}
	...
	for (key_index = 0; key_index < 7; key_index++) {
		if (key_valid[key_index] == VALID) {
			m_security_cam_hal_1115e(adapter, sec_cam_info,
						 mac_id,
						 sec_ent_keyid[key_index],
						 key_type,
						 sec_cam_index[key_index],
						 SEC_CAM_CLEAR);
		}
	}
...
u32 mac_sta_del_key_1115e(struct mac_ax_adapter *adapter,
			  u8 mac_id,
			  u8 key_id,
			  u8 key_type)
{
	...
	ret = delete_key_from_dmac_tbl_1115e(adapter, mac_id, key_type,
					     key_id, &sec_cam_idx);
	if (ret != MACSUCCESS)
		return ret;
	...
	fill_sec_cam_info_1115e(adapter, &sec_cam_info,
				(struct fwcmd_seccam_info *)sec_table, SEC_CAM_CLEAR);

	ret = (u8)mac_upd_sec_infotbl_1115e(adapter,
				      (struct fwcmd_seccam_info *)sec_table);
	if (ret != MACSUCCESS)
		return ret;

	m_security_cam_hal_1115e(adapter, NULL,
				 mac_id, key_id, key_type, sec_cam_idx, SEC_CAM_CLEAR);

	return MACSUCCESS;
}

u32 mac_sta_add_key_1115e(struct mac_ax_adapter *adapter,
			  struct mac_ax_sec_cam_info *sec_cam_info,
			  u8 mac_id,
			  u8 key_id,
			  u8 key_type)
{
	...
	ret = decide_sec_cam_index_1115e(adapter, &sec_cam_idx);
	if (ret != MACSUCCESS)
		return ret;
	...
	ret = insert_key_to_dmac_tbl_1115e(adapter, mac_id, key_type, key_id,
					   sec_cam_info->sec_cam_idx);
	if (ret != MACSUCCESS)
		return ret;

	fill_sec_cam_info_1115e(adapter, sec_cam_info,
				(struct fwcmd_seccam_info *)sec_table, SEC_CAM_NORMAL);
...
u32 mac_sta_search_key_idx_1115e(struct mac_ax_adapter *adapter,
				 u8 mac_id, u8 key_id, u8 key_type)
{
	u32 sec_cam_idx = 0;
...
u32 mac_sta_hw_security_support_1115e(struct mac_ax_adapter *adapter,
...
u8 check_key_type_1115e(u8 addr_cam_sec_mode, u8 key_index)
{
	switch (addr_cam_sec_mode) {
	case ADDR_CAM_SEC_MODE_ZERO:
		if (key_index <= 6)
...
u32 mac_wowlan_secinfo_1115e(struct mac_ax_adapter *adapter,
			     struct mac_ax_sec_iv_info *sec_iv_info)
{
	...
	case SEC_IV_UPD_TYPE_WRITE:
		...
		cpu_to_le32(SET_WORD(sec_iv_info->ptktxiv[2], DCTRL_TBL_SEC_IV2) |
			    SET_WORD(sec_iv_info->ptktxiv[3], DCTRL_TBL_SEC_IV3) |
			    SET_WORD(sec_iv_info->ptktxiv[4], DCTRL_TBL_SEC_IV4) |
			    SET_WORD(sec_iv_info->ptktxiv[5], DCTRL_TBL_SEC_IV5));

		ret = upt_dctl_secinfo_entry_8192xb(adapter, mac_id,
	...
	case SEC_IV_UPD_TYPE_READ:
		ret = refresh_security_cam_info_8192xb(adapter, sec_iv_info);
		break;
