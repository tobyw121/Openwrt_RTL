...
{
	u32 txdiff_tbl[MCS_SIZE_TAB][TABLE_SIZE_TAB] = {
	...
	u32 tmp_for_txdiff = 0;
	u16 cr = 0;
	u8 i, j;

	cr = 0xD56C; /*R_AX_PWR_SR_MCS0_TXDIFF_TABLE0*/

	for (i = 0; i < MCS_SIZE_TAB; i++) {
		for (j = 0; j < TABLE_SIZE_TAB; j++) {
			tmp_for_txdiff = txdiff_tbl[i][j];
			rtw_hal_mac_set_pwr_reg(bb->hal_com, (u8)bb->bb_phy_idx, cr, tmp_for_txdiff);
			cr += 0x4;
		}
	}
}
