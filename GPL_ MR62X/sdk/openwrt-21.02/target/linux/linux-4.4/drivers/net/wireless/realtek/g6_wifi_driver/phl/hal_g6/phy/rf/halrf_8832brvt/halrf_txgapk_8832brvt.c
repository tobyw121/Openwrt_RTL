...
void _txgapk_backup_bb_registers_8832brvt(
	struct rf_info *rf,
	u32 *reg,
	u32 *reg_bkup,
	u8 reg_num)
...
void _txgapk_backup_kip_8832brvt(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][TXGAPK_KIP_REG_NUM_8832BRVT],
	u8 path,
	u8 reg_num)
{
	u8 i;
...
void _txgapk_bkup_rf_8832brvt(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][TXGAPK_RF_REG_NUM_8832BRVT],
	u8 path,
	u8 reg_num)
{
	u8 i;
...
void _txgapk_reload_bb_registers_8832brvt(
	struct rf_info *rf,
	u32 *reg,
	u32 *reg_backup,
	u8 reg_num)
...
void _txgapk_reload_rf_8832brvt(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][TXGAPK_RF_REG_NUM_8832BRVT],
	u8 path,
...
void _txgapk_reload_kip_8832brvt(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][TXGAPK_KIP_REG_NUM_8832BRVT],
	u8 path,
...
void _halrf_txgapk_rxagc_onoff_8832brvt(
	struct rf_info *rf,
	enum rf_path path,
	bool turn_on)
{
...
void _halrf_txgapk_bb_afe_by_mode_8832brvt(struct rf_info *rf,
...
void _halrf_txgapk_iqk_preset_by_mode_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_txgapk_one_shot_nctl_done_check_8832brvt
	(struct rf_info *rf)
{
	/* for check status */
	u32 r_bff8 = 0;
...
void _halrf_txgapk_rf_dpk_5g_8832brvt
...
void _halrf_txgapk_rf_rxdck_5g_8832brvt
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	u8 i = 0;
...
void _halrf_txgapk_track_table_nctl_5g_8832brvt
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	...
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	#else
	halrf_do_one_shot_8832brvt(rf, path, 0x8000, MASKDWORD, process_id1[path]);
	...
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	#else
	halrf_do_one_shot_8832brvt(rf, path, 0x8000, MASKDWORD, process_id2[path]);
	#endif
...
void _halrf_txgapk_write_track_table_default_5g_8832brvt
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
...
void _halrf_txgapk_power_table_nctl_5g_8832brvt
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
...
void _halrf_txgapk_write_power_table_default_5g_8832brvt
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	//New reset table method for 52c later series	
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
...
void _halrf_txgapk_iqk_bk_reg_by_mode_8832brvt
...
#if 0
		halrf_before_one_shot_enable_8832brvt(rf);
		halrf_wreg(rf, 0x8000, MASKDWORD, path_setting[path]);

		halrf_one_shot_nctl_done_check_8832brvt(rf, path);
#else
		halrf_do_one_shot_8832brvt(rf, path, 0x8000, MASKDWORD, process_id[path]);
...
void _halrf_txgapk_restore_iqk_kmod_bb_8832brvt
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_txgapk_main_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	#if 0 //it doesn't need to write default
	_halrf_txgapk_write_track_table_default_8832brvt(rf, phy, path);
	_halrf_txgapk_write_power_table_default_8832brvt(rf, phy, path);
	#endif

	//v01_BB_AFE_DPK_S0_20210913
	_halrf_txgapk_bb_afe_by_mode_8832brvt(rf, phy, path, is_dbcc);
	
	//v02_IQK_Preset_path0_20210804	
	_halrf_txgapk_iqk_preset_by_mode_8832brvt(rf, phy, path, is_dbcc);
	
	//v03_S0_RF_for_DPK_20210924.txt
	//5g only
	_halrf_txgapk_rf_dpk_5g_8832brvt(rf, phy, path);
	
	

	//v04_RF_RxDCK_S0_20210818.txt	
	_halrf_txgapk_rf_rxdck_5g_8832brvt(rf, phy, path);

	
	//v05a_A_S0_GapK_Track_32BR.txt
	_halrf_txgapk_track_table_nctl_5g_8832brvt(rf, phy, path);

	
	//v05b_A_S0_GapK_Power_32BR.txt
	_halrf_txgapk_power_table_nctl_5g_8832brvt(rf, phy, path);
	
	

	//V 98_S0_IQK_Reg_Restore_52C_20210804
	_halrf_txgapk_iqk_bk_reg_by_mode_8832brvt(rf, phy, path, is_dbcc);

	//V99_BB_AFE_DPK_S0_restore_20210913.txt		
	_halrf_txgapk_restore_iqk_kmod_bb_8832brvt(rf, phy, path, is_dbcc);
...
void _halrf_do_reset_tbl_txgapk_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 path;
	
	u32 kip_bkup[TXGAPK_RF_PATH_MAX_8832BRVT][TXGAPK_KIP_REG_NUM_8832BRVT] = {{0}};
	//u32 bb_bkup[TXGAPK_BB_REG_NUM_8832BRVT] = {0};
	u32 rf_bkup[TXGAPK_RF_PATH_MAX_8832BRVT][TXGAPK_RF_REG_NUM_8832BRVT] = {{0}};
	...
	#if 0 //no need 
	_txgapk_backup_bb_registers_8832brvt(rf, bb_reg, bb_bkup, TXGAPK_BB_REG_NUM_8832BRVT);
	#endif

	for (path = 0; path < TXGAPK_RF_PATH_MAX_8832BRVT; path++) {	
		_txgapk_backup_kip_8832brvt(rf, kip_reg, kip_bkup, path, TXGAPK_KIP_REG_NUM_8832BRVT);
		_txgapk_bkup_rf_8832brvt(rf, rf_reg, rf_bkup, path, TXGAPK_RF_REG_NUM_8832BRVT);
	}				
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8832BRVT; path++) {
		...
		//follow dpk flow
		_halrf_txgapk_rxagc_onoff_8832brvt(rf, path, false);
	
		//main2 =========================
		//v01_BB_AFE_DPK_S0_20210913
		_halrf_txgapk_bb_afe_by_mode_8832brvt(rf, phy, path, false);
		
		//v02_IQK_Preset_path0_20210804	
		_halrf_txgapk_iqk_preset_by_mode_8832brvt(rf, phy, path, false);
		...
		//v05a_A_S0_GapK_Track_32BR.txt
		_halrf_txgapk_write_track_table_default_5g_8832brvt(rf, phy, path);
		
		//v05b_A_S0_GapK_Power_32BR.txt
		_halrf_txgapk_write_power_table_default_5g_8832brvt(rf, phy, path);
	
		//V 98_S0_IQK_Reg_Restore_52C_20210804
		_halrf_txgapk_iqk_bk_reg_by_mode_8832brvt(rf, phy, path, false);

		//V99_BB_AFE_DPK_S0_restore_20210913.txt		
		_halrf_txgapk_restore_iqk_kmod_bb_8832brvt(rf, phy, path, false);
		//======= end =======	
	}

	
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8832BRVT; path++) {
		_txgapk_reload_kip_8832brvt(rf, kip_reg, kip_bkup, path, TXGAPK_KIP_REG_NUM_8832BRVT);
		_txgapk_reload_rf_8832brvt(rf, rf_reg, rf_bkup, path, TXGAPK_RF_REG_NUM_8832BRVT);

		//follow dpk flow
		_halrf_txgapk_rxagc_onoff_8832brvt(rf, path, true);
	}

	#if 0 //no need 
	_txgapk_reload_bb_registers_8832brvt(rf, bb_reg, bb_bkup, TXGAPK_BB_REG_NUM_8832BRVT);
	#endif
}



void _halrf_do_non_dbcc_txgapk_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 path;
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8832BRVT; path++) {
		...
		_halrf_txgapk_main_8832brvt(rf, phy, path, false);
	}
...
void _halrf_do_dbcc_txgapk_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	enum rf_path path = 0;
...
void _halrf_txgapk_get_ch_info_8832brvt(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
...
void  _halrf_sel_hw_table_txgapk_8832brvt(struct rf_info *rf, bool is_hw) 
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s \n", __func__);
...
void halrf_do_txgapk_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
	
	_halrf_txgapk_get_ch_info_8832brvt(rf, phy);
	...
	//_halrf_sel_hw_table_txgapk_8832brvt(rf, phy, false); //use sw control
	...
	if (rf->hal_com->dbcc_en)
		_halrf_do_dbcc_txgapk_8832brvt(rf, phy);
	else
		_halrf_do_non_dbcc_txgapk_8832brvt(rf, phy);
...
void halrf_txgapk_init_8832brvt(struct rf_info *rf) 
{	
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	...
	if(!txgapk_info->is_gapk_init) {
		...
		_halrf_sel_hw_table_txgapk_8832brvt(rf, false); //use sw control			
	}
}


#if 0
void halrf_txgapk_enable_8832brvt
...
void halrf_txgapk_write_table_default_8832brvt
	(struct rf_info *rf, enum phl_phy_idx phy)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s   phy=%d\n", __func__, phy);

	_halrf_do_reset_tbl_txgapk_8832brvt(rf, phy);
}
