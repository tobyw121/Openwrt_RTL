...
//		halrf_quick_checkrf_8832crvu(rf);
#endif
			...
			enum phl_msg_evt_id event, enum phl_phy_idx phy_idx)
{
	...
	switch (event) {
		...
		break;
		case MSG_EVT_HAL_START_OK:
			halrf_set_power(rf, HW_PHY_0, PWR_BY_RATE);
			halrf_tssi_trigger_g6(rf, HW_PHY_0, false);
		break;
		default:
		break;
	}
	...
	switch (idx) {
		case RF_EVENT_PWR_TRK:
			if (func == RF_EVENT_OFF)
				...
				halrf_tssi_trigger_g6(rf, phy, true);
		break;
		...
		case RF_EVENT_IQK:
			if (func == RF_EVENT_OFF)
				...
				halrf_iqk_trigger_g6(rf, phy, false);
			}
		break;
		...
		case RF_EVENT_DPK:
			if (func == RF_EVENT_OFF)
				...
				halrf_dpk_trigger_g6(rf, phy, false);
		break;
...
#ifdef RF_8832CRVU_SUPPORT
//	if (hal_i->chip_id == CHIP_WIFI6_8832CRVU)
//		mcc_info->band[idx] = rf->hal_com->band[phy].cur_chandef.band;
#endif	
