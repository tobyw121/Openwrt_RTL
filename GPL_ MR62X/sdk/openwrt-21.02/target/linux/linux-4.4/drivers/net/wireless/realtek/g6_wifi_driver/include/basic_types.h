...
#else
#define N_BYTE_ALIGNMENT(__Value, __Alignment) \
	({ \
		size_t _v = (size_t)(__Value); \
		const size_t _a = (const size_t)(__Alignment); \
		if (_a > 1) { \
			size_t _r = _v % _a; \
			if (_r) \
				_v += (_a - _r); \
...
#define N_BYTE_ALIGMENT(__Value, __Aligment) N_BYTE_ALIGNMENT(__Value, __Aligment)
#endif
