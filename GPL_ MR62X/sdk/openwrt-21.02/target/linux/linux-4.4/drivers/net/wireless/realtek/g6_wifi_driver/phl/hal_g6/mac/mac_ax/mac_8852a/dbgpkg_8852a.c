...
u32 tx_dbg_dump_8852a(struct mac_ax_adapter *adapter)
{
	...
	for (i = 0; i < TRX_CNT_REPT_CNT; i++) {
		...
		if (adapter->hw_info->intf == MAC_AX_INTF_PCIE) {
			...
			PLTFM_MSG_ALWAYS("R_AX_CH10_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_AX_CH10_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_AX_CH11_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_AX_CH11_TXBD_IDX));
...
u8 is_dbg_port_not_valid_8852a(struct mac_ax_adapter *adapter, u32 dbg_sel)
{
	...
	if (check_mac_en(adapter, 0, MAC_AX_DMAC_SEL) != MACSUCCESS &&
		...
		return 1;
	if (dbg_sel >= MAC_AX_DBG_PORT_SEL_TXPKTCTRL_B0_UNIT4_0 &&
	    dbg_sel <= MAC_AX_DBG_PORT_SEL_TXPKTCTRL_B0_UNIT7_1)
		return 1;
