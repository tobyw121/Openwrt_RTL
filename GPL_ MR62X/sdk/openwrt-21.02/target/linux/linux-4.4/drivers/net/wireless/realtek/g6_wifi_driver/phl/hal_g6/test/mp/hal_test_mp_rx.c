	...
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	...
	hal_status = rtw_hal_bb_get_rx_ok(mp->hal, arg->rx_phy_idx, &arg->rx_ok);
	...
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	...
	arg->mp_fltr_rx_ok_cnt = trx_stat->mp_fltr_rx_ok_cnt;
...
enum rtw_hal_status rtw_hal_mp_rx_get_rpl(
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	PHL_INFO("%s: rx_phy_idx: %d, rx_path: %d\n", __FUNCTION__, arg->rx_phy_idx, arg->rx_path);

	hal_status = rtw_hal_bb_get_rpl(mp->hal, arg->rx_path, &arg->rpl_rssi, arg->rx_phy_idx);
	PHL_INFO("%s: status = %d\n", __FUNCTION__, hal_status);
	PHL_INFO("%s: rssi = %d\n", __FUNCTION__, arg->rpl_rssi);
...
enum rtw_hal_status rtw_hal_mp_rx_filter_start(
		struct mp_context *mp, struct mp_rx_arg *arg)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_info_t *hal_info = (struct hal_info_t *)mp->hal;
	...
	struct rtw_trx_stat *trx_stat = &hal_com->trx_stat;
	u8 i;
	hal_status = rtw_hal_mac_set_rxfltr_by_mode(hal_com, 0, RX_FLTR_MODE_SNIFFER);

	_rtw_memcpy(&trx_stat->mp_fltr_ta, &arg->mp_fltr_mac_ta, 6);
	_rtw_memcpy(&trx_stat->mp_fltr_sa, &arg->mp_fltr_mac_sa, 6);

	PHL_INFO("set mac filter ta %x:%x:%x:%x:%x:%x\n", trx_stat->mp_fltr_ta[0],
													  trx_stat->mp_fltr_ta[1],
													  trx_stat->mp_fltr_ta[2],
													  trx_stat->mp_fltr_ta[3],
													  trx_stat->mp_fltr_ta[4],
													  trx_stat->mp_fltr_ta[5]);


	PHL_INFO("set mac filter sa %x:%x:%x:%x:%x:%x\n", trx_stat->mp_fltr_sa[0],
													  trx_stat->mp_fltr_sa[1],
													  trx_stat->mp_fltr_sa[2],
													  trx_stat->mp_fltr_sa[3],
													  trx_stat->mp_fltr_sa[4],
													  trx_stat->mp_fltr_sa[5]);

	trx_stat->mp_fltr_rx_ok_cnt = 0;
	trx_stat->mp_fltr_rx_err_cnt = 0;

	return hal_status;
}

enum rtw_hal_status rtw_hal_mp_rx_filter_reset(
		struct mp_context *mp, struct mp_rx_arg *arg)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_info_t *hal_info = (struct hal_info_t *)mp->hal;
