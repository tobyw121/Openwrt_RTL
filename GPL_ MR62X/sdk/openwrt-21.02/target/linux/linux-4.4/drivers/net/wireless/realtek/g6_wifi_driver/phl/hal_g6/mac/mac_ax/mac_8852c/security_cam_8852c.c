...
u32 sec_info_tbl_init_8852c(struct mac_ax_adapter *adapter, u8 op_mode)
{
	u8 i = 0, keyidx = 0;
...
u32 free_sec_info_tbl_8852c(struct mac_ax_adapter *adapter, u8 op_mode)
{
	u8 i;
	struct sec_cam_table_t *sec_cam_table = adapter->hw_info->sec_cam_table;
	struct dctl_sec_info_t *dctl_sec_info = adapter->hw_info->dctl_sec_info;
...
u32 mac_sta_keycam_backup_8852c(struct mac_ax_adapter *adapter, u8 op_mode)
{
	...
	if (op_mode == SEC_CAM_BACKUP) {
		...
		ret = sec_info_tbl_init_8852c(adapter, SEC_CAM_BACKUP);
		if (ret != MACSUCCESS) {
		...
		for (sec_cam_idx = 0; sec_cam_idx < SEC_CAM_ENTRY_NUM;
			...
			if (sec_cam_table->sec_cam_entry[sec_cam_idx]->valid == VALID) {
				m_security_cam_hal_8852c(adapter, NULL, 0, 0, 0,
							 sec_cam_idx, SEC_CAM_BACKUP);
			}
		}

		ret = free_sec_info_tbl_8852c(adapter, SEC_CAM_NORMAL);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]sec info tbl backup %d\n", ret);
			return ret;
		}
		...
		ret = sec_info_tbl_init_8852c(adapter, SEC_CAM_NORMAL);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]sec info tbl backup %d\n", ret);
			return ret;
		}
		...
		for (sec_cam_idx = 0; sec_cam_idx < SEC_CAM_ENTRY_NUM; sec_cam_idx++) {
			...
			if (s_entry->valid == VALID) {
				...
				mac_sta_add_key_8852c(adapter, &sec_cam_info,
		...
		ret = free_sec_info_tbl_8852c(adapter, SEC_CAM_RESTORE);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]sec info tbl backup %d\n", ret);
			return ret;
		}
