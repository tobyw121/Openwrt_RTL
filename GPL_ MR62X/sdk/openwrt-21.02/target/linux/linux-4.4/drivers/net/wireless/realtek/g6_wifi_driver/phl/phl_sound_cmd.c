...
void _snd_cmd_timer_callback(void *context)
{
	...
	if (SND_CMD_BFER_SOUND == cur_snd_cmd->event) {
		...
		cur_snd_cmd->wrole = rtw_phl_get_wrole_by_ridx(snd_obj->phl_info->phl_com,
							   cur_snd_grp->wrole_idx);
		cur_snd_cmd->snd_grp = cur_snd_grp;

		if(snd_obj->phl_info->phl_com->txbf_auto_snd){
			if (RTW_PHL_STATUS_SUCCESS !=
				phl_snd_cmd_sound_evt(snd_obj->phl_info,
					      cur_snd_cmd->wrole,
			     		      cur_snd_grp,
					      SND_CMD_BFER_GROUPING)) {
			goto err;
		}
		}else{
		if (RTW_PHL_STATUS_SUCCESS !=
...
_phl_snd_cmd_bfer_cmd_proc(struct phl_info_t *phl, struct snd_cmd_bfer *snd_cmd)
{
	...
	u8 role_idx = 0,tmp_reg = 0;
	struct rtw_wifi_role_t *m_wrole =(struct rtw_wifi_role_t *)snd_obj->snd_param.m_wrole;
	bool is_wmm1_exist = false;
	...
	case SND_CMD_BFER_GROUPING:
	{
		...
		{
			m_wrole = rtw_phl_get_wrole_by_ridx(snd_obj->phl_info->phl_com, role_idx);
			if (m_wrole != NULL && m_wrole->active  && m_wrole->hw_wmm) {
				is_wmm1_exist = true;
				mstatus = MDL_RET_FAIL;
				tmp_reg = rtw_phl_read8(phl, 0xc600);
				if ((tmp_reg & BIT (1)) == 0) {
					rtw_phl_write8(phl, 0xc600, tmp_reg |= BIT (1));
				}
				break;
			}
		}

		if (is_wmm1_exist== false) {
	...
	case SND_CMD_BFER_CHECK_NEXT:
	{
		...
		if (phl->phl_com->txbf_auto_snd == 0){
		...
		do{/* trigger next sound group */
			if (snd_param->snd_func_grp_num ==0)
				break;

			snd_param->cur_proc_grp_idx++;

			if (snd_param->cur_proc_grp_idx >= MAX_SND_GRP_NUM)
				snd_param->cur_proc_grp_idx = 0;

		}while(!(snd_param->grp_used_map & BIT(snd_param->cur_proc_grp_idx)));
	...
	case SND_CMD_BFER_TERMINATE:
	{
		...
		if ((snd_param->test_flag & PHL_SND_TEST_F_ONE_TIME) ==0) {
		phl_snd_func_remove_grp_all(phl);
			}
...
//		  "===> %s() event id : 0x%x \n", __func__, MSG_EVT_ID_FIELD(msg->msg_id));
	...
	if (IS_MSG_FAIL(msg->msg_id)) {
		if (phl_info->phl_com->txbf_auto_snd == 0){
	...
	struct snd_cmd_bfer *cmd_buf)
{
	...
	case SND_CMD_BFER_GROUPING:
	{
		...
		if (snd_param->snd_func_grp_num != 0) {
			wrole = rtw_phl_get_wrole_by_ridx(phl_info->phl_com,
	...
	struct snd_cmd_bfer *cmd_buf)
{
	...
	{
		if (snd_param->test_flag&PHL_SND_TEST_F_ONE_TIME) {
			PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "%s %d: case SND_CMD_BFER_CHECK_NEXT/SND_CMD_BFER_LEAVE:\n", __func__,__LINE__);
			phl_snd_cmd_sound_evt(phl_info, cmd_buf->wrole,
					      cmd_buf->snd_grp,
					      SND_CMD_BFER_TERMINATE);
		}else{
                        ...
                        {
					PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s %d: SND_CMD_BFER_GROUPING\n", __func__,__LINE__);
					snd_param->snd_grp[snd_param->cur_proc_grp_idx].snd_cmd.event = SND_CMD_BFER_CHECK_NEXT;
					_os_cancel_timer(d, &(snd_obj->snd_timer));
					_os_set_timer(d, &(snd_obj->snd_timer), snd_param->snd_proc_period);
