...
static struct usb_device_id rtw_usb_id_tbl[] = {
	...
	/*=== Realtek demoboard ===*/
	{USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0x8852, 0xff, 0xff, 0xff), .driver_info = RTL8852A},
	{USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0x885A, 0xff, 0xff, 0xff), .driver_info = RTL8852A},
	{USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0x885B, 0xff, 0xff, 0xff), .driver_info = RTL8852A},
	...
	if (rx_pkt_num) {
		...
		while (rx_pkt_num--) {
			...
			if (rx_req) {
				...
				rtw_phl_return_rxbuf(GET_HAL_INFO(dvobj), (u8 *)rx_req);
				rtw_skb_free(skb);
			}
		}
	}
