...
u8 halrf_get_thermal_8852bp(struct rf_info *rf, enum rf_path rf_path)
{
...
u32 halrf_mac_get_pwr_reg_8852bp(struct rf_info *rf, enum phl_phy_idx phy,
	u32 addr, u32 mask)
{
...
u32 halrf_mac_set_pwr_reg_8852bp(struct rf_info *rf, enum phl_phy_idx phy,
...
void halrf_wlan_tx_power_control_8852bp(struct rf_info *rf,
	...
	u32 tx_power_val, bool enable)
{
	...
	if (enable == false) {
		if (pwr_ctrl_idx == ALL_TIME_CTRL) {
			...
			halrf_mac_set_pwr_reg_8852bp(rf, phy_tmp, 0xd200, 0x3ff, 0x0);
		} else if (pwr_ctrl_idx == GNT_TIME_CTRL) {
			/*GNT_BT control Disable*/
			halrf_mac_set_pwr_reg_8852bp(rf, phy_tmp, 0xd220, BIT(1), 0x0);
			halrf_mac_set_pwr_reg_8852bp(rf, phy_tmp, 0xd220, 0xff8, 0x0);
		}
	} else {
		if (pwr_ctrl_idx == ALL_TIME_CTRL) {
			halrf_mac_set_pwr_reg_8852bp(rf, phy_tmp, 0xd200, 0x3ff,
				((tx_power_val & 0x1ff) | BIT(9)));
		} else if (pwr_ctrl_idx == GNT_TIME_CTRL) {
			halrf_mac_set_pwr_reg_8852bp(rf, phy_tmp, 0xd220, BIT(1), 0x1);
			halrf_mac_set_pwr_reg_8852bp(rf, phy_tmp, 0xd220, 0xff8, tx_power_val & 0x1ff);
		} else {
...
bool halrf_wl_tx_power_control_8852bp(struct rf_info *rf, u32 tx_power_val)
{
	...
	if (pwr->coex_pwr_ctl_enable == false && pwr->dpk_pwr_ctl_enable == false) {
		/*all-time control Disable*/
		result = halrf_mac_set_pwr_reg_8852bp(rf, phy, 0xd200, 0x3ff, 0x0);
		...
		/*all-time control*/
		result = halrf_mac_set_pwr_reg_8852bp(rf, phy, 0xd200, 0x3ff, ((tmp_pwr & 0x1ff) | BIT(9)));
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd200) fail, error code=%d\n",
				__func__, result);
			return false;
	...
	if (gnt_bt_control == 0xffff) {
		...
		result = halrf_mac_set_pwr_reg_8852bp(rf, phy, 0xd220, BIT(1), 0x0);
		result = halrf_mac_set_pwr_reg_8852bp(rf, phy, 0xd220, 0xff8, 0x0);
		if (result) {
		...
		result = halrf_mac_set_pwr_reg_8852bp(rf, phy, 0xd220, BIT(1), 0x1);
		result = halrf_mac_set_pwr_reg_8852bp(rf, phy, 0xd220, 0xff8, gnt_bt_control & 0x1ff);
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd220) fail, error code=%d\n",
				__func__, result);
			return false;
...
s8 halrf_get_ther_protected_threshold_8852bp(struct rf_info *rf)
{
	u8 tmp_a, tmp_b, tmp;
	u8 therml_max = 0x32;
...
s8 halrf_xtal_tracking_offset_8852bp(struct rf_info *rf,
...
void halrf_syn1_onoff_8852bp(struct rf_info *rf, enum phl_phy_idx phy, u8 path, bool syn1_turn_on)
{
	if(path != RF_PATH_B)
		return;
...
void halrf_txck_force_8852bp(struct rf_info *rf, enum rf_path path, bool force, enum dac_ck ck)
{
...
void halrf_rxck_force_8852bp(struct rf_info *rf, enum rf_path path, bool force, enum adc_ck ck)
{
...
void halrf_set_gpio_8852bp(struct rf_info *rf, enum phl_phy_idx phy)
{
...
u32 bkup_kip_reg_8852bp[] = {0x813c, 0x8124, 0x8120, 0xc0d4, 0xc0d8};
u32 bkup_bb_reg_8852bp[] = {0x2344};
u32 bkup_rf_reg_8852bp[] = {0x5, 0x10005, 0xdf};

u32 bkup_kip_val_8852bp[RF_PATH_MAX_NUM][RF_BACKUP_KIP_REG_MAX_NUM] = {{0x0}};
u32 bkup_bb_val_8852bp[RF_BACKUP_BB_REG_MAX_NUM] = {0x0};
u32 bkup_rf_val_8852bp[RF_PATH_MAX_NUM][RF_BACKUP_RF_REG_MAX_NUM] = {{0x0}};

void do_bkup_kip_8852bp(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_kip_reg_8852bp);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_kip_val_8852bp[path][i] = halrf_rreg(rf, bkup_kip_reg_8852bp[i] + (path << 8), MASKDWORD);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Backup KIP(S%d) 0x%x = %x\n", path, bkup_kip_reg_8852bp[i] + (path << 8), bkup_kip_val_8852bp[path][i]);
	}
}

void do_bkup_bb_8852bp(struct rf_info *rf)
{
	u32 i;
	u32 num = ARRAY_SIZE(bkup_bb_reg_8852bp);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_bb_val_8852bp[i] =
			halrf_rreg(rf, bkup_bb_reg_8852bp[i], MASKDWORD);

		RF_DBG(rf, DBG_RF_RFK, "[RFK]backup bb reg : %x, value =%x\n",
		      bkup_bb_reg_8852bp[i], bkup_bb_val_8852bp[i]);
	}
}


void do_bkup_rf_8852bp(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_rf_reg_8852bp);
	...
	for (i = 0; i < num; i++) {
		...
		bkup_rf_val_8852bp[path][i] = halrf_rrf(rf, path, bkup_rf_reg_8852bp[i], MASKRF);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Backup RF S%d 0x%x = %x\n",
			path, bkup_rf_reg_8852bp[i], bkup_rf_val_8852bp[path][i]);
	}
}

void do_reload_kip_8852bp(struct rf_info *rf, u8 path) 
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_kip_reg_8852bp);

	for (i = 0; i < num; i++) {
		halrf_wreg(rf, bkup_kip_reg_8852bp[i] + (path << 8), MASKDWORD, bkup_kip_val_8852bp[path][i]);
		
		RF_DBG(rf, DBG_RF_RFK, "[RFK] Reload KIP(S%d) 0x%x = %x\n",path, bkup_kip_reg_8852bp[i] + (path << 8),
			bkup_kip_val_8852bp[path][i]);
	}
}

void do_reload_bb_8852bp(struct rf_info *rf)
{
	u32 i;
	u32 num = ARRAY_SIZE(bkup_bb_reg_8852bp);

	for (i = 0; i < num; i++) {
		halrf_wreg(rf,  bkup_bb_reg_8852bp[i], MASKDWORD, bkup_bb_val_8852bp[i]);
...
void do_reload_rf_8852bp(struct rf_info *rf, u8 path)
{
	u8 i;
	u32 num = ARRAY_SIZE(bkup_rf_reg_8852bp);

	for (i = 0; i < num; i++) {
		halrf_wrf(rf, path, bkup_rf_reg_8852bp[i], MASKRF, bkup_rf_val_8852bp[path][i]);
