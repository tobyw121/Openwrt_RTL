...
#ifndef CONFIG_CMD_DISP /* should remove after CONFIG_FSM is not defined */
	do {
		...
		rtw_hal_watchdog(phl_info->phl_com, phl_info->hal);
	} while (false);
#endif
}
#endif
...
static void _phl_watchdog_hw(struct phl_info_t *phl)
{
	...
	phl_mr_watchdog(phl);
	rtw_hal_watchdog(phl->phl_com, phl->hal);
}

static void _phl_watchdog_set_timer(void *drv_priv, struct phl_watchdog *wdog)
{
	if (wdog->active == true)
		_os_set_timer(drv_priv, &wdog->wdog_timer, wdog->period);
}
...
static void _phl_watchdog_done(void *drv_priv, u8 *cmd, u32 cmd_len, enum rtw_phl_status status)
{
	...
	_phl_watchdog_set_timer(drv_priv, wdog);
}
...
static void _phl_watchdog_timer_expired(void *context)
{
	...
	if ((false == is_cmd_enqueue(psts)) && (RTW_PHL_STATUS_SUCCESS != psts)) {
		_phl_watchdog_set_timer(phl_to_drvpriv(phl_info), wdog);
	}
#else
