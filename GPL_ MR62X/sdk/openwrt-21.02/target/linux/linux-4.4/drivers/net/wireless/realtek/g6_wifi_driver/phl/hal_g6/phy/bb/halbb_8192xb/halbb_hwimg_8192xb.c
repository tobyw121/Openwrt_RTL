...
#include "halbb_hwimg_raw_data_8192xb.h"
#include "halbb_hwimg_raw_data_8832br.h"

#ifdef BB_8192XB_SUPPORT

bool halbb_sel_headline_8192xb(struct bb_info *bb, u32 *array, u32 array_len,
...
void halbb_flag_2_default_8192xb(bool *is_matched, bool *find_target)
{
	*is_matched = true;
	*find_target = false;
}

bool halbb_cfg_bbcr_ax_8192xb(struct bb_info *bb, bool is_form_folder,
			  ...
			  enum phl_phy_idx phy_idx)
{
	...
	if (is_form_folder) {
		...
		array = folder_array;
	} else if (bb->ic_sub_type == BB_IC_SUB_TYPE_8192XB_8832BR) {
		array_len = sizeof(array_mp_8832br_phy_reg) / sizeof(u32);
		array = (u32 *)array_mp_8832br_phy_reg;
		BB_DBG(bb, DBG_INIT, "IC subtype = 8832BR\n");
	} else if (bb->ic_sub_type == BB_IC_SUB_TYPE_8192XB_8192XB) {
		array_len = sizeof(array_mp_8192xb_phy_reg) / sizeof(u32);
		array = (u32 *)array_mp_8192xb_phy_reg;
		BB_DBG(bb, DBG_INIT, "IC subtype = 8192XB\n");
	} else {
		BB_WARNING("[%s]Invalid BB IC subtype\n", __func__);
		return false;
	}
	...
	if (!halbb_sel_headline_8192xb(bb, array, array_len, &h_size, &h_idx)) {
	...
	halbb_flag_2_default_8192xb(&is_matched, &find_target);
	...
	while ((i + 1) < array_len) {
		...
		default:
			if (is_matched) 
				//#ifdef HALBB_FW_OFLD_SUPPORT
				//ret = halbb_fwcfg_bb_phy_8192xb(bb, v1, v2, phy_idx);
				//#else
				halbb_cfg_bb_phy_8192xb(bb, v1, v2, phy_idx);
				//#endif
			break;
		}
	}
...
bool halbb_cfg_bb_gain_ax_8192xb(struct bb_info *bb, bool is_form_folder,
				  u32 folder_len, u32 *folder_array)
{
	bool is_matched, find_target;
	u32 cfg_target = 0, cfg_para = 0;
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8832br_phy_reg_gain) / sizeof(u32);
		array = (u32 *)array_mp_8832br_phy_reg_gain;
		BB_DBG(bb, DBG_INIT, "IC subtype = 8832BR\n");
	} else if (bb->ic_sub_type == BB_IC_SUB_TYPE_8192XB_8192XB) {
		array_len = sizeof(array_mp_8192xb_phy_reg_gain) / sizeof(u32);
		array = (u32 *)array_mp_8192xb_phy_reg_gain;
		BB_DBG(bb, DBG_INIT, "IC subtype = 8192XB\n");
	} else {
		BB_WARNING("[%s]Invalid BB IC subtype\n", __func__);
		return false;
	...
	while ((i + 1) < array_len) {
		...
		default:
			if (is_matched)
				halbb_cfg_bb_gain_8192xb(bb, v1, v2);
			break;
		}
	}
...
u32
halbb_get_8192xb_phy_reg_ver(void)
{
		return (u32)BB_REG_RELEASE_VERSION_8192XB;
}
