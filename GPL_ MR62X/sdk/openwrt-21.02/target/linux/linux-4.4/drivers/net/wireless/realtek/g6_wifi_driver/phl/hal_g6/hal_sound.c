...
u32 _cal_he_csi_size(u8 mu, enum channel_width bw, u8 nr, u8 nc, u8 ng, u8 cb)
{
	...
	do {
		if (CHANNEL_WIDTH_160 == bw)
			ns = (ng == 4) ? 500 : 128;
		else if (CHANNEL_WIDTH_80 == bw)
			ns = (ng == 4) ? 250 : 64;
	...
	do {
		if (hal_sta->bf_entry != NULL) {
			...
			if(hal_status != RTW_HAL_STATUS_SUCCESS)
				PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_bf_release_target_bf_entry fail\n", __func__);
                        else
                                hal_sta->bf_entry = NULL;
		} else
			PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_sta->bf_entry is NULL\n", __func__);

		if (hal_is_csi_buf_valid(hal_info, &hal_sta->bf_csi_buf)) {
			hal_status = hal_csi_release_csi_buf(
					hal_info, &hal_sta->bf_csi_buf);
			if(hal_status != RTW_HAL_STATUS_SUCCESS)
				PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_csi_release_csi_buf fail\n", __func__);
		} else
			PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_is_csi_buf_valid fail\n", __func__);
		if (hal_is_csi_buf_valid(hal_info, &hal_sta->bf_csi_buf_swap)) {
			hal_status = hal_csi_release_csi_buf(
					hal_info, &hal_sta->bf_csi_buf_swap);
			if(hal_status != RTW_HAL_STATUS_SUCCESS)
				PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_csi_release_csi_buf swap fail\n", __func__);
		} else
			PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_is_csi_buf_valid swap fail\n", __func__);
	} while(0);
			     ...
			     u32 *ndpa, enum channel_width bw, u8 fb_type)
{
	...
	if (0 == fb_type) {
		...
		//	HAL_NPDA_AX_CB_SU42_MU75 : HAL_NPDA_AX_CB_SU64_MU97;
		ndpa_sta->cb = HAL_NPDA_AX_CB_SU64_MU97; //charlie
	} else {
	...
	if ((CHANNEL_WIDTH_160 == bw) || (CHANNEL_WIDTH_80_80 == bw)) {
		if (psta_info->asoc_cap.max_nc >
			psta_info->wrole->proto_role_cap.num_snd_dim_greater_80mhz) {
			ndpa_sta->nc = psta_info->wrole->proto_role_cap.num_snd_dim_greater_80mhz;
		} else {
			ndpa_sta->nc = psta_info->asoc_cap.max_nc;
		}
	} else {
		    ...
		    struct rtw_phl_stainfo_t *psta, u32 *npda_sta)
{
	...
	cmd->cr_idx |= BIT(psta->hal_sta->bfee_idx);
		  		      ...
		  		      u8 ndpa_idx, u8 last)
{
	...
	cmd->cr_idx |= BIT(sta->hal_sta->bfee_idx);
	...
	cmd->bfrp.vht_para[ndpa_idx - 1].rexmit_bmp = 0xFF;
	
	/*BFRP WD*/
	size = sizeof(cmd->wd)/sizeof(cmd->wd[0]);
	if ((size-1) >= (1 + ndpa_idx)){
		    ...
		    struct rtw_phl_stainfo_t *psta, u8 sta_nr, u32 *ndpa_sta)
{
	...
	cmd->wd[1].disdatafb = 1;
	if (1 == psta->wrole->proto_role_cap.num_snd_dim)
		cmd->wd[1].datarate = RTW_DATA_RATE_VHT_NSS2_MCS9; //To avoid NDP to enter CFR setting
	else if (2 == psta->wrole->proto_role_cap.num_snd_dim)
		cmd->wd[1].datarate = RTW_DATA_RATE_VHT_NSS3_MCS0;
	else if (3 == psta->wrole->proto_role_cap.num_snd_dim)
		cmd->wd[1].datarate = RTW_DATA_RATE_VHT_NSS4_MCS0;
		  		     ...
		  		     u8 ndpa_idx, u8 bfrp_idx, u8 bfrp_u_idx)
{
	...
	cmd->bfrp.he_para[bfrp_idx].fbseg_rexmit_bmp[bfrp_u_idx] = 0xFF;
	cmd->bfrp.he_para[bfrp_idx].user[bfrp_u_idx].aid12 = sta->aid;
	if ((cmd->bfrp.he_para[bfrp_idx].common.ul_bw > CHANNEL_WIDTH_80) && (ndpa_idx==1)) {
	...
	cmd->bfrp.he_para[bfrp_idx].user[bfrp_u_idx].ul_fec_code = sta->asoc_cap.he_ldpc;
	...
	cmd->bfrp.he_para[bfrp_idx].user[bfrp_u_idx].ss_alloc = 0;/* 1SS, TODO: Default value of golden */
 	cmd->bfrp.he_para[bfrp_idx].user[bfrp_u_idx].ul_tgt_rssi = (u8)(sta->hal_sta->rssi_stat.rssi >> 1);/*TODO: From Rx ST SU RSSI */

	if(cmd->bfrp.he_para[bfrp_idx].user[bfrp_u_idx].ul_tgt_rssi > 90)
		cmd->bfrp.he_para[bfrp_idx].user[bfrp_u_idx].ul_tgt_rssi = 90;
	...
	if (cmd->bfrp.he_para[bfrp_idx].f2p_info.csi_len_bfrp < (rpt_size/64))
		cmd->bfrp.he_para[bfrp_idx].f2p_info.csi_len_bfrp =
			(u16)((rpt_size / 64) + 2); /*unit 64 byte*/
}
		    ...
		    struct rtw_phl_stainfo_t *psta, u8 sta_nr1, u8 sta_nr2)
{
	...
	if (cmd->bfrp0_sta_nr == 1 && cmd->bfrp1_sta_nr ==0) //su trig
		cmd->mode = 1;
...
#if 1
	if (1 == psta->wrole->proto_role_cap.num_snd_dim)
		cmd->wd[1].datarate = RTW_DATA_RATE_HE_NSS2_MCS9;  //To avoid NDP to enter CFR setting
	else if (2 == psta->wrole->proto_role_cap.num_snd_dim)
		cmd->wd[1].datarate = RTW_DATA_RATE_HE_NSS3_MCS0;
	else if(3 == psta->wrole->proto_role_cap.num_snd_dim)
		cmd->wd[1].datarate = RTW_DATA_RATE_HE_NSS4_MCS0;
	...
	if (cmd->mode ==1) //SU trig enable
		cmd->wd[1].datarate = RTW_DATA_RATE_HE_NSS2_MCS0;



	if(psta->asoc_cap.ltf_gi & BIT(RTW_GILTF_LGI_4XHE32))
	{
		cmd->wd[1].gi_ltf = RTW_GILTF_LGI_4XHE32;
	}
	else
		cmd->wd[1].gi_ltf = RTW_GILTF_2XHE16;
#else
	cmd->wd[1].datarate = _hal_read32(hal_info->hal_com,0xc07c) & 0xfff ;
	cmd->wd[1].gi_ltf = (_hal_read32(hal_info->hal_com,0xc07c) & 0xf000) >> 12 ;
