...
enum rtw_phl_status _phl_in_token_usb(struct phl_info_t *phl_info, u8 pipe_idx)
{
	...
	PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "[1] %s:: ==> [%p] idle_rxbuf_list.cnt=%d\n",
	...
	PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "[1] %s:: <== [%p] busy_rxbuf_list.cnt=%d\n",
...
enum rtw_phl_status _phl_rx_start_usb(struct phl_info_t *phl_info)
{
	...
	for (pipe_idx = 0; pipe_idx < hal_spec->max_bulkin_num ; pipe_idx++) {
		...
		if( pipe_idx==WLAN_IN_MPDU_PIPE_IDX )
		{
			...
			{
				PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "[0] %s:: rx_buf_ring->busy_rxbuf_list.cnt =%d\n",
...
phl_release_rxbuf_usb(struct phl_info_t *phl_info, void *r, u8 ch, enum rtw_rx_type type)
{
	...
	if (rx_buf_ring != NULL && rx_buf != NULL) {
		...
		if(!rx_buf->pktcnt)
		{
			...
			PHL_TRACE(COMP_PHL_RECV, _PHL_INFO_, "[4] %s:: [%p] idle_rxbuf_list.cnt=%d\n", __FUNCTION__, rx_buf, rx_buf_ring->idle_rxbuf_list.cnt);
		}
		else
		{
			_os_spinunlock(drv, &rx_buf->lock, _irq, &sp_flags);
					 ...
					 struct rtw_rx_buf *rx_buf)
{
	...
	do {
		...
		if(phl_rxhead == NULL)
		{
			...
			PHL_TRACE(COMP_PHL_RECV, _PHL_INFO_, "[3] %s:: [head %p]: transfer_len=%d\n",
		...
		else
		{
			...
			PHL_TRACE(COMP_PHL_RECV,_PHL_INFO_,"[3] %s:: [child %p]: offset[%d -> %d] transfer_len=%d\n",
	...
	if(phl_rxhead)
	{
		if(brel)
		{
			PHL_TRACE(COMP_PHL_RECV, _PHL_INFO_, "[3] %s:: prealloc netbuff for all ampdu\n",
								__FUNCTION__);
...
static enum rtw_phl_status phl_rx_usb(struct phl_info_t *phl_info)
{
	...
	while (rx_buf_ring->pend_rxbuf_list.cnt)
	{
		PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "[3] %s:: pend_rxbuf_list.cnt =%d\n",
