...
{
	struct phl_led_timer_args_t *timer_args = (struct phl_led_timer_args_t *) args;
...
#else
	status = phl_disp_eng_send_msg(timer_args->phl_info, &msg, &attr, NULL);
#endif
}
			       ...
			       enum rtw_led_ctrl_mode ctrl_mode)
{
	...
	if (led_ctrl->led_info_arr[led_id].reg_ctrl_mode != ctrl_mode) {
		rtw_hal_led_set_ctrl_mode(phl_info->hal, led_id, ctrl_mode);
...
{
	PHL_TRACE(COMP_PHL_LED, _PHL_ALWAYS_, "===> %s()\n", __func__);
	_os_mem_free(priv, msg->inbuf, sizeof(enum rtw_led_id));
}
				...
				enum rtw_led_opt opt)
{
	...
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	void *drv_priv = phl_to_drvpriv(phl_info);
	struct phl_led_ctrl_t *led_ctrl = (struct phl_led_ctrl_t *)phl_info->led_ctrl;
	struct phl_led_info_t *led_info = NULL;
	struct rtw_led_action_args_t *action_args = NULL;
...
#ifdef RTW_WKARD_AP_CMD_DISPATCH
	_phl_led_ctrl_manual_hdlr(phl_info, (struct rtw_led_action_args_t *)(msg.inbuf));
#else
	if (RTW_PHL_STATUS_SUCCESS == (status = phl_disp_eng_send_msg(phl_info, &msg, &attr, NULL)))
		return;
#endif
	...
	if (status == RTW_PHL_STATUS_UNEXPECTED_ERROR)
		...
		sts = _phl_led_ctrl_write_opt(phl_info->hal, led_id, &(led_info->curr_opt), opt);
