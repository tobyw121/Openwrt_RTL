...
struct mcc_obj_priv {
	...
	_mutex mcc_mutex;
	_lock mcc_lock;
	_adapter *iface[MAX_MCC_NUM]; /* by order, use for mcc parameter cmd */
...
/* backup IQK val */
void rtw_hal_mcc_restore_iqk_val(_adapter *padapter);

/* check mcc status */
u8 rtw_hal_check_mcc_status(_adapter *padapter, u8 mcc_status);

/* set mcc status */
void rtw_hal_set_mcc_status(_adapter *padapter, u8 mcc_status);

/* clear mcc status */
void rtw_hal_clear_mcc_status(_adapter *padapter, u8 mcc_status);
...
/* handle C2H */
void rtw_hal_mcc_c2h_handler(_adapter *padapter, u8 buflen, u8 *tmpBuf);

/* switch channel successfully or not */
void rtw_hal_mcc_sw_status_check(_adapter *padapter);

/* change some scan flags under site survey */
u8 rtw_hal_mcc_change_scan_flag(_adapter *padapter, u8 *ch, u8 *bw, u8 *offset);

/* record data kernel TX to driver to check MCC concurrent TX  */
void rtw_hal_mcc_calc_tx_bytes_from_kernel(_adapter *padapter, u32 len);

/* record data to port to let driver do flow ctrl  */
void rtw_hal_mcc_calc_tx_bytes_to_port(_adapter *padapter, u32 len);

/* check stop write port or not  */
u8 rtw_hal_mcc_stop_tx_bytes_to_port(_adapter *padapter);

u8 rtw_hal_set_mcc_setting_scan_start(_adapter *padapter);

u8 rtw_hal_set_mcc_setting_scan_complete(_adapter *padapter);

u8 rtw_hal_set_mcc_setting_start_bss_network(_adapter *padapter, u8 chbw_grouped);

u8 rtw_hal_set_mcc_setting_disconnect(_adapter *padapter);

u8 rtw_hal_set_mcc_setting_join_done_chk_ch(_adapter *padapter);

u8 rtw_hal_set_mcc_setting_chk_start_clnt_join(_adapter *padapter, u8 *ch, u8 *bw, u8 *offset, u8 chbw_allow);
...
u8 *rtw_hal_mcc_append_go_p2p_ie(_adapter *padapter, u8 *pframe, u32 *len);

void rtw_hal_dump_mcc_policy_table(void *sel);

void rtw_hal_mcc_update_macid_bitmap(_adapter *padapter, int mac_id, u8 add);

void rtw_hal_mcc_process_noa(_adapter *padapter);
