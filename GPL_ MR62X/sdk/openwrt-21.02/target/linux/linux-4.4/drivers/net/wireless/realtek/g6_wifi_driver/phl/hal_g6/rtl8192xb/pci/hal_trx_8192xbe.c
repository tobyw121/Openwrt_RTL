 ...
 *
 *****************************************************************************/
#define _HAL_TRX_8192XBE_C_
#include "../../hal_headers.h"
#include "../rtl8192xb_hal.h"
#include "hal_trx_8192xbe.h"
 ...
 */
static u8 hal_query_txch_num_8192xbe(void)
{
	u8 ch_num = 0;

	ch_num = TX_DMA_CHANNEL_ENTRY_8192XBE;
 ...
 */
static u8 hal_query_rxch_num_8192xbe(void)
{
	u8 ch_num = 0;

	ch_num = RX_DMA_CHANNEL_ENTRY_8192XBE;

	return ch_num;
}
static u8 hal_qsel_to_tid_8192xbe(struct hal_info_t *hal, u8 qsel_id, u8 tid_indic)
{
	u8 tid = 0;
...
/**
 * Get target DMA channel's BD ram hw register of rtl8192xb
 * @dma_ch: input target dma channel with the hw definition of rtl8192xbe
 * return the BD ram hw register
 */
static u32 _hal_get_bd_ram_reg_8192xbe(u8 dma_ch)
{
	u32 reg = 0;

	switch (dma_ch) {
	case ACH0_QUEUE_IDX_8192XBE:
		reg = R_AX_ACH0_BDRAM_CTRL;
		break;
	case ACH1_QUEUE_IDX_8192XBE:
		reg = R_AX_ACH1_BDRAM_CTRL;
		break;
	case ACH2_QUEUE_IDX_8192XBE:
		reg = R_AX_ACH2_BDRAM_CTRL;
		break;
	case ACH3_QUEUE_IDX_8192XBE:
		reg = R_AX_ACH3_BDRAM_CTRL;
		break;
	case ACH4_QUEUE_IDX_8192XBE:
		reg = R_AX_ACH4_BDRAM_CTRL;
		break;
	case ACH5_QUEUE_IDX_8192XBE:
		reg = R_AX_ACH5_BDRAM_CTRL;
		break;
	case ACH6_QUEUE_IDX_8192XBE:
		reg = R_AX_ACH6_BDRAM_CTRL;
		break;
	case ACH7_QUEUE_IDX_8192XBE:
		reg = R_AX_ACH7_BDRAM_CTRL;
		break;
	case MGQ_B0_QUEUE_IDX_8192XBE:
		reg = R_AX_CH8_BDRAM_CTRL;
		break;
	case HIQ_B0_QUEUE_IDX_8192XBE:
		reg = R_AX_CH9_BDRAM_CTRL;
		break;
	case MGQ_B1_QUEUE_IDX_8192XBE:
		reg = R_AX_CH10_BDRAM_CTRL;
		break;
	case HIQ_B1_QUEUE_IDX_8192XBE:
		reg = R_AX_CH11_BDRAM_CTRL;
		break;
	case FWCMD_QUEUE_IDX_8192XBE:
...
/**
 * Get target DMA channel's BD num hw register of rtl8192xb
 * @dma_ch: input target dma channel with the hw definition of rtl8192xbe
 * return the BD num hw register
 */
static u32 _hal_get_bd_num_reg_8192xbe(u8 dma_ch)
{
	u32 reg = 0;
	...
	case FWCMD_QUEUE_IDX_8192XBE:
		...
		break;
	case RX_QUEUE_IDX_8192XBE:
		reg = R_AX_RXQ_RXBD_NUM;
		break;
	case RP_QUEUE_IDX_8192XBE:
...
/**
 * Get target DMA channel's BD Desc hw register of rtl8192xb
 * @dma_ch: input target dma channel with the hw definition of rtl8192xbe
 * return the BD Desc hw register
 */
static void _hal_get_bd_desc_reg_8192xbe(u8 dma_ch, u32 *addr_l, u32 *addr_h)
{
	u32 reg = 0;
...
/**
 * Get target DMA channel's BD Index hw register of rtl8192xb
 * @dma_ch: input target dma channel with the hw definition of rtl8192xbe
 * return the BD Index hw register
 */
static u32 _hal_get_bd_idx_reg_8192xbe(u8 dma_ch)
{
	u32 reg = 0;
 ...
 * to rtl8192xbe hw tx dma channel
 ...
 *
 * returns the mapping hw dma channel defined by XXX_QUEUE_IDX_8192XBE
 * if the input parameter is unknown value, returns ACH0_QUEUE_IDX_8192XBE
 */
static u8 hal_mapping_hw_tx_chnl_8192xbe(u16 macid, enum rtw_phl_ring_cat cat,
					u8 band)
{
	u8 dma_ch = 0;
	...
	if (0 == band) {
		...
		case RTW_PHL_RING_CAT_TID0:
		case RTW_PHL_RING_CAT_TID3:
			dma_ch = ACH4_QUEUE_IDX_8192XBE;
			break;
		case RTW_PHL_RING_CAT_TID1:
		case RTW_PHL_RING_CAT_TID2:
			dma_ch = ACH5_QUEUE_IDX_8192XBE;
			break;
		case RTW_PHL_RING_CAT_TID4:
		case RTW_PHL_RING_CAT_TID5:
			dma_ch = ACH6_QUEUE_IDX_8192XBE;
			break;
		case RTW_PHL_RING_CAT_TID6:
		case RTW_PHL_RING_CAT_TID7:
			dma_ch = ACH7_QUEUE_IDX_8192XBE;
			break;
		case RTW_PHL_RING_CAT_MGNT:
			dma_ch = MGQ_B1_QUEUE_IDX_8192XBE;
			break;
		case RTW_PHL_RING_CAT_HIQ:
			dma_ch = HIQ_B1_QUEUE_IDX_8192XBE;
			break;
		default:
			dma_ch = ACH0_QUEUE_IDX_8192XBE;
			PHL_TRACE(COMP_PHL_DBG, _PHL_WARNING_, "[WARNING]unknown category (%d)\n",
 ...
 */
static u16 hal_get_avail_txbd_8192xbe(struct rtw_hal_com_t *hal_com, u8 ch_idx,
				     u16 *host_idx, u16 *hw_idx)
{
	...
	tx_dma_ch = ACH0_QUEUE_IDX_8192XBE + ch_idx;

	reg = _hal_get_bd_idx_reg_8192xbe(tx_dma_ch);
	if (0xFFFF == reg) {
		...
		*hw_idx = (u16)((tmp32 >> 16) & 0x0FFF);
		...
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_get_avail_txbd_8192xbe => dma_ch %d, host_idx %d, "
...
static u16 hal_get_avail_txbd_sw_8192xbe(struct rtw_hal_com_t *hal_com,
...
static u16 hal_get_rxbd_num_8192xbe(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
...
static u16 hal_get_rxbuf_num_8192xbe(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
...
static u16 hal_get_rxbuf_size_8192xbe(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
 ...
 * NOTE, input host_idx and hw_idx ptr shall NOT be NULL
 */
static u16 hal_get_avail_rxbd_8192xbe(struct rtw_hal_com_t *hal_com, u8 ch_idx,
				     u16 *host_idx, u16 *hw_idx)
{
	u16 avail_rxbd = 0;
	u32 tmp32 = 0, reg = 0;
	u8 rx_dma_ch = 0;
	u16 rxbd_num = hal_get_rxbd_num_8192xbe(hal_com, ch_idx);

	rx_dma_ch = RX_QUEUE_IDX_8192XBE + ch_idx;

	reg = _hal_get_bd_idx_reg_8192xbe(rx_dma_ch);
	if (0xFFFF == reg) {
...
void _hal_fill_wp_seq_field_8192xbe(u8 *seq_info, u16 wp_seq)
{
	/* update sw wp seq */
...
void _hal_fill_wp_addr_info_8192xbe(struct rtw_hal_com_t *hal_com,
					...
					u8 num, u8 mpdu_ls, u8 msdu_ls, bool lastwp)
{
	...
	/* because there isn't MSDU_LS for 8192XB, driver should fill LS, provide by WD1 daniel */
	if(lastwp) {
...
u8 _hal_get_tid_indic_8192xbe(u8 tid)
{
	u8 tid_indic = 0;
	switch (tid) {
		case RTW_PHL_RING_CAT_TID0:
...
_hal_txsc_update_wd_8192xbe(struct hal_info_t *hal,
						struct rtw_phl_pkt_req *req, u32 *wd_len)
{
	...
	if (mdata->hw_seq_mode) {
		static const u32 tid_2_qsel[8] = {0 ,1, 1, 0, 2, 2, 3, 3};
		mdata->hw_ssn_sel = tid_2_qsel[mdata->tid];
	}
#endif /* CONFIG_RTW_TXSC_USE_HW_SEQ */
	dma_ch = hal_mapping_hw_tx_chnl_8192xbe(mdata->macid,
											mdata->cat,
											mdata->band);
 ...
 */
static enum rtw_hal_status
hal_update_wd_8192xbe(struct hal_info_t *hal,
				struct rtw_phl_pkt_req *req)
{
	...
	do {
...
#ifdef CONFIG_PHL_TXSC
		hstatus = _hal_txsc_update_wd_8192xbe(hal, req, &wd_len);
#else
		/* connect with halmac */
		hstatus = rtw_hal_mac_ax_fill_txdesc(hal->mac, tx_req, req->wd_page,
						&wd_len);
		...
		tid_indic = _hal_get_tid_indic_8192xbe(tx_req->mdata.tid);

		buf_len += (u8)wd_len;
		seq_ofst = wd_len;
		wp_seq = (1 == tid_indic) ?
				(req->wp_seq | WP_TID_INDIC_RESERVED_BIT) : req->wp_seq;
		_hal_fill_wp_seq_field_8192xbe(req->wd_page + seq_ofst, wp_seq);
		...
		for (i = 0; i < tx_req->mdata.addr_info_num; i++) {
			if (pkt_list[i].length > MAX_SUB_PKT_SIZE_8192XBE) {
				PHL_TRACE(COMP_PHL_XMIT, _PHL_ERR_,
				          "sub pkt %uB overflow! (%u)\n",
				          pkt_list[i].length,
				          MAX_SUB_PKT_SIZE_8192XBE);
			...
			if(i == tx_req->mdata.addr_info_num - 1) {
				_hal_fill_wp_addr_info_8192xbe(hal_com,
 ...
 */
static enum rtw_hal_status
hal_update_txbd_8192xbe(struct hal_info_t *hal,
		       ...
		       u8 ch_idx, u16 wd_num)
{
	...
	do {
		...
		avail_txbd = hal_get_avail_txbd_sw_8192xbe(hal->hal_com, txbd_ring, ch_idx,
						       &txbd_host_idx, &hw_idx);
		...
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_update_txbd_8192xbe => ch_idx %d, host_idx %d, "
		...
		while (wcnt > 0) {
			...
			if (ch_idx == 9) {
				u32 *wd32 = (u32 *)wd_page->vir_addr;
				u32 w2 = le32_to_cpu(wd32[2]);
				u32 wp_len = (w2 >> AX_TXD_TXPKTSIZE_SH) && AX_TXD_TXPKTSIZE_MSK;

				if (wp_len > 1500) {
					PHL_ERR("CH9 WP len %u!\n", wp_len);
				}
			}
 ...
 */
static enum rtw_hal_status
hal_trigger_txdma_8192xbe(struct hal_info_t *hal,
			struct tx_base_desc *txbd_ring, u8 ch_idx)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	u8 tx_dma_ch;
	...
	do {
		/* connect with halmac */
		tx_dma_ch = ACH0_QUEUE_IDX_8192XBE + ch_idx;
		txbd_reg = _hal_get_bd_idx_reg_8192xbe(tx_dma_ch);
		if (0xFFFF == txbd_reg)
			break;
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_trigger_txdma_8192xbe => dma_ch %d, host_idx %d.\n",
...
static enum rtw_hal_status hal_pltfm_tx_8192xbe(void *hal, struct rtw_h2c_pkt *h2c_pkt)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
#if 0
	...
	_os_spinlock(hal_to_drvpriv(hal_info), &txbd_ring[FWCMD_QUEUE_IDX_8192XBE].txbd_lock, _ps, NULL);
	hstatus = hal_update_txbd_8192xbe(hal_info, txbd_ring, &wd_page,
							FWCMD_QUEUE_IDX_8192XBE, 1);
	_os_spinunlock(hal_to_drvpriv(hal_info), &txbd_ring[FWCMD_QUEUE_IDX_8192XBE].txbd_lock, _ps, NULL);

	/* enqueue busy queue */

	hstatus = hal_trigger_txdma_8192xbe(hal_info, txbd_ring, FWCMD_QUEUE_IDX_8192XBE);
#endif
	return hstatus;
}

u8 hal_get_fwcmd_queue_idx_8192xbe(void)
{
	return FWCMD_QUEUE_IDX_8192XBE;
}

static u8 hal_check_rxrdy_8192xbe(struct rtw_phl_com_t *phl_com,
...
#ifdef PHL_PLATFORM_ECOS
#define RX_TAG_POLLING_TIMES (500)
#else
#define RX_TAG_POLLING_TIMES (100)
#endif /* PHL_PLATFORM_ECOS */
#else
#define RX_TAG_POLLING_TIMES (100000)
	...
	if (true == res) {
		hal_spec->rx_tag[ch_idx] = target_tag;
	} else {
	...
	/* RX ready. Ensure cache is cleared for all payload. */
	#if defined(PHL_DMA_NONCOHERENT)
	if ((res == true) && cache) {
...
static void hal_rewind_rx_tag_8192xbe(struct rtw_phl_com_t *phl_com,
                                      u8 ch_idx)
{

	struct hal_spec_t *hal_spec = &phl_com->hal_spec;
	u16	rewind_tag = hal_spec->rx_tag[ch_idx];
	...
	if (hal_spec->rx_tag[ch_idx] == 0)
		rewind_tag = 0x1fff;

	PHL_PRINT("R%u %u-%u\n", ch_idx, hal_spec->rx_tag[ch_idx], rewind_tag);

	hal_spec->rx_tag[ch_idx] = rewind_tag;
}

static void _hal_show_tx_failure_rsn_8192xbe(u8 txsts)
{
...
u16 hal_handle_rx_report_8192xbe(struct hal_info_t *hal, u8 *rp,
				...
				u8 *macid, u8* ac_queue, u8 *txsts)
{
	...
	do {
		...
		*wp_seq &= (WP_RESERVED_SEQ);
		tid = hal_qsel_to_tid_8192xbe(hal, qsel_value, tid_indic);
		*dma_ch = hal_mapping_hw_tx_chnl_8192xbe(*macid, tid, band);
		...
		if (TX_STATUS_TX_DONE != *txsts) {
			...
			_hal_show_tx_failure_rsn_8192xbe(*txsts);
			*sw_retry = true;
 ...
 */
static enum rtw_hal_status
hal_handle_ppdusts_8192xbe(void *hal, u8 *psbuf, u16 sz,
 ...
 **/
static enum rtw_hal_status
_hal_parsing_rx_wd_8192xbe(struct hal_info_t *hal, u8 *desc,
						struct rtw_r_meta_data *mdata)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;

	mdata->pktlen = GET_RX_AX_DESC_PKT_LEN_8192XB(desc);
	mdata->shift = GET_RX_AX_DESC_SHIFT_8192XB(desc);
	mdata->wl_hd_iv_len = GET_RX_AX_DESC_HDR_IV_L_8192XB(desc);
	mdata->bb_sel = GET_RX_AX_DESC_BB_SEL_8192XB(desc);
	mdata->mac_info_vld = GET_RX_AX_DESC_MAC_INFO_VLD_8192XB(desc);
	mdata->rpkt_type = GET_RX_AX_DESC_RPKT_TYPE_8192XB(desc);
	mdata->drv_info_size = GET_RX_AX_DESC_DRV_INFO_SIZE_8192XB(desc);
	mdata->long_rxd = GET_RX_AX_DESC_LONG_RXD_8192XB(desc);

	mdata->ppdu_type = GET_RX_AX_DESC_PPDU_TYPE_8192XB(desc);
	mdata->ppdu_cnt = GET_RX_AX_DESC_PPDU_CNT_8192XB(desc);
	mdata->sr_en = GET_RX_AX_DESC_SR_EN_8192XB(desc);
	mdata->user_id = GET_RX_AX_DESC_USER_ID_8192XB(desc);
	mdata->rx_rate = GET_RX_AX_DESC_RX_DATARATE_8192XB(desc);
	mdata->rx_gi_ltf = GET_RX_AX_DESC_RX_GI_LTF_8192XB(desc);
	mdata->non_srg_ppdu = GET_RX_AX_DESC_NON_SRG_PPDU_8192XB(desc);
	mdata->inter_ppdu = GET_RX_AX_DESC_INTER_PPDU_8192XB(desc);
	mdata->bw = GET_RX_AX_DESC_BW_8192XB(desc);

	mdata->freerun_cnt = GET_RX_AX_DESC_FREERUN_CNT_8192XB(desc);

	mdata->a1_match = GET_RX_AX_DESC_A1_MATCH_8192XB(desc);
	mdata->sw_dec = GET_RX_AX_DESC_SW_DEC_8192XB(desc);
	mdata->hw_dec = GET_RX_AX_DESC_HW_DEC_8192XB(desc);
	mdata->ampdu = GET_RX_AX_DESC_AMPDU_8192XB(desc);
	mdata->ampdu_end_pkt = GET_RX_AX_DESC_AMPDU_EDN_PKT_8192XB(desc);
	mdata->amsdu = GET_RX_AX_DESC_AMSDU_8192XB(desc);
	mdata->amsdu_cut = GET_RX_AX_DESC_AMSDU_CUT_8192XB(desc);
	mdata->last_msdu = GET_RX_AX_DESC_LAST_MSDU_8192XB(desc);
	mdata->bypass = GET_RX_AX_DESC_BYPASS_8192XB(desc);
	mdata->crc32 = GET_RX_AX_DESC_CRC32_8192XB(desc);
	mdata->icverr = GET_RX_AX_DESC_ICVERR_8192XB(desc);
	mdata->magic_wake = GET_RX_AX_DESC_MAGIC_WAKE_8192XB(desc);
	mdata->unicast_wake = GET_RX_AX_DESC_UNICAST_WAKE_8192XB(desc);
	mdata->pattern_wake = GET_RX_AX_DESC_PATTERN_WAKE_8192XB(desc);

	if (mdata->long_rxd==1) {
		mdata->macid = GET_RX_AX_DESC_MACID_8192XB(desc);
	}
...
static u8 hal_handle_rxbd_info_8192xbe(struct hal_info_t *hal,
 ...
 */
static enum rtw_hal_status
hal_update_rxbd_8192xbe(struct hal_info_t *hal, struct rx_base_desc *rxbd,
		       struct rtw_rx_buf *rx_buf, u8 ch_idx)
{
	...
	do {
		...
		if (hal_com->cv != CAV) {
			/* 8192XB doesn't support normal mode */
 ...
 */
static enum rtw_hal_status
hal_notify_rxdone_8192xbe(struct hal_info_t *hal,
		struct rx_base_desc *rxbd, u8 ch, u16 rxcnt)
{
	...
	do {
		rx_dma_ch = RX_QUEUE_IDX_8192XBE + ch;
		reg = _hal_get_bd_idx_reg_8192xbe(rx_dma_ch);
		/* connect with halmac */
		if (0xFFFF == reg)
			break;
...
struct bd_ram bdram_table_8192xb[] = {
		/* ACH0_QUEUE_IDX_8192XBE */ {0, 25, 4},
		/* ACH1_QUEUE_IDX_8192XBE */ {25, 25, 4},
		/* ACH2_QUEUE_IDX_8192XBE */ {50, 25, 4},
		/* ACH3_QUEUE_IDX_8192XBE */ {75, 25, 4},
		/* ACH4_QUEUE_IDX_8192XBE */ {100, 25, 4},
		/* ACH5_QUEUE_IDX_8192XBE */ {125, 25, 4},
		/* ACH6_QUEUE_IDX_8192XBE */ {150, 25, 4},
		/* ACH7_QUEUE_IDX_8192XBE */ {175, 25, 4},
		/* MGQ_B0_QUEUE_IDX_8192XBE */ {185, 10, 4},
		/* HIQ_B0_QUEUE_IDX_8192XBE */ {195, 10, 4},
		/* MGQ_B1_QUEUE_IDX_8192XBE */ {205, 10, 4},
		/* HIQ_B1_QUEUE_IDX_8192XBE */ {215, 10, 4},
		/* FWCMD_QUEUE_IDX_8192XBE */ {225, 10, 4}
};

static void _hal_tx_init_bd_ram_8192xbe(struct hal_info_t *hal)
{
	u32 reg = 0;
	u32 value = 0;
	u8 i = 0;

	for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8192XBE; i++) {
		/*if (FWCMD_QUEUE_IDX_8192XBE == i)
			continue;*/
		value = 0;
		value = bdram_table_8192xb[i].sidx +
			((bdram_table_8192xb[i].max << 8) & 0xFF00) +
			((bdram_table_8192xb[i].min << 16) & 0xFF0000);

		reg = _hal_get_bd_ram_reg_8192xbe(i);
		if (0 != reg)
...
static void _hal_trx_init_bd_num_8192xbe(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u16 txbd_num = (u16)hal_com->bus_cap.txbd_num;
	u16 rxbd_num = 0;
	...
	for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8192XBE; i++) {
		...
		reg = _hal_get_bd_num_reg_8192xbe(i);
		if (0 != reg)
	...
	for (i = RX_QUEUE_IDX_8192XBE;
		 i < RX_QUEUE_IDX_8192XBE + RX_DMA_CHANNEL_ENTRY_8192XBE; i++) {
		 /*if (FWCMD_QUEUE_IDX_8192XBE == i)
			 continue;*/

		 ch_idx = (u8)(i - RX_QUEUE_IDX_8192XBE);
		 rxbd_num = hal_get_rxbd_num_8192xbe(hal_com, ch_idx);
...
static void _hal_trx_init_bd_8192xbe(struct hal_info_t *hal, u8 *txbd_buf, u8 *rxbd_buf)
{
	...
	if (NULL != txbd_buf && NULL != rxbd_buf) {
		...
		for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8192XBE; i++) {
			...
			_hal_get_bd_desc_reg_8192xbe(i, &reg_addr_l, &reg_addr_h);
			if (0 != reg_addr_l)
		...
		for (i = 0; i < RX_DMA_CHANNEL_ENTRY_8192XBE; i++) {
			 rxch_idx = i + RX_QUEUE_IDX_8192XBE;
			_hal_get_bd_desc_reg_8192xbe(rxch_idx, &reg_addr_l, &reg_addr_h);
			if (0 != reg_addr_l)
...
static void hal_cfg_wow_txdma_8192xbe(struct hal_info_t *hal, u8 en)
{
...
static u8 hal_poll_txdma_idle_8192xbe(struct hal_info_t *hal)
{
	struct mac_ax_txdma_ch_map ch_map;
...
/**
 * the function will deinitializing 8192xbe specific data and hw configuration
 */
static void hal_trx_deinit_8192xbe(struct hal_info_t *hal)
{
	/*struct rtw_hal_com_t *hal_com = hal->hal_com;*/
}

/**
 * the function will initializing 8192xbe specific data and hw configuration
 */
static enum rtw_hal_status hal_trx_init_8192xbe(struct hal_info_t *hal, u8 *txbd_buf, u8 *rxbd_buf)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
 ...
 */
void hal_query_freerun_cnt_8192xb(struct hal_info_t *hal, u32 *freerun_l, u32 *freerun_h)
{
...
enum rtw_hal_status
hal_set_dlru_fix_tbl_8192xbe(struct hal_info_t *hal, struct rtw_phl_dlru_fix_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
...
enum rtw_hal_status
hal_set_ulru_fix_tbl_8192xbe(struct hal_info_t *hal, struct rtw_phl_ulru_fix_tbl *tbl)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
...
#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
extern u16 rtw_hal_tx_res_query(void *hal, u8 dma_ch, u16 *host_idx, u16 *hw_idx);
__IMEM_WLAN_SECTION__
enum rtw_hal_status hal_trx_watchdog_8192xbe(struct rtw_phl_com_t *phl_com,
                         struct hal_info_t *hal)
{
	...
	do {
		/* RX read pointer */
		if(phl_com->check_flag.rxq_stuck_check){
		...
		/* ACH Q */
		if(phl_com->check_flag.achq_stuck_check){
			for (ch = 0; ch < 8; ch++) {
				rtw_hal_tx_res_query(hal, ch, &host_idx, &hw_idx);
				if ((host_idx != hw_idx)
					&& (hw_idx == trx_wdg_data->last_tx_ach_hw_idx[ch])) {
					PHL_INFO("ACH%d Q stuck: host_idx %d hw_idx %d\n", ch, host_idx, hw_idx);
					fail_mask |= BIT(fail_idx);
				}
				trx_wdg_data->last_tx_ach_hw_idx[ch] = hw_idx;
				++fail_idx;
			}
		}
		...
		/* TX read pointer */
		/* MGNT Q */
		if(phl_com->check_flag.mgntq_stuck_check){
		...
		/* Hi Q */
		if(phl_com->check_flag.hiq_stuck_check){
		...
		/* H2C Q */
		if(phl_com->check_flag.h2cq_stuck_check){
		...
		if(phl_com->check_flag.rx_cck_ofdm_cnt_check){
		...
		/* RX data count */
		if(phl_com->check_flag.rx_data_cnt_check){
		...
		if(phl_com->check_flag.tx_cck_ofdm_cnt_check){
...
void hal_trx_ops_init_8192xbe(void)
{
	ops.init = hal_trx_init_8192xbe;
	ops.deinit = hal_trx_deinit_8192xbe;
	ops.query_tx_res = hal_get_avail_txbd_8192xbe;
	ops.query_tx_res_sw = hal_get_avail_txbd_sw_8192xbe;
	ops.query_rx_res = hal_get_avail_rxbd_8192xbe;
	ops.get_rxbd_num = hal_get_rxbd_num_8192xbe;
	ops.get_rxbuf_num = hal_get_rxbuf_num_8192xbe;
	ops.get_rxbuf_size = hal_get_rxbuf_size_8192xbe;
	ops.cfg_wow_txdma = hal_cfg_wow_txdma_8192xbe;
	ops.poll_txdma_idle = hal_poll_txdma_idle_8192xbe;
	ops.map_hw_tx_chnl = hal_mapping_hw_tx_chnl_8192xbe;
	ops.qsel_to_tid = hal_qsel_to_tid_8192xbe;
	ops.query_txch_num = hal_query_txch_num_8192xbe;
	ops.query_rxch_num = hal_query_rxch_num_8192xbe;
	ops.update_wd = hal_update_wd_8192xbe;
	ops.update_txbd = hal_update_txbd_8192xbe;
	ops.tx_start = hal_trigger_txdma_8192xbe;
	ops.check_rxrdy = hal_check_rxrdy_8192xbe;
	ops.handle_rxbd_info = hal_handle_rxbd_info_8192xbe;
	ops.handle_rx_buffer = hal_handle_rx_buffer_8192xb;
	#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
	ops.check_next_msdu = hal_check_next_msdu_8192xb;
	ops.rewind_rx_tag = hal_rewind_rx_tag_8192xbe;
	#endif /* CONFIG_RTW_HW_RX_AMSDU_CUT */
	ops.update_rxbd = hal_update_rxbd_8192xbe;
	ops.notify_rxdone = hal_notify_rxdone_8192xbe;
	ops.handle_wp_rpt = hal_handle_rx_report_8192xbe;
	ops.query_freerun = hal_query_freerun_cnt_8192xb;
	ops.get_fwcmd_queue_idx = hal_get_fwcmd_queue_idx_8192xbe;
#ifdef CONFIG_WFA_OFDMA_Logo_Test
	ops.set_dlru_fix_tbl = hal_set_dlru_fix_tbl_8192xbe;
	ops.set_ulru_fix_tbl = hal_set_ulru_fix_tbl_8192xbe;
#endif
#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
	ops.trx_watchdog = hal_trx_watchdog_8192xbe;
#endif /* CONFIG_RTW_HW_TRX_WATCHDOG */
}



u32 hal_hook_trx_ops_8192xbe(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal_info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;

	if (NULL != hal_info) {
		hal_trx_ops_init_8192xbe();
