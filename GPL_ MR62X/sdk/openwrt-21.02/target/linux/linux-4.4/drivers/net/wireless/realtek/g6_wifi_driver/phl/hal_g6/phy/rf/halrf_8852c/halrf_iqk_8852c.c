			      ...
			      u8 path)
{
	...
	kfail = _iqk_check_nbiqc_8852c(rf, phy_idx, ID_NBTXK, path);
	if (!kfail) {
...
//============START : _lok_coarse_fine_wi_swap ========
	...
	tmp = _iqk_one_shot_8852c(rf, phy_idx, path, tmp_id);
				...
				u8 path)
{
	...
	iqk_info->iqk_tx_fail[0][path] = _iqk_nbtxk_8852c(rf, phy_idx, path);
...
#if 0
__iram_func__
void halrf_doiqk_8852c(struct rf_info *rf, bool force, enum phl_phy_idx phy_idx, u8 path)
{
	...
	iqk_info->version = iqk_version_8852c;
	iqk_info->iqk_step = 0;
	...
	if (iqk_mcc_page_sel_8852c(rf,phy_idx, path))
		return;
	iqk_get_ch_info_8852c(rf,phy_idx, path);
	iqk_backup_mac_bb_8852c(rf, path, backup_mac, backup_bb, backup_mac_reg, backup_bb_reg);
	iqk_backup_rf0_8852c(rf, path, backup_rf0, backup_rf_reg0);
	iqk_backup_rf1_8852c(rf, path, backup_rf1, backup_rf_reg1);
	iqk_macbb_setting_8852c(rf, phy_idx, path);
	iqk_preset_8852c(rf, path);
	iqk_start_iqk_8852c(rf, phy_idx, path);
	iqk_restore_8852c(rf, path);
	iqk_afebb_restore_8852c(rf, phy_idx, path);	
	iqk_restore_mac_bb_8852c(rf, path, backup_mac, backup_bb, backup_mac_reg, backup_bb_reg);
	iqk_restore_rf0_8852c(rf, path, backup_rf0, backup_rf_reg0);
	iqk_restore_rf1_8852c(rf, path, backup_rf1, backup_rf_reg1);
	iqk_info->iqk_times++;
...
void halrf_iqk_8852c(struct rf_info *rf, enum phl_phy_idx phy_idx, bool force)
{
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	iqk_init_8852c(rf);
	switch(halrf_kpath_8852c(rf, phy_idx)) {
	case RF_A:
		halrf_doiqk_8852c(rf, force, phy_idx, RF_PATH_A);
		break;
	case RF_B:		
		halrf_doiqk_8852c(rf, force, phy_idx, RF_PATH_B);
		break;
	case RF_AB:		
		halrf_doiqk_8852c(rf, force, phy_idx, RF_PATH_A);
		halrf_doiqk_8852c(rf, force, phy_idx, RF_PATH_B);
