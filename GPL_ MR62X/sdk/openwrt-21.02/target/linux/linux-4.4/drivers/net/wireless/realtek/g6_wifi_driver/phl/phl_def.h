...
enum phl_ac_queue {
	...
	PHL_VI_QUEUE_SEL		= 2,
	PHL_VO_QUEUE_SEL		= 3,
	PHL_AC_QUEUE_TOTAL     = 4, /* do not move this element */
	PHL_AC_QUEUE_SUM
};
#ifdef PHL_RXSC_AMPDU
#define RXSC_INVALID_MACID	0x100
#endif
/**
...
/**
 * struct rtw_grp_info
 * @tx_list:list for tx performance
 * @rx_list:list for tx performance
 **/
struct rtw_grp_info {
	_os_list tx_list;
	_os_list rx_list;
	...
	u8 is_dlru_sta;
	u8 tx_cap_grp_idx;/* 0~31, bit 0~1:BW ; bit 2:stbc_he_rx ; bit 3~4:rate */
	u8 tx_grp_idx;/* 0~254: grp idx, 255: No grp */
	u8 tx_grp_idx_old;/* 0~254: grp idx, 255: No grp */

	u8 is_ulru_sta;
	u8 rx_cap_grp_idx;
	u8 rx_grp_idx;/* 0~254: grp idx, 255: No grp */
	u8 rx_grp_idx_old;/* 0~254: grp idx, 255: No grp */
};
#endif
...
enum rtw_phl_handler_id {
	...
	RTW_PHL_EVT_HANDLER,
	RTW_PHL_SER_HANDLER,
	RTW_PHL_FW_WDT_HANDLER,
	#ifdef CONFIG_PHL_TEST_SUITE
        RTW_PHL_TEST_HANDLER,
	#endif
};
...
enum phl_msg_evt_id {
	...
	MSG_EVT_HAL_START_OK = 12,
	MSG_EVT_HAL_START_FAIL = 13,
	...
	/* update tpe info */
	MSG_EVT_TPE_INFO_UPDATE = 124,

	/*DFS radar detecting*/
	MSG_EVT_DFS_RD_IS_DETECTING = 124,
	MSG_EVT_DFS_RD_SETUP = 125,
...
struct protocol_cap_t {
	...
	u8 stbc_vht_tx:1;
	u8 stbc_he_tx:1;
	u8 ltf_gi_cap; /* ltf_gi from STA's cap */
	u8 ltf_gi; /* ltf_gi final decision */
	u8 doppler_tx:1;
	u8 doppler_rx:1;
...
struct bus_sw_cap_t {
	...
	u32 rxbuf_size;
	u32 rpbuf_size;
	u32 rxbuf_num_effective;
	u32 rpbuf_num_effective;
	u32 rxbuf_size_effective;
	u32 rpbuf_size_effective;
...
struct rtw_wifi_role_t {
	...
	_os_atomic hiq_busy_cnt;
	u32 last_hiq_wp_time;
	u8 dropping_hiq;
...
#ifdef CONFIG_WFA_OFDMA_Logo_Test
struct rtw_HE_TB_stats {
	u32 HE_TB_rx_cnt;

	u32 HE_TB_rx_rate[4];
	u32 HE_TB_rate_cnt_per_sec[4];
	u32 HE_TB_avg_rx_rate; /* (HE_TB_rx_rate / HE_TB_rate_cnt_per_sec) */
	...
	u32 ampdu_agg_total_1;
	u32 avg_ampdu_agg_1; /* (ampdu_agg_total_1 / 50) */
	u32 ampdu_size_total_1;
	u32 avg_ampdu_size_1; /* (ampdu_size_total_1 / 50) */
	u32 avg_mpdu_size_1;
	u64 zero_padding_total_1;
	u32 avg_zero_padding_1; /* (zero_padding_total_1 / 50) */

	u32 ampdu_cnt_2;
	u32 ampdu_cnt_per_sec;
	u32 ampdu_agg_total_2;
	u32 avg_ampdu_agg_2; /* (ampdu_agg_total_2 / ampdu_cnt_per_sec) */
	u32 ampdu_size_total_2;
	u32 avg_ampdu_size_2; /* (ampdu_size_total_2 / ampdu_cnt_per_sec) */
	u8 avg_mpdu_size_2;
	u64 zero_padding_total_2;
	u32 avg_zero_padding_2; /* (zero_padding_total_2 / ampdu_cnt_per_sec) */
...
struct rtw_stats {
...
#ifdef CONFIG_WFA_OFDMA_Logo_Test
	/* su tx rate avg */
	u32 HE_tx_rate[4]; /* 1ss ~ 4ss */
	u32 HE_tx_cnt[4];
	u8  check_intvl;
	u16 average_HE_tx_rate_new;
	u16 average_HE_tx_rate_last;
	u8 last_rssi;

	/* su rx rate avg */
	u32 HE_rx_rate[4]; /* 1ss ~ 4ss */
	u32 HE_rx_cnt[4];
	u16 average_HE_rx_rate;

	u32 HE_rx_rssi;
	u32 HE_rx_rssi_cnt;
	u8 average_HE_rx_rssi;
	u16 HE_rx_avg_mcs;
	u8 HE_rx_avg_ss;
	...
	u32 ampdu_size;
	u32 ampdu_agg_total;
	u32 avg_ampdu_agg; /* (ampdu_agg_total / ampdu_cnt_per_sec) */
	u32 ampdu_size_total;
	u32 avg_ampdu_size; /* (ampdu_size_total / ampdu_cnt_per_sec) */

	struct rtw_HE_TB_stats HE_TB_stat;
#endif
};

#ifdef CONFIG_WFA_OFDMA_Logo_Test
struct rtw_ru_c2h_stats {
	u8 rssi_m;
	...
	u16 ru_rx_tf_cnt;
	u16 ru_rx_bsr_len;
	u8 ru_rx_uph_headroom;
	u8 ru_rx_min_flag_ratio;
	u8 ru_rx_evm;
};

struct rtw_phl_mac_ax_ru_fwc2h_en {
	u16 en;
	u16 intvl_ms;
};
#endif
...
struct rtw_phl_stainfo_t {
	...
	#if 0  //#ifdef CONFIG_WFA_OFDMA_Logo_Test		// Mark.CS_update
	struct rtw_ru_c2h_stats ru_stats;
	#endif
	...
	u32 add_tx_ring_ok;
	u32 add_tx_ring_fail;

	u32 enq_wd_pending_ok;
	u32 enq_wd_pending_fail;

	u32 wd_enq_pending;
	u32 wd_deq_pending;

	u32 wd_enq_busy;
	u32 wd_enq_busy_fail;
	u32 wd_deq_busy;

	u32 phl_tx_ring_start_cnt;
	#endif

	#ifdef CONFIG_WFA_OFDMA_Logo_Test
	struct rtw_ru_c2h_stats ru_stats;		// Mark.CS_update

	u16 cur_tx_data_rate;
	u16 cur_rx_data_rate;

	u16 phl_muru_wd_quota;
	u16 phl_su_wd_quota;

	_os_atomic cnt_current_wd;
	_os_atomic cnt_pending_wd;

	u32 current_wd_record;
	u32 pending_wd_record;

	u32 cnt_wd_over_quota;
	u32 cnt_wd_force_out;
	u32 cnt_wd_normal_out;

	u32 cnt_hold_times;
	//u16 su_wd_quota;

	struct rtw_grp_info grp_info;
	#endif
...
#define HW_SUP_RX_AMSDU_CUT		BIT11
...
struct hal_spec_t {
	...
	#ifdef CONFIG_WFA_OFDMA_Logo_Test	/* OFDMA*/
	u8 fw_dlru_grp_num;
	u8 fw_ulru_grp_num;
	u8 hw_dlru_grp_num;
	u8 hw_ulru_grp_num;
...
enum mr_tdmra_trigger {
	MR_TDMRA_TRIG_BY_BT,
	MR_TDMRA_TRIG_BY_LINKING,
	MR_TDMRA_TRIG_BY_DIS_LINKING,
	MR_TDMRA_TRIG_BY_CHG_SLOT,
	MR_TDMRA_TRIG_MAX,
};
...
#ifdef CONFIG_WFA_OFDMA_Logo_Test
struct ru_common_ctrl		// Mark.CS_update
{
	u8 GRP_CALLBACK_ONCE;
	u8 GRP_DL_ON;
	u8 GRP_UL_ON;
	u8 GRP_FORCE_FILL_DL_TBL;
	u8 GRP_FORCE_FILL_UL_TBL;

	u32 TX_TP_THRD;
	u32 RX_TP_THRD;

	/* 0:grp table not exist;
	[DL-OFDMA] BIT0(1):4RU(DL); BIT1(2):8RU(DL); BIT2(4):2RU(DL); BIT3(8):3RU(DL)
	[UL-OFDMA] BIT4(16):4RU(UL); BIT5(32):3RU(UL); BIT6(64):2RU(UL)
	*/
	u8 tbl_exist;
	u8 tx_phase;
 	u8 rotate;

	u8 ofdma_WFA_mode; /* 0: normal mode; 1: logo mode */
	u8 auto_config; /*0: by IO setting;	1: by STA capability */
	u8 rsp_type;
	u8 prot_type;
	u8 force_sumuru_en;
	u8 forceru;
	u8 forcesu;
	u8 ulmacid_cfg;
	u8 ulmacid_cfg_fix; /* 0: dynamic; 1: fix */
	u8 ul_psd; /* 0: hard code; 1: decided by rssi_m */
	u8 psd_ofst1;
	u8 psd_ofst2;

	u8 ul_crc32; /* 0: disable crc32 pkt; 1: enable crc32 pkt */

	u8 ru_rate_idx1; /* default MCS 9 */
	u8 ru_rate_idx2; /*default MCS 5 */

	u8 ru_rssi_level1; /* default 65 */
	u8 ru_rssi_level2; /* default 50 */

	u32 netif_drop_thd;

	u32 phl_wd_hi_thd;
	u32 phl_wd_hold_cnt_thd;
	u32 phl_wd_quota[9];

	u8 mu_edca;
};

struct dl_ru_rate_ent
{
	u8 mcs;
	u8 ss;
	u8 dcm;
...
struct dl_ru_grp_tf_ba_tbl
{
	u32 tf_rate;
	struct dl_ru_rate_ent rate;
	u8 tb_ppdu_bw;
	u8 gi_ltf;
	u8 fix_ba;
	u32 ru_psd;
...
struct dl_ru_grp_table_para
{
	u16 tx_pwr;
	int ppdu_bw;
	u8 txpwr_ofld_en;
	u8 pwrlim_dis;
	struct dl_ru_grp_tf_ba_tbl tf;
};

struct dl_fix_sta_info
{
	u16 macid;
	u8 mcs;
	u8 ss;
...
struct dl_ru_fix_grp_table_para
{
	u8 max_sta_num;
	u8 min_sta_num;
	u8 rupos_csht_flag;
	...
	struct dl_fix_sta_info sta_info[8];
};

struct ul_ru_grp_table_para
{
	enum channel_width ppdu_bw;
	u16 grp_psd_max;
	u16 grp_psd_min;
	u8 fix_tf_rate;
	u8 rf_gain_fix;
	u8 fix_mode_flags;
	enum rtw_data_rate tf_rate;
	u16 rf_gain_idx;
};

struct ul_fix_sta_info
{
	u16 macid;
	u8 mcs;
	u8 ss;
...
struct ul_ru_fix_grp_table_para
{
	u8 min_sta_num;
	u8 max_sta_num;
	int gi_ltf;
	...
	struct ul_fix_sta_info sta_info[8];
};
...
struct ulmacid_cfg_set_para
{
	struct _ulmacid_cfg ulmacid_cfg[8];
};

struct ru_grp_table
{
	struct ru_common_ctrl ru_ctrl;
	struct dl_ru_grp_table_para dl_ru_grp_table;
	struct dl_ru_fix_grp_table_para dl_ru_fix_grp_table;

	struct ul_ru_grp_table_para ul_ru_grp_table;
	struct ul_ru_fix_grp_table_para ul_ru_fix_grp_table;

	struct ulmacid_cfg_set_para ulmacid_cfg_set;
};
#endif
...
struct phl_rx_stats {
	...
	u32 rxbuf_empty;
	u32 rxbd_refill_fail;
#ifdef RTW_RX_CPU_BALANCE
	u32 refill_cpu[CONFIG_NR_CPUS];
#endif /* RTW_RX_CPU_BALANCE */
#endif /* CONFIG_DYNAMIC_RX_BUF */
#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
	u32 amsdu_cut_ok;
	u32 amsdu_cut_break;
	u32 amsdu_cut_cur_num;
	u32 amsdu_cut_max_num;
	u32 amsdu_cut_unfinished;
	u32 amsdu_cut_proc_pending;
#ifdef CONFIG_RTW_DEBUG_HW_RX_AMSDU_CUT
	u32 amsdu_cut_chk_drop;
	u32 amsdu_cut_acc_drop;
...
#define	RXQ_STUCK_CHECK			BIT(0)
#define	ACHQ_STUCK_CHECK		BIT(1)
#define	MGNTQ_STUCK_CHECK		BIT(2)
#define	HIQ_STUCK_CHECK			BIT(3)
#define	H2CQ_STUCK_CHECK		BIT(4)
#define	RX_CCK_OFDM_CNT_CHECK	BIT(5)
#define	RX_DATA_CNT_CHECK		BIT(6)
#define	TX_CCK_OFDM_CNT_CHECK	BIT(7)

struct trx_check {
	u16 rxq_stuck_check:1;
	u16 achq_stuck_check:1;
	u16 mgntq_stuck_check:1;
	u16 hiq_stuck_check:1;
	u16 h2cq_stuck_check:1;
	u16 rx_cck_ofdm_cnt_check:1;
	u16 rx_data_cnt_check:1;
	u16 tx_cck_ofdm_cnt_check:1;
	u16 rsvd:8;
};
#endif
...
struct rtw_phl_com_t {
	...
	#endif
	struct rtw_gpio_info_t gpio_info;
	struct rtw_iot_t id;
	...
	#ifdef CONFIG_WFA_OFDMA_Logo_Test
	struct ru_grp_table	rugrptable;
	#endif

	#ifdef CONFIG_WFA_OFDMA_Logo_Test_Statistic
	u32 update_txbd_ok;
	u32 update_txbd_fail;

	u32 trigger_txstart_ok;
	u32 trigger_txstart_fail;
	#endif
...
#endif
#ifdef CONFIG_RTW_DEBUG_HW_RX_AMSDU_CUT
	u32 rx_amsdu_cut_dump;
	u32 rx_hdr_conv_dump;
#endif /* CONFIG_RTW_DEBUG_HW_RX_AMSDU_CUT */
#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
	u8 disable_hw_trx_watchdog;
	struct trx_check check_flag;
...
enum rtw_phl_hangup_state {
	PHL_HANGUP_STATE_IDLE = 0,
	PHL_HANGUP_STATE_HW_OFF = 1,
	PHL_HANGUP_STATE_HW_ON = 2,
};
...
#ifdef CONFIG_WFA_OFDMA_Logo_Test
enum rtw_phl_ru_tbl_class {
	RTW_PHL_RU_TBL_DLRU,
	RTW_PHL_RU_TBL_DLRU_FIX,
	RTW_PHL_RU_TBL_ULRU,
	RTW_PHL_RU_TBL_ULRU_FIX,
	RTW_PHL_RU_DLMACID_CFG,
	RTW_PHL_RU_ULMACID_CFG,
	RTW_PHL_RU_SW_GRP_SET,
	RTW_PHL_RU_TBL_RU_STA,
	RTW_PHL_CH_BW_UPD,
	RTW_PHL_PWRTABL_NOTIFY
};

//#define RTW_PHL_MAX_RU_STA_NUM 4
#define RTW_PHL_MAX_RU_STA_NUM 8

enum rtw_phl_ru_tbl_type {
	RTW_PHL_RU_TBL_SW,
	RTW_PHL_RU_TBL_HW,
	RTW_PHL_RU_TBL_MAX
};
struct rtw_phl_ru_tbl_hdr {
	u8 is_upd_to_fw;
	u8 idx;
	enum rtw_phl_ru_tbl_type type;
};

struct rtw_phl_ru_rate_ent {
	u8 mcs:4;
	u8 ss:3;
	u8 dcm:1;
};
struct rtw_phl_tf_ba_tbl {
	enum rtw_data_rate tf_rate;
	struct rtw_phl_ru_rate_ent rate;
...
struct rtw_phl_dlru_tbl {
	struct rtw_phl_ru_tbl_hdr tbl_hdr;
	enum channel_width ppdu_bw;
	u16 tx_pwr;
	u8 pwr_boost_fac;
	bool fix_mode_flag;
	u8 txpwr_ofld_en;
	u8 pwrlim_dis;
	struct rtw_phl_tf_ba_tbl tf;
};

struct rtw_phl_dlru_fix_sta_ent {
	u16 mac_id;
	enum rtw_he_ru_idx ru_pos[7];
	bool fix_pwr_fac;
	bool fix_txbf;
	bool fix_coding;
	bool fix_rate;
...
struct rtw_phl_dlru_fix_tbl {
	...
	u8 stbc:1;
	u8 doppler:1;
	struct rtw_phl_dlru_fix_sta_ent sta[RTW_PHL_MAX_RU_STA_NUM];
};
...
struct rtw_phl_ulru_fix_tbl {
	...
	enum rtw_gi_ltf  gi_ltf;
	bool fixru_flag;
	bool fix_tb_t_pe_nom;
	u8 tb_t_pe_nom:2;
	u8 stbc:1;
	u8 ma_type:1;
	u8 doppler:1;
	struct rtw_phl_ul_fix_sta_ent sta[RTW_PHL_MAX_RU_STA_NUM];
};
...
struct rtw_phl_ru_sta_info {
	...
	u8 csi_info_bitmap;
	u32 dl_swgrp_bitmap;
	u16 dlru_ratetbl_ridx;
	bool dl_fwcqi_flag;
	bool dlsu_info_en;
	enum rtw_gi_ltf dlsu_gi_ltf;
	enum channel_width dlsu_bw;
	enum rtw_data_rate dlsu_rate;
	u8 dlsu_dcm:1;
	u8 dlsu_pwr:6;
	...
	/* ul su */
	u32 ul_swgrp_bitmap;
	u16 ulru_ratetbl_ridx;
	bool ul_fwcqi_flag;
	bool ulsu_info_en;
	enum rtw_gi_ltf ulsu_gi_ltf;
	enum channel_width ulsu_bw;
	struct rtw_phl_ru_rate_ent ulsu_rate;
	u8 ulsu_rssi;
...
struct rtw_phl_mac_ax_fixmode_para {
	struct rtw_phl_mac_ax_tbl_hdr tbl_hdr;
	...
	u32 force_to_one:1;
	u32 set_ulmode: 1;		// Mark.CS_update
	u32 ulmode: 2;
	u32 rsvd:27;
};

struct rtw_phl_mac_ss_dl_grp_upd {
	u8 grp_valid;  //0: non valid 1: valid
	u8 grp_id; //grp 0~16
	u8 is_hwgrp;
	u8 mru;
...
struct rtw_phl_mac_ss_ul_grp_upd {		// Mark.CS_update
	u8 macid_u0;
	u8 macid_u1;
	u16 grp_bitmap;
};

struct rtw_phl_dlmacid_cfg {
    u32 macid;
    u8 dl_su_rate_cfg;
    u16 dl_su_rate;
    u8 dl_su_bw;
    ...
    u8 gi_ltf_4x8_support;
    u8 gi_ltf_1x8_support;
    u8 dl_su_info_en;

    u8 dl_su_gi_ltf;
    u8 dl_su_doppler_ctrl;
    u8 dl_su_coding;
    u8 dl_su_txbf;
    u8 dl_su_stbc;
    u8 dl_su_dcm;
...
struct rtw_phl_ulmacid_set
{
	struct rtw_phl_ulmacid_cfg phl_ul_macid_cfg[8];
};

struct rtw_phl_sw_grp_bitmap {
	u8 macid;
...
struct rtw_phl_sw_grp_set {
	struct rtw_phl_sw_grp_bitmap phl_swgrp_bitmap[8];
};

enum snd_option {
	SND_ENABLE_TXBF_MU_2SS = BIT0,
	SND_DISABLE_TP_LIMIT = BIT1,
	SND_OFFLOAD = BIT2,
};

struct rtw_phl_ch_bw_notif{
	u8 band_idx;
	u8 band_type;
	u8 pri_ch;
	u8 central_ch;
