...
#if defined(CONFIG_PCI_HCI) && defined(CONFIG_RTW_RX_BUF_SHARING)
struct phl_rx_buf_info {
	void *rx_data;
	void *rx_os_priv;
	u32 rx_buf_len;
	u32 rx_phy_addr_l;
...
struct phl_info_t {
	...
	struct rtw_phl_handler phl_fw_wdt_handler;
	...
	struct rtw_phl_tring_list *phl_tring_list[PHL_MACID_MAX_NUM];/* _PHL_REFINE_VW_ */
	void *ring_sts_pool;
	void *rx_pkt_pool;
	...
	u32 vw_cnt_rev;
	u32 vw_cnt_err;
	enum rtw_phl_hangup_state hangup_reset;
	...
	struct rtw_phl_rx_pkt tmp_phl_rx;
