...
static u8 _send_ht_ndpa_packet(_adapter *adapter, u8 *ra, enum channel_width bw)
{
	...
	if (update_mgntframe_attrib(adapter, attrib) != _SUCCESS) {
		rtw_free_xmitframe(&adapter->xmitpriv, pmgntframe);
		return _FALSE;
	}
...
void rtw_bf_chk_per_sta(_adapter *adapter, struct sta_info *psta)
{
	u8 gid = 0, pid = 0;

	if((adapter->registrypriv.wifi_mib.txbf_mu) && !(psta->phl_sta->wmode & WLAN_MD_11AX)) {
		if(!rtw_phl_check_bf_entry(adapter->dvobj->phl, psta->phl_sta)) {
			rtw_phl_get_bf_gid_pid(adapter->dvobj->phl, psta->phl_sta, &gid, &pid);

				//DBGP("sending macid = %d, gid = %d, pid = %d\n", psta->phl_sta->macid, gid, pid);
			if(gid != 0)
			rtw_bf_send_vht_gid_mgnt_packet(adapter, psta->phl_sta->mac_addr, gid, pid);
		}
	}

	if(adapter->registrypriv.wifi_mib.txbf_mu_1ss) {
		psta->phl_sta->asoc_cap.nss_rx = 1;
		if(adapter->registrypriv.wifi_mib.su_trig_enable ==0)
			psta->phl_sta->asoc_cap.max_nc = 0;
	}
...
u32 rtw_bf_get_report_packet(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	u8 *pMIMOCtrlField, *pCSIMatrix;
	u8 Nc = 0, Nr = 0, CH_W = 0, Ng = 0, CodeBook = 0;
	u8 fb_type = 0, remain_fb_seg = 0, first_fb_seg = 0, ru_start_idx = 0;
	u8 ru_end_idx = 0, token_num = 0, disa_subc_bm_pre = 0, disa_subc_bm = 0;  
	u16 CSIMatrixLen = 0;

#ifdef RTK_WKARD_CSI_LEGACY_RATE
	...
	{		
		...
		u16 data_rate = pattrib->data_rate;
		if(adapter->registrypriv.wifi_mib.txbf_csi_rate)
			data_rate = HRATE_OFDM6;
		rtw_phl_set_bf_csi_rate(adapter->dvobj->phl, psta->phl_sta, data_rate);
	}
#endif
	...
	else if((category == RTW_WLAN_CATEGORY_HE)
		   && (action == RTW_WLAN_ACTION_HE_COMPRESSED_BEAMFORMING_CQI)) {
		pMIMOCtrlField = pframe + 26;
		Nc = ((*pMIMOCtrlField) & 0x7) + 1;
		Nr = (((*pMIMOCtrlField) & 0x38) >> 3 ) + 1;
		CH_W = ((*pMIMOCtrlField) & 0xC0) >> 6;
		Ng = ((*(pMIMOCtrlField+1)) & 0x1);
		CodeBook = ((*(pMIMOCtrlField+1)) & 0x2) >> 1;
		fb_type = ((*(pMIMOCtrlField+1)) & 0xC) >> 2;
		remain_fb_seg = ((*(pMIMOCtrlField+1)) & 0x70) >> 4;
		first_fb_seg = ((*(pMIMOCtrlField+1)) & 0x80) >> 7;
		ru_start_idx = ((*(pMIMOCtrlField+2)) & 0x7F);
		ru_end_idx = (((*(pMIMOCtrlField+2)) & 0x80) >> 7) | (((*(pMIMOCtrlField+3)) & 0x3F) << 1);
		token_num = (((*(pMIMOCtrlField+3)) & 0xC0) >> 6) | (((*(pMIMOCtrlField+4)) & 0x0F) << 2);
		disa_subc_bm_pre = (((*(pMIMOCtrlField+4)) & 0x10) >> 4);
		if(disa_subc_bm_pre)
		{
			disa_subc_bm = ((*(pMIMOCtrlField+5)) & 0xFF);
			pCSIMatrix = pMIMOCtrlField + 7 + Nr;
			CSIMatrixLen = frame_len  - 26 - 7 - Nr;
		}
		else
		{
			pCSIMatrix = pMIMOCtrlField + 5 + Nr;
			CSIMatrixLen = frame_len  - 26 - 5 - Nr;
		}
	...
	{
		RTW_INFO("%s: fb_type=%d, remain_fb_seg=%d, first_fb_seg=%d, ru_start_idx=%d, ru_end_idx=%d\n",
		 __FUNCTION__, fb_type, remain_fb_seg, first_fb_seg, ru_start_idx, ru_end_idx);
		RTW_INFO("%s: token_num=%d, disa_subc_bm_pre=%d, disa_subc_bm=%d\n",
		 __FUNCTION__, token_num, disa_subc_bm_pre, disa_subc_bm);
	}

	return ret;
}

u8 rtw_bf_send_vht_gid_mgnt_packet(_adapter *adapter, u8 *ra, u8 gid, u8 pid)
{
	/* General */
	struct xmit_priv *xmitpriv;	
	struct mlme_priv *mlmepriv;
	...
	//nathan, 0412
	//SetSeqNum(pframe, 0);
	SetSeqNum(pframe, pmlmeext->mgnt_seq);
	pmlmeext->mgnt_seq++;	
...
#endif
u8 rtw_bf_send_vht_gid_mgnt_packet_plus(_adapter *adapter, u8 *ra, u8 idx)
{
	/* General */
	struct xmit_priv *xmitpriv;	
	struct mlme_priv *mlmepriv;
	...
	struct rtw_ieee80211_hdr *wlanhdr;
	u8 *pframe, *ptr;
	u8 gid[8], position[16];	

	_rtw_memset((u8 *)gid, 0, 8);
	_rtw_memset((u8 *)position, 0, 16);	
	...
	gid[0] = 0x2;
	gid[2] = 0x1;
	_rtw_memcpy(ptr, (u8 *)gid, 8);
	/* Set User Position Array */
	ptr = pframe + 34;
	
	//STA1: position(0) at GID1, position(1) at GID16
	//STA2: position(1) at GID1, position(0) at GID16
	if (idx == 0)
		position[4] = 0x1;
	else
		position[0] = 0x4;	
	_rtw_memcpy(ptr, (u8 *)position, 16);
