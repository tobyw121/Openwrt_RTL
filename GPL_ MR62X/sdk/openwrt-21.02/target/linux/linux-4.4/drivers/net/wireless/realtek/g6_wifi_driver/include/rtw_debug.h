...
#define RTW_PRINT_LIMITED(x, ...) do {} while (0)
#define RTW_ERR_LIMITED(x, ...) do {} while (0)
#define RTW_WARN_LIMITED(x,...) do {} while (0)
#define RTW_INFO_LIMITED(x,...) do {} while (0)
#define RTW_DBG_LIMITED(x,...) do {} while (0)
...
#undef _dbgdump
#undef _dbgdump_limited
#undef _seqdump

#if defined(PLATFORM_LINUX)
	#define _dbgdump printk
	#define _dbgdump_limited printk_ratelimited
	#define _vdbgdump vprintk
	#if (LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 24))
	#define KERN_CONT
	#endif
	#define _seqdump seq_printf
	#define _sprintf(buf, fmt, args...) sprintf(buf, fmt, ##args)
	...
	#define _sprintf(buf, fmt, args...) do {} while (0)
#elif defined(PLATFORM_ECOS)
int wfo_seqdump(const char *fmt, ...);
	#define _dbgdump diag_printf
	#define _vdbgdump diag_vprintf
	#define _seqdump(sel, fmt, arg...) wfo_seqdump(fmt, ##arg) //_dbgdump(fmt, ##arg)
	#define _sprintf(buf, fmt, args...) sprintf(buf, fmt, ##args)
#endif
...
/* with driver-defined prefix */
#undef RTW_PRINT_LIMITED
#define RTW_PRINT_LIMITED(fmt, arg...)     \
	do {\
		if (_DRV_ALWAYS_ <= rtw_drv_log_level) {\
			_dbgdump_limited(DBG_PREFIX fmt, DBG_PREFIX_ARG, ##arg);\
		} \
	} while (0)

#undef RTW_ERR_LIMITED
#define RTW_ERR_LIMITED(fmt, arg...)     \
	do {\
		if (_DRV_ERR_ <= rtw_drv_log_level) {\
			_dbgdump_limited(DBG_PREFIX "ERROR " fmt, \
				 DBG_PREFIX_ARG, ##arg);\
		} \
	} while (0)


#undef RTW_WARN_LIMITED
#define RTW_WARN_LIMITED(fmt, arg...)     \
	do {\
		if (_DRV_WARNING_ <= rtw_drv_log_level) {\
			_dbgdump_limited(DBG_PREFIX "WARN " fmt, \
				 DBG_PREFIX_ARG, ##arg);\
		} \
	} while (0)

#undef RTW_INFO_LIMITED
#define RTW_INFO_LIMITED(fmt, arg...)     \
	do {\
		if (_DRV_INFO_ <= rtw_drv_log_level) {\
			_dbgdump_limited(DBG_PREFIX fmt, DBG_PREFIX_ARG, ##arg);\
		} \
...
#undef RTW_DBG_LIMITED
#define RTW_DBG_LIMITED(fmt, arg...)     \
	do {\
		if (_DRV_DEBUG_ <= rtw_drv_log_level) {\
			_dbgdump_limited(DBG_PREFIX fmt, DBG_PREFIX_ARG, ##arg);\
		} \
...
#endif /* defined(_dbgdump_limited) */
...
#define GDEBUG(fmt, args...) printk("[%s %d]"fmt,__FUNCTION__,__LINE__,## args)

#ifdef CONFIG_DBG_COUNTER
	#define DBG_COUNTER(counter) counter++
	#define DBG_COUNTER_NUM(counter, inc_num) counter += inc_num
	#define DBG_REVERSE_COUNTER_NUM(counter, inc_num) counter -= inc_num
#else
	#define DBG_COUNTER(counter)
	#define DBG_COUNTER_NUM(counter, inc_num)
	#define DBG_REVERSE_COUNTER_NUM(counter, inc_num)
#endif
...
#define RTW_PKT_MIRROR_DUMP(adapter, pkt, tx) rtw_pkt_mirror_dump(adapter, pkt, tx)
#else
#define RTW_PKT_MIRROR_DUMP(adapter, pkt, tx)
#endif
...
int proc_get_curr_bw(struct seq_file *m, void *v);
...
enum rtw_sta_dump_type {
	RTW_STA_DUMP_RSSI	= 0x0001,
	RTW_STA_DUMP_RETRY	= 0x0002,
	RTW_STA_DUMP_RATE	= 0x0004,
	RTW_STA_DUMP_FA		= 0x0008,
	RTW_STA_DUMP_CCA	= 0x0010,
	RTW_STA_DUMP_TP		= 0x0020,
	RTW_STA_DUMP_RSSI_PATH	= 0x0040,
	RTW_STA_DUMP_MAX	= 0xFFFF,
};

enum rtw_sta_ru_dump_type {
	RTW_STA_DUMP_RU_RSSI				= 0x0001,
	RTW_STA_DUMP_RU_TP					= 0x0002,
	RTW_STA_DUMP_RU_PHY_RATE			= 0x0004,
	RTW_STA_DUMP_RU_TX_RU_RATIO			= 0x0008,
	RTW_STA_DUMP_RU_TX_RU_FAIL_RATIO	= 0x0010,
	RTW_STA_DUMP_RU_TX_AGG_CNT			= 0x0020,
	RTW_STA_DUMP_RU_RX_AGG_CNT			= 0x0040,
	RTW_STA_DUMP_RU_RX_TF_CNT			= 0x0080,
	RTW_STA_DUMP_RU_RX_TF_BSR_LEN		= 0x0100,
	RTW_STA_DUMP_RU_RX_CCA_RATIO		= 0x0200,
	RTW_STA_DUMP_RU_RX_FAIL_RATIO		= 0x0400,
	RTW_STA_DUMP_RU_RX_CRC32_RATIO		= 0x0800,
	RTW_STA_DUMP_RU_RX_UPH_PW			= 0x1000,
	RTW_STA_DUMP_RU_RX_UPH_MIN_Flag		= 0x2000,
	RTW_STA_DUMP_RU_RX_EVM				= 0x4000,
	RTW_STA_DUMP_RU_MAX    = 0xFFFF,
};
...
extern int get_current_rate(struct sta_info *psta, u8 is_direction_tx, int *rate, unsigned char *rate_str);
