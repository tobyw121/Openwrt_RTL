...
rtw_hal_ser_get_error_status(void *hal, u32 *err)
{
	...
	if ((*err == MAC_AX_ERR_L1_ERR_DMAC) || (*err == MAC_AX_ERR_L0_PROMOTE_TO_L1)) {
		...
		rtw_hal_bb_ser_pause(hal);
		if (hal_info->hal_com->dbg_ser) {
			hal_info->hal_com->dbg_ser = 0;
			rtw_hal_mac_dbg_dump(hal_info->hal_com);
		}
...
enum rtw_hal_status rtw_hal_trigger_bus_err(void *hal)
{
	...
	return rtw_hal_mac_trigger_bus_err(hal_info);
}
