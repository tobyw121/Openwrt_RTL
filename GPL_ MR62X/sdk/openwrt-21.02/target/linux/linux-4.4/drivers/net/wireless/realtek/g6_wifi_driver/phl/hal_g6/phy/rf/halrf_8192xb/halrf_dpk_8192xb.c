...
/*8192XB DPK ver:0x7 20220420*/

void _dpk_bkup_kip_8192xb(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[][DPK_KIP_REG_NUM_8192XB],
	u8 path)
{
	u8 i;

	for (i = 0; i < DPK_KIP_REG_NUM_8192XB; i++) {
		reg_bkup[path][i] = halrf_rreg(rf, reg[i] + (path << 8), MASKDWORD);
		if (DPK_REG_DBG_8192XB)
			RF_DBG(rf, DBG_RF_DPK, "[DPK] Backup 0x%x = %x\n", reg[i]+ (path << 8), reg_bkup[path][i]);
	}
}

void _dpk_bkup_bb_8192xb(
	struct rf_info *rf,
	u32 *reg,
	u32 reg_bkup[DPK_BB_REG_NUM_8192XB])
{
	u8 i;

	for (i = 0; i < DPK_BB_REG_NUM_8192XB; i++) {
...
void _dpk_bkup_rf_8192xb(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][DPK_RF_REG_NUM_8192XB],
	u8 path)
{
	u8 i;

	for (i = 0; i < DPK_RF_REG_NUM_8192XB; i++) {
...
void _dpk_bb_afe_setting_8192xb(
	struct rf_info *rf,
	enum rf_path path)
{
...
void _dpk_bb_afe_restore_8192xb(
	struct rf_info *rf,
	enum rf_path path)
{
...
void _dpk_txpwr_bb_force_8192xb(
	struct rf_info *rf,
	enum rf_path path,
	bool is_force)
{
...
void _dpk_kip_pwr_clk_onoff_8192xb(
	struct rf_info *rf,
	bool turn_on)
{
	if (turn_on) {
...
void _dpk_kip_control_rfc_8192xb(
	struct rf_info *rf,
	enum rf_path path,
	bool ctrl_by_kip)
{
	...
	RF_DBG(rf, DBG_RF_DPK, "[DPK] S%d RFC is controlled by %s\n",
		path, ctrl_by_kip ? "KIP" : "BB");
}

void _dpk_kip_preset_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx)
{
	...
	_dpk_kip_control_rfc_8192xb(rf, path, true);
	halrf_wreg(rf, 0x8104 + (path << 8), BIT(8), kidx); /*ch0/ch1 selection*/

	_dpk_one_shot_8192xb(rf, phy, path, D_KIP_PRESET);
}

void _dpk_kip_restore_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path)
{
	_dpk_one_shot_8192xb(rf, phy, path, D_KIP_RESTORE);
	...
	//_dpk_kip_control_rfc_8192xb(rf, path, false);
	_dpk_txpwr_bb_force_8192xb(rf, path, false);
#if 0
	...
	struct rf_info *rf)
{
	...
	for (addr = 0; addr < 0x200; addr++) {
		...
		RF_DBG(rf, DBG_RF_DPK, "[DPK] RXSRAM[%03d] = 0x%08x\n", addr,
	...
	u32 reg)
{
	...
	RF_DBG(rf, DBG_RF_DPK, "[DPK] query RF 0x%x = 0x%x\n", reg,
		halrf_rrf(rf, path, reg, MASKRF));

	_dpk_kip_control_rfc_8192xb(rf, path, (bool)ori_ctrl);
#endif
}

void _dpk_kset_query_8192xb(
	struct rf_info *rf,
	enum rf_path path)
{
	struct halrf_dpk_info *dpk = &rf->dpk;
	...
	u8 kidx)
{
	...
	//_dpk_rf_reg_query_8192xb(rf, path, 0x00);
...
void _dpk_kip_set_txagc_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 dbm,
...
{	
	...
	_dpk_one_shot_8192xb(rf, phy, path, D_TXAGC);
	_dpk_kset_query_8192xb(rf, path);
	//_dpk_rf_reg_query_8192xb(rf, path, 0x11);
}

bool _dpk_kip_set_rxagc_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx)
{
	...
	_dpk_one_shot_8192xb(rf, phy, path, D_RXAGC);
#if 0
	halrf_wreg(rf, 0x80d4, 0x000F0000, 0x8);
	RF_DBG(rf, DBG_RF_DPK, "[DPK] set RXBB = 0x%x\n",
		halrf_rreg(rf, 0x80fc, 0x0000001F));
#endif
	return _dpk_sync_check_8192xb(rf, path, kidx);
}

void _dpk_lbk_rxiqk_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	bool is_bypass)
{
	...
	_dpk_one_shot_8192xb(rf, phy, path, LBK_RXIQK);
...
void _dpk_get_thermal_8192xb(struct rf_info *rf, u8 kidx, enum rf_path path)
{
	struct halrf_dpk_info *dpk = &rf->dpk;
...
#endif
	RF_DBG(rf, DBG_RF_DPK, "[DPK] S%d RF 0x0/0x83/0x9e/0xdf/0x1a/0x1001a = 0x%x/ 0x%x/ 0x%x/ 0x%x/ 0x%x/ 0x%x\n",
...
u8 _dpk_gainloss_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx)
{
	_dpk_one_shot_8192xb(rf, phy, path, D_GAIN_LOSS);
	_dpk_kip_set_txagc_8192xb(rf, phy, path, 0xff, false); /*set new tx power*/

	/*for k4 hw_bug*/
	halrf_wreg(rf, 0x81f0 + (path << 8), 0x0003FFFF, 0x0); /*[17:0]*/
	halrf_wreg(rf, 0x81f0 + (path << 8), 0xF0000000, 0x0); /*[31:28]*/

	return _dpk_gainloss_read_8192xb(rf);
}

u8 _dpk_pas_read_8192xb(
	struct rf_info *rf,
	u8 is_check)
{
	u8 i;
	...
	u8 loss_only)
{
	...
	do {
		...
		case 0: /*SYNC and Dgain*/
			is_fail = _dpk_kip_set_rxagc_8192xb(rf, phy, path, kidx);

			if (DPK_RXSRAM_DBG_8192XB)
				_dpk_read_rxsram_8192xb(rf);
			...
			dgain = _dpk_dgain_read_8192xb(rf);

			//_dpk_one_shot_8192xb(rf, HW_PHY_0, path, D_SYNC);

			//dgain = _dpk_dgain_read_8192xb(rf);

			if (agc_cnt == 0)
					_dpk_lbk_rxiqk_8192xb(rf, phy, path, false);
		...
		case 1: /*Gain adjustment*/
			...
			offset = _dpk_dgain_mapping_8192xb(rf, dgain);
			...
			if (offset != 0 || agc_cnt == 0) {
				if (rf->hal_com->band[phy].cur_chandef.bw < 2)
					_dpk_lbk_rxiqk_8192xb(rf, phy, path, true);
				else
					_dpk_lbk_rxiqk_8192xb(rf, phy, path, false);
			}
#endif
		...
		case 2: /*GAIN_LOSS and idx*/
			tmp_gl_idx = _dpk_gainloss_8192xb(rf, phy, path, kidx);
			//_dpk_pas_read_8192xb(rf, false);

			if ((_dpk_pas_read_8192xb(rf, true) == 2) && (tmp_gl_idx > 0))
				i = 4;
			else if ((tmp_gl_idx == 0 && _dpk_pas_read_8192xb(rf, true) == 1) || tmp_gl_idx >= 4)
				i = 3; /*GL > criterion*/
		...
		case 3: /*GL > criterion*/
			if (tmp_dbm <= 7) {
				...
				tmp_dbm = tmp_dbm - 3;
				_dpk_kip_set_txagc_8192xb(rf, phy, path, tmp_dbm, true);
			}
			i = 0;
			agc_cnt++;
			break;
...
void _dpk_set_mdpd_para_8192xb(
	struct rf_info *rf,
	enum rf_path path,
	u8 kidx,
	u8 order)
...
void _dpk_read_timingsync0_8192xb(
	struct rf_info *rf,
	enum rf_path path)
{
	struct halrf_dpk_info *dpk = &rf->dpk;
...
void _dpk_get_coef_for_avg_8192xb(
	struct rf_info *rf,
	enum rf_path path,
	u8 kidx,
	bool is_first)
{
	...
	for (i = 0; i < avg_num; i++) {
		...
		if (DPK_COEF_DBG_8192XB)
			RF_DBG(rf, DBG_RF_DPK, "[DPK] get 0x%x = 0x%08x\n", 0xA5E0 + path * 0x400 + addr[i] * 4, coef);
	...
	u8 kidx)
{
	...
	for (i = 0; i < avg_num; i++) {
		...
		if (DPK_COEF_DBG_8192XB) {
			RF_DBG(rf, DBG_RF_DPK, "[DPK] coef_i[0][%02d] / coef_i[1][%02d] / coef_i_avg[%02d] = 0x%04x / 0x%04x / 0x%04x\n",
				i, i, i, dpk->dpk_coef_i[0][i], dpk->dpk_coef_i[1][i], avg_i);
			RF_DBG(rf, DBG_RF_DPK, "[DPK] coef_q[0][%02d] / coef_q[1][%02d] / coef_q_avg[%02d] = 0x%04x / 0x%04x / 0x%04x\n",
	...
	u8 kidx)
{
	...
	for (k = 0; k < chk_num; k++) {
		...
		if (DPK_COEF_DBG_8192XB)
			RF_DBG(rf, DBG_RF_DPK, "[DPK][CHK] 0x%x = 0x%08x \n", 0xA5E0 + path * 0x400 + addr[k] * 4, coef);
	...
	if (fail)
		RF_DBG(rf, DBG_RF_DPK, "[DPK] coef check fail (0x%x/0x%x)\n", coef_i, coef_q);

	return fail;
}

u8 _dpk_ldl_phase_check_8192xb(
	struct rf_info *rf,
	enum rf_path path,
	u8 kidx)
{
...
//		val_i = 0x1000 - val_i;
	...
	if (fail)
		RF_DBG(rf, DBG_RF_DPK, "[DPK] LDL phase check fail (0x%x/0x%x)\n", val_i, val_q);

	return fail;
}

bool _dpk_rek_check_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx,
	...
	u8 rek_cnt)
{
	...
	//u8 corr_idx;
	bool is_fail = false;
	u32 corr_val1, corr_val2;

	corr_val1 = dpk->corr_idx[path][kidx];
	corr_val2 = (dpk->dpk_sync[path] & 0xff) - 16;

	if (HALRF_ABS(corr_val1, corr_val2) > 2) {
		RF_DBG(rf, DBG_RF_DPK, "[DPK] corr_idx diff(%d, %d) >2 happen!!!\n",
			corr_val1, corr_val2);
		dpk->rek_chk[path][is_first][rek_cnt] = 0x1;
		is_fail = true;
		return is_fail;
	}

	if (dpk->c_chk[path]) {
		RF_DBG(rf, DBG_RF_DPK, "[DPK] check C Fail!!\n");
		dpk->rek_chk[path][is_first][rek_cnt] = 0x2;
		is_fail = true;
		return is_fail;
	}
	...
	if (dpk->ov_flag[path]) {
		...
		dpk->rek_chk[path][is_first][rek_cnt] = 0x3;
		is_fail = true;
		return is_fail;
	}

	if (_dpk_coef_check_8192xb(rf, path, kidx)) {
		dpk->rek_chk[path][is_first][rek_cnt] = 0x4;
		is_fail = true;
		return is_fail;
	}

	if (_dpk_ldl_phase_check_8192xb(rf, path, kidx)) {
		dpk->rek_chk[path][is_first][rek_cnt] = 0x5;
		is_fail = true;
		return is_fail;
	}
...
bool _dpk_rek_check_40m_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx,
...
void _dpk_idl_mpa_one_shot_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx,
	u8 k_cnt)
{
	...
	_dpk_tpg_sel_8192xb(rf, path, kidx);
	halrf_wreg(rf, 0x81a0 + (path << 8), BIT(7), 0x1);

	_dpk_one_shot_8192xb(rf, phy, path, 0x38);
	_dpk_one_shot_8192xb(rf, phy, path, 0x34);

	_dpk_c_check_8192xb(rf, path, k_cnt);
	_dpk_read_timingsync0_8192xb(rf, path);

	_dpk_one_shot_8192xb(rf, phy, path, 0x35);

	if (dpk->bp[path][kidx].bw == CHANNEL_WIDTH_40) {
		halrf_wreg(rf, 0x806c, BIT(2) | BIT (1), 0x0);
		halrf_wreg(rf, 0x8094, MASKBYTE3, 0x9f); /*[31:24]*/
		halrf_wreg(rf, 0x80f4, MASKBYTE1, 0x65); /*[15:8]*/
	} else {
		halrf_wreg(rf, 0x806c, BIT(2) | BIT (1), 0x2);
		halrf_wreg(rf, 0x8094, MASKBYTE3, 0x00); /*[31:24]*/
		halrf_wreg(rf, 0x80f4, MASKBYTE1, 0x67); /*[15:8]*/
	}
	_dpk_one_shot_8192xb(rf, phy, path, 0x36);

	halrf_wreg(rf, 0x81cc + (path << 8), 0x0000003F, 0x3f); /*[5:0] ItQt*/
	_dpk_one_shot_8192xb(rf, phy, path, 0x37);
	halrf_wreg(rf, 0x81bc + (path << 8) + (kidx << 2), 0xf0000000, 0x0);
}

bool _dpk_idl_mpa_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx)
{
	...
	if (halrf_rreg(rf, 0x80ec, BIT(8)) == 0x1)
		_dpk_set_mdpd_para_8192xb(rf, path, kidx, 0x2); /*5,0,0*/
	else if (halrf_rreg(rf, 0x80ec, BIT(9)) == 0x1)
		_dpk_set_mdpd_para_8192xb(rf, path, kidx, 0x0); /*5,3,1*/
	else
		_dpk_set_mdpd_para_8192xb(rf, path, kidx, 0x1); /*5,3,0*/

	for (i = 0; i < k_cnt; i++) {
		_dpk_idl_mpa_one_shot_8192xb(rf, phy, path, kidx, i);
		is_rek = _dpk_rek_check_8192xb(rf, phy, path, kidx, i, dpk->rek_cnt[path][i]);

		while (is_rek == true && dpk->rek_cnt[path][i] < 5) {
			_dpk_idl_mpa_one_shot_8192xb(rf, phy, path, kidx, i);
			is_rek = _dpk_rek_check_8192xb(rf, phy, path, kidx, i, dpk->rek_cnt[path][i]);
			dpk->rek_cnt[path][i]++;
		}

		_dpk_get_coef_for_avg_8192xb(rf, path, kidx, i);
		if (i == 1)
			_dpk_avg_coef_8192xb(rf, path, kidx);
	}

	_dpk_coef_read_8192xb(rf, path, kidx, 1);

	if (dpk->rek_cnt[path][0] == 5 && dpk->rek_cnt[path][1] == 5)
		return true; /*fail*/
	else
		return false;
#if 0
	is_rek = _dpk_rek_check_8192xb(rf, phy, path, kidx, dpk->rek_cnt[path]);

	while (is_rek == true && dpk->rek_cnt[path] < 5) {
		_dpk_idl_mpa_one_shot_8192xb(rf, phy, path, kidx);
		is_rek = _dpk_rek_check_8192xb(rf, phy, path, kidx, dpk->rek_cnt[path]);
		dpk->rek_cnt[path]++;
	}
#endif
	...
	u8 kidx)
{
	...
	_dpk_one_shot_8192xb(rf, phy, path, D_MDPK_IDL);
	_dpk_one_shot_8192xb(rf, phy, path, 0x33);
	_dpk_one_shot_8192xb(rf, phy, path, D_MDPK_LDL);

	is_rek = _dpk_rek_check_40m_8192xb(rf, phy, path, kidx, dpk->rek_cnt[path][0]);
...
void _dpk_gain_normalize_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path,
	u8 kidx,
	bool is_execute)
{
	...
	if (is_execute) {
		...
		_dpk_one_shot_8192xb(rf, phy, path, D_GAIN_NORM);
	} else
		halrf_wreg(rf, reg[kidx][dpk->cur_k_set] + (path << 8), 0x0000007F, 0x5b);
	...
	u8 kidx)
{
	...
	halrf_wreg(rf, 0x81bc + (path << 8) + (kidx << 2),
			BIT(26) | BIT(25) | BIT(24), _dpk_order_convert_8192xb(rf));
	...
	_dpk_gain_normalize_8192xb(rf, phy, path, kidx, false);
	//_dpk_para_query_8192xb(rf, path, kidx);

	halrf_wreg(rf, 0x81b4 + (path << 8) + (kidx << 2), 0x07FC0000, 0x76); /*[26:18] k2*/
}

bool _dpk_reload_check_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	enum rf_path path)
{
	...
	enum rf_path path)
{
	...
	_dpk_rf_setting_8192xb(rf, path, kidx);
	//halrf_set_rx_dck_8192xb(rf, phy, path, false); /*****due to hw issue****/

	_dpk_kip_pwr_clk_onoff_8192xb(rf, true);
	_dpk_kip_preset_8192xb(rf, phy, path, kidx);
	_dpk_txpwr_bb_force_8192xb(rf, path, true);
	_dpk_kip_set_txagc_8192xb(rf, phy, path, init_xdbm, true);
	_dpk_tpg_sel_8192xb(rf, path, kidx);
#if 0
	is_fail = _dpk_kip_set_rxagc_8192xb(rf, phy, path, kidx);
	...
	_dpk_gainloss_8192xb(rf, phy, path, kidx);
	//_dpk_para_query_8192xb(rf, path, kidx);
#else
	is_fail = _dpk_agc_8192xb(rf, phy, path, kidx, init_xdbm, false);

	if (is_fail)
		goto _error;
#endif
	/*_dpk_pas_read_8192xb(rf, false);*/
	//_dpk_get_thermal_8192xb(rf, kidx, path);

	if (dpk->bp[path][kidx].bw == CHANNEL_WIDTH_40)
		is_fail = _dpk_idl_mpa_40m_8192xb(rf, phy, path, kidx);
	else
		is_fail = _dpk_idl_mpa_8192xb(rf, phy, path, kidx);
	_dpk_para_query_8192xb(rf, path, kidx);
	...
	//_dpk_get_thermal_8192xb(rf, kidx, path);
#if 0
	_dpk_coef_read_8192xb(rf, path, kidx, gain);
#endif
	_dpk_on_8192xb(rf, phy, path, kidx);
_error:
	_dpk_kip_control_rfc_8192xb(rf, path, false);
	...
	u8 kpath)
{
	...
	u32 kip_bkup[DPK_RF_PATH_MAX_8192XB][DPK_KIP_REG_NUM_8192XB] = {{0}};
	//u32 bb_bkup[DPK_BB_REG_NUM_8192XB] = {0};
	u32 rf_bkup[DPK_RF_PATH_MAX_8192XB][DPK_RF_REG_NUM_8192XB] = {{0}};

	u32 kip_reg[] = {0x813c, 0x8124, 0x8120,  0xc0c4, 0xc0e8, 0xc0d4, 0xc0d8, 0xc0ec};
	//u32 bb_reg[] = {0x2344, 0x5800, 0x7800};
	u32 rf_reg[DPK_RF_REG_NUM_8192XB] = {0xde, 0xdf, 0x5, 0x10005};

	u8 path;
	bool is_fail = true;
	bool reloaded[DPK_RF_PATH_MAX_8192XB] = {false};

	if ((!phl_is_mp_mode(rf->phl_com)) && DPK_RELOAD_EN_8192XB) {
		for (path = 0; path < DPK_RF_PATH_MAX_8192XB; path++) {
			reloaded[path] = _dpk_reload_check_8192xb(rf, phy, path);
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8192XB; path++) {
		if (kpath & BIT(path)) {
			_dpk_bkup_kip_8192xb(rf, kip_reg, kip_bkup, path);
			_dpk_bkup_rf_8192xb(rf, rf_reg, rf_bkup, path);
			_dpk_information_8192xb(rf, phy, path);
			_dpk_init_8192xb(rf, path);
			if (rf->is_tssi_mode[path])
				_dpk_tssi_pause_8192xb(rf, path, true);
		}
	}
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8192XB; path++) {
		if (kpath & BIT(path)) {
			...
			_dpk_rxagc_onoff_8192xb(rf, path, false);
			halrf_drf_direct_cntrl_8192xb(rf, path, false);
			_dpk_bb_afe_setting_8192xb(rf, path);
			is_fail = _dpk_main_8192xb(rf, phy, path);
			halrf_dpk_onoff_8192xb(rf, path, is_fail);
		}
	}

	for (path = 0; path < DPK_RF_PATH_MAX_8192XB; path++) {
		if (kpath & BIT(path)) {
			_dpk_kip_restore_8192xb(rf, phy, path);
			_dpk_reload_kip_8192xb(rf, kip_reg, kip_bkup, path);
			_dpk_reload_rf_8192xb(rf, rf_reg, rf_bkup, path);
			_dpk_bb_afe_restore_8192xb(rf, path);
			_dpk_rxagc_onoff_8192xb(rf, path, true);
			if (rf->is_tssi_mode[path])
				_dpk_tssi_pause_8192xb(rf, path, false);
		}
	}
	_dpk_kip_pwr_clk_onoff_8192xb(rf, false);
}

u8 _dpk_bypass_check_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy)
{
	...
	if (rf->hal_com->band[phy].cur_chandef.band == BAND_ON_5G) {
		RF_DBG(rf, DBG_RF_DPK, "[DPK] Skip DPK due to 5G!!\n");
		result = 1;
		...
		RF_DBG(rf, DBG_RF_DPK, "[DPK] Skip DPK due to RFE type != 50\n");
		result = 1;
	} else
		result = 0;
	...
	enum phl_phy_idx phy)
{
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8192XB; path++) {
		if (kpath & BIT(path))
			halrf_dpk_onoff_8192xb(rf, path, true);
	}
}

void halrf_dpk_8192xb(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	bool force)
{
	RF_DBG(rf, DBG_RF_DPK, "[DPK] ****** DPK Start (Ver: 0x%x, Cv: %d, RF_para: %d) ******\n",
		DPK_VER_8192XB, rf->hal_com->cv, halrf_get_radio_ver_from_reg(rf));

	RF_DBG(rf, DBG_RF_DPK, "[DPK] Driver mode = %d\n", rf->phl_com->drv_mode);
#if 1
	if (_dpk_bypass_check_8192xb(rf, phy))
		_dpk_force_bypass_8192xb(rf, phy);
	else
		_dpk_cal_select_8192xb(rf, force, phy, halrf_kpath_8192xb(rf, phy));
#else
	_dpk_information_8192xb(rf, 0, RF_PATH_B);
	halrf_drf_direct_cntrl_8192xb(rf, RF_PATH_B, false);
	_dpk_bb_afe_setting_8192xb(rf, RF_PATH_B);
	_dpk_rf_setting_8192xb(rf, RF_PATH_B, 0);

	halrf_rf_direct_cntrl_8192xb(rf, RF_PATH_A, false); /*switch control to direct write*/


	

	halrf_set_rx_dck_8192xb(rf, phy, RF_PATH_A, false);
	//_dpk_kip_pwr_clk_onoff_8192xb(rf, true);
	//_dpk_kip_preset_8192xb(rf, phy, RF_PATH_A, 0);
	//_dpk_txpwr_bb_force_8192xb(rf, RF_PATH_A, true);
	//_dpk_kip_set_txagc_8192xb(rf, phy, RF_PATH_A, 21, true);
	//_dpk_tpg_sel_8192xb(rf, RF_PATH_A, 0);
	//_dpk_kip_set_rxagc_8192xb(rf, phy, RF_PATH_A, 0);


#endif
}

void halrf_dpk_onoff_8192xb(
	struct rf_info *rf,
	enum rf_path path,
	bool off)
{
	...
	struct rf_info *rf)
{
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8192XB; path++) {
		...
		if (rf->rfk_is_processing != true && halrf_rreg(rf, 0x80f0, BIT(29)) == 0x0 && txagc_rf != 0) {
			...
			halrf_wreg(rf, 0x81b4 + (path << 8) + (kidx << 2), 0x07FC0000, ref_pwsf - delta_ther); /*[26:18] k2*/
		}
	}
#endif
}
