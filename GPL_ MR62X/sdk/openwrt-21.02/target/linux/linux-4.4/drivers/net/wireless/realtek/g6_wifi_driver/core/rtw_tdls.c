...
void rtw_tdls_process_vht_operation(_adapter *padapter, struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	if (GET_VHT_OPERATION_ELE_CHL_WIDTH(data) >= 1) {
		...
		if (rtw_hw_is_band_support(adapter_to_dvobj(padapter), operation_bw) && REGSTY_IS_BW_5G_SUPPORT(regsty, operation_bw)
			&& (operation_bw <= pmlmeext->cur_bwmode))
			ptdls_sta->phl_sta->chandef.bw = operation_bw;
		else
			ptdls_sta->phl_sta->chandef.bw = pmlmeext->cur_bwmode;
	} else
		ptdls_sta->phl_sta->chandef.bw = pmlmeext->cur_bwmode;
}
...
void rtw_tdls_process_vht_op_mode_notify(_adapter *padapter, struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	if (rtw_hw_is_band_support(adapter_to_dvobj(padapter), target_bw) && REGSTY_IS_BW_5G_SUPPORT(regsty, target_bw)
...
int issue_tdls_setup_req(_adapter *padapter, struct tdls_txmgmt *ptxmgmt, int wait_ack)
{
	...
	if (ptdls_sta == NULL) {
		ptdls_sta = rtw_alloc_stainfo(pstapriv, ptxmgmt->peer, PHL_CMD_DIRECTLY);
		if (ptdls_sta == NULL) {
...
sint On_TDLS_Setup_Req(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *ptdls_sta)
{
	...
	if (ptdls_sta == NULL) {
		ptdls_sta = rtw_alloc_stainfo(pstapriv, psa, PHL_CMD_DIRECTLY);
		if (ptdls_sta == NULL)
			goto exit;
...
int On_TDLS_Setup_Rsp(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *ptdls_sta)
{
	...
	status_code = 0;
	ptdls_sta->qos_info = 0;
	ptdls_sta->uapsd_bitmap = 0;
		...
		case _VENDOR_SPECIFIC_IE_:
			if (_rtw_memcmp((u8 *)pIE + 2, WMM_INFO_OUI, 6) == _TRUE) {
				...
				if ((pregistrypriv->wmm_enable == _TRUE) || (padapter->mlmepriv.htpriv.ht_option == _TRUE)) {
					ptdls_sta->qos_option = _TRUE;
					ptdls_sta->qos_info = *(pIE + 8);
					if (ptdls_sta->qos_info & 0xf) {
						if (ptdls_sta->qos_info & BIT0)
							ptdls_sta->uapsd_bitmap |= BIT(TXQ_VO);
						if (ptdls_sta->qos_info & BIT1)
							ptdls_sta->uapsd_bitmap |= BIT(TXQ_VI);
						if (ptdls_sta->qos_info & BIT2)
							ptdls_sta->uapsd_bitmap |= BIT(TXQ_BK);
						if (ptdls_sta->qos_info & BIT3)
							ptdls_sta->uapsd_bitmap |= BIT(TXQ_BE);
					}
				}
			}
			break;
...
int On_TDLS_Peer_Traffic_Rsp(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *ptdls_sta)
{
	...
	if (ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE) {
		if (ptdls_sta->sta_xmitpriv.tx_pending_bitmap) {
			struct sta_xmit_priv *pstaxmitpriv = &ptdls_sta->sta_xmitpriv;
			_queue *sta_queue = &padapter->dvobj->ps_trigger_sta_queue;
...
void rtw_build_tdls_peer_traffic_indication_ies(_adapter *padapter, struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt, struct sta_info *ptdls_sta)
{
	...
	/* PTI control */
	/* PU buffer status */
	if (ptdls_sta->sta_xmitpriv.tx_pending_bitmap & BIT(TXQ_BK))
		AC_queue |= BIT(0);
	if (ptdls_sta->sta_xmitpriv.tx_pending_bitmap & BIT(TXQ_BE))
		AC_queue |= BIT(1);
	if (ptdls_sta->sta_xmitpriv.tx_pending_bitmap & BIT(TXQ_VI))
		AC_queue |= BIT(2);
	if (ptdls_sta->sta_xmitpriv.tx_pending_bitmap & BIT(TXQ_VO))
		AC_queue |= BIT(3);
