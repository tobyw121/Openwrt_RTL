...
#define HAL_MAX_HE_BFRP_STA_NUM 8
...
enum rtw_hal_int_set_opt {
	...
	INT_SET_OPT_WOW_RESUME_HNDL_RX,
	INT_SET_OPT_WOW_RESUME_DONE,
...
#define RTW_HAL_PPDU_STS_MAX_USR 4
	...
	u8 physts_rpt_valid;
	u8 avg_rssi;
	u8 path_rssi[RF_PATH_D + 1];
	u8 is_bf;
	u8 ch_idx;
};

#define HAL_RSSI_MAVG_NUM 16
#define STA_UPDATE_MA_RSSI(_RSSI, _VAL) \
	do { \
		u8 oldest_rssi = 0; \
		if(_RSSI.ma_rssi_ele_idx < HAL_RSSI_MAVG_NUM) { \
			oldest_rssi = _RSSI.ma_rssi_ele[_RSSI.ma_rssi_ele_idx]; \
			_RSSI.ma_rssi_ele[_RSSI.ma_rssi_ele_idx] = \
				((_VAL > PHL_MAX_RSSI) ? PHL_MAX_RSSI : _VAL ); \
		} else { \
			_RSSI.ma_rssi_ele_idx = 0; \
			oldest_rssi = _RSSI.ma_rssi_ele[0]; \
			_RSSI.ma_rssi_ele[0] = \
				((_VAL > PHL_MAX_RSSI) ? PHL_MAX_RSSI : _VAL ); \
		} \
		_RSSI.ma_rssi_ele_sum -= oldest_rssi;\
		_RSSI.ma_rssi_ele_sum += \
			((_VAL > PHL_MAX_RSSI) ? PHL_MAX_RSSI : _VAL ); \
		_RSSI.ma_rssi_ele_idx++; \
		if(_RSSI.ma_rssi_ele_cnt < HAL_RSSI_MAVG_NUM) \
...
struct rtw_rssi_info {
	...
	u8 ma_rssi_ele_idx;
	u8 ma_rssi_ele_cnt; /* maximum : HAL_RSSI_MAVG_NUM */
...
enum rtw_hal_cnt_type {
	HAL_CNT_TX_REACH_RTYLMT,
	HAL_CNT_TX_LIFETIME_DROP,
	HAL_CNT_TX_MACID_DROP,
	HAL_CNT_WP_POLLUTED,
	HAL_CNT_MAX
};

#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
struct trx_watchdog_data {
	u32	trx_wdg_recovery_cnt;
	u32	trx_wdg_fail_cnt;
	u32	failure_mask;
	u32	last_wifi_pkt_cnt;
	#ifdef CONFIG_PCI_HCI
	u16	last_rx_host_idx;
	u16	last_tx_ach_hw_idx[8];
	u16	last_tx_mgnt_hw_idx;
	u16	last_tx_hiq_hw_idx;
	u16	last_tx_h2c_hw_idx;
	u32	last_mac_rx_cck_ofdm;
	u32	last_mac_tx_cck_ofdm;
...
#endif
	#ifdef CONFIG_RTW_HW_TRX_WATCHDOG
	struct trx_watchdog_data wdg_data;
	#endif /* CONFIG_RTW_HW_TRX_WATCHDOG */
	u8 wp_polluted;
	u32 mp_fltr_rx_ok_cnt;
	u32 mp_fltr_rx_err_cnt;
	u8 mp_fltr_ta[6];
	u8 mp_fltr_sa[6];
};
...
struct bus_hw_cap_t {
	...
	u8 addr_info_size;
	u8 seq_info_size;
u32 rp_buf_size;	/* Buffer size for release report */
