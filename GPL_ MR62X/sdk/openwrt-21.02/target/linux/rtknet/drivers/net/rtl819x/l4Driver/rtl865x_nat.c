...
EXPORT_SYMBOL_GPL(rtl865x_optimizeExtPort);
...
EXPORT_SYMBOL_GPL(rtl865x_getAsicNaptHashScore);
...
EXPORT_SYMBOL_GPL(rtl865x_preReserveConn);
...
int32 rtl865x_sw_napt_seq_read(struct seq_file *s, void *v)
{
	...
	#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)	
	seq_printf(s, "total napt connection number is %d\n",nat_tbl.connNum);
	seq_printf(s, "total free entry number is %d\n",nat_tbl.freeEntryNum);
	#else
...
EXPORT_SYMBOL(rtl_check_ext_port_napt_entry);
