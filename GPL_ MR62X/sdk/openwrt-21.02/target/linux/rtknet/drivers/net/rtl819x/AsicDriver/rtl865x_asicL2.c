...
#if defined(CONFIG_RTL_8197F) && defined(CONFIG_RTL819X_GPIO) && !defined(CONFIG_OPENWRT_SDK)
...
#ifdef CONFIG_RTL_MDC_D1_MDIO_D7
// MDC: D1, MDIO: D7
#define GPIO_GROUP_MDC	GPIO_PORT_D
#define GPIO_GROUP_MDIO	GPIO_PORT_D
#define GPIO_MDC 		1
#define GPIO_MDIO 		7
#elif defined(CONFIG_RTL_MDC_D5_MDIO_D7)
// MDC: D5, MDIO: D7
#define GPIO_GROUP_MDC	GPIO_PORT_D
#define GPIO_GROUP_MDIO	GPIO_PORT_D
#define GPIO_MDC 		5
...
#else
// MDC: C2, MDIO: C3
#define GPIO_GROUP_MDC	GPIO_PORT_C
#define GPIO_GROUP_MDIO	GPIO_PORT_C
#define GPIO_MDC 		2
#define GPIO_MDIO 		3
#endif
...
int rtl865x_enableQueueFlowControl(void);
#endif

#if defined(CONFIG_RTL_FC_THRES_REFINE_FOR_PERFORMANCE)
int rtl865x_adjustSysFCThreshold(unsigned int sysOff,unsigned int sysOn);
int rtl865x_adjustMaxShareFCThreshold(unsigned int maxShareOff,unsigned int maxShareOn);
int rtl865x_adjustQueueLen(int PortMask,int queueGroupMask,int queueLenFcOff,int queueLenFcOn);
#endif
...
static unsigned char thr_cum_8197F[5] = {6,7,7,6,6};
static unsigned char thr_cum_8197F_VG[5] = {9,9,7,9,8};
...
static unsigned int rtct_phy_val_8197F[13]={	
	((6<<9)|(68<<0)),
	(8<<6),
	(100<<0),
	(197<<0),
...
static unsigned int rtct_phy_val_8197F_VG[13]={	
	((7<<9)|(66<<0)),
	(9<<6),
	(101<<0),
	(198<<0),
...
void phy_setting_for_RTCT(void)
{
	int i, phyid;
	unsigned long flags=0;
	unsigned char *thr_cum;
	unsigned int *rtct_phy_val;
	
	SMP_LOCK_ETH(flags);
	if (IS_8197F_VG()) {
		thr_cum = &thr_cum_8197F_VG[0];
		rtct_phy_val = &rtct_phy_val_8197F_VG[0];
	}
	else {
		thr_cum = &thr_cum_8197F[0];
		rtct_phy_val = &rtct_phy_val_8197F[0];
	}
	...
	for(i=0; i<5; i++) {
		...
		Set_GPHYWB(phyid, 2, 20, 0xffff-((0x1f<<9)|(0x1ff<<0)), rtct_phy_val[0]);
		Set_GPHYWB(phyid, 2, 26, 0xffff-(0x1f<<6), rtct_phy_val[1]);
		Set_GPHYWB(phyid, 2, 25, 0xffff-((0xff<<8)|(0xff<<0)), ((thr_cum[i]<<8)|(88<<0)));
		
		// 		(3) Update PHYId($phyId) RTCT parmeters: (Threshold adjust)
		Set_GPHYWB(phyid, 2, 17, 0xffff-(0x1ff<<0), rtct_phy_val[2]);
		Set_GPHYWB(phyid, 2, 18, 0xffff-(0x1ff<<0), rtct_phy_val[3]);
		Set_GPHYWB(phyid, 2, 19, 0xffff-(0x1ff<<0), rtct_phy_val[4]);
		Set_GPHYWB(phyid, 2, 16, 0xffff-(0x3ff<<0), rtct_phy_val[5]);
		Set_GPHYWB(phyid, 2, 21, 0xffff-(0x7ff<<0), rtct_phy_val[6]);
		Set_GPHYWB(phyid, 2, 22, 0xffff-(0x7ff<<0), rtct_phy_val[7]);
		Set_GPHYWB(phyid, 2, 23, 0xffff-(0x3ff<<0), rtct_phy_val[8]);
		Set_GPHYWB(phyid, 2, 24, 0xffff-(0x1ff<<0), rtct_phy_val[9]);
		Set_GPHYWB(phyid, 2, 26, 0xffff-(0x1f<11),  rtct_phy_val[10]);
		Set_GPHYWB(phyid, 2, 19, 0xffff-(0xf<<12),  rtct_phy_val[11]);
		Set_GPHYWB(phyid, 2, 18, 0xffff-(0x1f<<10), rtct_phy_val[12]);
...
int Setting_RTL8197F_PHY(void)
{
	...
	if (IS_8197F_VG()) {
		Set_GPHYWB(8, 1, 18, 0xffff-(0xf<<4), 9<<4);
		Set_GPHYWB(1, 1, 18, 0xffff-(0xf<<4), 7<<4);
		Set_GPHYWB(2, 1, 18, 0xffff-(0xf<<4), 7<<4);
		Set_GPHYWB(3, 1, 18, 0xffff-(0xf<<4), 7<<4);
		Set_GPHYWB(4, 1, 18, 0xffff-(0xf<<4), 9<<4);
	}	
	else
	...
	if (IS_8197F_VG())
		/* amendment #6
		   purpose: to solve the 100M template mask fail issue,  
		   need to finetune CAP Cf parameter for each phyid: page1, reg17 bit[13~11]=0x3
		*/
		Set_GPHYWB(999, 1, 17, 0xffff-(7<<11), 3<<11);	
	else
	...
	/* amendment #7
		bandgap correlation: read efuse 0x7C value and adjust bandgap
	 */
	if (IS_8197F_VG()) {
		/* read efuse 0x7C */
		data = REG32(REG_EFUSE_P7_3);
		data = (data & 0xC0) >> 6;
		if (data == 1)
			Set_GPHYWB(999, 1, 23, 0xffff-0xE080, ((3<<13)|(1<<7)));  
		else if (data == 2)
			Set_GPHYWB(999, 1, 23, 0xffff-0xE080, ((2<<13)|(0<<7)));
	}
...
#if defined(CONFIG_RTK_VLAN_SUPPORT) || defined(CONFIG_RTL_DNS_TRAP) || defined(CONFIG_RTL_VLAN_8021Q) || defined(CONFIG_RTL_83XX_ACL_SUPPORT)
...
void init_8367r(void)
{
	...
	for (i=0; i<3; i++) 
	{
		...
		if (IS_8197F_VG())
			REG32(PIN_MUX_SEL2) &= ~(0xF << 16); // set BIT_REG_IOCFG_P0RXC to MII mode
		...
		smi_init(GPIO_GROUP_MDC, GPIO_MDC, GPIO_MDIO);
		mdelay(1000);
		ret = RTL8367R_init();
		if (ret == 0) 
			break;
...
#if defined(CONFIG_RTK_VLAN_SUPPORT) || defined(CONFIG_RTL_DNS_TRAP) || defined(CONFIG_RTL_83XX_ACL_SUPPORT)
extern int rtk_filter_igrAcl_init(void);
#endif
...
void init_83XX(void)
{
	...
	if ((bond_id == BSP_BOND_97FN) && (IS_8197F()))
		gpio_reset = GPIO_RESET_8197FH_83XX;
	else 
		gpio_reset = GPIO_RESET_8197FS_83XX;	
	...
	for (i=0; i<3; i++) 
	{
		...
		if (bond_id == BSP_BOND_97FN) {
			if (IS_8197F())
				...
				#ifdef CONFIG_RTL_MDC_H0_MDIO_G7
				// MDC: H0, MDIO: G7
				smi_init_83xx(GPIO_PORT_H, GPIO_PORT_G, 0, 7);
				#else
				// MDC: H0, MDIO: C3
				smi_init_83xx(GPIO_PORT_H, GPIO_PORT_C, 0, 3);	
				#endif
		}
		else 
			smi_init_83xx(GPIO_GROUP_MDC, GPIO_GROUP_MDIO, GPIO_MDC, GPIO_MDIO);	
	...
	extern int rtl83xx_set_port_flow_control(unsigned int port,unsigned int enable);
	
	/* link down until echo 1 > /proc/rtl865x/link */
	extern uint32 phy_enable_all;
	if (phy_enable_all)
		rtk_port_phyEnableAll_set(ENABLED);
	else
		rtk_port_phyEnableAll_set(DISABLED);

	/* RTL8367 Flow Control Off */
	for(i = 0; i <= 4; i++)
	{
		rtl83xx_set_port_flow_control(i, DISABLED);
	}

	/* disable Green Ethernet, maybe uesless */
...
int init_p0(void)
{
	...
	if (IS_8197F_VG()) {
		...
		REG32(P0GMIICR) = (REG32(P0GMIICR) & ~(0x000C0017)) | (0x00040015);
	}
#ifndef CONFIG_PARALLEL_NAND_FLASH
	REG32(PIN_MUX_SEL) = 0; // TODO: need to find out
	REG32(PIN_MUX_SEL1) = 0; // TODO: need to find out
...
void init_8197f_p0(void)
{
	...
	if (IS_8197F_VG()) {
		/* set 8-byte Tx IPG */
		REG32(PITCR) = (REG32(PITCR) & ~CF_FIX_IPG_MASK) | (1<<CF_FIX_IPG_OFFSET);
		REG32(EXTPCR0) = (REG32(EXTPCR0) & ~CF_TX_IPG_MASK) | (8<<CF_TX_IPG_OFFSET);
		//REG32(MACCR1) = (REG32(MACCR1) & ~CF_RMD_TAG_MASK) | (1<<CF_RMD_TAG_OFFSET); 		
	}	
	#endif
	...
	if (IS_8197F_VG()) {
		REG32(PAD_CTRL_1) = (REG32(PAD_CTRL_1) & ~(PAD_P0_RGMII_DN_MASK|PAD_P0_RGMII_DP_MASK))
					| (0x6 << PAD_P0_RGMII_DN_OFFSET) | (0x6 << PAD_P0_RGMII_DP_OFFSET)
					| PAD_P0_RGMII_MODE_MASK | PAD_P0_TX_E2_MASK | (0x1<<21);
		REG32(P0GMIICR) = (REG32(P0GMIICR) & ~(0x7)) | (0x6);
	}
	else {
...
void disable_unused_phy_power(void)
{
	...
	#if defined(CONFIG_RTL_8197F)
	if (rtl819x_bond_option() == 3){ /* 8197FS or 8197FS-VG */
		if (IS_8197F_VG())
			port_mask = RTL8197FS_VG_NOT_EXIST_PORTS_MASK;
		else 
			port_mask = RTL8197FS_NOT_EXIST_PORTS_MASK;
 	}
	#endif
...
#if defined (CONFIG_RTL_FC_THRES_REFINE_FOR_PERFORMANCE)
	rtl865x_adjustSysFCThreshold(0x1B2,0x1C4);
	rtl865x_adjustMaxShareFCThreshold(0x180,0x190);
	
	// config port0 and cpu port6's queue lenth for RTL819X+RTL83XX
	#if defined(CONFIG_TPLINK_QOS)
	//tplink need to do downlink qos in port6, make sure there can set 3 queues
	rtl865x_adjustQueueLen(0x40,0x7,0x008F,0x0091);
	rtl865x_adjustQueueLen(0x1,0x7,0x00FC,0x00FF);
	#else
	rtl865x_adjustQueueLen(0x41,0x7,0x00FC,0x00FF);
	#endif
#endif

#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
	rtl865x_enableQueueFlowControl();
#endif
	...
	*/
	...
	#if !(defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT))
	{
		if (rtl819x_bond_option() != 3)	
			...
			/* 8197FS-VG and non-83xx: use port 4 LED for FE port 1 */
			if (IS_8197F_VG())
				REG32(DIRECTLCR) |= (1 << LEDP14_SWAP_OFFSET);
		}
...
/* 	system clock = 50MHz --> token=0x40, tick=0x0b, l2=3*token=0xC0
	...
	#if !defined(CONFIG_RTL_8367R_SUPPORT) && !defined(CONFIG_RTL_8211F_SUPPORT) && !defined(CONFIG_RTL_83XX_SUPPORT)
	/* 8197F-VG does not support EN_ROUTER_MODE */
	if (IS_8197F())
	{
		if (rtl819x_bond_option() == 3)
	...
	if (IS_8197F_VG()) {
		REG32(SYS_CLK_MAG) |= (CM_ACTIVE_LX1_CLK | CM_ACTIVE_LX1_ARB);
		REG32(TXRINGCR) = CF_BLEN_ADJ_EN | (0x2 << CF_TXRING3_TO_FIFO_OFT) | (0x1 << CF_TXRING2_TO_FIFO_OFT) | 
			(0xf << CF_TXDCP_BP_EN_OFT) | CF_TX_RING_ROUND | TX_RING1_EN | TX_RING0_EN;
		
		REG32(PQPLGR) = (REG32(PQPLGR) & ~(QLEN_GAPG0_MASK|QLEN_GAPG1_MASK|QLEN_GAPG2_MASK)) | 
			(0x18 << QLEN_GAPG2_OFFSET) | (0x18 << QLEN_GAPG1_OFFSET) | (0x18 << QLEN_GAPG0_OFFSET);
		REG32(QRR) = 0;
		REG32(IGPRIVCR0) = 0;
		REG32(IGPRIVCR1) = 0;
		REG32(IGPRIVCR2) = 0;
...
{
	unsigned int temp;
	if(sysOn>511 || sysOff > 511)
	{
		return FAILED;
	}
	
	temp = (sysOff<<16)|sysOn;
	WRITE_MEM32(SBFCR1, temp); 

	return SUCCESS;
}

// shared buffer FC threshold
int rtl865x_adjustMaxShareFCThreshold(unsigned int maxShareOff,unsigned int maxShareOn)
{
	unsigned int temp;
	if(maxShareOn>511 || maxShareOff > 511)
	{
		return FAILED;
	}
	
	temp = (maxShareOff<<16)|maxShareOn;
	WRITE_MEM32(SBFCR2, temp); 

	return SUCCESS;
}

// queue length FC threshold
int rtl865x_adjustQueueLen(int PortMask,int queueGroupMask,int queueLenFcOff,int queueLenFcOn )
{	
	int port = 0, group = 0;
	if(PortMask>0x7F || PortMask<0)
		return FAILED;

	if(queueGroupMask>0x7 || queueGroupMask<0)
		return FAILED;
	...
	for(port = 0; port <= 6; port++)
	{
		if((1<<port)&PortMask)
		{
			...
			{
				if((1<<group)&queueGroupMask)
				{
					WRITE_MEM32((QPKTFCRP0G0+12*port+4*group), queueLenFcOff<<16 | queueLenFcOn); 
					printk("[%s:%d] memory: %x,  value: %x\n",__FUNCTION__,__LINE__,(QPKTFCRP0G0+12*port+4*group),queueLenFcOff<<16 | queueLenFcOn);
				}
...
int rtl865x_enableQueueFlowControl(void)
{
	...
	for(i =0; i < RTL8651_OUTPUTQUEUE_SIZE; i++)
	{
		for(j=PHY0;j<=CPU;j++)
		{
			...
			{
				printk("Set Queue Flow Control Para Error.\n");
			}
		}
	}
...
#if 1 //defined(CONFIG_RTL_HW_QOS_SUPPORT)
int32 rtl8651_disableVlanRemark(void)
{
	int regValue;
	WRITE_MEM32(RLRC, (READ_MEM32(RLRC) &~(0x7)));
	
	regValue = READ_MEM32(RMCR1P) & ~(0xFF << 24);
	WRITE_MEM32( RMCR1P, regValue);

	return SUCCESS;
}

int32 rtl8651_disableDscpRemark(void)
{
	int regValue;
	WRITE_MEM32(RLRC, (READ_MEM32(RLRC) &~(0x7 << 3)));
	
	regValue = READ_MEM32(DSCPRM1) & ~(0x1FF << 23);
	WRITE_MEM32( DSCPRM1, regValue);

	return SUCCESS;
}

#if defined(CONFIG_IPV6) && (defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F))
int32 rtl8651_disableV6DscpRemark(void)
{
	int regValue;
	WRITE_MEM32(IPV6CR1, (READ_MEM32(IPV6CR1) & ~(0x7 << 12)));

	regValue = READ_MEM32(V6DSCPRMK1) & ~(0x1FF << 23);
	WRITE_MEM32( V6DSCPRMK1, regValue);
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
/* v6 dscp remark*/
int32 rtl8651_setAsicV6DscpRemark(enum PORTID port, enum PRIORITYVALUE priority, int remark)
{
	int regValue0;
	...
	if(priority < 5){
		regValue0 = READ_MEM32(V6DSCPRMK0) &~(0x3f<<(6*priority));
		regValue0 |= remark << (6*priority);
		WRITE_MEM32( V6DSCPRMK0, regValue0);

		regValue0 = READ_MEM32(V6DSCPRMK1) &~(0x1 << (23 + port));
		regValue0 |= 0x1 << (23 + port);
		WRITE_MEM32( V6DSCPRMK1, regValue0);
	}else{
		regValue0 = READ_MEM32(V6DSCPRMK1) & ~((0x3f<<(6*(priority -5))) |(0x1 << (23 + port)));
		regValue0 |= ((remark<<(6*(priority -5))) |(0x1 << (23 + port)));
		WRITE_MEM32( V6DSCPRMK1, regValue0);
	}
...
int32 rtl8651_getAsicV6DscpRemark(enum PORTID port, enum PRIORITYVALUE priority, int* remark)
{
	int regValue0, regValue1;
	if ((port < PHY0) || (port > CPU) || (priority < PRI0) || (priority > PRI7) || (remark == NULL))
		return FAILED;

	regValue1 = READ_MEM32(V6DSCPRMK1);
	if(regValue1 & (0x1 << (23 + port))){
		if(priority < 5){
			regValue0 = READ_MEM32(V6DSCPRMK0);
			*remark = (regValue0>>(6*priority)) & 0x3f;
		}else{
			*remark = (regValue1>>(6*(priority-5))) & 0x3f;
		}
...
#endif
{
	...
	if ((portpri > 0xF) || (dot1qpri > 0xF) || (dscppri > 0xF) || (aclpri > 0xF) || (natpri > 0xF))
		return FAILED;
...
int32 rtl8651_getAsicCPUPriorityToQIDMappingTable( enum PORTID port, enum PRIORITYVALUE priority, enum QUEUEID *qid )
{
	uint32	reg;
	...
	reg = (uint32)(((uint32*)CPUQIDMCR0) + (port-CPU));
	*qid = (READ_MEM32(reg) & (0x7 << (priority<<2)))>>(priority<<2);
...
int32 rtl8651_setAsicPerQueuePhysicalLengthGapRegister(uint32 gap)
{
	...
	WRITE_MEM32(PQPLGR, (READ_MEM32(PQPLGR) & ~(QLEN_GAPG0_MASK)) | (gap << QLEN_GAPG0_OFFSET));
	WRITE_MEM32(PQPLGR, (READ_MEM32(PQPLGR) & ~(QLEN_GAPG1_MASK)) | (gap << QLEN_GAPG1_OFFSET));
	WRITE_MEM32(PQPLGR, (READ_MEM32(PQPLGR) & ~(QLEN_GAPG2_MASK)) | (gap << QLEN_GAPG2_OFFSET));
	return SUCCESS;
}
...
static int32 rtl865x_setExtendStormCtrlRate(uint32 type, uint32 percentage)
{
	...
	uint32 rate = RTL865XC_MAXALLOWED_BYTECOUNT * percentage / 100 / METER_COUNTER_UNNIT;
	...
	switch(type){
		case UNKNOWN_UNICAST_STORM_CONTROL:
			...
			WRITE_MEM32(EXTSTMCR4, rate|(READ_MEM32(EXTSTMCR4)&OMTR_THR_MASK));
			break;
		case UNKNOWN_MULTICAST_SOTRM_CONTROL:
			counter = 1;
			regData &= (~TUSSCM1);
			regData |= (minSpeed<<TUSSCM1_OFFSET);
			WRITE_MEM32(EXTSTMCR4, (rate<<16)|(READ_MEM32(EXTSTMCR4)&EMTR_THR_MASK));
			break;
		case ARP_STORM_CONTROL:
			counter = 2;
			regData &= (~TUSSCM2);
			regData |= (minSpeed<<TUSSCM2_OFFSET);
			WRITE_MEM32(EXTSTMCR5, rate|(READ_MEM32(EXTSTMCR5)&OMTR_THR_MASK));
			break;
		...
		case IGMP_MLD_STORM_CONTROL:
			...
			WRITE_MEM32(EXTSTMCR5, (rate<<16)|(READ_MEM32(EXTSTMCR5)&EMTR_THR_MASK));
			break;
		default:
			return FAILED;
	}
...
static int32 rtl865x_setPerPortExtendStormControl(uint32 type, uint32 port,uint32 enable)
{
	...
	if(port >= RTL8651_PORT_NUMBER){
		printk("invalid port %d for extend storm control.\n", port);
		return FAILED;
	}
	...
	case UNKNOWN_MULTICAST_SOTRM_CONTROL:
		...
		counter = 1;
		counter = (port & 0x1)? (counter << OPUMSC_OFFSET): (counter << EPUMSC_OFFSET);
		type = (port & 0x1) ? OUMSCS : EUMSCS;
		break;
	...
	case ARP_STORM_CONTROL:
		...
		counter = 2;
		counter = (port & 0x1)? (counter << OPASC_OFFSET): (counter << EPASC_OFFSET);
		type = (port & 0x1) ? OASCS : EASCS;
		break;
	...
	case DHCP_STORM_CONTROL:
		...
		counter = 3;
		counter = (port & 0x1)? (counter << OPDSC_OFFSET): (counter << EPDSC_OFFSET);
		type = (port & 0x1) ? ODSCS : EDSCS;
		break;
	...
	case IGMP_MLD_STORM_CONTROL:
		...
		counter = 3;
		counter = (port & 0x1)? (counter << OPIMSC_OFFSET): (counter << EPIMSC_OFFSET);
		type = (port & 0x1) ? OIMSCS : EIMSCS;
		break;
...
int rtl_setRateLimitTable(int rate_type, int rate, int index)
{
	rtl865x_tblAsicDrv_rateLimitParam_t asic_rl;
	uint32 token;
	
	memset(&asic_rl, 0, sizeof(rtl865x_tblAsicDrv_rateLimitParam_t));	
	asic_rl.t_intervalUnit		= 40;
	asic_rl.t_remainUnit		= 40;
	
	if(rate_type==DROP_BY_PPS)
	{
		//time unit = 0.0001, rate * intervelUnit * 0.0001 = maxToken,  rate: pkts/s, Token: pkts/s
		token = rate*asic_rl.t_intervalUnit/10000+1;
	}
	else if(rate_type==DROP_BY_BPS)
	{	
		//time unit = 0.0001, rate/8 * intervelUnit * 0.0001 = maxToken,  rate: bps, Token: Byte/s
		token = rate*asic_rl.t_intervalUnit/80000+1;
	}
	else
	{
		return FAILED;
	}
	asic_rl.maxToken			= token;
	asic_rl.refill_number		= token;
	asic_rl.token				= token;
	
	return rtl8651_setAsicRateLimitTable(index, &asic_rl);
}
...
#endif
#if defined(CONFIG_RTL_8197F)
int32 rtl_set_port_l2_learning_enable(int port_id, int enable, int liminum)
{
    WRITE_MEM32(L2LEARNLIMIT0+0x4*(port_id/2),((READ_MEM32(L2LEARNLIMIT0+0x4*(port_id/2)) & (~(0x1 << 16*(port_id%2)))) | ((enable & 0x1)<< 16*(port_id%2))));
    if (1 == enable)
        WRITE_MEM32(L2LEARNLIMIT0+0x4*(port_id/2),((READ_MEM32(L2LEARNLIMIT0+0x4*(port_id/2)) & (~(0x7FF << (3+16*(port_id%2))) ))| ((liminum & 0x7FF)<< (3+16*(port_id%2)))));

    return SUCCESS;
}
#endif
#if defined (CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
int32 rtl819x_setAsicv6DscpPriority( uint32 dscp, enum PRIORITYVALUE priority )
{
	/* Invalid input parameter */
	//if ((dscp < 0) || (dscp > 63))
	if(dscp > 63)
	...
	//if ((0 <= dscp) && (dscp <= 9))
	if(dscp <= 9)
		WRITE_MEM32(V6DSCPCR0, (READ_MEM32(V6DSCPCR0) & ~(0x7 << (dscp*3))) | (priority << (dscp*3)));
	else if ((10 <= dscp) && (dscp <= 19))
		WRITE_MEM32(V6DSCPCR1, (READ_MEM32(V6DSCPCR1) & ~(0x7 << ((dscp%10)*3))) | (priority << ((dscp%10)*3)));
	else if ((20 <= dscp) && (dscp <= 29))
		WRITE_MEM32(V6DSCPCR2, (READ_MEM32(V6DSCPCR2) & ~(0x7 << ((dscp%10)*3))) | (priority << ((dscp%10)*3)));
	else if ((30 <= dscp) && (dscp <= 39))
		WRITE_MEM32(V6DSCPCR3, (READ_MEM32(V6DSCPCR3) & ~(0x7 << ((dscp%10)*3))) | (priority << ((dscp%10)*3)));
	else if ((40 <= dscp) && (dscp <= 49))
		WRITE_MEM32(V6DSCPCR4, (READ_MEM32(V6DSCPCR4) & ~(0x7 << ((dscp%10)*3))) | (priority << ((dscp%10)*3)));
	else if ((50 <= dscp) && (dscp <= 59))
		WRITE_MEM32(V6DSCPCR5, (READ_MEM32(V6DSCPCR5) & ~(0x7 << ((dscp%10)*3))) | (priority << ((dscp%10)*3)));
	else if ((60 <= dscp) && (dscp <= 63))
		WRITE_MEM32(V6DSCPCR6, (READ_MEM32(V6DSCPCR6) & ~(0x7 << ((dscp%10)*3))) | (priority << ((dscp%10)*3)));
...
int32 rtl819x_getAsicv6DscpPriority( uint32 dscp, enum PRIORITYVALUE *pPriority )
{
	/* Invalid input parameter */
	//if ((dscp < 0) || (dscp > 63))
	if(dscp > 63)
	...
	if (pPriority != NULL)
	{
		//if ((0 <= dscp) && (dscp <= 9))
		if(dscp <= 9)
			*pPriority = (READ_MEM32(V6DSCPCR0) & (0x7 << (dscp*3))) >> (dscp*3);
		else if ((10 <= dscp) && (dscp <= 19))
			*pPriority = (READ_MEM32(V6DSCPCR1) & (0x7 << ((dscp%10)*3))) >> ((dscp%10)*3);
		else if ((20 <= dscp) && (dscp <= 29))
			*pPriority = (READ_MEM32(V6DSCPCR2) & (0x7 << ((dscp%10)*3))) >> ((dscp%10)*3);
		else if ((30 <= dscp) && (dscp <= 39))
			*pPriority = (READ_MEM32(V6DSCPCR3) & (0x7 << ((dscp%10)*3))) >> ((dscp%10)*3);
		else if ((40 <= dscp) && (dscp <= 49))
			*pPriority = (READ_MEM32(V6DSCPCR4) & (0x7 << ((dscp%10)*3))) >> ((dscp%10)*3);
		else if ((50 <= dscp) && (dscp <= 59))
			*pPriority = (READ_MEM32(V6DSCPCR5) & (0x7 << ((dscp%10)*3))) >> ((dscp%10)*3);
		else if ((60 <= dscp) && (dscp <= 63))
			*pPriority = (READ_MEM32(V6DSCPCR6) & (0x7 << ((dscp%10)*3))) >> ((dscp%10)*3);
	}

	return SUCCESS;
}
int32 rtl819x_setAsicv6DscpPriorityDecision(uint32 v6dscppri)
{
	/* Invalid input parameter */
	//if ((v6dscppri < 0) || (v6dscppri > 0xF))
	if(v6dscppri > 0xF)
		return FAILED;

	WRITE_MEM32(IPV6CR1, (READ_MEM32(IPV6CR1) & ~(v6DSCP_PRI_MASK<<v6DSCP_PRI_OFFSET))|(v6dscppri << v6DSCP_PRI_OFFSET));

	return SUCCESS;
}
int32 rtl819x_getAsicv6DscpPriorityDecision(uint32 *v6dscppri)
{
	/* Invalid input parameter */
	if (v6dscppri == NULL)
		return FAILED;

	*v6dscppri = (READ_MEM32(IPV6CR1)>>v6DSCP_PRI_OFFSET) & v6DSCP_PRI_MASK;
