...
int32 rtl8651_setAsicIpMulticastTable(rtl865x_tblAsicDrv_multiCastParam_t *mCast_t) {
	...
	entry.destInterface = mCast_t->destInterface;
#endif
...
#if defined(CONFIG_RTL_8197F)
	mCast_t->destInterface = entry.destInterface;
#endif
...
#endif
#if defined(CONFIG_RTL_8197F)
int32 rtl_setAsicMCastDifEnable(uint32 enable)
{
	if (enable == TRUE)
	{
		WRITE_MEM32(FFCR, READ_MEM32(FFCR)|EN_MCAST_DIF);
	} else
	{
		WRITE_MEM32(FFCR, READ_MEM32(FFCR) & ~EN_MCAST_DIF);
	}
...
int32 rtl_getAsicMCastDifEnable(uint32 *enable)
{
	if (enable == NULL)
	{
		return FAILED;
	...
	*enable = (READ_MEM32(FFCR) & EN_MCAST_DIF) ? TRUE : FALSE;

	return SUCCESS;
}
int32 rtl_setAsicMCastV6DifEnable(uint32 enable)
{
	if (enable == TRUE)
	{
		WRITE_MEM32(IPV6CR0, READ_MEM32(IPV6CR0)|EN_MCASTV6_DIF);
	} else
	{
		WRITE_MEM32(IPV6CR0, READ_MEM32(IPV6CR0) & ~EN_MCASTV6_DIF);
	}
...
int32 rtl_getAsicMCastV6DifEnable(uint32 * enable)
{
	if (enable == NULL)
	{
		return FAILED;
	...
	*enable = (READ_MEM32(IPV6CR0) & EN_MCASTV6_DIF) ? TRUE : FALSE;
...
int32 rtl8198C_getAsicMulticastv6Aging(uint32 *enable)
{
	if (enable == NULL)
	{
		return FAILED;
	...
	*enable = (READ_MEM32(TEACR) & IPv6McastAgingDisable) ? FALSE : TRUE;
...
int32 rtl8198C_setAsicArp6Aging(uint32 enable)
{
	if (enable == TRUE)
	{
		WRITE_MEM32(TEACR, READ_MEM32(TEACR) & ~IP6ArpAgingDisable);
...
int32 rtl8198C_getAsicArp6Aging(uint32 *enable)
{
	if (enable == NULL)
	{
		return FAILED;
	...
	*enable = (READ_MEM32(TEACR) & IP6ArpAgingDisable) ? FALSE : TRUE;
