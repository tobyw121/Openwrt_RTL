...
#endif
#if !defined(CONFIG_TP_IMAGE) && defined(CONFIG_RTL_VLAN_8021Q)&&defined(CONFIG_RTL_MULTICAST_PORT_MAPPING)
...
#endif
	{
		...
		for (index=0; index< RTL8651_MULTICASTTBL_SIZE; index++)
		{
			for (mCastEntry = TAILQ_FIRST(&mCastTbl.inuseList.mCastTbl[index]); mCastEntry; mCastEntry = nextMCastEntry)
			{
				...
				#if defined CONFIG_RTL_MULTICAST_PORT_MAPPING
				#if defined CONFIG_TP_IMAGE
					/* Non-VLAN: Multicast hardware-accelerated port is not restricted. */
