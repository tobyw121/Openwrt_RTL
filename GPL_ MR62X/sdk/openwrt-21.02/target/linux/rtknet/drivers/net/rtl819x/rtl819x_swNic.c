...
#undef _LOCAL_TX_DESC
#define	_DESC_CACHE_ACCESS_TX	1
#else
#define _LOCAL_TX_DESC	1
#undef	_DESC_CACHE_ACCESS_TX
#endif
...
int is_rxring_used(unsigned int data_addr)
{
	int i = 0, j = 0;
	...
	for (i = 0; i < NEW_NIC_MAX_RX_DESC_RING; i++)
	{
		...
		{
			if (data_addr == New_rxDescRing[i][j].addr)
				return 1;
		}
	}
				 ...
				 uint32	clusterSize)
{
	...
	REG32(DMA_CR1) = (((New_txDescRingCnt[1]-1) * (sizeof(struct tx_desc)))<<16) |  (New_txDescRingCnt[0]-1) * (sizeof(struct tx_desc)); 
	REG32(DMA_CR3) = (((New_txDescRingCnt[3]-1) * (sizeof(struct tx_desc)))<<16) |  (New_txDescRingCnt[2]-1) * (sizeof(struct tx_desc)); 
	REG32(DMA_CR4) = TX_RING0_TAIL_AWARE | TX_RING1_TAIL_AWARE | TX_RING2_TAIL_AWARE | TX_RING3_TAIL_AWARE;
	REG32(TXRINGCR) |= (TX_RING3_EN|TX_RING2_EN|TX_RING1_EN|TX_RING0_EN);
	REG32(PSRP6_RW) |= CMAC_PAUSE_NICTX_FIFO_MASK;
#else
	...
	if (srcPortNum < RTL8651_CPU_PORT)
	{
		...
		{	
			...
			*/
			...
			if(	desc->rx_dp_ext &&
				(5!=extPortMaskToPortNum[(desc->rx_dp_ext)&0x7]))
			{
			...
			if(extPortEnabled)
			{
				...
				else
				{
					...
					*vid = PKTHDR_EXTPORT_MAGIC4;
					//srcPortNum = extPortMaskToPortNum[(desc->rx_extspa)&0x7];
#if 0
...
#if 0
__IRAM_FWD
uint32 rx_desc_full(uint32 ring_idx)
{
	uint32	hw_idx;
	uint32 rx_idx;
	struct rx_desc *desc;
	...
	hw_idx = (REG32(CPURPDCR(ring_idx))	- New_rxDescRing_base[ring_idx]) / sizeof(struct dma_rx_desc);

	desc = (struct rx_desc *)&New_rxDescRing[ring_idx][rx_idx];
	desc = (struct rx_desc *)(((uint32)desc) |	UNCACHE_MASK);

	if ((rx_idx==hw_idx) && (desc->rx_own==0))
		return 1;
	else
		return 0;
}
...
__IRAM_FWD
uint32 rx_desc_not_empty(uint32 ring_idx)
{
	uint32	hw_idx;
	uint32 rx_idx;
	struct rx_desc *desc;
	...
	if (((rx_idx==hw_idx) && (desc->rx_own==0)) || (rx_idx!=hw_idx))
			return 1;
	return 0;
}
...
get_next:
		...
		if (buf) {
			...
			#if 0 //def _DESC_CACHE_ACCESS_RX
			desc = (struct rx_desc *)(((uint32)desc) |	UNCACHE_MASK);
			#endif
			info->vid =	desc->rx_dvlanid;
			info->pid =	desc->rx_spa;
			info->ext_pid = desc->rx_extspa;
...
static inline void fill_txd_misc(struct tx_desc *txd, rtl_nicTx_info *nicTx, struct sk_buff *skb)
{
	...
	txd->tx_dpri = 0x7 & rtk_get_vlan_priority(txd->tx_dvlanid);

	#if defined(CONFIG_NET_SCHED) && defined(CONFIG_RTL_QOS_8021P_SUPPORT)
	if(gQosEnabled){
	#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
	if(gHwNatEnabled==0)
	#endif
	...
	if (rtl_vlan_passthrough_enable){
		...
		{
			txd->tx_vi = 1;
			txd->tx_dpri = ntohs(*(unsigned	short *)(skb->data + (ETH_ALEN<<1) + 2))>>13;
			txd->tx_dvlanid = ntohs(*(unsigned	short *)(skb->data + (ETH_ALEN<<1) + 2)) & 0xfff;
		}
	}
	#endif
	#endif
...
//		stopCP3Ctrl(1, 0);	//added for romperf testing	
	...
	if(printk_ratelimit())
	{	
		struct sk_buff * sb = (struct sk_buff *)skb;
		if (sb && sb->data)
		{
			char *buf = sb->data;
			 int len = sb->len;
			 int i;
			 if(skb_mac_header_was_set(sb))
			{
				buf = (char *)(skb_mac_header(sb));
				printk("TX. len = %#4x, tci=0x%x, tx_vid=%x, tx_port=0x%x\n", len, sb->vlan_tci, nicTx->vid, nicTx->portlist);
			 }
			else
			{
				printk("TX len = %#4x, tci=0x%x, tx_vid=%x, tx_port=0x%x\n", len, sb->vlan_tci, nicTx->vid, nicTx->portlist);
	...
	if(first_len)
	{
		...
		if (skb_gso_size) {
			...
			*/
			...
			/* if tx_desc.mss > tx_desc.ph_len, need to set tx_desc.lso to 0 */
			if (skb_gso_size <= skb->len)
