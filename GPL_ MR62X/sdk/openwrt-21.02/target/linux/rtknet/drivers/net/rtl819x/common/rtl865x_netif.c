...
rtl865x_netif_local_t *_rtl865x_getSWNetifByName(char *name)
{
...
#endif
	{
		...
		//if(netifTbl[i].valid == 1 && strlen(name) >= strlen(netifTbl[i].name) && memcmp(netifTbl[i].name,name, strlen(netifTbl[i].name)) == 0)
...
rtl865x_netif_local_t *_rtl865x_getNotEthWanNetif(int ifType)
{
	int32 i;

	for (i = 0; i < NETIF_NUMBER; i++)
	{
		if (netifTbl[i].valid == 1 && netifTbl[i].is_wan == 1 &&
			(ifType & (1 << netifTbl[i].if_type)))
		{
			return &netifTbl[i];
		}
	}
...
static int32 _rtl865x_attachMasterNetif(char *slave, char *master)
{
	...
	printk("[%s-%u] %p %p\n", __func__, __LINE__, slave_netif, master_netif);
...
static int32 _rtl865x_detachMasterNetif(char *slave)
{
	...
	if(slave_netif == NULL)
		return RTL_EENTRYNOTFOUND;
	printk("[%s-%u] %s\n", __func__, __LINE__, slave_netif->name);
	...
	for(i = 0; i < NETIF_NUMBER; i++)
#endif
			{
				if(netifTbl[i].valid == 1 && netifTbl[i].is_slave == 1 && netifTbl[i].master == entry) {
					...
					printk("[%s-%u] %s\n", __func__, __LINE__, netifTbl[i].name);
				}
			}
		}
#ifdef CONFIG_RTL_LAYERED_DRIVER_ACL
...
rtl865x_netif_local_t *_rtl865x_getNetifByIdx(int idx)
{
	int32 i=idx;
	rtl865x_netif_local_t *netif = NULL;
...
static int8 _rtl865x_sameAclRule(rtl865x_AclRule_t *rule1, rtl865x_AclRule_t *rule2)
{
	...
	case RTL865X_ACL_IPV6_RANGE:
		if ((memcmp(&(rule1->srcIpV6Addr_), &(rule2->srcIpV6Addr_), sizeof(inv6_addr_t)) != 0) ||
			(memcmp(&(rule1->srcIpV6AddrMask_), &(rule2->srcIpV6AddrMask_), sizeof(inv6_addr_t)) != 0) ||
			(memcmp(&(rule1->dstIpV6Addr_), &(rule2->dstIpV6Addr_), sizeof(inv6_addr_t)) != 0) ||
			(memcmp(&(rule1->dstIpV6AddrMask_), &(rule2->dstIpV6AddrMask_), sizeof(inv6_addr_t)) != 0) ||
			(rule1->ipv6HttpFilter_ != rule2->ipv6HttpFilter_) || 
			(rule1->ipv6HttpFilterM_ != rule2->ipv6HttpFilterM_) || 
			(rule1->ipv6IdentSrcDstIp_ != rule2->ipv6IdentSrcDstIp_) || 
			(rule1->ipv6IdentSrcDstIpM_ != rule2->ipv6IdentSrcDstIpM_) || 
			(rule1->ipv6FlowLabel_ != rule2->ipv6FlowLabel_ ) || 
			(rule1->ipv6FlowLabelM_ != rule2->ipv6FlowLabelM_ ) ||
			(rule1->ipv6TrafficClass_ != rule2->ipv6TrafficClass_) || 
			(rule1->ipv6TrafficClassM_ != rule2->ipv6TrafficClassM_) || 
			(rule1->ipv6NextHeader_ != rule2->ipv6NextHeader_) || 
			(rule1->ipv6NextHeaderM_ != rule2->ipv6NextHeaderM_) ||
			(rule1->ipv6Invert_ != rule2->ipv6Invert_) || 
			(rule1->ipv6EntryType_ != rule2->ipv6EntryType_) || 
			(rule1->ipv6Combine_ != rule2->ipv6Combine_) || 
			(rule1->ipv6IPtunnel_ != rule2->ipv6IPtunnel_)
		)
		{
			return FALSE;
		}
#if defined(CONFIG_RTL_8197F)
	case RTL865X_ACL_PM:
		if(rule1->ipv6EntryType_ != rule2->ipv6EntryType_ || rule1->pmSPUB_ != rule2->pmSPUB_ || rule1->pmSPLB_ != rule2->pmSPLB_ || rule1->pmDPUB_ != rule2->pmDPUB_ || rule1->pmDPLB_ != rule2->pmDPLB_ || rule1->pmProtocol_ != rule2->pmProtocol_)
			return FALSE;
		if(rule1->pmOffset7_ != rule2->pmOffset7_ || rule1->pmPattern7_ != rule2->pmPattern7_ || rule1->pmPatternMask7_ != rule2->pmPatternMask7_ ||
		   rule1->pmOffset6_ != rule2->pmOffset6_ || rule1->pmPattern6_ != rule2->pmPattern6_ || rule1->pmPatternMask6_ != rule2->pmPatternMask6_ ||
		   rule1->pmOffset5_ != rule2->pmOffset5_ || rule1->pmPattern5_ != rule2->pmPattern5_ || rule1->pmPatternMask5_ != rule2->pmPatternMask5_ ||
		   rule1->pmOffset4_ != rule2->pmOffset4_ || rule1->pmPattern4_ != rule2->pmPattern4_ || rule1->pmPatternMask4_ != rule2->pmPatternMask4_ ||
		   rule1->pmOffset3_ != rule2->pmOffset3_ || rule1->pmPattern3_ != rule2->pmPattern3_ || rule1->pmPatternMask3_ != rule2->pmPatternMask3_ ||
		   rule1->pmOffset2_ != rule2->pmOffset2_ || rule1->pmPattern2_ != rule2->pmPattern2_ || rule1->pmPatternMask2_ != rule2->pmPatternMask2_ ||
		   rule1->pmOffset1_ != rule2->pmOffset1_ || rule1->pmPattern1_ != rule2->pmPattern1_ || rule1->pmPatternMask1_ != rule2->pmPatternMask1_ ||
		   rule1->pmOffset0_ != rule2->pmOffset0_ || rule1->pmPattern0_ != rule2->pmPattern0_ || rule1->pmPatternMask0_ != rule2->pmPatternMask0_ 
		   )
		{
		   	return FALSE;
		}
...
int32 sw_netif_read(struct seq_file *s, void *v)
{
...
#endif
	{
		...
		if ( intf->valid )
		{
			...
			seq_printf(s,"[%d]netif:%s type %u VID[%d] %x:%x:%x:%x:%x:%x",
				i, intf->name, intf->if_type, intf->vid, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
			seq_printf(s,"	Routing %s	defaultWan %s\n",
				intf->enableRoute==TRUE? "enabled": "disabled",
				intf->is_defaultWan==1?"YES":"NO");
			seq_printf(s,"is slave %d	",intf->is_slave);
			if(intf->is_slave){
				seq_printf(s,"master netif:(%p)%s", intf->master, (intf->master->name));
			}	
			seq_printf(s,"\n");
			#if 0
...
#endif
			seq_printf(s, "      enableRoutev4:%d,    mtuv4:%d\n", intf->enableRoute, intf->mtu);
...
int32 rtl_getLanIfAsicIndex(int32 *index)
{
	int i;
#if defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT)
	for(i = 0; i < NETIF_SW_NUMBER; i++)
...
#define ACL_PROC_DEBUG
#if defined(ACL_PROC_DEBUG)
#define ACL_PROC_PRINTK(format, args...) panic_printk(format, ## args)
#else
#define ACL_PROC_PRINTK(format, args...)
#endif

/*	enable/disable ingress/egress acl
	echo "config 1 in" > /proc/rtl865x/acl
	echo "config 0 out" > /proc/rtl865x/acl	*/
	
int32 config_acl_capacity(char *strptr)
{
	int ret = -1, dir, enable, num;
	char str_op[8] = {0}, str_dir[8] = {0};
	extern int32 rtl8651_configAclStatus(uint8 op, uint8 dir);
	...
	num = sscanf(strptr, "%s %d %s", str_op, &enable, str_dir);
	if(num != 3){
		ACL_PROC_PRINTK("[%s:%d] num: %d\n", __FUNCTION__, __LINE__, num);
		return ret;
	}

	if(strncmp(str_op, "config", 6)){
		ACL_PROC_PRINTK("[%s:%d] str_op: %s\n", __FUNCTION__, __LINE__, str_op);
		return ret;
	}
	
	if(!strncmp(str_dir, "in", 2)){
		dir = RTL865X_ACL_INGRESS;
	}
	else if(!strncmp(str_dir, "out", 3)){
		dir = RTL865X_ACL_EGRESS;
	}
	else{
		ACL_PROC_PRINTK("[%s:%d] str_dir: %s\n", __FUNCTION__, __LINE__, str_dir);
		return ret;
	}

	ret = rtl8651_configAclStatus(enable, dir);
	return ret;
}

/*	echo "reg eth1 -16000 in" > /proc/rtl865x/acl	
	echo "unReg eth1 -16000 out" > /proc/rtl865x/acl	*/
int32 config_acl_chain(char *strptr)
{
	int ret = -1, chain_prio, num, dir;
	char netif_name[32] = {0}, str_op[8] = {0}, str_dir[8] = {0};
	...
	num = sscanf(strptr, "%s %s %d %s", str_op, netif_name, &chain_prio, str_dir);
	if(num != 4){
		ACL_PROC_PRINTK("[%s:%d] num: %d\n", __FUNCTION__, __LINE__, num);
		return ret;
	}
	...
	if(!strncmp(str_op, "reg", 3)){
		ret = rtl865x_regist_aclChain(netif_name, chain_prio, dir);
		ACL_PROC_PRINTK("register: %d\n", ret);
	}
	else if(!strncmp(str_op, "unReg", 5)){
		ret = rtl865x_unRegist_aclChain(netif_name, chain_prio, dir);
		ACL_PROC_PRINTK("unRegister: %d\n", ret);
	}
	else{
		ACL_PROC_PRINTK("[%s:%d] str_op: %s\n", __FUNCTION__, __LINE__, str_op);
		return ret;
...
int32 config_acl_type(char *strptr, rtl865x_AclRule_t *rule)
{
	int ret = -1;

	if(strptr == NULL || rule == NULL){
		ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
		return ret;
	}

	if(!strncmp(strptr, "mac", 3)){
		rule->ruleType_		= RTL865X_ACL_MAC;
	}
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
	else if(!strncmp(strptr, "ipv6", 4)){
		rule->ruleType_		= RTL865X_ACL_IPV6;
	}
#endif
	else if(!strncmp(strptr, "ip", 2)){
		rule->ruleType_		= RTL865X_ACL_IP;
	}
	else if(!strncmp(strptr, "tcp", 3)){
		rule->ruleType_		= RTL865X_ACL_TCP;
	}
	else if(!strncmp(strptr, "udp", 3)){
		rule->ruleType_		= RTL865X_ACL_UDP;
	}
	else if(!strncmp(strptr, "srcfilter", 9)){
		rule->ruleType_		= RTL865X_ACL_SRCFILTER;
	}
	else if(!strncmp(strptr, "dstfilter", 9)){
		rule->ruleType_		= RTL865X_ACL_DSTFILTER;
	}
#if defined(CONFIG_RTL_8197F)
	else if(!strncmp(strptr, "pattern", 7)){
		rule->ruleType_		= RTL865X_ACL_PM;
	}
#endif
	else{
		panic_printk("	Do not support acl type %s\n", strptr);
		return ret;
	}
...
int32 config_acl_action(char *strptr, rtl865x_AclRule_t *rule)
{
	int ret = -1;
	...
	if(!strncmp(strptr, "permit", 6)){
		rule->actionType_		= RTL865X_ACL_PERMIT;
	}
	else if(!strncmp(strptr, "toCPU", 5)){
		rule->actionType_		= RTL865X_ACL_TOCPU;
	}
	else if(!strncmp(strptr, "drop", 4)){
		rule->actionType_		= RTL865X_ACL_DROP;
	}
	else if(!strncmp(strptr, "priority", 8)){
		rule->actionType_		= RTL865X_ACL_PRIORITY;
	}
	else if(!strncmp(strptr, "rateLimitBps", 12)){
		rule->actionType_		= RTL865X_ACL_DROP_RATE_EXCEED_BPS;
	}
	else{
		panic_printk("	Do not support acl action %s\n", strptr);
		return ret;
	}
...
int32 config_acl_layer(char *strptr, rtl865x_AclRule_t *rule)
{
	int ret = -1;
	...
	if(!strncmp(strptr, "all", 3)){
		rule->pktOpApp_		= RTL865X_ACL_ALL_LAYER;
	}
	else if(!strncmp(strptr, "l2_l3", 5)){
		rule->pktOpApp_		= RTL865X_ACL_L2_AND_L3;
	}
	else if(!strncmp(strptr, "l3_l4", 5)){
		rule->pktOpApp_		= RTL865X_ACL_L3_AND_L4;
	}
	else if(!strncmp(strptr, "l2", 2)){
		rule->pktOpApp_		= RTL865X_ACL_ONLY_L2;
	}
	else if(!strncmp(strptr, "l3", 2)){
		rule->pktOpApp_		= RTL865X_ACL_ONLY_L3;
	}
	else if(!strncmp(strptr, "l4", 2)){
		rule->pktOpApp_		= RTL865X_ACL_ONLY_L4;
	}
	else{
		panic_printk("	Do not support acl opLayer %s\n", strptr);
		return ret;
	}
...
/*	Hex
	{smac $mac $mask} {dmac $mac $mask} {ethType $type $mask}
	smac 000600030002 FFFFFFFFFFFF
	dmac 000700030002 FFFFFFFFFFFF ethType 0806 FFFF
*/
int32 config_mac_acl_rule(char *strptr, rtl865x_AclRule_t *rule)
{
	int ret = -1, smacFlag;
	char *cmd_addr;
	char str_tmp1[13] = {0}, str_tmp2[13] = {0};
	char str_mac[6] = {0}, str_mac_mask[6] = {0};
	uint32 intVal;
	...
	while(cmd_addr){
		if(!strncmp(cmd_addr, "smac", 4) || !strncmp(cmd_addr, "dmac", 4)){
			if(!strncmp(cmd_addr, "smac", 4)){
				smacFlag = 1;
			}
			else{
				smacFlag = 0;
			...
			cmd_addr = strsep(&strptr," ");
			if (cmd_addr==NULL || strlen(cmd_addr) != 12){
				ACL_PROC_PRINTK("[%s:%d] cmd_addr: %s, strlen(cmd_addr): %d\n", __FUNCTION__, __LINE__, cmd_addr, strlen(cmd_addr));
				return ret;
			}
			snprintf(str_tmp1, sizeof(str_tmp1), "%s", cmd_addr);

			/* 	somehow strptr will end with one line break.
				If macmask is the last input, strlen == 13	*/
			cmd_addr = strsep(&strptr," ");
			if (cmd_addr==NULL || (strlen(cmd_addr) != 12 && strlen(cmd_addr) != 13)){
				ACL_PROC_PRINTK("[%s:%d] cmd_addr: %s, strlen(cmd_addr): %d\n", __FUNCTION__, __LINE__, cmd_addr, strlen(cmd_addr));
				return ret;
			}
			snprintf(str_tmp2, sizeof(str_tmp2), "%s", cmd_addr);

			if(!string_to_hex(str_tmp1, str_mac, 12)){
				panic_printk("	Mac Invalid, should be XXXXXXXXXXXX\n");
				return ret;
			}

			if(!string_to_hex(str_tmp2, str_mac_mask, 12)){
				panic_printk("	Mac Mask Invalid, should be XXXXXXXXXXXX\n");
				return ret;
			}

			if(smacFlag){
				rule->srcMac_.octet[0] = str_mac[0];
				rule->srcMac_.octet[1] = str_mac[1];
				rule->srcMac_.octet[2] = str_mac[2];
				rule->srcMac_.octet[3] = str_mac[3];
				rule->srcMac_.octet[4] = str_mac[4];
				rule->srcMac_.octet[5] = str_mac[5];

				rule->srcMacMask_.octet[0] = str_mac_mask[0];
				rule->srcMacMask_.octet[1] = str_mac_mask[1];
				rule->srcMacMask_.octet[2] = str_mac_mask[2];
				rule->srcMacMask_.octet[3] = str_mac_mask[3];
				rule->srcMacMask_.octet[4] = str_mac_mask[4];
				rule->srcMacMask_.octet[5] = str_mac_mask[5];
			}
			else{
				rule->dstMac_.octet[0] = str_mac[0];
				rule->dstMac_.octet[1] = str_mac[1];
				rule->dstMac_.octet[2] = str_mac[2];
				rule->dstMac_.octet[3] = str_mac[3];
				rule->dstMac_.octet[4] = str_mac[4];
				rule->dstMac_.octet[5] = str_mac[5];

				rule->dstMacMask_.octet[0] = str_mac_mask[0];
				rule->dstMacMask_.octet[1] = str_mac_mask[1];
				rule->dstMacMask_.octet[2] = str_mac_mask[2];
				rule->dstMacMask_.octet[3] = str_mac_mask[3];
				rule->dstMacMask_.octet[4] = str_mac_mask[4];
				rule->dstMacMask_.octet[5] = str_mac_mask[5];
			}	
		}
		else if(!strncmp(cmd_addr, "ethType", 7)){
			cmd_addr = strsep(&strptr," ");
			if (cmd_addr==NULL){
				ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
				return ret;
			...
			sscanf(cmd_addr, "%x", &intVal);
			rule->typeLen_ = (uint16)intVal;
			...
			sscanf(cmd_addr, "%x", &intVal);
			rule->typeLenMask_ = (uint16)intVal;
		}
		else{
			panic_printk("	Do not support params %s\n", cmd_addr);
			return ret;
		}
...
/*	Hex
	{sip $ip $mask} {dip $ip $mask} {tos $tos $mask} {ipProto $proto $protomask}
	sip C0A8010A FFFFFFFF dip C0A8030A FFFFFF00 
	tos 20 FF ipProto 02 FF
*/
int32 config_ip_acl_rule(char *strptr, rtl865x_AclRule_t *rule)
{
	int ret = -1, flag;
	char *cmd_addr;
	uint32 intVal1, intVal2;
	...
	while(cmd_addr){
		if(!strncmp(cmd_addr, "sip", 3) || !strncmp(cmd_addr, "dip", 3) 
		|| !strncmp(cmd_addr, "tos", 3) || !strncmp(cmd_addr, "ipProto", 7)){
			if(!strncmp(cmd_addr, "sip", 3)){
				flag = 1;
			}
			else if(!strncmp(cmd_addr, "dip", 3)){
				flag = 2;
			}
			else if(!strncmp(cmd_addr, "tos", 3)){
				flag = 3;
			}
			else{
				flag = 4;
			...
			sscanf(cmd_addr, "%x", &intVal2);

			switch(flag){
			case 1:
				rule->srcIpAddr_		= intVal1;
				rule->srcIpAddrMask_	= intVal2;
				break;
			case 2:
				rule->dstIpAddr_		= intVal1;
				rule->dstIpAddrMask_	= intVal2;
				break;
			case 3:
				rule->tos_				= (uint8)intVal1;
				rule->tosMask_			= (uint8)intVal2;
				break;
			case 4:
				rule->ipProto_			= (uint8)intVal1;
				rule->ipProtoMask_		= (uint8)intVal2;
				break;	
			default:
				break;
			}
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
/*	Hex
	{nheader $nheader $mask}
	nheader 3A FF  -- ICMPv6 
	nheader 11 FF  -- UDP 
*/
int32 config_ipv6_acl_rule(char *strptr, rtl865x_AclRule_t *rule)
{
	int ret = -1;
	char *cmd_addr;
	uint32 intVal1, intVal2;
	...
	while(cmd_addr){
		if(!strncmp(cmd_addr, "nheader", 7)){
			...
			rule->ipv6NextHeader_		= intVal1;
			rule->ipv6NextHeaderM_		= intVal2;
		}
		else{
			panic_printk("	Do not support params %s\n", cmd_addr);
			return ret;
...
/*	Hex
	{sport $portL $portU} {dport $portL $portU} {sip $ip $mask} {dip $ip $mask} {tos $tos $mask}
	Note: port must before ip info.
	sport 4E20 4E30 sip C0A8010A FFFFFFFF
*/

int32 config_udp_tcp_acl_rule(char *strptr, rtl865x_AclRule_t *rule, uint8 tcpFlag)
{
	int ret = -1, sportFlag;
	char *cmd_addr;
	uint32 lport, uport, flags, flagMask;
	...
	if(tcpFlag){
		...
		rule->udpDstPortUB_  = 0xFFFF;
	}

	while(strptr){
		if(!strncmp(strptr, "sip", 3) || !strncmp(strptr, "dip", 3) || !strncmp(strptr, "tos", 3)){
			if(config_ip_acl_rule(strptr, rule) < 0){
				panic_printk("	IP params in TCP ACL fails\n");
				return ret;
			}

			// port must before ip info.
			return 0;
		}
		...
		if(!strncmp(cmd_addr, "sport", 5) || !strncmp(cmd_addr, "dport", 5)){
			if(!strncmp(cmd_addr, "sport", 5)){
				sportFlag = 1;
			}
			else{
				sportFlag = 0;
			...
			sscanf(cmd_addr, "%x", &uport);

			if(sportFlag){
				if(tcpFlag){
					rule->tcpSrcPortLB_	= (uint16)lport;
					rule->tcpSrcPortUB_	= (uint16)uport;
				}
				else{
					rule->udpSrcPortLB_  = (uint16)lport;
					rule->udpSrcPortUB_  = (uint16)uport;
				}
			}
			else{
				if(tcpFlag){
					rule->tcpDstPortLB_	= (uint16)lport;
					rule->tcpDstPortUB_	= (uint16)uport;
				}
				else{
					rule->udpDstPortLB_  = (uint16)lport;
					rule->udpDstPortUB_  = (uint16)uport;
				}
			}
		}
		else if(rule->ruleType_ == RTL865X_ACL_TCP && !strncmp(cmd_addr, "flags", 5)){
			cmd_addr = strsep(&strptr," ");
			if (cmd_addr==NULL){
				ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
				return ret;
			}
			sscanf(cmd_addr, "%x", &flags);
			if(flags > 0x3F){
				ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
				return ret;
			}
...
/*	Hex
	{smac $mac $mask} {svid $vid $mask} {sip $ip $mask} {sport $portL $portU} {proto $proto} {srcPhyport $port}
	smac 000600030002 FFFFFFFFFFFF sip C0A8010A C0A8010B proto all 
	svid 2D0 FFF sport 4E20 4E30 srcPhyport 111
*/

int32 config_src_dstFilter_acl_rule(char *strptr, rtl865x_AclRule_t *rule)
{
	int ret = -1, flag, src_filter_flag;
	char *cmd_addr;
	char str_tmp1[13] = {0}, str_tmp2[13] = {0};
	char str_mac[6] = {0}, str_mac_mask[6] = {0};
	uint32 val1, val2;
	...
	if(rule->ruleType_ == RTL865X_ACL_SRCFILTER){
		src_filter_flag = 1;
	}
	else{
		src_filter_flag = 0;
	...
	// default
	if(src_filter_flag){
		rule->srcFilterIpAddrLB_		= 0x0;
		rule->srcFilterIpAddrUB_		= 0xFFFFFFFF;
		rule->srcFilterPortLowerBound_	= 0x0;
		rule->srcFilterPortUpperBound_	= 0xFFFF;
		rule->srcFilterPort_ 			= 0x1FF;
	}
	else{
		rule->dstFilterIpAddrLB_		= 0x0;
		rule->dstFilterIpAddrUB_		= 0xFFFFFFFF;
		rule->dstFilterPortLowerBound_	= 0x0;
		rule->dstFilterPortUpperBound_	= 0xFFFF;
	}
	...
	while(cmd_addr){
		if((src_filter_flag && !strncmp(cmd_addr, "smac", 4)) || (!src_filter_flag && !strncmp(cmd_addr, "dmac", 4))){
			cmd_addr = strsep(&strptr," ");
			if (cmd_addr==NULL || strlen(cmd_addr) != 12){
				ACL_PROC_PRINTK("[%s:%d] cmd_addr: %s\n", __FUNCTION__, __LINE__, cmd_addr);
				return ret;
			}
			snprintf(str_tmp1, sizeof(str_tmp1), "%s", cmd_addr);
			...
			if(src_filter_flag){
				rule->srcFilterMac_.octet[0] = str_mac[0];
				rule->srcFilterMac_.octet[1] = str_mac[1];
				rule->srcFilterMac_.octet[2] = str_mac[2];
				rule->srcFilterMac_.octet[3] = str_mac[3];
				rule->srcFilterMac_.octet[4] = str_mac[4];
				rule->srcFilterMac_.octet[5] = str_mac[5];

				rule->srcFilterMacMask_.octet[0] = str_mac_mask[0];
				rule->srcFilterMacMask_.octet[1] = str_mac_mask[1];
				rule->srcFilterMacMask_.octet[2] = str_mac_mask[2];
				rule->srcFilterMacMask_.octet[3] = str_mac_mask[3];
				rule->srcFilterMacMask_.octet[4] = str_mac_mask[4];
				rule->srcFilterMacMask_.octet[5] = str_mac_mask[5];
			}
			else{
				rule->dstFilterMac_.octet[0] = str_mac[0];
				rule->dstFilterMac_.octet[1] = str_mac[1];
				rule->dstFilterMac_.octet[2] = str_mac[2];
				rule->dstFilterMac_.octet[3] = str_mac[3];
				rule->dstFilterMac_.octet[4] = str_mac[4];
				rule->dstFilterMac_.octet[5] = str_mac[5];

				rule->dstFilterMacMask_.octet[0] = str_mac_mask[0];
				rule->dstFilterMacMask_.octet[1] = str_mac_mask[1];
				rule->dstFilterMacMask_.octet[2] = str_mac_mask[2];
				rule->dstFilterMacMask_.octet[3] = str_mac_mask[3];
				rule->dstFilterMacMask_.octet[4] = str_mac_mask[4];
				rule->dstFilterMacMask_.octet[5] = str_mac_mask[5];
			}
		}
		else if((src_filter_flag && (!strncmp(cmd_addr, "svid", 4) || !strncmp(cmd_addr, "sip", 3) || !strncmp(cmd_addr, "sport", 5)))
			|| (!src_filter_flag && (!strncmp(cmd_addr, "dvid", 4) || !strncmp(cmd_addr, "dip", 3) || !strncmp(cmd_addr, "dport", 5)))){
			if(!strncmp(cmd_addr, "svid", 4) || !strncmp(cmd_addr, "dvid", 4)){
				flag = 1;
			}
			else if(!strncmp(cmd_addr, "sip", 3) || !strncmp(cmd_addr, "dip", 3)){
				flag = 2;
			}
			else{
				flag = 3;
			...
			sscanf(cmd_addr, "%x", &val2);

			switch(flag){
			case 1:
				if(src_filter_flag){
					rule->srcFilterVlanId_			= (uint16)val1;
					rule->srcFilterVlanIdMask_		= (uint16)val2;
				}
				else{
					rule->dstFilterVlanId_			= (uint16)val1;
					rule->dstFilterVlanIdMask_		= (uint16)val2;
				}
				break;
			case 2:
				if(src_filter_flag){
					rule->srcFilterIpAddrLB_		= val1;
					rule->srcFilterIpAddrUB_		= val2;
				}
				else{
					rule->dstFilterIpAddrLB_		= val1;
					rule->dstFilterIpAddrUB_		= val2;
				}
				break;
			case 3:
				if(src_filter_flag){
					rule->srcFilterPortLowerBound_	= (uint16)val1;
					rule->srcFilterPortUpperBound_	= (uint16)val2;
				}
				else{
					rule->dstFilterPortLowerBound_	= (uint16)val1;
					rule->dstFilterPortUpperBound_	= (uint16)val2;
				}
				break;
			default:
				break;
		...
		else if(!strncmp(cmd_addr, "proto", 5)){
			cmd_addr = strsep(&strptr," ");
			if (cmd_addr==NULL){
				ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
				return ret;
			...
			if(!strncmp(cmd_addr, "all", 3)){
				// do nothing
			}
			else if(!strncmp(cmd_addr, "l2_l3", 5)){
				if(src_filter_flag){
			...
			else if(!strncmp(cmd_addr, "l2", 2)){
				if(src_filter_flag){
					rule->srcFilterIgnoreL3L4_ = 1;
				}
				else{
					rule->dstFilterIgnoreL3L4_ = 1;
				}
			}
			else{
				if(src_filter_flag){
					panic_printk("	Do not support srcFilter proto %s\n", cmd_addr);
				}
				else{
					panic_printk("	Do not support dstFilter proto %s\n", cmd_addr);
				}
				return ret;
			}
		}
		else if(src_filter_flag && !strncmp(cmd_addr, "srcPhyport", 10)){
			cmd_addr = strsep(&strptr," ");
			if (cmd_addr==NULL){
				ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
				return ret;
			...
			sscanf(cmd_addr, "%x", &val1);
			rule->srcFilterPort_ = (uint16)val1;
		}
		else{
			panic_printk("	Do not support params %s\n", cmd_addr);
			return ret;
...
/*
Hex
	{$pattern_num} {$offset1 $val1 $val1mask} {$offset2 $val2 $val2mask}....   // most support 8 pattern
	2 C 8100 ffff D 0064 ffff  (match pkt with vid 100)
*/
#define PATTERN_NUM              		8
#define PATTERN_MAX_OFFSET             0x7D
int32 config_pattern_match_acl_rule(char *strptr, rtl865x_AclRule_t *rule)
{
	char *cmd_addr;
	int ret = -1, pa_flag = 0, i = 0;
	uint32 pattern_para[PATTERN_NUM][3];  
	uint32 pa_num = 0,offset = 0,value = 0,mask = 0;
	...
	if(rule->ruleType_ == RTL865X_ACL_PM){
		pa_flag = 1;
	}
	else{
		pa_flag = 0;
	...
	sscanf(cmd_addr, "%d", &pa_num);
	if (pa_num < 0 || pa_num > PATTERN_NUM){
		ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
		return ret;
	}

	memset(pattern_para,0,sizeof(pattern_para));
	for(i = 0;i < pa_num;i++)
	{
		cmd_addr = strsep(&strptr," ");
		if (cmd_addr==NULL){
		...
		sscanf(cmd_addr, "%x", &mask);

		if(pa_flag && offset>=0 && offset<=PATTERN_MAX_OFFSET  && value>=0 && value<=0xffff && mask>=0 && mask<=0xffff){
			pattern_para[i][0] = offset;
			pattern_para[i][1] = value;
			pattern_para[i][2] = mask;
		}
		else{
			ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
			return ret;
	...
	if(pa_flag && pa_num){
			rule->pmOffset7_ = (uint16)pattern_para[7][0]; 
			rule->pmPattern7_ = pattern_para[7][1];
			rule->pmPatternMask7_ = pattern_para[7][2];
			
			rule->pmOffset6_ = (uint16)pattern_para[6][0]; 
			rule->pmPattern6_ = pattern_para[6][1];
			rule->pmPatternMask6_ = pattern_para[6][2];

			rule->pmOffset5_ = (uint16)pattern_para[5][0]; 
			rule->pmPattern5_ = pattern_para[5][1];
			rule->pmPatternMask5_ = pattern_para[5][2];

			rule->pmOffset4_ = (uint16)pattern_para[4][0]; 
			rule->pmPattern4_ = pattern_para[4][1];
			rule->pmPatternMask4_ = pattern_para[4][2];

			rule->pmOffset3_ = (uint16)pattern_para[3][0]; 
			rule->pmPattern3_ = pattern_para[3][1];
			rule->pmPatternMask3_ = pattern_para[3][2];

			rule->pmOffset2_ = (uint16)pattern_para[2][0]; 
			rule->pmPattern2_ = pattern_para[2][1];
			rule->pmPatternMask2_ = pattern_para[2][2];

			rule->pmOffset1_ = (uint16)pattern_para[1][0]; 
			rule->pmPattern1_ = pattern_para[1][1];
			rule->pmPatternMask1_ = pattern_para[1][2];

			rule->pmOffset0_ = (uint16)pattern_para[0][0]; 
			rule->pmPattern0_ = pattern_para[0][1];
			rule->pmPatternMask0_ = pattern_para[0][2];
	}else{
		ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
		return ret;
	}
...
/*	Note -- required[], optional{}. 
			[rule ***] must be the last params.
	
	echo "add [netif eth1] [chain -16000] [act PERMIT]
			  {layer ALL} {rateIdx 1} {inv 0} {com 0} [type IP] [rule ***]" > /proc/rtl865x/acl
*/
int32 config_acl_rules(char *strptr)
{
	int ret = -1, chain_prio = 0, addFlag, intVal;
	char *cmd_addr, *cmd_addr2;
	char netif_name[32] = {0};
	rtl865x_AclRule_t	rule;
	...
	if(!strncmp(cmd_addr, "add", 3)){
		addFlag = 1;
	}
	else if(!strncmp(cmd_addr, "del", 3)){
		addFlag = 0;
	}
	else{
		ACL_PROC_PRINTK("[%s:%d] cmd_addr: %s\n", __FUNCTION__, __LINE__, cmd_addr);
	...
	bzero((void*)&rule,sizeof(rtl865x_AclRule_t));
	rule.pktOpApp_ 		= RTL865X_ACL_ALL_LAYER;	// default
	...
	while(cmd_addr){
		if(!strncmp(cmd_addr, "rule", 4)){	// rule length is variable 
			if(rule.ruleType_ == RTL865X_ACL_MAC){
				if(config_mac_acl_rule(strptr, &rule) < 0){
					return ret;
				}
			}
		#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
			else if(rule.ruleType_ == RTL865X_ACL_IPV6){
				if(config_ipv6_acl_rule(strptr, &rule) < 0){
					return ret;
				}
			}
		#endif
			else if(rule.ruleType_ == RTL865X_ACL_IP){
				if(config_ip_acl_rule(strptr, &rule) < 0){
					return ret;
				}
			}
			else if(rule.ruleType_ == RTL865X_ACL_TCP){
				if(config_udp_tcp_acl_rule(strptr, &rule, 1) < 0){
					return ret;
				}
			}
			else if(rule.ruleType_ == RTL865X_ACL_UDP){
				if(config_udp_tcp_acl_rule(strptr, &rule, 0) < 0){
					return ret;
				}
			}
			else if(rule.ruleType_ == RTL865X_ACL_SRCFILTER || rule.ruleType_ == RTL865X_ACL_DSTFILTER){
				if(config_src_dstFilter_acl_rule(strptr, &rule) < 0){
					return ret;
				}
			}
		#if defined(CONFIG_RTL_8197F)
			else if(rule.ruleType_ == RTL865X_ACL_PM){
				if(config_pattern_match_acl_rule(strptr, &rule)<0){
					return ret;
				}
			}
		#endif
			else{
				panic_printk("	Do not support acl type %d\n", rule.ruleType_);
				return ret;
			}
		...
		// other settings in pair
		cmd_addr2 = strsep(&strptr," ");
		if (cmd_addr2 == NULL){
			ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
			return ret;
		}

		if(!strncmp(cmd_addr, "netif", 5)){
			snprintf(netif_name, sizeof(netif_name), "%s", cmd_addr2);
		}
		else if(!strncmp(cmd_addr, "chain", 5)){
			sscanf(cmd_addr2, "%d", &chain_prio);
		}
		else if(!strncmp(cmd_addr, "type", 4)){
			if(config_acl_type(cmd_addr2, &rule) < 0){
				return ret;
			}
		}
		else if(!strncmp(cmd_addr, "act", 3)){
			if(config_acl_action(cmd_addr2, &rule) < 0){
				return ret;
			}

			if(rule.actionType_ == RTL865X_ACL_PRIORITY){
				cmd_addr = strsep(&strptr," ");
				if (cmd_addr==NULL){
					ACL_PROC_PRINTK("[%s:%d]\n", __FUNCTION__, __LINE__);
					return ret;
				}
				sscanf(cmd_addr, "%d", &intVal);
				rule.priority_ = intVal;
			}
		}
		else if(!strncmp(cmd_addr, "layer", 5)){
			if(config_acl_layer(cmd_addr2, &rule) < 0){
				return ret;
			}
		}
		else if(!strncmp(cmd_addr, "rateIdx", 7)){
			sscanf(cmd_addr2, "%x", &intVal);

			if(intVal > 15){
				panic_printk("	Rate limit table index %d out of range", intVal);
				return ret;
			}
			rule.ratelimtIdx_ = intVal;
		}
		#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		else if(!strncmp(cmd_addr, "inv", 3)){
			sscanf(cmd_addr2, "%x", &intVal);
			if(intVal){
				rule.ipv6Invert_ = 1;
			}
		}
		else if(!strncmp(cmd_addr, "com", 3)){
			sscanf(cmd_addr2, "%x", &intVal);
			if(intVal){
				rule.ipv6Combine_ = 1;
			}
	...
	if(addFlag){
		ret = rtl865x_add_acl(&rule, netif_name, chain_prio);
		ACL_PROC_PRINTK("[%s:%d] Add: %d\n", __FUNCTION__, __LINE__, ret);
	}
	else{
		ret = rtl865x_del_acl(&rule, netif_name, chain_prio);
		ACL_PROC_PRINTK("[%s:%d] Del: %d\n", __FUNCTION__, __LINE__, ret);
	}
	
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
#if defined(CONFIG_RTL_8198C) 
	if(rule.ruleType_ == RTL865X_ACL_IPV6)
#else 		
	if(rule.ruleType_ == RTL865X_ACL_IPV6 || rule.ruleType_ == RTL865X_ACL_PM)
#endif
	{
...
/*	
	echo "flush eth1 in" > /proc/rtl865x/acl	
	echo "flush eth1 out" > /proc/rtl865x/acl	*/
	
int32 flush_netif_acl(char *strptr)
{
	int i, num, dir, ret = -1;
	char netif_name[32] = {0}, str_op[8] = {}, str_dir[8] = {0};
	rtl865x_acl_chain_t *chain;
	rtl865x_AclRule_t *rule, *delRule;
	...
	num = sscanf(strptr, "%s %s %s", str_op, netif_name, str_dir);
	if(num != 3){
		ACL_PROC_PRINTK("[%s:%d] num: %d\n", __FUNCTION__, __LINE__, num);
		return ret;
	}

	if(strncmp(str_op, "flush", 5)){
		ACL_PROC_PRINTK("[%s:%d] str_op: %s\n", __FUNCTION__, __LINE__, str_op);
		return ret;
	}
	...
	for(i = 0; i < NETIF_NUMBER; i++){
		if(netifTbl[i].valid == 1 && !strcmp(netifTbl[i].name, netif_name)){
			chain = netifTbl[i].chainListHead[dir];
			while(chain){
				rule = chain->head;
				while(rule){
					delRule = rule;
					rule = rule->next;
					ACL_PROC_PRINTK("[%s:%d] del ruleType_: %d\n", __FUNCTION__, __LINE__, delRule->ruleType_);
				
					ret = rtl865x_del_acl(delRule, netif_name, chain->priority);
					ACL_PROC_PRINTK("[%s:%d] ret: %d\n", __FUNCTION__, __LINE__, ret);
				}
				chain = chain->nextChain;
			}
		}
