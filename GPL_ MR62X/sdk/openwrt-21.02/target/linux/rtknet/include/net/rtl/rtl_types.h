...
#elif defined (__linux__)&&defined(__KERNEL__)&& defined(CONFIG_RTL_8197F)
	#define __DRAM_GEN
        #define __DRAM_FWD
...
typedef struct inv6_addr_s
{
	...
	{
		__u8		u6_addr8[16];
		uint16		u6_addr16[8];  /*HF: in host endian*/
		uint32		u6_addr32[4];  /*HF: in host endian*/
	} inv6_u;
...
extern int lock_owner_proc_cmd;
extern int lock_owner_eth_phy;
...
extern spinlock_t lock_proc_cmd;
extern spinlock_t lock_eth_phy;
...
#define SMP_LOCK_ETH_EVENT(__x__)	\
			do { \
				if(lock_owner_event!=smp_processor_id()) \
					spin_lock_irqsave(&lock_eth_event, (__x__)); \
				else \
					rtlglue_printf("[%s %d] recursion detection in Event\n",__FUNCTION__,__LINE__); \
				lock_owner_event=smp_processor_id();\
			} while(0)
			
#define SMP_UNLOCK_ETH_EVENT(__x__) \
			do { \
				lock_owner_event=-1; \
				spin_unlock_irqrestore(&lock_eth_event, (__x__)); \
			} while(0)
...
#define SMP_LOCK_PROC_CMD(__x__)	\
	do { \
		if(lock_owner_proc_cmd !=smp_processor_id()) \
			spin_lock_irqsave(&lock_proc_cmd, (__x__)); \
		else \
			rtlglue_printf("[%s %d] recursion detection in HW\n",__FUNCTION__,__LINE__); \
		lock_owner_proc_cmd=smp_processor_id();\
	} while(0)
	
#define SMP_UNLOCK_PROC_CMD(__x__) \
	do { \
		lock_owner_proc_cmd=-1; \
		spin_unlock_irqrestore(&lock_proc_cmd, (__x__)); \
	} while(0)

#define SMP_LOCK_ETH_PHY(__x__)	\
	do { \
		if(lock_owner_eth_phy!=smp_processor_id()) \
			spin_lock_irqsave(&lock_eth_phy, (__x__)); \
		else \
			rtlglue_printf("[%s %d] recursion detection in HW\n",__FUNCTION__,__LINE__); \
		lock_owner_eth_phy=smp_processor_id();\
	} while(0)
	
#define SMP_UNLOCK_ETH_PHY(__x__) \
	do { \
		lock_owner_eth_phy=-1; \
		spin_unlock_irqrestore(&lock_eth_phy, (__x__)); \
	} while(0)	
#endif
...
#define SMP_LOCK_PROC_CMD(x)		local_irq_save(x)
#define SMP_UNLOCK_PROC_CMD(x)		local_irq_restore(x)

#define SMP_LOCK_ETH_PHY(x)			local_irq_save(x)
#define SMP_UNLOCK_ETH_PHY(x)		local_irq_restore(x)
#endif 
