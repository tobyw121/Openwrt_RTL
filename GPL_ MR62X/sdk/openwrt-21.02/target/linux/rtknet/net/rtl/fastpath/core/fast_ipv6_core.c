...
{	
	...
	if(!CTAILQ_EMPTY(&v6_path_list_free))
	{
		...
		if(proto == ETH_P_IPV6)
		{
			...
			if (rtl_hook_is_empty(pskb, PF_INET6, NF_INET_PRE_ROUTING) == 0)
				rtl_ipt6_do_table(pskb, NF_INET_PRE_ROUTING, InDev,NULL);

			if (rtl_hook_is_empty(pskb, PF_INET6, NF_INET_POST_ROUTING) == 0)
...
*/
int rtk_flushV6Connection(void)
{
	uint32	hash;
	struct V6_PATH_List_Entry *entry_path;
    #if defined(CONFIG_SMP)
...
*/
int rtk_idleV6Connection(struct V6_Tuple *PathEntry,uint32 interval, unsigned long *lastUsed)
{
	uint16 ipprotocol;
	uint32	hash;
...
int rtl_V6_Cache_Timer_update(void *ct)
{
	...
	
	{
		...
		V6PathEntry.sport = (rtl_get_ct_port_by_dir(ct, IP_CT_DIR_ORIGINAL, 0));
		V6PathEntry.dport = (rtl_get_ct_port_by_dir(ct, IP_CT_DIR_ORIGINAL, 1));

		if(rtk_idleV6Connection(&V6PathEntry,expires,&lastUsed) != LR_SUCCESS)
		{
	...
	*/
	...
	CTAILQ_FOREACH(entry_path, &v6_table_path->list[hash],path_link)
	{
			...
			(entry_path->protocol_v6 ==iphProtocol)))
		{
			...
			if(ipv6_rtcache) {
				if(rtl_ip_route_input(NFPROTO_IPV6, pskb, 0, entry_path->ct)) {
					#if defined(CONFIG_SMP)
	                spin_unlock(&v6_table_path->v6_path_table_lock);
					#endif
					return 0;
			...
			if (fast_path_post_process_xmit_check6(pskb) == NET_RX_DROP) {
				return NET_RX_DROP;
			}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
static int ipv6_forward_table_write_proc(struct file *file, const char *buffer,
		      unsigned long count, void *data);

static int ipv6_forward_table_read_proc(struct seq_file *s, void *v)
{
	struct V6_PATH_List_Entry *entry_path;
	CTAILQ_FOREACH(entry_path, &v6_path_list_inuse, tqe_link){
#if defined(IMPROVE_QOS) && defined (CONFIG_NET_SCHED)
		seq_printf(s, "V6 Path --- [protocol %d] in-S=%x:%x:%x:%x:%x:%x:%x:%x (%-5u)  out-D=%x:%x:%x:%x:%x:%x:%x:%x (%-5u), [Mark %d]\n", 
			entry_path->protocol_v6, NIPV6QUAD((entry_path->saddr_v6.s6_addr16)), ntohs(entry_path->sport_v6), 
			NIPV6QUAD((entry_path->daddr_v6.s6_addr16)), ntohs(entry_path->dport_v6), entry_path->mark);
#else
		seq_printf(s, "V6 Path --- [protocol %d] in-S=%x:%x:%x:%x:%x:%x:%x:%x (%-5u)  out-D=%x:%x:%x:%x:%x:%x:%x:%x (%-5u)\n", 
			entry_path->protocol_v6, NIPV6QUAD((entry_path->saddr_v6.s6_addr16)), ntohs(entry_path->sport_v6), 
			NIPV6QUAD((entry_path->daddr_v6.s6_addr16)), ntohs(entry_path->dport_v6));
#endif
	}	
...
int ipv6_fast_forward_table_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, ipv6_forward_table_read_proc, NULL));
}

static ssize_t ipv6_fast_forward_table_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return ipv6_forward_table_write_proc(file, userbuf,count, off);
}

struct file_operations ipv6_fast_forward_table_proc_fops= {
        .open           = ipv6_fast_forward_table_single_open,
        .write		    = ipv6_fast_forward_table_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#if defined(CONFIG_PROC_FS)
static struct proc_dir_entry *ipv6_res_table=NULL;
#endif

int __init ipv6_fast_forward_table_init(void)
{
#if defined(CONFIG_PROC_FS)
    #if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
	ipv6_res_table = proc_create_data("ipv6_fast_forward_table", 0, &proc_root,
			 &ipv6_fast_forward_table_proc_fops, NULL);
    #endif
#endif
	return 0;
}

void __exit ipv6_fast_forward_table_exit(void)
{
#if defined(CONFIG_PROC_FS)
    #if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
	if (ipv6_res_table) {
        remove_proc_entry("ipv6_fast_forward_table", &proc_root);
    }
    #endif
#endif
}
