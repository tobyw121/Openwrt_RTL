...
extern void nf_conntrack_all_lock(void);
extern void nf_conntrack_all_unlock(void);
#endif

extern int32 rtl_get_hw_fdb_port(uint32 fid, ether_addr_t *mac, uint32 flags, uint8 *port);

#define MAX_EXT_SERVICE_PORTS 30
static int ext_service_ports[MAX_EXT_SERVICE_PORTS] = {0};

struct ppp_magic_info fast_ppp_magic_info[MAX_PPP_MAGIC_ENTRY];
...
int magicNum[MAX_PPPOE_ENTRY];
#else
int magicNum=-1;
#endif
#ifdef CONFIG_RTL_ISP_MULTI_WAN_SUPPORT
int get_magicNum(struct sk_buff *skb, int *index)
{
	int idx=0;
	sscanf(skb->dev->name, "eth%d*", &idx);
	if(idx<0 || idx>=MAX_PPPOE_ENTRY)
	{
		//panic_printk("%s:%d idx is invalid!\n",__FUNCTION__,__LINE__);
		return -1;
	}
...
void clear_magicNum(char *ifname)
{
	int idx=0;	
	if(ifname==NULL)
		return;
	...
	if(strncmp(ifname, "ppp", 3)==0)
		sscanf(ifname, "ppp%d*", &idx);
	else if(strncmp(ifname, "eth", 3)==0)
		sscanf(ifname, "eth%d*", &idx);

	//panic_printk("\n%s:%d ifname=%s idx=%d\n",__FUNCTION__,__LINE__,ifname,idx);
	
	if(idx<0 || idx>=MAX_PPPOE_ENTRY)
	{
		panic_printk("%s:%d magicNum idx is invalid!\n",__FUNCTION__,__LINE__);
		return;
	}
...
int is_pppoe_lcp_echo_req(struct sk_buff *skb)
{
	...
	if(strncmp(skb->dev->name, ppp_ext, 5)==0)
	{
		return 0;
	}
...
void extract_magicNum(struct sk_buff *skb)
{
	...
	*/
	...
	if (*(uint16*)(&(skb->data[12])) == htons(0x8863) && (skb->data[15]==0x09 || skb->data[15]==0x19))
	{		
		//panic_printk("\n%s:%d send_ifname=%s\n",__FUNCTION__,__LINE__,skb->dev->name);		
		#ifdef CONFIG_RTL_ISP_MULTI_WAN_SUPPORT	
		clear_magicNum(skb->dev->name);
		#else
		clear_magicNum(); 
		#endif
		return;
	...
	if(mac_hdr[22]==0x01)
	{
		...
		while(payloadLen>0)
		{
			...
			{
			#ifdef CONFIG_RTL_ISP_MULTI_WAN_SUPPORT											
				memcpy(&magicNum[idx], ptr+2 , 4);					
				//panic_printk("%s:%d, idx=%d send_ifname=%s magic number is 0x%x\n",__FUNCTION__,__LINE__,idx,skb->dev->name,magicNum[idx]);		
			#else
...
{	
	...
	if(strncmp(skb->dev->name, ppp_ext , 5)!=0)
	{
		...
		memcpy((skb->data+26),&magicNum[idx] ,4);		
		//panic_printk("%s:%d, idx=%d recv_ifname=%s magic number is 0x%x\n",__FUNCTION__,__LINE__,idx,skb->dev->name,magicNum[idx]);
#else
		memcpy((skb->data+26),&magicNum ,4);		
		//panic_printk("%s:%d recv_ifname=%s magic number is 0x%x\n",__FUNCTION__,__LINE__,skb->dev->name,magicNum);
#endif
...
extern int urlPatchEnabled;
#endif
// echo "url_patch on" > proc/fast_nat
// echo "url_patch off" > proc/fast_nat
static struct proc_dir_entry *res1=NULL;
static struct proc_dir_entry *res_udp=NULL;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
extern int kill_all(struct nf_conn *i, void *data);
#endif
		      ...
		      unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpbuf, buffer, count))  {
		if (!strncmp(tmpbuf, "url_patch", 9)){
			strptr=tmpbuf;
			cmd_addr = strsep(&strptr," ");
			if ((strptr!=NULL)&&(!strncmp(strptr, "on", 2)))
			{
				urlPatchEnabled=1;
			}
			else if ((strptr!=NULL)&&(!strncmp(strptr, "off", 2)))
			{
				urlPatchEnabled=0;
			}
			else
			{
				...
				printk("	enable: echo \"url_patch on\" > proc/fast_nat\n");
				printk("	disable: echo \"url_patch off\" > proc/fast_nat\n");
			}
		}
		else if (tmpbuf[0] == '2'&&count==2){
			/* first byte == 2, second byte == "enter" */
			for_each_net(net) {
				#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
				// nf_conntrack_flush_report has been removed in kernel 4.4
...
{
	#if defined(CONFIG_RTL_URL_PATCH)
	seq_printf(s, "urlPatchEnabled = %d\n", urlPatchEnabled);
	#endif
	#if defined(CONFIG_RTL_NF_CONNTRACK_GARBAGE_NEW)
...
int ext_service_ports_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, ext_service_ports_read_proc, NULL));
}

static ssize_t ext_service_ports_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return ext_service_ports_write_proc(file, userbuf,count, off);
}

struct file_operations ext_service_ports_proc_fops = {
        .open           = ext_service_ports_single_open,
	 .write		= ext_service_ports_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
		     ...
		     int count, int *eof, void *data)
{
	int len = 0;
	#if defined(CONFIG_RTL_URL_PATCH)
	len = sprintf(page, "urlPatchEnabled = %d\n", urlPatchEnabled);
	#endif
	#if defined(CONFIG_RTL_NF_CONNTRACK_GARBAGE_NEW)
...
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
static int udp_entry_read_proc(struct seq_file *s, void *v)
{

	rtl_readUdpFragEntry();
	return 0;
}
static int udp_entry_write_proc(struct file *file, const char *buffer,
		      unsigned long count, void *data)
{
	return -EFAULT;
}

int udp_entry_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, udp_entry_read_proc, NULL));
}

static ssize_t udp_entry_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return udp_entry_write_proc(file, userbuf,count, off);
}

struct file_operations udp_entry_proc_fops = {
        .open           = udp_entry_single_open,
	 .write		= udp_entry_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
#else 
static int udp_entry_read_proc(char *page, char **start, off_t off,
		     int count, int *eof, void *data)
{
	int len = 0;
	rtl_readUdpFragEntry();
...
int rtl_ip_fragment_for_fastpath(struct sk_buff *skb, int (*output)(struct sk_buff *))
{
	...
	hlen = iph->ihl * 4;
	mtu = dst_mtu(dst) - hlen;	/* Size of data space */
...
void rtl_recheck_fp_policy_route_mark(struct sk_buff *skb, uint32 *fp_policy_route_mark)
{
	...
	skb_mark_bak = skb->mark;
	if (*fp_policy_route_mark == 0) {
		/*jwj: the pkt from lan dev to recheck mangle table to get mark.*/
		if ((strncmp(skb->dev->name, RTL_DRV_ETHLAN_P0_NETIF_NAME, strlen(RTL_DRV_ETHLAN_P0_NETIF_NAME))==0)   ||
		     (strncmp(skb->dev->name, RTL_DRV_LAN_P1_NETIF_NAME, strlen(RTL_DRV_LAN_P1_NETIF_NAME))==0) ||
		     (strncmp(skb->dev->name, RTL_DRV_LAN_P2_NETIF_NAME, strlen(RTL_DRV_LAN_P2_NETIF_NAME))==0) ||
		     (strncmp(skb->dev->name, RTL_DRV_LAN_P3_NETIF_NAME, strlen(RTL_DRV_LAN_P3_NETIF_NAME))==0) ||
		     (strncmp(skb->dev->name, "wlan", strlen("wlan"))==0)) {
				...
				#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
					(list_empty(&nf_hooks[PF_INET][NF_INET_PRE_ROUTING]))?: \
						rtl_ipt_do_table(skb, NF_INET_PRE_ROUTING, lanDev,wanDev);
					*fp_policy_route_mark = skb->mark;
				#else
					(list_empty(&nf_hooks[PF_INET][NF_IP_PRE_ROUTING]))?: \
						rtl_ipt_do_table(skb, NF_IP_PRE_ROUTING, lanDev,wanDev);
					*fp_policy_route_mark = skb->mark;
				#endif
...
static inline int enter_fast_path_fast_pptp_post_process(struct sk_buff *skb)
{
#if	defined(FAST_PPTP)
	if (fast_pptp_fw && skb->dev && (!memcmp(skb->dev->name, "pptp-internet", 4)) )
	{
		#if defined(RTL_FASTPATH_FRAGMENT_SUPPORT)
		if (skb->len > dst_mtu(skb_dst(skb))) 
		{
			if (fast_pptp_to_wan_check(skb) == 0)
				return NET_RX_SUCCESS;
			/*Acturally, rtl_ip_fragment_for_fastpath will never return 0.*/
			if (rtl_ip_fragment_for_fastpath(skb, fast_pptp_to_wan2))
				return NET_RX_DROP;
		} else
		#endif
		if (fast_pptp_to_wan(skb)) // success
		{
			return NET_RX_DROP;
		}
	}
...
static inline int enter_fast_path_fast_pppoe_post_process6(struct sk_buff *skb)
{
#if defined(CONFIG_RTL_FAST_PPPOE) && defined(CONFIG_RTL_FAST_IPV6)
	// do not support fragment packets now
	if(fast_pppoe_xmit6(skb)){
		return NET_RX_DROP;
	}
	else
#endif
...
{
	#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
	skb->network_header=network_header-skb->head;
	#else
...
{
	#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
	skb->transport_header=transport_header-skb->head;
	#else
	skb->transport_header=transport_header;
	#endif
}

struct ipv6_pinfo * rtl_inet6_sk(const struct sock *__sk)
{
	return inet6_sk(__sk);
}
...
unsigned int rtl_ipt_do_table(struct sk_buff * skb, unsigned int hook, void *in, void *out)
{
	...
	state.in = in_dev;
	state.out = out_dev;
	state.net = dev_net(in_dev);
	return ipt_do_table(skb, &state, dev_net(skb->dev)->ipv4.iptable_mangle);
#else
	return ipt_do_table(skb, hook, in_dev, out_dev, dev_net(skb->dev)->ipv4.iptable_mangle);
#endif
}

int rtl_ip_route_input(int pf, struct sk_buff  *skb, __be32 daddr, void *ct)
{
#if defined(CONFIG_NF_CONNTRACK_RTCACHE)
	if(pf == NFPROTO_IPV4) {
	    rtl_nf_rtcache_in(pf, skb,ct);
		if(!skb_dst(skb))
			return 1;
		return 0;
	}
	...
	if(pf == NFPROTO_IPV4) {
		...
		return ip_route_input(skb, daddr,iph->saddr, iph->tos, skb->dev);
	}
#if defined(CONFIG_IPV6)
	else {
		ip6_route_input(skb);
...
void rtl_dst_release_ipv6(struct sk_buff *skb)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
...
void rtl_set_skb_src_phy_port(struct sk_buff *skb,unsigned char* resMac, int is_wan)
{
	...
	if (gQosEnabled){
		if (is_wan == 0) {
			if (rtl_get_hw_fdb_port(RTL_LAN_FID, (ether_addr_t *)resMac, FDB_DYNAMIC, &phyPort) == SUCCESS){
				rtl_set_skb_phy_port(skb, phyPort);
			}
		} else if(is_wan == 1) {
			if (rtl_get_hw_fdb_port(RTL_WAN_FID, (ether_addr_t *)resMac, FDB_DYNAMIC, &phyPort) == SUCCESS){
				rtl_set_skb_phy_port(skb, phyPort);
			}
		}
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
int rtl_hook_is_empty(struct sk_buff *skb, int pf, int hooks)
{
	struct net *net;
	net = dev_net(skb->dev);
	
	return (list_empty(&net->nf.hooks[pf][hooks]));
}
#endif
...
void rtl_set_iph_dscp(struct Path_List_Entry *entry_path,struct iphdr *iph)
{
	...
	if((entry_path->dscp_in!=0) && (entry_path->course==2))//in bound flow
	{
		...
		FASTPATH_ADJUST_CHKSUM_NAT( ntohl(*(uint32 *)iph), ntohl(iph_firstword_bf), iph->check);
	}
...
{
#if defined(CONFIG_RTL_VLANID_IPTABLE_CHECK) && defined(CONFIG_RTL_QOS_VLANID_SUPPORT)
	return skb->srcVlanId;
#else
	return 0xffff;
#endif
}
...
{
	#if defined(CONFIG_RTL_VLANID_IPTABLE_CHECK) && defined(CONFIG_RTL_QOS_VLANID_SUPPORT)
		skb->srcVlanId = vlanId;
	#endif
}

void rtl_set_entry_vlanid_out(struct Path_List_Entry *entry_path, struct sk_buff *pskb)
{
	#if defined(CONFIG_RTL_VLANID_IPTABLE_CHECK) && defined(CONFIG_RTL_QOS_VLANID_SUPPORT)
	entry_path->vlanid_in = 0xffff;
	entry_path->vlanid_out = 0xffff;
	{ /* outbound flow */
		if(entry_path->vlanid_out != rtl_get_skb_vlanId(pskb))
			entry_path->vlanid_out = rtl_get_skb_vlanId(pskb);
	}
	#endif
}

void rtl_set_entry_vlanid_in(struct Path_List_Entry *entry_path, struct sk_buff *pskb)
{
	#if defined(CONFIG_RTL_VLANID_IPTABLE_CHECK) && defined(CONFIG_RTL_QOS_VLANID_SUPPORT)
	entry_path->vlanid_in = 0xffff;
	entry_path->vlanid_out = 0xffff;
	{ /* outbound flow */
		if(entry_path->vlanid_in != rtl_get_skb_vlanId(pskb))
			entry_path->vlanid_in = rtl_get_skb_vlanId(pskb);
	}
	#endif
}
...
__u16 rtl_get_skb_orig_vlanpri(struct sk_buff *skb)
{
	...
	return skb->original_vlanpri;
	#else
		// when disable CONFIG_RTL_VLANPRI_IPTABLE_CHECK, skb don't have original_vlanpri field
	return 0;
	#endif
}
...
{ 
	...
	if((entry_path->vlanpri_out!=0xff) && (entry_path->course==1))//out bound flow
		skb->srcVlanPriority = entry_path->vlanpri_out;
	#endif
}
...
{
#if defined(CONFIG_RTL_VLANPRI_IPTABLE_CHECK) && defined(CONFIG_RTL_QOS_8021P_SUPPORT)
	if(rtl_get_skb_orig_vlanpri(pskb) != rtl_get_skb_remark_vlanpri(pskb))
	{
		entry_path->vlanpri_out = rtl_get_skb_remark_vlanpri(pskb);
	}
#endif
}
...
int  rtl_Add_Pattern_ACL_For_ContentFilter(void)
{
 ...
 #if 0  //### add by sen_liu 2011.5.4 for lan(CONFIG_TP_LAN_BRIDGE_NAME),don't add pattern acl                     
	...
	#else
	rtl865x_add_pattern_acl_for_contentFilter(&rule,CONFIG_TP_LAN_BRIDGE_NAME);
	#endif
#endif
	...
	#else
	//rtl865x_add_pattern_acl_for_contentFilter(&rule,CONFIG_TP_LAN_BRIDGE_NAME); //### add by sen_liu 2011.5.4 for lan(CONFIG_TP_LAN_BRIDGE_NAME),don't add pattern acl 
 ...
 int  Del_Pattern_ACL_For_ContentFilter(void)
{
	...
	#else	
	rtl865x_del_pattern_acl_for_contentFilter(&rule,CONFIG_TP_LAN_BRIDGE_NAME);
	#endif
...
void rtl_get_ct_v6_ip_by_dir(void *ct_ptr, enum ip_conntrack_dir dir, int flag,struct in6_addr *addr)
{
	...
	if(flag == 0)
		memcpy(addr, &ct->tuplehash[dir].tuple.src.u3.in6, sizeof(struct  in6_addr));
	else if(flag == 1)
		memcpy(addr, &ct->tuplehash[dir].tuple.dst.u3.in6, sizeof(struct  in6_addr));
}
...
int rtl_ipt6_do_table(struct sk_buff * skb, unsigned int hook, void *in, void *out)
{
	...
	state.out = out_dev;
	state.net = dev_net(in_dev);
	ret = ip6t_do_table(skb, &state, dev_net(skb->dev)->ipv6.ip6table_mangle);
	#else
...
int fast_path_post_process_xmit_check(struct iphdr *iph, struct tcphdr *tcphupuh, struct sk_buff *skb)
{
	...
	else if (enter_fast_path_fast_pptp_post_process(skb) == NET_RX_DROP)
	{
		return NET_RX_DROP;
	}
...
int fast_path_post_process_xmit_check6(struct sk_buff *skb)
{
#if defined (CONFIG_RTL_FAST_PPPOE) && defined(CONFIG_RTL_FAST_IPV6)
	if(enter_fast_path_fast_pppoe_post_process6(skb)==NET_RX_DROP){
		return NET_RX_DROP;
	}
	else
#endif
		...
		if (skb->dev->flags & IFF_UP) {
...
#endif
			{
				...
				*/
				...
				return rtl_call_skb_ndo_start_xmit(skb);
			}
		}
//------------------------------- david+2007-05-25
...
#ifdef CONFIG_RTL_FAST_IPV6
extern unsigned int statistic_ipv6_hard_xmit;
extern unsigned int statistic_ipv6_neigh_xmit;
...
int FastPath_Enter(struct sk_buff **pskb)
{
	...
	mac_hdr = (struct ethhdr *)skb_mac_header(skb); /* rtl_skb_mac_header */
	...
	*/
...
#ifdef CONFIG_RTL_USB_IP_HOST_SPEEDUP
	if(mac_hdr->h_proto == ntohs(ETH_P_IP) && rtl_skb_to_dut(skb))
	{
		return 0;
	}
#endif
#ifdef CONFIG_RTK_OPENVPN_HW_CRYPTO
	if(strcmp(skb->dev->name, "tun0")==0)
		return 0;
#endif

#ifdef CONFIG_RTL_PPPOE_DIRECT_REPLY
#ifdef CONFIG_RTL_ISP_MULTI_WAN_SUPPORT
		if(fast_pppoe_fw && (get_magicNum(skb, NULL)!= -1) && is_pppoe_lcp_echo_req(skb))
#else
		if(fast_pppoe_fw && (magicNum != -1) && is_pppoe_lcp_echo_req(skb))
#endif
		{	
	...
	/*
	 * The path is determined by the Ethernet protocol 
	 */
	if(mac_hdr->h_proto == ntohs(ETH_P_IP))
	{
		/* Continue PPTP/L2TP/IP acceleration */
	}
	else if(mac_hdr->h_proto == ntohs(ETH_P_PPP_SES))
	{
#if defined(CONFIG_RTL_FAST_PPPOE)
		if(fast_pppoe_fw)
		{
			ret = pppoe_fast_enter(*pskb);
			if (ret == 1)
			{
				/* PPPoE/PPPoEv6 acceleration sucess, droped */
				return 1;
			}
		}	
#endif
		/* PPPoE/PPPoEv6 acceleration fail, return to cpu */
		return 0;	
	}
	else if(mac_hdr->h_proto == ntohs(ETH_P_IPV6))
	{
#ifdef CONFIG_RTL_FAST_IPV6
		if(fast_ipv6_fw){
			...
			if(ret == 1)
			{
				/* IPv6 acceleration sucess, droped */
				return 1;
			}
		}
#endif
		/* IPv6 acceleration fail, return to cpu */
		return 0;
	}
	else
	{
		/* other etype or invalid mac header, return to cpu */
		return 0;
	}
...
//bug fix:when port filter is enable,application will disable fast_nat_fw,at this moment,url filter is abnormal...
	...
	if ((skb->dev->priv_flags & IFF_RSMUX) && (skb->dev->priv_flags & IFF_DOMAIN_WAN)) {
		ret = 0;
		goto out;
	}
#endif
...
#if defined(CONFIG_RTL_FAST_IPV6)
static struct proc_dir_entry *res_rtcache6=NULL;

static int write_rtcache6_proc(struct file *file, const char *buffer,
		      unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpbuf, buffer, count))  {
		...
		if (cmd_addr && !memcmp(cmd_addr, "rtcache", strlen("rtcache"))){
			sscanf(strptr, "%x", &ipv6_rtcache);
			//printk("%s %d enable=%d \n", __FUNCTION__, __LINE__, ipv6_rtcache);
		}
		return count;
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
static int read_rtcache6_proc(struct seq_file *s, void *v)
{
	seq_printf(s, "enable: %d\n", ipv6_rtcache);
	return 0;
}

int rtcache6_single_open(struct inode *inode, struct file *file)
{
        return(single_open(file, read_rtcache6_proc, NULL));
}

static ssize_t rtcache6_single_write(struct file * file, const char __user * userbuf,
		     size_t count, loff_t * off)
{
	return write_rtcache6_proc(file, userbuf, count, off);
}

struct file_operations rtcache6_proc_fops = {
        .open           = rtcache6_single_open,
	 	.write			= rtcache6_single_write,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = single_release,
};
#else
static int read_rtcache6_proc(char *page, char **start, off_t off,
		     int count, int *eof, void *data)
{
	int len = 0;
	return len;
...
static int __init fastpath_init(void)
{
	...
	memset(fast_ppp_magic_info, 0, sizeof(fast_ppp_magic_info));
	...
	#endif
	#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
	res_udp = proc_create_data("udp_frag_entry", 0, &proc_root,
			 &udp_entry_proc_fops, NULL);
	#else
	res_udp = create_proc_entry("udp_frag_entry",0,NULL);
	if (res_udp) {
	    res_udp->read_proc=udp_entry_read_proc;
	    res_udp->write_proc=udp_entry_write_proc;
	}
	#endif

	#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
	res2 = proc_create_data("ext_service_ports", 0, &proc_root,
			 &ext_service_ports_proc_fops, NULL);
	#else
	...
	#ifdef CONFIG_RTL_FAST_IPV6
	#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
	res_rtcache6 = proc_create_data("rtl_rtcache6", 0, &proc_root, &rtcache6_proc_fops, NULL);
	#else
	res_rtcache6 = create_proc_entry("rtl_rtcache6", 0, NULL);
	if (res_rtcache6) {
	    res_rtcache6->read_proc = read_rtcache6_proc;
	    res_rtcache6->write_proc = write_rtcache6_proc;
	}
	#endif
	#endif
...
static void __exit fastpath_exit(void)
{
	...
	if (res_udp) {
        #if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
		remove_proc_entry("udp_frag_entry", &proc_root);
		#else
		remove_proc_entry("udp_frag_entry", res_udp);
		#endif
		res_udp = NULL;
	}
	if (res2) {
        #if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
		remove_proc_entry("ext_service_ports", &proc_root);
		#else
	...
	if(res_rtcache6) {
		#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
			remove_proc_entry("rtl_rtcache6", &proc_root);
		#else
			remove_proc_entry("rtl_rtcache6", res_rtcache6);
		#endif
		res_rtcache6 = NULL;
	}
	#endif
...
int Direct_Send_Reply(struct sk_buff * skb, int offset)
{
	...
	skb_push(skb,ETH_HLEN);			//mac header push 
	{
		...
		iph_new->tot_len	=	htons(46);/*46=20+30-sizeof(int),without lenth*/
		iph_new->id 	=	0;
		...
		{		
			memcpy(skb->data+ETH_HLEN+sizeof(struct iphdr), &l2tphone, sizeof(struct Rus_l2tp_ext_hdr)-sizeof(unsigned int));
			skb->len = ETH_HLEN+sizeof(struct iphdr)+sizeof(struct Rus_l2tp_ext_hdr)-sizeof(unsigned int);
		}
		
		//set the ethx device to xmit
		//skb->dev=l2tpInfo.wan_dev;
		rtl_set_skb_dev(skb, l2tpInfo.wan_dev);
		//It is not data packet, need not update last_xmit, otherwise l2tp demand will not disconnect
		//l2tp_info_ptr->last_xmit = jiffies;

		//panic_printk("%s[%d] --\n",__FUNCTION__,__LINE__);
		skb->dev->netdev_ops->ndo_start_xmit(skb,skb->dev);
		//kfree_skb(skb);
		//panic_printk("\n%s:%d l2tp send ppp lcp echo reply!!!\n",__FUNCTION__,__LINE__);
		return 1;		
	}
#else
    return 0;
...
{	
	...
	if(*ppp_protocol == htons(0xc021))
	{
		if( *((u_int8_t *)(((u_int8_t *)ppp_protocol) + 2)) == 0x09)//request
		{
			...
			if(Direct_Send_Reply(skb,38+rightShift)==1)
			return 1;
		}
	}
...
#if defined(CONFIG_RTL_PUBLIC_SSID)
	if(memcmp(rtl_get_skb_dev_name(skb),RTL_GW_WAN_DEVICE_NAME,3) == 0)
#else
#if defined(CONFIG_RTL_819X_SWCORE)
	if(((rtl865x_curOpMode==GATEWAY_MODE && memcmp(rtl_get_skb_dev_name(skb),RTL_PS_WAN0_DEV_NAME,4) == 0) 
			|| (rtl865x_curOpMode==WISP_MODE && memcmp(rtl_get_skb_dev_name(skb), RTL_WLAN_NAME,4) == 0))
		)
#endif
#endif
	{
...
struct ppp_magic_info* alloc_ppp_magic_info(void)
{
	int i;
	for(i=0; i<MAX_PPP_MAGIC_ENTRY; i++)
	{
		if(fast_ppp_magic_info[i].ppp_dev[0]==0 || fast_ppp_magic_info[i].ppp_imagic==0)
		{
			return &fast_ppp_magic_info[i];
		}	
	}
	memset(fast_ppp_magic_info, 0 , sizeof(fast_ppp_magic_info));
	return &fast_ppp_magic_info[0];	
}

void set_ppp_magic_info(char *ppp_device, unsigned int magicNum)
{
	 struct ppp_magic_info *ppp_magic_info_ptr=NULL;
	 ppp_magic_info_ptr=alloc_ppp_magic_info();

	strcpy(ppp_magic_info_ptr->ppp_dev, ppp_device);
	ppp_magic_info_ptr->ppp_imagic=magicNum;	
}

struct ppp_magic_info* find_ppp_magic_info(unsigned int magicNum)
{
	int i;
	for(i=0; i<MAX_PPP_MAGIC_ENTRY; i++)
	{
		if(fast_ppp_magic_info[i].ppp_imagic==magicNum)
		{
			return &fast_ppp_magic_info[i];
		}
	}
