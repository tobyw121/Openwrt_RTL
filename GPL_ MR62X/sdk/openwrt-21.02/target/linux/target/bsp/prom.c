...
void rtw_mempool_zero_region(unsigned long start, unsigned long size) {
	...
	unsigned long vm_end_offset = (unsigned long)_end - KSEG0;
...
#ifdef CONFIG_RTW_MEMPOOL
	rtw_mempool_zero_region(0, mem_size-RTL_COUNTER_DEBUG_MEM_SIZE);
#endif
	rtl_set_mem_addr((KSEG1+mem_size-RTL_COUNTER_DEBUG_MEM_SIZE));
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0)
	memblock_add(0,mem_size);
#else
	add_memory_region(0, mem_size, BOOT_MEM_RAM);
#ifdef CONFIG_RTW_MEMPOOL
	rtw_mempool_zero_region(0, mem_size);
#endif
#endif
#endif
