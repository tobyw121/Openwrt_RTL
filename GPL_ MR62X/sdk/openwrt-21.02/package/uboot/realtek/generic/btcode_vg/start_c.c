...
#undef printf 
#ifdef CONFIG_DECREASE_BOOTSIZE
...
//joyce test
#if defined(CONFIG_DRAM_BOOT_TEST) && !defined(CONFIG_SD_CARD_BOOTING)
...
void rtl8197f_vg_calibration(void)
{

	u4Byte efuse;
	u1Byte do_swr_cal, swr_val, do_ldo_cal, ldo_val;

	/* LX_SWR calibration */
	efuse = read_efuse_byte(0x7F);
	do_swr_cal = (efuse & (0x1 << 1)) >> 1;
	if (do_swr_cal) {
		efuse = read_efuse_byte(0x7E);
		swr_val = efuse & 0xf;
		REG32(0xb8000260) = REG32(0xb8000260) & (~0xf) | swr_val;
	}
	/* DDR_LDO calibration*/
	efuse = read_efuse_byte(0x7D);
	do_ldo_cal = (efuse & (0x1 << 3)) >> 3;
	if (do_ldo_cal) {
		ldo_val = efuse & 0x7;
		REG32(0xb8000280) = REG32(0xb8000280) & (~(0x7 << 3)) | (ldo_val << 3);
	}
