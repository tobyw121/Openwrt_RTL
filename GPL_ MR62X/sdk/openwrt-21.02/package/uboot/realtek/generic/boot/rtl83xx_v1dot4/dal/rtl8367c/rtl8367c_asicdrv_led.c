...
ret_t rtl8367c_setAsicLedGroupEnable(rtk_uint32 group, rtk_uint32 portmask)
{
    ...
    if((data == 0x6511) || (data == 0x0801))
    {
        regAddr = RTL8367C_REG_PARA_LED_IO_EN1 + group/2;
        regDataMask = 0xFF << ((group%2)*8);

        if((regValue & 0xff) == 0x80)
        {
            retVal = rtl8367c_setAsicRegBits(regAddr, regDataMask, (portmask >>1) & 0xff);
            if(retVal != RT_ERR_OK)
                return retVal;
        }
        else if((regValue & 0xff) == 0x60)
        {
            tmp = portmask >> 3;
            portmask = (tmp << 2) | (portmask & 0xf7);
