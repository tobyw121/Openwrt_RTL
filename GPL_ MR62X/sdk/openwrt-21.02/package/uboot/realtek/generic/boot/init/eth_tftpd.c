...
volatile unsigned short block_expected;
int it_is_EOF=0;
#if defined(HTTP_SERVER)
char message[128];
#endif
#if 0//<Coverity: OVERRUN>
char filename[TFTP_DEFAULTSIZE_PACKET];
#else
char filename[TFTP_DEFAULTSIZE_PACKET+1];
#endif
...
int check_image(unsigned long* data_to_transmit);
unsigned short ipheader_chksum(unsigned short*ip,int len);
void tftpd_send_data(char* filename,unsigned short block_number);
void kick_tftpd(void);
/*in misc.c*/
extern void twiddle(void);

extern void prepare_txpkt(int etherport,unsigned short type,unsigned char* destaddr,unsigned char* data ,unsigned short len); 
...
//dprintf("send ARP request\n");
 ...
 prepare_txpkt(0,FRAME_ARP,arp_req.thwaddr,(unsigned char*)&arp_req,(unsigned short)sizeof(arp_req));   
}
...
//dprintf("send TFTP-RRQ request\n");
 ...
 udp = (struct udphdr *)((unsigned char*)&tftp_tx + sizeof(struct iphdr));
 ...
 ip->chksum = ipheader_chksum((unsigned short *)&tftp_tx, sizeof(struct iphdr));
 ...
 prepare_txpkt(0,FRAME_IP,arptable_tftp[TFTP_SERVER].node,(unsigned char*)&tftp_tx,(unsigned short)sizeof(struct iphdr)+sizeof(struct udphdr)+length+2);
    	}                                               
...
/*we have two pairs of mac and ip. let http ack arp first if http's and tftp's ip are same*/
 if(targetIP ==arptable_tftp[HTTPD_ARPENTRY].ipaddr.s_addr )
 {
    ...
    prepare_txpkt(0,FRAME_ARP,arppacket->shwaddr,(unsigned char*)&arpreply,(unsigned short)sizeof(arpreply));	
 }
 else
 //now lock tftp.. 
...
#endif 	
 {
    ...
    prepare_txpkt(0,FRAME_ARP,arptable_tftp[TFTP_CLIENT].node,(unsigned char*)&arpreply,(unsigned short)sizeof(arpreply));
#endif
   } 
}
//----------------------------------------------------------------------------------------
 ...
 if( udpheader->dest==htons(TFTP_PORT) )
   {
    ...
    /*
	if (memcmp(arptable_tftp[TFTP_CLIENT].node,(unsigned char*)&(nic.packet[ETH_ALEN]),ETH_ALEN)) {
    ...
    if (memcmp(&(arptable_tftp[TFTP_CLIENT].ipaddr.s_addr),(unsigned char*)&nic.packet[ETH_HLEN+12],4)) {
    ...
    */
    ...
    memcpy(&(arptable_tftp[TFTP_CLIENT].ipaddr.s_addr),(unsigned char*)&nic.packet[ETH_HLEN+12],4);

    /*memorize CLIENT mac address*/
    memcpy(arptable_tftp[TFTP_CLIENT].node,(unsigned char*)&(nic.packet[ETH_ALEN]),ETH_ALEN);
    ...
    for(find_zero=0;find_zero<TFTP_DEFAULTSIZE_PACKET;find_zero++)
        if( *( (unsigned char*)(tftppacket->u.rrq)+find_zero ) ==0 )
           break;
...
SIGN_T sign_tbl[]={ //  sigature, name, siglen, skip_header, max_len, reboot
        {FW_SIGNATURE, "Linux kernel", SIG_LEN, 0, CONFIG_LINUX_IMAGE_OFFSET_END - CONFIG_LINUX_IMAGE_OFFSET_START,1},
        {FW_SIGNATURE_WITH_ROOT, "Linux kernel (root-fs)", SIG_LEN, 0, CONFIG_LINUX_IMAGE_OFFSET_END - CONFIG_LINUX_IMAGE_OFFSET_START, 1},
        {WEB_SIGNATURE, "Webpages", 3, 0, CONFIG_WEBPAGE_IMAGE_OFFSET_END - CONFIG_WEBPAGE_IMAGE_OFFSET_START, 0},
        {ROOT_SIGNATURE, "Root filesystem", SIG_LEN, 1, CONFIG_ROOT_IMAGE_OFFSET_END - CONFIG_ROOT_IMAGE_OFFSET_START, 0},
#ifdef CONFIG_NAND_FLASH_BOOTING
#ifdef CONFIG_CUSTOMER_BOOT_SIGNATURE_SUPPORT
		{CONFIG_CUSTOMER_BOOT_SIGNATURE, "customer Boot code", SIG_LEN, 0, 0x10000, 1},
#else
		{BOOT_SIGNATURE, "Boot code", SIG_LEN, 0, 0x10000, 1},
#endif
		{IRAM_SIGNATURE, "Iram Boot code", SIG_LEN, 0, 0x10000, 1},
#else
...
int imageFileValid(unsigned long startAddr, int len)
{
	...
	while((head_offset + sizeof(IMG_HEADER_T)) <  len){
		...
		Header.startAddr = ___swab32(Header.startAddr);
		Header.burnAddr = ___swab32(Header.burnAddr);
		//Header.len = ___swab32(Header.len);
		//prom_printf("burnAddr=0x%x startAddr=0x%x len=0x%x\n", Header.burnAddr, Header.startAddr, Header.len);
		...
		if(skip_check_signature || 
			...
			if(!memcmp(Header.signature, ALL1_SIGNATURE, SIG_LEN) ||
				...
				for (i=0; i< Header.len+sizeof(IMG_HEADER_T); i+=2) {
					//sum += ___swab32(*((unsigned short *)(startAddr+ head_offset + i)));
					sum += ___swab16(*((unsigned short *)(startAddr+ head_offset + i)));
				}				
			}	
			else {			
...
void checkAutoFlashing(unsigned long startAddr, int len)
{
	...
	{
		prom_printf("WARNING: fastcksum_hdr->len is updated!!! Old:%d, new:%d, sizeof(FASTCKSUM_PAYLOAD_T):%d, total_bank:%d\n", fastcksum_hdr->len, fastcksum_buf_size-sizeof(FASTCKSUM_HEADER_T), sizeof(FASTCKSUM_PAYLOAD_T), total_bank);
		fastcksum_hdr->len = fastcksum_buf_size-sizeof(FASTCKSUM_HEADER_T);
	}
	FCS_DBG("[%s:%d]fastcksum_hdr:%x, fastcksum_body:%x, fastcksum_hdr->payload:%x\n", __FUNCTION__, __LINE__, fastcksum_hdr, fastcksum_body, fastcksum_hdr->payload);
	...
	/* if iram image, need check first bootcode/second bootcode signature */
#ifdef CONFIG_NAND_FLASH_BOOTING
#ifndef CONFIG_IRAM_BOOT_IN_BLOCK0
	unsigned char iram_exist = 0,boot_exist = 0;
	unsigned long head_offset0 = head_offset;
	while( (head_offset0 + sizeof(IMG_HEADER_T)) <  len){
		memcpy(&Header, ((char *)startAddr + head_offset0), sizeof(IMG_HEADER_T));
			
		if(memcmp(Header.signature,IRAM_SIGNATURE,4) == 0){
			iram_exist = 1;
		}
#ifdef CONFIG_CUSTOMER_BOOT_SIGNATURE_SUPPORT
		else if(memcmp(Header.signature,CONFIG_CUSTOMER_BOOT_SIGNATURE,4) == 0){
#else
		else if(memcmp(Header.signature,BOOT_SIGNATURE,4) == 0){
#endif
			boot_exist = 1;
		}
		...
		if(!(memcmp(Header.signature,IRAM_SIGNATURE,4) == 0
#ifdef CONFIG_CUSTOMER_BOOT_SIGNATURE_SUPPORT
			||	memcmp(Header.signature,CONFIG_CUSTOMER_BOOT_SIGNATURE,4) == 0
#else		
			||	memcmp(Header.signature,BOOT_SIGNATURE,4) == 0
#endif
			))
#endif
		Header.len = ___swab32(Header.len);

		head_offset0 += Header.len + sizeof(IMG_HEADER_T);
	}

	if(	iram_exist == 1 && boot_exist == 0){
		prom_printf("boot image must exist if iram image exist\n");
		goto RETURN;
	}
#endif
#endif
		...
		if(!memcmp(compHeader.signature,(char*)COMP_HS_SIGNATURE,COMP_SIGNATURE_LEN)
			...
			while((head_offset + sizeof(struct compress_mib_header)) < len){
				...
				if(!memcmp(compHeader.signature,(char*)COMP_HS_SIGNATURE,COMP_SIGNATURE_LEN)){
					config_burnLen = ___swab32(compHeader.compLen)+sizeof(struct compress_mib_header);
	...
	//prom_printf("%s %d startAddr=%x len=%d\n", __FUNCTION__, __LINE__, startAddr, len);
	while( (head_offset + sizeof(IMG_HEADER_T)) <  len){
		sum=0; sum1=0;
		memcpy(&Header, ((char *)startAddr + head_offset), sizeof(IMG_HEADER_T));
		...
		if (!skip_check_signature) {
			for(i=0 ;i < MAX_SIG_TBL ; i++) {
				...
				if(!memcmp(Header.signature, (char *)sign_tbl[i].signature, sign_tbl[i].sig_len)) {
					dprintf("[%s] %c%c%c%c startAddr = 0x%08x burnAddr = 0x%08x len = 0x%08x | 0x%08x\n",
					__func__, Header.signature[0], Header.signature[2],
					Header.signature[1], Header.signature[3],
					Header.startAddr, Header.burnAddr, Header.len, len);
					break;
				}
			...
			prom_printf("\n%s upgrade. sh %d head_offset 0x%08x iht 0x%08x\n",
				sign_tbl[i].comment, skip_header, head_offset, sizeof(IMG_HEADER_T));
		}
		else {
			if(!memcmp(Header.signature, BOOT_SIGNATURE, SIG_LEN))
				skip_header = 1;
...
#endif
#else
				sum += ___swab16(*((unsigned short *)(startAddr+ head_offset + sizeof(IMG_HEADER_T) + i)));
		...
		if( (burnLen % 0x1000) == 0) //mean 4k alignemnt
		{
			...
			if( ((*((unsigned char *)(startAddr+burnLen))) == 0xde) &&
			    ((*((unsigned char *)(startAddr+burnLen+1))) == 0xad) &&
			    ((*((unsigned char *)(startAddr+burnLen+2))) == 0xc0) &&
			    ((*((unsigned char *)(startAddr+burnLen+3))) == 0xde)) //wrt jffs2 endof mark
			{	
...
#if defined(CONFIG_RTL_FAST_CHECKSUM_ENABLE) && defined(CONFIG_FLASH_SIZE)
		//correct the fastcksum_body position
#ifdef CONFIG_NAND_FLASH_BOOTING
		if(!(memcmp(Header.signature,IRAM_SIGNATURE,4) == 0
#ifdef CONFIG_CUSTOMER_BOOT_SIGNATURE_SUPPORT
			||	memcmp(Header.signature,CONFIG_CUSTOMER_BOOT_SIGNATURE,4) == 0
...
#endif
		{
			if( do_once==1 )
			{
				//fill in bank info
#ifdef CONFIG_RTL_FLASH_DUAL_IMAGE_ENABLE
				if( (Header.burnAddr+burn_offset)>CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET )
				{
					fastcksum_body++;
			...
			//clear valid bits before writing linux.bin or root.bin
			if( memcmp(Header.signature, FW_SIGNATURE, SIG_LEN)==0 || memcmp(Header.signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN)==0)
			{
				FCS_DBG("[%s:%d] clear LINUX VALID bits!\n", __FUNCTION__, __LINE__);
				fastcksum_body->reserved &= (~RTL_FASTCKSUM_FIELD_LINUX_VALID);
			}else if( memcmp(Header.signature, ROOT_SIGNATURE, SIG_LEN)==0 )
			{
				FCS_DBG("[%s:%d] clear ROOT VALID bits!\n", __FUNCTION__, __LINE__);
				fastcksum_body->reserved &= (~RTL_FASTCKSUM_FIELD_ROOT_VALID);
			}
			dump_fastcksum_struct(fastcksum_hdr);
			//write to flash fast checksum
			if(FASTCKSUM_FAILED==write_fastcksum_to_flash((unsigned char *)fastcksum_hdr, fastcksum_buf_size))
			{
				prom_printf("[%s:%d] write_fastcksum_to_flash() failed, abandon upgrade!!!\n", __FUNCTION__, __LINE__);
				goto RETURN;
			}
		}
#endif
		...
		//clear burn status register of the written bank
		{
			if(!memcmp(Header.signature, FW_SIGNATURE, SIG_LEN) || !memcmp(Header.signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN))
			{
				//do only once, when updating linux.bin
#ifdef CONFIG_RTL_FLASH_DUAL_IMAGE_ENABLE
				if((Header.burnAddr+burn_offset)>CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET)
				{
					prom_printf("[%s:%d] clear bank2 reg to 0\n", __FUNCTION__, __LINE__);
					REG32(BSP_DUMMY_REG_BANK2_BOOTSTATUS) = 0;
				}else
#endif
				{
					prom_printf("[%s:%d] clear bank1 reg to 0\n", __FUNCTION__, __LINE__);
					REG32(BSP_DUMMY_REG_BANK1_BOOTSTATUS) = 0;
				}
			}
		}
#endif
...
int trueorfalse = 0;
		...
		#else
			if(spi_flw_image(0,Header.burnAddr+burn_offset, srcAddr,
				burnLen > 0x1000000 ? 0x1000000 : burnLen))
				trueorfalse = 1;
		#endif
		...
		if (!memcmp(Header.signature, FW_SIGNATURE, SIG_LEN) ||
			!memcmp(Header.signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN)){
 			if(memcmp(Header.burnAddr+burn_offset+(unsigned long)FLASH_BASE,srcAddr,burnLen) != 0){
			...
			if(memcmp(Header.burnAddr+burn_offset+burnLen+(unsigned long)FLASH_BASE,&burn_serial, sizeof(burn_serial))!= 0){
				printf("[%s]%d: linux image burn fail!\n",__func__,__LINE__);
				memset(Header.signature,0,SIG_LEN);
				spi_flw_image_mio_8198(0,Header.burnAddr, &Header, sizeof(Header));
				trueorfalse = 0;
			...
			if(memcmp(Header.burnAddr+burn_offset+(unsigned long)FLASH_BASE,srcAddr,burnLen-SIZE_OF_CHECKSUM) != 0){
			...
			if (padding_len) {
				BDBG_BSN("[%s:%d] pad\n", __FUNCTION__, __LINE__);
				if(memcmp(Header.burnAddr+burn_offset+burnLen-SIZE_OF_CHECKSUM+(unsigned long)FLASH_BASE,(void *)(&rootfs_padding), sizeof(rootfs_padding)) != 0){
					printf("[%s]%d: root image burn fail!\n",__func__,__LINE__);
					memset(Header.signature,0,SIG_LEN);
					spi_flw_image_mio_8198(0,Header.burnAddr, &Header, sizeof(Header));
					trueorfalse = 0;
				...
				BDBG_BSN("[%s:%d] nopad\n", __FUNCTION__, __LINE__);
				if(memcmp(Header.burnAddr+burn_offset+burnLen-SIZE_OF_CHECKSUM+(unsigned long)FLASH_BASE,(void *)(&rootfs_padding)+sizeof(rootfs_padding.zero_pad),
		...
		if (flashwrite(Header.burnAddr+burn_offset, srcAddr, burnLen))
			trueorfalse = 1;
#elif defined(CONFIG_NAND_FLASH_BOOTING)
		if(nflashwrite(Header.burnAddr+burn_offset,srcAddr, burnLen) == 0)
			trueorfalse = 1;
#if defined(CONFIG_RTL_FLASH_DUAL_IMAGE_STATIC)
		prom_printf("write bank2 in static bank mode\n");
		if(nflashwrite(Header.burnAddr+burn_offset+CONFIG_RTL_FLASH_DUAL_IMAGE_OFFSET,srcAddr, burnLen) == 0)
			trueorfalse = 1;
		else
			trueorfalse = 0;
#endif
...
#if defined(CONFIG_RTL_FAST_CHECKSUM_ENABLE) && defined(CONFIG_FLASH_SIZE)
		//fill in fast checksum structure
		{
			unsigned long cksum_addr = 0;
			int tmp_len = 0, valid = 0;

			if( memcmp(Header.signature, FW_SIGNATURE, SIG_LEN)==0 || memcmp(Header.signature, FW_SIGNATURE_WITH_ROOT, SIG_LEN)==0)
			{
				FCS_DBG("linux.bin\n");
				//fill in linux_sig
				memcpy(fastcksum_body->linux_sig, Header.signature, SIG_LEN);
				FCS_DBG("[%s:%d]linux sig:%02x%02x%02x%02x\n", __FUNCTION__, __LINE__, fastcksum_body->linux_sig[0], fastcksum_body->linux_sig[1], 
							fastcksum_body->linux_sig[2], fastcksum_body->linux_sig[3]);
				//fill in linux_cksum
				cksum_addr = srcAddr + burnLen - SIZE_OF_CHECKSUM;
				memcpy((void *)&old_checksum, (void *)cksum_addr, SIZE_OF_CHECKSUM);
				tmp_len = ___swab32(Header.len);	//yes, need to swab back to original Header value, check line#1129
				FCS_DBG("[%s:%d]cksum_addr:0x%x, old_checksum:%x, len:%x\n", __FUNCTION__, __LINE__, cksum_addr, old_checksum, tmp_len);
				if(FASTCKSUM_FAILED==gen_fastcksum(&new_checksum, tmp_len, old_checksum))
				{
					prom_printf("[%s:%d]Generate fastchecksum for rootfs failed!\n", __FUNCTION__, __LINE__);
					goto RETURN;
				}
				fastcksum_body->linux_cksum = new_checksum;
				//fill in valid (linux is burned with IMG_HEADER_T, skip it!)
				if(revalidation(Header.burnAddr+burn_offset+sizeof(IMG_HEADER_T), burnLen-sizeof(IMG_HEADER_T), &valid)==FASTCKSUM_SUCCESS)
				{
					if(valid)
						fastcksum_body->reserved |= RTL_FASTCKSUM_FIELD_LINUX_VALID;
					else
			...
			}else if( memcmp(Header.signature, ROOT_SIGNATURE, SIG_LEN)==0 )
			{
				...
				} *squash_hdr = (struct SQUASH_HDR_T *)srcAddr;
				//fill in root_sig (squashfs header)
				...
				//fill in valid (root is burned without IMG_HEADER_T)
				if(revalidation(Header.burnAddr+burn_offset, burnLen, &valid)==FASTCKSUM_SUCCESS)
				{
					if(valid)
						fastcksum_body->reserved |= RTL_FASTCKSUM_FIELD_ROOT_VALID;
					else
...
#if defined(CONFIG_NAND_FLASH_BOOTING) && defined(CONFIG_IRAM_IMAGE) && defined(CONFIG_IRAM_BOOT_IN_BLOCK0)
		if(!memcmp(Header.signature,IRAM_SIGNATURE,sizeof(Header.signature)))
			head_offset += 0x2800;
		else
			head_offset += Header.len + sizeof(IMG_HEADER_T);
#else
...
#if defined(CONFIG_RTL_FAST_CHECKSUM_ENABLE) && defined(CONFIG_FLASH_SIZE)
	//update fast checksum block
	if(FASTCKSUM_FAILED==write_fastcksum_to_flash((unsigned char *)fastcksum_hdr, fastcksum_buf_size))
	{
		prom_printf("[%s:%d] WARNING: write_fastcksum_to_flash() failed!!!\n", __FUNCTION__, __LINE__);
	}else{
		FCS_DBG("[%s:%d] successfully update fastchecksum\n", __FUNCTION__, __LINE__);
		dump_fastcksum_struct(fastcksum_hdr);
	}
#endif
...
char *get_command_one(char *buffer,unsigned long total_length,unsigned int *i)
{
		...
		{      		  
			...
			if(tftpdata_length < TFTP_DEFAULTSIZE_PACKET)
			{
				...
				if(ret == 1)
				{
					...
					jump_to_test = 1;
					image_address = header.startAddr; //0x80700000
				}
				else 
				{
					prom_printf( "ret=%d\n", ret);		
				...
				if(jump_to_test==1)
				{
					...
					invalidate_cache();
#ifdef CONFIG_PARAM_PASSING
					boot_linux_set_ep(image_address);
					boot_jump_linux();
#else
					jumpF();	
#endif
...
static void prepareDATA(void)
{
	...
	unsigned short tftpopcode;
	unsigned long tftpdata_length;
	unsigned short block_received=0;
	...
	if(udpheader->dest==htons(SERVER_port))
	{
		...
		block_received = ntohs(tftppacket->u.ack.block);
		if(block_received != (block_expected))
		{
			//prom_printf("line=%d: block_received=%d, block_expected=%d\n", __LINE__,  block_received,  block_expected); // for debug
			prom_printf("\n**TFTP #\n");
...
#else
	arptable_tftp[TFTP_SERVER].ipaddr.s_addr = htonl((unsigned int)(IPTOUL(192,168,1,1)));
#endif
...
//prom_printf("line=%d:		block_expected=%d\n", __LINE__,  block_expected); // for debug
 ...
 if (is_client_mode) {
	int start_time;	
	arptable_tftp[TFTP_SERVER].ipaddr.s_addr = htonl((unsigned int)(IPTOUL(192,168,1,97)));	
	arptable_tftp[TFTP_CLIENT].ipaddr.s_addr = htonl((unsigned int)(IPTOUL(192,168,1,116)));
...
#ifdef SUPPORT_TFTP_CLIENT
 if (tftp_client_enabled)
	 prepare_txpkt(0,FRAME_IP,arptable_tftp[TFTP_SERVER].node,(unsigned char*)&tftp_tx,(unsigned short)sizeof(struct iphdr)+sizeof(struct udphdr)+4); 
 else
#endif  
 prepare_txpkt(0,FRAME_IP,arptable_tftp[TFTP_CLIENT].node,(unsigned char*)&tftp_tx,(unsigned short)sizeof(struct iphdr)+sizeof(struct udphdr)+4);
}
//----------------------------------------------------------------------------------------
void tftpd_send_data(char* filename, unsigned short block_number)
{
 ...
 /********************************************/  
   data=(unsigned long *)(image_address+ 512*(block_number-1));
 ...
 memcpy(tftp_tx.u.data.download,(unsigned char*)data,length);
 ...
 prepare_txpkt(0,FRAME_IP,arptable_tftp[TFTP_CLIENT].node,(unsigned char*)&tftp_tx,(unsigned short)sizeof(struct iphdr)+sizeof(struct udphdr)+length+4);
}
...
void kick_tftpd(void)
{
    ...
    unsigned long UDPIPETHheader = ETH_HLEN + sizeof(struct iphdr)  + sizeof(struct udphdr);		 
    
    
    if (nic.packetlen >= ETH_HLEN+sizeof(struct arprequest)) {
    	 //pkttype =( (unsigned short)(nic.packet[12]<< 8)  |(unsigned short)(nic.packet[13])   );   /*This BIG byte shifts right 8*/             
    	 pkttype = *(unsigned short *)(&(nic.packet[12]));
    } 
            ...
            if (nic.packetlen > UDPIPETHheader) {
                ...
                if (ipheader->verhdrlen==0x45) {
                    ...
                    if (ip_addr.s_addr==arptable_tftp[TFTP_SERVER].ipaddr.s_addr 
                        ...
                        //Cyrus Dick
                        if (!ipheader_chksum((unsigned short*)ipheader,sizeof(struct iphdr))) {
