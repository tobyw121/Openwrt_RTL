...
static unsigned char httpd_retok[] = "HTTP/1.1 200 OK\r\n\
					Pragma: no-cache\r\n\
					Cache-Control: no-cache\r\n\
					Connection: close\r\n\r\n";


static unsigned char postCnt_1[]="{\"success\":true,\"errorcode\":\"0\"}";
...
// static unsigned char postCnt_2[]="'; return false;}\n\
...
unsigned long httpd_mem =(IMAGE_MEM_ADDR);
...
#define SIGHEADLEN (4)
#define SIGHEADCNT (4)
static char sighead[SIGHEADCNT][SIGHEADLEN+1]={"r6cr", "boot", "ALL1", "cr6c"};
/******************************************************/
// http server init
static int is_sig_image(char *image)
{
	int i=0;
	for(i=0; i<SIGHEADCNT; i++)
	{
		if (!memcmp(image, sighead[i], SIGHEADLEN))
		{
			prom_printf("match %s\n", sighead[i]);
			return 1;
		}
...
static void writeImagetoflash(char *image,int len)
{
	...
	if(autoBurn)
	{
		...
		{
			nm_tpFirmwareRecovery(image, len); 
		}
 	}
#endif
}
...
static int createResponseData(int *packetlen)
{
	int len=0;
	...
	memcpy(postresponse+len,httpd_retok,sizeof(httpd_retok)-1);
	len+=sizeof(httpd_retok)-1;
	memcpy(postresponse+len,postCnt_1,sizeof(postCnt_1));
	len+=sizeof(postCnt_1)-1;
...
prom_printf(".");
	...
	*/
	...
	if(upload_len >= (content_length+content_offset))
	{
		...
		// if((retVal=imageFileValid((unsigned long)((unsigned char *)httpd_mem),httpd_mem_len)) >0)
		// {
		// 	prom_printf("get rrto fim\n");
		// 	retVal = 1;
		// }
		if (is_sig_image((unsigned char *)httpd_mem) == 1)
		{
			prom_printf("get sig fim\n");
			retVal = 1;
		}
		else
		{
			if (nm_tpFirmwareCheck((unsigned char *)httpd_mem, httpd_mem_len) == 0)
				retVal = 1;
		}
		if (retVal == 1)
		{
			...
			prom_printf("\n#fw check success! addr:0x%x len:0x%d\n", httpd_mem, httpd_mem_len);
			writeImagetoflash((unsigned char *)httpd_mem,httpd_mem_len);
			autoreboot();
		}
		else
...
// just return upgrade html
static int httpdownloadfile()
{
	static int i = 0;
	static int len = 19228; //httpd_retok + upgrade_len
	int sublen = 0;
	unsigned char* httpd_result_buf = (unsigned char*)httpd_mem;
	if (i+900>len)
	{
		sublen = len - i;
	}
	...
	tcpwrite(httpd_result_buf + i,sublen);
	i += sublen;
	if (sublen < 900)
	{
		i=0;
		utcp_info.downloadstarted = 0;
		utcp_info.tcpi_state = TCP_FIN_WAIT1;
		tcpSendFinAck();
	}
	return 1;
...
//	tcpheader->check=htons(inet_chksum_pseudo((char*)tcpheader,(Int32 *)&(ipheader->src.ip[0]),(Int32 *)&(ipheader->dest.ip[0]),IP_TCP,(length-sizeof(struct iphdr))));
	tcpheader->check=(inet_chksum_pseudo((char*)tcpheader,(Int32 *)&(ipheader->src.ip[0]),(Int32 *)&(ipheader->dest.ip[0]),IP_TCP,(length-sizeof(struct iphdr))));
	return;
}
/*BOOT_EVENT0_TCP_SYN*/
...
static int  tcpInputData()
{
	...
	{
		if (utcp_info.downloadstarted == 0)
		{
			/*Get Method*/
			memset(httpd_result_buf, 0, 1024 + upgrade_html_len);
			memcpy(httpd_result_buf, httpd_retok, sizeof(httpd_retok) - 1);
			memcpy(httpd_result_buf + sizeof(httpd_retok) - 1, upgrade_html, upgrade_html_len);
			retval=httpdownloadfile();
			utcp_info.downloadstarted = 1;
		}
		else
		{
			prom_printf("downloading, do nothing\n");
		}
...
//prom_printf("tcpinput start\n");
	...
	// hardware does the L2/L3/L4 checksum.
...
//	}
	...
	if(80==htons(tcpheader->dest))
	{
		...
		for (session_check=0; session_check<2; session_check++) {
