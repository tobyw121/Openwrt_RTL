...
extern int rtl8651_getAsicEthernetPHYReg(uint32 phyId, uint32 regId, uint32 *rData);
extern int rtl8651_setAsicEthernetPHYReg(uint32 phyId, uint32 regId, uint32 wData);
#define MDC_MDIO_READ(P, D, reg, pData) rtl8651_getAsicEthernetPHYReg(CONFIG_RTL_ESW_PHY_ID,reg,pData)
#define MDC_MDIO_WRITE(P, D, reg, data) rtl8651_setAsicEthernetPHYReg(CONFIG_RTL_ESW_PHY_ID,reg,data)
...
rtk_int32 smi_init_4(rtk_uint32 portSCK, rtk_uint32 pinSCK, rtk_uint32 portSDA, rtk_uint32 pinSDA)
{
#if defined(MDC_MDIO_OPERATION) || defined(SPI_OPERATION)
...
{
	return smi_init_4(portSCK, pinSCK, portSDA, pinSDA);
}
#else
rtk_int32 smi_init(rtk_uint32 port, rtk_uint32 pinSCK, rtk_uint32 pinSDA)
{
	return smi_init_4(port, pinSCK, port, pinSDA);
}
#endif
