...
#define PRODUCTINFO_DEBUG_L1(fmt, args...) \
    prom_printf("[PRDCT]%s():%5d @ "fmt, __FUNCTION__, __LINE__, ##args)
#define PRODUCTINFO_DEBUG_L2(fmt, args...) \
    prom_printf("[PRDCT]%s():%5d @ "fmt, __FUNCTION__, __LINE__, ##args)
#define PRODUCTINFO_ERROR(fmt, args...) \
    prom_printf("[PRDCT_ERROR]%s():%5d @ "fmt, __FUNCTION__, __LINE__, ##args)
#define PRODUCTINFO_INFO(fmt, args...) \
	prom_printf("[PRDCT_INFO]%s():%5d @ "fmt, __FUNCTION__, __LINE__, ##args)

#define SYSMGR_PROINFO_CONTENT_MAX_LEN          (0x1000)     /* 4kByte */
...
#define PRODUCTINFO_PRODUCT_VER_LEN         (32)
