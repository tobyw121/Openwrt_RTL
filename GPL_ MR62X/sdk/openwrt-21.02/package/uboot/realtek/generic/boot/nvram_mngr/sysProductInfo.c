    ...
    {    
        ...
        case PRODUCTINFO_PRODUCT_ID:
         ...
         case PRODUCTINFO_HW_ID:
            {
				ret = _strToHwId((char *)productInfo->hwId, argv[1]);
                if (ERROR == ret)
                {
...
void sysmgr_proinfo_structShow(PRODUCT_INFO_STRUCT* productInfo)
{
    if (NULL == productInfo)
    {
       prom_printf("productInfo param is null.\n");
        return;
    }
   prom_printf("--------------------------------------------------------------------\r\n");
...
/*
we define product info when compiling uboot.
no longer fetch info from nvram ptn.
If the product info in NVRAM is lost,we can also excute firmware recovery. 
*/
PRODUCT_INFO_STRUCT * get_fixed_product_info(void)
{
	 char buf[128];	 
	 ...
	 // transfer '_' to ' '.
	 strcpy(product_name,FACTORY_BOOT_PRODUCT_NAME);
	 ...
	 SprintF(buf,"product_name:%s\nproduct_ver:%s\nspecial_id:%s",
	 	product_name,
	 	FACTORY_BOOT_PRODUCT_VER,
	 	FACTORY_BOOT_SPECIAL_ID);

	 PRODUCTINFO_INFO("product-info:%s\n",buf);
	 if (ERROR == sysmgr_proinfo_buildStruct(&g_sysProductInfo, buf, strlen(buf)))
	    {
	        PRODUCTINFO_ERROR("sysmgr_proinfo_buildStruct() failed.\n");
	        return ERROR;
	    }
	 PRODUCTINFO_DEBUG_L1("get_fixed_product_info ok!\n");   
	 return &g_sysProductInfo;
}
#endif
...
void sysmgr_proinfo_show(void)
{
    ...
    {
       prom_printf("failed to get product info from nvram.\n");
        return;
    }
...
sysmgr_cfg_getProductInfoFromNvram(PRODUCT_INFO_STRUCT *productInfo)
{
    ...
    ret = nm_api_readPtnFromNvram(NM_PTN_NAME_PRODUCT_INFO, buf, SYSMGR_PROINFO_CONTENT_MAX_LEN);
    if (ret < 0)
    {
