...
static int rtk_nand_scan_bbt(struct mtd_info *mtd)
{
	...
	for(i=0;i<BACKUP_BBT;i++){
		...
		rc = rtk_block_isbad(mtd,numchips-1,(loff_t)(bbt_page+(i*ppb))*page_size);
		if(!rc){
...
static int rtk_scan_v2r_bbt(struct mtd_info *mtd)
{
	...
	for(i=0;i<BACKUP_BBT;i++){
		...
		page_counter=0;load_bbt_error=0;
		/* COVERITY: OVERFLOW_BEFORE_WIDEN */
		rc = rtk_block_isbad(mtd,numchips-1,(loff_t)(bbt_v2r_page+(i*ppb))*page_size);
		if(!rc){
...
int rtk_remapBBT_write_fail(struct mtd_info* mtd,unsigned int page,const uint8_t *buf,int oob_required)
{
	...
	for ( i=0; i<backup_offset; i++){
		...
		/* if read from bad block fail,skip this page, read next page */
		if(rtk_scan_read_oob(mtd,rtkn->tmpBuf,(block*ppb+i),page_size) < 0)
			continue;
...
int rtkn_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	unsigned int page_size = (1<<this->page_shift);
	unsigned int ppb = (1<<this->phys_erase_shift)/(1<<this->page_shift);
	unsigned int boot_end_page = BOOT_SIZE/page_size;
...
#ifdef CONFIG_RTK_REMAP_BBT
	{
		...
		/* COVERITY : OVERFLOW_BEFORE_WIDEN */
		ofs = ((loff_t)(rtkn->bbt_v2r[i].block_r) << this->phys_erase_shift);//real block index, addr.
	...
	if(page_size != ROM_BOOT_PAGE_SIZE){
...
int nflash_is_bad_block(loff_t offset)
{
	return rtk_block_isbad(&rtkn_mtd_info,0,offset);
}
