...
#define _SHRINK_BOOT_IMAGE_SIZE		1

static const unsigned char explain[] = "pls modify boot code and rebuild again";
...
ret_t rtl8367c_setAsicPortExtMode(rtk_uint32 id, rtk_uint32 mode)
{
    ...
    rtk_uint32 redDataSB[][2] = { {0x04D7, 0x0480}, {0xF994, 0x0481}, {0x2420, 0x0482}, {0x6960, 0x0483}, {0x9728, 0x0484}, {0x9D85, 0x0423}, {0xD810, 0x0424}, {0x83F2, 0x002E} };
    ...
    if (1==type)
    {
#ifdef _SHRINK_BOOT_IMAGE_SIZE
		// for shrinking boot image size
        if((mode == EXT_1000X_100FX) || (mode == EXT_1000X) || (mode == EXT_100FX) ||
            (mode == EXT_SGMII) || (mode == EXT_HSGMII) )
            dprintf("ERROR: %s [%d]: mode= %d, %s\n", __func__,__LINE__,mode,explain);
    ...
    {
#ifdef _SHRINK_BOOT_IMAGE_SIZE
        dprintf("ERROR: %s [%d]: type= %d, %s\n", __func__,__LINE__,type,explain);
#else
        if ((retVal = rtl8367c_setAsicRegBit(RTL8367C_REG_CHIP_RESET, RTL8367C_DW8051_RST_OFFSET, 1)) != RT_ERR_OK)
            return retVal;
...
ret_t rtl8367c_getAsicPortExtMode(rtk_uint32 id, rtk_uint32 *pMode)
{
    ...
    else if (2 == type)
    {
        ...
        else if (2 == id)
        {
            ...
            return rtl8367c_getAsicRegBits(0x13c3, 0xf, pMode);
        }
    }
    else if(3 == type)
    {
...
/* Function Name:
 *      rtl8367c_setFiberAbilityExt
 * Description:
 *      Set external interface serdes pause/asypause ability
 * Input:
 *      id          - external interface id (0~2)
 *      pause    -  pause
 *      asypause - asypause
 ...
 *      None
 */
ret_t rtl8367c_setFiberAbilityExt(rtk_uint32 ext_id, rtk_uint32 pause, rtk_uint32 asypause)
{
    ...
    else if(type == 1)
    {
        if (1 == ext_id)
        {
            if ((retVal = rtl8367c_setAsicRegBit(0x6210, 11, 1))!=RT_ERR_OK)
                    return retVal;

            if ((retVal = rtl8367c_setAsicRegBit(0x6214, 8, asypause))!=RT_ERR_OK)
                    return retVal;

            if ((retVal = rtl8367c_setAsicRegBit(0x6214, 7, pause))!=RT_ERR_OK)
                    return retVal;
        ...
        {
            if ((retVal = rtl8367c_setAsicRegBit(0x6200, 11, 1))!=RT_ERR_OK)
                    return retVal;

            if ((retVal = rtl8367c_setAsicRegBit(0x6204, 8, asypause))!=RT_ERR_OK)
                    return retVal;

            if ((retVal = rtl8367c_setAsicRegBit(0x6204, 7, pause))!=RT_ERR_OK)
                    return retVal;
 ...
 *      None
 */
ret_t rtl8367c_getFiberAbilityExt(rtk_uint32 ext_id, rtk_uint32* pPause, rtk_uint32* pAsypause)
{
    rtk_uint32 retVal, regValue, type;
    ...
    else if(type == 1)
    {
        if (1 == ext_id)
        {
            ...
            if ((retVal = rtl8367c_getAsicRegBit(0x6214, 8, pAsypause))!=RT_ERR_OK)
                    return retVal;

            if ((retVal = rtl8367c_getAsicRegBit(0x6214, 7, pPause))!=RT_ERR_OK)
                    return retVal;
        ...
        else if (2 == ext_id)
        {
            ...
            if ((retVal = rtl8367c_getAsicRegBit(0x6204, 8, pAsypause))!=RT_ERR_OK)
                    return retVal;

            if ((retVal = rtl8367c_getAsicRegBit(0x6204, 7, pPause))!=RT_ERR_OK)
                    return retVal;
