...
struct sheipa_spi g_sheipa_spi;
...
static uint32_t spi_flash_setser(struct sheipa_spi *dev, uint32_t ser_num)
{
	...
	if(ser_num >= spi_flash_para->spi_flash_num_slaves)
		return DW_EINVAL;
	else {
...
// This API implemented for spi calibration
static void dw_spi_set_baudrate(struct spi_device *spi, int baudrate_divider_ocp)
{
	//struct sheipa_spi *dws = spi_master_get_devdata(spi->master);
	struct sheipa_spi *dws = spi->master->driver_data;
	spi_flash_setbaudr(dws, baudrate_divider_ocp);
	spi_flash_wait_busy(dws);
	return ;
}
 ...
 */
static uint32_t dum_cycle_clb = -1;
static uint32_t tunning_dummy_clb = -1;
					...
					uint32_t dum_cycle)
{
	...
	//if(dum_cycle_clb!=-1)
	//	dum_cycle = dum_cycle_clb;
	if(tunning_dummy_clb!=-1)
		tunning_dummy = tunning_dummy_clb;
	else
		tunning_dummy = DEF_RD_TUNING_DUMMY_CYCLE;

	cycle = (cycle*dum_cycle*2) + tunning_dummy;
	if (cycle > 0x10000)
		return DW_ECHRNG;
...
// This API implemented for spi calibration
static void dw_spi_set_dummy_cycle_clb(struct spi_device *spi, uint32_t dum_cycle, uint32_t tunning_dummy)
{
	struct sheipa_spi *dws = spi->master->driver_data;
	struct spi_flash_portmap *spi_flash_map;
	uint32_t cycle;
	...
	cycle = 0;
	spi_flash_map= dws->regs;
	/* Disable SPI_FLASH */
	spi_flash_map->ssienr = 0;

	//dum_cycle_clb = dum_cycle;
	tunning_dummy_clb = tunning_dummy;
	cycle = (spi_flash_map->baudr);
	cycle = (cycle*dum_cycle*2) + tunning_dummy;
...
// This API implemented for spi calibration
static void dw_spi_clb_set_dummy_back(struct spi_device *spi)
{
	struct sheipa_spi *dws = spi->master->driver_data;
#ifndef SPI_AUTO_TUNING_DUMMY
	tunning_dummy_clb = -1;
#endif
	spi_flash_set_dummy_cycle(dws, 0);

}

// This API implemented for spi calibration
static uint32_t dw_spi_get_clb_startaddr(struct spi_device *spi)
{
	return SPI_CLB_TEST_START_ADDR;
}
 ...
 */
static void flash_init(struct sheipa_spi *dev, u8	chip_select)
{
	struct spi_flash_portmap *spi_flash_map;
	spi_flash_map = (struct spi_flash_portmap *)dev->regs;
	/* Set baud_rate */
	...
	/* Set slave(FLASH) num */
	spi_flash_setser(dev, chip_select);
				...
				uint32_t type, uint8_t cmd)
{
	...
	case M25P80_AUTO_WRITE:
		printk(KERN_INFO "INFO: flash mode error, cmd 0x%02X, flash_mode %d\n", cmd, flash_mode);
		break;
	case M25P80_QUAD:
		...
		if (len <= FIFO_HALF_SIZE) {
			...
			if (cnt > 0) {
				if (cnt == 3) {
					memcpy(&data, buf, 2);
					buf += 2;
					flash_write_data(dws, data, DATA_HALF);
					memcpy(&data, buf, 1);
					buf += 1;
					flash_write_data(dws, data, DATA_BYTE);
				}
				else {
					memcpy(&data, buf, cnt);
					buf += cnt;
...
static int dw_spi_transfer(struct spi_master *master, struct spi_message *msg)
{
	...
	struct sheipa_spi *dws = master->driver_data;
	struct spi_flash_param *spi_flash_para = dws->comp_param;
	u16 clk_div = 0;
	...
	spi_flash_setser(dws, spi->chip_select);

	list_for_each_entry(t, &msg->transfers, transfer_list) {

		if(spi->max_speed_hz != spi_flash_para->spi_flash_sclk) {
			if(spi->max_speed_hz > dws->max_freq) {
				printk("spi-sheipa: unsupported freq %dHz\n", spi->max_speed_hz);
				spi->max_speed_hz = 15000000;
			}
			else {
				clk_div = (dws->max_freq % spi->max_speed_hz)? dws->max_freq / spi->max_speed_hz + 1 : dws->max_freq / spi->max_speed_hz;
				spi_flash_setbaudr(dws, clk_div);
			}
			spi_flash_para->spi_flash_sclk = spi->max_speed_hz;
//			printk("spi-sheipa: change speed to %dHz, div %d\n",spi->max_speed_hz, clk_div);
		}
...
static int dw_spi_setup(struct spi_device *spi)
{
	...
	flash_init(dws, spi->chip_select);
	/* auto mode init setting */
//	auto_init(dws);
	return 0;
}

int sheipa_spi_probe(void)
{
	...
	spi = &g_sheipa_spi;
	master->driver_data = spi;
	spi->master = master;

	master->mode_bits = SPI_CPHA | SPI_CPOL;
	master->bus_num = 0;
	master->num_chipselect = 2;  //Maximum chips
	master->setup = dw_spi_setup;
	master->transfer_one_message = dw_spi_transfer;

	//for SPI calibration
	master->set_baudrate = dw_spi_set_baudrate;
	master->set_dummy_cycle_clb = dw_spi_set_dummy_cycle_clb;
	master->get_clb_startaddr = dw_spi_get_clb_startaddr;
	master->clb_set_dummy_back = dw_spi_clb_set_dummy_back;
	
	/* for spi user mode */
	spi->regs = (void *)KSEG1ADDR(BSP_SPIC_BASE);

	/* for auto mode */
	spi->auto_regs = (void *)KSEG1ADDR(BSP_SPIC_AUTO_BASE);
	
	/* spi_sckl = frequency of oc_clk/(2*SCKDV) where frequency of oc_clk is 200 MHz */
	spi->max_freq = 100000000;
	return 0;
}
