...
struct m25p g_m25p_0;
#ifdef CONFIG_SPI_FLASH_NUMBER
struct m25p g_m25p_1;
#endif
...
static int read_sr1(struct m25p *flash)
{
	ssize_t retval;
	u8 code = 0x35;
	u8 val;
 ...
 */
 ...
 int set_4byte(struct m25p *flash, u32 jedec_id, int enable)
{
	int status,value=0;
	...
	case CFI_MFR_ST: /* Micron, actually */
		...
		need_wren = true;
	#if 1//<Coverity: MISSING_BREAK> it's a falls through coding.
		if (need_wren)
			write_enable(flash);
	...
	case 0xEF /* winbond */:
//	case CFI_MFR_GD:
	#if 0//<Coverity: DEADCODE> fix above MISSING_BREAK will cause DEADCODE.
		if (need_wren)
			write_enable(flash);
	#endif
		...
		return status;
	case CFI_MFR_GD:
		value=read_sr1(flash);
		dprintf("\n---%x!\n",value);
		write_enable(flash);
		flash->command[0] = 0x31;
		flash->command[1] =(value|0x4)&0xff;
		status = spi_write(flash->spi, flash->command, 2);

		flash->command[0] = enable ? OPCODE_EN4B : OPCODE_EX4B;
		status = spi_write(flash->spi, flash->command, 1);

	#if 0//<Coverity: DEADCODE> need_wren = false
		if (need_wren)
			write_disable(flash);
	#endif
	...
	size_t *retlen, const u_char *buf)
{
	...
	unsigned int type;
	uint8_t mode;
#if 1//<Coverity: CHECKED_RETURN>
	int ret;
#endif

	dprintf("\x1b[38;5;1m %s to 0x%08x, len %d\x1b[0m\n", __func__, (u32)to, len);
	printk(">");
	spi_message_init(&m);
	memset(t, 0, (sizeof t));
...
static const struct spi_device_id m25p_ids[] = {
	...
	/* EON -- en25xxx */
	{ "en25f16",    INFO(0x1c3115, 0, 64 * 1024,   32, SECT_4K, 41000000, 75000000, 0x0) },
	{ "en25f32",    INFO(0x1c3116, 0, 64 * 1024,   64, SECT_4K, 41000000, 75000000, 0x0) },
	{ "en25p32",    INFO(0x1c2016, 0, 64 * 1024,   64, SECT_4K, 41000000, 75000000, 0x0) },
	{ "en25q16",   INFO(0x1c3015, 0, 64 * 1024,   32, SECT_4K, 41000000, 75000000, 0x0) },
	{ "en25q32b",   INFO(0x1c3016, 0, 64 * 1024,   64, SECT_4K, 41000000, 100000000, 0x0) },
	{ "en25p64",    INFO(0x1c2017, 0, 64 * 1024,  128, SECT_4K, 41000000, 75000000, 0x0) },
	{ "en25q64",    INFO(0x1c3017, 0, 64 * 1024,  128, SECT_4K, 41000000, 100000000, 0x0) },
	{ "en25qh64a",  INFO(0x1c7017, 0, 64 * 1024,  128, 0, 75000000, 100000000, 0x0) },
	{ "en25qh128a",  INFO(0x1c7018, 0, 64 * 1024,  256, 0, 75000000, 100000000, 0x0) },
	{ "en25qh256",  INFO(0x1c7019, 0, 64 * 1024,  512, 0, 41000000, 75000000, 0x0) },

	/* ESMT */
	{ "f25l08qa(2s)", INFO(0x8c4014, 0, 64 * 1024, 16, SECT_4K, 29000000, 41000000, 0x0) },
	{ "f25l16pa(2s)", INFO(0x8c2115, 0, 64 * 1024, 32, SECT_4K, 41000000, 41000000, 0x0) },
	{ "f25l16qa(2s)", INFO(0x8c4015, 0, 64 * 1024, 32, SECT_4K, 41000000, 41000000, 0x0) },
	{ "f25l32pa", INFO(0x8c2016, 0, 64 * 1024, 64, SECT_4K, 29000000, 41000000, 0x0) },
	{ "f25l32qa", INFO(0x8c4016, 0, 64 * 1024, 64, SECT_4K, 29000000, 41000000, 0x0) },
	{ "f25l32qa(2s)", INFO(0x8c4116, 0, 64 * 1024, 64, SECT_4K, 41000000, 41000000, 0x0) },
#endif
	{ "f25l64qa", INFO(0x8c4117, 0, 64 * 1024, 128, SECT_4K, 41000000, 41000000, FLASH_VERIFIED) },
#ifndef CONFIG_DECREASE_BOOTSIZE
	{ "f25l128qa", INFO(0x8c4118, 0, 64 * 1024, 256, SECT_4K, 41000000, 100000000, 0x0) },

	/* Everspin */
	{ "mr25h256", CAT25_INFO( 32 * 1024, 1, 256, 2, M25P_NO_ERASE | M25P_NO_FR, 29000000, 29000000, 0x0) },
	{ "mr25h10",  CAT25_INFO(128 * 1024, 1, 256, 3, M25P_NO_ERASE | M25P_NO_FR, 29000000, 29000000, 0x0) },

	/* GigaDevice */
	{ "gd25q80", INFO(0xc84014, 0, 64 * 1024,  16, SECT_4K, 41000000, 100000000, 0x0) },
	{ "gd25q16", INFO(0xc84015, 0, 64 * 1024,  32, SECT_4K, 41000000, 100000000, 0x0) },
	...
	{ "gd25q256", INFO(0xc84019, 0, 64 * 1024, 512, SECT_4K, 75000000, 100000000, 0x0) },
	...
	{ "mx25l8005",   INFO(0xc22014, 0, 64 * 1024,  16, SECT_4K, 29000000, 41000000, 0x0) },
	{ "mx25l1606e",  INFO(0xc22015, 0, 64 * 1024,  32, SECT_4K, 29000000, 75000000, 0x0) },
	{ "mx25l1633e",  INFO(0xc22415, 0, 64 * 1024,  32, SECT_4K, 29000000, 100000000, 0x0) },
	{ "mx25l1635e",  INFO(0xc22515, 0, 64 * 1024,  32, SECT_4K, 41000000, 100000000, 0x0) },
#endif
	{ "mx25l3205d",  INFO(0xc22016, 0, 64 * 1024,  64, SECT_4K, 29000000, 41000000, FLASH_VERIFIED) },
#ifndef CONFIG_DECREASE_BOOTSIZE
	{ "mx25l3235d",  INFO(0xc25e16, 0, 64 * 1024,  64, SECT_4K, 29000000, 100000000, 0x0) },
	{ "mx25l3255e",  INFO(0xc29e16, 0, 64 * 1024,  64, SECT_4K, 41000000, 100000000, 0x0) },
#endif
	{ "mx25l6405d",  INFO(0xc22017, 0, 64 * 1024, 128, SECT_4K, 29000000, 41000000, FLASH_VERIFIED) },	
	{ "mx25l12805d", INFO(0xc22018, 0, 64 * 1024, 256, SECT_4K, 29000000, 41000000, FLASH_VERIFIED) },	
#ifndef CONFIG_DECREASE_BOOTSIZE
	{ "mx25l12855e", INFO(0xc22618, 0, 64 * 1024, 256, SECT_4K, 41000000, 41000000, 0x0) },
	{ "mx25l25635e", INFO(0xc22019, 0, 64 * 1024, 512, SECT_4K, 41000000, 75000000, 0x0) },
	{ "mx25l25655e", INFO(0xc22619, 0, 64 * 1024, 512, SECT_4K, 41000000, 41000000, 0x0) },
	{ "mx25l51245g", INFO(0xc2201a, 0, 64 * 1024, 1024, 0, 41000000, 100000000, FLASH_VERIFIED) },
	{ "mx66l1g45g", INFO(0xc2201b, 0, 64 * 1024, 2048, 0, 41000000, 100000000, 0x0) },
	
	/* Micron */
	{ "n25q064",     INFO(0x20ba17, 0, 64 * 1024,  128, 0, 41000000, 100000000, FLASH_VERIFIED) },
	{ "n25q128a11",  INFO(0x20bb18, 0, 64 * 1024,  256, 0, 41000000, 100000000, 0x0) },
	{ "n25q128a13",  INFO(0x20ba18, 0, 64 * 1024,  256, 0, 41000000, 100000000, FLASH_VERIFIED) },
	{ "n25q256a",    INFO(0x20ba19, 0, 64 * 1024,  512, SECT_4K, 41000000, 100000000, 0x0) },
	{ "mt25ql512ab", INFO(0x20ba20, 0, 64 * 1024, 1024, 0, 41000000, 100000000, FLASH_VERIFIED) },
	{ "n25q512a",    INFO(0x20bb20, 0, 64 * 1024, 1024, SECT_4K, 41000000, 100000000, 0x0) },
	 ...
	 */
	{ "s25sl032p",  INFO(0x010215, 0x4d00,  64 * 1024,  64, 0, 0, 0, 0x0) },
	{ "s25sl064p",  INFO(0x010216, 0x4d00,  64 * 1024, 128, 0, 0, 0, 0x0) },
	{ "s25fl256s0", INFO(0x010219, 0x4d00, 256 * 1024, 128, 0, 41000000, 100000000, 0x0) },
	{ "s25fl256s1", INFO(0x010219, 0x4d01,  64 * 1024, 512, 0, 41000000, 100000000, 0x0) },
	{ "s25fl512s",  INFO(0x010220, 0x4d00, 256 * 1024, 256, 0, 41000000, 100000000, FLASH_VERIFIED) },
	...
	{ "s25fl129p0", INFO(0x012018, 0x4d00, 256 * 1024,  64, 0, 29000000, 100000000, 0x0) },
	{ "s25fl129p1", INFO(0x012018, 0x4d01,  64 * 1024, 256, 0, 29000000, 100000000, 0x0) },
...
//	{ "s25fl016k",  INFO(0xef4015,      0,  64 * 1024,  32, SECT_4K, 11000000, 11000000, 0x0) },
//	{ "s25fl064k",  INFO(0xef4017,      0,  64 * 1024, 128, SECT_4K, 33000000, 80000000, FLASH_VERIFIED) },
	...
	{ "n25q032", INFO(0x20ba16,  0,  64 * 1024,  64, 0, 41000000, 100000000, 0x0) },
	...
	{ "w25q16", INFO(0xef4015, 0, 64 * 1024,  32, SECT_4K, 41000000, 75000000, 0x0) },
	{ "w25q32", INFO(0xef4016, 0, 64 * 1024,  64, SECT_4K, 41000000, 75000000, 0x0) },
	{ "w25q32dw", INFO(0xef6016, 0, 64 * 1024,  64, SECT_4K, 41000000, 100000000, 0x0) },
	...
	{ "w25q128", INFO(0xef4018, 0, 64 * 1024, 256, 0, 29000000, 41000000, FLASH_VERIFIED) },
#ifndef	CONFIG_DECREASE_BOOTSIZE
	...
	{ "cat25c11", CAT25_INFO(  16, 8, 16, 1, M25P_NO_ERASE | M25P_NO_FR, 1000000, 1000000, 0x0) },
	{ "cat25c03", CAT25_INFO(  32, 8, 16, 2, M25P_NO_ERASE | M25P_NO_FR, 1000000, 1000000, 0x0) },
	{ "cat25c09", CAT25_INFO( 128, 8, 32, 2, M25P_NO_ERASE | M25P_NO_FR, 1000000, 1000000, 0x0) },
	{ "cat25c17", CAT25_INFO( 256, 8, 32, 2, M25P_NO_ERASE | M25P_NO_FR, 1000000, 1000000, 0x0) },
	{ "cat25128", CAT25_INFO(2048, 8, 64, 2, M25P_NO_ERASE | M25P_NO_FR, 4000000, 4000000, 0x0) },
	...
	/* Unknown Vendors info referenced by mx25l3205d */
	{ "unknown", INFO(0xffffff, 0, 64 * 1024,  64, 0, 0, 0, 0x0) },
#else
	/* Unknown Vendors info referenced by mx25l3205d */
	{ "unknown", INFO(0xffffff, 0, 64 * 1024,  64, SECT_4K, 0, 0, 0x0) },
#endif
	{ },
};
...
static const struct spi_device_id *jedec_probe(struct spi_device *spi)
{
	...
	printk("JEDEC id %06x, EXT id 0x%04x\n", jedec, ext_jedec);
	JEDEC_ID=jedec;
	...
	// if unrecognized, use unknown as default
	printk("use JEDEC id 0xffffff (unknown) instead\n");
	default_jedec = 0xffffff;
	for (tmp = 0; tmp < ARRAY_SIZE(m25p_ids) - 1; tmp++) {
		info = (void *)m25p_ids[tmp].driver_data;
 ...
 */
#if 1//<Coverity: CONSTANT_EXPRESSION_RESULT>
u16 get_spi_mode(struct spi_device *spi)
{
	u16 mode = spi->mode;
	return mode;
}
#endif
int m25p_probe(int spi_num)
{
	struct spi_device *spi = &g_spi_device[spi_num];
...
//		flash->mtd.name = dev_name(&spi->dev);
	...
	SPI_SECTOR_SIZE = info->sector_size;
...
//	flash->mtd.dev.parent = &spi->dev;
	...
	max_hz = spi->max_speed_hz;
	spi->max_speed_hz = info->normal_clk;
	...
	spi->max_speed_hz = info->fastrd_clk;
#endif
#endif
	...
	/* If this flash hasn't been verified by ourself, then set the transfer clock a safe value */
//	if(!(info->ex_flags & FLASH_VERIFIED))
//		spi->max_speed_hz = spi->max_speed_hz / 3;

	/* The datasheet doesn't exist, we use default value instead. */
...
#if 0//<Coverity: CONSTANT_EXPRESSION_RESULT>
	if (spi->mode & SPI_RX_QUAD && info->flags & M25P80_QUAD_READ) {
#else
	if (get_spi_mode(spi) & SPI_RX_QUAD && info->flags & M25P80_QUAD_READ) {
#endif
...
*/
	...
	printk("erasesize=%dKB, max_speed_hz=%dHz\n", flash->mtd.erasesize/1024, spi->max_speed_hz);
...
int flashread (unsigned long dst, unsigned int src, unsigned long length)
{
	...
	if (CONFIG_FLASH_NUMBER > 1) {
		if (src >= g_m25p_0.mtd.size)
			ret = g_m25p_1.mtd._read(&g_m25p_1.mtd, src, length, &retlen, (unsigned char *)dst);
		else if (src+length <= g_m25p_0.mtd.size)
			ret = g_m25p_0.mtd._read(&g_m25p_0.mtd, src, length, &retlen, (unsigned char *)dst);
		else if ((src < g_m25p_0.mtd.size)&&(src+length > g_m25p_0.mtd.size)) {
			src_1 = 0;
			length_1 = src + length - g_m25p_0.mtd.size;
			length = g_m25p_0.mtd.size - src;
			ret = g_m25p_0.mtd._read(&g_m25p_0.mtd, src, length, &retlen, (unsigned char *)dst);
			if(0 == ret)
				ret = g_m25p_1.mtd._read(&g_m25p_1.mtd, src_1, length_1, &retlen, (unsigned char *)(dst+length));
		}
	}
	else
		ret = g_m25p_0.mtd._read(&g_m25p_0.mtd, src, length, &retlen, (unsigned char *)dst);
...
static void norFlashAddr(unsigned int uiStart, unsigned int uiLenth, unsigned int uiSectorSize, unsigned int* uiStartAddr, unsigned int*  uiStartLen, unsigned int* uiSectorAddr, unsigned int* uiSectorCount, unsigned int* uiEndAddr, unsigned int* uiEndLen)
{
	unsigned int ui;
	// only one sector
...
int nor_flash_eraseWrite(struct m25p *m25p_type,unsigned int flash_addr_offset,unsigned char *image_addr,unsigned int image_size,int begin)
{
	int retlen, ret;
	struct erase_info er_info;
	unsigned int uiStartAddr, uiStartLen, uiSectorAddr, uiSectorCount, uiEndAddr, uiEndLen;
	unsigned int uiOffset,beginAddr,sectorsize;
	unsigned char *tempBuf=NULL;

	sectorsize = m25p_type->mtd.erasesize;
	uiOffset = flash_addr_offset % sectorsize;
	tempBuf = (unsigned char*)malloc(sizeof(char)*sectorsize);
	if(tempBuf == NULL ){
		printf("error: malloc error before writing nor flash\n");
		return -1;
	}
	norFlashAddr(flash_addr_offset,image_size,sectorsize,&uiStartAddr, &uiStartLen, &uiSectorAddr, &uiSectorCount, &uiEndAddr, &uiEndLen);
	if((uiSectorCount == 0x00) && (uiEndLen == 0x00))	// all data in the same sector
	{
		beginAddr = begin * sectorsize;
		er_info.addr = beginAddr;
		er_info.len = sectorsize;

		ret = m25p_type->mtd._read(&m25p_type->mtd, beginAddr, sectorsize, &retlen, tempBuf);
		if(0 == ret)
		{
			memcpy(tempBuf+uiOffset,image_addr,uiStartLen);	
			ret = m25p_type->mtd._erase(&m25p_type->mtd, &er_info);
			if (0 == ret)
				ret = m25p_type->mtd._write(&m25p_type->mtd, beginAddr, sectorsize, &retlen, tempBuf);	
		}	
		goto FINISH;
	}
	else{
		...
		if(uiSectorCount>0)
		{
			er_info.addr= uiSectorAddr;
			er_info.len = uiSectorCount*sectorsize;
			ret = m25p_type->mtd._erase(&m25p_type->mtd, &er_info);
			if (0 == ret)
			{
				ret = m25p_type->mtd._write(&m25p_type->mtd, er_info.addr, er_info.len, &retlen, image_addr);
				image_addr += er_info.len;
			}
			else
			{
				goto FINISH;
		...
		if(uiEndLen > 0)
		{
			er_info.addr= uiEndAddr;
			er_info.len = sectorsize;

			ret = m25p_type->mtd._read(&m25p_type->mtd, uiEndAddr, sectorsize, &retlen, tempBuf);
			if(0 == ret)
			{
				memcpy(tempBuf,image_addr,uiEndLen);
				ret = m25p_type->mtd._erase(&m25p_type->mtd, &er_info);
				if (0 == ret)
				{
					ret = m25p_type->mtd._write(&m25p_type->mtd, uiEndAddr, sectorsize, &retlen, tempBuf);
				}
			}
		}
		goto FINISH;	
...
int spi_flw_image(unsigned int chip, unsigned int flash_addr_offset ,unsigned char *image_addr, unsigned int image_size)
{
	...
	if(flash_addr_offset+image_size>=(2*(g_m25p_0.mtd.size)))
	{
		printf("Error: Write over flash size 0x%08x\n", (long )(2*(g_m25p_0.mtd.size)));
		return 0;
	}
	...
	if (CONFIG_FLASH_NUMBER > 1) {
		if (flash_addr_offset >= g_m25p_0.mtd.size) {
			flash_addr_offset = flash_addr_offset - g_m25p_0.mtd.size;
			begin = flash_addr_offset / g_m25p_1.mtd.erasesize;
			
			ret = nor_flash_eraseWrite(&g_m25p_1,flash_addr_offset,image_addr,image_size,begin);

		} else if (flash_addr_offset+image_size <= g_m25p_0.mtd.size) {
			begin = flash_addr_offset / g_m25p_0.mtd.erasesize;
			ret = nor_flash_eraseWrite(&g_m25p_0,flash_addr_offset,image_addr,image_size,begin);
			
		} else if ((flash_addr_offset < g_m25p_0.mtd.size)&&(flash_addr_offset+image_size > g_m25p_0.mtd.size)) {
			...
			ret = nor_flash_eraseWrite(&g_m25p_0,flash_addr_offset,image_addr,image_size,begin);
			if(0==ret)
				ret = nor_flash_eraseWrite(&g_m25p_1,flash_addr_offset,image_addr,image_size,begin_1);
...
#ifdef CONFIG_SPI_FLASH_NUMBER
void flash_erase_chip(unsigned int spi_num)
{
	if (spi_num==0)
		erase_chip(&g_m25p_0);
	else if (spi_num==1)
...
void flash_erase_sector(u32 offset)
{
	int ret;
#ifdef CONFIG_SPI_FLASH_NUMBER
	if (CONFIG_FLASH_NUMBER > 1) {
		if (offset < g_m25p_0.mtd.size)
			ret = erase_sector(&g_m25p_0, offset);
		else {
			offset = offset - g_m25p_0.mtd.size;
			ret = erase_sector(&g_m25p_1, offset);
		}
	}
#else
	ret = erase_sector(&g_m25p_0, offset);
...
int flash_erase_sector_clb(uint32_t flash_start_addr, uint32_t erase_length) 
{
	...
	if (erase_length % SPI_SECTOR_SIZE != 0) {
		erase_length =
		erase_length + (SPI_SECTOR_SIZE -
		(erase_length % SPI_SECTOR_SIZE));
	}

/*
	printf("SPI Flash will erase 0x%X bytes from 0x%X to 0x%X\n",
	erase_length, flash_start_addr,
	flash_start_addr + erase_length);
*/
	u32 addr_start = flash_start_addr;
	u32 addr;


	for (sector_erase_cnt = 0;
		sector_erase_cnt < (erase_length / SPI_SECTOR_SIZE);
		sector_erase_cnt++) 
	{
...
void flash_setbaudr(int baudrate_divider_ocp)
{
	//set_baudrate(&g_m25p_0, baudrate_divider_ocp);
	spi_set_baudrate(g_m25p_0.spi, baudrate_divider_ocp);
	return;
}

void flash_set_dummy_cycle_clb(uint32_t dum_cycle, uint32_t tunning_dummy)
{
	spi_set_dummy_cycle_clb(g_m25p_0.spi, dum_cycle, tunning_dummy);
	return;
}

u32 flash_get_jedec_id()
{
	return JEDEC_ID;
}

uint32_t flash_get_clb_startaddr()
{
	return spi_get_clb_startaddr(g_m25p_0.spi);
}

void flash_set_dummy_back()
{
	spi_set_dummy_back(g_m25p_0.spi);
}

#ifndef CONFIG_DECREASE_BOOTSIZE
void flash_set_4byte(u8 enable)
{
	if((&g_m25p_0)->mtd.size > 0x1000000)
		set_4byte(&g_m25p_0,((&g_m25p_0)->dev_info.device_id)<<16,enable);
}
#endif
...
#define ERASE_SIZE		0x1
#define FLASH_SIZE		0x2

int rtk_get_flashInfo(int type,void* value)
{
	switch(type){
		case ERASE_SIZE:
			*(int*)value = g_m25p_0.mtd.erasesize;
			break;
		case FLASH_SIZE:
			*(int*)value = g_m25p_0.mtd.size;
			break;
		default:
			prom_printf("type=%d not support\n",type);
	}
