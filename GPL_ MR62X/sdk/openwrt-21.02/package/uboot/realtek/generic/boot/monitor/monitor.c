...
int CmdMEMDUMP(int argc, char* argv[])
{
	...
	length= strtoul((const char*)(argv[1]), (char **)NULL, 16);		
	error = 0;
	tmpp = (unsigned int*)addr;
	dprintf("addr:%p, lend:%d\n", tmpp, length);
	for(i=0;i<length;i++) {
		dprintf("%x ", tmpp[i]);
		if (i>0 && i%16==0)
		{
...
int CmdPHYregR(int argc, char* argv[])
{
	...
	if (gpio_simulate_mdc_mdio && (phyid == P0_RG_PHYID))
		rtl_mdio_read( phyid, regnum, &tmp );
	else
#endif		
...
#if defined(CONFIG_RTL8197F) && defined(CONFIG_8211F_SUPPORT)
	if (gpio_simulate_mdc_mdio && (phyid == P0_RG_PHYID)) {
		rtl_mdio_write( phyid, regnum, data );
		rtl_mdio_read( phyid, regnum, &tmp );
	} else
#endif	
	{
...
COMMAND_TABLE	MainCmdTable[] =
{
	...
	{ "MEMDUMP",2, CmdMEMDUMP, "MEMDUMP:<addr><length>"},
...
#endif
	{"AUTOBOOT", 0, CmdReboot, "auto reboot "},
#if defined(CONFIG_BOOT_DEBUG_ENABLE)
#if SWITCH_CMD
