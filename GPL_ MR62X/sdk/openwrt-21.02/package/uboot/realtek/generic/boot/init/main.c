...
const char ic_name[] = "RTL8197F-VG";
#else
const char ic_name[] = "RTL8197F";
#endif
...
//-------------------------------------------------------
	...
	*/
	...
	RTL_W32(BSP_GREEN_LED_PIN_MUX_SEL, 
			(RTL_R32(BSP_GREEN_LED_PIN_MUX_SEL) 
			& ~(BSP_GREEN_LED_PIN_MASK) | (BSP_GREEN_LED_PIN_MUX)));

	RTL_W32(BSP_ORANGE_LED_PIN_MUX_SEL, 
			(RTL_R32(BSP_ORANGE_LED_PIN_MUX_SEL) 
			& ~(BSP_ORANGE_LED_PIN_MASK) | (BSP_ORANGE_LED_PIN_MUX)));

	RTL_W32(BSP_GPIO_CNR_REG(BSP_GREEN_LED_PIN), 
			(RTL_R32(BSP_GPIO_CNR_REG(BSP_GREEN_LED_PIN)) 
			& (~(1 << BSP_GPIO_BIT(BSP_GREEN_LED_PIN)))));

	RTL_W32(BSP_GPIO_CNR_REG(BSP_ORANGE_LED_PIN), 
			(RTL_R32(BSP_GPIO_CNR_REG(BSP_ORANGE_LED_PIN)) 
			& (~(1 << BSP_GPIO_BIT(BSP_ORANGE_LED_PIN)))));

	RTL_W32(BSP_GPIO_DIR_REG(BSP_GREEN_LED_PIN), 
			(RTL_R32(BSP_GPIO_DIR_REG(BSP_GREEN_LED_PIN)) 
			| (1 << BSP_GPIO_BIT(BSP_GREEN_LED_PIN))));

	RTL_W32(BSP_GPIO_DIR_REG(BSP_ORANGE_LED_PIN), 
			(RTL_R32(BSP_GPIO_DIR_REG(BSP_ORANGE_LED_PIN)) 
			| (1 << BSP_GPIO_BIT(BSP_ORANGE_LED_PIN))));
...
void rtl8197f_vg_cpu_cal(void)
{
	unsigned int  efuse;
	unsigned char  cpu_val;
	
	efuse = REG32(0xb80007f0);
	if ( ((REG32(0xb8000008) >> 15) & 0xF) == 0xB ){
		if ( (efuse & ( ~ (1<<20) ) ) == (1 << 20) ){
			cpu_val  = (( REG32(0xb8000024) >> 6 ) & 0x3) +1;
			if( cpu_val >= 3 ) cpu_val = 3;
			REG32(0xb8000024) = ( REG32(0xb8000024) & (~(0x3<<6))) | ((cpu_val&0x3)<<6) ;  //0xb8000024[7:6]
...
void showBoardInfo(void)
{
	...
	REG32(0xb8003000) =  REG32(0xb8003000) & (~(1<<21));  

	extern void adj_cpu_freq(void);
	adj_cpu_freq();
#ifdef CONFIG_RTL_8197F_VG
	...
	(*(volatile unsigned int *)(0xb8000008)) &= (~(0x1<<22));
 	...
 	REG32(0xb8000010) =  REG32(0xb8000010) | (1<<11); 
	prom_printf("\n---Realtek %s boot code at %s %s (%dMHz)\n", ic_name, BOOT_CODE_TIME, B_VERSION, cpu_speed);
