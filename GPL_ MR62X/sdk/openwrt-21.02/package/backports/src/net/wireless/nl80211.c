...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	{
		.cmd = NL80211_CMD_WAPI_UPDATE_KEY,
		.validate = GENL_DONT_VALIDATE_STRICT | GENL_DONT_VALIDATE_DUMP,
		.doit = nl80211_send_wapi_update_key,
		.flags = GENL_UNS_ADMIN_PERM,
		.internal_flags = NL80211_FLAG_NEED_NETDEV_UP |
				  NL80211_FLAG_NEED_RTNL,
	},
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
void nl80211_send_wapi_update_key(struct net_device *dev, 
 						const u8 *mac_addr, enum nl80211_key_type key_type, gfp_t gfp)
{
	struct sk_buff *msg;
	void *hdr;
	...
	hdr = nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_WAPI_UPDATE_KEY);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
...
EXPORT_SYMBOL(nl80211_send_wapi_update_key);
#endif
