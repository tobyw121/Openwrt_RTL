...
void phydm_c2h_ra_report_handler(void *dm_void, u8 *cmd_buf, u8 cmd_len)
{
	...
	if (macid < NUM_STAT + 1) {
		PHYDM_DBG(dm, DBG_RA, "427 macid=%d NUM_STAT = %d\n", macid, NUM_STAT);
...
#endif
	PHYDM_DBG(dm, DBG_RA, "466 macid=%d NUM_STAT = %d\n", macid, NUM_STAT);
	if (is_sta_active(sta) && (macid < NUM_STAT + 1)) {
		PHYDM_DBG(dm, DBG_RA, "468 macid=%d NUM_STAT = %d\n", macid, NUM_STAT);
...
u64 phydm_get_bb_mod_ra_mask(void *dm_void, u8 sta_idx)
{
	...
	PHYDM_DBG(dm, DBG_RA_MASK, "macid=%d ori_RA_Mask= 0x%llx\n", sta->mac_id,
		  ra_mask_bitmap);
	PHYDM_DBG(dm, DBG_RA_MASK,
	...
	if (sta->sm_ps == SM_PS_STATIC) /*@mimo_ps_enable*/
		...
		if (sta->iot_peer == HT_IOT_PEER_CMW)
	...
	if (wrls_mode == WIRELESS_CCK) { /*@B mode*/
		...
		{
			if (tx_stream_num == 1)
				ra_mask_bitmap &= 0x000ff000;
			else
				ra_mask_bitmap &= 0x0ffff000;
		}
		else
#endif
		{
		...
		PHYDM_DBG(dm, DBG_RA_MASK, "[Warrning] RA mask is Not found\n");
	}

	PHYDM_DBG(dm, DBG_RA_MASK, "Mod by mode=0x%llx\n", ra_mask_bitmap);

#ifdef MCR_WIRELESS_EXTEND	
	if (sta->iot_peer == HT_IOT_PEER_CMW) {
		if (dm->support_ic_type == ODM_RTL8822B) {
			if (sta->rssi_stat.rssi >= 20)
				ra_mask_bitmap &= 0x0fffffff;
			else if ((sta->rssi_stat.rssi >= 18) && (sta->rssi_stat.rssi <= 19))
				ra_mask_bitmap &= 0x07f7f7ff;
			else if ((sta->rssi_stat.rssi >= 16) && (sta->rssi_stat.rssi <= 17))
				ra_mask_bitmap &= 0x03f3f3ff;
			else if ((sta->rssi_stat.rssi >= 14) && (sta->rssi_stat.rssi <= 15))
				ra_mask_bitmap &= 0x01f1f1f7;
			else if ((sta->rssi_stat.rssi >= 12) && (sta->rssi_stat.rssi <= 13))
				ra_mask_bitmap &= 0x00f0f0f7;
			else if ((sta->rssi_stat.rssi >= 10) && (sta->rssi_stat.rssi <= 11))
				ra_mask_bitmap &= 0x00707073;
			else if ((sta->rssi_stat.rssi >= 7) && (sta->rssi_stat.rssi <= 9))
				ra_mask_bitmap &= 0x00303033;
			else if ((sta->rssi_stat.rssi <= 6))
				ra_mask_bitmap &= 0x00101011;
		} else if (dm->support_ic_type == ODM_RTL8814B) {
			if (sta->rssi_stat.rssi >= 42)
				ra_mask_bitmap &= 0x0fffffff;
			else if ((sta->rssi_stat.rssi >= 40) && (sta->rssi_stat.rssi <= 41))
				ra_mask_bitmap &= 0x07f7f7ff;
			else if ((sta->rssi_stat.rssi >= 38) && (sta->rssi_stat.rssi <= 39))
				ra_mask_bitmap &= 0x03f3f3ff;
			else if ((sta->rssi_stat.rssi >= 34) && (sta->rssi_stat.rssi <= 37))
				ra_mask_bitmap &= 0x01f1f1f7;
			else if ((sta->rssi_stat.rssi >= 30) && (sta->rssi_stat.rssi <= 33))
				ra_mask_bitmap &= 0x00f0f0f7;
			else if ((sta->rssi_stat.rssi >= 27) && (sta->rssi_stat.rssi <= 29))
				ra_mask_bitmap &= 0x00707073;
			else if ((sta->rssi_stat.rssi >= 25) && (sta->rssi_stat.rssi <= 26))
				ra_mask_bitmap &= 0x00303033;
			else if ((sta->rssi_stat.rssi <= 24))
				ra_mask_bitmap &= 0x00101011;
		} else if (dm->support_ic_type == ODM_RTL8814A) {
			if (sta->rssi_stat.rssi >= 38)
				ra_mask_bitmap &= 0x0fffffff;
			else if ((sta->rssi_stat.rssi >= 34) && (sta->rssi_stat.rssi <= 37))
				ra_mask_bitmap &= 0x07f7f7ff;
			else if ((sta->rssi_stat.rssi >= 30) && (sta->rssi_stat.rssi <= 33))
				...
				ra_mask_bitmap &= 0x01f1f1f7;
			else if ((sta->rssi_stat.rssi >= 23) && (sta->rssi_stat.rssi <= 26))
				ra_mask_bitmap &= 0x00f0f0f7;
			else if ((sta->rssi_stat.rssi >= 19) && (sta->rssi_stat.rssi <= 22))
				ra_mask_bitmap &= 0x00707073;
			else if ((sta->rssi_stat.rssi >= 15) && (sta->rssi_stat.rssi <= 18))
				ra_mask_bitmap &= 0x00303033;
			else if ((sta->rssi_stat.rssi <= 14))
				ra_mask_bitmap &= 0x00101011;
		} else if (dm->support_ic_type == ODM_RTL8192E) {
				if (wrls_mode == WIRELESS_CCK) {//B
				if (sta->rssi_stat.rssi >= 27)
					ra_mask_bitmap &= 0x8;
				else if ((sta->rssi_stat.rssi >= 23) && (sta->rssi_stat.rssi <= 26))
					ra_mask_bitmap &= 0x7;
				else if ((sta->rssi_stat.rssi >= 17) && (sta->rssi_stat.rssi <= 22))
					ra_mask_bitmap &= 0x2;
				else if (sta->rssi_stat.rssi <= 16)
					ra_mask_bitmap &= 0x1;
			} else if (wrls_mode == WIRELESS_OFDM) {//G
				if (sta->rssi_stat.rssi > 38)
					ra_mask_bitmap &= 0x800;
				else if ((sta->rssi_stat.rssi >= 34) && (sta->rssi_stat.rssi <= 38))	
					ra_mask_bitmap &= 0x400;
				else if ((sta->rssi_stat.rssi >= 29) && (sta->rssi_stat.rssi <= 33))
					ra_mask_bitmap &= 0x200;
				else if ((sta->rssi_stat.rssi >= 26) && (sta->rssi_stat.rssi <= 28))
					ra_mask_bitmap &= 0x1f0;
				else if ((sta->rssi_stat.rssi >= 23) && (sta->rssi_stat.rssi <= 25))
					ra_mask_bitmap &= 0x0f0;
				else if ((sta->rssi_stat.rssi >= 19) && (sta->rssi_stat.rssi <= 22))
					ra_mask_bitmap &= 0x070;
				else if ((sta->rssi_stat.rssi >= 15) && (sta->rssi_stat.rssi <= 18))
					ra_mask_bitmap &= 0x030;
				else if ((sta->rssi_stat.rssi <= 14))
					ra_mask_bitmap &= 0x010;
			} else if (wrls_mode == (WIRELESS_CCK | WIRELESS_OFDM | WIRELESS_HT)) {//N_24G 
				if (sta->rssi_stat.rssi > 38)
					ra_mask_bitmap &= 0xffff000;
				else if ((sta->rssi_stat.rssi >= 34) && (sta->rssi_stat.rssi <= 38))	
					ra_mask_bitmap &= 0x7f7f000;
				else if ((sta->rssi_stat.rssi >= 29) && (sta->rssi_stat.rssi <= 33))
					ra_mask_bitmap &= 0x3f3f000;
				else if ((sta->rssi_stat.rssi >= 26) && (sta->rssi_stat.rssi <= 28))	
					ra_mask_bitmap &= 0x1f1f000;
				else if ((sta->rssi_stat.rssi >= 23) && (sta->rssi_stat.rssi <= 25))
					ra_mask_bitmap &= 0x0f0f000;
				else if ((sta->rssi_stat.rssi >= 19) && (sta->rssi_stat.rssi <= 22))
					ra_mask_bitmap &= 0x0707000;
				else if ((sta->rssi_stat.rssi >= 15) && (sta->rssi_stat.rssi <= 18))
					ra_mask_bitmap &= 0x0303000;
				else if ((sta->rssi_stat.rssi <= 14))
					ra_mask_bitmap &= 0x0101000;
			}
			...
			if (wrls_mode == WIRELESS_CCK) {//B
				...
				if (sta->rssi_stat.rssi > 23)
					ra_mask_bitmap &= 0x800;
				else if ((sta->rssi_stat.rssi >= 22) && (sta->rssi_stat.rssi <= 23))	
					ra_mask_bitmap &= 0x400;
				else if ((sta->rssi_stat.rssi >= 20) && (sta->rssi_stat.rssi <= 21))
					ra_mask_bitmap &= 0x200;
				else if ((sta->rssi_stat.rssi >= 18) && (sta->rssi_stat.rssi <= 19))
					ra_mask_bitmap &= 0x1f0;
				else if ((sta->rssi_stat.rssi >= 16) && (sta->rssi_stat.rssi <= 17))
...
#if (DM_ODM_SUPPORT_TYPE == ODM_AP)
		if ((priv->pshare->veriwave_sta_num > 0) && (dm->support_ic_type & ODM_IC_JGR3_SERIES)) {
			ra_mask_bitmap &= 0xffffffffffffffff;
		}
		else if (priv->pshare->veriwave_sta_num == 0)
#endif
		{	
		if (rssi_lv == 0)
			ra_mask_bitmap &= 0xffffffffffffffff;
...
#endif	
	PHYDM_DBG(dm, DBG_RA_MASK, "Mod by RSSI=0x%llx\n", ra_mask_bitmap);
...
void phydm_ra_mask_watchdog(void *dm_void)
{
	...
	for (sta_idx = 0; sta_idx < ODM_ASSOCIATE_ENTRY_NUM; sta_idx++) {
		...
		//MCR_WIRELESS_EXTEND		
		PHYDM_DBG(dm, DBG_RA_MASK, "iot_peer = %d\n", sta->iot_peer);
