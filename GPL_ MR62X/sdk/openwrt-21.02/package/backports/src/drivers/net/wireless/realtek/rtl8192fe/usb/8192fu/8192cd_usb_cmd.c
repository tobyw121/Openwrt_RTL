...
u8 callback_request_tx_retry(struct rtl8192cd_priv *priv, struct cmd_header *pcmdhdr);
#endif
...
struct _cmd_callback rtw_cmd_callback[] = {
    ...
    { _CMD_REQUEST_TX_RETRY, callback_request_tx_retry },
#endif
#if defined(SW_ANT_SWITCH)
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (_rtw_queue_empty(&pshare->timer_evt_queue) == FALSE) {
                ...
                if (NULL != timer_event) {
                    ++pshare->nr_timer_evt;
#if defined(CMD_THREAD_FUNC_DEBUG)
...
void notify_request_tx_retry(struct rtl8192cd_priv *priv)
{
    struct cmd_header *pcmdhdr;

    if (test_and_set_bit(_CMD_REQUEST_TX_RETRY, priv->pshare->pending_cmd) != 0)
        return;
    ...
    pcmdhdr->cmd_code = _CMD_REQUEST_TX_RETRY;
    pcmdhdr->cmd_len = sizeof(struct cmd_header);
    pcmdhdr->priv = priv;
...
u8 callback_request_tx_retry(struct rtl8192cd_priv *priv, struct cmd_header *pcmdhdr)
{
    clear_bit(_CMD_REQUEST_TX_RETRY, priv->pshare->pending_cmd);

    requestTxRetry88XX(priv);

    return TRUE;
}
#endif /* defined(TXREPORT) */
...
void notify_IOT_EDCA_switch(struct rtl8192cd_priv *priv, unsigned char enable)
{
...
u8 callback_IOT_EDCA_switch(struct rtl8192cd_priv *priv, struct cmd_header *pcmdhdr)
{
    ...
    extern VOID __IotEdcaSwitch(struct rtl8192cd_priv *priv, IN	unsigned char enable);
	__IotEdcaSwitch(priv, param->enable);
...
#endif /* defined(CONFIG_RTL_WAPI_SUPPORT) */
...
u8 callback_update_sta_msr(struct rtl8192cd_priv *priv, struct cmd_header *pcmdhdr)
{
    ...
    if ((pstat->state & WIFI_ASOC_STATE)
		...
		if (param->opmode & 0x80) {
	        ...
	        GET_HAL_INTERFACE(priv)->UpdateHalMSRRPTHandler(priv, pstat, (param->opmode&0x1));
        if (param->opmode & 0x10)
            pcmdhdr->pstat->txpdrop_flag = ((DECREASE == (param->opmode&0x1)) ? 1:0);
        else
...
#endif /* defined(WIFI_WPAS_CLI) || defined(RTK_NL80211) */
