...
{
	RF_DBG(dm, DBG_RF_LCK, "[RTK]RTK start!!!!!!!\n");
	odm_set_rf_reg(dm, RF_PATH_A, 0xb0, BIT(4), 0x1);
	odm_set_rf_reg(dm, RF_PATH_A, 0xc2, BIT(19), 0x0);
	odm_set_rf_reg(dm, RF_PATH_A, 0xc2, BIT(19), 0x1);
	ODM_delay_ms(1);
	RF_DBG(dm, DBG_RF_LCK, "[RTK]CV= 0x%x\n", odm_get_rf_reg(dm, RF_PATH_A, 0xb2, 0xff));
	RF_DBG(dm, DBG_RF_LCK, "[RTK]RTK end!!!!!!!\n");
}
