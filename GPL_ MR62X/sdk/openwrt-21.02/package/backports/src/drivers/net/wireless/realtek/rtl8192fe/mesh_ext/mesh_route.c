...
#else
		rcu_read_lock();
		p = pid_task(find_vpid(priv->pid_pathsel), PIDTYPE_PID);
		rcu_read_unlock();
#endif
	    if(p)
	    {
...
void triband_set_mesh(void)
{
	extern u32 if_priv_mesh[NUM_WLAN_IFACE];
    struct rtl8192cd_priv *priv0, *priv1, *priv2;
    struct net_device *dev;

    priv0 = (struct rtl8192cd_priv *)if_priv_mesh[0];
    priv1 = (struct rtl8192cd_priv *)if_priv_mesh[1];
    priv2 = (struct rtl8192cd_priv *)if_priv_mesh[2];

#if defined(CONFIG_TRIBAND_MESH_SINGLE_IFACE)
    //priv1->mesh_dev = priv0->mesh_dev;
    priv1->mesh_priv_sc    = priv2;
    priv1->mesh_priv_first = priv0;
#endif

}

inline int triband_get_mesh_priv(struct rtl8192cd_priv *priv, struct rtl8192cd_priv *mesh_priv[])
{
	extern u32 if_priv_mesh[NUM_WLAN_IFACE];

    if (priv == ((struct rtl8192cd_priv *)if_priv_mesh[0])) {
        mesh_priv[0] = (struct rtl8192cd_priv *)if_priv_mesh[0];
        mesh_priv[1] = (struct rtl8192cd_priv *)if_priv_mesh[2];
        mesh_priv[2] = (struct rtl8192cd_priv *)if_priv_mesh[1];
    }
    else if (priv == ((struct rtl8192cd_priv *)if_priv_mesh[1])) {
        mesh_priv[0] = (struct rtl8192cd_priv *)if_priv_mesh[1];
        mesh_priv[1] = (struct rtl8192cd_priv *)if_priv_mesh[2];
        mesh_priv[2] = (struct rtl8192cd_priv *)if_priv_mesh[0];
    }
    else if (priv == ((struct rtl8192cd_priv *)if_priv_mesh[2])) {
        mesh_priv[0] = (struct rtl8192cd_priv *)if_priv_mesh[2];
        mesh_priv[1] = (struct rtl8192cd_priv *)if_priv_mesh[0];
        mesh_priv[2] = (struct rtl8192cd_priv *)if_priv_mesh[1];
    }
    else {
        printk("==> %s:%d, can't handle!!\n", __func__, __LINE__);
        return -1;
    }
    return 0;
}
    ...
    unsigned long flags;
        ...
        for (i=0;i<NUM_WLAN_IFACE;i++) {
            ...
            if(tmp_priv && IS_DRV_OPEN(tmp_priv) && 
                    ...
                    get_stainfo(tmp_priv, pEntry->nexthopMAC)) {
                pEntry->priv = tmp_priv;
                i = 0;
                break;
            }
        }
...
int set_metric_manually(DRV_PRIV *priv,unsigned char *str)
{
	...
	if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable) {
		...
		while (plist != phead) {
			...
			if(!memcmp(pstat->cmn_info.mac_addr,mac_address,MACADDRLEN)) {
