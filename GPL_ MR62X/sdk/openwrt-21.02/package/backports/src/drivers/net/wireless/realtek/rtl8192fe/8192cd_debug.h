...
//
#define DRV_DBG_SERIOUS				2

//
//	Abnormal, rare, or unexpeted cases.
//	For example, IRP/Packet/OID canceled, device suprisely unremoved and so on.
//
#define DRV_DBG_WARNING				3
...
//
#define DRV_DBG_LOUD					4

//
//	Normal case with detail execution flow or information.
//
#define DRV_DBG_TRACE					5


#define DRV_DBG_CONN_INFO 			BIT0
#define DRV_DBG_SYS_INFO 			BIT1
#define DRV_DBG_STATISTICS_INFO		BIT2


#define STR_DRV_DBG_CONN_INFO 			"conn"
#define STR_DRV_DBG_SYS_INFO 			"sys"
#define STR_DRV_DBG_STATISTICS_INFO		"stats"

#define DRV_RT_TRACE(priv, comp, level, fmt, args...) 										\
do {																						\
		if((priv->pshare->rf_ft_var.dbgcomp & (comp)) && (level <= priv->pshare->rf_ft_var.debuglevel || level == DRV_DBG_SERIOUS))										\
		{																									\
			panic_printk("[WiFi][%s %d][%lu][%s][%s]"fmt,__FUNCTION__,__LINE__,priv->up_time,priv->dev->name,STR_##comp,## args);	\
		}																									\
} while (0)
...
extern void drv_init_dbg_setting(
	struct rtl8192cd_priv *priv
);
extern void drv_cmd(
		struct rtl8192cd_priv	*priv,
...
#define __DEBUG_INFO(name, fmt, args...) \
	if (rtl8192cd_debug_info&_MODULE_DEFINE) \
		printk(KERN_INFO "%s-"_MODULE_NAME"-info: " fmt, name, ## args);
#define __DEBUG_WARN(name, fmt, args...) \
	if (rtl8192cd_debug_warn&_MODULE_DEFINE) \
		printk(KERN_WARNING "%s-"_MODULE_NAME"-warn: " fmt, name, ## args);
#endif
#endif
