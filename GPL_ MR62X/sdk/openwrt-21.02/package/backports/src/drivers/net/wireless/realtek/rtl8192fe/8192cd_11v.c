...
void update_bss_preference_zero_preference(struct rtl8192cd_priv *priv)
{
	unsigned char i,j=0,k, pref = 255;
	unsigned char ch_util, own_bss_index;

	for(i = 0; i < MAX_NEIGHBOR_REPORT; i++) {
		if((priv->rm_neighbor_bitmask[i>>3] & (1<<(i&7))) == 0)
			continue;
		if(memcmp(priv->rm_neighbor_report[i].bssid, GET_MY_HWADDR, MACADDRLEN) == 0){
			own_bss_index = i;
			priv->rm_neighbor_report[i].subelemnt.preference = 0;
		}
	}

	for(i = 0; i < MAX_NEIGHBOR_REPORT; i++) {
		if((priv->rm_neighbor_bitmask[i>>3] & (1<<(i&7))) == 0 || i == own_bss_index)
			continue;
		k = 0;
		ch_util = priv->rm_neighbor_info[i].bssload.channel_utilization;
		for(j = 0; j < MAX_NEIGHBOR_REPORT; j++){
			if(((priv->rm_neighbor_bitmask[j>>3] & (1<<(j&7))) != 0)
				&& (i != j) && (j != own_bss_index)){
				if(priv->rm_neighbor_info[j].bssload.channel_utilization < ch_util){
					k++;
				}
			}
		}
		priv->rm_neighbor_report[i].subelemnt.preference = (pref - k);
	}
}
...
void deleteEntryTransitionList(struct rtl8192cd_priv *priv, const unsigned char* macAddress)
{
	...
	for (i = 0 ; i < MAX_TRANS_LIST_NUM; i++) {
		...
		if(0 == memcmp(macAddress, priv->transition_list[i].addr, MACADDRLEN)) {
			...
			DOT11VDEBUG("REMOVE TRANSITION LIST STA (%d) :[%02x][%02x][%02x][%02x][%02x][%02x] \n",
								i, priv->transition_list[i].addr[0], priv->transition_list[i].addr[1], priv->transition_list[i].addr[2],
								priv->transition_list[i].addr[3], priv->transition_list[i].addr[4], priv->transition_list[i].addr[5]);
			priv->transition_list_bitmask[i>>3] &= ~(1<<(i&7));
...
#ifdef RTK_MULTI_AP
unsigned char * construct_self_neighbor_report_ie(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int *frlen, unsigned char pref_val)
{
	...
	*(pbuf + 1) = len;
	...
	neighbor_report.subelemnt.preference = pref_val;
...
unsigned char * construct_bss_termination_ie(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int *frlen){
	...
	se.len = len;
	se.bss_termination_tsf = cpu_to_le64(priv->BssTerminationTSF);
	se.duration = cpu_to_le16(priv->pmib->wnmEntry.dot11vBssTerminationDuration);
...
#else
unsigned char * construct_target_neighbor_report_ie(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int *frlen)
#endif
{
	//int i;
	unsigned int len = 16;
...
void send_bss_trans_event(struct rtl8192cd_priv *priv, struct stat_info *pstat, int i)
{
...
#ifdef RTK_MULTI_AP
    if(issue_BSS_Trans_Req(priv, pstat->cmn_info.mac_addr, 0, NULL, 0) == SUCCESS) //SUCCESS
	{
	...
	/* From priv parameter */
	if(i == -1) {
		if(issue_BSS_Trans_Req(priv, priv->bssTransPara.addr, 0, NULL, 0) == SUCCESS) {
			pstat->bssTransExpiredTime = 0;
			pstat->bssTransTriggered = TRUE;
	...
	/* From transition list */
		...
		if(issue_BSS_Trans_Req(priv, priv->transition_list[i].addr, 0, NULL, 0) == SUCCESS) {
			pstat->bssTransExpiredTime = 0;
			pstat->bssTransTriggered = TRUE;
			priv->startCounting = TRUE;
			priv->dot11vDiassocDeadline = priv->pmib->wnmEntry.dot11vDiassocDeadline;
...
void process_BssTransReq(struct rtl8192cd_priv *priv)
{
	...
	if(priv->pmib->wnmEntry.dot11vBssTermination == 1){
		...
		plist = phead;
		tsf = RTL_R32(TSFTR);
		// send BTM Req to all assoicated clients
		tsf = tsf + (priv->pmib->wnmEntry.dot11vBssTerminationTimer * 1000000); //5 seconds in microseconds
		priv->BssTerminationTSF = tsf;
		while((plist = asoc_list_get_next(priv, plist)) != phead){
			pstat = list_entry(plist, struct stat_info, asoc_list);
			send_bss_trans_event(priv, pstat, 0);
			if (plist == plist->next)
				break;
		}
		priv->pmib->wnmEntry.dot11vBssTermination = 2; // 2 means AP has already sent out the BTM request, only need to wait for TSF
		return;
	}
	else if(priv->pmib->wnmEntry.dot11vBssTermination > 1) {
		return;
	}
...
void BssTermination_ExpiredTimer(struct rtl8192cd_priv *priv)
{
	struct stat_info *pstat;
	struct list_head *phead, *plist;
	#ifndef SMP_SYNC
	...
	plist = phead;
	tsf = RTL_R32(TSFTR);
	// Send disassoc to all associated clients and do Bss Termination
	SAVE_INT_AND_CLI(flags);
	if(tsf >= priv->BssTerminationTSF){
		while((plist = asoc_list_get_next(priv, plist)) != phead){
			pstat = list_entry(plist, struct stat_info, asoc_list);
			panic_printk("issue disassoc for BSS Termination!!\n");
			del_station(priv, pstat, DEL_STA_SEND_DISASSOC, _RSON_DISASSOC_DUE_BSS_TRANSITION);
		...
		priv->BssTerminationDurationCountDown = 60 * priv->pmib->wnmEntry.dot11vBssTerminationDuration; // convert miniute to second
		priv->pmib->wnmEntry.dot11vBssTermination = 3; // 3 means AP enter Bss Termination Status.
	}
	RESTORE_INT(flags);
}

void BssTermination_DurationTimer(struct rtl8192cd_priv *priv)
{
	if(priv->BssTerminationDurationCountDown <= 0){
		RTL_W8(TXPAUSE, RTL_R8(TXPAUSE) & ~STOP_BCN);
		//priv->pmib->dot11DFSEntry.disable_tx = 0;
		priv->pmib->wnmEntry.dot11vBssTermination = 0;
	}
	priv->BssTerminationDurationCountDown--;
}
...
#ifdef RTK_MULTI_AP
			if(pstat->bssTransExpiredTime >= EVENT_TIMEOUT || (priv->transition_list[i].mode & BIT(6) == 0))
#else
...
void set_staBssTransCap(struct stat_info *pstat, unsigned char *pframe, int frameLen, unsigned short ie_offset)
{
	...
	if(p != NULL) {
		ext_cap =  p+2;
		if(ie_len>2 && ext_cap[2] & _WNM_BSS_TRANS_SUPPORT_) {
			pstat->bssTransSupport = TRUE;
//			panic_printk("(%s)line=%d, set normal extended capabilities IE \n", __FUNCTION__, __LINE__);
		}else {	// when there are 2 extended Capabilities IE (IOT issue with Fujitsu)
//			panic_printk("(%s)line=%d, Set 2 extended capabilities IE! \n", __FUNCTION__, __LINE__);
			p = p + ie_len + 2;
...
{	
...
#ifdef RTK_MULTI_AP
	unsigned char allReports, ports, self=0, report_count=0;
...
#endif
#ifdef CONFIG_MBO
	unsigned char non_preferred_ch = 0;
#endif
	...
	if(priv->pmib->wnmEntry.dot11vBssTermination == 1)
		req_mode |= _WNM_BSS_TERMINATION_INCLUDED_;
	...
	if(entry != NULL && memcmp(emptyBssid, entry->target_bssid, MACADDRLEN) != 0)
		allReports = 0;
	else
		allReports = 1; //Empty Bssid provided
#endif
	...
	// 	pstat->dialog_token++;
	if (pstat->dialog_token == 0) {
		pstat->dialog_token++;
	}
	
	DOT11VTRACE("Dialog token: %d\n", pstat->dialog_token);
	...
	if(req_mode & (_WNM_PREFERRED_CANDIDATE_LIST_|_WNM_ABRIDGED_)) {
		...
		if(req_mode & _WNM_BSS_TERMINATION_INCLUDED_){
		...
		if(priv->pmib->wnmEntry.dot11vBssTermination == 1){
			// update the preference of all neighbor reports (set preference 0 to own bss)
			update_bss_preference_zero_preference(priv);
		}
		for(i = 0; i < MAX_NEIGHBOR_REPORT; i++) {
#ifdef RTK_MULTI_AP
			if(allReports == 0)
			...
			if(frlen + neighbor_size > MAX_REPORT_FRAME_SIZE)
				break;
#ifdef CONFIG_MBO
			// check whether the neighbor report belongs to non-preferred channel
			for(k = 0; k < pstat->non_preferred_channels_num; k++){
				if(priv->rm_neighbor_report[i].channel == pstat->non_preferred_channels[k]){
					non_preferred_ch = 1;
					break;
				}
			}
			if(non_preferred_ch){ /* non-preferred channel doesn't need neighbor report (MBO test plan) */
				non_preferred_ch = 0;
				continue;
			}
#endif
			...
			{
				if(memcmp(priv->rm_neighbor_report[i].bssid, preferred_BSSID, MACADDRLEN) || (priv->rm_neighbor_report[i].op_class && (rm_get_op_class(priv,preferred_channel) != priv->rm_neighbor_report[i].op_class)))
					continue;
				else {
...
#endif
#ifdef RTK_MULTI_AP
			if (memcmp(BSSID, priv->rm_neighbor_report[i].bssid, MACADDRLEN) == 0) {
					self = 1;
			}
			//include all bss candidate in neighbor report list
			...
			if(priv->bssTransPara.FromUser) {  /* from set_BssTransPara() */
				if(!memcmp(priv->rm_neighbor_report[i].bssid ,priv->bssTransPara.bssid_mac, MACADDRLEN)) {
					unsigned char tmp_preference = priv->rm_neighbor_report[i].subelemnt.preference;
					priv->rm_neighbor_report[i].subelemnt.preference = MAX_PREFERRED_VAL - priv->bssTransPara.chan_until;
					pbuf = construct_neighbor_report_ie(pbuf, &frlen, &priv->rm_neighbor_report[i]);
					priv->rm_neighbor_report[i].subelemnt.preference = tmp_preference;
					report_found = 1;
...
#endif // DOT11K
#ifdef CONFIG_MBO
		pbuf = construct_btm_req_mbo_ie(priv, pbuf, &frlen);
#endif
#ifndef RTK_MULTI_AP
		if(!report_found && priv->bssTransPara.FromUser)  /* from set_BssTransPara() */
			pbuf = construct_target_neighbor_report_ie(priv, pbuf, &frlen);
#else
		if(allReports && self == 0 && report_count == 0){ //required by multi-band spec to include own bss
			pbuf = construct_self_neighbor_report_ie(priv, pbuf, &frlen, 255);
	}
#endif
	}else {
...
void OnBSSTransReq(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int frame_len)
{
	...
	if(pstat->wnm.req_mode & _WNM_ESS_DIASSOC_IMMINENT_) {
		...
		memcpy(url, pos+1, MIN(pos[0],sizeof(url)));
		url[pos[0]] ='\0';
		pos += 1 + pos[0];
		frlen += 1 + pos[0];
...
process_bss_trans_rsp_code:
	...
	if((frame_len - frlen) > 0) {	
#ifdef RTK_MULTI_AP
		bss_transition_response_notify(BSSID, pstat->cmn_info.mac_addr, &pframe[frlen], status_code);
#endif
		...
		if (resp_entry) {
			DOT11VTRACE("Get Target BSSID from BTM Transition List Entry info\n");
			bss_transition_response_notify(resp_entry->bssid, resp_entry->addr, resp_entry->target_bssid, status_code);
		} else {
			DOT11VTRACE("Unknown Target BSSID\n");
			unsigned char empty_bssid[6] = {0};
...
void OnBSSTransQuery(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char*pframe, int frame_len)
{
    ...
    if(priv->pmib->wnmEntry.Is11kDaemonOn){
		...
		//issue_BSS_Trans_Req(priv, pstat->cmn_info.mac_addr, dialog_token);
		memset(&entry, 0, sizeof(struct target_transition_list));
		memcpy(&entry.addr, pstat->cmn_info.mac_addr, MACADDRLEN);
#ifdef RTK_MULTI_AP
		memcpy(&entry.target_bssid, BSSID, MACADDRLEN);
		entry.opclass = rm_get_op_class(priv, priv->pmib->dot11RFEntry.dot11channel);
		entry.channel = priv->pmib->dot11RFEntry.dot11channel;
#endif
		SAVE_INT_AND_CLI(flags);
		SMP_LOCK_TRANSITION_LIST(flags);
	...
	if (GET_MAP_PROFILE(priv) >= MAP_PROFILE_TWO) {
		rtk_multi_ap_trigger_tunneled_message(pstat->cmn_info.mac_addr, 0x02, frame_len, pframe);
	}
#endif
...
#ifdef CONFIG_MBO
void OnWNMNotificationReq(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char*pframe, int frame_len)
{
	int subelementsLen;
	int i;
	unsigned char *p;
	unsigned char subelementID, subelementLength;
	unsigned char dialog_token = pframe[2];
	unsigned char type = pframe[3];
	unsigned char WFA_OUI[] = {0x50, 0x6f, 0x9a};
	subelementsLen = frame_len - 4;
	p = pframe + 4;

	if(type == _VENDOR_SPEC_IE_){
		while(subelementsLen > 0){
			subelementID = *p;
			subelementLength = *(p + 1);
			if(subelementID == _VENDOR_SPEC_IE_){
				if(!memcmp(p + 2, WFA_OUI, 3) && *(p + 5) == 0x02){ /* WFA oui and oui type is non-preferred channel report */
					if(*(p + subelementLength) == 0){
						...
						for(i = 0; i < (subelementLength - 7); i++){
							pstat->non_operable_channels[i] = *(p + 7 + i);
							pstat->non_operable_channels_num++;
							panic_printk(" %d", pstat->non_operable_channels[i]);
						}
					}
					else if(*(p + subelementLength) == 1){
						...
						for(i = 0; i < (subelementLength - 7); i++){
							pstat->preferred_channels[i] = *(p + 7 + i);
							pstat->preferred_channels_num++;
							panic_printk(" %d", pstat->preferred_channels[i]);
						}
					}
					subelementsLen = subelementsLen - (subelementLength + 2);
					p = p + subelementLength + 2;
				}
				else{
					panic_printk("not WFA oui and non-preferred channel report!\n");
					break;
				}
			}
			else{
				panic_printk("subelement ID is not Vendor Specific!\n");
				break;
			}
		...
		panic_printk("other WNM notification type!\n");

#if defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	if (GET_MAP_PROFILE(priv) >= MAP_PROFILE_TWO) {
		rtk_multi_ap_trigger_tunneled_message(pstat->cmn_info.mac_addr, 0x03, frame_len, pframe);
	}
#endif
}
#endif
...
void WNM_ActionHandler(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int frame_len)
{
	...
	#endif
	#ifdef CONFIG_MBO
		case _WNM_NOTIFICATION_ID_:
			OnWNMNotificationReq(priv, pstat, pframe, frame_len);
			break;
	#endif
		default:
...
unsigned char issue_ROAMING_BSS_Trans_Req(struct rtl8192cd_priv * priv, unsigned char * data)
{
	...
	if(priv->pmib->roamingEntry.roaming_enable)
	{
		...
		if(pstat)
		{
			...
			{
				ret = issue_BSS_Trans_Req(priv, MAC_STA, 0, Bssid, channel);//1:success, 0:fail
			}
		}
