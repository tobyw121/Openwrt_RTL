...
void phydm_get_read_counter_8812f(struct dm_struct *dm)
{
	u32 counter = 0x0;
...
#else
/*Originally config->do_iqk is hooked phy_iq_calibrate_8812F, but do_iqk_8812F and phy_iq_calibrate_8812F have different arguments*/
void do_iqk_8812f(
	void *dm_void,
	u8 delta_thermal_index,
	u8 thermal_value,
...
static u32
_iqk_btc_wait_indirect_reg_ready_8812f(struct dm_struct *dm)
{
	u32 delay_count = 0;
...
static u32
_iqk_btc_read_indirect_reg_8812f(struct dm_struct *dm, u16 reg_addr)
{
	u32 delay_count = 0;

	/* wait for ready bit before access 0x1700 */
	_iqk_btc_wait_indirect_reg_ready_8812f(dm);
...
static void
_iqk_btc_write_indirect_reg_8812f(struct dm_struct *dm, u16 reg_addr,
		       u32 bit_mask, u32 reg_value)
{
	...
	odm_write_4byte(dm, 0x1700, 0xc00F0000 | reg_addr);
		...
		val = _iqk_btc_read_indirect_reg_8812f(dm, reg_addr);
...
void _iqk_set_gnt_wl_high_8812f(struct dm_struct *dm)
{
	u32 val = 0;
	...
	_iqk_btc_write_indirect_reg_8812f(dm, 0x38, 0x3000, val); /*0x38[13:12]*/
	_iqk_btc_write_indirect_reg_8812f(dm, 0x38, 0x0300, val); /*0x38[9:8]*/
}

void _iqk_set_gnt_bt_low_8812f(struct dm_struct *dm)
{
	u32 val = 0;
	...
	/*GNT_BT = 0*/
	val = (sw_control) ? ((state << 1) | 0x1) : 0;
	_iqk_btc_write_indirect_reg_8812f(dm, 0x38, 0xc000, val); /*0x38[15:14]*/
	_iqk_btc_write_indirect_reg_8812f(dm, 0x38, 0x0c00, val); /*0x38[11:10]*/
}

void _iqk_set_gnt_wl_gnt_bt_8812f(struct dm_struct *dm, boolean beforeK)
{
	struct dm_iqk_info *iqk_info = &dm->IQK_info;

	if (beforeK) {
		_iqk_set_gnt_wl_high_8812f(dm);
		_iqk_set_gnt_bt_low_8812f(dm);
	} else {
		_iqk_btc_write_indirect_reg_8812f(dm, 0x38, MASKDWORD, iqk_info->tmp_gntwl);
	}
}



void _iqk_nctl_8812f(struct dm_struct *dm)
{
	...
	// ================= NCTL ============ //
	// nctl_8812F_20180626_wi_iqk_dpk_v1_driver
	odm_write_4byte(dm, 0x1b00, 0x00000008);
...
void _iqk_clean_cfir_8812f(struct dm_struct *dm, u8 mode, u8 path)
{
...
void _iqk_cal_path_off_8812f(struct dm_struct *dm)
{
	u8 path;

	odm_set_bb_reg(dm, 0x1bb8, BIT(20), 0x0);
	for(path = 0; path < SS_8812F; path++) {
...
void _iqk_rf0xb0_workaround_8812f(
	struct dm_struct *dm)
{
...
void _iqk_fill_iqk_report_8812f(
	void *dm_void,
	u8 channel)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
...
void _iqk_iqk_fail_report_8812f(
	struct dm_struct *dm)
{
	u32 tmp1bf0 = 0x0;
	u8 i;
...
void _iqk_backup_rf_8812f(
	struct dm_struct *dm,
	u32 RF_backup[][SS_8812F],
	u32 *backup_rf_reg)
{
	u32 i;

	for (i = 0; i < RF_REG_NUM_8812F; i++) {
...
void _iqk_bb_for_dpk_setting_8812f(struct dm_struct *dm)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]_iqk_bb_for_dpk_setting_8812f!!!!\n");
}

void _iqk_rf_setting_8812f(struct dm_struct *dm)
{	
	odm_set_bb_reg(dm, 0x1bb8, BIT(20), 0x0);
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]_iqk_rf_setting_8812f RF01!!!!\n");
}

void _iqk_afe_setting_8812f(
	struct dm_struct *dm,
	boolean do_iqk)
{
	u8 i;
...
void _iqk_restore_mac_bb_8812f(
	struct dm_struct *dm,
	u32 *MAC_backup,
	u32 *BB_backup,
	u32 *backup_mac_reg,
	...
	u32 temp[][SS_8812F])
{
	...
	/*0xdf[4]=0*/
	//_iqk_rf_set_check_8812f(dm, RF_PATH_A, 0xdf, temp[0][RF_PATH_A] & (~BIT(4)));
	//_iqk_rf_set_check_8812f(dm, RF_PATH_B, 0xdf, temp[0][RF_PATH_B] & (~BIT(4)));
...
void _iqk_reload_iqk_setting_8812f(
	struct dm_struct *dm,
	u8 channel,
	...
	)
{
	...
	for (path = 0; path < SS_8812F; path++) {
		if (reload_idx == 2) {
			...
			_iqk_rf_set_check_8812f(dm, (enum rf_path)path, 0xdf, tmp);
		...
		for (idx = 0; idx < reload_idx; idx++) {
			...
			if (idx == 0)
				odm_set_bb_reg(dm, iqk_apply[path], BIT(31), ~(iqk_info->iqk_fail_report[channel][path][idx]));
			else
				odm_set_bb_reg(dm, iqk_apply[path], BIT(31), ~(iqk_info->iqk_fail_report[channel][path][idx]));
		}
	...
	if (reset) {
		...
		for (i = 0; i < 2; i++) {
			if (iqk_info->rf_reg18 == iqk_info->iqk_channel[i]) {
				_iqk_reload_iqk_setting_8812f(dm, i, 2);
				_iqk_fill_iqk_report_8812f(dm, i);
...
void _iqk_rfe_setting_8812f(
	struct dm_struct *dm)
{
	odm_write_4byte(dm, 0x1840, 0x00707770);
	odm_write_4byte(dm, 0x4140, 0x00007700);
}

void _iqk_setrf_bypath_8812f(
	struct dm_struct *dm)
{
	u8 path;
	u32 tmp;
...
void _iqk_rf_direct_access_8812f(
	struct dm_struct *dm,
	u8 path,
	...
	struct dm_struct *dm)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]_iqk_macbb_8812f!!!!\n");
}

void _iqk_lok_setting_8812f(
	struct dm_struct *dm,
	u8 path,
	u8 idac_bs)
{
	u32 temp;
	_iqk_cal_path_off_8812f(dm);
	//_iqk_bbtx_path_8812f(dm, path);
...
void _iqk_reload_lok_setting_8812f(
	struct dm_struct *dm,
	u8 path)
{
#if 1
...
void _iqk_lok_for_rxk_setting_8812f(
	struct dm_struct *dm,
	u8 path)
{
	_iqk_cal_path_off_8812f(dm);
...
boolean
_iqk_rx_iqk_gain_search_fail_8812f(
	struct dm_struct *dm,
	u8 path,
	u8 step)
{
	...
	if (step == RXIQK1) {
		...
		fail = _iqk_check_cal_8812f(dm, path, 0x1);
	} else if (step == RXIQK2) {
		for (idx = 0; idx < 4; idx++) {
		...
		odm_write_4byte(dm, 0x1b00, IQK_CMD);
		odm_write_4byte(dm, 0x1b00, IQK_CMD + 0x1);
		k2fail = _iqk_check_cal_8812f(dm, path, 0x1);
...
boolean
_lok_check_8812f(void *dm_void, u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	...
	boolean for_rxk)
{
	...
	_iqk_set_gnt_wl_gnt_bt_8812f(dm, true);
	_iqk_rf_direct_access_8812f(dm, (enum rf_path)path, false);
	odm_write_4byte(dm, 0x1b00, IQK_CMD);
	odm_write_4byte(dm, 0x1b00, IQK_CMD + 1);	
	halrf_delay_10us(100);
	_iqk_rf_direct_access_8812f(dm, (enum rf_path)path, true);
	...
	LOK_notready = _iqk_check_cal_8812f(dm, path, 0x0);
	_iqk_set_gnt_wl_gnt_bt_8812f(dm, false);
	if(!for_rxk)
...
//		odm_write_4byte(dm, 0x1b24, (odm_read_4byte(dm, 0x1b24) & 0xffffe3ff) | ((iqk_info->lna_idx & 0x7) << 10));
	...
	if (idx == TXIQK) {
		if (!fail)
			_iqk_backup_iqk_8812f(dm, 0x2, path);
	}
#endif
	if (idx == RXIQK2) {
	...
	if (fail) {
		if (idx == TXIQK)
			_iqk_clean_cfir_8812f(dm, TXIQK, path);
		else if (idx == RXIQK2)
			_iqk_clean_cfir_8812f(dm, RXIQK, path);
	}
...
RF_DBG(dm, DBG_RF_IQK, "[IQK]rx_iqk_step = 0x%x\n", iqk_info->rxiqk_step);
	...
	case 0: //LOK for RXK 
#if 1
		_iqk_lok_for_rxk_setting_8812f(dm, path);
		_lok_one_shot_8812f(dm, path, true);
#endif
		iqk_info->rxiqk_step++;
		break;
	case 1: /*gain search_RXK1*/
#if 0
		_iqk_rxk1_setting_8812f(dm, path);
		gonext = false;
		while (1) {
			KFAIL = _iqk_rx_iqk_gain_search_fail_8812f(dm, path, RXIQK1);
			if (KFAIL && iqk_info->gs_retry_count[0][path][0] < 2)
				iqk_info->gs_retry_count[0][path][0]++;
			else if (KFAIL) {
				iqk_info->rxiqk_fail_code[0][path] = 0;
				iqk_info->rxiqk_step = RXK_STEP_8812F;
				gonext = true;
			} else {
				iqk_info->rxiqk_step++;
				gonext = true;
	...
	case 2: /*RXK1*/
		...
		gonext = false;
		while (1) {
			KFAIL = _iqk_one_shot_8812f(dm, path, RXIQK1);			
...
#if 1
		_iqk_rxk2_setting_8812f(dm, path, true);
		iqk_info->isbnd = false;
		while (1) {
			RF_DBG(dm, DBG_RF_IQK, "[IQK]gs2_retry = %d\n", iqk_info->gs_retry_count[0][path][1]);
			KFAIL = _iqk_rx_iqk_gain_search_fail_8812f(dm, path, RXIQK2);
	...
	case 4: /*RXK2*/
#if 1
		_iqk_rxk2_setting_8812f(dm, path, false);
		gonext = false;
		while (1) {
			KFAIL = _iqk_one_shot_8812f(dm, path, RXIQK2);			
...
#if 0
		_iqk_reload_lok_setting_8812f(dm, path);
		iqk_info->rxiqk_step++;
#else
		iqk_info->rxiqk_step++;
#endif
...
void _iqk_lok_tune_8812f(void *dm_void, u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u8 idac_bs = 0x4;

	while (1) {
		_iqk_lok_setting_8812f(dm, path, idac_bs);
		_lok_one_shot_8812f(dm, path, false);	
		RF_DBG(dm, DBG_RF_IQK, "[IQK]ibs = %d\n", idac_bs);
		if(!_lok_check_8812f(dm, path)) {	
			if(idac_bs == 0x6)
				break;
			else
				idac_bs++;
...
boolean
_lok_load_default_8812f(void *dm_void, u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct dm_iqk_info *iqk_info = &dm->IQK_info;
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	boolean segment_iqk)
{
...
#if 1
		while (1) {
			KFAIL = _iqk_rx_iqk_by_path_8812f(dm, RF_PATH_A);
			RF_DBG(dm, DBG_RF_IQK, "[IQK]S0RXK KFail = 0x%x\n", KFAIL);
			if (iqk_info->rxiqk_step == RXK_STEP_8812F) {
				iqk_info->iqk_step++;
	...
	case 1: /*S0 LOK*/
#if 1
		_iqk_lok_tune_8812f(dm, RF_PATH_A);
		if(_lok_load_default_8812f(dm, RF_PATH_A))
	...
	case 2: /*S0 TXIQK*/
#if 1
		_iqk_txk_setting_8812f(dm, RF_PATH_A);
		KFAIL = _iqk_one_shot_8812f(dm, RF_PATH_A, TXIQK);
...
#if 1
		while (1) {
			KFAIL = _iqk_rx_iqk_by_path_8812f(dm, RF_PATH_B);
	...
	case 4: /*S1 LOK*/
#if 1
		_iqk_lok_tune_8812f(dm, RF_PATH_B);
		if(_lok_load_default_8812f(dm, RF_PATH_B))
			RF_DBG(dm, DBG_RF_IQK, "[IQK]S1 Load LOK to default\n");
	...
	case 5: /*S1 TXIQK*/
#if 1
		_iqk_txk_setting_8812f(dm, RF_PATH_B);
		KFAIL = _iqk_one_shot_8812f(dm, RF_PATH_B, TXIQK);
		iqk_info->kcount++;
	...
	if (iqk_info->iqk_step == IQK_STEP_8812F) {
	...
	boolean segment_iqk)
{
	...
	while (i <  100) {
		_iqk_iqk_by_path_8812f(dm, segment_iqk);
		if (iqk_info->iqk_step == IQK_STEP_8812F)
			break;
	...
	struct dm_struct *dm)
{
	...
	if (firstrun) {
		firstrun = false;
		RF_DBG(dm, DBG_RF_IQK, "[IQK]=====>PHY_IQCalibrate_8812f_Init\n");
...
u32 _iqk_tximr_selfcheck_8812f(
	void *dm_void,
	u8 tone_index,
	u8 path)
{
...
void _iqk_start_tximr_test_8812f(
	struct dm_struct *dm,
	u8 imr_limit)
{
	boolean KFAIL;
	...
	for (path = 0; path < SS_8812F; path++) {
		_iqk_txk_setting_8812f(dm, path);
		KFAIL = _iqk_one_shot_8812f(dm, path, TXIQK);
		for (i = 0x0; i < imr_limit; i++) {
			tone_index = (u8)(0x08 | i << 4);
			imr_result = _iqk_tximr_selfcheck_8812f(dm, tone_index, path);
...
u32 _iqk_rximr_selfcheck_8812f(
	void *dm_void,
	u32 tone_index,
	u8 path,
	u32 tmp1b38)
		...
		case 3: /*get RXK1 IQC*/
			...
			for (side = 0; side < 2; side++) {
				for (i = 0; i < imr_limit; i++) {
					...
					while (count < 3) {
						_iqk_rxk1_setting_8812f(dm, path);
						kfail = _iqk_rximr_rxk1_test_8812f(dm, path, tone_index);
						RF_DBG(dm, DBG_RF_IQK, "[IQK]path = %x, kfail = %x\n", path, kfail);
						if (kfail) {
							count++;
							if (count == 3) {
			...
			for (side = 0; side < 2; side++) {
				for (i = 0x0; i < imr_limit; i++) {
					if (side == 0)
						...
						tone_index = 0x08 | (i << 4);
					_iqk_rxk2_setting_8812f(dm, path, false);
					imr_result = _iqk_rximr_selfcheck_8812f(dm, tone_index, path, temp1b38[side][i]);
...
void _iqk_start_rximr_test_8812f(
	struct dm_struct *dm,
	u8 imr_limit)
{
	u8 path;
	...
	for (path = 0; path < SS_8812F; path++)
		_iqk_rximr_test_8812f(dm, path, imr_limit);
}

void _iqk_start_imr_test_8812f(
	void *dm_void)
{
	u8 imr_limit;
	...
	if (*dm->band_width == 2)
		...
		imr_limit = 0x4;
	//	_iqk_start_tximr_test_8812f(dm, imr_limit);
	_iqk_start_rximr_test_8812f(dm, imr_limit);
}



void _phy_iq_calibrate_8812f(
	struct dm_struct *dm,
	boolean reset,
	boolean segment_iqk)
{
	u32 MAC_backup[MAC_REG_NUM_8812F], BB_backup[BB_REG_NUM_8812F], RF_backup[RF_REG_NUM_8812F][SS_8812F];
	u32 backup_mac_reg[MAC_REG_NUM_8812F] = {0x520, 0x550, 0x1c};
	u32 backup_bb_reg[BB_REG_NUM_8812F] = {
		...
		R_0x41a4, 0x1840, 0x4140};

	u32 backup_rf_reg[RF_REG_NUM_8812F] = {0x19, 0xdf, 0x9e};
	boolean is_mp = false;
	u8 i = 0;
...
#if 0
	if (!is_mp)
		if (_iqk_reload_iqk_8812f(dm, reset))
			return;
#endif
	...
	iqk_info->iqk_step = 0;
	iqk_info->rxiqk_step = 0;
	iqk_info->tmp_gntwl = _iqk_btc_read_indirect_reg_8812f(dm, 0x38);
	
	_iqk_information_8812f(dm);
	_iqk_backup_iqk_8812f(dm, 0x0, 0x0);
	_iqk_backup_mac_bb_8812f(dm, MAC_backup, BB_backup, backup_mac_reg, backup_bb_reg);
	_iqk_backup_rf_8812f(dm, RF_backup, backup_rf_reg);

	while (i < 3) {
		i++;
		_iqk_macbb_8812f(dm);		
		//odm_read_and_config_mp_8812f_cal_init(dm);
		//_iqk_rf_setting_8812f(dm);
		_iqk_bb_for_dpk_setting_8812f(dm);
		_iqk_afe_setting_8812f(dm, true);
		_iqk_rfe_setting_8812f(dm);
		_iqk_agc_bnd_int_8812f(dm);
		//_iqk_con_tx_8812f(dm, true);
		_iqk_start_iqk_8812f(dm, segment_iqk);
		//_iqk_con_tx_8812f(dm,false);
		_iqk_afe_setting_8812f(dm, false);
		_iqk_restore_rf_8812f(dm, backup_rf_reg, RF_backup);
		_iqk_restore_mac_bb_8812f(dm, MAC_backup, BB_backup, backup_mac_reg, backup_bb_reg);
		if (iqk_info->iqk_step == IQK_STEP_8812F)
			break;
		iqk_info->kcount = 0;
...
//	if (segment_iqk)
//		_iqk_reload_iqk_setting_8812f(dm, 0x0, 0x1);

	//_iqk_fill_iqk_report_8812f(dm, 0);
//	_iqk_rf0xb0_workaround_8812f(dm);
	RF_DBG(dm, DBG_RF_IQK, "[IQK]==========IQK end!!!!!==========\n");
}

void _check_fwiqk_done_8812f(struct dm_struct *dm)
{
	u32 counter = 0x0;
#if 1
	while (1) {
...
void _phy_iq_calibrate_by_fw_8812f(
	void *dm_void,
	u8 clear,
	u8 segment_iqk)
{
	...
	boolean segment_iqk)
{
	...
	if (dm->fw_offload_ability & PHYDM_RF_IQK_OFFLOAD) {
		_phy_iq_calibrate_by_fw_8812f(dm, clear, (u8)(segment_iqk));
		_check_fwiqk_done_8812f(dm);
		_iqk_check_if_reload(dm);
		RF_DBG(dm, DBG_RF_IQK, "!!!!!  FW IQK   !!!!!\n");
	} else {
		_iq_calibrate_8812f_init(dm);
		_phy_iq_calibrate_8812f(dm, clear, segment_iqk);
	}
	_iqk_fail_count_8812f(dm);
	if (dm->mp_mode)
		if (*dm->mp_mode)
			halrf_iqk_hwtx_check(dm, false);
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	_iqk_iqk_fail_report_8812f(dm);
#endif
	halrf_iqk_dbg(dm);
}
