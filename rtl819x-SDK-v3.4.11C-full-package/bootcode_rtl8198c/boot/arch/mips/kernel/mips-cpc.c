 ...
 *
 * smp-cpc.c
 *     MIPS SMP cluster power control code
 *
...
static void mips_gcmp_request(int core)
{
	unsigned int type;
	int i;
...
static void mips_gcmp_start(void)
{
	...
	for (i = 0; i < niocus; i++)
		mips_gcmp_request(4+i);
}

void mips_cpc_setup(void)
{
	/* If the CPC is not present, return */
	if (!GCMPGCB(CPCSR) & GCMP_GCB_CPCSR_EX_MSK)
		return;
