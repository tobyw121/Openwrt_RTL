...
static unsigned int __val(int val)
{
	return val;
}
#endif
...
#else
			while( (REG32(SYS_LX_CTRL)&(1<<12))==(__val(1<<0))) {};  //wait bit to 0  unlock
#endif
...
struct irqaction irq_SPEED = {(void *)SPEED_isr, (unsigned long)NULL, (unsigned long)SPEED_IRQ_NO,"SPEED", (void *)NULL, (struct irqaction *)NULL};   
...
int LetCPUDoSomething()
{
	...
	p=(unsigned int *)((unsigned int)buf|0xa0000000);
	for(i=0; i<256; i++)
		p[i]=i;
...
#if 0//<Coverity: CONSTANT_EXPRESSION_RESULT>
			while( (get_reg32(SYS_LX_CTRL)&(1<<13))==(1<<1)) ;  //wait bit to 0  unlock
#else
			while( (REG32(SYS_LX_CTRL)&(1<<13))==(__val(1<<1))) ;  //wait bit to 0  unlock
#endif		
...
#else
			while( (REG32(SYS_LX_CTRL)&(1<<14))==(__val(1<<2))) ;	//wait bit to 0  unlock
#endif
		#endif
...
unsigned int ExtractACmdLine(const char *pPattScript,  char *pOneCmdString, int first)
{
	...
	unsigned char *p=(unsigned char *)pPattScript+idx;
	int push=0;
