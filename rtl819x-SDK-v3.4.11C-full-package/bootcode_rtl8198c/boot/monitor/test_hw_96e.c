...
int CmdTestSRAM(int argc, char* argv[])
{
	...
	addr = (unsigned char *)(strtoul((const char*)(argv[0]), (char **)NULL, 16));

	unsigned char *paddr=(unsigned char *)(VIR2PHY(addr));
	unsigned char  *n8addr=(unsigned char *)(VIR2NOC(addr));
	unsigned short *n16addr=(unsigned short *)(VIR2NOC(addr));
	unsigned int   *n32addr=(unsigned int *)(VIR2NOC(addr));
	
	//===============================================
	//stage 1: r/w
	memset(n8addr, 0xff, len);
#if 1
	Enable_SRAM((unsigned int)paddr);
#endif
	//-----------------------------------------------------
	//test fix pattern
