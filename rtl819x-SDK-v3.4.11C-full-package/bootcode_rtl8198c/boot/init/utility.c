...
#ifdef CONFIG_NAND_FLASH_BOOTING
	nflashwrite(flash_addr,data,len);
#endif
}
...
#if 0 /*<Coverity: DEADCODE> "button_press_detected > 0" cannot be true.
		do {
		...
		button_press_detected=ret;
		if(ret > 0) return ret; <<< already return here
		}while
	  */
...
#ifdef CONFIG_NAND_FLASH_BOOTING
	#ifndef NAND_FLASH_BOOT_SPEEDUP
	val = nflashread( pheader->startAddr|0x20000000,(unsigned int)(addr-FLASH_BASE+sizeof(IMG_HEADER_T)),pheader->len-2,0);
	#endif
#else
	val = flashread( pheader->startAddr|0x20000000,	(unsigned int)(addr-FLASH_BASE+sizeof(IMG_HEADER_T)), pheader->len-2);
#endif
#endif		
...
void initFlash(void)
{
   	...
   	spi_probe();                                  
		...
		if(rtk_scan_v2r_bbt() < 0)
			return;
		if(rtk_nand_scan_bbt() < 0)
			return;
#endif
...
void doBooting(int flag, unsigned long addr, IMG_HEADER_Tp pheader)
{
...
#endif
	{
		...
		default:
			...
			//fix some abnormal interrupt during booting to kernel
			 REG32(0xbbdc0300)=0xffffffff;
	                 REG32(0xbbdc0304)=0xffffffff;
			 REG32(0xbbdc0400)=0xffffffff;
                         REG32(0xbbdc0404)=0xffffffff;
