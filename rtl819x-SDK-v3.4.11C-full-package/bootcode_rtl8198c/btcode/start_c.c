...
#define xprintf(x,...) 
	...
	unsigned char csum;
	unsigned char *ptr=(unsigned char *)JUMPADDR;
	...
	while(retry > 0)
	{
		...
		{	
			...
			#if 0//<Coverity: DEADCODE> i = packet_size, the condition "i < 128" cannot be true
...
#if (DDR3_CAS10_MAX_MHZ != DDR3_CAS9_MAX_MHZ)//<Coverity: DEADCODE> DDR3_CAS10_MAX_MHZ = DDR3_CAS9_MAX_MHZ
...
#if (DDR3_CAS8_MAX_MHZ != DDR3_CAS7_MAX_MHZ)//<Coverity: DEADCODE> DDR3_CAS8_MAX_MHZ = DDR3_CAS7_MAX_MHZ
...
#if (DDR3_CAS6_MAX_MHZ != DDR3_CAS5_MAX_MHZ)//<Coverity: DEADCODE> DDR3_CAS6_MAX_MHZ = DDR3_CAS5_MAX_MHZ
...
static void __set_DTR_DDR3_MRS_setting(unsigned int *rtt_nom, unsigned int *dic_value, unsigned int *rtt_wr)
{
#if 1 //350MHZ OK
...
void _DTR_DDR3_MRS_setting(unsigned int *sug_dtr, unsigned int *mr)
{
	...
	#endif
#else
	__set_DTR_DDR3_MRS_setting(&rtt_nom, &dic_value, &rtt_wr);
#endif
 ...
 *  	None
 */
#if 0//<Coverity: OVERRUN> /* the min dram_freq_mhz is 250, but the max DQS_RANGE[index].mhz is 200
				,i.e. forloop will not break, index will be (sizeof(DQS_RANGE)/sizeof(tap_info_t)) after forloop
				, in statement if(dqs0_tap < DQS_RANGE[index].tap_min || dqs0_tap > DQS_RANGE[index].tap_max \
					|| dqs1_tap < DQS_RANGE[index].tap_min || dqs1_tap > DQS_RANGE[index].tap_max) 
				, the index is overrun!!
	
				no one refer the function, comment out it! 
			*/
 ...
 *  	None
 */
#if 0//<Coverity: OVERRUN> /* the min dram_freq_mhz is 250, but the max W90_RANGE[index].mhz is 200
			,i.e. forloop will not break, index will be (sizeof(W90_RANGE)/sizeof(tap_info_t)) after forloop
			, in statement if(phase_tap < W90_RANGE[index].tap_min || phase_tap > W90_RANGE[index].tap_max), 
			the index is overrun!!
...
#if 1//<Coverity: DEADCODE>
static void __set_memctlc_ddr2_dll_reset(unsigned int *odt_value)
{
...
void memctlc_ddr2_dll_reset(void)
{
	...
	//odt_value=150;//for RTL8198C ,Solved DQ slew falling rate issue
#else
	__set_memctlc_ddr2_dll_reset(&odt_value);
#endif
...
static unsigned int __get_t_cas(void)
{
	return ((REG(DTR0) & (~(DTR0_CAS_MASK))) >> DTR0_CAS_FD_S);
}
#endif
...
#ifdef MEMCTL_DEBUG_PRINTF//<Coverity: UNUSED_VALUE>
			addr=uart_rx8b_val();
...
#if 1//<Coverity: DEADCODE>
static void __set_Dram_test_onflash(unsigned int *keep_W_R_mode, unsigned int *enable_delay,
		unsigned int *PM_MODE, unsigned int *cache_type, unsigned int *access_type,
		unsigned int *Data_pattern, unsigned int *random_test)
{
	*keep_W_R_mode= 0;
	*enable_delay= 0;
	*PM_MODE= 0;
...
void Dram_test_onflash()
{
   ...
   if((DCR_VALUE==  0x01A00000)) 
    ...
    unsigned int keep_W_R_mode;
	 ...
	 random_test=1;
#else
	__set_Dram_test_onflash(&keep_W_R_mode, &enable_delay, 
			&PM_MODE, &cache_type, &access_type, 
			&Data_pattern, &random_test);
#endif
...
{	
	...
	/*For DDR3 must , for DDR2 safe*/
#endif
	if(REG32(0xb800000C)== RTL8954ES)
	{
		   REG32(0xb8001064)|=(0xF<<24); //jitter
        REG32(0xb8001064)|=(0x1<<31); //new arbitor
        REG32(0xb8001064)|=(0x1<<30); //ddr phy
        REG32(0xb8001500)|=(0x1<<5); //IPC

	}
#ifdef CONFIG_SW_8367R
       #define PIN_MUX_SEL3 0xb8000108
       #define PEFGHCNR 0xB800351C
       #define PEFGHDIR 0xB8003524
       #define PEFGHDAT 0xB8003528
       REG32(PIN_MUX_SEL3) = (REG32(PIN_MUX_SEL3) & ~(3<<30)) | (0x3<<30);
        REG32(PEFGHCNR) &= (~(0x00000040));
        REG32(PEFGHDIR) |= (0x00000040);
                        REG32(PEFGHDAT) &= (~(0x00000040));
#endif
	...
	if(REG32(0xb800000C)== RTL8954ES)
        {
        	REG32(0xb8001500)|=(0x1<<5); //IPC
		REG32(0xb80000d0)= (REG32(0xb80000d0)&0xF)|0x5;
        }
