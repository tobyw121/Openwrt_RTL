...
#define _DISABLE_WCHAR_T_
#define _COOKIE_RFC2965_ /*jiunming*/
#define _SUPPORT_PUT_
#define WITH_NOIDREF
#define _WITH_ZXMLFILE_
/*ping_zhang:20090209 START:patch tr143 from telefonica linux 2.6*/
#define _SUPPORT_TR143_
/*ping_zhang:20090209 END*/
#ifndef _THREAD_SAFE
# define _THREAD_SAFE
#endif
...
#if 1 /*jiunming, don't support DIME, MIME, and etc.*/
#define WITH_LEANER
#else
#ifdef WITH_LEANER
# ifndef WITH_LEAN
...
#define CWMP_ENABLE_DIGEST
#define WITH_COOKIES

#ifdef CWMP_ENABLE_SSL
#define _WITH_MATRIXSSL_
#endif
...
do{ \
	...
	if(fp==stderr) \
		...
		xprintf( "fprintf: fp!=stderr\n" ); \
    } \
}while(0)
#define printf( ... )	fprintf( stderr, __VA_ARGS__ )

#define fflush(x)	while(0){}
#define int32_t 	int
#define EOF 		(-1)
#define atol(str) 	strtol(str, (char **)NULL, 10)
#define h_errno 	(-1) //gethostbyname
#define write 		write_cwmp
#define read 		read_cwmp
#define time 		time_cwmp
#define strdup		strdup_cwmp
#define inet_ntoa	inet_ntoa_cwmp
...
extern int gsoap_errno;
extern int gsoap_showmsg;

//star:20100825 IPV6_RELATED
#if 0
//for socket, EINTR, EAGAIN are netgative values
#define EINTR	EWOULDBLOCK
#define EAGAIN	EWOULDBLOCK

//for this, erange is positive
...
#if !defined(WITH_NOSTDLIB) && !defined(OSK)
# include <stdlib.h>
# ifndef PALM
#  include <stdio.h>
#  include <string.h>
...
#if !defined(UNDER_CE) && !defined(OSK)
# ifndef PALM
#  ifndef WITH_NOIO
#   include <errno.h>
#   include <sys/types.h>
...
#if !defined(WITH_NOIO) && !defined(OSK)
# ifndef WIN32
#  ifndef PALM
#   include <sys/socket.h>
#   ifdef VXWORKS
...
# if !defined(PALM) && !defined(OSK)
#  include <math.h>	/* for isnan() */
# endif
#endif

/*star:20110114 START add for ipv6 support*/
#ifdef CONFIG_IPV6_CWMP
#define WITH_IPV6
#endif
/*star:20110114 END add for ipv6 support*/
...
#elif defined(__socklen_t_defined) || defined(_SOCKLEN_T) || defined(CYGWIN) || defined(__QNX__) || defined(QNX)
//star:20100825 IPV6_RELATED END
# define SOAP_SOCKLEN_T socklen_t
...
#  define soap_closesocket(n) s_close(n)
# else
#  define SOAP_SOCKET int
...
# endif
#elif defined(OSK)
#  define soap_errno gsoap_errno
#  define soap_socket_errno gsoap_errno
#  define soap_reset_errno (gsoap_errno = 0)
#else
# ifndef WITH_NOIO
...
#if 1 /*jiunming, without linking with -lm*/
...
{ short state;			/* 0 = uninitialized, 1 = initialized, 2 = copy of another soap struct */
  ...
  char itfname[32];
  unsigned int dscp;
  int get_state_for_tr143;
  struct timeval tcp_open_request_time;
  struct timeval tcp_open_response_time;
  struct timeval start_send_time;
  struct timeval start_recv_time;
  unsigned int test_bytes_rx;
  ...
  sslConn_t		*conn;
  sslKeys_t		*keys;
  int (*fsslverify)(sslCertInfo_t *cert, void *arg);
#endif
...
/*star:20100207 START add to delay closing ssl connection*/
struct soap_shutdowned_socket
{
	struct soap_shutdowned_socket *next;
	int shutdownsocket;
	int shutdowntime;
};
/*star:20100207 END*/
...
#if 1 /*jiunming, keep these 2 functions for getting cwmp__ParameterValueStruct->__type*/
...
SOAP_FMAC1 void SOAP_FMAC2 soap_set_local_addr(struct soap*, struct sockaddr_in *, char *);
...
#ifdef _COOKIE_RFC2965_ /*jiunming*/
SOAP_FMAC1 extern int SOAP_FMAC2 soap_set_cookie_discard(struct soap *soap, const char *name, const char *domain, const char *path);
SOAP_FMAC1 extern int SOAP_FMAC2 soap_set_cookie_port(struct soap *soap, const char *name, char *port, const char *domain, const char *path);
#endif
#endif
/*star:20100207 START add to delay closing ssl connection*/
void soap_sdsocket_check(struct soap *soap);
/*star:20100207 END*/
#ifdef __cplusplus
} /* extern "C" */
#endif

/* Tsai: To eliminate warnings from no prototypes */
FILE *zfopen(const char *filename, const char *mode);
