...
{
#ifdef MULTI_WAN_SUPPORT
	fprintf( stderr, "cwmpClient [-i InterfaceName]:\n" );
	fprintf( stderr, "	-I/-i\t\tSpecify the interface for TR-069. Default interface will be that with TR069 WAN Type. If no such WAN interface, TR-069 won't start\n" );
#else
	...
	if( argc >= 2 )
	{
        ...
        if(!specifyWanIf)
            if(!specifyWanInterface()){
				fprintf( stderr, "ERROR: No WAN interface is specified for TR069!\n" );
                return 0;
            }
#endif
		for(i=1;i<argc;i++)
		{
			...
			}else if( strcmp( argv[i], "-I" )==0 ||  strcmp( argv[i], "-i" )==0 )
            {
                ...
                if( strstr( argv[i], "eth") || strstr( argv[i], "ppp") ){
                    fprintf( stderr, "<%s>Bind TR-069 to interface %s\n",__FILE__, argv[i]);
                    cwmpinit_WanIf(argv[i]);
                }else{
                    fprintf( stderr, "ERROR: %s is a wrong TR-069 interface. Must be eth1/eth1.1/...\n", argv[i] );
                    cwmp_show_help();
