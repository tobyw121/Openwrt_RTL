...
int objWLANConfiguration(char *name, struct CWMP_LEAF *e, int type, void *data)
{
	...
	int maxVWLAN=((NUM_VWLAN+1)*NUM_WLAN_INTERFACE);
	int i, j, instnum=0;
	...
	case eCWMP_tUPDATEOBJ:
		{
			...
			for(w_idx=0;w_idx<NUM_WLAN_INTERFACE;w_idx++)
			{
				...
				for(vw_idx=0;vw_idx<=NUM_VWLAN+1;vw_idx++)
				{
...
*/		
	...
	getWlanMib(rootIdx, vwlanIdx, MIB_WLAN_AUTH_TYPE, (void *)&vChar);
		...
		getWlanMib(rootIdx, vwlanIdx,MIB_WLAN_ENCRYPT,(void *)&vUint);
			...
			{
			getWlanMib(rootIdx, vwlanIdx,MIB_WLAN_WPA_CIPHER_SUITE,(void *)&vUint);
				if( vUint!=WPA_CIPHER_TKIP && vUint!=WPA_CIPHER_AES )
				getWlanMib(rootIdx, vwlanIdx,MIB_WLAN_WPA2_CIPHER_SUITE,(void *)&vUint);
				...
				if(fp)
				{
					if(has_info)
					{
						...
						if(fp2){
							...
							fclose(fp2);
					CWMPDBG( 1, ( stderr, "<%s:%d>wlanIface=%s keyg=%d\n", __FUNCTION__, __LINE__,wlan_ifname,keyg ) );
						}
