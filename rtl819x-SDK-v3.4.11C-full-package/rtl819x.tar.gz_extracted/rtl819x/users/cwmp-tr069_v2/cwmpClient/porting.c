...
int upgradeConfig( char *fwFilename )
{
	...
	CWMPDBG( 1, ( stderr, "<%s:%d>Ready to upgrade the new Config. Config length=%u, reboot_Wait=%u \n", __FUNCTION__, __LINE__,filelen,reboot_Wait  ) );
...
#else
        data->IsExecuting = 0;
        mib_get(MIB_CWMP_DL_COMMANDKEY, (void *)buf);
#endif
...
/*star:20100112 END*/
		{
			...
			mib_get(MIB_CWMP_RETRY_MIN_WAIT_INTERVAL, &uint);
			if (data->retryMinWaitInterval != uint)
			{
				printf("update the retryMinWaitInterval!\n" );
				data->retryMinWaitInterval = uint;
				cwmpRetryWaitTblInit(data);
			}

			mib_get(MIB_CWMP_RETRY_INTERVAL_MUTIPLIER, &uint);
			if (data->retryIntervalMutiplier != uint)
			{
				printf("update the retryIntervalMutiplier!\n" );
				data->retryIntervalMutiplier = uint;
				cwmpRetryWaitTblInit(data);
			}
		}
	}
...
void port_save_reboot( struct cwmp_userdata *user, int reboot_flag )
{
	...
	if( user )
	{
		...
		//related to download
#ifdef CWMP_QUEUE_SUPPORT
        shmid = shmget((key_t)CWMP_QUEUE_SHM_KEY, CWMP_QUEUE_SHM_SIZE, 0666|IPC_CREAT);
        if(shmid == -1){
            fprintf(stderr, "[%s %d] shmget failed\n", __FUNCTION__, __LINE__);
            return 0;
        }    
        shm = shmat(shmid, NULL, 0);
        if(shm == (void*)-1){
            fprintf(stderr, "[%s %d] shmat failed\n\n", __FUNCTION__, __LINE__);
            return 0;
        }
...
int port_after_download( int file_type, char *target )
{
	...
	switch(file_type)
	{
		CWMPDBG( 1, ( stderr, "<%s:%d>file_type = %d\n", __FUNCTION__, __LINE__, file_type ) );
	case DLTYPE_IMAGE:
		//update firmware
#ifdef ENABLE_SIGNATURE_ADV
		if (upgrade == 2) {
	...
	case DLTYPE_CONFIG:
		...
		if(upgradeConfig(target) < 0 ){
			CWMPDBG( 0, ( stderr, "<%s:%d>Config Checksum Failed!\n", __FUNCTION__, __LINE__ ) );
			return ERR_9010; 
		}
