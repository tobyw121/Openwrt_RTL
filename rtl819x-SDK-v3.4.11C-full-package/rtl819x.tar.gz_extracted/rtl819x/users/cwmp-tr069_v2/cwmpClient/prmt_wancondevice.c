...
{"NATEnabled",			eCWMP_tBOOLEAN,	CWMP_READ,	&tWANPPPCONENTITYLeafOP},
...
{"NATEnabled",			eCWMP_tBOOLEAN,	CWMP_READ,	&tWANIPCONENTITYLeafOP},
...
int setPORMAPTENTITY(char *name, struct CWMP_LEAF *entity, int type, void *data)
{
	...
	*((char *)&target[0]) = (char)port_chainid;
	...
	if( strcmp( lastname, "PortMappingEnabled" )==0 )
	{
		...
		//check enable or disable MIB_PORTFW_ENABLED
		...
		for (j=1; j<=entry_Num; j++) {
			...
			*((char *)&tmp_entry) = (char)j;
			apmib_get(MIB_PORTFW_TBL, (void *)&tmp_entry);
			if (tmp_entry.enabled){
				value++;
			}
		}
		printf("[%s:%d]port_entity.enabled=%d value=%d \n", __FUNCTION__, __LINE__, port_entity.enabled, value);
		mib_set( MIB_PORTFW_ENABLED, (void *)&value);

		mib_get( MIB_PORTFW_ENABLED, (void *)&value);
        printf("[%s:%d]MIB_PORTFW_ENABLED=%d\n", __FUNCTION__, __LINE__, value);
...
#if 0
        int remote_host_ip = atoi(data);
		port_ent->rmtipAddr[0] = 0xff&(remote_host_ip>>(8*3));
		port_ent->rmtipAddr[1] = 0xff&(remote_host_ip>>(8*2));
		port_ent->rmtipAddr[2] = 0xff&(remote_host_ip>>(8*1));
		port_ent->rmtipAddr[3] = 0xff&(remote_host_ip);
#else
        if(!strcmp(data, ""))
            inet_aton("0.0.0.0", (struct in_addr *)&port_ent->rmtipAddr);
        else
            inet_aton(data, (struct in_addr *)&port_ent->rmtipAddr);
        unsigned char debug_remote_host[32] = "";
        sprintf(debug_remote_host, "%d.%d.%d.%d", port_ent->rmtipAddr[0], port_ent->rmtipAddr[1],port_ent->rmtipAddr[2],port_ent->rmtipAddr[3]);
        //printf("[%s:%d] remote_host=%s\n", __FUNCTION__ ,__LINE__, debug_remote_host);
#endif
