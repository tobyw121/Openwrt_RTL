...
#ifdef __cplusplus
SOAP_SOURCE_STAMP("@(#) stdsoap2.cpp ver 2.7.7 2006-04-07 12:00:00 GMT")
extern "C" {
#else
SOAP_SOURCE_STAMP("@(#) stdsoap2.c ver 2.7.7 2006-04-07 12:00:00 GMT")
#endif
...
#if defined(WITH_OPENSSL) || defined(_WITH_MATRIXSSL_)
...
{ { 200, "OK" }, /*jiunming, handle status==200*/
  { 201, "Created" },
  { 202, "Accepted" },
...
*/
		/*the limit of max buf in matrixssl is 16k*/
		if( n > 0x4000 )
			nwritten = sslWrite(soap->conn, (char*)s, 0x4000, &status);
		else
			nwritten = sslWrite(soap->conn, (char*)s, n, &status);

		if(nwritten==0)
			soap_socket_errno = EAGAIN;
		else if (nwritten<0)
			fprintf( stderr,  "call sslWrite(), n=%d, nwritten:%d, status:%d\n", n, nwritten, status );
	}else
#endif
#ifdef WITH_UDP
...
#endif
{
	...
	int err;
        if (soap->ssl && (err = SSL_get_error(soap->ssl, nwritten)) != SSL_ERROR_NONE && err != SSL_ERROR_WANT_READ && err != SSL_ERROR_WANT_WRITE)
          return SOAP_EOF;
#elif defined(_WITH_MATRIXSSL_)
		if( soap->conn && (nwritten<0) )
		{
			fprintf( stderr, "sslwrite error( errno:%d )\n", soap_socket_errno );
			//in the case of eintr, ewouldblock, eagain, how to handle?, call sslwrite  again?
			//sslwrite seems not to handle this condition.
		}
#endif //WITH_OPENSSL
...
#ifdef _SUPPORT_TR143_
    if( nwritten>0 && soap->get_state_for_tr143 &&
        soap->start_send_time.tv_sec==0 && soap->start_send_time.tv_usec==0 )
    {
	gettimeofday( &soap->start_send_time, NULL );
	//fprintf( stderr, "set start_send_time={%d,%d}\n", soap->start_send_time.tv_sec, soap->start_send_time.tv_usec );
    }
#endif //_SUPPORT_TR143_
...
/*
		...
		> 0 indicates that some bytes were read.
*/
		r = sslRead(soap->conn, s, n, &status);
		if(r<=0) fprintf( stderr, "call ssRead(), n=%d, return=%d, status=%d\n", n, r, status );
		if(r>0)
		{
			return (size_t)r;
		}else if(r<0)
		{
			...
			if(status!=0) return 0; //error: eof or close_notify
			//else continue to read
		}
	}else
#endif
...
#endif
#ifdef _SUPPORT_TR143_
        if( r>0 && soap->get_state_for_tr143 )
        {
            if( soap->start_recv_time.tv_sec==0 && soap->start_recv_time.tv_usec==0 )
            {
                gettimeofday( &soap->start_recv_time, NULL );
                //fprintf( stderr, "set start_recv_time={%d,%d}\n", soap->start_recv_time.tv_sec, soap->start_recv_time.tv_usec );
            }
            //fprintf( stderr, "test_bytes_rx=%d, r=%d, %d\n", soap->test_bytes_rx, r, soap->test_bytes_rx+r );
            soap->test_bytes_rx+=r;
        }
#endif //_SUPPORT_TR143_
        if (r >= 0)
          return (size_t)r;
...
{ register struct soap_nlist *np = soap->nlist;
  ...
  if (n1 == 3 && n1 == n2 && !strcmp(id1, "xml") && !strcmp(id1, id2))
    return SOAP_OK;
/*jiunming, show why syntax error*/
fprintf( stderr, "<%s:%d><SOAP_SYNTAX_ERROR>id1:%s, id2:%s\n", __FUNCTION__, __LINE__, id1,id2 );
...
{ if (!ssl_init_done)
    soap_ssl_init();
  if (!soap->ctx)
  { if (!(soap->ctx = SSL_CTX_new(SSLv3_method()))) //jiunming, SSLv23_method
...
#if 0/*jiunming, disable the dh support*/
  else if (soap->dhfile)
  { DH *dh = 0;
    BIO *bio;
...
{ /* Note: for MT systems, the main program MUST call soap_ssl_init() before any threads are started */
  ...
  {
  	ssl_init_done = 1;
	//fprintf(stderr, "soap_ssl_init::matrixSslOpen!\n");
/*
...
*/
	if (matrixSslOpen() < 0)
	{
		fprintf(stderr, "matrixSslOpen failed!\n");
	}
  }
}
#endif
...
ssl_auth_init(struct soap *soap)
{
	...
	if(soap->keys==NULL)
	{
		//fprintf(stderr, "ssl_auth_init:matrixSslReadKeys\n");
		if (matrixSslReadKeys(&soap->keys, soap->keyfile, soap->keyfile, soap->password, soap->cafile ) < 0)
		{
			fprintf(stderr, "matrixSslReadKeys failed....\n");
			soap->keys=NULL;
			//return SOAP_ERR;
			return soap_set_receiver_error(soap, "SSL error", "Can't read certificates or key file", SOAP_SSL_ERROR);
		}
	}
	return SOAP_OK;
}
...
soap_done(struct soap *soap)
{
  ...
  { if (soap_valid_socket(soap->master))
	...
	if(soap->keys)
	{
		//fprintf( stderr,  " <<<<<<<<<<matrixSslFreeKeys\n" );
		matrixSslFreeKeys(soap->keys);
		soap->keys = NULL;
	}
	//never clean=>init once
	//matrixSslClose();
	//WSACleanup();
#endif
  }
...
again:
...
#ifdef WITH_IPV6
#else
    //bind(fd, &soap->local_addr, sizeof(soap->local_addr));
    setsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_BINDTODEVICE, soap->local_ifname, strlen(soap->local_ifname));
#endif
 ...
 if(!soap->get_state_for_tr143){
...
#endif
#ifdef _SUPPORT_TR143_
  if( strlen(soap->itfname) )
  {
	if (setsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_BINDTODEVICE, (char*)&soap->itfname, strlen(soap->itfname)+1 ))
	{
		soap->errnum = soap_socket_errno;
		soap_set_sender_error(soap, tcp_error(soap), "setsockopt SO_BINDTODEVICE failed in tcp_connect()", SOAP_TCP_ERROR);
		soap->fclosesocket(soap, (SOAP_SOCKET)fd);
		return SOAP_INVALID_SOCKET;
	}
	//fprintf( stderr, "tcp_connect> set SO_BINDTODEVICE=%s\n", soap->itfname );
  }
  if( soap->dscp )
  {
  	int tos_value=(soap->dscp<<2);
	if (setsockopt((SOAP_SOCKET)fd, IPPROTO_IP, IP_TOS, (char*)&tos_value, sizeof(int)))
	{
		soap->errnum = soap_socket_errno;
		soap_set_sender_error(soap, tcp_error(soap), "setsockopt IP_TOS failed in tcp_connect()", SOAP_TCP_ERROR);
		soap->fclosesocket(soap, (SOAP_SOCKET)fd);
		return SOAP_INVALID_SOCKET;
	}
	//fprintf( stderr, "tcp_connect> set IP_TOS=%d\n", soap->dscp );
  }
#endif //_SUPPORT_TR143_
#endif
  ...
  for (;;)
  {
    ...
    if(soap->get_state_for_tr143)
    {
    	gettimeofday( &soap->tcp_open_request_time, NULL );
    	//fprintf( stderr, "set tcp_open_request_time={%d,%d}\n", soap->tcp_open_request_time.tv_sec, soap->tcp_open_request_time.tv_usec );
    }
#endif //_SUPPORT_TR143_
#ifdef WITH_IPV6
  ...
  if(soap->get_state_for_tr143)
  {
  	gettimeofday( &soap->tcp_open_response_time, NULL );
        //fprintf( stderr, "set tcp_open_response_time={%d,%d}\n", soap->tcp_open_response_time.tv_sec, soap->tcp_open_response_time.tv_usec );
  }
#endif //_SUPPORT_TR143_
  ...
  if (!strncmp(endpoint, "https:", 6))
  {
	...
	if (!soap->keys && (soap->error = soap->fsslauth(soap)))
	{
	...
	soap->imode |= SOAP_ENC_SSL;
	soap->omode |= SOAP_ENC_SSL;
	if (sslConnect(&soap->conn, fd, soap->keys, soap->sessionId, 0, soap->fsslverify, soap) < 0)
	{
		soap_set_sender_error(soap, "sslConnect", "SSL connect failed in tcp_connect()", SOAP_SSL_ERROR);
          	soap->fclosesocket(soap, (SOAP_SOCKET)fd);
          	return SOAP_INVALID_SOCKET;
	}
...
#elif defined(_WITH_MATRIXSSL_)
     //using the code below to handle ==> recv(,,, MSG_PEEK)
#endif //WITH_OPENSSL
      if (soap_valid_socket(soap->socket)
       && FD_ISSET((SOAP_SOCKET)soap->socket, &sfd)
       && (!FD_ISSET((SOAP_SOCKET)soap->socket, &rfd)
...
tcp_disconnect(struct soap *soap)
{
  ...
  { int r, s = 0;
	...
	if(soap->sessionId)
	{
		//fprintf( stderr,  " <<<<<<<<<<matrixSslFreeSessionId\n" );
		matrixSslFreeSessionId(soap->sessionId);
		soap->sessionId = NULL;
	}
	if(soap->conn)
	{
		//fprintf( stderr,  " <<<<<<<<<<sslWriteClosureAlert\n" );
		sslWriteClosureAlert(soap->conn);
		//fprintf( stderr,  " <<<<<<<<<<sslFreeConnection\n" );
		sslFreeConnection(&soap->conn);
		soap->conn = NULL;
	}
#endif
...
#elif defined(_WITH_MATRIXSSL_)
  if (!soap->conn && soap_valid_socket(soap->socket) && !soap->keep_alive && !(soap->omode & SOAP_IO_UDP))
    soap->fshutdownsocket(soap, (SOAP_SOCKET)soap->socket, 1); /* Send TCP FIN */
#else //WITH_OPENSSL
...
void
SOAP_FMAC2
soap_set_local_addr(struct soap *soap, struct sockaddr_in *addr, char *ifname)
{
#ifdef MULTI_WAN_SUPPORT
    strncpy(soap->local_ifname, ifname, strlen(ifname));
    soap->local_addr.sin_family = addr->sin_family;
    soap->local_addr.sin_addr.s_addr = addr->sin_addr.s_addr;
#endif
}
...
#ifdef _SUPPORT_TR143_
  //fprintf( stderr, "reset tr143 variables: time...\n" );
  memset( &soap->tcp_open_request_time, 0, sizeof(soap->tcp_open_request_time) );
  memset( &soap->tcp_open_response_time, 0, sizeof(soap->tcp_open_response_time) );
  memset( &soap->start_send_time, 0, sizeof(soap->start_send_time) );
  memset( &soap->start_recv_time, 0, sizeof(soap->start_recv_time) );
  soap->test_bytes_rx=0;
#endif //_SUPPORT_TR143_
...
{
/* because some servers(zyxel) complain the attribute
 * remove the tag, SOAP-ENC:Array */
#if 0
...
soap_outstring(struct soap *soap, const char *tag, int id, char *const*p, const char *type, int n)
{
  ...
  if( tag && p && *p && *tag=='-' )
  {
  	char *flag_h="<SendFileString>";
  	char *flag_t="</SendFileString>";
  	char *pS, *pE;
  	pS = strstr( *p, flag_h );
  	pE = strstr( *p, flag_t );
  	if( (pS!=NULL) && (pE!=NULL) )
  	{
		fprintf( stderr, "<SendFileString></SendFileString> found, pasering...\n" );
  		pS+=strlen( flag_h );
  		if( (pE>pS) && (sizeof(soap->tmpbuf)>(pE-pS)) )
  		{
  			char *pTS, *pTE;
			strncpy( soap->tmpbuf, pS, pE-pS );
			soap->tmpbuf[pE-pS]='\0';
			//fprintf( stderr, "soap->tmpbuf:%s\n", soap->tmpbuf );

  			pS=NULL;pE=NULL;pTS=NULL;pTE=NULL;
  			pS=strstr( soap->tmpbuf, "<" );
  			if(pS) pE=strstr( pS, ">" );
  			if(pE) pTS=strstr( pE, "<" );
  			if(pTS) pTE=strstr( pTS, ">" );
  			if( pS && pE && pTS && pTE )
  			{
				if( soap_send_raw(soap, pS,  pE-pS+1 ) )
				{
					return soap->error;
				}

				strncpy( soap->tmpbuf, pE+1, pTS-pE-1 );
				soap->tmpbuf[pTS-pE-1]='\0';
				if(soap->tmpbuf[0])
				{
					FILE *fp;
					//fprintf( stderr, "filename:%s\n", soap->tmpbuf );
					fp = zfopen( soap->tmpbuf, "r" );
					if(fp)
					{
						//while( feof(fp)==0 )
						while(1)
						{
							...
							/*
							read_size = zfread( outbuf, 1, 1023, fp );
							outbuf[read_size]=0;
							//fprintf( stderr, "content:%s\n", outbuf );
							if( soap_send_raw(soap, outbuf, strlen(outbuf)) )
							*/
							read_size = zfread( outbuf, 1, sizeof(outbuf), fp );
							if( soap_send_raw(soap, outbuf, read_size) )
				...
				if( soap_send_raw(soap, pTS,  pTE-pTS+1 ) )
				{
					return soap->error;
				}
...
{ struct tm T, *pT = &T;
#if 1 /*jiunming tr-069 define the unknown time's format*/
{
	char *unknow_time="0001-01-01T00:00:00Z";
	if(n==0)
		strcpy(soap->tmpbuf, unknow_time);
	else{
...
#endif
		{
			...
			if( pT->tm_gmtoff )
				sprintf(soap->tmpbuf + strlen(soap->tmpbuf), "%+03d:%02d", pT->tm_gmtoff/(60*60)+(pT->tm_isdst!=0), (abs(pT->tm_gmtoff)/60)%60);
			else
				sprintf(soap->tmpbuf + strlen(soap->tmpbuf), "Z" );
		}
		else
			strcpy(soap->tmpbuf, unknow_time);
	}
...
{ register soap_wchar c = 0;
  ...
  { register size_t i, m = 0;
    ...
    if (soap->mime.last != soap->mime.first && soap->fmimewriteopen && ((content->ptr = (char*)soap->fmimewriteopen(soap, content->id, content->type, content->description, content->encoding)) || soap->error))
    { if (!content->ptr)
        return soap->error;
    }
    else if (soap_new_block(soap))
      return soap->error = SOAP_EOM;
    for (;;)
...
void soap_print_cookie( struct soap_cookie *p, int lineno )
{
	fprintf( stderr, "<%d>cookie:\n", lineno );
	if(p)
	{
...
#ifdef _COOKIE_RFC2965_ /*jiunming*/
...
{ struct soap_cookie **p, *q;
  ...
  //fprintf( stderr, "<%s>name:%s, value:%s, domain:%s, path:%s\n",  __FUNCTION__, name?name:"(null)", value?value:"(null)", domain?domain:"(null)", path?path:"(null)" );
  if (!domain)
    domain = soap->cookie_domain;
  if (!path)
    path = soap->cookie_path;
...
int
SOAP_FMAC2
soap_set_cookie_discard(struct soap *soap, const char *name, const char *domain, const char *path)
{ struct soap_cookie *p;
  if ((p = soap_cookie(soap, name, domain, path)))
  { p->discard = 1;
    p->modified = 1;
...
int
SOAP_FMAC2
soap_set_cookie_port(struct soap *soap, const char *name, char *port, const char *domain, const char *path)
{ struct soap_cookie *p;
  if ((p = soap_cookie(soap, name, domain, path)))
  {
    if( p->port )
    {
       //fprintf( stderr, "<%s:%d>p->port:0x%x\n", __FUNCTION__, __LINE__, p->port );
       SOAP_FREE(soap, p->port);
       p->port = NULL;
    }
    if (port && *port && (p->port = (char*)SOAP_MALLOC(soap, strlen(port)+1)))
      strcpy(p->port, port);
    //fprintf( stderr, "<%s:%d> new p->port:0x%x\n", __FUNCTION__, __LINE__, p->port );
    p->modified = 1;
    return SOAP_OK;
  }
  return SOAP_ERR;
...
{ struct soap_cookie *p;
  ...
  { if (p->modified || !p->env)
    { s = tmp;
...
#if 1 /*jiunming, old codes does not count the new length*/
      s += strlen(s);
      if (p->version > 0 && s-tmp < 4060)
      { sprintf(s, ";Version=%u", p->version);
        s += strlen(s);
      }
      if (p->expire >= 0 && s-tmp < 4060)
...
#endif
#ifdef _COOKIE_RFC2965_ /*jiunming*/
      if (p->port!=NULL && s-tmp<4060)
      { sprintf(s, ";Port=\"%s\"", p->port);
        s += strlen(s);
      }
      if (p->discard && s-tmp < 4060)
      {
        strcat(s, ";Discard");
        s += strlen(s);
      }
#if 0
      sprintf(s, ";Comment=\"This is a cookie's comment!\"");
      s += strlen(s);
      sprintf(s, ";CommentURL=\"http://www.yahoo.com.tw/index.html\"");
      s += strlen(s);
#endif
#endif
      ...
      //fprintf( stderr, "Header Set-cookie2:%s \n", tmp );
#ifdef _COOKIE_RFC2965_ /*jiunming*/
      if ((soap->error = soap->fposthdr(soap, "Set-Cookie2", tmp)))
        return soap->error;
#else
...
#ifdef _SUPPORT_302_
	//if redirection, cpe must include all cookies associated with the session.
      ...
      { s = tmp;
        ...
        s += strlen(s); /*jiunming, count the length*/
#else
...
#ifdef _COOKIE_RFC2965_ /*jiunming*/
        if (q->port && (int)strlen(q->port) < tmp-s+4080)
        { sprintf(s, ";$Port=\"%s\"", q->port);
          s += strlen(s);
        }
#endif
        ...
        //fprintf( stderr, "Header Cookie:%s \n", tmp );
...
{ struct soap_cookie *p = NULL, *q;
  ...
  { s = soap_decode_key(tmp, sizeof(tmp), s);
    ...
    else if (p && !soap_tag_cmp(tmp, "CommentURL"))
    {
	s = soap_decode_val(tmp, sizeof(tmp), s);
    }
    else if (p && !soap_tag_cmp(tmp, "Port"))
    {
	if (p->port)
	SOAP_FREE(soap, p->port);
	s = soap_decode_val(tmp, sizeof(tmp), s);
	if (*tmp)
	{
		if ((p->port = (char*)SOAP_MALLOC(soap, strlen(tmp)+1)))
			strcpy(p->port, tmp);
	}
	else
		p->port = NULL;
    }
    else if (p && !soap_tag_cmp(tmp, "Discard"))
	p->discard = 1;
#endif
    else
    { if (p)
      { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Got environment cookie %s=%s domain=%s path=%s expire=%ld secure=%d\n", p->name, p->value?p->value:"", p->domain?p->domain:"", p->path?p->path:"", p->expire, p->secure));
		...
		if( q->port )
			SOAP_FREE(soap, q->port);
		if( p->port && *p->port && (q->port=(char*)SOAP_MALLOC(soap, strlen(p->port)+1))  )
			strcpy( q->port, p->port );
		else
			q->port=NULL;
#endif
          q->env = 1;
...
#ifdef _COOKIE_RFC2965_ /*jiunming*/
    if (p->port)
    { if (((*q)->port = (char*)SOAP_MALLOC(soap, strlen(p->port)+1)))
        strcpy((*q)->port, p->port);
    }
#endif
    q = &(*q)->next;
...
{ soap_wchar c;
  ...
  *soap->endpoint = '\0';
  ...
  { soap->mode &= ~SOAP_IO;
    ...
    {
    	/*jiunming,if 401, don't close the current tcp connection*/
    	if( soap->error!=401 )
  ...
  *soap->endpoint = '\0';
  ...
  { if (soap_getline(soap, soap->msgbuf, sizeof(soap->msgbuf)))
    ...
    { k = (unsigned short)soap_strtoul(s, &s, 10);
      if (!soap_blank(*s))
        k = 0;
      soap->status = k; /*jiunming, save the Status-Code*/
    }
    else
      k = 0;
  } while (k == 100);
...
#if 0 /*jiunming, let high-layer handle 400/500 errors*/
...
#ifdef WITH_COOKIES
#ifdef _COOKIE_RFC2965_ /*jiunming*/
  else if (!soap_tag_cmp(key, "Cookie") || !soap_tag_cmp(key, "Set-Cookie") || !soap_tag_cmp(key, "Set-Cookie2"))
#else
...
/*star:20110901 START move encodingstyle form Body*/
  if (soap->version == 1)
    soap->encoding = 1;
/*star:20110901 END*/
...
/*jiunming*/
#ifdef WITH_COOKIES
soap->cookie_domain = soap->host;
soap->cookie_path = soap->path;
#endif
...
#ifdef _SUPPORT_PUT_
  if (  (http_command != SOAP_POST) && (http_command != SOAP_PUT)   ) /*jiunming*/
#else
...
{ register const char *s = NULL;
  ...
  if (status == SOAP_FILE && soap->http_content)
    s = soap->http_content;
#ifdef _SUPPORT_PUT_ /*jiunming, for SOAP_PUT*/
  else if( soap->status==SOAP_PUT && soap->http_content )
    s = soap->http_content;
#endif
...
#endif
/*jiunming, force to output the content-length (some servers will check this filed)*/
#if 0
...
{ register const char *s;
  ...
  if (soap_putcookies(soap, host, path, soap->ssl != NULL))
    return soap->error;
#elif defined(_WITH_MATRIXSSL_)
  if (soap_putcookies(soap, host, path, soap->conn != NULL))
    return soap->error;
#else
...
#endif
#endif
  if (count>0 && (soap->version == 1 || (action && *action))) //An empty HTTP POST MUST NOT contain a SOAPAction
  {
    //force soapaction:, instead of soapaction:""
...
{ register int err;
  ...
  else if (status >= 200 && status < 600) /*jiunming, handle status==200*/
