...
int lib1x_resolve()
{
	...
	l = libnet_open_link_interface( (char *)dev_svr, (char *)errbuf );
	if ( l == NULL ) {
		lib1x_message(MESS_ERROR_FATAL,"open1x: Could not open interface %s", dev_svr );
		return -1;	// david
	}
	tmpip.s_addr = ntohl(libnet_get_ipaddr(l, (char *)dev_svr, (char *)errbuf));
	if ( tmpip.s_addr == -1 ) {
		lib1x_message(MESS_ERROR_FATAL,"open1x: Could not obtain IP for interface %s", dev_svr );
		return -1;	// david
	}
	strcpy( (char *)ourip, inet_ntoa(tmpip));
	eth = libnet_get_hwaddr( l, (char *)dev_svr, (char *)errbuf);
	if (eth == NULL ) {
	...
	l = libnet_open_link_interface( (char *)dev_supp, (char *)errbuf );
	if ( l == NULL ) {
		lib1x_message(MESS_ERROR_FATAL,"open1x: Could not open interface %s", dev_supp );
		return -1;	// david
	}
	eth = libnet_get_hwaddr( l, (char *)dev_supp, (char *)errbuf);
	if (eth == NULL ) {
...
*/
	...
	strcpy((char *)dev_supp, theArgs[1]);
...
//-------------------------------
	...
	lib1x_print_etheraddr( (char *)eth, oursvr_addr );
	lib1x_print_etheraddr( (char *)eth, oursupp_addr );
...
#endif
		if(lib1x_init_fifo(&RTLAuthenticator, (char *)dev_supp)) {
