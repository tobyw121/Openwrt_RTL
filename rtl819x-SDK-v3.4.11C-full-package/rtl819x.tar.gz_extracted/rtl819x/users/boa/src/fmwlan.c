    	...
    	if ((strVal==NULL || strVal[0]==0)   // not called from wizard	
    		...
    		if (val == 1) {
				...
				if(strlen(strVal)>=MAX_SSID_LEN){
					sprintf(tmpBuf,"input 'SSID of Extended Interface' length is %d,oversize! Max should be %d!\n",strlen(strVal),MAX_SSID_LEN-1);
					goto setErr_wlan;
				}
    			if (strVal){
    				if (wlan_id == 0)
...
{
        char *submitUrl,*meshRootEnabled,*refresh, *strMeshID, *strEnabled, *strCrossbandEnabled;
        int enabled,meshenable=0,mesh_crossband=0;
        char tmpBuf[MAX_MSG_BUFFER_SIZE]={0};
        int warn=0;
		...
		{
		        strcpy( tmpBuf, ("Set mesh crossband enable error!"));
                goto setErr_mesh;
		}
#endif
...
// for 11N
	...
	strValue = req_get_cstream_var(wp, ("tdls_cs_prohibited_"), "");
       ...
       apmib_set(MIB_CAPWAP_MODE, (void *)&val); //mode=3, ac&wtp
       apmib_set(MIB_CAPWAP_WTP_ID, (void *)&wtp_id);//id=1
#endif
