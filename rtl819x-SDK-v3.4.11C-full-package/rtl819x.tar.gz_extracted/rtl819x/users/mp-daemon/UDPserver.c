...
int MP_get_ext(char *ifname, char *buf, unsigned int ext_num)
{
		...
		//printf("[MP_get_ext] ifname=%s, buf=%s, ext_num=%x\n",ifname,buf,ext_num);
...
int system_if(char *buf)
{
	...
	pwlan1 = str_replace(buf,"wlan0","wl1");//replace "wlan0" with "wl1"
	pwlan2 = str_replace(pwlan1,"wlan1","wl0");//replace "wlan1" with "wl0"
...
#ifdef IF_NAMING
				buf1 = str_replace(buf,"wl1","wlan0");
				buf2 = str_replace(buf1,"wl0","wlan1");				
				sprintf(pre_result, "data:%s", buf2);
#else
