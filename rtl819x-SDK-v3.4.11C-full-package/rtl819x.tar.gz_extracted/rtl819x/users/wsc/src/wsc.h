...
#define WSC_DYNAMIC_DEBUG

#ifdef WSC_DYNAMIC_DEBUG
#define DEBUG
#define PRINT_ERR
//#define TEST
//#define DEBUG_TRACE
...
//#define OVERLAPPING_BY_BAND // check overlapping by RF band
...
#define DEBUG_PRINT(fmt, args...) \
	do{\
	 if (debug_on&&pCtx->debug)\
	 	printf(fmt, ## args);\
	}while(0)

#define DEBUG_PRINT2(fmt, args...) \
	do{\
		if (debug_on&&pCtx->debug2)\
...
#define WSC_DEBUG(fmt, args...)\
	do{\
		if(debug_on)\
			printf("[wsc]%s %d:"fmt,__FUNCTION__ , __LINE__ , ## args);\
	}while(0)
#define RX_DEBUG(fmt, args...)\
	do{\
		if(debug_on)\
			printf("[rx]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args);\
	}while(0)
#define TX_DEBUG(fmt, args...)\
	do{\
		if(debug_on)\
			printf("[tx]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args);\
	}while(0)
#define UTIL_DEBUG(fmt, args...)\
	do{\
		if(debug_on)\
			printf("[util]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args);\
	}while(0)
#define SUM_DEBUG(fmt, args...)\
	do{\
		if(debug_on)\
			printf("[sum]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args);\
	}while(0)
#define UPNP_DEBUG(fmt, args...)\
	do{\
		if(debug_on)\
			printf("[upnp]%s,%d:"fmt,__FUNCTION__ , __LINE__ , ## args);\
	}while(0)
#define P2P_DEBUG(fmt, args...)\
	do{\
		if(debug_on)\
			printf("[p2p-wsc]%s %d:"fmt,__FUNCTION__ , __LINE__ , ## args);\
	}while(0)
...
#define WSCD_START_PARAMETERS		("/tmp/wscd_daemon_parameters")	// 2016-0607 ; keep daemon start parameters here
...
extern char StringbufferOut[256];
...
extern void add_prefix_padding(unsigned char *dst, int size);
