		     ...
		     struct iw_point *wrqu, char *extra)
{
	...
	input=kmalloc(wrqu->length+1,GFP_KERNEL);
    if(input == NULL)
    {
            printk("Kmalloc failed %s %d\n",__FUNCTION__,__LINE__);
            return -EFAULT;
    }
	_rtw_memset(input, 0, wrqu->length+1);
		    ...
		    struct iw_point *wrqu, char *extra)
{
	...
	if (path >= GET_HAL_RFPATH_NUM(adapter_to_dvobj(padapter)))
		return -EINVAL;
	/* RF A-die 0x0 ~ 0xff, RF D-die 0x10000 ~ 0x100ff */
	if (((addr > 0xFF) && (addr < 0x10000)) || (addr > 0x100FF))
		return -EINVAL;
	if (data > 0xFFFFF)
		return -EINVAL;
	...
	rtw_phl_write_rfreg(GET_HAL_INFO(dvobj), path, addr, 0xFFFFF, data);
		   ...
		   struct iw_point *wrqu, char *extra)
{
	...
	sprintf(data, "%08x",  rtw_phl_read_rfreg(GET_HAL_INFO(dvobj), path, addr , 0xFFFFF));
	/*add read data format blank*/
	for (i = 0 ; i <= strlen(data) ; i++) {
		if (i % 2 == 0) {
			tmp[j] = ' ';
 ...
 * Input Format: %x[%d:%d]
 *	1st %x is address(offset)
 *	1st %d is start bit(higher), should be in 0 ~ 31
 *	2nd %d is end bit(lower), should be in 0 ~ 31
 *
 * Return:
 *	%x for data read.
 *
 * Example:
 *  # iwpriv wlan0 read_reg_bit 0[31:0]
 */
int rtw_mp_read_reg_bit(struct net_device *dev,
		  		    	struct iw_request_info *info,
		  		    	struct iw_point *wrqu, char *extra)
{
	char input[128];
	...
	_adapter *padapter = rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	u32 start_bit = 0, end_bit = 0, i = 0;

	_rtw_memset(input, 0, wrqu->length);
	if (copy_from_user(input, wrqu->pointer, wrqu->length))
		return -EFAULT;

	ret = sscanf(input, "%x[%d:%d]", &addr, &start_bit, &end_bit);
	if (ret < 3) {
		ret = sscanf(input, "%x[%d]", &addr, &start_bit);
		if (ret < 2) {
			printk(" Wrong format!!! Ex: iwpriv wlan0 read_reg_bit 0[31:0] \n");
			return -EINVAL;
		} else {
			end_bit = start_bit;
		}
	...
	if ((start_bit > 31) || (end_bit > 31)) {
		printk(" Error!!! Start/End bit range is 0 ~ 31 \n");
		return -EINVAL;
	} else if (start_bit < end_bit) {
		printk(" Error!!! Start bit should >= end bit \n");
		return -EINVAL;
	}

	data = rtw_phl_read32(dvobj->phl, addr);
	data >>= end_bit;
	// clear other bit content exclude [start_bit:end_bit]
	for (i = start_bit - end_bit + 1; i <= 31; i++) {
		data &= (~(BIT(i)));
	}
 ...
 *
 * Example:
 *  # iwpriv wlan0 write_reg_bit 0[31:0],1a2b3c4d
 */
int rtw_mp_write_reg_bit(struct net_device *dev,
		     		 	 struct iw_request_info *info,
		    			 struct iw_point *wrqu, char *extra)
{
	u32 addr, data;
	...
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	char input[128];
	u32 start_bit = 0, end_bit = 0, i = 0, tmp = 0;

	_rtw_memset(input, 0, wrqu->length);
	if (copy_from_user(input, wrqu->pointer, wrqu->length))
		return -EFAULT;

	ret = sscanf(input, "%x[%d:%d],%x", &addr, &start_bit, &end_bit, &data);
	if (ret < 4) {
		ret = sscanf(input, "%x[%d],%x", &addr, &start_bit, &data);
		if (ret < 3) {
			printk(" Wrong format!!! Ex: iwpriv wlan0 write_reg_bit 0[31:0],1a2b3c4d \n");
			return -EINVAL;
		} else {
			end_bit = start_bit;
		}
	...
	if ((start_bit > 31) || (end_bit > 31)) {
		...
		for (i = 0; i <= start_bit - end_bit; i++)
			limit |= BIT(i);
		if (data > limit) {
			printk(" Error!!! Writing data over boundary \n");
			return -EINVAL;
		}
	}

	tmp = rtw_phl_read32(dvobj->phl, addr);
	// clear bit[start_bit:end_bit] content
	for (i = end_bit; i <= start_bit; i++) {
		tmp &= (~(BIT(i)));
	}
	// update bit[start_bit:end_bit] content
	data <<= end_bit;
	tmp |= data;
	rtw_phl_write32(dvobj->phl, addr, tmp);

	// report new content
	tmp = rtw_phl_read32(dvobj->phl, addr);
	_rtw_memset(extra, 0, wrqu->length);
	sprintf(extra, " (0x%x) = 0x%08x\n", addr, tmp);
	wrqu->length = strlen(extra);
...
/*
 * Input Format: %d,%x[%d:%d]
 *	1st %d is RF path, should be smaller than MAX_RF_PATH_NUMS
 *	1st %x is address(offset)
 *	2nd %d is start bit(higher), should be in 0 ~ 31
 *	3rd %d is end bit(lower), should be in 0 ~ 31
 *
 * Return:
 *	%x for data read
 *
 * Example:
 *  # iwpriv wlan0 read_rf_bit 0,0[31:0]
 */
int rtw_mp_read_rf_bit(struct net_device *dev,
		   struct iw_request_info *info,
		   struct iw_point *wrqu, char *extra)
{
	u32 path, addr, data;
	...
	ret = sscanf(input, "%d,%x[%d:%d]", &path, &addr, &start_bit, &end_bit);
	if (ret < 4) {
		ret = sscanf(input, "%d,%x[%d]", &path, &addr, &start_bit);
		if (ret < 3) {
			printk(" Wrong format!!! Ex: iwpriv wlan0 read_reg_bit 0[31:0] \n");
			return -EINVAL;
		} else {
	...
	if (path >= GET_HAL_RFPATH_NUM(adapter_to_dvobj(padapter))) {
		printk(" Error!!! RF path should be smaller than MAX_RF_PATH_NUMS \n");
		return -EINVAL;
	}
	/* RF A-die 0x0 ~ 0xff, RF D-die 0x10000 ~ 0x100ff */
	if (((addr > 0xFF) && (addr < 0x10000)) || (addr > 0x100FF)) {
		printk(" Error!!! RF address is not in range\n");
		return -EINVAL;
	}

	data = rtw_phl_read_rfreg(GET_HAL_INFO(dvobj), path, addr, 0xFFFFF);
	data >>= end_bit;
	// clear other bit content exclude [start_bit:end_bit]
	for (i = start_bit - end_bit + 1; i <= 31; i++) {
		data &= (~(BIT(i)));
 ...
 *
 * Example:
 *  # iwpriv wlan0 write_rf_bit 0,0[31:0],1a2b3c4d
 */
int rtw_mp_write_rf_bit(struct net_device *dev,
					    struct iw_request_info *info,
		    			struct iw_point *wrqu, char *extra)
{
	u32 path, addr, data;
	...
	ret = sscanf(input, "%d,%x[%d:%d],%x", &path, &addr, &start_bit, &end_bit, &data);
	if (ret < 5) {
		ret = sscanf(input, "%d,%x[%d],%x", &path, &addr, &start_bit, &data);
		if (ret < 4) {
			printk(" Wrong format!!! Ex: iwpriv wlan0 write_reg_bit 0[31:0],1a2b3c4d \n");
			return -EINVAL;
		} else {
	...
	if (data > 0xFFFFF) {
		printk(" Error!!! Writing data limit is 0xFFFFF \n");
		return -EINVAL;
	}

	tmp = rtw_phl_read_rfreg(GET_HAL_INFO(dvobj), path, addr, 0xFFFFF);
	// clear bit[start_bit:end_bit] content
	for (i = end_bit; i <= start_bit; i++) {
		tmp &= (~(BIT(i)));
	}
	...
	data <<= end_bit;
	tmp |= data;
	rtw_phl_write_rfreg(GET_HAL_INFO(dvobj), path, addr, 0xFFFFF, tmp);
 ...
 *
 * Example:
 *  # iwpriv wlan0 read_bb_bit 0[31:0]
 */
int rtw_mp_read_bb_bit(struct net_device *dev,
		   			   struct iw_request_info *info,
					   struct iw_point *wrqu, char *extra)
{
	u32 addr, data;
	...
	data = rtw_phl_read_bbreg(GET_HAL_INFO(dvobj), addr, 0xFFFFFFFF);
	data >>= end_bit;
	// clear other bit content exclude [start_bit:end_bit]
	for (i = start_bit - end_bit + 1; i <= 31; i++) {
		data &= (~(BIT(i)));
 ...
 *
 * Example:
 *  # iwpriv wlan0 write_bb_bit 0[31:0],1a2b3c4d
 */
int rtw_mp_write_bb_bit(struct net_device *dev,
					    struct iw_request_info *info,
		    			struct iw_point *wrqu, char *extra)
{
	u32 addr, data;
	...
	tmp = rtw_phl_read_bbreg(GET_HAL_INFO(dvobj), addr, 0xFFFFFFFF);
	// clear bit[start_bit:end_bit] content
	for (i = end_bit; i <= start_bit; i++) {
		tmp &= (~(BIT(i)));
	}
	...
	data <<= end_bit;
	tmp |= data;
	rtw_phl_write_bbreg(GET_HAL_INFO(dvobj), addr, 0xFFFFFFFF, tmp);
 ...
 *
 * Example:
 *  # iwpriv wlan0 read_mac_bit 0[31:0]
 */
int rtw_mp_read_mac_bit(struct net_device *dev,
		   			    struct iw_request_info *info,
					    struct iw_point *wrqu, char *extra)
{
	u32 addr, data;
	...
	data = rtw_phl_read_macreg(GET_HAL_INFO(dvobj), addr, 0xFFFFFFFF);
	data >>= end_bit;
	// clear other bit content exclude [start_bit:end_bit]
	for (i = start_bit - end_bit + 1; i <= 31; i++) {
		data &= (~(BIT(i)));
 ...
 *
 * Example:
 *  # iwpriv wlan0 write_mac_bit 0[31:0],1a2b3c4d
 */
int rtw_mp_write_mac_bit(struct net_device *dev,
					     struct iw_request_info *info,
		    			 struct iw_point *wrqu, char *extra)
{
	u32 addr, data;
	...
	tmp = rtw_phl_read_macreg(GET_HAL_INFO(dvobj), addr, 0xFFFFFFFF);
	// clear bit[start_bit:end_bit] content
	for (i = end_bit; i <= start_bit; i++) {
		tmp &= (~(BIT(i)));
	}
	...
	data <<= end_bit;
	tmp |= data;
	rtw_phl_write_macreg(GET_HAL_INFO(dvobj), addr, 0xFFFFFFFF, tmp);
		 ...
		 struct iw_point *wrqu, char *extra)
{
	...
	struct wifi_mib_priv *priv_mib = &(padapter->registrypriv.wifi_mib);

	if (rtw_mp_mode_check (padapter)) {
		RTW_WARN("mp_start failed, already in MP mode !!\n");
		goto exit;
	}
	...
	rtw_mp_phl_config_arg(padapter, MP_CONFIG_CMD_START_DUT);
	rtw_mp_phl_config_arg(padapter, MP_CONFIG_CDM_SET_PHY_INDEX);

	pmppriv->band = (dev_get_band_cap(dvobj->pci_data.ppcidev) >> 1);
	priv_mib->func_off = 1;
...
int rtw_mp_stop_hdl(_adapter *padapter)
{
	int ret = 0;
	u8 status = 0;
	struct mp_priv *pmppriv = &padapter->mppriv;
	...
	status = rtw_mp_phl_config_arg(padapter, MP_CONFIG_CMD_STOP_DUT);
		   ...
		   struct iw_point *wrqu, char *extra)
{
	...
	_rtw_memset(extra, 0, wrqu->length);
	sprintf(extra, "Set mp band to %s", band == 0 ? "2.4GHz" : (band == 1 ? "5GHz" : "6GHz"));
...
#ifdef RTW_WKARD_AP_MP
	if (pmp_priv->channel != (u8)channel) {
		ch_changed = true;
	} else {
		ch_changed = false;
	}
		   ...
		   struct iw_point *wrqu, char *extra)
{
	...
	if (MsetPower == 0) {
	...
	} else if(0 ==  strncmp(input, "default", 7)){
		rtw_mp_get_tmac_power(padapter);
		sprintf(pextra, "Set power dbm :%d\n", pmppriv->txpowerdbm);
	} else {
		if (sscanf(input, "patha=%d,pathb=%d,pathc=%d,pathd=%d", &idx_a, &idx_b, &idx_c, &idx_d) >= 1) {
				...
				#ifdef WKARD_DECIMAL_INPUT
				pmppriv->txpowerdbm = (u16)idx_a;
				#else
				pmppriv->txpowerdbm = (u8)idx_a;
				#endif /*WKARD_DECIMAL_INPUT*/

				pmppriv->bSetTxPower = 1;
				//DBGP("pmppriv->txpowerdbm = %d\n", pmppriv->txpowerdbm);
		} else {
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	input=kmalloc(wrqu->data.length+1,GFP_KERNEL);
    if(input == NULL)
    {
            printk("Kmalloc failed %s %d\n",__FUNCTION__,__LINE__);
            return -EFAULT;
    }
	_rtw_memset(input, 0, wrqu->data.length+1);
	       ...
	       struct iw_point *wrqu, char *extra)
{
	...
	if (strncmp(input, "setbssid=", 8) == 0) {
			...
			SetPacketRx(padapter, _TRUE, _FALSE);
				...
				padapter->mppriv.rx_pktcount, padapter->mppriv.rx_crcerrpktcount, padapter->mppriv.rx_pktcount_filter_out);

	} else if (strncmp(input, "phy", 3) == 0) {
				struct mp_rx_arg rx_arg;

				_rtw_memset((void *)&rx_arg, 0, sizeof(struct mp_rx_arg));
			...
			sprintf(extra, "Mac Received packet OK:%d CRC error:%d",
	...
	} else if (strncmp(input, "drvflt", 6) == 0) {
				struct mp_rx_arg rx_arg;

				_rtw_memset((void *)&rx_arg, 0, sizeof(struct mp_rx_arg));
				rtw_mp_phl_query_rx(padapter, &rx_arg, 2);
				if (rx_arg.cmd_ok)
					sprintf(extra, "Drv Received packet OK:%d CRC error:%d", rx_arg.mp_fltr_rx_ok_cnt, rx_arg.mp_fltr_rx_err_cnt);
				else
					sprintf(extra, "Drv Query Fail !!!");
	...
	} else if (strncmp(input , "smpcfg=" , 7) == 0) {
				int bSmpCfg = 0;
				...
				else if (sscanf(input, "gaina=%d,%s", &gain_offset_val, temp) >= 1)
					path_num = 0;
				else if (sscanf(input, "gainb=%d,%s", &gain_offset_val, temp) >= 1)
					path_num = 1;

				if (0 == strcmp("cck", temp))
					iscck = 1;
				else
					iscck = 0;

				RTW_INFO("%s:set Path:%d gain_offset=%d\n", __func__, path_num, gain_offset_val);
				RTW_INFO("%s:set iscck=%d\n", __func__, iscck);
				...
				if (strncmp(input, "rssia", 5) == 0)
					rssi_path = 0;
				else if (strncmp(input, "rssib", 5) == 0)
					rssi_path = 1;
				else
				...
				for (i = rssi_path ; i <= all_path_num; i++) {
					...
					rtw_mp_phl_rx_rssi(padapter, &rx_arg);
					{
						...
						s16 convert_rssi = (rx_arg.rssi * 100 / 2) - 110 * 100;
						s8 d = convert_rssi / 100;
						s8 f = convert_rssi % 100;
						if (rx_arg.cmd_ok)
							pcar += sprintf(pcar, "Path%d RSSI=%d.%d ", i, d, ((f >= 0) ? f : -f));
						else
							pcar += sprintf(pcar, "Path%d, RSSI Fail\n", i);
					}

				}
	} else if (strncmp(input, "setflt", 6) == 0){
				/*add mac addr filter here*/
				struct mp_rx_arg rx_arg;
				u8 i = 0;

				_rtw_memset((void *)&rx_arg, 0, sizeof(struct mp_rx_arg));

				if (6 == sscanf(input, "setflt,ta=%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &rx_arg.mp_fltr_mac_ta[0],
															          &rx_arg.mp_fltr_mac_ta[1],
															          &rx_arg.mp_fltr_mac_ta[2],
															          &rx_arg.mp_fltr_mac_ta[3],
															          &rx_arg.mp_fltr_mac_ta[4],
															          &rx_arg.mp_fltr_mac_ta[5]))
				{
					/*for (i=0; i<6; i++)
						printk("%x ", rx_arg.mp_fltr_mac_ta[i]);

					printk("\n");*/
					sprintf(extra , "set TA: %hhx:%hhx:%hhx:%hhx:%hhx:%hhx\n",  rx_arg.mp_fltr_mac_ta[0],
																				rx_arg.mp_fltr_mac_ta[1],
																				rx_arg.mp_fltr_mac_ta[2],
																				rx_arg.mp_fltr_mac_ta[3],
																				rx_arg.mp_fltr_mac_ta[4],
				...
				else if (sscanf(input, "setflt,sa=%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &rx_arg.mp_fltr_mac_sa[0],
																	  &rx_arg.mp_fltr_mac_sa[1],
																	  &rx_arg.mp_fltr_mac_sa[2],
																	  &rx_arg.mp_fltr_mac_sa[3],
																	  &rx_arg.mp_fltr_mac_sa[4],
																	  &rx_arg.mp_fltr_mac_sa[5]))
				{
					/*for (i=0; i<6; i++)
						printk("%x ", rx_arg.mp_fltr_mac_sa[i]);

					printk("\n");*/
					sprintf(extra , "set SA: %hhx:%hhx:%hhx:%hhx:%hhx:%hhx\n",  rx_arg.mp_fltr_mac_sa[0],
																				rx_arg.mp_fltr_mac_sa[1],
																				rx_arg.mp_fltr_mac_sa[2],
																				rx_arg.mp_fltr_mac_sa[3],
																				rx_arg.mp_fltr_mac_sa[4],
				...
				rx_arg.mp_class = MP_CLASS_RX;
				rx_arg.cmd = MP_RX_CMD_FILTER_RX_START;
				rx_arg.cmd_ok = 0;
				rtw_mp_set_phl_cmd(padapter, (void*)&rx_arg, sizeof(struct mp_rx_arg));

				while (i != 10) {
					rtw_mp_get_phl_cmd(padapter, (void*)&rx_arg, sizeof(struct mp_rx_arg));
					if (rx_arg.cmd_ok) {
						break;
					} else {
						if (i > 10) {
	...
	} else if (strncmp(input, "reset_flt_cnt", 13) == 0){

				struct mp_rx_arg rx_arg;

				_rtw_memset((void *)&rx_arg, 0, sizeof(struct mp_rx_arg));
				rx_arg.mp_class = MP_CLASS_RX;
				rx_arg.cmd = MP_RX_CMD_FILTER_RX_RESET;
				rx_arg.cmd_ok = 0;
				rtw_mp_set_phl_cmd(padapter, (void*)&rx_arg, sizeof(struct mp_rx_arg));
		     ...
		     struct iw_point *wrqu, char *extra)
{
	...
	if (pmp_priv->rtw_mp_tx_method == MP_PMACT_TX)
		rtw_phl_mp_tx_cmd(padapter, MP_TX_CMD_PHY_OK, pmp_priv->rtw_mp_tx_method, _FALSE);
		  ...
		  struct iw_point *wrqu, char *extra)
{
	...
	if (wrqu->length > 1) {
		...
		input (int/str): [0]:off / [1]:on / [2]:cal for TSSI Tracking", enable);
	}
	wrqu->length = strlen(extra);
	       ...
	       struct iw_point *wrqu, char *extra)
{
	...
	wrqu->length = g6_mp_query_psd(padapter, extra);
		   ...
		   struct iw_point *wrqu, char *extra)
{
	...
	} else if (strncmp(input, "get_xcap", 8) == 0) {
		if (dvobj->ic_id > RTL8834A) {
			_rtw_memset((void *)&cfg_arg, 0, sizeof(cfg_arg));
			cfg_arg.mp_class = MP_CLASS_REG;
			cfg_arg.cmd = MP_REG_CMD_GET_XSI;
			cfg_arg.xsi_offset = 4;
			cfg_arg.xsi_value = 0xFF;
			cfg_arg.cmd_ok = 0;

			rtw_mp_set_phl_cmd(padapter, (void*)&cfg_arg, sizeof(struct mp_reg_arg));

			while (i != 100) {
				rtw_msleep_os(10);
				rtw_mp_get_phl_cmd(padapter, (void*)&cfg_arg, sizeof(struct mp_reg_arg));

				if (cfg_arg.cmd_ok) {
					RTW_INFO("%s,SET CMD OK\n", __func__);
					sprintf(extra, "Get xtal_value = %d\n", cfg_arg.xsi_value);
					break;
				} else {
					RTW_INFO("%s,GET CMD FAIL !!! status %d\n", __func__, cfg_arg.status);
					sprintf(extra, "Get xtal_value failed !!\n");
					i++;
				}
			}
		} else {
			RTW_INFO("ic_id:%d is not support Xtal_si !\n", dvobj->ic_id);
			sprintf(extra, "Only support on 8852B or later IC !!\n");
		}
	}
	wrqu->length = strlen(extra) + 1;
...
int rtw_mp_switch_pwrtrk(struct net_device *dev,
		   struct iw_request_info *info,
		   struct iw_point *wrqu, char *extra)
{
	_adapter *padapter = rtw_netdev_priv(dev);
	...
	input[wrqu->length] = '\0';
	txpwrtrk_status = rtw_atoi(input);

	switch (txpwrtrk_status){

		case 0:
			sprintf(extra, "Set TX power track status: %d (OFF)", txpwrtrk_status);
			break;
		case 1:
			sprintf(extra, "Set TX power track status: %d (ON)", txpwrtrk_status);
			break;
		case 2:
			sprintf(extra, "Set TX power track status: %d (Calibration)", txpwrtrk_status);
			break;
		default:
			sprintf(extra, "input error: %d", txpwrtrk_status);
			break;
	}

	pmppriv->pwr_track_status = txpwrtrk_status;
	rtw_set_txpwr_track_status(padapter, txpwrtrk_status);
			 ...
			 struct iw_point *wrqu, char *extra)
{
	...
	if (wrqu->length == 2) {
	...
	} else if (sscanf(input+1, "dbm=%d,pwr=%d", &tgdbm , &out_pwr) == 2) {
		/*input:adbm=%d,pwr=%d
				bdbm=%d,pwr=%d
				cdbm=%d,pwr=%d
				ddbm=%d,pwr=%d*/
		...
		default:
			...
			tssi_de = (u8)rtw_mp_get_online_tssi_de(padapter, out_pwr, tgdbm, rfpath);
	}
		...
		sprintf(extra, "Invalid command format,please indicate RF path mp_get_tsside [0/1/2/3]\n\
						mp_get_tsside a_dbm=%%d,pwr=%%d\n\
						mp_get_tsside b_dbm=%%d,pwr=%%d\n...\n");
	}
...
int rtw_mp_pretx_proc(_adapter *padapter, u8 bstart, char *extra)
{
	...
	if (bstart == 1 && pmp_priv->mode != MP_ON) {
		...
		pmp_priv->tx.count = 1;
		if (pmp_priv->rtw_mp_tx_method == MP_PMACT_TX) {
			pmp_priv->rtw_mp_tx_method = MP_TMACT_TX;
			rtw_set_phl_packet_tx(padapter, bstart); /* send 1 pkt for trigger HW non-pkt Tx*/
			pmp_priv->rtw_mp_tx_method = MP_PMACT_TX;
		}
		/*SetPacketTx(padapter);*/
	} else
		pmp_priv->mode = MP_ON;
			 ...
			 struct iw_point *wrqu, char *extra)
{
	...
	while (1) {
		...
		if (cmd_arg != NULL && cmd_arg->cmd_ok) {
			RTW_WARN("%s,GET CMD OK !!!\n", __func__);
			break;
		} else {
			i++;
			rtw_msleep_os(10);
			if (i == 3) {
				RTW_WARN("%s,GET CMD FAIL !!!\n", __func__);
				break;
			}
		}
	}
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	if (sscanf(extra, "index=%d,type=%d", &tx_patt_idx, &ppdu_type) > 0) {
		...
		rtw_phl_mp_tx_cmd(padapter, MP_TX_CONFIG_PLCP_PATTERN, pmp_priv->rtw_mp_tx_method, _TRUE);
		_rtw_memset(extra, 0, wrqu->data.length);
		sprintf(extra, "Config Tx Pattern idx %d to %d", pmp_priv->rtw_mp_pmact_patt_idx, tx_patt_idx);
	} else if ((strncmp(extra, "stop", 4) == 0)) {
		rtw_phl_mp_tx_cmd(padapter, MP_TX_CONFIG_PLCP_PATTERN, pmp_priv->rtw_mp_tx_method, _FALSE);
		_rtw_memset(extra, 0, wrqu->data.length);
	...
	if ((strncmp(extra, "ppdu", 4) == 0)) {
		...
		if (sscanf(extra, "ppdu=%d", &ppdu_type) > 0) {
				...
				if (ppdu_type >= RTW_MP_TYPE_HE_MU_OFDMA) {
					...
					for (i = 0;i <= ru_num; i++)
						pextra += sprintf(pextra, "%d : [%s]\n",
												pmp_priv->ru_tone_sel_list[i < 6 ? i : 0],
												RU_TONE_STR(pmp_priv->ru_tone_sel_list[i < 6 ? i : 0]));
...
0:CCK\n1:LEGACY\n2:HT_MF\n3:HT_GF\n4:VHT\n5:HE_SU\n6:HE_ER_SU\n7:HE_MU_OFDMA\n8:HE_TB\n");
		...
		if (sscanf(extra, "giltf=%hhd", &idx) > 0) {
			...
			if (sscanf(extra, "tx_time=%d", &tx_time) > 0) {
				...
				_rtw_memset(extra, 0, wrqu->data.length);
				snprintf(extra, 14, "tx_time fail\n");
			}
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	if ((strncmp(extra, "PMACT", 5) == 0)) {
		...
		pmp_priv->rtw_mp_tx_method = MP_TMACT_TX;
		rtw_phl_mp_tx_cmd(padapter, MP_TX_MODE_SWITCH, pmp_priv->rtw_mp_tx_method, _FALSE);
		sprintf(extra, "set TMACT OK");
	} else if ((strncmp(extra, "FWPMACT", 7) == 0)) {
		pmp_priv->rtw_mp_tx_method = MP_FW_PMAC_TX;
		rtw_phl_mp_tx_cmd(padapter, MP_TX_MODE_SWITCH, pmp_priv->rtw_mp_tx_method, _FALSE);
		sprintf(extra, "set FWPMACT OK");
	} else {
		snprintf(extra, 22, "tx method input fail\n");
	}
	wrqu->data.length = strlen(extra);
	return 0;
}
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	rtw_mp_phl_config_arg(padapter, MP_CONFIG_CMD_SET_DBCC);
	wrqu->data.length = strlen(extra);
	return 0;
}
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	if (strncmp(extra, "iqk", 3) == 0) {
		...
		if (strncmp(extra, "dpk_trk on", 10) == 0) {
			...
			snprintf(extra, 20, "set dpk_trk trigger");
...
int rtw_mp_rx_gain(struct net_device *dev,
			 struct iw_request_info *info,
			 union iwreq_data *wrqu, char *extra)
{
	_adapter *padapter = rtw_netdev_priv(dev);
	...
	if (sscanf(input, "path=%d,gain=%d", &rf_path, &rx_gain) > 0) {
		RTW_INFO("get path=%d, gain=%d\n", rf_path, rx_gain);
	}

	rx_arg.mp_class = MP_CLASS_RX;
	rx_arg.cmd = MP_RX_CMD_SET_GAIN_OFFSET;
	rx_arg.cmd_ok = false;
	rx_arg.offset = rx_gain;
	...
	while (i < 100) {
		...
		if (rx_arg.cmd_ok) {
			RTW_INFO("%s,SET CMD OK\n", __func__);
			sprintf(extra, "set RX gain OK\n");
		} else {
			RTW_INFO("%s,GET CMD FAIL !!! status %d\n", __func__, rx_arg.status);
			sprintf(extra, "set RX gain Failed\n");
		}
		i++;
		rtw_msleep_os(10);
	}
...
int rtw_set_mp_drv_mode(struct net_device *dev,
			struct iw_request_info *info,
			union iwreq_data *wrqu, char *extra)
{
	_adapter *padapter = rtw_netdev_priv(dev);
	struct rtw_phl_com_t *phl_com = GET_HAL_DATA(adapter_to_dvobj(padapter));

	phl_com->drv_mode = RTW_DRV_MODE_MP;

	sprintf(extra, "SET DRV_MODE: RTW_DRV_MODE_MP\n");
	wrqu->data.length = strlen(extra);

	return 0;
}

int rtw_mp_tssi_slp_start(struct net_device *dev,
			struct iw_request_info *info,
			union iwreq_data *wrqu, char *extra)
{
	_adapter *padapter = rtw_netdev_priv(dev);

	rtw_phl_tssi_do_slope(GET_HAL_INFO(adapter_to_dvobj(padapter)));

	//sprintf(extra, "SET tssi_do_slope\n");
	/* Due to SD8 keyword rules, use snprintf instead of sprintf */
	snprintf(extra, strlen("SET tssi_do_slope\n"), "SET tssi_do_slope\n");
...
int rtw_mp_tssi_slp_stop(struct net_device *dev,
			struct iw_request_info *info,
			union iwreq_data *wrqu, char *extra)
{
	_adapter *padapter = rtw_netdev_priv(dev);

	rtw_phl_tssi_finish_slope(GET_HAL_INFO(adapter_to_dvobj(padapter)));

	//sprintf(extra, "SET tssi_finish_slope\n");
	/* Due to SD8 keyword rules, use snprintf instead of sprintf */
	snprintf(extra, strlen("SET tssi_finish_slope\n"), "SET tssi_finish_slope\n");
...
int rtw_mp_get_tssicw(struct net_device *dev,
			struct iw_request_info *info,
			union iwreq_data *wrqu, char *extra,
			u8 path)
{
	...
	tssi_cw = rtw_phl_tssi_get_cw(GET_HAL_INFO(adapter_to_dvobj(padapter)), path);
	snprintf(tssi_cw_str, sizeof(tssi_cw_str), "%d", tssi_cw);
	switch (path) {
		case 0:
			//sprintf(extra, "patha=%d\n", tssi_cw);
			/* Due to SD8 keyword rules, use snprintf instead of sprintf */
			snprintf(extra, (strlen("patha=\n") + strlen(tssi_cw_str)), "patha=%s\n", tssi_cw_str);
			break;
		case 1:
			//sprintf(extra, "pathb=%d\n", tssi_cw);
			/* Due to SD8 keyword rules, use snprintf instead of sprintf */
			snprintf(extra, (strlen("pathb=\n") + strlen(tssi_cw_str)), "pathb=%s\n", tssi_cw_str);
			break;
		default:
			break;
	}
...
int rtw_mp_set_tssi_slp(struct net_device *dev,
			struct iw_request_info *info,
			union iwreq_data *wrqu, char *extra,
			u8 path)
{
	_adapter *padapter = rtw_netdev_priv(dev);
	u32 gain_diff=0, cw_diff=0;
	u8 gain_diff_str[10] = {'\0'}, cw_diff_str[10] = {'\0'};
	...
	if (sscanf(extra, "gain_diff=%x,cw_diff=%x", &gain_diff, &cw_diff) == 2)
		RTW_INFO("get input: gain_diff=0x%x,cw_diff=0x%x\n", gain_diff, cw_diff);
	else {
		RTW_PRINT("input error, format: gain_diff=0x%%x,cw_diff=0x%%x\n");
		return -EFAULT;
	}
	rtw_phl_tssi_current_slope_apply(GET_HAL_INFO(adapter_to_dvobj(padapter)),
					gain_diff, cw_diff, path);

	//sprintf(extra, "set gain_diff=0x%x,cw_diff=0x%x\n", gain_diff, cw_diff);
	/* Due to SD8 keyword rules, use snprintf instead of sprintf */
	snprintf(gain_diff_str, sizeof(gain_diff_str), "%x", gain_diff);
	snprintf(cw_diff_str, sizeof(cw_diff_str), "%x", cw_diff);
	snprintf(extra,
		(strlen("set gain_diff=0x,cw_diff=0x\n") +
		strlen(gain_diff_str) +
		strlen(cw_diff_str)),
		"set gain_diff=0x%s,cw_diff=0x%s\n",
		gain_diff_str, cw_diff_str);
...
int rtw_mp_get_tssi_slp(struct net_device *dev,
			struct iw_request_info *info,
			union iwreq_data *wrqu, char *extra,
			u8 path)
{
	_adapter *padapter = rtw_netdev_priv(dev);
	u32 pwr1=0, pwr2=0, cw1=0, cw2=0;
	u32 gain_diff=0, cw_diff=0;
	u8 gain_diff_str[10] = {'\0'}, cw_diff_str[10] = {'\0'};
	...
	if (sscanf(extra, "pwr1=%d,pwr2=%d,cw1=%d,cw2=%d", &pwr1, &pwr2, &cw1, &cw2) == 4)
		RTW_INFO("get input: pwr1=%d,pwr2=%d,cw1=%d,cw2=%d\n", pwr1, pwr2, cw1, cw2);
	else {
		RTW_PRINT("input error, format: pwr1=%%d,pwr2=%%d,cw1=%%d,cw2=%%d\n"
			"pwr1 & pwr2 last two digits are decimals.\n"
			"if pwr is 18.50, your input should be pwr1=1850\n");
		return -EFAULT;
	}
	rtw_phl_tssi_slope_apply(GET_HAL_INFO(adapter_to_dvobj(padapter)),
				pwr1, pwr2, cw1, cw2, path,
				&gain_diff, &cw_diff);

	//sprintf(extra, " gain_diff=0x%x,cw_diff=0x%x\n", gain_diff, cw_diff);
	/* Due to SD8 keyword rules, use snprintf instead of sprintf */
	snprintf(gain_diff_str, sizeof(gain_diff_str), "%x", gain_diff);
	snprintf(cw_diff_str, sizeof(cw_diff_str), "%x", cw_diff);
	snprintf(extra,
		(strlen(" gain_diff=0x,cw_diff=0x\n") +
		strlen(gain_diff_str) +
		strlen(cw_diff_str)),
		" gain_diff=0x%s,cw_diff=0x%s\n",
		gain_diff_str, cw_diff_str);
...
//	_enter_critical_mutex(&(adapter_to_dvobj(padapter)->ioctrl_mutex), NULL);
	...
	case MP_SetRFPathSwh:
		...
		break;
	case WRITE_REG:
		RTW_INFO("mp_get  WRITE_REG\n");
		status = rtw_mp_write_reg(dev, info, wrqu, extra);
		break;
	case WRITE_RF:
		RTW_INFO("mp_get  WRITE_RF\n");
	...
	case MP_PHYPARA:
		...
		break;
	case MP_BAND:
		RTW_INFO("set case mp_band\n");
		status = rtw_mp_band(dev , info, wrqu, extra);
		break;
	case MP_CHANNEL:
	...
	case READ_RF:
		...
		break;
	case READ_REG_BIT:
		RTW_INFO("mp_get  READ_REG_BIT\n");
		status = rtw_mp_read_reg_bit(dev, info, wrqu, extra);
		break;
	case WRITE_REG_BIT:
		RTW_INFO("mp_get  WRITE_REG_BIT\n");
		status = rtw_mp_write_reg_bit(dev, info, wrqu, extra);
		break;
	case READ_RF_BIT:
		RTW_INFO("mp_get  READ_RF_BIT\n");
		status = rtw_mp_read_rf_bit(dev, info, wrqu, extra);
		break;
	case WRITE_RF_BIT:
		RTW_INFO("mp_get  WRITE_RF_BIT\n");
		status = rtw_mp_write_rf_bit(dev, info, wrqu, extra);
		break;
	case READ_BB_BIT:
		RTW_INFO("mp_get  READ_BB_BIT\n");
		status = rtw_mp_read_bb_bit(dev, info, wrqu, extra);
		break;
	case WRITE_BB_BIT:
		RTW_INFO("mp_get  WRITE_BB_BIT\n");
		status = rtw_mp_write_bb_bit(dev, info, wrqu, extra);
		break;
	case READ_MAC_BIT:
		RTW_INFO("mp_get  READ_MAC_BIT\n");
		status = rtw_mp_read_mac_bit(dev, info, wrqu, extra);
		break;
	case WRITE_MAC_BIT:
		RTW_INFO("mp_get  WRITE_MAC_BIT\n");
		status = rtw_mp_write_mac_bit(dev, info, wrqu, extra);
		break;
	case MP_RATE:
	...
	case MP_HW_TX_MODE:
		...
		break;
	case MP_SWITCH_PWRTRK:
		RTW_INFO("mp_get MP_HW_TX_MODE\n");
		status = rtw_mp_switch_pwrtrk(dev, info, wrqu, extra);
		break;
	case MP_GET_TSSIDE:
	...
	case MP_PHL_RFK:
		...
		break;
	case MP_RX_GAIN:
		RTW_INFO("mp_get MP_RX_GAIN\n");
		status = rtw_mp_rx_gain(dev, info, wdata, extra);
		break;
	case SET_MP_MODE:
		RTW_INFO("set MP_MODE\n");
		status = rtw_set_mp_drv_mode(dev, info, wdata, extra);
		break;
	case MP_TSSI_SLP_START:
		RTW_INFO("set MP_TSSI_SLP_START\n");
		status = rtw_mp_tssi_slp_start(dev, info, wdata, extra);
		break;
	case MP_TSSI_SLP_STOP:
		RTW_INFO("set MP_TSSI_SLP_STOP\n");
		status = rtw_mp_tssi_slp_stop(dev, info, wdata, extra);
		break;
	case MP_GET_TSSICW_A:
		RTW_INFO("mp_get MP_GET_TSSICW_A\n");
		status = rtw_mp_get_tssicw(dev, info, wdata, extra, 0);
		break;
	case MP_GET_TSSICW_B:
		RTW_INFO("mp_get MP_GET_TSSICW_B\n");
		status = rtw_mp_get_tssicw(dev, info, wdata, extra, 1);
		break;
	case MP_SET_TSSI_SLP_A:
		RTW_INFO("mp_set MP_SET_TSSI_SLP_A\n");
		status = rtw_mp_set_tssi_slp(dev, info, wdata, extra, 0);
		break;
	case MP_SET_TSSI_SLP_B:
		RTW_INFO("mp_set MP_SET_TSSI_SLP_B\n");
		status = rtw_mp_set_tssi_slp(dev, info, wdata, extra, 1);
		break;
	case MP_GET_TSSI_SLP_A:
		RTW_INFO("mp_get MP_GET_TSSI_SLP_A\n");
		status = rtw_mp_get_tssi_slp(dev, info, wdata, extra, 0);
		break;
	case MP_GET_TSSI_SLP_B:
		RTW_INFO("mp_get MP_GET_TSSI_SLP_B\n");
		status = rtw_mp_get_tssi_slp(dev, info, wdata, extra, 1);
		break;
	default:
		status = -EIO;
	}
