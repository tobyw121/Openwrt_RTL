...
void halrf_cmd_parser_g6(struct rf_info *rf, char input[][RF_MAX_ARGV],
		      u32 input_num, char *output, u32 out_len)
{
	...
	case HALRF_DUMP_RFK_REG:
		halrf_dump_rfk_reg_g6(rf, input, &used, output, &out_len);
		break;
	case HALRF_IQK:
...
s32 halrf_cmd(struct rf_info *rf, char *input, char *output, u32 out_len)
{
	...
	halrf_cmd_parser_g6(rf, argv, argc, output, out_len);
	return 0;
}
