...
struct pci_device_id rtw_pci_id_tbl[] = {
	...
	{PCI_DEVICE(PCI_VENDER_ID_REALTEK, 0xB814), .driver_data = RTL8852A},
#endif
	...
	{PCI_DEVICE(PCI_VENDER_ID_REALTEK, 0xC852), .driver_data = RTL8852C},/*FPGA*/
#endif
#ifdef CONFIG_RTL8832BR
	{PCI_DEVICE(PCI_VENDER_ID_REALTEK, 0xb832), .driver_data = RTL8832BR},
#endif
#ifdef CONFIG_RTL8192XB
	{PCI_DEVICE(PCI_VENDER_ID_REALTEK, 0x0192), .driver_data = RTL8192XB},
#endif
	{}
};
...
#ifdef PLATFORM_ECOS
void setup_wifi_module_param(void)
{
        extern int rtw_ht_enable;
        extern int rtw_vht_enable;
        extern int rtw_he_enable;
        extern uint rtw_drv_log_level;

        rtw_ht_enable = RTK_SHM_DRAM->wifi_param.rtw_ht_enable;
        rtw_vht_enable = RTK_SHM_DRAM->wifi_param.rtw_vht_enable;
        rtw_he_enable = RTK_SHM_DRAM->wifi_param.rtw_he_enable;
        rtw_drv_log_level = RTK_SHM_DRAM->wifi_param.rtw_drv_log_level;
}
#endif
...
#ifdef CORE_TX_HIGH
extern  enum rtw_phl_status rtw_core_tx_notify(void *phl);
#endif

static __inline__ void schedule_swq_tasklet(struct dvobj_priv *dvobj)
{
	_adapter *padapter = dvobj_get_primary_adapter(dvobj);

	padapter->hw_irq_cnt = (padapter->hw_irq_cnt+ 1 ) % 8192;

#ifdef CORE_TX_HIGH
	rtw_core_tx_notify(padapter->dvobj->phl);
#else
	rtw_tasklet_hi_schedule(&padapter->swq_xmit_tasklet);
#endif
}
#endif

extern u8 rtl8192cd_swq_is_empty(struct dvobj_priv *dvobj);

#ifdef CONFIG_AMSDU_HW_TIMER
static __inline__ void schedule_amsdu_tasklet(struct dvobj_priv *dvobj)
{
	_adapter *padapter = dvobj_get_primary_adapter(dvobj);

	if(padapter->amsdu_hw_timer_enable) {
		padapter->amsdu_hw_irq_cnt = (padapter->amsdu_hw_irq_cnt+ 1 ) % 8192;
		rtw_tasklet_hi_schedule(&padapter->amsdu_xmit_tasklet);
	}
}
#endif
...
static irqreturn_t rtw_pci_interrupt(int irq, void *priv, struct pt_regs *regs)
{
	...
	if (pstatus == RTW_PHL_STATUS_SH_TASK) {
#ifdef CONFIG_AMSDU_HW_TIMER
		if (dvobj->tx_mode == 0) {
			schedule_amsdu_tasklet(dvobj);
		}
#endif
#ifdef CONFIG_VW_REFINE
		if (dvobj->tx_mode == 1) {
			schedule_swq_tasklet(dvobj);
		} else if (dvobj->tx_mode == 0) {
		...
		if (dvobj->tx_mode == 2)
			rtw_tasklet_hi_schedule(&dvobj->txq_tasklet);
#endif
	} else
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)) && !defined(PLATFORM_ECOS)
...
#ifdef CONFIG_ONE_TXQ
extern void rtw_txq_hdl(struct dvobj_priv *dvobj);
#endif
//#if !defined(CONFIG_ONE_TXQ)
extern void service_sta_q_hdl2(struct dvobj_priv *dvobj);
//#endif

void rtw_init_txq(struct dvobj_priv *dvobj)
{
	_rtw_init_queue(&dvobj->ps_trigger_sta_queue);
	_rtw_init_queue(&dvobj->tx_pending_sta_queue);

#ifdef CONFIG_ONE_TXQ
	ATOMIC_SET(&dvobj->txq_total_len, 0);
	dvobj->txq_max_enq_len = 4096;
	dvobj->txq_max_agg_num = MAX_TXQ_AGG;
	dvobj->txq_hw_timeout = 500;
	dvobj->txq_pkt_timeout = TXQ_TIMEOUT;
	dvobj->txq_max_serv_time = 20;
	dvobj->txq_amsdu_merge = 0;
	dvobj->txq_ts_factor = 32;
	dvobj->txq_deq_factor = 0; /* decide by average timout measured */
	dvobj->txq_deq_loop = 8;
	dvobj->txq_debug = 0;
	dvobj->txq_serv_group_exp = 1;

	rtw_tasklet_init(&dvobj->txq_tasklet,
		(void(*)(unsigned long)) rtw_txq_hdl,
		(unsigned long)dvobj);
#endif

//#if !defined(CONFIG_ONE_TXQ)
	rtw_tasklet_init(&dvobj->ps_trigger_tasklet,
		(void(*)(unsigned long)) service_sta_q_hdl2,
		(unsigned long)dvobj);
//#endif
}
...
_adapter *rtw_pci_primary_adapter_init(struct dvobj_priv *dvobj, struct pci_dev *pdev)
{
	...
	/*set default value of rtw_mib*/
	set_rtw_mib_default_tbl(padapter, priv_mib);
#endif
...
static_assert(sizeof(struct mutex) <= sizeof(_mutex),
	"struct mutex is larger than _mutex");
#endif /* PLATFORM_ECOS */
#endif /* CONFIG_WFO_VIRT_MODULE */

#if defined(PLATFORM_ECOS) && defined(CONFIG_WFO_OFFLOAD_2G)
u8 dev_probe_num = 1;
#else
u8 dev_probe_num = 0;
#endif
...
static int rtw_dev_probe(struct pci_dev *pdev, const struct pci_device_id *pdid)
{
...
#ifdef DEBUG_SINGLE_IFACE
	if(dev_probe_num){
		DBGP("DEBUG_SINGLE_IFACE \n");
		return -ENODEV;
	}
#endif
	...
	RTW_PRINT("%s probing pci device \"%s\"...\n", DRV_NAME,
...
#if !defined(CONFIG_WFO_VIRT_SAME_CPU)
#if defined(WFO_SKIP_PCIE_SLOT)
	if (wfo_enable && ((dev_probe_num-1) == WFO_SKIP_PCIE_SLOT)) {
		printk("<WFO> PCIE%d: This device will be offload!!\n", dev_probe_num-1);
		wfo_pci_dev_id = pdid->device;
		goto free_dvobj;
	}
#endif /* WFO_SKIP_PCIE_SLOT */
#endif /* !CONFIG_WFO_VIRT_SAME_CPU */
	...
	if(dev_probe_num)
		dvobj->dev_id = (dev_probe_num-1);
	else
		dvobj->dev_id = 0;

	RTW_PRINT("[%d]padapter=0x%px dev_id=%d \n", dev_probe_num, padapter,
		  dvobj->dev_id);
#endif /* CONFIG_RTW_MULTI_DEV_MULTI_BAND */

	/* Initialize virtual interface */
#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_RTW_SUPPORT_MBSSID_VAP)
...
#ifdef CTC_WIFI_DIAG
	rtw_ctcwifi_adapter_set(padapter);
#endif
...
static void rtw_dev_remove(struct pci_dev *pdev)
{
	...
	if (GET_HAL_DATA(pdvobjpriv)->fw_ready == _TRUE) {
...
#if defined(PLATFORM_LINUX) && defined(CONFIG_WLAN_MANAGER) && !defined(RTW_COMMON_NETLINK)
	rtw_netlink_deinit();
#endif

#if defined(RTW_COMMON_NETLINK)
	rtw_netlink_exit_dev();
#endif
