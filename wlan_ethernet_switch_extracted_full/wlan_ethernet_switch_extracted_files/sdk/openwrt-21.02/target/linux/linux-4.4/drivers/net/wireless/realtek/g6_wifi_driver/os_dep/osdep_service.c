...
__DMEM_SECTION__ struct rtw_mem_stat rtw_mem_type_stat[mstat_tf_idx(MSTAT_TYPE_MAX)];
#ifdef RTW_MEM_FUNC_STAT
__DMEM_SECTION__ struct rtw_mem_stat rtw_mem_func_stat[mstat_ff_idx(MSTAT_FUNC_MAX)];
#endif
...
__DMEM_SECTION__ struct mstat_sniff_rule mstat_sniff_rules[] = {
	{MSTAT_TYPE_PHY, 4097, SIZE_MAX},
};

__DMEM_SECTION__ int mstat_sniff_rule_num = sizeof(mstat_sniff_rules) / sizeof(struct mstat_sniff_rule);
...
void *rtw_malloc2d_g6(int h, int w, size_t size)
{
	int j;
...
void rtw_mfree2d_g6(void *pbuf, int h, int w, int size)
{
...
{
	/* workaround, check list_empty to avoid infinite loop issue */
	if ((head == plist) || rtw_is_list_empty(plist))
		return _TRUE;
	else
		return _FALSE;
}
...
void rtw_blacklist_expire(_queue *blist)
{
	struct blacklist_ent *ent = NULL;
	_list *list, *head;
...
int rtw_blacklist_find_in_list(_queue *blist, const u8 *addr)
{
	struct blacklist_ent *ent = NULL;
	_list *list, *head;
	u8 exist = _FALSE;

	RTW_DBG("mac(%2X:%2X:%2X:%2X:%2X:%2X)\n",
	...
	while (rtw_end_of_queue_search(head, list) == _FALSE) {
		...
		list = get_next(list);
		if (_rtw_memcmp(ent->addr, addr, ETH_ALEN) == _TRUE) {
			RTW_DBG("!!!!!!find this mac in black list\n!!!!!");
			exist = _TRUE;
			break;
		}
	}
