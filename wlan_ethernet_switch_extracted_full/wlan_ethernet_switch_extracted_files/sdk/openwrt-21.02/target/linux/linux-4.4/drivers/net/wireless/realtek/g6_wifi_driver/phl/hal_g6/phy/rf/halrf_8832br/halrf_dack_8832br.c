...
void halrf_afe_init_8832br(struct rf_info *rf)
{
#if 0
	halrf_wreg(rf, 0xc0d4, MASKDWORD, 0xABC41460);
	halrf_wreg(rf, 0xc0d8, MASKDWORD, 0xC6FA7EE7);
	halrf_wreg(rf, 0xc0dc, MASKDWORD, 0x30c108c8);
	halrf_wreg(rf, 0xc0e0, MASKDWORD, 0x75008128);
	halrf_wreg(rf, 0xc0e4, MASKDWORD, 0x0000272b);
	halrf_wreg(rf, 0xc1d4, MASKDWORD, 0xABC41460);
	halrf_wreg(rf, 0xc1d8, MASKDWORD, 0xC6FA7EE7);
	halrf_wreg(rf, 0xc1dc, MASKDWORD, 0x30c108c8);
	halrf_wreg(rf, 0xc1e0, MASKDWORD, 0x75008128);
	halrf_wreg(rf, 0xc1e4, MASKDWORD, 0x0000272b);

	halrf_wreg(rf, 0xc0ec, MASKDWORD, 0x00010003);
	halrf_wreg(rf, 0xc1ec, MASKDWORD, 0x00010003);
	halrf_wreg(rf, 0xc004, MASKDWORD, 0x460C0000);
	halrf_wreg(rf, 0xc024, MASKDWORD, 0x460C0000);
	halrf_wreg(rf, 0xc104, MASKDWORD, 0x460C0000);
	halrf_wreg(rf, 0xc124, MASKDWORD, 0x460C0000);
	halrf_wreg(rf, 0xc0e8, MASKDWORD, 0x000A0C81);
	halrf_wreg(rf, 0xc0f0, MASKDWORD, 0x00000024);
	halrf_wreg(rf, 0xc1e8, MASKDWORD, 0x000A0C81);
	halrf_wreg(rf, 0xc1f0, MASKDWORD, 0x00000024);
	halrf_wreg(rf, 0xc0c4, MASKDWORD, 0x004E3A00);
#endif
}
void halrf_dack_init_8832br(struct rf_info *rf)
{

}

void halrf_dack_reset_8832br(struct rf_info *rf, enum rf_path path)
{
	if (path == RF_PATH_A) {
...
void halrf_drck_8832br(struct rf_info *rf)
{
	u32 c;
	u32 rck_d;
...
void halrf_addck_backup_8832br(struct rf_info *rf)
{
...
void halrf_addck_reload_8832br(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	/*S0*/
...
void halrf_dack_backup_s0_8832br(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
...
void halrf_dack_backup_s1_8832br(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
...
void halrf_dack_reload_by_path_8832br(struct rf_info *rf, enum rf_path path, u8 index)
{
	struct halrf_dack_info *dack = &rf->dack;
...
void halrf_dack_reload_8832br(struct rf_info *rf, enum rf_path path)
{
	u8 i;

	for (i = 0; i < 2; i++)
		halrf_dack_reload_by_path_8832br(rf, path, i);
}

void halrf_check_addc_8832br(struct rf_info *rf, enum rf_path path)
{
...
void halrf_addck_ori_8832br(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c;
	/*S0*/
...
//	halrf_wreg(rf, 0xc0d4, BIT(1), 0x0);
	halrf_wreg(rf, 0xc0d4, BIT(4), 0x0);
...
//	halrf_wreg(rf, 0xc1d4, BIT(1), 0x1);
	halrf_wreg(rf, 0xc1d4, BIT(4), 0x1);
	/*3.release ADC reset*/
	halrf_wreg(rf, 0x030c, 0x0f000000, 0x3);
...
//	halrf_wreg(rf, 0xc1d4, BIT(1), 0x0);
	halrf_wreg(rf, 0xc1d4, BIT(4), 0x0);
	halrf_wreg(rf, 0x032c, BIT(16), 0x1);
	halrf_wreg(rf, 0x030c, 0x0f000000, 0xc);
	halrf_wreg(rf, 0x032c, BIT(30), 0x1);
...
void halrf_addck_8832br(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c;
	/*S0*/
...
void halrf_check_dadc_8832br(struct rf_info *rf, enum rf_path path)
{
	halrf_wreg(rf, 0x032c, BIT(30), 0x0);
	halrf_wreg(rf, 0x030c, 0x0f000000, 0xf);
	halrf_wreg(rf, 0x030c, 0x0f000000, 0x3);
	...
	halrf_check_addc_8832br(rf, path);
	if (path == RF_PATH_A) {
...
void halrf_dack_8832br_s0(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c = 0;
...
//	halrf_wreg(rf, 0xc024, 0xfff00000, 0x30);
	...
	halrf_dack_reset_8832br(rf, RF_PATH_A);
...
//	halrf_check_dadc_8832br(rf, RF_PATH_A);

	/*backup here*/
	halrf_dack_backup_s0_8832br(rf);
	halrf_dack_reload_8832br(rf, RF_PATH_A);
	/*step 6: Set DAC & clk to normal mode */
	halrf_wreg(rf, 0x12b8, BIT(30), 0x0);
}

void halrf_dack_8832br_s1(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c = 0;
...
//	halrf_wreg(rf, 0xc124, 0xfff00000, 0x30);
	...
	halrf_dack_reset_8832br(rf, RF_PATH_B);
...
//	halrf_check_dadc_8832br(rf, RF_PATH_A);
	...
	/*backup here*/
	halrf_dack_backup_s1_8832br(rf);
	halrf_dack_reload_8832br(rf, RF_PATH_B);
	/*step 6: Set DAC & clk to normal mode */
	halrf_wreg(rf, 0x32b8, BIT(30), 0x0);
}

void halrf_dack_8832br(struct rf_info *rf)
{
	halrf_dack_8832br_s0(rf);
	halrf_dack_8832br_s1(rf);
}

void halrf_dack_dump_8832br(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u8 i;
	u8 t;
...
void halrf_dack_manual_off_8832br(struct rf_info *rf)
{
	...
	halrf_wreg(rf, 0xc238, BIT(0), 0x0);
	halrf_wreg(rf, 0xc24c, BIT(0), 0x0);
}
void halrf_dac_cal_8832br(struct rf_info *rf, bool force)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 rf0_0, rf1_0;
	u8 phy_map;
	...
	halrf_afe_init_8832br(rf);
	halrf_drck_8832br(rf);
	halrf_dack_manual_off_8832br(rf);
#if 1
	...
	halrf_rxck_force_8832br(rf, RF_PATH_A, true, ADC_960M);
	halrf_rxck_force_8832br(rf, RF_PATH_B, true, ADC_960M);
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_START);
	halrf_addck_ori_8832br(rf);
	halrf_rxck_force_8832br(rf, RF_PATH_A, false, ADC_960M);
	halrf_rxck_force_8832br(rf, RF_PATH_B, false, ADC_960M);
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_STOP);
	halrf_addck_backup_8832br(rf);
	halrf_addck_reload_8832br(rf);
...
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_START);
	...
	halrf_dack_8832br(rf);
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_STOP);
	halrf_dack_dump_8832br(rf);
	dack->dack_done = true;
