...
enum rtw_phl_status phl_snd_init_snd_grp(struct phl_info_t *phl_info)
{
	...
	u8 i = 0;
	do {
		//Comparing an array to null is not useful: "param->snd_grp == NULL", since the test will always evaluate as true
		#if 0
		    ...
		    u16 *macid, u8 num_sta, bool he, bool mu)
{
	...
	do {
		...
		snd_grp->wrole_idx = wrole->id;
		snd_grp->band = wrole->hw_band;
...
rtw_phl_sound_start_ex(void *phl, u8 wrole_idx, u8 st_dlg_tkn, u8 period, u8 test_flag)
{
	...
	wrole = rtw_phl_get_wrole_by_ridx(phl_info->phl_com, wrole_idx);
	do {
...
rtw_phl_sound_start(void *phl, u8 wrole_idx, u8 st_dlg_tkn, u8 period, u8 test_flag)
{
	...
	do {
		...
		snd_param->is_zld_exist_csi_sta_cnt= 0;
#ifdef TX_BEAMFORMING
		rtw_phl_mac_set_rxd_zld_en(phl_info, 1);
#endif
		/* start with grouping */
...
rtw_phl_sound_down_ev(void *phl)
{
	...
	do {
		...
		wrole = rtw_phl_get_wrole_by_ridx(phl_info->phl_com, snd_param->snd_grp[cur_idx].wrole_idx);
...
rtw_phl_sound_abort(void *phl)
{
	...
	_os_cancel_timer(d, &(snd->snd_timer));
#ifdef TX_BEAMFORMING
        rtw_phl_mac_set_rxd_zld_en(phl_info, 0);
#endif
	snd_param->test_flag= PHL_SND_TEST_F_FORCE_REMOVE_GRP;
	status = phl_snd_cmd_sound_evt(phl_info,
				       wrole,
				       &(snd_param->snd_grp[0]),
...
void
rtw_phl_get_bf_gid_pid(void *phl, struct rtw_phl_stainfo_t *phl_sta, u8* gid, u8* pid)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	rtw_hal_bf_get_gid_pid(phl_info, phl_sta, gid , pid);
}


enum rtw_phl_status
rtw_phl_check_bf_entry(void *phl, struct rtw_phl_stainfo_t *phl_sta)
{
struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct hal_bf_entry *bf_entry = (struct hal_bf_entry *)phl_sta->hal_sta->bf_entry;
	if(bf_entry)
...
phl_snd_func_snd_init(struct phl_info_t *phl_info)
{
	...
	u8 f_ru_tbl_160m[MAX_SND_HE_BFRP_USER_NUM][MAX_SND_HE_BFRP_USER_NUM] = {
		{RTW_HE_RU2x996_1, RTW_HE_RU2x996_1, RTW_HE_RU2x996_1 ,RTW_HE_RU2x996_1},
		{RTW_HE_RU996_1, RTW_HE_RU996_1, RTW_HE_RU996_1 ,RTW_HE_RU996_1},
		{RTW_HE_RU996_1, RTW_HE_RU484_1, RTW_HE_RU484_2 ,RTW_HE_RU996_1},
		{RTW_HE_RU484_1, RTW_HE_RU484_2, RTW_HE_RU484_1 ,RTW_HE_RU484_2}
	};
	...
	u8 f_ru_tbl_40m[MAX_SND_HE_BFRP_USER_NUM][MAX_SND_HE_BFRP_USER_NUM] = {
		{RTW_HE_RU484_1, RTW_HE_RU484_1, RTW_HE_RU484_1 ,RTW_HE_RU484_1},
		{RTW_HE_RU242_1, RTW_HE_RU242_2, RTW_HE_RU484_1 ,RTW_HE_RU484_1},
		{RTW_HE_RU242_1, RTW_HE_RU106_3, RTW_HE_RU106_4 ,RTW_HE_RU484_1},
		{RTW_HE_RU106_1, RTW_HE_RU106_2, RTW_HE_RU106_3 ,RTW_HE_RU106_4}
	};
	...
	u8 f_ru_tbl_20m[MAX_SND_HE_BFRP_USER_NUM][MAX_SND_HE_BFRP_USER_NUM] = {
		...
		{RTW_HE_RU106_1, RTW_HE_RU106_2, RTW_HE_RU242_1, RTW_HE_RU242_1},
	...
	snd->snd_param.is_zld_exist_csi_sta_cnt= 0;
	...
	_os_mem_cpy(d, snd->snd_param.fix_param.f_ru_tbl_40, f_ru_tbl_40m,
		    MAX_SND_HE_BFRP_USER_NUM * MAX_SND_HE_BFRP_USER_NUM);
	_os_mem_cpy(d, snd->snd_param.fix_param.f_ru_tbl_80, f_ru_tbl_80m,
		    MAX_SND_HE_BFRP_USER_NUM * MAX_SND_HE_BFRP_USER_NUM);
	_os_mem_cpy(d, snd->snd_param.fix_param.f_ru_tbl_160, f_ru_tbl_160m,
		    MAX_SND_HE_BFRP_USER_NUM * MAX_SND_HE_BFRP_USER_NUM);
...
u8 _phl_snd_check_mu_tx_rate(struct rtw_phl_stainfo_t *phl_sta)
{
	u8 ret = false;
	u16 rate;

	rate = phl_sta->hal_sta->ra_info.rpt_rt_i.mcs_ss_idx;

	switch (phl_sta->hal_sta->ra_info.rpt_rt_i.mode) {
		...
		case HAL_HT_MODE:
			...
			break;
		case HAL_VHT_MODE:
			if(((rate + 0x100) >= RTW_DATA_RATE_VHT_NSS1_MCS7) && ((rate + 0x100) <= RTW_DATA_RATE_VHT_NSS1_MCS9))
				ret = true;
			break;
		case HAL_HE_MODE:
			if(((rate + 0x180) >= RTW_DATA_RATE_HE_NSS1_MCS7) && ((rate + 0x180) <= RTW_DATA_RATE_HE_NSS1_MCS11))
				ret = true;
			break;
		default:
...
u8 _phl_snd_check_tx_rate(struct rtw_phl_stainfo_t *phl_sta)
{
	u8 ret = false;
	u16 rate;

#if 0
	if(phl_sta->asoc_cap.nss_rx > 1)
	{
		return ret;
	}
#endif
	...
	switch (phl_sta->hal_sta->ra_info.rpt_rt_i.mode) {
		...
		case HAL_HT_MODE:
			...
			break;
		case HAL_VHT_MODE:
			if(((rate + 0x100) >= RTW_DATA_RATE_VHT_NSS1_MCS3) && ((rate + 0x100) <= RTW_DATA_RATE_VHT_NSS1_MCS9))
				ret = true;
			break;
		case HAL_HE_MODE:
			if(((rate + 0x180) >= RTW_DATA_RATE_HE_NSS1_MCS0) && ((rate + 0x180) <= RTW_DATA_RATE_HE_NSS1_MCS11))
				ret = true;
			break;
		default:
...
enum rtw_phl_status
phl_snd_func_remove_grp_check(struct phl_info_t *phl_info, struct phl_snd_grp *grp)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_SUCCESS;
	struct phl_sound_obj *snd = (struct phl_sound_obj *)phl_info->snd_obj;
	struct phl_sound_param *snd_param = &snd->snd_param;
	...
	if (grp == NULL) {
		PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "%s :group is NULL!!!\n", __func__);
		return pstatus;
	}
	wrole = rtw_phl_get_wrole_by_ridx(phl_info->phl_com, grp->wrole_idx);
	...
	else
	{
		...
		if(grp ->num_sta ==1) {
		       ...
		       if (sta && sta->is_zld_exist_csi &&(sta->asoc_cap.trig_su_bfm_fb ==0)){
			   	   snd_param->is_zld_exist_csi_sta_cnt++;
				   pstatus = RTW_PHL_STATUS_FAILURE;
				   goto out;
		...
		for (idx = 0; idx < grp->num_sta; idx++) {
			...
			if (NULL == sta || sta->active == false)
			{
   
			if (NULL != sta)
				PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s %d:	 macid %d , sta->active = %d\n", __func__,__LINE__,sta->macid,sta->active);
				pstatus = RTW_PHL_STATUS_FAILURE;
				goto out;
			}
			else
			{
				if((snd_param->test_flag & PHL_SND_TEST_F_FORCE_TXBF_MU_1SS) && !_phl_snd_check_tx_rate(sta)){
					PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "%s %d:_phl_snd_check_tx_rate is fail!!!\n", __func__,__LINE__);
					pstatus = RTW_PHL_STATUS_FAILURE;
					}
				if(!(snd_param->test_flag & PHL_SND_TEST_F_DISABLE_TP_LIMIT) && ((sta->stats.tx_tp_kbits >> 10) < MIN_SND_TP))
					{
					PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "%s %d:  macid=%d , TP is fail!!!\n", __func__,__LINE__,sta->macid);
					pstatus = RTW_PHL_STATUS_FAILURE;
					}
				if((grp->num_sta == MAX_NUM_STA_SND_GRP) && (snd_param->test_flag & PHL_SND_TEST_F_FORCE_TXBF_MU_1SS) && !_phl_snd_check_mu_tx_rate(sta))
					{
					PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "%s %d:_phl_snd_check_mu_tx_rate is fail!!!\n", __func__,__LINE__);
					pstatus = RTW_PHL_STATUS_FAILURE;
					}

				if(pstatus == RTW_PHL_STATUS_FAILURE)
				{
					if(snd_sta->del_cnt < STABLE_AD_CNT)
					{
						snd_sta->del_cnt++;
	...
	if(snd_param->test_flag & PHL_SND_TEST_F_FORCE_REMOVE_GRP)
	{
		PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s %d: gidx=%d , PHL_SND_TEST_F_FORCE_REMOVE_GRP!!!\n", __func__,__LINE__,grp->gidx);
...
#if 0
	//MU Grouping
	if(snd_param->test_flag & PHL_SND_TEST_F_ENABLE_MU_SND)
	{
		if(grp->num_sta < MAX_NUM_STA_SND_GRP)
		{
			//Check assoc_num over 2
			struct rtw_wifi_role_t *wrole = rtw_phl_get_wrole_by_ridx(phl_info->phl_com, grp->wrole_idx);
			PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "MU Grouping Check wrole index %d\n", grp->wrole_idx);
			if(wrole != NULL)
			{
				void *drv = phl_to_drvpriv(phl_info);
				struct phl_queue *sta_queue = &wrole->assoc_sta_queue;
				_os_spinlockfg sp_flags;
				_os_spinlock(drv, &sta_queue->lock, _irq, &sp_flags);
				if(sta_queue->cnt >= 3 /*&& _phl_snd_check_mu_tp(sta)*/ && _phl_snd_check_mu_tx_rate(sta))
				{
					PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "%s %d:\n", __func__,__LINE__);
					pstatus = RTW_PHL_STATUS_FAILURE;
				}
				_os_spinunlock(drv, &sta_queue->lock, _irq, &sp_flags);
				PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "MU Grouping Check sta_queue->cnt %d\n", sta_queue->cnt);
			}
		}
		else
		{
...
enum rtw_phl_status
rtw_phl_set_fw_txcmd_num(void *phl, bool force_to_one){
	...
	mac_fix_tbl->fix_txcmdnum_en = 1;
	mac_fix_tbl->force_to_one = force_to_one;
	pstatus = rtw_phl_mac_set_fixmode_mib(phl_info, mac_fix_tbl);

	if (pstatus != RTW_PHL_STATUS_SUCCESS) {
		PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_,
		  "[%s] Set mac fixmode mib Fail\n",__func__);
		return pstatus;
	}
	return pstatus;
}
...
rtw_phl_get_fw_txcmd_num(void *phl){
	...
	return mac_fix_tbl->force_to_one;
}
/**
...
phl_snd_func_remove_grp_all(struct phl_info_t *phl_info)
{
	...
	for(idx = 0; idx < MAX_SND_GRP_NUM; idx++) {
		...
		if (grp != NULL) {
			//check
			if(phl_snd_func_remove_grp_check(phl_info, grp) == RTW_PHL_STATUS_FAILURE)
			{
...
phl_snd_func_grouping(struct phl_info_t *phl_info, u8 wroleidx)
{
	...
	struct rtw_wifi_role_t *wrole = NULL;
	struct rtw_phl_stainfo_t *self = NULL, *sta, *p_sta;
	struct phl_snd_grp *grp = NULL;
	void *drv = phl_to_drvpriv(phl_info);
	struct phl_queue *sta_queue;
	u8 gidx = 0;
	u8 cnt=0,num_sta_cnt = 0,num_grp_cnt=0;
	bool is_sta_exist_grp=false,wait_to_group_mu=false;
	u8 mu_grp_index = 0;
	...
	if (rtw_phl_role_is_client_category(wrole) && !phl_info->phl_com->txbf_auto_snd ) {
			...
			if(!sta->hal_sta->ra_info.ra_registered)
		{
			...
			if((sta->asoc_cap.vht_su_bfme==0) && (sta->asoc_cap.he_su_bfme==0))  //sta not support bfee
			{
				continue;
			}
			if((snd_param->test_flag & PHL_SND_TEST_F_FORCE_TXBF_MU_1SS) && !_phl_snd_check_tx_rate(sta))
			...
			if(!(snd_param->test_flag & PHL_SND_TEST_F_DISABLE_TP_LIMIT) && ((sta->stats.tx_tp_kbits >> 10) < MIN_SND_TP))
		{
			...
			if((snd_param->test_flag & PHL_SND_TEST_F_FORCE_TXBF_MU_1SS) && _phl_snd_check_mu_tx_rate(sta))
			{
				if(sta->mu_add_cnt < STABLE_AD_CNT)
					sta->mu_add_cnt++;
			}
			else
				sta->mu_add_cnt = 0;
				
			//STA already have bf_entry
			...
			if(snd_param->test_flag&PHL_SND_TEST_F_ENABLE_MU_SND)
			{
				...
				if(mu_grp_cnt < MAX_SND_MU_GRP_NUM)
				{
					for (gidx = 0; gidx < MAX_SND_GRP_NUM; gidx++)
					{
						grp = &snd_param->snd_grp[gidx];
						if(grp->num_sta == 1)
						{
							p_sta = rtw_phl_get_stainfo_by_macid(phl_info, grp->sta[0].macid);
							if(p_sta && (p_sta->active == true)
								&&(((p_sta->wmode&WLAN_MD_11AX) && (sta->wmode&WLAN_MD_11AX))
								||(((p_sta->wmode&WLAN_MD_11AC) && (sta->wmode&WLAN_MD_11AC)) 
								&& p_sta->asoc_cap.vht_mu_bfme  && sta->asoc_cap.vht_mu_bfme ))
								&& (p_sta->chandef.bw == sta->chandef.bw ))
							{
								if((snd_param->test_flag&PHL_SND_TEST_F_FORCE_TXBF_MU_1SS) &&
									(p_sta->asoc_cap.max_nc < 1) && (sta->asoc_cap.max_nc < 1)
									&& (p_sta->mu_add_cnt >= STABLE_AD_CNT) && (sta->mu_add_cnt >= STABLE_AD_CNT)
									/*&& _phl_snd_check_mu_tp(p_sta) && _phl_snd_check_mu_tp(sta)*/)
								{
									cnt = 1;
									break;
								}
								else if(!(snd_param->test_flag&PHL_SND_TEST_F_FORCE_TXBF_MU_1SS))
								{
									cnt = 1;
									break;
								}
								else{
									wait_to_group_mu = true;
									break;
								}
							}
						}
		...
		for (num_grp_cnt = 0; num_grp_cnt < MAX_SND_GRP_NUM; num_grp_cnt++) {
			grp = &snd_param->snd_grp[num_grp_cnt];

			for (num_sta_cnt = 0; num_sta_cnt < grp->num_sta; num_sta_cnt++){
				if (grp->sta[num_sta_cnt].macid == sta->macid){
					is_sta_exist_grp = true;
				}
			}
			
		}
			/* First STA */
if (is_sta_exist_grp == false){
			if (cnt == 0) {//TXBF or MU first
				#ifdef TX_BEAMFORMING
				if (sta->is_zld_exist_csi && (sta->asoc_cap.trig_su_bfm_fb ==0)  && (snd_param->is_zld_exist_csi_sta_cnt % 2)){
					continue;
					}
				#endif
				gidx = _phl_snd_get_available_gidx(snd);
				...
				pstatus = _phl_snd_func_grp_add_sta(phl_info, sta, gidx);
				cnt++;
				if (cnt >= MAX_NUM_STA_SND_GRP)
					cnt = 0;
			}
}
		}//end of phl_list_for_loop
		_os_spinunlock(drv, &sta_queue->lock, _bh, NULL);
		//if(pstatus != RTW_PHL_STATUS_SUCCESS)
			//return RTW_PHL_STATUS_FAILURE;


		for (cnt = 0; cnt < MAX_SND_GRP_NUM; cnt++) {
			grp = &snd_param->snd_grp[cnt];
			...
			if(grp->num_sta != 0)
			{
				...
				{
					_phl_snd_func_set_grp_fb_mu(grp);
					grp->mu_grp_index = mu_grp_index;
					mu_grp_index += 2;
				}

				//if((snd_param->test_flag&PHL_SND_TEST_F_OFFLOAD)
					//&& (grp->snd_offload_mode == SND_OFFLOAD_DISABLE))
					//grp->snd_offload_mode = SND_OFFLOAD_START;
			}
		}
#endif
	}
	...
	for (idx = 0; idx < grp->num_sta; idx++) {
		...
		if (true == qry_bf) {
			hstatus = rtw_hal_snd_query_proc_sta_res(
					...
					grp->en_swap_mode);
			if (hstatus != RTW_HAL_STATUS_SUCCESS) {
				PHL_ERR("_phl_snd_proc_get_res_bf: macid 0x%x query sounding resource FAIL \n",
				snd_sta->macid);
				if (grp->en_swap_mode) {
	...
	for (i = 0; i < MAX_NUM_STA_SND_GRP; i++) {
		...
		for (j = 0; j < MAX_NUM_STA_SND_GRP; j++) {
			...
			if (1UL > bfmu_idx_tmp){
				...
				PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "sta(macid = 0x%x) , invalid bfmu_idx_tmp = 0x%x \n",
			 	psta_info->macid, bfmu_idx_tmp);
			}
		}
...
phl_snd_proc_precfg(struct phl_info_t *phl_info, struct phl_snd_grp *grp)
{
	...
	if(grp && (pstatus != RTW_PHL_STATUS_SUCCESS))
		grp->snd_sts = PHL_SND_STS_FAILURE;
...
void
_phl_snd_proc_fw_cmd_he_tb_1sta(struct phl_info_t *phl_info,
				...
				u8 *cmd, u8 bfrp_num)
{
	...
	if (bfrp_num == 1) {
		...
		else if (CHANNEL_WIDTH_40 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_40[1][0];/* Fixed 40MHz RU Table of 2 STA */
		else if (CHANNEL_WIDTH_80 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_80[1][0];/* Fixed 80MHz RU Table of 2 STA */
		else 
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_160[1][0];/* Fixed 160MHz RU Table of 2 STA */
	} else {
		if (CHANNEL_WIDTH_20 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_20[0][0];/* Fixed 20MHz RU Table of 1 STA */
		else if (CHANNEL_WIDTH_40 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_40[0][0];/* Fixed 40MHz RU Table of 1 STA */
		else if (CHANNEL_WIDTH_80 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_80[0][0];/* Fixed 80MHz RU Table of 1 STA */
		else 
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_160[0][0];/* Fixed 160MHz RU Table of 1 STA */
	}
				...
				u8 *cmd, u8 bfrp_num)
{
	...
	rtw_hal_snd_ax_fwcmd_tb_pri(phl_info->hal, cmd, grp->sta[0].bw,
		sta_info, (bfrp_num == 1) ? grp->num_sta : 1, (bfrp_num == 1) ? 0 : 1);
				...
				u8 *cmd, u8 bfrp_num)
{
	...
	if (bfrp_num == 1) {
		if (CHANNEL_WIDTH_20 == grp->sta[0].bw)
			...
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_40[2][0];/* Fixed 40MHz RU Table of 3 STA */
		else if (CHANNEL_WIDTH_80 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_80[2][0];/* Fixed 80MHz RU Table of 3 STA */
		else 
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_160[2][0];/* Fixed 160MHz RU Table of 3 STA */
	} else {
		if (CHANNEL_WIDTH_20 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_20[1][0];/* Fixed 20MHz RU Table of 2 STA */
		else if (CHANNEL_WIDTH_40 == grp->sta[0].bw)
				...
				u8 *cmd, u8 bfrp_num)
{
	...
	if (bfrp_num == 1) {
		if (CHANNEL_WIDTH_20 == grp->sta[0].bw)
			...
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_40[3][0];/* Fixed 40MHz RU Table of 4 STA */	
		else if (CHANNEL_WIDTH_80 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_80[3][0];/* Fixed 80MHz RU Table of 4 STA */
		else 
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_160[3][0];/* Fixed 160MHz RU Table of 4 STA */
	} else {
		if (CHANNEL_WIDTH_20 == grp->sta[0].bw)
			f_ru_tbl = &snd->snd_param.fix_param.f_ru_tbl_20[1][0];/* Fixed 20MHz RU Table of 2 STA */
		else if (CHANNEL_WIDTH_40 == grp->sta[0].bw)
			       ...
			       struct phl_snd_grp *grp)
{
	...
	do {
		...
		if (NULL == sta_info) {
			PHL_ERR("%s: Cannot find STA macid 0x%x in PHL STA Info List \n",__func__, grp->sta[0].macid);
			break;
		}
		...
		case PHL_SND_TYPE_HE_SW:
		{
			...
			if (grp->num_sta == 1) {
				...
				if ((phl_info->phl_com->su_trig_enable)
#ifdef TX_BEAMFORMING
					|| (sta_info->is_zld_exist_csi && sta_info->asoc_cap.trig_su_bfm_fb)
#endif
					)

						_phl_snd_proc_fw_cmd_he_tb_1sta(phl_info, grp, cmd, 1);
					else
					...
					struct phl_snd_grp *grp)
{
	...
	for (i = 0; i < MAX_NUM_STA_SND_GRP; i++) {
		...
		if((bfmu_idx % 2) == 0)
		{
			...
			gid = _get_mu_mimo_gid_2sta(0, (grp->mu_grp_index >> 1) + 1);
			pid = _get_mu_mimo_gid_2sta((grp->mu_grp_index >> 1) + 1, 0);
			//RTW_PRINT("sta->macid = %d, gid= %d , pid = %d\r\n", sta->macid, gid,pid);
		}
		else
		{
			gid = _get_mu_mimo_gid_2sta((grp->mu_grp_index >> 1) + 1, 0);
			pid = _get_mu_mimo_gid_2sta(0, (grp->mu_grp_index >> 1) + 1);
			//RTW_PRINT("sta->macid = %d, gid= %d , pid = %d\r\n", sta->macid, gid,pid);
		}
		
		rtw_hal_bf_set_gid_pid(phl_info->hal, psta_info, gid, pid);
#else		
				...
				struct phl_snd_grp *grp)
{
	...
	for (idx = 0; idx < MAX_NUM_STA_SND_GRP; idx++) {
		...
		{
			u8 gid = 0, pid = 0;
			rtw_hal_bf_get_gid_pid(phl_info->hal, psta_info, &gid, &pid);
			if(gid != 0)
			{
				rtw_hal_bf_set_txmu_para(phl_info->hal, gid, 1,
			 		HAL_PROT_NO_PROETCT, HAL_ACK_N_MINUS_1USER_BA);
				hstatus = rtw_hal_bf_set_fix_mode(phl_info->hal, sta->macid, mu, he);
			}
			else
			{
				PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "ERROR: gid is ZERO\n");
			}
...
enum rtw_phl_status
rtw_phl_mac_mu_set_dl_grp_info(void *phl,
			       struct phl_snd_grp *grp)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	...
	info.macid_u0 = grp->sta[0].macid;
	info.macid_u1 = grp->sta[1].macid;
	info.macid_u2 = 0xff;
	info.macid_u3 = 0xff;
	info.macid_u4 = 0xff;
	info.macid_u5 = 0xff;
	...
	if (RTW_HAL_STATUS_SUCCESS == rtw_hal_mac_mu_set_dl_grp_info(phl_info->hal, &info)) {
		pstatus = RTW_PHL_STATUS_SUCCESS;
		PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s Success\n", __func__);
...
phl_snd_proc_postcfg(struct phl_info_t *phl_info, struct phl_snd_grp *grp)
{
	...
	hstatus = rtw_phl_mac_mu_set_dl_grp_info(phl_info, grp);
			#if 0
...
phl_snd_proc_chk_condition(struct phl_info_t *phl_info, struct phl_snd_grp *grp)
{
	...
	do {
		...
		/* get role from group */
		role = rtw_phl_get_wrole_by_ridx(phl_info->phl_com, grp->wrole_idx);
			...
			if (rtw_phl_role_is_client_category(role)) {
				...
				if (rtw_hal_bf_get_entry_snd_sts(
					...
					if (para->snd_fail_counter > 10) {
					 DBGP("Sounding Fail Count > 10, break sounding !!!!\n");
						break;
					}
				} else {
