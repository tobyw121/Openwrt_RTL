...
u8 WPA_CIPHER_SUITE_SMS4[] = { 0x00, 0x14, 0x72, 1 };
u8 WPA_CIPHER_SUITE_GCM_SM4[] = { 0x00, 0x14, 0x72, 2 };
u8 WPA_CIPHER_SUITE_SM4_128_CMAC[] = { 0x00, 0x14, 0x72, 3 };
#endif
...
u8 WLAN_AKM_OWE[] = {0x00, 0x0f, 0xac, 18};
#endif /* CONFIG_HAPD_OWE */
u8 WLAN_AKM_DPP[] = {0x50, 0x6f, 0x9a, 2};
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
u8 WLAN_AKM_WAPI_PSK[] = {0x00, 0x14, 0x72, 2};
u8 WLAN_AKM_WAPI_CERT[] = {0x00, 0x14, 0x72, 1};
#endif
...
)
{
	...
	*(pbuf + 1) = (u8)len;
	...
	if (frlen) {
#ifdef PLATFORM_ECOS
		tmp = get_unaligned(frlen) + (len + 2);
		put_unaligned(tmp, frlen);
#else
		*frlen = *frlen + (len + 2);
#endif
	}
...
inline u8 *rtw_set_ie_ch_switch_wrapper(u8 *buf, u32 *buf_len, u8 new_bw,
				u8 new_center_ch)
{
	u8 ie_data[5];

	ie_data[0] = WLAN_EID_WIDE_BANDWIDTH_CHANNEL_SWITCH;
	ie_data[1] = 3;
	ie_data[2] = (new_bw == CHANNEL_WIDTH_80) ? 1 : 0;
	ie_data[3] = new_center_ch;
	ie_data[4] = 0;
	return rtw_set_ie(buf, WLAN_EID_CHANNEL_SWITCH_WRAPPER,  5, ie_data, buf_len);
}
#endif
...
/**
 * rtw_ies_remove_redundant_ie - Find matching IEs and remove redundant ones
 * @ies: Address of IEs to search
 * @ies_len: Pointer of length of ies, will update to new length
 * @offset: The offset to start scarch
 * @eid: Element ID to match
 ...
 * Returns: _SUCCESS: ies is updated, _FAIL: not updated
 */
int rtw_ies_remove_redundant_ie(u8 *ies, uint *ies_len, uint offset, u8 eid, u8 *oui, u8 oui_len)
{
	int ret = _FAIL;
	u8 *target_ie;
	u32 target_ielen;
	...
	while (search_len >= oui_len) {
		target_ie = rtw_get_ie_ex(start, search_len, eid, oui, oui_len, NULL, &target_ielen);
		if (target_ie && target_ielen) {
			u8 *remain_ies = target_ie + target_ielen;
			uint remain_len = search_len - (remain_ies - start);

			// don't remove the first matching ie
			count += 1;
			if(count > 1) {
				_rtw_memmove(target_ie, remain_ies, remain_len);
				*ies_len = *ies_len - target_ielen;
...
/**
 * rtw_get_ie_ex_count - Get count of matching IEs
 * @ies: Address of IEs to search
 * @ies_len: Pointer of length of ies, will update to new length
 * @offset: The offset to start scarch
 * @eid: Element ID to match
 ...
 * @oui_len: OUI length
 *
 * Returns: count of matching IEs
 */
int rtw_get_ie_ex_count(u8 *ies, uint *ies_len, uint offset, u8 eid, u8 *oui, u8 oui_len)
{
	int ret = 0;
	u8 *target_ie;
	u32 target_ielen;
...
{
	u8 cck_en = _FALSE, ofdm_en = _FALSE;

	_rtw_memset(SupportedRates, 0, NDIS_802_11_LENGTH_RATES_EX);

	if (mode == WLAN_MD_11B) {
		/* only cck rate for B only */
		cck_en = _TRUE;
		ofdm_en = _FALSE;
	} else if (mode == WLAN_MD_11G || mode == WLAN_MD_11A) {
		cck_en = _FALSE;
		ofdm_en = _TRUE;
	} else {
		if (ch < 14) {
			/* cck & ofdm rate for 2G */
			cck_en = _TRUE;
			ofdm_en = _TRUE;
		} else {
			/* only ofdm rate for 5G */
			cck_en = _FALSE;
			ofdm_en = _TRUE;
		}
	}
...
u8 rtw_update_rate_bymode(WLAN_BSSID_EX *pbss_network, u32 mode)
{
	...
	if (mode == WLAN_MD_11B) {
		...
		p = rtw_get_ie(ie + _BEACON_IE_OFFSET_, _EXT_SUPPORTEDRATES_IE_, &ie_len, pbss_network->IELength - _BEACON_IE_OFFSET_);
		if (p)
			network_type = WLAN_MD_11BG;		/*	do nothing	*/
		else
			network_type = WLAN_MD_11B;
	}
...
int rtw_generate_ie(struct registry_priv *pregistrypriv)
{
...
#ifdef PLATFORM_ECOS
	put_unaligned(cpu_to_le16((u16)pdev_network->Configuration.BeaconPeriod), (u16 *)ie);
#else
...
#ifdef PLATFORM_ECOS
	tmp = get_unaligned((u16 *)ie);
	tmp |= cpu_to_le16(cap_IBSS);
	put_unaligned(tmp, (u16 *)ie);
#else
	*(u16 *)ie |= cpu_to_le16(cap_IBSS);
	...
	{
		tmp = get_unaligned((u16 *)ie);
		tmp |= cpu_to_le16(cap_ShortPremble);
		put_unaligned(tmp, (u16 *)ie);
	}
#else
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
int rtw_get_wapi_cipher_suite(u8 *s)
{
	if (_rtw_memcmp(s, WPA_CIPHER_SUITE_SMS4, WAPI_SELECTOR_LEN) == _TRUE)
		return WPA_CIPHER_SMS4;
	if (_rtw_memcmp(s, WPA_CIPHER_SUITE_GCM_SM4, WAPI_SELECTOR_LEN) == _TRUE)
		return WPA_CIPHER_GCM_SM4;
	if (_rtw_memcmp(s, WPA_CIPHER_SUITE_SM4_128_CMAC, WAPI_SELECTOR_LEN) == _TRUE)
		return WPA_CIPHER_BIP_CMAC_SM4_128;
...
#ifdef CONFIG_HAPD_OWE
	if (_rtw_memcmp(s, WLAN_AKM_OWE, RSN_SELECTOR_LEN) == _TRUE)
		return WLAN_AKM_TYPE_OWE;
#endif /* CONFIG_HAPD_OWE */
	if (_rtw_memcmp(s, WLAN_AKM_DPP, RSN_SELECTOR_LEN) == _TRUE)
		return WLAN_AKM_TYPE_DPP;
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	if (_rtw_memcmp(s, WLAN_AKM_WAPI_PSK, RSN_SELECTOR_LEN) == _TRUE)
		return WLAN_AKM_TYPE_WAPI_PSK;
	if (_rtw_memcmp(s, WLAN_AKM_WAPI_CERT, RSN_SELECTOR_LEN) == _TRUE)
		return WLAN_AKM_TYPE_WAPI_CERT;
#endif
	...
	int *pairwise_cipher, u32 *akm)
{
	...
	if (akm) {
		if (left >= 6) {
			...
			{
				if(_rtw_memcmp(pos,WPA_AUTH_KEY_MGMT_PSK_OVER_802_1X, 4) == 1)
					*akm = WLAN_AKM_TYPE_PSK;
			}
		}
	}
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
int rtw_wapi_info_parse(const u8 *ie, uint ie_len, struct wapie_info *info)
{
	const u8 *pos = ie;
	u16 cnt, ver;

	_rtw_memset(info, 0, sizeof(struct wapie_info));

	if (ie_len < MIN_WAPI_IE_LEN || ie_len > MAX_WAPI_IE_LEN) {
		RTW_INFO("%s: Invalid WAPI IE length\n", __func__);
		goto err;
	}

	if (*ie != WLAN_EID_WAPI || *(ie + 1) != ie_len - 2)
		goto err;

	pos += 2;

	ver = RTW_GET_LE16(pos);
	if (ver != WAPI_VERSION_V1) {
		RTW_INFO("%s: Invalid WAPI version %d\n", __func__,ver);
		goto err;
	}
	info->version = ver;
	pos += 2;
	...
	if (ie + ie_len < pos + WAPI_SELECTOR_LEN * cnt) {
		RTW_INFO("%s: AKM truncated\n", __func__);
		goto err;
	}
	info->akm_cnt = cnt;
	info->akm_list = (u8 *)pos;
	pos += WAPI_SELECTOR_LEN * cnt;
	...
	info->ukm_cnt = cnt;
	info->ukm_list = (u8 *)pos;
	pos += WAPI_SELECTOR_LEN * cnt;

	/* Group Cipher Suite */
	if (ie + ie_len < pos + WAPI_SELECTOR_LEN) {
	...
	/* WAPI Capability */
	if (ie + ie_len < pos + 2) {
		RTW_INFO("%s: WAPI Capability truncated\n", __func__);
		goto err;
	}
	info->cap = (u8 *)pos;
	pos += 2;
	...
	if (ie + ie_len < pos + WAPI_BKID_LEN * cnt) {
		if (ie + ie_len != pos || cnt) {
			RTW_INFO("%s: BKID list truncated\n", __func__);
			goto err;
		}
		goto exit;
	}
	info->bkid_cnt = cnt;
	info->bkid_list = (u8 *)pos;
	pos += WAPI_BKID_LEN * cnt;

	/* Group Management Cipher Suit */
	if (ie + ie_len < pos + WAPI_SELECTOR_LEN) {
		if (ie + ie_len != pos)
			goto err;
		goto exit;
...
#ifdef PLATFORM_ECOS
		{
			if ( ((int)info.cap & 0x1) != 0 )
			{
				u16 tmp = get_unaligned((u16 *)(info.cap));
				*mfp_opt = GET_RSN_CAP_MFP_OPTION(&tmp);
			}
			else
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
int rtw_parse_wapi_ie(u8 *wapi_ie, int wapi_ie_len, int *group_cipher,
		      int *pairwise_cipher, int *gmcs, u32 *akm, u8 *mfp_opt)
{
	struct wapie_info info;
	int i, ret = _SUCCESS;

	ret = rtw_wapi_info_parse(wapi_ie, wapi_ie_len, &info);
	if (ret != _SUCCESS)
		goto exit;

	if (group_cipher) {
		if (info.mkm)
			*group_cipher = rtw_get_wapi_cipher_suite(info.mkm);
		else
			*group_cipher = 0;
	}
	...
	if (pairwise_cipher) {
		*pairwise_cipher = 0;
		for (i = 0; i < info.ukm_cnt; i++)
			*pairwise_cipher |= rtw_get_wapi_cipher_suite(info.ukm_list + 4 * i);
	}

	if (gmcs) {
		if (info.gmcs)
			*gmcs = rtw_get_wapi_cipher_suite(info.gmcs);
		else
			*gmcs = 0;
	}
...
int rtw_get_sec_ie_g6(u8 *in_ie, uint in_len, u8 *rsn_ie, u16 *rsn_len, u8 *wpa_ie, u16 *wpa_len)
{
	u8 authmode, sec_idx;
...
u8 *rtw_get_wps_attr(u8 *wps_ie, uint wps_ielen, u16 target_attr_id , u8 *buf_attr, u32 *len_attr)
{
	...
	while (wps_ielen <= 0xFFFF && attr_ptr - wps_ie < wps_ielen) { // For coverity check wps_ielen <= 0xFFFF
...
void dump_ies_g6(void *sel, const u8 *buf, u32 buf_len)
{
	const u8 *pos = buf;
	u8 id, len;
	...
	if (ht || vht) {
		...
		if (vht) {
			...
			if (vht_op_ie && vht_op_ielen) {
				if ((GET_VHT_OPERATION_ELE_CHL_WIDTH(vht_op_ie + 2) == 1) && (GET_VHT_OPERATION_ELE_CENTER_FREQ2(vht_op_ie + 2) == 0)){
					*bw = CHANNEL_WIDTH_80;
				}else if ((GET_VHT_OPERATION_ELE_CHL_WIDTH(vht_op_ie + 2) == 1) && (GET_VHT_OPERATION_ELE_CENTER_FREQ2(vht_op_ie + 2))){
		   ...
		   , u8 *g_ch, u8 *g_bw, u8 *g_offset)
{
	...
	*req_ch = *g_ch;
	...
	if ((*req_bw == CHANNEL_WIDTH_80 || *req_bw == CHANNEL_WIDTH_160) && *g_ch <= 14) {
	...
	case CHANNEL_WIDTH_160:
		...
		if (*req_offset == CHAN_OFFSET_NO_EXT) {
			RTW_ERR("%s req 160MHz BW without offset, down to 20MHz\n", __func__);
			/* rtw_warn_on(1); */
	...
	while (1) {
		...
		if (target_ie && target_ie_len) {
			...
			if (DBG_DEL_P2P_IE && msg) {
				RTW_INFO("%s %d before\n", __func__, index);
				dump_ies_g6(RTW_DBGDUMP, ies, ies_len);
...
#define DBG_DEL_P2P_ATTR 0
	...
	while (1) {
		...
		if (target_attr && target_attr_len) {
			...
			if (DBG_DEL_P2P_ATTR) {
				RTW_INFO("%s %d before\n", __func__, index);
				dump_ies_g6(RTW_DBGDUMP, ie, ielen);
		...
		if (ie) {
			...
			if (DBG_BSS_EX_DEL_P2P_ATTR) {
				if (rtw_get_p2p_attr(ie, ie_len_ori, attr_id, NULL, NULL)) {
					RTW_INFO("%s %d before\n", __func__, index);
					dump_ies_g6(RTW_DBGDUMP, BSS_EX_TLV_IES(bss_ex), BSS_EX_TLV_IES_LEN(bss_ex));
