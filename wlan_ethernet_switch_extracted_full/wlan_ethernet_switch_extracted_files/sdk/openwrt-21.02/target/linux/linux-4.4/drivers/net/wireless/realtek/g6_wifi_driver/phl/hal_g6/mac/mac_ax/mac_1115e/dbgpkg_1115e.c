...
u32 tx_dbg_dump_1115e(struct mac_ax_adapter *adapter)
{
	...
	for (i = 0; i < TRX_CNT_REPT_CNT; i++) {
		...
		if (adapter->hw_info->intf == MAC_AX_INTF_PCIE) {
			PLTFM_MSG_ALWAYS("R_BE_PCIE_MIO_INTF=0x%x\n",
					 MAC_REG_R32(R_BE_PCIE_MIO_INTF));

			PLTFM_MSG_ALWAYS("R_BE_PCIE_MIO_INTD=0x%x\n",
					 MAC_REG_R32(R_BE_PCIE_MIO_INTD));

			PLTFM_MSG_ALWAYS("R_BE_HAXI_INIT_CFG1=0x%x\n",
					 MAC_REG_R32(R_BE_HAXI_INIT_CFG1));

			PLTFM_MSG_ALWAYS("R_BE_HAXI_DMA_STOP1=0x%x\n",
					 MAC_REG_R32(R_BE_HAXI_DMA_STOP1));

			PLTFM_MSG_ALWAYS("R_BE_HAXI_DMA_BUSY1=0x%x\n",
					 MAC_REG_R32(R_BE_HAXI_DMA_BUSY1));

			PLTFM_MSG_ALWAYS("R_BE_HAXI_DMA_BUSY2=0x%x\n",
					 MAC_REG_R32(R_BE_HAXI_DMA_BUSY2));

			PLTFM_MSG_ALWAYS("R_BE_HAXI_DBG_CTRL=0x%x\n",
					 MAC_REG_R32(R_BE_HAXI_DBG_CTRL));

			PLTFM_MSG_ALWAYS("R_BE_DBG_ERR_FLAG=0x%x\n",
					 MAC_REG_R32(R_BE_DBG_ERR_FLAG));

			PLTFM_MSG_ALWAYS("R_BE_PCIE_IO_RCY_M1=0x%x\n",
					 MAC_REG_R32(R_BE_PCIE_IO_RCY_M1));

			PLTFM_MSG_ALWAYS("R_BE_PCIE_PADDR_M1=0x%x\n",
					 MAC_REG_R32(R_BE_PCIE_PADDR_M1));

			PLTFM_MSG_ALWAYS("R_BE_PCIE_IO_RCY_M2=0x%x\n",
					 MAC_REG_R32(R_BE_PCIE_IO_RCY_M2));

			PLTFM_MSG_ALWAYS("R_BE_PCIE_PADDR_M2=0x%x\n",
					 MAC_REG_R32(R_BE_PCIE_PADDR_M2));

			PLTFM_MSG_ALWAYS("R_BE_PCIE_IO_RCY_E0=0x%x\n",
					 MAC_REG_R32(R_BE_PCIE_IO_RCY_E0));

			PLTFM_MSG_ALWAYS("R_BE_PCIE_PADDR_E0=0x%x\n",
					 MAC_REG_R32(R_BE_PCIE_PADDR_E0));

			PLTFM_MSG_ALWAYS("R_BE_CH0_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH0_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH1_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH1_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH2_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH2_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH3_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH3_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH4_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH4_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH5_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH5_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH6_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH6_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH7_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH7_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH8_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH8_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH9_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH9_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH10_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH10_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH11_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH11_TXBD_IDX));

			PLTFM_MSG_ALWAYS("R_BE_CH12_TXBD_IDX=0x%x\n",
					 MAC_REG_R32(R_BE_CH12_TXBD_IDX));
		}
#endif
		/* TMAC TX COUNTER */
...
u32 crit_dbg_dump_1115e(struct mac_ax_adapter *adapter)
{
	...
	PLTFM_MSG_ALWAYS("R_BE_FW_DEBUG_USE_ONLY_0=0x%x\n",
			 MAC_REG_R32(R_BE_FW_DEBUG_USE_ONLY_0));

	PLTFM_MSG_ALWAYS("R_BE_FW_DEBUG_USE_ONLY_1=0x%x\n",
			 MAC_REG_R32(R_BE_FW_DEBUG_USE_ONLY_1));

	PLTFM_MSG_ALWAYS("R_BE_FW_DEBUG_USE_ONLY_2=0x%x\n",
			 MAC_REG_R32(R_BE_FW_DEBUG_USE_ONLY_2));

	PLTFM_MSG_ALWAYS("R_BE_FW_DEBUG_USE_ONLY_3=0x%x\n",
			 MAC_REG_R32(R_BE_FW_DEBUG_USE_ONLY_3));
...
u32 cmac_dbg_dump_1115e(struct mac_ax_adapter *adapter, enum mac_ax_band band)
{
	...
	PLTFM_MSG_ALWAYS("[3]B%d R_AX_RX_ERR_FLAG_IMR=0x%x\n", band, MAC_REG_R32(reg));
	...
	PLTFM_MSG_ALWAYS("[3]B%d R_AX_TX_ERR_FLAG_IMR=0x%x\n", band, MAC_REG_R32(reg));
...
u32 dmac_dbg_dump_1115e(struct mac_ax_adapter *adapter)
{
	...
	/* DLE */
	PLTFM_MSG_ALWAYS("R_BE_WDE_ERRFLAG_MSG = 0x%x\n",
			 MAC_REG_R32(R_BE_WDE_ERRFLAG_MSG));
	PLTFM_MSG_ALWAYS("R_BE_WDE_DBGERR_LOCKEN = 0x%x\n",
			 MAC_REG_R32(R_BE_WDE_DBGERR_LOCKEN));
	PLTFM_MSG_ALWAYS("R_BE_WDE_DBGERR_STS = 0x%x\n",
			 MAC_REG_R32(R_BE_WDE_DBGERR_STS));
	PLTFM_MSG_ALWAYS("R_BE_PLE_ERRFLAG_MSG = 0x%x\n",
			 MAC_REG_R32(R_BE_PLE_ERRFLAG_MSG));
	PLTFM_MSG_ALWAYS("R_BE_PLE_DBGERR_LOCKEN = 0x%x\n",
			 MAC_REG_R32(R_BE_PLE_DBGERR_LOCKEN));
	PLTFM_MSG_ALWAYS("R_BE_PLE_DBGERR_STS = 0x%x\n",
			 MAC_REG_R32(R_BE_PLE_DBGERR_STS));
		       ...
		       struct mac_ax_dbg_port_info **info, u32 sel)
{
	...
	case MAC_AX_DBG_PORT_SEL_PAXI_TXDMA:
		*info = &dbg_port_paxi_txdma;
		...
		MAC_REG_W32(R_AX_FILTER_MODEL_ADDR, AXIDMA_BASE_ADDR_BE);
...
{
	switch (sel) {
	case MAC_AX_MEM_CPU_LOCAL:
		*base_addr = CPU_LOCAL_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_WCPU_DATA:
		*base_addr = WCPU_DATA_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_AXIDMA:
		*base_addr = AXIDMA_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_STA_SCHED:
		*base_addr = STA_SCHED_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_RXPLD_FLTR_CAM:
		*base_addr = RXPLD_FLTR_CAM_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_SECURITY_CAM:
		*base_addr = SEC_CAM_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_WOW_CAM:
		*base_addr = WOW_CAM_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_CMAC_TBL:
		*base_addr = CMAC_TBL_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_ADDR_CAM:
		*base_addr = ADDR_CAM_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_BSSID_CAM:
		*base_addr = BSSID_CAM_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_BA_CAM:
		*base_addr = BA_CAM_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_BCN_IE_CAM0:
		*base_addr = BCN_IE_CAM0_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_SHARED_BUF:
		*base_addr = SHARED_BUF_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_DMAC_TBL:
		*base_addr = DMAC_TBL_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_SHCUT_MACHDR:
		*base_addr = SHCUT_MACHDR_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_BCN_IE_CAM1:
		*base_addr = BCN_IE_CAM1_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_TXD_FIFO_0:
		*base_addr = TXD_FIFO_0_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_TXD_FIFO_1:
		*base_addr = TXD_FIFO_1_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_WD_PAGE:
		*base_addr = WD_PAGE_BASE_ADDR_BE;
		break;
	case MAC_AX_MEM_DCPU_LOCAL:
		*base_addr = DCPU_LOCAL_BASE_ADDR_BE;
		break;
	default:
		PLTFM_MSG_ERR("[ERR] mem base addr sel 1115e %d\n", sel);
		return MACNOITEM;
	}
			 ...
			 u32 val, enum mac_ax_sram_dbg_sel sel)
{
	...
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	enum mac_ax_mem_sel mem_sel;
	u32 reg_base, ind_reg, ret;

	switch (sel) {
	case CPU_LOCAL_SEL:
		mem_sel = MAC_AX_MEM_CPU_LOCAL;
		break;
	case WCPU_DATA_SEL:
		mem_sel = MAC_AX_MEM_WCPU_DATA;
		break;
	case AXIDMA_SEL:
		mem_sel = MAC_AX_MEM_AXIDMA;
		break;
	case STA_SCHED_SEL:
		mem_sel = MAC_AX_MEM_STA_SCHED;
		break;
	case RXPLD_FLTR_CAM_SEL:
		mem_sel = MAC_AX_MEM_RXPLD_FLTR_CAM;
		break;
	case SEC_CAM_SEL:
		mem_sel = MAC_AX_MEM_SECURITY_CAM;
		break;
	case WOW_CAM_SEL:
		mem_sel = MAC_AX_MEM_WOW_CAM;
		break;
	case CMAC_TBL_SEL:
		mem_sel = MAC_AX_MEM_CMAC_TBL;
		break;
	case ADDR_CAM_SEL:
		mem_sel = MAC_AX_MEM_ADDR_CAM;
		break;
	case BSSID_CAM_SEL:
		mem_sel = MAC_AX_MEM_BSSID_CAM;
		break;
	case BA_CAM_SEL:
		mem_sel = MAC_AX_MEM_BA_CAM;
		break;
	case BCN_IE_CAM0_SEL:
		mem_sel = MAC_AX_MEM_BCN_IE_CAM0;
		break;
	case SHARED_BUF_SEL:
		mem_sel = MAC_AX_MEM_SHARED_BUF;
		break;
	case DMAC_TBL_SEL:
		mem_sel = MAC_AX_MEM_DMAC_TBL;
		break;
	case SHCUT_MACHDR_SEL:
		mem_sel = MAC_AX_MEM_SHCUT_MACHDR;
		break;
	case BCN_IE_CAM1_SEL:
		mem_sel = MAC_AX_MEM_BCN_IE_CAM1;
		break;
	case DCPU_LOCAL_SEL:
		mem_sel = MAC_AX_MEM_DCPU_LOCAL;
		break;
	default:
		PLTFM_MSG_ERR("[ERR] sram dbg write sel 1115e %d\n", sel);
		return MACNOITEM;
	}

	ret = p_ops->get_mem_base_addr(adapter, &reg_base, mem_sel);
	if (ret != MACSUCCESS)
		return ret;

	reg_base += offset;
	ind_reg = R_BE_INDIR_ACCESS_ENTRY;

	PLTFM_MSG_WARN("%s ind access sel %d start\n", __func__, sel);
	PLTFM_MUTEX_LOCK(&adapter->hw_info->ind_access_lock);
	adapter->hw_info->ind_aces_cnt++;
	MAC_REG_W32(R_BE_FILTER_MODEL_ADDR, reg_base);
	MAC_REG_W32(ind_reg, val);
	adapter->hw_info->ind_aces_cnt--;
	PLTFM_MUTEX_UNLOCK(&adapter->hw_info->ind_access_lock);
	PLTFM_MSG_WARN("%s ind access sel %d end\n", __func__, sel);
			...
			enum mac_ax_sram_dbg_sel sel)
{
	...
	case BCN_IE_CAM1_SEL:
		...
		break;
	case WD_PAGE_SEL:
		mem_sel = MAC_AX_MEM_WD_PAGE;
		break;
	case DCPU_LOCAL_SEL:
		mem_sel = MAC_AX_MEM_DCPU_LOCAL;
		break;
	default:
		PLTFM_MSG_ERR("[ERR] sram dbg read sel 1115e %d\n", sel);
		return MACNOITEM;
	}
...
u8 is_dbg_port_not_valid_1115e(struct mac_ax_adapter *adapter, u32 dbg_sel)
{
	...
	if (check_mac_en(adapter, 0, MAC_AX_DMAC_SEL) != MACSUCCESS &&
		...
		return 1;
	if (dbg_sel == MAC_AX_DBG_PORT_SEL_STA_SCH)
		return 1;
