...
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(4, 11, 12)) && !defined(CONFIG_VERSION)
...
static const u32 rtw_cipher_suites[] = {
	...
	WLAN_CIPHER_SUITE_GCM_SM4,
#ifdef CONFIG_IEEE80211W
	WLAN_CIPHER_SUITE_SM4_CMAC_128,
...
int rtw_cfg80211_sync_iftype(_adapter *adapter)
{
...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 20, 0)) && !defined(PLATFORM_ECOS)
...
#if defined(WFO_RADIO_SENDER)
	if (padapter->dvobj->dev_id == WFO_SKIP_PCIE_SLOT)
		goto exit;
#endif /* WFO_RADIO_SENDER */
...
#endif
#if defined(CONFIG_VERSION) || LINUX_VERSION_CODE >= KERNEL_VERSION(4, 12, 0)
...
void rtw_cfg80211_indicate_disconnect(_adapter *padapter, u16 reason, u8 locally_generated)
{
	...
	if (padapter->ndev_unregistering || !rtw_wdev_not_indic_disco(pwdev_priv)) {
		...
		if (pwdev_priv->connect_req) {
			...
			int cached_pmkid = rtw_cached_pmkid(padapter, tgt_network->network.MacAddress);
			if((cached_pmkid >= 0) && (reason == WLAN_REASON_INVALID_IE)) {
				/* For SAE connection, if assoc success but received deauth with reason 13,
				   report connect result failed to uplayer in order to delete pmkid. (wqcap3440) */
				RTW_PRINT("%s: disconnect (%d), pmkid (%d), indicate connect result (%d)\n",
					padapter->pnetdev->name, reason, cached_pmkid, WLAN_STATUS_UNSPECIFIED_FAILURE);
...
static int rtw_cfg80211_ap_set_encryption(struct net_device *dev, struct ieee_param *param)
{
	...
	if (is_broadcast_mac_addr(param->sta_addr)) {
		...
		if (!psta) {
			...
			if (rtw_ft_enable(padapter)) {
				RTW_PRINT("[FT] going to alloc stainfo for sa="MAC_FMT"\n", MAC_ARG(param->sta_addr));

				//psta = rtw_alloc_stainfo_sw(pstapriv, param->sta_addr);
			#ifdef RTW_WKARD_AP_CMD_DISPATCH
				psta = rtw_alloc_stainfo(pstapriv, param->sta_addr, PHL_CMD_DIRECTLY);
			#else
				psta = rtw_alloc_stainfo(pstapriv, param->sta_addr, PHL_CMD_WAIT);
			#endif
				...
				psta->authalg = WLAN_AUTH_FT;
				psta->expire_to = pstapriv->assoc_to;

				_rtw_spinlock_bh(&pstapriv->auth_list_lock);
				if (rtw_is_list_empty(&psta->auth_list)) {

					rtw_list_insert_tail(&psta->auth_list, &pstapriv->auth_list);
	...
	if (strcmp(param->u.crypt.alg, "SMS4") == 0 ||
	    strcmp(param->u.crypt.alg, "GCM_SM4") == 0 ||
	    strcmp(param->u.crypt.alg, "SM4_CMAC_128") == 0) {
		return rtw_wapi_set_key(padapter, param);
	}
#endif
	...
	if (strcmp(param->u.crypt.alg, "WEP") == 0 && (psta == NULL)) {
		...
		RTW_INFO("%s: WEP: key_id = %u, key_len = %u\n",
			 __func__, wep_key_idx, wep_key_len);

		/* Should have been checked in nl80211, through */
		if ((wep_key_idx >= WEP_KEYS) ||
		    ((wep_key_len != WLAN_KEY_LEN_WEP40) &&
		     (wep_key_len != WLAN_KEY_LEN_WEP104))) {
			ret = -EINVAL;
			goto exit;
		}

		/* Update default key if it's not explicitly specified */
		...
		_rtw_memcpy(psecuritypriv->dot11DefKey[wep_key_idx].skey, param->u.crypt.key, wep_key_len);
	...
	if (!psta) { /* group key */
		if (param->u.crypt.set_tx == 0) { /* group key, TX only */
			...
			if (pbcmc_sta)
				rtw_phl_cmd_set_sta_seciv(padapter->dvobj->phl, padapter->phl_role, NULL, 0, PHL_CMD_DIRECTLY, 0);
		}
	...
	if ((psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_8021X || psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_Auto) && psta) { /* psk/802_1x */
		if (param->u.crypt.set_tx == 1) {
			...
			if (psta->authalg == WLAN_AUTH_FT) {
				RTW_PRINT("[FT][%s] delay set pairwise key\n", __func__);
			}
			else
			#endif
			{
			...
			rtw_phl_cmd_set_sta_seciv(padapter->dvobj->phl, padapter->phl_role, psta->phl_sta, psta->dot11txpn.val + 1, PHL_CMD_DIRECTLY, 0);
		} else {
...
static int rtw_cfg80211_set_encryption(struct net_device *dev, struct ieee_param *param)
{
	...
	if (strcmp(param->u.crypt.alg, "WEP") == 0) {
		/* WEP keys are set in rtw_set_wep before connection,
		 * and installed in rtw_ap_wep_pk_setting after association,
	...
	, const u8 *mac_addr, struct key_params *params)
{
	...
	case WLAN_CIPHER_SUITE_SMS4:
		alg_name = "SMS4";
		break;
	case WLAN_CIPHER_SUITE_GCM_SM4:
		alg_name = "GCM_SM4";
		break;
#ifdef CONFIG_IEEE80211W
	case WLAN_CIPHER_SUITE_SM4_CMAC_128:
		alg_name = "SM4_CMAC_128";
		break;
#endif /* CONFIG_IEEE80211W */
#endif
	default:
	...
	if (!mac_addr || is_broadcast_ether_addr(mac_addr)
		...
		if (params->cipher == WLAN_CIPHER_SUITE_SMS4 ||
		    params->cipher == WLAN_CIPHER_SUITE_GCM_SM4)
...
#define GET_KEY_PARAM_ARG_E , mac_addr
	...
	if (cipher == _WEP40_) {
		...
		key_len = 16;
	} else if (cipher == _GCM_SM4_) {
		cipher = WLAN_CIPHER_SUITE_GCM_SM4;
		key_len = 16;
#endif
...
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)) */
{
	...
	if ((key_index < WEP_KEYS) &&
		...
		if (key_index == psecuritypriv->dot11PrivacyKeyIndex) {
			...
			RTW_INFO("%s: delete default key!\n", __func__);
		}

		if (psecuritypriv->key_mask & BIT(key_index)) {
			psecuritypriv->key_mask &= ~BIT(key_index);
			RTW_INFO("%s: clear bit %d of key_mask!\n",
				 __func__, key_index);
		}

		RTW_INFO("%s: key_mask = %d\n",
			 __func__, psecuritypriv->key_mask);
	}
	...
	struct station_info *sinfo)
{
	...
	if (psta) {
		if (!MLME_IS_STA(padapter)
			...
			/* sinfo->signal = translate_percentage_to_dbm(psta->phl_sta->rssi_stat.rssi); */
		}
		sinfo->filled |= STATION_INFO_INACTIVE_TIME;
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 12, 0)) && !defined(CONFIG_VERSION)
				     u32 *flags,
#endif
				     struct vif_params *params)
{
	...
	/* Change type requires interface is down.
	 * Report error if adapter is up.
	 */
	if (padapter->netif_up) {
		ret = -EPERM;
		RTW_ERR(FUNC_NDEV_FMT": %s is called when interface is up!\n",
		        FUNC_NDEV_ARG(ndev), __FUNCTION__);
#if defined(CONFIG_RTW_MULTI_AP) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 33))
		if (params->use_4addr != -1) {
			DBGP("apply A4, use_4addr= %d \n", params->use_4addr);
			ret = 0;
		}
#endif
		goto exit;
	...
	/* Set wireless device type before enable netdev so that
	 * the register net device is in expected type and
	 * HW is initialized without type change.
	 */
	ndev->ieee80211_ptr->iftype = type;

#if defined(RTW_WKARD_INTF_RESET) || defined(WKARD_CHANGE_IFACE)
	DBGP("NOT call netdev_open in %s\n", __func__);
#else
	RTW_INFO(FUNC_NDEV_FMT" call netdev_open\n", FUNC_NDEV_ARG(ndev));
	if (netdev_open(ndev) != 0) {
		RTW_ERR(FUNC_NDEV_FMT" call netdev_open fail\n", FUNC_NDEV_ARG(ndev));
		ret = -EPERM;
		goto exit;
	}

	if (_FAIL == rtw_pwr_wakeup(padapter)) {
		RTW_ERR(FUNC_NDEV_FMT" call rtw_pwr_wakeup fail\n", FUNC_NDEV_ARG(ndev));
		ret = -EPERM;
		goto exit;
	}
#endif /* defined(RTW_WKARD_INTF_RESET) || defined(WKARD_CHANGE_IFACE) */
...
#if defined(CONFIG_VERSION) || (KERNEL_VERSION(4, 7, 0) <= LINUX_VERSION_CODE)
...
static void _rtw_cfg80211_surveydone_event_callback(_adapter *padapter, struct cfg80211_scan_request *scan_req)
{
	...
	struct dvobj_priv *d = padapter->dvobj;
	_queue *queue = &(d->scanned_queue);
#else
	...
	, struct cfg80211_scan_request *request)
{
	...
	if (   check_fwstate(pmlmepriv, WIFI_AP_STATE)
	    && !padapter->phl_role->bcn_cmn.bcn_added
	    && (padapter->registrypriv.coex_2g_40m == 0)) {
		int is_all_2g_ch = _TRUE;
		for (i = 0; i < request->n_channels && i < RTW_CHANNEL_SCAN_AMOUNT; i++) {
			if (request->channels[i]->hw_value > 14) {
				is_all_2g_ch = _FALSE;
				break;
			}
		}
		if (is_all_2g_ch) {
			RTW_PRINT("Skipping scan to force 40M in 2G band.\n");
			ssc_chk = SS_DENY_ADAPTIVITY;
...
#if (KERNEL_VERSION(4, 7, 0) <= LINUX_VERSION_CODE) || defined(CONFIG_VERSION)
		struct cfg80211_scan_info info;
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	else if (key_mgt == WLAN_AKM_SUITE_WAPI_PSK || key_mgt == WLAN_AKM_SUITE_WAPI_CERT)
		psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_WAPI;
#endif
...
static int rtw_cfg80211_set_wpa_ie(_adapter *padapter, u8 *pie, size_t ielen)
{
	...
	/*get RSNXE IE (WPA3 R3)*/
	prsnxe = rtw_get_ie(buf, _RSN_EX_IE_, &rsnxe_ie_len, ielen);
	if (prsnxe && rsnxe_ie_len > 0) {
		if(rsnxe_ie_len + 2 > MAX_RSN_XE_IE_LEN){
			RTW_ERR("need enlarge rsn_xe \n");
		}else{
			_rtw_memcpy(padapter->securitypriv.rsn_xe, prsnxe, rsnxe_ie_len + 2);
			padapter->securitypriv.rsn_xe_len = rsnxe_ie_len + 2;
		}
	}else{
		padapter->securitypriv.rsn_xe_len = 0;
	}

#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	pwapi = rtw_get_ie(buf, _WAPI_IE_, &wapi_ielen, ielen);
	if (pwapi && wapi_ielen > 0) {
		if (rtw_parse_wapi_ie(pwapi, wapi_ielen + 2, &group_cipher, &pairwise_cipher, NULL, NULL, &mfp_opt) == _SUCCESS) {
			padapter->securitypriv.dot11AuthAlgrthm = dot11AuthAlgrthm_WAPI;
			padapter->securitypriv.ndisauthtype = Ndis802_11AuthModeWAPI;
			_rtw_memcpy(padapter->securitypriv.supplicant_ie, pwapi, wapi_ielen + 2);
			RTW_INFO("got wapi ie, wapi_ielen:%u\n", wapi_ielen);
		}
	}
#endif
	...
	case WPA_CIPHER_SMS4:
		padapter->securitypriv.dot118021XGrpPrivacy = _SMS4_;
		padapter->securitypriv.ndisencryptstatus = Ndis802_11_EncrypteionWAPI;
		break;
	case WPA_CIPHER_GCM_SM4:
		padapter->securitypriv.dot118021XGrpPrivacy = _GCM_SM4_;
		padapter->securitypriv.ndisencryptstatus = Ndis802_11_EncrypteionWAPI;
		break;
#endif
	}
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	case WPA_CIPHER_SMS4:
		padapter->securitypriv.dot11PrivacyAlgrthm = _SMS4_;
		padapter->securitypriv.ndisencryptstatus = Ndis802_11_EncrypteionWAPI;
		break;
	case WPA_CIPHER_GCM_SM4:
		padapter->securitypriv.dot11PrivacyAlgrthm = _GCM_SM4_;
		padapter->securitypriv.ndisencryptstatus = Ndis802_11_EncrypteionWAPI;
		break;
#endif
	}
...
static int rtw_set_wep(struct _ADAPTER *a, struct cfg80211_connect_params *sme)
{
	struct security_priv *sec = &a->securitypriv;
	struct key_params *keys = sme->crypto.wep_keys;
	u32 cipher = 0;
	int i = 0;
	...
	/* should have been checked from nl80211_connect */
#ifdef WFO_VIRT_RECEIVER
	rtw_warn_on((sme->key_idx != sme->crypto.wep_tx_key) ||
		    (sme->key_len != keys[sme->key_idx].key_len) ||
		    !_rtw_memcmp(sme->key, keys[sme->key_idx].key, sme->key_len));
#else
	rtw_warn_on((sme->key_idx != sme->crypto.wep_tx_key) ||
		    (sme->key_len != keys[sme->key_idx].key_len) ||
		    (sme->key != keys[sme->key_idx].key));
#endif

	cipher = keys[sme->key_idx].cipher;
	if (cipher == WLAN_CIPHER_SUITE_WEP40) {
		sec->dot11PrivacyAlgrthm = _WEP40_;
		sec->dot118021XGrpPrivacy = _WEP40_;
	} else if (cipher == WLAN_CIPHER_SUITE_WEP104) {
		sec->dot11PrivacyAlgrthm = _WEP104_;
		sec->dot118021XGrpPrivacy = _WEP104_;
	}
	if (sec->dot11AuthAlgrthm == dot11AuthAlgrthm_Auto)
		sec->ndisauthtype = Ndis802_11AuthModeAutoSwitch;
	else if (sec->dot11AuthAlgrthm == dot11AuthAlgrthm_Shared)
		sec->ndisauthtype = Ndis802_11AuthModeShared;
	sec->ndisencryptstatus = Ndis802_11Encryption1Enabled;

	sec->dot11PrivacyKeyIndex = sme->key_idx;
	_rtw_memset(sec->dot11DefKey, 0, sizeof(sec->dot11DefKey));
	_rtw_memset(sec->dot11DefKeylen, 0, sizeof(sec->dot11DefKeylen));
	sec->key_mask = 0;
	...
	for (i = 0; i < CFG80211_MAX_WEP_KEYS; i++) {
		...
		_rtw_memcpy(sec->dot11DefKey[i].skey,
			    keys[i].key, keys[i].key_len);
		sec->dot11DefKeylen[i] = keys[i].key_len;
		sec->key_mask |= BIT(i);
	}
	...
	u8 *bssid, u8 *pmkid)
{
	...
	for (index = 0 ; index < NUM_PMKID_CACHE; index++) {
		if (_rtw_memcmp(psecuritypriv->PMKIDList[index].Bssid, bssid, ETH_ALEN) == _TRUE) {
			...
			psecuritypriv->PMKIDIndex = (index + 1) % NUM_PMKID_CACHE;
			blInserted = _TRUE;
			break;
		}
	}
				  ...
				  struct cfg80211_pmksa *pmksa)
{
	...
	bool dpp_auth = (padapter->dpp_enable == 1);
#endif
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
void rtw_cfg80211_indicate_wapi_key_update(_adapter *padapter, u8 *addr, enum nl80211_key_type key_type, int key_id)
{
	struct net_device *ndev = padapter->pnetdev;
	cfg80211_send_wapi_update_key(ndev, addr, key_type, key_id, GFP_ATOMIC);
}
#endif
...
#if defined(CONFIG_FC_WIFI_TX_GMAC_TRUNKING_SUPPORT) || defined(CONFIG_FC_WIFI_TRAP_HASH_SUPPORT)
		rtk_fc_external_lut_process(1, padapter->pnetdev->name, get_addr2_ptr(pmgmt_frame));
#endif
	}
...
#if defined(CONFIG_FC_WIFI_TX_GMAC_TRUNKING_SUPPORT) || defined(CONFIG_FC_WIFI_TRAP_HASH_SUPPORT)
	rtk_fc_external_lut_process(0, ndev->name, (char *)da);
#endif
#else /* defined(RTW_USE_CFG80211_STA_EVENT) */
	channel = pmlmeext->cur_channel;
	freq = rtw_ch2freq(channel);
...
static int rtw_cfg80211_monitor_if_xmit_entry(struct sk_buff *skb, struct net_device *ndev)
{
	...
	_rtw_memset(&skb->cb[_SKB_CB_MC2U_RA], 0, ETH_ALEN);
#endif
		...
		struct vif_params *params)
{
	...
	NDIS_802_11_NETWORK_INFRASTRUCTURE network_type =
			nl80211_iftype_to_rtw_network_type(type);
	...
	case NL80211_IFTYPE_MESH_POINT:
                ...
                /* If the creating interface exists already, return it
                 * instead of return failure */
                RTW_PRINT("Finding existing adapter of name \"%s\"...\n", name);
                padapter = dvobj_get_adapter_by_name(dvobj, name);
                if (padapter != NULL) {
                        RTW_PRINT(FUNC_ADPT_FMT" got existing adapter\n",
                                  FUNC_ADPT_ARG(padapter));
                        ndev = padapter->pnetdev;
                        if (!padapter->netif_up) {
                                cfg80211_rtw_change_iface(wiphy, ndev, type,
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 12, 0)) && !defined(CONFIG_VERSION)
								NULL,
#endif
								NULL);
                ...
                RTW_PRINT("Creating new interface \"%s\"...\n", name);
		...
		if (rtw_os_ndev_init(padapter, name, type) != _SUCCESS) {
		...
		if (ndev == pwdev_priv->pmon_ndev) {
			...
			RTW_INFO(FUNC_NDEV_FMT" remove monitor ndev\n", FUNC_NDEV_ARG(ndev));
                        ...
                        if (is_primary_adapter(adapter)) {
                                RTW_PRINT(FUNC_NDEV_FMT": unregister all ndev(s)\n", FUNC_NDEV_ARG(ndev));
                                rtw_os_ndevs_unregister(dvobj);
                        } else {
                                RTW_PRINT(FUNC_NDEV_FMT": Skip unregister ndev of non-primary adapter\n",
                                          FUNC_NDEV_ARG(ndev));
                                if (adapter->netif_up) {
                                        dev_close(ndev);
                                }
...
static int rtw_add_beacon(_adapter *adapter, const u8 *head, size_t head_len,
                          ...
                          const u8 *tail, size_t tail_len)
{
	...
	RTW_INFO("%s beacon_head_len=%zu, beacon_ies_len=%zu, beacon_tail_len=%zu\n",
	         __FUNCTION__, head_len, bcn_ies_len, tail_len);
	...
	pbuf = rtw_zmalloc(head_len + bcn_ies_len + tail_len);
	if (!pbuf) {
		ret = -ENOMEM;
		goto exit;
	}
	...
	if (bcn_ies_len > 0)
		_rtw_memcpy(pbuf + head_len - 24, (void *)bcn_ies, bcn_ies_len);
	_rtw_memcpy(pbuf + head_len + bcn_ies_len - 24, (void *)tail, tail_len);

	len = head_len + bcn_ies_len + tail_len - 24;

	/* check wps ie if inclued */
	wpsie = rtw_get_wps_ie(pbuf + _FIXED_IE_LENGTH_, len - _FIXED_IE_LENGTH_, NULL, &wps_ielen);
	if (wpsie && wps_ielen > 0) {
		RTW_INFO("add bcn, wps_ielen=%d\n", wps_ielen);
#if defined(CONFIG_WIFI_LED_SHARE_WITH_WPS) && defined(CONFIG_RTW_SW_LED)
		if (rtw_get_wps_attr_content(wpsie, wps_ielen, WPS_ATTR_SELECTED_REGISTRAR, NULL, NULL))
			rtw_led_control(adapter, LED_CTL_START_WPS);
		else
			rtw_led_control(adapter, LED_CTL_STOP_WPS);
#endif
	}
	...
	if(rtw_get_ie_ex_count(pbuf, &len, _BEACON_IE_OFFSET_, _VENDOR_SPECIFIC_IE_, WPS_OUI, 4) > 1)
	{
		RTW_INFO("Redundant WPS IEs exist, keep the first one and remove others.\n");
		rtw_ies_remove_redundant_ie(pbuf, &len, _BEACON_IE_OFFSET_,_VENDOR_SPECIFIC_IE_, WPS_OUI, 4);
	}

#ifdef CONFIG_RTW_MBO
	if(rtw_get_ie_ex_count(pbuf, &len, _BEACON_IE_OFFSET_, _VENDOR_SPECIFIC_IE_, MBO_OUI, 4) > 1)
	{
		RTW_INFO("Redundant MBO IEs exist, keep the first one and remove others.\n");
		rtw_ies_remove_redundant_ie(pbuf, &len, _BEACON_IE_OFFSET_,_VENDOR_SPECIFIC_IE_, MBO_OUI, 4);
	}
#endif

#ifdef CONFIG_HAPD_OWE
	if (rtw_get_ie_ex_count(pbuf, &len, _BEACON_IE_OFFSET_, _VENDOR_SPECIFIC_IE_, OWE_TRANSITION_OUI, 4) > 1)
	{
		RTW_INFO("Redundant OWE transition IEs exist, keep the first one and remove others.\n");
		rtw_ies_remove_redundant_ie(pbuf, &len, _BEACON_IE_OFFSET_, _VENDOR_SPECIFIC_IE_, OWE_TRANSITION_OUI, 4);
	}
#endif
...
static int rtw_add_probe_resp(_adapter *adapter,
                              const u8 *head, size_t head_len,
                              const u8 *probersp_ies, size_t probersp_ies_len,
                              const u8 *tail, size_t tail_len)
{
	struct mlme_priv *pmlmepriv = &(adapter->mlmepriv);
	uint wps_ielen = 0;
	...
	if ((head != NULL) && (head_len > 24)) {
		ie = rtw_get_wps_ie(head + 24, head_len - 24, NULL, &wps_ielen);
	}
	if ((ie == NULL) && (probersp_ies != NULL) && (probersp_ies_len > 0)) {
		ie = rtw_get_wps_ie(probersp_ies, probersp_ies_len,
		                    NULL, &wps_ielen);
	}
	if ((ie == NULL) && (tail != NULL) && (tail_len > 0)) {
		ie = rtw_get_wps_ie(tail, tail_len,
		                    NULL, &wps_ielen);
	}

	/* copy hostapd's wps ie since driver cannot auto generate*/
	if (ie && (wps_ielen > 0)) {
		RTW_INFO("[rtw_add_probe_resp]wps_ielen=%d\n", wps_ielen);
		...
		if (pmlmepriv->wps_probe_resp_ie == NULL) {
			RTW_ERR("%s()-%d: rtw_malloc() ERROR!\n", __FUNCTION__, __LINE__);
			return -EINVAL;
		}

		_rtw_memcpy(pmlmepriv->wps_probe_resp_ie, ie, wps_ielen);
...
void rtw_core_set_gt3(_adapter *padapter, u8 enable, long timeout)
{
	//#ifndef CONFIG_RTW_LINK_PHL_MASTER
	// 6a5288e8618563273674d6f7646dee35352c29d2 ystang
	void *phl = padapter->dvobj->phl;

	rtw_phl_set_gt3(phl, enable, timeout);
	//#endif /* LINK_MASTER_PHL */
}
#endif
		...
		struct cfg80211_ap_settings *settings)
{
	...
	u8 privacy_algo = psecuritypriv->dot11PrivacyAlgrthm;
...
#ifdef RTW_PHL_BCN
	{
		...
		if (wrole) {
			...
			wrole->dtim_period = settings->dtim_period;
			wrole->bcn_cmn.bcn_dtim = settings->dtim_period;
			wrole->bcn_cmn.bcn_interval = settings->beacon_interval;
		}
	}
#endif

	pmlmeinfo->dtim_period = settings->dtim_period;
	pmlmeinfo->bcn_interval = settings->beacon_interval;
...
#ifdef CONFIG_RTW_ACS
	if (adapter->registrypriv.wifi_mib.acs == 1
#if defined(CONFIG_CTC_FEATURE)
		|| 1 /* always use driver-base acs result */
#endif
	) {
		acs_replace_hapd_channel(adapter, settings->beacon.head, settings->beacon.head_len,
	...
	ret = rtw_add_probe_resp(adapter, settings->beacon.head, settings->beacon.head_len,
	                         settings->beacon.proberesp_ies,
	                         settings->beacon.proberesp_ies_len,
	                         settings->beacon.tail, settings->beacon.tail_len);

	/* WEP shared key setting */
	if ((privacy_algo == _WEP40_ || privacy_algo == _WEP104_)
		&& (settings->auth_type == 0 || settings->auth_type == 1))
		psecuritypriv->dot11AuthAlgrthm = settings->auth_type;
   ...
   if ( WIFI_ROLE_IS_ON_5G(adapter) ) {
        adapter->skb_max_que_len = 2048;
        adapter->max_enq_len = 4096;
        adapter->max_deq_len = 1024;
        adapter->sta_deq_len = 64;
        adapter->tx_lmt = 128;
        adapter->xmit_dsr_time = 600;
        adapter->xmit_dsr_num  = 2048;
        adapter->swq_amsdu_cnt = 0;
        adapter->amsdu_merge_cnt = 6;
        adapter->ring_lmt = 0;
        adapter->hw_swq_timeout = 1000;
        adapter->swq_timeout = 20;
		adapter->is_map = 0;
		adapter->manual_tx_amsdu[0] = 0;
		adapter->manual_tx_amsdu[1] = 4;
		adapter->manual_tx_amsdu[2] = 6;
		adapter->manual_tx_amsdu[3] = 6;
		adapter->current_tx_amsdu[0] = 0;
		adapter->current_tx_amsdu[1] = 0;
		adapter->current_tx_amsdu[2] = 0;
    } else {
        adapter->skb_max_que_len = 2048;
        adapter->max_enq_len = 4096;
        adapter->max_deq_len = 1024;
        ...
        adapter->ring_lmt = 0;
        adapter->hw_swq_timeout = 1000;
        adapter->swq_timeout = 10;
		adapter->is_map = 0;
		adapter->manual_tx_amsdu[0] = 0;
		adapter->manual_tx_amsdu[1] = 2;
		adapter->manual_tx_amsdu[2] = 6;
		adapter->manual_tx_amsdu[3] = 6;
		adapter->current_tx_amsdu[0] = 0;
		adapter->current_tx_amsdu[1] = 0;
	...
	if (2 == adapter->vw_enable) {
		extern void vw_latency_set(_adapter *adapter);
...
#if defined(CONFIG_VW_REFINE) || defined(CONFIG_ONE_TXQ)
	if (is_primary_adapter(adapter))
		rtw_core_set_gt3(adapter, 1, 1000000);
#endif
		...
		struct cfg80211_beacon_data *info)
{
	...
	ret = rtw_add_probe_resp(adapter, info->head, info->head_len,
		...
		const struct cfg80211_acl_data *params)
{
	...
	for (i = 0; i < params->n_acl_entries; i++)
		rtw_acl_add_sta(adapter, RTW_ACL_PERIOD_BSS, params->mac_addrs[i].addr, 0);
...
#ifdef RTW_MI_SHARE_BSS_LIST
		scanned = rtw_find_network(&padapter->dvobj->scanned_queue, mac);
#else
...
release_plink_ctl:
	...
	if (psta == NULL) {
#ifdef RTW_WKARD_AP_CMD_DISPATCH
		psta = rtw_alloc_stainfo(pstapriv, (u8 *)mac, PHL_CMD_DIRECTLY);
#else
		psta = rtw_alloc_stainfo(pstapriv, (u8 *)mac, PHL_CMD_WAIT);
#endif
		if (psta == NULL) {
...
)
{
	...
	if (is_broadcast_mac_addr(target_mac)) {
		if (MLME_IS_AP(padapter)) {
			bool has_assoc_sta = 0;
			_rtw_spinlock_bh(&pstapriv->asoc_list_lock);
			has_assoc_sta = !rtw_is_list_empty(&pstapriv->asoc_list);
			_rtw_spinunlock_bh(&pstapriv->asoc_list_lock);
			is_ucast = 0;
			/* Only issue deauth if there is any associated station */
			if (has_assoc_sta)
				issue_deauth_g6(padapter, (u8 *)target_mac, WLAN_REASON_PREV_AUTH_NOT_VALID);
			else
				RTW_WARN(ADPT_FMT" has no station. Ignore del BC station.\n",
				         ADPT_ARG(padapter));
			return 0;
		}
		else {
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		if ((_rtw_memcmp((u8 *)target_mac, psta->phl_sta->mac_addr, ETH_ALEN) || is_ucast == 0)
			...
			if (psta->dot8021xalg == 1 && psta->bpairwise_key_installed == _FALSE) {
				...
				if(params->subtype == 10)
					use_disassoc = _TRUE;

				/* _rtw_spinunlock_bh(&pstapriv->asoc_list_lock); */
				if (MLME_IS_AP(padapter))
					updated |= ap_free_sta(padapter, psta, is_ucast, params->reason_code, _TRUE, use_disassoc);
				else
...
*/
	RTW_INFO("ap_isolate=%d\n", params->ap_isolate);
	pregpriv->wifi_mib.block_relay = params->ap_isolate;
...
#if defined(CONFIG_VERSION) || (KERNEL_VERSION(4, 17, 0) <= LINUX_VERSION_CODE)
...
void rtw_cfg80211_rx_mframe(_adapter *adapter, union recv_frame *rframe, const char *msg)
{
	...
	RTW_INFO("RTW_Rx:ch=%d(%d), ta="MAC_FMT"\n", ch, sch, MAC_ARG(get_addr2_ptr(frame)));
	status = rtw_sae_preprocess_g6(adapter, frame, frame_len, _FALSE);
	if (status== _FAIL) {
		if (msg)
			RTW_INFO("RTW_Rx:%s\n", msg);
		else
	...
	if (status == 2) {
		RTW_INFO("SAE: SKIP the same commit\n");
		return;
	}
	...
	unsigned int duration, u64 *cookie)
{
	...
	*cookie = ATOMIC_INC_RETURN(&pcfg80211_wdinfo->ro_ch_cookie_gen);
	...
	//p2p_roch_cmd(padapter, *cookie, wdev, channel, channel_type, duration, 1);
...
#ifdef CONFIG_RTW_MULTI_AP_R3
int rtw_cfg80211_set_dpp_enable(_adapter *padapter, const u8 *buf)
{
	unsigned char	*frame_body;
	u8 category, action = 0;
	u8 dpp_oui[4] = {0x50, 0x6F, 0x9A, 0x1A};
	...
	if (category == RTW_WLAN_CATEGORY_PUBLIC) {
		...
		if (action == ACT_PUBLIC_VENDOR
			&& _rtw_memcmp(frame_body + 2, dpp_oui, 4) == _TRUE
			&& frame_body[7] ==  11
		) {
			padapter->dpp_enable = 1;
			return 1;
		}
	}
	return 0;
...
static int _cfg80211_rtw_mgmt_tx(_adapter *padapter, struct mgnt_tx_parm *mgnt_parm)
{
	u8 tx_ch = mgnt_parm->tx_ch;
	u8 no_cck = mgnt_parm->no_cck;
	const u8 *buf = mgnt_parm->buf;
	size_t len = mgnt_parm->len;
	int wait_ack = mgnt_parm->wait_ack;
	int wait_ms = mgnt_parm->wait_ms;
	struct xmit_frame	*pmgntframe;
	struct pkt_attrib	*pattrib;
	unsigned char	*pframe;
	int ret = _FAIL;
	...
	#endif
#endif
	systime start_time = _rtw_get_current_time();
	...
	if (tx_ch != rtw_get_oper_ch(padapter)) {
		set_channel_bwmode(padapter,
				   tx_ch, CHAN_OFFSET_NO_EXT,
				   CHANNEL_WIDTH_20, _TRUE);
		RTW_INFO(ADPT_FMT" switching to CH%u to TX MGNT\n",
			 ADPT_ARG(padapter), tx_ch);
	}
#ifdef CONFIG_MCC_MODE
issue_mgmt_frame:
#endif
	...
	if (padapter->dbg_tx_mgnt) {
		RTW_PRINT(FUNC_ADPT_FMT" TX MGNT %pX W%u\n",
		          FUNC_ADPT_ARG(padapter), pmgntframe, wait_ack);
	}
	pmgntframe->ack_rpt.dbg_tx_mgnt = padapter->dbg_tx_mgnt;
	...
	if (get_frame_sub_type(pframe) == WIFI_PROBERSP)
		rtw_update_probersp_data(padapter, tx_ch, pframe, &pattrib->pktlen);
	...
	#ifdef CONFIG_RTW_DBG_TX_MGNT
	if (padapter->dbg_tx_mgnt) {
		RTW_PRINT_DUMP("TX MGNT:", pmgntframe->buf_addr,
		               pmgntframe->attrib.pktlen);
	}
	#endif /* CONFIG_RTW_DBG_TX_MGNT */
	...
	/* Stay for a while as requested for peer response */
	if ((ret == _SUCCESS) && wait_ms) {
		systime end_time = _rtw_get_current_time();
		u32 elapse_time_ms = _rtw_systime_to_ms(end_time - start_time);

		if (elapse_time_ms < wait_ms) {
			#ifdef CONFIG_RTW_DBG_TX_MGNT
			if (padapter->dbg_tx_mgnt)
				RTW_PRINT("Wait for %ums\n", wait_ms - elapse_time_ms);
			#endif /* CONFIG_RTW_DBG_TX_MGNT */
			rtw_msleep_os(wait_ms - elapse_time_ms);
			#ifdef CONFIG_RTW_DBG_TX_MGNT
			if (padapter->dbg_tx_mgnt )
				RTW_PRINT("Resume\n");
			#endif /* CONFIG_RTW_DBG_TX_MGNT */
...
exit:
	...
	if (leave_op) {
		if (rtw_mi_check_status(padapter, MI_LINKED)) {
			...
			set_channel_bwmode(padapter, u_ch, u_offset, u_bw, _FALSE);
			RTW_INFO(ADPT_FMT" switching back to OPCH %u.\n",
			         ADPT_ARG(padapter), tx_ch);
		}
...
u8 rtw_mgnt_tx_handler(_adapter *adapter, u8 *buf)
{
	...
	if (_cfg80211_rtw_mgmt_tx(adapter, mgnt_parm) == _SUCCESS)
		rst = H2C_SUCCESS;

	return rst;
}

static int cfg80211_rtw_prob_client (struct wiphy *wiphy,
	struct net_device *ndev, const u8 *peer, u64 *cookie)
{
	...
	do {
		if (ndev == NULL) { break; }
		else { padapter = (_adapter *)rtw_netdev_priv(ndev); }

		psta = rtw_get_stainfo(&(padapter->stapriv), (u8 *)peer);
		if (psta == NULL) { break; }

		RTW_INFO(FUNC_ADPT_FMT" MAC:"MAC_FMT"\n", FUNC_ADPT_ARG(padapter),
					MAC_ARG(psta->phl_sta->mac_addr));
		issue_nulldata(padapter, psta->phl_sta->mac_addr, 1, 1, 0);
		ret = 0;
	} while(0);
	...
	u64 *cookie)
{
	...
	int ret = 0;
	u8 tx_ret;
	int wait_ack = params->dont_wait_for_ack ? 0 : 1;
	const u8 *dump_buf = buf;
	size_t dump_len = len;
	u32 dump_limit = wait_ack ? RTW_MAX_MGMT_TX_CNT : 1;
	u32 dump_cnt = 0;
	u32 wait = params->wait;
	u32 sleep_ms = 0;
...
#endif
#ifdef CONFIG_RTW_MBO
	u8 advertisement_proto_tag;
	u8 advertisement_proto_id;
#endif
	...
	if (IS_CH_WAITING(rfctl)) {
		#ifdef CONFIG_DFS_MASTER
		if (_rtw_rfctl_overlap_radar_detect_ch(rfctl, tx_ch,
		                                       CHANNEL_WIDTH_20,
		                                       CHAN_OFFSET_NO_EXT)) {
			ret = -EINVAL;
			goto exit;
	...
	*cookie = pwdev_priv->mgmt_tx_cookie++;
	RTW_INFO(FUNC_ADPT_FMT" TX MGNT @%uMHz CH%u %zuB W%u A%u",
	         FUNC_ADPT_ARG(padapter), chan->center_freq, tx_ch, len, wait,
	         wait_ack);
#ifdef CONFIG_DEBUG_CFG80211
	...
	if (IEEE80211_STYPE_PROBE_RESP == frame_styp) {
		...
		struct ieee80211_mgmt *mgmt = (struct ieee80211_mgmt *)dump_buf;
		if (le16_to_cpu(mgmt->u.auth.auth_alg) == WLAN_AUTH_FT) {
			rtw_ft_preprocess(padapter, buf, len, WIFI_AUTH);
			goto exit;
		} else {
		int retval = 0;
		...
		retval = rtw_sae_preprocess_g6(padapter, buf, len, _TRUE);
		if (retval == 2)
			goto exit;
		if (retval == 0)
	...
	else if (frame_styp == RTW_IEEE80211_STYPE_ASSOC_RESP) {
#ifdef CONFIG_RTW_80211R
		rtw_ft_preprocess(padapter, buf, len, WIFI_ASSOCRSP);
#endif
		rtw_assoc_preprocess(padapter, buf, len, WIFI_ASSOCRSP);

		goto exit;
	}
	else if (frame_styp == RTW_IEEE80211_STYPE_REASSOC_RESP) {
#ifdef CONFIG_RTW_80211R
		rtw_ft_preprocess(padapter, buf, len, WIFI_REASSOCRSP);
#endif
		rtw_assoc_preprocess(padapter, buf, len, WIFI_REASSOCRSP);
...
#endif
#ifdef CONFIG_RTW_MULTI_AP_R3
	if (rtw_cfg80211_set_dpp_enable(padapter, buf)){
		RTW_INFO("RTW_Tx: DPP enable is set.\n");
	}
#endif
	if (category == RTW_WLAN_CATEGORY_PUBLIC || category == RTW_WLAN_CATEGORY_PROTECTED_PUBLIC) {
...
#ifdef CONFIG_RTW_MBO
			advertisement_proto_tag = buf[sizeof(struct rtw_ieee80211_hdr_3addr) + 7];
			advertisement_proto_id = buf[sizeof(struct rtw_ieee80211_hdr_3addr) + 10];
			if (advertisement_proto_tag == 108 && advertisement_proto_id == 0) {
				rtw_gas_init_req_check_frames_tx(padapter, &dump_buf, &dump_len);
			}
#endif
			sleep_ms = 50;
			retry_guarantee_ms = RTW_MAX_MGMT_TX_MS_GAS;
		...
		rtw_rm_check_mgmt_tx(padapter, buf, len);
		RTW_INFO("RTW_Tx: RRM Action\n");
	}
#endif
#ifdef CONFIG_RTW_WNM
	else if (category == RTW_WLAN_CATEGORY_WNM) {
		if (action == RTW_WLAN_ACTION_WNM_BTM_REQ) {
			RTW_PRINT("[WNM] issue WNM-BTM-REQ\n");
			rtw_wnm_check_frames_tx(padapter, &dump_buf, &dump_len);
		}
	}
#endif
#ifdef CONFIG_RTW_80211R
	else if (category == RTW_WLAN_CATEGORY_FT) {
		if (action == RTW_WLAN_ACTION_FT_RSP) {
			RTW_PRINT("[FT] issue ft-action-rsp\n");
		}
	}
#endif
	else
...
dump:
	...
	while (1) {
		...
		rtw_mi_set_scan_deny(padapter, rtw_max(1000, wait));
		rtw_mi_scan_abort(padapter, _TRUE);
		tx_ret = rtw_mgnt_tx_cmd(padapter, tx_ch, no_cck, dump_buf,
					 dump_len, wait_ack, wait,
					 RTW_CMDF_WAIT_ACK | RTW_CMDF_DIRECTLY);
		if (   tx_ret == _SUCCESS
		    || (   dump_cnt >= dump_limit
		        && rtw_get_passing_time_ms(start) >= retry_guarantee_ms))
			break;

		RTW_WARN(FUNC_ADPT_FMT" TX MGNT failed. Retry %u...\n",
		         FUNC_ADPT_ARG(padapter), dump_cnt);
	...
	if (tx_ret != _SUCCESS || dump_cnt > 1) {
		RTW_ERR(FUNC_ADPT_FMT" %s (%d/%d) in %d ms\n",
			 FUNC_ADPT_ARG(padapter),
			 tx_ret == _SUCCESS ? "OK" : "FAIL",
			 dump_cnt, dump_limit,
	...
	struct mgmt_frame_regs *upd)
{
	...
	u32 auth_mask = BIT(IEEE80211_STYPE_AUTH >> 4);
	bool intf_change;
...
#ifdef CONFIG_DEBUG_CFG80211
	RTW_INFO(FUNC_ADPT_FMT" global_stypes:0x%x, interface_stypes:0x%x\n",
		FUNC_ADPT_ARG(adapter),	upd->global_stypes, upd->interface_stypes,
	RTW_INFO(FUNC_ADPT_FMT" global_mcast_stypes:0x%x, interface_mcast_stypes:0x%x\n",
		FUNC_ADPT_ARG(adapter), upd->global_mcast_stypes, upd->interface_mcast_stypes);
#endif

	if (upd->interface_stypes & auth_mask)
		SET_CFG80211_REPORT_MGMT(pwdev_priv, IEEE80211_STYPE_AUTH, 0);
	else
		CLR_CFG80211_REPORT_MGMT(pwdev_priv, IEEE80211_STYPE_AUTH, 0);

#ifndef RTW_WKARD_DIS_PROBE_REQ_RPT_TO_HOSTAPD
	if (upd->interface_stypes & preq_mask)
		SET_CFG80211_REPORT_MGMT(pwdev_priv, IEEE80211_STYPE_PROBE_REQ, 0);
	else
...
void dump_mesh_setup(void *sel, struct wiphy *wiphy, const struct mesh_setup *setup)
{
	...
	if (setup->ie && setup->ie_len) {
		...
		dump_ies_g6(RTW_DBGDUMP, setup->ie, setup->ie_len);
	}
#else
	if (setup->vendor_ie && setup->vendor_ie_len) {
		RTW_PRINT_SEL(sel, "ie:%p, len:%u\n", setup->vendor_ie, setup->vendor_ie_len);
		dump_ies_g6(RTW_DBGDUMP, setup->vendor_ie, setup->vendor_ie_len);
	}
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0))
	...
	, uint *ies_len)
{
	...
	*(c + 1) = 4;
	...
	if (vht) {
		...
		vht_cap[0] = (u8)sta_vht_cap->cap;
		vht_cap[1] = (u8)(sta_vht_cap->cap >> 8);
		vht_cap[2] = (u8)(sta_vht_cap->cap >> 16);
		vht_cap[3] = (u8)(sta_vht_cap->cap >> 24);
	...
	dump_ies_g6(RTW_DBGDUMP, ies + _BEACON_IE_OFFSET_, len - _BEACON_IE_OFFSET_);
#endif
		...
		struct cfg80211_sched_scan_request *request)
{
	...
	for (i=0;i<request->n_match_sets;i++) {
			...
			_rtw_memcpy(ssids[i].ssid, request->match_sets[i].ssid.ssid,
...
static int rtw_cfg80211_set_beacon_wpsp2pie(struct net_device *ndev, char *buf, int len)
{
	...
	if (len > 0) {
		...
		if (wps_ielen > 0xFFFFFFFF) { //for coverity check
			RTW_ERR("wps_ielen > 0xFFFFFFFF\n");
			return -EINVAL;
		}
...
static int rtw_cfg80211_set_assoc_resp_wpsp2pie(struct net_device *net, char *buf, int len)
{
	...
	if (ie_len > 0xFFFFFFFF) { // for coverity check
		RTW_ERR("ie_len > 0xFFFFFFFF\n");
		goto exit;
	}
	...
	struct protocol_cap_t *dft_proto_cap, struct role_cap_t *dft_cap)
{
	...
	if (BAND_ON_5G == rtw_get_phyband_on_dev(padapter))
	...
	RTW_PRINT("%s: band_type=0x%x dev_id=0x%x ht_cap->cap=%x\n",
		  __func__, padapter->registrypriv.band_type,
		  padapter->dvobj->dev_id, ht_cap->cap);
	...
	ht_cap->mcs.rx_highest = cpu_to_le16(
		rtw_ht_mcs_rate(rtw_hw_is_band_support(adapter_to_dvobj(padapter), CHANNEL_WIDTH_40)
			, rtw_hw_is_band_support(adapter_to_dvobj(padapter), CHANNEL_WIDTH_40) ? ht_cap->cap & IEEE80211_HT_CAP_SGI_40 : ht_cap->cap & IEEE80211_HT_CAP_SGI_20
	...
	struct role_cap_t *dft_cap)
{
	...
	/* cap is u32 and is handled as a u32. But vht_cap_ie is byte array
	   in which lower index should be lower byte in cap */
	/* _rtw_memcpy(&sta_vht_cap->cap, vht_cap_ie + 2, 4); */
	sta_vht_cap->cap =   ((u32)*(vht_cap_ie + 2))
			   | ((u32)*(vht_cap_ie + 3) << 8)
			   | ((u32)*(vht_cap_ie + 4) << 16)
			   | ((u32)*(vht_cap_ie + 5) << 24);
...
#if defined(CONFIG_80211AX_HE) || (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 20, 0))
static void rtw_cfg80211_init_he_capab(_adapter *padapter,
	...
	struct protocol_cap_t *dft_proto_cap)
{
	...
	cap_len = rtw_get_dft_he_cap_ie(padapter, phy_cap, dft_proto_cap, he_cap_ie);
	    ...
	    struct protocol_cap_t *dft_ap_proto_cap)
{
	...
	rtw_cfg80211_init_he_capab(padapter, he_iftype,
					NL80211_IFTYPE_AP, phy_cap, dft_ap_proto_cap);

	he_iftype += 1;
	rtw_cfg80211_init_he_capab(padapter, he_iftype,
					NL80211_IFTYPE_STATION, phy_cap, dft_sta_proto_cap);
}
#endif /* defined(CONFIG_80211AX_HE) && (defined(CONFIG_VERSION) || LINUX_VERSION_CODE >= KERNEL_VERSION(4, 20, 0)) */
...
struct ieee80211_iface_limit rtw_limits[] = {
	...
	#if (CONFIG_CLIENT_NUMBER > 0)
        {
                .max = rtw_min(CONFIG_IFACE_NUMBER, CONFIG_CLIENT_NUMBER),
...
/*  CONFIG_RTW_A4_STA  ?? multi-sta ifaces ?? */
...
static int rtw_cfg80211_init_wiphy(_adapter *adapter, struct wiphy *wiphy)
{
	...
	wiphy->akm_suites = rtw_akm_suites;
	wiphy->n_akm_suites = ARRAY_SIZE(rtw_akm_suites);
...
#if !defined(CONFIG_VERSION) && defined(CONFIG_PM) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0) && \
...
static void rtw_cfg80211_set_survey_info(_adapter *padapter, u8 acs_idx, struct survey_info *pinfo)
{
	s8 noise = -50;			/*channel noise in dBm. This and all following fields are optional */
	...
	time_busy = rtw_acs_get_clm_ratio_by_idx(padapter, acs_idx);
	noise = rtw_acs_get_noise_by_idx(padapter, acs_idx);
...
int cfg80211_rtw_dump_survey(struct wiphy *wiphy, struct net_device *netdev, int idx, struct survey_info *info)
{
	...
	rtw_cfg80211_set_survey_info(padapter, acs_idx, info);

	RTW_INFO("[%d] ch=%d, time=%llu(ms), time_busy=%llu(ms), noise=%d(dbm)\n",
		idx, info->channel->hw_value, info->time, info->time_busy, info->noise);
	...
	struct rtw_external_auth_params *params)
{
	...
	if (MLME_IS_AP(padapter)) {
		...
		for (i = 0; i < pdvobj->iface_nums; i++) {
			...
			psta->state |= WIFI_FW_AUTH_SUCCESS;
			psta->expire_to = iface->stapriv.assoc_to;
			...
			if (buf) {
				rtw_ps_deny(iface, PS_DENY_MGNT_TX);
				if (_SUCCESS == rtw_pwr_wakeup(iface)) {
					rtw_mi_set_scan_deny(iface, 1000);
					rtw_mi_scan_abort(iface, _TRUE);

					RTW_INFO("SAE: Tx auth Confirm\n");
					rtw_mgnt_tx_cmd(iface, pmlmeext->cur_channel, 1,
						buf, len, 0, 0, RTW_CMDF_DIRECTLY);
				}
				rtw_ps_deny_cancel(iface, PS_DENY_MGNT_TX);
...
struct wiphy *rtw_wiphy_alloc(_adapter *padapter, struct device *dev)
{
	...
	if (wfo_enable && wfo_check_offload(padapter))
	wiphy = wiphy_new(&wfo_virt_cfg80211_ops, sizeof(struct rtw_wiphy_data));
	else
	...
	if (wfo_check_offload(padapter)) {
		init_wfo_cmd_virt_receiver(wiphy);
	}
#endif /* WFO_VIRT_RECEIVER */
...
int rtw_wdev_alloc(_adapter *padapter, struct wiphy *wiphy, enum nl80211_iftype type)
{
	int ret = 0;
	...
	if (padapter->registrypriv.power_mgnt != PM_PS_MODE_ACTIVE)
...
int rtw_cfg80211_ndev_res_alloc(_adapter *adapter, enum nl80211_iftype type)
{
	int ret = _FAIL;

	if (rtw_wdev_alloc(adapter, adapter_to_wiphy(adapter), type) == 0)
		ret = _SUCCESS;
