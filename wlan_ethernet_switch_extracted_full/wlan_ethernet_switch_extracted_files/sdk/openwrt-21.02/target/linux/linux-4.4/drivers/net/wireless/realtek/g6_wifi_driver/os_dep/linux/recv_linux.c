	...
	if (pkt) {
		...
		if (MLME_IS_STA(padapter) || MLME_IS_ADHOC(padapter)) {
...
#endif
			{
				...
				if (br_port) {
					...
					if (nat25_handle_frame_g6(padapter, pkt) == -1) {
						/* priv->ext_stats.rx_data_drops++; */
						/* DEBUG_ERR("RX DROP: nat25_handle_frame_g6 fail!\n"); */
						/* return FAIL; */
		...
		if (MLME_IS_STA(padapter) && (padapter->a4_enable == 1))
		{
			if(IS_MCAST(ehdr->h_dest))
			{
				...
				{
					if(rtw_is_l2uf_frame(pkt)) {
						rtw_invalid_a4_loop_entry(padapter, ehdr->h_source);
					} else {
