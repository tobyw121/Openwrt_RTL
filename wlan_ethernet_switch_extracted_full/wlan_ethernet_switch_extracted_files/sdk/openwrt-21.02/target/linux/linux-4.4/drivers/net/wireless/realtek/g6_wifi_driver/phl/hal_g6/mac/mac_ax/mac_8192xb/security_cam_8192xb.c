...
#define SEC_CAM_SIZE_8192XB	0x14
...
u32 sec_info_tbl_init_8192xb(struct mac_ax_adapter *adapter, u8 op_mode)
{
	u8 i = 0, keyidx = 0;
	...
	struct dctl_secinfo_entry_t *dctl_sec_info_entry;

	if (op_mode == SEC_CAM_BACKUP) {
		sec_cam_table = &adapter->hw_info->sec_cam_table_bk;
		dctl_sec_info = &adapter->hw_info->dctl_sec_info_bk;
	}
...
u32 free_sec_info_tbl_8192xb(struct mac_ax_adapter *adapter, u8 op_mode)
{
	u8 i;
	struct sec_cam_table_t *sec_cam_table = adapter->hw_info->sec_cam_table;
	struct dctl_sec_info_t *dctl_sec_info = adapter->hw_info->dctl_sec_info;
	...
	if (dctl_sec_info) {
		...
		if (op_mode == SEC_CAM_RESTORE)
			adapter->hw_info->dctl_sec_info_bk = NULL;
		else
...
u32 upt_dctl_secinfo_entry_8192xb(struct mac_ax_adapter *adapter,
				  u8 macid, u8 keyidx, u8 sec_iv_upd)
{
	...
	ret = mac_upd_dctl_info_8192xb(adapter, &info, &mask, macid, 1);
	if (ret != MACSUCCESS)
		return ret;
...
u32 fill_sec_cam_info_8192xb(struct mac_ax_adapter *adapter,
			     struct mac_ax_sec_cam_info *s_info,
			     struct fwcmd_seccam_info *sec_info,
			     u8 op_mode)
{
	if (op_mode == SEC_CAM_NORMAL) {
		sec_info->dword0 =
...
u32 mac_upd_sec_infotbl_8192xb(struct mac_ax_adapter *adapter,
...
u8 check_key_index_8192xb(u8 addr_cam_sec_mode, u8 key_type, u8 keyidx)
{
...
u8 decide_key_index_8192xb(u8 addr_cam_sec_mode, u8 key_type)
{
	u8 keyidx = 0;
...
u8 decide_sec_cam_index_8192xb(struct mac_ax_adapter *adapter, u8 *sec_cam_idx)
{
	u8 sec_idx = 0, i = 0;
	/* call by pointer */
...
u32 delete_key_from_dmac_tbl_8192xb(struct mac_ax_adapter *adapter,
				    ...
				    u8 key_id, u8 *sec_cam_idx)
{
	...
	keyidx = decide_key_index_8192xb(sec_ent_mode, key_type);
	...
	*sec_cam_idx = dctl_secinfo_entry->sec_ent[keyidx];
	...
	ret = upt_dctl_secinfo_entry_8192xb(adapter, mac_id, keyidx,
...
u32 insert_key_to_dmac_tbl_8192xb(struct mac_ax_adapter *adapter,
				  u8 mac_id, u8 key_type,
				  u8 key_id, u8 sec_cam_idx)
{
	u32 ret = 0;
	...
	if (check_key_index_8192xb(sec_ent_mode, key_type, keyidx)) {
...
u32 m_security_cam_hal_8192xb(struct mac_ax_adapter *adapter,
			      ...
			      u8 sec_cam_idx, u8 opmode)
{
	...
	struct sec_cam_table_t *sec_cam_table = adapter->hw_info->sec_cam_table;
	struct sec_cam_table_t *sec_cam_table_bk = adapter->hw_info->sec_cam_table_bk;
...
u32 disconnect_flush_key_8192xb(struct mac_ax_adapter *adapter,
				struct mac_role_tbl *role)
{
	...
	for (keyidx = 0; keyidx < KEYNUM_PER_MACID; keyidx++) {
		if (key_valid[keyidx] == VALID) {
			m_security_cam_hal_8192xb(adapter, sec_cam_info,
						  macid,
	...
	ret = upt_dctl_secinfo_entry_8192xb(adapter, macid, DCTL_TBL_FLUSH_MODE,
					    SEC_IV_UPD_TYPE_NONE);
	if (ret != MACSUCCESS)
		return ret;
...
u32 mac_sta_del_key_8192xb(struct mac_ax_adapter *adapter,
			   u8 mac_id,
			   u8 key_id,
			   u8 key_type)
{
	...
	ret = delete_key_from_dmac_tbl_8192xb(adapter, mac_id, key_type,
					      key_id, &sec_cam_idx);
	if (ret != MACSUCCESS)
		return ret;
	...
	sec_cam_info.len = SEC_CAM_SIZE_8192XB;

	fill_sec_cam_info_8192xb(adapter, &sec_cam_info,
				 (struct fwcmd_seccam_info *)sec_table, SEC_CAM_CLEAR);

	ret = (u8)mac_upd_sec_infotbl_8192xb(adapter,
					     (struct fwcmd_seccam_info *)sec_table);
	if (ret != MACSUCCESS)
		return ret;

	m_security_cam_hal_8192xb(adapter, NULL,
				  mac_id, key_id, key_type, sec_cam_idx, SEC_CAM_CLEAR);

	return MACSUCCESS;
}

u32 mac_sta_add_key_8192xb(struct mac_ax_adapter *adapter,
			   struct mac_ax_sec_cam_info *sec_cam_info,
			   u8 mac_id,
			   u8 key_id,
			   u8 key_type)
{
	...
	if (sec_cam_info->offset != FORCE_KEYCAM_INDEX_TYPE) {
		ret = decide_sec_cam_index_8192xb(adapter, &sec_cam_idx);
		if (ret != MACSUCCESS)
			return ret;
	} else {
	...
	ret = insert_key_to_dmac_tbl_8192xb(adapter, mac_id, key_type, key_id,
...
u32 mac_sta_keycam_backup_8192xb(struct mac_ax_adapter *adapter, u8 op_mode)
{
	...
	if (op_mode == SEC_CAM_BACKUP) {
		...
		ret = sec_info_tbl_init_8192xb(adapter, SEC_CAM_BACKUP);
		if (ret != MACSUCCESS) {
		...
		for (sec_cam_idx = 0; sec_cam_idx < SEC_CAM_ENTRY_NUM;
			...
			if (sec_cam_table->sec_cam_entry[sec_cam_idx]->valid == VALID) {
				m_security_cam_hal_8192xb(adapter, NULL, 0, 0, 0,
							  sec_cam_idx, SEC_CAM_BACKUP);
			}
		}

		ret = free_sec_info_tbl_8192xb(adapter, SEC_CAM_NORMAL);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]sec info tbl backup %d\n", ret);
			return ret;
		}
		...
		ret = sec_info_tbl_init_8192xb(adapter, SEC_CAM_NORMAL);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]sec info tbl backup %d\n", ret);
			return ret;
		}
		...
		for (sec_cam_idx = 0; sec_cam_idx < SEC_CAM_ENTRY_NUM; sec_cam_idx++) {
			...
			if (s_entry->valid == VALID) {
				...
				mac_sta_add_key_8192xb(adapter, &sec_cam_info,
		...
		ret = free_sec_info_tbl_8192xb(adapter, SEC_CAM_RESTORE);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]sec info tbl backup %d\n", ret);
			return ret;
		}
...
u32 mac_sta_search_key_idx_8192xb(struct mac_ax_adapter *adapter,
				  u8 mac_id, u8 key_id, u8 key_type)
{
	u32 sec_cam_idx = 0;
	struct sec_cam_entry_t *s_entry = NULL;
...
u32 mac_sta_hw_security_support_8192xb(struct mac_ax_adapter *adapter,
...
u8 check_key_type_8192xb(u8 addr_cam_sec_mode, u8 keyidx)
{
	switch (addr_cam_sec_mode) {
	case ADDR_CAM_SEC_MODE_ZERO:
		if (keyidx <= 6)
...
u32 refresh_security_cam_info_8192xb(struct mac_ax_adapter *adapter,
				     struct mac_ax_sec_iv_info *sec_iv_info)
{
	...
	u32 dword[10] = {0};
	u8 dmac_tbl_size = DCTRL_TBL_SIZE_8192XB;
	...
	for (keyidx = 0; keyidx < 7; keyidx++) {
		if (key_valid[keyidx] == VALID) {
			key_type = check_key_type_8192xb(sec_ent_mode, keyidx);
			m_security_cam_hal_8192xb(adapter, NULL,
...
u32 mac_wowlan_secinfo_8192xb(struct mac_ax_adapter *adapter,
			      struct mac_ax_sec_iv_info *sec_iv_info)
{
	u8 mac_id = 0;
	u32 ret = 0;
