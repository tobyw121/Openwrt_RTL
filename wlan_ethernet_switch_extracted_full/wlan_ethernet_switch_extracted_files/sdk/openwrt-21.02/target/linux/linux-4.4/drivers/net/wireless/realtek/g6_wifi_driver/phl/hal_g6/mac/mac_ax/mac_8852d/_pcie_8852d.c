...
static struct mac_ax_intf_info intf_info_def_8852d = {
	MAC_AX_BD_NORM,
	MAC_AX_BD_NORM,
	MAC_AX_RXBD_SEP,
	MAC_AX_TAG_MULTI,
...
static struct mac_ax_pcie_ltr_param pcie_ltr_param_def_8852d = {
	0,
	0,
	MAC_AX_PCIE_ENABLE,
	MAC_AX_PCIE_ENABLE,
...
static u32 ltr_ltcy_regl_8852d[PCIE_LTR_IDX_MAX] = {
	R_AX_LTR_LATENCY_IDX0, R_AX_LTR_LATENCY_IDX1,
	R_AX_LTR_LATENCY_IDX2, R_AX_LTR_LATENCY_IDX3
};

static struct mac_ax_pcie_cfgspc_param pcie_cfgspc_param_def_8852d = {
	0,
	0,
	MAC_AX_PCIE_DISABLE,
	MAC_AX_PCIE_ENABLE,
...
struct txbd_ram mac_bdram_tbl_8852d[] = {
		/* ACH0_QUEUE_IDX_8852DE */ {0, 5, 2},
		/* ACH1_QUEUE_IDX_8852DE */ {5, 5, 2},
		/* ACH2_QUEUE_IDX_8852DE */ {10, 5, 2},
		/* ACH3_QUEUE_IDX_8852DE */ {15, 5, 2},
		/* ACH4_QUEUE_IDX_8852DE */ {20, 5, 2},
		/* ACH5_QUEUE_IDX_8852DE */ {25, 5, 2},
		/* ACH6_QUEUE_IDX_8852DE */ {30, 5, 2},
		/* ACH7_QUEUE_IDX_8852DE */ {35, 5, 2},
		/* MGQ_B0_QUEUE_IDX_8852DE */ {40, 5, 1},
		/* HIQ_B0_QUEUE_IDX_8852DE */ {45, 5, 1},
		/* MGQ_B1_QUEUE_IDX_8852DE */ {50, 5, 1},
		/* HIQ_B1_QUEUE_IDX_8852DE */ {55, 5, 1},
		/* FWCMD_QUEUE_IDX_8852DE */ {60, 4, 1}
};
...
get_pcie_info_def_8852d(struct mac_ax_adapter *adapter)
{
	return &intf_info_def_8852d;
}

struct txbd_ram *
get_bdram_tbl_pcie_8852d(struct mac_ax_adapter *adapter)
{
	return mac_bdram_tbl_8852d;
}

u32 mio_w32_pcie_8852d(struct mac_ax_adapter *adapter, u16 addr, u32 value)
{
...
u32 mio_r32_pcie_8852d(struct mac_ax_adapter *adapter, u16 addr, u32 *val)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u8 addr_lsb;
	u8 page_sel;
...
u32 get_txbd_reg_pcie_8852d(struct mac_ax_adapter *adapter, u8 dma_ch, u32 *reg,
...
u32 set_txbd_reg_pcie_8852d(struct mac_ax_adapter *adapter, u8 dma_ch,
...
u32 get_rxbd_reg_pcie_8852d(struct mac_ax_adapter *adapter, u8 dma_ch, u32 *reg,
			    enum pcie_bd_ctrl_type type)
{
	*reg = MAC_AX_R32_FF;
...
u32 set_rxbd_reg_pcie_8852d(struct mac_ax_adapter *adapter, u8 dma_ch,
			    enum pcie_bd_ctrl_type type, u32 val0, u32 val1, u32 val2)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
...
u32 pcie_cfgspc_write_8852d(struct mac_ax_adapter *adapter,
			    struct mac_ax_pcie_cfgspc_param *param)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	struct mac_ax_pcie_cfgspc_param *param_def =
		&pcie_cfgspc_param_def_8852d;
...
u32 pcie_cfgspc_read_8852d(struct mac_ax_adapter *adapter,
			   struct mac_ax_pcie_cfgspc_param *param)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
...
u32 pcie_ltr_write_8852d(struct mac_ax_adapter *adapter,
			 struct mac_ax_pcie_ltr_param *param)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	struct mac_ax_pcie_ltr_param *param_def = &pcie_ltr_param_def_8852d;
	...
	disable_ltcy = MAC_REG_R32(ltr_ltcy_regl_8852d[PCIE_LTR_IDX_DISABLE]);
	idle_ltcy = MAC_REG_R32(ltr_ltcy_regl_8852d[PCIE_LTR_IDX_IDLE]);
	act_ltcy = MAC_REG_R32(ltr_ltcy_regl_8852d[PCIE_LTR_IDX_ACT]);
	...
	MAC_REG_W32(ltr_ltcy_regl_8852d[PCIE_LTR_IDX_DISABLE], disable_ltcy);
	MAC_REG_W32(ltr_ltcy_regl_8852d[PCIE_LTR_IDX_IDLE], idle_ltcy);
	MAC_REG_W32(ltr_ltcy_regl_8852d[PCIE_LTR_IDX_ACT], act_ltcy);

	return MACSUCCESS;
}

u32 pcie_ltr_read_8852d(struct mac_ax_adapter *adapter,
			struct mac_ax_pcie_ltr_param *param)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 ctrl0, ctrl1, dec_ctrl, disable_ltcy, idle_ltcy, act_ltcy;
...
u32 ltr_sw_trigger_8852d(struct mac_ax_adapter *adapter,
...
u32 get_avail_txbd_8852d(struct mac_ax_adapter *adapter, u8 ch_idx,
...
u32 get_avail_rxbd_8852d(struct mac_ax_adapter *adapter, u8 ch_idx,
...
u32 get_io_stat_pcie_8852d(struct mac_ax_adapter *adapter,
			   struct mac_ax_io_stat *out_st)
{
	return MACNOTSUP;
}

u32 ctrl_hci_dma_en_pcie_8852d(struct mac_ax_adapter *adapter,
...
u32 ctrl_trxdma_pcie_8852d(struct mac_ax_adapter *adapter,
			   enum mac_ax_pcie_func_ctrl txen,
			   enum mac_ax_pcie_func_ctrl rxen,
...
u32 ctrl_txdma_ch_pcie_8852d(struct mac_ax_adapter *adapter,
...
u32 ctrl_wpdma_pcie_8852d(struct mac_ax_adapter *adapter,
...
u32 poll_io_idle_pcie_8852d(struct mac_ax_adapter *adapter)
{
...
u32 poll_txdma_ch_idle_pcie_8852d(struct mac_ax_adapter *adapter,
				  struct mac_ax_txdma_ch_map *ch_map)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_busy1 = R_AX_HAXI_DMA_BUSY1;
...
u32 poll_rxdma_ch_idle_pcie_8852d(struct mac_ax_adapter *adapter,
...
u32 poll_dma_all_idle_pcie_8852d(struct mac_ax_adapter *adapter)
{
	...
	ret = poll_txdma_ch_idle_pcie_8852d(adapter, &txch_map);
	if (ret != MACSUCCESS) {
	...
	ret = poll_rxdma_ch_idle_pcie_8852d(adapter, &rxch_map);
	if (ret != MACSUCCESS) {
...
u32 clr_idx_ch_pcie_8852d(struct mac_ax_adapter *adapter,
			  struct mac_ax_txdma_ch_map *txch_map,
			  struct mac_ax_rxdma_ch_map *rxch_map)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
...
u32 rst_bdram_pcie_8852d(struct mac_ax_adapter *adapter, u8 val)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_init = R_AX_HAXI_INIT_CFG1;
	u32 cnt, val32;
...
u32 trx_mit_pcie_8852d(struct mac_ax_adapter *adapter,
...
u32 mode_op_pcie_8852d(struct mac_ax_adapter *adapter,
...
u32 get_err_flag_pcie_8852d(struct mac_ax_adapter *adapter,
...
static u32 get_target_8852d(struct mac_ax_adapter *adapter, u16 *target,
			    u32 phy_offset)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u16 tmp_u16, count;
...
u32 mac_auto_refclk_cal_pcie_8852d(struct mac_ax_adapter *adapter,
				   enum mac_ax_pcie_func_ctrl en)
{
	...
	ret = get_target_8852d(adapter, &tar, phy_offset);
	if (ret != MACSUCCESS) {
