...
bool halrf_bw_setting_8832crvu(struct rf_info *rf, enum rf_path path, enum channel_width bw, bool is_dav)
{
	u32 rf_reg18 = 0;
	...
	/*==== [Write RF register] ====*/
	rf_reg18 = (u32)(rf_reg18 & 0xf0fff) | BIT12;
...
bool halrf_ctrl_bw_8832crvu(struct rf_info *rf, enum phl_phy_idx phy, enum channel_width bw)
{
	...
	for (path = 0; path < 2; path++) {
		if (kpath & BIT(path)) {
			is_dav = true;
			halrf_bw_setting_8832crvu(rf, path, bw, is_dav);
			is_dav = false;
			halrf_bw_setting_8832crvu(rf, path, bw, is_dav);
...
bool halrf_ch_setting_8832crvu(struct rf_info *rf,   enum rf_path path, u8 central_ch, enum band_type band, bool is_dav)
{
	u32 rf_reg18 = 0;	
	u32 reg_reg18_addr = 0x0;
...
bool halrf_ctrl_ch_8832crvu(struct rf_info *rf,  enum phl_phy_idx phy, u8 central_ch, enum band_type band)
{
	...
	for (path = 0; path < 2; path++) {
		if (kpath & BIT(path)) {
			is_dav = true;
			halrf_ch_setting_8832crvu(rf, path, central_ch, band, is_dav);
			is_dav = false;
			halrf_ch_setting_8832crvu(rf, path, central_ch, band, is_dav);	
		}
	}
...
void halrf_set_lo_8832crvu(struct rf_info *rf, bool is_on, enum rf_path path)
{
	u32 rf18;
	bool not2g = true;
	...
	if (is_on) {
		if (not2g) {
			halrf_rf_direct_cntrl_8832crvu(rf, path, false);
		...
		halrf_rf_direct_cntrl_8832crvu(rf, path, true);
	}
}
...
void halrf_lo_test_8832crvu(struct rf_info *rf, bool is_on, enum rf_path path)
{
	switch (path) {
		case RF_PATH_A:
			halrf_set_lo_8832crvu(rf, is_on, RF_PATH_A);
			halrf_set_lo_8832crvu(rf, false, RF_PATH_B);
			break;
		case RF_PATH_B:
			halrf_set_lo_8832crvu(rf, false, RF_PATH_A);
			halrf_set_lo_8832crvu(rf, is_on, RF_PATH_B);
			break;
		case RF_PATH_AB:
			halrf_set_lo_8832crvu(rf, is_on, RF_PATH_A);
			halrf_set_lo_8832crvu(rf, is_on, RF_PATH_B);
...
void _rx_dck_info_8832crvu(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_afe)
{
...
void halrf_rx_dck_toggle_8832crvu(struct rf_info *rf, enum rf_path path)
{
	u32 cnt = 0;
...
void halrf_rx_dck_8832crvu(struct rf_info *rf, enum phl_phy_idx phy, bool is_afe) 
{
	...
	RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] ****** RXDCK Start (Ver: 0x%x, Cv: %d) ******\n",
		RXDCK_VER_8832CRVU, rf->hal_com->cv);
	...
	for (path = 0; path < 2; path++) {
		...
		halrf_set_rx_dck_8832crvu(rf, phy, path, is_afe);
		rx_dck->ther_rxdck[path] = halrf_get_thermal_8832crvu(rf, path);
...
void halrf_rxdck_track_8832crvu(
	struct rf_info *rf)
{
	struct halrf_rx_dck_info *rx_dck = &rf->rx_dck;
...
void iqk_backup_8832crvu(struct rf_info *rf, enum rf_path path) 
{
	return;
}
...
bool halrf_rfk_reg_check_fail_8832crvu(struct rf_info *rf)
{
	...
	halrf_wreg(rf, 0x81d8, MASKDWORD, 0x00010001);
	halrf_wreg(rf, 0x82d8, MASKDWORD, 0x00010001);
	fail = halrf_rfk_reg_check_8832crvu(rf);
	halrf_wreg(rf, 0x81d8, MASKDWORD, 0x00000000);
	halrf_wreg(rf, 0x81dc, MASKDWORD, 0x00010003);	
	halrf_wreg(rf, 0x81dc, MASKDWORD, 0x00000002);
	halrf_wreg(rf, 0x82d8, MASKDWORD, 0x00000000);
...
void halrf_set_rxbb_bw_8832crvu(struct rf_info *rf, enum channel_width bw, enum rf_path path)
{
...
void halrf_rxbb_bw_8832crvu(struct rf_info *rf, enum phl_phy_idx phy, enum channel_width bw)
{
	...
	for (path = 0; path < 2; path++) {
		if ((kpath & BIT(path)) && (rf->pre_rxbb_bw[path] != bw)) {
			halrf_set_rxbb_bw_8832crvu(rf, bw, path);
...
bool halrf_one_shot_nctl_done_check_8832crvu(struct rf_info *rf, enum rf_path path) {	
	...
	while (count < 2000) {	
		...
		if (r_80fc == NCTL_FINAL_LINE_8832CRVU) {
			is_ready = true;
			break;
		}	
...
bool halrf_do_one_shot_8832crvu(struct rf_info *rf, enum rf_path path, u32 nctl_addr, u32 mask, u32 process_id)
{
	...
	return halrf_one_shot_nctl_done_check_8832crvu(rf, path);
}

bool halrf_set_ch_lck_8832crvu(struct rf_info *rf, u32 path, u32 val)
{
	u32 c = 0;
	bool timeout = false;
...
void halrf_lck_8832crvu(struct rf_info *rf)
{
	...
	if (!rf->hal_com->dbcc_en) {
		...
		halrf_set_ch_lck_8832crvu(rf, 0, temp18);
		...
		for(i = 0; i < 2; i++) {
			...
			halrf_set_ch_lck_8832crvu(rf, i, temp18);
...
void halrf_lck_tracking_8832crvu(struct rf_info *rf)
{
	...
	RF_DBG(rf, DBG_RF_LCK, "[LCK]MAX_delta = %d, LCK_TH=%x\n",
		temp, LCK_TH_8832CRVU);

	if(temp >= LCK_TH_8832CRVU)		
		halrf_lck_trigger_g6(rf);
}

static u32 check_rfc_reg_8832crvu[] = {0xfe, 0x0, 0x0, 0x0, 0x0,
	...
	0xb2, 0xc5};
static u32 check_dack_reg_8832crvu[] = {0x12a0, 0x32a0, 0x12b8, 0x32b8, 0x030c,
	...
	0xc164};
static u32 check_iqk_reg_8832crvu[] = {0x8000, 0x8004, 0x8008, 0x8080, 0x808c,
	...
	0x9f90, 0x9fa0};
static u32 check_dpk_reg_8832crvu[] = {0x80b0, 0x81bc, 0x82bc, 0x81b4, 0x82b4,
	0x81c4, 0x82c4, 0x81c8, 0x82c8, 0x58d4,
	0x78d4};
static u32 check_tssi_reg_8832crvu[] = {0x0304, 0x5818, 0x581c, 0x5820, 0x1c60,
...
{
	u32 path, temp, i;
	u32	len = sizeof(check_rfc_reg_8832crvu) / sizeof(u32);
	u32	*add = (u32 *)check_rfc_reg_8832crvu;
	...
	/*check DACK*/
	RF_TRACE("======DACK======\n");
	len = sizeof(check_dack_reg_8832crvu) / sizeof(u32);
	add = check_dack_reg_8832crvu;
	for (i = 0; i < len; i++ ) {
	...
	/*check IQK*/
	RF_TRACE("======IQK======\n");
	len = sizeof(check_iqk_reg_8832crvu) / sizeof(u32);
	add = check_iqk_reg_8832crvu;
	...
	/*check DPK*/
	RF_TRACE("======DPK======\n");
	len = sizeof(check_dpk_reg_8832crvu) / sizeof(u32);
	add = check_dpk_reg_8832crvu;
	for (i = 0; i < len; i++ ) {
		temp = halrf_rreg(rf, add[i], MASKDWORD);
		RF_TRACE("0x%x = 0x%x\n", add[i], temp);
	}
	/*check TSSI*/
	RF_TRACE("======TSSI======\n");
	len = sizeof(check_tssi_reg_8832crvu) / sizeof(u32);
	add = check_tssi_reg_8832crvu;
	for (i = 0; i < len; i++ ) {
		temp = halrf_rreg(rf, add[i], MASKDWORD);
		RF_TRACE("0x%x = 0x%x\n", add[i], temp);
	}
...
static u32 backup_mac_reg_8832crvu[] = {0x0};
static u32 backup_bb_reg_8832crvu[] = {0x2344,0xc0d4,0xc0d8, 0xc0e8, 0xc1d4,0xc1d8, 0xc1e8};
static u32 backup_rf_reg_8832crvu[] = {0xdc, 0xef, 0xde, 0x0, 0x1e, 0x5, 0x10005};

static struct halrf_iqk_ops iqk_ops= {
    .iqk_kpath = halrf_kpath_8832crvu,
    .iqk_mcc_page_sel = iqk_mcc_page_sel_8832crvu,
    .iqk_get_ch_info = iqk_get_ch_info_8832crvu,
    .iqk_preset = iqk_preset_8832crvu,
    .iqk_macbb_setting = iqk_macbb_setting_8832crvu,
    .iqk_start_iqk = iqk_start_iqk_8832crvu,
    .iqk_restore = iqk_restore_8832crvu,
    .iqk_afebb_restore = iqk_afebb_restore_8832crvu,  
};

struct rfk_iqk_info rf_iqk_hwspec_8832crvu = {
  	.rf_iqk_ops = &iqk_ops,
	.rf_max_path_num = 2,
	.rf_iqk_version = iqk_version_8832crvu,
	.rf_iqk_ch_num = 2,
	.rf_iqk_path_num = 2,

#if 0
	.backup_mac_reg = backup_mac_reg_8832crvu,
	.backup_mac_reg_num = ARRAY_SIZE(backup_mac_reg_8832crvu),
#else
	.backup_mac_reg = backup_mac_reg_8832crvu,
	.backup_mac_reg_num = 0,
#endif

    	.backup_bb_reg = backup_bb_reg_8832crvu,
    	.backup_bb_reg_num = ARRAY_SIZE(backup_bb_reg_8832crvu),
    	.backup_rf_reg = backup_rf_reg_8832crvu,
    	.backup_rf_reg_num = ARRAY_SIZE(backup_rf_reg_8832crvu),
};
