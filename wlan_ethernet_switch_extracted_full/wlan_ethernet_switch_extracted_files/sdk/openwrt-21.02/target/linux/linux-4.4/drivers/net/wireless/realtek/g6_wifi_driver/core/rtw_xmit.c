...
static s32 update_attrib_sec_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta, enum eap_type eapol_type)
{
	...
	if (psta->ieee8021x_blocked == _TRUE ||
		...
		if (eapol_type == EAPOL_2_4 || eapol_type == EAPOL_4_4)
			RTW_INFO(ADPT_FMT" respond unencrypted eapol key\n",
				 ADPT_ARG(padapter));
...
static void set_qos(_adapter *padapter, struct sk_buff *pkt, struct pkt_attrib *pattrib)
{
...
#ifdef CONFIG_RTW_MULTI_AP_R3 // store it first. It is already changed in tos_to_up based on dscp to pcp table.
       _adapter * primary_adapter;
       u8 dscp_pcp_table_mapped_value = pkt->cb[_SKB_CB_PRIORITY];
#endif
	...
	if (pattrib->ether_type == ETH_P_ARP)
       ...
       if (primary_adapter->service_priority_enabled == 1) {
               if (primary_adapter->service_priority_output <= 7) {
                       ...
                       if (primary_adapter->dscp_pcp_table_enabled == 1) {
                               UserPriority = dscp_pcp_table_mapped_value;
                       }
               }
       }
#endif
...
static s32 update_xmitframe_security(_adapter *padapter, struct xmit_frame *pxframe) //rtw_phl_tx todo
{
	...
	case _WEP104_:
		XF_IVLEN = 4;
		XF_ICVLEN = 4;
		WEP_IV(XF_IV, XF_STA->dot11txpn, XF_KEYIDX);
		break;
	...
	case _TKIP_:
		...
		if (bmcast)
			TKIP_IV(XF_IV, XF_STA->dot11txpn, XF_KEYIDX);
		else
			TKIP_IV(XF_IV, XF_STA->dot11txpn, 0);
	...
	case _AES_:
		...
		if (bmcast)
			AES_IV(XF_IV, XF_STA->dot11txpn, XF_KEYIDX);
		else
			AES_IV(XF_IV, XF_STA->dot11txpn, 0);
	...
	case _GCMP_256_:
		...
		if (bmcast)
			GCMP_IV(XF_IV, XF_STA->dot11txpn, XF_KEYIDX);
		else
			GCMP_IV(XF_IV, XF_STA->dot11txpn, 0);
...
#endif
	else if (pattrib->ether_type == ETH_P_1905) {
		pattrib->pkt_1905 = 1;
	}
			...
			/* if (!priv->pmib->ethBrExtInfo.nat25_disable)		 */
		{
			...
			if (*((unsigned short *)(skb->data + MACADDRLEN * 2)) == __constant_htons(ETH_P_IP)) {
				if (memcmp(padapter->scdb_mac, skb->data + MACADDRLEN, MACADDRLEN)) {
					void *scdb_findEntry_g6(_adapter *priv, unsigned char *macAddr, unsigned char *ipAddr);

					padapter->scdb_entry = (struct nat25_network_db_entry *)scdb_findEntry_g6(padapter,
			...
			if (do_nat25) {
				...
				if (nat25_db_handle_g6(padapter, skb, NAT25_CHECK) == 0) {
					#if 0
					struct sk_buff *newskb;
				...
				res = nat25_db_handle_g6(padapter, skb, NAT25_INSERT);
				if (res < 0) {
					if (res == -2) {
						/* priv->ext_stats.tx_drops++; */
						DEBUG_ERR("TX DROP: nat25_db_handle_g6 fail!\n");
						/* goto free_and_stop; */
						return -1;

					}
					/* we just print warning message and let it go */
					/* DEBUG_WARN("%s()-%d: nat25_db_handle_g6 INSERT Warning!\n", __FUNCTION__, __LINE__); */
			...
			dhcp_flag_bcast_g6(padapter, skb);
...
s32 core_br_client_tx(_adapter *padapter, struct xmit_frame *pxframe, struct sk_buff **pskb)
{
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE | WIFI_ADHOC_STATE) == _TRUE) {
		...
		if (linux_vlan_enable) {
			ndev = rtw_br_client_get_vlan_dev(padapter, *pskb);
			if (!ndev) {
				DBG_COUNTER(padapter->tx_logs.core_tx_err_brtx);
				return FAIL;
			}
			       ...
			       u32 frag_perfr, u32 wl_frags, struct sta_info *psta)
{
	...
	head_sz = XF_WL_HDRLEN + ((XF_WL_AMSDU || rtw_is_l2uf_frame(pxframe->pkt)) ? 0 : RTW_SZ_LLC);
	tail_sz = 0;
	if (XF_SEC_TYPE) {
		head_sz += XF_IVLEN;
		if (XF_SEC_TYPE == _TKIP_)
...
u8 _stbc_mapping(_adapter *padapter, struct sta_info *psta)
{
	...
	if (phl_sta_i->wmode & WLAN_MD_11AX) {
		if (!wrole->proto_role_cap.stbc_he_tx)
			stbc_en = 0;
		else if ((wrole->proto_role_cap.he_su_bfmr ||
		     wrole->proto_role_cap.he_mu_bfmr) &&
		    (asoc_cap_i->he_su_bfme || asoc_cap_i->he_mu_bfme))
			stbc_en = 0;
		else if (asoc_cap_i->stbc_he_rx != 0 && asoc_cap_i->nss_rx == 1)
			stbc_en = 1;
	} else if (phl_sta_i->wmode & WLAN_MD_11AC) {
		if (!wrole->proto_role_cap.stbc_vht_tx)
			stbc_en = 0;
		else if ((wrole->proto_role_cap.vht_su_bfmr ||
		     wrole->proto_role_cap.vht_mu_bfmr) &&
		    (asoc_cap_i->vht_su_bfme || asoc_cap_i->vht_mu_bfme))
			stbc_en = 1;
		else if (asoc_cap_i->nss_rx <= asoc_cap_i->stbc_vht_rx)
			stbc_en = 1;
	} else if  (phl_sta_i->wmode & WLAN_MD_11N) {
		if (!wrole->proto_role_cap.stbc_ht_tx)
			stbc_en = 0;
		else if (wrole->proto_role_cap.ht_su_bfmr &&
		    asoc_cap_i->ht_su_bfme)
			stbc_en = 0;
		else if (asoc_cap_i->nss_rx <= asoc_cap_i->stbc_ht_rx)
			stbc_en = 1;
	}
	return stbc_en;
}
...
void fill_txreq_mdata(_adapter *padapter, struct xmit_frame *pxframe)
{
	...
	if (XF_SEC_TYPE == _NO_PRIVACY_ || XF_SWENC == _TRUE) {
		...
		if (mdata->bc || mdata->mc)
		{
			mdata->hw_sec_iv = 0;
			mdata->sw_sec_iv = 0;
			memcpy(mdata->iv, pxframe->attrib.iv, sizeof(mdata->iv));
		}
#ifdef RTW_WKARD_UPDATE_DMAC_KEYID
		if (psta) {
			if (psta->dmac_updated == _TRUE) {
				mdata->hw_sec_iv = 1;
				mdata->sw_sec_iv = 0;
			} else {
				mdata->hw_sec_iv = 0;
				mdata->sw_sec_iv = 1;
				psta->dmac_updated = _TRUE;
			}
		}
#endif
	...
	if (XF_TYPE == RTW_TX_DRV_MGMT) {
		...
		if (pxframe->attrib.dhcp_pkt ||
			pxframe->attrib.arp_pkt ||
			pxframe->attrib.pkt_1905)
		{
			mdata->life_time_sel = 0;
		}
		else
...
void core_wlan_fill_head(_adapter *padapter, struct xmit_frame *pxframe)
{
	if(XF_TYPE == RTW_TX_OS){
		...
		for(idx=0; idx<XF_NR_FRAGS; idx++){
			...
			if (idx == 0 && !XF_WL_AMSDU && !rtw_is_l2uf_frame(pxframe->pkt)) {
...
s32 core_tx_call_phl(_adapter *padapter, struct xmit_frame *pxframe, void *txsc_pkt)
{
	...
	for(idx = 0; idx < txreq_cnt; idx++){
		...
		} else if (txreq->mdata.type == RTW_PHL_PKT_TYPE_MGNT)
...
s32 rtw_core_tx(_adapter *padapter, struct sk_buff **pskb, struct sta_info *psta)
{
	...
	if ((XF_SEC_TYPE!=_NO_PRIVACY_) && (XF_SWENC!=_TRUE)) {
		/*
		 * For hw enc, if not updated, do NOT create TXSC entry,
		 * in case mdata of TXSC entry abnormal for later packets.
		 */
		if (psta && psta->dmac_updated == _FALSE) {
			skip_add_txsc = 1;
			RTW_DBG("skip add TXSC, due to dmac_updated=%d\n", psta->dmac_updated);
		}
	}
#endif
...
u32 rtw_core_tx_dev_map(void *drv_priv, struct rtw_xmit_req *txreq)
{
	...
	if (   txreq->treq_type == RTW_PHL_TREQ_TYPE_CORE_TXSC
		...
		struct xmit_txreq_buf *ptxreq_buf = (struct xmit_txreq_buf *)txreq->os_priv;
...
void upd_hndsk_pkt_sts(_adapter *padapter, u8 category, u8 no_ack)
{
	switch (no_ack) {
	case TX_STATUS_TX_DONE:
		padapter->tx_logs.core_hndsk_tx_ok_cnt[category]++;
		break;
	case TX_STATUS_TX_FAIL_REACH_RTY_LMT:
	...
	case TX_STATUS_TX_FAIL_FORCE_DROP_BY_STUCK:
		RTW_PRINT("[%s] Handshake packet type:%d tx fail reason:%d", __func__, category, no_ack);
		padapter->tx_logs.core_hndsk_tx_fail_cnt[category]++;
		break;
	default:
		break;
	}
...
enum rtw_phl_status rtw_core_tx_recycle(void *drv_priv, struct rtw_xmit_req *txreq)
{
	...
	if (pxframe->attrib.ether_type == 0x888e) {
		upd_hndsk_pkt_sts(padapter, pxframe->attrib.hndsk_type, txreq->mdata.no_ack);
	}
#endif
#ifdef RTW_PHL_DBG_CMD
	core_add_record(padapter, padapter->record_enable, REC_TX_PHL_RCC, txreq);
