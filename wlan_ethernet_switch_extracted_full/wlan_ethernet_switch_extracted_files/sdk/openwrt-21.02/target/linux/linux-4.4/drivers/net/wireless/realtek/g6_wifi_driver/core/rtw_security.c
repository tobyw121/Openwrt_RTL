...
u32 rtw_core_aes_encrypt(_adapter *padapter, u8 *pxmitframe)
{
	...
	pwlanhdr = XF_WLHDR[0];
	payload = XF_SKB->data + XF_HDRLEN - RTW_SZ_LLC;

	/* start to encrypt each fragment */
	if ((XF_SEC_TYPE == _AES_) ||
	    (XF_SEC_TYPE == _CCMP_256_)) {

		if (IS_MCAST(XF_RA))
			prwskey = psecuritypriv->dot118021XGrpKey[psecuritypriv->dot118021XGrpKeyid].skey;
		else {
			prwskey = XF_STA->dot118021x_UncstKey.skey;
		}
...
#ifdef CONFIG_TDLS
		{
			...
			ptdls_sta = rtw_get_stainfo(&padapter->stapriv, XF_DA);
		...
		prwskeylen = (XF_SEC_TYPE == _CCMP_256_) ? 32 : 16;

		for (curfragnum = 0; curfragnum < XF_NR_FRAGS; curfragnum++) {
			u8 *pdata = XF_SKB->data + XF_HDRLEN - RTW_SZ_LLC + curfragnum*XF_FRAG_DATA_LEN;

			if ((curfragnum + 1) == XF_NR_FRAGS) {    /* the last fragment */
				u32 ls_datelen = XF_SKB->len - curfragnum*XF_FRAG_DATA_LEN;
				_rtw_core_ccmp_encrypt(prwskey, prwskeylen,
					XF_WL_HDRLEN, XF_WLHDR[curfragnum], ls_datelen, pdata);
			} else {
				_rtw_core_ccmp_encrypt(prwskey, prwskeylen,
					XF_WL_HDRLEN, XF_WLHDR[curfragnum], XF_FRAG_DATA_LEN, pdata);
			}
		}

		AES_SW_ENC_CNT_INC(psecuritypriv, XF_RA);
...
u32 rtw_aes_encrypt(_adapter *padapter, u8 *pxmitframe)
{
	...
	for (curfragnum = 0; curfragnum < pattrib->nr_frags; curfragnum++) {
		...
		plen = pkt_list->length - pattrib->hdrlen
		       - pattrib->iv_len - pattrib->icv_len;
		_rtw_ccmp_encrypt(prwskey, prwskeylen, pattrib->hdrlen,
				  pframe, plen);
	}
		      ...
		      u8 *mic)
{
	...
	*pos++ = trans_seq;
	...
	/* ret = omac1_aes_128_g6(kck, buf, pos - buf, mic); */
...
u32 rtw_gcmp_encrypt(_adapter *padapter, u8 *pxmitframe)
{
	...
	for (curfragnum = 0; curfragnum < pattrib->nr_frags; curfragnum++) {
		...
		plen = pkt_list->length - pattrib->hdrlen
		       - pattrib->iv_len - pattrib->icv_len;
		_rtw_gcmp_encrypt(prwskey, prwskeylen, pattrib->hdrlen,
				  pframe, plen);
	}
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	} else if (gmcs == _BIP_CMAC_SM4_128_) {
		if (_bip_sms4_protect(whdr_pos, len, key, 16,
				      data, data_len, mic) == _FALSE) {
			res = _FAIL;
			RTW_ERR("%s: _bip_sms4_protect(128) fail!\n", __func__);
		}
#endif
	} else {
		res = _FAIL;
...
u32 rtw_bip_verify(enum security_type gmcs, u8 *whdr_pos, sint flen,
		   const u8 *key, u16 keyid, u64 *ipn, u8 *precvframe)
{
	u8 *BIP_AAD = NULL, *mme = NULL;
	u32 res = _FAIL;
	uint ori_len = 0;
	u16 pkt_keyid = 0;
	u64 pkt_ipn = 0;
...
#ifdef CONFIG_RTL_CFG80211_WAPI_SUPPORT
	if (gmcs == _BIP_CMAC_SM4_128_)
	{
		mme_offset = sizeof(struct wapi_mmie);
		mme = whdr_pos + flen - mme_offset;

		if (rtw_wapi_verify_mmie(mme, keyid, (u8 *)ipn) == _FAIL) {
			RTW_INFO("Invalid WAPI MMIE!\n");
			return _FAIL;
		}
	}
	else
	...
	_rtw_memcpy(BIP_AAD + 2, whdr_pos + 4, 3 * ETH_ALEN);

	if (rtw_calculate_bip_mic(gmcs, whdr_pos, flen, key,
				  BIP_AAD, ori_len, mic) == _FAIL)
		goto BIP_exit;
...
BIP_exit:
	if (res == _SUCCESS) {
		...
		if (gmcs == _BIP_CMAC_SM4_128_)
		{
			_rtw_memcpy(ipn, ((struct wapi_mmie *)mme)->ipn, WAPI_PN_LEN);
		}
		else
#endif
		{
