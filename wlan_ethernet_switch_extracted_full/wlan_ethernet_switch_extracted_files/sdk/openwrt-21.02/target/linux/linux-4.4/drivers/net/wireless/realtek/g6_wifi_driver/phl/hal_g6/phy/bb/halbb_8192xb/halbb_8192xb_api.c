...
void halbb_edcca_per20_bitmap_sifs_8192xb(struct bb_info *bb,
					 enum channel_width bw,
					 enum phl_phy_idx phy_idx)
{
	if (bw == CHANNEL_WIDTH_20) {
		halbb_set_reg(bb, 0x0c9c, 0xFF000, 0xff);
		halbb_set_reg(bb, 0x0cA0, 0xFF000, 0);
	} else {
		halbb_set_reg(bb, 0x0c9c, 0xFF000, 0);
		halbb_set_reg(bb, 0x0cA0, 0xFF000, 0);
	}
}
...
void halbb_tx_triangular_shap_cfg_8192xb(struct bb_info *bb, u8 shape_idx,
	...
	/*switch Pwr compensation at txbb gain for pathB*/
	halbb_set_reg(bb, 0x76c4, BIT(4), 1);
	halbb_set_reg(bb, 0x766c, BIT(21), 1);
}

void halbb_set_digital_pwr_comp_8192xb(struct bb_info *bb, bool en, enum phl_phy_idx phy_idx)
{
	u16 i = 0;
	u16 pw_a_cr = 0;
	u16 pw_b_cr = 0;
	u32 table_len = sizeof(pwr_comp_1ss_32b)/(sizeof(u32) * 2);

	pw_a_cr = 0x5428;
	pw_b_cr = 0x7428;


	if (en && bb->ic_type == BB_RTL8192XB) {
		if (bb->bb_api_i.band == BAND_ON_5G) {
...
void halbb_bb_reset_all_8192xb(struct bb_info* bb, enum phl_phy_idx phy_idx)
{
	BB_DBG(bb, DBG_DBG_API, "%s\n", __func__);

	//Protest HW-SI
	halbb_set_reg(bb, 0x1200, BIT(28) | BIT(29) | BIT(30), 0x7);
	halbb_set_reg(bb, 0x3200, BIT(28) | BIT(29) | BIT(30), 0x7);
	halbb_delay_us(bb, 1);
	// === [BB reset] === //
	halbb_set_reg(bb, 0x704, BIT(1), 1);
	halbb_set_reg(bb, 0x704, BIT(1), 0);

	halbb_set_reg(bb, 0x1200, BIT(28) | BIT(29) | BIT(30), 0x0);
	halbb_set_reg(bb, 0x3200, BIT(28) | BIT(29) | BIT(30), 0x0);
	halbb_set_reg(bb, 0x704, BIT(1), 1);
}
...
u32 halbb_read_rf_reg_8192xb_a(struct bb_info *bb, enum rf_path path,
...
u32 halbb_read_rf_reg_8192xb_d(struct bb_info *bb, enum rf_path path,
			      u32 reg_addr, u32 bit_mask)
{
			    ...
			    u32 bit_mask)
{
	...
	if (ad_sel == DAV) {
		readback_value = halbb_read_rf_reg_8192xb_a(bb, path, reg_addr,
				 bit_mask);
		/*BB_DBG(bb, DBG_PHY_CONFIG, "A-die RF-%d 0x%x = 0x%x, bit mask = 0x%x\n",
		       path, reg_addr, readback_value, bit_mask);*/
	} else if (ad_sel == DDV) {
		readback_value = halbb_read_rf_reg_8192xb_d(bb, path, reg_addr,
				 bit_mask);
...
bool halbb_write_rf_reg_8192xb_a(struct bb_info *bb, enum rf_path path,
...
bool halbb_write_rf_reg_8192xb_d(struct bb_info *bb, enum rf_path path,
				u32 reg_addr, u32 bit_mask, u32 data)
{
	u32 direct_addr = 0;
...
bool halbb_write_rf_reg_8192xb(struct bb_info *bb, enum rf_path path,
			      u32 reg_addr, u32 bit_mask, u32 data)
{
	...
	if (ad_sel == DAV) {
		rpt = halbb_write_rf_reg_8192xb_a(bb, path, reg_addr, bit_mask,
		      data);
		/*BB_DBG(bb, DBG_PHY_CONFIG, "A-die RF-%d 0x%x = 0x%x , bit mask = 0x%x\n",
		       path, reg_addr, data, bit_mask);*/
	} else if (ad_sel == DDV) {
		rpt = halbb_write_rf_reg_8192xb_d(bb, path, reg_addr, bit_mask,
		      data);
		/*BB_DBG(bb, DBG_PHY_CONFIG, "D-die RF-%d 0x%x = 0x%x , bit mask = 0x%x\n",
		       path, reg_addr, data, bit_mask);*/
	} else {
...
void halbb_5m_mask_8192xb(struct bb_info *bb, u8 pri_ch, enum channel_width bw,
			   enum phl_phy_idx phy_idx)
{
	...
	if (!mask_5m_en) {
		...
		halbb_set_reg(bb, 0x4450, BIT(24), 0x0);
	} else {
		if (mask_5m_low) {
		...
		halbb_set_reg(bb, 0x4450, BIT(24), 0x1);
	}
}
...
#if 0
void halbb_set_btg_8192xb(struct bb_info *bb, bool s1_2g_band, bool dbcc_en)
{
	if (s1_2g_band) {
		// Path A
		...
		if (!dbcc_en) {
			...
			// [Phy0] Ignore Grant BT by PMAC Setting
			...
			/* [Phy0] To avoid abnormal 1R CCA without BT, set rtl only 0xc6c[21] = 0x1 */
			halbb_set_reg(bb, 0x441c, BIT(31), 0x0);
			halbb_set_reg(bb, 0xc6c, BIT(21), 0x0);
			// [Phy1] Apply Grant BT by TMAC Setting
			halbb_set_reg(bb, 0x2980, 0x1e0000, 0x0);
			BB_DBG(bb, DBG_PHY_CONFIG, "[BT] Apply Grant BT by TMAC Setting\n");
			// [Phy1] Add BT share
			halbb_set_reg(bb, 0x6978, BIT(14), 0x1);
			halbb_set_reg(bb, 0x6974, 0x3c00000, 0x2);
			/* [Phy1] To avoid abnormal 1R CCA without BT, set rtl only 0xc6c[21] = 0x1 */
			halbb_set_reg(bb, 0x641c, BIT(31), 0x1);
			halbb_set_reg(bb, 0x2c6c, BIT(21), 0x1);
		}
	} else {
		// Path A
		...
		if (dbcc_en) {
			// Ignore Grant BT by PMAC Setting
			halbb_set_reg(bb, 0x2980, 0x1e0000, 0xf);
			halbb_set_reg(bb, 0x2980, 0x3c000000, 0x4);
			BB_DBG(bb, DBG_PHY_CONFIG, "[BT] Ignore Grant BT by PMAC Setting\n");
			// Reset BT share
			halbb_set_reg(bb, 0x6978, BIT(14), 0x0);
			halbb_set_reg(bb, 0x6974, 0x3c00000, 0x0);
			/* To avoid abnormal 1R CCA without BT, set rtl only 0xc6c[21] = 0x1 */
			halbb_set_reg(bb, 0x641c, BIT(31), 0x0);
			halbb_set_reg(bb, 0x66c, BIT(21), 0x0);
		}
	}
}
...
void halbb_ctrl_btg_8192xb(struct bb_info *bb, bool btg)
{
	...
	if (!bb->hal_com->dbcc_en)
		s1_2g_band = ((bb->hal_com->band[0].cur_chandef.band == BAND_ON_24G) && ((bb->rx_path == RF_PATH_B) ||
			      (bb->rx_path == RF_PATH_AB))) ? true : false;
	else
		s1_2g_band = (halbb_get_reg(bb, 0x4970, BIT(1)) == 1) ? true : false;

	halbb_set_btg_8192xb(bb, s1_2g_band, bb->hal_com->dbcc_en);
#endif
}
#endif
			    ...
			    enum rf_path path)
{
	...
	ft_pass_efuse = bb->bb_efuse_i.efuse_ft & 0xff;
	ft_ctrl_efuse = bb->bb_efuse_i.efuse_adc_td & 0x1f;

	/*WB ADC clock = 40M for BW5; 80M for BW10; 160M for BW20/40/80 */
	...
	if (ft_pass_efuse == 0xff) {
		if (bw == CHANNEL_WIDTH_80) {
			/*op5_current*/
			halbb_set_reg(bb, adc_op5_bw_sel[path], 0x1E0, 0x5);
			halbb_set_reg(bb, rck_reset_count[path], 0xFFFF0000, 0xA);
			/*quantizer_sample_delay*/
			halbb_set_reg(bb, adc_sample_td[path], BIT(27) | BIT(26), 0x2);
			halbb_set_reg(bb, rck_offset[path], 0x3E0000, 0x9);
		} else {
			halbb_set_reg(bb, adc_sample_td[path], BIT(27) | BIT(26), 0x3);
			halbb_set_reg(bb, adc_op5_bw_sel[path], 0x1E0, 0xC);
			halbb_set_reg(bb, rck_offset[path], 0x3E0000, 0xC);
		}
		BB_DBG(bb, DBG_PHY_CONFIG, "[0xff][bw_setting] ADC TD setting = 0x3\n");
	} else if (ft_pass_efuse == 0xcc || ft_pass_efuse == 0xfc) {
		if (ft_ctrl_efuse == 0x19) { /*CBV*/
			/*idac2*/
			...
			/*RCK offset*/
			halbb_set_reg(bb, rck_offset[path], 0x3E0000, 0xF);
			/*rck_reset_count*/
			halbb_set_reg(bb, rck_reset_count[path], 0xFFFF0000, 0xA);
			/*adc_rst_cycle*/
			halbb_set_reg(bb, adc_rst_cycle[path], 0xFF0000, 0x3);
		} else if (ft_ctrl_efuse == 0x11) { /*CBV*/
			/*idac2*/
			halbb_set_reg(bb, idac2[path], 0x780, 0x8);
			halbb_set_reg(bb, idac2_1[path], 0x7800, 0x2);
			/*quantizer_sample_delay*/
			...
			halbb_set_reg(bb, adc_sample_td[path], BIT(27) | BIT(26), 0x2);
			/*op5_current*/
			halbb_set_reg(bb, adc_op5_bw_sel[path], 0x1E0, 0x3);
			/*RCK offset*/
			halbb_set_reg(bb, rck_offset[path], 0x3E0000, 0xF);
			/*rck_reset_count*/
			halbb_set_reg(bb, rck_reset_count[path], 0xFFFF0000, 0xA);
			...
			if (bw == CHANNEL_WIDTH_80) {
				...
				/*idac2*/
				halbb_set_reg(bb, idac2[path], 0x780, 0x5);
				halbb_set_reg(bb, idac2_1[path], 0x7800, 0x0);
				/*quantizer_sample_delay*/
				halbb_set_reg(bb, adc_sample_td[path], BIT(27) | BIT(26), 0x3);
				/*op5_current*/
				halbb_set_reg(bb, adc_op5_bw_sel[path], 0x1E0, 0xE);
				/*RCK offset*/
				halbb_set_reg(bb, rck_offset[path], 0x3E0000, 0xF);
				/*rck_reset_count*/
				halbb_set_reg(bb, rck_reset_count[path], 0xFFFF0000, 0x9);
			...
			if (bw == CHANNEL_WIDTH_80) {
				...
				halbb_set_reg(bb, adc_sample_td[path], BIT(27) | BIT(26), 0x3);
				/*op5_current*/
				halbb_set_reg(bb, adc_op5_bw_sel[path], 0x1E0, 0xD);
				/*RCK offset*/
				halbb_set_reg(bb, rck_offset[path], 0x3E0000, 0xF);
				/*rck_reset_count*/
				halbb_set_reg(bb, rck_reset_count[path], 0xFFFF0000, 0x9);
		...
		BB_DBG(bb, DBG_PHY_CONFIG,
		       "[0x%x][bw_setting] ft_ctrl_efuse = 0x%x\n", ft_pass_efuse, ft_ctrl_efuse);
	} else {
		if (bw == CHANNEL_WIDTH_80) {
			/*op5_current*/
			halbb_set_reg(bb, adc_op5_bw_sel[path], 0x1E0, 0x5);
	...
	if (bb->hal_com->cv == CAV) {
		/*CAV rx_adc_clk 480M for BW10/20/40*/
		halbb_set_reg(bb, rx_adc_clk[path], 0xff800000, 0x49);
		/*decim_filter*/
		if (bw == CHANNEL_WIDTH_5)
			halbb_set_reg(bb, decim_filter[path], 0x6000, 0x1);
		else if (bw == CHANNEL_WIDTH_10)
			halbb_set_reg(bb, decim_filter[path], 0x6000, 0x2);
		else
			halbb_set_reg(bb, decim_filter[path], 0x6000, 0);
	} else if (bb->hal_com->cv == CBV) {
		/*CBV rx_adc_clk 960M for BW10/20/40*/
		halbb_set_reg(bb, rx_adc_clk[path], 0xff800000, 0x92);
		/*CAV timing issue, remove DAC S0 clk inv patch for CBV*/
		halbb_set_reg(bb, 0xc000, BIT(18), 0);
		/*decim_filter*/
		if (bw == CHANNEL_WIDTH_5 || bw == CHANNEL_WIDTH_10)
			halbb_set_reg(bb, decim_filter[path], 0x6000, 0x1);
		else if (bw == CHANNEL_WIDTH_20 || bw == CHANNEL_WIDTH_40)
			halbb_set_reg(bb, decim_filter[path], 0x6000, 0x2);
		else
			halbb_set_reg(bb, decim_filter[path], 0x6000, 0);
	} else {
		BB_WARNING("Not support cv = %d \n", bb->hal_com->cv);
	}
	
	BB_DBG(bb, DBG_PHY_CONFIG, "[Success][bw_setting] ADC setting for Path-%d\n", path);
	BB_DBG(bb, DBG_PHY_CONFIG, "[FT 0x%x][bw %d] ft_ctrl_efuse = 0x%x\n", ft_pass_efuse, bw, ft_ctrl_efuse);
	BB_DBG(bb, DBG_PHY_CONFIG, "idac2   = 0x%x\n", halbb_get_reg(bb, idac2[path], 0x780));
	BB_DBG(bb, DBG_PHY_CONFIG, "idac2_1 = 0x%x\n", halbb_get_reg(bb, idac2_1[path], 0x7800));
	BB_DBG(bb, DBG_PHY_CONFIG, "td      = 0x%x\n", halbb_get_reg(bb, adc_sample_td[path], BIT(27)|BIT(26)));
	BB_DBG(bb, DBG_PHY_CONFIG, "op5     = 0x%x\n", halbb_get_reg(bb, adc_op5_bw_sel[path], 0x1E0));
	BB_DBG(bb, DBG_PHY_CONFIG, "rck+    = 0x%x\n", halbb_get_reg(bb, rck_offset[path], 0x3E0000));
	BB_DBG(bb, DBG_PHY_CONFIG, "rck cnt = 0x%x\n", halbb_get_reg(bb, rck_reset_count[path], 0xFFFF0000));
	BB_DBG(bb, DBG_PHY_CONFIG, "adc clk = 0x%x\n", halbb_get_reg(bb, rx_adc_clk[path], 0xff800000));
	BB_DBG(bb, DBG_PHY_CONFIG, "decim   = 0x%x\n", halbb_get_reg(bb, decim_filter[path], 0x6000));
	return true;
}
			 ...
			 enum phl_phy_idx phy_idx)
{
	...
	case CHANNEL_WIDTH_20:
		if (bw == CHANNEL_WIDTH_5) {
			/*RF_BW:[31:30]=0x0 */
			halbb_set_reg(bb, 0x4974, 0xC0000000, 0x0);
			/*small BW:[13:12]=0x1 */
			halbb_set_reg(bb, 0x4978, 0x3000, 0x1);
			/*Pri ch:[11:8]=0x0 */
			halbb_set_reg(bb, 0x4978, 0xf00, 0x0);
		} else if (bw == CHANNEL_WIDTH_10) {
			/*RF_BW:[31:30]=0x0 */
			halbb_set_reg(bb, 0x4974, 0xC0000000, 0x0);
			/*small BW:[13:12]=0x2 */
			halbb_set_reg(bb, 0x4978, 0x3000, 0x2);
			/*Pri ch:[11:8]=0x0 */
			halbb_set_reg(bb, 0x4978, 0xf00, 0x0);
			...
			halbb_set_reg(bb, 0x4974, 0xC0000000, 0x0);
			/*small BW:[13:12]=0x0 */
			halbb_set_reg(bb, 0x4978, 0x3000, 0x0);
			/*Pri ch:[11:8]=0x0 */
			halbb_set_reg(bb, 0x4978, 0xf00, 0x0);
		}

		/*==== r_fir_type_init : DFIR-0====*/
		halbb_set_reg(bb, 0x4c00, BIT(4), 0);
		halbb_set_reg(bb, 0x4cc4, BIT(4), 0);

		break;
	case CHANNEL_WIDTH_40:
		/*RF_BW:[31:30]=0x1 */
		halbb_set_reg(bb, 0x4974, 0xC0000000, 0x1);
		/*small BW:[13:12]=0x0 */
		halbb_set_reg(bb, 0x4978, 0x3000, 0x0);
		/*Pri ch:[11:8] */
		halbb_set_reg(bb, 0x4978, 0xf00, pri_ch);

		/*==== r_fir_type_init : DFIR-1====*/
		halbb_set_reg(bb, 0x4c00, BIT(4), 1);
		halbb_set_reg(bb, 0x4cc4, BIT(4), 1);

		break;
	case CHANNEL_WIDTH_80:
		/*RF_BW:[31:30]=0x2 */
		halbb_set_reg(bb, 0x4974, 0xC0000000, 0x2);
		/*small BW:[13:12]=0x0 */
		halbb_set_reg(bb, 0x4978, 0x3000, 0x0);
		/*Pri ch:[11:8] */
		halbb_set_reg(bb, 0x4978, 0xf00, pri_ch);
	...
	if (bw == CHANNEL_WIDTH_40) {
		halbb_set_reg(bb, 0x4e30, BIT(26), 1);
		halbb_set_reg(bb, 0x4424, BIT(2), 1);
	} else {
		halbb_set_reg(bb, 0x4e30, BIT(26), 0);
		halbb_set_reg(bb, 0x4424, BIT(2), 0);
	}

	/*==== r_tx_crest_factor_en setting ====*/
	if (bw == CHANNEL_WIDTH_80)
		halbb_set_reg(bb, 0x4ab0, BIT(0), 0);
	else
		halbb_set_reg(bb, 0x4ab0, BIT(0), 1);

	/*============== [Path A] ==============*/
	halbb_bw_setting_8192xb(bb, bw, RF_PATH_A);
	/*============== [Path B] ==============*/
	halbb_bw_setting_8192xb(bb, bw, RF_PATH_B);

	halbb_edcca_per20_bitmap_sifs_8192xb(bb, bw, phy_idx);
			   ...
			   enum phl_phy_idx phy_idx)
{
	...
	if (band != BAND_ON_6G) {
		...
		/* === Set Efuse === */
		halbb_set_hidden_efuse_8192xb(bb, central_ch, band, RF_PATH_A);
		halbb_set_normal_efuse_8192xb(bb, central_ch, band, RF_PATH_A);
		halbb_set_hidden_efuse_8192xb(bb, central_ch, band, RF_PATH_B);
		halbb_set_normal_efuse_8192xb(bb, central_ch, band, RF_PATH_B);
		/* === Set RXSC RPL Comp === */
		//halbb_set_rxsc_rpl_comp_8852b(bb, central_ch);
	}
	/* === Set Ch idx report in phy-sts === */
	halbb_set_reg(bb, 0x0734, 0x0ff0000, central_ch);
			     ...
			     enum phl_phy_idx phy_idx)
{
	if (cck_en) {
		...
		halbb_set_reg(bb, 0x2320, BIT(0), 0);
		halbb_set_reg(bb, 0xc80, BIT(31), 0);
	} else {
		//halbb_set_reg(bb, 0x2300, BIT(27), 1);
		halbb_set_reg(bb, 0x700, BIT(5), 0);
		halbb_set_reg(bb, 0x2320, BIT(0), 1);
		halbb_set_reg(bb, 0xc80, BIT(31), 1);
	}
			      ...
			      enum phl_phy_idx phy_idx)
{
	...
	if (is_2g_ch) {
		...
		/*==== [CCK SCO Compesate] ====*/
		rpt &= halbb_ctrl_sco_cck_8192xb(bb, pri_ch);
	...
	/*==== [Switch CH] ====*/
	rpt &= halbb_ctrl_ch_8192xb(bb, central_ch, band, phy_idx);
	/*==== [Switch BW] ====*/
	rpt &= halbb_ctrl_bw_8192xb(bb, pri_ch_idx, bw, phy_idx);
	/*==== [CCK Enable / Disable] ====*/
	halbb_ctrl_cck_en_8192xb(bb, cck_en, phy_idx);

#if 0
	if (is_2g_ch && ((bb->rx_path == RF_PATH_B) || (bb->rx_path == RF_PATH_AB)))
		halbb_ctrl_btg_8192xb(bb, true);
	else
		halbb_ctrl_btg_8192xb(bb, false);

#ifdef SUPPORT_BTG
	if (!bb->hal_com->dbcc_en)
		s1_2g_band = (is_2g_ch && ((bb->rx_path == RF_PATH_B) ||
			      (bb->rx_path == RF_PATH_AB))) ? true : false;
	else
		s1_2g_band = (halbb_get_reg(bb, 0x4970, BIT(1)) == 1) ? true : false;
	...
	/*==== [Spur elimination] ====*/
	if (band == BAND_ON_5G && central_ch == 153) {
		halbb_set_reg(bb, 0x4c14, 0xfff, 0x210);
		halbb_set_reg(bb, 0x4cd8, 0xfff, 0x210);
	...
	} else if (band == BAND_ON_5G && central_ch == 151) {
		halbb_set_reg(bb, 0x4c14, 0xfff, 0x210);
		halbb_set_reg(bb, 0x4cd8, 0xfff, 0x210);
		...
		halbb_set_reg(bb, 0x4cd8, BIT(12), 0x1);
		halbb_set_reg(bb, 0x42c4, BIT(23), 0x1);
	} else if (band == BAND_ON_5G && central_ch == 155) {
		halbb_set_reg(bb, 0x4c14, 0xfff, 0x2d0);
		halbb_set_reg(bb, 0x4cd8, 0xfff, 0x2d0);
		halbb_set_reg(bb, 0x42ac, 0xfff, 0x740);
		halbb_set_reg(bb, 0x4c14, BIT(12), 0x1);
		halbb_set_reg(bb, 0x4cd8, BIT(12), 0x1);
		halbb_set_reg(bb, 0x42c4, BIT(23), 0x1);
	} else {
		halbb_set_reg(bb, 0x4c14, BIT(12), 0x0);
		halbb_set_reg(bb, 0x4cd8, BIT(12), 0x0);
		halbb_set_reg(bb, 0x42c4, BIT(23), 0x0);
	}

	/* Dynamic 5M Mask Setting */
	halbb_5m_mask_8192xb(bb, pri_ch, bw, phy_idx);
	...
	/*==== [BB reset] ====*/
	halbb_bb_reset_all_8192xb(bb, phy_idx);
...
{
	u32 ofdm_rx = 0x0;
	s32 cck_ofst = 0, cck_ofst2 = 0, cck_ofst_ab = 0;
	s32 tmp1 = 0, tmp2 = 0, tmp3 = 0;
	bool s1_2g_band = false;
	...
	/*==== [PHY0] Rx HE n_user_max ====*/
	halbb_set_reg(bb, 0xd80, 0x3fc0, HE_N_USER_MAX_8192XB);
#if 0
	/* === [BTG setting] === */
...
#ifdef SUPPORT_BTG
	if (!bb->hal_com->dbcc_en)
		s1_2g_band = ((bb->hal_com->band[0].cur_chandef.band == BAND_ON_24G) && ((rx_path == RF_PATH_B) ||
			      (rx_path == RF_PATH_AB))) ? true : false;
	else
		s1_2g_band = (halbb_get_reg(bb, 0x4970, BIT(1)) == 1) ? true : false;
	...
	/*==== [CCK RPL offset] ====*/
	if (rx_path == RF_PATH_A || rx_path == RF_PATH_B) {
		cck_ofst = bb->bb_efuse_i.gain_offset[rx_path][BB_BAND_2G];
		cck_ofst *= (-1);
		tmp1 = bb->bb_cmn_backup_i.cck_ps_th_bk + cck_ofst;
		tmp2 = bb->bb_cmn_backup_i.cck_rssi_ofst_bk + cck_ofst;
		tmp3 = bb->bb_cmn_backup_i.cck_sbd_th_bk + cck_ofst;
		BB_DBG(bb, DBG_PHY_CONFIG, "[CCK RPL] gain_offset = 0x%x, cck_ofst = 0x%x\n", bb->bb_efuse_i.gain_offset[rx_path][BB_BAND_2G], cck_ofst);
	} else if (rx_path == RF_PATH_AB) {
		cck_ofst = bb->bb_efuse_i.gain_offset[RF_PATH_A][BB_BAND_2G];
		cck_ofst2 = bb->bb_efuse_i.gain_offset[RF_PATH_B][BB_BAND_2G];
		cck_ofst_ab = (cck_ofst + cck_ofst2) >> 1;
		cck_ofst_ab *= (-1);
		BB_DBG(bb, DBG_PHY_CONFIG, "[CCK RPL] cck_ofst = 0x%x, cck_ofst2 = 0x%x, cck_ofst_ab = 0x%x\n", cck_ofst, cck_ofst2, cck_ofst_ab);
		tmp1 = bb->bb_cmn_backup_i.cck_ps_th_bk + cck_ofst_ab;
		tmp2 = bb->bb_cmn_backup_i.cck_rssi_ofst_bk + cck_ofst_ab;
		tmp3 = bb->bb_cmn_backup_i.cck_sbd_th_bk + cck_ofst_ab;
	}
	BB_DBG(bb, DBG_PHY_CONFIG, "[CCK RPL] tmp1 = 0x%x, tmp2 = 0x%x, tmp3 = 0x%x\n", tmp1, tmp2, tmp3);
	halbb_set_reg(bb, 0x4b48, MASKBYTE0, tmp1 & 0xff);
	halbb_set_reg(bb, 0x4b48, MASKBYTE2, tmp2 & 0xff);
	halbb_set_reg(bb, 0x4ba4, MASKBYTE0, tmp3 & 0xff);
...
void halbb_ctrl_tx_path_tmac_8192xb(struct bb_info *bb, enum rf_path tx_path, u8 tx_nss)
{
	...
	if (tx_path == RF_PATH_A) {
		...
		// 0xD800
		path_com_cr_array[1] = 0x11111100;

		// 0xD804
		path_com_cr_array[3] = 0x22222222;

		// 0xD808
		path_com_cr_array[5] = 0x01209222;

		// 0xD81C
		path_com_cr_array[15] = 0x39383939;

		// 0xD820
		path_com_cr_array[17] = 0x00003938;

	} else if (tx_path == RF_PATH_AB) {
		// 0xD800
		path_com_cr_array[1] = 0x19999980;

		// 0xD804
		path_com_cr_array[3] = 0x33333333;

		// 0xD808
		path_com_cr_array[5] = 0x01209333;
...
void halbb_gpio_setting_all_8192xb(struct bb_info *bb, u8 rfe_idx)
{
	...
	if (rfe_idx == 51) {
		halbb_gpio_setting_8192xb(bb, 11, BB_PATH_A, false, PAPE_RFM);
		halbb_gpio_setting_8192xb(bb, 7, BB_PATH_A, false, TRSW_RFM);
		halbb_gpio_setting_8192xb(bb, 6, BB_PATH_A, false, LNAON_RFM);

		halbb_gpio_setting_8192xb(bb, 1, BB_PATH_B, false, PAPE_RFM);
		halbb_gpio_setting_8192xb(bb, 2, BB_PATH_B, false, TRSW_RFM);
		halbb_gpio_setting_8192xb(bb, 3, BB_PATH_B, false, LNAON_RFM);

	} else if (rfe_idx == 53) {
		halbb_gpio_setting_8192xb(bb, 11, BB_PATH_A, false, PAPE_RFM);
		halbb_gpio_setting_8192xb(bb, 7, BB_PATH_A, true, TRSW_RFM);
		halbb_gpio_setting_8192xb(bb, 6, BB_PATH_A, true, LNAON_RFM);

		halbb_gpio_setting_8192xb(bb, 1, BB_PATH_B, false, PAPE_RFM);
		halbb_gpio_setting_8192xb(bb, 2, BB_PATH_B, true, TRSW_RFM);
		halbb_gpio_setting_8192xb(bb, 3, BB_PATH_B, true, LNAON_RFM);
...
void halbb_gpio_setting_init_8192xb(struct bb_info *bb)
{
	...
	for (i = 0; i < 2; i++) {
		...
		halbb_set_reg(bb, (path_cr_base | 0x94), BIT(25), 1); /* PAPE from rfm_PAPE instead of PAPE_path*/
	}
	...
	/*[TRSW Table]*/
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_A, 0, 0, 0, 0, 1);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_A, 0, 0, 1, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_A, 0, 1, 0, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_A, 0, 1, 1, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_A, 1, 0, 0, 0, 1);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_A, 1, 0, 1, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_A, 1, 1, 0, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_A, 1, 1, 1, 1, 0);

	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_B, 0, 0, 0, 0, 1);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_B, 0, 0, 1, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_B, 0, 1, 0, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_B, 0, 1, 1, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_B, 1, 0, 0, 0, 1);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_B, 1, 0, 1, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_B, 1, 1, 0, 1, 0);
	halbb_gpio_trsw_table_8192xb(bb, BB_PATH_B, 1, 1, 1, 1, 0);

	if (rfe_idx == 51 || rfe_idx == 52) {
		halbb_gpio_rfm_8192xb(bb, BB_PATH_A, PAPE_RFM, 0, 0, 0, 0x4);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_A, LNAON_RFM, 0, 0, 0, 0x8);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_B, PAPE_RFM, 0, 0, 0, 0x4);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_B, LNAON_RFM, 0, 0, 0, 0x8);
	} else if (rfe_idx == 53 || rfe_idx == 54) {
		/* LNA/TRSW inv=true, standby mode of rfm_LNAON/rfm_TRSW = 0x1*/
		halbb_gpio_rfm_8192xb(bb, BB_PATH_A, PAPE_RFM, 0, 0, 0, 0x4);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_A, TRSW_RFM, 0, 0, 0, 0x6);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_A, LNAON_RFM, 0, 0, 0, 0xA);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_B, PAPE_RFM, 0, 0, 0, 0x4);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_B, TRSW_RFM, 0, 0, 0, 0x6);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_B, LNAON_RFM, 0, 0, 0, 0xA);
	} else if (rfe_idx == 55 || rfe_idx == 56) {
		/* LNA inv=false, standby mode of rfm_LNAON = 0x0*/
		halbb_gpio_rfm_8192xb(bb, BB_PATH_A, PAPE_RFM, 0, 0, 0, 0x14);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_A, LNAON_RFM, 0, 0, 0, 0x18);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_B, PAPE_RFM, 0, 0, 0, 0x14);
		halbb_gpio_rfm_8192xb(bb, BB_PATH_B, LNAON_RFM, 0, 0, 0, 0x18);
	}
}
...
u16 halbb_cfg_cmac_tx_ant_8192xb(struct bb_info *bb, enum rf_path tx_path)
{
			       ...
			       u8 tx_nss, enum rf_path rx_path, u8 rx_nss)
{
	...
	// Rx Config
	halbb_ctrl_rx_path_8192xb(bb, rx_path);
	...
	halbb_ctrl_tx_path_tmac_8192xb(bb, tx_path, tx_nss);
}
#if 0
				...
				enum phl_phy_idx phy_idx, bool tmac_en)
{
	...
	// ==== Rx Config ==== //
	halbb_cfg_rx_path_8192xb(bb, rx_path, rx_nss, phy_idx);
	// ==== Tx Config ==== //
	if (tmac_en)
		halbb_set_reg_cmn(bb, 0x09a4, 0x1c, 0x0, phy_idx);
	else
		halbb_cfg_tx_path_8192xb(bb, tx_path, tx_nss, phy_idx);
}
#endif
...
void halbb_cfg_dbcc_8192xb(struct bb_info *bb, bool dbcc_en)
{
	if (dbcc_en) {
		...
		// === DBCC En === //
		halbb_ctrl_dbcc_8192xb(bb, 1);
		// === Rx settings === //
		halbb_ctrl_rx_path_8192xb(bb, RF_PATH_AB);
		// === BB rst === //
		halbb_set_reg(bb, 0x704, BIT(1), 1);
...
void halbb_ctrl_rx_cca_8192xb(struct bb_info *bb, bool cca_en, enum phl_phy_idx phy_idx)
{
	if (cca_en) {
		...
		// BB reset
		halbb_bb_reset_en_8192xb(bb, false, phy_idx);
		halbb_bb_reset_en_8192xb(bb, true, phy_idx);
		// TSSI pause disable
			       ...
			       enum phl_phy_idx phy_idx)
{
	...
	power_dbm &= 0x1ff;
	halbb_set_reg(bb, 0x09a4, BIT(16), 1);
	halbb_set_reg(bb, 0x4594, 0x7fc00000, power_dbm);
	BB_DBG(bb, DBG_PHY_CONFIG,
...
s16 halbb_get_txpwr_dbm_8192xb(struct bb_info *bb, enum phl_phy_idx phy_idx)
{
	...
	txpwr_dbm = halbb_get_reg(bb, 0x4594, 0x7fc00000);
...
void halbb_backup_info_8192xb(struct bb_info *bb, enum phl_phy_idx phy_idx)
{
	...
	bb->bb_cmn_backup_i.cur_tx_path = (u8)halbb_get_reg(bb, 0x458c, 0xf0000000);
	bb->bb_cmn_backup_i.cur_rx_path = (u8)halbb_get_reg(bb, 0x4978, 0xf);
	bb->bb_cmn_backup_i.cur_tx_pwr = halbb_get_txpwr_dbm_8192xb(bb, phy_idx);
...
void halbb_restore_info_8192xb(struct bb_info *bb, enum phl_phy_idx phy_idx)
{
	...
	halbb_set_txpwr_dbm_8192xb(bb, bb->bb_cmn_backup_i.cur_tx_pwr, phy_idx);

	tx_nss = (bb->bb_cmn_backup_i.cur_tx_path == 0x3) ? 2 : 1;
	if (phl_is_mp_mode(bb->phl_com))
		halbb_ctrl_tx_path_8192xb(bb, tx_path);
	else
		halbb_ctrl_tx_path_tmac_8192xb(bb, tx_path, tx_nss);
	halbb_ctrl_rx_path_8192xb(bb, rx_path);
...
void halbb_lbk_comm_8192xb(struct bb_info *bb, bool lbk_en, bool is_dgt_lbk,
			   enum rf_path tx_path)
{
	if (!lbk_en) {
			  ...
			  enum channel_width bw, enum phl_phy_idx phy_idx)
{
	...
	halbb_lbk_comm_8192xb(bb, lbk_en, is_dgt_lbk, tx_path);
...
bool halbb_set_txsc_8192xb(struct bb_info *bb, u8 txsc, enum phl_phy_idx phy_idx)
{
	...
	halbb_set_reg(bb, 0x45ac, 0x7800000, txsc);
	BB_DBG(bb, DBG_PHY_CONFIG,
			       ...
			       enum phl_phy_idx phy_idx)
{
	...
	halbb_set_reg(bb, 0x43ac, BIT(28), 0x1);
	halbb_set_reg(bb, 0x43ac, 0xfc00000, bss_color);
	BB_DBG(bb, DBG_PHY_CONFIG,
			    ...
			    enum phl_phy_idx phy_idx)
{
	...
	//=== [Set Station ID] ===//
	halbb_set_reg(bb, 0x43ac, 0x3ff800, sta_id);
	BB_DBG(bb, DBG_PHY_CONFIG,
			       ...
			       u32 *_out_len, bool dump_2_buff)
{
	...
	if (bb->hal_com->dbcc_en) {
		BB_TRACE("BBCR: [WARNING] Will simultaneously dump phy-0/1 reg. ==>\n");
		BB_TRACE("BBCR: PHY-0 ==>\n");
	}
					   ...
					   enum phl_phy_idx phy_idx)
{
	...
	halbb_set_reg(bb, 0x0738, 0xC, val32);
}
...
void halbb_pre_agc_en_8192xb(struct bb_info *bb, bool enable)
{
	...
	BB_DBG(bb, DBG_DBG_API, "PreAGC en: 0x4730[31]=(0x%x)\n", en);
}

s8 halbb_efuse_exchange_8192xb(struct bb_info *bb, u8 value,
...
u8 halbb_ofdm_band_determine_8192xb(struct bb_info *bb, u8 central_ch, enum band_type band_type, bool is_normal)
{
	u8 ofdm_band = 0, ofst = 1;

	if (band_type == BAND_ON_24G) {
		// 2G Band(hidden,normal): (0,1)
		ofdm_band = BB_BAND_2G;
	} else {
		// 5G Band(hidden,normal): (1,2):Low, (2,3): Mid, (3,4):High
		if (central_ch >= 36 && central_ch <= 64)
			ofdm_band = BB_BAND_5G_L;
		else if (central_ch >= 100 && central_ch <= 144)
			ofdm_band = BB_BAND_5G_M;
		else if (central_ch >= 149 && central_ch <= 177)
			ofdm_band = BB_BAND_5G_H;
		else
			ofdm_band = BB_BAND_5G_L;
	}
...
void halbb_ext_loss_avg_update_8192xb(struct bb_info *bb)
{
	...
	#if 0 /*edcca pwrofst compensation is supported from halbb 029*/
...
void halbb_get_normal_efuse_init_8192xb(struct bb_info *bb)
{
	...
	gain->gain_offset[RF_PATH_A][0] = halbb_efuse_exchange_8192xb(bb, tmp, HIGH_MASK);
	gain->gain_offset[RF_PATH_B][0] = halbb_efuse_exchange_8192xb(bb, tmp, LOW_MASK);

	halbb_efuse_get_info(bb, EFUSE_INFO_RF_RX_GAIN_K_A_2G_OFMD, &tmp, 1);
	gain->gain_offset[RF_PATH_A][1] = halbb_efuse_exchange_8192xb(bb, tmp, HIGH_MASK);
	gain->gain_offset[RF_PATH_B][1] = halbb_efuse_exchange_8192xb(bb, tmp, LOW_MASK);

	halbb_efuse_get_info(bb, EFUSE_INFO_RF_RX_GAIN_K_A_5GL, &tmp, 1);
	gain->gain_offset[RF_PATH_A][2] = halbb_efuse_exchange_8192xb(bb, tmp, HIGH_MASK);
	gain->gain_offset[RF_PATH_B][2] = halbb_efuse_exchange_8192xb(bb, tmp, LOW_MASK);

	halbb_efuse_get_info(bb, EFUSE_INFO_RF_RX_GAIN_K_A_5GM, &tmp, 1);
	gain->gain_offset[RF_PATH_A][3] = halbb_efuse_exchange_8192xb(bb, tmp, HIGH_MASK);
	gain->gain_offset[RF_PATH_B][3] = halbb_efuse_exchange_8192xb(bb, tmp, LOW_MASK);

	halbb_efuse_get_info(bb, EFUSE_INFO_RF_RX_GAIN_K_A_5GH, &tmp, 1);
	gain->gain_offset[RF_PATH_A][4] = halbb_efuse_exchange_8192xb(bb, tmp, HIGH_MASK);
	gain->gain_offset[RF_PATH_B][4] = halbb_efuse_exchange_8192xb(bb, tmp, LOW_MASK);
...
void halbb_get_hidden_efuse_init_8192xb(struct bb_info *bb)
{
	...
	u32 hidden_efuse_addr[BB_PATH_MAX_8192XB][4] = {{GAIN_HIDE_EFUSE_A_2G_8192XB,
							GAIN_HIDE_EFUSE_A_5GL_8192XB,
							GAIN_HIDE_EFUSE_A_5GM_8192XB,
							GAIN_HIDE_EFUSE_A_5GH_8192XB},
							{GAIN_HIDE_EFUSE_B_2G_8192XB,
							GAIN_HIDE_EFUSE_B_5GL_8192XB,
							GAIN_HIDE_EFUSE_B_5GM_8192XB,
							GAIN_HIDE_EFUSE_B_5GH_8192XB}};

	BB_DBG(bb, DBG_PHY_CONFIG, "<====== %s ======>\n", __func__);

	halbb_phy_efuse_get_info(bb, GAIN_HIDE_EFUSE_START_ADDR_8192XB, BB_HIDE_EFUSE_SIZE, bb->bb_efuse_i.hidden_efuse);

	for (path = RF_PATH_A; path < BB_PATH_MAX_8192XB; path++) {
		for (i = 0; i < BB_BAND_NUM_MAX_8192XB; i++) {
			gain->gain_cs[path][i] = halbb_efuse_exchange_8192xb(bb, bb->bb_efuse_i.hidden_efuse[hidden_efuse_addr[path][i] - GAIN_HIDE_EFUSE_START_ADDR_8192XB], HIGH_MASK);
			gain->gain_cg[path][i] = halbb_efuse_exchange_8192xb(bb, bb->bb_efuse_i.hidden_efuse[hidden_efuse_addr[path][i] - GAIN_HIDE_EFUSE_START_ADDR_8192XB], LOW_MASK);
	...
	BB_DBG(bb, DBG_INIT, "[Efuse]check_tmp = %d\n", check_tmp);
	BB_DBG(bb, DBG_INIT, "[Efuse]HALBB_MAX_PATH * BB_BAND_NUM_MAX_8192XB = %d\n", HALBB_MAX_PATH * BB_BAND_NUM_MAX_8192XB);

	if (check_tmp == HALBB_MAX_PATH * BB_BAND_NUM_MAX_8192XB)
...
void halbb_set_hidden_efuse_8192xb(struct bb_info *bb, u8 central_ch, enum band_type band_type, enum rf_path path)
{
	...
	// CCK band = OFDM band (0) = BB_BAND_2G
	band = halbb_ofdm_band_determine_8192xb(bb, central_ch, band_type, false);
...
void halbb_set_normal_efuse_8192xb(struct bb_info *bb, u8 central_ch, enum band_type band_type, enum rf_path path)
{
	...
	ofdm_band = halbb_ofdm_band_determine_8192xb(bb, central_ch, band_type, true);
	...
	if (bb->bb_efuse_i.normal_efuse_check) {
		...
		if (band_type == BAND_ON_24G) {
			normal_efuse_cck = bb->bb_efuse_i.gain_offset[path][BB_BAND_2G];
		...
		// OFDM normal efuse
		normal_efuse = bb->bb_efuse_i.gain_offset[path][ofdm_band];
	...
	halbb_ext_loss_avg_update_8192xb(bb);
}
...
void halbb_set_gain_error_8192xb(struct bb_info *bb, u8 central_ch, enum band_type band_type)
{
	...
	u8 band;
	u8 path = 0, lna_idx = 0, tia_idx = 0, i = 0;
	s32 tmp = 0;
	u32 wb_gidx_tmp = 0;
	u32 lna_gain_g[BB_PATH_MAX_8192XB][7] = {{0x4678, 0x4678, 0x467C,
						   0x467C, 0x467C, 0x467C,
						   0x4680}, {0x475C, 0x475C,
						   0x4760, 0x4760, 0x4760,
						   0x4760, 0x4764}};
	u32 lna_gain_a[BB_PATH_MAX_8192XB][7] = {{0x45DC, 0x45DC, 0x4660,
	...
	u32 lna_gain_mask[7] = {0x00ff0000, 0xff000000, 0x000000ff,
				    ...
				    0x000000ff};
	u32 lna_gain_a_bypass[BB_PATH_MAX_8192XB][7] = {{0x4BB4, 0x4BB4, 0x4BB4,
	...
	u32 lna_gain_a_bypass_mask[7] = {0xff, 0xff00, 0xff0000, 0xff000000, 0xff,
				       0xff00, 0xff0000};
	u32 lna_gain_g_bypass[BB_PATH_MAX_8192XB][7] = {{0x4BB8, 0x4BBC, 0x4BBC,
	...
	u32 lna_gain_g_bypass_mask[7] = {0xff000000, 0xff, 0xff00, 0xff0000, 0xff000000,
				       0xff, 0xff00};
	u32 tia_gain_g[BB_PATH_MAX_8192XB][2] = {{0x4680, 0x4680}, {0x4764,
						   0x4764}};
	u32 tia_gain_a[BB_PATH_MAX_8192XB][2] = {{0x4664, 0x4664}, {0x4748,
						   0x4748}};
	u32 tia_gain_mask[2] = {0x00ff0000, 0xff000000};
	u32 wb_gidx_elna[BB_PATH_MAX_8192XB][2] = {{0x4734, 0x4738}, {0x4AA0, 0x4AA4}};
	u32 wb_gidx_lna_tia[BB_PATH_MAX_8192XB][2] = {{0x4724, 0x4728}, {0x4A78, 0x4A7C}};
	u32 gs_idx[BB_PATH_MAX_8192XB][2] = {{0x46B4, 0x46B8}, {0x4A68, 0x4A6C}};
	u32 g_elna[BB_PATH_MAX_8192XB] = {0x4678, 0x475C};
	...
	for (path = RF_PATH_A; path < BB_PATH_MAX_8192XB; path++) {
		...
		if (bb->phl_com->dev_cap.rfe_type >= 51) {
			// wb_gidx_elna
			wb_gidx_tmp = gain->wb_gidx_elna[band][path] & 0x1ff;
			halbb_set_reg(bb, wb_gidx_elna[path][0], 0xff800000, wb_gidx_tmp);
			wb_gidx_tmp = (gain->wb_gidx_elna[band][path] & 0xfe00) >> 9;
			halbb_set_reg(bb, wb_gidx_elna[path][1], 0x7f, wb_gidx_tmp);
			// wb_gidx_lna_tia
			wb_gidx_tmp = 0;
			for (i = 0; i < 7; i++)
				wb_gidx_tmp |= (gain->wb_gidx_lna_tia[band][path][i] << (3 * i));
			halbb_set_reg(bb, wb_gidx_lna_tia[path][0], 0x3FFFFE00, wb_gidx_tmp);

			wb_gidx_tmp = 0;
			for (i = 0; i < 9; i++)
				wb_gidx_tmp |= (gain->wb_gidx_lna_tia[band][path][i + 7] << (3 * i));
			halbb_set_reg(bb, wb_gidx_lna_tia[path][1], 0x07FFFFFF, wb_gidx_tmp);
			// gs_idx
			halbb_set_reg(bb, gs_idx[path][0], MASKDWORD, gain->gs_idx[band][path][0]);
			halbb_set_reg(bb, gs_idx[path][1], 0xFFF, gain->gs_idx[band][path][1]);
			// g_elna
...
void halbb_set_rxsc_rpl_comp_8192xb(struct bb_info* bb, u8 central_ch)
{
	struct bb_gain_info* gain = &bb->bb_gain_i;
	u8 band;
	u8 path = 0;
