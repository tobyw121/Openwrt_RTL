...
s32 init_mp_priv(_adapter *padapter)
{
	...
	pmppriv->rtw_mp_tx_method = MP_PMACT_TX;
...
{
	//halrf_iqk_trigger_g6(adapter_to_phydm(padapter), bReCovery);
}

static void PHY_LCCalibrate(_adapter *padapter)
{
	//halrf_lck_trigger_g6(adapter_to_phydm(padapter));
}
...
u8 rtw_mp_set_antdiv(_adapter *padapter, BOOLEAN bMain)
{
	...
	if (!adapter_to_efuse(padapter)->antdiv_cfg)
		return _FALSE;
	/*rtw_hal_get_phydm_var(padapter, HAL_PHYDM_ANTDIV_SELECT, &cur_ant, NULL);*/
...
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(adapter_to_dvobj(adapter));
...
{
	struct mp_cal_arg	mp_cal_arg;
	struct mp_priv *pmppriv = &padapter->mppriv;

	_rtw_memset((void *)&mp_cal_arg, 0, sizeof(struct mp_cal_arg));

	mp_cal_arg.cal_type = cal_tye;

	if (rtw_mp_phl_calibration(padapter, &mp_cal_arg, MP_CAL_CMD_TRIGGER_CAL))
		RTW_INFO("%s ok\n", __func__);
	else
		RTW_INFO("%s fail\n", __func__);
...
void rtw_mp_cal_capab(_adapter *padapter, u8 cal_tye, u8 benable)
{
	...
	if (rtw_mp_phl_calibration(padapter, &mp_cal_arg, MP_CAL_CMD_SET_CAPABILITY_CAL))
		RTW_INFO("%s ok\n", __func__);
	else
		RTW_INFO("%s fail\n", __func__);
...
{
	rtw_mp_cal_trigger(padapter, MP_CAL_IQK);
}

void rtw_mp_trigger_lck(_adapter *padapter)
{
	rtw_mp_cal_trigger(padapter, MP_CAL_LCK);
}

void rtw_mp_trigger_dpk(_adapter *padapter)
{
	rtw_mp_cal_trigger(padapter, MP_CAL_DPK);
}

void rtw_mp_trigger_tssi(_adapter *padapter)
{
	rtw_mp_cal_trigger(padapter, MP_CAL_TSSI);
}

void rtw_mp_trigger_ch_rfk(_adapter *padapter)
{
	rtw_mp_cal_trigger(padapter, MP_CAL_CHL_RFK);
}

void rtw_mp_trigger_dack(_adapter *padapter)
{
	rtw_mp_cal_trigger(padapter, MP_CAL_DACK);
}
...
u32 mp_join(_adapter *padapter, u8 mode)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_MP_STATE) == _TRUE)
		goto end_of_g6_mp_start_test;

	/* init g6_mp_start_test status */
	...
	psta = rtw_alloc_stainfo(&padapter->stapriv, bssid.MacAddress, PHL_CMD_DIRECTLY);
	if (psta == NULL) {
...
s32 g6_mp_start_test(_adapter *padapter)
{
	struct mp_priv *pmppriv = &padapter->mppriv;
...
#if 0
#ifdef CONFIG_PCI_HCI
	hal = GET_HAL_DATA(adapter_to_dvobj(padapter));
...
void g6_mp_stop_test(_adapter *padapter)
{
	struct mp_priv *pmppriv = &padapter->mppriv;
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
	struct wlan_network *tgt_network = &pmlmepriv->cur_network;
...
{
#ifdef CONFIG_MP_INCLUDED
	rtw_mp_phl_config_arg(padapter, MP_CONFIG_CMD_SET_CH_BW);
#else
...
void SetAntenna(_adapter *padapter)
{
	...
	rtw_mp_phl_config_arg(padapter, MP_CONFIG_CMD_SET_RF_TXRX_PATH);
}

int rtw_mp_txpoweridx(_adapter *adapter)
{
	struct mp_txpwr_arg	ptxpwr_arg;
	struct mp_priv *pmppriv = &adapter->mppriv;
	u8 tx_nss = GET_HAL_TX_NSS(adapter_to_dvobj(adapter));
	u8 i = 0;

	_rtw_memset((void *)&ptxpwr_arg, 0, sizeof(struct mp_txpwr_arg));
	...
	for (i = 0; i < tx_nss; i++) {
		...
		rtw_mp_phl_txpower(adapter, &ptxpwr_arg, MP_TXPWR_CMD_SET_TXPWR_INDEX);
	}
	return _TRUE;
}
...
s16 rtw_mp_get_pwr_refcw(_adapter *adapter, u8 rfpath, u8 is_cck)
{
	...
	rtw_mp_phl_txpower(adapter, &ptxpwr_arg, MP_TXPWR_CMD_GET_TXPWR_REF_CW);
...
u16 rtw_mp_get_pwr_ref(_adapter *adapter, u8 rfpath)
{
	...
	rtw_mp_phl_txpower(adapter,  &ptxpwr_arg, MP_TXPWR_CMD_GET_TXPWR_REF);
...
u16 rtw_mp_get_pwrtab_dbm(_adapter *adapter, u8 rfpath)
{
	...
	rtw_mp_phl_txpower(adapter,  &ptxpwr_arg, MP_TXPWR_CMD_READ_PWR_TABLE);
...
u16 rtw_mp_txpower_dbm(_adapter *adapter, u8 rf_path)
{
	...
	ptxpwr_arg.txpwr = pmppriv->txpowerdbm * TX_POWER_BASE / 100;
#else
	ptxpwr_arg.txpwr = pmppriv->txpowerdbm * TX_POWER_BASE;
	...
	//DBGP("set tx power = %d\n", ptxpwr_arg.txpwr);
	rtw_mp_phl_txpower(adapter, &ptxpwr_arg, MP_TXPWR_CMD_SET_TXPWR);
...
#ifdef WKARD_DECIMAL_INPUT
	agc_cw_val = pmppriv->txpowerdbm/100 - tmp_power_ref + pre_pwr_refcw_idx + pmppriv->txpoweridx;
#else
	...
	rtw_mp_phl_txpower(adapter, &ptxpwr_arg, MP_TXPWR_CMD_SET_TXPWR_INDEX);
	rtw_mp_set_tssi_avg(adapter, ptxpwr_arg.txpwr * 100);

	return agc_cw_val;
}

void rtw_mp_get_tmac_power(_adapter *padapter){
	/* AP get TMAC power */
	struct mp_priv *pmppriv = &padapter ->mppriv;
	u8 tx_nss = GET_HAL_TX_NSS(adapter_to_dvobj(padapter));
	struct mp_txpwr_arg txpwr_arg = {0};
	struct mp_tx_arg tx_arg = {0};
	s16 txinfo_pwr = 0;
	u16 agc_cw_val = 0;
	u8 i = 0;

	/* step 0: set TX mode to 0x0*/
	tx_arg.mp_class = MP_CLASS_TX;
	tx_arg.cmd = MP_TX_MODE_SWITCH;
	tx_arg.tx_mode = 0;
	tx_arg.cmd_ok = 0;

	rtw_mp_set_phl_cmd(padapter, (void*)&tx_arg, sizeof(struct mp_tx_arg));

	i = 0;
	while (i < 10) {
		rtw_mp_get_phl_cmd(padapter, (void*)&tx_arg, sizeof(struct mp_tx_arg));
		if (txpwr_arg.cmd_ok) {
			RTW_INFO("%s,SET CMD OK\n", __func__);
			//sprintf(extra, "set TX mode gain OK\n");
		} else {
			RTW_INFO("%s,GET CMD FAIL !!! status %d\n", __func__, tx_arg.status);
			//sprintf(extra, "set TX mode Failed\n");
		}
		i++;
		rtw_msleep_os(10);
	}

	/* step 1: set RF mode 0x1 */
	txpwr_arg.mp_class = MP_CLASS_TXPWR;
	txpwr_arg.cmd = MP_TXPWR_CMD_SET_RFMODE;
	txpwr_arg.rf_mode = 1;
	txpwr_arg.cmd_ok = 0;

	rtw_mp_set_phl_cmd(padapter, (void*)&txpwr_arg, sizeof(struct mp_txpwr_arg));

	i = 0;
	while (i < 10) {
		rtw_mp_get_phl_cmd(padapter, (void*)&txpwr_arg, sizeof(struct mp_txpwr_arg));
		if (txpwr_arg.cmd_ok) {
			RTW_INFO("%s,SET CMD OK\n", __func__);
			//sprintf(extra, "set RF mode gain OK\n");
		} else {
			RTW_INFO("%s,GET CMD FAIL !!! status %d\n", __func__, txpwr_arg.status);
			//sprintf(extra, "set RF mode Failed\n");
		}
		i++;
		rtw_msleep_os(10);
	}

	/* step 2: set SW TX Data*/
	tx_arg.mp_class = MP_CLASS_TX;
	tx_arg.cmd = MP_TX_PACKETS;
	tx_arg.tx_method = MP_SW_TX;
	tx_arg.plcp_usr_idx = 0 /*sw_tx_gen_usr_idx*/;
	tx_arg.dbw = pmppriv->bandwidth;
	tx_arg.dcm = 0 /*dcm*/;
	tx_arg.stbc = 0 /*stbc*/;
	tx_arg.gi = pmppriv->st_giltf[rtw_update_giltf(padapter)].gi /*gi*/;
	tx_arg.data_rate = pmppriv->rateidx/*data_rate*/;
	tx_arg.fec = 0 /*fec*/;
	tx_arg.cmd_ok = 0;
	...
	while (i < 10) {
		...
		if (tx_arg.cmd_ok) {
			RTW_INFO("%s,SET CMD OK\n", __func__);
			//sprintf(extra, "set SW TX OK\n");
		} else {
			RTW_INFO("%s,GET CMD FAIL !!! status %d\n", __func__, tx_arg.status);
			//sprintf(extra, "set SW TX Failed\n");
		}
		i++;
		rtw_msleep_os(10);
	}

	/* step 3: query txinfo power*/
	txpwr_arg.mp_class = MP_CLASS_TXPWR;
	txpwr_arg.cmd = MP_TXPWR_CMD_GET_TXINFOPWR;
	txpwr_arg.cmd_ok = 0;
	...
	while (i < 10) {
		...
		if (txpwr_arg.cmd_ok) {
			RTW_INFO("%s,SET CMD OK\n", __func__);
			//sprintf(extra, "get TXInfo power OK\n");
		} else {
			RTW_INFO("%s,GET CMD FAIL !!! status %d\n", __func__, txpwr_arg.status);
			//sprintf(extra, "get TXInfo power Failed\n");
		}
		i++;
		rtw_msleep_os(10);
	}

	txinfo_pwr = txpwr_arg.txpwr / 4;

#ifdef WKARD_DECIMAL_INPUT
	txinfo_pwr *= 100;;
#endif

	/* step 4: set RF mode 0x0*/
	txpwr_arg.mp_class = MP_CLASS_TXPWR;
	txpwr_arg.cmd = MP_TXPWR_CMD_SET_RFMODE;
	txpwr_arg.rf_mode = 0;
	txpwr_arg.cmd_ok = 0;
	...
	/* step 5: set xdbm power control*/
#ifdef WKARD_DECIMAL_INPUT
	pmppriv->txpowerdbm = (u16)txinfo_pwr;
#else
	pmppriv->txpowerdbm = (u8)txinfo_pwr;
#endif

	for (i = 0 ; i < tx_nss; i ++) {
		agc_cw_val = rtw_mp_txpower_dbm(padapter, i);
	}
}
...
void SetDataRate(_adapter *padapter)
{
	...
	rtw_mp_phl_config_arg(padapter, MP_CONFIG_CMD_SET_RATE_IDX);
...
void GetThermalMeter(_adapter *adapter, u8 rfpath ,u8 *value)
{
	...
	ptxpwr_arg.mp_class = MP_CLASS_TXPWR;
	ptxpwr_arg.cmd = MP_TXPWR_CMD_GET_THERMAL;
	ptxpwr_arg.rfpath = rfpath;

	rtw_mp_set_phl_cmd(adapter, (void*)&ptxpwr_arg, sizeof(struct mp_txpwr_arg));

	while (i != 100) {
		rtw_msleep_os(10);
		rtw_mp_get_phl_cmd(adapter, (void*)&ptxpwr_arg, sizeof(struct mp_txpwr_arg));

		if (ptxpwr_arg.cmd_ok) {
...
void rtw_mp_singletone_tx(_adapter *adapter, u8 bstart)
{
	...
	rtw_phl_mp_tx_cmd(adapter, MP_TX_SINGLE_TONE, pmp_priv->rtw_mp_tx_method, bstart);
}
...
void rtw_mp_carriersuppr_tx(_adapter *adapter, u8 bstart)
{
	...
	rtw_mp_txpwr_level(adapter);
	rtw_phl_mp_tx_cmd(adapter, MP_TX_CCK_Carrier_Suppression, pmp_priv->rtw_mp_tx_method, bstart);
}
...
void rtw_mp_continuous_tx(_adapter *adapter, u8 bstart)
{
	...
	rtw_mp_txpwr_level(adapter);
	rtw_phl_mp_tx_cmd(adapter, MP_TX_CONTINUOUS, pmp_priv->rtw_mp_tx_method, bstart);
}
...
u8 rtw_phl_mp_tx_cmd(_adapter *padapter, enum mp_tx_cmd cmdid,
							enum mp_tx_method tx_method, boolean bstart)
{
	struct mp_priv	*pmppriv = &padapter->mppriv;
	struct pkt_attrib *pattrib = &pmppriv->tx.attrib;
	struct mp_tx_arg tx_arg;
	u16 i = 0;
	u8 user_idx = pmppriv->mp_plcp_useridx;

#ifdef RTW_WKARD_AP_MP
	static u32 tmp_scrambler_seed = 1;
	static u32 tmp_random_init_seed = 1;
#endif

	_rtw_memset((void *)&tx_arg, 0, sizeof(struct mp_tx_arg));
	tx_arg.mp_class = MP_CLASS_TX;
	...
	switch (cmdid) {
		...
		case MP_TX_CONFIG_PLCP_COMMON_INFO:
			...
			tx_arg.long_preamble_en = 1 /*pmppriv->preamble*/;
			tx_arg.stbc = pmppriv->rtw_mp_stbc;
			tx_arg.gi = pmppriv->rtw_mp_plcp_gi;
			...
			if (pmppriv->rtw_mp_tx_method == MP_FW_PMAC_TX) {
		...
		case MP_TX_CONFIG_PLCP_USER_INFO:
			...
			if (ch_changed) {
				...
				tmp_scrambler_seed = tx_arg.scrambler_seed;
				tmp_random_init_seed = tx_arg.random_init_seed;
				ch_changed = false;

			} else {

				tx_arg.scrambler_seed = tmp_scrambler_seed;
				tx_arg.random_init_seed = tmp_random_init_seed;
			}

			RTW_WARN("seed: %x, %x\n", tx_arg.scrambler_seed, tx_arg.random_init_seed);
		...
		case MP_TX_MODE_SWITCH:
			if (pmppriv->rtw_mp_tx_method == MP_TMACT_TX)
...
static thread_return mp_xmit_phl_packet_thread(thread_context context)
{
	...
	RTW_INFO("%s: Start !!\n", __func__);
	rtw_phl_trx_default_param(GET_HAL_INFO(dvobj), &test_param);
	...
	while (1) {
		if (pmppriv->rtw_mp_tx_method == MP_TMACT_TX) {
			...
			if (rtw_phl_trx_testsuite(GET_HAL_INFO(dvobj), &test_param) == 0)
...
void rtw_phl_set_dpk(_adapter *padapter){

	struct mp_cal_arg cal_arg;
	u8 i = 0;

	_rtw_memset(&cal_arg, 0, sizeof(cal_arg));
	cal_arg.cmd = MP_CAL_CMD_TRIGGER_DPK_TRACKING;
	cal_arg.mp_class = MP_CLASS_CAL;
	cal_arg.cmd_ok = 0;

	//RTW_PRINT("==> Trigger DPK tracking\n");

	rtw_mp_set_phl_cmd(padapter, (void*)&cal_arg, sizeof(struct mp_cal_arg));
	while (i < 10) {
		rtw_mp_get_phl_cmd(padapter, (void*)&cal_arg, sizeof(struct mp_cal_arg));
		if (cal_arg.cmd_ok) {
			break;
		}
...
void rtw_phl_set_pre_tx(_adapter *padapter){
	...
	tx_arg.mp_class = MP_CLASS_TX;
	tx_arg.cmd = MP_TX_PACKETS;
	tx_arg.tx_method = MP_PMACT_TX;
	tx_arg.start_tx = 1;
	tx_arg.is_cck = rtw_mp_is_cck_rate(pmppriv->rateidx);
	tx_arg.tx_cnt = 0;
	tx_arg.period = (u16)pmppriv->pktInterval;
	tx_arg.tx_time = 5484;
	tx_arg.tx_concurrent_en = 0;

#ifdef WKARD_DECIMAL_INPUT

	/*Protect power dbm*/
	if (pmppriv->pwr_track_status && pmppriv->txpowerdbm / 100 >= 17){

		//RTW_PRINT("==> Backup Stage\n");
		txpwr_arg.mp_class = MP_CLASS_TXPWR;
		txpwr_arg.cmd = MP_TXPWR_CMD_SET_TXPWR;
		txpwr_arg.txpwr = (s16)(17 * TX_POWER_BASE);
		txpwr_arg.cmd_ok = 0;
	...
	if (pmppriv->pwr_track_status && pmppriv->txpowerdbm / 100 >= 17){
		...
		//RTW_PRINT("==> Restore Stage\n");
		txpwr_arg.mp_class = MP_CLASS_TXPWR;
		txpwr_arg.cmd = MP_TXPWR_CMD_SET_TXPWR;
		txpwr_arg.txpwr = (s16)((pmppriv->txpowerdbm * TX_POWER_BASE) / 100);
		txpwr_arg.cmd_ok = 0;
...
void rtw_mp_set_tssi_avg(_adapter *padapter, s32 xdbm){

	struct mp_cal_arg cal_arg = {0};
	u8 i = 0;

	cal_arg.mp_class = MP_CLASS_CAL;
	cal_arg.cmd = MP_CAL_CMD_SET_TSSI_AVG;
	cal_arg.xdbm = xdbm;
	...
	//RTW_PRINT("==> Set TSSI Average\n");

}

void rtw_mp_query_txidle(_adapter *padapter){

	struct mp_tx_arg tx_arg = {0};
	u8 i = 0;

	tx_arg.mp_class = MP_CLASS_TX;
	tx_arg.cmd = MP_TX_CHECK_TX_IDLE;
	tx_arg.tx_state = 0;
	...
	//RTW_PRINT("==> Query TX Idle State: %d\n", tx_arg.tx_state);
}

void rtw_mp_toggle_tx_mode(_adapter *padapter){
	...
	/* step 1: set TX mode to 0x0 */
	//DBGP("step 1: set TX mode 0x0\n");
	...
	/* step 3: set SW TX Data */
	//DBGP("step 3: set SW TX data\n");
...
void rtw_mp_set_dpd_bypass(_adapter *padapter, u8 phy_idx){

	struct mp_reg_arg reg_arg = {0};
	struct mp_tx_arg tx_arg = {0};
	u8 i;
	u32 addr = 0;
	s32 dpd_bypass;
	...
	/* step 1 */
	reg_arg.mp_class = MP_CLASS_REG;
	reg_arg.cmd = MP_REG_CMD_READ_MAC;
	reg_arg.io_type = 4;
	reg_arg.io_offset = addr;
	reg_arg.cmd_ok = 0;
	i = 0;

	rtw_mp_set_phl_cmd(padapter, (void*)&reg_arg, sizeof(struct mp_reg_arg));

	while (i < 10) {
		rtw_mp_get_phl_cmd(padapter, (void*)&reg_arg, sizeof(struct mp_reg_arg));
		if (tx_arg.cmd_ok) {
			break;
		}
	...
	//RTW_PRINT("0x%x = %x\n", addr, reg_arg.io_value);
	dpd_bypass = ((reg_arg.io_value & 0x4000000) >> 26);
	//RTW_PRINT("==> Get dpd_bypass = %x\n", dpd_bypass);


	/* step 2 */
	tx_arg.mp_class = MP_CLASS_TX;
	tx_arg.cmd = MP_TX_DPD_BYPASS;
	tx_arg.dpd_bypass = dpd_bypass;
	tx_arg.cmd_ok = 0;
	i = 0;
...
void rtw_set_phl_packet_tx(_adapter *padapter, u8 bStart)
{
	...
	if (bStart) {
		...
		if (pmp_priv->rtw_mp_tx_method == MP_PMACT_TX ||
				...
				if (rtw_phl_mp_tx_cmd(padapter, MP_TX_CONFIG_PLCP_USER_INFO, pmp_priv->rtw_mp_tx_method, _TRUE) == true){
					RTW_INFO("%s: PLCP_USER_INFO done!!\n", __func__);
				}

				if (rtw_phl_mp_tx_cmd(padapter, MP_TX_CONFIG_PLCP_COMMON_INFO, pmp_priv->rtw_mp_tx_method, _TRUE) == true){
					RTW_INFO(
					"%s: RTW_MP_TX_CONFIG_PLCP_COMMON_INFO done!!\n", __func__);
				}

				rtw_mp_query_txidle(padapter);

				rtw_phl_set_pre_tx(padapter);

				rtw_mp_toggle_tx_mode(padapter);
				rtw_mp_set_dpd_bypass(padapter, 0);

		}

		if (pmp_priv->rtw_mp_tx_method == MP_FW_PMAC_TX) {
			if (rtw_phl_mp_tx_cmd(padapter, MP_TX_PACKETS, pmp_priv->rtw_mp_tx_method, _TRUE))
				RTW_INFO("%s: RTW_MP_FW_PMACT_TX!!\n", __func__);
			else
				RTW_INFO("%s: RTW_MP_FW_PMACT_TX fail!!\n", __func__);
		} else if (pmp_priv->rtw_mp_tx_method == MP_PMACT_TX) {
			if (rtw_phl_mp_tx_cmd(padapter, MP_TX_PACKETS, pmp_priv->rtw_mp_tx_method, _TRUE))
	...
	if (!bStart) {
		if (pmp_priv->rtw_mp_tx_method == MP_FW_PMAC_TX) {
			if (rtw_phl_mp_tx_cmd(padapter, MP_TX_PACKETS, pmp_priv->rtw_mp_tx_method, false)) {
...
void SetPacketRx(_adapter *adapter, u8 bStartRx, u8 bAB)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(adapter_to_dvobj(adapter));
	struct mp_priv *pmppriv = &adapter->mppriv;
	u8 bEnable = 0x1;
	...
	if (bStartRx) {
		...
		if (pmppriv->bSetRxBssid == _TRUE) {
			...
			write_bbreg(adapter, 0x550, BIT3, bEnable);
#endif
			rtw_write16(adapter, REG_RXFLTMAP0, 0xFFEF); /* REG_RXFLTMAP0 (RX Filter Map Group 0) */
		...
		pHalData->ReceiveConfig = 0;
		rtw_write16(adapter, REG_RXFLTMAP0, 0xFFFF); /* REG_RXFLTMAP0 (RX Filter Map Group 0) */
	}
...
{
	RTW_INFO("%s()!!!\n", __func__);
	rtw_mp_phl_config_arg(adapter, MP_CONFIG_CMD_SET_RESET_PHY_COUNT);
}
...
u8 rtw_mp_phl_psd_cmd(_adapter *padapter, struct mp_cal_arg	*psd_arg, u8 cmdid)
{
	struct mp_priv	*pmppriv = &padapter->mppriv;
	u16 i = 0;
	u32 cmd_size = sizeof(struct mp_cal_arg);

	psd_arg->mp_class = MP_CLASS_CAL;
	psd_arg->cmd = cmdid;
 ...
 *
 */
u32 g6_mp_query_psd(_adapter *adapter, u8 *data)
{
	HAL_DATA_TYPE	*pHalData	= GET_HAL_DATA(adapter_to_dvobj(adapter));
	//struct dm_struct *p_dm = adapter_to_phydm(adapter);
	...
	if (check_fwstate(&adapter->mlmepriv, WIFI_MP_STATE) == _FALSE) {
		return 0;
	}
	...
	if (strlen(data) == 0) { /* default value */
		...
		if (rtw_mp_psd_init(adapter, &regs) != 0)
			return 0;
		psd_analysis = 1;
	...
	if (IS_HARDWARE_TYPE_8822C(adapter)) {
			...
			halrf_psd_init_g6(p_dm);
#ifdef CONFIG_LONG_DELAY_ISSUE
		rtw_msleep_os(100);
#else
		rtw_mdelay_os(100);
#endif
			halrf_psd_query_g6(p_dm, psdbuf, 256);
		...
		while (i < psd_stop) {
			if (i >= psd_pts)
				psd_data = rtw_GetPSDData(adapter, i - psd_pts);
			else
				psd_data = rtw_GetPSDData(adapter, i);
...
u16 rtw_mp_rate_parse(_adapter *adapter, u8 *target_str)
{
	...
	for (i = 0; i < MP_RATE_STR_NUM - mcs_idx_base; i++) {
...
u32 mpt_get_tx_power_finalabs_val(_adapter *padapter, u8 rf_path)
{
	...
	ptxpwr_arg.mp_class = MP_CLASS_TXPWR;
	ptxpwr_arg.cmd = MP_TXPWR_CMD_READ_PWR_TABLE;
	...
	rtw_mp_set_phl_cmd(padapter, (void*)&ptxpwr_arg, sizeof(struct mp_txpwr_arg));

	while (i != 100) {
		rtw_msleep_os(10);
		rtw_mp_get_phl_cmd(padapter, (void*)&ptxpwr_arg, sizeof(struct mp_txpwr_arg));
		if (ptxpwr_arg.cmd_ok) {
...
u8 rtw_mpt_set_power_limit_en(_adapter *padapter, bool en_val)
{
	...
	ptxpwr_arg.mp_class = MP_CLASS_TXPWR;
	ptxpwr_arg.cmd = MP_TXPWR_CMD_SET_PWR_LMT_EN;
...
bool rtw_mpt_get_power_limit_en(_adapter *padapter)
{
	...
	ptxpwr_arg.mp_class = MP_CLASS_TXPWR;
	ptxpwr_arg.cmd = MP_TXPWR_CMD_GET_PWR_LMT_EN;
...
u8 rtw_mp_phl_txpower(_adapter *padapter, struct mp_txpwr_arg	*ptxpwr_arg, u8 cmdid)
{
	struct mp_priv	*pmppriv = &padapter->mppriv;
	u16 i = 0;
...
u8 rtw_mp_phl_config_arg(_adapter *padapter, enum mp_config_cmd cmdid)
{
	struct mp_priv	*pmppriv = &padapter->mppriv;
	struct mp_config_arg pmp_arg;
	u16 i = 0;

	_rtw_memset((void *)&pmp_arg, 0, sizeof(struct mp_config_arg));

	pmp_arg.mp_class = MP_CLASS_CONFIG;
	pmp_arg.cmd = cmdid;
	RTW_INFO("%s, id: %d !!!\n", __func__, cmdid);
	...
	case MP_CONFIG_CMD_SET_CH_BW:
		...
		RTW_INFO("set mp_arg band: %d\n", pmp_arg.band);
		RTW_INFO("set mp_arg channel: %d\n", pmp_arg.channel);
		RTW_INFO("set mp_arg bandwidth: %d\n", pmp_arg.bandwidth);
		RTW_INFO("set mp_arg channel_offset: %d\n", pmp_arg.sc_idx);
		break;
	case MP_CONFIG_CMD_SET_RATE_IDX:
		pmp_arg.rate_idx = pmppriv->rateidx;
		RTW_INFO("set mp_arg rate_idx: %d\n", pmp_arg.rate_idx);
		break;
	case MP_CONFIG_CMD_SET_RF_TXRX_PATH:
		pmp_arg.ant_tx = pmppriv->antenna_trx;
		pmp_arg.ant_rx = pmppriv->antenna_trx;
		RTW_INFO("set mp_arg ant_tx: %d, ant_rx: %d\n", pmp_arg.ant_tx, pmp_arg.ant_rx);
		break;
	case MP_CONFIG_CMD_STOP_DUT:
	case MP_CONFIG_CMD_START_DUT:
	case MP_CONFIG_CDM_SET_PHY_INDEX:
		pmp_arg.cur_phy = pmppriv->rtw_mp_cur_phy;
		RTW_INFO("set mp_arg cur_phy: %d\n", pmp_arg.cur_phy);
		break;
	case MP_CONFIG_CMD_SET_RESET_PHY_COUNT:
		pmp_arg.cur_phy = pmppriv->rtw_mp_cur_phy;
		RTW_INFO("set mp_arg cur_phy: %d\n", pmp_arg.cur_phy);
		break;
	case MP_CONFIG_CMD_SET_DBCC:
		pmp_arg.dbcc_en = pmppriv->rtw_mp_dbcc;
		RTW_INFO("set mp_arg dbcc_en: %d\n", pmp_arg.dbcc_en);
		break;
	default:
	...
	rtw_mp_set_phl_cmd(padapter, (void*)&pmp_arg,  sizeof(struct mp_config_arg));
	while (1) {
		rtw_mp_get_phl_cmd(padapter, (void*)&pmp_arg,  sizeof(struct mp_config_arg));
		if (pmp_arg.cmd_ok) {
...
void rtw_mp_phl_rx_rssi(_adapter *padapter, struct mp_rx_arg *rx_arg)
{
	struct mp_priv *pmppriv = &padapter->mppriv;
	u8 i = 0;

	rx_arg->mp_class = MP_CLASS_RX;
	rx_arg->cmd = MP_RX_CMD_GET_RSSI;
...
void rtw_mp_phl_rx_gain_offset(_adapter *padapter, struct mp_rx_arg *rx_arg, u8 path_num)
{
	struct mp_priv *pmppriv = &padapter->mppriv;
...
void rtw_mp_phl_query_rx(_adapter *padapter, struct mp_rx_arg *rx_arg ,u8 rx_qurey_type)
{
	struct mp_priv *pmppriv = &padapter->mppriv;
	u8 rxcmd[2] = {0, 0};
	u8 cmd_idx = 0;
	...
	if (rx_qurey_type == 0) {
		rxcmd[0] = MP_RX_CMD_PHY_CRC_OK;
		rxcmd[1] = MP_RX_CMD_PHY_CRC_ERR;
	} else if (rx_qurey_type == 1){
		rxcmd[0] = MP_RX_CMD_MAC_CRC_OK;
		rxcmd[1] = MP_RX_CMD_MAC_CRC_ERR;
	} else if (rx_qurey_type == 2) {
		rxcmd[0] = MP_RX_CMD_DRV_CRC_OK;
		rxcmd[1] = MP_RX_CMD_DRV_CRC_ERR;
	}
...
void rtw_mp_set_crystal_cap(_adapter *padapter, u32 xcapvalue)
{
	...
	_rtw_memset(&cfg_arg, 0, sizeof(cfg_arg));
	cfg_arg.mp_class = MP_CLASS_REG;
	cfg_arg.cmd = MP_REG_CMD_SET_XCAP;
...
u8 rtw_mp_phl_calibration(_adapter *padapter, struct mp_cal_arg	*pcal_arg, u8 cmdid)
{
	struct mp_priv	*pmppriv = &padapter->mppriv;
	u16 i = 0;
	u32 cmd_size = sizeof(struct mp_cal_arg);

	pcal_arg->mp_class = MP_CLASS_CAL;
	pcal_arg->cmd = cmdid;
...
u8 rtw_set_txpwr_track_status(_adapter *padapter, u8 status){
	...
	ptxpwr_arg.txpwr_track_status = status;

	if (rtw_mp_phl_txpower(padapter, &ptxpwr_arg, MP_TXPWR_CMD_SET_PWR_TRACK_STATUS))
		return true;
	else
		return false;
}
...
u32 rtw_mp_get_tssi_de(_adapter *padapter, u8 rf_path)
{
	...
	if (rtw_mp_phl_txpower(padapter, &ptxpwr_arg, MP_TXPWR_CMD_GET_TSSI))
...
#ifdef WKARD_DECIMAL_INPUT
	ptxpwr_arg.dbm = tgdbm; //user multiply 100 manually
	ptxpwr_arg.pout = out_pwr; //user multiply 100 manually
#else
	ptxpwr_arg.dbm = tgdbm*100; //user multiply 100 manually
	ptxpwr_arg.pout = out_pwr*100; //user multiply 100 manually
#endif
	ptxpwr_arg.rfpath = rf_path;

	if (rtw_mp_phl_txpower(padapter, &ptxpwr_arg, MP_TXPWR_CMD_GET_ONLINE_TSSI_DE))
...
u8 rtw_mp_set_tsside2verify(_adapter *padapter, u32 tssi_de, u8 rf_path)
{
	...
	if (rtw_mp_phl_txpower(padapter, &ptxpwr_arg, MP_TXPWR_CMD_SET_TSSI))
		return true;
	else
		return false;
}
...
u8 rtw_mp_set_tssi_offset(_adapter *padapter, u32 tssi_offset, u8 rf_path)
{
	...
	if (rtw_mp_phl_txpower(padapter, &ptxpwr_arg, MP_TXPWR_CMD_SET_TSSI_OFFSET))
		return true;
	else
		return false;
}
...
u8 rtw_mp_get_tssi_pwrtrk(_adapter *padapter)
{
	...
	if (rtw_mp_phl_txpower(padapter, &ptxpwr_arg, MP_TXPWR_CMD_GET_PWR_TRACK_STATUS)) {
