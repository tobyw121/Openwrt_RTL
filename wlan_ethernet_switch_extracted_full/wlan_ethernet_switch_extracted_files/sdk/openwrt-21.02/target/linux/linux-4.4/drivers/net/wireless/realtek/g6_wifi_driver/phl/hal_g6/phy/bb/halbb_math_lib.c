...
const u32 db_invert_table_g6[12][8] = {
...
u32 halbb_convert_to_db(u64 val)
{
	...
	if (val >= db_invert_table_g6[11][7])
		return 96; /* @maximum 96 dB */

	for (i = 0; i < 12; i++) {
		if (i <= 2 && (val << FRAC_BITS) <= db_invert_table_g6[i][7])
			break;
		else if (i > 2 && val <= db_invert_table_g6[i][7])
			break;
	}

	for (j = 0; j < 8; j++) {
		if (i <= 2 && (val << FRAC_BITS) <= db_invert_table_g6[i][j])
			break;
		else if (i > 2 && i < 12 && val <= db_invert_table_g6[i][j])
			break;
	}
	...
	if (i == 3 && j == 0) {
		if (db_invert_table_g6[3][0] - val >
		    val - (db_invert_table_g6[2][7] >> FRAC_BITS)) {
			i = 2;
			j = 7;
		}
		goto end;
	...
	if (j == 0) {
		if (db_invert_table_g6[i][j] - val >
		    val - db_invert_table_g6[i - 1][7]) {
			i = i - 1;
			j = 7;
		}
	} else {
		if (db_invert_table_g6[i][j] - val >
		    val - db_invert_table_g6[i][j - 1]) {
			j = j - 1;
		}
	}
end:
