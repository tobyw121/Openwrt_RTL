...
unsigned int rtl8192cd_crc32(unsigned char *buf, int len)
{
	unsigned char *p;
	unsigned int  crc;
...
unsigned int crc32_frag(unsigned int crc, unsigned char *buf, int len)
{
	unsigned char *p;
...
void appendICV(unsigned char *src, unsigned int len, unsigned char *dest)
{
	...
	*((unsigned long *)CRC) = rtl8192cd_crc32(src,len);
...
do_tkip_decrypt:
	...
	crc = rtl8192cd_crc32(pframe + hdr_len + 8, fr_len - hdr_len - 8 - 4);
...
#if defined(_DEBUG_RTL8192CD_) && !defined(__ECOS)
{
...
#if defined(_DEBUG_RTL8192CD_) && !defined(__ECOS)
{
	...
	// now, let's check if icv is correct!
	crc = rtl8192cd_crc32(pframe+hdr_len+4, fr_len-hdr_len-4-4);
