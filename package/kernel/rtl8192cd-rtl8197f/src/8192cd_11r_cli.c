...
{		
	...
	if(r0kh_id) {
		...
		memcpy(pStaInfo->cache_r0kh_id, r0kh_id, MIN(r0kh_id_len,MAX_R0KHID_LEN));
		pStaInfo->cache_r0kh_id_len = r0kh_id_len;

		// sync to FT mib
		R0KH_ID_LEN = MIN(r0kh_id_len,sizeof(R0KH_ID));
	...
	if (pstat->ft_state >= state_ft_auth) {
		if (pStaInfo->r1kh) {
			...
			*pos++ = pStaInfo->r1kh->r0kh_id_len;
			memcpy(pos, pStaInfo->r1kh->r0kh_id, MIN(pStaInfo->r1kh->r0kh_id_len,sizeof(temp)));
			pos += pStaInfo->r1kh->r0kh_id_len;
		}
	}
	else {
	...
	if(memcmp(priv->ft_target_bssid, null_mac, MACADDRLEN)) {
		if(pstat->ft.bss_num > 1) {
			...
			|| priv->CliFTAuthState == state_ft_auth_done) {
				...
				memcpy(priv->init_r0kh_id, r1kh->r0kh_id, MIN(r1kh->r0kh_id_len,MAX_R0KHID_LEN));
