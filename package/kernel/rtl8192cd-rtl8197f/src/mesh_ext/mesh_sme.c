...
const UINT8 mesh_PeerLinkStatesTable[][7] =
{
    ...
    {MP_HOLDING, MP_SUBORDINATE_LINK_DOWN_E, MP_SUBORDINATE_LINK_DOWN_E, MP_HOLDING, MP_SUBORDINATE_LINK_DOWN_E, MP_SUBORDINATE_LINK_DOWN_E, MP_UNUSED},
    // state 6:MP_SUBORDINATE_LINK_UP
    {MP_HOLDING, MP_SUBORDINATE_LINK_UP, MP_SUBORDINATE_LINK_UP, MP_HOLDING, MP_SUBORDINATE_LINK_UP, MP_SUBORDINATE_LINK_UP, MP_UNUSED},
    // state 7:MP_SUPERORDINATE_LINK_DOWN
    {MP_HOLDING, MP_SUPERORDINATE_LINK_DOWN, MP_SUPERORDINATE_LINK_DOWN, MP_HOLDING, MP_SUPERORDINATE_LINK_DOWN, MP_SUPERORDINATE_LINK_DOWN, MP_UNUSED},
    // state 8:MP_SUPERORDINATE_LINK_UP
    {MP_HOLDING, MP_SUPERORDINATE_LINK_UP, MP_SUPERORDINATE_LINK_UP, MP_HOLDING, MP_SUPERORDINATE_LINK_UP, MP_SUPERORDINATE_LINK_UP, MP_UNUSED},
    // state 9:MP_HOLDING
...
{
    mw5_legacy_timer_init(&priv->mesh_peer_link_timer);
    ...
    if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
        priv->mesh_peer_link_timer.data = (unsigned long) &priv->mesh_peer_link_timer_event;
        priv->mesh_peer_link_timer.function = timer_event_timer_fn;
        INIT_TIMER_EVENT_ENTRY(&priv->mesh_peer_link_timer_event, mesh_peer_link_timer, (unsigned long)priv);
    }
#endif

#ifdef MESH_BOOTSEQ_AUTH
    mw5_legacy_timer_init(&priv->mesh_auth_timer);
...
static void updateMeshMetric(DRV_PRIV *priv, struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
    ...
    */
    ...
    pstat->mesh_neighbor_TBL.rssi_metric = pfrinfo->rf_info.rssi;
    ...
    UINT16 cur_m = (pfrinfo->rf_info.rssi<28)?(499+((28-pfrinfo->rf_info.rssi)<<6)):((100-pfrinfo->rf_info.rssi)<<1);
...
void peer_expire(DRV_PRIV* priv)
{
    ...
    while(priv&&plist&&(plist != phead)&&(phead))  	// 1.Check index  2.Check is it least element? (Because  next pointer to phead itself)
    {
    	if (priv &&(!(priv->drv_state & DRV_STATE_OPEN)))
			break;
        ...
        if (time_after(jiffies, pstat->mesh_neighbor_TBL.expire + MESH_EXPIRE_TO_STAGE2))  		// expire !!
        {
            ...
            clear_route_info(priv, pstat->cmn_info.mac_addr);	// call del pathselection table entry
        ...
        else if(time_after(jiffies, pstat->mesh_neighbor_TBL.expire))
        {
            ...
            mesh_sme_debug("MESH send probe req to neighbor: %02X:%02X:%02X:%02X:%02X:%02X\n",
                           pstat->cmn_info.mac_addr[0],pstat->cmn_info.mac_addr[1],pstat->cmn_info.mac_addr[2],
                           pstat->cmn_info.mac_addr[3],pstat->cmn_info.mac_addr[4],pstat->cmn_info.mac_addr[5]);
            issue_probereq_MP(priv, NULL, 0, pstat->cmn_info.mac_addr, TRUE);
        }
    }
...
int pathsel_table_expire(DRV_PRIV* priv)
{
    ...
    for (i = 0; i < tbl_sz; i++)
    {
        ...
        if (priv->pathsel_table->entry_array[i].dirty)
        {
			...
			!(get_stainfo(priv, pPathEntry->nexthopMAC) ||
			(priv->mesh_priv_sc && IS_DRV_OPEN(priv->mesh_priv_sc) && 
			GET_MIB(priv->mesh_priv_sc)->dot1180211sInfo.mesh_enable &&
			get_stainfo(priv->mesh_priv_sc, pPathEntry->nexthopMAC))) || 
#endif
			(RTL_JIFFIES_TO_SECOND(now - pPathEntry->update_time) > HWMP_ACTIVE_PATH_TIMEOUT))
			{
				if(pPathEntry->metric != 99999 
#if defined(RTK_MESH_METRIC_REFINE)
				|| priv->pmib->meshPathsel.mesh_crossbandEnable == 1
#endif
				)
				{
...
void mesh_standalone_timer_expire(unsigned long task_priv)
{
    ...
    aodv_expire(priv);
    mw5_legacy_mod_timer(&priv->mesh_expire_timer, jiffies + MESH_AODV_EXPIRE_TO);
}
#endif
...
// MESH NOP
			if(priv->pshare->CurrentChannelBW == CHANNEL_WIDTH_80 && 
				...
				int i, channel = priv->pmib->dot11RFEntry.dot11channel;
...
void static mesh_CSA_expire(DRV_PRIV *priv) {
	if(priv->mesh_CSA_blocking)
		priv->mesh_CSA_blocking--;
	if(priv->mesh_CSA_state == MESH_CSA_ONGOING) {
		if(priv->mesh_CSA_expire) {
			priv->mesh_CSA_expire--;
		}
		else {
			priv->pmib->dot11RFEntry.dot11channel = priv->mesh_CSA_channel;
			priv->pshare->is_40m_bw = priv->mesh_CSA_mode & ~RTK_MESH_CSA_MODE;
			...
			printk("CSA finished: swchnl=%u, BW=%u, offset=%u\n",
				priv->pmib->dot11RFEntry.dot11channel, priv->pshare->CurrentChannelBW, priv->pshare->offset_2nd_chan);
			...
			if (GET_ROOT(priv)->pmib->miscEntry.vap_enable) {
				for (i = 0; i < RTL8192CD_NUM_VWLAN; i++) {
					...
					priv->pvap_priv[i]->ht_cap_len = 0;  // reconstruct ie
				}
			}
#endif
		}
	}
	else if(priv->mesh_CSA_state == MESH_CSA_READY) {
		priv->mesh_CSA_expire = MESH_CHANNEL_SWITCH_COUNTER;
		priv->mesh_CSA_blocking = GET_MIB(priv)->meshCSA.mesh_csa_blocking;
		priv->mesh_CSA_state = MESH_CSA_ONGOING;
	}
	else if(priv->mesh_CSA_state == MESH_CSA_IGNORE) {
		priv->mesh_CSA_blocking = GET_MIB(priv)->meshCSA.mesh_csa_blocking;
		priv->mesh_CSA_state = MESH_CSA_NONE;
		printk("Mesh CSA ignored: same channel\n");
	}
}

int mesh_CSA_check(DRV_PRIV *priv, struct rx_frinfo *pfrinfo)
{
	unsigned char *pframe, *p;
    int len;
    UINT8 swchnl_mode = 0;
	...
	if(!IS_ROOT_INTERFACE(priv) || !GET_MIB(priv)->meshCSA.mesh_csa_enable)
		return ret;
    ...
    if (p)
    {
        swchnl_mode = *(p + MESH_IE_BASE_LEN);
        swchnl_channel = *(p + MESH_IE_BASE_LEN + 1);
        // check Secondary Channel Offset
        p = get_ie(pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_, _SECONDARY_CHANNEL_OFFSET_IE_, &len,
                   pfrinfo->pktlen - WLAN_HDR_A3_LEN - _BEACON_IE_OFFSET_);
	...
	if(swchnl_channel && (swchnl_mode & RTK_MESH_CSA_MODE) && swchnl_precedence && swchnl_ttl > 0)
    {
        if (swchnl_channel == priv->pmib->dot11RFEntry.dot11channel) {
            return ret;
        }

        if(priv->mesh_CSA_state == MESH_CSA_NONE)
        {
            priv->mesh_swchnl_ttl = swchnl_ttl - 1;
            priv->mesh_swchnl_flag = swchnl_flag & ~BIT1; /*erase initiator bit*/
            priv->mesh_swchnl_reason = swchnl_reason;
            ...
            priv->mesh_CSA_mode = swchnl_mode;
            priv->mesh_CSA_channel= swchnl_channel;

            printk("Detect CSA from other MAP: swchnl=%u, mode=%d, offset=%u\n",
                priv->mesh_CSA_channel, priv->mesh_CSA_mode, priv->mesh_swchnl_offset);
...
void mesh_expire(DRV_PRIV* priv)
{
	...
	if(priv->mesh_target_block)
		priv->mesh_target_block--;
	if(priv->ss_serial_timer)
		priv->ss_serial_timer--;

	/* Easy way to encrypt a string */
	if(priv->up_time < 5) {
		extern unsigned char MESH_SERIAL_KEY;
		int i;
		int len = strlen(priv->pmib->miscEntry.hw_serial_no);
		unsigned char *key_ptr = &MESH_SERIAL_KEY;
		memcpy(priv->hw_serial_no_cipher, priv->pmib->miscEntry.hw_serial_no, len);
		for(i = 0; i < len; i++)
			priv->hw_serial_no_cipher[i] ^= key_ptr[i%16];
	}
#endif
}
...
{
#ifdef RTK_MESH_SIMPLE_CSA
    if(priv->mesh_CSA_state == MESH_CSA_READY || priv->mesh_CSA_state == MESH_CSA_ONGOING) {
        meshiearray[0] = priv->mesh_CSA_mode;
        meshiearray[1] = priv->mesh_CSA_channel;
    } else
#endif
	{
...
static int mesh_ie_LocalLinkStateANNOU(DRV_PRIV *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat, UINT8 meshiearray[])
{
    ...
    // ept (temporary rssi)
    // pfrinfo->rf_info.rssi = 0xaa;//test
    tmp = cpu_to_le16((UINT16)pfrinfo->rf_info.rssi);
    memcpy(meshieptr, &tmp, 2);
    counter += 2;
...
void issue_disassoc_MP(DRV_PRIV *priv, struct stat_info *pstat, int reason, UINT8 peerLinkReason)
{
    ...
    unsigned char *da = pstat->cmn_info.mac_addr;
    ...
    MESH_DEBUG_MSG("*** Issue Disassocation, MAC=%02X:%02X:%02X:%02X:%02X:%02X, State=%d, Reason = %d\n"
                   , pstat->cmn_info.mac_addr[0], pstat->cmn_info.mac_addr[1], pstat->cmn_info.mac_addr[2], pstat->cmn_info.mac_addr[3], pstat->cmn_info.mac_addr[4], pstat->cmn_info.mac_addr[5], pstat->mesh_neighbor_TBL.State, peerLinkReason);
...
unsigned int issue_assocreq_MP(DRV_PRIV *priv,  struct stat_info *pstat)
{
	...
	if(IS_MCAST(pstat->cmn_info.mac_addr))
...
void issue_assocrsp_MP(DRV_PRIV *priv, unsigned short status, struct stat_info *pstat, int pkt_type)
{
    ...
    if (GET_CHIP_VER(priv)==VERSION_8188E && (!pstat->cmn_info.ra_info.disable_ra))
    {
...
static void issue_LocalLinkStateANNOU_MP(DRV_PRIV *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat)
{
    ...
    *pbuf = _MESH_LINK_REPORT_ACTION_ID_;
    ...
    memcpy((void *)GetAddr1Ptr((txinsn.phdr)), pstat->cmn_info.mac_addr, MACADDRLEN);
    memcpy((void *)GetAddr2Ptr((txinsn.phdr)), GET_MY_HWADDR, MACADDRLEN);
    memcpy((void *)GetAddr3Ptr((txinsn.phdr)), pstat->cmn_info.mac_addr, MACADDRLEN);	// Don't know how to set, How category? (Refer: Draft 1.06, Page 12, 7.2.3, Line 21~30 2007/08/11 by popen)
...
static void mesh_PeerLinkEstablish(DRV_PRIV *priv,  struct stat_info *pstat)
{
    ...
    if (memcmp(GET_MY_HWADDR, pstat->cmn_info.mac_addr, MACADDRLEN) > 0)
    ...
    if( priv->pmib->dot11sKeysTable.dot11Privacy)
        dot11s_mp_set_key(priv, pstat->cmn_info.mac_addr);
#endif
    ...
    memcpy(Entry.destMAC, pstat->cmn_info.mac_addr, MACADDRLEN);
    memcpy(Entry.nexthopMAC, pstat->cmn_info.mac_addr, MACADDRLEN);
	Entry.metric = 99999;
	Entry.hopcount = 1;
    ...
    MESH_DEBUG_MSG("Mesh: New MP is establish... MAC=%02X:%02X:%02X:%02X:%02X:%02X State=%d\n"
                   , pstat->cmn_info.mac_addr[0], pstat->cmn_info.mac_addr[1], pstat->cmn_info.mac_addr[2], pstat->cmn_info.mac_addr[3], pstat->cmn_info.mac_addr[4], pstat->cmn_info.mac_addr[5], pstat->mesh_neighbor_TBL.State);
...
restart:
    ...
    while(plist != phead)   // 1.Check index  2.Check is it least element? (Because next pointer to phead itself)
    {
        ...
        if (time_after(jiffies, pstat->mesh_neighbor_TBL.BSexpire_LLSAperiod))
        {
            MESH_DEBUG_MSG("\nMesh: Auth retry expire!!, MAC:%02X:%02X:%02X:%02X:%02X:%02X, State=%d, Retry=%d\n"
                           , pstat->cmn_info.mac_addr[0], pstat->cmn_info.mac_addr[1], pstat->cmn_info.mac_addr[2], pstat->cmn_info.mac_addr[3], pstat->cmn_info.mac_addr[4], pstat->cmn_info.mac_addr[5], pstat->mesh_neighbor_TBL.State, pstat->mesh_neighbor_TBL.retry);

            if ((MP_OPEN_SENT == pstat->mesh_neighbor_TBL.State) && (pstat->mesh_neighbor_TBL.retry++ < MESH_AUTH_RETRY_LIMIT))
            {
                RESTORE_INT(flags);
                pstat->auth_seq = 1;
                issue_auth_resp(priv, pstat, _STATS_SUCCESSFUL_);
	...
	if (&priv->mesh_auth_timer)
	{
	    if (!(list_empty(phead)))
	        mw5_legacy_mod_timer(&priv->mesh_auth_timer, jiffies + MESH_TIMER_TO);
	}
    RESTORE_INT(flags);
}
#endif
...
void mesh_peer_link_timer(unsigned long pVal)
{
    ...
    while ((phead != NULL) && (plist != NULL) && (plist != phead))   // 1.Check index  2.Check is it least element? (Because next pointer to phead itself)
    {
        pstat = list_entry(plist, struct stat_info, mesh_mp_ptr); // Find process MP
    ...
    if (&priv->mesh_peer_link_timer)
	{
		if (priv && (priv->drv_state & DRV_STATE_OPEN))
	    {
			if (!(mw5_legacy_timer_pending(&priv->mesh_peer_link_timer)))
				mw5_legacy_mod_timer(&priv->mesh_peer_link_timer, jiffies + MESH_TIMER_TO);
    	}
    }
    RESTORE_INT(flags);
}
...
#ifdef MESH_ESTABLISH_RSSI_THRESHOLD
        if (pfrinfo->rf_info.rssi < priv->mesh_fake_mib.establish_rssi_threshold)
        {
            return SUCCESS;
        }
#endif
...
reInitial:
		...
		if (&priv->mesh_peer_link_timer) {
	        if (!(mw5_legacy_timer_pending(&priv->mesh_peer_link_timer)))		// start timer if stop
	            mw5_legacy_mod_timer(&priv->mesh_peer_link_timer, jiffies + MESH_TIMER_TO);
		}
        RESTORE_INT(flags);
    ...
    if (0 < cap)  	// ActivePeerLinkOpen (line 10-14, page 113,  D1.0)
    {
        ...
        if (!(mw5_legacy_timer_pending(&priv->mesh_auth_timer)))	// start timer if stop
            mw5_legacy_mod_timer(&priv->mesh_auth_timer, jiffies + MESH_TIMER_TO);
...
#ifdef RTL8190_FASTEXTDEV_FUNCALL
    rtl865x_extDev_addHost(	pstat->cmn_info.mac_addr,
                            priv->WLAN_VLAN_ID,
                            priv->dev,
...
//            MESH_DEBUG_MSG("receive vht_cap len = %d \n", len);
 ...
 //           MESH_DEBUG_MSG("receive vht_oper len = %d \n", len);
    ...
    get_matched_rate(priv, supportRate, &supportRateNum, 1, sizeof(supportRate));
    update_support_rate(pstat, supportRate, supportRateNum);
    assign_tx_rate(priv, pstat, pfrinfo);
    assign_aggre_mthod(priv, pstat);
    assign_aggre_size(priv, pstat);
    /*
    	...
    	if (mw5_legacy_timer_pending(&priv->reauth_timer))
    		mw5_legacy_del_timer_sync(&priv->reauth_timer);
    	if (mw5_legacy_timer_pending(&priv->reassoc_timer))
...
// ==== modified by GANTOE for wrong parameters of issue_probersp_MP 2008/12/25 ====
    ...
    if ( !pmib->dot1180211sInfo.mesh_ap_enable && pmib->dot1180211sInfo.mesh_enable )
		...
		rec_ssid = get_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, _SSID_IE_, (int *)&len,
			pfrinfo->pktlen - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);
		...
		if (len == 0) {
			...
			unsigned char dummy_ssid[] = "MESH-SCAN";
			if (!memcmp(dummy_ssid, rec_ssid+2, 9)) {
				if (HIDDEN_AP || TAKEOVER_HIDDEN_AP)
					return FAIL;
			}
		}
...
unsigned int OnProbeRsp_MP(DRV_PRIV *priv, struct rx_frinfo *pfrinfo)
{
    ...
    if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable)
    {
        ...
        if(!priv||NULL == p ||
              (!mw5_legacy_timer_pending(&priv->ss_timer) &&
                  (len != strlen(GET_MIB(priv)->dot1180211sInfo.mesh_id) ||
        ...
        #ifdef SIMPLE_CH_UNI_PROTOCOL
        memcpy(&precedence, p+MESH_CAP_CH_PRECEDENCE_OFFSET, MESH_CAP_CH_PRECEDENCE_LEN);
		precedence = le32_to_cpu(precedence);
        #endif
        ...
        if(priv &&(!mw5_legacy_timer_pending(&priv->ss_timer)))
#endif
...
#ifdef RTK_MESH_SIMPLE_CSA
		if(mesh_CSA_check(priv, pfrinfo))
			return SUCCESS;
#endif
...
// CH1, CH6
		p = get_ie(pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_, _DSSET_IE_, &len,
			pfrinfo->pktlen - WLAN_HDR_A3_LEN - _BEACON_IE_OFFSET_);
		if (p != NULL) {
			if(*(p+2) !=  priv->pmib->dot11RFEntry.dot11channel) 
				return FAIL;
		} else {
...
//                MESH_DEBUG_MSG("Mesh: OnBeacon, Issue LocalLinkStateAnnouncement\n");
                ...
                // Use rssi temporary
                pstat->mesh_neighbor_TBL.ept = (UINT16)pfrinfo->rf_info.rssi;
                pstat->mesh_neighbor_TBL.Q = pfrinfo->rf_info.rssi;
                pstat->mesh_neighbor_TBL.BSexpire_LLSAperiod = jiffies + MESH_LocalLinkStateANNOU_TO;
        ...
        if((&priv->ss_timer)&&!mw5_legacy_timer_pending(&priv->ss_timer))
#endif
        {
            start_MeshPeerLink(priv, pfrinfo, pstat, cap);
        }
...
unsigned int OnDisassoc_MP(DRV_PRIV *priv, struct rx_frinfo *pfrinfo)
{
    ...
    DEBUG_INFO("receiving disassoc from station %02X%02X%02X%02X%02X%02X reason %d\n",
               pstat->cmn_info.mac_addr[0], pstat->cmn_info.mac_addr[1], pstat->cmn_info.mac_addr[2],
               pstat->cmn_info.mac_addr[3], pstat->cmn_info.mac_addr[4], pstat->cmn_info.mac_addr[5], reason);
    ...
    if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable)
    {
        ...
        if (((pstat->mesh_neighbor_TBL.LocalLinkID != recvPeerLinkID) && (0 != recvPeerLinkID)) 	// Allow peer MP have no PeerLinkID
                || ((pstat->mesh_neighbor_TBL.PeerLinkID != recvLocalLinkID) && (pstat->mesh_neighbor_TBL.PeerLinkID != 0)))  		// Denied peer MP disassoc request
        {
            SAVE_INT_AND_CLI(flags);
			auth_list_del(priv, pstat);
        ...
        MESH_DEBUG_MSG("\nMesh: Reciver DisAssoc, MAC=%02X:%02X:%02X:%02X:%02X:%02X State=%d, Reason=%d\n"
                       , pstat->cmn_info.mac_addr[0], pstat->cmn_info.mac_addr[1], pstat->cmn_info.mac_addr[2], pstat->cmn_info.mac_addr[3], pstat->cmn_info.mac_addr[4], pstat->cmn_info.mac_addr[5], pstat->mesh_neighbor_TBL.State, *(p+MESH_IE_BASE_LEN));
...
void OnProxyUpdate_MP(DRV_PRIV *priv, struct rx_frinfo *pfrinfo)
{
    ...
    else
    {
		...
		pstat = get_stainfo(priv, Entry.proxiedmac);

		if (pstat) {
			del_station(priv, pstat, DEL_STA_SEND_NONE, _RSON_UNSPECIFIED_);
		} else if (priv->mesh_priv_sc && IS_DRV_OPEN(priv->mesh_priv_sc)) {
			pstat = get_stainfo(priv->mesh_priv_sc, Entry.proxiedmac);
			
			if (pstat) {
				del_station(priv->mesh_priv_sc, pstat, DEL_STA_SEND_NONE, _RSON_UNSPECIFIED_);
			}
		}
...
unsigned int OnPathSelectionManagFrame(DRV_PRIV *priv, struct rx_frinfo *pfrinfo, int Is_6Addr)
{
    ...
    //RECV_data.Pre_Hop_Metric = (140 - pfrinfo->rf_info.rssi);
    // RECV_data.Pre_Hop_Metric = (pfrinfo->rf_info.rssi<28)?(499+((28-pfrinfo->rf_info.rssi)<<6)):((100-pfrinfo->rf_info.rssi)<<1);
...
int mesh_open(DRV_PRIV *priv, struct net_device *dev)
{
	unsigned int i;

#if defined(CONFIG_TRIBAND_MESH_SINGLE_IFACE)
	extern u32 if_priv_mesh[NUM_WLAN_IFACE];
	struct rtl8192cd_priv *tmp_priv;

	for (i = 0; i < NUM_WLAN_IFACE; i++) {
		tmp_priv = (struct rtl8192cd_priv *)if_priv_mesh[i];
		if (tmp_priv && IS_DRV_OPEN(tmp_priv) && GET_MIB(tmp_priv)->dot1180211sInfo.mesh_enable) {
			priv = tmp_priv;
			...
			dev->priv = priv;
#endif
			//tmp_priv->mesh_priv_first = priv;
			//tmp_priv->mesh_priv_sc = priv;
			
			memcpy((void *)dev->dev_addr, tmp_priv->pmib->dot11OperationEntry.hwaddr, MACADDRLEN);
			break;
		}
	}
...
#if defined(RTK_MESH_AODV_STANDALONE_TIMER)
	mw5_legacy_timer_init(&priv->mesh_expire_timer);
#ifdef __KERNEL__
...
#ifdef __KERNEL__
	mw5_legacy_add_timer(&priv->mesh_expire_timer);
#else
	if (&priv->mesh_expire_timer)
		mw5_legacy_mod_timer(&priv->mesh_expire_timer, MESH_AODV_EXPIRE_TO);
#endif
    ...
    #if defined(RTK_MESH_AODV_STANDALONE_TIMER)
		mw5_legacy_timer_init(&priv->mesh_priv_sc->mesh_expire_timer);
    #ifdef __KERNEL__
    ...
    #ifdef __KERNEL__
		mw5_legacy_add_timer(&priv->mesh_priv_sc->mesh_expire_timer);
    #else
		mw5_legacy_mod_timer(&priv->mesh_priv_sc->mesh_expire_timer, MESH_AODV_EXPIRE_TO);
    #endif
    #endif //RTK_MESH_AODV_STANDALONE_TIMER
...
#if defined(RTK_MESH_AODV_STANDALONE_TIMER)
	if (&priv->mesh_expire_timer) {
	    if (mw5_legacy_timer_pending(&priv->mesh_expire_timer))
	       mw5_legacy_del_timer_sync(&priv->mesh_expire_timer);
	}
#if defined(CONFIG_RTL_MESH_CROSSBAND)
    if (IS_DRV_OPEN(priv->mesh_priv_sc) && GET_MIB(priv->mesh_priv_sc)->dot1180211sInfo.mesh_enable) {
        if (mw5_legacy_timer_pending(&priv->mesh_priv_sc->mesh_expire_timer))
            mw5_legacy_del_timer_sync(&priv->mesh_priv_sc->mesh_expire_timer);
    }
#endif
#endif
