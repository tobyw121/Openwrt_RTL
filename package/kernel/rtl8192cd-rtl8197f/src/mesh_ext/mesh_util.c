...
void mesh_cnt_ASSOC_PeerLink_CAP(DRV_PRIV *priv, struct stat_info *pstat, int act)
{
	...
	INT16 mesh_assoc_sum = 0;

	/* we need to recalculate to prevent peer running out accidentally */
	phead = &priv->mesh_mp_hdr;
	plist = phead->next;
	while ((plist != phead) && phead && plist) {
		mesh_assoc_sum++;
		plist = plist->next;
	}
	MESH_PEER_LINK_CAP_NUM(priv) = MAX_MPP_NUM - mesh_assoc_sum;
	...
	if (DECREASE == act) {
		...
		if (MESH_PEER_LINK_CAP_NUM(priv) <= 0) {
			modify = FALSE;
			printk("MESH PeerLink CAP Number Error (0)!\n");
		}
	}
...
int remove_proxy_entry(DRV_PRIV *priv, unsigned char *owner, unsigned char* sta)
{
    ...
    SMP_TRY_LOCK_MESH_PROXY(flags, locked);
...
int mesh_proxy_update(DRV_PRIV *priv, unsigned char* owner, unsigned char* sta) {
    ...
    if(!pProxyEntry) {
		struct path_sel_entry *path_entry = NULL;
		path_entry = HASH_SEARCH(priv->pathsel_table, sta);
		if (path_entry) {
			SMP_UNLOCK_MESH_PROXY(flags);
			return 0;
		}
...
{
    /*
     * MW5 v44.36 safety hardening:
     *
     * Infrastructure AP association used to call this legacy 11s helper
     * whenever CONFIG_RTK_MESH was compiled, even when no mesh netdev had
     * ever been created.  netif_running(NULL) then dereferenced offset 0x24,
     * matching the reproducible MW5 BadVA=0x24 panic.
     *
     * The MW5 build now compiles legacy mesh out completely.  Keep this
     * defensive guard for the RD05/AC23 variants that still build mesh.
     */
    if (!priv || !sta || !priv->pmib || !priv->mesh_dev)
        return;
