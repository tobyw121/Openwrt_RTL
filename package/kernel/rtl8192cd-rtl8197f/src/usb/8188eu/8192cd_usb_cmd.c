...
int _rtw_init_cmd_priv(struct rtl8192cd_priv *priv)
{
	...
	mw5_legacy_timer_init(&pshare->chk_h2c_buf_timer);
...
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
		if (_rtw_queue_empty(&pshare->h2c_cmd_queue) == FALSE) {
			if (!mw5_legacy_timer_pending(&pshare->chk_h2c_buf_timer))
				mw5_legacy_mod_timer(&pshare->chk_h2c_buf_timer, jiffies + CHK_H2C_BUF_INTERVAL);
		}
#endif
	}
...
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
	if (mw5_legacy_timer_pending(&pshare->chk_h2c_buf_timer))
		mw5_legacy_del_timer_sync(&pshare->chk_h2c_buf_timer);
#endif
