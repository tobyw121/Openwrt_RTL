...
//static int tkip_mic_padding(struct rtl8192cd_priv *priv, unsigned char *da, unsigned char *sa, unsigned char priority, unsigned char *llc,struct sk_buff *pskb, struct tx_insn* txcfg);
...
void rtl8192cd_xmit_dsr(unsigned long task_priv)
{
	...
	if (!atomic_add_unless(&(priv->pshare->txdata_queue_dequeuing), 1, 1)) {
		return;
	}
#endif
...
#ifdef TX_PROCESS_REFINE
			while ((skb = rtk_txdata_queue_dequeue(priv, &priv->pshare->skb_xmit_queue[idx])) != NULL)
#else
    ...
    if(timer_timeout == 0) {
		if(priv->pshare->atm_swq_current_timeout) {
	    	...
	    	if(priv->pshare->atm_swq_use_hw_timer) {
	        	...
	        	if (mw5_legacy_timer_pending(&priv->pshare->atm_swq_sw_timer))
        ...
        if(priv->pshare->atm_swq_use_hw_timer) {
            ...
            if(set_timer){
				priv->pshare->atm_swq_current_timeout = timer_timeout;
        		mw5_legacy_mod_timer(&priv->pshare->atm_swq_sw_timer, (unsigned long)timer_timeout);
            }
        }
    }
}
...
__inline__ static int rtl8192cd_atm_swq_dequeue(struct rtl8192cd_priv *priv)
{
	int i, qnum, deq_num=0, burst_num=0;
	struct stat_info *pstat;
...
#elif (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
