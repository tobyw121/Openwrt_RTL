...
#include "./core/core_ieee802_mib.h"

#define MIB_VERSION				33
...
struct Dot11StationConfigEntry {
	...
	unsigned char	dot11DesiredSSID[33];
	unsigned int	dot11DesiredSSIDLen;
	unsigned char	dot11DefaultSSID[33];
	unsigned int	dot11DefaultSSIDLen;
	unsigned char	dot11SSIDtoScan[33];
	...
	unsigned int	desiredBcstRate;		// use the specific rate to send broadcast
...
//#ifdef WLAN_DIAGNOSTIC
	...
	unsigned int	probe_info_timeout;		// for proc probe_info mode 2
	unsigned int	scan_scope;			// 0: all channel, 1: scan_channel
	unsigned int	scan_channel;		// 0: current channel
	unsigned int	scan_interval;
	unsigned int	passive_scan;
	unsigned int	probe_info_collecting;
//#endif
	...
	int sc_fix_bw;
	int sc_fix_offset;
	int del_expire_station;
	unsigned int 	monitor_sta_enabled; //0:disable, 1:monitor current channel, 2:monitor with site survey
	...
	unsigned char   dot11RMRestrictEnabled;
	unsigned int    dot11RMRestrictThreshold;
    
    /* 11R client mode CONFIG_IEEE80211R_CLI */
    unsigned int 	dot11FTAuthReqTime;
    unsigned char 	dot11FTTestMode;
    unsigned char	ft_auth_mac[MACADDRLEN]; //CONFIG_IEEE80211W	
    unsigned char	ft_manual_idx;
...
struct Dot1180211AuthEntry {
	...
	int				dot11EnablePSK;			// 0: disable, bit0: WPA, bit1: WPA2, bit3: WPA3
	...
	/*AUTH_SAE*/
	int				dot11WPA3Cipher; // bit0-wep64, bit1-tkip, bit2-wrap,bit3-ccmp, bit4-wep128
	unsigned int	dot11SAEGroup;
	unsigned int	dot11SAETokenThres;
};
...
union Keytype {
	unsigned char	skey[32]; 	/*AUTH_SAE WPA3_ENT*/
	unsigned int	lkey[4];
};
...
struct Dot11RsnIE {
	unsigned char	rsnie[64];
	unsigned char	rsnielen;
};

struct Dot11SuppReguClassesIE {
...
//#ifdef CONFIG_POWER_SAVE
	unsigned int	ps_level;		// 2: SAPPS+32K, 1: SAPPS, 4: AP Offload only
	unsigned int	ps_timeout;
	unsigned int	ps_sleep_time;
//#endif
//#ifdef LIFETIME_FEATURE
    unsigned int	lifetime; //unit: millisecond
//#endif
//#ifdef FW_DROP_EXPIRE
	unsigned int	fwDropExpire;
//#endif
};
...
struct Dot11RFEntry {
	...
	unsigned char	pwrlevel_TSSICCK_A[MAX_2G_CHANNEL_NUM];
	unsigned char	pwrlevel_TSSICCK_B[MAX_2G_CHANNEL_NUM];
	unsigned char	pwrlevel_TSSICCK_C[MAX_2G_CHANNEL_NUM];
	unsigned char	pwrlevel_TSSICCK_D[MAX_2G_CHANNEL_NUM];
	unsigned char	pwrlevel_TSSIHT40_1S_A[MAX_2G_CHANNEL_NUM];
	unsigned char	pwrlevel_TSSIHT40_1S_B[MAX_2G_CHANNEL_NUM];
	unsigned char	pwrlevel_TSSIHT40_1S_C[MAX_2G_CHANNEL_NUM];
	unsigned char	pwrlevel_TSSIHT40_1S_D[MAX_2G_CHANNEL_NUM];	
...
// RTK_AC_SUPPORT
	...
	unsigned int	tssi1;
	unsigned int	tssi2;
	unsigned int	rx_data_offload;
	unsigned int	ther;
	unsigned int	ther2;
	unsigned int	thermal[4];
...
struct br_ext_info {
	...
	unsigned int	macclone_method;		// WiFi STA clone method
	unsigned int	macclone_eth_method;	// Eth STA clone method	
};
...
//#ifdef AUTO_CHANNEL_TIMEOUT
	...
	unsigned int	autoch_diff_pkt;
//#endif
	unsigned int	ss_score_timeout;
//#ifdef TCP_ACK_ACC
	unsigned int	tcpack_acc;
	unsigned int	tcpack_to;
	unsigned int	tcpack_delay_start;
	unsigned int	tcpack_cnt_th1;
	unsigned int	tcpack_cnt_th2;
//#endif
	...
	unsigned int    ss_cnt;
	unsigned int	autoch_3664157_enable;
	unsigned int	autoch_5gmask;			
	...
	unsigned int	fixed_igi;
	unsigned int	cckpd_enhance;
	unsigned char	rssi_th;	
	unsigned char	manual_thru_mode; 
	unsigned char	cckpd_txtp;		
	unsigned int	dyn_ampdu;
	unsigned int 	update_rxbd_th;
	unsigned char   disable_m3_retry;
	unsigned char	rate_var_dbg;
	unsigned char	ra_training;
	unsigned int	ra_clm_th;
	unsigned int	ra_cca_th;	
	unsigned char	ra_rssi_th;	
	unsigned int	ra_rty_th;
	unsigned char   normal_LA_on;
	unsigned int 	client_host_sniffer_enable;
	unsigned long 	client_host_sniffer_interval;
	unsigned int	manual_priority;
	unsigned int 	targetssid_weight_enable;
	unsigned char	prefer_bssid[MACADDRLEN];
	unsigned int 	thre_bssid;
	unsigned int 	ignore_rssi;
	unsigned int 	thre_rssi;
	unsigned int 	thre_rssi_tolerance;
	unsigned char   telco_selected;
	unsigned int 	ap_neighbor_info_enable;
	char			hw_serial_no[65];  // flash: HW_SERIAL_NO
};
...
//#ifdef CONFIG_RTK_WPS_MESH
	unsigned int	wsc_meshmode; // 0:original rule, 2:wps mesh reg, 3:wps mesh en 	
	unsigned char	dot11DesiredSSIDbak[32];
	unsigned int	dot11DesiredSSIDLenBak;	
	int				wscd_pid;
	unsigned char	wsc_mac_cipher[32]; // set from wscd 
};
...
struct Dot11nConfigEntry {
	...
	unsigned int	dot11nShortGIfor160M;
	unsigned int	dot11nVHTProtocol;
	unsigned int	dot11nComply160M;
	unsigned int	dot11nSTBC;
	unsigned int	dot11nLDPC;	
	unsigned int	dot11nAMPDU;
	unsigned int	dot11nAMSDU;
	unsigned int	dot11nAMSDUAMPDURx;
...
//#ifdef SUPPORT_TX_AMSDU_SMALL_PKTS_ONLY
	unsigned int	dot11nAMSDUSmallPkts;		//enable this function to xmit small pkts with AMSDU when AMPDU
	unsigned int	dot11nAMSDUSmallPktLen;	// queue pkts which length is smaller than this setting, queue TCP ACK only if this setting is 0
	unsigned int	dot11nAMSDUTCPAckDropDup;	// Drop duplicate TCP Ack at the same TCP connection, take effect when queue ACK only
//#endif
	...
	unsigned int	dot11nAMSDURestrict;
	unsigned int	dot11nCurAMSDUSendNum;	// current AMSDU packet number
	unsigned int	dot11nAMSDUSwqEn;		// enable tx amsdu swq
	...
	unsigned int	dot11nCoexist_ap_chk;
	...
	unsigned int	phydm_ability;
	unsigned int	halrf_ability;
};
...
struct MeshCSA {
	unsigned char	mesh_csa_enable;
	unsigned int	mesh_csa_blocking;
};
...
typedef struct __wapiMibInfo {
	...
	unsigned int			wapiUpdateUCastKeyTimeout;
	unsigned int			wapiUpdateUCastKeyPktNum;
	wapiMibKeyUpdateType	wapiUpdateMCastKeyType;
	unsigned int			wapiUpdateMCastKeyTimeout;
	unsigned int			wapiUpdateMCastKeyPktNum;
...
struct HotSpotConfigEntry {
	...
	/* 20180210*/
	// reuse rsn ie, let OSEN ie save in rsnie
	//unsigned char	osen_enable;
	unsigned char	osen_ie[64];
	unsigned char	osen_ie_len;
};
...
struct StaControl {
	...
	unsigned char stactrl_param_10;
	unsigned char stactrl_functions;  // BIT(0)-block5G feature, BIT(1)-hiddenAP feature

//#ifdef STA_CONTROL_BAND_TRANSITION
    /* Take 5G as preferband, 2G as non-preferband for example */
    unsigned char stactrl_band_transit;   // 0: disable, 1: enable
    unsigned char stactrl_rssi_th_high;   // higher-bound rssi threshold (for steering 2G to 5G)
    unsigned char stactrl_rssi_th_low;    // lower-bound rssi threshold (for steering 5G to 2G)
    unsigned char stactrl_ch_util_th;     // channel utilization threshold
    unsigned int  stactrl_steer_detect;   // the minimal time (sec) that STA keep satisfying the transition requirement
    unsigned int  stactrl_sta_load_th_2g; // max allowed load (Tx + Rx) that a STA can be steered to 5G (kbps)

	/* the following are load balance band transition mibs */
	unsigned char  stactrl_load_balance;  // 0: disable, >0: the preferband blocking time for load balance
    int            stactrl_ap_rate_weight;
    int            stactrl_sta_rate_weight;
    int            stactrl_sta_num_weight;
    int            stactrl_clm_weight;
    int            stactrl_score_upper;
	int            stactrl_score_lower;
//#endif  // STA_CONTROL_BAND_TRANSITION
};

struct MultiBand {
	unsigned char mbo_rssi_threshold;		// rssi threshold for Association Disallow
	unsigned char mbo_max_sta_num;			// maximum station number allowed
	unsigned char mbo_ch_util_threshold;	// channel utilization threshold for Association Disallow
};
...
struct RoamingPolicyEntry {
	unsigned char	roaming_enable;  //0:disable, 1:enable
	unsigned int	roaming_st_time; //time after STA association to start roaming check in secs
	unsigned char	roaming_rssi_th1; //rssi threshold1 (0~100)
	unsigned char	roaming_rssi_th2; //rssi threshold2 (0~100)
	unsigned int	roaming_wait_time; //in sec
};
...
struct SmartRoamingProfile {
	...
	unsigned char	ap_rate_max;
	int				ap_stanum_max;
	unsigned char	speed_weight;
	unsigned char	signal_weight;
	unsigned char	load_weight;
	unsigned char	ap_rate_weight;
	unsigned char	ap_stanum_weight;
	unsigned char	btm_fail_thres;
	unsigned char	enable;
	unsigned char	sr_guest_access;
	unsigned char	csa_metric_thres;
	int				csa_ready_timeout;
};
...
struct MultiAP {
	...
	unsigned char	multiap_steering_rcpi_threshold;
	unsigned char	multiap_max_device_reached;
	unsigned char	multiap_change_channel;
	unsigned char	multiap_dfs_debug_print;
	unsigned char	multiap_dfs_ap_mib_channel;
	unsigned char	multiap_csa_heard;
	unsigned char	multiap_assoc_disallowed_status;
	...
	unsigned char	multiap_association_check;
};
...
struct Dot11WNMEntry {
	...
	unsigned char dot11vRetryDelay;
	unsigned char dot11vBssTermination;
	unsigned char dot11vBssTerminationTimer;
	unsigned char dot11vBssTerminationDuration;
	unsigned char sta_mac[MACADDRLEN];
...
struct Dot11RFDPKEntry {
	...
	unsigned int	lut_2g_even_c[3][64];
	unsigned int	lut_2g_odd_c[3][64];
	unsigned int	lut_2g_even_d[3][64];
	unsigned int	lut_2g_odd_d[3][64];
	...
	unsigned int	lut_5g_even_a[9][64];
	unsigned int	lut_5g_odd_a[9][64];
	unsigned int	lut_5g_even_b[9][64];
	unsigned int	lut_5g_odd_b[9][64];

	unsigned char 	is_5g_pdk_pathc_ok;
	unsigned char 	is_5g_pdk_pathd_ok;
	unsigned char 	pwsf_5g_c[9];
	unsigned char 	pwsf_5g_d[9];
	unsigned int	lut_5g_even_c[9][64];
	unsigned int	lut_5g_odd_c[9][64];
	unsigned int	lut_5g_even_d[9][64];
	unsigned int	lut_5g_odd_d[9][64];
#endif
};

struct TdmaControl{
	unsigned char	tdma_enable;
	unsigned int	tx_duration;
	unsigned char	tdma_settimer;
	unsigned int	hwpolling_duration;
	unsigned int	enque_max;
	unsigned int	idle_thd;
};

struct Dot11VariableIEs {
...
struct wifi_mib {
	...
	struct Dot11SuppReguClassesIE	dot11SuppReguClassesIE;
	...
	struct MultiBand				multiBand;	// add for MBO
	struct CrossBand				crossBand; //add for repeater crossband
	struct RoamingPolicyEntry		roamingEntry;	// add for sta roaming
	struct SmartRoamingProfile		sr_profile;	//add for smart roaming
	struct MeshPathsel				meshPathsel;
	struct RtlLinkRoamingProfile    rlr_profile; //add for andlink
	struct MultiAP                  multi_ap;	//add for Multi AP
	struct Dot11WNMEntry 		    wnmEntry;	
	struct Dot11RFDPKEntry			dot11RFDPKEntry;
	struct TdmaControl				tdmaControl;	
};
