...
/* Runtime rate conversion is used by SME/mesh too; it must not live in procfs. */
int query_vht_rate(struct stat_info *pstat)
{
	...
	if (is_MCS_rate(txrate)) {
		...
		if (is_VHT_rate(txrate)) {
			txrate = VHT_MCS_DATA_RATE[MIN_NUM(pstat->tx_bw, 3)][sg]
				[txrate - VHT_RATE_ID];
		} else {
			char index = txrate & 0xf;

			txrate = VHT_MCS_DATA_RATE[MIN_NUM(pstat->tx_bw, 1)][sg]
				[(index < 8) ? index : (index + 2)];
		}
	}
...
struct pid *_wlanapp_pid = NULL;
struct pid *_wlanwapi_pid = NULL;
#ifdef CONFIG_IEEE80211R
struct pid *_wlanft_pid = NULL;
#endif
#if defined(GENERAL_EVENT)
struct pid *_wlangeneral_pid = NULL;
#endif
#endif
...
unsigned char wfa_oui_11k_ap_report[] = {0x00, 0x50, 0xf2 , 0x08};
...
unsigned char RSN_OUI2[] = {0x00, 0x0F, 0xAC};

/*  HS2 */
/*IE Element ID*/
unsigned char  _WPA_OUI_TYPE_[]=			{0x00, 0x50, 0xf2, 0x1};
unsigned char HS2_OUI[] = 					{0x50, 0x6F, 0x9A, 0x10};
unsigned char OSEN_OUI[] = 					{0x50, 0x6F, 0x9A, 0x12};

/*Cipher suite*/
unsigned char OSEN_GROUP_CIPHER_SUITE[] = 	{0x00, 0x0F, 0xAC, 0x07};
unsigned char WPA_TKIP_CIPHER_SUITE[] = 	{0x00, 0x0f, 0xac, 0x2};
unsigned char WPA_CCMP_CIPHER_SUITE[] = 	{0x00, 0x0f, 0xac, 0x4};
/*AKM suite list*/
unsigned char OSEN_AKM_SUITE[] = 			{0x50, 0x6f, 0x9a, 0x01};
unsigned char PSK_SHA256_AKM_SUITE[] = 		{0x00, 0x0F, 0xAC, 0x06};
unsigned char PSK_WPA3_AKM_SUITE[] = 		{0x00, 0x0F, 0xAC, 0x08};
/*HAPD_OWE*/
unsigned char OWE_AKM_SUITE[] = 		{0x00, 0x0F, 0xAC, 0x12};
...
// SMPS 4.2.28
		...
		if ((priv->pmib->dot11BssType.net_work_type & WIRELESS_11N) && pstat->ht_cap_len && (!should_restrict_Nrate(priv, pstat))) {
			...
			if (pstat->ht_cap_buf.support_mcs[3]) // no support RX 4ss
				sta_mimo_mode = RF_4T4R;
			else if (pstat->ht_cap_buf.support_mcs[2]) // no support RX 3ss
				sta_mimo_mode = RF_3T3R;
			else if (pstat->ht_cap_buf.support_mcs[1]) // no support RX 2ss
				sta_mimo_mode = RF_2T2R;
			else
				sta_mimo_mode = RF_1T1R;
...
// SMPS 4.2.28
		...
		pstat->mimo_type_bk = rf_mimo_mode;
#endif
	...
	if (priv->pshare->WlanSupportAbility & WLAN_BEAMFORMING_SUPPORT){
		if(priv->pmib->dot11RFEntry.txbf == 1) 
		{
			if(pstat->ht_cap_len && (pstat->ht_cap_buf.txbf_cap))
			   	pstat->cmn_info.bf_info.ht_beamform_cap |= BEAMFORMING_HT_BEAMFORMEE_ENABLE;
			
		#ifdef RTK_AC_SUPPORT		
			if (pstat->vht_cap_len && (cpu_to_le32(pstat->vht_cap_buf.vht_cap_info) & (BIT(SU_BFEE_S))))
				pstat->cmn_info.bf_info.vht_beamform_cap |= BEAMFORMING_VHT_BEAMFORMEE_ENABLE;
			if (pstat->vht_cap_len && (cpu_to_le32(pstat->vht_cap_buf.vht_cap_info) & (BIT(SU_BFER_S))))
				pstat->cmn_info.bf_info.vht_beamform_cap |= BEAMFORMING_VHT_BEAMFORMER_ENABLE;
		#endif
...
#ifdef DOT11K
	if (pstat->rm_timer.data == 0) {	// don't init_timer twice, may need refined
		mw5_legacy_timer_init(&pstat->rm_timer);
	...
	memset(pstat->rc_entry, 0, sizeof(pstat->rc_entry));
		...
		//if(priv->pshare->rf_ft_var.atm_en){
			...
			for(i=BK_QUEUE;i<HIGH_QUEUE;i++) {	
				skb_queue_head_init(&pstat->tdma_swq.swq_queue[i]);
			}
...
#ifndef  RTK_NL80211
		mw5_legacy_timer_init(&pstat->SA_timer);
...
//		wapiAssert(pstat->wapiInfo!=NULL);
	...
	skb_queue_head_init(&pstat->SBWC_txq);
	skb_queue_head_init(&pstat->SBWC_rxq);
	...
	pstat->sta_bwc_tx_cnt = 0;
	#if !defined(RTK_STA_BWC_DIRECT_XMIT)
	pstat->sta_bwc_txq.head = 0;
	pstat->sta_bwc_txq.tail = 0;
	pstat->sta_bwc_consuming_q = FALSE;
	#endif
#endif
	...
	if(priv->pshare->rf_ft_var.manual_pe_enable){
		//add STA
		//pe_stainfo_add(priv, pstat);
		//updtae stainfo
		#if defined(PE_INFO_SYNC)
		//pe_stainfo_update(priv, pstat);
		#endif
	}
#endif
...
void free_sta_tx_skb_pci(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
    ...
    if (IS_SUPPORT_AP_SWPS_OFFLOAD(priv))
		...
		if(priv->pshare->rf_ft_var.atm_en) {
			for(i = BK_QUEUE; i < HIGH_QUEUE; i++) {
				while (skb_queue_len(&pstat->tdma_swq.swq_queue[i])) {
					pskb = skb_dequeue(&pstat->tdma_swq.swq_queue[i]);
					rtl_kfree_skb(priv, pskb, _SKB_TX_);
				}
			}
		}
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE
#if defined(CONFIG_PE_ENABLE)		
		&& !priv->pshare->rf_ft_var.manual_pe_enable
#endif		
		) {
	    for(i = BK_QUEUE; i < HIGH_QUEUE; i++) {
...
#ifdef SBWC
	while (skb_queue_len(&pstat->SBWC_txq)) {
        pskb = skb_dequeue(&pstat->SBWC_txq);
        rtl_kfree_skb(priv, pskb, _SKB_TX_);
    }

    while (skb_queue_len(&pstat->SBWC_rxq)) {
        pskb = skb_dequeue(&pstat->SBWC_rxq);
        rtl_kfree_skb(priv, pskb, _SKB_RX_);
    }
#endif

#if defined(RTK_STA_BWC) && !defined(RTK_STA_BWC_DIRECT_XMIT)
	SMP_LOCK_STA_BWC(flags);
	while (CIRC_CNT(pstat->sta_bwc_txq.head, pstat->sta_bwc_txq.tail, NUM_TXPKT_QUEUE))
	{
		struct sk_buff *pskb = pstat->sta_bwc_txq.pSkb[pstat->sta_bwc_txq.tail];
		rtl_kfree_skb(priv, pskb, _SKB_TX_);
		pstat->sta_bwc_txq.tail++;
		pstat->sta_bwc_txq.tail = pstat->sta_bwc_txq.tail & (NUM_TXPKT_QUEUE - 1);
	}
...
void release_stainfo(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if(pstat->rm.beacon_report){
		kfree(pstat->rm.beacon_report);
		REDUCEMEM("kfree beacon_report\n");
	}
#endif
...
#ifdef CONFIG_IEEE80211W
#ifndef  RTK_NL80211
	if (mw5_legacy_timer_pending(&pstat->SA_timer))
	    mw5_legacy_del_timer(&pstat->SA_timer);
#endif
#endif
	...
	/* v44.32: MW5 deliberately compiles the vendor mesh code but does not
	 * create the legacy mesh netdev. release_stainfo() is also used for
	 * normal cfg80211 stations, so netif_running(NULL) caused the repeatable
	 * BadVA 0x24 panic at release_stainfo+0x30c. */
	root_priv = GET_ROOT(priv);
	if (root_priv->mesh_dev && netif_running(root_priv->mesh_dev)) {
		if (isMeshPoint(pstat)) {
			clear_route_info(root_priv, pstat->cmn_info.mac_addr);
		} else {
			remove_proxy_entry(root_priv, NULL, pstat->cmn_info.mac_addr);
		}
	}
#endif
    ...
    plist = phead->next;
    while (phead && plist && (plist != phead)) {
        prc_timer = list_entry(plist, struct reorder_ctrl_timer, list);
        plist = plist->next;
        
        if (prc_timer && prc_timer->pstat == pstat) {
	...
	for (i = 0; i < TCP_SESSION_MAX_ENTRY ; i++) {
		INIT_LIST_HEAD(&pstat->tcp_ses_active_list[i]);
		if (pstat->tcp_ses[i].buf_ack)
			rtl_kfree_skb(priv, pstat->tcp_ses[i].buf_ack, _SKB_TX_);
	}
	memset(pstat->tcp_ses, 0, sizeof(pstat->tcp_ses));
#endif
    SMP_UNLOCK_XMIT(flags);
    ...
    if (mw5_legacy_timer_pending(&pstat->rm_timer))
        mw5_legacy_del_timer_sync(&pstat->rm_timer);
...
#ifdef INCLUDE_WPA_PSK
	/* v44.30: release_stainfo can run from an IRQ before WPA state has been
	 * allocated.  v44.28 hardware hit BadVA 0x24 at this dereference. */
	if (pstat->wpa_sta_info &&
	    mw5_legacy_timer_pending(&pstat->wpa_sta_info->resendTimer)) {
		mw5_legacy_del_timer(&pstat->wpa_sta_info->resendTimer);
	}
#endif
...
#if 1
		pstat->cmn_info.dm_ctrl &= ~STA_DM_CTRL_ACTIVE;
		phydm_cmn_sta_info_hook(ODMPTR, pstat->cmn_info.mac_id, NULL);
#else
...
#if 1
#if defined(CONFIG_RTW_BRSC) || defined(CONFIG_RTL_EXT_PORT_SUPPORT)
	rtw_flush_brsc_info(NULL, pstat->cmn_info.mac_addr);
#endif
...
int del_station(struct rtl8192cd_priv *priv, struct stat_info *pstat, int send_disconnect, int reason)
{
#ifndef SMP_SYNC
	unsigned long flags;
#endif
	...
	if (asoc_list_del(priv, pstat))
	{
		...
		if (send_disconnect == DEL_STA_SEND_DEAUTH)
			issue_deauth(priv, pstat->cmn_info.mac_addr, reason);
		else if (send_disconnect == DEL_STA_SEND_DISASSOC)
			issue_disassoc(priv, pstat->cmn_info.mac_addr, reason);
		...
		add_sta_assoc_status(priv, pstat->cmn_info.mac_addr, pstat->rssi, reason | 0x300);
#endif
	}
...
#if defined(CONFIG_PE_ENABLE)
struct stat_info* get_stainfo_from_aidarray(struct rtl8192cd_priv *priv, unsigned short aid)
{
	int i;
	for(i= 0; i<NUM_STAT; i++) {
		if (priv->pshare->aidarray[i] && priv->pshare->aidarray[i]->used == TRUE
			&& priv->pshare->aidarray[i]->station.cmn_info.aid == aid) {
...
unsigned short get_80211frameVlanID(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	UINT8	*pframe 		= get_pframe(pfrinfo);
	INT 	payload_offset 	= get_hdrlen(priv, pframe);
...
#endif
		if (((privacy == _WEP_40_PRIVACY_) || (privacy == _WEP_104_PRIVACY_))) {
			payload_offset += 4;
		} else if ((privacy == _TKIP_PRIVACY_)) {
			payload_offset += 8;
	...
	if ((e_llc->dsap == 0xaa) && (e_llc->ssap == 0xaa) && (e_llc->ctl == 0x03)) {
		if (!memcmp(e_snap->oui, oui_rfc1042, WLAN_IEEE_OUI_LEN)) {
			if(e_snap->type == __constant_htons( ETH_P_8021Q)) {
				unsigned short *vlan = (unsigned short *)((unsigned long)e_snap + sizeof(struct wlan_snap_t));
				return ntohs(*vlan) & 0x0fff; 
			}
		} 
	}
	return 0;
	...
	if ((wlan_pkt_format == WLAN_PKT_FORMAT_SNAP_RFC1042) ||
		...
		if (prxsc_entry) {
			if (!(
				...
				((e_hdr->type == htons(ETH_P_8021Q)) && (*((UINT16 *)((UINT8 *)skb->data + WLAN_ETHADDR_LEN*2 + 4)) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))) ||
#endif
				(e_hdr->type == htons(0x888e))) && // for WIFI_SIMPLE_CONFIG
#ifdef CONFIG_RTL_WAPI_SUPPORT
...
#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8192FE) || defined(CONFIG_WLAN_HAL_8197F)
	/* RTL8197F/RTL8197FS is a two-RF-path device.  The recovered MW5 PHY
	 * source uses only path A/B; returning zero here made SwChnl() fall back
	 * to RF_PATH_MAX (4) during initial channel programming. */
	if(GET_CHIP_VER(priv) == VERSION_8192E || GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8812E || GET_CHIP_VER(priv) == VERSION_8192F || GET_CHIP_VER(priv) == VERSION_8197F)
		return 2;
	else
#endif
...
#ifdef DFS	
	if((priv->pmib->dot11DFSEntry.disable_DFS == 1) && (!priv->pshare->rf_ft_var.mp_specific)) {
...
void cnt_assoc_num(struct rtl8192cd_priv *priv, struct stat_info *pstat, int act, char *func)
{
	...
	if (act == INCREASE) {
		if (priv->assoc_num <= NUM_STAT) {
			...
			if(priv->pshare->veriwave_sta_num == 0 && priv->assoc_num == 5)
			{
				int y, z=0;
				for(y= 1; y<5; y++) {
					if (priv->pshare->aidarray[y] && priv->pshare->aidarray[y]->used == TRUE &&
						priv->pshare->aidarray[y-1] && priv->pshare->aidarray[y-1]->used == TRUE) {
						struct stat_info	*p1 = &(priv->pshare->aidarray[y]->station);
						struct stat_info	*p2 = &(priv->pshare->aidarray[y-1]->station);
						if(p1->cmn_info.mac_addr[5] == p2->cmn_info.mac_addr[5]+1)
							z++;
					}
				}
				if(z==4) {
					priv->pshare->rf_ft_var.veriwave_test =1;
					priv->pshare->veriwave_sta_num =4;
					for(y= 0; y<5; y++) {
						priv->pshare->aidarray[y]->station.IOTPeer = HT_IOT_PEER_VERIWAVE;
					}
				}
			}
			if ((pstat->IOTPeer == HT_IOT_PEER_VERIWAVE) || priv->pshare->rf_ft_var.veriwave_test)
			{
				if ((priv->pshare->veriwave_sta_num == 0  && priv->assoc_num == 1) 
					|| (priv->pshare->veriwave_sta_num == 4	 && priv->assoc_num == 5) )
				{
					...
					priv->pmib->dot11OperationEntry.block_relay=1;
					priv->pmib->reorderCtrlEntry.ReorderCtrlEnable=0;
				}
			...
			if (priv->pshare->total_assoc_num > 10 && !is_veriwave_testing(priv)) { // for veriwave QOE test
			...
			#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
				if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
					notify_update_sta_msr(priv, pstat, INCREASE | 0x80);
				}
			#endif
                }
            }
	...
	if(priv->pshare->rf_ft_var.manual_pe_enable){
		...
		if (OPMODE & WIFI_AP_STATE) {
			if(pstat) {
				rtl_wifi_fdb_add_hook(pstat->cmn_info.mac_addr);
			}
		}
#endif
		...
		if (priv->assoc_num > 0) {
			...
			if (pstat->IOTPeer == HT_IOT_PEER_VERIWAVE)
			{
				...
				if (priv->pshare->veriwave_sta_num == 0) {
					priv->pshare->rf_ft_var.mustAmsdu = 0;
					priv->pmib->dot11OperationEntry.block_relay = 0;
					priv->pmib->reorderCtrlEntry.ReorderCtrlEnable = 1;
				}
			}
#endif
	...
	if(priv->pshare->rf_ft_var.manual_pe_enable){
		...
		if (OPMODE & WIFI_AP_STATE) {
			if(pstat) {
				rtl_wifi_fdb_del_hook(pstat->cmn_info.mac_addr);
			}
		}
#endif
} else {
	...
	if ((priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) && priv->pshare->rf_ft_var.adaptivity_enable == 1) {
		if (!priv->pshare->idle_period_test && (priv->pshare->total_assoc_num == 1) && (pstat->vht_cap_len==0)) {
			priv->pshare->idle_period_test = 1;
			priv->pmib->miscEntry.auto_rts_mode = 2;
			priv->pshare->idle_period_reg_bak[0] = RTL_R8(0x510);
			priv->pshare->idle_period_reg_bak[1] = RTL_R16(0x4ca);
			RTL_W8(0x510, 0x1c);
			RTL_W16(0x4ca, 0x2020);
		} else if (priv->pshare->idle_period_test && (priv->pshare->total_assoc_num > 1 || pstat->vht_cap_len)) {
			priv->pshare->idle_period_test = 0;
			priv->pmib->miscEntry.auto_rts_mode = 1;
			RTL_W8(0x510, priv->pshare->idle_period_reg_bak[0]);
			RTL_W16(0x4ca, priv->pshare->idle_period_reg_bak[1]);
		}
...
void rtl8192cd_sta_bwc_timer(unsigned long task_priv)
{
	...
	int i;	
#endif
#if defined(RTK_STA_BWC_DIRECT_XMIT) && defined(RTK_STA_BWC_LEVEL_AVG)
	unsigned long	flags;
	int new_limit = 0, new_limit_bytes = 0;
#endif
	if (!IS_DRV_OPEN(priv))
		return;
	...
	if (priv->pshare->rf_ft_var.txduty && priv->assoc_num) {
		if (priv->pshare->rf_ft_var.total_tp) {
			new_limit = (priv->pshare->rf_ft_var.total_tp/10)*(10-(priv->pshare->rf_ft_var.txduty_level-1));
			new_limit = new_limit/priv->assoc_num;
			new_limit_bytes = (new_limit << 7) / (1000 / priv->pshare->rf_ft_var.sta_bwc_to); //kbits to bytes

			if (priv->pshare->rf_ft_var.dbg & DBG_OUT_KERNEL2) {
				if (net_ratelimit()) {
					therctl_print2("total tp %dkbps, per sta tp(lv%d) %dkbps\n", priv->pshare->rf_ft_var.total_tp, priv->pshare->rf_ft_var.txduty_level, new_limit);
				}
			}
		}
	}
	#endif /* defined(RTK_STA_BWC_LEVEL_AVG) */
	...
	for(i=0; i<NUM_STAT; i++) {
		if (priv->pshare->aidarray[i] && (priv->pshare->aidarray[i]->used == TRUE)){
			...
			if (new_limit) {
			#if defined(THER_DBG)
				if (pstat->sta_bwc_tx_limit == 0) printk("==> %s:%d, assoc_num=%d, new_limit_bytes=%d\n", __func__, __LINE__, priv->assoc_num, new_limit_bytes);
			#endif
				pstat->sta_bwc_tx_limit = new_limit;
				pstat->sta_bwc_tx_limit_byte = new_limit_bytes;
			}
		#endif /* defined(RTK_STA_BWC_LEVEL_AVG) */
		}
	}
	...
	while (!list_empty(plist) && (plist = asoc_list_get_next(priv, plist)) != phead)
	{
		pstat = list_entry(plist, struct stat_info, asoc_list);

		if (pstat->sta_bwc_tx_limit == 0)
			continue;
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE)
			SMP_UNLOCK_ASOC_LIST(flags);

		SMP_LOCK_STA_BWC(flags2);
		pstat->sta_bwc_consuming_q = 1;

		// clear bandwidth control counter
		pstat->sta_bwc_tx_cnt = 0;
		SMP_UNLOCK_STA_BWC(flags2);

		// consume Tx queue
		while(1)
		{
			SMP_LOCK_STA_BWC(flags2);
			pskb = (struct sk_buff *)deque(priv, &(pstat->sta_bwc_txq.head), &(pstat->sta_bwc_txq.tail),
				(unsigned long)(pstat->sta_bwc_txq.pSkb), NUM_TXPKT_QUEUE);
			SMP_UNLOCK_STA_BWC(flags2);
...
{
#if (NUM_STAT < FW_NUM_STAT)
		pstat->cmn_info.ra_info.disable_ra = 0;
		priv->pshare->fw_free_space --;
		pstat->cmn_info.mac_id = pstat->cmn_info.aid;
#else
	int array_idx;
    if (!pstat->cmn_info.ra_info.disable_ra)
...
//					panic_printk("%s, Free %d pkts of MAC ID:%d\n", __FUNCTION__, Q_pkt[minIdx], Q_aid[minIdx]);
	...
	if(priv->pshare->rf_ft_var.msta_refine&1 ) 
		mw5_legacy_mod_timer(&priv->pshare->PktAging_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(10));	
}
#endif
...
void add_ps_timer(unsigned long task_priv)
{
	...
	if (set_timer)
		mw5_legacy_mod_timer(&priv->add_ps_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(50));	// 50 ms
}
...
//#endif		
	if (is_h2c_buf_occupy(priv)) {
		if (list_empty(&pstat->addps_list)) {
			...
			if (!mw5_legacy_timer_pending(&priv->add_ps_timer))
				mw5_legacy_mod_timer(&priv->add_ps_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(50));	// 50 ms
		}
	} else {
		set_ps_cmd(priv, pstat);
...
void checkBandwidth(struct rtl8192cd_priv *priv)
{
    ...
    unsigned char curr_dig = (RTL_R8(0xc50) & 0x7f);
    ...
    if (mw5_legacy_timer_pending(&priv->ss_timer) || priv->ss_req_ongoing)
        return;
    ...
    if(IS_OUTSRC_CHIP(priv)){
        FA_counter = ODMPTR->false_alm_cnt.cnt_all;
			...
			if (ODMPTR->original_dig_restore == 0) 
				curr_dig = ODMPTR->dm_dig_table.cur_ig_value_tdma;
			else
	#endif
				curr_dig = ODMPTR->dm_dig_table.cur_ig_value;
    	}	
#endif
	...
	//check assoc num
	assoc_num = priv->pshare->total_assoc_num;
    ...
    if( priv->pmib->dot11nConfigEntry.dot11nCoexist) {
...
#if defined(CONFIG_PCI_HCI)
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
        	if((FA_counter> priv->pshare->rf_ft_var.bws_Thd)&&(curr_dig >= 0x32)) {
				run = 1;
			}
		}
#endif
        ...
        u1Byte IGI = RTL_R8(0xc50) & 0x7f;
        	if (((FA_counter> priv->pshare->rf_ft_var.bws_Thd) && (curr_dig >= 0x32)) || (curr_dig >= 0x42)) {
				run = 1;
			}
		}
#endif
...
void add_RATid_timer(unsigned long task_priv)
{
	...
	if (set_timer)
		mw5_legacy_mod_timer(&priv->add_RATid_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(50));	// 50 ms
}
...
void ap_offload_deactivate(struct rtl8192cd_priv *priv, int reason)
{
	...
	if (!priv->pshare->offload_prohibited) {
		...
		mw5_legacy_del_timer(&priv->pshare->ps_timer);
				...
				unsigned int size, int direction)
{
	...
	if (priv && priv->pshare && priv->pshare->pdev)
		dma_sync_single_for_device(&priv->pshare->pdev->dev,
...
void restore_backup_sta(struct rtl8192cd_priv *priv, void *pInfo)
{
	...
	int key_combo_len = sizeof(key_combo);
		...
		if (pBackup->sta[i]) {
			...
			if (!SWCRYPTO && pstat->dot11KeyMapping.keyInCam == TRUE) {
				...
				if (pstat->dot11KeyMapping.dot11Privacy) {
					...
					key1_len = pstat->dot11KeyMapping.dot11EncryptKey.dot11TTKeyLen;
					if( key1_len>16) key1_len = 16;

					key2_len = pstat->dot11KeyMapping.dot11EncryptKey.dot11TMicKeyLen;
					if( key2_len > 16 ) key2_len = 16;

					if ( key1_len && key_combo_len >= key1_len){
						memcpy(key_combo,pstat->dot11KeyMapping.dot11EncryptKey.dot11TTKey.skey,key1_len);
						key_combo_len -= key1_len;
					}
					if (key2_len && key_combo_len >= key2_len){
						memcpy(&key_combo[key1_len],pstat->dot11KeyMapping.dot11EncryptKey.dot11TMicKey1.skey,key2_len);
					}
...
//					priv->pmib->dot11GroupKeysTable.keyInCam = TRUE;
...
#endif // CONFIG_RTL_HW_WAPI_SUPPORT
		{
			...
			key1_len = priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TTKeyLen;
			if( key1_len > 16) key1_len = 16;

			key2_len = priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TMicKeyLen;
			if(key2_len > 16) key2_len = 16;

			if (key1_len && key_combo_len >= key1_len){
				memcpy(key_combo,priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TTKey.skey,key1_len);
				key_combo_len -= key1_len;
			}
			if (key2_len && key_combo_len >= key2_len){
				memcpy(&key_combo[key1_len],priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TMicKey1.skey,key2_len);
			}
...
#if defined(CONFIG_RTL8672) && defined(WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD)
__IRAM_IN_865X
        ...
        for (j = 0; j < 2; j++) {
            ...
            if (rpt1.macid) { 
                ...
                if (GET_CHIP_VER(priv) == VERSION_8822B &&
                    ...
                    rpt1.macid == priv->pshare->linux6_eapol_last_macid) {
                    priv->pshare->linux6_eapol_txrpt_reports++;
                    priv->pshare->linux6_eapol_txrpt_txok_sum += rpt1.txok;
                    priv->pshare->linux6_eapol_txrpt_txfail_sum += rpt1.txfail;
                    priv->pshare->linux6_eapol_txrpt_last_macid = rpt1.macid;
                    priv->pshare->linux6_eapol_txrpt_last_txok = rpt1.txok;
                    priv->pshare->linux6_eapol_txrpt_last_txfail = rpt1.txfail;
                    priv->pshare->linux6_eapol_txrpt_last_rate = rpt1.initil_tx_rate;
                    if (priv->pshare->linux6_eapol_txrpt_reports <= 32)
                        printk("rtl8197f-wlan: v44.50 EAPOL TXRPT if=%s macid=%u txok=%u txfail=%u initial_rate=%u reports=%llu sum=%llu/%llu\n",
                            priv->dev ? priv->dev->name : "?", rpt1.macid, rpt1.txok, rpt1.txfail,
                            rpt1.initil_tx_rate,
                            (unsigned long long)priv->pshare->linux6_eapol_txrpt_reports,
                            (unsigned long long)priv->pshare->linux6_eapol_txrpt_txok_sum,
                            (unsigned long long)priv->pshare->linux6_eapol_txrpt_txfail_sum);
                }
#endif
...
void _txrpt_handler(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct tx_rpt *report)
{
	...
	pstat->txrpt_tx_bytes_array[pstat->txrpt_tx_bytes_array_idx] = pstat->tx_bytes;
	pstat->txrpt_tx_bytes_array_idx = (pstat->txrpt_tx_bytes_array_idx + 1) % TXRPT_TX_BTYES_RANGE;

	// Compare with the oldest tx bytes record to ensure the real tx of the wlan driver
	if (pstat->txrpt_tx_bytes_array[pstat->txrpt_tx_bytes_array_idx] != pstat->tx_bytes) {
...
void rtl8192cd_cu_stop(struct rtl8192cd_priv *priv) {
    ...
    if(stop_cu) {
        if (mw5_legacy_timer_pending(&priv->pshare->cu_info.cu_cntdwn_timer)) {
            mw5_legacy_del_timer_sync(&priv->pshare->cu_info.cu_cntdwn_timer);
        }
...
void rtl8192cd_cu_start(struct rtl8192cd_priv *priv) {
    ...
    if(priv->pshare->cu_info.cu_enable == 0) {
        ...
        mw5_legacy_timer_init(&priv->pshare->cu_info.cu_cntdwn_timer);
#if defined(CONFIG_PCI_HCI)
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
...
#endif
        mw5_legacy_mod_timer(&priv->pshare->cu_info.cu_cntdwn_timer, jiffies + CU_TO);
...
void rtl8192cd_cu_cntdwn_timer(unsigned long task_priv)
{
    ...
    {
        mw5_legacy_mod_timer(&priv->pshare->cu_info.cu_cntdwn_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(10));	
    }
    else
    {
...
#if (defined(RTK_ATM)|| defined(RTK_TDMA_ATM)) && !defined(HS2_SUPPORT)
void staip_snooping_byarp(struct sk_buff *pskb, struct stat_info *pstat)
{
	struct arphdr *arp = (struct arphdr *) rtl_get_skb_ip_header(pskb, 0);
...
{
	struct ip6_hdr *hdr = (struct ip6_hdr *)rtl_get_skb_ip_header(pskb, 0);
	...
	if (pkt_len>0)
    {
		icmphdr = (struct icmp6hdr *)((unsigned long)hdr + sizeof(*hdr));
...
{
	struct ipv6hdr *hdr = (struct ipv6hdr *) rtl_get_skb_ip_header(pskb, 0);
	struct icmp6hdr *icmphdr;
	int pkt_len, type;
	int i;
...
struct dhcpMessage {
	...
	iph = (struct iphdr *)rtl_get_skb_ip_header(pskb, 0);
...
{
#ifdef __ECOS
	struct ip6_hdr *hdr = (struct ip6_hdr *)rtl_get_skb_ip_header(skb, 0);
#else
	struct ipv6hdr *hdr = (struct ipv6hdr *)rtl_get_skb_ip_header(skb, 0);
#endif
	...
	if (pkt_len>0)
    {
		icmpv6 = (struct icmp6hdr *)((unsigned long)hdr + sizeof(*hdr));
...
int proxy_icmpv6_ndisc(struct rtl8192cd_priv *priv, struct sk_buff *skb)
{
...
#else
	struct ipv6hdr *hdr = (struct ipv6hdr *)rtl_get_skb_ip_header(skb, 0); // ipv6hdr len = 40 bytes
	...
	if (pkt_len>0)
	{
		icmpv6_nsolic = (struct icmp6hdr *)((unsigned long)hdr + sizeof(*hdr));
		    ...
		    while (!list_empty(plist) && (plist = plist->next) != phead)
			{
		        ...
		        if (foundTarget)
			    {
					...
					else
					{
...
#ifdef __ECOS
						replyhdr = (struct ip6_hdr *) rtl_get_skb_ip_header(newskb, 0);
						ipv6_addr_copy(&replyhdr->ip6_src, &pstat->sta_v6ip);
						ipv6_addr_copy(&replyhdr->ip6_dst, &hdr->ip6_src);
						icmpv6_nadvt = (struct icmp6hdr *)((unsigned long)replyhdr + sizeof(*hdr));
						...
						#else				
						replyhdr = (struct ipv6hdr *) rtl_get_skb_ip_header(newskb, 0);
						...
						#endif						
						//target ip
						opt = (unsigned char *)((unsigned long)icmpv6_nadvt + sizeof(struct icmp6hdr));
...
{
	struct sk_buff *newskb = NULL;
	struct arphdr *arp = (struct arphdr *)rtl_get_skb_ip_header(skb, 0);
...
#else
		if (ipv4_is_loopback((__be32)ptarget) || ipv4_is_multicast((__be32)ptarget))
#endif
		{
		...
		while (!list_empty(plist) && (plist = plist->next) != phead)
		{
			...
			if (!memcmp(pstat->sta_ip, ptarget, 4))
			{
				...
				else
				{
					...
					//arp response			
					arp = (struct arphdr *) rtl_get_skb_ip_header(newskb, 0);
					...
					if ((pstat = get_stainfo(priv, newskb->data)) != NULL)
					{
					    ...
					    //struct mw5_legacy_timer *ptimer;
...
//			            mw5_legacy_mod_timer(ptimer, jiffies + 1);
		...
		)
{
	...
	#ifdef UNIVERSAL_REPEATER
	struct rtl8192cd_priv *vxdpriv=NULL;
	#endif
	#ifdef MBSSID
	struct rtl8192cd_priv *vappriv=NULL;
	#endif
	struct rtl8192cd_priv *rootpriv=NULL;
	rootpriv = GET_ROOT(priv);
	if (IS_DRV_OPEN(rootpriv))
		totaltp = rootpriv->ext_stats.tx_avarage;
#ifdef UNIVERSAL_REPEATER
	vxdpriv = GET_VXD_PRIV(rootpriv);
	if(IS_DRV_OPEN(vxdpriv))
		totaltp += vxdpriv->ext_stats.tx_avarage;
#endif
#ifdef MBSSID
	for(i=0; i<RTL8192CD_NUM_VWLAN; i++){
		vappriv = rootpriv->pvap_priv[i];
		if(IS_DRV_OPEN(vappriv))
			totaltp += vappriv->ext_stats.tx_avarage;
	}
#endif
		...
		)
{
	...
	rootpriv = GET_ROOT(priv);
	if (IS_DRV_OPEN(rootpriv))
		totaltp = rootpriv->ext_stats.rx_avarage;
#ifdef UNIVERSAL_REPEATER
	vxdpriv = GET_VXD_PRIV(rootpriv);
	if(IS_DRV_OPEN(vxdpriv))
		totaltp += vxdpriv->ext_stats.rx_avarage;
#endif
#ifdef MBSSID
	for(i=0; i<RTL8192CD_NUM_VWLAN; i++){
		vappriv = rootpriv->pvap_priv[i];
		if(IS_DRV_OPEN(vappriv))
			totaltp += vappriv->ext_stats.rx_avarage;
	}
#endif
	return totaltp>>17;
...
void start_monitor_mode(struct rtl8192cd_priv * priv)
{
	...
	if(!priv->pmib->miscEntry.chan_switch_disable) {
		mw5_legacy_timer_init(&priv->chan_switch_timer);
		priv->chan_switch_timer.data = (unsigned long) priv;
		priv->chan_switch_timer.function = rtl8192cd_chan_switch_timer;
		mw5_legacy_mod_timer(&priv->chan_switch_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(priv->pmib->miscEntry.chan_switch_time));
	}
...
void stop_monitor_mode(struct rtl8192cd_priv * priv)
{
	...
	if (mw5_legacy_timer_pending(&priv->chan_switch_timer))
		mw5_legacy_del_timer_sync(&priv->chan_switch_timer);
...
int rtk_wlan_event_indicate(char* ifname, int event, unsigned char* addr, char reason)
{
	...
	if(nl_eventd_sk==NULL)
	{
		_DEBUG_ERR("%s:%d, report wifi link status failed, pid=%d,sk=%p\n",__FUNCTION__,__LINE__, rtk_eventd_pid, nl_eventd_sk);
		return -1;
	}
...
{
	char data[MACADDRLEN+WLAN_INFO_LEN+IFNAMSIZ+2] = {0};
	int data_len = 0;
	int rtk_eventd_pid = get_nl_eventd_pid();
	struct sock* nl_eventd_sk = get_nl_eventd_sk();
	struct net_device	*net_dev;
	...
	if(nl_eventd_sk==NULL)
	{
		DEBUG_ERR("%s:%d, report wifi link status failed, pid=%d,sk=%p\n",__FUNCTION__,__LINE__, rtk_eventd_pid, nl_eventd_sk);
		return -1;
	}
	memcpy(data, addr, MACADDRLEN);
	//data[MACADDRLEN] = reason;
	memcpy(data+MACADDRLEN,info,len);
	memcpy(data+MACADDRLEN+len, ifname, IFNAMSIZ);
	data[MACADDRLEN+len+IFNAMSIZ] = '\0';
	data_len = MACADDRLEN+len+IFNAMSIZ+2;
...
{	
	...
	if ((p == NULL) || (len == 0) || (*(p+2) == '\0') || (len > WLAN_SSID_MAXLEN))	
	{
	...
	if (p != NULL)
		...
		if (p !=  NULL) 
			...
			_DEBUG_ERR("receive beacon with no channel\n");
			
			if(priv->site_survey->count > 0)
			{
				for(i=0;i<priv->site_survey->count;i++)
				{
					if(isEqualMACAddr(priv->site_survey->bss[i].bssid, pBss->bssid))
						pBss->channel = priv->site_survey->bss[i].channel;
				}
			}
			else
			{
	...
	if((p != NULL) && (len <= sizeof(struct ht_cap_elmt))){
		struct ht_cap_elmt tmp_ht_cap;
        memcpy((unsigned char *)&tmp_ht_cap, p+2, len);
		if (tmp_ht_cap.ht_cap_info & cpu_to_le16(_HTCAP_SUPPORT_CH_WDTH_))
		{
			pBss->stage = CHANNEL_WIDTH_40;
		}
		else
		{
			pBss->stage = CHANNEL_WIDTH_20;
		}
	}
	...
	{
		struct vht_oper_elmt	tmp_vht_oper_buf;

		p = get_ie(pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_, EID_VHTOperation, &len,
				pfrinfo->pktlen - WLAN_HDR_A3_LEN - _BEACON_IE_OFFSET_);

		if ((p !=  NULL) && (len <= sizeof(struct vht_oper_elmt))) {
			memcpy((unsigned char *)&tmp_vht_oper_buf, p+2, len);
		}

		if(len){
			if ((priv->vht_oper_buf.vht_oper_info[0] == 1) && (tmp_vht_oper_buf.vht_oper_info[0] == 1)) {
				pBss->stage = CHANNEL_WIDTH_80;
				//printk("=====%s(%d), it is 80M bandwidth\n", __FUNCTION__, __LINE__);
			}
		}
	}
#endif
...
{     
	...
	priv->pshare->ap_neighbor.ap_neighbor_info_ent[index].bandwidth= bss.stage;
...
void adaptivity_stats_check(struct rtl8192cd_priv *priv)
{
	...
	if (priv->pshare->ada_stats_prev != ada_stats) {
		...
		if (ada_stats) {
			...
			mw5_legacy_mod_timer(&priv->pshare->ada_stats_timer, 
				jiffies + RTL_MILISECONDS_TO_JIFFIES(ada_stats_to));
		}
		else {
			if (mw5_legacy_timer_pending(&priv->pshare->ada_stats_timer))
				mw5_legacy_del_timer(&priv->pshare->ada_stats_timer);
		}
		return;
	}
...
{
#if 0
	wpaslog("psk[%d] auth[%d] encr[%d] 11w[%d] sha256[%d]",
...
#ifdef HAPD_11K
int hapd_rm_parse_beacon_report(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int len, int frame_len)
{
    ...
    if(pframe[len + 3] == 0)  /*succeed*/
    {
        ...
        if(beacon_rep)
        {
			...
			printk("[ROAMING] Receive beacon_report:SUCCESS, CH = %d, OP_CLASS = %d\n", beacon_rep->info.channel, beacon_rep->info.op_class);
        }
    ...
    {
		printk("[ROAMING] Receive beacon_report:MEASUREMENT_INCAPABLE\n");
    ...
    {
		printk("[ROAMING] Receive beacon_report:MEASUREMENT_REFUSED\n");
...
void hapd_OnRadioMeasurementReport(struct rtl8192cd_priv *priv,
				   struct stat_info *pstat, unsigned char *pframe, int frame_len)
{
	int len;
	unsigned char element_id;
	unsigned char element_len;
	size_t BCN_RPT_BUF_LEN = 0;
	if (pstat->rm.beacon_report == NULL) {
		BCN_RPT_BUF_LEN = sizeof(struct dot11k_beacon_measurement_report) * MAX_BEACON_REPORT;
		REDUCEMEM("BCN_RPT_BUF_LEN=[%d]\n", BCN_RPT_BUF_LEN);
		pstat->rm.beacon_report =
		    (struct dot11k_beacon_measurement_report *)kmalloc(BCN_RPT_BUF_LEN, GFP_ATOMIC);

		if (pstat->rm.beacon_report == NULL) {
			REDUCEMEM("kmalloc fail\n");
			return;
		}
		memset((void *)pstat->rm.beacon_report,0,BCN_RPT_BUF_LEN);
		REDUCEMEM("kmalloc OK BCN_RPT_BUF_LEN=[%d]\n", BCN_RPT_BUF_LEN);
	}

	/*the report from iphone 6 always has dialog token 0, thus do not check dialog token when the value is 0 */
#if 0
	if ((pframe[2] != 0 && pstat->rm.dialog_token != pframe[2])) {
		printk("[ROAMING] Check dialog token fail!\n");
		return;
	}
#endif
	...
	while (len + 5 <= frame_len) {
		...
		if (element_id == _MEASUREMENT_REPORT_IE_) {
			if (pframe[len + 4] == MEASUREMENT_TYPE_BEACON) {
				len = hapd_rm_parse_beacon_report(priv, pstat, pframe, len, frame_len);
				continue;
			}
		}
		len += 2 + element_len;
...
void hapd_rm_check_mgmt_tx(struct rtl8192cd_priv *priv, const u8 *buf, u32 len)
{
	u8 action;
	const u8 *frame_body = buf + sizeof(struct ieee80211_mgmt_hrd);
	struct stat_info *psta = NULL;
	action = frame_body[1];
	len -= sizeof(struct ieee80211_mgmt_hrd);

	psta = get_stainfo(priv, GetAddr1Ptr(buf));

	if(len >= 10 && frame_body[9] == 5) {
		if(psta) {
			psta->rm.beacon_report_num = 0;
		}
	}
...
#ifdef HAPD_11V
void hapd_on_btm_resp(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int frame_len)
{
	u8 dialog_token = 0;
	u8 status_code = 0;
	u8 bss_termination_delay = 0;
	u8 frlen = 5;
	...
	u8 empty_bssid[6] = {0};
	if (frame_len - frlen)
		bss_transition_response_notify(BSSID, pstat->cmn_info.mac_addr, &pframe[5], status_code);
	else
		bss_transition_response_notify(BSSID, pstat->cmn_info.mac_addr, empty_bssid, status_code);
#endif
}
#endif
	...
	unsigned char *pmkid_cnt)
{
	...
	/*--- Ver---*/
	ver = le16_to_cpu(ie_head_ptr->version);
	...
	if (memcmp(gc_ptr->oui, RSN_OUI2, 3))
		return WPA2IE_E2;
	...
	pc_cnt = le16_to_cpu(pc_ptr->count);
	...
	akm_cnt = le16_to_cpu(akm_ptr->count);
	...
	if (ie_len) {
		...
		pmk_cnt = le16_to_cpu(pmk_ptr->count);

		log("pmk cnt[%d]", pmk_cnt);
		if (ie_len < (2 + pmk_cnt * 16)) {
			return WPA2IE_E522K;	/*truncated */
		}
		if(ie_ptr_pmksa) *ie_ptr_pmksa = ie_ptr;
		if(pmkid_cnt)  *pmkid_cnt = pmk_cnt;
		if(pmk_cnt)
...
#ifdef CONFIG_CROSSBAND_REPEATER_SUPPORT_WIFI5_WIFI6
int rtl_crossband_get_dev_status(struct net_device *dev, struct crossband_dev_status *dev_status)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	struct rtl8192cd_priv *rpriv = NULL;

	if(priv)
	{
		rpriv = GET_ROOT(priv);
		dev_status->crossband_assoc = rpriv->pmib->crossBand.crossband_assoc;
		dev_status->crossband_pathReady = rpriv->pmib->crossBand.crossband_pathReady;
		dev_status->crossband_prefer = rpriv->pmib->crossBand.crossband_prefer;
	}
	else
	{
...
int operate_btm_disallowed_sta(struct rtl8192cd_priv *priv, unsigned char *tmpbuf)
{
        ...
        if (pstat) {
                ...
                client_leave_notify((GET_MIB(priv))->dot11StationConfigEntry.dot11Bssid, pstat->cmn_info.mac_addr);
...
#ifdef CONFIG_MBO
static inline void rtk_put_led32(u8 *a, u32 val)
{
	a[3] = (val >> 24) & 0xff;
	a[2] = (val >> 16) & 0xff;
	a[1] = (val >> 8) & 0xff;
...
int hapd_rm_construct_nb_report(struct rtl8192cd_priv *priv, u8 *pbuf,
											struct nb_rpt_hdr *report,
											u8 disassoc_imminent, u8 preference_sta)
{
	u8 sub_len = 16;
	u8 *pbuf_ptr = pbuf;
	printk("[%s]Insert nb report %pM\n", __FUNCTION__, report->bssid);

	/* id */
	*pbuf_ptr = 52; //_NEIGHBOR_REPORT_IE_
	pbuf_ptr += 1;
	...
	*pbuf_ptr = sub_len;
	...
	/* BSSID */
	memcpy(pbuf_ptr, report->bssid, ETH_ALEN);
	pbuf_ptr += ETH_ALEN;

	rtk_put_led32(pbuf_ptr, cpu_to_le32(report->bss_info));
	pbuf_ptr += 4;
	...
	*pbuf_ptr = 3; //WNM_BTM_CAND_PREF_SUBEID
	pbuf_ptr += 1;

	*pbuf_ptr = 1;
	pbuf_ptr += 1;
	if (disassoc_imminent &&
		(memcmp(report->bssid, priv->pmib->dot11OperationEntry.hwaddr, ETH_ALEN) == 0)) {
		printk("[%s]disassoc_imminent = 1, set preference to 0\n", __FUNCTION__);
		*pbuf_ptr = 0;
	} else {
...
int hapd_rm_construct_self_nb_report(struct rtl8192cd_priv *priv, u8 *pbuf)
{
	struct nb_rpt_hdr report = {0};
	if(OPMODE & WIFI_AP_STATE) {
		memcpy(report.bssid, priv->pmib->dot11OperationEntry.hwaddr, MACADDRLEN);

		if (priv->pmib->dot11BssType.net_work_type & WIRELESS_11N) {
			report.phy_type= 7;	// DOT11_PHY_TYPE_HT
		}
		if (priv->pmib->dot11BssType.net_work_type & WIRELESS_11AC) {
			report.phy_type = 9;	// DOT11_PHY_TYPE_VHT
		}

		report.ch_num = priv->pmib->dot11RFEntry.dot11channel;
		report.reg_class= rm_get_op_class(priv, priv->pmib->dot11RFEntry.dot11channel);
		report.preference = 255;
		report.bss_info= 0;

		return hapd_rm_construct_nb_report(priv,
							pbuf, &report, 0, 255);
...
void gas_init_req_check_frames_tx(struct rtl8192cd_priv *priv, u8 **dump_buf, int *dump_len)
{
	int orig_len = *dump_len;
	int subelement_offset = 11;
	int new_offset = subelement_offset;
	u8 *tmp_buf = NULL;
	u16 new_query_rsp_len = 22;
	u16 nb_report_len = 18;
	u16 nb_rpt_element_id = 272;
	u16 gas_len = 0;

	gas_len += subelement_offset;
	gas_len += 2; //advertisement protocol element
	gas_len += (2 + new_query_rsp_len);

	tmp_buf = (unsigned char*)kmalloc((gas_len), GFP_ATOMIC);
	if (!tmp_buf) {
		goto exit;
	}
	memcpy(tmp_buf, *dump_buf, orig_len);

	/* Query Response Length */
	new_query_rsp_len = cpu_to_le16(new_query_rsp_len);
	memcpy((void*)(tmp_buf + new_offset), (void*)&new_query_rsp_len, 2);
	new_offset += 2;

	/* neighbor report element id 272 */
	nb_rpt_element_id = cpu_to_le16(nb_rpt_element_id);
	memcpy((void*)(tmp_buf + new_offset), (void*)&nb_rpt_element_id, 2);
	new_offset += 2;

	/* neighbor report element length 18 */
	nb_report_len = cpu_to_le16(nb_report_len);
	memcpy((void*)(tmp_buf + new_offset), (void*)&nb_report_len, 2);
	new_offset += 2;

	/* insert self nb report */
	new_offset += hapd_rm_construct_self_nb_report(priv, tmp_buf + new_offset);

	printk("GAS new_offset:%d dump_len:%d gas_len:%d\n", new_offset, *dump_len, gas_len);
...
unsigned char rm_get_op_class(struct rtl8192cd_priv *priv, unsigned char channel)
{
	...
	{
		channel_array = (unsigned char *)op_class_array[i].channel_array;
...
#endif
void rtk_netdev_status(struct rtl8192cd_priv *priv)
{
	struct net_device *dev = priv->dev;
	printk("[%s][%s],drv_state[0x%x]",__func__,dev->name,priv->drv_state);
	closeopenlog("operstate[%d] link_mode[%d]",,dev->operstate,dev->link_mode);
	closeopenlog("flags[0x%x] state[%lu] Qstate[%lu]",dev->flags,dev->state,dev->_tx[0].state);
	closeopenlog("netif_running[%d] ; netif_queue_stopped[%d]",netif_running(dev),netif_queue_stopped(dev));
}
