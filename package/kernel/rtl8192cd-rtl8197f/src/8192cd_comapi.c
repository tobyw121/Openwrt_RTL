			...
			union iwreq_data *wrqu, char *extra)
{
	...
	{
		p = wrqu->name;
		len = snprintf(p, sizeof(wrqu->name), "IEEE 802.11");

		if (pmib->dot11BssType.net_work_type & WIRELESS_11A) {
			len += snprintf(p+len, sizeof(wrqu->name)-len, "a");
		}

		if (pmib->dot11BssType.net_work_type & WIRELESS_11B) {
			len += snprintf(p+len, sizeof(wrqu->name)-len, "b");
		}

		if (pmib->dot11BssType.net_work_type & WIRELESS_11G) {
			len += snprintf(p+len, sizeof(wrqu->name)-len, "g");
		}

		if (pmib->dot11BssType.net_work_type & WIRELESS_11N) {
			len += snprintf(p+len, sizeof(wrqu->name)-len, "n");
		}
		
		if (pmib->dot11BssType.net_work_type & WIRELESS_11AC) {
			len += snprintf(p+len, sizeof(wrqu->name)-len, "ac");
		}
	}
	else
	{
			...
			struct iw_point *data, char *extra)
{
	...
	while (plist != phead && i < IW_MAX_AP) {
		...
		memcpy(addr[i].sa_data, &pstat->cmn_info.mac_addr, WLAN_ADDR_LEN);
...
#define MAX_RTS_THRESHOLD DEFAULT_RTS_THRESHOLD
...
#else
int rtw_get_sec_ie(u8 *in_ie, u16 in_len, u8 *rsn_ie, u16 *rsn_len, u8 *wpa_ie, u16 *wpa_len)
{
	u8 authmode, sec_idx;
	...
	while (cnt < in_len) {
		...
		if ((authmode == _RSN_IE_1_) && (memcmp(&in_ie[cnt + 2], &wpa_oui[0], 4) == _TRUE)) {
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	if (current_ev == previous_ev)
		...
		u8 *p;
		p = tmpBuf;
		if ((priv->site_survey->bss[i].capability & 0x10) == 0) {
			/* No Encryption */
			sprintf(p, "OPEN");
		} else {
			if (priv->site_survey->bss[i].t_stamp[0] == 0) {
				/* WEP */
				sprintf(p, "WEP");
			} else {
				int wpa_exist = 0;
				if (priv->site_survey->bss[i].t_stamp[0] & 0x0000ffff) {
					/* WPA */
					p += sprintf(p, "WPA");
					if (((priv->site_survey->bss[i].t_stamp[0] & 0x0000f000) >> 12) == 0x4) {
						/* WPA-PSK*/
						p += sprintf(p, "-PSK");
					}
					wpa_exist = 1;
				}
				if ((priv->site_survey->bss[i].t_stamp[0] & 0xf0000000) ||
					(priv->site_survey->bss[i].bss_akm_suite & 0x00000004) ||
					(!((priv->site_survey->bss[i].t_stamp[1] & 0x00000800) ||
					(priv->site_survey->bss[i].bss_akm_suite & 0x00000100)) &&
					(priv->site_survey->bss[i].t_stamp[0] & 0xffff0000))) {
					if (wpa_exist)
						p += sprintf(p, "/");
					/* WPA2*/
					p += sprintf(p, "WPA2");
					if (((priv->site_survey->bss[i].t_stamp[0] & 0xf0000000) >> 28) == 0x4 || (priv->site_survey->bss[i].bss_akm_suite & 0x00000004)){
						/*WPA2-PSK*/
						p += sprintf(p, "-PSK");
					}
					wpa_exist = 2;
				}
				if((priv->site_survey->bss[i].t_stamp[1] & 0x00000800) || (priv->site_survey->bss[i].bss_akm_suite & 0x00000100))
				{
					if (wpa_exist)
						p += sprintf(p, "/");
					p += sprintf(p, "WPA3");
					...
					iwe.cmd = IWEVCUSTOM;
					iwe.u.data.length = strlen(tmpBuf);
					iwe.u.data.flags = 1;

					previous_ev = current_ev;
					current_ev = IWE_STREAM_ADD_POINT(info, current_ev, end_buf, &iwe, (char *)tmpBuf);
					if (current_ev == previous_ev){
		...
		rtw_get_sec_ie(priv->site_survey->bss[i].WholeFrame ,priv->site_survey->bss[i].WholeFrameLength, rsn_ie, &rsn_len, wpa_ie, &wpa_len);
		...
		if(rsn_len > 0){
			...
			previous_ev = current_ev;
			current_ev = IWE_STREAM_ADD_POINT(info, current_ev, end_buf, &iwe, (char *)rsn_ie);
			if (current_ev == previous_ev){
				panic_printk("Add point fail %s %d\n", __FUNCTION__, __LINE__);
#if WIRELESS_EXT >= 17
				return -E2BIG;
