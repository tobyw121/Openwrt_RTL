...
void rtl8822bs_xmit_tasklet(unsigned long data);
...
int _rtw_init_xmit_priv(struct rtl8192cd_priv *priv)
{
	...
	// init xmit tasklet
	tasklet_init(&pshare->xmit_tasklet, rtl8822bs_xmit_tasklet, (unsigned long)priv);
...
void rtl8192cd_hal_fill_fwinfo(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, struct tx_desc  *pdesc, unsigned int frag_idx)
{
    ...
    // Use Cts2Self when RtsRate < 11M && MPDU <= rts_thrshld
