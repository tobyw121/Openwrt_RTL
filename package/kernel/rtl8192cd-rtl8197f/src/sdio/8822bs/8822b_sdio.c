...
const u32 reg_freepage_thres[SDIO_TX_FREE_PG_QUEUE] = {
	REG_22B_TQPNT1+2, REG_22B_TQPNT2+2, REG_22B_TQPNT3+2, 0, REG_22B_TQPNT4+2
};
...
void rtl8822bs_interface_configure(struct rtl8192cd_priv *priv)
{
...
int sdio_dvobj_init(struct rtl8192cd_priv *priv)
{
	...
	rtl8822bs_interface_configure(priv);
...
static void HalRxAggr8822BSdio(struct rtl8192cd_priv *priv)
{
	u8	valueDMATimeout;
...
static void pre_rtl8822bs_beacon_timer(unsigned long task_priv)
{
...
static void rtl8822bs_beacon_timer(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	struct priv_shared_info *pshare = priv->pshare;
	u32 beacon_interval;
	...
	if (pshare->offload_function_ctrl > RTW_PM_PREPROCESS) {
		mw5_legacy_mod_timer(&pshare->beacon_timer, jiffies+usecs_to_jiffies(beacon_interval));
		return;
	}
	...
	mw5_legacy_mod_timer(&pshare->beacon_timer, jiffies+usecs_to_jiffies(time_offset));
...
u8 rtw_init_drv_sw(struct rtl8192cd_priv *priv)
{
	...
	mw5_legacy_timer_init(&priv->pshare->beacon_timer);
	priv->pshare->beacon_timer.data = (unsigned long)priv;
	priv->pshare->beacon_timer.function = pre_rtl8822bs_beacon_timer;
	INIT_TIMER_EVENT_ENTRY(&priv->pshare->beacon_timer_event,
		rtl8822bs_beacon_timer, (unsigned long)priv);
