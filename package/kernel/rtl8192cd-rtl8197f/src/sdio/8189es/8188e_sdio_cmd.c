...
void set_ap_offload(struct rtl8192cd_priv *priv, unsigned int deny_ap, unsigned int hid_ap, int enable, unsigned int linked)
#else
void set_ap_offload(struct rtl8192cd_priv *priv, unsigned int deny_ap, unsigned int hid_ap, int enable)
#endif
{
	unsigned int content = 0;
	unsigned int ext_content = 0;
	...
	{
		content |= BIT(8);
		//content |= BIT(9);//no use
		ext_content |= 30;
		if (linked)
		{
			content |= BIT(10);//linked 1,idle 0
			content |= BIT(11);//EnAutoWake
			ext_content |= (252<<8)&0xffff;
		}
	}
#else
	if (enable)
...
void set_bcn_resv_page(struct rtl8192cd_priv *priv, unsigned int rootap, unsigned int vap1, unsigned int vap2)
{
	unsigned int content = 0;
	unsigned int h2c_cmd_bcn_resv = 0x09;

	content |= (rootap << 8);
	content |= (vap1 << 16);
	content |= (vap2 << 24);

	content |= h2c_cmd_bcn_resv;

	signin_h2c_cmd(priv, content, 0);
}

void set_probe_res_resv_page(struct rtl8192cd_priv *priv, unsigned int rootap, unsigned int vap1, unsigned int vap2)
{
	unsigned int content = 0;
	unsigned int h2c_cmd_probe_res = 0x0A;

	content |= (rootap << 8);
	content |= (vap1 << 16);
	content |= (vap2 << 24);

	content |= h2c_cmd_probe_res;
	signin_h2c_cmd(priv, content, 0);
}
...
{
	unsigned int content = 0;
	unsigned int h2c_cmd_repeat_pulse = 0xC4;
	...
	content |= h2c_cmd_repeat_pulse;
	signin_h2c_cmd(priv, content, 0);
}

#if defined(SOFTAP_PS_DURATION) || defined(CONFIG_POWER_SAVE)
void set_softap_ps(struct rtl8192cd_priv *priv, u8 enable, u8 en_32K, u8 lps, u8 duration)
{
	unsigned int content = 0;

	content |= (enable << 8);	//enter PS
	content |= (en_32K << 9);	//enter 32K
	content |= (lps << 10);	//low power rx
	content |= (duration << 16);
	content |= H2C_PS_SOFTAP;
...
void rtw_offload_remw5_legacy_timer_init(struct rtl8192cd_priv *priv)
{
	mw5_legacy_mod_timer(&priv->pshare->beacon_timer, jiffies + 
...
#endif
	mw5_legacy_mod_timer(&priv->pshare->xmit_check_timer, jiffies + msecs_to_jiffies(10));
	if ((LED_TYPE >= LEDTYPE_SW_LINK_TXRX) && (LED_TYPE < LEDTYPE_SW_MAX))
		mw5_legacy_mod_timer(&priv->pshare->LED_Timer, jiffies + priv->pshare->LED_Interval);
}
...
u8 callback_ap_offload(struct rtl8192cd_priv *priv, struct cmd_header *pcmdhdr)
{
	...
	loc_probe[0] = loc_bcn[0] + priv->pshare->ap_offload_res[0].probe_offset;
...
#ifdef SOFTAP_PS_DURATION
	set_softap_ps(priv, 1, 0, 0, 70);
#endif

	pshare->pwr_state = RTW_STS_SUSPEND;
	pshare->offload_function_ctrl = RTW_PM_SUSPEND;

	if (mw5_legacy_timer_pending(&pshare->beacon_timer))
		mw5_legacy_del_timer_sync(&pshare->beacon_timer);

	if (mw5_legacy_timer_pending(&pshare->ps_timer))
		mw5_legacy_del_timer_sync(&pshare->ps_timer);

	if (mw5_legacy_timer_pending(&pshare->xmit_check_timer))
		mw5_legacy_del_timer_sync(&pshare->xmit_check_timer);
...
void ap_offload_exit(struct rtl8192cd_priv *priv)
{
	...
	RTL_W8(REG_MBID_NUM, RTL_R8(REG_MBID_NUM) & ~BIT3);
	...
	if (priv->drv_state & DRV_STATE_OPEN)
		rtw_offload_remw5_legacy_timer_init(priv);
