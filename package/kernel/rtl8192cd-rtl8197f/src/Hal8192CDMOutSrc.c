...
unsigned char cck_swing_table_ch1_ch13[][8] = {
...
void MP_DIG_process(unsigned long task_priv)
{
	...
	mw5_legacy_mod_timer(&priv->pshare->MP_DIGTimer, RTL_MILISECONDS_TO_JIFFIES(700));
}
#endif
...
void add_rssi_timer(unsigned long task_priv)
{
	...
	if (set_timer)
		mw5_legacy_mod_timer(&priv->add_rssi_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(50));	// 50 ms
}
...
//1 6.Set next timer
	...
	if(priv->pshare->DM_SWAT_Table.TestMode == RSSI_MODE)	{
		mw5_legacy_mod_timer(&priv->pshare->swAntennaSwitchTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(400));		// 400 ms
	} else if(priv->pshare->DM_SWAT_Table.TestMode == TP_MODE)	{

		if(priv->pshare->TrafficLoad == TRAFFIC_HIGH)	{
			if(priv->pshare->DM_SWAT_Table.rssi_trying%2 == 0)
				mw5_legacy_mod_timer(&priv->pshare->swAntennaSwitchTimer, jiffies +  RTL_MILISECONDS_TO_JIFFIES(10));	// 10 ms
			else
				mw5_legacy_mod_timer(&priv->pshare->swAntennaSwitchTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(80));	// 80 ms

		} else if(priv->pshare->TrafficLoad == TRAFFIC_LOW) {
			if(priv->pshare->DM_SWAT_Table.rssi_trying%2 == 0)
				mw5_legacy_mod_timer(&priv->pshare->swAntennaSwitchTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(40));	// 40 ms
			else
				mw5_legacy_mod_timer(&priv->pshare->swAntennaSwitchTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(80));	// 80 ms
		}
	}
...
//			del_sta(priv, tmpbuf);
			...
			if(CHIP_VER_92X_SERIES(priv) {
			if (mw5_legacy_timer_pending(&priv->pshare->rl_recover_timer))
				del_timer_sync (&priv->pshare->rl_recover_timer);
			mw5_legacy_mod_timer(&priv->pshare->rl_recover_timer, jiffies + EXPIRE_TO*TFRL_RcvTime);
			}
...
void rtl8192cd_DPK_timer(unsigned long task_priv)
{
	...
	if (priv->pshare->pwr_trk_ongoing){
		...
		mw5_legacy_mod_timer(&priv->pshare->DPKTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(100));
	}else{
