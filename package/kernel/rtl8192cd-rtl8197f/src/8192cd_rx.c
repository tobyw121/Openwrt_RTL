...
//static int auth_filter(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct rx_frinfo *pfrinfo);
...
static unsigned int check_mcastL2L3Diff(struct sk_buff *skb)
{
	...
	#ifdef __ECOS
	struct iphdr *iph = (struct iphdr *)rtl_get_skb_ip_header(skb, 0);	
	#else
...
{
#ifdef __ECOS
	struct ip6_hdr *iph = (struct ip6_hdr *)rtl_get_skb_ip_header(pskb, 0);
...
static char ___igmp_type_check(struct sk_buff *skb, unsigned char *gmac)
{
	...
	if (unlikely((*(unsigned short *)(SKB_MAC_HEADER(skb) + ETH_ALEN * 2) == __constant_htons(ETH_P_8021Q))))
		vlanTag = 4;
	if (unlikely((*(unsigned short *)(SKB_MAC_HEADER(skb) + ETH_ALEN * 2 + vlanTag) == __constant_htons(ETH_P_IP)))) 
	{
		...
		switch (nextHeader)
		{
			...
			case ICMP_PROTOCOL:
				if (ptr[0] == MLDV2_REPORT) {
				    ...
				    unsigned short grpnum = ( (ptr[6]<<8) + ptr[7]);
					unsigned short srcnum = 0;
				    int i=0, op=0, action=0;
					//printk("MLDV2_REPORT\n");
					grec = (struct mld2_grec *)(ptr + 8);
					for (i=0; i<grpnum; i++ ) {
						srcnum = ntohs(grec->grec_nsrcs);
						___ConvertMulticatIPv6toMacAddr((unsigned char *)&grec->grec_mca, gmac);
						...
						//printk("MLD2_CHANGE_TO_EXCLUDE\n");
							...
							op = (srcnum == 0) ? 0x8B81 : 0x8B80; /* SIOCGIMCAST_DEL;  leave and delete it */
...
#else 
#if (LINUX_VERSION_CODE >= (KERNEL_VERSION(3,14,0))) && !defined(CONFIG_WLAN_HAL_8197G)
unsigned compare_ether_addr(const u8 *addr1, const u8 *addr2)
{
	const u16 *a = (const u16 *) addr1;
	const u16 *b = (const u16 *) addr2;
				...
				)
{
	...
	&& (GetFrameSubType(pframe) != WIFI_BLOCKACK)
#endif
	...
	{
#ifdef BA_RSSI_STATISTIC
	if(GetFrameSubType(pframe) != WIFI_BLOCKACK)
#endif
...
void translate_rssi_sq_outsrc(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, char rate)
{
	...
	struct stat_info 	*pstat = NULL;
	unsigned char		*addr2;
	memset(&pktinfo,0,sizeof(struct phydm_perpkt_info_struct));
...
#if 1
	pktinfo.station_id = (pstat ? ((u1Byte)(pstat->cmn_info.mac_id)) : 0);
#else
...
#if defined(A4_STA) && !(defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
			if(priv->pmib->miscEntry.a4_enable) {
...
//	printk("=====>> INSIDE rtl8192cd_rx_tkl_isr <<=====\n");
	...
	if ((priv->pwr_state == L2) || (priv->pwr_state == L1)) {
		rtk_tasklet_clear_triggered(&(priv->pshare->rx_tasklet));
		return;
	}
#endif
    ...
    // Filen: "HIMR Enable" should be atomic to avoid that "RX interrupt" is gone
#ifdef CONFIG_RTL_88E_SUPPORT
	if (GET_CHIP_VER(priv)==VERSION_8188E) {
...
#if defined(DELAY_REFILL_RX_BUF) && defined(RTL8190_ISR_RX) && defined(RTL8190_DIRECT_RX)
	SAVE_INT_AND_CLI(flags);
	SMP_LOCK_RECV(flags);
	delay_refill_rxdesc(priv);
...
)
{
	...
	SAVE_INT_AND_CLI(x);
#endif
	if (skb || (((cur_q->host_idx + cur_q->rxbd_ok_cnt)%cur_q->total_rxbd_num) != cur_q->cur_host_idx)) {
		if (skb) {
			new_skb = skb;
		} else {
			...
			new_skb = dev_alloc_8190_skb(data, RX_BUF_LEN);
	   		...
	   		new_skb = rtw_mem_pool_alloc_skb(RX_BUF_LEN);
#else
...
int refill_rx_ring(struct rtl8192cd_priv *priv, struct sk_buff *skb, unsigned char *data)
{
	...
	SAVE_INT_AND_CLI(x);
#endif
	if (skb || (phw->cur_rx_refill != phw->cur_rx)) {
		if (skb)
			new_skb = skb;
		else {
#ifdef CONFIG_RTL8190_PRIV_SKB				
...
#ifdef PREVENT_ARP_SPOOFING
int check_arp_spoofing(struct rtl8192cd_priv *priv, struct sk_buff *pskb)
{
	struct net_bridge_port *br_port;
	struct arphdr *arp;
	...
	if(protocol == __constant_htons( ETH_P_8021Q)) {
		vlan_tag = 4;
		protocol = *((unsigned short *)(pskb->data + 2 * ETH_ALEN + 4));
	}	
	if (protocol == __constant_htons(ETH_P_ARP)) {
		arp = (struct arphdr *)(pskb->data + ETH_HLEN +vlan_tag);
...
void process_all_queues(struct rtl8192cd_priv *priv)
{
	...
	struct rx_frinfo *pfrinfo = NULL;
	unsigned int data_loop_count = 0;
...
#ifdef RING_BASED_DATA_FRAME_QUEUE
	if (!atomic_add_unless(&(priv->data_queue_dequeuing), 1, 1)) {
		// flush_rx_queue() is called, return directly
		return;
	}
#endif

	// processing data frame first...
	while (data_loop_count < priv->pshare->rf_ft_var.limit_proc_all_queue)
	{
#ifdef RING_BASED_DATA_FRAME_QUEUE
		pfrinfo = rtk_data_queue_dequeue(priv, &(priv->data_queue));
...
#ifdef RING_BASED_DATA_FRAME_QUEUE
	atomic_set(&(priv->data_queue_dequeuing), 0);
#endif
...
#ifdef CONFIG_WLAN_HAL
__IRAM_IN_865X
void rtl8192cd_dsr_rxq(unsigned long data)
{
#ifdef MBSSID
	int i;
#endif
...
dsr_rxq_end:
	...
	rtk_tasklet_clear_triggered(&(priv->pshare->rxq_tasklet));
#endif
}

__IRAM_IN_865X
static inline void schedule_rxq_tasklet(struct rtl8192cd_priv *priv, struct net_device *dev)
{
#ifdef WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD
	if (priv->pmib->dot11RFEntry.rx_data_offload == 1)
	{
		if (rtk_tasklet_set_triggered(&(priv->pshare->rxq_tasklet))) {
			smp_call_function_single_async(RX_DATA_OFFLOAD_CPU,	&(priv->pshare->rxq_tasklet_csd));
		}
	} else
#endif
	{
		rtl8192cd_dsr_rxq((unsigned long)priv);
	}
}

#ifdef CONFIG_RTL8192CD_LINUX6_BUS
int rtl8192cd_linux6_napi_poll(struct napi_struct *napi, int budget)
{
	struct priv_shared_info *pshare =
		container_of(napi, struct priv_shared_info, linux6_rx_napi);
	struct rtl8192cd_priv *priv = pshare->linux6_root_priv;
	int work_done;

	if (!priv || !IS_DRV_OPEN(priv)) {
		napi_complete_done(napi, 0);
		return 0;
	}
#if defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
	pshare->linux6_napi_polls++;
#endif

	pshare->linux6_rx_budget = budget;
	pshare->linux6_rx_work_done = 0;
	pshare->linux6_napi_polling = true;

	rtl8192cd_rx_isr(priv);

	work_done = min(pshare->linux6_rx_work_done, budget);
#if defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
	pshare->linux6_napi_work_done += work_done;
#endif
	pshare->linux6_napi_polling = false;
	pshare->linux6_rx_budget = 0;

	/* Preserve the vendor software RX queues, but execute their drain from
	 * the NAPI context instead of scheduling another RX tasklet.
	 */
	schedule_rxq_tasklet(priv, priv->dev);

#if defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
	/* v44.55 MW5/RTL8812BRH stability: Report19 proves that MGT/HI0 DOK
	 * interrupt counters can stay at zero even while those rings advance.
	 * Do not duplicate any reclaim logic here.  Ask the vendor TX DSR to
	 * poll the hardware queue indices after real RX progress; its normal
	 * locking and rtl88XX_tx_recycle() path performs the DMA unmaps. */
	if (work_done > 0 && GET_CHIP_VER(priv) == VERSION_8822B &&
	    pshare->linux6_radio_index == 0 &&
	    !(pshare->linux6_napi_polls & 0x0f) &&
	    rtk_tasklet_set_triggered(&(pshare->tx_tasklet))) {
		pshare->linux6_tx_reclaim_kicks++;
		tasklet_schedule(&(pshare->tx_tasklet.tasklet));
	}
#endif
...
void flush_rx_queue(struct rtl8192cd_priv *priv)
{
	struct list_head *list = NULL;
	struct rx_frinfo *pfrinfo = NULL;
	unsigned int wait_rx_data_queue_counter = 0;
#if defined(SMP_SYNC)
	unsigned long x;
#endif

#ifdef RING_BASED_DATA_FRAME_QUEUE
	while (!atomic_add_unless(&(priv->data_queue_dequeuing), 1, 1)) {
		// wait until the process of data frames in process_all_queue() is completed
		if (10 > (wait_rx_data_queue_counter++)) {
			// break if wait more than 50 ms
			printk("flush_rx_queue wait more than 50 ms.\n");
			break;
		} else {
			delay_ms(5);
		}
	...
	while(1)
	{
		...
		rtl_atomic_dec(&priv->pshare->rx_enqueue_cnt);
#if !(defined(RING_BASED_DATA_FRAME_QUEUE))
		pfrinfo = list_entry(list, struct rx_frinfo, rx_list);
#endif
		rtl_kfree_skb(priv, pfrinfo->pskb, _SKB_RX_);
	}
...
static __inline__ unsigned char parse_ps_poll(struct rtl8192cd_priv *priv, struct stat_info *pstat) {
    unsigned char Q_id[20];      
    unsigned char Q_pktnum[20];     //8814B extend to 20 queue
    ...
    if (hw_queue_num) {
		...
		if (GET_CHIP_VER(priv) == VERSION_8814A || GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8822C || GET_CHIP_VER(priv) == VERSION_8197F || GET_CHIP_VER(priv) == VERSION_8812F) {
            ...
            if(GET_CHIP_VER(priv)== VERSION_8814B){
                for(i=0;i<5;i++){
                    RTL_W8(0x417,i);
                    Q_pktnum[i*4]   = (RTL_R32(REG_QUEUELIST_INFO0) & 0xFFF00)>>8;	
					if(Q_pktnum[i*4])
                        Q_id[i*4] = RTL_R8(REG_QUEUELIST_INFO0);
                    else
                        Q_id[i*4] = 0;
					
                    Q_pktnum[i*4+1] = (RTL_R32(REG_QUEUELIST_INFO1) & 0xFFF00)>>8;
					if(Q_pktnum[i*4+1])
                        Q_id[i*4+1] = RTL_R8(REG_QUEUELIST_INFO1);
                    else
                        Q_id[i*4+1] = 0;
					
                    Q_pktnum[i*4+2] = (RTL_R32(REG_QUEUELIST_INFO2) & 0xFFF00)>>8;	
					if(Q_pktnum[i*4+2])
                        Q_id[i*4+2] = RTL_R8(REG_QUEUELIST_INFO2);
                    else
                        Q_id[i*4+2] = 0;
					
                    Q_pktnum[i*4+3] = (RTL_R32(REG_QUEUELIST_INFO3) & 0xFFF00)>>8;
					if(Q_pktnum[i*4+3])
                        Q_id[i*4+3] = RTL_R8(REG_QUEUELIST_INFO3);
                    else
                        Q_id[i*4+3] = 0;
...
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8812FE) || defined (CONFIG_WLAN_HAL_8197G) || defined (CONFIG_WLAN_HAL_8814BE)
			if (GET_CHIP_VER(priv) == VERSION_8814A || GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8822C || GET_CHIP_VER(priv) == VERSION_8197F || GET_CHIP_VER(priv) == VERSION_8198F || GET_CHIP_VER(priv) == VERSION_8812F|| GET_CHIP_VER(priv) == VERSION_8197G || GET_CHIP_VER(priv) == VERSION_8814B) {
...
#ifdef BA_RSSI_STATISTIC
	if(pframe && GetFrameSubType(pframe) == WIFI_BLOCKACK )
	{
		struct stat_info *pstat2 = get_stainfo(priv, GetAddr2Ptr(pframe));
		if(pstat2 && pfrinfo->rf_info.rssi) {
			if(pstat2->rssi_ba == 0)
				pstat2->rssi_ba = pfrinfo->rf_info.rssi;
			else
				pstat2->rssi_ba = cal_rssi_avg(pstat2->rssi_ba, pfrinfo->rf_info.rssi);
		}
		goto end_ctrl;
	}
#endif
