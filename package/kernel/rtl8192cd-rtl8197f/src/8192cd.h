...
#define _FUNC_NAME(x) x ## _usb
#define _PARA_NAME(x) x ## _usb
#else
#define _FUNC_NAME(x) x
#define _PARA_NAME(x) x
#endif

#define TX_SC_ENT           _PARA_NAME(tx_sc_ent)
#define TX_SC_REPLACE_IDX   _PARA_NAME(tx_sc_replace_idx)
...
struct hci_ops_s {
	int (*start_xmit)(struct sk_buff *skb, struct net_device *dev, int tx_flag);
	int (*firetx)(struct rtl8192cd_priv *priv, struct tx_insn* txcfg);
    ...
    int (*dz_queue)(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *pskb);
...
#include "./sdio/8822bs/8822b_sdio.h"
#endif
#ifdef CONFIG_WLAN_HAL_8821CE
#include "./sdio/8821cs/8821c_sdio.h"
#endif
...
#define NUM_STATS_ENV_CNT 20
#endif

#ifdef SMP_LOAD_BALANCE_SUPPORT
#define MAX_SKB_XMIT_QUEUE		40
#endif

#ifdef ADDRESS_CAM
#define ADDR_CAM_ENTRY_NUM		128
#define ADDR_CAM_MAP_LEN		(ADDR_CAM_ENTRY_NUM >> 3)
#define SET_ADDR_CAM_USED(priv, i) \
	do { \
		if (i >= 0 && i <= ADDR_CAM_ENTRY_NUM) \
			priv->pshare->addr_cam_used[(i) >> 3] |= BIT((i) & 0x7); \
	} while (0)
#define CLEAR_ADDR_CAM_USED(priv, i) \
	do { \
		if (i >= 0 && i <= ADDR_CAM_ENTRY_NUM) \
			priv->pshare->addr_cam_used[(i) >> 3] &= ~(BIT((i) & 0x7)); \
	} while (0)
#define ADDR_CAM_USED(priv, i) ((priv->pshare->addr_cam_used[(i) >> 3]) & BIT((i) & 0x7))
#endif

#if defined(CONFIG_PE_ENABLE)
#define SET_DEBUG_STAT(priv, i) \
	do {\
		if(GET_CHIP_VER(priv) == VERSION_8814B && priv->pe_initialized) { \
			struct pe_addr_base* pe_addr = get_pe_addr_base(); \
			*(volatile unsigned int*)(pe_addr->pe_fw_virt_addr+0x00d00004) = i; \
			if(priv->pshare->rf_ft_var.swq_dbg == 57) \
				panic_printk("set a17 debug stat = 0x%x(0x%x=0x%x)\n", i, pe_addr->pe_fw_virt_addr+0x00d00004, *(volatile unsigned int*)(pe_addr->pe_fw_virt_addr+0x00d00004)); \
		} \
	} while(0)
...
#else
#define SET_DEBUG_STAT(priv, i) do { } while(0)
#endif

#if (BEAMFORMING_SUPPORT == 1)

#if defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8822CE) ||  defined(CONFIG_WLAN_HAL_8812FE)
#define NUM_TXBF_SU		2
#if defined(CONFIG_WLAN_HAL_8814BE)
#define NUM_TXBF_MU		4
#else
...
#define MAX_NUM_MU_PARTNER		1
#elif defined(CONFIG_WLAN_HAL_8814BE)
#define MAX_NUM_BEAMFORMEE_MU	4
#define MAX_NUM_BEAMFORMER_MU	1
#define MAX_NUM_MU_PARTNER		3
#else
#define MAX_NUM_BEAMFORMEE_MU	0
#define MAX_NUM_BEAMFORMER_MU	0
#define MAX_NUM_MU_PARTNER		0
#endif
#endif
...
typedef struct _RT_BEAMFORMING_ENTRY {
	...
	u2Byte	P_AID;
	u1Byte	MacAddr[6];
	enum channel_width	BW;	// Sounding BandWidth
	u2Byte					SoundPeriod;
...
typedef struct _RT_BEAMFORMING_INFO {
	...
	struct _RT_BEAMFORMER_ENTRY		BeamformerEntry[BEAMFORMER_ENTRY_NUM];
...
#if defined(HS2_SUPPORT) || defined(TDLS_SUPPORT)
...
enum {
	...
	OFFLOAD_PROHIBIT_RADIO_MEASURE = BIT9,
};
...
enum led_type {
	...
	LEDTYPE_SW_LED_DO_NOTHING = 60,					// 60 for xDSL/PON patform, only init, do nothing
	LEDTYPE_SW_MAX,
};
...
enum _ROAM_SUB_TYPE {
	...
	SR_VAPID                   = 1,
	SR_CHANNEL                 = 2,
	SR_AUTO_CHANNEL_BACKUP     = 3,
	SR_CHANNEL_UTILIZATION     = 4,
	SR_NEIGHBOR_STA_LIST       = 5,
	SR_ASSOC_STA_LIST          = 6,
	SR_NEIGHBOR_REPORT         = 7,
	SR_CSA_REQUEST             = 64,
	SR_MESH_PATHSEL            = 65
};
#endif
...
enum _VERIWAVE_PKT_SIZE {
	VERIWAVE_PKT_SIZE_BIG = 0,
	VERIWAVE_PKT_SIZE_SML = 1
};
//#endif
...
enum _LNA_TYPE {
	...
	LNA_TYPE_2 = 2,
	LNA_TYPE_3 = 3,
	LNA_TYPE_DSLPON_SKY85201_11_2L = 4,
	LNA_TYPE_DSLPON_SKY85201_11_4L = 5,
	LNA_TYPE_MAX
};
...
struct reprepare_info {
	struct sk_buff	*pSkb;
    unsigned short  swps_seq;
    char    amsdu_flag; //if 1, this pskb point to buf_pframe_amsdu, not a single skb
    char    *hdr;
    char    *llchdr;
    char    *icv;
    char    *mic;
...
enum WIFI_PROCESS_USER_IE_TYPE {
	WIFI_IE_DEL,
	WIFI_IE_ADD,
	WIFI_IE_DELALL,
};
...
#elif defined(AUTH_SAE)
#include "./hash_table.h"
#endif

enum {
	STATUS_11K_SUPPORT = BIT0,
	STATUS_11V_SUPPORT = BIT1
};
...
#endif
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
    struct h2d_txpkt_info  tx_pkt_Info;
#endif
	struct wlan_hdr		hdr;
};
...
#endif
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
    struct h2d_txpkt_info  tx_pkt_Info; /*record txpktinfo*/
#endif
...
// stucture storing the info of STA which has been or will be steered to another band
struct BandTransNode {
...
struct tx_insn	{
	...
	unsigned char		amsdu_sync_flag;
//	unsigned char		tpt_pkt;
	unsigned char		one_txdesc;
	unsigned char		is_test;  // cliff 
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
	/* v44.48 diagnostic-only tag, set from the already-decoded Ethernet protocol. */
	unsigned char		mw5_eapol_frame;
#endif

#if defined(AP_SWPS_OFFLOAD_VERI) || defined(AP_SWPS_OFFLOAD_WITH_AMPDU_VERI)
...
#endif
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
    unsigned short		h2d_type;
    unsigned char		h2d_tid;
    unsigned char		last_msdu;
#ifdef TX_SHORTCUT
	unsigned char		skip_txsc;
#endif
#endif
};
...
#if defined(CONFIG_RTL_TRIBAND_SUPPORT)
  #if defined(CONFIG_RTL_92E_SUPPORT)
    #include "./usb/8192eu/8192cd_usb_xmit.h"
  #elif defined(CONFIG_RTL_92F_SUPPORT)
    #include "./usb/8192fu/8192cd_usb_xmit.h"
  #else
    #error "triband undefined!!"
  #endif
#endif /* defined(CONFIG_RTL_TRIBAND_SUPPORT) */
...
#endif
#ifdef CONFIG_WLAN_HAL_8822BE
#include "./sdio/8822bs/8822b_sdio_xmit.h"
#endif
#ifdef CONFIG_WLAN_HAL_8821CE
#include "./sdio/8821cs/8821c_sdio_xmit.h"
#endif
#endif
...
struct reorder_ctrl_entry {
	...
	unsigned short		last_seq;
  	struct reorder_ctrl_timer *rc_timer;
};
...
#if CONFIG_WLAN_NOT_HAL_EXIST || defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
...
// 48 is SIZE_TXDESC_88XX
	u1Byte  hal_hw_desc[64]; //extend to MAX size, SIZE_TXDESC_88XX_MAX
  #endif // CONFIG_WLAN_HAL
#if CONFIG_WLAN_NOT_HAL_EXIST || defined(CONFIG_RTL_WAPI_SUPPORT)
...
#if defined(CONFIG_RTL_OFFLOAD_DRIVER) && defined(TX_SHORTCUT)
	struct h2d_txpkt_info txpkt_info;
#endif
};
#endif // TX_SHORTCUT
...
//set BTM response waiting list
struct response_waiting_list
{
	unsigned char addr[MACADDRLEN];
	unsigned char bssid[MACADDRLEN];
	unsigned char target_bssid[MACADDRLEN];
	unsigned char retry_counter;
	enum STA_BTM_STATUS status;
	unsigned char response_timer; // record the time after BTM request is sent out
};
...
#pragma pack(1)
struct dot11v_bss_termination_subelement{
	unsigned char subelement_id;
	unsigned char len;
	UINT64 bss_termination_tsf;
	unsigned short duration;
};

struct bss_candidate_entry{
	char index;
	unsigned char channel_utilisation;
};
#pragma pack()
...
#if defined(CONFIG_IEEE80211V) || defined(DOT11K) || defined(HAPD_11K)
#pragma pack(1)
...
#endif
	/*20210723 for reduce the emory consumption of sta_info*/
	//struct dot11k_beacon_measurement_report beacon_report[MAX_BEACON_REPORT];
	struct dot11k_beacon_measurement_report *beacon_report;
#ifdef RTK_MULTI_AP
	unsigned char beacon_report_start_index;
	unsigned char beacon_report_len[MAX_BEACON_REPORT];
	unsigned char beacon_measurement_token[MAX_BEACON_REPORT];
	unsigned char beacon_report_mode[MAX_BEACON_REPORT];
#endif
};
...
struct dot11r_sta_info {
	unsigned char bss_num;
	struct dot11r_ft_bss_report report[BSS_NUM];
};
...
#ifdef RTK_MULTI_AP
struct clientCapabilityEntry{
	unsigned char macaddr[6];
	unsigned char bssid[6];
	unsigned char			frameLength;
	unsigned char			frameBody[256];
};
//--------Channel Scan---------
struct map_channel_scan_neighbor {
	unsigned char		bssid[6];
	...
	unsigned short int	station_count;
};

struct map_channel_scan_result_per_channel {
	unsigned char		channel;
	unsigned char		scan_status;	//set this to be non zero to indicate an error
	char				timestamp[31];	//always 31 char long in our implementation
	unsigned char		channel_utilization;
	unsigned char		noise;
	unsigned short int	neighbor_nr;
	struct map_channel_scan_neighbor* neighbors;
};

struct map_channel_scan_result_per_radio {
	unsigned char channel_nr;
	struct map_channel_scan_result_per_channel* channels;
};
//-----------------------------
...
struct CACCompletionReportNonOccupancyClassChannelPairs
{
    ...
    unsigned char   nonoccup_remaining_time[3];  //Seconds
...
struct CACCompletionReportAvailabelChannels
{
    ...
    unsigned short int  identify_time;  //minutes
...
{
    unsigned char   radio_unique_identifier[6];
    unsigned char   op_class;
    unsigned char   channel;
    unsigned char   flags;  //CAC method to be used.
...
#define TCPACK_SESSION_MAX_ENTERY 	32
#define TCPACK_SESSION_TIMEOUT		3  // seconds
struct tcpack_session {
	unsigned char	ip_port[12];
	unsigned char	flag;
	unsigned int	used;
...
#define RA_OFFSET_TRAINING_PERIOD	10
#define RA_OFFSET_REV_TAG 			0x80
#define RA_OFFSET_TRIGGER_TH 		3
#define RA_OFFSET_TRAINING_STEPS	3
#define RA_OFFSET_TRAINING_TO		5
enum {
	RA_OFFSET_NON_TRIGGER,
	RA_OFFSET_POS_TRIGGER,
	RA_OFFSET_NEG_TRIGGER
};
enum {
	RA_OFFSET_STEP_IDLE,
	RA_OFFSET_STEP_TRAINING,
	RA_OFFSET_STEP_DONE
};
struct ra_offset_st {
	unsigned int		cur_time;
	unsigned char		rate_var;
	unsigned char		trigger_cnt;
	unsigned char		prev_rate;
	unsigned char		curr_edge;
	unsigned char		time_line[RA_OFFSET_TRAINING_PERIOD];
	unsigned char		alive;
	unsigned int		rty_ratio;
};
#endif

struct stat_info_pe{
//===sync with PE
	unsigned char		tx_bw_bak;
...
//======end sync with PE
	...
	unsigned int		is_qosenhance_sta;
	unsigned int		is_qosenhance_farsta;
	unsigned int		qosenhance_new_tid;
#endif
...
#ifdef TX_SHORTCUT
#if defined(CONFIG_PCI_HCI)
	struct tx_sc_entry	tx_sc_ent[MAX_TXSC_ENTRY];
	...
	int					amsdu_size[8];
#ifdef SUPPORT_TX_AMSDU_SMALL_PKTS_ONLY
	struct tcpack_session	amsdu_tcpack_ses[TCPACK_SESSION_MAX_ENTERY];
	unsigned short			amsdu_dropdupack_en;
#endif
#endif
	...
	struct tcp_session	tcp_ses[TCP_SESSION_MAX_ENTRY];
	unsigned short		tcpack_acc_en;  // 0: disable, 1: enable flag for 11AC, 2: enable flag for 11N
#endif
	unsigned short		veriwave_5G_TCP_Rx_test; // 0: none, 1: veriwave 5G throughput TCP Rx test
	unsigned short		veriwave_2G_RvR_test;    // 0: none, 1: veriwave 2G RvR test
	...
	******************************************************************/
	...
	unsigned int		rx_mgnt_pkts;

	unsigned int		tx_pkts2;
	unsigned int		rx_pkts2;
	unsigned int		tcpack_pkts;

	//#ifdef CONFIG_VERIWAVE_CHECK
	unsigned int		tx_big_pkts;
	unsigned int		tx_sml_pkts;
	...
	unsigned char		ADDBA_ready[16];
	unsigned char		ADDBA_req_num[16];
	unsigned char		ADDBA_sent[16];
...
struct stat_info {
	...
	int					swq_atm_txrate;
	int					swq_atm_max_deqnum;
	int					swq_atm_max_enqnum;
	unsigned long		swq_atm_drops;
	unsigned long		swq_atm_cnt;
	unsigned long		swq_atm_deqnum;
#endif
	...
	unsigned int		atm_sta_time;//sta tx time for mode 0/1
	unsigned int		atm_match_sta_time;//sta tx time for mode 2
	unsigned long		atm_drop_cnt;
	unsigned long		atm_swq_sent_cnt;//how many packet swq has sent
	unsigned long		atm_enque_cnt;
	unsigned int        tdma_airtime;
	unsigned char       sta_paused;
	unsigned int        Q_pkt;
    unsigned int        dqnum;
	unsigned char		wakeup_tx;
	struct sw_tx_q  	tdma_swq;//tdma_swq
#endif

#if defined(WIFI_QOS_ENHANCE)
	unsigned int		is_qosenhance_sta;
	unsigned int		is_qosenhance_farsta;
	unsigned int		qosenhance_old_tid;
	unsigned int		qosenhance_new_tid;
#endif
...
#endif
	/*https://jira.realtek.com/browse/WQCAP-3396
		to avoid some device send twice same sae_commit*/
	u8 sae_commit[32];
	u8 sae_commit_len;
#endif

#ifdef AUTH_SAE
	//struct sae_data 	sae;/*save PMK*/
	/*when rx peer's SAE confirem save PMK here 
	clean these when release_stainfo*/
	unsigned char sae_pmk[LEN_PMK];
	unsigned char sae_pmkid[LEN_PMKID];
	unsigned char		is_sae_sta;
...
#ifdef DOT11K
    struct mw5_legacy_timer	rm_timer;
#endif
	unsigned char		tx_bw_bak;	//backup station's bwndwidth ever used, keep tracking unless freed
#if defined(AP_SWPS_OFFLOAD) 
    unsigned short      SWPS_seq[4]; //need alloc init
    unsigned char       Drop_ID; //need alloc init
    unsigned char       EnSWPS;
    //int                 SWPS_pkt_Qos;
    unsigned short      SWPS_last_seq[4]; //the last seq that need to be dropped & to reprepare
    unsigned char       prepare_done[4];
    unsigned char       RPT_DropBit; 
    unsigned char       RPT_PSflag;
    unsigned short      RPT_TxDone_SEQ[4]; //0:BK, 1:BE, 2:VI, 3:VO , used to check if packet is ok to recycle 
    unsigned short      RPT_DropByID_SEQ[4]; //0:BK, 1:BE, 2:VI, 3:VO, used to check if packet drop is finished 
    unsigned short      RPT_reprepare_SEQ[4];
    unsigned char       check_drop_done;
    unsigned short      reprepare_num[4];

    unsigned short      SWPS_seq_head[4];//the last tx asigned seq //need alloc init
    unsigned short      SWPS_seq_tail[4];//the oldest tx assigned seq that hasn't been recycled  //need alloc init

    //for dz_queue
    struct list_head    dz_list_head;
    unsigned char       process_dz_ok;
    unsigned char       next_prepare_ok; //to indicate and distinquish different times prepare ok
    unsigned char       PS_get_SWPSRPT;
    unsigned short      DropDone_seq[4][4]; // 4 for BKBEVOVI, 4 for dropID0-3
    unsigned char       get_dropid_flag;
    unsigned short      NULL1_cnt;
    unsigned short      EndSequence[4];
    unsigned char       AMPDU_status[4][8];
#ifdef AP_SWPS_VERI_ANALYZE
    //debug, veri
    unsigned char       Sleep_times;
    unsigned int        sleep_ary_idx;
    unsigned char       TSF_record_flag;
    unsigned char       sleeptimes_idx[100];
    unsigned int        driverTSF[100];
    unsigned char       record_finish[100];
    unsigned char       record_finish_prepare[100];
    unsigned int        FWTSF[100];
    unsigned short      DropbyID_start[100];
    unsigned short      DropbyID_result[100];
    unsigned char       drop_pkt_num;
    unsigned int        prepareok_time[100];
    unsigned int        max_prepareok_time;
    unsigned int       record_drop_afterc2h_cnt;
    unsigned int       record_drop_beforec2h_cnt;
    unsigned int        prepareok_type[3][3];//for different caller counting
    unsigned int       has_insert_pkt[100];
#endif
    unsigned char       manual_controlSWPS;
    unsigned char       manual_EnSWPS;
#endif
    ...
    unsigned int		tx_sc_pkts_slow;
#ifdef SUPPORT_TX_AMSDU_SHORTCUT
    int			tx_sc_amsdu_replace_idx;
    int			tx_sc_amsdu_idx_cache;
    unsigned int		tx_sc_amsdu_pkts_lv1;
    unsigned int		tx_sc_amsdu_pkts_lv2;
    unsigned int		tx_sc_amsdu_pkts_slow;
#endif
...
#ifdef SUPPORT_TX_AMSDU_SMALL_PKTS_ONLY
	struct tcpack_session	amsdu_tcpack_ses[TCPACK_SESSION_MAX_ENTERY];
	unsigned short			amsdu_tcpack_ses_cnt;
	unsigned short			amsdu_tcpack_en;
	unsigned short			amsdu_dropdupack_en;
#endif
#endif

#ifdef ADDRESS_CAM
    short                 addr_cam_entry;
#endif
#if defined(CONFIG_MBO) && defined(CONFIG_IEEE80211V)
	unsigned char non_operable_channels[16];
	unsigned char non_operable_channels_num;
	unsigned char non_preferred_channels[16];
	unsigned char non_preferred_channels_num;
	unsigned char preferred_channels[16];
	unsigned char preferred_channels_num;
#endif
   ...
   struct	dot11v_stat_info wnm;
	...
	unsigned int		useShortPreamble;
	unsigned int		expire_to;
	unsigned int		expire_rx_data;
	unsigned int		brfwtbl_to;
#ifdef STA_ROAMING_CHECK
	unsigned int        rtl_link_roaming_value;
#endif
	unsigned char		rssi;
#ifdef BA_RSSI_STATISTIC
	unsigned char		rssi_ba;
#endif
	unsigned char		sq;
	unsigned char		rx_rate;
	unsigned char		rx_bw;
...
//#if !defined(USE_OUT_SRC) || defined(_OUTSRC_COEXIST)
	...
	unsigned int		txrpt_tx_bytes_array[TXRPT_TX_BTYES_RANGE];
	unsigned int		txrpt_tx_bytes_array_idx;
	...
	unsigned int		rx_mgnt_pkts_pre;	
	unsigned int		tx_fail_pre;
	unsigned int		tx_fail_pre2;
	unsigned int		tx_pkts2_pre;
	unsigned int		tx_pkts2_pre2;
	unsigned int		rx_pkts2_pre;
	unsigned int		rx_pkts2_pre2;
	unsigned int		tcpack_pkts_pre;
//#ifdef CONFIG_VERIWAVE_CHECK
	unsigned int		tx_big_pkts;
	unsigned int		tx_sml_pkts;
	unsigned int		tx_big_pkts_pre;
	unsigned int		tx_sml_pkts_pre;
	unsigned int		current_tx_pkt_size;
	unsigned int		veriwave_recover_cnt;
//#endif
#ifdef SUPPORT_TX_SWQ_AMSDU
	int					sml_big_pkt_diff;
#endif
	unsigned int		tx_fail_continue;
	unsigned int		tx_fail_addba_cnt;
	unsigned int		tx_fail_cnt;
	unsigned int		current_tx_rate;
	unsigned int		tx_byte_cnt;
	unsigned int		tx_byte_cnt_odm;
	unsigned int		tx_avarage;
	unsigned int		tx_average_int; // interval
	unsigned int		rx_average_int; // interval
	unsigned int		dl_active;
	unsigned int		ul_active;
	...
	unsigned int		cur_tx_retry_ratio;
	unsigned int		cur_rx_retry_ratio;
	unsigned int		prev_tx_retry_ratio;
#ifdef STA_ROAMING_CHECK
    unsigned int        prev_tx_pkts2;
    unsigned int        prev_tx_retry_pkts2;
    unsigned int        prev_tx_fail_pkts2;
#endif
#endif
   ...
   unsigned char      tv_auto_support; /*0: not support, 1:support*/
	...
	unsigned int   stactrl_trigger_time;
#endif

#if 1//defined(RTK_AC_SUPPORT) || defined(RTK_AC_TX_SUPPORT)
	struct vht_cap_elmt 	vht_cap_buf;
	unsigned int			vht_cap_len;
	struct vht_oper_elmt	vht_oper_buf;
	unsigned int			vht_oper_len;
...
#if (MU_BEAMFORMING_SUPPORT == 1)
#if defined(CONFIG_PE_ENABLE)
	unsigned int		muPartner_aid[MAX_NUM_MU_PARTNER];
#endif
	struct stat_info	*muPartner[MAX_NUM_MU_PARTNER];
	unsigned char		muPartner_num;
	unsigned char		muPartner_num_pre;
	...
	unsigned char		isMUCandidate;
	unsigned int		descLDPCCnt;
...
#if defined(HS2_SUPPORT) || defined(UNIVERSAL_REPEATER) || defined(RTK_ATM) || defined(RTK_TDMA_ATM)
    /* Hotspot 2.0 Release 1 */
	...
	struct mw5_legacy_timer		SA_timer;	//SA_timer: Security Association timer
	...
	unsigned char		bssTransPktSent;
	unsigned char 		bssTransExpiredTime;
	unsigned char		bssTransTriggered;
	unsigned char		bssTransDisassocCountdown;
	unsigned char 		rcvNeighborReport;	
	unsigned char		dual_band_capable;
#endif
#ifdef P2P_SUPPORT
...
//	UINT8		IOTPeer;			// Enum value.	HT_IOT_PEER_E
	...
	#if !defined(RTK_STA_BWC_DIRECT_XMIT)
	struct pkt_queue	sta_bwc_txq;
	unsigned char		sta_bwc_consuming_q;
	#endif
#endif
	enum rf_type  mimo_type_bk;
#endif
...
#endif
	unsigned int  ra_cnt_window[3];
	unsigned char ra_max_window[3];
	unsigned char ra_min_window[3];
	unsigned char ra_cnt;
	unsigned char ra_max;
	unsigned char ra_min;
	unsigned char ra_cur;
	unsigned char is_legacy_USB;
#ifdef TX2_BK_QUEUE
	unsigned char tx2bk_on;
#endif

#ifdef STA_CONTROL_BAND_TRANSITION
	unsigned int  stactrl_steer_detect_counter;  // time (sec) that STA keeps satisfying transition requirement
	int           stactrl_load_balance_score;
	int           stactrl_load_balance_score_sc;
#endif  // STA_CONTROL_BAND_TRANSITION
	...
	unsigned char roaming_indication; //0: rssi > rssi_th, 1: rssi < rssi_th
	unsigned int roaming_waitcount;
#endif

#ifdef RA_OFFSET_TRAINING
	struct ra_offset_st ra_offset; 
#endif

#ifdef RTK_MULTI_AP
	unsigned char last_rssi;
	unsigned short assoc_req_length;
	unsigned char assoc_req_body[512];
	unsigned char multiap_profile;
#endif

#if defined(CONFIG_PE_ENABLE)
	unsigned char *pstat_pe;
	unsigned char *pe_stainfo_t2h_list;//malloc when sta_add
	unsigned int  pe_txpkts_pre;
#if (BEAMFORMING_SUPPORT == 1)
	unsigned int		is_beamforming_entry;
	unsigned int		is_sounding_en;
#endif	
#endif
#ifdef MCR_WIRELESS_EXTEND
	unsigned int		cmw_rx_path_enable;
#endif
#ifdef CONFIG_RTK_WLAN_MANAGER
	unsigned char		supported_band;
#endif
#ifdef HAPD_OWE
...
struct monitor_channel_info {
	unsigned int				ap_count;
	unsigned char				max_rssi;
};
#endif
/*
...
struct extra_stats {
	...
	unsigned long		tx_drops_noasoc;
#ifdef SW_TX_QUEUE	
	unsigned long       swq_enque_pkt;	
	unsigned long       swq_xmit_out_pkt;
	unsigned long       swq_real_enque_pkt;
	unsigned long       swq_real_deque_pkt;
	unsigned long       swq_residual_drop_pkt;
	unsigned long       swq_overflow_drop_pkt;
#endif
	unsigned long       txdaq_overflow_drop_pkt;
#ifdef CONFIG_SPECIAL_ENV_TEST
	unsigned long       spec_start_xmit_pkt;
	unsigned long       spec_netif_rx_pkt;
	unsigned long       netif_rx_pkt;
#endif	
	unsigned long		rx_retrys;
	unsigned long		pre_rx_retrys;
	unsigned long		pre_rx_packets;
	...
	unsigned long		beacon_err_acc;
	unsigned long		beacon_dma_err;    
	unsigned long 		beaconQ_sts;
    unsigned char		is_init_beacon_finished;
	...
	unsigned long		rx_data_queue_drop;
#endif
#ifdef CONFIG_STATISTICS_INFO	
	unsigned int		stats_ofdm_fa[NUM_STATS_ENV_CNT];
	unsigned int		stats_cck_fa[NUM_STATS_ENV_CNT];
#endif	
	unsigned long		freeskb_err;

	unsigned long		rx_refill_alloc_fail;
	unsigned long		rx_refill_alloc_success;
	...
	unsigned long		tx_mgnt_pkts;   // not data frame
	unsigned long		rx_mgnt_pkts;   // not data frame
	...
	unsigned long		rx_break_by_time;
	unsigned long		rx_break_by_pkt;
};
...
struct ss_acs_score {
	unsigned char	channel;
	unsigned int 	score; // per channel 
	unsigned int 	relative_value;
};
...
struct mc2u_reserved_mac_entry{
	unsigned char valid;
	unsigned char macAddr[MACADDRLEN];
};
#endif
...
#if defined(WIFI_QOS_ENHANCE)
struct qos_enhance_sta_s
{
	unsigned char qos_enhance_sta_addr[MACADDRLEN];
};
#endif
...
struct rf_finetune_var {
	...
	unsigned char		ada_stats;
	unsigned int		ada_stats_to;
#endif
	unsigned char		digGoLowerLevel;
	unsigned char		digGoUpperLevel;
	unsigned char		dig_oper_mode;
...
//#endif
	...
	unsigned char		rts_intf_mode;
	unsigned char		rts_intf_dig_th;
	...
	// for watchdog verification
	unsigned char		triggerwdt;
	...
	// for mp test
#if defined(MP_TEST) || defined(__ECOS)
...
#endif
#if defined(USER_RESERVED_MAC_TO_DISABLE_M2U)
	struct mc2u_reserved_mac_entry mc2u_reserved_mac[MAX_RESERVED_MAC_NUM];
	unsigned int		mc2u_reserved_mac_num __attribute__ ((packed));
#endif
	...
	unsigned char		idle_ps_ext;
	unsigned char		idle_txpwr_reduction;
#endif
#ifdef PCIE_POWER_SAVING
	...
	unsigned int		probersp_retry;
	BOOLEAN				dynamic_link_adaptivity;
	BOOLEAN 			adaptivity_enable_flag;
	BOOLEAN				bLinked;
	//	for adaptivity
	...
	unsigned char		ampdu_min_spacing;
	...
	unsigned char		dfs_nhm_duty_cycle_th;
	...
	unsigned int 		dfs_acs_offset;
	unsigned char		dfs_radar_diff_on;
	boolean             nhm_dfs_en;
	unsigned char       nhm_dfs_cnt_th;
	boolean				jp_new_w53;
#if defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8812FE)
	// for 8814b and 8812f mib
	unsigned char		pw_pri_cnt_debug;
	unsigned char 		loop_cnt_th;
    unsigned char		pw_diff_th;		
	unsigned char		loct_diff_th;
	unsigned char		dc_cnt_th;
	unsigned char		dc_judge_left;
	unsigned char		dc_judge_right;
	boolean				adv_pri_byp_en;
	/* Added by Bill */
	unsigned char		pw_lth1;
	unsigned char		pw_uth1;
	unsigned char		pw_lth2;
	unsigned char		pw_uth2;
	unsigned char		pw_lth3;
	unsigned char		pw_uth3;
	unsigned char		pw_lth4;
	unsigned char		pw_uth4;
	unsigned char		pw_lth5;
	unsigned char		pw_uth5;
	unsigned int		pw_lth6;
	unsigned int		pw_uth6;
	unsigned int		pw_lth7;
	unsigned int		pw_uth7;
	unsigned char		pw_lth8;
	unsigned char		pw_uth8;
	unsigned int		pri_lth1;
	unsigned int		pri_uth1;
	unsigned int		pri_lth2;
	unsigned int		pri_uth2;
	unsigned int		pri_lth3;
	unsigned int		pri_uth3;
	unsigned int		pri_lth4;
	unsigned int		pri_uth4;
	unsigned int		pri_lth5;
	unsigned int		pri_uth5;
	unsigned int		pri_lth6;
	unsigned int		pri_uth6;
	unsigned int		pri_lth7;
	unsigned int		pri_uth7;
	unsigned int		pri_lth8;
	unsigned int		pri_uth8;
#if defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8812FE)
	unsigned char		pri_cnt_th;
    unsigned char		pri_var_offset;
	unsigned char		pri_var_tp_offset;
#endif
#endif
	...
	unsigned char		dfs_false_trigger;
	unsigned char		dfs_false_setting;	
	unsigned char		dfs_false_reason_code;
	unsigned char		dfs_false_mask;
	unsigned char		dfs_false_mask_th;
	unsigned char		false_peak_hist1_th;
	unsigned char		false_peak_hist1_th_max;
	unsigned char		false_peak_hist2_th_above0;
	unsigned char		false_peak_hist2_th_above10;
	unsigned char		false_width_hist_th_g4g5;
	unsigned char		false_width_hist_sum_th_g5;
	unsigned char		false_width_hist_th_g0;
	unsigned char		false_width_hist_th_gtotal;
	unsigned char		interval_hist_th;
	unsigned char		false_interval_hist_th_g5;
	unsigned char		pri_width_diff_th;
	unsigned char		pri_width_diff_fcc_th;
	unsigned char		pri_width_diff_fcc_idle_th;
	unsigned char		pri_width_diff_w53_th;
	unsigned char		pri_sum_g5_th;
	unsigned char		pri_sum_safe_th;
	unsigned char		pri_sum_safe_fcc_th;
	unsigned char		pri_type1_low_fcc_th;
	unsigned char		pri_type1_upp_fcc_th;
	unsigned char 		pri_type1_cen_fcc_th;
	unsigned char		pri_sum_g1_th;
	unsigned char		pri_sum_g1_fcc_th;
	unsigned char		pri_sum_g3_fcc_th;
	unsigned char		pri_sum_g5_under_g1_th;
	unsigned char		pw_g0_th;
	unsigned char		pri_std_th;
	unsigned char		pw_std_th;
	unsigned char		pri_std_idle_th;
	unsigned char		pw_std_idle_th;
	unsigned char		pri_sum_type4_th;
	unsigned char		pri_sum_type6_th;
	unsigned char		pulsewidth_long_lower_20m_th;
	unsigned char		pulsewidth_long_lower_th;
	unsigned char		pulsewidth_long_sum_upper_th;
	unsigned char		pri_long_upper_th;
	unsigned char		type4_pw_max_cnt;
	unsigned char		type4_safe_pri_sum_th;
#endif

#ifdef ROMEPERF_SUPPORT
	unsigned char		perf_index;
#endif
	int             	print_sta_txdesc;
	int 				swq_dbg;
#ifdef SW_TX_QUEUE
	int             	swq_en_highthd;
	int             	swq_dis_lowthd;
	...
	int 				swq_udelay2;
	int					swq_aggnum;
	int					manual_swqturboaggnum;
	int					manual_swqturbothd;
	int					swqmaxturbotime;
	int					swq_max_xmit;
	int			swq_ac_logo;
	int					timeout_thd;
	...
	unsigned int		tri_time_rx;

	unsigned int		swq_max_enqueue_len;
	unsigned int		swq_max_len;
#endif
	unsigned int		txdaq_max_enqueue_len;
	unsigned char		icmp_tid;
#ifdef SUPPORT_TX_SWQ_ATM
	int             	swq_atm_en;
	int					swq_atm_txratio_th;
#endif
...
#if defined(WIFI_QOS_ENHANCE)
	unsigned int	qos_enhance_enable;
	unsigned int	qos_enhance_active;
	unsigned int	qos_enhance_rssi_high_thd;
	unsigned int	qos_enhance_rssi_low_thd;
	unsigned int	qos_enhance_tp_low_thd;
	unsigned int	qos_enhance_tp_middle_thd;
	unsigned int	qos_enhance_tp_high_thd;	
	struct qos_enhance_sta_s	qos_enhance_sta[NUM_STAT];
	unsigned int	qos_enhance_rssi_edge_thd;
	unsigned int	qos_edge_enable;
	unsigned int	qos_edca_enable;
	
	unsigned int		qos_enhance_edca_bk[4];
#endif
#ifdef SUPPORT_TX_AMSDU_SMALL_PKTS_ONLY
	unsigned int		small_amsdu_th;
#endif
	unsigned int		amsdu_th;
	unsigned int		amsdu_th2;
	unsigned int		amsdu_pkt_th;
	unsigned int		amsdu_pkt_th2;
	unsigned int		brcm_amsdu_pkt_th;
	unsigned int		brcm_amsdu_pkt_th2;
	unsigned int		amsdu_tcpack_pkt_th;
	unsigned int		mustAmsdu;

	unsigned int		debuglevel;
	unsigned int		dbgcomp;
	unsigned int		rts_rssith;
	unsigned char		amsdu_dm_en;
#ifdef SUPPORT_TX_AMSDU_SMALL_PKTS_ONLY
	unsigned char		amsdusmallpkts_ori;
#endif
	...
	unsigned char		ndpaBW;
	unsigned char		applyVmatrix;
	unsigned char		Nsnding;
	unsigned char		autoRspCsi; // 0:not response CSI report only in TX Nss2 8~9;  1:not reply CSI report in "TX Nss2 8~9 && tx/4 > rx";  2:Not rsp Csi;  3: rsp Csi
...
// power constraint
...
#if defined( RTK_AC_SUPPORT) || defined(CONFIG_WLAN_HAL_8198F)/*eric-8822 add mibs*/
	unsigned int		cts_width;
	unsigned char		gidforce;
	unsigned char		ndpa_mac[6];
	unsigned char		ndpa_swap;
#endif
#ifdef AUTH_SAE/* WPA3_ENT  AUTH_SAE */
	unsigned char		saeforce;
	unsigned int		pmk_expire;
	//unsigned char		gcmpforce;
	//unsigned char		keyforce;
#endif
	...
	unsigned char		mu_hotfix;
	unsigned char       mu_group_mode;

	unsigned char		debug8822;
	unsigned char		txbf8822;
        unsigned char		txbf8814b;
        unsigned char		lock5d1;
	unsigned char		txpause_iqk; 
	unsigned int		manual_group;	
#if (MU_BEAMFORMING_SUPPORT == 1)
	...
	unsigned int 		mutime;
	unsigned char 		mu_rpt;
	unsigned char		mu_dump;
	unsigned char		mu_dump_snr;
	unsigned char		muPairResult[MAX_NUM_BEAMFORMEE_MU];
#ifdef CONFIG_WLAN_HAL_8812FE
	unsigned char		lowerPwr_8812F_MU;
	unsigned char		isSTAGrp;
	unsigned char		isSTAGrp_pre;
#endif
#ifdef CONFIG_VERIWAVE_MU_CHECK	
...
#if (BEAMFORMING_SUPPORT == 1)
	unsigned char		disable_snd;
	unsigned char		manual_snd;
#endif
#if (MU_BEAMFORMING_SUPPORT == 1)
	unsigned int		qtime;
	unsigned int		qlmt;
	unsigned int		dqnum;
	unsigned int		mudqnum;//for PE	
	unsigned int		mu_bdfull_cnt;//for PE
	unsigned int		mu_swqempty_break;//for PE
	...
	u1Byte				beamformee_mu_cnt;
#endif
	unsigned char		force_ndpa_rate;
	unsigned char		force_report_rate;
	unsigned char       set_ndp_rate;
	unsigned char       mu_group_aid;
#endif
	...
	unsigned char		sgi20Mforce; //1: enable this feature, 0: disable
	unsigned char		sgi_vw_force; //1: enable this feature, 0: disable
	unsigned char		ac_tx_force; //1: enable this feature, 0: disable	
	unsigned char		turbo_cckpd;
#ifdef RX_CRC_EXPTIMER
...
//	unsigned char		mp_dig_enable;
	...
	unsigned int		mcr_high_rssi_thd;
	unsigned int		mcr_low_rssi_thd;
	unsigned int		mcr_rxpath_to;
#endif
#ifdef CONFIG_WLAN_HAL_8814AE
	...
	unsigned int		rcr_val;
#ifdef CONFIG_VERIWAVE_CHECK
	unsigned char		vwchk_cov_en;
	unsigned int		vwchk_fa;
	unsigned char		vwchk_rxpkt;	
	unsigned char		veriwave_test;
	unsigned char		dis_veriwave_adaptivity;
#endif
#ifdef THER_TRIM
...
#ifdef RTK_STA_BWC
	/* STA Throughput Bandwidth Control */
	unsigned char		sta_bwc_en;
	unsigned int		sta_bwc_to;
	unsigned int		sta_bwc_dbg_lmt;
	unsigned int		sta_bwc_debug;
#endif
	...
	/* FW Tx Duty Cycle */
	unsigned char		fwduty; // 0: disable, 1: enable FW Tx ducy cycle limitation 
	unsigned char		fwduty_level; // 0 ~ 9, TxDuty = TxDutyTable - level * 10
#endif

#ifdef THERMAL_CONTROL_DAEMON
	unsigned char dbg;
	#if defined(RTK_STA_BWC_LEVEL_AVG)
	unsigned int total_tp; //in kbps
	#endif
#endif /* THERMAL_CONTROL_DAEMON */
#endif /* THERMAL_CONTROL */

	unsigned int		rx_rdu_dis_int_th;
	unsigned int		rx_rdu_dis_isr_th;
#ifdef _CORTINA_
	unsigned int		rx_min_rsvd_mem;
#endif
#if defined(CONFIG_WLAN_HAL_8197F)||defined(CONFIG_WLAN_HAL_8822BE)||defined(CONFIG_WLAN_HAL_8822CE)||defined(CONFIG_WLAN_HAL_8812FE)
	unsigned char		force_bcn;
	unsigned char		beacon_igi;	
#endif

#ifdef TX2_BK_QUEUE
	unsigned char		tx2bk_th_rssi;
	unsigned int		tx2bk_th_rate_11n;
	unsigned int		tx2bk_th_rate_11ac;
#endif

#ifdef CONFIG_SPECIAL_ENV_TEST
	unsigned char		spec_env_test_en; //bit 0: seperate seq number, bit 1: amsdu delay
	unsigned int		spec_env_test_fa;
	unsigned int		spec_pktsz;
	unsigned char       veriwave_interference;
#endif
	unsigned char       orientation_mode;
	unsigned char       interference_mode;
	unsigned int        orientation_fa;
	unsigned int        interference_fa;
	unsigned char       orientation_dig;
	unsigned int        interference_tx_tput;
	unsigned int        interference_tx_pkts;
#ifdef CTC_AP_COEXIST
	unsigned char	ap_coexist_enable;
#endif
#ifdef SMP_LOAD_BALANCE_SUPPORT
	unsigned int		xmit_dsr_time;
	int                 expire_timer_cpuid;
	unsigned int		skb_xmit_queue_num;
	unsigned int		xmit_dsr_num;
#endif
	unsigned int		limit_rxloop_time;		// by time, which in units of ms
	unsigned int		limit_rxloop;			// by packet
	unsigned int		limit_proc_all_queue;   // by packet

	unsigned int		acs_not60_inc;
#ifdef MP_ADC_SUPPORT
	int 	mp_adc_stats;
	int 	mp_adc_idx;
	char 	mp_adc_buf[328][500];
#endif
#if defined(CONFIG_PE_ENABLE)
    unsigned char manual_pe_enable;
#ifdef SW_TX_QUEUE
	int		pe_swq_enable;
#endif
#endif
    unsigned char           cn_acs_iot_enable;
#if defined(CONFIG_RTL_OFFLOAD_DRIVER) 
unsigned char dc_bcn_mode;
#endif
#ifdef SUPPORT_TX_SWQ_AMSDU
	unsigned char	amsdusmallpkts;
#endif
	unsigned char	TxPath;
	unsigned char	RxPath;
	unsigned char	bTx2Path;
...
#define ACCESS_SWAP_IO		0x01	/* Do byte-swap in access IO register */
...
enum {
    ...
    TYPE_PCI_BIOS_DT	= 3		/* PCI wlan controller and enable PCI by BIOS */
};
...
#ifdef STA_ASSOC_STATISTIC
struct sta_assoc_act {
	unsigned char 			step;
	unsigned char 			status;	
	unsigned char 			rssi;		
	unsigned char			Reserved[MACADDRLEN];
	struct tm				time_info;
	struct list_head		act_list;	
};

struct sta_assoc_record {
	unsigned char			addr[MACADDRLEN];
	unsigned char 			used;
	struct list_head		act_list;
	struct sta_assoc_act	action[MAX_ASSOC_STA_ACT];
	struct list_head		act_pool;
};
#endif
struct sta_mac_rssi {
	unsigned char			channel;
...
struct monitor_station {
	unsigned char				addr[MACADDRLEN];
	unsigned char				ssid[32];
	unsigned short				ssid_len;
	unsigned char				rssi;	
	unsigned char 				used;
	unsigned char 				type;			// 1:ap, 2: sta
	unsigned char 				channel;				
	unsigned int 				txbyte;
...
struct data_que {
	...
	int head;
	int tail;
	struct rx_frinfo *ring[MAX_SKB_NUM];
};
#endif

#define MAX_TX_SKB_NUM      8194
#ifdef TX_PROCESS_REFINE

struct tx_data_que {
	int head;
	int tail;
	struct sk_buff* ring[MAX_TX_SKB_NUM];
};
#endif
...
#ifdef STA_CONTROL_BAND_TRANSITION  // Use the same rssi threshold for band steering and band transition
#define STACTRL_PREFERBAND_RSSI		        (priv->pmib->staControl.stactrl_rssi_th_high) // 60
#define STACTRL_PREFERBAND_BLOCK_RSSI		(priv->pmib->staControl.stactrl_rssi_th_low)  // 20
#else
#define STACTRL_PREFERBAND_RSSI				(priv->pmib->staControl.stactrl_param_1) // 25
#define STACTRL_PREFERBAND_BLOCK_RSSI		(priv->pmib->staControl.stactrl_param_1 - priv->pmib->staControl.stactrl_param_2)
#endif
...
#define STACTRL_BLOCK_EXPIRE_X			(priv->pmib->staControl.stactrl_param_3) //300
#define STACTRL_BLOCK_RETRY_Y			(priv->pmib->staControl.stactrl_param_4) //  3
#define STACTRL_BLOCK_EXPIRE_Z			(priv->pmib->staControl.stactrl_param_5) // 20
#define STACTRL_NONPREFERBAND_RSSI		(priv->pmib->staControl.stactrl_param_6) // 60
#if STA_CONTROL_ALGO == STA_CONTROL_ALGO2
#define STACTRL_NON_PREFER_TIMER		100 // ms
#elif STA_CONTROL_ALGO == STA_CONTROL_ALGO3
#define STACTRL_KICKOFF_TIMES			(priv->pmib->staControl.stactrl_param_7)  //20170103 (2)
...
#define STACTRL_HIDDENAP_EXPIRE			(priv->pmib->staControl.stactrl_param_10) //20171124
#define STACTRL_BLOCK_5G				(priv->stactrl.stactrl_block5G)           //20171222
#define STACTRL_BLOCK_EXPIRE			10     //sec
#define STACTRL_KICKOFF_TIME			180    //sec  
#define STACTRL_KICKOFF_TP				625000 //byte
#define STACTRL_TRIGGER_TIME			20     //sec
#endif
...
struct stactrl_preferband_entry {
	...
	unsigned int			sta_not_in_preferband;	
	unsigned int 			sta_in_prefer_band;
	unsigned int			sta_in_non_preferband;
#else
	unsigned char			blockRetry;
	int						blockRetry_resetTimer;
	unsigned int			block_timer;
#endif	
...
struct stactrl_data {
...
#if STA_CONTROL_ALGO == STA_CONTROL_ALGO2
    struct mw5_legacy_timer        stactrl_timer;    
#endif
    ...
    unsigned char			stactrl_block5G;		   // 0: disable block_5G feature,	1: enable block_5G feature
    unsigned char			stactrl_enable_hiddenAP;   // 0: disable hiddenAP feature,	1: enable hiddenAP feature

	unsigned char           stactrl_hiddenAP_aging;
};
#endif //STA_CONTROL
...
#define MAC_CLONE_MBIDCAM_START_V1	(MAX_MBIDCAM_NUM*8 - MAX_MAC_CLONE_NUM*8)
...
#ifdef MBSSID
#define MAX_MBIDCAM_RESERVED (RTL8192CD_NUM_VWLAN + 2)
#else
#define MAX_MBIDCAM_RESERVED    2
#endif
...
struct mclone_sta_info {
	...
	struct mw5_legacy_timer	reauth_timer;
	int					reauth_count;
	struct mw5_legacy_timer	reassoc_timer;
	int					reassoc_count;
	int 				auth_seq;
	unsigned int		opmode;
...
struct channel_utilization_info {
    ...
    int                     chbusytime;
    struct mw5_legacy_timer       cu_cntdwn_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	cu_cntdwn_timer_event;
#endif
};
#endif
...
#define NUM_MONITOR             32

struct monitor_sta_entry {
	unsigned int valid;
	unsigned int isAP;
	unsigned char mac[6];
	unsigned char rssi;
...
{
	int sta_entry_num;
	struct monitor_sta_entry monitor_sta_ent[NUM_MONITOR];
};
#endif
...
struct deauth_sta_info {
    unsigned char mac_addr[MAX_DEAUTH_STA_NUM][MACADDRLEN];
    unsigned long issue_time[MAX_DEAUTH_STA_NUM];
    unsigned char head;
    unsigned char tail;
};

struct smh_cam_info{
    unsigned char macid:7;
...
// temp mark by Eric
	...
	struct mw5_legacy_timer xmit_check_timer;
	...
	struct mw5_legacy_timer chk_h2c_buf_timer;
#endif
	...
	struct mw5_legacy_timer beacon_timer;
	struct timer_event_entry beacon_timer_event;
	#if defined(CONFIG_ADAPTIVE_SOML)
    struct timer_event_entry adaptive_soml_event;
	#endif
	struct timer_event_entry tdma_dig_event;
#endif
#endif // CONFIG_USB_HCI

#ifdef WLAN_HAL_HW_TX_SHORTCUT_HDR_CONV
    struct smh_cam_info SmhInfo[64];//header conversion hw entry num is 64
#endif
	...
	u8 ps_ctrl;
	u8 pwr_state;
	struct mw5_legacy_timer ps_timer;
	...
	struct mw5_legacy_timer PktAging_timer;
#endif
...
#ifdef CONFIG_PCI_HCI
	struct rtk_tasklet	rx_tasklet;
	struct rtk_tasklet	tx_tasklet;
#ifdef WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD
	struct rtk_tasklet	rxq_tasklet;
#endif
#ifndef __ECOS
	struct tasklet_struct	oneSec_tasklet;
#ifdef AP_SWPS_OFFLOAD
    struct tasklet_struct	forcerecycle_tasklet;
#endif
#if defined(SMP_LOAD_BALANCE_SUPPORT) && !defined(CONFIG_USB_HCI)
	struct tasklet_struct	xmit_tasklet;
#endif
#endif
#endif
	...
	/* Legacy v43 DMA anchor, retained for non-linux6 builds. */
	struct device			*mw5_dma_dev;
#endif
#ifdef CONFIG_RTL8192CD_LINUX6_BUS
	/* Real bus device used for DMA and lifetime ownership on Linux 6.x. */
	struct device			*linux_dev;
	const void			*linux6_calibration;
	size_t				linux6_calibration_len;
	const void			*linux6_factory;
	size_t				linux6_factory_len;
	u8				linux6_radio_index;
	bool				linux6_irq_requested;
#endif
#ifdef CONFIG_PCI_HCI
    ...
    unsigned char		    phy_reg_buf[MAX_REG_FILE_SIZE];
#elif defined(WLAN_READ_FILE_REFINE)
	unsigned char			reg_file_buf[MAX_REG_FILE_SIZE];
#else
    unsigned char			phy_reg_buf[PHY_REG_SIZE];
#endif
#ifdef WLAN_RSV_BUF
	unsigned char			*rsv_buf;
#endif
...
#ifdef TXPWR_LMT
#if !defined(WLAN_REG_FW_RAM_REFINE)
	...
	unsigned char			tgpwr_CCK_NEW;
	unsigned char			tgpwr_OFDM_NEW;
	unsigned char			tgpwr_HT1S_NEW;
	unsigned char			tgpwr_HT2S_NEW;
	unsigned char			tgpwr_HT3S_NEW;
	unsigned char			tgpwr_HT4S_NEW;	
	unsigned char			tgpwr_VHT1S_NEW;
	unsigned char			tgpwr_VHT2S_NEW;
    unsigned char           tgpwr_VHT3S_NEW;
    unsigned char           tgpwr_VHT4S_NEW;
#ifdef RTK_MULTI_AP
    unsigned char			tgpwr_HT1S_NEW_offset;
#endif
#endif	
#ifdef _TRACKING_TABLE_FILE
...
#ifdef SBWC
	spinlock_t				lock_sbwc;
	int                     lock_sbwc_owner;
#endif
#if defined(RTK_STA_BWC) && !defined(RTK_STA_BWC_DIRECT_XMIT)
	spinlock_t				lock_sta_bwc;
	int                     lock_sta_bwc_owner;
#endif

#if (!(defined(__ECOS) && defined(CONFIG_SDIO_HCI)))
	spinlock_t				rc_packet_q_lock;
	int rc_packet_q_lock_owner;
#endif

	spinlock_t				txdata_queue_enqueue_lock;
	spinlock_t				indirect_read_write_lock;
#endif
...
#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8192FE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8198F)|| defined(CONFIG_WLAN_HAL_8197G)
...
//	unsigned short		fw_src_version;
...
#ifdef CONFIG_WLAN_HAL
	unsigned int			h2c_box_to_first;
	unsigned int			h2c_box_to_second;
	unsigned int			h2c_box_flag;
#endif
	...
	unsigned int			ss_score_timeout_count;
#ifdef SW_TX_QUEUE
    #if !defined(__ECOS) && !defined(__OSK__)
	struct rtk_tasklet	    swq_tasklet;
#if defined(SMP_LOAD_BALANCE_SUPPORT) && defined(CONFIG_RTL9607C) && defined(CONCURRENT_MODE)
	struct call_single_data     swq_tasklet_csd;
#endif
#ifdef TX_PROCESS_REFINE
	struct call_single_data     txq_tasklet_csd;
#endif
    #endif
#endif
#if defined SMP_LOAD_BALANCE_SUPPORT
	struct call_single_data     smp_assign_expire_timer_csd;
#endif
#ifdef WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD
	struct call_single_data     rxq_tasklet_csd;
#endif

#ifdef SUPPORT_TXDESC_IE
    unsigned char       txdesc_trans_err_status;
#endif
...
#endif
#ifdef VXD_DIFF_MAC
	unsigned char		use_vxd_diff_mac;
#endif

#if defined(RTK_NL80211)
	volatile struct cfg80211_chan_def *dfs_chan_def;
#ifdef CONFIG_RTL8192CD_LINUX6_BUS
	struct work_struct cfg80211_work;
#else
	struct tasklet_struct cfg80211_tasklet;
#endif
	struct list_head cfg80211_queue;
	spinlock_t cfg80211_queue_lock;
#endif
#ifdef CONFIG_RTL8192CD_LINUX6_BUS
	/* Linux 6.x RX bottom half: one NAPI instance per physical radio. */
	struct napi_struct linux6_rx_napi;
	struct rtl8192cd_priv *linux6_root_priv;
	int linux6_rx_budget;
	int linux6_rx_work_done;
	bool linux6_napi_polling;
	bool linux6_napi_added;
	bool linux6_napi_enabled;
#if defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
	/* v44.24.1: RX DMA lifecycle counters belong to the shared radio state. */
	u64 linux6_rx_dma_maps;
	u64 linux6_rx_dma_unmaps;
	u64 linux6_rx_dma_map_errors;
	u64 linux6_rx_direct_bus;
	u64 linux6_rx_direct_sync_device;
	u64 linux6_rx_direct_sync_cpu;
	/* v44.25: prove real interrupt-driven RX without unsafe forced polling. */
	u64 linux6_irq_count;
	u64 linux6_rx_irq_count;
	u64 linux6_napi_schedules;
	u64 linux6_napi_polls;
	/* v44.30: integrated RTL8197FS can assert the outer IRQ while its HAL
	 * RX status bits stay clear.  Only real recognized HW IRQs may trigger
	 * this one-shot NAPI fallback; there is no periodic polling worker. */
	u64 linux6_rx_irq_fallback_count;
	/* v44.58: descriptor-aware 8197FS fallback suppression. */
	u64 linux6_rx_irq_fallback_suppressed;
	/* v44.59: periodic safety probe prevents a stale producer register from
	 * suppressing RX forever while keeping the v44.58 CPU-load reduction. */
	u64 linux6_rx_irq_fallback_probe;
	u64 linux6_rx_pending_checks;
	u64 linux6_napi_work_done;
	/* v44.59: distinguish real RX integrity/refill faults from the old
	 * misleading delayed-refill "allocate failure" printk. */
	u64 linux6_rx_icv_errors;
	u64 linux6_rx_refill_alloc_failures;
	u64 linux6_c2h_oversize_reclassified;
	/* v44.33: final receive-filter snapshots after the complete HAL init. */
	u32 linux6_rcr_after_init;
	u32 linux6_rxfltmap0_after_init;
	u32 linux6_rxfltmap2_after_init;
	/* v44.33/v44.34: integrated RTL8197FS coherent RXBD ring and final
	 * post-calibration RX-engine/descriptor snapshots. */
	u32 linux6_rxbd_dma_base;
	u32 linux6_rxbd_hw_base;
	u32 linux6_cr_after_init;
	u32 linux6_sys_func_en_after_init;
	u32 linux6_rf_ctrl_after_init;
	u32 linux6_afe_ctrl4_after_init;
	u32 linux6_afe_ldo_ctrl_after_init;
	u32 linux6_rxbd_idx_after_init;
	u32 linux6_rxbd0_dw0;
	u32 linux6_rxbd0_dw1;
	/* v44.34: 8822B TXBD/TXDESC rings are coherent DMA. Dynamic MGT
	 * header/payload buffers remain Linux streaming-DMA mapped and are
	 * reclaimed symmetrically after hardware completion. */
	u32 linux6_txbd_dma_base;
	u32 linux6_txbd_hw_base;
	u32 linux6_txdesc_dma_base;
	u32 linux6_bcnbd_dma_base;
	u32 linux6_bcnbd_hw_base;
	u32 linux6_bcndesc_dma_base;
	u32 linux6_mgq_idx_after_init;
	u64 linux6_tx_dma_maps;
	u64 linux6_tx_dma_unmaps;
	u64 linux6_tx_dma_map_errors;
	u64 linux6_tx_dma_mgt_maps;
	u64 linux6_tx_dma_data_maps;
	/* v44.54: RTL8822B management/high queue completion proof. */
	u64 linux6_mgntdok_irq_count;
	u64 linux6_highdok_irq_count;
	u64 linux6_critical_hi0_tx;
	/* v44.55: the MW5 PCIe integration does not reliably deliver MGT/HI0
	 * DOK interrupts.  Count safe polls of the existing HAL TX recycler. */
	u64 linux6_tx_reclaim_kicks;
	/* v44.56: submission-driven MGT/HI0 reclaim and live RW pointer proof. */
	u64 linux6_tx_reclaim_tx_kicks;
	/* v44.58: adaptive data-queue reclaim for the 8822B PCIe radio. */
	u64 linux6_tx_data_submits;
	u64 linux6_tx_reclaim_data_kicks;
	/* v44.58: safe producer-index re-doorbell for proven TXBD backlog. */
	u64 linux6_tx_redoorbell_kicks;
	u64 linux6_tx_mgt_redoorbell_kicks;
	u64 linux6_tx_data_redoorbell_kicks;
	u32 linux6_mgq_rwptr;
	u32 linux6_bkq_rwptr;
	u32 linux6_beq_rwptr;
	u32 linux6_viq_rwptr;
	u32 linux6_voq_rwptr;
	u32 linux6_hi0_rwptr;
	u64 linux6_txdma_errors;
	u64 linux6_txdma_qsel_diff;
	u32 linux6_txdma_last_status;
	/* v44.25: RTL8197FS beacon payload lives in coherent DMA memory. */
	void *linux6_beacon_dma_buf;
	dma_addr_t linux6_beacon_dma_addr;
	size_t linux6_beacon_dma_size;
	u64 linux6_beacon_dma_updates;
	u64 linux6_beacon_dma_alloc_errors;
	/* v44.44: cfg80211/RTL8197FS beacon + SDK RF18 proof. */
	u32 linux6_beacon_payload_mode; /* 0=legacy/direct fallback, 1=coherent DMA */
	u32 linux6_beacon_payload_bus;
	u32 linux6_beacon_txbd0_dw0;
	u32 linux6_beacon_txbd0_dw1;
	u32 linux6_beacon_txbd1_dw0;
	u32 linux6_beacon_txbd1_dw1;
	u32 linux6_beacon_txdesc_dw0;
	u32 linux6_beacon_txdesc_dw1;
	u32 linux6_beacon_txdesc_dw2;
	u32 linux6_beacon_txdesc_dw3;
	u32 linux6_beacon_txdesc_dw4;
	u32 linux6_beacon_txdesc_dw5;
	u32 linux6_rf18_a;
	u32 linux6_rf18_b;
	/* v44.45.1: proof of the MW5 binary power sequence and final physical
	 * 8197FS MAC/BB/RF/FEM state after cfg80211 re-open. */
	u32 linux6_mw5_binary_pwrseq;
	u32 linux6_pwrseq_a0;
	u32 linux6_pwrseq_a1;
	u32 linux6_pwrseq_a3;
	u32 linux6_pwrseq_05;
	u32 linux6_pwrseq_06;
	u32 linux6_soc_wlan_gate;
	u32 linux6_gpio_muxcfg;
	/* v44.46: RFE5 GPIO8/TRSW0 pinmux is reasserted after the generic HAL
	 * clears REG_GPIO_MUXCFG during MAC init/reopen. */
	u64 linux6_trsw_reasserts;
	/* v44.46 legacy diagnostic. v44.47 no longer redirects EAPOL queues;
	 * keep the counter exported so reports can prove it remains zero. */
	u64 linux6_eapol_mgt_redirects;
	/* v44.48: preserve OEM RTL8822B EAPOL queue/rate/privacy and capture
	 * the exact frame-local V1 TXDESC/TXBD plus SW/HW queue indices. */
	u64 linux6_eapol_oem_tx;
	u64 linux6_eapol_desc_fills;
	u64 linux6_eapol_syncs;
	u32 linux6_eapol_last_q;
	u32 linux6_eapol_last_rate;
	u32 linux6_eapol_last_privacy;
	u32 linux6_eapol_last_fixed;
	u32 linux6_eapol_desc_q;
	u32 linux6_eapol_desc_idx;
	u32 linux6_eapol_desc_dw0;
	u32 linux6_eapol_desc_dw1;
	u32 linux6_eapol_desc_dw2;
	u32 linux6_eapol_desc_dw3;
	u32 linux6_eapol_desc_dw4;
	u32 linux6_eapol_desc_dw5;
	u32 linux6_eapol_txbd0_dw0;
	u32 linux6_eapol_txbd0_dw1;
	u32 linux6_eapol_txbd1_dw0;
	u32 linux6_eapol_txbd1_dw1;
	u32 linux6_eapol_txbd2_dw0;
	u32 linux6_eapol_txbd2_dw1;
	u32 linux6_eapol_host_before_fill;
	u32 linux6_eapol_host_after_fill;
	u32 linux6_eapol_host_before_sync;
	u32 linux6_eapol_host_after_sync;
	u32 linux6_eapol_hw_before_sync;
	u32 linux6_eapol_hw_after_sync;
	u32 linux6_eapol_rwptr_before_sync;
	u32 linux6_eapol_rwptr_after_sync;
	/* v44.50: frame-exact EAPOL header/LLC/payload metadata and station TX report proof. */
	u32 linux6_eapol_last_macid;
	u32 linux6_eapol_last_tid;
	u32 linux6_eapol_last_hdr_len;
	u32 linux6_eapol_last_llc_len;
	u32 linux6_eapol_last_payload_len;
	u32 linux6_eapol_need_ack;
	u32 linux6_eapol_hdr_dw0;
	u32 linux6_eapol_hdr_dw1;
	u32 linux6_eapol_hdr_dw2;
	u32 linux6_eapol_hdr_dw3;
	u32 linux6_eapol_hdr_dw4;
	u32 linux6_eapol_hdr_dw5;
	u32 linux6_eapol_hdr_dw6;
	u32 linux6_eapol_hdr_dw7;
	u32 linux6_eapol_hdr_dw8;
	u32 linux6_eapol_payload_dw0;
	u32 linux6_eapol_payload_dw1;
	u32 linux6_eapol_payload_dw2;
	u32 linux6_eapol_payload_dw3;
	u64 linux6_eapol_txrpt_reports;
	u64 linux6_eapol_txrpt_txok_sum;
	u64 linux6_eapol_txrpt_txfail_sum;
	u32 linux6_eapol_txrpt_last_macid;
	u32 linux6_eapol_txrpt_last_txok;
	u32 linux6_eapol_txrpt_last_txfail;
	u32 linux6_eapol_txrpt_last_rate;
	/* v44.48: RTL8197FS beacon DMA ownership/consumption proof. */
	u64 linux6_beacon_signins;
	u64 linux6_beacon_enable_calls;
	u64 linux6_beacon_disable_calls;
	u64 linux6_beacon_hw_consume_seen;
	u32 linux6_beacon_last_action; /* 0=none, 1=enable, 2=disable */
	u32 linux6_beacon_own_at_signin;
	u32 linux6_beacon_own_before_set;
	u32 linux6_beacon_own_after_set;
	u32 linux6_beacon_pslen;
	u32 linux6_beacon_txbd_bus;
	u32 linux6_beacon_txdesc_bus;
	u32 linux6_beacon_bcnq_desa;
	/* v44.47: final RTL8197FS RFE5 Type5 TXAGC register proof. */
	u32 linux6_txagc_e00;
	u32 linux6_txagc_e04;
	u32 linux6_txagc_e08;
	u32 linux6_txagc_e10;
	u32 linux6_txagc_e14;
	u32 linux6_txagc_e18;
	u32 linux6_txagc_e1c;
	u32 linux6_txagc_830;
	u32 linux6_txagc_834;
	u32 linux6_txagc_838;
	u32 linux6_txagc_83c;
	u32 linux6_txagc_848;
	u32 linux6_txagc_84c;
	u32 linux6_txagc_868;
	u32 linux6_txagc_86c;
	u32 linux6_txpause;
	u32 linux6_bb40;
	u32 linux6_bb4c;
	u32 linux6_bb64;
	...
	int rtk_remain_on_channel;
	struct mw5_legacy_timer remain_on_ch_timer;
	u8 remain_on_ch_u8;
...
#ifdef ADDRESS_CAM
    unsigned char		addr_cam_used[ADDR_CAM_MAP_LEN];
#endif
	unsigned int		set_led_in_progress;
	unsigned char       swr_mode; /* 0:sps mode, 1:ldo mode */

#if defined (HW_ANT_SWITCH) && !defined(CONFIG_RTL_92C_SUPPORT) && !defined(CONFIG_RTL_92D_SUPPORT)
	unsigned char		ant_orig;
#endif

	struct rtl8192cd_priv	*ifpriv[RTL8192CD_NUM_VWLAN+1];
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
    int						dev_core_state;
    unsigned int			rx_buf_len;
	unsigned char		    offload_en;
#endif
	...
	struct mw5_legacy_timer		LED_Timer;		// for SW LED
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	LED_Timer_event;
#endif
	struct mw5_legacy_timer		rc_sys_timer;
#ifdef SW_TX_QUEUE
	struct mw5_legacy_timer		swq_sw_timer;
#endif
	struct mw5_legacy_timer		amsdu_sw_timer;
#ifdef ADAPTIVITY_STATS_DEBUG
	struct mw5_legacy_timer		ada_stats_timer;
#endif

#if (BEAMFORMING_SUPPORT == 1)
	struct mw5_legacy_timer		BeamformingTimer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	BeamformingTimer_event;
#endif
	struct mw5_legacy_timer		BFSoundingTimeoutTimer;
#endif

	/*********************************************************
	 * from here on, data will be clear in rtl8192cd_init_sw() *
	 *********************************************************/
#ifndef CONFIG_WIFI_LED_SHARED		// check
	unsigned int			share_info_clear_start;
#endif
...
#ifdef CONFIG_WIFI_LED_SHARED		// check
	unsigned int			share_info_clear_start;
#endif
...
#ifdef CONFIG_SPECIAL_ENV_TEST
	unsigned char		in_spec_env_test_to;
	unsigned char       spec_env_test_chg;
	unsigned short		spirent_sta_num;
	unsigned short		veriwave_sta_num;
	unsigned char      	veriwave_noisy_mode;
#endif
#ifdef SMP_SYNC
	int                 expire_timer_cpuid;
#endif
	...
	unsigned char			thermal_value;
	unsigned char			thermal_value_lck;
	unsigned char			thermal_value_iqk;
#if defined(CONFIG_WLAN_HAL_8814AE)
	...
	struct mw5_legacy_timer		DPKTimer;
	...
	unsigned int			Reg664_pre;
#endif
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT) || defined(CONFIG_RTL_88E_SUPPORT)
	//for IQK
	unsigned int			RegC04;
	unsigned int			Reg874;
	unsigned int			RegC08;
	...
	unsigned char	thermal_value_avg[AVG_THERMAL_NUM];
	unsigned char	thermal_value_avg_index;
	unsigned char	ThermalValue_RxGain;
	unsigned char	ThermalValue_Crystal;
	unsigned char	ThermalValue_DPKstore;
	unsigned char	ThermalValue_DPKtrack;
	...
	#if defined(CONFIG_RTL_88E_SUPPORT) && !defined(CALIBRATE_BY_ODM) /*for 88e tx power tracking*/
	struct _IQK_MATRIX_REGS_SETTING IQKMatrixRegSetting[IQK_MATRIX_SETTINGS_NUM];
	#endif
	...
	// MP DIG
#ifdef CONFIG_RTL_92D_SUPPORT		
	struct mw5_legacy_timer		MP_DIGTimer;
#endif
...
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8881A) || defined(CONFIG_WLAN_HAL_8814AE)  || defined(CONFIG_RTL_8723B_SUPPORT) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8812FE)  /*for 8812 tx power tracking*/
	unsigned char	Power_tracking_on_8812;
	unsigned char	thermal_value_avg_8812[AVG_THERMAL_NUM_8812];
	unsigned char	thermal_value_avg_index_8812;
#endif
	...
	unsigned char	thermal_value_avg_88xx[AVG_THERMAL_NUM_88XX];
	unsigned char	thermal_value_avg_index_88xx;
...
//#endif
	...
	unsigned char	IGI_A_bk;
	unsigned char	IGI_B_bk;
#endif

#ifdef BSS_MONITOR_SUPPORT
	struct monitor_station     monitor_sta[MAX_MONITOR_STA];
	int						   ap_count;
#endif
#ifdef NHM_ACS2_SUPPORT
	struct monitor_channel_info		acs_info[MAX_CHANNEL_NUM];		// ap count and max rssi per channel
	struct monitor_channel_info		currch_info;					// ap count and max rssi in current channel
#endif	
#if defined(TXREPORT)
	int sta_query_idx;
	int sta_query_idx_pre;
#endif
#ifdef TXRETRY_CNT
...
#endif
#if (BEAMFORMING_SUPPORT == 1)
	struct _RT_BEAMFORMING_INFO	BeamformingInfo;
	...
	atomic_t			rx_enqueue_cnt;
	unsigned long		rx_rdu_dis_int;
#endif
	...
	unsigned int			tx_dma_status_bak;			
#else
	...
	unsigned int			datacpu_timer_cnt;
	unsigned int			datacpu_keep_alive_cnt;
#endif
#endif
...
#if defined(CONFIG_WLAN_HAL_8814AE)|| defined(CONFIG_WLAN_HAL_8814BE)|| defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8814BE)
...
//arx parameters
...
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8192FE) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8812FE) || defined(CONFIG_WLAN_HAL_8197G)
	UINT64					mp_rx_mac_ok;
	...
	unsigned long			check_MACBBTX_counter_pre;
	unsigned int			vw_dig_cnt;
...
//	unsigned int			pkt_in_hiQ;
...
#ifdef SMP_LOAD_BALANCE_SUPPORT
#ifdef TX_PROCESS_REFINE
	struct tx_data_que skb_xmit_queue[MAX_SKB_XMIT_QUEUE];
	atomic_t                txdata_queue_dequeuing;
#else
	struct sk_buff_head	skb_xmit_queue[MAX_SKB_XMIT_QUEUE];
#endif
	atomic_t				skb_xmit_queue_len;
	unsigned short			skb_xmit_queue_idx;
	unsigned short			skb_xmit_queue_num;
	unsigned int			max_xmit_dsr_skb;
	unsigned int			max_xmit_dsr_time;
	unsigned int			xmit_dsr_redo;
#endif
	atomic_t				tx_recycle_trigger;

#ifdef CONFIG_RG_NETIF_RX_QUEUE_SUPPORT
    struct sk_buff_head	rg_netif_rx_queue;
#endif

    struct reorder_ctrl_timer	rc_timer_pool[RC_TIMER_NUM];
	struct list_head		rc_timer_list;
	...
	UINT32                      timer2_last_tsf;
	unsigned long				amsdu_to_pkt;		// AMSDU xmit via amsduTimeout
	unsigned long				amsdu_chk_pkt;		// AMSDU xmit via amsduCheck
...
#endif
#ifdef SMP_LOAD_BALANCE_SUPPORT
	unsigned int				swq_timeout_seq;
#endif
#endif

#ifdef SUPPORT_TX_SWQ_ATM
	int							swq_atm_state;
	struct stat_info*			swq_atm_pmax_stat;
#endif
	unsigned char				amsdu_use_hw_timer;
	unsigned int				amsdu_cnt_timeout;

#ifdef RTK_ATM
    UINT32                      atm_swq_current_timeout;
    struct mw5_legacy_timer           atm_swq_sw_timer;    
...
#ifdef RTK_TDMA_ATM
			unsigned int	atm_ttl_stanum;//sta num of 2.4G/5G interface
			unsigned int	atm_ttl_active_stanum;	
			unsigned int	atm_ttl_match_stanum;// num of cli that in client list of interface 2.4G/5G
			unsigned int	atm_ttl_match_statime;// total matched sta time
			unsigned int	atm_sta_time_min;
			unsigned int	atm_sta_time_max;
			unsigned int	atm_sta_txrate_max;
			unsigned int	atm_max_rate;
			unsigned int	tx_avarage_max;
	
			unsigned char	tdma_last_polling_is_ap;//0:sta-to-ap; 1:ap-to-sta; 2-first polling 
			unsigned short	tdma_last_polling_macID;
			unsigned char	tdma_last_polling_aid;
			unsigned char	hwpolling_count;
			unsigned char	tdma_swq_en;
			unsigned int	q_len[4];	
			unsigned char	pq;
			unsigned int	hw_queue_num;
			unsigned int	cal_flag;
			unsigned long	pause_count;
	
	#ifndef __ECOS
			struct tasklet_struct		tdma_tasklet;
	#endif
...
//	STA_BITMAP			has_2r_sta; // Used when AP is 2T2R. bitmap of 2R aid
...
#ifdef AP_SWPS_OFFLOAD
    int						has_triggered_forcerecycle_tasklet;
#endif
#ifdef __ECOS
	int				call_dsr;
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
    unsigned short          dc_support_sta_num;
    unsigned short			dc_free_sta_space;
    unsigned char			cil_pkt_id_tx;
    unsigned char			cil_pkt_id_rx;
    unsigned char			cil_waiting_id;
    unsigned int			cil_flags;
    unsigned int			cil_pktid_unsync_cnt;
	unsigned short          current_dc_bcn_mode;
#endif
	...
	unsigned char			active_thru_mode;	
	unsigned int			HW_CRC_error_cnt;
	unsigned int			HW_CRC_err_FCS_ok_cnt;
...
//	unsigned short			F90_cnt;
	...
	struct mw5_legacy_timer	swAntennaSwitchTimer;
...
#if defined(DFS) || defined(RTK_AC_SUPPORT) || defined(CSA2G)
	...
	unsigned int	dl_active_sta_num;
	unsigned int	ul_active_sta_num;
	...
	struct mw5_legacy_timer	vxd_ibss_beacon; //timer to issue beacon for vxd_adhoc
#endif
	...
	unsigned char		idle_period_test;
	unsigned int		idle_period_reg_bak[2];
    ...
    struct deauth_sta_info    deauth_sta_pool; 

#ifdef ADAPTIVITY_STATS_DEBUG
	unsigned char		ada_stats_prev;
	unsigned int		ada_stats_total;
	unsigned int		ada_stats_raise;
	int					ada_stats_ratio;
#ifdef SMP_SYNC
	spinlock_t			ada_stats_lock;
#endif
#endif

#ifdef RA_OFFSET_TRAINING
	unsigned char	adjacent_intf;
	unsigned char	env_noisy;
	unsigned int	ra_training_tp[RA_OFFSET_TRAINING_STEPS];
	unsigned char	ra_training_to;
	unsigned char	ra_training_step;
	unsigned char	ra_offset_idx;
#endif
#ifdef CONFIG_RTL_MONITOR_STA_INFO
	struct monitor_sta monitor_sta_info;
#endif
...
#endif
#ifdef CTC_AP_COEXIST
	unsigned char	rtk_ap_detected_to;
	unsigned char	interf_mode;
#endif
#ifdef CONFIG_DIAG_WIFI
	/*AR018*/
	unsigned int arp_req_cnt1;
	unsigned int arp_req_cnt2;
	unsigned int arp_resp_cnt1;
	unsigned int arp_resp_cnt2;
	unsigned int free_and_stop1;
	unsigned int free_and_stop2;
#endif
	/*right now just add for report 2G channels, if 5G CHs needed enlarge it*/
	struct env_mntr_rpt clm_nhm_rtp[14];
...
struct beacon_vsie {
	...
	unsigned int	duration_count;
	unsigned char	data_len;
	unsigned char	data[BEACON_VSIE_LEN];
};
...
struct probersp_vsie {
	unsigned char	mac[6];
	unsigned char	data_len;
	unsigned char	data[BEACON_VSIE_LEN];
	unsigned char	service;
	struct ifname_info if_info;
};
#endif
...
typedef struct rtl8192cd_priv {
    ...
    struct halmac_adapter *pHalmac_adapter;
    //PHALMAC_API                      pHalmac_api;
    struct halmac_api *pHalmac_api;
    //PHALMAC_PLATFORM_API             pHalmac_platform_api;
    struct halmac_platform_api *pHalmac_platform_api;
	#endif
	...
	u32 registered_nl80211_mgmt_frame_type;
	enum NL80211_STATE		nl80211_state;
	struct work_struct cfg80211_cmd_queue;
#endif
	...
	unsigned int debugfs_addr;
	unsigned int debugfs_val;
#endif
    ...
    struct sta_info_ary sta_ary[256];
	 ...
	 */
	struct mw5_legacy_timer		ch52_timer;
	struct mw5_legacy_timer		ch56_timer;
	struct mw5_legacy_timer		ch60_timer;
	struct mw5_legacy_timer		ch64_timer;
	struct mw5_legacy_timer		ch100_timer;
	struct mw5_legacy_timer		ch104_timer;
	struct mw5_legacy_timer		ch108_timer;
	struct mw5_legacy_timer		ch112_timer;
	struct mw5_legacy_timer		ch116_timer;
	struct mw5_legacy_timer		ch120_timer;
	struct mw5_legacy_timer		ch124_timer;
	struct mw5_legacy_timer		ch128_timer;
	struct mw5_legacy_timer		ch132_timer;
	struct mw5_legacy_timer		ch136_timer;
	struct mw5_legacy_timer		ch140_timer;
	struct mw5_legacy_timer		ch144_timer;
	...
	/*for avoid waste time
	save country_code ie to this buf,
	don't call construct_country_ie every update_beacon*/
	unsigned char			country_ie[MAX_CHANNEL_SET_NUMBER * 3 + 3 + 1];
	unsigned char			country_ie_len;
#endif
...
#endif
#ifdef CONFIG_IEEE80211V
	struct rtl8192cd_priv	*bss11v_rootpriv_sc;
	struct rtl8192cd_priv	*bss11v_priv_sc;
#ifdef SMP_SYNC
			spinlock_t			transition_list_lock;
			spinlock_t			waiting_list_lock;
	...
	UINT64				BssTerminationTSF;
	unsigned int		BssTerminationDurationCountDown;
	struct response_waiting_list waiting_list[MAX_TRANS_LIST_NUM];
	unsigned char waiting_list_bitmask[(MAX_TRANS_LIST_NUM + 7)/8];
	struct assoc_control_block_list block_list[MAX_TRANS_LIST_NUM];
	unsigned char block_list_bitmask[(MAX_TRANS_LIST_NUM + 7)/8];
#endif // CONFIG_IEEE80211V
#ifdef STA_CONTROL_BAND_TRANSITION
	struct list_head 	BandTransList;		// List of STAs meet band steering requirement, but havn't been executed
	struct list_head	BandTransBlockList;	// List of STAs which are forced disassociation because of band steering
	spinlock_t			BandTransLock;
	unsigned int		load_balance_score;
#if defined(DOT11K) && defined(CONFIG_IEEE80211V)
	struct dot11k_neighbor_report band_trans_neighbor_report;
#endif // defined(DOT11K) && defined(CONFIG_IEEE80211V)
#endif // STA_CONTROL_BAND_TRANSITION
#ifdef SUPPORT_MONITOR
	struct mw5_legacy_timer		chan_switch_timer;
	BOOLEAN 				is_monitor_mode;
	unsigned int 			chan_num;
#endif
#if defined(CONFIG_IEEE80211W_CLI) || defined(CONFIG_IEEE80211R_CLI)	// all ap's cap
	//unsigned char 			support_sha256;		
	unsigned char 			support_pmf;
#endif
#ifdef CONFIG_IEEE80211R_CLI
	unsigned char 		FT_CheckRoamTime;
	unsigned char 	 	CliFTAuthState;
	unsigned char	 		ft_target_bssid[MACADDRLEN];
	unsigned char 		ft_target_idx;
	unsigned char			ft_target_channel;
	unsigned int			ft_target_t_stamp[2];
	unsigned char 		init_ap[MACADDRLEN];			//FT init's AP
	unsigned char 		init_r0kh_id[MAX_R0KHID_LEN];	//If roaming target is initial AP
	unsigned int 			init_r0kh_id_len;
	unsigned char			cur_ap[MACADDRLEN];
	OCTET_STRING		ftie_ANonce;
	OCTET_STRING		ftie_SNonce;
	unsigned char			FT_PTK[PTK_LEN_TKIP];
	unsigned char 		FT_MIC[KEY_MIC_LEN];
#endif
	...
	unsigned int			autoch_only_ss;
	unsigned int			autoch_only_ss_clear_ch;
	unsigned int			autoch_only_ss_clear_2ndch;
	unsigned int			autoch_only_ss_2ndch_backup;
	unsigned int			auto_channel_use_score;
#ifdef BEACON_VS_IE
	struct beacon_vsie bcn_vsie[NUM_BCNVSIE_SERVICE];
	struct probe_vsie  prb_vsie[NUM_PRBVSIE_SERVICE];
	struct probersp_vsie prbrsp_vsie[NUM_PRBVSIE_SERVICE];
	unsigned char probersp_vsie_idx;
#endif
	unsigned char 			led_flag;
#ifdef AUTH_SAE
	struct hash_table		*sae_peer_table;
	struct hash_table		*sae_blacklist_table;
	unsigned char sae_clean_pmk_cache;
	GD *gd;/* group definitions */
	int curr_open, open_threshold;
	unsigned long pmk_expiry;
	unsigned char giveup_threshold ;
	unsigned long token_generator;
	unsigned char allzero[MBEDTLS_DIGEST_LENGTH];
#endif
#ifdef CONFIG_RTK_MESH
    struct mw5_legacy_timer       mesh_expire_timer;
    struct wlan_ethhdr_t    ethhdr;
	...
	UINT8                  mesh_CSA_state;
	UINT8                  mesh_CSA_mode;
	UINT8                  mesh_CSA_channel;
	UINT8                  mesh_CSA_expire;
	UINT32                 mesh_CSA_blocking;
#endif
#ifdef MESH_BOOTSEQ_AUTH
	struct mw5_legacy_timer		mesh_auth_timer;		///< for unestablish (And establish to unestablish) MP mesh_auth_hdr
	...
	struct mw5_legacy_timer		mesh_peer_link_timer;	///< for unestablish (And establish to unestablish) MP mesh_unEstablish_hdr
	...
	struct timer_event_entry    mesh_peer_link_timer_event;
#endif
...
#ifdef CONFIG_RTK_WPS_MESH
	int 				mesh_target_block;
	char				hw_serial_no_cipher[65]; /* Cipher of priv->pmibs->miscEntry.hw_serial_no */
	int					ss_serial_timer;
	int					ss_serial_cnt;
	char				ss_serial_no[16][65];
	unsigned char		ss_serial_mac[16][6];
#if defined(LINUX_2_6_27_)
	struct				pid *_wscd_pid;
#endif
#endif
#ifdef EN_EFUSE
	...
	struct mw5_legacy_timer		waiMCastKeyUpdateTimer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	...
	struct ss_acs_score	acs_score[76];//ACS_SCORE_PROC
...
#endif
#if	defined( CONFIG_IEEE80211R ) && !defined( RTK_NL80211 )
	spinlock_t			ft_r0kh_lock;
	spinlock_t			ft_r1kh_lock;
#endif
#endif
#ifdef HAPD_11V
	unsigned char hapd_11v_enable;
#endif
	/*hapd_ap_2040m_scan
	purpose: AP mode force to 40M band width 
	val=0 ; don't report scan result to hapd then hapd won't auto 20/40M selection
	val=1 ; will report*/
	unsigned char hapd_ap_2040m_scan;
#if 1 /* TRIBAND_SUPPORT */
	unsigned char hci_type;
	const struct hci_ops_s *hci_ops; /* TRIBAND_OPS */
#endif

#ifdef RING_BASED_DATA_FRAME_QUEUE
	struct data_que			data_queue;
#endif

#ifdef HAPD_11K
	unsigned char hapd_11k_ie[7];
	unsigned char hapd_11k_ie_len;/*when len !=0 means has IEs from hapd*/
#endif

	unsigned char hapd_interworking_ie[3];
	unsigned char hapd_interworking_ie_len;/*when len !=0 means has IEs from hapd*/

	unsigned char hapd_anqp_ie[4];
	unsigned char hapd_anqp_ie_len;/*when len !=0 means has IEs from hapd*/

	unsigned char hapd_dpp_ie[6];
	unsigned char hapd_dpp_ie_len;/*when len !=0 means has IEs from hapd*/
#ifdef HAPD_OWE
	u8 owe_transition_ie[48];
	u8 owe_transition_ie_len;
#endif
#if defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(3,0,0)) // retain SP info after reinit.
	unsigned char  service_priority_enabled;
	unsigned char  service_priority_output;
	unsigned char  dscp_pcp_table_enabled;
	unsigned char  dscp_pcp_table[64];
#endif //(EASYMESH_VERSION >= MULTI_AP_VERSION(3,0,0))

	struct mw5_legacy_timer		frag_to_filter;
	struct mw5_legacy_timer		expire_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	expire_timer_event; 
#endif
	struct mw5_legacy_timer		ss_timer;	//ss_timer: site_survey timer
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	ss_timer_event;
#endif
#ifdef RTK_MULTI_AP
	struct mw5_legacy_timer		multiap_backhaul_steer_timer;
	struct mw5_legacy_timer		multiap_unassoc_sta_timer;
#if (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	struct mw5_legacy_timer		multiap_cac_process_timer;
#endif
#endif
#if defined(CONFIG_PCI_HCI) && (defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT))
	struct mw5_legacy_timer		add_ps_timer;
#endif
	struct mw5_legacy_timer		MIC_check_timer;		// for MIC check
	struct mw5_legacy_timer		assoc_reject_timer;
#ifdef CLIENT_MODE
	struct mw5_legacy_timer		reauth_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
...
#endif
	struct mw5_legacy_timer		idle_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	idle_timer_event;
#endif
#ifdef DFS
	struct mw5_legacy_timer		dfs_cntdwn_timer;
#endif
#endif // CLIENT_MODE

#ifdef TCP_ACK_ACC
	struct mw5_legacy_timer		tcpack_timer;
#endif

#ifdef DFS
	struct mw5_legacy_timer		DFS_timer;			/* timer for radar detection */
	struct mw5_legacy_timer		DFS_TXPAUSE_timer;	/* timer for checking whether restarting TX or not*/
	struct mw5_legacy_timer		ch_avail_chk_timer; /* timer for channel availability check */
#ifdef CONFIG_RTL_92D_SUPPORT
	struct mw5_legacy_timer		dfs_chk_timer;		/* timer for dfs trigger */
#endif
	struct mw5_legacy_timer		dfs_det_chk_timer;	/* timer for channel busy check under dfs */
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	 struct timer_event_entry	 DFS_timer_event;
	 struct timer_event_entry	 DFS_TXPAUSE_timer_event;
	 struct timer_event_entry	 ch_avail_chk_timer_event;
#if CONFIG_RTL_92D_SUPPORT
	 struct timer_event_entry	 dfs_chk_timer_event;
#endif
	 struct timer_event_entry	 dfs_det_chk_timer_event;
#endif
#endif

#ifdef GBWC
	struct mw5_legacy_timer		GBWC_timer;
#endif
#ifdef SBWC
	struct mw5_legacy_timer		SBWC_timer;
#endif
#ifdef RTK_STA_BWC
	struct mw5_legacy_timer		sta_bwc_timer;
#endif
#ifdef DOT11K
	struct mw5_legacy_timer		rm_delay_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	rm_delay_timer_event;
#endif
#endif
...
#ifdef RTK_MULTI_AP
	unsigned char			ss_passive_toggle;
	unsigned char			ss_passive_offset;
#endif
...
#if	defined( CONFIG_IEEE80211R ) && !defined( RTK_NL80211 )
    int                 ft_r0kh_lock_owner;
    unsigned char       ft_r0kh_lock_func[50];
    int                 ft_r1kh_lock_owner;
    unsigned char       ft_r1kh_lock_func[50];    
#endif

	int			asoc_list_lock_owner;
	unsigned char		asoc_list_lock_func[50];
#endif
#ifdef WIFI_SIMPLE_CONFIG
...
#endif
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	unsigned int			h2d_header[4]; /* put before beaconbuf */
#endif
#ifdef WIFI_SIMPLE_CONFIG
...
#ifdef RTK_SMART_ROAMING
	struct mw5_legacy_timer		send_timer_wlan0;
	struct mw5_legacy_timer		send_timer_wlan1;
#if defined(MBSSID)
	struct mw5_legacy_timer		send_timer_wlan0_vap[RTL8192CD_NUM_VWLAN];
	struct mw5_legacy_timer		send_timer_wlan1_vap[RTL8192CD_NUM_VWLAN];
#endif
	unsigned char			neighbor_clear_flag;
	unsigned char			vapid;	//0: root
	/* ********** for smart roaming csa ********** */
	unsigned char			csa_state; // bit0: has STA; bit1: no traffic
	int						csa_ready_time;
	unsigned char			csa_request;
	/* ******************************************* */
#if defined(CONFIG_RTL_TRIBAND_SUPPORT)
	struct mw5_legacy_timer		send_timer_wlan2;
#if defined(MBSSID)
	struct mw5_legacy_timer		send_timer_wlan2_vap[RTL8192CD_NUM_VWLAN];
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
    struct timer_event_entry    send_timer_wlan2_event;
#endif
#endif
	...
	unsigned char			sta_flag; // sta support 11k :1  11v: 2 : support both: 3
	struct smart_roaming_block_data	sr_block;
#endif

#ifdef RTK_MULTI_AP
	unsigned char			multiap_last_cu;
	unsigned char			multiap_measure_opClass;
	unsigned char			multiap_measure_channel;
	unsigned char			multiap_bssSteer_scan;
	unsigned char			multiap_bssSteer_channel;
	unsigned char			multiap_unassocSta_ongoing;

	unsigned char			multiap_target_bssid[MACADDRLEN];
	struct sta_mac_rssi     multiap_unassoc_sta[MAX_PROBE_REQ_STA];
	unsigned int			multiap_unassocStaEntryOccupied;	
	unsigned int			multiap_unassocStaEntryNum;

#if (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	//--------R2 Channel Scan--------------
	unsigned char			multiap_channel_scan;
	unsigned char			multiap_requested_channel_nr;
	unsigned char*			multiap_requested_channels;
	struct map_channel_scan_result_per_radio* multiap_scan_result;

	//----------------------------------
	//--------R2 DFS CAC------------------
	unsigned char			multiap_cac_channel_hold;
	unsigned char			multiap_cac_channel_nr;
	unsigned char*			multiap_cac_channels;
	unsigned char*			multiap_cac_op_class;
	unsigned char			multiap_cac_current_channel_idx;
	unsigned char*			multiap_cac_channel_time;
	//struct map_cac_result_per_radio* multiap_cac_result;
	//----------------------------------
#endif
	unsigned char			multiap_last_assoc_disallowed_status;
#endif

#ifdef STA_ASSOC_STATISTIC
	struct sta_mac_rssi     reject_sta[MAX_PROBE_REQ_STA];
	struct sta_mac_rssi     removed_sta[MAX_PROBE_REQ_STA];
	struct sta_assoc_record assoc_sta[MAX_ASSOC_STATUS_STA];
...
#if defined(RTK_AC_SUPPORT) || defined(RTK_AC_TX_SUPPORT)//defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8881A)
	struct vht_cap_elmt		vht_cap_buf;
	unsigned int			vht_cap_len;
	struct vht_oper_elmt 	vht_oper_buf;
	unsigned int			vht_oper_len;
...
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
	struct mw5_legacy_timer		add_RATid_timer;
	struct mw5_legacy_timer		add_rssi_timer;
#endif
#endif // CONFIG_PCI_HCI

#if defined(CONFIG_RTL_92D_SUPPORT) && defined(CONFIG_RTL_NOISE_CONTROL)
	struct mw5_legacy_timer		dnc_timer;
#endif
...
//	unsigned char			*pBeaconErp;			// ptr of ERP field in beacon buf
	...
	unsigned int			nhm_cnt_round[1][MAX_NUM_20M_CH][NHM_RPT_NUM];
	unsigned int			clm_cnt[MAX_NUM_20M_CH];
	unsigned int			acs2_round_cn;
	unsigned int			cck_th_backup;
#endif
	unsigned int			chnl_ss_score[MAX_CHANNEL_NUM];		// record channel score while ss
	...
	unsigned int			nhm_cnt[MAX_NUM_20M_CH][NHM_RPT_NUM];
	struct channel_info		ss_channel_info[MAX_CHANNEL_NUM];
#endif
#ifdef P2P_SUPPORT
...
#ifdef HS2_SUPPORT
	struct mw5_legacy_timer		disassoc_timer;
#endif
	unsigned int			join_req_ongoing;
	int						join_index;
...
//	unsigned int			rxMlcstDataNumInPeriod_pre;
...
#ifdef DYN_AMPDU_NUM
	unsigned int			tx_pktlen_avg;
	unsigned char			adj_ampdu_num;
	unsigned short			bak_ampdu_num;
#endif

#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	unsigned int			beacon_updated;
#endif
    unsigned long           mcast_drop;
	/*********************************************************************
	...
	unsigned char			hist_long_idx;
	unsigned char			mask_hist_checked;
	unsigned char			pulse_flag_hist[10];
	unsigned char			pulse_type_hist[10];
	unsigned char			det_asoc_clear;
	unsigned int			short_pulse_cnt_pre;
	unsigned int			long_pulse_cnt_pre;
	unsigned int			st_L2H_cur;
	unsigned int			pluseinterval_hold_sum[6];
	unsigned int			plusewidth_hold_sum[6];	
	unsigned int			pluseinterval_long_hold_sum[6];
	unsigned int			plusewidth_long_hold_sum[6];
#if defined(CONFIG_WLAN_HAL_8812FE)
	unsigned int			pw_12f_rslt[12];
	unsigned int			pri_12f_rslt[12];
	signed char 			loct_12f_rslt[12];
#endif
#if defined(CONFIG_WLAN_HAL_8814BE)
	unsigned char			seg1_flag;
	unsigned int			pw_14b_rslt[4];
	unsigned int			pri_14b_rslt;
	unsigned int			loct_14b_rslt[8];
#endif	
	unsigned char			pw_std;
	unsigned char			pri_std;
	...
	unsigned char			pri_cond1;
	unsigned char			pri_cond2;
	unsigned char			pri_cond3;
	unsigned char			pri_cond4;
	unsigned char			pri_cond5;
	unsigned char			pw_cond1;
	unsigned char			pw_cond2;
	unsigned char			pw_cond3;
	unsigned char			pri_type3_4_cond1;
	unsigned char			pri_type3_4_cond2;
	unsigned char			pw_long_cond1;
	unsigned char			pw_long_cond2;
	unsigned char			pri_long_cond1;
	unsigned char			pri_type3_4_flag;
	unsigned char			pulsewidth_flag;
	unsigned char			pulseinterval_flag;
	unsigned char			long_radar_flag;
	unsigned char			loct_flag;
	unsigned char	                nhm_dfs_flag;
	unsigned char			plusewidth_hold[4][6];
	unsigned char			pluseinterval_hold[4][6];
	unsigned char			plusewidth_long_hold[300][6];
	unsigned char			pluseinterval_long_hold[300][6];
...
//	unsigned int			ampdu_first_dma_desc;
	...
	int						amsdu_len;
	int						ampdu_len;
	unsigned char			wlan_addr[MACADDRLEN];
...
#ifdef ATM_REPORT
    u1Byte              ATMEnRanges; //BIT0 range1, BIT1 range2, BIT3 range3
    u1Byte              ATM_period;
    u1Byte              ATMSTAMACIDs[3]; //[0] MACID1, [1] MACID2, [2] MACID3
	u1Byte              ATMSTALengths[3]; //[0] length1, [1] length2, [2] length3
	u1Byte              ATMnumOfSTA;
#endif
    ...
    unsigned char dma_priority;
    unsigned char set_hw_amsdu;
#endif //#ifdef CONFIG_OFFLOAD_FUNCTION
...
#endif
#if defined(AP_SWPS_OFFLOAD)
    unsigned char SWPS_RPT_macid_num;
    u1Byte swps_recycle_queue[4];

    //unsigned short SWPS_PSTime_TH;
    //unsigned short SWPS_Time_TH;
    //unsigned short SWPS_PKT_TH;
#endif
...
/* merge from osk 8192su driver to limit tx sleep queue 20100818*/
...
#ifdef SMP_SYNC
	spinlock_t		a4_sta_db_lock[A4_STA_HASH_SIZE];
#endif
#else
   ...
   unsigned char      tv_mode_status; /*0: disable, 1:enable, 2:auto(disable), 3:auto(enable)*/
	...
	int func_backup;
#endif
#if defined(CONFIG_RTL_8812_SUPPORT)||defined(CONFIG_WLAN_HAL_8881A)||defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_RTL_8723B_SUPPORT) ||defined(CONFIG_WLAN_HAL_8822BE)||defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8814BE) ||defined(CONFIG_WLAN_HAL_8822CE) ||defined(CONFIG_WLAN_HAL_8812FE)|| defined(CONFIG_WLAN_HAL_8197G)
	char bFWReady;
#endif
//#ifdef HS2_SUPPORT
    /* Hotspot 2.0 Release 1 */
...
//#endif
	...
	union iwreq_data group_key_cache;
	u8 wk_bk[64];/*to record struct ieee80211req_key*/
#endif
#endif
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	unsigned int txie[6][2];
#endif
#if defined(CONFIG_PE_ENABLE)
	unsigned int 	pe_initialized;
	unsigned int	pe_ready;
	unsigned char*	pe_net_stats;
	unsigned char*	pe_ext_stats;
	unsigned char*	pe_pshare_phw;
	unsigned char*  pe_privparam_t2h_list;
	unsigned char*  pe_stainfo_t2h_list;
	unsigned long	pe_netstats_txpkts_pre;
	unsigned long	pe_netstats_txbytes_pre;
	unsigned long	pe_extstats_txbytecnt_pre;
	unsigned long	pe_extstats_rxbytecnt_pre;
#endif
...
#if defined(OSK)
//jim 20160617 we gather the three different difinition location to this new created head file.
#include "sta_info_2_web.h"
#else
...
#define AMSDU_SWQ_ENABLE	((GET_MIB(priv))->dot11nConfigEntry.dot11nAMSDUSwqEn)
...
#define PENDING_REAUTH_TIMER	((ACTIVE_ID > 0) ? \
			mw5_legacy_timer_pending(&priv->pshare->mclone_sta[ACTIVE_ID-1].reauth_timer) : \
					mw5_legacy_timer_pending(&priv->reauth_timer))
#define DELETE_REAUTH_TIMER	((ACTIVE_ID > 0) ? \
			mw5_legacy_del_timer(&priv->pshare->mclone_sta[ACTIVE_ID-1].reauth_timer) : \
					mw5_legacy_del_timer(&priv->reauth_timer))
#define PENDING_REASSOC_TIMER	((ACTIVE_ID > 0) ? \
			mw5_legacy_timer_pending(&priv->pshare->mclone_sta[ACTIVE_ID-1].reassoc_timer) : \
					mw5_legacy_timer_pending(&priv->reassoc_timer))
#define DELETE_REASSOC_TIMER	((ACTIVE_ID > 0) ? \
			mw5_legacy_del_timer(&priv->pshare->mclone_sta[ACTIVE_ID-1].reassoc_timer) : \
					mw5_legacy_del_timer(&priv->reassoc_timer))

#define MOD_REAUTH_TIMER(t)	((ACTIVE_ID > 0) ? \
			mod_timer (&priv->pshare->mclone_sta[ACTIVE_ID-1].reauth_timer, jiffies+t) : \
					mw5_legacy_mod_timer(&priv->reauth_timer, jiffies+t))
#define MOD_REASSOC_TIMER(t)	((ACTIVE_ID > 0) ? \
			mod_timer (&priv->pshare->mclone_sta[ACTIVE_ID-1].reassoc_timer, jiffies+t) : \
					mw5_legacy_mod_timer(&priv->reassoc_timer, jiffies+t))
#else
...
#define PENDING_REAUTH_TIMER	(mw5_legacy_timer_pending(&priv->reauth_timer))
#define DELETE_REAUTH_TIMER	(mw5_legacy_del_timer(&priv->reauth_timer))
#define PENDING_REASSOC_TIMER	(mw5_legacy_timer_pending(&priv->reassoc_timer))
#define DELETE_REASSOC_TIMER	(mw5_legacy_del_timer(&priv->reassoc_timer))
#define MOD_REAUTH_TIMER(t)	(mw5_legacy_mod_timer(&priv->reauth_timer, jiffies+t))
#define MOD_REASSOC_TIMER(t)	(mw5_legacy_mod_timer(&priv->reassoc_timer, jiffies+t))
#endif
...
#endif
#else
/* Linux6 MW5 deliberately compiles the legacy VXD repeater out.  Keep a
 * typed NULL accessor so old short-circuit cfg80211 helpers still compile;
 * under_apmode_repeater() is a constant-false path without UNIVERSAL_REPEATER. */
#define GET_VXD_PRIV(priv)			((struct rtl8192cd_priv *)NULL)
...
#define IS_METEOROLOGY_CHANNEL(_ch_)	\
	( (priv->pmib->dot11nConfigEntry.dot11nUse40M == CHANNEL_WIDTH_80 && (_ch_) >= 116 && (_ch_) <= 128) || \
	  (priv->pmib->dot11nConfigEntry.dot11nUse40M == CHANNEL_WIDTH_40 && (_ch_) >= 116 && (_ch_) <= 128) || \
	  (priv->pmib->dot11nConfigEntry.dot11nUse40M == CHANNEL_WIDTH_20 && (_ch_) >= 120 && (_ch_) <= 128) )
#endif
...
#ifdef RTK_SMART_ROAMING
#define SMART_ROAMING_ENABLE		(priv->pmib->sr_profile.enable)
#endif

#ifdef RTK_MULTI_AP
#define ETH_P_1905              0x893a		/* IEEE 1905.1 protocol*/
#define MAP_PROFILE_ONE         1
#define MAP_PROFILE_TWO         2
#define GET_MAP_PROFILE(priv)   (GET_ROOT(priv)->pmib->multi_ap.multiap_profile)
#define MAP_BSS_TYPE            (priv->pmib->multi_ap.multiap_bss_type)
#define MAP_VLAN_ENABLE         (GET_ROOT(priv)->pmib->multi_ap.multiap_vlan_enable)
#define MAP_VLAN_ID             (priv->pmib->multi_ap.multiap_vlan_id)
#endif

#ifdef STA_ROAMING_CHECK
#define RTL_LINK_ROAMING_ENABLE             (priv->pmib->rlr_profile.roaming_switch && priv->pmib->rlr_profile.roaming_qos)
#endif
#ifdef CONFIG_IEEE80211V
...
#ifdef CONFIG_IEEE80211R_CLI
	state_ft_auth_req				= 6,
	state_ft_reassoc_rsp				= 7,
	state_ft_auth_done				= 8,
#endif
	state_unknown 					=9
};
...
#define IS_EXPERIMENT_PROTO(mac)	((mac[12]==0x08) && (mac[13]==0x00) && (mac[23]==0xFD))
...
#define IS_ICMPV6_PROTO2(pro,pay)		( (pro[0]==0x86)&&(pro[1]==0xdd) && ((pay[20-14]==0x3a)||(pay[54-14]==0x3a)))

#define IS_MDNSV4_MAC(mac) ((mac[0]==0x01)&&(mac[1]==0x00)&&(mac[2]==0x5e)&& (mac[3]==0x00)&&(mac[4]==0x00)&&(mac[5]==0xFB)&& (((mac[12]==0x08)&&(mac[13]==0x00))||((mac[12]==0x81)&&(mac[13]==0x00)&&(mac[16]==0x08)&&(mac[17]==0x00))))
#define IS_MDNSV6_MAC(mac) ((mac[0]==0x33)&&(mac[1]==0x33)&&(mac[2]==0x00)&& (mac[3]==0x00)&&(mac[4]==0x00)&&(mac[5]==0xFB)&& (((mac[12]==0x86)&&(mac[13]==0xdd))||((mac[12]==0x81)&&(mac[13]==0x00)&&(mac[16]==0x86)&&(mac[17]==0xdd))))
...
struct _device_info_ {
	...
	struct rtl8192cd_priv *priv;
#ifdef CONFIG_RTL8192CD_LINUX6_BUS
	/* Linux 6.x owns MMIO/DMA/IRQ resources; the vendor core only borrows them. */
	struct device *linux_dev;
	void __iomem *managed_mmio;
	resource_size_t managed_mmio_len;
...
enum {
	SSFROM_WEB = 1,
	SSFROM_WSC = 2,
	SSFROM_REPEATER_VXD = 3,
	SSFROM_11K_BEACONREQ = 4,
	SSFROM_WPAS = 5,
	SSFROM_PASSIVE = 6
};
...
enum {
	THRU_OPER_MODE_NORMAL = 1,
	THRU_OPER_MODE_MODERATE,
	THRU_OPER_MODE_EXTREME
};
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
#define OFFLOAD_ENABLE(priv)		(priv->pshare->offload_en)
#endif
...
#define DEL_STA_SEND_NONE		0
#define DEL_STA_SEND_DEAUTH		1
#define DEL_STA_SEND_DISASSOC	2
...
typedef struct _G_OP_CLASS_ {
	u8        op_class;
	u8        band;     /* 0: 2g, 1: 5g*/
} G_OP_CLASS;

/*Global Operating Classes*/
static const G_OP_CLASS GLOBAL_OP_CLASS[] = {
	{ 81, BAND_ON_24G },
	{ 82, BAND_ON_24G },
	{ 83, BAND_ON_24G },
	{ 84, BAND_ON_24G },
