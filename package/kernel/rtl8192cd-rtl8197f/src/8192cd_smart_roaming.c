...
struct mw5_legacy_timer		*timer_wlan0;
struct mw5_legacy_timer		*timer_wlan1;
...
struct mw5_legacy_timer		*timer_wlan2;
...
static void sr_timer_rec_add(void) {
    if (wlan0_used) {
        if (!wlan0_deleted) {
            mw5_legacy_del_timer_sync(timer_wlan0);
            wlan0_deleted = 1;
        }
        if (wlan0_deleted) {
            mw5_legacy_add_timer(timer_wlan0);
            wlan0_deleted = 0;
        }
    }
    ...
    if (wlan1_used) {
        if (!wlan1_deleted) {
            mw5_legacy_del_timer_sync(timer_wlan1);
            wlan1_deleted = 1;
        }
        if (wlan1_deleted) {
            mw5_legacy_add_timer(timer_wlan1);
            wlan1_deleted = 0;
        }
    }
    ...
    if (wlan2_used) {
        if (!wlan2_deleted) {
            mw5_legacy_del_timer_sync(timer_wlan2);
            wlan2_deleted = 1;
        }

        if (wlan2_deleted) {
          mw5_legacy_add_timer(timer_wlan2);
          wlan0_deleted = 0;
        }
    }
#endif
	...
	for (j = 0; j < RTL8192CD_NUM_VWLAN; j++) {
		if (wlan0_vap_used[j]) {
			if (!wlan0_vap_delete[j]) {
				mw5_legacy_del_timer_sync(send_timer_wlan0_vap[j]);
				wlan0_vap_delete[j] = 1;
			}
			if (wlan0_vap_delete[j]) {
				mw5_legacy_add_timer(send_timer_wlan0_vap[j]);
		...
		if(wlan1_vap_used[j]) {
			if (!wlan1_vap_delete[j]) {
				mw5_legacy_del_timer_sync(send_timer_wlan1_vap[j]);
				wlan1_vap_delete[j] = 1;
			}
			if (wlan1_vap_delete[j]) {
				mw5_legacy_add_timer(send_timer_wlan1_vap[j]);
...
void timer_ready(struct rtl8192cd_priv *priv)
{
	if ((!strcmp(priv->dev->name, "wlan0")) && (wlan0_used != 1)){
		mw5_legacy_timer_init(&priv->send_timer_wlan0);
		...
		mw5_legacy_timer_init(&priv->send_timer_wlan1);
        ...
        mw5_legacy_timer_init(&priv->send_timer_wlan2);
...
void vap_timer_ready(struct rtl8192cd_priv *priv, int i) {
	...
	if (!strcmp(priv->dev->name, "wlan0")) {
		if (netif_running(GET_VAP_PRIV(priv, i)->dev) && (wlan0_vap_used[i] != 1)) {
			mw5_legacy_timer_init(&priv->send_timer_wlan0_vap[i]);
		...
		if (netif_running(GET_VAP_PRIV(priv, i)->dev) && (wlan1_vap_used[i] != 1)) {
			mw5_legacy_timer_init(&priv->send_timer_wlan1_vap[i]);
...
{
    if (!strcmp(priv->dev->name, "wlan0")) {
    	mw5_legacy_del_timer_sync(&priv->send_timer_wlan0);
    	wlan0_used = 0;
        wlan0_deleted = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
    }
	else if (!strcmp(priv->dev->name, "wlan1")) {
    	mw5_legacy_del_timer_sync(&priv->send_timer_wlan1);
        wlan1_used = 0;
        wlan1_deleted = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
    }
        ...
        mw5_legacy_del_timer_sync(&priv->send_timer_wlan2);
        wlan2_used = 0;
        wlan2_deleted = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
    }
		...
		int i = priv->vap_id;
		mw5_legacy_del_timer_sync(&GET_ROOT(priv)->send_timer_wlan0_vap[i]);
		wlan0_vap_used[i] = 0;
		wlan0_vap_delete[i] = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
	}
		...
		int i = priv->vap_id;
		mw5_legacy_del_timer_sync(&GET_ROOT(priv)->send_timer_wlan1_vap[i]);
		wlan1_vap_used[i] = 0;
		wlan1_vap_delete[i] = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
	}
...
void notify_hidden_ap(struct rtl8192cd_priv *priv, int type)
{
	...
	{
		panic_printk(KERN_ERR "rtl_smart_roaming_nl is NULL\n");
		return;
	}
...
void send_roam_info(unsigned long task_priv)
{
	...
	if(vapsr_enable != 1) {
		if ((!strcmp(priv->dev->name, "wlan0")) && (!mw5_legacy_timer_pending(&priv->send_timer_wlan0))) {
			mw5_legacy_mod_timer(&priv->send_timer_wlan0, jiffies+RTL_SECONDS_TO_JIFFIES(5));
			return;
		}
		if ((!strcmp(priv->dev->name, "wlan1")) && (!mw5_legacy_timer_pending(&priv->send_timer_wlan1))) {
			mw5_legacy_mod_timer(&priv->send_timer_wlan1, jiffies+RTL_SECONDS_TO_JIFFIES(4));
			return;
		}
#if defined(CONFIG_RTL_TRIBAND_SUPPORT)
	    if ((!strcmp(priv->dev->name, "wlan2")) && (!mw5_legacy_timer_pending(&priv->send_timer_wlan2))) {
	        mw5_legacy_mod_timer(&priv->send_timer_wlan2, jiffies+RTL_SECONDS_TO_JIFFIES(4));
			return;
		}
#endif
	}
#ifdef MBSSID
	if(vapsr_enable == 1) {
		if ((priv->dev->name[4] == '0') && (!mw5_legacy_timer_pending(&priv->send_timer_wlan0_vap[i]))) {
			mw5_legacy_mod_timer(&root_priv->send_timer_wlan0_vap[i], jiffies+RTL_SECONDS_TO_JIFFIES(5));
		}
		if ((priv->dev->name[4] == '1') && (!mw5_legacy_timer_pending(&priv->send_timer_wlan1_vap[i]))) {
			mw5_legacy_mod_timer(&root_priv->send_timer_wlan1_vap[i], jiffies+RTL_SECONDS_TO_JIFFIES(4));
		}
#if defined(CONFIG_RTL_TRIBAND_SUPPORT)
	    if ((priv->dev->name[4] == '2') && (!mw5_legacy_timer_pending(&priv->send_timer_wlan2_vap[i]))) {
			mw5_legacy_mod_timer(&root_priv->send_timer_wlan2_vap[i], jiffies+RTL_SECONDS_TO_JIFFIES(4));
		}
#endif
	}
#endif
...
void notify_new_sta(struct rtl8192cd_priv *priv, unsigned char *mac, int type, unsigned char rssi)
{
	...
	if(priv->pmib->sr_profile.sr_guest_access == 1)
#else
	...
	memcpy(send_buf+offset,&(tmp_value),sizeof(unsigned char));
	offset+=sizeof(unsigned char);
...
#ifdef CONFIG_RTK_MESH
	if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable && netif_running(priv->mesh_dev)
#ifdef NETDEV_NO_PRIV
		&& ((struct rtl8192cd_priv*)netdev_priv(priv->mesh_dev))->wlan_priv == priv
#else
		&& priv->mesh_dev->priv == priv
#endif
		) {
		send_buf[offset] = SR_MESH_PATHSEL;
		offset += sizeof(unsigned char);
		path_num_offset = offset;
		offset += sizeof(path_num);
		...
		for(i = 0; i < tbl_sz; i++) {		 
			if(priv->pathsel_table->entry_array[i].dirty != 0) {			
				...
				memcpy(send_buf+offset, &(ptable->destMAC), MACADDRLEN);
				offset+=MACADDRLEN;
				memcpy(send_buf+offset, &(ptable->nexthopMAC), MACADDRLEN);
				offset+=MACADDRLEN;
				path_num++;
			}
		}
		path_num = htonl(path_num);
		memcpy(send_buf+path_num_offset,&path_num,sizeof(path_num));
	}
#endif
