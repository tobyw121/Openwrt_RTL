/*
 *  802.11k neighbor processing routines
 *
...
#if defined(STA_CONTROL_BAND_TRANSITION) && defined(CONFIG_IEEE80211V)
// Store a neighbor report for band transition based on 11v BTM
void init_band_trans_neighbor_report(struct rtl8192cd_priv *priv)
{
	struct dot11k_neighbor_report *neighbor_report = &priv->band_trans_neighbor_report;
	unsigned char channel = priv->pmib->dot11RFEntry.dot11channel;

	memcpy(neighbor_report->bssid, (GET_MIB(priv))->dot11StationConfigEntry.dot11Bssid, MACADDRLEN);

	neighbor_report->channel = channel;
	neighbor_report->op_class = rm_get_op_class(priv, channel);
	neighbor_report->phytype = 0;

	// Clear bssinfo to 0
	neighbor_report->bssinfo.value = DEFAULT_BSSINFO_VALUE;

	// The bit value within the capability field of bssinfo should be set to the equivalent
	// bits within the Capability Information field being sent in the beacons

#ifdef DOT11H
	// Spectrum Management
	if((GET_MIB(priv))->dot11hTPCEntry.tpc_enable || priv->pshare->rf_ft_var.lpwrc)
		neighbor_report->bssinfo.field.cap_spectrum = 1;
#endif

	// APSD (this bit is not set in beacon frame, so comment out temporarily)
	//if ((GET_MIB(priv))->dot11QosEntry.dot11QosAPSD)
	//	neighbor_report->bssinfo.field.cap_apsd = 1;

#ifdef DOT11K
	// Radio Measurement
	if ((GET_MIB(priv))->dot11StationConfigEntry.dot11RadioMeasurementActivated)
		neighbor_report->bssinfo.field.cap_rm = 1;
#endif

#ifdef CONFIG_IEEE80211R
	// Mobility Domain
	if (FT_ENABLE)
		neighbor_report->bssinfo.field.mde = 1;
#endif

	// High Throughput
	if (priv->pmib->dot11BssType.net_work_type & WIRELESS_11N)
		neighbor_report->bssinfo.field.high_tp = 1;
	...
	if(WNM_ENABLE)
	{
		...
		neighbor_report->subelemnt.preference = MAX_PREFERRED_VAL;  // set as most preferred candidate
	}
#endif
}
#endif
...
int rm_construct_neighbor_report(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, unsigned char channel)
{
    ...
    priv->rm_neighbor_info[i].source = 2;
    ...
    if(WNM_ENABLE) {
   		...
   		priv->rm_neighbor_report[i].subelemnt.preference = getPreferredVal(priv, channel_utilization, pfrinfo->rf_info.rssi<<1, 0);
    }
#endif		
	return 0;
}
...
#endif
{
    ...
    for (i = 0 ; i < MAX_NEIGHBOR_REPORT; i++)
    {
	 ...
	 if(WNM_ENABLE) {
        ...
        PRINT_ONE("    Source:", "%s", 0);
        if(priv->rm_neighbor_info[i].source == 2)
        {
        	PRINT_ONE("Beacon listened", "%s", 1);
        }
        else if(priv->rm_neighbor_info[i].source == 1)
        {
        	PRINT_ONE("Beacon report from STA", "%s", 1);
        }
        else
       	{
        	PRINT_ONE("From writing proc file", "%s", 1);
        }
        j++;
    }
    ...
    {        
        ...
        if(command == 1)   /*add*/
        {
	   	...
	   	if(WNM_ENABLE) {
            ...
            priv->rm_neighbor_info[i].bssload.sta_count = sta_count;
            priv->rm_neighbor_info[i].bssload.channel_utilization = channel_utilization;  
			priv->rm_neighbor_info[i].source = 0;
            priv->rm_neighbor_bitmask[i>>3] |= (1<<(i&7));            
        }
