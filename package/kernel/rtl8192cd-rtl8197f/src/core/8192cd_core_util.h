...
void rtl8197f_wlan_dma_free(struct rtl8192cd_priv *priv, size_t size, void *cpu_addr, dma_addr_t dma_handle);
void rtl8197f_wlan_dma_sync(struct rtl8192cd_priv *priv, unsigned long start, unsigned int size, int direction);
void rtl8197f_wlan_dma_sync_rx_for_device(struct rtl8192cd_priv *priv, unsigned long dma_addr, unsigned int size);
void rtl8197f_wlan_dma_sync_rx_for_cpu(struct rtl8192cd_priv *priv, unsigned long dma_addr, unsigned int size);
#define os_dma_alloc(priv, size, dma)		rtl8197f_wlan_dma_alloc((priv), (size), (dma))
#define os_dma_free(priv, size, cpu, dma)	rtl8197f_wlan_dma_free((priv), (size), (cpu), (dma))
#elif defined(USE_DMA_ALLOCATE)
...
#define rtl_atomic_add_unless(ptr_atomic_t, a, u)	atomic_add_unless(ptr_atomic_t,a,u)
...
#define REMAP_AID(p)   p->cmn_info.aid
#else
...
// Return IPv4 or IPv6 header
static inline UINT8* rtl_get_skb_ip_header(struct sk_buff *skb, UINT8 v4)
{
#if 1
//#ifdef __ECOS
	UINT8 *ptr = (UINT8 *)(skb->data + 2*ETH_ALEN);
#else
	UINT8 *ptr = (UINT8 *)(SKB_MAC_HEADER(skb) + 2*ETH_ALEN);
#endif

	if(*(UINT16 *)(ptr) == __constant_htons(ETH_P_8021Q))
		ptr += 4;
	if((v4==0) || (*(UINT16 *)(ptr) == __constant_htons(ETH_P_IP)))
		return (ptr+2);
	else
		return NULL;
}
...
{
#if defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
	/* v43.46: the MW5 embedded radio has pdev == NULL.  Do not skip cache
	 * maintenance; route it through the same valid 32-bit DMA device anchor
	 * used by coherent descriptor allocations. */
	rtl8197f_wlan_dma_sync(priv, start, size, direction);
#else
	dma_addr_t dma_addr;

	if (!size || !priv || !priv->pshare || !priv->pshare->pdev)
		return;
	...
	if (priv) {
...
#endif
		{
			...
			da = get_da(txcfg->phdr);
                ...
                da = ((struct sk_buff *)txcfg->pframe)->data;
			if (da != NULL)
			{
			if (IS_BCAST2(da))
...
__inline static u8 is_eapol_key_pkt(struct sk_buff *pskb, u8 vlan_tagged)
{
	...
	if(vlan_tagged)
	{
		...
		if((*(u16*)(pskb->data+((ETH_ALEN<<1)+VLAN_HLEN)) == __constant_htons(ETH_P_8021X))
		&& ((*(u8*)(pskb->data+ETH_HLEN+VLAN_HLEN+1)) == EAP_TYPE_KEY))
			return 1;
	}
	else
	{
