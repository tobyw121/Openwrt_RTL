...
#else
#if defined(CONFIG_RTL_8198F) || defined(CONFIG_RTL_8197G) || defined(CONFIG_WLAN_HAL_8197G)
...
extern int check_arp_spoofing(struct rtl8192cd_priv *priv, struct sk_buff *pskb);
#endif
...
/* v44.59: The legacy rtl8192cd C2H path used by 8197F/8822B has a
 * C2H_CONTENT_LEN of 12 bytes, plus the two-byte command header.  Report22
 * captured many descriptor-C2H packets with 60..190 byte payloads and random
 * command IDs.  Those are impossible for the old non-extended C2H format and
 * were being dropped from the normal data path.  Preserve 0xff extended C2H
 * packets, but reclassify other oversized packets as normal RX frames. */
static inline void
mw5_linux6_sanitize_c2h(struct rtl8192cd_priv *priv,
                        struct rx_frinfo *pfrinfo,
                        PRX_DESC_STATUS_88XX status)
{
    unsigned char *frame;

    if (!priv || !priv->pshare || !pfrinfo || !status || !status->C2HPkt)
        return;
    if (pfrinfo->pktlen <= 14)
        return;

    frame = get_pframe(pfrinfo);
    if (!frame || frame[0] == 0xff)
        return;

    status->C2HPkt = FALSE;
    if (pfrinfo->pktlen >= _CRCLNG_)
        pfrinfo->pktlen -= _CRCLNG_;
    priv->pshare->linux6_c2h_oversize_reclassified++;
}
#else
#define mw5_linux6_sanitize_c2h(priv, pfrinfo, status) do { } while (0)
#endif
				...
				)
{
...
#ifdef SMP_SYNC
#if !defined(RTL8190_DIRECT_RX) && !defined(RING_BASED_DATA_FRAME_QUEUE)
	unsigned long x;
#endif
#endif
	...
	if ((QOS_ENABLE)
		...
		if (IS_BSSID(priv, bssid))
		{
			...
			if (rtk_data_queue_enqueue(priv, &priv->data_queue, pfrinfo)) {
				return 1;
			}
#else
			SMP_LOCK_RX_DATA(x);
...
int rtl_IsMcastIP(struct sk_buff *pskb)
{
	...
	#ifdef __ECOS
		struct ip6_hdr *iph;
		iph  = (struct ip6_hdr *)(pskb->data + ETH_HLEN + vlanTag);
	#else
		struct ipv6hdr *iph;
...
extern int rtl8197f_mw5_accel_rx(struct sk_buff *skb);
#endif

#ifdef SBWC
int SBWC_forward_check(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
{
	...
	return 0;
SBWC_forward_check_free_skb:
	priv->ext_stats.rx_data_drops++;
	rtl_kfree_skb(priv, pskb, _SKB_RX_);
SBWC_forward_check_queue_skb:	
	return 1;
}
#endif
...
void rtl_netif_rx(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
{
	...
	{
		if((unsigned char)pskb->data[0] == 0xff){
			...
			{			
				priv->ext_stats.rx_data_drops++;
				DEBUG_ERR("RX DROP: Drop broadcast on non prefer band!\n");
				rtl_kfree_skb(priv, pskb, _SKB_RX_);
				return;
			}
		}
	...
	if(priv->pshare->rf_ft_var.drop_multicast && IS_MCAST(pskb->data) && ((unsigned char)pskb->data[0] != 0xff) ) {
		...
		if(priv->pmib->miscEntry.client_host_sniffer_enable) {
			...
			else if(pstat && (!pstat->record_host_ip_timeout || time_after(jiffies, pstat->record_host_ip_timeout))){
				/*get skb ipv4 header*/
				STADEBUG("link_time %d, jiffies %lu, timeout %lu\n", pstat->link_time, jiffies, pstat->record_host_ip_timeout);
				offset = (ETH_ALEN << 1);
				if(*(uint16 *)&pskb->data[offset] == __constant_htons(ETH_P_8021Q))
					offset += 4;
				if(*(uint16 *)&pskb->data[offset] == __constant_htons(ETH_P_IP)){
					offset += 2;
					memcpy(pstat->client_host_ip, (uint8 *)&pskb->data[offset + 12], sizeof(pstat->client_host_ip));
					pstat->record_host_ip_timeout = jiffies + HZ*priv->pmib->miscEntry.client_host_sniffer_interval;
				}
			}
		}
#endif
...
#ifdef TDLS_SUPPORT
	//pDa = pskb->data;
	pSa = pskb->data + ETH_ALEN;
	ether_type = *(unsigned short *)(pSa +ETH_ALEN);
...
#ifdef GBWC
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		if (priv->pmib->gbwcEntry.GBWCMode && pstat && priv->GBWC_timer_alive) {
...
#ifdef SBWC
	if (SBWC_forward_check(priv, pskb, pstat))
		return;
#endif
...
#endif
	/* MW5 v44.62.11: unconditional telemetry for the Linux netif_rx backlog
	 * producer.  The failing v44.62.10 capture showed almost exactly 64
	 * processed softnet entries per NET_RX softirq, while eth0 DMA counters
	 * remained low.  This counter lets /proc/<wlan>/stats prove or exclude
	 * rtl8192cd as that enqueue source without changing RX behaviour.
	 */
...
/* Hotspot 2.0 Release 1 */
...
#endif
	{
		...
		if (GET_ROOT(priv)->pmib->multi_ap.multiap_vlan_enable) {
			if (*((uint16*)(pskb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021Q) || *((uint16*)(pskb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021AD))
			{
				if (0 == (priv->pmib->multi_ap.multiap_bss_type & BIT(7) || priv->pmib->multi_ap.multiap_bss_type & BIT(6))) {
					priv->ext_stats.rx_data_drops++;
					DEBUG_ERR("RX DROP: by vlan!\n");
					printk("Dropping VLAN packet received on non backhaul bss/sta interface!!!\n");
					dev_kfree_skb_any(pskb);
					return;
				}
			}
	...
	if(rtk_linux_vlan_enable){
		if(*((uint16*)(pskb->data+(ETH_ALEN<<1))) != __constant_htons(ETH_P_8021Q)){
			...
			rtk_index = priv->dev->vlan_member_map;
			if(rtk_index>=RTK_WLAN0_ROOT_MASK && rtk_index<=RTK_WLAN2_CLIENT_MASK){
				vid = rtk_vlan_ctl_p->pvid[rtk_index];
				if(vid){
					rtk_insert_vlan_tag_and_priority(vid, 0, (struct sk_buff **)(&pskb));
				}
			}		
		}
	}
...
//		uint8 vlan_type = 0;
...
#endif
#if (defined(CONFIG_RTW_BRSC) || defined(CONFIG_RTL_EXT_PORT_SUPPORT)) && !defined(CONFIG_MW5_ACCEL)
	if(check_br_port_ready(priv->dev))
	{
		/*update wifi side sc entry*/
		rtw_update_brsc_info(pskb->data + ETH_ALEN, priv->dev);

#if defined(CONFIG_RTW_BRSC)
		if(rtw_brsc_should_go(priv->dev, pskb))
		{
			if(rtw_brsc_go(pskb,priv->dev) != RTW_BRSC_CONTINUE)
			{
				pstat->rx_pkts2++;
				return;
			}
...
#ifdef CONFIG_MW5_ACCEL
        /* v44.65.17 unified accelerator: consume a frame only when an exact
         * nftables/conntrack/nf_flow_table rule owns the shortcut. */
        if (rtl8197f_mw5_accel_rx(pskb)) {
                pstat->rx_pkts2++;
                return;
        }
#endif
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT) && !defined(CONFIG_MW5_ACCEL)
		if(extPortEnabled)
			if(rtl_wlanFwdToEth(pskb)==NET_RX_SUCCESS)
				return;
#endif
	...
	)
	{
		...
		struct net_device *br_dev = __dev_get_by_name(&init_net, CONFIG_TP_LAN_BRIDGE_NAME);
#ifdef CONFIG_IPV6
...
#elif defined(CONFIG_RTK_DEV_AP)&& defined(CONFIG_RTL_MULTI_ETH_WAN)
	pskb->from_dev = priv->dev;
#endif
...
static void reorder_ctrl_timer_add(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid, int from_timeout)
{
    ...
    if (!from_timeout) {
        if (mw5_legacy_timer_pending(&priv->pshare->rc_sys_timer)) {
            mw5_legacy_del_timer(&priv->pshare->rc_sys_timer);
		}
	...
	if (!from_timeout) {
        ...
        while (phead && (plist != phead)) {
            ...
            if (next_timeout == 0 || time_before((unsigned long)prc_timer->timeout, next_timeout))
        ...
        if (next_timeout > 0) {
            ...
            mw5_legacy_mod_timer(&priv->pshare->rc_sys_timer, next_timeout);
		}
	}
}
...
void reorder_ctrl_timeout(unsigned long task_priv)
{
    ...
    struct list_head *phead, *plist; 
    struct reorder_ctrl_timer *prc_timer;
	int win_start=0, win_size, tid=0, i;
	unsigned long flags;
#ifdef SMP_SYNC
	int locked = 0;
#endif
    ...
    while (phead && (plist != phead) && plist) {
        ...
        if (time_after_eq(current_time, (unsigned long)prc_timer->timeout)) {
			...
			if (SN_LESS(win_start, rc_entry->last_seq) || (win_start == rc_entry->last_seq)) {
				...
				else if (next_timeout == 0 || time_before((unsigned long)prc_timer->timeout, next_timeout))
					next_timeout = prc_timer->timeout;
            } else {
				rc_entry->start_rcv = FALSE;
			}
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
 				&& (!
                                    _cut || rc_entry->packet_q[index]->prev==0xBEEFBEEF)
#endif
			) {
				SMP_UNLOCK_REORDER_CTRL(flags);
#ifdef _RX_REORDER_DEBUG_
...
/* RFC1042 LLC/SNAP prefix is driver-private; modern cfg80211 does not export it. */
static const unsigned char mw5_rfc1042_header[WLAN_LLC_HEADER_SIZE] =
...
int rx_shortcut(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
        ...
        (GetFragNum(pframe) == 0) && (GetMFrag(pframe) == 0))
    {
        ...
        if (pstat->rx_privacy == privacy)
        {
            ...
            offset = prxsc_entry->rx_payload_offset + sizeof(mw5_rfc1042_header);

            /*
                    printk("pstat->rx_payload_offset = %02x, 0xaa, cmp = %d \n",pframe[prxsc_entry->rx_payload_offset],
                    memcmp(&pframe[prxsc_entry->rx_payload_offset], mw5_rfc1042_header, 6));
                    ...
                    */
            // check snap header
            //if (memcmp(&pframe[prxsc_entry->rx_payload_offset], mw5_rfc1042_header, 6) ||
            //	memcmp(&prxsc_entry->rx_ethhdr.type,&pframe[offset], 2))
            if (!isEqualMACAddr(&pframe[prxsc_entry->rx_payload_offset], mw5_rfc1042_header) ||
...
#ifdef A4_STA
#if defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	int vid = 0;  unsigned char wlan_dev_name[IFNAMSIZ] = {0};  struct net_device *wlan_dev = NULL;
	vid = get_80211frameVlanID(priv, pfrinfo);
	if(vid)
	{
		snprintf(wlan_dev_name, IFNAMSIZ, "%s.%d", priv->dev->name, vid);
		wlan_dev = dev_get_by_name(&init_net, wlan_dev_name);
		if(wlan_dev == NULL)
		{
			snprintf(wlan_dev_name, IFNAMSIZ, "v%s.%d", priv->dev->name, vid);
			wlan_dev = dev_get_by_name(&init_net, wlan_dev_name);
			if(wlan_dev == NULL)
				return 1;
		}
		...
		wlan_dev =	priv->dev;
	if((GET_BR_PORT(priv->dev) && priv->pmib->miscEntry.a4_enable == 2 && fdb_find_for_driver(GET_BR_PORT(wlan_dev)->br, da) == NULL) || (priv->pmib->miscEntry.a4_enable == 1 && IS_MCAST(da)))
#else
	                if((GET_BR_PORT(priv->dev) && priv->pmib->miscEntry.a4_enable == 2 && fdb_find_for_driver(GET_BR_PORT(priv->dev)->br, da) == NULL) || (priv->pmib->miscEntry.a4_enable == 1 && IS_MCAST(da)))
#endif
...
#endif
                {
					unsigned char *iph = rtl_get_skb_ip_header(pfrinfo->pskb, 1);
                    if (!priv->pmib->ethBrExtInfo.nat25sc_disable &&
                        !(pfrinfo->pskb->data[0] & 1) && iph &&
                        isEqualIPAddr(priv->scdb_ip, iph +16)) {
...
static struct rx_frinfo *reassemble(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	unsigned char		*pframe=NULL, *pfirstframe=NULL, tail[16] = {0}; //4, 12, and 8 for WEP/TKIP/AES
	unsigned long		flags;
	int			i;
	union PN48	last_PN;
	...
	if(IEEE8021X_FUN) {
		if (pstat) {
			if (pstat->ieee8021x_ctrlport) // controlled port is enable...
				...
				if (
#if (defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
		(proto == __constant_htons(ETH_P_8021Q) && (*(unsigned short *)(pbuf + 4) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))) ||
#endif
		(proto == __constant_htons(0x888e))) {
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
					pr_info_ratelimited("rtl8197f-wlan: v44.38 MW5 EAPOL RX admitted if=%s peer=%pM\n",
						priv->dev ? priv->dev->name : "none", pstat->cmn_info.mac_addr);
#endif
					return SUCCESS;
				}
				else {
...
static struct rx_frinfo *defrag_frame_main(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	{
		DEBUG_INFO("RX : Recv unencrypted packet when encryption is WEP!, Don't drop the EAPOL packet when doing WPS!\n");
	}
	else
#endif
#if defined(CONFIG_RTL_WAPI_SUPPORT)
...
static int process_datafrme(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	if (OPMODE & WIFI_AP_STATE)
	{
		...
		if (is_qos_data(pframe))
		{
			if (pfrinfo->amsdu)	// A-MSDU present
			{
...
//			if (get_free_memory() < FREE_MEM_LOWER_BOUND) {
			...
			if ((IP_MCAST_MAC(pskb->data) && IS_IGMP_PROTO(pskb->data)) || IS_1905_MCAST(pskb->data))
			{
			...
			{
				RTL8192CD_SKB_LEGACY_UNUSED(pskb) = 0xe5;
#ifdef CONFIG_RTK_MESH
				if (priv->pmib->dot11OperationEntry.guest_access == 2)
					RTL8192CD_SKB_LEGACY_UNUSED(pskb) = 0;
#endif
			}
#endif
...
mcast_netif_rx:
	...
	if (GET_MAP_PROFILE(priv) == MAP_PROFILE_TWO) {
		if (((priv->pmib->multi_ap.multiap_bss_type & BIT(7)) || (priv->pmib->multi_ap.multiap_bss_type & BIT(6))) && MAP_VLAN_ID) {
			if (((*(unsigned short *)(SKB_MAC_HEADER(pskb) + MACADDRLEN*2) == __constant_htons(ETH_P_8021Q) && (*(unsigned short *)(SKB_MAC_HEADER(pskb) + MACADDRLEN*2 + 4) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))))) {
				printk("%s: %s %d, Removing vlan tag for eapol\n", priv->dev->name, __FUNCTION__, __LINE__);
				memmove((unsigned short *)(SKB_MAC_HEADER(pskb) + MACADDRLEN*2), (unsigned short *)(SKB_MAC_HEADER(pskb) + MACADDRLEN*2 + 4), pskb->len);
				// skb_push(pskb, 4);
				pskb->len -= 4;
				skb_set_tail_pointer(pskb, pskb->len);
			}
		}
	}
#endif
...
//TODO, handle eap frame to upper layer
		...
		#if (defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
		if(priv->pmib->miscEntry.a4_enable) {
			if(a4_rx_check_reuse(priv, pfrinfo, GET_MY_HWADDR))
				return FAIL;
		}
		#endif
				...
				struct rx_frinfo *inputPfrinfo)
{
	...
	unsigned char is_mesh_amsdu = 0;
		...
		extern struct mw5_legacy_timer G5_rx_drop_timer;
#endif
	/*AR018*/
...
#define G24_THRESHOLD (20<<20) /* Unit=bits per second */
			if(totalb >= G24_THRESHOLD) {
				...
				mw5_legacy_mod_timer(&G5_rx_drop_timer, jiffies+HZ);	
			}
			prev_jiffies = j;
			prev_rx_bytes = rxb;
			prev_tx_bytes = txb;
	...
	/* ============== Start to process RX dataframe ============== */
#ifndef __ECOS
#if (defined(CONFIG_RTK_VOIP_QOS)|| defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) || defined(CONFIG_RTL_MULTI_LAN_DEV) || defined(CONFIG_RTL_EN_UNKNOWNUC_ONLY_TO_CPU) || defined(CONFIG_RTL_NIC_QUEUE)) && (!(defined(CONFIG_RTL8672) && defined(__LINUX_3_18__))) && (!defined(NOT_RTK_BSP))
#ifdef MBSSID
		...
		{
#ifdef CONFIG_CROSSBAND_REPEATER_SUPPORT_WIFI5_WIFI6
				if((crossband_check_loopback(pfrinfo->sa, CROSSBAND_2G))
					|| !memcmp(pfrinfo->sa,GET_ROOT(priv)->pmib->dot11OperationEntry.hwaddr,MACADDRLEN))
#else
				if((priv->crossband.crossband_priv_sc && !memcmp(pfrinfo->sa,GET_ROOT(priv->crossband.crossband_priv_sc)->pmib->dot11OperationEntry.hwaddr,MACADDRLEN))
					|| !memcmp(pfrinfo->sa,GET_ROOT(priv)->pmib->dot11OperationEntry.hwaddr,MACADDRLEN))
#endif
				{
					rtl_kfree_skb(priv, pfrinfo->pskb, _SKB_RX_);
...
#ifdef CONFIG_DIAG_WIFI
		/*AR018 ; add for struct vap_status_diag_info*/
		if(OPMODE & WIFI_AP_STATE) {
#ifdef A4_STA
		if(priv->pmib->miscEntry.a4_enable)
			pbuf = pframe + pfrinfo->hdr_len + sizeof(struct wlan_llc_t) + 11;
		else
#endif
			pbuf = pframe + pfrinfo->hdr_len + sizeof(struct wlan_llc_t) + 3;

		if (*(unsigned short *)pbuf == __constant_htons(ETH_P_ARP)){
			struct arphdr *arp;
			arp = (struct arphdr *)(pbuf + 2);
			if (arp->ar_pro == __constant_htons(ETH_P_IP)) {
				if(arp->ar_op == __constant_htons(ARPOP_REQUEST)
					|| arp->ar_op == __constant_htons(ARPOP_RREQUEST)){
						priv->pshare->arp_req_cnt1++;
						pfrinfo->pskb->cb[8]=0x11;
					}
				}
			}	
		}
...
//	collect_monitor_info(priv, pfrinfo);
	...
	if (opmode & WIFI_AP_STATE)
	{
		...
		if (IS_BSSID(priv, bssid))
		{
			...
			{
#ifdef RING_BASED_DATA_FRAME_QUEUE
				if (!(reuse = rtk_data_queue_enqueue(priv, &priv->data_queue, pfrinfo))) {
					rtl_atomic_inc(&priv->pshare->rx_enqueue_cnt);
				}
#else
				SMP_LOCK_RX_DATA(x);
...
// No need due to data from Data CPU process now are all processed as 802.3 format packets
#if 0  // #ifdef A4_STA
    if(pfrinfo->to_fr_ds == 3 && priv->pmib->miscEntry.a4_enable) {
        da = GetAddr1Ptr(pframe);
        sa = GetAddr2Ptr(pframe);
	...
	if (OPMODE & WIFI_AP_STATE) {
        ...
        #ifdef A4_STA
            if(!pstat && priv->pmib->miscEntry.a4_enable && pfrinfo->to_fr_ds == 3) {
                pstat = get_macidinfo(priv, pfrinfo->rxpkt_info->macid);
            }
								...
								if (pfrinfo->da[0] == 0xff) {
									if (pstat->rx_pkts_bc > BROADCAST_STORM_THRESHOLD) {
										...
										if (1) {
											...
											//printk("BC PKT SEND %d, leng = %d, pnewskb->len = %d\r\n", __LINE__, pnewskb->len, pnewskb->len);
											dcache_flush_range(pnewskb->data,(unsigned int)pnewskb->data + pnewskb->len);
											taroko_t2h_ipc_pkt_send(priv, pnewskb, (unsigned int)pnewskb->data, pnewskb->len);
										}else
#else
										if (1) {
											dev_kfree_skb_any(pnewskb);
...
#if defined(TAROKO_0) 
#if defined(MBSSID)
#if defined(PE_WIFI_RX_TO_WIFI_TX_MC)
									if(1)
									{
										...
										for (i=0; i<RTL8192CD_NUM_VWLAN; i++) 
										{
											if (IS_DRV_OPEN(priv->pvap_priv[i])) 
											{
												...
												skb_cp = skb_copy(pnewskb, GFP_ATOMIC);
												if(skb_cp)
												{
													dcache_flush_range(skb_cp->data,(unsigned int)skb_cp->data + skb_cp->len);
													if (rtl8192cd_start_xmit(skb_cp, priv_vap->dev))
														rtl_kfree_skb(priv, skb_cp, _SKB_TX_);
												}
											}
										}
										if(priv->proot_priv)
										{
											if (rtl8192cd_start_xmit(pnewskb, priv->proot_priv->dev))
												rtl_kfree_skb(priv, pnewskb, _SKB_TX_);
										}
										else
										{
...
#endif
			{
...
#endif
				{
					if (IEEE8021X_FUN
						...
						unsigned int shift_length = pskb->truesize - pskb->len;
						taroko_t2h_ipc_pkt_recv(pskb, pskb->data - shift_length, pskb->truesize);	
...
{
	unsigned char	*sa, *da, *myhwaddr, *pframe, is_amsdu=0;
	unsigned int	frtype;
	...
	if(pfrinfo == NULL || pfrinfo->pskb==NULL || pfrinfo->pskb->data == NULL)
		return 1;
#endif
//	is_amsdu = (is_qos_data(pframe) & (*GetQosControl(pframe) & BIT(7)));
...
//		printk("pfrinfo->hdr_len == 0\n");
	...
	pfrinfo->amsdu = is_qos_data(pframe) && (*GetQosControl(pframe) & BIT7);
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	if (OFFLOAD_ENABLE(priv))
...
#endif
#ifdef A4_STA
		&& (priv->pmib->miscEntry.a4_enable ? (!is_amsdu || (pfrinfo->to_fr_ds != 0x03)) : TRUE)
#endif
	)
		return 1;
...
#if defined(UNIVERSAL_REPEATER) && defined(A4_STA)
	if(IS_DRV_OPEN(GET_VXD_PRIV(priv)) && isEqualMACAddr(GET_VXD_PRIV(priv)->br_mac, sa))
		return 1;
#endif
...
#ifdef BSS_MONITOR_SUPPORT
if(priv->pshare->rf_ft_var.monitor_enable || priv->pmib->dot11nConfigEntry.dot11nCoexist_ap_chk)
{
	int Addr_match=0;
	...
	if (pfrinfo->rf_info.rssi && memcmp(GET_MY_HWADDR, GetAddr1Ptr(pframe), MACADDRLEN)) {
		...
		if (!IS_MCAST(GetAddr1Ptr(pframe))) {
			...
			if (IS_DRV_OPEN(vxd_priv) && !memcmp(vxd_priv->pmib->dot11OperationEntry.hwaddr, GetAddr1Ptr(pframe), MACADDRLEN))
				need_transfer = 1;
#endif
...
void rtl88XX_ipc_rx_isr(struct rtl8192cd_priv *priv, struct sk_buff *pskb)
{
            ...
            mw5_linux6_sanitize_c2h(priv, pfrinfo, &rx_desc_status);
            if (rx_desc_status.C2HPkt) {
	...
	if (IS_SUPPORT_HW_FILL_MACID(priv)) {
			...
			if (!pfrinfo->physt) {
...
#endif
				{
					...
					------------------------------------------------------*/
#ifndef __ECOS
#if (defined(CONFIG_RTL_QOS_PATCH) || defined(CONFIG_RTK_VOIP_QOS) || defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) || defined(CONFIG_RTL_MULTI_LAN_DEV) || defined(CONFIG_RTL_EN_UNKNOWNUC_ONLY_TO_CPU) || defined(CONFIG_RTL_NIC_QUEUE)) && (!(defined(CONFIG_RTL8672) && defined(__LINUX_3_18__))) && (!defined(NOT_RTK_BSP))
	#ifndef CONFIG_OPENWRT_SDK
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
								priv->pshare->linux6_rx_icv_errors++;
#endif
								rx_pkt_exception_88XX(priv, rx_desc_status.CRC32, rx_desc_status.ICVERR, rx_desc_status.RX_RATE);
...
void rtl88XX_rx_isr(struct rtl8192cd_priv *priv)
{
	...
	if (priv->pshare->rf_ft_var.limit_rxloop)
		...
		max_cnt = (NUM_RX_DESC_IF(priv) *1/4);
#else
	else
	...
	/*
	 * The vendor RX engine remains responsible for descriptor ownership,
	 * refill and status parsing.  When it is entered from NAPI, let the
	 * NAPI budget be the outer scheduling limit instead of the historical
	 * tasklet packet limit.
	 */
	if (priv->pshare->linux6_napi_polling &&
	    priv->pshare->linux6_rx_budget > 0)
		max_cnt = priv->pshare->linux6_rx_budget;
#endif
	...
	{
#if defined(CP3_WIFI_RX) && defined(TAROKO_0)	
		CP3_test_start();
#endif
		...
		if (
#ifdef CONFIG_RTL8192CD_LINUX6_BUS
		    !priv->pshare->linux6_napi_polling &&
#endif
		    (jiffies - start_time) >= max_time)
		{
...
#if 1
		if ((((unsigned long)pskb & 0x80000000) != 0x80000000) || 
			(((unsigned long)pskb->data & 0x80000000) != 0x80000000)|| (pskb->end < pskb->head)) {
			panic_printk("[%s %d][%s] skb: %p\n", __FUNCTION__, __LINE__, priv->dev->name, pskb);
			if (pskb->data == NULL) {
				panic_printk("[%s %d][%s] skb->data: NULL\n", __FUNCTION__, __LINE__, priv->dev->name);
			} else {
				panic_printk("[%s %d][%s] skb->data: %p\n", __FUNCTION__, __LINE__, priv->dev->name, pskb->data);
			}
			if (pskb->head == NULL) {
				panic_printk("[%s %d][%s] skb->head: NULL\n", __FUNCTION__, __LINE__, priv->dev->name);
			} else {
				panic_printk("[%s %d][%s] skb->head: %p\n", __FUNCTION__, __LINE__, priv->dev->name, pskb->head);
			}
			if (pskb->tail == NULL) {
				panic_printk("[%s %d][%s] skb->tail: NULL\n", __FUNCTION__, __LINE__, priv->dev->name);
			} else {
				panic_printk("[%s %d][%s] skb->tail: %p\n", __FUNCTION__, __LINE__, priv->dev->name, pskb->tail);
			}
			if (pskb->end == NULL) {
				panic_printk("[%s %d][%s] skb->end: NULL\n", __FUNCTION__, __LINE__, priv->dev->name);
			} else {
				panic_printk("[%s %d][%s] skb->end: %p\n", __FUNCTION__, __LINE__, priv->dev->name, pskb->end);
			}
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
        /* v44.38: the integrated RTL8197FS RXBD stores an OEM direct bus
         * address, whereas RTL8812BRH uses a Linux streaming mapping. */
        if (phw->rx_infoL[cur_host_idx].paddr) {
            unsigned int mw5_type = (priv->pshare->type >> TYPE_SHIFT) & TYPE_MASK;
            unsigned int mw5_len = RX_BUF_LEN - sizeof(struct rx_frinfo) - 64;

            if (mw5_type == TYPE_EMBEDDED) {
                rtl8197f_wlan_dma_sync_rx_for_cpu(priv,
                    phw->rx_infoL[cur_host_idx].paddr, mw5_len);
            } else if (priv->pshare->linux_dev) {
                dma_unmap_single(priv->pshare->linux_dev,
                    (dma_addr_t)phw->rx_infoL[cur_host_idx].paddr,
                    mw5_len, DMA_FROM_DEVICE);
                phw->rx_infoL[cur_host_idx].paddr = 0;
...
#if	0 /*eric-8822 check csi*/
{
	...
	if(frtype_tmp == Type_Action_No_Ack) {
		...
		{	// 20100818 this case should not happened, but we still add here to avoid some potential queuing more buffer in wifi driver
			...
			pfrinfo->pskb = pskb;
#if defined(TAROKO_0)
			pfrinfo->pskb->truesize = rx_desc_status.RXBuffSize;
#endif
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
			if (OFFLOAD_ENABLE(priv))
				pfrinfo->pktlen = rx_desc_status.PKT_LEN;
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
			if (OFFLOAD_ENABLE(priv)) {
				if(pfrinfo->rxpkt_info == NULL) {
					//panic_printk("gather_state=%d, FS=%d, LS:%d, C2HPkt:%d, RXBuffSize:%d, PKT_LEN:%d\n", 
					//	priv->pshare->gather_state, rx_desc_status.FS, rx_desc_status.LS, rx_desc_status.C2HPkt, rx_desc_status.RXBuffSize, rx_desc_status.PKT_LEN);
  				    pskb->data -= skb_shift_size; // skb_shift_size == sizeof(RX_DESC_88XX)
	                skb_reset_tail_pointer(pskb); // skb_shift_size == sizeof(RX_DESC_88XX)
	                goto rx_reuse;
				} else {
					if (pfrinfo->rxpkt_info->pkt_type == RXPKTINFO_TYPE_D2H) {
						...
						pskb->data -= skb_shift_size;
						pskb->len = rx_desc_status.RXBuffSize;
						new_skb = skb_copy(pskb, GFP_ATOMIC);
						if(new_skb)
						{
							dcache_flush_range(new_skb->data,(unsigned int)new_skb->data + rx_desc_status.RXBuffSize);
							taroko_t2h_ipc_pkt_recv(new_skb, new_skb->data, rx_desc_status.RXBuffSize);
						}		
						else
						{
...
#endif // defined(MP_TEST)
			{
                ...
                if (skb_used_size <= RX_BUF_LEN) {
                    ...
                    if (rx_desc_status.ICVERR) {
						...
						if ((new_skb != NULL) && (priv != NULL))
							rtl_kfree_skb(priv, new_skb, _SKB_RX_);
						/* With DELAY_REFILL_RX_BUF, new_skb is intentionally NULL
						 * here.  Do not report an allocation failure for ICVERR. */
#endif
						priv->ext_stats.rx_reuse++;
						//printk("%s(%d), reuse(0x%x)\n", __FUNCTION__, __LINE__, (unsigned int)priv->ext_stats.rx_reuse);
						goto rx_reuse;
...
#ifdef CONFIG_WLAN_HAL_8822BE //eric-txbf, rx CSI & work around
					...
					if (reuse) {
						...
						if ((priv!=NULL) && (pskb != NULL))
							rtl_kfree_skb(priv, pskb, _SKB_RX_);
						else
							printk("%s %d allocate failure\r\n", __FUNCTION__,__LINE__);
						reuse = 0;
#else
                        pskb->data -= skb_shift_size; // skb_shift_size is (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size + sizeof(RX_DESC_88XX))
                        skb_reset_tail_pointer(pskb); // skb_shift_size is (pfrinfo->rxbuf_shift + pfrinfo->driver_info_size + sizeof(RX_DESC_88XX))
...
//					pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[cur_host_idx].paddr, (RX_BUF_LEN - sizeof(struct rx_frinfo)), PCI_DMA_FROMDEVICE);
...
#if defined(CONFIG_RTL8190_PRIV_SKB)
#ifdef CONCURRENT_MODE
			if (skb_free_num[priv->pshare->wlandev_idx] == 0 
#ifdef RX_SKB_QUEUE
			&& priv->pshare->skb_queue.qlen == 0
#endif
			)
#else
			if (skb_free_num == 0 
			...
			if(new_skb == NULL) {
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
				priv->pshare->linux6_rx_refill_alloc_failures++;
				pr_warn_ratelimited("rtl8192cd: MW5 actual RX refill allocation failed if=%s\n",
					priv->dev ? priv->dev->name : "?");
#endif
				refill = 0;
				goto rx_done;
			}
			...
			if (priv->dev) {
				if (pskb->data == NULL) {
					panic_printk("[%s %d][%s] skb->data NULL! skb: %p\n", __FUNCTION__, __LINE__, priv->dev->name, pskb);
				}
				if (pskb->head == NULL) {
					panic_printk("[%s %d][%s] skb->head: NULL\n", __FUNCTION__, __LINE__, priv->dev->name);
				}
...
rx_reuse:
    #if 0   /*Filen: for debug*/
    {
		...
		if (cmp_flags)
		{
			...
			skb_reserve(pskb, 32);
 #else
			skb_reserve(pskb, RTL_PRIV_DATA_SIZE);
 #endif
...
//			phw->rx_infoL[cur_host_idx].paddr = get_physical_addr(priv, pskb->data, (RX_BUF_LEN - offset), PCI_DMA_FROMDEVICE);
        ...
        rtl_cache_sync_wback(priv, (unsigned long)(bus_to_virt(phw->rx_infoL[cur_host_idx].paddr)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 
            ...
            _dma_cache_wback_inv((unsigned long)(bus_to_virt(phw->rx_infoL[cur_host_idx].paddr)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 
                RX_BUF_LEN - offset);
#endif //CONFIG_RTL_8198F
...
#ifdef CONFIG_RTL8192CD_LINUX6_BUS
	if (priv->pshare->linux6_napi_polling)
		priv->pshare->linux6_rx_work_done =
			min(n, priv->pshare->linux6_rx_budget);
#endif
