...
#define IS_UDP_PROTO(mac)		(((mac[12]==0x08) && (mac[13]==0x00) && (mac[23]==0x11)) ||((mac[12]==0x81)&&(mac[13]==0x00)&&(mac[16]==0x08)&&(mac[17]==0x00)&& (mac[27]==0x11)))
...
#define IS_IGMP_PROTO(mac)	(((mac[12]==0x08) && (mac[13]==0x00) && (mac[23]==0x02)) ||((mac[12]==0x81)&&(mac[13]==0x00)&&(mac[16]==0x08)&&(mac[17]==0x00)&& (mac[27]==0x02)))
#define IS_IGMP_PROTO2(pro,pay)	(((pro[0]==0x08) && (pro[1]==0x00) && (pay[23-14]==0x02)))
...
/* v44.5: pvxd_priv is optional during Linux6 root-radio hardware init.
 * The vendor macro used to dereference it unconditionally from SwChnl(),
 * which produced the observed BadVA 0x460 before any VXD had been created. */
#define OPMODE_VXD			((GET_VXD_PRIV(priv) && GET_MIB(GET_VXD_PRIV(priv))) ? (((ACTIVE_ID > 0) && ((GET_MIB((GET_VXD_PRIV(priv))))->dot11OperationEntry.opmode & WIFI_STATION_STATE)) ? priv->pshare->mclone_sta[ACTIVE_ID-1].opmode : (GET_MIB(GET_VXD_PRIV(priv)))->dot11OperationEntry.opmode) : 0)
...
#else
#define ACTIVE_ID 		(0)
#define OPMODE_VXD		((GET_VXD_PRIV(priv) && GET_MIB(GET_VXD_PRIV(priv))) ? (GET_MIB((GET_VXD_PRIV(priv))))->dot11OperationEntry.opmode : 0)
