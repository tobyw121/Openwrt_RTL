...
/* v44.48.2: explicit MW5 descriptor conversion.  The vendor GET_DESC()
 * macro depends on a hidden local variable named Adapter; diagnostics must
 * not inherit that implicit requirement. */
#define MW5_DESC_TO_CPU(_priv, _val) \
    ((_GET_HAL_DATA((_priv))->AccessSwapCtrl & HAL_ACCESS_SWAP_MEM) ? \
     HAL_le32_to_cpu((_val)) : (_val))

/* Pack up to four bytes in wire order without unaligned MIPS loads. */
static inline u32 mw5_diag_pack4(const u8 *buf, u32 len, u32 off)
{
    u32 v = 0;
    u32 i;
...
#endif
#if defined(CONFIG_RTK_LINUX_VLAN_SUPPORT)
extern rtk_linux_vlan_ctl_t *rtk_vlan_ctl_p;
extern int rtk_linux_vlan_enable;
#endif
...
__PE_DMEM__ unsigned short BG_TABLE[2][BG_TABLE_SIZE] = {{73,68,63,57,52,47,42,37,31,30,27,25,23,22,20,18,16,14,11,9,8},
				{108,108,108,108,108,108,108,108,108,108,108,108,72,72,48,48,36,12,11,11,4}};
__PE_DMEM__ unsigned short MCS_40M_TABLE[2][MCS_40M_TABLE_SIZE] = {{73,68,63,57,53,45,39,37,31,29,28,26,25,23,21,19,17,12},
					{7,7,7,7,7,7,7,7,5,4,4,4,2,2,2,0,0,0}};
__PE_DMEM__ unsigned short MCS_20M_TABLE[2][MCS_20M_TABLE_SIZE]={{73,68,65,60,55,50,45,40,31,32,27,26,24,22,21,19,18,16,14,11,9,8},
				...
				struct net_device *dev, struct net_device *wdsDev, struct tx_insn *txcfg)
{
    ...
    if (IEEE8021X_FUN && pstat && (pstat->ieee8021x_ctrlport == DOT11_PortStatus_Unauthorized) &&
		...
		((*((UINT16 *)((UINT8 *)skb->data + ETH_ALEN*2)) == __constant_htons(ETH_P_8021Q)) && (*((UINT16 *)((UINT8 *)skb->data + ETH_ALEN*2 + 4)) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))) ||
		#endif
	...
	)
{
	...
	if (is_qos_data(pframe)) {
		...
		if (pstat) {
#ifdef CONFIG_RTK_MESH		
			if (GetFrameSubType(pframe) == WIFI_11S_MESH_ACTION) {				
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
static inline int mw5_linux6_8822b_tx_dma(struct rtl8192cd_priv *priv,
                                           unsigned int halQnum)
{
	unsigned int type;

	if (!priv || !priv->pshare || !priv->pshare->linux_dev ||
	    !IS_HARDWARE_TYPE_8822B(priv))
		return 0;
	type = (priv->pshare->type >> TYPE_SHIFT) & TYPE_MASK;
	if (type != TYPE_PCI_DIRECT)
		return 0;

	/* RTL8822B uses the V1 queue layout.  Every queue before CMD_V1 is
	 * packet traffic and is mapped by mw5_linux6_tx_map(). */
	return halQnum < HCI_TX_DMA_QUEUE_CMD_V1;
}

static inline void mw5_linux6_tx_unmap(struct rtl8192cd_priv *priv,
                                       unsigned long desc_addr, unsigned int len)
{
	dma_addr_t dma;

	if (!desc_addr || !len)
		return;
	dma = (dma_addr_t)(desc_addr - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL);
	dma_unmap_single(priv->pshare->linux_dev, dma, len, DMA_TO_DEVICE);
	priv->pshare->linux6_tx_dma_unmaps++;
}
#endif
...
//        unsigned long flags;
    ...
    if (0 == rtl_atomic_add_unless(&priv->pshare->tx_recycle_trigger, 1, 1)) {
        return 0;
    }
#endif
...
#if defined(TAROKO_0) && defined(CP3_TEST_TX_RECYCLE)
	CP3_test_stop(PER_PKT_CYCLE, 1);
	panic_printk("TX RECYCLE loop: \r\n");
	CP3_test_start();	
#endif
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
		/* v44.56: pdescinfo->type is the memory-owner type, not a DMA-map
		 * lifetime flag.  Chained/reserved descriptors may still carry a real
		 * header mapping.  Unmap every recorded 8822B mapping exactly once. */
		if (mw5_linux6_8822b_tx_dma(priv, halQnum) &&
		    pdescinfo->paddr && pdescinfo->len) {
			mw5_linux6_tx_unmap(priv, pdescinfo->paddr, pdescinfo->len);
			pdescinfo->paddr = 0;
			pdescinfo->len = 0;
		}
#endif
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
			/* v44.56: MW5 uses dma_map_single() for each 8822B TXBD
			 * segment.  _RESERVED_FRAME_TYPE_ controls buffer ownership, not
			 * DMA lifetime (e.g. chained fragments).  Every non-zero mapping
			 * therefore needs a matching unmap once this descriptor completed. */
			if (mw5_linux6_8822b_tx_dma(priv, halQnum) &&
			    pdescinfo->buf_paddr[cnt] && pdescinfo->buf_len[cnt]) {
				mw5_linux6_tx_unmap(priv, pdescinfo->buf_paddr[cnt],
				                      pdescinfo->buf_len[cnt]);
				pdescinfo->buf_paddr[cnt] = 0;
				pdescinfo->buf_len[cnt] = 0;
			}
#endif
...
//					rtl_kfree_skb(pdescinfo->priv, (struct sk_buff *)(pdescinfo->pframe), _SKB_TX_IRQ_);
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
				/* Same DMA-lifetime rule for AMSDU segments: ownership type says
				 * whether memory is freed, never whether a Linux DMA mapping exists. */
				if (mw5_linux6_8822b_tx_dma(priv, halQnum) &&
				    pdescinfo->buf_paddr_amsdu[cnt] && pdescinfo->buf_len_amsdu[cnt]) {
					mw5_linux6_tx_unmap(priv,
						pdescinfo->buf_paddr_amsdu[cnt],
						pdescinfo->buf_len_amsdu[cnt]);
					pdescinfo->buf_paddr_amsdu[cnt] = 0;
					pdescinfo->buf_len_amsdu[cnt] = 0;
				}
#endif
	...
	//					rtl_kfree_skb(pdescinfo->priv, (struct sk_buff *)(pdescinfo->pframe), _SKB_TX_IRQ_);
        ...
        rtl_atomic_inc(&(ptx_dma->tx_queue[halQnum].avail_txbd_num));
...
#if defined(TAROKO_0) && defined(CP3_TEST_TX_RECYCLE)
	CP3_test_stop(PER_PKT_CYCLE, 1);
	panic_printk("TX RECYCLE loop end: \r\n");
	CP3_test_start();	
#endif
...
#ifdef TX_PROCESS_REFINE
#define SMP_LOCK_TXDSR(__x__)
#define SMP_UNLOCK_TXDSR(__x__)
#else
#define SMP_LOCK_TXDSR(__x__)		SMP_LOCK_XMIT(__x__)
#define SMP_UNLOCK_TXDSR(__x__)		SMP_UNLOCK_XMIT(__x__)
#endif
...
#ifdef PCIE_POWER_SAVING_TEST //yllin
    if((priv->pwr_state >= L2) || (priv->pwr_state == L1)) {
        rtk_tasklet_clear_triggered(&(priv->pshare->tx_tasklet));
        return;
    }
#endif
...
#if defined(RX_SKB_QUEUE) && !defined(WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD)
...
#endif
#endif // !(WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD)
...
#ifdef CONFIG_RTK_MESH
			if (txcfg->is_11s) {
				idx = get_tx_sc_free_entry(priv, txcfg->pstat, (unsigned char *)&priv->ethhdr, txcfg->aggre_en);
			} else
#endif
...
{
#ifdef CONFIG_VERIWAVE_CHECK    
    if (priv->pshare->veriwave_sta_num > 0 || priv->pshare->rf_ft_var.veriwave_test) {
            return true;
    }
#endif
...
inline void set_ampdu_aggregation_density(struct rtl8192cd_priv *priv, struct tx_insn *txcfg, PTX_DESC_DATA_88XX pdesc_data)
{
...
#if defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8812FE) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8197G)
	if ((GET_CHIP_VER(priv) == VERSION_8822B)||(GET_CHIP_VER(priv) == VERSION_8198F)||(GET_CHIP_VER(priv) == VERSION_8822C)||(GET_CHIP_VER(priv) == VERSION_8812F)|| (GET_CHIP_VER(priv)==VERSION_8814B) || (GET_CHIP_VER(priv)==VERSION_8197G))
	...
	if (priv->pshare->rf_ft_var.ampdu_density != 0xff) {
		pdesc_data->ampduDensity = priv->pshare->rf_ft_var.ampdu_density;
	} else if (txcfg->privacy && ! is_veriwave_testing(priv)) {
		// check sta's ability
...
//	BOOLEAN			bCts2SelfEnable;
        ...
        if ((txcfg->pstat && (txcfg->pstat->aggre_mthd & AGGRE_MTHD_MPDU) && txcfg->aggre_en)
            ...
            if (txcfg->pstat && txcfg->pstat->ADDBA_ready[TID] && !txcfg->pstat->low_tp_disable_ampdu) {
				...
				if (GET_CHIP_VER(priv) == VERSION_8812F) {
					if (txcfg->pstat->aggre_mthd == AGGRE_MTHD_MPDU_AMSDU) {
						if ((txcfg->pstat->IOTPeer == HT_IOT_PEER_BROADCOM || txcfg->pstat->IOTPeer == HT_IOT_PEER_APPLE || txcfg->pstat->IOTPeer == HT_IOT_PEER_INTEL)
							&& txcfg->pstat->is_8k_amsdu) {
							if ((txcfg->pstat->amsdu_level == 11454 - sizeof(struct wlan_hdr)) && pdesc_data->maxAggNum > 8)
								pdesc_data->maxAggNum = 8;
							else if (pdesc_data->maxAggNum > 11)
								pdesc_data->maxAggNum = 11;
						} else if (txcfg->pstat->IOTPeer==HT_IOT_PEER_REALTEK_8814 && !txcfg->pstat->is_8k_amsdu) {
							if (pdesc_data->maxAggNum > 20)
								pdesc_data->maxAggNum = 20;
						}
					}
				}
#endif
...
#if ((MU_BEAMFORMING_SUPPORT == 1) && defined(CONFIG_WLAN_HAL_8812FE) )
			if (GET_CHIP_VER(priv)==VERSION_8812F && (pBeamInfo->beamformee_mu_cnt >1 && txcfg->pstat->isMUCandidate)) {
				txcfg->pstat->descLDPCCnt++;
				pdesc_data->dataLdpc = 0;
			}
			else
#endif
			{
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE) && defined(RTK_NL80211)
    /* v44.52 / MW5 pre-PTK EAPOL bootstrap.  Report17 proves management TX/RX
     * and open 2.4 GHz RX, while the first unicast EAPOL data exchange fails on
     * both radios.  Management responses use the default MACID context and are
     * OTA-functional.  Until hostapd opens the controlled port, transmit EAPOL
     * with MACID 0 while preserving the real 802.11 destination/ACK semantics.
     * USE_RATE prevents firmware RA from consulting the not-yet-stable station
     * context.  Once authorized, normal traffic uses the real station MACID. */
    if (txcfg->mw5_eapol_frame && txcfg->pstat &&
        txcfg->pstat->ieee8021x_ctrlport == DOT11_PortStatus_Unauthorized &&
        (((GET_CHIP_VER(priv) == VERSION_8822B) && priv->pshare->linux6_radio_index == 0) ||
         ((GET_CHIP_VER(priv) == VERSION_8197F) && priv->pshare->linux6_radio_index == 1))) {
        unsigned int mw5_real_macid = pdesc_data->macId;
        ...
        pdesc_data->disRTSFB = TRUE;
        pdesc_data->dataRate = txRate;
        pr_info_ratelimited("rtl8197f-wlan: v44.52 MW5 EAPOL bootstrap if=%s macid=%u->0 rate=%u q=%u ack=%u\n",
            priv->dev ? priv->dev->name : "none", mw5_real_macid,
            (unsigned int)txRate, (unsigned int)txcfg->q_num,
            (unsigned int)txcfg->need_ack);
    }
#endif
    ...
    if (txcfg->need_ack) {
        ...
        if (txcfg->q_num == MANAGE_QUE_NUM
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
            || (GET_CHIP_VER(priv) == VERSION_8822B && priv->pshare &&
                priv->pshare->linux6_radio_index == 0 &&
                txcfg->q_num == HIGH_QUEUE && txcfg->fr_type == _PRE_ALLOCMEM_ &&
                (GetFrameSubType(txcfg->phdr) == WIFI_AUTH ||
                 GetFrameSubType(txcfg->phdr) == WIFI_ASSOCRSP ||
                 GetFrameSubType(txcfg->phdr) == WIFI_REASSOCRSP))
#endif
           ) {
            pdesc_data->rtyLmtEn = TRUE;
            ...
            if (GET_CHIP_VER(priv) == VERSION_8822B && priv->pshare &&
                priv->pshare->linux6_radio_index == 0 && txcfg->q_num == HIGH_QUEUE)
                pr_info_ratelimited("rtl8197f-wlan: v44.54 MW5 5G critical HI0 TXDESC if=%s subtype=0x%x rate=%u retry=%u\n",
                    priv->dev ? priv->dev->name : "none",
                    (unsigned int)GetFrameSubType(txcfg->phdr),
                    (unsigned int)pdesc_data->dataRate,
                    (unsigned int)pdesc_data->dataRtyLmt);
#endif
        }
        #ifdef WDS
        else if (txcfg->wdsIdx >= 0) {
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE) && defined(RTK_NL80211)
        /* Carry the OEM EAPOL retry request into the descriptor for both MW5
         * radios.  Fixed-rate data otherwise bypasses the normal station retry
         * branch above. */
        if (txcfg->mw5_eapol_frame && txcfg->pstat &&
            txcfg->pstat->ieee8021x_ctrlport == DOT11_PortStatus_Unauthorized &&
            (((GET_CHIP_VER(priv) == VERSION_8822B) && priv->pshare->linux6_radio_index == 0) ||
             ((GET_CHIP_VER(priv) == VERSION_8197F) && priv->pshare->linux6_radio_index == 1))) {
            pdesc_data->rtyLmtEn = TRUE;
            pdesc_data->dataRtyLmt = txcfg->retry ? txcfg->retry : 0x20;
            pr_info_ratelimited("rtl8197f-wlan: v44.52 MW5 EAPOL retry if=%s limit=%u\n",
                priv->dev ? priv->dev->name : "none",
                (unsigned int)pdesc_data->dataRtyLmt);
        }
#endif
		...
		if(priv->pshare->rf_ft_var.low_power == 1){ /* decided by STA*/
			if(txcfg->pstat && txcfg->fr_type == _SKB_FRAME_TYPE_ && txcfg->pstat->power <= 3){
				...
				if (GET_CHIP_VER(priv) == VERSION_8814B)
					pdesc_data->TXPowerOffset = 0; /* TABLE_1 OFFSET0 */
				else
#endif
...
#ifdef CONFIG_WLAN_HAL_8814BE
				if (GET_CHIP_VER(priv) == VERSION_8814B)
					pdesc_data->TXPowerOffset = 2; /* TABLE_2 OFFSET0 */
				else
#endif
...
void rtl88XX_signin_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, unsigned char convHdr)
{
    ...
    BOOLEAN use_hwtxsc = FALSE;
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    /* v44.48.2 compile fix: diagnostics use MW5_DESC_TO_CPU(priv, value),
     * which carries the adapter explicitly and cannot depend on a hidden
     * local variable named Adapter. */
    u32 mw5_eapol_fill_idx = 0;
#endif
#if 0
...
if((priv->pshare->rf_ft_var.txforce != 0xff || (txcfg->pstat && txcfg->pstat->force_rate))
		...
		if (OFFLOAD_ENABLE(priv)) {
			if (GetFrameType(pwlhdr) == WIFI_DATA) {
                ...
                if (txcfg->q_num >= HIGH_QUEUE_V2 && txcfg->q_num <= HIGH_QUEUE19_V1)
                    txcfg->h2d_tid = TXDESC_DMA_CHANNEL15;
                else
				{
					txcfg->h2d_tid = ((struct sk_buff *)txcfg->pframe)->cb[1];
					if(txcfg->h2d_tid > 7)
					{
						panic_printk("[%s] FrameSubType=%d, q_num=%d, h2d_tid=%d\n",__FUNCTION__,GetFrameSubType(pwlhdr), txcfg->q_num,txcfg->h2d_tid);
						txcfg->h2d_tid = 0;
					}
				}
			...
			rtl88XX_fill_txPktInfo(priv, txcfg, txcfg->h2d_type, pwlhdr);
		}
#endif
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
        if (txcfg->mw5_eapol_frame && cur_q)
            mw5_eapol_fill_idx = cur_q->host_idx;
#endif
#ifdef TX_SHORTCUT
        if (fit_shortcut) {
#ifdef WLAN_HAL_HW_TX_SHORTCUT_HDR_CONV
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
        if (txcfg->mw5_eapol_frame && GET_CHIP_VER(priv) == VERSION_8822B &&
            priv->pshare->linux6_radio_index == 0 && cur_q && cur_q->pTXBD_head &&
            cur_q->ptx_desc_head && cur_q->total_txbd_num) {
            u32 mw5_idx = mw5_eapol_fill_idx;
            PTX_DESC_88XX mw5_txd = ((PTX_DESC_88XX)cur_q->ptx_desc_head) + mw5_idx;
            PTX_BUFFER_DESCRIPTOR mw5_txbd = cur_q->pTXBD_head + mw5_idx;
            priv->pshare->linux6_eapol_desc_q = halQNum;
            priv->pshare->linux6_eapol_desc_idx = mw5_idx;
            priv->pshare->linux6_eapol_host_before_fill = mw5_idx;
            priv->pshare->linux6_eapol_host_after_fill = cur_q->host_idx;
            priv->pshare->linux6_eapol_desc_dw0 = MW5_DESC_TO_CPU(priv, mw5_txd->Dword0);
            priv->pshare->linux6_eapol_desc_dw1 = MW5_DESC_TO_CPU(priv, mw5_txd->Dword1);
            priv->pshare->linux6_eapol_desc_dw2 = MW5_DESC_TO_CPU(priv, mw5_txd->Dword2);
            priv->pshare->linux6_eapol_desc_dw3 = MW5_DESC_TO_CPU(priv, mw5_txd->Dword3);
            priv->pshare->linux6_eapol_desc_dw4 = MW5_DESC_TO_CPU(priv, mw5_txd->Dword4);
            priv->pshare->linux6_eapol_desc_dw5 = MW5_DESC_TO_CPU(priv, mw5_txd->Dword5);
            priv->pshare->linux6_eapol_txbd0_dw0 = MW5_DESC_TO_CPU(priv, mw5_txbd->TXBD_ELE[0].Dword0);
            priv->pshare->linux6_eapol_txbd0_dw1 = MW5_DESC_TO_CPU(priv, mw5_txbd->TXBD_ELE[0].Dword1);
            priv->pshare->linux6_eapol_txbd1_dw0 = MW5_DESC_TO_CPU(priv, mw5_txbd->TXBD_ELE[1].Dword0);
            priv->pshare->linux6_eapol_txbd1_dw1 = MW5_DESC_TO_CPU(priv, mw5_txbd->TXBD_ELE[1].Dword1);
            priv->pshare->linux6_eapol_txbd2_dw0 = MW5_DESC_TO_CPU(priv, mw5_txbd->TXBD_ELE[2].Dword0);
            priv->pshare->linux6_eapol_txbd2_dw1 = MW5_DESC_TO_CPU(priv, mw5_txbd->TXBD_ELE[2].Dword1);
            priv->pshare->linux6_eapol_last_macid = desc_data.macId;
            priv->pshare->linux6_eapol_last_tid = desc_data.tid;
            priv->pshare->linux6_eapol_last_hdr_len = desc_data.hdrLen;
            priv->pshare->linux6_eapol_last_llc_len = desc_data.llcLen;
            priv->pshare->linux6_eapol_last_payload_len = desc_data.frLen;
            priv->pshare->linux6_eapol_need_ack = txcfg->need_ack ? 1 : 0;
            priv->pshare->linux6_eapol_hdr_dw0 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 0);
            priv->pshare->linux6_eapol_hdr_dw1 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 4);
            priv->pshare->linux6_eapol_hdr_dw2 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 8);
            priv->pshare->linux6_eapol_hdr_dw3 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 12);
            priv->pshare->linux6_eapol_hdr_dw4 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 16);
            priv->pshare->linux6_eapol_hdr_dw5 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 20);
            priv->pshare->linux6_eapol_hdr_dw6 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 24);
            priv->pshare->linux6_eapol_hdr_dw7 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 28);
            priv->pshare->linux6_eapol_hdr_dw8 = mw5_diag_pack4(desc_data.pHdr, desc_data.hdrLen + desc_data.llcLen, 32);
            priv->pshare->linux6_eapol_payload_dw0 = mw5_diag_pack4(desc_data.pBuf, desc_data.frLen, 0);
            priv->pshare->linux6_eapol_payload_dw1 = mw5_diag_pack4(desc_data.pBuf, desc_data.frLen, 4);
            priv->pshare->linux6_eapol_payload_dw2 = mw5_diag_pack4(desc_data.pBuf, desc_data.frLen, 8);
            priv->pshare->linux6_eapol_payload_dw3 = mw5_diag_pack4(desc_data.pBuf, desc_data.frLen, 12);
            priv->pshare->linux6_eapol_desc_fills++;
            pr_info_ratelimited("rtl8197f-wlan: v44.50 EAPOL FRAME if=%s macid=%u tid=%u ack=%u hdr=%u+%u payload=%u bytes=%08x/%08x/%08x/%08x/%08x/%08x/%08x/%08x/%08x payload0=%08x/%08x/%08x/%08x\n",
                priv->dev ? priv->dev->name : "?", desc_data.macId, desc_data.tid,
                priv->pshare->linux6_eapol_need_ack, desc_data.hdrLen, desc_data.llcLen, desc_data.frLen,
                priv->pshare->linux6_eapol_hdr_dw0, priv->pshare->linux6_eapol_hdr_dw1,
                priv->pshare->linux6_eapol_hdr_dw2, priv->pshare->linux6_eapol_hdr_dw3,
                priv->pshare->linux6_eapol_hdr_dw4, priv->pshare->linux6_eapol_hdr_dw5,
                priv->pshare->linux6_eapol_hdr_dw6, priv->pshare->linux6_eapol_hdr_dw7,
                priv->pshare->linux6_eapol_hdr_dw8, priv->pshare->linux6_eapol_payload_dw0, priv->pshare->linux6_eapol_payload_dw1,
                priv->pshare->linux6_eapol_payload_dw2, priv->pshare->linux6_eapol_payload_dw3);
            pr_info_ratelimited("rtl8197f-wlan: v44.48 EAPOL TXDESC if=%s q=%u idx=%u host_after=%u dw=%08x/%08x/%08x/%08x/%08x/%08x txbd0=%08x/%08x txbd1=%08x/%08x txbd2=%08x/%08x fills=%llu\n",
                priv->dev ? priv->dev->name : "?", halQNum, mw5_idx, cur_q->host_idx,
                priv->pshare->linux6_eapol_desc_dw0, priv->pshare->linux6_eapol_desc_dw1,
                priv->pshare->linux6_eapol_desc_dw2, priv->pshare->linux6_eapol_desc_dw3,
                priv->pshare->linux6_eapol_desc_dw4, priv->pshare->linux6_eapol_desc_dw5,
                priv->pshare->linux6_eapol_txbd0_dw0, priv->pshare->linux6_eapol_txbd0_dw1,
                priv->pshare->linux6_eapol_txbd1_dw0, priv->pshare->linux6_eapol_txbd1_dw1,
                priv->pshare->linux6_eapol_txbd2_dw0, priv->pshare->linux6_eapol_txbd2_dw1,
                (unsigned long long)priv->pshare->linux6_eapol_desc_fills);
        }
#endif
...
init_deschead:
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    if (txcfg->mw5_eapol_frame && GET_CHIP_VER(priv) == VERSION_8822B &&
        priv->pshare->linux6_radio_index == 0 && cur_q && cur_q->reg_rwptr_idx) {
        u32 mw5_rw = RTL_R32(cur_q->reg_rwptr_idx);
        priv->pshare->linux6_eapol_host_before_sync = cur_q->host_idx;
        priv->pshare->linux6_eapol_rwptr_before_sync = mw5_rw;
        priv->pshare->linux6_eapol_hw_before_sync =
            (mw5_rw >> BIT_SHIFT_QUEUE_HW_IDX) & BIT_MASK_QUEUE_IDX;
    }
#endif
    ...
    if (txcfg->mw5_eapol_frame && GET_CHIP_VER(priv) == VERSION_8822B &&
        priv->pshare->linux6_radio_index == 0 && cur_q && cur_q->reg_rwptr_idx) {
        u32 mw5_rw = RTL_R32(cur_q->reg_rwptr_idx);
        priv->pshare->linux6_eapol_host_after_sync = cur_q->host_idx;
        priv->pshare->linux6_eapol_rwptr_after_sync = mw5_rw;
        priv->pshare->linux6_eapol_hw_after_sync =
            (mw5_rw >> BIT_SHIFT_QUEUE_HW_IDX) & BIT_MASK_QUEUE_IDX;
        priv->pshare->linux6_eapol_syncs++;
        pr_info_ratelimited("rtl8197f-wlan: v44.48 EAPOL SYNC if=%s q=%u host=%u->%u hw=%u->%u rwptr=%08x->%08x syncs=%llu\n",
            priv->dev ? priv->dev->name : "?", halQNum,
            priv->pshare->linux6_eapol_host_before_sync,
            priv->pshare->linux6_eapol_host_after_sync,
            priv->pshare->linux6_eapol_hw_before_sync,
            priv->pshare->linux6_eapol_hw_after_sync,
            priv->pshare->linux6_eapol_rwptr_before_sync,
            priv->pshare->linux6_eapol_rwptr_after_sync,
            (unsigned long long)priv->pshare->linux6_eapol_syncs);
    }
#endif
	return;
}
...
__IRAM_IN_865X unsigned int get_skb_priority(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat, int is_11s)
{
...
#if defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(3, 0, 0))
	unsigned char out_value = 0;
	unsigned char dscp      = 0;
	unsigned char skb_cb_priority_set = 0;
#endif
	...
	if (parsing) {
		...
		if (root_priv->service_priority_enabled == 1) {
			if (root_priv->service_priority_output != 9) {
				if (root_priv->service_priority_output >= 0 && root_priv->service_priority_output <= 7) {
					out_value = root_priv->service_priority_output;
				} else if (root_priv->service_priority_output == 8) {
					if ((skb->data[12] == 0x81) && (skb->data[13] == 0x00)) {
						dscp = (((skb->data[19]) & 0xFC) >> 2);
					} else {
						dscp = (((skb->data[15]) & 0xFC) >> 2);
					}
					if (root_priv->dscp_pcp_table && root_priv->dscp_pcp_table_enabled == 1 && dscp >= 0 && dscp <= 63) {
						out_value = *(root_priv->dscp_pcp_table + dscp);
					}
				} else {
					printk("[8192fe] error, unknown service priority output value \n");
					goto service_priority_abort;
				}
				if (!(a4_sta_lookup(priv, skb->data))) {
					skb->cb[_SKB_CB_PRIORITY] = out_value;
					skb_cb_priority_set = 1;
				} else {
					if ((skb->data[12] == 0x81) && (skb->data[13] == 0x00)) {
						skb->data[14] |= (out_value << 5);
						skb->cb[_SKB_CB_PRIORITY] = out_value;
						skb_cb_priority_set = 1;
					} else {
						skb->cb[_SKB_CB_PRIORITY] = out_value;
...
service_priority_abort:
...
#endif
		{
			...
			if ((protocol[0] == 0x81) && (protocol[1] == 0x00)) {
				pri = (skb->data[14] & 0xe0) >> 5;
			} else
...
#endif
		if(priv->pshare->rf_ft_var.icmp_tid && IS_ICMPV4_PROTO(skb->data))
			pri = priv->pshare->rf_ft_var.icmp_tid &7;
	}
...
#ifdef TX_PROCESS_REFINE
void smp_assign_txdsr_tasklet(void *info)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)info;
	tasklet_hi_schedule(&(priv->pshare->tx_tasklet.tasklet));
}
#endif

inline void schedule_txdsr_tasklet(struct rtl8192cd_priv *priv)
{
#ifdef TX_PROCESS_REFINE
	if (priv->pshare->curr_band == BAND_5G)
	{
		smp_call_function_single_async(TX_DSR_CPU, &(priv->pshare->txq_tasklet_csd));
	} else
#endif	
	{
		tasklet_hi_schedule(&(priv->pshare->tx_tasklet.tasklet));
	...
	while (skb_queue_len(&pstat->amsdu_tx_que[tid]) > 0) {
		...
		if (is_first) {
			if (skb_queue_len(&pstat->amsdu_tx_que[tid]) > 0) {
                if (getAvailableTXBD(priv, q_num) < (NUM_TX_DESC >> 1)) {
#ifdef SMP_SYNC
					if (rtk_tasklet_set_triggered(&(priv->pshare->tx_tasklet))) {
						schedule_txdsr_tasklet(priv);
					}
#else
...
int amsdu_timer_add(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid, int from_timeout)
{
	...
	if (CIRC_CNT(current_idx, priv->pshare->amsdu_timer_tail, AMSDU_TIMER_NUM) == 0) {
		...
		if( from_timeout == 2 ){
			//arrangement
			int arrange_idx, scan_idx;

			arrange_idx = scan_idx =  (current_idx-1) & (AMSDU_TIMER_NUM - 1);
			while(scan_idx != priv->pshare->amsdu_timer_tail ){
				if(priv->pshare->amsdu_timer[scan_idx].pstat){
					if(scan_idx != arrange_idx){
						priv->pshare->amsdu_timer[arrange_idx] = priv->pshare->amsdu_timer[scan_idx];
						priv->pshare->amsdu_timer[arrange_idx].pstat->amsdu_timer_id[tid] = arrange_idx + 1;
						priv->pshare->amsdu_timer[scan_idx].pstat = NULL;
					}
					arrange_idx = (arrange_idx - 1) & (AMSDU_TIMER_NUM - 1);
				}
				scan_idx = (scan_idx - 1) & (AMSDU_TIMER_NUM - 1);
			}
			if (arrange_idx == scan_idx) {
				pstat->tx_amsdu_timer_ovf++;
				return -1;
			}
			
			//tail is boundry case
			priv->pshare->amsdu_timer[arrange_idx] = priv->pshare->amsdu_timer[scan_idx];
			priv->pshare->amsdu_timer[arrange_idx].pstat->amsdu_timer_id[tid] = arrange_idx + 1;
			priv->pshare->amsdu_timer[priv->pshare->amsdu_timer_tail].pstat = NULL;
			priv->pshare->amsdu_timer_tail = arrange_idx;
		} else {
			pstat->tx_amsdu_timer_ovf++;
			return -1;
		}
	...
	if (!from_timeout || from_timeout == 2) {
		if (setup_timer)
...
#if defined(CONFIG_WLAN_HAL_8814BE) && (defined(CONFIG_RTL9607C) || defined(_CORTINA_))
	if (GET_CHIP_VER(priv) == VERSION_8814B)
	{
...
static int check_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	if (txcfg->fr_type == _SKB_FRAME_TYPE_)
	{
			...
			)
		{
			...
			if (
#if (defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
		((protocol == ETH_P_8021Q) && (*((UINT16 *)((UINT8 *)pskb->data + ETH_ALEN*2 + 4)) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))) ||
#endif
		(protocol == 0x888e))
				txcfg->frg_num = 1;
			...
			if (
				...
				((protocol == ETH_P_8021Q) && (*((UINT16 *)((UINT8 *)pskb->data + ETH_ALEN*2 + 4)) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))) ||
#endif
				(protocol == 0x888e) ){	/*EAP type packets*/
			...
			if (txcfg->aggre_en < FG_AGGRE_MSDU_FIRST) {
				...
				{ 
					...
					if(priv->pshare->rf_ft_var.manual_pe_enable)
					{
						//All BC/MC send to HI Q
						txcfg->q_num = HCI_TX_DMA_QUEUE_HI0_V2;
					}
					else
#endif
						txcfg->q_num = pri_to_qnum(priv, priority);
			...
			if (pskb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_URGENT) {
				txcfg->q_num = VO_QUEUE;
				pskb->cb[_SKB_CB_PRIORITY] = 7;
			}

#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE) && defined(RTK_NL80211)
			txcfg->mw5_eapol_frame = 0;
			/* v44.52 / Report17: both MW5 radios now receive AUTH/ASSOC, and
			 * open 2.4 GHz traffic is proven, but WPA2 never returns EAPOL M2.
			 * Tag pre-control-port EAPOL on both physical radios so the common
			 * HAL can bootstrap it without relying on the just-created station's
			 * firmware RA/MACID state.  Keep OEM queue/privacy/rate selection. */
			if (protocol == 0x888e && txcfg->pstat &&
			    txcfg->pstat->ieee8021x_ctrlport == DOT11_PortStatus_Unauthorized &&
			    ((GET_CHIP_VER(priv) == VERSION_8822B && priv->pshare->linux6_radio_index == 0) ||
			     (GET_CHIP_VER(priv) == VERSION_8197F && priv->pshare->linux6_radio_index == 1))) {
				txcfg->mw5_eapol_frame = 1;
				priv->pshare->linux6_eapol_oem_tx++;
				priv->pshare->linux6_eapol_last_q = txcfg->q_num;
				priv->pshare->linux6_eapol_last_rate = txcfg->tx_rate;
				priv->pshare->linux6_eapol_last_privacy = txcfg->privacy;
				priv->pshare->linux6_eapol_last_fixed = txcfg->fixed_rate;
				pr_info_ratelimited("rtl8197f-wlan: v44.52 MW5 pre-PTK EAPOL TX if=%s peer=%pM chip=%u aid=%u privacy=%u rate=%u fixed=%u q=%u count=%llu\n",
					priv->dev ? priv->dev->name : "none", da,
					(unsigned int)GET_CHIP_VER(priv),
					(unsigned int)REMAP_AID(txcfg->pstat),
					(unsigned int)txcfg->privacy, (unsigned int)txcfg->tx_rate,
					(unsigned int)txcfg->fixed_rate, (unsigned int)txcfg->q_num,
					(unsigned long long)priv->pshare->linux6_eapol_oem_tx);
			}
#endif
			...
			)
			{
				...
				if (!pstat->ADDBA_ready[priority]) {
					if ((pstat->ADDBA_req_num[priority] < 5) && !pstat->ADDBA_sent[priority]) {
						if(!pstat->recv_delba || (TSF_DIFF(jiffies, pstat->delay_addba) > RTL_SECONDS_TO_JIFFIES(5))){
						pstat->ADDBA_req_num[priority]++;
						pstat->ADDBA_sent[priority]++;
						SMP_UNLOCK_XMIT(flags);
						issue_ADDBAreq(priv, pstat, priority);
						SMP_LOCK_XMIT(flags);
						pstat->recv_delba = FALSE;
						}
					}
				}
			}
		...
		else
		{
			...
			if ((*da) == 0xff) { // broadcast 
				if (is_veriwave_testing(priv))
					txcfg->tx_rate = get_rate_from_bit_value(priv->pmib->dot11StationConfigEntry.desiredBcstRate);
				else
					txcfg->tx_rate = find_rate(priv, NULL, 0, 1);
			} else {				// multicast
...
#endif
	{
		...
		else if(!(txcfg->phdr && (GetFrameType(txcfg->phdr) == WIFI_MGT_TYPE) && 	((GetFrameSubType((unsigned char *) (txcfg->phdr)) == WIFI_PROBERSP)
			|| (GetFrameSubType((unsigned char *) (txcfg->phdr)) == WIFI_AUTH))))	//  exclude probe rsp, auth
		{
			pstat = get_stainfo(priv, da);
			txcfg->pstat = pstat;
		}
...
//	PHCI_TX_DMA_MANAGER_88XX	ptx_dma;
...
#ifdef SW_TXQ_RSVD_DESC
			{
				...
				if (txcfg->fr_type==_SKB_FRAME_TYPE_ && q_num_t>=BK_QUEUE && q_num_t<=VO_QUEUE)
				{
					...
					if (skb_t && pstat_t)
					{
						...
						int pri_t = skb_t->cb[_SKB_CB_PRIORITY];
	...
	if (txcfg->fr_type == _SKB_FRAME_TYPE_)
	{
		...
		/* do skb copy for cloned packets if need to do sw encryption*/
		if(txcfg->privacy )  {
#ifdef CONFIG_IEEE80211W
			if(UseSwCrypto(priv, txcfg->pstat, (txcfg->pstat ? FALSE : TRUE), txcfg->isPMF))
#else
		...
		if (txcfg->aggre_en >= FG_AGGRE_MSDU_FIRST) {
			...
			if(skb_cloned(pskb) ||
				(((ntohs(pethhdr->type) + WLAN_ETHHDR_LEN) > WLAN_MAX_ETHFRM_LEN) &&
				(skb_headroom(pskb) < sizeof(struct llc_snap)||skb_tailroom(pskb)<4))
			)
			{
		...
		if (txcfg->privacy == _WAPI_SMS4_)
		{
			if (((pskb->tail + SMS4_MIC_LEN) > pskb->end) || skb_cloned(pskb))
			{
				struct sk_buff *pnewskb;
        ...
        if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
			...
			if (IS_HAL_CHIP(priv)) {
				...
				if((!priv->pmib->dot11OperationEntry.disable_txsc) &&
					...
					(!(
#if (defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
					((pethhdr->type == __constant_htons(ETH_P_8021Q)) && (*((UINT16 *)((UINT8 *)pskb->data + 2)) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))) ||
#endif
...
{
	if (priv->GBWC_timer_alive && 
		(((priv->pmib->gbwcEntry.GBWCMode == GBWC_MODE_LIMIT_MAC_INNER) && (pstat->GBWC_in_group)) ||
...
/*static*/ int rtl8192cd_tx_sta_bwc(struct rtl8192cd_priv *priv, struct stat_info	*pstat, struct sk_buff *skb)
{
	...
	if (tx_count > pstat->sta_bwc_tx_limit_byte) 
	{
		// over the bandwidth
		// drop packet
		priv->ext_stats.tx_drops++;
	...
	{
		// can transmit directly
		pstat->sta_bwc_tx_cnt += skb->len;
	}
	/* Reply caller function : Continue process */
	return ret_val;
#else /* !defined(RTK_STA_BWC_DIRECT_XMIT) */
	SMP_TRY_LOCK_STA_BWC(flags, locked);
	...
	if (tx_count > pstat->sta_bwc_tx_limit_byte)
	{
		...
		{
			// in rtl8192cd_sta_bwc_timer context
			priv->ext_stats.tx_drops++;
			rtl_kfree_skb(priv, skb, _SKB_TX_);
		}
		else
		{
			ret = enque(priv, &(pstat->sta_bwc_txq.head), &(pstat->sta_bwc_txq.tail),
						(unsigned long)(pstat->sta_bwc_txq.pSkb), NUM_TXPKT_QUEUE, (void *)skb);
			if (ret == FALSE) 
			{
				priv->ext_stats.tx_drops++;
				rtl_kfree_skb(priv, skb, _SKB_TX_);
	...
	{
		// not over the bandwidth
		if (CIRC_CNT(pstat->sta_bwc_txq.head, pstat->sta_bwc_txq.tail, NUM_TXPKT_QUEUE) && !pstat->sta_bwc_consuming_q) 
		{
			// there are already packets in queue, put in queue too for order
			ret = enque(priv, &(pstat->sta_bwc_txq.head), &(pstat->sta_bwc_txq.tail),
						(unsigned long)(pstat->sta_bwc_txq.pSkb), NUM_TXPKT_QUEUE, (void *)skb);
	...
	if (locked)
		SMP_UNLOCK_STA_BWC(flags);
#endif /* defined(RTK_STA_BWC_DIRECT_XMIT) */
...
#ifdef SBWC
int rtl8192cd_tx_sbwc(struct rtl8192cd_priv *priv, struct stat_info	*pstat, struct sk_buff *skb)
{
	...
	if (!(pstat->SBWC_mode & SBWC_MODE_LIMIT_STA_TX))
		return TX_PROCEDURE_CTRL_CONTINUE;
	...
	return TX_PROCEDURE_CTRL_CONTINUE;
rtl8192cd_tx_sbwc_free_skb:
	priv->ext_stats.tx_drops++;
	rtl_kfree_skb(priv, skb, _SKB_TX_);
rtl8192cd_tx_sbwc_queue_skb:
	return TX_PROCEDURE_CTRL_STOP;
}
#endif
#if (!defined(__OSK__)) || (defined(__OSK__) && !defined(CONFIG_RTL6028))
...
#if defined(CP3_WIFI_TX_XMIT_OUT) && defined(TAROKO_0)	
		  CP3_test_start();
#endif
	int result=0;
...
if (GET_HCI_TYPE(priv) == RTL_HCI_USB && txcfg->is_11s) {
	...
	if ((OPMODE & WIFI_AP_STATE)
...
#endif
		{
			...
			if (dz_queue(priv, pstat, skb) == TRUE)
			{
				...
				#if defined(TAROKO_0)
				pe_sta_sw_queue_state_set(pstat->cmn_info.aid, 1);
				#endif
				goto stop_proc;
			}
			else {
...
#ifdef RTK_STA_BWC
	if (pstat && priv->pshare->rf_ft_var.sta_bwc_dbg_lmt > 0) {
		// ( limit(kbps) * 1024 / 8 ) / (1000 / bwc_to)
		pstat->sta_bwc_tx_limit = priv->pshare->rf_ft_var.sta_bwc_dbg_lmt;
		sta_bwc_printk("tx_limit: %u\n", pstat->sta_bwc_tx_limit);
		pstat->sta_bwc_tx_limit_byte = (pstat->sta_bwc_tx_limit << 7) / (1000 / priv->pshare->rf_ft_var.sta_bwc_to);
		sta_bwc_printk("tx_limit_byte: %u\n", pstat->sta_bwc_tx_limit_byte);
		priv->pshare->rf_ft_var.sta_bwc_dbg_lmt = 0;
	}
	if (priv->pshare->rf_ft_var.sta_bwc_en && pstat && pstat->sta_bwc_tx_limit > 0) {
	...
	if (skb->cb[_SKB_CB_AMSDU_TXSC]) {
		...
		// also, from swq_dequeue_amsdu
		txcfg->q_num = skb->cb[_SKB_CB_AMSDU_TXSC] & 15;
		txcfg->aggre_en = (skb->cb[_SKB_CB_AMSDU_TXSC] >> 4) & 0xf;
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
		if (OFFLOAD_ENABLE(priv))
			txcfg->h2d_type = (txcfg->aggre_en == 0) ? TXPKTINFO_TYPE_8023 : TXPKTINFO_TYPE_AMSDU;
#endif
	} else 
#endif    
	if (IS_SUPPORT_TX_AMSDU(priv) 
			...
			)
	{
			...
			)
		{
			...
			if (skb_cloned(skb)
#if !defined(MERGE_HEADER_PAYLOAD)
				&& (
				(txsc_txcfg->privacy == _TKIP_PRIVACY_)
#ifdef CONFIG_RTL_WAPI_SUPPORT
				 || (txsc_txcfg->privacy == _WAPI_SMS4_)
#endif
#ifdef SUPPORT_TX_AMSDU_SHORTCUT
...
#ifdef SUPPORT_TX_AMSDU_SHORTCUT
				if(txcfg->aggre_en >= FG_AGGRE_MSDU_FIRST)
					needed_tailroom += 4; //4 bytes  needed for padding
#endif
				if(txsc_txcfg->privacy == _TKIP_PRIVACY_)
					needed_tailroom += 8; // 8 bytes needed for TKIP MIC
				if (skb_tailroom(skb)< needed_tailroom)
					newskb = skb_copy_expand(skb, skb_headroom(skb), needed_tailroom, GFP_ATOMIC);
				else
					newskb = skb_copy(skb, GFP_ATOMIC);
				if (newskb == NULL) {
					priv->ext_stats.tx_drops++;
...

			{
				...
				if(txcfg->aggre_en >= FG_AGGRE_MSDU_FIRST){
					...
					if (txcfg->privacy == _TKIP_PRIVACY_)
						needed_tailroom += 8;	// 8 bytes needed for TKIP MIC
					if (skb_tailroom(skb)< needed_tailroom || skb_cloned(skb)) {
...
just_skip:
	...
	result = rtl8192cd_tx_slowPath(priv, skb, pstat, dev, wdsDev, txcfg);
	if ( result== TX_PROCEDURE_CTRL_STOP) {
		goto stop_proc;
	}else if(result == TX_PROCEDURE_CTRL_SUCCESS){
#ifdef CONFIG_DIAG_WIFI
		/*AR018*/
		if(OPMODE & WIFI_AP_STATE){
			if(skb->cb[9]==0x22)
			{
				priv->pshare->arp_resp_cnt2++;
			}
		}
		/*AR018*/
#endif
...
free_and_stop:		/* Free current packet and stop TX process */
	priv->pshare->free_and_stop2++;
	rtl_kfree_skb(priv, skb, _SKB_TX_);

/* Stop process and assume the TX-ed packet is already "processed" (freed or TXed) in previous code. */
...
static void del_tcp_session(struct stat_info *pstat, struct tcp_session *tcp_ses)
{
	...
	if (tcp_ses->buf_ack) {
		dev_kfree_skb_any(tcp_ses->buf_ack);
	}
...
void check_tcp_ack_timeout(unsigned long _priv)
{
	struct list_head *phead, *plist;
	struct stat_info *pstat;
	struct tcp_session *tcp_ses;
	...
	if (IS_DRV_OPEN(priv))
		mw5_legacy_mod_timer(&priv->tcpack_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(1));
}
...
int tx_tcp_ack_accelerate(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *skb)
{
	...
	do {
		iph = (struct iphdr *)rtl_get_skb_ip_header(skb, 1);
...
static 
#endif
int isICMPv6Mng(unsigned char *data)
{
	if((data[12] == 0x81) && (data[13] == 0x00))
		data +=4;
	if((data[12] == 0x86) &&(data[13] == 0xdd)&&
		((data[20] == 0x3a) || (data[54] == 0x3a)) //next header is icmpv6
		//&& skb->data[54] == 0x86 //RA
...
__PE_IMEM__
#ifdef CONFIG_RTK_MESH
int isICMPv6Mng2(unsigned char *protocol,unsigned char *payload)
{
	if((protocol[0] == 0x86) &&(protocol[1] == 0xdd)&&
		((payload[20-14] == 0x3a) || (payload[54-14] == 0x3a)) //next header is icmpv6
		//&& skb->data[54] == 0x86 //RA
		/*128,129,133,.....255 SHOULD BE MANAGMENT PACKET
		 REF:http://en.wikipedia.org/wiki/ICMPv6 */
	)
...
__PE_IMEM__
int isMDNS2(unsigned char *da, unsigned char *protocol, unsigned char *payload)
{
	if ((da[3] == 0x00) && (da[4] == 0x00) && (da[5] == 0xfb)) {
		if(((protocol[0] == 0x08) && (protocol[1] == 0x00) // IPv4
		  && (payload[23-14] == 0x11) // UDP
		  //&& (data[30] == 0xe0) // 224.0.0.251
		  && (payload[36-14] == 0x14) && (payload[37-14] == 0xe9)) // port 5353
		 ||
		 ((protocol[0] == 0x86) && (protocol[1] == 0xdd) // IPv6
		  && (payload[20-14] == 0x11) // next header is UDP
		  && (payload[56-14] == 0x14) && (payload[57] == 0xe9)) // port 5353
		)		
		return 1;
	}
	return 0;
...
int mlcst2unicst(struct rtl8192cd_priv *priv, struct sk_buff *skb)
{
	...
	if(*((UINT16*)(skb_data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021Q))
		skb_data +=4;
	...
	if ((priv->pmib->hs2Entry.hs2_ielen != 0) && (priv->dgaf_disable == 0)) {
		if (isICMPv6Mng(skb_data) || IS_ICMPV6_PROTO(skb_data))
			return 0;
    }
#endif
	...
	if( isSpecialFloodMac(priv,skb) /*|| IS_MDNSV4_MAC(skb->data)||IS_MDNSV6_MAC(skb->data)*/||IS_IGMP_PROTO(skb_data) || isICMPv6Mng(skb_data) || IS_ICMPV6_PROTO(skb_data)|| isMDNS(skb->data))
	{
...
#ifdef RTK_TDMA_ATM
int rtl8192cd_swq_bdfull(struct rtl8192cd_priv *priv, unsigned int num,  unsigned char qnum)
#else
...
__inline__ static unsigned char rtl8192cd_swq_settimer(struct rtl8192cd_priv *priv, UINT32 timer_timeout)
{
    ...
    if(timer_timeout == 0) { // from deltimer
        if(priv->pshare->swq_current_timeout) {
            ...
            if(priv->pshare->swq_use_hw_timer) {  
                ...
                */
                ...
                if (mw5_legacy_timer_pending(&priv->pshare->swq_sw_timer))
        ...
        if(priv->pshare->swq_use_hw_timer) {  
            ...
            if(set_timer) {                                
                priv->pshare->swq_current_timeout = timer_timeout;            
                mw5_legacy_mod_timer(&priv->pshare->swq_sw_timer, (unsigned long)timer_timeout);            
            }            
        }
    }
    return 0;
...
__inline__ static int rtl8192cd_swq_dequeue_amsdu(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char q_num, unsigned int dqnum, unsigned char releaseALL, int max_size)
{
	...
	if (releaseALL || qlen < dqnum)
		dqnum = qlen;

#ifdef SUPPORT_TX_SWQ_ATM
	if (priv->pshare->swq_atm_state && pstat->swq_atm_txrate != INT_MAX && pstat != priv->pshare->swq_atm_pmax_stat) {		
		if ((pstat->swq_atm_deqnum * (priv->pshare->swq_atm_pmax_stat->swq_atm_txrate / pstat->swq_atm_txrate)) >= priv->pshare->swq_atm_pmax_stat->swq_atm_deqnum) {
			goto skip;
		} else if (dqnum > pstat->swq_atm_max_deqnum) {
	...
	while (count < dqnum)
	{
		...
		if (is_first && priv->pshare->pwlanllc_hdr_poll->count < 10)	// tmp, check llc hdr count
			break;
		...
		if ((standalone == 1) || (is_first && !next_skb) || (avail_txbd_num <= 0)) {
			...
			if (next_skb) {
				...
				if (OFFLOAD_ENABLE(priv))
				{
					if (IS_MCAST(next_skb->data) || (!IS_MCAST(next_skb->data) && (next_skb->cb[16]==TX_NO_MUL2UNI)))
						standalone = 1;
					else
						standalone = 0;
				}
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
			if (OFFLOAD_ENABLE(priv) &&
				rtl8192cd_xmit_offload(pstat, pskb, txcfg)==SUCCESS)
				return 0;
#endif
...
#ifdef SUPPORT_TX_SWQ_ATM
	if (priv->pshare->swq_atm_state)
		pstat->swq_atm_deqnum += count;
#endif
...
__inline__ static int rtl8192cd_swq_dequeue(struct rtl8192cd_priv *priv, struct stat_info *pstat,  unsigned char qnum, unsigned int dqnum, unsigned char releaseALL)
{
...
#ifdef SUPPORT_TX_SWQ_AMSDU
	if (priv->pshare->rf_ft_var.tcpack_agg && AMSDU_SWQ_ENABLE) 
	{
	...
	if (pstat->AMSDU_AMPDU_support && !(pstat->state & WIFI_SLEEP_STATE)
#ifdef MULTI_MAC_CLONE
		&& !((OPMODE & WIFI_STATION_STATE) && IS_VXD_INTERFACE(priv) && priv->pmib->ethBrExtInfo.macclone_enable)
#endif
         )
	{
		if (pstat->aggre_mthd & AGGRE_MTHD_MSDU)
			return rtl8192cd_swq_dequeue_amsdu(priv, pstat, qnum, dqnum, releaseALL, pstat->amsdu_level);
		else if (priv->pshare->rf_ft_var.amsdusmallpkts && pstat->sml_big_pkt_diff > 0 && pstat->AMSDU_AMPDU_support)
	...
	while (1)
	{
		...
		if(rtl8192cd_swq_bdfull(priv, 5, qnum))
			break;
...
#define for_each_mu_partner(pstat, psta_mu) \
	...
	if(priv->pshare->swq_use_hw_timer) {
		...
		if(pstat->muPartner_num){
			if(pstat->swq.swq_en[q_num]) {
				...
				for_each_mu_partner(pstat, psta_mu) {
					if (psta_mu && psta_mu->tx_avarage < (MBPS_TO_BIT(priv->pshare->rf_ft_var.mutp_th_lower))) {
						cond_flag = 1;
						break;
					}
				}
				if(pstat->tx_avarage < (MBPS_TO_BIT(priv->pshare->rf_ft_var.mutp_th_lower)) || cond_flag) 
				{
					...
					for_each_mu_partner(pstat, psta_mu) {
						if(psta_mu && psta_mu->swq.swq_empty[q_num]) {
				...
				for_each_mu_partner(pstat, psta_mu) {
					if (psta_mu && !(psta_mu->tx_avarage > (MBPS_TO_BIT(priv->pshare->rf_ft_var.mutp_th_up))) ) {
						cond_flag = 0;
						break;
					}
				}
		...
		{
			if(!pstat->swq.swq_en[q_num] 
#if defined(SUPPORT_TX_SWQ_AMSDU)
				&& AMSDU_SWQ_ENABLE
#endif
			) {
				pstat->swq.swq_en[q_num] = 1; /*enable swq*/
...
#else
		{
...
#ifdef HUAWEI_PORT_UNAUTH_ENQUE
			if (IEEE8021X_FUN && pstat->ieee8021x_ctrlport == DOT11_PortStatus_Unauthorized) {
				if (!pstat->swq.swq_en[q_num]) {
			...
			if ((MBPS_TO_BIT(10) < pstat->rx_avarage) && ((pstat->tx_avarage * 5) < pstat->rx_avarage))
		    {
				...
				if (priv->pshare->rf_ft_var.swq_enable == 3 || assoc_num > assoc_num_low) {	// start enqueue
					if (!pstat->swq.swq_en[q_num]) {
						pstat->swq.swq_en[q_num] = 1;
						pstat->swq.swq_prev_timeout[q_num] = 1;
						pstat->swq.swq_timeout_change[q_num] = 0;
					...
					if (pstat->swq.swq_en[q_num]) {		// stop enqueue
	...
	if(change != 1) {
		...
		if (pBeamInfo->beamformee_mu_cnt >= 2 && pstat->muPartner_num) {
			...
			if (qlen <= priv->pshare->rf_ft_var.dqnum/*pstat->mu_deq_num*/) {
				need_fire = 0;
			}
			else {
				for_each_mu_partner(pstat, psta_mu) {
		...
		if(need_deque_mu) {
...
#endif
#ifdef CONFIG_WLAN_HAL_8814BE
			struct stat_info *psta[1 + MAX_NUM_MU_PARTNER] = {0};
			unsigned int psta_num = 1;
			unsigned int j;
            unsigned int deq_num_min = pstat->mu_deq_num;
            unsigned int deq_num_all = 0;
            unsigned int deq_num[1 + MAX_NUM_MU_PARTNER] = {0};
            unsigned int count[1 + MAX_NUM_MU_PARTNER] = {0};
            unsigned int qlen, done = 0;
			...
			for_each_mu_partner(pstat, psta_mu) {
				if (psta_mu) {
                    ...
                    if (psta_mu->mu_deq_num < deq_num_min)
                        deq_num_min = psta_mu->mu_deq_num;
				}
			}
            for (i = 0; i < psta_num; i++) {
                if (psta[i]->mu_deq_num <= deq_num_min)
                    deq_num[i] = 1;
                else
                    deq_num[i] = (psta[i]->mu_deq_num / deq_num_min) + 1;

                deq_num_all += deq_num[i];
                count[i] = psta[i]->mu_deq_num;
            ...
            while (!done) 
            {
                if (rtl8192cd_swq_bdfull(priv, deq_num_all, q_num))
                    break;
                ...
                for (i = 0; i < psta_num; i++) {
                    if (count[i] >= deq_num[i]) {
                        if (rtl8192cd_swq_dequeue(priv, psta[i], q_num, deq_num[i], SWQ_DEQ_LIMIT) == deq_num[i]) {
                            count[i] -= deq_num[i];
                            if (count[i] == 0) {
                                qlen = skb_queue_len(&psta[i]->swq.swq_queue[q_num]);
                                if (qlen > psta[i]->mu_deq_num)
                                    count[i] = psta[i]->mu_deq_num;
                                else
                                    done = 1;
                            }
                        ...
                        if (qlen > psta[i]->mu_deq_num) {
                            if (rtl8192cd_swq_dequeue(priv, psta[i], q_num, deq_num[i], SWQ_DEQ_LIMIT) == deq_num[i])
                                count[i] = psta[i]->mu_deq_num - (deq_num[i] - count[i]);
                            else
                                done = 1;
                        }
                        else {
                            rtl8192cd_swq_dequeue(priv, psta[i], q_num, count[i], SWQ_DEQ_LIMIT);
                            count[i] = 0;
                            done = 1;
                        }
                    }
                ...
                if (done) {
                    ...
                    for (i = 0; i < psta_num; i++) {
                        while (count[i] > 0) {
                            if (rtl8192cd_swq_bdfull(priv, 1, q_num)) {
                                bdfull = 1;
                                break;
                            }
                            rtl8192cd_swq_dequeue(priv, psta[i], q_num, 1, SWQ_DEQ_LIMIT);

                            count[i] --;
                            //tobe del
                            psta[i]->test_mu_su_cnt ++;
                        }
...
#endif
	        if (!rtl8192cd_swq_bdfull(priv, (pstat->swq.q_aggnum[q_num] + 1) * 2, q_num)) {
				if(priv->pshare->rf_ft_var.swq_max_xmit>0)
					rtl8192cd_swq_dequeue(priv, pstat, q_num, priv->pshare->rf_ft_var.swq_max_xmit, SWQ_DEQ_LIMIT);					
				else
					rtl8192cd_swq_dequeue(priv, pstat, q_num, skb_queue_len(&pstat->swq.swq_queue[q_num]), SWQ_DEQ_LIMIT);					
			priv->cnt_not_mu++;
	...
	if(pstat->muPartner_num) {
		...
		for_each_mu_partner(pstat, psta_mu) {
			if(psta_mu && psta_mu->swq.swq_empty[q_num]) { /*queue not empty, add timer for it*/        
...
#ifdef HUAWEI_PORT_UNAUTH_ENQUE
			if (need_dequeue && IEEE8021X_FUN && pstat->ieee8021x_ctrlport == DOT11_PortStatus_Unauthorized) {
			// do not check timeout dequeue
				need_dequeue = 0;
...
#if (MU_BEAMFORMING_SUPPORT == 1) && defined(CONFIG_WLAN_HAL_8814BE)
						if ((GET_CHIP_VER(priv) == VERSION_8814B) && (pstat->muPartner_num)) {
							struct stat_info *_psta_mu = (pstat ? pstat->muPartner : NULL);

							struct stat_info *psta[1 + MAX_NUM_MU_PARTNER] = {0};
							unsigned char min_mu_deq_num = priv_this->pshare->rf_ft_var.dqnum;
							unsigned int psta_num = 1;
							unsigned int j;

							if (pstat) {
								if (pstat->mu_deq_num < min_mu_deq_num)
									min_mu_deq_num = pstat->mu_deq_num;
							}	
							psta[0] = pstat;
							for_each_mu_partner(pstat, _psta_mu) {
								if (_psta_mu) {
									psta[psta_num++] = _psta_mu;

									if (_psta_mu->mu_deq_num < min_mu_deq_num)
										min_mu_deq_num = _psta_mu->mu_deq_num;
								}
							}
							for (j=min_mu_deq_num; j>0; j--) {
								if (!rtl8192cd_swq_bdfull(priv_this, psta_num+5, qnum)) {
									for (i = 0; i < psta_num; i++) {
										if (psta[i]->swq.swq_timer_id[qnum])
											rtl8192cd_swq_deltimer(priv_this, psta[i], qnum);
										if (psta[i]->swq.swq_empty[qnum])
											rtl8192cd_swq_dequeue(priv_this, psta[i], qnum, 1/*psta[i]->mu_deq_num*/, SWQ_DEQ_LIMIT);
									}
									priv_this->cnt_sta1_sta2++;
								}
								else
									break;
							}
							priv_this->ext_stats.swq_xmit_out_pkt++;
						}
						else
#endif
#if (MU_BEAMFORMING_SUPPORT == 1) && defined(CONFIG_WLAN_HAL_8812FE)
						if ((GET_CHIP_VER(priv) == VERSION_8812F) && (pstat->muPartner_num)) {
							struct stat_info *_psta_mu = (pstat ? pstat->muPartner : NULL);
...
__inline__ static int rtl8192cd_atm_swq_enqueue(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat)
{
	...
	//init timer
	{
		if(!priv->pshare->atm_timer_init){
			rtl8192cd_atm_swq_inittimer((unsigned long)priv);
	...
	if (priv->proxy_arp) {
		...
		if (IS_MCAST(skb->data)) {
			if (protocol == __constant_htons(ETH_P_8021Q))	{
				protocol = *((unsigned short *)(skb->data + 2 * ETH_ALEN +4));
			}		
			//proxy arp	
...
#endif
#ifdef CONFIG_PE_ENABLE //when PE enable, don't enter multi2uni path
	if(!priv->pshare->rf_ft_var.manual_pe_enable)
#endif
	{
...
//IGMP snooping
...
#if defined(TAROKO_0) 
#if defined(PE_RX_BCMC_PKT_RECV)
	//Copy Broadcast/Multicast frame to A17 through IPC
	if(IS_MCAST(skb->data))
	{
		//printk("BS vap = %d, %d, \r\n", priv->vap_id, __LINE__);
		taroko_t2h_ipc_pkt_send(priv, skb, (unsigned int)skb->data, skb->len);
		goto stop_proc;
	}
#else
	if(IS_MCAST(skb->data))
	...
	{	
		if(protocol == ETH_P_8021Q){
			...
			wlan_vlan_index = priv->dev->vlan_member_map;
			if(wlan_vlan_index>=RTK_WLAN0_ROOT_MASK && wlan_vlan_index<=RTK_WLAN2_CLIENT_MASK){
				if (!(rtk_vlan_ctl_p->group[skb_vid].tagMemberMask & (1 << wlan_vlan_index))){
					remove_vlan_tag((struct sk_buff **)(&skb));
				}
			}
	...
	*/
	...
	if (GET_MAP_PROFILE(priv) >= MAP_PROFILE_TWO && IS_VXD_INTERFACE(priv)) {
		if (priv->pmib->multi_ap.multiap_bss_type & BIT(7) && MAP_VLAN_ID) {
			unsigned short vid = MAP_VLAN_ID;
			if(*((u16*)(skb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_1905)){
	...
	if (pstat == NULL)
		pstat = get_stainfo(priv, skb->data);
	if ((GET_MAP_PROFILE(priv) == MAP_PROFILE_TWO) && (pstat && pstat->multiap_profile == MAP_PROFILE_TWO)) {
		if (((priv->pmib->multi_ap.multiap_bss_type & BIT(7)) || (priv->pmib->multi_ap.multiap_bss_type & BIT(6))) && MAP_VLAN_ID ) {
			unsigned short vid = MAP_VLAN_ID;
			if((*((u16*)(skb->data+(ETH_ALEN<<1))) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE)) && ((ntohs(*((u16*)(skb->data+(ETH_ALEN<<1)+2))) & 0x00ff) == 0x03)){
				if (skb_cow_head(skb, VLAN_HLEN) < 0) {
					printk("%s: %s, dev_alloc_skb() failed!\n", priv->dev->name, __FUNCTION__);
					goto free_and_stop;
				}
				...
				*((UINT16*)(skb->data+(ETH_ALEN<<1))) = __constant_htons(ETH_P_8021Q);
				*((UINT16*)(skb->data+(ETH_ALEN<<1)+2)) = __constant_htons(vid);
				printk("%s: %s %d, Inserting vlan tag %d\n", priv->dev->name, __FUNCTION__, __LINE__, vid);
			}
		}
	}
#endif
...
check_swq_enq:
	...
	//if(IP_MCAST_MAC(skb->data))
	//	panic_printk("MCAST_PKT(%02x%02x%02x%02x%02x%02x) da=%02x%02x%02x%02x%02x%02x\n", 
    ...
    #if defined(RTK_ATM) || defined(SW_TX_QUEUE)|| defined(RTK_TDMA_ATM)
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
    	if (pstat)
        {
            #ifdef RTK_TDMA_ATM
	    if(priv->pshare->tdma_swq_en)
	    {
        	swq_out = priv->pshare->tdma_swq_en | pstat->tdma_swq.swq_empty[BK_QUEUE] | 
         			      pstat->tdma_swq.swq_empty[BE_QUEUE] | 
				      pstat->tdma_swq.swq_empty[VO_QUEUE] | 
				      pstat->tdma_swq.swq_empty[VI_QUEUE];
	    }
            else
            #endif
...
#ifdef HUAWEI_PORT_UNAUTH_ENQUE
			unsigned short protocol = *((UINT16 *)((UINT8 *)skb->data + ETH_ALEN*2));
			if(protocol == __constant_htons(ETH_P_8021Q))
				protocol =  *((UINT16 *)((UINT8 *)skb->data + ETH_ALEN*2+4));
			if (IEEE8021X_FUN &&  pstat->ieee8021x_ctrlport == DOT11_PortStatus_Unauthorized && ( protocol != htons(0x888e))) {
				// forced enqueue
				swq_out = 1;
			} else
#endif
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		if (swq_out == 0
			...
			|| (pstat && pstat->sta_bwc_consuming_q)
			#endif
#ifdef SUPPORT_TX_AMSDU_SMALL_PKTS_ONLY
			|| (priv->pmib->dot11nConfigEntry.dot11nAMSDUSmallPkts &&
			   (skb->len <= priv->pmib->dot11nConfigEntry.dot11nAMSDUSmallPktLen || ((skb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_TCPACK) && pstat->amsdu_tcpack_en)))
			...
			if(priv->pshare->tdma_swq_en){
				if (*((UINT16 *)((UINT8 *)skb->data + MACADDRLEN * 2)) == __constant_htons(0x888e)) {
					//printk("send DOT11_EVENT_EAP_PACKET!\n");
					goto xmit_out;
				}
				else if(isDHCPpkt(skb)){
					//printk("send DHCP_PACKET!\n");
					goto xmit_out;
				}
				else if(pstat->wakeup_tx)
					goto xmit_out;
				return rtl8192cd_tdma_swq_enqueue(priv, skb, pstat);
			}
			else {
				for(temp = BK_QUEUE; temp <= VO_QUEUE; temp++)
				{
					if(pstat->tdma_swq.swq_empty[temp]) 											
						rtl8192cd_tdma_skb_dequeue(priv, pstat, temp , 0, SWQ_DEQ_ALL);//send out all packet in atm_swq
				}
				goto xmit_out;
			}
		}
...
#ifdef HUAWEI_PORT_UNAUTH_ENQUE
			|| (IEEE8021X_FUN && pstat && pstat->ieee8021x_ctrlport == DOT11_PortStatus_Unauthorized && (*((UINT16 *)((UINT8 *)skb->data + ETH_ALEN*2)) != htons(0x888e)))
#endif
			) {//swq_en=1
#if 1//ndef SMP_LOAD_BALANCE_SUPPORT
			if( pstat && priv->pshare->rf_ft_var.swq_max_len ) {
				if(skb_queue_len(&pstat->swq.swq_queue[(unsigned int)skb->cb[_SKB_CB_QNUM]]) > priv->pshare->rf_ft_var.swq_max_len) {
					priv->ext_stats.tx_drops++;
					goto free_and_stop;
				}
			}
...
#if defined(CP3_WIFI_TX_XMIT_PCI) && defined(TAROKO_0)	
		CP3_test_stop(WIFI_TX, 1);
#endif
...
int rtl8192cd_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,140) || LINUX_VERSION_CODE >= KERNEL_VERSION(4,16,0))
	/*Allow wifi driver to adjust default default skb_pacing_shift.
	 *Kernel 4.4 TCP allows roughly 1ms worth of queue. If drivers hold skb for too long,
	 *local TCP TP will be severly impacted.*/
	...
	if (
#if (defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
		((protocol == __constant_htons(ETH_P_8021Q)) && (*((u16*)(skb->data+ETH_ALEN*2+4)) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))) ||
#endif
		(protocol == __constant_htons(0x888e)))
	{
		FTDEBUG("Tx EAPOL pkt\n");
	...
	if(IS_VXD_INTERFACE(priv) && IS_DRV_OPEN(priv))
	{
		...
		if(1){
				...
				&& !GET_ROOT(priv)->pmib->crossBand.crossband_prefer) 
			{
				...
				{
					dev_kfree_skb_any(skb);
					DEBUG_ERR("TX DROP: Drop broadcast packet on non preferband!\n");
					return 0; 
				}
				else
				{
#ifdef CONFIG_CROSSBAND_REPEATER_SUPPORT_WIFI5_WIFI6
					if(crossband_check_prefer(CROSSBAND_2G))
					{
						if(crossband_dev_xmit(skb, CROSSBAND_2G) == 0)
						{
	...
	if (!(priv->drv_state & DRV_STATE_OPEN)){ 
		...
		/*AR018*/
		if(OPMODE & WIFI_AP_STATE){
			if(*(unsigned short *)(skb->data + MACADDRLEN*2) == __constant_htons(ETH_P_ARP))
			{
				struct arphdr *arp;
				arp = (struct arphdr *)(skb->data + ETH_HLEN);
				if (arp->ar_pro == __constant_htons(ETH_P_IP)) {
					if(arp->ar_op == __constant_htons(ARPOP_REPLY)	
						|| arp->ar_op == __constant_htons(ARPOP_RREPLY)){
						priv->pshare->arp_resp_cnt1++;
						skb->cb[9]=0x22;
					}
				}
			}
...
skip_check_eap:
	...
	if (skb_qlen >= priv->pshare->rf_ft_var.txdaq_max_enqueue_len) {
#if 0
#ifdef _CORTINA_
...
#endif /*_CORTINA_*/
#endif
		priv->ext_stats.txdaq_overflow_drop_pkt++;
		priv->ext_stats.tx_drops++;
		dev_kfree_skb_any(skb);
		return 0; 
	}
...
#ifdef TX_PROCESS_REFINE
	if (rtk_txdata_queue_enqueue(priv, &priv->pshare->skb_xmit_queue[index], skb) == 1){
		priv->ext_stats.txdaq_overflow_drop_pkt++;
		priv->ext_stats.tx_drops++;
		dev_kfree_skb_any(skb);
	} else {
