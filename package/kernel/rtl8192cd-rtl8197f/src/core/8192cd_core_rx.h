	...
	if (pfrinfo->physt) {
...

			{
				if (pfrinfo->rf_info.mimorssi[0] != 0 || pfrinfo->rf_info.mimorssi[1] != 0) {
					...
					memcpy(&pstat->rf_info.mimosq[0], &pfrinfo->rf_info.mimosq[0], sizeof(pfrinfo->rf_info.mimosq));
#ifdef USE_OUT_SRC
					if (IS_OUTSRC_CHIP(priv)) {
						for (i = 0; i < 4; i++) {
...
#else
#if defined(TAROKO_0)
    phw->rx_infoL[i].paddr  = VIRT_TO_PHYS_DRAM(pskb->data);
#else
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    /* v44.38: preserve the two DMA contracts from the OEM design instead of
     * forcing both radios through one streaming-mapping path.
     *
     * RTL8197FS / TYPE_EMBEDDED: RXBD contains the direct SDK bus address.
     * Explicit cache ownership sync replaces the old BSP cache primitives.
     * RTL8812BRH / TYPE_PCI_DIRECT: keep Linux streaming DMA mapping. */
    {
        ...
        if (mw5_type == TYPE_EMBEDDED) {
            phw->rx_infoL[i].paddr = get_physical_addr(priv, pskb->data,
                mw5_len, PCI_DMA_FROMDEVICE);
            if (phw->rx_infoL[i].paddr) {
                rtl8197f_wlan_dma_sync_rx_for_device(priv,
                    phw->rx_infoL[i].paddr, mw5_len);
                priv->pshare->linux6_rx_direct_bus++;
            } else {
                priv->pshare->linux6_rx_dma_map_errors++;
                pr_err_ratelimited("rtl8197f-wlan: v44.38 embedded RX direct address failed if=%s idx=%u\n",
                    priv->dev ? priv->dev->name : "none", (unsigned int)i);
            }
        } else if (priv->pshare->linux_dev) {
            dma_addr_t mw5_dma = dma_map_single(priv->pshare->linux_dev,
                pskb->data, mw5_len, DMA_FROM_DEVICE);
            if (dma_mapping_error(priv->pshare->linux_dev, mw5_dma)) {
                phw->rx_infoL[i].paddr = 0;
                priv->pshare->linux6_rx_dma_map_errors++;
                pr_err_ratelimited("rtl8197f-wlan: v44.38 PCI RX DMA map failed if=%s idx=%u\n",
                    priv->dev ? priv->dev->name : "none", (unsigned int)i);
            } else {
                phw->rx_infoL[i].paddr = (unsigned long)mw5_dma;
                priv->pshare->linux6_rx_dma_maps++;
            }
        } else {
            phw->rx_infoL[i].paddr = 0;
            priv->pshare->linux6_rx_dma_map_errors++;
