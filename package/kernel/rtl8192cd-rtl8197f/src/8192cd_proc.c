...
/*get git HEAD info & tag info*/
#include "git_info.h"

/*re-execute run_it_before_release.sh before release*/
#include "git_info2.h"
...
/* Linux 5.6 replaced procfs file_operations with struct proc_ops.
 * Passing the old layout to proc_create_data() can compile with a warning but
 * makes Linux 6.6 dispatch the wrong callback at runtime.
 */
#define RTK_DECLARE_READ_PROC_FOPS(read_proc) \
	static int read_proc##_open(struct inode *inode, struct file *file) \
	{ \
		return single_open(file, read_proc, PDE_DATA(inode)); \
	} \
	static const struct proc_ops read_proc##_fops = { \
...
#define RTK_DECLARE_WRITE_PROC_FOPS(write_proc) \
	...
	static const struct proc_ops write_proc##_fops = { \
...
#endif
{
	...
	if (priv->pmib->dot11StationConfigEntry.dot11DesiredSSIDLen < sizeof(tmpbuf) &&
		priv->pmib->dot11StationConfigEntry.dot11DesiredSSIDLen < sizeof(priv->pmib->dot11StationConfigEntry.dot11DesiredSSID)) {
	...
	if (priv->pmib->dot11StationConfigEntry.dot11DefaultSSIDLen < sizeof(tmpbuf) &&
		priv->pmib->dot11StationConfigEntry.dot11DefaultSSIDLen < sizeof(priv->pmib->dot11StationConfigEntry.dot11DefaultSSID)) {
	...
	if (priv->pmib->dot11StationConfigEntry.dot11SSIDtoScanLen < sizeof(tmpbuf) &&
		priv->pmib->dot11StationConfigEntry.dot11SSIDtoScanLen < sizeof(priv->pmib->dot11StationConfigEntry.dot11SSIDtoScan)) {
	...
	for (i=0; i<priv->pmib->dot11StationConfigEntry.dot11AclNum; i++) {
		...
		if (plist != phead) {
			...
			do {
				...
				PRINT_ONE((paclnode->mode== ACL_allow)?"allow":((paclnode->mode== ACL_deny)?"deny":"invalid"), "(%s)", 0);
...
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_git_info(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_git_info(char *buf, char **start, off_t offset,
		int length, int *eof, void *data)
#endif
{
	struct net_device *dev = PROC_GET_DEV();
	...
	PRINT_ONE("git_info.h:", "%s", 1);
	PRINT_SINGL_ARG("MAIN_SHA1: ",MAIN_SHA1, "%s");
	PRINT_SINGL_ARG("MAIN_TAGINFO: ",MAIN_TAGINFO, "%s");
	PRINT_SINGL_ARG("CMN_INFO_SHA1: ",CMN_INFO_SHA1, "%s");
	PRINT_SINGL_ARG("CMN_INFO_TAGINFO: ",CMN_INFO_TAGINFO, "%s");
	PRINT_SINGL_ARG("PHYDM_SHA1: ",PHYDM_SHA1, "%s");
	PRINT_SINGL_ARG("PHYDM_TAGINFO: ",PHYDM_TAGINFO, "%s");
	PRINT_SINGL_ARG("PHYDM_HALRF_SHA1: ",PHYDM_HALRF_SHA1, "%s");
	PRINT_SINGL_ARG("PHYDM_HALRF_TAGINFO: ",PHYDM_HALRF_TAGINFO, "%s");

	PRINT_ONE("git_info2.h:", "%s", 1);
	PRINT_SINGL_ARG("RTK_MAIN_SHA1: ",RTK_MAIN_SHA1, "%s");
	PRINT_SINGL_ARG("RTK_MAIN_TAGINFO: ",RTK_MAIN_TAGINFO, "%s");
	PRINT_SINGL_ARG("RTK_CMN_INFO_SHA1: ",RTK_CMN_INFO_SHA1, "%s");
	PRINT_SINGL_ARG("RTK_CMN_INFO_TAGINFO: ",RTK_CMN_INFO_TAGINFO, "%s");
	PRINT_SINGL_ARG("RTK_PHYDM_SHA1: ",RTK_PHYDM_SHA1, "%s");
	PRINT_SINGL_ARG("RTK_PHYDM_TAGINFO: ",RTK_PHYDM_TAGINFO, "%s");
	PRINT_SINGL_ARG("RTK_PHYDM_HALRF_SHA1: ",RTK_PHYDM_HALRF_SHA1, "%s");
	PRINT_SINGL_ARG("RTK_PHYDM_HALRF_TAGINFO: ",RTK_PHYDM_HALRF_TAGINFO, "%s");
...
#endif
{
	...
	for(idx=NUM_PMKID_CACHE-1; idx>=0; idx--){
...
#endif
{
	...
	PRINT_ARRAY_ARG("	 dfgw_mac: ", priv->pmib->dot11OperationEntry.dfgw_mac, "%02x", MACADDRLEN);
...
#endif
{
	...
	if(priv->pmib->dot11RFEntry.txbf_mu) {
		...
		PRINT_SINGL_ARG("    isSTAGrp: ", priv->pshare->rf_ft_var.isSTAGrp, "%d");		
		PRINT_SINGL_ARG("    isSTAGrp_pre: ", priv->pshare->rf_ft_var.isSTAGrp_pre, "%d");		
#endif	
#endif
	}
	#endif
...
#ifdef DFS
struct mw5_legacy_timer *get_channel_timer(struct rtl8192cd_priv *priv, int channel)
{
	switch(channel) {
		case 52:
...
#endif
{
	...
	int pos = 0;
	int i;
	struct mw5_legacy_timer *ch_timer;
...
#endif
{
	...
	if(IS_OUTSRC_CHIP(priv)) {
		memset(tmpstr,0,32);
		noise_level=odm_inband_noise_monitor(ODMPTR,TRUE,0x1E,10);
...
#endif
#if defined(CONFIG_WLAN_HAL_8192FE) || defined(CONFIG_WLAN_HAL_8814BE)
	char chip_ver;
#endif	
...
//	if ((GET_CHIP_VER(priv)==VERSION_8192D) && (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G)) {
...
#endif
	{
		...
		if (GET_CHIP_VER(priv)==VERSION_8197G){
			PRINT_ARRAY_ARG("    pwrlevel_TSSI_CCK_A: ", priv->pmib->dot11RFEntry.pwrlevel_TSSICCK_A, "%02x", MAX_2G_CHANNEL_NUM);
			PRINT_ARRAY_ARG("    pwrlevel_TSSI_CCK_B: ", priv->pmib->dot11RFEntry.pwrlevel_TSSICCK_B, "%02x", MAX_2G_CHANNEL_NUM);
...
#if defined(TXPWR_LMT_8198F) || defined(TXPWR_LMT_8814B) || defined(TXPWR_LMT_8812F) || defined(TXPWR_LMT_8197G)
		if((GET_CHIP_VER(priv)== VERSION_8198F)||(GET_CHIP_VER(priv)== VERSION_8814B)||(GET_CHIP_VER(priv)== VERSION_8812F)||(GET_CHIP_VER(priv)== VERSION_8197G)){
...
#endif
{
	...
	if (GET_CHIP_VER(priv)==VERSION_8197G){
		...
		phy_set_rf_reg(priv, RF_PATH_A, 0x42, BIT(17), 0x1);
		delay_ms(10);
		current_thermal = phy_query_rf_reg(priv, RF_PATH_A, 0xf6, 0x7e000, 1) + phydm_get_multi_thermal_offset(ODMPTR, RF_PATH_A);
		PRINT_SINGL_ARG("    current thermal path A: ", current_thermal, "%u");
		phy_set_rf_reg(priv, RF_PATH_B, 0x42, BIT(17), 0x1);
		phy_set_rf_reg(priv, RF_PATH_B, 0x42, BIT(17), 0x0);
		phy_set_rf_reg(priv, RF_PATH_B, 0x42, BIT(17), 0x1);
		delay_ms(10);
		current_thermal = phy_query_rf_reg(priv, RF_PATH_B, 0xf6, 0x7e000, 1) + phydm_get_multi_thermal_offset(ODMPTR, RF_PATH_B);
		PRINT_SINGL_ARG("    current thermal path B: ", current_thermal, "%u");
	}else
#endif
...
if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
	...
	if (GET_CHIP_VER(priv) == VERSION_8188C) {
		if(IS_UMC_B_CUT_88C(priv))
			snprintf(tmpbuf, sizeof(tmpbuf), "RTL6195B");
		else if(IS_88RE(priv))
			snprintf(tmpbuf, sizeof(tmpbuf), "RTL8188R");
		else
			snprintf(tmpbuf, sizeof(tmpbuf), "RTL8188C");
	} else if (GET_CHIP_VER(priv) == VERSION_8192C){
		snprintf(tmpbuf, sizeof(tmpbuf), "RTL8192C");
	}

	if ((GET_CHIP_VER(priv)==VERSION_8192C) || (GET_CHIP_VER(priv)==VERSION_8188C)) {
		if (IS_TEST_CHIP(priv))
			strncat(tmpbuf, "t", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));
		else
...
#ifdef CONFIG_RTL_92D_SUPPORT
	if (GET_CHIP_VER(priv) == VERSION_8192D)
		snprintf(tmpbuf, sizeof(tmpbuf), "RTL8192D");
#endif
#ifdef CONFIG_RTL_88E_SUPPORT
	if (GET_CHIP_VER(priv) == VERSION_8188E) {
		snprintf(tmpbuf, sizeof(tmpbuf), "RTL8188E");
	...
	if (GET_CHIP_VER(priv) == VERSION_8723B) {
		snprintf(tmpbuf, sizeof(tmpbuf), "RTL8188E");	// for MP tool use
	}
#endif
	...
	if (GET_CHIP_VER(priv) == VERSION_8812E) {
        ...
        snprintf(tmpbuf, sizeof(tmpbuf), "RTL8812AR-VN");
#else
		snprintf(tmpbuf, sizeof(tmpbuf), "RTL8812");
#endif
		if (IS_TEST_CHIP(priv))
			strncat(tmpbuf, "t", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));
		else if (IS_C_CUT_8812(priv))
			strncat(tmpbuf, "c", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));
	}
#endif
#ifdef CONFIG_WLAN_HAL_8881A
	if (GET_CHIP_VER(priv) == VERSION_8881A) {
		snprintf(tmpbuf, sizeof(tmpbuf), "RTL8881A");
		...
		if (get_bonding_type_8881A() == BOND_8881AB)
			...
			strncat(tmpbuf, "M", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));
		else if (get_bonding_type_8881A() == BOND_8881AQ)
			strncat(tmpbuf, "Q", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));
    ...
    if (GET_CHIP_VER(priv) == VERSION_8192E) {
		...
		{
		    snprintf(tmpbuf, sizeof(tmpbuf), "RTL8192Et");
		} else {
			snprintf(tmpbuf, sizeof(tmpbuf), "RTL8192En");
		}
		...
		if(_GET_HAL_DATA(priv)->cutVersion == ODM_CUT_A)
			...
			strncat(tmpbuf, "D", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));
		else
			strncat(tmpbuf, "O", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));//others
    }
#endif
#if defined(CONFIG_WLAN_HAL_8814AE)
    if (GET_CHIP_VER(priv) == VERSION_8814A) {
        if (IS_TEST_CHIP_8814(priv)) {
            snprintf(tmpbuf, sizeof(tmpbuf), "RTL8814A-TC");
        } else {
            if (GET_CHIP_VER_8814(priv) == 0x0) {
                snprintf(tmpbuf, sizeof(tmpbuf), "RTL8814A-MP-A");
            } else {
                snprintf(tmpbuf, sizeof(tmpbuf), "RTL8814A-MP-B");
            }
        }
    }
#endif
        ...
        if (GET_CHIP_VER(priv) == VERSION_8822B) {
            if (_GET_HAL_DATA(priv)->bTestChip)
    		{
    		    snprintf(tmpbuf, sizeof(tmpbuf), "RTL8822B Test");
    		} else {
    			snprintf(tmpbuf, sizeof(tmpbuf), "RTL8822B MP");
    		}
        }
#endif
        ...
        if (GET_CHIP_VER(priv) == VERSION_8822C) {
            if (_GET_HAL_DATA(priv)->bTestChip)
    		{
    		    snprintf(tmpbuf, sizeof(tmpbuf), "RTL8822C Test");
    		} else {
    			snprintf(tmpbuf, sizeof(tmpbuf), "RTL8822C MP");
    		}
        }
#endif
        ...
        if (GET_CHIP_VER(priv) == VERSION_8812F) {
            if (_GET_HAL_DATA(priv)->bTestChip)
    		{
    		    snprintf(tmpbuf, sizeof(tmpbuf), "RTL8812F Test");
    		} else {
    			snprintf(tmpbuf, sizeof(tmpbuf), "RTL8812F MP");
    		}
        }
#endif
        ...
        if (GET_CHIP_VER(priv) == VERSION_8821C) {
            if (_GET_HAL_DATA(priv)->bTestChip)
    		{
    		    snprintf(tmpbuf, sizeof(tmpbuf), "RTL8821C Test");
    		} else {
    			snprintf(tmpbuf, sizeof(tmpbuf), "RTL8821C MP");
    		}
        }
#endif
    ...
    if (GET_CHIP_VER(priv) == VERSION_8197F) {
        if (_GET_HAL_DATA(priv)->bTestChip)
        {
            snprintf(tmpbuf, sizeof(tmpbuf), "RTL8197F Test");
        } else {
            snprintf(tmpbuf, sizeof(tmpbuf), "RTL8197F");
            ...
            if(rtl819x_bond_option() == 1)
                ...
                strncat(tmpbuf, "S", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));
            strncat(tmpbuf, " MP", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));
	...
	if (GET_CHIP_VER(priv) == VERSION_8192F) {
		if(_GET_HAL_DATA(priv)->cutVersion < ODM_CUT_TEST)
			...
			chip_ver = 'O';
		snprintf(tmpbuf, sizeof(tmpbuf), "RTL8192Fn%c", chip_ver);
	}
#endif
	...
	if (GET_CHIP_VER(priv) == VERSION_8814B) {
        if (IS_TEST_CHIP_8814(priv)) {
            snprintf(tmpbuf, sizeof(tmpbuf), "RTL8814B-TC");
        } else {
			if(_GET_HAL_DATA(priv)->cutVersion < ODM_CUT_TEST)
				chip_ver = 'A' + _GET_HAL_DATA(priv)->cutVersion;
			else
				chip_ver = 'X';
			snprintf(tmpbuf, sizeof(tmpbuf), "RTL8814B-MP-%c", chip_ver);
        }
	}
#endif
#if defined(CONFIG_WLAN_HAL_8197G)
            if (GET_CHIP_VER(priv) == VERSION_8197G) {
                if (_GET_HAL_DATA(priv)->bTestChip)
                {
                    sprintf(tmpbuf, "RTL8197G");
                } else {
                    sprintf(tmpbuf, "RTL8197G Test");
                }
            }
#endif //#if defined(CONFIG_WLAN_HAL_8197G)
	if(IS_UMC_A_CUT(priv))
		strncat(tmpbuf, "u", (sizeof(tmpbuf) - strlen(tmpbuf) - 1));

	PRINT_SINGL_ARG("    chipVersion: ", tmpbuf, "%s");

#if defined(CONFIG_WLAN_HAL_8198F)
        if (GET_CHIP_VER(priv) == VERSION_8198F) {
			if(ODMPTR->cut_version == ODM_CUT_A)
				sprintf(tmpbuf, "CUT_A");
			else if(ODMPTR->cut_version == ODM_CUT_B)
				sprintf(tmpbuf, "CUT_B");
			else if(ODMPTR->cut_version == ODM_CUT_C)
				sprintf(tmpbuf, "CUT_C");
			else if(ODMPTR->cut_version == ODM_CUT_D)
				sprintf(tmpbuf, "CUT_D");
			else
				sprintf(tmpbuf, "CUT_O");
        }
	PRINT_SINGL_ARG("    cutVersion: ", tmpbuf, "%s");
#endif //#if defined(CONFIG_WLAN_HAL_8198F)
...
#endif
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8821CE) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8812FE)|| defined(CONFIG_WLAN_HAL_8197G)
...
#if defined(THERMAL_CONTROL) && !defined(THERMAL_CONTROL_DAEMON)
#ifdef CONFIG_RTL_PROC_NEW
...
#endif
{
	...
	PRINT_SINGL_ARG("	 ther_max : ", priv->pshare->rf_ft_var.ther_max, "%u");	
...
#ifdef CONFIG_RTL_PROC_NEW
int rtl8192cd_proc_mib_veriwave(struct seq_file *s, void *data)
#else	
static int rtl8192cd_proc_mib_veriwave(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif	
{
	struct net_device *dev = PROC_GET_DEV();
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	unsigned char curr_dig;
	int pos = 0;
	...
	PRINT_ONE("    veriwave_noisy_mode: ", "%s", 0);
	PRINT_ONE(priv->pshare->veriwave_noisy_mode, "%u, ", 0);
	PRINT_ONE(curr_dig, "curr_dig: %#02x", 1);
	...
	PRINT_SINGL_ARG("    RTS_protect_all: ", (RTL_R8(0x593) & BIT1) != 0, "%d");
	PRINT_SINGL_ARG("    Retry_limit: ", RTL_R16(REG_RETRY_LIMIT), "%#06x");

#ifdef SW_TX_QUEUE
	PRINT_ONE("    TP( ", "%s", 0);
	PRINT_ONE((priv->ext_stats.tx_avarage >> 17), "Tx: %lu, ", 0);
	PRINT_ONE((priv->ext_stats.rx_avarage >> 17), "Rx: %lu )", 0);
	PRINT_ONE("  Tx_pkt( ", "%s", 0);
	PRINT_ONE(priv->ext_stats.tx_drops, "drop: %lu, ", 0);
	PRINT_ONE(priv->net_stats.tx_errors, "fail: %lu )", 0);
	PRINT_ONE("  SWQ( ", "%s", 0);
	PRINT_ONE(priv->ext_stats.swq_enque_pkt, "en: %lu, ", 0);
	PRINT_ONE(priv->ext_stats.swq_xmit_out_pkt, "de: %lu, ", 0);
	PRINT_ONE(priv->ext_stats.swq_overflow_drop_pkt, "drop: %lu )", 1);
#endif

	PRINT_SINGL_ARG("    BE_EDCA: ", RTL_R32(0x508), "%#010x");
	PRINT_SINGL_ARG("      CWmin: ", ((RTL_R32(0x508) & 0xf00) >> 8), "%3d");
	PRINT_SINGL_ARG("      CWmax: ", ((RTL_R32(0x508) & 0xf000) >> 12), "%3d");
	PRINT_SINGL_ARG("      AIFS: ", (RTL_R32(0x508) & 0xff), "%3d");
	PRINT_SINGL_ARG("      TXOP: ", ((RTL_R32(0x508) & 0x7ff0000) >> 16), "%3d");

	PRINT_SINGL_ARG("    RRSR_DUP: ", (RTL_R32(0x440) & BIT22) != 0, "%d");
	PRINT_SINGL_ARG("    RTS_DUP: ", (RTL_R8(0x480) & BIT5) != 0, "%d");
	PRINT_SINGL_ARG("    Orig_DIG: ", ODMPTR->original_dig_restore, "%d");
	PRINT_SINGL_ARG("    Single_AMPDU: ", (RTL_R8(0x4c7) & BIT7) != 0, "%d");
	PRINT_SINGL_ARG("    Max_AMPDU_len: ", RTL_R32(REG_AMPDU_MAX_LENGTH), "%#010x");
	PRINT_SINGL_ARG("    Max_aggreation_num: ", RTL_R16(0x4ca), "%#06x");

	if (GET_CHIP_VER(priv) == VERSION_8197F) {
		PRINT_SINGL_ARG("    EDCCA_threshold: ", RTL_R32(0xc4c), "%#010x");
		PRINT_SINGL_ARG("      L2H: ", (RTL_R32(0xc4c) & 0xff), "%#04x");
		PRINT_SINGL_ARG("      H2L: ", ((RTL_R32(0xc4c) & 0xff0000) >> 16), "%#04x");

		if ((RTL_R8(0x998) & BIT6) == 1) {
			PRINT_ONE("    softML: ON", "%s", 1);
		} else {
			PRINT_ONE("    softML: OFF", "%s", 1);
		}
	} 
	else if (GET_CHIP_VER(priv) == VERSION_8822B) {
		PRINT_SINGL_ARG("    EDCCA_threshold: ", RTL_R32(0x8a4), "%#010x");
		PRINT_SINGL_ARG("      L2H: ", (RTL_R32(0x8a4) & 0xff), "%#04x");
		PRINT_SINGL_ARG("      H2L: ", ((RTL_R32(0x8a4) & 0xff00) >> 8), "%#04x");

		if (RTL_R32(0x19a8) == 0xd90a0000) {
			PRINT_ONE("    softML: ON", "%s", 1);
		} else if (RTL_R32(0x19a8) == 0x090a0000) {
			PRINT_ONE("    softML: OFF", "%s", 1);
		} else {
			PRINT_ONE("    softML: UNKNOWN", "%s", 1);
		}
	}
...
#ifdef BSS_MONITOR_SUPPORT
	PRINT_SINGL_ARG("    ap count: ", priv->pshare->ap_count, "%d");
#endif
	if (COEXIST_ENABLE) {
#ifdef CLIENT_MODE
		if (OPMODE & WIFI_STATION_STATE) {
...
#endif
{
	...
	PRINT_SINGL_ARG("    tx_packets_total: ", priv->pshare->tx_packets_total, "%lu");
	PRINT_SINGL_ARG("    tx_packets_pre: ", priv->pshare->tx_packets_pre, "%lu");
	PRINT_SINGL_ARG("    tx_packets_pre2: ", priv->pshare->tx_packets_pre2, "%lu");
	PRINT_SINGL_ARG("    rx_packets_total: ", priv->pshare->rx_packets_total, "%lu");
	PRINT_SINGL_ARG("    rx_packets_pre: ", priv->pshare->rx_packets_pre, "%lu");
	PRINT_SINGL_ARG("    rx_packets_pre2 ", priv->pshare->rx_packets_pre2, "%lu");
	...
	PRINT_SINGL_ARG("    client_host_sniffer_interval: ", priv->pmib->miscEntry.client_host_sniffer_interval, "%lu");
#endif
...
#endif
	{
		...
		CCA_total_cnt = ODMPTR->false_alm_cnt.cnt_cca_all;
#endif
		PRINT_SINGL_ARG("    False alarm: ", FA_total_cnt, "%d");
		PRINT_SINGL_ARG("    CCA: ", CCA_total_cnt, "%d");
	}
#endif
...
//20171205 ICV_DEBUG
	...
	PRINT_SINGL_ARG("	 dl_active_sta_num: ", priv->pshare->dl_active_sta_num, "%d");	
	PRINT_SINGL_ARG("	 ul_active_sta_num: ", priv->pshare->ul_active_sta_num, "%d");	
	PRINT_SINGL_ARG("    arp_req_cnt1: ",	priv->pshare->arp_req_cnt1, "%d");
	PRINT_SINGL_ARG("    arp_req_cnt2: ",	priv->pshare->arp_req_cnt2, "%d");
	PRINT_SINGL_ARG("    arp_resp_cnt1: ",	priv->pshare->arp_resp_cnt1, "%d");
	PRINT_SINGL_ARG("    arp_resp_cnt2: ",	priv->pshare->arp_resp_cnt2, "%d");
	PRINT_SINGL_ARG("    free_and_stop1: ",	priv->pshare->free_and_stop1, "%d");
	PRINT_SINGL_ARG("    free_and_stop2: ",	priv->pshare->free_and_stop2, "%d");
...
#endif
{
...
#ifdef __ECOS
    ecos_pr_fun("  Make info: v%d.%d.%d (%s)\n", DRV_VERSION_H, DRV_VERSION_L, DRV_VERSION_SUBL, DRV_RELDATE);
#elif defined(CONFIG_RTL_PROC_NEW)
    seq_printf(s, "  Make info: v%d.%d.%d (%s)\n", DRV_VERSION_H, DRV_VERSION_L, DRV_VERSION_SUBL, DRV_RELDATE);
#else
    size = snprintf(pTmp, length, "  Make info: v%d.%d.%d (%s)\n", DRV_VERSION_H, DRV_VERSION_L, DRV_VERSION_SUBL, DRV_RELDATE);
    CHECK_LEN_E;
#endif
    ...
    if (GET_CHIP_VER(priv) == VERSION_8814B)
	{
		...
		extern u32 get_ext_fw_version(HALMAC_FW_VERSION *fw_version);
		u32 fw_revision = get_ext_fw_version(&fw_version);
#ifdef __ECOS
	...
	rtl8192cd_proc_git_info(s, data);
#ifdef AUTH_SAE
	...
	size = rtl8192cd_proc_git_info(pTmp, start, offset, length, eof, data);
	CHECK_LEN_E;
...
#endif
{
...
#ifdef CONFIG_IEEE80211W
	#ifdef CONFIG_IEEE80211W_CLI
	PRINT_SINGL_ARG("    bss PMF cap: ", (priv->pmib->dot11Bss.t_stamp[1]>>9)&3, "%d");
	CHECK_LEN_B;
	#endif
	...
	PRINT_SINGL_ARG("    ft_state: ", pstat->ft_state, "%d");
	CHECK_LEN_B;
#endif
...
//#endif
	...
	PRINT_SINGL_ARG("    swq_atm_drops: ", pstat->swq_atm_drops, "%lu");
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    swq_atm_cnt: ", pstat->swq_atm_cnt, "%lu");
	CHECK_LEN_B;
#endif
	...
	PRINT_ONE("    rssi (ba): ", "%s", 0);
	PRINT_ONE(pstat->rssi_ba, "%u", 1);
#endif
	PRINT_ONE("    snr: ", "%s", 0);
...
#endif
	{
		...
		CHECK_LEN_B;
		PRINT_SINGL_ARG("    expire_rx_data: ", pstat->expire_rx_data, "%d");
		CHECK_LEN_B;
	}
...
#if 0 //def CONFIG_PCI_HCI
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
	        if(RT_STATUS_FAILURE ==  GET_HAL_INTERFACE(priv)->GetTxRPTHandler(priv, pstat->cmn_info.aid, TXRPT_VAR_PKT_DROP, 0, &bDrop))
	        {
...
/* Hotspot 2.0 Release 1 */
	...
	if(priv->pmib->miscEntry.client_host_sniffer_enable){
		PRINT_SINGL_ARG("    record_host_ip_timeout: ", pstat->record_host_ip_timeout, "%lu");
...
#if (BEAMFORMING_SUPPORT == 1)	 && (MU_BEAMFORMING_SUPPORT == 1)
	PRINT_SINGL_ARG("    txbfmu_isEntry: ", pstat->is_beamforming_entry, "%d");
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    txbfmu_isSundingEn: ", pstat->is_sounding_en, "%d");
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    txbfmu_paid: ", pstat->cmn_info.bf_info.p_aid, "%d");
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    txbfmu_g_id: ", pstat->cmn_info.bf_info.g_id, "%d");
	CHECK_LEN_B;
	PRINT_SINGL_ARG("    txbfmu_muPartenr: ", pstat->muPartner_num, "%d");
	CHECK_LEN_B;
#endif
#endif
#ifdef ADDRESS_CAM
		...
		{
			PRINT_SINGL_ARG("    addr_cam_entry: ", pstat->addr_cam_entry, "%d");
			CHECK_LEN_B;
		}
#endif
	...
	if(priv->pmib->miscEntry.ap_neighbor_info_enable)
	{
		...
		for(i=0; i<MAX_AP_NEIGHBOR_INFO_NUM; i++)
		{
			if(priv->pshare->ap_neighbor.ap_neighbor_info_ent[i].valid == 1)
			{
				...
				PRINT_SINGL_ARG("	Last seen: ", sec, "%lu seconds ago");
				PRINT_ONE("", "%s", 1);
			}
		}
	}
...
#ifdef USER_ADDIE
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_get_user_ie_info(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_get_user_ie_info(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif
{
	struct net_device *dev = PROC_GET_DEV();
	...
	PRINT_ONE("USERIE get Info...", "%s", 1);
	for(i=0; i<MAX_USER_IE; i++)
	{
		if(priv->user_ie_list[i].used && priv->user_ie_list[i].ie_len)
		{
			memset(pstring, 0, sizeof(pstring));
			data_ptr = pstring;
			for (j = 0 ; j < priv->user_ie_list[i].ie_len; j++)
			{
				snprintf(data_ptr , 3, "%02x" , priv->user_ie_list[i].ie[j]);
				data_ptr += 2;
			}

			if(OPMODE & WIFI_AP_STATE)
			{
				PRINT_ONE("	[Beacon]", "%s", 1);
				PRINT_ONE("	USER IE:", "%s", 1);
				PRINT_SINGL_ARG("	", pstring, "%s");

				PRINT_ONE("	[Probe Resp]", "%s", 1);
				PRINT_ONE("	USER IE:", "%s", 1);
				PRINT_SINGL_ARG("	", pstring, "%s");
			}
#ifdef CLIENT_MODE
...
#endif
{
...
#if defined(CONFIG_WLAN_HAL_8814BE)
	if (GET_CHIP_VER(priv)==VERSION_8814B) {
		RTL_W32(0x7e7c, 0x1fe00000);
		PRINT_SINGL_ARG("    dcpu alive:    ", RTL_R32(0x7e78), "%lu");
	}
#endif

#ifdef CH_LOAD_CAL
	/*reference IEEE,Std 802.11-2012,page567,use 0~255 to representing 0~100%*/
	PRINT_SINGL_ARG("    ch_utilization:	", ODMPTR->dm_ccx_info.clm_ratio, "%u");
	...
	PRINT_SINGL_ARG("    tx_dma_status1: ", priv->pshare->tx_dma_status1, "%lu");
	...
	PRINT_SINGL_ARG("    rx_refill_alloc_fail:   ", priv->ext_stats.rx_refill_alloc_fail, "%lu");
	PRINT_SINGL_ARG("	 rx_refill_alloc_success:	", priv->ext_stats.rx_refill_alloc_success, "%lu");
...
PRINT_SINGL_ARG("    mu_BB_fail:  ", priv->pshare->rf_ft_var.mu_BB_fail, "%d");
...
#else
		PRINT_SINGL_ARG("	 skb_free_num:	", skb_free_num[priv->pshare->wlandev_idx], "%d");
#endif
#else
		extern int skb_free_num;
#ifdef RX_SKB_QUEUE		
...
#else
		PRINT_SINGL_ARG("	 skb_free_num:	", skb_free_num, "%d");
#endif
#endif
	}
#else
	...
	PRINT_SINGL_ARG("    rx_enqueue_cnt:    ", rtl_atomic_read(&priv->pshare->rx_enqueue_cnt), "%d");
...
#ifdef RTK_AC_SUPPORT  //vht rate , todo, dump vht rates in Mbps
	if(current_rate >= VHT_RATE_ID){
		int tx_rate = VHT_MCS_DATA_RATE[MIN_NUM(priv->pshare->is_40m_bw, 3)][(priv->pshare->ht_current_tx_info&BIT(1))?1:0][(current_rate - VHT_RATE_ID)];
		tx_rate = tx_rate >> 1;
...
/* Hotspot 2.0 Release 1 */
	...
	PRINT_SINGL_ARG("    amsdu_cnt_timeout:  ", priv->pshare->amsdu_cnt_timeout, "%d");
	...
	if((priv->drv_state & DRV_STATE_OPEN) && (RTL_R8(0x1f)==0)){
	...
	PRINT_SINGL_ARG("    txdaq_overflow_drop_pkt: ", priv->ext_stats.txdaq_overflow_drop_pkt, "%lu");
	PRINT_SINGL_ARG("    swq_tasklet.tasklet_triggered: ", atomic_read(&priv->pshare->swq_tasklet.tasklet_triggered), "%d");
...
#endif
	PRINT_SINGL_ARG("    mw5_netif_rx_pkt:    ", priv->ext_stats.netif_rx_pkt, "%lu");
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	PRINT_SINGL_ARG("	 cnt_offload_sc: ", priv->cnt_offload_sc, "%d");
	PRINT_SINGL_ARG("	 cnt_offload_nosc: ", priv->cnt_offload_nosc, "%d");
	PRINT_SINGL_ARG("	 dc_free_sta_space: ", priv->pshare->dc_free_sta_space , "%d");
	...
	PRINT_SINGL_ARG("	 cil_pkt_id_unsync: ", priv->pshare->cil_pktid_unsync_cnt, "%d");
#endif
	PRINT_SINGL_ARG("    mcast_drop: ", priv->mcast_drop, "%lu");

#if defined(ADDRESS_CAM)
	PRINT_SINGL_ARG("    interface_addrcam_entry: ", priv->interface_addrcam_entry, "%d");
#endif
	RESTORE_INT(flags);
...
#endif
{
	...
	printk("priv->pmib->miscEntry.drop_rxpkt_en=%u\n", priv->pmib->miscEntry.drop_rxpkt_en);
	printk("priv->pmib->miscEntry.drop_rxpkt_rate=%u\n", priv->pmib->miscEntry.drop_rxpkt_rate);
	printk("priv->pmib->miscEntry.G5_drop_rxpkt_rate=%u\n", priv->pmib->miscEntry.G5_drop_rxpkt_rate);
	printk("priv->pmib->miscEntry.G5G24_drop_rxpkt_rate=%u\n", priv->pmib->miscEntry.G5G24_drop_rxpkt_rate);
...
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_led_r(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_led_r(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif
{
	...
	seq_printf(s, "%d\n",priv->led_flag);
#else
	len=snprintf(buf, length, "%d",priv->led_flag);
#endif
	return len;
...
#endif
{
	...
	unsigned int num, direction, value;
	int gpio_num=-1;
#endif
				...
				unsigned long count, void *data)
{
	...
	char tmp[32];
	unsigned int num, chnl=0, bw=40, pts=128;//fix bandwidth=40 scan_pts=128
				...
				unsigned long count, void *data)
{
    ...
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
    char tmp[64] = {0};
    int len = min(count, (unsigned long)(sizeof(tmp) - 1));
...
#ifdef CONFIG_RTL_PROC_NEW
static int proc_get_vdev_created(struct seq_file *s, void *data)
#else
static int proc_get_vdev_created(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif
{
	char enable[8] = {0};
	...
	//read priv and dump result to user
	sprintf(enable, "%d", priv->vdev_enabled);
	PRINT_ONE(enable, "%s", 1);

	return 0;
}

static int proc_set_vdev_created(struct file *file, const char *buffer,
				unsigned long count, void *data)
{
	char enable[8] = {0};
#ifdef CONFIG_RTL_PROC_NEW
	...
	if (buffer && !copy_from_user(enable, buffer, sizeof(enable))) {
		char vChar=simple_strtol(enable, NULL, 10);

		//vdev_enabled should changed
		if(priv->vdev_enabled^vChar)
		{
			char vname[IFNAMSIZ] = {0};
			snprintf(vname, sizeof(vname), "v%s", dev->name);
			if(vChar)
			{
				//create vdev if enable
				//printk("[%s:%d] create vdev for %s, previous type:%d, now type %d!\n", __FUNCTION__, __LINE__,
				//	dev->name, priv->vdev_enabled, vChar);
				rtl_wlan_vdev_init(vname, priv, vChar);
			}else{
				//destroy vdev if !enable
				//printk("[%s:%d] destroy vdev %s!\n", __FUNCTION__, __LINE__, vname);
				rtl_wlan_vdev_remove(vname, priv);
			}
		}
	}
...
#define DC_HCIDMA_START         0x18be0000
#define DC_AXIDMA_START         0x18bc0000
#define INDIRECT_READ(m_off, val)    \
    do {                            \
        RTL_W32(0x7e7c, (m_off & 0x7fffffff));   \
        val = RTL_R32(0x7e78);      \
    } while (0)
	...
	if (cookie == DCNT_CHECK_COOKIE) {
	    ...
	    dcnt_sys = (struct dcnt_sys_t *)value;
	    INDIRECT_READ(magic + (DCNT_CAT_PE * 4), value);
	    dcnt_pe = (struct dcnt_pe_t *)value;
#if 1
	    PRINT_ONE("    + DCOUNTER HCIDMA", "%s", 1);
	    INDIRECT_READ((u32)(&dcnt_pe->__hcidma_chan_rx), value);
	    PRINT_SINGL_ARG("      CH0 RX: ", value, "%d");
	    INDIRECT_READ((u32)(&dcnt_pe->__hcidma_chan_tx[0]), value);
	    PRINT_SINGL_ARG("      CH0 TX: ", value, "%d");
	    INDIRECT_READ((u32)(&dcnt_pe->__hcidma_chan_tx[1]), value);
	    PRINT_SINGL_ARG("      CH1 TX: ", value, "%d");
	    INDIRECT_READ((u32)(&dcnt_sys->__isr_run[15]), value);
	    PRINT_SINGL_ARG("      IRQ_RUN: ", value, "%d");

	    PRINT_ONE("    + DCOUNTER AXIDMA", "%s", 1);
	    INDIRECT_READ((u32)(&dcnt_pe->__axidma_chan_tx[0]), value);
	    PRINT_SINGL_ARG("      CH0 TX: ", value, "%d");
	    INDIRECT_READ((u32)(&dcnt_pe->__axidma_chan_tx[1]), value);
	    PRINT_SINGL_ARG("      CH1 TX: ", value, "%d");
	    INDIRECT_READ((u32)(&dcnt_pe->__axidma_chan_tx[2]), value);
	    PRINT_SINGL_ARG("      CH2 TX: ", value, "%d");
	    INDIRECT_READ((u32)(&dcnt_pe->__axidma_chan_tx[3]), value);
	    PRINT_SINGL_ARG("      CH3 TX: ", value, "%d");
		INDIRECT_READ((u32)(&dcnt_pe->__axidma_chan_tx[18]), value);
        PRINT_SINGL_ARG("      CH18 TX: ", value, "%d");
        INDIRECT_READ((u32)(&dcnt_pe->__axidma_chan_tx_still[18]), value);
        PRINT_SINGL_ARG("      CH18 TX STILL: ", value, "%d");
        INDIRECT_READ((u32)(&dcnt_pe->__err_drop_high), value);
        PRINT_SINGL_ARG("      err_drop_high: ", value, "%d");	
        INDIRECT_READ((u32)(&dcnt_pe->__cnt_hiq), value);
        PRINT_SINGL_ARG("      cnt_hiq: ", value, "%d");	
        INDIRECT_READ((u32)(&dcnt_pe->__sil_pkt_num[7]), value);
        PRINT_SINGL_ARG("      rx_all: ", value, "%d");	

	    INDIRECT_READ((u32)(&dcnt_pe->__axidma_chan_rx[0]), value);
	    PRINT_SINGL_ARG("      CH0 RX: ", value, "%d");
	    INDIRECT_READ((u32)(&dcnt_pe->__axidma_chan_rx[1]), value);
	    PRINT_SINGL_ARG("      CH1 RX: ", value, "%d");
	    INDIRECT_READ((u32)(&dcnt_sys->__isr_run[14]), value);
	    PRINT_SINGL_ARG("      IRQ_RUN: ", value, "%d");
		PRINT_ONE("    + SIL_PKT", "%s", 1);
        for (i=0; i<8; i++) {
            PRINT_ONE(i, "      CLASS %d :", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__sil_pkt_res_cnt[i]), value);
            PRINT_ONE(value, " NUM %u", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__sil_pkt_num[i]), value);
            PRINT_ONE(value, " CURR %u", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__sil_pkt_alloc[i]), value);
            PRINT_ONE(value, " alloc %u", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__sil_pkt_allocFail[i]), value);
            PRINT_ONE(value, " fail %u", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__sil_pkt_free[i]), value);
            PRINT_ONE(value, " free %u", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__sil_pkt_freeReuse[i]), value);
            PRINT_ONE(value, " reuse %u", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__sil_pkt_freeChain[i]), value);
            PRINT_ONE(value, " chain %d", 1);
        }
        PRINT_ONE("    + SKBUF", "%s", 1);
        for (i=0; i<5; i++) {
            PRINT_ONE(i, "      CLASS %d :", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__skbuf_alloc_cnt[i]), value);
            PRINT_ONE(value, " alloc %u", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__skbuf_free_cnt[i]), value);
            PRINT_ONE(value, " free %u", 0);
            INDIRECT_READ((u32)(&dcnt_pe->__skbuf_allocFail_cnt[i]), value);
            PRINT_ONE(value, " fail %u", 1);
        }
...
#undef DC_SRAM_START
#undef DC_HCIDMA_START
#undef DC_AXIDMA_START
#undef INDIRECT_READ
}
#endif
...
#if defined(ADDRESS_CAM)
#ifdef CONFIG_RTL_PROC_NEW
    static int rtl8192cd_proc_addr_cam_info(struct seq_file *s, void *data)
#else
    static int rtl8192cd_proc_addr_cam_info(char *buf, char **start, off_t offset,
                int length, int *eof, void *data)
#endif	
{
	struct net_device *dev = PROC_GET_DEV();
	...
	if (GET_CHIP_VER(priv)==VERSION_8814B)
	{
		u2Byte      camIdx = 0;
		unsigned int i = 0;
		unsigned int set_value = 0x8000007f;
		while(camIdx < ADDR_CAM_ENTRY_NUM)
		{
			PRINT_ONE(camIdx, " cam idx %2d ", 1);
			for(i = camIdx *8; i < (camIdx + 1)*8;i++)
			{
				RTL_W32(REG_ADDRCAM_CFG, set_value | (i << 8)); 
				PRINT_ONE( (i - (camIdx * 8)), " %2d : ", 0);
				PRINT_SINGL_ARG("", RTL_R32(REG_ADDRCAM_READ_CONTENT), "%08x");
				set_value &= ~(i << 8); 
			}
			camIdx++;
		}

		PRINT_ONE("", "addr_cam_used", 1);
		camIdx = 0;
		while(camIdx < ADDR_CAM_MAP_LEN)
		{
			PRINT_ONE( camIdx, " %2d : ", 0);
			PRINT_SINGL_ARG("", priv->pshare->addr_cam_used[camIdx], "%08x");
			camIdx++;
		}
...
#ifdef CONFIG_RTL_PROC_NEW
    static int rtl8192cd_proc_pe_stats(struct seq_file *s, void *data)
#else
    static int rtl8192cd_proc_pe_stats(char *buf, char **start, off_t offset,
                int length, int *eof, void *data)
#endif			
{
	struct net_device *dev = PROC_GET_DEV();
	...
	{
		PRINT_SINGL_ARG("pe state: ", pe_cb->pe_state, "%d");
		PRINT_SINGL_ARG("pe wifi state: ", pe_cb->pe_wifi_state, "%d");
		PRINT_SINGL_ARG("pe up time: ", pe_cb->up_time, "%d");

		{
			int i = 0;
			for (i = (NUM_STAT/8 - 1); i >= 0; i--) 
			{
				PRINT_ONE( pe_cb->sta_state[i], " %02x ", 0);
			}
			PRINT_ONE("", ",", 1);
			for (i = (NUM_STAT/8 - 1); i >= 0; i--) 
			{
				PRINT_ONE( pe_cb->sta_sw_queue_state[i], " %02x ", 0);
			}
			PRINT_ONE("", ",", 1);
			for (i = (NUM_STAT/8 - 1); i >= 0; i--) 
			{
				PRINT_ONE( pe_cb->sta_hw_queue_state[i], " %02x ", 0);
			}
			PRINT_ONE("", ",", 1);
...
#endif			
{
    ...
    if (num_sta >= 2) {
        PRINT_ONE("1-1", "%s", 1);
        PRINT_SINGL_ARG("  2f20 34: ", RTL_R32(0x2f20), "%08x");
        PRINT_SINGL_ARG("  2f24 35: ", RTL_R32(0x2f24), "%08x");
        PRINT_SINGL_ARG("  2f2c 43: ", RTL_R32(0x2f2c), "%08x");
        PRINT_SINGL_ARG("  2f30 45: ", RTL_R32(0x2f30), "%08x");
        PRINT_SINGL_ARG("  2f38 53: ", RTL_R32(0x2f38), "%08x");
        PRINT_SINGL_ARG("  2f3c 54: ", RTL_R32(0x2f3c), "%08x");
    }
    if (num_sta >= 3) {
        PRINT_ONE("1-1-1", "%s", 1);
        PRINT_SINGL_ARG("  2f54 345: ", RTL_R32(0x2f54), "%08x");
        PRINT_SINGL_ARG("  2f60 435: ", RTL_R32(0x2f60), "%08x");
        PRINT_SINGL_ARG("  2f6c 534: ", RTL_R32(0x2f6c), "%08x");
    }
    if (num_sta >= 4) {
        PRINT_ONE("1-1-1-1", "%s", 1);
        PRINT_SINGL_ARG("  2f70 2345: ", RTL_R32(0x2f70), "%08x");
        PRINT_SINGL_ARG("  2f74 3245: ", RTL_R32(0x2f74), "%08x");
        PRINT_SINGL_ARG("  2f78 4235: ", RTL_R32(0x2f78), "%08x");
        PRINT_SINGL_ARG("  2f7c 5234: ", RTL_R32(0x2f7c), "%08x");
    }
...
#endif			
{
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 128; j += 4) {
            ...
            PRINT_ONE(val32, "%08x ", 0);
            *(unsigned int *)&mu_rate[i][j] = val32;
        }
        PRINT_ONE("", "%s", 1);
    }
    PRINT_ONE("1-1", "%s", 1);
...
#if (MU_BEAMFORMING_SUPPORT == 1)
#ifdef CONFIG_RTL_PROC_NEW
    static int rtl8192cd_proc_mudbg(struct seq_file *s, void *data)
#else
    static int rtl8192cd_proc_muinfo(char *buf, char **start, off_t offset,
                int length, int *eof, void *data)
#endif			
{
    ...
    int pos = 0;
    int i;
    extern unsigned int mu_bitmap_in[16], mu_bitmap_out[16];
	extern int mu_bitmap_index;

    PRINT_ONE(priv->pshare->rf_ft_var.mu_group_mode, "MU group mode: %d", 1);
    PRINT_ONE("  0: default mode", "%s", 1);
    PRINT_ONE("  1: 4-3-2 => 2-3, 3-4, 4-2", "%s", 1);
    PRINT_ONE("  2: x", "%s", 1);
    PRINT_ONE("  3: PE deQ", "%s", 1);
    PRINT_ONE("  4: manual grouping", "%s", 1);
    PRINT_ONE("  5: pass table (choose left)", "%s", 1);
    PRINT_ONE("  6: choose right", "%s", 1);
    PRINT_ONE("", "%s", 1);

    PRINT_ONE("MU bitmap", "%s", 1);
    for (i = 0; i < 16; i++) {
    	if (i == mu_bitmap_index)
    		PRINT_ONE("*", "%s", 0);
		PRINT_ONE(mu_bitmap_in[i], "%08x ->", 0);
		PRINT_ONE(mu_bitmap_out[i], "%08x", 1);
    }
...
#endif
{
	...
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_pmk);
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_git_info);
	#ifdef AUTH_SAE
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_sae_peertable);
	#endif
...
#if defined(CONFIG_RTL_MONITOR_STA_INFO)
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_monitor_stainfo);
#endif
#if defined(AP_NEIGHBOR_INFO)
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_ap_neighbor_info);
#endif
#if defined(BEACON_VS_IE)
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_specific_ap_probereq_info);
#endif
#ifdef USER_ADDIE
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_get_user_ie_info);
#endif
	...
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_acs_score_info);//ACS_SCORE_PROC
#ifdef STA_ASSOC_STATISTIC
...
#endif
#ifdef BSS_MONITOR_SUPPORT
	RTK_DECLARE_READ_WRITE_PROC_FOPS(rtl8192cd_proc_monitor_info, rtl8192cd_proc_monitor_info_clear);
#endif
#ifdef STA_RATE_STATISTIC
	...
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_mib_veriwave);

#ifdef CONFIG_RTK_BAND_STEERING
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_get_band_steering_block_list);
#endif
	...
	RTK_DECLARE_READ_WRITE_PROC_FOPS(rtl8192cd_proc_led_r, rtl8192cd_proc_led);
#ifdef AUTO_TEST_SUPPORT
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
    RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_ofld);
#endif
    RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_muinfo);
    RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_murate);
#if (MU_BEAMFORMING_SUPPORT == 1)	
    RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_mudbg);
#endif
#ifdef RTK_ATM
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_atm);
#endif

#ifdef WLAN_VDEV_SUPPORT
	RTK_DECLARE_READ_WRITE_PROC_FOPS(proc_get_vdev_created, proc_set_vdev_created);
#endif

#ifdef CONFIG_PE_ENABLE
    RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_pe_stats);
#endif

#if defined(ADDRESS_CAM)
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_addr_cam_info);
#endif
...
{ // dump RxBD
	...
	for(i=0; i<=CMD_QUEUE; i++) {
		...
		PRINT_SINGL_ARG("    avail_txbd_num: ", ptx_dma->tx_queue[halQnum].avail_txbd_num, "%d");
...
#if defined(CONFIG_8814B_FWLOG_UDPSOCK)
{
	...
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_halq_info);
#endif
...
void MDL_DEVINIT rtl8192cd_proc_init(struct net_device *dev)
{
	...
	RTK_CREATE_PROC_READ_ENTRY(p, "git_info", rtl8192cd_proc_git_info);
    #ifdef AUTH_SAE
...
#endif
#ifdef USER_ADDIE
	RTK_CREATE_PROC_READ_ENTRY(p, "user_ie_info", rtl8192cd_proc_get_user_ie_info);
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	...
	RTK_CREATE_PROC_READ_ENTRY(p, "mib_veriwave", rtl8192cd_proc_mib_veriwave);
...
#endif
    {
		...
		RTK_CREATE_PROC_READ_WRITE_ENTRY(p, "led", rtl8192cd_proc_led_r, rtl8192cd_proc_led);
...
#ifdef WLAN_VDEV_SUPPORT
    RTK_CREATE_PROC_READ_WRITE_ENTRY(p, "vdev", proc_get_vdev_created, proc_set_vdev_created); 
#endif
...
#if (MU_BEAMFORMING_SUPPORT == 1)	
    RTK_CREATE_PROC_READ_ENTRY(p, "mudbg", rtl8192cd_proc_mudbg);
#endif
#ifdef RTK_ATM
...
#endif
#if defined(CONFIG_PE_ENABLE)
	RTK_CREATE_PROC_READ_ENTRY(p, "pe_stats", rtl8192cd_proc_pe_stats);
#endif

#if defined(ADDRESS_CAM)
	RTK_CREATE_PROC_READ_ENTRY(p, "add_cam_info", rtl8192cd_proc_addr_cam_info);
#endif
}
...
void /*__devexit*/MDL_EXIT rtl8192cd_proc_remove (struct net_device *dev)
{
	...
	//log3("dev[%p]priv=[%p] proc_root_ptr", dev, priv, rtl8192cd_proc_root);
	if (rtl8192cd_proc_root == NULL) {
		printk("NULL;CHK!!");
		return;
	}
	...
	remove_proc_entry("git_info", rtl8192cd_proc_root);
#ifdef AUTH_SAE
	...
	remove_proc_entry("acs_score_info", rtl8192cd_proc_root);
	...
	proc_remove(rtl8192cd_proc_root);
	//ONGGG
	priv->proc_root = NULL;
...
static struct _proc_table_ proc_table[] =
{
	...
	{"git_info",		rtl8192cd_proc_git_info,NULL},
#ifdef AUTH_SAE
...
#endif
#ifdef USER_ADDIE
	{"user_ie_info",	rtl8192cd_proc_get_user_ie_info, NULL},
#endif
#ifdef PROC_STA_CONN_FAIL_INFO
...
#endif
	{"mib_veriwave",		rtl8192cd_proc_mib_veriwave,NULL},
#ifdef CONFIG_RTK_BAND_STEERING
...
#ifdef WLAN_VDEV_SUPPORT
	{"vdev", proc_get_vdev_created, proc_set_vdev_created},
#endif
#if defined(THERMAL_CONTROL) && !defined(THERMAL_CONTROL_DAEMON)
...
#endif
    {"led",				rtl8192cd_proc_led_r,rtl8192cd_proc_led},
#ifdef CONFIG_PE_ENABLE
    {"pe_stats",				rtl8192cd_proc_pe_stats},
#endif
#if defined(ADDRESS_CAM)
	{"addr_cam_info",				rtl8192cd_proc_addr_cam_info},
#endif
};
...
#if defined(CONFIG_RTL_OFFLOAD_DRIVER) && !defined(__ECOS)
