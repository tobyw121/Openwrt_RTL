	...
	struct dm_struct *dm)
{
	...
	if (dm->rfe_type == 0) {
		...
		/* v44.44: recovered OEM SDK topology. RFE5 board_type is GPA1+GLNA1.
		 * Do not synthesize ODM EXT_TRSW here: the vendor RFE5 classifier does
		 * not advertise it. The MW5 MIB trswitch=1 remains preserved separately. */
		panic_printk("[97F] RFE type 5 PHY paratemters: GPA1+GLNA1 (MW5 SDK)\n");
		odm_cmn_info_init(dm, ODM_CMNINFO_BOARD_TYPE, ODM_BOARD_EXT_PA | ODM_BOARD_EXT_LNA);
