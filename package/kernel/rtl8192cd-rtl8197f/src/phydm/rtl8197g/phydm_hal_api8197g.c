...
void phydm_init_hw_info_by_rfe_type_8197g(struct dm_struct *dm)
{
	...
	} else if ( dm->rfe_type == 3 || dm->rfe_type == 4 || dm->rfe_type == 6) {
		panic_printk("[97G] RFE type 3,4,6 PHY paratemters: internal with TRSW\n");
		odm_cmn_info_init(dm, ODM_CMNINFO_BOARD_TYPE,
				  ODM_BOARD_EXT_TRSW);
	...
	} else if (dm->rfe_type == 1 || dm->rfe_type == 5) {
			       ...
			       enum bb_path tx_path_sel_1ss)
{
	...
	} else if (dm->rfe_type == 5 || dm->rfe_type == 6) {
		if (tx_path_en == BB_PATH_A && rx_path == BB_PATH_A) {
			odm_set_bb_reg(dm, R_0x4140, MASKBYTE0, 0x77);
...
phydm_ofdm_agc_tab_set_8197g(struct dm_struct *dm)
{
	...
	} else if (dm->rfe_type == 1 || dm->rfe_type == 3 || dm->rfe_type == 4 || dm->rfe_type == 5 || dm->rfe_type == 6) {
