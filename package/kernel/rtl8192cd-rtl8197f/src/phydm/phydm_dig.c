...
void phydm_fa_threshold_check(void *dm_void, boolean is_dfs_band)
{
	...
	if (dig_t->is_dbg_fa_th) {
		...
		if (dm->support_ic_type & (ODM_RTL8197G | ODM_RTL8192F | ODM_RTL8812F)) {
			dig_t->fa_th[0] = dm->priv->pshare->rf_ft_var.dbg_dig_th1;
			dig_t->fa_th[1] = dm->priv->pshare->rf_ft_var.dbg_dig_th2;
			dig_t->fa_th[2] = dm->priv->pshare->rf_ft_var.dbg_dig_th3;
		} 
		else
		{
...
void phydm_dig_init(void *dm_void)
{
	...
	#endif
	if (dm->support_ic_type & (ODM_RTL8192F | ODM_RTL8197G)) {
		dm->tdmadig_active_en = true;	    		
	} 
	dm->tdma_adptv_en = true;	
	dm->tdma_dig_adptv_trb_state_number = DIG_NUM_OF_TDMA_ADPTV_STATES;
	dm->tdma_dig_adptv_trb_swtch_th = 14;
...
void phydm_tdma_dig_sta_check(void *dm_void)
{
		struct dm_struct *dm = (struct dm_struct *)dm_void;
		struct cmn_sta_info *sta = NULL;
		u8	i;
		u8 active_sta_num = 0, atcive_rssi = dm->rssi_min;
		...
		for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
			...
			PHYDM_DBG(dm, DBG_CMN, "STA[%d] rx_moving_average_tp=%d, rssi=%d\n",
			  i, sta->rx_current_tp, sta->rssi_stat.rssi);

			if (sta->rx_current_tp >= 5) {
				active_sta_num++;
				atcive_rssi = sta->rssi_stat.rssi;
			}

		}
		
		PHYDM_DBG(dm, DBG_CMN, "active_sta_num=%d\n", active_sta_num);

		if (active_sta_num == 1) {
			dm->rssi_min_h_state = atcive_rssi;
		}else
			dm->rssi_min_h_state = dm->rssi_min;
}
...
void phydm_tdma_dig_timers_usb(void *dm_void, u8 state)
{
	...
	if (state == INIT_TDMA_DIG_TIMMER) {
		...
		mw5_legacy_timer_init(&dm->tdma_dig_timer);
...
void phydm_tdma_dig_cbk(void *dm_void)
{
	...
	/*@Dyanmically update state*/
	if (dm->tdma_adptv_en) {
		if (dm->tdma_adptv_trb_flag) {
			dm->tdma_dig_state_number = dm->tdma_dig_adptv_trb_state_number;
			PHYDM_DBG(dm, DBG_DIG, "[A-TDMADIG] tdma_dig_adptv_trb_state_number = %d\n",
				  dm->tdma_dig_adptv_trb_state_number);
		} else {
			dm->tdma_dig_state_number = DIG_NUM_OF_TDMA_STATES;
		}
	}
...
void phydm_tdma_high_dig(void *dm_void)
{
	...
	u32 fa_cnt = falm_cnt->cnt_all;
	boolean dfs_mode_en = false, is_performance = true;
	u8 rssi_min = (dm->tdmadig_active_en ? dm->rssi_min_h_state : dm->rssi_min);
	u8 igi_upper_rssi_min = 0;
	u8 offset = 15;
//	u8 step[3] = {0};
	...
	if (!dm->is_linked) {
		...
		if ((dig_t->cur_ig_value_tdma >= igi_upper_rssi_min) || (dm->tdmadig_active_en && (dm->rssi_min < 30)))
			dm->tdma_adptv_trb_flag = FALSE;
		else
			dm->tdma_adptv_trb_flag = TRUE;

		if (dm->tdma_adptv_en) {
			if (!(dm->tdma_adptv_trb_flag)) {
				igi_upper_rssi_min = dm->rssi_max + offset;
				PHYDM_DBG(dm, DBG_DIG, "[A-TDMADIG], A-TDMADIG is enable! tdma_h_dyn_max = %d\n",
                                 	 (dm->rssi_max + offset));
				if (dig_t->cur_ig_value_tdma <= (rssi_min + offset + dm->tdma_dig_adptv_trb_swtch_th))
					dm->tdma_adptv_trb_flag = TRUE;
			}
		} else {
			dm->tdma_adptv_trb_flag = FALSE;
		     ...
		     u32 *_out_len)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 ...
			 dm->tdma_adptv_en);
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "{7} {tdma_dig_adptv_trb_state_number = %d}\n",
			 dm->tdma_dig_adptv_trb_state_number);
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "{8} {tdma_dig_adptv_trb_swtch_th = %d}\n",
			 dm->tdma_dig_adptv_trb_swtch_th);
		#endif
	} else {
		PHYDM_SSCANF(input[1], DCMD_DECIMAL, &var1[0]);
			...
			if (var1[1] == 0) {
				...
				PDM_SNPF(out_len, used, output + used,
					 ...
					 dig_t->tdma_force_h_igi);
                        ...
                        dm->tdma_adptv_en = (u8)var1[1];
			PDM_SNPF(out_len, used, output + used,
				 out_len - used, "tdma_adptv_en = %d\n",
				 dm->tdma_adptv_en);
		} else if (var1[0] == 7) {
			dm->tdma_dig_adptv_trb_state_number = (u8)var1[1];
			PDM_SNPF(out_len, used, output + used,
				 out_len - used, "tdma_dig_adptv_trb_state_number = %d\n",
				 dm->tdma_dig_adptv_trb_state_number);
		} else if (var1[0] == 8) {
			dm->tdma_dig_adptv_trb_swtch_th = (u8)var1[1];
			PDM_SNPF(out_len, used, output + used,
				 out_len - used, "tdma_dig_adptv_trb_swtch_th = %d\n",
				 dm->tdma_dig_adptv_trb_swtch_th);
		#endif
		}
