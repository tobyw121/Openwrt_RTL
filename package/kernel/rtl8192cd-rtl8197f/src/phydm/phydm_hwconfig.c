...
/*@jj add 20170822*/
	...
	if (dm->support_ic_type == ODM_RTL8192F) {
		if (config_type == CONFIG_RF_RADIO) {
			...
			rfelog("[8192FE] CONFIG_RF_TXPWR_LMT type[%d]",dm->rfe_type);
			if (dm->rfe_type == 0)
				...
				printk("[%s][%d]type 19",__func__,__LINE__);
...
#if RTL8192F_SUPPORT
	if (dm->support_ic_type == ODM_RTL8192F) {
		rfelog("[8192FE] type[%d]",dm->rfe_type);
		if (dm->rfe_type == 0)
...
/*@jj add 20170822*/
	...
	if (dm->support_ic_type == ODM_RTL8192F) {
		if (config_type == CONFIG_BB_PHY_REG) {
			...
			rfelog("8192FE CONFIG_BB_PHY_REG_PG type[%d]",dm->rfe_type);
			if (dm->rfe_type == 0)
