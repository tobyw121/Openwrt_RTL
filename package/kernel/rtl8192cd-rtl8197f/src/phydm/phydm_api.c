...
#if (RTL8197F_SUPPORT) || (RTL8812A_SUPPORT) || (RTL8822B_SUPPORT)
	struct dm_struct *dm = (struct dm_struct *)dm_void;
#endif
...
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
#if (RTL8192E_SUPPORT || RTL8192F_SUPPORT) || (RTL8812A_SUPPORT || RTL8822B_SUPPORT)
	u8 val = 0;
#endif
...
void phydm_bw_fixed_enable(void *dm_void, boolean enable)
{
	...
	else if (dm->support_ic_type & (ODM_RTL8822C | ODM_RTL8197G))
		odm_set_bb_reg(dm, R_0x878, BIT(28), val);
	if ((val == 0) && (dm->support_ic_type & ODM_RTL8192F))
...
//	s8 evm_org, cfo_org, rxsnr_org;
	u8 return_info = 0, tmp_lsb = 0, tmp_msb = 0, tmp_info = 0;
...
phydm_api_shift_txagc(void *dm_void, u32 pwr_offset, enum rf_path path,
		      boolean is_positive) {
	boolean ret = false;
#if (RTL8822C_SUPPORT || RTL8812F_SUPPORT || RTL8197G_SUPPORT || RTL8198F_SUPPORT || RTL8814B_SUPPORT)
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u32 txagc_cck = 0;
	u32 txagc_ofdm = 0;
	u32 r_txagc_ofdm[4] = {R_0x18e8, R_0x41e8, R_0x52e8, R_0x53e8};
		   ...
		   enum bb_path tx_path_ctrl)
{
	...
	// phydm_api_trx_mode
	dm->priv->pshare->rf_ft_var.TxPath = tx_path;
	dm->priv->pshare->rf_ft_var.RxPath = rx_path;
	dm->priv->pshare->rf_ft_var.bTx2Path = is_2tx;	
	// phydm_api_trx_mode
