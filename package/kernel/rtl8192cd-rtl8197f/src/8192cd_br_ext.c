...
#include <net/ip.h>
/* net/ipx.h was removed from modern kernels; mw5_linux_compat.h provides the wire header. */
...
#if defined(__OSK__)
#undef ASSERT   /*different redefined  */
#include "p2port.h"
#include "ipport.h"
#undef ASSERT   /*useless ASSERT in this file*/
#endif
...
#if defined(OSK)
/*here define ip6 checksum function for osk platform*/
/*
...
static uint32 do_csum(unsigned short *ptr, int count)
{
    ...
    if (count > 0) 
        sum += (*((uint8*)ptr) << 8) & 0xff00;
    if ( sum>>31)
            sum = (sum&0xffff) + ((sum>>16)&0xffff);
    return sum;
}
static unsigned int csum_partial(const void *buff, int len,  unsigned int wsum)
 {
         unsigned int sum = wsum;
         unsigned int result = do_csum((unsigned short*)buff, len);
...
static __inline__  unsigned short csum_ipv6_magic(const struct in6_addr *saddr,
					   ...
					   unsigned int csum)
{
	...
	sum += ( u32)natohl(&saddr->s6_addr32[0]);
	carry = (sum < ( u32)natohl(&saddr->s6_addr32[0]));
	sum += carry;

	sum += ( u32)natohl(&saddr->s6_addr32[1]);
	carry = (sum < ( u32)natohl(&saddr->s6_addr32[1]));
	sum += carry;

	sum += ( u32)natohl(&saddr->s6_addr32[2]);
	carry = (sum < ( u32)natohl(&saddr->s6_addr32[2]));
	sum += carry;

	sum += ( u32)natohl(&saddr->s6_addr32[3]);
	carry = (sum < ( u32)natohl(&saddr->s6_addr32[3]));
	sum += carry;

	sum += ( u32)natohl(&daddr->s6_addr32[0]);
	carry = (sum < ( u32)natohl(&daddr->s6_addr32[0]));
	sum += carry;

	sum += ( u32)natohl(&daddr->s6_addr32[1]);
	carry = (sum < ( u32)natohl(&daddr->s6_addr32[1]));
	sum += carry;

	sum += ( u32)natohl(&daddr->s6_addr32[2]);
	carry = (sum < ( u32)natohl(&daddr->s6_addr32[2]));
	sum += carry;

	sum += ( u32)natohl(&daddr->s6_addr32[3]);
	carry = (sum < ( u32)natohl(&daddr->s6_addr32[3]));
	sum += carry;
	...
	return csum_fold((unsigned int)sum);
}
#endif
...
#ifdef __ECOS
unsigned short ip_csum(unsigned short *ptr, int len, unsigned short resid)
{
	register unsigned int csum = resid;
	int odd = 0;
	...
	for(;len > 0 ; len--,ptr++){
		csum +=((((*(unsigned char *)(ptr)) <<8)&0xff00) |(*((unsigned char *)(ptr) +1)));		
	}
	if(odd){
		csum += (*((unsigned char *)ptr) <<8) & 0xff00;
	}
...
#endif
{
...
#endif
	unsigned char *mcmac;
	unsigned char *ipaddr = (unsigned char*)dst_ip;

	// don't care if NOT multicast IP address
	if ((ipaddr[0] & 0xf0) != 0xe0)
		return 0;
	...
	while (plist != phead) {
		...
		for (i=0; i < pstat->ipmc_num; i++) {
			// compare last 23 bits of MAC address with lower-order 23bit of multicast IP address
			mcmac = pstat->ipmc[i].mcmac;
			if (mcmac[0] == 0x01 && mcmac[3] == (ipaddr[1] & 0x7F) 
					&& mcmac[4] == ipaddr[2] && mcmac[5] == ipaddr[3]) {
				memcpy(skb->data, mcmac, ETH_ALEN);
				result = 1;
				goto exit;
			}
		}
...
int nat25_db_handle(struct rtl8192cd_priv *priv, struct sk_buff *skb, int method)
{
	...
	/*                 Handle ARP frame                  */
	/*---------------------------------------------------*/
	else if(ntohs(protocol) == ETH_P_ARP)
	{
	...
	/*---------------------------------------------------*/
	else if((ntohs(protocol) == ETH_P_IPX) ||
	...
	/*                Handle PPPoE frame                 */
	/*---------------------------------------------------*/
	else if((ntohs(protocol) == ETH_P_PPP_DISC) ||
			(ntohs(protocol) == ETH_P_PPP_SES))
	{
		struct pppoe_hdr *ph = (struct pppoe_hdr *)(skb->data + ETH_HLEN);
	...
	/*---------------------------------------------------*/
	else if((ntohs(protocol) == 0xe2ae) ||
			(ntohs(protocol) == 0xe2af))
	{
		switch(method)
		{
			case NAT25_CHECK:
	...
	/*---------------------------------------------------*/
#ifdef CL_IPV6_PASS
	else if(ntohs(protocol) == ETH_P_IPV6)
	{
		struct ipv6hdr *iph = (struct ipv6hdr *)(skb->data + ETH_HLEN);
					...
					if (memcmp(&iph->saddr, "\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0", 16)) {
						...
						if (iph->nexthdr == IPPROTO_ICMPV6 &&
							...
							if (update_nd_link_layer_addr(skb->data + ETH_HLEN + sizeof(*iph),
								...
								struct icmp6hdr  *hdr = (struct icmp6hdr *)(skb->data + ETH_HLEN + sizeof(*iph));
#ifdef __OSK__                  /*20160727 filled by jim*/
                                /*the icmpv6 checksum test code is located in gendrv.c, 
                                and switched by macro DEBUG_ICMPv6CHECKSUM*/
...
unsigned char FindWiFiSta(struct rtl8192cd_priv *inPriv, unsigned char *addr)
{
    ...
    phead = &priv->asoc_list;
	    ...
	    while((((GET_MIB(priv)->dot11OperationEntry.opmode) & WIFI_AP_STATE) == WIFI_AP_STATE) && (!list_empty(plist) && (plist = plist->next) != phead))
	    {
		    pstat = list_entry(plist, struct stat_info, asoc_list);
            if (pstat && (!memcmp(addr, pstat->cmn_info.mac_addr, ETH_ALEN)) ){
				SMP_UNLOCK_ASOC_LIST(flags);
                return 1;
            }
        }
	...
	if (direction == MAC_CLONE_SA_FIND){
...
#endif
		{
			...
			for (i=0; i<priv->pshare->mclone_num_max; i++) {
				if ((priv->pshare->mclone_sta[i].priv) && priv->pshare->mclone_sta[i].usedStaAddrId!=0xff
				        && (isEqualMACAddr(addr, priv->pshare->mclone_sta[i].sa_addr))) {
					return i+1;						
				}
			}
		}else{
			...
			if (priv->pmib->ethBrExtInfo.macclone_eth_method){
				for (i=0; i<priv->pshare->mclone_num_max; i++) {
					if ((priv->pshare->mclone_sta[i].priv) && priv->pshare->mclone_sta[i].usedStaAddrId!=0xff
							&& (isEqualMACAddr(addr, priv->pshare->mclone_sta[i].sa_addr))) {
						return i+1; 					
		...
		for (i=0; i<priv->pshare->mclone_num_max; i++) {
			if (priv->pshare->mclone_sta[i].usedStaAddrId!=0xff && 
				isEqualMACAddr(addr, priv->pshare->mclone_sta[i].hwaddr)) 
				return i+1;						
		}
	}else if (direction == MAC_CLONE_DA_FIND && pskb){
	}

	if (isEqualMACAddr(addr, priv->pmib->dot11OperationEntry.hwaddr)) 
		return 0;

#if defined(VXD_DIFF_MAC) || defined(VXD_DIFF_MAC_DRV)
	if (isEqualMACAddr(addr, GET_VXD_PRIV(priv)->pmib->dot11OperationEntry.hwaddr)) 
		return 0;
#endif	

	for (i=0; i<priv->pshare->mclone_num_max; i++) {
		if ( (priv->pshare->mclone_sta[i].priv) && (isEqualMACAddr(addr, priv->pshare->mclone_sta[i].hwaddr)) ) 
			return i+1;						
	}
...
void mclone_get_cloned_MAC_addr(struct rtl8192cd_priv *priv, unsigned char *sa_addr, int usedID)
{
	unsigned char *targetAddr = priv->pshare->mclone_sta_fixed_addr[usedID].clone_addr;
	unsigned char *rootAddr = GET_MIB(GET_ROOT(priv))->dot11OperationEntry.hwaddr;
	
	memcpy(targetAddr, sa_addr, ETH_ALEN);	 //copy source addr
	targetAddr[3] ^= rootAddr[4] ^ rootAddr[3];
	targetAddr[4] ^= rootAddr[5];
	targetAddr[0] |= BIT1;
}
...
int mac_clone_handle_frame(struct rtl8192cd_priv *priv, struct sk_buff *skb)
{
	...
	if(priv->pmib->ethBrExtInfo.macclone_enable && !priv->macclone_completed)
	{
		if(!(skb->data[ETH_ALEN] & 1))	//// check any other particular MAC add
		{
			...
			pstat = get_stainfo_from_all_wlan(skb->data+ETH_ALEN);
			if(memcmp(skb->data+ETH_ALEN, GET_MY_HWADDR, ETH_ALEN) && (
#if defined( __KERNEL__) || defined(__OSK__)
            ...
            memcmp(skb->data+ETH_ALEN, priv->br_mac, ETH_ALEN))  
		{
            ...
            { //from wifi AP
				...
				if (priv->pmib->ethBrExtInfo.macclone_method == 1){
					...
					memcpy(priv->pshare->mclone_sta[id-1].hwaddr, priv->pshare->mclone_sta_fixed_addr[usedID].clone_addr, ETH_ALEN); //Cloned Mac Addr vary from STA
				} else if (priv->pmib->ethBrExtInfo.macclone_method == 2 ){
					mclone_get_cloned_MAC_addr(priv, skb->data+ETH_ALEN, usedID);
					memcpy(priv->pshare->mclone_sta[id-1].hwaddr, priv->pshare->mclone_sta_fixed_addr[usedID].clone_addr, ETH_ALEN); //Cloned Mac Addr vary from STA + Random
				} else {	// macclone_method=0
					memcpy(priv->pshare->mclone_sta[id-1].hwaddr, priv->pshare->mclone_sta_fixed_addr[usedID].clone_addr, ETH_ALEN); //Cloned Mac Addr vary from from vxd
				}
				...
				if (priv->pmib->ethBrExtInfo.macclone_eth_method == 1){
				    unsigned char usedID=0xff;					
					usedID = mclone_find_staFixedAddr(priv);
					if (usedID != 0xff)
					    priv->pshare->mclone_sta_fixed_addr[usedID].used = 1;
					mclone_get_cloned_MAC_addr(priv, skb->data+ETH_ALEN, usedID);			//Cloned Mac Addr vary from Eth Mac Addr + Random
					memcpy(priv->pshare->mclone_sta[id-1].sa_addr, skb->data+ETH_ALEN, ETH_ALEN);  
					memcpy(priv->pshare->mclone_sta[id-1].hwaddr, priv->pshare->mclone_sta_fixed_addr[usedID].clone_addr, ETH_ALEN);	// Cloned Mac Addr vary from eth Addr
					priv->pshare->mclone_sta[id-1].usedStaAddrId = usedID;
				} else 	{
					memcpy(priv->pshare->mclone_sta[id-1].hwaddr, skb->data+ETH_ALEN, ETH_ALEN);	// Original Eth Mac Addr
				}
			}
...
void dhcp_flag_bcast(struct rtl8192cd_priv *priv, struct sk_buff *skb)
{
	...
	//if(skb->data_len <32)
	if(skb->len <42) //14(ethernet header)+20(ip header)+8(udp header)
		return;
...
void dhcp_dst_bcast(struct rtl8192cd_priv * priv,struct sk_buff * skb)
{
	...
	if(!priv->pmib->ethBrExtInfo.dhcp_bcst_disable) {
		iph = (struct iphdr*) rtl_get_skb_ip_header(skb, 0);
		...
		if(iph->protocol == IPPROTO_UDP) {
			...
			if (ip_check_recalc) {
				...
				iph->check = 0;
				res = ip_csum((unsigned short *)iph, iph->ihl << 2, 0);
				iph->check = htons(~res);
#elif defined(__OSK__)
				uint16 res = 0;
				iph->check = 0;
...
unsigned char nat25_filter(struct rtl8192cd_priv *priv, struct sk_buff *skb) {
    ...
    if(found) {
        if(ethtype == __constant_htons(ETH_P_IP)) {
            ...
            /* Get next header from IPv6 extension header if it exists */
            if (ipv6_ext_hdr(ipproto)) {
                struct ipv6hdr *hdr = ipv6_hdr(skb);
                int offset;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 3, 0))
                __be16 frag_off;
                offset = ipv6_skip_exthdr(skb, sizeof(*hdr)+WLAN_ETHHDR_LEN, &ipproto, &frag_off);
#else
                offset = ipv6_skip_exthdr(skb, sizeof(*hdr)+WLAN_ETHHDR_LEN, &ipproto);
#endif
                if (offset < 0) {
