...
void rtl8192cd_dfs_chk_timer(unsigned long task_priv)
{
	...
	if (mw5_legacy_timer_pending(&priv->dfs_chk_timer)){
...
void set_CHXX_timer(struct rtl8192cd_priv *priv, unsigned int channel)
{
	switch(channel) {
		case 52:
			mw5_legacy_mod_timer(&priv->ch52_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 56:
			mw5_legacy_mod_timer(&priv->ch56_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 60:
			mw5_legacy_mod_timer(&priv->ch60_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 64:
			mw5_legacy_mod_timer(&priv->ch64_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 100:
			mw5_legacy_mod_timer(&priv->ch100_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 104:
			mw5_legacy_mod_timer(&priv->ch104_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 108:
			mw5_legacy_mod_timer(&priv->ch108_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 112:
			mw5_legacy_mod_timer(&priv->ch112_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 116:
			mw5_legacy_mod_timer(&priv->ch116_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 120:
			mw5_legacy_mod_timer(&priv->ch120_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 124:
			mw5_legacy_mod_timer(&priv->ch124_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 128:
			mw5_legacy_mod_timer(&priv->ch128_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 132:
			mw5_legacy_mod_timer(&priv->ch132_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 136:
			mw5_legacy_mod_timer(&priv->ch136_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 140:
			mw5_legacy_mod_timer(&priv->ch140_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		case 144:
			mw5_legacy_mod_timer(&priv->ch144_timer, jiffies + NONE_OCCUPANCY_PERIOD);
			break;
		default:
...
void rtl8192cd_DFS_TXPAUSE_timer(unsigned long task_priv)
{
	...
	if (priv->available_chnl_num != 0) {
		if (mw5_legacy_timer_pending(&priv->DFS_TXPAUSE_timer)) 
		...
		mw5_legacy_mod_timer(&priv->DFS_TXPAUSE_timer, jiffies + DFS_TXPAUSE_TO);
	}
...
void dfs_init_para_by_manual_dfs_regdomain(struct rtl8192cd_priv *priv)
{
	...
	if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 1) {
		...
		if ((priv->pmib->dot11RFEntry.dot11channel >= 52) && (priv->pmib->dot11RFEntry.dot11channel <= 64)) {
			if(priv->pshare->rf_ft_var.jp_new_w53 == 1){
			...
			if (GET_CHIP_VER(priv) == VERSION_8814B) {
				...
				priv->pshare->rf_ft_var.pri_lth3 = 0;
				...
				printk("\n>>dfs VERSION_8812f for new w53 Jap.~\n");
				...
				priv->pshare->rf_ft_var.pw_lth1 = 2;
				priv->pshare->rf_ft_var.pw_uth1 = 3;
				priv->pshare->rf_ft_var.pw_lth2 = 6;
				priv->pshare->rf_ft_var.pw_uth2 = 7;
				priv->pshare->rf_ft_var.pri_lth1 = 30;
				priv->pshare->rf_ft_var.pri_uth1 = 155;
				priv->pshare->rf_ft_var.pri_lth2 = 30;
				priv->pshare->rf_ft_var.pri_uth2 = 230;
				printk("\n>>dfs VERSION_8812f for original w53 Jap.~\n");
			}
		}
		else{
		priv->pshare->rf_ft_var.pw_lth1 = 1;
		...
		if (GET_CHIP_VER(priv) == VERSION_8814B) {
				...
				priv->pshare->rf_ft_var.pri_lth1 = 33;
				priv->pshare->rf_ft_var.pri_uth1 = 73;
				priv->pshare->rf_ft_var.pri_lth2 = 35;
				priv->pshare->rf_ft_var.pri_uth2 = 75;
				priv->pshare->rf_ft_var.pri_lth3 = 130;
				priv->pshare->rf_ft_var.pri_uth3 = 170;
...
void rtl8192cd_DFS_timer(unsigned long task_priv)
{
	...
	if (!priv->pmib->dot11DFSEntry.disable_DFS && priv->pmib->dot11DFSEntry.DFS_detected) {
		if (GET_CHIP_VER(priv) == VERSION_8192D) {
			panic_printk("Radar is detected as %x %08x (%d)!\n",
				priv->radar_type, phy_query_bb_reg(priv, 0xcf4, bMaskDWord), (unsigned int)RTL_JIFFIES_TO_MILISECONDS(jiffies));
		}
		else if ((GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv) == VERSION_8881A) || (GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B)) {
			panic_printk("Radar is detected as %x %08x (%d)!\n",
...
#ifdef CONFIG_RTL_92D_SUPPORT
		if (mw5_legacy_timer_pending(&priv->dfs_chk_timer)) {
			mw5_legacy_del_timer(&priv->dfs_chk_timer);
			if (GET_CHIP_VER(priv) == VERSION_8192D)
				phy_set_bb_reg(priv, 0xcdc, BIT(8)|BIT(9), 1);
			...
			if (dfs_chk){
				...
				PRINT_INFO("DFS CP1.\n");
				mw5_legacy_mod_timer(&priv->dfs_chk_timer, jiffies + RTL_SECONDS_TO_JIFFIES(300));
...
#if defined(RTK_MULTI_AP) && defined(RTK_MULTI_AP_R1_DFS)	//skip reset dfsSwitchChCountDown and skip selecting new channel for multiap csa, instead use the channel from in received csa
...
exit_timer:
	mw5_legacy_mod_timer(&priv->DFS_timer, jiffies + DFS_TO);
	if (locked)
		SMP_UNLOCK(flags);
	RESTORE_INT(flags);
}
...
void DFS_SwitchChannel(struct rtl8192cd_priv *priv)
{
	...
	if(!is_DFS_channel(priv, priv->pmib->dot11RFEntry.dot11channel) && under_apmode_repeater(priv))
	{
		if (mw5_legacy_timer_pending(&priv->DFS_timer))
		   mw5_legacy_del_timer(&priv->DFS_timer);
...
void rtl8192cd_dfs_det_chk_timer(unsigned long task_priv)
{
	...
	if ((GET_CHIP_VER(priv) == VERSION_8814B) || (GET_CHIP_VER(priv) == VERSION_8812F)) {
		...
		priv->st_L2H_cur = (phy_query_bb_reg(priv, 0xa40, 0x00007f00) >> 1);
	}
	else {
	...
	if(((priv->pmib->dot11RFEntry.dot11channel >= 52) &&
		...
		// MKK w53
		mw5_legacy_mod_timer(&priv->dfs_det_chk_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(priv->pshare->rf_ft_var.dfs_det_period_jp_w53*10));
	}
	else{
		mw5_legacy_mod_timer(&priv->dfs_det_chk_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(priv->pshare->rf_ft_var.dfs_det_period*10));
	}
...
void DFS_SetReg(struct rtl8192cd_priv *priv)
{
	...
	if(GET_CHIP_VER(priv) == VERSION_8812F || GET_CHIP_VER(priv) == VERSION_8814B)
		dfs_init_para_by_manual_dfs_regdomain(priv);
	...
	if (GET_CHIP_VER(priv) == VERSION_8192D) {
		...
		if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 1) {
			...
			phy_set_bb_reg(priv, 0xa40, bMaskDWord, 0xb25dc0bd);
			phy_set_bb_reg(priv, 0xa44, bMaskDWord, 0x3830bea5);
			phy_set_bb_reg(priv, 0xa48, bMaskDWord, 0x2a521254);
			phy_set_bb_reg(priv, 0xa4c, bMaskDWord, 0xa2533345);
			phy_set_bb_reg(priv, 0xa50, bMaskDWord, 0x605be0c3);
			phy_set_bb_reg(priv, 0xa54, bMaskDWord, 0x50000928);
