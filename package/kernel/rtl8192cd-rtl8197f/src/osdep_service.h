...
typedef struct mw5_legacy_timer _timer;
...
__inline static void _mw5_legacy_timer_init(_timer *ptimer, _nic_hdl nic_hdl,void *pfunc,void* cntx)
{
	//setup_timer(ptimer, pfunc,(u32)cntx);	
	mw5_legacy_timer_init(ptimer);
...
{	
	mw5_legacy_mod_timer(ptimer , jiffies+msecs_to_jiffies(delay_time));
}

__inline static void _cancel_timer(_timer *ptimer,u8 *bcancelled)
{
	mw5_legacy_del_timer_sync(ptimer); 	
	*bcancelled = TRUE;//TRUE ==1; FALSE==0
}

__inline static unsigned char _cancel_timer_ex(_timer *ptimer)
{
	return mw5_legacy_del_timer_sync(ptimer);
}
#endif
