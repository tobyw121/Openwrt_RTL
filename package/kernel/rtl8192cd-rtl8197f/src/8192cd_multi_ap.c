...
void rtk_multi_ap_switch_channel(struct rtl8192cd_priv *priv)
{
	...
	if (JOIN_RES == STATE_Sta_Ibss_Idle) {
		...
		mw5_legacy_mod_timer(&priv->idle_timer, jiffies + RTL_SECONDS_TO_JIFFIES(5));
	}
#endif
#endif
...
int rtk_multi_ap_recordUnassocStaMetric(struct rtl8192cd_priv *priv, unsigned char *buf){
	...
	if(priv->multiap_unassoc_sta_timer.function != NULL){
		mw5_legacy_mod_timer(&priv->multiap_unassoc_sta_timer ,jiffies + RTL_SECONDS_TO_JIFFIES(7));
...
error:
	ret = 1;
	mw5_legacy_mod_timer(&priv->multiap_backhaul_steer_timer,jiffies + RTL_SECONDS_TO_JIFFIES(10));
	return ret;	
}
#endif
...
int rtk_multi_ap_do_cac(struct rtl8192cd_priv *priv, unsigned char *tmpbuf)
{
	...
	if (0 == type) {	//CAC Request
		...
		if(priv->multiap_cac_process_timer.function != NULL) {
			mw5_legacy_mod_timer(&priv->multiap_cac_process_timer,jiffies + RTL_MILISECONDS_TO_JIFFIES(delay_start));
		}
		...
		if (mw5_legacy_timer_pending(&priv->multiap_cac_process_timer)){
			mw5_legacy_del_timer_sync(&priv->multiap_cac_process_timer);
		}
...
void rtk_multi_ap_cac_channel_switch(unsigned long task_priv)
{
	...
	if(priv->multiap_cac_process_timer.function != NULL) {
		// mw5_legacy_mod_timer(&priv->multiap_cac_process_timer,jiffies + RTL_MILISECONDS_TO_JIFFIES(5000));
		mw5_legacy_mod_timer(&priv->multiap_cac_process_timer,jiffies + RTL_SECONDS_TO_JIFFIES(10));
	}
