...
)
{
	...
	// In 8192F,8198F,8197G empty CAM entry must follow the sequence
	// Entry1 => Entry0 => Enrty2 => ... => Entry8
	// Entry1 must empty before Entry0
	...
	case HW_VAR_MAC_CONFIG: {
		...
		if ( _GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1 ) {
            ...
            if ( RT_OP_MODE_AP == pMacCfgPara->OP_Mode ) {
				...
				HAL_RTL_W16(REG_RXFLTMAP1, BIT_CTRLFLT10EN | BIT_CTRLFLT9EN);
#else
...
#if IS_RTL88XX_MAC_V1_V2
		if(_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1_v2)
			HAL_RTL_W8(REG_HIQ_NO_LMT_EN, (u1Byte)(HiQNoLMTEn2));
#endif
		}
	break;
...
#if IS_RTL88XX_MAC_V2_V3
		if (_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v2_v3)	
			HAL_RTL_W8(0x52c, HAL_RTL_R8(0x52c) & 0xbf);
#endif	
#if IS_RTL88XX_MAC_V1_V2
	if (_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1_v2)
    		//HAL_RTL_W8(REG_MBSSID_CTRL, MBssid);
    		HAL_RTL_W8(REG_MBSSID_CTRL, (u1Byte)(MBssid2&0xff));
...
)
{
	...
	if ( IS_HARDWARE_TYPE_8192F(Adapter) ) {
		if (IS_HAL_TEST_CHIP(Adapter)) {
			...
			rfelog("[PHY_REG_PG_8192F]\n");
			*((unsigned long*)(val)) = (unsigned long)data_PHY_REG_PG_8192F_start;
		}else if(HAL_RFE_TYPE==3){
			rfelog("[%s][start PHY_REG_PG_8192F_type3]",__func__);
			*((unsigned long*)(val)) = (unsigned long)data_PHY_REG_PG_8192F_type3_start;
		} else { // mp chip
...
//       				u4Byte size = (u4Byte)(data_PHY_REG_PG_8197Gmp_end - data_PHY_REG_PG_8197Gmp_start);
		...
		if ( IS_HARDWARE_TYPE_8192F(Adapter) ) {
			if (IS_HAL_TEST_CHIP(Adapter)) {
					*((pu4Byte)(val)) = (u4Byte)(data_PHY_REG_PG_8192F_end - data_PHY_REG_PG_8192F_start);
					...
					printk("[%s][size PHY_REG_PG_8192F_Type3 ]\n",__func__);
					rfelog("[%s][size PHY_REG_PG_8192F_type3]",__func__);
					*((pu4Byte)(val)) = (u4Byte)(data_PHY_REG_PG_8192F_type3_end - data_PHY_REG_PG_8192F_type3_start);
				}
				else { // mp chip
					printk("[%s][PHY_REG_PG_8192Fmp size]\n",__func__);
...
#ifdef TXPWR_LMT_8192F
#if IS_RTL8192F_SERIES
		if ( IS_HARDWARE_TYPE_8192F(Adapter) ) {
			if(HAL_RFE_TYPE==3){
				printk("[%s][TXPWR_LMT_92FE_type3]\n",__func__);
				rfelog("[%s][TXPWR_LMT_92FE_type3]\n",__func__);
				*((dma_addr_t*)(val)) = (dma_addr_t)data_TXPWR_LMT_92FE_type3_start;
			}else{
				printk("[%s][TXPWR_LMT_92FE_new]\n",__func__);
				*((dma_addr_t*)(val)) = (dma_addr_t)data_TXPWR_LMT_92FE_new_start;
			}
		}
#endif
#endif
    ...
    if ( IS_HARDWARE_TYPE_8197G(Adapter) ) {
		...
		if ( IS_HARDWARE_TYPE_8192F(Adapter) ) {
			if(HAL_RFE_TYPE==3){
				printk("[%s][TXPWR_LMT_92FE_type3] size\n",__func__);
				*((pu4Byte)(val)) = (u4Byte)(data_TXPWR_LMT_92FE_type3_end - data_TXPWR_LMT_92FE_type3_start);
			}else{
				printk("[%s][TXPWR_LMT_92FE_new] size\n",__func__);
				*((pu4Byte)(val)) = (u4Byte)(data_TXPWR_LMT_92FE_new_end - data_TXPWR_LMT_92FE_new_start);
			}
		}		
#endif
#endif
...
#if IS_RTL88XX_MAC_V1_V2
		if (_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1_v2)	{
			*((pu2Byte)(val)) = platform_efio_read_1byte(Adapter, REG_HIQ_NO_LMT_EN);
		}
#endif
	}
	break;
	...
	if (_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1_v2)  {
    		...
    		*((pu2Byte)(val)) = platform_efio_read_1byte(Adapter, REG_MBSSID_CTRL);
        }
#endif //IS_RTL88XX_MAC_V1_V2
#if IS_RTL88XX_MAC_V3
        if (_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v3) {
...
)
{
	...
	HALMAC_WLAN_ADDR halmac_mac_addr;
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)Adapter;
#endif
	i = 0;
	...
	if (Adapter->hci_type == RTL_HCI_SDIO) {
		...
		if(priv->pshare->rf_ft_var.manual_pe_enable)
		{
			...
			if(!ret)
				panic_printk("%s offload start fail, timeout = %d\r\n", priv->dev->name, timeout);
			else
	...
	if ((&(Adapter->pshare->_dmODM))->la_mode)
        ...
        HAL_memcpy(halmac_mac_addr.addr, Adapter->dev->dev_addr, ETH_ALEN);
        if (HALMAC_RET_SUCCESS != GET_MACHALAPI_INTERFACE(Adapter)->halmac_cfg_mac_addr(Adapter->pHalmac_adapter, 0, &halmac_mac_addr) ||
            HALMAC_RET_SUCCESS != GET_MACHALAPI_INTERFACE(Adapter)->halmac_cfg_operation_mode(Adapter->pHalmac_adapter,HALMAC_WIRELESS_MODE_AC)) 
        {
            panic_printk("halmac_init_mac_cfg Failed\n");
            return RT_STATUS_FAILURE;            
...
GET_MACHALAPI_INTERFACE(Adapter)->halmac_cfg_drv_info(Adapter->pHalmac_adapter,HALMAC_DRV_INFO_PHY_STATUS);
	...
	if ((IS_HARDWARE_TYPE_8822B(priv)) || (IS_HARDWARE_TYPE_8821C(priv)) || (IS_HARDWARE_TYPE_8814B(priv)) || (IS_HARDWARE_TYPE_8822C(priv)) || (IS_HARDWARE_TYPE_8812F(priv))) {
	...
	if (Adapter->hci_type == RTL_HCI_SDIO) {
		...
		if (Adapter->hci_type == RTL_HCI_SDIO) {
			...
			if (IS_HARDWARE_TYPE_8814B(Adapter)) { //yllin8814B
				...
				/* v44.54 / Report18b: wlan0 can receive AUTH while the 8822B
				 * management streaming-DMA map count grows without matching
				 * unmaps.  The generic IMR intentionally omits per-TX-queue
				 * completion bits.  Enable them only for MW5 radio0 so Linux
				 * can promptly reclaim MGT and the HI0 critical-response ring. */
				if (GET_CHIP_VER(priv) == VERSION_8822B &&
				    priv->pshare->linux6_radio_index == 0) {
					GET_HAL_INTERFACE(priv)->AddInterruptMaskHandler(priv, HAL_INT_TYPE_MGNTDOK);
					GET_HAL_INTERFACE(priv)->AddInterruptMaskHandler(priv, HAL_INT_TYPE_HIGHDOK);
					pr_info("rtl8197f-wlan: v44.54 MW5 8822B TX completion IRQs enabled if=%s MGT+HI0\n",
...
#endif
#ifdef RTK_TDMA_ATM
				GET_HAL_INTERFACE(priv)->AddInterruptMaskHandler(priv, HAL_INT_TYPE_GTIMER3);
#endif

#endif

#if 0 // Flow Control
				if (priv->pshare->rf_ft_var.flowctrl) {
					GET_HAL_INTERFACE(priv)->AddInterruptMaskHandler(priv, HAL_INT_TXDMA_START);
					GET_HAL_INTERFACE(priv)->AddInterruptMaskHandler(priv, HAL_INT_TXDMA_STOP);
					RTL_W8(REG_TXDMA_STOP_HIMR, _GET_HAL_DATA(priv)->IntMask[9]);
					RTL_W8(REG_TXDMA_START_HIMR, _GET_HAL_DATA(priv)->IntMask[10]);
				} else {
					RTL_W32(REG_TXDMA_STOP_HIMR, 0);
					RTL_W32(REG_TXDMA_START_HIMR, 0);
				}
#endif
...
#if defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE) && (IS_RTL8197F_SERIES || IS_RTL8822B_SERIES)
	/* v44.31.1: MAC-v2 initializes REG_RCR without the top-level AMF/ADF
	 * acceptance gates even though RXFLTMAP0/2 later enables the individual
	 * management/data subtypes.  On MW5 this produced real RX DMA/NAPI on
	 * RTL8822B but no beacons/auth/assoc, and an empty RTL8197FS RX ring.
	 * Keep the vendor subtype maps and open the missing master gates. */
	if (IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter)) {
		/* v44.37: AAP is required on MW5 because the 8822B trace proves
		 * AUTH/ASSOC RX but loses the client's EAPOL M2 before hostapd.
		 * auth_filter() still drops every non-EAPOL frame while the station
		 * controlled port is unauthorized, so this does not bypass WPA. */
		u4Byte mw5_rcr = HAL_RTL_R32(REG_RCR) | BIT(13) | BIT(11) | BIT(0);
		HAL_RTL_W32(REG_RCR, mw5_rcr);
#ifdef CONFIG_RTL8192CD_LINUX6_BUS
		if (Adapter->pshare)
			Adapter->pshare->linux6_rcr_after_init = mw5_rcr;
#endif
		printk(KERN_INFO "rtl8197f-wlan: v44.37 MW5 RCR accept management+data+all-unicast enabled rcr=0x%08x\n",
		       mw5_rcr);
	}
#endif
...
)
{
	...
	//	| BIT_ACRC32 | BIT_AB | BIT_AM | BIT_APM | BIT_AAP);
 	    ...
 	    HAL_RTL_W32(REG_RCR, BIT_APP_FCS | BIT_APP_MIC | BIT_APP_ICV | BIT_APP_PHYSTS | BIT_VHT_DACK |  BIT_ACRC32 | BIT_AB | BIT_AM | BIT_APM);
...
)
{
    ...
    #endif
    #ifdef RTK_TDMA_ATM
    GET_HAL_INTERFACE(Adapter)->AddInterruptMaskHandler(Adapter, HAL_INT_TYPE_GTIMER3);
    #endif
	// TODO: Filen, no need to sync !?
...
)
{
    ...
    u4Byte  bit_hisr1_ind_int=0, bit_bedok=0;
    ...
    if(Adapter->pshare->rf_ft_var.manual_pe_enable){
        pHalData->IntMask[0]        = bit_hisr1_ind_int | BIT_RDU;
    }else
#endif
    {
...
#endif
    {
		...
		if(GET_CHIP_VER(Adapter) == VERSION_8812F)
			pHalData->IntMask[1] |= BIT_TXFOVW | BIT_FOVW;
    } 
#ifdef TXFOVW_INT_EN
...
)
{
	...
	if (HAL_IS_PCIBIOS_TYPE(Adapter)) {
		...
		rtl8197f_wlan_phase_set(137, "hci-dma-desc-allocation-entry", 0);
		pr_info("rtl8197f-wlan: v43.46 InitHCIDMAMem88XX PCIBIOS branch type=%u pdev=%p netdev=%s\n",
			(unsigned int)((Adapter->pshare->type >> TYPE_SHIFT) & TYPE_MASK),
			Adapter->pshare->pdev, Adapter->dev ? Adapter->dev->name : "none");
#endif
		...
		if (IS_HARDWARE_TYPE_8814B(Adapter))
...
#if defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
			rtl8197f_wlan_phase_set(138, "hci-dma-desc-allocated", 0);
#endif
			...
			printk("DESC: cpu_addr %p, dma_handle %p, size %d\n", cpu_addr_desc, (void *)(unsigned long)dma_handle_desc, dma_len);
		...
		if (IS_HARDWARE_TYPE_8814B(Adapter))
			dma_len_amsdu = DESC_DMA_PAGE_SIZE_HAL_FOR_AMSDU_8814B;
		else
#endif
		...
		printk("AMSDU: cpu_addr %p, dma_handle %pad, size %d\n", cpu_addr_amsdu, &dma_handle_amsdu, dma_len_amsdu);
...
{
	u4Byte      camData[2]={0}, camIdx=0;
    u4Byte      mask_MBIDCAM_addr=0;
    u4Byte      shift_MBIDCAM_addr=0;
    u4Byte      bit_MBIDCAM_valid=0;
...
#endif  // #if IS_RTL8197F_SERIES    
#if (IS_RTL8192E_SERIES || IS_RTL8192F_SERIES || IS_RTL8881A_SERIES || IS_RTL8814A_SERIES || IS_RTL8822B_SERIES || IS_RTL8822C_SERIES || IS_RTL8812F_SERIES)
    if( IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8881A(Adapter) ||IS_HARDWARE_TYPE_8814A(Adapter) ||IS_HARDWARE_TYPE_8822B(Adapter) 
		|| IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {
...
)
{
	...
	/*gave default to avoid coverity warming*/
	mask_MBIDCAM_addr = 0;
	shift_MBIDCAM_addr = 0;
...
#endif  // #if IS_RTL8197F_SERIES    
#if (IS_RTL8192E_SERIES || IS_RTL8192F_SERIES || IS_RTL8881A_SERIES || IS_RTL8814A_SERIES || IS_RTL8822B_SERIES || IS_RTL8192F_SERIES)
...
)
{
...
#endif  // #if IS_RTL8197F_SERIES    
#if (IS_RTL8192E_SERIES || IS_RTL8881A_SERIES || IS_RTL8814A_SERIES || IS_RTL8822B_SERIES || IS_RTL8192F_SERIES)
    if( IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8881A(Adapter) ||IS_HARDWARE_TYPE_8814A(Adapter) ||IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter))
    {
        mask_MBIDCAM_addr = BIT_MASK_MBIDCAM_ADDR;
        shift_MBIDCAM_addr = BIT_SHIFT_MBIDCAM_ADDR;
    }
	...
	if (HAL_IS_ROOT_INTERFACE(Adapter)) {
...
#ifdef RTK_NL80211
					_rtl_net80211_setkey(tmpAdapter->dev, NULL, &tmpAdapter->group_key_cache, NULL);
#else
        ...
        if (_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v2) {  
					...
					tempVal = ((HAL_VAR_BCN_INTERVAL / ((HAL_RTL_R8(REG_MBID_NUM) & 0x07))));
        ...
        else if (HAL_IS_VAP_INTERFACE(Adapter) && (HAL_VAR_VAP_INIT_SEQ >= 0))
        {
            ...
            if (HAL_OPMODE & WIFI_AP_STATE)
            {
                ...
                if (HAL_RTL_R8(REG_MBID_NUM)& BITS_MBID_BCN_NUM_V2 & ~mask_MBID_BCN_NUM)
                {
					...
					HAL_RTL_W16(REG_MBSSID_BCN_SPACE,
                        (HAL_VAR_BCN_INTERVAL & BIT_MASK_BCN_SPACE0) << BIT_SHIFT_BCN_SPACE0);
					tempVal = ((HAL_VAR_BCN_INTERVAL / (((HAL_RTL_R8(REG_MBID_NUM) & 0xf0) >> 4))));
					HAL_RTL_W32(REG_MBSSID_BCN_SPACE3, (HAL_RTL_R32(REG_MBSSID_BCN_SPACE3) & 0xFF00FFFF) | BIT_SUB_BCN_SPACE(tempVal));
...
)
{
   // clear HW TX shortCut SRAM buffer
    int lenTXSC = 0;
    int baseReg = 0, start_addr=0, index=0, newidx=0, bndy0=0, bndy1 = 0, addr0=0;
...
{
    int lenTXSC = 0;
    int baseReg = 0, start_addr=0, shift, bndy0=0, bndy1 = 0, addr0=0, read_addr = 0;
...
#if ( HALMAC_8192F_SUPPORT ) || (HALMAC_8192E_SUPPORT || HALMAC_8881A_SUPPORT)
    u4Byte bit_dbgbuf,rw_sts,value;
    u1Byte dbgbuf_mode;
	u4Byte WaitWriteLimmit=0;

	/*gave default val to avoid coverity warming */
	bit_dbgbuf=0;
	dbgbuf_mode=0;
	rw_sts=0;
...
//	u4Byte  data[2];
    ...
    if(_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v4){
        ...
        reg_addr = REG_MACID_RELEASE_INFO;
    }
#endif
...
)
{
	...
	u4Byte  i = 0;
#endif	
#if IS_EXIST_RTL8192FE || IS_EXIST_RTL8192EE
	u4Byte  cnt = 0;
#endif
#if IS_RTL88XX_MAC_V2 || IS_EXIST_RTL8192FE
	u4Byte  data[2];
#endif	
...
)
{
    ...
    u4Byte      mask_MBIDCAM_addr;
    u4Byte      shift_MBIDCAM_addr;
#if (IS_RTL8197G_SERIES || IS_RTL8198F_SERIES || IS_RTL8197F_SERIES || IS_RTL8822C_SERIES || IS_RTL8812F_SERIES || IS_RTL8192E_SERIES || IS_RTL8881A_SERIES || IS_RTL8814A_SERIES || IS_RTL8822B_SERIES || IS_RTL8192F_SERIES)
    u4Byte      bit_MBIDCAM_valid;
#endif
	// clear the rest area of CAM
	/*gave default val*/
...
)
{
	...
	pHalData->IntMask[0] = 0;
	pHalData->IntMask[1] = 0;
...
)
{
	...
	u4Byte value32;
#endif
#if (IS_RTL8192E_SERIES || IS_RTL8822B_SERIES || IS_RTL8192F_SERIES || IS_RTL8812F_SERIES || IS_RTL8814A_SERIES)
	u1Byte stateVal;
#endif
#if IS_RTL8881A_SERIES
	if ( IS_HARDWARE_TYPE_8881A(Adapter) ) {
...
#if (IS_RTL8192E_SERIES || IS_RTL8822B_SERIES || IS_RTL8192F_SERIES || IS_RTL8812F_SERIES)
	if (IS_HARDWARE_TYPE_8192EE(Adapter) || IS_HARDWARE_TYPE_8822BE(Adapter) || 
		IS_HARDWARE_TYPE_8192FE(Adapter) || IS_HARDWARE_TYPE_8812FE(Adapter)) {
