...
/* v44.45.1: RTL8197F OEM semantics require this callback to be a no-op,
 * but Linux 6.6/GCC 13 requires an exact NicPHYUpdateBBRFValHandler
 * prototype. HalGeneralDummy only takes Adapter and therefore cannot be
 * assigned without an incompatible-function-pointer error. */
static VOID
Hal8197FNoopPHYUpdateBBRFVal(
...
)
{
    ...
    if (Adapter->pshare) {
        Adapter->pshare->linux6_pwrseq_a0 = HAL_RTL_R8(0x00a0);
        Adapter->pshare->linux6_pwrseq_a1 = HAL_RTL_R8(0x00a1);
        Adapter->pshare->linux6_pwrseq_a3 = HAL_RTL_R8(0x00a3);
        Adapter->pshare->linux6_pwrseq_05 = HAL_RTL_R8(0x0005);
        Adapter->pshare->linux6_pwrseq_06 = HAL_RTL_R8(0x0006);
        Adapter->pshare->linux6_mw5_binary_pwrseq = 1;
        printk(KERN_INFO "rtl8197f-wlan: v44.45.1 MW5 binary power-on flow a0=%02x a1=%02x a3=%02x r05=%02x r06=%02x\n",
               Adapter->pshare->linux6_pwrseq_a0, Adapter->pshare->linux6_pwrseq_a1,
               Adapter->pshare->linux6_pwrseq_a3, Adapter->pshare->linux6_pwrseq_05,
               Adapter->pshare->linux6_pwrseq_06);
    }
#endif
...
)
{
    ...
    /* v44.65.16: rtl8192cd_open() may take the surprise-reset path after
     * cfg80211/hostapd stop->start.  The 8197F table historically left this
     * callback NULL even though the generic 88XX implementation is valid for
     * this MAC generation, which caused EPC=0 on reopen. */
...
//    pHalFunc->ReleaseShortCutTxDescHandler  = ReleaseShortCutTxDesc88XX;
    ...
    /* v44.45.1/MW5: recovered RTL8197F SDK/binary uses no generic BBRF update
     * callback. Channel programming is performed by the SDK RF18 path. */
    pHalFunc->PHYUpdateBBRFValHandler       = Hal8197FNoopPHYUpdateBBRFVal;
    ...
    pHalFunc->SetATMReportCfgHandler        = SetATMReportCfg88XX;
