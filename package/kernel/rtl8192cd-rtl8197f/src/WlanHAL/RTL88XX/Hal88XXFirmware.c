...
#ifdef WLAN_RSV_BUF
	if(GET_MAX(dmemSZ, imemSZ) > MAX_RSV_BUF_SIZE) {
		fwBufPtr = kmalloc(GET_MAX(dmemSZ, imemSZ), GFP_KERNEL);
		tofree = 1;
	}
	else {
		fwBufPtr = priv->pshare->rsv_buf;
	}

	if(fwBufPtr != NULL) {
		memset(fwBufPtr, 0, GET_MAX(dmemSZ, imemSZ));
	}
	else {
		RT_TRACE_F(COMP_INIT, DBG_WARNING, ("Allocate FW buffer error ! \n"));
		return RT_STATUS_FAILURE;
	}
#elif (defined(CONFIG_NET_PCI) && defined(NOT_RTK_BSP)) || defined(CONFIG_RTL8192CD_MODULE)
    fwBufPtr = kmalloc(GET_MAX(dmemSZ, imemSZ), GFP_KERNEL);
    if (NULL == fwBufPtr) {
        RT_TRACE_F(COMP_INIT, DBG_WARNING, ("Allocate FW buffer error ! \n"));
        return RT_STATUS_FAILURE;
...
out:
	...
	if(tofree)
		...
		RT_TRACE_F(COMP_INIT, DBG_WARNING, ("Skip the free process for fwBufPtr.\n"));
#elif (defined(CONFIG_NET_PCI) && defined(NOT_RTK_BSP)) || defined(CONFIG_RTL8192CD_MODULE)
    HAL_free(fwBufPtr);
#endif
...
{	
	...
	BOOLEAN	bShortGI20MHz = FALSE,bShortGI40MHz = FALSE;
#if CFG_HAL_RTK_AC_SUPPORT
	BOOLEAN	bShortGI80MHz = FALSE;
#endif
	...
	)
{
	...
	u1Byte		        WirelessMode    = WIRELESS_MODE_A;
	u1Byte		        BW              = CHANNEL_WIDTH_20;
	u1Byte			MimoPs          = MIMO_PS_NOLIMIT, MimoPs_enable = FALSE, H2CCommand[7] ={ 0};
#if 0
	u1Byte			ratr_index = 8
#endif
	...
	struct stat_info    *pstat          = pEntry;
	u1Byte              rf_mimo_mode    = get_rf_mimo_mode(Adapter);
	u1Byte		sta_mimo_mode = 0;
#if (CFG_HAL_RTK_AC_SUPPORT)|| defined(RTK_AC_SUPPORT)
	...
	)
{
	...
	if (Adapter && Adapter->pshare && Adapter->pshare->linux6_radio_index <= 1)
		pr_info_ratelimited("rtl8197f-wlan: v44.52 MW5 MSRRPT if=%s radio=%u op=%u aid=%u macid=%u\n",
			Adapter->dev ? Adapter->dev->name : "none",
			(unsigned int)Adapter->pshare->linux6_radio_index,
			(unsigned int)(opmode & 0x01),
			(unsigned int)pEntry->cmn_info.aid,
			(unsigned int)(REMAP_AID(pEntry) & 0xff));
#endif
...
)
{
    ...
    u1Byte macid_pause_1 = 0;
    u1Byte macid_pause_2 = 0;
    u1Byte macid_en_1 = 0;
    u1Byte macid_en_2 = 0;
    u1Byte macid_en;
...
static WLAN_TXDUTY_CFG Rate2TxDutyTable[] =
{
    data_8814B_TxDuty
};
...
(
    IN      HAL_PADAPTER        Adapter,
    IN      PWLAN_TXDUTY_CFG    TxDutyArray
...
void TxDutyCycleLevel88XX(
    IN HAL_PADAPTER     Adapter,
    IN u1Byte           level /* 0 ~ 9 */
)
{
	WLAN_TXDUTY_CFG txduty_table[HALMAC_VHT_NSS4_MCS9+2];
	int i;

	for (i=0; i<=HALMAC_VHT_NSS4_MCS9; i++) {
		txduty_table[i].valid = Rate2TxDutyTable[i].valid;
		txduty_table[i].rate = Rate2TxDutyTable[i].rate;
		if (Rate2TxDutyTable[i].duty > level * 10)
			txduty_table[i].duty = Rate2TxDutyTable[i].duty - level * 10;
		else
			txduty_table[i].duty = 5;
	}
	memset(&txduty_table[i], 0xff, 3);

    HalTxDutyParsing88XX(Adapter, txduty_table);
    GET_HAL_INTERFACE(Adapter)->SetPWRReduceHandler(Adapter, 1, 0, 0, 84, 4);
}


void TxDutyCycleDump88XX(
    IN HAL_PADAPTER     Adapter
)
{
	u32	DlRsvdAddr,page,addr;
	...
	printk("  Valid Rate  Table FW    Addr\n");
	for (i=0; i<sizeof(Rate2TxDutyTable)/sizeof(WLAN_TXDUTY_CFG); i++)
	{
		printk("  %-5d %-5d %-5d %-5d %x\n", Rate2TxDutyTable[i].valid,
										Rate2TxDutyTable[i].rate,
										Rate2TxDutyTable[i].duty,
										HAL_RTL_R8(0x8000+(addr&0xfff)),
										0x8000+(addr&0xfff));
		addr += 2;
	}
}
...
)
{
	...
	while(1)
	{
		...
		if (Adapter->pshare->h2c_box_flag == 0) {
			Adapter->pshare->h2c_box_flag = 1;
		...
		if (i >= 50000) {
			printk("FillH2CCmd88XX : can't get h2c_box_flag\n");
			return RT_STATUS_FAILURE;
		}
	...
	while(!bFwReadClear)
	{
		...
		if(WaitH2cLimmit == 10000) { //do the first stage, clear last cmd
			...
			printk("H2C cmd-TO, final stage!! REG_HMETFR:0x%x, BoxNum:%d\n", HAL_RTL_R8(REG_HMETFR), BoxNum);
                        printk("ElementID=%x, len=%x\n", ElementID, CmdLen);
			Adapter->pshare->h2c_box_full++;
			spin_lock_irqsave(&Adapter->h2c_lock, flags);
			Adapter->pshare->h2c_box_flag = 0;
			spin_unlock_irqrestore(&Adapter->h2c_lock, flags);
			return RT_STATUS_FAILURE;
		}
...
)
{
    ...
    case 0x34:
        printk("MU_RPT: macid %d avg_agg %d ok %d ng %d total %d tp %d\n",
        		tmpBuf[0], tmpBuf[1],
        		((tmpBuf[2]<<8)|tmpBuf[3]), ((tmpBuf[4]<<8)|tmpBuf[5]),
        		((tmpBuf[6]<<8)|tmpBuf[7]), ((tmpBuf[8]<<8)|tmpBuf[9]));
        break;
...
{
#if defined(RTK_AC_SUPPORT) || defined(CONFIG_WLAN_HAL_8197F)|| defined(CONFIG_WLAN_HAL_8198F)
HAL_PADAPTER    Adapter = priv;
#endif
...
)
{
	...
	if (interval) {
		H2CCommand[0] = 5; /* [7:1] type [0] en */
		H2CCommand[1] = interval;
	} else {
		H2CCommand[0] = 4;
		H2CCommand[1] = 0;
	}

	printk("request mu report: %02x %02x\n", H2CCommand[0], H2CCommand[1]);
	GET_HAL_INTERFACE(priv)->fill_h2c_cmd_handler(priv, 0xb9, 2, H2CCommand);
}

const struct mu_group_st mu_group[28] = {
...
const u1Byte group_ra_map[28][4] = {
...
int
find_mu_rate_idx(int primary, int secondary)
{
    ...
    for (i = 0; i < 28; i++) {
        ...
        if (group->user_num == 2 &&
                group->user[0] == primary &&
                group->user[1] == secondary)
            return group_ra_map[i][0];
    }
    return -1;
}
...
)
{
	...
	unsigned int tx_rate_all = 0;
  unsigned char tx_rate_zero = 0;
	...
	for (i = 0; i < group->user_num; i++) {
		...
		if ((priv->pshare->rf_ft_var.mu_group_mode == 1) && (group->user_num > 2)) {
            j = (group->user_num==(i+1)) ? 0 : (i+1);
            pEntry2 = CandidateEntry[group->user[j]];
            pstat2 = pEntry2->pSTA;

            pstat->muPartner[pstat->muPartner_num++] = pstat2;
        } else if ((priv->pshare->rf_ft_var.mu_group_mode == 2) && (group->user_num == 3)) {
        	//TODO
        	if (group->user[0] == 3) { /* 312 -> 1+1+1 */
	...
	for (i = 0; i < group->user_num; i++) {
		...
		if ((priv->pshare->rf_ft_var.mu_group_mode == 1) && (group->user_num > 2)) {
            j = (group->user_num==(i+1)) ? 0 : (i+1);
            pEntry->mu_rate_index = find_mu_rate_idx(group->user[i], group->user[j]);
            if (pEntry->mu_rate_index < 0)
                pEntry->mu_rate_index = group_ra_map[group_index][i];
        } else if ((priv->pshare->rf_ft_var.mu_group_mode == 2) && (group->user_num == 3)) {
        	//TODO
			if (group->user[0] == 3) {
				pEntry->mu_rate_index = group_ra_map[group_index][i];
            ...
            pEntry->mu_rate_index = group_ra_map[group_index][i];
		pstat->mu_rate = ODMPTR->dm_ra_table.mu1_rate[pEntry->mu_rate_index];
		pstat->mu_tx_rate = query_mu_vht_rate(pstat);
		
		if (!tx_rate_zero) {
            if (pstat->mu_tx_rate != 0)
                tx_rate_all += pstat->mu_tx_rate;
            else
                tx_rate_zero = 1;
    	}
	}
	...
	for (i = 0; i < group->user_num; i++) {
        ...
        if (tx_rate_zero)
            ...
            pstat->mu_deq_num = dqnum * group->user_num * (pstat->mu_tx_rate) / tx_rate_all;
  }
}

VOID
beamform_mu_match_m6_8814b(
	struct rtl8192cd_priv *priv,
	u4Byte bitmap
)
{
	...
	unsigned int dqnum = priv->pshare->rf_ft_var.dqnum;
	unsigned char flags = 0;
	int pri, sec, idx, found;
	u1Byte match_sta[4] = {0xff, 0xff, 0xff, 0xff};
	struct mu_group_st *group;
	unsigned int tx_rate_all = 0;
	unsigned char tx_rate_zero = 0;
	...
	for (pri=0; pri<4; pri++) {
		...
		pEntry = CandidateEntry[pri];
		pstat = pEntry->pSTA;
		match_sta[pri] = i;
        group = &mu_group[i];

		for (j=1; j<group->user_num; j++) {
			pEntry2 = CandidateEntry[group->user[j]];
			pstat2 = pEntry2->pSTA;
	...
	for (pri=0; pri<4; pri++) {
		...
		pstat->muFlagForAMSDU = 1;
		pstat->mu_deq_num = dqnum;
        pEntry->mu_rate_index = group_ra_map[match_sta[pri]][0];
		pstat->mu_rate = ODMPTR->dm_ra_table.mu1_rate[pEntry->mu_rate_index];
		pstat->mu_tx_rate = query_mu_vht_rate(pstat);
...
#define ARR_LEN(arr) (sizeof(arr)/sizeof(arr[0]))
	...
	for(i = 0; i < BEAMFORMEE_ENTRY_NUM; i++) {
		...
		if( pEntry->bUsed && (pEntry->is_mu_sta == TXBF_TYPE_MU) && pEntry->mu_reg_index < MAX_NUM_BEAMFORMEE_MU) {	
			...
			is_mu_group = 1;
			if(priv->pshare->rf_ft_var.mu_group_aid != 0xf)
				if(!(priv->pshare->rf_ft_var.mu_group_aid & BIT(pstat->cmn_info.aid-1)))
					is_mu_group = 0;
				
			if(pstat && is_mu_group) {
...
if ((priv->pshare->rf_ft_var.mu_group_mode == 6) || (priv->pshare->rf_ft_var.mu_group_mode == 4)) {
	int pri, sec;
	u1Byte match_sta;

	if (priv->pshare->rf_ft_var.mu_group_mode == 4)
		*bitmap = priv->pshare->rf_ft_var.manual_group;
	...
	for (pri=0; pri<4; pri++) {
		...
		if (match_sta != 0xff) {
			new_bitmap |= BIT(match_sta);
			selected_sta_map |= BIT(pri);
		}
	}
	
	beamform_mu_match_m6_8814b(priv, new_bitmap);
} else {
	...
	if ((priv->pshare->rf_ft_var.mu_group_mode == 1) && (match_4sta || match_3sta)) {
        group = &mu_group[selected_group];
        for (i = 0; i < group->user_num; i++) {
            int primary = group->user[i];
            int secondary = group->user[(group->user_num==(i+1))?0:(i+1)];
            for (j = 0; j < ARR_LEN(mu_group); j++) {
                if (mu_group[j].user_num == 2 &&
                        mu_group[j].user[0] == primary &&
                        mu_group[j].user[1] == secondary) {
                    new_bitmap |= BIT(j);
                }
            }
        }
    } else if ((priv->pshare->rf_ft_var.mu_group_mode == 2) && (match_3sta)) {
    	//TODO
		if (group->user[0] == 3) {
	        group = &mu_group[selected_group];
	        for (i = 0; i < group->user_num; i++) {
...
unsigned int mu_bitmap_in[16], mu_bitmap_out[16];
int mu_bitmap_index=0;
...
)
{
	...
	mu_bitmap_in[mu_bitmap_index] = bitmap;
	...
	if (priv->pshare->rf_ft_var.mu_group_mode == 5)
		bitmap = in_bitmap;
	/* send h2c to notify wifi fw */
	request_mu_group(priv, bitmap);
	/* mu_group_mode
		0:
		1: SC - 1+1 1+1 1+1, 12 23 31
		2: sky - mode0 + mode1, 12 21 312 (incomplete)
		3: Sean - PE dequeue 12 21 312
		4: sky - manual_group
		5: sky - 2sta > 3sta > 4sta
		6: sky - 4sta > 3sta > 2sta, also fix group selection rule
	*/
	mu_bitmap_out[mu_bitmap_index] = bitmap;
	mu_bitmap_index = (mu_bitmap_index + 1) & 0xf;
