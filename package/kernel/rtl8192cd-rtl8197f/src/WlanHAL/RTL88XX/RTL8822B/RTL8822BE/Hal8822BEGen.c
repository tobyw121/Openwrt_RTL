...
/*
 * Keep the legacy 8822B HAL behaviour, but adapt old one-argument/dummy
 * callbacks to the current HAL interface types.  These wrappers do not
 * change descriptor/RF/firmware algorithms.
 */
static enum rt_status InitPON8822BEHalmac(
    HAL_PADAPTER Adapter, u4Byte ClkSel)
{
    (void)ClkSel;
    return InitPON88XX(Adapter);
}

#if CFG_HAL_TX_SHORTCUT
static BOOLEAN FillShortCutTxHwCtrl8822BE(
    HAL_PADAPTER Adapter, u4Byte queueIndex, PVOID pDescData,
    PVOID pTxDesc, u4Byte direction, u4Byte useHW)
{
    return FillShortCutTxHwCtrl88XX(Adapter, queueIndex, pDescData, pTxDesc,
                                    direction, (BOOLEAN)useHW);
}
#endif

static BOOLEAN DownloadRsvdPage8822BEDummy(
    HAL_PADAPTER Adapter, pu4Byte beaconbuf, u4Byte beaconPktLen,
    u1Byte bReDownload)
{
    (void)Adapter;
    (void)beaconbuf;
...
static void SetATMReportCfg8822BEDummy(
    HAL_PADAPTER Adapter, u1Byte bEn, u1Byte EnRanges, u1Byte ATM_period,
    pu1Byte STAMACIDs, pu1Byte STALengths, u1Byte numOfSTA)
{
    (void)Adapter;
    (void)bEn;
    (void)EnRanges;
...
)
{
    ...
    pHalFunc->InitPONHandler        = InitPON8822BEHalmac;
...
//    pHalFunc->ReleaseShortCutTxDescHandler  = ReleaseShortCutTxDesc88XX;
    ...
    pHalFunc->FillShortCutTxHwCtrlHandler   = FillShortCutTxHwCtrl8822BE;
    ...
    pHalFunc->DownloadRsvdPageHandler	    = DownloadRsvdPage8822BEDummy;
...
#if CFG_HAL_ATM_REPORT        
    pHalFunc->SetATMReportCfgHandler        = SetATMReportCfg8822BEDummy;
	...
	)
{
	...
	odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8822B, (u1bTmp|BIT6)); //eric-mu
	...
	odm_set_bb_reg(p_dm_odm, 0x19E0 , BIT5|BIT4, 3); /*8822B grouping method*/

	if (p_dm_odm->rf_type == RF_2T2R) { /*2T2R*/
		PHYDM_DBG(p_dm_odm, DBG_TXBF, "%s: rf_type is 2T2R\n", __func__);
		config_phydm_trx_mode_8822b(p_dm_odm, 3, 3, 1);/*Tx2path*/
	}
}
	...
	)
{
	...
	if ((pBeamformingInfo->beamformer_su_cnt > 0) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		...
		if (pBeamformerEntry->su_reg_index == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(p_dm_odm, (REG_ASSOCIATED_BFMER0_INFO_8822B+i), pBeamformerEntry->MacAddr[i]);
		} else {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(p_dm_odm, (REG_ASSOCIATED_BFMER1_INFO_8822B+i), pBeamformerEntry->MacAddr[i]);
		}

		/*CSI report parameters of Beamformer*/
		Nc_index = halTxbf8822B_GetNrx(p_dm_odm);	/*for 8814A Nrx = 3(4 Ant), min=0(1 Ant)*/
	...
	if ((pBeamformingInfo->beamformee_su_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
			...
			pBeamformeeEntry->su_reg_index == 0xFF)
		{
	...
	#endif
			halTxbf8822B_RfMode(p_dm_odm, pBeamformingInfo, BFeeIdx);
			...
			if (pBeamformeeEntry->su_reg_index == 0) {	
				odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822B, STAid);	
				odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8822B+3, odm_read_1byte(p_dm_odm, REG_TXBF_CTRL_8822B+3)|BIT4|BIT6|BIT7);
			} else {
				odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822B+2, STAid | BIT14 | BIT15 | BIT12);
			}	

			/*CSI report parameters of Beamformee*/
			if (pBeamformeeEntry->su_reg_index == 0) {
				/*Get BIT24 & BIT25*/
				u1Byte	tmp = odm_read_1byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822B+3) & 0x3;
			
				odm_write_2byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822B, STAid | BIT9);
			} else		
				odm_write_2byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822B+2, STAid | 0x0200);	/*Set BIT25*/
	...
	if ((pBeamformingInfo->beamformer_mu_cnt > 0) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		...
		if(pBeamformerEntry->BeamformEntryCap & BEAMFORM_CAP_VHT_MU_BFEE)
		{
			...
			/* Set partial AID */
			odm_write_2byte(p_dm_odm, (REG_ASSOCIATED_BFMER0_INFO_8822B+6), pBeamformerEntry->P_AID);
			...
			u1bTmp = odm_read_1byte(p_dm_odm, REG_RXFLTMAP0_8822B+1);
			u1bTmp |= 0x40;
			odm_write_1byte(p_dm_odm, REG_RXFLTMAP0_8822B+1, u1bTmp);
			/* Set 0x6A2[5:4] = 1 to NDPA and BF report poll */
			u1bTmp = odm_read_1byte(p_dm_odm, REG_RXFLTMAP1_8822B);
			u1bTmp |= 0x30;
			odm_write_1byte(p_dm_odm, REG_RXFLTMAP1_8822B, u1bTmp);
		
			/*CSI report parameters of Beamformer*/
			Nc_index = halTxbf8822B_GetNrx(p_dm_odm);	/* Depend on RF type */
	...
	if ((pBeamformingInfo->beamformee_mu_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		...
		if((pBeamformeeEntry->BeamformEntryCap & BEAMFORM_CAP_VHT_MU_BFER) && pBeamformeeEntry->mu_reg_index == 0xFF)
		{
			...
			/* 0x42C[30] = 1 (0: from Tx desc, 1: from 0x45F) */
			u1bTmp = odm_read_1byte(p_dm_odm, REG_TXBF_CTRL_8822B+3);
			u1bTmp |= 0xD0; // Set bit 28, 30, 31 to 3b'111

			odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8822B+3, u1bTmp);

				if(AC_SIGMA_MODE == AC_SIGMA_APUT) {
					/* Set NDPA to VHT-1SS0 BW = 80M for 11ac logo r2*/
					odm_write_1byte(p_dm_odm, REG_NDPA_RATE_8822B, 0x2c); // VHT 1SS-0
					odm_write_1byte(p_dm_odm, REG_NDPA_OPT_CTRL_8822B, 0x12); //BW=80M
				}
				else
				{
					/* Set NDPA to 6M*/
					odm_write_1byte(p_dm_odm, REG_NDPA_RATE_8822B, 0x4); // 6M

					u1bTmp = odm_read_1byte(p_dm_odm, REG_NDPA_OPT_CTRL_8822B);
					u1bTmp &= 0xFC; // Clear bit 0, 1
					odm_write_1byte(p_dm_odm, REG_NDPA_OPT_CTRL_8822B, u1bTmp);
				}

			u4bTmp = odm_read_4byte(p_dm_odm, REG_SND_PTCL_CTRL_8822B);
			u4bTmp = ((u4bTmp & 0xFF0000FF) | 0x020200); // Set [23:8] to 0x0202
			odm_write_4byte(p_dm_odm, REG_SND_PTCL_CTRL_8822B, u4bTmp);	
	...
	)
{
	...
	if(pBeamformeeEntry && pBeamformeeEntry->BeamformEntryCap == BEAMFORMING_CAP_NONE) { //eric-txbf
		halTxbf8822B_RfMode(p_dm_odm, pBeamformingInfo, Idx);
#if (MU_BEAMFORMING_SUPPORT == 1)		
		...
		{ /*SU BFee*/
			if(pBeamformeeEntry->su_reg_index != 0xFF) { 
				...
				if (pBeamformeeEntry->su_reg_index == 0) {	
					...
					odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822B+2, 0x0 | BIT14 | BIT15 | BIT12);
	...
	)
{
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_SOUNDDOWN) {
		...
		tmpVal = odm_read_2byte(p_dm_odm, REG_TXBF_CTRL_8822B);
		odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822B, tmpVal|BIT15);
	} 
#if (MU_BEAMFORMING_SUPPORT == 1)	
	else {
