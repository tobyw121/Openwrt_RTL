...
)
{
	...
	int i=0;
	u4Byte reg_rxq_rxbd_idx=0, reg_rxq_rxbd_desa=0;
	...
	if (HAL_IS_PCIBIOS_TYPE(Adapter)) {
		PROC_PRINT("pRXBD_head=%08x, %08lx, reg(%x):%08x\n",
			(unsigned int)prx_dma->rx_queue[q_num].pRXBD_head , 
		...
		for (i=0;i<RX_Q_RXBD_NUM_IF(Adapter);i++ ){
			PROC_PRINT("pRXBD_head[%d], addr:%08x,%08lx: Dword0: 0x%x, Dword1: 0x%x\n", 
				i, 
				(unsigned int)&prx_dma->rx_queue[q_num].pRXBD_head[i],
...
#endif
	{
	    ...
	    for (i=0;i<RX_Q_RXBD_NUM_IF(Adapter);i++ ){
			PROC_PRINT("pRXBD_head[%d], addr:%08x,%08x: Dword0: 0x%x, Dword1: 0x%x\n", 
				i, 
				(unsigned int)&prx_dma->rx_queue[q_num].pRXBD_head[i],
				(unsigned int)HAL_VIRT_TO_BUS1(Adapter, (PVOID)&prx_dma->rx_queue[q_num].pRXBD_head[i],sizeof(RX_BUFFER_DESCRIPTOR), PCI_DMA_TODEVICE),
				GET_DESC(prx_dma->rx_queue[q_num].pRXBD_head[i].Dword0), 
                GET_DESC(prx_dma->rx_queue[q_num].pRXBD_head[i].Dword1)); 
	    }	
    }
...
)
{
...
#endif
    //printk("%s %d \n",__func__,__LINE__);
	/*gave default val(V1~V3) to avoid coverity warming */
	reg_rxq_rxbd_idx  = 0;
	reg_rxq_rxbd_desa = 0;
    ...
    if (HAL_IS_PCIBIOS_TYPE(Adapter)) {
        ...
        printk("%s(%d):size=%d, ring_dma_addr:%08lx, alloc_dma_buf:%08lx, ring_virt_addr:%08lx\n",
			...
			(unsigned long)_GET_HAL_DATA(Adapter)->ring_dma_addr, _GET_HAL_DATA(Adapter)->alloc_dma_buf,
...
#endif                 
        HAL_RTL_W32(RXBD_Reg[q_num], (unsigned int)(_GET_HAL_DATA(Adapter)->desc_phy_rxdma_buf));

//      HAL_RTL_W32(RXBD_Reg[q_num], HAL_VIRT_TO_BUS1(Adapter, (PVOID)_GET_HAL_DATA(Adapter)->rxbdBuf_phy_addr, sizeof(RX_BUFFER_DESCRIPTOR) * RXBD_NUM[q_num], HAL_PCI_DMA_TODEVICE);
...
)
{
    ...
    if (HAL_IS_PCIBIOS_TYPE(Adapter)) {
        ...
        pdesc_dma_buf = (pu1Byte)(_GET_HAL_DATA(Adapter)->ring_virt_addr);
        printk("%s(%d):size=%lu, alloc_dma_buf:%08lx, ring_virt_addr:%08lx, ring_dma_addr:%08lx\n",
			__FUNCTION__,__LINE__, DESC_DMA_PAGE_SIZE_MAX_HAL_IF(Adapter), _GET_HAL_DATA(Adapter)->alloc_dma_buf,
			_GET_HAL_DATA(Adapter)->ring_virt_addr, (unsigned long)_GET_HAL_DATA(Adapter)->ring_dma_addr);
...
#endif
#if defined(NEW_BUUFER_ALLOCATION) || defined(DMA_BUF_SEP)
  ...
  // Do nothing for un-cachable      
    ...
    for (q_num = 0; q_num < HCI_RX_DMA_QUEUE_MAX_NUM; q_num++)
    {
...
#ifdef CONCURRENT_MODE
            prx_dma->rx_queue[q_num].pRXBD_head = prx_dma->rx_queue[q_num-1].pRXBD_head + RXBD_NUM[idx][q_num-1];
#else
            prx_dma->rx_queue[q_num].pRXBD_head = prx_dma->rx_queue[q_num-1].pRXBD_head + RXBD_NUM[q_num-1];
#endif
        }
        ...
        if (IS_HARDWARE_TYPE_8197F(Adapter) &&
            (((Adapter->pshare->type >> TYPE_SHIFT) & TYPE_MASK) == TYPE_EMBEDDED)) {
            unsigned long cpu_off = (unsigned long)prx_dma->rx_queue[q_num].pRXBD_head -
                                    (unsigned long)_GET_HAL_DATA(Adapter)->desc_rxdma_buf;
            unsigned long ring_dma = (unsigned long)_GET_HAL_DATA(Adapter)->desc_phy_rxdma_buf + cpu_off;

            prx_dma->rx_queue[q_num].rxbd_dma_addr = ring_dma;
            Adapter->pshare->linux6_rxbd_dma_base = (u32)ring_dma;
            HAL_RTL_W32(RXBD_Reg[q_num], (u4Byte)ring_dma);
            Adapter->pshare->linux6_rxbd_hw_base = HAL_RTL_R32(RXBD_Reg[q_num]);
            printk(KERN_INFO "rtl8197f-wlan: v44.33 RTL8197FS RXBD coherent ring if=%s q=%u cpu=%p dma=0x%08lx hw=0x%08x off=%lu\n",
                   Adapter->dev ? Adapter->dev->name : "?", q_num,
                   prx_dma->rx_queue[q_num].pRXBD_head, ring_dma,
                   Adapter->pshare->linux6_rxbd_hw_base, cpu_off);
        } else
#endif
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    /* v44.32: InitRXDesc88XX already maps every MW5 RX payload with the
     * Linux DMA API and stores that device address in rx_infoL[].  The
     * vendor BSP branch below discarded it and regenerated the address
     * with virt_to_bus(), which is not a valid DMA-API contract.  Program
     * the RXBD with the exact address returned by dma_map_single(). */
    dma_addr = GET_HW(Adapter)->rx_infoL[rxbd_idx].paddr;
    /* v44.62.7: UpdateRXBDInfo88XX() is also called for every RXBD refill.
     * Do not printk here: rxbd_idx wraps to zero under traffic and a serial
     * console print in this path stalls the single-core MIPS receive path.
     */
#elif defined(CONFIG_NET_PCI) && defined(NOT_RTK_BSP)
    dma_addr = GET_HW(Adapter)->rx_infoL[rxbd_idx].paddr;
#else
...
)
{
    ...
    cur_q           = &(prx_dma->rx_queue[queueIndex]);
    prx_desc_status = (PRX_DESC_STATUS_88XX)pRxDescStatus;
    nonCacheAddr = (PRX_BUFFER_DESCRIPTOR)HAL_TO_NONCACHE_ADDR((dma_addr_t)(&(cur_q->pRXBD_head[cur_q->cur_host_idx])));
#ifdef CONFIG_NET_PCI
