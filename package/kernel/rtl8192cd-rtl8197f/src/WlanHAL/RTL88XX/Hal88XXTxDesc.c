...
/* v44.38: RTL8812BRH is TYPE_PCI_DIRECT, so it bypasses the vendor
 * IS_PCIBIOS_TYPE pci_map_single() path.  Report5 proves management TX/RX
 * works but WPA stalls immediately after association: EAPOL M1 is a data
 * frame and used the stale direct-bus payload path.  Use Linux streaming DMA
 * for every RTL8822B V1 traffic queue (MGT/AC/HI). Keep BCN/CMD and RTL8197FS on their
 * existing OEM paths. */
static inline int mw5_linux6_8822b_stream_queue(unsigned int queueIndex)
{
	/* RTL8822B uses the V1 layout: MGT, four AC queues and HI0..HI15
	 * are traffic queues; CMD and BCN have separate lifetime rules. */
	return queueIndex < HCI_TX_DMA_QUEUE_CMD_V1;
}

static unsigned long mw5_linux6_tx_map(HAL_PADAPTER Adapter, void *ptr,
				       unsigned int len, unsigned int queueIndex)
{
	dma_addr_t dma;
	unsigned int type;

	if (!ptr || !len || !Adapter || !Adapter->pshare ||
	    !Adapter->pshare->linux_dev)
		return 0;

	type = (Adapter->pshare->type >> TYPE_SHIFT) & TYPE_MASK;
	if (!IS_HARDWARE_TYPE_8822B(Adapter) || type != TYPE_PCI_DIRECT ||
	    !mw5_linux6_8822b_stream_queue(queueIndex))
		return HAL_VIRT_TO_BUS1(Adapter, ptr, len, HAL_PCI_DMA_TODEVICE);

	dma = dma_map_single(Adapter->pshare->linux_dev, ptr, len, DMA_TO_DEVICE);
	if (dma_mapping_error(Adapter->pshare->linux_dev, dma)) {
		Adapter->pshare->linux6_tx_dma_map_errors++;
		pr_err_ratelimited("rtl8197f-wlan: v44.38 MW5 TX DMA map failed if=%s q=%u len=%u\n",
			Adapter->dev ? Adapter->dev->name : "?", queueIndex, len);
		return 0;
	}

	Adapter->pshare->linux6_tx_dma_maps++;
	if (queueIndex == HCI_TX_DMA_QUEUE_MGT)
		Adapter->pshare->linux6_tx_dma_mgt_maps++;
	else
		Adapter->pshare->linux6_tx_dma_data_maps++;
	return (unsigned long)dma;
}

#define MW5_TX_PACKET_DMA(Adapter, ptr, len, q) \
	mw5_linux6_tx_map((Adapter), (void *)(ptr), (unsigned int)(len), (q))
#else
#define MW5_TX_PACKET_DMA(Adapter, ptr, len, q) \
	HAL_VIRT_TO_BUS1((Adapter), (PVOID)(ptr), (len), HAL_PCI_DMA_TODEVICE)
#endif
...
)
{
	...
	PROC_PRINT("total_txbd_num=%d,avail_txbd_num= %d,reg_rwptr_idx:%x\n",
		ptx_dma->tx_queue[q_num].total_txbd_num, rtl_atomic_read(&(ptx_dma->tx_queue[q_num].avail_txbd_num)), ptx_dma->tx_queue[q_num].reg_rwptr_idx);
		...
		for (i=0;i<pTXBD_NUM[q_num];i++ ){
			...
			PROC_PRINT("ptx_desc_head[%d], addr:%p,%08lx\n",
				...
				PROC_PRINT("%08x %08x %08x %08x  ", 
					...
					(u4Byte)GET_DESC(ptx_desc_head[i].Dword11));
				PROC_PRINT("%08x %08x %08x %08x\n", 
					(u4Byte)GET_DESC(ptx_desc_head[i].Dword12),
					(u4Byte)GET_DESC(ptx_desc_head[i].Dword13),
					(u4Byte)GET_DESC(ptx_desc_head[i].Dword14),
					(u4Byte)GET_DESC(ptx_desc_head[i].Dword15));
		}
	}else 
#endif
	{
...
#endif
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    unsigned long mw5_txbd_dma = 0, mw5_txdesc_dma = 0;
    int mw5_coherent_tx = 0;
#endif
...
// setting TXDESC size by chip
    ...
    printk("head:%p, ring_dma_addr:%08lx, size:%x\n", prx_dma->rx_queue[HCI_RX_DMA_QUEUE_MAX_NUM-1].pRXBD_head, 
        (unsigned long)_GET_HAL_DATA(Adapter)->ring_dma_addr, prx_dma->rx_queue[HCI_RX_DMA_QUEUE_MAX_NUM-1].total_rxbd_num * sizeof(RX_BUFFER_DESCRIPTOR));
 ...
 printk("ptxbd_bcn_head:%p, tmp_tx_dma_ring_addr3:%08lx,\n", ptxbd_bcn_head, tmp_tx_dma_ring_addr3);
    ...
    for (q_num = 0; q_num < HCI_TX_DMA_QUEUE_MAX_NUM; q_num++)
    {
        ...
        rtl_atomic_set(&(ptx_dma->tx_queue[q_num].avail_txbd_num), pTXBD_NUM[q_num]);
        ...
        if (q_num != HCI_TX_DMA_QUEUE_BCN) {
            ...
            for (i = 0; i < 1+HAL_NUM_VWLAN; i++)
            {
                ptxbd_bcn_cur = ptxbd + beacon_offset * i;
...
if(IS_SUPPORT_TX_AMSDU(Adapter)){
    ...
    ptx_dma_amsdu->tx_amsdu_queue[HCI_TX_AMSDU_DMA_QUEUE_BE].pTXBD_head_amsdu = (PTX_BUFFER_DESCRIPTOR_AMSDU)((pu1Byte)ptx_dma_amsdu->tx_amsdu_queue[HCI_TX_AMSDU_DMA_QUEUE_BK].pTXBD_head_amsdu + TX_BKQ_TXBD_NUM * sizeof(TXBD_ELEMENT) * MAX_NUM_OF_MSDU_IN_AMSDU);
    ptx_dma_amsdu->tx_amsdu_queue[HCI_TX_AMSDU_DMA_QUEUE_VI].pTXBD_head_amsdu = (PTX_BUFFER_DESCRIPTOR_AMSDU)((pu1Byte)ptx_dma_amsdu->tx_amsdu_queue[HCI_TX_AMSDU_DMA_QUEUE_BE].pTXBD_head_amsdu + TX_BEQ_TXBD_NUM * sizeof(TXBD_ELEMENT) * MAX_NUM_OF_MSDU_IN_AMSDU);
    ptx_dma_amsdu->tx_amsdu_queue[HCI_TX_AMSDU_DMA_QUEUE_VO].pTXBD_head_amsdu = (PTX_BUFFER_DESCRIPTOR_AMSDU)((pu1Byte)ptx_dma_amsdu->tx_amsdu_queue[HCI_TX_AMSDU_DMA_QUEUE_VI].pTXBD_head_amsdu + TX_VIQ_TXBD_NUM * sizeof(TXBD_ELEMENT) * MAX_NUM_OF_MSDU_IN_AMSDU);
...
original:
...
#ifdef DMA_BUF_SEP
#ifdef TRXBD_CACHABLE_REGION
	ptxbd_head      =  (PTX_BUFFER_DESCRIPTOR)_GET_HAL_DATA(Adapter)->desc_txbd_dma_buf;
	ptx_desc_head   =  (PTX_DESC_88XX)_GET_HAL_DATA(Adapter)->desc_txdesc_dma_buf;
	ptxbd_bcn_head  =  (PTX_BUFFER_DESCRIPTOR)_GET_HAL_DATA(Adapter)->desc_bcn_dma_buf;
#else
	ptxbd_head      =  (PTX_BUFFER_DESCRIPTOR)HAL_TO_NONCACHE_ADDR((unsigned long)_GET_HAL_DATA(Adapter)->desc_txbd_dma_buf);
	ptx_desc_head   =  (PTX_DESC_88XX)HAL_TO_NONCACHE_ADDR((unsigned long)_GET_HAL_DATA(Adapter)->desc_txdesc_dma_buf);
	ptxbd_bcn_head  =  (PTX_BUFFER_DESCRIPTOR)HAL_TO_NONCACHE_ADDR((unsigned long)_GET_HAL_DATA(Adapter)->desc_bcn_dma_buf);
#endif

	platform_zero_memory(ptxbd_head, _GET_HAL_DATA(Adapter)->desc_dma_txbd_buf_len);
	platform_zero_memory(ptx_desc_head, _GET_HAL_DATA(Adapter)->desc_dma_txdesc_buf_len);
	platform_zero_memory(ptxbd_bcn_head, _GET_HAL_DATA(Adapter)->desc_dma_bcn_buf_len);
#else
    ...
    for (q_num = 0; q_num < HCI_TX_DMA_QUEUE_MAX_NUM; q_num++)
    {
        ...
        if ((IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter)) &&
            q_num != HCI_TX_DMA_QUEUE_BCN &&
            _GET_HAL_DATA(Adapter)->alloc_desc_txbd_addr &&
            _GET_HAL_DATA(Adapter)->alloc_desc_txdesc_addr) {
            unsigned long txbd_off = (unsigned long)((pu1Byte)ptxbd -
                _GET_HAL_DATA(Adapter)->desc_txbd_dma_buf);
            unsigned long txdesc_off = (unsigned long)((pu1Byte)ptx_desc -
                _GET_HAL_DATA(Adapter)->desc_txdesc_dma_buf);

            mw5_txbd_dma = (unsigned long)_GET_HAL_DATA(Adapter)->desc_txbd_dma_addr + txbd_off;
            mw5_txdesc_dma = (unsigned long)_GET_HAL_DATA(Adapter)->desc_txdesc_dma_addr + txdesc_off;
            mw5_coherent_tx = 1;
        } else if ((IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter)) &&
                   q_num == HCI_TX_DMA_QUEUE_BCN &&
                   _GET_HAL_DATA(Adapter)->alloc_desc_bcn_addr) {
            unsigned long txbd_off = (unsigned long)((pu1Byte)ptxbd -
                _GET_HAL_DATA(Adapter)->desc_bcn_dma_buf);
            unsigned long txdesc_off = (unsigned long)((pu1Byte)ptx_desc -
                _GET_HAL_DATA(Adapter)->desc_bcn_dma_buf);

            mw5_txbd_dma = (unsigned long)_GET_HAL_DATA(Adapter)->desc_bcn_dma_addr + txbd_off;
            mw5_txdesc_dma = (unsigned long)_GET_HAL_DATA(Adapter)->desc_bcn_dma_addr + txdesc_off;
            mw5_coherent_tx = 1;
        }
#endif
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
            if (mw5_coherent_tx) {
                HAL_RTL_W32(TXBD_Reg[q_num], (u4Byte)mw5_txbd_dma);
                _GET_HAL_DATA(Adapter)->txBD_dma_ring_addr[q_num] = mw5_txbd_dma;
                _GET_HAL_DATA(Adapter)->txDesc_dma_ring_addr[q_num] = mw5_txdesc_dma;
                if (q_num == HCI_TX_DMA_QUEUE_MGT) {
                    Adapter->pshare->linux6_txbd_dma_base = (u32)mw5_txbd_dma;
                    Adapter->pshare->linux6_txbd_hw_base = HAL_RTL_R32(TXBD_Reg[q_num]);
                    Adapter->pshare->linux6_txdesc_dma_base = (u32)mw5_txdesc_dma;
                    Adapter->pshare->linux6_mgq_idx_after_init = TXBD_RWPtr_Reg[q_num] ?
                        HAL_RTL_R32(TXBD_RWPtr_Reg[q_num]) : 0;
                    printk(KERN_INFO "rtl8197f-wlan: v44.35 MGT coherent rings if=%s chip=0x%x txbd_dma=0x%08lx hw=0x%08x txdesc_dma=0x%08lx idx=0x%08x\n",
                           Adapter->dev ? Adapter->dev->name : "?", GET_CHIP_VER(Adapter), mw5_txbd_dma,
                           Adapter->pshare->linux6_txbd_hw_base, mw5_txdesc_dma,
                           Adapter->pshare->linux6_mgq_idx_after_init);
                } else if (q_num == HCI_TX_DMA_QUEUE_BCN) {
                    Adapter->pshare->linux6_bcnbd_dma_base = (u32)mw5_txbd_dma;
                    Adapter->pshare->linux6_bcnbd_hw_base = HAL_RTL_R32(TXBD_Reg[q_num]);
                    Adapter->pshare->linux6_bcndesc_dma_base = (u32)mw5_txdesc_dma;
                    printk(KERN_INFO "rtl8197f-wlan: v44.35 BCN coherent rings if=%s chip=0x%x txbd_dma=0x%08lx hw=0x%08x txdesc_dma=0x%08lx\n",
                           Adapter->dev ? Adapter->dev->name : "?", GET_CHIP_VER(Adapter), mw5_txbd_dma,
                           Adapter->pshare->linux6_bcnbd_hw_base, mw5_txdesc_dma);
                }
            } else
#endif
            {
                HAL_RTL_W32(TXBD_Reg[q_num], HAL_VIRT_TO_BUS1(Adapter,(void *)ptxbd,sizeof(TX_BUFFER_DESCRIPTOR) * pTXBD_NUM[q_num],HAL_PCI_DMA_TODEVICE) + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL);
        ...
        if (q_num != HCI_TX_DMA_QUEUE_BCN) {
            for(i = 0; i < pTXBD_NUM[q_num]; i++)        
            {
                unsigned long txdesc_dma_addr;

#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
                if (mw5_coherent_tx)
                    txdesc_dma_addr = mw5_txdesc_dma + sizeof(TX_DESC_88XX) * i;
                else
#endif
                    txdesc_dma_addr = HAL_VIRT_TO_BUS1(Adapter, (void *)&ptx_desc[i],
                        sizeof(TX_BUFFER_DESCRIPTOR), HAL_PCI_DMA_TODEVICE) +
                        CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL;
		...
		rtl_cache_sync_wback(Adapter,(unsigned long)(&(ptxbd[i].TXBD_ELE[0])-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 
            ...
            for (i = 0; i < 1+HAL_NUM_VWLAN; i++)
            {
                unsigned long bcn_txdesc_dma_addr;
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
                if (mw5_coherent_tx)
                    bcn_txdesc_dma_addr = mw5_txdesc_dma + sizeof(TX_DESC_88XX) * i;
                else
#endif
                    bcn_txdesc_dma_addr = HAL_VIRT_TO_BUS1(Adapter, (void *)&ptx_desc[i],
                        sizeof(TX_BUFFER_DESCRIPTOR), HAL_PCI_DMA_TODEVICE) +
                        CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL;
...
)
{
    ...
    rtl_atomic_dec(&(cur_q->avail_txbd_num));
}
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
        /* v44.56: Report20 showed MGT/HI0 DOK IRQs stay at zero and the
         * streaming-DMA map/unmap gap grows under load.  Snapshot the real
         * 8822B producer/consumer register and request the existing vendor TX
         * DSR after each latency-critical MGT/HI0 doorbell.  The tasklet still
         * owns all ring locking and frees descriptors only after HW advances. */
        if (Adapter && Adapter->pshare && IS_HARDWARE_TYPE_8822B(Adapter) &&
            Adapter->pshare->linux6_radio_index == 0 && cur_q->reg_rwptr_idx) {
            if (queueIndex == HCI_TX_DMA_QUEUE_MGT || queueIndex == HCI_TX_DMA_QUEUE_HI0) {
                u4Byte mw5_rw = HAL_RTL_R32(cur_q->reg_rwptr_idx);
                if (queueIndex == HCI_TX_DMA_QUEUE_MGT) {
                    u2Byte mw5_host = mw5_rw & 0x0fff;
                    u2Byte mw5_hw = (mw5_rw >> 16) & 0x0fff;
                    u2Byte mw5_pending;

                    Adapter->pshare->linux6_mgq_rwptr = mw5_rw;
                    if (mw5_host >= mw5_hw)
                        mw5_pending = mw5_host - mw5_hw;
                    else if (cur_q->total_txbd_num > mw5_hw)
                        mw5_pending = cur_q->total_txbd_num - mw5_hw + mw5_host;
                    else
                        mw5_pending = 0;

                    /* Report21 showed MGT producer 45->104->167 while the
                     * hardware consumer stayed at 39.  Do not touch
                     * REG_PCIE_CTRL here: on RTL8822B its low bits are queue
                     * STOP controls, not POLL doorbells.  The proven TXBD
                     * producer notification is the low 16-bit write to this
                     * queue's RW pointer register, which is already used by
                     * the normal submit path above.  For a real backlog,
                     * safely repeat that same producer-index write. */
                    if (mw5_pending >= 8) {
                        HAL_RTL_W16(cur_q->reg_rwptr_idx, cur_q->host_idx);
                        Adapter->pshare->linux6_tx_redoorbell_kicks++;
                        Adapter->pshare->linux6_tx_mgt_redoorbell_kicks++;
                    }
                } else {
                    Adapter->pshare->linux6_hi0_rwptr = mw5_rw;
                }
                if (rtk_tasklet_set_triggered(&(Adapter->pshare->tx_tasklet))) {
                    Adapter->pshare->linux6_tx_reclaim_kicks++;
                    Adapter->pshare->linux6_tx_reclaim_tx_kicks++;
                    tasklet_schedule(&(Adapter->pshare->tx_tasklet.tasklet));
                }
            } else if (queueIndex >= HCI_TX_DMA_QUEUE_BK &&
                       queueIndex <= HCI_TX_DMA_QUEUE_VO) {
                u64 submits = ++Adapter->pshare->linux6_tx_data_submits;

                /* Report21: data DMA mappings continued growing while unmaps
                 * stopped.  Do one MMIO snapshot/reclaim decision per 256 data
                 * doorbells, not per packet.  The existing TX DSR remains the
                 * sole owner of descriptor recycling and only frees entries
                 * after the hardware consumer index advances. */
                if ((submits & 0xff) == 0) {
                    u4Byte mw5_rw = HAL_RTL_R32(cur_q->reg_rwptr_idx);
                    switch (queueIndex) {
                    case HCI_TX_DMA_QUEUE_BK:
                        Adapter->pshare->linux6_bkq_rwptr = mw5_rw;
                        break;
                    case HCI_TX_DMA_QUEUE_BE:
                        Adapter->pshare->linux6_beq_rwptr = mw5_rw;
                        break;
                    case HCI_TX_DMA_QUEUE_VI:
                        Adapter->pshare->linux6_viq_rwptr = mw5_rw;
                        break;
                    case HCI_TX_DMA_QUEUE_VO:
                        Adapter->pshare->linux6_voq_rwptr = mw5_rw;
                        break;
                    default:
                        break;
                    }

                    if (Adapter->pshare->linux6_tx_dma_maps >
                        Adapter->pshare->linux6_tx_dma_unmaps + 256) {
                        u2Byte mw5_host = mw5_rw & 0x0fff;
                        u2Byte mw5_hw = (mw5_rw >> 16) & 0x0fff;
                        u2Byte mw5_pending;
                        ...
                        /* Same safety rule as MGT: repeat only the queue's
                         * normal producer-index notification.  No global
                         * PCIe STOP/control register is modified. */
                        if (mw5_pending >= 16) {
                            HAL_RTL_W16(cur_q->reg_rwptr_idx, cur_q->host_idx);
                            Adapter->pshare->linux6_tx_redoorbell_kicks++;
                            Adapter->pshare->linux6_tx_data_redoorbell_kicks++;
                        }

                        if (rtk_tasklet_set_triggered(&(Adapter->pshare->tx_tasklet))) {
                            Adapter->pshare->linux6_tx_reclaim_kicks++;
                            Adapter->pshare->linux6_tx_reclaim_data_kicks++;
                            tasklet_schedule(&(Adapter->pshare->tx_tasklet.tasklet));
                        }
                    }
                }
...
//if (queueIndex == HCI_TX_DMA_QUEUE_BE) {
    ...
    SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[1].Dword1,
        MW5_TX_PACKET_DMA(Adapter, pdesc_data->pHdr, (hdrLen + payloadLen), queueIndex) + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
        TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
...
//}
...
#endif
    {
        ...
        dma_addr = MW5_TX_PACKET_DMA(Adapter, pdesc_data->pHdr, hdrLen, queueIndex);
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[1].Dword1,
                    dma_addr + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                    TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
#ifdef AP_SWPS_OFFLOAD
    ...
    // for FG_AGGRE_MSDU_FIRST, FG_AGGRE_MSDU_MIDDLE, and FG_AGGRE_MSDU_LAST
        ...
        SET_DESC_FIELD_NO_CLR(cur_txbd_amsdu->TXBD_ELE[ptx_dma_amsdu->tx_amsdu_queue[queueIndex-1].cur_txbd_element].Dword1,
                MW5_TX_PACKET_DMA(Adapter, pdesc_data->pBuf, payloadLen, queueIndex) + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
...
#endif
    {
        ...
        if (payloadLen) {
            dma_addr = MW5_TX_PACKET_DMA(Adapter, pdesc_data->pBuf, payloadLen, queueIndex);
            SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[2].Dword1,
                   dma_addr + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                    TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
        }
    ...
    if (pdesc_data->pIcv != NULL) {
        ...
        dma_addr = MW5_TX_PACKET_DMA(Adapter, pdesc_data->pIcv, pdesc_data->icv, queueIndex);
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[3].Dword1,
                    dma_addr + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                    TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
#ifdef AP_SWPS_OFFLOAD        
        ...
        dma_addr = MW5_TX_PACKET_DMA(Adapter, pdesc_data->pMic, pdesc_data->mic, queueIndex);
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[3].Dword1,
                    dma_addr + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                    TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
#ifdef AP_SWPS_OFFLOAD        
...
{
    PHCI_TX_DMA_MANAGER_88XX    ptx_dma;
    u4Byte                      QueueIdx=0;   //HCI_TX_DMA_QUEUE_88XX
    u4Byte                      q_max=0;      //HCI_TX_DMA_QUEUE_88XX
...
{
    PHCI_TX_DMA_MANAGER_88XX        ptx_dma = (PHCI_TX_DMA_MANAGER_88XX)(_GET_HAL_DATA(Adapter)->PTxDMA88XX);
    u4Byte                          TXBDBeaconOffset = 0;
    u4Byte                          DMA_QUEUE_BCN = 0;
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    if (Adapter && Adapter->pshare && IS_HARDWARE_TYPE_8197F(Adapter)) {
        Adapter->pshare->linux6_beacon_own_before_set =
            GET_DESC_FIELD(pTXBD->TXBD_ELE[0].Dword0, TXBD_DW0_BCN_OWN_MSK, TXBD_DW0_BCN_OWN_SH);
        Adapter->pshare->linux6_beacon_pslen =
            GET_DESC_FIELD(pTXBD->TXBD_ELE[0].Dword0, TXBD_DW0_BCN_PSLEN_MSK, TXBD_DW0_BCN_PSLEN_SH);
        Adapter->pshare->linux6_beacon_txbd_bus =
            (u32)_GET_HAL_DATA(Adapter)->txBD_dma_ring_addr[HCI_TX_DMA_QUEUE_BCN];
        Adapter->pshare->linux6_beacon_txdesc_bus =
            (u32)_GET_HAL_DATA(Adapter)->txDesc_dma_ring_addr[HCI_TX_DMA_QUEUE_BCN];
        Adapter->pshare->linux6_beacon_bcnq_desa = HAL_RTL_R32(REG_BCNQ_TXBD_DESA);
        if (Value == HW_VAR_BEACON_ENABLE_DOWNLOAD &&
            Adapter->pshare->linux6_beacon_last_action == 1 &&
            Adapter->pshare->linux6_beacon_own_before_set == 0)
            Adapter->pshare->linux6_beacon_hw_consume_seen++;
    }
#endif
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    if (Adapter && Adapter->pshare && IS_HARDWARE_TYPE_8197F(Adapter)) {
        Adapter->pshare->linux6_beacon_own_after_set =
            GET_DESC_FIELD(pTXBD->TXBD_ELE[0].Dword0, TXBD_DW0_BCN_OWN_MSK, TXBD_DW0_BCN_OWN_SH);
        if (Value == HW_VAR_BEACON_ENABLE_DOWNLOAD) {
            Adapter->pshare->linux6_beacon_enable_calls++;
            Adapter->pshare->linux6_beacon_last_action = 1;
        } else if (Value == HW_VAR_BEACON_DISABLE_DOWNLOAD) {
            Adapter->pshare->linux6_beacon_disable_calls++;
            Adapter->pshare->linux6_beacon_last_action = 2;
        }
        /* v44.56: Report20 showed thousands of beacon hot-path printk calls on
         * the 1 GHz RTL8197F.  Keep all counters/debugfs proof, but print only
         * the first few ownership transitions needed for boot diagnostics. */
        if (Adapter->pshare->linux6_beacon_enable_calls +
            Adapter->pshare->linux6_beacon_disable_calls <= 12)
            pr_info("rtl8197f-wlan: v44.57 BCN OWN startup if=%s action=%u own=%u->%u pslen=%u txbd_bus=%08x txdesc_bus=%08x bcnq_desa=%08x enable=%llu disable=%llu hw_consume=%llu\n",
                Adapter->dev ? Adapter->dev->name : "?", Value,
                Adapter->pshare->linux6_beacon_own_before_set, Adapter->pshare->linux6_beacon_own_after_set,
                Adapter->pshare->linux6_beacon_pslen, Adapter->pshare->linux6_beacon_txbd_bus,
                Adapter->pshare->linux6_beacon_txdesc_bus, Adapter->pshare->linux6_beacon_bcnq_desa,
                (unsigned long long)Adapter->pshare->linux6_beacon_enable_calls,
                (unsigned long long)Adapter->pshare->linux6_beacon_disable_calls,
                (unsigned long long)Adapter->pshare->linux6_beacon_hw_consume_seen);
    }
#endif
...
)
{
    ...
    u4Byte                          TXDESCSize=0;
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    unsigned long                   mw5_beacon_payload_bus = 0;
#endif
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    if (Adapter && Adapter->pshare && IS_HARDWARE_TYPE_8197F(Adapter)) {
        Adapter->pshare->linux6_beacon_signins++;
        Adapter->pshare->linux6_beacon_own_at_signin =
            GET_DESC_FIELD(pTXBD->TXBD_ELE[0].Dword0, TXBD_DW0_BCN_OWN_MSK, TXBD_DW0_BCN_OWN_SH);
        Adapter->pshare->linux6_beacon_txbd_bus =
            (u32)_GET_HAL_DATA(Adapter)->txBD_dma_ring_addr[HCI_TX_DMA_QUEUE_BCN];
        Adapter->pshare->linux6_beacon_txdesc_bus =
    ...
    if ( IS_HARDWARE_TYPE_8814B(Adapter))  {
        ...
        if (!priv->pmib->dot11DFSEntry.disable_DFS &&
            (mw5_legacy_timer_pending(&GET_ROOT(priv)->ch_avail_chk_timer))) {
    ...
    //Segment 1: Payload
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    /* v44.49: Report14 proves that the RTL8197FS consumes the beacon TXBD,
     * while the SSID is still not visible OTA.  Restore the OEM 8197F
     * payload contract exactly: the vendor-owned priv->beaconbuf is stable,
     * its direct bus address is written to segment 1, and the normal OEM
     * cache writeback below makes the payload visible to the integrated DMA.
     * Descriptor-ring coherency and the RTL8822B streaming-DMA path are not
     * changed. */
    mw5_beacon_payload_bus =
        HAL_VIRT_TO_BUS1(Adapter, (PVOID)beaconbuf, frlen, HAL_PCI_DMA_TODEVICE) +
        CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL;
    if (IS_HARDWARE_TYPE_8197F(Adapter)) {
        Adapter->pshare->linux6_beacon_payload_mode = 0;
        Adapter->pshare->linux6_beacon_payload_bus = (u32)mw5_beacon_payload_bus;
    }
#endif
    SET_DESC_FIELD_CLR(pTXBD->TXBD_ELE[1].Dword0, frlen, TXBD_DW0_TXBUFSIZE_MSK, TXBD_DW0_TXBUFSIZE_SH);
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    SET_DESC_FIELD_CLR(pTXBD->TXBD_ELE[1].Dword1, mw5_beacon_payload_bus,
                TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
#else
...
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    if (IS_HARDWARE_TYPE_8197F(Adapter) && Adapter->pshare->linux6_beacon_signins <= 4) {
        /* v44.57: the following descriptor snapshots are debugfs/printk proof,
         * not part of beacon programming. Stop re-reading them every beacon. */
        Adapter->pshare->linux6_beacon_txbd0_dw0 = GET_DESC(pTXBD->TXBD_ELE[0].Dword0);
        Adapter->pshare->linux6_beacon_txbd0_dw1 = GET_DESC(pTXBD->TXBD_ELE[0].Dword1);
        Adapter->pshare->linux6_beacon_txbd1_dw0 = GET_DESC(pTXBD->TXBD_ELE[1].Dword0);
        Adapter->pshare->linux6_beacon_txbd1_dw1 = GET_DESC(pTXBD->TXBD_ELE[1].Dword1);
        Adapter->pshare->linux6_beacon_txdesc_dw0 = GET_DESC(ptx_desc->Dword0);
        Adapter->pshare->linux6_beacon_txdesc_dw1 = GET_DESC(ptx_desc->Dword1);
        Adapter->pshare->linux6_beacon_txdesc_dw2 = GET_DESC(ptx_desc->Dword2);
        Adapter->pshare->linux6_beacon_txdesc_dw3 = GET_DESC(ptx_desc->Dword3);
        Adapter->pshare->linux6_beacon_txdesc_dw4 = GET_DESC(ptx_desc->Dword4);
        Adapter->pshare->linux6_beacon_txdesc_dw5 = GET_DESC(ptx_desc->Dword5);
        Adapter->pshare->linux6_beacon_pslen =
            GET_DESC_FIELD(pTXBD->TXBD_ELE[0].Dword0, TXBD_DW0_BCN_PSLEN_MSK, TXBD_DW0_BCN_PSLEN_SH);
        printk(KERN_INFO
                "rtl8197f-wlan: v44.57 BCN SIGNIN startup payload=%s bus=0x%08x len=%u own_in=%u pslen=%u txbd_bus=%08x txdesc_bus=%08x bcnq_desa=%08x txbd=%08x/%08x %08x/%08x txdesc0-5=%08x/%08x/%08x/%08x/%08x/%08x signins=%llu\n",
                "oem-direct",
                Adapter->pshare->linux6_beacon_payload_bus, frlen,
                Adapter->pshare->linux6_beacon_own_at_signin, Adapter->pshare->linux6_beacon_pslen,
                Adapter->pshare->linux6_beacon_txbd_bus, Adapter->pshare->linux6_beacon_txdesc_bus,
                Adapter->pshare->linux6_beacon_bcnq_desa,
                Adapter->pshare->linux6_beacon_txbd0_dw0, Adapter->pshare->linux6_beacon_txbd0_dw1,
                Adapter->pshare->linux6_beacon_txbd1_dw0, Adapter->pshare->linux6_beacon_txbd1_dw1,
                Adapter->pshare->linux6_beacon_txdesc_dw0, Adapter->pshare->linux6_beacon_txdesc_dw1,
                Adapter->pshare->linux6_beacon_txdesc_dw2, Adapter->pshare->linux6_beacon_txdesc_dw3,
                Adapter->pshare->linux6_beacon_txdesc_dw4, Adapter->pshare->linux6_beacon_txdesc_dw5,
                (unsigned long long)Adapter->pshare->linux6_beacon_signins);
    }
#endif
    ...
    //write back cache: Payload (v44.49 restores the OEM direct 8197F payload path)
        HAL_CACHE_SYNC_WBACK(Adapter,
...
//    BOOLEAN TX_DESC_GF               = 0;
...
#endif
    {
		...
		IE4_SET_TX_DESC_TXPWR_OFSET_TYPE_NO_CLR(IE4_head, TX_DESC_POWER_OFFSET);
...
original:
...
#else
#ifdef NEW_BUUFER_ALLOCATION
    ptxbd_head      =   (PTX_BUFFER_DESCRIPTOR) (_GET_HAL_DATA(Adapter)->desc_dma_buf);
    platform_zero_memory(ptxbd_head, _GET_HAL_DATA(Adapter)->desc_dma_buf_len);
#else
    ptxbd_head      = (PTX_BUFFER_DESCRIPTOR)(prx_dma->rx_queue[HCI_RX_DMA_QUEUE_MAX_NUM-1].pRXBD_head + \
                        prx_dma->rx_queue[HCI_RX_DMA_QUEUE_MAX_NUM-1].total_rxbd_num);
    ...
    for (q_num = 0; q_num < DMA_MAX_QUEUE_NUM; q_num++)
    {
        ...
        if (IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8197G(Adapter)) {
    		if(pTXBD_Reg[q_num]){        
                HAL_RTL_W32(pTXBD_Reg[q_num], HAL_VIRT_TO_BUS((void *)(u4Byte)ptxbd));
        ...
        if (q_num != DMA_QUEUE_BCN) {
            ...
            {   
                SET_DESC_FIELD_CLR(ptxbd[i].TXBD_ELE[0].Dword0, \
                        ...
                        TXBD_DW0_TXBUFSIZE_MSK, TXBD_DW0_TXBUFSIZE_SH);
                SET_DESC_FIELD_CLR(ptxbd[i].TXBD_ELE[0].Dword1, \
                        HAL_VIRT_TO_BUS((void *)(u4Byte)&ptx_desc[i]), \
                        TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
