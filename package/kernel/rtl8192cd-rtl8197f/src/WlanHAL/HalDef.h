...
typedef struct _HAL_DATA_COMMON_
{
    ...
    RT_INT_REG          IntArray[11];
    RT_INT_REG          IntArray_bak[2];
    RT_INT_REG			IntMask[11];
...
#ifdef DMA_BUF_SEP
    pu1Byte             desc_txbd_dma_buf;
    pu1Byte             desc_txdesc_dma_buf;
    pu1Byte             desc_bcn_dma_buf;
#if defined(CONFIG_RTL8192CD_LINUX6_BUS) && defined(CONFIG_MW5_FORCE_WLAN_DEVICE_TABLE)
    /* v44.35: MW5 RTL8822B and RTL8197F TX descriptor blocks use real
     * Linux coherent DMA addresses.  Keep original allocation handles and
     * aligned CPU/DMA views separately so teardown never kfree()s an
     * aligned interior pointer. */
    pu1Byte             alloc_desc_txbd_addr;
    dma_addr_t          alloc_desc_txbd_dmahandle;
    u4Byte              alloc_desc_txbd_buf_size;
    dma_addr_t          desc_txbd_dma_addr;
    pu1Byte             alloc_desc_txdesc_addr;
    dma_addr_t          alloc_desc_txdesc_dmahandle;
    u4Byte              alloc_desc_txdesc_buf_size;
    dma_addr_t          desc_txdesc_dma_addr;
    pu1Byte             alloc_desc_bcn_addr;
    dma_addr_t          alloc_desc_bcn_dmahandle;
    u4Byte              alloc_desc_bcn_buf_size;
    dma_addr_t          desc_bcn_dma_addr;
#endif
#else
...
#ifdef DMA_BUF_SEP
    u4Byte              desc_dma_txbd_buf_len;
    u4Byte              desc_dma_txdesc_buf_len;
    u4Byte              desc_dma_bcn_buf_len;
#else
...
#else
#define IS_HARDWARE_TYPE_8192FE(_Adapter)	0
#endif
