...
VOID *HALAPI_MALLOC(VOID *pDriver_adapter,u32 size);
u8 HALAPI_FREE(VOID *pDriver_adapter,VOID *pBuf,u32 size);
u8 HALAPI_MEMCPY(VOID *pDriver_adapter,VOID *dest,VOID *src,u32 size);
u8 HALAPI_MEMSET(VOID *pDriver_adapter,VOID *pAddress,u8 value,u32 size);
VOID HALAPI_DELAY_US(VOID *pDriver_adapter,u32 us);
VOID HALAPI_MSLEEP(VOID *pDriver_adapter,u32 ms);
u8 HALAPI_MUTEX_INIT(VOID *pDriver_adapter,HALMAC_MUTEX *pMutex);
u8 HALAPI_MUTEX_DEINIT(VOID *pDriver_adapter,HALMAC_MUTEX *pMutex);
u8 HALAPI_MUTEX_LOCK(VOID *pDriver_adapter,HALMAC_MUTEX *pMutex);
u8 HALAPI_MUTEX_UNLOCK(VOID *pDriver_adapter,HALMAC_MUTEX *pMutex);
u8 HALAPI_EVENT_INDICATION(VOID *pDriver_adapter,HALMAC_FEATURE_ID feature_id,HALMAC_CMD_PROCESS_STATUS process_status,u8* buf, u32 size);
