...
static int HAPD_Process_Set_Port(struct net_device *dev, unsigned char *MACAddr, int PortStatus)
{
	...
	HAPD_MSG("HAPD_Process_Set_Port +++ %s: "MACSTR" Status %X\n",
		(OPMODE & WIFI_AP_STATE) ? "sta" : "ap",
		MAC2STR(MACAddr), PortStatus);
...
int	rtl_wpas_join(struct rtl8192cd_priv *priv, int bss_num)
{
	...
	wpaslog("priv->pmib->dot11Bss be copied\n");
#if defined(RTK_NL80211)
	memcpy((void *)&(priv->pmib->dot11Bss), (void *)&priv->site_survey->bss_target[bss_num], sizeof(struct bss_desc));
#else
	memcpy((void *)&(priv->pmib->dot11Bss), (void *)&priv->site_survey->bss_backup[bss_num], sizeof(struct bss_desc));
#endif
	...
	if(profile_chk_stage(priv,bss_num) == 0)
		return -1;
	...
	NDEBUG2(" going to join bss: [%s] [channel=%d]\n", tmpbuf, priv->pmib->dot11Bss.channel);
	...
	//mw5_legacy_mod_timer(&priv->WPAS_timer, jiffies + 300);
...
int rtl_net80211_setmlme(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	{
		if(!memcmp(mlme->im_macaddr, BROADCAST_MAC, 6)) //_Eric ??How to free all stainfo??
			return 0;
...
int rtl_net80211_setkey(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	HAPD_MSG("[%s]\n",priv->dev->name);
	HAPD_MSG("keyid = %d, mac = %pm\n", wk->ik_keyix, wk->ik_macaddr);
	...
	if(!memcmp(wk->ik_macaddr, BROADCAST_MAC, 6)){
		group_key = 1;
	}

#ifdef WIFI_WPAS //_Eric ?? if oxff not group addr in wpas ??
	if(OPMODE & WIFI_STATION_STATE){
		if(!memcmp(wk->ik_macaddr, NULL_MAC, 6)){
			group_key = 1;
		}
	}
#endif
	...
	if (keyType == DOT11_KeyType_Group)
	{
		...
		case DOT11_ENC_TKIP:
			...
			if (!SWCRYPTO) {
				if (set_gkey_to_cam)
				{
					retVal = CamDeleteOneEntry(priv, BROADCAST_MAC, wk->ik_keyix, 0);
					if (retVal) {
						priv->pshare->CamEntryOccupied--;
						pmib->dot11GroupKeysTable.keyInCam = FALSE;
					}
					retVal = CamAddOneEntry(priv, BROADCAST_MAC, wk->ik_keyix, DOT11_ENC_TKIP<<2, 0, wk->ik_keydata);
					if (retVal) {
						priv->pshare->CamEntryOccupied++;
		...
		case DOT11_ENC_WEP40:
			...
			if (!SWCRYPTO) {
				if (set_gkey_to_cam)
				{
					...
					retVal = CamAddOneEntry(priv, BROADCAST_MAC, wk->ik_keyix, DOT11_ENC_WEP40<<2, 0, wk->ik_keydata);
					if (retVal) {
						priv->pshare->CamEntryOccupied++;
						pmib->dot11GroupKeysTable.keyInCam = TRUE;
					}
		...
		case DOT11_ENC_WEP104:
			...
			if (!SWCRYPTO) {
				if (set_gkey_to_cam)
				{
					...
					retVal = CamAddOneEntry(priv, BROADCAST_MAC, wk->ik_keyix, DOT11_ENC_WEP104<<2, 0, wk->ik_keydata);
					if (retVal) {
						priv->pshare->CamEntryOccupied++;
						pmib->dot11GroupKeysTable.keyInCam = TRUE;
					}
		...
		case DOT11_ENC_CCMP:
			...
			if (!SWCRYPTO) {
				if (set_gkey_to_cam)
				{
					...
					retVal = CamAddOneEntry(priv, BROADCAST_MAC, wk->ik_keyix, DOT11_ENC_CCMP<<2, 0, wk->ik_keydata);
					if (retVal) {
						priv->pshare->CamEntryOccupied++;
						pmib->dot11GroupKeysTable.keyInCam = TRUE;
					} else {
	...
	{		
		wpaslog("set BIP group key[IGTK]\n");
	...
	if (cam_failed) {
		pr_err("rtl8197f-wlan: MW5-KEY-CAM-FAIL if=%s peer=%pM keyid=%u cipher=%d\n",
		       priv->dev->name, wk->ik_macaddr, wk->ik_keyix, cipher);
		return -EIO;
	}
...
int rtl_net80211_delkey(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if (!memcmp(wk->idk_macaddr, BROADCAST_MAC, 6)){
		...
		if (CamDeleteOneEntry(priv, BROADCAST_MAC, 1, 0))
			priv->pshare->CamEntryOccupied--;
...
#ifdef CONFIG_RTL8192CD_LINUX6_BUS
/*
 * The historical hostapd private ioctl copied a large vendor structure from
 * userspace.  It is deliberately not part of the Linux 6.6 control plane:
 * hostapd must configure AP state through nl80211/cfg80211.
 */
int rtl_hapd_config(struct net_device *dev, struct iw_request_info *info,
