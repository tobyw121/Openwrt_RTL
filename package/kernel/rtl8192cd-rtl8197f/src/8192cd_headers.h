...
#include "./WlanHAL/RTL88XX/RTL8197G/RTL8197GE/Hal8197GEDef.h"
#endif
...
#ifdef P2P_DEBUGMSG
#define P2P_DEBUG(fmt, args...)	(panic_printk("[%s %d][%s]:"fmt"\n",__func__ , __LINE__ ,priv->dev->name, ## args))
#define P2P_PRINT(fmt, args...)	( panic_printk(" "fmt"\n",## args))
#define P2P_PRINT2(fmt, args...)	( panic_printk(" "fmt,## args))
...
#define P2P_PRINT(fmt, args...) do{}while(0)
#define P2P_PRINT2(fmt, args...) do{}while(0)
#define MAC_PRINT(fmt, args...) do{}while(0)
#endif
...
//#define FT_DEBUGMSG 
#ifdef FT_DEBUGMSG
#define FTDEBUG(fmt, args...) panic_printk("FT[%s %d]"fmt "\n",__func__,__LINE__,## args)
#else
#define FTDEBUG(fmt, args...) {}
#endif

#ifdef RTK_STA_BWC
#define sta_bwc_printk(fmt, args...) \
	if (priv->pshare->rf_ft_var.sta_bwc_debug) \
		panic_printk("[%s %d][%s] "fmt, __FUNCTION__, __LINE__, priv->dev->name, ## args);
#endif

#define EDEBUG(fmt, args...) {}
//#define SDEBUG(fmt, args...) panic_printk("[%s %d]"fmt,__FUNCTION__,__LINE__,## args)
#define SDEBUG(fmt, args...) { }
//#define STADEBUG(fmt, args...) panic_printk("[%s %d][%s]"fmt,__FUNCTION__,__LINE__, priv->dev->name,## args)
#define STADEBUG(fmt, args...) { }
#ifdef _DEBUG_RTL8192CD_
#define PSKDEBUG(fmt, args...) panic_printk("[%s %d][%s]"fmt,__FUNCTION__,__LINE__, priv->dev->name,## args)
#else
...
//#define WM_DEBUGMSG
#ifdef WM_DEBUGMSG
#define WMDEBUG(fmt, args...) panic_printk("WM[%s %d]"fmt "\n",__FUNCTION__,__LINE__,## args)
#else
#define WMDEBUG(fmt, args...) {}
#endif

//#define REDUCEMEM(fmt, args...) printk("[%s %d]"fmt "\n",__func__,__LINE__,## args)
#define  REDUCEMEM(fmt, args...) do{ }while(0)
//#define DEBUG_NL80211

#ifdef DEBUG_NL80211
#define NDEBUG(fmt, args...) printk("[%s %d][%s]"fmt,__func__,__LINE__,priv->dev->name,## args)
#define NDEBUG2(fmt, args...) printk("[%s %d]"fmt,__func__,__LINE__,## args)
#else
#define  NDEBUG(fmt, args...)  do{ }while(0)
#define  NDEBUG2(fmt, args...) do{ }while(0)

#endif

//#define log2(fmt, args...) printk("[%s %d]"fmt"\n",__func__,__LINE__,## args)
#define log2(msg, ...) do {} while(0)


//#define owelog(fmt, args...) printk("[hapdowe][%s %d]"fmt"\n",__func__,__LINE__,## args)
#define owelog(msg, ...) do {} while(0)

//#define hapd11klog(fmt, args...) printk("[hapd11k][%s %d]"fmt"\n",__func__,__LINE__,## args)
#define hapd11klog(msg, ...) do {} while(0)

//#define hapd11vlog(fmt, args...) printk("[hapd11v][%s %d]"fmt"\n",__func__,__LINE__,## args)
#define hapd11vlog(msg, ...) do {} while(0)

#define GDEBUG(fmt, args...) panic_printk("[%s %d]"fmt,__func__,__LINE__,## args)
#define HALDEBUG(fmt, args...) panic_printk("[%s %d][%s]"fmt,__func__,__LINE__,Adapter->dev->name,## args)

//#define ACSDEBUG(fmt, args...) panic_printk("[%s:%d] "fmt,__FUNCTION__,__LINE__,## args)
#define ACSDEBUG(fmt, args...) do { } while(0)
//#define closeopenlog(fmt, args...) printk("[%s %d]"fmt"\n",__func__,__LINE__,## args)
#define closeopenlog(msg, ...) do {} while(0)

//#define hapdlog(fmt, args...) printk("[hapd][%s %d]"fmt"\n",__func__,__LINE__,## args)
#define hapdlog(msg, ...) do {} while(0)

//#define wpaslog(fmt, args...) printk("[wpas][%s %d]"fmt"\n",__func__,__LINE__,## args)
#define wpaslog(msg, ...) do {} while(0)

//#define hwgrouplog(fmt, args...) printk("[%s %d]"fmt"\n",__func__,__LINE__,## args)
#define hwgrouplog(msg, ...) do {} while(0)
#if defined(__ECOS)
...
EXTERN unsigned char BROADCAST_MAC[];
EXTERN unsigned char NULL_MAC[];
EXTERN unsigned char RSN_OUI2[];
EXTERN unsigned char wfa_oui_11k_ap_report[];
EXTERN unsigned char IGTK_KDE_TYPE[];

EXTERN int wpa2_ie_check(
	struct rtl8192cd_priv *priv,
	unsigned char *ie_ptr_in,
	int ie_len_in,
	unsigned char **ie_ptr_pmksa,
	unsigned char *pmkid_cnt);


EXTERN int peer_support_wpa3(struct rtl8192cd_priv *priv, unsigned int t_stamp);
EXTERN unsigned char support_wpa3(struct rtl8192cd_priv *priv);
EXTERN void dump_security_mib(struct rtl8192cd_priv *priv);
EXTERN void dump_hex(unsigned char *name, unsigned char *key, unsigned char len);
EXTERN unsigned long rtw_get_current_time(void);
EXTERN unsigned long rtw_get_passing_time_ms(unsigned long start_time);
EXTERN int get_ch_bitmap_idx(struct rtl8192cd_priv *priv , int channel);
EXTERN unsigned long get_total_tx_average(struct rtl8192cd_priv *priv);
EXTERN unsigned long get_total_rx_average(struct rtl8192cd_priv *priv);
...
#endif
EXTERN void _txrpt_handler(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct tx_rpt *report);
...
EXTERN int is_DFS_channel(struct rtl8192cd_priv *priv , int channelVal);


EXTERN void mem_dump(unsigned char *ptitle, unsigned char *pbuf, int len);
#if defined(AP_SWPS_OFFLOAD)
EXTERN int enque_before_Queuehead(struct rtl8192cd_priv *priv,struct tx_desc_info *pdescinfo, struct apsd_pkt_queue* AC_dz_queue, unsigned short last_seq, int ffsize, void *elm, unsigned char* prepare_done, int* rinsert_position);
EXTERN int find_dzposition(struct list_head* dzhead, struct tx_desc_info *pdescinfo);
EXTERN int insert_dzposition(struct tx_desc_info* pdescinfo,struct stat_info * pstat,int position_count,unsigned short last_seq,unsigned char * prepare_done,struct sk_buff *pskb);
#endif
...
EXTERN void init_rainfo(struct rtl8192cd_priv *priv, struct stat_info *pstat);
...
EXTERN void free_swps_info(struct rtl8192cd_priv *priv, struct stat_info *pstat);
#endif
...
#else /* TRIBAND_OPS */
#define free_sta_tx_skb(PRIV, PSTAT) \
	priv->hci_ops->free_sta_tx_skb(PRIV, PSTAT)
#endif
...
EXTERN int del_station(struct rtl8192cd_priv *priv, struct stat_info *pstat, int send_disconnect, int reason);
...
EXTERN struct stat_info* get_stainfo_from_aidarray(struct rtl8192cd_priv *priv, unsigned short aid);
#endif
#ifdef TXREPORT
...
EXTERN unsigned int get_curMaxRFPath(struct rtl8192cd_priv * priv);
...
EXTERN int get_bssrate_set(struct rtl8192cd_priv *priv, int bssrate_ie, unsigned char **pbssrate, unsigned int *bssrate_len);
EXTERN int get_available_channel(struct rtl8192cd_priv *priv);
EXTERN void set_rts_protect_all(struct rtl8192cd_priv *priv, unsigned char enable);
EXTERN int is_which_band_channel(unsigned int channel, unsigned char band);
EXTERN int is80MSubChannel(unsigned int channel, unsigned int channel2);
EXTERN int get_80p80_sub_channel(unsigned int chnl_list[], unsigned int chnl_num,unsigned int channel);
...
EXTERN unsigned int priv_max_tp(struct rtl8192cd_priv *priv);
...
EXTERN void set_ap_ps_mode(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
...
#if defined(DELAY_REFILL_RX_BUF) //&& defined(WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD)
EXTERN void delay_refill_rxdesc(struct rtl8192cd_priv *priv);
#endif
#ifdef RX_SKB_QUEUE
...
#ifndef __OSK__
EXTERN void rtk_tasklet_init(struct rtk_tasklet *t, void (*func)(unsigned long), unsigned long data);
EXTERN void rtk_tasklet_kill(struct rtk_tasklet *t);
EXTERN BOOLEAN rtk_tasklet_set_triggered(struct rtk_tasklet *t);
EXTERN void rtk_tasklet_clear_triggered(struct rtk_tasklet *t);
#endif // !(__OSK__)
...
EXTERN int rtk_queue_tail(struct rtl8192cd_priv *priv, struct ring_que *que, struct sk_buff *skb);
#endif

#ifdef TX_PROCESS_REFINE
EXTERN void rtk_txdata_queue_init(struct rtl8192cd_priv *priv, struct tx_data_que *que);
EXTERN int rtk_txdata_queue_flush(struct rtl8192cd_priv *priv);
EXTERN int rtk_txdata_queue_enqueue(struct rtl8192cd_priv *priv, struct tx_data_que *que, struct sk_buff* skb);
EXTERN struct sk_buff* rtk_txdata_queue_dequeue(struct rtl8192cd_priv *priv, struct tx_data_que *que);
#endif

#ifdef RING_BASED_DATA_FRAME_QUEUE
EXTERN void rtk_data_queue_init(struct rtl8192cd_priv *priv, struct data_que *que);
EXTERN int rtk_data_queue_enqueue(struct rtl8192cd_priv *priv, struct data_que *que, struct rx_frinfo *frinfo);
EXTERN struct rx_frinfo *rtk_data_queue_dequeue(struct rtl8192cd_priv *priv, struct data_que *que);
#endif
...
EXTERN int rtk_wlan_with_info_event_indicate(char* ifname, int event, unsigned char* addr, unsigned char *info, int len);
#endif
...
EXTERN void general_event_indicate(struct rtl8192cd_priv *priv);
#endif
...
EXTERN unsigned short get_80211frameVlanID(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);
#ifdef STA_ASSOC_STATISTIC
EXTERN struct tm * get_current_timeInfo ( struct tm * const	tmp, int tz);
#endif
#ifdef ADAPTIVITY_STATS_DEBUG
EXTERN void adaptivity_stats_check(struct rtl8192cd_priv *priv);
EXTERN void adaptivity_stats_timer(unsigned long task_priv);
#endif
#if !defined(__ECOS)
...
#endif
#ifdef HAPD_11K
EXTERN int hapd_rm_parse_beacon_report(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int len, int frame_len);
EXTERN void hapd_OnRadioMeasurementReport(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int frame_len);
EXTERN void hapd_rm_check_mgmt_tx(struct rtl8192cd_priv *priv, const u8 *buf, u32 len);
#endif

#ifdef HAPD_11V
EXTERN void hapd_on_btm_resp(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int frame_len);
#endif
...
#ifdef CONFIG_MBO
EXTERN void gas_init_req_check_frames_tx(struct rtl8192cd_priv *priv, u8 **dump_buf, int *dump_len);
EXTERN int hapd_rm_construct_nb_report(struct rtl8192cd_priv *priv, u8 *pbuf, struct nb_rpt_hdr *report, u8 disassoc_imminent, u8 preference_sta);
EXTERN int hapd_rm_construct_self_nb_report(struct rtl8192cd_priv *priv, u8 *pbuf);
#endif
...
#ifdef TX_PROCESS_REFINE
EXTERN void smp_assign_txdsr_tasklet(void *_dev);
#endif

#ifdef SW_TX_QUEUE
EXTERN unsigned int get_swq_turbo_thd(struct rtl8192cd_priv *priv);
...
#endif
EXTERN void rtl8192cd_amsdu_timeout(unsigned long task_priv);

#ifdef RTK_ATM
EXTERN void rtl8192cd_atm_swq_timeout(unsigned long task_priv);
EXTERN void atm_check_refill(struct rtl8192cd_priv *priv);
EXTERN void rtl8192cd_atm_swq_inittimer(unsigned long task_priv);
EXTERN int atm_skb_dequeue(struct rtl8192cd_priv *priv, struct stat_info *pstat, int deq_size, int qnum);
#endif

#if defined(CONFIG_PUMA_UDMA_SUPPORT)
EXTERN int rtl8192cd_udma_start_xmit(struct sk_buff *skb, struct net_device *dev);
#endif
...
EXTERN __IRAM_IN_865X
int rtl8192cd_xmit_offload(struct stat_info *pstat, struct sk_buff *pskb, struct tx_insn* txcfg);
EXTERN __IRAM_IN_865X int rtl88XX_fill_txPktInfo(struct rtl8192cd_priv *priv, struct tx_insn *txcfg, unsigned char pktType, unsigned char *phdr);
EXTERN void rtl_cfg_offload_enable(struct rtl8192cd_priv * priv);
EXTERN unsigned char dc_sta_was_full(struct rtl8192cd_priv *priv);
EXTERN void dc_sta_add(struct rtl8192cd_priv *priv, struct stat_info *pstat);
EXTERN void dc_sta_update(struct rtl8192cd_priv *priv, struct stat_info *pstat);
EXTERN void dc_sta_remove(struct rtl8192cd_priv *priv, struct stat_info *pstat);
EXTERN int realloc_dc_sta(struct rtl8192cd_priv *priv);

EXTERN void dc_if_update(struct rtl8192cd_priv *priv);
EXTERN void dc_if_remove(struct rtl8192cd_priv *priv);
EXTERN void dc_if_bcn_mode(struct rtl8192cd_priv *priv, int mode);

EXTERN void rtl88XX_signin_h2d_cmd(struct rtl8192cd_priv *priv, void *data, enum h2d_cmd_type cmd_type);
EXTERN void rtl88XX_signin_bcn(struct rtl8192cd_priv *priv, unsigned int *beaconbuf, unsigned int frlen);
#endif
...
#if defined(AP_SWPS_OFFLOAD)
void SWPS_force_recycle_packet(unsigned long task_priv);

EXTERN int SWPS_check_reprepare_done(struct rtl8192cd_priv *priv, struct stat_info * pstat);
EXTERN int SWPS_check_drop_done(struct stat_info * pstat);
#endif
...
EXTERN int isICMPv6Mng(unsigned char *data);
EXTERN int isICMPv6Mng2(unsigned char *protocol,unsigned char *payload);
EXTERN int isMDNS(unsigned char *data);
EXTERN int isMDNS2(unsigned char *da, unsigned char *protocol, unsigned char *payload);
...
#else /* TRIBAND_OPS */
//#define __rtl8192cd_start_xmit(SKB, DEV, TX_FLAG) priv->hci_ops->start_xmit(SKB, DEV, TX_FLAG)
#endif

EXTERN int rtl8192cd_tx_queueDsr(struct rtl8192cd_priv *priv, unsigned int txRingIdx);
...
EXTERN __MIPS16 __IRAM_IN_865X unsigned int pri_to_qnum(struct rtl8192cd_priv *priv, int priority);
EXTERN __MIPS16 __IRAM_IN_865X unsigned int get_skb_priority(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat, int is_11s);
EXTERN unsigned int get_tx_rate(struct rtl8192cd_priv *priv, struct stat_info *pstat);
EXTERN unsigned int get_lowest_tx_rate(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned int tx_rate);
#if (BEAMFORMING_SUPPORT == 1)
EXTERN BOOLEAN IsMgntNDPA(pu1Byte pdu);
#endif
...
EXTERN void RtsCheckAC(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, BOOLEAN *bRtsEnable, BOOLEAN *bCts2SelfEnable, BOOLEAN *bHwRts);
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
...
#ifdef TCP_ACK_ACC
EXTERN void check_tcp_ack_timeout(unsigned long _priv);
EXTERN int tx_tcp_ack_accelerate(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *skb);
#endif

EXTERN int mlcst2unicst(struct rtl8192cd_priv *priv, struct sk_buff *skb);
EXTERN void rtl8192cd_tx_xmitSkbFail(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct net_device *dev,
                    struct net_device *wdsDev, struct tx_insn *txcfg);
EXTERN int rtl8192cd_tx_gbwc(struct rtl8192cd_priv *priv, struct stat_info    *pstat, struct sk_buff *skb);
EXTERN int rtl8192cd_tx_tkip(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info*pstat, struct tx_insn *txcfg);
EXTERN int rtl8192cd_tx_sbwc(struct rtl8192cd_priv *priv, struct stat_info     *pstat, struct sk_buff *skb);
...
#if (BEAMFORMING_SUPPORT == 1) && (defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8812FE))
EXTERN unsigned char is_support_bf(struct stat_info* psta, unsigned char type);
#endif
EXTERN int profile_chk_stage(struct rtl8192cd_priv *priv, int loop_idx1);
EXTERN void start_clnt_assoc(struct rtl8192cd_priv *priv);
...
EXTERN int signin_beacon_desc_usb(struct rtl8192cd_priv * priv,unsigned int * beaconbuf,unsigned int frlen);
#else
EXTERN int signin_beacon_desc(struct rtl8192cd_priv * priv,unsigned int * beaconbuf,unsigned int frlen);
#endif
#endif

EXTERN void update_all_wifi_txrx_pkts(struct rtl8192cd_priv *rootPriv);
...
EXTERN unsigned int rtl8192cd_crc32(unsigned char *buf, int len);
#endif
...
#ifdef AP_SWPS_OFFLOAD
EXTERN void tx_reprepare_pkt(struct rtl8192cd_priv *priv, struct stat_info *pstat, u4Byte queueIndex, struct reprepare_info* pkt_info);
#endif


EXTERN void issue_asocrsp(struct rtl8192cd_priv *priv,	unsigned short status, struct stat_info *pstat, int pkt_type, unsigned int assoc_disallow);
...
#else /* TRIBAND_OPS */
#define check_dz_mgmt(PRIV, PSTAT, TXCFG) \
	priv->hci_ops->check_dz_mgmt(PRIV, PSTAT, TXCFG)
#endif
...
#endif
EXTERN int fill_probe_rsp_content(struct rtl8192cd_priv*, UINT8*, UINT8*, UINT8*, int , int , UINT8, UINT8, unsigned int);
...
#endif
void issue_NullData(struct rtl8192cd_priv *priv, struct stat_info *pstat);

#ifdef WIFI_WMM
#if defined(WIFI_QOS_ENHANCE)
EXTERN bool qos_enhance_query_sta(struct rtl8192cd_priv *priv, unsigned char *macaddr);
EXTERN void qos_enhance_proc(struct rtl8192cd_priv *priv);
#endif
EXTERN void init_WMM_Para_Element(struct rtl8192cd_priv *priv, unsigned char *temp, unsigned long len);
...
#else /* TRIBAND_OPS */
#ifdef CONFIG_USB_HCI
EXTERN void process_mcast_dzqueue_usb(struct rtl8192cd_priv *priv);
#endif
#define process_mcast_dzqueue(PRIV) \
	priv->hci_ops->process_mcast_dzqueue(PRIV)
#endif
...
#else /* TRIBAND_OPS */
#ifdef CONFIG_PCI_HCI
EXTERN int check_hangup_pci(struct rtl8192cd_priv *priv);
#endif
#ifdef CONFIG_USB_HCI
EXTERN int check_hangup_usb(struct rtl8192cd_priv *priv);
#endif
#define check_hangup(PRIV) \
    priv->hci_ops->check_hangup(PRIV)
#endif
#endif
...
EXTERN unsigned char *get_oui(unsigned char *pbuf, unsigned char *oui, int *len, int limit);
...
EXTERN void get_matched_rate(struct rtl8192cd_priv *priv, unsigned char *pRate, int *pLen, int which, int size);
...
#if defined(CONFIG_RTK_MESH) || defined(CONFIG_IEEE80211R)||defined(RTK_NL80211)
EXTERN void issue_auth_resp(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned short status);
#endif
...
EXTERN void auto_channel_timeout_expire(struct rtl8192cd_priv *priv);
#endif

#ifdef BEACON_VS_IE
EXTERN void beacon_vsie_timeout_expire(struct rtl8192cd_priv *priv);
#endif
...
EXTERN unsigned char *ft_auth_set_ft_rsnie_with_pmkid(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int *frlen);
#endif
#ifdef STA_ASSOC_STATISTIC
EXTERN void add_sta_assoc_status(struct rtl8192cd_priv *priv,unsigned char* addr, unsigned char rssi, unsigned short status);
#endif
#ifdef BSS_MONITOR_SUPPORT
EXTERN void collect_monitor_info(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);
#endif
#ifdef RA_OFFSET_TRAINING
EXTERN void check_txrate_variation(struct rtl8192cd_priv *priv, struct stat_info *pstat);
EXTERN int all_stations_rate_variation(struct rtl8192cd_priv *priv);
EXTERN void ra_offset_training(struct rtl8192cd_priv *priv);
#endif
#ifdef BEACON_VS_IE
EXTERN unsigned char set_beacon_vsie(struct rtl8192cd_priv * priv, unsigned char * data, unsigned int len);
EXTERN unsigned char set_probe_vsie(struct rtl8192cd_priv * priv, unsigned char * data, unsigned int len);
EXTERN unsigned char set_probersp_vsie(struct rtl8192cd_priv * priv, unsigned char * data, unsigned int len);
#endif
...
EXTERN void LED_Interval_timeout(unsigned long task_priv);
EXTERN void enable_sw_LED(struct rtl8192cd_priv *priv, int init);
#ifdef CONFIG_WIFI_LED_SHARED
extern struct rtl8192cd_priv *priv_led;
extern struct rtl8192cd_priv *priv_led_o;
EXTERN void disable_sw_LED(struct rtl8192cd_priv *priv, int deltimer);
#else
...
EXTERN void phydm_dfs_segment_distinguish_for_8814b(struct rtl8192cd_priv *priv, enum rf_syn syn_path);
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
EXTERN void pre_rtl8192cd_DFS_timer(unsigned long task_priv);
EXTERN void pre_rtl8192cd_DFS_TXPAUSE_timer(unsigned long task_priv);
EXTERN void pre_rtl8192cd_ch_avail_chk_timer(unsigned long task_priv);
EXTERN void pre_rtl8192cd_dfs_chk_timer(unsigned long task_priv);
EXTERN void pre_rtl8192cd_dfs_det_chk_timer(unsigned long task_priv);
#endif
...
EXTERN void DFS_SwitchChannel(struct rtl8192cd_priv *priv);
#endif
EXTERN void set_CHXX_timer(struct rtl8192cd_priv *priv, unsigned int channel);
#undef EXTERN
...
#ifdef DFS
EXTERN void dfs_init_para_by_manual_dfs_regdomain (struct rtl8192cd_priv *priv);
#if defined(CONFIG_WLAN_HAL_8814BE)
EXTERN void dfs_radar_distinguish_for_8814b (struct rtl8192cd_priv *priv, void *dm_void);
#endif
...
EXTERN int rtl8192cd_linux6_napi_poll(struct napi_struct *napi, int budget);
#endif
EXTERN void rtl8192cd_dsr_rxq(unsigned long task_priv);
#endif
EXTERN
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
EXTERN int rtl8192cd_rx_dispatch_offload(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);
#endif
...
EXTERN int rtk_monitor_sta_info(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);
#endif
#undef EXTERN
...
#define EXTERN
#endif
EXTERN u32 getAvailableTXBD(struct rtl8192cd_priv * priv, unsigned int qNum);
...
EXTERN int SWPSTestFunction(struct rtl8192cd_priv *priv, unsigned char *data);
...
EXTERN void rtl8197f_mw5_reassert_trsw(struct rtl8192cd_priv *priv, const char *reason);
#endif
#ifdef CONFIG_WLAN_HAL_8192EE
...
EXTERN unsigned int phy_query_rf_reg(struct rtl8192cd_priv *priv, unsigned int eRFPath,
				unsigned int RegAddr, unsigned int BitMask, unsigned int dbg_avoid);
EXTERN unsigned int phy_query_bb_reg(struct rtl8192cd_priv *priv, unsigned int RegAddr, unsigned int BitMask);
EXTERN void phy_set_bb_reg(struct rtl8192cd_priv *priv, unsigned int RegAddr, unsigned int BitMask, unsigned int Data);
EXTERN void phy_set_rf_reg(struct rtl8192cd_priv *priv, unsigned int eRFPath, unsigned int RegAddr,
				unsigned int BitMask, unsigned int Data);
#if defined(CONFIG_WLAN_HAL_8814BE)
EXTERN void phy_set_rf_syn_reg(struct rtl8192cd_priv *priv, unsigned int eRFPath, unsigned int RegAddr,
				unsigned int BitMask, unsigned int Data);
EXTERN unsigned int phy_query_rf_syn_reg(struct rtl8192cd_priv *priv, unsigned int eRFPath,
				unsigned int RegAddr, unsigned int BitMask);
#endif
...
EXTERN void setup_timer1(struct rtl8192cd_priv *priv, unsigned int timeout, unsigned int current_value);
...
#if defined(CONFIG_WLAN_HAL_8814AE) && !defined(NOT_RTK_BSP)
...
EXTERN void phy_iq_calibrate(struct rtl8192cd_priv *priv);
...
EXTERN void phy_rf6052_set_cck_tx_power(struct rtl8192cd_priv *priv, unsigned int channel);
EXTERN void phy_rf6052_set_ofdm_tx_power(struct rtl8192cd_priv *priv, unsigned int channel);
#endif
...
EXTERN void RTLWIFINIC_GPIO_write_proc(struct rtl8192cd_priv *priv, int gpio_num, unsigned int value);
...
/*remove old style cln nhm function*/
#if 0//defined(HS2_SUPPORT) || defined(RTK_NL80211) /* survey_dump */ || defined(DOT11K) || defined(CH_LOAD_CAL) || defined(RTK_SMART_ROAMING)
...
EXTERN void channel_loading_measurement(struct rtl8192cd_priv *priv, unsigned short clm_sample_num);
#endif

#ifdef USE_OUT_SRC
EXTERN UINT8 phy_nhm_clm_set_trigger(struct rtl8192cd_priv *priv, UINT8 action);
#endif
...
EXTERN unsigned int DMDP_PHY_QueryRFReg(unsigned int phy,unsigned int eRFPath,unsigned int RegAddr,unsigned int BitMask,unsigned int dbg_avoid);
EXTERN void DMDP_PHY_SetRFReg(unsigned int phy,unsigned int eRFPath,unsigned int RegAddr,unsigned int BitMask,unsigned int Data);
...
EXTERN int _convert_2_pwr_dot_new(char *s, int base);
...
EXTERN void rtk_cfg80211_rx_mgmt(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);
EXTERN void realtek_cfg80211_ch_switch_notify(struct work_struct *work);
/*for HW_ENC_FOR_GROUP_CIPHER feature*/
EXTERN void _rtl_net80211_setkey(struct net_device *dev, void *info, void *wrqu, char *extra);
#endif
...
EXTERN int rtl8192cd_ioctl_priv_req(struct net_device *dev, struct iw_point *data);
EXTERN int rtl8192cd_ioctl_daemonreq(struct net_device *dev, struct iw_point *data);
EXTERN int DOT11_Process_Delete_Key(struct net_device *dev, DOT11_DELETE_KEY *Delete_Key);
...
EXTERN int __DOT11_Process_Disconnect_Req(struct net_device *dev, DOT11_DISCONNECT_REQ	*Disconnect_Req);
#endif // CONFIG_USB_HCI || CONFIG_SDIO_HCI
...
EXTERN int debugfs_init(struct net_device *dev);
EXTERN void debugfs_deinit(struct net_device *dev);
#endif
#endif

#if defined(_INCLUDE_PROC_FS_) || defined(__ECOS) || defined(CONFIG_RTL8192CD_LINUX6_BUS)
#ifdef __ECOS
EXTERN const char* MCS_DATA_RATEStr[2][2][32];
#else
...
#define EXTERN
#endif
EXTERN void chk_insmod_rmmod(void);
...
#endif
EXTERN u8 Efuse_OneByteRead(struct dm_struct *dm, u16 addr, u8 *data);
EXTERN u8 get_cut_version_8198F(struct dm_struct *dm, unsigned long *data);
EXTERN int rtl8192cd_busenable(struct rtl8192cd_priv *priv,unsigned char value);
EXTERN u8 PHY_GetTxPowerFinalAbsoluteValue(struct dm_struct *dm, u8 rf_path,	u8 rate, u8 bandwidth, u8 channel);
EXTERN s16 PHY_GetTxPowerFinalAbsoluteValuembm(struct dm_struct *dm, u8 rf_path,	u8 rate, u8 bandwidth, u8 channel);
EXTERN int get_bonding_type_8881A(void);
EXTERN int get_bonding_type_8198F(void);
...
EXTERN int k_tpt_task_init(struct rtl8192cd_priv *priv);
EXTERN void k_tpt_task_stop(struct rtl8192cd_priv *priv);
#endif
...
EXTERN void rtl_wlan_vdev_init(char *name, void *priv, char type);
EXTERN void rtl_wlan_vdev_remove(char *ifname, void *priv);
#endif
EXTERN struct stat_info *get_stainfo_from_all_wlan(unsigned char *hwaddr);

#if defined CONFIG_RTK_MESH
EXTERN int is_dest_mesh_dev_tx(unsigned char *da);
#endif

EXTERN void gpio_func_ptr_init(void);
EXTERN void gpio_func_ptr_deinit(void);
...
EXTERN void GKRekeyTimeout(unsigned long task_priv);
...
#endif
#ifdef CONFIG_IEEE80211R_CLI
EXTERN int ft_check_imd_assocrsp(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pbuf, unsigned int limit, unsigned int *status); 
EXTERN int ft_check_ft_auth_rsp(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int limit, unsigned int *status, struct rx_frinfo *pfrinfo);
EXTERN void derive_ft_auth_keys(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);
EXTERN int ft_check_ft_reassocrsp(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pbuf, unsigned int limit, unsigned int *status);
EXTERN void ft_set_reassoc_keyInfo(struct rtl8192cd_priv *priv, struct stat_info *pstat);
#endif
#if defined(GENERAL_EVENT)
EXTERN void general_IndicateEvent(struct rtl8192cd_priv *priv, unsigned char event, void *data);
EXTERN void general_wlan_IndicateEvent(struct rtl8192cd_priv *priv, unsigned char event, void *data);
#endif
#undef EXTERN
...
#ifdef AUTH_SAE_STA
EXTERN int open_threshold;
EXTERN int sae_chk_peer_pmkid_cache(struct rtl8192cd_priv *priv,struct stat_info *pstat,
	unsigned char* ie_buf,unsigned short ie_len);
#endif
EXTERN unsigned char search_pmkid_cache(struct rtl8192cd_priv *priv, unsigned char *pmkid);
EXTERN unsigned char pmkid_cached(struct stat_info *pstat);
EXTERN unsigned char search_by_mac_pmkid_cache(struct rtl8192cd_priv *priv, unsigned char *peermac);
EXTERN void pmkid_cache_del(struct rtl8192cd_priv *priv, int idx);
EXTERN void recv_invaild_pmkid_report(struct rtl8192cd_priv *priv, unsigned char* peermac);
EXTERN void reset_pmkid_cache(struct rtl8192cd_priv *priv);
EXTERN void report_event_sae(struct rtl8192cd_priv *priv, void *p1, void *p2, unsigned char id);
EXTERN int sae_peer_table_expire(struct rtl8192cd_priv *priv);
EXTERN void do_reauth (struct rtl8192cd_priv *priv, struct candidate *peer);
EXTERN void retransmit_peer (struct rtl8192cd_priv *priv, struct candidate *peer);
EXTERN void remove_from_blacklist (struct rtl8192cd_priv *priv, unsigned char* peer_mac);

EXTERN int sae_rand_range(mbedtls_mpi *output, mbedtls_mpi *range);
EXTERN void rand_bytes(unsigned char* pb, size_t cb);
EXTERN int issue_sae_frame(struct rtl8192cd_priv *priv, u8 *Entry, size_t len);
EXTERN void sae_received_beacon(struct rtl8192cd_priv * priv, unsigned char * sa) ;
EXTERN 	enum result sae_received_auth(struct rtl8192cd_priv * priv, unsigned int len, unsigned char	*sae_frame);
EXTERN int sae_initialize(struct rtl8192cd_priv * priv);
#ifdef AUTH_SAE_STA
EXTERN int on_blacklist (struct rtl8192cd_priv *priv, unsigned char *mac);
EXTERN struct candidate *find_peer2 (struct rtl8192cd_priv *priv, unsigned char *mac);
EXTERN int peer_support_wpa3(struct rtl8192cd_priv *priv,unsigned int t_stamp_1);
EXTERN int client_send_commit_to_peer(	struct rtl8192cd_priv *priv,unsigned char *da);
EXTERN int clean_up_sae_peer_table(struct rtl8192cd_priv *priv);
EXTERN int clean_up_sae_blacklist_table(struct rtl8192cd_priv *priv);
...
#ifdef WPA3_ENT
#define EXTERN  extern
EXTERN int rtk_gcmp_encrypt(struct rtl8192cd_priv *priv, unsigned char *pwlhdr,
				unsigned char *pllc,
				unsigned char *pfr, unsigned int frlen,
				unsigned char *pmic, unsigned char isMgmt				
			);
...
EXTERN int mp_query_stats(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
...
EXTERN int mp_query_tssi(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
EXTERN int mp_query_ther(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
EXTERN int mp_query_rssi(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
#ifdef MP_PSD_SUPPORT
EXTERN int mp_query_psd(struct rtl8192cd_priv *priv, unsigned char * data, unsigned int data_len);
#endif
#ifdef MP_ADC_SUPPORT
EXTERN int mp_query_adc(struct rtl8192cd_priv *priv, unsigned char * data, unsigned int data_len);
#endif
EXTERN int mp_tx(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
EXTERN void mp_validate_rx_packet(struct rtl8192cd_priv *priv, unsigned char *data, int len);
EXTERN int mp_brx(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
EXTERN int mp_arx(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
...
EXTERN int mp_get_txpwr(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len);
...
EXTERN void get_pg_val(s16 val,char *str);
EXTERN int set_reg_pg(struct rtl8192cd_priv *priv, unsigned char *data);
EXTERN void reg_pg_info(struct rtl8192cd_priv *priv);

#ifndef CFG_HAL_PWR_REDUCE
...
EXTERN void set_pwr_reduce(struct rtl8192cd_priv *priv, unsigned char *data);
#endif
#undef EXTERN
...
EXTERN int mib_get(struct rtl8192cd_priv *priv, char *oid, unsigned char *data, unsigned int data_len, int *pLen);
...
#if defined(CONFIG_PE_ENABLE)
EXTERN struct net_device *pe_offload_dev;
EXTERN struct rtl8192cd_priv *pe_offload_priv;
#endif
#undef EXTERN
...
#define EXTERN
#endif
EXTERN void mesh_standalone_timer_expire(unsigned long task_priv);
...
#endif
EXTERN int mesh_open(DRV_PRIV *priv, struct net_device *dev);
...
EXTERN struct BandTransNode *band_trans_get(struct rtl8192cd_priv *priv, unsigned char *mac);
EXTERN void TriggerBandTrans(struct rtl8192cd_priv *priv, struct BandTransNode *blockNode);
EXTERN void stactrl_band_transition(struct rtl8192cd_priv *priv);
EXTERN bool checkBandTransBlockList(struct rtl8192cd_priv *priv, unsigned char *mac, int frame_type);
#endif
...
EXTERN unsigned char stactrl_hidden_ap(struct rtl8192cd_priv *priv);//20171124
EXTERN unsigned char stactrl_check_request_on_nonpreferband(struct rtl8192cd_priv *priv, unsigned char *mac, int frame_type, unsigned char rssi);
#else
...
EXTERN unsigned char stactrl_pass_request(struct rtl8192cd_priv *priv, unsigned char *mac, int frame_type);
#endif
...
EXTERN unsigned char stactrl_check_request_on_preferband(struct rtl8192cd_priv *priv, unsigned char *mac, int frame_type, unsigned char rssi);
#ifdef CONFIG_RTL_PROC_NEW
...
EXTERN void phy_lc_calibrate(struct rtl8192cd_priv *priv);
...
EXTERN void _phy_save_adda_registers(struct rtl8192cd_priv *priv, unsigned int *adda_reg,	unsigned int *adda_backup, unsigned int RegisterNum);
EXTERN void _PHY_SetADDARegisters(struct rtl8192cd_priv *priv, unsigned int* adda_reg,	unsigned int* ADDASettings, unsigned int RegisterNum);
EXTERN void _phy_save_mac_registers(struct rtl8192cd_priv *priv, unsigned int *MACReg, unsigned int *MACBackup);
EXTERN void _phy_reload_adda_registers(struct rtl8192cd_priv *priv, unsigned int *adda_reg, unsigned int *adda_backup, unsigned int RegiesterNum);
EXTERN void _phy_reload_mac_registers(struct rtl8192cd_priv *priv,unsigned int *MACReg, unsigned int *MACBackup);
EXTERN void _phy_mac_setting_calibration(struct rtl8192cd_priv *priv, unsigned int* MACReg, unsigned int* MACBackup);
EXTERN void _phy_path_adda_on(struct rtl8192cd_priv *priv, unsigned int* adda_reg, char isPathAOn, char is2T);
#endif
...
#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8197G)
...
EXTERN void dynamic_tx_power_ap(struct rtl8192cd_priv	*priv);
...
#ifndef CALIBRATE_BY_ODM
EXTERN void odm_txpowertracking_callback_thermal_meter_8188e(struct rtl8192cd_priv * priv);
#endif
#endif
...
EXTERN int process_p2p_get_cmd(struct rtl8192cd_priv *priv, struct iwreq *wrq);
...
EXTERN int p2p_get_role_from_ie(struct rtl8192cd_priv *priv,unsigned char *p2p_ie ,int p2pIElen );
...
EXTERN void RTL8188E_DetectSTAExistance(struct rtl8192cd_priv *priv, struct tx_rpt *report, struct stat_info *pstat);
#endif
...
#ifndef CALIBRATE_BY_ODM
EXTERN void phy_iq_calibrate_8188e(struct rtl8192cd_priv *priv, char bReCovery);
#endif
...
EXTERN void construct_netlink_send_extend(struct rtl8192cd_priv *priv);
#ifdef STA_CONTROL
EXTERN void notify_hidden_ap(struct rtl8192cd_priv *priv, int type);
#endif
EXTERN void notify_new_sta(struct rtl8192cd_priv *priv, unsigned char *mac, int type, unsigned char rssi);
EXTERN void notify_connect_fail(struct rtl8192cd_priv *priv, unsigned char *mac, int type);
EXTERN void timer_ready(struct rtl8192cd_priv *priv);
#if defined(MBSSID)
EXTERN void vap_timer_ready(struct rtl8192cd_priv *priv, int i);
#endif
...
EXTERN void smart_roaming_csa_expire_check(struct rtl8192cd_priv *priv);
...
------------------------------------------------------------------------------*/
#ifdef RTK_MULTI_AP
#ifndef _8192CD_MULTI_AP_C_
#define EXTERN extern
#else
#define EXTERN
#endif

#define MULTI_AP_DFS_COUNTDOWN 10

EXTERN int  rtk_multi_ap_init(void);
EXTERN void rtk_multi_ap_exit(void);
EXTERN unsigned char *construct_multiap_ie(unsigned char *pbuf, unsigned int *frlen, unsigned char profile, unsigned char bss_type, unsigned short vlan_id);
EXTERN void rtk_multi_ap_switch_channel(struct rtl8192cd_priv *priv);
EXTERN void client_join_notify(unsigned char bssid[MACADDRLEN], unsigned char mac[MACADDRLEN]);
EXTERN void client_leave_notify(unsigned char bssid[MACADDRLEN], unsigned char mac[MACADDRLEN]);
EXTERN void update_unassoc_MAC_RSSI_Entry(struct rtl8192cd_priv *priv,unsigned char* addr, unsigned char rssi, unsigned char status, struct sta_mac_rssi *EntryDB);
EXTERN void add_unassoc_MAC_RSSI_Entry(struct rtl8192cd_priv *priv,unsigned char* addr, unsigned char status, struct sta_mac_rssi *EntryDB, unsigned int *EntryOccupied, unsigned int *EntryNum);
EXTERN void bss_transition_response_notify(unsigned char bssid[MACADDRLEN], unsigned char mac[MACADDRLEN], unsigned char target_bssid[MACADDRLEN], unsigned char status);
EXTERN int rtk_multi_ap_getApCapability(struct rtl8192cd_priv *priv, unsigned char* result_buf2);
EXTERN int rtk_multi_ap_get_general(struct rtl8192cd_priv *priv, unsigned char *tlv_buffer, const unsigned short limit, int *processed_bytes);
EXTERN int rtk_multi_ap_set_general(struct rtl8192cd_priv *priv, unsigned char *tlv_buffer, const unsigned short limit, int *processed_bytes);
EXTERN int rtk_multi_ap_get_link_metric(struct rtl8192cd_priv *priv, unsigned char *tlv_buffer, const unsigned short limit, int *processed_bytes);
EXTERN int rtk_multi_ap_getClientCapability(struct rtl8192cd_priv *priv, unsigned char* result_buf2);
EXTERN int rtk_multi_ap_doBackhaulSteer(struct rtl8192cd_priv *priv, unsigned char *tmpbuf);
EXTERN int rtk_multi_ap_getMetric(struct rtl8192cd_priv *priv, unsigned char *buf);
EXTERN int rtk_multi_ap_getAssocStaMetric(struct rtl8192cd_priv *priv, unsigned char *buf);
EXTERN int rtk_multi_ap_recordUnassocStaMetric(struct rtl8192cd_priv *priv, unsigned char *buf);
EXTERN void rtk_multi_ap_beaconMetricsResponseNotify(struct stat_info *pstat);
EXTERN void rtk_multi_ap_updateBackhaulSteerResults(unsigned long task_priv);
EXTERN void rtk_multi_ap_ch_util_trigger(struct rtl8192cd_priv *priv);
EXTERN void rtk_multi_ap_sta_rssi_trigger(struct rtl8192cd_priv *priv, struct stat_info *pstat);
EXTERN int rtk_multi_ap_updatePolicy(struct rtl8192cd_priv *priv, unsigned char *tmpbuf);
EXTERN void rtk_multi_ap_trigger_unassocStaMetric(unsigned long task_priv);
EXTERN int rtk_multi_ap_terminateBackhaulApConnection(struct rtl8192cd_priv *priv);
EXTERN int rtk_multi_ap_terminateBackhaulStaConnection(struct rtl8192cd_priv *priv);
EXTERN int rtk_multi_ap_vxd_send_disassoc(struct rtl8192cd_priv *priv);
...
EXTERN int rtk_multi_ap_set_tx_power(struct rtl8192cd_priv *priv, unsigned char *buf);
EXTERN int rtk_multi_ap_get_available_channels(struct rtl8192cd_priv *priv, unsigned char *buf);
EXTERN void rtk_multi_ap_channel_change_notify(void);
#if (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
EXTERN void rtk_multi_ap_channel_scan_trigger_ss(struct rtl8192cd_priv *priv);
EXTERN int rtk_multi_ap_do_channel_scan(struct rtl8192cd_priv *priv, unsigned char *tmpbuf);
EXTERN int rtk_multi_ap_do_cac(struct rtl8192cd_priv *priv, unsigned char *tmpbuf);
EXTERN void rtk_multi_ap_update_channel_scan_results(struct rtl8192cd_priv *priv);
EXTERN void rtk_multi_ap_update_cac_results(unsigned long task_priv);
EXTERN void rtk_multi_ap_update_cac_status(struct rtl8192cd_priv *priv);
EXTERN void rtk_multi_ap_cac_channel_switch(unsigned long task_priv);
EXTERN void rtk_multi_ap_trigger_association_status_notification(unsigned char *bssid, unsigned char association_allowance_status);
EXTERN void rtk_multi_ap_trigger_tunneled_message(unsigned char* mac_address, unsigned char message_type, unsigned short payload_length, unsigned char *payload);
EXTERN void rtk_multi_ap_trigger_fail_connection_message(unsigned char* mac_address, unsigned short status_code, unsigned short reason_code);
EXTERN void rtk_multi_ap_trigger_disassoc_stats_message(struct stat_info *pstat, unsigned short reason_code);
#if (EASYMESH_VERSION >= MULTI_AP_VERSION(3,0,0))
EXTERN int rtk_multi_ap_set_service_prioritization_rule(struct rtl8192cd_priv *priv, unsigned char *tlv_buffer, const unsigned short limit, int *processed_bytes);
EXTERN int __rtk_multi_ap_set_service_prioritization_rule_apply(struct rtl8192cd_priv *priv, unsigned char add_remove, unsigned char output, unsigned char always_match);
EXTERN int rtk_multi_ap_set_dscp_mapping_table(struct rtl8192cd_priv *priv, unsigned char *tlv_buffer, const unsigned short limit, int *processed_bytes);
#endif // (EASYMESH_VERSION >= MULTI_AP_VERSION(3,0,0))
#endif // (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
#undef EXTERN
// #else
// #define MULTI_AP_VERSION(a,b,c) (((a) << 16) + ((b) << 8) + (c))
// #define EASYMESH_VERSION 0
#endif // end RTK_MULTI_AP 8192cd_multi_ap.c
...
#endif
EXTERN unsigned char rm_get_op_class(struct rtl8192cd_priv *priv, unsigned char channel);
EXTERN int rm_parse_beacon_report(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int len, int frame_len);
...
#endif
#ifdef STA_CONTROL_BAND_TRANSITION
EXTERN void init_band_trans_neighbor_report(struct rtl8192cd_priv *priv);
#endif
...
/*-----------------------------------------------------------------------------
								8192cd_tdma.c
------------------------------------------------------------------------------*/
#ifndef _8192CD_TDMA_C_
#define EXTERN  extern
#else
#define EXTERN
#endif
EXTERN void tdma_update_polling(struct rtl8192cd_priv *priv, unsigned short macID);
EXTERN void tdma_exit_polling(struct rtl8192cd_priv *priv, struct stat_info *pstat);
EXTERN void tdma_check_polling(struct rtl8192cd_priv *priv);
EXTERN void tdma_timeout(unsigned long task_priv);
EXTERN int get_hw_queue_num(struct rtl8192cd_priv * priv);
EXTERN int rtl8192cd_tdma_swq_enqueue(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat);
EXTERN int rtl8192cd_tdma_skb_dequeue(struct rtl8192cd_priv *priv, struct stat_info *pstat,  unsigned char qnum,unsigned int dqnum, unsigned char releaseALL);
EXTERN __MIPS16 __IRAM_IN_865X unsigned int pri_to_qnum(struct rtl8192cd_priv *priv, int priority);
EXTERN __MIPS16 __IRAM_IN_865X unsigned int get_skb_priority(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat, int is_11s);
EXTERN int rtl8192cd_swq_bdfull(struct rtl8192cd_priv *priv, unsigned int num,  unsigned char qnum);
#undef EXTERN
#endif
...
#if 1 //defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8881A) || defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_RTL_8723B_SUPPORT) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8821CE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8812FE)
...
EXTERN void update_bbrf_val8812(struct rtl8192cd_priv *priv, unsigned char channel);
...
EXTERN u1Byte fill_h2c_cmd8812(struct rtl8192cd_priv *, u1Byte , u4Byte ,	pu1Byte	);
...
#define EXTERN
#endif
EXTERN int ap_neighbor_add(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);
EXTERN void ap_neighbor_deinit(struct rtl8192cd_priv *priv);
#undef EXTERN
...
/*-----------------------------------------------------------------------------
								rtw_band_steering.c
------------------------------------------------------------------------------*/
#ifndef _RTW_BAND_STEERING_C_
#define EXTERN  extern
#else
#define EXTERN
#endif

EXTERN void _band_steering_expire(struct rtl8192cd_priv *priv);
EXTERN s32  _band_steering_block_chk(struct rtl8192cd_priv *priv, u8 *mac);
EXTERN void _band_steering_block_entry_add(struct rtl8192cd_priv *priv, u8 *mac);
EXTERN void _band_steering_block_entry_del(struct rtl8192cd_priv *priv, u8 *mac);
EXTERN void _band_steering_roam_block_entry_add(struct rtl8192cd_priv *priv, u8 *mac);
EXTERN void _band_steering_init(struct rtl8192cd_priv *priv);
...
/*-----------------------------------------------------------------------------
								rtw_wlan_manager.c
------------------------------------------------------------------------------*/
#ifndef _RTW_WLAN_MANAGER_C_
#define EXTERN  extern
#else
#define EXTERN
#endif
struct nl_message;
EXTERN void rtw_wlan_manager_recv_msg(struct nl_message *msg);
EXTERN void rtw_netlink_send_del_sta_msg(struct rtl8192cd_priv *priv, u8 *mac);
EXTERN void rtw_netlink_send_new_sta_msg(struct rtl8192cd_priv *priv, u8 *mac);
EXTERN void rtw_netlink_send_intf_rpt_msg(struct rtl8192cd_priv *priv);
EXTERN void rtw_netlink_send_sta_rpt_msg(struct rtl8192cd_priv *priv, struct stat_info *pstat);
EXTERN void rtw_netlink_send_frame_rpt_msg(struct rtl8192cd_priv *priv, u16 frame_type, u8 *sa, u8 rssi);
EXTERN void rtw_netlink_send_time_tick_msg(struct rtl8192cd_priv *priv);
EXTERN void rtw_netlink_hook(struct rtl8192cd_priv *priv, u8 band, u8 iface_id);
EXTERN void rtw_netlink_init(void);
EXTERN void rtw_netlink_deinit(void);
...
EXTERN int rtk_netlink_init(void);
EXTERN void rtk_netlink_exit(void);
EXTERN void rtk_netlink_rcv(struct sk_buff *skb);
EXTERN void rtk_netlink_send(char *data, int data_len, int to_daemon);
