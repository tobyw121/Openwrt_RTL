...
int init_p0(void)
{
	...
	REG32_ANDOR(P0GMIICR, ~((1<<4)|(3<<0)) , (1<<4) | (3<<0) );	
	...
	REG32(0xb8000040) = (REG32(0xb8000040) & ~((7<<7) | (7<<2))) | (4<<2);

	// Reset 8211 ==> GPIOA3, JTAG reset pin, pin_mux3 (0xb800004c) bit7:4 (reg_iocfg_jtag_trstn) = GPIO (4'b0110)
	REG32(0xb800004c) |= (6<<4);
	REG32(0xb8003500) &= ~(1<<GPIOA3);
	REG32(0xb8003508) |= (1<<GPIOA3);
	REG32(0xb800350c) &= ~(1<<GPIOA3);
	__mdelay(500);
	REG32(0xb800350c) |= (1<<GPIOA3);
	__mdelay(300);
...
//		smi_init(GPIO_PORT_F, 5, 6);
		...
		prom_printf(" ==> RTL8367R_init: [%d] ret= %d (0 is okay)\n",i, ret);
