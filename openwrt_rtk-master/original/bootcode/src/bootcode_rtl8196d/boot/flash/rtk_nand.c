...
#define F59L1G81A		  0x92F18095	  /* ESMT 128M*/	
...
const struct device_type nand_device[] = { 
  ...
  {"F59L1G81A",	F59L1G81A,0x8000000, 0x8000000, 2048, 1024, 64*2048, 64, 1, 0, 0xff, 0x00, 0x00, 0x00} ,
/*debug cl*/
...
int nand_write_ecc_ob (unsigned int to, unsigned int len, unsigned char *data_buf, unsigned char *oob_buf)
{
	...
	prom_printf("%s-%d: to %d, len %d oob:%x,chip_size=%x\n\r",__FUNCTION__,__LINE__,to,len,oob_buf,chip_size);
	{
        unsigned int offset=0, aa=0;
...
SKIP_BBT_CHECK:
		...
		unsigned char tmpTestData[2048];
		unsigned char tmpTestoob[64];
		int i;
		memset(tmpTestData,0xff,2048);
		memset(tmpTestoob,0xff,64);
		
		if(page == 0x600){
			prom_printf("==================================\n");
			for(i = 0;i < 2048;i++){
				prom_printf("%02x ",data_buf[data_len + i]);
			}
			prom_printf("\n");

			for(i = 0;i < 64;i++){
				prom_printf("%02x ",oob_buf[oob_len + i]);
			}
			prom_printf("\n");
		
			prom_printf("==================================\n");
			rc = rtk_read_ecc_page_a ( page, tmpTestData, tmpTestoob, page_size);

			prom_printf("data\n");
			for(i = 0;i < 2048;i++){
				prom_printf("%02x ",tmpTestData[i]);
			}
			prom_printf("\n");

			prom_printf("oob\n");
			for(i = 0;i < 64;i++){
				prom_printf("%02x ",tmpTestoob[i]);
			}
			prom_printf("\n");

			prom_printf("\n==================================\n");
			for(i = 0;i < 2048;i++){
				if(tmpTestData[i] != data_buf[data_len + i]){
					prom_printf("i=%x,val1=%x,val2=%x\n",i,tmpTestData[i],data_buf[data_len + i]);
				}
			}
		}
		#endif
...
int nand_write_ecc_ob2 (unsigned int to, unsigned int len, unsigned char *data_buf, unsigned char *oob_buf)
{
	unsigned int page, realpage;
	int data_len, oob_len;
	int rc;
...
#define PINMUX_REG_8196D_97D   0xb8000040
  ...
  REG32 (PINMUX_REG_8196D_97D) |= 0x1100;
#endif
  ...
  for (ui = 0; ui < uiCount; ui++)
      ...
      if ((nand_device[ui].id) == (id_chain))
        {
          ...
          dprintf ("2.ID= 0x%x\n", nand_device[ui].id);
          dprintf ("3.Chip_size= 0x%x = %d MB\n", nand_device[ui].chipsize,
                    nand_device[ui].chipsize / 0x00100000);
          dprintf ("4.Block_cnt= 0x%x = %d \n", nand_device[ui].BlockCnt , nand_device[ui].BlockCnt);
          dprintf ("5.Block_size= 0x%x = %d KB\n", nand_device[ui].BlockSize,
                    nand_device[ui].BlockSize / 0x400);
          dprintf ("6.Page_size= 0x%x = %d Bytes\n",
                    nand_device[ui].PageSize, nand_device[ui].PageSize);
    ...
    //rtk_writel(ECC_enable | CE_TWP(3) | CE_TWH(3) | CE_TRR(3) | CE_TH(3) | CE_TS(5), NACR);
    dprintf ("\nAfter NAND probed,set safe parameter NACR=0x%X",
             rtk_readl (NACR));
  dprintf ("\n========================================= \n");
...
#if 0
    memcpy (&nand_info, &nand_device[nand_index],
...
int 
isBadBlockCreate (int block_start_cnt, int block_end_cnt ) 
{
  char *nand_verify_buf;
  nand_verify_buf = malloc (page_size + oob_size);     //Based on minimum NAND DMA size
  int block_indicator;         //"1':good block , "0":bad block
...
unsigned char*  tmp_oob = (unsigned char*)malloc((sizeof(char)*ppb*oob_size));
//unsigned char*  tmp_oob = (unsigned char*)malloc((sizeof(char)*64*64));
unsigned char* dataBuf = (unsigned char*)0x81000000;
        ...
        block_scan_loop++)
 {
 	flag = 0;
 	#if 1
 	for(j = 1;j <= block_scan_loop_bad;j++){
		if(initial_bad_block_table[j] == block_scan_loop){
			flag = 1;
			break;
		}
 	}
	...
	//if(flag == 1)
 		//memset(tmp_oob,0,ppb*oob_size);
 	//else
 		memset(tmp_oob,0xff,ppb*oob_size);

 	unsigned int addr = (block_scan_loop << block_shift);
	nand_write_ecc_ob2(addr, block_size, dataBuf, tmp_oob);
 }
  

//  rtk_write_ecc_page_a(,imageBuf,oobBuf,2048)
