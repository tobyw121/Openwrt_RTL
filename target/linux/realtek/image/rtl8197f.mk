...
define Device/iball_wrd12gn
  DEVICE_VENDOR := iBall
  DEVICE_MODEL := Baton WRD12GN
  DEVICE_DTS := rtl8197f_iball_wrd12gn
  KERNEL := \
    kernel-bin | \
    append-dtb | \
    lzma | \
  ...
  KERNEL_INITRAMFS := \
    ...
    lzma | \
    uImage lzma
  # Complete writable stock firmware window: 0x030000..0x790000.
  IMAGE_SIZE := 7552k
  BLOCKSIZE := 4k
  ...
  RTL8197F_KERNEL_CMDLINE := console=ttyS0,38400n8 lpj=4000000 rootfstype=squashfs,jffs2 clk_ignore_unused
  RTL8197F_HEADER_SIZE := 16
  SUPPORTED_DEVICES += iball,wrd12gn
  DEVICE_PACKAGES += ethtool dnsmasq dropbear firewall4 iw wireless-regdb hostapd-mini rtl8197f-iball-wrd12gn-base kmod-rtl8192cd-rtl8197f-wrd12gn
  IMAGES := sysupgrade.bin firmware-cr6c.bin $(IBALL_WRD12GN_FULLFLASH_IMAGE)
  ...
  IMAGE/squashfs-spi-full.bin := append-kernel | rtl8197f-image | append-rtl8197f-rootfs | rtl8197f-firmware-budget | pad-rootfs | rtl8197f-firmware-budget | rtl8197f-iball-wrd12gn-fullflash
endef
TARGET_DEVICES += iball_wrd12gn
...
define Device/xiaomi_r4_rd05
  ...
  DEVICE_PACKAGES += ip-full ip-bridge ethtool dnsmasq odhcpd-ipv6only firewall4 luci uhttpd uhttpd-mod-ubus rpcd rtl8367d-compat rtl8197f-rd05-base kmod-rtl8192cd-rtl8197f-rd05 wireless-tools
...
define Device/tenda_ac23
  ...
  DEVICE_PACKAGES += ip-full ip-bridge ethtool dnsmasq odhcpd-ipv6only firewall4 rtl8197f-tenda-ac23-base kmod-rtl8192cd-rtl8197f-ac23 wireless-tools
...
define Device/tenda_nova_mw5
  ...
  # pad-rootfs normally appends its 4-byte EOF marker after alignment; with the
  # MW5 image close to the fixed window end that becomes 0x590004.  The -x 4
  # offset places the marker inside the final aligned 0x590000 byte window.
  BLOCKSIZE := 64k
  RTL8197F_IMAGE_SIGNATURE := cr6c
  ...
  RTL8197F_LZMA_TEXT_START := 0x80d00000
  RTL8197F_BURNADDR := 0x00030000
  RTL8197F_KERNEL_CMDLINE := console=ttyS0,115200n8 lpj=5000000 rootfstype=squashfs,jffs2 clk_ignore_unused ipv6.disable=1 loglevel=4
  RTL8197F_HEADER_SIZE := 16
  SUPPORTED_DEVICES += tenda,nova-mw5
  # v44.62.3: board-only RD05 userspace files were split out of the common
  # Realtek base-files overlay into rtl8197f-rd05-base.  The MW5 image no
  # longer pays for rd05-ioctl/UART/DSA/firewall defaults inside its fixed
  # 0x590000-byte KernelFS window; MW5 WLAN/Ethernet/WebUI/PCAP stay intact.
  # v44.62.4: compact the staged MW5 shell scripts before squashfs creation.
  # The readable sources remain unchanged; only comments and blank lines outside
  # here-documents are removed from the installed copies. This recovers several
  # KiB after XZ without removing WLAN, Ethernet, WebUI, PCAP or diagnostics.
  # Flash-safe 8 MiB profile.  Keep the functional IPv4 router path
  # (dnsmasq + firewall4), both WLAN radios and cfg80211/nl80211 AP control,
  # but remove tools that are not required for boot/routing. The strict budget gate
  # measures the actual result; v44.62.4 does not relax the verified flash limit.
  # The large overlay-repair utility package was tested but removed again: MW5 8 MiB image exceeded the fixed SPI window by one erase block.
  # v44.62.13: the MW5 uses its own mw5-wifi + hostapd/nl80211 control path.
  # Generic wifi-scripts/iwinfo are therefore omitted only for this device;
  # iw + wireless-regdb stay installed. This recovers real squashfs headroom
  # without changing IMAGE_SIZE, BLOCKSIZE or any OEM flash boundary.
  ...
  IMAGE/sysupgrade.bin := append-kernel | rtl8197f-image | append-rtl8197f-rootfs | rtl8197f-firmware-budget | pad-rootfs -x 4 | rtl8197f-firmware-budget | append-metadata
  IMAGE/firmware-cr6c.bin := append-kernel | rtl8197f-image | append-rtl8197f-rootfs | rtl8197f-firmware-budget | pad-rootfs -x 4 | rtl8197f-firmware-budget
  IMAGE/squashfs-spi-full.bin := append-kernel | rtl8197f-image | append-rtl8197f-rootfs | rtl8197f-firmware-budget | pad-rootfs -x 4 | rtl8197f-firmware-budget | rtl8197f-tenda-mw5-fullflash
endef
