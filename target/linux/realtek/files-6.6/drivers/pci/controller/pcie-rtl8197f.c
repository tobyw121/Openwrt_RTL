...
static int rtl8197f_pcie_probe(struct platform_device *pdev)
{
	...
	struct resource *rc_res, *ep_res;
	u32 value;
	int ret;

	/* v44.4 MW5 reference recovered from the OEM WLAN device table:
	 * cfg 0x18b10000, BAR/MMIO aperture 0x19000000, IRQ5, ID 10ec:b822.
	 */
	dev_info(dev, "probe enter: OEM MW5 expects endpoint 10ec:b822 on IRQ5\n");
	...
	rc_res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "rc");
	ep_res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "ep");
	if (!rc_res || !ep_res)
		return dev_err_probe(dev, -EINVAL, "missing rc/ep resources\n");

	dev_info(dev, "resources: rc=%pa ep=%pa OEM-cfg=0x18b10000 OEM-mem=0x19000000\n",
		 &rc_res->start, &ep_res->start);

	pcie->rc = devm_ioremap_resource(dev, rc_res);
	if (IS_ERR(pcie->rc))
		return dev_err_probe(dev, PTR_ERR(pcie->rc), "cannot map RC window\n");

	pcie->ep = devm_ioremap_resource(dev, ep_res);
	if (IS_ERR(pcie->ep))
		return dev_err_probe(dev, PTR_ERR(pcie->ep), "cannot map endpoint config window\n");
	...
	if (pcie->irq < 0)
		return dev_err_probe(dev, pcie->irq, "cannot map PCIe IRQ\n");
	dev_info(dev,
		 "MW5 PCIe/WLAN IRQ mapped: linux_irq=%d ICTL-input=21 route=CPU5\n",
		 pcie->irq);
	...
	if (ret)
		return dev_err_probe(dev, ret, "RTL8197F PCIe hardware init failed\n");
	...
	/*
	 * devm_pci_alloc_host_bridge() already called devm_of_pci_bridge_init(),
	 * which parsed the DT ranges and reserved bridge->windows.  Do not add
	 * or request the same 0x18c00000 IO / 0x19000000 MEM apertures again:
	 * doing so self-conflicts with the resources owned by this very device.
	 */
	dev_info(dev, "using OF PCI windows reserved by devm_pci_alloc_host_bridge\n");
...
/*
 * Some RTL8197F OpenWrt platform-device builds expose the OF node as the
 * address-derived platform name instead of matching the OF table during the
 * initial driver-core pass.  Keep the OF match as the primary path and add
 * exact name fallbacks for the controller at physical 0x18b00000.
 */
static const struct platform_device_id rtl8197f_pcie_id_table[] = {
	{ "rtl8197f-pcie", 0 },
	{ "18b00000.pcie", 0 },
	{ "b00000.pcie", 0 },
	{ }
};
MODULE_DEVICE_TABLE(platform, rtl8197f_pcie_id_table);

static struct platform_driver rtl8197f_pcie_driver = {
	.probe = rtl8197f_pcie_probe,
	.remove_new = rtl8197f_pcie_remove,
	.id_table = rtl8197f_pcie_id_table,
	.driver = {
		.name = "rtl8197f-pcie",
