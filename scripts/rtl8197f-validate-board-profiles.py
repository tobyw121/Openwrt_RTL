...
"""Validate the RTL8197F board evidence database against DTS/image files."""
...
DTS = {
    ...
    "iball,wrd12gn": TOP / "target/linux/realtek/dts/rtl8197f_iball_wrd12gn.dts",
}
...
def main() -> int:
    ...
    for board, path in DTS.items():
        ...
        if board == "iball,wrd12gn":
            # DTC requires all properties in a node before its first child node.
            # Keep ethernet0 status before the embedded MDIO bus; v44.65.2 had
            # status after mdio and therefore failed target/linux/install.
            eth_start = text.find("&ethernet0 {")
            eth_end = text.find("\n};\n\n&wlan", eth_start)
            if eth_start < 0 or eth_end < 0:
                errors.append("WRD12GN DTS: unable to isolate ethernet0 node")
            else:
                eth = text[eth_start:eth_end]
                status_pos = eth.find('status = "okay";')
                mdio_pos = eth.find("\n\tmdio {")
                if status_pos < 0 or mdio_pos < 0 or status_pos > mdio_pos:
                    errors.append("WRD12GN DTS: ethernet0 properties must precede mdio child node")
    ...
    wrd12gn_image = device_block("iball_wrd12gn")
    ...
    require(wrd12gn_image, "RTL8197F_HEADER_SIZE := 16", "WRD12GN image", errors)
    require(wrd12gn_image, "BLOCKSIZE := 4k", "WRD12GN image", errors)
    require(wrd12gn_image, "IMAGE_SIZE := 7552k", "WRD12GN image", errors)
