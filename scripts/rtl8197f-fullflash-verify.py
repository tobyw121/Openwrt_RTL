...
def main() -> int:
    ...
    ap.add_argument('--board',required=True,choices=('rd05','mw5','ac23','wrd12gn'))
    ap.add_argument('--template',required=True,type=Path)
    ap.add_argument('--image',type=Path)
    args=ap.parse_args()
    t=args.template.read_bytes()
    sizes={'rd05':0x1000000,'mw5':0x800000,'ac23':0x800000,'wrd12gn':0x800000}
    ...
    if args.board=='rd05':
        ...
        length,end=header(t,0x30000,0x80a00000,0x30000)
        if t[0x20000:0x20004]!=b'H601': raise SystemExit('ERROR: WRD12GN H601 factory marker missing')
        if t[0x230000:0x230004]!=b'hsqs': raise SystemExit('ERROR: WRD12GN stock rootfs magic missing at 0x230000')
        preserved=((0,0x30000),(0x790000,0x800000))
