...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--length-includes-header-padding", action="store_true",
                    help="include bytes between the 16-byte logical header and payload in IMG_HEADER_T.len")
    ...
    # Most RTL8197 images count only payload+checksum.  Some vendor payloads
    # (currently the AC23 reconstruction) include bytes between the logical
    # IMG_HEADER_T and executable payload in len.  This option is valid only
    # when the payload itself was linked for that prefix.  MW5 must use a
    # 16-byte header because its OpenWrt loader is linked at 0x80a00000.
    length = len(payload) + len(payload_pad) + 2
    if args.length_includes_header_padding:
        length += args.header_size - 16
