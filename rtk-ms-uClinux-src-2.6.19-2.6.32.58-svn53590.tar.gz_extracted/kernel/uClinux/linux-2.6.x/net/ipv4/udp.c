 ...
 * 2013-04-10	:	UDP Source Port Randomization
 *   UDP source port randomization merged from the kernel 2.6.24 above.
 */
		 ...
		 int (*saddr_cmp)(const struct sock *sk1, const struct sock *sk2))
{
	...
	/* RTK Patch: UDP source port randomization.
	 * UDP source port randomization from the 2.6.24 above.
	 */
	if (!snum) {
		...
		for (i = 0; i < UDP_HTABLE_SIZE; i++) {
			...
			head = &udp_hash[rover & (UDP_HTABLE_SIZE - 1)];
			if (hlist_empty(head))
				goto gotit;
