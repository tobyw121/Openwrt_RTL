...
#define socket_fd (pfd[0].fd)
    ...
    /* RTK: TFTP broadcast support (Start) */
	setsockopt_broadcast(socket_fd);
    /* RTK: TFTP broadcast support (End) */
...
# endif
 ...
 recv_again:
		...
		if (opcode == TFTP_ERROR) {
			...
			/* RTK: TFTP error reason export (Start) */
			{
				FILE *_fp = NULL;
				if (NULL != (_fp = fopen("/var/tftperr", "w")))
				{
					fprintf(_fp, "%u", recv_blk);
					fclose(_fp);
				}
			}
			/* RTK: TFTP error reason export (End) */
			goto ret;
		}
