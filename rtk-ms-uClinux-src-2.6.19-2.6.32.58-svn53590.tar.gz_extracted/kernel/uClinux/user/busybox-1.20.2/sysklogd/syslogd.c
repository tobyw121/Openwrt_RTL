...
#if CONFIG_RTK_PATCH
#define RTK_SYSLOG_MSG_PREFIX	"syslog: "
#define RTK_KERNEL_MSG_PREFIX	"kernel: "

#define RTK_TIMEZONE_SIGNAL		SIGUSR1
#define RTK_TIMEZONE_FILE       "/tmp/timezone_diff"

#define RTK_SYSLOGD_GATEWAY		"eth0"
#endif
...
static void parse_syslogdcfg(const char *file)
{
	...
	parser = config_open2(file ? file : "/etc/syslogd.conf",
				file ? xfopen_for_read : fopen_for_read);
	if (!parser)
		/* didn't find default /etc/syslogd.conf */
	...
	while (config_read(parser, tok, 4, 3, "# \t", PARSE_NORMAL | PARSE_MIN_DIE)) {
		logType_t type = SYSLOG_TYPE_NONE;
		char *cur_selector;
		logRule_t *cur_rule;
		...
		if (!strcmp(tok[0], "local-ram"))
			type = SYSLOG_TYPE_LOCAL;
		else if (!strcmp(tok[0], "local-console"))
			type = SYSLOG_TYPE_CONSOLE;
		else if (!strcmp(tok[0], "local-flash"))
			type = SYSLOG_TYPE_FLASH;
		else if (!strncmp(tok[0], "server", 6) &&
				 tok[0][6] >= '0' && tok[0][6] <= '9')
			type = SYSLOG_TYPE_REMOTE;
		else
			type = SYSLOG_TYPE_NONE;

		if (type == SYSLOG_TYPE_NONE)
			continue;
		else if (type == SYSLOG_TYPE_CONSOLE)
			*token[2] = _PATH_CONSOLE;
 ...
 next:
		...
		if (type == SYSLOG_TYPE_LOCAL || type == SYSLOG_TYPE_CONSOLE || type == SYSLOG_TYPE_FLASH) {
			/* local */
			/* check whether current file name was mentioned in previous rules or
			 * as global logfile (G.logFile).
			 */
			if (strcmp(G.logFile.path, tok[2]) == 0) {
				cur_rule->target.file = &G.logFile;
				goto found;
			}
			...
			for (cur_rule = G.log_rules; cur_rule != *pp_rule; cur_rule = cur_rule->next) {
				if (cur_rule->type == type &&
					strcmp(cur_rule->target.file->path, tok[2]) == 0
				) {
					/* found - reuse the same file structure */
					(*pp_rule)->target.file = cur_rule->target.file;
					cur_rule = *pp_rule;
					goto found;
				}
			}
			cur_rule->target.file = xzalloc(sizeof(*cur_rule->target.file));
			cur_rule->target.file->fd = -1;
			cur_rule->target.file->path = xstrdup(tok[2]);

			if (type == SYSLOG_TYPE_LOCAL) {
				cur_rule->target.file->logFileSize = 1 << CONFIG_USER_SYSLOG_RAM_SIZE;
				cur_rule->target.file->logFileRotate = 0;
			}
			else {
				cur_rule->target.file->logFileSize = 1 << CONFIG_USER_SYSLOG_FLASH_SIZE;
				cur_rule->target.file->logFileRotate = 0;
			}
		}
		else if (type == SYSLOG_TYPE_REMOTE) {
			/* remote */
			char *host;
			uint32_t port;
			...
			/* get host and port */
			host = tok[2];
			cur_selector = strrchr(host, ':');
			if (cur_selector)
				*cur_selector++ = '\0';

			port = bb_strtou(cur_selector, NULL, 10);
			if (errno || port > 0xff)
				port = 514;
			...
			for (cur_rule = G.log_rules; cur_rule != *pp_rule; cur_rule = cur_rule->next) {
				if (cur_rule->type == type &&
					strcmp(cur_rule->target.remote->remoteHostname, host) == 0 && 
					cur_rule->target.remote->remotePort == port
				) {
					/* found - reuse the same file structure */
					(*pp_rule)->target.remote = cur_rule->target.remote;
					cur_rule = *pp_rule;
					goto found;
				}
			}
			cur_rule->target.remote = xzalloc(sizeof(*cur_rule->target.remote));
			cur_rule->target.remote->remoteFD = -1;
			cur_rule->target.remote->last_dns_resolve = monotonic_sec() - DNS_WAIT_SEC - 1;
			cur_rule->target.remote->remoteHostname = xstrdup(host);
			cur_rule->target.remote->remotePort = port;
			cur_rule->target.remote->facility_alias = LOG_FAC(LOG_LOCAL7);

			t = strchr(tok[0], '=');
			if (t) {
				cur_rule->target.remote->facility_alias = bb_strtou(cur_selector, NULL, 10);
				if (errno || cur_rule->target.remote->facility_alias > LOG_FAC(LOG_LOCAL7))
					cur_rule->target.remote->facility_alias = LOG_FAC(LOG_LOCAL7);
			}
		}
		else {
...
#if CONFIG_RTK_PATCH
static void update_timezone(int sig)
{
    FILE *fp = NULL;

	if (sig != RTK_TIMEZONE_SIGNAL)
		return;

	timezone = 0;

	fp = fopen("RTK_TIMEZONE_FILE", "r+");
	if (fp != NULL) {
		fscanf(fp, "%d", &G.timezone);
		fclose(fp);
	}
}

static void log_console(char *msg)
{
	int fd;

	/* Always send console messages to /dev/console so people will see them. */
	if ((fd = device_open(_PATH_CONSOLE, O_WRONLY | O_NOCTTY |
...
static int try_to_resolve_remote(remoteHost_t *rh)
{
	if (!rh->remoteAddr) {
		...
		rh->remoteAddr = host2sockaddr(rh->remoteHostname, rh->remotePort);
		if (!rh->remoteAddr)
			return -1;
	}
...
static void log_remote(char *msg, remoteHost_t *rh)
{
	 ...
	 */
	if (sendto(rh->remoteFD, msg, strlen(msg),
...
static void log_locally(time_t now, char *msg, logFile_t *log_file)
{
 ...
 reopen:
		log_file->fd = open(log_file->path, O_RDWR | O_CREAT | O_NOCTTY |
...
#if ENABLE_FEATURE_ROTATE_LOGFILE
	if (log_file->logFileSize && log_file->isRegular && log_file->size > log_file->logFileSize) {
		if (log_file->logFileRotate) { /* always 0..99 */
			int i = strlen(log_file->path) + 3 + 1;
			char oldFile[i];
			char newFile[i];
			i = log_file->logFileRotate - 1;
	...
	if (log_file->logFileSize && log_file->isRegular && log_file->size > log_file->logFileSize) {
		int tmpfd;
		int i = strlen(log_file->path) + 1 + 1;	 /* always filename~ */
		char tmpFile[i];
		char tmpbuf[256];
		char *c = NULL;
		ssize_t r = -1;
		off_t off = (log_file->size - log_file->logFileSize) + (log_file->logFileSize / 10 * 2);

		sprintf(tmpFile, "%s%c", log_file->path, '~');

		tmpfd = open(tmpFile, O_RDWR | O_CREAT | O_TRUNC | O_NOCTTY | O_APPEND | O_NONBLOCK, 0666);
		if (tmpfd >= 0) {
			while ((r = safe_read(log_file->fd, tmpbuf, sizeof(tmpbuf))) > 0) {
				c = strchr(tmpbuf, '\n');
				if (c) {
					off += (c - tmpbuf) + 1;
					break;
			...
			lseek(log_file->fd, off, SEEK_SET);

			/* copy and rename */
			bb_copyfd_eof(log_file->fd, tmpfd);
			dup2(tmpfd, log_file->fd);
			rename(tmpFile, log_file->path);
...
static void timestamp_and_log(int pri, char *msg, int len)
{
	...
	/* skip the leading Jan 18 00:11:22 msg... */
	...
	if (LOG_FAC(pri) > LOG_FAC(LOG_USER))
		return;

	/* Drop leading "syslog: " or "kernel: " */
	if (!strncmp(msg, RTK_SYSLOG_MSG_PREFIX, strlen(RTK_SYSLOG_MSG_PREFIX)))
		msg += strlen(RTK_SYSLOG_MSG_PREFIX);
	else if (!strncmp(msg, RTK_KERNEL_MSG_PREFIX, strlen(RTK_KERNEL_MSG_PREFIX)))
		msg += strlen(RTK_KERNEL_MSG_PREFIX);
	...
	/* junk message ? */
	if (!strcmp(msg, SYS_LOG_JUNK_MSG))
		return;
	...
	now += G.timezone * 60;
	tm = localtime(&now);
	strftime(timestamp, 32, "%b %d %T" , tm);

#if ENABLE_FEATURE_REMOTE_LOG
	sock_fd = socket(PF_INET, SOCK_DGRAM, 0);
	if (sock_fd >= 0) {
		strcpy(ifr.ifr_name, RTK_SYSLOGD_GATEWAY);
...
#if ENABLE_FEATURE_SYSLOGD_CFG
	{
		...
		for (rule = G.log_rules; rule; rule = rule->next) {
			if (rule->enabled_facility_priomap[facility] & prio_bit) {
				switch (rule->type) {
				case SYSLOG_TYPE_LOCAL:
				case SYSLOG_TYPE_FLASH:
					sprintf(G.printbuf, "%d;%s;%s\n", LOG_PRI(pri), timestamp, msg);
					log_locally(now, G.printbuf, rule->target.file);
					break;

				case SYSLOG_TYPE_CONSOLE:
					sprintf(G.printbuf, "*%s: %s\n", timestamp, msg);
					log_console(G.printbuf);
					break;

#if ENABLE_FEATURE_REMOTE_LOG
				case SYSLOG_TYPE_REMOTE:
					sprintf(G.printbuf, "<%d>%s %s %s\n",
							LOG_MAKEPRI(rule->target.remote->facility_alias,
							LOG_PRI(pri)), timestamp, ipaddr, msg);
					log_remote(G.printbuf, rule->target.remote);
					break;
#endif
				default:
					break;
...
#if CONFIG_RTK_PATCH
	signal(RTK_TIMEZONE_SIGNAL, update_timezone);
	update_timezone(RTK_TIMEZONE_SIGNAL);
#endif
