...
#ifdef CONFIG_FEATURE_INSMOD_LOAD_MAP
	while ((opt = getopt(argc, argv, "fkqsvxmdLo:")) > 0)
#else
	while ((opt = getopt(argc, argv, "fkqsvxdLo:")) > 0)
#endif
#else
#ifdef CONFIG_FEATURE_INSMOD_LOAD_MAP
...
extern int insmod_ng_main( int argc, char **argv)
{
	...
	while ((opt = getopt(argctmp, argvtmp, "d")) > 0)
		{
			switch (opt) {
				case 'd':
					flag_decry = 1;
	...
	{
		flag_decry=0;
//		ret = syscall(__NR_decry_module, map, len, options); /*MK by SD6*/
	}
	else
	{
