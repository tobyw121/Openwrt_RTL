		...
		while (wpid > 0) {
			...
			for (a = init_action_list; a; a = a->next) {
				if (a->pid == wpid) {
					...
					/* RTK: Console block reminder (Start) */
					if (a->runcount >= MAXSPAWN - 1 && a->runcount < MAXSPAWN) {
					    if (0 == strcmp(a->command, "/bin/sh")) {
    						message(CONSOLE, "CLI is restarting too fast.\n"
							                 "Console will be blocked %d seconsd if you restart CLI less than %d seconds next time.\n",
							                 SLEEPTIME, TESTTIME);
						}
					}
					/* RTK: Console block reminder (End) */
					...
					if (a->runcount >= MAXSPAWN) {
						...
						/* RTK: Console block reminder (Start) */
					    if (0 == strcmp(a->command, "/bin/sh")) {
    						message(CONSOLE, "Console is blocking %d seconsd\n", SLEEPTIME);
						}
					    /* RTK: Console block reminder (End) */
					} else {
