...
static void doKlogd(const int console_log_level)
{
	...
	while (1) {
        ...
        //klogctl(6, log_buffer, sizeof(log_buffer));
		...
		for (i = 0; i < n; i++) {
			...
			if (log_buffer[i] == '\n') {
                ...
                if (0 == strncmp(start, SYS_LOG_KERN_KEY, strlen(SYS_LOG_KERN_KEY)))
                {
                    start += strlen(SYS_LOG_KERN_KEY);
                    do_syslog(priority, start);
                }
