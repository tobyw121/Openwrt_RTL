...
#define SYSLOG_TARGET_LOCAL_FLASH 3
#define SYSLOG_TARGET_LOCAL_CONSOLE 4


#define SYSLOG_MAX_INDEX 24    /*unit is KB, the size is 2^14 K*/
#define SYSLOG_MIN_INDEX  12    /*unit is KB, the size is 2^24 k*/
...
typedef struct {
	...
	/* This is to store in the FLASH */
	syslogd_local_config_t flash;
} syslogd_config_t;

int syslogd_write_pid(char* filename);
