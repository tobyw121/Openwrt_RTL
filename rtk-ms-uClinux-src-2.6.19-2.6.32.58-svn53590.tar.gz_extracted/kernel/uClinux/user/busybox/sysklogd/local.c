...
again:
	if ((fd =
			 device_open(local->logfile,
							 ...
							 O_RDWR | O_CREAT | O_NOCTTY | O_APPEND |
							 O_NONBLOCK)) >= 0) {
		fl.l_type = F_WRLCK;
		...
		if (local->maxsize > 0) {
			...
			if (!fstat(fd, &statf) &&
				...
				statf.st_size > local->maxsize << 10)
			{
				if (local->numfile > 0) {
					debug_printf("rotating");
                
					for (i = local->numfiles - 1; i > 0; i--) {
							snprintf(old_file, sizeof old_file, "%s.%d", local->logfile, i - 1);
							snprintf(new_file, sizeof new_file, "%s.%d", local->logfile, i);
    	                    rename(old_file, new_file);
					}

					snprintf(new_file, sizeof new_file, "%s.%d", local->logfile, 0);
					rename(local->logfile, new_file);
		...
		if (local->maxsize > 0) {
			...
			char *c = NULL;
			ssize_t r = -1;
			off_t off = (local->maxsize << 10) / 10 * 2;

			if (!fstat(fd, &statf) &&
				S_ISREG(statf.st_mode) && 
				statf.st_size >= (local->maxsize << 10) &&
				(off += (statf.st_size - (local->maxsize << 10))) &&
				lseek(fd, off, SEEK_SET) >= 0)
			{
				snprintf(tmpfile, sizeof tmpfile, "%s%s", local->logfile, "~");

				tmpfd = device_open(tmpfile,
									O_RDWR | O_CREAT | O_NOCTTY | O_TRUNC | O_APPEND | O_NONBLOCK);
				if (tmpfd >= 0) {
					fl.l_type = F_WRLCK;
					fcntl (tmpfd, F_SETLKW, &fl);
				}

				while ((r = safe_read(fd, strbuf, sizeof(strbuf))) > 0) {
					c = strchr(strbuf, '\n');
					if (c) {
						off += (c - strbuf) + 1;
                        break;
				...
				if (c && tmpfd >= 0 &&
					lseek(fd, off, SEEK_SET) >= 0 &&
					bb_copyfd_eof(fd, tmpfd) >= (statf.st_size - off) &&
					rename(tmpfile, local->logfile) == 0)
				{
					fl.l_type = F_UNLCK;
					fcntl (fd, F_SETLKW, &fl);
