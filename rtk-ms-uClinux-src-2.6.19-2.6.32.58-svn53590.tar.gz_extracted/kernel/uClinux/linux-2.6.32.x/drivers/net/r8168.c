			...
			void __iomem *ioaddr)
{
    ...
    printk("MAC: val32=%x, reg=%x, ICVerID=%x\n", val32, reg, ICVerID);
}
		   ...
		   void __iomem **ioaddr_out)
{
	...
	struct net_device *dev;
	struct rtl8168_private *tp;
	int rc = -ENOMEM, acpi_idle_state = 0, pm_cap;
		 ...
		 const struct pci_device_id *ent)
{
	...
	tp->max_jumbo_frame_size = rtl_chip_info.jumbo_frame_sz;
...
rtl8168_open(struct net_device *dev)
{
	...
	retval = request_irq(dev->irq, rtl8168_interrupt, 
                         (tp->features & RTL_FEATURE_MSI) ? 0 : IRQF_SHARED, dev->name, dev);
...
rtl8168_hw_start(struct net_device *dev)
{
	...
	if (tp->rx_fifo_overflow == 0) {
		...
		RTL_W16_C(IntrMask, rtl8168_intr_mask);
		     ...
		     void __iomem *ioaddr, u32 budget)
{
	...
	rx_left = rtl8168_rx_quota(rx_left, RTL_RX_QUOTA(dev, budget));
...
static irqreturn_t rtl8168_interrupt(int irq, void *dev_instance)
{
	...
	do {
		...
		if (status & rtl8168_napi_event) {
			...
			RTL_W16_C(IntrMask, rtl8168_intr_mask & tp->intr_mask);
...
out:
	RTL_W16_C(IntrMask, tp->intr_mask);
...
rtl8168_set_rx_mode(struct net_device *dev)
{
	...
	tp->rtl8168_rx_config = rtl_chip_info.RCR_Cfg;
	tmp = tp->rtl8168_rx_config | rx_mode | (RTL_R32(RxConfig) & rtl_chip_info.RxConfigMask);
