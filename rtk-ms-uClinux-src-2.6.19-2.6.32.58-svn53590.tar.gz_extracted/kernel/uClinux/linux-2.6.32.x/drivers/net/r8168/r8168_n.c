...
_R ("RTL8168B/8111B",
    ...
    _R ("RTL8168B/8111B",
          CFG_METHOD_2,
          (Reserved2_data << Reserved2_shift) | (RX_DMA_BURST <<
                                                   RxCfgDMAShift),
          0xff7e1880, Jumbo_Frame_4k), _R ("RTL8168B/8111B", CFG_METHOD_3,
                                             (Reserved2_data <<
                                                Reserved2_shift) |
                                             (RX_DMA_BURST <<
                                                RxCfgDMAShift), 0xff7e1880,
                                             Jumbo_Frame_4k),
    _R ("RTL8168C/8111C", CFG_METHOD_4,
          RxCfg_128_int_en | RxCfg_fet_multi_en | (RX_DMA_BURST <<
                                                     RxCfgDMAShift),
          0xff7e1880, Jumbo_Frame_6k), _R ("RTL8168C/8111C", CFG_METHOD_5,
                                             RxCfg_128_int_en |
                                             RxCfg_fet_multi_en |
                                             (RX_DMA_BURST <<
                                                RxCfgDMAShift), 0xff7e1880,
    ...
    _R ("RTL8168C/8111C", CFG_METHOD_6,
          RxCfg_128_int_en | RxCfg_fet_multi_en | (RX_DMA_BURST <<
                                                     RxCfgDMAShift),
          0xff7e1880, Jumbo_Frame_6k), _R ("RTL8168CP/8111CP", CFG_METHOD_7,
                                             RxCfg_128_int_en |
                                             RxCfg_fet_multi_en |
                                             (RX_DMA_BURST <<
                                                RxCfgDMAShift), 0xff7e1880,
    ...
    _R ("RTL8168CP/8111CP", CFG_METHOD_8,
          RxCfg_128_int_en | RxCfg_fet_multi_en | (RX_DMA_BURST <<
                                                     RxCfgDMAShift),
          0xff7e1880, Jumbo_Frame_6k), _R ("RTL8168D/8111D", CFG_METHOD_9,
                                             RxCfg_128_int_en |
                                             (RX_DMA_BURST <<
                                                RxCfgDMAShift), 0xff7e1880,
                                             Jumbo_Frame_9k),
    _R ("RTL8168D/8111D", CFG_METHOD_10,
          ...
          Jumbo_Frame_9k), _R ("RTL8168DP/8111DP", CFG_METHOD_11,
                                 RxCfg_128_int_en | (RX_DMA_BURST <<
                                                       RxCfgDMAShift),
                                 0xff7e1880, Jumbo_Frame_9k),
    _R ("RTL8168DP/8111DP", CFG_METHOD_12,
          RxCfg_128_int_en | (RX_DMA_BURST << RxCfgDMAShift), 0xff7e1880,
          Jumbo_Frame_9k), _R ("RTL8168DP/8111DP", CFG_METHOD_13,
                                 RxCfg_128_int_en | (RX_DMA_BURST <<
                                                       RxCfgDMAShift),
                                 0xff7e1880, Jumbo_Frame_9k),
    _R ("RTL8168E/8111E", CFG_METHOD_14,
          RxCfg_128_int_en | (RX_DMA_BURST << RxCfgDMAShift), 0xff7e1880,
          Jumbo_Frame_9k), _R ("RTL8168E/8111E", CFG_METHOD_15,
                                 RxCfg_128_int_en | (RX_DMA_BURST <<
                                                       RxCfgDMAShift),
                                 0xff7e1880, Jumbo_Frame_9k),};
#undef _R
...
static int rtl8168_rx_interrupt (struct net_device *,
...
static int rtl8168_set_speed (struct net_device *dev, u8 autoneg, u16 speed,
                              u8 duplex);
...
static inline u32
netif_msg_init (int debug_value, int default_msg_enable_bits)
{
  /* use default */
...
static inline void
eth_copy_and_sum (struct sk_buff *dest,
...
_kc_jiffies_to_msecs (const unsigned long j)
{
...
_kc_msecs_to_jiffies (const unsigned int m)
{
...
pci_power_t
pci_choose_state (struct pci_dev *dev, pm_message_t state)
{
...
static void
mdio_write (struct rtl8168_private *tp, u32 RegAddr, u32 value)
{
...
static u32
mdio_read (struct rtl8168_private *tp, u32 RegAddr)
{
  void __iomem *ioaddr = tp->mmio_addr;
  int i, value = -1;
...
static u32
OCP_read (struct rtl8168_private *tp, u8 mask, u16 Reg)
{
  void __iomem *ioaddr = tp->mmio_addr;
  int i;
...
static void
OCP_write (struct rtl8168_private *tp, u8 mask, u16 Reg, u32 data)
{
  void __iomem *ioaddr = tp->mmio_addr;
  int i;
...
static void
OOB_mutex_lock (struct rtl8168_private *tp)
{
...
static void
OOB_mutex_unlock (struct rtl8168_private *tp)
{
...
static void
OOB_notify (struct rtl8168_private *tp, u8 cmd)
{
  void __iomem *ioaddr = tp->mmio_addr;
  int i;
...
static void
rtl8168_mac_loopback_test (struct rtl8168_private *tp)
{
  void __iomem *ioaddr = tp->mmio_addr;
  ...
  while (1)
    {
      ...
      pci_dma_sync_single_for_device (tp->pci_dev, le64_to_cpu (mapping), len,
                                      PCI_DMA_TODEVICE);
      ...
      RTL_W32 (RxConfig,
               RTL_R32 (RxConfig) & ~(AcceptErr | AcceptRunt | AcceptBroadcast
      ...
      if (rx_len == len)
        {
          pci_dma_sync_single_for_cpu (tp->pci_dev, le64_to_cpu (rxd->addr),
...
//                              dev_printk(KERN_INFO, &tp->pci_dev->dev, "loopback test finished\n",rx_len,len);
  ...
  tp->cur_tx++;
  tp->cur_rx++;
  pci_unmap_single (tp->pci_dev, le64_to_cpu (mapping), len,
                    PCI_DMA_TODEVICE);
...
static void
rtl8168_driver_start (struct rtl8168_private *tp)
{
  int timeout;
...
static void
rtl8168_driver_stop (struct rtl8168_private *tp)
{
  int timeout;
...
static void
rtl8168_ephy_write (void __iomem * ioaddr, int RegAddr, int value)
{
  int i;
...
static u16
rtl8168_ephy_read (void __iomem * ioaddr, int RegAddr)
{
  int i;
  u16 value = 0xffff;
...
static void
rtl8168_csi_write (void __iomem * ioaddr, int addr, int value)
{
  int i;
  ...
  RTL_W32 (CSIAR,
           CSIAR_Write |
           CSIAR_ByteEn << CSIAR_ByteEn_shift | (addr & CSIAR_Addr_Mask));
...
u32
rtl8168_eri_read (void __iomem * ioaddr, int addr, int len, int type)
{
...
int
rtl8168_eri_write (void __iomem * ioaddr, int addr, int len, u32 value,
                   int type)
{
...
static int
rtl8168_csi_read (void __iomem * ioaddr, int addr)
{
  int i, value = -1;
      ...
      while (RTL_R8 (TxPoll) & NPQ)
        {
          udelay (20);
           ...
           (tp->mcfg != CFG_METHOD_2) && (tp->mcfg != CFG_METHOD_3))
    {
...
rtl8168_check_link_status (struct net_device *dev,
                           struct rtl8168_private *tp, void __iomem * ioaddr)
{
  unsigned long flags;
...
static void
rtl8168_link_option (int idx, u8 * aut, u16 * spd, u8 * dup)
{
  ...
  if ((opt_speed == 0xff) | (opt_duplex == 0xff) | (opt_autoneg == 0xff))
    {
      *spd = SPEED_1000;
      *dup = DUPLEX_FULL;
      *aut = AUTONEG_ENABLE;
...
rtl8168_powerdown_pll (struct net_device *dev)
{
  ...
  if (tp->wol_enabled == WOL_ENABLED)
    {
      RTL_W32 (RxConfig,
               RTL_R32 (RxConfig) | AcceptBroadcast | AcceptMulticast |
               AcceptMyPhys);
      return;
    }
...
static void
rtl8168_get_wol (struct net_device *dev, struct ethtool_wolinfo *wol)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  void __iomem *ioaddr = tp->mmio_addr;
  u8 options;
...
static int
rtl8168_set_wol (struct net_device *dev, struct ethtool_wolinfo *wol)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  void __iomem *ioaddr = tp->mmio_addr;
  int i;
...
static void
rtl8168_get_drvinfo (struct net_device *dev, struct ethtool_drvinfo *info)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  ...
  if ((autoneg == AUTONEG_ENABLE) || (speed == SPEED_1000))
    {
      ...
      if ((speed == SPEED_10) && (duplex == DUPLEX_HALF))
        ...
        {
          auto_nego |= ADVERTISE_100HALF |
            ADVERTISE_100FULL | ADVERTISE_10HALF | ADVERTISE_10FULL;
        }
...
static int
rtl8168_set_speed (struct net_device *dev, u8 autoneg, u16 speed, u8 duplex)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  int ret;
...
static int
rtl8168_set_settings (struct net_device *dev, struct ethtool_cmd *cmd)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  unsigned long flags;
  int ret;
...
static int
rtl8168_set_tx_csum (struct net_device *dev, u32 data)
{
  if (data)
...
static int
rtl8168_set_rx_csum (struct net_device *dev, u32 data)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  void __iomem *ioaddr = tp->mmio_addr;
  unsigned long flags;
...
static inline u32
rtl8168_tx_vlan_tag (struct rtl8168_private *tp, struct sk_buff *skb)
{
...
static void
rtl8168_vlan_rx_register (struct net_device *dev, struct vlan_group *grp)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  void __iomem *ioaddr = tp->mmio_addr;
  unsigned long flags;
...
static void
rtl8168_vlan_rx_kill_vid (struct net_device *dev, unsigned short vid)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  unsigned long flags;
...
rtl8168_rx_vlan_skb (struct rtl8168_private *tp,
                     struct RxDesc *desc, struct sk_buff *skb)
{
...
static void
rtl8168_gset_xmii (struct net_device *dev, struct ethtool_cmd *cmd)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  void __iomem *ioaddr = tp->mmio_addr;
  u8 status;
...
static int
rtl8168_get_settings (struct net_device *dev, struct ethtool_cmd *cmd)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  unsigned long flags;
...
static void
rtl8168_get_regs (struct net_device *dev, struct ethtool_regs *regs, void *p)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  unsigned long flags;
...
static void
rtl8168_set_msglevel (struct net_device *dev, u32 value)
{
  struct rtl8168_private *tp = netdev_priv (dev);
...
static void
rtl8168_get_strings (struct net_device *dev, u32 stringset, u8 * data)
{
  switch (stringset)
    {
    case ETH_SS_STATS:
...
static int
rtl_get_eeprom_len (struct net_device *dev)
{
  struct rtl8168_private *tp = netdev_priv (dev);
...
static int
rtl_get_eeprom (struct net_device *dev, struct ethtool_eeprom *eeprom,
                u8 * buf)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  int i, j, ret;
...
u32
_kc_ethtool_op_get_link (struct net_device * dev)
{
...
u32
_kc_ethtool_op_get_sg (struct net_device * dev)
{
#ifdef NETIF_F_SG
...
int
_kc_ethtool_op_set_sg (struct net_device *dev, u32 data)
{
#ifdef NETIF_F_SG
  if (data)
...
#if 0
static int
rtl8168_enable_EEE (struct rtl8168_private *tp)
{
  void __iomem *ioaddr = tp->mmio_addr;
  int ret;
  unsigned long flags;
...
static int
rtl8168_disable_EEE (struct rtl8168_private *tp)
{
  void __iomem *ioaddr = tp->mmio_addr;
  int ret;
  unsigned long flags;
...
static int
rtl8168_enable_green_feature (struct rtl8168_private *tp)
{
  __u16 data;
  ...
  switch (tp->mcfg)
    ...
    default:
      dev_printk (KERN_DEBUG, &tp->pci_dev->dev,
                  "Not Support Green Feature\n");
      break;
    }
...
static int
rtl8168_disable_green_feature (struct rtl8168_private *tp)
{
  __u16 data;
...
static int
rtl8168_get_mac_version (struct rtl8168_private *tp, void __iomem * ioaddr)
{
  u32 reg, val32;
  u32 ICVerID;
...
static inline void
rtl8168_delete_esd_timer (struct net_device *dev, struct timer_list *timer)
{
  struct rtl8168_private *tp = netdev_priv (dev);
...
static inline void
rtl8168_request_esd_timer (struct net_device *dev)
{
  struct rtl8168_private *tp = netdev_priv (dev);
...
static inline void
rtl8168_delete_link_timer (struct net_device *dev, struct timer_list *timer)
{
  struct rtl8168_private *tp = netdev_priv (dev);
...
static inline void
rtl8168_request_link_timer (struct net_device *dev)
{
  struct rtl8168_private *tp = netdev_priv (dev);
 ...
 **/
static int
rtl8168_set_mac_address (struct net_device *dev, void *p)
{
  struct rtl8168_private *tp = netdev_priv (dev);
 ...
 *****************************************************************************/
void
rtl8168_rar_set (struct rtl8168_private *tp, uint8_t * addr, uint32_t index)
{
  void __iomem *ioaddr = tp->mmio_addr;
  uint32_t rar_low = 0;
  uint32_t rar_high = 0;
...
static int
ethtool_get_settings (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_set_settings (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_get_drvinfo (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_get_regs (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_get_wol (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_set_wol (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_get_msglevel (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_set_msglevel (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_nway_reset (struct net_device *dev)
{
...
static int
ethtool_get_link (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_get_eeprom (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_set_eeprom (struct net_device *dev, void *useraddr)
{
  struct ethtool_eeprom eeprom;
  struct ethtool_ops *ops = ethtool_ops;
  u8 *data;
...
static int
ethtool_get_coalesce (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_set_coalesce (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_get_ringparam (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_set_ringparam (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_get_pauseparam (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_set_pauseparam (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_get_rx_csum (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_set_rx_csum (struct net_device *dev, char *useraddr)
{
  struct ethtool_value edata;
...
static int
ethtool_get_tx_csum (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_set_tx_csum (struct net_device *dev, char *useraddr)
{
  struct ethtool_value edata;
...
static int
ethtool_get_sg (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_set_sg (struct net_device *dev, char *useraddr)
{
  struct ethtool_value edata;
...
static int
ethtool_get_tso (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_set_tso (struct net_device *dev, char *useraddr)
{
  struct ethtool_value edata;
...
static int
ethtool_self_test (struct net_device *dev, char *useraddr)
{
...
static int
ethtool_get_strings (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_phys_id (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_get_stats (struct net_device *dev, void *useraddr)
{
...
static int
ethtool_ioctl (struct ifreq *ifr)
{
...
static int
rtl8168_do_ioctl (struct net_device *dev, struct ifreq *ifr, int cmd)
{
  struct rtl8168_private *tp = netdev_priv (dev);
    ...
    case SIOCRTLTOOL:
      {
        ...
        if (copy_from_user
            (&my_cmd, ifr->ifr_data, sizeof (struct rtltool_cmd)))
          {
            ret = -EFAULT;
            break;
          }
        ...
        switch (my_cmd.cmd)
          ...
          case RTLTOOL_READ_MAC:
            ...
            if (copy_to_user
                (ifr->ifr_data, &my_cmd, sizeof (struct rtltool_cmd)))
              {
                ret = -EFAULT;
                break;
              }
...
rtl8168_init_board (struct pci_dev *pdev,
                    struct net_device **dev_out, void __iomem ** ioaddr_out)
{
  void __iomem *ioaddr;
...
rtl8168_esd_timer (unsigned long __opaque)
{
  ...
  if (io_base_h != tp->pci_cfg_space.io_base_h)
    {
      pci_write_config_word (pdev, PCI_BASE_ADDRESS_0 + 2,
...
static unsigned
rtl8168_try_msi (struct pci_dev *pdev, void __iomem * ioaddr)
{
  unsigned msi = 0;
...
static void
rtl8168_disable_msi (struct pci_dev *pdev, struct rtl8168_private *tp)
{
...
rtl8168_init_one (struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  printk (KERN_INFO
          "%s: This product is covered by one or more of the following patents: US5,307,459, US5,434,872, US5,732,094, US6,570,884, US6,115,776, and US6,327,625.\n",
          MODULENAME);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,11)
static void
rtl8168_shutdown (struct pci_dev *pdev)
{
  struct net_device *dev = pci_get_drvdata (pdev);
  struct rtl8168_private *tp = netdev_priv (dev);
...
static void
rtl8168_set_rxbufsize (struct rtl8168_private *tp, struct net_device *dev)
{
  void __iomem *ioaddr = tp->mmio_addr;
...
rtl8168_open (struct net_device *dev)
{
  ...
  retval =
    request_irq (dev->irq, rtl8168_interrupt,
                 (tp->features & RTL_FEATURE_MSI) ? 0 : SA_SHIRQ, dev->name,
                 dev);
  if (retval < 0)
    goto err_free_rx;
  ...
  if (tp->mcfg == CFG_METHOD_4)
    {
      ...
      RTL_W16 (CPlusCmd, RTL_R16 (CPlusCmd) &
               ~(EnableBist | Macdbgo_oe | Force_halfdup | Force_rxflow_en |
                 Force_txflow_en | Cxpl_dbg_sel | ASF | PktCntrDisable |
                 Macdbgo_sel));
...
//              RTL_W8(0xD3, RTL_R8(0xD3) | BIT_3 | BIT_2);
      ...
      RTL_W8 (0xF1,
              RTL_R8 (0xF1) | BIT_7 | BIT_6 | BIT_5 | BIT_4 | BIT_2 | BIT_1);
  ...
  if ((options1 & LinkUp) || (options1 & MagicPacket) || (options2 & UWF)
      || (options2 & BWF) || (options2 & MWF))
...
static int
rtl8168_change_mtu (struct net_device *dev, int new_mtu)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  int ret = 0;
...
static inline void
rtl8168_mark_to_asic (struct RxDesc *desc, u32 rx_buf_sz)
{
...
static inline void
rtl8168_map_to_asic (struct RxDesc *desc, dma_addr_t mapping, u32 rx_buf_sz)
{
  ...
  for (i = 0; i < NUM_RX_DESC; i++)
    {
      if (tp->Rx_skbuff[i])
        {
          rtl8168_free_rx_skb (tp, tp->Rx_skbuff + i, tp->RxDescArray + i);
        }
    }
}
  ...
  for (i = 0; i < NUM_RX_DESC; i++)
    {
      if (i == (NUM_RX_DESC - 1))
        tp->RxDescArray[i].opts1 =
          cpu_to_le32 ((ownbit | RingEnd) | (unsigned long) tp->rx_buf_sz);
      else
        tp->RxDescArray[i].opts1 =
          cpu_to_le32 (ownbit | (unsigned long) tp->rx_buf_sz);
    }
}
      ...
      if (len)
        {
          ...
          rtl8168_unmap_tx_skb (tp->pci_dev, tx_skb, tp->TxDescArray + entry);
          if (skb)
            {
              dev_kfree_skb (skb);
              tx_skb->skb = NULL;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20)
static void
rtl8168_schedule_work (struct net_device *dev, void (*task) (void *))
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,0)
  struct rtl8168_private *tp = netdev_priv (dev);
...
#else
static void
rtl8168_schedule_work (struct net_device *dev, work_func_t task)
{
  struct rtl8168_private *tp = netdev_priv (dev);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20)
static void
rtl8168_reinit_task (void *_data)
#else
static void
rtl8168_reinit_task (struct work_struct *work)
#endif
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20)
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20)
static void
rtl8168_reset_task (void *_data)
{
  struct net_device *dev = _data;
  struct rtl8168_private *tp = netdev_priv (dev);
#else
static void
rtl8168_reset_task (struct work_struct *work)
{
  struct rtl8168_private *tp =
    container_of (work, struct rtl8168_private, task.work);
  struct net_device *dev = tp->dev;
...
static inline u32
rtl8168_tso (struct sk_buff *skb, struct net_device *dev)
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,0)
...
static inline u32
rtl8168_tx_csum (struct sk_buff *skb, struct net_device *dev)
{
  struct rtl8168_private *tp = netdev_priv (dev);
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,22)
...
static int
rtl8168_start_xmit (struct sk_buff *skb, struct net_device *dev)
{
  struct rtl8168_private *tp = netdev_priv (dev);
  ...
  if (tp->dirty_tx != dirty_tx)
    {
      ...
      smp_wmb ();
      if (netif_queue_stopped (dev) && (TX_BUFFS_AVAIL (tp) >= MAX_SKB_FRAGS))
        {
          netif_wake_queue (dev);
        }
    }
...
rtl8168_try_rx_copy (struct sk_buff **sk_buff,
                     int pkt_size, struct RxDesc *desc, int rx_buf_sz)
{
  int ret = -1;
  ...
  for (; rx_left > 0; rx_left--, cur_rx++)
    {
      ...
      if (unlikely (status & RxRES))
      ...
      else
        {
          ...
          if (rtl8168_try_rx_copy (&skb, pkt_size, desc, tp->rx_buf_sz))
            {
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,19)
static irqreturn_t
rtl8168_interrupt (int irq, void *dev_instance, struct pt_regs *regs)
#else
static irqreturn_t
rtl8168_interrupt (int irq, void *dev_instance)
#endif
{
  ...
  u16 intr_clean_mask = SYSErr | PCSTimeout | SWInt |
    LinkChg | RxDescUnavail | TxErr | TxOK | RxErr | RxOK;
...
#ifdef CONFIG_R8168_NAPI
static int
rtl8168_poll (napi_ptr napi, napi_budget budget)
{
  struct rtl8168_private *tp = RTL_GET_PRIV (napi, struct rtl8168_private);
  void __iomem *ioaddr = tp->mmio_addr;
  RTL_GET_NETDEV (tp) unsigned int work_to_do = RTL_NAPI_QUOTA (budget, dev);
...
rtl8168_set_rx_mode (struct net_device *dev)
{
  ...
  tmp =
    tp->
    rtl8168_rx_config | rx_mode | (RTL_R32 (RxConfig) &
...
rtl8168_get_stats (struct net_device *dev)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,22)
  struct rtl8168_private *tp = netdev_priv (dev);
#endif
